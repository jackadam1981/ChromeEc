...
static uint32_t __wait_evt(int timeout_us, task_id_t resched)
{
	...
	if (timeout_us > 0) {
		...
		deprecated_atomic_clear_bit(&tsk->events, TASK_EVENT_TIMER);
	}
	return evt;
}
...
{
	deprecated_atomic_clear_bit(&tasks_enabled, BIT(tskid));
...
void mutex_lock(struct mutex *mtx)
{
	...
	deprecated_atomic_clear_bit(&mtx->waiters, id);
}
...
void mutex_unlock(struct mutex *mtx)
{
	...
	deprecated_atomic_clear_bit(&tsk->events, TASK_EVENT_MUTEX);
}
