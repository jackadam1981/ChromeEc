 ...
 *
 * Start off with only the hooks task (task ID: 1) marked as ready such that all
 * the modules can do their init within a task switching context.  The hooks
 * task will then make a call to enable all tasks.
 */
static uint32_t tasks_ready = 0x2;
