...
#define TASK_SCRATCHPAD_SIZE (29+33)
#else
...
#ifdef CONFIG_FPU
		sp[27+33] = tasks_init[i].a0;          /* a0 */
		sp[28+33] = (uint32_t)task_exit_trap;  /* ra */
		sp[12] = tasks_init[i].pc;             /* pc/mepc */
#else
		sp[27] = tasks_init[i].a0;          /* a0 */
		sp[28] = (uint32_t)task_exit_trap;  /* ra */
