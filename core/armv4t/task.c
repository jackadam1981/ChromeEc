...
{
	/* not in System mode */
	return (get_cpsr() & PSR_M_MASK) != 0x1F;
}

inline int get_interrupt_context(void)
{
	return get_cpsr() & PSR_M_MASK;
}
...
static void __nvic_init_irqs(void)
{
	...
	for (i = 0; i < exc_calls; i++) {
		...
		uint8_t prio __attribute__((unused)) = __irqprio[i].priority;
		uint32_t prio_shift __attribute__((unused)) = irq % 4 * 8 + 5;
	}
}
