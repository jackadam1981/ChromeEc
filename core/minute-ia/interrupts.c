...
static inline uint32_t get_current_vector(void)
{
	uint32_t vec, i;
	/* In service register */
	uint32_t *lapic_isr_reg = (uint32_t *)LAPIC_ISR_REG;

	/* Scan ISRs from highest priority */
	for (i = 7; i >= 0; i--, lapic_isr_reg -= 4) {
		vec = *lapic_isr_reg;
		if (vec) {
...
/* Should only be called in interrupt context */
unsigned int get_current_irq(void)
{
	return VEC_TO_IRQ(get_current_vector());
}
