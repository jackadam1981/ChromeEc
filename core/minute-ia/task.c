 ...
 */
#define MIA_TASK_FLAG_MASK		0x00000007
...
#undef MIA_ROUTINE_FPU
#define MIA_ROUTINE_FPU(r)		.pc = (uint32_t)r, \
					.flags = MIA_TASK_FLAG_USE_FPU,
#undef MIA_ROUTINE
#define MIA_ROUTINE(r)			.pc = (uint32_t)r,

/*
 * n : name
 * mia_r : program counter and flags
 * d : opaque parameter
 * s : stack size
 */
#define TASK(n, mia_r, d, s)  {	\
	.r0 = (uint32_t)d,	\
	.stack_size = s, \
	mia_r		\
},
static const struct {
	...
	TASK(IDLE, MIA_ROUTINE(__idle), 0, IDLE_TASK_STACK_SIZE)
	CONFIG_TASK_LIST
	CONFIG_TEST_TASK_LIST
};
