...
void task_trigger_irq(int irq)
{
	 ...
	 * so here, we use one vector(SOFTIRQ_VECTOR) which gets irq
	 * as an argment and the handler of the vector runs corresponding
	 * irq's ISR.  saving/restoring registers here makes SOFTIRQ_VECTOR
	 * handler simple. but due to this, we shouldn't have access to
	 * stack variable between pusha and popa.
	 */
