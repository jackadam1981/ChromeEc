 ...
 */
#define TASK_FLAG_MASK		0x00000007
#define TASK_FLAG_USE_FPU	0x00000001 /* task uses FPU */
...
#define TASK(n, r, d, s)  {	\
	.r0 = (uint32_t)d,	\
	.pc = (uint32_t)r,	\
	.stack_size = (s) & ~TASK_FLAG_MASK,	\
	.flags = (s) & TASK_FLAG_MASK \
},
static const struct {
	uint32_t r0;
	uint32_t pc;
...
void task_pre_init(void)
{
	...
	for (i = 0; i < TASK_ID_COUNT; i++) {
		...
		if (tasks_init[i].flags & TASK_FLAG_USE_FPU)
			tasks[i].use_fpu = 1;
#endif
