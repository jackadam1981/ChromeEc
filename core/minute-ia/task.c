...
void __idle(void)
{
	...
	/* Hold off HPET till the first task start */
	HPET_GENERAL_CONFIG 	|= HPET_ENABLE_CNF | HPET_LEGACY_RT_CNF;

	/* __idle is task 0, we invoke scheduler to start HOOK first */
	__schedule(0, 0);
...
task_ *current_task = (task_ *) scratchpad, *next_task;
/*
 ...
 * task unblocking.  After checking for a task switch, switch_handler() will clear
...
void __schedule(int desched, int resched)
{
	__asm__ __volatile__ ("" : : "d" (desched), "c" (resched));
	__asm__ ("int %0;" : : "i" (APIC_YIELD_INT_VECTOR));
}
...
void task_enable_all_tasks(void)
{
	...
	start_called = 1;
	CPUTS("*** MNG FW ready ****\n");
	REG32(IPC_ISH2PEER0_DOORBELL) = IPC_BUILD_MNG_MSG(0x8,1);
...
void task_trigger_irq(int irq)
{
	...
	case ISH30_HPET_TIMER0_IRQ:
		LAPIC_ICR_REG_ADDR = LAPIC_ICR_VECTOR | HPET_TIMER0_VEC;
		break;

	case ISH30_HPET_TIMER1_IRQ:
	case ISH30_HPET_TIMER2_IRQ:
		LAPIC_ICR_REG_ADDR = LAPIC_ICR_VECTOR | HPET_TIMER12_VEC;
		break;
...
void task_pre_init(void)
{
	...
	for (i = 0; i < TASK_ID_COUNT; i++) {
		 ...
		 * stack, plus 8 for task context. Even if using FPU, the first frame
	...
	*(uint32_t *)scratchpad = STACK_UNUSED_VALUE;
	...
	current_task = __task_id_to_ptr(0);
