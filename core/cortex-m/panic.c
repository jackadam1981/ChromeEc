...
static void panic_show_extra(const struct panic_data *pdata)
{
	...
	if (pdata->flags & PANIC_DATA_FLAG_FRAME_VALID) {
		/* ~3 for masking control bits
		 * 8*sizeof(uint32_t) for the exception frame */
		psp = (psp & ~3) + 8 * sizeof(uint32_t);
		for (i = 0; i < 16; i++) {
			if (psp + sizeof(uint32_t) >
			    CONFIG_RAM_BASE + CONFIG_RAM_SIZE)
				break;
			if (i % 4 == 0)
				panic_printf("%08x: ", psp);
			panic_printf("%08x%c", *(uint32_t *)psp,
				     i % 4 != 3 ? ' ' : '\n');
			psp += sizeof(uint32_t);
		}
		panic_printf("\n");
	} else {
		panic_printf("Invalid value in psp: %08x\n", psp);
	}
}
