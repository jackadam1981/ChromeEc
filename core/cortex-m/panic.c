...
/* Direct access to status and data registers of the panic UART */
struct uart_regs {
	uint32_t sr;
	uint32_t dr;
};
...
{
	struct uart_regs *regs = (struct uart_regs *)CONFIG_UART_ADDRESS;

	uart_emergency_flush();
	if (ch == '\n')
		panic_putc('\r');
	while (!(regs->sr & CONFIG_UART_SR_TXEMPTY))
		;
	regs->dr = ch;
	while (!(regs->sr & CONFIG_UART_SR_TXEMPTY))
		;
 ...
 *
 * We show fault regisetr information, including the fault address registers
 * if valid.
 */
