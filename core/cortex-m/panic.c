...
void __keep report_panic(void)
{
	...
	if (IS_ENABLED(CONFIG_RW_SAFE_MODE)) {
		/* TODO: check for nested panic */
		if (start_rw_safe_mode() == EC_SUCCESS) {
			/* Return from exception on process stack.
			 * We should not actually land in return_from_exception
			 * function. Instead the scheduler should scheduler
			 * should interrupt and schedule a different task since
			 * the current task has been disabled.
			 */
