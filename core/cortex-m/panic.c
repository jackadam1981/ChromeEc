...
int panic_cbor_fill_cortex_m(struct cortex_panic_data *cm_panic_data,
			     uint32_t *sp)
{
	PANIC_CBOR_LABEL(PANIC_CBOR_LABEL_REGISTERS);
	...
	const int stack_capture_count = 16;
	PANIC_CBOR_LABEL(PANIC_CBOR_LABEL_STACK);
	panic_cbor_list_start();
	for (int i = 0; i < stack_capture_count; i++, sp++) {
		// make sure sp is in bounds before dereferencing
		if ((uintptr_t)sp < CONFIG_RAM_BASE ||
		    (uintptr_t)sp > CONFIG_RAM_BASE + CONFIG_RAM_SIZE)
			break;
		int rv = panic_cbor_uint32(*sp);
		if (rv != EC_SUCCESS)
			break;
	}
	/* Close stack list */
