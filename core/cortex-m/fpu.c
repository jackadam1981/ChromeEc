...
void fpu_warn(void)
{
	...
	CPRINTF("\n### FPU exception at PC=%08x LR=%08x ", fpu_pc, fpu_lr);
	if (fpu_task == -1)
		CPUTS("(exc) ###\n");
	else
		CPRINTF("(task %d) ###\n", fpu_task);

	CPRINTF("### FPSCR=%08x => ", fpu_fpscr);
...
#endif /* CONFIG_FPU_WARNINGS */

	/* Clear FPSCR on stack */
	fpu_state[FPU_IDX_REG_FPSCR] &= ~0x8f;
	asm volatile("dmb");
}
