			   ...
			   uint8_t texscb)
{
	return mpu_config_region(region, addr, size,
				 MPU_ATTR_NX | MPU_ATTR_RW_RW | texscb, 1);
}
...
int mpu_protect_ram(void)
{
	int ret;
	ret = mpu_lock_region(
		0, CONFIG_RAM_BASE, CONFIG_RAM_SIZE, MPU_ATTR_INTERNAL_SRAM);
	if (ret != EC_SUCCESS)
		return ret;
	/* Allow execution from iram.text. Using higher region# allows us to
...
{
	return mpu_lock_region(1, CONFIG_FW_RO_OFF, CONFIG_FW_IMAGE_SIZE,
			       MPU_ATTR_FLASH_MEMORY);
}

int mpu_lock_rw_flash(void)
{
	return mpu_lock_region(1, CONFIG_FW_RW_OFF, CONFIG_FW_IMAGE_SIZE,
			       MPU_ATTR_FLASH_MEMORY);
}
