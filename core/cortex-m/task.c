...
void task_enable_resets(void)
{
...
#ifdef CR50_DEV
		cprints(CC_TASK,
			"task_enable_resets called from non-resettable task,"
			" id: %d", id);
#endif
		return;
	}
	...
	if (*state == TASK_RESET_STATE_UNLOCKED) {
		cprints(CC_TASK,
			"task_enable_resets called, but resets already "
			"enabled, task id: %d", id);
		return;
	}
...
#ifdef CR50_DEV
		cprints(CC_TASK,
			"task_disable_resets called from non-resettable task");
#endif
		return;
        }
