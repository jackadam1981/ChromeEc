...
static int command_test(int argc, char **argv)
{
	...
	if (argc == 4) {
		i = strtoi(argv[1], NULL, 10);
		t = strtoi(argv[2], NULL, 10);
		f = strtoi(argv[3], NULL, 10);

		ccprintf("Current Value: %d\nIf: %d then %d else %d\n",
			 val, i, t, f);

		r = atomic_cond_or(&val, i, t, f);
...
DECLARE_CONSOLE_COMMAND(aor, command_test,
			"[setmask]",
			"Print/set ready tasks");
...
DECLARE_CONSOLE_COMMAND(aclear, command_test2,
			"[setmask]",
			"Print/set ready tasks");
