 ...
 */
#if defined(CONFIG_FPU) && defined(CONIFG_FPU_STORE_CONTEXT)
/* R4-R11 plus LR. */
...
#elif defined(CONFIG_FPU) && defined(CONIFG_FPU_STORE_CONTEXT)
...
void task_pre_init(void)
{
	...
	for (i = 0; i < TASK_ID_COUNT; i++) {
		...
		/*
		 * Update stack used by first frame: 8 words for the normal
		 * stack, plus 8 for R4-R11. First frame does not use FPU,
		 * but does have a plus 1 for EXC_RETURN.
		 */
		sp = stack_next + ssize - 16 - 1;
#else
		/*
		 * Update stack used by first frame: 8 words for the normal
		 * stack, plus 8 for R4-R11.
		 */
