 ...
 * and if it matches if_value, updates the state to new_value.
 */
int update_reset_state(uint32_t *state, uint32_t if_value, uint32_t to_value)
{
	int update;
...
int task_reset(task_id_t id, int wait)
{
	...
	*state |= (resets_disabled ? waiter_id : TASK_RESET_LOCK);
	interrupt_enable();
