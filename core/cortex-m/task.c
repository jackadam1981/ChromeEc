...
static const struct {
	...
	TASK(IDLE, __idle, 0, TASK_STACK_SIZE)
	CONFIG_TASK_LIST
};
#undef TASK
...
{
	/*
	 * TODO: div is more expensive than mul, so might be better to track
	 * the current task id and index tasks[] each time than keep track of
	 * the task pointer each time and divide down to get the task_id.
	 */
...
int task_pre_init(void)
{
	...
	for (i = 0; i < TASK_ID_COUNT; i++) {
		...
		sp[8  /*r0*/ ] = tasks_init[i].r0;
		sp[13 /*lr*/ ] = (uint32_t)task_exit_trap;
		sp[14 /*pc*/ ] = tasks_init[i].pc;
		sp[15 /*psr*/] = 0x01000000;
