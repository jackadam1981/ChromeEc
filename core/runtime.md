...
- the [interrupt handlers](#interruptions)
...
They also a static fixed priority implicitly defined at compile-time by their
order in the [ec.tasklist](../board/host/ec.tasklist) file (the top-most one
being the lowest priority aka *task* *1*).
...
architecture-specific (e.g. 4 on Cortex-M0, on Cortex-M3/M4) and several
...
be seen as interruptions with a priority strictly greater than all IRQ vectors.
...
specific events.
The 19 least significant bits have a task-specific meaning.

Those event bits are used an inter-task communication and scheduling mechanism,
...
to masking interruptions with `interrupt_disable()` spinlocks, but it's
...
startup and interruptions/exceptions.
