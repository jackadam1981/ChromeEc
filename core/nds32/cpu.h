...
/* write Interruption Program Counter priviledged register */
static inline void set_ipc(uint32_t val)
{
	asm volatile ("mtsr %0, $IPC \n"::"r"(val));
}

/* read Interruption Program Counter priviledged register */
static inline uint32_t get_ipc(void)
{
	uint32_t ret;
	asm volatile ("mfsr %0, $IPC \n":"=r"(ret));
	return ret;
}
