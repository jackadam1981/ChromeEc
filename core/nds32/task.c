...
task_ *next_sched_task(void)
{
	...
	if (current_task != new_task) {
		...
		if (IS_ENABLED(CONFIG_PANIC_TRACE))
			panic_trace_add_1(PANIC_TRACE_TAG_TASK_SWITCH,
					  (new_task - tasks));
	}
#endif
...
void __ram_code task_set_event(task_id_t tskid, uint32_t event)
{
	...
	if (IS_ENABLED(CONFIG_PANIC_TRACE))
		panic_trace_add_2(PANIC_TRACE_TAG_TASK_SET_EVENT, tskid,
				  __builtin_ctz(event));
...
void __ram_code mutex_lock(struct mutex *mtx)
{
	...
	interrupt_disable();
	if (IS_ENABLED(CONFIG_PANIC_TRACE))
		panic_trace_add_1(PANIC_TRACE_TAG_MUTEX_LOCK,
				  task_get_current());
	mtx->waiters |= id;
	while (1) {
...
void __ram_code mutex_unlock(struct mutex *mtx)
{
	...
	if (IS_ENABLED(CONFIG_PANIC_TRACE))
		panic_trace_add_1(PANIC_TRACE_TAG_MUTEX_UNLOCK,
				  task_get_current());
	/*
