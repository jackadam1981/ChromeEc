...
/* USB charging control module for Asurada */
...
static const int usb_port_enable[] = {
	GPIO_EN_PP5000_USB_A0_VBUS,
};
BUILD_ASSERT(ARRAY_SIZE(usb_port_enable) == USB_PORT_COUNT);

static void usb_port_set_enabled(int port_id, bool en)
{
...
static void usb_port_all_ports_on(void)
{
	for (int i = 0; i < USB_PORT_COUNT; i++)
		usb_port_set_enabled(i, true);
}

static void usb_port_all_ports_off(void)
{
	for (int i = 0; i < USB_PORT_COUNT; i++)
		usb_port_set_enabled(i, false);
}
