...
#define PP3300_A_PGOOD_ADC_CHANNEL	9
#define PP3300_A_PGOOD_ON_THR_ID	0
#define PP3300_A_PGOOD_OFF_THR_ID	1

/* PP3300_A_PGOOD voltages in milivolts */
#define PP3300_A_PGOOD_ON_ADC_VOLTAGE	2700
#define PP3300_A_PGOOD_OFF_ADC_VOLTAGE	600

int pp3300_a_pgood = 0;
...
void pp3300_a_pgood_on_cb(void)
{
	atomic_or(&pp3300_a_pgood, 1);

	/* Disable this interrupt while it's asserted. */
	adc_disable_threshold_interrupt(PP3300_A_PGOOD_ON_THR_ID);
	/* Enable the voltage low interrupt. */
	adc_enable_threshold_interrupt(PP3300_A_PGOOD_OFF_THR_ID);
...
void pp3300_a_pgood_off_cb(void)
{
	atomic_clear_bits(&pp3300_a_pgood, 1);

	/* Disable this interrupt while it's asserted. */
	adc_disable_threshold_interrupt(PP3300_A_PGOOD_OFF_THR_ID);
	/* Enable the voltage high interrupt. */
	adc_enable_threshold_interrupt(PP3300_A_PGOOD_ON_THR_ID);
...
static int baseboard_init(const struct device *device)
{
	...
	cfg.channel_id = PP3300_A_PGOOD_ADC_CHANNEL;
	cfg.threshold_id = PP3300_A_PGOOD_ON_THR_ID;
	cfg.value = PP3300_A_PGOOD_ON_ADC_VOLTAGE;
	cfg.mode = ADC_THR_MODE_RISE;
	cfg.adc_threshold_cb = pp3300_a_pgood_on_cb;
	adc_config_threshold_interrupt(&cfg);
	adc_enable_threshold_interrupt(PP3300_A_PGOOD_ON_THR_ID);

	cfg.threshold_id = PP3300_A_PGOOD_OFF_THR_ID;
	cfg.value = PP3300_A_PGOOD_OFF_ADC_VOLTAGE;
	cfg.mode = ADC_THR_MODE_FALL;
	cfg.adc_threshold_cb = pp3300_a_pgood_off_cb;
	adc_config_threshold_interrupt(&cfg);
	adc_enable_threshold_interrupt(PP3300_A_PGOOD_OFF_THR_ID);
