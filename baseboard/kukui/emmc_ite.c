...
static void emmc_send_data_over_spi(uint32_t *tx, int tx_size, int rst_tx)
{
	int i;
...
static void emmc_bootblock_transfer(void)
{
	...
	for (i = 0; i < remaining; i += byte_sended) {
		 ...
		 */
		if (!i) {
			emmc_send_data_over_spi(&raw[raw_idx], 256, 1);
			raw_idx += (256 / 4);
			byte_sended = 256;
			continue;
		}
		...
		emmc_send_data_over_spi(&raw[raw_idx], 128, 0);
		raw_idx += (128 / 4);
		byte_sended = 128;
	}
}
