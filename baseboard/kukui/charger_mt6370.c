/* Copyright 2019 The Chromium OS Authors. All rights reserved.
 * Use of this source code is governed by a BSD-style license that can be
 * found in the LICENSE file.
 */

#include "charge_manager.h"
#include "charge_state_v2.h"
#include "charger_mt6370.h"
#include "console.h"
#include "driver/charger/rt946x.h"
#include "driver/tcpm/mt6370.h"
#include "hooks.h"
#include "math_util.h"
#include "power.h"
#include "timer.h"
#include "usb_pd.h"
#include "util.h"

#define BAT_LEVEL_PD_LIMIT 85

#define CPRINTS(format, args...) cprints(CC_CHARGER, format, ## args)
#define CPRINTF(format, args...) cprintf(CC_CHARGER, format, ## args)

#ifndef CONFIG_BATTERY_SMART
int board_cut_off_battery(void)
{
	/* The cut-off procedure is recommended by Richtek. b/116682788 */
	rt946x_por_reset();
	mt6370_vconn_discharge(0);
	rt946x_cutoff_battery();

	return EC_SUCCESS;
}
#endif

/* Dynamicly change current based on battery's desired watt */
static void battery_desired_curr_dynamic(struct charge_state_data *curr)
{
	static timestamp_t wait_deadline;
	static int prev_batt_stable_current;
	static int prev_requested_current;
	static int prev_supply_voltage;
	static int prev_in_cv;
	int active_port, supply_voltage;

	if (curr->state != ST_CHARGE) {
		wait_deadline.val = 0;
		prev_in_cv = prev_batt_stable_current = prev_supply_voltage = 0;
		return;
	}

	/* If it's not in CV stage, reset the status. */
	if (curr->batt.state_of_charge <= 75 || curr->batt.current < 0) {
		wait_deadline.val = 0;
		prev_in_cv = prev_batt_stable_current = 0;
		return;
	}

	/* We are in CV stage */
	/* We haven't waited for the current stable, wait for it */
	if (!prev_in_cv) {
		prev_in_cv = 1;
		wait_deadline.val = get_time().val + (10 * SECOND);
		goto chg_override_exit;
	} else if (!timestamp_expired(wait_deadline, NULL)) {
		/* If it hasn't stable, don't alter the requested current. */
		curr->requested_current =
			MIN(prev_requested_current, curr->requested_current);
		goto chg_override_exit;
	}

	/* wait ending, we are at stable current stage. */
	active_port = charge_manager_get_active_charge_port();

	if (active_port == CHARGE_PORT_NONE)
		return;

	supply_voltage = pd_get_supply_voltage(active_port);

	/* mt6370 workaround: */
	if (supply_voltage == 5000 && prev_supply_voltage > supply_voltage &&
	    prev_batt_stable_current - curr->batt.current > 300 &&
	    curr->batt.desired_voltage - curr->batt.voltage <= 500) {
		/* Raise perfer voltage above 5000mV. */
		pd_set_prefer_voltage(6000);
		curr->requested_current =
			MIN(prev_batt_stable_current, curr->requested_current);
		/* Let's wait for 50 seconds and see */
		wait_deadline.val = get_time().val + (50 * SECOND);
	} else {
		prev_batt_stable_current = curr->batt.current;
		pd_set_prefer_voltage(5000);
		/*
		 * Battery current should be stable, request extra 200mA for
		 * desired current to ensure it could request more than
		 * desired if needed.
		 */
		curr->requested_current = MIN(curr->batt.current + 200,
					      curr->requested_current);
		wait_deadline.val = get_time().val + (10 * SECOND);
	}

	/* Update the supply voltage at this stable stage */
	prev_supply_voltage = supply_voltage;

chg_override_exit:
	prev_requested_current = curr->requested_current;
}

void mt6370_charger_profile_override(struct charge_state_data *curr)
{
	static int previous_chg_limit_mv;
	int chg_limit_mv = pd_get_max_voltage();

	battery_desired_curr_dynamic(curr);

	/* Limit input (=VBUS) to 5V when soc > 85% and charge current < 1A. */
	if (!(curr->batt.flags & BATT_FLAG_BAD_CURRENT) &&
	    charge_get_percent() > BAT_LEVEL_PD_LIMIT &&
	    curr->batt.current < 1000 && power_get_state() != POWER_S0)
		chg_limit_mv = 5500;
	else
		chg_limit_mv = PD_MAX_VOLTAGE_MV;

	if (chg_limit_mv != previous_chg_limit_mv)
		CPRINTS("VBUS limited to %dmV", chg_limit_mv);
	previous_chg_limit_mv = chg_limit_mv;

	/* Pull down VBUS */
	if (pd_get_max_voltage() != chg_limit_mv)
		pd_set_external_voltage_limit(0, chg_limit_mv);

	/*
	 * When the charger says it's done charging, even if fuel gauge says
	 * SOC < BATTERY_LEVEL_NEAR_FULL, we'll overwrite SOC with
	 * BATTERY_LEVEL_NEAR_FULL. So we can ensure both Chrome OS UI
	 * and battery LED indicate full charge.
	 *
	 * Enable this hack on on-board gauge only (b/142097561)
	 */
	if (IS_ENABLED(CONFIG_BATTERY_MAX17055) && rt946x_is_charge_done()) {
		curr->batt.state_of_charge = MAX(BATTERY_LEVEL_NEAR_FULL,
						 curr->batt.state_of_charge);
	}

}

static void board_charge_termination(void)
{
	static uint8_t te;
	/* Enable charge termination when we are sure battery is present. */
	if (!te && battery_is_present() == BP_YES) {
		if (!rt946x_enable_charge_termination(1))
			te = 1;
	}
}
DECLARE_HOOK(HOOK_BATTERY_SOC_CHANGE,
	     board_charge_termination,
	     HOOK_PRIO_DEFAULT);

void board_set_charge_limit(int port, int supplier, int charge_ma,
			    int max_ma, int charge_mv)
{
	charge_set_input_current_limit(
		MAX(charge_ma, CONFIG_CHARGER_INPUT_CURRENT), charge_mv);
}

__override int board_get_desired_mw(void)
{
	return PLT_SHIFT_MW + charge_get_desired_mw();
}
