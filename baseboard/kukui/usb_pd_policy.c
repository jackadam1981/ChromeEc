...
{
	/* Only allow Vconn swap if battery exists.  */
...
static void board_usb_mux_set(int port, enum typec_mux mux_mode,
...
__override void svdm_safe_dp_mode(int port)
{
	...
	dp_flags[port] = 0;
	dp_status[port] = 0;
	board_usb_mux_set(port, TYPEC_MUX_NONE, USB_SWITCH_CONNECT,
