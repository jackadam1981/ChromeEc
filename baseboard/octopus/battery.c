...
BUILD_ASSERT(ARRAY_SIZE(brd_bat_info) == BATTERY_TYPE_COUNT);
...
static int board_get_battery_type(void)
{
	 ...
	 * manufacture names defined in the brd_bat_info table above. If a
	 * device name has been specified in the brd_bat_info table, then both
	 * the manufacture and device name must match.
	 */
	for (i = 0; i < BATTERY_TYPE_COUNT; i++) {
		if (strcasecmp(manu_name,
			brd_bat_info[i].fuel_gauge.manuf_name))
			continue;

		if (brd_bat_info[i].fuel_gauge.device_name == NULL) {
		...
		if (strcasecmp(device_name, brd_bat_info[i].fuel_gauge.device_name))
			continue;

		board_battery_type = i;
		CPRINTS("found batt:%s",
			brd_bat_info[board_battery_type].fuel_gauge.manuf_name);
		break;
 ...
 *
 * Very first battery brd_bat_info is called by the charger driver to initialize
...
static inline const struct board_batt_params *board_get_batt_params(void)
{
	...
	return &brd_bat_info[type == BATTERY_TYPE_COUNT ?
		     DEFAULT_3S_BATTERY_TYPE : type];
}

const struct brd_bat_info *battery_get_info(void)
{
...
int board_cut_off_battery(void)
{
	...
	cmd = brd_bat_info[type].fuel_gauge.ship_mode.reg_addr;
	data = brd_bat_info[type].fuel_gauge.ship_mode.reg_data[0];
	rv = sb_write(cmd, data);
	if (rv != EC_SUCCESS)
		return EC_RES_ERROR;

	data = brd_bat_info[type].fuel_gauge.ship_mode.reg_data[1];
	rv = sb_write(cmd, data);
...
int charger_profile_override(struct charge_state_data *curr)
{
	 ...
	 */
	if (type != BATTERY_TYPE_COUNT &&
	    brd_bat_info[type].fuel_gauge.override_nil) {
		int v = brd_bat_info[type].batt_info.voltage_max;
		int i = brd_bat_info[type].batt_info.precharge_current;
...
static int battery_check_disconnect(void)
{
	...
	if (brd_bat_info[type].fuel_gauge.fet.mfgacc_support == 1) {
		rv = sb_read_mfgacc(PARAM_OPERATION_STATUS,
				SB_ALT_MANUFACTURER_ACCESS, data, sizeof(data));
		/* Get the lowest 16bits of the OperationStatus() data */
		reg = data[2] | data[3] << 8;
	} else
		rv = sb_read(brd_bat_info[type].fuel_gauge.fet.reg_addr,
					&reg);
	...
	CPRINTS("Battery FET: reg 0x%04x mask 0x%04x disc 0x%04x", reg,
		brd_bat_info[type].fuel_gauge.fet.reg_mask,
		brd_bat_info[type].fuel_gauge.fet.disconnect_val);
	reg &= brd_bat_info[type].fuel_gauge.fet.reg_mask;
	if (reg == brd_bat_info[type].fuel_gauge.fet.disconnect_val)
