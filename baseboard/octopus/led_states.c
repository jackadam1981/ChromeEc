...
static void led_update_battery(void)
{
	...
	if (period == 0) {
		cprints(CC_GPIO, "Undefined LED behavior for charger state %d,"
...
{
	enum pwr_led_states new_state = PWR_LED_NUM_STATES;

	switch (power_get_state()) {
	case POWER_S0:
		new_state = PWR_LED_STATE_S0;
		break;
	case POWER_S3:
...
#endif
		if (extpower_is_present())
			new_state = PWR_LED_STATE_S3_CHARGE;
		else
			new_state = PWR_LED_STATE_S3_DISCHARGE;
		break;
	case POWER_S5:
		new_state = PWR_LED_STATE_S5;
		break;
	default:
		/* Transition states shouldn't affect LED behavior */
		break;
	}
...
static void led_update_power(void)
{
	...
	/* If this state is undefined, turn the LED off */
	if (period == 0) {
		cprints(CC_GPIO, "Undefined LED behavior for power state %d,"
			"turning off LED", led_state);
