...
/* Common code for VARIANT_OCTOPUS_USBC_STANDALONE_TCPCS_SEC configuration */
...
#define USB_PD_PORT_PS8751_0	0
#define USB_PD_PORT_PS8751_1	1

/******************************************************************************/
/* USB-C TPCP Configuration */
const struct tcpc_config_t tcpc_config[CONFIG_USB_PD_PORT_COUNT] = {
	[USB_PD_PORT_PS8751_0] = {
		.i2c_host_port = I2C_PORT_TCPC0,
		.i2c_slave_addr = PS8751_I2C_ADDR1,
		.drv = &ps8xxx_tcpm_drv,
		.pol = TCPC_ALERT_ACTIVE_LOW,
	},
	[USB_PD_PORT_PS8751_1] = {
...
void tcpc_alert_event(enum gpio_signal signal)
{
	...
	if ((signal == GPIO_USB_C0_MUX_INT_ODL) &&
	    !gpio_get_level(GPIO_USB_C0_PD_RST_ODL))
		return;
	else if ((signal == GPIO_USB_C1_MUX_INT_ODL) &&
...
static void set_ps8751_gpio3(int port, int enable)
{
	int rv, reg, i2c_port;

	if(port == USB_PD_PORT_PS8751_1)
		i2c_port = I2C_PORT_TCPC1;
	else
		i2c_port = I2C_PORT_TCPC0;
	/*
	 ...
	 */
	pd_prevent_low_power_mode(port, 1);

	/* Enable debug page access */
	rv = tcpc_write(port, PS8XXX_REG_I2C_DEBUGGING_ENABLE,
			0x30);
	if (rv)
		goto error;

	/* Enable GPIO3 (bit3) output by setting to bit3 to 1 */
	rv = i2c_read8(i2c_port, PS8751_DEBUG_ADDR, PS8751_GPIO_ENABLE,
		       &reg);
	if (rv)
		goto error;
	...
	if (!(reg & PS8751_GPIO3_VAL)) {
		...
		rv = i2c_write8(i2c_port, PS8751_DEBUG_ADDR,
	...
	rv = i2c_read8(i2c_port, PS8751_DEBUG_ADDR, PS8751_GPIO_LVL,
		       &reg);
	if (rv)
		goto error;
...
error:
	...
	tcpc_write(port, PS8XXX_REG_I2C_DEBUGGING_ENABLE, 0x31);
	pd_prevent_low_power_mode(port, 0);
}
...
{
	set_ps8751_gpio3(USB_PD_PORT_PS8751_0, 1);
	set_ps8751_gpio3(USB_PD_PORT_PS8751_1, 1);
}
...
{
	set_ps8751_gpio3(USB_PD_PORT_PS8751_0, 0);
	set_ps8751_gpio3(USB_PD_PORT_PS8751_1, 0);
}
