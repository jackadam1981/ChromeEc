...
static void set_ps8751_gpio3(int enable)
{
	 ...
	 */
	pd_prevent_low_power_mode(USB_PD_PORT_PS8751_1, 1);

	/* Enable debug page access */
	rv = tcpc_write(USB_PD_PORT_PS8751_1, PS8XXX_REG_I2C_DEBUGGING_ENABLE,
			0x30);
	if (rv)
		goto error;
...
error:
	...
	tcpc_write(USB_PD_PORT_PS8751_1, PS8XXX_REG_I2C_DEBUGGING_ENABLE, 0x31);
	pd_prevent_low_power_mode(USB_PD_PORT_PS8751_1, 0);
}
