...
/**
 * Hang fallback recovery routines.
 *
 * Only runs in RW to to de-risk causing an unrecoverable boot loop in RO.
 ...
 * fallback recovery remedies are added here.
 */
...
static void board_s0ix_hang_reset(void)
{
	...
	chipset_reset(EC_HOST_EVENT_HANG_REBOOT);
...
{
	ccprints(
		"S0ix hang persisted after ap reset. Attempting forced shutdown now!");
	chipset_force_shutdown(CHIPSET_SHUTDOWN_BOARD_CUSTOM);
}

static void board_handle_chipset_suspend_complete(void)
{
	/* Turn off hang timers if suspend completes */
	hook_call_deferred(&board_s0ix_hang_reset_data, -1);
	hook_call_deferred(&board_s0ix_hang_shutdown_data, -1);
}
DECLARE_HOOK(HOOK_CHIPSET_SUSPEND, board_handle_chipset_suspend_complete,
	     HOOK_PRIO_DEFAULT);

static void board_handle_chipset_resume_complete(void)
{
	/* Turn off hang timers if resume completes */
	hook_call_deferred(&board_s0ix_hang_reset_data, -1);
	hook_call_deferred(&board_s0ix_hang_shutdown_data, -1);
}
DECLARE_HOOK(HOOK_CHIPSET_RESUME, board_handle_chipset_resume_complete,
	     HOOK_PRIO_DEFAULT);

static void board_handle_chipset_reset(void)
{
	/* Turn off hang timers if ap resets */
	hook_call_deferred(&board_s0ix_hang_reset_data, -1);
	hook_call_deferred(&board_s0ix_hang_shutdown_data, -1);
}
DECLARE_HOOK(HOOK_CHIPSET_RESET, board_handle_chipset_reset,
	     HOOK_PRIO_DEFAULT);
