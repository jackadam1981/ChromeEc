...
/* PS8811 is just a retimer. We use the mux structure for convience */
...
static int baseboard_a1_anx7491_retimer_init(const struct usb_mux *me)
{
	...
	do {
		...
		rv = i2c_read8(me->i2c_port, me->i2c_addr_flags, 0,
			       &val);
	} while (rv && --tries);
	if (rv) {
...
void baseboard_a1_retimer_setup(void)
{
	...
	case USB_A1_RETIMER_ANX7491:
		usba1_anx7491.board_init(&usba1_anx7491);
		break;
	case USB_A1_RETIMER_PS8811:
		usba1_ps8811.board_init(&usba1_ps8811);
		break;
	default:
