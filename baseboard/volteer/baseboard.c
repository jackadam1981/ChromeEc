...
void board_reset_pd_mcu(void)
{
	...
	if (fw_config.usb_db == USB_DB_USB3_ACTIVE) {
		ps8815_reset();
...
{
	uint32_t cbi_val;
	union volteer_cbi_fw_config fw_config;
	...
	case USB_DB_USB4_GEN3:
		...
		break;
	case USB_DB_USB3_ACTIVE:
		config_db_usb3();
		CPRINTS("%sUSB3 Active", db_type_prefix);
		break;
	default:
		CPRINTS("%sID %d not supported", db_type_prefix,
			fw_config.usb_db);
	}
}
