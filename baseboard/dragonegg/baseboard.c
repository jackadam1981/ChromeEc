...
static void board_dragonegg_hpd_update(int port, int hpd_lvl, int hpd_irq)
{
	...
	static const enum gpio_signal hpd_gpio[CONFIG_USB_PD_PORT_COUNT] = {
		[USB_PD_PORT_ITE_0] = GPIO_EC_USB_C0_HPD,
		[USB_PD_PORT_ITE_1] = GPIO_EC_USB_C1_HPD,
		[USB_PD_PORT_TUSB422_2] = GPIO_EC_USB_C2_HPD,
	};

	enum gpio_signal gpio = hpd_gpio[port];

	/* Update the virtual mux about hpd */
	virtual_hpd_update(port, hpd_lvl, hpd_irq);
