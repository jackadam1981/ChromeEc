...
static struct espi_callback __oob_cb;

/* Callback for PUT_OOB message received */
static void __espi_oob_handler(const struct device *dev,
			       struct espi_callback *cb,
			       struct espi_event event)
{
	int ret;

	printk("=== Callback: %s, evt_details(data bytes:%d) ===\n",
		__func__, event.evt_details);

	resp_pckt.buf = (uint8_t *)&buf;
	resp_pckt.len = event.evt_details;
...
#ifdef CONFIG_ESPI_OOB_CHANNEL_RX_ASYNC
	espi_init_callback(&__oob_cb, __espi_oob_handler, ESPI_BUS_EVENT_OOB_RECEIVED);
	espi_add_callback(DEVICE_DT_GET(DT_CHOSEN(cros_ec_espi)), &__oob_cb);
#endif
...
/* Callback for PUT_OOB message received */
static void espi_oob_handler(const struct device *dev,
			       struct espi_callback *cb,
			       struct espi_event event)
{
	int ret;
	...
	//resp_pckt.len = MAX_ESPI_BUF_LEN;
	/* +3: eSPI oob cycle type, tag, and length fields. */
	resp_pckt.len = event.evt_details + 3;
...
#ifdef CONFIG_ESPI_OOB_CHANNEL_RX_ASYNC
	espi_init_callback(&oob_cb, espi_oob_handler, ESPI_BUS_EVENT_OOB_RECEIVED);
	espi_add_callback(DEVICE_DT_GET(DT_CHOSEN(cros_ec_espi)), &oob_cb);
#endif
