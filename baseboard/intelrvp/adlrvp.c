...
const struct bb_usb_control bb_controls[] = {
	[TYPE_C_PORT_0] = {
		.retimer_rst_gpio = (enum gpio_signal)IOEX_USB_C0_BB_RETIMER_RST,
		.usb_ls_en_gpio = (enum gpio_signal)IOEX_USB_C0_BB_RETIMER_LS_EN,
	},
#if defined(HAS_TASK_PD_C1)
	[TYPE_C_PORT_1] = {
		.retimer_rst_gpio = (enum gpio_signal)IOEX_USB_C1_BB_RETIMER_RST,
		.usb_ls_en_gpio = (enum gpio_signal)IOEX_USB_C1_BB_RETIMER_LS_EN,
	},
#endif
#if defined(HAS_TASK_PD_C2)
	[TYPE_C_PORT_2] = {
		.retimer_rst_gpio = (enum gpio_signal)IOEX_USB_C2_BB_RETIMER_RST,
		.usb_ls_en_gpio = (enum gpio_signal)IOEX_USB_C2_BB_RETIMER_LS_EN,
	},
#endif
#if defined(HAS_TASK_PD_C3)
	[TYPE_C_PORT_3] = {
		...
		.usb_ls_en_gpio = (enum gpio_signal)IOEX_USB_C3_BB_RETIMER_LS_EN,
	},
#endif
};
...
__override int bb_retimer_power_enable(const struct usb_mux *me, bool enable)
{
	...
	if (enable) {
		gpio_or_ioex_set_level(bb_controls[me->usb_port].usb_ls_en_gpio,
				       1);
		...
		gpio_or_ioex_set_level(
			bb_controls[me->usb_port].retimer_rst_gpio, 1);
		...
		gpio_or_ioex_set_level(
			bb_controls[me->usb_port].retimer_rst_gpio, 0);
		msleep(1);
		gpio_or_ioex_set_level(bb_controls[me->usb_port].usb_ls_en_gpio,
				       0);
	}
