...
bool is_typec_port(int port)
{
	...
	return !(port == charge_manager_get_dedicated_charge_port() ||
...
static void board_charge_init(void)
{
	...
	for (port = 0; port < charge_manager_get_port_count(); port++) {
...
int board_set_active_charge_port(int port)
{
	 ...
	 */
	if (port != charge_manager_get_dedicated_charge_port() &&
