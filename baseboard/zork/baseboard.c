 ...
 */
static mux_state_t saved_polarity_state = TYPEC_MUX_NONE;
...
static int zork_c1_set_mux(int port, mux_state_t mux_state)
{
	...
	saved_polarity_state = TYPEC_MUX_NONE;
	if (zork_c1_retimer == C1_RETIMER_PS8802) {
		saved_polarity_state = MUX_POLARITY_INVERTED;
...
static int zork_c1_get_mux(int port, mux_state_t *mux_state)
{
	...
	*mux_state |= saved_polarity_state;
