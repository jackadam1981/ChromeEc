 ...
 */
static int ps8802_tune_mux(struct usb_mux *this, mux_state_t mux_state)
{
	int rv = EC_SUCCESS;
	...
	if (mux_state & USB_PD_MUX_USB_ENABLED) {
		/* Boost the USB gain */
		rv = ps8802_i2c_field_update16(this,
					PS8802_REG_PAGE2,
	...
	if (mux_state & USB_PD_MUX_DP_ENABLED) {
		/* Boost the DP gain */
		rv = ps8802_i2c_field_update8(this,
					PS8802_REG_PAGE2,
 ...
 * Adds in board specific gain and DP lane count configuration
 */
static int ps8818_tune_mux(struct usb_mux *this, mux_state_t mux_state)
{
	int rv = EC_SUCCESS;
	...
	if (mux_state & USB_PD_MUX_USB_ENABLED) {
		/* Boost the USB gain */
		rv = ps8818_i2c_field_update8(this,
					PS8818_REG_PAGE1,
...
extern struct usb_mux usbc1_ps8818;
extern struct usb_mux usbc1_amd_fp5_usb_mux;

enum zork_c1_retimer zork_c1_retimer = C1_RETIMER_UNKNOWN;
static int zork_c1_detect(struct usb_mux *this, int err_if_power_off)
{
	int rv;
	...
	if (rv == EC_SUCCESS) {
		...
		usb_muxes[USBC_PORT_C1] = &usbc1_amd_fp5_usb_mux;
		usbc1_amd_fp5_usb_mux.next_mux = &usbc1_ps8818;
		return rv;
	}
	...
	if (rv == EC_SUCCESS) {
		...
		usb_muxes[USBC_PORT_C1] = &usbc1_ps8802;
		usbc1_ps8802.next_mux = &usbc1_amd_fp5_usb_mux;
		usbc1_amd_fp5_usb_mux.exclude_state =
 ...
 */
static int zork_c1_init_mux(struct usb_mux *this)
{
	int rv;
	const int port = this->usb_port;

	/* Try to detect, but don't give an error if no power */
	rv = zork_c1_detect(this, 0);
	if (rv)
		return rv;
...
static int zork_c1_set_mux(struct usb_mux *this, mux_state_t mux_state)
{
	int rv;
	const int port = this->usb_port;
	 ...
	 */
	rv = zork_c1_detect(this, mux_state != USB_PD_MUX_NONE);
	if (rv)
		return rv;
	 ...
	 */
	if (zork_c1_retimer != C1_RETIMER_UNKNOWN) {
		struct usb_mux *mux_ptr = usb_muxes[port];
		...
		for (; mux_ptr; mux_ptr = mux_ptr->next_mux) {
			...
			if (drv && drv->set) {
				...
				state = mux_state & ~this->exclude_state;
				rv = drv->set(mux_ptr, state);
				if (rv)
					break;
			}
		}
...
static int zork_c1_get_mux(struct usb_mux *this, mux_state_t *mux_state)
{
	int rv;
	const int port = this->usb_port;

	/* Try to detect the hardware */
	rv = zork_c1_detect(this, 1);
	if (rv) {
		/*
...
struct usb_mux usbc0_pi3dpx1207_usb_retimer = {
	.usb_port = USBC_PORT_C0,
	.i2c_port = I2C_PORT_TCPC0,
	.i2c_addr_flags = PI3DPX1207_I2C_ADDR_FLAGS,
	.driver = &pi3dpx1207_usb_retimer,
};
struct usb_mux usbc0_amd_fp5_usb_mux = {
	.usb_port = USBC_PORT_C0,
...
struct usb_mux zork_c1_usb_mux = {
	/*
...
struct usb_mux *usb_muxes[] = {
	[USBC_PORT_C0] = &usbc0_amd_fp5_usb_mux,
	[USBC_PORT_C1] = &zork_c1_usb_mux,
};
