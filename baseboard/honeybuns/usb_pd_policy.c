...
void pd_power_supply_reset(int port)
{
	...
	if (IS_ENABLED(BOARD_C1_NO_PPC) && port) {
		prev_en = gingerbread_c1_is_sourcing_vbus(port);
		/* Disable VBUS via PPC. */
		gingerbread_c1_vbus_source_enable(port, 0);
	} else {
...
int pd_set_power_supply_ready(int port)
{
	 ...
	 */
	if (IS_ENABLED(BOARD_C1_NO_PPC) && port)
		rv = gingerbread_c1_vbus_source_enable(port, 1);
	else
...
{
	if (IS_ENABLED(BOARD_C1_NO_PPC) && port)
		return gingerbread_c1_is_vbus_present(port);
	else
...
{
	if (IS_ENABLED(BOARD_C1_NO_PPC) && port)
		return gingerbread_c1_is_sourcing_vbus(port);
	else
