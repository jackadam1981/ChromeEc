...
#ifdef SECTION_IS_RW
#define PS8805_GPIO_CONTROL_I2C_ADDR 0x1A
#define PS8805_GPIO_CONTROL_OFFSET 0x21
#define PS8805_GPIO1 BIT(5)
int gingerbread_c1_is_vbus_present(int port)
{
	int vbus;
...
int gingerbread_c1_is_sourcing_vbus(int port)
{
	int reg;
	int rv;

	rv = i2c_read8(tcpc_config[port].i2c_info.port,
		       PS8805_GPIO_CONTROL_I2C_ADDR,
		       PS8805_GPIO_CONTROL_OFFSET, &reg);
	if (rv)
		return 0;

	return reg & PS8805_GPIO1;
}


int gingerbread_c1_vbus_source_enable(int port, int enable)
{
	int reg;
	int rv;
