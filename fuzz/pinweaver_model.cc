...
size_t PinweaverModel::GetMetadata(
  ...
  if (itr == leaf_metadata_.end()) {
    ...
    // because there are an unimported_leaf_data fields in the TryAuth and
...
size_t PinweaverModel::SerializeInsertLeaf(
  ...
  fuzz::CopyWithPadding(fuzzer_data.delay_schedule(),
                        fuzz::ToSpan<delay_schedule_entry_t,
                                     sizeof(req_data->delay_schedule),
                                     uint8_t>(req_data->delay_schedule), 0);
...
void PinweaverModel::UpdateMetadata(
  ...
  if (unimported_leaf_data) {
    ...
    stored_leaf_data.wrapped_data.assign(data, data + length);
...
void PinweaverModel::ApplyInsertLeaf(
  ...
  UpdateMetadata(pinweaver.insert_leaf().label(), &response->header,
                 &resp->unimported_leaf_data, leaf_data);
}
