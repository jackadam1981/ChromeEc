...
size_t assign_pw_field_from_bytes(
  ...
  destination.FillWith(
      fuzz::DataToSpan<const uint8_t, std::string>(bytes.data()), 0);
...
void PinweaverModel::ApplyPinweaver(const fuzz::PinWeaver& pinweaver,
  ...
  const pw_request_t* request =
      reinterpret_cast<const pw_request_t*>(buffer.begin());
  pw_response_t* response = reinterpret_cast<pw_response_t*>(buffer.begin());
...
fuzz::span<const uint8_t> PinweaverModel::GetRootHash(size_t index) const {
  if (index < root_history_.size()) {
    ...
    return fuzz::DataToSpan<const uint8_t, std::vector<uint8_t>>(
        root_history_[index].first);
  } else {
