...
void PinweaverModel::ApplyPinweaver(const fuzz::PinWeaver& pinweaver,
  ...
  switch(pinweaver.request_case()) {
    ...
    case fuzz::PinWeaver::kRemoveLeaf:
      ...
      break;
    case fuzz::PinWeaver::kTryAuth:
      HandleTryAuth(pinweaver, buffer);
      break;
    case fuzz::PinWeaver::kResetAuth:
      HandleResetAuth(pinweaver, buffer);
      break;
...
size_t PinweaverModel::GetMetadata(
    uint64_t label, struct unimported_leaf_data_t* unimported_leaf_data) const {
...
const uint8_t* PinweaverModel::GetRootHash(size_t index) const {
...
void PinweaverModel::UpdateMetadata(
  ...
  LogRootHash(header->root, label);
  if (unimported_leaf_data) {
    uint8_t* data = (uint8_t*) unimported_leaf_data;
    size_t length = header->data_length;
    if (insert) {
      insert->wrapped_data_.resize(length);
      insert->wrapped_data_.assign(data, data + length);
      leaf_metadata_.insert(std::make_pair(label, std::move(insert)));
    } else {
      auto* ptr = leaf_metadata_.at(label).get();
      ptr->wrapped_data_.resize(length);
      ptr->wrapped_data_.assign(data, data + length);
    }
    mem_hash_tree_.UpdatePathHashes(label, unimported_leaf_data->hmac);
  } else {
    leaf_metadata_.erase(label);
    mem_hash_tree_.UpdatePathHashes(label, nullptr /*path_hash*/);
  }
...
void PinweaverModel::HandleInsertLeaf(
  ...
  UpdateMetadata(pinweaver.insert_leaf().label(), &response->header,
                 &resp->unimported_leaf_data, std::move(metadata));
}
...
void PinweaverModel::HandleTryAuth(const fuzz::PinWeaver& pinweaver,
                                   uint8_t* buffer) {
  struct pw_response_t* response = (struct pw_response_t*)buffer;
  struct pw_response_try_auth_t* resp = &response->data.try_auth;
...
void PinweaverModel::HandleResetAuth(const fuzz::PinWeaver& pinweaver,
                                     uint8_t* buffer) {
  struct pw_response_t* response = (struct pw_response_t*)buffer;
  struct pw_response_reset_auth_t* resp = &response->data.reset_auth;
