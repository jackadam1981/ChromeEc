...
const uint8_t PinweaverModel::NULL_ROOT_HASH[PW_HASH_SIZE] = {};
...
size_t PinweaverModel::GetMetadata(
  ...
  return data.size() + GetPathHashes(label, *path_hashes);
}

size_t PinweaverModel::GetPathHashes(uint64_t label,
                                     uint8_t path_hashes[][PW_HASH_SIZE]) {
...
size_t PinweaverModel::GetPathHashes(
  ...
  return GetPathHashes(label, path_hashes);
}
