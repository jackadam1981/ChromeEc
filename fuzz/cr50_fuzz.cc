...
namespace {
constexpr size_t kBufferAlignment =
    alignof(pw_request_t) > alignof(pw_response_t) ? alignof(pw_request_t) :
...
bool SelfTest() {
  ...
  insert_leaf->mutable_delay_schedule()
      ->assign(delay_schedule, delay_schedule + sizeof(delay_schedule));
  assert(pinweaver_model.ApplyRequest(request, buffer_view) == EC_SUCCESS);
