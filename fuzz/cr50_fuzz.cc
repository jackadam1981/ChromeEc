...
void apply_random_bytes(const fuzz::RandomBytes& random_bytes) {
  ...
  if (value.size() >= ARRAY_SIZE(tpm_io_buffer)) {
    memcpy(tpm_io_buffer, value.data(), ARRAY_SIZE(tpm_io_buffer));
  } else {
    memcpy(tpm_io_buffer, value.data(), value.size());
    memset(tpm_io_buffer + value.size(), 0,
           ARRAY_SIZE(tpm_io_buffer) - value.size());
  }
}
...
extern "C" int test_fuzz_one_input(const uint8_t *data, unsigned int size) {
  ...
  srand(0);
  memset(tpm_io_buffer, 0, sizeof(tpm_io_buffer));
  pinweaver_model.Reset();
  for (const fuzz::SubAction& action : input.sub_actions()) {
    ...
    case fuzz::SubAction::kRandomBytes:
      ...
      pinweaver_model.SendBuffer(tpm_io_buffer);
      break;
    case fuzz::SubAction::kPinweaver:
      pinweaver_model.ApplyPinweaver(action.pinweaver(), tpm_io_buffer);
      break;
