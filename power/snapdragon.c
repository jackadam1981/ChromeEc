...
/*
 * Snapdragon SoC power sequencing module for Chrome EC
 *
...
#define IN_POWER_GOOD POWER_SIGNAL_MASK(SNAPDRAGON_POWER_GOOD)
 ...
 *
 * XXX: Rename it. Not a GPIO style.
 * XXX: Explain waiting for AP_PS_HOLD
 *
...
static void set_pmic_pwron(int asserted)
{
	...
	/* Power-on sequence:
	 * XXX Update it.
	 *   hold SYS_RESET_L -> RESIN_N of PMIC
	 *   wait for AP_PS_HOLD up, timeout 1 second
	 *   release SYS_RESET_L
	 */

	/* XXX: Is RESIN_N pin the same as KPDPWR_N pin , a toggle button? */
	/* GPIO_SYS_RESET_L connects to RESIN_N pin of PMIC */
	gpio_set_level(GPIO_SYS_RESET_L, 0);

	/* XXX: Use power_wait_signals()? */
	poll_deadline = get_time();
	/* XXX: Tune the deadline timeout and declare it as a constant */
...
static void snapdragon_lid_event(void)
{
...
DECLARE_HOOK(HOOK_LID_CHANGE, snapdragon_lid_event, HOOK_PRIO_DEFAULT);
...
static void chipset_turn_off_power_rails(void)
{
	...
	/* system power off */
	/* XXX: tune this delay or wait PS_HOLD signal */
...
void chipset_reset(int is_cold)
{
	if (is_cold) {
		...
		/* XXX: Try another secured way to reset AP */
		set_pmic_pwron(1);
...
static void powerbtn_snapdragon_changed(void)
{
	task_wake(TASK_ID_CHIPSET);
}
DECLARE_HOOK(HOOK_POWER_BUTTON_CHANGE, powerbtn_snapdragon_changed, HOOK_PRIO_DEFAULT);
