		...
		if (timeout < 0) {
			task_wait_event(-1);
				...
				(task_wait_event(deadline.val - now.val) ==
					TASK_EVENT_TIMER)) {
			CPRINTF("power timeout waiting for GPIO %d/%s",
...
static int gaia_power_init(void)
{
	...
	if (!(system_get_reset_flags() & RESET_FLAG_AP_OFF)) {
		CPRINTF("auto_power_on is set due to reset_flag 0x%x",
	...
	*/
	if (!(system_get_reset_flags() & RESET_FLAG_SYSJUMP)) {
		CPRINTF("not sysjump; forcing AP reset");
...
static int check_for_power_on_event(void)
{
	...
	if (gpio_get_level(GPIO_EN_PP3300)) {
		CPRINTF("system is on, thus clear auto_power_on");
...
static int power_on(void)
{
	...
	if (gpio_get_level(GPIO_PP1800_LDO2) == 0) {
		...
		gpio_set_level(GPIO_EN_PP3300, 0);
		usleep(DELAY_5V_SETUP);
		CPRINTF("power error: PMIC failed to enable");
		return -1;
	}
...
static int wait_for_power_on(void)
{
	...
	while (1) {
		...
		if (value != 1 && charge_keep_power_off()) {
			CPRINTF("power on ignored due to low battery");
			continue;
		}
#endif
	...
	while (1) {
		...
		if (!power_on()) {
			...
			if (continue_power) {
				...
				CPRINTF("power ending loop %d", value);
			}
		}
		power_off();
