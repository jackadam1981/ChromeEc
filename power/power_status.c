 ...
 *
 * In order to use this module, the board must:
 *
 * 1) Provide a
...
/***************************************************************
 * Wrapper around the 2 ways to get battery SoC.
 */
...
static void update_power_source(void)
{
	 ...
	 * can be calculated.
	 * If the battery SoC is less than BATTERY_LEVEL_LOW, inform the AP
...
/***************************************************************
 * Host command to retrieve power info v2
 */
static int host_command_power_info(struct host_cmd_handler_args *args)
{
	const struct board_power_config *config = board_get_power_config();
	const unsigned int ac_power = charge_manager_get_power_limit_uw() /
		1000000;
	 ...
	 * This is a conservative setting, explained below.
	 *
	 * a) 0.8 is Battery EOL Derating factor
	 ...
	 *    full is diminished
	 *
	 * b) 0.86 is Battery SoC Derating factor This factory relates
	...
	r->batt_dbpt_support_level = dbpt_level;
	r->rop_avg = config->rop_avg;
	r->rop_peak = config->rop_peak;

	/* These values are dynamic */
	r->battery_1cd = batt_1C_derated;
	r->batt_dbpt_max_peak_power = batt_max_peak_power;
	r->batt_dbpt_sus_peak_power = batt_sus_peak_power;
