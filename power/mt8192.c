...
#define SYS_RST_PULSE_LENGTH (250 * MSEC)
...
{
	/* Pass AP_EC_WACHDOG_ODL signal to PMIC */
	...
	if (power_get_signals() & IN_PGOOD_PMIC &&
	    ...
	    gpio_get_level(GPIO_SYS_RST_ODL))
		chipset_reset(CHIPSET_RESET_AP_WATCHDOG);

	/*
	 * case 2&3: Just fall through. The chipset_reset should have been
	 * invoked.
	 */
}
...
void chipset_reset(enum chipset_reset_reason reason)
{
	...
	if (reason != CHIPSET_RESET_AP_WATCHDOG &&
	    !(power_get_signals() & IN_AP_WDT_ASSERTED))
		/* TODO(b:150341779): add */
		CPRINTS("AP not assert WDT, doing PMIC reset...");
}
