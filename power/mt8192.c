...
#define IN_AP_WDT_DEASSERTED POWER_SIGNAL_MASK(AP_WDT_DEASSERTED)
...
enum power_state power_handle_state(enum power_state state)
{
	...
	case POWER_S5S3:
		...
		GPIO_SET_LEVEL(GPIO_SYS_RST_ODL, 1);
		if (EC_ERROR_TIMEOUT ==
		    power_wait_signals_timeout(IN_AP_WDT_DEASSERTED,
					       AP_EC_WDT_TIMEOUT)) {
			CPRINTS("WDT not deasserted, retry S5S3");
			return POWER_S5S3;
		}
	...
	case POWER_S3S5:
		...
		if (EC_ERROR_TIMEOUT ==
		    power_wait_signals_timeout(IN_AP_WDT_ASSERTED,
					       AP_EC_WDT_TIMEOUT)) {
			CPRINTS("Timeout waitting AP watchdog, turn if off "
				"forcely, AP_EC_WATCHDOG_L=%d", gpio_get_level(GPIO_AP_EC_WATCHDOG_L));
