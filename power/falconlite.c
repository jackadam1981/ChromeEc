...
#define IN_AP_WDT		POWER_SIGNAL_MASK(FCL_AP_WDT)

#define IN_SUSPEND_DEASSERTED	POWER_SIGNAL_MASK(FCL_AP_IN_SLEEP_DEASSERTED)
#define IN_AP_SHUTDOWN_REQ	IN_SUSPEND_DEASSERTED

#define IN_PGOOD_S0		(POWER_SIGNAL_MASK(FCL_PG_VDD1_VDD2) | \
...
#define IN_PGOOD_S3            IN_ALL_S0

#define IN_ALL_S0              IN_PGOOD_S0
#define IN_ALL_S3              IN_PGOOD_S3

/*
 * TODO(yllin): AP_IN_SLEEP_L will be repurposed to AP initiated shutdown
 *              indicator
 */
#define GPIO_AP_SHUTDOWN_REQ_L	GPIO_AP_IN_SLEEP_L
#define FCL_AP_SHUTDOWN_REQ	FCL_AP_IN_SLEEP_DEASSERTED
...
const struct power_signal_info power_signal_list[] = {
	[FCL_AP_IN_SLEEP_DEASSERTED] = {
		GPIO_AP_IN_SLEEP_L,
		POWER_SIGNAL_ACTIVE_HIGH,
		"AP_IN_SLEEP_DEASSERTED"
	},
	[FCL_AP_WDT] = {
...
void chipset_reset_request_interrupt(enum gpio_signal signal)
{
	if (signal == GPIO_AP_EC_WARM_RST_REQ) {
		...
		return;
	} else if (signal == GPIO_AP_SHUTDOWN_REQ_L) {
		/*
		 * AP_IN_SLEEP_L is repurposed for AP shutdown request at the
		 * falling edge.
		 */
		if (!gpio_get_level(signal)) {
			/*
			 * AP initiated shutdown, AP_EC_WATCHDOG_L should be
			 * deasserted before process shutdown, or it is a
			 * reboot.
			 */
#if 0
			if (gpio_get_level(GPIO_AP_EC_WATCHDOG_L) == 1) {
#endif
...
enum power_state power_chipset_init(void)
{
	...
	/* Start from S5 if the is already up. */
...
enum power_state power_handle_state(enum power_state state)
{
	...
	case POWER_S5:
		/*
		 * If AP initiated shutdown, we can transition to G3
		 * immediately.
		 */
		if (!boot_from_off)
			boot_from_off = check_for_power_on_event();
		...
		if (ap_shutdown) {
			ap_shutdown = 0;
			return POWER_S5G3;
		} else if (!forcing_shutdown || boot_from_off) {
			/* Powering up. */
			s5s3_retry = 1;
			return POWER_S5S3;
		}
	...
	case POWER_S3:
		if (!power_has_signals(IN_PGOOD_S3) || forcing_shutdown ||
			...
			return POWER_S3S5;
			 ...
			 ap_sleep_event == HOST_SLEEP_EVENT_S0IX_RESUME ||
			 boot_from_off)
			return POWER_S3S0;
		break;
	case POWER_S0:
		if (ap_sleep_event == HOST_SLEEP_EVENT_S3_SUSPEND ||
		    ap_sleep_event == HOST_SLEEP_EVENT_S0IX_SUSPEND ||
		...
		struct host_sleep_event_context *ctx)
{
	...
	if (state == HOST_SLEEP_EVENT_S3_RESUME ||
	    state == HOST_SLEEP_EVENT_S0IX_SUSPEND ||
