...
static void board_handle_hard_sleep_hang(void)
{
	...
	} else if ((hard_sleep_hang_count == 1) &&
		   IS_ENABLED(CONFIG_EMULATED_SYSRQ)) {
		/*
		 * Send |SysRq| signal to generate a kernel panic. If the AP is
		 * already in the kernel, the intent is that the current CPU
		 * stack traces will be output so the hang can be debugged. If
		 * the CPU is not in the kernel, it should be woken, as keyboard
		 * input events are wake events. Once the AP has booted/woken
		 * far enough to process the keyboard event, the |SysRq| will
		 * generate a panic, which will generate a crash report (and the
		 * corresponding metrics).
		 * A single |SysRq| restarts chrome, while two trigger a kernel
		 * panic.
		 */
