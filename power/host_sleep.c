...
static void board_handle_hard_sleep_hang(void)
{
	...
	/*
	 * When |CONFIG_EMULATED_SYSRQ| is not enabled for a board,
	 * |host_send_sysrq()| will not be defined, resulting in compiler
	 * errors for those boards (error: undefined symbol). Splitting the
	 * conditional here into two if-statements looks awkward, but allows us
	 * to avoid |#ifdef CONFIG_EMULATED_SYSRQ| here by taking advantage of
	 * the optimizer removing everything inside the braces when
	 * CONFIG_EMULATED_SYSRQ is not enabled.
	 */
	if (IS_ENABLED(CONFIG_EMULATED_SYSRQ)) {
		if (hard_sleep_hang_count == 1) {
			/*
			 * Send |SysRq| signal to generate a kernel panic. If
			...
			host_send_sysrq('x');
			ccprints(
				"AP will be force reset in %dms if hang persists",
