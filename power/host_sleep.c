...
{
	switch (hang_type) {
	case SLEEP_HANG_S0IX_SUSPEND:
		CPRINTS("Warning: Detected S0iX suspend hang! Waking host up!");
	break;
	case SLEEP_HANG_S0IX_RESUME:
		CPRINTS("Warning: Detected S0iX resume hang! Waking host up!");
	break;
	default:
...
static void resume_transition_timeout(void)
{
	...
	power_chipset_handle_sleep_hang(SLEEP_HANG_S0IX_RESUME);
}
