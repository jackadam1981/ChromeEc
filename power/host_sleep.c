...
static void (*resume_timeout_callback)(void);
static void (*suspend_timeout_callback)(void);
...
static void suspend_transition_timeout(void)
{
	...
	if (suspend_timeout_callback)
		suspend_timeout_callback();
}
...
static void resume_transition_timeout(void)
{
	...
	/* Call the custom callback */
	if (resume_timeout_callback)
		resume_timeout_callback();
}
			 ...
			 void (*resume_hang_callback)(void))
{
	...
	resume_timeout_callback = resume_hang_callback;
	suspend_timeout_callback = suspend_hang_callback;
...
void sleep_reset_tracking(void)
{
	sleep_signal_transitions = 0;
	sleep_signal_timeout = 0;
	suspend_timeout_callback = NULL;
	resume_timeout_callback = NULL;
}
