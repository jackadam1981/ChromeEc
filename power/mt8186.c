...
#define IN_ALL_S0 (IN_AP_RST & ~IN_SUSPEND_ASSERTED)
...
enum power_state power_chipset_init(void)
{
	...
	if (power_get_signals() & IN_AP_RST) {
		...
		/*
		 * Start from S0, may drop to S3 if AP_IN_SLEEP is asserted,
		 * or to S5 if the chipset_force_shutdown() above is called.
		 */
		return POWER_S0;
	}
...
enum power_state power_handle_state(enum power_state state)
{
	...
	case POWER_S3:
		if (power_wait_mask_signals_timeout(IN_AP_RST, IN_AP_RST,
						    20 * MSEC))
			return POWER_S3S5;
	...
	case POWER_S0:
		if (!power_has_signals(IN_AP_RST) ||
	...
	case POWER_S5S3:
		...
		if (power_wait_signals_timeout(IN_AP_RST,
					       PMIC_EN_TIMEOUT)) {
	...
	case POWER_S3S0:
		if (power_wait_signals(IN_AP_RST)) {
