...
void chipset_force_shutdown(enum chipset_shutdown_reason reason)
{
	...
	/*
	 * Try to force PMIC shutdown with a long press.
	 */
...
enum power_state power_chipset_init(void)
{
	...
	if (power_get_signals() & IN_PGOOD_PMIC) {
		...
		/* TODO: check if the actual state is S3, will go back to S5
		 * after chipset_force_shutdown() complete */
		return POWER_S0;
	}
...
/*
 * Power state is determined from the following two signals:
 *
 *     | IN_PGOOD_PMIC | IN_SUSPEND_ASSERTED |
 * ----------------------------------------------
 *  S0 |             1 |                   0 |
 *  S3 |             1 |                   1 |
 *  S5 |             0 |                   x |
...
enum power_state power_handle_state(enum power_state state)
{
	...
	case POWER_S5:
		...
		if (ap_shutdown) {
			ap_shutdown = 0;
			return POWER_S5G3;
		} else if (power_get_signals() & IN_PGOOD_PMIC) {
			return POWER_S5S3;
		}
