...
enum power_state power_handle_state(enum power_state state)
{
	...
	case POWER_S5G3:
		...
		if (power_wait_mask_signals_timeout(
			    IN_PMIC_AP_RST, IN_PMIC_AP_RST, 13000 * MSEC)) {
			CPRINTS("PMIC reset AP timeout. Forcing PMIC off. 0x%x", power_get_signals());
		} else {
			CPRINTS("\033[31mno timeout wtf 0x%x\033[m", power_get_signals());
		}
