...
static void mt8186_exit_off(void)
{
	...
	/* ignore all power on requests if we just reboot */
	if (get_time().val < CONFIG_PLATFORM_EC_PP3700_DISCHARGE_TIME_MS * MSEC)
		return;
#endif
	is_exiting_off = true;
...
enum power_state power_handle_state(enum power_state state)
{
	...
	case POWER_S5G3:
		...
		msleep(CONFIG_PLATFORM_EC_PP3700_DISCHARGE_TIME_MS);
