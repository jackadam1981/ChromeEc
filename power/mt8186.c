 ...
 */
static enum power_state power_get_next_state(void)
{
...
{
	int exit_hard_off = 1;
	enum power_state init_state = power_get_next_state();
	...
	if (system_jumped_late()) {
		if (init_state != POWER_G3) {
			disable_sleep(SLEEP_MASK_AP_RUN);
			CPRINTS("already in S0 / S3");
		}
...
{
	enum power_state next_state = power_get_next_state();
