...
static void siglog_deferred(void)
{
	...
	for (i = siglog_head; i < tmp_siglog_tail; i++) {
		if (i != siglog_head)
			tdiff.val = siglog[PTR2IDX(i)].time.val -
				    siglog[PTR2IDX(i - 1)].time.val;
		CPRINTF("  %.6lld  +%.6lld  %s => %d\n",
			siglog[PTR2IDX(i)].time.val, tdiff.val,
			power_signal_get_name(siglog[PTR2IDX(i)].signal),
			siglog[PTR2IDX(i)].level);
	}
	if (tmp_siglog_truncated)
		CPRINTF("  SIGNAL LOG TRUNCATED...\n");

	siglog_truncated = 0;
	siglog_head = tmp_siglog_tail;
}
...
static void siglog_add(enum gpio_signal signal)
{
	const struct power_signal_info *s = power_signal_list;
	int i;
	unsigned int siglog_entries = siglog_tail - siglog_head;
