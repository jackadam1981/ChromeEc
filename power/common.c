...
/* Flag indicating if request to reboot AP on G3 is raised or not */
static bool want_reboot_at_g3;

static void check_system_reboot_ap_on_g3(void)
{
	if (want_reboot_at_g3) {
	    want_g3_exit = 1; /* Set flag to leave G3 */
	    want_reboot_at_g3 = false; /* Clear AP reboot request flag */
...
host_command_reboot_on_g3(struct host_cmd_handler_args *args)
{
        /* Store request for processing at g3 */
	want_reboot_at_g3 = true;
