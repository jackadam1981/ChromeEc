...
static int host_command_hibernation_delay(struct host_cmd_handler_args *args)
{
	...
	/* Approximation for dividing by 1 second. */
	uint64_t time_g3 = (get_time().val - last_shutdown_time) >> 20;
	...
	if ((time_g3 != 0) && (time_g3 > (uint64_t)hibernate_delay))
		r->time_remaining = 0;
	else
		r->time_remaining = (uint64_t)hibernate_delay - time_g3;
