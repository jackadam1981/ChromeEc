...
enum power_state power_handle_state(enum power_state state)
{
	...
	case POWER_S0:
		/*
		 * Charge_manager will change voltage, current from supplier
		 * that case, input_current also changed. If system is turned on
		 * with drained battery, NVDC will will be disabled due to lack
		 * of power. Turning off system until we have enough power
		 */
#ifdef CONFIG_CHARGER_LIMIT_POWER_THRESH_BAT_PCT
		if (chipset_is_just_jumped_in) {
			...
			if (tries_s0 < BATTERY_INITIALIZED_TRIES &&
				...
				CPRINTS("Turn off AP and give decision to state machine");
