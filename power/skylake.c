...
void slp_s0_signal_check_deferred(void)
{
	if (gpio_get_level(GPIO_PCH_SLP_S0_L) == 0)
		...
		/*
		 * Raising a special event which will be checked by the kernel
		 * on resume to set the appropriate return code.
		 */
...
{
	/*
	 * Using the lid open directly to initiate the EC resume from S0ix
	 * sequence. It is important that the host event for lid open is
	 * sent and PCH_WAKE_L pulsed before EC starts the state transition.
	 *
	 * Just setting the flag is enough for lidopen/close to work from
	 * ec console. The physical lid open/close is debounced and initiates
	 * the EC transition before the PCH_WAKE_L is pulsed. So this ordering
	 * is enforced using HOOK_PRIO_FIRST and host_set_single_event.
	 */
...
DECLARE_HOOK(HOOK_LID_CHANGE, lid_change, HOOK_PRIO_FIRST);
...
static int host_event_sleep_event(struct host_cmd_handler_args *args)
{
	...
	if (p->sleep_event == HOST_SLEEP_EVENT_S0IX_SUSPEND) {
		...
		if (p->signal_detect) {
			CPRINTS("Initiating signal detect with timeout %d", p->signal_detect_timeout_msec);
