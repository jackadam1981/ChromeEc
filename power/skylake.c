...
static int chipset_get_sys_sleep_state(enum sys_sleep_state state)
{
...
static enum power_state _power_handle_state(enum power_state state)
{
...
#endif
		if (chipset_get_sys_sleep_state(SYS_SLEEP_S4) == 1)
			return POWER_S5S3; /* Power up to next state */
		break;
	...
	case POWER_S3:
		if (!power_has_signals(IN_PGOOD_ALL_CORE)) {
			...
			return POWER_S3S5;
		} else if (chipset_get_sys_sleep_state(SYS_SLEEP_S3) == 1) {
			/* Power up to next state */
			return POWER_S3S0;
		} else if (chipset_get_sys_sleep_state(SYS_SLEEP_S4) == 0) {
	...
	case POWER_S0:
		if (!power_has_signals(IN_PGOOD_ALL_CORE)) {
			...
			return POWER_S0S3;
			   ...
			   (chipset_get_sys_sleep_state(SYS_SLEEP_S3) == 1)) {
			return POWER_S0S0ix;
#endif
		} else if (chipset_get_sys_sleep_state(SYS_SLEEP_S3) == 0) {
			/* Power down to next state */
			return POWER_S0S3;
		}
