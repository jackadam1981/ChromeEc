...
static enum power_state _power_handle_state(enum power_state state)
{
	...
	case POWER_S0S0ix:
		...
		hook_notify(HOOK_CHIPSET_STANDBY);
		/*
		 * Enable idle task deep sleep. Allow the low power idle task
...
#ifdef CONFIG_SUPPORT_S0IX
static int slp_s0_debounced = 1;

int chipset_get_ps_debounced_level(enum gpio_signal signal)
{
	int level = gpio_get_level(signal);
	return signal == GPIO_PCH_SLP_S0_L ? (level & slp_s0_debounced) : level;
}
...
{
	if((power_get_signals() & IN_PCH_SLP_S0_DEASSERTED) == 0) {
	//if (gpio_get_level(GPIO_PCH_SLP_S0_L)) {
