 ...
 * Experimented that it is 65~70ms. Pick a value of its double.
 */
#define PMIC_POWER_AP_RESPONSE_TIMEOUT	(140 * MSEC)
 ...
 * the individual chip.
 *
 * @return non-zero if the PMIC/AP is powered, 0 if not
 */
 ...
 *
 * @param asserted	Assert (=1) to wait the PMIC/AP on.
			Deassert (=0) to wait the PMIC/AP off.
 */
static void wait_pmic_pwron(int asserted)
{
	timestamp_t poll_deadline;

	/* Check the AP power status */
	if (!(asserted ^ is_pmic_pwron()))
		return;

	poll_deadline = get_time();
	poll_deadline.val += PMIC_POWER_AP_RESPONSE_TIMEOUT;
	while ((asserted ^ is_pmic_pwron()) &&
 ...
 *
 * @param asserted	Assert (=1) to power the PMIC/AP on.
			Deassert (=0) to power the PMIC/AP off.
 */
static void set_pmic_pwron(int asserted)
{
	...
	if (!asserted) {
		/*
		 * Before we can reprogram the PMIC to make the PMIC_KPD_PWR_ODL
		 * as an instant power-off trigger, we skip doing anything here
		 * and falling back to the next step, which cuts off the system
		 * power.
		 */
		CPRINTS("SKIP power-off PMIC/AP");
		return;
	}
...
static void power_off(void)
{
	...
	set_system_power(0);
	/*
	 * Wait the PMIC/AP power state off to ensure, in case it is forced
	 * off by power rails.
	 */
	wait_pmic_pwron(0);
