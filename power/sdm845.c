...
/* Delay between power-on the system and power-on the PMIC/AP */
#define PMIC_POWER_ON_DELAY (50 * MSEC)

/* TODO(waihong): Tune the timeout value */
/* After power-on/off PMIC, how long the AP should response it to turn on/off */
#define PMIC_POWER_AP_RESPONSE_TIMEOUT (1 * SECOND)

/* Delay between power-off the PMIC/AP and power-off the system */
#define PMIC_POWER_OFF_DELAY (50 * MSEC)

/* Delay between power-off the PMIC/AP and power-on it again */
#define PMIC_POWER_RESET_DELAY (50 * MSEC)
...
static void set_pmic_pwron(int asserted)
{
	...
	/*
	 * Power-on sequence:
	 * 1. hold down KPDPWR_N pin of PM845, which is a power-on trigger
	 * 2. PM845 pulls up AP_RESET_L signal to power-on SDM845
	 * 3. SDM845 pulls up AP_PS_HOLD signal
	 * 4. wait for AP_PS_HOLD up, which may take X seconds timeout
	 * 5. release KPDPWR_ signal
	 */

	/* XXX: GPIO_XXX connects to KPDPWR_N pin of PMIC */
	...
	while ((asserted ^ is_ap_powered()) &&
		...
		/* Timeout too long that needs to reload watchdog */
		watchdog_reload();
	}
	if (asserted ^ is_ap_powered()) {
		if (asserted)
...
enum power_state power_chipset_init(void)
{
	...
	if (!(reset_flags & RESET_FLAG_SYSJUMP)) {
		...
		/*
		 * TODO(waihong): Trigger chipset special mode, for flashing
		 * SPI from USB. Need it?
		 */
...
{
	/*
	 * XXX: Before we can reprogram the PMIC to change the timers that
	 * triggers power-off instantly, we cut the power directly
	 */
	set_system_power(0);
}
...
{
	/*
	 * Before we can reprogram the PMIC to make the RESIN_N pin as
	 * reset pin and zero-latency. We do warm reset the same as cold
	 * reset.
	 */
