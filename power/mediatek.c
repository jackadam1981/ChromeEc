...
void chipset_reset(int is_cold)
{
	if (is_cold) {
		...
		gpio_set_level(GPIO_SYSTEM_POWER_H, 0);
...
/* OAK_PD: TODO: SUSPEND signal is not stable at Rev1 board now,
		 Disable SUSPEND detection temporary, let AP boot up to S0.
	         Need to debug */
#if 0
		else if (power_get_signals() & IN_SUSPEND) {
			return POWER_S0S3;
