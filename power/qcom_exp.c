...
#ifdef CONFIG_PLATFORM_EC_HOSTCMD_OFFMODE_CHARGING_ACTIVE
...
DECLARE_HOST_COMMAND(EC_CMD_OFFMODE_CHARGING_ACTIVE,
...
void board_chipset_cache_soc_on_shutdown(void)
{
	...
	/* If chipset shutdown and external power is connected, boot the AP to
	 * enable charging */
	if (extpower_is_present()) {
		ac_on = 1;
		task_wake(TASK_ID_CHIPSET);
