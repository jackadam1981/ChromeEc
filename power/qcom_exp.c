...
#define IN_AP_RST_ASSERTED POWER_SIGNAL_MASK(AP_RST_ASSERTED)
#define IN_SUSPEND POWER_SIGNAL_MASK(AP_SUSPEND)
 ...
 */
#define DISABLE_SWITCHCAP_DURING_SHUTDOWN_DELAY (10 * MSEC)
...
static void power_off_seq(uint8_t shutdown_event)
{
	...
	/*
	 * For Chipset QC_EXP, wait for the S4c rail to assert then disable
	 * switchcap after a delay
	 */
	if (EC_SUCCESS == wait_for_s4c_power_off()) {
		/* Wait for DISABLE_SWITCHCAP_DURING_SHUTDOWN_DELAY
		 * before switching off all rails.
		 */
		crec_usleep(DISABLE_SWITCHCAP_DURING_SHUTDOWN_DELAY);
		set_system_power(0);
	} else
		CPRINTS("S4C_PWR_OFF is not asserted");
...
test_mockable enum power_state power_handle_state(enum power_state state)
{
	...
	case POWER_G3S5:
		/*
		 * wait for power button release before actually boot
		 * ap. it may be a long-hold power button with volume
		 * buttons to trigger the recovery button. we don't want
		 * ap up during the long-hold.
		 */
