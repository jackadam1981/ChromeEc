...
enum slp_s0ix_state {
	S0IX_NOT_TIMING,
	S0IX_WAITING_FOR_SUSPEND,
	S0IX_SUSPENDED,
	S0IX_WAITING_FOR_AP_UP,
	S0IX_TIMEOUT
};

uint16_t slp_s0ix_timeout;
enum slp_s0ix_state slp_s0ix_state = S0IX_NOT_TIMING;
uint16_t slp_s0ix_transitions;
...
void power_chipset_power_update_signals(uint32_t old, uint32_t new)
{
	...
	/* Bail early if the S0ix pin hasn't changed. */
	if (((old ^ new) & POWER_SIGNAL_MASK(CONFIG_SLP_S0_POWER_PIN)) == 0)
		return;

	if (slp_s0ix_transitions < EC_HOST_RESUME_SLEEP_TRANSITIONS_MASK)
		slp_s0ix_transitions += 1;

	switch (slp_s0ix_state) {
	case S0IX_WAITING_FOR_SUSPEND:
		/* Stop the timer from worrying if the AP went down.*/
		if (!(new & POWER_SIGNAL_MASK(CONFIG_SLP_S0_POWER_PIN))) {
			slp_s0ix_state = S0IX_SUSPENDED;
	...
	case S0IX_SUSPENDED:
		/*
		 * If the AP comes out of suspend, arm the timer to
		 * ensure that resume happens in a timely manner.
		 */
		if (new & POWER_SIGNAL_MASK(CONFIG_SLP_S0_POWER_PIN)) {
			slp_s0ix_state = S0IX_WAITING_FOR_AP_UP;
	...
	case S0IX_WAITING_FOR_AP_UP:
		/* If the AP went back down, stop the timer. */
		if (!(new & POWER_SIGNAL_MASK(CONFIG_SLP_S0_POWER_PIN))) {
			slp_s0ix_state = S0IX_SUSPENDED;
			hook_call_deferred(&s0ix_transition_timeout_data, -1);
		}
...
static void s0ix_transition_timeout(void)
{
	if ((slp_s0ix_state == S0IX_WAITING_FOR_AP_UP) ||
	    (slp_s0ix_state == S0IX_WAITING_FOR_SUSPEND)) {

		/* Mark the timeout. */
		slp_s0ix_state = S0IX_TIMEOUT;
		hook_call_deferred(&s0ix_transition_timeout_data, -1);
...
static uint16_t s0ix_complete_suspend(void)
{
	uint16_t sleep_transitions = slp_s0ix_transitions;

	slp_s0ix_transitions = 0;
	if (slp_s0ix_state == S0IX_TIMEOUT)
		sleep_transitions |= EC_HOST_RESUME_SLEEP_TIMEOUT;

	slp_s0ix_state = S0IX_NOT_TIMING;
	hook_call_deferred(&s0ix_transition_timeout_data, -1);
