 ...
 * Power off the AP
 *
 * @param shutdown_from_off	reason of shutdown, which is a return value of
 *				check_for_power_off_event()
 */
static void power_off_seq(uint8_t shutdown_from_on)
{
	/* Check PMIC POWER_GOOD */
	if (is_pmic_pwron()) {
		if (shutdown_from_on == POWER_OFF_BY_POWER_GOOD_LOST) {
			/*
			 * The POWER_GOOD was lost previously, which sets the
			 * shutdown_from_on flag. But now it is up again. This
