...
static int check_for_power_off_event(void)
{
	...
	if (pressed) {
		...
		if (!power_button_was_pressed) {
			...
			CPRINTF("waiting for long press %u",
				power_off_deadline.le.lo);
		} else if (timestamp_expired(power_off_deadline, &now)) {
			power_off_deadline.val = 0;
			CPRINTF("off after long press now=%u, %u",
...
enum power_state power_chipset_init(void)
{
	...
	if (!(reset_flags & RESET_FLAG_SYSJUMP)) {
		CPRINTF("not sysjump; forcing AP shutdown");
	...
	if (!(reset_flags & RESET_FLAG_AP_OFF) &&
		...
		CPRINTF("auto_power_on set due to reset_flag 0x%x",
...
static void power_on(void)
{
	...
	if (t < PMIC_RTC_STARTUP) {
		...
		CPRINTF("wait for %dms for PMIC RTC start-up",
			wait / MSEC);
		usleep(wait);
	}
...
static int wait_for_power_button_release(unsigned int timeout_us)
{
	...
	while (power_button_is_pressed()) {
		...
		if (timeout_us < 0) {
			...
			TASK_EVENT_TIMER)) {
			CPRINTF("power button not released in time");
			return EC_ERROR_TIMEOUT;
		}
	}

	CPRINTF("power button released");
...
static void power_off(void)
{
	...
	CPRINTF("power shutdown complete");
}

void chipset_reset(int is_cold)
{
	if (is_cold) {
		CPRINTF("EC triggered cold reboot");
		power_off();
		/* After XPSHOLD is dropped off, the system will be on again */
		power_request = POWER_REQ_ON;
	} else {
		CPRINTF("EC triggered warm reboot");
		CPRINTF("assert GPIO_PMIC_WARM_RESET_L for %d ms",
	...
	case POWER_S5S3:
		...
		if (power_wait_signals(IN_XPSHOLD) == EC_SUCCESS) {
			...
			if (wait_for_power_button_release(
				...
				CPRINTF("long-press button, shutdown");
				power_off();
				/*
			...
			CPRINTF("XPSHOLD not seen in time");
		}
		set_pmic_pwron(0);
		return POWER_S5;
