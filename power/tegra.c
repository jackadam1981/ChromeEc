 ...
 *  - If SUSPEND_L goes low, enter the suspend mode.
 *
 */
...
#define IN_SUSPEND POWER_SIGNAL_MASK(TEGRA_SUSPEND)
...
enum power_state power_handle_state(enum power_state state)
{
	...
	case POWER_S5S3:
		power_on();
		if (!power_wait_signals(IN_XPSHOLD)) {
	...
	case POWER_S3:
		if (!gpio_get_level(GPIO_SOC1V8_XPSHOLD))
			return POWER_S3S5;
		else if (gpio_get_level(GPIO_SUSPEND_L))
			return POWER_S3S0;
		return state;
