 ...
 * up, the board.h file can override this delay.
 */
#ifndef SYS_PWROK_DELAY_MS
	#define SYS_PWROK_DELAY_MS		0
#endif
...
enum power_state power_handle_state(enum power_state state)
{
	...
	case POWER_S0:
		 ...
		 * board specified delay (default is no delay)
		 */
		...
		if (all_sys_pwrgd_in != all_sys_pwrgd_out) {
			if (all_sys_pwrgd_in && (SYS_PWROK_DELAY_MS != 0))
