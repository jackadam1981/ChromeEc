...
__overridable int get_pg_ec_dsw_pwrok(void)
{
	return gpio_get_level(GPIO_PG_EC_DSW_PWROK);
}

__overridable int get_pg_ec_all_sys_pwrgd(void)
{
...
void chipset_force_shutdown(enum chipset_shutdown_reason reason)
{
	...
	while (get_pg_ec_dsw_pwrok() && gpio_get_level(GPIO_PG_EC_RSMRST_ODL)
	       && (timeout_ms > 0)) {
		msleep(1);
		timeout_ms--;
	};
...
{
	int dswpwrok_in = get_pg_ec_dsw_pwrok();
	...
	case POWER_G3S5:
		...
		dswpwrok_in = get_pg_ec_dsw_pwrok();
		/*
		 * A minimum 10 msec delay is required between PP3300_A being
		 * stable and the DSW_PWROK signal being passed to the PCH.
		 */
	...
	case POWER_S0:
		 ...
		 */
		all_sys_pwrgd_in = get_pg_ec_all_sys_pwrgd();
