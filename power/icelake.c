...
enum power_state power_handle_state(enum power_state state)
{
	 ...
	 * Assert VCCST power good and PCH_PWROK, when ALL_SYS_PWRGD is received with a 2ms
	 * delay minimum.
	 */
	if (all_sys_pwrgd_in && !gpio_get_level(GPIO_EC_AP_VCCST_PWRGD_OD)) {
		board_jsl_all_sys_pwrgd(all_sys_pwrgd_in);
		hook_call_deferred(&assert_ec_ap_vccst_pwrgd_pch_pwrok_data, 2 * MSEC);
	...
	case POWER_S3S0:
		...
		while (!intel_x86_get_pg_ec_all_sys_pwrgd() && (timeout_ms > 0)) {
			msleep(1);
			timeout_ms--;
		};
		if (!timeout_ms)
