...
#define MEMFAULT_BSS_COLLECTION_SIZE (1024)
#define MEMFAULT_DATA_COLLECTION_SIZE (64)
#define MEMFAULT_EXTRA_COLLECTION_SIZE (1024)
...
#define MEMFAULT_COREDUMP_SIZE ( \
  ...
  MEMFAULT_BSS_COLLECTION_SIZE + \
  MEMFAULT_DATA_COLLECTION_SIZE + \
...
const sMfltCoredumpRegion *memfault_platform_coredump_get_dynamic_regions(
  ...
  s_coredump_dynamic_regions[region_index++] = MEMFAULT_COREDUMP_MEMORY_REGION_INIT(BSS_START, MEMFAULT_BSS_COLLECTION_SIZE);

  *num_regions = region_index;
  MEMFAULT_LOG_INFO("num_dynamic_regions = %d!", *num_regions);

  MEMFAULT_LOG_DEBUG("s_ram_backed_coredump_region = %x", (uint32_t)s_coredump_region);
