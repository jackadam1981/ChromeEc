...
#define MEMFAULT_RAM_BACKED_LOG_SIZE 1024
MEMFAULT_PUT_IN_SECTION(".noinit.mflt_log") MEMFAULT_ALIGNED(8)
static uint8_t s_ram_backed_log_region[MEMFAULT_RAM_BACKED_EVENTS_SIZE];
...
extern int memfault_platform_metrics_init(const sMemfaultEventStorageImpl *evt_storage);
extern int memfault_platform_coredump_init(void);
...
uint64_t memfault_platform_get_time_since_boot_ms(void) {
	MEMFAULT_LOG_INFO("memfault_platform_get_time_since_boot_ms %" PRIu64, get_time().val / MSEC);
...
static eMemfaultRebootReason reset_flags_to_memfault_reboot_reason(uint32_t reset_flags) {
	...
	if (reset_flags & EC_RESET_FLAG_OTHER) {
		...
		return kMfltRebootReason_Watchdog;
...
int memfault_platform_init(void) {
	...
	memfault_reboot_tracking_boot(s_ram_backed_reboots_region, &reset_reason);

	memfault_reboot_tracking_collect_reset_info(evt_storage);

	memfault_log_set_min_save_level(kMemfaultPlatformLogLevel_Info);
	bool rv = memfault_log_boot(s_ram_backed_log_region, sizeof(s_ram_backed_log_region));
	if (!rv) {
		MEMFAULT_LOG_ERROR("Failed to init log");
	}
