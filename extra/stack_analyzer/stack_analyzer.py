...
class StackAnalyzer:
    ...
    def LoadAnnotation(self):  # pylint:disable=invalid-name
        ...
        def ExpandArray(dic):  # pylint:disable=invalid-name
            ...
            for addr in range(begin_address + offset, end_address, stride):
                ...
                if not name:
                    raise StackAnalyzerError(
                        f"Cannot find function for address {val:h}."
                    )
    ...
    def Analyze(self):  # pylint:disable=invalid-name
        ...
        for task in self.tasklist:
            ...
            print(
                f"Task: {task.name}, Max size: {routine_func.stack_max_usage + extra_stack_frame} ({routine_func.stack_max_usage} + {extra_stack_frame}), Allocated size: {task.stack_max_size}"
            )
            ...
            for depth, curr_func in enumerate(max_stack_path):
                ...
                print(
                    f"    {curr_func.name} ({curr_func.stack_frame}) [{os.path.relpath(path)}:{linenum}] {curr_func.address:x}"
                )
