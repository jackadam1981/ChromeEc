...
class Function(object):
  ...
  Attributes:
    ...
    annotation_mark: Mark of annotation. None if it isn't assigned.
  ...
  def __eq__(self, other):
    ...
    if not (self.address == other.address and
            ...
            self.annotation_mark == other.annotation_mark and
...
class StackAnalyzer(object):
  ...
  def ResolveAnnotation(self, function_map):
    ...
    for sig in signature_set:
      ...
      if name not in symbol_path_map:
        # Lazy symbol path resolving.
        path_group = collections.defaultdict(list)
        for address in addresses:
          ...
          # Ignore the address not in disassembly.
          function = function_map.get(address)
          if function is None:
            continue

          path_group[os.path.realpath(symbol_path)].append(function)

        group_map = {}
        for symbol_path, functions in path_group.items():
          for function in functions:
            function.annotation_mark = annotation_mark_counter

          group_map[symbol_path] = annotation_mark_counter
          annotation_mark_counter += 1

        symbol_path_map[name] = group_map

      annotation_mark = None
      ...
      if len(group_map) > 0:
        if path is None:
          ...
          # No path signature but all symbol signatures of addresses are same.
          # Assume they are the same functions, so there is no ambiguity.
          (annotation_mark,) = group_map.values()
        else:
          annotation_mark = group_map.get(os.path.realpath(path.strip()))

      if annotation_mark is None:
        failed_sigs.add((sig, self.ANNOTATION_ERROR_NOTFOUND))
        continue

      signature_map[sig] = annotation_mark
      annotation_mark_counter += 1
    ...
    for src_sig, dst_sigs in annotation_add_map.items():
      src_mark = signature_map.get(src_sig)
      if src_mark is None:
        continue

      for dst_sig in dst_sigs:
        dst_mark = signature_map.get(dst_sig)
        if dst_mark is not None:
          add_set.add((src_mark, dst_mark))

    for remove_sig in annotation_remove_list:
      remove_mark = signature_map.get(remove_sig)
      if remove_mark is not None:
        remove_set.add(remove_mark)
  ...
  def PreprocessCallGraph(self, function_map, add_set, remove_set):
    ...
    """
    # Build the mark map indexed by annotation mark.
    mark_map = collections.defaultdict(list)
    for function in function_map.values():
      mark = function.annotation_mark
      if mark is not None:
        mark_map[mark].append(function)

    for src_mark, dst_mark in add_set:
      # TODO(cheyuw): Support tailing call annotation.
      src_funcs = mark_map.get(src_mark)
      dst_funcs = mark_map.get(dst_mark)
      # Assume all marks here exist.
      assert src_funcs is not None and dst_funcs is not None
    ...
    for function in function_map.values():
      ...
      for callsite in function.callsites:
        if callsite.callee is not None:
          if callsite.callee.annotation_mark not in remove_set:
