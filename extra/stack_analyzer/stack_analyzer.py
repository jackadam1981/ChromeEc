...
class StackAnalyzer(object):
  ...
  def ResolveAnnotation(self, function_map):
    ...
    for remove_sigs in remove_rules:
      # Since each signature can be mapped to mulitple functions, generate
      # mulitple remove paths from all the combinations of these functions.
      remove_paths = [[]]
      skip_flag = False
      for remove_sig in remove_sigs:
        # Transfrom each signature to the corresponding functions.
        remove_funcs = signature_map.get(remove_sig)
        if remove_funcs is None:
          # Exists an unresolved signature in the remove path. Ignore the whole
          # broken remove path.
          skip_flag = True
          break
        else:
