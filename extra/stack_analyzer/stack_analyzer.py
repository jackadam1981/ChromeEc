...
class StackAnalyzer(object):
  ...
  def AnalyzeDisassembly(self, disasm_text):
    ...
    disasm_iter = iter(disasm_text.splitlines())
    line = next(disasm_iter, None)
    while True:
      if function_symbol is None:
        ...
        if function_symbol is not None:
          # Found the function head, initialize the step 2.
          instructions = []
        ...
        # Move to the next line in the both found and not found cases.
        line = next(disasm_iter, None)
      else:
        ...
        if instruction is None or line is None:
          # Either the instruction is None or the next line is None indicates
          # the end of the function, finalize the function analysis.
  ...
  def AnalyzeCallGraph(self, function_map):
    ...
    def BuildSCC(function, scc_depth, scc_stack, cycle_groups):
      ...
      for callsite in function.callsites:
        ...
        substack = callee.stack_max_usage
      ...
      function.stack_max_usage = function.stack_frame + max_substack
  ...
  def Analyze(self):
    ...
    for task in self.tasklist:
      ...
      print('Task: {}, Max size: {} ({} + 64), Allocated size: {}'.format(
          task.name,
          routine_func.stack_max_usage + 64,
...
def ParseTasklistFile(tasklist_file, symbols):
  ...
  Returns:
    tasklist: Task list.
  """
  tasklist_text = tasklist_file.read()
  # Example: ("HOOKS",hook_task,LARGER_TASK_STACK_SIZE) ("USB_CHG_P0", ...
  results = re.findall(r'\("([^"]+)", ([^,]+), ([^\)]+)\)', tasklist_text)
  tasklist = []
...
def main():
  ...
  try:
    ...
    # Parse the tasklist file.
    try:
      with open(options.tasklist_path, 'r') as tasklist_file:
        tasklist = ParseTasklistFile(tasklist_file, symbols)
