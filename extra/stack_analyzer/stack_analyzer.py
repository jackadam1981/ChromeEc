...
def ParseArgs():
  ...
  parser.add_argument('dump_taskinfo', help="the path of ec_dump_taskinfo")
  parser.add_argument('--section', default='RW',
                      help='the section. the default is RW.',
...
def ParseTasklistText(taskinfo_text, symbols):
  """Parse the task information.

  Args:
    taskinfo_text: Text of the taskinfo.
    symbols: Symbol list.
...
def main():
  ...
  try:
    ...
    # Generate and parse the taskinfo.
    try:
      taskinfo_text = subprocess.check_output([options.dump_taskinfo,
                                               '-s',
                                               options.section])
    except subprocess.CalledProcessError:
      raise StackAnalyzerError('ec_dump_taskinfo failed to dump taskinfo.')
    except OSError:
      raise StackAnalyzerError('Failed to run ec_dump_taskinfo.')

    tasklist = ParseTasklistText(taskinfo_text, symbols)
