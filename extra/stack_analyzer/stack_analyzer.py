...
class StackAnalyzer(object):
  ...
  ANNOTATION_ERROR_DUPLICATE = 'remove path is invalid because of duplication'
  ...
  def ResolveAnnotation(self, function_map):
    ...
    for remove_sigs in remove_rules:
      ...
      # For our simplifed KMP, only allow the remove path to have duplicated
      # functions at the head and tail.
      # TODO(cheyuw): Implement real KMP algorithm and remove this restriction.
      duplicate_flag = False
      for remove_path in remove_paths:
        middle_set = set(remove_path[1:-1])
        # Expect of the head and tail, the middle part should be unique.
        if len(middle_set) != len(remove_path[1:-1]):
          duplicate_flag = True
          break

        if remove_path[0] in middle_set or remove_path[-1] in middle_set:
          duplicate_flag = True
          break

      if duplicate_flag:
        invalid_remove_sigs.append(remove_sigs)
      else:
        remove_list.extend(remove_paths)
    ...
    for remove_sigs in invalid_remove_sigs:
      sigtxt = ' -> '.join(StringifySignature(sig) for sig in remove_sigs)
      failed_sigtxts.add((sigtxt, self.ANNOTATION_ERROR_DUPLICATE))
  ...
  def PreprocessAnnotation(self, function_map, add_set, remove_list,
    ...
    less than three vertices) from the function_map.
    ...
    for remove_path in remove_list:
      if len(remove_path) == 1:
        remove_set.add((None, remove_path[0]))
      elif len(remove_path) == 2:
        remove_set.add((remove_path[0], remove_path[1]))

    edge_set = set()
    for function in function_map.values():
      cleaned_callsites = []
      for callsite in function.callsites:
        if ((None, callsite.callee) in remove_set or
            (function, callsite.callee) in remove_set):
          continue
    ...
    for remove_path in remove_list:
      if len(remove_path) < 3:
        continue

      skip_flag = False
      for index in range(len(remove_path) - 1):
        if (remove_path[index], remove_path[index + 1]) not in edge_set:
          skip_flag = True
          break
      ...
      for prev_path in remaining_remove_list:
        if remove_path == prev_path:
          skip_flag = True
          break

      if not skip_flag:
        remaining_remove_list.append(remove_path)

    return remaining_remove_list
  ...
  def AnalyzeCallGraph(self, function_map, remove_list):
    ...
    def Traverse(curr_state):
      ...
      for index, position in enumerate(curr_positions):
        path = remove_list[index]
        if path[position] is not curr_func:
          # The matching is aborted, reset the pointer. Assume the remove path
          # only can have duplicated functions at the head and tail, so we do
          # the simplify KMP algorithm by hands.
          if path[0] is curr_func:
            # The current function matches the head of remove path, move to the
            # next one.
            new_positions[index] = 1
          else:
            # Doesn't match, skip the current function.
            new_positions[index] = 0
        else:
          if position + 1 == len(path):
            # Matched the remove path.
            invalid_flag = True
            break
          else:
            # Matched the pointed function, move to the next one.
      ...
      if not invalid_flag:
        ...
        for callsite in curr_func.callsites:
          ...
          if done_result is not None:
            # Alreay done this state and use its result. If the state reaches a
      ...
      if curr_positions == initial_positions:
        ...
        # the function.
        # If the length of remove path is one. The stack usage of matched
        # functions will be set to 0, which is not expected (we still calculate
        # their stack usages, but no one will call them). Those remove paths
        # are preprocessed and filtered out, so it won't happen.
