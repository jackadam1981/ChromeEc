  ...
  std::vector<Constant *> concretizeValue(Value *V, const DataLayout &DL) {
    ...
    if (auto *LI = dyn_cast<LoadInst>(V)) {
      ...
      for (Constant *Ptr : Ptrs) {
        ...
        if (auto *C = ConstantFoldLoadFromConstPtr(Ptr, LI->getType(), DL)) {
          AllCVs.push_back(C);
        } else {
          bool Found = false;
          if (auto *CE = dyn_cast<ConstantExpr>(Ptr)) {
            if (CE->getOpcode() == Instruction::GetElementPtr) {
              if (auto *PCE = dyn_cast<ConstantExpr>(CE->getOperand(0))) {
                // Try to dereference the identical but renamed structure.
                if (PCE->getOpcode() == Instruction::BitCast) {
                  PointerType *SrcPtrTy = dyn_cast<PointerType>(
                      PCE->getOperand(0)->getType());
                  PointerType *DstPtrTy = dyn_cast<PointerType>(PCE->getType());

                  if (SrcPtrTy && DstPtrTy) {
                    // The first operand of GEP is a pointer.
                    TypePair StackedTy;
                    if (isLayoutIdentical(SrcPtrTy->getElementType(),
                                          DstPtrTy->getElementType(),
                                          &StackedTy)) {
                      auto *NewGEP = CE->getWithOperandReplaced(
                          0, PCE->getOperand(0));
                      if (auto *C = ConstantFoldLoadFromConstPtr(NewGEP,
                                                                 LI->getType(),
                                                                 DL)) {
                        AllCVs.push_back(C);
                        Found = true;
        ...
        for (Instruction &I : BB) {
          if (auto CI = dyn_cast<CallInst>(&I)) {
            if (auto VI = dyn_cast<Instruction>(CI->getCalledValue())) {
              ...
              bool Error = false;
              std::vector<DISubprogram *> Targets;
              ...
              if (!Error) {
                errs() << F.getName() << "[" << DL->getFilename() << ":" <<
                       DL->getLine() << "]:\n";
                for (DISubprogram *DS : Targets) {
                  errs() << "- " << DS->getName() << "[" << DS->getFilename() <<
                         ":" << DS->getLine() << "]\n";
                }
              }
            }
