...
from stack_analyzer import ArmAnalyzer, StackAnalyzer
from stack_analyzer import Task, Function, Symbol, Callsite, StackAnalyzerError
import subprocess
import tempfile
import unittest
...
class StackAnalyzerTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    with tempfile.NamedTemporaryFile(mode='w+') as tasklist_file:
      tasklist_file.write('("HOOKS", hook_task, 2048) '
                          '("CONSOLE", console_task, STACK_SIZE)')
      tasklist_file.flush()
      tasklist_file.seek(0)
      ...
      with patch('subprocess.check_call') as checkall_mock:
        ...
        options = MagicMock(elf_path='./ec.RW.elf',
                            tasklist_path=tasklist_file.name,
                            objdump='objdump',
                            addr2line='addr2line')
        self.analyzer = StackAnalyzer(options)

  @patch('subprocess.check_call')
  def testConstructor(self, checkall_mock):
    def fakeObjdumpSymbol(args, stdout):
      # pylint: disable=unused-argument
      stdout.write('1000 g     F .text  0000015c .hidden hook_task\n'
                   '2000 g     F .text  0000051c .hidden console_task\n')
    ...
    with self.assertRaisesRegex(StackAnalyzerError,
                                'objdump failed to dump symbol table.'):
    ...
    with self.assertRaisesRegex(StackAnalyzerError, 'Failed to open tasklist.'):
      checkall_mock.side_effect = fakeObjdumpSymbol
      self.analyzer = StackAnalyzer(options)
  ...
  def testParseSymbolFile(self):
    with tempfile.TemporaryFile(mode='w+') as temp:
      ...
      temp.flush()
      temp.seek(0)
      symbols = StackAnalyzer.ParseSymbolFile(temp)
      expect_symbols = [
  ...
  def testParseTasklist(self):
    with tempfile.TemporaryFile(mode='w+') as temp:
      ...
      temp.flush()
      temp.seek(0)
      tasklist = StackAnalyzer.ParseTasklistFile(temp, self.analyzer.symbols)
      expect_tasklist = [
  ...
  def testAnalyzeCallGraph(self):
    ...
    for func in funcs.values():
      ...
      self.assertEqual(set(scc_group[func.scc_index]), scc)
