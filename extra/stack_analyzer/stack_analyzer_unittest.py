...
class StackAnalyzerTest(unittest.TestCase):
  ...
  def testResolveAnnotation(self):
    ...
    self.analyzer.annotation = {
        'add': {
            ...
            'console_task': ['touchpad_calc[b.c]'],
    ...
    self.assertEqual(funcs[0x13000].annotation_mark,
                     funcs[0x13100].annotation_mark)
    self.assertEqual(add_set, {
        (funcs[0x1000].annotation_mark, funcs[0x4000].annotation_mark),
        (funcs[0x1000].annotation_mark, funcs[0x1000].annotation_mark),
        (funcs[0x2000].annotation_mark, funcs[0x5000].annotation_mark),
        (funcs[0x13000].annotation_mark, funcs[0x13000].annotation_mark),
    })
    self.assertEqual(remove_set, {funcs[0x4000].annotation_mark})
  ...
  def testPreprocessCallGraph(self):
    ...
    funcs[0x1000].annotation_mark = 0
    funcs[0x2000].annotation_mark = 1
    funcs[0x4000].annotation_mark = 2
    funcs[0x13000].annotation_mark = 3
    funcs[0x13100].annotation_mark = 3
    add_set = {(1, 2), (2, 0), (3, 3)}
    remove_set = {0}
    ...
    expect_calltargets = {
        ...
        0x2000: {0x4000},
        0x4000: set(),
        0x13000: {0x13000, 0x13100},
        0x13100: {0x13000, 0x13100},
    }
    for address, func in funcs.items():
      self.assertEqual(set(callsite.target for callsite in func.callsites),
                       expect_calltargets[address])
