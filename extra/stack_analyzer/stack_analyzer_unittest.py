...
class StackAnalyzerTest(unittest.TestCase):
  ...
  def testParseTasklist(self):
    with tempfile.TemporaryFile(mode='w+') as temp:
      ...
      for task_a, task_b in zip(tasklist, expect_tasklist):
        ...
        self.assertEqual(task_a.stack_max_size, task_b.stack_max_size)
  ...
  def testAnalyze(self, checkall_mock, popen_mock):
    ...
    with self.assertRaisesRegexp(StackAnalyzerError,
      ...
      checkall_mock.side_effect = subprocess.CalledProcessError(1, "")
      self.analyzer.Analyze()
