...
def do_reboot(pty, serialname):
    ...
    devno = c.check_usb_dev(set({STM_VIDPID}), serialname=serialname)
    if not devno:
    ...
    try:
        c.wait_for_usb_remove(set({STM_VIDPID}), timeout=3.0, serialname=serialname)
    except Exception as e:
    ...
    try:
        c.wait_for_usb(set({STM_VIDPID}), timeout=3.0, serialname=serialname)
    except Exception as e:
    ...
    newdevno = c.check_usb_dev(set({STM_VIDPID}), serialname=serialname)
    if newdevno == devno:
...
def main(argv):
    ...
    c.wait_for_usb(set({STM_VIDPID}), timeout=5.0, serialname=opts.serialno)
