...
struct mpsse_context* MPSSE(enum modes mode, int freq, int endianess) {
  ...
  for (i = 0; supported_devices[i].vid != 0; i++) {
    mpsse = Open(supported_devices[i].vid, supported_devices[i].pid, mode, freq,
                 endianess, IFACE_B, NULL, "905537-00474");
    if (mpsse) {
...
struct mpsse_context* OpenIndex(int vid,
  ...
  status = ftdi_usb_open_desc_index(&mpsse->ftdi, vid, pid, description, serial,
                                  index);
  if (status != -3)
	  printf("00 - open %04x:%04x serial %s index %d => %d:%s\n", vid, pid, serial, index, status,
                 ftdi_get_error_string(&mpsse->ftdi));
