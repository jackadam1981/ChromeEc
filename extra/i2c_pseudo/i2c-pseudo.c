	...
	size_t in_size, bool non_blocking)
{
	...
	default:
		 ...
		 */
		ret = -ENOTRECOVERABLE;
		pr_err("%s: cmd_data->state is unexpectedly %d.  This is a bug."
	...
	size_t in_size, bool non_blocking)
{
	...
	if (!(i2c_msg->flags & I2C_M_RD)) {
		...
		ret = -EIO;
		pr_warn("%s: I2C controller responded to a write with data.  "
			"Will return %d.\n", __func__, ret);
		goto unlock_and_return;
	}
	...
	while (in_size > 0 && cmd_data->current_buf_idx < i2c_msg->len) {
		...
		if (in_size < 2 ||
			 ...
			 */
			ret = -EIO;
			pr_warn("%s: I2C controller responded with a null "
				"character in I2C data.  Will return %d.\n",
				__func__, ret);
			goto unlock_and_return;
		}
		/*
	...
	bool non_blocking)
{
	...
	if (mxfer_reply != NULL) {
		...
		if (ret) {
			if (ret > 0)
				ret = -EINTR;
			pr_warn("%s: mxfer_reply->lock acquisition interrupted "
				"by signal, returning %d.\n", __func__, ret);
		} else if (mxfer_reply->completed[cmd_data->current_msg_idx]) {
			/* We already received a reply for this msg. */
			ret = -EIO;
			pr_warn("%s: I2C controller responded multiple times "
				"for same I2C msg request.  Will return %d.\n",
				__func__, ret);
		} else {
	...
	if (mxfer_reply == NULL) {
		ret = -EIO;
		pr_warn("%s: I2C controller responded for an unknown I2C msg "
			"request.  This could be caused by the controller "
			"responding too slowly, after the I2C adapter timeout."
			"  Will return %d.\n", __func__, ret);
		goto reset_cmd_data_and_return;
	}
	if (ret) {
	...
	struct i2c_msg *msgs, int num)
{
	...
	default:
		...
		ret = -ENOTRECOVERABLE;
		pr_err("%s: Unexpected struct i2c_pseudo_controller."
			"startstop_state value %d.  This is a bug.  Will "
			"return %d.\n", __func__, i, ret);
		goto just_return;
	}
