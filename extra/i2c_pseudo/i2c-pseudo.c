	...
	size_t in_size, bool non_blocking)
{
	...
	default:
		/* Reaching here is a bug. */
		ret = -ENOTRECOVERABLE;
		pr_err("%s: cmd_data->state is unexpectedly %d.  "
	...
	default:
		/* Reaching here is a bug. */
		ret = -ENOTRECOVERABLE;
		pr_err("%s: cmd_data->state is %d, expected %d.  "
			"This is a bug.  Will return %d.\n", __func__,
			cmd_data->state,
	...
	bool non_blocking)
{
	...
	mxfer_reply = cmd_data->reply_queue_current_item;
	if (mxfer_reply != NULL) {
		++cmd_data->num_completed_msgs;
		ret = mutex_lock_killable(&mxfer_reply->lock);
		if (!ret &&
		    cmd_data->num_completed_msgs >= mxfer_reply->num_msgs) {
	 ...
	 */
	if (cmd_data->state == I2C_PSEUDO_CMD_MXFER_REPLY_STATE_DATA_NEXT)
		++cmd_data->current_msg_idx;

	if (cmd_data->current_msg_idx >= mxfer_reply->num_msgs)
		complete_all(&mxfer_reply->data_filled);
