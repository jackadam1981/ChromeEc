...
static ssize_t i2c_pseudo_rsp_master_xfer_formatter(void *data, char **out)
{
	...
	if (mxfer_rsp->num_msgs_done >= mxfer_rsp->num) {
		pr_debug("%s: Returning zero after matching (mxfer_rsp->num <= "
	...
	if (mxfer_rsp->buf_start_plus_one <= 0) {
		pr_debug("%s: Condition (mxfer_rsp->buf_start_plus_one <= 0) "
		...
		pr_debug("%s: anprintf() for I2C message header returned %zd.  "
			"data=%p buf_start=%p\n", __func__, ret, data,
			buf_start);
		if (ret > 0) {
			*out = buf_start;
	...
	pr_debug("%s: Attempting to kzalloc() a data bytes read response "
	...
	pr_debug("%s: Successfully kzalloc()'d a data bytes read response "
		"buffer.  data=%p mxfer_rsp->num=%d "
		"mxfer_rsp->num_msgs_done=%d mxfer_rsp->buf_start_plus_one=%zu "
		"i2c_msg->addr=0x%04X i2c_msg->flags=0x%04X i2c_msg->len=%u "
...
#if 0
		pr_debug("%s: data bytes read response buffer write loop: buf_start=%p buf_pos=%p i=%zu byte_limit=%zu\n", __func__, buf_start, buf_pos, i, byte_limit);
		pr_debug("%s: data bytes read response buffer write loop: buf_start=%p buf_pos=%p i=%zu byte_limit=%zu sep_char=%hhu\n", __func__, buf_start, buf_pos, i, byte_limit, (char)((i || byte_start) ? i2c_pseudo_ctrlr_data_sep_char : i2c_pseudo_ctrlr_header_sep_char));
		pr_debug("%s: data bytes read response buffer write loop: buf_start=%p buf_pos=%p i=%zu byte_limit=%zu byte=0x%02X\n", __func__, buf_start, buf_pos, i, byte_limit, i2c_msg->buf[byte_start + i]);
#endif
	...
	pr_debug("%s: Successfully wrote data bytes to a read response buffer.  "
	...
	*out = buf_start;
	pr_debug("%s: Successfully wrote data byte read response buffer address "
 ...
 definitely_free_and_definitely_return:
			pr_debug("%s: Attempting to kfree(mxfer_rsp->msgs).  "
			...
			pr_debug("%s: Attempting to kfree(mxfer_rsp).  "
				"data=%p\n", __func__, data);
			kfree(mxfer_rsp);
		/*
		 ...
		 */
			...
			pr_debug("%s: Attempting to kfree(mxfer_rsp->msgs) and "
	...
	size_t in_size, bool non_blocking)
{
	...
	cmd_data = data;
	pr_debug("%s called for data=%p in=%p in_size=%zu in=\"%*pE\"\n",
	...
	pr_debug("%s successfully acquired cmd_data->reply_queue_lock for "
	...
	int receive_status, bool non_blocking)
{
	...
	cmd_data = data;
	pr_debug("%s called for data=%p receive_status=%d\n", __func__, data,
		receive_status);
	...
	i2c_msg = &mxfer_reply->msgs[mxfer_reply->current_msg_idx];
	pr_debug("%s: mxfer_reply=%p mxfer_reply->id=%u mxfer_reply->ret=%d "
		...
		struct i2c_msg *msgs, int num)
{
	...
	struct i2c_pseudo_cmd_mxfer_reply_data *cmd_data;
	struct i2c_pseudo_cmd_mxfer_reply *mxfer_reply;
	pr_debug("i2c_pseudo_adapter_master_xfer(adap=%p, msgs=%p, num=%d) "
	...
	pr_debug("%s: For I2C pseudo adapter %d list_empty("
...
{
	struct i2c_pseudo_controller *pdata;
	pr_debug("i2c_pseudo_adapter_smbus_xfer(adap=%p, addr=0x%02X, "
...
static int i2c_pseudo_cdev_open(struct inode *inodep, struct file *filep)
{
	...
	struct i2c_pseudo_controller *pdata;
	pr_debug("i2c_pseudo_cdev_open(inodep=%p, filep=%p) called\n", inodep,
		filep);
 ...
 unlock_counters:
	...
	pr_debug("%s: i2c_add_adapter() for inodep=%p filep=%p successfully "
...
static int i2c_pseudo_cdev_release(struct inode *inodep, struct file *filep)
{
	...
	pr_debug("i2c_pseudo_cdev_release(inodep=%p, filep=%p) called for I2C "
	...
	struct i2c_pseudo_cmd_mxfer_reply *mxfer_reply;
	cmd_data = pdata->cmd_data[I2C_PSEUDO_CMD_MXFER_REPLY_IDX];
	mxfer_reply->data_filled
#endif
#if 0
	/* Deliberately ignore the return value. */
	i2c_pseudo_adap_set_shutdown(pdata);
#endif
	/*
		...
		size_t count, loff_t *f_ps)
{
	...
	pdata = filep->private_data;
	pr_debug("i2c_pseudo_cdev_read(filep=%p, buf=%p, count=%zu, f_ps=%p) "
	...
	if (pdata->rsp_buf_remaining < 0) {
		BUILD_BUG_ON(ret != 0);
#if 0
		pr_debug("%s: Error code %zd left in "
	...
	while (count > 0) {
		...
		while (pdata->rsp_buf_remaining == 0) {
#if 0
			pr_debug("%s: Looking for current read response "
			...
			if (!list_empty(&pdata->read_rsp_queue_head))
				...
				pr_debug("%s: Encountered empty read response "
...
#if 0
			pr_debug("%s: rsp_wrapper->formatter() returned %zd and "
	...
	bool non_blocking)
{
	...
	pr_debug("%s(%p, %p, %zu) called for buffer \"%*pE\" with existing "
	...
	pr_debug("%s: Will copy %zu header bytes from the %zu remaining "
	...
	bool non_blocking)
{
	...
	pr_debug("%s: Will copy %zu data bytes from the %zu remaining "
		...
		size_t count, loff_t *f_ps)
{
	...
	non_blocking = !!(filep->f_flags & O_NONBLOCK);
	pdata = filep->private_data;
	pr_debug("%s(filep=%p, buf=%p, count=%zu, f_ps=%p) with non_blocking=%u "
	...
	pr_debug("%s(filep=%p, buf=%p, count=%zu, f_ps=%p) called for buffer "
	...
	if (non_blocking) {
		if (!mutex_trylock(&pdata->cmd_lock)) {
			...
			ret = -EAGAIN;
			pr_debug("%s: Could not immediately acquire I2C adapter "
				"%d cmd_lock for O_NONBLOCK write, returning "
				"%zd.\n", __func__, pdata->i2c_adapter.nr, ret);
			goto free_and_return;
		}
	} else {
...
static int __init i2c_pseudo_init(void)
{
	...
	pr_debug("%s: alloc_chrdev_region(&dev, " STR(I2C_PSEUDO_CDEV_BASEMINOR)
