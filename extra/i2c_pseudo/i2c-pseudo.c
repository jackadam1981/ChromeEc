...
/*
 * Marks the end of a controller command or read response.  These are constants
 * so
 *
...
/* TODO: Move the integer conversion functions somewhere?  If so, where? */

/*
 * Convert int to ssize_t if it fits, else return a negative error number.
 *
 * This is intended for return values where negative values are error numbers.
 * Thus, positive values that are out of range become -EOVERFLOW, whereas
 * negative values that are out of range become -ENOTRECOVERABLE.
 *
 * This should be optimized away as a no-op when compiled for architectures
 * where sizeof(ssize_t) >= sizeof(int).
 */
static inline ssize_t i2c_pseudo_int_to_ssize_t(int value)
{
	if (value < SSIZE_MIN)
 ...
 */
static inline int i2c_pseudo_ssize_t_to_int(ssize_t value)
{
	if (value < INT_MIN)
		return -ENOTRECOVERABLE;
	if (value > INT_MAX)
 ...
 */
static inline ssize_t i2c_pseudo_long_to_ssize_t(long value)
{
	if (value < SSIZE_MIN)
		return -ENOTRECOVERABLE;
	if (value > SSIZE_MAX)
 ...
 */
static inline int i2c_pseudo_long_to_int(long value)
{
	if (value < INT_MIN)
		return -ENOTRECOVERABLE;
	if (value > INT_MAX)
...
/* TODO: Consider moving kvanprintf() and kanprintf() to lib/kasprintf.c */
/*
 * kvanprintf - Format a string and place it into a newly allocated buffer.
 ...
 *        a bug.
 */
static ssize_t kvanprintf(char **out, ssize_t max_size, const char *fmt,
	va_list args0)
{
	int ret;
	ssize_t buf_size;
	...
	if (ret < 0) {
		pr_err("%s: Reached success section with negative return value %d set.  This is a bug.  Will _not_ return an output buffer in *out.\n",
			__func__, ret);
		goto fail_after_args1;
	}
	*out = buf;
...
/*
 * kanprintf - Format a string and place it into a newly allocated buffer.
 * @out: Address of the pointer to place the buffer address into.  Will only be
 *     written to with a successful positive return value.
 * @max_size: If non-negative, the maximum buffer size that this function will
 *     attempt to allocate.  If the formatted string including trailing null
 ...
 *        a bug.
 */
static ssize_t kanprintf(char **out, ssize_t max_size, const char *fmt, ...)
{
	ssize_t ret;
	va_list args;

	va_start(args, fmt);
	ret = kvanprintf(out, max_size, fmt, args);
	va_end(args);
	return ret;
}
...
static ssize_t i2c_pseudo_rsp_master_xfer_formatter(void *data, char **out)
{
	...
	if (mxfer_rsp->buf_start_plus_one <= 0) {
		 ...
		 */
		ret = kanprintf(&buf_start, I2C_PSEUDO_MAX_MSG_BUF_SIZE,
		 ...
		 */
			...
			ret = -ENOTRECOVERABLE;
			pr_err("%s: Unexpectedly received zero formattered characters / one buffer size (including null terminator) from kanprintf(), freeing the buffer and returning %zd.\n",
				__func__, ret);
			kfree(buf_start);
		}
		goto maybe_free_and_definitely_return;
	...
	bool non_blocking)
{
	...
	ret = kanprintf(&rsp_buf->buf, I2C_PSEUDO_MAX_MSG_BUF_SIZE, "%*s%c%d",
	...
	if (ret < 0) {
		...
		ret = -ENOTRECOVERABLE;
		pr_debug("%s: Unexpectedly received zero formattered characters / one buffer size (including null terminator) from kanprintf(), freeing the buffer and returning %zd.\n",
			__func__, ret);
	...
	struct i2c_msg *msgs, int num)
{
	...
	default:
		...
		ret = -ENOTRECOVERABLE;
		pr_err("%s: Unexpected struct i2c_pseudo_controller.startstop_state value %d.  This is a bug.  Will return %d.\n",
			__func__, i, ret);
		goto just_return;
	}
...
static int i2c_pseudo_cdev_open(struct inode *inodep, struct file *filep)
{
	...
	for (ctrlr_id = i2c_pseudo_counters.next_ctrlr_id;;) {
		...
		if (++ctrlr_id == i2c_pseudo_counters.next_ctrlr_id) {
			pr_err("%s: Cycled through every possible I2C pseudo controller ID without finding a free one.  This is implausible, and may indicate a bug.  i2c_pseudo_counters.count=%u\n",
				__func__, i2c_pseudo_counters.count);
			ret = -ENOSPC;
			break;
		}
	}
...
static inline void i2c_p_device_remove_file_count(void)
{
	device_remove_file(&i2c_pseudo_device, &i2c_pseudo_count_dev_attr);
}

static inline void i2c_p_device_remove_file_limit(void)
{
	device_remove_file(&i2c_pseudo_device, &i2c_pseudo_limit_dev_attr);
}

static inline void i2c_p_device_delete(void)
{
	device_del(&i2c_pseudo_device);
}

static inline void i2c_p_device_put(void)
{
	put_device(&i2c_pseudo_device);
	memset(&i2c_pseudo_device, 0, sizeof(i2c_pseudo_cdev));
}

static inline void i2c_p_cdev_delete(void)
{
	cdev_del(&i2c_pseudo_cdev);
}

static inline void i2c_p_cdev_put(bool cdev_del_called)
{
	/*
...
static inline void i2c_p_chrdev_deregister(void)
{
...
static int __init i2c_pseudo_init(void)
{
	...
	if (ret < 0) {
		pr_err("%s: device_create_file(&i2c_pseudo_device, &i2c_pseudo_limit_dev_attr) failed with error %d\n",
			__func__, -ret);
	...
	if (ret < 0) {
		pr_err("%s: device_create_file(&i2c_pseudo_device, &i2c_pseudo_count_dev_attr) failed with error %d\n",
			__func__, -ret);
 ...
 fail_beginning:
	if (ret >= 0) {
		pr_err("%s: Jumped to failure cleanup section with non-negative return value %d set.  This is a bug.  Will return -1 instead.\n",
			__func__, ret);
		ret = -1;
	}
 just_return:
 ...
 */
static inline void i2c_pseudo_build_assertions(void)
{
	BUILD_BUG_ON(ARRAY_SIZE(i2c_pseudo_cmds) <= 0);
	BUILD_BUG_ON(ARRAY_SIZE(i2c_pseudo_cmds) >
		I2C_PSEUDO_CMDS_SANITY_LIMIT);
	BUILD_BUG_ON(ARRAY_SIZE(i2c_pseudo_cmds) != I2C_PSEUDO_NUM_WRITE_CMDS);

	BUILD_BUG_ON(I2C_PSEUDO_DEFAULT_LIMIT < I2C_PSEUDO_ADAPTERS_MIN);
	BUILD_BUG_ON(I2C_PSEUDO_DEFAULT_LIMIT > I2C_PSEUDO_ADAPTERS_MAX);
	 ...
	 */
	BUILD_BUG_ON(FIELD_SIZEOF(struct i2c_pseudo_controller, cmd_buf) !=
		I2C_PSEUDO_CTRLR_CMD_BUF_SIZE);
}
