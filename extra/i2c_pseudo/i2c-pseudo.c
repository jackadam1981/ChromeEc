 ...
 */
/* TODO: Audit EIO uses and replace ones that indicate a userspace controller bug with read response errors. */
...
#if 0
	/* Same type as struct i2c_algorithm.master_xfer @num arg. */
	int num_completed_msgs;
#endif

	i2c_pseudo_mxfer_id_t current_id;
...
inline static bool i2c_pseudo_mxfer_reply_is_current(
	struct i2c_pseudo_cmd_mxfer_reply_data *cmd_data,
	struct i2c_pseudo_cmd_mxfer_reply *mxfer_reply)
{
	int i;
...
inline static struct i2c_pseudo_cmd_mxfer_reply
...
/* Upon success cmd_data->reply_queue_current_item->lock will be held. */
inline static int i2c_pseudo_mxfer_reply_update_current(
	struct i2c_pseudo_cmd_mxfer_reply_data *cmd_data)
{
	int ret;
	struct i2c_pseudo_cmd_mxfer_reply *mxfer_reply;
	...
	mxfer_reply = i2c_pseudo_mxfer_reply_find_current(cmd_data);
	if (mxfer_reply != NULL)
		ret = mutex_lock_killable(&mxfer_reply->lock);
	mutex_unlock(&cmd_data->reply_queue_lock);
	if (mxfer_reply == NULL)
		/*
		 * The cmd_completer callback will indicate this failure to the
		 * controller.
		 */
		return 0;
	...
	cmd_data->reply_queue_current_item = mxfer_reply;
	return 0;
}
	...
	size_t in_size, bool non_blocking)
{
	...
	case I2C_PSEUDO_CMD_MXFER_REPLY_STATE_CMD_NEXT:
		...
		cmd_data->state = I2C_PSEUDO_CMD_MXFER_REPLY_STATE_ADDR_NEXT;
		return 0;
	...
	struct i2c_msg *msgs, int num)
{
	...
	if (mxfer_reply->completed == NULL) {
		pr_err("%s: kcalloc(%d, %zu, GFP_KERNEL) returned NULL\n",
			__func__, num, sizeof(*mxfer_reply->completed));
		ret = -ENOMEM;
...
inline static void i2c_pseudo_build_assertions(void)
{
	...
	/* These types must be unsigned. */
	BUILD_BUG_ON((i2c_pseudo_ctrlr_id_t)0 - 1 < (i2c_pseudo_ctrlr_id_t)0);
	BUILD_BUG_ON((i2c_pseudo_mxfer_id_t)0 - 1 < (i2c_pseudo_mxfer_id_t)0);

	/*
	 * The integer wrapping detection performed when processing xfer replies
	 * from the userspace controller depends on this assertion.
	 */
	BUILD_BUG_ON(I2C_PSEUDO_CTRLR_RSP_QUEUE_LIMIT >=
		~(i2c_pseudo_mxfer_id_t)0 >> 1);
