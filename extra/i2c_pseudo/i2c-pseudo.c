...
struct i2c_pseudo_adap_shutdown_data {
	/* TODO: Consider making this a RW lock. */
	struct mutex lock;
	bool is_shutdown;
};
...
{
	int ret;
	struct i2c_pseudo_adap_shutdown_data *shutdown_data;
	shutdown_data = pdata->cmd_data[I2C_PSEUDO_CMD_ADAP_SHUTDOWN_IDX];
	if (mutex_lock_killable(&shutdown_data->lock)) {
		ret = -EINTR;
		pr_warn("%s: I2C adapter %d shutdown_data->lock acquisition "
			"interrupted by signal, returning %d.\n",
			__func__, pdata->i2c_adapter.nr, ret);
	} else {
		ret = !!shutdown_data->is_shutdown;
		mutex_unlock(&shutdown_data->lock);
	}
	return ret;
}
...
static int i2c_pseudo_adap_set_shutdown(struct i2c_pseudo_controller *pdata)
{
	...
	if (mutex_lock_killable(&shutdown_data->lock)) {
		...
		shutdown_data->is_shutdown = true;
		mutex_unlock(&shutdown_data->lock);
		ret = 0;
	}
	return ret;
...
static int i2c_pseudo_cmd_adap_shutdown_data_creator(void **data,
	struct i2c_pseudo_controller *pdata)
{
	struct i2c_pseudo_adap_shutdown_data *shutdown_data;
	shutdown_data = kzalloc(sizeof(*shutdown_data), GFP_KERNEL);
	if (shutdown_data == NULL) {
		pr_err("%s: kzalloc(%zu, GFP_KERNEL) returned NULL\n", __func__,
			sizeof(*shutdown_data));
		return -ENOMEM;
	}
	mutex_init(&shutdown_data->lock);
	*data = shutdown_data;
	return 0;
}

static void i2c_pseudo_cmd_adap_shutdown_data_destroyer(void *data)
{
	kfree(data);
}
...
{
	int ret = 0;
	ret = i2c_pseudo_adap_set_shutdown();
	if (ret < 0)
		goto just_return;
...
/* TODO: Implement handling of an error write command from the controller. */
...
#endif
	{
		...
		.data_creator = i2c_pseudo_cmd_adap_shutdown_data_creator,
		.data_destroyer = i2c_pseudo_cmd_adap_shutdown_data_destroyer,
...
static int i2c_pseudo_cdev_release(struct inode *inodep, struct file *filep)
{
	...
	/* The I2C adapter could be in use up until i2c_del_adapter(). */
		...
		size_t count, loff_t *f_ps)
{
	...
	while (count > 0) {
		...
		while (pdata->rsp_buf_remaining == 0) {
			...
			mutex_unlock(&pdata->read_rsp_queue_lock);
			if (rsp_wrapper == NULL) {
				if (!i2c_pseudo_adap_is_shutdown(pdata)) {
					ret = -ENOTRECOVERABLE;
		...
		size_t count, loff_t *f_ps)
{
	 ...
	 * message, even if blocking is permitted.
	 */
	non_blocking = !!(filep->f_flags & O_NONBLOCK);
	pdata = filep->private_data;
