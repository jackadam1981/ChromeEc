 ...
 * not use or expect null, but they still must handle it gracefully and fail
...
struct i2cp_cmd {
	 ...
	 * The return value should be less than I2CP_CTRLR_CMD_BUF_SIZE, as that
...
static int i2cp_adap_get_state(struct i2cp_controller *pdata)
{
	int ret;
...
static inline int i2cp_mxfer_reply_update_current(
	struct i2cp_cmd_mxfer_reply_data *cmd_data)
{
	int ret;
	...
	size_t in_size, bool non_blocking)
{
	...
	case I2CP_CMD_MXFER_REPLY_STATE_ERRNO_NEXT:
		 ...
		 * mxfer_reply.
		 */
		ret = i2cp_mxfer_reply_update_current(cmd_data);
		if (ret < 0)
			return ret;
		break;
	default:
	...
	ssize_t *ret, bool non_blocking, struct i2cp_controller *pdata)
{
	...
	while (pdata->rsp_buf_remaining == 0) {
		...
		mutex_unlock(&pdata->read_rsp_queue_lock);
		if (!rsp_wrapper) {
			int_ret = i2cp_adap_get_state(pdata);
			if (int_ret < 0) {
				if (*ret == 0)
					*ret = int_ret;
...
static __poll_t i2cp_cdev_poll(struct file *filep, poll_table *ptp)
{
	...
	if (int_ret < 0)
		 ...
		 * from i2cp_adap_get_state() indicates.  POLLERR is used here
		 * to punt the decision of whether and when to retry polling to
		 * the userspace process, without falsely claiming that the
		 * file descriptor is readable, writable, or will return EOF.
		 *
		 * At the time of this writing, i2cp_adap_get_state() can only
		 * return negative (error) upon killable signal, so what we
		 * return here should be irrelevant.  However that is
		 * subject to change!
		 */
		poll_ret |= POLLERR;
	else if (int_ret == I2CP_CTRLR_STATE_SHUTDN_REQ)
