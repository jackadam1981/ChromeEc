...
#define I2CP_CTRLR_CMD_BUF_SIZE		256
 ...
 */
static unsigned int i2c_pseudo_limit = I2CP_DEFAULT_LIMIT;
 ...
 */
static unsigned int i2c_pseudo_default_timeout_ms = I2CP_DEFAULT_TIMEOUT_MS;
...
struct i2c_pseudo_cmd {
	 ...
	 * I2CP_CTRLR_CMD_BUF_SIZE, as that minus one is the maximum that will
	 * ever be buffered at once, and thus the maximum that will ever be sent
	 * to a single invocation of data_receiver.
	 *
	 ...
	 * either the chunk size is >= I2CP_CTRLR_CMD_BUF_SIZE, or
...
struct i2c_pseudo_controller {
	...
	size_t cmd_size;
	char cmd_buf[I2CP_CTRLR_CMD_BUF_SIZE];
	...
	bool non_blocking)
{
	...
	ret = anprintf(&rsp_buf->buf, I2CP_MAX_MSG_BUF_SIZE, GFP_KERNEL,
		...
		i2c_pseudo_ctrlr_header_sep_char, i2c_adap_nr);
	if (ret < 0) {
	...
	while (pdata->rsp_buf_remaining == 0) {
		...
		if (!rsp_wrapper) {
			...
			if (int_ret < 0) {
				if (*ret == 0)
					*ret = int_ret;
			} else if (int_ret != I2CP_CTRLR_STATE_SHUTDN_REQ)
				*ret = -ENOTRECOVERABLE;
			return false;
		}
...
static int __init i2c_pseudo_init(void)
{
	...
	if (i2c_pseudo_limit < I2CP_ADAPTERS_MIN ||
	    i2c_pseudo_limit > I2CP_ADAPTERS_MAX) {
	...
	i2c_pseudo_class = class_create(THIS_MODULE, I2CP_CLASS_NAME);
	if (IS_ERR(i2c_pseudo_class))
		return PTR_ERR(i2c_pseudo_class);

	i2c_pseudo_class->dev_groups = i2c_pseudo_device_sysfs_groups;

	ret = alloc_chrdev_region(&i2c_pseudo_dev_num, I2CP_CDEV_BASEMINOR,
	...
	ret = dev_set_name(&i2c_pseudo_device->device, "%s", I2CP_DEVICE_NAME);
	if (ret < 0)
 ...
 fail_after_chrdev_register:
	unregister_chrdev_region(i2c_pseudo_dev_num, I2CP_CDEV_COUNT);
