...
/* TODO: Now that cmd_completer get access to pdata, use that for ADAPTER_SHUTDOWN instead of indicating it by return value. */
	...
	bool non_blocking)
{
	...
	if (ret < 0) {
		...
		ret = I2C_PSEUDO_CMD_COMPLETER_SUCCESS;
	}
 unlock_and_return:
...
/* TODO: Implement handling of an error write command from the controller. */
...
#if 0
	{
		...
		.data_creator = i2c_pseudo_cmd_get_number_data_creator,
		.data_destroyer = i2c_pseudo_cmd_get_number_data_destroyer,
	...
	struct i2c_pseudo_controller *pdata, bool non_blocking)
{
	...
	if (cmd_idx >= 0 && i2c_pseudo_cmds[cmd_idx].cmd_completer != NULL) {
		...
		case I2C_PSEUDO_CMD_COMPLETER_SHUTDOWN:
			ret = i2c_pseudo_adap_set_state(pdata,
				I2C_PSEUDO_CTRLR_STATE_SHUTDOWN);
			if (ret > 0)
				ret = 0;
			break;
