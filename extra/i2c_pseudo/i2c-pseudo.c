...
#define I2C_PSEUDO_ADAPTERS_MAX		256
/* Default i2c_pseudo_limit module parameter value. */
#define I2C_PSEUDO_DEFAULT_LIMIT	8
/* Value for alloc_chrdev_region() baseminor arg. */
#define I2C_PSEUDO_CDEV_BASEMINOR	0
#define I2C_PSEUDO_TIMEOUT_MS_MIN	0
#define I2C_PSEUDO_TIMEOUT_MS_MAX	(60 * MSEC_PER_SEC)
#define I2C_PSEUDO_DEFAULT_TIMEOUT_MS	(3 * MSEC_PER_SEC)
 ...
 */
static unsigned int i2c_pseudo_limit = I2C_PSEUDO_DEFAULT_LIMIT;
module_param(i2c_pseudo_limit, uint, 0444);
...
module_param(i2c_pseudo_default_timeout_ms, uint, 0444);
...
struct i2c_pseudo_counters {
	 ...
	 */
	/* Same type as struct i2c_pseudo_controller.id field. */
...
struct i2c_pseudo_device {
	struct i2c_pseudo_counters counters;
	struct cdev cdev;
	struct device device;
};

static struct i2c_pseudo_device *i2c_pseudo_device;
...
struct i2c_pseudo_cmd {
	 ...
	 * imminen, and no further I2C replies can be processed.
	 *
...
/*
 * These are indexes of i2c_pseudo_cmds[].  Every element in that array should
 * have a corresponding value in this enum, and the enum value should be used in
 * the i2c_pseudo_cmds[] initializer.
 */
enum {
	...
	/* Keep this at the end! This must equal ARRAY_SIZE(i2c_pseudo_cmds). */
...
struct i2c_pseudo_cmd_mxfer_reply {
	 ...
	 *
	 * This should be an unsigned integer type large enough to hold
	 * I2C_PSEUDO_CTRLR_RSP_QUEUE_LIMIT.  If changing this type, audit for
	 * printf format strings that need updating!
	 */
	unsigned int id;
...
enum i2c_pseudo_cmd_mxfer_reply_state {
	...
	I2C_PSEUDO_CMD_MXFER_REPLY_STATE_ID_NEXT,
	I2C_PSEUDO_CMD_MXFER_REPLY_STATE_INDEX_NEXT,
...
struct i2c_pseudo_cmd_mxfer_reply_data {
	 ...
	 */
	/* Same type as struct i2c_pseudo_cmd_mxfer_reply.id field. */
	...
	struct i2c_pseudo_cmd_mxfer_reply *reply_queue_current_item;
...
static ssize_t i2c_pseudo_rsp_buffer_formatter(void *data, char **out)
{
	...
	if (rsp_buf->buf != NULL) {
		if (rsp_buf->size > 0) {
			*out = rsp_buf->buf;
			rsp_buf->buf = NULL;
			return min_t(size_t, rsp_buf->size, SSIZE_MAX);
		}
		kfree(rsp_buf->buf);
	}
	kfree(rsp_buf);
...
static ssize_t i2c_pseudo_rsp_master_xfer_formatter(void *data, char **out)
{
	...
	if (mxfer_rsp->num_msgs_done < 0) {
		pr_err("%s: mxfer_rsp->num_msgs_done is negative (%d).  This is a bug, and may result in a memory leak or in freed memory being used.\n",
			__func__, mxfer_rsp->num_msgs_done);
	...
	if (mxfer_rsp->buf_start_plus_one <= 0) {
		...
		ret = anprintf(&buf_start, I2C_PSEUDO_MAX_MSG_BUF_SIZE,
			GFP_KERNEL, "%*s%c%u%c%d%c0x%04X%c0x%04X%c%u",
			(int)strlen(I2C_PSEUDO_MXFER_REQ_CMD),
			I2C_PSEUDO_MXFER_REQ_CMD,
			i2c_pseudo_ctrlr_header_sep_char, mxfer_rsp->id,
		 ...
		 */
			...
			ret = -ENOTRECOVERABLE;
			pr_err("%s: Unexpectedly received zero formattered characters / one buffer size (including null terminator) from anprintf(), freeing the buffer and returning %zd.\n",
				__func__, ret);
			kfree(buf_start);
		}
		goto maybe_free_and_definitely_return;
	...
	byte_limit = min_t(size_t, i2c_msg->len - byte_start,
		I2C_PSEUDO_MAX_MSG_BUF_SIZE / 3);
	/* 3 chars per byte == 2 chars for hex + 1 char for separator */
	buf_size = byte_limit * 3;
	if ((ssize_t)buf_size <= 0) {
		pr_err("%s: Calculated implausible buffer size %zu for data bytes, returning early.\n",
			__func__, buf_size);
...
static int i2c_pseudo_adap_get_state(struct i2c_pseudo_controller *pdata)
{
	...
	ret = mutex_lock_killable(&pdata->startstop_lock);
	if (ret) {
		if (ret > 0)
			ret = -EINTR;
		pr_warn("%s: I2C pseudo ID %u startstop_lock acquisition interrupted by signal, returning %d.\n",
...
/*
 * Everything here is best-effort to not make pending I2C requests wait for the
 * adapter timeout when we know we're shutting down and there is no possibility
 * of further I2C replies.  Thus if any lock acquisitions fail, just quietly
 * continue/return early.
 */
static void i2c_pseudo_cmd_mxfer_reply_data_shutdown(void *data)
{
	...
	list_for_each(list_ptr, &cmd_data->reply_queue_head) {
		mxfer_reply = list_entry(list_ptr,
			struct i2c_pseudo_cmd_mxfer_reply, reply_queue_item);
		if (mutex_lock_killable(&mxfer_reply->lock))
			continue;
...
static inline bool i2c_pseudo_mxfer_reply_is_current(
	struct i2c_pseudo_cmd_mxfer_reply_data *cmd_data,
	struct i2c_pseudo_cmd_mxfer_reply *mxfer_reply)
{
	int i;
...
static inline struct i2c_pseudo_cmd_mxfer_reply
*i2c_pseudo_mxfer_reply_find_current(
	struct i2c_pseudo_cmd_mxfer_reply_data *cmd_data)
{
	struct list_head *list_ptr;
	struct i2c_pseudo_cmd_mxfer_reply *mxfer_reply;
	...
	list_for_each(list_ptr, &cmd_data->reply_queue_head) {
		mxfer_reply = list_entry(list_ptr,
			struct i2c_pseudo_cmd_mxfer_reply, reply_queue_item);
		if (i2c_pseudo_mxfer_reply_is_current(cmd_data, mxfer_reply))
			return mxfer_reply;
	}
	return NULL;
}
...
static inline int i2c_pseudo_mxfer_reply_update_current(
	struct i2c_pseudo_cmd_mxfer_reply_data *cmd_data)
{
	int ret;

	ret = mutex_lock_killable(&cmd_data->reply_queue_lock);
	if (ret) {
		if (ret > 0)
			ret = -EINTR;
		pr_warn("%s: cmd_data->reply_queue_lock acquisition interrupted by signal, returning %d.\n",
			__func__, ret);
		return ret;
	}
	cmd_data->reply_queue_current_item =
		i2c_pseudo_mxfer_reply_find_current(cmd_data);
	mutex_unlock(&cmd_data->reply_queue_lock);
	/*
	 * Return success regardless of whether or not we found a matching
	 * mxfer_reply.  If we did not find a match, the cmd_completer callback
	 * will indicate that failure to the controller.
	 */
	return 0;
}
	...
	size_t in_size, bool non_blocking)
{
	...
	case I2C_PSEUDO_CMD_MXFER_REPLY_STATE_CMD_NEXT:
		if (STRING_NEQ(in, in_size, I2C_PSEUDO_MXFER_REPLY_CMD))
			/* Reaching here is a bug. */
			return -ENOTRECOVERABLE;
		/* Expect the msg/reply ID header field next. */
		cmd_data->state = I2C_PSEUDO_CMD_MXFER_REPLY_STATE_ID_NEXT;
		return 0;
	case I2C_PSEUDO_CMD_MXFER_REPLY_STATE_ID_NEXT:
	case I2C_PSEUDO_CMD_MXFER_REPLY_STATE_INDEX_NEXT:
	...
	default:
		 ...
		 */
		ret = -ENOTRECOVERABLE;
		pr_err("%s: cmd_data->state is unexpectedly %d.  This is a bug.  Will return %d.\n",
			__func__, cmd_data->state, ret);
		return ret;
	}
	...
	case I2C_PSEUDO_CMD_MXFER_REPLY_STATE_ERRNO_NEXT:
		 ...
		 * mxfer_reply.
		 */
		ret = i2c_pseudo_mxfer_reply_update_current(cmd_data);
		if (ret < 0)
			return ret;
		break;
	default:
	...
	if (reply_errno || cmd_data->reply_queue_current_item == NULL) {
		/*
		 * reply_errno:
		 ...
		 * controller.
		 */
		cmd_data->state = I2C_PSEUDO_CMD_MXFER_REPLY_STATE_INVALID;
		/*
	...
	cmd_data->state = I2C_PSEUDO_CMD_MXFER_REPLY_STATE_DATA_NEXT;
	/*
	...
	size_t in_size, bool non_blocking)
{
	...
	if (cmd_data->state == I2C_PSEUDO_CMD_MXFER_REPLY_STATE_INVALID)
		return 0;
	if (cmd_data->state != I2C_PSEUDO_CMD_MXFER_REPLY_STATE_DATA_NEXT)
		/* Reaching here is a bug. */
		return -ENOTRECOVERABLE;

	if (mutex_lock_killable(&cmd_data->reply_queue_lock)) {
		pr_warn("%s: cmd_data->reply_queue_lock acquisition interrupted by signal, returning -EINTR.\n",
			__func__);
		return -EINTR;
	}
	...
	if (mxfer_reply == NULL)
		...
		return -ENOTRECOVERABLE;
	if (ret) {
		pr_warn("%s: mxfer_reply->lock acquisition interrupted by signal, returning -EINTR.\n",
			__func__);
		return -EINTR;
	}
	...
	if (!(i2c_msg->flags & I2C_M_RD)) {
		...
		ret = -EIO;
		pr_warn("%s: I2C controller responded to a write with data.  Will return %d.\n",
			__func__, ret);
		goto unlock_and_return;
	}
	...
	if (i2c_msg->flags & I2C_M_RECV_LEN) {
		...
		if (cmd_data->current_buf_idx == 0) {
			if (i2c_msg->len > 1) {
				ret = -EINVAL;
				pr_warn("%s: I2C_M_RECV_LEN is set with initial struct i2c_msg.len=%u, expected <= 0.  Will return %d now.\n",
					__func__, (unsigned int)i2c_msg->len,
					ret);
				goto unlock_and_return;
			}
			/*
	...
	while (in_size > 0 && cmd_data->current_buf_idx < i2c_msg->len) {
		...
		if (in_size < 2 ||
			 ...
			 */
			ret = -EIO;
			pr_warn("%s: I2C controller responded with a null character in I2C data.  Will return %d.\n",
				__func__, ret);
			goto unlock_and_return;
		}
		/*
	...
	bool non_blocking)
{
	...
	if (mutex_lock_killable(&cmd_data->reply_queue_lock)) {
		...
		pr_warn("%s: cmd_data->reply_queue_lock acquisition interrupted by signal, returning %d.\n",
			__func__, ret);
		goto reset_cmd_data_and_return;
	}
	mxfer_reply = cmd_data->reply_queue_current_item;
	if (mxfer_reply != NULL) {
		ret = mutex_lock_killable(&mxfer_reply->lock);
		if (ret) {
			if (ret > 0)
				ret = -EINTR;
			pr_warn("%s: mxfer_reply->lock acquisition interrupted by signal, returning %d.\n",
				__func__, ret);
		} else if (mxfer_reply->completed[cmd_data->current_msg_idx]) {
			/* We already received a reply for this msg. */
			ret = -EIO;
			pr_warn("%s: I2C controller responded multiple times for same I2C msg request.  Will return %d.\n",
				__func__, ret);
		} else {
			mxfer_reply->completed[cmd_data->current_msg_idx] =
				true;
			if (++mxfer_reply->num_completed_true >=
			    mxfer_reply->num_msgs) {
	...
	if (mxfer_reply == NULL) {
		ret = -EIO;
		pr_warn("%s: I2C controller responded for an unknown I2C msg request.  This could be caused by the controller responding too slowly, after the I2C adapter timeout.  Will return %d.\n",
			__func__, ret);
		goto reset_cmd_data_and_return;
	}
	if (ret) {
 ...
 reset_cmd_data_and_return:
	cmd_data->state = I2C_PSEUDO_CMD_MXFER_REPLY_STATE_CMD_NEXT;
...
static int i2c_pseudo_cmd_adap_start_header_receiver(void *data, char *in,
	size_t in_size, bool non_blocking)
{
	if (STRING_NEQ(in, in_size, I2C_PSEUDO_ADAP_START_CMD))
		/* Reaching here is a bug. */
		return -ENOTRECOVERABLE;
	/*
	...
	bool non_blocking)
{
	...
	if (pdata->startstop_state != I2C_PSEUDO_CTRLR_STATE_NEW) {
		ret = -EISCONN;
		pr_err("%s: I2C pseudo ID %u controller attempted to start already-started I2C adapter, returning %d.\n",
			__func__, pdata->id, ret);
		goto unlock_and_return;
	}

	pr_info("%s: pdata->i2c_adapter.name is \"%*s\"\n", __func__, (int)sizeof(pdata->i2c_adapter.name), pdata->i2c_adapter.name);
	pr_info("%s: pdata->i2c_adapter.name is \"%s\"\n", __func__, pdata->i2c_adapter.name);
	pr_info("%s: pdata->i2c_adapter.timeout is %d (%d ms)\n", __func__, pdata->i2c_adapter.timeout, jiffies_to_msecs(pdata->i2c_adapter.timeout));
	/* Add the I2C adapter. */
	ret = i2c_add_adapter(&pdata->i2c_adapter);
	if (ret < 0) {
		pr_err("%s: i2c_add_adapter() for I2C pseudo ID %u failed with error %d\n",
			__func__, pdata->id, -ret);
		goto unlock_and_return;
	}
	...
	if (ret < 0) {
		pr_err("%s: device_create_file(&pdata->i2c_adapter.dev, &i2c_pseudo_id_dev_attr) failed with error %d\n",
			__func__, -ret);
		goto unlock_and_return;
	}
...
static int i2c_pseudo_cmd_adap_shutdown_header_receiver(void *data, char *in,
	size_t in_size, bool non_blocking)
{
	if (STRING_NEQ(in, in_size, I2C_PSEUDO_ADAP_SHUTDOWN_CMD))
		/* Reaching here is a bug. */
		return -ENOTRECOVERABLE;
	/*
	 * No more header fields or data are expected.  This directs any further
	...
	bool non_blocking)
{
	...
	if (mutex_lock_killable(&pdata->startstop_lock)) {
		pr_warn("%s: I2C pseudo ID %u startstop_lock acquisition interrupted by signal, returning -EINTR.\n",
			__func__, pdata->id);
		return -EINTR;
	}
...
static int i2c_pseudo_cmd_get_number_header_receiver(void *data, char *in,
	size_t in_size, bool non_blocking)
{
	if (STRING_NEQ(in, in_size, I2C_PSEUDO_GET_NUMBER_CMD))
		/* Reaching here is a bug. */
		return -ENOTRECOVERABLE;
	/*
	 * No more header fields or data are expected.  This directs any further
	...
	bool non_blocking)
{
	 ...
	 *
	 * Acquire pdata->startstop_lock manually instead of using
	 * i2c_pseudo_adap_get_state() in order to keep the lock while
	 * retrieving the I2C adapter number.
	 */
	ret = mutex_lock_killable(&pdata->startstop_lock);
	if (ret < 0) {
		pr_warn("%s: I2C pseudo ID %u startstop_lock acquisition interrupted by signal, returning %zd.\n",
			__func__, pdata->id, ret);
		return ret;
	}
	if (pdata->startstop_state == I2C_PSEUDO_CTRLR_STATE_RUNNING) {
		i2c_adap_nr = pdata->i2c_adapter.nr;
	} else {
		ret = -ENOTCONN;
		pr_err("%s: I2C pseudo ID %u controller attempted to get the number of a non-running I2C adapter.  Will return %zd.\n",
			__func__, pdata->id, ret);
	}
	mutex_unlock(&pdata->startstop_lock);
	if (ret < 0)
	...
	ret = anprintf(&rsp_buf->buf, I2C_PSEUDO_MAX_MSG_BUF_SIZE, GFP_KERNEL,
		"%*s%c%d", (int)strlen(I2C_PSEUDO_NUMBER_REPLY_CMD),
	...
	if (ret < 0) {
		...
		ret = -ENOTRECOVERABLE;
		pr_debug("%s: Unexpectedly received zero formattered characters / one buffer size (including null terminator) from anprintf(), freeing the buffer and returning %zd.\n",
			__func__, ret);
	...
	if (mutex_lock_killable(&pdata->read_rsp_queue_lock)) {
		ret = -EINTR;
		pr_warn("%s: I2C pseudo ID %u read_rsp_queue_lock acquisition interrupted by signal, returning %zd.\n",
			__func__, pdata->id, ret);
		goto fail_after_buf_alloc;
	}
	if (pdata->read_rsp_queue_length >= I2C_PSEUDO_CTRLR_RSP_QUEUE_LIMIT) {
		...
		pr_debug("%s: I2C pseudo ID %u has reached "
			STR(I2C_PSEUDO_CTRLR_RSP_QUEUE_LIMIT)
			" request queue limit.  Returning %zd now.\n", __func__,
			pdata->id, ret);
...
static int i2c_pseudo_cmd_set_name_suffix_header_receiver(void *data, char *in,
	size_t in_size, bool non_blocking)
{
	if (STRING_NEQ(in, in_size, I2C_PSEUDO_SET_NAME_SUFFIX_CMD))
		/* Reaching here is a bug. */
		return -ENOTRECOVERABLE;
	return 1;
}
	...
	bool non_blocking)
{
	...
	if (pdata->startstop_state != I2C_PSEUDO_CTRLR_STATE_NEW) {
		ret = -EISCONN;
		pr_err("%s: I2C pseudo ID %u controller attempted to change the name of an already-started I2C adapter.  Will return %d.\n",
			__func__, pdata->id, ret);
		goto unlock_and_return;
	}
	...
	if (ret < 0) {
		pr_err("%s: snprintf() for I2C pseudo ID %u i2c_adapter.name failed with error %d\n",
			__func__, pdata->id, -ret);
		goto unlock_and_return;
	}
	ret = 0;
	...
	size_t in_size, bool non_blocking)
{
	...
	case 0:
		if (STRING_NEQ(in, in_size, I2C_PSEUDO_SET_TIMEOUT_CMD))
			/* Reaching here is a bug. */
			return -ENOTRECOVERABLE;
		return 0;
	case 1:
	...
	bool non_blocking)
{
	...
	if (pdata->startstop_state != I2C_PSEUDO_CTRLR_STATE_NEW) {
		ret = -EISCONN;
		pr_err("%s: I2C pseudo ID %u controller attempted to change the timeout of an already-started I2C adapter.  Will return %d.\n",
			__func__, pdata->id, ret);
		goto unlock_and_return;
	}
...
/* All elements should be initialized in their I2C_PSEUDO_CMD_*_IDX position. */
static const struct i2c_pseudo_cmd i2c_pseudo_cmds[] = {
	[I2C_PSEUDO_CMD_MXFER_REPLY_IDX] = {
		...
		.data_shutdown = i2c_pseudo_cmd_mxfer_reply_data_shutdown,
...
static inline bool i2c_pseudo_poll_in(struct i2c_pseudo_controller *pdata)
{
	return pdata->rsp_invalidated || pdata->rsp_buf_remaining != 0 ||
		!list_empty(&pdata->read_rsp_queue_head);
}

static inline int i2c_pseudo_fill_rsp_buf(struct i2c_pseudo_rsp *rsp_wrapper,
	...
	struct i2c_msg *msgs, int num)
{
	...
	if (i < 0)
		return i;
	switch (i) {
	case I2C_PSEUDO_CTRLR_STATE_RUNNING:
		break;
	case I2C_PSEUDO_CTRLR_STATE_SHUTDN_REQUESTED:
		return ret;
	default:
		/* Reaching here is a bug, even with a valid enum value. */
		pr_err("%s: Unexpected struct i2c_pseudo_controller.startstop_state value %d.  This is a bug.  Will return -ENOTRECOVERABLE.\n",
			__func__, i);
		return -ENOTRECOVERABLE;
	}
	...
	if (ret < 0) {
		pr_err("%s: I2C_PSEUDO_FILL_RSP_BUF_WITH_LITERAL(rsp_wrappers[wrapper_idx++], rsp_bufs[rsp_bufs_idx++], I2C_PSEUDO_BEGIN_MXFER_REQ_CMD) returned %d.\n",
			__func__, ret);
		goto fail_after_individual_rsp_wrappers_alloc;
	}
	...
	if (ret < 0) {
		pr_err("%s: I2C_PSEUDO_FILL_RSP_BUF_WITH_LITERAL(rsp_wrappers[wrapper_idx++], rsp_bufs[rsp_bufs_idx++], I2C_PSEUDO_COMMIT_MXFER_REQ_CMD) returned %d.\n",
			__func__, ret);
		goto fail_after_individual_rsp_wrappers_alloc;
	}
	...
	if (mutex_lock_killable(&pdata->read_rsp_queue_lock)) {
		ret = -EINTR;
		pr_warn("%s: I2C adapter %d read_rsp_queue_lock acquisition interrupted by signal, returning %d.\n",
			__func__, adap->nr, ret);
		goto fail_after_individual_rsp_wrappers_alloc;
	}
	if (pdata->read_rsp_queue_length >= I2C_PSEUDO_CTRLR_RSP_QUEUE_LIMIT) {
		ret = -ENOBUFS;
		pr_debug("%s: I2C pseudo controller for I2C adapter %d has reached "
			STR(I2C_PSEUDO_CTRLR_RSP_QUEUE_LIMIT)
	...
	if (mutex_lock_killable(&cmd_data->reply_queue_lock)) {
		ret = -EINTR;
		pr_warn("%s: I2C adapter %d reply_queue_lock acquisition interrupted by signal, returning %d.\n",
			__func__, adap->nr, ret);
		goto fail_with_read_rsp_queue_lock;
	}
	...
	if (wait_ret == -ERESTARTSYS) {
		ret = -EINTR;
		pr_warn("%s: I2C adapter %d data_filled completion wait interrupted by signal, returning %d.\n",
			__func__, adap->nr, ret);
	} else if (wait_ret < 0) {
		ret = wait_ret;
		pr_warn("%s: I2C adapter %d data_filled completion returned unexpected error %ld, this function will return %d now.\n",
			__func__, adap->nr, wait_ret, ret);
	} else {
		ret = mxfer_reply->ret;
		if (wait_ret == 0)
			pr_warn("%s: I2C adapter %d data_filled completion wait timed out after %u ms, this function will return %d from mxfer_reply->ret now.\n",
				__func__, adap->nr,
				jiffies_to_msecs(adap->timeout), ret);
		else
			pr_debug("%s: I2C adapter %d data_filled completion wait returned successfully before its %u ms timeout, this function will return %d from mxfer_reply->ret now.\n",
				__func__, adap->nr,
				jiffies_to_msecs(adap->timeout), ret);
	}
...
static void i2c_pseudo_remove_from_counters(struct i2c_pseudo_controller *pdata,
	struct i2c_pseudo_device *this_pseudo)
{
...
static int i2c_pseudo_cdev_open(struct inode *inodep, struct file *filep)
{
	...
	for (i = 0; i < ARRAY_SIZE(i2c_pseudo_cmds); ++i) {
	...
	if (mutex_lock_killable(&this_pseudo->counters.lock)) {
		ret = -EINTR;
		pr_warn("%s: Request for new pseudo I2C adapter lock acquisition interrupted by signal, returning %d.\n",
			__func__, ret);
		goto fail_after_cmd_data_created;
	}
	for (i = 0; i < i2c_pseudo_limit; ++i)
		if (this_pseudo->counters.all_controllers[i] == NULL)
			break;
	if (i >= i2c_pseudo_limit) {
		pr_err("%s: The limit of %u pseudo I2C adapters has already been reached, cannot add another one.\n",
	...
	for (ctrlr_id = this_pseudo->counters.next_ctrlr_id;;) {
		...
		if (++ctrlr_id == this_pseudo->counters.next_ctrlr_id) {
			pr_err("%s: Cycled through every possible I2C pseudo controller ID without finding a free one.  This is implausible, and may indicate a bug.  this_pseudo->counters.count=%u\n",
				__func__, this_pseudo->counters.count);
			ret = -ENOSPC;
			break;
		}
	}
 ...
 unlock_counters:
	mutex_unlock(&this_pseudo->counters.lock);
	if (ret < 0)
		/* Assume this_pseudo->counters was never mutated. */
		goto fail_after_cmd_data_created;
	...
	if (ret < 0) {
		pr_err("%s: snprintf() for inodep=%p filep=%p i2c_adapter.name failed with error %d\n",
 ...
 fail_after_counters_update:
	i2c_pseudo_remove_from_counters(pdata, this_pseudo);
...
static int i2c_pseudo_cdev_release(struct inode *inodep, struct file *filep)
{
	...
	this_pseudo = container_of(pdata->i2c_adapter.dev.parent,
		struct i2c_pseudo_device, device);
	 ...
	 */
	for (i = 0; i < ARRAY_SIZE(i2c_pseudo_cmds); ++i)
		if (i2c_pseudo_cmds[i].data_shutdown != NULL)
			i2c_pseudo_cmds[i].data_shutdown(pdata->cmd_data[i]);
...
static bool i2c_pseudo_cdev_read_iteration(char __user **buf, size_t *count,
	ssize_t *ret, bool non_blocking, struct i2c_pseudo_controller *pdata)
{
	int int_ret;
	long wait_ret;
	ssize_t copy_size;
	...
	while (pdata->rsp_buf_remaining == 0) {
		...
		if (non_blocking || *ret != 0) {
			...
			if (wait_ret == -ERESTARTSYS) {
				if (*ret == 0)
					*ret = -EINTR;
				pr_warn("%s: I2C pseudo ID %u read_rsp_queued completion wait interrupted by signal, returning %zd.\n",
					__func__, pdata->id, *ret);
				return false;
			} else if (wait_ret < 0) {
				if (*ret == 0)
					*ret = wait_ret;
				pr_warn("%s: I2C pseudo ID %u read_rsp_queued completion returned unexpected error %ld, this function will return %zd now.\n",
					__func__, pdata->id, wait_ret, *ret);
				return false;
			} else if (wait_ret > 0) {
				/* unexpected return value */
				if (*ret == 0)
					*ret = -ENOTRECOVERABLE;
				pr_warn("%s: I2C pseudo ID %u read_rsp_queued completion returned unexpected value %ld, this function will return %zd now.\n",
					__func__, pdata->id, wait_ret, *ret);
				return false;
			}
		}
		...
		if (rsp_wrapper == NULL) {
			...
			I2C_PSEUDO_CTRLR_STATE_SHUTDN_REQUESTED) {
				*ret = -ENOTRECOVERABLE;
				pr_err("%s: I2C pseudo ID %u pdata->read_rsp_queue_head was empty after achieving pdata->read_rsp_queued completion, and the I2C adapter is not marked for shutdown.  This is a bug.  Returning %zd now.\n",
					__func__, pdata->id, *ret);
			}
			return false;
		}
		...
		if (pdata->rsp_buf_start != NULL) {
			pr_err("%s: An I2C pseudo adapter controller read response formatter callback returned non-positive (%zd) yet mutated the response buffer address pointer (and made it non-NULL).  This is a bug in the formatter callback function.  This read response buffer will be freed without being returned to the controller.\n",
				__func__, pdata->rsp_buf_remaining);
			kfree(pdata->rsp_buf_start);
			pdata->rsp_buf_start = NULL;
		}
		...
		size_t count, loff_t *f_ps)
{
	...
	if (non_blocking) {
		if (!mutex_trylock(&pdata->rsp_lock)) {
			BUILD_BUG_ON(ret != 0);
			ret = -EAGAIN;
			pr_debug("%s: Could not immediately acquire I2C pseudo ID %u rsp_lock for O_NONBLOCK read, returning %zd.\n",
				__func__, pdata->id, ret);
			return ret;
		}
	} else {
		if (mutex_lock_killable(&pdata->rsp_lock)) {
			BUILD_BUG_ON(ret != 0);
			ret = -EINTR;
			pr_warn("%s: I2C pseudo ID %u rsp_lock acquisition interrupted by signal, returning %zd.\n",
				__func__, pdata->id, ret);
			return ret;
		}
	}
	...
	while (count > 0 && i2c_pseudo_cdev_read_iteration(
	...
	bool non_blocking)
{
	...
	if (copy_size > buf_remaining) {
		...
		if (!pdata->cmd_receive_status) {
			pr_warn("%s: Error: Exceeded maximum %zu size of I2C pseudo controller command buffer for I2C pseudo ID %u.  The command currently being written will be ignored.\n",
				__func__, sizeof(pdata->cmd_buf), pdata->id);
	...
	if (cmd_idx < 0) {
		for (i = 0; i < ARRAY_SIZE(i2c_pseudo_cmds); ++i)
			if (i2c_pseudo_cmds[i].cmd_size == pdata->cmd_size &&
				...
				break;
		if (i >= ARRAY_SIZE(i2c_pseudo_cmds)) {
			ret = -EIO;
	...
	if (!pdata->cmd_receive_status) {
		...
		if (ret > 0) {
			if (ret > sizeof(pdata->cmd_buf)) {
				pr_err("%s: A header_receiver callback returned a data buffer increment size of %zd, which is greater than the maximum controller write reply buffer size of %zu.  This is a bug.  Will return -ENOTRECOVERABLE.\n",
					__func__, ret, sizeof(pdata->cmd_buf));
				ret = -ENOTRECOVERABLE;
				goto clear_buffer_and_return;
			}
	...
	bool non_blocking)
{
	...
	if (cmd_idx < 0) {
		pr_err("%s: Attempting to process write bytes as data without a write command previously identified.  This is a bug.  Will return -ENOTRECOVERABLE.\n",
			__func__);
		return -ENOTRECOVERABLE;
	}
	 ...
	 */
	size_holder = min_t(ssize_t, size_holder, (sizeof(pdata->cmd_buf) - (
		sizeof(pdata->cmd_buf) % pdata->cmd_data_increment)));
	/*
		...
		size_t count, loff_t *f_ps)
{
	...
	if (non_blocking) {
		if (!mutex_trylock(&pdata->cmd_lock)) {
			BUILD_BUG_ON(ret != 0);
			ret = -EAGAIN;
			pr_debug("%s: Could not immediately acquire I2C pseudo ID %u cmd_lock for O_NONBLOCK write, returning %zd.\n",
				__func__, pdata->id, ret);
			goto free_and_return;
		}
	} else {
		if (mutex_lock_killable(&pdata->cmd_lock)) {
			BUILD_BUG_ON(ret != 0);
			ret = -EINTR;
			pr_warn("%s: I2C pseudo ID %u cmd_lock acquisition interrupted by signal, returning %zd.\n",
				__func__, pdata->id, ret);
			goto free_and_return;
		}
	}
	...
	while (remaining) {
		...
		if (ret == 0 || ret > remaining) {
			pr_err("%s: Received %zd return value from either i2c_pseudo_receive_ctrlr_cmd_header() or i2c_pseudo_receive_ctrlr_cmd_data(), which is either zero or greater than the %zu remaining write bytes.  Either case is a bug.  Will return -ENOTRECOVERABLE.\n",
				__func__, ret, remaining);
			ret = -ENOTRECOVERABLE;
			break;
		}
		...
		if (ret > 0 && start[-1] == i2c_pseudo_ctrlr_end_char) {
			ret = i2c_pseudo_receive_ctrlr_cmd_complete(
				pdata, non_blocking);
			if (ret < 0)
				break;
		}
...
static struct device_attribute i2c_pseudo_limit_dev_attr = {
	.attr = {
		.name = "limit",
		.mode = 0444,
	},
	...
	struct device_attribute *attr, char *buf)
{
	...
	this_pseudo = container_of(dev, struct i2c_pseudo_device, device);

	if (mutex_lock_killable(&this_pseudo->counters.lock)) {
		pr_warn("%s: Request for new pseudo I2C adapter lock acquisition interrupted by signal, returning -EINTR.\n",
			__func__);
		return -EINTR;
	}
...
static struct device_attribute i2c_pseudo_count_dev_attr = {
	.attr = {
		.name = "count",
		.mode = 0444,
	},
...
static struct attribute *i2c_pseudo_device_sysfs_attrs[] = {
	&i2c_pseudo_limit_dev_attr.attr,
	&i2c_pseudo_count_dev_attr.attr,
	NULL,
};

static const struct attribute_group i2c_pseudo_device_sysfs_group = {
	.attrs = i2c_pseudo_device_sysfs_attrs,
};

static const struct attribute_group *i2c_pseudo_device_sysfs_groups[] = {
	&i2c_pseudo_device_sysfs_group,
	NULL,
};
...
static int __init i2c_pseudo_init(void)
{
	...
	i2c_pseudo_class->dev_groups = i2c_pseudo_device_sysfs_groups;
	...
	i2c_pseudo_device = kzalloc(sizeof(*i2c_pseudo_device), GFP_KERNEL);
	if (i2c_pseudo_device == NULL) {
		ret = -ENOMEM;
		goto fail_after_chrdev_register;
	}

	i2c_pseudo_device->device.devt = i2c_pseudo_dev_num;
	i2c_pseudo_device->device.class = i2c_pseudo_class;
	i2c_pseudo_device->device.release = i2c_p_device_release;
	device_initialize(&i2c_pseudo_device->device);

	ret = dev_set_name(&i2c_pseudo_device->device, "%s",
		I2C_PSEUDO_DEVICE_NAME);
	if (ret < 0) {
	...
	mutex_init(&i2c_pseudo_device->counters.lock);
	i2c_pseudo_device->counters.all_controllers = kcalloc(
		i2c_pseudo_limit,
		sizeof(*i2c_pseudo_device->counters.all_controllers),
		GFP_KERNEL);
	if (i2c_pseudo_device->counters.all_controllers == NULL) {
		ret = -ENOMEM;
		goto fail_after_device_init;
	}

	cdev_init(&i2c_pseudo_device->cdev, &i2c_pseudo_fileops);
	i2c_pseudo_device->cdev.owner = THIS_MODULE;

	ret = cdev_device_add(&i2c_pseudo_device->cdev,
		&i2c_pseudo_device->device);
	if (ret < 0) {
		pr_err("%s: cdev_device_add(&i2c_pseudo_device->cdev, &i2c_pseudo_device->device) failed with error %d\n",
			__func__, -ret);
		goto fail_after_device_init;
	}
 ...
 fail_after_device_init:
	put_device(&i2c_pseudo_device->device);
...
{
	cdev_device_del(&i2c_pseudo_device->cdev, &i2c_pseudo_device->device);
	put_device(&i2c_pseudo_device->device);
	unregister_chrdev_region(i2c_pseudo_dev_num, I2C_PSEUDO_CDEV_COUNT);
	i2c_p_class_destroy();
}
