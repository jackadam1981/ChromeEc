...
#define I2C_PSEUDO_SET_TIMEOUT_MS_CMD "SET_ADAPTER_TIMEOUT_MS"
 ...
 */
static unsigned short i2c_pseudo_default_timeout_ms = 5 * 1000;
module_param(i2c_pseudo_default_timeout_ms, ushort, S_IRUGO);
...
enum i2c_pseudo_ctrlr_state {
	 ...
	 * controller.
	 */
	I2C_PSEUDO_CTRLR_STATE_SHUTDOWN,
};
...
struct i2c_pseudo_rsp {
	 ...
	 * - A negative return value indicates an error occured and the data cannot be formatted successfully.  The error code may or may not eventually be propagated back to the I2C pseudo adapter controller.
...
static int i2c_pseudo_adap_set_state(struct i2c_pseudo_controller *pdata,
	enum i2c_pseudo_ctrlr_state state)
{
	int ret;
	...
	size_t in_size, bool non_blocking)
{
	 ...
	 * start command line after the initial command name.  That is
	...
	bool non_blocking)
{
	...
	if (receive_status)
		return 0;
	ret = i2c_pseudo_adap_set_state(pdata, I2C_PSEUDO_CTRLR_STATE_SHUTDOWN);
	if (ret < 0)
		return ret;
	return 0;
}
	...
	bool non_blocking)
{
	...
	ret = anprintf(&rsp_buf->buf, I2C_PSEUDO_MAX_MSG_BUF_SIZE,
		"I2C_ADAPTER_NUM %d", pdata->i2c_adapter.nr);
	if (ret < 0) {
...
/* TODO: Implement handling of an error write command from the controller. */
	...
	{
		.cmd_string = I2C_PSEUDO_SET_TIMEOUT_MS_CMD,
		.cmd_size = STRLEN(I2C_PSEUDO_SET_TIMEOUT_MS_CMD),
	...
	struct i2c_msg *msgs, int num)
{
	...
	if (i == I2C_PSEUDO_CTRLR_STATE_SHUTDOWN)
		goto just_return;
...
static __poll_t i2c_pseudo_cdev_poll(struct file *filep, poll_table *ptp)
{
	...
	if (int_ret < 0)
		 ...
		 */
		poll_ret |= POLLERR;
	else if (int_ret == I2C_PSEUDO_CTRLR_STATE_SHUTDOWN)
		poll_ret |= POLLHUP;
