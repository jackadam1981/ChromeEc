...
enum i2c_pseudo_ctrlr_state {
	 ...
	 * controller.
	 */
	I2C_PSEUDO_CTRLR_STATE_SHUTDN_REQ,
};
...
static ssize_t i2c_pseudo_rsp_master_xfer_formatter(void *data, char **out)
{
 ...
 maybe_free:
	if (ret <= 0) {
		 ...
		 * i2c_pseudo_adapter_master_xfer() reuses a single
		 * struct i2c_pseudo_rsp_master_xfer (mxfer_rsp) across multiple
		 * struct i2c_pseudo_rsp (rsp_wrappers), one for each
...
static void i2c_pseudo_cmd_mxfer_reply_data_shutdown(void *data)
{
	...
	list_for_each(list_ptr, &cmd_data->reply_queue_head) {
		...
		if (mutex_lock_killable(&mxfer_reply->lock))
			/*
			 * If we got stuck acquiring any of these locks, and
			 * userspace wants to die, avoid the rest of these
			 * locks.
			 */
			break;
	...
	bool non_blocking)
{
	...
	if (ret < 0)
		return ret;
	pdata->startstop_state = I2C_PSEUDO_CTRLR_STATE_SHUTDN_REQ;
	mutex_unlock(&pdata->startstop_lock);
	...
	bool non_blocking)
{
	...
	ret = mutex_lock_killable(&pdata->read_rsp_queue_lock);
	if (ret < 0)
		goto fail_after_buf_alloc;
	...
	struct i2c_msg *msgs, int num)
{
	...
	case I2C_PSEUDO_CTRLR_STATE_RUNNING:
		break;
	case I2C_PSEUDO_CTRLR_STATE_SHUTDN_REQ:
		return ret;
	default:
...
static int i2c_pseudo_cdev_open(struct inode *inodep, struct file *filep)
{
	...
	for (i = 0; i < ARRAY_SIZE(i2c_pseudo_cmds); ++i) {
		if (!i2c_pseudo_cmds[i].data_creator)
			continue;
	...
	ret = mutex_lock_killable(&this_pseudo->counters.lock);
	if (ret < 0)
		goto fail_after_cmd_data_created;
 ...
 fail_after_cmd_data_created:
	for (i = 0; i < num_cmd_data_created; ++i)
		if (i2c_pseudo_cmds[i].data_destroyer)
...
static int i2c_pseudo_cdev_release(struct inode *inodep, struct file *filep)
{
	 ...
	 */
	for (i = 0; i < ARRAY_SIZE(i2c_pseudo_cmds); ++i)
		if (i2c_pseudo_cmds[i].data_shutdown)
	...
	ssize_t *ret, bool non_blocking, struct i2c_pseudo_controller *pdata)
{
	...
	while (pdata->rsp_buf_remaining == 0) {
		...
		int_ret = mutex_lock_killable(&pdata->read_rsp_queue_lock);
		if (int_ret < 0) {
			if (*ret == 0)
				*ret = int_ret;
			return false;
		...
		if (!rsp_wrapper) {
			...
			if (int_ret < 0) {
				if (*ret == 0)
					*ret = int_ret;
			} else if (int_ret != I2C_PSEUDO_CTRLR_STATE_SHUTDN_REQ)
				*ret = -ENOTRECOVERABLE;
			return false;
		}
		...
		size_t count, loff_t *f_ps)
{
	...
	if (non_blocking) {
		...
		int_ret = mutex_lock_killable(&pdata->rsp_lock);
		if (int_ret < 0)
			return int_ret;
	}
	...
	struct i2c_pseudo_controller *pdata, bool non_blocking)
{
	...
	if (cmd_idx >= 0 && i2c_pseudo_cmds[cmd_idx].cmd_completer) {
		...
		size_t count, loff_t *f_ps)
{
	...
	if (non_blocking) {
		...
		int_ret = mutex_lock_killable(&pdata->cmd_lock);
		if (int_ret < 0) {
			BUILD_BUG_ON(ret != 0);
			ret = int_ret;
			goto free_kbuf;
...
static int __init i2c_pseudo_init(void)
{
	...
	if (IS_ERR(i2c_pseudo_class))
		return PTR_ERR(i2c_pseudo_class);
	...
	i2c_pseudo_device->counters.all_controllers = kcalloc(
		...
		GFP_KERNEL);
	if (!i2c_pseudo_device->counters.all_controllers) {
		ret = -ENOMEM;
		goto fail_after_device_init;
	}
