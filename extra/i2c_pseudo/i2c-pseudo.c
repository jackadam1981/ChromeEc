...
struct i2c_pseudo_cmd {
	 ...
	 *
	 * - This is called exactly once for each write command.
	 *
	 ...
	 *   callback.
	 */
	int (*cmd_completer)(void *data, int receive_status);
};
		...
		size_t count, loff_t *f_ps)
{
	...
	pr_info("i2c_pseudo_cdev_read(filep=%p, buf=%p, count=%zu, f_ps=%p) "
		"called for I2C adapter %d\n", filep, buf, count, f_ps,
		pdata->i2c_adapter.nr);
	if (mutex_lock_killable(&pdata->rsp_lock)) {
		BUILD_BUG_ON(ret != 0);
		ret = -EINTR;
		pr_warn("%s: I2C adapter %d rsp_lock acquisition interrupted "
	...
	while (count > 0) {
		...
		while (pdata->rsp_buf_remaining == 0) {
			...
			/*
			 * If we are out of read responses, return whatever we
			 * have written to the userspace buffer so far, even if
			 * it's nothing.
			 */
	...
	struct i2c_pseudo_controller *pdata, char *start, size_t remaining)
{
	...
	if (cmd_idx < 0 && found_deliminator_char) {
		for (i = 0; i < ARRAYLEN(i2c_pseudo_cmds); ++i)
	 ...
	 */
	if (pdata->cmd_size > 0 && found_deliminator_char) {
		if (!pdata->cmd_receive_status) {
		...
		size_t count, loff_t *f_ps)
{
	...
	pdata = filep->private_data;
	pr_info("%s(filep=%p, buf=%p, count=%zu, f_ps=%p) called for "
		"I2C adapter %d\n", __func__, filep, buf, count, f_ps,
		pdata->i2c_adapter.nr);
	...
	while (remaining) {
		...
		if (ret > 0 && start[-1] == i2c_pseudo_ctrlr_end_char) {
			ret = i2c_pseudo_int_to_ssize_t(
				i2c_pseudo_receive_ctrlr_cmd_complete(pdata));
			if (ret < 0)
				break;
		}
	}
...
static const struct file_operations i2c_pseudo_fileops = {
	...
	/* TODO: Any ioctl()s needed? */
	/* TODO: Consider implementing poll(). */
