 ...
 *
 * *arg must be struct i2cp_ioctl_xfer_arg.
 *
 ...
 *
 * EMSGSIZE:
 * - arg->msgs_len indicates arg->msgs is of insufficient size ot hold all
 ...
 *
 * *arg must be struct i2cp_ioctl_xfer_arg, which is never modified by this.
 *
...
/*
 * Report counters of I2C transfers requested of this psuedo controller.
 *
...
struct i2cp_ioctl_start_output {
	...
	__u64 adapter_num;
	/*
	 * Identifier for this I2C pseudo adapter.
	 * Will never be reused while i2c-pseudo is loaded unless an internal
	 * counter wraps.
	 * Guaranteed unique among active I2C pseudo adapters.
	 * I2C device drivers, such as userspace programs using i2c-dev, can use
	 * this to track specific instances of I2C pseudo adapters in the face
	 * of I2C adapter number reuse.
	 */
	__u64 pseudo_id;
};
...
struct i2cp_ioctl_xfer_reply_arg {
	...
	__u64 xfer_id;
	/*
	 * length of msgs array, must match i2cp_ioctl_xfer_req_output.num_msgs
	 * from the xfer_id
	 */
	__u16 num_msgs;
	/*
	 * Return value for master_xfer. Gets returned to I2C device driver.
