	...
	struct i2c_msg *msgs, int num)
{
	...
	if (!i2cp_sum_buf_lens(msgs, num, &pdata->total_bufs_size)) {
	...
	u8 *data_buf_copy, u8 __user *user_data_buf)
{
	u16 i;
	size_t pos;
	for (i = 0, pos = 0; i < num_msgs; ++i, pos += msgs[i].len) {
		/*
		 ...
		 */
		memcpy(&data_buf_copy[pos], msgs[i].buf, msgs[i].len);
	...
	u8 __user *user_data_buf, u32 data_buf_len, long *ret)
{
	...
	if (!i2cp_sum_buf_lens(msgs_copy, num_msgs, &data_buf_size) ||
	    data_buf_size > data_buf_len) {
		i2cp_null_bufs(msgs_copy, num_msgs);
		*ret = -ENOBUFS;
		return true;
	}

	data_buf_copy = kzalloc(data_buf_size, GFP_KERNEL);
	if (!data_buf_copy) {
		*ret = -ENOMEM;
		return false;
	}
	...
	if (copy_to_user(user_data_buf, data_buf_copy, data_buf_size)) {
		*ret = -EFAULT;
		copy_msgs = false;
	}
	kfree(data_buf_copy);
	...
	struct i2cp_ioctl_xfer_req_arg *arg_copy)
{
	...
	msgs_copy = kmemdup(xfer_msgs, num_msgs, GFP_KERNEL);
	if (!msgs_copy)
		return -ENOMEM;
