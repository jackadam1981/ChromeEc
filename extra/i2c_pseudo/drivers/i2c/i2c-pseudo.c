	...
	struct i2cp_controller *pdata)
{
	...
	ret = strncpy_from_user(pdata->i2c_adapter.name, name,
	                        sizeof(pdata->i2c_adapter.name));
	// TODO: Is this needed, or okay to not have trailing Null?
	if (ret >= sizeof(pdata->i2c_adapter.name)) {
		pdata->i2c_adapter.name[ret - 1] = '\0';
		--ret;
	}
	return ret;
}
...
static long i2cp_cdev_ioctl_xfer_reply(struct file *filep, unsigned long arg)
{
	...
	pdata->xfer_num_msgs = arg_copy.num_msgs;
	if (arg_copy.error == 0)
		pdata->xfer_ret = arg_copy.num_msgs;
	else
		pdata->xfer_ret = max(INT_MIN, -((s32)arg_copy.error));
	pdata->xfer_state = I2CP_STATE_XFER_RETURN;
