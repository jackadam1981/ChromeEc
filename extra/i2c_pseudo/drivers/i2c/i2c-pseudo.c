...
/* u32_max is for params used to limit values put into u32 UAPI fields */
	...
	size_t *ret)
{
	for (size_t i = 0; i < num_msgs; ++i)
		// TODO: What's the correct and portable way to do this safely?
...
static inline bool i2cp_master_xfer_wait_event_cond(enum i2cp_state xfer_state)
{
	...
	struct i2c_msg *msgs, int num)
{
	...
	time_left = wait_event_interruptible_timeout(
		...
		i2cp_master_xfer_wait_event_cond(READ_ONCE(pdata->xfer_state)),
		adap->timeout);
...
static inline void i2cp_set_timeout(u32 timeout_ms,
	struct i2cp_controller *pdata)
{
...
static long i2cp_cdev_ioctl_start(struct file *filep, unsigned long arg)
{
	...
	if (ret < 0)
		goto unlock;
	i2cp_set_timeout(arg_copy.timeout_ms, pdata);
...
static inline bool i2cp_xfer_req_wait_event_cond(enum i2cp_state xfer_state)
{
 ...
 check_xfer_state:
	...
	case I2CP_STATE_XFER_RETURN:
		...
		ret = wait_event_interruptible(
			pdata->state_wait_queue,
			i2cp_xfer_req_wait_event_cond(
				READ_ONCE(pdata->xfer_state)));
		if (ret == -ERESTARTSYS)
			return ret;
		if (ret != 0)
...
static long i2cp_cdev_ioctl_xfer_reply(struct file *filep, unsigned long arg)
{
	...
	for (u32 i = 0; i < arg_copy.num_msgs; ++i) {
		if ((msgs_copy[i].flags & I2C_M_RD) && copy_from_user(
			pdata->xfer_msgs[i].buf, msgs_copy[i].buf,
			pdata->xfer_msgs[i].len * sizeof(*(msgs_copy[i].buf))))
		{
			ret = -EFAULT;
			goto unlock;
		}
