...
class ServodClient(xmlrpc.client.ServerProxy):
    ...
    def fwup_start(self, port: int):
        ...
        self._run_ec_command_get_output(
            f"pdc fwup_rtk start {port}", ["RTK_FWUP: Started"]
        )

    def fwup_write(self, data: bytes) -> int:
        """Transfer FW payload data to the EC via the console"""

        cmd = f"pdc fwup_rtk write {base64.b64encode(data).decode('ascii')}"
    ...
    def fwup_finish(self):
        ...
        try:
            self._run_ec_command_get_output(
                "pdc fwup_rtk finish", ["PDC FWUP successful"]
            )
        finally:
    ...
    def fwup_abort(self):
        ...
        self._run_ec_command("pdc fwup_rtk abort")


# Number of host FWUP packets to send at a time, per call to
# `pdc fwup_rtk write`. This is impacted by the Zephyr shell buffer size.
...
def main(
    ...
    try:
        ...
        log.info("Finishing firmware update session")
        servo.fwup_finish()
