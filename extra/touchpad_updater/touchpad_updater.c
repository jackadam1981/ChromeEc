...
static int i2c_passthru(const uint8_t *to_write,
			...
			uint16_t read_length)
{
	...
	memmove(tx_buf + 3, tx_buf, write_length);
	tx_buf[0] = 0x12;
	tx_buf[1] = read_length & 0xFF;
	tx_buf[2] = read_length >> 8;

	ret = write(fd, tx_buf, write_length + 3);
	if (ret < 0) {
		close(fd);
		return ret;
	}
					...
					uint16_t read_length)
{
	...
	if (1)
		return i2c_passthru(to_write, write_length, to_read, read_length);
