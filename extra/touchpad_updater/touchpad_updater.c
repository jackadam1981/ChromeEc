...
static int check_read_status(int r, int expected, int actual)
{
	...
	if (actual != (expected + 4)) {
		printf("Warning: Not reading %d bytes as expected.\n", expected);
		r = 1;
	}
		...
		uint8_t *to_read, uint8_t read_length)
{
	...
	tx_buf[0] = 0x00;		// Primary port
	tx_buf[1] = 0x15;		// I2C address for trackpad
...
static int elan_write_and_read(
		int reg, unsigned char *buf, int read_length, int with_cmd, int cmd)
{
	...
	return libusb_single_write_and_read(tx_buf, with_cmd ? 4 : 2, rx_buf, read_length);
}
