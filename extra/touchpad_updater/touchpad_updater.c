...
static void elan_prepare_for_update(void)
{
	...
	if (le_bytes_to_int(rx_buf + 4) != ETP_I2C_IAP_PASSWORD)
		printf("Got an unexpected IAP password %4x\n",
...
static uint16_t elan_calc_checksum(unsigned char *data, int length)
{
	uint16_t checksum = 0;
...
static int elan_write_fw_block(
		unsigned char *raw_data, uint16_t checksum)
{
