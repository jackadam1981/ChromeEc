...
void rand_bytes(void *buffer, size_t len)
{
	...
	/*
	 * Seed random number generator, replace time(0) with a constant value
 ...
 *	      size at 32 bytes.
 * @secret -  the product of multiplying of the server point by our private
 *	      key, only the 32 bytes of X coordinate are returned.
 */
static void p256_key_and_secret(uint8_t pub_key[32], uint8_t secret[32])
{
	 ...
	 *
	 * Key pair generation is based on code snippets found at
	 * https://davidederosa.com/basic-blockchain-programming/elliptic-curve-keys
	 */
	while (1) {
		uint8_t priv_key[EC_PRIV_KEY_SZ];

		/* Fill out the private key candidate. */
		rand_bytes(priv_key, sizeof(priv_key));

		/*
		 * Make sure it's a positive number, otherwise an extra byte
		 * of zeros would be required to be fed into BN.
		 */
		priv_key[0] &= 0x7f;

		/* Set private key as BIGNUM */
...
static void print_params(int p_flag)
{
	...
	dump_key("Server Private Key:", priv_key, 32);
