...
class Cmd {
			       ...
			       struct ucsi_control *ctrl, uint8_t *lpm_data_out)
	{
		...
		DLOG("RECEIVED COMMAND ITER(%d): command 0x%x, data_length 0x%x,"
		     ...
		     ++iter, ctrl->command, ctrl->data_length,
 ...
 * The commands were captured on the Realtek EVB with firmware 0.3.1
 */
...
Cmd get_conn_caps_c1("get_conn_caps_c1", vector<uint8_t>({ 0x7, 0x0, 0x1 }),
		     vector<uint8_t>({ 0xe7, 0x03 }));

/* GET_PDO conn 1 source command */
Cmd get_pdo_c1_src("get_pdo_c1_src",
		   vector<uint8_t>({ 0x10, 0x0, 0x01, 0x00, 0x07 }), -1);

/* GET_PDO conn 1 sink command */
Cmd get_pdo_c1_snk("get_pdo_c1_snk",
		   vector<uint8_t>({ 0x10, 0x0, 0x01, 0x00, 0x03 }), -1);
...
Cmd get_alt_mode_conn_c1_i1(
	"get_alt_mode_conn_c1_i1_",
	vector<uint8_t>({ 0x0c, 0x00, 0x00, 0x01, 0x01, 0x00 }),
	vector<uint8_t>({ 0xef, 0x17, 0x00, 0x00, 0x00, 0x00, 0x1, 0xff, 0x46,
			  0x1c, 0x0, 0x40 }));
...
Cmd get_pdo_c2_src("get_pdo_c2_src",
		   vector<uint8_t>({ 0x10, 0x00, 0x02, 0x00, 0x07 }), -1);

/* GET_PDO conn 2 sink command */
Cmd get_pdo_c2_snk("get_pdo_c2_snk",
		   vector<uint8_t>({ 0x10, 0x00, 0x02, 0x00, 0x03 }), -1);
...
/* Messages when LPM alert for port 1 happens */

/* GET_CONNECTOR_STATUS conn 1 update 1 command */
Cmd get_conn_status_c1_update1("get_conn_status_c1_update1",
			       vector<uint8_t>({ 0x12, 0x00, 0x01 }),
			       vector<uint8_t>({ 0x00, 0x40, 0x3d, 0x40, 0x00,
						 0x00, 0x00, 0x00, 0x08 }));

/* GET_CONNECTOR_STATUS conn 1 update 2 command */
Cmd get_conn_status_c1_update2("get_conn_status_c1_update2",
			       vector<uint8_t>({ 0x12, 0x00, 0x01 }),
			       vector<uint8_t>({ 0x0, 0x40, 0x0, 0x0, 0x0, 0x0,
						 0x0, 0x0, 0x1 }));

/* GET_CONNECTOR_STATUS conn 1 update 3 command */
Cmd get_conn_status_c1_update3("get_conn_status_c1_update3",
			       vector<uint8_t>({ 0x12, 0x00, 0x01 }),
			       vector<uint8_t>({ 0x0, 0x40, 0x2d, 0x20, 0x0,
						 0x0, 0x0, 0x0, 0x1 }));

/* GET_CONNECTOR_STATUS conn 1 update 4 command */
Cmd get_conn_status_c1_update4("get_conn_status_c1_update4",
			       vector<uint8_t>({ 0x12, 0x00, 0x01 }),
			       vector<uint8_t>({ 0x60, 0xa, 0x2b, 0x40, 0x45,
						 0x15, 0x85, 0x43, 0x1 }));

/* GET_CONNECTOR_STATUS conn 1 update 5 command */
Cmd get_conn_status_c1_update5("get_conn_status_p1_update5",
			       vector<uint8_t>({ 0x12, 0x00, 0x01 }),
			       vector<uint8_t>({ 0x60, 0xb, 0x4b, 0x40, 0x45,
						 0x15, 0x85, 0x43, 0x1 }));

/* GET_CONNECTOR_STATUS conn 1 update 6 command */
Cmd get_conn_status_c1_update6("get_conn_status_c1_update6",
			       vector<uint8_t>({ 0x12, 0x00, 0x01 }),
			       vector<uint8_t>({ 0x0, 0x0, 0x4b, 0x40, 0x45,
						 0x15, 0x85, 0x43, 0x1 }));

/* GET_PDOS conn 1 partner source command */
Cmd get_pdo_c1_partner_src("get_pdo_c1_partner_src",
...
class Checker {
	...
	void addDirToCheck(string &dir, bool exist)
	{
		location.push_back(make_tuple(dir, exist, "", 0));
	}
	...
	int verify(int timeout_ms)
	{
		for (auto tuple : location) {
			...
			if (tmo <= 0) {
				DLOG("Failed to verify location '%s' %s",
				     loc.c_str(),
				     exist ? "does exist" : "does not exist");
				return -ETIMEDOUT;
			} else
				DLOG("Verified location '%s' %s", loc.c_str(),
				     exist ? "does exist" : "does not exist");
		}
	...
	vector<tuple<string, bool, string, int> > location;
...
TEST_F(OpmUnitTest, opm_initialization)
{
...

	{
		...
		EXPECT_CALL(*_pdDriverMock, execute_cmd(_, _, _))
			...
			.WillOnce(Invoke(&ack_cc_ci, fptr))
			.WillOnce(Invoke(&get_pdo_c1_snk, fptr))
			.WillOnce(Invoke(&ack_cc_ci, fptr))
			...
			.WillOnce(Invoke(&ack_cc_ci, fptr))
			.WillOnce(Invoke(&get_pdo_c2_snk, fptr))
			.WillOnce(Invoke(&ack_cc_ci, fptr))
	...
	// verify that port2 and port3 directories exist
	string port2("/sys/class/typec/port2");
	string port3("/sys/class/typec/port3");
	checker.addDirToCheck(port2, true);
	checker.addDirToCheck(port3, true);
	EXPECT_EQ(0, checker.verify())
		<< "FAILED TO VERIFY EXISTENCE OF PORT2 AND PORT3 DIRECTORIES";
	//  Initialization is completed now ;)

	this_thread::sleep_for(std::chrono::milliseconds(500));
}

TEST_F(OpmUnitTest, opm_lpm_alert_connector_1)
{
	Cmd::handle_execute_cmd_ptr fptr = &Cmd::handle_execute_cmd;
	Checker checker;

	{
		...
		EXPECT_CALL(*_pdDriverMock, execute_cmd(_, _, _))
			...
			.WillOnce(Invoke(&ack_cc_ci, fptr))
			.WillOnce(Invoke(&ack_cc_ci_conn, fptr))
			.WillOnce(Invoke(&get_pdo_c1_partner_src, fptr))
			.WillOnce(Invoke(&ack_cc_ci, fptr))
			...
			.WillOnce(Invoke(&get_alt_mode_sop_i0, fptr))
			.WillOnce(Invoke(&ack_cc_ci, fptr))
			.WillOnce(Invoke(&get_conn_status_c1_update6, fptr))
			.WillOnce(Invoke(&get_conn_status_c1_update6, fptr))
			.WillOnce(Invoke(&ack_cc_ci, fptr))
			.WillOnce(Invoke(&get_pdo_c1_partner_src, fptr))
			.WillOnce(Invoke(&ack_cc_ci, fptr))
	...
	/*
	 * when partner is connected then Realtek triggers a number
	 * of consecutive interrupts, try to simulate it
	 */
	...
	/* verify that port2-partner exists */
	checker.clear();
	string port2_partner("/sys/class/typec/port2-partner");
	checker.addDirToCheck(port2_partner, true);
	EXPECT_EQ(0, checker.verify())
		<< "FAILED TO VERIFY EXISTENCE OF PORT2-PARTNER DIRECTORY";
