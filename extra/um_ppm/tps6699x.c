...
#define TPS6699X_DATA_CHUNK_LEN 64
			      ...
			      size_t buf_len)
{
	...
	for (int chunk_offset = 0; chunk_offset < buf_len;
		...
		int write_length =
			MIN(TPS6699X_DATA_CHUNK_LEN, buf_len - chunk_offset);

		rv = dev->smbus->stream_write(dev->smbus->dev,
					      broadcast_address,
					      (uint8_t *)write_start_pos,
					      write_length);
		if (rv < 0) {
			ELOG("%s: Streaming data block failed (ret=%d, "
			     "offset_into_block=%d, total_block_size=%zu,"
			     "chunk_size=%d, write_length=%d)",
			     "tps6699x", rv, chunk_offset, buf_len,
			     TPS6699X_DATA_CHUNK_LEN, write_length);
			return rv;
		}

		/* Periodically print a progress log message */
		if ((chunk_offset / TPS6699X_DATA_CHUNK_LEN) % 32 == 0) {
			ELOG("%s:  Block progress %u / %zu\n", "tps6699x",
