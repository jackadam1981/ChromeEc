...
class StatsManager(object):
  ...
  def AddUnit(self, domain, unit):
    """Add the unit for a domain. There can be only one unit for each domain.
  ...
  def SaveSummaryJSON(self, directory, fname='summary.json'):
    ...
    for domain in self._summary:
      if domain.startswith(NOSHOW_PREFIX):
        continue
      unit = self._unit[domain]
      if unit in LONG_UNIT:
        unit = LONG_UNIT[unit]
      data_entry = {'mean': self._summary[domain]['mean'],
                    'unit': unit}
