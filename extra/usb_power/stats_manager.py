...
class StatsManager(object):
  ...
  def AddValue(self, domain, value):
    ...
    if isinstance(value, float):
      ...
      return
    print ('Error: new value %s for domain %s is not a number, thus ignored.' %
           (value, domain))
...
def main():
  # As a quick test.
  data = StatsManager()
  data.AddValue('A', 0.000001)
  data.AddValue('A', 0.000003)
  data.AddValue('A', 'ERROR')
  data.AddValue('B', 1)
  data.AddValue('B', 2)
  data.AddValue('B', 3)
  ...
  data.CalculateStats()
  data.PrintSummary()
  data.SaveSummary('./summary')
  data.SaveRawData('./raw_data')
