...
def main(argv=None):
  ...
  parser.add_argument('--save_stats', type=str, nargs='?',
      ...
      specified; otherwise save to where %(prog)s is located; if this flag is \
  ...
  parser.add_argument('--save_raw_data', type=str, nargs='?',
      ...
      const=os.path.dirname(os.path.abspath(__file__)), default=None,
      help="Save raw data for sweetberry readings to %(metavar)s if specified; \
      otherwise save to where %(prog)s is located; if this flag is not set, \
