...
"""Accesses power logging through usb endpoint."""
...
class Spower(object):
  ...
  def append_ina_struct(self, name, rs, port, addr, data=None):
    ...
    ina['port'] = port
    ina['addr'] = addr
    ina['mAscale'] = 80000000. / (rs * 0x8000);
    ina['mWscale'] = 25. * ina['mAscale'];
    ina['data'] = data
  ...
  def interpret_line(self, data):
    ...
    for i in range(0, size):
      ...
      mw = raw_w * self._inas[i]['mWscale']
      name = self._inas[i]['name']
      debuglog("READ %d %s: %fs: %fmW" % (i, name, ftimestamp, mw))
      output += ", %.02f" % mw
...
parser.add_argument('-b', '--board', type=str,
    help="Board configuration json file", default="marlin.board")
parser.add_argument('-c', '--config', type=str,
    help="Rails to monitor in json file", default="marlin_short.scenario")
