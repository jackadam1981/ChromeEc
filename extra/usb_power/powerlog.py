...
# potential config file locations: same directory as powerlog.py file,
# current working directory, or LIB_DIR.
CONFIG_LOCATIONS = [os.path.dirname(os.path.realpath(__file__)), os.getcwd(),
                    LIB_DIR]
...
class Spower(object):
  ...
  def __init__(self, board, vendor=0x18d1,
    ...
    self._logger = logging.getLogger('powerlog')
    self._board = board
  ...
  def interpret_line(self, data):
    ...
    for i in range(0, size):
      ...
      self._logger.debug("READ %d %s: %fs: 0x%04x %f" % (i,
