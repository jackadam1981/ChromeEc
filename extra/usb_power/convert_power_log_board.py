...
"""
Program to convert sweetberry config to power logging config.
"""
...
def write_to_file(file, sweetberry, inas):
    ...
    for rec in inas:
        if not rec['sweetberry'] == sweetberry:
            continue
        if start:
            start = False
        else:
...
def main(argv):
    ...
    has_sweetberry_a = False
    has_sweetberry_b = False

    for rec in inas:
        if rec['sweetberry'] == 'A':
            has_sweetberry_a = True
        if rec['sweetberry'] == 'B':
            has_sweetberry_a = True

    if has_sweetberry_a and has_sweetberry_b:
        ...
        if has_sweetberry_a:
            write_to_file(basename + '.py', 'A', inas)
        elif has_sweetberry_b:
            write_to_file(basename + '.py', 'B', inas)
