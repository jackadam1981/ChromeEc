		    ...
		    void *inbuf, int inlen, int allow_less, size_t *rxed_count)
{
	...
	if (outbuf && outlen) {
		actual = 0;
		r = libusb_bulk_transfer(uep->devh, uep->out_ep.addr, outbuf, outlen,
					 &actual, 2000);
		if (r < 0) {
			 ...
			 struct usb_endpoint *uep)
{
	...
	if (iface->bInterfaceClass == 255 &&
		...
		if (iface->bNumEndpoints == 2) {
			...
			USB_ERROR("Invalid endpoint number", iface->bNumEndpoints);
			return 0;
		}
		return 1;
	}
		       ...
		       struct usb_endpoint *uep)
{
	...
	printf("Found interface %d, IN ep 0x%x(%d), OUT ep 0x%x(%d)\n", iface_num,
	       uep->in_ep.addr, uep->in_ep.len, uep->out_ep.addr, uep->out_ep.len);
