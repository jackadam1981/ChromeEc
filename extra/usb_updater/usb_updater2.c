 ...
 * EC device firmware.
 *
 * The EC firmware image consists of multiple sections, of interest to this
 * app are the RO and RW code sections, two of each. When firmware update
 * session is established, the EC device reports locations of backup RW and RO
 ...
 * the full EC firmware binary image and sends them to the device for
 ...
 *
 * There are two ways to communicate with the EC device: USB and /dev/tpm0
 * (when this app is running on a chromebook with the EC device). Originally
 * different protocols were used to communicate over different channels,
 * starting with version 3 the same protocol is used.
 *
 * This app provides backwards compatibility to ensure that earlier EC
 ...
 * is transferred to the EC in 1K PDUs. In versions 1 and 2 the address in
 ...
 *
 * The EC device responds to each update PDU with a confirmation which is 4
...
static char *short_opts = "bd:fhjsu";
...
static void stay_in_ro(struct transfer_descriptor *td)
{
	uint16_t subcommand = UPDATE_EXTRA_CMD_STAY_IN_RO;
...
int main(int argc, char *argv[])
{
	...
	if (!show_fw_ver && !jump_rw && !stay_ro) {
		if (optind >= argc) {
			fprintf(stderr,
