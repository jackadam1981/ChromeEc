		       ...
		       struct addr_range *parsed_range)
{
	...
	/* Range type. */
	next_colon = strchr(line_cursor, ':');
	if (next_colon)
		*next_colon = '\0';
	if (!next_colon || (strlen(line_cursor) != 1)) {
		fprintf(stderr, "Invalid entry type, section %d\n",
			section_count_);
		return EINVAL;
	}
	switch (*line_cursor) {
	...
	if (next_colon)
		*next_colon = '\0';
	if (!next_colon || (strlen(line_cursor) != 1)) {
		fprintf(stderr, "Invalind entry kind %d\n",
			section_count_);
		return EINVAL;
	}
	switch (*line_cursor) {
	...
	if (next_colon)
		*next_colon = '\0';
	if (!next_colon) {
		fprintf(stderr, "Invalid entry base address in section %d\n",
			section_count_);
		return EINVAL;
	}
	parsed_range->base_addr = strtoul(line_cursor, &e, 16);
	if (e && *e) {
		fprintf(stderr, "Invalid entry base address %s in section %d\n",
			line_cursor, section_count_);
		return EINVAL;
	}
	line_cursor = ++next_colon;
	...
	parsed_range->range_size = strtoul(line_cursor, &e, 16);
	if (e && *e) {
		fprintf(stderr, "Invalid entry size %s in section %d\n",
			line_cursor, section_count_);
		return EINVAL;
	}
	...
	if (!is_a_hash_range)
		return 0; /* No more input for dump rages. */
	...
	do { /* While line is not over. */
		...
		line_cursor = next_colon + 1;
		next_colon = strchr(line_cursor, ':');
		if (next_colon)
			*next_colon = '\0';
		if (strlen(line_cursor) != sizeof(*node)) {
			fprintf(stderr,
...
int parser_get_next_range(const struct addr_range **range)
{
	...
	*range = NULL;
	...
	/* This shure will be enough to fit parsed structure contents. */
