...
def main():
    ...
    parser.add_argument(
        "-a", "--all", action="store_true", help="Allow updating multiple matching devices."
    )
    parser.add_argument(
    ...
    if len(devs) > 1 and not args.all:
        raise ServoUpdaterException("Found %d matching devices to update. Use --all if updating multiple devices is intended." % len(devs))
