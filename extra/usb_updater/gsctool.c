 ...
 */
static bool is_allow_unverified_ro(struct transfer_descriptor *td)
{
	...
	if (ccd_info_version != 1) {
		fprintf(stderr,
			...
			"Only Ti50 chips support this operation.\n",
			ccd_info_version);
		exit(update_error);
	}

	/* Convert BE back to host endian format. */
	ccd_info.ccd_caps_current[1] = be32toh(ccd_info.ccd_caps_current[1]);
				   ...
				   enum wp_options wp)
{
	...
	if (rv == VENDOR_RC_NOT_ALLOWED) {
		fprintf(stderr, "Error: CCD must be open first.\n"
				"Use `gsctool -o`\n");
		return update_error;
	}
