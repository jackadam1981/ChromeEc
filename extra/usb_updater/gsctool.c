			...
			uint32_t bid_flags)
{
	...
	if (bid_type == FOUR_FF_BYTES && bid_type_inv == FOUR_FF_BYTES &&
	    bid_flags == FOUR_FF_BYTES) {
		printf("Board ID is not programmed.\n");
		return;
	}
	...
	while (i >= 8) {
		...
		/* If not in printable ASCII range, print '?' instead. */
		if (ch < 0x20 || ch >= 0x7F)
			ch = '?';
