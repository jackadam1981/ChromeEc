...
static bool valid_image_header(const struct typed_image_header h,
			       const size_t size, enum gsc_device *out_image)
{
...
static int transfer_image(struct transfer_descriptor *td, struct image *image)
{
	...
	for (i = 0; i < ARRAY_SIZE(update_order); i++) {
		...
		if (!sections[sect].needs_upgrade)
			continue;
		if (num_txed_sections && needs_delay) {
			...
			setup_connection(td);
			/* Pick sections again incase GSC versions changed. */
			pick_sections(td, image);
		}
			    ...
			    bool show_machine_output)
{
	 ...
	 * requested.
	 */
	is_dauntless = (gsc_dev != GSC_DEVICE_H1);
...
int main(int argc, char *argv[])
{
	...
	if (td.ep_type == usb_xfer) {
		...
		int pid_count = sizeof(pids) / sizeof(uint16_t);
