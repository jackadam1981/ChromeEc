...
static int get_factory_config(struct transfer_descriptor *td)
{
	uint32_t rv;
	uint8_t response[2048] = {0};
	size_t response_size = sizeof(response);
...
static int set_factory_config(struct transfer_descriptor *td,
			uint64_t val)
{
	uint32_t val_be = htobe64(val);
	uint32_t rv;
	uint8_t response[2048] = {0};
	size_t response_size = sizeof(response);

	rv = send_vendor_command(td, VENDOR_CC_SET_FACTORY_CONFIG, &val_be,
				 sizeof(val_be), response, &response_size);
	if (rv != VENDOR_RC_SUCCESS) {
...
int main(int argc, char *argv[])
{
	...
	if (factory_config) {
		if (factory_config_is_set)
			exit(set_factory_config(&td, factory_config_arg));
		else
			exit(get_factory_config(&td));
	}
