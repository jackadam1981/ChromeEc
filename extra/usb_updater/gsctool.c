 ...
 * descriptions, for short and long forms. The short form is built based on
 * the contents of the long_opts table.
 *
 * The 'required_argument' short options are followed by ':', and the
 * 'optional_argument' options are followed by '::'. The passed in short_opts
 * string is long enough to accommodate all options with ::.
 */
static void set_opt_descriptors(struct option *long_opts, char *short_opts)
{
	...
	for (i = j = 0; i < ARRAY_SIZE(cmd_line_options); i++) {
		...
		if (long_opts[i].has_arg == optional_argument) {
			short_opts[j++] = ':';
			short_opts[j++] = ':';
			continue;
		}
 ...
 *
 * When command line arguments allow optional parameters, gsctool users expect
 * to be able to use any of the following styles:
 *
 * a)   -x<param value>
 * b)   -x=<param_value>
 * c)   -x <param value>
 * d)  --x_long <param_value>
 * e)  --x_long=<param_value>
 *
 * Styles a), b) and e) are supported by libc getopt_long(), styles c) and d)
 * are not.
 *
 * This function adds ability to handle cases c) and d) on top of
 * getopt_long().
 */
static int getopt_all(int argc, char *argv[])
{
	int longindex = -1;
	static char short_opts[3 * ARRAY_SIZE(cmd_line_options)] = {};
