...
static const struct option_container cmd_line_options[] = {
	...
	{ { "nuvotitan", optional_argument, NULL, 'N' },
...
static bool valid_signed_manifest(const struct SignedManifest *const m,
				  const size_t size)
{
	if (m->length > size)
		return false;
...
static bool valid_image_header(const union ImageHeader *const h,
			       const size_t size)
{
	if (size < sizeof(union ImageHeader))
		return false;

	/*
	 * Check for NT first as this check cannot pass on well formed DT/Cr50
	 * image.
	 */
	if (h->m.identifier == ID_ROM_EXT || h->m.identifier == ID_OWNER_FW)
		return valid_signed_manifest(&h->m, size);

	if (h->h.magic == MAGIC_HAVEN || h->h.magic == MAGIC_DAUNTLESS)
		return valid_signed_header(&h->h, size);
...
static bool locate_headers(const void *image, const uint32_t size)
{
	...
	h = as_header(image, 0);
	if (!valid_image_header(h, slot_a_end)) {
		fprintf(stderr, "\nERROR: RO_A header is invalid\n");
		return false;
	}

	/* Check if this is a NuvoTitan image first, then try dauntless */
	if (h->m.identifier == ID_ROM_EXT) {
		if ((gsc_dev != GSC_DEVICE_ANY) && (gsc_dev != GSC_DEVICE_NT))
			return false;

		gsc_dev = GSC_DEVICE_NT;
	...
	if (!valid_image_header(h, size - slot_a_end)) {
