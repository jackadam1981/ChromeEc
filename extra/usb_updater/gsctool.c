...
const struct ccd_capability_info static ti50_cap_info[] = {
...
static void print_ccd_info(void *response, size_t response_size)
{
	...
	if ((ccd_info_header.ccd_magic == CCD_INFO_MAGIC) &&
		...
		/*
		 * Presently ccd_info sizes reported by Cr50 and Ti50 are the
		 * same, this could change in the future, in which case the
		 * structure to copy the payload to will depend on the
		 * ccd_info version.
		 */
		memcpy(&ccd_info,
		       (uint8_t*)response + sizeof(struct ccd_info_response_header),
		       sizeof(ccd_info));
	} else if (response_size == sizeof(ccd_info)) {
