...
static int process_get_flog(struct transfer_descriptor *td, uint32_t prev_stamp)
{
	...
	while (1) {
		...
		if (rv == 0) {
			...
			memcpy(&prev_stamp, &entry.r.timestamp,
			       sizeof(prev_stamp));
			printf("%10u:%02x", prev_stamp, entry.r.type);
			for (i = 0; i < FLASH_LOG_PAYLOAD_SIZE(entry.r.size);
			     i++)
		...
		/*
		 * Flash log could be momentarily locked by a concurrent
		 * access, let it settle and try again, 10 ms should be
		 * enough.
		 */
		usleep(10 * 1000);
	}
