...
static void usage(int errs)
{
	printf("\nUsage: %s [options] [<binary image>]\n"
	       ...
	       "                           Get or set Info1 SN bits fields\n"
			    ...
			    uint32_t *sn_bits)
{
	...
	uint8_t response;
	size_t response_size = sizeof(command_body);

	for (i = 0; i < 3; i++)
		command_body[i] = htobe32(sn_bits[i]);

	send_vendor_command(td, VENDOR_CC_SN_SET_HASH,
			    command_body, sizeof(command_body),
			    &command_body, &response_size);
...
static void process_sn_inc_rma(struct transfer_descriptor *td,
			       uint8_t arg)
{
	size_t response_size = sizeof(arg);

	send_vendor_command(td, VENDOR_CC_SN_INC_RMA,
			    &arg, sizeof(arg),
			    &arg, &response_size);
