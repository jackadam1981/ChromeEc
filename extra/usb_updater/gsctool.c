...
enum arv_config_setting_command_e {
	arv_config_setting_command_write_protect_descriptors = 0,
	arv_config_setting_command_spi_addressing_mode = 1,
};
...
int main(int argc, char *argv[])
{
	...
	while ((i = getopt_all(argc, argv)) != -1) {
		...
		case 'B':
			...
			break;
		case 'C':
			if (optarg && !strncmp(optarg, "3byte", 5))
				arv_config_spi_addr_mode = arv_config_spi_addr_mode_choice_set_3byte;
			else if (optarg && !strncmp(optarg, "4byte", 5))
				arv_config_spi_addr_mode = arv_config_spi_addr_mode_choice_set_4byte;
			else
				arv_config_spi_addr_mode = arv_config_spi_addr_mode_choice_get;
			break;
		case 'd':
	...
	if ((bid_action == bid_none) &&
	    (arv_config_spi_addr_mode == arv_config_spi_addr_mode_choice_none) &&
	    !ccd_info &&
	    !ccd_lock &&
	    !ccd_open &&
	    !ccd_unlock &&
	...
	if (arv_config_spi_addr_mode)
		exit(process_arv_config_spi_addr_mode(&td, arv_config_spi_addr_mode));
