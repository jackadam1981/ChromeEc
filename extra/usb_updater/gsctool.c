...
static void process_endorsement_seed(struct transfer_descriptor *td,
				     const char *endorsement_seed_str)
{
	...
	if (rv == VENDOR_RC_NOT_ALLOWED) {
		...
		for (i = 0; i < response_size; i++) {
			if (response_seed[i] != endorsement_seed[i]) {
...
int main(int argc, char *argv[])
{
	...
	if (get_endorsement_seed)
		process_endorsement_seed(&td, endorsement_seed_str);
	if (rma)
