 ...
 * 'optional_argument' options are followed by '::'.
 *
 ...
 *
 * Styles a) and d) are supported by libc getopt_long(), styles b), and c) are
 * not.
 *
 * This function adds ability to handle cases b) and c) on top of what's
 * included with getopt_long().
 */
