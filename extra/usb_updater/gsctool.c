...
static int process_get_metrics(struct transfer_descriptor *td)
{
	struct ti50_stats {
		...
		uint32_t rdd_keepalive;
	};
	uint32_t rv;
	/* Allocate extra space in case future versions add more data. */
	uint8_t response[4 * sizeof(struct ti50_stats)] = {0};
	size_t response_size = sizeof(response);
	...
	struct ti50_stats stats = *(struct ti50_stats *) response;
	...
	stats.rdd_keepalive = be32toh(stats.rdd_keepalive);
