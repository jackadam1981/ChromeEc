...
/* Max. key string length in machine-friendly outputs. */
#define MAX_MACH_OUTPUT_KEY_SIZE 100

/*
 * Max. length of the board ID string representation.
 *
 * Board ID is either shown as a 4-character ASCII alphanumeric string or a
 * 8-digit hex.
 */
#define MAX_BOARD_ID_LENGTH 8
...
static void print_machine_output(const char *key, const char *format, ...)
{
	...
	if (strchr(key, '=') != NULL || strchr(key, '\n') != NULL) {
		fprintf(stderr,
			"Error: key should not contain '=' or a newline "
			"character.\n");
		return;
	}
	...
	if (strchr(format, '\n') != NULL) {
		fprintf(stderr,
			"Error: value should not contain a newline character. "
			"\n");
		return;
	}
...
{
	const struct {
		const char *fw_name;    // either "RO" or "RW"
		const char fw_part;     // either 'A' or 'B'
		uint32_t offset;
	} sections[] = {
		{"RO", 'A', CONFIG_RO_MEM_OFF},
		{"RW", 'A', CONFIG_RW_MEM_OFF},
		{"RO", 'B', CHIP_RO_B_MEM_OFF},
		{"RW", 'B', CONFIG_RW_B_MEM_OFF}
	};
	size_t i;
	...
	for (i = 0; i < ARRAY_SIZE(sections); i++) {
		...
		char bid_string[MAX_BOARD_ID_LENGTH + 1];

		char machine_output_key[MAX_MACH_OUTPUT_KEY_SIZE];

		h = (const struct SignedHeader *)((uintptr_t)image +
						  sections[i].offset);

		if (show_machine_output) {
			snprintf(machine_output_key, MAX_MACH_OUTPUT_KEY_SIZE,
				 "IMAGE_%s_FW_VER_%c", sections[i].fw_name,
				 sections[i].fw_part);

			print_machine_output(machine_output_key,
					     "%d.%d.%d",
					     h->epoch_,
					     h->major_,
					     h->minor_);
		} else {
			printf("%s%s_%c:%d.%d.%d", i ? " " : "",
			       sections[i].fw_name,
			       sections[i].fw_part,
			       h->epoch_, h->major_, h->minor_);
		}

		if (sections[i].fw_name[1] != 'W')
			continue;
		...
		if (j == sizeof(bid)) {
			bid = be32toh(bid);
			snprintf(bid_string, MAX_BOARD_ID_LENGTH + 1,
				 "%.4s", (const char *)&bid);
		} else {
			snprintf(bid_string, MAX_BOARD_ID_LENGTH + 1,
				 "%08x", bid);
		...
		if (!show_machine_output) {
			printf("[%s:%08x:%08x]",
			       bid_string, bid_mask, bid_flags);
		} else {
			snprintf(machine_output_key, MAX_MACH_OUTPUT_KEY_SIZE,
				 "IMAGE_BOARD_ID_%c", sections[i].fw_part);
			print_machine_output(machine_output_key, "%s:%08x:%08x",
					     bid_string, bid_mask, bid_flags);
		}
	}
	printf("\n");
...
int main(int argc, char *argv[])
{
	...
	if (data || show_fw_ver) {
		...
		if (show_fw_ver) {
			if (show_machine_output) {
				print_machine_output("GSC_ON_USB_RO_FW_VER",
						     "%d.%d.%d",
						     targ.shv[0].epoch,
						     targ.shv[0].major,
						     targ.shv[0].minor);
				print_machine_output("GSC_ON_USB_RW_FW_VER",
						     "%d.%d.%d",
						     targ.shv[1].epoch,
						     targ.shv[1].major,
						     targ.shv[1].minor);
