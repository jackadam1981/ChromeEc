					    ...
					    bool show_machine_output)
{
	...
	rv = send_vendor_command(td, VENDOR_CC_GET_DEVICE_IDS, NULL, 0,
				 &response, &response_size);
	...
	if (show_machine_output) {
		print_machine_output("DEVID0", "%08x", response.dev_id0);
		print_machine_output("DEVID1", "%08x", response.dev_id1);
	} else {
