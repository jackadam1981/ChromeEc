...
static void print_board_info(uint32_t bid_type, uint32_t bid_flags)
{
	...
	printf("  Board Name: ");
	switch (bid_type) {
	/* 'FOOB': Board ID is not even assigned yet. */
	case 0x464F4F42:
		printf("FOOB: unknown because of incompatible firmware\n");
		break;

	/* 'ZZCR': Board ID is not even assigned yet. */
	case 0x5A5A4352:
		printf("ZZCR: not assigned\n");
		break;

	/* Board ID is not programmed yet. */
	case 0xFFFFFFFF:
