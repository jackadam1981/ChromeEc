 ...
 * version.
 */
static int supports_rw_upgrade(const struct signed_header_version *current_rw,
			       const struct signed_header_version *to_rw)
{
	...
	case MAGIC_DAUNTLESS:
		if (current_less_than_15 && to_greater_than_15) {
			printf("Must upgrade to 0.0.15 first!\n");
			return false;
		} else {
			return true;
		}
	...
	for (i = 0; i < ARRAY_SIZE(sections); i++) {
		...
		if ((i == RW_A) || (i == RW_B)) {
			...
			if (a_newer_than_b(&sections[i].shv, &targ.shv[1]) ||
			    !td->upstart_mode) {
				if (supports_rw_upgrade(&targ.shv[1],
							&sections[i].shv)) {
