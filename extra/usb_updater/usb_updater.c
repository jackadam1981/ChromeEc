...
static void generate_reset_request(struct transfer_descriptor *td)
{
	...
	send_vendor_command(td, subcommand, command_body,
...
/* By default run on all current genreation boards of a particular board ID. */
			...
			struct board_id *bid)
{
	...
	if (bid_action == bid_set) {
		...
		/*
		 * Speculative assignment: the response is expected to by one
		...
		if (response_size == 1) {
			...
			fprintf(stderr, "Error %d wile setting board id\n",
				response);
		} else {
