 ...
 * and actual image transfer. Image is transferred to the CR50 in 1K PDUs, one
 * at a time. Each PDU starts with a header including the address where the
 * block is destined and its digest. In versions 1 and 2 the address in the
 * header is the absolute address to place the block to, in version 3 and
 ...
 * connection. The first PDU does not have any data, receiving such a PDU
			     ...
			     size_t data_len)
{
	...
	while (data_len) {
		...
		if (td->ep_type == usb_xfer) {
			 ...
			 * would indicate a syncrhonization problem).
			 */
