 ...
 * for both active RO and RW images running on the target.
 *
...
/* Header versions retrieved from the target. */
...
/* This array describes all four sections of the image being programmed. */
static struct {
	const char *name;
	uint32_t    offset;
	uint32_t    size;
...
/*
 * Pick sections to transfer based on information about the target, the new
 * image, and the protocol the target is running.
 */
static void pick_sections(struct transfer_descriptor *td)
{
	...
	for (i = 0; i < ARRAY_SIZE(sections); i++) {
		 ...
		 * Is it newer in the new image than the running RW section on
		 * the device?
		 */
...
int main(int argc, char *argv[])
{
	...
	for (j = 0; j < ARRAY_SIZE(sections); j++)
		if (sections[j].ustatus == not_possible) {
			fprintf(stderr,
				"Failed to update RO, run the command again\n");
			return 2;  /* This will allow scripting repeat attempts. */
		}
