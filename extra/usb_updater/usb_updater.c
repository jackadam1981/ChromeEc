 ...
 *
 * See docs/usb_updater.md for protocol details.
 *
 * This app provides backwards compatibility (down to protocol version 5 only),
 * to ensure that earlier CR50 devices still can be updated.
 */
...
#define MAX_BUF_SIZE (UPDATE_MAX_PDU_PAYLOAD_SIZE + sizeof(struct upgrade_pkt))
			     ...
			     size_t data_len)
{
	...
	while (data_len) {
		...
		payload_size = MIN(data_len, UPDATE_MAX_PDU_PAYLOAD_SIZE);
