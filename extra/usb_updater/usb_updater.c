...
/* check the offset here. */
	...
	while (data_len) {
		...
		SHA1_Init(&ctx);
		SHA1_Update(&ctx, &updu.cmd.block_base, sizeof(updu.cmd.block_base));
		...
		for (transfer_size = 0; transfer_size < payload_size;) {
			...
			chunk_size = MIN(chunk_len, payload_size - transfer_size);
			xfer(devh, ep_num, transfer_data_ptr, chunk_size, NULL, 0);
		...
		if (reply) {
			fprintf(stderr, "error: status 0x%08x remaining 0x%08x\n",
				reply, data_len);
			exit(1);
		}
		/*
