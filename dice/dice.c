...
const slice_ref_t kIdLabel = { 2, (const uint8_t *)"ID" };
const slice_ref_t kKeyPairLabel = { 8, (const uint8_t *)"Key Pair" };
...
static bool generate_key(
	const uint8_t input[DIGEST_BYTES], /* [IN] CDI_attest or UDS */
...
static void fill_dice_id_string(
	const uint8_t dice_id[DICE_ID_BYTES], /* [IN] IDS_ID or CDI_ID */
	uint8_t dice_id_str[DICE_ID_HEX_BYTES] /* [OUT] hexdump of this ID */
)
{
	size_t idx;
...
size_t get_dice_handover_bytes(
	...
	size_t offset, /* [IN] starting offset in the DICE handover struct */
	size_t size /* [IN] size of the DICE handover struct to copy */
)
{
	dice_ctx_t ctx;
