...
#define RET_IF_FAIL_RETVAL(f, e) \
	if (!f) {                \
		return e;        \
	}
#define RET_IF_FAIL_RETVAL_S(f, e, s)  \
	if (!f) {                      \
		__platform_log_str(s); \
		return e;              \
	}
...
// before we temporarily use dice_handover_t::options.sig_struct to calc
// signature.
...
const size_t kDiceHandoverSize = sizeof(dice_handover_t);
...
const uint8_t kIdSalt[64] = { 0xDB, 0xDB, 0xAE, 0xBC, 0x80, 0x20, 0xDA, 0x9F,
	...
	CWT_CLAIMS_BSTR_HDR,
	{
		...
		// cose_key_ecdsa_bstr_t
		{
			...
			// cose_key_ecdsa_t
			{
				// Map header: 6 entries
				CBOR_HDR1(kCborMap, 6),
				// 1. Key type: uint(1, 0bytes) => uint(2,
				// 0bytes)
				COSE_KEY_LABEL_KTY,
				CBOR_UINT0(2), // EC2
				...
				CBOR_NINT0(-1), // ECDSA w/ SHA-256
						...
						// }
				COSE_KEY_LABEL_KEY_OPS, CBOR_HDR1(kCborArr, 1),
				CBOR_UINT0(2),
...
const combined_hdr_t kCombinedHdrTemplate = {
	// dice_cert_chain_hdr_t cert_chain
	{
		...
		// cose_key_ecdsa_t
		{
			...
			COSE_KEY_LABEL_ALG,
			CBOR_NINT0(-1), // ECDSA w/ SHA-256
			// 3. Key ops: uint(4, 0bytes) => array(1) { uint(2,
			// 0bytes) }
			COSE_KEY_LABEL_KEY_OPS, CBOR_HDR1(kCborArr, 1),
			CBOR_UINT0(2),
			// 4. Curve: nint(-1, 0bytes) => uint(1, 0bytes)
		...
		// consists of hdr=cdi_cert_hdr_t, payload=cwt_claims_bstr_t,
		// sig=cbor_bstr64_t
	},
	// cdi_cert_hdr_t cert
	{
...
const slice_ref_t kCdiAttestLabel = { 10 /* strlen("CDI_Attest") */,
				      (uint8_t *)"CDI_Attest" };
const slice_ref_t kCdiSealLabel = { 8 /* strlen("CDI_Seal") */,
				    (uint8_t *)"CDI_Seal" };
...
static bool calc_cdi(const uint8_t uds[DIGEST_BYTES], // [IN] UDS
...
)
{
	...
	RET_IF_FAIL_S(__platform_sha256(inputs, inputs_digest),
		      "Failed to hash inputs");
...
static inline void fill_inputs_seal(const dice_ctx_t *ctx, // [IN] dice context
...
// Assumes that ctx->cfg and CfgDescr in ctx->output are already filled.
static inline void fill_inputs_attest(const dice_ctx_t *ctx, // [IN] dice
							     // context
				      cdi_attest_inputs_t *inputs // [OUT]
								  // inputs
)
{
...
// Assumes that ctx->cfg and CfgDescr in ctx->output are already filled.
static inline bool calc_cdi_attest(const dice_ctx_t *ctx, // [IN] dice context
				   uint8_t cdi[DIGEST_BYTES] // [OUT] CDI
)
{
...
// Assumes that ctx->cfg and CfgDescr in ctx->output are already filled.
static inline bool calc_cdi_seal(const dice_ctx_t *ctx, // [IN] dice context
				 uint8_t cdi[DIGEST_BYTES] // [OUT] CDI
)
{
...
// - Debug - in all other cases
static inline uint8_t calc_mode(const dice_ctx_t *ctx // [IN] dice context
)
{
	bool allows_normal =
		__platform_aprov_status_allows_normal(ctx->cfg.aprov_status);
	if (allows_normal) {
...
// Generates CDI cert signature for the initialized builder with pre-filled CWT
// claims.
static inline bool fill_cdi_cert_signature(dice_ctx_t *ctx, // [IN/OUT] dice
							    // context
					   ecdsa_handle_t key // [IN] key handle
							      // to sign
)
{
	uint8_t *sig_struct = ((uint8_t *)&ctx->output.payload) -
			      sizeof(cdi_sig_struct_hdr_t);
...
static bool generate_key(const uint8_t input[DIGEST_BYTES], // [IN] CDI_attest
							    // or UDS value
...
)
{
	...
	RET_IF_FAIL_S(__platform_hkdf_sha256(input_slice, kAsymSaltSlice,
...
static bool generate_id_from_pub_key(const ecdsa_public_t *pub_key, // [IN]
								    // public
								    // key
				     uint8_t dice_id[DICE_ID_BYTES] // [OUT]
								    // generated
								    // id
)
{
	slice_ref_t pub_key_slice = { sizeof(ecdsa_public_t),
...
static inline void hexdump_byte(uint8_t byte, // [IN] byte to hexdump
				uint8_t *str // [OUT] str (always 2 bytes) with
					     // hexdump
)
{
...
static void fill_dice_id_string(const uint8_t dice_id[DICE_ID_BYTES], // [IN]
								      // {IDS,CDI}
								      // ID as
								      // bytes
								      // (20
								      // bytes)
				uint8_t dice_id_str[DICE_ID_HEX_BYTES] // [OUT]
								       // hexdump
								       // of
								       // this
								       // same
...
// Assumes that all fields in COSE_Key except for X, y are already filled from
// template.
static inline void fill_cose_pubkey(const ecdsa_public_t *pub_key, // [IN] pub
								   // key
				    cose_key_ecdsa_t *cose_key // [IN/OUT] COSE
							       // key structure
)
{
...
// CDI_attest key.
static inline bool fill_cdi_details_with_key(dice_ctx_t *ctx, // [IN/OUT] dice
							      // context
					     ecdsa_handle_t cdi_key // [IN]
								    // CDI_attest
								    // key
								    // handle
)
{
	...
	RET_IF_FAIL_S(__platform_ecdsa_p256_get_pub_key(cdi_key, &cdi_pub_key),
		      "Failed to get CDI pubkey");
	fill_cose_pubkey(&cdi_pub_key, &cwt_claims->subject_pk.data);
	RET_IF_FAIL_S(generate_id_from_pub_key(&cdi_pub_key, cdi_id),
		      "Failed to generate CDI_ID");
...
// Assumes that ctx->cfg and CfgDescr in ctx->output are already filled.
static inline bool fill_cdi_details(dice_ctx_t *ctx // [IN/OUT] dice context
)
{
	...
	__platform_memcpy(&ctx->output.hdr, &kDiceHandoverHdrTemplate,
			  sizeof(dice_handover_hdr_t));
	RET_IF_FAIL_S(calc_cdi_attest(ctx, ctx->output.hdr.cdi_attest.value),
		      "Failed to calc CDI_attest");
	RET_IF_FAIL_S(calc_cdi_seal(ctx, ctx->output.hdr.cdi_seal.value),
		      "Failed to calc CDI_seal");

	RET_IF_FAIL_S(generate_key(ctx->output.hdr.cdi_attest.value, &cdi_key),
		      "Failed to generate CDI key");
...
// Fills UDS_ID, signature into the certificate in builder using generated UDS
// key. Assumes that all other fields of CDI certificate were filled already.
static inline bool fill_uds_details_with_key(dice_ctx_t *ctx, // [IN/OUT] dice
							      // context
					     ecdsa_handle_t uds_key // [IN] UDS
								    // key
								    // handle
)
{
	...
	RET_IF_FAIL_S(__platform_ecdsa_p256_get_pub_key(uds_key, &uds_pub_key),
		      "Failed to get UDS pubkey");
	RET_IF_FAIL_S(generate_id_from_pub_key(&uds_pub_key, uds_id),
		      "Failed to generate UDS_ID");
	fill_dice_id_string(uds_id, cwt_claims->iss.value); // ISS = hex(UDS_ID)
	RET_IF_FAIL_S(fill_cdi_cert_signature(ctx, uds_key),
		      "Failed to sign CDI cert");
	...
	__platform_memcpy(&ctx->output.options.dice_handover,
			  &kCombinedHdrTemplate, sizeof(combined_hdr_t));
	fill_cose_pubkey(
		&uds_pub_key,
...
static inline bool fill_uds_details(dice_ctx_t *ctx // [IN/OUT] dice context
)
{
	ecdsa_handle_t uds_key;
	bool result;

	RET_IF_FAIL_S(generate_key(ctx->cfg.uds, &uds_key),
		      "Failed to generate UDS key");
...
static inline void set_cbor_u32(uint32_t value, // [IN] value to set
				cbor_uint32_t *cbor_var // [OUT] CBOR UINT32
							// variable to fill
)
{
...
static inline bool fill_config_details(dice_ctx_t *ctx // [IN/OUT] dice context
)
{
	...
	RET_IF_FAIL_S(__platform_sha256(cfg_descr_slice,
					cwt_claims->cfg_hash.value),
		      "Failed to calc CfgDescr digest");
...
static inline bool generate_dice_handover(dice_ctx_t *ctx // [IN/OUT] dice
							  // context
)
{
...
size_t get_dice_handover_bytes(uint8_t *dest, // [OUT] destination buffer to
					      // fill
			       size_t offset, // [IN] starting offset in the
					      // DICE handover structure
			       size_t size // [IN] size of the DICE handover
					   // structure to copy
)
{
	dice_ctx_t ctx;
	...
	RET_IF_FAIL_RETVAL_S(__platform_get_dice_config(&ctx.cfg), 0,
			     "Failed to get DICE config");
