...
const struct cwt_claims_bstr_s kCwtClaimsTemplate = {
	CWT_CLAIMS_BSTR_HDR,
	{
...
const struct dice_handover_hdr_s kDiceHandoverHdrTemplate = {
...
const struct cdi_sig_struct_hdr_s kSigStructFixedHdr = {
...
const struct combined_hdr_s kCombinedHdrTemplate = {
	/* struct dice_cert_chain_hdr_s cert_chain */
	{
		 ...
		 */
		{
			 ...
			 *    array(1) { uint(2, 0bytes) }
			 */
			COSE_KEY_LABEL_KEY_OPS, CBOR_HDR1(CBOR_MAJOR_ARR, 1),
			CBOR_UINT0(2),
			/* 4. Curve: nint(-1, 0bytes) => uint(1, 0bytes) */
			COSE_KEY_LABEL_CRV,
			CBOR_UINT0(1), /* P-256 */
...
const struct slice_ref_s kCdiAttestLabel = { 10 /* strlen("CDI_Attest") */,
					     (const uint8_t *)"CDI_Attest" };
const struct slice_ref_s kCdiSealLabel = { 8 /* strlen("CDI_Seal") */,
					   (const uint8_t *)"CDI_Seal" };
const struct slice_ref_s kIdSaltSlice = { 64, (const uint8_t *)kIdSalt };
const struct slice_ref_s kAsymSaltSlice = { 64, (const uint8_t *)kAsymSalt };
const struct slice_ref_s kIdLabel = { 2, /* streln("ID") */
				      (const uint8_t *)"ID" };
const struct slice_ref_s kKeyPairLabel = { 8, /* strelen("Key Pair") */
...
)
{
	...
	struct slice_mut_s cdi_slice = digest_as_slice_mut(cdi);
...
)
{
	...
	struct slice_mut_s drbg_seed_slice = digest_as_slice_mut(drbg_seed);
...
)
{
	...
	struct slice_mut_s dice_id_slice = {
