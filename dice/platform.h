...
bool __platform_hkdf_sha256(slice_ref_t ikm, // [IN] input key material
			    slice_ref_t salt, // [IN] salt
			    slice_ref_t info, // [IN] info
			    slice_mut_t result // [IN/OUT] result.size sets
					       // length for hkdf, result.data
					       // is where the digest will be
					       // placed
);

// Calculate SH256 for the provided buffer
bool __platform_sha256(slice_ref_t data, // [IN] data to hash
		       uint8_t digest[DIGEST_BYTES] // [OUT] resulting digest
);

// Get DICE config
bool __platform_get_dice_config(dice_config_t *cfg // [OUT] DICE config
);
...
bool __platform_ecdsa_p256_sign(ecdsa_handle_t key, // [IN] ECDSA key handle
				slice_ref_t data, // [IN] data to sign
				uint8_t signature[ECDSA_SIG_BYTES] // [OUT]
								   // resulting
								   // signature:
								   // 64-bytes
								   // (R | S)
...
bool __platform_ecdsa_p256_get_pub_key(ecdsa_handle_t key, // [IN] ECDSA key
							   // handle
				       ecdsa_public_t *pub_key // [OUT] public
							       // key structure
);

// Free ECDSA key handle
void __platform_ecdsa_p256_free(ecdsa_handle_t key // [IN] ECDSA key handle
);

// Check if APROV status allows making 'normal' boot mode decision
bool __platform_aprov_status_allows_normal(uint32_t aprov_status // [IN] APROV
								 // status
);

// Print error string to log
void __platform_log_str(const char *str // [IN] string to print
);
