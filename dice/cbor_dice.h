...
} cdi_sig_struct_hdr_t;

// typedef struct {
//     cdi_sig_struct_hdr_t hdr;
//     cwt_claims_bstr_t payload;
// } cdi_sig_struct_t;
#define CDI_SIG_STRUCT_LEN (sizeof(cdi_sig_struct_hdr_t) + sizeof(cwt_claims_bstr_t))

////////////////////////////////////////////////////////////////////////////////
// CDI certificate = COSE_Sign1 structure - see go/gsc-dice

// Header of the certificate that includes protected & unprotected parameters,
// but not payload (CWT claims) and signature.
// This header is actually fixed (no variable fields).
// We need it to be able to prepend either it or Sig_structure header to payload:
// the former to return as a part of AndroidDiceHandover, the latter to calculate
...
typedef struct {
    ...
    uint8_t unprotected;
    // 3. Payload: bstr(CWT claims) - not fixed, cwt_claims_bstr_t
    // 4. Signature: bstr(64 bytes) - not fixed, cbor_bstr64_t
} cdi_cert_hdr_t;
...
// } cdi_cert_t;
#define CDI_CERT_LEN (sizeof(cdi_cert_hdr_t) + sizeof(cwt_claims_bstr_t) + sizeof(cbor_bstr64_t))
...
// We need it to be able to prepend either it + cert header or Sig_structure header
// to CWT claims: the former to return as a part of AndroidDiceHandover, the latter
// to calculate certificate signature.
typedef struct {
    // Array header: 2 elements
    uint8_t array_hdr;
    // 1. UDS pub key: COSE_Key
    cose_key_ecdsa_t uds_pub_key;
    // 2. CDI DICE cert: - not included,
    // consists of hdr=cdi_cert_hdr_t, payload=cwt_claims_bstr_t, sig=cbor_bstr64_t
} dice_cert_chain_hdr_t;
...
// Header of the DICE handover structure that contains the CDIs, but not the DICE chain.
// This header contains variable fields (CDIs).
// We need it to be able to prepend either it + DICE chain header + cert header or 
// Sig_structure header to CWT claims: the former to return as a part of AndroidDiceHandover,
