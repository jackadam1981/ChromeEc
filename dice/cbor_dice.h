...
#define CFG_DESCR_COMP_NAME                                                    \
	{                                                                      \
		CBOR_HDR1(kCborBstr, CFG_DESCR_COMP_NAME_VALUE_LEN), 'C', 'r', \
			'O', 'S', ' ', 'A', 'P', ' ', 'F', 'W'                 \
	}
...
// Size of TSTR containing an {UDS,CDI}_ID: (24 =< DICE_ID_HEX_LEN < 255) => 1
// byte size encoding
...
#define CWT_PROFILE_NAME                                                    \
	{                                                                   \
		CBOR_HDR1(kCborTstr, CWT_PROFILE_NAME_VALUE_LEN), 'a', 'n', \
			'd', 'r', 'o', 'i', 'd', '.', '1', '6'              \
	}
...
#define COSE_PARAM_BSTR                                                       \
	{                                                                     \
		/* BSTR of size 3 - see the rest of the struct */             \
		CBOR_HDR1(kCborBstr, 3), /* Map header: 1 element */          \
			CBOR_HDR1(kCborMap, 1), /* 1. Alg: uint(1, 0bytes) => \
						   nint(-7, 0bytes) */        \
			COSE_PARAM_LABEL_ALG, CBOR_NINT0(-7) /* ECDSA w/      \
								SHA-256 */    \
	}
...
#define CDI_SIG_STRUCT_CONTEXT                                               \
	{                                                                    \
		CBOR_HDR1(kCborBstr, CDI_SIG_STRUCT_CONTEXT_VALUE_LEN), 'S', \
			'i', 'g', 'n', 'a', 't', 'u', 'r', 'e', '1'          \
	}
...
#define CDI_SIG_STRUCT_LEN \
	(sizeof(cdi_sig_struct_hdr_t) + sizeof(cwt_claims_bstr_t))
...
// payload: the former to return as a part of AndroidDiceHandover, the latter to
// calculate certificate signature.
typedef struct {
	// Array header: 4 elements
	uint8_t array_hdr;
...
// } cdi_cert_t;
#define CDI_CERT_LEN                                          \
	(sizeof(cdi_cert_hdr_t) + sizeof(cwt_claims_bstr_t) + \
	 sizeof(cbor_bstr64_t))
...
// header to CWT claims: the former to return as a part of AndroidDiceHandover,
...
// able to prepend either it + DICE chain header + cert header or Sig_structure
// header to CWT claims: the former to return as a part of AndroidDiceHandover,
// the latter to calculate certificate signature.
typedef struct {
