...
typedef struct {
    ...
    uint8_t hidden_digest[DIGEST_BYTES]; // derived from owner seed, changes on owner clear
    uint8_t code_digest[DIGEST_BYTES]; // GSCVD digest (or 0..0 where not available)
