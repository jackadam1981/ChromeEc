...
)
{
	...
	written = fwrite(dice_handover, 1, size, f);
	fclose(f);
	return written == size;
}

#define MODIFY_RESULT
#ifdef MODIFY_RESULT
static size_t modify_result(
	uint8_t dice_handover[kDiceHandoverSize]
)
{
	memmove(dice_handover, dice_handover+0x48, kDiceHandoverSize-0x48);
	return kDiceHandoverSize-0x48;
}
#else
static size_t modify_result(
	uint8_t dice_handover[kDiceHandoverSize]
...
static bool test_dice_handover(const char *filename)
{
	...
	size = kDiceHandoverSize;
	size = modify_result(dice_handover);
	return save_to_file(filename, dice_handover, size);
}
