...
static int lid_opened;	/* Force lid opened */
...
static void lid_change_deferred(void)
{
	...
	if (new_open)
		...
		lid_opened ? lid_switch_open() : lid_switch_close();
}
 ...
 */
static int force_lid_open(struct host_cmd_handler_args *args)
{
	const struct ec_params_force_lid_open *p = args->params;
	lid_opened = p->enabled;
	return EC_RES_SUCCESS;
}
DECLARE_HOST_COMMAND(EC_CMD_FORCE_LID_OPEN, force_lid_open, EC_VER_MASK(0));
