...
static void physical_detect_done(void)
{
	...
	/* No longer care about power button presses */
...
{
	CPRINTF("\n\nPress the power button now!\n\n");
}
DECLARE_DEFERRED(physical_detect_prompt);

/**
 * Handle a physical present power button press
 *
...
static void physical_detect_check_press(void)
{
	...
	/* Ignore presses outside the expected interval interval */
...
int physical_detect_start(int is_long, void (*callback)(void))
{
	...
	/* Start capturing power button presses */
