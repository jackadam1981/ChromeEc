...
static inline void motion_sense_read(int i)
{
	int ret;
	const struct motion_sensor_t *sensor = &motion_sensors[i];
	struct motion_sensor_ctrl *ctrl = &sense_ctrl[i];
...
void motion_sense_task(void)
{
	...
	while (1) {
		 ...
		 * the base sensor.
		 */
		rotate(sense_ctrl[LOCATION_LID].xyz,
			&p_acc_orient->rot_align, sense_ctrl[LOCATION_LID].xyz);
		...
		rotate(sense_ctrl[LOCATION_BASE].xyz,
