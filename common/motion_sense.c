...
static void motion_sense_shutdown(void)
{
     ...
     *  that command disable double tap (it was set when we init the sensor).
     * + fastboot oem Powerdown
     *  ends up calling motion_sense_shutdown() while in RO.
     * For older RO (and this change), we do not re-enable double tap,
     * so ryu will ship with double tap disabled.
     * When we are in RW we are re-enabling double tap: that will be the
     * state when the user is shutting down the tablet.
