...
#define AUTO_CAL_DIR_THRESHOLD (ACCEL_G * 2 / 3)
#define AUTO_CAL_MAG_THRESHOLD (ACCEL_G / 50)
...
static struct vector acc_lid, acc_base, acc_lid_raw;
static float lid_angle_deg;

/* Vector pointing in direction of hinge. */
static struct vector hinge_axis;
 ...
 */
static struct vector rotate(const struct vector v, float (* const R)[3][3])
{
...
static float calculate_lid_angle(struct vector base, struct vector lid)
{
	...
	up = rotate(base, get_rot_up_direction());
...
void motion_sense_task(void)
{
	...
	/* Get the hinge axis. */
	hinge = get_hinge_axis();
	hinge_axis.x = hinge[0];
	hinge_axis.y = hinge[1];
	hinge_axis.z = hinge[2];

	while (1) {
		ts0 = get_time();

		/* Read all accelerations. */
		read_accel(ACCEL_ADDR_LID, &acc_lid.x, &acc_lid.y, &acc_lid.z);
		read_accel(ACCEL_ADDR_BASE, &acc_base.x, &acc_base.y,
				&acc_base.z);
		 ...
		 */
		acc_lid_raw = acc_lid;
		acc_lid = rotate(acc_lid, get_rot_sense_oreintation());
...
static int command_print_rotation(int argc, char **argv)
{
	...
	R = get_rot_sense_oreintation();
	ccprintf("Orientation R:\n%d\t%d\t%d\n%d\t%d\t%d\n%d\t%d\t%d\n\n",
	(int)((*R)[0][0]*100), (int)((*R)[0][1]*100), (int)((*R)[0][2]*100),
	(int)((*R)[1][0]*100), (int)((*R)[1][1]*100), (int)((*R)[1][2]*100),
	(int)((*R)[2][0]*100), (int)((*R)[2][1]*100), (int)((*R)[2][2]*100));

	R = get_rot_up_direction();
