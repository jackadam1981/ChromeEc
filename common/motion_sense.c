...
void motion_sense_task(void)
{
	...
	ret = lid->method->init(lid);
	if (ret != EC_SUCCESS)
		goto end_ms_task;

	ret = base->method->init(base);
	if (ret != EC_SUCCESS)
		goto end_ms_task;
...
static int host_cmd_motion_sense(struct host_cmd_handler_args *args)
{
	...
	case MOTIONSENSE_CMD_SENSOR_ODR:
		...
		if (in->sensor_odr.data != EC_MOTION_SENSE_NO_VALUE) {
			if (sensor->method->set_data_rate(sensor,
						      in->sensor_odr.data,
		...
		sensor->method->get_data_rate(sensor, &data);
	...
	case MOTIONSENSE_CMD_SENSOR_RANGE:
		...
		if (in->sensor_range.data != EC_MOTION_SENSE_NO_VALUE) {
			if (sensor->method->set_range(sensor,
		...
		sensor->method->get_range(sensor, &data);
...
static int command_accelresolution(int argc, char **argv)
{
	...
	if (argc >= 3) {
		 ...
		 */
		if (sensor->method->set_resolution(sensor, data, round)
			== EC_ERROR_INVAL)
			return EC_ERROR_PARAM2;
	} else {
		sensor->method->get_resolution(sensor, &data);
...
static int command_accel_data_rate(int argc, char **argv)
{
	...
	if (argc >= 3) {
		 ...
		 * return a parameter error.
		 */
		if (sensor->method->set_data_rate(sensor, data, round)
			== EC_ERROR_INVAL)
			return EC_ERROR_PARAM2;
	} else {
		sensor->method->get_data_rate(sensor, &data);
...
static int command_accel_read_xyz(int argc, char **argv)
{
	...
	sensor = &motion_sensors[id];
	sensor->method->read(sensor, &x, &y, &z);
...
static int command_accelerometer_interrupt(int argc, char **argv)
{
	...
	sensor->method->set_interrupt(sensor, thresh);
