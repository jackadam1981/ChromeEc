...
void motion_sense_task(void)
{
	...
	while (1) {
		...
		*lpc_status |= EC_MEMMAP_ACC_STATUS_BUSY_BIT;
		 ...
		 */
		((uint16_t *)lpc_data)[0] = (int)lid_angle_deg;
		((uint16_t *)lpc_data)[1] = acc_base_host[X];
		((uint16_t *)lpc_data)[2] = acc_base_host[Y];
		((uint16_t *)lpc_data)[3] = acc_base_host[Z];
		((uint16_t *)lpc_data)[4] = acc_lid_host[X];
		((uint16_t *)lpc_data)[5] = acc_lid_host[Y];
		((uint16_t *)lpc_data)[6] = acc_lid_host[Z];
