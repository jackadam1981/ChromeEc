...
motion_sense_handle_interrupt_change(struct motion_sensor_t *sensor, int odr)
{
	...
	if (odr > 0) {
		sample_interval_usec = SEC_UL / odr;
	}
	...
	if (enable_interrupt) {
		...
		/*
		 * Interrupts are disabled, we want to sleep at the sensor's
		 * requested data rate (not the actual one). This will allow us
		 * to miss samples that aren't needed. Example:
		 * - Requested ODR is 10Hz
		 * - Sensor can't accommodate 10Hz so it uses a rounded up value
		 *   of 12.5Hz.
		 * - We set the ec_rate to 100ms so that's the sleep interval of
		 *   the motion sense loop.
		 * - While the sensor is actually sampling every 80ms, we'll
		 *   sleep longer so on the 5th time we sample the sensor
		 *   (500ms) in, the sensor would have taken 6 samples, but we
		 *   would have discarded the 5th.
		 */
		sensor->config[motion_sense_get_ec_config()].ec_rate =
...
int motion_sense_set_data_rate(struct motion_sensor_t *sensor)
{
	...
	if (ret == EC_SUCCESS) {
		interrupt_mode = motion_sense_handle_interrupt_change(sensor, odr);
	}
