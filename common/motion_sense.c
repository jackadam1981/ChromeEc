...
const vector_3_t orientation_modes[] = {
	{ 0, 1, 0 },  /* PORTRAIT_MODE */
	{ 1, 0, 0 },  /* LANDSCAPE_MODE */
	{ 0, -1, 0 }, /* UPSIDE_DOWN_PORTRAIT_MODE */
	{ -1, 0, 0 },  /* UPSIDE_DOWN_LANDSCAPE_MODE */
};

/* If device naturally in landscape mode, must also rotate by landscape_ref */
const matrix_3x3_t landscape_ref = {
	{ 0,  FLOAT_TO_FP(-1),  0},
	{ FLOAT_TO_FP(1),  0,  0},
	{ 0,  0,  FLOAT_TO_FP(1)}
};
...
{
	enum motionsensor_orientation rotated_orientation = orientation;
	...
	case MOTIONSENSE_ORIENTATION_UPSIDE_DOWN_LANDSCAPE:
		orientation_v = &orientation_modes[orientation-1];
		break;
	default:
		return MOTIONSENSE_ORIENTATION_UNKNOWN;
	}
	rotate(*orientation_v, *s->rot_standard_ref, rotated_orientation_v);
	rotate(rotated_orientation_v, landscape_ref, rotated_orientation_v);
	for (x = 0; x < ARRAY_SIZE(orientation_modes); x++) {
		if (!memcmp(rotated_orientation_v, orientation_modes[x],
				sizeof(rotated_orientation_v))) {
			/*
			 * Known orientations are 1-based, so add one to convert
			 * index to a known enum motionsensor_orientation
			 * equivalent
			 */
			rotated_orientation = x + 1;
			break;
		}
	}
	...
	if (SENSOR_ACTIVE(sensor) && (sensor->state == SENSOR_INITIALIZED)) {
		...
		if (ORIENTATION_CHANGED(sensor) && (GET_ORIENTATION(sensor) !=
			...
			case MOTIONSENSE_ORIENTATION_LANDSCAPE:
				...
				break;
			case MOTIONSENSE_ORIENTATION_UPSIDE_DOWN_PORTRAIT:
				CPRINTS("Inv_Portrait");
				break;
			case MOTIONSENSE_ORIENTATION_UPSIDE_DOWN_LANDSCAPE:
				CPRINTS("Inv_Landscape");
				break;
			default:
				break;
			}
