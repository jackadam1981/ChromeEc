...
static void motion_sense_suspend(void)
{
	...
	/*
	 * During shutdown sequence some PMICs (ex:TPS65094x) turn off the
	 * sensor rails asynchronously to the EC. If we access the sensors
	 * when the sensor power rails are off we get I2C errors. To avoid
	 * this issue, defer switching the sensors rate with a configurable
	 * delay if in S3. By the time deferred function is serviced if the
	 * chipset is in S5 we can back out from switching the sensor rate.
	 *
