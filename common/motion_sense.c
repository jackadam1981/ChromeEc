...
void motion_sense_task(void)
{
	...
	while (1) {
		...
		*lpc_status |= EC_MEMMAP_ACC_STATUS_BUSY_BIT;
		...
		((uint16_t *)lpc_data)[1] = acc_base[X];
		((uint16_t *)lpc_data)[2] = acc_base[Y];
		((uint16_t *)lpc_data)[3] = acc_base[Z];
		((uint16_t *)lpc_data)[4] = acc_lid[X];
		((uint16_t *)lpc_data)[5] = acc_lid[Y];
		((uint16_t *)lpc_data)[6] = acc_lid[Z];
