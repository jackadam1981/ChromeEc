...
static void motion_sense_set_fifo_info(
		struct ec_response_motion_sense_fifo_info *fifo_info)
{
	...
	fifo_info->timestamp = fifo_timestamp.le.lo;
}
...
static int motion_sense_get_next_event(uint8_t *out)
{
	...
	motion_sense_set_fifo_info(&data->sensor_fifo.info);
...
static int host_cmd_motion_sense(struct host_cmd_handler_args *args)
{
	...
	case MOTIONSENSE_CMD_FIFO_INFO:
		motion_sense_set_fifo_info(&out->fifo_info);
