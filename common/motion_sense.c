...
void motion_sense_task(void)
{
	...
	while (1) {
		 ...
		 * Increment the counter and set the busy bit before writing
		 * the sensor data. Increment the counter and clear the busy
		 * but after writing the sensors data. On the host side, the
		 * host needs to make sure the busy bit is not set and that the
		 * counter remains the same before and after reading the data.
		 */
		sample_id = (sample_id + 1) & ~0x80;
		(*(mptr + EC_MEMMAP_GESTURE_SAMPLE_ID)) = sample_id | 0x80;
