...
#define IS_TIMESTAMP(d) ((d).flags & MOTIONSENSE_SENSOR_FLAG_TIMESTAMP)
...
static void motion_sense_fifo_ensure_space(void)
{
	...
	/*
	 * Pop at least 1 spot, but if the following conditions are met we will
	 * continue to pop:
	 * 1. We're operating with tight timestamps.
	 * 2. The new head isn't a timestamp.
	 * 3. We data that we can possibly pop.
	 *
	 * This is needed because if we pop a timestamp and the next head is
	 * data, the AP would assign a bad timestamp to it.
	 */
	do {
 ...
 * checking and is purely for convinience.
 */
static inline struct ec_response_motion_sensor_data *
motion_sense_peek_fifo_staged(size_t offset)
{
	return (struct ec_response_motion_sensor_data *)
			queue_get_write_chunk(&motion_sense_fifo, offset)
					.buffer;
}
