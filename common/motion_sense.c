...
#define REPORT_ID_3D_ACCEL                      0x01

/* Reports (double buffered) */
struct touch_report touch_reports[2];
...
static const uint8_t report_desc[] = {
        ...
        0x85, REPORT_ID_3D_ACCEL,         /*   Report ID (3DAccel) */
...
//HID_USAGE_SENSOR_DATA(HID_USAGE_SENSOR_DATA_MOTION_ACCELERATION,HID_USAGE_SENSOR_DATA_MOD_MAX),
        0x16,0x01,0x80,                /* HID_LOGICAL_MIN_16 */
        0x26,0xFF,0x7F,                /* HID_LOGICAL_MAX_16*/
...
//input reports (transmit)
        ...
        0x95, 1,                        /* HID_REPORT_COUNT */
        0xA1, 0x02,                    /* HID_COLLECTION, (Logical) */
        0x0A,0x10,0x08,                /*   SENSOR_EVENT_UNKNOWN */
        0x0A,0x11,0x08,                /*   SENSOR_EVENT_STATE_CHANGED */
        0x0A,0x12,0x08,                /*   SENSOR_EVENT_PROPERTY_CHANGED */
        0x0A,0x13,0x08,                /*   SENSOR_EVENT_DATA_UPDATED */
        0x0A,0x14,0x08,                /*   SENSOR_EVENT_POLL_RESPONSE */
        0x0A,0x15,0x08,                 /*  SENSOR_EVENT_CHANGE_SENSITIVITY*/
        0x0A,0x16,0x08,                 /*  SENSOR_EVENT_MAX_REACHED */
        0x0A,0x17,0x08,                 /*  SENSOR_EVENT_MIN_REACHED */
        0x0A,0x18,0x08,                 /*  SENSOR_EVENT_HIGH_THRESHOLD_CROSS_UPWARD*/
        0x0A,0x19,0x08,                 /*  SENSOR_EVENT_HIGH_THRESHOLD_CROSS_DOWNWARD*/
        0x0A,0x1A,0x08,                 /*  SENSOR_EVENT_LOW_THRESHOLD_CROSS_UPWARD*/
        0x0A,0x1B,0x08,                 /*  SENSOR_EVENT_LOW_THRESHOLD_CROSS_DOWNWARD*/
        0x0A,0x1C,0x08,                 /*  SENSOR_EVENT_ZERO_THRESHOLD_CROSS_UPWARD*/
        0x0A,0x1D,0x08,                 /*  SENSOR_EVENT_ZERO_THRESHOLD_CROSS_DOWNWARD*/
        0x0A,0x1E,0x08                 /*   SENSOR_EVENT_PERIOD_EXCEEDED */
        0x0A,0x1F,0x08                 /*   SENSOR_EVENT_FREQUENCY_EXCEEDED*/
        0x0A,0x20,0x08                 /*   SENSOR_EVENT_COMPLEX_TRIGGER */
        0x81, 0x03,                    /*   HID_INPUT(Const_Arr_Abs) */
        0xC0,                          /* HID_END_COLLECTION*/
        0x0A,0x53,0x04,                /* SENSOR_DATA_MOTION_ACCELERATION_X_AXIS*/
        ...
        0x55,0x0E,                     /* HID_UNIT_EXPONENT*/
        0x81, 0x03,                    /*   HID_INPUT(Const_Arr_Abs) */
        0x0A,0x54,0x04,                /* SENSOR_DATA_MOTION_ACCELERATION_Y_AXIS */
        0x16,0x01,0x80,                /* LOGICAL_MINIMUM (-32767)*/
        0x2A,0xFF,0x7F,                /*LOGICAL_MAXIMUM (32767)*/
        0x075, 16,                       /* HID_REPORT_SIZE */
        0x95, 1,                        /* HID_REPORT_COUNT */
        0x55,0x0E,                     /* HID_UNIT_EXPONENT*/
        0x81, 0x03,                    /*   HID_INPUT(Const_Arr_Abs) */
        0x0A,0x55,0x04,                /* SENSOR_DATA_MOTION_ACCELERATION_Z_AXIS*/
        0x16,0x01,0x80,                /* LOGICAL_MINIMUM (-32767)*/
        0x2A,0xFF,0x7F,                /*LOGICAL_MAXIMUM (32767)*/
        0x075, 16,                       /* HID_REPORT_SIZE */
        0x95, 1,                        /* HID_REPORT_COUNT */
        0x55,0x0E,                     /* HID_UNIT_EXPONENT*/
        0x81, 0x03,                    /*   HID_INPUT(Const_Arr_Abs) */
        0xC0,                          /* HID_END_COLLECTION  end of accelerometer TLC */
...
static size_t fill_hid_report(uint8_t* buffer, uint8_t report_id, const void* data,
			  size_t data_len)
{
	size_t response_len = I2C_HID_HEADER_SIZE + data_len;
	buffer[0] = response_len & 0xFF; //length byte 1
	buffer[1] = (response_len >> 8) & 0xFF; //length byte 2
	buffer[2] = report_id; //report id 1 byte
...
static int host_cmd_motion_sense(struct host_cmd_handler_args *args)
{
	...
	struct hid_ec_response_motion_sense *out = args->response;
	struct motion_sensor_t *sensor;
	struct hid_accel_report *report;
	...
	case I2C_HID_CMD_GET_REPORT:
		...
		out->len;
		out->x = sensor->xyz[X];
		out->data_y = sensor->xyz[Y];
		out->data_z = sensor->xyz[Z];
		mutex_unlock(&g_sensor_mutex);
