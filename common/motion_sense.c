...
void motion_sense_task(void)
{
	...
	while (1) {
		...
		/*
		 * Increment a sample id before and after updating sensor
		 * data. If host reads the same sample id before and after it
		 * reads the sensor data, then it is guaranteed to have data
		 * from the same sample.
		 */
		(*(mptr + EC_MEMMAP_GESTURE_SAMPLE_ID))++;
		...
		*(mptr + 0xb) = acc_baseZ & 0xff;
		...
		/* Increment sample id to signal we finished updating data. */
		(*(mptr + EC_MEMMAP_GESTURE_SAMPLE_ID))++;
