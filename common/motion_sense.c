...
#define CPRINTF(format, args...) info_printf(CC_MOTION_SENSE, format, ## args)
...
void motion_sense_task(void)
{
	...
	if (ret != EC_SUCCESS) {
		CPRINTF("Accelerometers failed to initialize. Stopping "
			"motion sense task");
		return;
	}
...
#ifdef CONFIG_CMD_LID_ANGLE
		if (accel_disp) {
			CPRINTF("ACC base=%-5d, %-5d, %-5d  lid=%-5d, "
					"%-5d, %-5d  a=%-6.1d r=%d",
...
void accel_int_lid(enum gpio_signal signal)
{
	 ...
	 */
	CPRINTF("Accelerometer wake-up interrupt occurred on lid");
}
...
void accel_int_base(enum gpio_signal signal)
{
	 ...
	 * Remove print statement when interrupt is used for real.
	 */
	CPRINTF("Accelerometer wake-up interrupt occurred on base");
}
...
static int host_cmd_motion_sense(struct host_cmd_handler_args *args)
{
	...
	default:
		CPRINTF("MS bad cmd 0x%x", in->cmd);
		return EC_RES_INVALID_PARAM;
	}
