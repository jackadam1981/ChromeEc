...
static void motion_sense_fifo_add_unit(struct ec_response_motion_sensor_data *data,
...
static void motion_sense_insert_timestamp(uint32_t time);
				...
				const timestamp_t *ts)
{
	...
	if (sensor->drv->load_fifo != NULL) {
		...
		if (ret == EC_SUCCESS) {
			...
			vector.data[Y] = v[Y];
			vector.data[Z] = v[Z];
			motion_sense_fifo_add_data(&vector, sensor, 3, __hw_clock_source_read());
		}
		sensor->last_collection = ts->le.lo;
	} else {
		ret = EC_ERROR_BUSY;
...
static void check_and_queue_gestures(uint32_t *event)
{
	...
	if (*event & CONFIG_GESTURE_TAP_EVENT) {
		...
		motion_sense_fifo_add_data(&vector, NULL, 0, __hw_clock_source_read());
#endif
		CPRINTS("double tap!");
...
static int host_cmd_motion_sense(struct host_cmd_handler_args *args)
{
	...
	case MOTIONSENSE_CMD_SENSOR_ODR:
		...
		if (in->sensor_odr.data != EC_MOTION_SENSE_NO_VALUE) {
			...
			CPRINTS("amstan ignore for timinig");
#endif
