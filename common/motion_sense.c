 ...
 */
unsigned motion_interval;

/* Delay between FIFO interruption. */
static unsigned motion_int_interval;
...
int motion_sense_set_motion_intervals(void)
{
	...
	for (i = 0; i < motion_sensor_count; ++i) {
		...
		if ((ec_int_rate == 0 && sensor_ec_rate != 0) ||
		    (sensor_ec_rate != 0 && sensor_ec_rate < ec_int_rate))
	...
	motion_int_interval = ec_int_rate * MSEC;
	if (wake_up)
...
void motion_sense_task(void)
{
	...
	while (1) {
		...
		if (fifo_flush_needed ||
		    ...
		    (motion_int_interval > 0 &&
		     time_after(ts_end_task.val,
				ts_last_int.val + motion_int_interval))) {
			if (!fifo_flush_needed)
