...
static void motion_sense_fifo_ensure_space(void)
{
	...
	/*
	 * Pop at least 1 spot, but if the new head isn't a timestamp and we
	 * have tight timestamps enabled, keep going. This is needed because if
	 * we pop a timestamp and the next head is data, the AP would assign a
	 * bad timestamp to it.
	 */
	do {
