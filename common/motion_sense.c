...
static int host_cmd_motion_sense(struct host_cmd_handler_args *args)
{
	...
	case MOTIONSENSE_CMD_INFO:
		...
		case 1:
			...
			break;
		default:
			CPRINTF("[%T MS bad sensor number %d]\n",
	...
	case MOTIONSENSE_CMD_EC_RATE:
		 ...
		 * has a value.
		 */
		if (in->ec_rate.data != EC_CMD_MOTION_SENSE_NO_VALUE) {
	...
	case MOTIONSENSE_CMD_SENSOR_ODR:
		...
		if (in->sensor_odr.data != EC_CMD_MOTION_SENSE_NO_VALUE) {
	...
	case MOTIONSENSE_CMD_SENSOR_RANGE:
		switch (in->sensor_range.sensor_num) {
		case 0:
			id = ACCEL_BASE;
			break;
		case 1:
		...
		/* Set new datarate if the data arg has a value. */
		if (in->sensor_range.data != EC_CMD_MOTION_SENSE_NO_VALUE) {
