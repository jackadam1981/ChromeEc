...
static int moc_fit(struct mag_cal_t *moc, fpv3_t bias, fp_t *radius)
{
	...
	moc->acc_w[X] = moc->acc_w[X] * FLOAT_TO_FP(-1);
	moc->acc_w[Y] = moc->acc_w[Y] * FLOAT_TO_FP(-1);
	moc->acc_w[Z] = moc->acc_w[Z] * FLOAT_TO_FP(-1);
	moc->acc_w[W] = moc->acc_w[W] * FLOAT_TO_FP(-1);
...
int mag_cal_update(struct mag_cal_t *moc, const intv3_t v)
{
	...
	moc->acc[0][1] += fp_sq(v[X]);
	moc->acc[0][2] += fp_sq(v[X]);
	...
	if (moc->batch_size > 0 && moc->nsamples >= moc->batch_size) {
		fp_t inv = fp_div(FLOAT_TO_FP(1.0f),
