...
#ifdef CONFIG_ROLLBACK
	if (good && rw_rollback_version > min_rollback_version) {
		/*
		 * We should increment rollback now. This will fail if the
		 * rollback block is protected (RW image will unprotect that
		 * block later on).
		 */
...
static void rwsig_jump_deferred(void)
{
	if (!rwsig_aborted)
		rwsig_jump_now();
}
DECLARE_DEFERRED(rwsig_jump_deferred);
...
void rwsig_task(void)
{
	if (system_get_image_copy() == SYSTEM_IMAGE_RO &&
		...
		hook_call_deferred(&rwsig_jump_deferred_data,
				CONFIG_RWSIG_JUMP_TIMEOUT*MSEC);
	}
}
#endif
