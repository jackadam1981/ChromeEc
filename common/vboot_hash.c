...
static int read_and_hash_chunk(int offset, int size)
{
	...
	if (rv == EC_ERROR_BUSY) {
		...
		hook_call_deferred(&__deferred_vboot_hash_next_chunk,
				   WORK_INTERVAL_US);
		return rv;
...
static void vboot_hash_next_chunk(void)
{
	...
	hook_call_deferred(&__deferred_vboot_hash_next_chunk, WORK_INTERVAL_US);
}
			    ...
			    const uint8_t *nonce, int nonce_size)
{
	...
	hook_call_deferred(&__deferred_vboot_hash_next_chunk, 0);
