...
static int command_hash(int argc, char **argv)
{
	...
	if (argc == 2) {
		if (!strcasecmp(argv[1], "abort")) {
			...
			return vboot_hash_start(CONFIG_EC_WRITABLE_STORAGE_OFF
						+ CONFIG_RW_STORAGE_OFF,
						CONFIG_RW_SIZE, NULL, 0);
		} else if (!strcasecmp(argv[1], "ro")) {
			return vboot_hash_start(CONFIG_EC_PROTECTED_STORAGE_OFF
						+ CONFIG_RO_STORAGE_OFF,
						CONFIG_RO_SIZE, NULL, 0);
		}
	}
