...
#define CPRINTF(format, args...) info_printf(CC_VBOOT, format, ## args)
...
static void vboot_hash_next_chunk(void)
{
	...
	if (curr_pos >= data_size) {
		...
		CPRINTF("hash done %.*h", SHA256_DIGEST_SIZE, hash);
			    ...
			    const uint8_t *nonce, int nonce_size)
{
	...
	CPRINTF("hash start 0x%08x 0x%08x", offset, size);
	SHA256_init(&ctx);
	if (nonce_size)
...
int vboot_hash_invalidate(int offset, int size)
{
	...
	/* Invalidate the hash */
	CPRINTF("hash invalidated 0x%08x 0x%08x", offset, size);
	vboot_hash_abort();
	return 1;
}
