...
enum power_state trickle_charge(struct power_state_context *ctx)
{
	...
	/* 4 hours is enough to pre-charge a large battery (8000mAh)
	...
	/* If the trickle charging current dropped to zero, raise charging
	 ...
	 */
	if (kicking_count < 5 && batt->voltage >
			(binfo->voltage_min * 105 / 100)) {
		kicking_count++;
	...
	/* R = dV / A_batt */
	charging_resistance = (curr->charging_voltage * LOG_BUFFER_SIZE -
		sum_volt) * 1000 / sum_curr;

	/* V_desired = A_target * R + V_batt */
	desired_volt = (LOG_BUFFER_SIZE * (1 + batt->desired_current) *
		charging_resistance + sum_volt * 1000) / 1000 / LOG_BUFFER_SIZE;
