...
#define CPRINTS3(format, args...) cprints(CC_USBPD, "\033[35m"  format  "\33[0m", ##args)
 ...
 *                         flags[port] |= APDO_FLAGS_MORE_PWR;
	...
	if (timeout.val < next_wakeup_time)
		timeout.val = next_wakeup_time;

	if (ret)
		CPRINTS3("APDO: C%d %s power request", port,
			candidate_mv > last_mv ? "more" : "less");
...
DECLARE_CONSOLE_COMMAND(apdo, command_apdo, "debug|enable|disable|setkmore <int>|setkless <int>", "Print/set apdo state.");
