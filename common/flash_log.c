 ...
 *
 * To satisfy flash access limitations, log entries are padded to reach proper
 * flash programming alignment.
 *
 ...
 * new events while a log entry is being saved or retrieved will be silently
 * ignored. Attempts to retrieve an entry while another entry is being
 * saved or retrieved will return the appropriate return value.
 *
 * At run time log compaction is attempted if a request to add an entry is
 * made and the log is more than 90% full. If compaction is not possible and
 * the new entry does not fit, it would be silently dropped.
 *
...
static uint8_t log_initted;
static uint8_t log_event_in_progress;

/*
 * Callback set by the platform if flash log space access requires additional
 * access control.
 */
...
{
	/* Entry header aligned to the flash write size boundary. */
	union {
