...
void reassembly_get_payload(void *c, struct reassembly_payload *payload)
{
	...
	payload->rs_data_size = be16toh(ctx->pdu.size);
}
				       ...
				       size_t count)
{
	...
	ccprintf("%s:%d verification failed\n", __func__, __LINE__);
	return RS_CHECK_ERROR;
}

/* Check if the PDU was received properly. */
int upgrade_pdu_sha1_check(const void *check, size_t check_size,
			   const void *data, size_t data_size)
{
	...
	if (memcmp(sha1_digest, check, check_size)) {
		ccprintf("%s:%d PDU check failed size %d %.10h", __func__, __LINE__, data_size, data);
		return 1;
	}
