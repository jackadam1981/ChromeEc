...
int tcpm_transmit(int port, enum tcpm_transmit_type type, uint16_t header,
		   const uint32_t *data)
{
	static uint8_t out[4*PDO_MAX_OBJECTS+1] = {TCPC_REG_TX_DATA};
	...
	if (cnt > 0) {
		memcpy(&out[1], data, cnt);
		i2c_lock(I2C_PORT_TCPC, 1);
		rv = i2c_xfer(I2C_PORT_TCPC, I2C_ADDR_TCPC(port), out, cnt + 1,
