...
static int command_set_mode(int argc, char **argv)
{
	...
	case 1:
		for (i = 0; i < USB_PORT_COUNT; i++)
			ccprintf("Port %d: %s\n",
				 i, usb_ports[i].charge_mode ? "on" : "off");
		return EC_SUCCESS;
	}
...
static void usb_port_init(void)
{
	...
	for (i = 0; i < USB_PORT_COUNT; i++)
		usb_port_set_enabled(i, prev[i].charge_mode);
}
