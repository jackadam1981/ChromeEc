...
static int handle_command(char *input)
{
	...
	/* The next 2 hex digits should be the length of the raw command. */
	if (i+2 >= input_len)
		goto command_has_error;
	for (j = 0; j < 2; j++) {
		if ((input[i+j] >= 'a') && (input[i+j] <= 'f'))
			command_len |= ((input[i+j] - ('a' - 10)) << (4*(1-j)));
		else if ((input[i+j] >= '0') && (input[i+j] <= '9'))
			command_len |= (((input[i+j] - '0') << (4*(1-j))));
	}
	i += 2;

	/* The next 2 hex digits should be the CRC8. */
	if (i+2 >= input_len)
		goto command_has_error;
	for (j = 0; j < 2; j++) {
		if ((input[i+j] >= 'a') && (input[i+j] <= 'f'))
			packed_crc8 |= ((input[i+j] - ('a' - 10)) << (4*(1-j)));
		else if ((input[i+j] >= '0') && (input[i+j] <= '9'))
			packed_crc8 |= (((input[i+j] - '0') << (4*(1-j))));
	}
	i += 2;

	/* Then, one more ampersand. */
	if (input[i] != '&')
		goto command_has_error;
	i++;
