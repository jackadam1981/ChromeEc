...
static int command_atomic_add(int argc, char **argv)
{
	uint32_t a, b;
	ccprintf("new_atomic_add\n");
	ccprintf("1 + 2\n");
	a = 1;
	b = 0;
	b = atomic_add(&a, 2);
	...
	ccprintf("0xffffff00 + 0xfff\n");
	a = 0xffffff00;
	b = 0;
	b = atomic_add(&a, 0xfff);

	ccprintf("a: %d\n", a);
	ccprintf("b: %d\n", b);


	ccprintf("old_atomic_add\n");
	ccprintf("1 + 2\n");
	a = 1;
	deprecated_atomic_add(&a, 2);

	ccprintf("a: %d\n", a);

	ccprintf("0xffffff00 + 0xfff\n");
	a = 0xffffff00;
	deprecated_atomic_add(&a, 0xfff);
...
DECLARE_CONSOLE_COMMAND(atomic_add, command_atomic_add, "atomic_add", "atomic");

static int command_atomic_sub(int argc, char **argv)
{
	uint32_t a, b;
	ccprintf("new_atomic_sub\n");
	...
	ccprintf("old_atomic_sub\n");

	ccprintf("4 - 2\n");
	a = 4;
	deprecated_atomic_sub(&a, 2);

	ccprintf("a: %d\n", a);

	ccprintf("4 - 6\n");
	a = 4;
	deprecated_atomic_sub(&a, 6);
...
DECLARE_CONSOLE_COMMAND(atomic_sub, command_atomic_sub, "atomic_sub", "atomic");

static int command_atomic_or(int argc, char **argv)
{
	uint32_t a, b;
	ccprintf("new_atomic_or\n");

	ccprintf("0xa00005 |\n");
	ccprintf("0x0c00a0\n");
	a = 0xa00005;
	b = 0;
	b = atomic_or(&a, 0x0c00a0);

	ccprintf("a: %08x\n", a);
	ccprintf("b: %08x\n", b);

	ccprintf("old_atomic_or\n");

	ccprintf("0xa00005 |\n");
	ccprintf("0x0c00a0\n");
	a = 0xa00005;
	deprecated_atomic_or(&a, 0x0c00a0);
...
DECLARE_CONSOLE_COMMAND(atomic_or, command_atomic_or, "atomic_or", "atomic");

static int command_atomic_clear_bit(int argc, char **argv)
{
	uint32_t a;
	ccprintf("new_atomic_clear_bit\n");

	ccprintf("0xffffffff & ~(0xf0f0ff21)\n");
	a = 0xffffffff;
	atomic_clear_bits(&a, 0xf0f0ff21);

	ccprintf("a: %08x\n", a);

	ccprintf("old_atomic_clear_bit\n");

	ccprintf("0xffffffff & ~(0xf0f0ff21)\n");
	a = 0xffffffff;
	deprecated_atomic_clear_bits(&a, 0xf0f0ff21);

	ccprintf("a: %08x\n", a);

	return EC_SUCCESS;
}
DECLARE_CONSOLE_COMMAND(atomic_clr_bit, command_atomic_clear_bit, "atomic_clear_bit", "atomic");

static int command_atomic_read_clear(int argc, char **argv)
{
	uint32_t a;
	ccprintf("new_atomic_read_clear\n");

	ccprintf("a:0xffffffff\n");
	a = 0xffffffff;
	atomic_read_clear(&a);

	ccprintf("a: %08x\n", a);

	ccprintf("old_atomic_read_clear\n");

	ccprintf("a:0xffffffff\n");
	a = 0xffffffff;
	deprecated_atomic_read_clear(&a);

	ccprintf("a: %08x\n", a);

	return EC_SUCCESS;
}
DECLARE_CONSOLE_COMMAND(atomic_rd_clr, command_atomic_read_clear, "atomic_rd_clr", "atomic");
