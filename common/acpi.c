...
static char read_battery_string(size_t field_offset, size_t field_size, size_t str_offset)
{
	uint8_t battery_index = *host_get_memmap(EC_MEMMAP_BATT_INDEX);
	CPRINTS("Reading ACPI battery string for battery index %d", battery_index);
	...
	return *(const char *)((uintptr_t)(&battery_static[battery_index]) + field_offset + str_offset);
}
#endif
...
static char strings_fifo_read() {
	char out = strings_fifo_read_work(strings_fifo_offset);
	CPRINTS("ACPI strings FIFO read @ %d => %d", strings_fifo_offset, out);
	if (out != 0) {
...
#endif
		case EC_ACPI_MEM_STRINGS_FIFO:
			CPRINTS("Handling ACPI strings FIFO write (index %d)", data);
