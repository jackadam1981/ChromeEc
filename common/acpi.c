...
/*
 * Current DPTF profile number. This is by default initialized to 1 to allow
 * use of default DPTF profile on host even if multi-profile DPTF is not
 * supported.
 */
...
int acpi_ap_to_ec(int is_cmd, uint8_t value, uint8_t *resultptr)
{
	...
	if (acpi_cmd == EC_CMD_ACPI_READ && acpi_data_count == 1) {
		...
		case EC_ACPI_MEM_DEVICE_ORIENTATION:
			result = (tablet_get_mode() & EC_ACPI_MEM_TBMD_MASK)
