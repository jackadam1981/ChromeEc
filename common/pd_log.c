...
/* Event log FIFO */
#define LOG_SIZE CONFIG_USB_PD_LOG_SIZE
static uint8_t log_events[LOG_SIZE];
...
#define ENTRY_SIZE(payload_sz) (sizeof(struct ec_response_pd_log)+(payload_sz))
		  ...
		  uint16_t data, void *payload)
{
	...
	while ((LOG_SIZE - (current_tail - log_head)) < total_size) {
		...
		interrupt_disable();
		oldest = (void *)(log_events + (log_head & (LOG_SIZE - 1)));
	...
	r = (struct ec_response_pd_log *)(log_events +
	    (current_tail & (LOG_SIZE - 1)));
...
retry:
	...
	entry = (struct ec_response_pd_log *)(log_events +
		(current_head & (LOG_SIZE - 1)));
	total_size = ENTRY_SIZE(PD_LOG_SIZE(entry->size_port));
	memcpy(r, entry, total_size);
