		  ...
		  uint16_t data, void *payload)
{
	...
	size_t current_tail, first_byte;
	...
	first_byte = MIN(payload_size, (LOG_SIZE -
			 ((current_tail + 1) & (LOG_SIZE - 1))) * UNIT_SIZE);
	memcpy(r->payload, payload, first_byte);
	if (first_byte < total_size * UNIT_SIZE)
		memcpy(log_events, ((uint8_t *)payload) + first_byte,
			total_size * UNIT_SIZE - first_byte);
