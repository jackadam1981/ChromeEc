...
#define ENTRY_SIZE(payload_sz) (1+DIV_ROUND_UP((payload_sz), \
				sizeof(struct ec_response_pd_log)))
		  ...
		  uint16_t data, void *payload)
{
	...
	first = MIN(payload_size, LOG_SIZE -
		((current_tail + 1) & (LOG_SIZE - 1)));
	memcpy(r->payload, payload, first);
	if (first < total_size)
