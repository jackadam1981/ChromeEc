...
#define CPRINTF(format, args...) info_printf(CC_SWITCH, format, ## args)
...
static void set_pwrbtn_to_pch(int high)
{
	...
	if (!high && charge_want_shutdown()) {
		CPRINTF("PB PCH pwrbtn ignored due to battery level");
		high = 1;
	}

	CPRINTF("PB PCH pwrbtn=%s", high ? "HIGH" : "LOW");
	gpio_set_level(GPIO_PCH_PWRBTN_L, high);
}

void power_button_pch_release(void)
{
	CPRINTF("PB PCH force release");
...
static void set_initial_pwrbtn_state(void)
{
	...
	if (system_jumped_to_this_image() &&
		 ...
		 */
		if (power_button_is_pressed()) {
			CPRINTF("PB init-jumped-held");
			set_pwrbtn_to_pch(0);
		} else {
			CPRINTF("PB init-jumped");
		}
...
static void state_machine(uint64_t tnow)
{
	...
	case PWRBTN_STATE_T1:
		 ...
		 */
		if (chipset_in_state(CHIPSET_STATE_ANY_OFF))
			CPRINTF("PB chipset already off");
		else
			set_pwrbtn_to_pch(0);
...
void power_button_task(void)
{
	...
	while (1) {
		...
		CPRINTF("PB task %d = %s", pwrbtn_state,
		...
		if (tsleep > t) {
			 ...
			 * early.)
			 */
			CPRINTF("PB task %d = %s, wait %d", pwrbtn_state,
