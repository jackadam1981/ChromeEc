...
static void state_machine(uint64_t tnow)
{
	...
	case PWRBTN_STATE_INIT_ON:
		 ...
		 * ready to supply sufficient power.Check every 100 milliseconds
		 * , and give up CONFIG_POWER_BUTTON_INIT_TIMEOUT seconds after
		 * the PB task was started. Here, it is important to check the
		 * current time against PB task start time to prevent
		 * unnecessary timeouts happening in recovery case where the
		 * tasks could start as late as 30 seconds after EC reset.
		 */
