...
/*
 * Return the size of the the specified bank
...
int flash_bank_size(int bank)
{
	...
	for (i = 0; i < ARRAY_SIZE(flash_bank_array); i++) {
		if (bank < flash_bank_array[i].sector_nb)
			return flash_bank_array[i].sector_size;
		bank -= flash_bank_array[i].sector_nb;
	}
	return -1;
}
 ...
 * Return -1 is the offset is not at the beginning of that bank.
 */
...
#if (EC_VER_MASK(0) != CONFIG_FLASH_ERASE_SUPPORT)
...
#error "Flash: Bank size expected bigger or equal to erase size."
		...
		r->banks[0].sector_protect_size = CONFIG_FLASH_ERASE_SIZE;
