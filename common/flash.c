...
int flash_erase(int offset, int size)
{
	...
	if (!flash_range_ok(offset, size, CONFIG_FLASH_ERASE_SIZE))
		return EC_ERROR_INVAL;  /* Invalid range */
	/* check in the physical function otherwise */
#endif
...
static int flash_command_erase(struct host_cmd_handler_args *args)
{
	...
	const struct ec_params_flash_erase *params = args->params;

#if CONFIG_FLASH_ERASE_SUPPORT != 1
	const struct ec_params_flash_erase_v1 *p = args->params;
	if (args->version > 0) {
		cmd = p->cmd;
		params = &p->params;
	}
	...
	if (system_unsafe_to_overwrite(params->offset, params->size))
		return EC_RES_ACCESS_DENIED;
...
#endif
		if (flash_erase(params->offset, params->size))
			rc = EC_RES_ERROR;
		break;
#ifdef HAS_TASK_FLASHERASE
