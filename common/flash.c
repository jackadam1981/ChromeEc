...
int crec_flash_read_pstate_poweron_conf(uint8_t *poweron_conf)
{
	...
	if ((pstate->version == PERSIST_STATE_VERSION) &&
		...
		memcpy(poweron_conf, pstate->poweron_conf,
		       CONFIG_POWERON_CONF_LEN);
		return EC_SUCCESS;
	}
...
int crec_flash_write_pstate_poweron_conf(const uint8_t *poweron_conf)
{
	...
	if (sizeof(newpstate.poweron_conf) != CONFIG_POWERON_CONF_LEN) {
		return EC_ERROR_INVAL;
	}
	 ...
	 */
	memset(newpstate.poweron_conf, 0, CONFIG_POWERON_CONF_LEN);
