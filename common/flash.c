...
extern const uint8_t __pstate_data_offset[];
...
{
	uintptr_t addr = (uintptr_t)__pstate_data_offset;
...
static int command_flash_write(int argc, char **argv)
{
	...
	/* If additional command, it's the offset to the pattern */
	if (argc >= 4) {
		char *e;

		patoffs = (uint32_t)strtoi(argv[3], &e, 0);
		if (*e)
