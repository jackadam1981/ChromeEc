 ...
 */
const char *crec_flash_read_pstate_poweron_conf(void)
{
	const struct persist_state *pstate =
		(const struct persist_state *)flash_physical_dataptr(
			CONFIG_FW_PSTATE_OFF);
	...
	if ((pstate->version == PERSIST_STATE_VERSION) &&
		...
		return (const char *)(pstate->poweron_conf);
	}
 ...
 * @return EC_SUCCESS, or nonzero if error.
 */
int crec_flash_write_pstate_poweron_conf(const char *poweron_conf)
{
	int length;
	struct persist_state newpstate;
	const struct persist_state *pstate =
	...
	length = strnlen(poweron_conf, sizeof(newpstate.poweron_conf));
	if (length > CONFIG_POWERON_CONF_LEN) {
		return EC_ERROR_INVAL;
	}
	 ...
	 * be shorter than the buffer so a null terminator always remains.
	 */
	memset(newpstate.poweron_conf, '\0', sizeof(newpstate.poweron_conf));
	memcpy(newpstate.poweron_conf, poweron_conf, length);
