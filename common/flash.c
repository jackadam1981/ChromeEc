...
static enum ec_status flash_command_get_info(struct host_cmd_handler_args *args)
{
	...
	if (args->version >= 2) {
		 ...
		 * r_2->banks. For those who wonder why we are passing the whole
		 * response structure instead of pointer to r_2->banks: The
		 * response structure is marked as '__ec_align4' but it's packed
		 * and aligned only if CONFIG_HOSTCMD_ALIGNED is enabled. On
		 * most systems the option is not enabled, so the response
		 * structure is packed only. It means that the structure can be
		 * placed at ANY address. Passing the response structure gives
		 * information to the compiler how members should be accessed.
