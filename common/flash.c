...
static int flash_range_ok(int offset, int size_req, int align)
{
	if (offset < 0 || size_req < 0 ||
	    offset > CONFIG_FLASH_SIZE * 1024 ||
	    size_req > CONFIG_FLASH_SIZE * 1024 ||
	    offset + size_req > CONFIG_FLASH_SIZE * 1024 ||
...
int flash_dataptr(int offset, int size_req, int align, const char **ptrp)
{
	...
	return CONFIG_FLASH_SIZE * 1024 - offset;
}
#endif
...
#error "Flash: Bank size expected bigger or equal to erase size."
	...
	struct ec_flash_bank single_bank = {
		.count = CONFIG_FLASH_SIZE * 1024 / CONFIG_FLASH_BANK_SIZE,
	...
	if (args->version >= 2) {
		...
		r_2->flash_size = CONFIG_FLASH_SIZE * 1024
...
#else
	r_1->flash_size = CONFIG_FLASH_SIZE * 1024 - EC_FLASH_REGION_START;
	r_1->flags = 0;
