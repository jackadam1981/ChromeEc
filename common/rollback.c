...
#ifdef CONFIG_ROLLBACK_ENTROPY_SIZE
	uint8_t entropy[CONFIG_ROLLBACK_ENTROPY_SIZE];
#endif
...
#ifdef CONFIG_ROLLBACK_ENTROPY_SIZE
static void update_entropy(uint8_t *dst, const uint8_t *src,
			   uint8_t *add, unsigned int add_len)
{
#ifdef CONFIG_SHA256
BUILD_ASSERT(SHA256_DIGEST_SIZE == CONFIG_ROLLBACK_ENTROPY_SIZE);
	struct sha256_ctx ctx;
	uint8_t *hash;

	SHA256_init(&ctx);
	SHA256_update(&ctx, src, CONFIG_ROLLBACK_ENTROPY_SIZE);
	SHA256_update(&ctx, add, add_len);
	/* TODO(b:38486828): Add other sources of entropy (e.g. device id) */
	hash = SHA256_final(&ctx);

	memcpy(dst, hash, CONFIG_ROLLBACK_ENTROPY_SIZE);
#else
#error "Entropy in rollback region requires SHA256 for hashing."
#endif
}
#endif /* CONFIG_ROLLBACK_ENTROPY_SIZE */
...
#ifdef CONFIG_ROLLBACK_ENTROPY_SIZE
	if (entropy)
		update_entropy(data.entropy, data.entropy, entropy, length);
#endif
...
static int command_rollback_info(int argc, char **argv)
{
	...
	for (region = 0; region < ROLLBACK_REGIONS; region++) {
		...
		/* If system is unlocked, we can show some of the entropy */
		ccprintf(" [%02x..%02x]", data.entropy[0],
			data.entropy[CONFIG_ROLLBACK_ENTROPY_SIZE-1]);
#endif
#endif
