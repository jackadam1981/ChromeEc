 ...
 */
static int get_latest_rollback(uint32_t *rollback_min_version)
{
	int region;
	int min_region = 0;
...
int lock_rollback(void)
{
	...
	if (!(flash_get_protect() & EC_FLASH_PROTECT_ROLLBACK_NOW) &&
		 ...
		 */
		cflush();
		system_reset(SYSTEM_RESET_PRESERVE_FLAGS);
	}
...
static int command_rollback_update(int argc, char **argv)
{
	...
	return update_rollback(min_version);
}
DECLARE_SAFE_CONSOLE_COMMAND(rollbackupdate, command_rollback_update,
			     "min_version",
			     "Update rollback");
