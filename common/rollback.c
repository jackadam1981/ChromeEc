...
BUILD_ASSERT(SHA256_DIGEST_SIZE == CONFIG_ROLLBACK_SECRET_SIZE);
	...
	/* Add some self-produced entropy */
	for (i = 0; i < CONFIG_ROLLBACK_SECRET_SIZE/sizeof(extra); i++) {
		if (!board_get_entropy(extra, sizeof(extra)))
			return 0;
		SHA256_update(&ctx, extra, sizeof(extra));
	}
