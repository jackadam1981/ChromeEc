...
static int rollback_first_init(const struct device *unused)
{
	uint8_t block[CONFIG_FLASH_WRITE_SIZE *
		      DIV_ROUND_UP(sizeof(struct rollback_data),
				   CONFIG_FLASH_WRITE_SIZE)];
	...
	/*
	 * If region is smaller than ROLLBACK_REGIONS then we know that valid
	 * rollback region was found, so we can finish now
	 */
