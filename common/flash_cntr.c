 ...
 *
 * This algorithm makes one big assumption. IF a write (not erase) is
 * interrupted, the result is that some of the bits that were meant to be
 * written to 0 may remain 1, but NONE of the bits that were written to 1 should
 * somehow become 0.
 */
...
test_mockable uint32_t FLASH_CNT_HI[PAGE_WORDS]
__attribute__((section(".flash_cntr.hi")));
test_mockable uint32_t FLASH_CNT_LO[PAGE_WORDS]
__attribute__((section(".flash_cntr.lo")));


test_mockable void _write(uint32_t *p, size_t i, uint32_t v)
{
	int offset = (intptr_t)p + i*(sizeof(v)) - CONFIG_PROGRAM_MEMORY_BASE;

	/* p[i] = v */
	ccprintf("\nWriting Offset 0x%08x with value 0x%08x\n", offset, v);
	flash_write(offset, sizeof(v), (void *)&v);
}

test_mockable void _erase(void *p)
{
...
uint32_t flash_cntr_incr(void)
{
	...
	if (lo == ARRAY_SIZE(FLASH_CNT_LO)) {
		...
		/* All LOW page is striken, time to advance HIGH page, the
		 * following write also re-affirms the 0 value of the last 4
		 * bits that were striken
		 */
