...
#ifdef CONFIG_I2C_REG_ADDR_16_BIT
int i2c_read_addr16_data16(int port, int slave_addr, int offset, int *data)
{
	int rv;
	...
	/* I2C read 16-bit word: transmit 16-bit offset, and read 16-bits */
	rv = i2c_xfer(port, slave_addr, addr, 2, buf, sizeof(uint16_t));
...
int i2c_read_addr16_data8(int port, int slave_addr, int offset, int *data)
{
	int rv;
	uint8_t buf[sizeof(uint8_t)],addr[sizeof(uint16_t)];

    addr[0]=(offset >> 8) & 0xff;
    addr[1]= offset & 0xff;

	/* I2C read 16-bit word: transmit 8-bit offset, and read 8-bits */
	rv = i2c_xfer(port, slave_addr, addr, 2, buf, sizeof(uint8_t));
...
int i2c_write_addr16_data16(int port, int slave_addr, int offset, int data)
{
	...
	return i2c_xfer(port, slave_addr, buf, 2 + sizeof(uint16_t), NULL, 0);
}

int i2c_write_addr16_data8(int port, int slave_addr, int offset, int data)
{
	...
	return i2c_xfer(port, slave_addr, buf, 2 + sizeof(uint8_t), NULL, 0);
}

int i2c_read_addr16_block(int port, int slave_addr, int offset, uint8_t *data,
		    int len)
{
	int rv;
...
int i2c_write_addr16_block(int port, int slave_addr, int offset, const uint8_t *data,
		    int len)
{
	int rv;
	uint8_t addr[sizeof(uint16_t)];
...
static int command_i2cxfer(int argc, char **argv)
{
	...
	if (strcasecmp(argv[1], "r") == 0) {
		...
		/* 16-bit addr read */
		rv = i2c_read_addr16_data8(port, slave_addr, offset, &v);
		if (!rv)
			ccprintf("0x%02x [%d]\n", v, v);
		...
		/* 16-bit addr read */
		rv = i2c_read_addr16_data16(port, slave_addr, offset, &v);
		if (!rv)
			ccprintf("0x%04x [%d]\n", v, v);
		...
		rv = i2c_write_addr16_data8(port, slave_addr, offset, v);
#endif
		...
		rv = i2c_write_addr16_data16(port, slave_addr, offset, v);
#endif
	} else {
