...
static int command_i2cxfer(int argc, char **argv)
{
	...
	if (strcasecmp(argv[1], "r") == 0) {
		/* 8-bit read */
                if(strlen(argv[4]) == 6)
                        rv = i2c_read_offset16(port, slave_addr, offset16, &v, 1);
                else
		...
		/* 16-bit read */
                if(strlen(argv[4]) == 6)
                        rv = i2c_read_offset16(port, slave_addr, offset16, &v, 2);
                else
		...
		if (argc < 6)
			return EC_ERROR_PARAM5;
                if(strlen(argv[4]) == 6)
		        rv = i2c_write8(port, slave_addr, offset16, v);
                else
