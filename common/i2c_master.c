...
static int i2c_command_passthru(struct host_cmd_handler_args *args)
{
	...
	if (port_protected[params->port] && i2c_port->passthru_allowed) {
		for (i = 0; i < params->num_msgs; i++) {
			if (!i2c_port->passthru_allowed(i2c_port,
				  I2C_ADDR__7b(params->msg[i].addr_flags__7bf)))
				return EC_RES_ACCESS_DENIED;
		}
	}
	...
	for (resp->num_msgs = 0, msg = params->msg;
	     ...
	     resp->num_msgs++, msg++) {
		const uint16_t addr__7bf = msg->addr_flags__7bf;
		...
		if (rv) {
			...
			rv = i2c_xfer_unlocked__7bf(params->port, addr__7bf,
					       out, write_len,
...
static void i2c_passthru_protect_tcpc_ports(void)
{
	...
	for (i = 0; i < CONFIG_USB_PD_PORT_COUNT; i++) {
		...
		if (!I2C_ADDR__7b(tcpc_config[i].i2c_info.addr__7bf))
			continue;
...
static void scan_bus(int port, const char *desc)
{
	...
	for (i2c_addr__7bf = 0; i2c_addr__7bf <= 0xEF; ++i2c_addr__7bf) {
		watchdog_reload();  /* Otherwise a full scan trips watchdog */
		ccputs(".");

		/* Do a single read */
		if (!i2c_xfer_unlocked__7bf(port, i2c_addr__7bf,
				       NULL, 0, &tmp, 1, I2C_XFER_SINGLE))
			ccprintf("\n  0x%02x", i2c_addr__7bf);
	}
...
static int command_i2cxfer(int argc, char **argv)
{
	...
	i2c_addr__7bf = strtoi(argv[3], &e, 0);
	if (*e)
	...
	if (strcasecmp(argv[1], "r") == 0) {
		/* 8-bit read */
		if (offset_size == 2)
			rv = i2c_read_offset16__7bf(port, i2c_addr__7bf,
					       offset, &v, 1);
		else
			rv = i2c_read8__7bf(port, i2c_addr__7bf,
				       offset, &v);
		if (!rv)
			ccprintf("0x%02x [%d]\n", v, v);
		...
		if (offset_size == 2)
			...
			rv = i2c_read16__7bf(port, i2c_addr__7bf,
					offset, &v);
		if (!rv)
			ccprintf("0x%04x [%d]\n", v, v);
		...
		rv = i2c_xfer__7bf(port, i2c_addr__7bf,
		...
		if (argc < 6)
			return EC_ERROR_PARAM5;
		if (offset_size == 2)
			rv = i2c_write_offset16__7bf(port, i2c_addr__7bf,
						offset, v, 1);
		else
			rv = i2c_write8__7bf(port, i2c_addr__7bf,
					offset, v);
		...
		if (offset_size == 2)
			...
			rv = i2c_write16__7bf(port, i2c_addr__7bf,
					 offset, v);
...
static int command_i2ctest(int argc, char **argv)
{
	...
	for (i = 0; i < count; i++) {
		...
		slave_addr__7bf = i2c_stress_tests[test_dev].slave_addr__7bf;
		...
		if (rand & 0x1) {
			...
			rv = i2c_s_test->i2c_read__7bf ?
				i2c_s_test->i2c_read__7bf(port, slave_addr__7bf,
