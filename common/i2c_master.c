				...
				uint8_t reg, uint8_t *in, int in_size)
{
	...
	if (IS_ENABLED(CONFIG_SMBUS_PEC) && I2C_USE_PEC(slave_addr_flags)) {
		...
		for (i = 0; i <= CONFIG_I2C_NACK_RETRY_COUNT; i++) {
			...
			pec_local = crc_8(out, ARRAY_SIZE(out));
			pec_local = crc_8_arg(in, in_size, pec_local);
				 ...
				 const uint8_t *out, int out_size)
{
	...
	if (IS_ENABLED(CONFIG_SMBUS_PEC) && I2C_USE_PEC(slave_addr_flags)) {
		...
		pec = crc_8(&addr_8bit, 1);
		pec = crc_8_arg(out, out_size, pec);
		    ...
		    int offset, uint8_t *data, int len)
{
	...
	for (i = 0; i <= CONFIG_I2C_NACK_RETRY_COUNT; i++) {
		...
		if (IS_ENABLED(CONFIG_SMBUS_PEC) &&
			...
			pec = crc_8(out, sizeof(out));
			pec = crc_8_arg(&block_length, 1, pec);
			pec = crc_8_arg(data, data_length, pec);
		    ...
		    int offset, const uint8_t *data, int len)
{
	...
	if (IS_ENABLED(CONFIG_SMBUS_PEC) && I2C_USE_PEC(slave_addr_flags)) {
		...
		pec = crc_8(&addr_8bit, sizeof(uint8_t));
		pec = crc_8_arg(data, len, pec);
	}
