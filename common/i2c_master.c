...
static int i2c_command_passthru(struct host_cmd_handler_args *args)
{
	...
	if (port_protected[params->port] && i2c_port->passthru_allowed__7b) {
		for (i = 0; i < params->num_msgs; i++) {
			if (!i2c_port->passthru_allowed__7b(i2c_port,
	...
	for (resp->num_msgs = 0, msg = params->msg;
		...
		if (params->port == I2C_PORT_VIRTUAL_BATTERY &&
		    VIRTUAL_BATTERY_ADDR__7bf == msg->addr_flags__7bf) {
		...
		PTHRUPRINTS("xfer port=%x addr=0x%x rlen=%d flags=0x%x",
			    params->port, msg->addr_flags__7bf,
			    read_len, xferflags);
		if (write_len) {
			PTHRUPRINTF("  out:");
