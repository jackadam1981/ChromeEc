...
static int i2c_read_pec(const int port, const uint16_t slave_addr_flags,
			uint8_t reg, uint8_t *in, int in_size)
{
#ifdef CONFIG_SMBUS_PEC
	int i, rv;
	uint8_t out[3] = {
		I2C_GET_ADDR(slave_addr_flags) << 1,
		reg,
		(I2C_GET_ADDR(slave_addr_flags) << 1) | 1,
	};
	uint8_t pec_local = 0, pec_remote;

	if (!(slave_addr_flags & I2C_FLAG_PEC))
...
static int i2c_write_pec(const int port, const uint16_t slave_addr_flags,
			 uint8_t *out, int out_size)
{
#ifdef CONFIG_SMBUS_PEC
	int i, rv;
	uint8_t addr_8bit = I2C_GET_ADDR(slave_addr_flags) << 1, pec;
	...
	i2c_lock(port, 1);
	for (i = 0; i <= CONFIG_I2C_NACK_RETRY_COUNT; i++) {
		rv = i2c_xfer_unlocked(port, slave_addr_flags, out, out_size,
				       NULL, 0, I2C_XFER_START);
		if (rv)
			continue;

		rv = i2c_xfer_unlocked(port, slave_addr_flags, &pec, 1, NULL, 0,
				       I2C_XFER_START);
		if (!rv)
			break;
	}
...
#ifdef CONFIG_SMBUS_PEC
static int i2c_read_string_pec(const int port,
			const uint16_t slave_addr_flags,
...
static int i2c_write_block_pec(const int port,
			const uint16_t slave_addr_flags,
	       ...
	       int offset, int *data)
{
	...
	rv = i2c_read_pec(port, slave_addr_flags, reg, buf, sizeof(uint32_t));
		...
		int offset, int data)
{
	...
	return i2c_write_pec(port, slave_addr_flags,
	       ...
	       int offset, int *data)
{
	...
	rv = i2c_read_pec(port, slave_addr_flags, reg, buf, sizeof(uint16_t));
	      ...
	      int offset, int *data)
{
	...
	rv = i2c_read_pec(port, slave_addr_flags, reg, &buf, sizeof(uint8_t));
	if (!rv)
		*data = buf;
	       ...
	       int offset, int data)
{
	...
	return i2c_write_pec(port, slave_addr_flags, buf, 2);
}
...
#ifdef CONFIG_SMBUS_PEC
	if (slave_addr_flags & I2C_FLAG_PEC)
		return i2c_read_string_pec(port, slave_addr_flags, offset, data,
					   len);
#endif
...
#ifdef CONFIG_SMBUS_PEC
	if (slave_addr_flags & I2C_FLAG_PEC)
		return i2c_write_block_pec(port, slave_addr_flags, offset, data,
					   len);
#endif
