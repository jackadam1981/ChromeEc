...
int i2c_read_addr16(int port, int slave_addr, int offset, int *data, int len)
{
	int rv;
	uint8_t buf[sizeof(uint16_t)],addr[sizeof(uint16_t)];
	uint8_t length = (len == 1) ? 1 : 2;
...
int i2c_write_addr16(int port, int slave_addr, int offset, int data, int len)
{
	int rv;
	...
	return i2c_xfer(port, slave_addr, buf, 2 + length, NULL, 0);
}
...
static int command_i2cxfer(int argc, char **argv)
{
	...
	if (strcasecmp(argv[1], "r") == 0) {
		...
		/* 16-bit addr read */
		rv = i2c_read_addr16(port, slave_addr, offset, &v, 1);
		if (!rv)
			ccprintf("0x%02x [%d]\n", v, v);
		...
		/* 16-bit addr read */
		rv = i2c_read_addr16(port, slave_addr, offset, &v, 2);
		if (!rv)
			ccprintf("0x%04x [%d]\n", v, v);
		...
		rv = i2c_write_addr16(port, slave_addr, offset, v, 1);
#endif
		...
		rv = i2c_write_addr16(port, slave_addr, offset, v, 2);
#endif
	} else {
