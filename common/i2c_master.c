...
static int chip_i2c_xfer_with_notify(struct i2c_xfer_params *p)
{
	int ret;

#ifdef CONFIG_I2C_XFER_CALLBACK
	i2c_start_xfer_notify(p);
#endif

	ret = chip_i2c_xfer(p);

#ifdef CONFIG_I2C_XFER_CALLBACK
	i2c_end_xfer_notify(p);
#endif
 ...
 */
static int i2c_xfer_no_retry(struct i2c_xfer_params *p)
{
	int ret;
	int flags = p->flags;
	...
	out_flags |= in_size ? 0 : (flags & I2C_XFER_STOP);
	p->flags = out_flags;
	p->in_size = in_chunk_size;

	ret = chip_i2c_xfer_with_notify(p);
	...
	while (in_size && ret == EC_SUCCESS) {
		...
		in_size -= in_chunk_size;
		p->in_size = in_chunk_size;
		p->flags = in_size ? 0 : (flags & I2C_XFER_STOP);
		ret = chip_i2c_xfer_with_notify(p);
		p->in += in_chunk_size;
	}
	return ret;
...
int i2c_xfer(const struct i2c_xfer_params *p)
{
	int i;
	int ret = EC_SUCCESS;
	struct i2c_xfer_params pcopy = *p;

	for (i = 0; i <= CONFIG_I2C_NACK_RETRY_COUNT; i++) {
#ifdef CONFIG_I2C_XFER_LARGE_READ
		ret = i2c_xfer_no_retry(&pcopy);
#else
		ret = chip_i2c_xfer_with_notify(&pcopy);
#endif /* CONFIG_I2C_XFER_LARGE_READ */
...
int i2c_read32(int port, int slave_addr, int offset, int *data)
{
	...
	struct i2c_xfer_params p = I2C_XFER_PARAMS(port, slave_addr, &reg,
						   sizeof(reg), buf,
						   sizeof(buf),
						   I2C_XFER_SINGLE);
...
int i2c_write32(int port, int slave_addr, int offset, int data)
{
	...
	struct i2c_xfer_params p = I2C_XFER_PARAMS_WRITE(port, slave_addr,
							 buf, sizeof(buf));
		    ...
		    int len)
{
	 ...
	 */
	p.out = &reg;
	p.out_size = sizeof(reg);
	p.in = &block_length;
	p.in_size = sizeof(block_length);
...
static int i2c_command_passthru(struct host_cmd_handler_args *args)
{
	...
	for (resp->num_msgs = 0, msg = params->msg;
		...
		if (rv) {
			...
			p.slave_addr = addr;
			p.out = out;
			p.out_size = write_len;
			p.in = &resp->data[in_len];
			p.in_size = read_len;
			p.flags = xferflags;
			rv = i2c_xfer(&p);
		}
