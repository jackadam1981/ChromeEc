...
static int chip_i2c_xfer_with_notify(const struct slave_addr_t slave_addr,
				     ...
				     uint8_t *in, int in_size, int flags)
{
	...
	i2c_start_xfer_notify(slave_addr);
#endif

	ret = chip_i2c_xfer(slave_addr, out, out_size, in, in_size,
			    flags);

#ifdef CONFIG_I2C_XFER_BOARD_CALLBACK
	i2c_end_xfer_notify(slave_addr);
#endif
 ...
 */
static int i2c_xfer_no_retry(const struct slave_addr_t slave_addr,
			     const uint8_t *out, int out_size,
			     uint8_t *in, int in_size, int flags)
{
	int ret;
	...
	ret = chip_i2c_xfer_with_notify(slave_addr, out, out_size, in,
					in_chunk_size, out_flags);
	in += in_chunk_size;
	while (in_size && ret == EC_SUCCESS) {
		in_chunk_size = MIN(in_size, CONFIG_I2C_CHIP_MAX_READ_SIZE);
		in_size -= in_chunk_size;
		ret = chip_i2c_xfer_with_notify(slave_addr, NULL, 0, in,
...
int i2c_xfer_unlocked(const struct slave_addr_t slave_addr,
		      const uint8_t *out, int out_size,
		      uint8_t *in, int in_size, int flags)
{
	int i;
	...
	if (!i2c_port_is_locked(slave_addr.port)) {
...
#ifdef CONFIG_I2C_XFER_LARGE_READ
		ret = i2c_xfer_no_retry(slave_addr, out, out_size, in,
			in_size, flags);
#else
		ret = chip_i2c_xfer_with_notify(slave_addr, out, out_size,
						in, in_size, flags);
#endif /* CONFIG_I2C_XFER_LARGE_READ */
...
int i2c_xfer(const struct slave_addr_t slave_addr,
	     const uint8_t *out, int out_size,
	     uint8_t *in, int in_size)
{
	int rv;

	i2c_lock(slave_addr.port, 1);
	rv = i2c_xfer_unlocked(slave_addr, out, out_size, in, in_size,
			I2C_XFER_SINGLE);
	i2c_lock(slave_addr.port, 0);
...
int i2c_read32(const struct slave_addr_t slave_addr, int offset, int *data)
{
	int rv;
	...
	rv = i2c_xfer(slave_addr, &reg, 1, buf, sizeof(uint32_t));

	if (rv)
		return rv;

	if (slave_addr.is_big_endian)
...
int i2c_write32(const struct slave_addr_t slave_addr, int offset, int data)
{
	...
	return i2c_xfer(slave_addr, buf, sizeof(uint32_t) + 1,
			NULL, 0);
}

int i2c_read16(const struct slave_addr_t slave_addr, int offset, int *data)
{
	int rv;
	...
	rv = i2c_xfer(slave_addr, &reg, 1, buf, sizeof(uint16_t));
	if (rv)
		return rv;
...
int i2c_write16(const struct slave_addr_t slave_addr, int offset, int data)
{
	...
	return i2c_xfer(slave_addr, buf, 1 + sizeof(uint16_t), NULL, 0);
}

int i2c_read8(const struct slave_addr_t slave_addr, int offset, int *data)
{
	int rv;
	uint8_t reg = offset;
	uint8_t buf;
	...
	rv = i2c_xfer(slave_addr, &reg, 1, &buf, 1);
	if (!rv)
		*data = buf;
...
int i2c_write8(const struct slave_addr_t slave_addr, int offset, int data)
{
	uint8_t buf[2];
	...
	return i2c_xfer(slave_addr, buf, 2, 0, 0);
}

int i2c_read_offset16(const struct slave_addr_t slave_addr,
		      uint16_t offset, int *data, int len)
{
	...
	rv = i2c_xfer(slave_addr, addr, 2, buf, len);
...
int i2c_write_offset16(const struct slave_addr_t slave_addr,
		       uint16_t offset, int data, int len)
{
	...
	return i2c_xfer(slave_addr, buf, 2 + len, NULL, 0);
}

int i2c_read_offset16_block(const struct slave_addr_t slave_addr,
			    uint16_t offset, uint8_t *data, int len)
{
	...
	return i2c_xfer(slave_addr, addr, 2, data, len);
}

int i2c_write_offset16_block(const struct slave_addr_t slave_addr,
			     uint16_t offset, const uint8_t *data, int len)
{
	 ...
	 */
	i2c_lock(slave_addr.port, 1);
	rv = i2c_xfer_unlocked(slave_addr, addr, 2, NULL, 0, I2C_XFER_START);
	if (!rv)
		rv = i2c_xfer_unlocked(slave_addr, data, len, NULL, 0,
				       I2C_XFER_STOP);
	i2c_lock(slave_addr.port, 0);
...
int i2c_read_string(const struct slave_addr_t slave_addr,
		    int offset, uint8_t *data, int len)
{
	 ...
	 */
	rv = i2c_xfer_unlocked(slave_addr, &reg, 1, &block_length, 1,
			       I2C_XFER_START);
	if (rv)
		goto exit;
	...
	rv = i2c_xfer_unlocked(slave_addr, 0, 0, data, block_length,
			       I2C_XFER_STOP);
	data[block_length] = 0;
...
int i2c_read_block(const struct slave_addr_t slave_addr,
		   int offset, uint8_t *data, int len)
{
	int rv;
	uint8_t reg_address = offset;

	rv = i2c_xfer(slave_addr, &reg_address, 1, data, len);
	return rv;
}

int i2c_write_block(const struct slave_addr_t slave_addr,
		    int offset, const uint8_t *data, int len)
{
	 ...
	 */
	i2c_lock(slave_addr.port, 1);
	rv = i2c_xfer_unlocked(slave_addr, &reg_address, 1, NULL, 0,
			       I2C_XFER_START);
	if (!rv) {
		rv = i2c_xfer_unlocked(slave_addr, data, len, NULL, 0,
				       I2C_XFER_STOP);
...
static int i2c_command_passthru(struct host_cmd_handler_args *args)
{
	...
	for (resp->num_msgs = 0, msg = params->msg;
		...
		unsigned int addr__7b = msg->addr_flags__7b & EC_I2C_ADDR_MASK;
		...
		if (params->port == I2C_PORT_VIRTUAL_BATTERY &&
		    VIRTUAL_BATTERY_ADDR__7b == addr__7b) {
		...
		PTHRUPRINTS("xfer port=%x addr=0x%x rlen=%d flags=0x%x",
			    params->port, addr__7b, read_len, xferflags);
		if (write_len) {
			PTHRUPRINTF("  out:");
		...
		if (rv) {
			...
			slave_addr.port = params->port;
			slave_addr.i2c_addr__7b = addr__7b;

			rv = i2c_xfer_unlocked(slave_addr,
					       out, write_len,
...
static void i2c_passthru_protect_tcpc_ports(void)
{
	...
	for (i = 0; i < CONFIG_USB_PD_PORT_COUNT; i++) {
		...
		if (!tcpc_config[i].slave_addr.i2c_addr__7b)
			continue;
		i2c_passthru_protect_port(tcpc_config[i].slave_addr.port);
	}
#endif
}
...
static void scan_bus(int port, const char *desc)
{
	...
	for (addr__7b = 0; addr__7b <= 0xEF; ++addr__7b) {
		...
		/* Do a single read */
		if (!i2c_xfer_unlocked(slave_addr, NULL, 0, &tmp, 1,
				       I2C_XFER_SINGLE))
			ccprintf("\n  0x%02x", addr__7b);
	}
...
static int command_i2cxfer(int argc, char **argv)
{
	...
	slave_addr.port = strtoi(argv[2], &e, 0);
	if (*e)
		return EC_ERROR_PARAM2;

	slave_addr.i2c_addr__7b = strtoi(argv[3], &e, 0);
	if (*e)
	...
	if (strcasecmp(argv[1], "r") == 0) {
		/* 8-bit read */
		if (offset_size == 2)
			rv = i2c_read_offset16(slave_addr, offset, &v, 1);
		else
			rv = i2c_read8(slave_addr, offset, &v);
		if (!rv)
			ccprintf("0x%02x [%d]\n", v, v);
		...
		/* 16-bit read */
		if (offset_size == 2)
			rv = i2c_read_offset16(slave_addr, offset, &v, 2);
		else
			rv = i2c_read16(slave_addr, offset, &v);
		if (!rv)
			ccprintf("0x%04x [%d]\n", v, v);
		...
		rv = i2c_xfer(slave_addr, (uint8_t *)&offset, 1, data, v);
		...
		if (argc < 6)
			return EC_ERROR_PARAM5;
		if (offset_size == 2)
			rv = i2c_write_offset16(slave_addr, offset, v, 1);
		else
			rv = i2c_write8(slave_addr, offset, v);
		...
		if (argc < 6)
			return EC_ERROR_PARAM5;
		if (offset_size == 2)
			rv = i2c_write_offset16(slave_addr, offset, v, 2);
		else
			rv = i2c_write16(slave_addr, offset, v);
...
static int command_i2ctest(int argc, char **argv)
{
	...
	for (i = 0; i < count; i++) {
		...
		slave_addr.port = i2c_stress_tests[test_dev].port;
		slave_addr.i2c_addr__7b = i2c_stress_tests[test_dev].addr__7b;
		...
		if (rand & 0x1) {
			...
			rv = i2c_s_test->i2c_read ?
				i2c_s_test->i2c_read(slave_addr,
					reg_s_info->read_reg, &data) :
				i2c_s_test->i2c_read_dev(
					reg_s_info->read_reg, &data);
			if (rv) {
			...
			do {
				...
				rv = i2c_s_test->i2c_write ?
					i2c_s_test->i2c_write(slave_addr,
