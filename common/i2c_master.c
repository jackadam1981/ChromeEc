		...
		const enum mask_update_action action)
{
	...
	if (val != oldval) {
		rv = i2c_write8(port, slave_addr_flags, offset, val);
		if (rv)
			return rv;
	}
	return EC_SUCCESS;
		 ...
		 const enum mask_update_action action)
{
	...
	if (val != oldval) {
		rv = i2c_write16(port, slave_addr_flags, offset, val);
		if (rv)
			return rv;
	}
	return EC_SUCCESS;
...
/**
 * Read/Modify/Write allowing a field to be cleared before setting
 * to a new value.
 */
