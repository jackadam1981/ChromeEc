...
/*
 * Define CONFIG_CMD_I2CWEDGE, CONFIG_I2C_WEDGE_SCL and CONFIG_I2C_WEDGE_SDA
 * to enable the 'i2cwedge' console command to allow us to bang the bus into
 * a wedged state. For example, include the following lines in board/pit/board.h
 * to enable it on pit:
 *
 * #define CONFIG_CMD_I2CWEDGE
 * #define CONFIG_I2C_WEDGE_SCL GPIO_I2C1_SCL
 * #define CONFIG_I2C_WEDGE_SDA GPIO_I2C1_SDA
 */
...
static int i2c_bang_read_scl(void)
{
	...
	/*
	 * For the lm4 chip, to read the i2c lines we need to toggle to an
	 * input. This should have no affect on the state of the line since
	 * the lines are open drain outputs with pull-ups and we are outputting
	 * high.
	 */
	gpio_set_flags(i2c_bang_scl, GPIO_INPUT);
	ret = gpio_get_level(i2c_bang_scl);
...
static int i2c_bang_read_sda(void)
{
	 ...
	 * high.
	 */
	gpio_set_flags(i2c_bang_sda, GPIO_INPUT);
	ret = gpio_get_level(i2c_bang_sda);
...
{
	i2c_bang_scl = CONFIG_I2C_WEDGE_SCL;
	i2c_bang_sda = CONFIG_I2C_WEDGE_SDA;
	i2c_bang_started = 0;

	gpio_set_flags(i2c_bang_scl, GPIO_ODR_HIGH);
	gpio_set_flags(i2c_bang_sda, GPIO_ODR_HIGH);

	/* Take out of alternate function mode. Doesn't matter for STM chips. */
	gpio_set_alternate_function(gpio_list[CONFIG_I2C_WEDGE_SDA].port,
				gpio_list[CONFIG_I2C_WEDGE_SDA].mask, -1);
	gpio_set_alternate_function(gpio_list[CONFIG_I2C_WEDGE_SCL].port,
				gpio_list[CONFIG_I2C_WEDGE_SCL].mask, -1);
}
...
{
	i2c_unwedge(CONFIG_I2C_WEDGE_SDA, CONFIG_I2C_WEDGE_SCL, 1);
