...
static enum cr50_comm_err send_to_cr50(const uint8_t *data, size_t size)
{
	...
	/* Make sure console task won't steal the response (in case in the futre
	 * EFS runs after tasks start). */
...
__overridable void request_power(void)
{
	CPRINTS("%s", __func__);
}
...
void vboot_main(void)
{
	...
	if (!(flash_get_protect() & EC_FLASH_PROTECT_GPIO_ASSERTED)) {
		/*
		 * If hardware WP is disabled, TCPM enables PD communication.
		 * EFS can return and wait for more power from PD.
