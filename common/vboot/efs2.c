...
void hook_shutdown(void)
{
	...
	/*
	 * We have to filter the cases which can be inhibited if we execute
	 * system_reset in HOOK_CHIPSET_SHUTDOWN. Most cases are filtered out by
	 * system_is_in_rw (e.g. system_common_shutdown, check_pending_cutoff).
	 */
	if (system_is_in_rw())
		return;
	...
	CPRINTS("Reboot\n\n");
	cflush();
	system_reset(SYSTEM_RESET_LEAVE_AP_OFF/* |SYSTEM_RESET_HARD */);
}
/*
