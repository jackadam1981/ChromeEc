...
static void read_vboot_info(struct ec_response_vboot_info *r)
{
	...
	BUILD_ASSERT_INLINE(sizeof(r->key_id) == sizeof(vb21_key->id),
			    "key ID sizes must match");
	BUILD_ASSERT_INLINE(sizeof(vb21_key->id) == sizeof(vb21_key->id.raw),
			    "key ID sizes must match");
	memcpy(r->key_id, vb21_key->id.raw, sizeof(vb21_key->id));

	rv = vb21_is_packed_key_valid(vb21_key);
	r->key_is_valid = (rv == EC_SUCCESS);
}

static int command_vboot_info(int argc, char **argv)
{
	int i;
	struct ec_response_vboot_info r;
...
DECLARE_CONSOLE_COMMAND(vboot_info, command_vboot_info, NULL,
			"Display vboot info on console.");

static enum ec_status
host_command_vboot_info(struct host_cmd_handler_args *args)
{
	struct ec_response_vboot_info *r = args->response;
...
DECLARE_HOST_COMMAND(EC_CMD_VBOOT_INFO, host_command_vboot_info,
	EC_VER_MASK(EC_VER_VBOOT_INFO));
