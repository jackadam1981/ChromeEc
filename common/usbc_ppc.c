...
/*
 * A per-port table that indicates how many times a request was made to clear
 * the overcurrent event counter table.  A high number of requests indicates
 * that a port is appearing to be disconnected in rapid succession which may be
 * be indicative of an overcurrent loop.  In order to break this loop, once the
 * number of requests exceed the PPC_OC_CLR_CNT_THRESH, the overcurrent event
 * table will not be cleared for at least 2 seconds.  If the overcurrent loop
 * continues, this will cause the port to become latched off.
 */
...
int ppc_clear_oc_event_counter(int port)
{
	 ...
	 */
	if (clear_oc_event_att[port] > PPC_OC_CLR_CNT_THRESH) {
