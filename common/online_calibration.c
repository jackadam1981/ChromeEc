...
int online_calibration_read(int capacity_bytes,
			    struct ec_response_online_calibration_data *out)
{
	int i, count, sensor_num;

	mutex_lock(&g_calib_cache_mutex);
	count = MIN(capacity_bytes /
		    sizeof(struct ec_response_online_calibration_data),
		    __builtin_popcount(sensor_calib_cache_dirty_map));
	for (i = 0; i < count; ++i) {
		...
		memcpy(out->data,
		       motion_sensors[sensor_num].online_calib_data->cache, 6);
		/* Clear dirty bit */
