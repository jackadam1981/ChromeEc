...
/*
 * Expected PKCS1.5 signature padding bytes, for a keytool SHA256WithRSA
 * signature.
...
/*
 * SHA256 of PKCS1.5 signature padding for 2048 bit, as above.
 * At the location of the bytes of the hash all 00 are hashed.
 */
static const uint8_t expected_pad_sha256_rsa2048[SHA256_DIGEST_SIZE] = {
	       ...
	       const uint8_t *sha, uint32_t *workbuf32)
{
	...
	struct sha256_ctx *ctx = (void *)workbuf32;
	int i;
	...
	for (i = RSANUMBYTES - SHA256_DIGEST_SIZE; i < RSANUMBYTES; ++i)
		buf[i] ^= *sha++;

	/* Hash resulting buf. */
	SHA256_init(ctx);
	SHA256_update(ctx, buf, RSANUMBYTES);
	SHA256_final(ctx);

	/* Compare against expected hash value. */
	if (memcmp(ctx->buf, expected_pad_sha256_rsa2048, SHA256_DIGEST_SIZE))
		return 0;
