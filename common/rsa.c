...
static void mont_mul_add(const struct rsa_public_key *key,
			 ...
			 const uint32_t *b)
{
	uint64_t A = mac32(a, b[0], c[0]);
	uint32_t d0 = (uint32_t)A * key->n0inv;
	uint64_t B = mac32(d0, key->n[0], A);
	uint32_t i;

	for (i = 1; i < RSANUMWORDS; ++i) {
		A = mac32_2(a, b[i], c[i], A >> 32);
		B = mac32_2(d0, key->n[i], A, B >> 32);
		c[i - 1] = (uint32_t)B;
	}
			 ...
			 const uint32_t *b)
{
	...
	uint64_t B = mac32(d0, key->n[0], c[0]);
	uint32_t i;

	for (i = 1; i < RSANUMWORDS; ++i) {
		B = mac32_2(d0, key->n[i], c[i], B >> 32);
		c[i - 1] = (uint32_t)B;
	}
