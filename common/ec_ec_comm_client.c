 ...
 * uart_alt_pad_write_read API to write the actual client response to a separate
 * buffer.
 */
			 ...
			 int timeout_us)
{
	...
	struct ec_host_response4 *response_header =
		(void *)&data[tx_length];
	/* RX length is TX length + response from client. */
	int rx_length = tx_length +
		sizeof(*request_header) + ((resp_len > 0) ? (resp_len + 1) : 0);

	/*
	 * Make sure there is a gap between each command, so that the client
