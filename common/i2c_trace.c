...
trace_enabled:
	CPRINTF("i2c %s: port=%d, address=0x%X, data=",
				   ...
				   int slave_addr_hi)
{
	...
	for (i = 0; i < ARRAY_SIZE(trace_entries); i++) {
		if (trace_entries[i].enabled && trace_entries[i].port == port) {
			...
			if (trace_entries[i].slave_addr_lo <= slave_addr_lo &&
			    trace_entries[i].slave_addr_hi >= slave_addr_hi)
				return EC_SUCCESS;

			/* Extends exising range on both directions, relpace */
			if (trace_entries[i].slave_addr_lo >= slave_addr_lo &&
			    trace_entries[i].slave_addr_hi <= slave_addr_hi) {
				trace_entries[i].enabled = 0;
				return command_i2ctrace_enable(
					port, slave_addr_lo, slave_addr_hi);
			}

			/* Extends existing range below */
			if (trace_entries[i].slave_addr_lo <= slave_addr_hi &&
			    trace_entries[i].slave_addr_hi >= slave_addr_hi) {
				trace_entries[i].enabled = 0;
				return command_i2ctrace_enable(
					port,
			...
			if (trace_entries[i].slave_addr_lo <= slave_addr_lo &&
			    trace_entries[i].slave_addr_hi >= slave_addr_lo) {
				trace_entries[i].enabled = 0;
				return command_i2ctrace_enable(
					port,
					trace_entries[i].slave_addr_lo,
	...
	for (i = 0; i < ARRAY_SIZE(trace_entries); i++) {
		if (!trace_entries[i].enabled) {
			trace_entries[i].enabled = 1;
			trace_entries[i].port = port;
			trace_entries[i].slave_addr_lo = slave_addr_lo;
			trace_entries[i].slave_addr_hi = slave_addr_hi;
			return EC_SUCCESS;
		}
	}
