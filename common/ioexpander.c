...
int ioex_enable_interrupt(enum ioex_signal signal)
{
	...
	return drv->enable_interrupt(g->ioex, g->port, g->mask);
}
...
int ioex_disable_interrupt(enum ioex_signal signal)
{
	 ...
	 * the interrupt function
	 */
	if (drv->disable_interrupt == NULL) {
		CPRINTF("IOEX chip port %d doesn't support INT\n", g->ioex);
		return EC_ERROR_UNIMPLEMENTED;
	}
	return drv->disable_interrupt(g->ioex, g->port, g->mask);
}
