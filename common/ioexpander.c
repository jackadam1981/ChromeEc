...
static uint8_t altgrp[CONFIG_IO_EXPANDER_PORT_COUNT] = {
	[0 ... CONFIG_IO_EXPANDER_PORT_COUNT - 1] = 0x00 };
static uint8_t ioex_altgrp_inited = 0;
...
__overridable int board_get_ioex_altgrp(int port, uint8_t *alt_grp)
{
	*alt_grp = 0;
...
static bool ioex_is_valid_signal(const struct ioex_info *g)
{
	int i;

	if (!ioex_altgrp_inited) {
		for (i = 0; i < CONFIG_IO_EXPANDER_PORT_COUNT; i++)
			board_get_ioex_altgrp(i, &altgrp[i]);
		ioex_altgrp_inited = 1;
	}

        for (i = 0; i < CONFIG_IO_EXPANDER_PORT_COUNT; i++) {
		if (altgrp[i] == g->altgrp && i == g->ioex)
			return true;
	}
...
static bool ioex_io_pin_being_used(int ioex, int alt_grp)
{
	int i;

	if (!ioex_altgrp_inited) {
		for (int i = 0; i < CONFIG_IO_EXPANDER_PORT_COUNT; i++)
			board_get_ioex_altgrp(i, &altgrp[i]);
		ioex_altgrp_inited = 1;
	}

	for (i = 0; i < CONFIG_IO_EXPANDER_PORT_COUNT; i++) {
		if (alt_grp == altgrp[i] && i == ioex)
			return true;
	}
...
int ioex_get_flags_by_mask(int ioex, int port, int mask, int alt_grp, int *flags)
{
	if (!ioex_io_pin_being_used(ioex, alt_grp))
...
int ioex_set_flags_by_mask(int ioex, int port, int mask, int alt_grp, int flags)
{
	if (!ioex_io_pin_being_used(ioex, alt_grp))
		return EC_ERROR_INVAL;
		...
		if (g->mask && !(g->flags & GPIO_DEFAULT)) {
			if (ioex_is_valid_signal(g))
				ioex_set_flags_by_mask(g->ioex, g->port,
						g->mask, g->altgrp, g->flags);
		}
	}

}
DECLARE_HOOK(HOOK_INIT, ioex_init_default, HOOK_PRIO_INIT_I2C + 2);
