...
int ioex_get_flags(enum ioex_signal signal, int *flags)
{
	...
	return ioex_config[g->ioex_port].drv->get_flags_by_mask(g->ioex_port,
						g->port, g->mask, flags);
...
int ioex_set_flags(enum ioex_signal signal, int flags)
{
	...
	return ioex_config[g->ioex_port].drv->set_flags_by_mask(g->ioex_port,
						g->port, g->mask, flags);
...
int ioex_get_level(enum ioex_signal signal, int *val)
{
	...
	rv = ioex_config[g->ioex_port].drv->get_level(g->ioex_port, g->port,
							g->mask, val);
	return rv;
...
int ioex_set_level(enum ioex_signal signal, int value)
{
	...
	return ioex_config[g->ioex_port].drv->set_level(g->ioex_port, g->port,
							g->mask, value);
...
static void ioex_init_default(void)
{
	...
	for (i = 0; i < IOEX_COUNT; i++, g++) {
		if (g->mask && !(g->flags & GPIO_DEFAULT)) {
			ioex_set_flags_by_mask(g->ioex_port, g->port,
						g->mask, g->flags);
		}
	}
...
int ioex_is_implemented(enum ioex_signal signal)
{
	return !!ioex_list[signal].mask;
}
...
static enum ioex_signal find_ioex_by_name(const char *name)
{
	...
	for (i = 0; i < IOEX_COUNT; i++)
		if (ioex_is_implemented(i) &&
		    !strcasecmp(name, ioex_get_name(i)))
			return i;
...
static enum ec_error_list ioex_set(const char *name, int value)
{
	...
	rv = ioex_set_level(signal, value);
