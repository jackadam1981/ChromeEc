...
int power_button_wait_for_release(int timeout_us)
{
	...
	if (t1.val < now.val) {
		 ...
		 * first. Note that the sleep duration is slightly longer than
		 * necessary (because now is sampled earlier). Thus, when we
		 * wake up, power_button_change_deferred should have been
		 * already called.
		 */
