...
/*
 * Time of last password attempt; initialized to 0 at boot.  Yes, we're only
 * keeping the bottom 32 bits of the timer here, so on a wraparound (every
 * ~4000 seconds) it's possible for an attacker to get one extra attempt.  But
 * it still behaves properly at boot, requiring the system to be up
 * PASSWORD_RATE_LIMIT usec before allowing the first attempt.
 */
...
static int raw_check_password(const char *password)
{
	...
	t = get_time().le.lo;
	if (t - last_password_time < (uint32_t)PASSWORD_RATE_LIMIT_US)
