 ...
 *
 * The payload of the command is a text string to use to set the password.
 * Empty string will trigger clearing the password.
 */
				       ...
				       size_t *response_size)
{
	...
	if (!input_size) {
		strncpy(password, "clear", sizeof(password));
	} else {
		if (input_size >= sizeof(password)) {
			rv = EC_ERROR_PARAM1;
		} else {
