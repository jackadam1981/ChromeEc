 ...
 * Current CCD state.
 *
 * Note: External callers should NOT use this directly to enable/disable CCD
 * functionality.  They should always use ccd_get_capability().
 */
enum ccd_state {
	CCD_STATE_LOCKED = 0,
	CCD_STATE_UNLOCKED,
...
/* Time of last password attempt; initialized to 0 at boot */
static uint64_t last_password_time;
 ...
 * @return EC_SUCCESS or non-zero error code on failure
 */
...
/******************************************************************************/
/* State machine for async unlock / open requests */
...
static int command_ccdset(int argc, char **argv)
{
	...
	enum ccd_capability_state old, new;
