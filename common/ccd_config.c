...
/*
 * Could be invoked syncronously on the TPM task context, or asynchronously,
...
static int command_ccd_body(int argc, char **argv)
{
	...
	if (!strcasecmp(argv[1], "unlock")) {
		if (!raw_has_password()) {
			ccprintf("Cann't unlock without password\n");
			return EC_ERROR_ACCESS_DENIED;
		}
				      ...
				      size_t *response_size)
{
	...
	if ((ccd_state == CCD_STATE_OPENED) ||
	    (ccd_state == CCD_STATE_UNLOCKED)) {
		buffer[0] = CCDPP_PP_DONE;
	} else {
		switch (physical_presense_fsm_state()) {
		case PP_AWAITING_PRESS:
			buffer[0] = CCDPP_AWAITING_PRESS;
			break;
		case PP_BETWEEN_PRESSES:
			buffer[0] = CCDPP_BETWEEN_PRESSES;
			break;
		default:
			buffer[0] = CCDPP_CLOSED;
			break;
		}
	}
	*response_size = 1;
