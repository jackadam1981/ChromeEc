...
static void ccd_tpm_reset_callback(void);
...
void ccd_config_init(enum ccd_state state)
{
	...
	register_tpm_reset_callback(ccd_tpm_reset_callback);
}
...
static int do_ccd_password(char *password)
{
	...
	if (raw_has_password()) {
		...
		/*
		 * If password was set from Opened, can't clear it if just
		 * Unlocked
		 */
		if ((ccd_state == CCD_STATE_UNLOCKED) &&
