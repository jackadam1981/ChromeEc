...
#define CCD_CAPS_DEFAULT_BYTE	(CCD_CAP_STATE_DEFAULT \
				 | CCD_CAP_STATE_DEFAULT << CCD_CAP_BITS \
				 | CCD_CAP_STATE_DEFAULT << (CCD_CAP_BITS * 2) \
				 | CCD_CAP_STATE_DEFAULT << (CCD_CAP_BITS * 3))
#define CCD_CAPS_ALL_SET_BYTE	(0xFF)
 ...
 *         0 otherwise.
 */
static int raw_check_reset_default(void)
{
	uint32_t i;
	uint32_t rest_bits;
	uint8_t bitmask = CCD_CAPS_ALL_SET_BYTE;

	if (ccd_state != CCD_STATE_LOCKED)
		return 0;

	rest_bits = CCD_CAP_COUNT;
	for (i = 0; i < CCD_CAP_COUNT;
	     i += CCD_CAPS_PER_BYTE, rest_bits -= CCD_CAPS_PER_BYTE) {
		if (rest_bits < CCD_CAPS_PER_BYTE)
			bitmask >>= (CCD_CAPS_PER_BYTE - rest_bits);

		if (((config.capabilities[i / 8] ^ CCD_CAPS_DEFAULT_BYTE)
		    & bitmask) != 0)
			return 0;
	}
...
static int command_ccd_info(void)
{
	...
	ccprintf("CCD reset default: %c\n", raw_check_reset_default() ?
		 'Y' : '-');
...
static enum vendor_cmd_rc ccd_get_info(struct vendor_cmd_params *p)
{
	...
	response.ccd_indicator_bitmap |= raw_check_reset_default()
				<< CCD_INDICATOR_SHIFT_RESET_DEFAULT;
