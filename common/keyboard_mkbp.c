...
static int fifo_remove(uint8_t *buffp)
{
	...
	memcpy(buffp, &fifo[fifo_start]+1, size); /* skip over event_type. */
...
test_mockable int mkbp_fifo_add(uint8_t event_type, const uint8_t *buffp)
{
	...
	memcpy(&fifo[fifo_end], &event_type, sizeof(uint8_t));
	memcpy(&fifo[fifo_end]+1, buffp,
...
void mkbp_update_switches(uint32_t sw, int is_pressed)
{
	static uint32_t switches;

	switches &= ~(1 << sw);
	switches |= (is_pressed << sw);

	mkbp_fifo_add(EC_MKBP_EVENT_SWITCH, (const uint8_t *)&switches);
}
...
{
	mkbp_update_switches(EC_MKBP_LID_SWITCH, lid_is_open());
}
...
void keyboard_update_button(enum keyboard_button_type button, int is_pressed)
{
	...
	case KEYBOARD_BUTTON_POWER:
		buttons &= ~(1 << EC_MKBP_POWER_BUTTON);
		buttons |= (is_pressed << EC_MKBP_POWER_BUTTON);
		break;

	case KEYBOARD_BUTTON_VOLUME_UP:
		buttons &= ~(1 << EC_MKBP_VOL_UP);
		buttons |= (is_pressed << EC_MKBP_VOL_UP);
		break;

	case KEYBOARD_BUTTON_VOLUME_DOWN:
		buttons &= ~(1 << EC_MKBP_VOL_DOWN);
		buttons |= (is_pressed << EC_MKBP_VOL_DOWN);
		break;
	...
	mkbp_fifo_add(EC_MKBP_EVENT_BUTTON, (const uint8_t *)&buttons);
}
