...
void mkbp_update_switches(uint32_t sw, int state)
{
	...
	/*
	 * Only inform AP mkbp changes when the init done, in case of the
	 * middle states causing the weird behaviour in the AP side,
...
static void mkbp_update_switch_on_init(void)
{
	/* All switches init, update switches to AP */
	mkbp_init_done = 1;
	mkbp_fifo_add(EC_MKBP_EVENT_SWITCH,
		      (const uint8_t *)&mkbp_switch_state);
}
DECLARE_HOOK(HOOK_INIT, mkbp_update_switch_on_init, HOOK_PRIO_LAST);
