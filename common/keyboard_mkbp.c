...
#ifdef CONFIG_ZEPHYR
static int init_fifo_mutex(const struct device *dev)
{
	ARG_UNUSED(dev);

	k_mutex_init(&fifo_add_mutex);
	k_mutex_init(&fifo_remove_mutex);

	return 0;
}
SYS_INIT(init_fifo_mutex, POST_KERNEL, 50);
#endif /* CONFIG_ZEPHYR */
