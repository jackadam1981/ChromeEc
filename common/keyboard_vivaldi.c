...
BUILD_ASSERT(ARRAY_SIZE(vivaldi) == MAX_TOP_ROW_KEYS);
...
__overridable
const struct ec_response_keybd_config *board_set_vivaldi_keybd_config(void)
{
	return NULL;
}
...
static void vivaldi_init(void)
{
	...
	vivaldi_keybd = board_set_vivaldi_keybd_config();
	...
	for (i = 0; i < ARRAY_SIZE(vivaldi); i++) {
		...
		if (col >= KEYBOARD_COLS_MAX || row >= KEYBOARD_ROWS) {
			cprints(CC_KEYBOARD,
				"VIVALDI: Bad (row,col) for T-%u: (%u,%u)",
				i, row, col);
			panic_printf("VIVALDI: Bad internal config\n");
		}
