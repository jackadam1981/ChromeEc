...
void vivaldi_init(const struct vivaldi_config *keybd)
{
	...
	if (keybd->num_top_row_keys > MAX_VIVALDI_KEYS ||
		...
		cprints(CC_KEYBOARD,
			"BAD VIVALDI CONDFIG! Some keys may not work\n");

	for (key = T1; key < keybd->num_top_row_keys; key++) {

		/* Only override scancodes with non zero value */
		if (!keybd->scancodes[key])
			continue;
