...
int usb_charge_set_mode(int port_id, enum usb_charge_mode mode)
{
	...
	inhibit_charge = mode & 0x80 ? 1 : 0;
	mode &= 0x7f;
	CPRINTS("USB charge p%d m%d e:%d", port_id, mode, inhibit_charge);
 ...
 * bit8 = Enable/Disable charging during system suspend,
 *        0: enable charging
 *        1: disable charging
 */
...
static void usb_charge_resume(void)
{
	...
	for (i = 0; i < CONFIG_USB_PORT_POWER_SMART_PORT_COUNT; i++)
		usb_charge_set_mode(i,
			...
			charge_mode[i].inhibit_charging_in_suspend << 7);
}
...
static void usb_charge_suspend(void)
{
	...
	/*
	 * Inhibit charging during S3 or S0ix if the inhibit_charging_in_suspend
	 * is set to 1.
	 */
	for (i = 0; i < CONFIG_USB_PORT_POWER_SMART_PORT_COUNT; i++)
		usb_charge_set_enabled(i,
