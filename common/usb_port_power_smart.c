...
#ifndef CONFIG_USB_CHARGE_PORT_COUNT
...
struct usb_state {
	uint8_t port_mode[CONFIG_USB_CHARGE_PORT_COUNT];
};

static uint8_t charge_mode[CONFIG_USB_CHARGE_PORT_COUNT];
...
{
#ifndef CONFIG_MULTIPLE_USB_PORT_POWER
	if (port_id == 0)
...
static void usb_charge_all_ports_ctrl(enum usb_charge_mode mode)
{
	...
	for (index = 0; index < CONFIG_USB_CHARGE_PORT_COUNT; index++)
...
int usb_charge_set_mode(int port_id, enum usb_charge_mode mode)
{
	...
	if (port_id >= CONFIG_USB_CHARGE_PORT_COUNT)
...
static int command_set_mode(int argc, char **argv)
{
	...
	if (*e || port_id < 0 || port_id >= CONFIG_USB_CHARGE_PORT_COUNT)
