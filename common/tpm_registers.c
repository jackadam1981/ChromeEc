...
/*
 * Mutex for users of the alternative TPM command execution context, one at a
 * time is allowed.
 */
static struct mutex alt_interface_mutex;

/*
 * This structure stores the context of the alternative TPM command execution
 * path. The command and response share the buffer, when TPM task finishes
 * processing the command it sets the 'tpm_task_done' field to a non-zero
 * value.
 *
 * To save SRAM space this structure is allocated on the stack only when
 * needed.
 */
...
void tpm_alt_extension(struct tpm_cmd_header *command,
		       size_t buffer_size)
{
	struct alt_tpm_interface alt_if;

	mutex_lock(&alt_interface_mutex);
	...
	/*
	 * This is not very elegant, but simple and acceptable for this TPM
	 * command execution path, as in most cases it would be drven by a
	 * human operator.
	 */
	while (!alt_if.tpm_task_done)
		msleep(10);

	mutex_unlock(&alt_interface_mutex);
}
...
void tpm_task(void)
{
	...
	while (1) {
		...
		if (evt & TASK_EVENT_WAKE) {
			...
			tpmh = alt_if_p->alt_hdr;
			buffer_size = alt_if_p->alt_buffer_size;
			alt_if_command = 1;
		} else {
			if (evt) {
				cprints(CC_TASK, "%s:%d unexpected event %x",
