...
#define CPRINTS(format, args...) cprints(CC_TPM_REG, format, ## args)
...
void tpm_register_get(uint32_t regaddr, uint8_t *dest, uint32_t data_size)
{
	...
	case TPM_BOARD_CFG:
		...
		break;
	default:
		CPRINTS("%s(0x%06x, %u) => ??", __func__, regaddr, data_size);
		return;
	}
}
...
void tpm_task(void *u)
{
	...
	while (1) {
		...
		CPRINTST("%s: received %scommand 0x%04x, size %u", __func__,
