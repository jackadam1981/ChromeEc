...
static void call_backdoor_command(struct tpm_cmd_header *tpmh,
				  size_t *total_size)
{
	size_t command_size = be32toh(tpmh->size);

	/* verify there is room for at least the backdoor command header */
	if (command_size >= sizeof(struct tpm_cmd_header)) {
		...
		*total_size -= sizeof(struct tpm_cmd_header);
		...
		backdoor_route_command(subcommand_code,
				       tpmh + 1,
				       command_size -
...
void tpm_task(void)
{
	...
	while (1) {
		...
		if (command_code == CONFIG_BACKDOOR_COMMAND) {
			...
			call_backdoor_command(tpmh, &response_size);
		} else
#endif
		{
		...
		if (response_size &&
			...
			{
				/*
				 * Backdoor commands reuse FIFO buffer, the
				 * rest need to copy.
				 */
