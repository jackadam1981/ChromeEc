 ...
 * time is allowed.
 */
static struct mutex interface_mutex;
		       ...
		       size_t buffer_size)
{
	 ...
	 * command execution path, as in most cases it would be dirven by a
	 * human operator.
	 */
...
void tpm_task(void)
{
	...
	while (1) {
		...
		/* Process unporcessed events or wait for the next event */
		if (!evt)
			evt = task_wait_event(-1);
