...
static void stillDetReset(struct StillDet *still_det)
{
...
static int stillnessBatchComplete(struct StillDet *still_det,
...
void stillDetInit(struct StillDet *still_det, float var_threshold,
		  uint32_t min_batch_window, uint32_t max_batch_window,
		  uint32_t min_batch_size)
{
	memset(still_det, 0, sizeof(struct StillDet));
...
int stillDetUpdate(struct StillDet *still_det, uint32_t sample_time,
		   float x, float y, float z)
{
	...
	if (stillnessBatchComplete(still_det, sample_time)) {
