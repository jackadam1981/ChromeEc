...
int setvar(const char *key, const char *val)
{
	...
	if (getvar_kv(key, &k, &v)) {
		...
		if (next_key(&v)) {
			 ...
			 * Delete our entry by shifting from there to the end
			 * of rbuf, so that it covers ours up.
			 *
			 * Before:
			 *           [k]      [v]
			...
			/* Now advance k to point to the end of variables */
			while (next_key(&k))
				;
		}
	...
	/*
	 * [k] shows where the new string should go.
	 *
	 * Either this:
	 *                       [k]
	 *   foo=bar\0hey=splat\0\0
	...
	if (!vlen)
		/* No value. We've already deleted the key, so we're done. */
		return EC_SUCCESS;

	klen = strlen(key);
	/* We need to write KEY=VAL\0\0. '=' + '\0' + '\0' is 3 more chars */
	if (k + klen + vlen + 3 > SIZE)
...
int writevars(void)
{
	...
	/*
	 * We hope that nvmem_write() is smart enough to do nothing if nothing
	 * needs writing, but that's just an optimization.
	*/
