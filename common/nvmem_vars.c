...
const uint8_t *KEY(const struct tuple *t)
{
	return t->data_;
}

const uint8_t *VAL(const struct tuple *t)
{
...
static int command_get(int argc, char **argv)
{
	...
	print_blob(tuple->data_ + tuple->key_len, tuple->val_len);
	ccprintf("\n");
	return EC_SUCCESS;
}
...
static int command_print(int argc, char **argv)
{
	...
	do {
		tuple = (const struct tuple *)(rbuf + i);
		print_blob(tuple->data_, tuple->key_len);
		ccprintf("=");
		print_blob(tuple->data_ + tuple->key_len, tuple->val_len);
		ccprintf("\n");
	} while (next_tuple(&i));
