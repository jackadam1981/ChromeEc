/*
 * Copyright 2019 The Chromium OS Authors. All rights reserved.
 * Use of this source code is governed by a BSD-style license that can be
 * found in the LICENSE file.
 */

#include "audio_codec.h"
#include "host_command.h"

static struct {
	struct audio_codec_driver driver;

	uint32_t capabilities;

	struct {
		uint8_t cap;
		uintptr_t addr;
		uint32_t len;
		uint8_t type;
	} shms[EC_CODEC_SHM_ID_LAST];
} priv;

static int get_capabilities(struct host_cmd_handler_args *args)
{
	struct ec_response_ec_codec_get_capabilities *r = args->response;

	r->capabilities = priv.capabilities;

	args->response_size = sizeof(*r);
	return EC_RES_SUCCESS;
}

static int get_shm_addr(struct host_cmd_handler_args *args)
{
	struct ec_param_ec_codec *p = (struct ec_param_ec_codec *)args->params;
	struct ec_param_ec_codec_get_shm_addr *pp = &p->get_shm_addr_param;
	struct ec_response_ec_codec_get_shm_addr *r = args->response;
	uint8_t shm_id = pp->shm_id;

	if (shm_id >= EC_CODEC_SHM_ID_LAST)
		return EC_RES_INVALID_PARAM;
	if (!priv.shms[shm_id].addr ||
	    !audio_codec_capable(priv.shms[shm_id].cap))
		return EC_RES_INVALID_PARAM;

	r->phys_addr = (uint64_t)priv.shms[shm_id].addr;
	r->len = priv.shms[shm_id].len;
	r->type = priv.shms[shm_id].type;

	if (r->type == EC_CODEC_SHM_TYPE_SYSTEM_RAM &&
			priv.driver.translate_addr_ec_to_ap) {
		int ret;
		uintptr_t addr;

		ret = priv.driver.translate_addr_ec_to_ap(r->phys_addr, &addr);
		if (ret != EC_SUCCESS)
			return ret;

		r->phys_addr = (uint64_t)addr;
	}

	args->response_size = sizeof(*r);
	return EC_RES_SUCCESS;
}

static int (*sub_cmds[])(struct host_cmd_handler_args *) = {
	[EC_CODEC_GET_CAPABILITIES] = get_capabilities,
	[EC_CODEC_GET_SHM_ADDR] = get_shm_addr,
};

#ifdef DEBUG_AUDIO_CODEC
static char *strcmd[EC_CODEC_SUBCMD_COUNT] = {
	[EC_CODEC_GET_CAPABILITIES] = "EC_CODEC_GET_CAPABILITIES",
	[EC_CODEC_GET_SHM_ADDR] = "EC_CODEC_GET_SHM_ADDR",
};
#endif

static int host_command(struct host_cmd_handler_args *args)
{
	struct ec_param_ec_codec *p = (struct ec_param_ec_codec *)args->params;

	DBG("subcommand: %s", strcmd[p->cmd]);

	if (p->cmd < EC_CODEC_SUBCMD_COUNT)
		return sub_cmds[p->cmd](args);

	WARN("invalid subcommand: %d", p->cmd);
	return EC_RES_INVALID_PARAM;
}
DECLARE_HOST_COMMAND(EC_CMD_EC_CODEC, host_command, EC_VER_MASK(0));

/*
 * Exported interfaces.
 */
int audio_codec_capable(uint8_t cap)
{
	if (!cap)
		return 1;
	return priv.capabilities & BIT(cap);
}

int audio_codec_register_shm(uint8_t shm_id, uint8_t cap,
		uintptr_t addr, uint32_t len, uint8_t type)
{
	if (shm_id >= EC_CODEC_SHM_ID_LAST)
		return EC_ERROR_INVAL;
	if (cap >= EC_CODEC_CAP_LAST)
		return EC_ERROR_INVAL;

	priv.shms[shm_id].cap = cap;
	priv.shms[shm_id].addr = addr;
	priv.shms[shm_id].len = len;
	priv.shms[shm_id].type = type;

	return EC_SUCCESS;
}

int audio_codec_register_driver(struct audio_codec_driver *driver)
{
	if (!driver)
		return EC_SUCCESS;

	if (driver->translate_addr_ec_to_ap)
		priv.driver.translate_addr_ec_to_ap =
			driver->translate_addr_ec_to_ap;

	return EC_SUCCESS;
}
