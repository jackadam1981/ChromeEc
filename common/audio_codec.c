...
static const uint32_t capabilities =
	...
	| BIT(EC_CODEC_CAP_WOV_AUDIO_SHM_ACTIVE)
#endif
#ifdef CONFIG_AUDIO_CODEC_CAP_WOV_AUDIO_SHM_PASSIVE
	| BIT(EC_CODEC_CAP_WOV_AUDIO_SHM_PASSIVE)
#endif
#ifdef CONFIG_AUDIO_CODEC_CAP_WOV_LANG_SHM_ACTIVE
	| BIT(EC_CODEC_CAP_WOV_LANG_SHM_ACTIVE)
#endif
#ifdef CONFIG_AUDIO_CODEC_CAP_WOV_LANG_SHM_PASSIVE
	| BIT(EC_CODEC_CAP_WOV_LANG_SHM_PASSIVE)
#endif
	;
...
static int get_shm_addr(struct host_cmd_handler_args *args)
{
	...
	r->phys_addr = (uint64_t)*(shms[shm_id].addr);
...
static int set_shm_addr(struct host_cmd_handler_args *args)
{
	...
	const uint8_t cap = p->set_shm_addr_param.cap;

	if (shm_id >= EC_CODEC_SHM_ID_LAST)
		return EC_RES_INVALID_PARAM;
	if (!shms[shm_id].addr || !audio_codec_capable(cap))
		return EC_RES_INVALID_PARAM;
	if (*(shms[shm_id].addr))
		return EC_RES_BUSY;

	*(shms[shm_id].addr) = (uintptr_t)p->set_shm_addr_param.phys_addr;
	*(shms[shm_id].len) = p->set_shm_addr_param.len;
	shms[shm_id].type = p->set_shm_addr_param.type;
