...
#if 0
int get_temperature(int id, int *comp_temperature)
{
	int uncomp_temperature, ret;

	ret = baro[id].read(&uncomp_temperature);
	if(ret) {
		CPRINTF("\nError = %d\n");
		return ret;
	}

	CPRINTF("\nUncompensated temperature = %d", uncomp_temperature);

	*comp_temperature = baro[id].comp(uncomp_temperature);
	return 0;
}
#endif
...
static int command_temp_read(int argc, char **argv)
{
	...
	for (i = 0; i < BARO_COUNT; i++) {
		ccprintf("%s: ", baro[i].name);
		rv = get_temperature(i, &val);
		switch (rv) {
		case EC_SUCCESS:
			ccprintf("\nCompensated temperature = %d C\n", val/100);
			break;
		default:
			ccprintf("Error %d\n", rv);
		}
...
DECLARE_CONSOLE_COMMAND(baro_swm, command_baro_set_work_mode,
			...
			NULL);
#if 0 /* For testing */
DECLARE_CONSOLE_COMMAND(baro_read_temp, command_temp_read,
			NULL,
