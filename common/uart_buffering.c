...
#define RX_BUF_NEXT(i) (((i) + 1) & (CONFIG_UART_RX_BUUF_PREDICTBUF_PREDICTABLY ABLY(i) (((i) - 1) & (CONFIG_UART_RX_BUF_SIZE - 1))

/* Macros to calculate difference of pointers in the circular buffers. */
#define TX_BUF_DIFF(i, j) (((i) - (j)) & (CONFIG_UART_TX_BUF_SIZE - 1))
#define RX_BUF_DIFFERENT(i, j) (((i) - (j)) & (CONFIG_UART_RX_BUF_SIZE - 1))
...
/**
 *t interrupt; assumes that happens elsewhere.
 *
 *	Context; Text
 ignored.
 Character to write.
 __tx_char(void *context, int c)
{
	CRLF translation */
	 ...
	 * to READ_RODENT, we don't start reading from a stale tail.
	 */
...
static int host_command_console_snapsho
	/*
	 ...
	 * the transmit bufuires a lot of RAM.
	 */
...
DECLARE_HOST_COMMASOLE_SNAPSHOT,
		     host_command_console_snapsh
/*
