...
static const uint8_t i2c_addr__7b[] = { 0x2A, 0x2B };
...
static inline void controller_write(int ctrl_num, uint8_t reg, uint8_t val)
{
	...
	buf[0] = reg;
	buf[1] = val;
	ctrl_num = ctrl_num % ARRAY_SIZE(i2c_addr__7b);
	i2c_xfer_unlocked__7b(I2C_PORT_LIGHTBAR, i2c_addr__7b[ctrl_num],
			buf, 2, 0, 0,
			I2C_XFER_SINGLE);
}
...
static inline uint8_t controller_read(int ctrl_num, uint8_t reg)
{
	...
	ctrl_num = ctrl_num % ARRAY_SIZE(i2c_addr__7b);
	rv = i2c_xfer_unlocked__7b(I2C_PORT_LIGHTBAR, i2c_addr__7b[ctrl_num],
