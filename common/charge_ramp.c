...
void chg_ramp_task(void)
{
	...
	int i;
	enum chg_ramp_state ramp_st_prev, ramp_st_new;

	ramp_st = ramp_st_prev = CHG_RAMP_DISCONNECTED;
	...
	while (1) {
		...
		ccprintf("CHG RAMP: %d->%d", ramp_st_prev, ramp_st);
		...
		case CHG_RAMP_RAMP:
			...
			if (active_icl == MAX_CHARGING_CURRENT_MA)
				...
				active_icl = MAX_CHARGING_CURRENT_MA;
			else
				active_icl += RAMP_CURR_INCR_MA;
