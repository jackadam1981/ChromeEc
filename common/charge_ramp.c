...
void chg_ramp_task(void)
{
	...
	while (1) {
		...
		case CHG_RAMP_STABLIZE:
			if (active_port == stablize_port && active_sup == stablize_sup) {
				ramp_st_new = CHG_RAMP_STABLE;
				break;
			}
			if (active_port == CHARGE_PORT_NONE)
				...
				ramp_st_new = CHG_RAMP_CHARGE_DETECT;
			task_wait_time = 0;
			break;
		 ...
		 * limit.
		 */
		if (ramp_st == CHG_RAMP_RAMP || ramp_st == CHG_RAMP_STABLE || ramp_st == CHG_RAMP_STABLIZE)
