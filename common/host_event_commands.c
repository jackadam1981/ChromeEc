...
static uint32_t lpc_host_event_mask[3];
...
static void lpc_post_sysjump_restore_mask(void)
{
	...
	prev_mask = (const uint32_t *)system_get_jump_tag(LPC_SYSJUMP_TAG,
			&version, &size);
	if (!prev_mask || version != 1 || size != sizeof(lpc_host_event_mask))
		return;

	memcpy(lpc_host_event_mask, prev_mask, sizeof(lpc_host_event_mask));
}
DECLARE_HOOK(HOOK_INIT, lpc_post_sysjump_restore_mask, HOOK_PRIO_FIRST);
