...
#define HOST_EVENT_MASK(x)	(1UL << ((x) - 1))
...
int lpc_get_next_host_event(void)
{
	...
	if (evt_idx)
		host_clear_events(HOST_EVENT_MASK(evt_idx));
	return evt_idx;
}
...
void host_set_single_event(uint8_t event)
{
	host_event_t ev = 0;
...
#ifdef CONFIG_HOST_EVENT64
	if (event >= 32)
		*(ptr + 1) = HOST_EVENT_MASK(event - 32);
	else
#endif
		*ptr = HOST_EVENT_MASK(event);
...
static int command_host_event(int argc, char **argv)
{
	...
	if (argc == 3) {
		char *e;
		host_event_t i = strtoi(argv[2], &e, 0);
		if (*e)
