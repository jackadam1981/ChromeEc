...
static uint8_t active_mask_set_by_host;
...
static int host_event_set_wake_mask(struct host_cmd_handler_args *args)
{
	...
	active_mask_set_by_host = !!p->mask;
	return EC_RES_SUCCESS;
}
...
uint8_t lpc_is_active_mask_set_by_host(void)
{
	return active_mask_set_by_host;
}
...
static int program_host_event_set(struct host_cmd_handler_args *args)
{
	...
	case EC_HOST_EVENT_SMI_MASK:
		...
		break;
	case EC_HOST_EVENT_ALWAYS_REPORT_MASK:
		lpc_set_host_event_mask(LPC_HOST_EVENT_ALWAYS_REPORT, p->value);
		break;
	case EC_HOST_EVENT_ACTIVE_WAKE_MASK:
		active_mask_set_by_host = !!p->value;
