...
static void host_event_ccprintf(const char *str, uint64_t e)
{
	ccprintf("%s 0x%08x 0x%08x\n", str, (uint32_t)((e >> 32) & 0xffffffff),
		 (uint32_t)(e & 0xffffffff));
}

static void host_event_cprints(const char *str, uint64_t e)
{
	CPRINTS("%s 0x%08x 0x%08x", str, (uint32_t)((e >> 32) & 0xffffffff),
		(uint32_t)(e & 0xffffffff));
}
