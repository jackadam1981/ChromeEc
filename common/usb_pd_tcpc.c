 ...
 * performance.
 */
/* TODO: provide way to change debug level w/ console command */
...
/* PD transmit errors */
#define PD_TX_ERROR_GOODCRC   -1 /* Failed to receive goodCRC */
#define PD_TX_ERROR_DISABLED  -2 /* Attempted transmit even though disabled */
#define PD_TX_ERROR_INV_ACK   -4 /* Received different packet instead of gCRC */
#define PD_TX_ERROR_COLLISION -5 /* Collision detected during transmit */

struct mutex pd_crc_lock;

/* TODO: do we need this still? if so, what enables/disables it? */
...
void pd_phy_task(void)
{
	...
	while (1) {
		...
		if (pd_rx_started(port)) {
			...
			} else if (pd[port].rx_head == PD_ERR_HARD_RESET) {
		...
		if (evt & PD_EVENT_TX) {
			...
			else if (res == PD_TX_ERROR_GOODCRC)
				alert(port, TCPC_ALERT0,
...
{
	task_set_event(PORT_PHY_TO_TASK_ID(port), PD_EVENT_RX, 0);
}
...
/*
 * TODO: add more functionality to this console command. Some of it needs to
 * be moved over from the 'pd' console command.
 */
static int command_tcpc(int argc, char **argv)
{
	...
	ccprintf("CC: %d, CC0: %d, CC1: %d\n", pd[port].cc_pull,
		 pd[port].cc_status[0], pd[port].cc_status[1]);

	return EC_SUCCESS;
}
DECLARE_CONSOLE_COMMAND(tcpc, command_tcpc, "", "", NULL);
