		  ...
		  hash_t *new_root)
{
	...
	const hash_t (*view)[merkle_tree->param_h][param_k - 1] =
			(void *)hashes;
	hash_t temp_a;
	hash_t temp_b;
	param_h_t hx = 0;
...
static int test_rate_limit(leaf_data_t *leaf_data)
{
	...
	ccprintf("\ndelay_schedule entry -> {%u, %u}\n",
		 leaf_data->idat.delay_schedule[x - 1].attempt_count,
		 delay);
			...
			hash_t *new_root)
{
	...
	if (memcmp(request->low_entropy_secret,
		...
		/* ret most not be overwritten after this. */
