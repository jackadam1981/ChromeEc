 ...
 */
static uint8_t task_is_safe_mode_critical(task_id_t task_id)
{
...
uint8_t current_task_is_safe_mode_critical(void)
{
...
uint8_t system_is_in_safe_mode(void)
{
	return !!in_safe_mode;
}

uint8_t command_is_allowed_in_safe_mode(int command)
{
...
#ifdef TEST_BUILD
void set_system_safe_mode(uint8_t mode)
{
	in_safe_mode = mode;
}
#endif
