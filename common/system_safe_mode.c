...
#define CANNOT_ENTER_SAFE_MODE_FMT "Cannot enter Safe Mode: %s\n"
...
{
	panic_printf("Safe Mode timeout\n");
	panic_reboot();
}
...
int start_system_safe_mode(void)
{
	...
	if (system_is_in_safe_mode()) {
		panic_printf(CANNOT_ENTER_SAFE_MODE_FMT, "Already in Safe Mode");
		return EC_ERROR_INVAL;
	}

	if (is_current_task_safe_mode_critical()) {
		/* TODO: Restart critical tasks */
		panic_printf(CANNOT_ENTER_SAFE_MODE_FMT, "Panic in critical task");
		return EC_ERROR_INVAL;
	}
	...
	panic_printf("Starting Safe Mode\n");
