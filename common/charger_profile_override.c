			...
			int batt_vtg_max)
{
	...
	if (!(curr->batt.flags & BATT_FLAG_BAD_VOLTAGE)) {
		if (curr->batt.voltage < fast_chg_params->vtg_low_limit_mV)
			voltage_range = VOLTAGE_RANGE_LOW;
		else if (curr->batt.voltage >
				fast_chg_params->vtg_high_limit_mV)
...
static int command_fastcharge_test(int argc, char **argv)
{
	...
	if ((test_on && argc != 5) || !test_on)
		return EC_ERROR_PARAM_COUNT;

	test_batt_flag = strtoi(argv[2], &e, 0);
	if (*e || test_batt_flag > 1 || test_batt_flag < 0)
		return EC_ERROR_PARAM3;

	test_temp_c = strtoi(argv[3], &e, 0);
	if (*e)
		return EC_ERROR_PARAM4;

	test_vtg_mV = strtoi(argv[4], &e, 0);
	if (*e || test_vtg_mV < 0) {
		test_vtg_mV = -1;
...
DECLARE_CONSOLE_COMMAND(fastchgtest, command_fastcharge_test,
			"off | on bflag[1|0] temp_c vtg_mV",
