			...
			int batt_vtg_max)
{
	...
	/* voltage range */
	enum fast_chg_voltage_ranges voltage_range = VOLTAGE_RANGE_LOW;
	/* Current and previous battery voltage */
	int batt_voltage;
	static int prev_batt_voltage;
	int temp_ranges = fast_chg_params->chg_config_info->total_temp_ranges;
	...
	const struct fast_charge_config *chg_config_info =
				fast_chg_params->chg_config_info;
	...
	batt_voltage = curr->batt.flags & BATT_FLAG_BAD_VOLTAGE ?
			prev_batt_voltage : curr->batt.voltage;

	prev_batt_voltage = batt_voltage;

	if (batt_voltage < chg_config_info->vtg_low_limit_mV)
		voltage_range = VOLTAGE_RANGE_LOW;
	else if (batt_voltage > chg_config_info->vtg_high_limit_mV)
	...
	curr->requested_current = (voltage_range == VOLTAGE_RANGE_HIGH) ?
				(*prev_chg_prof_info)->curr_vtg_high_limit_mA :
				(*prev_chg_prof_info)->curr_vtg_low_limit_mA;
