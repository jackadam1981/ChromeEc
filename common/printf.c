...
#ifndef NO_UINT64_SUPPORT
__maybe_unused static char *itoa(char *buff, int buff_len, uint64_t i)
{
	int digits = 0, idx;
	uint64_t copy = i;
	      ...
	      const char *format, va_list args)
{
	...
	while (*format) {
		...
		if (c == 's') {
			...
			continue;
#if defined(CONFIG_FPU) && !defined(NO_UINT64_SUPPORT)
		} else if (c == 'f') {
			...
			*vstr = '\0';
			vlen = vstr - intbuf;
			vstr = intbuf;
#endif /* CONFIG_FPU && !NO_UINT64_SUPPORT */
		} else {
			int base = 10;
