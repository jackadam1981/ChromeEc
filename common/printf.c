	      ...
	      const char *format, va_list args)
{
	...
	while (*format) {
		...
		if (c == 's') {
			...
			/*
			 * Divide pad_width to avoid unnecessary overflow
			 * error.
			 * The /2 and 2* can/will be optimized out.
			 */
			...
			for (; pad_width > 0 && !(flags & PF_LEFT); pad_width--) {
		...
		if (precision < 0) {
			...
			/* If precision is set, ensure that we do not overrun precision */
