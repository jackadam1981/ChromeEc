...
static int print_float(int (*addchar)(void *context, int c), void *context,
		       char *buff, size_t bufflen, float vf)
{
	uint32_t i;
	char *vstr = buff;
	int count;
	...
	if (bufflen - (vstr - buff) >= 2) {
		*(vstr++) = '.';
		while (precision-- && bufflen - (vstr - buff) > 0) {
			vf *= 10;
			*(vstr++) = '0' + (int)vf;
			vf -= (int)vf;
		}
		...
		if (c == 's') {
			...
			if (c == 'p') {
				...
				if (ptrspec == 'T' &&
					...
					int rc = print_float(addchar, context,
							     intbuf,
							     sizeof(intbuf),
							     vf);
