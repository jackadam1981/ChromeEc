	...
	while (*format) {
		...
		if (c == 's') {
			...
			if (c == 'l') {
				...
				/*
				 * The
				 * CONFIG_PRINTF_DISABLE_32BIT_LONG_WORKAROUND
				 * flag is required to enable the %l flag on
				 * systems where it would signify a 32-bit
				 * value. Otherwise, %l on 32-bit systems is
