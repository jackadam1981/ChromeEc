		...
		uint8_t *in, int in_size, int flags)
{
	...
	if (out_size == 1) {
		...
		ccprints("%s TCPCI read %s = 0x%x", task_get_name(task_get_current()), reg->name, reg->value);
		if (reg->size == 1)
			in[0] = reg->value;
		else if (reg->size == 2) {
			in[0] = reg->value;
		...
		if (reg->size == 1)
			...
			value = out[1] + (out[2] << 8);
		ccprints("%s TCPCI write %s = 0x%x", task_get_name(task_get_current()), reg->name, value);
		reg->value = value;
	}
	return EC_SUCCESS;
}
