...
/* Mock compute_sha256 for unit or fuzz tests. */
void compute_sha256(uint8_t *output, const uint8_t *key, const int key_len,
		    const uint8_t *message, const int message_len)
{
	CPRINTF("mocked %s using type %d\n", __func__,
	...
	case MOCK_CTRL_FPSENSOR_CRYPTO_SHA256_TYPE_FF:
		CPRINTF("returning all 0xFF for sha256\n");
		memcpy(output, fake_fpsensor_crypto[MESSAGE_ZERO_IDX],
		       FP_POSITIVE_MATCH_SECRET_BYTES);
		break;
	default:
