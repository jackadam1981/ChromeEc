...
int mkbp_button_get_next_event(uint8_t *out)
{
	memcpy(out, &mkbp_button_state, sizeof(mkbp_button_state));
	return sizeof(mkbp_button_state);
}
DECLARE_EVENT_SOURCE(EC_MKBP_EVENT_BUTTON, mkbp_button_get_next_event);

static int switch_get_next_event(uint8_t *out)
{
	memcpy(out, &mkbp_switch_state, sizeof(mkbp_switch_state));
	return sizeof(mkbp_switch_state);
}
