...
enum lpc_status battery_command_get_info(uint8_t *data)
{
	...
	r->designed_capacity         = val;
	r->designed_capacity_warning = val * BATTERY_WARNING_PERCENTAGE / 100;
	r->designed_capacity_low     = val * BATTERY_LOW_PERCENTAGE / 100;
...
enum lpc_status battery_command_get_type(uint8_t *data)
{
	...
	if (battery_device_chemistry(r->text, EC_LPC_BATT_SIZE_MAX))
		return EC_LPC_RESULT_ERROR;
...
enum lpc_status battery_command_get_model_number(uint8_t *data)
{
	...
	if (battery_device_name(r->text, EC_LPC_BATT_SIZE_MAX))
		return EC_LPC_RESULT_ERROR;
...
enum lpc_status battery_command_get_oem(uint8_t *data)
{
	...
	if (battery_manufacturer_name(r->text, EC_LPC_BATT_SIZE_MAX))
		return EC_LPC_RESULT_ERROR;
...
enum lpc_status battery_command_get_cycle_count(uint8_t *data)
{
	struct lpc_response_battery_cycle_count *r =
			(struct lpc_response_battery_cycle_count *)data;

	int cycle;
	if (battery_cycle_count(&cycle))
		return EC_LPC_RESULT_ERROR;

	r->cycle = cycle;
	return EC_LPC_RESULT_SUCCESS;
}
DECLARE_HOST_COMMAND(EC_LPC_COMMAND_BATTERY_CYCLE_COUNT,
		battery_command_get_cycle_count);
