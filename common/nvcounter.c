 ...
 * they are between the code/read-only area and the NVMEM area of each RW partition.
 * the 'LOW' page in RW_A and the 'HIGH' page in RW_B (at the same relative
 * offset).
 */
...
static uint32_t _decode(const uint32_t *p, size_t i)
{
	...
	return INCORRECT_FLASH_CNT;	/* unkown state */
}
...
static void _inc(const uint32_t *p, size_t i)
{
	...
	if (v == 8) {
		 ...
		 * read it as 0
		 * this run. Making sure next run will see it as 0 for sure.
		 * Note this is extra hit past the 8 / word.. should be ok, even
		 * 8 writes
		 * per word is far below the word line requirement, an extra
		 * should be
		 * negligible
		 */
		_write(p, i, 0);
...
uint32_t nvcounter_incr(void)
{
	...
	for (hi = 0; hi < PAGE_WORDS; ++hi) {
		 ...
		 * was interrupt
		 * and we need to finish it off again
		 */
		if (result & 1) {
			_erase(FLASH_CNT_LO);
			/* mark erase done */
		 ...
		 * entirely 0, so we
		 * have not yet reached the end, continue counting, otherwise,
		 * breakout of
		 * the for loop
		 */
		if (result != 8)
