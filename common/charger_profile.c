...
#define TEMP_RANGE fast_charge_config_info.temp_last_range
...
int charger_profile_override(struct charge_state_data *curr)
{
	...
	if (!(curr->batt.flags & BATT_FLAG_BAD_TEMPERATURE)) {
		for (i = 0; i < fast_charge_config_info.temp_ranges; i++) {
	...
	if (curr->batt.flags & BATT_FLAG_BAD_VOLTAGE) {
		...
		if (batt_voltage < fast_charge_config_info.vtg_low_limit)
			voltage_range = VOLTAGE_RANGE_LOW;
		else if (batt_voltage > fast_charge_config_info.vtg_high_limit)
	...
	curr->requested_current = (voltage_range == VOLTAGE_RANGE_HIGH) ?
				fast_charge_info[TEMP_RANGE].curr_high_vtg :
				fast_charge_info[TEMP_RANGE].curr_low_vtg;
