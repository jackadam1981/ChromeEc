...
{
	buf->preserved->checksum = 0;
	buf->preserved->head = 0;
	buf->preserved->version = buf->constants.version;
	buf->preserved->capacity = buf->constants.capacity;
	buf->preserved->word_size = buf->constants.word_size;
...
{
	return (buf->preserved->version == buf->constants.version &&
		buf->preserved->capacity == buf->constants.capacity &&
		buf->preserved->word_size == buf->constants.word_size &&
		buf->preserved->checksum ==
...
{
	return (buf->preserved->head < buf->constants.capacity) ?
		       buf->preserved->head :
...
preserved_ring_buf_write(const preserved_ring_buf_t *buf, uint32_t word)
{
	const uint32_t index =
		(buf->preserved->head++ % buf->constants.capacity);
	switch (buf->constants.word_size) {
	case 1:
		buf->preserved->checksum +=
	...
	case 2:
		...
		break;
	case 4:
		buf->preserved->checksum += word - buf->buffer.as_uint32[index];
...
{
	uint32_t tail = 0;
	if (buf->preserved->head > buf->constants.capacity)
		tail = buf->preserved->head;
