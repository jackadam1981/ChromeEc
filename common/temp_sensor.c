...
int temp_sensor_read(enum temp_sensor_id id)
{
	...
	return sensor->read(sensor->idx) / 100;
}


int temp_sensor_read_hi_precision(enum temp_sensor_id id)
{
	const struct temp_sensor_t *sensor;
...
static void update_mapped_memory(void)
{
	...
	for (i = 0; i < TEMP_SENSOR_COUNT; i++, mptr32++) {
		...
		t = temp_sensor_read_hi_precision(i);
		if (t == -1)
...
static int command_temps(int argc, char **argv)
{
	...
	for (i = 0; i < TEMP_SENSOR_COUNT; ++i) {
		...
		t = temp_sensor_read(i);
		t100 = temp_sensor_read_hi_precision(i);
		t100 = t100 - 27315;
		if (t < 0) {
			ccprintf("Error\n");
			rv = EC_ERROR_UNKNOWN;
