 ...
 * make availabe for master read accesses.
 *
...
static void wr_complete_handler(void *i2cs_data, size_t i2cs_data_size)
{
	...
	struct i2c_tpm_reg_map* i2c_reg_entry = NULL;
	uint16_t reg_size;
	...
	if (!i2cs_data_size) {
		...
		if (reg_size == 4) {
			tpm_register_get(tpm_reg, &reg_value,
					 sizeof(reg_value));

			/* Convert it to network byte order. */
			for(i = 0; i < sizeof(reg_value); i++)
				i2cs_post_read_data(reg_value[sizeof(reg_value) - 1 - i]);
			return;
		}
