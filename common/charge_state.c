...
enum led_pwr_state led_pwr_get_state(void)
{
	...
	case ST_CHARGE:
		...
		} else if (charge_is_charger_sufficient()) {
			return LED_PWRS_CHARGE;
		} else {
			return LED_PWRS_INSUFFIENT_CHARGER;
		}
...
{
	int min_voltage, voltage;

	if (charger_get_min_required_voltage(0, &min_voltage)) {
		return false;
	}
	...
	CPRINTS("min_voltage=%d, voltage=%d", min_voltage, voltage);
	...
	case CHARGE_STATE_CMD_GET_PARAM:
		...
		if (IS_ENABLED(CONFIG_CHARGER_PROFILE_OVERRIDE) &&
			...
			case CS_PARAM_CHG_MIN_REQ_VOLTAGE:
				if (charger_get_min_required_voltage(0, &val)) {
