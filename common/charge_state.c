...
enum led_pwr_state led_pwr_get_state(void)
{
	...
	case ST_IDLE:
		...
		if (battery_seems_dead || curr.batt.is_present == BP_NO)
			return LED_PWR_STATE_ERROR;

		if (chflags & CHARGE_FLAG_FORCE_IDLE)
			return LED_PWR_STATE_FORCED_IDLE;
		else
			return LED_PWR_STATE_IDLE;
	case ST_DISCHARGE:
#ifdef CONFIG_PWR_STATE_DISCHARGE_FULL
		if (battery_near_full())
			return LED_PWR_STATE_DISCHARGE_FULL;
		else
#endif
			return LED_PWR_STATE_DISCHARGE;
	case ST_CHARGE:
		...
		if (IS_ENABLED(CONFIG_CHARGE_MANAGER) &&
			...
			return LED_PWR_STATE_CHARGE_NEAR_FULL;
		else
			return LED_PWR_STATE_CHARGE;
	case ST_PRECHARGE:
		chflags = charge_get_flags();
