...
static enum power_state state_charge(struct power_state_context *ctx)
{
	...
	const struct charger_info *c_info = ctx->charger;
	int next_current;
	static timestamp_t debounce_ts;
	...
	if (batt->desired_current != curr->charging_current) {
		...
		/* Battery voltage debouncing logic */
		next_current = batt->desired_current;

		if (batt->desired_current > curr->charging_current) {
			if (!timestamp_expired(debounce_ts, NULL))
				next_current = curr->charging_current;
		} else {
			debounce_ts = get_time();
			debounce_ts.val += DEBOUNCE_TIME;
		}

		if (charger_set_current(next_current))
			return PWR_STATE_ERROR;
	...
	if (curr->ts.val - ctx->charger_update_time.val >
