...
{
	enum ec_charge_control_mode new_mode = CHARGE_CONTROL_COUNT;
	 ...
	 * Hosts specify EC_CHARGE_CONTROL_FLAG_NO_IDLE for such boards to use
	 * DISCHARGE instead of IDLE.
	 */
	switch (mode) {
...
static void sustain_battery_soc(void)
{
	...
	if (new_mode == CHARGE_CONTROL_COUNT || new_mode == mode)
		return;
		...
		if (p->cmd == EC_CHARGE_CONTROL_CMD_SET) {
			if (p->mode == CHARGE_CONTROL_NORMAL) {
				...
				if (args->version == 2) {
					 ...
					 * NO_IDLE. TODO: Remove this if-branch
					 * once all hosts are updated to v3.
					 */
					if (sustain_soc.lower <
					    sustain_soc.upper)
						sustain_soc.flags =
