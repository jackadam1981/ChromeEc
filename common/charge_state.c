...
#define CHARGE_MAX_PPS_VOLTAGE_INCREMENT (320) /* 320 mV*/
#define CHARGE_PPS_VOLTAGE_INCREMENT (30) /* 30 mV*/

#define CHARGE_MIN_PPS_VOLTAGE_DECREMENT (150) /* 150 mV*/
#define CHARGE_PPS_VOLTAGE_DECREMENT (20) /* 20 mV*/

#define CHARGE_PPS_CHARGER_INCREMENT (150) /* 150 mV*/
#define CHARGE_PPS_MAX_CURRENT (3000) /* 3 A*/
...
static void charge_handle_adaptive_legacy(int chgnum)
{
	...
	pdata->req_mv = pdata->chg_mv = curr.batt.voltage + 500;
	charge_adative_legacy_find_voltage(chgnum, &pdata->req_mv,
					   &pdata->req_ma);
	pdata->req_ma = CHARGE_PPS_MAX_CURRENT;
	pdata->chg_ma = CHARGE_PPS_MAX_CURRENT;
...
static void charger_init_adaptive_pps(int chgnum)
{
	struct charge_adaptive_data *pdata = &adaptive_data[chgnum];
	...
	pdata->req_ma = CHARGE_PPS_MAX_CURRENT;
	pdata->req_mv = curr.batt.voltage + 100;
	if (curr.batt.flags & BATT_FLAG_WANT_CHARGE) {
		pdata->chg_ma = pdata->req_ma;
		pdata->chg_mv = pdata->req_mv + CHARGE_PPS_CHARGER_INCREMENT;
	} else {
		pdata->chg_ma = 0;
		pdata->chg_mv = 0;
	}
	ccprintf("C%d: Init Adaptive PPS = %d mV\n", chgnum,
		 pdata->req_mv);
	pd_enable_pps(chgnum, true);
	pd_set_new_power_request(chgnum);
}

static void charger_start_adaptive_pps(int chgnum)
{
	struct charge_adaptive_data *pdata = &adaptive_data[chgnum];
	...
	if (curr.batt.flags & BATT_FLAG_WANT_CHARGE) {
		...
		charger_enable_ptm(chgnum, true);
		pdata->chg_ma = 0;
		pdata->chg_mv = 0;
	}
	ccprintf("C%d: Start Adaptive PPS = %d mV\n", chgnum,
		 pdata->req_mv);
	pdata->ts = get_time();
}

static void charger_fix_adaptive_pps(int chgnum)
{
	struct charge_adaptive_data *pdata = &adaptive_data[chgnum];
...
static void charger_adjust_adaptive_pps(int chgnum)
{
	struct charge_adaptive_data *pdata = &adaptive_data[chgnum];
...
static void charge_handle_adaptive_pps(int chgnum)
{
	...
	if (pdata->prev_mode != CHARGE_ADAPTIVE_PPS) {
		charger_init_adaptive_pps(chgnum);
		return;
	}

	if (!pdata->ts.val) {
		charger_start_adaptive_pps(chgnum);
		return;
	}
	...
	now = get_time();
	if (now.val - pdata->ts.val >= CHARGE_PPS_FIX_VOLTAGE_INTERVAL) {
		charger_is_pps_enabled(chgnum, &enabled);
		if (!enabled) {
			charger_fix_adaptive_pps(chgnum);
			return;
		}
	}

	if (now.val - pdata->ts.val >= CHARGE_PPS_REFRESH_VOLTAGE_INTERVAL) {
		charger_adjust_adaptive_pps(chgnum);
	}
}
				...
				bool is_full)
{
	...
	if (charge_handle_adaptive() != CHARGE_ADAPTIVE_DISABLE && curr.ac) {
