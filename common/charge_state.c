...
static void update_battery_info(void)
{
	...
	memset(batt_str, 0, EC_LPC_MEMMAP_STRING_LEN);
	battery_manufacturer_name(batt_str, EC_LPC_MEMMAP_STRING_LEN);

	/* Battery Model string */
	batt_str = (char *)(lpc_get_memmap_range() + EC_LPC_MEMMAP_BATT_MODEL);
	memset(batt_str, 0, EC_LPC_MEMMAP_STRING_LEN);
	battery_device_name(batt_str, EC_LPC_MEMMAP_STRING_LEN);
