...
/**
 * Enable or disable charging, and set requested voltage and current. If both
 * voltage and current are set to 0, charging is disable.
 *
 ...
 */
static int charge_set_charging(int voltage, int current)
{
	int rv = EC_SUCCESS;
...
static int state_common(struct power_state_context *ctx)
{
	...
	if (rv) {
		...
		if (curr->ac && ctx->battery_responsive &&
			 ...
			 */
			charge_set_charging(ctx->battery->voltage_max,
...
static enum power_state state_idle(struct power_state_context *ctx)
{
	...
	if (batt->desired_voltage && batt->desired_current) {
		...
		if (charge_set_charging(batt->desired_voltage, want_current))
			return PWR_STATE_ERROR;
...
static enum power_state state_charge(struct power_state_context *ctx)
{
	...
	if (want_voltage != curr->charging_voltage) {
		...
		if (charge_set_charging(want_voltage, -1))
			return PWR_STATE_ERROR;
	...
	if (charge_set_charging(-1, want_current))
		return PWR_STATE_ERROR;
