...
static void poweroff_wait_ac(void)
{
	...
	if (chipset_in_state(CHIPSET_STATE_ON)) {
		...
		/* Workaround: current code can not shutdown system
		 * currectly. This workaround use EC reset to make CPU off.
		 * TODO(rong): remove the system reset workaround
		 */
		system_reset(1);
	}
 ...
 *	- new states: TRICKLE, CHARGE, INIT
 */
	...
	if (ctx->curr.batt.desired_voltage &&
		...
		if (batt->desired_current < ctx->charger->current_min) {
			...
			if (charger_set_current(c_info->current_min) ||
			    charger_set_voltage(batt->voltage + 500))
				return PWR_STATE_ERROR;
...
static void charging_progress(struct power_state_context *ctx)
{
	...
	if (ctx->curr.charging_voltage != ctx->prev.charging_voltage &&
			ctx->curr.batt.state_of_charge == 0) {
		minutes = get_time().val - ctx->trickle_charging_time.val;
		/* Prevent GCC generate 64bit div calls */
		minutes /= 1000000;
		minutes /= 60;
