...
void charge_state_machine_task(void)
{
	...
	while (1) {
		...
		if (force_state || new_state) {
			...
			force_state = PWR_STATE_UNCHANGE;
...
static int command_charging_state(int argc, char **argv)
{
	...
	if (argc == 1) {
		uart_printf("Usage:\n    %s [state]\n", argv[0]);
		uart_puts("Available states: ");
		for (i = 1; state_name[i]; i++) {
			uart_puts(state_name[i]);
			uart_puts(" ");
		}
		uart_puts("\n\n");
		uart_printf("Current state   : %s ", state_name[curr->state]);
		if (curr->state == PWR_STATE_CHARGE) {
			battery_time_to_full(&minutes);
			uart_printf("(%d%%) time to full : %dh:%d",
				batt->state_of_charge,
				minutes / 60, minutes % 60);
		} else {
			battery_time_to_empty(&minutes);
			uart_printf("(%d%%) time to empty: %dh:%d",
				batt->state_of_charge,
				minutes / 60, minutes % 60);
		}
		uart_puts("\n");
