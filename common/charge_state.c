...
static inline void uart_print_binary(int value, int bits)
{
	int i;
	for (i = 0; i < bits; i++)
		uart_puts((value & (1 << i)) ? "1 " : "0 ");
}
...
{
	struct batt_params batt;
	uint32_t *memmap_batt_flags = (uint32_t*)(lpc_get_memmap_range() +
...
static enum POWER_STATE state_error(void)
{
	...
	if (error_flags != last_error_flags) {
		uart_puts("errors   : ");
		uart_print_binary(error_flags, F_LAST);
		uart_puts("\nprevious : ");
		uart_print_binary(last_error_flags, F_LAST);
		uart_puts("\n");
...
/* Debugging constants and routines */
const static char *_state_name[] = {
	"null", "init", "idle", "discharge", "charge", "error", "x"
};
...
void charge_state_machine_task(void)
{
	...
	enum POWER_STATE power_state, new_state;
	uint32_t *memmap_batt_flags = (uint32_t*)(lpc_get_memmap_range() +
					EC_LPC_MEMMAP_BATT_FLAG);
	uint32_t batt_flags;

	prev_ts.val = 0;
	power_state = PWR_STATE_INIT;
