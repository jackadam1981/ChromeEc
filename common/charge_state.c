...
static int state_common(struct power_state_context *ctx)
{
	...
	if (curr->ac != prev->ac) {
		if (curr->ac) {
			 ...
			 * TODO(rong):
			 * crosbug.com/8309 Send AC present SMI event
			 * crosbug.com/7937 Send PCH AC present
			 */
	...
	while (1) {
		...
		if (new_state) {
			...
			uart_printf("[Charge state %s -> %s]\n",
				_state_name[ctx.prev.state],
				_state_name[new_state]);
		}

		switch (new_state) {
		case PWR_STATE_IDLE:
			*ctx.memmap_batt_flags &= ~EC_BATT_FLAG_CHARGING;
			*ctx.memmap_batt_flags &= ~EC_BATT_FLAG_DISCHARGING;
