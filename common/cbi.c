...
static uint8_t cbi[256];
struct cbi_header * const head = (struct cbi_header *)cbi;

static uint8_t cbi_crc8(const struct cbi_header *head)
{
	return crc8((uint8_t *)&head->crc + 1, head->total_size - 4);
}
...
static int do_read_board_info(void)
{
	...
	if (cbi_crc8((struct cbi_header *)cbi) != head->crc) {
		CPRINTS("Bad CRC");
		return EC_ERROR_INVAL;
	}
...
int cbi_get_board_info(enum cbi_data_tag tag, uint8_t *buf, uint8_t size)
{
	...
	for (p = cbi + sizeof(*head); p + 2 < cbi + head->total_size; p++) {
		uint8_t t = *p++;
		uint8_t len = *p++;
		if (t == tag) {
			if (len > size)
...
int cbi_set_board_info(enum cbi_data_tag tag, const uint8_t *buf, uint8_t size)
{
	...
	/* Check if new item would fit */
	if (p + 2 + size > cbi + sizeof(cbi))
		return EC_ERROR_OVERFLOW;
...
static int write_board_info(void)
{
	...
	uint8_t buf[EEPROM_PAGE_WRITE_SIZE + 1];
	uint8_t *p = cbi;
	...
	buf[0] = 0;	/* Offset 0 */
	while (rest > 0) {
		size = MIN(EEPROM_PAGE_WRITE_SIZE, rest);
		rest -= size;
		memcpy(&buf[1], p, size);
...
{
	return cbi_get_board_info(CBI_DATA_BOARD_VERSION,
				  (uint8_t *)ver, sizeof(*ver));
}

int cbi_get_sku_id(uint32_t *id)
{
	return cbi_get_board_info(CBI_DATA_SKU_ID, (uint8_t *)id, sizeof(*id));
}

int cbi_get_oem_id(uint32_t *id)
{
	return cbi_get_board_info(CBI_DATA_OEM_ID, (uint8_t *)id, sizeof(*id));
}
...
{
	uint32_t ver;
	if (cbi_get_board_info(CBI_DATA_BOARD_VERSION,
			       (uint8_t *)&ver, sizeof(ver)))
		return -1;
	return ver;
}
...
static int hc_cbi_get(struct host_cmd_handler_args *args)
{
	...
	if (cbi_get_board_info(p->tag, (uint8_t *)&val, sizeof(val)))
		return EC_RES_INVALID_PARAM;

	*(uint32_t *)args->response = val;
	args->response_size = sizeof(val);
