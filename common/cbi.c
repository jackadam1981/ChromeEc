...
static enum ec_status hc_cbibin_get(struct host_cmd_handler_args *args)
{
	const struct __ec_align4 ec_params_get_cbibin *p = args->params;
	uint8_t size = MIN(args->response_max, UINT8_MAX);
...
DECLARE_HOST_COMMAND(EC_CMD_GET_CROS_BOARD_INFO_BIN, hc_cbibin_get,
		     EC_VER_MASK(0));
...
static enum ec_status hc_cbibin_set(struct host_cmd_handler_args *args)
{
	/*
	...
	const struct __ec_align4 ec_params_set_cbibin *p = args->params;
	...
	if (p->commit == EC_CBI_BIN_COMMIT_FLAG_BUFFER_CLEAR) {
		memset(cbi_buf, 0xFF, CBI_IMAGE_SIZE);
	} else if (p->commit == EC_CBI_BIN_COMMIT_FLAG_BUFFER_UPDATE) {
		if (p->offset < CBI_IMAGE_SIZE) {
	...
	} else if (p->commit == EC_CBI_BIN_COMMIT_FLAG_WRITE) {
		...
		CPRINTS("Invalid commit flag");
		return EC_RES_ERROR;
	}
	return EC_RES_SUCCESS;
}
DECLARE_HOST_COMMAND(EC_CMD_SET_CROS_BOARD_INFO_BIN, hc_cbibin_set,
		     EC_VER_MASK(0));
