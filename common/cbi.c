...
static int cache_status = EC_CBI_CACHE_INVALID;
...
static int cbi_read(void)
{
	...
	if (cache_status != EC_CBI_CACHE_INVALID)
		return EC_SUCCESS;

	for (i = 0; i < 2; i++) {
		rv = cbi_storage_config.drv->load(cbi);
		if (rv == EC_SUCCESS) {
			cache_status = EC_CBI_CACHE_SYNCED;
			return EC_SUCCESS;
		}
...
int cbi_write(void)
{
	return cbi_storage_config.drv->store(cbi);
}
...
{
	cache_status = EC_CBI_CACHE_INVALID;
}
							...
							ec_params_set_cbi * p)
{
	 ...
	 */
	if (cbi_storage_config.drv->is_protected() &&
...
static void dump_cbi(void)
{
	...
	if (cache_status != EC_CBI_CACHE_SYNCED) {
