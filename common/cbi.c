 ...
 */
uint8_t buf[sizeof(struct ec_params_set_cbi) + CONFIG_CONSOLE_INPUT_LINE_SIZE];
...
static int cc_cbi(int argc, char **argv)
{
	...
	if (argc > last_arg) {
		if (strcasecmp(argv[last_arg], "init") == 0) {
			setter->flag = CBI_SET_INIT;
		} else if (strcasecmp(argv[last_arg], "skip_write") == 0) {
			setter->flag = CBI_SET_NO_SYNC;
		} else {
			ccprintf("Invalid additional option\n");
			return EC_ERROR_PARAM1 + last_arg - 1;
		}
	} else {
		setter->flag = 0;
	}
