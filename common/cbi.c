...
static void cbi_remove_tag(void *const cbi, const struct cbi_data *const d)
{
	struct cbi_header *h = cbi;
	const size_t size = sizeof(*d) + d->size;
	const uint8_t *next = (uint8_t *)d + size;
	const size_t bytes_after =
		h->total_size - ((uint8_t *)next - (uint8_t *)cbi);
	memmove((void *)d, (void *)next, bytes_after);
	h->total_size -= size;
}
