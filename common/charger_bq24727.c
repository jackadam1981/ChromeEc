 ...
 */
static const struct charger_info bq24727_charger_info = {
	.name         = "bq24727",
	.voltage_max  = 19200,
	.voltage_min  = 1024,
	.voltage_step = 16,
...
/* bq24727 specific interfaces */

int charger_set_input_current(int input_current)
{
	return sbc_write(BQ24727_INPUT_CURRENT,
...
int charger_get_input_current(int *input_current)
{
	...
	rv = sbc_read(BQ24727_INPUT_CURRENT, &reg);
	if (rv)
		return rv;
...
{
	return sbc_read(BQ24727_MANUFACTURE_ID, id);
}

int charger_device_id(int *id)
{
	return sbc_read(BQ24727_DEVICE_ID, id);
}

int charger_get_option(int *option)
{
	return sbc_read(BQ24727_CHARGE_OPTION, option);
}

int charger_set_option(int option)
{
	return sbc_write(BQ24727_CHARGE_OPTION, option);
}
