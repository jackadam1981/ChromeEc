...
static void spi_it8xxx2_init0(void)
{
	printk("%s\r\n", __func__);
...
static void spi_it8xxx2_init1(void)
{
	printk("%s\r\n", __func__);
...
static int command_spi_cmd0(int argc, char **argv)
{
	int ret=0;
	char *e;
	int opt;
...
DECLARE_CONSOLE_COMMAND(spi_test0, command_spi_cmd0,
			NULL,
			"SPI Test");


static int command_spi_cmd1(int argc, char **argv)
{
	int ret=0;
	char *e;
	int opt;
	...
	printk("\r\n");
	printk("SPI1 Command Tests\r\n");
	printk("%s\r\n", __func__);

	if (argc > 2)
		return EC_ERROR_PARAM_COUNT;

	if ((spi_channel_status & SPI_CH1_RDY) != SPI_CH1_RDY) {
		printk("[user][spi_cmd1] The SPI CH1 isn't ready.\r\n");
		return -EIO;
	}
...
DECLARE_CONSOLE_COMMAND(spi_test1, command_spi_cmd1,
			NULL,
			"SPI Test");
