...
#define REG_MAX_LOAD_TIME_TO_EMPTY0x20
...
int battery_device_name(char *device_name, int buf_size)
{
	...
	if (rv)
		return rv;
	if (dev_type == BQ27541_TYPE_ID)
		strzcpy(device_name, "BQ27541", buf_size);
	else
		strzcpy(device_name, "Unknown", buf_size);
...
{
	return bq27541_read(REG_FULL_AVAILABLE_CAPACITY, capacity);
}
