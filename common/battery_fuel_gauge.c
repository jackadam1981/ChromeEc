...
void init_battery_type(void)
{
	...
	if (device_fail ||
	    battery_device_name(batt_device_name, sizeof(batt_device_name))) {
...
const struct batt_conf_embed *get_batt_conf(void)
{
	...
	/* BATT_FLAG_RESPONSIVE is changed */
	if ((prev_bf ^ batt->flags) & BATT_FLAG_RESPONSIVE) {
		if ((batt->flags & BATT_FLAG_RESPONSIVE) && need_retry)
			deadline.val = now.val + BATTERY_INIT_TYPE_TIMEOUT;
	}

	prev_bf = batt->flags;

	if (need_retry && !timestamp_expired(deadline, &now)) {
