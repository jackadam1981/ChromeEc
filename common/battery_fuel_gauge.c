...
void init_battery_type(void)
{
	...
	battery_conf = &board_battery_info[type].config;
	strncpy(default_battery_conf.manuf_name,
		board_battery_info[type].manuf_name,
		sizeof(default_battery_conf.manuf_name));
	strncpy(default_battery_conf.device_name,
		board_battery_info[type].device_name,
		sizeof(default_battery_conf.device_name));
}
...
static int bcfg_search_in_cbi(struct batt_conf_export *conf)
{
	char manuf[32];
	char device[32];
...
test_export_static void batt_conf_main(void)
{
	if (IS_ENABLED(CONFIG_BATTERY_CONFIG_IN_CBI) &&
		...
		if (bcfg_search_in_cbi(&default_battery_conf) == EC_SUCCESS) {
			battery_conf = &default_battery_conf.config;
			return;
		}
	}
