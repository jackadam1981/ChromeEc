...
batt_const struct board_batt_params *battery_params;
...
static void detect_battery(void)
{
	...
	if (IS_ENABLED(TEST_BUILD) && battery_fuel_gauge_type_override >= 0) {
		battery_params =
			&board_battery_info[battery_fuel_gauge_type_override];
		return;
	}
	...
	if (battery_manufacturer_name(manuf_name, sizeof(manuf_name))) {
		CPRINTS("Battery not found (no manuf name)");
		return;
	}
	...
	if (battery_type == BATTERY_TYPE_COUNT) {
		CPRINTS("Battery not found. Set to default");
		battery_type = board_get_default_battery_type();
	}

	battery_params = &board_battery_info[battery_type];
}
