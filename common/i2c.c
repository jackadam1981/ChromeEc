...
#ifdef CONFIG_I2C_PASSTHRU_RESTRICTED
__attribute__((weak)) const struct i2c_cmd_desc_t i2c_cmd_whitelist[] = { };
__attribute__((weak)) int i2c_whitelist_size = ARRAY_SIZE(i2c_cmd_whitelist);
...
static int i2c_cmd_in_whitelist(const struct i2c_cmd_desc_t *cmd_desc)
{
	...
	for (i = 0; i < i2c_whitelist_size; ++i) {
		if (i2c_cmd_whitelist[i].port == cmd_desc->port &&
		    i2c_cmd_whitelist[i].cmd == cmd_desc->cmd &&
		    i2c_cmd_whitelist[i].slave_addr == cmd_desc->slave_addr &&
		    i2c_cmd_whitelist[i].leng_rd >= cmd_desc->leng_rd &&
		    i2c_cmd_whitelist[i].leng_wr >= cmd_desc->leng_wr)
			return 1;
	}
	return 0;
}
