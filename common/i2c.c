...
static int i2c_test_read(int test_dev, int *data, int report)
{
	int rv;
	struct i2c_stress_test_dev *i2c_s_test =
		i2c_stress_tests[test_dev].i2c_test;
	struct i2c_test_reg_info *reg_s_info =
		&i2c_s_test->reg_info;

	rv = i2c_s_test->i2c_read ?
		i2c_s_test->i2c_read(i2c_stress_tests[test_dev].port,
			i2c_stress_tests[test_dev].addr,
			reg_s_info->read_reg, data) :
		i2c_s_test->i2c_read_dev(reg_s_info->read_reg, data);
...
static int i2c_test_write(int test_dev, int data, int report)
{
	int rv;
	struct i2c_stress_test_dev *i2c_s_test =
		i2c_stress_tests[test_dev].i2c_test;
	...
	rv = i2c_s_test->i2c_write ?
		i2c_s_test->i2c_write(i2c_stress_tests[test_dev].port,
			i2c_stress_tests[test_dev].addr,
			reg_s_info->write_reg, data) :
		i2c_s_test->i2c_write_dev(reg_s_info->write_reg, data);
...
void i2c_stress_test(int dev, int count, int udelay, int read,
		     int rand_seq, int report)
{
	int i, j, k;
	int rv, rand;
	...
	CPRINTS("dev=%d count=%d udelay=%d read=%d seq=%d report=%d",
		dev, count, udelay, read, rand_seq, report);
	...
	for (i = 0, k = 0; i < count; i++, k++) {
		...
		if (rand & 0x1) {
			/* read */
			rv = i2c_test_read(test_dev, &data, report);
			...
			do {
				...
				rv = i2c_test_write(test_dev, data, report);
				i++;
				if (rv) {
				...
				rv = i2c_test_read(test_dev, &data_verify, report);
				i++;
				if (rv) {
...
static int command_i2ctest(int argc, char **argv)
{
	...
	int read = 0, rand_seq = 0, report = 0;
	char *e;
	char *args[] = {
		[0] = "dev",
		[1] = "count",
	...
	i2c_stress_test(dev, count, udelay, read, rand_seq, report);
 ...
 * udelay - delay betwen read / write in micro seconds
 * read - only do reads
 * seq - test devices sequencially
 * report - send report to host on each read / write
 */
DECLARE_CONSOLE_COMMAND(i2ctest, command_i2ctest,
			"i2ctest dev|count|udelay|read|seq|report",
			"I2C stress test");
...
static int hc_i2c_test(struct host_cmd_handler_args *args)
{
	...
	if (p->dev > i2c_test_dev_used)
		return EC_ERROR_PARAM2;

	i2c_stress_test(p->dev, p->count, p->udelay, p->read,
			p->rand_seq, p->report);
