...
static uint8_t __bss_slow debounced_state[KEYBOARD_KSO_PINS];
/* Matrix from previous scan */
static uint8_t __bss_slow prev_state[KEYBOARD_KSO_PINS];
/* Mask of keys being debounced */
static uint8_t __bss_slow debouncing[KEYBOARD_KSO_PINS];
/* Keys simulated-pressed */
static uint8_t __bss_slow simulated_key[KEYBOARD_KSO_PINS];
...
static uint8_t __bss_slow scan_edge_index[KEYBOARD_KSO_PINS][KEYBOARD_KSI_PINS];
...
static int read_matrix(uint8_t *state)
{
	...
	keyboard_raw_drive_kso_pins(KEYBOARD_KSO_PINS_NONE);
...
static int has_ghosting(const uint8_t *state)
{
	...
	for (c = 0; c < KEYBOARD_KSO_PINS; c++) {
		...
		for (c2 = c + 1; c2 < KEYBOARD_KSO_PINS; c2++) {
			/*
			 * A little bit of cleverness here.  Ghosting happens
			 * if 2 columns (kso pins) share at least 2 keys.
			 * So we OR the columns(kso pins) together
			 * and then see if more than one bit
...
static int check_keys_changed(uint8_t *state)
{
	...
	int c, i;
	int any_change = 0;
	static uint8_t __bss_slow new_state[KEYBOARD_KSO_PINS];
...
{
	uint8_t curr_state[KEYBOARD_KSO_PINS];
	int c;
	...
	/*
	 * Check if KSI2 is asserted for all columns
	 * (kso pins) due to power button hold,
	 * and ignore it if so.
	 */
	for (c = 0; c < KEYBOARD_KSO_PINS; c++)
	...
	while (1) {
		...
		while (1) {
			...
			if (!local_disable_scanning &&
			    (keyboard_raw_read_ksi_pins() || force_poll))
				break;
			else
				task_wait_event(-1);
		}
...
static int mkbp_command_simulate_key(struct host_cmd_handler_args *args)
{
	...
	if (p->col >= KEYBOARD_KSO_PINS || p->row >= KEYBOARD_KSI_PINS)
...
static int command_keyboard_press(int argc, char **argv)
{
	if (argc == 1) {
		...
		c = strtoi(argv[1], &e, 0);
		if (*e || c < 0 || c >= KEYBOARD_KSO_PINS)
			return EC_ERROR_PARAM1;

		r = strtoi(argv[2], &e, 0);
		if (*e || r < 0 || r >= KEYBOARD_KSI_PINS)
