...
static int read_matrix(uint8_t *state)
{
	...
	for (c = 0; c < keyboard_cols; c++) {
		...
		for (c2 = 0; c2 < c; c2++) {
			 ...
			 * two "keyboard_raw_read_rows"s.
			 *
			 * If this happened, update both columns to the union of
			 * them, and hopefully has_ghosting will reject this
			 * updated state.
			 *
			 * Note that in theory we need to rescan from col 0 if
			 * thing is updated. Let's ignore this rare case for
			 * now.
			 */
