...
static int read_matrix(uint8_t *state, bool at_boot)
{
	...
	if (pb_pressed && at_boot) {
		...
		for (c = 0; c < keyboard_cols; c++) {
			...
			if (!(state[c] & KEYBOARD_MASKED_BY_POWERBTN(key_typ.row_refresh))) {
				break;
			}
#else
			if (!(state[c] & KEYBOARD_MASKED_BY_POWERBTN(KEYBOARD_ROW_REFRESH))) {
				break;
			}
#endif
		}
		...
		if (c == keyboard_cols) {
			for (c = 0; c < keyboard_cols; c++) {
				...
				if (c == key_typ.col_refresh)
					continue;
				state[c] &= ~KEYBOARD_MASKED_BY_POWERBTN(key_typ.row_refresh);
#else
				if (c == KEYBOARD_COL_REFRESH)
					continue;
				state[c] &= ~KEYBOARD_MASKED_BY_POWERBTN(KEYBOARD_ROW_REFRESH);
#endif
			}
		}
	}
...
static int check_runtime_keys(const uint8_t *state)
{
...
#ifndef CONFIG_KEYBOARD_MULTIPLE
	if (state[KEYBOARD_COL_RIGHT_ALT] != KEYBOARD_ROW_TO_MASK(KEYBOARD_ROW_RIGHT_ALT) &&
	    state[KEYBOARD_COL_LEFT_ALT] != KEYBOARD_ROW_TO_MASK(KEYBOARD_ROW_LEFT_ALT))
		return 0;
#else
	if (state[key_typ.col_right_alt] != KEYBOARD_ROW_TO_MASK(key_typ.row_right_alt) &&
	    state[key_typ.col_left_alt] != KEYBOARD_ROW_TO_MASK(key_typ.row_left_alt))
		return 0;
#endif
	...
	if ((state[KEYBOARD_COL_KEY_R] & ~mask_key_vol_up_row) ==
		...
		return 1;
	} else if (state[KEYBOARD_COL_KEY_H] == KEYBOARD_ROW_TO_MASK(KEYBOARD_ROW_KEY_H)) {
		/* H = hibernate */
		CPRINTS("hibernate");
...
#else
	/* Check individual keys */
	if (state[key_typ.col_key_r] == KEYBOARD_ROW_TO_MASK(key_typ.row_key_r)) {
		/* R = reboot */
		CPRINTS("warm reboot");
		keyboard_clear_buffer();
		chipset_reset(CHIPSET_RESET_KB_WARM_REBOOT);
		return 1;
	} else if (state[key_typ.col_key_h] == KEYBOARD_ROW_TO_MASK(key_typ.row_key_h)) {
		/* H = hibernate */
		CPRINTS("hibernate");
		system_enter_hibernate(0, 0);
		return 1;
