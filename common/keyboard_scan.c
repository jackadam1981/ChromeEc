...
static int read_matrix(uint8_t *state, bool at_boot)
{
	...
	if (!keyboard_scan_is_enabled()) {
		...
		uint32_t all_mask = GENMASK(keyboard_cols - 1, 0);
		uint32_t mask;
		int nbit;
		bool hit;
		uint8_t row;

		/* Mark all the keys possible */
		for (c = 0; c < keyboard_cols; c++)
			state[c] = GENMASK(7, 0);

		/* Binary search: start driving 8 columns, and then 4, 2, 1. */
		for (nbit = BIT(3); nbit > 0; nbit >>= 1) {
			/* Iterate the columns, until out-of-range */
			for (mask = GENMASK(nbit - 1, 0); ; mask = mask << nbit)
			{
				mask &= all_mask;
				if (!mask)
					break;
