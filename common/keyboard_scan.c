 ...
 * least once.
 */
static void ensure_keyboard_scanned(void)
{
	int old_polls = kbd_polls;
	uint64_t start_time;

	start_time = get_time().val;

	/* Ensure at least 2 updates so we know the loop fully ran */
	while ((kbd_polls - old_polls < 2) &&
