...
static int keyscan_is_enabled(void)
{
#ifdef BOARD_link
 ...
 */
static void keyscan_print_state(const uint8_t *state, const char *msg)
{
	int c;
 ...
 */
static void keyscan_simulate_key(int row, int col, int pressed)
{
	...
	keyscan_print_state(simulated_key, "simulated ");
 ...
 */
static int keyscan_read_matrix(uint8_t *state)
{
	int c;
	uint8_t r;
	int pressed = 0;
 ...
 */
static int keyscan_check_keys_changed(uint8_t *state)
{
	int any_pressed = 0;
	int any_change = 0;
	int c, i;
	...
	any_pressed = keyscan_read_matrix(new_state);
	...
	if (any_change) {
		...
		if (print_state_changes)
			keyscan_print_state(state, "state");
 ...
 * boot keys.
 */
static enum boot_key keyscan_check_boot_key(const uint8_t *state)
{
...
void keyboard_scan_init(void)
{
	...
	keyscan_read_matrix(debounced_state);
	memcpy(prev_state, debounced_state, sizeof(prev_state));

	/* Check for keys held down at boot */
	boot_key_value = keyscan_check_boot_key(debounced_state);
...
void keyboard_scan_task(void)
{
	...
	keyscan_print_state(debounced_state, "init state");
	...
	while (1) {
		...
		do {
			 ...
			 */
			if (!keyboard_raw_read_rows() || !keyscan_is_enabled())
				task_wait_event(-1);
		} while (!keyscan_is_enabled());
		...
		while (keyscan_is_enabled()) {
			...
			/* Check for keys down */
			if (keyscan_check_keys_changed(debounced_state)) {
...
static int mkbp_command_simulate_key(struct host_cmd_handler_args *args)
{
	...
	keyscan_simulate_key(p->row, p->col, p->pressed);
...
static int command_ksstate(int argc, char **argv)
{
	if (argc > 1) {
		...
		keyscan_print_state(debounced_state, "debounced ");
		keyscan_print_state(prev_state, "prev      ");
		keyscan_print_state(debouncing, "debouncing");
	}
