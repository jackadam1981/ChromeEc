...
static const
	...
	struct boot_key_entry boot_key_list[] = {
		...
		{ KEYBOARD_COL_REFRESH, KEYBOARD_ROW_REFRESH,
		  BOOT_KEY_REFRESH },
#endif
	};
 ...
 *
 * Note this doesn't take care of other scanning activities. For example, it
 * doesn't restore column states after a scan. Thus, it can be safely used only
 * before the scan task starts. Otherwise, it may cause the scan task to behave
 * unexpectedly.
 *
 * @param column
 * @return row value
