...
static uint32_t check_boot_key(const uint8_t *state)
{
	...
	if (system_jumped_late())
		...
		/*
		 * Boot keys are available only through reset-pin reset, which
		 * can be issued only by GSC (through refresh+power combo).
		 *
		 * If the EC resets differently (e.g. watchdog, power-on,
		 * exception), we don't want to accidentally enter recovery mode
		 * even if a refresh key or whatever key is pressed (as
		 * previously allowed).
		 */
#ifndef CONFIG_KEYBOARD_MULTIPLE
