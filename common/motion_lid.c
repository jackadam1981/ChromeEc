			       ...
			       int *lid_angle)
{
	...
	base_range = accel_base->drv->get_range(accel_base) * 10;
	lid_range = accel_lid->drv->get_range(accel_lid) * 10;
	...
	base_dot_product /= (base_range * base_range);
	lid_dot_product /= (lid_range * lid_range);
