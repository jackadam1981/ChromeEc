...
int motion_lid_calc_now(void)
{
	...
	if (motion_sense_read(accel_base) || motion_sense_read(accel_lid))
		/* Sensors are not ready or powered. We have no time
		 * to get them ready. */
		return LID_ANGLE_UNRELIABLE;

	memcpy(accel_base->xyz, accel_base->raw_xyz, sizeof(accel_base->xyz));
	memcpy(accel_lid->xyz, accel_lid->raw_xyz, sizeof(accel_lid->xyz));
