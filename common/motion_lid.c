...
static void lid_angle_set_tablet_mode_threshold(int ang, int deg)
{
	tablet_mode_lid_ang = ang;
	tablet_mode_hys_deg = deg;

	tablet_zone_lid_angle = INT_TO_FP(TABLET_ZONE_ANGLE(ang, deg));
	laptop_zone_lid_angle = INT_TO_FP(LAPTOP_ZONE_ANGLE(ang, deg));
}
...
int host_cmd_motion_lid(struct host_cmd_handler_args *args)
{
	...
	case MOTIONSENSE_CMD_TABLET_MODE_LID_ANGLE:
		...
		if ((in->tablet_mode_threshold.lid_ang !=
		     EC_MOTION_SENSE_NO_VALUE) &&
		    (in->tablet_mode_threshold.hys_deg !=
		     EC_MOTION_SENSE_NO_VALUE))
			lid_angle_set_tablet_mode_threshold(
					in->tablet_mode_threshold.lid_ang,
					in->tablet_mode_threshold.hys_deg);

		out->tablet_mode_threshold.lid_ang = tablet_mode_lid_ang;
		out->tablet_mode_threshold.hys_deg = tablet_mode_hys_deg;

		args->response_size = sizeof(out->tablet_mode_threshold);
#else
		return EC_RES_INVALID_PARAM;
#endif
		break;
