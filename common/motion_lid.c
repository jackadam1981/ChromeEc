			       ...
			       int *lid_angle)
{
...
#endif
	int base_dot_product, lid_dot_product;
	...
	base_dot_product = scaled_base[X] * scaled_base[X] +
		scaled_base[Y] * scaled_base[Y] +
		scaled_base[Z] * scaled_base[Z];
	lid_dot_product = scaled_lid[X] * scaled_lid[X] +
	 ...
	 *
	 * Note, that we don't actually have to take the square root of the dot
	 * product to get the magnitude, but we can work with the dot products
	 * directly as shown below:
	 *
	 *          (A^2 - B^2) < 2 * 1g * NOISY_MAGNITUDE_DEVIATION
	 *
	 * Where A is base magnitude, and B is lid magnitude.
	 */
	if (ABS(base_dot_product - lid_dot_product) >
