			       ...
			       int *lid_angle)
{
	...
	if (ABS(scaled_base[0]) > x_max)
		x_max = ABS(scaled_base[0]);
	if (ABS(scaled_base[1]) > y_max)
		y_max = ABS(scaled_base[1]);
	if (ABS(scaled_base[2]) > z_max)
		z_max = ABS(scaled_base[2]);

	if (ABS(scaled_lid[0]) > x_max)
		x_max = ABS(scaled_lid[0]);
	if (ABS(scaled_lid[1]) > y_max)
		y_max = ABS(scaled_lid[1]);
	if (ABS(scaled_lid[2]) > z_max)
		z_max = ABS(scaled_lid[2]);

	if (scan_f > SCAN_F_MAX) {
		cprintf(CC_MOTION_SENSE," x_max: %-5d\n", x_max);
		cprintf(CC_MOTION_SENSE," y_max: %-5d\n", y_max);
		cprintf(CC_MOTION_SENSE," z_max: %-5d\n", z_max);
	}
	/*
