/* Copyright 2020 The Chromium OS Authors. All rights reserved.
 * Use of this source code is governed by a BSD-style license that can be
 * found in the LICENSE file.
 *
 * Non-volatile memory
 */

#include "common.h"
#include "console.h"
#include "flash.h"
#include "nvram.h"
#include "util.h"

#undef DEBUG_NVRAM

#define CPRINTS(format, args...) cprints(CC_SYSTEM, "NVM " format, ## args)
#ifdef DEBUG_NVRAM
#define DPRINTS(format, args...) CPRINTS(format, ## args)
#else
#define DPRINTS(format, args...)
#endif

#define NVRAM_ERR_CORRUPT	-1
#define NVRAM_ERR_INVALID	-2
#define NVRAM_ERR_READ		-3

const int nvram_block_size = NVRAM_BLOCK_SIZE;
const int nvram_flash_offset = NVRAM_FLASH_OFF;
static int next_offset;

static struct datablob_nvram {
	const struct datablob_driver *driver;
	const int record_size;
	int cache_status;
	uint8_t cache[NVRAM_RECORD_SIZE];
} nvram;

static int nvram_is_free(uint8_t *record, int size)
{
	while (size) {
		if (record[--size] != 0xff)
			return 0;
	}
	return 1;
}

/*
 * Read through the entire block and stores the last valid record and return its
 * offset. If a valid record isn't found or invalid sequence is detected, it
 * returns the following:
 *
 * - ERR_INVALID: Invalid record found.
 * - ERR_CORRUPT: Invalid sequence detected. Found used slot after free slot.
 * - ERR_READ:    Failed to read.
 *
 * We try not to use extra buffer. So, all data goes first into the cache and
 * gets examined. After the scan, the last valid record will be read again.
 */
static int _flash_scan(void)
{
	struct datablob_header *h = (struct datablob_header *)nvram.cache;
	int offset;
	int used = 0;
	int found_free = 0;
	int rv;

	for (offset = nvram_flash_offset;
			offset < nvram_flash_offset + nvram_block_size;
			offset += nvram.record_size) {
		rv = flash_read(offset, nvram.record_size, nvram.cache);
		if (rv) {
			CPRINTS("Failed to read (0x%x)", rv);
			return NVRAM_ERR_READ;
		}

		if (memcmp(h->magic, datablob_magic, sizeof(h->magic))
				|| (h->major_version > DATABLOB_VERSION_MAJOR)
				|| (h->total_size < sizeof(*h))
				|| (nvram.record_size < h->total_size)
				|| (datablob_crc8(h) != h->crc)) {
			if (nvram_is_free(nvram.cache, nvram.record_size)) {
				found_free = 1;
				DPRINTS("0x%x free", offset);
			} else {
				CPRINTS("0x%x bad record", offset);
				return NVRAM_ERR_INVALID;
			}
		} else {
			if (found_free) {
				CPRINTS("0x%x bad sequence", offset);
				return NVRAM_ERR_CORRUPT;
			}
			used = offset;
			DPRINTS("0x%x valid", offset);
		}
	}

	if (used && found_free) {
		/* Re-read valid record which was overwritten by free record */
		offset = used;
		flash_read(offset, nvram.record_size, nvram.cache);
	}

	return offset;
}

static int _flash_erase(void)
{
	int rv;

	CPRINTS("Formatting...");
	rv = flash_erase(nvram_flash_offset, nvram_block_size);
	if (rv) {
		next_offset = 0;
		CPRINTS("Failed to format (0x%x)", rv);
		return EC_ERROR_UNKNOWN;
	}

	/* Set pointer to the beginning of the block. */
	next_offset = nvram_flash_offset;
	return EC_SUCCESS;
}

static int _flash_is_protected(void)
{
	/* NVRAM resides in RW, which is always writable. */
	return 0;
}

static int _flash_write(const uint8_t *record, int record_size)
{
	if (!next_offset) {
		int offset;

		CPRINTS("Initializing");
		offset = _flash_scan();
		if (offset == NVRAM_ERR_READ) {
			return EC_ERROR_UNKNOWN;
		} else if (offset == NVRAM_ERR_CORRUPT ||
				offset == NVRAM_ERR_INVALID) {
			/* No records found */
			datablob_create(&nvram);
			if (_flash_erase())
				return EC_ERROR_UNKNOWN;
		}
	}

	if (next_offset == nvram_flash_offset + nvram_block_size) {
		/* All used */
		if (_flash_erase())
			return EC_ERROR_UNKNOWN;
	}

	if (flash_write(next_offset, record_size, record)) {
		CPRINTS("Failed to write");
		return EC_ERROR_UNKNOWN;
	}

	next_offset += record_size;

	return EC_SUCCESS;
}

static int _flash_read(uint8_t *record, int record_size)
{
	int offset = _flash_scan();

	if (offset == NVRAM_ERR_READ) {
		return EC_ERROR_UNKNOWN;
	} else if (offset == NVRAM_ERR_CORRUPT || offset == NVRAM_ERR_INVALID) {
		return EC_ERROR_INVAL;
	} else if (offset == nvram_flash_offset + nvram_block_size) {
		/* All slots are free */
		CPRINTS("All free");
		datablob_create(&nvram);
		next_offset = nvram_flash_offset;
	} else {
		CPRINTS("Read record from 0x%x", offset);
		next_offset = offset + record_size;
	}

	return EC_SUCCESS;
}

/*
 * NVRAM APIs
 */
int nvram_get_offset(void)
{
	return next_offset;
}

int nvram_get_cache_status(void)
{
	return nvram.cache_status;
}

int nvram_create(void)
{
	datablob_create(&nvram);
	return EC_SUCCESS;
}

int nvram_get(int tag, uint8_t *buf, uint8_t *size)
{
	return datablob_get_data(&nvram, tag, buf, size);
}

int nvram_set(int tag, const uint8_t *buf, uint8_t size)
{
	return datablob_set_data(&nvram, tag, buf, size);
}

int nvram_write(void)
{
	return datablob_write(&nvram);
}

void nvram_reset(void)
{
	uint8_t free_record[NVRAM_RECORD_SIZE];
	int p;

	memset(free_record, 0xff, nvram.record_size);

	for (p = nvram_flash_offset; p < nvram_flash_offset + nvram_block_size;
			p += nvram.record_size)
		flash_write(p, sizeof(free_record), free_record);

	memset(&nvram.cache, 0, sizeof(nvram.cache));
	nvram.cache_status = DATABLOB_CACHE_INVALID;
}

const struct datablob_driver flash_drv = {
	.save = _flash_write,
	.load = _flash_read,
	.erase = _flash_erase,
	.is_protected = _flash_is_protected,
};

static struct datablob_nvram nvram = {
	.driver = &flash_drv,
	.record_size = NVRAM_RECORD_SIZE,
};

#ifdef DEBUG_NVRAM
static int cc_nvram(int argc, char **argv)
{
	char *e;
	int tag;
	uint8_t size;
	uint8_t buf[NVRAM_RECORD_SIZE - sizeof(struct datablob_header)];
	int rv = EC_SUCCESS;

	if (argc < 2) {
		ccprintf("block_size=%d\n", nvram_block_size);
		ccprintf("flash_offset=0x%x\n", nvram_flash_offset);
		ccprintf("next_offset=0x%x\n", next_offset);
		ccprintf("cache_status=%d\n", nvram.cache_status);
		hexdump(nvram.cache, nvram.record_size);
		return EC_SUCCESS;
	}

	if (!strncasecmp(argv[1], "get", 3)) {
		if (argc != 3)
			return EC_ERROR_PARAM_COUNT;
		tag = strtoi(argv[2], &e, 0);
		size = sizeof(buf);
		rv = nvram_get(tag, buf, &size);
		if (rv)
			return rv;
		hexdump(buf, size);
	} else if (!strncasecmp(argv[1], "set", 3)) {
		if (argc != 5)
			return EC_ERROR_PARAM_COUNT;
		tag = strtoi(argv[2], &e, 0);
		size = strtoi(argv[3], &e, 0);
		rv = nvram_set(tag, argv[4], size);
		if (rv)
			return rv;
	} else if (!strncasecmp(argv[1], "list", 4)) {
		if (argc != 2)
			return EC_ERROR_PARAM_COUNT;
		for (tag = 0; tag < 0xff; tag++) {
			size = sizeof(buf);
			rv = nvram_get(tag, buf, &size);
			if (rv == EC_SUCCESS) {
				ccprintf("[%d]", tag);
				hexdump(buf, size);
			} else if (rv != EC_ERROR_NOT_FOUND) {
				return rv;
			}
		}
	} else if (!strncasecmp(argv[1], "write", 5)) {
		return nvram_write();
	} else {
		return EC_ERROR_PARAM1;
	}

	return EC_SUCCESS;
}
DECLARE_CONSOLE_COMMAND(nvram, cc_nvram,
			"set <tag> <size> <data>\n"
			"get <tag>\n"
			"list\n"
			"write\n",
			"Set/Get NVRAM");
#endif /* DEBUG_NVRAM */
