...
/* RX (Host->EC) */
static struct queue const usb_to_hostcmd = QUEUE_DIRECT(64, uint8_t,
							usbhc_stream.producer,
							hostcmd_consumer);
/* TX (EC->Host) */
static struct queue const hostcmd_to_usb = QUEUE_DIRECT(64, uint8_t,
							hostcmd_producer,
...
static void usbhc_written(struct consumer const *consumer, size_t count)
{
	...
	if (state == USBHC_STATE_READY_TO_RX) {
		CPRINTS("Beginning of Rx. Received %d bytes", count);
		block_index = 0;
		/* Only version 3 is supported. Using in_msg as a courtesy. */
		QUEUE_REMOVE_UNITS(consumer->queue, in_msg + block_index,
				   count);
	...
	if (block_index < header->data_len + sizeof(*header)) {
		return;	/* More to come. */
	}
	...
	if (expected_size < block_index) {
		CPRINTS("Packet is larger (expected %d bytes)", expected_size);
		/*
