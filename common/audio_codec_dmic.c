...
static int dmic_set_gain_idx(struct host_cmd_handler_args *args)
{
	...
	if (audio_codec_dmic_set_gain_idx(pp->channel, pp->gain) != EC_SUCCESS)
		return EC_RES_ERROR;
...
static int dmic_get_gain_idx(struct host_cmd_handler_args *args)
{
	...
	if (audio_codec_dmic_get_gain_idx(pp->channel, &r->gain) != EC_SUCCESS)
		return EC_RES_ERROR;
...
static int dmic_host_command(struct host_cmd_handler_args *args)
{
	...
	CPRINTS("invalid DMIC subcommand: %d", p->cmd);
