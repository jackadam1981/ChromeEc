 ...
 *    CONFIG_FLASH_NVMEM_PARTITION_SIZE -> size in bytes of a partition
 *
 * The board.c file must include 2 functions and an array of user buffer lengths
 *    nvmem_buffer_tab[] -> array of user buffer lengths
 *    nvmem_compute_sha() -> function used to compute 4 byte sha (or equivalent)
 *    nvmem_get_users() -> function used to get pointer to user length array
 */
...
struct nvmem_partition {
	...
	uint8_t buffer[CONFIG_FLASH_NVMEM_PARTITION_SIZE -
...
static int nvmem_verify_partition_sha(int index)
{
	...
	nvmem_compute_sha(p_data,
			  (CONFIG_FLASH_NVMEM_PARTITION_SIZE - NVMEM_SHA_SIZE),
			  sha_comp,
			  NVMEM_SHA_SIZE);
...
static int nvmem_acquire_cache(void)
{
	...
	if (shared_mem_size() < CONFIG_FLASH_NVMEM_PARTITION_SIZE) {
		CPRINTF("Not enough shared mem! avail = 0x%x < reqd = 0x%x\n",
			shared_mem_size(), CONFIG_FLASH_NVMEM_PARTITION_SIZE);
		return EC_ERROR_OVERFLOW;
	}

	while (attempts < NVMEM_ACQUIRE_CACHE_MAX_ATTEMPTS) {
		ret = shared_mem_acquire(CONFIG_FLASH_NVMEM_PARTITION_SIZE,
					 (char **)&cache_base_ptr);
		if (ret == EC_SUCCESS)
			return EC_SUCCESS;
...
static int nvmem_update_cache_ptr(void)
{
	...
	if (cache_base_ptr == NULL) {
		...
		p_src = (uint8_t *)(CONFIG_FLASH_NVMEM_BASE +
				    nvmem_act_partition *
				    CONFIG_FLASH_NVMEM_PARTITION_SIZE);
...
int nvmem_setup(uint16_t starting_version)
{
	 ...
	 * both corrupted
	 */
	for (part = 0; part < CONFIG_FLASH_NVMEM_NUM_PARTITIONS; part++) {
...
int nvmem_init(void)
{
	...
	nvmem_get_users(&p_buffer_tab, &nvmem_num_buffers);
		...
		enum nvmem_users user)
{
	...
	uintptr_t base_addr;
	int32_t s_buff_offset, d_buff_offset;
