...
#ifdef DEBUG_NVMEM
static int cc_nvmem(int argc, char **argv)
{
	char *e;
	int tag;
	uint8_t size;
	uint8_t buf[NVMEM_RECORD_SIZE - sizeof(struct datablob_header)];
	int rv = EC_SUCCESS;
	...
	if (argc < 2) {
		...
		hexdump(nvmem.cache, nvmem.record_size);
		return EC_SUCCESS;
	}
...
DECLARE_CONSOLE_COMMAND(nvmem, cc_nvmem,
			"set <tag> <size> <data>\n"
			"get <tag>\n"
			"list\n"
			"write\n",
