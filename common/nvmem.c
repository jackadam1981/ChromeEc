...
struct nvmem_mutex_ {
	task_id_t task;
	/* If set, do not release the lock until nvmem_save() is called. */
	int held_by_write;
	struct mutex mtx;
};
...
int nvmem_setup(uint8_t starting_generation)
{
	...
	for (part = 0; part < NVMEM_NUM_PARTITIONS; part++) {
		...
		/* Make sure the contents change between runs of nvmem_save(). */
		nvmem_cache[0] = part;
		 ...
		 void *data, enum nvmem_users user)
{
	...
	dest_addr = (uintptr_t)nvmem_cache;
