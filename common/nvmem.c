...
/*
 * The NAME contents are stored in flash memory. At run time there is an SRAM
 * cache and two instances of the contents if the flash in two partitions.
 *
 ...
 *
 * There is a semaphore controlling access to the NVMEM. There are two levels
 * of protection - for read only accesses and for write accesses. When the
 * module is initialized the semaphore is opened, each read access locks the
 * emaphore, reads out the data and unlocks the semaphore.
 *
 * If a write access ever occurs things get more complicated. The write access
 * leaves the semaphore locked and stores the flag, indicating that the
 * contents have changed and need to be saved, and stores the task id of the
 * task performing the write access.
 *
 * The semaohore remains locked in this case. Next time a read access happens,
 * if it comes from the same task, the unlock is bypassed because the 'write
 * in progress' flag is set. If the read request comes from another task, it
 * will be blocked.
 *
 * In case of pending write access the semaphore is released after
 * nvmem_commit() is invoked. It calls the nvmem_save() function which checks
 * if the cache contents indeed changed (by calculating the hash again). If
 * there is no change - the semaphore is release and the function exits. If
 * there is a change, the new generation value is set, the new hash is
 * calculated and the copy is saved in the least recently used flash
 * partition.
 */
...
static uint8_t nvmem_cache[NVMEM_PARTITION_SIZE];
...
static int nvmem_save(void)
{
	...
	tag = (struct nvmem_tag *)nvmem_cache;
	...
	tag->generation += 1;
	if (tag->generation == NVMEM_GENERATION_MASK)
		tag->generation += 1;
...
int nvmem_commit(void)
{
	...
	if (nvmem_mutex.task == TASK_ID_COUNT)
		return EC_SUCCESS;  /* Noting to commit. */
