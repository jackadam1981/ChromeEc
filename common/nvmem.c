...
/*
 * Given the nvmem tag address return the base address
 * of the header SHA calculation.
 */
static void nvmem_set_sha(struct nvmem_tag *tag, void *sha_buf)
{
	compute_hash(tag->padding, NVMEM_PARTITION_SIZE - NVMEM_SHA_SIZE,
...
static int nvmem_partition_sha_match(int index)
{
	...
	nvmem_set_sha(&p_copy->tag, sha_comp);
...
int nvmem_setup(uint8_t starting_generation)
{
	...
	for (part = 0; part < NVMEM_NUM_PARTITIONS; part++) {
		...
		nvmem_set_sha(&p_part->tag, p_part->tag.sha);
