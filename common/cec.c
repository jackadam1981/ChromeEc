				...
				uint8_t msg_len)
{
	...
	if (cec_find_action(cec_ports[port].offline_policy, command) ==
...
void cec_event_tx(void)
{
	...
	if (cec_state == CEC_STATE_IDLE) {
		/*
		 * Only update the interrupt time it's idle, otherwise it will
		 * interfere with the timing of the current transfer.
		 */
...
static enum ec_status hc_cec_write(struct host_cmd_handler_args *args)
{
	...
	if (cec_ports[port].drv->send(port, params->msg, args->params_size) !=
	    EC_SUCCESS)
		return EC_RES_BUSY;
...
static enum ec_status cec_set_enable(int port, uint8_t enable)
{
	...
	if (cec_ports[port].drv->set_enable(port, enable) != EC_SUCCESS)
		return EC_RES_ERROR;
...
static enum ec_status cec_set_logical_addr(int port, uint8_t logical_addr)
{
	...
	if (cec_ports[port].drv->set_logical_addr(port, logical_addr) !=
	    EC_SUCCESS)
		return EC_RES_ERROR;
...
static enum ec_status hc_cec_get(struct host_cmd_handler_args *args)
{
	...
	switch (params->cmd) {
	case CEC_CMD_ENABLE:
		if (cec_ports[port].drv->get_enable(port, &response->val) !=
		    EC_SUCCESS)
			return EC_RES_ERROR;
		break;
	case CEC_CMD_LOGICAL_ADDRESS:
		if (cec_ports[port].drv->get_logical_addr(
...
static void handle_received_message(void)
{
	...
	if (cec_ports[port].drv->get_received_message(port, &msg, &msg_len) !=
	    EC_SUCCESS) {
