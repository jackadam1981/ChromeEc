...
static struct vstore_slot vstore_slots[CONFIG_VSTORE_SLOTS];
static int vstore_size = sizeof(struct vstore_slot) * CONFIG_VSTORE_SLOTS;
...
static int vstore_info(struct host_cmd_handler_args *args)
{
	...
	r->slot_count = CONFIG_VSTORE_SLOTS;
	r->slot_locked = 0;
	for (i = 0; i < CONFIG_VSTORE_SLOTS; i++)
...
static int vstore_read(struct host_cmd_handler_args *args)
{
	...
	uint8_t *data = args->response;

	if (p->slot > CONFIG_VSTORE_SLOTS)
		return EC_RES_INVALID_PARAM;

	memcpy(data, vstore_slots[p->slot].data, EC_VSTORE_SLOT_SIZE);

	args->response_size = EC_VSTORE_SLOT_SIZE;
...
static void vstore_init(void)
{
	...
	if (prev && version == VSTORE_HOOK_VERSION && size == vstore_size)
		...
		memset(vstore_slots, 0, vstore_size);
}
