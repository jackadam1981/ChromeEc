...
static int read_keyblock(struct kb_container *kbc)
{
	...
	sig_offset = offsetof(struct vb2_keyblock, keyblock_signature) +
		kbc->kb->keyblock_signature.sig_offset;
	hash_offset = offsetof(struct vb2_keyblock, keyblock_hash) +
		kbc->kb->keyblock_hash.sig_offset;
	key_offset = offsetof(struct vb2_keyblock, data_key) +
	...
	if ((kb.header_version_major != KEYBLOCK_MAJOR_VERSION) ||
	    ...
	    (sig_offset > size) ||
	    ((sig_offset + kbc->kb->keyblock_signature.sig_size) > size) ||
	    (hash_offset > size) ||
	    ((hash_offset + kbc->kb->keyblock_hash.sig_size) > size) ||
	    (key_offset > size) ||
	    ((key_offset + kbc->kb->data_key.key_size) > size)) {
