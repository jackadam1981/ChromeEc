...
static uint8_t just_validated;

void ap_ro_device_reset(void)
{
	/*
	 * Save the AP RO verification state until the next tpm reset. There's
	 * a tpm reset right after AP RO verification when EC_RST_L gets
	 * released, so it shouldn't get cleared immediately.
	 */
	if (just_validated) {
		just_validated = 0;
		return;
