	...
	const uint8_t *expected_digest, struct gbb_descriptor *gbbd)
{
	...
	for (i = 0; i < ARRAY_SIZE(possible_factory_flags); i++) {
		...
		/*
		 * If the gbb is outside of the hash, then it's not going to
		 * change using other factory flags. There's no point in
		 * retrying verification with other flags when they're not
		 * getting injected.
		 */
		if (!(gbbd->status & GS_INJECT_FLAGS))
			return ROV_FAILED;
