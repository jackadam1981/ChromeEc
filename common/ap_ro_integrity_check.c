...
struct gbb_descriptor {
	...
	/* Pad the struct, so a gbb descriptor struct can be written to flash */
	uint8_t  pad[3];
} __packed;
...
/*
 * Reserve 512 bytes for AP RO check data.
 *
 * Saved AP RO check data includes the ap ro check header, the sha digest of the
 * firmware and the RO ranges.
 *
 * The V1 header, hash, and 32 ranges take up 296 bytes. 512 is a very
 * conservative amount of room to leave between the AP RO check data and
 * gbb descriptor.
 *
 * The entire AP_RO_DATA_SIZE is 2048, so it also leaves plenty of room after
 * the AP RO check space.
 */
#define AP_RO_CHECK_DATA_MAX_SIZE	512
BUILD_ASSERT((sizeof(struct ap_ro_check_header) + SHA256_DIGEST_SIZE +
	 AP_RO_MAX_NUM_RANGES * sizeof(struct ro_range)) <=
	 AP_RO_CHECK_DATA_MAX_SIZE);

/* AP RO Data Region information. */
/* The V1 AP RO hash payload is stored at the start of the AP RO space page. */
#define AP_RO_CHECK_DATA_OFFSET		0
/* The AP RO gbb payload is stored after the AP RO check v1 data. */
#define AP_RO_GBBD_DATA_OFFSET		\
		(AP_RO_CHECK_DATA_OFFSET + AP_RO_CHECK_DATA_MAX_SIZE)

#define AP_RO_CHECK_DATA_ADDR		\
		(AP_RO_DATA_SPACE_ADDR + AP_RO_CHECK_DATA_OFFSET)
#define AP_RO_GBBD_DATA_ADDR		\
		(AP_RO_DATA_SPACE_ADDR + AP_RO_GBBD_DATA_OFFSET)
...
static const uint32_t h1_apro_gbbd_data_flash_offset_ =
	h1_flash_offset_ + AP_RO_GBBD_DATA_OFFSET;
...
/* Fixed pointer at the H1 flash page storing the AP RO gbb information. */
static const struct ap_ro_check *p_chk_gbbd =
	(const struct ap_ro_check *)AP_RO_GBBD_DATA_ADDR;
...
/*
 * Verify the gbb data fits in the AP RO data space. The GBB data includes an AP
 * RO check header and gbb descriptor.
 */
BUILD_ASSERT(AP_RO_GBBD_DATA_OFFSET +
	(sizeof(struct ap_ro_check_header) + sizeof(struct gbb_descriptor)) <=
	AP_RO_DATA_SPACE_SIZE);
...
BUILD_ASSERT(sizeof(struct ro_range) % CONFIG_FLASH_WRITE_SIZE == 0);
...
BUILD_ASSERT(AP_RO_CHECK_DATA_OFFSET % CONFIG_FLASH_WRITE_SIZE == 0);
BUILD_ASSERT(AP_RO_GBBD_DATA_OFFSET % CONFIG_FLASH_WRITE_SIZE == 0);
							 ...
							 size_t input_size)
{
	...
	if (data_offset < h1_flash_offset_ ||
	    data_offset + prog_size > h1_flash_offset_ + AP_RO_DATA_SPACE_SIZE)
					      ...
					      size_t *response_size)
{
	...
	*response_size = 1; /* Just in case there is an error. */
	...
	rv = write_ap_ro_check_data(AP_RO_HASH_LAYOUT_VERSION_1,
				    ...
				    vc_num_of_ranges,
				    h1_apro_check_data_flash_offset_,
				    (uint32_t *)p_chk,
				    buf,
				    input_size);
	if (rv != ARCVE_OK) {
...
static int verify_ap_ro_gbb_space(void)
{
	...
	if ((p_chk_gbbd->header.type != AP_RO_HASH_TYPE_GBBD) ||
	    (p_chk_gbbd->header.version != AP_RO_GBBD_LAYOUT_VERSION_2))
		return EC_ERROR_CRC;
	...
	if (p_chk_gbbd->header.num_ranges != p_chk->header.num_ranges) {
		CPRINTS("%s: gbbd doesn't match v1 header", __func__);
		return EC_ERROR_CRC;
	}

	app_compute_hash(&p_chk_gbbd->gbbd, sizeof(struct gbb_descriptor),
			 &checksum, sizeof(checksum));

	if (memcmp(&checksum, &p_chk_gbbd->header.checksum, sizeof(checksum))) {
...
static int get_saved_factory_flags(struct gbb_descriptor *actual_gbbd)
{
	if (verify_ap_ro_gbb_space() != EC_SUCCESS)
		return EC_ERROR_CRC;

	if (!p_chk_gbbd->gbbd.validate_flags) {
		CPRINTS("%s: saved gbbd doesn't support validating flags",
			__func__);
		return EC_ERROR_CRC;
	}
	/*
	 * If the saved gbbd doesn't match the actual gbbd, use the actual
	 * gbbd.
	 */
	if (memcmp(&p_chk_gbbd->gbbd, actual_gbbd, sizeof(actual_gbbd) -
		   sizeof(actual_gbbd->injected_flags))) {
		CPRINTS("%s: actual gbbd doesn't match saved", __func__);
		return EC_ERROR_CRC;
	}
	actual_gbbd->injected_flags = p_chk_gbbd->gbbd.injected_flags;
	CPRINTS("%s: loaded 0x%x", __func__, actual_gbbd->injected_flags);
	return EC_SUCCESS;
}

/**
 * Find the GBB using the FMAP table in AP flash.
 *
 ...
 *
 * @return zero on success, -1 if both the GBB is not found.
 */
static int find_gbb(uint32_t offset, uint16_t nareas,
 ...
 *
 * If the GBB flags are in the hash, validate the gbb. Verify the falgs
 *
...
static enum ap_ro_check_result init_gbbd(struct gbb_descriptor *gbbd)
{
	...
	for (offset = 0; offset < MAX_SUPPORTED_FLASH_SIZE;
		...
		/* Find the GBB in AP SPI */
		if (find_gbb(offset + sizeof(struct fmap_header),
		...
		if (rv == EC_ERROR_CRC) {
			/*
			 * p_chk is invalid. It's not possible to find a fmap
			 * in range.
			 */
			return ROV_FAILED;
...
static uint8_t do_ap_ro_check(void)
{
	...
	if (rv == ROV_SUCCEEDED) {
		if (gbbd.validate_flags &&
		     get_saved_factory_flags(&gbbd) != EC_SUCCESS) {
			rv = validate_ranges_sha_with_factory_flags(
				...
				&gbbd);
			/*
			 * Had to go through the factory flags. Attempt to save
			 * the flags if there verification passed.
			 */
			save_flags = rv == ROV_SUCCEEDED;
		} else {
	...
	/* Attempt to save the flags before releasing the EC from reset. */
	if (save_flags) {
...
static int ap_ro_info_cmd(int argc, char **argv)
{
	...
	ccprintf("gbb       : ");
	if (rv == EC_SUCCESS)
		ccprintf("saved (0x%x)\n", p_chk_gbbd->gbbd.injected_flags);
	else
		ccprintf("invalid (%u)\n", rv);
	rv = ap_ro_check_unsupported(false);
