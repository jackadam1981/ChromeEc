...
#define AP_RO_HASH_TYPE 0
#define GVD_HASH_TYPE	1
...
struct ap_ro_check_header {
	uint8_t version;
	uint8_t type;
	 /* This field is ignored when type is GVD_HASH_TYPE. */
	uint16_t num_ranges;
	uint32_t checksum;
};
...
/*
 * Supported signature and hashing algorithms, subset of the values defined
 * in vboot_reference.
 */
 ...
 * @param pubk dcrypto represenation of the RSA key, used for signature
 *             verification.
 *
 * @return zero on success, -1 on failure.
 */
		       ...
		       struct vb_rsa_pubk *pubk)
{
	...
	case VB2_ALG_RSA4096_SHA256:
		...
		break;
	default:
		ccprintf("%s: unsupported algorithm %d\n", __func__,
			 packedk->algorithm);
		return -1;
	}
	...
	if (packedk->key_size != exp_key_size) {
		ccprintf("%s: key size mismatch %d %d\n", __func__,
			 packedk->key_size, exp_key_size);
		return -1;
	}
	...
	if (arr_size != (exp_sig_size / sizeof(uint32_t))) {
		ccprintf("%s: array size mismatch %d %d\n", __func__, arr_size,
 ...
 */
static int verify_keblock(const struct kb_container *kbc,
			  const struct vb_rsa_pubk *pubk)
{
	int rv;
					      ...
					      size_t *response_size)
{
	...
	*response_size = 1; /* Just in case there is an error. */
	...
	check_header.type = AP_RO_HASH_TYPE;
...
static const struct gvd_descriptor *find_v2_entry(void)
{
	...
	if ((p_chk->header.version < AP_RO_HASH_LAYOUT_VERSION_1) ||
	    (p_chk->header.type != GVD_HASH_TYPE))
		return NULL;

	/* Verify entry integrity. */
	SHA256_hw_init(&ctx);
	SHA256_update(&ctx, &p_chk->descriptor, sizeof(p_chk->descriptor));
	if (memcmp(SHA256_final(&ctx), &p_chk->header.checksum,
		   sizeof(p_chk->header.checksum))) {
		ccprintf("%s: checksum mismatch!\n", __func__);
		return NULL;
	}
 ...
 *
 * @retrun zero on success, -1 if both areas not found.
 */
...
static uint32_t find_fmap(struct fmap_header *fmh)
{
	...
	} while (step >= LOWEST_FMAP_ALIGNMENT);

	if (offset == MAX_SUPPORTED_FLASH_SIZE) {
	...
	if (read_ap_spi(
		    ...
		    sizeof(*fmh) - offsetof(struct fmap_header, fmap_ver_major),
		    __LINE__))
		return 0;
 ...
 */
static int read_gscvd(uint32_t fmap_offset, struct gvd_container *gvdc)
{
...
static int read_keyblock(struct kb_container *kbc)
{
	...
	if ((kb.header_version_major != KEYBLOCK_MAJOR_VERSION) ||
	    ...
	    ((offsetof(struct vb2_keyblock, keyblock_signature) +
	      kbc->kb->keyblock_signature.sig_offset +
	      kbc->kb->keyblock_signature.sig_size) > size) ||
	    ((offsetof(struct vb2_keyblock, keyblock_hash) +
	      kbc->kb->keyblock_hash.sig_offset +
	      kbc->kb->keyblock_hash.sig_size) > size) ||
	    ((offsetof(struct vb2_keyblock, data_key) +
	      kbc->kb->data_key.key_offset + kbc->kb->data_key.key_size) >
	     size)) {
...
static int read_rootk(struct rootk_container *rootkc)
{
	...
	if (memcmp(SHA256_final(&ctx), root_key_hash, sizeof(root_key_hash))) {
...
/**
 * Read ranges as defined in gsc_verification_data structure and verify them.
 *
 * The expected SHA256 hash value of the ranges is included in the passed in
 * gsc_verification_data.
 *
 * @param gvdc pointer to the gsc_verifcation_data container
 * @param ranges pointer to the aray of ro_ranges structure to fill up
 * @param fmap pointer to fmap area header for fmap, if passed in - verify
 *             that it is covered by one of the ranges
 *
 * @return zero on success, non zero on failure.
 */
static int verify_ranges(const struct gvd_container *gvdc,
			 struct ro_ranges *ranges,
			 struct fmap_area_header *fmap)
{
	/* Read the ranges from flash. */
	...
	/* Read the ranges from AP SPI flash. */
	if (read_ap_spi(ranges->ranges, gvdc->offset + sizeof(gvdc->gvd),
			sizeof(struct ro_range) * range_count, __LINE__))
		return EC_ERROR_HW_INTERNAL;

	if (fmap) {
		/* Make sure FMAP fits into one of the ranges. */
		for (range = ranges->ranges, i = 0; i < range_count; i++) {
			if ((fmap->area_offset >= range->flash_offset) &&
			    (fmap->area_offset + fmap->area_size) <=
				    (range->flash_offset + range->range_size))
				break;
			range++;
		}
		if (i == range_count) {
			CPRINTS("%s: FMAP not covered by GVD", __func__);
			return -1;
		}
	}
	return validate_ranges_sha(ranges->ranges, range_count,
...
static int save_gvd_hash(struct gvd_container *gvdc, struct ro_ranges *ranges)
{
	...
	ro_check.header.type = GVD_HASH_TYPE;
	 /*
			   ...
			   const struct gvd_descriptor *descriptor)
{
	...
	return memcmp(SHA256_final(&ctx), descriptor->digest,
 ...
 *
 * If a NULL descriptor value is passed, the function looks for
 * gsc_verification_data structure through FMAP, and then verifies
 * cryptographically the validity of the structures, starting with the hash of
 * the root key, then signature of the key block, and then signature of
 * gsc_verification_data.
 *
 * This function calls others which require huge amounts of stack, this is why
 * this function must run on TPM task context.
 *
 * @param descriptor if non NULL points to locally cached hash of
 *		     gsc_verification_data.
 *
 * @return zero on success, non zero on failure.
 */
static int8_t ap_ro_check_version2(const struct gvd_descriptor *descriptor)
{
	...
	if (!descriptor) {
		...
		CPRINTS("%s: found v2 descriptor", __func__);
		fmap_offset = descriptor->fmap_offset;
		gvdc.offset = descriptor->gvd_offset;
	}

	if (read_gscvd(fmap_offset, &gvdc) != EC_SUCCESS)
		return -1;

	/* Check fmap inclusion only if this is a fresh read. */
	if (verify_ranges(&gvdc, &ranges, descriptor ? NULL : &fmap))
		return -1;

	/* See if what we read matches the cached hash. */
	if (descriptor) {
	...
	if (verify_keblock(&kbc, &pubk))
		goto exit;

	shared_mem_release(rootkc.rootk);
	rootkc.rootk = NULL;

	rv = verify_gvd_signature(&gvdc.gvd, &ranges, &kbc.kb->data_key, &sigc);
...
/*
 * Pseudo vendor command to invoke AP RO verification on  TPM task context.
 */
					       ...
					       size_t *response_size)
{
	...
	rv = ap_ro_check_version2(descriptor);
	if (rv && descriptor) {
		/* There could have been a legitimate RO change. */
		rv = ap_ro_check_version2(NULL);
	}
	*response_size = 1;
	*((int8_t *)buf) = rv;
...
int validate_ap_ro(void)
{
	...
	if ((support_status == ARCVE_NOT_PROGRAMMED) ||
		...
		pack.tpmh.size = htobe32(sizeof(pack)); /* TPM_ST_NO_SESSIONS */
		...
		rv = pack.rv ? EC_ERROR_CRC : EC_SUCCESS;
	} else {
...
static enum vendor_cmd_rc vc_get_ap_ro_hash(enum vendor_cmd_cc code, void *buf,
					    size_t input_size,
					    size_t *response_size)
{
	int rv;
