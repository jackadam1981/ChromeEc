...
static enum vendor_cmd_rc vc_ap_ro_check(enum vendor_cmd_cc code, void *buf,
					 size_t input_size,
...
DECLARE_VENDOR_COMMAND(VENDOR_CC_SEED_AP_RO_CHECK, vc_ap_ro_check);
...
int validate_ap_ro(void)
{
	...
	for (i = 0; i < p_chk->header.num_ranges; i++) {
		 ...
		 * This also provides enough time for the SPI bus to
		 * stabilize (b/154966209).
		 */
		cflush();
...
static int ap_ro_check_cmd(int argc, char **argv)
{
	int rv;
	int i;
#ifdef CR50_DEV
...
DECLARE_SAFE_CONSOLE_COMMAND(ap_ro_check, ap_ro_check_cmd,
#ifdef CR50_DEV
