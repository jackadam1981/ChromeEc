...
/**
 * Check if an element fits under a certain address.
 *
 * @param top_addr  type casted address to check against
 * @param el  address of the element, guaranteed be below top_addr
 * @param data_offset  element's data base offset from the element address
 * @param data_size  elemet's data size
 *
 * @return true if the element fits, false otherwise
 */
static bool element_fits(uintptr_t top_addr,
			 const void *el,
			 uint32_t data_offset,
			 uint32_t data_size)
{
	uintptr_t headroom = top_addr - (uintptr_t)el;

	return ((data_offset < headroom) &&
