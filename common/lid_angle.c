...
void lidangle_update(int lid_ang)
{
	...
	/*
	 * Any time the chipset is off, manage whether or not peripherals
	 * are enabled based on lid angle history.
	 */
	if (!chipset_in_state(CHIPSET_STATE_ON)) {
		for (i = 0; i < LID_ANGLE_BUFFER_SIZE; i++) {
			 ...
			 */
			if (!lid_in_range_to_enable_peripherals(
						lidangle_buffer[i]))
				accept = 0;
			if (!lid_in_range_to_ignore_peripherals(
						lidangle_buffer[i]))
				ignore = 0;
		}
