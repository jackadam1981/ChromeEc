 ...
 */
static int kb_disable_large_angle = 180;
static const int kb_disable_small_angle = 13;

/* Define hysteresis value to add stability to the keyboard scanning flag. */
#define KB_DIS_HYSTERESIS_DEG  2

/* Define max and min values for kb_disable_large_angle. */
...
{
	/*
	 * If the keyboard disable large angle is min or max, then this
	 * function should return false or true respectively, independent of
	 * input angle.
	 */
	if (kb_disable_large_angle == KB_DIS_MIN_LARGE_ANGLE)
		return 0;
	else if (kb_disable_large_angle == KB_DIS_MAX_LARGE_ANGLE)
		return 1;

	return  (ang >= (kb_disable_small_angle + KB_DIS_HYSTERESIS_DEG)) &&
		(ang <= (kb_disable_large_angle - KB_DIS_HYSTERESIS_DEG));
}
