...
void lidangle_keyscan_update(float lid_ang)
{
	...
	if (!chipset_in_state(CHIPSET_STATE_ON)) {
		...
		kb_disabled = keyboard_scan_is_disabled() &
				KB_SCAN_DISABLE_LID_ANGLE;
		if (keys_accept && kb_disabled) {
			CPRINTF("[%T Laptop mode, lid ang at %d]\n",
					(int)lidangle_buffer[index]);
			keyboard_scan_enable(1, KB_SCAN_DISABLE_LID_ANGLE);
		} else if (keys_ignore && !keys_accept && !kb_disabled) {
			CPRINTF("[%T Tablet mode, lid ang at %d]\n",
					(int)lidangle_buffer[index]);
