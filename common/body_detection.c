...
/*
 * The console command can force the value of motion_state. If
 * body_detect_set_spoof is true, all other (sensor) updates to motion_state are
 * ignored.
 */
static bool body_detect_set_spoof;
...
void body_detect_change_state(enum body_detect_states state, bool spoof)
{
	if (IS_ENABLED(CONFIG_ACCEL_SPOOF_MODE) && spoof_enable && !spoof)
		return;
	if (body_detect_set_spoof && !spoof)
		return;
