...
{
	/* When spoof mode is enabled, return if not using spoofing flag. */
	...
	if (IS_ENABLED(CONFIG_GESTURE_HOST_DETECTION)) {
		...
		if (flags & BODY_DETECTION_WAKEUP)
...
void body_detect_set_spoof(int enable)
{
	...
	if (!enable)
		body_detect_change_state(motion_state, BODY_DETECTION_WAKEUP);
}
