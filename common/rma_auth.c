...
/* Server key.  TODO: REPLACE WITH ACTUAL KEY */
static const uint8_t server_pub_key[32] = RMA_TEST_SERVER_PUBLIC_KEY;
static const uint8_t server_key_id = RMA_TEST_SERVER_KEY_ID;
...
int rma_try_authcode(const char *code)
{
	...
	if (!strncmp(authcode, code, RMA_AUTHCODE_CHARS)) {
		rv = EC_SUCCESS;
		tries_left = 0;
	} else {
		tries_left--;
