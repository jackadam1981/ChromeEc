...
#if defined(TEST_BUILD) && !defined(CC_EXTENSION)
/* Keel the test mode build happy. */
...
/*
 * Trigger generating of the new challenge/authcode pair. If successfull,
 * store the challenge in the vendor command response buffer and send it to
 * the sender. If not successful - return the error value to the sender.
 */
...
/*
 * Handle the VENDOR_CC_RMA_CHALLENGE_RESPONSE command. When receiveid with
...
DECLARE_VENDOR_COMMAND(VENDOR_CC_RMA_CHALLENGE_RESPONSE, rma_challenge_response);
