 ...
 * memory available in the system. It then gets fragmented/defragmented
 * based on actuall allocations/releases.
 */
TEST_GLOBAL struct shm_buffer *free_buf_chain;

/* At the begiining there is no allocated buffers */
...
static int do_acquire(int size, struct shm_buffer **dest_ptr)
{
	...
	/*
	 * To keep things simple let's align the size and make sure there is
	 * enough room to fit the buffer header.
	 */
	if (size < sizeof(struct shm_buffer))
