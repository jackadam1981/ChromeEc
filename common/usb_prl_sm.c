...
static enum l_state local_state = PRL_INIT;
...
static unsigned int prl_tx_wait_for_message_request_exit(int port);
...
static unsigned int prl_tx_layer_reset_for_transmit_exit(int port);
...
static unsigned int prl_tx_src_source_tx_exit(int port);
...
static unsigned int prl_tx_snk_start_ams_exit(int port);
...
static unsigned int prl_tx_src_pending_exit(int port);
...
static unsigned int prl_tx_snk_pending_exit(int port);
...
static unsigned int prl_tx_discard_message_exit(int port);
...
static unsigned int prl_hr_wait_for_request_exit(int port);
...
static unsigned int prl_hr_reset_layer_exit(int port);
...
static unsigned int prl_hr_wait_for_phy_hard_reset_complete_exit(int port);
...
static unsigned int rch_wait_for_message_from_protocol_layer_exit(int port);
...
static unsigned int rch_processing_extended_message_exit(int port);
...
static unsigned int rch_requesting_chunk_exit(int port);
...
static unsigned int rch_waiting_chunk_exit(int port);
...
static unsigned int rch_report_error_exit(int port);
...
static unsigned int tch_wait_for_message_request_from_pe_exit(int port);
...
static unsigned int tch_wait_for_transmission_complete_exit(int port);
...
static unsigned int tch_construct_chunked_message_exit(int port);
...
static unsigned int tch_sending_chunked_message_exit(int port);
...
static unsigned int tch_wait_chunk_request_exit(int port);
...
static unsigned int tch_message_received_exit(int port);
...
static const state_sig prl_tx_wait_for_message_request_sig[] = {
	...
	prl_tx_wait_for_message_request_exit,
	get_super_state
};
...
static const state_sig prl_tx_layer_reset_for_transmit_sig[] = {
	...
	prl_tx_layer_reset_for_transmit_exit,
	get_super_state
};
...
static const state_sig rch_wait_for_message_from_protocol_layer_sig[] = {
	...
	rch_wait_for_message_from_protocol_layer_exit,
	get_super_state
};
...
static const state_sig rch_processing_extended_message_sig[] = {
	...
	rch_processing_extended_message_exit,
	get_super_state
};
...
static const state_sig tch_wait_for_message_request_from_pe_sig[] = {
	...
	tch_wait_for_message_request_from_pe_exit,
	get_super_state
};
...
static const state_sig tch_wait_for_transmission_complete_sig[] = {
	...
	tch_wait_for_transmission_complete_exit,
	get_super_state
};
...
static const state_sig tch_construct_chunked_message_sig[] = {
	...
	tch_construct_chunked_message_exit,
	get_super_state
};
...
static const state_sig tch_sending_chunked_message_sig[] = {
	...
	tch_sending_chunked_message_exit,
	get_super_state
};
...
#if defined(CONFIG_ENABLE_USB_PRL_TESTING)
int get_rch_state_id(int port)
{
	return get_state_id(SM_OBJ(rch[port].obj));
}

int get_tch_state_id(int port)
{
	return get_state_id(SM_OBJ(tch[port].obj));
}

int get_prl_tx_state_id(int port)
{
	return get_state_id(SM_OBJ(prl_tx[port].obj));
}

int get_prl_hr_state_id(int port)
{
	return get_state_id(SM_OBJ(prl_hr[port].obj));
}
#endif
...
void protocol_layer(int port, int evt, int en)
{
	...
	case PRL_INIT:
		...
		/* Enable RX */
#if defined(CONFIG_USB_TYPEC_VPD_CT) || defined(CONFIG_USB_TYPEC_VPD)
		vpd_rx_enable(1);
#endif
...
static unsigned int prl_tx_wait_for_message_request_entry(int port)
{
	SET_TEST(PRL_TX_TEST_OBJ(port), PRL_TX_WAIT_FOR_MESSAGE_REQUEST);
...
static unsigned int prl_tx_wait_for_message_request_run(int port)
{
	if (prl_tx[port].sm_flags & SM_FLAGS_MSG_XMIT) {
		...
		(prl_tx[port].sm_flags &
				(SM_FLAGS_START_AMS | SM_FLAGS_END_AMS))) {
		if (tc_get_power_role(port) == PD_ROLE_SOURCE) {
			 ...
			 * Policy Engine
			 */
			else if (prl_tx[port].sm_flags & SM_FLAGS_END_AMS) {
				prl_tx[port].sm_flags &= ~SM_FLAGS_END_AMS;
				/* Set Rp = SinkTxOk */
...
static unsigned int prl_tx_discard_message_entry(int port)
{
	SET_TEST(PRL_TX_TEST_OBJ(port), PRL_TX_DISCARD_MESSAGE);

	/* Increment msgidCounter */
	increment_msgid_counter(port);
	set_state(port, PRL_TX_OBJ(port), prl_tx_wait_for_message_request);
...
static unsigned int prl_tx_src_source_tx_entry(int port)
{
	SET_TEST(PRL_TX_TEST_OBJ(port), PRL_TX_SRC_SOURCE_TX);
...
static unsigned int prl_tx_snk_start_ams_entry(int port)
{
	SET_TEST(PRL_TX_TEST_OBJ(port), PRL_TX_SNK_START_OF_AMS);
	return 0;
}
...
static unsigned int prl_tx_layer_reset_for_transmit_entry(int port)
{
	SET_TEST(PRL_TX_TEST_OBJ(port), PRL_TX_LAYER_RESET_FOR_TRANSMIT);
...
static unsigned int prl_tx_wait_for_phy_response_entry(int port)
{
	SET_TEST(PRL_TX_TEST_OBJ(port), PRL_TX_WAIT_FOR_PHY_RESPONSE);
...
static unsigned int prl_tx_wait_for_phy_response_run(int port)
{
	...
	if (prl_tx[port].xmit_status == TCPC_TX_COMPLETE_SUCCESS) {
		...
		pdmsg[port].status_flags |= SM_FLAGS_TX_COMPLETE;
		set_state(port, PRL_TX_OBJ(port),
			prl_tx_wait_for_message_request);
		return 0;
	}
...
static unsigned int prl_tx_src_pending_entry(int port)
{
	SET_TEST(PRL_TX_TEST_OBJ(port), PRL_TX_SRC_PENDING);
...
static unsigned int prl_tx_snk_pending_entry(int port)
{
	SET_TEST(PRL_TX_TEST_OBJ(port), PRL_TX_SNK_PENDING);
	return 0;
}
...
static unsigned int prl_hr_wait_for_request_entry(int port)
{
	SET_TEST(PRL_HR_TEST_OBJ(port), PRL_HR_WAIT_FOR_REQUEST);
...
static unsigned int prl_hr_reset_layer_entry(int port)
{
	SET_TEST(PRL_HR_TEST_OBJ(port), PRL_HR_RESET_LAYER);
...
static unsigned int prl_hr_wait_for_phy_hard_reset_complete_run(int port)
{
	 ...
	 * or timeout
	 */
	if ((pdmsg[port].status_flags & SM_FLAGS_TX_COMPLETE) ||
...
static unsigned int prl_hr_wait_for_pe_hard_reset_complete_entry(int port)
{
	SET_TEST(PRL_HR_TEST_OBJ(port), PRL_HR_WAIT_FOR_PE_HARD_RESET_COMPLETE);
	return 0;
}
...
{
	/* Clear Abort flag */
	pdmsg[port].status_flags &= ~SM_FLAGS_ABORT;
...
static unsigned int rch_wait_for_message_from_protocol_layer_entry(int port)
{
	SET_TEST(RCH_TEST_OBJ(port), RCH_WAIT_FOR_MESSAGE_FROM_PROTOCOL_LAYER);
...
static unsigned int rch_processing_extended_message_entry(int port)
{
	...
	SET_TEST(RCH_TEST_OBJ(port), RCH_PROCESSING_EXTENDED_MESSAGE);
...
static unsigned int rch_processing_extended_message_run(int port)
{
	...
	/*
	 * Convert the number of 32-bit data objects to number of
	 * data bytes and remove size of 16-bit Extended Message Header
	 * by subtracting 2-bytes.
	 */
	 ...
	 * Abort Flag Set
	 */
	if (pdmsg[port].status_flags & SM_FLAGS_ABORT) {
		set_state(port, RCH_OBJ(port),
			rch_wait_for_message_from_protocol_layer);
	}
	/*
	 ...
	 */
		...
		if (pdmsg[port].num_bytes_received + byte_num > 260) {
...
static unsigned int rch_requesting_chunk_entry(int port)
{
	SET_TEST(RCH_TEST_OBJ(port), RCH_REQUESTING_CHUNK);
...
static unsigned int rch_requesting_chunk_run(int port)
{
	 ...
	 */
	if (rch[port].sm_flags & SM_FLAGS_MSG_RECEIVED ||
			pdmsg[port].status_flags & SM_FLAGS_TX_ERROR) {
		/*
	 ...
	 */
	else if (pdmsg[port].status_flags & SM_FLAGS_TX_COMPLETE) {
		pdmsg[port].status_flags &= ~SM_FLAGS_TX_COMPLETE;
...
static unsigned int rch_waiting_chunk_entry(int port)
{
	SET_TEST(RCH_TEST_OBJ(port), RCH_WAITING_CHUNK);
...
static unsigned int rch_waiting_chunk_run(int port)
{
	if ((rch[port].sm_flags & SM_FLAGS_MSG_RECEIVED)) {
		/*
		 * Leave SM_FLAGS_MSG_RECEIVED flag set just in case an error is
		 * detected. If an error is detected, SM_FLAGS_MSG_RECEIVED will
		 * be cleared in rch_report_error state.
		 */
...
static unsigned int rch_report_error_entry(int port)
{
	SET_TEST(RCH_TEST_OBJ(port), RCH_REPORT_ERROR);
...
static unsigned int tch_wait_for_message_request_from_pe_entry(int port)
{
	SET_TEST(TCH_TEST_OBJ(port), TCH_WAIT_FOR_MESSAGE_REQUEST_FROM_PE);
	...
	if (tch[port].sm_flags & SM_FLAGS_MSG_RECEIVED) {
		...
		if (rch[port].obj.task_state !=
			 ...
			 */
			{
				...
				memset((uint8_t *)pdmsg[port].chk_buf, 0, 28);
...
static unsigned int tch_wait_for_transmission_complete_entry(int port)
{
	SET_TEST(TCH_TEST_OBJ(port), TCH_WAIT_FOR_TRANSMISSION_COMPLETE);
	return 0;
}
...
static unsigned int tch_wait_for_transmission_complete_run(int port)
{
	 ...
	 */
	else if (pdmsg[port].status_flags & SM_FLAGS_TX_ERROR) {
		pdmsg[port].status_flags &= ~SM_FLAGS_TX_ERROR;
...
static unsigned int tch_construct_chunked_message_entry(int port)
{
	...
	SET_TEST(TCH_TEST_OBJ(port), TCH_CONSTRUCT_CHUNKED_MESSAGE);
...
static unsigned int tch_sending_chunked_message_entry(int port)
{
	SET_TEST(TCH_TEST_OBJ(port), TCH_SENDING_CHUNKED_MESSAGE);
	return 0;
}
...
static unsigned int tch_sending_chunked_message_run(int port)
{
	 ...
	 * Transmission Error
	 */
	if (pdmsg[port].status_flags & SM_FLAGS_TX_ERROR) {
		pe_report_error(port, ERR_TCH_XMIT);
		set_state(port, TCH_OBJ(port),
			tch_wait_for_message_request_from_pe);
	}
...
static unsigned int tch_wait_chunk_request_entry(int port)
{
	SET_TEST(TCH_TEST_OBJ(port), TCH_WAIT_CHUNK_REQUEST);
...
static unsigned int tch_message_received_entry(int port)
{
	SET_TEST(TCH_TEST_OBJ(port), TCH_MESSAGE_RECEIVED);
