...
#define PRL_FLAGS_TX_COMPLETE             (1 << 0)
#define PRL_FLAGS_START_AMS               (1 << 1)
#define PRL_FLAGS_END_AMS                 (1 << 2)
#define PRL_FLAGS_TX_ERROR                (1 << 3)
#define PRL_FLAGS_PE_HARD_RESET           (1 << 4)
#define PRL_FLAGS_HARD_RESET_COMPLETE     (1 << 5)
#define PRL_FLAGS_PORT_PARTNER_HARD_RESET (1 << 6)
#define PRL_FLAGS_MSG_XMIT                (1 << 7)
#define PRL_FLAGS_MSG_RECEIVED            (1 << 8)
#define PRL_FLAGS_ABORT                   (1 << 9)
#define PRL_FLAGS_CHUNKING                (1 << 10)
...
static struct protocol_layer_rx {
	/* message ids for all 6 port partners */
	int msg_id[6];
...
static unsigned int prl_tx_phy_layer_reset_exit(int port);
...
{
	if (tch[port].obj.task_state != tch_wait_for_message_request_from_pe)
...
static unsigned int prl_tx_phy_layer_reset_entry(int port)
{
	...
	/* Reset PHY Layer */
	/*
	 * TODO(shurst): figure out why calling tcpm_init from
	 * here prevents rx/tx after call returns.
	 */
	return 0;
}
...
static void prl_tx_construct_message(int port)
{
	...
	/* Save SOP* so msg_id_counter can be incremented */
