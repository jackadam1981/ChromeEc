...
static uint32_t aps_flags[CONFIG_USB_PD_PORT_MAX_COUNT];

#define CPRINTF(format, args...) cprintf(CC_USBPD, "APS " format, ##args)
#define CPRINTS(format, args...) cprints(CC_USBPD, "APS " format, ##args)

__overridable struct aps_config_t aps_config = {
...
{
	atomic_or(&aps_flags[port], flags);
}

void aps_clr_flags(int port, uint32_t flags)
{
	atomic_clear_bits(&aps_flags[port], flags);
}
...
static void aps_init(void)
{
	...
	timeout.val = get_time().val + aps_config.t_check;
	for (int i = 0; i < CONFIG_USB_PD_PORT_MAX_COUNT; ++i)
		aps_clr_flags(i, APS_FLAG_ALL);
}

/*
 * APS reset.
 */
static void aps_reset(void)
{
	aps_init();
}
...
{
	return val >= (limit * aps_config.k_more_pwr / 100);
}

bool is_more_efficient(int curr_mv, int prev_mv, int batt_mv, int batt_mw,
		       int input_mw)
{
	if (aps_config.is_more_efficient)
		return aps_config.is_more_efficient(curr_mv, prev_mv, batt_mv,
						    batt_mw, input_mw);
	...
	for (int i = 0; i < CONFIG_USB_PD_PORT_MAX_COUNT; ++i) {
		...
		for (int j = 0; j < pd_get_src_cap_cnt(i); ++j) {
			...
			if (near_pwr_limit) {
				...
				int adjust_pwr =
					mw * aps_config.k_less_pwr / 100;
				int adjust_cand_mw =
					cand->mw * aps_config.k_less_pwr / 100;
...
{
	if (aps_flags[port] & APS_FLAG_NEW_CONTRACT)
		timeout.val = get_time().val + aps_config.t_stable;

	aps_clr_flags(port, APS_FLAG_NEW_CONTRACT);
}

void aps_task(void *u)
{
	...
	while (1) {
		...
		if (sample_count == aps_config.k_sample) {
		...
		timeout.val = now.val + aps_config.t_check;
	}
}
...
static int command_apdo(int argc, char **argv)
{
	...
	if (argc == 1) {
		...
		ccprintf("k_more=%d k_less=%d k_sample=%d\n",
			 aps_config.k_more_pwr, aps_config.k_less_pwr,
			 aps_config.k_sample);
		ccprintf("t_stable=%d t_check=%d\n",
			 aps_config.t_stable / SECOND,
			 aps_config.t_check / SECOND);
		if (!is_enabled) {
			ccprintf("APS Disabled\n");
			return EC_SUCCESS;
		}
	...
	if (!strcasecmp(argv[1], "debug")) {
		...
		if (holder > 100 && holder < 0 &&
		    holder < aps_config.k_less_pwr)
			return EC_ERROR_INVAL;

		aps_config.k_more_pwr = holder;
	} else if (!strcasecmp(argv[1], "setkless")) {
		holder = atoi(argv[2]);
		if (!holder)
			return EC_ERROR_PARAM2;
		if (holder > 100 && holder < 0 &&
		    holder > aps_config.k_more_pwr)
			return EC_ERROR_INVAL;

		aps_config.k_less_pwr = holder;
		...
		if (holder < 0)
			return EC_ERROR_INVAL;
		aps_config.t_check = holder * SECOND;
		...
		aps_config.t_stable = holder * SECOND;
...
DECLARE_CONSOLE_COMMAND(aps, command_apdo,
			"debug|enable|disable|set(kmore|kless|ksample|tstable|"
			"tcheck) <int>",
			"Print/set aps state.");
