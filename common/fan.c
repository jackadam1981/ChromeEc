				 ...
				 int temp_ratio, void (*on_change)(void))
{
	...
	if (temperature <= fan_table[0].decreasing_temp_threshold) {
		 ...
		 */
		int i = fan_table_index;

		if (temperature < previous_temperature) {
			while (i > 0 &&
			       temperature <=
				       fan_table[i].decreasing_temp_threshold) {
				i--;
				fan_table_index = i;
			}
			...
			while (i < num_fan_levels &&
			       temperature >=
				       fan_table[i].increasing_temp_threshold) {
				fan_table_index = i;
				i++;
				fan_stopped = false;
			}
	...
	if (rpm != fan_get_rpm_target(FAN_CH(fan_index))) {
