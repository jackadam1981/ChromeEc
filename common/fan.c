				 ...
				 int temp_ratio, void (*on_change)(void))
{
	...
	const int temperature =
		temp_fan_off + (temp_fan_max - temp_fan_off) * temp_ratio / 100;
	const int previous_rpm = fan_get_rpm_target(FAN_CH(fan_index));
	int rpm;

	if (temperature <= fan_table[0].decreasing_temp_threshold) {
		rpm = 0;
	} else if (previous_rpm == 0 &&
		   temperature < fan_table[0].increasing_temp_threshold) {
		rpm = 0;
	} else {
		/*
		 * Comparing temperature and previous temperature, trichotomy:
		...
		if (temperature < previous_temperature) {
			fan_table_index = num_fan_levels - 1;
			while (fan_table_index > 0 &&
			       temperature <=
				       fan_table[fan_table_index]
					       .decreasing_temp_threshold) {
				fan_table_index--;
			}
			rpm = fan_table[fan_table_index].rpm;
		} else if (temperature > previous_temperature) {
			fan_table_index = 0;
			while (fan_table_index < num_fan_levels &&
			       temperature >=
				       fan_table[fan_table_index]
					       .increasing_temp_threshold) {
				fan_table_index++;
			}
