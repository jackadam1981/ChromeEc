...
struct {
	...
	int t[4];
	int idx;
} tmp006_data[TMP006_COUNT];

static int tmp006_read_die_temp(int idx)
{
	int pidx = (tmp006_data[idx].idx - 1) & 0x3;
...
static int tmp006_poll_sensor(int sensor_id)
{
	...
	idx = tmp006_data[sensor_id].idx;
	tmp006_data[sensor_id].t[idx] = t;
	tmp006_data[sensor_id].v = v;
	tmp006_data[sensor_id].idx = (idx + 1) & 3;
...
{
	if (param == TMP006_DIE)
		return tmp006_read_die_temp(idx);
	else
		return tmp006_read_object_temp(idx);
}
