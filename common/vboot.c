...
#define RW_A_OFF		CONFIG_RW_STORAGE_OFF
#define RW_B_OFF		(CONFIG_RW_STORAGE_OFF + CONFIG_RW_SIZE)
#define RW_A_SIGN_OFF		(RW_A_OFF + CONFIG_RW_SIZE - CONFIG_RW_SIG_SIZE)
#define RW_B_SIGN_OFF		(RW_B_OFF + CONFIG_RW_SIZE - CONFIG_RW_SIG_SIZE)
...
static int verify_slot(int slot)
{
	...
	if (slot == VBOOT_EC_SLOT_A) {
		data = (const uint8_t *)(
				CONFIG_EC_WRITABLE_STORAGE_OFF + RW_A_OFF);
		vb21_sig = (const struct vb21_signature *)(
				CONFIG_EC_WRITABLE_STORAGE_OFF + RW_A_SIGN_OFF);
	} else {
		data = (const uint8_t *)(
				CONFIG_EC_WRITABLE_STORAGE_OFF + RW_B_OFF);
		vb21_sig = (const struct vb21_signature *)(
				CONFIG_EC_WRITABLE_STORAGE_OFF + RW_B_SIGN_OFF);
	}
	...
	/* Verify the data */
	if (rsa_verify(key, sig, hash, workbuf)) {
