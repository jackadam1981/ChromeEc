...
static enum howgood GoodImage(uint8_t *key_data,
...
int vboot_init(void)
{
	...
	ts1 = get_time();
	r = GoodImage((uint8_t *)CONFIG_VBOOT_ROOTKEY_OFF,
		      (uint8_t *)CONFIG_VBLOCK_A_OFF, CONFIG_VBLOCK_A_SIZE,
		      (uint8_t *)CONFIG_FW_A_OFF, CONFIG_FW_A_SIZE);
	ts2 = get_time();

	CPRINTF("[result=%d, elapsed time=%d,%d]\n", r,
		    ts2.le.hi - ts1.le.hi, ts2.le.lo - ts1.le.lo);
