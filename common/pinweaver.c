						     ...
						     size_t *response_size)
{
	...
	if (code != VENDOR_CC_PINWEAVER) {
		ccprintf("PinWeaver: command code mismatch 0x%x != 0x%x.\n",
			 code, VENDOR_CC_PINWEAVER);
	...
	if (*response_size < PW_MAX_RESPONSE_SIZE) {
		ccprintf("PinWeaver: insufficient buffer for the response.\n");
		return VENDOR_RC_INTERNAL_ERROR;
	}

	/* The return code is not needed because at this point the response
	 * needs to be transmitted regardless of the return code.
	 */
