...
static int derive_keys(struct merkle_tree_t *merkle_tree)
{
	...
	memcpy(input.v, merkle_tree->key_derivation_nonce,
...
{
	uint32_t uld_len = head->pub_len + head->sec_len;
	...
	if (head->leaf_version.minor == PW_LEAF_MINOR_VERSION) {
		if (uld_len != PW_RESP_SIZE_DISCREPANCY)
			return PW_ERR_LENGTH_INVALID;
	} else if (uld_len < PW_RESP_SIZE_DISCREPANCY)
		return PW_ERR_LENGTH_INVALID;
				 ...
				 uint8_t new_root[PW_HASH_SIZE])
{
	...
	*response_size = sizeof(*response) + PW_RESP_SIZE_DISCREPANCY;
			      ...
			      uint8_t new_root[PW_HASH_SIZE])
{
	...
	if (ret != EC_SUCCESS) {
		*data_length = sizeof(*response) + PW_RESP_SIZE_DISCREPANCY;
		      ...
		      struct pw_response_t *response)
{
	...
	resp_length = 0;
	/* Initialize root to the current root. */
