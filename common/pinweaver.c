...
static void compute_hmac(const struct merkle_tree_t *merkle_tree,
			 ...
			 const void *cipher_text, size_t cipher_text_len,
			 uint8_t result[PW_HASH_SIZE])
{
	...
	HASH_update(&hmac.hash, pub, pub_len);
	HASH_update(&hmac.hash, cipher_text, cipher_text_len);
		...
		uint8_t new_root[PW_HASH_SIZE])
{
	...
	compute_hmac(merkle_tree, &wrapped_leaf_data->pub,
		...
		struct leaf_data_t *leaf_data)
{
	...
	ret = validate_request_with_path(merkle_tree,
					 wrapped_leaf_data->pub.label, hashes,
					 wrapped_leaf_data->hmac);
	if (ret != EC_SUCCESS)
		return ret;
	...
	compute_hmac(merkle_tree, &wrapped_leaf_data->pub,
		     sizeof(wrapped_leaf_data->pub),
		     &wrapped_leaf_data->cipher_text,
		     sizeof(wrapped_leaf_data->cipher_text), hmac);
	 ...
	 */
	if (safe_memcmp(hmac, wrapped_leaf_data->hmac, sizeof(hmac)))
		return PW_ERR_HMAC_AUTH_FAILED;

	return decrypt_leaf_data(merkle_tree, wrapped_leaf_data, leaf_data);
}
			      ...
			      uint16_t *data_length)
{
	...
	volatile struct {
		...
		uint16_t data_length;
			...
			{ 0, PW_ERR_LOWENT_AUTH_FAILED, zeros,
					sizeof(response->wrapped_leaf_data) },
			{ 0, EC_SUCCESS, leaf_data.sec.high_entropy_secret,
					sizeof(*response)},
	};

	ret = validate_request_with_wrapped_leaf(
			merkle_tree, &request->wrapped_leaf_data,
			request->path_hashes, &leaf_data);
	if (ret != EC_SUCCESS)
		return ret;
	...
	ret = handle_leaf_update(merkle_tree, &leaf_data, request->path_hashes,
				 NULL, &wrapped_leaf_data, new_root);
	if (ret != EC_SUCCESS)
		return ret;
	...
	*data_length = results_table[auth_result].data_length;
				...
				struct pw_response_log_replay_t *response)
{
	...
	compute_hmac(merkle_tree, &request->wrapped_leaf_data.pub,
		     sizeof(request->wrapped_leaf_data.pub),
		     &request->wrapped_leaf_data.cipher_text,
		     sizeof(request->wrapped_leaf_data.cipher_text), hmac);
	...
	ret = handle_leaf_update(merkle_tree, &leaf_data, request->path_hashes,
				 log.entries[x].root, &wrapped_leaf_data,
				 root);
	if (ret != EC_SUCCESS)
		return ret;
		      ...
		      struct pw_response_t *response)
{
	...
	case PW_INSERT_LEAF:
		...
		ret = pw_handle_insert_leaf(merkle_tree,
					    ...
					    &response->data.insert_leaf, root);
		if (ret == EC_SUCCESS)
			data_length = sizeof(response->data.insert_leaf);
		break;
	case PW_REMOVE_LEAF:
		if (request->header.data_length !=
	...
	case PW_RESET_AUTH:
		...
		ret = pw_handle_reset_auth(merkle_tree,
					   ...
					   &response->data.reset_auth, root);
		if (ret == EC_SUCCESS)
			data_length = sizeof(response->data.reset_auth);
		break;
	case PW_GET_LOG:
		if (request->header.data_length !=
		...
		ret = pw_handle_get_log(merkle_tree, &request->data.get_log,
					(void *)&response->data, &data_length);
		break;
	case PW_LOG_REPLAY:
		if (request->header.data_length !=
		...
		if (ret == EC_SUCCESS)
			data_length = sizeof(response->data.log_replay);
		break;
	default:
...
cleanup:
	...
	response->header.data_length = data_length;
