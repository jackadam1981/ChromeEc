			      ...
			      uint16_t *data_length)
{
	...
	struct imported_leaf_data_t ptrs;
	struct wrapped_leaf_data_t wrapped_leaf_data;
	struct pw_request_try_auth_t auth_req;
	uint8_t new_root[PW_HASH_SIZE];

	cprints(CC_TASK, "PinWeaver: === seal_to_pcr");
	memcpy(auth_req.low_entropy_secret,
		   request->low_entropy_secret,
		   sizeof(auth_req.low_entropy_secret));
	memcpy(&auth_req.unimported_leaf_data,
		   &request->unimported_leaf_data,
	       sizeof(auth_req.unimported_leaf_data));

	ret = pw_handle_try_auth(
			merkle_tree, &auth_req, req_size, response,
			data_length);
	if (ret != EC_SUCCESS)
		return ret;
