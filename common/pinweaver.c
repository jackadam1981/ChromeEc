...
{
	cprints(CC_TASK, "PinWeaver: === import leaf %d",
		unimported->head.leaf_version.minor);
	imported->head = &unimported->head;
	imported->hmac = unimported->hmac;
	imported->iv = unimported->iv;
	imported->pub = (struct leaf_public_data_t *)unimported->payload;
	 ...
	 *    imported->pub->valid_pcr_criteria[0].bitmask = 0;
	 *  }
	 */
	cprints(CC_TASK, "PinWeaver: === import leaf done, label = %d",
		imported->pub->label.v);
}
		...
		const struct valid_pcr_value_t valid_pcr_criteria[])
{
	...
	for (x = 0; x < PW_MAX_PCR_CRITERIA_COUNT; ++x) {
		...
		for (y = 0; y < digest.digests[0].b.size; ++y) {
			cprints(CC_TASK, "PinWeaver: === checking y = %d", y);
				...
				uint16_t *data_length)
{
	...
	minor_version = unimported_leaf_data->head.leaf_version.minor;
	cprints(CC_TASK, "PinWeaver: === minor1: %d", minor_version);
	...
	if (minor_version == 0) {
		cprints(CC_TASK, "PinWeaver: === setting bitmask to zero");
		// imported_leaf_data.pub->valid_pcr_criteria[0].bitmask = 0;
		// wrapped_leaf_data.pub.valid_pcr_criteria[0].bitmask = 0;
		leaf_data.pub.valid_pcr_criteria[0].bitmask = 0;
	}
			      ...
			      uint16_t *data_length)
{
	...
	cprints(CC_TASK, "PinWeaver: === pub0: %d",
		request->unimported_leaf_data.head.pub_len);

	ret = pw_handle_try_auth(
			merkle_tree, request->low_entropy_secret,
			&request->unimported_leaf_data,
			req_size +
			sizeof(struct pw_request_try_auth_t) -
			sizeof(struct pw_request_seal_to_pcr_t),
			response,
			data_length);
	if (ret != EC_SUCCESS)
		return ret;

	cprints(CC_TASK, "PinWeaver: === auth passed");
	cprints(CC_TASK, "PinWeaver: === pub00: %d",
		request->unimported_leaf_data.head.pub_len);

    /** This is the reverse code from what happens in auth, but it's safer
     *	to do it this way, than to duplicate the code from auth.
     */
	memcpy(&wrapped_leaf_data, &response->unimported_leaf_data,
	       sizeof(wrapped_leaf_data));

    cprints(CC_TASK, "PinWeaver: === pub_len1 : %d",
		wrapped_leaf_data.head.pub_len);
	cprints(CC_TASK, "PinWeaver: === pub01: %d",
		request->unimported_leaf_data.head.pub_len);
	/** Check that there's no PCR criteria set in the leaf. */
	if (wrapped_leaf_data.pub.valid_pcr_criteria[0].bitmask != 0) {
		cprints(CC_TASK,
			"PinWeaver: PCR criteria already set in the leaf, bitmask = %d",
			 wrapped_leaf_data.pub.valid_pcr_criteria[0].bitmask);
		return PW_ERR_PCR_CRITERIA_PRESENT;
	}

	cprints(CC_TASK, "PinWeaver: === criteria presence check passed");
    /** Set the PCR criteria from request and update the tree. */
	memcpy(&wrapped_leaf_data.pub.valid_pcr_criteria,
		   &request->valid_pcr_criteria,
	       sizeof(request->valid_pcr_criteria));

    cprints(CC_TASK, "PinWeaver: === pub_len2 : %d",
			wrapped_leaf_data.head.pub_len);
	cprints(CC_TASK, "PinWeaver: === pub02: %d",
		request->unimported_leaf_data.head.pub_len);

	/** Update the tree after the leaf change */
	import_leaf((const struct unimported_leaf_data_t *)&wrapped_leaf_data,
			&ptrs);

	cprints(CC_TASK, "PinWeaver: === pub_len3 : %d",
			ptrs.head->pub_len);
	cprints(CC_TASK, "PinWeaver: === pub_len4 : %d",
			wrapped_leaf_data.head.pub_len);
	cprints(CC_TASK, "PinWeaver: === pub03: %d",
		request->unimported_leaf_data.head.pub_len);

	compute_hmac(merkle_tree, &ptrs, wrapped_leaf_data.hmac);
	cprints(CC_TASK, "PinWeaver: === hmac computed");
	cprints(CC_TASK, "PinWeaver: === pub_len5 : %d",
			wrapped_leaf_data.head.pub_len);
	cprints(CC_TASK, "PinWeaver: === pub05: %d",
		request->unimported_leaf_data.head.pub_len);

	ret = validate_request_with_wrapped_leaf(
			merkle_tree, req_size - sizeof(*request) + 136,
			&request->unimported_leaf_data, &imported_leaf_data,
			&leaf_data);
	if (ret != EC_SUCCESS)
		return ret;

	cprints(CC_TASK, "PinWeaver: === wrapped leaf validated");
	compute_root_hash(merkle_tree, leaf_data.pub.label,
			  imported_leaf_data.hashes, wrapped_leaf_data.hmac,
			  new_root);
	...
	cprints(CC_TASK, "PinWeaver: === log auth passed");
	memcpy(&response->unimported_leaf_data, &wrapped_leaf_data,
		   sizeof(wrapped_leaf_data));
