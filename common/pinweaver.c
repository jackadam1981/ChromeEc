...
static int pw_handle_insert_leaf(
	...
	const uint8_t path_hashes[][PW_HASH_SIZE],
	...
	uint16_t *response_size)
{
	...
	ret = handle_leaf_update(merkle_tree, &leaf_data, path_hashes,
		      ...
		      struct pw_response_t *response)
{
	...
	case PW_INSERT_LEAF:
		 ...
		 *  TODO(igorcov): Remove the ability to handle 00 here
		 *  when there's no way for user to have old cryptohome
		 *  version with this update (in version 74?).
		 */
