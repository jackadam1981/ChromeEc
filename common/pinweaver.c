...
static bool compute_hmac(const struct merkle_tree_t *merkle_tree,
...
static bool compute_root_hash(const struct merkle_tree_t *merkle_tree,
			      struct label_t path,
		...
		const struct imported_leaf_data_t *optional_old_wrapped_data)
{
	...
	if (!compute_hmac(merkle_tree, &ptrs, wrapped_leaf_data->hmac))
		return PW_ERR_CRYPTO_FAILURE;

	if (!compute_root_hash(merkle_tree, leaf_data->pub.label, hashes,
		...
		struct leaf_data_t *leaf_data)
{
	...
	if (!compute_hmac(merkle_tree, imported_leaf_data, hmac))
		return PW_ERR_CRYPTO_FAILURE;
				 ...
				 uint16_t req_size)
{
	...
	if (!compute_root_hash(merkle_tree, request->leaf_location,
 ...
 */
bool compute_hash(const uint8_t hashes[][PW_HASH_SIZE], uint16_t num_hashes,
