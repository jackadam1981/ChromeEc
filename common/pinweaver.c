...
_Static_assert(PW_HASH_SIZE >= SHA256_DIGEST_SIZE,
	       "uint8_t is[PW_HASH_SIZE] too small for a SHA256 digest/");

_Static_assert(sizeof(struct leaf_data_t) % PW_WRAP_BLOCK_SIZE == 0,
	       "sizeof(struct leaf_data_t) % 16 should be zero");

_Static_assert(sizeof(((struct pw_request_t *)0)->data) ==
			       sizeof(((struct pw_request_t *)0)->data.raw),
	       "pw_request_t.data.raw should be the largest member of the "
			       "union");

_Static_assert(sizeof(((struct pw_response_t *)0)->data) ==
	       sizeof(((struct pw_response_t *)0)->data.raw),
	       "pw_response_t.data.raw should be the largest member of the "
			       "union");
		       ...
		       uint8_t (*new_root)[PW_HASH_SIZE])
{
	...
	for (hx = 1; hx < merkle_tree->height - 1; ++hx) {
		...
		compute_hash(&(*view)[hx], fan_out - 1, index,
			     (const uint8_t (*)[PW_HASH_SIZE])&temp_a,
			     &temp_b);
		...
		index = get_index(merkle_tree, path,
				  merkle_tree->height - hx - 1);
		compute_hash(&(*view)[hx], fan_out - 1, index,
			     (const uint8_t (*)[PW_HASH_SIZE])&temp_b,
			     &temp_a);
	}
...
int pw_handle_reset_tree(struct merkle_tree_t *merkle_tree,
			 ...
			 uint8_t (*new_root)[PW_HASH_SIZE])
{
	uint8_t fan_out = 1 << request->bits_per_level;
	int ret;

	if (request->bits_per_level > BITS_PER_LEVEL_MAX ||
			request->bits_per_level < BITS_PER_LEVEL_MIN)
		return PW_ERR_BITS_PER_LEVEL_INVALID;

	if (request->height > HEIGHT_MAX(request->bits_per_level) ||
			request->height < HEIGHT_MIN ||
			((fan_out - 1) * request->height) * PW_HASH_SIZE + 128 >
					PW_MAX_PATH_SIZE)
			  ...
			  uint8_t (*new_root)[PW_HASH_SIZE])
{
	...
	compute_hmac(merkle_tree, &wrapped_leaf_data,
