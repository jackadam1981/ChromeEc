			       ...
			       valid_pcr_criteria[PW_MAX_PCR_CRITERIA_COUNT])
{
	...
	for (x = 0; x < PW_MAX_PCR_CRITERIA_COUNT; ++x) {
		...
		selection.pcrSelections[0].sizeofSelect =
			sizeof(valid_pcr_criteria[x].bitmask);
		for (y = 0; y < selection.pcrSelections[0].sizeofSelect; ++y) {
			selection.pcrSelections[0].pcrSelect[y] =
				(valid_pcr_criteria[x].bitmask >> (y * 8));
		}
		...
		digest_index = 0;
		for (y = 0;
		     y < 8 * sizeof(valid_pcr_criteria[x].bitmask);
		     ++y) {
			if ((valid_pcr_criteria[x].bitmask & (1 << y)) != 0) {
				HASH_update(
					&ctx,
			      ...
			      uint16_t *data_length)
{
	...
	/** This function is not supposed to be called for a leaf that has old
	 *  version.
	 */
			      ...
			      uint16_t *data_length)
{
	...
	if (request->unimported_leaf_data.head.leaf_version.minor != 0 ||
	    request->unimported_leaf_data.head.leaf_version.major != 0) {
		cprints(CC_TASK, "PinWeaver: Invalid leaf version: %d",
