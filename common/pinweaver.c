...
static int validate_pcr_value(
		...
		uint8_t pcr_criteria_size)
{
	size_t x;
	size_t y;
	TPML_DIGEST digest;

	if (pcr_criteria_size == 0)
		return EC_SUCCESS;

	for (x = 0; x < pcr_criteria_size; ++x) {
		uint32_t pcr_counter;
		TPML_PCR_SELECTION selection;
		TPMS_PCR_SELECTION selection_s;

		selection.count = 1;
		selection_s.hash = TPM_ALG_SHA256;
		selection_s.sizeofSelect = 1;
		selection_s.pcrSelect[0] = valid_pcr_criteria[x].bitmask;
		selection.pcrSelections[0] = selection_s;

		PCRRead(&selection, &digest, &pcr_counter);
		for (y = 0; y < digest.count; ++y) {
			if (digest.digests[y].b.buffer[0] !=
				 ...
				 uint16_t *response_size)
{
	...
	leaf_data.pub.valid_pcr_criteria_size =
			request->valid_pcr_criteria_size;
