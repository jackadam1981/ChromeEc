...
static int create_merkle_tree(bits_per_level_t bits_per_level, height_t height,
		  ...
		  uint8_t result[PW_HASH_SIZE])
{
	...
	DCRYPTO_SHA256_init(&ctx, 0);
	if (location > 0)
		HASH_update(&ctx, hashes[0], PW_HASH_SIZE * location);
	HASH_update(&ctx, child_hash, PW_HASH_SIZE);
	if (location < num_hashes)
		SHA256_update(&ctx, hashes[location],
