...
BUILD_ASSERT(sizeof(struct pw_log_storage_t) ==
	     PW_LOG_VAR0_SIZE + PW_LOG_VAR1_SIZE);
 ...
 * won't change.
 *
 * If any data of in the original struct leaf_sensitive_data_t changes, a new
 * IV should be generated and stored as part of the log for a replay to be
 * possible.
 */
...
{
	return (const struct unimported_leaf_data_t *)ptr;
}
...
{
	return (const struct leaf_public_data_t *)ptr->payload;
}

inline const uint8_t *get_cipher_text(const struct unimported_leaf_data_t *ptr)
{
	return ptr->payload + ptr->head.pub_len;
}

inline void
*get_path_hashes(const struct unimported_leaf_data_and_hashes_t *ptr)
{
	return (void *)(ptr->payload + ptr->head.pub_len + ptr->head.sec_len);
}
...
static int derive_keys(struct merkle_tree_t *merkle_tree)
{
	...
	memcpy(input, merkle_tree->key_derivation_nounce,
	       sizeof(merkle_tree->key_derivation_nounce));
	input[6] = KEY_TYPE_AES;
	if (DCRYPTO_appkey_derive(PINWEAVER, input,
				  (uint32_t *)merkle_tree->wrap_key))
		return PW_ERR_CRYPTO_FAILURE;

	input[6] = KEY_TYPE_HMAC;
	if (DCRYPTO_appkey_derive(PINWEAVER, input,
			      ...
			      struct merkle_tree_t *merkle_tree)
{
	...
	rand_bytes(merkle_tree->key_derivation_nounce,
		   sizeof(merkle_tree->key_derivation_nounce));
		...
		uint8_t result[PW_HASH_SIZE])
{
	...
	HASH_update(&hmac.hash, &unimported_leaf_data->head,
		    sizeof(unimported_leaf_data->head));
	HASH_update(&hmac.hash, &unimported_leaf_data->iv,
		    sizeof(unimported_leaf_data->iv));
	HASH_update(&hmac.hash, unimported_leaf_data->payload,
...
{
	memcpy(&leaf_data->pub, get_pub(unimported_leaf_data),
	       sizeof(leaf_data->pub));
	if (DCRYPTO_aes_ctr((uint8_t *)&leaf_data->sec, merkle_tree->wrap_key,
			    sizeof(merkle_tree->wrap_key) << 3,
			    unimported_leaf_data->iv,
			    get_cipher_text(unimported_leaf_data),
			    sizeof(leaf_data->sec)) != EC_SUCCESS) {
		return PW_ERR_CRYPTO_FAILURE;
	}
	return EC_SUCCESS;
...
static int handle_leaf_update(
		...
		struct wrapped_leaf_data_t *wrapped_leaf_data,
		uint8_t new_root[PW_HASH_SIZE],
		const struct unimported_leaf_data_t *optional_old_wrapped_data,
		const uint8_t optional_old_root[PW_HASH_SIZE])
{
	int ret;
	...
	if (optional_old_wrapped_data == NULL) {
		...
		memcpy(wrapped_leaf_data->cipher_text,
		       get_cipher_text(optional_old_wrapped_data),
		       sizeof(wrapped_leaf_data->cipher_text));
	}

	compute_hmac(merkle_tree, cast_wld_uld(wrapped_leaf_data),
	...
	if (optional_old_root &&
	    memcmp(new_root, optional_old_root, PW_HASH_SIZE))
		return PW_ERR_PATH_AUTH_FAILED;
...
{
	uint32_t uld_len = head->pub_len +
			   head->sec_len;

	if (head->leaf_version.major !=
	    PW_LEAF_MAJOR_VERSION)
		return PW_ERR_LEAF_VERSION_MISMATCH;

	if (head->leaf_version.minor ==
	    PW_LEAF_MINOR_VERSION) {
		if (uld_len != PW_RESP_SIZE_DISCREPENCY)
			return PW_ERR_LENGTH_INVALID;
	} else if (uld_len < PW_RESP_SIZE_DISCREPENCY)
		return PW_ERR_LENGTH_INVALID;

	if (payload_len != uld_len + aux_hash_len * PW_HASH_SIZE)
		return PW_ERR_LENGTH_INVALID;
...
static int validate_request_with_wrapped_leaf(
		...
		const struct unimported_leaf_data_and_hashes_t
		...
		struct leaf_data_t *leaf_data)
{
	...
	ret = validate_request_with_path(
			merkle_tree,
			get_pub(cast_uldah_uld(unimported_leaf_data))->label,
			get_path_hashes(unimported_leaf_data),
			unimported_leaf_data->hmac);
	if (ret != EC_SUCCESS)
		return ret;

	compute_hmac(merkle_tree, cast_uldah_uld(unimported_leaf_data), hmac);
	...
	return decrypt_leaf_data(merkle_tree,
				 cast_uldah_uld(unimported_leaf_data),
				 leaf_data);
}
...
static int load_log_data(struct pw_log_storage_t *log)
{
	...
	if (ptr->val_len != PW_LOG_VAR0_SIZE)
	...
	ptr = getvar(PW_LOG_VAR1, sizeof(PW_LOG_VAR1) - 1);
	if (ptr == NULL)
		return PW_ERR_NV_EMPTY;

	if (ptr->val_len != PW_LOG_VAR1_SIZE)
		return PW_ERR_NV_LENGTH_MISMATCH;

	view = (void *)tuple_val(ptr);
	memcpy((((uint8_t *)log) + PW_LOG_VAR0_SIZE), view, ptr->val_len);
	return EC_SUCCESS;
}
...
int store_log_data(const struct pw_log_storage_t *log)
{
	...
	ret = setvar(PW_LOG_VAR1, sizeof(PW_LOG_VAR1) - 1,
		     ((uint8_t *)log) + PW_LOG_VAR0_SIZE, PW_LOG_VAR1_SIZE);
	if (ret != EC_SUCCESS)
		return ret;
...
static int load_merkle_tree(struct merkle_tree_t *merkle_tree)
{
	...
	/* Handle the immutable data. */
	{
		...
		memcpy(merkle_tree->key_derivation_nounce,
		       tree->key_derivation_nounce,
		       sizeof(tree->key_derivation_nounce));
...
int store_merkle_tree(const struct merkle_tree_t *merkle_tree)
{
	...
	/* Handle the immutable data. */
	{
		...
		memcpy(data.key_derivation_nounce,
		       merkle_tree->key_derivation_nounce,
		       sizeof(data.key_derivation_nounce));
				 ...
				 uint8_t new_root[PW_HASH_SIZE])
{
	...
	ret = handle_leaf_update(merkle_tree, &leaf_data, request->path_hashes,
				 &wrapped_leaf_data, new_root, NULL, NULL);
	if (ret != EC_SUCCESS)
		return ret;
	...
	*response_size = sizeof(*response) + PW_RESP_SIZE_DISCREPENCY;
			      ...
			      uint8_t new_root[PW_HASH_SIZE])
{
	...
	ret = validate_request_with_wrapped_leaf(
			...
			&request->unimported_leaf_data, &leaf_data);
	if (ret != EC_SUCCESS)
		return ret;

	ret = test_rate_limit(&leaf_data, &response->seconds_to_wait);
	if (ret != EC_SUCCESS) {
		*data_length = sizeof(response->seconds_to_wait);
		return ret;
	}
	...
	ret = handle_leaf_update(
			...
			get_path_hashes(&request->unimported_leaf_data),
			&wrapped_leaf_data, new_root,
			cast_uldah_uld(&request->unimported_leaf_data), NULL);
	if (ret != EC_SUCCESS)
		return ret;

	/* ret needs to remain unchanged so post_auth_ret is used instead. */
	...
	*data_length = sizeof(wrapped_leaf_data) + PW_SECRET_SIZE;
				...
				uint16_t *response_size)
{
	...
	compute_hmac(merkle_tree,
		     cast_uldah_uld(&request->unimported_leaf_data), hmac);
	if (safe_memcmp(hmac, request->unimported_leaf_data.hmac, sizeof(hmac)))
		return PW_ERR_HMAC_AUTH_FAILED;

	ret = decrypt_leaf_data(merkle_tree,
				cast_uldah_uld(&request->unimported_leaf_data),
				&leaf_data);
	if (ret != EC_SUCCESS)
		return ret;
		      ...
		      struct pw_response_t *response)
{
	...
	memcpy(root, merkle_tree->root, sizeof(merkle_tree->root));
	...
	case PW_RESET_TREE:
		ret = pw_handle_reset_tree(merkle_tree,
					   ...
					   request->header.data_length, root);
		break;
	case PW_INSERT_LEAF:
...
cleanup:
	...
	memcpy(&response->header.root, root, sizeof(root));
	return ret;
};
