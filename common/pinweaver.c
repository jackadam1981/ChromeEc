...
_Static_assert(sizeof(leaf_data_t) % PW_WRAP_BLOCK_SIZE == 0,
	       "sizeof(leaf_data_t) % 16 should be zero");

_Static_assert(sizeof(((pw_request_t *)0)->data) ==
			       sizeof(((pw_request_t *)0)->data.raw),
	       "pw_request_t.data.raw should be the largest member of the "
			       "union");

_Static_assert(sizeof(((pw_response_t *)0)->data) ==
	       sizeof(((pw_response_t *)0)->data.raw),
...
pw_timestamp_t MOCK_update_timestamp;
/* Sets the value of ts to the current notion of time. */
static void update_timestamp(pw_timestamp_t *ts)
{
#ifdef CHIP_HOST
			   ...
			   hash_t *new_root)
{
	...
	compute_root_hash(merkle_tree, leaf_data.idat.label,
			  &request->path_hashes,
			  (const hash_t *)&wrapped_leaf_data.hmac,
			  new_root);
