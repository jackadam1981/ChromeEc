...
static const char *_text_event(enum pchg_event event)
{
	...
	static const char * const event_names[] = {
		...
		[PCHG_EVENT_ERROR_FOREIGN_OBJECT] = "ERROR_FO",
...
static void pchg_state_enabled(struct pchg *ctx)
{
	...
	case PCHG_EVENT_DEVICE_DETECTED:
		...
		break;
	case PCHG_EVENT_ERROR_FOREIGN_OBJECT:
		ctx->error |= PCHG_ERROR_MASK(PCHG_ERROR_FO);
		break;
...
static int pchg_run(struct pchg *ctx)
{
	...
	/*
	 * Notify the host of
	 * - [S0] Charge update with SoC change and all other events and
	 *   errors except FW update events.
	...
	/* Report errors (except update errors) */
	if (ctx->error)
		return 1;
...
static enum ec_status hc_pchg(struct host_cmd_handler_args *args)
{
	 ...
	 * long as:
	 * 1. Errors are handled on the host not locally (in the EC) OR
	 * 2. Errors are handled immediately as they come. (eg. OT -> stop
	 *    charging)
