...
struct pchg_policy_t pchg_policy_on = {
	.evt_mask = BIT(PCHG_EVENT_IRQ) |
		    BIT(PCHG_EVENT_RESET) |
		    BIT(PCHG_EVENT_INITIALIZED) |
		    BIT(PCHG_EVENT_ENABLED) |
		    BIT(PCHG_EVENT_DISABLED) |
		    BIT(PCHG_EVENT_DEVICE_DETECTED) |
		    BIT(PCHG_EVENT_DEVICE_CONNECTED) |
		    BIT(PCHG_EVENT_DEVICE_LOST) |
		    BIT(PCHG_EVENT_CHARGE_STARTED) |
		    BIT(PCHG_EVENT_CHARGE_UPDATE) |
		    BIT(PCHG_EVENT_CHARGE_ENDED) |
		    BIT(PCHG_EVENT_CHARGE_STOPPED) |
		    BIT(PCHG_EVENT_ERROR) |
		    BIT(PCHG_EVENT_IN_NORMAL) |
		    BIT(PCHG_EVENT_ENABLE) |
		    BIT(PCHG_EVENT_DISABLE),
	.err_mask = UINT32_MAX,
};
...
static int pchg_should_notify(struct pchg *ctx, enum pchg_chipset_state state,
			      uint32_t previous_error, uint8_t previous_battery)
{
	if (ctx->event == PCHG_EVENT_ERROR) {
		uint32_t err = ctx->error & ctx->policy[state]->err_mask;
		/* Report only 0->1. */
		return (err ^ previous_error) & err;
	}
