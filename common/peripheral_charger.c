...
static void _send_host_event(struct pchg *ctx, uint32_t event)
{
	int port = PCHG_CTX_TO_PORT(ctx);

	atomic_or(&pchg_host_events, event);
	atomic_or(&pchg_host_events, port << EC_MKBP_PCHG_PORT_SHIFT);
...
static void pchg_state_download(struct pchg *ctx)
{
	...
	case PCHG_EVENT_UPDATE_ERROR:
		...
		ctx->error &= ~PCHG_ERROR_MASK_UPDATE;
		break;
	default:
		break;
	}
