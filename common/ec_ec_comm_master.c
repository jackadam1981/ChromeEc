 ...
 * @param rxlen is the size of rx.info (0 if no information is returned).
 *
 * @return
			 ...
			 int timeout)
{
	...
	/*
	 * Make sure there is a gap between each commands, so that the slave
 ...
 * @param request_result is data.rx.head.result (response result value)
 *
