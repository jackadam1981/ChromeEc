			 ...
			 int timeout_us)
{
	...
	request_header->header_crc =
		crc_8((uint8_t *)request_header, sizeof(*request_header)-1);
	if (req_len > 0)
		data[sizeof(*request_header) + req_len] =
			crc_8(&data[sizeof(*request_header)], req_len);
	...
	if (hascrc && data[rx_length - 1] !=
		    crc_8(&data[tx_length + sizeof(*request_header)],
				 resp_len)) {
