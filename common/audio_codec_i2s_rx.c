...
static int i2s_rx_set_sample_depth(struct host_cmd_handler_args *args)
{
	...
	if (i2s_rx_enabled)
		return EC_RES_BUSY;
	if (pp->depth >= EC_CODEC_I2S_RX_SAMPLE_DEPTH_COUNT)
		return EC_RES_INVALID_PARAM;

	if (audio_codec_i2s_rx_set_sample_depth(pp->depth) != EC_SUCCESS)
		return EC_RES_ERROR;
...
static int i2s_rx_set_daifmt(struct host_cmd_handler_args *args)
{
	...
	if (i2s_rx_enabled)
		return EC_RES_BUSY;
	if (pp->daifmt >= EC_CODEC_I2S_RX_DAIFMT_COUNT)
		return EC_RES_INVALID_PARAM;

	if (audio_codec_i2s_rx_set_daifmt(pp->daifmt) != EC_SUCCESS)
		return EC_RES_ERROR;
...
static int i2s_rx_set_bclk(struct host_cmd_handler_args *args)
{
	struct ec_param_ec_codec_i2s_rx *p =
		(struct ec_param_ec_codec_i2s_rx *)args->params;
	struct ec_param_ec_codec_i2s_rx_set_bclk *pp = &p->set_bclk_param;

	if (i2s_rx_enabled)
		return EC_RES_BUSY;

	if (audio_codec_i2s_rx_set_bclk(pp->bclk) != EC_SUCCESS)
		return EC_RES_ERROR;
