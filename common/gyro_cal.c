 ...
 *			range set by 'temperature_delta_limit_celsius'.
 */
			  ...
			  fp_t x, fp_t y, fp_t z, int temperature_celsius)
{
	...
	gyro_temperature_stats_tracker_update(gyro_cal, temperature_celsius);
...
void gyro_temperature_stats_tracker_reset(struct gyro_cal *gyro_cal)
{
	...
	gyro_cal->temperature_mean_tracker.temperature_min_celsius = 0x7f;
	gyro_cal->temperature_mean_tracker.temperature_max_celsius = 0xff;
}
...
void gyro_still_mean_tracker_reset(struct gyro_cal *gyro_cal)
{
	...
	for (i = 0; i < 3; i++) {
		...
		gyro_cal->window_mean_tracker.gyro_winmean_max[i] = -FLT_MIN;
	}
}
