...
#if defined(CONFIG_PLATFORM_EC_BATT_FULL_CHIPSET_OFF_MV_LIMIT)

#if CONFIG_PLATFORM_EC_BATT_FULL_CHIPSET_OFF_MV_LIMIT < 5000 || \
    CONFIG_PLATFORM_EC_BATT_FULL_CHIPSET_OFF_MV_LIMIT >= PD_MAX_VOLTAGE_MV
...
static void reduce_input_voltage_when_full(void)
{
	...
	if (chipset_in_or_transitioning_to_state(CHIPSET_STATE_ANY_OFF) &&
		...
		if (max_pd_voltage_mv !=
		    CONFIG_PLATFORM_EC_BATT_FULL_CHIPSET_OFF_MV_LIMIT) {
