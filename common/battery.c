...
{
	const int poll_msec = CONFIG_BATTERY_CUTOFF_POLL_MSEC * MSEC;
	const int count_start = (CONFIG_BATTERY_CUTOFF_TIMEOUT_SEC * SECOND) /
			poll_msec;
	static int count;
...
static void battery_on_ac_change(void)
{
	if (extpower_is_present()) {
		...
		if (keyboard_scan_get_boot_keys() & BOOT_KEY_LEFT_ALT) {
			CPRINTS("RAPU detected. Scheduling cutoff.");
...
static int command_cutoff(int argc, const char **argv)
{
	...
	return battery_cutoff_start() == EC_RES_SUCCESS ?
