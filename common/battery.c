...
#if defined(CONFIG_BATT_FULL_CHIPSET_OFF_INPUT_LIMIT_MV)
...
static bool board_wants_reduced_input_voltage(void) {
	...
	/* Battery needs charge, so we don't want to reduce woltage */
...
/* Lower our input voltage in S5/G3 when battery is full. */
static void reduce_input_voltage_when_full(void)
{
	...
	if (board_wants_reduced_input_voltage()) {
		 ...
		 * restored when chipset leaves S5/G3 or battery wants charge.
		 */
		...
		/*
		 * Board doesn't want to reduce input voltage, so restore
		 * previously saved voltage.
		 */
