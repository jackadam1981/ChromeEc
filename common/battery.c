...
#define CUTOFFPRINTS(info) CPRINTS("%s %s", "Battery cutoff", info)
...
#ifndef CONFIG_BATTERY_CUTOFF_TIMEOUT_MSEC
...
/* Count down timer to catch cutoff failure due to timeout. */
static int cutoff_timeout;
/* Interval between cutoff completion checks. */
static const int cutoff_poll_msec = 250;
/* Initial value of cutoff_timeout */
static const int cutoff_timeout_start =
	CONFIG_BATTERY_CUTOFF_TIMEOUT_MSEC / cutoff_poll_msec;
...
static int battery_cutoff_start(void)
{
	...
	if (rv == EC_RES_SUCCESS) {
		...
		cutoff_timeout = cutoff_timeout_start;
		/* Start monitor loop. */
...
static void pending_cutoff_deferred(void)
{
	if (battery_cutoff_state == BATTERY_CUTOFF_STATE_IN_PROGRESS) {
		...
		/* Cutoff hasn't completed. Keep monitoring. */
		CPRINTS("Waiting for cutoff completion (%d/%d)", cutoff_timeout,
			cutoff_timeout_start);
		cflush();
		...
		/* Delayed start */
		CPRINTS("Starting cutoff as scheduled");
		battery_cutoff_start();
		return;
	}

	CPRINTS("Bad call to %s (%d)", __func__, battery_cutoff_state);
}
