...
{
	int numer, denom, remain, full;

	if ((batt->flags & BATT_FLAG_BAD_FULL_CAPACITY) ||
			(batt->flags & BATT_FLAG_BAD_REMAINING_CAPACITY))
		return;

	if (batt->full_capacity <= 0 || batt->remaining_capacity <= 0)
		return;

	batt->full_capacity = batt->full_capacity * batt_full_factor / 100;

	if (batt->remaining_capacity > batt->full_capacity)
		batt->remaining_capacity = batt->full_capacity;
	 ...
	 *   100 * (charge - shutdown_pct) / (100 - shutdown_pct);
	 */
	numer = (100 * remain - full * batt_host_shutdown_pct) * 100;
	denom = full * (100 - batt_host_shutdown_pct);
	/* Rounding (instead of truncating) */
	batt->display_pct = (numer + denom / 2) / denom;
}
