...
static void pending_cutoff_deferred(void)
{
	...
	/*
	 * Lock the irq until the battery_cutoff_state is updated to prevent
	 * preemption.
	 * This is to prevent from waking the battery up after cut-off from the
	 * other devices on the same i2c port.
	 */
	key = irq_lock();
