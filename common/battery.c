...
#ifdef CONFIG_BATTERY_CUTOFF
#define BATTERY_CUTOFF_DELAY_US (5 * SECOND)

enum battery_cutoff_states battery_cutoff_state =
...
{
	return (battery_cutoff_state == BATTERY_CUTOFF_STATE_CUTOFF);
}
...
static int battery_command_cutoff(struct host_cmd_handler_args *args)
{
	...
	rv = board_cutoff_battery();
	if (!rv)
		battery_cutoff_state = BATTERY_CUTOFF_STATE_CUTOFF;
