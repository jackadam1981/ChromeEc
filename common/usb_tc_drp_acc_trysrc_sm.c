...
#define TC_FLAGS_LPM_TRANSITION           BIT(4)
...
DECLARE_STATE(tc, error_recovery, WITH_EXIT);
...
/* Super States */
DECLARE_STATE(tc, cc_rd, NOOP_EXIT);
DECLARE_STATE(tc, cc_rp, NOOP_EXIT);
DECLARE_STATE(tc, cc_open, NOOP_EXIT);
...
static unsigned int tc_error_recovery_exit(int port)
{
	return 0;
}
...
static unsigned int tc_unoriented_dbg_acc_src_run(int port)
{
	 ...
	 */
	set_state(port, TC_OBJ(port), tc_oriented_dbg_acc_src);
#endif
	return 0;
}
...
static unsigned int tc_oriented_dbg_acc_src(int port, enum signal sig)
{
	int ret;

	ret = (*tc_oriented_dbg_acc_snk_sig[sig])(port);
	return SUPER(ret, sig, tc_cc_rp);
}

static unsigned int tc_oriented_dbg_acc_src_entry(int port)
{
...
static unsigned int tc_oriented_dbg_acc_src_run(int port)
{
	return 0;
}
...
/*
 * TODO(shurst): DO WE NEED tc_unoriented_dbg_acc_src
 */
#if 0
...
static unsigned int tc_cc_rd_run(int port)
{
	return 0;
}
...
static unsigned int tc_cc_rp_run(int port)
{
	return 0;
}
...
static unsigned int tc_cc_open_run(int port)
{
	return 0;
}
