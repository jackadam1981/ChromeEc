...
static void sink_power_sub_states(int port)
{
	...
	if (new_cc_voltage != tc[port].cc_voltage) {
		/*
		 * NOTE: Using enum variable cc_state as generic
		 * variable to debounce cc voltage status.
		 */
...
static unsigned int tc_dbg_acc_snk_entry(int port)
{
	...
	/*
	 * TODO(b/137759869): Set up for debug accessory or remove state
	 * if not needed.
	 */
...
static unsigned int tc_attached_src_entry(int port)
{
	...
	/* Enable VBUS */
	ret = pd_set_power_supply_ready(port);
