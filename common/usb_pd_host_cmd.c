...
{
	const struct ec_params_usb_pd_dp21_cable *p = args->params;
	struct ec_response_usb_dp21_cable_info *dp21_disc = args->response;
	...
	if (IS_ENABLED(CONFIG_USB_PD_DP21_MODE)) {
		...
		dp21_disc->uhbr_13_5_supported =
			is_dp_uhbr13_5_supported(p->port);
...
DECLARE_HOST_COMMAND(EC_CMD_USB_PD_DP21_CABLE_DISCOVERY, hc_usb_pd_dp21_discovery,
		     EC_VER_MASK(0));
#endif /* CONFIG_COMMON_RUNTIME */
