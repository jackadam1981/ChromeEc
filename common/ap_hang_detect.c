...
static void hang_detect_power_button(void)
{
	if (power_button_is_pressed()) {
		if (hdparams.flags & EC_HANG_START_ON_POWER_PRESS)
			hang_detect_start_event("power button");
	} else {
		if (hdparams.flags & EC_HANG_STOP_ON_POWER_RELEASE)
			hang_detect_cancel_event("power button");
	}
}
...
static void hang_detect_lid(void)
{
	if (lid_is_open()) {
		if (hdparams.flags & EC_HANG_START_ON_LID_OPEN)
			hang_detect_start_event("lid open");
	} else {
		if (hdparams.flags & EC_HANG_START_ON_LID_CLOSE)
			hang_detect_start_event("lid close");
	}
}
...
#ifdef CONFIG_AP_HANG_DETECT_FOR_EVENT
		hang_detect_cancel_event("ap request");
#endif
		/* Ignore the other params */
		return EC_RES_SUCCESS;
	}
...
#ifdef CONFIG_AP_HANG_DETECT_FOR_EVENT
		hang_detect_start_event("ap request");
#endif
		/* Ignore the other params */
		return EC_RES_SUCCESS;
	}
...
#ifdef CONFIG_AP_HANG_DETECT_FOR_EVENT
		hang_detect_cancel_event("ap flags=0");
#endif
	}
...
static int command_hang_detect(int argc, const char **argv)
{
	...
	ccprintf("status for event: %s",
		 (hook_call_is_active(&hang_detect_event_data) ? "active" :
								 "inactive"));
#endif

	ccprintf("status for reboot: %s",
		 (hook_call_is_active(&hang_detect_reboot_data) ? "active" :
								  "inactive"));
