...
hang_detect_host_command(struct host_cmd_handler_args *args)
{
	...
	if (p->flags & EC_GET_HANG_STATUS) {
		 ...
		 * EC_CLEAR_HANG_STATUS set everytime the AP is shutting down or
		 * rebooting gracefully (gracefully here means "not triggered by
		 * watchdog") to inform that AP is closing normally.
		 */
