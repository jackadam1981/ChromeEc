...
#if (IS_ENABLED(CONFIG_AP_HANG_DETECT_FOR_EVENT))
/**
...
static void hang_detect_start_for_event(const char *why)
{
	if (hdparams.host_event_timeout_msec &&
	    !hook_call_is_active(&hang_detect_deferred_for_event_data)) {
...
#if (IS_ENBLED(CONFIG_AP_HANG_DETECT_FOR_EVENT))
...
static int command_hang_detect(int argc, const char **argv)
{
	...
	ccprintf("status for reboot: %s",
		 (hook_call_is_active(&hang_detect_deferred_for_reboot_data) ?
			  "active" :
			  "inactive"));
