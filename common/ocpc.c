				  ...
				  int desired_batt_current_ma)
{
	...
	int min_vsys_target;
	int error = 0;
#ifdef CONFIG_PLATFORM_EC_OCPC_USE_CHARGER_INPUT_ERROR
	int charger_input_error = 0;
#endif
	int derivative = 0;
...
#ifdef CONFIG_PLATFORM_EC_OCPC_USE_CHARGER_INPUT_ERROR
		/* Should only be used for proportional controller as the
		 * controller is not currently tracking 2 states.
		 */
		charger_input_error = *desired_charger_input_current -
...
__overridable void ocpc_get_drive_limit(int *drive_limit)
{
}
...
static void ocpc_set_drive_limit(void)
{
	ocpc_get_drive_limit(&drive_limit);
}
DECLARE_HOOK(HOOK_INIT, ocpc_set_drive_limit, HOOK_PRIO_DEFAULT);
