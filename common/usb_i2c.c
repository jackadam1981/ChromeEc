...
void usb_i2c_execute(struct usb_i2c_config const *config)
{
	...
	if (!usb_i2c_board_is_enabled()) {
		...
		/*
		 * TODO: Add security.  This currently blindly passes through
		 * ALL I2C commands on any bus the EC knows about.  It should
		 * behave closer to EC_CMD_I2C_PASSTHRU, which can protect
		 * ports and ranges.
		 */
