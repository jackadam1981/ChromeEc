...
#define CPRINTF5(format, args...) info_printf(CC_KEYBOARD, format, ## args)
#else
...
{
	CPRINTF("KB IRQ %s", enable ? "enable" : "disable");
					  ...
					  uint8_t *scan_code, int32_t *len)
{
	...
	default:
		CPRINTF("KB scancode set %d unsupported", code_set);
		return EC_ERROR_UNIMPLEMENTED;
	}

	if (!make_code) {
		CPRINTF("KB scancode %d:%d missing", row, col);
		return EC_ERROR_UNIMPLEMENTED;
	}
...
void keyboard_state_changed(int row, int col, int is_pressed)
{
	...
	CPRINTF5("KB (%d,%d)=%d", row, col, is_pressed);
...
static void update_ctl_ram(uint8_t addr, uint8_t data)
{
	...
	CPRINTF5("KB set CTR_RAM(0x%02x)=0x%02x (old:0x%02x)",
		 addr, data, orig);
...
static int handle_keyboard_data(uint8_t data, uint8_t *output)
{
	...
	CPRINTF5("KB recv data: 0x%02x", data);
	kblog_put('d', data);

	switch (data_port_state) {
	case STATE_SCANCODE:
		CPRINTF5("KB eaten by STATE_SCANCODE: 0x%02x", data);
		if (data == SCANCODE_GET_SET) {
			output[out_len++] = I8042_RET_ACK;
			output[out_len++] = scancode_set;
		} else {
			scancode_set = data;
			CPRINTF("KB scancode set to %d", scancode_set);
			output[out_len++] = I8042_RET_ACK;
		}
		data_port_state = STATE_NORMAL;
		break;
	...
	case STATE_SETLEDS:
		CPRINTF5("KB eaten by STATE_SETLEDS");
		output[out_len++] = I8042_RET_ACK;
		data_port_state = STATE_NORMAL;
		break;

	case STATE_EX_SETLEDS_1:
		CPRINTF5("KB eaten by STATE_EX_SETLEDS_1");
		output[out_len++] = I8042_RET_ACK;
		data_port_state = STATE_EX_SETLEDS_2;
		break;

	case STATE_EX_SETLEDS_2:
		CPRINTF5("KB eaten by STATE_EX_SETLEDS_2");
		output[out_len++] = I8042_RET_ACK;
		data_port_state = STATE_NORMAL;
		break;

	case STATE_WRITE_CMD_BYTE:
		CPRINTF5("KB eaten by STATE_WRITE_CMD_BYTE: 0x%02x",
			 data);
	...
	case STATE_WRITE_OUTPUT_PORT:
		CPRINTF5("KB eaten by STATE_WRITE_OUTPUT_PORT: 0x%02x",
			 data);
	...
	case STATE_ECHO_MOUSE:
		CPRINTF5("KB eaten by STATE_ECHO_MOUSE: 0x%02x", data);
		output[out_len++] = data;
		data_port_state = STATE_NORMAL;
		break;

	case STATE_SETREP:
		CPRINTF5("KB eaten by STATE_SETREP: 0x%02x", data);
	...
	case STATE_SEND_TO_MOUSE:
		CPRINTF5("KB eaten by STATE_SEND_TO_MOUSE: 0x%02x",
			 data);
		data_port_state = STATE_NORMAL;
		break;
	...
	default:  /* STATE_NORMAL */
		...
		default:
			...
			CPRINTF("KB Unsupported i8042 data 0x%02x",
				data);
			break;
		}
	}
...
static int handle_keyboard_command(uint8_t command, uint8_t *output)
{
	...
	CPRINTF5("KB recv cmd: 0x%02x", command);
	kblog_put('c', command);
	...
	default:
		if (command >= I8042_READ_CTL_RAM &&
			...
			CPRINTF("KB unsupported cmd: 0x%02x", command);
			reset_rate_and_delay();
			keyboard_clear_buffer();
			output[out_len++] = I8042_RET_NAK;
			data_port_state = STATE_NORMAL;
