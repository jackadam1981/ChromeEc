...
#ifdef CONFIG_KEYBOARD_ALTOS
static int fn_key_enabled; /* default the key is disable. */
...
{
	memmap_os_type = host_get_memmap(EC_MEMMAP_KB_OS_TYPE);
	clear_os_type();
}
 ...
 * @param len		The number of valid bytes to send in scan_code
 */
static void special_scancode_bytes(enum special_key_func_list key_func, int8_t pressed,
			   enum scancode_set_list code_set, uint8_t *scan_code,
			   int32_t *len)
{
	*len = 0;
					  ...
					  uint8_t *scan_code, int32_t *len)
{
	...
	case SCANCODE_SET_1:
		...
		if (*memmap_os_type)
			make_code = altos_scancode_set1[row][col];
#else
		make_code = scancode_set1[row][col];
#endif
		break;
	...
	case SCANCODE_SET_2:
		...
		if (*memmap_os_type)
			make_code = altos_scancode_set2[row][col];
#else
		make_code = scancode_set2[row][col];
#endif
		break;
	...
	make_code = check_fn_scancode_set(make_code, code_set,
								  pressed, scan_code, len);
#endif
					  ...
					  uint8_t *scan_code, int32_t *len)
{
	...
	if (make_code >= 0xf000 && make_code < 0xf100) {
		...
		case SCANCODE_SET_1:
			...
			case SWITCH_DISPLAY:
				if (pressed) {
					scancode_bytes(0xe05b, 1, code_set, scan_code, len);
					i8042_send_to_host(*len, scan_code);
					scancode_bytes(0x0019, 1, code_set, scan_code, len);
					i8042_send_to_host(*len, scan_code);
					scancode_bytes(0x0019, 0, code_set, scan_code, len);
					i8042_send_to_host(*len, scan_code);
					scancode_bytes(0xe05b, 0, code_set, scan_code, len);
					i8042_send_to_host(*len, scan_code);
				}
				return 0;
			...
			case DIM_SCREEN:
				if (pressed) {
					host_set_single_event(EC_HOST_EVENT_DIM_SCREEN);
				}
				return 0;

			case BRIGHTEN_SCREEN:
				if (pressed) {
					host_set_single_event(EC_HOST_EVENT_BITGHTEN_SCREEN);
				}
				return 0;

			case KEY_PAUSE:
				if (pressed) {
					special_scancode_bytes(key_func, 1, code_set, scan_code, len);
					i8042_send_to_host(*len, scan_code);
				}
				return 0;
		...
		case SCANCODE_SET_2:
			...
			case SWITCH_DISPLAY:
				if (pressed) {
					scancode_bytes(0xe01f, 1, code_set, scan_code, len);
					i8042_send_to_host(*len, scan_code);
					scancode_bytes(0x004d, 1, code_set, scan_code, len);
					i8042_send_to_host(*len, scan_code);
					scancode_bytes(0x004d, 0, code_set, scan_code, len);
					i8042_send_to_host(*len, scan_code);
					scancode_bytes(0xe01f, 0, code_set, scan_code, len);
					i8042_send_to_host(*len, scan_code);
				}
				return 0;
					  ...
					  uint8_t *scan_code, int32_t *len)
{
	...
	make_code = check_combine_key_set(make_code, code_set,
								  pressed, scan_code, len);
