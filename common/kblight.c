...
{
	int prev, curr;
	int deferred = 0, print_err = 0;

	/*
	 * Store _percent to prev to avoid the race in between
	 * the acpi interrupt and hook task.
	 */
	prev = _percent;
	if (prev == _kblight_get())
		return;
	...
	/*
	 * If acpi interrupt has been called during configuration,
	 * reschedule the deferred function with a delay to avoid
	 * the starvation and watchdog timeout.
	 */
	if (curr != _percent) {
		/* Disable interrupt to avoid race with interrupt */
		interrupt_disable();
		/*
	...
	if (print_err)
		ccprintf("Update fail, expected:% got:%d", _percent, curr);
}
