...
bool accel_cal_accumulate(struct accel_cal *cal, uint32_t sample_time, float x,
			  float y, float z, float temp)
{
	...
	CPRINTS("Still sample (%.6d, %.6d, %.6d)", (int) (x * 1000000),
		(int) (y * 1000000), (int) (z * 1000000));

	/* Compute the temp gate. */
	algo = &cal->algos[(
		int)((temp - CONFIG_ACCEL_CAL_MIN_TEMP) /
		     (CONFIG_ACCEL_CAL_MAX_TEMP - CONFIG_ACCEL_CAL_MIN_TEMP))];
