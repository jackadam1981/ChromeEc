 ...
 *
 * NVMEM TPM objects are stored in separate containers, each one protected by
 * hash and possibly encrypted. When nvmem_commit() is invoked, only objects
 * changed in the NVMEM cache are updated in the flash.
 *
 * The (key, value) pairs are also stored in the same separate containers.
 * There is no special area allocated to the (key, value) pairs in flash, they
 * are interleaved with TPM objects.
 *
 ...
 *
 *  - the (key. value) pairs if modified are updated in flash immediately, not
 ...
 * then the older container's type field is erased, thus marking it as a deleted
 * object. The idea is that when initializing NVMEM cache after reset, in case
 * two instances of the same object are found in the flash because the new
 * instance was saved, but the old instance was not erased because of some
 ...
 *
 * In order to able to erase the type the container must start at the 4 byte
 * boundary. This in turn requires that each container is padded such that
 * total storage taken by the container is divisible by 4.
 *
 ...
 *
 * Don't forget that the contents are encrypted. Decryption is needed each
 ...
 * changed, a new instance is saved in flash. This approach requires that all
 * reserved objects are present in the flash, otherwise there is nothing to
 * compare the cached instance of the object with. This is enforced by the
 * init function.
 *
 ...
 *
 * To enforce that each reserved TPM object is present in the flash, the init
 ...
 *
 * Starting with a fully erased flash is a case when all reserved objects are
 * missing, so they all get created.
 *
 * ===== Handling failures
 *
 ...
 * in the legacy case. If failure happens during this implementation's save or
 * compaction process, there is a risk of ending up with a corrupted or
 * inconsistent flash contents.
 *
 ...
 *   yet supported, this is something to consider, is it really necessary?
 */
...
/* When initializing flash for the first time - set the proper first page header. */
...
/*
 * At startup intialize the list of pages which contain NVMEM data and erased
...
/* Restore a reserved object found in flash on intialization. */
...
/* Restore an evictable object found in flash on intialization. */
 ...
 */
static void retrieve_nvmem_contents(uint8_t *res_bitmap)
{
	int rv;
	struct max_var_container *vc;
...
enum ec_error_list new_nvmem_init(void)
{
	...
	uint8_t reserved_bitmap[(NV_PSEUDO_RESERVE_LAST + 7)/8];
	total_var_space = 0;
	...
	memset(reserved_bitmap, 0, sizeof(reserved_bitmap));

	retrieve_nvmem_contents(reserved_bitmap);

	rv = verify_reserved(reserved_bitmap);
