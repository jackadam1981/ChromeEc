 ...
 */
#define MAX_DELETE_CANDIDATES 40
...
/**
 * Store serialized reserved object into provided buffer.
 * @param index virtual reserved space index (including PCRs)
 * @param out pointer to output buffer (can be updated with location in cache
 * to avoid copy).
 * @param outsize output size of object
 */
static enum ec_error_list make_reserved_from_cache(uint8_t index, uint8_t **out,
						   size_t *outsize)
{
	NV_RESERVED_ITEM ri;
	const struct pcr_descriptor *pcrd;
	...
	*outsize = ri.size;
	...
	scd = get_scd();
	*out = (uint8_t *)&scd->pcrSave + pcrd->pcr_array_offset +
	       pcr_size * pcr_index;
					   ...
					   uint8_t *res_bitmap)
{
	...
	/* Placeholder after the current object size. */
	marshalled = ((uint8_t *)(ch + 1)) + ch->size;
	rv = make_reserved_from_cache(index, &marshalled, &new_size);
	if (rv != EC_SUCCESS)
		return rv;
...
static enum ec_error_list new_nvmem_save_(void)
{
	...
	for (i = 0; i < NV_VIRTUAL_RESERVE_LAST + NUM_OF_PCRS; i++) {
		...
		if (i == NV_RAM_INDEX_SPACE || i == NV_MAX_COUNTER ||
		    bitmap_bit_check(res_bitmap, i))
			continue;
		...
		rv = make_reserved_from_cache(i, &out, &out_size);
		/* Check if this is just an empty PCR. */
		if (i >= NV_VIRTUAL_RESERVE_LAST && is_empty(out, out_size))
			continue;
		/* If out point to NV cache, copy content to container. */
