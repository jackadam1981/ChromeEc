 ...
 * list.
 */
test_export_static uint8_t page_list[NEW_NVMEM_TOTAL_PAGES];
/* Number of available pages. Usually ARRAY_SIZE(page_list) or less. */
test_export_static uint8_t page_count;
...
static void bitmap_bit_set(uint8_t *bitmap, size_t index)
{
 ...
 * images.
 */
static uint32_t calculate_page_header_hash(const struct nn_page_header *ph)
{
	uint32_t hash = -1U;
...
{
	return (struct nn_page_header *)(((uintptr_t)index *
...
static void dump_page_list(void)
{
	...
	ccprintf("Available %u pages [", page_count);
	for (i = 0; i < ARRAY_SIZE(page_list); i++) {
		ccprintf(" %u/", page_list[i]);
		ph = list_element_to_ph(i);
		if (i >= page_count)
			ccprintf("x"); /* eXcluded (bad) */
		else if (ph)
			ccprintf("%d", ph->page_number);
		else
			ccprintf("+"); /* free page */
	}
	ccprintf("]\n");
...
/* Remove page at `index`, move it at then of list */
static void remove_page_from_list(uint8_t index)
{
	uint8_t bad_page;
...
{
	struct nn_page_header ph = {};
	enum ec_error_list rv = EC_ERROR_OVERFLOW;
	...
	while (page_count) {
		...
		remove_page_from_list(0);
	}
	if (!page_count)
		log_no_payload_failure(NVMEMF_PAGE_LIST_OVERFLOW);
	return rv;
}
...
/* Erase flash page with index `0` and add it to the pool of empty pages. */
static enum ec_error_list release_first_page(struct access_tracker *at)
{
...
test_export_static enum ec_error_list compact_nvmem(void)
{
	 ...
	 * at the start of the page already (either at the end, so next save
	 * will be on the new page, or just after page header) and have at
	 * least one free page.
	 */
	if (controller_at.mt.data_offset != sizeof(*controller_at.mt.ph) &&
	    controller_at.mt.data_offset != CONFIG_FLASH_BANK_SIZE &&
	    (controller_at.list_index + 1) < page_count) {
		/* Size of containers for fill object and delimiter. */
		const size_t size_of_two_containers = 2 * sizeof(*ch);
		size_t to_go_in_page =
			CONFIG_FLASH_BANK_SIZE - controller_at.mt.data_offset;
		/* 2 cases:
		 *   >= 8 bytes, so we can create fake object + delimiter, or
		 *   == 4 bytes, so we can just add a final delimiter.
		 * `controller_at.mt.data_offset` is always aligned to
		 * CONFIG_FLASH_WRITE_SIZE, so does container size.
		 */
		if (to_go_in_page >= size_of_two_containers) {
			/* Create fake object till end of page. */
			ch->container_type = NN_OBJ_OLD_COPY;
			ch->container_type_copy = NN_OBJ_OLD_COPY;
			/* Account for container size and delimiter. */
			ch->size = to_go_in_page - size_of_two_containers;
			save_container(ch);
		}
		/* Final delimiter is always required. */
		add_final_delimiter();
	}
...
static enum ec_error_list start_new_flash_page(size_t data_size)
{
	...
	enum ec_error_list rv = EC_ERROR_OVERFLOW;
	uint8_t next_index = controller_at.list_index + 1;

	while (next_index < page_count) {
		mt_ph = flash_index_to_ph(page_list[next_index]);
		...
		if (rv == EC_SUCCESS) {
			/* Update controller with acquired page. */
			controller_at.list_index++;
			controller_at.mt.ph = mt_ph;
			controller_at.mt.data_offset = sizeof(ph);
			return rv;
		}
		/* Remove page where we can't write from use. */
		remove_page_from_list(next_index);
	}
				    ...
				    int room)
{
	...
	if (room < sizeof(*dst) + sizeof(new_scd->platformAlg))
		return 0;
	...
	*dst++ = (!!new_scd->shEnable) | ((!!new_scd->ehEnable) << 1) |
	...
	if (room < sizeof(new_scd->pcrSave.pcrCounter))
		return dst - base;
					 ...
					 int room)
{
	...
	if (room < sizeof(new_srd->contextArray))
		return dst - base;
	...
	if (room < sizeof(new_srd->contextCounter))
		return dst - base;
					       ...
					       int room)
{
	...
	for (index = 0; index < NV_VIRTUAL_RESERVE_LAST; index++) {
		...
		case NV_STATE_CLEAR:
			...
			ri.size = marshal_state_clear(
				scd, p_container_body + sizeof(index),
				room - sizeof(index));
			copy_needed = 0;
			break;
		...
		case NV_STATE_RESET:
			...
			ri.size = marshal_state_reset_data(
				srd, p_container_body + sizeof(index),
				room - sizeof(index));
			copy_needed = 0;
			break;
		}
...
static enum ec_error_list migrate_tpm_nvmem(struct nn_container *ch, int room)
{
	/* Call this to initialize NVMEM indices. */
	NvEarlyStageFindHandle(0);

	migrate_tpm_reserved(ch, room);
	migrate_objects(ch);
...
enum ec_error_list new_nvmem_migrate(unsigned int act_partition)
{
	...
	migrate_vars(ch);
	migrate_tpm_nvmem(ch, CONFIG_FLASH_BANK_SIZE - sizeof(*ch));
	...
	for (i = 0; i < ARRAY_SIZE(page_list); i++) {
		...
		if (!page_header_is_valid(ph)) {
			...
			if (verify_empty_page(ph) != EC_SUCCESS)
				/* Mark it as bad */
				remove_page_from_list(tail_index);
			continue;
		}
 ...
 *
 * @param index NV reserved index for PCR (not PCR index starting from 0)
 * @param data content of NV container
 * @param size size of data
 */
static void restore_pcr(size_t index, const uint8_t *data, size_t size)
{
	size_t pcr_size;
	uint8_t *cached; /* This PCR's position in the NVMEM cache. */

	if (index < NV_FIRST_PCR_INDEX)
		return;

	cached = get_pcr_nv_addr(index - NV_FIRST_PCR_INDEX, &pcr_size);
	/* get_pcr_nv_addr() returns NULL if index is invalid. */
	if (cached == NULL || size != pcr_size)
		log_object_size_error(index, size, pcr_size);
	else
		memcpy(cached, data, size);
}
 ...
 * content.
 */
static void restore_ram_index_space(const uint8_t *pad, size_t size,
				    uint8_t *cached)
{
	uint32_t index_size;
 ...
 */
static void restore_tpm2b_space(NV_RESERVE index, uint8_t *data, size_t size,
...
static void restore_reserved(void *pad, size_t size)
{
	...
	if (size == 0) {
		log_object_size_error(NV_RESERVE_INVALID, size, size);
		return;
	}
	...
	if (type < NV_VIRTUAL_RESERVE_LAST) {
		...
		switch (type) {
		case NV_STATE_CLEAR:
			unmarshal_state_clear(pad, size, cached);
			break;

		case NV_STATE_RESET:
			unmarshal_state_reset(pad, size, cached);
			break;
		...
		case NV_SH_PROOF:
		case NV_EH_PROOF:
			restore_tpm2b_space(type, pad, size, ri.size, cached);
			break;

		case NV_RAM_INDEX_SPACE:
			restore_ram_index_space(pad, size, cached);
			break;
		...
		/* This is probably PCR. */
		restore_pcr(type, pad, size);
}
...
static enum ec_error_list retrieve_nvmem_contents(void)
{
	...
	for (tries = 0; tries < 3; tries++) {
		...
		while ((rv = get_next_object(&controller_at, nc, false)) ==
			...
			case NN_OBJ_TPM_RESERVED:
				/**
				 * Don't check error as it is already reported
				 * and logged. Invalid object will be recreated
				 * later on nvmem_save(), when they are
				 * initialized during TPM manufacturing.
				 */
						  ...
						  size_t *outsize, int room)
{
	...
	*outsize = ri.size;
	if (ri.size) {
		...
		switch (index) {
		case NV_STATE_CLEAR:
			*outsize = marshal_state_clear(cached, *out, room);
			break;
		case NV_STATE_RESET:
			*outsize = marshal_state_reset_data(cached, *out, room);
			break;
					   ...
					   uint8_t *res_bitmap, int room)
{
	...
	/* For marshalled objects update how much room do we have. */
	if (room < ch->size + 1)
		room = 0;
	else
		room -= ch->size + 1;

	rv = get_reserved_from_cache(index, &marshalled, &new_size, room);
...
static bool compaction_needed(void)
{
	...
	return (controller_at.list_index >= compation_limit);
}

static enum ec_error_list create_reserved_object(size_t index,
						 struct nn_container *ch,
						 int room)
{
	...
	/* Check validness of input parameters. */
	if (index > UINT8_MAX || room < 1)
		return EC_ERROR_UNKNOWN;

	ch->container_type = ch->container_type_copy = NN_OBJ_TPM_RESERVED;
	container_body = (uint8_t *)(ch + 1);

	container_body[0] = index;
	out = container_body + 1;
	rv = get_reserved_from_cache(index, &out, &out_size, room);

	/* Check if this is an error or just an empty PCR. */
	if (rv != EC_SUCCESS ||
	    (index >= NV_VIRTUAL_RESERVE_LAST && is_empty(out, out_size)))
		return rv;
...
static enum ec_error_list new_nvmem_save_(void)
{
	...
	/* Save NV reserved space objects which are not in flash. */
	for (i = 0; i < NV_VIRTUAL_RESERVE_LAST + NUM_OF_PCRS; i++) {
		/**
		 * TODO: in tpm_registers.c `tpm_orderly_state_capture()`
		 * and `tpm_orderly_state_restore()` seems to handle
		 * NV_RAM_INDEX_SPACE preservation, so it can be
		 * excluded from saving.
		 * Check if NV_MAX_COUNTER needs saving.
		 */
		if (bitmap_bit_check(res_bitmap, i))
			continue;

		rv = create_reserved_object(
			i, ch, CONFIG_FLASH_BANK_SIZE - sizeof(*ch));
		if (rv != EC_SUCCESS)
			goto cleanup;
	}

	/* Now save new TPM objects, if any. */
...
cleanup:
	...
	if (del_candidates->num_candidates ||
		...
		/* TODO(b/320703802): implement proper error handling. */
		add_delimiter();
...
enum ec_error_list nvmem_erase_tpm_data_selective(const uint32_t *objs_to_erase)
{
	 ...
	 * This recreates TPM reserved objects to ensure correct operation.
	 */
