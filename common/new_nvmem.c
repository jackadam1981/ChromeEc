...
test_export_static enum ec_error_list compact_nvmem(void)
{
	...
	/* Check if we can start from empty page. To do this we need to have
	 * at least one free page and this makes sense only if we can write
	 * container + delimeter (2 * sizeof(*ch)) in the remaining space.
	 */
	...
	do {
		...
		/* We completed with first page, so can try to release it. */
		if (at.list_index != 0) {
			/*
