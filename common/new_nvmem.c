 ...
 *
 * Transaction delimtiers are represented by containers of the appropriate
 ...
 * transaction delimtier is written, ending up at the top of the used flash.
 * After that the objects scheduled for deltetion are deleted, and then the
 * transaction delimtier is also marked 'deleted'.
 *
 * So, during intialization the flash could be in one of three states:
 *
 * - thre is an erased transaction delimiter n the top
 ...
 *     the old instances might still be present not erased. The revovery
 ...
 *   . this i s the case where nvmem_save() was interrupted before all new
 *     values have been written into the flash. The recovery procdeure finds
 *     all TPM objecst about the last valid delimiter in the flash and erases
 *     them all.
 *
 * ===== Handling failures
 *
	...
	if (!at->mt.ph && togo) {
		...
		if (at->mt.ph->data_offset < (sizeof(*at->mt.ph) + togo)) {
			ccprintf("%s: size mismatch %d != %d!\n", __func__,
				 at->mt.ph->data_offset, at->mt.data_offset);
				   ...
				   struct nn_container *ch, int include_deleted)
{
	...
	do {
		...
		if (nvmem_read_bytes(at, sizeof(temp_ch), &temp_ch, 1) != sizeof(*ch)) {
			/* TODO: b/63760920 add entry to the log. */
			ccprintf("%s:%d\n", __func__, __LINE__);
		...
		/*
		 * The read data could be a container header, copy it into the
		 * user provided space and continue reading there.
		 */
		*ch = temp_ch;
...
test_export_static enum ec_error_list compact_nvmem(void)
{
	...
	before = master_at.list_index * CONFIG_FLASH_BANK_SIZE +
	...
	ccprintf("Compaction done, went from %d to %d bytes\n", before,
		 master_at.list_index * CONFIG_FLASH_BANK_SIZE +
			 master_at.mt.data_offset);
...
static enum ec_error_list migrate_tpm_reserved(struct nn_container *ch)
{
	...
	while (1) {
		...
		case NV_STATE_CLEAR:
			...
			ri.size = marshal_state_clear(scd, p_container_body + 1,
						      room);
			copy_needed = 0;
			break;
					...
					struct nn_container *ch)
{
	...
	if (memcmp(ch, flash_ch, sizeof(uint32_t))) {
		ccprintf("%s: pre-erase mismatch ptr %p, page %p, offset %d!\n",
			 __func__, flash_ch, at->ct.ph, at->ct.data_offset);
		return EC_ERROR_INVAL;
	}
	...
	if (del_candidates->num_candidates ==
		...
		/* TODO: b/63760920 add entry to the log. */
		ccprintf("%s:%d candidates array overflow!\n", __func__,
			 __LINE__);
		return EC_ERROR_INVAL;
	}
...
static enum ec_error_list verify_last_section(const struct page_tracker *prev_del,
					      struct nn_container *ch)
{
	/*
	...
	while (get_next_object(&at, ch, 0) == EC_SUCCESS) {
		...
		if (++(newobjs->num_objects) == ARRAY_SIZE(newobjs->objects)) {
			/* Should never happen... TODO b/63760920. */
	...
	/* Last object read from flash should have been a non-finalized delimiter. */
	if (ch->container_type != NN_OBJ_TRANSACTION_DEL) {
		/* TODO: b/63760920 add entry to the log, but this should never happen. */
		ccprintf("%s:%d inconsistent flash state!\n", __func__, __LINE__);
		return EC_ERROR_INVAL;
	}
	...
	while ((at.mt.ph != prev_del->ph) &&
		...
		for (i = 0, po = newobjs->objects;
		     i < newobjs->num_objects;
		     i++, po++) {
	...
	if (master_at.mt.data_offset > sizeof(struct nn_page_header)) {
		...
		top_del.data_offset = master_at.mt.data_offset - sizeof(struct nn_container);
	} else {
		top_del.ph = list_element_to_ph(master_at.list_index - 1);
		top_del.data_offset = CONFIG_FLASH_BANK_SIZE -  - sizeof(struct nn_container);
	}
...
static enum ec_error_list verify_delimiter(struct nn_container *nc)
{
	...
	if ((master_at.list_index == 0) &&
		...
		if (!master_at.dt.ph)
			...
			return EC_ERROR_INVAL; /* This is bad, will have to wipe
						  out everything. */
	}
	...
	if (rv == EC_ERROR_INVAL) {
		...
		remainder_size = CONFIG_FLASH_BANK_SIZE - master_at.ct.data_offset;
	...
	if (del_candidates->num_candidates ||
		...
		if (del_candidates->num_candidates) {
			...
			for (i = 0; i < del_candidates->num_candidates; i++)
				invalidate_object(del_candidates->candidates[i]);
		}
...
int nvmem_erase_tpm_data(void)
{
	...
	} while (master_at.list_index != (saved_list_index + 2));
