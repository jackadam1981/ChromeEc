...
test_export_static enum ec_error_list compact_nvmem(void)
{
	...
	/* Check if we can start from empty page. It makes sense if we are
	 * not at the start of the page already, have at least one free page
	 * and if we can write container + delimiter (2 * sizeof(*ch)) in
	 * the remaining space or just a delimiter.
	 */
	if (controller_at.mt.data_offset != sizeof(*controller_at.mt.ph) &&
		...
		/* 2 cases: >= 8 bytes, so we can create container + delimiter
		 * or == 4 bytes, so we can just add a final delimiter.
		 */
	...
	do {
		...
		/* We switched to next page, so can free first one(s). */
		if (at.list_index != 0) {
			/*
