...
static const uint8_t led_masks[POWERLED_COLORS] = {0xff, 0xfe, 0xfc, 0xfd};
static const char * const color_names[POWERLED_COLORS] = {
...
static int command_powerled(int argc, char **argv)
{
	...
	for (i = 0; i < POWERLED_COLORS; i++) {
		if (!strcasecmp(argv[1], color_names[i]))
			break;
	}
	if (i >= POWERLED_COLORS)
		return EC_ERROR_INVAL;
