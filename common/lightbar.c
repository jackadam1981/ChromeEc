...
#define CPRINTF(format, args...) info_printf(CC_LIGHTBAR, format, ## args)
...
static void lightbar_restore_state(void)
{
	...
	if (old_state && size == sizeof(st)) {
		...
		CPRINTF("LB state restored: %d %d - %d/%d",
...
void demo_battery_level(int inc)
{
	...
	CPRINTF("LB demo: battery_level=%d", st.battery_level);
}
...
void demo_is_charging(int ischarge)
{
	...
	CPRINTF("LB demo: battery_is_charging=%d",
...
static uint32_t sequence_STOP(void)
{
	...
	do {
		...
		CPRINTF("LB_stop got pending_msg %d", pending_msg);
...
static int lpc_cmd_lightbar(struct host_cmd_handler_args *args)
{
	...
	case LIGHTBAR_CMD_VERSION:
		...
		break;
	default:
		CPRINTF("LB bad cmd 0x%x", in->cmd);
