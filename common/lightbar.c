...
/* The battery charging task will update this for us. */
int battery_is_charging;			/* 0 - 1 */
...
static struct {
	...
	/* Remember this too, since it's updated infrequently. */
	int battery_percent;
} st;
...
{
	CPRINTF("[%T LB saving state]\n");
	st.battery_percent = battery_percent;
...
static void lb_restore_state(void)
{
	...
	if (old_state && size == sizeof(st)) {
		...
		/* Battery task may take longer to come up, so reset it. */
	...
	CPRINTF("[%T LB state: %d %d - %d %d]\n",
		...
		st.battery_level, st.battery_percent);
}
...
static const int pulse_stagger[2][NUM_LEDS] = {
	{ MSEC(0), MSEC(4800), MSEC(16000), MSEC(11000) }, /* discharging */
	{ MSEC(0), MSEC(2400), MSEC(8000), MSEC(5500) } /* charging */
};
...
static void pulse(timestamp_t now, int period_offset)
{
	...
	t = (now.le.lo + period_offset) % pulse_period[battery_is_charging];
	/* Convert phase to 0-255 */
	i = ((t >> 8) / (pulse_period[battery_is_charging] >> 16));
...
static uint32_t sequence_S0(void)
{
	...
	last_battery_is_charging = !battery_is_charging; /* force update */
	...
	while (1) {
		...
		if (battery_is_charging != last_battery_is_charging ||
			...
			last_battery_is_charging = battery_is_charging;
		...
		for (i = 0; i < NUM_LEDS; i++) {
			...
			/* Pulse sinusoidally */
			pulse(now, pulse_stagger[battery_is_charging][i]);
