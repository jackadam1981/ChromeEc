...
static void get_battery_level(void)
{
	...
	if (pwm_get_keyboard_backlight_enabled()) {
		...
		if (pct > st.p.bright_bl_on_max[ac])
			pct = st.p.bright_bl_on_max[ac];
		else if (pct < st.p.bright_bl_on_min[ac])
			pct = st.p.bright_bl_on_min[ac];
	} else
		pct = st.p.bright_bl_off_fixed[ac];
