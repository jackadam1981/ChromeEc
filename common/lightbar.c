...
static int lux_level_to_google_color(int lux)
{
	...
	if (!lid_is_open())
		/*
		 * The light sensor is shadded by the lid,
		 * use full brightness
		 */
		return 0;

	for (i = 0; i < lb_brighness_levels_count ; i++)
		if (lux >= lb_brighness_levels[i].lux)
			break;
	return i;
}
#endif
...
static void get_battery_level(void)
{
	...
	color_id = lux_level_to_google_color(
			motion_sensors[CONFIG_ALS_LIGHTBAR_DIMMING].raw_xyz[0]);

	if (color_id != last_google_color) {
		last_google_color = pct;
		memcpy(st.p.color, lb_brighness_levels[color_id].color,
		       sizeof(lb_brighness_levels[color_id].color));
	}
#endif
}
