...
/* Declare preserved pring buffer */
...
test_export_static void panic_log_init(void)
{
	...
	if (!panic_log_is_valid()) {
		...
		ccprintf("New panic detected, keeping panic log frozen\n");
		freeze = true;
	} else {
...
host_command_read_panic_log(struct host_cmd_handler_args *args)
{
	...
	if (offset >= panic_log_len())
		return EC_RES_INVALID_PARAM;

	while (offset < panic_log_len() &&
...
static int command_panic_log(int argc, const char **argv)
{
	if ((argc == 1) || (argc == 2 && !strcasecmp(argv[1], "info"))) {
		...
		for (int i = 0; i < panic_log_len(); i++) {
			char c = panic_log_read(i);
			if (c == 0)
				continue;
			ccprintf("%c", c);
			cflush();
	...
	} else if (argc == 2 && !strcasecmp(argv[1], "printk")) {
		printk("hello\n");
	} else {
