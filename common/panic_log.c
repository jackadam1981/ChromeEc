...
#ifdef SECTION_IS_RO
#error Panic log should not be enabled in RO images.
#endif
...
void panic_log_write_chars(const char *buf, size_t size)
{
	...
	for (int i = 0; i < size; i++)
		preserved_ram_ring_buf_write_char(panic_log, buf[i]);
}
...
{
	if (argc == 1) {
		int cur_frozen = panic_trace_freeze(true);
		frozen = true;
		ccprintf("vvvvvvvvvvvvv PANIC LOG vvvvvvvvvvvvv\n");
		for (int i = 0; i < preserved_ram_ring_buf_len_char(panic_log);
		     i++) {
		...
		if (!strcasecmp(argv[1], "reset")) {
			...
			char *e;
			int measure_count = panic_log->noinit->buffer_size;
			if (argc == 3) {
				measure_count = strtoi(argv[2], &e, 10);
			}
			bool disable_interrupts = true;
			MEASURE(panic_log_write_char, 'x', measure_count,
				disable_interrupts);
			MEASURE(panic_log_write_char, 'x', measure_count,
				disable_interrupts);
			MEASURE(panic_log_write_uint16, 0x1234, measure_count,
				disable_interrupts);
			MEASURE(panic_log_write_uint32, 0x12345678,
				measure_count, disable_interrupts);
			MEASURE(panic_log_write_uint64, 0x1234567812345678,
				measure_count, disable_interrupts);
