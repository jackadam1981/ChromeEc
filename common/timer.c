...
timestamp_t get_time(void)
{
	...
	if (IS_ENABLED(CONFIG_HWTIMER_64BIT)) {
		...
		/*
		 * TODO(patrykd) If statement below doesn't catch overflows
		 * when interrupts are disabled or currently processed
...
void force_time(timestamp_t ts)
{
	...
	int interrupt_enabled = is_interrupt_enabled();

	/*
	 * Updating timer shouldn't be interrupted (eg. when counter overflows)
	 * because it could lead to some unintended consequences. Please note
	 * that this function can be called with disabled or enabled interrupts
	 * so we need to restore the original state later.
	 */
	interrupt_disable();
