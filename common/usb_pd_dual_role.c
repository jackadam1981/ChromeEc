		      ...
		      uint32_t *mv, int port)
{
	...
	if (charging_allowed && max_request_allowed) {
		if (charge_get_pps_mode(port) ==
				   CHARGE_PPS_MODE_HARDWARE) {
			pdo_index = pd_find_apdo_index(src_cap_cnt, src_caps,
					   charge_get_pps_voltage(port), 3000, &pdo);
			max_request_mv = charge_get_pps_voltage(port);
		} else if (charge_get_pps_mode(port) ==
			   CHARGE_PPS_MODE_SOFTWARE) {
			uint32_t unused1, unused2;

			pdo_index = pd_find_pdo_index(src_cap_cnt, src_caps,
					  charge_get_pps_voltage(port), &pdo);
			pd_extract_pdo_power(src_caps[pdo_index], &unused1,
				     &max_request_mv, &unused2);
		} else {
	...
	if ((pdo & PDO_TYPE_MASK) == PDO_TYPE_BATTERY) {
		...
		*ma = 3000;
		*mv = max_request_mv;
		*rdo = PRDO_PPS(pdo_index + 1, max_request_mv, *ma, flags);
	}else {
