...
{
	return priv.capabilities & BIT(cap);
}

static int wov_get_capabilities(struct host_cmd_handler_args *args)
{
	struct ec_response_ec_codec_wov_get_capabilities *r = args->response;

	r->capabilities = priv.capabilities;

	args->response_size = sizeof(*r);
	return EC_RES_SUCCESS;
}

static int wov_get_shm_addr(struct host_cmd_handler_args *args)
{
	...
	struct ec_param_ec_codec_wov_get_shm_addr *pp = &p->get_shm_addr_param;
	...
	case EC_CODEC_WOV_SHM_ID_AUDIO:
		r->phys_addr = (uint64_t)priv.driver.audio_buf_addr;
		r->len = priv.driver.audio_buf_len;
		r->type = priv.driver.audio_buf_type;
		break;
	case EC_CODEC_WOV_SHM_ID_LANG:
		r->phys_addr = (uint64_t)priv.driver.lang_buf_addr;
		r->len = priv.driver.lang_buf_len;
		r->type = priv.driver.lang_buf_type;
		break;
	}

	if (r->type == EC_CODEC_WOV_SHM_TYPE_SYSTEM_RAM &&
			priv.driver.translate_addr_ec_to_ap) {
		int ret;
		uintptr_t addr;

		ret = priv.driver.translate_addr_ec_to_ap(r->phys_addr, &addr);
		if (ret)
			return ret;
...
static int wov_is_hotword_detected(struct host_cmd_handler_args *args)
{
	...
	mutex_lock(&priv.lock);
	r->detected = priv.hotword_detected;
	mutex_unlock(&priv.lock);
...
static int wov_reset(struct host_cmd_handler_args *args)
{
...
static int (*sub_cmds[])(struct host_cmd_handler_args *) = {
	[EC_CODEC_WOV_GET_CAPABILITIES] = wov_get_capabilities,
	[EC_CODEC_WOV_GET_SHM_ADDR] = wov_get_shm_addr,
	...
	[EC_CODEC_WOV_IS_HOTWORD_DETECTED] = wov_is_hotword_detected,
	[EC_CODEC_WOV_RESET] = wov_reset,
...
int audio_codec_register_wov_driver(struct audio_codec_wov_driver *driver)
{
	...
	if (driver->translate_addr_ec_to_ap)
		priv.driver.translate_addr_ec_to_ap =
			driver->translate_addr_ec_to_ap;
...
void audio_codec_wov_task(void *arg)
{
	...
	while (1) {
		...
		if (GoogleHotwordDspProcess(p + priv.audio_buf_wp, n, &ret)) {
			DBG("hotword detected");
