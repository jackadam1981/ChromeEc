...
{
	return ((audio_buf_wp + 1) % AUDIO_BUF_LEN) == audio_buf_rp;
}
...
#ifndef CONFIG_AUDIO_CODEC_CAP_WOV_LANG_SHM
...
#ifndef CONFIG_AUDIO_CODEC_CAP_WOV_AUDIO_SHM
...
void audio_codec_wov_task(void *arg)
{
	...
	while (1) {
		...
		if (GoogleHotwordDspProcess(p, n / 2, &r)) {
			...
			mutex_lock(&lock);
			audio_buf_rp = audio_buf_wp + (AUDIO_BUF_LEN / 2 * 0.8);
			if (audio_buf_rp >= AUDIO_BUF_LEN)
