...
static int check_lang_hash(uint8_t *data, uint32_t len, const uint8_t *hash)
{
	/*
...
static int check_lang_buf_tail_zero(uint32_t total_len)
{
	int i;
	uint8_t *p = (uint8_t *)audio_codec_wov_lang_buf_addr;

	for (i = total_len; i < LANG_BUF_LEN; ++i)
		if (p[i])
...
static int wov_set_lang(struct host_cmd_handler_args *args)
{
	...
	if (pp->offset + pp->len == pp->total_len) {
		...
		if (check_lang_buf_tail_zero(pp->total_len) != EC_SUCCESS)
			return EC_RES_ERROR;
...
void audio_codec_wov_task(void *arg)
{
	...
	while (1) {
		...
		/*
		 * GoogleHotwordDspProcess() needs numbers of sample.  In the
		 * case, sample is S16_LE.  Thus, n / 2.
		 */
		if (GoogleHotwordDspProcess(p, n / 2, &r)) {
