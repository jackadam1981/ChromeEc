...
/*
 * To shorten the variable names, or the existing code is likely to greater
 * than 80 columns.
 */
...
static int check_lang_buf(uint32_t total_len)
{
	int i;
...
static int wov_set_lang(struct host_cmd_handler_args *args)
{
	...
	if (pp->offset + pp->len == pp->total_len) {
		...
		if (check_lang_buf(pp->total_len) != EC_SUCCESS)
			return EC_RES_ERROR;
