...
static struct {
	...
	/* shared data between host command and task */
	uint8_t wov_enabled;
	uint8_t hotword_detected;
	uint32_t audio_buf_rp;
	uint32_t audio_buf_wp;
} priv;
...
static int check_lang_hash(uint8_t *data, uint32_t len, uint8_t hash[32])
{
