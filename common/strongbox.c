 ...
 */
static enum strongbox_error generate_key(struct km *km, uint32_t *buf,
 ...
 */
static enum strongbox_error begin_operation(struct km *km, uint32_t *buf,
					    size_t buf_size_words,
					    size_t req_len_words,
					    size_t *out_len_bytes)
{
 ...
 */
static enum strongbox_error update_operation(struct km *km, uint32_t *buf,
					     size_t buf_size_words,
					     size_t req_len_words,
					     size_t *out_len_bytes)
{
 ...
 */
static enum strongbox_error finish_operation(struct km *km, uint32_t *buf,
					     size_t buf_size_words,
					     size_t req_len_words,
					     size_t *out_len_bytes)
{
 ...
 * byte[] generateEcdsaP256KeyPair(in boolean testMode, out MacedPublicKey
   macedPublicKey);
*/
static enum strongbox_error generate_key_pair(struct km *km, uint32_t *buf,
					      size_t buf_size_words,
					      size_t req_len_words,
					      size_t *out_len_bytes)
{
 ...
 * challenge);
 */
static enum strongbox_error generate_cert_req(struct km *km, uint32_t *buf,
					      size_t buf_size_words,
					      size_t req_len_words,
					      size_t *out_len_bytes)
{
	size_t maced_len;
	size_t challenge_len, challenge_words, uds_cert_len;
	enum dcrypto_result result;
	const struct sha256_digest *digest;
	struct hmac_sha256_ctx sha;
	...
	*out_len_bytes = 0;
	...
	b8[1] = CBOR_UINT0(1);
	/* Copy UdsCerts */
	memcpy(b8 + 2, buf + 27, uds_cert_len);
	/* DiceCertChain */
	return SB_OK;
}
