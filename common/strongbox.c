					...
					const uint32_t *p_tag)
{
	...
	case KM_TAG_DIGEST:
		...
		break;
	case KM_TAG_OS_VERSION:
		if (params->osVersion != 0)
			return SBERR_InvalidTag;
		params->osVersion = p_tag[1];
		break;
	case KM_TAG_OS_PATCHLEVEL:
		if (params->osPatchLevel != 0)
			return SBERR_InvalidTag;
		params->osPatchLevel = p_tag[1];
		break;
	case KM_TAG_BOOT_PATCHLEVEL:
		if (params->bootPatchLevel != 0)
			return SBERR_InvalidTag;
		params->bootPatchLevel = p_tag[1];
		break;
	case KM_TAG_VENDOR_PATCHLEVEL:
		if (params->vendorPatchLevel != 0)
			return SBERR_InvalidTag;
		params->vendorPatchLevel = p_tag[1];
		break;
	case KM_TAG_ORIGIN:
		params->origin = p_tag[1];
		break;
	...
	case KM_TAG_NO_AUTH_REQUIRED:
		if (params->noAuthRequired)
			return SBERR_InvalidTag;
		params->noAuthRequired = true;
		break;
	case KM_TAG_INCLUDE_UNIQUE_ID:
		if (params->includeUniqueId)
			return SBERR_InvalidTag;
		params->includeUniqueId = true;
		break;
	case KM_TAG_EARLY_BOOT_ONLY:
		if (params->earlyBootOnly)
			return SBERR_InvalidTag;
		params->earlyBootOnly = true;
		break;
	case KM_TAG_PURPOSE: {
	...
	case KM_TAG_APPLICATION_ID:
		if (params->application_id.p != NULL)
			return SBERR_InvalidTag;
		params->application_id =
			SLICE(&p_tag[1], prefix_get_data_len_bytes(tag_word));
		break;
	case KM_TAG_APPLICATION_DATA:
		if (params->application_data.p != NULL)
			return SBERR_InvalidTag;
		params->application_data =
			SLICE(&p_tag[1], prefix_get_data_len_bytes(tag_word));
		break;
	case KM_TAG_ROOT_OF_TRUST:
		if (params->rootOfTrust.p != NULL)
			return SBERR_InvalidTag;
		params->rootOfTrust =
			SLICE(&p_tag[1], prefix_get_data_len_bytes(tag_word));
		break;
	case KM_TAG_ATTESTATION_CHALLENGE:
		if (params->attestationChallenge.p != NULL)
			return SBERR_InvalidTag;
		params->attestationChallenge =
			SLICE(&p_tag[1], prefix_get_data_len_bytes(tag_word));
		break;
	case KM_TAG_ATTESTATION_ID_BRAND:
		if (params->attestationIdBrand.p != NULL)
			return SBERR_InvalidTag;
		params->attestationIdBrand =
			SLICE(&p_tag[1], prefix_get_data_len_bytes(tag_word));
		break;
	case KM_TAG_ATTESTATION_ID_DEVICE:
		if (params->attestationIdDevice.p != NULL)
			return SBERR_InvalidTag;
		params->attestationIdDevice =
			SLICE(&p_tag[1], prefix_get_data_len_bytes(tag_word));
		break;
	case KM_TAG_ATTESTATION_ID_PRODUCT:
		if (params->attestationIdProduct.p != NULL)
			return SBERR_InvalidTag;
		params->attestationIdProduct =
			SLICE(&p_tag[1], prefix_get_data_len_bytes(tag_word));
		break;
	case KM_TAG_ATTESTATION_ID_SERIAL:
		if (params->attestationIdSerial.p != NULL)
			return SBERR_InvalidTag;
		params->attestationIdSerial =
			SLICE(&p_tag[1], prefix_get_data_len_bytes(tag_word));
		break;
	case KM_TAG_ATTESTATION_ID_IMEI:
		if (params->attestationIdImei.p != NULL)
			return SBERR_InvalidTag;
		params->attestationIdImei =
			SLICE(&p_tag[1], prefix_get_data_len_bytes(tag_word));
		break;
	case KM_TAG_ATTESTATION_ID_MEID:
		if (params->attestationIdMeid.p != NULL)
			return SBERR_InvalidTag;
	...
	case KM_TAG_ATTESTATION_ID_MANUFACTURER:
		if (params->attestationIdManufacturer.p != NULL)
			return SBERR_InvalidTag;
	...
	case KM_TAG_ATTESTATION_ID_MODEL:
		if (params->attestationIdModel.p != NULL)
			return SBERR_InvalidTag;
	...
	case KM_TAG_CERTIFICATE_SUBJECT:
		if (params->certificate_subject.p != NULL)
			return SBERR_InvalidTag;
		params->certificate_subject =
			SLICE(&p_tag[1], prefix_get_data_len_bytes(tag_word));
		break;
	case KM_TAG_CERTIFICATE_SERIAL:
		if (params->certificate_serial.p != NULL)
			return SBERR_InvalidTag;
	...
	case KM_TAG_CERTIFICATE_NOT_AFTER:
		if (params->date_not_after != 0)
			return SBERR_InvalidTag;
		params->date_not_after = make64(p_tag[2], p_tag[1]);
		break;
	case KM_TAG_CERTIFICATE_NOT_BEFORE:
		if (params->date_not_before != 0)
			return SBERR_InvalidTag;
		params->date_not_before = make64(p_tag[2], p_tag[1]);
		break;
	case KM_TAG_CREATION_DATETIME:
		if (params->creationDateTime != 0)
			return SBERR_InvalidTag;
		params->creationDateTime = make64(p_tag[2], p_tag[1]);
		break;
					...
					uint32_t *key)
{
	...
	result = cryptokey_import_bound(
		km, buf, tag_words, params->application_id.p,
		params->application_id.n, params->application_data.p,
		params->application_data.n, buf + tag_words,
	...
	uint32_t *out_buf, size_t *out_words, enum attest attest)
{
	...
	result = cryptokey_export_bound(
		...
		params.application_id.p, params.application_id.n,
		params.application_data.p, params.application_data.n,
			     ...
			     const struct km_key_params *params)
{
	...
	SEQ_START(*ctx, V_ASN1_BYTES, SEQ_LARGE)
	{
		SEQ_START(*ctx, V_SEQ, SEQ_LARGE)
		{
			...
			asn1_int(ctx, 400); // keyMintVersion
			...
			{
				add_key_purpose_ext(
					ctx, params->attrs.purpose_flags);
				asn1_explicit_int(ctx, 2,
						  params->attrs.algorithm);
				if (params->attrs.key_size)
					asn1_explicit_int(
						ctx, 3, params->attrs.key_size);
				...
				if (params->osVersion)
					asn1_explicit_int(ctx, 705,
							  params->osVersion);
				if (params->osPatchLevel)
					asn1_explicit_int(ctx, 706,
							  params->osPatchLevel);
		...
		SEQ_START(ctx, V_SEQ, SEQ_LARGE)
		{
			...
			SEQ_START(ctx, V_ASN1_CCL | 3, SEQ_LARGE)
			{
				...
				SEQ_START(ctx, V_SEQ, SEQ_LARGE)
				{
					 ...
					 * (1..MAX) OF Extension Extension  ::=
					 * SEQUENCE  { extnID      OBJECT
					 * IDENTIFIER, critical    BOOLEAN
					 * DEFAULT FALSE, extnValue   OCTET
					 * STRING }
					 */
					SEQ_START(ctx, V_SEQ, SEQ_SMALL)
					{
