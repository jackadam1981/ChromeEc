					...
					const uint32_t *p_tag)
{
	...
	case KM_TAG_ALGORITHM:
		if (attrs->algorithm != 0)
			return SBERR_InvalidTag;
		attrs->algorithm = (keymint_algorithm_t)p_tag[1];
		break;
	case KM_TAG_KEY_SIZE:
		if (attrs->key_size != 0)
			return SBERR_InvalidTag;
		attrs->key_size = p_tag[1];
		break;
	case KM_TAG_EC_CURVE:
		if (attrs->curve_id != 0)
			return SBERR_InvalidTag;
		attrs->curve_id = p_tag[1];
		break;
	case KM_TAG_RSA_PUBLIC_EXPONENT:
		if (attrs->rsa_exponent != 0)
			return SBERR_InvalidTag;
		memcpy(&attrs->rsa_exponent, &p_tag[1], sizeof(uint64_t));
		break;
	...
	case KM_TAG_PURPOSE: {
		...
		if (purpose >= 32)
			return SBERR_InvalidTag;
		attrs->purpose_flags |= (1 << purpose);
		break;
	}
	case KM_TAG_APPLICATION_ID:
		if (attrs->application_id != NULL)
			return SBERR_InvalidTag;
		attrs->application_id_len = prefix_get_data_len_bytes(tag_word);
		attrs->application_id = (const uint8_t *)&p_tag[1];
		break;
	case KM_TAG_APPLICATION_DATA:
		if (attrs->application_data != NULL)
			return SBERR_InvalidTag;
		attrs->application_data_len =
			prefix_get_data_len_bytes(tag_word);
		attrs->application_data = (const uint8_t *)&p_tag[1];
		break;
	case KM_TAG_ATTESTATION_CHALLENGE:
		if (attrs->attestation_challenge != NULL)
			return SBERR_InvalidTag;
		attrs->attestation_challenge_len =
			prefix_get_data_len_bytes(tag_word);
		attrs->attestation_challenge = (const uint8_t *)&p_tag[1];
		break;
...
static enum strongbox_error process_gen_import_tags(
	keymint_t *km, key_attributes_t *attrs, const uint32_t *buf,
	...
	size_t *out_written_words)
{
	...
	while (tag_pos < tags_len_words + 1) {
		...
		err = process_tag(tag, attrs, p_tag);
		if (err != SB_OK)
			return err;
				  ...
				  size_t *out_len_bytes)
{
	...
	if (attrs.algorithm == ALGORITHM_RSA) {
		if (attrs.key_size != 1024 && attrs.key_size != 2048)
			return SBERR_UnsupportedKeySize;
		if (attrs.rsa_exponent != 3 && attrs.rsa_exponent != 65537)
			return SBERR_InvalidArgument;
		return SBERR_Unimplemented;
	} else {
		uint32_t key[8];
		...
		if (attrs.algorithm == ALGORITHM_AES) {
			if ((attrs.key_size != 128 && attrs.key_size != 192 &&
			     attrs.key_size != 256)) {
				return SBERR_UnsupportedKeySize;
			}
		}
		...
		result = cryptokey_export_bound(
			km, attrs.algorithm, out_buf, blob_start_words,
			attrs.application_id, attrs.application_id_len,
			attrs.application_data, attrs.application_data_len,
		...
		result = cryptokey_import_bound(
			km, out_buf, blob_start_words, attrs.application_id,
			attrs.application_id_len, attrs.application_data,
			attrs.application_data_len, &out_buf[blob_start_words],
			blob_size_words, key);
		if (result != DCRYPTO_OK)
			return SBERR_UnknownError;
	}
...
*/
enum strongbox_error begin_operation(keymint_t *km, uint32_t *buf,
				     ...
				     size_t *out_len_bytes)
{
	...
	CPRINTS("tag-pos = %u, purpose=%08x, adata=%ph, aid=%ph", tag_pos,
		attrs.purpose_flags,
		HEX_BUF(attrs.application_data, attrs.application_data_len),
		HEX_BUF(attrs.application_id, attrs.application_id_len));

	if ((attrs.purpose_flags & (1 << purpose)) == 0)
		return SBERR_IncompatiblePurpose;

	result = cryptokey_import_bound(
		km, buf + 1, tag_words, attrs.application_id,
		attrs.application_id_len, attrs.application_data,
		attrs.application_data_len, buf + tag_words + 1,
