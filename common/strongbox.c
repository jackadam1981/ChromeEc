				  ...
				  size_t *out_len_bytes)
{
	...
	err = process_gen_import_tags(km, &params, KEY_ORIGIN_GENERATED, buf,
				      req_len_words, out_buf,
				      ARRAY_SIZE(out_buf), &blob_start_words);
	if (err != SB_OK)
		return err;

	if (blob_start_words >= ARRAY_SIZE(out_buf))
	...
	memcpy(buf + 1, out_buf, total_words * sizeof(uint32_t));
	/* Size of key blob in 32-bit words */
	buf[0] = total_words;
	/* Placeholder for the certificate size */
	buf[total_words + 1] = 0;
	*out_len_bytes = total_words * sizeof(uint32_t) + 2* sizeof(uint32_t);
					...
					key_parameters_t *params, uint32_t *key)
{
	...
	CPRINTS("import_blob %u, %u, %u, %u, %u", blob_words, hw_tag_words,
		tag_words, key_blob_size, key_blob_size / sizeof(uint32_t) + tag_words);
