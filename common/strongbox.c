	...
	if (attest) {
		...
		if (attest_key_space >= 1) {
			...
			err = import_blob(km, tags + tags[0] + 2,
					  ...
					  &sign_params, attest_key);
			if (err != SB_OK)
				return err;
			/* TODO: Add certificate generation with `attest_key` */
		}
	}
						      ...
						      size_t *out_len_bytes)
{
	...
	*b8++ = CBOR_HDR1(CBOR_MAJOR_BSTR, 3);
	*b8++ = CBOR_HDR1(CBOR_MAJOR_MAP, 1);
	*b8++ = CBOR_HDR1(CBOR_MAJOR_UINT, 1);
