...
static size_t SB_cert_name(const p256_int *d, const p256_int *pk_x,
			   const p256_int *pk_y, struct km_key_params *params,
					...
					const uint32_t *p_tag)
{
	...
	case KM_TAG_CERTIFICATE_SERIAL:
		if (params->certificate_serial != NULL)
			return SBERR_InvalidTag;
	...
	uint32_t *out_buf, size_t *out_words, enum attest attest)
{
	...
	*key_blob_size = total_words;
	...
	if (attest == ATTEST_TRUE) {
		/**
		 * Tag::CERTIFICATE_SERIAL specifies the serial number to be
		 * assigned to the attestation certificate to be generated for
		 * the given key.  This parameter should only be passed to
		 * keyMint in the attestation parameters during generateKey()
		 * and importKey().  If not provided, the serial shall default
		 * to 1.
		 */
		if (!params.certificate_serial) {
			params.certificate_serial = (const uint8_t *)"\0x01";
			params.certificate_serial_len = 1;
		}
		/**
		 ...
		 */
		if (!params.certificate_subject) {
			static const char cname[] = "Android Keystore Key";
			params.certificate_subject = cname;
			params.certificate_subject_len = sizeof(cname) - 1;
		}
 ...
 */
// static const uint8_t OID_keyConstraints[3] = { 0x55, 0x1D, 0x13 };
...
/* DER encode bytes */
static void asn1_bytes(struct asn1 *ctx, uint8_t tag, const void *d, size_t n)
{
	uint8_t *p = asn1_tag(ctx, tag);
			   ...
			   uint8_t *cert, const size_t n)
{
	...
	{ /* outer seq */
		...
		SEQ_START(ctx, V_SEQ, SEQ_LARGE)
		{
			...
			/* Serial number */
			if (params->certificate_serial)
				asn1_be_int(&ctx, params->certificate_serial,
					    params->certificate_serial_len);
			else
				asn1_int(&ctx, 1);
			 ...
			 * attestation key.
			 */
			add_name(&ctx, "Google LLC", "CR50", 4);
			...
			/* Subject */
			add_name(&ctx, "StrongBox", params->certificate_subject,
				 params->certificate_subject_len);
