...
int pd_chk_vbus_level(int port, enum vbus_level level)
{
	if (IS_ENABLED(CONFIG_USB_PD_VBUS_DETECT_TCPC))
		return tcpm_chk_vbus_level(port, level);
	else if (level == VBUS_LEVEL_SAFE5V)
...
{
	return pd_chk_vbus_level(port, VBUS_LEVEL_SAFE5V);
}
