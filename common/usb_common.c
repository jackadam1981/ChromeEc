...
int usb_get_battery_soc(void)
{
	...
	(defined(PD_PREFER_HIGH_VOLTAGE) && defined(PD_PREFER_LOW_VOLTAGE)) || \
	(defined(CONFIG_USB_PD_PREFER_MV) && defined(PD_PREFER_HIGH_VOLTAGE))
#error "PD preferred voltage strategy should be mutually exclusive."
#endif

STATIC_IF_NOT(CONFIG_USB_PD_PREFER_MV)
struct pd_pref_config_t __attribute__((unused)) pd_pref_config;
					...
					int max_mv, uint32_t *selected_pdo)
{
	...
	if (IS_ENABLED(CONFIG_USB_PD_PREFER_MV))
		desired_uw = charge_get_plt_and_desired_mw() * 1000;
