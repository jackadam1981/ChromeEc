...
hc_regulator_set_enable(struct host_cmd_handler_args *args)
{
	...
	/*
	 * TODO(pihsun): Should the index be allowed to have "holes"?
	 */
	if (p->index >= CONFIG_REGULATOR_COUNT)
		return EC_RES_INVALID_PARAM;

	rv = board_regulator_set_enable(p->index, p->enabled);
...
hc_regulator_is_enabled(struct host_cmd_handler_args *args)
{
	...
	r->enabled = board_regulator_is_enabled(p->index);
...
hc_regulator_get_voltage(struct host_cmd_handler_args *args)
{
	...
	r->selector = board_regulator_get_voltage(p->index);
