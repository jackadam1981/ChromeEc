...
static uint32_t events[DIV_ROUND_UP(EC_MKBP_EVENT_COUNT, 32)];

static void set_event(uint8_t event_type)
{
	atomic_or(&events[event_type/32], (1 << (event_type & 31)));
}

static void clear_event(uint8_t event_type)
{
	atomic_clear(&events[event_type/32], (1 << (event_type & 31)));
}

static int event_is_set(uint8_t event_type)
{
	return events[event_type/32] & (1 << (event_type & 31));
}
