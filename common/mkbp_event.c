 ...
 */
static host_event_t active_wake_mask = CONFIG_MKBP_WAKEUP_MASK;
#endif
...
#ifdef CONFIG_MKBP_WAKEUP_MASK
inline host_event_t mkbp_get_active_wake_mask(void)
{
	return active_wake_mask;
}
inline void mkbp_set_active_wake_mask(host_event_t mask)
{
	active_wake_mask = CONFIG_MKBP_WAKEUP_MASK & mask;
}
/* For boards that do not support UHEPI. */
