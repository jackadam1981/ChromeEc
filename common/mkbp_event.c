 ...
 *
 * The transition state (INTERRUPT_INACTIVE_2_ACTIVE) is used to track the
...
enum interrupt_state {
	INTERRUPT_INACTIVE,
	INTERRUPT_INACTIVE_2_ACTIVE, /* Transitioning */
	INTERRUPT_ACTIVE,
};
...
struct mkbp_state {
	...
	/*
	 * Tracks unique transitions to INTERRUPT_INACTIVE_2_ACTIVE allowing
	 * only the most recent transition to finish the transition to a final
	 * state, ether active or inactive depending on the result of the
	 * operation.
	 */
...
/*
 * This is the deferred function that ensures that we attempts to set the MKBP
...
static void activate_mkbp_with_events(uint32_t events_to_add)
{
	...
	if (schedule_deferred) {
		hook_call_deferred(&force_mkbp_if_events_data, 500 * MSEC);
		if (rv != EC_SUCCESS)
...
static int set_inactive_if_no_events(void)
{
	...
	/* Only simple tasks (i.e. gpio set or no-op) are allowed here */
