...
static int command_espi_oob(int argc, char **argv)
{
	...
	if (argc == 2) {
		if (!strcasecmp(argv[1], "temp"))
			cmd = OOB_SMBUS_GET_TEMP;
		else if (!strcasecmp(argv[1], "rtc"))
			cmd = OOB_SMBUS_GET_RTC;
		...
		/* tag + len[11:8] */
		espi_oob_data[1] = ESPI_TAG_LEN_FIELD(0, ESPI_OOB_DATA_LEN);
		/* len[7:0] */
		espi_oob_data[2] = ESPI_OOB_DATA_LEN;
		/* SMBus destination addr */
		espi_oob_data[3] = OOB_SMBUS_DEST_ADDR_PCH;
		/* SMBus cmd */
		espi_oob_data[4] = cmd;
		/* SMBus byte count */
		espi_oob_data[5] = OOB_SMBUS_BYTE_CNT;
		/* SMBus source addr */
		espi_oob_data[6] = OOB_SMBUS_SRC_ADDR_EC;
