...
#ifdef CONFIG_HOSTCMD_ESPI_OOB
static int command_espioob_init(int argc, char **argv)
{
	char *pCmd[2] = {"rtc",
			 "temp"};
	...
	if (**argv == *pCmd[0])
		espi_oob_data[4] = 0x02; /* SMBus cmd: get RTC */
	else if (**argv == *pCmd[1])
		espi_oob_data[4] = 0x01; /* SMBus cmd: get temperature */
	espi_oob_data[5] = 0x01; /* SMBus byte count */
	espi_oob_data[6] = 0x1f; /* SMBus source addr */

	return espi_oob_send(espi_oob_data);
}
DECLARE_CONSOLE_COMMAND(rtc, command_espioob_init,
			NULL, "Send ESPI OOB RTC command");

DECLARE_CONSOLE_COMMAND(temp, command_espioob_init,
			NULL, "Send ESPI OOB temperature command");
#endif
