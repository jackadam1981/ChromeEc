...
/* TODO: */
#if 0
static void process_crashlog_data(uint8_t len, uint8_t *dataptr)
{
}
#endif
#endif
...
void host_command_task(void *u)
{
	...
	while (1) {
		...
		if (evt & TASK_EVENT_ESPI_OOB_RECEIVE) {
			...
			CPRINTS("ESPI OOB message received, len=0x%x", oob_len);
		}
...
#ifdef CONFIG_HOSTCMD_ESPI_OOB
static int command_espioob_crashlog(int argc, char **argv)
{
	/* ADD data */
	espi_oob_send(espi_oob_data);

	return EC_SUCCESS;
}
DECLARE_CONSOLE_COMMAND(crashlog, command_espioob_crashlog,
			NULL, "Send ESPI OOB crashlog command");
#endif
