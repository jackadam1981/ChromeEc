...
void host_command_received(int slot, int command)
{
	...
	if (command == EC_CMD_REBOOT) {
		...
		host_send_response(slot, -EC_RES_ERROR, NULL);
		return;
	}
...
static int host_command_proto_version(uint8_t *data)
{
	...
	return sizeof(struct ec_response_proto_version);
}
...
static int host_command_hello(uint8_t *data)
{
	...
	return sizeof(struct ec_response_hello);
}
...
static int host_command_read_test(uint8_t *data)
{
	...
	return sizeof(struct ec_response_read_test);
}
...
static void command_process(int slot)
{
	...
	if (cmd)
		...
		host_send_response(slot, -EC_RES_INVALID_COMMAND, NULL);
}
