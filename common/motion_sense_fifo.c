...
struct fifo_staged {
	uint32_t read_ts;
	uint16_t count;
	uint8_t sample_count[ALL_MOTION_SENSORS];
 ...
 */
static struct timestamp_state next_timestamp[ALL_MOTION_SENSORS];
...
static void fifo_pop(void)
{
	...
	if (--fifo_staged.sample_count[head->sensor_num] < 2) {
		...
		for (i = 0; i < ALL_MOTION_SENSORS; i++) {
...
{
	return sensor_num < ALL_MOTION_SENSORS &&
...
void motion_sense_fifo_commit_data(void)
{
	...
	static uint32_t data_periods[ALL_MOTION_SENSORS];
	struct ec_response_motion_sensor_data *data;
