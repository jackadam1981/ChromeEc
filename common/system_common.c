...
static int host_command_get_version(uint8_t *data)
{
	...
	return sizeof(struct ec_response_get_version);
}
...
static int host_command_build_info(uint8_t *data)
{
	...
	return sizeof(struct ec_response_get_build_info);
}
...
static int host_command_get_chip_info(uint8_t *data)
{
	...
	return sizeof(struct ec_response_get_chip_info);
}
...
#ifdef CONFIG_LPC
	host_send_response(0, EC_RES_SUCCESS, NULL);
	host_send_response(1, EC_RES_SUCCESS, NULL);
#endif
}
