...
struct jump_data {
	...
	uint8_t startup_required; /* Start x86 if true */
			  ...
			  int startup_required)
{
	...
	jdata->startup_required = startup_required != 0;
			  ...
			  int startup_required)
{
	...
	jump_to_image(init_addr, recovery_required, startup_required);
...
static int command_sysjump(int argc, char **argv)
{
	...
	if (!strcasecmp(argv[1], "RO")) {
		return system_run_image_copy(SYSTEM_IMAGE_RO, 0, 0);
	} else if (!strcasecmp(argv[1], "A")) {
		return system_run_image_copy(SYSTEM_IMAGE_RW_A, 0, 0);
	} else if (!strcasecmp(argv[1], "B")) {
		return system_run_image_copy(SYSTEM_IMAGE_RW_B, 0, 0);
	}
...
enum lpc_status host_command_reboot(uint8_t *data)
{
	...
	int start_request = p->reboot_flags &
		EC_LPC_COMMAND_REBOOT_BIT_START_UP;
	...
	system_run_image_copy(copy, recovery_request, start_request);
