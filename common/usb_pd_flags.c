...
enum usb_pd_vbus_detect get_usb_pd_vbus_detect(void)
{
	if (IS_ENABLED(CONFIG_USB_PD_RUNTIME_FLAGS))
		return usb_pd_flags.vbus_detect;
	else if (IS_ENABLED(CONFIG_USB_PD_VBUS_DETECT_TCPC))
		return USB_PD_VBUS_DETECT_TCPC;
	else if (IS_ENABLED(CONFIG_USD_PD_VBUS_DETECT_GPIO))
		return USB_PD_VBUS_DETECT_GPIO;
	else if (IS_ENABLED(CONFIG_USB_PD_VBUS_DETECT_PPC))
		return USB_PD_VBUS_DETECT_PPC;
	else if (IS_ENABLED(CONFIG_USB_PD_VBUS_DETECT_CARGER))
		return USB_PD_VBUS_DETECT_CHARGER;
	else
		return USB_PD_VBUS_DETECT_NONE;
}

enum usb_pd_discharge get_usb_pd_discharge(void)
{
	if (IS_ENABLED(CONFIG_USB_PD_RUNTIME_FLAGS))
		return usb_pd_flags.discharge;
	else if (IS_ENABLED(CONFIG_USB_PD_DISCHARGE_TCPC))
		return USB_PD_DISCHARGE_TCPC;
	else if (IS_ENABLED(CONFIG_USD_PD_DISCHARGE_GPIO))
		return USB_PD_DISCHARGE_GPIO;
	else if (IS_ENABLED(CONFIG_USB_PDDISCHARGE_PPC))
		return USB_PD_DISCHARGE_PPC;
	else
		return USB_PD_DISCHARGE_NONE;
}
