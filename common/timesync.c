...
void set_rtc_epoch(timestamp_t local_time, uint32_t host_rtc_sec)
{
	local_epoch.local_timestamp = local_time;
	local_epoch.host_rtc_sec = host_rtc_sec;
}
...
struct calendar_date get_host_synced_rtc(void)
{
	...
	const uint64_t sec_diff = us_diff / SECOND;
	const uint32_t current_rtc_sec = local_epoch.host_rtc_sec +
		(uint32_t)sec_diff;
...
static int host_command_sync_rtc(struct host_cmd_handler_args *args)
{
	...
	const uint32_t host_rtc = r->host_rtc_sec;

	/* Synchronize EC time with RTC time */
	set_rtc_epoch(tm, host_rtc);
	print_synced_rtc();
