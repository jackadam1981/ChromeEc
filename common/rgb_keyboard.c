...
static const struct rgbkbd_coord *rgbkbd_table[EC_RGBKBD_MAX_KEY_COUNT];
...
static void rgbkbd_init_lookup_table(void)
{
	bool add = true;
	int i, k = 0;

	if (rgbkbd_map[rgbkbd_map_size - 1] != RGBKBD_TERM) {
		CPRINTS("Key-LED map isn't terminated.");
		return;
	}
	/*
	 * rgbkbd_map[] consists of LED IDs separated by a terminating byte
	 * (TERM). When 'add' is true, the next byte will be the beginning of a
	 * new LED group, thus, its pointer will be added to rgbkbd_table. If
	 * the next byte is a back-to-back 0xff, it's an empty group and still
	 * added to rgbkbd_table.
	 */
	for (i = 0; i < rgbkbd_map_size && k < EC_RGBKBD_MAX_KEY_COUNT; i++) {
		if (rgbkbd_map[i] != RGBKBD_TERM) {
			if (add)
				rgbkbd_table[k++] =
					(struct rgbkbd_coord *)&rgbkbd_map[i];
	 ...
	 * terminated. The rest of the table entries point to NULL.
	 */
}
...
static enum ec_status hc_rgbkbd_set_color(struct host_cmd_handler_args *args)
{
	...
	for (i = 0; i < p->length; i++) {
		const union rgbkbd_coord_u8 *led =
			(union rgbkbd_coord_u8 *)rgbkbd_table[p->start_key + i];
		if (led == NULL)
			continue;
		while (led->u8 != RGBKBD_TERM) {
