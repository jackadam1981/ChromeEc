...
static enum rgbkbd_demo demo = CONFIG_RGBKBD_DEMO;

static int _set_color_single(struct rgb_s color, int x, int y)
{
...
static uint8_t _get_grid_size(const struct rgbkbd *ctx)
{
...
static void _sync(void)
{
	...
	for (i = 0; i < rgbkbd_count; i++) {
		ctx = &rgbkbds[i];
		len = _get_grid_size(ctx);
		ctx->cfg->drv->set_color(ctx, 0, ctx->buf, len);
	}
}

static struct rgb_s _rotate_color(struct rgb_s color, int step)
{
	color.r += step;
	if (color.r == 0) {
		color.g += step;
...
static void _demo_dot(void)
{
	...
	if (x >= rgbkbd_hsize) {
		...
		if (y >= rgbkbd_vsize) {
			y = 0;
			color = _rotate_color(color, 0x80);
		}
	}

	/* Turn on next dot. */
	_set_color_single(color, x, y);

	msleep(250);
}

static void _demo(enum rgbkbd_demo id)
{
	switch (id) {
...
static int cc_rgbk(int argc, char **argv)
{
	...
	if (y < 0 && x < 0) {
		...
		for (i = 0; i < rgbkbd_count; i++) {
			ctx = &rgbkbds[i];
			for (j = 0; j < _get_grid_size(ctx); j++)
				ctx->buf[j] = color;
		}
		_sync();
	} else if (y < 0) {
