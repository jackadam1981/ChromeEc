 ...
 */
static int set_valid_sections(void)
{
	int i;
	uint32_t run_time_offs = (uint32_t) set_valid_sections -
	...
	*response_size = 1; /* One byte response unless this is a start PDU. */
	...
	if (!cmd_body->block_base && !body_size) {
		...
		*response_size = sizeof(*rpdu);
		...
		/* TODO(b/36375666): Move this code to a separate function. */
		...
		rpdu->backup_ro_offset =
			htobe32(valid_section->sect_base_offset);
