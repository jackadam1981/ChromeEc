 ...
 * ciphered data.
 */
static uint8_t fp_enc_buffer[FP_ALGORITHM_TEMPLATE_SIZE +
	sizeof(struct ec_fp_template_encryption_metadata)] FP_TEMPLATE_SECTION;
...
static int fp_command_frame(struct host_cmd_handler_args *args)
{
	...
	if (idx == FP_FRAME_INDEX_RAW_IMAGE) {
		...
		max_size = sizeof(fp_enc_buffer);
		content = fp_enc_buffer;
	}
	...
	if (!offset) {
		...
		memset(fp_enc_buffer, 0, sizeof(fp_enc_buffer));
		enc_info =
		    (struct ec_fp_template_encryption_metadata *)fp_enc_buffer;
		init_trng();
		rand_bytes(enc_info->nonce, FP_CONTEXT_NONCE_BYTES);
		rand_bytes(enc_info->salt, FP_CONTEXT_SALT_BYTES);
		exit_trng();

		if (derive_encryption_key(key, enc_info->salt)) {
			CPRINTS("fgr%d: Failed to derive key", fgr);
			return EC_RES_UNAVAILABLE;
		}

		if (aes_gcm_encrypt(key, SBP_ENC_KEY_LEN,
				    fp_template[fgr],
...
static int fp_command_template(struct host_cmd_handler_args *args)
{
	...
	if (params->size & FP_TEMPLATE_COMMIT) {
		...
		if (aes_gcm_decrypt(key, SBP_ENC_KEY_LEN,
				    fp_template[idx],
				    fp_enc_buffer + sizeof(*enc_info),
				    sizeof(fp_template[0]),
