...
/* Current user ID */
static uint32_t user_id[32 / sizeof(uint32_t)];
...
static uint32_t fp_process_enroll(void)
{
	...
	res = fp_finger_enroll(fp_template[templ_valid], fp_buffer,
			       &percent);
...
static void fp_clear_context(void)
{
	...
	for (i = 0; i < FP_MAX_FINGER_COUNT; i++)
		memset(fp_template[i], 0, sizeof(fp_template[0]));
}
...
static int fp_command_template(struct host_cmd_handler_args *args)
{
	...
	if ((args->params_size != size + offsetof(struct ec_params_fp_template, data))
...
static int fp_command_context(struct host_cmd_handler_args *args)
{
	...
	/* TODO(b/Xx): real crypto protocol */
...
int command_fpenroll(int argc, char **argv)
{
	...
	int percent = 0;
	uint32_t event;
	static const char *enroll_str[] = {"OK", "Low Quality", "Immobile",
					   "Low Coverage"};

	do {
		rc = fp_console_action(FP_MODE_ENROLL);
		if (rc != EC_SUCCESS)
			break;
