...
static void fp_clear_context(void)
{
	...
	memset(templ_nonce, 0, sizeof(templ_nonce));
	memset(templ_salt, 0, sizeof(templ_salt));
	memset(templ_tag, 0, sizeof(templ_tag));
			   ...
			   uint8_t *tag, int tag_size)
{
	...
	res = AES_set_decrypt_key(key, 8 * key_size, &aes_key);
	if (res) {
		CPRINTS("Failed to set decryption key: %d", res);
		cflush();
		return res;
	}
	CRYPTO_gcm128_init(&ctx, &aes_key, (block128_f)AES_decrypt, 0);
	CRYPTO_gcm128_setiv(&ctx, &aes_key, nonce, nonce_size);
	/* CRYPTO functions return 1 on success, 0 on error. */
...
static int fp_command_frame(struct host_cmd_handler_args *args)
{
	...
	if (idx == FP_FRAME_INDEX_RAW_IMAGE) {
		...
		memcpy(r->data, fp_buffer + offset, params->size);

		args->response_size = sizeof(*r) + params->size;
		return EC_RES_SUCCESS;
	...
	if (!offset) {
		...
		for (i = 0; i < FP_CONTEXT_NONCE_WORDS; i++)
			CPRINTS("enc:fgr%d:nonce[%d]=0x%x", idx - 1, i,
				templ_nonce[idx - 1][i]);
		for (i = 0; i < FP_CONTEXT_SALT_WORDS; i++)
			CPRINTS("enc:fgr%d:salt[%d]=0x%x", idx - 1, i,
				templ_salt[idx - 1][i]);
		...
		/* TODO: Remove debug. */
		for (i = 0; i < SBP_ENC_KEY_LEN; i++)
			CPRINTS("enc:fgr%d:key[%d]=0x%x", idx - 1, i,
				templ_key[idx - 1][i]);
		cflush();
		...
		if (aes_gcm_encrypt(templ_key[idx - 1], SBP_ENC_KEY_LEN,
				    fp_template[idx - 1],
				    fp_enc_buffer, sizeof(fp_enc_buffer),
				    templ_nonce[idx - 1],
		...
		for (i = 0; i < FP_CONTEXT_TAG_WORDS; i++)
			CPRINTS("enc:fgr%d:tag[%d]=0x%x", idx - 1, i,
				templ_tag[idx - 1][i]);
	}
	...
	memcpy(r->data, fp_enc_buffer + offset, params->size);
	templ_dirty &= ~(1 << (idx - 1));
...
static int fp_command_template(struct host_cmd_handler_args *args)
{
	...
	if (params->size & FP_TEMPLATE_COMMIT) {
		...
		for (i = 0; i < FP_CONTEXT_NONCE_WORDS; i++)
			CPRINTS("dec:fgr%d:nonce[%d]=0x%x", idx, i,
				templ_nonce[idx][i]);
		for (i = 0; i < FP_CONTEXT_SALT_WORDS; i++)
			CPRINTS("dec:fgr%d:salt[%d]=0x%x", idx, i,
				templ_salt[idx][i]);
		for (i = 0; i < FP_CONTEXT_TAG_WORDS; i++)
			CPRINTS("dec:fgr%d:tag[%d]=0x%x", idx, i,
				templ_tag[idx][i]);
		...
		/* TODO: Remove debug. */
		for (i = 0; i < SBP_ENC_KEY_LEN; i++)
			CPRINTS("dec:fgr%d:key[%d]=0x%x", idx, i,
				templ_key[idx][i]);
		cflush();

		if (aes_gcm_decrypt(templ_key[idx], SBP_ENC_KEY_LEN,
				    fp_template[idx],
				    fp_enc_buffer, FP_ALGORITHM_TEMPLATE_SIZE,
