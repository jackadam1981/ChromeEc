...
int gpio_config_module(enum module_id id, int enable)
{
	...
	for (af = gpio_alt_funcs;
	     af < gpio_alt_funcs + gpio_alt_funcs_count;
	     af++) {
		if (af->module_id != id)
...
int gpio_config_pin(enum module_id id, enum gpio_signal signal, int enable)
{
	...
	/* Find signal and set to alternate functions */
	for (af = gpio_alt_funcs;
	     af < gpio_alt_funcs + gpio_alt_funcs_count;
	     af++) {
		if (af->module_id        == id &&
		    af->port             == g->port &&
		    (af->mask & g->mask) == g->mask) {
			gpio_set_flags(signal, enable ? af->flags : g->flags);
			gpio_set_alternate_function(g->port,
						    g->mask,
						    enable ? af->func : -1);
			return EC_SUCCESS;
		}
