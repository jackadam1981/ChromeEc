	...
	*hid_host_sensor_id_to_real_sensor(int report_id)
{
	...
	ccprintf("report_id_mapped %d\n", report_id_mapped);
	sensor = &motion_sensors[report_id_mapped];
	ccprintf("&motion_sensors[report_id_mapped]: %d\n", sensor);
		     ...
		     void (*send_response)(int len))
{
	...
	case COMMAND_REGISTER:
		...
		ccprintf("inside i2c_hid_process: data_set %d\n", data_set);
		return data_set;
	default:
				   ...
				   void (*send_response)(int len))
{
	...
	case I2C_HID_CMD_SET_REPORT:
		...
		case REPORT_ID_BASE_ACCEL_SAMPLING_RATE:
			...
			sensor_report_id = hid_get_sensorid_from_featureid(report_id);
			sensor = hid_host_sensor_id_to_real_sensor(sensor_report_id);
			ccprintf("data_set %d\n", data_set);
