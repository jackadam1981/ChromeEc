...
//static struct hid_accel_feature_report feature_reports[2];
...
int hid_compile_input(int report_id)
{
	...
	ccprintf("In hid compile input after spoofing x %d\n", sensor->xyz[X]);
	ccprintf("Get sensor y %d\n", sensor->xyz[Y]);
	ccprintf("Get sensor z %d\n", sensor->xyz[Z]);
	if (sensor == NULL)
	...
	ccprintf("In hid compile input Get sensor x %d\n", input.x);
	ccprintf("Get sensor y %d\n", input.y);
	ccprintf("Get sensor z %d\n", input.z);
#
	for (i = 0; i < 2 ; i++)
	...
	*hid_host_sensor_id_to_real_sensor(int report_id)
{
	...
	ccprintf("In mapping fn Get spoof sensor x %d\n", sensor->xyz[X]);
	ccprintf("Get spoof sensor y %d\n", sensor->xyz[Y]);
	ccprintf("Get spoof sensor z %d\n", sensor->xyz[Z]);
#else
