...
uint8_t ll_set_scan_response_data(uint8_t *params)
{
	...
	memcpy(&ll_scan_rsp_pdu.payload[6], &params[1], params[0]);
	ll_scan_rsp_pdu.header.adv.length = params[0] + 6;
...
uint8_t ll_set_adv_data(uint8_t *params)
{
	...
	/* Skip the address */
	memcpy(&ll_adv_pdu.payload[6], &params[1], params[0]);
	ll_adv_pdu.header.adv.length = params[0] + 6;
...
{
	uint16_t handle = params[0] | (params[1] << 8);
...
uint8_t ll_set_advertising_params(uint8_t *params)
{
	...
	if (ll_adv_params.useRandomAddress)
		memcpy(ll_adv_pdu.payload, &ll_random_address, 6);
	else
		memcpy(ll_adv_pdu.payload, &ll_public_address, 6);
	...
	if (ll_adv_params.advType == BLE_ADV_HEADER_PDU_TYPE_ADV_DIRECT_IND) {
		...
		memcpy(&ll_adv_pdu.payload[6], ll_adv_params.directAddr,
	...
	if (ll_adv_params.useRandomAddress)
		memcpy(ll_scan_rsp_pdu.payload, &ll_random_address, 6);
	else
		memcpy(ll_scan_rsp_pdu.payload, &ll_public_address, 6);
...
int ble_ll_adv(int chan)
{
	...
	case BLE_ADV_HEADER_PDU_TYPE_SCAN_REQ:
		...
		if ((ll_adv_pdu.header.adv.type !=
		    ...
		    (memcmp(&ll_rcv_packet.payload[6], &ll_adv_pdu.payload[0],
			    6))) {
	...
	case BLE_ADV_HEADER_PDU_TYPE_CONNECT_REQ:
		...
		if (ll_adv_pdu.header.adv.type !=
			...
			return rv;
		/* The advertising address needs to match */
		if (memcmp(&ll_rcv_packet.payload[6], &ll_adv_pdu.payload[0],
			   6))
			return rv;
		/* The InitAddr address needs to match for ADV_DIRECT_IND */
		if (ll_adv_pdu.header.adv.type ==
			BLE_ADV_HEADER_PDU_TYPE_ADV_DIRECT_IND &&
		    memcmp(&ll_adv_pdu.payload[6], &ll_rcv_packet.payload[0],
			   6))
			return rv;
	break;
