...
void usb_charger_task(void)
{
	...
	int pd_type = CHARGE_SUPPLIER_VBUS;
	int send_host_evt;
	struct charge_port_info charge = {
	...
	while (1) {
		...
		if (evt & USB_CHG_EVENT_ATTACH) {
			pd_type = get_charge_supplier();

			/*
			 * If PD already negotiated as VBUS nothing to do.
			 * Otherwise check if there is a BC1.2 charger attached.
			 */
			if (pd_type != CHARGE_SUPPLIER_VBUS)
			...
			if (bc12_type != CHARGE_SUPPLIER_NONE) {
				...
				charge.current =
					bd99955_get_bc12_ilim(bc12_type);
		...
		if (evt & USB_CHG_EVENT_DETACH) {
			...
			if (pd_type != CHARGE_SUPPLIER_NONE &&
				bc12_type == CHARGE_SUPPLIER_NONE) {
				charge.current = 0;
				charge_manager_update_charge(pd_type,
							port,
							&charge);
				send_host_evt = 1;
