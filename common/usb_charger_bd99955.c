...
void usb_charger_task(void)
{
	...
	uint32_t evt;
	int pd_type = get_charge_supplier(port);
	int bc12_type = CHARGE_SUPPLIER_NONE;
	int vbus_source = usb_charger_port_is_sourcing_vbus(port);

	/* Get PD type as BC1.2 is not enabled during initialization */
	if (pd_type == CHARGE_SUPPLIER_NONE && pd_snk_is_vbus_provided(port)) {
		do {
			msleep(1000);
			pd_type = get_charge_supplier(port);
		} while (pd_type == CHARGE_SUPPLIER_NONE);
	}

	bd99955_enable_usb_switch(port, vbus_source);
