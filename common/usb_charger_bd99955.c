...
void usb_charger_task(void)
{
	...
	/* Enable bus attached or removed interrupt */
	bd99955_enable_bus_detect_interrupts(port, 1);

	while (1) {
		/* Wait for interrupt */
		evt = task_wait_event(-1);

		/* Check if there is bus attached or removed */
		for (i = 0; i < BUS_DETECT_RETRY; i++) {
			if (bd99955_get_bus_detect_interrupts(port, 1))
				break;
		...
		/* Clear bus attached or removed interrupt */
		bd99955_get_bus_detect_interrupts(port, 0);
	}
}
