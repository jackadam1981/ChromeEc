...
#define T_NEXT_CHECK_TIME (10 * SECOND)
		       ...
		       int input_mw)
{
	...
	get_desired_input_power(&vbus, &input_curr);
	/*CPRINTS("is_more_efficient vbus=%d curr_mv=%d prev_mv=%d",
			vbus, curr_mv, prev_mv);*/
	/* Prefer 15V unless we're at 15 and considering 12. */
	if (vbus >= 14000 && vbus <= 16000 && curr_mv == 12000) {
		return true;
	} else if (curr_mv == 15000) {
		return true;
	}
	return false;
}
...
void dps_task(void *u)
{
	...
	while (1) {
		...
		if (sample_count == dps_config.k_sample) {
			...
			gpio_pin_set_dt(debug_gpio_spec, curr_cand.mv == 15000);
			pd_dpm_request(dps_port, DPM_REQUEST_NEW_POWER_LEVEL);
			sample_count = 0;
			atomic_clear_bits(&flag, (DPS_FLAG_SAMPLED |
