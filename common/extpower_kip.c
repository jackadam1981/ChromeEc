...
int ad_input_current[][NUM_AC_TURBO_STATES] = {
	...
	{0x0800, 0x0800},			/* ADAPTER_UNKNOWN ~ 2.5 A */
	{0x0800, 0x0800},			/* ADAPTER_45W ~ 1.5-2.0 A */
	{0x0c00, 0x0c00},			/* ADAPTER_65W ~ 2.5-3.0 A */
	{0x1100, 0x1100}			/* ADAPTER_90W ~ 3.8-4.3 A */
};
...
struct adapter_limits ad_limits[][NUM_AC_TURBO_STATES][NUM_AC_THRESHOLDS] = {
	/* ADAPTER_UNKNOWN - treat as 45W, no turbo */
	{
		/* Turbo off */
		{
			{ 2310, 1960, 16, 80, },
...
void watch_adapter_closely(struct charge_state_context *ctx)
{
	 ...
	 * errors and we recognize the adapter, then we always set turbo off,
	 * since BQ24715 charger cannot support hybrid boost */
	if (extpower_is_present()) {
		set_turbo(0);
	} else {
