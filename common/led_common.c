...
{
	uint32_t led_auto_control_flag = 0;

	switch (led_id) {
	case EC_LED_ID_ADAPTER_LED:
		led_auto_control_flag = LED_AUTO_CONTROL_ADAPTER;
		break;
	case EC_LED_ID_BATTERY_LED:
		led_auto_control_flag = LED_AUTO_CONTROL_BATTERY;
		break;
	case EC_LED_ID_POWER_LED:
		led_auto_control_flag = LED_AUTO_CONTROL_POWER;
		break;
	default:
		break;
	}
	...
	if (enable)
		led_auto_control_flags |= led_auto_control_flag;
	else
		led_auto_control_flags &= ~led_auto_control_flag;
}
...
int led_auto_control_is_enabled(enum ec_led_id led_id)
{
	...
	return (led_auto_control_flags & led_auto_control_flag) != 0;
}
...
static int led_command_control(struct host_cmd_handler_args *args)
{
	...
	led_brightness_range(p->led_id, r->brightness_range);
