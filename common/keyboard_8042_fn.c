...
void keyboard_state_changed(int row, int col, int is_pressed)
{
	...
	if (!is_pressed) {
		...
		for (uint8_t i = 0; i < ARRAY_SIZE(fn_keys); i++) {
			...
			LOG_DBG("release override key=%d", i);

			break;
		}
	} else if (fn_key_status) {
		...
		for (uint8_t i = 0; i < ARRAY_SIZE(fn_keys); i++) {
			...
			LOG_DBG("hold override key=%d", i);

			break;
		}
	}

	LOG_DBG("fn_key_status=%d fn_keys_statust=%02x", fn_key_status,
		fn_keys_status);
