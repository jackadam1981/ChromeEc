...
/* Ensure enough space for panic_data, jump_data and atleast one jump tag */
BUILD_ASSERT((sizeof(struct panic_data) + sizeof(struct jump_data) +
	      JUMP_TAG_MAX_SIZE) <= CONFIG_END_OF_RAM_DATA_SIZE,
...
int system_add_jump_tag(uint16_t tag, int version, int size, const void *data)
{
	...
	if (system_usable_ram_end() - new_entry_size < JUMP_DATA_MIN_ADDRESS) {
		printk("Error: Not enough room for additional jump tag\n");
		return EC_ERROR_INVAL;
	}
