...
int system_run_image_copy(enum system_image_copy_t copy)
{
	...
	base = flash_image_get_base(copy);
	if (base == 0xffffffff)
		return EC_ERROR_INVAL;
...
#ifndef EMU_BUILD
	if (init_addr < base || init_addr >= base + flash_image_get_size(copy))
		return EC_ERROR_UNKNOWN;
#endif
#endif
...
const char *system_get_version(enum system_image_copy_t copy)
{
	...
	addr = flash_image_get_base(copy);
	if (addr == 0xffffffff)
		return "";
	...
	version_offset = ((uintptr_t)&version_data -
		flash_image_get_base(system_get_image_copy()));
#endif


#ifdef CONFIG_FLASH_SPI
	flash_physical_read(flash_image_get_base_spi(copy) +
