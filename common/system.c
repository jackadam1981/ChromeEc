					    ...
					    uint32_t add_reset_flags)
{
	...
	CPRINTS("running_host_cmd=%d", running_host_cmd);
	if (!running_host_cmd && system_get_image_copy() == copy)
		return EC_SUCCESS;
	running_host_cmd = 0;
	...
	CPRINTS("addr_entry=0x%x",
		*(uintptr_t *)(0x40000 + CONFIG_MAPPED_STORAGE_BASE + 4));
