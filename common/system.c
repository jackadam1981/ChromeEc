...
const char *system_get_cros_fwid(enum ec_image copy)
{
	...
	if (IS_ENABLED(CONFIG_CROS_FWID_VERSION)) {
		...
		if (data && data->cookie3 == CROS_EC_IMAGE_DATA_COOKIE3)
...
host_command_get_version(struct host_cmd_handler_args *args)
{
	...
	/* cros_fwid_rw[32] is not present in version 0, so remove 32 bytes */
	args->response_size = args->version == 0 ? sizeof(*r) - 32 : sizeof(*r);
