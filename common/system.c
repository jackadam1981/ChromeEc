...
void system_disable_jump(void)
{
	...
	if (system_is_locked()) {
		...
		CPRINTF("MPU type: %08x", mpu_get_type());
		/*
		...
		if (ret == EC_SUCCESS) {
			...
			CPRINTF("Failed to lock RAM (%d)", ret);
		}
		...
		if (ret == EC_SUCCESS) {
			enable_mpu = 1;
			CPRINTF("%s image locked", image_names[copy]);
		} else {
			CPRINTF("Failed to lock %s image (%d)",
		...
		CPRINTF("System is unlocked. Skip MPU configuration");
	}
#endif
}
...
int system_run_image_copy(enum system_image_copy_t copy)
{
	...
	CPRINTF("Jumping to image %s", image_names[copy]);
...
int host_command_reboot(struct host_cmd_handler_args *args)
{
	...
	CPRINTF("Executing host reboot command %d", p.cmd);
