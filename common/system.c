...
static void system_common_shutdown(void)
{
	...
	if (IS_ENABLED(CONFIG_VBOOT_EFS2) && system_is_manual_recovery()) {
		/*
		 * We're here because the system shuts down in recovery mode.
		 *
		 * We assume AP wants to reboot (e.g. after a recovery drive is
		 * removed), thus we execute a hard reset (without AP_OFF). It
		 * will be followed by a normal boot (i.e. sysjump from RO to
		 * RW then power on).
		 *
		 * If AP wants to shut down and stay, it should have requested
		 * SYSTEM_RESET_LEAVE_AP_OFF with FLAG_ON_AP_SHUTDOWN, which is
		 * handled by the handle_pending_reboot above.
		 */
