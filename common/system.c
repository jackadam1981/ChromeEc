...
__overridable const char *board_read_poweron_conf(void)
{
	if (IS_ENABLED(CONFIG_FLASH_PSTATE) &&
	    IS_ENABLED(CONFIG_FLASH_PSTATE_BANK))
		return crec_flash_read_pstate_poweron_conf();
	else
		return "";
}

/* By default, write servo poweron config from flash, can be overridden. */
__overridable int board_write_poweron_conf(const char *poweron_conf)
{
	if (IS_ENABLED(CONFIG_FLASH_PSTATE) &&
	    IS_ENABLED(CONFIG_FLASH_PSTATE_BANK))
