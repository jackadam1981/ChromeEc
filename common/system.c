 ...
 */
static uint32_t __attribute__((unused)) get_size(enum ec_current_image copy)
{
...
void system_disable_jump(void)
{
	...
	if (system_is_locked()) {
		int ret;
		enum ec_current_image __attribute__((unused)) copy;
		...
		if (ret == EC_SUCCESS) {
			CPRINTS("%s image locked",
				ec_current_image_to_string(copy));
		} else {
			CPRINTS("Failed to lock %s image (%d)",
				ec_current_image_to_string(copy), ret);
			return;
		}
...
test_mockable enum ec_current_image system_get_image_copy(void)
{
...
test_mockable int system_unsafe_to_overwrite(uint32_t offset, uint32_t size)
{
	uint32_t r_offset;
	uint32_t r_size;
	enum ec_current_image copy = system_get_image_copy();
...
{
	return ec_current_image_to_string(system_get_image_copy());
}
...
static int is_rw_image(enum ec_current_image copy)
{
...
test_mockable int system_run_image_copy(enum ec_current_image copy)
{
	uintptr_t base;
	uintptr_t init_addr;
	...
	CPRINTS("Jumping to image %s", ec_current_image_to_string(copy));
...
const struct image_data *system_get_image_data(enum ec_current_image copy)
{
	...
	uintptr_t addr;
	enum ec_current_image active_copy = system_get_image_copy();
...
static int command_sysinfo(int argc, char **argv)
{
	...
	ccprintf(")\n");
	ccprintf("Copy:   %s\n",
		ec_current_image_to_string(info.current_image));
	ccprintf("Jumped: %s\n",
...
host_command_get_version(struct host_cmd_handler_args *args)
{
	...
	enum ec_current_image active_slot = system_get_active_copy();
