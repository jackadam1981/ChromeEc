...
static int debug_button_mask(void)
{
	...
	mask = power_button_is_pressed() ? DEBUG_BTN_POWER : 0;

	/* Get volume up state */
	mask |= state[BUTTON_VOLUME_UP].debounced_pressed ? DEBUG_BTN_VOL_UP :
		0;

	/* Get volume down state */
	mask |= state[BUTTON_VOLUME_DOWN].debounced_pressed ? DEBUG_BTN_VOL_DN :
		0;
...
static void debug_mode_input_deferred(void)
{
	debug_mode_handle();
}
DECLARE_DEFERRED(debug_mode_input_deferred);
...
static void debug_mode_transition(enum debug_state next_state)
{
	...
	hook_call_deferred(&debug_mode_input_deferred_data, -1);
	...
	case STATE_STAGING:
		 ...
		 * button state if it does not changed during the timeout
		 * duration.
		 */
		hook_call_deferred(&debug_mode_input_deferred_data,
					DEBUG_TIMEOUT);
		break;
...
DECLARE_HOOK(HOOK_BUTTON_STATE_CHANGE, debug_mode_handle, HOOK_PRIO_LAST);
