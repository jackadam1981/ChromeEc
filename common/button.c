...
static int console_command_button(int argc, const char **argv)
{
	...
	for (argv_idx = 1; argv_idx < argc; argv_idx++) {
		if (!strcasecmp(argv[argv_idx], "vup"))
			...
			button =
				button_present(KEYBOARD_BUTTON_BRIGHTNESS_DOWN);
		else if (!strcasecmp(argv[argv_idx], "slock"))
			button = button_present(KEYBOARD_BUTTON_SCREEN_LOCK);
		else {
...
DECLARE_CONSOLE_COMMAND(button, console_command_button,
			"vup|vdown|rec|bup|bdown|slock msec",
...
#define DEBUG_BTN_SCREEN_LK BIT(5)
...
static int debug_button_mask(void)
{
	...
	if (state[BUTTON_SCREEN_LOCK].debounced_pressed)
		mask |= DEBUG_BTN_SCREEN_LK;
...
struct button_config buttons[BUTTON_COUNT] = {
	...
	[BUTTON_SCREEN_LOCK] = {
		.name = "Screen Lock",
		.type = KEYBOARD_BUTTON_SCREEN_LOCK,
		.gpio = GPIO_SCREEN_LOCK_L,
		.debounce_us = BUTTON_DEBOUNCE_US,
		.flags = 0,
	},
#endif /* defined(CONFIG_SCREENLOCK_BUTTONS) */
};
