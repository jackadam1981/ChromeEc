 ...
 * Bit numbers are aligned to enum keyboard_button_type */
...
{
	int button_mask = BIT(button->type);
	int ret_val;
...
{
	int physical_value =
		!!gpio_get_level(button->gpio) == !!(button->flags & BUTTON_FLAG_ACTIVE_HIGH);
	int simulated_value = 0;
#ifdef CONFIG_SIMULATED_BUTTON
	simulated_value =
		!!simulated_button_pressed(button) == !!(button->flags & BUTTON_FLAG_ACTIVE_HIGH);
#endif
...
static void simulate_button_release_deferred(void)
{
	...
	for (button_idx = 0; button_idx < BUTTON_COUNT; button_idx++) {
		...
		if (sim_button_state & BIT(buttons[button_idx].type)) {
			...
			atomic_clear( &sim_button_state, BIT(buttons[button_idx].type));
...
static void simulate_button(uint32_t button_mask, int press_ms)
{
	...
	for (button_idx = 0; button_idx < BUTTON_COUNT; button_idx++) {
		if (button_mask & BIT(button_idx)) {
			...
			atomic_or( &sim_button_state, BIT(buttons[button_idx].type));
	...
	hook_call_deferred(&simulate_button_release_deferred_data, press_ms * MSEC);
}
...
static enum ec_status host_command_button(struct host_cmd_handler_args *args)
{
	...
	simulate_button(BIT(button), p->press_ms);
