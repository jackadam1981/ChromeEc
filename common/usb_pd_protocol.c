...
void pd_charge_manager_update(int port, int cc_voltage)
{
	struct charge_port_info charge;

	/* Detect type C charger current limit based upon vbus voltage. */
	if (cc_voltage > TYPE_C_3000_MIN_MV)
		charge.current = 3000;
	else if (cc_voltage > TYPE_C_1500_MIN_MV)
		charge.current = 1500;
	...
	if (cc_voltage > PD_SNK_VA)
		charge.current = PD_MIN_MA;
	else
		charge.current = 0;
	charge.voltage = PD_MIN_MV;
...
void pd_task(void)
{
	...
	int cc_voltage;
#ifdef CONFIG_CHARGE_MANAGER
	static int initialized[PD_PROT_COUNT];
#endif /* CONFIG_CHARGE_MANAGER */
#endif /* CONFIG_USB_PD_DUAL_ROLE */
...
#ifdef CONFIG_CHARGE_MANAGER
			/*
			 * Update charge manager if we haven't done so yet,
			 * or if we're transitioning to discovery state.
			 */
			if (!initialized[port] || cc_voltage >= PD_SNK_VA) {
				initialized[port] = 1;
				pd_charge_manager_update(port, cc_voltage);
			}
#endif
			if (cc_voltage < PD_SNK_VA) {
				/*
