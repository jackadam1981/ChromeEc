...
#ifdef BOARD_OAK_PD
		/* OAK_PD: Update port status to EC */
		if (this_state != PD_STATE_SRC_DISCONNECTED &&
		    this_state != PD_STATE_SNK_DISCONNECTED &&
		    (pd_port_status[port].power_role != pd[port].power_role ||
		     pd_port_status[port].data_role  != pd[port].data_role ||
		     pd_port_status[port].polarity   != pd[port].polarity)) {
			pd_port_status[port].power_role = pd[port].power_role;
			pd_port_status[port].data_role  = pd[port].data_role;
			pd_port_status[port].polarity   = pd[port].polarity;
			CPRINTS("PwrRole:%d, DataRole%d, Polarity%d",
				pd[port].power_role, pd[port].data_role,
				pd[port].polarity);
			pd_send_ec_int();
		}
#endif
