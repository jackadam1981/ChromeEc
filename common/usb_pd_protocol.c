...
static struct pd_protocol {
	...
	/*
	 * When a discover identity command is sent, cable_type stores the
		...
		uint32_t *payload)
{
	...
	if (IS_ENABLED(CONFIG_USB_PD_DECODE_SOP)) {
		...
		/* Restting the message type to SOP */
	...
	case VDM_STATE_READY:
		...
		if (IS_ENABLED(CONFIG_USB_PD_DECODE_SOP)) {
			if (pd[port].msg_type != TCPC_TX_SOP_PRIME &&
			    pdo_busy(port))
				/*
				 * SOP Prime is being transmitted before the
				 * source capabilities have been established,
				 * hence skip checking if the PDO is in ready
				 * state.
				 */
				break;
		/*
...
void pd_task(void *u)
{
	...
	while (1) {
		...
		case PD_STATE_SRC_DISCOVERY:
			...
			if (IS_ENABLED(CONFIG_USB_PD_DECODE_SOP))
				/*
				 * Send SOP Prime packet before eastablishing
				 * the source capabilities.
				 * reference : USB_PD_R2_0 Section 4.5.
				 */
				if (pd[port].flags &
				    PD_FLAGS_CHECK_CABLE_ENABLE) {
		...
		case PD_STATE_SNK_HARD_RESET_RECOVER:
			...
			if (IS_ENABLED(CONFIG_USB_PD_DECODE_SOP))
				pd[port].flags |= PD_FLAGS_CHECK_CABLE_ENABLE;
...
static void pd_chipset_startup(void)
{
	...
	for (i = 0; i < CONFIG_USB_PD_PORT_COUNT; i++) {
		...
		if (IS_ENABLED(CONFIG_USB_PD_DECODE_SOP))
			pd[i].flags |= PD_FLAGS_CHECK_CABLE_ENABLE;
...
static int command_pd(int argc, char **argv)
{
	...
	if (!strncasecmp(argv[2], "state", 5)) {
		...
		if (pd[port].cable_type != IDH_PTYPE_UNDEF) {
			ccprintf("Cable type: ");
			switch (pd[port].cable_attr.cable_type) {
			case CABLE_ATYPE:
				ccprintf("A");
				break;
			case CABLE_BTYPE:
			...
			ccprintf("Cable connector: %s\n",
				pd[port].cable_attr.connector_type ?
							"Receptacle" : "Plug");

			ccprintf("Latency: %dms\n",
					pd[port].cable_attr.cable_latency);

			if (pd[port].cable_type == IDH_PTYPE_ACABLE)
				ccprintf("Cable termation type: Active\n");

			ccprintf("Current handling capacity: ");
			switch (pd[port].cable_attr.cable_current) {
			...
			ccprintf("VBUS through the Cable: %s\n",
					pd[port].cable_attr.cable_vbus?
								"Yes" : "No");

			ccprintf("SOP\" controller: %sbled\n",
					pd[port].cable_attr.cable_controller ?
								"Ena" : "Dis");

			ccprintf("USB superspeed signalling support: ");
			switch (pd[port].cable_attr.ss_support) {
			case USB_SS_U2_ONLY:
				ccprintf("USB 2.0");
				break;
