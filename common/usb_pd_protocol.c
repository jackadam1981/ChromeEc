...
enum pd_dual_role_states pd_get_dual_role_port(int port)
{
	...
	for (i = 0; i < CONFIG_USB_PD_PORT_COUNT; i++) {
		...
		if (charge_manager_get_active_charge_port() != i)
			...
			pd[i].flags |= PD_FLAGS_CHECK_PR_ROLE |
				       PD_FLAGS_CHECK_DR_ROLE;
		pd_set_dual_role_port(i, PD_DRP_TOGGLE_ON);
	}
...
static void pd_chipset_suspend(void)
{
	...
	for (i = 0; i < CONFIG_USB_PD_PORT_COUNT; i++)
		pd_set_dual_role_port(i, PD_DRP_TOGGLE_OFF);
	CPRINTS("PD:S0->S3");
}
...
static void pd_chipset_shutdown(void)
{
	...
	for (i = 0; i < CONFIG_USB_PD_PORT_COUNT; i++)
		pd_set_dual_role_port(i, PD_DRP_FORCE_SINK);
	CPRINTS("PD:S3->S5");
}
