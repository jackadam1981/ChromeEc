		...
		uint32_t *payload)
{
	 ...
	 */
	if (!cnt && !PD_HEADER_EXT(head) &&
	    PD_HEADER_TYPE(head) == PD_CTRL_SOFT_RESET) {
		/* If it is a soft-reset, then do nothing and fall through. */
	} else if (pd[port].flags & PD_FLAGS_RCV_MSG_RX_ID &&
		   PD_HEADER_ID(head) == pd[port].msg_rx_id) {
		/*
		 * Receive the same msg ID, and it's not a PD_CTRL_SOFT_RESET.
		 * Should only return GoodCRC and don't reply.
		 */
		return;
	} else {
		/* It's a new MessageID, record it. */
		pd[port].flags |= PD_FLAGS_RCV_MSG_RX_ID;
