...
static inline void set_state(int port, enum pd_states next_state)
{
	...
	if (next_state == PD_STATE_SRC_DISCONNECTED) {
		...
		if (actual_disconnect) {
			/* Unattached should not have polarity */
			pd[port].polarity = TYPEC_POLARITY_NONE;
		 ...
		 * automatically as it needs, so don't adjust the role in
		 * that case.
		 */
		if (!IS_ENABLED(CONFIG_USB_PD_DUAL_ROLE_AUTO_TOGGLE) ||
		    pd_get_dual_role(port) != PD_DRP_TOGGLE_ON) {
			if (tcpm_set_polarity(port, TYPEC_POLARITY_NONE))
...
void pd_toggle_detect_polarity(int port)
{
	...
	if (cc1 == cc2)
		...
		pd[port].polarity = TYPEC_POLARITY_NORMAL;
	else
		pd[port].polarity = TYPEC_POLARITY_FLIPPED;
}
	...
	while (1) {
		...
		case PD_STATE_DRP_AUTO_TOGGLE:
		{
			...
			if (next_state == DRP_TC_UNATTACHED_SNK) {
				/* Detect polarity for new attach */
