	...
	while (1) {
		...
		case PD_STATE_SRC_DISCONNECTED:
			...
			if (!CC_NC(cc1_volt) || !CC_NC(cc2_volt)) {
				/*
				 * Set to USB SS initially (might change
				 * after the debounce)
				 */
#ifdef CONFIG_USBC_SS_MUX
		...
		case PD_STATE_SRC_DISCONNECTED_DEBOUNCE:
			...
			if (CC_NC(cc1_volt) && CC_NC(cc2_volt)) {
				...
				pd_power_supply_reset(port);
#ifdef CONFIG_DUAL_ROLE
				if (drp_state != PD_DRP_FORCE_SOURCE) {
					pd[port].power_role = PD_ROLE_SINK;
					set_state(port, PD_STATE_SNK_DISCONNECTED);
					pd_set_host_mode(port, 0);
				} else {
		...
		case PD_STATE_SNK_DISCONNECTED:
			 ...
			 * check for role swap. Do not role swap if Rp is
			 * detected.
			 */
			hard_reset_count = 0;
			cc1_volt = pd_adc_read(port, 0);
			...
			if (drp_state == PD_DRP_TOGGLE_ON &&
			    get_time().val >= next_role_swap &&
			    cc1_volt < PD_SNK_VA && cc2_volt < PD_SNK_VA) {
