...
static struct pd_protocol {
	...
	uint8_t vdo_count;
	/* VDO to retry do to responder's busy */
	uint32_t vdo_retry[1];
...
static inline void queue_vdm(int port, uint32_t *header, const uint32_t *data,
			     int data_cnt)
{
...
static void handle_vdm_request(int port, int cnt, uint32_t *payload)
{
	...
	if (pd[port].vdm_state == VDM_STATE_BUSY) {
		...
		if (PD_VDO_CMDT(payload[0]) == CMDT_RSP_BUSY) {
			pd[port].vdm_timeout = get_time().val + PD_T_VDM_BUSY;
			pd[port].vdm_state = VDM_STATE_WAIT_RSP_BUSY;
			pd[port].vdo_retry[0] = payload[0] & ~VDO_CMDT_MASK;
			return;
		} else {
...
static void pd_vdm_send_state_machine(int port, int incoming_packet)
{
	...
	case VDM_STATE_READY:
		...
		/*
		 * if there's traffic or we're not in ready state don't send a
		 * VDM */
		...
		if (res < 0) {
			...
			pd[port].vdm_timeout = get_time().val +
				vdm_get_ready_timeout(pd[port].vdo_data[0]);
		}
		break;
	case VDM_STATE_WAIT_RSP_BUSY:
		/* wait and then initiate request again */
		if (get_time().val > pd[port].vdm_timeout) {
			pd[port].vdo_data[0] = pd[port].vdo_retry[0];
			pd[port].vdo_count = 1;
			pd[port].vdm_state = VDM_STATE_READY;
		}
		break;
	case VDM_STATE_BUSY:
		...
		if (pd[port].vdm_timeout &&
		    (get_time().val > pd[port].vdm_timeout)) {
