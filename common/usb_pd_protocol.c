		...
		if (pd[port].power_role == PD_ROLE_SOURCE) {
			...
			if (type != TCPC_TX_HARD_RESET) {
				...
				if (cc1 == TYPEC_CC_VOLT_SNK_1_5 ||
					...
					if (pd[port].ca_buffer[0] == 0) {
						/*
						 * Buffer message and send
						 * later.
						 */
						pd[port].ca_type = type;
						pd[port].ca_buffer[0] = header;
						memcpy(&pd[port].ca_buffer[1],
...
static void pd_ca_send_pending(int port)
{
	...
	if (pd[port].ca_buffer[0]) {
		tcpm_get_cc(port, &cc1, &cc2);
		if ((cc1 != TYPEC_CC_VOLT_SNK_1_5) &&
				(cc2 != TYPEC_CC_VOLT_SNK_1_5))

			ret = pd_transmit(port, pd[port].ca_type,
				pd[port].ca_buffer[0], &pd[port].ca_buffer[1]);
