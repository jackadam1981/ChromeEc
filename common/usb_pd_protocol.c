...
#ifdef CONFIG_USB_PD_INITIAL_DRP_MULTI_STATE
	{ CONFIG_USB_PD_INITIAL_DRP_MULTI_STATE };
#else
		...
		uint32_t *payload)
{
	...
	if (pd[port].data_role == data_role) {
		CPRINTF("C%d DR conflict! %x %x\n", port, pd[port].data_role, data_role);
		/*
...
static void pd_update_try_source(void)
{
	 ...
	 */
	pd_try_src_enable = drp_state[port] == PD_DRP_TOGGLE_ON &&
...
void pd_set_dual_role(int port, enum pd_dual_role_states state)
{
	...
	CPRINTS("Set dualrole p%d to %s", port,
		(state == PD_DRP_FORCE_SINK) ? "snk" : ((state == PD_DRP_FORCE_SOURCE) ? "src" : "??"));
...
{
	CPRINTS("Update dualrole p%d pwr:%d", port, pd[port].power_role);
	/*
	...
	if (pd[port].power_role == PD_ROLE_SOURCE &&
	    ((drp_state[port] == PD_DRP_FORCE_SINK && !pd_ts_dts_plugged(port)) ||
	     (drp_state[port] == PD_DRP_TOGGLE_OFF
	      && pd[port].task_state == PD_STATE_SRC_DISCONNECTED))) {
		CPRINTS("Update dualrole p%d to snk", port);
		pd_set_power_role(port, PD_ROLE_SINK);
	...
	if (pd[port].power_role == PD_ROLE_SINK &&
		...
		CPRINTS("Update dualrole p%d to src", port);
...
#ifdef CONFIG_USB_PD_BOARD_INIT
	pd_board_init();
#endif
