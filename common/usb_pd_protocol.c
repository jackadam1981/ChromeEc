...
static struct pd_protocol {
	...
	/* Attached device is a dual-role power device */
	uint8_t drp;

#ifdef CONFIG_USB_PD_DUAL_ROLE
	/* original port power role (SOURCE or SINK) */
	uint8_t original_power_role;
...
static void execute_hard_reset(int port)
{
	...
	set_state(port, pd[port].original_power_role == PD_ROLE_SINK ?
		PD_STATE_SNK_DISCONNECTED : PD_STATE_SRC_DISCONNECTED);

	if (pd[port].power_role != pd[port].original_power_role) {
		/* Restore power role and CC resister to original state */
		pd[port].power_role = pd[port].original_power_role;
		pd_set_host_mode(port, (pd[port].power_role == PD_ROLE_SOURCE)
					? 1 : 0);
	}
...
static void pd_send_request_msg(int port)
{
	...
	if (res == EC_SUCCESS) {
		...
		if ((pd_src_caps[port][0] & PDO_TYPE_MASK) == PDO_TYPE_FIXED)
			pd[port].drp = (pd_src_caps[port][0] &
		...
		uint32_t *payload)
{
	...
	case PD_DATA_SINK_CAP:
		...
		if ((payload[0] & PDO_TYPE_MASK) == PDO_TYPE_FIXED)
			pd[port].drp = (payload[0] & PDO_FIXED_DUAL_ROLE) ?
					1 : 0;
		break;
...
static int analyze_rx(int port, uint32_t *payload)
{
	...
	while (bit > 0) {
		...
		if (val == PD_SOP) {
			...
			CPRINTF("SOP''");
			return bit;
		} else if (val ==PD_HARD_RESET) {
			CPRINTF("RCV HARD RST");
			return PD_ERR_HARD_RESET;
		}
	}
	if (bit < 0) {
		msg = "SOP";
...
int pd_get_dualrole_capable(int port)
{
	return pd[port].drp;
}
...
#ifdef CONFIG_USB_PD_DUAL_ROLE
				pd[port].original_power_role = PD_ROLE_SOURCE;
#endif
			}
#ifdef CONFIG_USB_PD_DUAL_ROLE
...
static int command_pd(int argc, char **argv)
{
	...
	if (!strcasecmp(argv[2], "tx")) {
		...
		ccprintf("Port C%d, %s - Role: %s-%s Polarity: CC%d DRP: %d, "
			...
			pd[port].polarity + 1, pd[port].drp,
