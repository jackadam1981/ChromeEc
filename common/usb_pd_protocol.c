	...
	while (1) {
		...
		case PD_STATE_SRC_DISCONNECTED:
			...
			if (pd[port].flags & PD_FLAGS_TRY_SRC) {
				...
				if (now < pd[port].try_src_marker ||
				    pd_is_vbus_present(port))
					break;

				if (now < pd[port].try_timeout)
					break;

				/*
				 * We need to transition now, so update src
				 * marker time and fall through to setting
				 * state below.
				 */
				 ...
				 */
				if (get_time().val < next_role_swap ||
				    drp_state[port] == PD_DRP_FORCE_SOURCE ||
				    drp_state[port] == PD_DRP_FREEZE)
					break;

				/*
				 * We need to transition now, so update next
				 * role swap time and fall through to setting
				 * state below.
				 */
