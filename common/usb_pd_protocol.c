...
#define PDO_FIXED_CURR(ma)  (((ma)/10) << 0)  /* Max current in 10mV units */
...
/* current port role */
static uint8_t pd_role = PD_ROLE_SOURCE;
				 ...
				 uint32_t *data)
{
	...
	for (r = 0; r <= PD_RETRY_COUNT; r++) {
		...
		if (head > 0) { /* we got a good packet, analyze it */
			...
			if (type == PD_CTRL_GOOD_CRC && cnt == 0 &&
			   id == pd_message_id) {
...
static void send_source_cap(void *ctxt)
{
	...
	bit_len = send_validate_message(ctxt, header, ARRAY_SIZE(pdo), pdo);
	CPRINTF("srcCAP>%d\n", bit_len);
}
...
void pd_task(void)
{
	...
	while (1) {
		...
		case PD_STATE_TX_PING:
		case PD_STATE_TX_CAP:
		case PD_STATE_TX:
			if (pd_task_state == PD_STATE_TX_CAP)
				send_sink_cap(ctxt);
			else
		...
		case PD_STATE_RX:
			...
			break;
		case PD_STATE_DISCONNECTED: /* TODO */
			/* Vnc monitoring */
			task_wait_event(-1);
		case PD_STATE_IDLE:
			pd_rx_enable_monitoring();
			/* ping every source every XxX */
			pd_task_state = PD_STATE_TX_PING;
			task_wait_event(-1);
			break;
		}
	}
