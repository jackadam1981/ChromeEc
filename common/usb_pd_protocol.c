...
#ifdef BOARD_ZINGER
#define CONFIG_USB_PD_DEVID_MAJOR USB_PD_HW_DEV_ID_ZINGER
#define CONFIG_USB_PD_DEVID_MINOR 1
#elif defined(BOARD_MINIMUFFIN)
#define CONFIG_USB_PD_DEVID_MAJOR USB_PD_HW_DEV_ID_MINIMUFFIN
#define CONFIG_USB_PD_DEVID_MINOR 0
#elif defined(BOARD_DINGDONG)
#define CONFIG_USB_PD_DEVID_MAJOR USB_PD_HW_DEV_ID_DINGDONG
#define CONFIG_USB_PD_DEVID_MINOR 0
#elif defined(BOARD_HOHO)
#define CONFIG_USB_PD_DEVID_MAJOR USB_PD_HW_DEV_ID_HOHO
#define CONFIG_USB_PD_DEVID_MINOR 0
#endif

#if defined(CONFIG_USB_PD_DEVID_MAJOR) && defined(CONFIG_USB_PD_DEVID_MINOR)
	info_data[5] = VDO_INFO(CONFIG_USB_PD_DEVID_MAJOR,
				CONFIG_USB_PD_DEVID_MINOR,
...
static int command_pd(int argc, char **argv)
{
	...
	if (!strcasecmp(argv[1], "dualrole")) {
		...
		for (i = 0; i < RW_HASH_ENTRIES; i++) {
			p = &rw_hash_table[i];
			pd_dev_dump_info(p->dev_id, p->dev_rw_hash.b);
		}
		return EC_SUCCESS;
	}
