	...
	case VDM_STATE_READY:
		...
		if ((msg_type == PD_MSG_SOP_PRIME) ||
			...
			res = pd_transmit(port,
					  ...
					  pd[port].vdo_data);
			/*
			 * In the case of SOP', if there is no ack from the
			 * cable, it's a non-emark cable and therefore the pd
			 * flow should continue irrespective of cable response,
			 * sending discover_identity so the pd flow remains
			 * intact.
			 *
			 * In the case of SOP'', if there is no ack from the
			 * cable, exit Thunderbolt-Compatible mode discovery,
			 * reset the mux state since, the mux will be set to a
			 * safe state before entering Thunderbolt-Compatible
			 * mode and enter the appropriate mode.
			 */
			if (res < 0) {
				...
				pd[port].vdo_data[0] =
					VDO(USB_SID_PD, 1,
					(msg_type == PD_MSG_SOP_PRIME) ?
					CMD_DISCOVER_SVID : CMD_ENTER_MODE);
