...
static int pd_get_saved_vbus_more_5v(int port)
{
	uint8_t val;
...
static void pd_set_saved_vbus_more_5v(int port, int val)
{
	/*
	 ...
	 * received and VBUS > 5V. In all other instances, this flah will be
	 * reset.
	 */
		...
		uint32_t *payload)
{
	...
	case PD_CTRL_PS_RDY:
		if (pd[port].task_state == PD_STATE_SNK_SWAP_SRC_DISABLE) {
			 ...
			 */
			pd_set_saved_vbus_more_5v(port,
...
static void pd_partner_port_reset(int port)
{
	 ...
	 */
	if (!pd_get_saved_vbus_more_5v(port) ||
	...
	while (get_time().val < timeout && pd_is_vbus_present(port))
		msleep(10);
	pd_set_saved_vbus_more_5v(port, 0);
}
#endif /* CONFIG_USB_PD_DUAL_ROLE */
