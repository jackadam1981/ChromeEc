...
static void bist_mode_2_tx(int port)
{
	...
	if (pd[port].tx_bist_test_msec == 0) {
		/* do not let pd task state machine run anymore */
		while (1)
			task_wait_event(-1);
	} else {
		/* Length of BIST specified in tx_bist_test_msec. */
		task_wait_event(pd[port].tx_bist_test_msec);
		/* clear dma circular mode, will also stop dma */
		pd_tx_clear_circular_mode(port, pd[port].polarity);
		/* Set to appropriate port disconnected state */
#ifdef CONFIG_USB_PD_DUAL_ROLE
		set_state(port, pd[port].power_role == PD_ROLE_SINK ?
	...
	case PD_DATA_BIST:
		...
		if (pd[port].task_state == PD_STATE_SRC_READY) {
			...
			if ((payload[0] >> 28) == 5) {
				...
				pd[port].tx_bist_test_msec = PD_T_BIST_TRANSMIT;
				bist_mode_2_tx(port);
			}
		}
		break;
...
static int command_pd(int argc, char **argv)
{
	...
	if (!strcasecmp(argv[2], "tx")) {
		...
		if (*e)
			return EC_ERROR_PARAM3;
		pd[port].tx_bist_test_msec = duration;
