...
static int reset_device_and_notify(int port)
{
	 ...
	 * tasks gets an interrupt during the above tcpm_init method. When that
	 * occurs the higher priority task is waiting correctly, but is has
	 * enqueue another reset event which would get processes causing a
	 * second init to get called.
	 */
