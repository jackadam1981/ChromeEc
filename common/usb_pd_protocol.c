...
{
#ifdef CONFIG_USB_PD_DISCHARGE_GPIO
	gpio_set_level(pd_discharge_gpio[1], 0);
#elif defined(CONFIG_USB_PD_DISCHARGE_TCPC)
	tcpc_discharge_vbus(1, 0);
#endif
}
...
{
#ifdef CONFIG_USB_PD_DISCHARGE_GPIO
	gpio_set_level(pd_discharge_gpio[0], 0);
#elif defined(CONFIG_USB_PD_DISCHARGE_TCPC)
	tcpc_discharge_vbus(0, 0);
#endif
}
...
#elif defined(CONFIG_USB_PD_DISCHARGE_TCPC)
	tcpc_discharge_vbus(port, 1);
#endif
	if (port)
		hook_call_deferred(
			&pd_power_discharge_port1_deferred_data, 250 * MSEC);
	else
		hook_call_deferred(
			&pd_power_discharge_port0_deferred_data, 250 * MSEC);
