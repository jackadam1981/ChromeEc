...
#define RAW_PRINTF(format, args...) cprintf(CC_USBPD, format, ## args)

/* dump full packet on RX error */
static int debug_dump;
#else
#define CPRINTF(format, args...)
#define RAW_PRINTF(format, args...)
				 ...
				 const uint32_t *data)
{
	...
	/* TODO: try HardReset */
	CPRINTF("PD TX NO ACK %04x/%d", header, cnt);
	return -1;
}
...
static int send_control(void *ctxt, int type)
{
	...
	CPRINTF("PD CTRL[%d]>%d", type, bit_len);
...
#endif
	CPRINTF("PD Unhandled VDM VID %04x CMD %04x",
...
static void handle_data_request(void *ctxt, uint16_t head, uint32_t *payload)
{
	...
	case PD_DATA_VENDOR_DEF:
		...
		break;
	default:
		CPRINTF("PD unhandled data message type %d", type);
	}
}
...
static void handle_ctrl_request(void *ctxt, uint16_t head, uint32_t *payload)
{
	...
	default:
		CPRINTF("PD unhandled ctrl message type %d", type);
	}
}
...
static void handle_request(void *ctxt, uint16_t head, uint32_t *payload)
{
	...
	RAW_PRINTF("[%T PD RECV %04x/%d ", head, cnt);
	for (p = 0; p < cnt; p++)
		RAW_PRINTF("[%d]%08x ", p, payload[p]);
	RAW_PRINTF("]\n");
...
static int analyze_rx(uint32_t *payload)
{
	...
	if (bit < 0 || pcrc != ccrc) {
		...
		if (pcrc != ccrc)
			bit = PD_ERR_CRC;
		/* DEBUG */
		CPRINTF("PD CRC %08x <> %08x", pcrc, crc32_result());
		goto packet_err;
	}
