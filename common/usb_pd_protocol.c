...
static int pd_get_saved_port_reset(int port)
{
	uint8_t val;
...
static void pd_set_saved_port_reset(int port, int val)
{
...
static void pd_partner_port_reset(int port)
{
	 ...
	 */
	if (!pd_get_saved_port_reset(port) ||
	...
	while (get_time().val < timeout && pd_is_vbus_present(port))
		msleep(10);
	pd_set_saved_port_reset(port, 0);
}
#endif /* CONFIG_USB_PD_DUAL_ROLE */
