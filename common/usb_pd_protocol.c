		...
		uint32_t *payload)
{
	...
	case PD_CTRL_PR_SWAP:
		...
		if (pd_check_power_swap(port)) {
			...
			/* Disable fast swap when power swap */
			pd[port].flags &= ~PD_FLAGS_FAST_SWAP;
...
void pd_task(void *u)
{
	...
	while (1) {
		...
		case PD_STATE_SNK_SWAP_COMPLETE:
			...
			pd[port].msg_id = 0;
			pd[port].flags &= ~PD_FLAGS_FAST_SWAP;
			//tcpm_set_fast_swap(port, PD_ROLE_SINK, 0);//HW auto dis
			pd_set_power_role(port, PD_ROLE_SOURCE);
			pd_update_roles(port);
			set_state(port, PD_STATE_SRC_DISCOVERY);
			timeout = 10*MSEC;
		...
		case PD_STATE_SNK_FAST_SWAP_SNK_DISABLE:
			...
			timeout = 2*MSEC; // timing?
			/* moniter Vbus <= 5v, but ppc config 5v ? */
			if (pd_is_vbus_present(port) == 0) {
		...
		case PD_STATE_SNK_FAST_SWAP_INIT:
			if (pd[port].last_state != pd[port].task_state) {
				/*
				 * If Vbus <= 5v, enable Vbus to 5v within
				 * tSrcFRSwap.
				 */
				if (pd_set_power_supply_ready(port)) {
					timeout = 10*MSEC;
