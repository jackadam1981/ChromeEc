				 ...
				 const uint32_t *data)
{
	...
	/* TODO: try HardReset */
	CPRINTS("PD TX NO ACK %04x/%d", header, cnt);
	return -1;
}
...
static int send_control(void *ctxt, int type)
{
	...
	CPRINTS("PD CTRL[%d]>%d", type, bit_len);
...
#endif
	CPRINTS("PD Unhandled VDM VID %04x CMD %04x",
...
static void handle_data_request(void *ctxt, uint16_t head, uint32_t *payload)
{
	...
	case PD_DATA_VENDOR_DEF:
		...
		break;
	default:
		CPRINTS("PD unhandled data message type %d", type);
	}
}
...
static void handle_ctrl_request(void *ctxt, uint16_t head, uint32_t *payload)
{
	...
	default:
		CPRINTS("PD unhandled ctrl message type %d", type);
	}
}
...
static int analyze_rx(uint32_t *payload)
{
	...
	if (bit < 0 || pcrc != ccrc) {
		...
		if (pcrc != ccrc)
			bit = PD_ERR_CRC;
		/* DEBUG */
		CPRINTS("PD CRC %08x <> %08x", pcrc, crc32_result());
		goto packet_err;
	}
