...
static void pd_send_enter_usb(int port)
{
	...
	res = pd_transmit(port, TCPC_TX_SOP, header, &usb4_payload.raw_value);
	if (res < 0) {
...
static uint8_t get_pd_control_flags(int port)
{
	 ...
	 */
	return ((cable_resp.lsrx_comm << USB_PD_MUX_TBT_LINK) |
		(cable_resp.tbt_cable << USB_PD_MUX_TBT_CABLE_TYPE) |
		(device_resp.tbt_adapter << USB_PD_MUX_TBT_ADAPTER) |
		 cable_resp.tbt_retimer << USB_PD_MUX_TBT_ACTIVE_CABLE);
}
...
static enum ec_status hc_usb_pd_control(struct host_cmd_handler_args *args)
{
	...
	case 2:
		...
		r_v2->flags = get_pd_control_flags(p->port);
