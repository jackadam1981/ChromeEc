...
static inline void set_state(int port, enum pd_states next_state)
{
	...
	uint32_t flags_to_clear = PD_FLAGS_RESET_ON_DISCONNECT_MASK;
	...
	if (next_state == PD_STATE_SRC_DISCONNECTED) {
		...
		pd[port].dev_id = 0;
		/*
		 * Don't clear the DR initialized flag if and only if the PD
		 * state machine is just starting.  We may have already set it
		 * based on an existing explicit contract.
		 */
		if (!last_state)
			flags_to_clear &= ~PD_FLAGS_DR_INITIALIZED;
...
{
	if (system_get_bbram(get_bbram_idx(port), flags)) {
...
static void pd_partner_port_reset(int port)
{
	 ...
	 * renegotiate our contract.  This SoftReset comes as a result of us
	 * transitioning to PD_STATE_SNK_DISCOVERY and waiting for SRC_CAP
	 * messages to arrive.  These messages don't arrive because we're
	 * already in an explicit contract, therefore we issue a SoftReset.
	 * This particular condition only applies to unlocked RO images with an
	 * explicit contract in place.
	 */
...
#ifdef CONFIG_USB_PD_DUAL_ROLE
	/*
	 * If there's an explicit contract in place, let's restore the data role
	 * such that any messages we send to the port partner will still be
	 * valid.
	 */
