...
static inline void set_state(int port, enum pd_states next_state)
{
	...
	if (next_state == PD_STATE_SRC_DISCONNECTED) {
		 ...
		 */
		if (!last_state && pd_data_role_initialized(port))
...
{
	if (system_set_bbram(get_bbram_idx(port), flags))
		CPRINTS("PD NVRAM FAIL");
}
...
static void pd_update_saved_port_flags(int port, uint8_t flag, uint8_t val)
{
	...
	if (pd_get_saved_port_flags(port, &saved_flags))
		return;
		...
		uint32_t *payload)
{
	...
	if (pd[port].data_role == data_role) {
		 ...
		 */
		if (!tcpm_set_cc(port, TYPEC_CC_OPEN))
...
static void pd_partner_port_reset(int port)
{
	...
	if (flags & PD_BBRMFLG_EXPLICIT_CONTRACT)
		explicit_contract_in_place = 1;
	else
...
void pd_task(void *u)
{
	 ...
	 * valid.
	 */
	if (!pd_get_saved_port_flags(port, &saved_flgs))
