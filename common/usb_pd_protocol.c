...
static void send_sink_cap(int port)
{
	...
	const uint32_t *dyn_snk_pdo;
	const int dyn_snk_pdo_cnt = board_get_snk_cap(&dyn_snk_pdo, port);
#else
	const uint32_t *dyn_snk_pdo = pd_snk_pdo;
	const int dyn_snk_pdo_cnt = pd_snk_pdo_cnt;
#endif
	...
	bit_len = pd_transmit(port, TCPCI_MSG_SOP, header, dyn_snk_pdo,
			      AMS_RESPONSE);
	if (debug_level >= 2)
...
__overridable int board_get_snk_cap(const uint32_t **snk_pdo, const int port)
{
	 ...
	 * UNCONSTRAINED_POWER bit or other PDO based on dynamic state.
	 */
	return 0;
}
