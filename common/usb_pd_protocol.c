 ...
 * since the concern over "low powercharger" is not relevant when connected as a
 * source and the additional delay avoids a race condition where th partner port
 * sends a power role swap request close to when the VDM discover idenity
 * message gets sent.
 */
