...
static void pd_send_enter_usb(int port)
{
	int res;
	uint16_t header;
	uint32_t usb4_payload = enter_usb_payload(port);
...
static void usb_pd_mux_enter_usb(int port)
{
	if (IS_ENABLED(CONFIG_USBC_SS_MUX) && IS_ENABLED(CONFIG_USB_PD_USB4))
	...
	while (1) {
		...
		if (is_enter_usb(port)) {
			...
			if (IS_ENABLED(CONFIG_USBC_SS_MUX) &&
				...
				usb_pd_mux_enter_usb(port);
			}
			disable_enter_usb(port);
 ...
 * Bit 3: Link_training
 */
static uint8_t get_flags(int port)
{
	struct tbt_mode_resp_cable cable_resp = get_cable_resp_vdo(port);
	struct tbt_mode_resp_device device_resp = get_dev_resp_vdo(port);
...
static enum ec_status hc_usb_pd_control(struct host_cmd_handler_args *args)
{
	...
	case 3:
		...
		r_v3->flags = get_flags(p->port);
		r_v3->cable_speed = get_cable_speed(p->port);
		r_v3->cable_gen = get_rounded_support(p->port);
