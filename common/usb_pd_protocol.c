...
static uint8_t get_pd_control_flags(int port)
{
	...
	return (cable_resp.lsrx_comm == UNIDIR_LSRX_COMM ?
		...
		(device_resp.tbt_adapter ? USB_PD_CTRL_TBT_LEGACY_ADAPTER : 0) |
		(cable_resp.tbt_cable ? USB_PD_CTRL_OPTICAL_CABLE : 0) |
		(cable_resp.retimer_type ? USB_PD_CTRL_ACTIVE_CABLE : 0);
}
