...
static enum pd_states drp_auto_toggle_next_state(int port, int cc1, int cc2)
{
	...
	if (cc1 == TYPEC_CC_VOLT_OPEN &&
		...
		/*
		 * When the chipset is off, our policy is to become a sink only.
		 */
		if (chipset_in_state(CHIPSET_STATE_ANY_OFF |
				     CHIPSET_STATE_ANY_SUSPEND)) {
			/* Stop dual role toggling and become a sink. */
			next_state = PD_STATE_SNK_DISCONNECTED;
		} else {
...
void pd_task(void *u)
{
	...
	while (1) {
		...
		case PD_STATE_SNK_DISCONNECTED:
			 ...
			 * auto toggling and not try.src, and the chipset is on.
			 */
			if (auto_toggle_supported &&
			    !(pd[port].flags & PD_FLAGS_TCPC_DRP_TOGGLE) &&
			    !(pd[port].flags & PD_FLAGS_TRY_SRC) &&
