		       ...
		       const uint32_t *data)
{
	...
	tcpm_transmit(port, type, header, cnt, data);

	/* Wait until TX is complete */
	do {
		evt = task_wait_event(100 * MSEC);
...
static int send_control(int port, int type)
{
	...
	bit_len = pd_transmit(port, TRANSMIT_SOP, header, 0, NULL);
...
static int send_source_cap(int port)
{
	...
	bit_len = pd_transmit(port, TRANSMIT_SOP, header, src_pdo_cnt, src_pdo);
	if (debug_level >= 1)
		CPRINTF("srcCAP>%d\n", bit_len);
...
static void send_sink_cap(int port)
{
	...
	bit_len = pd_transmit(port, TRANSMIT_SOP, header, pd_snk_pdo_cnt,
			      pd_snk_pdo);
	if (debug_level >= 1)
		CPRINTF("snkCAP>%d\n", bit_len);
}
...
static int send_request(int port, uint32_t rdo)
{
	...
	bit_len = pd_transmit(port, TRANSMIT_SOP, header, 1, &rdo);
	if (debug_level >= 1)
		CPRINTF("REQ%d>\n", bit_len);
...
static int send_bist_cmd(int port)
{
	...
	bit_len = pd_transmit(port, TRANSMIT_SOP, header, 1, &bdo);
	CPRINTF("BIST>%d\n", bit_len);
	...
	case PD_DATA_BIST:
		...
		if (DUAL_ROLE_IF_ELSE(port,
			...
			if ((payload[0] >> 28) == 5) {
				...
				pd_transmit(port, TRANSMIT_BIST_MODE_2, 0, 0,
					    NULL);
...
static void pd_vdm_send_state_machine(int port)
{
	...
	case VDM_STATE_READY:
		...
		res = pd_transmit(port, TRANSMIT_SOP, header,
				  pd[port].vdo_count, pd[port].vdo_data);
		if (res < 0) {
...
void pd_task(void)
{
	...
	while (1) {
		...
		if (res != EC_SUCCESS) {
			...
			pd_transmit(port, TRANSMIT_HARD_RESET, 0, 0, NULL);
		}

		/* wait for next event/packet or timeout expiration */
		incoming_packet = task_wait_event(timeout) & PD_EVENT_RX;
		if (incoming_packet) {
		...
		case PD_STATE_SRC_DISCONNECTED:
			...
			/* Vnc monitoring */
			if (tcpm_get_cc(port, 0) != TYPEC_CC_SRC_OPEN ||
			    tcpm_get_cc(port, 1) != TYPEC_CC_SRC_OPEN) {
		...
		case PD_STATE_BIST_TX:
			pd_transmit(port, TRANSMIT_BIST_MODE_2, 0, 0, NULL);
			timeout = 2 * PD_T_BIST_TRANSMIT;
			/* Set to appropriate port disconnected state */
			set_state(port, DUAL_ROLE_IF_ELSE(port,
						PD_STATE_SNK_DISCONNECTED,
