...
static inline void set_state(int port, enum pd_states next_state)
{
	...
	if (next_state == PD_STATE_SRC_DISCONNECTED ||
		...
		if (pd[port].flags & PD_FLAGS_PARTNER_FAST_SWAP) {
			tcpm_set_frs_enable(port, 0);
			ppc_fast_swap_to_src_enable(port, 0);
...
void pd_execute_hard_reset(int port)
{
	...
	if ((pd[port].power_role == PD_ROLE_SINK) &&
		...
		ppc_fast_swap_to_src_enable(port, 0);
		ppc_vbus_source_enable(port, 0);
		/* After hard reset, we will check FRS again in SNK_RDY state */
...
void pd_request_power_swap(int port)
{
	if (pd[port].task_state == PD_STATE_SRC_READY) {
		/* Later in SNK_RDY state check policy for enable fast swap */
		pd[port].flags |= PD_FLAGS_CHECK_FAST_SWAP;
		set_state(port, PD_STATE_SRC_SWAP_INIT);
	} else if (pd[port].task_state == PD_STATE_SNK_READY) {
		/* Disable fast role swap when we request power swap to SRC */
		if (pd[port].flags & PD_FLAGS_PARTNER_FAST_SWAP) {
			tcpm_set_frs_enable(port, 0);
			ppc_fast_swap_to_src_enable(port, 0);
			ppc_vbus_source_enable(port, 0);
		...
		uint32_t *payload)
{
	...
	case PD_CTRL_PR_SWAP:
		...
		if (pd_check_power_swap(port)) {
			...
			/* Later in SNK_RDY check policy for enable fast swap */
			pd[port].flags |= PD_FLAGS_CHECK_FAST_SWAP;
			/* Disable fast role swap when we receive power swap */
			if ((pd[port].power_role == PD_ROLE_SINK) &&
			    (pd[port].flags & PD_FLAGS_PARTNER_FAST_SWAP)) {
				tcpm_set_frs_enable(port, 0);
				ppc_fast_swap_to_src_enable(port, 0);
...
void pd_task(void *u)
{
	...
	while (1) {
		...
		/*
		 * Detect SRC cc signal from Rp to GND for fast swap request.
		 * If ppc detect Vbus <= 5v, auto enable Vbus to 5v within
		 * tSrcFRSwap (150us).
		 */
		...
		case PD_STATE_SNK_FAST_SWAP_SNK_DISABLE:
			...
			charge_manager_set_ceil(port, CEIL_REQUESTOR_PD,
						CHARGE_CEIL_NONE);
#endif
			/* Disable PP2_EN and charger setting */
			pd_set_power_supply_ready(port);
			set_state(port, PD_STATE_SNK_FAST_SWAP_INIT);
			/* Within tFRSwapInit 15ms tx FR_SWAP */
			timeout = 5*MSEC;
			break;
