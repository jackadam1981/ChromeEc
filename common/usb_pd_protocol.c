...
#define PD_FLAGS_VBUS_PRESENT      (1 << 12)/* vbus present in sink disconn. */
...
#ifdef CONFIG_CHARGE_MANAGER
	/* Initilize PD supplier current limit to 0 */
	pd_set_input_current_limit(port, 0, 0);
#endif
...
#ifdef CONFIG_TYPEC_FSM_BACKWARDS_COMPATIBLE_DFP
				/* Enable VBUS */
		...
		case PD_STATE_SRC_DISCONNECTED_DEBOUNCE:
			...
			if (new_cc_state == PD_CC_UFP_ATTACHED) {
				...
				board_set_usb_mux(port, TYPEC_MUX_USB,
						  pd[port].polarity);
#endif
#ifndef CONFIG_TYPEC_FSM_BACKWARDS_COMPATIBLE_DFP
				/* Enable VBUS */
				if (pd_set_power_supply_ready(port)) {
#ifdef CONFIG_USBC_SS_MUX
				board_set_usb_mux(port, TYPEC_MUX_NONE,
