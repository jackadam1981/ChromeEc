...
/* Keep track of when each supplier is registered. */
static timestamp_t registration_time[CHARGE_SUPPLIER_COUNT][PD_PORT_COUNT];
...
static void charge_manager_refresh(void)
{
	...
	if (new_port != charge_port || new_charge_current != charge_current ||
		...
		chg_ramp_charge_supplier_change(
				...
				registration_time[new_supplier][new_port]);
				       ...
				       struct charge_port_info *charge)
{
	...
	if (change == CHANGE_CHARGE) {
		...
		registration_time[supplier][port] = get_time();
