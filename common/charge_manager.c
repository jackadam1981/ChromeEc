...
{
	return 0 <= port && port < CHARGE_PORT_COUNT;
}
...
{
	if (!is_pd_port(port))
		/* Dedicated charge port is sink-only */
		return 1;
	...
	struct ec_response_usb_pd_power_info *r)
{
	...
	else if (is_pd_port(port) && pd_is_connected(port) && !is_sink(port))
...
#ifdef CONFIG_USB_PD_LOGGING
		if (pd_is_connected(p) && !is_sink(p))
