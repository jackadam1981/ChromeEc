...
static void charge_manager_refresh(void)
{
	...
	if (new_port != charge_port || new_charge_current != charge_current ||
		...
		/*
		 * Wake the power button task, in case it is waiting to power
		 * on the system after init (PWRBTN_STATE_INIT_ON), which
		 * requires the charge current has been initialized.
		 */
