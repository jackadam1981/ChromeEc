 ...
 */
int charge_manager_is_valid_pd_port(int port)
{
...
static void charge_manager_init(void)
{
	...
	for (i = 0; i < CHARGE_PORT_COUNT; ++i) {
		...
		if (spoof_capability || !charge_manager_is_valid_pd_port(i))
			dualrole_capability[i] = CAP_DEDICATED;
		if (charge_manager_is_valid_pd_port(i))
...
{
	ASSERT(charge_manager_is_valid_pd_port(port));
	...
	struct ec_response_usb_pd_power_info *r)
{
	...
	else if (charge_manager_is_valid_pd_port(port) &&
...
void charge_manager_save_log(int port)
{
	...
	if (!charge_manager_is_valid_pd_port(port))
		return;
...
static void charge_manager_refresh(void)
{
	...
	if (charge_manager_is_valid_pd_port(updated_new_port))
		pd_set_new_power_request(updated_new_port);
	if (charge_manager_is_valid_pd_port(updated_old_port))
...
{
	ASSERT(port >= 0 && port < CHARGE_PORT_COUNT);
