...
/* Periodically log info about active port */
#define LOG_ACTIVE_PORT_TIMEOUT (60 * SECOND)
...
static void charge_manager_log_active_port(void);
#endif
...
#ifdef CONFIG_USB_PD_LOGGING
	hook_call_deferred(charge_manager_log_active_port,
			   LOG_ACTIVE_PORT_TIMEOUT);
#endif
}
...
DECLARE_DEFERRED(charge_manager_log_active_port);
...
static void charge_manager_make_change(enum charge_manager_change_type change,
				       ...
				       enum dualrole_capabilities cap)
{
	int i;
				  ...
				  struct charge_port_info *charge)
{
	...
	charge_manager_make_change(CHANGE_CHARGE, supplier, port, charge, 0);
}
...
{
	ASSERT(port >= 0 && port < PD_PORT_COUNT);
	charge_manager_make_change(CHANGE_DUALROLE, 0, port, NULL, cap);
}
