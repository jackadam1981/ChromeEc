				   ...
				   uint32_t supply_voltage)
{
	 ...
	 */
	if (dts)
		choosed_supplier = charge.current < 1500 ?
				CHARGE_SUPPLIER_TYPEC_DTS_UNDER_1_5A :
				CHARGE_SUPPLIER_TYPEC_DTS;
	else
		choosed_supplier = charge.current < 1500 ?
	...
	choosed_supplier =
		dts ? CHARGE_SUPPLIER_TYPEC_DTS : CHARGE_SUPPLIER_TYPEC;
#endif /* CHARGE_MANAGER_BC12 */

	charge_manager_update_charge(choosed_supplier, port, &charge);
	...
	for (i = 0; i < ARRAY_SIZE(typec_suppliers); ++i)
		if (choosed_supplier != typec_suppliers[i])
