...
static enum ec_status hc_smart_discharge(struct host_cmd_handler_args *args)
{
	...
	dzone.stayup = MIN(hours_to_survive * drate.hibern /1000, capacity);
	dzone.cutoff = MIN(hours_to_survive * drate.cutoff /1000, dzone.stayup);

	/* Let the caller know the effective values. */
	r->hours_to_survive = hours_to_survive;
	r->dzone = dzone;
	r->drate = drate;
	args->response_size = sizeof(*r);
