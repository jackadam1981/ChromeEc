...
/* Flag if PD ceil has been enforced by PSNKSTDBY. */
...
void charge_manager_force_ceil(int port, int ceil)
{
	...
	if (left_safe_mode && port == charge_port && ceil < charge_current) {
		/*
		 * Set pd_ceil_enforced flag, if the ceil has been forced. The
		 * flag will enforce the charge_manager to set the new CL to
		 * prevent a race condition. There are two consecutive CL
		 * changes in PE_SNK_SELECT_CAPABILITY (to apply PSNKSTDBY), and
		 * in PE_SNK_TRANSITION_SINK(to lift PSNKSTDBY) respectively.
		 * The time frame of two hook calls is short, and sometimes the
		 * first charge_manager_refresh hook may be in process when the
		 * second hook is deferred. This might cause the CL evaluation
		 * not detecting the charge_ceil applied from the
		 * PE_SNK_SELECT_CAPABILITY, and hence the CL has been capped at
		 * the PSNKSTDBY.
		 */
