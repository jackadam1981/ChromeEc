...
__maybe_unused static int chip_i2c_xfer_with_notify(
	const int port, const uint16_t peripheral_addr_flags,
	const uint8_t *out, int out_size,
	uint8_t *in, int in_size, int flags)
{
	int ret;
	uint16_t addr_flags = peripheral_addr_flags;
	const struct i2c_port_t *i2c_port = get_i2c_port(port);

	if (IS_ENABLED(CONFIG_I2C_XFER_BOARD_CALLBACK))
		i2c_start_xfer_notify(port, peripheral_addr_flags);
	...
	if (IS_ENABLED(CONFIG_I2C_XFER_BOARD_CALLBACK))
		i2c_end_xfer_notify(port, peripheral_addr_flags);

	if (IS_ENABLED(CONFIG_I2C_DEBUG)) {
		i2c_trace_notify(port, peripheral_addr_flags, out, out_size,
				 in, in_size);
	}
...
int i2c_xfer_unlocked(const int port,
		      const uint16_t peripheral_addr_flags,
		      const uint8_t *out, int out_size,
		      uint8_t *in, int in_size, int flags)
{
	int i;
	int ret = EC_SUCCESS;
	uint16_t addr_flags = peripheral_addr_flags & ~I2C_FLAG_PEC;
