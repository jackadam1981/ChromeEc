...
static void enable_factory_mode_failed(void)
{
	ccd_hook_active = 0;
	CPRINTF("%s: factory mode enable failed\n");
	deassert_ec_rst();
}
DECLARE_DEFERRED(enable_factory_mode_failed);
...
static void enable_factory_mode_deferred(void)
{
	int rv;
	...
	CPRINTF("%s: TPM reset succeeded, enabling factory mode\n", __func__);

	ccd_hook_active = 1;
	rv = ccd_reset_config(CCD_RESET_FACTORY);
	if (rv != EC_SUCCESS)
		enable_factory_mode_failed();

	/*
	 * Make sure we never end up with the EC held in reset, no matter what
	 * prevents the proper factory reset flow from succeeding.
	 */
	hook_call_deferred(&enable_factory_mode_failed_data, TPM_RESET_TIME);
}
DECLARE_DEFERRED(enable_factory_mode_deferred);

void enable_ccd_factory_mode(void)
{
	delay_sleep_by(DISABLE_SLEEP_TIME);
	hook_call_deferred(&enable_factory_mode_deferred_data,
		TPM_PROCESSING_TIME);
}
