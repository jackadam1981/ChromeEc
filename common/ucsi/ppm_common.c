    ...
    case UCSI_CMD_ACK_CC_CI:
      ...
      if (ack_cmd->connector_change_ack) {
        if (dev->last_connector_changed != -1) {
          ...
          ELOG(
              "Ack invalid at state %d. Ack byte (0x%x), Last Connector "
              "Changed Notification (%d)",
              dev->ppm_state, control->command_specific[0],
              dev->last_connector_changed);

          // Set error condition to invalid cmd specific params.
          clear_last_error(dev);
          dev->last_error = ERROR_PPM;
          dev->ppm_error_result.error_information.invalid_cmd_specific_params =
              1;
          goto error_cci;
        }
      }
...
success:
  ...
  switch (ucsi_command) {
    case UCSI_CMD_ACK_CC_CI:
      // If we handled an async event ack.
      if (ack_ci) {
        struct ucsiv3_get_connector_status_data* port_status =
        ...
        // Flag a pending async event but keep port status read true.
...
error_cci:
  // Set CCI error, command complete and return.
  clear_cci(dev);
  cci->error = 1;
  cci->cmd_complete = 1;
  return ret;
  ...
  if (dev->pending.command) {
    ...
    switch (dev->ppm_state) {
      ...
      case PPM_STATE_PROCESSING_COMMAND:
        ...
        // are allowed here.
        if (next_command == UCSI_CMD_ACK_CC_CI) {
          struct ucsiv3_ack_cc_ci_cmd* cmd =
              (struct ucsiv3_ack_cc_ci_cmd*)
                  dev->ucsi_data.control.command_specific;

          if (!cmd->connector_change_ack || cmd->command_complete_ack) {
            ELOG("Invalid ack usage (CI=%d CC=%d) in state %d",
      ...
      case PPM_STATE_WAITING_ASYNC_EV_ACK:
        // If it wasn't UCSI_CMD_ACK_CC_CI, it's an error
        if (next_command != UCSI_CMD_ACK_CC_CI) {
          ELOG("Got command 0x%x when expecting ACK_CC_CI (0x%x). Dropping it.",
               next_command, UCSI_CMD_ACK_CC_CI);

          // Technically just invalid command but not unrecognized command.
          clear_last_error(dev);
          dev->last_error = ERROR_PPM;
          dev->ppm_error_result.error_information
              .invalid_cmd_specific_params = 1;
        ...
        // Only update CCI if we successfully acknowledge changes.
        ret = ppm_common_execute_pending_cmd(dev);
        if (ret != -1) {
          dev->ppm_state = PPM_STATE_IDLE_NOTIFY;
...
static void ppm_common_task(void* context) {
  ...
  do {
    ...
    switch (dev->ppm_state) {
      ...
      case PPM_STATE_WAITING_ASYNC_EV_ACK:
        if (is_pending_command(dev)) {
          // Waiting ASYNC_EV_ACK is a weird state. It can directly ACK the CI
          // or it can go into a PROCESSING_COMMAND state (in which case it
          // should be treated as a IDLE_NOTIFY).
          //
          // Thus, if we don't get UCSI_CMD_ACK_CC_CI here, we just treat this
          // as IDLE_NOTIFY state.
          if (!match_pending_command(dev, UCSI_CMD_ACK_CC_CI)) {
