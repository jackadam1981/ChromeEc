			     ...
			     void (*send_response)(int len), uint8_t *data)
{
	...
	case I2C_HID_COMMAND_REGISTER:
		ret = i2c_hid_touchpad_command_process(len, buffer,
						       send_response, data);
		break;
	default:
					    ...
					    uint8_t *data)
{
	...
	case I2C_HID_CMD_GET_REPORT:
		//ccprints("I2C-HID: command get_report (%04x)", report_id);
		switch (report_id) {
	...
	case I2C_HID_CMD_SET_REPORT:
		//ccprints("I2C-HID: command set_report (%04x)", report_id);
		switch (report_id) {
		case REPORT_ID_INPUT_MODE:
