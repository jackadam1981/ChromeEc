...
/* We sometime need to pad some bits at the end of each finger struct to match
 * the allocation unit so the array indexing may work correctly.
 */
#define N_VAR_BITS \
( \
...
#define N_PADDING_BITS \
( \
		((((N_VAR_BITS - 1) >> 3) + 1) << 3) - N_VAR_BITS \
)
...
struct finger {
	...
	uint16_t :N_PADDING_BITS;	/* Nameless padding bits */
} __packed;
...
#define FINGER(FINGER_NUMBER)						       \
	...
	0x75, N_PADDING_BITS,		/*   Report Size (variable) */         \
	0x95, 0x01,			/*   Report Count (1) */               \
