...
/* Touchpad properties */
#ifndef I2C_HID_TOUCHPAD_MAX_X
#define I2C_HID_TOUCHPAD_MAX_X			13184
#endif
#ifndef I2C_HID_TOUCHPAD_MAX_Y
#define I2C_HID_TOUCHPAD_MAX_Y			8704
#endif
/* Physical dimensions are in the unit of mms */
#ifndef I2C_HID_TOUCHPAD_MAX_PHYSICAL_X
#define I2C_HID_TOUCHPAD_MAX_PHYSICAL_X		1030
#endif
#ifndef I2C_HID_TOUCHPAD_MAX_PHYSICAL_Y
#define I2C_HID_TOUCHPAD_MAX_PHYSICAL_Y		680
#endif
 ...
 * device's spec and set these values manually. In case these settings are
 * missing, we simply use a trivial estimation X+Y.
 */
#ifndef I2C_HID_TOUCHPAD_MAX_WIDTH
#define I2C_HID_TOUCHPAD_MAX_WIDTH \
		(I2C_HID_TOUCHPAD_MAX_X + I2C_HID_TOUCHPAD_MAX_Y)
#endif
#ifndef I2C_HID_TOUCHPAD_MAX_HEIGHT
#define I2C_HID_TOUCHPAD_MAX_HEIGHT \
		(I2C_HID_TOUCHPAD_MAX_X + I2C_HID_TOUCHPAD_MAX_Y)
#endif
 ...
 * of each contact. This can be computed simply as bool(X>Y).
 */
#ifndef I2C_HID_TOUCHPAD_MAX_ORIENTATION
#define I2C_HID_TOUCHPAD_MAX_ORIENTATION	1
#endif
#ifndef I2C_HID_TOUCHPAD_MOUSE_SCALE_X
#define I2C_HID_TOUCHPAD_MOUSE_SCALE_X		10
#endif
#ifndef I2C_HID_TOUCHPAD_MOUSE_SCALE_Y
#define I2C_HID_TOUCHPAD_MOUSE_SCALE_Y		10
#endif
...
struct finger {
	...
	uint16_t orientation:N_BITS(I2C_HID_TOUCHPAD_MAX_ORIENTATION);
} __packed;
...
#define FINGER(FINGER_NUMBER)						       \
	...
	0x26, I2C_HID_TOUCHPAD_MAX_ORIENTATION&0xff,			       \
	      I2C_HID_TOUCHPAD_MAX_ORIENTATION>>8,			       \
					/*     Logical Maximum */	       \
	0x75, N_BITS(I2C_HID_TOUCHPAD_MAX_ORIENTATION),			       \
					/*     Report Size */		       \
	0x81, 0x02,			/*     Input (Data,Var,Abs) */         \
	0xC0,				/*   End Collection */
...
static struct hid_descriptor hid_desc = {
	...
	.wMaxInputLength = I2C_HID_HEADER_SIZE +
			   MAX(sizeof(struct touch_report),
 ...
 * the protocol initialization. The device would typically send a dummy report
 * since itself may not be ready yet.
 *
 * This variable records if this dummy input report has been sent, i.e., if the
 * protocol reset has been done.
 */
...
/* TODO(sheckylin): Selectively report surface contact and button state in
...
int i2c_hid_touchpad_process(int len, uint8_t *buffer,
			     void (*send_response)(int len), uint8_t *data)
{
	...
	case COMMAND_REGISTER:
		return i2c_hid_touchpad_command_process(len, buffer,
							send_response, data);
	default:
		/* Return a negative number to indicate that an unsupported
...
static int i2c_hid_touchpad_command_process(int len, uint8_t *buffer,
					    void (*send_response)(int len),
					    uint8_t *data)
{
	uint8_t command = buffer[3] & 0x0F;
	uint8_t power_state = buffer[2] & 0x3;
	uint8_t report_id = buffer[2] & 0x0F;
	size_t response_len;

	switch (command) {
	case I2C_HID_CMD_RESET:
		ccprints("I2C-HID: command reset\n");
		cflush();
		i2c_hid_init();
		break;
	case I2C_HID_CMD_GET_REPORT:
		ccprints("I2C-HID: command get_report (%04x)\n", report_id);
		switch (report_id) {
	...
	case I2C_HID_CMD_SET_REPORT:
		ccprints("I2C-HID: command set_report (%04x)\n", report_id);
		switch (report_id) {
		case REPORT_ID_INPUT_MODE:
	...
	case I2C_HID_CMD_SET_POWER:
		ccprints("I2C-HID: command set_power: %s\n",
...
static void i2c_hid_compile_report(struct touchpad_event *event)
{
	...
	for (int i = 0; i < I2C_HID_TOUCHPAD_MAX_FINGERS; i++) {
		...
		if (event->finger[i].valid) {
			 ...
			 *
			 * TODO(sheckylin): Revisit this once we have met such
			 * a device.
			 */
