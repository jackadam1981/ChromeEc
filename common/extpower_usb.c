...
static void usb_force_detach(void)
{
	if (!(current_dev_type & TSU6721_TYPE_JIG_UART_ON))
		return;
	set_video_power(0);
...
DECLARE_HOOK(HOOK_CHIPSET_SUSPEND, usb_force_detach, HOOK_PRIO_DEFAULT);
DECLARE_HOOK(HOOK_CHIPSET_SHUTDOWN, usb_force_detach, HOOK_PRIO_DEFAULT);
