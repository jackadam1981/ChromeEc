...
static int command_cable(int argc, char **argv)
{
	...
	ptype_type = get_usb_pd_cable_type(port);

	ccprintf("Cable Type: ");
	if (ptype_type != IDH_PTYPE_PCABLE &&
		ptype_type != IDH_PTYPE_ACABLE) {
		ccprintf("Not Emark Cable\n");
		return EC_SUCCESS;
	}
	ccprintf("%s\n", cable_type[ptype_type]);

	cable_rev = get_usb_pd_cable_revision(port);
	 ...
	 */
	if (ptype_type == IDH_PTYPE_ACABLE)
