...
static void dump_pe(int port)
{
	...
	const char * const idh_ptype_names[]  = {
		...
		"RSV6", "RSV7"};
	const char * const tx_names[] = {"SOP", "SOP'", "SOP''"};
	...
	if (pd_get_svid_count(port) < 1) {
		ccprintf("No SVIDS discovered yet.\n");
		return;
	}

	for (i = 0; i < pd_get_svid_count(port); i++) {
		ccprintf("SVID[%d]: %04x MODES:", i, svid_disc->svids[i].svid);
		for (j = 0; j < svid_disc->svids[j].mode_cnt; j++)
			ccprintf(" [%d] %08x", j + 1,
					svid_disc->svids[i].mode_vdo[j]);
		ccprintf("\n");

		modep = pd_get_amode_data(port, svid_disc->svids[i].svid);
		if (modep) {
