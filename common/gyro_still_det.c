...
fp_t gyro_still_det_compute(struct gyro_still_det* gyro_still_det)
{
	...
	gyro_still_det->win_mean_x = fp_mul(
		gyro_still_det->win_mean_x, tmp_denom_mean);
	gyro_still_det->win_var_x = fp_mul(
		(gyro_still_det->acc_var_x -
			fp_mul(gyro_still_det->win_mean_x, tmp)),
		tmp_denom);

	tmp = gyro_still_det->win_mean_y;
	gyro_still_det->win_mean_y = fp_mul(
		gyro_still_det->win_mean_y, tmp_denom_mean);
	gyro_still_det->win_var_y = fp_mul(
		(gyro_still_det->acc_var_y -
			fp_mul(gyro_still_det->win_mean_y, tmp)),
		tmp_denom);

	tmp = gyro_still_det->win_mean_z;
	gyro_still_det->win_mean_z *= fp_mul(
		gyro_still_det->win_mean_z, tmp_denom_mean);
	gyro_still_det->win_var_z = fp_mul(
		(gyro_still_det->acc_var_z -
			  ...
			  bool reset_stats)
{
	...
	if (gyro_still_det->num_acc_samples > INT_TO_FP(1))
		tmp_denom = fp_div(
			INT_TO_FP(1), gyro_still_det->num_acc_samples);

	 gyro_still_det->prev_mean_x =
	 	fp_mul(gyro_still_det->mean_x, tmp_denom);
	 gyro_still_det->prev_mean_y =
	 	fp_mul(gyro_still_det->mean_y, tmp_denom);
	 gyro_still_det->prev_mean_z =
	 	fp_mul(gyro_still_det->mean_z, tmp_denom);
