...
{
	return DCRYPTO_x509_gen_cert(d, pk_x, pk_y, NULL, cert, n);
}
			   ...
			   const p256_int* pk_y,  uint8_t *cert, const int n)
{
	...
	return DCRYPTO_x509_gen_cert(d, pk_x, pk_y, &serial, cert, n);
}
...
static uint16_t u2f_authenticate(struct apdu apdu, uint8_t* obuf, uint16_t* obuf_len)
{
	...
	resp->flags = pop_check_presence(apdu.p1 & G2F_CONSUME) == POP_TOUCH_YES;
