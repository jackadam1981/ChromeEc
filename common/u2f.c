...
static int verify_kh_pubkey(const uint8_t *key_handle,
	...
	p256_to_bin(&opk_x, kh_pubkey.x); /* endianness */
	p256_to_bin(&opk_y, kh_pubkey.y); /* endianness */
	kh_pubkey.pointFormat = U2F_POINT_UNCOMPRESSED;

	*matches = safe_memcmp(&kh_pubkey, public_key, sizeof(U2F_EC_POINT)) == 0;
					     ...
					     const uint8_t *data)
{
	...
	if (verify_kh_owned(user_secret, msg->app_id, msg->key_handle, &verified) !=
	    EC_SUCCESS)
		return VENDOR_RC_INTERNAL_ERROR;
