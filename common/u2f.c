...
bool u2f_try_load_state(struct u2f_state *state)
{
	...
	if (read_tpm_nvmem_hidden(TPM_HIDDEN_U2F_KEK, sizeof(state->hmac_key),
				  state->hmac_key) != TPM_READ_SUCCESS)
		return false;

	state->drbg_entropy_size = read_tpm_nvmem_size(TPM_HIDDEN_U2F_KH_SALT);

	if (((state->drbg_entropy_size != sizeof(state->drbg_entropy)) &&
...
bool u2f_load_or_create_state(struct u2f_state *state)
{
	if (!u2f_try_load_state(state)) {
		/*
		 * We have never used u2f before or it is corrupted - generate
		 * new seeds.
		 */
		if (!u2f_generate_state_keys(state))
			return false;
...
struct u2f_state *u2f_get_state(void)
{
	...
	if (!state_loaded)
		state_loaded = u2f_load_or_create_state(&state);
				    ...
				    size_t input_size, size_t *response_size)
{
	...
	bool u2f_generate_result = false;
	...
	*response_size = 0;
	 ...
	 * being overwritten.
	 */
	u2f_generate_result = u2f_generate(state, req->userSecret, req->appId,
					   authTimeSecretHash, kh_buf,
					   kh_version, pubKey);

	always_memset(authTimeSecretHash, 0, sizeof(authTimeSecretHash));

	if (!u2f_generate_result)
		return VENDOR_RC_INTERNAL_ERROR;
				...
				size_t input_size, size_t *response_size)
{
	...
	*response_size = 0;
	 ...
	 */
	if (!u2f_sign(state, kh, kh_version, user, origin,
					 ...
					 size_t *response_size)
{
	...
	*response_size = 0;
	...
	if (!u2f_attest(state, (union u2f_key_handle_variant *)&msg->key_handle,
			0, req->userSecret, msg->app_id, NULL, &msg->public_key,
