...
static uint16_t u2f_register(struct apdu apdu, uint8_t *obuf,
			     uint16_t *obuf_len, uint16_t max_len)
{
		      ...
		      uint16_t max_len)
{
	...
	if (cla == 0x00) { /* Always 0x00 */
		...
		case (U2F_REGISTER):
			CPRINTF("REGISTER");
			sw = u2f_register(apdu, obuf, &obuf_len, max_len);
			break;

		case (U2F_AUTHENTICATE):
			CPRINTF("AUTHENTICATE");
			sw = u2f_authenticate(apdu, obuf, &obuf_len, max_len);
			break;

		case (U2F_VERSION):
			CPRINTF("VERSION");
			sw = u2f_version(apdu, obuf, &obuf_len, max_len);
			break;
		}
