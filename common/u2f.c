				       ...
				       size_t input_size, size_t *response_size)
{
	...
	uint8_t kh_buf[sizeof(struct u2f_versioned_key_handle)] __aligned(4);
	size_t kh_size = (kh_version == 0) ?
	...
	*response_size = 0;
	 ...
	 */
	if (kh_version == 0) {
		copy_kh_pubkey_out(&opk_x, &opk_y,
				   (struct u2f_key_handle *)kh_buf, buf);
		*response_size = sizeof(struct u2f_generate_resp);
	} else {
		copy_versioned_kh_pubkey_out(
			&opk_x, &opk_y,
			(struct u2f_versioned_key_handle *)kh_buf, buf);
