...
static uint8_t ir3570_settings[][2] = {
...
static uint8_t ir3571_settings[][2] = {
	...
	{0x74, 0x00}, {0x75, 0xbf}, {0x76, 0x06}, {0x77, 0xfb},
	{0x78, 0xfb}, {0x79, 0x04}, {0x7a, 0x00}, {0x7b, 0x0d},
...
void ir357x_prog(void)
{
	...
	if (version == 3570) {
		for (i = 0; i < ARRAY_SIZE(ir3570_settings); i++)
			ir357x_write(ir3570_settings[i][0],
				     ir3570_settings[i][1]);
	} else if (version == 3571) {
		for (i = 0; i < ARRAY_SIZE(ir3571_settings); i++)
			ir357x_write(ir3571_settings[i][0],
				     ir3571_settings[i][1]);
	} else {
...
int ir357x_check(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(ir3570_settings); i++)
		val = ir357x_read(ir3570_settings[i][0]);
		if (val != ir3570_settings[i][1]) {
			ccprintf("DIFF reg 0x%02x %02x->%02x\n",
				 ir3570_settings[i][0],
				 ir3570_settings[i][1], val);
			cflush();
			diff++;
		}
	return !!diff;
