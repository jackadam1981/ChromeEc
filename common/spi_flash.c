 ...
 *
 * SPI flash driver for Chrome EC, particularily Winbond W25Q64FV.
 */
 ...
 */
#define SPI_FLASH_SLEEP_USEC	1000
...
static int spi_flash_write_enable(void)
{
	...
	return spi_transaction(buf, 1, NULL, 0);
}
...
uint8_t spi_flash_get_status1(void)
{
	...
	if (spi_transaction(buf, 1, &resp, 1) != EC_SUCCESS)
		return -1;
...
int spi_flash_set_status(int reg1, int reg2)
{
	...
	if (reg2 == -1)
		rv = spi_transaction(buf, 2, NULL, 0);
	else
		rv = spi_transaction(buf, 3, NULL, 0);
	if (rv)
		return rv;
...
int spi_flash_read(uint8_t *buf_usr, unsigned int offset, unsigned int bytes)
{
	...
	return spi_transaction(buf, 4, buf_usr, bytes);
}
...
static int spi_flash_erase_block(unsigned int offset, unsigned int block)
{
	...
	rv = spi_transaction(buf, 4, NULL, 0);
	if (rv)
		return rv;
	...
	const uint8_t const *data)
{
	...
	while (bytes > 0) {
		...
		rv = spi_transaction(buf, 4 + write_len, NULL, 0);
		if (rv)
			return rv;
...
uint16_t spi_flash_get_id(void)
{
	...
	if (spi_transaction(buf, 4, resp, 2) != EC_SUCCESS)
		return -1;
...
uint32_t spi_flash_get_jedec_id(void)
{
	...
	if (spi_transaction(buf, 1, (uint8_t *)&resp, 4) != EC_SUCCESS)
		return -1;
...
uint64_t spi_flash_get_unique_id(void)
{
	...
	if (spi_transaction(buf, 5, (uint8_t *)&resp, 8) != EC_SUCCESS)
		return -1;
