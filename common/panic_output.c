...
#ifdef CONFIG_ZEPHYR
	/*
	 * ccprintf acquires a mutex lock internally. This mutex lock will not
	 * be released after the stack overflow panic. This interferes with
	 * system safe mode recovery testing. See b/278792557 and b/280673945.
	 */
	printk("+%d", n);
#else
	ccprintf("+%d", n);
