...
static int batt_cbi_read(enum cbi_data_tag tag, uint8_t *data, uint8_t size)
{
	int rv;
...
test_export_static int batt_cbi_read_fet_info(
		struct board_batt_params *info)
{
...
test_export_static int batt_cbi_read_fuel_gauge_info(
		struct board_batt_params *info)
{
	struct fuel_gauge_info *fg = &info->fuel_gauge;
	uint8_t d8;

	batt_cbi_read(CBI_TAG_FUEL_GAUGE_MANUF_NAME,
...
test_export_static int batt_cbi_read_battery_info(
		struct board_batt_params *info)
{
