...
static enum tbt_compat_cable_speed convert_to_tbt_speed(enum usb_rev30_ss ss)
{
	switch (ss) {
...
enum tbt_compat_cable_speed get_tbt_cable_speed(int port)
{
	 ...
	 * from USB Highest Speed field from the Passive Cable VDO.
	 */
	if (!cable_mode_resp.raw_value) {
		...
		disc = pd_get_am_discovery(port, TCPC_TX_SOP_PRIME);
		cable_tbt_speed =
		     convert_to_tbt_speed(disc->identity.product_t1.p_rev30.ss);
	} else {
