		...
		uint16_t svid)
{
	...
	for (i = 0; i < supported_modes_cnt; i++) {
		for (j = 0; j < disc->svid_cnt; j++) {
			...
			modep->data = (struct svid_mode_data *)&disc->svids[j];
			active->amode_idx++;
		...
		uint32_t *payload)
{
	...
	struct pd_discovery *disc = pd_get_am_discovery_and_notify_access(port, type);
...
{
	struct pd_discovery *pd = pd_get_am_discovery_and_notify_access(port, type);
		...
		uint16_t svid, uint32_t *vdo_out)
{
	...
	for (idx = 0; idx < disc->svid_cnt; ++idx) {
		if (pd_get_svid(port, idx, type) == svid) {
			...
			return (int)disc->svids[idx].mode_cnt;
		}
	}
	return 0;
}
		...
		enum tcpci_msg_type type)
{
	...
	for (svid_idx = 0; svid_idx < disc->svid_cnt; ++svid_idx) {
		struct svid_mode_data *mode_data = (struct svid_mode_data *)&disc->svids[svid_idx];
		...
		enum tcpci_msg_type type)
{
	...
	return (uint32_t *)disc->svids[svid_idx].mode_vdo;
}
