...
uint32_t pd_get_mode_vdo_for_svid(int port, enum tcpm_transmit_type type,
		uint16_t svid)
{
	int idx;
	struct pd_discovery *disc;
...
enum tbt_compat_cable_speed get_tbt_cable_speed(int port)
{
	union tbt_mode_resp_cable cable_mode_resp = {
		.raw_value =  pd_get_mode_vdo_for_svid(port, TCPC_TX_SOP_PRIME,
						USB_VID_INTEL) };
			...
			uint32_t *payload)
{
	...
	dev_mode_resp.raw_value = pd_get_mode_vdo_for_svid(port, TCPC_TX_SOP,
						USB_VID_INTEL);
	cable_mode_resp.raw_value =
			pd_get_mode_vdo_for_svid(port, TCPC_TX_SOP_PRIME,
						USB_VID_INTEL);
