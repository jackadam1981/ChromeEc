...
void dfp_consume_identity(int port, int cnt, uint32_t *payload)
{
	...
	memcpy(pd_get_identity_response(port, TCPC_TX_SOP)->raw_value,
...
{
	struct pd_discovery *pd = pd_get_am_policy(port);
						  ...
						  enum tcpm_transmit_type type)
{
	...
	return pd->identity[type].discovery;
}

union disc_ident_ack *pd_get_identity_response(int port,
					       enum tcpm_transmit_type type)
{
	if (type > DISCOVERY_TYPE_MAX)
		return NULL;

	return &pd_get_am_policy(port)->identity[type].response;
}
