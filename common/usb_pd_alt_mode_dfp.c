		...
		enum tcpm_transmit_type type)
{
	...
	if (type == TCPC_TX_SOP_PRIME && !IS_ENABLED(CONFIG_USB_PD_DECODE_SOP))
		return;
...
enum tbt_compat_cable_speed get_tbt_cable_speed(int port)
{
	union tbt_mode_resp_cable cable_mode_resp = {
		.raw_value = get_tbt_mode_response(port, TCPC_TX_SOP_PRIME) };
			...
			uint32_t *payload)
{
	...
	dev_mode_resp.raw_value = get_tbt_mode_response(port, TCPC_TX_SOP);
	cable_mode_resp.raw_value =
			get_tbt_mode_response(port, TCPC_TX_SOP_PRIME);
