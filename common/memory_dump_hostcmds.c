					...
					void *user_data)
{
	...
	if (memory_dump_entry_count >= MAX_DUMP_ENTRIES) {
		printk("ERROR: Memory dump count exceeds max");
		return;
	}

	entries[memory_dump_entry_count].address = thread->stack_info.start;
...
static enum ec_status read_memory_dump(struct host_cmd_handler_args *args)
{
	...
	memcpy(r->mem, (void *)p->address, r->size);
