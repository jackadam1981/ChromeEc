...
/* tx_rx must be one of ec_comm_*_tx_rx structure)
 * txlen is the request data length in bytes
 * rxlen is the response data length in bytes
 */
			 ...
			 int timeout)
{
	...
	/* Request (TX) size is header + (data + crc8), response follows. */
	int tx_total_length =
		sizeof(*request_header) + ((txlen > 0) ? (txlen + 1) : 0);
	struct ec_host_request4 *response_header =
...
static void base_get_dynamic_info(void)
{
	...
	ret = write_command(EC_CMD_BATTERY_GET_DYNAMIC,
			(void *)&data, 0, sizeof(data.rx.info), 15000);
	if (ret != EC_SUCCESS) {
		CPRINTF("%s: error (%d)\n", __func__, ret);
		return;
	}
...
static void base_charge_control(int max_current, int allow_charging)
{
	...
	/* FIXME: Change OTG voltage. */
	data.tx.ctrl.otg_voltage = 5000;

	ret = write_command(EC_CMD_CHARGER_CONTROL,
		(void *)&data, sizeof(data.tx.ctrl), sizeof(data.rx.ctrl),
			30000);
	if (ret != EC_SUCCESS) {
		CPRINTF("%s: error (%d)\n", __func__, ret);
		return;
...
void charge_allocate_input_current_limit(void)
{
	...
	if (total_power > 0) { /* Charging */
		/* Algo 1a.+1b. */
		CHG_ALLOCATE(power_base, total_power, 1000 * 1000);
		CHG_ALLOCATE(power_lid, total_power,
		...
		base_charge_control(current_base, 1);
...
static int update_static_battery_info(void)
{
	...
	/* Smart battery serial number is 16 bits */
	memset(base_battery_static.serial, 0,
	...
	/* Battery Manufacturer string */
	memset(base_battery_static.manufacturer, 0,
		sizeof(base_battery_static.manufacturer));
	rv |= battery_manufacturer_name(base_battery_static.manufacturer,
				sizeof(base_battery_static.manufacturer));

	/* Battery Model string */
	memset(base_battery_static.model, 0, sizeof(base_battery_static.model));
	rv |= battery_device_name(base_battery_static.model,
				sizeof(base_battery_static.model));

	/* Battery Type string */
	memset(base_battery_static.type, 0, sizeof(base_battery_static.type));
	rv |= battery_device_chemistry(base_battery_static.type,
				sizeof(base_battery_static.type));

	/* Zero the dynamic entries. They'll come next. */
	base_battery_dynamic.voltage = 0;
	base_battery_dynamic.current = 0;
	base_battery_dynamic.remaining_capacity = 0;
	base_battery_dynamic.full_capacity = 0;
	base_battery_dynamic.status = 0;
	base_battery_dynamic.flags = 0;
	base_battery_dynamic.desired_voltage = 0;
	base_battery_dynamic.desired_current = 0;
