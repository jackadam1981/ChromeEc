...
#define CHARGE_PPS_REFRESH_VOLTAGE_INTERVAL \
	((9 * 1000) * MSEC) /* 9 seconds    \
			     */
...
static bool charge_ac_supports_pps(int chgnum)
{
	...
	if (pd_find_apdo_index(src_cap_cnt, src_caps, batt_info->voltage_max,
			       batt_info->voltage_min, CHARGE_PPS_MAX_CURRENT,
			       NULL) < 0) {
		return false;
	}
	return true;
...
static void charge_handle_adaptive_legacy(int chgnum)
{
	...
	charge_adative_legacy_find_voltage(chgnum, &pdata->req_mv,
					   &pdata->req_ma);
	pdata->req_ma = 2000;
	pdata->chg_ma = 2000;
...
static void charge_handle_adaptive_pps(int chgnum)
{
	...
	if (!enabled) {
		...
		pdata->req_mv =
			pdata->req_mv - CHARGE_PPS_VOLTAGE_DECREMENT <
					curr.batt.voltage -
						CHARGE_MIN_PPS_VOLTAGE_DECREMENT ?
				pdata->req_mv :
				pdata->req_mv - CHARGE_PPS_VOLTAGE_DECREMENT;
	...
	pdata->req_mv =
		pdata->req_mv + CHARGE_PPS_VOLTAGE_INCREMENT >
				curr.batt.voltage +
					CHARGE_MAX_PPS_VOLTAGE_INCREMENT ?
			pdata->req_mv :
			pdata->req_mv + CHARGE_PPS_VOLTAGE_INCREMENT;
...
static enum charge_adaptive_mode charge_handle_adaptive(void)
{
	...
	if (charge_get_active_chg_chip() != chgnum) {
		...
		if (chgnum < board_get_charger_chip_count() && chgnum >= 0) {
			/*
			 * Current charger ID is still valid, force
			 * disable mode.
			 */
			set_adaptive_mode(chgnum, CHARGE_ADAPTIVE_DISABLE);
		} else {
			chgnum = charge_get_active_chg_chip();
		}
	}
