...
/*
 * TODO(b:65697620): Make this depend on a config option instead, so that we can
 * decide to use these structures, and pass battery information via a host
 * command instead, even on systems that would typically use memmap.
 */
static int update_static_battery_info(void)
{
	int batt_serial;
