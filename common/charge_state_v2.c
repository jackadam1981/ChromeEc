...
static void prevent_hot_discharge(void)
{
	...
	/* TODO(HEY): If the AP isn't on there's not much we can do. Right? */
	if (!chipset_in_state(CHIPSET_STATE_ON))
		return;

	/* TODO(HEY): Same if we can't read the battery temp. Right? */
	 ...
	 *
	 * TODO(HEY): The thermal loop should watch the battery temp anyway, so
	 * it can turn fans on. It could also force an AP shutdown if it's too
	 * hot, but AFAIK we don't have anything in place to do a battery
	 * shutdown if it's really hot. We probably should, just in case.
	 * TODO(HEY): It'd also be nice if we had a persistent error log for
	 * that somewhere too. It would have to be in the EC's eeprom and not
	 * in the nvram if we're going to cut the battery.
	 */
...
void charger_task(void)
{
	...
	while (1) {
		...
		/*
		 * HEY: but make sure we protect the battery even if we can't
		 * talk to the charger.
		 */
		if (!curr.ac) {
			curr.state = ST_DISCHARGE;
		...
		/*
		 * HEY: But see chrome-os-partner:26418. Can we do away with
		 * state_machine_force_idle if DPTF current=0 does the same
		 * thing?
		 */
		 ...
		 * (CONFIG_CHARGER_TIMEOUT_HOURS, chrome-os-partner:20145)
		 */
...
wait_for_it:
		...
		if (sleep_usec <= 0) {
			...
			if (curr.state == ST_IDLE ||
				...
				/* Charging, so pay close attention */
