				 ...
				 int current_lid, int allow_charge_lid)
{
	...
	/* FIXME: This is still quite noisy during charging. */
...
static void charge_allocate_input_current_limit(void)
{
	...
	lid_system_power = MAX(charger_get_system_power(), 0);

	/* Smooth system power, as it is very spiky. */
	if (prev_lid_system_power > -1)
		lid_system_power =
			(lid_system_power + 7*prev_lid_system_power)/8;

	base_battery_power = (base_battery_dynamic.flags &
			(BATT_FLAG_BAD_VOLTAGE | BATT_FLAG_BAD_CURRENT)) ? 0 :
		(base_battery_dynamic.actual_current *
	...
	if (total_power > 0) { /* Charging */
		...
		if (charge_base < 0) {
			...
			set_base_lid_current(
				800, charge_base <= BATTERY_LEVEL_SHUTDOWN,
				-1000, 0);
		}
	}
