 ...
 * stablize the battery charging current.
 * stable_current should be evaluated when stable_ts expired.
 * stable_ts shuold be reset if the charger input voltage/current changed,
...
wait_for_it:
		...
		if (IS_ENABLED(CONFIG_USB_PD_PREFER_MV)) {
			...
			prev_plt_and_desired_mw =
				charge_get_plt_and_desired_mw();

			/*
			 * Update desired power by the following rules:
			 * 1. If the battery SoC under battery's constant
			 ...
			 * 2. If the battery is not charging, we reset the
			 * desired_mw to the battery spec. The actual
			...
			if (curr.state != ST_CHARGE ||
			    curr.batt.state_of_charge < pd_pref_config.cv)
				desired_mw = curr.batt.desired_current *
					     curr.batt.desired_voltage / 1000;
			else if (curr.batt.state_of_charge >=
					 pd_pref_config.cv &&
...
#ifndef TEST_BUILD
int charge_get_plt_and_desired_mw(void)
{
	/*
...
{
	/* it takes 8 to 10 seconds to stablize battery current in practice */
