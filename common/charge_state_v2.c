...
/* Spew problems to the console until we figure out what to do about them. */
#define PROBLEM \
	ccprintf("[%T unhandled failure in %s, line %d\n", __FILE__, __LINE__)
...
static void update_static_battery_info(void)
{
	char *batt_str;
	int batt_serial;
...
{
	/* TODO(HEY): the memmap address is constant. Make it so somehow. */
	...
	if (curr.batt_is_present == BP_YES)
		tmp |= EC_BATT_FLAG_BATT_PRESENT;

	/*
	 * TODO(HEY): How do we know these are good? What if they're not?
	 * curr.batt.flags has BATT_FLAG_BAD_VOLTAGE for voltage, but
	 * doesn't have a bit for the current except for BAD_ANY. And
	 * remaining_capacity is still handled separately. For now, we'll just
	 * skip all of them on any error.
	 */
	if (!(curr.batt.flags & BATT_FLAG_BAD_ANY) &&
	    curr.batt_remaining_capacity >= 0) {

		*memmap_volt = curr.batt.voltage;
		*memmap_rate = ABS(curr.batt.current);
		*memmap_cap = curr.batt_remaining_capacity;
	...
	*memmap_flags = tmp;
	...
	/* Keep an eye on the capacity. Poke the AP if it changes. */
	if (!battery_full_charge_capacity(&lfcc) && lfcc != *memmap_lfcc) {
		*memmap_lfcc = lfcc;
...
#define DUMP(FLD, FMT) ccprintf("  " #FLD " = " #FMT "\n", curr.FLD)
	ccprintf("  state = %s\n", state_list[curr.state]); // DWR: bounds
	DUMP(ac,                      %d);
	DUMP(chg.charging_voltage,    %dmV);
	DUMP(chg.charging_current,    %dmA);
	...
	DUMP(batt_remaining_capacity, %dmAh);
	ccprintf("  batt_is_present = %s\n",
		 batt_pres[curr.batt_is_present]); // DWR: bounds
	DUMP(requested_voltage,       %dmV);
	DUMP(requested_current,       %dmA);
	ccprintf("  force_idle = %d\n", state_machine_force_idle);
	ccprintf("  user_current_limit = %umA\n", user_current_limit);
 ...
 */
static void charge_request(int voltage, int current)
{
	if (!voltage || !current)
		/* TODO(HEY): should we call charger_set_mode() too? */
		voltage = current = 0;

	CPRINTF("[%T %s(%dmV, %dmA)]\n", __func__, voltage, current);

	if (charger_set_voltage(voltage))
		PROBLEM;

	if (charger_set_current(current))
		PROBLEM;
}


/* TODO(HEY): This should be part of charger.h  */
static void charger_get_params(struct charger_params *chg)
{
	if (!chg->info)
		chg->info = charger_get_info();

	if (charger_get_voltage(&chg->charging_voltage)) {
		PROBLEM;
		chg->charging_voltage = -1;
	}

	if (charger_get_current(&chg->charging_current)) {
		PROBLEM;
		chg->charging_current = -1;
	}
...
static void prevent_hot_discharge(void)
{
	int batt_temp_c;
	/*
	 * If we couldn't read the battery temp, it will report as 0.0K, so
	 * we have to assume it's okay. HEY: Right?
	 */
	if (!curr.batt.temperature)
		return;

	/* TODO(HEY): If the AP isn't on and it's still too hot, now what? */
	if (!chipset_in_state(CHIPSET_STATE_ON))
		return;

	/* TODO(HEY): Shouldn't we do this in stages, like
	 * prevent_deep_discharge()?  Send an event, give the AP time to react,
	 * maybe even hibernate the EC if things are really hot?
	 */
...
{
	return ((!(curr.batt.flags & BATT_FLAG_BAD_CHARGE_PERCENT) &&
 ...
 * host events.
 *
 * TODO(HEY): Do we always send these? On AC too? Even with the AP off?
 */
static void notify_host_of_low_battery(void)
{
	/* We can't tell what the current charge is. Assume it's okay. */
	if (curr.batt.flags && BATT_FLAG_BAD_CHARGE_PERCENT)
		return;
...
void charger_task(void)
{
	...
	curr.batt_is_present = BP_NOT_SURE;
	prev_ac = prev_volt = prev_curr = -1;
	prev_charge = 0;
	...
	/*
	 * Initialze the static memmap values. The dynamic parts will be
	 * updated at the bottom of the loop, so there aren't any glitches.
	 */
	...
	while(1) {
		...
		if (curr.ac != prev_ac) {
			if (curr.ac) {
				 ...
				 * comes back.
				 */
				if (charger_post_init() != EC_SUCCESS)
					PROBLEM;
			} else {
		...
		/* Now go see what it's doing */
		charger_get_params(&curr.chg);

		/*
		 * TODO(HEY): This should be in struct batt_params, since it's
		 * generally expected that a missing battery won't respond.
		 */
#if defined(CONFIG_BATTERY_PRESENT_CUSTOM) ||	\
	defined(CONFIG_BATTERY_PRESENT_GPIO)
		curr.batt_is_present = battery_is_present();
#else
		/* don't know yet */
		curr.batt_is_present = BP_NOT_SURE;
#endif
		...
		if (curr.batt.flags & BATT_FLAG_BAD_ANY) {
			PROBLEM;
		} else if (curr.batt_is_present == BP_NOT_SURE) {
			/* If we can talk to it, it's present. */
			curr.batt_is_present = BP_YES;
		}

		/* TODO(HEY): This should probably in struct batt_params too. */
		if (battery_remaining_capacity(&curr.batt_remaining_capacity)) {
			PROBLEM;
			curr.batt_remaining_capacity = -1;
		}
		...
		/*
		 * HEY: If we know we've had trouble talking to the battery or
		 * the charger, we should probably do nothing for a bit, and if
		 * it doesn't get better then flag it as an error. Make sure we
		 * still protect the battery if only the charger is bad.
		 */
		...
		/* HEY: What if the battery temp is too high? */
		...
		/*
		 * HEY: quit trying if charging too long without getting full
	...
	wait_for_it:
		...
		if (curr.batt.state_of_charge != prev_charge)
			show_charging_progress();
		prev_charge = curr.batt.state_of_charge;

		/* Now sleep a bit */
		/* HEY: If there are errors, don't wait very long. */
		if (curr.state == ST_IDLE || curr.state == ST_DISCHARGE) {
			/* If the AP is off, we can sleep a long time. */
			if (chipset_in_state(CHIPSET_STATE_ANY_OFF |
					     CHIPSET_STATE_SUSPEND))
				sleep_usec = POLL_PERIOD_VERY_LONG;
			else
				/* AP is on, so don't sleep too long */
				sleep_usec = POLL_PERIOD_LONG;
		} else {
			/* Charging, so pay attention */
...
int charge_want_shutdown(void)
{
	return (curr.state == ST_DISCHARGE) &&
		!(curr.batt.flags & BATT_FLAG_BAD_CHARGE_PERCENT) &&
		(curr.batt.state_of_charge < BATTERY_LEVEL_SHUTDOWN);
}
...
#ifdef CONFIG_CHARGER_PROFILE_OVERRIDE
	/* HEY: call charger_profile_override_enable() somehow? */
#endif
