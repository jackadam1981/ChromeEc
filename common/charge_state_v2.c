...
static enum ec_charge_control_mode state_machine_force_idle;
...
static int charge_request(int voltage, int current)
{
	...
	if (current > 0 || state_machine_force_idle == CHARGE_CONTROL_DISCHARGE)
#else
...
static int charge_force_idle(enum ec_charge_control_mode mode)
{
	int enable = (mode != CHARGE_CONTROL_NORMAL);
	/*
...
void charger_task(void)
{
	...
	state_machine_force_idle = CHARGE_CONTROL_NORMAL;
	...
	while (1) {
		...
		if (state_machine_force_idle != CHARGE_CONTROL_NORMAL) {
			curr.state = ST_IDLE;
			goto wait_for_it;
		}
