				 ...
				 int current_lid, int allow_charge_lid)
{
	...
	if (current_lid >= 0) {
		...
		charger_set_otg_current_voltage(-current_lid, otg_voltage);
...
static void charge_allocate_input_current_limit(void)
{
	...
	if (total_power > 0) { /* Charging */
		...
		if (charge_base > 3) {
			 ...
			 * between 1 and 2 percent, which might not be great for
			 * battery life. We need some hysteresis.
			 */
			set_base_lid_current(800, charge_base <= 1, -1000, 0);
		}
	}
...
wait_for_it:
		...
		if (curr.ac) {
			 ...
			 * do want to use it for charging.
			 */
			curr.requested_current = 0;
#endif
		}
