...
struct dual_battery_policy {
	...
	/* Voltage to use when using OTG mode between lid and bas (mV) */
	 ...
	 * limit, to make sure that input charger does not overcurrents output
...
/**
 * Add marging m to value. Compute (1+m)*value, where m is in 1/128 unit.
 */
...
static void charge_allocate_input_current_limit(void)
{
	 ...
	 * gives numbers in uW, which are dangereously close to overflowing when
	 * doing intermediate computations (60W * 100 overflows a signed int,
	 * for example). We therefore need to divide and re-multiply the power
	 * numbers by 1000 when converting them back to current.
	 */
	int total_power = 0;
...
void charger_task(void *u)
{
	...
	while (1) {
		...
		if (prev_base_connected != base_connected) {
			/* TODO(b:71881017): Invalidate static information. */
