	...
	if (args->version >= 2) {
		if (p->cmd == EC_CHARGE_CONTROL_CMD_SET) {
			if (p->mode == CHARGE_CONTROL_NORMAL)
				ccprints("##########p->mode == CHARGE_CONTROL_NORMAL##########");
			else if (p->mode == CHARGE_CONTROL_IDLE)
				ccprints("##########p->mode == CHARGE_CONTROL_IDLE##########");
			else if (p->mode == CHARGE_CONTROL_DISCHARGE)
				ccprints("##########p->mode == CHARGE_CONTROL_DISCHARGE##########");

			if (get_chg_ctrl_mode() == CHARGE_CONTROL_NORMAL) {
				ccprints("##########get_chg_ctrl_mode() == CHARGE_CONTROL_NORMAL##########");
				ccprints("###sustain_soc.lower = %d; sustain_soc.upper = %d###",
									p->sustain_soc.lower, p->sustain_soc.upper);
