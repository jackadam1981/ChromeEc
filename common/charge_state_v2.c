...
static const char *const prob_text[] = {
	"static update", "set voltage",	      "set current",
	"set mode",	 "set input current", "post init",
	"chg params",	 "batt params",	      "custom profile",
};
...
static int set_base_current(int current_base, int allow_charge_base)
{
	...
	ret = ec_ec_master_base_charge_control(current_base, otg_voltage,
					       allow_charge_base);
	if (ret) {
				 ...
				 int current_lid, int allow_charge_lid)
{
	...
	if (current_lid >= 0) {
		...
		ret = charge_set_output_current_limit(-current_lid,
						      otg_voltage);
	}
...
static int set_chg_ctrl_mode(enum ec_charge_control_mode mode)
{
	...
	/* cherry-piceked: kukui branch doesn't have the charger API refactor
	 * changes, so modify the code to adapt to.
	 */
	if (IS_ENABLED(CONFIG_CHARGER_DISCHARGE_ON_AC_CUSTOM)) {
		rv = board_discharge_on_ac(discharge_on_ac);
		if (rv != EC_SUCCESS)
			return rv;
	} else if (IS_ENABLED(CONFIG_CHARGER_DISCHARGE_ON_AC)) {
