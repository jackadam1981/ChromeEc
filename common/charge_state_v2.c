...
static void shutdown_on_critical_battery(void)
{
	...
	if (!shutdown_warning_time.val) {
		CPRINTS("charge warn shutdown due to critical batt");
		...
		if (chipset_in_state(CHIPSET_STATE_ANY_OFF)) {
			...
			CPRINTS(
			  "charge force EC hibernate due to critical batt");
			system_hibernate(0, 0);
#elif defined(CONFIG_BATTERY_CRITICAL_SHUTDOWN_CUT_OFF)
			CPRINTS(
			  "charge force batt cut-off due to critical level");
			board_cut_off_battery();
#endif
		} else {
			/* Timeout waiting for AP to shut down, so kill it */
			CPRINTS(
			  "charge force shutdown due to critical batt");
	...
	while (1) {
		...
		if (!(curr.batt.flags & BATT_FLAG_RESPONSIVE)) {
			...
			if (curr.requested_voltage == 0 &&
				 ...
				 */
				CPRINTS("found batt in disconnect state");
				curr.requested_voltage =
					batt_info->voltage_max;
				curr.requested_current =
					batt_info->precharge_current;
