			 ...
			 int timeout)
{
	...
	ret = uart_alt_pad_read_write((void*)tx_rx, txlen,
...
{
	int ret;
	struct ec_comm_dynamic_info_tx_rx data;

	ret = write_command(EC_COMM_DYNAMIC_INFO,
...
*/
	memcpy(&base_dynamic, &data.rx.info, sizeof(base_dynamic));
}

static void base_get_static_info(void)
{
	int ret;
	struct ec_comm_static_info_tx_rx data;

	ret = write_command(EC_COMM_STATIC_INFO,
			(void*)&data, sizeof(data.tx), sizeof(data), 15000);
	if (ret != EC_SUCCESS) {
		CPRINTF("%s: error (%d)\n", __func__, ret);
		return;
...
/*
	...
	CPRINTF("Type:       %s\n", data.info.type);
*/
	memcpy(&base_static, &data.rx.info, sizeof(base_static));
}
...
void charge_allocate_input_current_limit(void) {
	...
	int base_battery_power = base_dynamic.current *
		base_dynamic.voltage;
	int base_battery_power_max = base_dynamic.desired_current *
		base_dynamic.desired_voltage;
	...
	if (base_dynamic.full_capacity > 0)
		percent_base = 100 * base_dynamic.remaining_capacity
			/ base_dynamic.full_capacity;
...
/*
 * FIXME: These 2 functions duplicate a lot of code with the ones in
 * the #else branch, try to improve that.
 */
...
#elif defined(CONFIG_EC_COMM_BATTERY_SLAVE)
static int update_static_battery_info(void)
{
	int batt_serial;
	int val;
	...
	/* Smart battery serial number is 16 bits */
	memset(base_static.serial, 0, sizeof(base_static.serial));
	rv = battery_serial_number(&batt_serial);
	if (!rv)
		snprintf(base_static.serial, sizeof(base_static.serial),
			 "%04X", batt_serial);

	/* Design Capacity of Full */
	ret = battery_design_capacity(&val);
	if (!ret)
		base_static.design_capacity = val;
	rv |= ret;

	/* Design Voltage */
	ret = battery_design_voltage(&val);
	if (!ret)
		base_static.design_voltage = val;
	rv |= ret;

	/* Cycle Count */
	ret = battery_cycle_count(&val);
	if (!ret)
		base_static.cycle_count = val;
	rv |= ret;

	/* Battery Manufacturer string */
	memset(base_static.manufacturer, 0, sizeof(base_static.manufacturer));
	rv |= battery_manufacturer_name(base_static.manufacturer,
					sizeof(base_static.manufacturer));

	/* Battery Model string */
	memset(base_static.model, 0, sizeof(base_static.model));
	rv |= battery_device_name(base_static.model, sizeof(base_static.model));

	/* Battery Type string */
	memset(base_static.type, 0, sizeof(base_static.type));
	rv |= battery_device_chemistry(base_static.type,
				sizeof(base_static.type));

	/* Zero the dynamic entries. They'll come next. */
	base_dynamic.voltage = 0;
	base_dynamic.current = 0;
	base_dynamic.remaining_capacity = 0;
	base_dynamic.full_capacity = 0;
	base_dynamic.status = 0;
	base_dynamic.flags = 0;
	base_dynamic.desired_voltage = 0;
	base_dynamic.desired_current = 0;

	if (rv)
		problem(PR_STATIC_UPDATE, rv);
	else
		; /* FIXME: Set a flag telling that the information is valid. */
...
static void update_dynamic_battery_info(void)
{
	...
	if (!(curr.batt.flags & BATT_FLAG_BAD_VOLTAGE))
		base_dynamic.voltage = curr.batt.voltage;

	if (!(curr.batt.flags & BATT_FLAG_BAD_CURRENT))
		base_dynamic.current = curr.batt.current;

	if (!(curr.batt.flags & BATT_FLAG_BAD_DESIRED_VOLTAGE))
		base_dynamic.desired_voltage = curr.batt.desired_voltage;

	if (!(curr.batt.flags & BATT_FLAG_BAD_DESIRED_CURRENT))
		base_dynamic.desired_current = curr.batt.desired_current;
	...
	if (!(curr.batt.flags & BATT_FLAG_BAD_REMAINING_CAPACITY)) {
		 ...
		 */
		if (curr.batt.remaining_capacity == 0 && !curr.batt_is_charging)
			base_dynamic.remaining_capacity = 1;
		else
			base_dynamic.remaining_capacity =
				curr.batt.remaining_capacity;
	}

	if (!(curr.batt.flags & BATT_FLAG_BAD_FULL_CAPACITY) &&
		(curr.batt.full_capacity <=
			(base_dynamic.full_capacity - LFCC_EVENT_THRESH) ||
		 curr.batt.full_capacity >=
			(base_dynamic.full_capacity + LFCC_EVENT_THRESH))) {
		base_dynamic.full_capacity = curr.batt.full_capacity;
	}
