...
static enum ec_status fp_command_frame(struct host_cmd_handler_args *args)
{
	...
	ret = validate_fp_buffer_offset(
		global_context.fp_enc_buffer.size_bytes(), offset, size);
	if (ret != EC_SUCCESS)
		return EC_RES_INVALID_PARAM;
	...
	if (!offset) {
		...
		std::span templ(global_context.fp_enc_buffer.data() + sizeof(*enc_info),
		...
		memset(global_context.fp_enc_buffer.data(), 0,
		...
		enc_info = reinterpret_cast<
			...
			global_context.fp_enc_buffer.data());
		...
		/* TODO(b/): We should be able to use fp_template[fgr]
		 * directly in std::ranges::copy instead of creating a span. It
		 * works in clang, but not the version of gcc used for the host
		 * tests. */
	...
	memcpy(out, global_context.fp_enc_buffer.data() + offset, size);
	args->response_size = size;
...
static enum ec_status fp_command_template(struct host_cmd_handler_args *args)
{
	...
	memcpy(&global_context.fp_enc_buffer[offset], params->data, size);
