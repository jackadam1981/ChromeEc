...
static enum ec_status fp_command_frame(struct host_cmd_handler_args *args)
{
	...
	if (fgr >= global_context.templ_valid)
		return EC_RES_UNAVAILABLE;
	ret = validate_fp_buffer_offset(sizeof(global_context.fp_enc_buffer),
					offset, size);
	if (ret != EC_SUCCESS)
		return EC_RES_INVALID_PARAM;
	...
	if (!offset) {
		...
		memset(&global_context.fp_enc_buffer, 0,
	...
	memcpy(out,
	       reinterpret_cast<uint8_t *>(&global_context.fp_enc_buffer) +
		       offset,
	       size);
	args->response_size = size;
...
enum ec_status fp_commit_template(std::span<const uint8_t> context)
{
	...
	if (global_context.fp_encryption_status & FP_CONTEXT_USER_ID_SET) {
		...
		ret = aes_128_gcm_decrypt(
			...
			encrypted_template_and_positive_match_salt,
			enc_info->nonce, enc_info->tag);
		OPENSSL_cleanse(key.data(), sizeof(key));
		if (ret != EC_SUCCESS) {
...
static enum ec_status fp_command_template(struct host_cmd_handler_args *args)
{
	...
	enum ec_error_list ret = validate_fp_buffer_offset(
		sizeof(global_context.fp_enc_buffer), offset, size);
	if (ret != EC_SUCCESS)
		return EC_RES_INVALID_PARAM;

	memcpy(reinterpret_cast<uint8_t *>(&global_context.fp_enc_buffer) +
		       offset,
	       params->data, size);
