...
enum ec_status fp_commit_template(const uint8_t *id, size_t id_size)
{
	...
	if (fp_encryption_status & FP_CONTEXT_USER_ID_SET) {
		ret = derive_encryption_key_with_info(
			key, enc_info->encryption_salt, id, id_size);
		if (ret != EC_SUCCESS) {
...
static enum ec_status
fp_command_migrate_legacy_template(struct host_cmd_handler_args *args)
{
	const auto *params =
		static_cast<const ec_params_fp_migrate_legacy_template *>(
			args->params);
	uint16_t idx = templ_valid;
	...
	/*
	 * Make sure salt data is cleared. Note that this operation can be think
	 * of as making template idx (the one we just committed) a freshly
	 * enrolled template.
	 */
...
DECLARE_HOST_COMMAND(EC_CMD_FP_MIGRATE_LEGACY_TEMPLATE,
		     fp_command_migrate_legacy_template, EC_VER_MASK(0));
