...
static uint32_t fp_process_match(void)
{
	...
	if (templ_valid) {
		res = fp_driver->finger_match(fp_template, templ_valid,
...
static void fp_process_finger(void)
{
	...
	CPRINTS("Capturing ...");
	res = fp_driver->sensor_acquire_image_with_mode_(
	...
	while (1) {
		...
		if (evt & TASK_EVENT_UPDATE_CONFIG) {
			...
			if (is_test_capture(mode)) {
				fp_driver->sensor_acquire_image_with_mode_(
				...
				fp_driver->sensor_configure_detect_();
			}
			...
			if (sensor_mode & FP_MODE_ANY_DETECT_FINGER) {
				st = fp_driver->sensor_finger_status_();
...
static enum ec_status fp_command_frame(struct host_cmd_handler_args *args)
{
	...
	if (idx == FP_FRAME_INDEX_RAW_IMAGE) {
		 ...
		 */
		ret = fp_sensor_validate_buffer_offset(
