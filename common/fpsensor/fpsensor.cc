...
static int command_fpupload(int argc, const char **argv)
{
	...
	pixels_str = argv[2];
	while (pixels_str) {
		char hex_str[3] = { pixels_str[0], pixels_str[1], '\0' };
		*dest = (uint8_t)strtol(hex_str, NULL, 16);
		pixels_str += 2;
	}

	return EC_SUCCESS;
}
DECLARE_CONSOLE_COMMAND(fpuplaod, command_fpupload, NULL,
