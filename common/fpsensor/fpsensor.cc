...
static uint32_t fp_process_enroll(void)
{
	...
	if (percent == 100) {
		...
		if (res) {
			...
			fp_init_unlocked_template_state_with_user_id(
				templ_valid);
			templ_valid++;
		}
...
static bool authenticate_fp_match_state(void)
{
	...
	for (uint16_t idx = 0; idx < templ_valid; idx++) {
		auto *dec_state = std::get_if<fp_decrypted_template_state>(
			&template_states[idx]);
		/* For safety reason, we should not proceed the matching
		 * operation if the any template is not in the intended state.
		 */
