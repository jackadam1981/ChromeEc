...
extern "C" void fp_task(void)
{
	...
	/* Initialize the frame size cache object. */
	global_context.fp_frame_size_cache = FpFrameSizeCache::Create();
	if (!global_context.fp_frame_size_cache) {
		CPRINTS("Fatal Error: Failed to create FpFrameSizeCache!");
		return;
	}
			...
			if (!is_finger_needed(mode)) {
				...
				global_context.current_frame_size =
					global_context.fp_frame_size_cache
						->get_frame_size(capture_type);
				fp_acquire_image(fp_buffer, capture_type);
