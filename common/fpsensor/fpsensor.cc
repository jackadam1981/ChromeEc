...
static enum ec_error_list prepare_template_for_encryption(uint16_t fgr)
{
...
static enum ec_error_list encrypt_template()
{
	...
	if (ret != EC_SUCCESS) {
		CPRINTS("Failed to encrypt template");
		global_context.fp_encryption_status &=
			~FP_TEMPLATE_ENCRYPTION_IN_PROGRESS;
		return EC_ERROR_UNAVAILABLE;
	}
...
static void encrypt_template_deferred(void)
{
	encrypt_template();
}
DECLARE_DEFERRED(encrypt_template_deferred);
...
static enum ec_status fp_command_frame_v0(struct host_cmd_handler_args *args)
{
	...
	if (!offset) {
		...
		if (prepare_template_for_encryption(fgr) != EC_SUCCESS) {
			return EC_RES_UNAVAILABLE;
		}

		if (encrypt_template() != EC_SUCCESS) {
			return EC_RES_UNAVAILABLE;
		}
	}
...
static enum ec_status fp_command_frame_v1(struct host_cmd_handler_args *args)
{
	...
	case FP_FRAME_ENCRYPT_TEMPLATE:
		...
		global_context.fp_encryption_status |=
			FP_TEMPLATE_ENCRYPTION_IN_PROGRESS;
		hook_call_deferred(&encrypt_template_deferred_data, 0);
		break;
