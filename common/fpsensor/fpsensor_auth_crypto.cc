		      ...
		      uint8_t *data, size_t data_size)
{
	...
	RAND_bytes(info.encryption_salt, izeof(info.encryption_salt));
			       ...
			       size_t ck_size)
{
	if (auth_nonce_size != 32 || gsc_nonce_size != 32 ||
	    pairing_key_size != 32 || ck_size != SHA256_DIGEST_SIZE) {
		return EC_ERROR_INVAL;
	}
	...
	std::copy(result, result + SHA256_DIGEST_SIZE, ck);
...
enum ec_error_list decrypt_data_with_ck_in_place(const uint8_t *ck,
						 ...
						 size_t data_size)
{
	if (ck_size != 32 || iv_size != AES_BLOCK_SIZE) {
		return EC_ERROR_INVAL;
	}
