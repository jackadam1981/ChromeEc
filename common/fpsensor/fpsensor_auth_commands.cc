...
uint8_t fp_xfer_buffer[FP_MAX_FINGER_COUNT]
...
enum ec_error_list fill_pubkey(const EC_KEY &key, Pubkey &pubkey)
{
...
enum ec_error_list inplace_encrypt_data(uint16_t version,
...
enum ec_error_list decrypt_data(const EncryptedData &encrypted_data, Data &data)
{
	static_assert(!std::is_pointer_v<Data>,
		      "The argument must not be a pointer.");
	static_assert(sizeof(data) == sizeof(encrypted_data.data));
	...
	std::copy(std::begin(encrypted_data.data),
		  std::end(encrypted_data.data), std::begin(data));

	/* Decrypt the secret blob in-place. */
	ret = aes_gcm_decrypt(enc_key, SBP_ENC_KEY_LEN, std::begin(data),
			      std::begin(data), sizeof(data),
			      encrypted_data.info.nonce,
			      sizeof(encrypted_data.info.nonce),
			      encrypted_data.info.tag,
			      sizeof(encrypted_data.info.tag));
...
template <typename Pubkey>
bssl::UniquePtr<EC_KEY> create_ec_key_from_pubkey(const Pubkey &pubkey)
{
...
fp_command_read_match_secret_with_pubkey(struct host_cmd_handler_args *args)
{
	...
	ret = fill_ecdh_share_secret(*private_key, *public_key, enc_key);
	if (ret != EC_SUCCESS) {
