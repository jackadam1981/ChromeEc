...
fp_command_unlock_template(struct host_cmd_handler_args *args)
{
	...
	for (uint16_t idx = 0; idx < fgr_num; idx++) {
		...
		if (auto *tmpl_user_id =
			    std::get_if<fp_template_state::decrypted_state>(
				    &template_states[idx].state)) {
			if (safe_memcmp(tmpl_user_id, user_id,
					sizeof(user_id)) != 0) {
				return EC_RES_ACCESS_DENIED;
			}
		...
		auto *enc_info =
			std::get_if<fp_template_state::encrypted_state>(
				&template_states[idx].state);
		...
		/* fp_enc_buffer = fp_template || fp_positive_match_salt */
		const size_t fp_enc_buffer_size =
			sizeof(fp_template[0]) +
			sizeof(fp_positive_match_salt[0]);
		static_assert(fp_enc_buffer_size <= sizeof(fp_enc_buffer));
		...
		std::copy(std::begin(fp_template[idx]),
			  std::end(fp_template[idx]), std::begin(enc_template));
		...
		if (derive_encryption_key(key.data(),
		...
		if (aes_gcm_decrypt(key.data(), SBP_ENC_KEY_LEN, fp_enc_buffer,
				    fp_enc_buffer, fp_enc_buffer_size,
