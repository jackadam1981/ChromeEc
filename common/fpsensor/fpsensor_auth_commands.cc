...
{
	auto *dec_state = std::get_if<fp_template_state::decrypted_state>(
	...
	if (dec_state) {
		...
		dec_state->is_locked = false;
		return EC_RES_SUCCESS;
	}

	auto *enc_state = std::get_if<fp_template_state::encrypted_state>(
		&template_states[idx].state);
	...
	std::copy(std::begin(fp_template[idx]), std::end(fp_template[idx]),
		  std::begin(enc_template));
	std::copy(std::begin(fp_positive_match_salt[idx]),
		  std::end(fp_positive_match_salt[idx]), std::begin(enc_salt));
	...
	if (aes_gcm_decrypt(key.data(), SBP_ENC_KEY_LEN, fp_enc_buffer,
			    fp_enc_buffer, fp_enc_buffer_size, enc_info.nonce,
