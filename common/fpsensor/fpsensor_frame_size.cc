...
FpFrameSizeCache::FpFrameSizeCache(uint32_t max_frame_size_bytes)
{
	is_initialized_ = populate_cache(max_frame_size_bytes);
}

bool FpFrameSizeCache::is_initialized() const
{
...
{
	if (!is_initialized_) {
		CPRINTF("Error: FpFrameSizeCache is uninitialized, returning size 0.");
		return 0;
	}
