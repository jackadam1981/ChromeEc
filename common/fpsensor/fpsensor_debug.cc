...
test_export_static uint8_t get_sensor_bpp(int capture_type)
{
	...
	return info
		.image_frame[convert_vendor_capture_type_to_fp_capture_type(
			capture_type)]
		.bpp;
#else
	return EC_ERROR_UNKNOWN;
...
static int command_fpdownload(int argc, const char **argv)
{
	...
	return upload_pgm_image(
		...
		get_sensor_bpp(FP_CAPTURE_TYPE(global_context.sensor_mode)));
}
