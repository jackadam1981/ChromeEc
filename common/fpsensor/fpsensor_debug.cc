 ...
 *
 */
static int upload_pgm_image(uint8_t *frame, uint8_t bpp)
{
	uint8_t *ptr = frame;
	uint8_t bytes_per_pixel = DIV_ROUND_NEAREST(bpp, 8);
...
static int command_fpdownload(int argc, const char **argv)
{
	...
	upload_pgm_image(fp_buffer + FP_SENSOR_IMAGE_OFFSET, get_sensor_bpp());
	return EC_SUCCESS;
}
