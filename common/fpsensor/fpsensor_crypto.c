			 ...
			 const uint8_t *ikm, size_t ikm_size)
{
	 ...
	 */
	compute_sha256(prk, salt, salt_size, ikm, ikm_size);
}
				...
				size_t info_size)
{
	...
	compute_sha256(key_buf, prk, prk_size, message_buf, info_size + 1);
...
#define HASH_LEN SHA256_DIGEST_SIZE
	...
	while (L > 0) {
		...
		compute_sha256(T_buffer, prk, prk_size, info_buffer,
