...
int derive_pos_match_secret(uint8_t *output,
			    const uint8_t *input_encryption_salt)
{
	...
	if (bytes_are_trivial(output, FP_POS_MATCH_SECRET_BYTES)) {
		CPRINTS("Failed to derive positive match secret: "
...
{
	hmac_SHA256(output, positive_match_secret, FP_POS_MATCH_SECRET_BYTES,
