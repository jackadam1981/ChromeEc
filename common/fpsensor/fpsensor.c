...
#define FP_SENSOR_RES_X_FPC 0
#define FP_SENSOR_RES_Y_FPC 0
#endif
#if !defined(CONFIG_FP_SENSOR_ELAN80) && !defined(CONFIG_FP_SENSOR_ELAN515)
#define FP_SENSOR_RES_X_ELAN 0
#define FP_SENSOR_RES_Y_ELAN 0
#endif

/* Fp sensor driver interface. */
struct fp_sensor_interface_t *fp_driver;
...
static uint32_t fp_process_enroll(void)
{
	...
	res = fp_driver->fp_finger_enroll((uint8_t *)&fp_buffer, &percent);
	...
	if (percent == 100) {
		res = fp_driver->fp_enrollment_finish(
			(uint8_t *)&fp_template +
			templ_valid * fp_sensor_config.algorithm_template_size);
		if (res) {
...
static uint32_t fp_process_match(void)
{
	...
	if (templ_valid) {
		res = fp_driver->fp_finger_match((uint8_t *)&fp_template,
						 templ_valid,
						 (uint8_t *)&fp_buffer, &fgr,
						 &updated);
...
{
	timestamp_t t0 = get_time();
	int res = fp_driver->fp_sensor_acquire_image_with_mode_ptr(
		(uint8_t *)&fp_buffer, FP_CAPTURE_TYPE(sensor_mode));
...
#ifdef HAVE_FP_PRIVATE_DRIVER
	/*
	 * Initialize variables based on run-time sensor detection. This is
	 * done here instead of board init because board.c should not see
	 * driver-related constants.
	 */
	if (fp_driver->sensor_type == FP_SENSOR_TYPE_ELAN) {
		fp_sensor_config.buffer_size = sizeof(fp_buffer.elan);
		fp_sensor_config.algorithm_template_size =
			sizeof(fp_template.elan[0]);
		fp_sensor_config.enc_buffer_size = sizeof(fp_enc_buffer.elan);
		fp_sensor_config.res_x = FP_SENSOR_RES_X_ELAN;
		fp_sensor_config.res_y = FP_SENSOR_RES_Y_ELAN;
	} else {
		fp_sensor_config.buffer_size = sizeof(fp_buffer.fpc);
		fp_sensor_config.algorithm_template_size =
			sizeof(fp_template.fpc[0]);
		fp_sensor_config.enc_buffer_size = sizeof(fp_enc_buffer.fpc);
		fp_sensor_config.res_x = FP_SENSOR_RES_X_FPC;
		fp_sensor_config.res_y = FP_SENSOR_RES_Y_FPC;
	}
	...
	while (1) {
		...
		if (evt & TASK_EVENT_UPDATE_CONFIG) {
			...
			if (is_test_capture(mode)) {
				fp_driver->fp_sensor_acquire_image_with_mode_ptr(
					(uint8_t *)&fp_buffer,
				...
				fp_driver->fp_sensor_configure_detect_ptr();
			}
			...
			if (sensor_mode & FP_MODE_ANY_DETECT_FINGER) {
				st = fp_driver->fp_sensor_finger_status_ptr();
...
static enum ec_status fp_command_info(struct host_cmd_handler_args *args)
{
	...
	r->template_size = fp_sensor_config.enc_buffer_size;
...
static enum ec_status fp_command_frame(struct host_cmd_handler_args *args)
{
	...
	if (idx == FP_FRAME_INDEX_RAW_IMAGE) {
		...
		ret = validate_fp_buffer_offset(fp_sensor_config.buffer_size,
						offset, size);
		if (ret != EC_SUCCESS)
			return EC_RES_INVALID_PARAM;

		memcpy(out, (uint8_t *)&fp_buffer + offset, size);
	...
	if (fgr >= templ_valid)
		return EC_RES_UNAVAILABLE;
	ret = validate_fp_buffer_offset(fp_sensor_config.enc_buffer_size,
					offset, size);
	if (ret != EC_SUCCESS)
		return EC_RES_INVALID_PARAM;
	...
	if (!offset) {
		...
		uint8_t *encrypted_template =
			(uint8_t *)&fp_enc_buffer + sizeof(*enc_info);
		...
		memset((uint8_t *)&fp_enc_buffer, 0,
		       fp_sensor_config.enc_buffer_size);
		/*
		...
		memcpy(encrypted_template,
		       (uint8_t *)&fp_template +
			       fgr * fp_sensor_config.algorithm_template_size,
		       fp_sensor_config.algorithm_template_size);
	...
	memcpy(out, (uint8_t *)&fp_enc_buffer + offset, size);
	args->response_size = size;
...
static enum ec_status fp_command_template(struct host_cmd_handler_args *args)
{
	...
	memcpy((uint8_t *)&fp_enc_buffer + offset, params->data, size);
...
static void upload_pgm_image(uint8_t *frame)
{
	...
	CPRINTF("P2\n%d %d\n255\n", fp_sensor_config.res_x,
		fp_sensor_config.res_y);

	for (y = 0; y < fp_sensor_config.res_y; y++) {
		watchdog_reload();
		for (x = 0; x < fp_sensor_config.res_x; x++, ptr++)
			CPRINTF("%d ", *ptr);
		CPRINTF("\n");
		cflush();
	}
...
int command_fpcapture(int argc, char **argv)
{
	...
	if (rc == EC_SUCCESS) {
		upload_pgm_image((uint8_t *)&fp_buffer +
