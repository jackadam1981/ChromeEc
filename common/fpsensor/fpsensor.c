...
static uint32_t fp_process_enroll(void)
{
	...
	if (percent == 100) {
		res = fp_driver->fp_enrollment_finish(fp_template[templ_valid]);
		if (res) {
...
static uint32_t fp_process_match(void)
{
	...
	if (templ_valid) {
		res = fp_driver->fp_finger_match(fp_template[0], templ_valid,
...
static enum ec_status fp_command_frame(struct host_cmd_handler_args *args)
{
	...
	if (fgr >= templ_valid)
		return EC_RES_UNAVAILABLE;
	ret = validate_fp_buffer_offset(FP_ALGORITHM_ENCRYPTED_TEMPLATE_SIZE,
					offset, size);
	if (ret != EC_SUCCESS)
		return EC_RES_INVALID_PARAM;
	...
	if (!offset) {
		...
		memset(fp_enc_buffer, 0, FP_ALGORITHM_ENCRYPTED_TEMPLATE_SIZE);
		/*
