...
static enum ec_status fp_command_establish_pk_wrap(struct host_cmd_handler_args *args)
{
	...
	ret = derive_encryption_key(key, params->enc_privkey_info.encryption_salt);
	if (ret != EC_SUCCESS) {
	...
	/* Decrypt the secret blob in-place. */
	ret = aes_gcm_decrypt(key, SBP_ENC_KEY_LEN, privkey,
					privkey, FP_PK_EC_PRIVATE_KEY_LEN,
					params->enc_privkey_info.nonce, FP_PK_ENCRYPTION_SALT_BYTES,
	...
	ret = X25519(r->enc_pk, privkey, params->peers_pubkey);
	if (ret != EC_SUCCESS) {
		CPRINTS("pk_wrap: Failed to call Diffie-Hellman function");
		return EC_RES_UNAVAILABLE;
	}
	...
	/* Encrypt the secret blob in-place. */
	ret = aes_gcm_encrypt(key, SBP_ENC_KEY_LEN, r->enc_pk,
					r->enc_pk, FP_PK_LEN,
					r->enc_pk_info.nonce, FP_PK_ENCRYPTION_SALT_BYTES,
					r->enc_pk_info.tag, FP_PK_TAG_BYTES);
	always_memset(key, 0, sizeof(key));
	if (ret != EC_SUCCESS) {
...
DECLARE_HOST_COMMAND(EC_CMD_FP_ESTABLISH_PK_WRAP, fp_command_establish_pk_wrap, EC_VER_MASK(0));
