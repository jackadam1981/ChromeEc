...
/*
 * Pointers to fingerprint functions.
 */
int (*fp_sensor_init)(void);
int (*fp_sensor_deinit)(void);
int (*fp_sensor_get_info)(struct ec_response_fp_info *resp);
int (*fp_sensor_acquire_image_with_mode_ptr)(uint8_t *image_data, int mode);
...
void (*fp_sensor_configure_detect_ptr)(void);
enum finger_state (*fp_sensor_finger_status_ptr)(void);
...
static uint32_t fp_process_enroll(void)
{
	...
	res = (*fp_finger_enroll)((uint8_t *)&fp_buffer, &percent);
	...
	if (percent == 100) {
		res = (*fp_enrollment_finish)(
			(uint8_t *)&fp_template +
			templ_valid * fp_algorithm_template_size);
		if (res) {
...
static uint32_t fp_process_match(void)
{
	...
	if (templ_valid) {
		res = (*fp_finger_match)((uint8_t *)&fp_template, templ_valid,
					 (uint8_t *)&fp_buffer, &fgr, &updated);
...
{
	timestamp_t t0 = get_time();
	int res = (*fp_sensor_acquire_image_with_mode_ptr)(
...
void fp_task(void)
{
	 ...
	 */
	if (elan) {
		fp_buffer_size = sizeof(fp_buffer.elan);
		fp_algorithm_template_size = sizeof(fp_template.elan[0]);
		fp_enc_buffer_size = sizeof(fp_enc_buffer.elan);
		fp_sensor_res_x = FP_SENSOR_RES_X_ELAN;
		fp_sensor_res_y = FP_SENSOR_RES_Y_ELAN;
	} else {
		fp_buffer_size = sizeof(fp_buffer.fpc);
		fp_algorithm_template_size = sizeof(fp_template.fpc[0]);
		fp_enc_buffer_size = sizeof(fp_enc_buffer.fpc);
		fp_sensor_res_x = FP_SENSOR_RES_X_FPC;
		fp_sensor_res_y = FP_SENSOR_RES_Y_FPC;
	}
	...
	while (1) {
		...
		if (evt & TASK_EVENT_UPDATE_CONFIG) {
			...
			if (sensor_mode & FP_MODE_ANY_DETECT_FINGER) {
				st = (*fp_sensor_finger_status_ptr)();
...
static enum ec_status fp_command_info(struct host_cmd_handler_args *args)
{
	...
	r->template_size = fp_enc_buffer_size;
...
static enum ec_status fp_command_frame(struct host_cmd_handler_args *args)
{
	...
	if (!offset) {
		...
		uint8_t *positive_match_salt =
			encrypted_template + fp_algorithm_template_size;
		size_t encrypted_blob_size = fp_algorithm_template_size +
		...
		memset((uint8_t *)&fp_enc_buffer, 0, fp_enc_buffer_size);
		/*
		...
		memcpy(encrypted_template,
		       (uint8_t *)&fp_template +
			       fgr * fp_algorithm_template_size,
...
static enum ec_status fp_command_template(struct host_cmd_handler_args *args)
{
	...
	if (xfer_complete) {
		...
		if (enc_info->struct_version <= 3) {
			encrypted_blob_size = fp_algorithm_template_size;
		} else {
			encrypted_blob_size = fp_algorithm_template_size +
					      sizeof(fp_positive_match_salt[0]);
		}
		...
		memcpy((uint8_t *)&fp_template +
			       idx * fp_algorithm_template_size,
		       encrypted_template, fp_algorithm_template_size);
		if (template_needs_validation_value(enc_info)) {
