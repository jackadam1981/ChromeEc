...
static uint32_t fp_process_match(void)
{
	...
	if (templ_valid) {
		...
		/*
		 * Internal error when res < 0 or match succeded but finger
		 * is invalid.
		 */
		if (res < 0 || (EC_MKBP_FP_MATCH_SUCCESS(res) &&
		    (fgr < 0 || fgr >= FP_MAX_FINGER_COUNT))) {
			res = EC_MKBP_FP_ERR_MATCH_NO_INTERNAL;
		}

		if (EC_MKBP_FP_MATCH_SUCCESS(res)) {
