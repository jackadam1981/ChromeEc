	...
	if (templ_valid) {
		...
		if (res < 0) {
			...
			if (fgr >= 0 && fgr < FP_MAX_FINGER_COUNT)
				fp_pos_match_secret_readable[fgr] = 1;
		}
...
static int fp_command_template(struct host_cmd_handler_args *args)
{
	...
	if (xfer_complete) {
		...
		memcpy(finger_id[idx],
			...
			+ FP_POS_MATCH_SALT_BYTES,
		       FP_FINGER_ID_BYTES);
	ret = derive_pos_match_secret(fp_pos_match_secret[idx],
				      fp_pos_match_salt[idx],
				      finger_id[idx]);
	if (ret != EC_RES_SUCCESS) {
		CPRINTS("fgr%d: Failed to re-derive positive match secret",
			idx);
		return EC_RES_UNAVAILABLE;
	}
		templ_valid++;
