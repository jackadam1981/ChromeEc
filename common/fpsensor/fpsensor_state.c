...
fp_command_generate_nonce(struct host_cmd_handler_args *args)
{
	...
	fp_context_status |= FP_CONTEXT_AUTH_NONCE;
...
fp_command_nonce_context(struct host_cmd_handler_args *args)
{
	...
	if (!(fp_context_status & FP_CONTEXT_AUTH_NONCE)) {
...
fp_command_read_match_secret_with_pubkey(struct host_cmd_handler_args *args)
{
	...
	p256_to_bin(&x, response->pubkey_x);
	p256_to_bin(&y, response->pubkey_y);

	p256_from_bin(params->pubkey_x, &x);
	p256_from_bin(params->pubkey_y, &y);
	p256_point_mul(&n, &x, &y, &out_x, &out_y);
	p256_to_bin(&out_x, share_secret);
