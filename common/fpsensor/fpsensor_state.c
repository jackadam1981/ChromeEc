...
static int fp_command_read_match_secret(struct host_cmd_handler_args *args)
{
	...
	if (fgr != state_copy.template_matched ||
	    state_copy.readable == false) {
		CPRINTS("Positive match secret for finger %d is not meant to "
			"be read now.");
		return EC_RES_ACCESS_DENIED;
	}

	if (derive_positive_match_secret(args->response,
					 fp_encryption_salt[fgr])
		!= EC_SUCCESS) {
	...
	args->response_size = FP_POSITIVE_MATCH_SECRET_BYTES;
