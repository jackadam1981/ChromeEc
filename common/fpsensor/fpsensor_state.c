...
union fp_template_t fp_template FP_FRAME_SECTION;
...
static void _fp_clear_context(void)
{
	...
	templ_valid = 0;
	templ_dirty = 0;
	always_memset(fp_buffer.elan, 0, sizeof(fp_buffer.elan));
	always_memset(fp_enc_buffer.elan, 0, sizeof(fp_enc_buffer.elan));
	always_memset(fp_buffer.fpc, 0, sizeof(fp_buffer.fpc));
	always_memset(fp_enc_buffer.fpc, 0, sizeof(fp_enc_buffer.fpc));
...
{
	if (elan && fp_sensor_deinit_elan() != EC_SUCCESS)
		CPRINTS("Failed to deinit sensor");
	if (!elan && fp_sensor_deinit_fpc() != EC_SUCCESS)
		CPRINTS("Failed to deinit sensor");
	_fp_clear_context();
	if (elan && fp_sensor_init_elan() != EC_SUCCESS)
		CPRINTS("Failed to init sensor");
	if (!elan && fp_sensor_init_fpc() != EC_SUCCESS)
		CPRINTS("Failed to init sensor");
}
