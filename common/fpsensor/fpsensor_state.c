...
void fp_clear_finger_context(int idx)
{
	...
	memset(fp_encryption_salt[idx], 0, sizeof(fp_encryption_salt[0]));
}
...
int fp_enable_match_secret_for_finger(uint32_t fgr)
{
	...
	if (fgr != template_with_secret) {
		CPRINTS("Error: secret for finger %d is not supposed to be "
			"read now.");
		return EC_ERROR_INVAL;
	}
...
void fp_disable_match_secret(void)
{
	read_secret_deadline.val = 0;
	template_with_secret = -1;
