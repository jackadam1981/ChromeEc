...
{
	struct ec_response_fp_establish_pk_keygen *r =
...
{
	const struct ec_params_fp_establish_pk_wrap *params =
...
BUILD_ASSERT(FP_CONTEXT_USERID_LEN == AES_BLOCK_SIZE);
...
{
	const struct ec_params_fp_preload_template *params =
		static_cast<const ec_params_fp_preload_template *>(
			args->params);
