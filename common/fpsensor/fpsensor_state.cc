...
uint8_t *get_tpm_seed()
{
	return global_context.state.tpm_seed;
}
...
void fp_reset_context()
{
	...
	global_context.state.templ_dirty = 0;
	global_context.state.template_newly_enrolled = FP_NO_SUCH_TEMPLATE;
	global_context.state.fp_encryption_status &= FP_ENC_STATUS_SEED_SET;
	OPENSSL_cleanse(fp_enc_buffer, sizeof(fp_enc_buffer));
	OPENSSL_cleanse(global_context.state.user_id,
			sizeof(global_context.state.user_id));
...
void fp_init_decrypted_template_state_with_user_id(uint16_t idx)
{
	...
	std::ranges::copy(global_context.state.user_id, raw_user_id.begin());
...
{
	uint32_t event_out = atomic_clear(&global_context.state.fp_events);
...
static enum ec_status fp_command_tpm_seed(struct host_cmd_handler_args *args)
{
	...
	memcpy(global_context.state.tpm_seed, params->seed,
	       sizeof(global_context.state.tpm_seed));
	global_context.state.fp_encryption_status |= FP_ENC_STATUS_SEED_SET;
...
fp_command_encryption_status(struct host_cmd_handler_args *args)
{
	...
	r->status = global_context.state.fp_encryption_status;
...
static int validate_fp_mode(const uint32_t mode)
{
	...
	uint32_t cur_mode = global_context.state.sensor_mode;
	...
	if ((mode & FP_MODE_ENROLL_SESSION) &&
	    global_context.state.templ_valid >= FP_MAX_FINGER_COUNT) {
...
enum ec_status fp_set_sensor_mode(uint32_t mode, uint32_t *mode_output)
{
	...
	if (!(mode & FP_MODE_DONT_CHANGE)) {
		global_context.state.sensor_mode = mode;
		task_set_event(TASK_ID_FPSENSOR, TASK_EVENT_UPDATE_CONFIG);
	}

	*mode_output = global_context.state.sensor_mode;
	return EC_RES_SUCCESS;
}
...
static enum ec_status fp_command_context(struct host_cmd_handler_args *args)
{
	...
	case FP_CONTEXT_ASYNC:
		if (global_context.state.sensor_mode & FP_MODE_RESET_SENSOR)
			return EC_RES_BUSY;
	...
	case FP_CONTEXT_GET_RESULT:
		...
		memcpy(global_context.state.user_id, p->userid,
		       sizeof(global_context.state.user_id));

		/* Set the FP_CONTEXT_USER_ID_SET bit if the user_id is
		 * non-zero. */
		for (size_t i = 0; i < std::size(global_context.state.user_id);
		     i++) {
			if (global_context.state.user_id[i] != 0) {
	...
	uint8_t positive_match_secret[FP_POSITIVE_MATCH_SECRET_BYTES])
{
	...
	if (derive_positive_match_secret(
		    ...
		    fp_positive_match_salt[fgr], global_context.state.user_id,
		    global_context.state.tpm_seed) != EC_SUCCESS) {
