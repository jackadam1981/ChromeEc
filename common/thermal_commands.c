...
enum lpc_status thermal_command_toggle_auto_fan_ctrl(uint8_t *data)
{
	struct lpc_params_thermal_toggle_auto_fan *p =
			(struct lpc_params_thermal_toggle_auto_fan *)data;

	if (thermal_toggle_auto_fan_ctrl(p->auto_fan_on))
		return EC_LPC_STATUS_ERROR;
	return EC_LPC_STATUS_SUCCESS;
}
DECLARE_HOST_COMMAND(EC_LPC_COMMAND_THERMAL_TOGGLE_AUTO_FAN_CTRL,
		thermal_command_toggle_auto_fan_ctrl);
