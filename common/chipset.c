...
static int PRESERVE_LOGS_SECT(preserve_reset_logs) next_reset_log;
static uint32_t ap_resets_since_ec_boot;
/* keep reset_logs size a power of 2 */
static struct ap_reset_log_entry PRESERVE_LOGS_SECT(preserve_reset_logs)
		reset_logs[4];
static int PRESERVE_LOGS_SECT(preserve_reset_logs) reset_log_checksum;
...
void init_reset_log(void)
{
	if (next_reset_log < 0 || next_reset_log >= ARRAY_SIZE(reset_logs) ||
		...
		next_reset_log = 0;
		for (int i = 0; i < ARRAY_SIZE(reset_logs); i++) {
			reset_logs[i].reset_cause = 0;
			reset_logs[i].reserved = 0;
			reset_logs[i].reset_time_ms = 0;
		}
	}
}
