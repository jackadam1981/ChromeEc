...
static uint32_t ap_resets_since_ap_boot;
...
void report_ap_reset(enum chipset_shutdown_reason reason)
{
	...
	uint32_t log_now = (uint32_t)(now.val / 1000);

	mutex_lock(&reset_log_mutex);
	reset_logs[next_reset_log].reset_cause = reason;
	reset_logs[next_reset_log++].reset_time_ms = log_now;
	next_reset_log &= ARRAY_SIZE(reset_logs) - 1;
	ap_resets_since_ap_boot++;
...
static int host_command_get_uptime_info(struct host_cmd_handler_args *args)
{
	...
	r->time_since_ec_boot_ms = log_now;
	r->ec_reset_flags = system_get_reset_flags();

	mutex_lock(&reset_log_mutex);
	r->ap_resets_since_ec_boot = ap_resets_since_ap_boot;
	for (i = 0; i != ARRAY_SIZE(reset_logs); ++i) {
		log_address = (next_reset_log + i) &
			(ARRAY_SIZE(reset_logs) - 1);
		r->recent_ap_reset[i].reset_cause =
			reset_logs[log_address].reset_cause;
		r->recent_ap_reset[i].reset_time_ms =
			reset_logs[log_address].reset_time_ms;
	}
	mutex_unlock(&reset_log_mutex);
