...
static void dfp_consume_cable_response(int port, int cnt, uint32_t *payload)
{
	if (cnt > 4) {
		...
		cable[port].attr.raw_value = payload[4];
	}
	 ...
	 */
	if (cnt > 5 && cable[port].rev == PD_REV30 &&
				cable[port].type == IDH_PTYPE_ACABLE)
		cable[port].attr2.raw_value = payload[5];
#endif
}
...
void reset_pd_cable(int port)
{
	...
	cable[port].attr.raw_value = 0;
	cable[port].flags = 0;
	cable[port].rev = PD_REV20;
#ifdef CONFIG_USB_PD_REV30
	cable[port].attr2.raw_value = 0;
#endif
}
...
#ifdef CONFIG_CMD_USB_PD_CABLE_CHAR
...
static int command_cable(int argc, char **argv)
{
	...
	/*
	 * For Rev 3.0 passive cables and Rev 2.0 active and passsive cables,
	...
	/*
	 * For Rev 3.0 active cables and Rev 2.0 active and passsive cables,
	 * SOP" controller preset have same bit 3
	 */
	if (cable[port].type == IDH_PTYPE_ACABLE) {
		ccprintf("SOP\" Controller present : %s\n",
			cable[port].attr.rev20.controller ? "Yes" : "No");
	}

	if (cable[port].rev == PD_REV30) {
		/*
		 * For Rev 3.0 active and passsive cables, Max Vbus vtg have
		 * same bits 10:9.
		 */
		...
		if (cable[port].type == IDH_PTYPE_ACABLE) {
			ccprintf("SS signaling: USB_SS_GEN%u\n",
					cable[port].attr2.sss ? 2 : 1);
			ccprintf("Number of SS lanes supported: %u\n",
					cable[port].attr2.lanes);
		}
	}
	return EC_SUCCESS;
}

DECLARE_CONSOLE_COMMAND(cable, command_cable,
			"<port>",
			"Cable Characteristics");
#endif /* CONFIG_CMD_USB_PD_CABLE_CHAR */
