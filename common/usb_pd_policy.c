...
bool is_tbt_compat_enabled(int port)
{
	if (IS_ENABLED(CONFIG_USB_PD_TBT_COMPAT_MODE))
		return !!(cable[port].flags & CABLE_FLAGS_TBT_COMPAT_ENABLE);
...
{
	/*
	 * Return Discover mode SOP prime response for thunderbolt
	 * compatible mode SVDO.
	 */
...
{
	/*
	 * Return Discover mode SOP response for thunderbolt
	 * compatible mode SVDO.
	 */
...
static int is_usb4_enabled(int port)
{
	if (IS_ENABLED(CONFIG_USB_PD_USB4))
		return !!(cable[port].flags & CABLE_FLAGS_USB4_CAPABLE);
	return 0;
}

uint8_t is_usb4_allowed(int port)
{
	if (is_usb4_enabled(port))
		return !!(cable[port].flags & CABLE_FLAGS_ENTER_USB);
	return 0;
}
...
{
	if (is_usb4_enabled(port))
		cable[port].flags &= ~CABLE_FLAGS_ENTER_USB;
}

static void disable_usb4(int port)
{
	if (IS_ENABLED(CONFIG_USB_PD_USB4))
...
{
	/* Disable USB4 mode if no response from the cable */
	disable_usb4(port);
	cable[port].is_identified = 1;
}

union enter_usb_data_obj get_enter_usb_payload(int port)
{
	union enter_usb_data_obj eudo;

	eudo.payload.mode = USB_PD_4;
	eudo.payload.usb4_drd_cap = IS_ENABLED(CONFIG_USB_PD_USB4);
	eudo.payload.usb3_drd_cap = IS_ENABLED(CONFIG_USB_PD_REV30);
	eudo.payload.cable_speed = cable[port].cable_gen;
	eudo.payload.cable_type = get_usb_pd_mux_cable_type(port) ==
			       IDH_PTYPE_ACABLE ? CABLE_TYPE_ACTIVE_RETIMER :
						  CABLE_TYPE_PASSIVE;
	switch (cable[port].attr.rev20.current) {
	case CABLE_CURRENT_3A:
		eudo.payload.cable_current = USB4_CABLE_CURRENT_3A;
		break;
	case CABLE_CURRENT_5A:
		eudo.payload.cable_current = USB4_CABLE_CURRENT_5A;
		break;
	default:
		eudo.payload.cable_current = USB4_CABLE_CURRENT_INVALID;
		break;
	}
	eudo.payload.pcie_supported = IS_ENABLED(CONFIG_USB_PD_ALT_MODE_DFP);
	eudo.payload.dp_supported = IS_ENABLED(CONFIG_USB_PD_ALT_MODE_DFP);
	eudo.payload.tbt_supported = IS_ENABLED(CONFIG_USB_PD_TBT_COMPAT_MODE);
	eudo.payload.host_present = 1;
...
static bool is_intel_svid(int port, uint32_t *payload)
{
	 ...
	 */
	if (is_tbt_compat_enabled(port) || is_usb4_enabled(port)) {
		/*
...
static inline bool is_tbt_compat_mode(int port, int cnt)
{
...
static inline void usb4_enable_enter_usb(int port)
{
	cable[port].flags |= CABLE_FLAGS_ENTER_USB;
}

static int is_dfp_gen3_capable(int port)
{
	/* TODO: Check if dfp is gen3 capable */
	return 1;
}

static int is_cable_usb4_capable(int port)
{
	/*
	 ...
	 * USB Signaling      Enter
	 * -----------------------------------------------------
	 * USB4 Gen3          USB4 with USB4 Gen3 passive cable
	 * -----------------------------------------------------
	 * USB3.2 Gen2        USB4 with TBT3 Gen3 passive cable
	 * with DFP capable
	 * ----------------------------------------------------
	 * USB3.2 Gen2        USB4 with USB3.2 Gen2 passive cable
	 * without DFP cap
	 * -----------------------------------------------------
	 * USB3.2 Gen1        USB4 with USB3.2 Gen1 passive cable
	 * ------------------------------------------------------
	 * USB2.0             Exit USB4 discovery
	 * ------------------------------------------------------
	 */
	...
	if (is_usb4_enabled(port) &&
	   (get_usb_pd_mux_cable_type(port) == IDH_PTYPE_PCABLE)) {
		/* USB signaling: USB4 Gen 3 */
		if (cable[port].attr.p_rev30.ss == USB_SS_U40_GEN3)
			cable[port].cable_gen = PCABLE_USB4_GEN3;
		/* USB signaling USB3.2 Gen 2 */
		else if (cable[port].attr.p_rev30.ss ==
			 USB_SS_U31_GEN2) {
			cable[port].cable_gen = PCABLE_USB32_GEN2;
			if (is_dfp_gen3_capable(port))
				return 0;
		/* USB signalling USB3.2 Gen 1 */
		} else if (cable[port].attr.p_rev30.ss ==
			 USB_SS_U31_GEN1)
			cable[port].cable_gen = PCABLE_USB32_GEN2;
		/* USB signalling USB2.0 */
		else {
	...
	disable_transmit_sop_prime(port);
	disable_usb4(port);
	/* Fall back to Thunderbolt Compat mode */
...
static void dfp_consume_identity(int port, int cnt, uint32_t *payload)
{
	...
	if (is_vdo_present(cnt, VDO_INDEX_PTYPE_DEVICE) &&
	    IS_ENABLED(CONFIG_USB_PD_USB4) &&
	    PD_PRODUCT_IS_USB4(payload[VDO_INDEX_PTYPE_DEVICE])) {
		cable[port].flags |= CABLE_FLAGS_USB4_CAPABLE;
	/*
	 * If port parter is not USB4 capable, check
	 * if it is Thunderbolt compatible
	 */
	} else {
		enable_tbt_compat_mode(port);
		disable_usb4(port);
...
static void dfp_consume_cable_response(int port, int cnt, uint32_t *payload)
{
	...
	/* Disable TBT mode if no response from the cable */
	if (IS_ENABLED(CONFIG_USB_PD_TBT_COMPAT_MODE) &&
...
{
	uint16_t svid = is_transmit_msg_sop_prime(port) ?
			USB_VID_INTEL : pe[port].svids[pe[port].svid_idx].svid;
...
static void dfp_consume_modes(int port, int cnt, uint32_t *payload)
{
	...
	if (pe[port].svids[idx].mode_cnt < 0) {
		CPRINTF("ERR:NOMODE\n");
	} else if (is_tbt_compat_enabled(port) || is_usb4_enabled(port)) {
		if (is_transmit_msg_sop_prime(port)) {
			...
			memcpy(pe[port].svids[pe[port].svid_idx].mode_vdo,
			       &payload[1],
...
static bool enter_tbt_compat_mode(int port, uint32_t *payload)
{
	/* If Passive cable Enter mode SOP */
	if (get_usb_pd_mux_cable_type(port) == IDH_PTYPE_PCABLE)
		disable_transmit_sop_prime(port);

	/* Enter TBT compatible mode */
	...
	if (cmd_type == CMDT_INIT) {
		...
		case CMD_DISCOVER_IDENT:
			...
			if (is_transmit_msg_sop_prime(port)) {
				...
				if (is_cable_usb4_capable(port)) {
					usb4_enable_enter_usb(port);
					usb_pd_mux_enter_safe_mode(port);
					rsize = 0;
					break;
				}
				if (is_tbt_compat_enabled(port))
				...
				/*
				 * Disable compatible thunderbolt mode if cable
				...
				/*
				 * Disable compatible thunderbolt mode
				...
				/*
				 * Enable cable detection only if device
				 * supportes Thunderbolt compatible mode
				 * or USB4 mode
				 */
				if (is_tbt_compat_enabled(port) ||
				    is_usb4_enabled(port)) {
		...
		case CMD_DISCOVER_SVID:
			 ...
			 * If not, disable compatible Thunderbolt mode
			 * Ref: USB Type-C Cable and Connector Specification,
			 ...
			 */
				...
				/*
				 * For USB4 mode: if cable is not thunderbolt
				 * compatible, Enter USB4 with USB3.2 Gen2
				 * passive cable
				 */
				if (is_usb4_enabled(port)) {
					disable_transmit_sop_prime(port);
		...
		case CMD_DISCOVER_MODES:
			...
			if (is_tbt_compat_mode(port, cnt)) {
				if (is_transmit_msg_sop_prime(port)) {
		...
		case CMD_ENTER_MODE:
			 ...
			 * Ref: USB Type-C Cable and Connector Specification,
			 * figure F-1: TBT3 Discovery Flow and
			 * Section F.2.7 TBT3 Cable Enter Mode Command
			 */
			/*
			 * TODO: Support for entering TBT comaptible mode
			 * for active cables.
			 */
			if (is_transmit_msg_sop_prime(port)) {
				disable_transmit_sop_prime(port);
			...
			/* No response once device (and cable) acks */
				...
				/* Update Mux state to compat TBT */
				set_tbt_compat_mode_ready(port);
				rsize = 0;
			/* Continue with PD flow if compat TBT is disabled */
			} else if (modep) {
				if (modep->opos) {
