...
/*
 * TODO: Make the below three functions independent of Thunderbolt-compatible
 * mode enabled as these USB PD 3.0 VDO responses can be used to identify
 * other cable attributes like cable speed.
 */
...
static bool is_cable_usb4_capable(int port)
{
	   ...
	   (get_usb_pd_mux_cable_type(port) == IDH_PTYPE_PCABLE)) {
		...
		case USB_SS_U31_GEN2:
			...
			if (is_dfp_gen3_capable(port))
				return false;
			break;
		/* USB signaling USB3.2 Gen 1 */
		case USB_SS_U31_GEN1:
			cable[port].cable_gen = PCABLE_USB32_GEN2;
			break;
		/* USB signaling USB2.0 */
		default:
	...
	disable_transmit_sop_prime(port);
	disable_usb4(port);
	/* Fall back to Thunderbolt-compatible mode */
	...
	if (cmd_type == CMDT_INIT) {
		...
		case CMD_DISCOVER_IDENT:
			...
			if (is_transmit_msg_sop_prime(port)) {
				 ...
				 * supports Thunderbolt-compatible mode
				 * or USB4 mode
				 */
				if (is_tbt_compat_enabled(port) ||
		...
		case CMD_ENTER_MODE:
			 ...
			 */
			/*
			 * TODO: Support for entering Thunderbolt-compatible
			 * mode for active cables.
			 */
			if (is_transmit_msg_sop_prime(port)) {
				disable_transmit_sop_prime(port);
