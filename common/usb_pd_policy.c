...
static int process_tbt_compat_discover_modes(int port, uint32_t head,
					     uint32_t *payload)
{
	int rsize;
	...
	if (is_transmit_msg_sop_prime(port)) {
		...
		if (is_usb4_mode_enabled(port)) {
			 ...
			 */
			if (pd_cable_supports_tbt_speed(port)) {
		...
		rsize = enter_tbt_compat_mode(port, head, payload);
	} else {
		/* Store Discover Mode SOP response */
...
static int enter_mode_tbt_compat(int port, uint32_t head, uint32_t *payload)
{
	...
	if (is_transmit_msg_sop_prime(port)) {
		...
		return enter_tbt_compat_mode(port, head, payload);
	}
		...
		uint16_t head)
{
	...
	*rpayload = payload;
	...
	if (cmd_type == CMDT_INIT) {
		...
		case CMD_ENTER_MODE:
			if (is_tbt_compat_enabled(port)) {
				rsize = enter_mode_tbt_compat(port, head,
							      payload);
			/*
