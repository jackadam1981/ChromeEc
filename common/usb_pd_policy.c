...
uint32_t get_enter_usb_msg_payload(int port)
{
	...
	eudo.payload.cable_speed = cable[port].attr.p_rev30.ss;
 ...
 */
static void usb_pd_mux_enter_safe_mode(int port)
{
...
static bool is_tbt_cable_superspeed(int port)
{
	if (IS_ENABLED(CONFIG_USB_PD_TBT_COMPAT_MODE) &&
		...
		if (IS_ENABLED(CONFIG_USB_PD_REV30) &&
			...
			return cable[port].attr.p_tbt_rev30.ss ==
				USB_R30_SS_U32_USB40_GEN2 ||
				cable[port].attr.p_tbt_rev30.ss ==
				USB_R30_SS_U40_GEN3;

		return cable[port].attr.p_tbt_rev20.ss ==
...
static bool is_cable_usb4_capable(int port)
{
	...
	if (IS_ENABLED(CONFIG_USB_PD_USB4) &&
	   (get_usb_pd_mux_cable_type(port) == IDH_PTYPE_PCABLE)) {
		...
		case USB_R30_SS_U32_GEN1:
		/*
		 * USB signaling USB3.2/USB4 Gen 2
		 * TODO: Check if DFP is Gen3 capable
		 */
		case USB_R30_SS_U32_USB40_GEN2:
			break;
		default:
...
static void dfp_consume_cable_response(int port, int cnt, uint32_t *payload)
{
	...
	if (is_vdo_present(cnt, VDO_INDEX_HDR))
		cable[port].rev = PD_HEADER_REV(payload[VDO_INDEX_HDR]);
	...
	if (cmd_type == CMDT_INIT) {
		...
		case CMD_DISCOVER_IDENT:
			...
			if (is_transmit_msg_sop_prime(port)) {
				...
				/*
				 * Enable Thunderbolt-compatible mode if cable
				 * supports superspeed
				 */
