...
static int process_am_discover_svids(int port, int cnt,
		enum tcpm_transmit_type sop, uint32_t *head, uint32_t *payload)
{
		...
		enum tcpm_transmit_type sop, uint32_t *head, uint32_t *payload)
{
	...
	if (sop == TCPC_TX_SOP_PRIME) {
		...
		/*
		 * Enable Enter Mode SOP' (Cable Enter Mode) and Enter USB SOP'
		 * for active cables.
		 */
		...
		uint32_t *head)
{
	...
	*rpayload = payload;
	...
	/* Reset transmit type to SOP */
	*head &= ~PD_HEADER_RESET_SOP_MASK;
#endif
	...
	if (cmd_type == CMDT_INIT) {
		...
		case CMD_DISCOVER_IDENT:
			...
			if (sop == TCPC_TX_SOP_PRIME) {
				/* Store cable type */
				dfp_consume_cable_response(port, cnt, head,
							payload);
			...
			/* Received a SOP Discover Ident Message */
				...
				if (is_modal(port, cnt, payload) ||
					...
					*head |=
					       PD_HEADER_SOP(PD_MSG_SOP_PRIME);
				} else {
					rsize = dfp_discover_svids(payload);
				}
			} else {
			...
			break;
		case CMD_DISCOVER_SVID:
			rsize = process_am_discover_svids(port, cnt, sop, head,
							payload);
			break;
		case CMD_DISCOVER_MODES:
			...
			if (is_tbt_compat_enabled(port) &&
				...
				rsize = process_tbt_compat_discover_modes(port,
							    sop, head, payload);
				break;
			}
		...
		case CMD_ENTER_MODE:
			if (is_tbt_compat_enabled(port)) {
				rsize = obj_cnt_enter_tbt_compat_mode(port, sop,
								head, payload);
			/*
