...
static uint8_t get_usb4_cable_speed(int port)
{
	if ((cable[port].rev == PD_REV30) &&
	   ((cable[port].attr.p_rev30.ss != USB_R30_SS_U32_U40_GEN2) ||
	     IS_ENABLED(CONFIG_USB_PD_GEN3_CAPABLE))) {
...
static bool is_cable_usb4_capable(int port, int cnt)
{
	/*
	 * Ref: USB Type-C Cable and Connector Specification
	 * Figure 5-1 USB4 Discovery and Entry Flow Model
	 *
	 * For passive cables Revision 3 cables:
	 * -----------------------------------------------------
	 * USB Signaling      Enter
	 * -----------------------------------------------------
	 ...
	 * capable
	 * -----------------------------------------------------
	 * USB3.2 Gen2        USB4 with TBT Gen2 passive cable
	 * with DFP Gen3
	 * and not TBT3
	 * Gen3 capable
	 * ----------------------------------------------------
	...
	/*
	 * Note: This is not a part of USB Type-C Cable and Connector
	 * Specification.
	 *
	 * For Passive Revison 2 cables,
	 * -----------------------------------------------------
	 * USB Signaling      Enter
	 * -----------------------------------------------------
	 * USB3.1 Gen 1       Exit USB4 discovery
	 * -----------------------------------------------------
	 * USB3.1 Gen1 Gen2   USB4 with TBT Gen 2 passive cable
	 * with TBT Gen 2
	 * capable
	 * -----------------------------------------------------
	 * USB3.1 Gen1 Gen2   Exit USB4 discovery
	 * with TBT Gen2 not
	 * capable
	 * -----------------------------------------------------
	 * USB2.0             Exit USB4 discovery
	   ...
	   (get_usb_pd_mux_cable_type(port) == IDH_PTYPE_PCABLE) &&
		...
		case PD_REV30:
			...
			case USB_R30_SS_U32_U40_GEN2:
				...
				if (IS_ENABLED(CONFIG_USB_PD_GEN3_CAPABLE))
					return false;
				return true;
			default:
...
static bool is_usb4_vdo(int port, int cnt, uint32_t *payload)
{
	 ...
	 */
	return IS_ENABLED(CONFIG_USB_PD_USB4) &&
		is_vdo_present(cnt, VDO_INDEX_PTYPE_AMA) &&
		PD_PRODUCT_IS_USB4(payload[VDO_INDEX_PTYPE_AMA]);
}
...
static int dfp_discover_modes(int port, uint32_t *payload)
{
	 ...
	 * errata: All the Thunderbolt certified cables and docks
	 * tested have SVID1 = 0x8087.
	 * In the case of USB4 PD flow, since we send Discover Mode SOP'
		...
		case CMD_DISCOVER_IDENT:
			...
			if (is_transmit_msg_sop_prime(port)) {
				...
				if (is_usb4_mode_enabled(port) &&
					is_cable_usb4_capable(port, cnt)) {
