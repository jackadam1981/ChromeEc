			     ...
			     mat33_fp_t e_vecs)
{
	...
	for (;;) {
		...
		/*
		 * K_EPSILON(1E-5) is too small to fit into 32-bit fixed-point
		 * (with 16 fp bits). The minimun positive value is 1 which is
		 * approximately 1.52E-5, so the FLOAT_TO_FP(K_EPSILON) becomes
		 * zero.
		 */
