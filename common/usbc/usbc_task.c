...
/* Used to make usbc_pd_task_suspend/resume functions idempotent */
__test_only static volatile bool port_is_suspended[CONFIG_USB_PD_PORT_MAX_COUNT];
...
static bool pd_task_loop(int port)
{
	if (IS_ENABLED(CONFIG_ZTEST)) {
		...
		k_mutex_lock(&pd_mutex, K_FOREVER);
		is_suspending = port_to_suspend[port];
		k_mutex_unlock(&pd_mutex);
