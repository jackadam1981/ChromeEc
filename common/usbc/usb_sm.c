...
usb_state_ptr shared_parent_state(usb_state_ptr a, usb_state_ptr b)
{
...
static void call_entry_methods(const int port,
			       ...
			       const usb_state_ptr stop)
{
	...
	call_entry_methods(port, internal, current->parent, stop);

	/* Entering a new state, may call set_state, then don't enter remaining states */
	if (!internal->enter) {
		return;
	}
...
static void call_exit_methods(const int port, const usb_state_ptr current,
			      const usb_state_ptr stop)
{
	if (current == stop)
		return;
	...
	call_exit_methods(port, current->parent, stop);
}

static void call_run_methods(const int port,
			     ...
			     const usb_state_ptr current)
{
	...
	/* If set_state is called during run, don't call parents after that. */
	if (!internal->running)
		return;
	...
	call_run_methods(port, internal, current->parent);
}
	       ...
	       const usb_state_ptr new_state)
{
	 ...
	 */
	if (internal->exit) {
		CPRINTF("C%d: Ignoring set_state to 0x%08x within 0x%08x",
	 ...
	 */
	last_state = internal->enter ? ctx->current : internal->last_entered;
	 ...
	 */
	internal->exit = true;
	call_exit_methods(port, last_state, shared_parent);
	internal->exit = false;
	...
	internal->enter = true;
	call_entry_methods(port, internal, ctx->current, shared_parent);
	/*
...
void exe_state(const int port, struct sm_ctx *const ctx)
{
	...
	call_run_methods(port, internal, ctx->current);
	internal->running = false;
}
