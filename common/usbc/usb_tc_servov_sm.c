...
/*
 * 8-bit timers running at 5ms
 */
#define ENABLE_TIMER8        0x80
#define TC_T_CC_DEBOUNCE     25   /* (125*MSEC) between 100ms and 200ms */
#define TC_T_PD_DEBOUNCE     3    /* (15*MSEC) between 10ms and 20ms */
#define TC_T_POWER_SUPPLY_TURN_ON_DELAY 33 /* (165*MSEC) */
#define TC_T_ERROR_RECOVERY  48   /* (240*MSEC) min 240ms if sourcing VConn */
#define TC_T_SRC_TURN_ON     55   /* (275*MSEC) 275ms */
#define TC_T_RP_VALUE_CHANGE 4    /* (20*MSEC) 20ms */
/* DRP_SNK + DRP_SRC must be between 50ms and 100ms with 30%-70% duty cycle */
#define TC_T_DRP_SNK         8    /* (40*MSEC) toggle time for sink DRP */
#define TC_T_DRP_SRC         6    /* (30*MSEC) toggle time for source DRP */
#define TC_T_SRC_DISCONNECT  75   /* (15*MSEC) 15ms */
#define TC_T_DEBOUNCE        3    /* (15*MSEC) between 10ms and 20ms */

/*
 * 16-bit timers running at 5ms
 */
#define ENABLE_TIMER16       0x8000
#define TC_T_SRC_RECOVER     152  /* (760*MSEC) between 660ms and 1000ms */
#define TC_T_SAFE_0V         130  /* (650*MSEC) 650ms */
#define TC_T_SRC_RECOVER_MAX 200  /* (1000*MSEC) 1000ms */
...
static struct type_c {
	...
	uint8_t power_supply_turn_on_delay_timer;
	uint8_t err_recovery_timer;
	uint8_t src_turn_on_timer;
	uint8_t rp_value_change_timer;
	uint8_t drp_snk_timer;
	uint8_t drp_src_timer;
	uint8_t src_disconnect_timer;
	uint8_t debounce_timer;
	uint16_t save_0v_timer;
...
static void start_timer(int port, enum timer_t timer)
{
	switch (timer) {
	case TC_CC_DEBOUNCE:
		tc[port].cc_debounce_timer = ENABLE_TIMER8 | TC_T_CC_DEBOUNCE;
		break;
	case TC_PD_DEBOUNCE:
		tc[port].pd_debounce_timer = ENABLE_TIMER8 | TC_T_PD_DEBOUNCE;
		break;
	case TC_SRC_RECOVER:
		tc[port].src_recover_timer = ENABLE_TIMER16 | TC_T_SRC_RECOVER;
		break;
	case TC_POWER_SUPPLY_TURN_ON_DELAY:
		tc[port].power_supply_turn_on_delay_timer =
			ENABLE_TIMER8 | TC_T_POWER_SUPPLY_TURN_ON_DELAY;
		break;
	case TC_ERR_RECOVERY:
		tc[port].err_recovery_timer =
			ENABLE_TIMER8 | TC_T_ERROR_RECOVERY;
		break;
	case TC_SAFE_0V:
		tc[port].save_0v_timer = ENABLE_TIMER16 | TC_T_SAFE_0V;
		break;
	case TC_SRC_RECOVER_MAX:
		...
		break;
	case TC_SRC_TURN_ON:
		tc[port].src_turn_on_timer = ENABLE_TIMER8 | TC_T_SRC_TURN_ON;
		break;
	case TC_RP_VALUE_CHANGE:
		tc[port].rp_value_change_timer =
			ENABLE_TIMER8 | TC_T_RP_VALUE_CHANGE;
		break;
	case TC_DRP_SNK:
		tc[port].drp_snk_timer = ENABLE_TIMER8 | TC_T_DRP_SNK;
		break;
	case TC_DRP_SRC:
		tc[port].drp_src_timer = ENABLE_TIMER8 | TC_T_DRP_SRC;
		break;
	case TC_SRC_DISCONNECT:
		tc[port].src_disconnect_timer =
			ENABLE_TIMER8 | TC_T_SRC_DISCONNECT;
		break;
	case TC_DEBOUNCE:
		tc[port].debounce_timer = ENABLE_TIMER8 | TC_T_DEBOUNCE;
		break;
	}
}
...
{
	if (tc[port].cc_debounce_timer > ENABLE_TIMER8)
		tc[port].cc_debounce_timer--;

	if (tc[port].pd_debounce_timer > ENABLE_TIMER8)
		tc[port].pd_debounce_timer--;

	if (tc[port].power_supply_turn_on_delay_timer > ENABLE_TIMER8)
		tc[port].power_supply_turn_on_delay_timer--;

	if (tc[port].err_recovery_timer > ENABLE_TIMER8)
		tc[port].err_recovery_timer--;

	if (tc[port].src_turn_on_timer > ENABLE_TIMER8)
		tc[port].src_turn_on_timer--;

	if (tc[port].rp_value_change_timer > ENABLE_TIMER8)
		tc[port].rp_value_change_timer--;

	if (tc[port].drp_snk_timer > ENABLE_TIMER8)
		tc[port].drp_snk_timer--;

	if (tc[port].drp_src_timer > ENABLE_TIMER8)
		tc[port].drp_src_timer--;

	if (tc[port].src_disconnect_timer > ENABLE_TIMER8)
		tc[port].src_disconnect_timer--;

	if (tc[port].debounce_timer > ENABLE_TIMER8)
		tc[port].debounce_timer--;

	if (tc[port].save_0v_timer > ENABLE_TIMER16)
		tc[port].save_0v_timer--;

	if (tc[port].src_recover_max_timer > ENABLE_TIMER16)
		tc[port].src_recover_max_timer--;

	if (tc[port].src_recover_timer > ENABLE_TIMER16)
...
static bool is_expired_timer(int port, enum timer_t timer)
{
	switch (timer) {
	case TC_CC_DEBOUNCE:
		return (tc[port].cc_debounce_timer == ENABLE_TIMER8);
	case TC_PD_DEBOUNCE:
		return (tc[port].pd_debounce_timer == ENABLE_TIMER8);
	case TC_POWER_SUPPLY_TURN_ON_DELAY:
		return
		(tc[port].power_supply_turn_on_delay_timer == ENABLE_TIMER8);
	case TC_ERR_RECOVERY:
		return (tc[port].err_recovery_timer == ENABLE_TIMER8);
	case TC_SRC_TURN_ON:
		return (tc[port].src_turn_on_timer == ENABLE_TIMER8);
	case TC_RP_VALUE_CHANGE:
		return (tc[port].rp_value_change_timer == ENABLE_TIMER8);
	case TC_DRP_SNK:
		return (tc[port].drp_snk_timer == ENABLE_TIMER8);
	case TC_DRP_SRC:
		return (tc[port].drp_src_timer == ENABLE_TIMER8);
	case TC_SRC_DISCONNECT:
		return (tc[port].src_disconnect_timer == ENABLE_TIMER8);
	case TC_DEBOUNCE:
		return (tc[port].debounce_timer == ENABLE_TIMER8);

	case TC_SAFE_0V:
		return (tc[port].save_0v_timer == ENABLE_TIMER16);
	case TC_SRC_RECOVER_MAX:
		return (tc[port].src_recover_max_timer == ENABLE_TIMER16);
	case TC_SRC_RECOVER:
		return (tc[port].src_recover_timer == ENABLE_TIMER16);
	}
...
{
	if (port == DUT) {
		return TC_CHK_FLAG(port, TC_FLAGS_POLICY_PD_ENABLE) &&
				TC_CHK_FLAG(port, TC_FLAGS_PD_ENABLE);
	} else {
