...
static int count[CONFIG_USB_PD_PORT_MAX_COUNT];
static int max_count[CONFIG_USB_PD_PORT_MAX_COUNT];
static uint32_t timer_active[CONFIG_USB_PD_PORT_MAX_COUNT];
static uint32_t timer_disabled[CONFIG_USB_PD_PORT_MAX_COUNT];
static uint64_t timer_expires[CONFIG_USB_PD_PORT_MAX_COUNT][MAX_PD_TIMERS];
...
__maybe_unused static __const_data const char * const pd_timer_names[] = {
	...
	[PE_TIMER_FAST_WAKEUP]		= "PE-FAST_WAKEUP",
...
{
	if (pd_timer_is_inactive(port, timer))
		return true;
	else if (pd_timer_is_active(port, timer))
		return get_time().val > timer_expires[port][timer];
	return false;
}
...
int pd_timer_next_expiration(int port)
{
	...
	for (timer = 0; timer < MAX_PD_TIMERS; ++timer) {
		...
		if (!pd_timer_is_disabled(port, timer) &&
		    pd_timer_is_active(port, timer)) {
			if (timer_expires[port][timer] < now)
				ret_value = EXPIRE_NOW;
			else if (ret_value > timer_expires[port][timer] - now)
				ret_value = timer_expires[port][timer] - now;
		}
	}
