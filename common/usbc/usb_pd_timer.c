...
/*
 * There are some 32 bit platforms that do not handle 64 bit shifts.
 * So make do with 32 bit shifts and a union to make this mapping of
 * offset in the range 0-63 to uint64_t mask value.
 *
 * This code
 *	uint64_t mask = pd_mask(offset)
 * is replacing the following code
...
static uint64_t pd_mask(int offset)
{
	union mask64_t mask;
