...
#define PD_CHK_ACTIVE(p, bit) \
	atomic_test_bit(timer_active, p *MAX_PD_TIMERS + bit)

#define PD_CHK_SET_ACTIVE(p, bit) \
	atomic_test_and_set_bit(timer_active, p *MAX_PD_TIMERS + bit)

#define PD_CHK_CLR_ACTIVE(p, bit) \
	atomic_test_and_clear_bit(timer_active, p *MAX_PD_TIMERS + bit)

#define PD_SET_DISABLED(p, bit) \
	atomic_set_bit(timer_disabled, p *MAX_PD_TIMERS + bit)

#define PD_CLR_DISABLED(p, bit) \
	atomic_clear_bit(timer_disabled, p *MAX_PD_TIMERS + bit)

#define PD_CHK_DISABLED(p, bit) \
	atomic_test_bit(timer_disabled, p *MAX_PD_TIMERS + bit)

#define TIMER_FIELD_NUM_UINT32S 2

test_mockable_static ATOMIC_DEFINE(timer_active, MAX_PD_TIMERS *MAX_PD_PORTS);
test_mockable_static ATOMIC_DEFINE(timer_disabled, MAX_PD_TIMERS *MAX_PD_PORTS);
...
{
	if (PD_CHK_CLR_ACTIVE(port, timer)) {
...
void pd_timer_init(int port)
{
	...
	for (int i = 0; i < ARRAY_SIZE(timer_active); i++) {
		atomic_set(timer_active + i, 0);
		atomic_set(timer_disabled + i, ATOMIC_BITS - 1U);
	}
}

void pd_timer_enable(int port, enum pd_task_timer timer, uint32_t expires_us)
{
	if (!PD_CHK_SET_ACTIVE(port, timer)) {
		if (IS_ENABLED(CONFIG_CMD_PD_TIMER)) {
			count[port]++;
