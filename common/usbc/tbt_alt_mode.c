		...
		uint32_t *vdm)
{
	...
	case TBT_ENTER_SOP_PRIME:
		...
		if (cable_mode_resp.tbt_active_passive != TBT_CABLE_ACTIVE &&
			...
			/*
			 * When the port enters USB4 mode SOP and Thunderbolt
			 * mode SOP prime, it skips entering Thunderbolt mode
			 * SOP and SOP''.  So, on call to exit Thunderbolt alt
			 * mode, skip exiting Thunderbolt mode SOP and SOP''
			 *
			 * TODO(b/141363146): Remove once data reset feature is
			 * in place.
			 */
			if (enter_usb_port_partner_is_capable(port) &&
	...
	case TBT_ENTER_SOP_PRIME_PRIME:
		...
		TBT_SET_FLAG(port, TBT_FLAG_CABLE_ENTRY_DONE);
		/*
		 * When the port enters USB4 mode SOP and Thunderbolt mode with
		 * the cable plug, it skips entering Thunderbolt mode SOP. So,
		 * on call to exit Thunderbolt alt mode, skip exiting
