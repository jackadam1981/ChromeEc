...
{
	CPRINTS("C%d: TBT alt mode protocol failed!", port);
	tbt_state[port] = TBT_INACTIVE;
		...
		uint32_t *vdm)
{
	...
	case TBT_ENTER_SOP_RETRY_SENT:
		...
		CPRINTS("C%d: Entered TBT mode", port);
		break;
	case TBT_EXIT_SOP_SENT:
		 ...
		 * inactive state.
		 */
		CPRINTS("C%d: Exited TBT mode SOP", port);
		tbt_state[port] = TBT_INACTIVE;
		break;
...
int tbt_setup_next_vdm(int port, int vdo_count, uint32_t *vdm)
{
	...
	case TBT_ENTER_SOP_RETRY:
		if (tbt_state[port] == TBT_START)
			CPRINTS("C%d: Attempting to enter TBT mode", port);
