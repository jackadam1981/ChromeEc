...
static int command_pd(int argc, char **argv)
{
	...
	if (!strcasecmp(argv[2], "tx")) {
		...
		if (!strncasecmp(argv[3], "ping", 4)) {
			...
			pe_send_vdm(port, USB_VID_GOOGLE, VDO_CMD_PING_ENABLE,
					&enable, 1);
		} else if (!strncasecmp(argv[3], "curr", 4)) {
			pe_send_vdm(port, USB_VID_GOOGLE, VDO_CMD_CURRENT,
				    NULL, 0);
		} else if (!strncasecmp(argv[3], "vers", 4)) {
			pe_send_vdm(port, USB_VID_GOOGLE, VDO_CMD_VERSION,
				    NULL, 0);
		} else {
			return EC_ERROR_PARAM_COUNT;
		}
