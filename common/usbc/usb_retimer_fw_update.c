 ...
 *
 */
static void retimer_fw_update_port_handler(int port, bool port_enable)
{
	pd_set_suspend(port, port_enable);
	if (port_enable == SUSPEND)
		retimer_fw_update_set_port_state(port, RETIMER_OFFLINE);
	else
		retimer_fw_update_set_port_state(port, RETIMER_ONLINE);
}
...
static void retry_online(int port)
{
	...
	query_usb_mux_set_completed_timeout(cur_port);
