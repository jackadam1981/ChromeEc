 ...
 *    --------|-------|-------|       |                       |
 *    |       |       |       |       >=2.0                   |
 *  >=Gen3   Gen2  USB2.0   Gen 1     |                       |
 *    |       |       |       |   VDO version--- <1.3 ---> Modal op? -- N --|
 * Enter USB  |   Skip USB4   |   (B21:23 of                  |             |
 * SOP  with  |  mode entry   |    Discover ID SOP'-          y             |
 * Gen3 cable |               |    Active cable VDO1)         |             |
 * speed      |               |       |                    TBT SVID? -- N --|
 *            |               |       >=1.3                   |             |
 *         Is modal op?       |       |                       y             |
 *            |               |   Cable USB4  - N             |             |
...
static bool enter_usb_cable_is_capable(struct pd_discovery *disc_sop_prime)
{
...
bool enter_usb_is_capable(int port)
{
	...
	if (get_usb_pd_cable_type(port) == IDH_PTYPE_ACABLE) {
		...
		if (pd_get_vdo_ver(port, TCPC_TX_SOP_PRIME) >= VDM_VER20 &&
			 ...
			 * capable.
			 */
			if (!enter_usb_cable_is_capable(disc_sop_prime))
				return false;
		/*
		 ...
		 * doesn't have rounded support or
		 * hasn't entered Thunderbolt mode.
		 */
	...
	/*
	 * For LRD cables, do not enter USB4 mode is the cable hasn't entered
	 * Thunderbolt mode SOP'
	 */
	...
	case USB4_START:
		if (tbt_cable_entry_is_done(port) ||
			...
			*type = TCPC_TX_SOP;
			usb4_state[port] = USB4_ENTER_SOP;
			/*
			 * Set the USB mux to safe state to avoid damaging the
			 * mux pins, since they are being re-purposed for USB4.
			 */
