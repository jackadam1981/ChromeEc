...
static bool dp_is_lrd_active_cable(int port)
{
	...
	cable_mode_resp.raw_value = dp_get_mode_vdo(port, TCPCI_MSG_SOP_PRIME);
...
)
{
	...
	if (IS_ENABLED(CONFIG_USB_PD_DP21_MODE) &&
		...
		payload[1] = VDO_DP2_1_CFG(DPAM_VERSION_21, cable_type,
					   dp_is_uhbr13_5_supported(port),
					   pin_mode, bit_rate, DP_SINK);
	} else {
		payload[1] = VDO_DP_CFG(pin_mode, DP_HBR3, DP_SINK);
	}
...
#ifdef CONFIG_USB_PD_TBT_COMPAT_MODE
		} else if ((cable_dpam_ver == DPAM_VERSION_20) ||
			   (tbt_cable_mode_resp.raw_value &&
			    !tbt_cable_mode_resp.retimer_type &&
			    !tbt_cable_mode_resp.tbt_active_passive)) {
