...
static bool dp_is_lrd_active_cable(int port)
{
	...
	return (get_usb_pd_cable_type(port) == IDH_PTYPE_PCABLE &&
		cable_mode_resp.active_comp == DP21_ACTIVE_REDRIVER_CABLE);
}
...
{
	/*
	 * We require SOP' entry if cable is active cable, or
	 * an LRD cable (passive in DiscoverIdentity, active in DP mode)
	 */
	return ((get_usb_pd_cable_type(port) == IDH_PTYPE_ACABLE ||
					    ...
					    enum tcpci_msg_type *tx_type)
{
	...
	case DP_ENTER_RETRY:
		...
		if (!dp21_mode_entry_allowed(port))
			return MSG_SETUP_ERROR;
		...
		msg_type = dp_sop_prime_needed(port) ? TCPCI_MSG_SOP_PRIME :
						       TCPCI_MSG_SOP;
		vdm[0] |= VDO_SVDM_VERS(pd_get_vdo_ver(port, msg_type));
	...
	case DP_ENTER_SOP_PRIME_ACKED:
		...
		/* CMDT_INIT is 0, so this is a no-op */
		vdm[0] |= VDO_CMDT(CMDT_INIT);
		msg_type = dp_sop_prime_prime_needed(port) ?
...
)
{
	...
	if (IS_ENABLED(CONFIG_USB_PD_DP21_MODE) &&
		...
		bit_rate = dp_get_cable_bit_rate(port);
...
uint8_t get_pd_cable_type_flags(int port)
{
	...
	if (dp_cable_resp.raw_value &&
	    (resolve_dpam_version(port, TCPCI_MSG_SOP_PRIME) ==
	     DPAM_VERSION_21)) {
		cable_type |= (dp_cable_resp.active_comp ==
		...
		cable_type |= dp_cable_resp.active_comp ==
				      ...
				      0;
		cable_type |= dp_cable_resp.active_comp == DP21_OPTICAL_CABLE ?
