					    ...
					    uint32_t *vdm)
{
	...
	case DP_ENTER_RETRY:
		...
		if (pd_get_mode_vdo_for_svid(port, TCPCI_MSG_SOP,
					     ...
					     mode_vdos) == 0)
			return MSG_SETUP_ERROR;
		if (svdm_enter_dp_mode(port, mode_vdos[opos - 1]) < 0)
			return MSG_SETUP_ERROR;
...
int pd_dfp_dp_get_pin_mode(int port, uint32_t status)
{
	...
	CPRINTS("mf_allow: %d", dp_port_mf_allow[port]);
	...
	if (pd_get_mode_vdo_for_svid(port, TCPCI_MSG_SOP, USB_SID_DISPLAYPORT,
				     mode_vdos) == 0)
		return 0;
	CPRINTS("mf_pref: %d", mf_pref);

	mode_caps = mode_vdos[opos - 1];
...
__overridable int svdm_dp_attention(int port, uint32_t *payload)
{
	...
	// REMINDER - figure out DP status ACK stuff
