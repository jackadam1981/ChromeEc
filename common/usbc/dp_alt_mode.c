...
{
	/* Return true until exit is totally done */
...
static bool dp_is_lrd_active_cable(int port)
{
	union dp_mode_resp_cable cable_mode_resp;

	cable_mode_resp.raw_value =
		pd_get_dp_mode_vdo(port, TCPCI_MSG_SOP_PRIME);
	if (get_usb_pd_cable_type(port) == IDH_PTYPE_PCABLE &&
	    cable_mode_resp.cable_type == DP21_ACTIVE_REDRIVER_CABLE)
		return true;
...
static bool dp_sop_prime_needed(int port)
{
	/*
	 * We require SOP' entry if cable is
	 * active cable, or
	 * an LRD cable (passive in DiscoverIdentity, active in DP mode)
	 */
	if ((get_usb_pd_cable_type(port) == IDH_PTYPE_ACABLE ||
	     dp_is_lrd_active_cable(port)) &&
		  ...
		  uint32_t *vdm)
{
	...
	case DP_PREPARE_EXIT:
		...
		} else if (!dp_sop_prime_needed(port)) {
					    ...
					    enum tcpci_msg_type *tx_type)
{
	...
	case DP_ENTER_RETRY:
		...
		if (dp_sop_prime_needed(port)) {
			/* TODO: Supposed to check capabilities versus SOP */
			vdm[0] |= VDO_SVDM_VERS(
				pd_get_vdo_ver(port, TCPCI_MSG_SOP_PRIME));
			vdm[0] |= IS_ENABLED(CONFIG_USB_PD_DP21_MODE) ?
	...
	case DP_ENTER_ACKED:
		...
		vdm[0] |= IS_ENABLED(CONFIG_USB_PD_DP21_MODE) ?
				  VDO_SVDM_VERS_MINOR(1) :
				  0;
		/* WIP: DP status to SOP' optional, skipping in WIP */
		*tx_type = TCPCI_MSG_SOP;
		break;
	...
	case DP_PREPARE_CONFIG:
		if (dp_sop_prime_needed(port)) {
			vdo_count_ret =
				svdm_dp_config(port, vdm, TCPCI_MSG_SOP_PRIME);
			if (vdo_count_ret == 0)
				return MSG_SETUP_ERROR;
			*tx_type = TCPCI_MSG_SOP_PRIME;
		} else {
				 ...
				 enum tcpci_msg_type type)
{
	...
	if (IS_ENABLED(CONFIG_USB_PD_DP21_MODE) &&
	    ...
	    type == TCPCI_MSG_SOP) {
		cable_flags = get_pd_cable_type_flags(port);

		if (cable_flags & USB_DP_OPTICAL_CABLE) {
			cable_type = BIT(0) | BIT(1);
		} else if (cable_flags & USB_DP_ACTIVE_CABLE) {
			cable_type = (cable_flags & USB_DP_RETIMER_CABLE) ?
					     BIT(0) :
					     BIT(1);
		}
...
{
	enum usb_pd_svdm_ver cable_svdm_ver;
	enum usb_pd_svdm_ver device_svdm_ver;
	enum dpam_version cable_dpam_ver;
	enum dpam_version device_dpam_ver;
	...
	if (IS_ENABLED(CONFIG_USB_PD_DP21_MODE)) {
		cable_svdm_ver =
			resolve_svdm_version(port, TCPCI_MSG_SOP_PRIME);
		CPRINTF("Port: %d Cable respond with SVDM version: %s\n", port,
			(cable_svdm_ver == SVDM_VER_2_1) ? "SVDM2.1" :
							   "SVDM2.0");

		device_svdm_ver = resolve_svdm_version(port, TCPCI_MSG_SOP);
		CPRINTF("Port: %d UFP respond with SVDM version: %s\n", port,
			(device_svdm_ver == SVDM_VER_2_1) ? "SVDM2.1" :
							    "SVDM2.0");

		cable_dpam_ver =
			resolve_dpam_version(port, TCPCI_MSG_SOP_PRIME);
		CPRINTF("Port: %d Cable respond with DPAM version: %s\n", port,
			(cable_dpam_ver == DPAM_VERSION_21) ? "DPAM2.1" :
							      "DPAM2.0");

		device_dpam_ver = resolve_dpam_version(port, TCPCI_MSG_SOP);
		CPRINTF("Port: %d UFP respond with DPAM version: %s\n", port,
			(device_dpam_ver == DPAM_VERSION_21) ? "DPAM2.1" :
							       "DPAM2.0");
		...
		/* No DP Support, if emark cable and USB2.0 only */
...
enum dpam_version resolve_dpam_version(int port, enum tcpci_msg_type type)
{
	...
	if (resolve_svdm_version(port, type) == SVDM_VER_2_1) {
		/* TBD:  Test device UFP (SOP) is not setting DPAM version
		 * (Bit 31:30) in Discover Mode Response VDO, So returning
		 * DPAM_VERSION_21, if UFP device respond with SVDM version 2.1
		 */
		if (type == TCPCI_MSG_SOP)
...
uint8_t get_dp_cable_bit_rate(int port)
{
	...
	if ((disc->identity.idh.product_type == IDH_PTYPE_ACABLE ||
		...
		if (dp_cable_mode_resp.raw_value &&
		    (resolve_dpam_version(port, TCPCI_MSG_SOP_PRIME) ==
		     DPAM_VERSION_21)) {
			if (dp_cable_mode_resp.signaling & DP21_SPEED_UHBR20)
				return DP21_SPEED_UHBR20;
			else if (dp_cable_mode_resp.signaling &
				 DP21_SPEED_UHBR10)
				return DP21_SPEED_UHBR10;
			else if (dp_cable_mode_resp.signaling & DP21_SPEED_HBR3)
				return DP21_SPEED_HBR3;
		} else if ((dp_cable_mode_resp.raw_value &&
			    (resolve_dpam_version(port, TCPCI_MSG_SOP_PRIME) ==
			     DPAM_VERSION_20)) ||
			   (tbt_cable_mode_resp.raw_value &&
			    !tbt_cable_mode_resp.retimer_type &&
...
uint8_t get_pd_cable_type_flags(int port)
{
	...
	dp_cable_resp.raw_value = pd_get_dp_mode_vdo(port, TCPCI_MSG_SOP_PRIME);
	tbt_cable_resp.raw_value =
		IS_ENABLED(CONFIG_USB_PD_TBT_COMPAT_MODE) ?
			pd_get_tbt_mode_vdo(port, TCPCI_MSG_SOP_PRIME) :
			0;

	if (dp_cable_resp.raw_value &&
	    (resolve_dpam_version(port, TCPCI_MSG_SOP_PRIME) ==
	     DPAM_VERSION_21)) {
		cable_type |= (dp_cable_resp.cable_type ==
				       DP21_ACTIVE_RETIMER_CABLE ||
			       dp_cable_resp.cable_type ==
				       DP21_ACTIVE_REDRIVER_CABLE) ?
				      USB_DP_ACTIVE_CABLE :
				      0;
		cable_type |= dp_cable_resp.cable_type ==
					      DP21_ACTIVE_RETIMER_CABLE ?
				      USB_DP_RETIMER_CABLE :
				      0;
		cable_type |= dp_cable_resp.cable_type == DP21_OPTICAL_CABLE ?
				      USB_DP_OPTICAL_CABLE :
				      0;
	} else if (IS_ENABLED(CONFIG_USB_PD_TBT_COMPAT_MODE)) {
		cable_type |=
			(get_usb_pd_cable_type(port) == IDH_PTYPE_ACABLE ||
			 tbt_cable_resp.tbt_active_passive ==
				 TBT_CABLE_ACTIVE) ?
				USB_DP_ACTIVE_CABLE :
				0;
		cable_type |= tbt_cable_resp.retimer_type == USB_RETIMER ?
				      USB_DP_RETIMER_CABLE :
				      0;
		cable_type |= tbt_cable_resp.tbt_cable == TBT_CABLE_OPTICAL ?
				      USB_DP_OPTICAL_CABLE :
				      0;
	}
	return cable_type;
