...
/*
 * Map of states to expected VDM commands.
 * Default of 0 indicates no command expected.
 */
static const uint8_t state_vdm_cmd[DP_MAX_STATE] = {
...
void dp_shutdown(int port)
{
	CPRINTS("C%d: DP shutdown", port);
	dp_state[port] = DP_SHUTDOWN;
}
...
static int dp_check_response(int port, enum tcpm_transmit_type type,
			     char *cmdt, int vdm_cmd)
{
	 ...
	 * If DP is shutdown, ignore the command.
	 */
	if (type != TCPC_TX_SOP ||
	    (st != DP_SHUTDOWN && state_vdm_cmd[st] != vdm_cmd)) {
		...
		uint32_t *vdm)
{
	...
	if (dp_check_response(port, type, "ACK", vdm_cmd))
		return;
...
{
	if (dp_check_response(port, type, "NAK", vdm_cmd))
		return;
