...
bool dp21_mode_entry_allowed(int port)
{
	...
	cable_svdm_ver = resolve_svdm_version(port, TCPCI_MSG_SOP_PRIME);
	device_svdm_ver = resolve_svdm_version(port, TCPCI_MSG_SOP);
	cable_dpam_ver = resolve_dpam_version(port, TCPCI_MSG_SOP_PRIME);
	...
	if (disc->identity.idh.product_type == IDH_PTYPE_PCABLE && usb20_only)
		return false;
...
static enum dp21_speed dp_signaling_to_speed(uint8_t signaling)
{
	if (signaling & DP21_SPEED_UHBR20)
		return DP21_SPEED_UHBR20;
	else if (signaling & DP21_SPEED_UHBR10)
		return DP21_SPEED_UHBR10;
