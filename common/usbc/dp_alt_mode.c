					    ...
					    enum tcpci_msg_type *tx_type)
{
	...
	case DP_ENTER_RETRY:
		...
		msg_type = dp_sop_prime_needed(port) ? TCPCI_MSG_SOP_PRIME :
						       TCPCI_MSG_SOP;
		vdm[0] |= VDO_SVDM_VERS_MAJOR(pd_get_vdo_ver(port, msg_type));
...
bool is_dp_uhbr13_5_supported(int port)
{
...
)
{
	...
	if (IS_ENABLED(CONFIG_USB_PD_DP21_MODE) &&
	    ...
	    type == TCPCI_MSG_SOP) {
		enum dp_bit_rate bit_rate = 0;
		uint8_t cable_flags = 0;
		enum dp21_cable_type cable_type = 0;
		...
		payload[1] = VDO_DP2_1_CFG(DPAM_VERSION_21, cable_type,
					   is_dp_uhbr13_5_supported(port),
					   pin_mode, bit_rate, DP_SINK);
	} else {
		payload[1] = VDO_DP_CFG(pin_mode, /* pin mode */
					DP_HBR3, /* DPv1.3 signaling */
					DP_SINK); /* UFP connected */
	}
...
enum dp_bit_rate dp_get_cable_bit_rate(int port)
{
	...
	if ((product_type == IDH_PTYPE_ACABLE ||
		...
		if (resolve_dpam_version(port, TCPCI_MSG_SOP_PRIME) ==
		    DPAM_VERSION_21) {
			return dp_signaling_to_speed(
				dp_cable_mode_resp.signaling);
		} else if ((resolve_dpam_version(port, TCPCI_MSG_SOP_PRIME) ==
			    DPAM_VERSION_20) ||
			   (tbt_cable_mode_resp.raw_value &&
			    !tbt_cable_mode_resp.retimer_type &&
			    !tbt_cable_mode_resp.tbt_active_passive)) {
