 ...
 */
#define IS_ATTACHED_OR_DBGACC_SRC(port) \
		(get_state_tc(port) == TC_ATTACHED_SRC || \
 ...
 */
#define IS_ATTACHED_OR_DBGACC_SNK(port) \
		(get_state_tc(port) == TC_ATTACHED_SNK || \
...
void pd_update_contract(int port)
{
	if (IS_ENABLED(CONFIG_USB_PE_SM)) {
		if (IS_ATTACHED_OR_DBGACC_SRC(port))
...
void pd_request_source_voltage(int port, int mv)
{
	if (IS_ENABLED(CONFIG_USB_PE_SM)) {
		...
		if (IS_ATTACHED_OR_DBGACC_SNK(port))
...
void pd_request_data_swap(int port)
{
	 ...
	 */
	if (IS_ATTACHED_OR_DBGACC_SRC(port) ||
	    IS_ATTACHED_OR_DBGACC_SNK(port)) {
...
{
	return IS_ATTACHED_OR_DBGACC_SRC(port);
}
...
int tc_is_attached_or_dbgacc_snk(int port)
{
	return IS_ATTACHED_OR_DBGACC_SNK(port);
}
...
void typec_select_drp(int port, int drp)
{
	...
	tc[port].select_drp = drp;
}
void typec_select_pull(int port, enum tcpc_cc_pull pull)
{
	ccprintf("TC%d: select cache-pull=%d\n", port, pull);
...
int typec_update_cc(int port)
{
	...
	polarity = polarity_rm_dts(tc[port].select_polarity);
	...
	if (tc[port].select_drp == tc[port].set_drp &&
	    rp == tc[port].set_rp_value &&
	    polarity == tc[port].set_polarity &&
	    cc1_pull == tc[port].set_cc1_pull &&
	    cc2_pull == tc[port].set_cc2_pull) {
		ccprintf(" - Already Set\n");
		return EC_SUCCESS;
	}

	ccprintf(" - Setting\n");
	tc[port].set_drp = tc[port].select_drp;
	tc[port].set_rp_value = rp;
	tc[port].set_polarity = polarity;
	tc[port].set_cc1_pull = cc1_pull;
	tc[port].set_cc2_pull = cc2_pull;
...
static void tc_dbg_acc_snk_run(const int port)
{
	...
	/*
	 * The sink will be powered off but we don't want to trigger a
	 * disconnect
