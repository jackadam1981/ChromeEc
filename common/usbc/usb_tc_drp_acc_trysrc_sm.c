...
static void tc_detached(int port)
{
	...
	if ((port == 1) && (print_tc_log == 1)) {
		CPRINTS("c1 unattach snk1");
	}
	tc_enable_pd(port, 0);
	tc_pd_connection(port, 0);
	tcpm_debug_accessory(port, 0);
	if ((port == 1) && (print_tc_log == 1)) {
		CPRINTS("c1 unattach snk2");
	}
	set_ccd_mode(port, 0);
...
void tc_event_check(int port, int evt)
{
	...
	//if (/*(port == 1) &&*/ (print_tc_evt_log == 1)) {
		//if (port == 0) {
		//	tc_event[array_index] = 0;
		//} else {
		//	tc_event[array_index] = 1;
		//}
		//array_index++;
		//tc_event[array_index] = evt;
		//array_index++;

	//	LOG_ERR("p%d evt 0x%x", port, evt);
...
static void pd_update_dual_role_config(int port)
{
	if (tc[port].power_role == PD_ROLE_SOURCE &&
		...
		//if ((port == 1) && (print_tc_log == 1)) {
			//LOG_ERR("p%d evt update role set unatta snk ", port);
		//}
		/*
...
__maybe_unused static void handle_new_power_state(int port)
{
	...
	if (IS_ENABLED(CONFIG_AP_POWER_CONTROL) &&
		...
		if (chipset_in_or_transitioning_to_state(
			...
			//if ((port == 1) && (print_tc_log == 1)) {
				//LOG_ERR("p%d evt dpm request mode_exit", port);
			//}
		}
	}
...
{
	//if ((port == 1) && (print_tc_log == 1)) {
		//LOG_ERR("p%d tc attached src exit", port);
	//}
	/*
...
static void pd_chipset_shutdown(void)
{
	...
	//	CPRINTS("tc_event[%d] = 0x%x", ii, tc_event[ii]);
	//	tc_event[ii] = 0;
	//	if ((ii % 2 == 1) && (tc_event[ii + 2] == 0)) {
	//		break;
	//	}
	//}
