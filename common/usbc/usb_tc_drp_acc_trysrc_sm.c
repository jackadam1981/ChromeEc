...
#define TC_FLAGS_PREVIOUS_PD_CONN         BIT(16)
...
DECLARE_STATE(tc, ct_unattached_snk, WITH_RUN, NOOP);
DECLARE_STATE(tc, ct_attached_snk, WITH_RUN, WITH_EXIT);
#endif
...
void pd_request_source_voltage(int port, int mv)
{
	if (IS_ENABLED(CONFIG_USB_PE_SM)) {
		...
		if (tc[port].state_id == TC_ATTACHED_SNK)
...
{
	return TC_CHK_FLAG(port, TC_FLAGS_PREVIOUS_PD_CONN);
}
...
{
	if (en)
		TC_SET_FLAG(port, TC_FLAGS_PREVIOUS_PD_CONN);
	else
		TC_CLR_FLAG(port, TC_FLAGS_PREVIOUS_PD_CONN);
}
...
static int hc_usb_pd_control(struct host_cmd_handler_args *args)
{
	...
	if (args->version == 0) {
		...
		r_v1->enabled =
			...
			(TC_CHK_FLAG(p->port, TC_FLAGS_PREVIOUS_PD_CONN) ?
		...
		strzcpy(r_v1->state, tc_state_names[tc[p->port].state_id],
				sizeof(r_v1->state));
...
static int tc_attached_src_run(int port)
{
	...
	if (tc[port].pd_enable && (prl_get_local_state(port) == SM_RUN)) {
		...
		if (TC_CHK_FLAG(port, TC_FLAGS_CTVPD_DETECTED)) {
			...
			sm_set_state(port, TC_OBJ(port), tc_ct_unattached_snk);
		}
	}
#endif
 ...
 * CTUnattached.SNK
 */
static int tc_ct_unattached_snk(int port, enum sm_signal sig)
{
	int ret;

	ret = (*tc_ct_unattached_snk_sig[sig])(port);
	return SM_SUPER(ret, sig, 0);
}

static int tc_ct_unattached_snk_entry(int port)
{
	tc[port].state_id = TC_CTUNATTACHED_SNK;
	CPRINTS("C%d: %s", port, tc_state_names[tc[port].state_id]);
...
static int tc_ct_unattached_snk_run(int port)
{
	int cc1;
	int cc2;
	int new_cc_state;
	 ...
	 */
	if (pd_is_vbus_present(port))
		sm_set_state(port, TC_OBJ(port), tc_ct_attached_snk);
 ...
 * CTAttached.SNK
 */
static int tc_ct_attached_snk(int port, enum sm_signal sig)
{
	int ret;

	ret = (*tc_ct_attached_snk_sig[sig])(port);
	return SM_SUPER(ret, sig, 0);
}

static int tc_ct_attached_snk_entry(int port)
{
	tc[port].state_id = TC_CTATTACHED_SNK;
	CPRINTS("C%d: %s", port, tc_state_names[tc[port].state_id]);
...
static int tc_ct_attached_snk_run(int port)
{
	/*
	 * Hard Reset is sent when the PE layer is disabled due to a
	 * CTVPD connection.
	 ...
	 */
	if (!pd_is_vbus_present(port))
		return sm_set_state(port, TC_OBJ(port), tc_ct_unattached_snk);
...
static int tc_ct_attached_snk_exit(int port)
{
	/* Stop drawing power */
	sink_stop_drawing_current(port);
