...
__maybe_unused static struct bit_name flag_bit_names[] = {
...
__maybe_unused static struct bit_name event_bit_names[] = {
...
__maybe_unused static void print_bits(const char *desc, int value,
		       struct bit_name *names, int names_size)
{
	int i;

	ASSERT(IS_ENABLED(DEBUG_PRINT_FLAG_AND_EVENT_NAMES));
...
__maybe_unused void print_flag(int set_or_clear, int flag)
{
	ASSERT(IS_ENABLED(DEBUG_PRINT_FLAG_AND_EVENT_NAMES));
...
{
	ASSERT(IS_ENABLED(CONFIG_USB_PD_TRY_SRC));
...
__maybe_unused void pd_dev_dump_info(uint16_t dev_id, uint32_t *hash)
{
	int j;

	ASSERT(IS_ENABLED(CONFIG_CMD_PD_DEV_DUMP_INFO));
 ...
 */
__maybe_unused static bool tc_perform_src_hard_reset(int port)
{
	ASSERT(IS_ENABLED(CONFIG_USB_PE_SM));
 ...
 */
__maybe_unused static bool tc_perform_snk_hard_reset(int port)
{
	ASSERT(IS_ENABLED(CONFIG_USB_PE_SM));
...
{
	if (IS_ENABLED(DEBUG_PRINT_FLAG_AND_EVENT_NAMES) &&
...
__maybe_unused static void handle_new_power_state(int port)
{
	ASSERT(IS_ENABLED(CONFIG_POWER_COMMON));
...
void pd_send_hpd(int port, enum hpd_event hpd)
{
	...
	ASSERT(IS_ENABLED(CONFIG_USB_PD_ALT_MODE) &&
...
{
	ASSERT(IS_ENABLED(CONFIG_USBC_VCONN_SWAP));
...
{
	ASSERT(IS_ENABLED(CONFIG_USB_PD_TCPC_LOW_POWER));
...
{
	ASSERT(IS_ENABLED(CONFIG_USB_PD_DUAL_ROLE_AUTO_TOGGLE));
 ...
 *   Set power role to SOURCE
 */
__maybe_unused static void tc_try_src_entry(const int port)
{
	ASSERT(IS_ENABLED(CONFIG_USB_PD_TRY_SRC));
...
__maybe_unused static void tc_try_src_run(const int port)
{
	enum tcpc_cc_voltage_status cc1, cc2;
	enum pd_cc_states new_cc_state;
 ...
 *   Set power role to SINK
 */
__maybe_unused static void tc_try_wait_snk_entry(const int port)
{
	ASSERT(IS_ENABLED(CONFIG_USB_PD_TRY_SRC));
...
__maybe_unused static void tc_try_wait_snk_run(const int port)
{
	enum tcpc_cc_voltage_status cc1, cc2;
	enum pd_cc_states new_cc_state;
