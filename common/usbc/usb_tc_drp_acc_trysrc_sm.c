...
/*
 * These two bits are used to control PD_ENABLE and TRY_SRC_ENABLE.
 * Both bits must be set for the particular function to be enabled.
 * MASTER_ENABLE is set by console commands and it overrides the
 * ENABLE bit.
 */
...
{
	if (en)
		tc[port].pd_enable |= MASTER_ENABLE;
	else
		tc[port].pd_enable &= ~MASTER_ENABLE;
}
