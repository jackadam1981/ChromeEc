...
{
	strzcpy(state, tc_state_names[get_state_tc(port)], sizeof(state));
}
...
uint8_t pd_get_enabled(int port)
{
	return ((pd_comm_is_enabled(port) ?
		...
		(TC_CHK_FLAG(port, TC_FLAGS_PARTNER_PD_CAPABLE) ?
			PD_CTRL_RESP_ENABLED_PD_CAPABLE : 0));
}

uint8_t pd_get_pwr_and_data_role(int port)
{
	return ((tc[port].power_role ? PD_CTRL_RESP_ROLE_POWER : 0) |
		(tc[port].data_role ? PD_CTRL_RESP_ROLE_DATA : 0) |
		(TC_CHK_FLAG(port, TC_FLAGS_VCONN_ON) ?
			PD_CTRL_RESP_ROLE_VCONN : 0) |
		(TC_CHK_FLAG(port, TC_FLAGS_PARTNER_DR_POWER) ?
			PD_CTRL_RESP_ROLE_DR_POWER : 0) |
		(TC_CHK_FLAG(port, TC_FLAGS_PARTNER_DR_DATA) ?
			PD_CTRL_RESP_ROLE_DR_DATA : 0) |
		(TC_CHK_FLAG(port, TC_FLAGS_PARTNER_USB_COMM) ?
			 PD_CTRL_RESP_ROLE_USB_COMM : 0) |
		(TC_CHK_FLAG(port, TC_FLAGS_PARTNER_EXTPOWER) ?
