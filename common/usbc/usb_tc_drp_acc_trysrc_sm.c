...
void print_flag(int set_or_clear, int flag, int port);
#define TC_SET_FLAG(port, flag)                     \
	do {                                        \
		print_flag(1, flag, port);          \
		atomic_or(&tc[port].flags, (flag)); \
	} while (0)
#define TC_CLR_FLAG(port, flag)                             \
	do {                                                \
		print_flag(0, flag, port);                  \
...
/*
 * 35 ms is enough time for any TCPC transaction to complete
 *
 ...
 */
#define PD_LPM_DEBOUNCE_US (35 * MSEC)
