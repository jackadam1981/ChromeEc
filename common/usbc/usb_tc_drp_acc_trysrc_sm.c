...
{
	/*
	 * If EFS or software sync aren't working in early bring up, then we
	 * want to ensure that we can get higher power.
	 */
	if (IS_ENABLED(CONFIG_BRINGUP))
		return true;

	return system_is_in_rw() || vboot_allow_usb_pd();
}
...
static void tc_cc_open_entry(const int port)
{
	...
	/* We may brown out after appplying CC open, so flush console first. */
