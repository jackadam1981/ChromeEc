...
/* Initialize role toggle counter (fresh cycle) */
static uint8_t drp_sw_toggle_count = 0;
...
static void tc_unattached_src_run(const int port)
{
	...
	if (new_cc_state == PD_CC_UFP_ATTACHED ||
		...
		/* Enforce up to 10 software toggles before LPM/AutoToggle */
		if (drp_sw_toggle_count < 10) {
			drp_sw_toggle_count++;
			set_state_tc(port, TC_UNATTACHED_SNK);
		} else {
