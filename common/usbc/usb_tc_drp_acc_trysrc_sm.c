...
{
	/* This state machine is always from a chromebook */
...
static void tc_attached_snk_run(const int port)
{
	...
	if (tc[port].pd_enable && prl_is_running(port)) {
		...
		if (TC_CHK_FLAG(port, TC_FLAGS_REQUEST_DR_SWAP)) {
			...
			/*
			 * Perform Data Role Swap. Disconnected role is invalid
			 * but would set the data role as SNK default: UFP
			 */
...
static void tc_attached_src_run(const int port)
{
	...
	if (tc[port].pd_enable && prl_is_running(port)) {
		...
		if (TC_CHK_FLAG(port, TC_FLAGS_REQUEST_DR_SWAP)) {
			...
			/*
			 * Perform Data Role Swap. Disconnected role is invalid
			 * but would set the data role as SRC default: DFP
			 */
			tc_set_data_role(port,
