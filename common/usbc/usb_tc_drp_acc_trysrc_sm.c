...
/*
 * While in states TC_DRP_AUTO_TOGGLE or TC_LOW_POWR_MODE,
...
static int tc_drp_and_lpm_next_state(const int port,
		...
		enum tcpc_state_voltage_status cc1,
		enum tcpc_state_voltage_status cc2)
{
	enum pd_drp_next_states next_state;
	int ret;

	/*
	 * Must only be called from states TC_DRP_AUTO_TOGGLE or
	 * TC_LOW_POWER_MODE
	 */
	assert(get_state_tc(port) == TC_DRP_AUTO_TOGGLE ||
