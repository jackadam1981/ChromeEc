...
static inline void pd_set_dual_role_event(int port,
...
{
	pd_set_dual_role_event(port, state, PD_EVENT_UPDATE_DUAL_ROLE);
}
...
void tc_state_init(int port)
{
	...
	if (chipset_in_state(CHIPSET_STATE_ANY_OFF))
		pd_set_dual_role_event(port, PD_DRP_FORCE_SINK, 0);
	else if (chipset_in_state(CHIPSET_STATE_ANY_SUSPEND))
		pd_set_dual_role_event(port, PD_DRP_TOGGLE_OFF, 0);
	else /* CHIPSET_STATE_ON */
		pd_set_dual_role_event(port, PD_DRP_TOGGLE_ON, 0);
