...
static bool tc_perform_src_hard_reset(int port)
{
	...
	case PS_STATE0:
		...
		pd_timer_active(port, TC_TIMER_TIMEOUT, PD_T_SRC_RECOVER);
		return false;
	case PS_STATE1:
		/* Enable VBUS */
		...
		pd_timer_active(port, TC_TIMER_TIMEOUT,
...
static bool tc_perform_snk_hard_reset(int port)
{
	...
	case PS_STATE0:
		...
		tc[port].ps_reset_state = PS_STATE1;
		pd_timer_active(port, TC_TIMER_TIMEOUT, PD_T_SAFE_0V);
		return false;
	case PS_STATE1:
...
static void handle_device_access(int port)
{
	if (IS_ENABLED(CONFIG_USB_PD_TCPC_LOW_POWER) &&
		...
		pd_timer_active(port, TC_TIMER_LOW_POWER_TIME,
				PD_LPM_DEBOUNCE_US);
}
...
static void tc_error_recovery_entry(const int port)
{
	...
	pd_timer_active(port, TC_TIMER_TIMEOUT, PD_T_ERROR_RECOVERY);
}
...
static void tc_error_recovery_run(const int port)
{
	...
	if (!pd_timer_is_expired(port, TC_TIMER_TIMEOUT))
		return;
	pd_timer_inactive(port, TC_TIMER_TIMEOUT);
...
static void tc_unattached_snk_entry(const int port)
{
	...
	pd_timer_active(port, TC_TIMER_NEXT_ROLE_SWAP, PD_T_DRP_SNK);
...
static void tc_unattached_snk_run(const int port)
{
	...
	if (!pd_timer_is_expired(port, TC_TIMER_NEXT_ROLE_SWAP))
		return;
	pd_timer_inactive(port, TC_TIMER_NEXT_ROLE_SWAP);
...
static void tc_unattached_src_entry(const int port)
{
	...
	pd_timer_active(port, TC_TIMER_NEXT_ROLE_SWAP, PD_T_DRP_SRC);
}
...
static void tc_unattached_src_run(const int port)
{
	 ...
	 * but we leave the state before handling it.
	 */
	pd_timer_expired_to_inactive(port, TC_TIMER_NEXT_ROLE_SWAP);
...
static void tc_attached_src_entry(const int port)
{
	...
	/* Run function relies on timeout being inactive or meaningful */
	pd_timer_inactive(port, TC_TIMER_TIMEOUT);
...
static void tc_attached_src_run(const int port)
{
	 ...
	 * turned on
	 */
	if (pd_timer_is_active(port, TC_TIMER_TIMEOUT) &&
...
__maybe_unused static void tc_low_power_mode_entry(const int port)
{
	...
	print_current_state(port);
	pd_timer_active(port, TC_TIMER_LOW_POWER_TIME, PD_LPM_DEBOUNCE_US);
	pd_timer_inactive(port, TC_TIMER_LOW_POWER_EXIT_TIME);
}
...
__maybe_unused static void tc_low_power_mode_run(const int port)
{
	...
	if (TC_CHK_FLAG(port, TC_FLAGS_CHECK_CONNECTION)) {
		tc_start_event_loop(port);
		if (pd_timer_is_inactive(port, TC_TIMER_LOW_POWER_EXIT_TIME)) {
			pd_timer_inactive(port, TC_TIMER_LOW_POWER_TIME);
			pd_timer_active(port, TC_TIMER_LOW_POWER_EXIT_TIME,
...
static void tc_try_src_entry(const int port)
{
	...
	tc[port].cc_state = PD_CC_UNSET;
	pd_timer_active(port, TC_TIMER_TRY_WAIT_DEBOUNCE, PD_T_DRP_TRY);
	pd_timer_active(port, TC_TIMER_TIMEOUT, PD_T_TRY_TIMEOUT);
...
static void tc_try_src_run(const int port)
{
	 ...
	 */
	pd_timer_expired_to_inactive(port, TC_TIMER_CC_DEBOUNCE);
	pd_timer_expired_to_inactive(port, TC_TIMER_TIMEOUT);
...
static void tc_try_wait_snk_entry(const int port)
{
	...
	tc_enable_pd(port, 0);
	tc[port].cc_state = PD_CC_UNSET;
	pd_timer_active(port, TC_TIMER_TRY_WAIT_DEBOUNCE, PD_T_CC_DEBOUNCE);
...
__maybe_unused static void tc_ct_unattached_snk_entry(int port)
{
	...
	pd_timer_active(port, TC_TIMER_TIMEOUT, PD_POWER_SUPPLY_TURN_ON_DELAY);
}
