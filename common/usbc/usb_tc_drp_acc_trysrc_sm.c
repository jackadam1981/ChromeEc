...
static void handle_device_access(int port)
{
	if (IS_ENABLED(CONFIG_USB_PD_TCPC_LOW_POWER)) {
		...
		if (TC_CHK_FLAG(port, TC_FLAGS_LPM_ENGAGED)) {
			...
			/*
			 * Wake to ensure we make another pass through the
			 * main task loop after clearing the flags.
			 */
			task_wake(PD_PORT_TO_TASK_ID(port));
		}
	}
...
static int reset_device_and_notify(int port)
{
	...
	if (IS_ENABLED(CONFIG_USB_PD_TCPC_LOW_POWER)) {
		...
		/*
		 * Before getting the other tasks that are waiting, clear the
		 * reset event from this PD task to prevent multiple reset/init
		 * events occurring.
		 *
		 * The double reset event happens when the higher priority PD
		 * interrupt task gets an interrupt during the above tcpm_init
		 * function. When that occurs, the higher priority task waits
		 * correctly for us to finish waking the TCPC, but it has also
		 * set PD_EVENT_TCPC_RESET again, which would result in a
		 * second, unnecessary init.
		 */
		atomic_clear(task_get_event_bitmap(task_get_current()),
			PD_EVENT_TCPC_RESET);

		waiting_tasks = atomic_read_clear(
				&tc[port].tasks_waiting_on_reset);

		/*
		 * Now that we are done waking up the device, handle device
		 * access manually because we ignored it while waking up
		 * device.
		 */
		handle_device_access(port);

		/*
		 * Clear SW LPM state; the state machine will set it again if
		 * needed
		 */
		TC_CLR_FLAG(port, TC_FLAGS_LPM_REQUESTED);
