...
/* Get the previous TCH state. */
static enum usb_tch_state get_last_state_tch(const int port)
{
	return tch[port].ctx.previous - &tch_states[0];
}
...
{
	/*
	 * NOTE: USB PD ECR, Return to previous state instead of
	 * transitioning to TCH_WAIT_FOR_MESSAGE_REQUEST_FROM_POLICY_ENGINE.
	 */
	set_state_tch(port, get_last_state_tch(port));
}
