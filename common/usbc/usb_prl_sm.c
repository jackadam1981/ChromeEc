...
static timestamp_t tcpc_tx_success_ts;
...
{
	return tcpc_tx_success_ts;
}

/* Sets the time stamp when TCPC reports TX success. */
static void set_tcpc_tx_success_ts(void)
{
	tcpc_tx_success_ts = get_time();
}

void pd_transmit_complete(int port, int status)
{
	if (status == TCPC_TX_COMPLETE_SUCCESS)
		set_tcpc_tx_success_ts();
