...
static void prl_tx_wait_for_phy_response_entry(const int port)
{
	...
	pd_timer_active(port, PR_TIMER_TCPC_TX_TIMEOUT, PD_T_TCPC_TX_TIMEOUT);
}
...
static void prl_tx_src_pending_entry(const int port)
{
	...
	/* Start SinkTxTimer */
	pd_timer_active(port, PR_TIMER_SINK_TX, PD_T_SINK_TX);
}
...
static void prl_hr_wait_for_phy_hard_reset_complete_entry(const int port)
{
	...
	pd_timer_active(port, PR_TIMER_HARD_RESET_COMPLETE,
			PD_T_PS_HARD_RESET);
}
...
static void rch_waiting_chunk_entry(const int port)
{
	 ...
	 */
	pd_timer_active(port, PR_TIMER_CHUNK_SENDER_RESPONSE,
...
static void tch_wait_chunk_request_entry(const int port)
{
	...
	pd_timer_active(port, PR_TIMER_CHUNK_SENDER_REQUEST,
