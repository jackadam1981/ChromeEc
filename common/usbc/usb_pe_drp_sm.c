...
enum usb_pe_state {
	...
	PE_COMMON_XMIT_SEND,
	PE_COMMON_XMIT_WAIT,
	PE_COMMON_XMIT_RESPONSE,
	PE_COMMON_XMIT_RESEND,
	/* Super States */
	PE_PRS_FRS_SHARED,
};
...
static const char * const pe_state_names[] = {
	...
	[PE_COMMON_XMIT_SEND] = "PE_COMMON_XMIT_Send",
	[PE_COMMON_XMIT_WAIT] = "PE_COMMON_XMIT_Wait",
	[PE_COMMON_XMIT_RESPONSE] = "PE_COMMON_XMIT_Response",
	[PE_COMMON_XMIT_RESEND] = "PE_COMMON_XMIT_Resend",
	/* Super States */
...
enum send_swap_t {
	...
	SEND_SWAP_PRS,	/* Send power role swap */
...
static void pe_drs_send_swap_entry(int port)
{
	...
	set_state_pe(port, PE_COMMON_XMIT_SEND);
}
...
static void pe_prs_src_snk_send_swap_entry(int port)
{
	 ...
	 */
	pe[port].retry_counter = 0;
	pe[port].swap_cmd = SEND_SWAP_PRS;
	set_state_pe(port, PE_COMMON_XMIT_SEND);
}
...
static void pe_common_xmit_send_entry(int port)
{
	print_current_state(port);
}

static void pe_common_xmit_send_run(int port)
{
	/*
	...
	set_state_pe(port, PE_COMMON_XMIT_WAIT);
}

static void pe_common_xmit_wait_entry(int port)
{
	print_current_state(port);
}

static void pe_common_xmit_wait_run(int port)
{
	/* Wait until message is sent */
	if (PE_CHK_FLAG(port, PE_FLAGS_TX_COMPLETE)) {
		PE_CLR_FLAG(port, PE_FLAGS_TX_COMPLETE);
		set_state_pe(port, PE_COMMON_XMIT_RESPONSE);
	}
}

static void pe_common_xmit_response_entry(int port)
{
	print_current_state(port);
...
static void pe_common_xmit_response_run(int port)
{
	int type;
	int cnt;
	int ext;
...
static void pe_common_xmit_resend_entry(int port)
{
	print_current_state(port);
...
static void pe_common_xmit_resend_run(int port)
{
	if (get_time().val > pe[port].sender_response_timer)
		set_state_pe(port, PE_COMMON_XMIT_SEND);
...
static const struct usb_state pe_states[] = {
	...
	[PE_COMMON_XMIT_SEND] = {
		.entry = pe_common_xmit_send_entry,
		.run = pe_common_xmit_send_run,
	},
	[PE_COMMON_XMIT_WAIT] = {
		.entry = pe_common_xmit_wait_entry,
		.run = pe_common_xmit_wait_run,
	},
	[PE_COMMON_XMIT_RESPONSE] = {
		.entry = pe_common_xmit_response_entry,
		.run = pe_common_xmit_response_run,
	},
	[PE_COMMON_XMIT_RESEND] = {
		.entry = pe_common_xmit_resend_entry,
		.run = pe_common_xmit_resend_run,
	}
};
