...
void pd_set_vbus_discharge(int port, int enable)
{
	...
	if (IS_ENABLED(CONFIG_USB_PD_DISCHARGE_GPIO)) {
		switch (port) {
#if defined(CONFIG_USB_PD_DISCHARGE_GPIO) && CONFIG_USB_PD_PORT_COUNT >= 3
		case 2:
			gpio_set_level(GPIO_USB_C2_DISCHARGE, enable);
			break;
#endif
#if defined(CONFIG_USB_PD_DISCHARGE_GPIO) && CONFIG_USB_PD_PORT_COUNT >= 2
		case 1:
			gpio_set_level(GPIO_USB_C1_DISCHARGE, enable);
			break;
#endif
#if defined(CONFIG_USB_PD_DISCHARGE_GPIO) && CONFIG_USB_PD_PORT_COUNT >= 1
		case 0:
