...
static void pe_vcs_cbl_send_soft_reset_entry(int port)
{
	...
	if (!pe_can_send_sop_prime(port)) {
		/*
		 * This state should only be entered if we're already
		 * VCONN source.  Either because we started as SRC/DFP or
		 * as SNK we've performed a VCONN swap.  Nothing more to do
		 * if we're not permitted to send SOP'.
		 */
		pe_set_ready_state(port);
		return;
	}
