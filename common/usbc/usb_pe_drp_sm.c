...
static void pe_bist_tx_entry(int port)
{
	...
	/* Ignore BIST messages when not operation at vSafe5V */
	if (pe[port].power_role == PD_ROLE_SOURCE) {
		const uint32_t *src_pdo;
	...
	CPRINTS("pe[%d]: BIST tx entry: vbus = %d mV", port, vbus_mv);

	if (vbus_mv != PD_V_SAFE5V_NOM) {
		CPRINTS("pe[%d]: BIST tx entry: Ignore bist request!", port);
		pe_set_ready_state(port);
		return;
	}
