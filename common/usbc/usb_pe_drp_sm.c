 ...
 * FRS requirements (because it is not a fixed PDO) or PD 3.0 support is not
 * enabled, do nothing.
 */
static void pe_evaluate_frs_snk_pdo(int port, uint32_t pdo)
{
	if (!IS_ENABLED(CONFIG_USB_PD_REV30))
		return;
...
static void pe_snk_ready_entry(int port)
{
	...
	if (!PE_CHK_FLAG(port, PE_FLAGS_FAST_ROLE_SWAP_ENABLED)) {
		if (pd_get_snk_cap_cnt(port) > 0) {
			 ...
			 * in fixed PDOs, which shall be the first PDO returned.
			 */
