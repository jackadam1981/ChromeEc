...
#define PE_CLR_FLAG(port, flag) \
	deprecated_atomic_clear_bit(&pe[port].flags, (flag))
...
#define PE_CLR_DPM_REQUEST(port, req) \
	deprecated_atomic_clear_bit(&pe[port].dpm_request, (req))
...
{
	deprecated_atomic_clear_bit(&task_access[port][type], 0xFFFFFFFF);
}
