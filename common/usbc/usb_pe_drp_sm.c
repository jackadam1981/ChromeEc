...
{
	int response_size = 0; /* in unit of byte */
	uint32_t *rx_payload;
	uint32_t *tx_payload;
	uint8_t vdo_cmd;
	int cmd_type;
	...
	if (func) {
		 ...
		 * buffer to take received data and overwrite with reponse
		 * data. To work with this interface, here copy rx data to
		 * tx buffer and pass tx_payload to func.
		 * TODO: change the interface to pass both rx and tx buffer
		 */
		 ...
		 */
		response_size = func(port, tx_payload) * sizeof(*tx_payload);
		if (response_size)
			/* ACK */
			tx_payload[0] = VDO(
				USB_VID_GOOGLE,
	...
	tx_emsg[port].len = response_size;
