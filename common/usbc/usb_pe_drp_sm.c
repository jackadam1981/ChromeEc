...
static void pe_set_frs_enable(int port, int enable)
{
	if (IS_ENABLED(CONFIG_USB_TYPEC_PD_FAST_ROLE_SWAP)) {
		...
		if ((current && !enable) || (!current && enable)) {
			/*
			 * The actual enable/disable is done in tcpc and/or ppc
			 * so request it from both.
			 */
...
/* DENIS: Somewhere near here I should check the SNK capability of the
 * SRC to save if we can handle FRS requirements
 */
