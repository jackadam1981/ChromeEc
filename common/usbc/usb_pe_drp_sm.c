...
static enum pe_msg_check pe_sender_response_msg_run(const int port)
{
	if (pd_timer_is_disabled(port, PE_TIMER_SENDER_RESPONSE)) {
		...
		if (PE_CHK_FLAG(port, PE_FLAGS_TX_COMPLETE)) {
			...
			pd_timer_active(port, PE_TIMER_SENDER_RESPONSE,
	 ...
	 */
	pd_timer_expired_to_inactive(port, PE_TIMER_SENDER_RESPONSE);
...
static void pe_src_startup_entry(int port)
{
	...
	if (PE_CHK_FLAG(port, PE_FLAGS_PR_SWAP_COMPLETE)) {
		...
		pd_timer_active(port, PE_TIMER_SWAP_SOURCE_START,
		 ...
		 */
		pd_timer_inactive(port, PE_TIMER_SWAP_SOURCE_START);
...
static void pe_src_startup_run(int port)
{
	 ...
	 * but we leave the state before handling it.
	 */
	pd_timer_expired_to_inactive(port, PE_TIMER_SWAP_SOURCE_START);
...
static void pe_src_discovery_run(int port)
{
	 ...
	 * but we leave the state before handling it.
	 */
	pd_timer_expired_to_inactive(port, PE_TIMER_NO_RESPONSE);
...
static void pe_src_transition_supply_run(int port)
{
	...
	if (PE_CHK_FLAG(port, PE_FLAGS_TX_COMPLETE)) {
		...
		if (PE_CHK_FLAG(port, PE_FLAGS_PS_READY)) {
			...
			pd_timer_active(port, PE_TIMER_SRC_TRANSITION,
					PD_T_SRC_TRANSITION);
		}
...
static void pe_src_ready_run(int port)
{
	 ...
	 * but we leave the state before handling it.
	 */
	pd_timer_expired_to_inactive(port, PE_TIMER_WAIT_AND_ADD_JITTER);
...
static void pe_src_hard_reset_entry(int port)
{
	...
	pd_timer_active(port, PE_TIMER_NO_RESPONSE, PD_T_NO_RESPONSE);

	/* Start PSHardResetTimer */
	pd_timer_active(port, PE_TIMER_PS_HARD_RESET, PD_T_PS_HARD_RESET);
...
static void pe_snk_transition_sink_entry(int port)
{
	...
	pd_timer_active(port, PE_TIMER_PS_TRANSITION, PD_T_PS_TRANSITION);
}
...
static void pe_snk_ready_run(int port)
{
	 ...
	 * but we leave the state before handling it.
	 */
	pd_timer_expired_to_inactive(port, PE_TIMER_SINK_REQUEST);
	pd_timer_expired_to_inactive(port, PE_TIMER_WAIT_AND_ADD_JITTER);
...
__maybe_unused static void pe_chunk_received_entry(int port)
{
	...
	print_current_state(port);
	pd_timer_active(port, PE_TIMER_CHUNKING_NOT_SUPPORTED,
...
static void pe_bist_tx_entry(int port)
{
	...
	if (mode == BIST_CARRIER_MODE_2) {
		...
		pd_timer_active(port, PE_TIMER_BIST_CONT_MODE,
				PD_T_BIST_CONT_MODE);
...
static void pe_vdm_send_request_run(int port)
{
	if (PE_CHK_FLAG(port, PE_FLAGS_TX_COMPLETE) &&
		...
		pd_timer_active(port, PE_TIMER_VDM_RESPONSE,
				PD_T_VDM_SNDR_RSP);
	}
...
static void pe_vcs_wait_for_vconn_swap_entry(int port)
{
	...
	pd_timer_active(port, PE_TIMER_VCONN_ON, PD_T_VCONN_SOURCE_ON);
}
