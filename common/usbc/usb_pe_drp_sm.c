...
static void pe_src_startup_entry(int port)
{
	...
	if (PE_CHK_FLAG(port, PE_FLAGS_PR_SWAP_COMPLETE)) {
		...
		/*
		 * Request partners revision information. The PE_Get_Revision
	...
	if ((msg_check & PE_MSG_SENT) &&
		...
		if (ext == 0 && cnt == 1) {
			if (type == PD_DATA_REVISION) {
				/* RMDO Received from partner */
				uint32_t *rmdo = (uint32_t *)rx_emsg[port].buf;

				pe[port].partner_rmdo = RMDO_GET_DATA(*rmdo);
			}
			/* Return if response is not a Revision data message */
			pe_set_ready_state(port);
			return;
		}

		/* Soft reset on extedned or count pd header mismatch */
		pe_send_soft_reset(port, sop);
		return;
	}

	/*
	 * Return to ready state if message is discarded or there is a protocol
	 * error.
	 */
	if ((msg_check & PE_MSG_DISCARDED) ||
	     PE_CHK_FLAG(port, PE_FLAGS_PROTOCOL_ERROR))
