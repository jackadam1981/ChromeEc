...
static void pe_snk_apply_psnkstdby(int port)
{
	...
	/*
	 * Apply 2.5W ceiling during transition. Even if the active port
	 * is different, conform to the spec as long as it's a PD port.
	 */
	if (charge_manager_get_supplier() == CHARGE_SUPPLIER_PD)
		high = MAX(charge_manager_get_charger_voltage(), mv);
	else
		high = mv;
	charge_manager_force_ceil(port,
			high > 0 ? PD_SNK_STDBY_MW * 1000 / high : PD_MIN_MA);
}
