...
{
	return pd_get_requested_voltage(port) <= PD_REV3_MAX_VOLTAGE;
}
...
static bool sink_dpm_requests(int port)
{
	...
	if (pe[port].dpm_request) {
		...
		if (PE_CHK_DPM_REQUEST(port, DPM_REQUEST_PR_SWAP)) {
			...
			pd_request_source_voltage(port, PD_MAX_EPR_VOLTAGE);
			 ...
			 * Send an RDO with EPR mode capable bit (b22) unset to
			 * negotiate an SPR contract.
			 * Keep PE_SNK_SEND_EPR_MODE_EXIT so that we can retry.
			 */
			CPRINTS("C%d: Request SPR before EPR exit", port);
			pd_request_source_voltage(port, PD_REV3_MAX_VOLTAGE);
			pe_set_dpm_curr_request(port,
						DPM_REQUEST_NEW_POWER_LEVEL);
			set_state_pe(port, PE_SNK_SELECT_CAPABILITY);
			return true;
...
static void pe_send_request_msg(int port)
{
	...
	if (IS_ENABLED(CONFIG_USB_PD_EPR) && pe_snk_in_epr_mode(port)) {
		...
		tx_emsg[port].len = 8;
		/* Skip setting EPR_MODE_CAPABLE when exiting EPR mode. */
		if (supply_voltage > PD_MAX_EPR_VOLTAGE)
			rdo |= RDO_EPR_MODE_CAPABLE;
...
static void pe_snk_epr_keep_alive_run(int port)
{
	...
	if (msg_check & PE_MSG_SENT &&
		...
		if (ecdb->type == PD_EXT_CTRL_EPR_KEEPALIVE_ACK) {
			pe_sender_response_msg_exit(port);
			set_state_pe(port, PE_SNK_READY);
		}
...
/*
 * 8.3.3.27.2 of USB PD R3.1 V1.6
 * Sink EPR Mode Entry States
 */
...
/*
 * 8.3.3.27.4 of USB PD R3.1 V1.6
 * Sink EPR Mode Exit States
 */
...
static void pe_snk_epr_mode_exit_received_entry(int port)
{
	...
	/*
	 * Table 8-22 Steps for Exiting EPR Mode (Source Initiated) states
	 * 'The Port Partners are in an Explicit Contract using an SPR PDO.'
	 * Thus, it's expected Source has sent new SPR PDOs (and we switched
	 * to a SPR contract already) before it sent EPR mode exit.
	 */
	if (!pe_in_spr_contract(port)) {
		CPRINTS("C%d: Received EPR mode exit before SPR", port);
		set_state_pe(port, PE_SNK_READY);
		return;
	}
