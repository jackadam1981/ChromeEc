...
#endif
#define CONFIG_USB_PD_DEBUG_LEVEL
...
#define PE_FLAGS_DISCOVER_CABLE_IDENTITY_DONE   BIT(17)
...
enum vdm_discover_state {
	VDM_WAIT_FOR_SEND,
	VDM_WAIT_FOR_RESPONSE,
	VDM_DO_NOTHING
};
...
static enum l_state local_state[CONFIG_USB_PD_PORT_COUNT] = {SM_INIT};
...
static struct policy_engine {
	...
	enum partner is_cable;
	enum vdm_discover_state vdm_discover_state;

	enum sub_state sub;
	int saved_power_role;
	int saved_data_role;
...
DECLARE_STATE(pe, src_vdm_identity, WITH_RUN, NOOP);
#if 0
NOTE: See state implementation
DECLARE_STATE(pe, src_get_sink_cap, WITH_RUN, NOOP);
#endif
...
/* Initiator to Port VDM Discover Identity States */
DECLARE_STATE(pe, handle_svdm_request, WITH_RUN, WITH_EXIT);
DECLARE_STATE(pe, handle_custom_vdm_request, WITH_RUN, WITH_EXIT);
...
{
	return local_state[port] == PE_RUN;
}
...
void pe_send_vdm(int port, uint32_t vid, int cmd, const uint32_t *data,
						int count)
{
	pe[port].is_cable = PORT;
	pe[port].vdm_vid = vid;
	pe[port].vdm_cnt = count;
	pe[port].vdm_cmd = cmd;

	if (count > 0)
		memcpy(pe[port].vdm_data, data, count);
...
static void pe_prl_execute_hard_reset(int port)
{
	...
	PE_SET_FLAG(port, PE_FLAGS_HARD_RESET_PENDING);
}
...
static int pe_src_startup_entry(int port)
{
	 ...
	 *
	 * NOTE: This delay also gives time for a Cable and VCONN
	 *       Powered device to startup before sending a discover
	 *       identity message, so it is run every time
...
static int pe_src_startup_run(int port)
{
	...
	if (get_time().val > pe[port].swap_source_start_timer)
		sm_set_state(port, PE_OBJ(port), pe_src_vdm_identity);
 ...
 * PE_SRC_VDM_Identity
 */
static int pe_src_vdm_identity(int port, enum sm_signal sig)
{
	int ret;

	ret = (*pe_src_vdm_identity_sig[sig])(port);
	return SM_SUPER(ret, sig, 0);
}

static int pe_src_vdm_identity_entry(int port)
{
	DEBUG_PRINTF("C%d: PE_SRC_VDM_IDENTITY", port);

	/* Set state id */
	pe[port].state_id = PE_SRC_VDM_IDENTITY;

	/*
	 * The time between the application of
	 * VCONN until SOP’ and SOP” shall be ready
	 * for communication.
	 */
	pe[port].timeout = get_time().val + PD_T_VCONN_STABLE;
	return 0;
}

static int pe_src_vdm_identity_run(int port)
{
	/* Wait until cable plug is ready to communicate */
	 ...
	 */
	if (!PE_CHK_FLAG(port, PE_FLAGS_DISCOVER_CABLE_IDENTITY_DONE) &&
		pe[port].cable_discover_identity_count <
					N_DISCOVER_IDENTITY_COUNT) {
		pe[port].cable_discover_identity_count++;

		pe[port].is_cable = CABLE;
		pe[port].vdm_vid = USB_SID_PD;
		pe[port].vdm_cnt = 0;
		pe[port].vdm_cmd = DISCOVER_IDENTITY;

		return sm_set_state(port, PE_OBJ(port), pe_handle_svdm_request);
	}
...
/* NOTE: DO WE NEED THIS STATE? */

/**
 * PE_SRC_Get_Sink_Cap
 */
static int pe_src_get_sink_cap(int port, enum sm_signal sig)
{
	int ret;

	ret = (*pe_src_get_sink_cap_sig[sig])(port);
	return SM_SUPER(ret, sig, 0);
}

static int pe_src_get_sink_cap_entry(int port)
{
	DEBUG_PRINTF("C%d: PE_SRC_GET_SINK_CAP", port);

	/* Set state id */
	pe[port].state_id = PE_SRC_GET_SINK_CAP;

	/*
	 * Request the Protocol Layer to send a Get_Sink_Cap
	 * Message in order to retrieve the Sink’s capabilities.
	 */
...
static int pe_src_get_sink_cap_run(int port)
{
	...
	if (PE_CHK_FLAG(port, PE_FLAGS_MSG_RECEIVED)) {
		...
		/*
		 * Sink Cap Message Received?
		 */
		if (PD_HEADER_CNT(emsg[port].header) > 0 &&
		PD_HEADER_TYPE(emsg[port].header) == PD_DATA_SINK_CAP) {

			/* HOW SHOULD THIS DATA BE HANDLED? */
			/* Inform the Device Policy Manager of capabilities. */
	...
	if (pe[port].sender_response_timer > 0 &&
		...
		/* HOW SHOULD THIS DATA BE HANDLED? */
		/* Inform the Device Policy Manager of response timeout. */
...
static int pe_src_discovery_entry(int port)
{
	 ...
	 * to PE_SRC_Discovery.
	 */
	if (pe[port].obj.last_state != pe_handle_svdm_request)
...
static int pe_src_discovery_run(int port)
{
	...
	pe[port].cable_discover_identity_count < N_DISCOVER_IDENTITY_COUNT) {
		...
		sm_set_state(port, PE_OBJ(port), pe_handle_svdm_request);
	}
...
static int pe_src_ready_entry(int port)
{
	...
	if (!PE_CHK_FLAG(port, PE_FLAGS_MODAL_OPERATION) &&
		!PE_CHK_FLAG(port, PE_FLAGS_DISCOVER_PORT_IDENTITY_DONE)) {
		pe[port].port_discover_identity_count = 0;
...
static int pe_src_ready_run(int port)
{
...
#if 0
		/* NOTE: See state implementation */
		else if ((PE_CHK_DPM_REQUEST(port, DPM_REQUEST_SINK_CAP)) {
			PE_CLR_DPM_REQUEST(port, DPM_REQUEST_SINK_CAP);
			sm_set_state(port, PE_OBJ(port), pe_src_get_sink_cap);
		}
#endif
		else if (PE_CHK_DPM_REQUEST(port,
	...
	if (PE_CHK_FLAG(port, PE_FLAGS_SEND_SVDM)) {
		PE_CLR_FLAG(port, PE_FLAGS_SEND_SVDM);
		return sm_set_state(port, PE_OBJ(port), pe_handle_svdm_request);
	}
	...
	if (PE_CHK_FLAG(port, PE_FLAGS_MSG_RECEIVED)) {
		...
		/* Data Message Requests */
			...
			case PD_DATA_VENDOR_DEF:
				/*
				 * Re-Set the received flag so
				 * PE_Handle_SVDM_Request state
				 * can handle the received data.
				 * This will be cleared when
				 * pe_handle_svdm_request exits.
				 */
				PE_SET_FLAG(port, PE_FLAGS_MSG_RECEIVED);
...
static int pe_snk_startup_entry(int port)
{
	...
	/*
	 * Clear port discovery identity done flag.
	 * Clear explicit contract.
	 */
...
static int pe_handle_custom_vdm_request_exit(int port)
{
 ...
 *   PE_RESP_VDM_Get_Send_Modes
 */
static int pe_handle_svdm_request(int port, enum sm_signal sig)
{
	int ret;

	ret = (pe_handle_svdm_request_sig[sig])(port);
	return SM_SUPER(ret, sig, 0);
}

static int pe_handle_svdm_request_entry(int port)
{
	DEBUG_PRINTF("C%d: PE_HANDLE_SVDM_REQUEST\n", port);

	/* Set state id */
	pe[port].state_id = PE_HANDLE_SVDM_REQUEST;

	/* This is an Interruptible AMS */
	PE_SET_FLAG(port, PE_FLAGS_INTERRUPTIBLE_AMS);
	/*
	 * This is a request to discover the identity of the cable or
	 * port partner.
	 */
	if (pe[port].vdm_cmd == DISCOVER_IDENTITY) {
		if (pe[port].is_cable) {
			/* Initialize VDOs to default values */
			pe[port].active_cable_vdo1 = -1;
			pe[port].active_cable_vdo2 = -1;
			...
			/*
			 * Inform the Device Policy Manager that
			 * the Discover Identity is in progress.
			 */
			tc_disc_ident_in_progress(port);
		}
		 ...
		 * message buffer
		 */
		*(uint32_t *)emsg[port].buf =
			VDO(pe[port].vdm_vid,
			((pe[port].vdm_vid & USB_SID_PD) == USB_SID_PD) ?
				1 : (PD_VDO_CMD(pe[port].vdm_cmd) <=
							CMD_ATTENTION),
				VDO_SVDM_VERS(1) | pe[port].vdm_cmd);
		/* Update len with number of VDM Header bytes */
		emsg[port].len = 4;

		/* Copy Vendor Data Objects (VDOs) into message buffer */
		if (pe[port].vdm_cnt > 0) {
			/* Copy data after header */
			memcpy(&emsg[port].buf[emsg[port].len],
		...
		if (pe[port].is_cable) {
			/*
			 * When sending a message to a cable plug,
			 * the power role should be set to PD_PLUG_DFP_UFP.
			 */
			tc_set_power_role(port, PD_PLUG_DFP_UFP);
			tc_set_data_role(port, 0);

			/* Send message to cable plug */
			prl_send_data_msg(port, TCPC_TX_SOP_PRIME,
							PD_DATA_VENDOR_DEF);
		} else {
			prl_send_data_msg(port, TCPC_TX_SOP,
							PD_DATA_VENDOR_DEF);
		}

		pe[port].vdm_discover_state = VDM_WAIT_FOR_SEND;
		pe[port].vdm_cmd = 0;
	} else {
		pe[port].vdm_discover_state = VDM_DO_NOTHING;
	}

	return 0;
}

static int pe_handle_svdm_request_run(int port)
{
	int ret = 0;
	uint32_t *payload;
	int cmd_type;
	...
	switch (pe[port].vdm_discover_state) {
	case VDM_WAIT_FOR_SEND:
		if (PE_CHK_FLAG(port, PRL_TX_RX_SIGNAL)) {
			PE_CLR_FLAG(port, PE_FLAGS_TX_COMPLETE);
			...
			pe[port].vdm_discover_state = VDM_WAIT_FOR_RESPONSE;
			pe[port].vdm_response_timer = get_time().val +
					PD_T_VDM_SNDR_RSP;
		} else if (PE_CHK_FLAG(port, PE_FLAGS_PROTOCOL_ERROR)) {
			PE_CLR_FLAG(port, PE_FLAGS_PROTOCOL_ERROR);
			...
			tc_set_power_role(port, pe[port].saved_power_role);
			tc_set_data_role(port, pe[port].saved_data_role);
			if (pe[port].obj.last_state == pe_src_vdm_identity) {
				sm_set_state(port, PE_OBJ(port),
							pe_src_vdm_identity);
		...
		return 0;
	case VDM_WAIT_FOR_RESPONSE:
		if (get_time().val > pe[port].vdm_response_timer) {
			CPRINTF("VDM %s Response Timeout\n",
					pe[port].is_cable ? "Cable" : "Port");
			/*
			 * Set PE_FLAGS_DISCOVER_CABLE_IDENTITY_DONE or
			 * PE_FLAGS_DISCOVER_PORT_IDENTITY_DONE flags so
			 * returning state doesn't try to detect the cable
			 * again
			 */
			if (pe[port].is_cable)
				PE_SET_FLAG(port,
	 ...
	 * PE_FLAG_MSG_RECEIVED will be cleared on exit.
	 */
	if (!PE_CHK_FLAG(port, PE_FLAGS_MSG_RECEIVED))
		return 0;
	...
	/* Did this message come from the cable plug? */
	if (sop > TCPC_TX_SOP) {
		if (sop == TCPC_TX_SOP_PRIME) {
			...
			if (PD_VDO_CMD(vdm_header) == CMD_DISCOVER_IDENT &&
				...
				if (PD_VDO_CMDT(vdm_header) == CMDT_RSP_ACK) {
			...
			/*
			 * Set PE_FLAGS_DISCOVER_CABLE_IDENTITY_DONE flag so
			 * returning state doesn't try to detect the cable
			 * again
			 */
			PE_SET_FLAG(port,
		...
		if (pe[port].obj.last_state == pe_src_vdm_identity) {
			sm_set_state(port, PE_OBJ(port), pe_src_vdm_identity);
		} else if (pe[port].obj.last_state == pe_src_discovery) {
			sm_set_state(port, PE_OBJ(port), pe_src_discovery);
		} else {
			if (pe[port].power_role == PD_ROLE_SOURCE)
				sm_set_state(port, PE_OBJ(port), pe_src_ready);
			else
	...
	/*
	 * Message came from port partner
	 */
	...
	if (cmd_type == CMDT_INIT) {
		...
		/* Set ret to 0 to prevent another message send below */
		ret = 0;
		...
		case CMD_DISCOVER_IDENT:
			...
			ret = dfp_discover_svids(port, payload);
#ifdef CONFIG_CHARGE_MANAGER
		...
		case CMD_DISCOVER_SVID:
			...
			ret = dfp_discover_modes(port, payload);
			break;
		case CMD_DISCOVER_MODES:
		...
		case CMD_DP_STATUS:
			...
			dfp_consume_attention(port, payload);
			if (modep && modep->opos)
				ret = modep->fx->config(port, payload);
			else
				ret = 0;
			break;
		case CMD_DP_CONFIG:
		...
		/*
		 * Set PE_FLAGS_DISCOVER_PORT_IDENTITY_DONE flag so returning
		 * state doesn't try to detect the port again.
		 */
...
static int pe_handle_svdm_request_exit(int port)
{
	/* clear receive flag */
	PE_CLR_FLAG(port, PE_FLAGS_MSG_RECEIVED);

	/*
	 * If a CTVPD device was not discovered, inform the
	 * Device Policy Manager that the Discover Identity
	 * is done.
	 *
	 * If a CTVPD device is discovered, the Device Policy
	 * Manager will clear the DISC_IDENT flag set by
	 * tc_disc_ident_in_progress.
	 */
	if (pe[port].vpd_vdo < 0 || !VPD_VDO_CTS(pe[port].vpd_vdo))
...
{
	memset(&pe[port].am, 0, sizeof(struct pd_policy));
}
...
static void dfp_consume_identity(int port, int cnt, uint32_t *payload)
{
	...
	size_t identity_size = MIN(sizeof(pe[port].am.identity),
				(cnt - 1) * sizeof(uint32_t));

	pd_dfp_pe_init(port);
	memcpy(&pe[port].am.identity, payload + 1, identity_size);
...
static void dfp_consume_svids(int port, int cnt, uint32_t *payload)
{
	...
	for (i = pe[port].am.svid_cnt; i < pe[port].am.svid_cnt + 12; i += 2) {
		...
		if (!svid0)
			break;
		pe[port].am.svids[i].svid = svid0;
		pe[port].am.svid_cnt++;

		svid1 = PD_VDO_SVID_SVID1(*ptr);
		if (!svid1)
			break;
		pe[port].am.svids[i + 1].svid = svid1;
		pe[port].am.svid_cnt++;
		ptr++;
		vdo++;
	}
...
static int dfp_discover_modes(int port, uint32_t *payload)
{
	uint16_t svid = pe[port].am.svids[pe[port].am.svid_idx].svid;

	if (pe[port].am.svid_idx >= pe[port].am.svid_cnt)
		return 0;
...
static void dfp_consume_modes(int port, int cnt, uint32_t *payload)
{
	int idx = pe[port].am.svid_idx;

	pe[port].am.svids[idx].mode_cnt = cnt - 1;

	if (pe[port].am.svids[idx].mode_cnt < 0) {
		CPRINTF("ERR:NOMODE\n");
	} else {
		memcpy(pe[port].am.svids[pe[port].am.svid_idx].mode_vdo,
			&payload[1],
			sizeof(uint32_t) * pe[port].am.svids[idx].mode_cnt);
	}
...
static int get_mode_idx(int port, uint16_t svid)
{
	...
	for (i = 0; i < PD_AMODE_COUNT; i++) {
		if (pe[port].am.amodes[i].fx->svid == svid)
			return i;
	}
...
static struct svdm_amode_data *get_modep(int port, uint16_t svid)
{
	...
	return (idx == -1) ? NULL : &pe[port].am.amodes[idx];
}
...
int allocate_mode(int port, uint16_t svid)
{
	...
	if (pe[port].am.amode_idx == PD_AMODE_COUNT) {
	...
	for (i = 0; i < supported_modes_cnt; i++) {
		...
		for (j = 0; j < pe[port].am.svid_cnt; j++) {
			struct svdm_svid_data *svidp = &pe[port].am.svids[j];

			if ((svidp->svid != supported_modes[i].svid) ||
					(svid && (svidp->svid != svid)))
				continue;

			modep = &pe[port].am.amodes[pe[port].am.amode_idx];
			modep->fx = &supported_modes[i];
			modep->data = &pe[port].am.svids[j];
			pe[port].am.amode_idx++;
			return pe[port].am.amode_idx - 1;
		}
	}
	return -1;
}
...
uint32_t pd_dfp_enter_mode(int port, uint16_t svid, int opos)
{
	...
	modep = &pe[port].am.amodes[mode_idx];
...
int pd_dfp_exit_mode(int port, uint16_t svid, int opos)
{
	...
	if (!svid) {
		for (idx = 0; idx < PD_AMODE_COUNT; idx++)
			if (pe[port].am.amodes[idx].fx)
				pe[port].am.amodes[idx].fx->exit(port);
...
{
	return PD_IDH_VID(pe[port].am.identity[0]);
}

uint16_t pd_get_identity_pid(int port)
{
	return PD_PRODUCT_PID(pe[port].am.identity[2]);
}
...
static void dump_pe(int port)
{
	...
	if (pe[port].am.identity[0] == 0) {
		ccprintf("No identity discovered yet.\n");
		return;
	}
	idh_ptype = PD_IDH_PTYPE(pe[port].am.identity[0]);
	ccprintf("IDENT:\n");
	ccprintf("\t[ID Header] %08x :: %s, VID:%04x\n",
				pe[port].am.identity[0],
				idh_ptype_names[idh_ptype],
				pd_get_identity_vid(port));
	ccprintf("\t[Cert Stat] %08x\n", pe[port].am.identity[1]);
	for (i = 2; i < ARRAY_SIZE(pe[port].am.identity); i++) {
		ccprintf("\t");
		if (pe[port].am.identity[i])
			ccprintf("[%d] %08x ", i, pe[port].am.identity[i]);
	}
	ccprintf("\n");

	if (pe[port].am.svid_cnt < 1) {
		ccprintf("No SVIDS discovered yet.\n");
		return;
	}

	for (i = 0; i < pe[port].am.svid_cnt; i++) {
		ccprintf("SVID[%d]: %04x MODES:", i, pe[port].am.svids[i].svid);
		for (j = 0; j < pe[port].am.svids[j].mode_cnt; j++)
			ccprintf(" [%d] %08x", j + 1,
			 pe[port].am.svids[i].mode_vdo[j]);
		ccprintf("\n");
		modep = get_modep(port, pe[port].am.svids[i].svid);
		if (modep) {
			mode_caps = modep->data->mode_vdo[modep->opos - 1];
...
static int hc_remote_pd_discovery(struct host_cmd_handler_args *args)
{
	...
	r->ptype = PD_IDH_PTYPE(pe[*port].am.identity[0]);

	/* pid only included if vid is assigned */
	if (r->vid)
		r->pid = PD_PRODUCT_PID(pe[*port].am.identity[2]);
...
static int hc_remote_pd_get_amode(struct host_cmd_handler_args *args)
{
	...
	/* no more to send */
	if (p->svid_idx >= pe[p->port].am.svid_cnt) {
		r->svid = 0;
		args->response_size = sizeof(r->svid);
		return EC_RES_SUCCESS;
	}

	r->svid = pe[p->port].am.svids[p->svid_idx].svid;
	r->opos = 0;
	memcpy(r->vdo, pe[p->port].am.svids[p->svid_idx].mode_vdo, 24);
