...
static struct policy_engine {
	...
	/* port events - set from PD task but may be cleared by host command */
	uint32_t events;
...
{
	atomic_clear(&pe[port].events, clear_mask);
}
...
static void pe_init_vdm_svids_request_run(int port)
{
	...
	case VDM_RESULT_NAK:
		...
		pe_notify_event(port, pe[port].tx_type == TCPC_TX_SOP_PRIME ?
...
static void pe_init_vdm_modes_request_exit(int port)
{
	if (pd_get_modes_discovery(port, pe[port].tx_type) != PD_DISC_NEEDED)
		/* Mode discovery complete, notify the AP */
		pe_notify_event(port, pe[port].tx_type == TCPC_TX_SOP_PRIME ?
				PD_STATUS_EVENT_SOP_DISC_DONE :
				PD_STATUS_EVENT_SOP_PRIME_DISC_DONE);
