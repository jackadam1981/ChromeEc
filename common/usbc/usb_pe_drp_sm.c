						...
						int count)
{
	...
	/*
	 * Copy Data after VDM Header. Note that the count that's passed in is a
	 * VDO count and each VDO is 4 bytes in length.
	 */
...
static void pe_vdm_response_entry(int port)
{
	...
	/*
	 * The SVDM header is dependent on VDM command request. The SVDM command
	 * message was copied into tx_payload. tx_payload[0] is the VDM header
	 ...
	 * <12:11>  :: reserved
	 * <10:8>   :: object position (1-7 used for enter/exit mode only)
	...
	if (func) {
		/*
		 * Call the SVID response handler. The return value vdo_len
		 * contains number of VDO objects in the response. This value
		 * also encodes whether the VDM command is going to be ack'd,
		 ...
		 *
		 * vdo_len encodes the response type as follows:
		 *    vdo_len > 0  --> ACK
		 *    vdo_len == 0 --> NAK
		 *    vdo_len < 0  --> BUSY
		 */
...
static void pe_vdm_response_run(int port)
{
	 ...
	 * inerrupted by another rx message. Since VDM sequences are AMS
