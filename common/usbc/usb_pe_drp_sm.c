...
__maybe_unused static void pe_give_sink_cap_ext_entry(int port)
{
	...
	skedb.battery_info = IS_ENABLED(CONFIG_BATTERY) ? 1 : 0;
	skedb.sink_modes = SKEDB_SM_VBUS_POWERED;
#ifdef CONFIG_BATTERY
	skedb.sink_modes |= SKEDB_SM_BATTERY_POWERED;
#endif
#if CONFIG_DEDICATED_CHARGE_PORT_COUNT > 0
	skedb.sink_modes |= SKEDB_SM_MAINS_POWERED;
#endif
