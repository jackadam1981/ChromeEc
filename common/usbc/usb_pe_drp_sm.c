...
int pe_set_ado(int port, uint32_t data)
{
	...
	if ((port == 1) && (print_tc_log == 1)) {
		CPRINTS("c1 set ado_lock 0x%x, INT %d", pe[port].ado_lock.lock_count, tt);
	}
	...
	if ((port == 1) && (print_tc_log == 1)) {
		CPRINTS("c1 set ado_lock 0x%x", pe[port].ado_lock.lock_count);
	}
	//irq_unlock(key);
	return ret;
}
...
void pe_clear_ado(int port)
{
	...
	if ((port == 1) && (print_tc_log == 1)) {
		CPRINTS("c1 clr ado_lock 0x%x, INT %d", pe[port].ado_lock.lock_count, tt);
	}
	mutex_lock(&pe[port].ado_lock);
	//key = irq_lock();
	pe[port].ado = 0x0;
	mutex_unlock(&pe[port].ado_lock);
	if ((port == 1) && (print_tc_log == 1)) {
		CPRINTS("c1 clr ado_lock 0x%x", pe[port].ado_lock.lock_count);
	}
	//irq_unlock(key);
}
...
static void pe_send_alert_entry(int port)
{
	...
	if (msg == NULL || len == NULL) {
		...
		if ((port == 1) && (print_tc_log == 1)) {
			CPRINTS("c1 alert ado_lock 0x%x, INT %d", pe[port].ado_lock.lock_count, tt);
		}
		...
		*len = sizeof(pe[port].ado);
		mutex_unlock(&pe[port].ado_lock);
		if ((port == 1) && (print_tc_log == 1)) {
			CPRINTS("c1 alert ado_lock 0x%x", pe[port].ado_lock.lock_count);
		}
		//irq_unlock(key);
	}
