...
__maybe_unused static bool pe_attempt_port_discovery(int port)
{
	...
	if (port_discovery_dr_swap_policy(
		...
		/*
		 * If port partner (SRC) initializes an AMS message to us and
		 * before checking DPM_REQUEST_DR_SWAP in sink_dpm_requests(),
		 * then we will wait for SNKTXOK in the prl state when responding
		 * to the port partner message, so it's too early to set the
		 * waiting SNKTXOK flag here.
		 */
		PE_SET_FLAG(port, PE_FLAGS_LOCALLY_INITIATED_AMS); //since this flag will be set in sink_dpm_requests(), then this line can be deleted.
...
static void pe_snk_ready_run(int port)
{
	...
	if (pd_timer_is_disabled(port, PE_TIMER_WAIT_AND_ADD_JITTER) ||
		...
		if (pd_timer_is_expired(port, PE_TIMER_SINK_REQUEST)) {
			pd_timer_disable(port, PE_TIMER_SINK_REQUEST);
			CPRINTS("c%d PE_TIMER_SINK_REQUEST", port);
			set_state_pe(port, PE_SNK_SELECT_CAPABILITY);
			return;
		}
...
static void pe_drs_evaluate_swap_entry(int port)
{
	...
	if (pd_check_data_swap(port, pe[port].data_role)) {
		...
		if (dump == 1) {
			CPRINTS("c%d pe evaluate accept", port);
		}
	} else {
		/*
