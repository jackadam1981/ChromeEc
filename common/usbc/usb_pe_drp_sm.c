	...
	if (IS_ENABLED(CONFIG_USB_PD_ALT_MODE_DFP)) {
		for (i = 0; i < supported_modes_cnt; i++) {
			...
			if (opos > 0 &&
				...
				pe[port].alt_svid = supported_modes[i].svid;
				pe_dpm_request(port, DPM_REQUEST_EXIT_ALT_MODE);
			}
		}
	}
	/*
	 * TODO: Delay deleting the data until after the the EXIT_MODE message
	 * is sent. Unfortunately the callers of this function expect the mode
