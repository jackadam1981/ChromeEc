...
static enum vdm_response_result pe_discovery_vdm_request_run_common(int port,
...
static void pe_vdm_send_request_run(int port)
{
	...
	if (PE_CHK_FLAG(port, PE_FLAGS_MSG_RECEIVED)) {
		PE_CLR_FLAG(port, PE_FLAGS_MSG_RECEIVED);
		pe[port].vdm_result = pe_discovery_vdm_request_run_common(port,
				pe[port].vdm_ack_min_data_objects);
		return;
	}
...
static void pe_vdm_identity_request_cbl_run(int port)
{
	...
	case VDM_RESULT_ACK:
		...
		/*
		 * Note: If port partner runs PD 2.0, we must use PD 2.0
		 * to communicate with the cable plug when in an
		 * explicit contract.
		 *
		 * PD Spec Table 6-2: Revision Interoperability during
		 * an Explicit Contract
		 */
		/* FIXME: Use the received ACK's version. */
		if (prl_get_rev(port, TCPC_TX_SOP) != PD_REV20)
