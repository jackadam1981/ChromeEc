 ...
 */
#define SNK_READY_HOLD_OFF_US (400 * MSEC)
/*
...
static void pe_update_wait_and_add_jitter_timer(int port)
{
	 ...
	 * some jitter of up to ~768ms, to prevent
	 ...
	 * ~768ms to prevent multiple collisions.
	 */
	if (prl_get_rev(port, TCPC_TX_SOP) == PD_REV20 &&
	...
	/* Clear the PE_FLAGS_RESEND_MSG flag if it was set. */
	PE_CLR_FLAG(port, PE_FLAGS_RESEND_MSG);
}
