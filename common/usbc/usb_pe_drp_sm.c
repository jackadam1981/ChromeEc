 ...
 * TODO(b/150611251): Replace with shell entry states from spec to fill in vdm,
 * then shell states to fill in resulting data as well.
 */
enum vdm_cmd {
...
static struct policy_engine {
	...
	/* TODO(b/150611251): Store full partner DiscoverIdentity repsonse */
 ...
 *
 * When no Contract of an Implicit Contract is in place (e.g. after a Power Role
 * Swap or Fast Role Swap) only the Source port that is supplying Vconn is
 * allowed to send packets to a Cable Plug
 *
 * When in an explicit contract, PD 3.0 requries that a port be Vconn source to
