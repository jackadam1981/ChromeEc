...
static void pe_send_msg_entry(const int port);
static enum pe_msg_check pe_send_msg_run(const int port);
 ...
 * Common message send checking
 *
 * This is setup like a pseudo state machine parent state. I implemented
 * as I did to easily obtain the pe_msg_check state for each call as well
 * as know exactly when each is being called.
 */
/*
 * pe_send_msg_entry
 * Initiallization for handling discardable send messages.
 *
...
/*
 * pe_send_msg_run
 * Check status of discardable send messages.
 *
 * The normal progression of pe_msg_check is:
 ...
 * another message has been sent and pe_send_msg_entry is called
 * again.
 *
 * @param port USB-C Port number
...
static enum pe_msg_check pe_send_msg_run(const int port)
{
	if (pe[port].sender_response_timer == TIMER_DISABLED) {
		/*
		 * Check for Discard only if the initial message has
...
static void pe_src_send_capabilities_run(int port)
{
	 ...
	 */
	msg_check = pe_send_msg_run(port);
