	...
	if ((msg_check & PE_MSG_SENT) &&
		...
		if ((ext == 0) && (cnt == 0)) {
			if (type == PD_CTRL_ACCEPT) {
				...
				/*
				 * An edge case of DR Swap fail (port still
				 * UFP) and partner in PD 2.0. PD 2.0 allows
				 * only DFP to initiate Discover Identity,
				 * but partner may reject a DR Swap.
				 */
				if (pe[port].data_role == PD_ROLE_UFP &&
				    prl_get_rev(port,
					  TCPCI_MSG_SOP) == PD_REV20 &&
					  type == PD_CTRL_REJECT) {
					pd_set_identity_discovery(port,
					  TCPCI_MSG_SOP, PD_DISC_FAIL);
					pd_set_identity_discovery(port,
					  TCPCI_MSG_SOP_PRIME, PD_DISC_FAIL);
