...
static int command_pelog(int argc, const char **argv)
{
	...
	for (int i = 0; i < ARRAY_SIZE(pe_event_log_buffer); i++) {
		...
		if (oldest_index == INT_MAX) {
			oldest_index = i;
		} else if (entry->timestamp.val <
			   pe_event_log_buffer[oldest_index].timestamp.val) {
			oldest_index = i;
			/*
	...
	for (unsigned int i = 0; i < ARRAY_SIZE(pe_event_log_buffer); i++) {
		const struct pe_event_log_entry *entry =
			&pe_event_log_buffer[(i + oldest_index) %
					     ARRAY_SIZE(pe_event_log_buffer)];
