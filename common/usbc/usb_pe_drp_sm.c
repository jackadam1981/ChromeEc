...
static void pe_init_vdm_svids_request_run(int port)
{
	...
	if (PE_CHK_FLAG(port, PE_FLAGS_MSG_RECEIVED)) {
		...
		if (sop == pe[port].tx_type && type == PD_DATA_VENDOR_DEF &&
							cnt > 0 && ext == 0) {
			/*
			 * Valid Discover SVIDs ACKs should have at least
			 * 2 objects (header, SVID VDO).
			 */
			if (PD_VDO_CMDT(payload[0]) == CMDT_RSP_ACK &&
								cnt >= 2) {
				 ...
				 */
				/*
				 * TODO(b/155431551): Need another timer for
				 * backoff. Might want to generalize BUSY case
				 * to work with multiple VDM request states.
				 */
			} else {
				/*
				 * Cable gave us an incorrect size or command,
