...
static enum ec_status hc_typec_control(struct host_cmd_handler_args *args)
{
	...
	case TYPEC_CONTROL_COMMAND_SEND_VDM_REQ:
		if (!IS_ENABLED(CONFIG_USB_VDM_AP_CONTROL))
			return EC_RES_INVALID_PARAM;

		if (p->vdm_req_params.vdm_message_count <= 0 ||
		    p->vdm_req_params.vdm_message_count > VDO_MAX_SIZE)
			return EC_RES_INVALID_PARAM;

		memcpy(data, p->vdm_req_params.vdm_message,
		       sizeof(uint32_t) * p->vdm_req_params.vdm_message_count);
		...
		return pd_request_vdm(p->port, data,
				      p->vdm_req_params.vdm_message_count,
				      tx_type);
	default:
		return EC_RES_INVALID_PARAM;
	}
