...
static enum usb_tc_state tc_last_state(const int port)
{
...
{
	if (tc_last_state(port) != TC_UNATTACHED_SRC)
		print_current_state(port);
...
{
	if (tc_last_state(port) != TC_UNATTACHED_SNK)
		print_current_state(port);
...
{
	if (tc_last_state(port) != TC_CT_UNATTACHED_VPD)
		print_current_state(port);
...
{
	if (tc_last_state(port) != TC_CT_UNATTACHED_UNSUPPORTED)
		print_current_state(port);
 ...
 * Type-C states
 */
/* At botton so no need to forward declare all static methods */
static const struct usb_state tc_states[] = {
	/* Super States */
	[TC_VBUS_CC_ISO] = {
