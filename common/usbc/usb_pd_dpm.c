...
union sido dpm_get_source_info_msg(int port)
{
	...
	source_info.sidos.sido1.port_type = (CONFIG_USB_PD_3A_PORTS > 0) ?
					PD_SOURCE_PORT_CAPABILITY_MANAGED :
					PD_SOURCE_PORT_CAPABILITY_GUARANTEED;

	source_info.sidos.sido1.reserved = 0;

	/* Max PDP: 5V * 3A = 15W; floor(5V * 1.5A = 7W */
	source_info.sidos.sido1.port_maximum_pdp = (CONFIG_USB_PD_3A_PORTS > 0) ? 15 : 7;
	...
	if (pdo_count <= 0) {
		 ...
		 * default.
		 */
		source_info.sidos.sido1.port_reported_pdp = 0;
	} else {
		uint32_t highest_pdo = pdos[pdo_count - 1];

		source_info.sidos.sido1.port_reported_pdp = PDO_FIXED_VOLTAGE(highest_pdo) *
	...
	if (max_current_claimed & BIT(port) ||
		...
		source_info.sidos.sido1.port_present_pdp = source_info.sidos.sido1.port_maximum_pdp;
	} else {
		source_info.sidos.sido1.port_present_pdp = source_info.sidos.sido1.port_reported_pdp;
	}
