		   ...
		   uint32_t *vdm)
{
	...
	case USB_VID_DOCKING_MONITOR:
		docking_monitor_vdm_acked(port, type, vdo_count, vdm);
		break;
#endif
	default:
...
static void dpm_attempt_mode_entry(int port)
{
	 ...
	 */
#ifndef USB_VID_DOCKING_MONITOR
...
#ifdef USB_VID_DOCKING_MONITOR
	     && (docking_monitor_mode_entry_is_done(port) ||
		 !pd_is_mode_discovered_for_svid(port, TCPCI_MSG_SOP,
						 USB_VID_DOCKING_MONITOR))
#endif
		     ) ||
	...
	if (status == MSG_SETUP_UNSUPPORTED &&
		...
		status = dp_setup_next_vdm(port, &vdo_count, vdm);
		   ...
		   pd_is_mode_discovered_for_svid(port, TCPCI_MSG_SOP,
						  USB_VID_DOCKING_MONITOR)) {
		vdo_count = ARRAY_SIZE(vdm);
		status = docking_monitor_setup_next_vdm(port, &vdo_count, vdm);
#endif
	}
