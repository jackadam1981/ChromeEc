...
#define DPM_FLAG_MSG_COMPLETE BIT(10)
#define DPM_FLAG_PD_READY BIT(11)
...
__maybe_unused static __const_data const char * const dpm_state_names[] = {
	...
	[DPM_MSG_WAIT_RESP] = "DPM Msg Wait",
...
__maybe_unused test_export_static enum usb_dpm_state get_state_dpm(const
								   int port)
{
...
{
	CPRINTS("C%d: %s", port,
			dpm_state_names[get_state_dpm(port)]);
}
...
static void dpm_set_mode_entry_done(int port)
{
	...
	DPM_CLR_FLAG(port, DPM_FLAG_ENTER_DP | DPM_FLAG_ENTER_TBT |
				   DPM_FLAG_ENTER_USB4);
	CPRINTS("dpm: set mode entry done");
}
...
{
	DPM_SET_FLAG(port, DPM_FLAG_MSG_COMPLETE);
}
...
__maybe_unused static void dpm_run_pd_button_sm(int port)
{
...
static bool dpm_dfp_enter_mode_msg(int port)
{
	...
	bool enter_mode_requested = false;
	...
	/* Thunberbolt Alt-Mode Entry Check */
	if (IS_ENABLED(CONFIG_USB_PD_TBT_COMPAT_MODE) &&
	...
	/* Disply Port Alt-Mode Entry Check */
...
static bool dpm_exit_mode_msg(const int port)
{
	uint32_t vdm[VDO_MAX_SIZE];
...
void dpm_sm_run(int port, int evt, int en)
{
...
static void dpm_idle_run(const int port)
{
	...
	if (pd_get_svids_discovery(port, TCPCI_MSG_SOP) ==
	    PD_DISC_COMPLETE &&
	    pd_get_modes_discovery(port, TCPCI_MSG_SOP) ==
	    PD_DISC_COMPLETE)
	...
	if (DPM_CHK_FLAG(port, DPM_FLAG_PD_READY)) {
		if (dr == PD_ROLE_UFP) {
			...
			CPRINTS("C%d: IDLE ---> UFP Ready, pr = %d, dr = %d",
				port, pd_get_data_role(port),
				pd_get_data_role(port));
		} else if (dr == PD_ROLE_DFP && discovery_complete) {
			DPM_CLR_FLAG(port, DPM_FLAG_PD_READY);
			set_state_dpm(port, DPM_DFP_READY);
			CPRINTS("C%d: IDLE ---> DFP Ready, pr = %d, dr = %d",
				port, pd_get_data_role(port),
				pd_get_data_role(port));
		}
	}
...
static void dpm_dfp_ready_run(const int port)
{
	...
	/* Check for mode exit requests */
	if (DPM_CHK_FLAG(port, DPM_FLAG_EXIT_REQUEST)) {
		if (dpm_exit_mode_msg(port)) {
			set_state_dpm(port, DPM_MSG_WAIT_RESP);
			return;
		}
	}

	/* Check for mode entry requests, if mode entry is not completed */
	if (!DPM_CHK_FLAG(port, DPM_FLAG_MODE_ENTRY_DONE)) {
	...
	/*
	 * Mode entry is not done, AP is powered on, and mux is in stable state.
	 * Check to see if there is a Alt Mode or USB4 entry message to send. If
	 * a message is ready to send, then change states to wait for the port
	 * partners reply.
	 */
...
static void dpm_msg_wait_resp_entry(const int port)
{
	print_current_state(port);

	/* Clear DPM flag for msg received */
	DPM_CLR_FLAG(port, DPM_FLAG_MSG_COMPLETE);
}

static void dpm_msg_wait_resp_run(const int port)
{

	if (DPM_CHK_FLAG(port, DPM_FLAG_MSG_COMPLETE)) {
		DPM_CLR_FLAG(port, DPM_FLAG_MSG_COMPLETE);
		if (pd_get_data_role(port) == PD_ROLE_DFP) {
			set_state_dpm(port, DPM_DFP_READY);
...
static __const_data const struct usb_state dpm_states[] = {
	...
	[DPM_MSG_WAIT_RESP] = {
		.entry = dpm_msg_wait_resp_entry,
		.run   = dpm_msg_wait_resp_run,
	},
};

static int command_dpm(int argc, const char **argv)
{
	int i;

	for (i = 0; i < CONFIG_USB_PD_PORT_MAX_COUNT; i++) {
		print_current_state(i);
		ccprintf("DPM_FLAGS = 0x%04x\n", (uint32_t)dpm[i].flags);
	}

	return EC_SUCCESS;
}
DECLARE_CONSOLE_COMMAND(dpm, command_dpm, "",
			"DPM Info");
