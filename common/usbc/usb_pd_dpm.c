		   ...
		   uint32_t *vdm)
{
	...
	if (IS_ENABLED(CONFIG_USB_PD_VDM_AP_CONTROL) &&
		...
		ap_vdm_messages[port].type = type;
...
enum ec_status dpm_copy_vdm_reply(int port, enum tcpci_msg_type *type,
		uint8_t *size, uint32_t *buf)
{
	...
	if (ap_vdm_messages[port].vdm_reply_cnt == 0)
		return EC_RES_UNAVAILABLE;

	*type = ap_vdm_messages[port].type;
	*size = ap_vdm_messages[port].vdm_reply_cnt;
	memcpy(buf, ap_vdm_messages[port].vdm_reply, size * sizeof(uint32_t));
