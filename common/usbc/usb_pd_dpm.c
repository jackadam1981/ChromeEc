...
/*
 * Note: These interfaces are no longer fully supported and linked only for
 * EC_CMD_USB_PD_SET_AMODE and EC_CMD_USB_PD_GET_AMODE
 * TODO(b/267562375): TCPMv2: Obsolete GFU host commands
 */
...
/*
 * Note: This interface is used by EC_CMD_USB_PD_SET_AMODE, but also by the
 * standard PD task
 */
...
/* Note: this interface is linked only for the pe console command */
...
enum ec_status dpm_copy_vdm_reply(int port, uint8_t *type, uint8_t *size,
				  uint32_t *buf)
{
	return ap_vdm_copy_reply(port, type, size, buf);
}

uint8_t dpm_attention_pop(int port, uint32_t *buf, uint8_t *items_left)
{
	return ap_vdm_attention_pop(port, buf, items_left);
}
