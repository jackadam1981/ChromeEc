...
static void battery_update(enum battery_index i)
{
	...
	*memmap_ccnt = battery_static[i].cycle_count;
	...
	/*
	 * If any of the dynamic info is invalid, conservatively report all of the
	 * values as unknown because the flags aren't specific enough to know which
	 * value(s) are invalid.
	 *
	 * The charger keeps more detailed validity flags for each field, but that
	 * updates independently of battery_dynamic so we can't use those flags to
	 * present whatever data we do have because the charger's current flags may
	 * not match what's actually in battery_dynamic right now.
	 */
