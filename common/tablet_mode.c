...
#endif
static void gmr_tablet_mode_interrupt_debounce(void)
{
...
DECLARE_DEFERRED(gmr_tablet_mode_interrupt_debounce);

/* Debounce time for gmr sensor tablet mode interrupt */
#define GMR_SENSOR_DEBOUNCE_US    (30 * MSEC)

void gmr_tablet_mode_isr(enum gpio_signal signal)
{
	hook_call_deferred(&gmr_tablet_mode_interrupt_debounce_data,
...
static void gmr_tablet_switch_init(void)
{
	 ...
	 */
	gmr_tablet_mode_interrupt_debounce();
}
...
void gmr_tablet_switch_disable(void)
{
	...
	hook_call_deferred(&gmr_tablet_mode_interrupt_debounce_data, -1);
	tablet_disable();
}
#endif
