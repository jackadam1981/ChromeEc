...
/* 1: all calls to tablet_set_mode are ignored and tablet_mode => 0 */
static int disabled;
...
void tablet_set_mode(int mode)
{
	...
	if (hall_sensor_at_360 && !mode) {
		CPRINTF("Ignoring tablet mode exit while hall sensor active.");
		return;
	}
...
{
	hall_sensor_at_360 = !gpio_get_level(HALL_SENSOR_GPIO_L);
