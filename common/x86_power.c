...
void x86_power_task(void)
{
	...
	while (1) {
		...
		case X86_S3S0:
			/* Deassert prochot so it's low when +VCCP powers up */
		...
		case X86_S0S3:
			...
			/* Deassert prochot since CPU is now shut down */
			/* TODO: If PROCHOT is externally pulled up, we should
			 * set PROCHOT here instead, so we don't leak power
			 * through the pullup - and we should set its default
			 * state in board.h to HIGH.  See crosbug.com/p/8982. */
			gpio_set_flags(GPIO_CPU_PROCHOT, 0);
