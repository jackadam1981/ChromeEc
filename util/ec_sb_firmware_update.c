...
enum {
	F_AC_PRESENT         = 1, /* AC Present flag */
	F_SKIP_VERSION_CHECK = 2 /* skip version checking, debug only */
};
		...
		struct fw_update_ctrl *fw_update)
{
	...
	if (rv == 0 && !(fw_update->flags & F_SKIP_VERSION_CHECK)) {
		fw_update->rv = 0;
...
int main(int argc, char *argv[])
{
	...
	uint8_t val = 0;
	if (argc > 3) {
		printf("Usage: %s [protect] [skip_version_check]\n"
			"	protect: 0 or 1, default 1\n"
			"	skip_version_check: 0 or 1, default 0\n",
			argv[0]);
		return -1;
	}
	...
	if (argc >= 3)
		skip_version_check = atoi(argv[2]);
	...
	if (skip_version_check)
		fw_update.flags |= F_SKIP_VERSION_CHECK;
