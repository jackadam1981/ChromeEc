...
int ec_sb_firmware_update(const char *fw_image_name)
{
	...
	info->hdr.state = EC_CMD_SB_FW_UPDATE_INFO;
	rv = ec_command(EC_CMD_SB_FW_UPDATE, 0, NULL, 0, info, sizeof(*info));
	if (rv) {
		fprintf(stderr,
	...
	rv = ec_command(EC_CMD_SB_FW_UPDATE, 0, NULL, 0, NULL, 0);
	if (rv) {
		fprintf(stderr,
	...
	for (i = 0; i < size; i += step_size) {
		...
		status->hdr.state = EC_CMD_SB_FW_UPDATE_STATUS;
