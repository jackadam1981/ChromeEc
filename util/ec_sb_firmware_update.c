...
#define DELAY_VALUE_F 25000
#define DELAY_VALUE_G 50000
		...
		struct fw_update_ctrl *fw_update)
{
	...
	if (rv) {
		fw_update->rv = 0;
		/* Assume Simplo Battery */
		fw_update->info.maker_id = SIMPLO_MAKER_ID;
		fw_update->info.hardware_id = SIMPLO_HWID;
		fw_update->info.fw_version = 0;
		fw_update->info.data_version = 0;
	}
...
static enum fw_update_state s7_read_status(struct fw_update_ctrl *fw_update)
{
	...
	} while (fw_update->status.busy && (++cnt < 10));
