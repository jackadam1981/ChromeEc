...
step0:
	...
	/*Step 0: cmd.0x35 write word 0x1000 */
	printf("setp0: cmd.0x35 write word 0x1000\n");
...
step1:
	...
	printf("cmd.0x37 read info: sz:%ld\n", sizeof(info));
	rv = ec_sb_fw_get_info(&info);
	if (rv)
		goto error_return;

	printf("cmd.0x35 get status\n");
	rv = ec_sb_fw_get_status(&status);
	if (rv)
		goto error_return;
	...
	rv = 1; /* Remove Me after debug checksum */
	if (rv == 0) {
