...
#define DELAY_US_VALUE_BEGIN  500000
#define DELAY_US_VALUE_END   1000000
#define DELAY_US_VALUE_BUSY  1000000
#define DELAY_US_VALUE_WRITE_END  50000
...
static enum fw_update_state s6_write_block(struct fw_update_ctrl *fw_update)
{
	...
	if ((offset + 3*fw_update->step_size) >= fw_update->size)
		usleep(DELAY_US_VALUE_WRITE_END);
