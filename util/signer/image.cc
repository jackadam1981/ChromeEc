...
void Image::toIntelHex(const string& filename) const {
...
void Image::generate(const std::string& filename, bool hex_output) const {
  ...
  FILE* fout = fopen(filename.c_str(), "w");
  size_t image_size = high_ - base_;

  if (!fout) return;

  if (fwrite(mem_, 1, image_size, fout) != image_size) return;
  fclose(fout);
}
