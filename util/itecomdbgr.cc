...
static void flush_com(struct itecomdbgr_config *conf)
{
	...
	/*
	 * Do the same as above, but with longer wait time, in order to find out
	 * if the threshold should be adjusted.
	 */
	for (;;) {
		FD_ZERO(&read_fds);
		FD_SET(conf->g_fd, &read_fds);
		...
		fprintf(stderr, "Additional serial data after %ld us\n",
			100000 - timeout.tv_usec);
		/*
		 * Select indicated that data is available to read, get whatever
		 * we can, discard it, and then go back and ask if there is
		 * more.
