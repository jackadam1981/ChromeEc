...
uint8_t fast_read_burst_cdata(struct itecomdbgr_config *conf, uint8_t *C_Data,
			      bool check_erased)
{
	unsigned int i = 0;
	uint8_t DBG_BUF[256];
...
bool check_flash(struct itecomdbgr_config *conf)
{
	...
	if (fast_read_burst_cdata(conf, NULL, 1)) {
		printf("\n\rcheck_flash : error");
		result = FAIL;
	}
	printf("\n\r");
...
bool verify_flash(struct itecomdbgr_config *conf)
{
	...
	if (fast_read_burst_cdata(conf, conf->g_writebuf, 0)) {
		printf("\n\rverify_flash : error");
		result = FAIL;
	}
	printf("\n\r");
