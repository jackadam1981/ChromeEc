...
bool write_com(struct itecomdbgr_config *conf, const uint8_t *lpOutBuffer,
	       int WriteBytes)
{
	bool bWriteStat;
...
void rw_reg(struct itecomdbgr_config *conf, unsigned long Address, uint8_t RW)
{
...
void getchipid(struct itecomdbgr_config *conf)
{
...
int read_id_2(struct itecomdbgr_config *conf)
{
	...
	printf(" Flash ID :%02x %02x %02x", FlashID[0], FlashID[1], FlashID[2]);
...
bool erase_4k(struct itecomdbgr_config *conf)
{
	int i = 0;
	bool result = SUCCESS;
	...
	while (start_addr < end_addr) {
		...
		if (check_status(conf, 0x02, 1) < 0) {
			printf("\n\rerase_4k:check_status error 1");
			result = FAIL;
			goto out;
		}
		write_com(conf, cs_low, sizeof(cs_low));
		...
		if (check_status(conf, 0x01, 0) < 0) {
			printf("\n\rerase_4k:check_status error 2");
			result = FAIL;
			goto out;
		}
...
bool erase_flash(struct itecomdbgr_config *conf)
{
	bool result = SUCCESS;
...
bool fast_read_burst_cdata(struct itecomdbgr_config *conf, uint8_t *C_Data,
			   bool check_erased)
{
	unsigned int i = 0;
	uint8_t DBG_BUF[256];
	...
	while (start_addr < end_addr) {
		...
		if (check_status(conf, 0x01, 0) < 0) {
			printf("\n\rfast_read_burst_cdata:check_status error 1");
			result = FAIL;
			goto out;
		}
		write_com(conf, cs_low, sizeof(cs_low));
		...
		write_com(conf, cs_high, sizeof(cs_high));
		if (check_status(conf, 0x01, 0) < 0) {
			printf("\n\rfast_read_burst_cdata:check_status error 2");
			result = FAIL;
			goto out;
		}
...
bool page_program_burst_v2(struct itecomdbgr_config *conf, uint8_t *wr_data)
{
	bool result = SUCCESS;
	unsigned int j = 0;
	unsigned long start_addr = conf->update_start_addr;
	...
	while (start_addr < end_addr) {
		...
		if (check_status(conf, 0x02, 1) < 0) {
			printf("\n\rpage_program_burst_v2: check_status WEL err");
			result = FAIL;
			goto out;
		}
		...
		/* Check WIP bit off */
		if (check_status(conf, 0x01, 0) < 0) {
			printf("\n\rpage_program_burst_v2: check_status WIP err");
			result = FAIL;
			goto out;
		}
...
bool write_flash(struct itecomdbgr_config *conf)
{
	bool result = SUCCESS;
	if (result = page_program_burst_v2(conf, conf->g_writebuf)) {
		printf("\n\rwrite_flash : error");
	}
	printf("\n\r");
	return result;
...
bool check_flash(struct itecomdbgr_config *conf)
{
	...
	if (result = fast_read_burst_cdata(conf, NULL, 1)) {
		printf("\n\rcheck_flash : error");
	}
	printf("\n\r");
	return result;
...
bool read_flash(struct itecomdbgr_config *conf)
{
	bool result = SUCCESS;
	...
	if (result = fast_read_burst_cdata(conf, NULL, 2)) {
		printf("\n\rread_flash : error");
		result = FAIL;
	}
	printf("\n\r");
...
int main(int argc, char **argv)
{
	...
	int option_index = 0;
	int c;
	const char *optstring = "f:d:b:hnr:";
	...
	while (1) {
		...
		default:
			...
			printf("\n\r	-b [baudrate]");
			printf("\n\r	-n : no verify");
			printf("\n\r	-r : [read start addr] [range]");
			printf("\n\rExample :\n\r");
			printf("\n\r    %s -f ec.bin -d /dev/ttyUSB3\n\r",
			       argv[0]);
			printf("\n\r    %s -f ec.bin -d /dev/ttyUSB3 -n\n\r",
			       argv[0]);
			printf("\n\r    %s -r 0 0x100000\n\r", argv[0]);
			exit(1);
		}
	...
	if ((conf.baudrate != 115200) && (conf.baudrate != 3000000)) {
		printf("\n\rUART Baudrate only support 115200  or 3M\n\r");
		return 0;
	}
