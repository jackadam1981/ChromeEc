...
struct stm32_device_signature {
	/*
	 * * Address of the *last* byte in the One Time Programmable data.
...
struct stm32_def {
	...
	{0x416, "STM32L15xxB",   0x20000,   256, {13, 13}, { 0 } },
	{0x429, "STM32L15xxB-A", 0x20000,   256, {13, 13}, { 0 } },
	{0x427, "STM32L15xxC",   0x40000,   256, {13, 13}, { 0 } },
	{0x435, "STM32L44xx",    0x40000,  2048, {13, 13}, { 0 } },
	{0x420, "STM32F100xx",   0x20000,  1024, {13, 13}, { 0 } },
	{0x410, "STM32F102R8",   0x10000,  1024, {13, 13}, { 0 } },
	{0x440, "STM32F05x",     0x10000,  1024, {13, 13}, { 0 } },
	{0x444, "STM32F03x",     0x08000,  1024, {13, 13}, { 0 } },
	{0x448, "STM32F07xB",    0x20000,  2048, {13, 13}, { 0 } },
	{0x432, "STM32F37xx",    0x40000,  2048, {13, 13}, { 0 } },
	{0x442, "STM32F09x",     0x40000,  2048, {13, 13}, { 0 } },
	{0x431, "STM32F411",     0x80000, 16384, {13, 19}, { 0 } },
	/*
	 * STM32F412:
	 * See https://www.st.com/resource/en/reference_manual/dm00180369.pdf
	 * Section 3.3 Embedded Flash Memory
	 */
	{0x441, "STM32F412",    0x100000, 16384, {13, 19},
		/*
		 * STM32F412:
		 * See https://www.st.com/resource/en/reference_manual/dm00180369.pdf
		 * Section 31 Device electronic signature
		 */
		{ 0x1FFF7A0F, 0x1FFF7A10, 0x1FFF7A22,  0x1FFF7BF0 } },
	{0x450, "STM32H74x",    0x200000, 131768, {13, 19}, { 0 } },
	{0x451, "STM32F76x",    0x200000, 32768, {13, 19}, { 0 } },
	{ 0 }
};
 ...
 * starts a read in that region and reads an additional number of bytes to
				   ...
				   uint8_t *out_buffer)
{
	int res;
	uint8_t *buffer;
	uint32_t otp_end_addr = chip->device_signature.otp_end_addr;
	uint32_t offset = addr - otp_end_addr;
	uint32_t read_size_bytes = offset + size_bytes;

	if (!otp_end_addr) {
		fprintf(stderr, "No otp_end_addr specified for given chip.\n");
		return -EINVAL;
	}

	if (read_size_bytes <= 0) {
		fprintf(stderr, "Attempting to invalid number of %d bytes\n",
			read_size_bytes);
		return -EINVAL;
	}
	...
	if (read_size_bytes > PAGE_SIZE) {
		fprintf(stderr, "Requested register is outside read range\n");
		return -EINVAL;
	}
