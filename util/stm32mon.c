...
/* Return 0 if we are using UART boot protocol, 1 if we are using i2c */
#define OVER_I2C (i2c_adapter != INVALID_I2C_ADAPTER)
...
{
	fprintf(stderr,
		"Usage: %s [-a <i2c_adapter> [-s <address>] |"
	...
	fprintf(stderr, "--s[lave] <address> : use address <address>.\n\n");
	...
	fprintf(stderr, "--l[ngth] : amount to read/write\n");
...
int parse_parameters(int argc, char **argv)
{
	...
	while ((opt = getopt_long(argc, argv, "a:s:b:d:eghl:o:r:w:uU?",
				  longopts, &idx)) != -1) {
		switch (opt) {
		case 'a':
