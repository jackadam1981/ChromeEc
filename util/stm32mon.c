...
static int wait_for_ack(int fd)
{
	...
	while (time(NULL) < deadline) {
		...
		if (resp == RESP_ACK_ISH && retry_on_ackish == 1) {
			/* It is suspicious ACK. Let's treat it as if ACK, but
			 * acknowledge the caller, so that the caller can
			 * retry.
			 */
			stat_resp[RESP_ACK_ISH_IDX].event_count++;
			fprintf(stderr, "ACK_ISH\n");
			if (mode == MODE_SPI) /* Ack the ACK_ISH */
				if (write_wrapper(fd, &ack, 1) != 1)
					return -EIO;
			return -EUCLEAN;
		}
...
int init_monitor(int fd)
{
	...
	while (1) {
		...
		if (res < 0 && res != -ETIMEDOUT && res != -EUCLEAN)
			return res;
		fflush(stdout);
	}
	printf("Done.\n");
...
int command_get_commands(int fd, struct stm32_def *chip)
{
	 ...
	 */
	cmds_len = chip->cmds_len[(mode == MODE_I2C ? 1 : 0)];
	do {
		res = send_command(fd, CMD_GETCMD, NULL, 0, cmds, cmds_len, 1);
	} while (--retries && res == -EUCLEAN);
...
int command_read_mem(int fd, uint32_t address, uint32_t size, uint8_t *buffer)
{
	...
	while (remaining) {
		...
		do {
			res = send_command(fd, CMD_READMEM, loads, 2, buffer,
					cnt + 1, 0);
		} while (--retries && res == -EUCLEAN);
