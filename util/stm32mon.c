...
int command_ext_erase(int fd, uint16_t count, uint16_t start)
{
	...
	if (count < 0xfff) {
		 ...
		 */
		load_cnt = (i2c_adapter == INVALID_I2C_ADAPTER ? 1 : 2);
		load_to_send = &load[2-load_cnt];
		load[1].size = 2 * (count + 2 - load_cnt);
		...
		for (i = 0; i < count; i++)
			pages[i+2-load_cnt] = htons(start + i);
	} else {
		load_to_send = load;
		load_cnt = 1;
	}

	res = send_command(fd, CMD_EXTERASE, load_to_send, load_cnt,
			   NULL, 0, 1);
	if (res >= 0)
		printf("Flash erased.\n");
	...
	if (flags & FLAG_ERASE || output_filename) {
		if (!strcmp("STM32L15", chip->name)) {
			...
			for (i = 0; i < page_count; i += 128) {
				...
				if (has_exterase)
					ret = command_ext_erase(ser, count, i);
				else
					ret = command_erase(ser, count, i);
				if (ret)
					goto terminate;
			}
		} else {
			if (has_exterase)
				ret = command_ext_erase(ser, 0xFFFF, 0);
			else
				ret = command_erase(ser, 0xFF, 0);
			if (ret)
				goto terminate;
		}
	}
