...
class FileConversionFormat:
    """Parses the "file:conversion" specifier format given as in_files.

    Example specifiers: testdata/image1.png:raw

    Attributes:
        path: The path component of the specifier.
        conversion: The optional conversion part of the specifier.
    """
...
class TOC(HeaderSourceContent):
    ...
    def source_definitions(self, name_prefix: str) -> List[str]:
        ...
        for path_full in self._paths:
            path = path_full
            if self._strip:
                path = path_full.relative_to(self._strip)
            name = self._scoped_name(name_prefix + "_", path_full, self._strip)
...
def main(argv: List[str]) -> int:
    ...
    for file in args.in_files:
        file: FileConversionFormat
        decomposer = AnyFileToBytes

        if file.conversion:
            if file.conversion not in REGISTERED_DECOMPOSERS:
                parser.error(
                    f"Error - decomposition '{file.conversion}' does not exist"
                )
            decomposer = REGISTERED_DECOMPOSERS[file.conversion]
        else:
            mime = mimetypes.guess_type(file.path)
            if mime in REGISTERED_MIMETYPES:
                decomposer = REGISTERED_MIMETYPES[mime]

        rel_path = Path(os.path.relpath(file.path))
