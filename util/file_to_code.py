...
class AnyFileToBytes(HeaderSourceContent):
    ...
    def header_declarations(self, name_prefix: str) -> List[str]:
        name = self._scoped_name(name_prefix + "_", self._path)
...
class TOC(HeaderSourceContent):
    ...
    def source_definitions(self, name_prefix: str) -> List[str]:
        ...
        for path in self._paths:
            name = self._scoped_name(name_prefix + "_", path)
    ...
    for file in args.in_files:
        ...
        if len(parts) == 1:
            ...
            if parts[1] not in REGISTERED_DECOMPOSERS:
                parser.error(f"Error - decomposition '{parts[1]}' does not exist")
            decomposer = REGISTERED_DECOMPOSERS[parts[1]]

        rel_path = Path(os.path.relpath(file_path, str(ec_root)))
        out.add_content(decomposer(rel_path))

    if args.toc:
        out.add_content(TOC(args.in_files))
