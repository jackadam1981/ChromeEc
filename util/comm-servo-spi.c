...
/* SPI mode 0:
 * propogates data on the falling edge
...
static int mpsse_set_pins(int port)
{
			...
			size_t outsize)
{
	...
	if (raw_read(txbuf, block_size) != block_size)
		goto free_request;
