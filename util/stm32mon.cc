...
int command_ext_erase(int fd, uint16_t count, uint16_t start)
{
	...
	if (count < 0xfff0) {
		...
		load.size = 2 * (count + 1);
		pages = static_cast<uint16_t *>(malloc(load.size));
		if (!pages)
			return STM32_ENOMEM;
...
int command_erase_i2c(int fd, uint16_t count, uint16_t start)
{
	...
	if (count < 0xfff) {
		...
		load_cnt = 2;
		load[1].size = 2 * count;
		pages = static_cast<uint16_t *>(malloc(load[1].size));
		if (!pages)
			return STM32_ENOMEM;
...
int command_erase(int fd, uint16_t count, uint16_t start)
{
	...
	if (count < 0xff) {
		...
		/* not a special value : build a list of pages */
		load.size = count + 1;
		pages = static_cast<uint8_t *>(malloc(load.size));
		if (!pages)
			return STM32_ENOMEM;
		load.data = (uint8_t *)pages;
		pages[0] = count - 1;
				   ...
				   uint8_t *out_buffer)
{
	...
	buffer = static_cast<uint8_t *>(malloc(read_size_bytes));
	if (!buffer) {
