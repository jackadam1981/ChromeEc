...
# a + or - to enforce that it is added or removed from the active set of boards.
# Boards are added and removed in the order in which they appear.
...
if ! KEEP_TEMP_DIR="$(parse-bool "${FLAGS_keep}")"; then
        echo "# Error - Invalid argument to --keep" >&2
        exit 1
fi
if ! PARALLEL="$(parse-bool "${FLAGS_parallel}")"; then
        echo "# Error - Invalid argument to --parallel" >&2
        exit 1
fi
