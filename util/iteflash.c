...
#define I2C_MUX_CMD_ADDR_DEFAULT	0x70
...
static int config_i2c_mux(struct common_hnd *chnd, uint8_t cmd)
{
	...
	ret = i2c_byte_transfer(chnd, chnd->conf.i2c_mux_addr, &cmd, 1, 1);
	if (ret < 0) {
...
static void display_usage(char *program)
{
	...
	fprintf(stderr, "-m, --i2c-mux <0|1|false|true>: Config i2c-mux to EC."
		" Default is false(0). Works with CCD i2c-interface."
		" Set to true if I2C Mux is equipped on the board.\n");
...
int main(int argc, char **argv)
{
	...
	struct common_hnd chnd = {
		...
		.conf = {
			.send_waveform = 1,
			.i2c_mux_addr = I2C_MUX_CMD_ADDR_DEFAULT,
