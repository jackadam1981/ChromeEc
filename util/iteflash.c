...
/* Zero value is the flag default value. */
/* TODO(b/79684405): Expand to include Linux i2c-dev support. */
enum i2c_if_type {
	I2C_IF_FTDI = 0,
	I2C_IF_CCD,
};
...
/*
 * Optional command line flags.  All are zero-initialized, so all default to
 * zero values when not specified on the command line.
 */
struct flag_settings {
	/* TODO(b/79684405): Invert meaning so that default behavior is skip. */
	int skip_special_waveform;  /* boolean */
	int unprotect;  /* boolean */
	int erase;  /* boolean */
	enum i2c_if_type i2c_if_type;
};
...
struct i2c_interface {
	...
	/*
	 * TODO(b/79684405): Move responsibility for sending the
	 * special waveform to flash_ec.
	 */
	/* Optional, may be NULL. */
...
static const struct common_hnd zero_init_common_hnd;
static const struct flag_settings zero_init_flag_settings;
...
static int ccd_send_special_waveform(struct common_hnd *chnd)
{
	uint8_t response[20];
	size_t rsize;
	uint8_t req[] = {
...
static int ftdi_send_special_waveform(struct common_hnd *chnd)
{
	...
	if (!wave) {
		fprintf(stderr, "malloc(" STR(SPECIAL_BUFFER_SIZE) ") failed");
		return -1;
	}
...
/*
 * TODO(b/79684405): Move FTDI special waveform support to a separate binary,
 * and invoke from flash_ec.
 *
 * TODO(b/79684405): Move CCD special waveform invocation to flash_ec.
 */
static int send_special_waveform(struct common_hnd *chnd)
{
	...
	if (!chnd->i2c_if.send_special_waveform) {
	...
	do {
		ret = chnd->i2c_if.send_special_waveform(chnd);
		if (ret)
			break;
...
void display_usage(char *program)
{
	fprintf(stderr, "Usage: %s [-d] [-v <VID>] [-p <PID>] "
		"[-c <linux|ccd|ftdi] [-i <1|2>] [-s <serial>] [-u] [-e] "
	...
	fprintf(stderr, "-W, --special-waveform <0|1|false|true> : Send the "
			"specal waveform?\n");
	...
	while ((opt = getopt_long(argc, argv, "?dehc:i:p:r:s:uv:w:",
		...
		case 'c':
			if (!strcmp(optarg, "ccd")) {
				flags->i2c_if_type = I2C_IF_CCD;
			} else if (!strcmp(optarg, "ftdi")) {
				flags->i2c_if_type = I2C_IF_FTDI;
			} else {
				fprintf(stderr, "Unexpected -c / "
					"--i2c-interface value: %s", optarg);
				return -1;
			}
			break;
		case 'd':
		...
		case 'v':
			...
			break;
		case 'W':
			if (!strcmp(optarg, "0") || !strcmp(optarg, "false")) {
				flags->skip_special_waveform = 1;
			} else if (!strcmp(optarg, "1") ||
				   !strcmp(optarg, "true")) {
				flags->skip_special_waveform = 0;
			} else {
				fprintf(stderr, "Unexpected -W / "
					"--special-waveform value: %s", optarg);
				return -1;
			}
			break;
		case 'w':
...
{
	int ret = 1, other_ret;
	struct flag_settings flags = zero_init_flag_settings;
	struct common_hnd chnd = zero_init_common_hnd;
	...
	switch (flags.i2c_if_type) {
	case I2C_IF_CCD:
		chnd.i2c_if.send_special_waveform = ccd_send_special_waveform;
		chnd.i2c_if.byte_transfer = ccd_i2c_byte_transfer;
		...
		break;
	case I2C_IF_FTDI:
		chnd.i2c_if.interface_init = ftdi_i2c_interface_init;
		chnd.i2c_if.interface_shutdown = ftdi_i2c_interface_shutdown;
		chnd.i2c_if.send_special_waveform = ftdi_send_special_waveform;
		chnd.i2c_if.byte_transfer = ftdi_i2c_byte_transfer;
		...
		break;
	default:
		fprintf(stderr, "Unexpected i2c_interface enum value %d.  This "
			"is a bug!", flags.i2c_if_type);
		/* TODO(b/79684405): Should this simply return ret? */
		return ret;
	}
	...
	if (!flags.skip_special_waveform && send_special_waveform(&chnd) < 0)
		goto terminate;

	if (chnd.i2c_if.interface_init && chnd.i2c_if.interface_init(&chnd) < 0)
		goto terminate;
...
terminate:
	...
	if (chnd.i2c_if.interface_shutdown) {
		other_ret = chnd.i2c_if.interface_shutdown(&chnd);
		if (!ret && other_ret)
			ret = other_ret;
	}
