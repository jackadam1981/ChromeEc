...
/* TODO(b/79684405): Delete this before sending for review. */
#define EXTRA_I2C_DEBUG
#undef EXTRA_I2C_DEBUG
...
static int ftdi_config_i2c(struct ftdi_context *ftdi)
{
	...
	/* configure the clock */
	divisor = 60000000 / (2 * FTDI_I2C_FREQ * 3 / 2 /* 3-phase CLK */) - 1;
	buf[0] = EN_3_PHASE;
	buf[1] = DIS_DIV_5;
	buf[2] = TCK_DIVISOR;
	buf[3] = divisor & 0xff;
			      ...
			      uint32_t size, uint8_t *buffer)
{
	...
	while (remaining) {
		...
		res += i2c_write_byte(chnd, 0x08, addr_L);
