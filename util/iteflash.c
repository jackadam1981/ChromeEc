...
static int config_i2c(struct ftdi_context *ftdi)
{
	...
	/* configure the clock */
	divisor = DIV_VALUE(I2C_FREQ * 3 / 2 /* 3 phases */);
	buf[0] = EN_3_PHASE;
	buf[1] = DIS_DIV_5;
	buf[2] = TCK_DIVISOR;
	buf[3] = divisor & 0xff;
...
static struct ftdi_context *open_ftdi_device(int vid, int pid, int interface)
{
	...
	ret = ftdi_usb_open(ftdi, vid, 0x5002);
	if (ret < 0) {
...
int main(int argc, char **argv)
{
	...
	/* Open the USB device */
	hnd = open_ftdi_device(SERVO_USB_VID, SERVO_USB_PID, SERVO_INTERFACE);
	if (hnd == NULL)
		return 1;
