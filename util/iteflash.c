...
static int exit_dbgr_mode(struct ftdi_context *ftdi)
{
	 ...
	 * 'servod' will be able to initiate ftdi_i2c interface
	 * if we try to run it again.
	 */
	 ...
	 * For now, we do a cold reset for EC manually after flashing.
	 */
...
terminate:
	...
	/* Exit DBGR mode */
	exit_dbgr_mode(hnd);
	/* TODO(crosbug.com/p/53664) */
	printf("=== PLEASE COLD RESET EC ===\n");
