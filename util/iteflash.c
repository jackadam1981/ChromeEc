...
static int ccd_i2c_interface_init(struct common_hnd *chnd)
{
	...
	usb_findit(usb_vid, usb_pid, CR50_I2C_SUBCLASS, CR50_I2C_PROTOCOL,
		&chnd->uep);
...
{
	chnd->ftdi_hnd = open_ftdi_device(usb_vid, usb_pid, usb_interface,
		usb_serial);
	...
	while ((opt = getopt_long(argc, argv, "?dehc:i:p:r:s:uv:w:",
		...
		case 'W':
			if (!strcmp(optarg, "0") || !strcmp(optarg, "false")) {
				flags->send_waveform = 0;
			} else if (!strcmp(optarg, "1") ||
				   !strcmp(optarg, "true")) {
				flags->send_waveform = 1;
			} else {
...
static const struct flag_settings default_flag_settings = {
	/*
	 * TODO(b/79684405): Change the default to false after updating
	 * flash_ec to handle sending the waveform.
	 */
	.send_waveform = 1,
...
{
	int ret = 1, other_ret;
	struct flag_settings flags = default_flag_settings;
