...
int check_status(struct itecomdbgr_config *conf, uint8_t wait_mask, uint8_t f)
{
	uint8_t status;
	int timeout = 0;
			     ...
			     bool check_erased)
{
	...
	while (start_addr < end_addr) {
		...
		printf("\n\rtest 1\n\r");
		read_com(conf, DBG_BUF, 256);
		hexdump(DBG_BUF,256);
		hexdump(conf->G_DBG_BUF,256);
		msleep(10);
		printf("\n\rtest 2\n\r");
		if(check_erased) {
			printf("\n\rtest 3\n\r");
			...
			count = memcmp(&C_Data[start_addr],DBG_BUF,256);
			printf("\rVerifying...    : %d%%               ",
				(++j * 100) / (total_size - 1));

			if(conf->SaveFlag) {
				memcpy(&conf->g_readbuf[start_addr] ,
						DBG_BUF , 256);
			}
		}
		if(count) {
