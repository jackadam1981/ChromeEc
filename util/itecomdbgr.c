...
int init_file(struct itecomdbgr_config *conf)
{
	...
	if (conf->fi != NULL) {
		...
		len = fread(g_writebuf, 1, conf->file_size, conf->fi);
	} else {
...
BOOL write_com(struct itecomdbgr_config *conf, char *lpOutBuffer,
	       int WriteBytes)
{
	BOOL bWriteStat;
...
int FastRead(struct itecomdbgr_config *conf, UCHAR BA, UCHAR A1, UCHAR A0,
	     UCHAR *buffer)
{
	int result;
	int i;
	...
	write_com(conf, FastRead_burst_read, 52);
	tcflush(conf->g_fd, TCIOFLUSH);
	if (USE_3M) {
		Sleep(6);
	} else {
			     ...
			     uint8_t FLAG)
{
	...
	while (start_addr < end_addr) {
		...
		for (i = 0; i < 0x100; i++) {
			...
			if (!FLAG && conf->SaveFlag) {
				g_readbuf[start_addr + i] = DBG_BUF[i];
			}
	...
	if (!FLAG && conf->SaveFlag) {
		...
		pW = fopen("save.bin", "w");
		fseek(pW, 0, SEEK_SET);
		fwrite(g_readbuf, 1, conf->eflash_size_in_k * 1024, pW);
		fclose(pW);
...
{
	Page_Program_Burst_v2(conf, g_writebuf);
	printf("\n\r");
}
...
BOOL verify_flash(struct itecomdbgr_config *conf)
{
	...
	if (FastRead_Burst_CData(conf, g_writebuf, 0)) {
		printf("\n\rverify_flash : error");
		while (1)
			;
	}
