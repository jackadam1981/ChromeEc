...
/*
 * Find the requested USB endpoint, as determined by vid, pid, subclass,
 * protocol parameters, and serial number. If found, fill up the uep structure.
 * If succeeded, usb_shut_down() must be invoked before program exits.
 *
 * Return 0 on success, -1 on failure.
 */
int usb_findit(uint16_t vid, uint16_t pid, uint16_t subclass,
	       uint16_t protocol, char *serialno, struct usb_endpoint *uep);
