        ...
        for ch_cap in captured:
          if ch_cap not in CHARGEN_TXT:
            ...
            if ch_cap not in CRLF:
              ...
              self.logger.error('Broken char captured: Expected %r but got %s'
                                ' after %d char received', ch_exp,
                                hex(ord(ch_cap)), self.num_ch_cap)
          ...
          if ch_exp != ch_cap:
            ...
            # are lost.
            self.logger.error('Detected char loss: : Expected %r but got %r'
                              ' after %d char received', ch_exp, ch_cap,
                              self.num_ch_cap)
