...
This tester runs the command 'chargen' to EC and/or AP, captures the
...
LF = '\n'                        # Line Feed
CRLF = CR + LF
FLAG_FILE_PATH = '/tmp/chargen_testing'
...
class UartSerial(object):
  ...
  UART_DEV_PROFILES = (
      # Kernel
      {
          ...
          'prepare_cmd':[
              ...
              'touch ' + FLAG_FILE_PATH, # Create a temp file
          ],
          'cleanup_cmd':[
              'rm -f ' + FLAG_FILE_PATH, # Remove the temp file
  ...
  def __init__(self, port, duration, timeout=1,
    ...
    self.duration = duration             # Time to keep chargen running
    self.cr50_workload = cr50_workload   # Boolean whether to stress CR50 or not

    # Initialize logging object
    self.logger = logging.getLogger(type(self).__name__ + '| ' + port)

    # Initialize thread object that captures the UART output
    self.test_thread = threading.Thread(target=self.stress_test_thread)

    self.dev_prof = {}                # Dictionary of device profile
    self.cleanup_cli = []             # Command List to cleanup the test
    self.test_cli = []                # Command list to run for chargen test
    self.eol = CRLF                   # Characters to add at the end of input
    self.num_ch_exp = 0               # Expected number of characters in output
    self.num_ch_cap = 0               # Number of captured characters in output
    self.char_loss_occurrences = 0    # Number of character loss occurrence
  ...
  def cleanup(self):
    ...
    try:
      ...
      self.logger.error('Cannot open: ' + str(e))
    finally:
  ...
  def prepare(self):
    ...
    Raises:
      ChargenTestError if UART source can't identified.
    """
    try:
      ...
      for dev_prof in self.UART_DEV_PROFILES:
        ...
        raise ChargenTestError(self.serial.port + ': Got an unknown prompt' +
                               ' text: \"' + prompt_txt + '\"')

      self.logger.debug('Detected as ' + self.dev_prof['device_type'] + ' UART')
      ...
      self.run_command(['chargen 1 4'], delay=1)
      tmp_txt = self.get_output()

      # Check whether chargen command is available.
      if '0000' not in tmp_txt:
        raise ChargenTestError(self.serial.port + ': Chargen got an' +
                               ' unexpected result: \'' + tmp_txt + '\'')
  ...
  def stress_test_thread(self):
    ...
    try:
      ...
      if self.dev_prof['device_type'] == 'AP' and self.cr50_workload:
        ...
        self.logger.debug('run TPM job while ' + FLAG_FILE_PATH + ' exists')
        self.get_output()      # drain data
      ...
      err_msg = 'Expected \'{0}\' but got \'{1}\' after {2} char received'
      ...
      ch_exp = ''       # Null char as initial value.
      ...
      while self.num_ch_cap < total_num_ch:
        ...
        if captured:
          ...
          data_starve_count += 1
          if data_starve_count > 10:
            # If nothing was captured more than 10 times, then just terminate
            # the test.
            break

        for ch_cap in captured:
          if (ch_cap in CRLF) or (self.num_ch_cap >= total_num_ch):
            # If it meets a CR or LF, or the more characters then expected are
            # captured, terminates the test.
          ...
          if not ch_cap.isalnum():
            # If it is not alpha-numeric, then the character output is broken.
            self.logger.error('Broken char captured: ' +
                              err_msg.format(ch_exp, ch_cap, self.num_ch_cap))
            self.char_loss_occurrences += 1
            ch_exp = ''
            continue
          elif ch_exp and ch_exp != ch_cap:
            # If it is alpha-numeric but not continuous, then some characters
            # are lost.
            self.logger.error('Detected character loss: ' +
                              err_msg.format(ch_exp, ch_cap, self.num_ch_cap))
            self.char_loss_occurrences += 1
          ...
          ch_exp = CHARGEN_TXT[(CHARGEN_TXT.find(ch_cap) + 1) % \
                    CHARGEN_TXT_LEN]
