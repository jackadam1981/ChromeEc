...
CR = '\r'
LF = '\n'
TPM_CLI = 'trunks_client --key_create --rsa=2048 --usage=sign' \
          ' --key_blob=/tmp/blob &> /dev/null'
CR50_LOAD_GEN_CLI = 'while true; do   %s; done &' % TPM_CLI
...
class UartSerial(object):
  ...
  def cleanup(self):
    ...
    try:
      ...
      self.logger.error(port + ': Cannot open: ' + str(e))
    finally:
  ...
  def prepare(self):
    ...
    Raises:
      ChargenTestError if UART source cannot not identified.
    """
    try:
      self.serial.open()
      ...
      dev_prof = {}
      for p in self.TEST_PROFILE_DICT:
        if p['prompt'] in prompt_txt:
          dev_prof = p
          break
      else:
  ...
  def stress_test_thread(self):
    ...
    try:
      ...
      if self.is_ap and self.cr50_workload:
        self.run_command([CR50_LOAD_GEN_CLI])
        tmp_txt = self.get_output()      # drain data
  ...
  def get_result(self):
    ...
    self.logger.info(self.serial.port + ': %d char lost / %d (%.1f %%),' \
        ' in %d occurrence(s)' % (char_lost, self.num_ch_exp,
