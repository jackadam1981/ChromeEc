...
class UartSerial(object):
  ...
  def get_output(self, stop_char=''):
    ...
    Args:
      stop_char: Read output buffer until it reads any of stop_chars.
  ...
  def prepare(self):
    ...
    try:
      ...
      for dev_prof in self.UART_DEV_PROFILES:
        ...
        if prompt_txt:
          raise ChargenTestError(self.serial.port + ': Got an unknown prompt' +
                                 ' text: \"' + prompt_txt + '\"\n' +
                                 'Check manually whether ' + self.serial.port +
                                 ' is available.')
        else:
          raise ChargenTestError(self.serial.port + ': Got no input.' +
                                 'Close any other connections to this port,' +
                                 ' and try it again.')

      self.logger.info('Detected as ' + self.dev_prof['device_type'] + ' UART')
      ...
      if '0000' not in tmp_txt:
        raise ChargenTestError(self.dev_prof['device_type'] + ': Chargen got' +
                               ' an unexpected result: \'' + tmp_txt + '\'')
  ...
  def stress_test_thread(self):
    ...
    try:
      ...
      if self.dev_prof['device_type'] == 'AP' and self.cr50_workload:
        ...
        self.logger.debug('run TPM job while ' + FLAG_FILENAME + ' exists')

      # Run the command 'chargen', one time
      self.get_output()    # Drain the output
      self.run_command(self.test_cli)
      self.get_output(LF)    # Drain the echoed command line.
          ...
          if not ch_cap.isalnum():
            if ch_cap != '\0':
              ...
              self.logger.error('Broken char captured: ' +
                                err_msg.format(ch_exp, hex(ord(ch_cap)),
                                               self.num_ch_cap))
              self.char_loss_occurrences += 1
            ch_exp = ''
            continue

          if ch_exp and ch_exp != ch_cap:
            # If it is alpha-numeric but not continuous, then some characters
            # are lost.
            self.logger.error('Detected char loss: ' +
                              err_msg.format(ch_exp, '\'' + ch_cap +'\'',
                                             self.num_ch_cap))
            self.char_loss_occurrences += 1
  ...
  def get_result(self):
    ...
    if self.num_ch_exp < self.num_ch_cap:
      raise ChargenTestError(self.dev_prof['device_type'] + ': UART output' +
                             ' is corrupted.')
