...
def get_legacy_ec_make_vars(board):
    ...
    with futures.ThreadPoolExecutor() as executor:
        for board_dir in board_dirs:
            make_vars_futures.append(
                executor.submit(get_legacy_ec_make_vars, board_dir.name)
            )
...
def find_all_projects():
    ...
    zephyr_projects = dict(find_zephyr_ec_projects())
    legacy_projects = dict(find_legacy_ec_projects())
...
def gen_bazel_lines():
    ...
    for name, args in sorted(find_all_projects().items()):
        yield "    ec_target("
        yield f"        name = {name!r},"
...
def main(argv: Optional[List[str]] = None) -> Optional[int]:
    ...
    parser.add_argument(
        ...
        "--output-file",
        type=Path,
        default=EC_DIR / "bazel" / "all_targets.bzl",
