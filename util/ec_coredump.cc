...
static struct mem_segment *get_segment(uint16_t index)
{
	...
	while (offset < segment->size) {
		...
		if (!memcpy(segment->mem + offset, ec_inbuf, rv)) {
...
int merge_segments(struct mem_segment *root)
{
	...
	while (current && current->next) {
		...
		if (!memcpy(current->mem + current->size, next->mem + overlap,
			    next->size - overlap)) {
			std::cerr << "Error: memcpy failed" << std::endl;
			return -1;
		}
...
void print_help()
{
	std::cout << "Usage: prog_name [OPTIONS] OUTPUT_PATH\n"
		  << "Options:\n"
...
int main(int argc, char *argv[])
{
	...
	if (verbose)
		std::cout << "Finalizing coredump at "
			  << coredump_output_filename << "..." << std::endl;
