...
static int get_panic_info(struct panic_data *pdata)
{
	...
	size = ec_command(EC_CMD_GET_PANIC_INFO, 1, &params, sizeof(params),
	...
	if (size > sizeof(*pdata)) {
		if (verbose) {
			std::cerr << "Warning: Panic larger than expected"
				  << std::endl;
		}
		size = sizeof(*pdata);
	}
	...
	if (pdata->reserved != 0) {
		std::cerr << "Error: Unexpected panic reserve value: "
...
struct mem_segment *get_segments(void)
{
	...
	for (uint16_t entry_index = 0; entry_index < entry_count;
		...
		if (rv < 0) {
			std::cerr
				<< "Error: Failed to get memory dump info for entry "
				<< std::endl;
			goto cmd_memory_dump_cleanup;
		}
		...
		if (verbose)
			std::cout << "Entry info: " << std::endl
				  << "\tStart address: " << std::hex
				  << entry_info_response.address << std::endl
				  << "\tEnd address: " << std::hex
				  << entry_address_end << std::endl
				  << "\tSize: " << std::hex
				  << entry_info_response.size << std::endl;
		...
		while (offset < seg->size) {
			...
			if (rv <= 0) {
				std::cerr
					<< "Error: Failed to read EC memory at "
	...
	for (int i = 0; seg && seg->next && i < entry_count; i++) {
		...
		if (new_size != seg->next->addr_end - seg->addr_start) {
			std::cerr << "Error: Segment size is not aligned"
				  << std::endl;
			goto cmd_memory_dump_cleanup;
		}
...
int main(int argc, char *argv[])
{
	...
	if (verbose) {
		std::cout << "Opened output file: '" << output_filename
			  << std::endl;
	}
	...
	switch (pdata.arch) {
	case PANIC_ARCH_CORTEX_M:
		rv = write_zephyr_coredump_cortex_arch_info(&pdata,
							    output_file);
		break;
	default:
	...
	while (current_segment != nullptr) {
		rv = write_zephyr_coredump_memory_block(current_segment,
							output_file);
		if (rv != 0) {
			std::cerr
	...
	if (verbose) {
		std::cout << "Closed output file: '" << output_filename << "'"
			  << std::endl;
	}
