...
class FlashPD(client.ServoClient):
  ...
  def flash_command(self, cmd, expect='DONE'):
    ...
    Args:
      cmd    : string of 'pd flash' command to execute
      expect : string of expected response after 'cmd'

    Returns:
      found : boolean, whether response matches expected.
    """
    self._serial.write('pd flash %s\n' % cmd)
    (found, _) = self.expect(expect)
    return found

  def get_version(self):
    """Retreive PSU firmware version.

    Looks like: 'version: zinger_v1.1.1917-046'
    ...
    """
    self._serial.write('pd flash version\n')
    (found, line) = self.expect('version:')
    if found:
      logging.debug('is version in: %s', line)
      m = re.match(r'.*version:\s+(\w+_v\d+\.\d+\.\d+-\d+).*', line)
      return m.group(1)
    else:
...
def flash_pd(options):
  ...
  done = ec.flash_command('rw_hash', expect=ERASED_RW_HASH)
  if done:
    done = ec.expect('DONE')
