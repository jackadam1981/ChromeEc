...
class FlashPD(client.ServoClient):
  ...
  def do_command(self, cmd, expect='DONE 0', retries=2, ignore_fail=False):
  ...
  def flash_command(self, cmd, expect='DONE 0', retries=2, ignore_fail=False):
    ...
    return self.do_command(flash_cmd, expect, retries, ignore_fail)
...
def flash_pd(options):
  ...
  if done:
    ...
    # UFP devices share their hash upon PD connect so check the hash that way.
    (done, _) = ec.do_command('hash', expect=ERASED_RW_HASH)
