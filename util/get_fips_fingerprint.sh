...
main() {
  local objcopy="${1}"
  local rw_flat="${2}"
  local build_dir=$(dirname "${rw_flat}")
  local rw_ext="_B"
  ...
  local offset=0x$( objdump -h "${rw_elf}" | awk '/ .text / {print $4}' )
  local sections=$( objdump -t "${rw_elf}" )
  ...
  local fips_start=0x$( echo "${sections}" | \
		awk '/__fips_module_start/ {print $1}' )
  local fips_end=0x$( echo "${sections}" |\
		awk '/__fips_module_end/ {print $1}' )
  local size=$((fips_end - fips_start))
  local fips_offset=$((fips_start - offset))

  local result=$(dd if="${rw_flat}" skip="${fips_offset}" count="${size}" \
		bs=1 | sha256sum)
