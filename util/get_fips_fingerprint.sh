...
main() {
  ...
  if [ "$(basename "${rw_elf}")" = "ec.RW.elf" ] ; then
    rw_ext=""
  fi

  build_dir=$(dirname "${rw_elf}")
  local checksum_section=text.fips_checksum
  local fips_checksum=${build_dir}/ec.RW${rw_ext}.fips.checksum
  local fips_checksum_dump=${fips_checksum}.dump
  local rw_flat=${build_dir}/ec.RW${rw_ext}.flat.prefips
