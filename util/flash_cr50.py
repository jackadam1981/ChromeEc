...
def run_cmd(cmd, check_error=True):
    ...
    """
    try:
        result = cros_build_lib.run(shlex.split(cmd),
                                    check=check_error,
                                    print_cmd=True,
                                    capture_output=True,
                                    ...
                                    log_output=True)
    except cros_build_lib.RunCommandError, cmd_error:
        if check_error:
            raise
...
class Servo(object):
    ...
    def dut_control(self, cmd, check_error=True, wait=False):
        ...
        exit_status, output = run_cmd(dut_control_cmd, check_error)
        if wait:
...
class FlashCr50(object):
    ...
    def update_command_runs(cmd):
        ...
        exit_status, output = run_cmd('%s -h' % cmd, check_error=False)
        if 'Usage' in output:
            return 0
        if exit_status:
...
class GsctoolUpdater(FlashCr50):
    ...
    def update(self, image):
        ...
        exit_status, output = run_cmd(update_cmd, check_error=False)
