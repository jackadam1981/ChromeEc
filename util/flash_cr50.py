...
def run_cmd(cmd, raise_error=True):
    ...
    try:
        output = subprocess.check_output(split_cmd, stderr=subprocess.STDOUT)
        return 0, output.strip()
    except subprocess.CalledProcessError, process_error:
        if raise_error:
            raise
        return process_error.returncode, process_error.output.strip()
    except OSError, os_error:
        if raise_error:
            raise
        return os_error.errno, os_error.strerror
...
class Cr50Image(object):
    ...
    def __init__(self, image, artifacts_dir):
        ...
        self._update_file = self._original_image
    ...
    def _generate_file_names(self):
        ...
        suffix = ''.join([random.choice(string.ascii_letters) for _ in
                          range(self.SUFFIX_LEN)])
        name = self.RW_NAME_BASE + suffix
        ...
        self._tmp_cr50_bin = os.path.join(self._artifacts_dir,
                                          name + '.orig.bin')
        self._original_ext = os.path.splitext(self._original_image)[1]


    def _original_is_ext(self, ext):
        """Returns True if the original has the file extension in the filename.

        Release images won't have .bin at the end of the filename. They have the
        .phase extension. If ext doesn't match the file extension. Check if it
        is somewhere else in the image.

        Args:
            ext: The file extension.
        """
        return self._original_ext == ext or (ext + '.') in self._original_image


    def _original_is_bin(self):
        """Returns True if .bin is in original image filename."""
        return self._original_is_ext('.bin')


    def _original_is_hex(self):
        """Returns True if .hex is in original image filename."""
        return self._original_is_ext('.hex')


    def extract_rw_a_hex(self):
        """Extract RW_A from the original image in .hex form."""
        if self._original_is_bin():
            self._save_rw_a_hex_from_bin(self._original_image)
        elif self._original_is_hex():
            self._save_original_hex(self._original_image)
        else:
            raise FlashCr50Error('Unsupported image format %r use .bin or .hex'
                                 % self._original_ext)


    def _save_original_hex(self, image):
        """The original image is .hex. Save it."""
        run_cmd('cp %s %s' % (image, self._tmp_rw_hex))


    def _save_rw_a_hex_from_bin(self, image):
        """Convert the image from .bin to .hex."""
        run_cmd('cp %s %s' % (image, self._tmp_cr50_bin))
    ...
    def get_cr50_bin(self):
        """Get the filename of the update image."""
        if not self._original_is_bin():
            raise FlashCr50Error('Unsupported image format %r use .bin')
...
class Servo(object):
    ...
    def dut_control(self, cmd, raise_error=True, wait=False):
        ...
        Returns:
            ...
            the command failed and raise_error is False.

        Raises:
            subprocess.CalledProcessError if the command fails and raise_error
            is true.
        """
        dut_control_cmd = 'dut-control %s -p %s' % (cmd, self._port)
        exit_status, output = run_cmd(dut_control_cmd, raise_error)
...
class Cr50Reset(object):
    ...
    def verify_setup(self):
        ...
        for required_control in required_controls:
            ...
            if returncode:
                ...
                setup_issues.append('%s: need %s running %s' %
        ...
        if setup_issues:
            raise FlashCr50Error('Cannot run update with %s due to setup '
                                 'issues: %s' %
    ...
    def enter_reset(self):
        ...
        try:
            self.run_reset()
        except Exception, reset_error:
            logging.warn('%s enter reset failed: %s', self._reset_name,
                         str(reset_error))
            raise
    ...
    def exit_reset(self):
        ...
        try:
            ...
            logging.warn('%s exit reset failed: %s', self._reset_name,
                         str(reset_error))
            raise
...
class FlashCr50(object):
    ...
    def __init__(self, cmd):
        ...
        Raises:
            FlashCr50Error if no valid updater command was found.
        """
        updater = self.get_updater(cmd)
        if not updater:
            emerge_msg = (('Try emerging ' + self.PACKAGE) if self.PACKAGE
                          else '')
            raise FlashCr50Error('Could not find %s command.%s' %
                                 (self, emerge_msg))
    ...
    def get_updater(self, cmd):
        ...
        Returns:
            ...
            command.
        """
        if not self.update_command_runs(cmd):
            return cmd

        use_default = (self.DEFAULT_UPDATER and
                       not self.update_command_runs(self.DEFAULT_UPDATER))
        if use_default:
...
class GsctoolUpdater(FlashCr50):
    ...
    def __init__(self, cmd, serial):
        ...
        Args:
            ...
            serial: The serialnumber of the CCD device being updated.
        """
    ...
    def update(self, image):
        ...
        """
        update_file = image.get_cr50_bin()
        update_cmd = '%s %s' % (self._gsctool_cmd, update_file)
        exit_status, output = run_cmd(update_cmd, raise_error=False)
        ...
        raise FlashCr50Error('gsctool update error: %s' %
                             output.splitlines()[-1])
...
class Cr50RescueUpdater(FlashCr50):
    ...
    def start_rescue_process(self, update_file):
        ...
        rescue_cmd = self._rescue_cmd % (update_file, pty)
...
def parse_args():
    ...
    parser.add_argument('-i', '--image', type=str,
                        help='Cr50 image path or release type. Use %s to use '
                        'the current release.' %
                        (' or '.join(RELEASE_PATHS.keys())),
                        required=True)
    parser.add_argument('-c', '--updater_cmd', type=str, default='gsctool',
    ...
    parser.add_argument('-p', '--port', type=str, default='',
                        ...
                        'rescue).')
    parser.add_argument('-r', '--reset_type', default='battery_cutoff',
                        type=str, help='Cr50 has to be reset during rescue. '
                        'This is the method for cr50 reset. One of %s' %
                        SUPPORTED_RESETS)
...
def get_updater(args):
    """Get the updater object from the command line args."""
    if 'cr50-rescue' in args.updater_cmd:
        if not args.port:
            raise FlashCr50Error('Servo port is required for cr50 rescue')
        return Cr50RescueUpdater(args.updater_cmd, args.port, args.reset_type)
    if 'gsctool' in args.updater_cmd:
        return GsctoolUpdater(args.updater_cmd, args.serial)
    raise FlashCr50Error('Unsupported update command %r' % args.updater_cmd)
...
def main_function():
    ...
    image = Cr50Image(RELEASE_PATHS.get(args.image, args.image),
                      args.artifacts_dir)
    flash_cr50 = get_updater(args)

    logging.info('Using %r to update to %s', flash_cr50,
