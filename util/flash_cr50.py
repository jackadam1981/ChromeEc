...
class Cr50Image(object):
    ...
    def _save_original_hex(self, image):
        ...
        run_command(['cp', image, self._tmp_rw_hex])


    def _save_rw_a_hex_from_bin(self, image):
        """Convert the image from .bin to .hex."""
        run_command(['cp', image, self._tmp_cr50_bin])
...
class Cr50Reset(object):
    ...
    def enter_reset(self):
        ...
        try:
            ...
            logging.warning('%s enter reset failed: %s', self._reset_name,
                            str(reset_error))
            raise
    ...
    def exit_reset(self):
        ...
        try:
            ...
            logging.warning('%s exit reset failed: %s', self._reset_name,
                            str(reset_error))
            raise
...
class GsctoolUpdater(FlashCr50):
    ...
    def update(self, image):
        ...
        update_cmd = [self._gsctool_cmd, update_file]
...
def parse_args():
    ...
    parser.add_argument('-i', '--image', type=str,
                        ...
                        'the current release.' %
                        (' or '.join(list(RELEASE_PATHS.keys()))),
                        required=True)
