...
class Cr50Image(object):
    ...
    def extract_rw_a_hex(self):
        ...
        if self._original_is_bin():
            ...
            raise Error('Unsupported image format %r use .bin or .hex' %
                        self._original_ext)
    ...
    def get_cr50_bin(self):
        ...
        if not self._original_is_bin():
            raise Error('Unsupported image format %r use .bin')
...
class Cr50Reset(object):
    ...
    def verify_setup(self):
        ...
        if setup_issues:
            raise Error('Cannot run update with %s due to setup issues: %s' %
...
class Cr50RescueUpdater(FlashCr50):
    ...
    def get_cr50_reset(self, reset_type):
        ...
        raise Error('Unsupported cr50 reset method %r' % reset_type)
...
def main_function():
    ...
    image = Cr50Image(RELEASE_PATHS.get(opts.image, opts.image),
                      opts.artifacts_dir)
