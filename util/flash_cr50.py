...
class Cr50Image(object):
    ...
    def __init__(self, image, artifacts_dir):
        ...
        file_type = run_command(['file', image, '-b'])[1]
        logging.info('File type: %s', file_type)
        if 'data' not in file_type:
            raise Error('Use binary image not %r' % file_type)
    ...
    def _save_rw_a_hex_from_bin(self, image):
        """Extract RW_A.hex from the image and save it to _tmp_rw_hex."""
...
def parse_args(argv):
    ...
    parser.add_argument('-i', '--image', type=str,
                        help='Path to cr50 binary image or release type. Use '
                        '%s to use the current release.' %
