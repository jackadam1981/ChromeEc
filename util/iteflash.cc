...
/* Get E-flash Size for 51xxx */
static int it5x_get_eflash_size(struct common_hnd *chnd, uint8_t *eflash_size)
{
	int ret = 0;
...
static int check_chipid(struct common_hnd *chnd)
{
	...
	if (chnd->it5x) {
		...
		if (chnd->flash_size)
			chnd->flash_size = chnd->flash_size << 10;
