...
def process_data(servo, data):
  to_press = convert_input(data)

  if not to_press:
    return

  cmds = ("kbpress %d %d %d; waitms 0" % (col, row, val)
          for (row, col, val) in to_press)
  servo.set('ec_uart_multicmd', ';'.join(cmds))

def main(port, to_send=""):
  servo = xmlrpclib.ServerProxy("http://localhost:%s" % port)

  if to_send:
    process_data(servo, to_send)
    return
