...
"""Provides a console through J-Link RTT.
...
def rtt_client(jlink_exe, ip, device, interface, cmd_file):
    cmd = [
        jlink_exe,
    ]
    ...
    jlink_process = subprocess.Popen(cmd)
    time.sleep(10)

    rtt_client_cmd = ['JLinkRTTClient']
    logging.debug('Running command: "%s"', ' '.join(rtt_client_cmd))
    rtt_process = subprocess.run(rtt_client_cmd)

    # TODO: kill jlink

    logging.debug('JLink return code: %d', rtt_process.returncode)
    return rtt_process.returncode
...
def main(argv: list):
    ...
    ret_code = rtt_client(args.jlink, args.ip, config.device, config.interface,
                          cmd_file.name)
    cmd_file.close()
    return ret_code
