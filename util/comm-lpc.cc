...
static int read_byte_indexed_io(int port, char offset)
{
	outb(offset, port);
	return inb(port + 1);
}
...
int comm_init_lpc(void)
{
	...
	if (inb(EC_LPC_ADDR_MEMMAP + EC_MEMMAP_ID) != 'E' ||
		...
		if (read_byte_indexed_io(EC_LPC_ADDR_MEMMAP_INDEXED_IO,
					 EC_MEMMAP_ID) != 'E' ||
		    read_byte_indexed_io(EC_LPC_ADDR_MEMMAP_INDEXED_IO,
					 EC_MEMMAP_ID + 1) != 'C') {
