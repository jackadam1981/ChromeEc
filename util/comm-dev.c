			     ...
			     void *indata, int insize)
{
	...
	/*
	 * Retry if result is EC_RES_IN_PROGRESS.
	 * Newer CrOS EC driver does not set command result and instead
	 * sets errno to EINPROGRESS, so check for both cases.
	 */
	if (r < 0 &&
	    (s_cmd->result == EC_RES_IN_PROGRESS || errno == EINPROGRESS)) {
		s_cmd->command = EC_CMD_RESEND_RESPONSE;
		r = ioctl(fd, CROS_EC_DEV_IOCXCMD_V2, &s_cmd);
	}
	...
	if (r < 0) {
		/*
		 * Newer CrOS EC driver does not set s_cmd->result on error.
		 * In this case, infer a result from errno.
		 */
		...
		/*
		 * If the result is unexpectedly unset, set result to
		 * EC_RES_UNAVAILABLE.
		 */
		if (result == EC_COMMAND_RESULT_UNINITIALIZED)
