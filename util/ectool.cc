...
static int get_expected_fan_ratio(int temp, int fan_off, int fan_max)
{
	if (temp < fan_off)
		return 0;
	if (temp > fan_max)
...
static int cmd_temperature_print(int id, int mtemp)
{
	...
	if (rc >= 0 && r.temp_fan_off && r.temp_fan_max)
		printf("  %10d%% (%dK and %dK)",
		       get_expected_fan_ratio(temp, r.temp_fan_off,
		       r.temp_fan_max), r.temp_fan_off, r.temp_fan_max);
	else
		printf("         error(%d) (%dK and %dK)", rc, r.temp_fan_off,
		       r.temp_fan_max);
