...
constexpr char kFpCaptureCacheFile[] = "/tmp/ectool_fp_capture_type";
		  ...
		  struct TemplateInfo &template_info, int index)
{
	...
	if (images.size() == 1) {
		...
		bool found = false;
		FILE *f = fopen(kFpCaptureCacheFile, "r");
		if (f) {
...
int cmd_fp_mode(int argc, char *argv[])
{
	...
	if (mode & FP_MODE_CAPTURE) {
		...
		int fd = open(kFpCaptureCacheFile, O_WRONLY | O_CREAT | O_TRUNC,
			      0600);
		if (fd != -1) {
			char buf[16];
			...
			if (written < 0) {
				fprintf(stderr,
					...
					kFpCaptureCacheFile, strerror(errno));
			}
			close(fd);
		} else {
			fprintf(stderr,
