...
static void ppm_dump_cci(const struct ucsi_cci *cci)
{
	...
	printf("\tConnector Change = ");
	if (cci->connector_changed)
		printf("%d (USB-C%d)\n",
		       cci->connector_changed, cci->connector_changed - 1);
	else
		printf("None\n");
...
int cmd_ppm(int argc, char *argv[])
{
	...
	if (argc == 2 && !strcmp(argv[1], "reset")) {
		...
		if (argc == 3 && !strcmp(argv[2], "cap")) {
		...
		} else if (argc == 4 && !strcmp(argv[2], "conn")) {
			conn = strtoul(argv[3], &e, 0);
			if ((e && *e) || conn > 2) {
				fprintf(stderr, "ERROR: Invalid arg: '%s'\n",
					argv[3]);
				return -1;
			...
			struct ucsi_control ctrl = {
				.command = UCSI_CMD_GET_CONNECTOR_CAPABILITY,
				.data_length = 0,
				.command_specific = { conn },
			};

			if (ppm_set_control(&ctrl) < 0)
				return -1;
			printf("Successfully executed GET_CONNECTOR_CAPABILITY for Connector %u.\n", conn);
		}
	} else {
		fprintf(stderr,
			"Sub-command '%s' is unknown or number of args is invalid.\n",
			argv[1]);
		return -1;
	}
