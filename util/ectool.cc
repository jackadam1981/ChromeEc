...
struct board_batt_params cmd_get_battery_config_in_board_batt_params(
...
static int cmd_battery_config_set(int argc, char *argv[])
{
	...
	struct board_batt_params battery_config =
		cmd_get_battery_config_in_board_batt_params(
			root_dict_identifier);
	...
	struct batt_conf_header battery_config_header;
	battery_config_header.struct_version = EC_BATTERY_CONFIG_STRUCT_VERSION;
	battery_config_header.manuf_name_size = strlen(manuf_name);
	battery_config_header.device_name_size = strlen(device_name);

	int mem_size = sizeof(struct batt_conf_header) +
		       battery_config_header.manuf_name_size +
		       battery_config_header.device_name_size +
		       sizeof(battery_config);
	uint8_t *p = (uint8_t *)malloc(size);
	memcpy(p, &battery_config_header, sizeof(struct batt_conf_header));
	p += sizeof(battery_config_header);
	memcpy(p, manuf_name, battery_config_header.manuf_name_size);
	p += battery_config_header.manuf_name_size;
	memcpy(p, device_name, battery_config_header.device_name_size);
	p += battery_config_header.device_name_size;
	memcpy(p, &battery_config, sizeof(battery_config));
	p += sizeof(battery_config);
	int rv = ec_command(EC_CMD_SET_CROS_BOARD_INFO, 0, p - mem_size,
			    mem_size, NULL, 0);
	if (rv < 0) {
		if (rv == -EC_RES_ACCESS_DENIED - EECRESULT)
