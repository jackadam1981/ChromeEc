...
static void cmd_battery_config_help(char *cmd)
{
	fprintf(stderr,
		...
		"Usage: %s <file_path> <manuf_name> [<device_name>]\n"
		"    Write battery config from file to CBI.\n"
		"\n"
		"    file_path: Path to Json file containing battery configs\n"
		"    manuf_name: Manufacturer's name. Up to 31 chars.\n"
		"    device_name: Battery's name. Up to 31 chars.\n"
		"    See `ectool battery` for <manuf_name> and <model_name>\n",
		cmd, cmd);
}
...
static int cmd_battery_config_set(int argc, char *argv[])
{
	...
	if (root_dict == nullptr) {
		fprintf(stderr,
			"Matching config for identifier=%s not found in %s.\n",
	...
	read_battery_config_from_json(root_dict, &config);
