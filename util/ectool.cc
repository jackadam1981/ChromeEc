...
#define FP_FRAME_INDEX_IMAGE_CAPTURE -1
		  ...
		  uint8_t capture_type = FP_CAPTURE_VENDOR_FORMAT)
{
	...
	size_t size;
	if (index == FP_FRAME_INDEX_IMAGE_CAPTURE) {
...
int cmd_fp_frame(int argc, char *argv[])
{
	...
	int idx = FP_FRAME_INDEX_IMAGE_CAPTURE;

	if (argc != 2) {
		fprintf(stderr,
			"Only one capture type is allowed. The number of arguments is wrong '%d'\n",
			argc);
		return -1;
	}
	...
	for (int i = 1; i < argc; i++) {
		if (!strcmp(argv[i], "vendor"))
			capture_type = FP_CAPTURE_VENDOR_FORMAT;
		else if (!strcmp(argv[i], "pattern0"))
			capture_type = FP_CAPTURE_PATTERN0;
		else if (!strcmp(argv[i], "pattern1"))
			capture_type = FP_CAPTURE_PATTERN1;
		else if (!strcmp(argv[i], "qual"))
			capture_type = FP_CAPTURE_QUALITY_TEST;
		else if (!strcmp(argv[i], "test_reset"))
			capture_type = FP_CAPTURE_RESET_TEST;
		else if (!strcmp(argv[i], "test_defect_pxl"))
			capture_type = FP_CAPTURE_DEFECT_PXL_TEST;
		else if (!strcmp(argv[i], "test_abnormal"))
			capture_type = FP_CAPTURE_ABNORMAL_TEST;
		else if (!strcmp(argv[i], "test_noise"))
			capture_type = FP_CAPTURE_NOISE_TEST;
		else {
			fprintf(stderr,
				"Warning: Unknown argument '%s', aborting\n",
				argv[i]);
			return -1;
		}
	}
	...
	if (sensor_image.frame_size > sensor_image.width * sensor_image.height *
					      ceil(sensor_image.bpp / 8.0)) {
