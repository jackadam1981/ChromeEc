...
static void cmd_cbi_bin_help(char *cmd)
{
	fprintf(stderr,
		...
		"    read from cbi in flash into file.\n"
		"  Usage: %s write <file> <size>\n",
		"    write from file into cbi flash.\n"
		cmd, cmd);
}
...
static int cmd_cbi_bin(int argc, char *argv[])
{
	...
	if (!strcasecmp(argv[1], "read")) {
		...
		for (i = 0; i < size; i += packet_max_size) {
			...
			rv = ec_command(EC_CMD_CBI_BIN_READ, 0, &p,
					sizeof(p), ec_inbuf, ec_max_insize);
			if (rv < 0) {
				fprintf(stderr, "Error code: %d\n", rv);
				return rv;
			...
			if (rv == 0) {
				fwrite((uint8_t *)ec_inbuf), sizeof(*ec_inbuf), p.size, fp);
			}
		}
		...
		for (i = 0; i < size; i += packet_max_size) {
			...
			if (fread(buffer, sizeof(*buffer), p->size, fp)) {
				p->data = buffer;
			} else { /* file size is smaller than size specified by command so pad with 0s */
				p->data = {0, 0, 0, 0};
			}
