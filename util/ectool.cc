...
static void cmd_memory_dump_usage(int argc, char *argv[])
{
	fprintf(stderr,
		"Usage:\n"
		"\t%s info\n"
...
static int cmd_memory_dump(int argc, char *argv[])
{
	...
	if (argc > 3 || (argc == 2 && strcmp(argv[1], "help") == 0)) {
		cmd_memory_dump_usage(argc, argv);
		return -1;
	}
	...
	if (argc == 3 && !just_info) {
		...
		requested_address_end = fmin(
			requested_address_start + requested_size, UINT32_MAX);
	}
	...
	for (uint16_t entry_index = 0; entry_index < entry_count;
		...
		/* Clip memory segment boundries based on requested range */
		seg->addr_start = fmax(entry_info_response.address,
				       requested_address_start);
		seg->addr_end = fmin(entry_address_end, requested_address_end);
	...
	while (seg && seg->next) {
		...
		assert(new_size == seg->next->addr_end - seg->addr_start);
