...
static int cmd_battery_config_hex(int argc, char *argv[])
{
	if (argc != 3) {
		fprintf(stderr, "Usage: %s <file_path> <battery_identity>\n",
			argv[0]);
		return -1;
	}
	FILE *fp;
	...
	base::Value::Dict *root_dict_identity = root_dict->FindDict(argv[2]);
	base::Value::Dict *fuel_gauge =
		root_dict_identity->FindDict("fuel_gauge");
	...
	base::Value::Dict *batt_info =
		root_dict_identity->FindDict("batt_info");
	...
	printf(cmd_convert_to_hex(battery_config).c_str());
	return 0;
}
...
const struct command commands[] = {
	...
	{ "button", cmd_button },
	{ "battery_config_hex", cmd_battery_config_hex },
	{ "cbi", cmd_cbi },
