...
static int get_battery_command_common(
	uint8_t index,
...
static int get_battery_command_v2(uint8_t index)
{
	...
	return get_battery_command_common(index, &static_r);
}
...
static int get_battery_command_v1(uint8_t index)
{
	...
	return get_battery_command_common(index, &static_v2);
}
...
int cmd_battery(int argc, char *argv[])
{
	 ...
	 */
	if (ec_cmd_version_supported(EC_CMD_BATTERY_GET_STATIC, 2))
		return get_battery_command_v2(index);
	else if (ec_cmd_version_supported(EC_CMD_BATTERY_GET_STATIC, 1))
