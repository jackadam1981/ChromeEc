...
static int cmd_battery_config(int argc, char *argv[])
{
	...
	/* Print statically known parameters in case ec_command fail. */
	printf("batt_conf_header size = %lu\n", sizeof(r));
	printf("board_batt_params size = %lu\n", sizeof(r.config));
	printf("compatible struct versions: 0x00 ~ 0x%02x\n", struct_version);

	rv = ec_command(EC_CMD_BATTERY_CONFIG, 0, NULL, 0, &r, sizeof(r));
	if (rv < 0)
		return rv;

	printf("batt_conf_header version = 0x%02x\n", r.struct_version);
	printf("\n");

	if (r.struct_version > struct_version) {
		fprintf(stderr, "Returned struct version is incompatible.\n");
		return -1;
	}
	batt_conf_dump(&r);
