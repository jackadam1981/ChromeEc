...
Upload the image file:
   ...
   (chroot) ./util/fpmcu_upload.py -i /tmp/fpimages/img1.png -b bloonchipper
"""
import argparse
...
def split_hex_str(hex_str: str, chunk_size: int) -> List[Tuple[int, str]]:
    ...
    hex_str is expected to contains hex values represented with two
    characters each.
    ...
    if len(hex_str) % 2:
        raise ValueError("Input string does not represet hex values")
...
class TestGetFpuploadCmds(unittest.TestCase):
    ...
    def test_valid_size(self):
        ...
        self.assertEqual(cmds[-1].split()[0], "fpupload")
        # The image has 10752 pixels
        self.assertEqual(cmds[-1].split()[1], "10740")
