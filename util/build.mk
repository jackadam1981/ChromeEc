...
host-util-bin=ectool lbplay stm32mon ec_sb_firmware_update lbcc \
	ec_parse_panicinfo
build-util-bin=ec_uartd iteflash export_taskinfo.so
ifeq ($(CHIP),npcx)
build-util-bin+=ecst
endif
...
$(out)/util/export_taskinfo.so: BUILD_LDFLAGS += \
			$(out)/util/export_taskinfo_ro.o \
			$(out)/util/export_taskinfo_rw.o \
			--shared -fPIC

$(out)/util/export_taskinfo_ro.o: util/export_taskinfo.so.c
	$(call quiet,c_to_taskinfo,BUILDCC,RO)

$(out)/util/export_taskinfo_rw.o: util/export_taskinfo.so.c
