...
static bool can_act_as_device(void)
{
	...
	was_overridden = get_vif_field_tag_bool(
...
static void vif_output_vif_xml(FILE *vif_file, struct vif_t *vif, int level)
{
	...
	vif_out_field(vif_file, level, &vif->vif_app_field[Vendor]);
	vif_out_field(vif_file, level, &vif->vif_app_field[Name]);
	vif_out_field(vif_file, level, &vif->vif_app_field[Version]);
	vif_out_str(vif_file, --level, "</VIF_App>");
			...
			const char *str_value)
{
	...
	/*
	 * Override already set or trying to set to nthing should do
	 * nothing. Just return
	 */
...
static void init_vif_product_fields(struct vif_field_t *vif_fields)
{
	...
	sprintf(hex_str, "0x%04X", USB_VID_GOOGLE);
			...
			enum dtype type)
{
	...
	if (IS_ENABLED(CONFIG_USB_PD_MANUFACTURER_INFO)) {
		...
		set_vif_field_itss(&vif_fields[Manufacturer_Info_VID_Port],
			...
			sprintf(hex_str, "0x%04X", CONFIG_USB_PID);
		...
		#else
			sprintf(hex_str, "0x%04X", DEFAULT_MISSING_PID);
			set_vif_field_itss(&vif_fields[
				Manufacturer_Info_PID_Port],
				vif_component_name[Manufacturer_Info_PID_Port],
