...
static int32_t realloc_copy_free(char **vif, char *buf, uint32_t len,
							uint32_t *offset)
{
	if (!*vif || !buf) {
	...
	*vif = (char *)realloc(*vif, len + *offset);
	if (!*vif) {
		free(buf);
	...
	sprintf(*vif + *offset, "%s", buf);
	*offset += len;
	free(buf);
	return 0;
}

static int32_t write_pdo_to_buf(char **vif, uint32_t pdo,
				enum dtype type, uint32_t pnum)
{
	...
	if ((pdo & PDO_TYPE_MASK) == PDO_TYPE_FIXED) {
		...
		ret = asprintf(&tmp, "\t%s_PDO_Supply_Type%d: 0\r\n",
		...
		if (realloc_copy_free(vif, tmp, ret, &offset) < 0)
			return -1;

		if (type == SRC) {
			ret = asprintf(&tmp,
				"\tSrc_PDO_Peak_Current%d: 0\r\n", pnum);
			if (ret < 0) {
				fprintf(stderr, "ERROR: Out of memory.\n");
				return -1;
			}
		...
		ret = asprintf(&tmp, "\t%s_PDO_Voltage%d: %d\r\n",
		...
		if (type == SRC)
			ret = asprintf(&tmp, "\tSrc_PDO_Max_Current%d: %d\r\n",
					pnum, current);
		else
			ret = asprintf(&tmp, "\tSnk_PDO_Op_Current%d: %d\r\n",
					pnum, current);
		if (ret < 0) {
			fprintf(stderr, "ERROR: Out of memory.\n");
			return -1;
		...
		ret = asprintf(&tmp, "\t%s_PDO_Supply_Type%d: 1\r\n",
				(type == SRC) ? "Src" : "Snk", pnum);
		if (ret < 0) {
			fprintf(stderr, "ERROR: Out of memory.\n");
			return -1;
		...
		ret = asprintf(&tmp, "\t%s_PDO_Min_Voltage%d: %d\r\n",
		...
		ret = asprintf(&tmp, "\t%s_PDO_Max_Voltage%d: %d\r\n",
		...
		if (type == SRC)
			ret = asprintf(&tmp, "\tSrc_PDO_Max_Power%d: %d\r\n",
						pnum, power);
		else
			ret = asprintf(&tmp, "\tSnk_PDO_Op_Power%d: %d\r\n",
						 pnum, power);
		if (ret < 0) {
			fprintf(stderr, "ERROR: Out of memory.\n");
			return -1;
		...
		ret = asprintf(&tmp, "\t%s_PDO_Supply_Type%d: 2\r\n",
				(type == SRC) ? "Src" : "Snk", pnum);
		if (ret < 0) {
			fprintf(stderr, "ERROR: Out of memory.\n");
			return -1;
					...
					const char *vif_producer)
{
	...
	if (is_drp()) {
		...
		type = SRC;
		sprintf(type_name, "SRC");
	} else if (is_snk()) {
		type = SNK;
		sprintf(type_name, "SNK");
	} else {
		return 1;
	}
