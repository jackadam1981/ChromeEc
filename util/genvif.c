...
static int get_vif_name(enum dtype type, char *name,
			const char *board, const char *out)
{
	char *extpwr = "_extpwr";
 ...
 */
static int gen_vif(enum dtype type, const char *name,
		const char *board, const char *vif_producer)
{
	FILE *vif;
	...
	if (type == DRP || type == SRC) {
		...
		/* Write Source PDOs */
		{
			...
			for (i = 0; i < pd_src_pdo_cnt; i++) {
				pwr = write_pdo_to_vif(vif, pd_src_pdo[i],
							SRC, i+1);
				if (pwr > max_power)
					max_power = pwr;
			}
...
int main(int argc, char **argv)
{
	...
	if (get_vif_name(DRP, name, board, out)) {
		if (gen_vif(DRP, name, board, vif_producer))
			return 1;
	} else {
		if (get_vif_name(SRC, name, board, out))
			if (gen_vif(SRC, name, board, vif_producer))
				return 1;
		if (get_vif_name(SNK, name, board, out))
			if (gen_vif(SNK, name, board, vif_producer))
				return 1;
	}
