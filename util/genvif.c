...
#define DEFAULT_MISSING_BCD_DEVICE	0x0000
			...
			struct vif_field_t *vif_fields)
{
	...
	#if defined(CONFIG_USB_BCD_DEVICE)
		sprintf(hex_str, "%04X", CONFIG_USB_BCD_DEVICE);
		set_vif_field_itss(&vif_fields[bcdDevice_SOP],
			vif_component_name[bcdDevice_SOP],
			CONFIG_USB_BCD_DEVICE, hex_str);
	#else
		sprintf(hex_str, "%04X", DEFAULT_MISSING_BCD_DEVICE);
		set_vif_field_itss(&vif_fields[bcdDevice_SOP],
			vif_component_name[bcdDevice_SOP],
			DEFAULT_MISSING_BCD_DEVICE, hex_str);
	#endif
}
