...
int append(char **buf, int *offset, const char *fmt, ...)
{
	va_list ap1, ap2;
	int n;
				...
				enum dtype type, uint32_t pnum)
{
	...
	if ((pdo & PDO_TYPE_MASK) == PDO_TYPE_FIXED) {
		...
		if (!append(buf, &offset, "\t%s_PDO_Supply_Type%d: 0\r\n",
					(type == SRC) ? "Src" : "Snk", pnum))
			return -1;

		if (type == SRC)
			if (!append(buf, &offset,
				"\tSrc_PDO_Peak_Current%d: 0\r\n", pnum))
				return -1;

		if (!append(buf, &offset, "\t%s_PDO_Voltage%d: %d\r\n",
				(type == SRC) ? "Src" : "Snk", pnum, voltage))
			return -1;

		if (type == SRC) {
			if (!append(buf, &offset,
					"\tSrc_PDO_Max_Current%d: %d\r\n",
					pnum, current))
				return -1;
		} else {
			if (!append(buf, &offset,
					"\tSnk_PDO_Op_Current%d: %d\r\n",
					pnum, current))
				return -1;
		}
		...
		if (!append(buf, &offset, "\t%s_PDO_Supply_Type%d: 1\r\n",
				(type == SRC) ? "Src" : "Snk", pnum))
			return -1;

		if (!append(buf, &offset, "\t%s_PDO_Min_Voltage%d: %d\r\n",
			(type == SRC) ? "Src" : "Snk", pnum, min_voltage))
			return -1;

		if (!append(buf, &offset, "\t%s_PDO_Max_Voltage%d: %d\r\n",
			(type == SRC) ? "Src" : "Snk", pnum, max_voltage))
			return -1;

		if (type == SRC) {
			if (!append(buf, &offset,
					"\tSrc_PDO_Max_Power%d: %d\r\n",
					pnum, power))
				return -1;
		} else {
			if (!append(buf, &offset,
					"\tSnk_PDO_Op_Power%d: %d\r\n",
					 pnum, power))
			return -1;
		}
		...
		if (!append(buf, &offset, "\t%s_PDO_Supply_Type%d: 2\r\n",
				(type == SRC) ? "Src" : "Snk", pnum))
			return -1;
