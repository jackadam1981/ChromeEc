...
static void write_unconstrained_power(FILE *vif)
{
	...
	if (is_src())
		yn = !!(src_pdo[0] & PDO_FIXED_EXTERNAL);
...
#ifdef CONFIG_USB_PD_MANUFACTURE_INFO
	yn = "YES";
#endif
...
static void write_num_fixed_batteries(FILE *vif)
{
	...
	fprintf(vif, "Num_Fixed_Batteies: %d\r\n", num);
}
...
static void write_sop_pp_capabl(FILE *vif)
{
...
{
	fprintf(vif, "Type_C_Is_Alt_Mode_Adapter: NO\r\n");
}
...
static void write_device_contains_captive_retimer(FILE *vif)
{
	fprintf(vif, "Device_Contains_Captive_Retimer: NO\r\n");
}

static void write_device_truncates_dp_for_tdhp_response(FILE *vif)
{
	fprintf(vif, "Device_Truncates_DP_for_tDHPResponse: NO\r\n");
}

static void write_device_gen1x1_tlink_turnaround(FILE *vif)
{
/* TODO: what's the correct value */
#if 0
	int time = 0;

	fprintf(vif, "Device_Gen1x1_tLinkTurnaround: %d\r\n", time);
#endif
}

static void write_device_gen2x1_tlink_turnaround(FILE *vif)
{
/* TODO: what's the correct value */
#if 0
	int time = 0;

	fprintf(vif, "Device_Gen2x1_tLinkTurnaround: %d\r\n", time);
#endif
}
 ...
 * PD Source Fields
 */
static void write_pd_source_fields(FILE *vif, enum dtype type)
{
...
/*TODO: Do we have over current protection? */
#if 0
		fprintf(vif, "PD_OC_Protection: NO\r\n");
		fprintf(vif, "PD_OCP_Method: 0\r\n");
#endif
		fprintf(vif, "\r\n%s\r\n", pdo_buf);
		free(pdo_buf);
	}
 ...
 * PD Sink Fields
 */
static void write_pd_sink_fields(FILE *vif, enum dtype type)
{
#ifdef CONFIG_USB_PD_DUAL_ROLE
	uint32_t max_power = 0;
	char *pdo_buf = NULL;
...
static void write_bc_1_2_charging_port_type(FILE *vif)
{
	/*
	 * 0 - DCP
	 * 1 - CDP
...
/* TODO: What's the correct value? */
	...
	fprintf(vif, "BC_1_2_Charging_Port_Type: %d\n", bc);
}
					...
					const char *vif_producer)
{
	...
	write_device_contains_captive_retimer(vif);
	write_device_truncates_dp_for_tdhp_response(vif);
	write_device_gen1x1_tlink_turnaround(vif);
	write_device_gen2x1_tlink_turnaround(vif);
	...
	write_bc_1_2_charging_port_type(vif);
