...
def main():
    ...
    with open(args.input, "rb") as fff:
        bbs = fff.read()
    ...
    ctypes.memmove(ctypes.addressof(header), bbs, ctypes.sizeof(header))
    if (
    ...
    for key, _ in header._fields_:  # pylint:disable=protected-access
        vvv = getattr(header, key)
        if isinstance(vvv, ctypes.Array):
            print(key, list(map(hex, vvv)))
        else:
            print(key, hex(vvv))
    ...
    data = bbs[FW_HEADER_SIZE : FW_HEADER_SIZE + dimension]

    with open(args.output, "wb") as fff:
        # ensure the file size
        fff.seek(OUTPUT_FILE_SIZE - 1, os.SEEK_SET)
        fff.write(b"\x00")

        fff.seek(0, os.SEEK_SET)
        fff.write(bbs[0 : ctypes.sizeof(header)])
        ...
        for i, addr in enumerate(FLASH_SEC_ADDR):
            ...
            fff.seek(UPDATE_PDU_SIZE + addr, os.SEEK_SET)
            fff.write(data[offset : offset + size])
            offset += size
