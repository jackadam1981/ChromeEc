...
/* Minimum max command response size, along all PD ECs. */
#define EC_PD_MAX_RESPONSE_SIZE 128
...
int cmd_charge_state_log(int argc, char *argv[])
{
	...
	int num_entries;
	int response_size;
	const int max_responses = (EC_PD_MAX_RESPONSE_SIZE -
				  sizeof(struct ec_host_response) -
				  sizeof(struct ec_response_charge_state_log)) /
				  sizeof(struct ec_charge_state_log_entry);
	...
	response_size = sizeof(*r) + sizeof(r->log_entries[0]) * max_responses;
	...
	while (num_entries > 0) {
		p.num_entries = (num_entries > max_responses) ?
				max_responses : num_entries;
