...
struct keyscan_test {
	...
	struct ec_params_keyscan_seq_item *items;	/* key data for EC */
};
...
static const unsigned char kbd_plain_xlate[] = {
	0xff,0x1b, '1', '2', '3', '4', '5', '6', '7', '8', '9', '0', '-', '=','\b','\t',	/* 0x00 - 0x0f */
	 ...
	 'b', 'n', 'm', ',', '.', '/',0xff,0xff,0xff,0xfe,0xff,0xff,0xff,0xff,0xff,0xff,	/* 0x30 - 0x3f */
...
{
	struct ec_params_keyscan_seq_item *item;
	...
	if (!test->item_alloced || test->item_count == test->item_alloced) {
		...
		size = test->item_alloced *
				sizeof(struct ec_params_keyscan_seq_item);
	...
	if (beat > 255) {
		fprintf(stderr, "Line %d: Invalid beat %lu (0-255)\n",
			linenum, beat);
		return -1;
	}
	item->beat = (uint8_t)beat;
 ...
 */
static void keyscan_get_input(char *input, int max_len, int wait)
{
	int len;
	int fd = 0;
...
{
	struct ec_params_keyscan_seq_add *add;
	int max_per_msg, to_send, upto;
	int rv = 0;

	/* How many can we send at once? */
	add = (struct ec_params_keyscan_seq_add *)malloc(EC_HOST_PARAM_SIZE);
	if (!add) {
		fprintf(stderr, "Out of memory for message\n");
		return -1;
	}
	max_per_msg = (EC_HOST_PARAM_SIZE - sizeof(*add)) /
			sizeof(struct ec_params_keyscan_seq_item);

	for (upto = 0; rv >= 0 && upto < test->item_count; upto += to_send) {
		int size;

		/* How many can we send this time? */
		to_send = test->item_count - upto;
		if (to_send > max_per_msg)
			to_send = max_per_msg;
		add->num_items = to_send;
		size = to_send * sizeof(struct ec_params_keyscan_seq_item);
		memcpy(add->item, &test->items[upto], size);
		size += sizeof(*add);
		rv = ec_command(EC_CMD_KEYSCAN_SEQ_ADD, 0, add, size, NULL, 0);
		upto += to_send;
	}
	free(add);
	if (rv < 0)
...
static int run_test(struct keyscan_info *keyscan, struct keyscan_test *test)
{
	...
	ctrl.cmd = EC_CMD_KEYSCAN_SEQ_CLEAR;
	...
	/* Start it */
	ctrl.cmd = EC_CMD_KEYSCAN_SEQ_START;
	ctrl.beat_us = keyscan->beat_us;
	rv = ec_command(EC_CMD_KEYSCAN_SEQ_CTRL, 0, &ctrl, sizeof(ctrl),
			NULL, 0);
	if (rv < 0)
		return rv;
	...
	if (test->item_count) {
		...
		item = &test->items[test->item_count - 1];
		wait_us += item->beat * keyscan->beat_us;
	}

	/* Wait for input */
	keyscan_get_input(input, sizeof(input), wait_us);
 ...
 * @param keyscan	keyscan information
 */
static void keyscan_run_tests(struct keyscan_info *keyscan)
{
	int testnum;
 ...
 * Test low-level key scanning
 *
 * extool keyscan <beat_us> <filename>
 *
 ...
 * @return 0 if ok, -1 on error
 */
static int cmd_keyscan(int argc, char *argv[])
{
