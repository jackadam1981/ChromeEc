...
static int wait_cec_event(struct ec_response_get_next_event *buffer,
			  long timeout)
{
	int rv;

	rv = ec_pollevent(1 << EC_MKBP_EVENT_CEC, buffer,
			  sizeof(struct ec_response_get_next_event), timeout);
	if (rv == 0) {
...
int cmd_cec_read(int argc, char *argv[])
{
	...
	rv = wait_cec_event(&buffer, timeout);
	if (rv != 0)
		return -1;
