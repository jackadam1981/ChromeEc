...
int cmd_damu(int argc, char *argv[])
{
	...
	printf("Charging voltage:%d, Charging current:%d, Adapter voltage:%d, Adapter current:%d\n",r.charge_voltage,
	r.charge_current, r.voltage_from_ADT, r.current_from_ADT);
	printf("RSOC:%d%%, Time-full:%dh:%d\n", r.RSOC, r.hour, r.minute);
	return rv;
}
