...
int cmd_fp_check_pixels(int argc, char *argv[])
{
	...
	void *pattern0, *pattern1;
	uint8_t *ptr0, *ptr1;
	int rv0, rv1;
	int bad0 = 0, bad1 = 0, dead = 0;
	long cnt = 0, lo_avg = 0, hi_avg = 0, lo_dev = 0, hi_dev = 0;
	int x, y;

	p.mode = FP_MODE_CAPTURE
		| (FP_CAPTURE_PATTERN0 << FP_MODE_CAPTURE_TYPE_SHIFT);
	rv0 = ec_command(EC_CMD_FP_MODE, 0, &p, sizeof(p), &r, sizeof(r));
	/* ensure the capture has happened without using event support */
	usleep(50000);
	pattern0 = fp_download_frame(&info);

	p.mode = FP_MODE_CAPTURE
		| (FP_CAPTURE_PATTERN1 << FP_MODE_CAPTURE_TYPE_SHIFT);
	rv1 = ec_command(EC_CMD_FP_MODE, 0, &p, sizeof(p), &r, sizeof(r));
	/* ensure the capture has happened without using event support */
	usleep(50000);
	pattern1 = fp_download_frame(&info);

	if (rv0 < 0 || rv1 < 0 || !pattern0 || !pattern1) {
	...
	for (y = 0; y < info.height; y++)
		for (x = 0; x < info.width; x++, ptr0++, ptr1++) {
			int bool0 = !!(*ptr0 & 128);
			int bool1 = !!(*ptr1 & 128);
			...
			if (!(bool0 ^ bool1)) {
				...
				int hi = bool0 ? *ptr0 : *ptr1;
				int lo = bool0 ? *ptr1 : *ptr0;
	...
	printf("Defects: dead %d (pattern0 %d pattern1 %d)\n",
	       dead, bad0, bad1);
	if (cnt)
		printf("Distribution: average %ld/%ld variance %ld/%ld\n",
		       lo_avg / cnt, hi_avg / cnt,
		       (lo_dev / cnt - lo_avg / cnt * lo_avg / cnt),
		       (hi_dev / cnt - hi_avg / cnt * hi_avg / cnt));
