...
int cmd_raw(int argc, char *argv[])
{
	...
	fprintf(stderr, "Read in %lu bytes for outgoing\n", out_len);
	...
	in_len = ec_command(cmd, ver, out_buf, (int)out_len, in_buf, (int)in_len);
	/* returned length should be larger than the actual data */
	fprintf(stderr, "Read in %lu bytes for incoming\n", in_len);
