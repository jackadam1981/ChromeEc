...
const char help_str[] =
	...
	"  dsleep [on | off | stats]\n"
	"      Enable/disable EC deep sleep, get deep sleep statistic data \n"
	"  echash [CMDS]\n"
...
int cmd_dsleep(int argc, char *argv[])
{
	...
	if (p.cmd == DEEP_SLEEP_GET_STATS) {
		...
		struct pm_statistics *stats = &r.pm_stats;
		...
		printf("        counts: %ld\n", stats->d0i0_cnt);
		printf("        time:   %fs\n",
				(double)stats->d0i0_time_us / 1000000);
		...
		if (IS_ENABLED(CONFIG_ISH_PM_D0I1)) {
			...
			printf("        counts: %ld\n", stats->d0i1_cnt);
			printf("        time:   %fs\n",
					(double)stats->d0i1_time_us / 1000000);
		}
		...
		if (IS_ENABLED(CONFIG_ISH_PM_D0I2) && r.aon_valid) {
			...
			printf("        counts: %ld\n", stats->d0i2_cnt);
			printf("        time:   %fs\n",
					(double)stats->d0i2_time_us / 1000000);
		}
		...
		if (IS_ENABLED(CONFIG_ISH_PM_D0I3) && r.aon_valid) {
			...
			printf("        counts: %ld\n", stats->d0i3_cnt);
			printf("        time:   %fs\n",
					(double)stats->d0i3_time_us / 1000000);
		}
