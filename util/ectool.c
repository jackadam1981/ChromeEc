...
int cmd_charge_port_override(int argc, char *argv[])
{
	...
	/* Process version 1 parameters (parameter 2 to 4). */
	if (argc == 2) {
		/* Use version 0 to skip current & voltage override. */
		cmdver = 0;
		psize = sizeof(struct ec_params_charge_port_override);
	} else if (argc == 5 && cmdver == 1 && p.override_port >= 0) {
		int voltage, current;
		...
		voltage = strtol(argv[4], &e, 0);
		if (e && *e) {
			fprintf(stderr, "Bad parameter.\n");
			return -1;
		}
		if (current > SHRT_MAX || voltage > SHRT_MAX) {
			fprintf(stderr,
				"Current and voltage can't exceed %d.\n",
				SHRT_MAX);
			return -1;
		}
		p.current_ma = current;
		p.voltage_mv = voltage;
	} else {
		fprintf(stderr, "Bad parameter.\n");
		return -1;
	}
