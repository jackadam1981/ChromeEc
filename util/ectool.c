...
const char help_str[] =
	...
	"  toggleautofanctrl <on>\n"
	"      Toggle automatic fan speed control.\n"
	"  pwmgetfanrpm\n"
...
int cmd_thermal_toggle_auto_fan_ctrl(int argc, char *argv[])
{
	struct lpc_params_thermal_toggle_auto_fan p;
	char *e;
	int rv;

	if (argc != 1) {
		fprintf(stderr, "Usage: toggleautofanctrl <on>\n");
		return -1;
	}

	p.auto_fan_on = strtol(argv[0], &e, 0);
	if (e && *e) {
		fprintf(stderr, "Bad value: 1 for on, 0 for off.\n");
		return -1;
	}

	rv = ec_command(EC_LPC_COMMAND_THERMAL_TOGGLE_AUTO_FAN_CTRL,
			&p, sizeof(p), NULL, 0);
	if (rv)
		return rv;

	printf("Automatic fan control is now %s.\n",
			p.auto_fan_on ? "on" : "off");
	return 0;
}
...
int main(int argc, char *argv[])
{
	...
	if (!strcasecmp(argv[1], "toggleautofanctrl"))
		return cmd_thermal_toggle_auto_fan_ctrl(argc - 2, argv + 2);
