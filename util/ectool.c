				...
				struct keyscan_test *test, char *keys)
{
	...
	if (beat > 0xffff) {
		fprintf(stderr, "Line %d: Invalid beat %lu (0-%d)\n",
			linenum, beat, 0xffff);
		return -1;
	}
	item->beat = (uint16_t)beat;
...
/*
	for (i = 0; i < test->item_count; i++) {
		...
		item = &test->items[i];
		if (resp->done[i])
			printf(" [skip %d at beat %u] ", i, item->beat);
	}
*/
...
static const struct param_info keyconfig_params[] = {
	...
	FIELD("disable_wait", disable_wait_us, "time to wait when disabled"),
