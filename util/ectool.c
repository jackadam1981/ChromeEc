...
static int ms_help(const char *cmd)
{
	...
	printf("  %s fifo_enable [0/1]          - set/get fifo status\n", cmd);
...
static int cmd_motionsense(int argc, char **argv)
{
	...
	if (argc >= 2 && !strcasecmp(argv[1], "fifo_enable")) {
		param.cmd = MOTIONSENSE_CMD_FIFO_ENABLE;
		if (argc == 3)
			param.fifo_enable.enable = strtol(argv[2], &e, 0);
		else
			param.fifo_enable.enable = EC_MOTION_SENSE_NO_VALUE;
		rv = ec_command(EC_CMD_MOTION_SENSE_CMD, 2,
				&param, ms_command_sizes[param.cmd].outsize,
				resp, ms_command_sizes[param.cmd].insize);
		if (rv < 0)
