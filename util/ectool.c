...
int cmd_flash_protect(int argc, char *argv[])
{
	...
	printf("Flash protect flags: 0x%08x", r.flags);
	if (r.flags & EC_FLASH_PROTECT_GPIO_ASSERTED)
	...
	if (r.flags & EC_FLASH_PROTECT_RW_NOW)
		printf(" rw_now");
	if (r.flags & EC_FLASH_PROTECT_ERROR_STUCK)
		printf(" STUCK");
	if (r.flags & EC_FLASH_PROTECT_ERROR_INCONSISTENT)
		printf(" INCONSISTENT");
	printf("\n");
