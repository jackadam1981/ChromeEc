...
int cmd_kakadu(int argc, char *argv[])
{
	...
	printf("Charger Temp:%d, Charging voltage:%d, Charging current:%d, Adapter voltage:%d, Adapter current:%d, RSOC:%d%%, Time-full:%dh:%d, BTEMP:%d C\n", r.temp, r.charge_voltage,
	r.charge_current, r.voltage_from_ADT, r.current_from_ADT, r.RSOC, r.hour, r.minute, (r.temperature-2731)/10);
