...
static int cmd_mkbp_get(int argc, char *argv[])
{
	...
	if (p.event_type == EC_MKBP_EVENT_BUTTON) {
		printf("MKBP buttons state: 0x%08x (supported: 0x%08x)\n",
		       r.buttons, supported);
		...
		if (supported)
			printf("Unknown buttons: 0x%08x\n", supported);
	} else if (p.event_type == EC_MKBP_EVENT_SWITCH) {
		printf("MKBP switches state: 0x%08x (supported: 0x%08x)\n",
		...
		if (supported)
			printf("Unknown switches: 0x%08x\n", supported);
	}
