...
const char help_str[] =
	...
	"  pchg [<port> [reset/init/enable/disable]]\n"
	"      Get and set peripheral charge port info & status\n"
...
static void cmd_pchg_help(char *cmd)
{
	...
	"  Usage1: %s\n"
	"  Usage2: %s <port>\n"
	"  Usage3: %s <port> reset/init/enable/disable\n"
	"    <port> is a peripheral port number.\n"
	"  Usage1 prints the number of ports.\n"
	"  Usage2 prints the status of a port.\n"
	"  Usage3 resets, initializes, enables, or disables a port.\n",
	cmd, cmd, cmd);
}
...
int cmd_pchg(int argc, char *argv[])
{
	...
	if (argc < 3) {
		/* Usage2 */
		struct ec_params_pchg_status *p = ec_outbuf;
		struct ec_response_pchg_status *r = ec_inbuf;

		p->port = port;
		rv = ec_command(EC_CMD_PCHG_STATUS, 0, ec_outbuf, sizeof(*p),
				ec_inbuf, ec_max_insize);
		if (rv < 0) {
			fprintf(stderr, "Error code: %d\n", rv);
			return rv;
		...
		/* Usage3 */
		fprintf(stderr, "Usage3 not implemented\n");
		return -1;
	}
