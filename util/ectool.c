...
int cmd_damu(int argc, char *argv[])
{
	...
	printf("Charging voltage:%d, Charging current:%d, Adapter voltage:%d, Adapter current:%d RSOC:%d%%, Time-full:%dh:%d\n",r.charge_voltage,
	r.charge_current, r.current_from_ADT, r.voltage_from_ADT, r.RSOC, r.hour, r.minute);
	return rv;
}
