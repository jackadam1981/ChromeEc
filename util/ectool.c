...
static void lb_show_params_v1(const struct lightbar_params_v1 *p)
{
	...
	printf("%d\t\t# .s3_ramp_up\n", p->s3_ramp_up);
	printf("%d\t\t# .s3_ramp_down\n", p->s3_ramp_down);
	printf("0x%02x\t\t# .tap_tick_delay\n", p->tap_tick_delay);
	printf("0x%02x\t\t# .tap_display_time\n", p->tap_display_time);
	printf("0x%02x\t\t# .tap_pct_red\n", p->tap_pct_red);
	printf("0x%02x\t\t# .tap_pct_green\n", p->tap_pct_green);
...
static int cmd_lightbar(int argc, char **argv)
{
	...
	if (!strcasecmp(argv[1], "params")) {
		...
		if (r == EC_RES_INVALID_PARAM) {
			/* Link had no version command */
			return cmd_lightbar_params_v0(argc, argv);
		} else if (r == EC_RES_SUCCESS && resp.version.num == 1) {
			/* Newer systems do */
			return cmd_lightbar_params_v1(argc, argv);
		} else if (r == EC_RES_INVALID_COMMAND) {
			printf("No lightbar present?\n");
			return 1;
		} else {
			printf("Can't determine lightbar params version\n");
			return r;
		}
	}
