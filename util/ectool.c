...
static const char * const mkbp_buttons_strings[] = {
...
BUILD_ASSERT(ARRAY_SIZE(mkbp_buttons_strings) == 4);

static const char * const mkbp_switches_strings[] = {
	[EC_MKBP_LID_OPEN] = "Lid open",
	[EC_MKBP_TABLET_MODE] = "Tablet mode",
	[EC_MKBP_BASE_ATTACHED] = "Base attached",
};
/* Make sure the array is not sparse. */
BUILD_ASSERT(ARRAY_SIZE(mkbp_switches_strings) == 3);
...
static int cmd_mkbp_get(int argc, char *argv[])
{
	...
	if (p.event_type == EC_MKBP_EVENT_BUTTON) {
		printf("MKBP buttons state: 0x%08x (supported: %08x)\n",
		       r.buttons, supported);
		for (i = 0; i < ARRAY_SIZE(mkbp_buttons_strings); i++) {
			if (supported & BIT(i))
				printf("%s: %s\n", mkbp_buttons_strings[i],
				       r.buttons & BIT(i) ? "ON" : "OFF");
		}
	} else if (p.event_type == EC_MKBP_EVENT_SWITCH) {
		printf("MKBP switches state: 0x%08x (supported: %08x)\n",
		       r.switches, supported);
		for (i = 0; i < ARRAY_SIZE(mkbp_switches_strings); i++) {
			if (supported & BIT(i))
				printf("%s: %s\n", mkbp_switches_strings[i],
