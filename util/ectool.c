...
int cmd_usb_pd(int argc, char *argv[])
{
	...
	struct ec_response_usb_pd_control_v3 *r_v3 =
		(struct ec_response_usb_pd_control_v3 *)ec_inbuf;
	...
	if (cmdver == 0) {
		...
		if (cmdver == 3) {
			printf("Adapter type:%s\n",
				r_v3->flags & USB_PD_MUX_TBT_ADAPTER ?
					"Legacy Thunderbolt" : "Type-C");

			printf("Cable type:%sOptical\n",
				r_v3->flags & USB_PD_MUX_TBT_CABLE_TYPE ?
					"" : "Non-");

			printf("Link LSRX Communication:%s-directional\n",
				r_v3->flags & USB_PD_MUX_TBT_LINK ?
					"Uni" : "Bi");

			printf("Cable Speed:");
			if (r_v3->cable_speed == TBT_GEN1)
				printf("TBT Gen1");
			else if (r_v3->cable_speed == TBT_GEN1_GEN2)
				printf("TBT Gen1 and TBT Gen2");
			else if (r_v3->cable_speed == TBT_GEN2)
				printf("TBT Gen2");
			else
				printf("UNKNOWN");
			printf("\n");
