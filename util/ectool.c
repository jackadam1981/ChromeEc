...
static int ms_help(const char *cmd)
{
	...
	printf("  %s kb_dis NUM                 - set/get KB dis angle\n", cmd);
...
static int cmd_motionsense(int argc, char **argv)
{
	...
	if (argc < 4 && !strcasecmp(argv[1], "kb_dis")) {
		param.cmd = MOTIONSENSE_CMD_KB_DISABLE_ANGLE;
		param.kb_dis_angle.data = EC_MOTION_SENSE_NO_VALUE;

		if (argc == 3) {
			param.kb_dis_angle.data = strtol(argv[2], &e, 0);
			if (e && *e) {
				fprintf(stderr, "Bad %s arg.\n", argv[1]);
				return -1;
			}
