...
const char help_create[] =
	...
	"  --rework_id <value>        REWORK_ID\n"
	"\n"
	"<value> must be a positive integer <= 0XFFFFFFFF for all args and for\n"
	"  rework_id arg it must be a positive integer <= 0xFFFFFFFFFFFFFFFF\n"
	"  and field size can be optionally specified by <value:size>\n"
	"  notation: e.g. 0xabcd:4.\n"
...
static int parse_long_integer_field(const char *arg, struct long_integer_field *f)
{
	uint64_t val;
	char *e;
	char *ch;
...
static int cmd_create(int argc, char **argv)
{
	...
	while ((i = getopt_long(argc, argv, "", opts_create, NULL)) != -1) {
		...
		case OPT_SSFC:
			...
			break;
		case OPT_REWORK_ID:
			if (parse_long_integer_field(optarg, &bi.rework))
				return -1;
			break;
		}
	}
