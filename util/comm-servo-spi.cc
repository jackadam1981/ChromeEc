			...
			const uint8_t *outdata, size_t outsize)
{
	...
	txbuf = static_cast<uint8_t *>(calloc(1, total_len));
	if (!txbuf)
		return -ENOMEM;
...
static int get_response(uint8_t *bodydest, size_t bodylen)
{
	...
	if (spi_read(reinterpret_cast<uint8_t *>(&hdr), sizeof(hdr)))
		goto read_error;
				...
				void *indata, int insize)
{
	...
	if (send_request(cmd, version,
			 reinterpret_cast<const uint8_t *>(outdata),
			 outsize) == 0)
		ret = get_response(reinterpret_cast<uint8_t *>(indata), insize);
