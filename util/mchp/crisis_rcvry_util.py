...
EC_RESPONSE_BYTES_MIN_COUNT=128
...
def ec_payload_transfer(uart_handle, ec_file):
    ...
    while offset < os.path.getsize(ec_file):
        ...
        if offset == EC_RESPONSE_BYTES_COUNT or (os.path.getsize(ec_file) - offset) == EC_RESPONSE_BYTES_MIN_COUNT:
            logging.info(
...
def transfer(uart_handle, command_id, pay_length, total_len, data):
    ...
    command_id     : Given command ID
    pay_length     : data length of the payload
    total_len      : Total length of the payload
    data           : Given payload
...
def __crc32(_bytes):
    ...
    Args :
    bytes : Given bytes
    return : binacii crc32 of the bytes
    """
...
def main():
    """Crisis recovery utility routine ."""
