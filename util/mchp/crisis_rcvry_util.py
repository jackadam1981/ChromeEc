...
def update_progress(curr_val, max_val):
    ...
    """
    print("Transfer {0} of {1}.".format(curr_val, max_val),end="\r")
...
def ec_payload_transfer(uart_handle, ec_file):
    ...
    while offset < max_len:
        ...
        if (
            ...
            logging.info(
                "Waiting for EC to program. Will take about %ds."
                % ACK_TIMEOUT_SECS
            )
        ...
        if rf_cont_disp != resp_bytes:
            ...
            logging.info(
                "For the EC FW Image transferring for the internal flash programming"
            )
...
def command_write(uart_handle, cmd_seq, resp_seq):
    ...
    Args:
        uart_handle: Handle of the UART port
        cmd_seq: Sequence of command  bytes has to transfer
