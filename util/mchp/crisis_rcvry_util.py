...
def ec_payload_transfer(uart_handle, ec_file):
    ...
    while offset < max_len:
        ...
        program_length = min(IMAGE_CHUNK_LENGTH, max_len - offset)
        file_content = image[offset : offset + program_length]
        cmd_bytes = parse_packet(
            FW_IMAGE_WRITE_CMD_ID, program_length, offset, file_content
        )

        uart_handle.write(cmd_bytes)

        uart_handle.reset_input_buffer()
        rf_cont_disp = uart_handle.read(RESPONSE_LENGTH)
        ...
        if rf_cont_disp != resp_bytes:
            ...
            if retry_count == 0:
                ...
                sys.exit(1)
            logging.info("Response pattern not match, retrying")
            continue
...
def transfer(
    ...
    while offset < payload_length:
        ...
        program_length = min(max_write_length, payload_length - offset)
        file_content = payload[offset : offset + program_length]
        cmd_bytes = parse_packet(
            command_id, program_length, offset, file_content
        )
...
def parse_packet(command, length, offset, data_bytes):
...
def main(args):
    ...
    parser.add_argument(
        "-s",
        "--second_loader_image",
        dest="second_loader_file",
        type=str,
        required=False,
        help="second loader image file",
    )
    parser.add_argument(
        "-p",
        "--com_port_path",
    ...
    if args.second_loader_file:
        if not os.path.exists(args.second_loader_file):
        ...
        second_loader_image_transfer(uart_handle, args.second_loader_file)
        uart_handle.reset_input_buffer()
        ...
        logging.error(
            "Specify Second loader (-s) or EC (-e) image path as argument"
        )
