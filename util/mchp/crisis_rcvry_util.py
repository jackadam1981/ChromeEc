...
HOST_NEGO_CMD_RESP_SEQ = [90, 165]
FLASH_PGM_CMD_SEQ = [0x33,0xcc]
FLASH_PGM_CMD_RESP_SEQ = [60,195]
...
def ec_payload_transfer(uart_handle, ec_file):
    ...
    while offset < os.path.getsize(ec_file):
        ...
        for i in range(len(cmd_bytes)):
            uart_handle.write(cmd_bytes[i:i+1])
        ...
        if rf_cont_disp != resp_bytes:
            ...
            if retry_count == 0:
                logging.critical("Response pattern not match")
                sys.exit(1)
...
def ec_header_transfer(uart_handle):
    ...
    vendor_id = b'\x4D\x43\x48\x50'
    pgm_header = b'\x01\x08'
    prg_header_len = b'\x08'
    terminator_id = b'\x58\x45\x4F\x46'
    prg_hdr_total_len = 0xF0

    mchp_vendor_id_loc = 0
    mchp_vendor_id_size = len(vendor_id)
    mchp_pgm_hdr_loc = 6
    mchp_pgm_hdr_size = mchp_pgm_hdr_loc+len(pgm_header)
    mchp_pgm_hdr_len_loc = 13
    mchp_pgm_hdr_len_size = mchp_pgm_hdr_len_loc+len(prg_header_len)
    mchp_term_id_loc = prg_hdr_total_len - 8
    mchp_term_id_size = mchp_term_id_loc+len(terminator_id)

    header_gen = bytearray(prg_hdr_total_len)

    header_gen[mchp_vendor_id_loc:mchp_vendor_id_size] = vendor_id
    header_gen[mchp_pgm_hdr_loc:mchp_pgm_hdr_size] = pgm_header
    header_gen[mchp_pgm_hdr_len_loc:mchp_pgm_hdr_len_size] = prg_header_len
    header_gen[mchp_term_id_loc:mchp_term_id_size] = terminator_id

    transfer(uart_handle, HEADER_WRITE_CMD_ID, 240, 240, header_gen)
...
def payload_transfer(uart_handle, spi_file):
    ...
    addr = image[0 : 0 + 1][0]
    addr |= image[0 + 1 : 0 + 2][0] << 8
    addr |= image[0 + 2 : 0 + 3][0] << 16
    ...
    img_len = (
        struct.unpack(
            "<H", hdr_file_data[0x10:0x11] + hdr_file_data[0x11:0x12]
        )[0]
    ) * 128
...
def command_write(uart_handle, cmd_seq, resp_seq):
    ...
    while i < 3:
        ...
        if len(read) == 0:
            ...
            if (
                read[0] == resp_seq[0]
                and read[1] == resp_seq[1]
            ):
                return True
            if (
                read[0] == resp_seq[1]
                and read[1] == resp_seq[0]
            ):
                return True
...
def main(args):
    ...
    if args.spi_file:
        ...
        if not os.path.exists(spi_file):
            ...
            logging.info("Secondary loader programming is in progress")
            uart_handle = open_serial_port(comp_port, 9600)
            process = command_write(uart_handle, HOST_NEGO_CMD_SEQ, HOST_NEGO_CMD_RESP_SEQ)
            if process:
            ...
            uart_handle.close()
            if process is False:
                logging.critical("Timeout Error - Did not receive response from EC.")
                sys.exit(1)
    ...
    if args.ec_file:
        ...
        if not os.path.exists(ec_file):
            ...
            logging.info("EC programming is in progress")

            uart_handle = open_serial_port(comp_port, 57600)

            process = command_write(uart_handle, FLASH_PGM_CMD_SEQ, FLASH_PGM_CMD_RESP_SEQ)
            if process:
