...
file_line = [
    ...
    "SF 65 header.bin 40",
    "RF 65 CRC32",
    "SF 66 key_hash.bin 80",
    "RF 66 CRC32",
    "SF 67 second_loader_fw.bin 80",
    "RF 67 CRC32",
    "SC 70 CRC32",
    "SF 65 header.bin 40",
    "RF 65 CRC32",
...
operation_1 = 1  # 1=WR, 2=RD, 3=ERASE
file_line_1 = [""]
...
cmd_3byte_offset_1 = [0x65, 0x67, 0x66]
cmd_4byte_offset_1 = [0x68]
NULL_BYTE_1 = b""
INV_CMD = 3


def check_resp_1(b_rx, b_tx):
    if b_rx != b"":
...
def uart_communicator_1(uart):
    global srpt_list_1, NULL_BYTE_1
    find_s = False
    find_sc = False
    find_sf = False
    find_r = False
        ...
        for lines in file_line_1:
            ...
            if content != []:
                ...
                if find_s == False:
                    ...
                    if content[0].lower() == srpt_list_1[0]:  # if 'S'
                        print("\n")
                        find_s = True
                    ...
                    if content[0].lower() == srpt_list_1[4]:  # if 'C'
                ...
                if find_sc == False:
                    ...
                    if content[0].lower() == srpt_list_1[5]:  # if 'SC'
                        print("\n")
                        find_sc = True

                        if content[-1].lower() == srpt_list_1[7]:  # CRC32
                            calc_crc_sts = True
                            last_byte = -1
                ...
                if find_gf == False:
                    ...
                    if content[0].lower() == srpt_list_1[10]:  # if 'GF'
                        # print ("\n #if 'GF' ")
                        find_gf = True
                ...
                if find_sf == False:
                    ...
                    if content[0].lower() == srpt_list_1[8]:  # if 'SF'
                        print("\n")
                        # print ("\n #if 'SF' ")
                        find_sf = True
                ...
                if find_s == True:
                    r_content = []
                    if content[0].lower() == srpt_list_1[1]:  # if 'R'
                        find_r = True
                        for items in content[1:]:
                            r_content.append(items.lower())

                    if content[0].lower() == srpt_list_1[2]:  # if 'F'
                        find_f = True
                ...
                if find_sc == True:
                    ...
                    rc_data_count = 0
                    crc_input_bytes = bytearray()
                    if content[0].lower() == srpt_list_1[6]:  # if 'RC'
                        find_rc = True
                        ...
                        if content[2].lower() == srpt_list_1[7]:  # CRC32
                            rc_data_count = 0
                        else:
                ...
                if find_gf == True:
                    ...
                    if content[0].lower() == srpt_list_1[11]:  # if 'RG'
                        find_rg = True
                ...
                if find_sf == True:
                    ...
                    if content[0].lower() == srpt_list_1[9]:  # if 'RF'
                        find_rf = True
                ...
                if content[0].lower() == srpt_list_1[3]:  # if 'RS'
                    rs_content = []
                    rs_cont_disp = []
                    find_rs = True
            ...
            if find_s == True and find_r == True:  # for Combination of S AND R
                ...
                r_len = len(
                    r_content
                )  # ****this is the S 33 CC : R 3C C3 section
            ...
            if (
                ...
                for n in range(sc_count):
                    ...
                    resp_sts = check_resp_1(first_cmd_read, rc_content[0])

                    if (
                        (first_cmd_read == NULL_BYTE_1)
                        or (resp_sts == INV_CMD)
                        or (resp_sts == False)
                    ):
                ...
                try:
                    ...
                    for n in range(gf_count):
                        while rx_file_size > 0:
                            ...
                            if gf_cmd in cmd_3byte_offset_1:
                                gf_payload_offset = (
                                    gf_payload_offset & 0x00FFFFFF
                                )
                                byte_count = 3

                            elif gf_cmd in cmd_4byte_offset_1:
                                gf_payload_offset = (
                ...
                try:
                    ...
                    for n in range(sf_count):
                        while tx_file_size > 0:
                            ...
                            ## Transmit payload offset ##
                            if sf_cmd in cmd_3byte_offset_1:
                                payload_offset = payload_offset & 0x00FFFFFF
                                byte_count = 3

                            elif sf_cmd in cmd_4byte_offset_1:
...
def uart_communicator(uart):
    ...
    for (
        lines
    ) in file_line:  # for each command line (e.g. 1st line : 'S 55 AA 55 AA'
        content = lines.split()
        ...
        if find_s == True and find_r == True:  # for Combination of S AND R
            ...
            r_len = len(
                r_content
            )  # This is the S 55 AA 55 AA : R 5A A5 section
            print("Send data ----------------------- Receive Data ")
            for n in range(s_count):
                for items in s_content:
                    # print("data sent[",items,"]")
            ...
            try:
                ...
                for n in range(sf_count):
                    while tx_file_size > 0:
                        ...
                        time.sleep(
                            0.05
                        )  # Servo needs slight delay here to catch up
...
def main_1(argv):
    ...
    global operation_1, file_line_1
    prompt_text = ""

    print("************************************************")
    print("**         !!! REQUIREMENT !!!                **")
    print("** Please run gen_pgm_hdr.py to generate       **")
    print("** flash_pgm_hdr.bin before continuing    **")
    print("************************************************")
    print("** Flash Programming Operation to Perform on SPI Flash   **")
