...
{
	if (respBuf[0] == (uint8_t)(UFPP_D2H_SYNC_CMD)) {
		displayColorMsg(SUCCESS, "Host/Device are synchronized\n");
		return TRUE;
	}
...
BOOLEAN CMD_DispWrite(uint8_t *respBuf, uint32_t respSize, uint32_t respNum,
		      uint32_t totalSize)
{
	if (respBuf[0] == (uint8_t)(UFPP_WRITE_CMD)) {
...
BOOLEAN CMD_DispRead(uint8_t *respBuf, uint32_t respSize, uint32_t respNum,
		     uint32_t totalSize)
{
	if (respBuf[0] == (uint8_t)(UFPP_READ_CMD)) {
		displayColorMsg(SUCCESS,
...
{
	if (respBuf[0] == (uint8_t)(UFPP_WRITE_CMD)) {
		displayColorMsg(SUCCESS, "Flash Erase of device [%lu] Passed\n",
			devNum);
	} else {
		displayColorMsg(
...
{
	if (respBuf[0] == (uint8_t)(UFPP_FCALL_CMD))
...
{
	if (respBuf[1] == (uint8_t)(UFPP_FCALL_RSLT_CMD)) {
		displayColorMsg(SUCCESS,
