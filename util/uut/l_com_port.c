 ...
 *--------------------------------------------------------------------------
 */
static tcflag_t convert_parity_to_parity_mask(uint32_t parity)
{
	switch (parity) {
	case 0: /* None         */
		return 0x0000;
 ...
 *--------------------------------------------------------------------------
 */
static tcflag_t convert_byte_size_to_byte_size_mask(uint32_t byte_size)
{
	switch (byte_size) {
	case 5:
		return CS5;
				...
				struct COMPORT_FIELDS ComPortFields)
{
	...
	if (tcgetattr(hDevice_Driver, &tty) != 0) {
		displayColorMsg(FAIL,
			"ConfigureUart Error: Fail to get attribute from "
			"Device number %lu.\n",
			hDevice_Driver);
		return FALSE;
	}
...
HANDLE ComPortOpen(const char *ComPortDeviceName,
				struct COMPORT_FIELDS ComPortFields)
{
	int32_t port_handler;

	port_handler = open(ComPortDeviceName, O_RDWR | O_NOCTTY);

	if (port_handler < 0) {
		displayColorMsg(FAIL, "ComPortOpen Error %d opening %s: %s\n",
	...
	if (!ConfigureUart(port_handler, ComPortFields)) {
		displayColorMsg(FAIL,
			"ComPortOpen() Error %d, Failed on ConfigureUart() %s, "
			"%s\n",
			errno, ComPortDeviceName, strerror(errno));
		return INVALID_HANDLE_VALUE;
	}
...
BOOLEAN ComPortClose(HANDLE nDeviceID)
{
	...
	if (close(nDeviceID) == INVALID_HANDLE_VALUE) {
		displayColorMsg(FAIL,
			"ComPortClose() Error: %d Device com%u was not opened, "
			"%s.\n",
			errno, (uint32_t)nDeviceID, strerror(errno));
		return FALSE;
	}
 ...
 *****************************************************************************
 */
BOOLEAN ComPortWriteBin(HANDLE nDeviceID, const uint8_t *Buffer,
						uint32_t BufSize)
{
	...
	if (bytes_written != BufSize) {
		displayColorMsg(FAIL,
			"ComPortWriteBin() Error: %d  Failed to write data to "
			"Uart Port %d, %s.\n",
			errno, (uint32_t)nDeviceID, strerror(errno));
...
/******************************************************************************
 * Function: uint32_t ComPortReadBin()
 *
 ...
 *****************************************************************************
 */
uint32_t ComPortReadBin(HANDLE nDeviceID, uint8_t *Buffer, uint32_t BufSize)
{
	int32_t read_bytes;
	...
	if (read_bytes == -1) {
		displayColorMsg(FAIL,
			"ComPortReadBin() Error: %d Device number %lu was not "
			"opened, %s.\n",
			errno, (uint32_t)nDeviceID, strerror(errno));
	}
...
/******************************************************************************
 * Function: uint32_t ComPortWaitForRead()
 *
 ...
 *****************************************************************************
 */
uint32_t ComPortWaitForRead(HANDLE nDeviceID)
{
	int32_t bytes;
	int32_t ret_val;
	struct pollfd fds;
	...
	if (ret_val > 0) {
		...
		if (ioctl(nDeviceID, FIONREAD, &bytes) < 0) {
			displayColorMsg(FAIL,
				"ComPortWaitForRead() Error: %d Device number "
				"%lu %s\n",
				errno, (uint32_t)nDeviceID, strerror(errno));
			return 0;
		}
