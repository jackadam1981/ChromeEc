...
static void param_parse_cmd_line(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, short_opts,
		...
		case 'b':
			...
			break;
		case 'o':
			if (sscanf(optarg, "%s", (char *)opr_name) == 0)
				exit(EC_OPR_MUM_ERR);
			break;
		case 'p':
			if (sscanf(optarg, "%s", (char *)port_name) == 0)
				exit(EC_PORT_ERR);
			break;
		case 'f':
			if (sscanf(optarg, "%s", (char *)file_name) == 0)
				exit(EC_FILE_ERR);
			break;
		case 'a':
			if (sscanf(optarg, "%s", (char *)addr_str) == 0)
				exit(1);
			break;
		case 's':
			if (sscanf(optarg, "%s", (char *)size_str) == 0)
				exit(1);
			break;
		}
	}
