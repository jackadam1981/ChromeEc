...
static void param_parse_cmd_line(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, short_opts,
		...
		case 'o':
			size = strlen(optarg);
			if (size > (sizeof(opr_name) - 1)) {
				display_color_msg(FAIL,
					"Error: Opr length can't exceed %d\n"
					, (sizeof(opr_name) - 1));
				exit(EC_OPR_MUM_ERR);
			}
			strncpy((char *)opr_name, optarg, size);
			opr_name[size] = '\0';
			break;
		case 'p':
			size = strlen(optarg);
			if (size > (sizeof(port_name) - 1)) {
				display_color_msg(FAIL,
					"Error: Port length can't exceed %d\n"
					, (sizeof(port_name) - 1));
				exit(EC_PORT_ERR);
			}
			strncpy((char *)port_name, optarg, size);
			port_name[size] = '\0';
			break;
		case 'f':
			size = strlen(optarg);
			if (size > (sizeof(file_name) - 1)) {
				display_color_msg(FAIL,
					"Error: File length can't exceed %d\n"
					, (sizeof(file_name) - 1));
				exit(EC_FILE_ERR);
			}
			strncpy((char *)file_name, optarg, size);
			file_name[size] = '\0';
			break;
		case 'a':
			size = strlen(optarg);
			if (size > (sizeof(addr_str) - 1)) {
				display_color_msg(FAIL,
					"Error: Addr length can't exceed %d\n"
					, (sizeof(addr_str) - 1));
				exit(1);
			}
			strncpy((char *)addr_str, optarg, size);
			addr_str[size] = '\0';
			break;
		case 's':
			size = strlen(optarg);
			if (size > (sizeof(size_str) - 1)) {
				display_color_msg(FAIL,
					"Error: Size length can't exceed %d\n"
					, (sizeof(size_str) - 1));
				exit(1);
			}
			strncpy((char *)size_str, optarg, size);
			size_str[size] = '\0';
			break;
		}
	}
