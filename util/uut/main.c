...
static uint32_t PARAM_GetFileSize(const char *fileName);
static uint32_t PARAM_GetStrSize(char *string);
static void MAIN_PrintVersion(void);
static void ToolUsage(void);
static void ExitUartApp(uint32_t exitStatus);
static void ToolUsage(void);
...
static void PARAM_CheckPortNum(char *port_name)
{
	if ((strncmp(port_name, COMP_PORT_PREFIX_1,
		...
		displayColorMsg(FAIL, "ERROR: %s is an invalid serial port\n",
			port_name);
