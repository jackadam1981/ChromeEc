...
static void param_parse_cmd_line(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, short_opts,
		...
		case 'b':
			...
			break;
		case 'o':
			size = sizeof(opr_name);
			ret = strlcpy((char *)opr_name, optarg, size);
			if (ret >= size) {
				display_color_msg(FAIL,
		...
		case 'p':
			...
			ret = strlcpy((char *)port_name, optarg, size);
			if (ret >= size) {
				display_color_msg(FAIL,
		...
		case 'f':
			...
			ret = strlcpy((char *)file_name, optarg, size);
			if (ret >= size) {
				display_color_msg(FAIL,
			...
			break;
		case 'a':
			size = sizeof(addr_str);
			ret = strlcpy((char *)addr_str, optarg, size);
			if (ret >= size) {
				display_color_msg(FAIL,
			...
			break;
		case 's':
			size = sizeof(size_str);
			ret = strlcpy((char *)size_str, optarg, size);
			if (ret >= size) {
				display_color_msg(FAIL,
