...
/* The start address to store the firmware segment to be programed */
 ...
 *
 * Parameters:    offset - The start address of the flash where the firmware
 *                         to be programed.
 *                buffer - The buffer holds data of the firmware.
 *                file_size - the size to be programed.
							...
							uint32_t file_size)
{
	...
	while (file_seg) {
		...
		data_buf[1] = seg;
		/* The offset of the flash where the segment to be programed. */
...
int main(int argc, char *argv[])
{
	...
	if (auto_mode) {
		...
		strip_size = size;
		while ((buffer[strip_size-1] == 0xFF) && strip_size > 0)
			strip_size--;
...
static void tool_usage(void)
{
	...
	printf("  -O, --offset <num>   - With --auto, assgin the offset of");
