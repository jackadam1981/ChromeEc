 ...
 *---------------------------------------------------------------------------
 */
struct CommandNode cmdBuf[MAX_CMD_BUF_SIZE];
uint8_t respBuf[MAX_RESP_BUF_SIZE];

/*---------------------------------------------------------------------------
 * Functions prototypes
 *---------------------------------------------------------------------------
 */
static BOOLEAN OPR_SendCmds(struct CommandNode *cmdBuf, uint32_t cmdNum);
 ...
 *---------------------------------------------------------------------------
 */
void OPR_WriteMem(char *input, uint32_t addr, uint32_t size)
{
	...
	char *token = NULL;
	char *stopStr;
	uint32_t blockSize = (console) ? sizeof(uint32_t) : MAX_RW_DATA_SIZE;
	struct CommandNode wCmdBuf;
	...
	while (TRUE) {
		if (console) {
			...
			writeSize = (uint32_t)fread(dataBuf, 1, blockSize,
						inputFileID);
		...
		CMD_CreateWrite(curAddr, writeSize, dataBuf, wCmdBuf.cmd,
			&wCmdBuf.cmdSize);
 ...
 *---------------------------------------------------------------------------
 */
void OPR_ReadMem(char *output, uint32_t addr, uint32_t size)
{
	...
	for (curAddr = addr; curAddr < (addr + size);
		curAddr += MAX_RW_DATA_SIZE) {
		bytesLeft = (uint32_t)(addr + size - curAddr);
		readSize = MIN(bytesLeft, MAX_RW_DATA_SIZE);

		CMD_CreateRead(curAddr, ((uint8_t)readSize - 1), rCmdBuf.cmd,
			&rCmdBuf.cmdSize);
...
void OPR_ReadStatusMsg(char *outputFileName)
{
	...
	if (outputFileID == NULL) {
		displayColorMsg(FAIL, "Error openning output file: %s\n",
			outputFileName);
		return;
	}
	...
	while (1) {
		...
		bytesRead = ComPortReadBin(portHandle, respBuf,
					STS_MSG_MIN_SIZE);
		...
		if (*((uint32_t *)respBuf) == (uint32_t)STS_MSG_APP_END)
			break;
...
{
	uint32_t cmdNum;
	struct CommandNode *curCmd = cmdBuf;
...
BOOLEAN OPR_ScanBaudRate(void)
{
	...
	brStep = (BR_LOW_LIMIT * BR_BIG_STEP) / 100;
	for (bdRate = BR_LOW_LIMIT; bdRate < BR_HIGH_LIMIT; bdRate += brStep) {
		...
		if (sr == SR_OK) {
			...
			printf("SR_WRONG_DATA: Baud rate - %d, respBuf - "
			       "0x%x\n", bdRate, respBuf[0]);
			dataReceived = TRUE;
			...
			printf("Unknown error code: Baud rate - %d, respBuf - "
				"0x%x\n", bdRate, respBuf[0]);
	}
			...
			if (chipNum == Yarkon) {
				...
				do {
					...
					elapsed_time =
					 ...
					 (elapsed_time <= OPR_TIMEOUT));
				ComPortReadBin(portHandle, respBuf,
							curCmd->respSize);
			}
