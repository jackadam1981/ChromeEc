/*
 * Copyright (c) 2018 The Chromium OS Authors. All rights reserved.
 * Use of this source code is governed by a BSD-style license that can be
 * found in the LICENSE file.
 */

/* This file defines the UART console application operations. */

#ifndef _OPR_H_
#define _OPR_H_

#include "uut_common.h"
/*---------------------------------------------------------------------------
 * Constant definitions
 *---------------------------------------------------------------------------
 */

/* Baud rate scan steps: */
#define BR_BIG_STEP 20       /* in percents from current baud rate */
#define BR_MEDIUM_STEP 10    /* in percents from current baud rate */
#define BR_SMALL_STEP 1      /* in percents from current baud rate */
#define BR_MIN_STEP 5        /* in absolute baud rate units */
#define BR_LOW_LIMIT 400     /* Automatic BR detection starts at this value */
#define BR_HIGH_LIMIT 150000 /* Automatic BR detection ends at this value */

#define OPR_WRITE_MEM "wr"      /* Write To Memory/Flash */
#define OPR_READ_MEM "rd"       /* Read From Memory/Flash */
#define OPR_EXECUTE_EXIT "go"   /* Execute a non-return code */
#define OPR_EXECUTE_CONT "call" /* Execute returnable code */

enum SYNC_RESULT {
	SR_OK = 0x00,
	SR_WRONG_DATA = 0x01,
	SR_TIMEOUT = 0x02,
	SR_ERROR = 0x03
};

/*----------------------------------------------------------------------------
 * External Variables
 *---------------------------------------------------------------------------
 */
extern struct COMPORT_FIELDS portCfg;

/*---------------------------------------------------------------------------
 * Functions prototypes
 *---------------------------------------------------------------------------
 */

void OPR_Usage(void);
BOOLEAN OPR_ClosePort(void);
BOOLEAN OPR_OpenPort(const char *port_name, struct COMPORT_FIELDS portCfg);
void OPR_WriteMem(char *inputFileName, uint32_t addr, uint32_t size);
void OPR_ReadMem(char *outputFileName, uint32_t addr, uint32_t size);
void OPR_FlashEraseDevice(uint32_t devNum);
void OPR_FlashEraseSector(uint32_t devNum, uint32_t addr);
void OPR_ExecuteExit(uint32_t addr);
void OPR_ExecuteReturn(uint32_t addr);
void OPR_PrintFlashId(uint32_t devNum);
void OPR_PrintFlashSts(uint32_t devNum);
void OPR_UnlockFlash(uint32_t devNum);
BOOLEAN OPR_WaitTillReady(uint32_t devNum);
void OPR_GetFlashSts(uint32_t devNum);
BOOLEAN OPR_ScanBaudRate(void);
enum SYNC_RESULT OPR_CheckSync(uint32_t bdRate);
void OPR_ReadStatusMsg(char *outputFileName);
#endif /* _OPR_H_ */
