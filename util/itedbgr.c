...
static void d2ec(void *ctxt)
{
	...
	printf("READ %02x%02x %02x\n", d2ec_read8(ctxt, 0x2000),
		d2ec_read8(ctxt, 0x2001), d2ec_read8(ctxt, 0x2002));
}
...
void jtag_edm_init(void *ctxt)
{
	...
	/* clear debug event */
	jtag_write_misc_reg(ctxt,
		MISC_REG_DBGER, DBG_EVENT_DPED | DBG_EVENT_CRST);
...
static void jtag(void *ctxt)
{
	...
	printf("READ MEMB : %02x %02x %02x\n",
		jtag_read_mem_b(ctxt, 0xF02000),
		jtag_read_mem_b(ctxt, 0xF02001),
		jtag_read_mem_b(ctxt, 0xF02002));
	printf("READ MEMB : %02x %02x %02x\n",
		jtag_read_mem_b(ctxt, 0xF02000),
		jtag_read_mem_b(ctxt, 0xF02001),
		jtag_read_mem_b(ctxt, 0xF02002));
	...
	printf("SCR %02x\n", jtag_read_mem_b(ctxt, 0xF02707));
	printf("SCR %02x\n", jtag_read_mem_b(ctxt, 0xF02707));
	printf("READ MEMH : %04x %04x\n", jtag_read_mem_h(ctxt, 0xF02000),
		jtag_read_mem_h(ctxt, 0xF02002));
	printf("READ MEMH : %04x %04x\n", jtag_read_mem_h(ctxt, 0xF02000),
		jtag_read_mem_h(ctxt, 0xF02002));
...
void display_usage(char *program)
{
	fprintf(stderr, "Usage: %s [-d] [-v <VID>] [-p <PID>] [-i <1|2>] "
		...
		program);
	fprintf(stderr, COMMON_CMD_HELP);
