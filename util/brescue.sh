...
# Ti50.
#
# The two input paramters are the file name of the full binary mage (both ROs
...
rw_offset() {
  local src="$1"
  local base=$(/usr/bin/od -Ax -t x1 -v "${src}" |
    egrep '^....00 fd ff ff ff' |
    head -2 |
    tail -1 |
    sed 's/ .*//')
  printf '%d' "0x${base}"
...
if [[ ! -z $(lsof "${device}" 2>/dev/null) ]]; then
