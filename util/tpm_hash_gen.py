...
def hash_cr50_file(cr50_file):
    """Calculate UML key of a Cr50 image.
    ...
    uml_digest_list = [hex_digest[2*x:2*x + 2] for x in range(4)]
    hex_uml_key = ''.join(uml_digest_list[::-1])
    uml_key = int(hex_uml_key, 16) & 0x7fffffff
    return uml_key


def main(cr50_files):
    """Calculate and report UML keys for passed in Cr50 images.
