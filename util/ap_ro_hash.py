...
def send_to_cr50(ranges, digest):
    ...
    if tpm_response.rc == 0:
        return 0
    if tpm_response.rc == VENDOR_RC_NO_SUCH_COMMAND_ERROR:
...
def main(args):
    ...
    if rv != 0:
        err_str = error_codes.get(rv, 'Unkonwn')
        LOG.error('Cr50 returned %d (%s)' % (rv, err_str))
        return rv
    print('SUCCEEDED')
    return 0
