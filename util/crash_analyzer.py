...
# These Regexs have only been tested in Cortex-M0+ / NDS32 crash reporter.
...
def process_crash_file(filename: str) -> dict:
    ...
    with open(filename, "r") as crash_file:
        ...
        for key, arch in get_archs().items():
            regex = arch["regex"]
...
"""
    ...
    if args.command == "lite":
        cmd_report_lite(args.crash_folder, args.with_file_name)
    elif args.command == "full":
        cmd_report_full(args.crash_folder, args.with_file_name)
    else:
