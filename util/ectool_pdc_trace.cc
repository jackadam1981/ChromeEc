...
static void pl_entries(const uint8_t *data, size_t data_size);
...
int cmd_pdc_trace(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "d:p:vw:")) != -1) {
		char *s;
		int port;
	...
	while (1) {
		...
		pl_entries(gr->payload, payload_size);
	}
...
BUILD_ASSERT(sizeof(struct pdc_trace_msg_header) == 3);
...
static void pl_entries(const uint8_t *data, size_t data_size)
{
	...
	for (;;) {
		...
		if (consumed_bytes + e_size > data_size) {
			fprintf(stderr, "entry header out of bounds\n");
			break;
		}

		e_size += e->pdc_data_size;
		if (consumed_bytes + e_size > data_size) {
			fprintf(stderr, "entry out of bounds\n");
			break;
		}
		...
		if (verbose) {
			...
			if (e->direction)
				...
				rxtx = 'R';
			printf("%3zu byte entry at %3zu/%zu: C%u %cX T%02x",
			       e_size, consumed_bytes, data_size, e->port_num,
			       rxtx, e->msg_type);
		...
		if (pcap != NULL) {
			...
			ts.tv_sec = e->time_us / 1000000;
			ts.tv_nsec = (e->time_us % 1000000) * 1000;
			pdc_pcap_append(pcap, ts, pcap_buf,
