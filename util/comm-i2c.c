			    ...
			    void *indata, int insize)
{
	...
	req_len = 1 + sizeof(struct ec_host_request) + outsize;
	...
	memcpy(&req_buf[1 + sizeof(struct ec_host_request)], outdata, outsize);

	req->checksum = (uint8_t)(-sum_bytes(&req_buf[1], req_len - 1));
	...
	resp_len = 2 + sizeof(struct ec_host_response) + insize;
	...
	if (error < 0) {
		fprintf(stderr, "I²C write failed: %d (err: %d, %s)\n",
	...
	error = ioctl(i2c_fd, I2C_RDWR, &data);
	if (error < 0) {
		fprintf(stderr, "I²C read failed: %d (err: %d, %s)\n",
			error, errno, strerror(errno));
		goto done;
	}
	...
	if ((uint8_t)sum_bytes(&resp_buf[2], resp_buf[1]) != 0) {
		debug("Bad checksum on EC response.\n");
		ret = -EC_RES_ERROR;
		goto done;
	}

	memcpy(indata, &resp_buf[2 + sizeof(struct ec_host_response)], insize);
...
int comm_init_i2c(void)
{
	...
	ec_max_outsize =
		I2C_MAX_HOST_PACKET_SIZE - 1 - sizeof(struct ec_host_request);
	ec_max_insize =
		I2C_MAX_HOST_PACKET_SIZE - 2 - sizeof(struct ec_host_response);
