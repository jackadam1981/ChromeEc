...
def git_commit_msg(branch, head, merge_head, rel_paths):
  ...
  order to extract the bug numbers. It constructs the git commit message showing
  the command used to find the relevant commits.
  ...
  msg ="Merge remote-tracking branch 'cros/main' into " + branch
  msg += """

Relevant changes:

"""
  msg += textwrap.fill(relevant_commits_cmd + '\n\n', width=72)
  msg += '\n\n' + relevant_commits

  msg += '\n' + 'BRANCH=None\n'
  bug_field = textwrap.wrap(' '.join(list(relevant_bugs)), width=68)
  for line in bug_field:
    msg += 'BUG=' + line + '\n'
  msg += 'TEST=`make -j buildall`\n'
...
def get_relevant_commits(head, merge_head, fmt, relevant_paths):
  ...
  Args:
    ...
    relevant_paths: String containing all the relevant paths for this particular
                    baseboard or board.
  Returns:
    A tuple containing the agruments passed to the git log command and stdout.
  """
  if fmt:
...
def main(argv):
  ...
  For the commit message, it finds all the commits that have modified a relevant
  path. By default this is the baseboard or board directory.  The user may
  optionally specify a path to a text file which contains a longer list of
  relevant files.  The format should be in the glob syntax that git log expects.
  ...
  subprocess.run(['git', 'merge', '--no-ff', '--no-commit', 'cros/main', '-s',
                  'recursive', '-Xtheirs'])
