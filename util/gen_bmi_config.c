...
const char cmd_name[] = "gen_bmi_config";
			 ...
			 const char *outfilename)
{
	...
	if (repeat_count > 2) {
		...
		fwrite(&key, 2, 1, file_p);
		fwrite(&repeat_count, 2, 1, file_p);
		outsize += 4;
...
static void print_help(void)
{
	printf("\nUsage: %s <compress|decompress> <infile> <outfile>\n"
	       "\n"
	       "Utility to compress/decompress BMI IMU configuration binaries.\n",
	       cmd_name);
}
...
int main(int argc, char *argv[])
{
	...
	if (!strncmp(argv[1], "compress", sizeof("compress"))) {
		...
		printf("Compressed file %s created - %d bytes "
			"(saves %d bytes\n",
			outfilename, outsize, (8 * 1024) - outsize);
