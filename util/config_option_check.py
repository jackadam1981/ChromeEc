...
class MissingConfigOption(object):
  ...
  Attributes:
    ...
    max_option_length = Used for formatting, the longest CONFIG_* option length
  """
  def __init__(self, n, l, f, m):
    """Inits MissingConfigOption with its attributes."""
    self.line_number = l
    self.filename = f
    self.name = n
    self.max_option_length = m

  def __str__(self):
    """Neatly formats output such that filenames are left-aligned."""
    res = '> '
    res += self.name + " "
    res += ' ' * (self.max_option_length + 2 - len(self.name))
...
def obtain_current_config_options():
  ...
  return config_options, max_option_length

def check_for_missing_config_options(file_list, config_options, max_opt_length):
...
def main():
  ...
  config_options, max_option_length = obtain_current_config_options()
  # Find any missing config options from file list.
  missing_config_options = check_for_missing_config_options(file_list,
                                                            config_options,
                                                            max_option_length)

  # Print any misssing config options.
