...
static int wait_for_ec(int status_addr, int timeout_usec)
{
	...
	for (i = 0; i < timeout_usec; i += delay) {
		int remind = timeout_usec - i;
		int actual = (remind < delay) ? remind : delay;

		/* Delay first, in case we just sent a command */
		usleep(actual);
		...
		delay = delay * 2;
		delay = delay > MAXIMUM_UDELAY ? MAXIMUM_UDELAY : delay;
	}
	return -1;  /* Timeout */
}
