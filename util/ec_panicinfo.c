...
int parse_panic_info(const char *data, size_t size)
{
	...
	if (size < (header_size + trailer_size)) {
		fprintf(stderr, "Panic data too short (%zd).", size);
		return -1;
	}

	if (size > sizeof(pdata)) {
		fprintf(stderr, "Panic data too large (%zd > %zd). "
	 ...
	 */
	if (pdata.struct_version > 2)
		fprintf(stderr, "Unknown panic data version (%d). "
			"Following data may be incorrect!\n",
			pdata.struct_version);

	/* Validate magic number */
	if (pdata.magic != PANIC_DATA_MAGIC)
		fprintf(stderr, "Incorrect panic magic (%d). "
			"Following data may be incorrect!\n",
			pdata.magic);

	if (pdata.struct_size != size)
		fprintf(stderr, "Panic struct size inconsistent (%u vs %zd). "
			"Following data may be incorrect!\n",
