...
int parse_panic_info(const char *data, size_t size)
{
	...
	/*
	 * First, copy the raw data into pdata, as the structure size may have
	 * changed.
	 */
	if (size < (header_size + trailer_size)) {
		printf("Panic data too short (%zd).", size);
		return -1;
	}

	if (size > sizeof(pdata)) {
		printf("Panic data too large (%zd > %zd). "
			"Following data may be incorrect!\n",
			size, sizeof(pdata));
	}
	memcpy(&pdata, data, MIN(sizeof(pdata), size));
	 ...
	 */
	if (pdata.struct_version > 2)
		printf("Unknown panic data version (%d). "
			"Following data may be incorrect!\n",
			pdata.struct_version);

	/* Validate magic number */
	if (pdata.magic != PANIC_DATA_MAGIC)
		printf("Incorrect panic magic (%d). "
			"Following data may be incorrect!\n",
			pdata.magic);

	if (pdata.struct_size != size)
		printf("Panic struct size inconsistent (%u vs %zd). "
			"Following data may be incorrect!\n",
