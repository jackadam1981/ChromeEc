...
def readline(file_name: str):
    try:
        ...
        logging.warning("\t\t OSError:", sys.exc_info()[1].strerror)
        return ""
...
def get_uart_dev_name(deviceid: str) -> str:
    ...
    if not dirs:
        logging.warning("Failed to locate device for:", deviceid)
        return ""
    if len(dirs) > 1:
...
def get_gpio_by_index(ranges: list, idx: int) -> int:
    ...
    for g_range in ranges:
        ...
        logging.error("GPIO pin index", idx, "does not belong to any supported"
            "range")
...
def get_gpio_by_name(bases: dict, name: str) -> int:
    ...
    if not bases or rc != 0:
        logging.error("Failed to find GPIO", name)
        sys.exit(ExitCode.EXIT_RUNTIME)
    gpio = stdout.split()
    # e.g.
    #   gpiofind FP_RST_L
...
def gpio(cmd, *signals) -> list:
    ...
    if not cmd in cmds:
        logging.error("Invalid gpio command:", cmd)
        sys.exit(ExitCode.EXIT_RUNTIME)
...
def warn_gpio(signal: str, expected_value: str, msg: str):
    ...
    if not value:
        logging.error("Error reading gpio", signal, "value")
        sys.exit(ExitCode.EXIT_RUNTIME)
        ...
        try:
            with open("/etc/lsb-release", 'r') as fp:
                for line in fp.readlines():
                    ...
                    logging.warning("Failed to find", key,
                            "in /etc/lsb-release")
        except OSError:
...
def get_default_fw():
    ...
    if rc != 0:
        logging.error("Failed to identify fingerprint board name")
...
def flash_fp_mcu_stm32(args):
    ...
    if args.hello:
        logging.info(f"# Saying hello over {args.transport}")
    else:
        ...
        if args.read:
            ...
            if os.path.isfile(args.binary):
                logging.error(f"Output file already exists: {args.binary}")
                sys.exit(ExitCode.EXIT_PRECONDITION)
            logging.info(f"# Reading to {args.binary} over {args.transport}")
            stm32_flags += f" -r {args.binary}"
        elif os.path.isfile(args.binary):
            # Write to FPMCU from file
            logging.info(f"# Flashing '{args.binary}' over {args.transport}")
            stm32_flags += f" -e -w {args.binary}"
        else:
            logging.error(f"Invalid image file: {args.binary}")
            sys.exit(ExitCode.EXIT_PRECONDITION)
    ...
    if not deviceid:
        logging.error(f"Unable to find FP sensor {args.transport} device")
        sys.exit(ExitCode.EXIT_PRECONDITION)

    logging.info(f"Flashing {args.transport} device ID: {deviceid}")
    ...
    if args.transport == "UART":
        ...
        logging.info(f"Serial device: {dev_name}")
