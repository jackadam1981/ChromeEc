...
def readline(file_name: str):
    try:
        ...
        perror(f"------------------- Error reading from {file_name}")
        perror("                     OSError:", sys.exc_info()[1].strerror)
        return ""
...
def writeline(file_name: str, data: str):
    try:
        ...
        perror(f"------------------- Error writing: {data} to {file_name}")
        perror("                     OSError:", sys.exc_info()[1].strerror)
...
def assert_wp_is_disabled():
    ...
    if rc != 0:
        perror("Failed to get hardware write protect status")
        sys.exit(ExitCode.EXIT_PRECONDITION)

    if wp_enabled != '0':
        perror("Please make sure hardware write protect is disabled.")
        perror("See https://www.chromium.org/chromium-os/firmware-porting-guide"
...
def get_uart_dev_name(deviceid: str) -> str:
    ...
    if not dirs:
        perror("Failed to locate device for:", deviceid)
        return ""
    if len(dirs) > 1:
        perror(f"Device for {deviceid} is ambiguous")
        return ""
...
# idx[first] - idx[last] is the sequential index of the gpio in the device
#
...
def read_gpio_ranges():
    ...
    for filename in path:
        try:
            ...
            perror(f"------------------- Error reading from {filename}")
            perror("                     OSError:", sys.exc_info()[1].strerror)
...
#
# The result is a dictionary of lables (device names) and bases
# bases
#       device1: base1
#       device2: base2
...
def get_gpio_by_index(ranges: list, idx: int) -> int:
    ...
    for g_range in ranges:
        ...
        perror("GPIO pin index", idx, "does not belong to any supported range")
        perror("Supported ranges:")
        for g_range in ranges:
            perror(f"\t{g_range['idx']['first']} - {g_range['idx']['last']}")
...
def get_gpio_by_name(bases: dict, name: str) -> int:
    ...
    if not bases or rc != 0:
        perror("Failed to find GPIO", name)
        sys.exit(ExitCode.EXIT_RUNTIME)
    gpio = stdout.split()
    # e.g.
    #   gpiofind FP_RST_L
...
def gpio(cmd, *signals) -> list:
    ...
    if not cmd in cmds:
        perror("Invalid gpio command:", cmd)
        sys.exit(ExitCode.EXIT_RUNTIME)
...
def warn_gpio(signal: str, expected_value: str, msg: str):
    ...
    if not value:
        perror("Error reading gpio", signal, "value")
        sys.exit(ExitCode.EXIT_RUNTIME)
        ...
        try:
            with open("/etc/lsb-release", 'r') as fp:
                for line in fp.readlines():
                    ...
                    perror("Failed to find", key, "in /etc/lsb-release")
        except OSError:
            perror(f"------------------- Error reading /etc/lsb-release")
            perror("                     OSError:", sys.exc_info()[1].strerror)

    perror("Getting platform name from /etc/lsb-release.")
...
def get_default_fw():
    ...
    if rc != 0:
        perror("Failed to identify fingerprint board name")
    ...
    if len(fws) == 0:
        perror("Failed to identify the default fingerprint fw name")
        sys.exit(ExitCode.EXIT_CONFIG)
    if len(fws) != 1:
        perror("Multiple fingerprint fw names")
        sys.exit(ExitCode.EXIT_CONFIG)
    return fws[0]
...
def flash_fp_mcu_stm32(args):
    ...
    if args.transport == "UART":
        stm32_flags += f" --baudrate {args.baudrate} --device {args.device}"
    else:
        stm32_flags += f" -s {args.device}"

    if args.hello:
        print(f"# Saying hello over {args.transport}")
    else:
        ...
        if args.read:
            ...
            if os.path.isfile(args.binary):
                perror(f"Output file already exists: {args.binary}")
                sys.exit(ExitCode.EXIT_PRECONDITION)
            print(f"# Reading to {args.binary} over {args.transport}")
            stm32_flags += f" -r {args.binary}"
        elif os.path.isfile(args.binary):
            # Write to FPMCU from file
            print(f"# Flashing '{args.binary}' over {args.transport}")
            stm32_flags += f" -e -w {args.binary}"
        else:
            perror(f"Invalid image file: {args.binary}")
            sys.exit(ExitCode.EXIT_PRECONDITION)
    ...
    if not deviceid:
        perror(f"Unable to find FP sensor {args.transport} device")
        sys.exit(ExitCode.EXIT_PRECONDITION)

    print(f"Flashing {args.transport} device ID: {deviceid}")
    ...
    writeline(unbind_file, deviceid)

    # Configure the MCU Boot0 and NRST GPIOs
    gpio("export", args.gpios.BOOT0, args.gpios.NRST)
    gpio("out", args.gpios.BOOT0, args.gpios.NRST)

    # Reset sequence to enter bootloader mode
    gpio("1", args.gpios.BOOT0)
    gpio("0", args.gpios.NRST)
    time.sleep(0.001)

    klog("Binding raw driver")
    if args.transport == "UART":
        dev_name = get_uart_dev_name(deviceid)
        print(f"Serial device: {dev_name}")
        ...
        driver_override = f"/sys/bus/spi/devices/{deviceid}/driver_override"
        writeline(driver_override, "spidev")
        writeline("/sys/bus/spi/drivers/spidev/bind", deviceid)
    ...
    warn_gpio(f"{args.gpios.BOOT0}", '1',
            "WARNING: One of the drivers changed BOOT0 pin state on bind "
            + "attempt.")
    warn_gpio(f"{args.gpios.NRST}", '0',
            "WARNING: One of the drivers changed NRST pin state on bind "
            + "attempt.")

    if (not os.path.exists(args.device)
        or not stat.S_ISCHR(os.stat(args.device).st_mode)):
        perror("Failed to bind raw device driver.")
        sys.exit(ExitCode.EXIT_RUNTIME)
    ...
    for attempt in range(args.retries):
        ...
        print(f"# {cmd}")
        rc = run_system_cmd(cmd, show_output=True)
        if rc == 0:
            break
        perror(f"Attempt {attempt} failed.")
        time.sleep(1)

    # unload device
    if args.transport != "UART":
        klog("Unbinding raw driver")
        writeline("/sys/bus/spi/drivers/spidev/unbind", deviceid)

    # Go back to normal mode
    gpio("out", args.gpios.NRST)
    gpio('0', args.gpios.BOOT0, args.gpios.NRST)
    gpio('1', args.gpios.NRST)
    ...
    if args.platform_name != "strongbad" and args.platform_name != "herobrine":
        gpio("in", args.gpios.BOOT0, args.gpios.NRST)

    gpio("unexport", args.gpios.BOOT0, args.gpios.NRST)
    ...
    # See to b/146428434.
    if args.gpios.PWREN > 0:
        print("Power cycling the FPMCU.")
        gpio("export", args.gpios.PWREN)
        gpio("out", args.gpios.PWREN)
        gpio('0', args.gpios.PWREN)
        # Must outlast hardware soft start, which is typically ~3ms.
        time.sleep(0.5)
        gpio('1', args.gpios.PWREN)
        # Power enable line is externally pulled down, so leave as output-high.
        gpio("unexport", args.gpios.PWREN)
    ...
    if args.transport != "UART":
        ...
        writeline("/sys/bus/spi/drivers/cros-ec-spi/bind", deviceid)
    ...
    if args.transport == "UART":
        print("Please reboot this device.")
    else:
        # Test it
...
def cmd_flash(args: argparse.Namespace) -> int:
    ...
    if not args.platform_name:
        perror("Failed to get platform name")
        sys.exit(ExitCode.EXIT_CONFIG)

    platform_base_name = get_platform_base_name(args.platform_name)
    if not platform_base_name:
        perror("Failed to get platform base name")
        sys.exit(ExitCode.EXIT_CONFIG)

    print(f"Platform name is {args.platform_name} ({platform_base_name}).")
    ...
    if config_func in locals():
        ...
        if config_func in locals():
            ...
            perror(f"No config for platform {args.platform_name}")
            sys.exit(ExitCode.EXIT_CONFIG)
    ...
    print(f"Using config for {args.platform_name}.")
    ...
    if not args.noservices:
        print("# Stopping biod and timberslide")
    ...
    if (not os.path.exists("/dev/cros_fp") or not
        ...
        perror("WARNING: The cros-ec driver was not bound on startup.")

    if (os.path.exists(args.device) and
        stat.S_ISCHR(os.stat(args.device).st_mode)):
        perror(f"WARNING: The raw driver {args.device} was bound on startup.")
    ...
    if files_open:
        perror("WARNING: Another process has a cros_fp device file open.")
        perror(f"{os.linesep.join(files_open)}")
        perror("Try 'stop biod' and")
        perror("'stop timberslide LOG_PATH=/sys/kernel/debug/cros_fp/"
            + "console_log'")
        perror("before running this script.")
        perror("See b/188985272.")
    ...
    if files_open:
        perror(f"WARNING: Another process has {args.device} open.")
        perror(f"{os.linesep.join(files_open)}")
        perror(f"Try 'fuser -k {args.device}' before running this script.")
        perror("See b/188985272.")

    rc = flash_fp_mcu_stm32(args)

    if not args.noservices:
        print("# Restarting biod and timberslide")
