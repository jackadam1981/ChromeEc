...
def proc_open_files(pattern) -> list:
    ...
    """
    pids = []
    _rc, ls_l, _stderr = run_system_cmd('ls -l /proc/*/fd/* 2>/dev/null | grep '
                                        + '\"' + pattern + '\"')
    ls_l = ls_l.split('\n')
    for ls in ls_l:
        if ls:
...
def cmd_flash(args: argparse.Namespace):
    ...
    class config_platform:
        ...
        def __new__(cls, board: str):
            if not board in config_platform.config_func:
                return None
            return super().__new__(cls)

        def __init__(self, board: str):
            self.transport, self.device, self.gpios = self.config_func[board]()
