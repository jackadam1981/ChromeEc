...
def main():
    ...
    if not args.image.endswith('.png'):
        logging.error("Input file must be png, sorry. you input", args.image)
    img = Image.open(args.image)

    #check that image is correct size for fpmcu.
    h, w = img.size
    if args.board  == "bloonchipper":
        if w != h != 160:
            logging.error("This img size is not 160x160")
    elif args.board == "dartmonkey":
        if w != 56 or h != 192:
    ...
    with ExitStack() as stack:
        ...
        # fpimageload takes image as a single string of hex values. Each pixel gets 2 hex vals.
        imgstr = ""
        for i in range(h):
            for j in range(w):
                if args.board  == "dartmonkey":
                    coordinates = x, y = i, j
                else:
                    coordinates = x, y = j, i
                tmp = hex(img.getpixel(coordinates))
                if len(tmp) == 4:
                    imgstr += tmp[-2:]
                else:
                    imgstr += "0" + tmp[2]
        if len(imgstr) == 0:
            logging.error("empty string, failed to read file")
        imgstrlen = int(len(imgstr)/2)
        if args.debug:
            upload_cmd = "fpimageload 1 " + str(imgstrlen) + " " + imgstr + "\n"
        else:
            upload_cmd = "fpimageload 0 " + str(imgstrlen) + " " + imgstr + "\n"
