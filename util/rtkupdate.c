...
#if TOOL_DBG
#define DBG_PRINT(format, args...) fprintf(format, ##args)
#else
	...
	while (retry--) {
		...
		if (bytes_read > 0) {
			...
			if (sync_receive == UART_SYNC_RESPONSE) {
				...
				fprintf("Unexpected response: 0x%X (expected 0xA5)\n",
					sync_receive);
			}
		} else if (bytes_read == 0) {
			fprintf("Read timeout occurred\n");
		} else {
		...
		if (retry > 0) {
			fprintf("Retrying UART sync... (%d attempts left)\n",
				retry);
			sleep(1); /* Wait 1 second before retrying */
		}
	}

	fprintf("UART sync failed after 3 attempts\n");
...
void set_read_blocking(int dev_drv, int block)
{
	...
	if (tcgetattr(dev_drv, &tty) != 0) {
		fprintf("read_blocking step 1: Fail.");
	}
	tty.c_cc[VMIN] = block;
	tty.c_cc[VTIME] = 5; /* 0.5 seconds read timeout */
	if (tcsetattr(dev_drv, TCSANOW, &tty) != 0) {
		fprintf("read_blocking step 2: Fail.");
	}
}
	...
	while (total_wait_time < timeout_seconds * 1000) {
		...
		if (bytes_read > 0) {
			...
			if (response == expected_response) {
				...
				fprintf("Unexpected response: 0x%X (expected: 0x%X)\n",
			...
			fprintf("No data received, retrying...\n");
			usleep(WAIT_INTERVAL * 1000); /* Wait 100ms */
			total_wait_time += WAIT_INTERVAL;
		}
	}

	fprintf("Timeout waiting for response: 0x%X\n", expected_response);
	       ...
	       size_t *total_bytes_sent, size_t *page)
{
	...
	if (retry == 1) {
		...
		if (wait_for_response(uart_fd, 0x09, RESPONSE_TIMEOUT) != 0) {
			fprintf("Failed to retry expected response for data page %zu\n",
				*page + 1);
			return -1;
		}
	}
...
int flash(int uart_fd, uint32_t spi_start, const char *file_name)
{
	...
	while (total_bytes_sent < total_file_size) {
		...
		/* Wait for EC to respond with 0x09 */
		if (wait_for_response(uart_fd, 0x09, RESPONSE_TIMEOUT) != 0) {
			fprintf("Failed to receive expected response for upload header\n");
			fclose(file);
			return -1;
		}
...
int write_protect(int uart_fd, uint32_t protect)
{
	...
	if (wait_for_response(uart_fd, WP_COMMAND, WP_RESPONSE_TIMEOUT) != 0) {
		fprintf("Failed to receive expected response for WP operation\n");
		return -1;
	}
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, "m:s:f:u:o:p:", long_options,
		...
		case 'p':
			...
			break;
		default:
			printf("support 4 method : flash frame wp read_bin\n"
			       "Usage:\n"
	...
	if (!method || !uart_device) {
		fprintf("Missing required arguments\n");
		fprintf("support 4 method : flash frame wp read_bin\n"
			"Usage:\n"
			"    %s --method flash --uart_dev <dev> [flash options]\n"
			"    %s --method frame --uart_dev <dev> [frame options]\n"
			"    %s --method wp --uart_dev <dev> [wp options]\n"
	...
	if (strcmp(method, "flash") == 0) {
		if (!file_name || !spi_start) {
			fprintf("Flash missing required arguments\n");
			return 1;
		}
		if (flash(uart_fd, spi_start, file_name) != 0) {
			fprintf("Flash process failed\n");
			close(uart_fd);
			return 1;
		}
	} else if (strcmp(method, "frame") == 0) {
		if (!file_name) {
			fprintf("Frame missing required arguments\n");
			return 1;
		}
		if (frame(uart_fd, file_name) != 0) {
			fprintf("Frame process failed\n");
			close(uart_fd);
			return 1;
		}
	} else if (strcmp(method, "wp") == 0) {
		if (!protect) {
			fprintf("WP missing required arguments\n");
			return 1;
		}
		if (write_protect(uart_fd, protect) != 0) {
			fprintf("WP process failed\n");
			close(uart_fd);
			return 1;
		}
	} else if (strcmp(method, "read_bin") == 0) {
		if (!file_name || !bin_length || !spi_start) {
			fprintf("Read Bin missing required arguments\n");
			return 1;
		}
		if (read_bin(uart_fd, spi_start, file_name, bin_length) != 0) {
			fprintf("Read Bin failed\n");
			close(uart_fd);
			return 1;
		}
	} else {
