...
#define WRITE_DATA_TO_SRAM 0x09
#define START_FRAME_TO_WRITE_TO_FLASH 0x06
...
int uart_sync(int uart_fd)
{
	...
	while (retry--) {
		...
		if (bytes_read > 0) {
			...
			DBG_PRINT(stderr, "Read timeout occurred\n");
		} else {
...
int com_port_wait_read(int device_id)
{
	...
	/* Wait up to 1 sec until byte is received for read. */
	fds.fd = device_id;
	fds.events = POLLIN;
		      ...
		      int timeout_seconds)
{
	...
	if (read_temp < 1) {
		fprintf(stderr, "Timeout waiting for initial byte");
		return -1;
	}
	...
	while (total_wait_time < timeout_seconds * 1000) {
		...
		if (bytes_read > 0) {
			...
			DBG_PRINT(stderr, "No data received, retrying...\n");
