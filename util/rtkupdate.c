		       ...
		       uint32_t data_size_to_write)
{
	...
	DBG_PRINT("Sent Upload Header: SRAM Address=0x%08X, SPI Address=0x%08X, Data Size to Write=0x%08X, CRC=0x%04X\n",
		  ...
		  uint32_t sram_address, const unsigned char *data)
{
	...
	DBG_PRINT("Sent Packet A: Command=0x%X, Data Size=%zu, SRAM Address=0x%08X, CRC=0x%04X\n",
...
int send_packet_b(int uart_fd, uint8_t command, uint32_t func_pointer)
{
	...
	DBG_PRINT("Sent Packet B: Command=0x%X, Function Pointer=0x%08X, CRC=0x%04X\n",
...
int flash(int uart_fd, uint32_t spi_start, const char *file_name)
{
	...
	while (total_bytes_sent < total_file_size) {
		...
		for (size_t i = 0;
		     ...
		     i++) {
			size_t bytes_read =
				fread(data_buffer, 1, PAGE_SIZE, file);
			retry = 0;
			if (bytes_read == 0) {
				if (feof(file))
					break; /* End of file */
			...
			if (send_packet_a(uart_fd, 0x09, (bytes_read - 1),
			 ...
			 * this page) */
			if (wait_for_response(uart_fd, 0x09,
					      RESPONSE_TIMEOUT) != 0) {
				DBG_PRINT("Failed to receive expected response for data page %zu\n",
				       page + 1);
				retry = 1;
			}
...
retry_r:
        /* Send Packet B to request EC to move data, even if this round is not full 16
         * pages */
		DBG_PRINT("Round %zu complete, sending function pointer to EC.\n",
		...
		if (wait_for_response(uart_fd, 0x06, RESPONSE_TIMEOUT) != 0) {
			DBG_PRINT("Failed to receive expected response for function pointer (first 0x06)\n");
			if (!retry_round) {
				retry_round = 1;
				goto retry_r;
			}
		...
		if (bytes_read != 2 || response[0] != 0x06 ||
		    response[1] != 0x03) {
			DBG_PRINT("Failed to receive expected 0x06 0x03 response, received: 0x%X 0x%X\n",
...
int frame(int uart_fd, uint32_t spi_start, const char *file_name)
{
	...
	while (total_bytes_sent < total_file_size) {
		...
		for (size_t i = 0; total_bytes_sent < total_file_size; i++) {
			size_t bytes_read =
				fread(data_buffer, 1, FRAME_PAGE_SIZE, file);
			if (bytes_read == 0) {
				if (feof(file)) {
					break; /* End of file */
				} else {
