...
FILE *file_ptr;
#if TOOL_DBG
#define DBG_PRINT(format, args...) fprintf(file_ptr, format, ##args)
#else
...
int uart_sync(int uart_fd)
{
	...
	while (retry--) {
		...
		if (bytes_read > 0) {
			...
			fprintf(file_ptr, "Read timeout occurred\n");
		} else {
	...
	fprintf(file_ptr, "UART sync failed after 3 attempts\n");
...
void set_read_blocking(int dev_drv, int block)
{
	...
	if (tcgetattr(dev_drv, &tty) != 0) {
		fprintf(file_ptr, "read_blocking step 1: Fail.");
	}
	tty.c_cc[VMIN] = block;
	tty.c_cc[VTIME] = 5; /* 0.5 seconds read timeout */
	if (tcsetattr(dev_drv, TCSANOW, &tty) != 0) {
		fprintf(file_ptr, "read_blocking step 2: Fail.");
	}
}
...
int com_port_read_bin(int device_id, uint8_t *buffer, uint32_t buf_size)
{
	...
	if (read_bytes == -1) {
		fprintf(file_ptr, "port_read step 1: Fail.");
		return -1;
	}
...
int com_port_wait_read(int device_id)
{
	...
	if (ret_val < 0) {
		fprintf(file_ptr, "port_wait step 1: Fail.");
		return -1;
	}
	bytes = 0;
	...
	if (ret_val > 0) {
		...
		if (ioctl(device_id, FIONREAD, &bytes) < 0) {
			fprintf(file_ptr, "port_wait step 2: Fail.");
			return -1;
		}
	}
		      ...
		      int timeout_seconds)
{
	...
	while (total_wait_time < timeout_seconds * 1000) {
		...
		if (bytes_read > 0) {
			...
			fprintf(file_ptr, "No data received, retrying...\n");
			usleep(WAIT_INTERVAL * 1000); /* Wait 100ms */
			total_wait_time += WAIT_INTERVAL;
		}
	}

	fprintf(file_ptr, "Timeout waiting for response: 0x%X\n",
		expected_response);
...
int main(int argc, char *argv[])
{
	...
	file_ptr = fopen("flash_ec_log.txt", "w");
	if (file_ptr == NULL) {
		printf("ERROR openging File");
		return 1;
	}
	...
	if (!method || !uart_device) {
		fprintf(file_ptr, "Missing required arguments\n");
		fprintf(file_ptr,
			"support 4 method : flash frame wp read_bin\n"
			"Usage:\n"
			"    %s --method flash --uart_dev <dev> [flash options]\n"
			"    %s --method frame --uart_dev <dev> [frame options]\n"
			"    %s --method wp --uart_dev <dev> [wp options]\n"
	...
	if (uart_sync(uart_fd) != 0) {
		fprintf(file_ptr, "UART sync failed\n");
		close(uart_fd);
		return 1;
	}
	...
	if (strcmp(method, "flash") == 0) {
		if (!file_name || (spi_start == 0xFFFFFFFF)) {
			fprintf(file_ptr, "Flash missing required arguments\n");
			return 1;
		}
		if (flash(uart_fd, spi_start, file_name) != 0) {
			fprintf(file_ptr, "Flash process failed\n");
			close(uart_fd);
			return 1;
		}
	} else if (strcmp(method, "frame") == 0) {
		if (!file_name) {
			fprintf(file_ptr, "Frame missing required arguments\n");
			return 1;
		}
		if (frame(uart_fd, file_name) != 0) {
			fprintf(file_ptr, "Frame process failed\n");
			close(uart_fd);
			return 1;
		}
	} else if (strcmp(method, "wp") == 0) {
		if (!protect) {
			fprintf(file_ptr, "WP missing required arguments\n");
			return 1;
		}
		if (write_protect(uart_fd, protect) != 0) {
			fprintf(file_ptr, "WP process failed\n");
			close(uart_fd);
			return 1;
		}
	} else if (strcmp(method, "read_bin") == 0) {
		if (!file_name || !bin_length || (spi_start == 0xFFFFFFFF)) {
			fprintf(file_ptr,
				"Read Bin missing required arguments\n");
			return 1;
		}
		if (read_bin(uart_fd, spi_start, file_name, bin_length) != 0) {
			fprintf(file_ptr, "Read Bin failed\n");
			close(uart_fd);
			return 1;
		}
	} else {
		fprintf(file_ptr, "Unknown method: %s\n", method);
		close(uart_fd);
		return 1;
	}
