...
int configure_uart(int fd)
{
	...
	/* Disable output processing and enable others */
	tty.c_oflag = ~OPOST;
...
int uart_sync(int uart_fd)
{
	...
	while (retry--) {
		...
		/* Wait for response */
		ssize_t bytes_read =
			read_exact(uart_fd, &sync_receive, 1, 1000);
		if (bytes_read > 0) {
			...
			DBG_PRINT("Read timeout occurred\n");
		} else {
		  ...
		  uint32_t sram_address, const unsigned char *data)
{
	...
	size_t packet_length =
		PACKET_HEADER_LENGTH + actual_data_size + CHECKSUM_LENGTH;
	...
	uint16_t Checksum = calculate_Checksum_tool(
		packet, PACKET_HEADER_LENGTH + actual_data_size);

	/* Add Checksum to the end of the packet */
	packet[PACKET_HEADER_LENGTH + actual_data_size] = (Checksum >> 8) &
							  0xFF; /* High byte */
	packet[PACKET_HEADER_LENGTH + actual_data_size + 1] = Checksum &
							      0xFF; /* Low
								       byte
								     */
		      ...
		      int timeout_seconds)
{
	...
	while (total_wait_time < timeout_seconds * 1000) {
		...
		bytes_read = read_exact(uart_fd, &response, 1, 1000);
		...
		if (bytes_read > 0) {
			...
			DBG_PRINT("Error reading from UART");
			return -1;
		} else if (bytes_read == 0) {
			DBG_PRINT("No data received, retrying...\n");
	       ...
	       size_t *total_bytes_sent, size_t *page)
{
	...
	if (send_packet_a(uart_fd, WRITE_DATA_TO_SRAM, (bytes_read - 1),
...
int flash(int uart_fd, uint32_t spi_start, const char *file_name)
{
	...
	while (total_bytes_sent < total_file_size) {
		...
		for (uint8_t retry_round = 0; retry_round < FLASH_RETRY_CNT;
			...
			ssize_t bytes_read =
				read_exact(uart_fd, response, 2, 1000);
			usleep(200 * 1000);
			if (bytes_read <= 0) {
				ERR_PRINT(
			...
			if ((response[0] != 0x06) || (response[1] != 0x03)) {
				ERR_PRINT(
...
void out_of_read_bin(int uart_fd)
{
	...
	send_packet_a(uart_fd, WRITE_DATA_TO_SRAM, 3, sram_address,
		      data_buffer);
}
	     ...
	     uint32_t bin_length)
{
	...
	while (total_bytes_get < bin_length) {
		...
		/* Send this page's data */
		if (send_packet_a(uart_fd, WRITE_DATA_TO_SRAM, 3, sram_address,
				  data_buffer) != 0) {
			goto read_bin_err;
		}
		...
		bytes_read = read_exact(uart_fd, bootrom_response, 2, 1000);
		usleep(200 * 1000);
		if (bytes_read <= 0) {
			ERR_PRINT(
