		       ...
		       uint32_t data_size_to_write)
{
	...
	DBG_PRINT(
		"Sent Upload Header: SRAM Address=0x%08X, SPI Address=0x%08X, Data Size to Write=0x%08X, CRC=0x%04X\n",
		  ...
		  uint32_t sram_address, const unsigned char *data)
{
	...
	DBG_PRINT(
		"Sent Packet A: Command=0x%X, Data Size=%zu, SRAM Address=0x%08X, CRC=0x%04X\n",
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, "m:s:f:u:o:", long_options,
		...
		case 'o':
			...
			break;
		default:
			printf("Usage: %s --method <method> --spi_start <spi_start> --file <binary_file> --uart_device <uart_device>\n",
			       argv[0]);
			return 1;
		}
	}
