...
def get_flash_file(board, is_zephyr):
    if is_zephyr:
        return os.path.abspath(f'../build/zephyr/{board}/output/zephyr.bin')
    else:
        raise RuntimeError('get_flash_file does not currently support non-zephyr builds')

def get_executable_file(board, is_zephyr):
    if is_zephyr:
        return os.path.abspath(f'../build/zephyr/{board}/output/zephyr.ro.elf')
    else:
        raise RuntimeError('get_executable_file does not currenly support non-zephyr builds')
...
def main():
    ...
    flash_parser = sub_parsers.add_parser('flash', help='flash help')
    flash_parser.set_defaults(command='flash')
    flash_parser.add_argument(
        '--verify', '-v',
        help='Verify flash after writing image',
        default=True
    )

    debug_parser = sub_parsers.add_parser('debug', help='debug help')
    ...
    if not args.zephyr:
        print('--file/-f needs to be used if --zephyr/-z is not given')
        return

    if args.command == 'flash':
        image_file = get_flash_file(args.board, args.zephyr) if target_file == '' else target_file
        flash(args.interface, args.board, image_file, args.verify)
    elif args.command == 'debug':
        executable_file = get_executable_file(args.board, args.zephyr) if target_file == '' else target_file
