...
// we can almost start the VM here
// 1. "echo | vmc start termina"
// 2. "echo | vmc container termina penguin"
//    (succeeds on 2nd invocation)

/*
 * wireshark -X lua_script:/mnt/chromeos/MyFiles/rts54.lua
 *     -n -k -f 'udp port 2012'
 *
 * tshark -X lua_script:/mnt/chromeos/MyFiles/rts54.lua -n -f 'udp port 2012'
 */

// 3. "echo wireshark -k | vmc container termina penguin"
...
int pdc_net_open(const char *hostname)
{
	...
	dst_want.ai_family = AF_UNSPEC; // AF_INET, AF_INET6
...
void pdc_net_out(const void *buf, size_t buf_len)
{
	...
	do {
		int cc = send(udp_tx_fd, buf, buf_len, 0);
		if (cc < 0 && retries == 0)
			perror("send");
	} while (retries-- > 0);
}
