...
class FlashInfoCmd(HostCommand):
    def __init__(self, num_banks_desc: int):
        ...
        self.response_fmt = "<IIIHH" + num_banks_desc * ec_flash_bank_fmt
        self.response_type = namedtuple(
            "Response",
            [
                "flash_size",
...
class FpInfoCmd2(HostCommand):
    ...
    def run(self, comm) -> int:
        ...
        if ret == 0:
            ...
            image_frame_len = struct.calcsize(image_frame_fmt)
            image_frame_nr = int(
                len(self.response.image_frame_params) / image_frame_len
            )
            new_image_frame_params = [
                image_frame_type(
                    *struct.unpack_from(
                        image_frame_fmt,
                        self.response.image_frame_params,
                        i * image_frame_len,
                    )
                )
                for i in range(image_frame_nr)
            ]
            self.response = self.response._replace(
                image_frame_params=new_image_frame_params
            )
        return ret
