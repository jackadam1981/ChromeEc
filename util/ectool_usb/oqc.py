...
def _check_serial(comm, results) -> int:
    ...
    try:
        ...
        return OqcTestError.SERIAL_NUMBER_PROBLEM
...
def run_oqc(configuration_file, comm, results) -> int:
    ...
    results[OqcTestStages.TEST_SEQUENCE] = (
        run_oqc.test_sequence,
        OqcTestResult.PASS,
    )
    run_oqc.test_sequence += 1
...
def cmd_oqc(args) -> int:
    ...
    with open(args.output, "a", newline="", encoding="utf-8") as csvfile:
        ...
        if os.fstat(csvfile.fileno()).st_size == 0:
            csv_writer.writerow(list(OqcTestStages) + ["Overall result"])

        while True:
            if args.batch:
                print("Press SPACE to run a test, ESC to exit.")
                key = get_key()
            else:
                key = " "
            ...
            if key == " ":
                ...
                csv_writer.writerow(
                    [comment for comment, _ in results.values()]
                    + [overall_result]
                )
                csvfile.flush()
                if not args.batch:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "--batch",
        action="store_true",
        help="Enable batch mode for interactive testing.",
    )
