...
def add_subcommand(subparsers, name, help_text, func):
    """Adds a subcommand to the subparsers object and returns the parser."""
...
def main():
    ...
    add_subcommand(subcmd, "version", "Get version", cmd_get_version)

    sub_fl_info = add_subcommand(subcmd, "flinfo", "Flash info", cmd_fl_info)
    sub_fl_info.add_argument("num_banks_desc", type=auto_int)

    add_subcommand(subcmd, "prinfo", "Protocol info", cmd_pr_info)

    sub_fp_mode = add_subcommand(subcmd, "fpmode", "FP mode", cmd_fp_mode)
    ...
    add_subcommand(subcmd, "fpinfo", "FP info", cmd_fp_info)

    sub_fp_vendor = add_subcommand(
        subcmd, "fpvendor", "Vendor specific command", cmd_fp_vendor
    )
    sub_fp_vendor.add_argument("param1", type=auto_int)

    sub_flash_read = add_subcommand(
        subcmd, "flashread", "Flash read", cmd_flash_read
    )
    add_arg_offset(sub_flash_read)
    add_arg_size(sub_flash_read)
    add_arg_file(sub_flash_read)

    sub_flash_write = add_subcommand(
        subcmd, "flashwrite", "Flash write", cmd_flash_write
    )
    add_arg_offset(sub_flash_write)
    add_arg_file(sub_flash_write)

    sub_flash_erase = add_subcommand(
        subcmd, "flasherase", "Flash erase", cmd_flash_erase
    )
    add_arg_offset(sub_flash_erase)
    add_arg_size(sub_flash_erase)

    sub_flash_region_info = add_subcommand(
        subcmd, "flashregioninfo", "Flash region info", cmd_flash_region_info
    )
    ...
    add_subcommand(
        subcmd, "bootloader", "Enter bootloader", cmd_enter_bootloader
    )

    sub_reboot_ec = add_subcommand(
        subcmd, "reboot_ec", "Reboot EC", cmd_reboot_ec
    )
    ...
    add_subcommand(subcmd, "reflash_rw", "Try reflashing rw", cmd_reflash_rw)
