...
def cmd_fl_info(args) -> int:
    ...
    for i in range(flash_info.response.num_banks_desc):
        print(f"Bank {i}:")
        print("\tNumber of sectors: " + str(flash_info.response["count" + i]))
        print(
            "\tSize of sector (in power of 2): "
            + hex(flash_info.response["size_exp" + i])
        )
        print(
            "\tMinimal write size (in power of 2): "
            + str(flash_info.response["write_size_exp" + i])
        )
        print(
            "\tErase size (in power of 2): "
            + str(flash_info.response["erase_size_exp" + i])
        )
        print(
            "\tSize for write protection (in power of 2): "
            + str(flash_info.response["protect_size_exp" + i])
        )
    return ret
...
def cmd_fp_mode(args) -> int:
    print(args)
    fp_mode_ec = commands.FpModeCmd(args.mode)
    ret = fp_mode_ec.run()
    if ret != 0:
        print(f"Failure with {ret}")
        return ret
    print("Mode: " + hex(fp_mode_ec.response.mode))
...
def main():
    ...
    sub_oqc.add_argument("configuration_file", type=argparse.FileType("r"))
