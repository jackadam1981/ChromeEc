...
def get_versions(command, comm) -> list[int]:
    get_ver = commands.GetVersionsCmd(command)
...
def cmd_get_version(args, comm) -> int:
...
def cmd_enter_bootloader(args, comm) -> int:
    enter_bootloader = commands.EnterBootloaderCmd()
    return enter_bootloader.run(comm)


def flash_read_to_file(file: str, offset: int, size: int, comm):
    read_bytes = 0
    ret = 0
...
def flash_write_from_file(file: str, offset: int, comm):
...
def cmd_reflash_rw(args, comm) -> int:
    ...
    flash_write_from_file(rw_file, flash_region_info.response.offset, comm)
    print("RW re-written")
