...
class TestUARTDisconnection(unittest.TestCase):
  ...
  def test_CommandsBufferedWhenDisconnected(self):
    """Verify that when in disconnected state, commands are buffered."""
    ...
    # The command queue should be filled with the chars.
    self.assertEqual(len(command), self.itpr.ec_cmd_queue.qsize())
    ...
    # Since we sent some commands in the disconnected state, the EC UART PTY
    # should be added to the outputs since there are commands pending.
    self.assertTrue(self.itpr.ec_uart_pty in self.itpr.outputs)

    for _ in range(len(command)):
      # The EC UART PTY should be a member of the outputs.
      self.assertTrue(self.itpr.ec_uart_pty in self.itpr.outputs)
      self.itpr.SendCmdToEC()

    # The commands should all be sent out.
    self.assertEqual(0, self.itpr.ec_cmd_queue.qsize())

    # The EC UART PTY should no longer be a member of the outputs since there's
    # nothing pending to be sent.
    self.assertFalse(self.itpr.ec_uart_pty in self.itpr.outputs)
