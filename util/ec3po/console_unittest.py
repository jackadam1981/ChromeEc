...
class OutputStream(object):
  ...
  def MoveCursorLeft(count):
    ...
    """
    string = binascii.unhexlify('%x' % console.ControlKey.ESC)
    string += '[' + str(count) + 'D'
    return string
...
class TestConsoleEditingMethods(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.console = console.Console(self.tempfile.fileno(),
                                   self.tempfile)
    # Mock out writes to the EC
    self.console.SendCmdToEC = mock.MagicMock()
  ...
  def StringToByteList(self, string):
    ...
    Returns:
      l: A list of integers representing the byte value of each character in the
        string.
    """
    l = []
    for c in string:
  ...
  def test_BackspaceOnEmptyLine(self):
    ...
    # Check the input position.
    self.assertEqual(0, self.console.input_buffer_pos,
                     'input buffer position is non-zero.\npos: ' +
                     str(self.console.input_buffer_pos))

    # Check that buffer is empty.
    self.assertEqual('', self.console.input_buffer,
