...
class TestConsoleCompatibility(unittest.TestCase):
  ...
  def test_ActAsPassThruInNonEnhancedMode(self, mock_check):
    ...
    # Mock the pipe and just verify that we're calling send() with what we
    # expect.
    with mock.patch.object(self.console, 'cmd_pipe') as mock_pipe:
      # Press enter.
      input_stream = []
      input_stream.append(console.ControlKey.CARRIAGE_RETURN)
      ...
      mock_pipe.send.assert_has_calls(expected_calls)
  ...
  def test_TransitionFromNonEnhancedToEnhanced(self, mock_check):
    ...
    # Mock the command pipe so we can inspect the send() calls.
    with mock.patch.object(self.console, 'cmd_pipe') as mock_pipe:
      expected_calls = []
