...
class Interpreter(object):
  ...
  def SendCmdToEC(self, packed_cmd):
  ...
  while True:
    ...
    for obj in readable:
      ...
      if obj is interp.ec_uart_pty:
        ...
        if interp.cmd_in_progress:
          ...
          data = os.read(obj.fileno(), 4)
          if '&E' not in data:
            # No error received.  Clear the command in progress.
            interp.cmd_in_progress = ''
            # Reset the retry count.
            interp.cmd_retries = COMMAND_RETRIES
            # Forward the data to the user.
            interp.dbg_pipe.send(data)
          else:
            if not interp.cmd_retries:
              ...
              interp.cmd_retries -= 1
              logging.warning('Retries remaining: %d', interp.cmd_retries)
              # Add the EC UART to the writers again.
              interp.outputs.append(interp.ec_uart_pty)
    ...
    for obj in writeable:
      ...
      if obj is interp.ec_uart_pty:
        ...
        if interp.cmd_retries < COMMAND_RETRIES:
          cmd = interp.cmd_in_progress
        else:
          # If we're not retrying, we should not be writing to the EC if we have
          # no items in our command queue.
          assert not interp.ec_cmd_queue.empty()
          # Get the command to send.
          cmd = interp.ec_cmd_queue.get()
