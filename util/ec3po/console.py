...
class Console(object):
  ...
  def HandleChar(self, byte):
    ...
    if self.receiving_oobm_cmd and byte != ControlKey.CARRIAGE_RETURN:
      self.pending_oobm_cmd += chr(byte).encode('utf-8')
      self.logger.debug('%s', chr(byte).encode('utf-8'))
      os.write(self.master_pty, chr(byte).encode('utf-8'))
      return
    ...
    if not self.enhanced_ec:
      ...
      self.cmd_pipe.send(chr(byte).encode('utf-8'))
    ...
    # Only print printable chars.
      ...
      if buffer_full:
        ...
        return
      # Print the character.
      os.write(fd, chr(byte).encode('utf-8'))
