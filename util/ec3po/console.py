...
def StartLoop(console):
  ...
  while True:
    # Check to see if pipes have any data to read.
    cmd_pipe_data_available = console.cmd_pipe.poll(PIPE_POLL_TIMEOUT)
    dbg_pipe_data_available = console.dbg_pipe.poll(PIPE_POLL_TIMEOUT)
    # Check to see if the console has any input.
    console_ready = select.select([console.master_pty], [], [], 0)[0]
...
def main():
  ...
  cmd_pipe_interactive, cmd_pipe_interpreter = multiprocessing.Pipe()
  # The debug pipe is unidirectional from interpreter to console only.
  dbg_pipe_interactive, dbg_pipe_interpreter = multiprocessing.Pipe(False)

  # Create an interpreter instance.
  itpr = interpreter.Interpreter(args.ec_uart_pty, cmd_pipe_interpreter,
                                 dbg_pipe_interpreter, log_level)
