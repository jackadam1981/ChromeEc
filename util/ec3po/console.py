...
class Console(object):
  ...
  def __str__(self):
    ...
    string.append('history: [\'' + '%s' % b'\', \''.join(self.history) + '\']')
  ...
  def HandleChar(self, byte):
    ...
    if self.receiving_oobm_cmd and byte != ControlKey.CARRIAGE_RETURN:
      self.pending_oobm_cmd += bytes([byte])
      self.logger.debug('%s', bytes([byte]))
      os.write(self.master_pty, bytes([byte]))
      return
    ...
    # Only print printable chars.
      ...
      print('[kerker] self.input_buffer = %s' % self.input_buffer)
