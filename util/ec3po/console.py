...
class Console(object):
  ...
  def HandleChar(self, byte):
    ...
    # Check to see if this is an out of band command.  These will start with a
    # colon which displays a different prompt.
    if byte == ord('#'):
    ...
    # Add chars to the peding OOBM command if we're currently receiving one.
    ...
    if byte == ControlKey.CARRIAGE_RETURN:
      if self.receiving_oobm_cmd:
        # Terimnate the command and send it to the OOBM queue.
