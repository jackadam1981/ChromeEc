...
def get_portage_deps(eclass_path=DEFAULT_ECLASS):
    ...
    Returns:
        ...
        Dict of architectures and their GCS bucket overrides
    """
...
def main(argv: Optional[List[str]] = None):
    ...
    toolchains, overrides = get_portage_deps(opts.eclass)

    for arch, (version, name) in toolchains.items():
        print(f"Toolchain: {arch} {version} {name}")

    for arch, bucket in overrides.items():
        print(f"Override: {arch} {bucket}")
