...
{
	//	static const char pcap_file[] = "/tmp/rts.pcap";
	FILE *fp;
...
int pdc_pcap_append(FILE *fp, struct timespec ts, const void *pl, size_t pl_sz)
{
	struct pcaprec_hdr_s pkt;

	pkt.ts_sec = ts.tv_sec;
	pkt.ts_usec = ts.tv_nsec / 1000;
	...
	printf("pl_sz %2zu: ", pl_sz);
	for (int i = 0; i < pl_sz; ++i)
		printf(" %02x", ((const uint8_t *)pl)[i]);
	printf("\n");
