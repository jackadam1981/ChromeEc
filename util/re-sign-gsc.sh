...
main () {
  ...
  case "${bin_size}" in
    (524288) rw_a_base=16384 # RO area size is fixed at 16K
             ...
             rw_key="util/signer/cr50-hsm-backed-node-locked-key.pem.pub"
             manifest="util/signer/ec_RW-manifest-dev.json"
             xml="util/signer/fuses.xml"
             codesigner_params+=(
               --b
               --pkcs11_engine="${PKCS11_MODULE_PATH}:0:${KMS_PROJECT_PATH}/cr50-hsm-backed-node-locked-key/cryptoKeyVersions/1"
             )
             flash_base=262144
             prefix="cr50"
