...
static int display_loop(void)
{
	...
	if (chargesplash_hostcmd(EC_CHARGESPLASH_INIT_DISPLAY, &response) < 0) {
	...
	do {
		if (get_charge_state(&charge_state) < 0) {
			fprintf(frecon_vt, "Failed to get charge state");
			continue;
		}
		...
		if (chargesplash_hostcmd(EC_CHARGESPLASH_GET_STATE, &response) <
		    0) {
			fprintf(frecon_vt, "Failed to get splash state");
			continue;
		}
...
int cmd_chargesplash(int argc, char **argv)
{
	...
	for (int i = 0; i < ARRAY_SIZE(actions); i++) {
		if (!strcasecmp(actions[i].name, argv[1])) {
			...
			printf("requested = %s\n",
			       resp.requested ? "true" : "false");
			printf("display_initialized = %s\n",
			       resp.display_initialized ? "true" : "false");
			printf("tries = %hhu\n", resp.tries);
			printf("max_tries = %hhu\n", resp.max_tries);
			return 0;
		}
	}
