...
class KconfigCheck:
    ...
    def scan_kconfigs(cls, srcdir, prefix=''):
        ...
        """
        kconf = kconfiglib.Kconfig(os.path.join(srcdir, 'Kconfig'), warn=False)
    ...
    def do_check(self, configs_file, srcdir, allowed_file, prefix):
        ...
        new_adhoc, unneeded_adhoc, updated_adhoc = self.check_adhoc_configs(
            configs_file, srcdir, allowed_file, prefix)
        if new_adhoc:
...
""" % '\n'.join(['CONFIG_%s' % name for name in new_adhoc]), file=sys.stderr)
        ...
        if unneeded_adhoc:
            ...
            print("""Congratulations! The The following options are now in Kconfig:
