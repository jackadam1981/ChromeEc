...
/*
 * see how lists and things like product and component can have multiple
 * or 0 entries. Not seeing it. Also instead of using a sull on structure,
 * try some offsets and arrays of these items. This will allow me to
 * display by looping instead of a print for every line of structure.
 */
...
typedef struct vif_field_struct {
	char *name;
	char *tag_value;
	char *str_value;
} vif_field_t;
...
typedef struct vif_cableSVIDModeList_struct {
	vif_field_t		vif_field[CableSVID_Mode_Indexes];
} vif_cableSVIDModeList_t;
...
typedef struct vif_cableSVIDList_struct {
	vif_field_t		vif_field[CableSVID_Indexes];

	vif_cableSVIDModeList_t CableSVIDModeList[MAX_NUM_CABLE_SVID_MODES];
} vif_cableSVIDList_t;
...
typedef struct vif_sopSVIDModeList_struct {
	vif_field_t		vif_field[SopSVID_Mode_Indexes];
} vif_sopSVIDModeList_t;
...
typedef struct vif_sopSVIDList_struct {
	vif_field_t		vif_field[SopSVID_Indexes];

	vif_sopSVIDModeList_t	SOPSVIDModeList[MAX_NUM_SOP_SVID_MODES];
} vif_sopSVIDList_t;
...
typedef struct vif_snkPdoList_struct {
	vif_field_t		vif_field[Snk_PDO_Indexes];
} vif_snkPdoList_t;
...
typedef struct vif_srcPdoList_struct {
	vif_field_t		vif_field[Src_PDO_Indexes];
} vif_srcPdoList_t;
...
typedef struct vif_PCIeEndpointListType_struct {
	vif_field_t		vif_field[PCIe_Endpoint_Indexes];
} vif_PCIeEndpointListType_t;
...
typedef struct vif_Usb4RouterListType_struct {
	vif_field_t		vif_field[USB4_Router_Indexes];

	vif_PCIeEndpointListType_t PCIeEndpointList[MAX_NUM_PCIE_ENDPOINTS];
} vif_Usb4RouterListType_t;
...
typedef struct vif_Component_struct {
	vif_field_t		vif_field[Component_Indexes];

	vif_srcPdoList_t	SrcPdoList[MAX_NUM_SRC_PDOS];
	vif_snkPdoList_t	SnkPdoList[MAX_NUM_SNK_PDOS];
	vif_sopSVIDList_t	SOPSVIDList[MAX_NUM_SOP_SVIDS];
	vif_cableSVIDList_t	CableSVIDList[MAX_NUM_CABLE_SVIDS];
...
typedef struct vif_Product_struct {
	vif_field_t		vif_field[Product_Indexes];

	vif_Usb4RouterListType_t USB4RouterList[MAX_NUM_USB4_ROUTERS];
} vif_Product_t;
...
typedef struct vif_struct {
	vif_field_t		vif_field[VIF_Indexes];

	vif_Product_t		Product;
	vif_Component_t		Component[MAX_NUM_COMPONENTS];
} vif_t;
