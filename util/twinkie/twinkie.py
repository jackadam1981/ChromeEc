...
class Twinkie:
  def __init__(self, consoleOnly=False, log=None):
  ...
  def tryClaimInterface(self, iface):
    ...
    try:
      ...
      self._log.Error("Failed to get Twinkie interface %d\n" % (iface))
      return False
    return True
  ...
  def sendCommand(self, cmd, waitAnswer=True):
    cmdStr = cmd+"\r\n"
    self._log.Debug("CMD:%s" % (cmdStr.strip()))
  ...
  def read2U16(self, idx):
    ...
    self._log.Debug("RD/%d = %s" %(idx,val))
    u32 = int(val,16)
