...
if [[ -n "${FLAGS_pty2}" ]]; then
	[[ -e "${FLAGS_pty2}" ]] || die "Device '${FLAGS_pty2}' does not exist."
	PTY2=${FLAGS_pty2}
	info "\tpty2\t= ${PTY2}"
	UART_COUNT=$(( UART_COUNT+1 ))
fi
...
[[ -n "${PTY1}" ]] && SAMPLE_TXT["${PTY1}"]="${DIR_TMP}/sample.${PTY1##*/}.cap"
[[ -n "${PTY2}" ]] && SAMPLE_TXT["${PTY2}"]="${DIR_TMP}/sample.${PTY2##*/}.cap"
[[ -n "${PTY3}" ]] && SAMPLE_TXT["${PTY3}"]="${DIR_TMP}/sample.${PTY3##*/}.cap"
...
stress_test() {
	...
	FILE_RES1="${DIR_TMP}/res_${DNAME1}_w_${DNAME2}_${DNAME3}.${ITER}.cap"
	FILE_RES2="${DIR_TMP}/res_${DNAME2}_w_${DNAME1}_${DNAME3}.${ITER}.cap"
	FILE_RES3="${DIR_TMP}/res_${DNAME3}_w_${DNAME1}_${DNAME2}.${ITER}.cap"
