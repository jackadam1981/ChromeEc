...
if dut-control -i &>/dev/null ; then
	...
	for (( i=0; ; i++ )) do
		...
		DEVINFO=$(udevadm info -q property -n "${DEVFILE}")
		if ! echo ${DEVINFO} | grep -iq "ID_VENDOR_ID=18d1" ; then
			continue
		fi
		if ! echo ${DEVINFO} | grep -iq "ID_MODEL_ID=5014" ; then
			continue
		fi
...
if [[ "${FLAGS_ec}" -eq "${FLAGS_TRUE}" ]] ; then
	PTY_EC="$(get_uart_pty "EC")" || \
		die "line ${LINENO}: EC UART device is not detected"
	info "  EC PTY= ${PTY_EC}"
fi
if [[ "${FLAGS_cpu}" -eq "${FLAGS_TRUE}" ]] ; then
	PTY_CPU="$(get_uart_pty "CPU")" || \
		die "line ${LINENO}: CPU UART device is not detected"
