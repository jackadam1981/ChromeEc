...
declare -A expected_diffs=(
  # b/389761200 4919881523df2ef5dae9cacebdeb3e72936ec793 Revert "drivers: watchdog: stm32 iwdg: explicit single channel"
  # b/460502081 69c7fccd5e707b3f016d0c4efd536791371ecb15 FROMPULL: kernel: Add Kconfig option to disable LTO for kernel sources
  # b/460504453 1797ed0688a1b226d9ea861bbb3675c1bd7bb650 FROMPULL: soc: it8xxx2: Select KERNEL_NO_LTO only when LTO is enabled
  ['zephyr/main']='4919881523df2ef5dae9cacebdeb3e72936ec793 69c7fccd5e707b3f016d0c4efd536791371ecb15 1797ed0688a1b226d9ea861bbb3675c1bd7bb650'
)
...
for repo in "${!repos[@]}"; do
  ...
  rm -rf /tmp/diff_upstream
  mkdir /tmp/diff_upstream
  git clone --quiet --no-checkout "${upstream_repo}" /tmp/diff_upstream
	git -C /tmp/diff_upstream checkout --quiet "${upstream_commit}"
  ...
  for frompull_commit in "${expected_commits[@]}" ; do
    ...
    git format-patch -1 "${frompull_commit}" --stdout --relative | git -C /tmp/diff_upstream am --3way || die "Failed to apply known diff"
  done

  log_output=$(git -C /tmp/diff_upstream --no-pager log --no-decorate --format='%h %s %cr' \
    "${upstream_commit}..origin/${upstream_branch}") \
    || die "git log failed"
  if [ "${log_output}" != "" ]; then
  ...
  ( git ls-files . ; git -C /tmp/diff_upstream/ ls-files ) | sort -u | while IFS= read -r  file; do
    # Per repo exceptions
    case "${repo}" in
      pigweed)
        case "${file}" in
          cloudbuild_pigweed.yaml|OWNERS)
            continue
            ;;
        esac
        ;;
      android/platform/system/chre)
        case "${file}" in
          OWNERS)
            continue
            ;;
        ...
        ;;
      android/platform/system/chre_internal)
        case "${file}" in
          chre_api/OWNERS|OWNERS)
            continue
            ;;
        esac
        ;;
    ...
    case "${file}" in
      .gitkeep)
        continue
        ;;
      DIR_METADATA|OWNERS|PRESUBMIT.cfg|*/OWNERS|.vpython3) # Skip these files if they are only downstream
        if ! [ -f "/tmp/diff_upstream/${file}" ] ; then
          continue
        fi
        ;;
    esac
    if ! diff_output=$(diff --no-dereference -u  -L "Upstream/${file}" -L "Downstream/${file}" "/tmp/diff_upstream/${file}" "${file}" 2>&1 ); then
