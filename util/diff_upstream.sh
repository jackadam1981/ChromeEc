...
declare -A expected_diffs=(
  ...
  ['zephyr/main']='4919881523df2ef5dae9cacebdeb3e72936ec793 \
...
for repo in "${!repos[@]}"; do
  ...
  for frompull_commit in "${expected_commits[@]}" ; do
    ...
    git format-patch -1 "${frompull_commit}" --stdout --relative | \
      git -C /tmp/diff_upstream am --3way || die "Failed to apply known diff"
  done

  log_output=$(git -C /tmp/diff_upstream --no-pager log --no-decorate \
  ...
  ( git ls-files . ; git -C /tmp/diff_upstream/ ls-files ) | sort -u | \
  while IFS= read -r  file; do
    ...
    case "${file}" in
      .gitkeep)
        continue
        ;;
      DIR_METADATA|OWNERS|PRESUBMIT.cfg|*/OWNERS|.vpython3)
    ...
    if ! diff_output=$(diff --no-dereference -u  -L "Upstream/${file}" \
      -L "Downstream/${file}" "/tmp/diff_upstream/${file}" "${file}" \
      2>&1 ); then
