...
Control of hardware write protect is restricted by CCD capabilities. On Cr50 it's
controlled by the `OverrideWP` capability. On [Ti50][hw-wp-ti50] it's controlled by the
`OverrideWP` capability and the `AllowUnverifiedRo` capability.
...
[disable hardware write protect][hw-wp]
