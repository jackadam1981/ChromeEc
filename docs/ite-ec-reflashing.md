...
implementing the new `i2c-pseudo` driver?  There are a couple reasons.

By staying with the existing `servod` Servo I2C implementations, we have not
introduced any dependency on new kernel modules for *existing* `servod`
functionality. Only this new functionality depends on `i2c-pseudo`. As with
`i2c-pseudo` we would need to rely on out-of-tree kernel module distribution
for these new I2C modules until eventual upstream acceptance + percolation down
to distribution Linux kernels, with no guarantee of acceptance. Doing that just
for this one new function of ITE EC reflashing is one thing. Requiring
out-of-tree modules for all `servod` use would be quite another. Realistically
we would probably instead maintain fallback code in `servod` to use its
existing internal Servo I2C interface implementations when the kernel ones
aren't available, but that has a maintenance cost too.  These same issues would
be faced with every new generation of Servo, so this broad Servo + `servod`
architectural change is not something to be considered lightly or just for
ITE EC reflashing.
