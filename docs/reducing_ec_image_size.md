...
There are two options available that reduce the size of strings stored with the `ASSERT()` calls.
...
with many other non-critical features. Refer to the shell subsystem [Kconfig][1]
source file for the complete list of shell features than can be configured.

[1]:https://github.com/zephyrproject-rtos/zephyr/blob/main/subsys/shell/Kconfig
