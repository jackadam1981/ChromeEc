...
*   Minimize review latency for partner CLs.
*   Minimize duplication of effort among Googlers and partners.

## CL author guidelines for all code reviews

* Keep each CL to one logical change.
  * Where there are several layers of dependencies, breaking the patch up into
   multiple CLs
 * Avoid significant scope increases in subsequent patchsets; when
	reviewers ask for significant additions, consider adding them in a
	follow-up CL instead of uploading a new patchset
 * Refactor in a separate CL from functional changes; this also applies to
	unrelated style and typo fixes
 * See Google small CL
	[guidelines](https://google.github.io/eng-practices/review/developer/small-cls.html)
	for more tips
 * If you are about to send out more than 400 lines of non-trivial changes,
	and you haven’t written a design doc, consider whether you need one and
 ...
 * Use `repo upload --no-email` to avoid sending emails for CLs not ready
	for review
 * Use comments as needed to clarify that a CL is now ready for review
 * Recommended: Leave “WIP” in the CL title until it is ready to review
 * Optional: Mark the CL as “Work in Progress” in Gerrit (or use `repo
	upload --wip`) until it is ready to review
 ...
 * If multiple domain experts are required, indicate in the review request
	...
	scope
 * If someone needs to be aware that a change is happening, add them as CC;
...
* Consider multi-stage review: Get LGTM within a certain scope before moving
    to the next reviewer
 * May help reduce the number of patchsets and duplicated review effort
 * Probably most helpful when multiple reviewers should look at the same
	files
 ...
 * If not, consider using
	[topics](https://gerrit-review.googlesource.com/Documentation/intro-user.html#topics)
	in Gerrit to group related CLs
* Try to make visible updates actionable for reviewers
 * Don’t post a comment saying that you’ve done something before the
	patchset that does it
 * Don’t post comments that say you’ll do something in the next patchset;
	just wait for that patchset
 * If practical, don’t upload a new patchset until you can usefully respond
	to all unresolved comments
 * Use `repo upload --no-email` for patchsets that address comments before
 ...
 * Typically, a reviewer should not CR+2 a CL until the reviewer’s comments
	have been resolved to their satisfaction. To reduce latency, a reviewer
	may CR+2 a CL with the stated assumption that the author will address
...
* Complete partner-internal code review (using Gerrit) before assigning to
    Google reviewers
* Send CLs to the EC GWSQ for review by default
 * Or send to specific Googlers with domain expertise, consistent with
	above guidelines

## Guidelines for extended partner efforts

* Discuss at beginning of effort which Googlers should review or be CCed on
    CLs
...
* [Coreboot Gerrit
    Guidelines](https://doc.coreboot.org/getting_started/gerrit_guidelines.html)
