...
*   Keep each CL to one logical change
    *   Where there are several layers of dependencies, breaking the patch up into multiple CLs
    ...
    *   See Google small CL [guidelines](https://google.github.io/eng-practices/review/developer/small-cls.html) for more tips
...
*   Indicate clearly when a CL is not yet ready for review and when it is; don’t start the review until you’ve stopped making changes
    *   Use `repo upload --no-email` to avoid sending emails for CLs not ready for review
    ...
    *   Optional: Mark the CL as “Work in Progress” in Gerrit (or use `repo upload --wip`) until it is ready to review
...
*   Consider multi-stage review: Get LGTM within a certain scope before moving to the next reviewer
    ...
    *   Probably most helpful when multiple reviewers should look at the same files
...
*   Keep related changes in a single relation chain if practical
    *   If not, consider using [topics](https://www.google.com/url?q=https://gerrit-review.googlesource.com/Documentation/intro-user.html%23topics&sa=D&ust=1581536219866000&usg=AFQjCNGldJSMqhY9rd3aMHf_hK_XZvDxMQ) in Gerrit to group related CLs
*   Try to make visible updates actionable for reviewers
    ...
    *   Use `repo upload --no-email` for patchsets that address comments before the responses to those comments
...
*   Discuss at beginning of effort which Googlers should review or be CCed on CLs
    *   Optionally create a GWSQ specific to the effort

## Reference

*   [Coreboot Gerrit Guidelines](https://doc.coreboot.org/getting_started/gerrit_guidelines.html)
