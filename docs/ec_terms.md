...
*   **UART - Universal Asynchronous Receiver Transceiver**<a name="uart"></a>
    ...
    Also known as a serial port.  An asynchronous communication channel with a
    dedicated receive pin, transmit pin, and ground. Optional hardware flow
    control signals require additional connections between the   Standard
    transmission rates are slow (up to 115200 bits per second).  Typical use is
    to provide a debug console to the EC. [RS-232] is the protocol standard used
    by UARTs.
