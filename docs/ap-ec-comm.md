...
Command racing can be done on the EC console (`hc debug` commnad) and in
the kernel:
```
...
*   User space/Deepcharge use `ec_commands.h` installed by
...
`/build/kernel/include/linux/mfd/cros_ec_commands.h`. (It is far from
perfect, the kernel include file has not be update with clang as
`ec_commands.h` has been).
