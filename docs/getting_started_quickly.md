...
While it is possible to build an EC image outside the Chromium OS chroot, some
boards will fail due to dependencies on external binaries. Also, development
tools such as `flash_ec` and `servod` must be run from the Chromium OS chroot so
it's highly recommended that you create one in your development environment.

## Building

Here is a set of steps to setup a development environment to build EC images
