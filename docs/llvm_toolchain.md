...
Ultimately, `cros_setup_toolchains` is just invoking `crossdev` with a series of
flags. You can see the exact `crossdev` command invoked in the output:
...
[CLs required to enable the RISC-V baremetal toolchain][riscv cls].
...
*   `binutils`
    ...
    *   [`binutils` ebuild]
*   `compiler-rt`
    *   [Compiler Runtime libraries] for LLVM and `clang`.
    *   [`compiler-rt` ebuild]
*   `gcc`
    *   `gcc` should not be needed for an LLVM toolchain, but [`crossdev`]
        assumes that you need a minimal [`stage1` `gcc`] and builds it anyway.
    *   [`gcc` ebuild]
*   `gdb`
...
*   `newlib`
    *   The [`newlib` C standard library] is an implementation of the C standard
        library targeting embedded systems.
    *   [`newlib` ebuild]
...
*   Enable the target in LLVM, if not already enabled:
    https://crrev.com/c/6300619.
*   Create the [toolchain overlay] for the target triple:
    https://crrev.com/c/6349977.
*   Add the target triple in a `toolchain.conf` file in an existing overlay:
    https://crrev.com/c/6300631.
*   Enable the LLVM packages in `cros_setup_toolchains`:
    https://crrev.com/c/6302748.
*   Add the target triple to `is_baremetal_abi`: https://crrev.com/c/6300601.
...
[cros_setup_toolchains compiler-rt]: https://source.chromium.org/chromiumos/chromiumos/codesearch/+/main:chromite/scripts/cros_setup_toolchains.py;l=237-238;drc=0d2c24efbee71592f3dcf4b859b9a51175137762
[target triple]: https://clang.llvm.org/docs/CrossCompilation.html#target-triple
[`chromiumos/overlays/toolchains/cross-riscv32-elf`]: https://source.chromium.org/chromiumos/chromiumos/codesearch/+/main:src/third_party/toolchains-overlay/
