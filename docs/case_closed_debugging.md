...
### Udev rules file
There is a udev rules file, `extra/usb_serial/50-google-serial.rules` that
...
3) Console interfaces appear and then quickly disappear
	...
	that is not compatible with case closed debugging.  This shouldn't happen
	as the Udev rules file marks case closed debugging devices with
	`ID_MM_DEVICE_IGNORE`.  As a last resort ModemManager can be purged from your
	workstation.
