# FPMCU Partner Development

[TOC]

## Hardware Required

*   ![Servo v2](../images/servo_v2.png)
*   Yoshi flex cable
    *   ![Regular Yoshi Flex]
...
*   Connect Sensor to DragonClaw
*   [Connect Yoshi Flex to DragonClaw] ([Another image])
...
```bash
sudo screen $(dut-control raw_fpmcu_uart_pty | cut -d: -f2)
```
...
From there you can see commands with "help":
...
If you’re not familiar with Gerrit (code review) and `repo`, here are some docs
...
The Chromebook you receive from Google should already have [developer mode]
enabled and [hardware write protection] disabled, so you should be able to SSH
into the device and have full access to flashing the FPMCU firmware.
...
```bash
(dut) $ ectool --name=cros_fp fpmode capture test_reset; ectool --name=cros_fp waitevent 5 500; ectool --name=cros_fp fpframe > /tmp/test_reset.pnm
```
...
### ectool over servo SPI

There is code that should
[allow ectool to work over servo’s SPI interface][ectool_servo_spi], though when
I did a quick test it did not work.

#### Build `ectool_servo`

*   Apply the following patch to make sure that `ectool_servo` is compiled:

    ```
    diff --git a/util/build.mk b/util/build.mk
    index 4fb6b7e1f..fab4764eb 100644
    --- a/util/build.mk
    +++ b/util/build.mk
    ...
    +ifeq ($(BOARD),bloonchipper)
    build-util-bin+=ectool_servo
    endif
    ```

*   Compile `ectool_servo`
...
```bash
(chroot)$  sudo ./build/bloonchipper/util/ectool_servo --name=YOUR_SERVO_ID version
```
...
[Regular Yoshi Flex]: ../images/yoshi_flex.png
[Yoshi Flex Reworked to Support SWD]: ../images/yoshi_flex_swd_rework.png
[DragonClaw board]: ../images/dragonclaw.png
[Connect USB to Servo]: ../images/servo_v2_with_micro_usb.png
[Connect Yoshi Flex]: ../images/servo_v2_with_yoshi_flex.png
[Another Yoshi Flex image]: ../images/servo_v2_with_yoshi_flex2.png
[Connect Yoshi Flex to DragonClaw]: ../images/dragonclaw_yoshi_flex_header.png
[Another image]: ../images/dragonclaw_yoshi_flex_header2.png
[Connect SWD Debugger]: ../images/servo_v2_jtag_header.png
