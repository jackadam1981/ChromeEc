...
fingerprint sensor or not. For each SKU, there is an associated [fingerprint
config]. [Chrome OS Config] determines the
[SKU information](and thus the [fingerprint config]) from [CBI Info]. The SKU
for a given device can be found by viewing
`chrome://system/#platform_identity_sku`.
...
\[fingerprint config]: [Chrome OS Config Fingerprint]
