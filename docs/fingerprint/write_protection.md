...
Provides an overview of how FPMCU firmware write protection works. This is a
somewhat tricky topic since there's a lot of historical baggage in the existing
documentation (as well as many different terms with essentially the same
meaning). I'll do my best to decode it here, but please edit or open a bug if
...
probably refers to "the" EC (i.e. the first one that existed). All Chrome OS
...
running on several different MCUs on the device with various tweaks (e.g. the
FPMCU, the touchpad one that can do palm rejection, etc.). It's quite confusing,
so try to use FPMCU to distinguish the fingerprint MCU from "the EC".

## Hardware Write Protect

The Fingerprint MCU for Chrome OS only exists on nocturne devices and newer. On
these modern devices, the Cr50 (aka H1 / Haven / GSC / TPM) provides a "hardware
...
(["CCD open"](./write_protection.md)) and corresponds to
...
a mass erase of the chip, due to RDP1 (see below). (I haven’t fully traced the
code path. It might be because of the check where
[`PROTECT_RO_NOW` is set to in `unwanted_prot_flags`](https://chromium.googlesource.com/chromiumos/platform/ec/+/a85c958633de765e529f2fb539fdc130dd99a6a2/chip/stm32/flash-stm32h7.c#487)
and the [chip does a hard reboot, which comments say clears the flash protection
registers](https://chromium.googlesource.com/chromiumos/platform/ec/+/a85c958633de765e529f2fb539fdc130dd99a6a2/chip/stm32/flash-stm32h7.c#504).)
