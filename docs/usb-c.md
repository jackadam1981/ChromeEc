...
*   Reversed cable mode. This requires a mux to switch the signals before
    getting to SoC.
*   Debug accessory mode, e.g. (Case Closed Debugging
    (CCD))[./case_closed_debugging_cr50.md]
...
*   TCPC
    *   Type-C Port Controller. Typically a separate IC connected through I2C,
        ...
        acted as the TCPM. More info in the official (TCPC
        spec)[USB TCPM Spec Id].
*   TCPM
...
*   Saving PD state between EC jumps
    ...
    *   We save and restore PD state between RO -> RW and RW -> RO jump to allow
        ...
        restore and existing SNK contract (Chromebook as SNK) without cutting
...
full descriptions see [config.h](../include/config.h)
