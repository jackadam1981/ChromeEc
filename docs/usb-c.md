...
*   Multiple uses for the 4 differential pairs high-speed signals including
...
*   PPC
    *   Power Path Controller. An optional, separate IC that isolates various
        ...
        prevent shorts and over current/voltage scenarios.
*   SSMUX
...
*   `CONFIG_USB_SM_FRAMEWORK`
*   `CONFIG_USB_TYPEC_SM`
*   `CONFIG_USB_PRL_SM`
*   And one of `CONFIG_USB_PE_*` policy engine options

Most USB-C options will start with `CONFIG_USB_PD_` or `CONFIG_USBC_`.

Below is a non-exhaustive list of commonly use config options. For their full
descriptions see [config.h](../include/config.h)

*   `CONFIG_USB_POWER_DELIVERY`
    *   Device will speak USB-C PD for power. Includes the USB-C state machine
        (new or old implementation). Should be defined.
