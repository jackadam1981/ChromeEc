...
states of power delivery, the operating system and what it is connected to
...
This allows for setting power characteristics to default USB2 (600mA) default
...
modes (Example: DisplayPort) and send a class of messages called Structuted Vendor
Defined Message (SVDMs), which are not related to power delivery. The additional
flexablity and functionality in USBC requires support from the OS.

From the operating system, USB-C PD requires a complex state machine as USB-C PD can
