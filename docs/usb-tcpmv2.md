...
decisions and improve DPM code organization is ongoing; see the [DPM refactor
hotlist](https://issutracker.google.com/hotlists/3279746).
...
*   If the cable supports TBT3, the TCPM sends an SOP' VDM _Enter Mode_ REQ with
    the Intel VID.
*   If the cable is also an active cable, the TCPM sends an SOP'' VDM _Enter
    Mode_ REQ. (The TBT3 mode VDO may identify the cable as active for this
    purpose when the ID Header VDO does not.)
