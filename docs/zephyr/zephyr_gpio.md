...
automatically initialized and can be referred to using the specificied `enum-name`.


Named GPIO properties: Property | Description | Settings
:------------------------------ | :---------- | :-------
...
`enum-name` | The enum used to refer to the GPIO in the code. | `GPIO\_<NAME>`

The list of valid `enum-name` values is defined in [named-gpio.yaml].
...
### Shell Command: `gpioget` and `gpioset`
...
GPIO parameter summary: Parameter | Description
:-------- | :----------
...
[GPIO Init Priority]: https://source.chromium.org/chromiumos/chromiumos/codesearch/+/main:src/platform/ec/zephyr/Kconfig?q=%22config%20PLATFORM_EC_GPIO_INIT_PRIORITY%22&ss=chromiumos
[named-gpio.yaml]: ../../zephyr/dts/bindings/gpio/named-gpios.yaml
