...
`gpios` | GPIO phandle, identifies the port, pin number and flags. | `<&gpioX Y flags>`
...
In the GPIO declaration it is conventional use the lowercase net name used in
the schematic as the *node name*, and the same prefixed with `gpio_` as *node
label*. For example:
...
Unused GPIOs should be listed separately in an `unused-gpio` node, so that they
get initialized correctly for the specific platform.
...
Projects that use GPIO interrupts have to specifiy the interrupt to routine
mappings in a board specific file. This normally happens in the [gpio_map.h] in
...
The GPIO can then be referred from in the code with the `enum-name` directly, or through other DTS nodes by using the node label, for example for `&gpio_ec_wp_l`:
...
The `named-gpios` node and subnodes are normally declared in a separate [gpio.dts] file, which is added as an overlay in the [BUILD.py] file for the project.
...
[dt-bindings/gpio/gpio.h]: https://source.chromium.org/chromiumos/chromiumos/codesearch/+/main:src/third_party/zephyr/main/include/dt-bindings/gpio/gpio.h
[dt-bindings/gpio_defines.h]: ../../zephyr/include/dt-bindings/gpio_defines.h
[include/drivers/gpio.h]: //source.chromium.org/chromiumos/chromiumos/codesearch/+/main:src/third_party/zephyr/main/include/drivers/gpio.h
