...
The current Zephyr EC build only the CMSIS module from the upstream Zephyr
...
* `./common` - Upper-level code shared across boards and the EC implementations.
  This incudes the following features:
  * Battery charging
  * USB Power Delivery
...
* `./zephyr/inlude` - Header files for files found in the `zephyr`
  sub-directory.
