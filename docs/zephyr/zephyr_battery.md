...
### Initially Enable Batteries on a Board
...
#### Add initial battery.dts

Create `ec/zephyr/projects/{project}/{board}/battery.dts` with a batteries node
like:
...
This new `battery.dts` file will have to be added to the dts_overlay list in
`ec/zephyr/projects/{project}/{board}/BUILD.py`.

#### Add Battery I2C Signals

<!-- TODO: Ask Keith if we should document these as gpios or not based on their comment -->
<!-- on enabling batteries in brya -->

Add GPIO battery nodes to `gpio.dts`

[Example CL enabling batteries on a board]:
(https://chromium-review.googlesource.com/c/chromiumos/platform/ec/+/3384482/7)

### Enable a Battery on a Board

Append the battery node to the batteries node in
  `ec/zephyr/projects/{project}/{board}/battery.dts`
...
One can check for battery information with the `battery` EC console command.
