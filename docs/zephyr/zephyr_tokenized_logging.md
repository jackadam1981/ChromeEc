...
EC logs should be transparent to the developer in most scenarios, detokenizing
occurs before the log is outputted to console or cros_ec.log.  Servod maintains
the database internally and the DUT is prepopulated with the token database.
...
There are three types of databases available when using tokenization in EC.
 1. <b>Board Specific</b> - This database is only compatible with the board its built against.
 2. <b>Unified</b> - This database is a merged database of all board specific databases of a build,
 this database is compatible with all boards in the current build.
 3. <b>Historical</b> - This database contains a history of all tokens.  This will work
 with all boards built at any time.  Ideally you'll only need to use this database
 with released images.  See Historical Database section for more details.

## Building EC with Tokenization

After enabling tokenization, build your board as [normal](zephyr_build.md).
The databases can be found in the following locations based on compilation method.

* <b>ZMake</b>
  * <b>Board Specific</b>: `build/zephyr/${BOARD}/output/database.bin`
  * <b>Unified</b> `build/tokens.bin`

* <b>Portage/Ebuild</b>
  * <b>Board Specific</b>: `"${root_build_dir}/${project}"/output/database.bin`
  * <b>Unified</b>: `"${root_build_dir}/tokens.bin`
...
argument.  This uses `scp` to copy the token database over.  Please enter DUT's
...
<i>For prebuilt images, the database is preloaded on DUT and servod docker images.
Viewing logs should not require any additional steps!</i>
...
* Over console via [servod](https://chromium.googlesource.com/chromiumos/third_party/hdctools)

The token database needs to be referenced by each of these applications to
properly detokenize and read the logs.
...
The first path can be updated via secure copy
```
scp tokens.bin root@${DUT_IP}:/usr/local/cros_ec/tokens.bin
```

The second path is on a read-only partitiions. `cros deploy` can be used to
manually update this path.
```
...
The docker image supports fetching the historical database from gcs upon start-up
when provided the `--fetch-token-db` argument. This database will automatically
be used to detokenize logs.
```
