...
`build/zephyr/<board>/output/database.bin` zmake will also merge all
database.bin's found under `build/zephyr` into a single database which can be
found at `build/tokens.bin`. This `tokens.bin` can be used to detokenize logs
for any board compiled with tokenizatinon enabled.
...
Location 3 can be updated via secure copy (Reorder timberslide to prefer
writable partition -- TODO(b/315363538)
```
...
```
start-servod --channel=release --mount=<your_path>/ec/build:/tmp/firmware_to_flash -n flashing_servod
```
...
## Historical Database Management -- TODO(b/287268093)
