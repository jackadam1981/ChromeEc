...
database.bin's found under `build/zephyr` into a single unified database which
can be found at `build/tokens.bin`. This `tokens.bin` can be used to detokenize
logs for any board compiled with tokenizatinon enabled.
