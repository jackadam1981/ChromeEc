...
Database management is handled in `recipes/build_firmware.py`.  The
`firmware-zephyr-informational` builder will update the database on a daily basis.
Race conditions are handled using [request-preconditions](https://cloud.google.com/storage/docs/request-preconditions).
