...
Zephyr EC leverages [Pigweeds Tokenizer](https://pigweed.dev/pw_tokenizer/) module
to accomplish tokenizing and detokenizing of logs.

## Enabling Tokenization in EC

Enable Kconfig `PLATFORM_EC_LOG_TOKENIZED` and its dependencies for the board
...
The fist two locations are read-only partitiions. To manually update the first
...
Location 3 can be updated via secure copy (TODO: reorder timberslide to prefer
writable partition)
```
...
above the base64 message is encapsulated with a prefix of `` ` `` (backtick) and suffix `~`.
