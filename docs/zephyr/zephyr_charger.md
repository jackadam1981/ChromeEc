...
&i2c_0 {
	...
	vendor_part: vendor_part@{SOME_REG_VALUE} {
...
in [charger bindings directory].

See the [I2C doc](./zephyr_i2c.md) for more information on configuring the I2C
node.

#### Add corresponding child node to `named-i2c-port`
...
named-i2c-ports {
   	...
   	charger {
   		i2c-port = {PHANDLE};
		/* This could be any name, but must match board-specific-code */
   		enum-name = "I2C_PORT_CHARGER";
   	};
};
```

See the [I2C doc](./zephyr_i2c.md) for more information on configuring the
`named-i2c-port` node.
...
### Define or append to charger\_config\_t global array
...
<!-- TODO(b/226925642) - charger chips should be defined in code via DT
macros. -->

## Threads

Enabling `CONFIG_PLATFORM_EC_CHARGER` also enables the charger thread defined in [Kconfig.tasks].
