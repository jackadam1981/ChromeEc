...
The [pwr_avg] [EC console command] may be invoked to see battery charging rates.
...
The `chgoverride` [EC console command] may be invoked to disable or force
charging from a specific charging enabled port.
See [Kconfig.charger] for configuring multiple charging ports.

Example output of `uart:~$ chgoverride 1`:

```
[14757.947279 Charge Override: 1] # EC LOG
```
...
The `chglim` [EC console command] may be invoked to set a max charger IC current
and voltage charging limit.

#### chgsup

The `chgsup` [EC console command] may be invoked to get the status of the port
that is the chosen charge supplier.
...
The `chargecurrentlimit` [ectool] command may be invoked to set the charge limit
in milliamps.
...
The `chargecontrol` [ectool] command may be invoked to set whether the board is
idle, discharging, or resume normal operation.
...
The `chargeoverride` [ectool] command may be invoked to disable or force
charging from a specific charging enabled port.  See [Kconfig.charger] for
configuring multiple charging ports.
...
The [chargestate](./zephyr_battery.md#chargestate) command may also be invoked.
