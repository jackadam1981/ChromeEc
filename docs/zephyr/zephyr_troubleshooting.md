...
The device tree is made out of different dts and dtsi files, gets aggregated
into a single `zephyr.dts` file and generates a `devicetree-unfixed.h` file
...
```
build/zephyr/$BOARD/build-ro/zephyr/zephyr.dts
```
...
```
./build/zephyr/$BOARD/build-rw/zephyr/include/generated/devicetree_unfixed.h
```

Header file with all `struct device` pointer (useful for referencing missing
`dts_ord_...`)

```
./build/zephyr/$BOARD/build-ro/zephyr/include/generated/device_extern.h
```

https://docs.zephyrproject.org/latest/build/cmake/index.html?highlight=gen_defines%20py#configuration-phase
...
Very common error, happens when some code refer to a device using
`DT_DEVICE_GET`, but the corresponding `struct device` is not instantiated,
either because the driver has not been enabled or because of a devicetree
misconfiguration (missing `status = "okay"`).
...
Kconfig `config ..._LOG_LEVEL`) definition, they are generated using the
...
Proper fix: make the driver depends on the logging sybsystem being enabled
(`depends on LOG`).

## LTO

Many compiler and linker error are quite informative if LTO is enabled, for
...
adding `CONFIG_LTO=n` to the corresponding `prj.conf` usually results in better
...
```
zmake build $BOARD --extra-cflags=-save-temps=obj
```

This leaves a bunch of `.i` files in the build/ directory.

For more information: https://docs.zephyrproject.org/latest/build/dts/troubleshooting.html?highlight=save%20temps#look-at-the-preprocessor-output
...
```
$ arm-none-eabi-gdb build/zephyr/brya/build-ro/zephyr/zephyr.elf
(gdb) p fan_config
