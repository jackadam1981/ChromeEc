...
Battery Charging Specification 1.2 describes mechanisms which allows to draw
current in excess of the USB 2.0 specification. In Zephyr EC, BC1.2 referes to
devices that are an USB Charger Detector i.e. allows to detect when a BC1.2
complaiant charger is connected.
...
A BC1.2 device node should be child of an USBC port node with "named-usbc-port"
compatible. The USBC port node should have only one BC1.2 device node.

### Richtek RT1739

There are two nodes describing Richtek RT1739, one for BC1.2
[richtek,rt1739-bc12.yaml] and one for PPC [richtek,rt1739-ppc.yaml]. Node for
PCC contains information about I2C bus and address.

### Richtek RT9490

Richtek RT9490 is described by [richtek,rt9490-bc12.yaml]. It allows to define
GPIO which can trigger interrupt from the RT9490.

### Pericom PI3USB9201

Pericom PI3USB9201 is described by [pericom,pi3usb9201.yaml]. It allows to
define GPIO which can trigger interrupt from the PI3USB9201. DTS node contains
information about I2C bus and address.
...
Some boards may implement [board_vbus_sink_enable()] function.

## Threads

When enabled, the `usb_charger_task` will be created for each USBC port.
...
Example of defining BC1.2 chip in DTS:
...
The `bc12` is BC1.2 device node ("pericom,pi3usb9201" is a compatible that is
