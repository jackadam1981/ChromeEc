...
`CONFIG_PLATFORM_EC_ALS`                         | y (if `CONFIG_HAS_TASK_ALS=y`) | [ALS]
...
The following options are available only when `CONFIG_PLATFORM_EC_ALS=y`.

Kconfig sub-option                     | Default | Documentation
:------------------------------------- | :-----: | :------------
`CONFIG_PLATFORM_EC_ALS_COUNT`         | 1       | [ALS COUNT]
...
[TIGHT TIMESTAMPS]: https://source.chromium.org/chromiumos/chromiumos/codesearch/+/main:src/platform/ec/zephyr/Kconfig.motionsense;?q="config%20PLATFORM_EC_SENSOR_TIMESTAMPS"&ss=chromiumos
