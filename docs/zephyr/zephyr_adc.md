...
Either Nuvoton NPCX and ITE IT8xxx2 ECs use single devicetree node `adc0` to
describe ADC. To enable the ADC set the `status` property to `"okay"`:
```
&adc0 {
	status = "okay";
};
...
`CONFIG_PLATFORM_EC_ADC_CMD` respectevly.
...
to ADC channels with following values:
