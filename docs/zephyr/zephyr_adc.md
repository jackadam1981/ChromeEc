...
:--------------------------------- | :------------ | :------------
`CONFIG_PLATFORM_EC_ADC`           | Enabled       | [zephyr/Kconfig](../zephyr/Kconfig.adc)

The following options are available only when `CONFIG_PLATFORM_EC_ADC=y`.

Kconfig sub-option                               | Default | Documentation
:----------------------------------------------- | :-----: | :------------
`CONFIG_ADC_SHELL`                               | y       | [CONFIG_ADC_SHELL]
`CONFIG_PLATFORM_EC_ADC_CMD`                     | n       | [ADC cmd]
`CONFIG_PLATFORM_EC_ADC_RESOLUTION`              | n       | [ADC resolution]
`CONFIG_PLATFORM_EC_ADC_OVERSAMPLING`            | n       | [ADC oversampling]
