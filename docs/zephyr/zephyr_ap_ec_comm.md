...
The following options are exclusive to each other as they select the protocol
that will be used to transport the messages.
...
device tree. See `Device Tree nodes` paragraph for details.
...
`CONFIG_HAS_TASK_HOSTCMD` which creates the thread.
...
[CrOS EC documentation]

## Examples

[Trogdor MKBP wake-up](https://source.chromium.org/chromiumos/chromiumos/codesearch/+/main:src/platform/ec/zephyr/boards/arm/trogdor/trogdor.dts?q=ec-mkbp-host-event-wakeup-mask)

[Trogdor MKBP Kconfig](https://source.chromium.org/chromiumos/chromiumos/codesearch/+/main:src/platform/ec/zephyr/projects/trogdor/trogdor/prj.conf?q=CONFIG_PLATFORM_EC_MKBP_EVENT_WAKEUP_MASK)
...
[MKBP event mask enums]:https://source.chromium.org/chromiumos/chromiumos/codesearch/+/main:src/platform/ec/zephyr/include/dt-bindings/wake_mask_event_defines.h?q=HOST_EVENT_MASK
