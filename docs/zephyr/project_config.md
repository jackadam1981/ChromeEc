...
- [**`<program>`**`/BUILD.py`](#BUILD.py): [BUILD_py]{#BUILD_py} [build_py]{#build_py} Defines which **projects** can be made from this
...
- **`<program>`**`/program.conf` - Sets the default Kconfig settings for all
  **projects**. {#program_conf}
...
- **`<project>`**`/project.conf`: Kconfig settings for the **project**. Kconfig
  ...
  **`<program>`**`/program.conf`. {#project_conf}
...
- `zephyr_board` (required): The name of the EC chip used. **Note:** the concept
  ...
  `boards/${ARCH}/${ZEPHYR_BOARD_NAME}`. For the Zephyr EC applicaiton, the EC
