...
used by the Embedded Controller (EC) and the Application Processor (AP) and
2) the Second Source Firmware Cache (SSFC) describes later decisions
...
## FW_CONFIG Device Tree node

CBI Firmware Config fields definition. Each field is defined via a
start bit (from LSB) and a size. The total size of all FW_CONFIG bit fields
must not exceed 32 bits. One default value for each field may be indicated.
