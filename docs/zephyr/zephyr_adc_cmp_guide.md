...
`attr`    | Sensor attribute - This parameter offers wide range of functionality and it can be extended accordingly,<br>minimal parameters supported must be:<br>`SENSOR_ATTR_LOWER_THRESH`: Set voltage corresponding to lower threshold in ADC raw value.<br>`SENSOR_ATTR_UPPER_THRESH`: Set voltage corresponding to upper threshold in ADC raw value.<br>`SENSOR_ATTR_ALERT`: Enable or disable triggering event when analog magnitude meets threshold selection.|
...
`dev`     | Pointer to ADC comparator driver.|
`chan`    | Sensor channel - For the purpose of analog comparator, this enum must support `SENSOR_CHAN_VOLTAGE`.|
`attr`    | Sensor attribute - This parameter offers wide range of functionality and it can be extended accordingly,<br>minimal parameters supported must be:<br>`SENSOR_ATTR_LOWER_THRESH`: Obtain voltage corresponding to lower threshold in ADC raw value.<br>`SENSOR_ATTR_UPPER_THRESH`: Obtain voltage corresponding to upper threshold in ADC raw value.<br>`SENSOR_ATTR_ALERT`: Get comparator current alert status: enabled or disabled.|
...
:---      | :----------
`dev`     | Pointer to ADC comparator driver.|
`trig`    | Pointer to sensor trigger structure, as minimal, members should support the following values:<br>type - `SENSOR_TRIG_THRESHOLD`<br>chan - `SENSOR_CHAN_VOLTAGE` |
...
Return 0 when the ADC sample has been successfully fetched, nevative value otherwise.
...
Return 0 when successfully and val will have sensor sampled value.
