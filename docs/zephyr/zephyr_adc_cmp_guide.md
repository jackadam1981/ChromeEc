...
minimal parameters supported must be:<br>
  SENSOR_ATTR_LOWER_THRESH: Set voltage corresponding to lower threshold in ADC raw value.<br>
  SENSOR_ATTR_UPPER_THRESH: Set voltage corresponding to upper threshold in ADC raw value.<br>
...
return    | Return 0 upon success.<br>
Return -ENOTSUP when the attribute is not being supported.<br>
Return -EINVAL when value is out of valid range.<br>
...
`attr`    | Sensor attribute - This parameter offers wide range of functionality and it can be extended accordingly,
minimal parameters supported must be:<br>
  SENSOR_ATTR_LOWER_THRESH: Obtain voltage corresponding to lower threshold in ADC raw value.<br>
  SENSOR_ATTR_UPPER_THRESH: Obtain voltage corresponding to upper threshold in ADC raw value.<br>
...
:-------- | :----------
`dev`     | Pointer to ADC comparator driver.
`trig`    | Pointer to sensor trigger structure, as minimal, members should support the following values:<br>
    type - SENSOR_TRIG_THRESHOLD<br>
    chan - SENSOR_CHAN_VOLTAGE<br>
