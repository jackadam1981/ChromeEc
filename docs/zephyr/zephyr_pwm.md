...
Configure the PWM module by declaring all the available PWM channels in the
...
:------ | :---------- | :----
`pwmduty` | Get/set PWM duty cycles | `[channel [<percent> or -1=disable] or [raw <value>]]`
...
The corresponding PWM devices nodes have to be explicitly configured and
enabled, and the corresponding properties are platform specific. For example on
NPCX ([nuvoton,npcx-pwm]) based platforms:
