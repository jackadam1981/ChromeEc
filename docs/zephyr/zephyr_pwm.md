...
PWM channels are configured by enabling the PWM framework in the project
...
The command is enabled by setting `CONFIG_PWM_SHELL=y` in the project `prj.conf` file.
...
`period` | Duty cycle percentage to set the specified channel to.
`pulse` | Raw value to set the specified channel to.
...
PWM nodes can be used by a few drivers directly to implement specific features
...
This requires defining a PWM node with `kblight` as a node label, normally in a
separate overlay `keyboard.dts` file, for example on the Volteer reference board:
