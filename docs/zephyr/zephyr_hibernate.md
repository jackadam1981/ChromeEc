...
For NPCX based platforms using PSL, the driver maps the PSL input that caused the
wake to a specific wake source (e.g., AC, Lid, Power Button). To enable this mapping,
you must label the corresponding PSL input nodes in the devicetree with `wake_source_acok`,
`wake_source_lid_open`, and `wake_source_pwr_btn`.
