...
Step 1 configures an LED color based on the state of one or more pins. Step 2 configures the LED policies.
...
### PWM based LEDs
PWM based LEDs are configured with two separate nodes. The cros-ec,pwm-pin-config node, described in [cros-ec,pwm_led_pin_config.yaml], configures the PWM channel and frequency.
...
Color policies are added to configure physical behavior of an LED

e.g. If you want an LED to blink, then create 2 color policies, one to turn on the LED and one to turn off the LED.
