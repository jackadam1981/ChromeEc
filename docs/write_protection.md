...
### Toggling Hardware Write Protection
...
write protect GPIO is disabled or the read-only firmware is not protected. We
mainly use it to guard test/debug functionality in the firmware, so that we can
run tests against the exact firmware we ship by disabling the hardware write
protection.
