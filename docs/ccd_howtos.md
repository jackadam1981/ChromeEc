...
4.  **Disable write protect** using Cr50 [wp console command].
...
5.  (optional) Check write protect is disabled. AP will have left dev mode, so
    you can reenter dev mode and check the wp status from the AP or you can use
    ccd to check.
...
6.  **(recommended) Setup capabilities**, so you can flash the device or open
