...
servo v4, but these guides are not guaranteed to work for them.
...
*   you have [SuzyQ]. You can get one from [sparkfun]
*   you have a ChromeOS device that supports CCD.

### Steps

1.  Charge your chromebook. Suzyq can't charge your device. If it's not charged,
    the device may run out of power while debugging.
...
4.  Verify the CCD device exists.
    ...
    *   Cr50 vid:pid is 18d1:5014. You can use lsusb to check that it shows up.

                outside of chroot > lsusb -vd 18d1:5014
    ...
    *   **Debug connection issues**. If the device doesn't show up, disconnect
        ...
        *   **Which Port**: The DUT only supports CCD on one type C port. Try
            the other port if CCD doesn't show up.

        *   **Suzyq Orientation**: Suzyq is orientation dependent, so it may be
            on the correct port, but it needs to be flipped.
...
5.  Check basic CCD functionality. After the CCD device shows up, the cr50, ec,
    ...
    *   Search for console names.
                ...
                outside of chroot > ls /dev/ttyUSB*
    ...
    *   Open the console.
                ...
                outside of chroot > minicom -D /dev/ttyUSB0
    ...
    *   AP and EC consoles may be read-only depending on the CCD state. See the
        ...
        console is a good enough sign that your Suzyq setup is good.
...
== Opening CCD might require the AP can boot. If you remove write protect,
reflash RO and brick the machine, you may not be able to boot the AP to open
CCD. If you're relying on CCD to recover devices you may want to set some of
CCD flash capabilities to Always.==

### Okay Opening Case and Don't Need it to Stay Disabled.
...
4.  Check write protect is disabled from the AP.

                AP > flashrom --wp-status

**The rest of these steps are optional.**

5.  Disable SW write protect if you want to be able to rewrite RO firmware
    without removing the battery, you can disable SW write protect.

6.  [Setup CCD]. It's really easy to open cr50 with the battery removed. You
    might want to setup CCD while it's removed. If you reflash RO firmware, you
    might brick your device. Setting up CCD now may make it easier to recover.
    This will require SuzyQ. Recovering a bricked machine using CCD will also
    require SuzyQ.

        *   Open Cr50 from the AP or Cr50 console. This will happen
            immediately.
...
### Closed Case or You Want to Permanently Disable It.
...
#### Requirements
    ...
    *   You need a [SuzyQ]. You need access to the cr50 console to disable write
        protect.
...
#### Steps
1.  Open CCD. Here's the most generic way to open ccd. For the full open options
    see [Setup CCD].
...
2.  Start open process from the AP and press the power button when prompted.
    This will take ~5 minutes.

                AP > gsctool -a -o

3.  [Connect][#How-to-Use-SuzyQ] to the Cr50 console using SuzyQ or servo v4.

                outside chroot > minicom -D /dev/ttyUSB0

4.  Disable write protect using Cr50 console command.
    disable until cr50 reboots:
...
5.  Check write protect is disabled. AP will have left dev mode, so you can
    reenter dev mode and check the wp status from the AP or you can use ccd
    to check.
    From AP:
...
**Optional.**
6.  Setup capabilities, so you can flash the device or open ccd without being
    able to boot the AP.
                Cr50 > ccd set OverrideWP Always
                Cr50 > ccd set FlashAP Always
                Cr50 > ccd set FlashEC Always
