...
The entering dev mode instructions will be for chromebooks. If your device isn't
a chromebook. Check out the dev mode instructions in the full
...
![Servo V4 FAFT Setup][ServoV4jpg]


*** note
  NOTE: This image shows the full Servo V4 FAFT setup. We just need the CCD part
  to setup ccd.
***
...
7.  **Verify the CCD device exists.**
    ...
    *   **Debug connection issues**.
        ...
        *   **Orientation:** Orientation doesn't matter with Servo V4. It
	    shouldn't matter.
...
10.  **Set all capabilities to Always.**

                cr50 > ccd reset factory

    Run `ccd` to make sure all capabilities are Always

11.  **Enable Testlab Mode.** This will make it so tests can always open cr50
    without physical presence.

                cr50 > ccd testlab enable

    press power button when prompted. Power button presses are quick for this.
    It'll take seconds not minutes.
...
12.  **Start servod and make sure the EC console works.**
    ...
    * **Debug**
        ...
        *   **Cr50 capabilities:** EC uart capabilitiy nees to always be
	    accessible. **Make sure UartGscTxECRx and all other capabilities are
	    always.**
...
[ServoV4jpg]: https://chromium.googlesource.com/chromiumos/third_party/hdctools/+/refs/heads/master/docs/images/servo_v4_typec.jpg
