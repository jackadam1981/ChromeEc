...
Note that if you created a variant board, you can typically skip changing the
...
- `GPIO_EC_RST_ODL` - On some EC chipsets, low power modes can disable the reset
  pin. In this case, no interrupt handler needs to be registered to the signal,
  but the GPIO pin must still be configured to wake on both edge types. The GPIO
  pin should also be locked prevent the pin configuration from changing after
  the EC read-only code runs.
