...
physically removed, then the 1.5A is available to any Type-A port. In practice,
Type-A devices only determine current limits when they are first inserted, so
any Type-A device that is still plugged in when the 1.5A device is removed will
not notice that it can pull more current. This means that the first Type-A
device **inserted** after removing the original 1.5A device gets access to 1.5A.
...
1.  Insert Type-A disk drive first
    *   Since no other Type-A port is currently supplying more than 900mA, this
        port can supply 1.5A as the CDP.
    *   Disk drive pulls 1.5A, others Type-A ports are now marked as SDPs
        limiting current to 900mA, each.
    *   Current state: `disk drive @ 1.5A`.
2.  Insert Type-A mouse second
    *   Mouse is only allowed 900mA since port is SDP.
    *   Current state: `disk drive @ 1.5A` and `mouse @ 900ma`.
3.  Remove disk drive
    *   CDP port status is relinquished. Now first Type-A port to draw more than
        900mA will claim CDP status.
...
4.  Insert Type-A audio speakers
    *   Since no other Type-A port is currently supplying more than 900mA, this
        port can supply 1.5A as the CDP.
    *   Speaker pulls 1.5A, others Type-A ports are now marked as SDPs limiting
        current to 900mA, each.
    *   Current state: `mouse @ 900ma` and `speakers @ 1.5A`.
...
3.  A PD-capable Type-C dock is inserted third
    *   Since this is the first PD device, we offer it 3A during power
        negotiation
    *   Dock does not want high power from Chromebook; docks selects 1.5A.
    *   Keyboard gets to maintain higher 3A current supply.
    *   Current state: `keyboard @ 3A` and `mouse @ 1.5A` and `dock @ 1.5A`.
4.  A PD-capable Type-C phone is inserted fourth
    ...
    *   Current state: `keyboard @ 1.5A` and `mouse @ 1.5A` and `dock @ 1.5A`
        and `phone @ 3A`.
5.  A PD-capable Type-C tablet is inserted fifth
    *   Since there is already a PD-capable device claiming 3A, the tablet is
        only offered 1.5A.
    *   Current state: `keyboard @ 1.5A` and `mouse @ 1.5A` and `dock @ 1.5A`
        and `phone @ 3A` and `tablet @ 1.5A`.
6.  The PD-capable phone is done charging so it downgrades its power contract to
...
7.  The PD-capable tablet is removed
    ...
    *   Current state: `keyboard @ 3A` and `mouse @ 1.5A` and `dock @ 1.5A` and
        `phone @ 1.5A`.
8.  The non-PD capable keyboard is removed
    *   The next oldest PD-capable device is offered 3A. If there are no
...
9.  The non-PD capable mouse is removed
...
on every port. Note: the 1.5W for Vconn may also be supplied at other voltages,
