...
ChromeOS prefer that the first PD-capable Type-C device **that claims 3A**
should get 3A guaranteed at 5V. Once a PD-capable Type-C device has claimed the
3A slot, then other PD-capable Type-C devices will only be offered a maximum of
1.5A. Once the PD-capable 3A slot has been claimed, it is only relinquished when
...
PD-capable device is offered 3A in order. If no PD-capable devices claims 3A,
...
6.  The PD-capable phone is done charging so it downgrades its power contract to
    ...
    *   Since the user has not interacted with the system, the power
        distribution remains unaffected even though the phone is no longer
        pulling 3A.
    *   The phone continues to claim the 3A slot until it is removed.
    *   Current state: `keyboard @ 1.5A` and `mouse @ 1.5A` and `dock @ 1.5A`
        and `phone @ 3A` and `tablet @ 1.5A`.
7.  The PD-capable phone is removed.
    *   The next oldest PD-capable device is offered 3A in order: dock then
        tablet.
    *   The dock continues to select 1.5A, then the tablet takes 3A.
    *   Current state: `keyboard @ 1.5A` and `mouse @ 1.5A` and `dock @ 1.5A`
        and `tablet @ 3A`.
8.  The PD-capable tablet is removed.
    *   The next oldest PD-capable device is offered 3A. If there are no
        ...
        device is given 3A
    *   The dock continues to select 1.5A, so keyboard is given 3A via CC
        resistor change
    *   Current state: `keyboard @ 3A` and `mouse @ 1.5A` and `dock @ 1.5A`.
9.  The non-PD capable keyboard is removed
    *   The next oldest PD-capable device is offered 3A. If there are no
        PD-capable devices claiming 3A, then the next oldest non-PD capable
        device is given 3A
    *   The dock continues to select 1.5A, so mouse is given 3A via CC resistor
        change
    *   Current state: `mouse @ 3A` and `dock @ 1.5A`.
10. The non-PD capable mouse is removed
    *   The dock continues to select 1.5A
