...
3A is inserted.  Devices will indicate they require 3A operating current in
their sink capabilities, and this will be used as the trigger to let the EC
know to offer that port a 3A source contract.  FRS source partners will also
indicate the need for 3A in their sink capabilities, under their Fast Role
Swap required current.

This policy is laid out in the following flow chart.
...
4.  A PD-capable Type-C hub is inserted third
    ...
    *   Dock does not want high power from Chromebook; hub continues to receive
        1.5A.
...
8.  A FRS-capable dock is inserted
    ...
    *   Since this is a PD capable device, query its FRS current thorugh
        requesting Sink Capabilities.
    ...
    *   Current state: `keyboard @ 1.5A` and `mouse @ 1.5A` and `hub @ 1.5A`
        and `tablet @ 3A`; FRS detection for dock disabled
9.  The PD-capable tablet is removed
    *   The lowest port PD-capable device requiring 3A is offered 3A. If there
        are no PD-capable devices requiring 3A, then the lowest port FRS device
        is allocated 3A.
    *   The hub only requires 1.5A, so FRS is enabled for the dock.
    *   Current state: `keyboard @ 1.5A` and `mouse @ 1.5A` and `hub @ 1.5A`;
        FRS detection for dock is enabled
10.  The FRS dock is removed
    *   The lowest port PD-capable  device requiring 3A is offered 3A. If
        there are no PD-capable devices requiring 3A, then the lowest port FRS
        device is allocated 3A.  If there are no FRS devices, then the lowest
        port non-PD capable device is given 3A.
