...
The 3A is offered with a delay after any connection, disconnection, or power
contract negotiation event. A port will only be offered the addition 1.5A after
a delay of at least 500 ms. One main reason for this delay is to protect against
non-PD capabale devices that only sample the CC resistors once at initial
connection from continuing to consume 3A after we downgrade the CC resistors to
1.5A at a later point in the future. The motivation for this is that any non-PD
device that notices that it can draw more current from a CC resistor change that
happens 500 ms after the initial connection will also notice a CC resistor
change if we downgrade the CC resistors to a lower current advertisement. We
want consistent behavior across non-PD capable devices and PD-capable devices,
so we will only offer the additional 1.5A to PD ports after the same delay.
...
1.  A non-PD capable Type-C keyboard is inserted first
    *   Keybaord will be offered 1.5A initially
    *   Current state: `keyboard @ 1.5A`.
2.  More than 500ms pass.
...
4.  A PD-capable Type-C dock is inserted third
    *   Initially negotiate for 1.5A, then wait 500ms after negotiating.
    *   Since this is the first PD device, we offer it 3A after 500ms from
...
5.  A PD-capable Type-C phone is inserted fourth
    ...
    *   Since there isn't an existing PD-capable device claiming 3A, the phone
        is offered 3A after waiting the 500ms delay from initial negotiation.
...
7.  The PD-capable phone is done charging so it downgrades its power contract to
    ...
    *   After waiting 500ms delay, the next oldest PD-capable device is offered
        3A in order: dock then phone then tablet.
...
8.  The PD-capable tablet is removed
    *   After waiting 500ms delay, the next oldest PD-capable device is offered
        3A. If there are no PD-capable devices claiming 3A, then the oldest
        non-PD capable device is given 3A.
...
9.  The non-PD capable keyboard is removed
    *   After waiting 500ms delay, tshe next oldest PD-capable device is offered
        3A. If there are no PD-capable devices claiming 3A, then the next oldest
        non-PD capable device is given 3A.
