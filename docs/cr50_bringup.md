...
In Chromeboxes H1 is starts from reset every time the system is powered up.
...
In case of X86, when `PLT_RST_L` is held low longer than a certain time (a
...
In the `brdp` command output bit D5 when set signifies `SYS_RST_L` ('regular'
ARM devices) and bit D6 - `PLT_RST_L` (X86 and modified ARM) type of reset.
