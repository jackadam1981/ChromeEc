...
*   Enforce WP forced enabled when FWMP dec mode disable is present.
*   Add extended AP RO verification status.
*   Ensure ccd mode active and power button resets GSC if failed APRO.
*   Enable system reset enforcement for failed AP RO verification if theAllowUnverifiedRo CCD capability is not enabled.
