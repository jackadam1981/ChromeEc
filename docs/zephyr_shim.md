...
can use to detect weather the OS is Zephyr OS.  This is the
...
that under the `foo.h` header with a `CONFIG_ZEPHYR` guard.
...
To run all unit tests, you can run `zmake testall`.
