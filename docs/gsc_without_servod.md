...
[TOC]
This write up desciribes an alternative method of controling GSC and Chrome OS
devices using Servo Micro or C2D2 called `adapters` below.
...
Two helpful scripts available in the same directory with these instructions
are `maptty.sh` which shows how the TTY devices map to the `adapter(s)` and
...
`adapter`, first obtain a firmare image to rescue it to. If you have a chroot
...
```
$ ./brescue.sh <path to>/ti50.bin.prepvt /dev/ttyUSB4
