...
int kDefaultTemperatureCelsius = 25;
 ...
 */
void gyro_still_det_initialization(struct gyro_still_det *det,
...
void gyro_cal_initialization(struct gyro_cal *gyro_cal)
{
	 ...
	 */
	gyro_still_det_initialization(&gyro_cal->gyro_stillness_detect,
				      /* var_threshold */ 5e-5f,
				      /* confidence_delta */ 1e-5f);
	gyro_still_det_initialization(&gyro_cal->accel_stillness_detect,
				      /* var_threshold */ 8e-3f,
				      /* confidence_delta */ 1.6e-3f);
	gyro_still_det_initialization(&gyro_cal->mag_stillness_detect,
				      /* var_threshold */ 1.4f,
...
static int test_gyro_cal_calibration(void)
{
	...
	gyro_cal_get_bias(&gyro_cal, bias, &temperature_celsius,
			  &calibration_time_us);
	...
	TEST_EQ(temperature_celsius, kDefaultTemperatureCelsius, "%d");
...
static int test_gyro_cal_temperature_shift(void)
{
	...
	const float sample_rate = 400.0f; /* [Hz] */
	const float gyro_bias = MDEG_TO_RAD * 150.0f; /* [rad/sec] */
	const float temperature_shift_celsius = 2.6f;
	const uint64_t sample_interval_nanos = HZ_TO_PERIOD_NANOS(sample_rate);
	...
	for (i = 0; i < (int)(8.0f * sample_rate); i++) {
		const uint32_t timestamp_us =
			(i * sample_interval_nanos) / 1000;
		float temperature_celsius = kDefaultTemperatureCelsius;
		...
		if (timestamp_us > 4 * SECOND)
			temperature_celsius += temperature_shift_celsius;
...
static int test_gyro_cal_stillness_timestamp(void)
{
	...
	TEST_EQ(kDefaultTemperatureCelsius, temperature_celsius, "%d");
...
static int test_gyro_cal_set_bias(void)
{
	...
	TEST_EQ(0, temperature_celsius, "%d");
	...
	TEST_EQ(31, temperature_celsius, "%d");
...
static int test_gyro_cal_remove_bias(void)
{
	...
	gyro_cal_set_bias(&gyro_cal, bias, kDefaultTemperatureCelsius,
			  5 * 60 * SECOND);
