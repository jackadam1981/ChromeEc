...
__maybe_unused static int test_connect_as_pd3_source(void)
{
	...
	TEST_ASSERT(!test_startup_and_resume());
	 ...
	 */
	TEST_ASSERT(!mock_tcpci_wait_for_transmit(
		TCPC_TX_SOP, 0, PD_DATA_SOURCE_CAP));
	/*
	...
	TEST_ASSERT(!mock_tcpci_wait_for_transmit(
		TCPC_TX_SOP, PD_CTRL_ACCEPT, 0));
	mock_set_alert(TCPC_REG_ALERT_TX_SUCCESS);
	/*
	 * f) The Tester waits for PS_RDY for tPSSourceOn max.
	 */
	TEST_ASSERT(!mock_tcpci_wait_for_transmit(
		TCPC_TX_SOP, PD_CTRL_PS_RDY, 0));
	mock_set_alert(TCPC_REG_ALERT_TX_SUCCESS);
	 ...
	 */
	TEST_ASSERT(!mock_tcpci_wait_for_transmit(
		TCPC_TX_SOP_PRIME, 0, PD_DATA_VENDOR_DEF));
	mock_set_alert(TCPC_REG_ALERT_TX_SUCCESS);
	task_wait_event(10 * MSEC);
	...
	TEST_ASSERT(!mock_tcpci_wait_for_transmit(
		TCPC_TX_SOP, 0, PD_DATA_VENDOR_DEF));
	mock_set_alert(TCPC_REG_ALERT_TX_SUCCESS);
	task_wait_event(10 * MSEC);
	mock_tcpci_receive(PD_MSG_SOP,
