 ...
 */
	...
	spoof_host_buffer[0] = 0x01; /* cmd reg lsb */
	...
	spoof_host_buffer[4] = 0x02; /* data reg lsb */
	spoof_host_buffer[5] = 0x30; /* data reg msb */
	spoof_host_buffer[6] = 0x00; /* report len lsb */
	spoof_host_buffer[7] = 0x04; /* rpt len msb. len=4 as for odr int32 */
	spoof_host_buffer[8] = REPORT_ID_BASE_ACCEL_SAMPLING_RATE; /* rpt ID */
...
static int test_read_report_descr(void)
{
	...
	i2c_hid_process(sizeof(spoof_host_buffer),
	spoof_host_buffer, &send_response);
	same = memcmp(spoof_host_buffer, report_desc, sizeof(report_desc));
	TEST_ASSERT(same == 0);
	return EC_SUCCESS;
...
static int test_input_report_content(void)
{
	int i;
	struct motion_sensor_t *sensor;
	/* Request spoof buffer received with get report cmd */
	spoof_host_buffer[0] = 0x01; /* cmd reg lsb */
	spoof_host_buffer[1] = 0x30; /* cmd reg msb */
	spoof_host_buffer[2] = 0x11; /* report type (input) & report ID */
	spoof_host_buffer[3] = 0x02; /* get_report cmd opcode */
	spoof_host_buffer[4] = 0x02; /* data reg lsb */
	spoof_host_buffer[5] = 0x30; /* data reg msb */
	spoof_host_buffer[6] = 0x00; /* report len lsb */
	spoof_host_buffer[7] = sizeof(struct hid_accel_input_report); /* len */
