...
test_static int test_write_protect(void)
{
	if (write_protect_enabled) {
		TEST_EQ(write_protect_is_asserted(), true, "%d");
	} else {
		TEST_EQ(write_protect_is_asserted(), false, "%d");
	}
...
void test_run_step(uint32_t state)
{
	...
	/*
	 * Step 2: Check if RO protection is enabled and system_is_locked()
	 * returns 1. It's not necessary to check write protect because it was
	 * checked in step 1.
	 */
...
void run_test(int argc, char **argv)
{
	...
	if (IS_ENABLED(CONFIG_WP_ALWAYS) || (strncmp(argv[1], "wp_on", 5) == 0))
