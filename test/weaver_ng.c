...
static const char *wng_error_str(int code)
{
	...
	WNG_ES_HELPER(WNG_ERR_BITS_PER_LEVEL_INVALID)
	WNG_ES_HELPER(WNG_ERR_HEIGHT_INVALID)
...
static void compute_empty_path(hash_t (*path)[], uint8_t fan_out,
			       height_t height) __attribute__((unused));
static void compute_empty_path(hash_t (*path)[], uint8_t fan_out,
			       height_t height)
{
	hash_t (*view)[height][fan_out - 1] = (void *)path;
	hash_t temp_a = {};
	hash_t temp_b;
	height_t hx = 0;
	uint8_t kx = 0;
	...
	for (hx = 0; hx < height - 1; ++hx) {
		/* Case temp_a -> temp_b */
		compute_hash((const hash_t (*)[])&(*view)[hx], fan_out - 1, 0,
			     (const hash_t *)&temp_a, &temp_b);
		print_array(temp_b, sizeof(temp_b));
		for (kx = 0; kx < fan_out - 1; ++kx)
				      ...
				      wng_response_t *response)
{
	...
	request->data.reset_tree.bits_per_level = 2; /* k = 4 */
	request->data.reset_tree.height = 6; /* L = 12 */
...
static void setup_default_empty_path(hash_t (*hashes)[])
{
	uint8_t fan_out = 1 << EMPTY_TREE.bits_per_level;
	hash_t (*view)[EMPTY_TREE.height]
			[(1 << EMPTY_TREE.bits_per_level) - 1] = (void *)hashes;
	...
	for (hx = 1; hx < EMPTY_TREE.height; ++hx) {
		for (kx = 0; kx < fan_out - 1; ++kx) {
...
//			   1 << EMPTY_TREE.bits_per_level, EMPTY_TREE.height);
...
static int get_index_test(void)
{
	...
	merkle_tree.bits_per_level = 8;
	merkle_tree.height = 8;
...
static int handle_reset_tree_bits_per_level_invalid(void)
{
	...
	/* Test lower bound. */
	request.data.reset_tree.bits_per_level = BITS_PER_LEVEL_MIN - 1;

	TEST_RET_EQ(test_handle_error_msg(&merkle_tree, &request, &response,
					  WNG_ERR_BITS_PER_LEVEL_INVALID),
		    EC_SUCCESS);

	/* Test upper bound. */
	request.data.reset_tree.bits_per_level = BITS_PER_LEVEL_MAX + 1;
...
static int handle_reset_tree_height_invalid(void)
{
	...
	/* Test lower bound. */
	request.data.reset_tree.height = HEIGHT_MIN - 1;

	TEST_RET_EQ(test_handle_error_msg(&merkle_tree, &request, &response,
					  WNG_ERR_HEIGHT_INVALID),
		    EC_SUCCESS);

	/* Test upper bound. */
	request.data.reset_tree.height =
			HEIGHT_MAX(request.data.reset_tree.bits_per_level) + 1;
