...
int test_prl_all_states_named(void)
{
        ...
        for (i = 0; i < test_prl_sm_data_size; ++i) {
                int rv;
                ccprintf("PRL State testing machine %d for empty name!\n", i);
                rv = test_all_states_named(&test_prl_sm_data[i]);
                if (rv) {
...
//         /* Ensure the PD state machine is at least initialized */
