...
static void setup_default_unimported_leaf_data_and_hashes(
		...
		struct unimported_leaf_data_and_hashes_t *data)
{
	...
	setup_default_empty_path((void *)(data->payload + sizeof(*leaf_data)));
}
				      ...
				      struct pw_request_t *request)
{
	...
	MOCK_rand_bytes_src = (uint8_t *)EMPTY_TREE.key_derivation_nounce;
	MOCK_rand_bytes_offset = 0;
	MOCK_rand_bytes_len = sizeof(EMPTY_TREE.key_derivation_nounce);
...
static void auth_hash_update_cb(const void *data, size_t len)
{
	...
	if (len != sizeof(*leaf_data) && len != sizeof(*leaf_data) + 8)
		return;
			  ...
			  uint32_t output[8])
{
	...
	if (MOCK_appkey_derive_fail != EC_SUCCESS)
		return MOCK_appkey_derive_fail;
...
static int handle_reset_tree_success(void)
{
	...
	TEST_ASSERT(MOCK_pw_long_term_storage.height.v ==
		    EMPTY_TREE.height.v);
	TEST_ASSERT_ARRAY_EQ(MOCK_pw_long_term_storage.key_derivation_nounce,
			     EMPTY_TREE.key_derivation_nounce,
			     sizeof(EMPTY_TREE.key_derivation_nounce));
