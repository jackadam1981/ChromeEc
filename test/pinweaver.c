				      ...
				      pw_response_t *response)
{
	...
	request->header.data_length = sizeof(pw_request_reset_tree_t);
				       ...
				       pw_response_t *response)
{
	...
	request->header.data_length = sizeof(pw_request_insert_leaf_t) +
				       ...
				       pw_response_t *response)
{
	...
	request->header.data_length = sizeof(pw_request_remove_leaf_t) +
				      get_path_length(&EMPTY_TREE);
				    ...
				    pw_response_t *response)
{
	...
	request->header.data_length = sizeof(pw_request_try_auth_t) +
				      get_path_length(&EMPTY_TREE);
				      ...
				      pw_response_t *response)
{
	...
	request->header.data_length = sizeof(pw_request_reset_auth_t) +
				      get_path_length(&EMPTY_TREE);
...
static int test_handle_error_msg(merkle_tree_t *merkle_tree,
				 ...
				 pw_response_t *response, uint32_t code)
{
...
static int handle_request_version_mismatch(void)
{
	...
	setup_reset_tree_defaults(&merkle_tree, &buf.request, &buf.response);
...
static int handle_insert_leaf_invalid_length(void)
{
	...
	setup_insert_leaf_defaults(&merkle_tree, &buf.request, &buf.response);
...
static int handle_remove_leaf_invalid_length(void)
{
	...
	setup_remove_leaf_defaults(&merkle_tree, &buf.request, &buf.response);
...
static int handle_reset_auth_invalid_length(void)
{
	...
	setup_reset_auth_defaults(&merkle_tree, &buf.request, &buf.response);
...
static int handle_reset_auth_success(void)
{
	...
	TEST_ASSERT(buf.response.header.data_length ==
		    sizeof(pw_response_reset_auth_t));
	TEST_RET_EQ(buf.response.header.result_code, EC_SUCCESS);
