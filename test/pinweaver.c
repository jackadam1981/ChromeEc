...
{
	int ret = pw_handle_request(merkle_tree, request, response);

	TEST_ASSERT(response->header.version == PW_PROTOCOL_VERSION);
	TEST_ASSERT(response->header.data_length == 0);
	TEST_RET_EQ(response->header.result_code, ret);
	TEST_ASSERT_ARRAY_EQ(response->header.root, root, PW_HASH_SIZE);
	return ret;
}
...
static int handle_request_version_mismatch(void)
{
	...
	TEST_RET_EQ(test_handle_short_msg(&merkle_tree, &buf.request,
					  &buf.response, merkle_tree.root),
...
static int handle_reset_auth_label_invalid(void)
{
	...
	setup_reset_auth_defaults(&merkle_tree, &buf.request);
	buf.request.data.reset_auth.wrapped_leaf_data.pub.label.v |= 0x030000;
...
static int handle_get_log_success(void)
{
	...
	TEST_ASSERT((*view)[2].type.v == PW_TRY_AUTH);
	...
	TEST_ASSERT((*view)[3].type.v == PW_INSERT_LEAF);
	...
	TEST_ASSERT((*view)[4].type.v == PW_RESET_TREE);
