				    ...
				    struct pw_request_t *request)
{
	...
	request->data.try_auth.leaf_location = DEFAULT_LEAF.pub.label;
	...
	memcpy(&request->data.try_auth.wrapped_leaf_data.pub,
...
static void setup_reset_auth_defaults(struct merkle_tree_t *merkle_tree,
				      struct pw_request_t *request)
{
	memcpy(&request->data.reset_auth.wrapped_leaf_data.pub,
	       &DEFAULT_LEAF.pub, sizeof(DEFAULT_LEAF.pub));
	...
	request->data.reset_auth.leaf_location = DEFAULT_LEAF.pub.label;
	...
	DCRYPTO_aes_ctr(request->data.reset_auth.wrapped_leaf_data.cipher_text,
			EMPTY_TREE.wrap_key, sizeof(EMPTY_TREE.wrap_key) * 8,
			DEFAULT_IV, (const uint8_t *)&DEFAULT_LEAF.sec,
...
static void auth_hash_update_cb(const void *data, size_t len)
{
	const struct leaf_public_data_t *pub = data;

	if (len != sizeof(*pub))
		return;

	switch (pub->attempt_count.v) {
	case 10:
	case 6:
		MOCK_hmac = EMPTY_HMAC;
		break;
...
const struct tuple *getvar(const uint8_t *key, uint8_t key_len)
{
	...
	if (key_len == (sizeof(PW_TREE_VAR) - 1) &&
		...
		MOCK_pw_tuple.val_len = PW_LOG_VAR0_SIZE;
		MOCK_tuple_val_ret = &MOCK_pw_log_storage;
		return &MOCK_pw_tuple;
	} else if (key_len == (sizeof(PW_LOG_VAR1) - 1) &&
		   memcmp(key, PW_LOG_VAR1, (sizeof(PW_LOG_VAR1) - 1)) == 0) {
		MOCK_pw_tuple.val_len = PW_LOG_VAR1_SIZE;
		MOCK_tuple_val_ret = ((uint8_t *)&MOCK_pw_log_storage) +
				     PW_LOG_VAR0_SIZE;
		return &MOCK_pw_tuple;
	} else
		return NULL;
	   ...
	   const uint8_t *val, uint8_t val_len)
{
	...
	if (key_len == (sizeof(PW_TREE_VAR) - 1) &&
		...
		return EC_SUCCESS;
		   ...
		   memcmp(key, PW_LOG_VAR0, (sizeof(PW_LOG_VAR0) - 1)) == 0) {
		TEST_ASSERT(val_len == PW_LOG_VAR0_SIZE);
		memcpy(&MOCK_pw_log_storage, val, val_len);
		return EC_SUCCESS;
	} else if (key_len == (sizeof(PW_LOG_VAR1) - 1) &&
		   memcmp(key, PW_LOG_VAR1, (sizeof(PW_LOG_VAR1) - 1)) == 0) {
		TEST_ASSERT(val_len == PW_LOG_VAR1_SIZE);
		memcpy(((uint8_t *)&MOCK_pw_log_storage) + PW_LOG_VAR0_SIZE,
		       val, val_len);
		return EC_SUCCESS;
	} else
		return EC_ERROR_UNKNOWN;
...
static int get_path_auxilary_hash_count_test(void)
{
	struct merkle_tree_t merkle_tree;

	memcpy(&merkle_tree, &EMPTY_TREE, sizeof(merkle_tree));

	TEST_ASSERT(get_path_auxilary_hash_count(&merkle_tree) ==
...
static int handle_insert_leaf_success(void)
{
	...
	TEST_ASSERT_ARRAY_EQ(
			(uint8_t *)&buf.response.data.insert_leaf
					.wrapped_leaf_data.pub,
...
static int handle_try_auth_label_invalid(void)
{
	...
	buf.request.data.try_auth.leaf_location.v |= 0x030000;
...
static int handle_try_auth_rate_limit_reached(void)
{
	...
	TEST_ASSERT(buf.response.header.data_length ==
		    sizeof(buf.response.data.try_auth.seconds_to_wait));
	TEST_RET_EQ(buf.response.header.result_code, PW_ERR_RATE_LIMIT_REACHED);
	...
	TEST_ASSERT_ARRAY_EQ(buf.response.header.root, merkle_tree.root,
			     sizeof(merkle_tree.root));
	TEST_ASSERT(buf.response.data.try_auth.seconds_to_wait.v == 1);
...
static int handle_try_auth_lowent_auth_failed(void)
{
	...
	struct leaf_public_data_t *pub =
			&buf.response.data.try_auth.wrapped_leaf_data.pub;
	...
	TEST_ASSERT(buf.response.header.type.v == PW_MTA_TRY_AUTH);
	TEST_ASSERT(buf.response.header.data_length ==
		    sizeof(buf.response.data.try_auth.wrapped_leaf_data));
...
static int handle_try_auth_success(void)
{
	...
	DCRYPTO_aes_ctr((uint8_t *)&sec, EMPTY_TREE.wrap_key,
			sizeof(EMPTY_TREE.wrap_key) * 8, DEFAULT_IV,
			buf.response.data.try_auth.wrapped_leaf_data
					.cipher_text,
			sizeof(sec));
	TEST_ASSERT(pub->label.v == leaf_data.pub.label.v);
	TEST_ASSERT_ARRAY_EQ(
...
static int handle_reset_auth_label_invalid(void)
{
	...
	buf.request.data.reset_auth.leaf_location.v |= 0x030000;
	...
	setup_reset_auth_defaults(&merkle_tree, &buf.request);
	buf.request.data.reset_auth.wrapped_leaf_data.pub.label.v |= 3;
...
static int handle_reset_auth_success(void)
{
	...
	struct leaf_public_data_t *pub =
			&buf.response.data.reset_auth.wrapped_leaf_data.pub;
	struct leaf_sensitive_data_t sec = {};
	...
	DCRYPTO_aes_ctr((uint8_t *)&sec, EMPTY_TREE.wrap_key,
			sizeof(EMPTY_TREE.wrap_key) * 8, DEFAULT_IV,
			buf.response.data.reset_auth.wrapped_leaf_data
					.cipher_text,
			sizeof(sec));
...
void run_test(void)
{
	...
	RUN_TEST(get_path_auxilary_hash_count_test);
