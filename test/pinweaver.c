...
static void setup_try_auth_defaults_with_leaf(
	const struct leaf_data_t *leaf_data, uint8_t protocol_version,
	uint8_t minor_version, struct merkle_tree_t *merkle_tree,
	struct pw_request_t *request)
{
...
struct tuple *getvar(const uint8_t *key, uint8_t key_len)
{
	...
	for (i = 0; i < ARRAY_SIZE(vars); i++) {
		if ((key_len == vars[i].key_len) &&
			...
			var = malloc(sizeof(struct tuple) + key_len +
				     vars[i].val_size);
			if (var) {
				var->flags = 0;
				var->val_len = vars[i].val_size;
				memcpy(var->data_ + var->key_len, vars[i].val,
				       var->val_len);
				break;
			}
		}
