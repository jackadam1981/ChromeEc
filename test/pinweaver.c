...
struct tuple *getvar(const uint8_t *key, uint8_t key_len)
{
	...
	const struct {
		...
		{
			sizeof(PW_TREE_VAR) - 1,
			PW_TREE_VAR,
			sizeof(MOCK_pw_long_term_storage),
			&MOCK_pw_long_term_storage
		},
		{
			sizeof(PW_LOG_VAR0) - 1,
			PW_LOG_VAR0,
			sizeof(MOCK_pw_log_storage),
			&MOCK_pw_log_storage
		},
	};
	if (!key || !key_len)
	...
	for (i = 0; i < ARRAY_SIZE(vars); i++) {
		if ((key_len == vars[i].key_len) &&
			...
			var = malloc(sizeof(struct tuple) +
				     key_len + vars[i].val_size);
			if (var) {
				var->flags = 0;
				var->val_len = vars[i].val_size;
				memcpy(var->data_ + var->key_len,
				       vars[i].val, var->val_len);
				break;
			}
		}
	}
