...
static uint8_t mock_state[KEYBOARD_KSO_COUNT];
...
void keyboard_raw_drive_pins(int out)
{
	kso_pin_driven = out;
}
...
int keyboard_raw_read(void)
{
	...
	if (kso_pin_driven == KEYBOARD_KSO_COUNT_NONE) {
		return 0;
	} else if (kso_pin_driven == KEYBOARD_KSO_COUNT_ALL) {
