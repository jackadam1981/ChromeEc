...
#define CHECK_KEY_MATRIX_EQ(actual, ...) \
	do { \
		uint8_t expected[KEYBOARD_COLS] = {__VA_ARGS__}; \
		TEST_ASSERT_ARRAY_EQ(actual, expected, KEYBOARD_COLS); \
	} while (0)
...
static int test_kbatboot(void)
{
	...
	CHECK_KEY_MATRIX_EQ(key_matrix,
			    0, 0, 0, 0x80, 0,
			    0, 0, 0, 0x40, 0,
			    0, 0, 0);
