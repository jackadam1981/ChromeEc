...
{
	ccprintf("Test normal stroke.\n");
	reset_key_state();
	mock_key(1, 1, 1);
	TEST_ASSERT(expect_keychange() == EC_SUCCESS);
	TEST_ASSERT(key_state_change[1][1] == 1);
	ccprintf("Pass.\n");

	reset_key_state();

	ccprintf("Test short stroke.\n");
	mock_key(1, 1, 1);
	task_wake_then_sleep_1ms(TASK_ID_KEYSCAN);
	mock_key(1, 1, 0);
	task_wake_then_sleep_1ms(TASK_ID_KEYSCAN);
	TEST_ASSERT(expect_no_keychange() == EC_SUCCESS);
	TEST_ASSERT(key_state_change[1][1] == 0);
	ccprintf("Pass.\n");

	reset_key_state();

	/* A ripple is suppressed by debouncer. */
	ccprintf("Test a ripple.\n");
	mock_key(1, 1, 1);
	task_wake_then_sleep_1ms(TASK_ID_KEYSCAN);
	mock_key(1, 1, 0);
	task_wake_then_sleep_1ms(TASK_ID_KEYSCAN);
	...
	TEST_ASSERT(expect_keychange() == EC_SUCCESS);
	TEST_ASSERT(key_state_change[1][1] == 1);
	TEST_ASSERT(key_state_change[1][2] == 1);
	ccprintf("Pass.\n");
	...
	TEST_ASSERT(expect_keychange() == EC_SUCCESS);
	TEST_ASSERT(key_state_change[1][1] == 0);
	TEST_ASSERT(key_state_change[2][1] == 1);
	ccprintf("Pass.\n");
