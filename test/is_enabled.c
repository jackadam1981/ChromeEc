/* Copyright 2019 The Chromium OS Authors. All rights reserved.
 * Use of this source code is governed by a BSD-style license that can be
 * found in the LICENSE file.
 *
 * Test the IS_ENABLED macro.
 */
#include "common.h"
#include "test_util.h"

#define CONFIG_BLANK
#define CONFIG_ZERO	0
#define CONFIG_ONE	1
#define CONFIG_FIVE	5
#define CONFIG_EXPRESSION CONFIG_FIVE*CONFIG_FIVE

static int test_undef(void)
{
	TEST_ASSERT(IS_ENABLED(CONFIG_NOT_SET) == 0);

	return EC_SUCCESS;
}

static int test_blank(void)
{
	TEST_ASSERT(IS_ENABLED(CONFIG_BLANK) == 1);

	return EC_SUCCESS;
}

static int test_zero(void)
{
	TEST_ASSERT(IS_ENABLED(CONFIG_ZERO) == 0);

	return EC_SUCCESS;
}

static int test_one(void)
{
	TEST_ASSERT(IS_ENABLED(CONFIG_ONE) == 1);

	return EC_SUCCESS;
}

/* The following tests should cause a build breakage. */

/*
static int test_five(void)
{
	TEST_ASSERT(IS_ENABLED(CONFIG_FIVE) == 0);

	return EC_ERROR_UNKNOWN;
}

static int test_expression(void)
{
	TEST_ASSERT(IS_ENABLED(CONFIG_EXPRESSION) == 0);

	return EC_ERROR_UNKNOWN;
}
*/

void run_test(void)
{
	test_reset();

	RUN_TEST(test_undef);
	RUN_TEST(test_blank);
	RUN_TEST(test_zero);
	RUN_TEST(test_one);
	/*
	RUN_TEST(test_five);
	RUN_TEST(test_expression);
	*/

	test_print_result();
}
