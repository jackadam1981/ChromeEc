...
static void test_vbus_enable_reset(void)
{
	int i;

	for (i = 0; i < CONFIG_USB_PD_PORT_MAX_COUNT; i++)
		board_vbus_enabled[i] = false;
}

int pd_set_power_supply_ready(int port)
{
	board_vbus_enabled[port] = true;
	return EC_SUCCESS;
}

void pd_power_supply_reset(int port)
{
	board_vbus_enabled[port] = false;
}
...
__maybe_unused static int test_typec_dis_as_src(void)
{
	...
	TEST_EQ(board_vbus_enabled[0], true, "%d");
	...
	TEST_EQ(board_vbus_enabled[0], false, "%d");
