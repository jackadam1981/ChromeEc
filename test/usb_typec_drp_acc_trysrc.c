 ...
 * thorugh statemachine plus 1000 calls to clock
 */
#define FUDGE (6 * MSEC)
...
static int switch_to_opposite_role(int port, int pull)
{
	...
	if (pull == TYPEC_CC_RP) {
		/* If host is setting Rp, then CCs will negotaite as SNK */
		mock_tcpc.cc1 = TYPEC_CC_VOLT_OPEN;
		mock_tcpc.cc2 = TYPEC_CC_VOLT_RD;
		mock_tcpc.vbus_level = 0;
...
/* TODO(jettrink): test as SNK monitor for Vbus disconnect (not CC line) */
/* TODO(jettrink): test as SRC monitor for CC line state change */
