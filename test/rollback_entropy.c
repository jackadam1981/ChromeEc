...
/* Clang warns that "_EXPECTED_SECRET is not needed and will not be emitted".
 * The same warning occurs for _EXPECTED_SECRET2, and
 * _EXPECTED_UNINITIALIZED_ROLLBACK_SECRET. While this is true, we still want
 * the BUILD_ASSERT() evaluated at compile time, so we disable the warning.
 */
#ifdef __clang__
