...
static const uint8_t FAKE_ENTROPY[] __maybe_unused = {
	0xff, 0xff, 0xff, 0xff
};
...
BUILD_ASSERT(sizeof(EXPECTED_SECRET) == CONFIG_ROLLBACK_SECRET_SIZE);
 ...
 */
static const uint8_t EXPECTED_SECRET2[] = {
...
static const uint8_t EXPECTED_UNINITIALIZED_ROLLBACK_SECRET[] = {
	0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
};
BUILD_ASSERT(sizeof(EXPECTED_UNINITIALIZED_ROLLBACK_SECRET) ==
	     CONFIG_ROLLBACK_SECRET_SIZE);

/* All zeroes */
static const uint8_t
EXPECTED_INITIALIZED_ROLLBACK_SECRET[CONFIG_ROLLBACK_SECRET_SIZE];
...
test_static int test_add_entropy(void)
{
	...
	expected.id = 0;
	expected.rollback_min_version = 0;
	memcpy(expected.secret, EXPECTED_INITIALIZED_ROLLBACK_SECRET,
	       sizeof(expected.secret));
	expected.cookie = VALID_ROLLBACK_COOKIE;
	TEST_EQ(check_equal(&rb_data, &expected), EC_SUCCESS, "%d");
	...
	expected.id = -1;
	expected.rollback_min_version = -1;
	memcpy(expected.secret, EXPECTED_UNINITIALIZED_ROLLBACK_SECRET,
	       sizeof(expected.secret));
	expected.cookie = UNINITIALIZED_ROLLBACK_COOKIE;
	TEST_EQ(check_equal(&rb_data, &expected), EC_SUCCESS, "%d");
	...
	expected.id = 1;
	expected.rollback_min_version = 0;
	memcpy(expected.secret, EXPECTED_SECRET, sizeof(expected.secret));
	expected.cookie = VALID_ROLLBACK_COOKIE;
	TEST_EQ(check_equal(&rb_data, &expected), EC_SUCCESS, "%d");
	...
	expected.id = 0;
	expected.rollback_min_version = 0;
	memcpy(expected.secret, EXPECTED_INITIALIZED_ROLLBACK_SECRET, sizeof
	(expected.secret));
	expected.cookie = VALID_ROLLBACK_COOKIE;
	TEST_EQ(check_equal(&rb_data, &expected), EC_SUCCESS, "%d");
	...
	expected.id = 2;
	expected.rollback_min_version = 0;
	memcpy(expected.secret, EXPECTED_SECRET2, sizeof(expected.secret));
	expected.cookie = VALID_ROLLBACK_COOKIE;
	TEST_EQ(check_equal(&rb_data, &expected), EC_SUCCESS, "%d");
