				    ...
				    const struct batt_conf_header *head)
{
	...
	if (memcmp(&head->config, conf, sizeof(head->config)))
		rv = false;
	else if (head->manuf_name && manuf)
		rv = !strcmp(head->manuf_name, manuf);
	else if (head->manuf_name != NULL || manuf != NULL)
		rv = false;
	else if (head->device_name && device)
		rv = !strcmp(head->device_name, device);
	else
		rv = true;
...
DECLARE_EC_TEST(test_batt_conf_main)
{
	...
	/* Enable BCIC. */ mock_common_control.bcic_enabled = 1;
