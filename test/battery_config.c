...
static uint32_t mock_fw_config;
static int cbi_get_fw_config_return;

int cbi_get_fw_config(uint32_t *fw_config)
{
	*fw_config = mock_fw_config;

	return cbi_get_fw_config_return;
}

DECLARE_EC_TEST(test_batt_conf_main)
{
	mock_fw_config = FW_CONFIG_BCIC_ENABLED;
	cbi_get_fw_config_return = EC_SUCCESS;
...
{
	/* cbi_get_fw_config returns error. */
	mock_fw_config = 0;
	cbi_get_fw_config_return = EC_ERROR_UNKNOWN;
	init_battery_type_called = false;
	batt_conf_main();
	zassert_equal(init_battery_type_called, true);

	/* FW_CONFIG_BCIC_ENABLED isn't set. */
	mock_fw_config = 0;
	cbi_get_fw_config_return = EC_SUCCESS;
	init_battery_type_called = false;
	batt_conf_main();
