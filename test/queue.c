...
static int test_queue8_chunks_offset(void)
{
	...
	TEST_ASSERT(queue_get_write_chunk(&test_queue8, 10).count == 6);
	TEST_ASSERT(queue_get_write_chunk(&test_queue8, 10).buffer ==
			test_queue8.buffer + 2);

	/*
	 * Check offsetting into used memory [0,3]. This is not actually
	 * readable, so we should get back 0.
	 */
	TEST_ASSERT(queue_advance_tail(&test_queue8, 4) == 4);
	TEST_ASSERT(queue_get_write_chunk(&test_queue8, 4).count == 0);
	TEST_ASSERT(queue_get_write_chunk(&test_queue8, 4).buffer == NULL);

	/*
	 * Check offset wrapping around into writable memory.
	 */
	TEST_ASSERT(queue_get_write_chunk(&test_queue8, 8).count == 4);
	TEST_ASSERT(queue_get_write_chunk(&test_queue8, 8).buffer ==
			test_queue8.buffer + 4);
