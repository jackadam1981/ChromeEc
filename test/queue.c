...
static struct consumer const test_consumer;
static struct queue const test_queue_direct =
	QUEUE_DIRECT(16, int8_t, null_producer, test_consumer);
static struct consumer const test_consumer = {
	.queue = &test_queue_direct,
...
{
	struct queue const *q = &test_queue_direct;
	int8_t data[2] = { 17, 42 };
