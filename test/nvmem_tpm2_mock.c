...
void drop_evictable_obj(void *obj)
{
	...
	read_from_cache(obj_addr - sizeof(next_addr), sizeof(next_addr), &next_addr);
	ccprintf("%s:%d dropping obj at cache addr %x, offset %x, addr %p next addr %x aka %x (off s_evictNvStart)\n",
		 __func__, __LINE__, obj_addr - s_evictNvStart, obj_addr, obj, next_addr, next_addr - s_evictNvStart);

	/*
	 * Now, to make it easier to add objects behind the current one, let's
	 * pretend there is no more objects.
	 */
	write_to_cache(obj_addr - sizeof(next_addr), sizeof(list_end), &list_end);
	...
	while (1) {
		...
		read_from_cache(next_addr, sizeof(next_next_addr), &next_next_addr);

		if (!next_next_addr || (next_next_addr == s_evictNvEnd))
			return;

		next_obj_size = next_next_addr - obj_addr - sizeof(uint32_t);
		add_evictable_obj((void  *)((uintptr_t)nvmem_cache_base(NVMEM_TPM) + next_addr + sizeof(uint32_t)), next_obj_size);
