...
static int test_request_with_wait_and_contract(void)
{
	...
	uint32_t expected_status_bsdo =
	   BSDO_CAP((DIV_ROUND_NEAREST(BATTERY_REMAINING_CAPACITY, 1000) *
	   DIV_ROUND_NEAREST(BATTERY_DESIGN_VOLTAGE, 1000)) / 10) |
	   BSDO_PRESENT;
	...
	uint16_t expected_cap_des =
	   ((DIV_ROUND_NEAREST(BATTERY_DESIGN_CAPACITY, 1000) *
	     DIV_ROUND_NEAREST(BATTERY_DESIGN_VOLTAGE, 1000)) / 10);
	uint16_t expected_cap_ful =
	   ((DIV_ROUND_NEAREST(BATTERY_FULL_CHARGE_CAPACITY, 1000) *
	     DIV_ROUND_NEAREST(BATTERY_DESIGN_VOLTAGE, 1000)) / 10);
	...
	/*
	 * Test Exteneded Get_Battery_Cap and Get_Battery_Status messages.
	 */
#ifdef CONFIG_USB_PD_REV30
