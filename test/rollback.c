...
struct rollback_info {
	char * const flash;
	uint32_t region_0_offset;
	uint32_t region_1_offset;
				     ...
				     int region)
{
	...
	volatile char data __unused;
	uint32_t bytes_read = 0;

	uint32_t offset = region == 0 ? info->region_0_offset :
					info->region_1_offset;

	for (i = 0; i < info->region_size_bytes; i++) {
		data = info->flash[offset + i];
		bytes_read++;
	}
