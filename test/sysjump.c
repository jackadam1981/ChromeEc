...
/*
 * original size:
 * 1762de9d19 (Vincent Palatin   2013-12-04 17:15:12 -0800  45)
 */

struct cortex_panic_data_size1 {
...
struct panic_data_size1 {
	...
	union {
		struct cortex_panic_data_size1 cm; /* Cortex-Mx registers */
	};
...
BUILD_ASSERT(sizeof(struct panic_data_size1) == 116);

/*
 * size2 (144 bytes) as of:
...
/*
 * if one of these assertions fails, we must have changed the size of
 * panic_data.  add the definitions here and add tests for the new
 * size.
 */
BUILD_ASSERT(sizeof(struct panic_data_size2) == 144);
BUILD_ASSERT(sizeof(struct panic_data_size2) == sizeof(struct panic_data));
