...
static void initialize_charge_table(int current, int voltage, int ceil)
{
	...
	charge_manager_set_override(CHARGE_PORT_NONE);
...
static int test_override(void)
{
	...
	id = charge_manager_update(CHARGE_SUPPLIER_TEST1, 1, &charge);
	wait_for_charge_manager_update(id);
	TEST_ASSERT(active_charge_port == 1);
	TEST_ASSERT(active_charge_limit == 500);

	/* Set override to p0 and verify p0 is selected */
	id = charge_manager_set_override(0);
	wait_for_charge_manager_update(id);
	TEST_ASSERT(active_charge_port == 0);

	/* Remove override and verify p1 is again selected */
	id = charge_manager_set_override(CHARGE_PORT_NONE);
	wait_for_charge_manager_update(id);
	TEST_ASSERT(active_charge_port == 1);
	...
	/* Set non-zero charge and verify p0 is selected */
	charge.current = 250;
	id = charge_manager_update(CHARGE_SUPPLIER_TEST5, 0, &charge);
	wait_for_charge_manager_update(id);
	TEST_ASSERT(active_charge_port == 0);
	TEST_ASSERT(active_charge_limit == 250);
	 ...
	 */
	charge.current = 300;
	id = charge_manager_update(CHARGE_SUPPLIER_TEST2, 0, &charge);
	wait_for_charge_manager_update(id);
	TEST_ASSERT(active_charge_port == 0);
	TEST_ASSERT(active_charge_limit == 300);
	charge.current = 100;
	id = charge_manager_update(CHARGE_SUPPLIER_TEST1, 0, &charge);
	wait_for_charge_manager_update(id);
	TEST_ASSERT(active_charge_port == 0);
