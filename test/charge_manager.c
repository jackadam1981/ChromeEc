 ...
 */
static void set_dual_role(int port, int role)
{
...
static int test_dual_role(void)
{
	 ...
	 * unplugged and replaced with a single-role source.
	 */
	set_dual_role(0, SINGLE_ROLE);
	charge.current = 0;
	charge_manager_update(CHARGE_SUPPLIER_TEST2, 0, &charge);
	charge.current = 500;
	charge_manager_update(CHARGE_SUPPLIER_TEST2, 0, &charge);
	...
	/*
	 * Verify that we charge from the single-role port if a dual-role
