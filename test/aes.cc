				  ...
				  std::vector<uint8_t> *output)
{
	...
	for (uintptr_t i = 0; i < count / 2; ++i) {
		...
		std::optional<uint8_t> msb = HexCharToDigit(input[i * 2]);
		// least significant 4 bits
		std::optional<uint8_t> lsb = HexCharToDigit(input[i * 2 + 1]);
		if (!msb || !lsb) {
			return false;
		}
				...
				const uint8_t *tag, int tag_size)
{
	...
	int seal_result = EVP_AEAD_CTX_seal_scatter(
		ctx.get(), result, out_tag.data(), &out_tag_len, tag_size,
		nonce, nonce_size, plaintext, plaintext_size,
		extra_input.data(), extra_input.size(), additional_data.data(),
...
test_static int test_aes_gcm(void)
{
	...
	TEST_EQ(test_vectors.size(), 10lu, "%zu");
