...
# TODO(267803007): refactor into multiple modules
...
RW_APP_READY_REGEX = re.compile(r".*Sensor create: 0x0.*")
    ...
    def get_public_tests(board_config: BoardConfig) -> List[TestConfig]:
        ...
        tests = [
            TestConfig(
                ...
                finish_regexes=[RW_APP_READY_REGEX],
                image_to_use=ImageType.PRODUCTION_RW,
            ),
...
def build(
    ...
    if image_type != ImageType.PRODUCTION_RW:
        cmd = cmd + [
            "test-" + test_name,
        ]
    run_subprocess(cmd, check=True)
...
def run_test(
    ...
    if test.image_to_use != ImageType.PRODUCTION_RW:
