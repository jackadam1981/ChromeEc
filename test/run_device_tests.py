...
class TestConfig:
    ...
    def __init__(self, name, image_to_use=ImageType.RW, finish_regexes=None,
                 ...
                 fail_regexes=None, ro_image=None):
...
def find_section_offset_size(section: str, image: bytes) -> (int, int):
    ...
    rw_area = next(area for area in areas if area['name'] == section)
    return (rw_area['offset'], rw_area['size'])

def backup_section(src: bytes, section: str) -> bytearray:
    """Backup section content in byte array"""
    (src_start, src_size) = find_section_offset_size(section, src)
    src_end = src_start + src_size
    return src[src_start:src_end]

def replace_section(src: bytes, dst: bytearray, section: str) -> bool:
    """Replace section content with data from bytearray"""
...
def replace_ro(image: bytearray, ro: bytes):
    ...
    ro_pubkey = backup_section(image, 'KEY_RO')
    ...
    if not copy_section(ro, image, 'EC_RO'):
        return False

    # Restore RO public key.
    if not replace_section(ro_pubkey, image, 'KEY_RO'):
        return False
...
def patch_image(test: TestConfig, board: str) -> bool:
    """Apply changes to built image"""
    # Replace RO part of the firmware with provided one.
    if test.ro_image is not None:
        image_path = os.path.join(EC_DIR, 'build', board, test.name,
                                  test.name + '.bin')
        image = read_file(image_path)
        ro = read_file_gsutil(test.ro_image)
        # If there was a problem with downloading or replacing RO image
        # fails, return false.
        if len(ro) == 0 or not replace_ro(image, ro):
            return False
...
def main():
    ...
    for test in test_list:
        ...
        if not patch_image(test, args.board):
            test.passed = False
            continue
