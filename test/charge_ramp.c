			 ...
			 int vbus_low_current, int overcurrent_current)
{
	...
	chg_ramp_charge_supplier_change(port, supplier_type);
...
static int test_equal_priority_overcurrent(void)
{
	 ...
	 */
	while (1) {
		plug_charger(CHARGE_SUPPLIER_TEST4, 0, 500, 3000, 2000);
		if (wait_stable_no_overcurrent())
			break;
		plug_charger(CHARGE_SUPPLIER_TEST4, 1, 500, 3000, 2000);
		if (wait_stable_no_overcurrent())
			break;
