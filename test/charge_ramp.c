...
static int charge_limit_close_to(int ma)
{
	return charge_limit_ma > ma - 200 && charge_limit_ma <= ma;
}

static int charge_limit_slightly_over(int ma)
{
	return charge_limit_ma >= ma && charge_limit_ma < ma + 100;
}
...
static int test_full_ramp(void)
{
	...
	TEST_ASSERT(charge_limit_ma >= 500 && charge_limit_ma < 800);
	TEST_ASSERT(wait_stable_no_overcurrent());
	/* And ramp up to 3A */
...
static int test_vbus_dip(void)
{
	...
	TEST_ASSERT(wait_stable_no_overcurrent());
	TEST_ASSERT(charge_limit_close_to(1500));
...
static int test_overcurrent(void)
{
	...
	TEST_ASSERT(charge_limit_ma >= 500 && charge_limit_ma <= 700);
...
static int test_partial_load(void)
{
	...
	TEST_ASSERT(wait_stable_no_overcurrent());
	TEST_ASSERT(charge_limit_slightly_over(1500));

	/* Ok someone just started watching YouTube */
	system_load_current_ma = 2000;
	TEST_ASSERT(wait_stable_no_overcurrent());
	TEST_ASSERT(charge_limit_slightly_over(2000));
	...
	TEST_ASSERT(charge_limit_close_to(2500));
...
static int test_charge_supplier_stable_ramp(void)
{
	...
	TEST_ASSERT(wait_stable_no_overcurrent());
	TEST_ASSERT(charge_limit_close_to(1400));
...
static int test_charge_port_change(void)
{
	...
	TEST_ASSERT(wait_stable_no_overcurrent());
	TEST_ASSERT(charge_limit_close_to(2000));
...
static int test_vbus_shift(void)
{
	...
	TEST_ASSERT(wait_stable_no_overcurrent());
	TEST_ASSERT(charge_limit_close_to(1900));

	/* The charger heats up and VBUS voltage drops by 100mV */
	vbus_low_current_ma = 1800;
	TEST_ASSERT(wait_stable_no_overcurrent());
	TEST_ASSERT(charge_limit_close_to(1800));
