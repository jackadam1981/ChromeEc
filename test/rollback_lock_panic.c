...
/* Mock the MPU lock function to fail, just like the Zephyr version */
 ...
 * They are expected to panic when mpu_lock_rollback fails.
 */
...
test_static int verify_panic_reason(void)
{
	...
	/* For ARM, R4 usually holds the panic reason in EC */
...
{
	/* Only run in RW to avoid unnecessary reboots in RO */
