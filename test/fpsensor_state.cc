...
test_static enum ec_error_list test_fp_set_sensor_mode(void)
{
	...
	TEST_ASSERT(global_context.state.templ_valid == 0);
	TEST_ASSERT(global_context.state.sensor_mode == 0);
	...
	TEST_ASSERT(output_mode == global_context.state.sensor_mode);
	...
	TEST_ASSERT(global_context.state.sensor_mode == requested_mode);

	/* GIVEN max number of fingers already enrolled */
	global_context.state.sensor_mode = 0;
...
test_static enum ec_error_list test_fp_set_maintenance_mode(void)
{
	...
	/* THEN sensor_mode is updated */
	TEST_ASSERT(global_context.state.sensor_mode ==
...
test_fp_command_read_match_secret_fail_timeout(void)
{
	...
	TEST_ASSERT(
		global_context.state.positive_match_secret_state.deadline.val ==
		0);
...
test_fp_command_read_match_secret_unmatched_fgr(void)
{
	...
	global_context.state.positive_match_secret_state = test_state;
...
test_fp_command_read_match_secret_derive_succeed(void)
{
	...
	memset(global_context.state.user_id, 0,
