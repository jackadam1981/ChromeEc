...
const uint8_t rgbkbd_map[] = {
	RGBKBD_TERM,
	RGBKBD_COORD(1, 2), RGBKBD_TERM,
	RGBKBD_COORD(3, 4), RGBKBD_COORD(5, 6), RGBKBD_TERM,
	RGBKBD_TERM,
	RGBKBD_TERM,
};
const size_t rgbkbd_map_size = ARRAY_SIZE(rgbkbd_map);

extern const struct rgbkbd_coord *rgbkbd_table[EC_RGBKBD_MAX_KEY_COUNT];
...
{
	const struct rgbkbd_coord *led;

	led = rgbkbd_table[0];
	zassert_equal(RGBKBD_COORD(led->x, led->y),
		      RGBKBD_TERM, "key[0] -> None");
	...
	led = rgbkbd_table[3];
	zassert_equal(RGBKBD_COORD(led->x, led->y),
		      RGBKBD_TERM, "key[3] -> None");

	led = rgbkbd_table[4];
	zassert_equal(RGBKBD_COORD(led->x, led->y),
		      RGBKBD_TERM, "key[4] -> None");
