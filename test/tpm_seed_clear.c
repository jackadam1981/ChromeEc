...
test_static uint8_t tpm_seed_value[FP_CONTEXT_TPM_BYTES];

test_export_static void set_tpm_seed(const uint8_t *input_tpm_seed);
test_export_static void get_tpm_seed(uint8_t *output_tpm_seed);

#ifdef SECTION_IS_RW
void set_tpm_seed(const uint8_t *input_tpm_seed)
{
	memcpy(tpm_seed, input_tpm_seed, FP_CONTEXT_TPM_BYTES);
};

void get_tpm_seed(uint8_t *output_tpm_seed)
{
	memcpy(output_tpm_seed, tpm_seed, FP_CONTEXT_TPM_BYTES);
};
#endif
...
test_static int test_tpm_seed_before_reboot(void)
{
	if (IS_ENABLED(SECTION_IS_RW)) {
		get_tpm_seed(tpm_seed_value);

		TEST_ASSERT_ARRAY_EQ(tpm_seed_value, zero_fake_tpm_seed,
				     FP_CONTEXT_TPM_BYTES);

		set_tpm_seed(default_fake_tpm_seed);
		get_tpm_seed(tpm_seed_value);

		TEST_ASSERT_ARRAY_EQ(tpm_seed_value, default_fake_tpm_seed,
				     FP_CONTEXT_TPM_BYTES);
	}
	return EC_SUCCESS;
}
