...
#define t10MSEC  (10 * MSEC)
#define t15MSEC  (15 * MSEC)
#define t20MSEC  (20 * MSEC)
#define t30MSEC  (30 * MSEC)
#define t40MSEC  (40 * MSEC)
#define t50MSEC  (50 * MSEC)
#define t60MSEC  (60 * MSEC)
#define t70MSEC  (70 * MSEC)
#define t80MSEC  (80 * MSEC)
#define t90MSEC  (90 * MSEC)
#define t100MSEC (100 * MSEC)
#define t200MSEC (200 * MSEC)
...
static uint64_t state_transition(int port, uint64_t timeout)
{
	uint64_t s, e, stop;
	int state;

	s = get_time().val;
	stop = get_time().val + timeout;

	state = get_typec_state_id(port);
	while (get_typec_state_id(port) == state && get_time().val < stop) {
		task_wake(PD_PORT_TO_TASK_ID(port));
		task_wait_event(CYCLE_TIME);
	}
...
static int in_range(uint64_t timeout, uint64_t st, uint64_t et)
{
	return (st <= timeout) && (timeout <= et);
}
...
static int test_vpd_host_src_detection(void)
{
	 ...
	 */
	state_transition(port, t200MSEC);
	state_transition(port, t200MSEC);
	...
	timeout = state_transition(port, t20MSEC);

	TEST_ASSERT(in_range(timeout, t10MSEC, t20MSEC));
	...
	timeout = state_transition(port, t200MSEC);

	TEST_ASSERT(in_range(timeout, 0, t40MSEC));
	TEST_ASSERT(get_typec_state_id(port) == UNATTACHED_SNK);
...
static int test_ctvpd_ctattached_unsupported(void)
{
	...
	timeout = state_transition(port, 50 * MSEC);

	TEST_ASSERT(in_range(timeout, PD_T_DRP_SRC, PD_T_DRP_SRC + t15MSEC));
	TEST_ASSERT(get_typec_state_id(port) == CTUNATTACHED_UNSUPPORTED);
...
static int test_ctvpd_behavior_case1(void)
{
	...
	TEST_ASSERT(in_range(timeout, PD_T_CC_DEBOUNCE,
						PD_T_CC_DEBOUNCE + t30MSEC));
	TEST_ASSERT(get_typec_state_id(port) == ATTACHED_SNK);
	...
	timeout = state_transition(port, t50MSEC);

	TEST_ASSERT(in_range(timeout, PD_T_PD_DEBOUNCE,
						PD_T_PD_DEBOUNCE + t30MSEC));
	TEST_ASSERT(get_typec_state_id(port) == CTUNATTACHED_VPD);
	...
	TEST_ASSERT(in_range(timeout, PD_T_DRP_SRC, PD_T_DRP_SRC + t30MSEC));
	TEST_ASSERT(get_typec_state_id(port) == CTUNATTACHED_UNSUPPORTED);

	timeout = state_transition(port, t20MSEC);

	TEST_ASSERT(in_range(timeout, 0, t20MSEC));
	TEST_ASSERT(get_typec_state_id(port) == CTATTACH_WAIT_UNSUPPORTED);
...
static int test_ctvpd_behavior_case2(void)
{
	...
	TEST_ASSERT(in_range(timeout, 0, t200MSEC));
	TEST_ASSERT(get_typec_state_id(port) == CTUNATTACHED_VPD);
	TEST_ASSERT(check_ct_ccs_rd());
...
static int test_ctvpd_behavior_case3(void)
{
	...
	TEST_ASSERT(in_range(timeout, PD_T_CC_DEBOUNCE,
				PD_T_CC_DEBOUNCE + PD_T_DRP_SRC + t30MSEC));
	TEST_ASSERT(get_typec_state_id(port) == UNATTACHED_SRC);

	timeout = state_transition(port, t200MSEC);

	TEST_ASSERT(in_range(timeout, t20MSEC, PD_T_DRP_SRC + t20MSEC));
	TEST_ASSERT(get_typec_state_id(port) == UNATTACHED_SNK);
	...
	TEST_ASSERT(in_range(timeout, 0, t15MSEC));
	TEST_ASSERT(get_typec_state_id(port) == ATTACH_WAIT_SRC);
...
static int test_ctvpd_behavior_case5(void)
{
	 ...
	 */
	state_transition(port, t100MSEC);
	state_transition(port, t100MSEC);
	TEST_ASSERT(get_typec_state_id(port) == UNATTACHED_SNK);
	...
	TEST_ASSERT(in_range(timeout, PD_T_DRP_TRY + PD_T_DEBOUNCE,
				PD_T_DRP_TRY + PD_T_DEBOUNCE + t30MSEC));
	TEST_ASSERT(get_typec_state_id(port) == TRY_WAIT_SRC);
	TEST_ASSERT(check_host_rpusb());

	timeout = state_transition(port, t200MSEC);

	TEST_ASSERT(in_range(timeout, PD_T_TRY_CC_DEBOUNCE,
					PD_T_TRY_CC_DEBOUNCE + t30MSEC));
	TEST_ASSERT(get_typec_state_id(port) == ATTACHED_SRC);
	TEST_ASSERT(mock_get_vbus_pass_en());
...
static int test_ctvpd_behavior_case6(void)
{
	...
	TEST_ASSERT(in_range(timeout, PD_T_VPDCTDD, PD_T_VPDCTDD + t30MSEC));
	TEST_ASSERT(get_typec_state_id(port) == CTUNATTACHED_VPD);
	TEST_ASSERT(check_host_rp3a0());
	...
	timeout = state_transition(port, (1000 * MSEC));

	TEST_ASSERT(in_range(timeout, PD_T_TRY_WAIT,
					PD_T_TRY_WAIT + t200MSEC));
	TEST_ASSERT(get_typec_state_id(port) == CTATTACHED_UNSUPPORTED);
