...
static int host_disconnect_source(void)
{
	...
	return mock_set_host_cc_source_voltage(0);
}

static int host_connect_source(enum vbus_type vbus)
{
	mock_set_host_vbus(vbus);
	return mock_set_host_cc_source_voltage(PD_SNK_VA_MV);
}
...
static int host_connect_sink(enum snk_con_voltage_type v)
{
	switch (v) {
	case SRC_CON_DEF:
		return mock_set_host_cc_sink_voltage(PD_SRC_DEF_RD_THRESH_MV);
	case SRC_CON_1_5:
		return mock_set_host_cc_sink_voltage(PD_SRC_1_5_RD_THRESH_MV);
	case SRC_CON_3_0:
		return mock_set_host_cc_sink_voltage(PD_SRC_3_0_RD_THRESH_MV);
	}
...
{
	return mock_set_host_cc_sink_voltage(PD_SRC_DEF_VNC_MV);
}
#endif
...
static int test_ctvpd_attach_wait_src(void)
{
	...
	/* Host CC Debounce */
	task_wake(PD_PORT_TO_TASK_ID(port));
	task_wait_event(PD_T_DRP_SRC + 5 * MSEC);
...
static int test_ctvpd_try_snk(void)
{
	...
	/* try_src timer */
	task_wake(PD_PORT_TO_TASK_ID(port));
	task_wait_event(PD_T_TRY_SRC);

	/* host cc debounce */
	task_wake(PD_PORT_TO_TASK_ID(port));
	task_wait_event(PD_T_DEBOUNCE + SM_TRANSITION_TIME);
...
static int test_ctvpd_try_wait_src(void)
{
	...
	task_wake(PD_PORT_TO_TASK_ID(port));
	task_wait_event(PD_T_TRY_SRC + SM_TRANSITION_TIME);
...
static int test_ctvpd_ctunattached_vpd(void)
{
	...
	task_wake(PD_PORT_TO_TASK_ID(port));
	task_wait_event(PD_T_VPDCTDD + SM_TRANSITION_TIME);
...
static int test_ctvpd_ctdisabled_vpd(void)
{
	...
	task_wake(PD_PORT_TO_TASK_ID(port));
	task_wait_event(PD_T_PD_DEBOUNCE + SM_TRANSITION_TIME);
	...
	task_wake(PD_PORT_TO_TASK_ID(port));
	task_wait_event(PD_T_VPDDISABLE + SM_TRANSITION_TIME);
...
static int test_ctvpd_ctunattached_unsupported(void)
{
	...
	task_wake(PD_PORT_TO_TASK_ID(port));
	task_wait_event(PD_T_DRP_SRC + SM_TRANSITION_TIME);
...
static int test_ctvpd_ctattached_unsupported(void)
{
	...
	task_wake(PD_PORT_TO_TASK_ID(port));
	task_wait_event(PD_T_TRY_WAIT + PD_T_TRY_SRC);
	...
	task_wake(PD_PORT_TO_TASK_ID(port));
	task_wait_event(PD_T_DEBOUNCE + PD_T_TRY_SRC + SM_TRANSITION_TIME);
