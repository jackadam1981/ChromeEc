...
test_static int test_derive_positive_match_secret_fail_trivial_key(void)
{
	static uint8_t output[FP_POSITIVE_MATCH_SECRET_BYTES];

	/* GIVEN that the user ID is valid */
	memset(user_id, 0, sizeof(user_id));
	...
	/* GIVEN that the sha256 output is trivial */
	...
	/* OR GIVEN that the sha256 output is trivial (0xFF) */
	mock_ctrl_fpsensor_crypto.output_type =
...
void run_test(int argc, char **argv)
{
	...
	RUN_TEST(test_derive_positive_match_secret_fail_trivial_key);
