...
static int verify_chunk_data_reception(int port, uint16_t header)
{
	int i;
...
static int simulate_receive_260_byte_data(int port,
						enum pd_data_msg_type msg_type)
{
	...
	for (j = 0; j < 10; j++) {
		*(uint16_t *)td = PD_EXT_HEADER(chunk_num, 0, 260);
		td[2] = *(expected_data + data_offset++);
		td[3] = *(expected_data + data_offset++);

		for (i = 4; i < 28; i++)
			td[i] = *(expected_data + data_offset++);

		header = PD_HEADER(msg_type, pd_port[port].power_role,
			pd_port[port].data_role, pd_port[port].msg_rx_id,
			7, pd_port[port].rev, 1);

		cycle_through_state_machine(port, 2, 10 * MSEC);
		...
		simulate_rx_msg(port, header, 7, (uint32_t *)td);
		task_wait_event(30 * MSEC);
		...
		cycle_through_state_machine(port, 4, 20 * MSEC);
	...
	return verify_chunk_data_reception(port, header);
}
...
static int verify_extended_260_byte_data_msg_transmission(int port,
			enum pd_data_msg_type msg_type, int request_extra_chunk)
{
	int i;
	int j;
	uint32_t td;
	...
	for (j = 0; j < 10; j++) {
		...
		td = PD_EXT_HEADER(chunk_number_to_send, 0, 260);
...
static int simulate_send_extended_260_byte_data_msg_request_from_pe(int port,
		enum tcpm_transmit_type type, enum pd_ctrl_msg_type msg_type,
		int request_extra_chunk)
{
	int i;
	uint8_t *buf = emsg[port].buf;
	uint8_t *td = (uint8_t *)test_data;
	...
	return verify_extended_260_byte_data_msg_transmission(port, msg_type,
							request_extra_chunk);
}
...
static int test_ctrl_msg_xmit(void)
{
	int i;
	int port = PORT0;
...
static int test_ctrl_msg_xmit_with_retry_and_fail(void)
{
	int i;
	int port = PORT0;
...
static int test_ctrl_msg_xmit_with_retry_and_success(void)
{
	int i;
	int port = PORT0;
...
static int test_data_msg_xmit(void)
{
	int i;
	int port = PORT0;
...
static int test_data_msg_xmit_to_much_data(void)
{
	int port = PORT0;
...
static int test_extended_260_byte_data_msg_xmit(void)
{
	int port = PORT0;
	...
	TEST_ASSERT(simulate_send_extended_260_byte_data_msg_request_from_pe(
			port, TCPC_TX_SOP, PD_EXT_MANUFACTURER_INFO, 0));
...
static int test_extended_260_byte_data_msg_xmit_with_extra_chunk(void)
{
	int port = PORT0;
	...
	TEST_ASSERT(simulate_send_extended_260_byte_data_msg_request_from_pe(
			port, TCPC_TX_SOP, PD_EXT_MANUFACTURER_INFO, 1));
	...
	TEST_ASSERT(!pd_port[port].mock_got_soft_reset);
	TEST_ASSERT(pd_port[port].mock_pe_message_sent);
	TEST_ASSERT(pd_port[port].mock_pe_error == ERR_RCH_CHUNKED);
...
static int test_receive_extended_260_byte_data_msg(void)
{
	int port = PORT0;
	...
	TEST_ASSERT(simulate_receive_260_byte_data(port,
...
static int test_xmit_soft_reset_msg(void)
{
	int port = PORT0;
...
void run_test(void)
{
	...
	RUN_TEST(test_ctrl_msg_xmit);
	RUN_TEST(test_ctrl_msg_xmit_with_retry_and_fail);
	RUN_TEST(test_ctrl_msg_xmit_with_retry_and_success);
	RUN_TEST(test_data_msg_xmit);
	RUN_TEST(test_data_msg_xmit_to_much_data);

	RUN_TEST(test_receive_control_msg);
	RUN_TEST(test_receive_data_msg);
	RUN_TEST(test_receive_soft_reset_msg);
	RUN_TEST(test_xmit_soft_reset_msg);
	...
	RUN_TEST(test_data_msg_xmit);
	RUN_TEST(test_data_msg_xmit_to_much_data);
	RUN_TEST(test_extended_260_byte_data_msg_xmit);
	RUN_TEST(test_extended_260_byte_data_msg_xmit_with_extra_chunk);

	RUN_TEST(test_receive_control_msg);
	RUN_TEST(test_receive_data_msg);
	RUN_TEST(test_receive_extended_260_byte_data_msg);
	RUN_TEST(test_receive_soft_reset_msg);
	RUN_TEST(test_xmit_soft_reset_msg);
	RUN_TEST(test_pe_execute_hard_reset_msg);
	RUN_TEST(test_phy_execute_hard_reset_msg);
