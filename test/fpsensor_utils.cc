...
test_static int test_remove_image_offset()
{
	TEST_ASSERT(!remove_image_offset(FP_CAPTURE_VENDOR_FORMAT
					 << FP_MODE_CAPTURE_TYPE_SHIFT));
	TEST_ASSERT(remove_image_offset(FP_CAPTURE_SIMPLE_IMAGE
					<< FP_MODE_CAPTURE_TYPE_SHIFT));
	TEST_ASSERT(remove_image_offset(FP_CAPTURE_PATTERN0
					<< FP_MODE_CAPTURE_TYPE_SHIFT));
	TEST_ASSERT(remove_image_offset(FP_CAPTURE_PATTERN1
					<< FP_MODE_CAPTURE_TYPE_SHIFT));
	TEST_ASSERT(!remove_image_offset(FP_CAPTURE_QUALITY_TEST
					 << FP_MODE_CAPTURE_TYPE_SHIFT));
	TEST_ASSERT(remove_image_offset(FP_CAPTURE_RESET_TEST
					<< FP_MODE_CAPTURE_TYPE_SHIFT));

	/* Check the case when FP_MODE_CAPTURE is set. */
	TEST_ASSERT(!remove_image_offset(
		FP_MODE_CAPTURE | FP_CAPTURE_QUALITY_TEST
					  << FP_MODE_CAPTURE_TYPE_SHIFT));
...
void run_test(int argc, const char **argv)
{
	...
	RUN_TEST(test_remove_image_offset);
