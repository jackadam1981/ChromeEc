...
test_static int test_fp_command_establish_and_load_pairing_key(void)
{
	int rv;
	struct ec_response_fp_establish_pairing_key_keygen keygen_response;
	...
	if (BN_bn2binpad(x_bn.get(), wrap_params.peers_pubkey.x,
			 FP_PAIRING_KEY_EC_PUBLIC_KEY_LEN) !=
	    FP_PAIRING_KEY_EC_PUBLIC_KEY_LEN) {
		return EC_RES_UNAVAILABLE;
	}

	if (BN_bn2binpad(y_bn.get(), wrap_params.peers_pubkey.y,
			 FP_PAIRING_KEY_EC_PUBLIC_KEY_LEN) !=
	    FP_PAIRING_KEY_EC_PUBLIC_KEY_LEN) {
		return EC_RES_UNAVAILABLE;
	}
...
test_static int test_fp_command_establish_pairing_key_fail(void)
{
	int rv;
	struct ec_response_fp_establish_pairing_key_keygen keygen_response;
	struct ec_params_fp_establish_pairing_key_wrap wrap_params;
...
test_static int test_fp_command_load_pairing_key_fail(void)
{
	int rv;
	struct ec_response_fp_establish_pairing_key_keygen keygen_response;
	struct ec_params_fp_establish_pairing_key_wrap wrap_params;
...
test_static int test_fp_command_read_match_secret_with_pubkey_succeed(void)
{
	...
	if (BN_bn2binpad(x_bn.get(), params.pubkey.x,
			 FP_PAIRING_KEY_EC_PUBLIC_KEY_LEN) !=
	    FP_PAIRING_KEY_EC_PUBLIC_KEY_LEN) {
		return EC_RES_UNAVAILABLE;
	}

	if (BN_bn2binpad(y_bn.get(), params.pubkey.y,
			 FP_PAIRING_KEY_EC_PUBLIC_KEY_LEN) !=
	    FP_PAIRING_KEY_EC_PUBLIC_KEY_LEN) {
		return EC_RES_UNAVAILABLE;
	}
	...
	x_bn = bssl::UniquePtr<BIGNUM>(BN_bin2bn(
		response.pubkey.x, FP_PAIRING_KEY_EC_PUBLIC_KEY_LEN, nullptr));
	if (x_bn == nullptr) {
		return EC_RES_UNAVAILABLE;
	}

	y_bn = bssl::UniquePtr<BIGNUM>(BN_bin2bn(
		response.pubkey.y, FP_PAIRING_KEY_EC_PUBLIC_KEY_LEN, nullptr));
	if (y_bn == nullptr) {
		return EC_RES_UNAVAILABLE;
	}
