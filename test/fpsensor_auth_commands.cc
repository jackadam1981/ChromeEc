...
test_static enum ec_error_list test_fp_command_migrate_legacy_template(void)
{
	fp_reset_and_clear_context();
	...
	struct ec_params_fp_migrate_legacy_template migrate_params = {};

	/* Migrate command should fail outside a nonce session. */
	TEST_EQ(test_send_host_command(EC_CMD_FP_MIGRATE_LEGACY_TEMPLATE, 0,
				       &migrate_params, sizeof(migrate_params),
				       NULL, 0),
		EC_RES_ACCESS_DENIED, "%d");
...
test_static enum ec_error_list
test_fp_command_migrate_legacy_template_failure(void)
{
	fp_reset_and_clear_context();
...
extern "C" void run_test(int argc, const char **argv)
{
	...
	RUN_TEST(test_fp_command_migrate_legacy_template);
	RUN_TEST(test_fp_command_migrate_legacy_template_failure);
	test_print_result();
}
