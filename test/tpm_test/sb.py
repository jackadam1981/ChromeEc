...
def sb_test(tpm):
    ...
    maced_key_words = int.from_bytes(rsp[key_blob_words*4 + 4: key_blob_words*4 + 8], 'little')
    maced_key = rsp[key_blob_words*4 + 4: key_blob_words*4 + 8 + maced_key_words*4]
    print("Maced key words=", maced_key_words, "actual ", (len(maced_key) - 4)/4)
    print("Maced key=", maced_key.hex())

    # Gen CSR includes mac'ed key and challenge
    gen_csr = maced_key + U32(4) + U32(0xff)
    w_rsp = tpm.command(wrap_sb_command(SB_RpcGenerateCertificateV2Request, gen_csr))
    print("GenerateCertificateV2Request:", w_rsp.hex())
    rsp = tpm.unwrap_ext_response(SB_RpcGenerateCertificateV2Request, w_rsp)
    print("GenerateCertificateV2Request:", rsp.hex())


    # Prepre KeyParameters for the generateKey()
    g = (U32(15) # size in words
    + Tag(KM_TAG_ALGORITHM, 0) + U32(ALGORITHM_EC)
    + Tag(KM_TAG_KEY_SIZE, 0) + U32(256)
    + Tag(KM_TAG_PURPOSE, 0) + U32(KEY_PURPOSE_SIGN)
    + Tag(KM_TAG_DIGEST, 0) + U32(KM_DIGEST_SHA_2_256)
    + Tag(KM_TAG_ALLOW_WHILE_ON_BODY, 0)
    + Tag(KM_TAG_USER_ID, 4) + U32(0xf00)
    + Tag(KM_TAG_APPLICATION_ID, 4) + U32(0xaaaa)
    + Tag(KM_TAG_APPLICATION_DATA, 4) + U32(0xaaaa))
    print("g=", g.hex())
    ...
    expected_blob_hdr = (U32(SECURITY_LEVEL_STRONGBOX) + U32(18) +
         Tag(KM_TAG_ORIGIN, 0) + U32(KEY_ORIGIN_GENERATED) +
         Tag(KM_TAG_OS_VERSION, 0) + U32(0) +
         Tag(KM_TAG_OS_PATCHLEVEL, 0) + U32(0) +
         Tag(KM_TAG_VENDOR_PATCHLEVEL, 0) + U32(0) +
         Tag(KM_TAG_BOOT_PATCHLEVEL, 0) + U32(0) +
         Tag(KM_TAG_ALGORITHM, 0) + U32(ALGORITHM_EC) +
         Tag(KM_TAG_KEY_SIZE, 0) + U32(256) +
         Tag(KM_TAG_PURPOSE, 0) + U32(KEY_PURPOSE_SIGN) +
         Tag(KM_TAG_DIGEST, 0) + U32(KM_DIGEST_SHA_2_256) +
         U32(SECURITY_LEVEL_KEYSTORE) + U32(3) +
        Tag(KM_TAG_ALLOW_WHILE_ON_BODY, 0) +
        Tag(KM_TAG_USER_ID, 4) + U32(0xf00) +
        U32(80)
         )
    ...
    key_params = U32(4) + (Tag(KM_TAG_APPLICATION_ID, 4) + U32(0xaaaa)
    + Tag(KM_TAG_APPLICATION_DATA, 4) + U32(0xaaaa))
    begin_cmd = U32(KEY_PURPOSE_SIGN) + key_blob + key_params
    w_rsp = tpm.command(wrap_sb_command(SB_DeviceBegin, begin_cmd))
    rsp = tpm.unwrap_ext_response(SB_DeviceBegin, w_rsp)
    print("r=", rsp.hex())
    operation_id = rsp[-4:]
    w_rsp = tpm.command(wrap_sb_command(SB_OperationUpdate, operation_id + U32(32) + b"0123456789ABCDEF0123456789ABCDEF"))
    rsp = tpm.unwrap_ext_response(SB_OperationUpdate, w_rsp)
    print("r=", rsp.hex())

    w_rsp = tpm.command(wrap_sb_command(SB_OperationFinish, operation_id + U32(0)))
