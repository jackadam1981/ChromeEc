...
def sb_SetStrongboxState(tpm, state: int):
    cmd = U32(state)
    print("sb_SetStrongboxState:", cmd.hex())
    w_rsp = tpm.command(wrap_sb_command(SB_SetStrongboxState, cmd))
    return tpm.unwrap_ext_response(SB_SetStrongboxState, w_rsp)
