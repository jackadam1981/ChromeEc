...
def sb_GenerateCertificateV2Request(tpm, maced_keys, challenge, uds_cert):
    cmd = wrap_bytes(maced_keys) + wrap_bytes(challenge) + wrap_bytes(uds_cert)
...
def sb_GenerateKey(tpm, key_params, attest_key):
    ...
    sw_size = int.from_bytes(rsp[8 + hw_size * 4 : 12 + hw_size * 4], "little")
    key_tags = key_blob[4 : 8 + hw_size * 4 + 8 + sw_size * 4]
...
def sb_test(tpm):
    ...
    csr = sb_GenerateCertificateV2Request(tpm, maced_key1, b"1234", b"\xa0")
    ...
    if key_tags != expected_tags:
        print(
            f"KeyCharacteristics doesn't match:\nexpected: {expected_tags.hex()}\nreceived: {key_tags.hex()}"
        )
