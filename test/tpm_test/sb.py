...
def sb_test(tpm):
    ...
    key_blob = rsp[4: key_blob_words*4 + 4]
    print("Key blob words=", key_blob_words)
    print("Key blob=", key_blob.hex())
    # expected blob header
    expected_blob_hdr = (U32(SECURITY_LEVEL_STRONGBOX) + U32(16) +
    ...
    if key_blob[:len(expected_blob_hdr)] != expected_blob_hdr:
        print("Prefix doesn't match")

    key_params = U32(4) + (Tag(KM_TAG_APPLICATION_ID, 4) + U32(0xaaaa)
    + Tag(KM_TAG_APPLICATION_DATA, 4) + U32(0xaaaa))
    w_rsp = tpm.command(wrap_sb_command(SB_DeviceBegin, U32(KEY_PURPOSE_SIGN) + key_blob + key_params))
