...
def _encrypt_tests(tpm):
    ...
    for data in _ENCRYPT_INPUTS:
        ...
        if msg != plaintext:
            print 'FAIL: RSA ENCRYPT TEST: %s' % str(data)
            return
        else:
            print '  PASS: RSA ENCRYPT TEST: %s' % str(data)
    print 'SUCCESS: RSA ENCRYPT TESTS'
...
def _sign_tests(tpm):
    ...
    for data in _SIGN_INPUTS:
        ...
        if verified != '\x01':
            print 'FAIL: RSA SIGN TEST: %s' % str(data)
            return
        else:
            print '  PASS: RSA SIGN TEST %s' % str(data)
    print 'SUCCESS: RSA SIGN TESTS'
