...
def cros_ack():
  ...
  perso_data = ''
  chip_perso_data_file = os.path.join(FLAGS.input, chip_id + '.res')
  if not os.path.exists(chip_perso_data_file):
    print('Error: chip file %s not found' % chip_perso_data_file)
    sys.exit(1)
  ...
  ok = struct.unpack(CrosOkResponse_v0_FMT, response[TPM_HEADER_SIZE:])
  print(utils.cursor_back() + 'RSA PERSO RESULT: %x' % ok)

  # Blobs include serialized TPM header.
  response = t.command(blob.ecc_blob)
  # Skip over TPM header and unpack Ok response.
  ok = struct.unpack(CrosOkResponse_v0_FMT, response[TPM_HEADER_SIZE:])
  print(utils.cursor_back() + 'ECC PERSO RESULT: %x' % ok)
