...
class SPI(object):
  __OFFSET = 0               # TODO: what should this be?
  __FRAME_SIZE = 1024        # Including TPM header.

  def __init__(self, debug_mode, dev_path='/dev/tpm0'):
    self.__debug = debug_mode
    self.__fd = os.open(dev_path, os.O_RDWR)
  ...
  def __transfer(self, data):
    ...
    remaining = len(data)
    while remaining:
      count = min(remaining, self.__FRAME_SIZE)

      written = os.write(self.__fd, data[:count])
      if written != count:
        raise subcmd.TPMTestError(
          'Write failed, tried: %d bytes, wrote: %d bytes' % (
            count, written))

      data = data[count:]
      remaining -= count

    response = os.read(self.__fd, self.__FRAME_SIZE)
    if len(response) > self.__FRAME_SIZE:
        raise subcmd.TPMTestError(
          'Invalid read, got %d bytes, expected < %d' % (
            len(response), self.__FRAME_SIZE))
    return response

  def command(self, cmd_data):
    TPM.validate(cmd_data)
    response = self.__transfer(data)
    self.validate(response, response_mode=True)
    return response

  def wrap_ext_command(self, subcmd_code, cmd_body):
    return TPM.wrap_ext_command(subcmd_code, cmd_body)
...
def parse_args():
  ...
  parser.add_argument(
    '-f', '--ftdi',
    help='Use the FTDI protocol to speak to CR50',
    action='store_true')
  parser.add_argument(
    '-s', '--spi',
    help='Use the SPI protocol to speak to CR50',
    action='store_true')

  global FLAGS
  FLAGS = parser.parse_args()

  if (not FLAGS.ftdi and not FLAGS.spi) or \
     (FLAGS.ftdi and FLAGS.spi):
    print('Error: one (and only one) of -s or -f should be specified')
    sys.exit(1)
