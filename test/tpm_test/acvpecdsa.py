...
def ecc_test(tpm):
    """Run ECDSA tests"""
    test_inputs = utils.read_vectors("wycheproof_vectors/ecdsa_secp256r1_sha256_test.json")
    ...
    for test_group in test_inputs['testGroups']:
        ...
        for test in test_group['tests']:
          test_name = 'Test '+ str(test['tcId']) + ' ' + test['comment']
          print(test_name)
          msg = bytes.fromhex(test['msg'])
          sig = bytes.fromhex(test['sig'])
          cmd = _verify_any_cmd(_ECC_CURVES['NIST-P256'], _HASH_FUNC['NIST-P256'],
                              _SIGN_MODE['ECDSA'], msg, sig[1:],
                              key_x, key_y)
          expected = b'\x01'
          wrapped_response = tpm.command(tpm.wrap_ext_command(subcmd.ECC, cmd))
          verified = tpm.unwrap_ext_response(subcmd.ECC, wrapped_response)
