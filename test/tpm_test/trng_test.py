...
def trng_test(tpm, trng_output):
  ...
  # compute least common multiplier (LCM) of 1000 * TRNG_SAMPLE_BITS and 32
  # and convert it in bytes, as such it's divisible by 8
  sample_size = (1000 * TRNG_SAMPLE_BITS * 32 /
                 gcd(1000 * TRNG_SAMPLE_BITS, 32)) / 8
  if sample_size > 2032:
    raise subcmd.TpmTestError("Can't meet NIST requirement of 1000 samples" \
                              " in batch - %d bytes can't fit TPM command" %
			      sample_size)
  # combine reads of small batches if possible, 2032 is max size of TPM
  # command response, adjusted to header size
  sample_size = int((2032 / sample_size)) * sample_size
