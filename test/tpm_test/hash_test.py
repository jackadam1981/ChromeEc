...
test_inputs = (
  ...
  (CMD_START,   ALG_SHA256, 1, '', 'some more text, this time for sha256'),
  (CMD_CONT,    ALG_SHA256, 1, '', 'some more text, this time for sha256'),
  (CMD_START,   ALG_SHA256, 2, '', 'this could be anything, we just need to'),
  (CMD_HASH,    ALG_SHA1,   3, '', 'interleave a SHA1 single calculation'),
  (CMD_HASH,    ALG_SHA256, 3, '', 'interleave a SHA256 single calculation'),
  (CMD_START,   ALG_SHA1,   3, '', 'let\'s interleave a sha1 calculation'),
  (CMD_CONT,    ALG_SHA256, 2, '', 'fill up a second context with something'),
  (CMD_CONT,    ALG_SHA256, 1, '', 'let\'s feed some more into context 1'),
  (CMD_FINISH,  ALG_SHA256, 1, '', 'some more text, this time for sha256'),
  (CMD_CONT,    ALG_SHA1,   3, '', 'with two active sha256 calculations'),
  (CMD_FINISH,  ALG_SHA1,   3, '', 'this should be enough'),
  (CMD_FINISH,  ALG_SHA256, 2, '', 'it does not really matter what'),
  (CMD_HASH,    ALG_SHA384, 0, '', 'some more text, this time for sha384'),
  (CMD_HASH,    ALG_SHA512, 0, '', 'some more text, this time for sha512'),
  (CMD_START,   ALG_SHA256, 0, '', 'some more text, this time for sha256'),
  (CMD_START,   ALG_SHA384, 1, '', 'some more text, this time for sha384'),
  (CMD_CONT,    ALG_SHA384, 1, '', 'some more text, this time for sha384'),
  (CMD_CONT,    ALG_SHA256, 0, '', 'some more text, this time for sha256'),
  (CMD_START,   ALG_SHA512, 2, '', 'some more text, this time for sha512'),
  (CMD_CONT,    ALG_SHA512, 2, '', 'some more text, this time for sha512'),
  (CMD_FINISH,  ALG_SHA512, 2, '', 'this should be enough'),
  (CMD_FINISH,  ALG_SHA256, 0, '', 'this should be enough'),
  (CMD_FINISH,  ALG_SHA384, 1, '', 'this should be enough'),
)
...
def hash_test(tpm):
  ...
  cmd_map = {
    CMD_START: 'hash start',
    CMD_CONT: 'hash cont',
    CMD_FINISH: 'hash finish',
    CMD_HASH: 'hash',
    CMD_HMAC_SW: 'hmac sw',
    CMD_HMAC_HW: 'hmac hw'
  }
  ...
  for test in test_inputs:
    hash_cmd, hash_mode, handle, hmac_key, text = test
    mode_name = cmd_map[hash_cmd]
    alg_name, hash_func = alg_map[hash_mode]
