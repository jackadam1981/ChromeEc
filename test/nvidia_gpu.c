...
static int gpio_nvidia_gpu_acoff_odl = 1;
...
__override int gpio_get_level(enum gpio_signal signal)
{
	if (signal == GPIO_NVIDIA_GPU_ACOFF_ODL)
		return gpio_nvidia_gpu_acoff_odl;
	return 0;
}
...
{
	if (signal == GPIO_NVIDIA_GPU_ACOFF_ODL)
		gpio_nvidia_gpu_acoff_odl = value;
}
		  ...
		  enum d_notify_level level)
{
	...
	gpio_nvidia_gpu_acoff_odl = gpio_acoff;
...
static int test_ac_unplug(void)
{
	...
	/* Unplug AC. D1 -> D5 */
	plug_ac(0);
	TEST_EQ(gpio_get_level(GPIO_NVIDIA_GPU_ACOFF_ODL), 0, "%d");
	...
	usleep(NVIDIA_GPU_ACOFF_DURATION / 2);
	TEST_EQ(gpio_get_level(GPIO_NVIDIA_GPU_ACOFF_ODL), 1, "%d");
