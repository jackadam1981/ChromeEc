...
test_static int benchmark_unaligned_access_array()
{
	...
	alignas(int32_t) std::array<int8_t, 50> test_array_1;
	alignas(int32_t) std::array<int8_t, 50> test_array_2;
	std::array<int32_t, 10> test_array_3;
	std::array<int32_t, 10> test_array_4 = { 0 };

	for (int i = 0; i < test_array_1.size(); ++i) {
		test_array_1[i] = static_cast<int8_t>(i);
	}
	for (int i = 0; i < test_array_2.size(); ++i) {
		test_array_2[i] = static_cast<int8_t>(i + 1);
	}
	...
	for (int t = 0; t < iteration; ++t) {
		const int32_t *test_array_1_ptr =
			reinterpret_cast<const int32_t *>(test_array_1.data() +
							  1);
		const int32_t *test_array_2_ptr =
			reinterpret_cast<const int32_t *>(test_array_2.data() +
							  1);

		for (int i = 0; i < test_array_3.size(); ++i) {
			test_array_3[i] = (*test_array_1_ptr++) /
					  (*test_array_2_ptr++); /* unaligned */
		}
		TEST_ASSERT_ARRAY_EQ(test_array_3.data(), test_array_4.data(),
				     test_array_3.size());
	}
	t1 = get_time();
	ccprintf(" (speed gain: %" PRId64 " ->", t1.val - t0.val);
