...
static int test_insert_async_event(void)
{
	...
	TEST_HASBITS(data[0].flags, ASYNC_EVENT_FLUSH);
	TEST_EQUAL(data[0].sensor_num, 0, %d);

	TEST_HASBITS(data[1].flags, ASYNC_EVENT_ODR);
...
static int test_adding_timestamp(void)
{
	...
	TEST_HASBITS(data[0].flags, MOTIONSENSE_SENSOR_FLAG_TIMESTAMP);
...
static int test_stage_data_removed_oversample(void)
{
	...
	TEST_HASBITS(data[0].flags, MOTIONSENSE_SENSOR_FLAG_TIMESTAMP);
	TEST_EQUAL(data[0].timestamp, 100, %u);
	TEST_NHASBITS(data[1].flags, MOTIONSENSE_SENSOR_FLAG_TIMESTAMP);
	TEST_EQUAL(data[1].data[0], 1, %d);
	TEST_EQUAL(data[1].data[1], 2, %d);
	TEST_EQUAL(data[1].data[2], 3, %d);
	TEST_HASBITS(data[2].flags, MOTIONSENSE_SENSOR_FLAG_TIMESTAMP);
...
static int test_stage_data_remove_all_oversampling(void)
{
	...
	TEST_HASBITS(data[0].flags, MOTIONSENSE_SENSOR_FLAG_TIMESTAMP);
	TEST_EQUAL(data[0].timestamp, 100, %u);
	TEST_HASBITS(data[1].flags, MOTIONSENSE_SENSOR_FLAG_TIMESTAMP);
...
static int test_stage_data_evicts_data_with_timestamp(void)
{
	...
	TEST_HASBITS(data->flags, MOTIONSENSE_SENSOR_FLAG_TIMESTAMP);
	TEST_EQUAL(data->timestamp, 100, %u);
	TEST_HASBITS(data[CONFIG_ACCEL_FIFO_SIZE - 2].flags,
