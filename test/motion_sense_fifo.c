...
static int test_insert_async_event(void)
{
	...
	TEST_EQUAL(read_count, 2, %d);
	TEST_EQUAL(data_bytes_read,
		   2 * sizeof(struct ec_response_motion_sensor_data),
		   %d);

	TEST_HAS_BITS_SET(data[0].flags, ASYNC_EVENT_FLUSH);
	TEST_EQUAL(data[0].sensor_num, 0, %d);

	TEST_HAS_BITS_SET(data[1].flags, ASYNC_EVENT_ODR);
	TEST_EQUAL(data[1].sensor_num, 1, %d);
...
static int test_wake_up_needed(void)
{
	...
	TEST_EQUAL(motion_sense_fifo_wake_up_needed, 0, %d);

	motion_sense_fifo_commit_data();
	TEST_EQUAL(motion_sense_fifo_wake_up_needed, 1, %d);
...
static int test_adding_timestamp(void)
{
	...
	TEST_EQUAL(read_count, 1, %d);
	TEST_HAS_BITS_SET(data[0].flags, MOTIONSENSE_SENSOR_FLAG_TIMESTAMP);
	TEST_EQUAL(data[0].timestamp, 100, %u);
	return EC_SUCCESS;
}
...
static int test_stage_data_sets_xyz(void)
{
	...
	TEST_EQUAL(motion_sensors->xyz[0], 1, %d);
	TEST_EQUAL(motion_sensors->xyz[1], 2, %d);
	TEST_EQUAL(motion_sensors->xyz[2], 3, %d);
...
static int test_stage_data_removed_oversample(void)
{
	...
	read_count = motion_sense_fifo_read(
		sizeof(data), CONFIG_ACCEL_FIFO_SIZE,
		data, &data_bytes_read);
	TEST_EQUAL(read_count, 3, %d);
	TEST_HAS_BITS_SET(data[0].flags, MOTIONSENSE_SENSOR_FLAG_TIMESTAMP);
	TEST_EQUAL(data[0].timestamp, 100, %u);
	TEST_HAS_BITS_CLEARED(data[1].flags, MOTIONSENSE_SENSOR_FLAG_TIMESTAMP);
	TEST_EQUAL(data[1].data[0], 1, %d);
	TEST_EQUAL(data[1].data[1], 2, %d);
	TEST_EQUAL(data[1].data[2], 3, %d);
	TEST_HAS_BITS_SET(data[2].flags, MOTIONSENSE_SENSOR_FLAG_TIMESTAMP);
	TEST_EQUAL(data[2].timestamp, 110, %u);
...
static int test_stage_data_remove_all_oversampling(void)
{
	...
	TEST_HAS_BITS_SET(data[0].flags, MOTIONSENSE_SENSOR_FLAG_TIMESTAMP);
	TEST_EQUAL(data[0].timestamp, 100, %u);
	TEST_HAS_BITS_SET(data[1].flags, MOTIONSENSE_SENSOR_FLAG_TIMESTAMP);
	TEST_EQUAL(data[1].timestamp, 110, %u);
...
static int test_stage_data_evicts_data_with_timestamp(void)
{
	...
	read_count = motion_sense_fifo_read(
		sizeof(data), CONFIG_ACCEL_FIFO_SIZE,
		data, &data_bytes_read);
	TEST_EQUAL(read_count, CONFIG_ACCEL_FIFO_SIZE - 1, %d);
	TEST_HAS_BITS_SET(data->flags, MOTIONSENSE_SENSOR_FLAG_TIMESTAMP);
	TEST_EQUAL(data->timestamp, 100, %u);
	TEST_HAS_BITS_SET(data[CONFIG_ACCEL_FIFO_SIZE - 2].flags,
		     MOTIONSENSE_SENSOR_FLAG_TIMESTAMP);
	TEST_EQUAL(data[CONFIG_ACCEL_FIFO_SIZE - 2].timestamp,
...
static int test_add_data_no_spreading_when_different_sensors(void)
{
	...
	read_count = motion_sense_fifo_read(
		sizeof(data), CONFIG_ACCEL_FIFO_SIZE,
		data, &data_bytes_read);
	TEST_EQUAL(read_count, 4, %d);
	TEST_HAS_BITS_SET(data[0].flags, MOTIONSENSE_SENSOR_FLAG_TIMESTAMP);
	TEST_EQUAL(data[0].timestamp, 100, %u);
	TEST_HAS_BITS_SET(data[2].flags, MOTIONSENSE_SENSOR_FLAG_TIMESTAMP);
	TEST_EQUAL(data[2].timestamp, 100, %u);
...
static int test_add_data_no_spreading_different_timestamps(void)
{
	...
	TEST_EQUAL(data[0].timestamp, 100, %u);
	TEST_HAS_BITS_SET(data[2].flags, MOTIONSENSE_SENSOR_FLAG_TIMESTAMP);
	TEST_EQUAL(data[2].timestamp, 120, %u);
...
static int test_spread_data_in_window(void)
{
	...
	TEST_EQUAL(read_count, 4, %d);
	TEST_HAS_BITS_SET(data[0].flags, MOTIONSENSE_SENSOR_FLAG_TIMESTAMP);
	TEST_EQUAL(data[0].timestamp, now - 18000, %u);
	TEST_HAS_BITS_SET(data[2].flags, MOTIONSENSE_SENSOR_FLAG_TIMESTAMP);
	TEST_EQUAL(data[2].timestamp, now, %u);
...
static int test_spread_data_by_collection_rate(void)
{
	...
	TEST_EQUAL(read_count, 4, %d);
	TEST_HAS_BITS_SET(data[0].flags, MOTIONSENSE_SENSOR_FLAG_TIMESTAMP);
	TEST_EQUAL(data[0].timestamp, now - 20500, %u);
	TEST_HAS_BITS_SET(data[2].flags, MOTIONSENSE_SENSOR_FLAG_TIMESTAMP);
	TEST_EQUAL(data[2].timestamp, now - 500, %u);
