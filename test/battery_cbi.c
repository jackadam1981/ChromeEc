...
int batt_cbi_read_fet_info(void);
int batt_cbi_read_fuel_gauge_info(void);
int batt_cbi_read_battery_info(void);
...
DECLARE_EC_TEST(test_read_ship_mode)
{
	...
	struct ship_mode_info *dflt = &default_battery_info.fuel_gauge.ship_mode;
	enum cbi_data_tag tag;
	uint8_t d8;

	/* Read without data in CBI. */
...
DECLARE_EC_TEST(test_read_fet_info)
{
	...
	/* Read without data in CBI. */
	zassert_equal(batt_cbi_read_fet_info(), EC_SUCCESS);
	...
	zassert_equal(cbi_set_board_info(tag, (uint8_t *)&info->reg_mask,
					 sizeof(info->reg_mask)), EC_SUCCESS);
	...
	zassert_equal(cbi_set_board_info(tag, (uint8_t *)&info->cfet_mask,
					 sizeof(info->cfet_mask)), EC_SUCCESS);
...
DECLARE_EC_TEST(test_read_fuel_gauge_info)
{
	...
	/* Read without data in CBI. */
	zassert_equal(batt_cbi_read_fuel_gauge_info(), EC_SUCCESS);
	...
	zassert_equal(cbi_set_board_info(tag, (uint8_t *)&info->manuf_name,
					 sizeof(info->manuf_name)), EC_SUCCESS);
...
DECLARE_EC_TEST(test_read_battery_info)
{
	...
	/* Read without data in CBI. */
	zassert_equal(batt_cbi_read_battery_info(), EC_SUCCESS);
...
TEST_SUITE(test_suite_battery_cbi)
{
	ztest_test_suite(
		...
		ztest_unit_test_setup_teardown(test_read_sleep_mode,
					       test_setup, test_teardown),
		ztest_unit_test_setup_teardown(test_read_fet_info,
					       test_setup, test_teardown),
