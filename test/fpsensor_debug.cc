...
test_static int test_get_image_frame_params(void)
{
	...
	for (enum fp_capture_type current_capture_type : kCaptureTypesArray) {
		...
		if (expected_params) {
			...
			TEST_EQ(image_frame_params.width,
				expected_params->width, "%d");
			TEST_EQ(image_frame_params.height,
				expected_params->height, "%d");
			TEST_EQ(image_frame_params.bpp, expected_params->bpp,
				"%d");
			TEST_EQ(image_frame_params.frame_size,
				expected_params->frame_size, "%d");
			TEST_EQ(image_frame_params.fp_capture_type,
				expected_params->fp_capture_type, "%d");
		} else {
			TEST_EQ(rv, EC_ERROR_INVAL, "%d");
			TEST_EQ(image_frame_params.width, zero_params.width,
				"%d");
			TEST_EQ(image_frame_params.height, zero_params.height,
				"%d");
			TEST_EQ(image_frame_params.bpp, zero_params.bpp, "%d");
			TEST_EQ(image_frame_params.frame_size,
				zero_params.frame_size, "%d");
			TEST_EQ(image_frame_params.fp_capture_type,
				zero_params.fp_capture_type, "%d");
		}
	}
	return EC_SUCCESS;
}
