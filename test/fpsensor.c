...
static int test_hkdf_expand_raw(const uint8_t *prk, int prk_size,
				const uint8_t *info, int info_size,
				const uint8_t *expected_okm, int okm_size)
{
...
{
	/*
	 * The fp_encryption_salt and user_id are generated by rand_bytes in
	 * chip/host/trng.c, and the expected vector is obtained by running
	 * boringSSL locally.
	 */
	static const uint8_t expected[] = {
		0x7d, 0x0c, 0x07, 0x1d, 0x65, 0x9b, 0xdd, 0xb2,
		0xe5, 0x4b, 0x8e, 0xfd, 0x80, 0xec, 0xca, 0x03,
		0x33, 0xb8, 0x63, 0x1a, 0x48, 0x04, 0x34, 0x91,
		0xfd, 0x8c, 0xa7, 0x43, 0x4f, 0x82, 0x44, 0x3e,
	};
	...
	fp_clear_context();
	init_trng();
	rand_bytes(fp_encryption_salt[0], sizeof(fp_encryption_salt[0]));
	rand_bytes(user_id, sizeof(user_id));
	exit_trng();

	TEST_ASSERT(bytes_nontrivial(fp_encryption_salt[0],
				     sizeof(fp_encryption_salt[0])));

	/* THEN the derivation will succeed. */
	TEST_ASSERT(derive_pos_match_secret(output, fp_encryption_salt[0])
		== EC_SUCCESS);
