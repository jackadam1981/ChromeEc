...
test_static int test_fp_rand_finger_id(void)
{
	...
	uint8_t zeros[FP_FINGER_ID_BYTES] = { 0 };
	...
	TEST_ASSERT(sizeof(new_finger_id) == 2 * sizeof(fake_rand_bytes));
	...
	TEST_ASSERT_ARRAY_EQ(zeros, new_finger_id, sizeof(new_finger_id));
	fp_clear_context();
...
test_static int test_derive_new_pos_match_secret(void)
{
	...
	/* THEN generating positive match salt and finger id will succeed. */
	TEST_ASSERT(generate_pos_match_salt_finger_id() == EC_RES_SUCCESS);
	templ_valid++;

	/* THEN the derivation will succeed. */
	TEST_ASSERT(derive_pos_match_secret(output,
					    fp_pos_match_salt[templ_valid - 1],
					    finger_id[templ_valid - 1])
		== EC_RES_SUCCESS);
...
test_static int test_command_read_match_secret(void)
{
	...
	TEST_ASSERT(generate_pos_match_salt_finger_id() == EC_RES_SUCCESS);
	fp_pos_match_secret_readable |= BIT(templ_valid);
	templ_valid++;
