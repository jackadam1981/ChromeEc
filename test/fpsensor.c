...
test_static int test_enable_positive_match_secret(void)
{
	struct positive_match_secret_state dumb_state = {
		.template_with_secret = FP_NO_SUCH_TEMPLATE,
		.readable = false,
		.deadline.val = 0,
	};
	...
	TEST_ASSERT(fp_enable_positive_match_secret(0, &dumb_state) ==
		EC_SUCCESS);
	TEST_ASSERT(dumb_state.template_with_secret == 0);
	...
	TEST_ASSERT(fp_enable_positive_match_secret(0, &dumb_state) ==
		EC_ERROR_UNKNOWN);
	TEST_ASSERT(dumb_state.template_with_secret == FP_NO_SUCH_TEMPLATE);
...
test_static int test_command_read_match_secret_unreadable(void)
{
	...
	/* EVEN IF the finger is "with secret". */
	TEST_ASSERT(positive_match_secret_state.template_with_secret
		== params.fgr);
