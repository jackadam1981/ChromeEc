...
/* Increment in microseconds */
static const uint64_t timestamp_increment = 10;
...
{
	int rv;
	uint8_t new_finger_id[FP_FINGER_ID_BYTES] = { 0 };
	...
	TEST_ASSERT_ARRAY_EQ(new_finger_id, fake_rand_bytes,
			     sizeof(fake_rand_bytes));
	TEST_ASSERT_ARRAY_EQ(new_finger_id + sizeof(fake_rand_bytes),
			     fake_rand_bytes, sizeof(fake_rand_bytes));
	...
	TEST_ASSERT_ARRAY_EQ(finger_id[0], new_finger_id,
			     sizeof(new_finger_id));
	fp_clear_context();
