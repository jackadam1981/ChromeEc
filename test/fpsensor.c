...
static const uint64_t timestamp_increment = MSEC / 100;
...
test_static int test_derive_pos_match_secret(void)
{
	...
	/* THEN the derivation will succeed. */
	TEST_ASSERT(derive_pos_match_secret(output) == EC_RES_SUCCESS);
...
test_static int
test_derive_pos_match_secret_failure_rand_finger_id_timeout(void)
{
	...
	fp_rand_finger_id(finger_id[templ_valid++]);
	TEST_ASSERT(fp_rand_finger_id(finger_id_buffer) == EC_RES_ERROR);
	/* THEN the derivation will fail. */
	TEST_ASSERT(derive_pos_match_secret(output_buffer) == EC_RES_ERROR);
	fp_clear_context();

	return EC_SUCCESS;
}

test_static int test_derive_pos_match_secret_failure_seed_not_set(void)
{
	static uint8_t output_buffer[FP_POS_MATCH_SECRET_BYTES];
...
test_static int test_derive_pos_match_secret_failure_rollback_fail(void)
{
	static uint8_t output_buffer[FP_POS_MATCH_SECRET_BYTES];
...
test_static int test_command_read_match_data(void)
{
	int rv;
	struct ec_params_fp_read_match_data params;
	struct ec_response_fp_read_match_data resp;
	uint8_t expected_finger_id[FP_FINGER_ID_BYTES];
	const uint8_t *expected_pos_match_salt = fake_rand_bytes;

	rand_bytes(expected_finger_id, FP_FINGER_ID_BYTES);

	fp_clear_context();
	TEST_ASSERT(derive_pos_match_secret(fp_pos_match_secret[0]) ==
		    EC_RES_SUCCESS);
	...
	params.fgr = 0;
	rv = test_send_host_command(EC_CMD_FP_READ_MATCH_DATA, 0, &params,
	...
	TEST_ASSERT_ARRAY_EQ(resp.pos_match_secret, expected_pos_match_secret,
			     FP_POS_MATCH_SECRET_BYTES);
	TEST_ASSERT_ARRAY_EQ(resp.finger_id, expected_finger_id,
			     FP_FINGER_ID_BYTES);
	TEST_ASSERT_ARRAY_EQ(resp.pos_match_salt, expected_pos_match_salt,
			     FP_POS_MATCH_SALT_BYTES);

	return EC_RES_SUCCESS;
}

test_static int test_command_read_match_data_again(void)
{
	int rv;
	struct ec_params_fp_read_match_data params;
...
void run_test(void)
{
	...
	RUN_TEST(test_derive_pos_match_secret_failure_seed_not_set);
	...
	RUN_TEST(test_derive_pos_match_secret);
	RUN_TEST(test_derive_pos_match_secret_failure_rand_finger_id_timeout);
	RUN_TEST(test_derive_pos_match_secret_failure_rollback_fail);
	RUN_TEST(test_command_read_match_data);
	RUN_TEST(test_command_read_match_data_again);
