...
static void reboot_to_clean_up(int passed)
{
	ccprintf("Rebooting to clear WP...\n");
	cflush();
	if (passed)
		system_set_scratchpad(TEST_STATE_CLEAN_UP | TEST_STATE_PASSED);
	else
		system_set_scratchpad(TEST_STATE_CLEAN_UP);
