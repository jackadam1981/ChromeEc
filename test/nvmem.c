...
static uint8_t write_buffer[CONFIG_FLASH_NVMEM_PARTITION_SIZE];
static uint8_t read_buffer[CONFIG_FLASH_NVMEM_PARTITION_SIZE];
static int flash_write_fail;


void nvmem_get_users(int32_t **p_user_buffers, int *p_num_users)
{
	*p_user_buffers = nvmem_buffer_tab;
	*p_num_users = NV_NUM_USERS;
}
...
static int generate_random_data(int offset, int num_bytes)
{
	...
	TEST_ASSERT((num_bytes + offset) <= CONFIG_FLASH_NVMEM_PARTITION_SIZE);
...
static int test_corrupt_nvmem(void)
{
	...
	for (n = 0; n < CONFIG_FLASH_NVMEM_NUM_PARTITIONS; n++) {
		offset = CONFIG_FLASH_NVMEM_PARTITION_SIZE * n;
...
static int test_write_read_sequence(void)
{
	...
	for (user = 0; user < NV_NUM_USERS; user++) {
		/* Length for each write/read segment */
		length = nvmem_buffer_tab[user] / WRITE_READ_SEGMENTS;
...
static int test_write_full_multi(void)
{
	 ...
	 * buffer.
	 */
	for (n = 0; n < NV_NUM_USERS; n++) {
		ret = write_full_buffer(nvmem_buffer_tab[n], n);
		if (ret != EC_SUCCESS)
			return ret;
	}
	return EC_SUCCESS;
...
static int test_write_fail(void)
{
	...
	if (test_write_read(offset, num_bytes, NV_USER_0))
		return EC_ERROR_UNKNOWN;

	/* Prevent flash erase/write operations */
	flash_write_fail = 1;
	/* Attempt flash write */
	ret = test_write_read(offset, num_bytes, NV_USER_0);
...
static int test_cache_not_available(void)
{
	...
	/* Do write/read sequence that's expected to be successful */
	if (test_write_read(offset, num_bytes, NV_USER_1))
		return EC_ERROR_UNKNOWN;
	...
	ret = test_write_read(offset, num_bytes, NV_USER_1);
	...
	return test_write_read(offset, num_bytes, NV_USER_1);
}
...
static int test_buffer_overflow(void)
{
	...
	for (n = 0; n < NV_NUM_USERS; n++) {
		...
		ret = write_full_buffer(nvmem_buffer_tab[n] + 1, n);
		if (!ret)
			return EC_ERROR_UNKNOWN;
	}

	/* Test case where user buffer number is valid */
	ret = test_write_read(0, 0x100, NV_USER_0);
	if (ret != EC_SUCCESS)
		return ret;
	/* Attempt same write, but with invalid user number */
	ret = test_write_read(0, 0x100, NV_NUM_USERS);
	if (!ret)
		return ret;
...
static int test_move(void)
{
	...
	nv1_offset = 0;
	nv2_offset = nvmem_buffer_tab[user];
	...
	nv1_offset = nvmem_buffer_tab[user];
	nv2_offset = 0;
	/* Attempt to move just 1 byte */
	ret = nvmem_move(nv1_offset, nv2_offset, 1, user);
	if (!ret)
	...
	nv1_offset = 0;
	nv2_offset = nvmem_buffer_tab[user] - len;
	...
	nv1_offset = 0;
	nv2_offset = nvmem_buffer_tab[user] - len + 1;
	ret = nvmem_move(nv1_offset, nv2_offset,
			 len, user);
	if (!ret)
		return EC_ERROR_UNKNOWN;
