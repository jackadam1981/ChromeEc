...
static void dump_test_result(const char *title, const struct nvmem_test_result *tr)
{
	ccprintf("\n%s:\n", title);
...
static void *page_to_flash_addr(int page_num)
{
	...
	if (page_num > NEW_NVMEM_PAGES_COUNT)
		return NULL;

	if (page_num >= (NEW_NVMEM_PAGES_COUNT/2)) {
		page_num -= (NEW_NVMEM_PAGES_COUNT/2);
...
static int post_inint_from_scratch(uint8_t flash_value)
{
	...
	TEST_ASSERT(test_result.valid_data_size == 1078);
	TEST_ASSERT(total_var_space == 0);

	for (i = 0; i < (NEW_NVMEM_PAGES_COUNT - 1); i++) {
...
ccprintf("%s:%d\n", __func__, __LINE__);
	...
	dump_test_result("after first save", &test_result);
	TEST_ASSERT(test_result.deleted_obj_count == 22);
	TEST_ASSERT(test_result.var_count == 0);
	TEST_ASSERT(test_result.reserved_obj_count == 36);
	TEST_ASSERT(test_result.evictable_obj_count == 9);
	TEST_ASSERT(test_result.unexpected_count == 0);
	TEST_ASSERT(test_result.valid_data_size == 5544);
	TEST_ASSERT(test_result.erased_data_size == 688);
...
ccprintf("%s:%d\n", __func__, __LINE__);
	...
	TEST_ASSERT(iterate_over_flash() == EC_SUCCESS);
	dump_test_result("test_result", &test_result);
	dump_test_result("old result", &old_result);
...
static int test_nvmem_compaction(void)
{
	...
	key_len = strlen(key);
	TEST_ASSERT(prepare_new_flash() == EC_SUCCESS);
	dump_test_result("post init", &test_result);
	//	old_result = test_result;

	/* it takes this many updates to fill up flash. */
	for (i = 0; i < 198; i++) {
	...
	iterate_over_flash();
	dump_test_result("pre compaction", &test_result);

	val_len = snprintf(value, sizeof(value), "variable value is %03d", i);
	TEST_ASSERT(setvar(key, key_len, value, val_len) == EC_SUCCESS);
	TEST_ASSERT(iterate_over_flash() == EC_SUCCESS);
	dump_test_result("post compaction", &test_result);
...
static int test_migration(void)
{
	...
	TEST_ASSERT(test_result.deleted_obj_count == 0);
	TEST_ASSERT(test_result.unexpected_count == 0);
	TEST_ASSERT(test_result.valid_data_size == 5630);
