...
static int iterate_over_flash(void)
{
	...
	while ((rv = get_next_object(&at, ch, 1))
	       == EC_SUCCESS)
	...
	if (at.mt.data_offset > sizeof(*at.mt.ph)) {
		...
		if ((at.dt.ph == list_element_to_ph(at.list_index)) &&
		    (at.dt.data_offset == (CONFIG_FLASH_BANK_SIZE -
	...
	ccprintf("%s:%d bad delimiter location: ph %p, "
		 ...
		 __func__, __LINE__,
		 at.mt.ph, at.dt.ph,
		 at.mt.data_offset, at.dt.data_offset);
...
static size_t get_free_nvmem_room(void)
{
	...
	ccprintf("list index %d, data offset 0x%x\n",master_at.list_index, master_at.mt.data_offset);
...
static size_t fill_obj_offsets(uint16_t *offsets, size_t max_objects)
{
	...
	for (i = 0; i < obj_count; i++) {
		...
		ccprintf("offs %04x:%08x:%08x:%08x addr %p size %d\n", offsets[i], op[-1],
			 op[0], op[1], op,
			 (uintptr_t)nvmem_cache_base(NVMEM_TPM) + op[-1] - (uintptr_t)op);
	}

	return obj_count;
}

static size_t fill_cache_offsets(const void *cache, uint16_t *offsets, size_t max_objects)
{
...
static uint32_t get_evict_size(const uint8_t *cache, uint16_t offset)
{
	...
	memcpy(&next_addr, cache + cache_offset - sizeof(next_addr), sizeof(next_addr));

	return next_addr - cache_offset;
}

/* Returns zero if the two objects are identical. */
static int compare_objects(const uint8_t *cache1, uint16_t offset1, const uint8_t *cache2, uint16_t offset2)
{
	uint32_t size1;
	uint32_t size2;
...
static int caches_match(const uint8_t *cache1, const uint8_t *cache2)
{
		...
		for (j = 0; j < ARRAY_SIZE(ranges); j++) {
			...
			for (k = offset; k < (offset + size); k++)
				if (cache1[k] != cache2[k])
					ccprintf(" %3d:%02x", k - offset, cache1[k]);
			ccprintf("\n");
			for (k = offset; k < (offset + size); k++)
				if (cache1[k] != cache2[k])
					ccprintf(" %3d:%02x", k - offset, cache2[k]);
			ccprintf("\n");
	...
	cache1_offs_count = fill_cache_offsets(cache1, cache1_offsets, ARRAY_SIZE(cache1_offsets));
	cache2_offs_count = fill_cache_offsets(cache2, cache2_offsets, ARRAY_SIZE(cache2_offsets));

	TEST_ASSERT(cache1_offs_count == cache2_offs_count);

	for (i = 0; (i < ARRAY_SIZE(cache1_offsets)) && cache2_offs_count; i++) {
		size_t j;

		for (j = 0; j < cache2_offs_count; j++) {
			if (compare_objects(cache1, cache1_offsets[i], cache2, cache2_offsets[j]))
				continue;
...
static int test_nvmem_incomplete_transaction(void)
{
	...
	TEST_ASSERT(caches_match(buf, nvmem_cache_base(NVMEM_TPM)) == EC_SUCCESS);
	drop_evictable_obj(evictable_offs_to_addr(offsets[4]));
	drop_evictable_obj(evictable_offs_to_addr(offsets[3]));
...
static int test_nvmem_interrupted_compaction(void)
{
	...
	do {
		/* A few reserved objects to modify to create need for compaction. */
		...
		for (i = 0; i < ARRAY_SIZE(objs_to_modify); i++) {
			...
			memset((uint8_t *)nvmem_cache_base(NVMEM_TPM) + ri.offset,
			       filler++, ri.size);
		}
		TEST_ASSERT(new_nvmem_save() == EC_SUCCESS);
...
int nvmem_first_task(void *unused) { return EC_SUCCESS; }

int nvmem_second_task(void *unused) { return EC_SUCCESS; }
...
static int verify_ram_index_space(size_t verify_size)
{
	 ...
	 */
	for (i = 0; i < RAM_INDEX_SPACE; i++) {
		nvmem_read(ri.offset + sizeof(casted_size) + i,
			   sizeof(byte),
			   &byte,
			   NVMEM_TPM);
		if (i < verify_size)
