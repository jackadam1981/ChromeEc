...
test_static int test_vsnprintf_pointers(void)
{
	...
	T(expect_success("0",         "%pb",     &val));
	val = 0x5E;
	T(expect_success("1011110",   "%pb",     &val));
	T(expect_success("0000000001011110", "%016pb", &val));
	val = 0x12345678;
	T(expect_success("10010001101000101011001111000", "%pb", &val));
	val = 0xFEDCBA90;
	/* Test a number that makes the longest string possible */
	T(expect_success("11111110110111001011101010010000", "%pb", &val));
	/* %lpb is not currently supported, as it makes intbuf bigger */
	T(expect(EC_ERROR_INVAL, NO_BYTES_TOUCHED,
		 false, -1, "%lpb", 0x1234ull));
	return EC_SUCCESS;
}
