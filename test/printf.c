...
int run(char *out, size_t out_size,
		int expect_ret, const char *expect,
		size_t size_limit, const char *format, va_list args)
{
	...
	TEST_ASSERT(expect_size <= out_size);
	TEST_ASSERT(expect_size <= size_limit);
	memset(out, INIT_VALUE, out_size);

	rv = vsnprintf(out, size_limit, format, args);
	ccprintf("received='%.*s'   | ret          =%d\n", MIN(out_size, 30), out, rv);

	TEST_ASSERT_ARRAY_EQ(out, expect, expect_size);
	TEST_ASSERT_MEMSET(&out[expect_size], INIT_VALUE, out_size-expect_size);
...
int expect_success(const char *expect, const char *format, ...)
{
	...
	rv = run(output, sizeof(output),
			EC_SUCCESS, expect,
			sizeof(output), format, args);
	va_end(args);
...
int expect(char *out, size_t out_size,
			int expect_ret, const char *expect,
...
test_static int test_vsnprintf_args(void)
{
	...
	T(expect(output, sizeof(output), EC_ERROR_INVAL, NO_BYTES_TOUCHED, 0, ""));
	T(expect(output, sizeof(output), EC_SUCCESS, "", 1, ""));
	T(expect(output, sizeof(output), EC_ERROR_OVERFLOW, "a", 2, "abc"));
	return EC_SUCCESS;
}
...
test_static int test_vsnprintf_strings(void)
{
	 ...
	 * Ignoring the padding parameter is slightly odd behavior
	 * and could use a review.
	 */
	 ...
	 * if we ask for zero precision, expect no bytes to be read from
	 * the malformed address and a blank output string.
	 */
	T(expect(output, sizeof(output),
			EC_SUCCESS, "", sizeof(output),
			"%.0s", (char *)1));
