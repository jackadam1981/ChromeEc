...
static int str_matches(const char *key, const char *expected_val)
{
	...
	return !memcmp(VAL(t), expected_val, expected_len);
}
...
static int weird_keys(void)
{
	...
	TEST_ASSERT(memcmp(VAL(t), valA, strlen(valA)) == 0);

	t = getvar(keyB, sizeof(keyB));
	TEST_ASSERT(t);
	TEST_ASSERT(t->val_len == strlen(valB));
	TEST_ASSERT(memcmp(VAL(t), valB, strlen(valB)) == 0);
...
static int weird_values(void)
{
	...
	TEST_ASSERT(t);
	TEST_ASSERT(memcmp(VAL(t), valA, sizeof(valA)) == 0);

	t = getvar(keyB, strlen(keyB));
	TEST_ASSERT(t);
	TEST_ASSERT(memcmp(VAL(t), valB, sizeof(valB)) == 0);
