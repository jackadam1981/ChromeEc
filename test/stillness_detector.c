...
static int test_not_still_short_window(void)
{
	...
	still_det_init(&det, 0.00025f, MS(800), MS(1200), 5);
	for (i = 0; i < 6; ++i)
		TEST_ASSERT(!still_det_update(&det, i * MS(100),
					      0.0f, 0.0f, 0.0f));
...
static int test_not_still_long_window(void)
{
	...
	still_det_init(&det, 0.00025f, MS(800), MS(1200), 5);
	for (i = 0; i < 5; ++i)
		TEST_ASSERT(!still_det_update(&det, i * MS(300),
					      0.0f, 0.0f, 0.0f));
...
static int test_not_still_not_enough_samples(void)
{
	...
	still_det_init(&det, 0.00025f, MS(800), MS(1200), 5);
	for (i = 0; i < 4; ++i)
		TEST_ASSERT(!still_det_update(&det, i * MS(200),
					      0.0f, 0.0f, 0.0f));
...
static int test_is_still_all_axes(void)
{
	...
	still_det_init(&det, 0.00025f, MS(800), MS(1200), 5);
	for (i = 0; i < 9; ++i) {
		int result = still_det_update(&det, i * MS(100),
					      i * 0.001f, i * 0.001f,
					      i * 0.001f);
