...
#define EC_EFS_BOOT_MODE_INITIAL EC_EFS_BOOT_MODE_RO
#endif
...
static int test_ec_comm_packet_failure(void)
{
	...
	TEST_ASSERT(ec_efs_get_boot_mode() == EC_EFS_BOOT_MODE_INITIAL);
...
static int test_ec_comm_set_boot_mode(void)
{
	...
	boot_mode_expected = EC_EFS_BOOT_MODE_INITIAL;
	TEST_ASSERT(ec_efs_get_boot_mode() == boot_mode_expected);

	/*
	 * Test 1-1: Attempt to set boot mode to EC_EFS_BOOT_MODE_INITIAL.
	 ...
	 *           RO -> VERIFIED (x) RO (o)
	 */
	boot_mode_expected = EC_EFS_BOOT_MODE_RO;
	 ...
	 */
	boot_mode_expected = EC_EFS_BOOT_MODE_INITIAL;
	pk.ph.data[0] = EC_EFS_BOOT_MODE_INITIAL;
	calculate_crc8(&pk);
...
static int test_ec_comm_verify_hash(void)
{
	 ...
	 */
#if CONFIG_EC_EFS2_VERSION == 0
	boot_mode_expected = EC_EFS_BOOT_MODE_NORMAL;
#elif CONFIG_EC_EFS2_VERSION == 1
