...
static int test_ec_comm_verify_efs2_1(void)
{
	 ...
	 *           Should fail, since VERIFIED is not allowd as a parameter
	 *           of SET_BOOT_MODE command.
	 *           VERIFIED -> VERIFIED
	 */
	...
	/*
	 * Test 2-1: Attempt to verify EC Hash while RO
	 *           RO -> VERIFIED
	 */
	TEST_ASSERT(!test_ec_comm(&pk_verify, preambles, CR50_COMM_SUCCESS));
	TEST_ASSERT(!ec_has_reset());
	...
	/*
	 * Test 2-2: Attempt to verify corrupted EC Hash while VERIFIED
	 *           VERIFIED -> NO_BOOT
	 */
	pk_verify.ph.data[0] ^= 0xff;	/* corrupt the payload */
	calculate_crc8(&pk_verify);
	TEST_ASSERT(!test_ec_comm(&pk_verify, preambles,
		CR50_COMM_ERROR_BAD_PAYLOAD));
	TEST_ASSERT(!ec_has_reset());
	TEST_ASSERT(ec_efs_get_boot_mode() == EC_EFS_BOOT_MODE_NO_BOOT);
