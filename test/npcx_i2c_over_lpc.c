...
int npcx_iol_host_int(void)
{
	...
	TEST_ASSERT((msg_from_host->semaphore &
		     (NPCX_IOL_CORE_SEND | NPCX_IOL_CORE_CONT)) ==
...
int setup_cont_pattern(int len)
{
	memset(msg_from_host->host_request_cont.write_buf, PATTERN,
	       MIN(len, LPC_HOST_MEM_WINDW_SIZE - 1));
	return 0;
}
...
struct inter host_cmd_transition[] = {
	...
	{ NPCX_IOL_CORE_CONT | NPCX_IOL_CORE_SEND,
		NPCX_IOL_HOST_ACK, NPCX_IOL_HOST_REQUEST, check_first_pattern,
		SMALL_PATTERN_LEN},
...
int setup_large_pattern(int len)
{
	...
	memset(msg_from_host->host_request.write_buf, PATTERN,
	       MIN(len, LPC_HOST_MEM_WINDW_SIZE - sizeof(*msg_from_host)));
	return 0;
}

struct inter host_multiple_cmd_transition[] = {
	{ 0, NPCX_IOL_HOST_REQUEST, 0, setup_large_pattern, 64 },
	{ NPCX_IOL_CORE_CONT, NPCX_IOL_HOST_ACK, NPCX_IOL_HOST_REQUEST, NULL, 0 },
	{ 0, 0, NPCX_IOL_HOST_ACK, setup_cont_pattern, 40 },
	{ NPCX_IOL_CORE_CONT, NPCX_IOL_HOST_ACK, 0, NULL, 0 },
	{ 0, 0, NPCX_IOL_HOST_ACK, setup_cont_pattern, 9 },
