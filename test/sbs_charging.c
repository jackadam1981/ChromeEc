...
test_static int test_charge_state(void)
{
	...
	TEST_ASSERT(state == LED_PWR_STATE_ERROR);
	...
	state = wait_charging_state();
	TEST_ASSERT(state == LED_PWR_STATE_CHARGE);
	...
	sb_write(SB_CURRENT, -1000);
	state = wait_charging_state();
	TEST_ASSERT(state == LED_PWR_STATE_DISCHARGE);
	...
	state = wait_charging_state();
	TEST_ASSERT(state == LED_PWR_STATE_FORCED_IDLE);
	flags = charge_get_flags();
	TEST_ASSERT(flags & CHARGE_FLAG_EXTERNAL_POWER);
	TEST_ASSERT(flags & CHARGE_FLAG_FORCE_IDLE);
	charge_control(CHARGE_CONTROL_NORMAL);
...
test_static int test_external_funcs(void)
{
	...
	/* and the rest */
	TEST_ASSERT(led_pwr_get_state() == LED_PWR_STATE_CHARGE);
