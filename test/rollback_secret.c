...
test_static int test_get_rollback_secret_latest_rollback_fail(void)
{
	...
	rv = MyFunction2(3);
	ccprints("The value of rv after function2 is: %d", rv);

	// it will generate an error for MyFunction3 undefined.
	rv = MyFunction3(3);
	ccprints("The value of rv after function3 is: %d", rv);
    ...
    ccprints("The value of rv after get_secret is: %d", rv);
...
test_static int test_get_rollback_secret_latest_rollback_secret_succeed(void)
{
	...
	for (int i = 0; i < 32; ++i) {
    	...
    	for (int i = 0; i < 32; ++i) {
		ccprints("The value of secret[%d] is: 0x%02x.", i,
			 secret[i]);
	}
