 ...
 * signature.
 */
_Static_assert(sizeof(struct combined_hdr_s) >=
...
_Static_assert(sizeof(struct dice_handover_s) -
...
static const struct dice_handover_hdr_s kDiceHandoverHdrTemplate = {
	...
	DICE_HANDOVER_LABEL_CDI_ATTEST, CBOR_BSTR32_EMPTY, /* CALC - CDI_attest
							    */
	/* 2. CDI_Seal: uint(2, 0bytes) => bstr(32bytes) */
	DICE_HANDOVER_LABEL_CDI_SEAL, CBOR_BSTR32_EMPTY, /* CALC - CDI_seal */
...
static const struct slice_ref_s kIdLabel = { 2, /* streln("ID") */
					     (const uint8_t *)"ID" };
static const struct slice_ref_s kKeyPairLabel = { 8, /* strelen("Key Pair") */
	...
	/* [OUT] inputs */ struct cdi_attest_inputs_s *inputs)
{
	...
	__platform_memcpy(inputs->code_digest, cwt_claims->code_hash.value,
			  DIGEST_BYTES);
	...
	/* [OUT] key handle */ const void **key)
{
	...
	return __platform_ecdsa_p256_keygen_hmac_sha512_opendice_drbg(drbg_seed,
								      key);
}
