...
/* Get (part of) BootParam structure for the modified DICE chain.
 * Use GSC CDI as UDS key for AP DICE.
 * For DEBUG purposes only.
 */
size_t get_boot_param_bytes_dbg(
	/* [OUT] destination buffer to fill */
	uint8_t *dest,
	/* [IN] starting offset in the BootParam struct */
	size_t offset,
...
)
{
	...
	struct boot_param_s *boot_param =
		(struct boot_param_s *)dest;
	struct dice_handover_nvmem_s *dice_handover =
		(struct dice_handover_nvmem_s *)
			&boot_param->dice_handover.options;
	...
	if (offset != 0 || size != BOOT_PARAM_SIZE)
		return 0;

	/* First get the UDS-signed AP BootParam with AP CDI cert */
	if (get_boot_param_bytes(dest, offset, size) != size)
		return 0;
