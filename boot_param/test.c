...
static void test_hkdf(void)
{
	...
	const struct slice_ref_s ikm = { 3, (const uint8_t *)"key" };
	const struct slice_ref_s salt = { 4, (const uint8_t *)"salt" };
	const struct slice_ref_s info = { 5, (const uint8_t *)"label" };
...
static void test_sha(void)
{
	...
	const struct slice_ref_s input = { 4, (const uint8_t *)"test" };
...
static bool save_to_file(const char *filename, uint8_t *buf, size_t size)
{
	FILE *f;
	size_t written;
...
static bool test_boot_param(
	const char *filename_handover, const char *filename_chain)
{
	...
	return save_to_file(filename_handover, boot_param, BOOT_PARAM_SIZE) &&
	       save_to_file(filename_chain, dice_chain, DICE_CHAIN_SIZE);
}
...
static const uint8_t kPcr0DebugMode[DIGEST_BYTES] = { 0x23, 0xE1, 0x4D, 0xD9,
	0xBB, 0x51, 0xA5, 0x0E, 0x16, 0x91, 0x1F, 0x7E, 0x11, 0xDF, 0x1E, 0x1A,
	0xAF, 0x0B, 0x17, 0x13, 0x4D, 0xC7, 0x39, 0xC5, 0x65, 0x36, 0x07, 0xA1,
	0xEC, 0x8D, 0xD3, 0x7A };
static const uint8_t kPcr0Zeroes[DIGEST_BYTES] = { 0x00, 0x00, 0x00, 0x00, 0x00,
