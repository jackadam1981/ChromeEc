...
void mutex_unlock_recursive(struct mutex_r *mtx)
{
	...
	/* Panic if not called by the owner owner */
	...
	if (mtx->count == 0) {
		/*
		 * Invalidate mutex owner, so current task won't be able to take
		 * the mutex before the new owner updates it.
		 */
		mtx->owner = -1;
		...
		if (atomic_exchange(&mtx->state, MUTEX_R_UNLOCKED) ==
			 ...
			 * Ping waiting tasks
			 *
			 * It's possible that will be no waiting tasks (because
			 * waiting bit could be cleared by task that got the
			 * mutex after unlocking it).
			 */
