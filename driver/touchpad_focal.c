...
#define GPIO_TOUCHPAD_INT  GPIO_SIGNAL(DT_PROP(DT_PROP(FTS_TP_NODE, irq), irq_pin))
...
//#define CONFIG_USB_HID_TOUCHPAD_LOGICAL_MAX_X DT_PROP(FTS_TP_NODE, logical_max_x)
//#define CONFIG_USB_HID_TOUCHPAD_LOGICAL_MAX_Y DT_PROP(FTS_TP_NODE, logical_max_y)
//#define CONFIG_USB_HID_TOUCHPAD_PHYSICAL_MAX_X DT_PROP(FTS_TP_NODE, physical_max_x)
//#define CONFIG_USB_HID_TOUCHPAD_PHYSICAL_MAX_Y DT_PROP(FTS_TP_NODE, physical_max_y)
...
//u8 fts_fw[] = {
//#include "FT3639_Lenovo_Dutt_V02_D01_20241021_app.i"
//};
...
* functions body
*****************************************************************************/
int fts_read(uint8_t *cmd, uint32_t cmdlen, uint8_t *data, uint32_t datalen)
{
    int ret = 0;
    int i = 0;

    for (i = 0; i < 3; i++) {
        ret = i2c_xfer(CONFIG_TOUCHPAD_I2C_PORT,
			CONFIG_TOUCHPAD_I2C_ADDR_FLAGS, cmd, cmdlen,
			data, datalen);
        if (ret == 0) break;
...
int fts_write(u8 *writebuf, u32 writelen)
{
    ...
    for (i = 0; i < 3; i++) {
        ret = i2c_xfer(CONFIG_TOUCHPAD_I2C_PORT,
			CONFIG_TOUCHPAD_I2C_ADDR_FLAGS, writebuf, writelen, NULL, 0);
        if (ret == 0) break;
...
static int fts_tp_read_report(void)
{
	...
	for (i = 0; i < 5; i++) {
        ...
        if (EVENT_DOWN(event_flag)) {
    		...
    		//report.finger[ri].width = touch_buf[7 + offset];
    		//report.finger[ri].height = touch_buf[7 + offset];
    		report.finger[ri].x = ((touch_buf[2 + offset] & 0x0F) << 8) \
                          + (touch_buf[3 + offset] & 0xFF);
    		report.finger[ri].y =((touch_buf[4 + offset] & 0x0F) << 8) \
...
static int fts_read_chip_id(void) 
{
    ...
    for (i = 0; i < 5; i++) {
        ...
        if (ret < 0) {
            CPRINTS("fts_read_reg fail ret = %x\n", ret);
        }
...
/*test_mockable_static const uint8_t fts_fw_hash[SHA256_DIGEST_SIZE] = {
...
int touchpad_get_info(struct touchpad_info *tp)
{
    ...
    //tp->allowed_fw_hash = fts_fw_hash;
    //memcpy(tp->allowed_fw_hash, fts_fw_hash, SHA256_DIGEST_SIZE);
    //tp->fw_size = 256 * 1024;
    if (!fts_fw_is_valid()) {
         ...
         * values.
         */
        tp->fts.id = (FTS_CHIP_ID_H << 8) + FTS_CHIP_ID_L;
        tp->fts.fw_version = 0;
    ...
    int retries_delay)
{
    ...
    CPRINTS("flash status fail,ok:%04x,read:%04x,retries:%d",
 ...
 ***********************************************************************/
int fts_fwupg_erase(uint32_t delay)
{
    int ret = 0;
    uint8_t cmd = 0;
    bool flag = false;
...
*****************************************************************************/
/************************************************************************
* Name: fts_ft5z8b_crc16_calc_host
* Brief:
* Input:
* Output:
* Return: return ecc
***********************************************************************/
static u16 fts_ft5z8b_crc16_calc_host(u16 ecc_pre, const u8 *pbuf, u32 length)
{
    u16 ecc = ecc_pre;
    u32 i = 0;
    u32 j = 0;
 ...
 ***********************************************************************/
static int fts_flash_write_buf(u32 saddr, const u8 *buf, u32 len, u32 delay, u16 ecc_pre)
{
    int ret = 0;
    u32 i = 0;
    u32 j = 0;
    ...
    u32 remainder = 0;
    u32 cmdlen = 0;
    u8 packet_buf[FTS_FLASH_PACKET_SIZE + 6] = { 0 };
    int ecc_in_host = 0;
    u8 cmd = 0;
    u8 val[2] = { 0 };
    u16 read_status = 0;
    ...
    ecc_in_host = (int)fts_ft5z8b_crc16_calc_host(ecc_pre,buf, len);
    return ecc_in_host;
}
...
int touchpad_update_write(int offset, int size, const uint8_t *data)
{
    ...
    start_addr = offset;
    /* write app */
    delay = 1;//(FTS_FLASH_PACKET_SIZE / FTS_SIZE_PAGE) * 2;
    //crec_msleep(5000);
    //return EC_SUCCESS;
    ecc_in_host = fts_flash_write_buf(start_addr, data, size, delay, ecc_in_host);
    if (ecc_in_host < 0 ) {
...
void touchpad_task(void *u)
{
	...
	while (1) {
       ...
       // if (event & TASK_EVENT_TP_UPDATED)
			//touchpad_update_write(0, sizeof(fts_fw), fts_fw);
	}
}
...
/* Debugging commands */
static int fts_command_touchpad(int argc, const char **argv)
{
    u8 val = 0;
	if (argc < 2)
...
DECLARE_CONSOLE_COMMAND(fts_touchpad, fts_command_touchpad,
