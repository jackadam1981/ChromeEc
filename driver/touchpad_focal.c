...
u32 saddr, const u8 *buf, u32 len,
u32 delay, u8 ecc_pre)
{
    int ret = 0;
    u32 i = 0;
    u32 j = 0;
...
int touchpad_update_write(int offset, int size, const uint8_t *data)
{
    ...
    ecc_in_host = fts_flash_write_buf(
        start_addr, data, size, delay, ecc_in_host);
    if (ecc_in_host < 0 ) {
