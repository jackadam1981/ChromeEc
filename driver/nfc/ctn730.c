...
static const int _message_buffer_size = 0x20;
...
{
	uint8_t buf[_message_buffer_size];
	...
	rv = _i2c_write(ctx->cfg->i2c_port, buf, sizeof(*cmd) + cmd->length);
	if (rv)
		return rv;
...
static int _process_payload_event(struct pchg *ctx, struct ctn730_msg *res)
{
	...
	case WLC_CHG_CTRL_CHARGING_STATE:
		...
		case WLC_CHG_CTRL_CHARGING_STATE_CHARGE_STOPPED:
			/* This includes over temperature, DISABLE_CMD, and
			 * device removal. */
...
static int cc_ctn730(int argc, char **argv)
{
	...
	if (!strcasecmp(argv[2], "dump")) {
		...
		_i2c_write(cfg->i2c_port, buf, sizeof(*cmd) + cmd->length);
