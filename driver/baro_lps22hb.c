 ...
 * For more details on LPS22HB device please refers to www.st.com.
 */
...
static int lps22hb_read(const struct motion_sensor_t *s, intv3_t v)
{
	uint8_t raw[3];
	int ret;
	struct stprivate_data *data = s->drv_data;
	...
	ret = st_raw_read_n(s->port, s->i2c_spi_addr_flags,
			    LPS22HB_REG_PRESS_OUT_XL, raw, 3);
	if (ret)
		return ret;
...
static int lps22hb_init(const struct motion_sensor_t *s)
{
	...
	if (st_raw_write8(s->port, s->i2c_spi_addr_flags, LPS22HB_REG_CTRL_REG2,
		       LPS22HB_SWRESET | LPS22HB_IF_ADD_INC) != EC_SUCCESS) {
		ret = EC_ERROR_UNKNOWN;
		goto err_unlock;
	}

	/* Must wait at least 50 us as reported in AN5209. */
	msleep(1);
