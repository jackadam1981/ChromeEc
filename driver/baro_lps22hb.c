				 ...
				 int rate, int rnd)
{
	...
	if (st_write_data_with_mask(s, LPS22HB_REG_CTRL_REG1,
				    LPS22HB_MASK_ODR, reg_val) != EC_SUCCESS)
 ...
 */
static int lps22hb_set_range(const struct motion_sensor_t *s, int range, int rnd)
{
	return EC_SUCCESS;
}
...
static int lps22hb_read(const struct motion_sensor_t *s, vector_3_t v)
{
	...
	if (st_raw_read_n(s->port, s->addr,
...
static int lps22hb_init(const struct motion_sensor_t *s)
{
	...
	data->resol = LPS22HB_P_RESOLUTION;
	sensor_init_done(s, lps22hb_get_range(s));
