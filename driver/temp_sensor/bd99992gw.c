...
{
	int ret;
	ret = i2c_read8(I2C_PORT_PMIC, BD99992GW_I2C_ADDR, offset, data_ptr);
	if (ret != EC_SUCCESS)
...
{
	int ret;
	ret = i2c_write8(I2C_PORT_PMIC, BD99992GW_I2C_ADDR, offset, data);
	if (ret != EC_SUCCESS)
...
static void bd99992gw_init(void)
{
	...
	raw_write8(BD99992GW_REG_ADC1CNTL1, BD99992GW_ADC1CNTL1_SLP27MS |
		   BD99992GW_ADC1CNTL1_ADEN | BD99992GW_ADC1CNTL1_ADSTRT);
}
/*
 * Some regs only work in S0, so we must initialize on AP startup, not
 * EC startup.
 */
...
int bd99992gw_get_val(int idx, int *temp_ptr)
{
	...
	*temp_ptr = C_TO_K(bd99992gw_get_temp(adc));
	...
	/* Resume conversions */
	ret = raw_write8(BD99992GW_REG_ADC1CNTL1, BD99992GW_ADC1CNTL1_SLP27MS |
		   BD99992GW_ADC1CNTL1_ADEN | BD99992GW_ADC1CNTL1_ADSTRT);
	if (ret)
		goto bd99992gw_get_val_fail;

	return EC_SUCCESS;
bd99992gw_get_val_fail:
	return ret;
}
