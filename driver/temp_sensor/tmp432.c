...
static int tmp432_temp_val_local;
static int tmp432_temp_val_remote1;
static int tmp432_temp_val_remote2;
 ...
 */
static int tmp432_has_power(void)
{
...
static int tmp432_read8(const int offset, int *data_ptr)
{
	return i2c_read8(I2C_PORT_THERMAL, TMP432_I2C_ADDR, offset, data_ptr);
}

static int tmp432_write8(const int offset, int data)
{
	return i2c_write8(I2C_PORT_THERMAL, TMP432_I2C_ADDR, offset, data);
}

static int tmp432_get_temp(const int offset, int *temp_ptr)
{
	int rv;
	int temp_raw = 0;

	rv = tmp432_read8(offset, &temp_raw);
	if (rv < 0)
		return rv;
...
static int tmp432_set_temp(const int offset, int temp)
{
	...
	return tmp432_write8(offset, (uint8_t)temp);
}
...
int tmp432_get_val(int idx, int *temp_ptr)
{
	...
	case TMP432_IDX_LOCAL:
		*temp_ptr = tmp432_temp_val_local;
		break;
	case TMP432_IDX_REMOTE1:
		*temp_ptr = tmp432_temp_val_remote1;
		break;
	case TMP432_IDX_REMOTE2:
		*temp_ptr = tmp432_temp_val_remote2;
		break;
	default:
...
static void tmp432_temp_sensor_poll(void)
{
	int temp_c;
	...
	if (tmp432_get_temp(TMP432_LOCAL, &temp_c) == EC_SUCCESS)
		tmp432_temp_val_local = C_TO_K(temp_c);

	if (tmp432_get_temp(TMP432_REMOTE1, &temp_c) == EC_SUCCESS)
		tmp432_temp_val_remote1 = C_TO_K(temp_c);

	if (tmp432_get_temp(TMP432_REMOTE2, &temp_c) == EC_SUCCESS)
		tmp432_temp_val_remote2 = C_TO_K(temp_c);
}
DECLARE_HOOK(HOOK_SECOND, tmp432_temp_sensor_poll, HOOK_PRIO_TEMP_SENSOR);

static void tmp432_show_temps(
		const char *name,
		...
		const int tmp432_low_limit_reg)
{
	...
	if (tmp432_get_temp(tmp432_temp_reg, &value) == EC_SUCCESS)
		ccprintf("  Temp       %3dC\n", value);

	if (tmp432_get_temp(tmp432_therm_limit_reg, &value) == EC_SUCCESS)
		ccprintf("  Therm Trip %3dC\n", value);

	if (tmp432_get_temp(tmp432_high_limit_reg, &value) == EC_SUCCESS)
		ccprintf("  High Alarm %3dC\n", value);

	if (tmp432_get_temp(tmp432_low_limit_reg, &value) == EC_SUCCESS)
		ccprintf("  Low Alarm  %3dC\n", value);
}

static int tmp432_show_status(void)
{
	int value;

	tmp432_show_temps("Local", TMP432_LOCAL,
			  TMP432_LOCAL_THERM_LIMIT,
			  TMP432_LOCAL_HIGH_LIMIT_R,
			  TMP432_LOCAL_LOW_LIMIT_R);

	tmp432_show_temps("Remote1", TMP432_REMOTE1,
			  TMP432_REMOTE1_THERM_LIMIT,
			  TMP432_REMOTE1_HIGH_LIMIT_R,
			  TMP432_REMOTE1_LOW_LIMIT_R);

	tmp432_show_temps("Remote2", TMP432_REMOTE2,
	...
	if (tmp432_read8(TMP432_STATUS, &value) == EC_SUCCESS)
		ccprintf("STATUS:  %08b\n", value);

	if (tmp432_read8(TMP432_CONFIGURATION1_R, &value) == EC_SUCCESS)
		ccprintf("CONFIG1: %08b\n", value);

	if (tmp432_read8(TMP432_CONFIGURATION2_R, &value) == EC_SUCCESS)
