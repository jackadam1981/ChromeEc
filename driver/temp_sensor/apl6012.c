...
static void apl6012_sensor_poll(void)
{
	...
	for (index = 0; index < APL6012_IDX_COUNT; index++) {
		rv = get_ch_val(index, (APL6012_TD1 + index), &ch_val);
		if (rv != 0)
			continue;
		...
		temp_c = thermistor_linear_interpolate(
			mv, &apl6012_thermistor_info[index]);
...
DECLARE_CONSOLE_COMMAND(
	apl6012, apl6012_debug, "<index> <mV value>",
	"Set a channel fake mV to get temperature C of sensor apl6012.");
#endif /* CONFIG_ZEPHYR */
