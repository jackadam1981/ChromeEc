...
#ifdef CONFIG_TEMP_SENSOR_HW_INIT
static int status_read8(int *data_ptr)
{
	return i2c_addr_read8(I2C_PORT_THERMAL, G781_I2C_ALERT_ADDR, data_ptr);
}
#endif
...
static void temp_sensor_g781_int_init(void)
{
#ifdef CONFIG_TEMP_SENSOR_HW_INIT
	gpio_enable_interrupt(GPIO_FAN_ALERT_L);
#endif
}
DECLARE_HOOK(HOOK_INIT, temp_sensor_g781_int_init, HOOK_PRIO_TEMP_SENSOR);
...
void temp_sensor_g781_register_init(void)
{
#ifdef CONFIG_TEMP_SENSOR_HW_INIT
	int alert_status, rv;
	if (!g781_reg_init)
