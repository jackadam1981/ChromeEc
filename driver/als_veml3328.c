...
static int veml3328_read(const struct motion_sensor_t *s, intv3_t v)
{
	...
	RETURN_ERROR(i2c_read16(port, addr, VEML3328_REG_C, (int *)&c));
	RETURN_ERROR(i2c_read16(port, addr, VEML3328_REG_R, (int *)&r));
	RETURN_ERROR(i2c_read16(port, addr, VEML3328_REG_G, (int *)&g));
	RETURN_ERROR(i2c_read16(port, addr, VEML3328_REG_B, (int *)&b));
	RETURN_ERROR(i2c_read16(port, addr, VEML3328_REG_IR, (int *)&ir));
	...
	v[0] = FP_TO_INT(calib->per_system.C_lux * Ch * Cl *
...
static int veml3328_get_scale(const struct motion_sensor_t *s, uint16_t *scale,
			      int16_t *temp)
{
	scale[X] = VEML3328_GET_DATA(s)->scale;
	scale[Y] = 0;
	scale[Z] = 0;
			      ...
			      const uint16_t *scale, int16_t temp)
{
	...
	VEML3328_GET_DATA(s)->scale = scale[X];
	return EC_SUCCESS;
}
...
{
	VEML3328_GET_DATA(s)->calibration_mode = enable;
	return EC_SUCCESS;
}
