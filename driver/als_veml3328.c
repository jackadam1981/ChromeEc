...
static int veml3328_read_lux(const struct motion_sensor_t *s,
			     struct veml3328_calib *calib, int *lux)
{
	int ret;
	fp_t CCTi, CCTi0;
	int raw_data[5];
	int IT_idx, DG_idx, gain_idx, SENS_idx;
	fp_t c, r, g, b, ir, lux_gain, Ch, Cl;
	 ...
	 */
	fp_t it[] = { 0.5, 1.0, 2.0, 4.0 };
	fp_t dg[] = { 1.0, 2.0, 4.0, 1.0 };
	fp_t gain[] = { 1.0, 2.0, 4.0, 0.5 };
	fp_t sens[] = { 1.0, 1 / 3.0f };
	...
	CCTi0 = calib->per_model.Lccti0 * (fp_t)(r + g - b) / (fp_t)c +
		calib->per_model.Lccti1 * (fp_t)(r + g) / (fp_t)b;
	...
	/* Calculate lux gain */
	IT_idx = (veml3328_conf & VEML3328_IT_MASK) >> VEML3328_IT_SHIFT;
	DG_idx = (veml3328_conf & VEML3328_DG_MASK) >> VEML3328_DG_SHIFT;
	gain_idx = (veml3328_conf & VEML3328_GAIN_MASK) >> VEML3328_GAIN_SHIFT;
	SENS_idx = (veml3328_conf & VEML3328_HD_MASK) >> VEML3328_HD_SHIFT;
	lux_gain = it[IT_idx] * dg[DG_idx] * gain[gain_idx] * sens[SENS_idx];
	...
	*lux = (int)(calib->per_system.C_lux * Ch * Cl *
		     (calib->per_model.LG * g + calib->per_model.LC * c) /
		     lux_gain);
