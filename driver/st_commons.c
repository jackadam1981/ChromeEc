...
/**
 * st_dbg_sensor - Set sensors debug level
 * command: accdbg <0,1> - 0 Disable, 1 Enable
 */
static int st_dbg_sensor(int argc, char **argv)
{
	char *e;

	if (argc == 2)
		debug = strtoi(argv[1], &e, 0);

	return EC_SUCCESS;
}
DECLARE_CONSOLE_COMMAND(accdbg, st_dbg_sensor,
			"dbg lvl",
			"Set sensors debug level [0:1]");
 ...
 * @data: LSB raw data
 */
void normalize(const struct motion_sensor_t *s, vector_3_t axis, uint8_t *data)
{
	int i;
	...
	for (i = 0; i < 3; i++) {
		...
		case 16:
			...
			axis[i] = (axis[i] * drvdata->base.range) / 1000;
			break;
		}
	}
