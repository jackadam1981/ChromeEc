...
/**
 * BMI220 accelerometer and gyro module for Chrome EC
...
#include "accelgyro_bmi220.h"
#include "third_party/bmi220/accelgyro_bmi220_config_tbin.h"
#include "hwtimer.h"
#include "i2c.h"
#include "init_rom.h"
#include "math_util.h"
 ...
 * };
 */
#if DT_NODE_EXISTS(DT_ALIAS(bmi220_int))
#define CONFIG_ACCELGYRO_BMI220_INT_EVENT	\
	TASK_EVENT_MOTION_SENSOR_INTERRUPT(SENSOR_ID(DT_ALIAS(bmi220_int)))
#endif
#endif
...
{
	int ret;
	ret = bmi_enable_reg8(s, BMI220_PWR_CTRL,
			      BMI220_PWR_EN(s->type),
			      enable);
	if (ret)
		return ret;
			...
			int16_t    temp)
{
	...
	ret = bmi_read8(s->port, s->i2c_spi_addr_flags,
			BMI220_OFFSET_EN_GYR98, &val98);
	if (ret)
		return ret;
	ret = bmi_read8(s->port, s->i2c_spi_addr_flags,
			BMI220_NV_CONF, &val_nv_conf);
	if (ret)
		return ret;
	...
	case MOTIONSENSE_TYPE_ACCEL:
		...
		ret = bmi_write8(s->port, s->i2c_spi_addr_flags,
				 BMI220_NV_CONF,
				 val_nv_conf | BMI220_ACC_OFFSET_EN);
		break;
	case MOTIONSENSE_TYPE_GYRO:
		bmi_set_gyro_offset(s, v, &val98);
		ret = bmi_write8(s->port, s->i2c_spi_addr_flags,
				 BMI220_OFFSET_EN_GYR98,
				 val98 | BMI220_OFFSET_GYRO_EN);
		break;
	default:
			      ...
			      intv3_t v, int try_cnt, int msec)
{
	...
	while (try_cnt && !(status & BMI220_DRDY_ACC)) {
		msleep(msec);
		ret = bmi_read8(s->port, s->i2c_spi_addr_flags,
				BMI220_STATUS, &status);
		if (ret)
			return ret;
		try_cnt -= 1;
	}
	if (!(status & BMI220_DRDY_ACC))
...
static int perform_calib(struct motion_sensor_t *s, int enable)
{
	...
	switch (s->type) {
	case MOTIONSENSE_TYPE_ACCEL:
		target[Z] = BMI220_ACC_DATA_PLUS_1G(range);
		break;
	case MOTIONSENSE_TYPE_GYRO:
		break;
	default:
...
/**
 * bmi220_interrupt - called when the sensor activates the interrupt line.
 *
 * This is a "top half" interrupt handler, it just asks motion sense ask
 * to schedule the "bottom half", ->irq_handler().
 */
void bmi220_interrupt(enum gpio_signal signal)
{
	if (IS_ENABLED(CONFIG_ACCEL_FIFO))
		last_interrupt_timestamp = __hw_clock_source_read();

	task_set_event(TASK_ID_MOTIONSENSE, CONFIG_ACCELGYRO_BMI220_INT_EVENT);
}
...
static int config_interrupt(const struct motion_sensor_t *s)
{
	...
	bmi_write8(s->port, s->i2c_spi_addr_flags,
		   BMI220_CMD_REG, BMI220_CMD_FIFO_FLUSH);

	/* configure int1 as an interrupt */
	ret = bmi_write8(s->port, s->i2c_spi_addr_flags,
			 BMI220_INT1_IO_CTRL,
			 BMI220_INT1_OUTPUT_EN);
	if (IS_ENABLED(CONFIG_ACCELGYRO_BMI220_INT2_OUTPUT))
		/* TODO(chingkang): Test it if we want int2 as an interrupt */
		/* configure int2 as an interrupt */
		ret = bmi_write8(s->port, s->i2c_spi_addr_flags,
				BMI220_INT2_IO_CTRL,
				BMI220_INT2_OUTPUT_EN);
	else
		/* configure int2 as an external input. */
		ret = bmi_write8(s->port, s->i2c_spi_addr_flags,
				BMI220_INT2_IO_CTRL,
				BMI220_INT2_INPUT_EN);

	if (IS_ENABLED(CONFIG_ACCEL_FIFO)) {
		/* map fifo water mark to int 1 */
		ret = bmi_write8(s->port, s->i2c_spi_addr_flags,
				 BMI220_INT_MAP_DATA,
				 BMI220_INT_MAP_DATA_REG(1, FWM) |
				 BMI220_INT_MAP_DATA_REG(1, FFULL));
		...
		if (IS_ENABLED(CONFIG_ACCELGYRO_BMI220_INT2_OUTPUT))
			ret = bmi_write8(s->port, s->i2c_spi_addr_flags,
					BMI220_FIFO_CONFIG_1,
					BMI220_FIFO_HEADER_EN);
		else
			ret = bmi_write8(s->port, s->i2c_spi_addr_flags,
					BMI220_FIFO_CONFIG_1,
					(BMI220_FIFO_TAG_INT_LEVEL <<
					 BMI220_FIFO_TAG_INT2_EN_OFFSET) |
					BMI220_FIFO_HEADER_EN);
		/* disable FIFO sensortime frame */
		ret = bmi_write8(s->port, s->i2c_spi_addr_flags,
				 BMI220_FIFO_CONFIG_0, 0);
	}
	mutex_unlock(s->mutex);
	return ret;
}
...
static int irq_handler(struct motion_sensor_t *s, uint32_t *event)
{
	...
	if ((s->type != MOTIONSENSE_TYPE_ACCEL) ||
			(!(*event & CONFIG_ACCELGYRO_BMI220_INT_EVENT)))
		return EC_ERROR_NOT_HANDLED;

	do {
		rv = bmi_read16(s->port, s->i2c_spi_addr_flags,
				BMI220_INT_STATUS_0, &interrupt);
		/*
		...
		if (IS_ENABLED(CONFIG_ACCEL_FIFO) &&
			interrupt & (BMI220_FWM_INT | BMI220_FFULL_INT)) {
...
static int bmi_config_load(const struct motion_sensor_t *s)
{
	...
	for (i = 0; i < g_bmi220_config_tbin_len; i += burst_write_len) {
		uint8_t addr[2];
		const int len = MIN(burst_write_len,
				    g_bmi220_config_tbin_len - i);

		addr[0] = (i / 2) & 0xF;
		addr[1] = (i / 2) >> 4;
		ret = bmi_write_n(s->port, s->i2c_spi_addr_flags,
				  BMI220_INIT_ADDR_0, addr, 2);
		if (ret)
			break;
...
static int init_config(const struct motion_sensor_t *s)
{
	...
	bmi_write8(s->port, s->i2c_spi_addr_flags, BMI220_PWR_CONF, 2);
	msleep(1);
	/* prepare for config load */
	bmi_write8(s->port, s->i2c_spi_addr_flags, BMI220_INIT_CTRL, 0);

	/* load config file to INIT_DATA */
	ret = bmi_config_load(s);
	/* finish config load */
	bmi_write8(s->port, s->i2c_spi_addr_flags, BMI220_INIT_CTRL, 1);
	...
	for (i = 0; i < 15; ++i) {
		msleep(10);
		ret = bmi_read8(s->port, s->i2c_spi_addr_flags,
			BMI220_INTERNAL_STATUS, &init_status);
		if (ret)
			break;
		init_status &= BMI220_MESSAGE_MASK;
		if (init_status == BMI220_INIT_OK)
			break;
	}
	if (ret || init_status != BMI220_INIT_OK)
...
static int init(struct motion_sensor_t *s)
{
	...
	if (s->type == MOTIONSENSE_TYPE_ACCEL) {
		...
		bmi_write8(s->port, s->i2c_spi_addr_flags,
				 BMI220_CMD_REG, BMI220_CMD_SOFT_RESET);
		msleep(2);
		if (init_config(s))
			return EC_ERROR_INVALID_CONFIG;
...
const struct accelgyro_drv bmi220_drv = {
	.init = init,
	.read = bmi_read,
...
struct i2c_stress_test_dev bmi220_i2c_stress_test_dev = {
	.reg_info = {
		.read_reg = BMI220_CHIP_ID,
		.read_val = BMI220_CHIP_ID_MAJOR,
		.write_reg = BMI220_PMU_TRIGGER,
	},
	.i2c_read = &bmi_read8,
