...
static void received_byte(int port, uint8_t data)
{
	...
	if (cec_rx.transfer.byte >= MAX_CEC_MSG_LEN) {
		CPRINTF("CEC error: received message > %d bytes\n",
			MAX_CEC_MSG_LEN);
		enter_idle_state();
	}
}
...
static void cec_event_dbd(int port)
{
	...
	case CEC_STATE_FREE_TIME:
		 ...
		 */
		CPRINTF("CEC error: DBD in invalid state %d\n", cec_state);
		enter_idle_state();
		break;
	case CEC_STATE_TRANSMITTING:
...
static void cec_event_hdrcv(int port)
{
	...
	case CEC_STATE_RECEIVING:
		 ...
		 */
		CPRINTF("CEC error: HDRCV in invalid state %d\n", cec_state);
		enter_idle_state();
		break;
	}
}
...
static void cec_event_transmit(int port)
{
	...
	case CEC_STATE_TRANSMITTING:
		 ...
		 */
		CPRINTF("CEC error: transmit event in invalid state %d\n",
			cec_state);
		enter_idle_state();
		break;
	case CEC_STATE_RECEIVING:
...
static void cec_event_free_time_complete(int port)
{
	...
	case CEC_STATE_IDLE:
		...
		CPRINTF("CEC error: free time complete in invalid state %d\n",
			cec_state);
		enter_idle_state();
		break;
	case CEC_STATE_FREE_TIME:
...
static int it83xx_cec_set_enable(int port, uint8_t enable)
{
	...
	if (enable) {
		...
		/* Set logical address to unregisterd (default is 0 = TV) */
