...
uint8_t cec_event_cap_record[4096] __keep;
uint8_t cec_enter_state_record[4096] __keep;
uint8_t cec_event_timeout_record[4096] __keep;
...
static void enter_state(int port, enum cec_state new_state)
{
	...
	if (cec_index_test != 4096) {
		cec_enter_state_record[cec_index_test] = port_data->state;
	}
...
void cec_event_timeout(int port)
{
	...
	if (cec_index_test != 4096) {
		cec_event_timeout_record[cec_index_test] = port_data->state;
	}
...
void cec_event_cap(int port)
{
	...
	if (cec_index_test != 4096) {
		cec_event_cap_record[cec_index_test++] = port_data->state;
	}
