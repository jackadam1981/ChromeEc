...
#define IT81XX2_GPIO_GPDRA       ECREG(EC_REG_BASE_ADDR + 0x1608)      // GPIO_H Data Register
#define IT81XX2_GPIO_HIGH_H4     (IT81XX2_GPIO_GPDRA | (uint8_t)0x10)  // GPIO_H4 拉 High
#define IT81XX2_GPIO_LOW_H4      (IT81XX2_GPIO_GPDRA & (uint8_t)~0x10) // GPIO_H4 拉 Low
#define IT81XX2_GPIO_REVERSE_H4  (IT81XX2_GPIO_GPDRA ^ (uint8_t)0x10)  // GPIO_H4 反向
...
#if 1 // ITE Debug
#define START_BIT_LATENCY CEC_US_TO_TICKS(100)
#define SOC_INITIATOR_LATENCY CEC_US_TO_TICKS(100)
#endif
...
__soc_ram_code static void enter_state(int port, enum cec_state new_state)
{
...
#if 1 // ITE Debug
		timeout = START_BIT_LOW_TICKS - SOC_INITIATOR_LATENCY; // 3.7 - 0.1 = 3.6ms
#else
	...
	case CEC_STATE_INITIATOR_START_HIGH:
		...
		cap_edge = CEC_CAP_EDGE_FALLING;
#if 1 // ITE Debug
		timeout = START_BIT_HIGH_TICKS - SOC_INITIATOR_LATENCY; // 0.8ms - 0.1 = 0.7ms
#else
...
#if 1 // ITE Debug
		timeout =
			DATA_LOW(cec_transfer_get_bit(&port_data->tx.transfer)) - SOC_INITIATOR_LATENCY; // 0.6 - 0.1ms or 1.5 - 0.1ms
#else
		timeout =
...
#if 1 // ITE Debug
		timeout = DATA_HIGH(
			cec_transfer_get_bit(&port_data->tx.transfer)) - SOC_INITIATOR_LATENCY; // 1.8 - 0.1ms or 0.9 - 0.1ms
#else
		timeout = DATA_HIGH(
	...
	case CEC_STATE_INITIATOR_EOM_LOW:
		...
		timeout = DATA_LOW(cec_transfer_is_eom(&port_data->tx.transfer,
						       port_data->tx.len)) - SOC_INITIATOR_LATENCY; // 0.6 - 0.1ms or 1.5 - 0.1ms
#else
		timeout = DATA_LOW(cec_transfer_is_eom(&port_data->tx.transfer,
	...
	case CEC_STATE_INITIATOR_EOM_HIGH:
		...
		timeout = DATA_HIGH(cec_transfer_is_eom(&port_data->tx.transfer,
							port_data->tx.len)) - SOC_INITIATOR_LATENCY; // 1.8 - 0.1ms or 0.9 - 0.1ms
#else
		timeout = DATA_HIGH(cec_transfer_is_eom(&port_data->tx.transfer,
	...
	case CEC_STATE_INITIATOR_ACK_LOW:
		gpio = 0;
#if 1 // ITE Debug
		timeout = DATA_LOW(1) - SOC_INITIATOR_LATENCY; // 0.6 - 0.1ms
#else
	...
	case CEC_STATE_INITIATOR_ACK_HIGH:
		...
		timeout = (DATA_ONE_LOW_TICKS + DATA_ZERO_LOW_TICKS) / 2 -
			  DATA_ONE_LOW_TICKS - SOC_INITIATOR_LATENCY;
#else
		timeout = (DATA_ONE_LOW_TICKS + DATA_ZERO_LOW_TICKS) / 2 -
			  DATA_ONE_LOW_TICKS;
#endif
	...
	case CEC_STATE_FOLLOWER_ACK_LOW:
		 ...
		 */
		timeout = NOMINAL_SAMPLE_TIME_TICKS - SOC_CEC_ISR_LATENCY; // 1.05ms - 0.2ms = 8.5ms
		break;
	case CEC_STATE_FOLLOWER_ACK_VERIFY:
		 ...
		 */
		timeout = DATA_ZERO_LOW_TICKS - NOMINAL_SAMPLE_TIME_TICKS - SOC_CEC_ISR_LATENCY; // 1.5 - 1.05 - 0.15 = 0.3ms
		break;
...
static int bitbang_cec_init(int port)
{
	...
	// GPIO_H4 拉 HIGH
	IT81XX2_GPIO_GPDRA = IT81XX2_GPIO_HIGH_H4;
