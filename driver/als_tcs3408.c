...
{
 	sat_p->atime = MIN(sat_p->atime + TCS_ATIME_DEC_STEP, TCS_MAX_ATIME);
}
...
{
	sat_p->atime = MAX(sat_p->atime - TCS_ATIME_INC_STEP, TCS_MIN_ATIME);
}
...
static int tcs3408_get_new_thresh(struct motion_sensor_t *chip, uint16_t *high, uint16_t *low, uint16_t value)
{
    ...
    margin = value * TCS_3408_DELTA_P / 100;
						...
						uint32_t status)
{
	...
	tcs3408_i2c_read8(s, TCS_I2C_ASTAPL, &data[2]); // astep
    tcs3408_i2c_read8(s, TCS_I2C_ASTAPH, &data[3]);
	...
	again = tcs3408_als_gains[data[1] & TCS3408_MASK_AGAIN];
    saturation = (data[0] + 1) * (((uint32_t)data[1] << 8 | (uint32_t)data[2]) + 1); //atime * astep
    again_adjust_high_thr = (saturation * 8) / 10;
    again_adjust_low_thr = saturation / 10;
	save_atime = data[0];
	sat_p->again = data[1] & TCS3408_MASK_AGAIN;
	sat_p->atime = save_atime;

	if( crgb_data[0] >= again_adjust_high_thr && sat_p->again > 0){
		// dec again
        new_cfg1 = (uint8_t)tcs3408_get_als_new_gain(data[1], -1);
	}else if (crgb_data[0] <= again_adjust_low_thr && sat_p->again < 12 ){
		new_cfg1 = (uint8_t)tcs3408_get_als_new_gain(data[1], 1);
	}else{
		again_adjusted = 0;
	}
	...
	if ((max_val >= TCS_SATURATION_LEVEL)) {
		if ((sat_p->atime > TCS_MIN_ATIME)&&
			(((new_cfg1&TCS3408_MASK_AGAIN )== sat_p->again)||
			(again_adjusted == 0)))
		{	/* reduce accumulation time by decrementing ATIME reg */
		...
		if ((sat_p->atime < TCS_MAX_ATIME)&&
			...
			(again_adjusted == 0))) {
				/*
				 * increase accumulation time by incrementing
				 * ATIME register
				 */
				percent_left =
					TSC_SATURATION_LOW_BAND_PERCENT -
					(max_val * 100 / TCS_SATURATION_LEVEL);
				increment_atime(sat_p, cur_lux, percent_left);
		}
	}
	...
	if (again_adjusted) {
        if (new_cfg1 >= 0 && new_cfg1 <= 12) {
            tcs3408_i2c_modify(s, TCS_I2C_ENABLE, 0x3, 0x1);
            tcs3408_i2c_modify(s, TCS_I2C_CFG1, TCS3408_MASK_AGAIN, new_cfg1);
            tcs3408_i2c_modify(s, TCS_I2C_ENABLE, 0x3, 0x3);
        }
		#ifndef CONFIG_ALS_TCS3408_EMULATED_IRQ_EVENT
			uint16_t high,low;
			if (tcs3408_get_new_thresh(s, &high, &low, (uint16_t)crgb_data[0])) 
			{
	...
	if (save_atime != sat_p->atime) {
		ret = tcs3408_i2c_write8(s, TCS_I2C_ATIME, (sat_p->atime));
		if (ret)
			return ret;
	}
	return EC_SUCCESS;
...
static int32_t get_lux_from_xyz(struct motion_sensor_t *s, uint16_t *crgb_data)
{
	uint16_t save_again ;
	uint16_t save_atime ;
			       ...
			       uint32_t status)
{
	...
	/* get lux value */
	lux = is_calibration ? xyz_data[Y] : get_lux_from_xyz(s, raw_data);
				 ...
				 const uint16_t *scale, int16_t temp)
{
	...
	if (scale[X] == 0 || scale[Y] == 0 || scale[Z] == 0|| scale[U] ==0|| scale[W] ==0)
	...
	rgb_cal[CLEAR_IDX].scale.k_channel_scale = scale[U];
	rgb_cal[WIDEBAND_IDX].scale.k_channel_scale = scale[W];
	return EC_SUCCESS;
}
