...
/* helper */
static int ktu1125_power_path_enable(int port, int enable)
{
...
/* helper */
static int ktu1125_sbu_enable(int port, int enable)
{
	int status = enable ? set_flags(port, KTU1125_CTRL_SW_CFG,
					KTU1125_SBU_SHUT)
			    : clr_flags(port, KTU1125_CTRL_SW_CFG,
...
/* ************* why not using ktu1125_set_vbus_source_current_limit ????? */
	...
	/* Set Vbus OVP threshold to 25V. */
	regval = KTU1125_SYSB_VLIM_25_00;
...
/* BM: enable VCONN KTU1125_VCONN_EN */
	...
	status = ktu1125_sbu_enable(port, 1);
	if (status)
		return status;
...
{
	clr_flags(port, KTU1125_CTRL_SW_CFG,
		  KTU1125_CC1S_VCONN | KTU1125_CC2S_VCONN);
...
static int ktu1125_enter_low_power_mode(int port)
{
	...
	rv = ktu1125_power_path_enable(port, 0);
	if (rv)
		return rv;
...
static int ktu1125_vbus_sink_enable(int port, int enable)
{
	...
	return ktu1125_power_path_enable(port, enable);
}
...
{
	return ktu1125_sbu_enable(port, enable);
}
