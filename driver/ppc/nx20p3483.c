						 ...
						 enum tcpc_rp_value rp)
{
	...
	regval &= ~0xF; /* The current limit settings are 3:0. */
	switch (rp) {
	case TYPEC_RP_3A0:
	...
	status = write_reg(port, NX20P3483_5V_SRC_OCP_THRESHOLD_REG, regval);
...
static int nx20p3483_vbus_sink_enable(int port, int enable)
{
	...
	/*
	 * IF PPC_CFG_FLAGS_AUTO is set then the PPC for this port is not
	 * controlled by the EC and is instead controlled by the TCPC.
	 */
	if (!(ppc_chips[port].flags & PPC_CFG_FLAGS_AUTO)) {
	...
	enable = !!enable;
	return (status ^ enable) ? EC_ERROR_UNKNOWN : EC_SUCCESS;
}
...
static void nx20p3483_handle_interrupt(int port)
{
	...
	if (reg & NX20P3483_INT1_OC_5VSRC) {
		/*
		 * TODO (crbug.com/p/69935262): The overcurrent action hasn't
	 ...
	 */
	/*
	 * TODO (crbug.com/p/75272421): Not sure if any of these interrupts
