...
static int nx20p3483_vbus_sink_enable(int port, int enable)
{
	...
	/*
	 * IF PPC_CFG_FLAGS_GPIO_COMNTROL is set, then the SNK/SRC switch
...
static void nx20p3483_handle_interrupt(int port)
{
	...
	if (reg & NX20P3483_INT1_DBEXIT_ERR) {
		 ...
		 */
		if (++db_exit_fail_count >= NX20P3483_DB_EXIT_FAIL_THRESHOLD) {
			CPRINTS("NX20P3483 PPC failed to exit DB mode");
