...
static int syv682x_is_vbus_present(int port)
{
	...
	if (read_reg(port, SYV682X_STATUS_REG, &val))
		return vbus;
	/* Check register value to see if there are interrupts to avoid race
	 * conditions */
...
#define SYV682X_INTERRUPT_DEFER(port) syv682x_interrupt_delayed(port, 10)
...
static void syv682x_handle_interrupt(int port)
{
	...
	/* Repeat until interrupts are cleared */
	/* Will not spam indefinitely on OCP, but may on OVP, RVS, or TSD */

	if (IS_ENABLED(CONFIG_USBC_PPC_DEDICATED_INT) &&
	    ppc_get_alert_status(port)) {
		SYV682X_INTERRUPT_DEFER(port);
	} else {
		read_reg(port, SYV682X_CONTROL_4_REG, &control4);
		read_reg(port, SYV682X_STATUS_REG, &status);
