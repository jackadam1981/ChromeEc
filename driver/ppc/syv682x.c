...
{
	return i2c_read8__7b(ppc_chips[port].i2c_port,
			 ppc_chips[port].i2c_addr__7b,
			 reg,
			 regval);
}
...
{
	return i2c_write8__7b(ppc_chips[port].i2c_port,
			  ppc_chips[port].i2c_addr__7b,
			  reg,
			  regval);
}
...
static int syv682x_dump(int port)
{
	...
	const int i2c_addr__7b = ppc_chips[port].i2c_addr__7b;

	for (reg_addr = SYV682X_STATUS_REG; reg_addr <= SYV682X_CONTROL_4_REG;
	     reg_addr++) {
		rv = i2c_read8__7b(i2c_port, i2c_addr__7b, reg_addr, &data);
		if (rv)
