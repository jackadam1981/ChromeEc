...
/* Signals an FRS is in-progress (after FRS signal, before VBUS swap) */
#define SYV682X_FLAGS_FRS		BIT(6)
...
static void syv682x_handle_status_interrupt(int port, int regval)
{
	 ...
	 * source path if VBUS is <5V. If VBUS is >5V, manually enable sourcing
	 * so we can disable FRS. If already sourcing, this does nothing.
	 */
	if (IS_ENABLED(CONFIG_USB_PD_FRS_PPC)) {
		if (syv682x_interrupt_filter(port, regval, SYV682X_STATUS_FRS,
					     SYV682X_FLAGS_FRS)) {
			syv682x_vbus_source_enable(port, 1);
