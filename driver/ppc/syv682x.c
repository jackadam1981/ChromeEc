...
static int cc_polarity[CONFIG_USB_PD_PORT_COUNT];
...
static int syv682x_vbus_sink_enable(int port, int enable)
{
	 ...
	 */
	rv = read_reg(port, SYV682X_CONTROL_1_REG, &val);
	if (rv)
		return rv;

	if (enable) {
		before = val;
		val |= SYV682X_CONTROL_1_CH_SEL;
		val &= ~SYV682X_CONTROL_1_HV_DR;
		ccprintf("syv682x: sink en: before = %x after = %x\n",
			 before, val);
	} else {
		ccprintf("syv682x: sink en: disable\n");
	}

	return write_reg(port, SYV682X_CONTROL_1_REG, val);
}
...
static int syv682x_is_vbus_present(int port)
{
	...
	rv = read_reg(port, SYV682X_STATUS_REG, &val);
	if (rv)
		return rv;
	...
	vbus = adc_read_channel(ADC_VBUS_C1);

	return vbus > SYV682X_VBUS_DET_THRESH_MV;
#endif
}
#endif
...
static int syv682x_vbus_source_enable(int port, int enable)
{
	...
	if (enable) {
		...
		ccprintf("syv682: enabling source mode\n");
	} else {
		regval |= SYV682X_CONTROL_1_CH_SEL;
		regval &= ~SYV682X_CONTROL_1_HV_DR;

		flags[port] &= ~SYV682X_FLAGS_SOURCE_ENABLED;
		ccprintf("syv682: disabling source mode\n");
	}
...
static int syv682x_set_polarity(int port, int polarity)
{
	 ...
	 * CC line. So this function saves the CC polarity in a static variable
	 * that can be used in the set_vconn function;
	 */
	cc_polarity[port] = polarity;
...
static int syv682x_dump(int port)
{
	...
	/* Temp hack to read VBUS */
	data = adc_read_channel(ADC_VBUS_C1);
	ccprintf("adc vbus = %d mV\n", data);
