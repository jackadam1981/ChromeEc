...
static void syv682x_handle_status_interrupt(int port, int regval)
{
	...
	if (IS_ENABLED(CONFIG_USB_PD_FRS_PPC)) {
		if (syv682x_interrupt_filter(port, regval, SYV682X_STATUS_FRS,
					     SYV682X_FLAGS_FRS)) {
			/*
			 * According to the syv682 manual, the FRS process of
			 * SYV682 only determines Rd pull down. FRS may be
			 * triggered by unplugging the device. base on USB
			 * PD 3.2 spec, version ,sections 8.3.2.9. Its Source
			 * Port drives CC to ground no larger than
			 * tFRSwapTx(MAX). In order to avoid FRS errors in
			 * syv682. Add the check CC state judgment after FRS is
			 * triggered.
			 */
