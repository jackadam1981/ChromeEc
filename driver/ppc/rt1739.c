...
{
	CPRINTS("%s(%d, %02X, %02X)", __func__, port, reg, val);
	return i2c_write8(
		ppc_chips[port].i2c_port,
		ppc_chips[port].i2c_addr_flags,
		reg, val);
...
{
	CPRINTS("%s(%d, %02X, %02X)", __func__, port, reg, mask);
	return i2c_update8(
		ppc_chips[port].i2c_port,
		ppc_chips[port].i2c_addr_flags,
		reg, mask, action);
...
static enum charge_supplier rt1739_bc12_get_device_type(int port)
{
	...
	switch (reg & RT1739_REG_BC12_STAT_PORT_STAT_MASK) {
...
static void rt1739_handle_interrupt(int port)
{
	int event;

	if (read_reg(port, RT1739_REG_INT_EVENT5, &event))
		return;

	if (event & RT1739_REG_INT_EVENT5_BC12_SNK_DONE)
		task_set_event(USB_CHG_PORT_TO_TASK_ID(port),
			       USB_CHG_EVENT_BC12);

	write_reg(port, RT1739_REG_INT_EVENT5, event);
}

/* TODO: move to board.c */
static void rt1739_handle_interrupt_deferred(void)
{
	rt1739_handle_interrupt(0);
}
DECLARE_DEFERRED(rt1739_handle_interrupt_deferred);

void rt1739_interrupt(enum gpio_signal signal) {
	CPRINTS("%s triggered!", __func__);
	hook_call_deferred(&rt1739_handle_interrupt_deferred_data, 0);
}
