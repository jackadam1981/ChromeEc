...
static int nct38xx_ioex_init(int expander)
{
	int rv, val;
	struct ioexpander_config_t *ioex = &ioex_config[expander];
	 ...
	 *  010: NCT3808
	 */
	rv = i2c_read8(ioex->i2c_host_port, ioex->i2c_slave_addr,
	...
	if (rv)
		...
		ioex->chip_info = ((uint8_t)val & NCT38XX_VARIANT_MASK) >> 2;

	return rv;
}

static int nct38xx_ioex_get_level(int expander, int port, int mask, int *val)
{
	int rv, reg;
	struct ioexpander_config_t *ioex = &ioex_config[expander];

	rv = nct38xx_ioex_check_is_valid(ioex->chip_info, port, mask);
	if (rv !=  EC_SUCCESS)
		return rv;

	reg = NCT38XXX_REG_GPIO_DATA_IN(port);
	rv = i2c_read8(ioex_config[expander].i2c_host_port,
			ioex_config[expander].i2c_slave_addr, reg, val);

	*val =  !!(*val & mask);
	return rv;
}

static int nct38xx_ioex_set_level(int expander, int port, int mask, int value)
{
	int rv, reg, val;
	struct ioexpander_config_t *ioex = &ioex_config[expander];
	...
	if (value)
		...
		val &= ~mask;
	rv |= i2c_write8(ioex_config[expander].i2c_host_port,
			ioex_config[expander].i2c_slave_addr, reg, val);
	return rv;
}

static int nct38xx_ioex_get_flags(int expander, int port, int mask, int *flags)
{
	int rv, reg, val, i2c_port, i2c_addr;
	struct ioexpander_config_t *ioex = &ioex_config[expander];

	i2c_port = ioex_config[expander].i2c_host_port;
	i2c_addr = ioex_config[expander].i2c_slave_addr;
...
static int nct38xx_ioex_set_flags_by_mask(int expander, int port, int mask,
					int flags)
{
	int rv, reg, val, i2c_port, i2c_addr;
	struct ioexpander_config_t *ioex = &ioex_config[expander];
	...
	if (port == 0) {
		...
		if (!(ioex->chip_info == NCT38XX_VARIANT_3807 && mask & 0x08)) {
