...
void normalize(const struct motion_sensor_t *s, vector_3_t v)
{
	int range;
...
static void *vectorcpy(void *dest, const void *src, size_t n)
{
	/* copy a short vector to a regular vector */
	int *v = dest;
	const uint8_t *sv = src;
	while (n) {
		n -= sizeof(short);
		*v++ = ((int16_t)((sv[1] << 8) | sv[0]));
		sv += sizeof(short);
	}
	return dest;
}
...
static int load_fifo(const struct motion_sensor_t *s)
{
	int done = 0;
	uint8_t fifo_reg;
	...
	fifo_reg = BMI160_FIFO_LENGTH_0;
	i2c_xfer(I2C_PORT_ACCEL, s->i2c_addr,
				&fifo_reg, 1, (uint8_t *)&fifo_length,
				sizeof(fifo_length), I2C_XFER_SINGLE);
	i2c_lock(I2C_PORT_ACCEL, 0);
	fifo_length &= BMI160_FIFO_LENGTH_MASK;
	if (fifo_length == 0) {
		/* nothing to read, FIFO disabled */
		mutex_unlock(s->mutex);
		return EC_SUCCESS;
	}
	do {
		...
		uint8_t *bp = buffer;
		i2c_lock(I2C_PORT_ACCEL, 1);
		fifo_reg = BMI160_FIFO_DATA;
		i2c_xfer(I2C_PORT_ACCEL, s->i2c_addr,
				&fifo_reg, 1, buffer,
				sizeof(buffer), I2C_XFER_SINGLE);
		i2c_lock(I2C_PORT_ACCEL, 0);
		while (!done && bp != buffer + sizeof(buffer)) {
			switch (state) {
			case FIFO_HEADER: {
...
void bmi160_sense_task(void *arg)
{
	int iter;
	uint32_t interrupt;
	uint8_t interrupt_reg = BMI160_INT_STATUS_0;
	const struct motion_sensor_t *s = arg;
	do {
		if (task_wait_event_mask(TASK_EVENT_SENSOR_PENDING, -1) ==
			...
			do {
				...
				i2c_xfer(I2C_PORT_ACCEL, s->i2c_addr,
					&interrupt_reg, 1,
					(uint8_t *)&interrupt,
					sizeof(interrupt), I2C_XFER_SINGLE);
				i2c_lock(I2C_PORT_ACCEL, 0);
...
#ifdef CONFIG_ACCEL_FIFO
static int bmi160_read_fifo(int argc, char **argv)
{
	char *e;
	int id, count, i;
	...
	q = BMI160_GET_QUEUE(sensor, sensor->type);
	/* Limit the amount of data to avoid saturating the UART buffer */
	count = MIN(queue_count(q), 16);
	for (i = 0; i < count; i++) {
		queue_peek_memcpy(q, v, i, 1, vectorcpy);
		normalize(sensor, v);
...
DECLARE_CONSOLE_COMMAND(fiforead, bmi160_read_fifo,
	"id",
...
static int read(const struct motion_sensor_t *s, vector_3_t v)
{
...
#ifdef CONFIG_ACCEL_FIFO
	struct queue *q = BMI160_GET_QUEUE(s, s->type);
	mutex_lock(s->mutex);
	ret = queue_peek_memcpy(q, v, queue_count(q) - 1, 1, vectorcpy);
	mutex_unlock(s->mutex);
	if (ret == 0)
	...
	i2c_lock(I2C_PORT_ACCEL, 1);
	ret = i2c_xfer(I2C_PORT_ACCEL, s->i2c_addr, &xyz_reg, 1, data, 6,
			I2C_XFER_SINGLE);
	i2c_lock(I2C_PORT_ACCEL, 0);
...
#ifdef CONFIG_ACCEL_FIFO
	.queue = {
		QUEUE_NULL(CONFIG_ACCELGYRO_BMI160_ACCEL_FIFO_EVENT, short[3]),
		QUEUE_NULL(CONFIG_ACCELGYRO_BMI160_GYRO_FIFO_EVENT, short[3]),
		QUEUE_NULL(CONFIG_ACCELGYRO_BMI160_MAG_FIFO_EVENT, short[4]),
	},
#endif
};
