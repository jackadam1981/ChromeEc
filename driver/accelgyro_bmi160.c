...
static int enable_fifo(const struct motion_sensor_t *s, int enable)
{
	...
	if (enable) {
		...
		ret = bmi_read8(s->port, s->addr, BMI160_FIFO_CONFIG_1, &val);
		val |= BMI160_FIFO_SENSOR_EN(s->type);
		ret = bmi_write8(s->port, s->addr, BMI160_FIFO_CONFIG_1, val);
		if (ret == EC_SUCCESS)
				...
				int rnd)
{
	...
	case MOTIONSENSE_TYPE_ACCEL:
		if (normalized_rate > MIN(BMI_ACCEL_MAX_FREQ,
					CONFIG_EC_MAX_SENSOR_FREQ_MILLIHZ) ||
		    normalized_rate < BMI_ACCEL_MIN_FREQ)
			return EC_RES_INVALID_PARAM;
		break;
	case MOTIONSENSE_TYPE_GYRO:
		if (normalized_rate > MIN(BMI_GYRO_MAX_FREQ,
					CONFIG_EC_MAX_SENSOR_FREQ_MILLIHZ) ||
		    normalized_rate < BMI_GYRO_MIN_FREQ)
			return EC_RES_INVALID_PARAM;
		break;
#ifdef CONFIG_MAG_BMI160_BMM150
	...
	ret = bmi_read8(s->port, s->addr, ctrl_reg, &val);
	if (ret != EC_SUCCESS)
		goto accel_cleanup;

	val = (val & ~BMI160_ODR_MASK) | reg_val;
	ret = bmi_write8(s->port, s->addr, ctrl_reg, val);
	if (ret != EC_SUCCESS)
		goto accel_cleanup;
			...
			int16_t    temp)
{
	...
	case MOTIONSENSE_TYPE_ACCEL:
		for (i = X; i <= Z; i++) {
			...
			if (val < 0)
				val = 256 + val;
			bmi_write8(s->port, s->addr, BMI160_OFFSET_ACC70 + i,
				   val);
		}
	...
	case MOTIONSENSE_TYPE_GYRO:
		for (i = X; i <= Z; i++) {
			...
			if (val < 0)
				val = 1024 + val;
			bmi_write8(s->port, s->addr, BMI160_OFFSET_GYR70 + i,
					val & 0xFF);
			val98 &= ~(0x3 << (2 * i));
			val98 |= (val >> 8) << (2 * i);
		}
...
static int perform_calib(const struct motion_sensor_t *s)
{
	...
	ret = bmi_read8(s->port, s->addr, BMI160_OFFSET_EN_GYR98, &val);
	ret = bmi_write8(s->port, s->addr, BMI160_OFFSET_EN_GYR98,
			 val | en_flag);
end_perform_calib:
		    ...
		    const struct ec_motion_sense_activity *param)
{
	...
	case MOTIONSENSE_ACTIVITY_SIG_MOTION: {
		...
		ret = bmi_write8(s->port, s->addr, BMI160_INT_EN_0, tmp);
		if (ret)
...
static int config_interrupt(const struct motion_sensor_t *s)
{
	...
	/* Set fifo*/
	ret = bmi_read8(s->port, s->addr, BMI160_INT_EN_1, &tmp);
	tmp |= BMI160_INT_FWM_EN | BMI160_INT_FFUL_EN;
	ret = bmi_write8(s->port, s->addr, BMI160_INT_EN_1, tmp);
#endif
	mutex_unlock(s->mutex);
	return ret;
}
...
static int init(const struct motion_sensor_t *s)
{
	...
	if (s->type == MOTIONSENSE_TYPE_MAG) {
		...
		if ((data->flags & BMI160_FLAG_SEC_I2C_ENABLED) == 0) {
			 ...
			 */
			bmi_read8(s->port, s->addr, BMI160_CMD_EXT_MODE_ADDR,
					&ext_page_reg);
			bmi_write8(s->port, s->addr, BMI160_CMD_EXT_MODE_ADDR,
			...
			bmi_read8(s->port, s->addr, BMI160_COM_C_TRIM_ADDR,
					&pullup_reg);
			bmi_write8(s->port, s->addr, BMI160_COM_C_TRIM_ADDR,
...
int bmi160_get_sensor_temp(int idx, int *temp_ptr)
{
	...
	ret = bmi_read_n(s->port, s->addr, BMI160_TEMPERATURE_0,
