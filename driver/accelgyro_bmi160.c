...
static inline int raw_write8(const int addr, const uint8_t reg, int data)
{
	...
	return spi_transaction(CONFIG_SPI_ACCEL_PORT, addr, cmd, 2, NULL, 0);
}
			       ...
			       const int len)
{
	...
	return spi_transaction(CONFIG_SPI_ACCEL_PORT, addr, &cmd, 1, data, len);
}
...
{
	int rv;
	rv = spi_raw_read(addr, reg, (uint8_t *)data, 1);
	*data &= 0xff;
	return rv;
}
...
{
	int rv;
	rv = spi_raw_read(addr, reg, (uint8_t *)data, 2);
	*data &= 0xffff;
	return rv;
}
