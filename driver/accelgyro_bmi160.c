...
static int irq_handler(struct motion_sensor_t *s, uint32_t *event)
{
	...
	if (interrupt & BMI160_ORIENT_INT) {
		...
		case BMI160_ORIENT_PORTRAIT:
			*event |= CONFIG_ORIENT_PORTRAIT_EVENT;
			break;
		case BMI160_ORIENT_PORTRAIT_INVERT:
			*event |= CONFIG_ORIENT_INVERT_PORTRAIT_EVENT;
			break;
		case BMI160_ORIENT_LANDSCAPE:
			*event |= CONFIG_ORIENT_LANDSCAPE_EVENT;
			break;
		case BMI160_ORIENT_LANDSCAPE_INVERT:
			*event |= CONFIG_ORIENT_INVERT_LANDSCAPE_EVENT;
			break;
		default:
			break;
		}
