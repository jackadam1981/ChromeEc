...
#endif
#ifdef CONFIG_BMI160_ORIENTATION_SENSOR
	if (interrupt & BMI160_ORIENT_XY_MASK) {
		int shifted_masked_orient;

		shifted_masked_orient =
			(interrupt >> 24) & BMI160_ORIENT_XY_MASK;
		if (BMI160_GET_DATA(s)->raw_orient != shifted_masked_orient) {
			enum motionsensor_orientation orient =
				MOTIONSENSE_ORIENTATION_UNKNOWN;

			BMI160_GET_DATA(s)->raw_orient = shifted_masked_orient;

			switch (shifted_masked_orient) {
			case BMI160_ORIENT_PORTRAIT:
				orient = MOTIONSENSE_ORIENTATION_PORTRAIT;
				break;
			...
			case BMI160_ORIENT_LANDSCAPE:
				orient = MOTIONSENSE_ORIENTATION_LANDSCAPE;
				break;
			...
			orient = motion_sense_remap_orientation(s, orient);
			SET_ORIENTATION(s, orient);
		}
	}
#endif
	/*
