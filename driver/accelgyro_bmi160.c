			...
			int16_t    *temp)
{
	...
	raw_read8(s->i2c_addr, BMI160_TEMPERATURE_0, &val);
	if (val == EC_MOTION_SENSE_INVALID_CALIB_TEMP)
		*temp = EC_MOTION_SENSE_INVALID_CALIB_TEMP;
	else
		/*
		 * temperature is centered at 23C and
		 * 0x7fff is 87 - .5^-9 C
		 */
		*temp = 23 * 100 + ((int16_t)val * (87 - 23)) / 327;
	return EC_SUCCESS;
}
