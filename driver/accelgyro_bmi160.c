...
#ifdef CONFIG_SPI_ACCEL_PORT
		uint8_t val;
		rv = spi_raw_read(slave_addr.spi_dev_id, reg, &val, 1);
		if (rv == EC_SUCCESS)
			*data_ptr = val;
#endif
	} else {
 ...
 */
static int raw_read32(const struct slave_addr_t slave_addr,
		      const uint8_t reg, int *data_ptr)
{
	int rv = -EC_ERROR_PARAM1;
	if (slave_addr.is_spi) {
#ifdef CONFIG_SPI_ACCEL_PORT
		rv = spi_raw_read(slave_addr.spi_dev_id, reg,
				  (uint8_t *)data_ptr, 4);
#endif
	} else {
#ifdef I2C_PORT_ACCEL
		rv = i2c_read32(slave_addr, reg, data_ptr);
#endif
	}
	return rv;
}
 ...
 */
static int raw_read_n(const struct slave_addr_t slave_addr,
		      const uint8_t reg, uint8_t *data_ptr, const int len)
{
	...
	if (slave_addr.is_spi) {
#ifdef CONFIG_SPI_ACCEL_PORT
		rv = spi_raw_read(slave_addr.spi_dev_id, reg, data_ptr, len);
#endif
	} else {
#ifdef I2C_PORT_ACCEL
		rv = i2c_read_block(slave_addr, reg, data_ptr, len);
#endif
	}
	return rv;
}
 ...
 */
static int bmi160_sec_access_ctrl(const struct slave_addr_t slave_addr,
				  const int enable)
{
	int mag_if_ctrl;
	raw_read8(slave_addr, BMI160_MAG_IF_1, &mag_if_ctrl);
	if (enable) {
	...
	return raw_write8(slave_addr, BMI160_MAG_IF_1, mag_if_ctrl);
}
 ...
 */
int bmi160_sec_raw_read8(const struct slave_addr_t slave_addr,
			 const uint8_t reg, int *data_ptr)
{
	/* Only read 1 bytes */
	raw_write8(slave_addr, BMI160_MAG_I2C_READ_ADDR, reg);
	return raw_read8(slave_addr, BMI160_MAG_I2C_READ_DATA, data_ptr);
}
 ...
 */
int bmi160_sec_raw_write8(const struct slave_addr_t slave_addr,
			  const uint8_t reg, int data)
{
	raw_write8(slave_addr, BMI160_MAG_I2C_WRITE_DATA, data);
	return raw_write8(slave_addr, BMI160_MAG_I2C_WRITE_ADDR, reg);
}
#endif
...
static int enable_fifo(const struct motion_sensor_t *s, int enable)
{
	...
	if (enable) {
		...
		ret = raw_read8(s->slave_addr, BMI160_FIFO_CONFIG_1, &val);
		val |= BMI160_FIFO_SENSOR_EN(s->type);
		ret = raw_write8(s->slave_addr, BMI160_FIFO_CONFIG_1, val);
		if (ret == EC_SUCCESS)
				...
				int rnd)
{
	...
	ret = raw_write8(s->slave_addr, ctrl_reg, reg_val);
				...
				int rnd)
{
...
#endif
		/* go to suspend mode */
		ret = raw_write8(s->slave_addr, BMI160_CMD_REG,
	...
	ret = raw_read8(s->slave_addr, ctrl_reg, &val);
	if (ret != EC_SUCCESS)
		goto accel_cleanup;

	val = (val & ~BMI160_ODR_MASK) | reg_val;
	ret = raw_write8(s->slave_addr, ctrl_reg, val);
	if (ret != EC_SUCCESS)
		goto accel_cleanup;
			...
			int16_t    *temp)
{
	...
	case MOTIONSENSE_TYPE_ACCEL:
		 ...
		 */
		for (i = X; i <= Z; i++) {
			raw_read8(s->slave_addr, BMI160_OFFSET_ACC70 + i,
				  &val);
			if (val > 0x7f)
				val = -256 + val;
	...
	case MOTIONSENSE_TYPE_GYRO:
		/* Read the MSB first */
		raw_read8(s->slave_addr, BMI160_OFFSET_EN_GYR98, &val98);
		/*
		 ...
		 */
		for (i = X; i <= Z; i++) {
			raw_read8(s->slave_addr, BMI160_OFFSET_GYR70 + i,
				  &val);
			val |= ((val98 >> (2 * i)) & 0x3) << 8;
			if (val > 0x1ff)
				val = -1024 + val;
			...
			int16_t    temp)
{
	...
	ret = raw_read8(s->slave_addr, BMI160_OFFSET_EN_GYR98, &val98);
	if (ret != 0)
		return ret;
	...
	case MOTIONSENSE_TYPE_ACCEL:
		for (i = X; i <= Z; i++) {
			...
			if (val < 0)
				val = 256 + val;
			raw_write8(s->slave_addr, BMI160_OFFSET_ACC70 + i,
				   val);
		}
		ret = raw_write8(s->slave_addr, BMI160_OFFSET_EN_GYR98,
	...
	case MOTIONSENSE_TYPE_GYRO:
		for (i = X; i <= Z; i++) {
			...
			if (val < 0)
				val = 1024 + val;
			raw_write8(s->slave_addr, BMI160_OFFSET_GYR70 + i,
					val & 0xFF);
			val98 &= ~(0x3 << (2 * i));
			val98 |= (val >> 8) << (2 * i);
		}
...
static int perform_calib(const struct motion_sensor_t *s)
{
	...
	ret = raw_write8(s->slave_addr, BMI160_FOC_CONF, val);
	ret = raw_write8(s->slave_addr, BMI160_CMD_REG,
	...
	do {
		...
		msleep(50);
		ret = raw_read8(s->slave_addr, BMI160_STATUS, &status);
		if (ret != EC_SUCCESS)
			goto end_perform_calib;
	} while ((status & BMI160_FOC_RDY) == 0);

	/* Calibration is successful, and loaded, use the result */
	ret = raw_read8(s->slave_addr, BMI160_OFFSET_EN_GYR98, &val);
	ret = raw_write8(s->slave_addr, BMI160_OFFSET_EN_GYR98,
			 val | en_flag);
end_perform_calib:
		    ...
		    const struct ec_motion_sense_activity *param)
{
	...
	case MOTIONSENSE_ACTIVITY_SIG_MOTION: {
		...
		if (enable) {
			...
			raw_write8(s->slave_addr, BMI160_INT_MOTION_3,
			...
			raw_write8(s->slave_addr, BMI160_INT_MOTION_1,
				BMI160_MOTION_TH(s,
		...
		ret = raw_write8(s->slave_addr, BMI160_INT_EN_0, tmp);
		if (ret)
	...
	case MOTIONSENSE_ACTIVITY_DOUBLE_TAP: {
		...
		ret = raw_read8(s->slave_addr, BMI160_INT_EN_0, &tmp);
		if (ret)
			return ret;
		if (enable)
...
static int load_fifo(struct motion_sensor_t *s, uint32_t last_ts)
{
	...
	if (!(data->flags &
		 ...
		 */
		raw_write8(s->slave_addr, BMI160_CMD_REG,
			   BMI160_CMD_FIFO_FLUSH);
		return EC_SUCCESS;
	}

	raw_read_n(s->slave_addr, BMI160_FIFO_LENGTH_0,
	...
	raw_read_n(s->slave_addr, BMI160_FIFO_DATA, bmi160_buffer,
		   length);
...
static int config_interrupt(const struct motion_sensor_t *s)
{
	...
	mutex_lock(s->mutex);
	raw_write8(s->slave_addr, BMI160_CMD_REG, BMI160_CMD_FIFO_FLUSH);
	raw_write8(s->slave_addr, BMI160_CMD_REG, BMI160_CMD_INT_RESET);

#ifdef CONFIG_GESTURE_SENSOR_BATTERY_TAP
	raw_write8(s->slave_addr, BMI160_INT_TAP_0,
		BMI160_TAP_DUR(s, CONFIG_GESTURE_TAP_MAX_INTERSTICE_T));
	ret = raw_write8(s->slave_addr, BMI160_INT_TAP_1,
	...
	if (s->location == MOTIONSENSE_LOC_LID) {
		ret = raw_write8(s->slave_addr, BMI160_INT_ORIENT_0,
			BMI160_INT_ORIENT_0_INIT_VAL);
		ret = raw_write8(s->slave_addr, BMI160_INT_ORIENT_1,
			BMI160_INT_ORIENT_1_INIT_VAL);
	}
#endif

#ifdef CONFIG_ACCELGYRO_BMI160_INT2_OUTPUT
	ret = raw_write8(s->slave_addr, BMI160_INT_LATCH, BMI160_LATCH_5MS);
#else
	/* Also, configure int2 as an external input. */
	ret = raw_write8(s->slave_addr, BMI160_INT_LATCH,
		BMI160_INT2_INPUT_EN | BMI160_LATCH_5MS);
#endif

	/* configure int1 as an interrupt */
	ret = raw_write8(s->slave_addr, BMI160_INT_OUT_CTRL,
...
#endif
	ret = raw_write8(s->slave_addr, BMI160_INT_MAP_REG(1), tmp);

#ifdef CONFIG_ACCEL_FIFO
	/* map fifo water mark to int 1 */
	ret = raw_write8(s->slave_addr, BMI160_INT_FIFO_MAP,
			BMI160_INT_MAP(1, FWM) |
			BMI160_INT_MAP(1, FFULL));

	/* configure fifo watermark to int whenever there's any data in there */
	ret = raw_write8(s->slave_addr, BMI160_FIFO_CONFIG_0, 1);
#ifdef CONFIG_ACCELGYRO_BMI160_INT2_OUTPUT
	ret = raw_write8(s->slave_addr, BMI160_FIFO_CONFIG_1,
	...
	/* Set fifo*/
	ret = raw_read8(s->slave_addr, BMI160_INT_EN_1, &tmp);
	tmp |= BMI160_INT_FWM_EN | BMI160_INT_FFUL_EN;
	ret = raw_write8(s->slave_addr, BMI160_INT_EN_1, tmp);
#endif
	mutex_unlock(s->mutex);
	return ret;
}
...
static int irq_handler(struct motion_sensor_t *s, uint32_t *event)
{
	...
	do {
		rv = raw_read32(s->slave_addr, BMI160_INT_STATUS_0,
				&interrupt);
		/*
...
static int read(const struct motion_sensor_t *s, intv3_t v)
{
	...
	ret = raw_read_n(s->slave_addr, get_xyz_reg(s->type), data, 6);
...
static int init(const struct motion_sensor_t *s)
{
	...
	ret = raw_read8(s->slave_addr, BMI160_CHIP_ID, &tmp);
	if (ret)
	...
	if (tmp != BMI160_CHIP_ID_MAJOR && tmp != BMI168_CHIP_ID_MAJOR) {
		...
		raw_write8(s->slave_addr, BMI160_CMD_EXT_MODE_ADDR,
				BMI160_CMD_PAGING_EN);
		raw_write8(s->slave_addr, BMI160_CMD_EXT_MODE_ADDR, 0);
	...
	if (s->type == MOTIONSENSE_TYPE_ACCEL) {
		...
		raw_write8(s->slave_addr, BMI160_PMU_TRIGGER, 0);
	}
	...
	if (s->type == MOTIONSENSE_TYPE_MAG) {
		...
		if ((data->flags & BMI160_FLAG_SEC_I2C_ENABLED) == 0) {
			 ...
			 */
			raw_read8(s->slave_addr, BMI160_CMD_EXT_MODE_ADDR,
					&ext_page_reg);
			raw_write8(s->slave_addr, BMI160_CMD_EXT_MODE_ADDR,
			...
			raw_read8(s->slave_addr, BMI160_COM_C_TRIM_ADDR,
					&pullup_reg);
			raw_write8(s->slave_addr, BMI160_COM_C_TRIM_ADDR,
			...
			ret = raw_write8(s->slave_addr, BMI160_MAG_IF_0,
					CONFIG_ACCELGYRO_SEC_ADDR__8b);

			/* Enable the secondary interface as I2C */
			ret = raw_write8(s->slave_addr, BMI160_IF_CONF,
				BMI160_IF_MODE_AUTO_I2C << BMI160_IF_MODE_OFF);
			data->flags |= BMI160_FLAG_SEC_I2C_ENABLED;
		}

		bmi160_sec_access_ctrl(s->slave_addr, 1);
...
int bmi160_get_sensor_temp(int idx, int *temp_ptr)
{
	...
	ret = raw_read_n(s->slave_addr, BMI160_TEMPERATURE_0,
