...
#ifdef CONFIG_SPI_ACCEL_PORT
		uint8_t val;
		rv = spi_raw_read(BMI_SPI_ADDRESS(addr), reg, &val, 1);
		if (rv == EC_SUCCESS)
			*data_ptr = val;
#endif
	} else {
#ifdef I2C_PORT_ACCEL
		rv = i2c_read8(port, BMI_I2C_ADDRESS(addr),
			       reg, data_ptr);
#endif
	}
	return rv;
					  ...
					  int data)
{
	...
	if (BMI_IS_SPI(addr)) {
		...
		rv = spi_transaction(&spi_devices[BMI_SPI_ADDRESS(addr)],
				     cmd, 2, NULL, 0);
#endif
	} else {
#ifdef I2C_PORT_ACCEL
		rv = i2c_write8(port, BMI_I2C_ADDRESS(addr),
				reg, data);
#endif
	}
	/*
					  ...
					  int *data_ptr)
{
	...
	if (BMI_IS_SPI(addr)) {
#ifdef CONFIG_SPI_ACCEL_PORT
		rv = spi_raw_read(BMI_SPI_ADDRESS(addr), reg,
				  (uint8_t *)data_ptr, 4);
#endif
	} else {
#ifdef I2C_PORT_ACCEL
		rv = i2c_read32(port, BMI_I2C_ADDRESS(addr),
				reg, data_ptr);
#endif
	}
	return rv;
		...
		uint8_t *data_ptr, const int len)
{
	...
	if (BMI_IS_SPI(addr)) {
#ifdef CONFIG_SPI_ACCEL_PORT
		rv = spi_raw_read(BMI_SPI_ADDRESS(addr), reg, data_ptr, len);
#endif
	} else {
#ifdef I2C_PORT_ACCEL
		rv = i2c_read_block(port, BMI_I2C_ADDRESS(addr), reg,
				data_ptr, len);
#endif
	}
	return rv;
