...
#define RAW_READ(_size, _addr, _reg, _data_ptr) \
	i2c_read##_size(I2C_PORT_ACCEL, (_addr), (_reg), (_data_ptr))
#define RAW_WRITE(_size, _addr, _reg, _data) \
	i2c_write##_size(I2C_PORT_ACCEL, (_addr), (_reg), (_data))
...
{
	int mag_if_ctrl;
	RAW_READ(8, addr, BMI160_MAG_IF_1, &mag_if_ctrl);
	if (enable) {
	...
	return RAW_WRITE(8, addr, BMI160_MAG_IF_1, mag_if_ctrl);
}
...
{
	/* Only read 1 bytes */
	RAW_WRITE(8, addr, BMI160_MAG_I2C_READ_ADDR, reg);
	return RAW_READ(8, addr, BMI160_MAG_I2C_READ_DATA, data_ptr);
}
...
{
	RAW_WRITE(8, addr, BMI160_MAG_I2C_WRITE_DATA, data);
	return RAW_WRITE(8, addr, BMI160_MAG_I2C_WRITE_ADDR, reg);
}
#endif
				...
				int rnd)
{
	...
	ret = RAW_WRITE(8, s->i2c_addr, ctrl_reg, reg_val);
				...
				int rnd)
{
	...
	if (rate == 0) {
		/* go to suspend mode */
		ret = RAW_WRITE(8, s->i2c_addr, BMI160_CMD_REG,
	...
	ret = RAW_READ(8, s->i2c_addr, ctrl_reg, &val);
	if (ret != EC_SUCCESS)
		goto accel_cleanup;

	val = (val & ~BMI160_ODR_MASK) | reg_val;
	ret = RAW_WRITE(8, s->i2c_addr, ctrl_reg, val);
			       ...
			       unsigned int threshold)
{
	...
	mutex_lock(s->mutex);
	RAW_WRITE(8, s->i2c_addr, BMI160_CMD_REG, BMI160_CMD_FIFO_FLUSH);
	msleep(30);
	RAW_WRITE(8, s->i2c_addr, BMI160_CMD_REG, BMI160_CMD_INT_RESET);

	/* Latch until interupts */
	/* configure int2 as an external input */
	tmp = BMI160_INT2_INPUT_EN | BMI160_LATCH_FOREVER;
	ret = RAW_WRITE(8, s->i2c_addr, BMI160_INT_LATCH, tmp);

	/* configure int1 as an interupt */
	ret = RAW_WRITE(8, s->i2c_addr, BMI160_INT_OUT_CTRL,
		BMI160_INT_CTRL(1, OUTPUT_EN));

	/* Map Simple/Double Tap to int 1
	 * Map Flat interrupt to int 1
	 */
	ret = RAW_WRITE(8, s->i2c_addr, BMI160_INT_MAP_REG(1),
		BMI160_INT_FLAT | BMI160_INT_D_TAP | BMI160_INT_S_TAP);

#ifdef CONFIG_ACCEL_FIFO
	/* map fifo water mark to int 1 */
	ret = RAW_WRITE(8, s->i2c_addr, BMI160_INT_FIFO_MAP,
			BMI160_INT_MAP(1, FWM));

	/* configure fifo watermark at 50% */
	ret = RAW_WRITE(8, s->i2c_addr, BMI160_FIFO_CONFIG_0,
			512 / sizeof(uint32_t));
	ret = RAW_WRITE(8, s->i2c_addr, BMI160_FIFO_CONFIG_1,
	...
	ret = RAW_READ(8, s->i2c_addr, BMI160_INT_EN_0, &tmp);
	tmp |= BMI160_INT_FLAT_EN | BMI160_INT_D_TAP_EN | BMI160_INT_S_TAP_EN;
	ret = RAW_WRITE(8, s->i2c_addr, BMI160_INT_EN_0, tmp);

#if 0
	/* TODO(crbug/494021): By FIFO interrupt, I lose the other events */
	/* #ifdef CONFIG_ACCEL_FIFO */
	ret = RAW_READ(8, s->i2c_addr, BMI160_INT_EN_1, &tmp);
	tmp |= BMI160_INT_FWM_EN;
	ret = RAW_WRITE(8, s->i2c_addr, BMI160_INT_EN_1, tmp);
#endif
...
static int load_fifo(struct motion_sensor_t *s)
{
	...
	/* Read fifo length */
	RAW_READ(16, s->i2c_addr, BMI160_FIFO_LENGTH_0, &fifo_length);
...
int irq_handler(const struct motion_sensor_t *s)
{
	int interrupt;
	do {
		RAW_READ(32, s->i2c_addr, BMI160_INT_STATUS_0, &interrupt);
		RAW_WRITE(8, s->i2c_addr, BMI160_CMD_REG, BMI160_CMD_INT_RESET);
...
static int is_data_ready(const struct motion_sensor_t *s, int *ready)
{
	...
	ret = RAW_READ(8, s->i2c_addr, BMI160_STATUS, &tmp);
...
static int init(const struct motion_sensor_t *s)
{
	...
	ret = RAW_READ(8, s->i2c_addr, BMI160_CHIP_ID, &tmp);
	if (ret)
	...
	if (s->type == MOTIONSENSE_TYPE_ACCEL) {
		...
		RAW_WRITE(8, s->i2c_addr, BMI160_CMD_REG,
				BMI160_CMD_SOFT_RESET);
		msleep(30);
		data->flags &= ~BMI160_FLAG_SEC_I2C_ENABLED;
		/* To avoid gyro wakeup */
		RAW_WRITE(8, s->i2c_addr, BMI160_PMU_TRIGGER, 0);
	}
	...
	if (s->type == MOTIONSENSE_TYPE_MAG) {
		...
		if ((data->flags & BMI160_FLAG_SEC_I2C_ENABLED) == 0) {
			 ...
			 */
			RAW_READ(8, s->i2c_addr, BMI160_CMD_EXT_MODE_ADDR,
					&ext_page_reg);
			RAW_WRITE(8, s->i2c_addr, BMI160_CMD_EXT_MODE_ADDR,
					ext_page_reg | BMI160_CMD_TARGET_PAGE);
			RAW_READ(8, s->i2c_addr, BMI160_CMD_EXT_MODE_ADDR,
					&ext_page_reg);
			RAW_WRITE(8, s->i2c_addr, BMI160_CMD_EXT_MODE_ADDR,
					ext_page_reg | BMI160_CMD_PAGING_EN);
			RAW_READ(8, s->i2c_addr, BMI160_COM_C_TRIM_ADDR,
					&pullup_reg);
			RAW_WRITE(8, s->i2c_addr, BMI160_COM_C_TRIM_ADDR,
			...
			ret = RAW_WRITE(8, s->i2c_addr, BMI160_MAG_IF_0,
					BMM150_I2C_ADDRESS);

			/* Enable the secondary interface as I2C */
			ret = RAW_WRITE(8, s->i2c_addr, BMI160_IF_CONF,
		...
		RAW_WRITE(8, s->i2c_addr, BMI160_MAG_I2C_READ_ADDR,
				BMM150_BASE_DATA);
		/*
