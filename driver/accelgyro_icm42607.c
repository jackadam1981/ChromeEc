					     ...
					     uint32_t ts)
{
	...
	for (i = 0; i < count; i += size) {
		...
		if (size <= 0)
			return -size;
		if (accel != NULL) {
			ret = icm42607_check_sensor_stabilized(st->accel, ts);
			if (ret == EC_SUCCESS)
				icm42607_push_fifo_data(st->accel, accel, ts);
		}
		if (gyro != NULL) {
			ret = icm42607_check_sensor_stabilized(st->gyro, ts);
			if (ret == EC_SUCCESS)
				icm42607_push_fifo_data(st->gyro, gyro, ts);
		}
	}
...
static int icm42607_config_interrupt(const struct motion_sensor_t *s)
{
	...
	/* configure INT1 pin */
	val = ICM42607_INT1_PUSH_PULL;
	if (s->flags & MOTIONSENSE_FLAG_INT_ACTIVE_HIGH)
		val |= ICM42607_INT1_ACTIVE_HIGH;

	ret = icm_write8(s, ICM42607_REG_INT_CONFIG, val);
	if (ret != EC_SUCCESS)
		return ret;
				  ...
				  intv3_t offset)
{
	...
	case MOTIONSENSE_TYPE_ACCEL:
		...
		for (i = 0; i < sizeof(raw); ++i) {
			reg = ICM42607_MREG_OFFSET_USER4 + i;
			ret = icm_read_mclk_reg(s, reg, &val);
			if (ret != EC_SUCCESS)
				break;
			raw[i] = val;
	...
	case MOTIONSENSE_TYPE_GYRO:
		mutex_lock(s->mutex);
		for (i = 0; i < sizeof(raw); ++i) {
			reg = ICM42607_MREG_OFFSET_USER0 + i;
			ret = icm_read_mclk_reg(s, reg, &val);
			if (ret != EC_SUCCESS)
				break;
			raw[i] = val;
