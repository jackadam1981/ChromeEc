...
{
	return i2c_write16(I2C_PORT_MP4245, MP4245_ADDR_FLAGS, offset,
			   data);
}
...
int mp4245_votlage_out_enable(int enable)
{
	...
	return i2c_write8(I2C_PORT_MP4245, MP4245_ADDR_FLAGS,
...
int mp3245_get_vbus(int *mv, int *ma)
{
	...
	rv = i2c_read16(I2C_PORT_MP4245, MP4245_ADDR_FLAGS,
...
static void mp4245_dump_reg(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(mp4245_cmds); i++) {
		if (mp4245_cmds[i].len == 1) {
			rv = i2c_read8(I2C_PORT_MP4245, MP4245_ADDR_FLAGS,
...
void mp4245_get_status(void)
{
	...
	i2c_read8(I2C_PORT_MP4245, MP4245_ADDR_FLAGS,
		  MP4245_CMD_OPERATION, &on);
	/* Vbus on/off is bit 7 */
	on >>= 7;

	/* Get status word */
	i2c_read16(I2C_PORT_MP4245, MP4245_ADDR_FLAGS,
