...
/*
 * MAX1463X USB BC 1.2 Charger Detector driver.
 *
 ...
 */
static int is_chg_det_activated(const struct max1463x_config_t * const cfg)
{
	return !!gpio_get_level(cfg->chg_det_pin) ^
		!!(cfg->flags & MAX1463X_FLAGS_CHG_DET_ACTIVE_LOW);
}
...
static void activate_chip_enable(
	const struct max1463x_config_t * const cfg, const int enable)
{
	CPRINTS("bc12: enable = %d level = %d", enable,
		!!enable ^ !!(cfg->flags & MAX1463X_FLAGS_ENABLE_ACTIVE_LOW));
	gpio_set_level(
		cfg->chip_enable_pin,
		!!enable ^ !!(cfg->flags & MAX1463X_FLAGS_ENABLE_ACTIVE_LOW));
}
...
{
	const struct max1463x_config_t * const cfg = &max1463x_config[port];
	...
	CPRINTS("BC 1.2 C%d detection complete: I_Chg = %d mA",
		port, new_chg.current);

	charge_manager_update_charge(CHARGE_SUPPLIER_OTHER, port, &new_chg);
}

/**
 * Turn off the MAX1463X detector.
 *
...
static void detect_or_power_down_ic(const int port)
{
	...
	if (vbus_present) {
		CPRINTS("Starting port %d BC 1.2 Detection", port);
		bc12_detect(port);
	} else {
		CPRINTS("Turning off port %d BC 1.2 chip", port);
		power_down_ic(port);
	}
}
...
{
	/* The MAX1463X automatically sets up the USB 2.0 high-speed switches. */
}
...
{
	/*
	 * Due to the limitations in the application of the MAX1463X, we
...
{
	/* Use the current limit that was decided by the MAX1463X. */
