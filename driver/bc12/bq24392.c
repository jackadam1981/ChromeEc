...
/**
 *  Returns true if the specified port is out of bounds.
 */
static inline int is_port_invalid(const int port)
{
	return port < 0 || port > CONFIG_USB_PD_PORT_COUNT;
}

/**
 * Returns true if the charger detect pin is activated.
 *
 * @parm drv driver for chip to read the charger detect pin for.
 * @return 1 if charger detect is activated (high when active high or
 *	low with active low), otherwise 0.
 */
static int is_chg_det_activated(const struct bq24392_config_t * const drv)
{
	return !!gpio_get_level(drv->chg_det_pin) ^
		!!(drv->flags & BQ24392_FLAGS_CHG_DET_ACTIVE_LOW);
}

/**
 * Activates the Chip Enable GPIO based on the enabled value.
 *
 * @param drv driver for chip that will set chip enable gpio.
 * @param enable 1 to activate gpio (high for active high and low for active
 *	low).
 */
static void activate_chip_enable(
	const struct bq24392_config_t * const drv, const int enable)
{
	gpio_set_level(
		drv->chip_enable_pin,
		!!enable ^ !!(drv->flags & BQ24392_FLAGS_ENABLE_ACTIVE_LOW));
}
...
{
	const struct bq24392_config_t * const drv = &bq24392_config[port];
	 ...
	 */
	new_chg.current = is_chg_det_activated(drv) ? 2400 : 500;
#else
	/*
