...
static void bc12_detect(const int port)
{
	...
	new_chg.current = is_high_power ? 2400 : 100;
#else
	/*
	 * If the board doesn't support charge ramping, then assume the lowest
	 * denominator; that is assume the charger detected is a charging
	 * downstream port (CDP).
	 */
	new_chg.current = is_high_power ? 1500 : 100;
...
static void power_down_ic(const int port)
{
	...
	charge_manager_update_charge(CHARGE_SUPPLIER_NONE, port, &no_chg);
}
