...
/* Regulator: LDO & BUCK */
static int mt6360_regulator_write8(int reg, int val)
{
	/*
...
static int mt6360_regulator_read8(int reg, int *val)
{
	int rv;
...
static int mt6360_regulator_update_bits(int reg, int mask, int val)
{
	int rv;
	int reg_val = 0;

	rv = mt6360_regulator_read8(reg, &reg_val);
	if (rv)
		return rv;
	reg_val &= ~mask;
	reg_val |= (mask & val);
	rv = mt6360_regulator_write8(reg, reg_val);
	return rv;
}
			      ...
			      uint16_t *num_voltages, uint16_t *voltages_mv)
{
	...
	if (rgl_id >= MT6360_REGULATOR_COUNT)
...
int mt6360_regulator_enable(enum mt6360_regulator_id rgl_id, uint8_t enable)
{
	const struct mt6360_regulator_data *data;
...
int mt6360_regulator_is_enabled(enum mt6360_regulator_id rgl_id,
				uint8_t *enabled)
{
	int rv;
	int value;
	...
	rv = mt6360_regulator_read8(data->reg_en_ctrl2, &value);
	if (rv) {
...
int mt6360_regulator_set_voltage(enum mt6360_regulator_id rgl_id, int min_mv,
				 int max_mv)
{
	int i;
	const struct mt6360_regulator_data *data;
...
int mt6360_regulator_get_voltage(enum mt6360_regulator_id rgl_id,
				 int *voltage_mv)
{
	int value;
	int rv;
	...
	rv = mt6360_regulator_read8(data->reg_ctrl3, &value);
	if (rv) {
