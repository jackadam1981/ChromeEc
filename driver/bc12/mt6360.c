			...
			uint16_t *num_voltages, uint16_t *voltages_mv)
{
	...
	switch (ldo_id) {
	case MT6360_LDO3:
		strzcpy(name, "mt6360_ldo3", EC_REGULATOR_NAME_MAX_LEN);
		for (i = 0; i < ARRAY_SIZE(MT6360_LDO3_VOSEL_TABLE); i++) {
			int mv = MT6360_LDO3_VOSEL_TABLE[i];
			if (!mv)
				continue;
		...
		*num_voltages = cnt;
		return EC_SUCCESS;
	case MT6360_LDO5:
		strzcpy(name, "mt6360_ldo5", EC_REGULATOR_NAME_MAX_LEN);
		for (i = 0; i < ARRAY_SIZE(MT6360_LDO5_VOSEL_TABLE); i++) {
			int mv = MT6360_LDO5_VOSEL_TABLE[i];
			if (!mv)
				continue;
			if (cnt < EC_REGULATOR_VOLTAGE_MAX_COUNT) {
				voltages_mv[cnt++] = mv;
			} else {
				CPRINTS("LDO5 Voltage info overflow: %d", mv);
			}
		}
		*num_voltages = cnt;
		return EC_SUCCESS;
...
int mt6360_ldo_set_voltage(enum mt6360_ldo_id ldo_id, int min_mv, int max_mv)
{
	...
	case MT6360_LDO3:
		for (i = 0; i < ARRAY_SIZE(MT6360_LDO3_VOSEL_TABLE); i++) {
			...
			if (mv + 100 < min_mv)
				continue;
			mv = DIV_ROUND_UP(mv, 10) * 10;
			if (mv > max_mv)
				continue;
			step = (mv - MT6360_LDO3_VOSEL_TABLE[i]) / 10;
	...
	case MT6360_LDO5:
		for (i = 0; i < ARRAY_SIZE(MT6360_LDO5_VOSEL_TABLE); i++) {
			...
			if (mv > max_mv)
				continue;
			step = (mv - MT6360_LDO5_VOSEL_TABLE[i]) / 10;
		...
		CPRINTS("LDO5 voltage %d - %d out of range", min_mv, max_mv);
		return EC_ERROR_INVAL;
	default:
		return EC_ERROR_INVAL;
	}
...
int mt6360_ldo_get_voltage(enum mt6360_ldo_id ldo_id, int *voltage_mv)
{
	...
	case MT6360_LDO3:
		...
		*voltage_mv += MIN(10, data & MT6360_MASK_LDO3_VOCAL) * 10;
		return EC_SUCCESS;
	case MT6360_LDO5:
		...
		*voltage_mv += MIN(10, data & MT6360_MASK_LDO5_VOCAL) * 10;
		return EC_SUCCESS;
	default:
		return EC_ERROR_INVAL;
	}
