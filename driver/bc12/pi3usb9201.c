...
static int pi3usb9201_get_status(int port, int *client, int *host)
{
	...
	rv = raw_read8(port, PI3USB9201_REG_CLIENT_STS, client);
	rv |= raw_read8(port, PI3USB9201_REG_HOST_STS, host);
...
static int bc12_detect_start(int port)
{
	 ...
	 */
	pi3usb9201_get_status(port, &client, &host);
	...
	while (1) {
		...
		if (evt & USB_CHG_EVENT_BC12) {
			...
			if (!rv && client)
				 ...
				 * indicates that BC1.2 completion has
				 * completed.
				 */
