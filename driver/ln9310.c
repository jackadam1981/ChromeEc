...
void ln9310_software_enable(int enable)
{
	...
	if (enable ) {
		 ...
		 * (3) if < 100 ms elapsed since (2) -> trigger LN9310 internal
		 * startup seq. (4) else -> abort and optionally retry
		 */
		retry_count = 0;
		 ...
		 * nEN=0
		 * 2. reset cfly precharge related registers to known initial
		 * state
		 */
		field_update8(LN9310_REG_STARTUP_CTRL,
				LN9310_STARTUP_STANDBY_EN,
