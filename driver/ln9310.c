...
void ln9310_software_enable(int enable)
{
	...
	uint64_t precharge_timeout = get_time().val;
	bool ln9310_init_completed = false;
	/*
	* LN9310 startup requires (nEN=0 AND STANDBY_EN=0) where nEN is a pin and
	* STANDBY_EN is a register bit. Previous EC firmware set STANDBY_EN=1 in 
	* ln9310_init and toggled nEN to startup/shutdown.  This function implements 
	* an alternate software (i.e. controlled by EC through I2C commands) startup 
	* sequence so one option is to set nEN=1 and just used ln9310_software_enable
	* to startup/shutdown.  ln9310_software_enable can also be used in conjunction 
	* w/ the nEN pin (in case nEN pin is desired as visible signal ) as follows:
	* 
	...
	*/
	if (enable ) {
		/*
		* Software modification of LN9310 startup sequence w/ retry loop.
		* 
		* (1) Precharge Cfly w/ overrides of internal LN9310 signals
		* (2) disable overrides -> stop precharging Cfly
		* (3) if < 100 ms elapsed since (2) -> trigger LN9310 internal startup seq.
		* (4) else -> abort and optionally retry
		*/
		retry_count = 0;
			...
			*/
			if (get_time().val < precharge_timeout ) {
				...
				field_update8(LN9310_REG_STARTUP_CTRL,
						...
						0);
				if (get_time().val > precharge_timeout ) {
				/* if timed out during prev. I2C command, abort startup attempt  */
				field_update8(LN9310_REG_STARTUP_CTRL,
						LN9310_STARTUP_STANDBY_EN,
						1);
				} else {
		...
		if (retry_count == LN9310_INIT_RETRY_COUNT ) {
			CPRINTS("LN9310 failed to start after %d retry attempts", LN9310_INIT_RETRY_COUNT);
		}
	} else {
		/*
		* Internal LN9310 shutdown sequence is ok as is, so just reset the state to prepare
		* for subsequent startup sequences.  
		*
