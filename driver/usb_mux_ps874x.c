...
static int ps874x_init(int i2c_addr)
{
	...
	uint8_t rev_reg = PS874X_REG_REVISION_ID1;
	int rev_reg_val = PS874X_REVISION_ID1;
#elif defined(CONFIG_USB_MUX_PS8743)
	uint8_t rev_reg = PS874X_REG_REVISION_ID2;
	int rev_reg_val = PS874X_REVISION_ID2;
#endif
	...
	res = ps874x_read(i2c_addr, rev_reg, &val);
	if (res)
		return res;
	if (val != rev_reg_val)
