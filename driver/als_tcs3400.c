	...
	do {
		...
		case TCS_TEST_STAGE_S5:
			...
			for (x = 0; x < 4; x++) {
				...
				if (y == 0)
					g_running_avg[x] = TCS_MAX_ATIME;
				else
					g_running_avg[x] = running_avg_sum / y;
			}
			ALSLOG(TEST, "AGAIN BUMPS = [ %d, %d, %d, %d ",
			       g_running_avg[0], g_running_avg[1],
				    ...
				    uint16_t *raw_light_data, int32_t *xyz_data)
{
	...
	fp_t cover_scale = als_drv_data->als_cal.channel_scale.cover_scale;
	...
	for (i = 0; i < TCS_CHANNEL_COUNT; i++) {
		...
		crgb_data[i] = fp_div_dbz(crgb_data[i], k_channel_scale);
		...
		crgb_data[i] = fp_mul(cover_scale, crgb_data[i]);
...
;
		...
		next_test_setting(s, saturation, percentage);
	}
#else
	if (calibration_mode == TCS_RUN_MODE)
...
{
	fp_t k_channel_scale =
	    TCS3400_DRV_DATA(s)->als_cal.channel_scale.k_channel_scale;

	scale[X] = FP_TO_ZERO_TO_TWO(k_channel_scale);
	scale[Y] = 0;
	scale[Z] = 0;
	*temp = EC_MOTION_SENSE_INVALID_CALIB_TEMP;
	return EC_SUCCESS;
...
static int tcs3400_set_scale(const struct motion_sensor_t *s,
				 ...
				 int16_t temp)
{
	struct crgb_channel_scale_t *scale_p =
	    &TCS3400_DRV_DATA(s)->als_cal.channel_scale;

	scale_p->k_channel_scale = ZERO_TO_TWO_TO_FP(scale[X]);
	return EC_SUCCESS;
}
