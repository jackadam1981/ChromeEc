				     ...
				     uint16_t *crgb_data)
{
	...
	ALSLOG(GRAPH, "%d %d%% (%d:%d)", max_light_val,
	       max_light_val * 100 / TCS_SATURATION_LEVEL,
	...
	if ((status & TCS_I2C_STATUS_ALS_VALID) ||
	    (max_light_val >= TCS_SATURATION_LEVEL)) {
		...
		if (max_light_val < ((TCS_SATURATION_LEVEL * 90) / 100)) {
			/* increase AGAIN if we can without saturating */
			if (max_light_val <= TCS_GAIN_SAT_LEVEL) {
			...
			ALSLOG(SATURATION, "saturation sweet spot (0x%08x < "
			       ...
			       TCS_SATURATION_LEVEL,
			       TCS_SATURATION_LEVEL * 100 / 90);
		}
	}
