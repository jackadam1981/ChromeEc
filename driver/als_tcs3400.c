...
static int tcs3400_get_range(const struct motion_sensor_t *s)
{
	...
	return (drv_data->als_cal.scale << 16) | (drv_data->als_cal.uscale);
}
			     ...
			     int rnd)
{
	...
	drv_data->als_cal.scale = range >> 16;
	drv_data->als_cal.uscale = range & 0xffff;
	return EC_SUCCESS;
}
			...
			int16_t    *temp)
{
	...
	offset[X] = drv_data->als_cal.offset;
	offset[Y] = 0;
	offset[Z] = 0;
			...
			int16_t    temp)
{
	...
	drv_data->als_cal.offset = offset[X];
	return EC_SUCCESS;
}
