...
static int tcs3400_get_saturation_point(uint16_t atime)
{
	/* Calculation from TCS3400 data sheet */
	return MIN((256 - atime) * 1024, 65535);
}

void increment_atime(struct tcs_saturation_t *sat_p)
{
	sat_p->atime = MIN(sat_p->atime + TCS_ATIME_STEP, TCS_MAX_ATIME);
}

void decrement_atime(struct tcs_saturation_t *sat_p)
{
	sat_p->atime = MAX(sat_p->atime - TCS_ATIME_STEP, TCS_MIN_ATIME);
}
 ...
 */
static uint16_t g_saturation_count;

static int
tcs3400_adjust_sensor_for_saturation(struct motion_sensor_t *s,
				     uint16_t *crgb_data)
{
	/* amount sample will change on gain adjustment */
	const uint16_t gain_adjust_factor = 4;
	...
	uint16_t max_light_val =  0;
	uint16_t saturation_level = tcs3400_get_saturation_point(sat_p->atime);
	uint16_t gain_saturation_level = saturation_level / gain_adjust_factor;
	int ret = EC_SUCCESS;
	int status = 0;
	...
	if ((status & TCS_I2C_STATUS_ALS_VALID) ||
		...
		if (max_light_val < saturation_level) {
			/* increase AGAIN if we can */
			if (max_light_val < gain_saturation_level) {
