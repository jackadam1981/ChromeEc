...
/* Turn on TCS3400 Logging */
#define CONFIG_CMD_ALSLOG

#define CPRINTF(fmt, args...) cprintf(CC_ACCEL, "%s "fmt, __func__, ## args)
...
static int command_log_als_data(int argc, char **argv)
{
	...
	CPRINTF("ALS data logging now %sabled", gAlsLogMask ? "en" : "dis");
	if (argc > 1) {
		gAlsLogMask = atoi(argv[1]);
		CPRINTF("ALS data logging mask set to %d", gAlsLogMask);
	}
	return EC_SUCCESS;
}
...
void log(uint16_t light_val, uint16_t lux, struct tcs_saturation_t *sat_p,
		uint16_t *raw_data)
{
	ALSLOG((SATURATION|GRAPH|TEST), "lux: %d, %d%% saturation, GAIN: %d, "
	       "ATIME: %d, [ 0x%x 0x%x 0x%x 0x%x ]",
				     ...
				     uint16_t *crgb_data)
{
	...
	if ((status & TCS_I2C_STATUS_ALS_SATURATED) ||
		...
		if (max_val <= TCS_GAIN_SAT_LEVEL) {
			if (sat_p->again < TCS_MAX_AGAIN) {
				...
				ALSLOG(SATURATION, "%d%%` of "
				       "saturation, dec ATIME to %d",
				       max_val*100/65535, sat_p->atime);
			} else {
			...
			decrement_atime(sat_p, cur_lux, percent_left);
			ALSLOG(SATURATION, "%d%% saturation, dec ATIME to %d",
				       max_val*100/65535, sat_p->atime);
			...
			if (max_val < TCS_GAIN_UPSHIFT_LEVEL) {
				...
				ALSLOG(GRAPH, "increment AGAIN = %d",
				       sat_p->again);
				ALSLOG(SATURATION, "%d%% saturation, "
				       "upshift case (%d, %d)",
				       max_val*100/65535, sat_p->again,
				       sat_p->atime);
			} else {
				ALSLOG(SATURATION, "%d%% saturation, "
					"can't adjust (%d, %d)",
			...
			ALSLOG(SATURATION, "%d%% saturation, "
			       "can't adjust (%d, %d)",
			       max_val*100/65535, sat_p->again, sat_p->atime);
		}
	}
	...
	if (save_atime != sat_p->atime) {
		...
		if (ret)
			return ret;
		ALSLOG(SATURATION, "Set ATIME = %d", sat_p->atime);
	}
				     ...
				     int32_t *crgb_data, int32_t *xyz_data)
{
	...
	/* IR removal */
	ir = FP_TO_INT(fp_div(fp_mul(INT_TO_FP(crgb_data[1] + crgb_data[2] +
				crgb_data[3] - crgb_data[0]),
				rgb_drv_data->calibration.irt), INT_TO_FP(2)));

	ALSLOG(XYZ_XLATE, "IR = %d  ((%d + %d + %d - %d) * %s%d.%04d / 2)", ir,
			crgb_data[1], crgb_data[2], crgb_data[3], crgb_data[0],
			negative_fp(rgb_drv_data->calibration.irt) ?  "-" : "",
			ones_from_fp(rgb_drv_data->calibration.irt),
			remainder_from_fp(rgb_drv_data->calibration.irt));
	...
	for (i = 0; i < 3; i++) {
		...
		if (xyz_data[i] >= MIN_LUX_OFFSET_COMPENSATION) {
			xyz_data[i] += p->offset;
			ALSLOG(XYZ_XLATE, "xyz_data[%d] += %d", i, p->offset);
		}
...
static int tcs3400_post_events(struct motion_sensor_t *s, uint32_t last_ts)
{
	...
	do {
		...
		if (!(data & TCS_I2C_STATUS_RGBC_VALID)) {
			retries--;
			if (retries == 0) {
				CPRINTF("RGBC not valid (0x%x)", data);
	...
	if ((raw_data[CLEAR_CRGB_IDX] != TCS_SATURATION_LEVEL) &&
		...
		if (IS_ENABLED(CONFIG_ACCEL_FIFO)) {
			...
			if (calibration_mode) {
				ALSLOG(PRIORITY, "Sending raw clear data "
				       "[ 0x%04x ]",
				       (unsigned short) vector.data[X]);
			} else {
				struct tcs_saturation_t *satP =
					&TCS3400_RGB_DRV_DATA(s+1)->saturation;

				ALSLOG(PRIORITY, "Sending CLEAR data LUX = "
				       "`%d [%d, %d]",
					(unsigned short) vector.data[X],
					satP->again,
					satP->atime);
			...
			if (raw_data[CLEAR_CRGB_IDX] != TCS_SATURATION_LEVEL) {
				ALSLOG(FLOW, "Clear channel data unchanged "
				       "[ 0x%04x ]",
				       (unsigned short) xyz_data[Y]);
			} else {
				ALSLOG((FLOW|GRAPH), "Clear channel data "
				     "saturated [ 0x%04x ]",
...
static int tcs3400_init(const struct motion_sensor_t *s)
{
	...
	if (ret) {
		CPRINTF("failed reading ID reg 0x%x, ret=%d", TCS_I2C_ID, ret);
		return ret;
	}
	if ((data != TCS340015_DEVICE_ID) && (data != TCS340037_DEVICE_ID)) {
		CPRINTF("no ID match, data = 0x%x", data);
