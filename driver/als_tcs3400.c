...
static int command_log_als_data(int argc, char **argv)
{
	...
	CPRINTS("%s ALS data logging now %sabled", __func__,
		gLogAlsData ? "en" : "dis");
	return EC_SUCCESS;
}
...
static int tcs3400_post_events(struct motion_sensor_t *s, uint32_t last_ts)
{
	...
	do {
		...
		if (!(data & TCS_I2C_STATUS_RGBC_VALID)) {
			...
			CPRINTS("%s RGBC not valid (0x%x)", __func__, data);
			msleep(20);
		}
	...
	if (data < 0) {
		CPRINTS("%s Negative clear val 0x%x set to 0", __func__, data);
		data = 0;
	}
	...
	if (data != drv_data->last_value) {
		...
		ALSLOG("%s Sending Clear channel data (0x%x)",
				__func__, data);

		motion_sense_fifo_add_data(&vector, s, 3, last_ts);
	} else {
		ALSLOG("%s Clear channel data unchanged (0x%x)",
				__func__, data);
	}
...
static int tcs3400_irq_handler(struct motion_sensor_t *s, uint32_t *event)
{
	...
	ALSLOG("%s: status=0x%x", __func__, status);
	...
	if ((status & TCS_I2C_STATUS_RGBC_VALID) ||
		...
		if ((status & TCS_I2C_STATUS_ALS_IRQ) &&
			(status & TCS_I2C_STATUS_ALS_VALID)) {
			CPRINTS("%s clearing CICLEAR", __func__);
			ret = tcs3400_i2c_write8(s, TCS_I2C_CICLEAR, 0);
			if (ret)
				goto unlock;
		}
	}
...
{
	return (TCS3400_GET_DATA(s)->als_cal.scale << 16) |
			(TCS3400_GET_DATA(s)->als_cal.uscale);
}
...
static int tcs3400_set_range(const struct motion_sensor_t *s,
			     ...
			     int rnd)
{
	TCS3400_GET_DATA(s)->als_cal.scale = range >> 16;
	TCS3400_GET_DATA(s)->als_cal.uscale = range & 0xffff;
	return EC_SUCCESS;
}
...
static int tcs3400_get_offset(const struct motion_sensor_t *s,
			      ...
			      int16_t *temp)
{
	offset[X] = TCS3400_GET_DATA(s)->als_cal.offset;
	offset[Y] = 0;
	offset[Z] = 0;
...
static int tcs3400_set_offset(const struct motion_sensor_t *s,
			      ...
			      int16_t temp)
{
	TCS3400_GET_DATA(s)->als_cal.offset = offset[X];
	return EC_SUCCESS;
}

static int tcs3400_get_data_rate(const struct motion_sensor_t *s)
{
	return TCS3400_GET_DATA(s)->rate;
}
...
static int tcs3400_init(const struct motion_sensor_t *s)
{
	...
	if (ret) {
		CPRINTS("%s failed reading ID reg 0x%x, ret=%d", __func__,
			TCS_I2C_ID, ret);
		return ret;
	}
	if ((data != TCS340015_DEVICE_ID) && (data != TCS340037_DEVICE_ID)) {
		CPRINTS("%s no ID match, data = 0x%x", __func__, data);
