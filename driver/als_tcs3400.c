...
static int tcs3400_post_events(struct motion_sensor_t *s, uint32_t last_ts)
{
	...
	rgb_data[X] += rgb_drv_data->rgb_cal.offset[X];
	rgb_data[X] = rgb_data[X] * rgb_drv_data->rgb_cal.scale +
			rgb_data[X] * rgb_drv_data->rgb_cal.uscale / 10000;
	rgb_data[Y] = ((light_data[5] << 8) | light_data[4]);
	rgb_data[Y] += rgb_drv_data->rgb_cal.offset[Y];
	rgb_data[Y] = rgb_data[Y] * rgb_drv_data->rgb_cal.scale +
			rgb_data[Y] * rgb_drv_data->rgb_cal.uscale / 10000;
	rgb_data[Z] = ((light_data[7] << 8) | light_data[6]);
	rgb_data[Z] += rgb_drv_data->rgb_cal.offset[Z];
	rgb_data[Z] = rgb_data[Z] * rgb_drv_data->rgb_cal.scale +
			rgb_data[Z] * rgb_drv_data->rgb_cal.uscale / 10000;
...
{
	return (TCS3400_RGB_DRV_DATA(s)->rgb_cal.scale << 16) |
			TCS3400_RGB_DRV_DATA(s)->rgb_cal.uscale;
}
...
static int tcs3400_rgb_get_offset(const struct motion_sensor_t *s,
				  ...
				  int16_t *temp)
{
	offset[X] = TCS3400_RGB_DRV_DATA(s)->rgb_cal.offset[Y];
	offset[Y] = TCS3400_RGB_DRV_DATA(s)->rgb_cal.offset[Y];
	offset[Z] = TCS3400_RGB_DRV_DATA(s)->rgb_cal.offset[Z];
...
static int tcs3400_rgb_set_offset(const struct motion_sensor_t *s,
				  ...
				  int16_t temp)
{
	TCS3400_RGB_DRV_DATA(s)->rgb_cal.offset[X] = offset[X];
	TCS3400_RGB_DRV_DATA(s)->rgb_cal.offset[Y] = offset[Y];
	TCS3400_RGB_DRV_DATA(s)->rgb_cal.offset[Z] = offset[Z];
	return EC_SUCCESS;
}
