...
static int tcs3400_read(const struct motion_sensor_t *s, intv3_t v)
{
	...
	data = (data & ~TCS_I2C_ENABLE_MASK) | TCS3400_MODE_CONTINUOUS;
...
static int tcs3400_post_events(struct motion_sensor_t *s, uint32_t last_ts)
{
	...
	rgb_data[X] += rgb_drv_data->als_cal[X].offset;
	rgb_data[X] = rgb_data[X] * rgb_drv_data->als_cal[X].scale +
			rgb_data[X] * rgb_drv_data->als_cal[X].uscale / 10000;
	rgb_data[Y] = ((light_data[5] << 8) | light_data[4]);
	rgb_data[Y] += rgb_drv_data->als_cal[Y].offset;
	rgb_data[Y] = rgb_data[Y] * rgb_drv_data->als_cal[Y].scale +
			rgb_data[Y] * rgb_drv_data->als_cal[Y].uscale / 10000;
	rgb_data[Z] = ((light_data[7] << 8) | light_data[6]);
	rgb_data[Z] += rgb_drv_data->als_cal[Z].offset;
	rgb_data[Z] = rgb_data[Z] * rgb_drv_data->als_cal[Z].scale +
			rgb_data[Z] * rgb_drv_data->als_cal[Z].uscale / 10000;
...
static int tcs3400_rgb_get_range(const struct motion_sensor_t *s)
{
	...
	return (TCS3400_RGB_DRV_DATA(s)->als_cal[X].scale << 16) |
			TCS3400_RGB_DRV_DATA(s)->als_cal[X].uscale;
}
...
{
	/* Currently, calibration info is same for all channels */
	for (int i = 0; i < ARRAY_SIZE(TCS3400_RGB_DRV_DATA(s)->als_cal); i++) {
		TCS3400_RGB_DRV_DATA(s)->als_cal[i].scale = range >> 16;
		TCS3400_RGB_DRV_DATA(s)->als_cal[i].uscale = range & 0xffff;
	}
	return EC_SUCCESS;
}
...
static int tcs3400_rgb_get_offset(const struct motion_sensor_t *s,
				  ...
				  int16_t *temp)
{
	offset[X] = TCS3400_RGB_DRV_DATA(s)->als_cal[X].offset;
	offset[Y] = TCS3400_RGB_DRV_DATA(s)->als_cal[Y].offset;
	offset[Z] = TCS3400_RGB_DRV_DATA(s)->als_cal[Z].offset;
...
static int tcs3400_rgb_set_offset(const struct motion_sensor_t *s,
				  ...
				  int16_t temp)
{
	TCS3400_RGB_DRV_DATA(s)->als_cal[X].offset = offset[X];
	TCS3400_RGB_DRV_DATA(s)->als_cal[Y].offset = offset[Y];
	TCS3400_RGB_DRV_DATA(s)->als_cal[Z].offset = offset[Z];
	return EC_SUCCESS;
}
