...
{ 11300, 11300, 11300, 11300, 11300, 5650, 5650, 5650, 3766, 3766, 2825, 2825 },
{ 11300, 11300, 3766, 2260, 1883, 1412, 1130, 1130, 869, 807, 706, 627 },
{ 11300, 1883, 807, 565, 418, 322, 264, 256, 203, 194, 187, 300 },
{ 3766, 418, 207, 0, 0, 0, 0, 0, 0, 0, 0, 0 } };
...
decrement_atime(struct tcs_saturation_t *sat_p, uint16_t cur_lux, int percent)
{
	...
	if (lux > TCS_GAIN_TABLE_MAX_LUX)
		lux = TCS_GAIN_TABLE_MAX_LUX;
 ...
 */
#define TEST_MODE_START 0
#define TEST_MODE_GAIN_WALK 1

static int g_test_mode = TEST_MODE_START;

static int next_test_setting(struct motion_sensor_t *s)
{
	...
	case TEST_MODE_START:
		...
		g_test_mode = TEST_MODE_GAIN_WALK;
		break;

	case TEST_MODE_GAIN_WALK:
		if (sat_p->atime == TCS_MIN_ATIME) {
			sat_p->atime = TCS_MAX_ATIME;
			sat_p->again = (sat_p->again+1) % (TCS_MAX_AGAIN+1);
		} else {
			if (sat_p->again >= 2)
				sat_p->atime = MAX(sat_p->atime -
				   1, TCS_MIN_ATIME);
			else
				sat_p->atime = MAX(sat_p->atime -
				   TCS_ATIME_DEC_STEP, TCS_MIN_ATIME);
		}
...
#endif
	ALSLOG(SATURATION, "Saturation level=0x%x : "
		"0x%x 0x%x 0x%x 0x%x",
		TCS_SATURATION_LEVEL, crgb_data[0],
				     ...
				     int32_t *crgb_data, int32_t *xyz_data)
{
	...
	for (i = 0; i < 3; i++) {
		...
		xyz_data[i] = p->offset +
			(uint32_t) FP_TO_INT(fp_mul(p->coeff[RED_CRGB_IDX],
				INT_TO_FP(crgb_prime[RED_CRGB_IDX]))) +
			(uint32_t) FP_TO_INT(fp_mul(p->coeff[GREEN_CRGB_IDX],
				INT_TO_FP(crgb_prime[GREEN_CRGB_IDX]))) +
			(uint32_t) FP_TO_INT(fp_mul(p->coeff[BLUE_CRGB_IDX],
				INT_TO_FP(crgb_prime[BLUE_CRGB_IDX]))) +
			(uint32_t) FP_TO_INT(fp_mul(p->coeff[CLEAR_CRGB_IDX],
				INT_TO_FP(crgb_prime[CLEAR_CRGB_IDX])));
		...
		ALSLOG(XYZ_XLATE, "xyz_data[%d] = %d + %d + %d + %d + %d "
		       "(0x%08x)", i, p->offset,
			(uint32_t) FP_TO_INT(fp_mul(p->coeff[RED_CRGB_IDX],
			    (uint32_t) INT_TO_FP(crgb_prime[RED_CRGB_IDX]))),
			(uint32_t) FP_TO_INT(fp_mul(p->coeff[GREEN_CRGB_IDX],
			    (uint32_t) INT_TO_FP(crgb_prime[GREEN_CRGB_IDX]))),
			(uint32_t) FP_TO_INT(fp_mul(p->coeff[BLUE_CRGB_IDX],
			    (uint32_t) INT_TO_FP(crgb_prime[BLUE_CRGB_IDX]))),
			(uint32_t) FP_TO_INT(fp_mul(p->coeff[CLEAR_CRGB_IDX],
				(uint32_t) INT_TO_FP(
				      (uint32_t) crgb_prime[CLEAR_CRGB_IDX]))),
			(uint32_t) FP_TO_INT(fp_mul(p->coeff[CLEAR_CRGB_IDX],
				(uint32_t) INT_TO_FP(
				      (uint32_t) crgb_prime[CLEAR_CRGB_IDX]))));
...
{
	struct als_drv_data_t *als_drv_data =
			TCS3400_DRV_DATA(s);
	...
	fp_t cover_scale =
		als_drv_data->als_cal.channel_scale.cover_scale;
	int i;
#ifdef NORMALIZE_DATA
	int32_t old;
#endif
	...
	for (i = 0; i < TCS_CHANNEL_COUNT; i++) {
		...
		ALSLOG((SCALING|RAW_DATA), "raw: crgb_data[%d] = %d (0x%04x)",
		...
		ALSLOG(SCALING, "kscale: crgb_data[%d] = %d (%d / %s%d.%d)", i,
				(uint16_t) FP_TO_INT(fp_div_dbz(INT_TO_FP(
		...
		crgb_data[i] = (uint16_t) FP_TO_INT(fp_div_dbz(
						    INT_TO_FP(crgb_data[i]),
						    k_channel_scale));

		/* compensate for the light cover */
		ALSLOG(SCALING, "cover: rgb_data[%d] = %d (%d * %s%d.%d)", i,
			FP_TO_INT(fp_mul(cover_scale,
					INT_TO_FP(crgb_data[i]))),
			(uint32_t) crgb_data[i],
			negative_fp(cover_scale) ?  "-" : "",
			ones_from_fp(cover_scale),
		...
		crgb_data[i] = (uint16_t) FP_TO_INT(fp_mul(cover_scale,
						    INT_TO_FP(crgb_data[i])));
		...
		crgb_data[i] = normalize_channel_data(s,
						      (uint32_t) crgb_data[i]);
		ALSLOG(TEST, "Normalize:  was %d, now %d", (uint32_t) old,
			      (uint32_t) crgb_data[i]);
#endif
	}

	ALSLOG(SCALING, "");
	ALSLOG(SCALING, "crgb = [ 0x%04x, 0x%04x, 0x%04x, 0x%04x ",
			crgb_data[0], crgb_data[1], crgb_data[2], crgb_data[3]);

	ALSLOG(SCALING, "crgb = [ %d, %d, %d, %d ",
			crgb_data[0], crgb_data[1], crgb_data[2], crgb_data[3]);
	ALSLOG(SCALING, "");

	if ((calibration_mode == TCS_RUN_MODE) &&
		IS_ENABLED(CONFIG_RGB_XYZ_XLATE)) {
...
static int tcs3400_post_events(struct motion_sensor_t *s, uint32_t last_ts)
{
	...
	ALSLOG(GRAPH, "RAW VALUES : 0x%04x 0x%04x 0x%04x 0x%04x [%d, %d",
	       raw_data[0], raw_data[1], raw_data[2], raw_data[3],
	...
	if ((raw_data[CLEAR_CRGB_IDX] != TCS_SATURATION_LEVEL) &&
		...
		if (calibration_mode)
			...
			ALSLOG(PRIORITY, "Sending CLEAR data LUX = %d ",
			       (unsigned short) vector.data[X]);
#endif
	} else {
		if (raw_data[CLEAR_CRGB_IDX] != TCS_SATURATION_LEVEL) {
	...
	if (((last_v[X] != xyz_data[X]) || (last_v[Y] != xyz_data[Y]) ||
		...
		ALSLOG(PRIORITY, "Sending %s data [ 0x%04x 0x%04x 0x%04x ",
			calibration_mode ? "raw RGB" : "RGB XYZ",
			(unsigned short) vector.data[X],
...
#ifdef TEST_MODE
	{
		...
		if ((raw_data[0] == 0xffff) ||
		    ...
		    (raw_data[2] == 0xffff) ||
		    (raw_data[3] == 0xffff)) {
			/* wrap once we saturate */
			TCS3400_RGB_DRV_DATA(s+1)->saturation.again = 0;
			TCS3400_RGB_DRV_DATA(s+1)->saturation.atime =
					TCS_MAX_ATIME;
		} else {
			next_test_setting(s);
				 ...
				 int16_t *temp)
{
	...
	scale[X] =
		FP_TO_ZERO_TO_TWO(rgb_cal[RED_RGB_IDX].scale.k_channel_scale);
	scale[Y] =
		FP_TO_ZERO_TO_TWO(rgb_cal[GREEN_RGB_IDX].scale.k_channel_scale);
	scale[Z] =
		FP_TO_ZERO_TO_TWO(rgb_cal[BLUE_RGB_IDX].scale.k_channel_scale);
				 ...
				 int16_t temp)
{
	...
	rgb_cal[RED_RGB_IDX].scale.k_channel_scale =
			ZERO_TO_TWO_TO_FP(scale[X]);
	rgb_cal[GREEN_RGB_IDX].scale.k_channel_scale =
			ZERO_TO_TWO_TO_FP(scale[Y]);
	rgb_cal[BLUE_RGB_IDX].scale.k_channel_scale =
			ZERO_TO_TWO_TO_FP(scale[Z]);
	return EC_SUCCESS;
}
...
{
	TCS3400_RGB_DRV_DATA(s+1)->calibration_mode = TCS_CAL_MODE;
	return EC_SUCCESS;
}
#else
static int tcs3400_rgb_perform_calib(const struct motion_sensor_t *s,
