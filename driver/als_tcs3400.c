...
static int tcs3400_post_events(struct motion_sensor_t *s, uint32_t last_ts)
{
	...
	if ((raw_data[CLEAR_CRGB_IDX] != TCS_SATURATION_LEVEL) &&
		...
		if (s->flags & MOTIONSENSE_FLAG_IN_SPOOF_MODE) {
			for (i = 0; i < 3; i++)
				vector.udata[i] = last_v[i] = s->spoof_xyz[i];
		}
	...
	if (((last_v[X] != xyz_data[X]) || (last_v[Y] != xyz_data[Y]) ||
		...
		vector.flags = 0;
		if (calibration_mode) {
			vector.udata[0] = raw_data[RED_CRGB_IDX];
			vector.udata[1] = raw_data[GREEN_CRGB_IDX];
			vector.udata[2] = raw_data[BLUE_CRGB_IDX];
			...
			for (i = 0; i < 3; i++) {
				last_v[i] = xyz_data[i];
				vector.udata[i] =
					ec_motion_sensor_clamp_u16(last_v[i]);
			}
		}
#ifdef CONFIG_ACCEL_SPOOF_MODE
		if (rgb_s->flags & MOTIONSENSE_FLAG_IN_SPOOF_MODE) {
			for (i = 0; i < 3; i++) {
				vector.udata[i] = last_v[i] = rgb_s->spoof_xyz[i];
			}
		}
#endif /* CONFIG_ACCEL_SPOOF_MODE */
