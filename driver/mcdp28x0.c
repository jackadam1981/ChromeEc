 ...
 *
 * NOTE, must call finalize_checksum prior to sending or validating a received
 * checksum.
 *
 * @seed initial checksum value.  0 for first call or result of previous
 *       otherwise.
 * @msg message bytes to compute checksum on.
 * @cnt count of message bytes.
 * @return partial checksum.
 */
static uint8_t partial_checksum(uint8_t seed, const uint8_t *msg, int cnt)
{
	int i;
	int chksum = seed;
	for (i = 0; i < cnt; i++)
 ...
 * Finalize checksum
 *
 * @return final checksum  (two's complement of sum of msg)
 */
static inline uint8_t finalize_checksum(int chksum)
{
	return ~chksum + 1;
}
...
static int tx_serial(const uint8_t *msg, int cnt)
{
	...
	chksum = partial_checksum(0, &out, 1);

	if (out_stream_write(&usart_mcdp.out, msg, cnt) != cnt)
		return EC_ERROR_UNKNOWN;

	chksum = partial_checksum(chksum, msg, cnt);
	chksum = finalize_checksum(chksum);
...
int mcdp_get_info(struct mcdp_info  *info)
{
	...
	memcpy(info, &inbuf[2], MCDP_RSP_LEN(MCDP_LEN_GETINFO));
