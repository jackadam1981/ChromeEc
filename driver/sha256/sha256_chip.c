...
#define _sha256_pseudo_block __attribute__((section(".__sha256_pseudo_block")))
...
const _sha256_pseudo_block uint32_t sha256_pseudo[128] __keep = {0xffffffff};

void SHA256_init(struct sha256_ctx *ctx)
{
	int i;

	/* clear block of h and w */
	memset(ctx, 0, 256);
	for (i = 0; i < 8; i++)
		ctx->h[i] = sha256_h0[i];
	for (i = 0; i < 64; i++)
...
void SHA256_chip_calculation(void)
{
...
void SHA256_update(struct sha256_ctx *ctx, const uint8_t *data, uint32_t len)
{
	...
	while (rem_len) {
		...
		cal_len = rem_len < SHA256_BLOCK_SIZE ?
