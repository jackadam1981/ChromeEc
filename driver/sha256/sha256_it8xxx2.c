...
{
	uint32_t block_len, cal_len, rem_len = len, rem_w_index, data_index = 0;
	...
	while (rem_len) {
		rem_w_index = ctx->w_index;

		block_len = (rem_len + (ctx->w_index * 4)) < SHA256_BLOCK_SIZE ?
				    (rem_len + (ctx->w_index * 4)) :
				    SHA256_BLOCK_SIZE;

		cal_needed = (block_len == SHA256_BLOCK_SIZE) ? true : false;

		for (; (ctx->w_index * 4) < block_len; ctx->w_index++) {
			ctx->w[ctx->w_index] = htobe32(p[data_index]);
			data_index++;
		}

		cal_len = (ctx->w_index - rem_w_index) * 4;
		rem_len -= cal_len;
