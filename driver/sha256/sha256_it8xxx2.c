...
{
	uint32_t cal_len, rem_len = len, p_index = 0;
	...
	while (rem_len) {
		...
		for (w_index = 0; (w_index * 4) < cal_len; w_index++, p_index++) {
			ctx->w[w_index] = htobe32(p[p_index]);
		}
...
uint8_t *SHA256_final(struct sha256_ctx *ctx)
{
	...
	//msg case:   56B           60B           64B
	//update():   w_index = 13, w_index = 14, w_index = 15, loop end w_index++
	//final(): so w_index = 14, w_index = 15, w_index = 16
	//not enough pad 1b and bit len, or
	//can pad 1b but not enough bit len
	//need cal first then pad rest
	//else
	//enough pad 1b and bit len
	if (w_index >= 16) {
		/* Message buffer not enough pad 1b and bit len */
		SHA256_chip_calculation();
		...
		/* Message buffer can pad 1b but not enough bit len */
		ctx->w[w_index++] = 0x80000000;
		for (; (w_index * 4) < SHA256_BLOCK_SIZE; w_index++) {
			ctx->w[w_index] = 0;
		}
		SHA256_chip_calculation();
		...
		/* Message buffer can pad 1b and bit len enough */
		ctx->w[w_index++] = 0x80000000;
		for (; (w_index * 4) < SHA256_BLOCK_SIZE; w_index++) {
			ctx->w[w_index] = 0;
		}
