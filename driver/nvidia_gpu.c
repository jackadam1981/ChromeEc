...
static void set_d_notify_level(enum d_notify_level level)
{
	...
	*memmap_gpu = (*memmap_gpu & ~MEMMAP_D_NOTIFY_MASK) | d_notify_level;
	...
	CPRINTS("Set new D-notify level to D%c", ('1' + (int)d_notify_level));
}
...
static void evaluate_d_notify_level(void)
{
	...
	const int watts = on_ac ? charge_manager_get_power_limit_uw() : 0;
	...
	if (on_ac) {
		for (lvl = D_NOTIFY_1; lvl < D_NOTIFY_COUNT; lvl++) {
