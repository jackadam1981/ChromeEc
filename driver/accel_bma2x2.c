...
static int set_range(const struct motion_sensor_t *s, int range, int rnd)
{
	...
	/* Set output data rate. */	ret = raw_write8(s->port, s->addr, reg, reg_val);
...
static int set_data_rate(const struct motion_sensor_t *s, int rate, int rnd)
{
	...
	index = find_param_index(rate, rnd, datarates,
				 ARRAY_SIZE(datarates));
...
static int read(const struct motion_sensor_t *s, vector_3_t v)
{
	 ...
	 *
	 * acc[0] = X_AXIS_LSB -> bit 7~4 for value, bit 0 for data bit
	 * acc[1] = X_AXIS_MSB
	 * acc[2] = Y_AXIS_LSB -> bit 7~4 for value, bit 0 for data bit
	 * acc[3] = Y_AXIS_MSB
	 * acc[4] = Z_AXIS_LSB -> bit 7~4 for value, bit 0 for data bit
	 * acc[5] = Z_AXIS_MSB
	 *
	 * Add calibration offset before returning the data.
	 */
	for (i = X; i <= Z; i++)
		v[i] = (((int8_t)acc[i * 2 + 1]) << 8) | (acc[i * 2] & 0xe);
...
static int init(const struct motion_sensor_t *s)
{
	...
	do {
		...
		/* Check for timeout. */
		if (timeout++ > SENSOR_ENABLE_ATTEMPTS) {
