 ...
 */
static inline int raw_read8__8b(const int port, const int addr, const int reg,
			    int *data_ptr)
{
 ...
 */
static inline int raw_write8__8b(const int port, const int addr, const int reg,
			     int data)
{
...
static int set_range(const struct motion_sensor_t *s, int range, int rnd)
{
	...
	ret = raw_read8__8b(s->port, s->addr__8b, BMA2x2_RANGE_SELECT_ADDR,
			&range_reg_val);
	if (ret != EC_SUCCESS) {
	...
	ret = raw_write8__8b(s->port, s->addr__8b, BMA2x2_RANGE_SELECT_ADDR, reg_val);
...
static int set_data_rate(const struct motion_sensor_t *s, int rate, int rnd)
{
	...
	/* Determine the new value of control reg and attempt to write it. */
	ret = raw_read8__8b(s->port, s->addr__8b, BMA2x2_BW_SELECT_ADDR, &odr_reg_val);
	if (ret != EC_SUCCESS) {
		mutex_unlock(s->mutex);
		return ret;
	}
	...
	ret = raw_write8__8b(s->port, s->addr__8b, BMA2x2_BW_SELECT_ADDR, reg_val);
		      ...
		      int16_t *temp)
{
	...
	for (i = X; i <= Z; i++) {
		ret = raw_read8__8b(s->port, s->addr__8b, BMA2x2_OFFSET_X_AXIS_ADDR + i,
				&val);
		if (ret)
			return ret;
...
static int read(const struct motion_sensor_t *s, intv3_t v)
{
	...
	mutex_lock(s->mutex);
	ret = i2c_read_block__7b(s->port, s->addr__8b >> 1, BMA2x2_X_AXIS_LSB_ADDR, acc, 6);
	mutex_unlock(s->mutex);
...
static int perform_calib(const struct motion_sensor_t *s)
{
	...
	ret = raw_read8__8b(s->port, s->addr__8b, BMA2x2_OFFSET_CTRL_ADDR, &val);
	if (ret)
		return ret;
	...
	raw_write8__8b(s->port, s->addr__8b, BMA2x2_OFC_SETTING_ADDR, val);

	for (i = X; i <= Z; i++) {
		val = (i + 1) << BMA2x2_OFFSET_TRIGGER_OFF;
		raw_write8__8b(s->port, s->addr__8b, BMA2x2_OFFSET_CTRL_ADDR, val);
		/*
		...
		do {
			...
			msleep(50);
			ret = raw_read8__8b(s->port, s->addr__8b,
...
static int init(const struct motion_sensor_t *s)
{
	...
	ret = raw_read8__8b(s->port, s->addr__8b, BMA2x2_CHIP_ID_ADDR, &val);
	if (ret)
	...
	ret = raw_read8__8b(s->port, s->addr__8b, reg, &val);
	if (ret != EC_SUCCESS) {
		mutex_unlock(s->mutex);
		return ret;
	}
	val |= reset_field;
	ret = raw_write8__8b(s->port, s->addr__8b, reg, val);
	if (ret != EC_SUCCESS) {
		mutex_unlock(s->mutex);
		return ret;
	}
