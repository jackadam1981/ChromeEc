...
static int elan_tp_read_block(uint16_t reg, uint8_t *val, uint16_t len)
{
	...
	rv = i2c_xfer(CONFIG_TOUCHPAD_I2C_PORT, CONFIG_TOUCHPAD_I2C_ADDR,
		      buf, sizeof(buf), val, len, I2C_XFER_SINGLE);
	if (rv)
		CPRINTS("read block error");

	return rv;
}

static int elan_tp_read_cmd(uint16_t reg, uint8_t *val)
{
	int rv;

	rv = elan_tp_read_block(reg, val, ETP_I2C_INF_LENGTH);
	if (rv)
		CPRINTS("read cmd error");
...
{
	int rv;
	uint8_t val[ETP_I2C_INF_LENGTH];

	CPRINTS("ELAN_TP_init");
	elan_tp_write_cmd(ETP_I2C_STAND_CMD, ETP_I2C_RESET);
	msleep(100);
	rv = i2c_xfer(CONFIG_TOUCHPAD_I2C_PORT, CONFIG_TOUCHPAD_I2C_ADDR,
		      NULL, 0, val, ETP_I2C_INF_LENGTH, I2C_XFER_SINGLE);
	CPRINTS("reset rv %d buf=%04x", rv, *((uint16_t *)val));
	if (rv)
		goto error;

	/* FIXME: Linux driver reads (but throws away) these 2 descriptors,
	 * we can't really do the same without overflowing the stack...
	 */
#if 0
...
#endif
	/* Read min/max */
	rv = elan_tp_read_cmd(ETP_I2C_MAX_X_AXIS_CMD,
			      (uint8_t *)&elan_tp_params.max_x);
	if (rv)
		goto error;
	rv = elan_tp_read_cmd(ETP_I2C_MAX_Y_AXIS_CMD,
			      (uint8_t *)&elan_tp_params.max_y);
	if (rv)
		goto error;

	/* Read min/max */
	rv = elan_tp_read_cmd(ETP_I2C_XY_TRACENUM_CMD, val);
	if (rv)
		goto error;
	if (val[0] == 0 || val[1] == 0) {
	...
	rv = elan_tp_read_cmd(ETP_I2C_PRESSURE_CMD, val);
	if (rv)
		goto error;
	...
	if (rv)
		goto error;
	CPRINTS("ELAN_TP_init done");

	return 0;

error:
	CPRINTS("ELAN_TP_init error");
	return rv;
}
