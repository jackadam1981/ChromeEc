...
{
	uint8_t page_store[elan_tp_params.page_size + 4];
	uint16_t checksum = 0;
	uint16_t rx_buf;
	int i, rv;
	...
	memcpy(page_store + 2, data, elan_tp_params.page_size);
	page_store[elan_tp_params.page_size + 2 + 0] = checksum & 0xff;
	page_store[elan_tp_params.page_size + 2 + 1] = (checksum >> 8) & 0xff;
	...
	if (rv)
		return rv;
	msleep(elan_tp_params.page_size >= 512 ? 50 : 30);
