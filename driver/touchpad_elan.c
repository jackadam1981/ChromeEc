...
static void elan_tp_init_deferred(void)
{
	elan_tp_init();  /* Ignore the return value */
...
int touchpad_update_page(const uint8_t *data)
{
	uint8_t page_store[FW_PAGE_SIZE + 4];
	uint16_t rx_buf = 0;
	int rv;

	for (rv = 0; rv < FW_PAGE_SIZE; rv += 2)
		rx_buf += ((uint16_t)(data[rv + 1]) << 8) | (data[rv]);

	page_store[0] = ETP_I2C_IAP_REG_L;
	page_store[1] = ETP_I2C_IAP_REG_H;
	memcpy(page_store + 2, data, FW_PAGE_SIZE);
	page_store[FW_PAGE_SIZE + 2 + 0] = rx_buf & 0xff;
	page_store[FW_PAGE_SIZE + 2 + 1] = (rx_buf >> 8) & 0xff;
...
DECLARE_DEFERRED(elan_tp_init_deferred);
...
int touchpad_update_write(int offset, int size, const uint8_t *data)
{
	...
	if (offset + size == FW_SIZE) {
		...
		hook_call_deferred(&elan_tp_init_deferred_data, 600 * 1000);
	}
	return EC_SUCCESS;
}
#endif
