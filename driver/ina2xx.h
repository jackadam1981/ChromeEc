...
#define INA2XX_TYPE_231       1
#define INA2XX_TYPE_219       1
...
#define INA219_CALIB_1MA(rsense_mohm) (40960/(rsense_mohm))

/* Bus voltage: mV per LSB */
#define INA231_BUS_MV(reg) ((reg) * 125 / 100)
#define INA219_BUS_MV(reg) ((reg) / 2)

/* Shunt voltage: uV per LSB */
#define INA231_SHUNT_UV(reg) ((reg) * 25 / 10)
#define INA219_SHUNT_UV(reg) ((reg) * 20 / 10)

/* Power LSB: mW per current LSB */
#define INA231_POW_MW(reg) ((reg) * 25 * 1/*Current mA/LSB*/)
#define INA219_POW_MW(reg) ((reg) * 20 * 1/*Current mA/LSB*/)
...
int ina2xx_init(uint8_t idx, uint16_t config, uint16_t calib, int ina_type);
