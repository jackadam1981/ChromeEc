...
static volatile uint32_t last_read_timestamp;
#endif
...
int lis2mdl_read(const struct motion_sensor_t *s, intv3_t v)
{
	...
	if (ret == EC_SUCCESS) {
#ifdef CONFIG_ACCEL_FIFO
		last_read_timestamp = __hw_clock_source_read();
#endif
		lis2mdl_normalize(s, v, raw);
		rotate(v, *s->rot_standard_ref, v);
	}
...
/**
 * Interrupt request handler - bottom half of the interrupt stack
 *
 * @param s Motion sensor pointer
 * @param event Pointer to the event code
 * @return EC_SUCCESS when the interrupt was handled
 */
static int irq_handler(struct motion_sensor_t *s, uint32_t *event)
{
	...
	uint32_t timestamp = last_read_timestamp;
	int odr = LIS2MDL_ST_DATA(s)->base.odr;

	ret = lis2mdl_read(s, s->raw_xyz);

	if (ret == EC_SUCCESS && odr > 0 && timestamp != last_read_timestamp) {
		vector.flags = 0;
		vector.data[0] = s->raw_xyz[0];
		vector.data[1] = s->raw_xyz[1];
		vector.data[2] = s->raw_xyz[2];
		vector.sensor_num = s - motion_sensors;
		motion_sense_fifo_add_data(&vector, s, 3, last_read_timestamp);
	}
#endif
...
int lis2mdl_init(const struct motion_sensor_t *s)
{
	...
	ret = st_raw_write8(s->port, LIS2MDL_ADDR, LIS2MDL_INT_CTRL_REG, 0);
	if (ret != EC_SUCCESS)
		goto lis2mdl_init_error;
...
int lis2mdl_set_data_rate(const struct motion_sensor_t *s, int rate, int rnd)
{
	...
	if (ret == EC_SUCCESS) {
		ret = st_raw_write8(s->port, LIS2MDL_ADDR, LIS2MDL_INT_CTRL_REG,
				rate > 0 ? 1 : 0);
	}
	mutex_unlock(s->mutex);
