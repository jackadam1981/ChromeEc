...
static int lis2mdl_is_data_ready(const struct motion_sensor_t *s, int *ready)
{
	...
	ret = st_raw_read8__8b(s->port, s->addr__8b, LIS2MDL_STATUS_REG, &tmp);
	if (ret != EC_SUCCESS) {
		*ready = 0;
		return ret;
	}
...
int lis2mdl_read(const struct motion_sensor_t *s, intv3_t v)
{
	...
	mutex_lock(s->mutex);
	ret = st_raw_read_n__8b(s->port, s->addr__8b, LIS2MDL_OUT_REG, raw,
			    OUT_XYZ_SIZE);
	mutex_unlock(s->mutex);
	if (ret == EC_SUCCESS) {
		lis2mdl_normalize(s, v, raw);
...
int lis2mdl_init(const struct motion_sensor_t *s)
{
	...
	do {
		ret = st_raw_read8__8b(s->port, LIS2MDL_ADDR__7b << 1, LIS2MDL_WHO_AM_I_REG,
				   &who_am_i);
		if (ret != EC_SUCCESS) {
	...
	/* Reset the sensor */
	ret = st_raw_write8__8b(s->port, LIS2MDL_ADDR__7b << 1, LIS2MDL_CFG_REG_A_ADDR,
...
int lis2mdl_set_data_rate(const struct motion_sensor_t *s, int rate, int rnd)
{
	...
	mutex_lock(s->mutex);
	if (rate <= 0) {
		ret = st_raw_write8__8b(s->port, LIS2MDL_ADDR__7b << 1,
				    LIS2MDL_CFG_REG_A_ADDR,
				    LIS2MDL_FLAG_SW_RESET);
	} else {
