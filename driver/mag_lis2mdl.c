...
{
	int ret = EC_SUCCESS, tmp = 0;
	uint8_t raw[OUT_XYZ_SIZE];

	ret = lis2mdl_is_data_ready(s, &tmp);
	if (ret != EC_SUCCESS)
		return ret;
...
{
	int ret = EC_ERROR_UNIMPLEMENTED, tmp, count = 10;
	struct mag_cal_t *cal = LIS2MDL_CAL(s);
	struct stprivate_data *data = s->drv_data;
...
int lis2mdl_set_data_rate(const struct motion_sensor_t *s, int rate, int rnd)
{
	...
	if (reg_val < 0) {
		...
		reg_val = reg_val | LIS2MDL_MODE_CONT |
			LIS2MDL_FLAG_TEMP_COMPENSATION;
