...
static int lis2dw12_load_fifo(struct motion_sensor_t *s)
{
	...
	do {
		ret = raw_read8(s->port, s->addr, LIS2DW12_FIFO_SAMPLES_ADDR, &tmp);
		if (ret != EC_SUCCESS)
			return ret;
		...
		ret = st_raw_read_n(s->port, s->addr, LIS2DW12_OUT_X_L_ADDR, fifo,
				    nsamples);
		if (ret != EC_SUCCESS)
			return ret;
...
static int lis2dw12_config_interrupt(const struct motion_sensor_t *s)
{
	...
	/* Enable s/d tap detection on int_1 pad. */
