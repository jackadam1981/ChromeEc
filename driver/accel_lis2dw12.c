...
/* Only when configured as base accel sensor, fifo an interrupt
 * are supported.
 */
...
static int set_data_rate(const struct motion_sensor_t *s, int rate, int rnd)
{
	...
	/* lis2dwl support 14 bit resolution only at high performance mode,
	 * and it will always stay at high performance mode from initialization.
	 * But lis2dw12 need switch low power mode according to odr value.
	 */
...
static int init(const struct motion_sensor_t *s)
{
	...
	/* lis2dwl support 14bit resolution only at high perfomance mode */
	ret = set_power_mode(s, LIS2DW12_HIGH_PERF, 0);
#else
