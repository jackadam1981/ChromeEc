...
static int it8801_ioex_init(int ioex)
{
	...
	it8801_write(IT8801_REG_GPIO00_KSO19, IT8801_REG_MASK_GPIOAFS_FUNC1);
...
static int it8801_ioex_get_level(int ioex, int port, int mask, int *val)
{
	...
	it8801_ioex_read(ioex, IT8801_REG_GPIO_IPSR(port), val);
...
static int it8801_ioex_set_level(int ioex, int port, int mask, int value)
{
	...
	value ? (val |= mask) : (val &= ~mask);

	it8801_ioex_write(ioex, IT8801_REG_GPIO_SOVR(port), val);
		...
		int mask, int *flags)
{
	...
	it8801_ioex_read(ioex, IT8801_REG_GPIO_CR(port, mask), &val);

	/* Select open drain 0:push-pull 1:open-drain */
	(val & IT8801_GPIODIR) ? (*flags |= GPIO_OUTPUT)
				: (*flags |= GPIO_INPUT);
	...
	(val & mask) ? (*flags |= GPIO_HIGH) : (*flags |= GPIO_LOW);
		...
		int mask, int flags)
{
	...
	it8801_ioex_read(ioex, IT8801_REG_GPIO_CR(port, mask), &val);
	it8801_ioex_read(ioex, IT8801_REG_GPIO_SOVR(port), &val_level);

	/* Select open drain 0:push-pull 1:open-drain */
	(flags & GPIO_OPEN_DRAIN) ? (val |= IT8801_GPIOIOT)
				: (val &= ~IT8801_GPIOIOT);

	/* Select GPIO direction */
	if (flags & GPIO_OUTPUT) {
		/* Configure the output level */
		(flags & GPIO_HIGH) ? (val_level |= mask)
				: (val_level &= ~mask);
		val |= IT8801_GPIODIR;
	}
	else
	...
	it8801_ioex_write(ioex, IT8801_REG_GPIO_CR(port, mask), val);
	it8801_ioex_write(ioex, IT8801_REG_GPIO_SOVR(port), val_level);
