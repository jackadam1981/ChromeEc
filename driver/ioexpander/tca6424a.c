...
int tca6424a_write_bit(int port, enum tca6424a_bank bank, uint8_t bit, int val)
{
	...
	ret = i2c_read8(port, TCA6424A_ADDR, bank, &tmp);
	if (ret != EC_SUCCESS)
		return ret;
	...
	ret = i2c_write8(port, TCA6424A_ADDR, bank, tmp);
	if (ret != EC_SUCCESS)
		return ret;
...
int tca6424a_write_byte(int port, enum tca6424a_bank bank, uint8_t val)
{
	...
	ret = i2c_write8(port, TCA6424A_ADDR, bank, val);
	if (ret != EC_SUCCESS)
		return ret;
...
int tca6424a_read_byte(int port, enum tca6424a_bank bank)
{
	...
	if (i2c_read8(port, TCA6424A_ADDR, bank, &tmp) != EC_SUCCESS)
		return -1;
