 ...
 */
uint8_t pcal6408_int_mask[CONFIG_IO_EXPANDER_PORT_COUNT] = { 0 };
...
static int pcal6408_ioex_get_level(int ioex, int port, int mask, int *val)
{
	...
	rv = pcal6408_read(ioex, PCAL6408_INPUT, val);
	if (rv != EC_SUCCESS)
		return rv;
...
static int pcal6408_ioex_set_level(int ioex, int port, int mask, int value)
{
	...
	rv = pcal6408_read(ioex, PCAL6408_OUTPUT, &val);
	if (rv != EC_SUCCESS)
		return rv;
	...
	return pcal6408_write(ioex, PCAL6408_OUTPUT, val);
}
					...
					int *flags)
{
	...
	*flags = GPIO_FLAG_NONE;
	...
	rv = pcal6408_read(ioex, PCAL6408_CONFIG, &val)
	if (rv != EC_SUCCESS)
		return rv;
	...
	rv = pcal6408_read(ioex, PCAL6408_OUT_CONFIG, &val);
	if (rv != EC_SUCCESS)
		return rv;
	...
	rv = pcal6408_read(ioex, PCAL6408_PULL_ENABLE, &val);
	if (rv != EC_SUCCESS)
		return rv;

	if (val & mask) {
		rv = pcal6408_read(ioex, PCAL6408_PULL_UP_DOWN, &val);
		if (rv != EC_SUCCESS)
			return rv;
	...
	rv = pcal6408_read(ioex, PCAL6408_INT_MASK, &val);
	if (rv != EC_SUCCESS)
		return rv;
					...
					int flags)
{
	...
	if (flags & GPIO_OPEN_DRAIN)
		val = PCAL6408_OUT_OPEN_DRAIN;
	else
		val = 0;

	rv = pcal6408_write(ioex, PCAL6408_OUT_CONFIG, val);
	if (rv != EC_SUCCESS)
		return rv;
	...
	rv = pcal6408_write(ioex, PCAL6408_CONFIG, val);
	if(rv != EC_SUCCESS)
		return rv;
	...
	if (flags & GPIO_OUTPUT) {
		...
		rv = pcal6408_write(ioex, PCAL6408_OUTPUT, &val);
		if (rv != EC_SUCCESS)
			return rv;
	}
	...
	if (!(flags & (GPIO_PULL_UP | GPIO_PULL_DOWN))) {
		...
		rv = pcal6408_write(ioex, PCAL6408_PULL_ENABLE, val);
		if (rv != EC_SUCCESS)
			return rv;
	} else {
		rv = pcal6408_read(ioex, PCAL6408_PULL_ENABLE, &val);
		...
		rv = pcal6408_write(ioex, PCAL6408_PULL_UP_DOWN, val);
		if (rv != EC_SUCCESS)
			return rv;
	}
					...
					int enable)
{
	...
	rv = pcal6408_read(ioex, PCAL6408_INPUT_LATCH, &val);
	if (rv != EC_SUCCESS)
		return rv;
	...
	rv = pcal6408_write(ioex, PCAL6408_INPUT_LATCH, val);
	if (rv != EC_SUCCESS)
		return rv;
	...
	return pcal6408_write(ioex, PCAL6408_INT_MASK, pcal6408_int_mask[ioex]);
}
...
int pcal6408_ioex_event_handler(int ioex)
{
	 ...
	 */
	rv = i2c_read8(ioex_p->i2c_host_port, ioex_p->i2c_slave_addr,
				PCAL6408_INT_STATUS, &int_status);
	if (rv != EC_SUCCESS)
		return rv;
