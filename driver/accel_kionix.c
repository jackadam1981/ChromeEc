...
static int raw_xfer(const int port, const int addr, const uint8_t *txdata,
...
#ifdef I2C_PORT_ACCEL
		i2c_lock(port, 1);
		rv = i2c_xfer(port, addr, txdata, txlen, rxdata, rxlen,
			      I2C_XFER_SINGLE);
		i2c_lock(port, 0);
#endif
	}
...
static int read(const struct motion_sensor_t *s, vector_3_t v)
{
	...
	mutex_lock(s->mutex);
	ret = raw_xfer(s->port, s->addr, &reg, 1, acc, 6);
	mutex_unlock(s->mutex);
