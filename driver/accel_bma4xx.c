 ...
 */
static inline int bmi_read8(const int port, const uint16_t i2c_spi_addr_flags,
 ...
 */
static inline int bmi_write8(const int port, const uint16_t i2c_spi_addr_flags,
			     const int reg, int data)
{
	int ret;

	ret = i2c_write8(port, i2c_spi_addr_flags, reg, data);
 ...
 */
static inline int bmi_set_reg8(const struct motion_sensor_t *s, int reg,
			       uint8_t bits,
			       int mask)
{
	int ret, val;
...
static int set_range(struct motion_sensor_t *s, int range, int round)
{
	...
	ret = bmi_set_reg8(s, BMA4_ACCEL_RANGE_ADDR, range_reg_val,
...
static int set_data_rate(const struct motion_sensor_t *s, int rate, int round)
{
	...
	/* Determine the new value of control reg and attempt to write it. */
	ret = bmi_set_reg8(s, BMA4_ACCEL_CONFIG_ADDR, odr_reg_val,
			   BMA4_ACCEL_ODR_MSK);
...
static int init(struct motion_sensor_t *s)
{
	...
	ret = bmi_set_reg8(s, BMA4_POWER_CTRL_ADDR, BMA4_POWER_ACC_EC_MASK,
