...
static int init(struct motion_sensor_t *s)
{
	...
	if (IS_ENABLED(BMA4XX_USE_INTERRUPTS)) {
		CPRINTF("BMA4xx enabling FIFO mode");
...
static _Atomic uint32_t last_irq_timestamp;

/* Handle IRQ from sensor: schedule read from task context */
void bma4xx_interrupt(enum gpio_signal signal)
{
	atomic_store_explicit(&last_irq_timestamp, __hw_clock_source_read(),
...
static void process_fifo_data(struct motion_sensor_t *s, uint8_t (*data)[24],
			      size_t data_bytes, uint32_t timestamp)
{
	...
	for (int i = 0; i < data_bytes; i += 6) {
		...
		swizzle_sample_data(s, (const uint8_t *)&data[i], v);
...
static int irq_handler(struct motion_sensor_t *s, uint32_t *event)
{
	uint32_t irq_timestamp =
		atomic_load_explicit(&last_irq_timestamp, memory_order_relaxed);
