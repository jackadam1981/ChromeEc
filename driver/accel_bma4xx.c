...
/**
 * Return the data rate in millihz cooresponding to an acc_odr register value.
 *
...
static int bma4_reg_to_odr(uint8_t reg)
{
	 ...
	 */
	__ASSERT(reg >= 1 && reg <= 15,
		 "ODR register value %#x is outside the valid range", reg);
...
static int set_data_rate(const struct motion_sensor_t *s, int rate, int round)
{
	...
	if (rate == 0) {
		...
		__ASSERT((odr_reg_val & BMA4_ACCEL_ODR_MSK) == odr_reg_val &&
				 odr_reg_val != 0,
			 "bma4_odr_to_reg returned invalid value: "
			 "rate %d => value %#x",
			 rate, odr_reg_val);
