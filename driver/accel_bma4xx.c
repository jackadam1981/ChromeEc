...
{
	int i, ret = EC_ERROR_TIMEOUT;

	/* Retry 5 times*/
	uint8_t reg_data[6] = {0}, try_cnt = 5;

	/* Check if data is ready */
	while (try_cnt && (!(reg_data[0] & 0x80))) {
				...
				int sens_range)
{
	...
	for (loop = 0; loop < BMA4_FOC_SAMPLE_LIMIT; loop++) {
		...
		ret = wait_and_read_data(s, accel_data);
	...
	/*
	 * The data is in LSB so -> [(LSB)*1000*range/2^11] (unit of offset:mg
	 */
	for (i = X; i <= Z; ++i) {
	...
	ret = write_accel_offset(s, offset);
...
static int perform_calib(struct motion_sensor_t *s, int enable)
{
	...
	/* Set the sensor configuration before calibration */
