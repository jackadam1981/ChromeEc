...
void keyboard_raw_task_start(void)
{
	...
	if (ret) {
		CPRINTF("Failed to read IT8801 verdor id %x\n", ret);
		return;
	}

	/* KSO alternate function switching(KSO[21:18]) */
	it8801_write8(IT8801_REG_GPIO00_KSO19,
		IT8801_REG_MASK_GPIOAFS_FUNC2);
	it8801_write8(IT8801_REG_GPIO01_KSO18,
		IT8801_REG_MASK_GPIOAFS_FUNC2);
	it8801_write8(IT8801_REG_GPIO22_KSO21,
		IT8801_REG_MASK_GPIOAFS_FUNC2);
	it8801_write8(IT8801_REG_GPIO23_KSO20,
		IT8801_REG_MASK_GPIOAFS_FUNC2);

	if (IS_ENABLED(CONFIG_KEYBOARD_COL2_INVERTED))
		/* KSO[2] is high, others are low. */
		it8801_write8(IT8801_REG_KSOMCR, IT8801_REG_MASK_KSOSDIC |
...
__overridable const uint8_t kso_mapping[KEYBOARD_COLS_MAX] = {
	0, 1, 2, 3, 4, 5, 6, 17, 18, 16, 15, 11, 12,
