...
static int it8801_i2c_read8(int reg, int *data)
{
	return i2c_read8(I2C_PORT_IO_EXPANDER_IT8801, IT8801_I2C_ADDR,
		reg, data);
}

static int it8801_i2c_write8(int reg, int data)
{
...
static int it8801_check_vendor_id(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(it8801_vendor_id_verify); i++) {
		ret = it8801_i2c_read8(it8801_vendor_id_verify[i].reg, &val);

		if (ret != EC_SUCCESS)
			return ret;

		if (val != it8801_vendor_id_verify[i].chip_id) {
			ccprintf("Failed to read IT8801 verdor id %x\n", val);
...
void keyboard_raw_task_start(void)
{
	...
	if (it8801_check_vendor_id())
		return;

	/* KSO alternate function switching(KSO[21:18]) */
	it8801_i2c_write8(IT8801_REG_GPIO00_KSO19,
		IT8801_REG_MASK_GPIOAFS_FUNC2);
	it8801_i2c_write8(IT8801_REG_GPIO01_KSO18,
		IT8801_REG_MASK_GPIOAFS_FUNC2);
	it8801_i2c_write8(IT8801_REG_GPIO22_KSO21,
		IT8801_REG_MASK_GPIOAFS_FUNC2);
	it8801_i2c_write8(IT8801_REG_GPIO23_KSO20,
		IT8801_REG_MASK_GPIOAFS_FUNC2);

	if (IS_ENABLED(CONFIG_KEYBOARD_COL2_INVERTED))
		/* KSO[2] is high, others are low. */
		it8801_i2c_write8(IT8801_REG_KSOMCR, IT8801_REG_MASK_KSOSDIC |
			IT8801_REG_MASK_SELKSO2);
	else
		/* KSO[21:18,12:11,6:0] pins low. */
		it8801_i2c_write8(IT8801_REG_KSOMCR, IT8801_REG_MASK_AKSOSC);

	/* Keyboard scan in interrupt enable register */
	it8801_i2c_write8(IT8801_REG_KSIIER, 0xff);
	/* Gather KSI interrupt enable */
	it8801_i2c_write8(IT8801_REG_GIECR, IT8801_REG_MASK_GKSIIE);
	/* Alert response enable */
	it8801_i2c_write8(IT8801_REG_SMBCR, IT8801_REG_MASK_ARE);
...
test_mockable void keyboard_raw_drive_column(int col)
{
	...
	if (col == KEYBOARD_COLUMN_NONE)
		if (IS_ENABLED(CONFIG_KEYBOARD_COL2_INVERTED))
			/*
			 * Inverted KSO[2] output low,
			 * all others KSO output high.
			 */
			kso_val = IT8801_REG_MASK_SELKSO2;
		else
	...
	/* Selected KSO output low, all others KSO output high. */
		...
		if (IS_ENABLED(CONFIG_KEYBOARD_COL2_INVERTED))
			...
			/* Selected KSO[2] and others KSO output high. */
				kso_val = IT8801_REG_MASK_KSOSDIC |
					IT8801_REG_MASK_AKSOSC;
			else
			/*
			 * Selected kso_val and KSO[2] output low,
	...
	it8801_i2c_write8(IT8801_REG_KSOMCR, kso_val);
}
...
test_mockable int keyboard_raw_read_rows(void)
{
	...
	it8801_i2c_read8(IT8801_REG_KSIDR, &data);

	/* This register needs to write clear after reading data */
	it8801_i2c_read8(IT8801_REG_KSIEER, &ksieer);
	it8801_i2c_write8(IT8801_REG_KSIEER, ksieer);
...
void keyboard_raw_enable_interrupt(int enable)
{
	...
	if (enable) {
		it8801_i2c_write8(IT8801_REG_KSIEER, 0xff);
