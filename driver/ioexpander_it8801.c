...
const static struct it8801_pwm_gpio_map it8801_pwm_gpio_map[10] = {
	[1] = {.port = 1, .mask = BIT(2)},
	[2] = {.port = 1, .mask = BIT(3)},
	[3] = {.port = 1, .mask = BIT(4)},
	[4] = {.port = 1, .mask = BIT(5)},
...
void pwm_enable(enum pwm_channel ch, int enabled)
{
	...
	it8801_read(IT8801_REG_PWMMCR(pwm_channels[ch].index), &val);
	val &= (~IT8801_PWMMCR_MCR_MASK);
	if (enabled)
		val |= IT8801_PWMMCR_MCR_BLINKING;
	it8801_write(IT8801_REG_PWMMCR(pwm_channels[ch].index), val);
}
...
int pwm_get_enabled(enum pwm_channel ch)
{
	...
	if (it8801_read(IT8801_REG_PWMMCR(pwm_channels[ch].index), &val))
		return 0;
	return (val & IT8801_PWMMCR_MCR_MASK) != IT8801_PWMMCR_MCR_OFF;
}
...
void pwm_set_raw_duty(enum pwm_channel ch, uint16_t duty)
{
	duty = MIN(duty, 255);
	duty = MAX(duty, 0);
	it8801_write(IT8801_REG_PWMDCR(pwm_channels[ch].index), duty);
}
...
uint16_t pwm_get_raw_duty(enum pwm_channel ch)
{
	...
	if (it8801_read(IT8801_REG_PWMDCR(pwm_channels[ch].index), &val))
		return 0;
	return val;
}
...
{
	return pwm_set_raw_duty(ch, (100 - percent) * 255 / 100);
}

int pwm_get_duty(enum pwm_channel ch)
{
	return 100 - pwm_get_raw_duty(ch) * 100 / 255;
}
