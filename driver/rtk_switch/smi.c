...
rtk_uint32  smi_SCK = GPIO_MASTER_I2C_SCL;    /* GPIO used for SMI Clock Generation */
rtk_uint32  smi_SDA = GPIO_MASTER_I2C_SDA;    /* GPIO used for SMI Data signal */

/* I2C, redefine/implement the following Macro */
#define GPIO_DIRECTION_SET(gpioID, direction) gpio_set_flags(gpioID, ((direction) == GPIO_DIR_IN)? GPIO_INPUT: GPIO_OUTPUT)
#define GPIO_DATA_SET(gpioID, data) gpio_set_level(gpioID, data)
#define GPIO_DATA_GET(gpioID, pData) do{ *(pData) = gpio_get_level(gpioID); }while(0)
...
static void rtlglue_drvMutexLock(void)
{
    ...
    i2c_raw_mode(I2C_PORT_MASTER, 1);
    interrupt_disable();
    return;
}
...
static void rtlglue_drvMutexUnlock(void)
{
    /* It is empty currently. Implement this function if Lock/Unlock function is needed */
    interrupt_enable();
    i2c_raw_mode(I2C_PORT_MASTER, 0);
