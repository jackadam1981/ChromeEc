...
rtk_api_ret_t rtk_rate_shareMeter_get(rtk_meter_id_t index, rtk_meter_type_t *pType, rtk_rate_t *pRate, rtk_enable_t *pIfg_include)
{
    ...
    if ((retVal = rtl8367c_getAsicShareMeter(index, &regData, (rtk_uint32*)pIfg_include)) != RT_ERR_OK)
        return retVal;
...
rtk_api_ret_t rtk_rate_igrBandwidthCtrlRate_get(rtk_port_t port, rtk_rate_t *pRate, rtk_enable_t *pIfg_include, rtk_enable_t *pFc_enable)
{
    ...
    if ((retVal = rtl8367c_getAsicPortIngressBandwidth(rtk_switch_port_L2P_get(port), &regData, (rtk_uint32*)pIfg_include, (rtk_uint32*)pFc_enable)) != RT_ERR_OK)
        return retVal;
...
rtk_api_ret_t rtk_rate_egrQueueBwCtrlEnable_get(rtk_port_t port, rtk_qid_t queue, rtk_enable_t *pEnable)
{
    ...
    if ((retVal = rtl8367c_getAsicAprEnable(rtk_switch_port_L2P_get(port),(rtk_uint32*)pEnable))!=RT_ERR_OK)
        return retVal;
