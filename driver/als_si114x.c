 ...
 */
static inline int raw_read8__8b(const int port, const int addr__8b, const int reg,
			    int *data_ptr)
{
	return i2c_read8__7b(port, addr__8b >> 1, reg, data_ptr);
}

/**
 * Write 8bit register from device.
 */
static inline int raw_write8__8b(const int port, const int addr__8b, const int reg,
			     int data)
{
	return i2c_write8__7b(port, addr__8b >> 1, reg, data);
}

/**
 * Read 16bit register from device.
 */
static inline int raw_read16__8b(const int port, const int addr__8b, const int reg,
			     int *data_ptr)
{
	return i2c_read16__7b(port, addr__8b >> 1, reg, data_ptr);
}
			   ...
			   int *value)
{
	...
	if (op != SI114X_CMD_PARAM_QUERY) {
		ret = raw_write8__8b(s->port, s->addr__8b, SI114X_REG_PARAM_WR, *value);
		if (ret != EC_SUCCESS)
			goto error;
	}

	ret = raw_write8__8b(s->port, s->addr__8b, SI114X_REG_COMMAND,
			 op | (param & 0x1F));
	if (ret != EC_SUCCESS)
		goto error;

	ret = raw_read8__8b(s->port, s->addr__8b, SI114X_REG_PARAM_RD, value);
	if (ret != EC_SUCCESS)
		goto error;
...
static int si114x_read_results(struct motion_sensor_t *s, int nb)
{
	...
	/* Read ALX result */
	for (i = 0; i < nb; i++) {
		ret = raw_read16__8b(s->port, s->addr__8b,
...
static int irq_handler(struct motion_sensor_t *s, uint32_t *event)
{
	...
	ret = raw_read8__8b(s->port, s->addr__8b, SI114X_REG_IRQ_STATUS, &val);
	if (ret)
		return ret;
	...
	/* clearing IRQ */
	ret = raw_write8__8b(s->port, s->addr__8b, SI114X_REG_IRQ_STATUS,
...
static int read(const struct motion_sensor_t *s, intv3_t v)
{
	...
	case SI114X_IDLE:
		...
		ret = raw_write8__8b(s->port, s->addr__8b, SI114X_REG_COMMAND, cmd);
#ifdef CONFIG_ALS_SI114X_POLLING
...
static int si114x_initialize(const struct motion_sensor_t *s)
{
	...
	ret = raw_write8__8b(s->port, s->addr__8b, SI114X_REG_HW_KEY, 0x17);
	if (ret != EC_SUCCESS)
		return ret;
	msleep(20);

	/* interrupt configuration, interrupt output enable */
	ret = raw_write8__8b(s->port, s->addr__8b, SI114X_REG_INT_CFG,
			 SI114X_INT_CFG_OE);
	if (ret != EC_SUCCESS)
		return ret;

	/* enable interrupt for certain activities */
	ret = raw_write8__8b(s->port, s->addr__8b, SI114X_REG_IRQ_ENABLE,
	...
	/* Only forced mode */
	ret = raw_write8__8b(s->port, s->addr__8b, SI114X_REG_MEAS_RATE, 0);
	if (ret != EC_SUCCESS)
		return ret;

	/* measure ALS every time device wakes up */
	ret = raw_write8__8b(s->port, s->addr__8b, SI114X_REG_ALS_RATE, 0);
	if (ret != EC_SUCCESS)
		return ret;

	/* measure proximity every time device wakes up */
	ret = raw_write8__8b(s->port, s->addr__8b, SI114X_REG_PS_RATE, 0);
	if (ret != EC_SUCCESS)
		return ret;
	...
	switch (SI114X_NUM_LEDS) {
	case 3:
		ret = raw_write8__8b(s->port, s->addr__8b,
