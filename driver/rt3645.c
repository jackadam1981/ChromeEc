...
static int rt3645_imvp_write(uint8_t reg, uint8_t val)
{
	int rv = 1;
	uint8_t data = val;
	int retry = 0;
...
static int rt3645_imvp_read(uint8_t reg, uint8_t *val)
{
	int rv = 1;
	int data = 0;
...
static int imvp_select_page(uint8_t page)
{
	...
	if ((page < IMVP_PAGE_LIMIT) && (page >= IMVP_PAGE_GLOBAL))
		rv = rt3645_imvp_write(PAGE_SET_REG, page);
	return rv;
}
...
static int imvp_read_crc()
{
	...
	rt3645_imvp_read((uint8_t)CRC_REG, &value);
...
static int imvp_check_nvm_status(int pos)
{
	...
	while (retry < retry_cnt) {
		...
		rt3645_imvp_read((uint8_t)NVM_STAT_REG, &value);
		if (pos == NVM_STAT) {
			if (value == 0xE0) {
...
static void imvp_unlock_nvm()
{
	...
	imvp_select_page(IMVP_PAGE_GLOBAL);
	rt3645_imvp_write((uint8_t)CONFIG_MODE_REG, 0x24);
	rt3645_imvp_write((uint8_t)CONFIG_MODE_REG, 0x54);
	rt3645_imvp_write((uint8_t)CONFIG_MODE_REG, 0x2);
	k_msleep(1);
}
...
static void imvp_lock_nvm()
{
	...
	imvp_select_page(IMVP_PAGE_GLOBAL);
	rt3645_imvp_write((uint8_t)CONFIG_MODE_REG, 0x00);
	rt3645_imvp_write((uint8_t)CONFIG_MODE_REG, 0xFF);
}
...
static int imvp_read_product_id()
{
	uint8_t value = 0;
	imvp_select_page(IMVP_PAGE_GLOBAL);
	rt3645_imvp_read((uint8_t)PRODUCT_ID_REG, &value);
	if (value == 0x45) {
...
{
	rt3645_imvp_write(0xEF, 0x05);
	rt3645_imvp_write(RLL_REG, RLL_PAGE5_VAL);
	rt3645_imvp_write(DEM_SHRINK_TON_REG, DEM_SHRINK_TON_PAGE5_VAL);
	k_msleep(1);
...
static void imvp_update_page_RAILA_VCC_CORE()
{
	...
	rt3645_imvp_write(ICC_MAX_REG, ICC_MAX_RAILA_VAL);
	rt3645_imvp_write(ICCMAX_HC_SR_KTON_REG, ICCMAX_HC_SR_KTON_RAILA_VAL);
	rt3645_imvp_write(RIMON_REG, RIMON_RAILA_VAL);
	rt3645_imvp_write(RLL_REG, RLL_RAILA_VAL);
	rt3645_imvp_write(DVID_ENHANCE_SPM_EN_REG, DVID_ENHANCE_SPM_EN_RAILA_VAL);
	k_msleep(1);
...
static void imvp_update_page_RAILB_VCCSA()
{
	...
	rt3645_imvp_write(ICCMAX_HC_SR_KTON_REG, ICCMAX_HC_SR_KTON_RAILB_VAL);
	rt3645_imvp_write(DVID_ENHANCE_SPM_EN_REG, DVID_ENHANCE_SPM_EN_RAILB_VAL);
	k_msleep(1);
...
static void imvp_update_page_RAILC_VCCGT()
{
	...
	rt3645_imvp_write(ICC_MAX_REG, ICC_MAX_RAILC_VAL);
	rt3645_imvp_write(ICCMAX_HC_SR_KTON_REG, ICCMAX_HC_SR_KTON_RAILC_VAL);
	rt3645_imvp_write(RIMON_REG, RIMON_RAILC_VAL);
	rt3645_imvp_write(RLL_REG, RLL_RAILC_VAL);
	rt3645_imvp_write(VID_STEP_COMP_GAIN_REG, VID_STEP_COMP_GAIN_RAILC_VAL);
	rt3645_imvp_write(COMP_MODE_PZ_REG, COMP_MODE_PZ_RAILC_VAL);
	rt3645_imvp_write(VSEN_COMP_LPF_REG, VSEN_COMP_LPF_RAILC_VAL);
	rt3645_imvp_write(DVID_ENHANCE_SPM_EN_REG, DVID_ENHANCE_SPM_EN_RAILC_VAL);
	rt3645_imvp_write(DEM_SHRINK_TON_REG, DEM_SHRINK_TON_RAILC_VAL);
	rt3645_imvp_write(ZCD_VID_R_TH_REG, ZCD_VID_R_TH_RAILC_VAL);
	rt3645_imvp_write(ZCD_I_TH_HYS_REG, ZCD_I_TH_HYS_RAILC_VAL);
	rt3645_imvp_write(DVID_TAU_AQR_TH_REG, DVID_TAU_AQR_TH_RAILC_VAL);
	rt3645_imvp_write(RIPPLE_COMP_SVID_ADDR_REG, RIPPLE_COMP_SVID_ADDR_RAILC_VAL);
	rt3645_imvp_write(SVID_DCLL_REG, SVID_DCLL_RAILC_VAL);
	k_msleep(1);
...
static void imvp_update_page_RAILD_VCC_ECORE()
{
	...
	rt3645_imvp_write(ICCMAX_HC_SR_KTON_REG, ICCMAX_HC_SR_KTON_RAILD_VAL);
	rt3645_imvp_write(VSEN_COMP_LPF_REG, VSEN_COMP_LPF_RAILD_VAL);
	rt3645_imvp_write(DVID_ENHANCE_SPM_EN_REG, DVID_ENHANCE_SPM_EN_RAILD_VAL);
	rt3645_imvp_write(RIPPLE_COMP_SVID_ADDR_REG, RIPPLE_COMP_SVID_ADDR_RAILD_VAL);
	k_msleep(1);
...
{
	imvp_select_page(IMVP_PAGE_RAIL_GEN);
	rt3645_imvp_write(SPM_HYS_DVIDUP_PH_A_REG, SPM_HYS_DVIDUP_PH_A_REG_VAL);
	rt3645_imvp_write(DVIDUP_PH_B_DVIDDN_PH_SET_REG,
...
{
	imvp_select_page(IMVP_PAGE_GLOBAL);
	rt3645_imvp_write(NVM_PRGRM_CTRL_REG, NVM_PRGRM_DAT);
	k_msleep(800);
...
{
	imvp_select_page(IMVP_PAGE_GLOBAL);
	rt3645_imvp_write(NVM_PRGRM_CTRL_REG, NVM_RESTORE_DAT);
	k_msleep(100);
...
__overridable void set_board_ready_before_imvp_update(void)
{
}

__overridable void set_board_ready_after_imvp_update(void)
{
...
{
	set_board_ready_before_imvp_update();
	if(!imvp_update())
		ccprintf("IMVP Updated Successfully!");
	set_board_ready_after_imvp_update();
...
DECLARE_HOOK(HOOK_INIT, impv_cb_init, HOOK_PRIO_DEFAULT);
#endif
