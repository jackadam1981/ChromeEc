...
static int convert_fp_capture_mode_to_elan_get_image_type(int mode)
{
	switch (mode) {
...
{
	int rc = convert_fp_capture_mode_to_elan_get_image_type(mode);

	if (rc < 0) {
		CPRINTF("Unsupported mode %d provided", mode);
		return rc;
	}
