...
int elan_write_cmd(uint8_t fp_cmd)
{
	...
	tx_buf[0] = fp_cmd;
	rc = spi_transaction(&spi_devices[0], tx_buf,
				2, rx_buf, SPI_READBACK_ALL);
	return rc;
}
...
int elan_read_cmd(uint8_t fp_cmd, uint8_t *regdata)
{
	...
	ret = spi_transaction(&spi_devices[0], tx_buf,
				2, rx_buf, SPI_READBACK_ALL);
	*regdata = rx_buf[1];
...
int raw_capture(uint16_t *short_raw)
{
	...
	for (i = 0; i < dma_loop; i++) {
		...
		ret = spi_transaction(&spi_devices[0], tx_buf,
			2, rx_buf, dma_len);
		memcpy(&img_buf[dma_len * i], rx_buf,
			dma_len);
	}
	...
	for (index = 0; index < IMAGE_TOTAL_PIXEL; index++)
		short_raw[index] = (img_buf[index * 2] << 8) +
				img_buf[index * 2 + 1];
