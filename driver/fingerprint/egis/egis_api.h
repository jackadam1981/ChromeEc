 ...
 * @return EGIS_INCORRECT_PARAMETER : when there are incorrect parameters.
 * @return EGIS_OUT_OF_MEMORY : when memody memory fails.
 * @return EGIS_NO_DEVICE : when the sensor cannot be detected.
 * @return EGIS_ESD_NEED_RESET : when the sensor needs to be reset.
 * @return EGIS_SPI_FAIL : when the spi transfer fails.
 * @return EGIS_CALIBRATION_SENSING_FAIL : when the sensor sensinf mode
 * calibration fails.
 * @return EGIS_CALIBRATION_DETECT_FAIL : when the sensor detect mode
 * calibration fails.
 * @return EGIS_GET_OTP_FAIL : when the get sensor's OTP fails.
 * @return EGIS_OCP_DETECT : when the sensor detects over current event and cuts
 * off power to the sensing area.
 */
 ...
 *
 * @return EGIS_OK : on success.
 * @return EGIS_OUT_OF_MEMORY : when allocation memory fails.
 * @return ET630_IPP_ERR_ILLEGAL_PARAMS : when there are incorrect parameters.
 * @return ET630_IPP_ERR_OTUOFMEMORY : when allocation memory fails.
 */
int egis_get_image_with_mode(uint8_t *image_data, int mode);

/**
 * @brief Get 16bits image data from EGIS fingerprint sensor.
 *
 * @param[out] image_data Image from sensor. Buffer must be allocated by caller
 * with size FP_SENSOR_IMAGE_SIZE.
 *
 ...
 * @return EGIS_API_FINGER_UNSTABLE when the sensor detects ESD or is reset.
 */
