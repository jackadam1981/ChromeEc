 ...
 *
 * @param[out] image_data raw image from sensor. Buffer must be allocated by
 ...
 *
 * @param[out] image_data the memory buffer to receive fingerprint image. The
 * buffer length is:(IMAGE_WIDTH*IMAGE_HEIGHT)*sizeof(uint16_t).
 *
 * @return 0 on success.
 * @return negative value on error.
 */
