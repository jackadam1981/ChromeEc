...
/**
 * @brief Resert and initialize the sensor IC
 *
 * @return 0 on success
 ...
 *
 * @param[out] image_data The memory buffer to receive fingerprint image
 * image data, buffer length is:(IMAGE_WIDTH*IMAGE_HEIGHT)*sizeof(uint16_t)
 *
 * @return 0 on success.
 * @return negative value on error.
 */
...
/**
 * Set the detection mode for the Egis sensor
 *
 */
void egis_set_detect_mode(void);

/**
 * Check the sensor initialization status
 *
 * @return 0 on success.
 * @return negative value on error.
 */
