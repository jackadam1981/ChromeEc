...
{
	switch (vendor_capture_type) {
	case EGIS_CAPTURE_NORMAL_FORMAT:
		return FP_CAPTURE_SIMPLE_IMAGE;
	case EGIS_CAPTURE_BLACK_PXL_TEST:
		return FP_CAPTURE_PATTERN0;
	case EGIS_CAPTURE_WHITE_PXL_TEST:
		return FP_CAPTURE_PATTERN1;
	case EGIS_CAPTURE_DEFECT_PXL_TEST:
		return FP_CAPTURE_DEFECT_PXL_TEST;
	case EGIS_CAPTURE_NOISE_TEST:
		return FP_CAPTURE_NOISE_TEST;
	case EGIS_CAPTURE_ABNORMAL_TEST:
		return FP_CAPTURE_ABNORMAL_TEST;
	case EGIS_CAPTURE_RV_INT_TEST:
		return FP_CAPTURE_QUALITY_TEST;
	default:
		assert(false);
		break;
	}
