...
{
	return ((capture_mode == FP_CAPTURE_SIMPLE_IMAGE) ||
		(capture_mode == FP_CAPTURE_PATTERN0) ||
...
static bool is_capture_normal_image(uint32_t capture_mode)
{
	return ((capture_mode == FP_CAPTURE_VENDOR_FORMAT) ||
		(capture_mode == FP_CAPTURE_RESET_TEST));
}
