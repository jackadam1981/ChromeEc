...
/* Sensor description */
static struct fp_sensor_info fpc_sensor_info_2 = {
	/* Sensor identification */
	.vendor_id = FOURCC('F', 'P', 'C', ' '),
	.product_id = 9,
	.model_id = 1,
...
static const struct fp_image_frame_params fpc_image_frame_params[6] = {
...
{
	uint16_t sensor_id;
	memcpy(&resp->sensor_info, &fpc_sensor_info_2,
	       sizeof(struct fp_sensor_info));

	memcpy(&resp->image_frame_params, &fpc_image_frame_params,
	       sizeof(fpc_image_frame_params));
