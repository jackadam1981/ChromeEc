...
static inline int anx7452_ctltop_read(const struct usb_mux *me, uint8_t reg,
				      int *val)
{
	return i2c_read8(me->i2c_port, ctltop_reg_i2c_addr >> 1, reg, val);
}

static inline int anx7452_ctltop_write(const struct usb_mux *me, uint8_t reg,
				       uint8_t val)
{
	return i2c_write8(me->i2c_port, ctltop_reg_i2c_addr >> 1, reg, val);
}

int anx7452_ctltop_update(const struct usb_mux *me, uint8_t reg, uint8_t bits,
			  uint8_t val)
{
	int reg_val = 0;
	RETURN_ERROR(anx7452_ctltop_read(me, reg, &reg_val));
	reg_val = (reg_val & ~bits) | (val & bits);
	rv = anx7483_ctltop_write(me, reg, reg_val);
	if (rv) {
...
static int anx7452_init(const struct usb_mux *me)
{
	...
	RETURN_ERROR(anx7452_read(me, ANX7452_TOP_CTLTOP_REG_ADDR_REG,
				  &ctltop_reg_i2c_addr));
