			  ...
			  bool *ack_required)
{
	...
	*ack_required = false;
	...
	if (rv) {
		CPRINTSUSB("C%d: PS8828 read failed %d", me->usb_port, rv);
		return rv;
	}
	...
	if (rv)
		CPRINTSUSB("C%d: PS8828 write failed %d", me->usb_port, rv);

	/* Handle DP HPD. */
	dphpd &= ~(PS8828_DPHPD_PLUGGED | PS8828_DPHPD_INHPD_DISABLE);
