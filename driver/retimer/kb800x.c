			  ...
			  enum kb800x_phy_lane phy_lane, enum kb800x_eb eb)
{
	...
	/* need to shift by 4 for reverse EB or 3rd EB in set */
	switch (eb) {
	case KB800X_EB1:
...
static int kb800x_bulk_write(const struct usb_mux *me,
			     ...
			     const uint8_t values[1], const uint8_t size)
{
	int i;
	uint8_t rv;
...
static int kb800x_dp_common_init(const struct usb_mux *me,
...
static int kb800x_usb3_common_init(const struct usb_mux *me)
{
	return kb800x_bulk_write(me, usb3_init_addresses, usb3_init_values,
...
static int kb800x_dpmf_init(const struct usb_mux *me, mux_state_t mux_state)
{
	int rv;
	...
	if (rv)
		return rv;
	rv = kb800x_dp_common_init(me, mux_state);
	if (rv)
		return rv;
	return kb800x_write(me, KB800X_REG_PROTOCOL, KB800X_PROTOCOL_DPMF);
}
...
static int kb800x_dp_init(const struct usb_mux *me, mux_state_t mux_state)
{
	...
	if (rv)
		return rv;
	return kb800x_write(me, KB800X_REG_PROTOCOL, KB800X_PROTOCOL_DP);
}
...
static int kb800x_cio_init(const struct usb_mux *me, mux_state_t mux_state)
{
	...
	rv = kb800x_write(me, KB800X_REG_ORIENTATION, orientation);
	if (rv)
		return rv;
	return kb800x_write(me, KB800X_REG_PROTOCOL, KB800X_PROTOCOL_CIO);
}
...
static int kb800x_set_state(const struct usb_mux *me, mux_state_t mux_state)
{
	...
	/* DP alt modes */
		...
		if (mux_state & USB_PD_MUX_USB_ENABLED)
			rv = kb800x_dpmf_init(me, mux_state);
		else
