...
/**
 * Set USB retimer connection state
 *
 * @param port Port number.
 * @param mux_state current MUX state
 * @return Non-zero if fail; otherwise, zero.
 */
int bb_set_connection_state(int port, mux_state_t mux_state);

/**
 * Enter USB retimer into low power mode.
 *
 * @param port Port number.
 * @return Non-zero if fail; otherwise, zero.
 */
int bb_enter_low_power_mode(int port);
