...
#define KB8010_LANE_CONFIG_LEN 8
enum kb8010_modes {
...
static int kb8010_sequential_write(const struct usb_mux *me,
			     const uint16_t start_addr, const uint8_t *values,
			     const uint8_t size)
{
	int i;
	int rv;
	...
	if (cable_type == IDH_PTYPE_ACABLE ||
		...
		if (cable_resp.lsrx_comm == UNIDIR_LSRX_COMM) {
			...
			if (rv)
				return rv;
			cable_type |=
				KB8010_CABLE_TYPE_ACTIVE_BIDIR;
		}
	} else {
		/* Passive Cable */
...
static int console_command_kb8010_xfer(int argc, const char **argv)
{
	...
	while (mux_chain) {
		if (mux_chain->mux->driver == &kb8010_usb_retimer_driver)
			break;
