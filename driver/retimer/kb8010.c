				  ...
				  mux_state_t mux_state)
{
	...
	printk("%s: usb mux port %d, cable_type %d, raw_value 0x%08x\n", __func__,
	       me->usb_port,
	       cable_type,
	...
	const uint8_t r_806b[] = { 0x40, 0x40, };
	...
	if (cable_type == IDH_PTYPE_ACABLE ||
		...
		printk("%s: usb mux port %d, passive cable\n", __func__,
			me->usb_port);

		kb8k_cable_type = KB8010_CABLE_TYPE_PASSIVE;
	}

	return kb8010_write(me, KB8010_REG_ORIENTATION, kb8k_cable_type);
}
...
//	enum kb8010_modes mode = KB8010_USB3;
	...
	printk("%s: mux port %d, mux_state 0x%02x\n", __func__,
	       me->usb_port,
	       mux_state);
...
//	cached_mux_state[me->usb_port] = mux_state;
	...
	if (mux_state & USB_PD_MUX_POLARITY_INVERTED) {
		...
		printk("%s: mux port %d, cable inverted\n", __func__, me->usb_port);
		...
		printk("%s: mux port %d, cable normal\n", __func__, me->usb_port);
...
//	return kb8010_write(me, KB8010_REG_RESET, 0x00);
	rv = kb8010_write(me, KB8010_REG_RESET, 0x00);

	if (mode == KB8010_USB4_TBT) {
		if (mux_state & USB_PD_MUX_POLARITY_INVERTED) {
			rv = kb8010_write(me, 0x81fd, 0x08);
			rv = kb8010_write(me, 0x81fe, 0x80);
		} else {
			rv = kb8010_write(me, 0x81fd, 0x04);
			rv = kb8010_write(me, 0x81fe, 0x20);
		}
		kb8010_write(me, 0x81fd, 0x00);
	}
...
static int kb8010_init(const struct usb_mux *me)
{
	...
	printk("%s: mux port %d, rst_gpio %d\n", __func__,
	       me->usb_port,
	       kb8010_controls[me->usb_port].retimer_rst_gpio);
