				  ...
				  uint32_t *set_retimer_con)
{
	...
	union dp_mode_resp_cable cable_dp_mode_resp = {
		.raw_value = pd_get_dp_mode_vdo(port, TCPCI_MSG_SOP_PRIME)
	};
	...
	if (IS_ENABLED(CONFIG_USB_PD_DP21_SUPPORT) &&
		 ...
		 */
		if ((resolve_dpam_version(port, TCPCI_MSG_SOP) ==
			     DPAM_VERSION_20 &&
		     is_active_cable_element_retimer(port)) ||
		    (resolve_dpam_version(port, TCPCI_MSG_SOP) ==
			     DPAM_VERSION_21 &&
		 ...
		 * 1 - Optical cable
		 */
		if (cable_dp_mode_resp.cable_type == DP21_OPTICAL_CABLE &&
		    resolve_dpam_version(port, TCPCI_MSG_SOP) ==
			    DPAM_VERSION_21)
			*set_retimer_con |= BB_RETIMER_TBT_CABLE_TYPE;
		...
		if (((resolve_dpam_version(port, TCPCI_MSG_SOP) ==
		    ...
		    ((resolve_dpam_version(port, TCPCI_MSG_SOP) ==
		      DPAM_VERSION_21) &&
		     (cable_dp_mode_resp.cable_type != DP21_PASSIVE_CABLE)))
			*set_retimer_con |= BB_RETIMER_ACTIVE_PASSIVE;
		 ...
		 * 010b - UHBR10
		 * 100b - UHBR20
		 * 111b - HBR3,UHBR10 and UHBR20, all DP bitrate supported
		 */
		*set_retimer_con |= BB_RETIMER_USB4_TBT_CABLE_SPEED_SUPPORT(
			resolve_dpam_version(port, TCPCI_MSG_SOP) ==
					DPAM_VERSION_21 ?
				get_dp_cable_bit_rate(port) :
				get_usb4_cable_speed(port));

	} else if (!IS_ENABLED(CONFIG_USB_PD_DP21_SUPPORT) &&
		   (mux_state & USB_PD_MUX_DP_ENABLED)) {
		/*
		 * Bit 2: RE_TIMER_DRIVER
		 * 0 - Re-driver
