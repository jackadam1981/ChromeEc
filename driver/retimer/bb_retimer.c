...
static int bb_retimer_console_cmd(const struct usb_mux *me,
				  ...
				  bool read)
{
	...
	return read ? bb_retimer_read(me, (const uint8_t)offset, data) :
		      bb_retimer_write(me, (const uint8_t)offset, *data);
}
...
#ifdef CONFIG_CMD_RETIMER
	.retimer_console_cmd = bb_retimer_console_cmd,
#endif /* CONFIG_CMD_RETIMER */
};
