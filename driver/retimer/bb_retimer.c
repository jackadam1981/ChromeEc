...
static int retimer_set_state(const struct usb_mux *me, mux_state_t mux_state)
{
	...
	val = bb_retimer_write(me, BB_RETIMER_REG_CONNECTION_STATE,
			set_retimer_con);
	CPRINTS("\nC:%d: Write retimer register 0x%x\n", port, set_retimer_con);

	if (val)
		CPRINTS("\nC%d: BB write I2C error %d\n", port, val);

	val = bb_retimer_read(me, BB_RETIMER_REG_CONNECTION_STATE, &data);

	if(val)
		CPRINTS("\nC:%d**** Read retimer register error val = %d\n", port, val);
	else
		CPRINTS("\nC:%d: Read retimer register val = 0x%x\n", port, data);
