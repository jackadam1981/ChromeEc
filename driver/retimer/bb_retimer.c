...
int retimer_set_state(int port, mux_state_t mux_state)
{
	...
	enum pd_cc_states cc_state = pd_get_cc_state(port);
	uint8_t dp_pin_mode = board_get_dp_pin_mode(port);
	 ...
	 * 1 - UPF
	 */
	if (cc_state == PD_CC_UFP_ATTACHED)
	...
	/*
	 * Bit 10-11: DP_PIN_ASSIGNMENT
	 * 00 – Pin assignments E/E’
	 * 01 – Pin assignments C/C’/D/D’1,2
	 * 10, 11 - reserved
	 */
	if (dp_pin_mode == MODE_DP_PIN_C || dp_pin_mode == MODE_DP_PIN_D)
