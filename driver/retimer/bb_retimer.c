				  ...
				  uint32_t *set_retimer_con)
{
	...
	if (mux_state & USB_PD_MUX_TBT_COMPAT_ENABLED) {
		 ...
		 *     detected
		 */
		if ((dev_resp.vendor_spec_b1 | dev_resp.intel_spec_b0) ==
		     VENDOR_SPECIFIC_SUPPORT)
	...
	if (mux_state & USB_PD_MUX_USB4_ENABLED)
		*set_retimer_con |= BB_RETIMER_USB4_CABLE_SPEED_SUPPORT(
...
static int retimer_set_state(const struct usb_mux *me, mux_state_t mux_state)
{
	 ...
	 */
	if (is_cable_speed_usb3_capable(port))
