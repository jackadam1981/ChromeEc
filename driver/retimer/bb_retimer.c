...
static int retimer_set_state(const struct usb_mux *me, mux_state_t mux_state)
{
	 ...
	 * For active cable, USB2_CONNECTION = According to Active cable VDO2
	 *                                     Bit 5, USB 2.0 support.
	 *
	 * For UFP:
	 * Don't care
	 */
	 ...
	 */
	if (get_usb_pd_rev20_cable_speed(port) == USB_R20_SS_U31_GEN1_GEN2 ||
	    get_usb_pd_rev30_cable_speed(port) == USB_R30_SS_U32_U40_GEN2 ||
	    get_usb_pd_rev30_cable_speed(port) == USB_R30_SS_U40_GEN3)
	...
	if (mux_state & (USB_PD_MUX_TBT_COMPAT_ENABLED |
		...
		if (mux_state & USB_PD_MUX_TBT_COMPAT_ENABLED) {
			...
			if (is_port_dfp) {
				retimer_set_tbt_bits_dfp(&set_retimer_con,
							cable_resp, dev_resp);
			}
