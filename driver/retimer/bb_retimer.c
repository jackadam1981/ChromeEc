			   ...
			   const uint8_t offset, uint32_t *data)
{
	...
	while (retry < BB_RETIMER_I2C_RETRY) {
	...
	if (rv)
        {
          	CPRINTS("C%d: Retimer I2C read err=%d", me->usb_port, rv);
		return rv;
        }
...
static int retimer_init(const struct usb_mux *me)
{
	 ...
	 */
	for (retry = 0; retry < BB_RETIMER_I2C_RETRY; retry++) {
		/*
		...
		if (!rv || (BB_RETIMER_I2C_RETRY - 1) == 2)
			break;
