...
static uint32_t safe_mode_connection;
static uint32_t safe_mode_deferred;
static uint64_t deferred_start;
...
static int retimer_set_state(const struct usb_mux *me, mux_state_t mux_state)
{
	...
	if (safe_mode_deferred) {
		...
		time_elapsed = time_now - deferred_start;
	...
	if (mux_state & USB_PD_MUX_SAFE_MODE) {
		safe_mode_connection = set_retimer_con;
		hook_call_deferred(&delay_retimer_data, 50 * MSEC);
		deferred_start = get_time().val;
