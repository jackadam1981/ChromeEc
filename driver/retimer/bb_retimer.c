...
int retimer_set_state(int port, mux_state_t mux_state)
{
	...
	/* REF: Retimer AIC & PD spec for Intel Type-C USB4 systems: Table 7 */
	if (mux_state & (USB_PD_MUX_TBT_COMPAT_ENABLED |
			 USB_PD_MUX_USB4_ENABLED)) {
		cable_resp = get_cable_resp_vdo(port);
		dev_resp = get_dev_resp_vdo(port);
		...
		if (get_usb_pd_mux_cable_type(port) == IDH_PTYPE_ACABLE)
			set_retimer_con |= BB_RETIMER_TBT_COMPAT_ACTIVE;
		...
		if (is_tbt_compat_enabled(port))
			set_retimer_con |= BB_RETIMER_TBT_COMPAT_CONN;
		 ...
		 */
		if (dev_resp.tbt_adapter)
			set_retimer_con |= BB_RETIMER_TBT_COMPAT_TYPE;
		 ...
		 * 1 - Optical cable
		 */
		if (cable_resp.tbt_cable_type)
			set_retimer_con |= BB_RETIMER_TBT_COMPAT_CTYPE;
		 ...
		 */
		if (cable_resp.link_lsrx_comm &&
		   (get_usb_pd_mux_cable_type(port) & IDH_PTYPE_ACABLE))
			set_retimer_con |= BB_RETIMER_TBT_COMPAT_LSRX_LINK;
		 ...
		 * 10..11b - Reserved
		 */
		set_retimer_con |= BB_RETIMER_TBT_COMPAT_CABLE_SPEED(
						cable_resp.tbt_cable_speed);

		/*
		 * Bits 28...29: TBT/USB4_GEN_SUPPORT
		 * 00b - 3rd generation TBT (10.3125 and 20.625Gb/s)
		 * 01b - 4th generation TBT (10.0, 10.3125, 20.0, 20.625)
		 * 10..11b - Reserved
		 */
		set_retimer_con |= BB_RETIMER_TBT_COMPAT_CABLE_GEN(
					       cable_resp.tbt_rounded_support);
	}
