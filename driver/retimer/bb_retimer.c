...
static void bb_retimer_power_handle(int port, int on_off)
{
	...
	if (on_off) {
		gpio_set_level(retimer->ctrl.usb_ls_en_gpio, 1);
		msleep(1);
		gpio_set_level(retimer->ctrl.retimer_rst_gpio, 1);
		msleep(10);
		gpio_set_level(retimer->ctrl.force_power_gpio, 1);
		 ...
		 */
		if (retimer->ctrl.shared_nvm)
			msleep(40);
		else
			msleep(20);
	} else {
		gpio_set_level(retimer->ctrl.force_power_gpio, 0);
		msleep(1);
		gpio_set_level(retimer->ctrl.retimer_rst_gpio, 0);
		msleep(1);
		gpio_set_level(retimer->ctrl.usb_ls_en_gpio, 0);
	}
}
