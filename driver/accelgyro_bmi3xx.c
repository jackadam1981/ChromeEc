			      ...
			      int rate,int rnd)
{
	...
	comm_rslt = bmi_get_normalized_rate(s, rate, rnd,
	...
	if (comm_rslt == EC_SUCCESS) {
		...
		comm_rslt = bmi3_i2c_read(BMI3_REG_ACC_CONF + s->type, reg_data, 4, s);
		...
		if (comm_rslt == EC_SUCCESS) {
			...
			if (s->type == MOTIONSENSE_TYPE_GYRO) {
				if (rate == 0) {
					 ...
					 */
					reg_data[3] = BMI3_SET_BITS(reg_data[3],
					BMI3_POWER_MODE, BMI3_GYR_MODE_NORMAL);
				}
				reg_data[2] = BMI3_SET_BIT_POS0(reg_data[2], \
							BMI3_SENS_ODR, reg_val);
			}
			...
			comm_rslt = bmi3_i2c_write(BMI3_REG_ACC_CONF + \
...
int bmi3_set_range(struct motion_sensor_t *s, int range, int rnd)
{
	...
	if (comm_rslt == EC_SUCCESS) {
		...
		/* Set the accel/gyro configurations. */
		comm_rslt = bmi3_i2c_write(BMI3_REG_ACC_CONF + s->type, \
					   &reg_data[2], 2, s);
...
int bmi3_read(const struct motion_sensor_t *s, intv3_t v)
{
	...
	comm_rslt = bmi3_i2c_read(BMI3_REG_STATUS, reg_data, 4, s);

	if (comm_rslt == EC_SUCCESS) {
		status = (reg_data[2] | ((uint16_t)reg_data[3] << 8));
		/*
		...
		if (s->type == MOTIONSENSE_TYPE_ACCEL) {
			...
			comm_rslt = bmi3_i2c_read(BMI3_REG_ACC_DATA_X, \
						  reg_data, 8, s);
		} else if (s->type == MOTIONSENSE_TYPE_GYRO) {
			/* Read the sensor data */
			comm_rslt = bmi3_i2c_read(BMI3_REG_GYR_DATA_X, \
						  reg_data, 8, s);
		}
	...
	if (comm_rslt != EC_SUCCESS) {
		(void)CPRINTF("%s: type:0x%X RD XYZ Error %d", s->name, 
			      s->type, comm_rslt);
		return comm_rslt;
	}
...
static int init(struct motion_sensor_t *s)
{
	...
	reg_data[1] = (uint8_t)((BMI3_CMD_SOFT_RESET & BMI3_SET_HIGH_BYTE) \
				>> 8);
	comm_rslt = bmi3_i2c_write(BMI3_REG_CMD, reg_data, 2, s);
	...
	if (comm_rslt == EC_SUCCESS) {
		...
		reg_data[0] = BMI3_ENABLE;
		reg_data[1] = 0;
		comm_rslt = bmi3_i2c_write(BMI3_REG_FEATURE_ENGINE_GLOB_CTRL, \
					   reg_data, 2, s);
		if (comm_rslt == EC_SUCCESS) {
			/* Read chip id */
			comm_rslt = bmi3_i2c_read(BMI3_REG_CHIP_ID, reg_data, \
						  4, s);
			if (comm_rslt)
