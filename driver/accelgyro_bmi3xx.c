...
int bmi3_parse_fifo_data(struct motion_sensor_t *s,
			 struct bmi3_fifo_frame *fifo_frame, uint32_t last_ts)
{
...
static int irq_handler(struct motion_sensor_t *s, uint32_t *event)
{
	...
	if ((ret == EC_SUCCESS) && ((int_status & BMI3_INT_STATUS_FWM) ||
		...
		reg_data[3] =
			BMI3_GET_BIT_POS0(reg_data[3], BMI3_FIFO_FILL_LVL);
...
static int perform_calib(struct motion_sensor_t *s, int enable)
{
	...
	RETURN_ERROR(
		bmi3_read_n(s, BMI3_REG_ACC_CONF + s->type, saved_conf, 4));
...
static int set_data_rate(const struct motion_sensor_t *s, int rate, int rnd)
{
	...
	if (s->type == MOTIONSENSE_TYPE_ACCEL) {
		...
		reg_data[2] =
			BMI3_SET_BIT_POS0(reg_data[2], BMI3_SENS_ODR, reg_val);
...
static int init(struct motion_sensor_t *s)
{
	...
	reg_data[1] =
		(uint8_t)((BMI3_CMD_SOFT_RESET & BMI3_SET_HIGH_BYTE) >> 8);
