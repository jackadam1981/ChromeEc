...
static int config_interrupt(const struct motion_sensor_t *s)
{
	...
	if (comm_rslt == EC_SUCCESS) {
		...
		comm_rslt |= bmi3_write_n(s, BMI3_REG_INT_MAP1, &reg_data[2],
					  4);
		...
		if (comm_rslt == EC_SUCCESS) {
			...
			reg_data[0] = 1;
			reg_data[1] = 0;
			comm_rslt = bmi3_write_n(s, BMI3_REG_FIFO_WATERMARK,
						   reg_data, 2);

			if (comm_rslt == EC_SUCCESS) {
				/* Get the previous configuration data */
				comm_rslt |= bmi3_read_n(s,
						BMI3_REG_IO_INT_CTRL,
						reg_data, 6);
				reg_data[2] = BMI3_SET_BIT_POS0(reg_data[2],
							BMI3_INT1_LVL,
							BMI3_INT_ACTIVE_LOW);

				reg_data[2] = BMI3_SET_BITS(reg_data[2],
							BMI3_INT1_OD,
							BMI3_INT_PUSH_PULL);
				...
				reg_data[4] = BMI3_SET_BIT_POS0(reg_data[4],
						BMI3_INT_LATCH,
						BMI3_INT_LATCH_EN);
				 ...
				 * latch settings
				 */
				comm_rslt |= bmi3_write_n(s,
						BMI3_REG_IO_INT_CTRL,
						&reg_data[2], 4);

				if (comm_rslt == EC_SUCCESS) {
					/* Set FIFO config to enable accel gyro
					 * data
					 */
					reg_data[0] = 0;
					reg_data[1] = BMI3_FIFO_ACC_EN
						      | BMI3_FIFO_GYR_EN;
					comm_rslt = bmi3_write_n(s,
							BMI3_REG_FIFO_CONF,
							reg_data, 2);
		...
		uint32_t *event)
{
	...
	comm_rslt = bmi3_read_n(s, BMI3_REG_INT_STATUS_INT1, reg_data, 4);
	...
	if ((comm_rslt == EC_SUCCESS) && ((int_status & BMI3_INT_STATUS_FWM) ||
		...
		comm_rslt = bmi3_read_n(s, BMI3_REG_FIFO_CONF, reg_data, 4);
		fifo_frame.available_fifo_sens = reg_data[3] & BMI3_FIFO_ALL_EN;

		/* Get the FIFO fill level in words */
		comm_rslt = bmi3_read_n(s, BMI3_REG_FIFO_FILL_LVL, reg_data, 4);
		...
		/* Read FIFO data */
		comm_rslt = bmi3_read_n(s, BMI3_REG_FIFO_DATA, bmi3_buffer,
					fifo_fill_level);
...
int get_accel_offset(const struct motion_sensor_t *s, intv3_t v)
{
	...
	v[1] = ((uint16_t)(reg_data[5] << 8) | reg_data[4]) & 0x1FFF;
	v[2] = ((uint16_t)(reg_data[7] << 8) | reg_data[6]) & 0x1FFF;
...
static int perform_calib(struct motion_sensor_t *s, int enable)
{
	...
	(void)bmi3_write_n(s, BMI3_REG_ACC_CONF + s->type, &saved_conf[2], 2);
