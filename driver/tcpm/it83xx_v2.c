	...
	enum usbpd_cc_pin cc_pin)
{
	 ...
	 * when we receive SOP'/SOP'', the getting power role synchronize with
	 * pd[port].power_role (also synchronization with tcpm_set_cc)
	 * instead of message header.
	 */
	/* Sink */
	if (pd_get_role(port) == USBPD_POWER_ROLE_SNK) {
	...
	const uint32_t *buf)
{
	...
	for (r = 0; r <= PD_RETRY_COUNT; r++) {
		...
		/*
		 * Check Tx error status (HW won't set multi-errors on once tx):
		...
		if (tx_error_status[port] || (evt & TASK_EVENT_TIMER)) {
			if (tx_error_status[port] &
				...
				CPRINTS("p%d TxErr: Not receive GoodCRC", port);
				tx_error_status[port] &=
					~USBPD_REG_MASK_TX_NO_RESPONSE_STAT;
