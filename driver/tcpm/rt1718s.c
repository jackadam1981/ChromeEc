...
int rt1718s_set_frs_enable(int port, int enable)
{
	...
	if (enable) {
		...
		if (FLAG_FRS_RX_SIGNALED ==
		    (frs_flag[port] &
		     (FLAG_FRS_RX_SIGNALED | FLAG_FRS_VBUS_VALID_FALL))) {
			/*
			 * Skip disable if we had these signals, and deferred
			 * the FRS register disable process in
	 ...
	 * If this function gets called when FRS RX signaled, then
	 * we'll deferred the GPIO disabled until the VBUS valid drop. So
	 * don't disable it here.
	 */
	if (enable || !(frs_flag[port] & FLAG_FRS_RX_SIGNALED))
