...
int rt1718s_write8_no_lpm(int port, int reg, int val)
{
	return i2c_write8(
...
int rt1718s_read8_no_lpm(int port, int reg, int *val)
{
	return i2c_read8(
		tcpc_config[port].i2c_info.port,
		tcpc_config[port].i2c_info.addr_flags,
...
void rt1718s_vendor_defined_alert(int port)
{
	...
	if (IS_ENABLED(CONFIG_USB_PD_FRS)) {
		...
		rv = rt1718s_read8_no_lpm(port, RT1718S_RT_INT1, &int1);
		if (rv)
			return;
		rv = rt1718s_write8_no_lpm(port, RT1718S_RT_INT1, 0b11000000);
		if (rv)
			return;
	...
	/* Process BC12 alert */
	rv = rt1718s_read8_no_lpm(port, RT1718S_RT_INT6, &int6);
	if (rv)
		return;

	if (int6) {
		/* clear BC12 alert */
		rv = rt1718s_write8_no_lpm(port, RT1718S_RT_INT6, int6);
		if (rv)
			return;
	...
	rv = rt1718s_write8_no_lpm(port, RT1718S_RT_INT2, 0xFF);
	if (rv)
		return;
	/* ES1 workaround: disable Vconn discharge */
	rv = rt1718s_write8_no_lpm(port, RT1718S_SYS_CTRL2, 0x87);
	if (rv)
		return;
