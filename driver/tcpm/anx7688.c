...
static int anx7688_init(int port)
{
	 ...
	 * Instead we poll TCPC 0x50:0x29 bit0 here to make sure bootdone is
...
static int anx7688_mux_set(int i2c_addr, mux_state_t mux_state)
{
	...
	/*
	 * TODO: only support DP + USB until FW 1.12
	 * should remove this after we can deploy the latest FW to all Elms
	 */
...
DECLARE_HOOK(HOOK_INIT, anx7688_driver_init, HOOK_PRIO_INIT_TCPC);
