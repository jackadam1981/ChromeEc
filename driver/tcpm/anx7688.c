...
static int anx7688_init(int port)
{
	 ...
	 * due to it is default 0 in HW, and we cannot do TCPC initialization
	 * until it is ready, or something goes wrong.
	 * 7688 issues power status change alert 60ms after being powered, so
	 * we use this as an indication of 7688 is ready. However experiments
	 * shown it needs another "20" ms to be really ready. Delay here
	 * because alert will trigger a RESET and then tcpm_init().
	 * The delay is actually not needed for the first tcpm_init() after
	 * system boot up since it fails and loops anyway due to 7688 is not
	 * powered. However this extra delay should be harmless since EC starts
	 * up multiple threads at the same time.
	 *
	 * TODO: what is the exact number of this delay?
	 */
	msleep(20);
