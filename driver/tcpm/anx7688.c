...
#define ANX7688_REG_DEBUG24		0x29
#define ANX7688_REG_DEBUG24_TCPC1_DONE	(1 << 1)
#define ANX7688_REG_DEBUG24_BOOT_DONE	(1 << 0)
...
static int anx7688_init(int port)
{
	...
	while (1) {
		rv = i2c_read8(I2C_PORT_TCPC, ANX7688_USBC_ADDR,
			       ANX7688_REG_DEBUG24, &mask);

		if (rv == EC_SUCCESS &&
		    (mask & ANX7688_REG_DEBUG24_BOOT_DONE)) {
			CPRINTS("boot load done = 0x%x", mask & ANX7688_REG_DEBUG24_BOOT_DONE);
			break;
		}
		msleep(10);
	}
