...
static bool raa489000_bist_test_mode_pre[CONFIG_USB_PD_PORT_MAX_COUNT];
...
enum ec_error_list raa489000_set_bist_test_mode(const int port, const bool enable)
{
	...
	if(raa489000_bist_test_mode[port] != raa489000_bist_test_mode_pre[port]){
		CPRINTS("C%d: bist test mode %s", port, enable ? "enable" : "disable");
		tcpci_set_bist_test_mode(port, enable);
		raa489000_bist_test_mode_pre[port] = raa489000_bist_test_mode[port];
	}
