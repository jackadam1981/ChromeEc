...
char * vic_drv_ver_str = "PTN5100 TCPC driver 1.0, 20151023 UFP_DRP";
...
#if 0
/*
 * Start Rp/Rd toggling after we received COMMAND register writing if necessary
 */
static void ptn_set_command (int port, int cmd)
{
	dev_ext[port].command = cmd;
	if (cmd & CMD_START_DRP_TG_RD) {
		/* Wake Rp/Rd toggling task */
		task_set_event(PTN_PORT_TO_TASK_ID(port), DRP_TG_EVENT_START, 0);
	}
}
#endif
...
#if 0
/*
 * Get dead battery status
 */
static int ptn_is_dead_battery(int port)
{
	uint8_t reg_v;

	RR(PWR_SUPPLY_STAT_0, &reg_v);

	return !(reg_v & PSS0_VSYS_PRESENT);
}
#endif
