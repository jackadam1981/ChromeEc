...
{
	return write_register(SLAVE1, 0x4b, 0x10);
}

static int enable_wp(void)
{
	return write_register(SLAVE1, 0x4b, 0x00);
}
...
static int get_ROM_id(uint8_t buf[2])
{
	int status;
	int rc;
	int val;
	...
	read_register(SLAVE2, 0x91, &val);
	buf[0] = val;
	rc = read_register(SLAVE2, 0x91, &val);
	buf[1] = val;
...
static int write_status_register(int data, uint8_t access)
{
...
static int program(int address, const uint8_t data[8])
{
	...
	if ((address & 0xff) == 0) {
		write_register(SLAVE2, 0x8e, address >> 8);
		write_register(SLAVE2, 0x8f, 0x03);
		offset = 0;
	}

	for (i = 0; i < 8; i++)
		write_register(0x1e, offset++, data[i]);
