 ...
 * from tcpc_init.
 */
int tcpci_tcpm_mux_init(struct usb_mux *this)
{
	int error;
	int power_status;
	int tries = TCPM_INIT_TRIES;
	...
	if (!(this->flags & USB_MUX_FLAG_NOT_TCPC))
		return EC_SUCCESS;

	/* Wait for the device to exit low power state */
	while (1) {
		error = mux_read(this, TCPC_REG_POWER_STATUS, &power_status);
		/*
		 * If read succeeds and the uninitialized bit is clear, then
	...
	error = mux_write16(this, TCPC_REG_ALERT_MASK, 0);
	error |= mux_write16(this, TCPC_REG_ALERT, 0xffff);

	return error ? EC_ERROR_UNKNOWN : EC_SUCCESS;
}

int tcpci_tcpm_mux_enter_low_power(struct usb_mux *this)
{
	/* If this MUX is also the TCPC, then skip low power */
	if (!(this->flags & USB_MUX_FLAG_NOT_TCPC))
		return EC_SUCCESS;

	return mux_write(this, TCPC_REG_COMMAND, TCPC_REG_COMMAND_I2CIDLE);
}

int tcpci_tcpm_mux_set(struct usb_mux *this, mux_state_t mux_state)
{
	int rv;
	int reg = 0;

	/* Parameter is port only */
	rv = mux_read(this, TCPC_REG_CONFIG_STD_OUTPUT, &reg);
	if (rv != EC_SUCCESS)
		return rv;
	...
	/* Parameter is port only */
	return mux_write(this, TCPC_REG_CONFIG_STD_OUTPUT, reg);
}

/* Reads control register and updates mux_state accordingly */
int tcpci_tcpm_mux_get(struct usb_mux *this, mux_state_t *mux_state)
{
	int rv;
	int reg = 0;
