...
void tcpci_tcpc_discharge_vbus(int port, int enable)
{
	tcpc_update(port,
		    TCPC_REG_POWER_CTRL,
		    (enable) ? 0 : TCPC_REG_POWER_CTRL_FORCE_DISCHARGE,
...
{
	tcpc_update(port, TCPC_REG_POWER_CTRL,
		    (connected)
			? 0
...
int tcpci_tcpm_set_polarity(int port, int polarity)
{
	return tcpc_update(port,
			   TCPC_REG_TCPC_CTRL,
			   (polarity) ? 0 : TCPC_REG_TCPC_CTRL_SET(1),
			   (polarity) ? TCPC_REG_TCPC_CTRL_SET(1) : 0);
}
...
void tcpci_tcpc_fast_role_swap_enable(int port, int enable)
{
	tcpc_update(port,
		    TCPC_REG_POWER_CTRL,
		    (enable) ? 0 : TCPC_REG_POWER_CTRL_FRS_ENABLE,
		    (enable) ? TCPC_REG_POWER_CTRL_FRS_ENABLE : 0);
