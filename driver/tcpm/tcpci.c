...
/*
 * For PS8751, this function will fail if the chip is in low power mode. It can
 * be woken up by reading a random register first then wait for 10ms.
 *
 ...
 * will be stored in cache, which can be accessed by tcpc_get_chip_info without
