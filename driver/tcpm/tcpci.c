...
int tcpc_write(int port, int reg, int val)
{
	...
	rv = i2c_write8(tcpc_config[port].slave_addr, reg, val);
...
int tcpc_write16(int port, int reg, int val)
{
	...
	rv = i2c_write16(tcpc_config[port].slave_addr, reg, val);
...
int tcpc_read(int port, int reg, int *val)
{
	...
	rv = i2c_read8(tcpc_config[port].slave_addr, reg, val);
...
int tcpc_read16(int port, int reg, int *val)
{
	...
	rv = i2c_read16(tcpc_config[port].slave_addr, reg, val);
...
int tcpc_read_block(int port, int reg, uint8_t *in, int size)
{
	...
	rv = i2c_read_block(tcpc_config[port].slave_addr, reg, in, size);
...
int tcpc_write_block(int port, int reg, const uint8_t *out, int size)
{
	...
	rv = i2c_write_block(tcpc_config[port].slave_addr, reg, out, size);
			    ...
			    uint8_t *in, int in_size, int flags)
{
	...
	rv = i2c_xfer_unlocked(tcpc_config[port].slave_addr,
