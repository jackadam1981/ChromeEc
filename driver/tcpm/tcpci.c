...
/* For PS8751, this function will fail if the chip is in low power mode. As a
 * good practice, this function should be called immediately after the chip is
 * reset or initialized because it'll gurantee the chip is awake. Once it's
 * called, the chip info will be stored in cache, which can be accessed by
 * tcpc_get_chip_info without worrying about chip states. */
int tcpci_read_chip_info(int port, struct ec_response_pd_chip_info *info)
{
	int error;
	int val;
