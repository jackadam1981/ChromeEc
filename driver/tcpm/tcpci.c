...
/*
 * Should only be called by a single task (can be different that dequeuing
 * task though).
 *
 * Note this method can be called from an interrupt context.
 */
int tcpm_cache_message(const int port)
{
	int rv;
...
/*
 * Should only be called by a single task (can be different that enqueuing
 * task though)
 */
