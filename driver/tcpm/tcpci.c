 ...
 *
 * BIT(VBUS_LEVEL_SAFE0V) will indicate if in SAFE0V
 * BIT(VBUS_LEVEL_SAFE5V) will indicate if in SAFE5V
 */
...
#ifdef CONFIG_USB_PD_VBUS_DETECT_TCPC
int tcpci_tcpm_chk_vbus_level(int port, enum vbus_level level)
{
	if (level == VBUS_LEVEL_SAFE0V)
		return !!(tcpc_vbus[port] & BIT(VBUS_LEVEL_SAFE0V));
	else
		return !!(tcpc_vbus[port] & BIT(VBUS_LEVEL_SAFE5V));
}
#endif
...
void tcpci_tcpc_alert(int port)
{
	...
	if ((tcpc_config[port].flags & TCPC_FLAGS_TCPCI_REV2_0) &&
		...
		if (ext_status & TCPC_REG_EXT_STATUS_SAFE0V)
			tcpc_vbus[port] = BIT(VBUS_LEVEL_SAFE0V);
	}
	if (alert & TCPC_REG_ALERT_POWER_STATUS) {
		...
		if (pwr_status & TCPC_REG_POWER_STATUS_VBUS_PRES)
			...
			tcpc_vbus[port] = BIT(VBUS_LEVEL_SAFE5V);
	...
	if (tcpc_config[port].flags & TCPC_FLAGS_TCPCI_REV2_0) {
		...
		tcpc_vbus[port] = (power_status &
				   TCPC_REG_POWER_STATUS_VBUS_PRES)
					? BIT(VBUS_LEVEL_SAFE5V)
					: BIT(VBUS_LEVEL_SAFE0V);
	}
...
#ifdef CONFIG_USB_PD_VBUS_DETECT_TCPC
	.chk_vbus_level		= &tcpci_tcpm_chk_vbus_level,
#endif
