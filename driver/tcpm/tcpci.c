	...
	*cc2 = TCPC_REG_CC_STATUS_CC2(status);
	...
	if (TCPC_REG_ROLE_CTRL_DRP(role)) {
		if (*cc1 != TYPEC_CC_VOLT_OPEN)
			cc1_term = TCPC_REG_CC_STATUS_TERM(status);
		if (*cc2 != TYPEC_CC_VOLT_OPEN)
			cc2_term = TCPC_REG_CC_STATUS_TERM(status);
	} else {
		if (*cc1 != TYPEC_CC_VOLT_OPEN) {
			cc1_term = (TCPC_REG_ROLE_CTRL_CC1(role) ==
					TYPEC_CC_RD) ? 1 : 0;
		}
		if (*cc2 != TYPEC_CC_VOLT_OPEN) {
			cc2_term = (TCPC_REG_ROLE_CTRL_CC2(role) ==
					TYPEC_CC_RD) ? 1 : 0;
		}
	}
	*cc1 |= cc1_term << 2;
...
int tcpci_tcpm_set_polarity(int port, enum tcpc_cc_polarity polarity)
{
	...
	/*
	 * If our last set_cc was to OPEN then we do not want to set
	 * the polarity if NONE, otherwise the connection will be disabled
	 * for both polarities and we will not be able to use the port.
	 */
