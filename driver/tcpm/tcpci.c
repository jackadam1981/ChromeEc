...
static bool all_bist_mode[CONFIG_USB_PD_PORT_MAX_COUNT];
...
static bool all_tcpm_should_enter_bist_mode(int port, uint32_t *payload,
					    int *head)
{
	uint32_t hdr = *head;
	/*
	...
	if ((PD_HEADER_EXT(hdr) == 0) && (PD_HEADER_CNT(hdr) > 0) &&
	    ...
	    (!all_bist_mode[port]) && pd_vbus_valid_for_bist(port)) {
		return true;
	}
...
int all_tcpm_get_message_raw(int port, uint32_t *payload, int *head)
{
	...
	if (all_tcpm_should_enter_bist_mode(port, payload, head)) {
		all_bist_mode[port] = true;
		ret = tcpci_set_bist_test_mode(port, true);
	}
	return ret;
}

enum ec_error_list all_set_bist_test_mode(const int port, const bool enable)
{
	all_bist_mode[port] = enable;
	 ...
	 */
	if (!all_bist_mode[port]) {
		return tcpci_set_bist_test_mode(port, enable);
	}
	return EC_SUCCESS;
}

enum ec_error_list all_get_bist_test_mode(const int port, bool *enable)
{
	*enable = all_bist_mode[port];
...
const struct tcpm_drv tcpci_tcpm_drv = {
	...
	.get_message_raw = &all_tcpm_get_message_raw,
...
#endif
	.set_bist_test_mode = &all_set_bist_test_mode,
	.get_bist_test_mode = &all_get_bist_test_mode,
