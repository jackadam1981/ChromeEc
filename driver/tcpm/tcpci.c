	...
	enum tcpc_cc_voltage_status *cc2)
{
	...
	*cc2 = TCPC_REG_CC_STATUS_CC2(status);
	...
	if (TCPC_REG_ROLE_CTRL_DRP(role)) {
		/*
		 * When we are Looking4Connection the CC_STATUS will
		 * correctly provide the term value.
		 */
		...
		/*
		 * When we are not Looking4Connection the ROLE is
		 * needed to get the correct term value.
		 */
		if (*cc1 != TYPEC_CC_VOLT_OPEN) {
