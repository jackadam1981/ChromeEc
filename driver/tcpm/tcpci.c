...
/*
 * Save the selected role values.  Leaving these public and should
 * be updated appropriately by any driver in order to make sure
 * generic functions will continue to work if only part of the
 * tcpci is overridden by the chip driver.
 */
int tcpc_selected_rp[CONFIG_USB_PD_PORT_MAX_COUNT];
enum tcpc_cc_pull tcpc_selected_pull[CONFIG_USB_PD_PORT_MAX_COUNT];
...
int tcpci_tcpm_set_cc(int port, int pull)
{
	...
	return tcpc_write(port, TCPC_REG_ROLE_CTRL,
			  TCPC_REG_ROLE_CTRL_SET(0, tcpc_selected_rp[port],
						 cc1, cc2));
}
...
int tcpci_tcpm_set_polarity(int port, enum tcpc_cc_polarity polarity)
{
	 ...
	 * one is driven.
	 */
	rv = tcpm_set_cc(port, tcpc_selected_pull[port]);
	if (rv)
		return rv;
...
int tcpci_tcpm_init(int port)
{
	...
	tcpc_selected_pull[port] = TYPEC_CC_OPEN;
