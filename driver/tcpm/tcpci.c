 ...
 * that have been reset to TCPCI default values (step 4 below).
 *
 * USB TCPCI Specification R2 V1P1 the TCPC will do the following
 ...
 *      If a previous TRANSMIT request has not yet completed, the TCPC shall
 *      assert the TransmitSOP*MessageDiscarded bit in the ALERT register.
 ...
 * 3. The TCPC clears the RECEIVE_DETECT and READABLE_BYTE_COUNT registers to
 *    disable the USB PD message passing.
 * 4. The TCPC resets the mask registers (ALERT_MASK, POWER_STATUS_MASK,
 *    EXTENDED_STATUS_MASK, ALERT_EXTENDED_MASK) per Table 4-1
 * 5. The TCPM clears the Alert by writing a logical 1 to the asserted bit
 *    in the ALERT register. If ALERT.ReceiveSOP*MesageStatus bit is asserted,
 *    the TCPM shall also clear the ALERT.ReceiveSOP*MessageStatus bit.
 * 6. The TCPM writes to the RECEIVE_DETECT register to enable USB PD message
 *    passing.
 */
...
const struct tcpm_drv tcpci_tcpm_drv = {
	...
	.hard_reset_init	= &tcpci_tcpm_hard_reset_init,
