...
static int init_alert_mask(int port)
{
	...
	if (get_usb_pd_vbus_detect() == USB_PD_VBUS_DETECT_TCPC) {
		mask = TCPC_REG_ALERT_TX_SUCCESS | TCPC_REG_ALERT_TX_FAILED |
			...
			;
		CPRINTS("mask with VBUS DETECT TCPC: 0x%X", mask);
	} else {
		mask = TCPC_REG_ALERT_TX_SUCCESS | TCPC_REG_ALERT_TX_FAILED |
			TCPC_REG_ALERT_TX_DISCARDED | TCPC_REG_ALERT_RX_STATUS |
			TCPC_REG_ALERT_RX_HARD_RST | TCPC_REG_ALERT_CC_STATUS |
			TCPC_REG_ALERT_FAULT
			;
		CPRINTS("mask without VBUS DETECT TCPC: 0x%X", mask);
	}
...
static int init_power_status_mask(int port)
{
	...
	if (get_usb_pd_vbus_detect() == USB_PD_VBUS_DETECT_TCPC)
	{
		...
		CPRINTS("Power Status mask with VBUS DETECT TCPC enabled: 0x%X", mask);
	}
	else
	{
		mask = 0;
		CPRINTS("Power Status mask without VBUS DETECT TCPC: 0x0");
	}
