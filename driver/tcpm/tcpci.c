...
int tcpci_tcpc_drp_toggle(int port)
{
	 ...
	 * that we are disconnecting from. TCPCI shows going from
	 * SNK->DRP that we should be going to Rd/Rd and going from
	 * SRC->DRP that we should be going to Rp/Rp. The function
	 * tcpci_tcpc_set_connection performs this action and it may
	 * be wise as chips can use this to make this the standard
	 * and remove this set_role_ctrl call.
	 */
