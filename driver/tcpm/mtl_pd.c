...
//!memcmp(&g_data_status[port].raw_value[0], &status_data.raw_value[0], sizeof(union mtl_data_status)))
...
static void mtl_pd_process_data(int port)
{
	...
	if (!mtl_pd_read(i2c_port, i2c_addr[port], &status_data)) {
		if (pd_data_changed(&g_data_status[port].raw_value[0],
				...
				sizeof(union mtl_data_status));
			{
				...
				for (i = 0; i < DATA_STATUS_REG_LEN; i++)
					ccprintf("0x%x 0x%x\n", g_data_status[port].raw_value[i], status_data.raw_value[i]);
			}
