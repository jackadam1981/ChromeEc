...
extern int image1_rows;
extern int image2_rows;
...
static int ccgxxf_fw_update(int port)
{
  ...
  /* Prepare metadata */
  if ( (fwu_status >> 8) == 2 )
    rv = tcpc_write_block(port, CCGXXF_REG_FWU_BUFFER, image1_metadata, 129);
  else
    rv = tcpc_write_block(port, CCGXXF_REG_FWU_BUFFER, image2_metadata, 129);
  if (rv)
    return rv;
  msleep(5); 
  ...
  rv = ccgxxf_get_fwu_status_reg(port, 5000, &fwu_status);
