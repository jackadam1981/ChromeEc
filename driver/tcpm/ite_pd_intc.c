...
void auto_toggle_timer_interrupt(enum usbpd_port port)
{
	...
	if (port == USBPD_PORT_A) {
		...
		if (USBPD_GET_CC1_PULL_REGISTER_SELECTION(port) ==  USBPD_POWER_ROLE_SRC) {
			/* We're SRC, CCs toggle to Rd */
			tcpm_set_cc(port, TYPEC_CC_RD);
			hw_cnt = MS_TO_COUNT(1024, (PD_T_DRP_SNK / MSEC));
		} else {
			/* We're SNK, CCs toggle to Rp */
			tcpm_set_cc(port, TYPEC_CC_RP);
			hw_cnt = MS_TO_COUNT(1024, (PD_T_DRP_SRC / MSEC));
		}
		...
		if (USBPD_GET_CC1_PULL_REGISTER_SELECTION(port) ==  USBPD_POWER_ROLE_SRC) {
			/* We're SRC, CCs toggle to Rd */
			tcpm_set_cc(port, TYPEC_CC_RD);
			hw_cnt = MS_TO_COUNT(32768, (PD_T_DRP_SNK / MSEC));
		} else {
			/* We're SNK, CCs toggle to Rp */
			tcpm_set_cc(port, TYPEC_CC_RP);
			hw_cnt = MS_TO_COUNT(32768, (PD_T_DRP_SRC / MSEC));
		}
