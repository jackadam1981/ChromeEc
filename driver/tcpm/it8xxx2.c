...
static void it83xx_tcpm_cc_disconnect_int_enable(int port, int enable)
{
	...
	if (enable)
		...
		/*
		 * Enable detect type-c plug in/out interrupt,
		 * type select sink,
		 * clear detect type-c device plug in/out status,
		 * switch to detect type-c plug out.
		 */
		...
		/* Disable detect type-c plug in/out interrupt */
		IT83XX_USBPD_TCDCR(port) |=
			USBPD_REG_PLUG_IN_OUT_DETECT_DISABLE;
}
...
static void it83xx_tcpm_sw_reset(void)
{
	...
	if (IS_ENABLED(IT83XX_INTC_PLUG_IN_OUT_SUPPORT))
		/*
		 * Switch to detect type-c plug in and enable detect type-c
		 * plug in interrupt, since the pd task has detected a type-c
