...
static void it83xx_tcpm_set_frs_enable(int port, int enable)
{
	...
	if (enable) {
		...
		IT83XX_USBPD_PDFSCR(port)  = (IT83XX_USBPD_PDFSCR(port) & ~mask)
					| USBPD_REG_FAST_SWAP_DETECT_ENABLE;
		/*
		 * TODO: Enable HW auto trigger GPH3(port0)/GPH4(port1) output
		 *       H/L when we detect FRS cc low signal.
		 */
	} else {
		...
		/*
		 * TODO: Disable HW auto trigger GPH3(port0)/GPH4(port1) output
		 *       H/L when we detect FRS cc low signal.
		 */
	}
}
