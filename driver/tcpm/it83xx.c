...
static void it83xx_set_power_role(enum usbpd_port port, int power_role)
{
	...
	if (power_role == PD_ROLE_SOURCE) {
		/*
		 * Measure BMC Rx threshold setting of sourcing power(Y3Rx),
		 * High to low Y3Rx = 0.64,
		 * Low to high Y3Rx = 0.79
		 */
		IT83XX_USBPD_CCADCR(port) = USBPD_REG_MASK_BMC_RX_THRESHOLD_SRC;
		/* bit0: source */
		...
		/*
		 * Measure BMC Rx threshold setting of sinking power(Y3Rx),
		 * High to low Y3Rx = 0.38,
		 * Low to high Y3Rx = 0.54
		 */
		IT83XX_USBPD_CCADCR(port) = USBPD_REG_MASK_BMC_RX_THRESHOLD_SNK;
		/* bit0: sink */
