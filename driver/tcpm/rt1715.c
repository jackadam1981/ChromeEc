...
static int rt1715_enable_ext_messages(int port, int enable)
{
	return tcpc_update8(port, RT1715_REG_VENDOR_5,
		  RT1715_REG_VENDOR_5_ENEXTMSG, enable
		    ? MASK_SET : MASK_CLR);
}
...
static int rt1715_tcpci_tcpm_init(int port)
{
	 ...
	 * Bit 7 <ENRETRY_DISCARD>: Wait for tReceive before retrying
	 * transmit in response to a bad GoodCRC.
	 * Bit 6:4 <TRANSCNT>: Consider CC to be idle if there are 7
	 * or fewer BMC transients observed in <46.67us>
	 */
	...
	/*
	 * Set PHY control registers to Richtek recommended values
	 * Decrease the time that the PHY will wait for a second
	 * transition to detect a BMC-encoded 1 bit from 2.67 us to
	 * 2.25 us. Timeout = register value * .04167 us.
	 */
...
/*
 * Selects the CC PHY noise filter voltage level according to
 * the current CC voltage level.
 *
