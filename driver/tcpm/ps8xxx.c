			...
			struct ec_response_pd_chip_info_v1 *chip_info)
{
	...
	CPRINTS("fw version %lld", chip_info->fw_version_number);
	...
	if (chip_info->fw_version_number == 0 ||
		...
		CPRINTS("=== %s: get chip info FW version %x", __func__, val);
		if (rv != EC_SUCCESS)
			return rv;
...
static int ps8xxx_dci_disable(int port)
{
	...
	CPRINTS("%s: product_id %x", __func__, default_product_id[port]);
...
#else
	default_product_id[port] = PS8XXX_PRODUCT_ID;
#endif
