...
static int tcpm_get_power_status(int port, int *status)
{
...
#ifdef CONFIG_USB_PD_VBUS_DETECT_TCPC
static int mt6370_tcpm_get_vbus_level(int port)
{
	return tcpc_vbus[port];
}
#endif

static void mt6370_tcpc_alert(int port)
{
	int status;
	int power_status;
...
static int mt6370_get_cc(int port, int *cc1, int *cc2)
{
	...
	*cc2 = TCPC_REG_CC_STATUS_CC2(status);
	 ...
	 *
	 * The standard way to retreive Rd is checking DRP_RESULT bits in
	 * TCPC_REG_CC_STATUS. However, mt6370 tcpc doesn't correctly update
	 * the bit with CONFIG_USB_PD_DUAL_ROLE_AUTO_TOGGLE disabled.
	 * To workaround this, we check CC1/CC2 bits in TCPC_REG_ROLE_CTRL.
	 */
	rv = tcpc_read(port, TCPC_REG_ROLE_CTRL, &role);

	if (*cc1 != TYPEC_CC_VOLT_OPEN)
		*cc1 |= (TCPC_REG_ROLE_CTRL_CC1(role) ==
			 TCPC_REG_ROLE_CTRL_CC_RD) << 2;
	if (*cc2 != TYPEC_CC_VOLT_OPEN)
		*cc2 |= (TCPC_REG_ROLE_CTRL_CC2(role) ==
			 TCPC_REG_ROLE_CTRL_CC_RD) << 2;
...
#endif
#ifdef CONFIG_USB_PD_VBUS_DETECT_TCPC
	.get_vbus_level		= &mt6370_tcpm_get_vbus_level,
#endif
