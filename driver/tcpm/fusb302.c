...
#ifdef CONFIG_ZEPHYR
static struct k_mutex measure_lock;
static int init_fusb302_mutex(const struct device *dev)
{
	ARG_UNUSED(dev);
	k_mutex_init(&measure_lock);
	return 0;
}
SYS_INIT(init_fusb302_mutex, POST_KERNEL, 50);
