...
#ifdef CONFIG_USB_PD_VBUS_DETECT_TCPC
static int fusb302_tcpm_chk_vbus_level(int port, enum vbus_level level)
{
	int reg;
...
void fusb302_tcpc_alert(int port)
{
	...
	if (interrupt & TCPC_REG_INTERRUPT_VBUSOK) {
		...
		usb_charger_vbus_change(port,
					fusb302_tcpm_chk_vbus_level(port,
							VBUS_LEVEL_SAFE5V));
#else
		if (!fusb302_tcpm_chk_vbus_level(port, VBUS_LEVEL_SAFE5V))
			pd_vbus_low(port);
#endif
...
#ifdef CONFIG_USB_PD_VBUS_DETECT_TCPC
	.chk_vbus_level		= &fusb302_tcpm_chk_vbus_level,
#endif
