...
static int fusb302_i2c_write8(int port, int reg, int val)
{
	...
	rv = i2c_write8(I2C_PORT_TCPC, I2C_ADDR_TCPC(port), reg, val);
...
static int fusb302_i2c_read8(int port, int reg, int *val)
{
	...
	rv = i2c_read8(I2C_PORT_TCPC, I2C_ADDR_TCPC(port), reg, val);
...
static void detect_cc_pin_source(int port, int *cc1_lvl, int *cc2_lvl)
{
	...
	*cc2_lvl = TYPEC_CC_VOLT_OPEN;
	...
	if (togdone_pullup_cc1 == 1) {
		...
		fusb302_i2c_write8(port, TCPC_REG_MEASURE, reg);
...
static void detect_cc_pin_sink(int port, int *cc1, int *cc2)
{
	...
	bc_lvl_cc1 &= (TCPC_REG_STATUS0_BC_LVL0 |
	...
	/* mask away unwanted bits */
	bc_lvl_cc2 &= (TCPC_REG_STATUS0_BC_LVL0 |
			TCPC_REG_STATUS0_BC_LVL1);
...
int tcpm_init(int port)
{
	...
	fusb302_i2c_write8(port, TCPC_REG_POWER, reg);
...
int tcpm_set_vconn(int port, int enable)
{
	...
	if (enable) {
		...
		tcpm_set_polarity(port, cc_polarity);
	} else {
...
int tcpm_set_rx_enable(int port, int enable)
{
	...
	if (enable) {
		...
		tcpm_set_cc(port, previous_pull);
