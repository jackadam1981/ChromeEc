...
static void nct38xx_connect_state_change(int port,
					 enum tcpc_connect_state state)
{
	switch (state) {
	case TCPC_CONNECT_STATE_CONNECT:
		/* Enable Auto Discharge Disconnect */
		tcpci_tcpc_auto_discharge_disconnect(port, 1);
		break;
	case TCPC_CONNECT_STATE_DISCONNECT:
		/* Disable Auto Discharge Disconnect */
		tcpci_tcpc_auto_discharge_disconnect(port, 0);

		/* Put the ROLE Control back to IDLE Default */
		tcpc_write(port, TCPC_REG_ROLE_CTRL,
			   TCPC_REG_ROLE_CTRL_SET(
				!!IS_ENABLED(CONFIG_USB_PD_DUAL_ROLE),/* DRP */
				0,		/* Rp USB Default */
				TYPEC_CC_RD,	/* CC2 */
				TYPEC_CC_RD));	/* CC1 */
		break;
