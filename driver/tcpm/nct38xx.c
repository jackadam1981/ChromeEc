...
static int nct38xx_tcpm_init(int port)
{
	 ...
	 * [1] - FASTEN    : VBUS fast switching control and VBUS source high
	 *                   voltage enable output enable
	 * [0] - SRCEN     : VBUS source voltage enable output enable
	 */
	reg = NCT38XX_REG_CTRL_OUT_EN_SRCEN |
			NCT38XX_REG_CTRL_OUT_EN_FASTEN |
	...
	if (rv)
		return rv;
	reg = reg | TCPC_REG_FAULT_CTRL_VBUS_OVP_FAULT;
	...
	if (rv)
		return rv;
	reg = reg & ~TCPC_REG_POWER_CTRL_VBUS_VOL_MONITOR;
...
int tcpci_nct38xx_drp_toggle(int port)
{
	...
	reg = reg & ~TCPC_REG_TCPC_CTRL_SET(1);
			...
			uint16_t header, const uint32_t *data)
{
	...
	rv = tcpc_read(port, TCPC_REG_FAULT_STATUS, &tmp);
	if (rv)
		return rv;
