...
static int nct38xx_tcpm_init(int port)
{
	 ...
	 * [6] - VBSRC_EN    : VBUS source voltage enable pin
	 * [2] - ASTSW_CTL_P : VBUS fast switching control pin
	 * [1] - BSNK_EN     : VBUS sink enable pin
	 * [0] - ONN_DIR     : Connector direction indication pin
	 */
	rv = tcpc_write(port, NCT38XX_REG_CTRL_OUT_EN, 0x47);
	if (rv)
		return rv;
	...
	/* Start VBus monitor */
	rv = tcpc_write(port, TCPC_REG_COMMAND, 0x33);
	return rv;
}
...
int tcpci_nct38xx_set_polarity(int port, int polarity)
{
	...
	reg = reg & ~(1 << 4);
	reg = reg | TCPC_REG_TCPC_CTRL_SET(polarity);
...
{
	int i, rv, num_obj, tmp;
	uint32_t *buf_ptr;

	num_obj = PD_HEADER_CNT(header);
