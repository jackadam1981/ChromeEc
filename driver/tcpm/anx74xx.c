...
#define clear_recvd_msg_int(port) do {\
		int reg; \
		tcpc_read(port, ANX74XX_REG_RECVD_MSG_INT, &reg); \
		tcpc_write(port, ANX74XX_REG_RECVD_MSG_INT, reg | 0x01); \
	} while(0)
				...
				uint8_t len)
{
	...
	if (((header & 0x000d) == 0x000d) && ((header & 0x7000) == 0x0000)) {
		/*
...
#define TYPEC_CC_VOLT_OPEN           0x00
