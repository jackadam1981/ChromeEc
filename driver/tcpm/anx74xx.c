				...
				unsigned char len)
{
	...
	/* Enqueue failed, donot request anx to transmit
...
int tcpm_get_cc(int port, int *cc1, int *cc2)
{
	...
	if (!anx[port].pull) {/* get CC in sink mode */
		...
		if (reg & TCPC_REG_STATUS_CC1) {
			if (((TCPC_REG_SHIFT_2(reg)) & TCPC_REG_STATUS_CC_RA) ==
			    TCPC_REG_STATUS_CC_RA)
				*cc1 = TYPEC_CC_VOLT_RA;
			else if ((TCPC_REG_SHIFT_2(reg)) & TCPC_REG_STATUS_CC_RD)
...
int tcpm_set_cc(int port, int pull)
{
	...
	rv |= tcpc_read(port, TCPC_REG_CC_SOFTWARE_CTRL, &reg);
	reg &= TCPC_REG_CC_SW_CTRL_DISABLE;
	rv |= tcpc_write(port, TCPC_REG_CC_SOFTWARE_CTRL, reg);
...
int tcpm_init(int port)
{
	...
	/* Wait for hardware interrupt to wake up anx chip, Chip informs
	 * EC when cable connection is detected, then EC power on anx chip
	 * to bring it back to normal mode
	 */
