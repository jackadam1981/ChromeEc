...
void tcpc_clear_hpd_status(int port)
{
	int reg;
...
static int tcpm_mux_set(int i2c_addr, mux_state_t mux_state)
{
	...
	if (mux_state & MUX_USB_ENABLED) { // Pin assignment D
	...
	else if (mux_state & MUX_DP_ENABLED) {  // Pin assignment C
		if (mux_state & MUX_POLARITY_INVERTED) {
...
static int tcpm_mux_exit(int port)
{
	int  rv = EC_SUCCESS;
				...
				uint8_t len)
{
	...
	rv |= tcpc_read(port, ANX74XX_REG_TX_FIFO_CTRL, &reg);
	if (rv)
		return EC_ERROR_UNKNOWN;
	reg |= 0x01;
	rv |= tcpc_write(port, ANX74XX_REG_TX_FIFO_CTRL, reg);
	if (rv)
		return EC_ERROR_UNKNOWN;
	reg &= ~0x01;
	rv |= tcpc_write(port, ANX74XX_REG_TX_FIFO_CTRL, reg);
	...
	/* Enqueue Header */
	rv = tcpc_write(port, ANX74XX_REG_TX_WR_FIFO, (header & 0xff));
	if (rv)
		return EC_ERROR_UNKNOWN;
	rv = tcpc_write(port, ANX74XX_REG_TX_WR_FIFO, (header >> 8));
	if (rv)
		return EC_ERROR_UNKNOWN;
	...
	buf = (uint8_t *)payload;
	while (len > 0) {
		rv = tcpc_write(port, ANX74XX_REG_TX_WR_FIFO, *buf);
		if (rv) {
			num_retry++;
		} else {
			buf++;
	 ...
	 */
	rv = tcpc_read(port, ANX74XX_REG_TX_CTRL_1, &reg);
	if (rv)
		return EC_ERROR_UNKNOWN;
	reg |= ANX74XX_REG_TX_SEND_DATA_REQ;
	rv |= tcpc_write(port, ANX74XX_REG_TX_CTRL_1, reg);
...
#ifdef CONFIG_USB_PD_TCPM_MUX
	tcpm_mux_set(port, TYPEC_MUX_DOCK);
#endif
	return rv;
}
...
static int anx74xx_alert_status(int port, int *alert)
{
	...
	/* Clear soft irq bit */
	rv |= tcpc_write(port, ANX74XX_REG_IRQ_EXT_SOURCE_2,
...
int tcpm_init(int port)
{
	...
	rv |= tcpc_write(port, ANX74XX_REG_IRQ_STATUS, ANX74XX_REG_IRQ_POL_LOW);

	/* disable all interrupts */
	rv |= tcpc_write(port, ANX74XX_REG_IRQ_EXT_MASK_0,
			 ANX74XX_REG_CLEAR_SET_BITS);

	/* unmask interrupts */
	rv |= tcpc_read(port, ANX74XX_REG_IRQ_EXT_MASK_0, &reg);
	reg &= (~ANX74XX_REG_ALERT_TX_MSG_ERROR);
	reg &= (~ANX74XX_REG_ALERT_TX_CABLE_RESETOK);
	reg &= (~ANX74XX_REG_ALERT_TX_HARD_RESETOK);
	rv |= tcpc_write(port, ANX74XX_REG_IRQ_EXT_MASK_0, reg);
