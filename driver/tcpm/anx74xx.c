...
static void anx74xx_update_cable_det(int port, int mode)
{
	...
	if (mode == ANX74XX_STANDBY_MODE) {
		int cc_reg;
		int ra_mask = BIT_VALUE_OF_SRC_CC_RA |
			(BIT_VALUE_OF_SRC_CC_RA << 4);
		int rd_mask =  BIT_VALUE_OF_SRC_CC_RD |
		...
		if (!cc_reg || (cc_reg & ra_mask && !(cc_reg & rd_mask)))
...
static int anx74xx_tcpc_drp_toggle(int port, int enable)
{
	 ...
	 */
	if (!enable)
		anx74xx_handle_power_mode(port, ANX74XX_NORMAL_MODE);
	else
