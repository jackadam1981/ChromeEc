...
static int anx74xx_tcpm_mux_exit(int port)
{
	...
	rv |= tcpc_write(port, ANX74XX_REG_ANALOG_CTRL_2, reg & 0xf);
	if (rv)
		return EC_ERROR_UNKNOWN;
...
{
	int reg;
	int aux_mask = ANX74XX_REG_AUX_SWAP_SET_CC2 |
		ANX74XX_REG_AUX_SWAP_SET_CC1;

	if (tcpc_read(port, ANX74XX_REG_ANALOG_CTRL_2, &reg))
		return EC_ERROR_UNKNOWN;

	/* If aux is connected and going to disconnect, then enter safe mode */
	if (reg & aux_mask && !enabled)
		if (tcpc_write(port, ANX74XX_REG_ANALOG_CTRL_2,
			       reg | ANX74XX_REG_MODE_TRANS))
			return EC_ERROR_UNKNOWN;
