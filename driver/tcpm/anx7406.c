...
static int anx7406_set_hpd(int port, int hpd_lvl)
{
	...
	if (hpd_lvl) {
		CPRINTF("set hpd to HIGH\n");
		val = ANX7406_REG_HPD_OEN | HPD_DEGLITCH_TIME;
	} else {
		CPRINTF("set hpd to LOW\n");
...
static int anx7406_init(int port)
{
	...
	CPRINTF("C%d TCPC config disable bleed discharge\n", port);
	...
	if (rv)
		CPRINTF("C%d TCPC filter set failed\n", port);

	anx7406_hpd_reset(port);

	CPRINTF("C%d TCPC anx7406 initial success\n", port);
	return rv;
}
...
static int anx7406_set_polarity(int port, enum tcpc_cc_polarity polarity)
{
	...
	if (rv)
		CPRINTF("Update VCONN power failed\n");
...
int anx7406_m0_read(int port, int slave, int offset)
{
	...
	if (rv) {
		CPRINTF("initial cisco I2C master failed!\n");
		return rv;
	}

	usleep(EXT_I2C_OP_DELAY);
	...
	if (rv) {
		CPRINTF("read cisco register failed!\n");
		return rv;
	}
...
int anx7406_m0_write(int port, int slave, int offset, int data)
{
	...
	if (rv) {
		CPRINTF("write data to cisco register failed!\n");
		return rv;
	}
...
int anx7406_set_aux(int port, int flip)
{
	...
	rv = anx7406_m1_write(port, I2C1_CISCO_SLAVE, I2C1_CISCO_CTRL_3,
			      flip ? AUX_FLIP_EN : 0);
	if (rv) {
		CPRINTF("Config CISCO_CTRL_3 register failed\n");
		return rv;
	}
	...
	if (rv) {
		CPRINTF("Config CISCO_CTRL_1 register failed\n");
		return rv;
	}
