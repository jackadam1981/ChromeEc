...
static int anx7406_init(int port)
{
	...
	if (rv) {
		ccprintf("C%d TCPC config VBUS OCP failed(%d)\n", port, rv);
		return rv;
	}

	/* Disable CAP write protect */
	rv = tcpc_update8(port, ANX7406_REG_TCPCCTRL,
			  ANX7406_REG_CAP_WP, MASK_CLR);
	if (rv) {
		ccprintf("C%d TCPC enable CAP WP failed\n", port);
		return rv;
	}
	...
	if (rv) {
		ccprintf("C%d TCPC config capability failed(%d)\n", port, rv);
		return rv;
	}
	/* Enable CAP write protect */
	rv = tcpc_update8(port, ANX7406_REG_TCPCCTRL,
			  ANX7406_REG_CAP_WP, MASK_SET);
	if (rv) {
		ccprintf("C%d TCPC enable CAP WP failed\n", port);
		return rv;
	}
