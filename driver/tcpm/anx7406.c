...
static struct anx7406_i2c_addr anx7406_i2c_slave[CONFIG_USB_PD_PORT_MAX_COUNT];
...
static int anx7406_set_hpd(int port, int hpd_lvl)
{
	...
	if (hpd_lvl) {
		ccprintf("set hpd to HIGH\n");
		val = ANX7406_REG_HPD_OEN | HPD_DEGLITCH_TIME;
	} else {
		ccprintf("set hpd to LOW\n");
		val = HPD_DEGLITCH_TIME;
	}

	return i2c_write8(tcpc_config[port].i2c_info.port,
			  anx7406_i2c_slave[port].top_addr_flags,
...
int anx7406_hpd_reset(const int port)
{
	...
	if (rv) {
		ccprintf("Clear HPD_MODE failed\n");
		return rv;
	}
...
static int anx7406_init(int port)
{
	...
	ccprintf("C%d TCPC anx7406 initial enter\n", port);
	/*
	...
	for (i = 0; i < ARRAY_SIZE(anx7406_i2c_addrs_flags); i++) {
		if (I2C_STRIP_FLAGS(tcpc_config[port].i2c_info.addr_flags) ==
			...
			anx7406_i2c_slave[port].tcpc_addr_flags =
				anx7406_i2c_addrs_flags[i].tcpc_addr_flags;
			anx7406_i2c_slave[port].top_addr_flags =
	...
	if (!I2C_STRIP_FLAGS(anx7406_i2c_slave[port].top_addr_flags)) {
		ccprintf("TCPC I2C addr 0x%x is invalid for ANX7406\n",
	...
	/* Set VBUS OCP */
	rv = tcpc_write(port, ANX7406_REG_VBUS_OCP, 0xFF);

	/* Disable bleed discharge */
	rv |= tcpc_update16(port, TCPC_REG_DEV_CAP_1,
			    TCPC_REG_DEV_CAP_1_BLEED_DISCHARGE, MASK_CLR);
	ccprintf("C%d TCPC config disable bleed discharge\n", port);
	if (rv) {
		ccprintf("C%d TCPC config capability failed\n", port);
		return rv;
	}
	...
	if (rv) {
		ccprintf("C%d TCPC clear UNINIT bit failed (%d)\n", port, rv);
		return rv;
	}

	rv = tcpci_tcpm_init(port);
	if (rv) {
		ccprintf("C%d TCPC tcpm init failed(%d)\n", port, rv);
		return rv;
	}

	/* Enable sink_ctrl & source_ctrl GPIO pin */
	...
	if (rv) {
		ccprintf("C%d TCPC clear CABLE_DET_DIG failed\n", port);
		return rv;
	}
	...
	if (rv) {
		ccprintf("C%d TCPC update POWER_CTRL failed\n", port);
		return rv;
	}
	...
	if (rv)
		ccprintf("C%d TCPC filter set failed\n", port);

	anx7406_hpd_reset(port);

	ccprintf("C%d TCPC anx7406 initial success\n", port);
	return rv;
}
...
static int anx7406_set_polarity(int port, enum tcpc_cc_polarity polarity)
{
	...
	if (rv)
		ccprintf("Update VCONN power failed\n");
...
static void anx7406_dump_registers(int port)
{
...
#ifdef CONFIG_CMD_TCPC_DUMP
	.dump_registers		= &anx7406_dump_registers,
#endif
};

#if defined(CONFIG_USB_PD_TCPM_MUX) && defined(CONFIG_USB_PD_MUX_ANX7443)
static int anx7406_m0_config(int port, int slave, int offset)
{
	int rv;
	...
	rv |= i2c_write8(tcpc_config[port].i2c_info.port,
			 anx7406_i2c_slave[port].top_addr_flags,
			 EXT_I2C0_OFFSET, offset);
	/* Configure external I2C transfer byte count */
	rv |= i2c_write8(tcpc_config[port].i2c_info.port,
			 anx7406_i2c_slave[port].top_addr_flags,
			 EXT_I2C0_ACCESS_DATA_BYTE_CNT, 1);
	/* Clear DATA buffer */
	rv |= i2c_write8(tcpc_config[port].i2c_info.port,
			 anx7406_i2c_slave[port].top_addr_flags,
			 EXT_I2C0_ACCESS_CTRL, I2C0_DATA_CLR);
	/* Clear release */
	rv |= i2c_write8(tcpc_config[port].i2c_info.port,
			 anx7406_i2c_slave[port].top_addr_flags,
			 EXT_I2C0_ACCESS_CTRL, 0);

	return rv;
}

static int anx7406_m0_read(int port, int slave, int offset)
{
	int rv, val;

	rv = anx7406_m0_config(port, slave, offset);

	/* Send I2C read command */
	rv |= i2c_write8(tcpc_config[port].i2c_info.port,
			 anx7406_i2c_slave[port].top_addr_flags,
			 EXT_I2C0_CTRL, I2C0_CMD_READ);
	if (rv) {
		ccprintf("initial cisco I2C master failed!\n");
		return rv;
	}
	...
	rv = i2c_read8(tcpc_config[port].i2c_info.port,
		       anx7406_i2c_slave[port].top_addr_flags,
		       EXT_I2C0_ACCESS_DATA, &val);
	if (rv) {
		ccprintf("read cisco register failed!\n");
		return rv;
	}
...
static int anx7406_m0_write(int port, int slave, int offset, int data)
{
	int rv;
	...
	rv |= i2c_write8(tcpc_config[port].i2c_info.port,
			 anx7406_i2c_slave[port].top_addr_flags,
			 EXT_I2C0_ACCESS_DATA, data);

	/* Send I2C write command */
	rv |= i2c_write8(tcpc_config[port].i2c_info.port,
			 anx7406_i2c_slave[port].top_addr_flags,
			 EXT_I2C0_CTRL, I2C0_CMD_WRITE);
	if (rv) {
		ccprintf("write data to cisco register failed!\n");
		return rv;
	}
...
static int anx7406_m1_read(int port, int slave, int offset)
{
	...
	rv |= i2c_write8(tcpc_config[port].i2c_info.port,
			 anx7406_i2c_slave[port].top_addr_flags,
			 EXT_I2C1_CTRL, I2C1_CMD_READ | I2C1_SPEED_100K);
	if (rv) {
		ccprintf("initial cisco I2C master failed!\n");
		return rv;
	}
...
static int anx7406_m1_write(int port, int slave, int offset, int data)
{
	...
	rv |= i2c_write8(tcpc_config[port].i2c_info.port,
			 anx7406_i2c_slave[port].top_addr_flags,
			 EXT_I2C1_CTRL, I2C1_CMD_WRITE | I2C1_SPEED_100K);
	if (rv) {
		ccprintf("write data to cisco register failed!\n");
		return rv;
	}
...
static int anx7406_set_aux(int port, int flip)
{
	int rv;

	ccprintf("Set SBU %s flip\n", flip ? "" : "not");
	/* Configure SBU orientation */
	//rv = anx7406_m1_read(port, 0x80, 0x06);
	//ccprintf("the default 0x80:0x06 = 0x%x\n", rv);
	rv = anx7406_m1_write(port, I2C1_CISCO_SLAVE, I2C1_CISCO_LOCAL_REG,
			      SELECT_SBU_1_2);
	if (rv) {
		ccprintf("Config CISCO_LOCAL_REG register failed\n");
		return rv;
	}
	...
	if (rv) {
		ccprintf("Config CISCO_CTRL_1 register failed\n");
		return rv;
	}
...
static int anx7406_set_retimer(int port, int flip, int mux_type)
{
	int rv, val;

	rv = anx7406_set_aux(port, flip);
	if (rv) {
	...
	rv = anx7406_m0_write(port, I2C0_TOP_SLAVE, INSERT_CR_PATTERN_0, 0xAA);
	if (rv) {
		ccprintf("Write INSERT_CR_PATTERN_0 register failed\n");
		return rv;
	}

	rv = anx7406_m0_write(port, I2C0_TOP_SLAVE, DP_RX_REG0,
	...
	rv = anx7406_m0_write(port, I2C0_TOP_SLAVE, DP_RX_REG3,
	...
	rv = anx7406_m0_write(port, I2C0_DP_SLAVE, POWER_DOWN, 0x00);
	if (rv) {
		ccprintf("Write POWER_DOWN register failed\n");
		return rv;
	}

	rv = anx7406_m0_write(port, I2C0_USB_SLAVE, FLIP_CTRL, USB_AUX_FLIP_EN);
	if (rv) {
	...
	rv = anx7406_m0_write(port, I2C0_TOP_SLAVE, CONFIG_MODE, val);
	if (rv) {
		ccprintf("Write CONFIG_MODE register failed\n");
		return rv;
	}
	rv = anx7406_m0_read(port, I2C0_TOP_SLAVE, CONFIG_MODE);
			   ...
			   bool *ack_required)
{
	...
	*ack_required = false;
	...
	return anx7406_set_retimer(port, cc_direction, mux_type);
}
...
const struct usb_mux_driver anx7443_usb_mux_driver = {
