...
#ifndef CONFIG_USB_PD_TCPM_SOFT_PORTS_COUNT
#define CONFIG_USB_PD_TCPM_SOFT_PORTS_COUNT CONFIG_USB_PD_PORT_MAX_COUNT
#endif

#ifndef CONFIG_USB_PD_TCPM_SOFT_PORTS_OFFSET
#define CONFIG_USB_PD_TCPM_SOFT_PORTS_OFFSET 0
#endif

#if CONFIG_USB_PD_TCPM_SOFT_PORTS_OFFSET > 0
...
} pd[CONFIG_USB_PD_TCPM_SOFT_PORTS_COUNT];
...
static int rx_buf_is_full(int port)
{
	...
	if (port >= CONFIG_USB_PD_TCPM_SOFT_PORTS_COUNT) {
		return 0;
	}

	diff = pd[port].rx_buf_tail - pd[port].rx_buf_head;
...
static void rx_buf_clear(int port)
{
	if (port >= CONFIG_USB_PD_TCPM_SOFT_PORTS_COUNT) {
		return;
	}
...
static void send_goodcrc(int port, int id)
{
	...
	header = PD_HEADER(PD_CTRL_GOOD_CRC, pd[port].power_role,
			pd[port].data_role, id, 0, 0, 0);
	bit_len = prepare_message(port, header, 0, NULL);
...
static int tcpc_alert_status(int port, int *alert)
{
...
static int tcpc_alert_status_clear(int port, uint16_t mask)
{
	/*
...
static int tcpc_alert_mask_set(int port, uint16_t mask)
{
...
static int soft_set_cc(int port, int pull)
{
...
static int soft_get_cc(int port, enum tcpc_cc_voltage_status *cc1,
...
static int soft_select_rp_value(int port, int rp)
{
...
static int soft_set_polarity(int port, enum tcpc_cc_polarity polarity)
{
...
static int soft_set_power_status_mask(int port)
{
	pd[port].power_status_mask = 0;
	return EC_SUCCESS;
}

static int soft_set_vconn(int port, int enable)
{
...
static int soft_set_rx_enable(int port, int enable)
{
	...
	for (i = 0; i < CONFIG_USB_PD_TCPM_SOFT_PORTS_COUNT; ++i)
		if (pd[i].rx_enabled)
			break;

	if (i == CONFIG_USB_PD_TCPM_SOFT_PORTS_COUNT)
...
static int soft_transmit(int port, 
...
static int soft_set_msg_header(int port, int power_role, int data_role)
{
	if (port >= CONFIG_USB_PD_TCPM_SOFT_PORTS_COUNT) {
		return EC_ERROR_INVAL;
	}
...
static int tcpc_get_message(int port, uint32_t *payload, int *head)
{
	int idx;
...
static void tcpc_pre_init(void)
{
	int i;
...
void tcpc_init(int port)
{
	...
	for (i = 0; i < 2; i++) {
		int adc_value = pd_adc_read(port, i);
...
#ifdef CONFIG_USB_PD_TCPC_TRACK_VBUS
static void pd_vbus_evt_p0(enum gpio_signal signal)
{
...
#if CONFIG_USB_PD_PORT_MAX_COUNT >= 2
static void pd_vbus_evt_p1(enum gpio_signal signal)
{
	if (CONFIG_USB_PD_TCPM_SOFT_PORTS_COUNT == 1)
		return;
...
static void tcpc_i2c_process(int read, int port, int len, uint8_t *payload,
...
static int command_tcpc(int argc, char **argv)
{
	...
	if (argc < 3)
		return EC_ERROR_PARAM_COUNT;
	if (*e || port >= CONFIG_USB_PD_TCPM_SOFT_PORTS_COUNT)
		return EC_ERROR_PARAM2;
...
static int soft_init(int port)
{
	...
	return soft_set_power_status_mask(port);
}

static int soft_release(int port)
{
	return EC_SUCCESS;
}
...
int soft_tcpm_dequeue_message(int port, uint32_t *payload, int *head)
{
	...
	ret = tcpc_get_message(port, payload, head);
...
{
	return soft_tcpm_dequeue_message(port, payload, head);
}
...
{
	return soft_tcpm_clear_pending_messages(port);
}

static void soft_alert(int port)
{
	int status;
...
static int soft_get_message_raw(int port, uint32_t *payload, int *head)
{
	return EC_ERROR_UNIMPLEMENTED;
}

static int soft_get_chip_info(int port, 
			      int live, 
...
const struct tcpm_drv soft_tcpm_drv = {
	.init			= &soft_init,
	.release		= &soft_release,
	.get_cc			= &soft_get_cc,
	.select_rp_value	= &soft_select_rp_value,
	.set_cc			= &soft_set_cc,
	.set_polarity		= &soft_set_polarity,
#ifdef CONFIG_USB_PD_DECODE_SOP
	.sop_prime_enable	= NULL,
#endif
	.set_vconn		= &soft_set_vconn,
	.set_msg_header		= &soft_set_msg_header,
	.set_rx_enable		= &soft_set_rx_enable,
	.get_message_raw	= &soft_get_message_raw,
	.transmit		= &soft_transmit,
	.tcpc_alert		= &soft_alert,
#ifdef CONFIG_USB_PD_DUAL_ROLE_AUTO_TOGGLE
	.drp_toggle		= NULL,
#endif
	.get_chip_info		= &soft_get_chip_info,
