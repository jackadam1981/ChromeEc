...
static void anx7447_vendor_defined_alert(int port)
{
	...
	anx7447_reg_read(port, ANX7447_REG_VD_ALERT, &alert);

	/* write to clear alerts */
	anx7447_reg_write(port, ANX7447_REG_VD_ALERT, alert);
...
static int anx7447_set_frs_enable(int port, int enable)
{
	...
	RETURN_ERROR(anx7447_reg_read(port, ANX7447_REG_FRSWAP_CTRL, &val));

	if (enable)
		val |= ANX7447_FRSWAP_DETECT_ENABLE;
	else
		val &= ~ANX7447_FRSWAP_DETECT_ENABLE;

	RETURN_ERROR(anx7447_reg_write(port, ANX7447_REG_FRSWAP_CTRL, val));

	RETURN_ERROR(tcpc_update8(port, ANX7447_REG_ADDR_GPIO_CTRL_1,
				  ANX7447_ADDR_GPIO_CTRL_1_FRS_EN_DATA,
				  enable ? 0xFF : 0));
	return EC_SUCCESS;
}
