...
/*
 * ANX7447 has two co-existence I2C peripheral addresses, TCPC address and
 * SPI address. The registers of TCPC peripheral address are partly compliant
 * with standard USB TCPC specification, and the registers in SPI peripheral
 * address controls the other functions (ex, hpd_level, mux_switch, and
 * so on). It can't use tcpc_read() and tcpc_write() to access SPI peripheral
 ...
 * ANX7447 SPI peripheral address.
 */
...
static int anx7447_init(int port)
{
	...
	/*
	 * find corresponding anx7447 SPI peripheral address according to
	 * specified TCPC peripheral address
	 */
