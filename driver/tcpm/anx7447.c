...
const uint8_t ANX7447_TCPM_VERSION = 0x04;
...
/*
 * ANX7447 has two co-existence I2C slave addresses, TCPC slave address 0x52
 * and SPI slave address 0x62. The registers of TCPC slave address are partly
 * compliant with standard USB TCPC specification, and the registers in SPI
 * slave address controls the other functions (ex, hpd_level, mux_switch, and
 ...
 * ANX7447 SPI slave address. And related slave address should be defined like
 * ANX7447_PORT1_I2C_ADDR (ANX7447_PORT2_I2C_ADDR, ANX7447_PORT3_I2C_ADDR, ...
 * for multiple port) in ./board/$board_name/board.h
 */
extern const int anx7447_i2c_addr[];

static inline int anx7447_reg_write(int port, int reg, int val)
{
	return i2c_write8(tcpc_config[port].i2c_host_port,
			  anx7447_i2c_addr[port],
			  reg, val);
}
...
static int anx7447_init(int port)
{
	...
	/* clear ALERT mask */
	tcpc_write(port, ANX7447_REG_INTR_ALERT_MASK_0, 0x00);
	tcpc_write(port, ANX7447_REG_INTP_CTRL_0, 0x80);
	...
	tcpc_write(port, ANX7447_REG_TCPC_CTRL_2, reg);

	rv = tcpc_read(port, ANX7447_REG_ADC_CTRL_1, &reg);
	if (rv)
		return rv;
	reg |= ANX7447_REG_ADCFSM_EN;
	tcpc_write(port, ANX7447_REG_ADC_CTRL_1, reg);
...
static int anx7447_mux_set(int port, mux_state_t mux_state)
{
	...
	tcpc_write(port, ANX7447_REG_TCPC_SWITCH_0, sw_sel);
	tcpc_write(port, ANX7447_REG_TCPC_SWITCH_1, sw_sel);
	tcpc_write(port, ANX7447_REG_TCPC_AUX_SWITCH, aux_sw);
