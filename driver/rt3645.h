...
#ifndef __CROS_EC_PMIC_RT3645_H
#define __CROS_EC_PMIC_RT3645_H
...
/*
 * CRC value changes, whenever any register value changes. Hence, update
 * it, based on new register value update.
 */
#define GOOD_CRC 0xAB
...
/*
 * Pages between 0 to 9 also exists, which are not exposed in datasheet
 * by vendor. Based on request, such registers may also need update.
 */
enum imvp_page {
	IMVP_PAGE_GLOBAL = 0x00,
	IMVP_PAGE_RAILA = 0x09,
	IMVP_PAGE_RAILB,
	IMVP_PAGE_RAILC,
	IMVP_PAGE_RAILD,
	IMVP_PAGE_RAIL_GEN,
...
/**
 * Run board specific code to set the board ready for imvp update. The
 * default implementation does nothing, but a board may override it.
 */
__override_proto void set_board_ready_before_imvp_update(void);

/**
 * Run board specific code to set the board after imvp update. The
 * default implementation does nothing, but a board may override it.
 */
__override_proto void set_board_ready_after_imvp_update(void);

#endif /* __CROS_EC_PMIC_RT3645_ */
