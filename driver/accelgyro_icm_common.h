...
static inline void icm_set_sensor_ts(const struct motion_sensor_t *s,
				     uint32_t delay)
{
	struct icm_drv_data_t *st = ICM_GET_DATA(s);
	uint32_t sensor_ts;

	sensor_ts = __hw_clock_source_read() + delay;
	/* prevent 0 value used for disabling time checking */
	st->sensor_ts[s->type] = sensor_ts | 1;
}

static inline void icm_reset_sensor_ts(const struct motion_sensor_t *s)
{
	struct icm_drv_data_t *st = ICM_GET_DATA(s);

	st->sensor_ts[s->type] = 0;
}
...
{
	struct icm_drv_data_t *st = ICM_GET_DATA(s);
	uint32_t sensor_ts = st->sensor_ts[s->type];

	if (sensor_ts == 0)
		return 0;

	return time_until(ts, sensor_ts);
}
