...
static inline int max695x_i2c_write_blk(uint8_t offset, uint8_t *data, int len)
{
	/*
...
int seven_seg_display_write(enum seven_seg_module_display module, int data)
{
	 ...
	 *
	 * Segment 0-1 : Module name
	 *	  0xEC : EC
	 *	  0x80 : PORT80
	 * Segment 2-3 : Data
	 * For console Command segment 0-3 : Data
	 */
	if (module == SEVEN_SEG_CONSOLE_DISPLAY) {
		/* Segment - 3 */
		buf[0] = (data >> 12) & 0x0F;
		/* Segment - 2 */
		buf[1] = (data >> 8) & 0x0F;
	...
	return max695x_i2c_write_blk(MAX695X_DIGIT0_ADDR, buf, ARRAY_SIZE(buf));
}
...
static void max695x_init(void)
{
	...
	max695x_i2c_write_blk(MAX695X_REG_DECODE_MODE, buf, ARRAY_SIZE(buf));
}
