...
__maybe_unused
static int _aw20198_read(struct rgbkbd *ctx, uint8_t addr, uint8_t *value)
{
	return i2c_xfer(ctx->cfg->i2c, AW20198_I2C_ADDR_FLAG,
			&addr, sizeof(addr), value, sizeof(*value));
}

static int _aw20198_write(struct rgbkbd *ctx, uint8_t addr, uint8_t value)
{
	uint8_t buf[2] = {
		[0] = addr,
		[1] = value,
...
static int _aw20198_set_page(struct rgbkbd *ctx, uint8_t page)
{
	return _aw20198_write(ctx, AW20198_REG_PAGE, page);
}

static int _aw20198_get_config(struct rgbkbd *ctx, uint8_t addr, uint8_t *value)
{
	int rv = _aw20198_set_page(ctx, AW20198_PAGE_FUNC);
	if (rv)
		return rv;

	return _aw20198_read(ctx, addr, value);
}

static int _aw20198_set_config(struct rgbkbd *ctx, uint8_t addr, uint8_t value)
{
	int rv = _aw20198_set_page(ctx, AW20198_PAGE_FUNC);
	if (rv)
		return rv;

	return _aw20198_write(ctx, addr, value);
}

static int aw20198_reset(struct rgbkbd *ctx)
{
	return _aw20198_set_config(ctx, AW20198_REG_RSTN, AW20198_RESET_MAGIC);
}
...
static int aw20198_enable(struct rgbkbd *ctx, bool enable)
{
	...
	rv = _aw20198_get_config(ctx, AW20198_REG_GCR, &cfg);
	if (rv)
		return rv;

	return _aw20198_write(ctx, AW20198_REG_GCR,
			      cfg | (enable ? BIT(0) : 0));
}
			     ...
			     struct rgb_s *color, uint8_t len)
{
	...
	rv = _aw20198_set_page(ctx, AW20198_PAGE_PWM);
	if (rv)
		return rv;
			     ...
			     uint8_t len)
{
	...
	rv = _aw20198_set_page(ctx, AW20198_PAGE_SCALE);
	if (rv)
		return rv;
...
{
	return _aw20198_set_config(ctx, AW20198_REG_GCC, level);
}
...
static int aw20198_init(struct rgbkbd *ctx)
{
	...
	rv = _aw20198_read(ctx, AW20198_REG_RSTN, &id);
	if (rv)
		return rv;
