...
#define AW20198_CMD_ID		0b101
#define AW20198_PAGE_FUNC	0
#define AW20198_PAGE_PWM	1
#define AW20198_PAGE_SCALE	2

#define AW20198_REG_GCR			0x00
#define AW20198_REG_GCC			0x01
#define AW20198_REG_PD_PU		0x02
#define AW20198_REG_SPREAD_SPECTRUM	0x25
		      ...
		      uint8_t len)
{
	...
	buf[0] = offset * RGB;
	memset(&buf[1], scale, frame_len - 1);
...
static int _init(struct rgbkbd *ctx)
{
	...
	rv |= _set_scale(ctx, 0x00, AW20198_BUF_SIZE, 0x7f);
	/* Enable chip (CHIPEN=1). */
	rv |= _set_config(ctx, AW20198_REG_GCR, 0xb0 | BIT(0));
