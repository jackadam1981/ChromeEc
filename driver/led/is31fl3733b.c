...
static int is31fl3733b_enable(struct rgbkbd *ctx, bool enable)
{
	...
	rv = is31fl3733b_get_config(ctx, IS31FL3733B_FUNC_CFG, &cfg);
	if (rv) {
		return rv;
	}
	/* TODO: check pwm frequency 4.2kHz(bit 3) */
	return is31fl3733b_write(ctx, IS31FL3733B_FUNC_CFG,
			      cfg | BIT(3) | (enable ? BIT(0) : 0));
}
