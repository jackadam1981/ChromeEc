...
static int tlc59116f_init(struct rgbkbd *ctx)
{
	...
	for (i = 0; i < 0x10; i++) {
		rv = tlc59116f_write(ctx, TLC59116F_PWM0 + i, 0x00);
		if (rv) {
			return rv;
		}
	}
	...
	for (j = 0; j < 4; j++) {
		rv = tlc59116f_write(ctx, TLC59116F_LEDOUT0 + j, 0xAA);
		if (rv) {
			return rv;
		}
	}
...
static int tlc59116f_enable(struct rgbkbd *ctx, bool enable)
{
	...
	return tlc59116f_write(ctx, TLC59116F_MODE1,
				 cfg & ~(enable ? BIT(4) : 0));
}
...
static int tlc59116f_set_gcc(struct rgbkbd *ctx, uint8_t level)
{
	...
	for (j = 0; j < 4; j++) {
		rv = tlc59116f_write(ctx, TLC59116F_LEDOUT0 + j, 0xFF);
		if (rv) {
			return rv;
		}
	}

	rv = tlc59116f_write(ctx, TLC59116F_GRPPWM, level);
	if (rv) {
		return rv;
	}
