...
static int hc_set_kb_light(struct host_cmd_handler_args *args)
{
	...
	if (p->percent > 0) {
		for (i = 1; i < ARRAY_SIZE(lm3509_brightness); i++) {
			 ...
			 */
			if (level_high < (p->percent)*10)
				continue;

			level_low = lm3509_brightness[i-1];
			if (level_high - (p->percent)*10 >
				(p->percent)*10 - level_low)
				val = i-1;
			else
				val = i;
			break;
...
/* convert LM3590 brightness */
#define LM3509_BRIGHTNESS_GET(brightness)  (((brightness)+5)/10)
...
static int hc_get_kb_light(struct host_cmd_handler_args *args)
{
	...
	r->percent = LM3509_BRIGHTNESS_GET(lm3509_brightness[val&0x1F]);
