 ...
 * Due to host command range is 0~100%, actually we only
 * need to report an integer value to host.
 * So we only need the first decimal place.
 */
const float lm3509_brightness[32] = {
	0, 0.1, 0.6, 1, 1.1, 1.3,
	1.6, 2, 2.4, 2.8, 3.1,
	3.7, 4.3, 5.2, 6.2, 7.5,
	8.7, 10, 12.5, 15, 16.8,
	18.7, 22.5, 26.2, 31.2, 37.5,
	43.7, 52.5, 61.2, 70, 87.5, 100};
...
static int hc_set_kb_light(struct host_cmd_handler_args *args)
{
	...
	if (p->percent > 0) {
		for (i = 1; i < ARRAY_SIZE(lm3509_brightness); i++) {
			/* The lm3509 brightness percentage has
			 * decimal separator, so enlarge the lm3509
			 * brightness percentage and percentage ten times
			 * for comparison.
			 */
			level_high = ENLARGE_DECUPLE(lm3509_brightness[i]);
			...
			level_low = ENLARGE_DECUPLE(lm3509_brightness[i-1]);
