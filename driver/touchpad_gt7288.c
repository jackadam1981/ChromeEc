...
static int gt7288_stationary_read(uint8_t *data, size_t max_length)
{
	return i2c_xfer(CONFIG_TOUCHPAD_I2C_PORT, GT7288_SLAVE_ADDRESS,
...
int gt7288_read_ptp_report(struct gt7288_ptp_report *report)
{
	...
	RETURN_ERROR(gt7288_stationary_read(data, sizeof(data)));
	...
	for (i = 1; i < report->num_contacts; i++) {
		RETURN_ERROR(i2c_xfer(CONFIG_TOUCHPAD_I2C_PORT,
