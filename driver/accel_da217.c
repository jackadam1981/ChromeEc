...
static int set_range(struct motion_sensor_t *s, int range, int rnd)
{
	...
	ret = raw_read8(s->port, s->i2c_spi_addr_flags,
			DA217_RESOLUTION_RANGE, &range_reg_val);
	if (ret != EC_SUCCESS) {
		mutex_unlock(s->mutex);
		return ret;
	}
	reg_val = (range_reg_val & ~DA217_RANGE_MSK) | range_val;
	ret = raw_write8(s->port, s->i2c_spi_addr_flags,
			 DA217_RESOLUTION_RANGE, reg_val);
...
static int set_data_rate(const struct motion_sensor_t *s, int rate, int rnd)
{
	...
	if (rate == 0) {
		mutex_lock(s->mutex);
		/* power off. */
		reg_val = DA217_POWER_OFF;
		ret = raw_write8(s->port, s->i2c_spi_addr_flags,
				 DA217_MODE_BW, reg_val);
		mutex_unlock(s->mutex);
		return ret;
	}

	odr_val = DA217_ODR_TO_REG(rate);
	if ((DA217_REG_TO_BW(odr_val) < rate) && rnd)
		odr_val = DA217_ODR_TO_REG(rate * 2);

	mutex_lock(s->mutex);

	/* Determine the new value of control reg and attempt to write it. */
	ret = raw_read8(s->port, s->i2c_spi_addr_flags,
			DA217_ODR_AXIS, &odr_reg_val);
	if (ret != EC_SUCCESS) {
		mutex_unlock(s->mutex);
		return ret;
	}
	reg_val = (odr_reg_val & ~DA217_ODR_MSK) | odr_val;
	/* Set output data rate. */
	ret = raw_write8(s->port, s->i2c_spi_addr_flags,
			 DA217_ODR_AXIS, reg_val);

	/* If successfully written, then save the new data rate. */
	if (ret == EC_SUCCESS)
		data->odr = DA217_REG_TO_BW(odr_val);

	/* power on. */
	reg_val = DA217_POWER_ON;
	ret = raw_write8(s->port, s->i2c_spi_addr_flags,
			 DA217_MODE_BW, reg_val);
