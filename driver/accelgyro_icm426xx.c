...
static int normalize(const struct motion_sensor_t *s, intv3_t v,
		     const uint8_t *raw)
{
...
static int __maybe_unused load_fifo(struct motion_sensor_t *s, uint32_t ts)
{
	...
	for (i = 0; i < count; i += size) {
		...
		if (size <= 0)
			return -size;
		if (accel != NULL)
			push_fifo_data(st->accel, (const uint8_t *)accel, ts);
		if (gyro != NULL)
			push_fifo_data(st->gyro, (const uint8_t *)gyro, ts);
	}
...
static int calibrate_offset(const struct motion_sensor_t *s, intv3_t target,
			    int rate, int16_t *offset)
{
	intv3_t v, mean;
	int64_t acc[3] = {0, 0, 0};
	const unsigned int nb = rate / 1000;	/* number of samples for 1s */
	int i, ret;
	...
	for (i = X; i <= Z; ++i) {
		mean[i] = round_divide(acc[i], nb);
		offset[i] = target[i] - mean[i];
	}

	return enable_sensor(s, 0);
...
{
	const int odr = ICM426XX_ODR_100HZ;
	const int rate = 100000;
	int save_rate, save_range;
	int reg_conf, range;
	intv3_t target;
	int16_t temp = EC_MOTION_SENSE_INVALID_CALIB_TEMP;
	int16_t offset[3];
	int ret;
	...
	/* calibration use case is device still on a flat surface */
	switch (s->type) {
	case MOTIONSENSE_TYPE_ACCEL:
		reg_conf = ICM426XX_REG_ACCEL_CONFIG0;
		range = 16;
		/* +1g on Z, 0 on X and Y */
		target[X] = target[Y] = 0;
		target[Z] = 32768 / range;
		break;
	case MOTIONSENSE_TYPE_GYRO:
		reg_conf = ICM426XX_REG_GYRO_CONFIG0;
		range = 1000;
		/* 0 for all axes */
	...
	save_rate = icm_get_data_rate(s);
	save_range = icm_get_range(s);

	/* force sensor off */
	ret = set_data_rate(s, 0, 0);
	if (ret != EC_SUCCESS)
		return ret;

	ret = set_range(s, range, 0);
	if (ret != EC_SUCCESS)
		goto error_restore_rate;

	mutex_lock(s->mutex);
	ret = icm_field_update8(s, reg_conf, ICM426XX_ODR_MASK,
				ICM426XX_ODR(odr));
	mutex_unlock(s->mutex);
	if (ret != EC_SUCCESS)
		goto error_restore;

	/* compute the calibration offsets */
	ret = calibrate_offset(s, target, rate, offset);
	if (ret != EC_SUCCESS)
		goto error_restore;
	...
	/* restore range */
	ret = set_range(s, save_range, 0);
	if (ret != EC_SUCCESS)
		goto error_restore_rate;

	/* restore rate */
	return set_data_rate(s, save_rate, 0);

error_restore:
	set_range(s, save_range, 0);
error_restore_rate:
	set_data_rate(s, save_rate, 0);
	return ret;
}
