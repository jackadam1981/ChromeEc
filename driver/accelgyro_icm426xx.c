...
STATIC_IF(CONFIG_ACCELGYRO_ICM426XX_INT_ENABLE)
...
#ifdef CONFIG_ACCELGYRO_ICM426XX_INT_ENABLE
...
#endif	/* CONFIG_ACCELGYRO_ICM426XX_INT_ENABLE */
...
static int icm426xx_init(struct motion_sensor_t *s)
{
	...
	if (s->type == MOTIONSENSE_TYPE_ACCEL) {
		...
		if (IS_ENABLED(CONFIG_ACCELGYRO_ICM426XX_INT_ENABLE))
