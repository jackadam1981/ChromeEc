					    ...
					    uint32_t ts)
{
	...
	/* rem < 0: reset check since ts has passed sensor_ts */
	icm_reset_sensor_ts(s);
	return EC_SUCCESS;
}
...
static int icm426xx_enable_sensor(const struct motion_sensor_t *s, int enable)
{
	...
	if (enable) {
		...
		rem = icm_get_sensor_stabilized(s, now);
...
static int icm426xx_read(const struct motion_sensor_t *s, intv3_t v)
{
	...
	now = __hw_clock_source_read();
	ret = icm426xx_check_sensor_stabilized(s, now);
	if (ret == EC_SUCCESS)
