...
static int lis2ds_load_fifo(struct motion_sensor_t *s)
{
	...
	do {
		...
		if (tmp & LIS2DS_FIFO_DIFF8_MASK) {
			...
			/* Unread FIFO samples */
			ret = raw_read8(s->port, s->addr, LIS2DS_FIFO_SAMPLES_ADDR, &tmp);
			if (ret != EC_SUCCESS)
				return ret;
...
static int set_range(const struct motion_sensor_t *s, int range, int rnd)
{
	...
	if (rnd && (newrange < LIS2DS_NORMALIZE_FS(newrange)))
		newrange <<= 1;
	...
	if (err == EC_SUCCESS)
		...
		data->base.range = LIS2DS_FS_GAIN(newrange);
...
static int get_range(const struct motion_sensor_t *s)
{
	...
	return LIS2DS_GAIN_FS(data->base.range);
}
