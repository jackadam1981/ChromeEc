...
static int opt3001_i2c_read(const int reg, int *data_ptr)
{
	...
	*data_ptr = ((*data_ptr << 8) & 0xFF00) | ((*data_ptr >> 8) & 0x00FF);
...
static void opt3001_init(void)
{
	...
	if (ret != EC_SUCCESS || data != OPT3001_MANUFACTURER_ID) {
		CPRINTF("ALS init failed: ret=%d, data=0x%x\n", ret, data);
		return;
	}

	ret = opt3001_i2c_read(OPT3001_REG_DEV_ID, &data);
	if (ret != EC_SUCCESS || data != OPT3001_DEVICE_ID) {
		CPRINTF("ALS init failed: ret=%d, data=0x%x\n", ret, data);
		return;
	}
...
int opt3001_read_lux(int *lux, int af)
{
	...
	/*
	 * lux = 0.01 × (2EXP[3:0]) × R[11:0]
	 */
	if (exponent == 0)
		*lux = (data & 0x0FFF) * af / 100;
	else
		*lux = (2 << (exponent - 1)) * (data & 0x0FFF) * af / 100;
