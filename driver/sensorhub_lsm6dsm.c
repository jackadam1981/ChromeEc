...
static int config_slv0_read__8b(const struct motion_sensor_t *s,
			    const uint16_t addr__8b, uint8_t reg, uint8_t len)
{
	int ret;
			       ...
			       uint16_t slv_addr__7bf, uint8_t reg, int len)
{
	...
	ret = config_slv0_read__8b(s, I2C_ADDR__7b(slv_addr__7bf) << 1,
			       reg, len);
