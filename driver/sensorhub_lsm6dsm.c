				...
				uint8_t slv_addr, uint8_t reg, uint8_t val)
{
	...
	ret = st_raw_write8__7bf(s->port, slv_addr, reg, val);
					...
					uint8_t rst_reg, uint8_t rst_val)
{
	...
	ret = st_raw_read8__7bf(s->port, slv_addr, whoami_reg, &tmp);
	if (ret != EC_SUCCESS) {
	...
	ret = st_raw_write8__7bf(s->port, slv_addr, rst_reg, rst_val);
