...
/** LSM6DS0 accelerometer and gyro module for Chrome EC
   ...
   ---------------
       power_down
   --> Config CTRL_REG6_XL (20h) && Write CTRL_REG1_G (10h) = PD
   --> Accelerometer Only

       power_down
   --> Config CTRL_REG1_G (10h) && Write CTRL_REG6_XL (20h) = PD
...
accel_cleanup:
	...
	CPRINTF("lsm6ds0: Done Set Range:%d\n", range);
	return EC_SUCCESS;
}
				...
				const int rnd)
{
	...
	ctrl_reg = get_ctrl_reg(s->type);
	data_rates = get_odr_table(s->type);
...
accel_cleanup:
	mutex_unlock(s->mutex);
	CPRINTF("lsm6ds0: Done Set ODR:%d\n", rate);
	return EC_SUCCESS;
}
...
static int init(struct motion_sensor_t *s)
{
	...
	CPRINTF("lsm6ds0: Done Init type:0x%X\n", s->type);
	return ret;
}
