				...
				int rnd)
{
	...
	ret = raw_read8__8b(s->port, s->addr__8b, ctrl_reg, &ctrl_val);
	if (ret != EC_SUCCESS)
		goto accel_cleanup;

	ctrl_val = (ctrl_val & ~LSM6DS0_RANGE_MASK) | reg_val;
	ret = raw_write8__8b(s->port, s->addr__8b, ctrl_reg, ctrl_val);
				...
				int rnd)
{
	...
	if (MOTIONSENSE_TYPE_GYRO == s->type) {
		ret = raw_read8__8b(s->port, s->addr__8b, LSM6DS0_CTRL_REG3_G, &val);
		if (ret != EC_SUCCESS)
			goto accel_cleanup;
		...
		ret = raw_write8__8b(s->port, s->addr__8b, LSM6DS0_CTRL_REG3_G, val);
	}
...
static int is_data_ready(const struct motion_sensor_t *s, int *ready)
{
	...
	ret = raw_read8__8b(s->port, s->addr__8b, LSM6DS0_STATUS_REG, &tmp);
...
static int read(const struct motion_sensor_t *s, intv3_t v)
{
	...
	ret = i2c_read_block__7b(s->port, s->addr__8b >> 1, xyz_reg, raw, 6);
...
static int init(const struct motion_sensor_t *s)
{
	...
	ret = raw_read8__8b(s->port, s->addr__8b, LSM6DS0_WHO_AM_I_REG, &tmp);
	if (ret)
	...
	if (MOTIONSENSE_TYPE_ACCEL == s->type) {
		...
		mutex_lock(s->mutex);
		ret = raw_read8__8b(s->port, s->addr__8b, LSM6DS0_CTRL_REG8, &tmp);
		if (ret) {
			mutex_unlock(s->mutex);
			return EC_ERROR_UNKNOWN;
		}
		tmp |= (1 | LSM6DS0_BDU_ENABLE);
		ret = raw_write8__8b(s->port, s->addr__8b, LSM6DS0_CTRL_REG8, tmp);
		mutex_unlock(s->mutex);
