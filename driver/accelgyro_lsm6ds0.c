...
static inline float get_sensitivity(enum sensor_type_t type)
{
	return (SENSOR_ACCELEROMETER == type) ?
		SENSITIVITY_ACC : SENSITIVITY_GYRO;
}
			...
			int *z)
{
	...
	if (ret != EC_SUCCESS)
		return ret;
	if (LSM6DS0_STS_DOWN == (tmp & LSM6DS0_STS_MASK)) {
		*x = 0;
		*y = 0;
		*z = 0;
		return 0;
...
static int init(const struct motion_sensor_t *s)
{
	...
	if (SENSOR_ACCELEROMETER == s->type) {
		ret = raw_write8(s->i2c_addr, LSM6DS0_CTRL_REG8,
			1 | LSM6DS0_BDU_ENABLE);
