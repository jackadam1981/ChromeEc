...
#define CPS8X00_FW_VER_MASK 0xff
...
static uint8_t cps8200_send_cmd(struct pchg *ctx, uint32_t cmd, uint8_t *id)
{
	...
	*id = ctx->update.u8.cmd_id;
	cmd = cmd | *id;
	ctx->update.u8.cmd_id++;
	ctx->update.u8.cmd_id &= 0x0f;

	rv = cps8200_write32(port, CPS8200_ADDR_CMD, cmd);
	return rv;
}
...
static int cps8x00_read_firmware_ver(struct pchg *ctx)
{
	...
	if (!cps8x00_read32(port, CPS8100_REG_FW_INFO, &u32)) {
		ctx->fw_version = u32 & CPS8X00_FW_VER_MASK;
...
static int cps8200_update_open(struct pchg *ctx)
{
	...
	ctx->update.u8.cmd_id = 0;
	ctx->update.u16.crc = 0;
	ctx->update.u32.firmware_len = 0;
	...
	if (ctx->update.u16.crc != (u32 & 0x0000ffff)) {
		CPRINTS("crc = %04x, expect %04x", u32, ctx->update.u16.crc);
		CPRINTS("CRC of bootloader is wroing!");
		return EC_ERROR_UNKNOWN;
	}
	CPRINTS("Successfully load bootloader!");

	ctx->update.u16.crc = 0x0000;
...
static int cps8200_update_write(struct pchg *ctx)
{
	...
	ctx->update.u32.firmware_len += ctx->update.size;
	ctx->update.u16.crc = cros_crc16_arg(ctx->update.data, ctx->update.size,
					     ctx->update.u16.crc);
...
static int cps8200_update_close(struct pchg *ctx)
{
	...
	uint32_t u32;
	uint8_t id;
	uint32_t len = ctx->update.u32.firmware_len;
