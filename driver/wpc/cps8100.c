...
#define CPS8200_ADDR_CMD 0x200009FC
...
static int cps8200_write_mem(int port, uint32_t addr, uint8_t *data, size_t len)
{
	...
	i2c_lock(port, 1);
	/* Addr */
	rv = i2c_xfer_unlocked(port, CPS8601_I2C_ADDR, buf, sizeof(buf), NULL,
			       0, I2C_XFER_START);
	if (rv) {
		i2c_lock(port, 0);
		return rv;
	...
	/* Data */
	rv = i2c_xfer_unlocked(port, CPS8601_I2C_ADDR, data, len, NULL, 0,
			       I2C_XFER_STOP);
	i2c_lock(port, 0);
...
static int cps8601_update_write(struct pchg *ctx)
{
	...
	if (cps8200_write_mem(port, CPS8601_ADDR_BUFFER0, buf,
			      ctx->update.size))
		return EC_ERROR_UNKNOWN;
	...
	if (rv)
		return rv;
	/* write bootloader length */
	// rv = cps8601_write32(port, CPS8601_ADDR_BUFFER0, cps8601_boot_hex_len
	// * 4);
...
static int cps8601_wls_program_firmware(int port, uint8_t *data, size_t len)
{
	...
	rv = i2c_xfer_unlocked(port, CPS8601_I2C_ADDR, data,
