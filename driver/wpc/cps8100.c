...
static uint8_t cps8200_send_cmd(struct pchg *ctx, uint32_t cmd, uint8_t *id)
{
	...
	*id = ctx->update.driver_data.cps8200_update.cmd_id;
	cmd |= *id;
	ctx->update.driver_data.cps8200_update.cmd_id++;
	ctx->update.driver_data.cps8200_update.cmd_id &= 0x0f;
...
static int cps8200_update_open(struct pchg *ctx)
{
	...
	ctx->update.driver_data.cps8200_update.cmd_id = 0;
	ctx->update.driver_data.cps8200_update.crc = 0;
	ctx->update.driver_data.cps8200_update.firmware_len = 0;
	...
	if (rv)
		return rv;
	ctx->update.driver_data.cps8200_update.crc =
		cros_crc16_arg((uint8_t *)boot_hex, boot_hex_len * 4, 0);
	if (ctx->update.driver_data.cps8200_update.crc != (u32 & 0x0000ffff)) {
		CPRINTS("crc = %04x, expect %04x", u32,
			ctx->update.driver_data.cps8200_update.crc);
	...
	ctx->update.driver_data.cps8200_update.crc = 0x0000;
...
static int cps8200_update_write(struct pchg *ctx)
{
	...
	ctx->update.driver_data.cps8200_update.firmware_len += ctx->update.size;
	ctx->update.driver_data.cps8200_update.crc =
		cros_crc16_arg(ctx->update.data, ctx->update.size,
			       ctx->update.driver_data.cps8200_update.crc);
...
static int cps8200_update_close(struct pchg *ctx)
{
	...
	uint32_t u32;
	uint8_t id;
	uint32_t len = ctx->update.driver_data.cps8200_update.firmware_len;
