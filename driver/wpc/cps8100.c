...
struct cps8100_state_t {
	struct mutex mtx;
	uint32_t reg;
};

/* Note: Assume there is only one port. */
static struct cps8100_state_t cps8100_state;
...
static const char *cps8100_func_names[] = {
	[0] = "DPL",
	[1] = "OPP",
	[2] = "OTP",
	[3] = "OVPK",
...
static const char *cps8100_irq_type_names[] = {
...
static const char *cps8100_profile_names[] = {
	[0] = "NONE",
	[1] = "BPP",
	[2] = "EPP",
	[3] = "PRIVATE",
...
__maybe_unused
static int cps8100_read8(int port, uint32_t reg, uint8_t *val)
{
	uint8_t buf[CPS8100_MESSAGE_BUFFER_SIZE];

	if (cps8100_set_password(port) ||
	    cps8100_set_write_mode(port, CPS8100_ACCESS_MODE_8) ||
...
__maybe_unused
static int cps8100_read16(int port, uint32_t reg, uint16_t *val)
{
	uint8_t buf[CPS8100_MESSAGE_BUFFER_SIZE];

	if (cps8100_set_password(port) ||
	    cps8100_set_write_mode(port, CPS8100_ACCESS_MODE_16) ||
	    cps8100_set_high_address(port, reg))
		return EC_ERROR_UNKNOWN;
...
__maybe_unused
static int cps8100_write8(int port, uint32_t reg, uint8_t val)
{
	uint8_t buf[CPS8100_MESSAGE_BUFFER_SIZE];
	...
	/* Set low 16 bits of register address and write a byte. */
	buf[0] = (reg >> 8) & 0xff;
	buf[1] = (reg >> 0) & 0xff;
	buf[2] = val;

	return i2c_xfer(port, CPS8100_I2C_ADDR_L, buf, 3, NULL, 0);
}

__maybe_unused
static int cps8100_write32(int port, uint32_t reg, uint32_t val)
{
	uint8_t buf[CPS8100_MESSAGE_BUFFER_SIZE];
	...
	/* Set low 16 bits of register address and write a word. */
	buf[0] = (reg >> 8) & 0xff;
	buf[1] = (reg >> 0) & 0xff;
	buf[2] = (val >> 24) & 0xff;
	buf[3] = (val >> 16) & 0xff;
	...
	return i2c_xfer(port, CPS8100_I2C_ADDR_L, buf, 6, NULL, 0);
}
