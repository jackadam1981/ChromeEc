...
static const char *const cps8100_func_names[] = {
    [0] = "DPL",  [1] = "OPP",     [2] = "OTP",  [3] = "OVPK",    [4] = "OCP",
    [5] = "UVP",  [6] = "OVP",     [7] = "FOD",  [8] = "SAMSUNG", [9] = "APPLE",
    [10] = "EPP", [11] = "HUAWEI", [12] = "CPS",
};
...
static void cps8100_print_func_names(const char *preamble, uint32_t reg) {
  ...
  if (reg) CPRINTF("UNKNOWN(0x%x)", reg);

  CPUTS("\n");
}

static void cps8100_print_status_flag_names(const char *preamble,
                                            uint32_t reg) {
  CPRINTFP("%s", preamble);

  if (reg & CPS8100_STATUS_FOD) CPRINTF("FOD,");
  if (reg & CPS8100_STATUS_OCP) CPRINTF("OCP,");
  if (reg & CPS8100_STATUS_OVP) CPRINTF("OVP,");
  if (reg & CPS8100_STATUS_OTP) CPRINTF("OTP,");
  if (reg & CPS8100_STATUS_UVP) CPRINTF("UVP,");
...
static int cps8100_i2c_write(int port, int addr, const uint8_t *buf,
                             size_t len) {
  int rv;
  ...
  if (rv) CPRINTS("Failed to write: %d", rv);
...
static int cps8100_set_unlock(int port) {
  ...
  return cps8100_i2c_write(port, CPS8100_I2C_ADDR_H, cps8100_unlock_cmd, 4);
}
...
static int cps8601_i2c_enable(int port) {
  ...
  return cps8100_i2c_write(port, CPS8601_I2C_ADDR, cps8601_i2c_enable_cmd, 8);
}
...
static int cps8200_i2c_enable(int port) {
  ...
  return cps8100_i2c_write(port, CPS8200_I2C_ADDR, cps8200_i2c_enable_cmd, 8);
}
...
static int cps8100_read32(int port, uint32_t reg, uint32_t *val) {
  ...
  if (cps8100_set_high_address(port, reg)) return EC_ERROR_UNKNOWN;

  /* Set low 16 bits of register address and read a byte. */
  buf[0] = (reg >> 8) & 0xff;
  buf[1] = (reg >> 0) & 0xff;

  return i2c_xfer(port, CPS8100_I2C_ADDR_L, buf, 2, (void *)val, sizeof(*val));
}
...
static int cps8200_write_mem(int port, uint32_t addr, uint8_t *data,
                             size_t len) {
  int rv;
  uint8_t buf[4];
  ...
  i2c_lock(port, 1);
  /* Addr */
  rv = i2c_xfer_unlocked(port, CPS8200_I2C_ADDR, buf, sizeof(buf), NULL, 0,
                         I2C_XFER_START);
  if (rv) {
    i2c_lock(port, 0);
    return rv;
...
static int cps8601_write_mem(int port, uint32_t addr, uint8_t *data,
                             size_t len) {
  int rv;
  size_t offset = 0;
  ...
  for (size_t i = 0; i < segments; i++) {
    ...
    rv = i2c_xfer_unlocked(port, CPS8601_I2C_ADDR, buf, sizeof(buf), NULL, 0,
                           I2C_XFER_START);
    if (rv) {
      i2c_lock(port, 0);
    ...
    rv = i2c_xfer_unlocked(port, CPS8601_I2C_ADDR, data + offset, block_size,
  ...
  if (remaining) {
    ...
    rv = i2c_xfer_unlocked(port, CPS8601_I2C_ADDR, data + offset, remaining,
                           NULL, 0, I2C_XFER_STOP);
    i2c_lock(port, 0);

    if (rv) {
      CPRINTS("Failed to write remaining data at 0x%08x", addr);
      return rv;
    }
    crec_msleep(short_sleep_ms);
  }
...
static int cps8200_wait_cmd_done(int port, uint8_t id) {
  ...
  while (1) {
    ...
    if (rv) return EC_ERROR_UNKNOWN;

    if ((CMD_STATUS_PASS | id) == (u32 & 0x00ff)) break;
...
static int cps8601_wait_cmd_done(int port) {
  ...
  while (1) {
    ...
    if ((CMD_STATUS_PASS) == (u32 & 0x00ff)) break;
    if (CPS8200_CMD_STATUS(u32) == CMD_STATUS_FAIL ||
        CPS8200_CMD_STATUS(u32) == CMD_STATUS_ILLEGAL) {
      CPRINTS("Command failed or illegal: %02lx", CPS8200_CMD_STATUS(u32));
      return EC_ERROR_UNKNOWN;
    }
...
static int cps8x00_enable(struct pchg *ctx, bool enable) { return EC_SUCCESS; }

static int cps8100_get_alert_info(struct pchg *ctx, uint32_t *reg) {
  int rv;

  if (chip_id == CPS8601_CHIPID)
    rv = cps8x00_read32(ctx->cfg->i2c_port, CPS8601_REG_ALERT_INFO, reg);
  else
...
static int cps8x00_get_chip_info(struct pchg *ctx) {
  ...
  if (!rv) CPRINTS("FW=0x%02x", ctx->fw_version);
...
static int cps8x00_get_event(struct pchg *ctx) {
  ...
  if (IS_ENABLED(CPS8100_DEBUG)) cps8100_print_alert_info(r2);
...
static int cps8200_update_open(struct pchg *ctx) {
  ...
  rv = cps8200_write32(port, CPS8200_ADDR_BUF_SIZE, ctx->cfg->block_size / 4);
  if (rv) {
...
static int cps8200_update_write(struct pchg *ctx) {
  ...
  if (cps8200_write_mem(port, CPS8200_ADDR_BUFFER0, buf, ctx->update.size))
    return EC_ERROR_UNKNOWN;
  crec_msleep(short_sleep_ms);
...
static int cps8601_update_open(struct pchg *ctx) {
  ...
  /* Program bootloader to SRAM */
  CPRINTS("Loading bootloader hex!");
  if (cps8601_write_mem(port, CPS8200_ADDR_SRAM, (uint8_t *)&cps8601_boot_hex,
  ...
  /* write bootloader length */
  rv = cps8601_write32(port, CPS8601_ADDR_BUFFER0, cps8601_boot_hex_len * 4);
  if (rv) return rv;
  crec_msleep(short_sleep_ms);
  ...
  if (rv) return rv;
  upd->crc =
      cros_crc16((uint8_t *)cps8601_boot_hex, cps8601_boot_hex_len * 4, 0);
  if (upd->crc != (u32 & 0x0000ffff)) {
    CPRINTS("crc = %04x, expect %04x", u32, upd->crc);
    CPRINTS("CRC of bootloader is wroing!");
    return EC_ERROR_UNKNOWN;
...
static int cps8601_update_write(struct pchg *ctx) {
  ...
  /* Write data to buffer */
  if (cps8601_write_mem(port, CPS8601_ADDR_BUFFER0, buf, ctx->update.size))
    return EC_ERROR_UNKNOWN;
...
static void cps8100_dump(struct pchg *ctx) {
  ...
  if (chip_id == CPS8601_CHIPID)
    ...
    rv = cps8x00_read32(ctx->cfg->i2c_port, CPS8601_REG_FUNC_EN, &val);
  if (rv == EC_SUCCESS) cps8100_print_func_names("FEATURES: ", val);

  rv = cps8100_get_alert_info(ctx, &val);
  if (rv == EC_SUCCESS) cps8100_print_alert_info(val);
}
...
static int cc_cps8100(int argc, const char **argv) {
  ...
  if (argc < 2 || 3 < argc) return EC_ERROR_PARAM_COUNT;
