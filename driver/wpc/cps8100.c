...
static int cps8100_init(struct pchg *ctx)
{
	...
	if (chip_id == CPS8100_CHIPID) {
		...
		if (!cps8200_set_unlock(port) && !cps8200_set_unlock(port))
			return EC_SUCCESS;
		else
			return EC_ERROR_UNKNOWN;
	} else {
...
static int cps8100_get_chip_info(struct pchg *ctx)
{
	...
	else if (!cps8200_i2c_enable(port) && !cps8200_set_unlock(port))
	...
	/* Skip probing if chip_id is already set */
	if (chip_id == CPS8100_CHIPID || chip_id == CPS8200_CHIPID)
		return EC_SUCCESS;
