...
#define CMD_PGM_BUFFER0 BIT(4)
#define CMD_PGM_BUFFER1 BIT(5)
#define CMD_PGM_WR_FLAG BIT(7)
#define CMD_CACL_CRC_APP (BIT(4) | BIT(7))
#define CMD_CACL_CRC_BOOT (BIT(4) | BIT(5) | BIT(7))
#define CMD_STATUS_RUNNING BIT(4)
#define CMD_STATUS_PASS BIT(5)
#define CMD_STATUS_FAIL (BIT(4) | BIT(5))
#define CMD_STATUS_ILLEGAL BIT(6)
#define CFG_BUFF_SIZE 128
#define CMD_TIMEOUT (3 * SECOND)
...
static int cps8200_wait_cmd_done(int port, uint8_t id)
{
	...
	deadline.val = get_time().val + CMD_TIMEOUT;
	while (1) {
		msleep(10);
