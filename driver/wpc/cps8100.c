...
static int cps8601_write_mem(int port, uint32_t addr, uint8_t *data, size_t len)
{
	...
	for (size_t i = 0; i < segments; i++) {
		...
		rv = i2c_xfer_unlocked(port, CPS8601_I2C_ADDR, data + offset, block_size, NULL, 0,
			I2C_XFER_STOP);
		i2c_lock(port, 0);
	...
	if (remaining) {
		...
		rv = i2c_xfer_unlocked(port, CPS8601_I2C_ADDR, data + offset, remaining, NULL, 0,
			I2C_XFER_STOP);
		i2c_lock(port, 0);
...
static int cps8601_update_open(struct pchg *ctx)
{
	...
	if (rv)
		return rv;
	upd->crc = cros_crc16((uint8_t *)cps8601_boot_hex, cps8601_boot_hex_len * 4, 0);
	if (upd->crc != (u32 & 0x0000ffff)) {
		CPRINTS("crc = %04x, expect %04x", u32, upd->crc);
		CPRINTS("CRC of bootloader is wroing!");
		return EC_ERROR_UNKNOWN;
