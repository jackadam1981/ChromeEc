...
static struct wpc_charger_info *wpc_get_charger_info(void)
{
...
static int p9221_is_online(struct wpc_charger_info *wpc)
{
	...
	if (p9221_read16(wpc, P9221_CHIP_ID_REG, &chip_id)
...
{
	return p9221_is_online(&p9221_charger_info);
}
...
static int p9221r7_clear_interrupts(struct wpc_charger_info *wpc, uint16_t mask)
{
	int ret;

	ret = p9221_write16(wpc, P9221R7_INT_CLEAR_REG, mask);
	if (ret) {
 ...
 */
static int p9221r7_enable_interrupts(struct wpc_charger_info *wpc)
{
	uint16_t mask = 0;
	int ret;
	...
	ret = p9221r7_clear_interrupts(wpc, mask);
	if (ret)
...
static int p9221_send_csp(struct wpc_charger_info *wpc, uint8_t stat)
{
	...
	if (!p9221_write8(wpc, P9221R7_CHARGE_STAT_REG, stat))
		ret = p9221_set_cmd_reg(wpc, P9221R7_COM_SENDCSP);
...
static int p9221_send_eop(struct wpc_charger_info *wpc, uint8_t reason)
{
	...
	if (!p9221_write8(wpc, P9221R7_EPT_REG, reason))
		ret = p9221_set_cmd_reg(wpc, P9221R7_COM_SENDEPT);
				 ...
				 uint16_t orign_irq_src)
{
	...
	for (i = 0; i < P9221R7_OVER_CHECK_NUM; i++) {
		ret = p9221r7_clear_interrupts(wpc,
...
static void p9221r7_irq_handler(struct wpc_charger_info *wpc, uint16_t irq_src)
{
	...
	if (irq_src & P9221R7_STAT_PPRCVD) {
		...
		if (res) {
			CPRINTS("Failed to read PP buf: %d", res);
...
static int p9221_is_epp(struct wpc_charger_info *wpc)
{
	...
	if (!p9221_read8(wpc, P9221R7_SYSTEM_MODE_REG, &reg))
		return (reg & P9221R7_SYSTEM_MODE_EXTENDED_MASK) > 0;

	/* Check based on power supply voltage */
	ret = p9221_reg_read_cooked(wpc, P9221R7_VOUT_ADC_REG, &vout_uv);
	if (ret) {
...
static void p9221_config_fod(struct wpc_charger_info *wpc)
{
	...
	CPRINTS("write fod start");

	if (p9221_is_epp(wpc)) {
		fod_count = board_get_epp_fod(&fod);
		epp = true;
	} else {
		fod_count = board_get_fod(&fod);
	}
	...
	while (retries) {
		...
		CPRINTS("Writing %s FOD (n=%d try=%d)",
			epp ? "EPP" : "BPP", fod_count, retries);

		ret = p9221_block_write(wpc, P9221R7_FOD_REG, fod, fod_count);
		if (ret) {
			ret = -1;
			goto no_fod;
		}
		...
		ret = p9221_block_read(wpc, P9221R7_FOD_REG, fod_read, fod_count);
		if (ret) {
			ret = -2;
			goto no_fod;
		}
...
no_fod:
	CPRINTS("FOD not set! fod_count:%d retries:%d ret:%d",
		fod_count, retries, ret);
}
...
static void p9221_set_online(struct wpc_charger_info *wpc)
{
	...
	ret = p9221r7_enable_interrupts(wpc);
	if (ret)
		CPRINTS("enable int fail: %d", ret);

	/* NOTE: depends on _is_epp() which is not valid until DC_IN */
	p9221_config_fod(wpc);
}
...
static void p9221_vbus_check_timeout_defer(void)
{
	...
	CPRINTS("timeout VUBS, online=%d", wpc->online);
	if (wpc->online)
...
static int p9221_get_charge_supplier(struct wpc_charger_info *wpc)
{
	...
	if (p9221_is_epp(wpc)) {
		...
		ret = p9221_block_read(wpc, P9221R7_PROP_TX_ID_REG,
					(uint8_t *) &tx_id,
	...
	CPRINTS("ret=%d txmf_id=0x%04x tx_id=0x%08x charge_supplier=%d",
				ret, txmf_id, tx_id, wpc->charge_supplier);
	return ret;
}
...
static int p9221_set_dc_icl(struct wpc_charger_info *wpc)
{
	...
	if (p9221_reg_write_cooked_r7(wpc, P9221R7_ILIM_SET_REG,
					P9221R7_ILIM_MAX_UA))
		CPRINTS("%s set rx_iout limit fail.\n", __func__);
...
static void p9221_detect_defer_work(void)
{
	...
	CPRINTS("%s on:%d check_vbus:%d check_det:%d vbus:%d", __func__,
...
static void p9221r7_irq_thread(struct wpc_charger_info *wpc)
{
	int ret, irq_src;

	ret = p9221_read16(wpc, P9221_INT_REG, &irq_src);
	if (ret) {
		CPRINTS("Read INT REG failed.\n");
		return;
	}

	CPRINTS("INT SRC 0x%04x", irq_src);

	if (p9221r7_clear_interrupts(wpc, irq_src))
		return;
...
void wireless_power_charger_task(void *u)
{
	...
	if (p9221_read8(wpc, P9221_CUSTOMER_ID_REG, &cid)
			|| cid != P9221R7_CUSTOMER_ID_VAL)
		assert(0);

	while (1) {
		task_wait_event(-1);
		p9221r7_irq_thread(wpc);
	}
}
