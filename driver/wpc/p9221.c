...
static int p9221_reg_read_cooked(uint16_t reg, uint32_t *val)
{
	int ret = 0;
	int data = 0;
 ...
 */
static int p9221r7_enable_interrupts(void)
{
	uint16_t mask = 0;
	int ret;
	...
	ret = p9221r7_clear_interrupts(mask);
	if (ret)
...
static int p9221_send_csp(uint8_t stat)
{
	int ret;
	...
	if (!p9221_write8(P9221R7_CHARGE_STAT_REG, stat))
...
static int p9221_send_eop(uint8_t reason)
{
	...
	if (!p9221_write8(P9221R7_EPT_REG, reason))
		ret = p9221_set_cmd_reg(P9221R7_COM_SENDEPT);
 ...
 */
static void p9221_over_handle_r7(uint16_t orign_irq_src)
{
	uint8_t reason = 0;
	int i;
	int ret;
	...
	for (i = 0; i < P9221R7_OVER_CHECK_NUM; i++) {
		...
		ret = p9221_reg_read_cooked(P9221R7_IOUT_REG, &iout_val[i]);
		if (ret) {
...
static int p9221_is_epp(void)
{
	...
	if (!p9221_read8(P9221R7_SYSTEM_MODE_REG, &reg))
		return (reg & P9221R7_SYSTEM_MODE_EXTENDED_MASK) > 0;

	/* Check based on power supply voltage */
	ret = p9221_reg_read_cooked(P9221R7_VOUT_ADC_REG, &vout_uv);
	if (ret) {
...
static void p9221_config_fod(void)
{
	...
	while (retries) {
		...
		ret = p9221_block_write(P9221R7_FOD_REG, fod, fod_count);
		if (ret) {
			ret = -1;
			goto no_fod;
		}
		...
		ret = p9221_block_read(P9221R7_FOD_REG, fod_read, fod_count);
		if (ret) {
			ret = -2;
			goto no_fod;
		}
...
int p9221_notifier_check_det(void)
{
	if (wpc->online)
		goto done;
 ...
 */
static int p9221_reg_write_cooked_r7(uint16_t reg, uint32_t val)
{
	int ret = 0;
	uint16_t data;
	int i;
...
static int p9221_set_dc_icl(void)
{
	...
	if (p9221_reg_write_cooked_r7(P9221R7_ILIM_SET_REG,
				      P9221R7_ILIM_MAX_UA))
...
static void p9221r7_irq_thread(void)
{
	int ret, irq_src;
...
{
	int cid;
	if (p9221_read8(P9221_CUSTOMER_ID_REG, &cid)
