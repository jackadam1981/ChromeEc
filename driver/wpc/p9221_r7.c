...
static int p9221_get_charge_supplier(struct wpc_charger_info *wpc)
{
	...
	if (p9221_is_r7(wpc) && p9221_is_epp(wpc)) {
		...
		if (!ret && txmf_id == P9221_GPP_TX_MF_ID && (tx_id & 1 << 25))
	...
	CPRINTS("WPC:04 ret=%d txmf_id=0x%04x tx_id=0x%04x chare_supplier=%d",
...
static void p9221_r7_irq_thread(struct wpc_charger_info *wpc)
{
	int ret, irq_src, cid;
