 ...
 */
static int send_write_hw_reg_cmd_32bit(uint32_t address,
				       uint32_t data)
{
	uint8_t tx_buf[] = {
...
static int send_write_hw_reg_cmd_8bit(uint32_t address,
				      uint8_t data)
{
	uint8_t tx_buf[] = {
		ST_TP_CMD_WRITE_HW_REG,
...
static int erase_flash(void)
{
	...
	ret = send_write_hw_reg_cmd_32bit(0x20000128, 0xFFFFFF83);
	if (ret)
		return ret;
	ret = send_write_hw_reg_cmd_8bit(0x2000006B, 0x00);
	if (ret)
		return ret;
	ret = send_write_hw_reg_cmd_8bit(0x2000006A, 0xA0);
	if (ret)
		return ret;
...
{
	/* hold m3 */
	send_write_hw_reg_cmd_8bit(0x20000024, 0x01);
	/* unlock flash */
	send_write_hw_reg_cmd_8bit(0x20000025, 0x20);
	/* unlock flash erase */
	send_write_hw_reg_cmd_8bit(0x200000DE, 0x03);
	erase_flash();
...
static int st_tp_start_flash_dma(void)
{
	...
	ret = send_write_hw_reg_cmd_8bit(0x20000071, 0xC0);
	if (ret)
		return ret;
...
static int st_tp_read_frame(void)
{
	...
	uint8_t tx_buf[] = {
		...
		(get_heat_map_addr() >> 8) & 0xFF,
		(get_heat_map_addr() >> 0) & 0xFF,
	};
