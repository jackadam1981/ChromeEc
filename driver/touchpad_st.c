...
static int st_tp_read_system_info(int load)
{
	...
	ret = st_tp_command_response(ST_TP_CMD_READ_HOST_DATA_MEMORY, 0x0000,
				     _rx_buf, ARRAY_SIZE(_rx_buf));
	CPRINTS("chip_id: %02x %02x %02x %02x, %02x %02x %02x %02x",
		rx_buf[0], rx_buf[1], rx_buf[2], rx_buf[3],  /* Chip0 ID, VER */
		rx_buf[4], rx_buf[5], rx_buf[6], rx_buf[7]   /* Chip1 ID, VER */
		);
	...
	ret = st_tp_command_response(ST_TP_CMD_READ_HOST_DATA_MEMORY, 0x0010,
				     _rx_buf, ARRAY_SIZE(_rx_buf));
	CPRINTS("chip_id: %02x %02x %02x %02x, %02x %02x %02x %02x",
		rx_buf[0], rx_buf[1], rx_buf[2], rx_buf[3],  /* Chip0 ID, VER */
		rx_buf[4], rx_buf[5], rx_buf[6], rx_buf[7]   /* Chip1 ID, VER */
