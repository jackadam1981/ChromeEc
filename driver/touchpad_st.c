...
static int st_tp_handle_error_report(struct st_tp_event_t *e)
{
	...
	/*
	 * All of these are "FW shall halt" errors **AND** can be recovered by
	 * resetting touchpad after dumping errors.
	 */
	if (error_type <= ST_TP_ERROR_INVALID_CHIP_VERSION ||
	    (error_type >= 0x46 && error_type <= 0x4E)) {
		set_bits(&tp_control,
	...
	if (error_type == ST_TP_ERROR_PANEL_CONFIG_CRC_HEADER ||
		...
		set_bits(&tp_control, TP_CONTROL_SHALL_INITIALIZE,
			 TP_CONTROL_SHALL_INITIALIZE);
		/*
		 * full initialization wll be triggered in touchpad_task(), no
...
static void st_tp_init(void)
{
	...
	system_state = 0;
	set_bits(&tp_control, 0, TP_CONTROL_SHALL_RESET);
...
static void st_tp_full_initialize_end(void)
{
	...
	if (ret == EC_SUCCESS) {
		...
		set_bits(&tp_control, 0, TP_CONTROL_SHALL_INITIALIZE);
		st_tp_init();
...
void touchpad_task(void *u)
{
	...
	while (1) {
		...
		if (event & TASK_EVENT_WAKE)
			while (!(tp_control & TP_CONTROL_SHALL_RESET) &&
		...
		if (tp_control & TP_CONTROL_SHALL_RESET)
			st_tp_init();
