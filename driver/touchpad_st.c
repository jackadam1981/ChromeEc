...
#ifdef CONFIG_USB_ISOCHRONOUS
static struct packet_header_t {
	uint8_t index;

#define HEADER_FLAGS_NEW_FRAME	(1 << 0)
	uint8_t flags;
} __packed packet_header = {
	.index = 0,
	.flags = 0,
};
...
{
	/* TODO(stimim): we should not use `tp->elan`. */
 ...
 */
static int send_write_hw_reg_cmd(uint32_t address,
				 uint32_t data,
				 size_t data_len)
{
	uint8_t tx_buf[1 + 4 + 4] = {
	...
	for (i = 0; i < data_len; i++)
		tx_buf[1 + 4 + i] = (data >> ((data_len - i - 1) * 8)) & 0xFF;
	return spi_transaction(SPI, tx_buf, 1 + 4 + data_len, NULL, 0);
}

#define HOLD_M3()		send_write_hw_reg_cmd(0x20000024, 0x01, 1)
#define UNLOCK_FLASH()		send_write_hw_reg_cmd(0x20000025, 0x20, 1)
#define UNLOCK_FLASH_ERASE()	send_write_hw_reg_cmd(0x200000DE, 0x03, 1)
...
static int erase_flash(void)
{
	...
	ret = send_write_hw_reg_cmd(0x20000128, 0xFFFFFF83, 4);
	if (ret)
		return ret;
	ret = send_write_hw_reg_cmd(0x2000006B, 0x00, 1);
	if (ret)
		return ret;
	ret = send_write_hw_reg_cmd(0x2000006A, 0xA0, 1);
	if (ret)
		return ret;
...
static int st_tp_start_flash_dma(void)
{
	...
	ret = send_write_hw_reg_cmd(0x20000071, 0xC0, 1);
	if (ret)
		return ret;
...
static int st_tp_write_flash(int offset, int size, const uint8_t *data)
{
	...
	while (head < tail) {
		...
		tx_buf[index++] = (uint8_t)((flash_buffer_size & 0x000000FF));
		tx_buf[index++] = (uint8_t)((flash_buffer_size & 0x0000FF00)
					    >> 8);
		tx_buf[index++] = 0x00;
