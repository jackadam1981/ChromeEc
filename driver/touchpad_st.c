	...
	if (ret == EC_SUCCESS) {
		...
		for (i = 0; i < ST_TOUCH_COLS * ST_TOUCH_ROWS; i++) {
			...
			if (v < ST_TP_HEAT_MAP_THRESHOLD)
				v = 0;
			dest[i] = v;
			max_value = MAX(max_value, dest[i]);
		}
...
static int st_tp_usb_set_interface(usb_uint alternate_setting,
				   usb_uint interface);
static size_t heatmap_send_packet(usb_uint *usb_addr, size_t tx_size);
...
void heatmap_task(void *unused)
{
	...
	while (1) {
		...
		dtog = usb_isochronous_get_tx_dtog(config);
		addr = usb_isochronous_get_app_addr(config, dtog);

		count = heatmap_send_packet(addr, config->tx_size);
		if (dtog != usb_isochronous_get_tx_dtog(config))
			CPRINTS("%s: heatmap_send_packet too slow", __func__);
		usb_isochronous_set_app_count(config, dtog, count);
	}
}
