...
static int st_tp_read_all_events(int ignore_error);
 ...
 */
static int st_tp_handle_error_report(struct st_tp_event_t *e, int ignore_error)
{
...
static int st_tp_read_all_events(int ignore_error)
{
	...
	for (i = 0; i < ARRAY_SIZE(rx_buf.events); i++) {
		...
		if (e->evt_id == ST_TP_EVENT_ID_ERROR_REPORT) {
			ret = st_tp_handle_error_report(e, ignore_error);
			if (ret)
				return -ret;
		}
	}
