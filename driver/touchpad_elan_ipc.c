...
#define ETP_I2C_HID_DESCRPT		0x0001
...
/* for Sr (Repeated Start testing) */
static int elan_tp_read_hid_descriptor(void)
{
	uint16_t command = ETP_I2C_HID_DESCRPT;
	uint8_t buf[2];
	int rv;
	uint16_t val[15]; /* HID decriptor 30 byte */
	int i;
	...
	if (rv) {
		CPRINTS("reading 'HID Descriptor' error! (%d)", rv);
		return rv;
	}

	for (i = 0; i < 15; i++)
		CPRINTF("HID descrpt[%d] = 0x%04x\n", i, val[i]);

	return 0;

}
static int command_elan_HID_descrioptor(int argc, char **argv)
{
	int rv;

	rv = elan_tp_read_hid_descriptor();
	if (rv)
		return rv;

	return EC_SUCCESS;
}
DECLARE_CONSOLE_COMMAND(elanhid, command_elan_HID_descrioptor,
			"elanhid",
			"show Elan TP HID descriptor");
...
#if 0
	for (i = 0; i < ETP_MAX_FINGERS; i++) {
		CPRINTF("finger %d: w=%d, h=%d, x=%d, y=%d, pressure=%d\n",
				i,
				report.finger[i].width,
				report.finger[i].height,
				report.finger[i].x,
				report.finger[i].y,
				report.finger[i].pressure
		       );
	}
	CPRINTF("[ElanTP] report to IPC !\n");
#endif
...
static void elan_tp_init(void)
{
	...
	/* read FW version */
	rv = elan_tp_read_cmd(ETP_I2C_FW_VERSION_CMD, &ver);
	if (rv)
		goto out;
	CPRINTS("ELAN FW version = 0x%02x\n", ver & 0xFF);
