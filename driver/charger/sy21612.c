...
void sy21612_task(void)
{
	...
	while (1) {
		...
		if (flags & SY21612_INT_VBUS_OCP)
			CPRINTS("buck-boost VBUS OCP");
		if (flags & SY21612_INT_INDUCTOR_OCP)
			CPRINTS("buck-boost inductor OCP");
		if (flags & SY21612_INT_UVP)
			CPRINTS("buck-boost UVP");
		if (flags & SY21612_INT_OTP)
			CPRINTS("buck-boost OTP");
	}
}
#endif
...
static int command_sy21612(int argc, char **argv)
{
	...
	ccprintf("vbat voltage: %d mV", sy21612_get_vbat_voltage());
	ccprintf("vbus voltage: %d mV", sy21612_get_vbus_voltage());
	ccprintf("vbus current: %d mA", sy21612_get_vbus_current());
