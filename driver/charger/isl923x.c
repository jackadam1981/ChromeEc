...
#ifdef CONFIG_CHARGER_RAA489000
static enum ocpc_pid_status raa489000_set_vsys_compensation(int chgnum,
							    struct ocpc_data *o,
							    int current_ma,
							    int voltage_mv)
{
	...
	if (device_id < RAA489000_DEV_ID_B0)
		return OCPC_PID_LOOP_NEEDED;

	/*
	 * Set the desired current and voltage which if tripped indicates
	 * over charging.
	 */
	rv |= isl923x_set_current(chgnum, current_ma);
	rv |= isl923x_set_voltage(chgnum, voltage_mv);

	/*
	 * Need to set board resistance values: Rp1 and Rp2.
	 *
	 ...
	 * defined leaving Rp1 as the difference, but at least 36mOhms.
	 */
	rp1 = o->combined_rsys_rbatt_mo - CONFIG_OCPC_DEF_RBATT_MOHMS;
	rp1 /= 4;
	rp1 = CLAMP(rp1, RAA489000_RP1_MIN, RAA489000_RP1_MAX);
	/* Rp2 is set between 0-124mOhms in 4mOhm increments. */
	rp2 = CONFIG_OCPC_DEF_RBATT_MOHMS / 4;
	...
	if (!rv) {
		...
		regval &= ~RAA489000_C10_ENABLE_DVC_CC_LOOP;
	...
	return OCPC_PID_LOOP_NOT_NEEDED;
}
#endif /* CONFIG_CHARGER_RAA489000 */
