...
#if defined(CONFIG_CHARGER_RAA489000)// && defined(CONFIG_CHARGER_PPS_ENABLE)
static enum ec_error_list raa489000_enable_pps(int chgnum, bool enable)
{
	...
	if (enable) {
		reg |= RAA489000_C0_PPS_MODE_ON | RAA489000_C0_PTM_ENABLE;
	} else {
		reg &= ~(RAA489000_C0_PPS_MODE_ON | RAA489000_C0_PTM_ENABLE);
	}
...
static enum ec_error_list raa489000_is_pps_enabled(int chgnum, bool *enabled)
{
	...
	*enabled =
		(reg & (RAA489000_INFO_PPS_MODE | RAA489000_INFO_PTM_ACTIVE)) ==
		(RAA489000_INFO_PPS_MODE | RAA489000_INFO_PTM_ACTIVE);
