			   ...
			   int resistor)
{
...
#ifdef CONFIG_CHARGER_ISL9238
	ret = i2c_read16(slave_addr, ISL9238_REG_CONTROL3, &reg);
	if (ret)
		return ret;
	...
	if (direction)
		...
		reg &= ~ISL9238_C3_AMON_BMON_DIRECTION;
	ret = i2c_write16(slave_addr, ISL9238_REG_CONTROL3, reg);
	if (ret)
		return ret;
#endif
	...
	ret = i2c_read16(slave_addr, ISL923X_REG_CONTROL1, &reg);
	if (!ret) {
		...
		ret = i2c_write16(slave_addr, ISL923X_REG_CONTROL1, reg);
	}
...
static void dump_reg_range(int low, int high)
{
	...
	for (reg = low; reg <= high; reg++) {
		CPRINTF("[%Xh] = ", reg);
		rv = i2c_read16(slave_addr, reg, &regval);
		if (!rv)
			CPRINTF("0x%04x\n", regval);
		else
			CPRINTF("ERR (%d)\n", rv);
