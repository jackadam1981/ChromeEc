...
#define INPUT_RESISTOR_RATIO \
	((CONFIG_CHARGER_SENSE_RESISTOR_AC) / CONFIG_CHARGER_SENSE_RESISTOR)
#define REG_TO_INPUT_CURRENT(REG) ((REG + 1) * 50 / INPUT_RESISTOR_RATIO)
#define INPUT_CURRENT_TO_REG(CUR) (((CUR / 50) - 1) * INPUT_RESISTOR_RATIO)
/* TODO can I unit the above */

#define CHARGING_RESISTOR_RATIO \
	((CONFIG_CHARGER_SENSE_RESISTOR) / CONFIG_CHARGER_SENSE_RESISTOR)
...
int charger_get_status(int *status)
{
	...
	*status = CHARGER_LEVEL_2;
	...
	/* TODO(b/76429930): should we add more statuses? */
...
int charger_get_current(int *current)
{
	...
	rv = raw_read16(BQ25703_REG_CHAGE_CURRENT, &reg);
	if (!rv)
...
{
	return raw_write16(BQ25703_REG_CHAGE_CURRENT,
