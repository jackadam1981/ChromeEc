...
static enum ec_error_list isl9241_nvdc_to_bypass(int chgnum)
{
	...
	*/
	...
	isl9241_write(chgnum, ISL9241_REG_MAX_SYSTEM_VOLTAGE, voltage - 250);
...
static enum ec_error_list isl9241_enable_bypass_mode(int chgnum, bool enable)
{
	...
	if (enable) {
		...
		if (isl9241_is_in_chrg(chgnum)) {
			...
			CPRINTS("nvdc_chrg -> nvdc: %s", rv ? "failed" : "succeed");
		}
		/* A */
		rv = isl9241_nvdc_to_bypass(chgnum);
		CPRINTS("nvdc -> bypass: %s", rv ? "failed" : "succeed");
		return rv;
		...
		if (isl9241_is_in_chrg(chgnum)) {
			...
			CPRINTS("bypass_chrg -> bypass: %s", rv ? "failed" : "succeed");
		}
		/* B */
		rv = isl9241_bypass_to_nvdc(chgnum);
		CPRINTS("bypass -> nvdc: %s", rv ? "failed" : "succeed");
		return rv;
	} else {
		/* AC removal */
		if (isl9241_is_in_chrg(chgnum)) {
			...
			CPRINTS("bypass_chrg -> bat: %s", rv ? "failed" : "succeed");
			return rv;
		}
		/* M */
		rv = isl9241_bypass_to_bat(chgnum);
		CPRINTS("bypass -> bat: %s", rv ? "failed" : "succeed");
		return rv;
	}
