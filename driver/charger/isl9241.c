				      ...
				      int *out_vbus)
{
	...
	*out_vbus *= ISL9241_VIN_ADC_STEP_MV;
	...
	rv = isl9241_update(chgnum, ISL9241_REG_CONTROL3,
			    ISL9241_CONTROL3_ENABLE_ADC, MASK_CLR);
	if (rv) {
		CPRINTS("%s: failed to disable ADC", __func__);
		return rv;
	}
...
void isl9241_dump_prochot_status(int chgnum)
{
	int input_current = 0;
