...
int charger_set_current(int current)
{
	...
	if (isl9237_status & CHARGER_CHARGE_INHIBITED) {
...
int charger_post_init(void)
{
	...
	ret_cmd = raw_read16(ISL9237_REG_SYS_VOLTAGE_MAX, &value);
	if (!ret_cmd)
		isl9237_voltage = value;
	ret |= ret_cmd;

	ret_cmd = raw_read16(ISL9237_REG_CHG_CURRENT, &value);
	if (!ret_cmd)
		isl9237_current = REG_TO_CURRENT(value);
	ret |= ret_cmd;
