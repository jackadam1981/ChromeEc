...
/*
 * There's currently no known documented register to inhibit charging.
 * Therefore, the previous current level needs to be tracked as well as
 * if charginging is inhibited. On sysjump these values will be zero, however
 * the current variable will be read from the charger's register. If it's
 * zero, it's assumed that the charger state machine will update the current.
 */
static int isl9237_status;
static int isl9237_current;
...
int charger_get_status(int *status)
{
	...
	*status = CHARGER_LEVEL_2 |
			(isl9237_status & CHARGER_CHARGE_INHIBITED);
...
int charger_set_mode(int mode)
{
	...
	if (mode & CHARGE_FLAG_INHIBIT_CHARGE) {
		rv = isl9237_set_current(0);
		if (!rv)
			isl9237_status |= CHARGER_CHARGE_INHIBITED;
	} else {
		rv = isl9237_set_current(isl9237_current);
		if (!rv)
			isl9237_status &= ~CHARGER_CHARGE_INHIBITED;
	}
...
int charger_set_current(int current)
{
	...
	rv = isl9237_set_current(current);
	if (!rv)
		isl9237_current = current;
	return rv;
}
...
{
	return charger_get_current(&isl9237_current);
}
