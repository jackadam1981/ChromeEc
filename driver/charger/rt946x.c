...
{
	return i2c_read_block(I2C_PORT_CHARGER, RT946X_ADDR, reg, data, len);
}
...
{
	int rv = 0, i = 0, adc_start = 0, adc_result = 0, reg_data = 0;
	int adc_data_h = 0, adc_data_l = 0, aicr = 0;
	const int max_wait_times = 6;

	CPRINTF("%s\n", __func__);
	if (adc_sel == MT6370_ADC_TEMP_JC)
		CPRINTF("%s: Select ADC channel to TEMP_JC\n",  __func__);
	...
	if (rv < 0) {
		CPRINTF("%s: select ch to %d fail(%d)\n",
					__func__, adc_sel, rv);
		goto out;
	}
	...
	if (rv < 0) {
		CPRINTF("%s: start conversation failed, sel = %d, rv = %d\n",
			__func__, adc_sel, rv);
		goto out;
	}
	...
	if (i == max_wait_times) {
		CPRINTF("%s: wait conversation failed, sel = %d, rv = %d\n",
			__func__, adc_sel, rv);
		/* Add for debug */
		/* ZCV, reg0x10 */
		rv = rt946x_read8(MT6370_REG_OSCCTRL, &reg_data);
		if (rv < 0)
			CPRINTF("%s: read reg0x10 failed\n", __func__);
		else
			CPRINTF("%s: reg0x10 = 0x%02X\n", __func__, reg_data);

		/* TS auto sensing */
		rv = rt946x_read8(MT6370_REG_CHGHIDDENCTRL15, &reg_data);
		if (rv < 0)
			CPRINTF("%s: read reg0x10 failed\n", __func__);
		else
			CPRINTF("%s: reg0x3E = 0x%02X\n", __func__, reg_data);
	}
	...
	if (rv < 0) {
		CPRINTF("%s: read ADC data failed(%d)\n", __func__, rv);
		goto out;
	}
...
static int mt6370_toggle_cfo(void)
{
	...
	if (rv < 0) {
		CPRINTF("%s: check strobe fail\n", __func__);
		goto out;
	}
	if (data & MT6370_STROBE_EN_MASK) {
		CPRINTF("%s: in strobe mode\n", __func__);
		goto out;
	}
...
static int mt6370_irq_handler(void)
{
	...
	uint8_t stat_chg[16] = {0}, stat_old[16] = {0}, stat_new[16] = {0};
	uint8_t valid_chg[16] = {0};
	int i = 0, j = 0, ret = 0, reg_vale = 0;

	CPRINTF("%s\n", __func__);
	ret = rt946x_write8(MT6370_REG_IRQMASK, 0xfe);
	if (ret < 0) {
		CPRINTF("mask irq indicators fail\n");
		goto out_irq_handler;
	}
	ret = rt946x_read8(MT6370_REG_IRQIND, &reg_vale);
	if (ret < 0) {
		CPRINTF("read irq indicator fail\n");
		goto out_irq_handler;
	}
	...
	if (ret < 0) {
		CPRINTF("read post irq stat fail\n");
		goto out_irq_handler;
	}
	ret = rt946x_block_read(MT6370_REG_CHGMASK1, mask, 16);
	if (ret < 0) {
		CPRINTF("read irq mask fail\n");
		goto out_irq_handler;
	}
	ret = rt946x_write8(MT6370_REG_IRQMASK, 0x00);
	if (ret < 0) {
		CPRINTF("unmask irq indicators fail\n");
		goto out_irq_handler;
	}
	for (i = 0; i < 16; i++) {
