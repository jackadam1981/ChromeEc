...
static int rt946x_get_ieoc(uint32_t *ieoc)
{
	...
	*ieoc = rt946x_closest_reg(
		RT946X_IEOC_MIN, RT946X_IEOC_MAX, RT946X_IEOC_STEP,
...
static int rt946x_set_ieoc(unsigned int ieoc)
{
	...
	if (IS_ENABLED(CONFIG_CHARGER_MT6370) && !rv)
		/* store IEOC */
		return rt946x_get_ieoc(&curr_ieoc);
