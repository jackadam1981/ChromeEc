...
static int rt946x_set_current(int chgnum, int current);
static int rt946x_get_current(int chgnum, int *current);
static int rt946x_set_voltage(int chgnum, int voltage);
static int rt946x_enable_otg_power(int chgnum, int enabled);
static const struct charger_info *rt946x_get_info(void);
...
static int rt946x_read8(int chgnum, int reg, int *val)
{
	return i2c_read8(chg_chips[chgnum].i2c_port,
			 chg_chips[chgnum].i2c_addr_flags, reg, val);
}

static int rt946x_write8(int chgnum, int reg, int val)
{
	return i2c_write8(chg_chips[chgnum].i2c_port,
			  chg_chips[chgnum].i2c_addr_flags, reg, val);
}

static int rt946x_block_write(int chgnum, int reg, const uint8_t *val, int len)
{
...
int rt946x_por_reset(void)
{
	 ...
	 * logics.
	 */
	rv = rt946x_write8(0, MT6370_REG_RSTPASCODE1, MT6370_MASK_RSTPASCODE1);
	rv |= rt946x_write8(0, MT6370_REG_RSTPASCODE2, MT6370_MASK_RSTPASCODE2);
#else
	/* Hard reset, may take several milliseconds. */
	val = RT946X_MASK_RST;
	rv = rt946x_enable_hz(0, 0);
#endif
	if (rv)
		return rv;

	return rt946x_set_bit(0, RT946X_REG_CORECTRL_RST, val);
}
...
int charger_is_sourcing_otg_power(int port)
{
	...
	if (rt946x_read8(0, RT946X_REG_CHGCTRL1, &val))
		return 0;
...
static int rt946x_set_input_current(int chgnum, int input_current)
{
	uint8_t reg_iin = 0;
...
static int rt946x_get_input_current(int chgnum, int *input_current)
{
	int rv;
	int val = 0;
	const struct charger_info * const info = rt946x_get_info();
...
static int rt946x_manufacturer_id(int chgnum, int *id)
{
	return EC_ERROR_UNIMPLEMENTED;
}

static int rt946x_device_id(int chgnum, int *id)
{
	int rv;
...
static int rt946x_get_option(int chgnum, int *option)
{
...
static int rt946x_set_option(int chgnum, int option)
{
	/* Ignored: does not exist */
	return EC_SUCCESS;
}
...
static int rt946x_get_status(int chgnum, int *status)
{
	int rv;
	int val = 0;
...
static int rt946x_set_mode(int chgnum, int mode)
{
	int rv;
...
static int rt946x_get_voltage(int chgnum, int *voltage)
{
	int rv;
	int val = 0;
	const struct charger_info * const info = rt946x_get_info();
...
static int rt946x_discharge_on_ac(int chgnum, int enable)
{
...
static int rt946x_post_init(int chgnum)
{
#ifdef CONFIG_CHARGER_ILIM_PIN_DISABLED
	int rv;
...
static int rt946x_set_hw_ramp(int chgnum, int enable)
{
	int rv;
	unsigned int mivr = 0;
...
{
	int rv;
	rv = rt946x_enable_bc12_detection(0, 0);
	if (rv)
		return rv;
	/* mt6370 requires 40us delay to toggle RT946X_MASK_USBCHGEN */
	udelay(40);
	return rt946x_enable_bc12_detection(0, 1);
}
...
int rt946x_get_adc(enum rt946x_adc_in_sel adc_sel, int *adc_val)
{
...
#ifdef CONFIG_CHARGER_MT6370
	mt6370_enable_hidden_mode(0, 1);
#endif

	/* Select ADC to desired channel */
	rv = rt946x_update_bits(0, RT946X_REG_CHGADC, RT946X_MASK_ADC_IN_SEL,
	...
	rv = rt946x_set_bit(0, RT946X_REG_CHGADC, RT946X_MASK_ADC_START);
	if (rv)
		goto out;

	for (i = 0; i < max_wait_times; i++) {
		msleep(35);
		rv = mt6370_pmu_reg_test_bit(0, RT946X_REG_CHGADC,
	...
	/* Read ADC data */
	rv = rt946x_read8(0, RT946X_REG_ADCDATAH, &adc_data_h);
	rv = rt946x_read8(0, RT946X_REG_ADCDATAL, &adc_data_l);
	if (rv)
		goto out;
...
static int mt6370_toggle_cfo(void)
{
	...
	rv = rt946x_read8(0, MT6370_REG_FLEDEN, &data);
	if (rv)
		return rv;
	...
	/* read data */
	rv = rt946x_read8(0, RT946X_REG_CHGCTRL2, &data);
	if (rv)
		return rv;

	/* cfo off */
	data &= ~RT946X_MASK_CFO_EN;
	rv = rt946x_write8(0, RT946X_REG_CHGCTRL2, data);
	if (rv)
		return rv;

	/* cfo on */
	data |= RT946X_MASK_CFO_EN;
	return rt946x_write8(0, RT946X_REG_CHGCTRL2, data);
}
...
void usb_charger_task(void *u)
{
	...
	while (1) {
#ifdef CONFIG_CHARGER_MT6370
		mt6370_irq_handler(0);
#endif /* CONFIG_CHARGER_MT6370 */

		rt946x_read8(0, RT946X_REG_DPDMIRQ, &reg);
...
bc12_none:
			rt946x_enable_bc12_detection(0, 0);
		}
...
{
	return (en ? rt946x_set_bit : rt946x_clr_bit)
		(0, RT946X_REG_CHGCTRL2, RT946X_MASK_CHG_EN);
}
...
int rt946x_is_vbus_ready(void)
{
	...
	return rt946x_read8(0, RT946X_REG_CHGSTATC, &val) ?
...
int rt946x_is_charge_done(void)
{
	...
	if (rt946x_read8(0, RT946X_REG_CHGSTAT, &val))
		return 0;
 ...
 */
	...
	rv = rt946x_write8(0, RT946X_REG_CORECTRL2, 0x7F);
	if (rv)
		goto out;

	/* disable chg auto sensing */
	mt6370_enable_hidden_mode(0, 1);
	rv = rt946x_clr_bit(0, MT6370_REG_CHGHIDDENCTRL15,
	...
	msleep(50);
	/* enter shipping mode */
	rv = rt946x_set_bit(0, RT946X_REG_CHGCTRL2, RT946X_MASK_SHIP_MODE);
...
#endif
	/* enter shipping mode */
	return rt946x_set_bit(0, RT946X_REG_CHGCTRL2, RT946X_MASK_SHIP_MODE);
}

int rt946x_enable_charge_termination(int en)
{
	return (en ? rt946x_set_bit : rt946x_clr_bit)
		(0, RT946X_REG_CHGCTRL2, RT946X_MASK_TE);
}

int rt946x_enable_charge_eoc(int en)
{
	return (en ? rt946x_set_bit : rt946x_clr_bit)
		(0, RT946X_REG_CHGCTRL9, RT946X_MASK_EOC);
}
...
int mt6370_set_ldo_voltage(int mv)
{
	...
	rv = rt946x_update_bits(0, MT6370_REG_LDOCFG, MT6370_MASK_LDOCFG_OMS,
				0);
	if (rv)
		return rv;
	...
	if (mv == 0)
		return rt946x_clr_bit(0, MT6370_REG_LDOVOUT,
				      MT6370_MASK_LDOVOUT_EN);

	vout_val = 1 << MT6370_SHIFT_LDOVOUT_EN;
	vout_val |= rt946x_closest_reg(MT6370_LDO_MIN, MT6370_LDO_MAX,
				       MT6370_LDO_STEP, mv);
	return rt946x_update_bits(0, MT6370_REG_LDOVOUT, vout_mask, vout_val);
}

/* MT6370 Display bias */
int mt6370_db_external_control(int en)
{
	return rt946x_update_bits(0, MT6370_REG_DBCTRL1, MT6370_MASK_DB_EXT_EN,
...
int mt6370_backlight_set_dim(uint16_t dim)
{
	...
	rv = rt946x_write8(0, MT6370_BACKLIGHT_BLDIM2,
			   dim & MT6370_MASK_BLDIM2);

	if (rv)
		return rv;

	rv = rt946x_write8(0, MT6370_BACKLIGHT_BLDIM,
			    ...
			    enum mt6370_led_dim_mode mode)
{
	...
	rt946x_update_bits(0, MT6370_REG_RGBDIM_BASE + index,
...
{
	return rt946x_update_bits(0, MT6370_REG_RGBEN,
...
int mt6370_led_set_brightness(enum mt6370_led_index index, uint8_t brightness)
{
	...
	rt946x_update_bits(0, MT6370_REG_RGBISNK_BASE + index,
...
int mt6370_reduce_db_bl_driving(void)
{
	...
	/* Enter test mode */
	rv = rt946x_block_write(0, MT6370_REG_TM_PAS_CODE1,
				mt6370_val_en_test_mode,
				ARRAY_SIZE(mt6370_val_en_test_mode));
	if (rv)
		return rv;
	msleep(1);
	rv = rt946x_write8(0, MT6370_REG_BANK, MT6370_MASK_REG_TM);
	if (rv)
		return rv;
	msleep(1);
	/* reduce bl driving */
	rv = rt946x_update_bits(0, MT6370_TM_REG_BL3, MT6370_TM_MASK_BL3_SL,
				MT6370_TM_REDUCE_BL3_SL);
	if (rv)
		return rv;
	msleep(1);
	/* reduce db driving */
	rv = rt946x_update_bits(0, MT6370_TM_REG_DSV1, MT6370_TM_MASK_DSV1_SL,
				MT6370_TM_REDUCE_DSV1_SL);
	if (rv)
		return rv;
	msleep(1);
	/* Leave test mode */
	return rt946x_write8(0, MT6370_REG_TM_PAS_CODE1, MT6370_LEAVE_TM);
}
#endif /* CONFIG_CHARGER_MT6370 */
