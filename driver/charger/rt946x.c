...
static int mt6370_detect_apple_samsung_ta(int usb_stat)
{
	...
	int dp_0_9v = false, dp_1_5v = false, dp_2_3v = false, dm_2_3v = false;
	...
	ret = rt946x_update_bits(
		MT6370_REG_QCSTATUS2, 0x0F, 0x03);
	if (ret < 0)
		return ret;
	...
	/* Samsung charger */
	if (!dp_1_5v)
		return MT6370_CHG_TYPE_DCP;

	/* Check DP > 2.3 V */
	ret = rt946x_update_bits(
		MT6370_REG_QCSTATUS2, 0x0F, 0x0B);
	if (ret < 0)
		return ret;
...
static int rt946x_get_bc12_device_type(void)
{
	...
	result = mt6370_detect_apple_samsung_ta(reg);
