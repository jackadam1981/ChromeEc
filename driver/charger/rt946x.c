...
#ifdef CONFIG_LED_SET_COLOR_INDEPENDENT
	mt6370_led_set_color_independent(index);
#endif
...
#ifdef CONFIG_LED_SET_COLOR_INDEPENDENT
int mt6370_led_set_color_independent(enum mt6370_led_index index)
{
	int val;
	int mask;
	if (index >= MT6370_LED_ID_COUNT)
	...
	switch (index) {
	case MT6370_LED_ID1:
		mask = MT6370_MASK_RGB_ISNK1DIM_EN;
		break;
	case MT6370_LED_ID2:
		mask = MT6370_MASK_RGB_ISNK2DIM_EN;
		break;
	case MT6370_LED_ID3:
		mask = MT6370_MASK_RGB_ISNK3DIM_EN;
		break;
	default:
		mask = MT6370_MASK_RGB_ISNK_ALL_EN;
	}

	rt946x_update_bits(MT6370_REG_RGBEN, mask, val);
	return EC_SUCCESS;
}
#endif
#endif
