...
int rt946x_bc12_workaround(int *bc12_type_old)
{
	int rv;
	/*
	 * first toggle bc12 detection for D+ pull up to high level,
	 * then fast toggle secondary, we can use the result to resolve
	 * D+ is more increase time by high parasitic capacitance
	 */
...
void usb_charger_task(void *u)
{
	...
	while (1) {
		...
		if (reg & RT946X_MASK_DPDMIRQ_ATTACH) {
			...
			if (bc12_type == CHARGE_SUPPLIER_BC12_SDP && bc12_cnt < max_bc12_cnt) {
