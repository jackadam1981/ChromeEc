...
#if 0
/*
 * These are for miss INT workaround full interrupts workaround.
 * But we just use MIVR_IRQ in this project.
 */
...
/* Now we just use MIVR */
static const uint8_t mt6370_irqr_filter[MT6370_USED_IRQ_MAX] = {
	0xF1,
};

static const uint8_t mt6370_irqf_filter[MT6370_USED_IRQ_MAX] = {
	0xF1,
};

typedef int (*mt6370_irq_fptr)(void);
static mt6370_irq_fptr mt6370_irq_handler_tbl[MT6370_USED_IRQ_MAX] = {
...
static int mt6370_irq_handler(void)
{
	...
	valid_chg = (stat_new & mt6370_irqr_filter[0]) |
		    (~stat_new & mt6370_irqf_filter[0]);
	data |= (stat_chg & valid_chg);
	data &= ~mask;
	if (data)
		ret = mt6370_irq_handler_tbl[MT6370_MIVR_IRQ]();
out_irq_handler:
	return ret;
}
