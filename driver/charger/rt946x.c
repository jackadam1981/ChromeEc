...
static int rt946x_set_boost_current(unsigned int current)
{
	...
	/*
	 * Find the lowest output current threshold which can
	 * support our output current requirement
	 */
	for (i = 0; i < ARRAY_SIZE(rt946x_boost_current); i++) {
		if (current < rt946x_boost_current[i])
			break;
	}

	reg_current = (i >= ARRAY_SIZE(rt946x_boost_current)) ?
		      ARRAY_SIZE(rt946x_boost_current) - 1 : i;
