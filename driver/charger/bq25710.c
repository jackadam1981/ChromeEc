...
/* adc_en_mask maps to bit[7:0] in ADCOption register */
...
int charger_get_vbus_voltage(int port)
{
	...
	if (bq25710_adc_start(BQ25710_ADC_OPTION_EN_ADC_VBUS))
		goto error;

	/* Read ADC value */
	if (raw_read16(BQ25710_REG_ADC_VBUS_PSYS, &reg))
		goto error;
...
error:
	CPRINTF("Could not read VBUS ADC!\n");
	return 0;
}
...
int chg_ramp_get_current_limit(void)
{
	...
	if (bq25710_adc_start(BQ25710_ADC_OPTION_EN_ADC_IIN))
		goto error;
