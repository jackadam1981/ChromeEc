...
#ifndef CONFIG_BQ25720_CHARGE_OPTION_4_IDCHG_DEG2_CUSTOM
...
#define BQ257X0_CHARGE_CURRENT_MASK BQ_FIELD_MASK(BQ257X0, CHARGE_CURRENT, CHARGE_CURRENT)

#define REG_TO_CHARGING_CURRENT(REG) (((REG) & BQ257X0_CHARGE_CURRENT_MASK) / \
				      CHARGING_RESISTOR_RATIO)
#define CHARGING_CURRENT_TO_REG(CUR) (((CUR) * CHARGING_RESISTOR_RATIO) & \
				      BQ257X0_CHARGE_CURRENT_MASK)
...
static int bq25710_set_low_power_mode(int chgnum, int enable)
{
	...
	if (enable)
		reg = SET_CO0(EN_LWPWR, true, reg);
	else
		reg = SET_CO0(EN_LWPWR, false, reg);
...
static int bq25710_set_cmp_ref_1p2(int chgnum, bool enable)
{
	...
	if (enable)
		...
		reg = SET_CO1_BY_NAME(CMP_REF, 2P3, reg);
...
static int bq25710_set_psys_sensing(int chgnum, bool enable)
{
	...
	if (IS_ENABLED(CONFIG_CHARGER_BQ25720)) {
		...
		reg = SET_BQ_FIELD(BQ25710, CHARGE_OPTION_1, EN_PSYS, enable, reg);
	}
...
static int bq25710_adc_start(int chgnum, int adc_en_mask)
{
	...
	} while (--tries_left && (reg & BQ_FIELD_MASK(BQ257X0, ADC_OPTION, ADC_START)));
...
static void bq25710_init(int chgnum)
{
	...
	if (!raw_read16(chgnum, BQ25710_REG_PROCHOT_OPTION_1, &reg)) {
		...
		{
			int ma_512 = CONFIG_CHARGER_BQ25710_IDCHG_LIMIT_MA >> 9;
		reg = SET_PO1(IDCHG_VTH, ma_512, reg);
		}
...
static enum ec_error_list bq25710_set_mode(int chgnum, int mode)
{
	...
	if (mode & CHARGER_CHARGE_INHIBITED)
		option |= BQ_FIELD_MASK(BQ257X0, CHARGE_OPTION_0, CHRG_INHIBIT);
	else
		option &= ~BQ_FIELD_MASK(BQ257X0, CHARGE_OPTION_0, CHRG_INHIBIT);
...
static enum ec_error_list bq25710_discharge_on_ac(int chgnum, int enable)
{
	...
	if (enable)
		option |= BQ_FIELD_MASK(BQ257X0, CHARGE_OPTION_0, EN_LEARN);
	else
		option &= ~BQ_FIELD_MASK(BQ257X0, CHARGE_OPTION_0, EN_LEARN);
						   ...
						   int *voltage)
{
	...
	rv = bq25710_adc_start(chgnum, BQ_FIELD_MASK(BQ257X0, ADC_OPTION, EN_ADC_VBUS));
	if (rv)
		goto error;
...
static enum ec_error_list bq25710_set_hw_ramp(int chgnum, int enable)
{
	...
	if (enable) {
		...
		option3_reg |= BQ_FIELD_MASK(BQ257X0, CHARGE_OPTION_3, EN_ICO_MODE);

		/* 0b: Input current limit is set by BQ25710_REG_IIN_HOST */
		option2_reg &= ~BQ_FIELD_MASK(BQ257X0, CHARGE_OPTION_2, EN_EXTILIM);
