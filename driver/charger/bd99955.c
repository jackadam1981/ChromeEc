...
static int bd99955_is_bc12_charging_enabled(enum bd99955_charge_port port)
{
	int rv;
	int reg;
	...
	return !rv && !(reg & ((port == BD99955_CHARGE_PORT_VBUS) ?
...
static void bd99955_bc12_detach(int port, int type)
{
	...
	int bc12_chg_en = bd99955_is_bc12_charging_enabled(port);

	/* Update charge manager */
	if (bc12_chg_en)
		charge_manager_update_charge(CHARGE_SUPPLIER_VBUS, port,
						&charge);
