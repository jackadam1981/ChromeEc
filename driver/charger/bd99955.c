...
static int bd99955_amon_bmon_chg_adc(void)
{
	...
	/*
	 * VIDCHG = GIDCHG * (VSRN- VSRP) = IDCHG / IDCHG_REG
	 * VIADP = GIADP * (VACP- VACN) = IADP / IADP_REG
	 */
...
static int bd99955_amon_bmon(int amon_bmon)
{
	...
	if (amon_bmon) {
		...
		sns_reg = CONFIG_CHARGER_SENSE_RESISTOR_AC;
	} else {
		reg &= ~BD99955_CMD_PMON_IOUT_CTRL_SET_IOUT_SOURCE_SEL;
		sns_reg = CONFIG_CHARGER_SENSE_RESISTOR;
	}
