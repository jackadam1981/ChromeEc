...
static void usb_charger_process(enum bd99955_charge_port port,
				int *vbus_provided, int *bc12_type)
{
	vbus_provided[port] = pd_snk_is_vbus_provided(port);

	if (vbus_provided[port]) {
...
void usb_charger_task(void)
{
	...
	int vbus_provided[CONFIG_USB_PD_PORT_COUNT];
