				...
				enum bd99955_command map_cmd)
{
	...
	if (charger_map_cmd != map_cmd) {
		...
		if (rv) {
			charger_map_cmd = BD99955_INVALID_COMMAND;
...
static int bd99955_charger_enable(int enable)
{
	...
	if (enable) {
		/*
		 * make sure the charge current > 0 before
		 * enable charging
		 */
...
int charger_set_current(int current)
{
	...
	if (current == 0) {
		/* disable charge before set charge current to 0 */
		rv = bd99955_charger_enable(0);
		if (rv)
			return rv;
	}
...
#elif (CONFIG_BD99955_DCDC_CLK_KHZ == 857)
	reg |= BD99955_CMD_CHGOP_SET2_DCDC_CLK_SEL_857;
#elif (CONFIG_BD99955_DCDC_CLK_KHZ == 600)
	reg |= BD99955_CMD_CHGOP_SET2_DCDC_CLK_SEL_600;
#else
...
int charger_get_vbus_voltage(int port)
{
	...
	read_reg = (bd99955_pd_port_to_chg_port(port) ==
		    BD99955_CHARGE_PORT_VBUS) ? BD99955_CMD_VBUS_VAL :
						BD99955_CMD_VCC_VAL;

	return ch_raw_read16(read_reg, &voltage, BD99955_EXTENDED_COMMAND) ?
			     0 : voltage;
}
