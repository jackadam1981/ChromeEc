...
static inline int rt9490_enable_hz(int chgnum, bool en)
{
	return (en ? rt9490_set_bit : rt9490_clr_bit)
		   (chgnum, RT9490_REG_CHG_CTRL0, RT9490_EN_HZ);
}
...
static enum ec_error_list rt9490_get_current(int chgnum, int *current)
{
	...
	rv = rt9490_read16(chgnum, RT9490_REG_ICHG_CTRL, &val);
	if (rv)
		return rv;
...
static enum ec_error_list rt9490_get_voltage(int chgnum, int *voltage)
{
	...
	uint16_t val = 0;
	const struct charger_info * const info = rt9490_get_info(chgnum);
	rv = rt9490_read16(chgnum, RT9490_REG_VCHG_CTRL, &val);
	if (rv)
		return rv;
...
static enum ec_error_list rt9490_enable_otg_power(int chgnum, int enabled)
{
	return (enabled ? rt9490_set_bit : rt9490_clr_bit)
		(chgnum, RT9490_REG_CHG_CTRL3, RT9490_EN_OTG);
}
...
enum ec_error_list rt9490_set_otg_current_voltage(int chgnum, int output_current, int output_voltage)
{
	...
	rv = rt9490_write16(chgnum, RT9490_REG_IOTG_REGU, reg_cur);
	if (rv)
		return rv;
...
static int rt9490_reset_chip(int chgnum)
{
	...
	if (rv < 0) {
		CPRINTS("%s: disable hz fail", __func__);
		return rv;
	}
...
{
	return (en ? rt9490_set_bit : rt9490_clr_bit)
		(chgnum, RT9490_REG_CHG_CTRL2, RT9490_BC12_EN);
}

static inline int rt9490_enable_wdt(int chgnum, bool en)
{
	int val = en ? RT9490_WATCHDOG_DISABLE : RT9490_WATCHDOG_40_SEC;
...
{
	CPRINTS("%s: en = %d", __func__, en);
	return (en ? rt9490_clr_bit : rt9490_set_bit)
			(chgnum, RT9490_REG_JEITA_CTRL1, RT9490_JEITA_DIS);
}
...
static int rt9490_init_setting(int chgnum)
{
	...
	rv = rt9490_clr_bit(chgnum, RT9490_REG_CHG_CTRL5, RT9490_ILIM_HZ_EN);
	if (rv)
		return rv;
	...
	if (rv)
		return rv;
	/* Disable WDT */
	rv = rt9490_enable_wdt(chgnum, false);
	if (rv)
		return rv;
	...
	rv = rt9490_clr_bit(chgnum, RT9490_REG_ADD_CTRL0,
			RT9490_AUTO_AICR | RT9490_AUTO_MIVR);
	if (rv)
		return rv;
	/* Disable charge timer */
	rv = rt9490_clr_bit(chgnum, RT9490_REG_SAFETY_TMR_CTRL,
	...
	rv = rt9490_set_bit(chgnum, RT9490_REG_ADC_CHANNEL0, RT9490_VSYS_ADC_DIS);
    if (rv)
    	return rv;
    ...
    rv = rt9490_clr_bit(chgnum, RT9490_REG_ADC_CHANNEL0, RT9490_VSYS_ADC_DIS);
    if (rv)
    	return rv;
#endif
	...
	RETURN_ERROR(rt9490_set_bit(chgnum, RT9490_REG_CHG_IRQ_MASK2, 0x7F));
	RETURN_ERROR(rt9490_set_bit(chgnum, RT9490_REG_CHG_IRQ_MASK3, 0x1F));
	RETURN_ERROR(rt9490_set_bit(chgnum, RT9490_REG_CHG_IRQ_MASK4, 0xFF));
	RETURN_ERROR(rt9490_set_bit(chgnum, RT9490_REG_CHG_IRQ_MASK5, 0xF4));
...
static enum ec_error_list rt9490_get_actual_current(int chgnum, int *current)
{
	int rv;
	uint16_t reg_val;
	rv = rt9490_read16(chgnum, RT9490_REG_IBAT_ADC, &reg_val);
	if (rv)
		return rv;
...
static enum ec_error_list rt9490_get_actual_voltage(int chgnum, int *voltage)
{
	int rv;
	uint16_t reg_val;
	rv = rt9490_read16(chgnum, RT9490_REG_VBAT_ADC, &reg_val);
	if (rv)
		return rv;
...
static enum ec_error_list rt9490_get_vbus_voltage(int chgnum, int port, int *voltage)
{
	int rv;
	uint16_t reg_val;
	rv = rt9490_read16(chgnum, RT9490_REG_VBUS_ADC, &reg_val);
	if (rv)
		return rv;
	*voltage = (int)reg_val * 1000;
	return EC_SUCCESS;
...
static enum ec_error_list rt9490_get_input_current_limit(int chgnum, int *input_current)
{
	int rv;
	uint16_t val = 0;
	rv = rt9490_read16(chgnum, RT9490_REG_AICR_CTRL, &val);
	if (rv)
		return rv;
...
static enum ec_error_list rt9490_get_input_current(int chgnum, int *input_current)
{
	int rv;
	uint16_t reg_val;
	rv = rt9490_read16(chgnum, RT9490_REG_IBUS_ADC, &reg_val);
	if (rv)
		return rv;
...
static enum charge_supplier rt9490_get_bc12_device_type(int chgnum)
{
	...
	switch (vbus_stat) {
	case RT9490_SDP:
		CPRINTS("RT9490 BC12 SDP");
		return CHARGE_SUPPLIER_BC12_SDP;
	case RT9490_CDP:
		CPRINTS("RT9490 BC12 CDP");
		return CHARGE_SUPPLIER_BC12_CDP;
	case RT9490_DCP:
		CPRINTS("RT9490 BC12 DCP");
		return CHARGE_SUPPLIER_BC12_DCP;
	default:
		CPRINTS("RT9490 BC12 UNKNOWN 0x%02X", vbus_stat);
		return CHARGE_SUPPLIER_NONE;
	}
}
...
{
	__rt9490_enable_chgdet_flow(CHARGER_SOLO, false);
	...
	while (1) {
		...
		if (evt & USB_CHG_EVENT_VBUS) {
			...
			if (is_non_pd_sink)
				__rt9490_enable_chgdet_flow(CHARGER_SOLO, true);
			else
...
void rt9490_deferred_interrupt(void)
{
	...
	ret = rt9490_read8(CHARGER_SOLO, RT9490_REG_CHG_IRQ_FLAG1, &irq_flag);
	if (ret)
		return;
