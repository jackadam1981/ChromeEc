...
#define be16toh sys_be16_to_cpu
#define htobe16 sys_cpu_to_be16
...
static int rt9490_init_setting(int chgnum)
{
...
#endif
	/* Disable ILIM_HZ_EN */
	rv = rt9490_clr_bit(chgnum, RT9490_REG_CHG_CTRL5, RT9490_ILIM_HZ_EN_MASK);
	if (rv)
		return rv;
	...
	rv = rt9490_clr_bit(chgnum, RT9490_REG_ADD_CTRL0,
			RT9490_AUTO_AICR_MASK | RT9490_AUTO_MIVR_MASK);
	if (rv)
		return rv;
	...
	if (rv)
		return rv;
	/* disable JEITA */
	rv = rt9490_set_bit(chgnum, RT9490_REG_JEITA_CTRL1, RT9490_DIS_JEITA_FN_MASK);
	if (rv)
		return rv;
