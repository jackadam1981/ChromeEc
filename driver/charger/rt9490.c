...
static const struct rt9490_init_setting default_init_setting = {
	.eoc_current = 200000,
	.mivr = 4000000,
	.boost_voltage = 5050000,
	.boost_current = 1500000,
};
...
static enum ec_error_list rt9490_get_current(int chgnum, int *current)
{
	...
	if (val < info->current_min)
		*current = info->current_min;
	else if (val > info->current_max)
...
static enum ec_error_list rt9490_get_voltage(int chgnum, int *voltage)
{
	...
	if (val < info->voltage_min)
		*voltage = info->voltage_min;
	else if (val > info->voltage_max)
		*voltage = info->voltage_max;
	else
		*voltage = val * info->voltage_step;
	return EC_SUCCESS;
}
...
enum ec_error_list rt9490_set_otg_current_voltage(int chgnum, int output_current, int output_voltage)
{
	...
	if (output_current < RT9490_IOTG_MIN || output_current > RT9490_IOTG_MAX)
		return EC_ERROR_PARAM2;
	if (output_voltage < RT9490_VOTG_MIN || output_voltage > RT9490_VOTG_MAX)
