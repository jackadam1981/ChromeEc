...
_overridable struct charger_thermistor_info charger_thermistor_info = {
...
int thermistor_linear(uint16_t mv, const struct charger_thermistor_info *info)
{
	...
	const int charger_thermistor_data_size =
		ARRAY_SIZE(charger_thermistor_data);
	int v_high = 0, v_low = 0, t_low, t_high, num_steps;
	int head, tail, mid = 0;
	/* We need at least two points to form a line. */
	BUILD_ASSERT(charger_thermistor_data_size >= 2);
	...
	if (mv > data[0].mv)
		return data[0].temp;
	else if (mv < data[charger_thermistor_data_size - 1].mv)
		return data[charger_thermistor_data_size - 1].temp;

	head = 0;
	tail = charger_thermistor_data_size - 1;
	while (head != tail) {
		mid = (head + tail) / 2;
		v_high = data[mid].mv;
		v_low = data[mid + 1].mv;
