...
static void sm5803_init(int chgnum)
{
	...
	for (try = 0; ; try++) {
		...
		if (rv == EC_SUCCESS) {
			...
			if (reg & SM5803_STATUS1_VSYS_OK)
				break;

		}
		if (try >= 10) {
			CPRINTS("%s %d: Failed init conditions", CHARGER_NAME,
								 chgnum);
			return;
		}
		/* Waiting for either charger to be responsive or Vsys OK */
		msleep(10);
	}
	...
	rv |= chg_write8(chgnum, 0x69, 0xC5);
