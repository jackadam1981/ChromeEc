...
static void sm5803_init(int chgnum)
{
	...
	/* TODO: set TINT high threshold to... something? */
...
void sm5803_handle_interrupt(int chgnum)
{
	...
	if (int_reg & SM5803_INT2_TINT) {
		CPRINTS("CHG%d: High temp, pulling PROCHOT low", chgnum);
		gpio_set_level(GPIO_EC_PROCHOT_ODL, 0);
	}

	if (int_reg & SM5803_INT2_VBUS) {
		rv = meas_read8(chgnum, SM5803_REG_VSYS_PREREG_MSB, &vbus_reg);
		if (vbus_reg <= SM5803_VBUS_LOW_LEVEL)
			...
			CPRINTS("CHG%d: Unexpected Vbus interrupt: 0x%02x",
				chgnum, vbus_reg);
	}

	/* Clear out the interrupts now that they're serviced */
	rv = main_write8(chgnum, SM5803_REG_INT2_REQ, int_reg);
	if (rv)
