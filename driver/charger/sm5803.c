...
static void sm5803_init(int chgnum)
{
	...
	/*
	 * TODO: when to set automatic charge bits for primary charger?  Do we
	 * need a driver call to enable charging and also set these bits?  Is
	 * that implied by setting the current for the battery?  Should they be
	 * set when enabling charging?
	 */
	...
	/* TODO: May need to tweak the Vbus and TINT lower/upper thresholds */
	if (rv)
		CPRINTS("CHG%d: Failed initialization", chgnum);
}
...
void sm5803_handle_interrupt(int chgnum)
{
	...
	rv = main_read8(chgnum, SM5803_REG_INT2_REQ, &reg);
	if (rv) {
		CPRINTS("CHG%d: Failed read int2 register", chgnum);
		return;
	}

	if (reg)
		CPRINTS("CHG%d: Interrupt2 0x%02x", chgnum, reg);

	/* TODO: check vbus low/high, call usb_charger_vbus_change */

	if (reg & SM5803_INT2_TINT)
		gpio_set_level(GPIO_EC_PROCHOT_ODL, 0);

	/* Clear out the interrupts now that they're serviced */
	rv = main_write8(chgnum, SM5803_REG_INT2_REQ, reg);
	if (rv)
		CPRINTS("CHG%d:, Failed write int2 register", chgnum);
						   ...
						   int *voltage)
{
	...
	/*
	 * TODO: check conversion?  No obvious mention in datasheet of what
	 * bits mean
	 */
	*voltage = volt_bits;
	return rv;
