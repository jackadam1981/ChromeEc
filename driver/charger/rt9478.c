...
#define CONFIG_CHARGER_RT9478_IAICR2 \
	RT9478_PROCHOT_OPTION_0_IAICR2__1P10
#endif
 ...
 */
#define RT9478_IAICR_CODE0_OFFSET REG_TO_CHARGING_CURRENT(100)
...
static inline int iaicr_reg_to_current(int reg)
{
	 ...
	 * Richtek don’t have exactly 0A setting for input current limit,
	...
	if (reg == 0)
		...
		return REG_TO_CHARGING_CURRENT_IN(
			reg * RT9478_AICR_CURRENT_STEP_MA);
}
...
static int rt9478_set_low_power_mode(int chgnum, int enable)
{
	int rv;
	int reg;
	 ...
	 */
	if (enable)
		rt9478_perf_mode_req &= ~(1 << task_get_current());
	else
		rt9478_perf_mode_req |= (1 << task_get_current());
...
static int rt9478_get_low_power_mode(int chgnum, int *mode)
{
	int rv;
	int reg;
...
static int rt9478_adc_start(int chgnum, int adc_en_mask)
{
	...
	if (rt9478_set_low_power_mode(chgnum, 0))
		return EC_ERROR_UNKNOWN;
	...
	do {
		...
		crec_msleep(2);
		raw_read16(chgnum, RT9478_REG_ADC_OPTION, &reg);
	} while (--tries_left &&
...
{
	if (enable)
		reg = SET_RT_FIELD_BY_NAME(RT9478, CHARGE_OPTION_1,
...
static void rt9478_init(int chgnum)
{
	...
	if (!system_jumped_late() && battery_is_present() &&
		...
		rv = rt9478_set_low_power_mode(chgnum, 0);
		/* Allow enough time for VDDA to be powered */
		crec_msleep(RT9478_VDDA_STARTUP_DELAY_MSEC);

		if (IS_ENABLED(
			    CONFIG_CHARGER_RT9478_VSYS_MIN_VOLTAGE_CUSTOM)) {
		...
		rt9478_set_low_power_mode(chgnum, 1);
	}
...
static enum ec_error_list rt9478_set_mode(int chgnum, int mode)
{
	...
	option = SET_CO0(CHG_INHIBIT,
			 mode & CHARGE_FLAG_INHIBIT_CHARGE ? 1 : 0, option);
							 ...
							 int *input_current)
{
	...
	if (!rv)
		*input_current = iaicr_reg_to_current(
			reg >> RT9478_AICR_CURRENT_SHIFT);
...
static int reg_adc_input_current_to_ma(int reg)
{
	 ...
	 * LSB => 50mA.
	 */
	return reg * REG_TO_CHARGING_CURRENT(50);
}
...
static enum ec_error_list rt9478_get_vbus_voltage(int chgnum, int port,
						  int *voltage)
{
	int reg, rv;
...
static enum ec_error_list rt9478_set_hw_ramp(int chgnum, int enable)
{
	...
	if (enable) {
		...
		option3_reg = SET_RT_FIELD(RT9478, CHARGE_OPTION_3,
					   EN_AICC, 1, option3_reg);
...
{
	rt9478_set_low_power_mode(CHARGER_SOLO, 0);
}
...
static void console_rt9478_dump_regs(int chgnum)
{
	...
	static const uint8_t regs[] = {
		RT9478_REG_CHARGE_OPTION_0,
		RT9478_REG_CHARGE_CURRENT,
		RT9478_REG_CHARGE_VOLTAGE,
		RT9478_REG_CHARGER_STATUS,
		RT9478_REG_PROCHOT_STATUS,
		RT9478_REG_AICR,
		RT9478_REG_ADC_VBUS_PSYS,
		RT9478_REG_ADC_IBAT,
		RT9478_REG_ADC_CMPIN_IIN,
		RT9478_REG_ADC_VSYS_VBAT,
		RT9478_REG_CHARGE_OPTION_1,
		RT9478_REG_CHARGE_OPTION_2,
		RT9478_REG_CHARGE_OPTION_3,
		RT9478_REG_PROCHOT_OPTION_0,
		RT9478_REG_PROCHOT_OPTION_1,
		RT9478_REG_ADC_OPTION,
		RT9478_REG_CHARGE_OPTION_4,
		RT9478_REG_VMIN_ACTIVE_PROTECTION,
		RT9478_REG_OTG_VOLTAGE,
		RT9478_REG_OTG_CURRENT,
		RT9478_REG_INPUT_VOLTAGE,
		RT9478_REG_MIN_SYSTEM_VOLTAGE,
		RT9478_REG_AICR_HOST,
		RT9478_REG_MANUFACTURER_ID,
