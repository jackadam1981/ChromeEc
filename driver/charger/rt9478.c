...
static inline int iaicr_reg_to_current(int reg)
{
	 ...
	 * you will see 00, but actually it’s 100mA@10mOhm right now.
	 * Richtek don’t have exactly 0A setting for input current limit,
	 * it set the 100mA@10mOhm offset so that the converter can
	 * work normally.
	 */
	if (reg == 0)
		return RT9478_IAICR_CODE0_OFFSET;
	else
		return REG_TO_CHARGING_CURRENT_IN(reg *
						  RT9478_AICR_CURRENT_STEP_MA);
}
