...
static void bd9995x_init(void)
{
	...
	/* Reflect 1200 kHz DC-DC switching frequency */
	dcdc_freq = 3;
...
{
	if (chop_mode == mode_auto)
		chop_all = 0;
	if (dcdc_mode == mode_auto)
		dcdc_freq = 3;
}
DECLARE_HOOK(HOOK_CHIPSET_STARTUP, bd99995x_chopper_mode_enable,
	     HOOK_PRIO_DEFAULT);
DECLARE_HOOK(HOOK_CHIPSET_RESUME, bd99995x_chopper_mode_enable,
	     HOOK_PRIO_DEFAULT);
...
DECLARE_HOOK(HOOK_CHIPSET_SUSPEND, bd99995x_chopper_mode_disable,
	     HOOK_PRIO_DEFAULT);
DECLARE_HOOK(HOOK_CHIPSET_SHUTDOWN, bd99995x_chopper_mode_disable,
	     HOOK_PRIO_DEFAULT);
