...
int charger_set_current(int current)
{
	int rv;
	int rechg_vtg;
	static int prev_batt_flags = -1;
	int new_batt_flags = charger_current_battery_params()->flags;
	...
	/* Re-charge Battery Voltage Settings */
	if (new_batt_flags & BATT_FLAG_RESPONSIVE &&
		new_batt_flags != prev_batt_flags) {
		if (new_batt_flags & BATT_FLAG_WANT_CHARGE)
			rechg_vtg = battery_get_info()->voltage_max;
		else {
			rv = ch_raw_read16(BD9995X_CMD_VBAT_VAL,
					   &rechg_vtg,
					   BD9995X_EXTENDED_COMMAND);
			if (rv)
				return rv;
		...
		rv = ch_raw_write16(BD9995X_CMD_VRECHG_SET,
				    rechg_vtg & 0x7FC0,
				    BD9995X_EXTENDED_COMMAND);
		if (rv)
			return rv;

		prev_batt_flags = new_batt_flags;
	}
