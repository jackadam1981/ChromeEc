...
static int bd9995x_set_current(int chgnum, int current);
static int bd9995x_set_voltage(int chgnum, int voltage);
...
static inline int ch_raw_read16(int chgnum, int cmd, int *param,
...
static inline int ch_raw_write16(int chgnum, int cmd, int param,
					enum bd9995x_command map_cmd)
{
	int rv;
...
static int bd9995x_set_input_current(int chgnum, int input_current)
{
	int rv;
...
static int bd9995x_get_input_current(int chgnum, int *input_current)
{
	return ch_raw_read16(chgnum, BD9995X_CMD_CUR_ILIM_VAL, input_current,
			     BD9995X_EXTENDED_COMMAND);
}

static int bd9995x_manufacturer_id(int chgnum, int *id)
{
	return EC_ERROR_UNIMPLEMENTED;
}

static int bd9995x_device_id(int chgnum, int *id)
{
	return ch_raw_read16(chgnum, BD9995X_CMD_CHIP_ID, id,
			     BD9995X_EXTENDED_COMMAND);
}

static int bd9995x_get_option(int chgnum, int *option)
{
	int rv;
	int reg;
...
static int bd9995x_set_option(int chgnum, int option)
{
	int rv;
...
static const struct charger_info *bd9995x_get_info(void)
{
	return &bd9995x_charger_info;
}

static int bd9995x_get_status(int chgnum, int *status)
{
	int rv;
	int reg;
	int ch_status;
...
static int bd9995x_set_mode(int chgnum, int mode)
{
	int rv;
...
static int bd9995x_get_current(int chgnum, int *current)
{
...
static int bd9995x_get_voltage(int chgnum, int *voltage)
{
	if (vsys_priority) {
...
static void bd9995x_init(void)
{
	 ...
	 */
	if (ch_raw_read16(0, BD9995X_CMD_CHGOP_SET1, &reg,
			  BD9995X_EXTENDED_COMMAND))
		return;
	...
	ch_raw_write16(0, BD9995X_CMD_CHGOP_SET1, reg,
		       BD9995X_EXTENDED_COMMAND);
	 ...
	 */
	bd9995x_set_vsysreg(0, battery_get_info()->voltage_max +
			    BD9995X_VSYS_PRECHARGE_OFFSET_MV);

	/* Enable BC1.2 USB charging and DC/DC converter @ 1200KHz */
	if (ch_raw_read16(0, BD9995X_CMD_CHGOP_SET2, &reg,
			  BD9995X_EXTENDED_COMMAND))
		return;
	...
	reg |= BD9995X_CMD_CHGOP_SET2_CHG_EN;
#endif
	ch_raw_write16(0, BD9995X_CMD_CHGOP_SET2, reg,
		       BD9995X_EXTENDED_COMMAND);
	 ...
	 */
	if (ch_raw_read16(0, BD9995X_CMD_VM_CTRL_SET, &reg,
			  BD9995X_EXTENDED_COMMAND))
		return;
	reg &= ~BD9995X_CMD_VM_CTRL_SET_EXTIADPEN;
	ch_raw_write16(0, BD9995X_CMD_VM_CTRL_SET, reg,
		       BD9995X_EXTENDED_COMMAND);
	/*
	 ...
	 */
	if (ch_raw_read16(0, BD9995X_CMD_VIN_CTRL_SET, &reg,
			  BD9995X_EXTENDED_COMMAND))
		return;
	reg |= BD9995X_CMD_VIN_CTRL_SET_VSYS_PRIORITY;
	ch_raw_write16(0, BD9995X_CMD_VIN_CTRL_SET, reg,
		       BD9995X_EXTENDED_COMMAND);
	/* Mirror the state of this bit */
	vsys_priority = 1;

	/* Define battery charging profile */
	bd9995x_battery_charging_profile_settings(0);
	...
	ch_raw_write16(0, BD9995X_CMD_VBUS_TH_SET,
		       BD9995X_VBUS_DISCHARGE_TH, BD9995X_EXTENDED_COMMAND);
	ch_raw_write16(0, BD9995X_CMD_VCC_TH_SET,
		       BD9995X_VBUS_DISCHARGE_TH, BD9995X_EXTENDED_COMMAND);
#endif

	/* Unlock debug regs */
	ch_raw_write16(0, BD9995X_CMD_PROTECT_SET, 0x3c,
		       BD9995X_EXTENDED_COMMAND);

	/* Undocumented - reverse current threshold = -50mV */
	ch_raw_write16(0, 0x14, 0x0202, BD9995X_DEBUG_COMMAND);
	/* Undocumented - internal gain = 2x */
	ch_raw_write16(0, 0x1a, 0x80, BD9995X_DEBUG_COMMAND);

	/* Re-lock debug regs */
	ch_raw_write16(0, BD9995X_CMD_PROTECT_SET, 0x0,
		       BD9995X_EXTENDED_COMMAND);
}
DECLARE_HOOK(HOOK_INIT, bd9995x_init, HOOK_PRIO_INIT_EXTPOWER);

static int bd9995x_post_init(int chgnum)
{
	return EC_SUCCESS;
}

static int bd9995x_discharge_on_ac(int chgnum, int enable)
{
	int rv;
	int reg;
...
int bd9995x_is_vbus_provided(enum bd9995x_charge_port port)
{
	...
	if (ch_raw_read16(0, BD9995X_CMD_VBUS_VCC_STATUS, &reg,
			  BD9995X_EXTENDED_COMMAND))
		return 0;
...
#endif
{
	...
	mutex_lock(&bd9995x_vin_mutex);
	rv = ch_raw_read16(0, BD9995X_CMD_VIN_CTRL_SET, &reg,
			   BD9995X_EXTENDED_COMMAND);
	if (rv)
	...
	rv = ch_raw_write16(0, BD9995X_CMD_VIN_CTRL_SET, reg,
			      BD9995X_EXTENDED_COMMAND);
...
int bd9995x_get_battery_temp(int *temp_ptr)
{
	...
	rv = ch_raw_read16(0, BD9995X_CMD_THERM_VAL, temp_ptr,
			BD9995X_EXTENDED_COMMAND);
	if (rv)
		return rv;
...
{
	ch_raw_write16(0, BD9995X_CMD_SMBREG, mode, BD9995X_EXTENDED_COMMAND);
}
...
int bd9995x_get_battery_voltage(void)
{
	...
	rv = ch_raw_read16(0, BD9995X_CMD_VBAT_VAL, &vbat_val,
			BD9995X_EXTENDED_COMMAND);
...
int bd9995x_bc12_enable_charging(int port, int enable)
{
	 ...
	 * for USB-C.
	 */
	rv = ch_raw_read16(0, BD9995X_CMD_CHGOP_SET1, &reg,
			BD9995X_EXTENDED_COMMAND);
	if (rv)
		return rv;
	...
	return ch_raw_write16(0, BD9995X_CMD_CHGOP_SET1, reg,
			BD9995X_EXTENDED_COMMAND);
}
...
void usb_charger_set_switches(int port, enum usb_switch setting)
{
	...
	bd9995x_update_ucd_set_reg(0, port, BD9995X_CMD_UCD_SET_USB_SW,
...
void usb_charger_task(void *u)
{
	...
	for (port = 0; port < board_get_usb_pd_port_count(); port++) {
		bc12_detected_type[port] = CHARGE_SUPPLIER_NONE;
		bd9995x_enable_vbus_detect_interrupts(0, port, 1);
	...
	while (1) {
		...
		for (port = 0; port < board_get_usb_pd_port_count(); port++) {
			/* Get port interrupts */
			interrupts = bd9995x_get_interrupts(0, port);
			if (interrupts & BD9995X_CMD_INT_VBUS_DET ||
				 ...
				 */
				bc12_det_mark[port] = usb_charger_process(0,
									  port)
 ...
 */
	if (select_input_port_update) {
		...
		case STABLE:
			...
			break;
		case DEBOUNCE:
			if (bd9995x_vbus_debounce(0, port_update) ||
...
static int console_command_bd9995x(int argc, char **argv)
{
	...
	if (rw == 'r')
		rv = ch_raw_read16(0, reg, &data, cmd);
	else {
		val = strtoi(argv[4], &e, 16);
		if (*e || val < 0)
			return EC_ERROR_PARAM4;

		rv = ch_raw_write16(0, reg, val, cmd);
		if (rv == EC_SUCCESS)
			rv = ch_raw_read16(0, reg, &data, cmd);
	}
...
static int console_command_psys(int argc, char **argv)
{
	...
	rv = bd9995x_enable_psys(0);
	if (rv)
		return rv;

	CPRINTS("PSYS from chg_adc: %d mW",
			bd9995x_psys_charger_adc(0));
...
static int console_command_amon_bmon(int argc, char **argv)
{
	...
	if (argc == 1 || (argc >= 2 && argv[1][0] == 'a'))
		rv = bd9995x_amon_bmon(0, 1);

	/* Switch to BMON */
	if (argc == 1 || (argc >= 2 && argv[1][0] == 'b'))
		rv = bd9995x_amon_bmon(0, 0);
...
{
	return ch_raw_read16(0, reg, data, BD9995X_EXTENDED_COMMAND);
}

static int bd9995x_i2c_write(const int reg, int data)
{
	return ch_raw_write16(0, reg, data, BD9995X_EXTENDED_COMMAND);
}
