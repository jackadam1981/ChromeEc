...
/* Charge port is selected when VBUS is at least 9V */
#define SELECT_INPUT_MIN_VOLTAGE 9000
/* Sample VBUS every 100mS before charge port is selected */
#define SELECT_INPUT_POLL_TIME (100*MSEC)
/* Charge port is also selected if VBUS is not aleast 10V within 1s */
#define SELECT_INPUT_TIMEOUT (10*SELECT_INPUT_POLL_TIME)
...
void usb_charger_task(void *u)
{
	...
	while (1) {
		...
		if (select_input_port_update) {
			...
			/*
			 * Wait until VBUS is greater than
			 * SELECT_INPUT_MIN_VOLTAGE or timeout occurs.
			 */
