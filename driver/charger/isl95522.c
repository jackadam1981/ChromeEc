...
static K_MUTEX_DEFINE(control3_mutex_isl95522);
...
static inline enum ec_error_list isl95522_read(int chgnum, int offset,
					      int *value)
{
...
static inline enum ec_error_list isl95522_write(int chgnum, int offset,
					       int value)
{
...
static inline enum ec_error_list isl95522_update(int chgnum, int offset,
						uint16_t mask,
 ...
 */
static enum ec_error_list isl95522_discharge_on_ac_unsafe(int chgnum, int enable)
{
...
int isl95522_set_dc_prochot(int chgnum, int ma)
{
	...
	if (ma > ISL95522_DC_PROCHOT_CURRENT_MAX)
		ma = ISL95522_DC_PROCHOT_CURRENT_MAX;
	else if (ma < ISL95522_DC_PROCHOT_CURRENT_MIN)
		ma = ISL95522_DC_PROCHOT_CURRENT_MIN;

	rv = isl95522_write(chgnum, ISL95522_REG_DC_PROCHOT, ma);
	if (rv)
...
static void isl95522_init(int chgnum)
{
	...
	mutex_lock(&control3_mutex_isl95522);
	/*
	...
	mutex_unlock(&control3_mutex_isl95522);
