...
#include "driver/accelgyro_icm40608.h"

#define CPUTS(outstr) cputs(CC_ACCEL, outstr)
#define CPRINTF(format, args...) cprintf(CC_ACCEL, format, ## args)
#define CPRINTS(format, args...) cprints(CC_ACCEL, format, ## args)

#define ICM40608_I2C_ADDRESS(_addr) ((_addr) << 1)

static int icm_bank_sel(const struct motion_sensor_t *s, const int reg)
{
	struct icm_drv_data_t *st = ICM_GET_DATA(s);
	uint8_t bank = ICM40608_REG_GET_BANK(reg);
	int ret;

	if (bank == st->bank)
		return EC_SUCCESS;

	ret = i2c_write8(s->port, ICM40608_I2C_ADDRESS(s->addr), ICM40608_REG_BANK_SEL,
		bank);
	if (ret == EC_SUCCESS)
		st->bank = bank;
...
{
	const uint8_t addr = ICM40608_REG_GET_ADDR(reg);
	int ret;
	...
	return i2c_read8(s->port, ICM40608_I2C_ADDRESS(s->addr), addr,
			data_ptr);
}
...
int icm_write8(const struct motion_sensor_t *s, const int reg, int data)
{
	...
	return i2c_write8(s->port, ICM40608_I2C_ADDRESS(s->addr), addr, data);
}
...
int icm_read16(const struct motion_sensor_t *s, const int reg, int *data_ptr)
{
	...
	return i2c_read16(s->port, ICM40608_I2C_ADDRESS(s->addr), addr,
			data_ptr);
}
...
int icm_write16(const struct motion_sensor_t *s, const int reg, int data)
{
	...
	return i2c_write16(s->port, ICM40608_I2C_ADDRESS(s->addr), addr, data);
}
	       ...
	       uint8_t *data_ptr, const int len)
{
	...
	return i2c_read_block(s->port, ICM40608_I2C_ADDRESS(s->addr), addr,
		data_ptr, len);
}

static int i2c_field_update8(const int port,
		const uint16_t second_addr_flags,
		const int offset,
		...
		const uint8_t set_value)
{
	...
	rv = i2c_read8(port, second_addr_flags, offset, &read_val);
	if (rv)
		return rv;
	...
	return i2c_write8(port, second_addr_flags, offset, write_val);
}
		...
		const uint8_t field_mask, const uint8_t set_value)
{
	...
	return i2c_field_update8(s->port, ICM40608_I2C_ADDRESS(s->addr), addr,
