...
#define ICM426XX_I2C_ADDRESS(_addr) ((_addr) << 1)
...
static int icm_bank_sel(const struct motion_sensor_t *s, const int reg)
{
	...
	if (SLAVE_IS_SPI(s->addr)) {
#ifdef CONFIG_SPI_ACCEL_PORT
		ret = icm_spi_raw_write(
				SLAVE_GET_SPI_ADDR(s->addr),
				ICM426XX_REG_BANK_SEL, &bank, 1);
#endif
	} else {
#ifdef I2C_PORT_ACCEL
		ret = i2c_write8(s->port, ICM426XX_I2C_ADDRESS(s->addr),
...
#ifdef I2C_PORT_ACCEL
		ret = i2c_read8(s->port, ICM426XX_I2C_ADDRESS(s->addr),
				addr, data_ptr);
#endif
	}
	return ret;
...
#ifdef I2C_PORT_ACCEL
		ret = i2c_read16(s->port, ICM426XX_I2C_ADDRESS(s->addr),
				addr, data_ptr);
#endif
	}
	return ret;
...
#ifdef I2C_PORT_ACCEL
		ret = i2c_write16(s->port, ICM426XX_I2C_ADDRESS(s->addr),
				  addr, data);
#endif
	}
	return ret;
...
#ifdef I2C_PORT_ACCEL
		ret = i2c_read_block(s->port, ICM426XX_I2C_ADDRESS(s->addr),
				addr, data_ptr, len);
#endif
	}
	return ret;
...
#ifdef I2C_PORT_ACCEL
		ret = i2c_field_update8(s->port, ICM426XX_I2C_ADDRESS(s->addr),
					addr, field_mask, set_value);
#endif
	}
	return ret;
}
