...
{
	const struct icm_param_pair *prev, *curr;
	unsigned int i;
	...
	for (i = 0; i < len; ++i) {
		...
		if (curr->val == eng_val)
			return curr;
		else if (curr->val > eng_val)
			return round_up ? curr : prev;
		prev = curr;
	}
...
#define ICM_CHECKED_ASSIGN(_p, _v)		\
	do {					\
		if ((_p) != NULL)		\
			*(_p) = (_v);		\
	} while (0)
			       ...
			       const void **timestamp, unsigned int *odr)
{
	...
	if (header & ICM_FIFO_HEADER_MSG) {
		ICM_CHECKED_ASSIGN(accel, NULL);
		ICM_CHECKED_ASSIGN(gyro, NULL);
		ICM_CHECKED_ASSIGN(temp, NULL);
		ICM_CHECKED_ASSIGN(timestamp, NULL);
		ICM_CHECKED_ASSIGN(odr, 0);
		return 0;
	}
	...
	if (odr != NULL) {
		*odr = 0;
		if (header & ICM_FIFO_HEADER_ODR_ACCEL)
			*odr |= BIT(MOTIONSENSE_TYPE_ACCEL);
		if (header & ICM_FIFO_HEADER_ODR_GYRO)
			*odr |= BIT(MOTIONSENSE_TYPE_GYRO);
	}

	/* accel + gyro */
	if ((header & ICM_FIFO_HEADER_ACCEL) &&
	    (header & ICM_FIFO_HEADER_GYRO)) {
		ICM_CHECKED_ASSIGN(accel, &pack2->accel);
		ICM_CHECKED_ASSIGN(gyro, &pack2->gyro);
		ICM_CHECKED_ASSIGN(temp, &pack2->temp);
		ICM_CHECKED_ASSIGN(timestamp, &pack2->timestamp);
		return ICM_FIFO_2SENSORS_PACKET_SIZE;
	}

	/* accel only */
	if (header & ICM_FIFO_HEADER_ACCEL) {
		ICM_CHECKED_ASSIGN(accel, &pack1->data);
		ICM_CHECKED_ASSIGN(gyro, NULL);
		ICM_CHECKED_ASSIGN(temp, &pack1->temp);
		ICM_CHECKED_ASSIGN(timestamp, NULL);
	...
	if (header & ICM_FIFO_HEADER_GYRO) {
		ICM_CHECKED_ASSIGN(accel, NULL);
		ICM_CHECKED_ASSIGN(gyro, &pack1->data);
		ICM_CHECKED_ASSIGN(temp, &pack1->temp);
		ICM_CHECKED_ASSIGN(timestamp, NULL);
		return ICM_FIFO_1SENSOR_PACKET_SIZE;
	}
