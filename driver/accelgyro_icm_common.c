...
#define ICM40608_I2C_ADDRESS(_addr)   (_addr)
...
static int icm_bank_sel(const struct motion_sensor_t *s, const int reg)
{
	...
	ret = i2c_write8(s->port, ICM40608_ADDR0_FLAGS,
			 ICM40608_REG_BANK_SEL, bank);
...
int icm_read16(const struct motion_sensor_t *s, const int reg, int *data_ptr)
{
	...
	return i2c_read16(s->port, ICM40608_ADDR0_FLAGS, addr, data_ptr);
}
...
int icm_write16(const struct motion_sensor_t *s, const int reg, int data)
{
	...
	return i2c_write16(s->port, ICM40608_ADDR0_FLAGS, addr, data);
}
	       ...
	       uint8_t *data_ptr, const int len)
{
	...
	return i2c_read_block(s->port, ICM40608_ADDR0_FLAGS, addr, data_ptr,
			len);
}
		...
		const uint8_t field_mask, const uint8_t set_value)
{
	...
	return i2c_field_update8(s->port, ICM40608_ADDR0_FLAGS, addr,
