...
static uint8_t fifo_base[FIFO_READ_LEN];
#endif /* CONFIG_ACCEL_FIFO */

#ifdef CONFIG_ACCEL_LIS2DH_MULTI
/* Add private data struct for LID accelerometer instance */
struct stprivate_data lis2dh_l_data;
#ifdef CONFIG_ACCEL_FIFO
static uint8_t fifo_lid[FIFO_READ_LEN];
#endif /* CONFIG_ACCEL_FIFO */
...
static int load_fifo(struct motion_sensor_t *s)
{
	...
	do {
		...
		ret = raw_read_n(s->port, s->addr, LIS2DH_OUT_X_L_ADDR, drvdata->fifo,
				 nsamples, AUTO_INC);
		if (ret != EC_SUCCESS)
			return ret;
...
#ifdef CONFIG_ACCEL_FIFO
	if (s->location == MOTIONSENSE_LOC_BASE)
		/* Initialize fifo for BASE Acc */
		data->fifo = fifo_base;

#ifdef CONFIG_ACCEL_LIS2DH_MULTI
	else
		/* Initialize fifo for LID Acc */
		data->fifo = fifo_lid;
#endif /* CONFIG_ACCEL_LIS2DH_MULTI */
#endif /* CONFIG_ACCEL_FIFO */
