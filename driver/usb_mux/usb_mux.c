...
void usb_mux_init(int port)
{
	...
	if (IS_ENABLED(CONFIG_USBC_PD_RETIMER_MUX)) {
		if (usb_retimers[port].driver->init) {
			res = usb_retimers[port].driver->init(port);
			if (res) {
		 ...
		 enum usb_switch usb_mode, int polarity)
{
	...
	if (IS_ENABLED(CONFIG_USBC_PD_RETIMER_MUX)) {
		if (usb_retimers[port].driver->set) {
			res = usb_retimers[port].driver->set(port, mux_state);
			if (res) {
...
static enum ec_status hc_usb_pd_mux_info(struct host_cmd_handler_args *args)
{
	...
	if (IS_ENABLED(CONFIG_USB_MUX_VIRTUAL))
		if ((r->flags & USB_PD_MUX_HPD_IRQ) &&
			...
			mux->hpd_update(port, r->flags & USB_PD_MUX_HPD_LVL,
					0);
