...
#define USB_MUX_FLAG_EXIT_LPM		BIT(3)
...
void usb_mux_set_in_exit_lpm(int port)
{
	if (IS_ENABLED(CONFIG_USB_MUX_VIRTUAL))
		atomic_or(&flags[port], USB_MUX_FLAG_EXIT_LPM);
}
			 ...
			 mux_state_t *mux_state)
{
	...
	for (mux_ptr = &usb_muxes[port];
		...
		case USB_MUX_SET_MODE:
			...
			if (drv && drv->set) {
				...
				if (rv &&
				    !(flags[port] & USB_MUX_FLAG_EXIT_LPM))
					break;
			}
	...
	atomic_clear_bits(&flags[port], USB_MUX_FLAG_EXIT_LPM);
