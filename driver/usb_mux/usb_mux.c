...
/* Need to send host event to the AP */
			 ...
			 mux_state_t *mux_state)
{
	...
	if ((config == USB_MUX_SET_MODE && *mux_state == USB_PD_MUX_NONE) ||
	     config == USB_MUX_INIT &&
	     !(flags[port], USB_MUX_FLAG_CLEARED_HOST_EVENT)) {
...
{
	for (i < board_get_usb_pd_port_count()) {
		usb_mux_set_send_host_event_flag(i, false);
		atomic_or(&flags[port], USB_MUX_FLAG_CLEARED_HOST_EVENT);
	}
}
