...
/* Configure the MUX */
static int configure_mux(struct usb_mux *this,
			 enum mux_config_type config,
			 mux_state_t *mux_state)
{
	struct usb_mux *mux_ptr;
	int rv = EC_SUCCESS;
	...
	switch (config) {
	case USB_MUX_INIT:
		for (mux_ptr = this; mux_ptr; mux_ptr = mux_ptr->next_mux) {
	...
	case USB_MUX_SET_MODE:
		for (mux_ptr = this; mux_ptr; mux_ptr = mux_ptr->next_mux) {
			...
			if (drv && drv->set) {
				...
				state = *mux_state & ~mux_ptr->exclude_state;
...
{
	mux_state_t mux_state;
	struct usb_mux *this = usb_muxes[port];

	exit_low_power_mode(port);

	if (configure_mux(this, USB_MUX_GET_MODE, &mux_state))
		return;
	...
	configure_mux(this, USB_MUX_SET_MODE, &mux_state);
}
...
void usb_mux_hpd_update(int port, int hpd_lvl, int hpd_irq)
{
	...
	if (this->hpd_update)
		this->hpd_update(this, hpd_lvl, hpd_irq);

	if (!configure_mux(this, USB_MUX_GET_MODE, &mux_state)) {
		mux_state |= (hpd_lvl ? USB_PD_MUX_HPD_LVL : 0) |
			     (hpd_irq ? USB_PD_MUX_HPD_IRQ : 0);
		configure_mux(this->next_mux, USB_MUX_SET_MODE, &mux_state);
	}
}
...
static enum ec_status hc_usb_pd_mux_info(struct host_cmd_handler_args *args)
{
	...
	if (IS_ENABLED(CONFIG_USB_MUX_VIRTUAL) &&
	    ...
	    (this->hpd_update == &virtual_hpd_update)) {
