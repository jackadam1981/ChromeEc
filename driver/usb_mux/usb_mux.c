...
/* Default function: HSL & SBU follow CC line orientation */
__overridable void board_get_sbu_hsl_orientation(int port, uint16_t *flags)
{
...
static enum ec_status hc_usb_pd_mux_info(struct host_cmd_handler_args *args)
{
	...
	if (args->version) {
		board_get_sbu_hsl_orientation(port, &r1->flags);
