...
static inline int retimer_read_write(char rw, const struct usb_mux *mux,
				     uint32_t reg, uint32_t val)
{
	...
	if (rw == 'r') {
		rv = mux->driver->retimer_console_cmd(mux, reg, &data, true);
	} else {
		rv = mux->driver->retimer_console_cmd(mux, reg, &val, false);
		if (rv == EC_SUCCESS) {
			rv = mux->driver->retimer_console_cmd(mux, reg, &data,
							      true);
	...
	if (rv == EC_SUCCESS)
		CPRINTS("Addr 0x%x register %d = 0x%x", mux->i2c_addr_flags,
			reg, data);
	return rv;
}
...
static int console_command_retimer(int argc, const char **argv)
{
	...
	mux = mux_chain->mux;
	if (!mux || !mux->driver || !mux->driver->retimer_console_cmd)
		return EC_ERROR_PARAM1;
	...
	for (; mux_chain != NULL; mux_chain = mux_chain->next) {
		mux = mux_chain->mux;
		if (mux->driver && mux->driver->retimer_console_cmd)
			rv = retimer_read_write(rw, mux, reg, val);
	}
