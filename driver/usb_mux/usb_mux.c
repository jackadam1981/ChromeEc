					    ...
					    int polarity)
{
	...
	if ((mux_queue.head - mux_queue.tail) < MUX_QUEUE_DEPTH) {
		int new = mux_queue.head & MUX_QUEUE_DEPTH_MASK;

		mux_queue.entries[new].port = port;
		mux_queue.entries[new].type = type;
		mux_queue.entries[new].mux_mode = mux_mode;
...
{
	while (1) {
		struct mux_queue_entry const *next;

		if (mux_queue.head == mux_queue.tail) {
			task_wait_event(-1);
		} else {
			/*
...
bool usb_mux_set_completed(int port)
{
	...
	for (i = mux_queue.tail; i < mux_queue.head; i++) {
		struct mux_queue_entry const *check =
...
void usb_mux_hpd_update(int port, int hpd_lvl, int hpd_irq)
{
	...
	if (IS_ENABLED(HAS_TASK_USB_MUX))
		mux_task_enqueue(port, USB_MUX_HPD_UPDATE, mux_state,
				 0, 0);
	else
		perform_mux_hpd_update(port, mux_state);
}
