...
static void anx7440_power_handle(int port, int level)
{
	const struct usb_mux *mux = &usb_muxes[port];

	gpio_set_level(mux->usb_ls_en_gpio, level);
}
...
static int anx7440_low_power_mode(int port)
{
	anx7440_power_handle(port, 0);
	return EC_SUCCESS;
}
...
const struct usb_mux_driver anx7440_usb_mux_driver = {
	...
	.set = anx7440_set_mux,
	.get = anx7440_get_mux,
	.enter_low_power_mode = anx7440_low_power_mode,
};
