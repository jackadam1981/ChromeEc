					    ...
					    bool *ack_required)
{
	...
	*ack_required = false;
	 ...
	 */
	cprints("[virtual_mux_update_state] mux_state: %d, previous_mux_state: %d\n", mux_state, previous_mux_state);
	...
	if (!IS_ENABLED(CONFIG_HOSTCMD_EVENTS))
		cprints("[virtual_mux_update_state] !IS_ENABLED(CONFIG_HOSTCMD_EVENTS)\n");
		return;
...
static int virtual_set_mux(const struct usb_mux *me, mux_state_t mux_state,
			   bool *ack_required)
{
	cprints("[virtual_set_mux] virtual_set_mux: %d\n", mux_state);
	...
	cprints("[virtual_set_mux] new_mux_state: %d\n", new_mux_state);
