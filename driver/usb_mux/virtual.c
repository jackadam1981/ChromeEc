...
static int virtual_set_mux(const struct usb_mux *me, mux_state_t mux_state,
			   bool *ack_required)
{
	cprint("[virtual_set_mux] virtual_set_mux: %d\n", mux_state);
	...
	cprint("[virtual_set_mux] new_mux_state: %d\n", new_mux_state);
