...
#define USB_PD_MUX_ACK_STATE	(USB_PD_MUX_USB_ENABLED | \
...
static inline void virtual_mux_update_state(int port, mux_state_t mux_state)
{
	 ...
	 * disconnect and for entering and exiting the safe state. This is
	 * needed to remove timing senstivity between BB retimer and TCSS Mux
	 * to allow better synchronization between them and thereby remain in
	 * the same state for achieving proper safe state terminations.
	 *
	...
	if ((previous_mux_state != mux_state) &&
	    mux_state & USB_PD_MUX_ACK_STATE) {
