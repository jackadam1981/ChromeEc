...
static inline void virtual_mux_update_state(int port, mux_state_t mux_state)
{
	if (virtual_mux_state[port] != mux_state) {
		...
		if (IS_ENABLED(CONFIG_USBC_MUX_RETIMER)) {
			...
			if (retimer->driver &&
			    retimer->driver->set &&
			    retimer->driver->set(port, mux_state))
				return;
		}
...
static int virtual_init(int port)
{
	if (IS_ENABLED(CONFIG_USBC_MUX_RETIMER)) {
		...
		if (retimer->driver &&
		    retimer->driver->init)
			return retimer->driver->init(port);
	}

	return EC_SUCCESS;
}

static __maybe_unused int virtual_low_power_mode(int port)
{
	if (IS_ENABLED(CONFIG_USBC_MUX_RETIMER)) {
		const struct usb_retimer *retimer = &usb_retimers[port];

		if (retimer->driver &&
		    retimer->driver->enter_low_power_mode)
			return retimer->driver->enter_low_power_mode(port);
	}
...
const struct usb_mux_driver virtual_usb_mux_driver = {
	...
	.enter_low_power_mode = virtual_low_power_mode,
#endif
};
