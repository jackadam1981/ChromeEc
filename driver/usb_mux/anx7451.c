...
{
	/*
	 * Disabling USB and DP will power off mux.
	 * The mux will not send an acknowledgment in this case, so ignore
	 * response and always return success.
	 */
	anx7451_write(me, ANX7451_REG_ULP_CFG_MODE, 0x0);
	return EC_SUCCESS;
}
...
static int anx7451_wake_up(const struct usb_mux *me)
{
	...
	/* Keep reading top register until mux wakes up */
	start = get_time();
	do {
