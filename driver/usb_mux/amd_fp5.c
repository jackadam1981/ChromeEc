...
static inline int amd_fp5_mux_read(struct usb_mux *this, uint8_t *val)
{
	uint8_t buf[3] = { 0 };
	int rv;
	...
	*val = buf[this->usb_port + 1];

	return EC_SUCCESS;
}

static inline int amd_fp5_mux_write(struct usb_mux *this, uint8_t val)
{
	return i2c_write8(I2C_PORT_USB_MUX, AMD_FP5_MUX_I2C_ADDR_FLAGS,
			  this->usb_port, val);
}

static int amd_fp5_init(struct usb_mux *this)
{
	return EC_SUCCESS;
}

static int amd_fp5_set_mux(struct usb_mux *this, mux_state_t mux_state)
{
	uint8_t val = 0;
	...
	return amd_fp5_mux_write(this, val);
}

static int amd_fp5_get_mux(struct usb_mux *this, mux_state_t *mux_state)
{
