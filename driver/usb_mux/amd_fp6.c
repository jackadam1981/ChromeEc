...
static int amd_fp6_mux_read(const struct usb_mux *me, uint8_t *val)
{
	uint8_t payload[3] = { 0 };
	int rv;
	bool mux_ready;
	...
	/*
	 * payload[0]: Status/ID
	 * payload[1]: Index 0 Control/Status
	 * payload[2]: Index 1 Control/Status (unused on FP6)
	 */
	mux_ready = !!(payload[0] & AMD_FP6_MUX_STATUS_READY);
	if (!mux_ready)
...
static int amd_fp6_mux_write(const struct usb_mux *me, uint8_t val)
{
	if (chipset_in_state(CHIPSET_STATE_HARD_OFF))
		return EC_ERROR_NOT_POWERED;
