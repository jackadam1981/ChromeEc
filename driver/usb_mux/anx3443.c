...
static bool anx3443_port_is_usb_only(int port)
{
	return (saved_mux_state[port] & USB_PD_MUX_USB_ENABLED) &&
		!(saved_mux_state[port] & USB_PD_MUX_DP_ENABLED);
}
...
static void anx3443_suspend(void)
{
	for (int i = 0; i < CONFIG_USB_PD_PORT_MAX_COUNT; i++) {
		...
		if (anx3443_port_is_usb_only(port))
			anx3443_power_off(&usb_muxes[i]);
	}
}
...
static void anx3443_resume(void)
{
	for (int i = 0; i < CONFIG_USB_PD_PORT_MAX_COUNT; i++) {
		...
		anx3443_set_mux(&usb_muxes[i], saved_mux_state[port],
				&ack_required);
	}
}
