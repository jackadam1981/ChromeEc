...
static int anx3443_init(const struct usb_mux *me)
{
	 ...
	 */
	now = get_time().val;
	if (now < ANX3443_I2C_READY_DELAY_MS*MSEC)
		usleep(ANX3443_I2C_READY_DELAY_MS*MSEC - now);
...
static int anx3443_enter_low_power_mode(const struct usb_mux *me)
{
	...
	return anx3443_write(me, ANX3443_REG_ULTRA_LOW_POWER, ANX3443_ULTRA_LOW_POWER_EN);
}
