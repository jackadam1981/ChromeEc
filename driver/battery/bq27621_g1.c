...
/* This only works for scaling factors 10, 1, & 0.1 */
static inline int unscale_param(int scaled_value)
{
	if (BQ27621_SCALE_FACTOR == 10)
		return scaled_value / 10;

	if (BQ27621_SCALE_FACTOR == 0.1) /* Avoid floating point division */
		return scaled_value * 10;
...
int battery_remaining_capacity(int *capacity)
{
	...
	*capacity = unscale_param(scaled_value);
