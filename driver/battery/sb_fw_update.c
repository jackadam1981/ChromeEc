...
/* smbus Write Word
 *   [S][Slave Address][Wr=0][A][cmd][A][DL][A][DH][A][PEC][A][P]
 */
int smbus_write16(int port, int slave_addr, int offset, int data)
{
	...
	buf[0] = (slave_addr & 0xFF); /* I2C_WRITE_ADDR 0xB << 1 */
	buf[1] = offset & 0xff;
	...
	i2c_lock(port, 1);
	rv = i2c_xfer(port, slave_addr, &buf[1], 4, NULL, 0, I2C_XFER_SINGLE);
	i2c_lock(port, 0);
...
static int ec_sb_fw_update_begin(struct host_cmd_handler_args *args)
{
	...
	if (ec_sb_fw_i2c_access_enable) {
		rv = smbus_write16(I2C_PORT_BATTERY, BATTERY_ADDR,
			SB_FW_UPDATE_CMD_WRITE_WORD,
			SB_FW_UPDATE_CMD_WRITE_WORD_PREPARE);
		if (rv)
			return EC_RES_ERROR;
...
static int ec_sb_fw_update_info(struct host_cmd_handler_args *args)
{
	...
	smbus_cmd = SB_FW_UPDATE_CMD_READ_INFO;
	rv = i2c_xfer(I2C_PORT_BATTERY, BATTERY_ADDR,
		&smbus_cmd, 1, NULL, 0, I2C_XFER_START);
	if (!rv) {
		rv = i2c_xfer(I2C_PORT_BATTERY, BATTERY_ADDR, NULL, 0,
			(uint8_t *)&resp->info,
			SB_FW_UPDATE_CMD_READ_INFO_SIZE+1,
			I2C_XFER_SINGLE);
	}
...
static int ec_sb_fw_update_status(struct host_cmd_handler_args *args)
{
	...
	smbus_cmd = SB_FW_UPDATE_CMD_READ_STATUS;

#if 1
	rv = i2c_read16(I2C_PORT_BATTERY, BATTERY_ADDR,
		smbus_cmd, &status);
	*((uint16_t *)&resp->status) = status;
#endif
#if 0
	i2c_lock(I2C_PORT_BATTERY, 1);
	rv = i2c_xfer(I2C_PORT_BATTERY, BATTERY_ADDR,
		&smbus_cmd, 1, NULL, 0, I2C_XFER_START);
	if (!rv) {
		p = (uint8_t *)&resp->status;
		rv = i2c_xfer(I2C_PORT_BATTERY, BATTERY_ADDR, NULL, 0,
			p,
			3, /* 2 bytes status + 1 byte PEC */
			I2C_XFER_SINGLE);
		cprintf(CC_I2C, "%02X %02X sts:%02X %02X\n",
			p[0], p[1], crc8(p, 2), p[2]);
	}
	i2c_lock(I2C_PORT_BATTERY, 0);
#endif
...
static int ec_sb_fw_update_write(struct host_cmd_handler_args *args)
{
	...
	write->slave_addr = BATTERY_ADDR /* I2C_WRITE_ADDR 0xB << 1 */;

	write->smbus_cmd  = SB_FW_UPDATE_CMD_WRITE_BLOCK;

	write->data[write->size] =
		crc8(&write->slave_addr, SB_FW_UPDATE_CMD_WRITE_BLOCK_SIZE+3);
