...
static int max17055_load_ocv_table(const struct max17055_batt_profile *config)
{
	...
	while (--retries) {
		 ...
		 * monitor the capacity of the battery. Therefore, it is
		 * very critical that the ocv table is locked. To verify it
		 * is locked, simply read back the values. However, this time,
