...
static int max17055_init_config(void)
{
	...
	int hib_cfg;
	int retries = 50;
	const struct max17055_batt_params *config;

	config = max17055_get_batt_params();

	if (max17055_write(REG_DESIGN_CAPACITY, config->design_cap) ||
	    max17055_write(REG_DQACC, config->design_cap / 32) ||
	    max17055_write(REG_CHARGE_TERM_CURRENT, config->ichg_term)
	    || max17055_write(REG_EMPTY_VOLTAGE, config->v_empty))
	...
	if (max17055_write(REG_DPACC, (config->design_cap / 32) *
			   51200 / config->design_cap))
		return EC_ERROR_UNKNOWN;
