...
static int max17055_load_ocv_table(const struct max17055_batt_profile *config)
{
	...
	while(--retries) {
		...
		/*
		 * If the ocv table remains unlocked, the MAX17055 cannot monitor
		 * the capacity of the battery. Therefore, it is very critical
		 * that the ocv table is locked. To verify it is locked, simply
		 * read back the values. However, this time, all values should
		 * be read as 0x0000.
		 */
		for (i = 0; i < MAX17055_OCV_TABLE_SIZE; i++) {
			reg = 0xff;
