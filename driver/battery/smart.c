...
int battery_maximum_power(void)
{
	...
	sb_read(SB_MAX_PEAK_POWER, &power);
	return power;
}
...
int battery_sustained_power(void)
{
	...
	sb_read(SB_SUS_PEAK_POWER, &sus_power);
	return sus_power;
}
...
int battery_high_frequency_impedance(void)
{
	...
	sb_read(SB_HIGH_FREQ_IMPEDANCE, &high_freq_imp);
	return high_freq_imp;
}
...
int battery_instantaneous_no_load_volt(void)
{
	...
	sb_read(SB_NL_VOLTAGE, &nl_volt);
	return nl_volt;
}

int battery_discharge_current_capability(void)
{
	int discharge_current_cap;

	sb_read(SB_DISCHARGE_CURRENT_CAPABILITY, &discharge_current_cap);
	return discharge_current_cap;
}
...
{
	return sb_write(SB_SYS_RESISTANCE, resistance);
}
...
{
	return sb_write(SB_MIN_SYS_VOLTAGE, min_volt);
}
