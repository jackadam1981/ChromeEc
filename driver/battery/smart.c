...
static int sb_check_print_status_error(void)
{
	...
	rv = i2c_read16(I2C_PORT_BATTERY, BATTERY_ADDR_FLAGS, SB_BATTERY_STATUS,
			&status);
	if (rv && IS_ENABLED(CONFIG_SB_I2C_DEBUG)) {
		CPRINTS("%s i2c error offset:%x rv:%d", __func__,
			SB_BATTERY_STATUS, rv);
		return rv;
	}
