...
#if defined(BOARD_spring) || defined(TEST_led_spring)
/*
 * TODO(crosbug.com/p/20881): Spring's USB charging and LED logic make
 * low-level calls into the battery interface, rather than using the charger
 * interface.  The Spring code is extremely fragile and difficult to test
 * fully, so persist these old calls only for the Spring board.  Remove these
 * when Spring is deprecated.
 */
...
/* Discharge current in mA; negative = charging. */
