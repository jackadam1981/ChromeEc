...
int charger_profile_override(struct charge_state_data *curr)
{
	...
	case TEMP_NORMAL:
		if (normal_temp_phase == 0) {
			...
			/*
			 * if we have a good read of voltage and current, and
			 * they are above the threshold, go to phase next phase
			 */
			if (!(curr->batt.flags &
			      (BATT_FLAG_BAD_VOLTAGE | BATT_FLAG_BAD_CURRENT))
			    && curr->batt.current <= 4759
			    && curr->batt.voltage >= 8200)
				normal_temp_phase = 1;
		}
	...
	case TEMP_HIGH:
		...
		if (prev_temp_range != TEMP_HIGH) {
			...
			high_temp_charging_voltage = MAX(8300,
				charger_closest_voltage(curr->batt.voltage +
							info->voltage_step));
		}
