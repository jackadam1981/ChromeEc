...
{
	return ina3221_write(unit, INA3221_REG_CONFIG, config);
}

#ifdef CONFIG_CMD_INA
static void ina3221_dump(int unit)
{
	uint16_t cfg;
	...
	for (chan = 0; chan < INA3221_CHAN_COUNT; chan++) {
		sv[chan] = ina3221_chan_read(unit, chan, INA3221_SHUNT_VOLT);
		bv[chan] = ina3221_chan_read(unit, chan, INA3221_BUS_VOLT);
		crit[chan] = ina3221_chan_read(unit, chan, INA3221_CRITICAL);
		warn[chan] = ina3221_chan_read(unit, chan, INA3221_WARNING);
	}
	...
	for (chan = 0; chan < INA3221_CHAN_COUNT; chan++) {
		...
		ccprintf("      Shunt voltage: %04x => %d uV\n", sv[chan],
			 INA3221_SHUNT_UV((int)sv[chan]));
		ccprintf("      Bus voltage  : %04x => %d mV\n", bv[chan],
			 INA3221_BUS_MV((int)bv[chan]));
...
static int command_ina(int argc, char **argv)
{
	...
	if (*e || unit < 0 || unit >= ina3221_count)
