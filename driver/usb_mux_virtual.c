...
{
	/* Current USB & DP mux status + Previous HPD related mux status */
	mux_state_t new_mux_state = mux_state | (virtual_mux_state[port] &
				(USB_PD_MUX_HPD_LVL | USB_PD_MUX_HPD_IRQ));
...
{
	/* Current HPD related mux status + previous USB & DP mux status */
	mux_state_t new_mux_state = (hpd_lvl ? USB_PD_MUX_HPD_LVL : 0) |
			(hpd_irq ? USB_PD_MUX_HPD_IRQ : 0) |
			(virtual_mux_state[port] & (USB_PD_MUX_USB_ENABLED |
