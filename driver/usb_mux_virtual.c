...
static int hpd_lvl_state[CONFIG_USB_PD_PORT_COUNT];
...
static int virtual_get_mux(int port, mux_state_t *mux_state)
{
	...
	*mux_state |= hpd_lvl_state[port] ? USB_PD_MUX_HPD_IRQ : 0;
...
{
	if (hpd_lvl_state[port] != hpd_lvl || hpd_irq) {
		hpd_lvl_state[port] = hpd_lvl;
		host_set_single_event(EC_HOST_EVENT_USB_MUX);
	}
}
