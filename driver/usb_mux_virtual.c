...
{
	mux_state_t prev_mux = virtual_mux_state[port];

	if (hpd_lvl)
		virtual_mux_state[port] |= USB_PD_MUX_HPD_LVL;
	else
		virtual_mux_state[port] &= ~USB_PD_MUX_HPD_LVL;

	if (hpd_irq)
		virtual_mux_state[port] |= USB_PD_MUX_HPD_IRQ;
	else
		virtual_mux_state[port] &= ~USB_PD_MUX_HPD_IRQ;

	if (virtual_mux_state[port] != prev_mux)
		host_set_single_event(EC_HOST_EVENT_USB_MUX);
}
