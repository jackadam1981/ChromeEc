...
int ina231_write(uint8_t idx, uint8_t reg, uint16_t val)
{
	...
	res = i2c_write8(I2C_PORT_MASTER, addr, reg, val);
	if (res)
...
static void ina231_dump(uint8_t idx)
{
	...
	uint16_t sv = ina231_read(idx, INA231_REG_SHUNT_VOLT);
	uint16_t bv = ina231_read(idx, INA231_REG_BUS_VOLT);
	uint16_t pow = ina231_read(idx, INA231_REG_POWER);
	uint16_t curr = ina231_read(idx, INA231_REG_CURRENT);
	...
	ccprintf("Shunt voltage: %04x\n", sv);
	ccprintf("Bus voltage  : %04x\n", bv);
	ccprintf("Power        : %04x\n", pow);
	ccprintf("Current      : %04x\n", curr);
