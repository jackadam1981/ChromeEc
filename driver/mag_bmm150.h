...
#define BMI160_SMOOTH_ALPA(_axis) \
	 (fp_div(INT_TO_FP(1), \
		 INT_TO_FP(CONCAT3(CONFIG_MAG_, _axis, _SMOOTH_SAMPLES))))
...
#ifdef CONFIG_MAG_X_SMOOTH_SAMPLES
	DECLATE_BMI_SMOOTH_AXIS_VAR(X);
#endif
#ifdef CONFIG_MAG_Y_SMOOTH_SAMPLES
	DECLATE_BMI_SMOOTH_AXIS_VAR(Y);
#endif
#ifdef CONFIG_MAG_Z_SMOOTH_SAMPLES
