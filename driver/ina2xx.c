...
/*
 * Bit mask indicating INA chip type. Bit 'x' corresponds to chip with
 * index 'x'. If the bit is set, it's an INA231. Otherwise, INA219.
 */
static uint16_t ina_type;

#define IS_INA231(x) (ina_type & (1 << (x)))
...
int ina2xx_init(uint8_t idx, uint16_t config, uint16_t calib, int type)
{
	int res;
	uint16_t mask = 1 << idx;
	...
	ina_type = (ina_type & ~mask) | (type << mask);
...
static void ina2xx_dump(uint8_t idx)
{
	...
	int sv_uv = IS_INA231(idx) ? INA231_SHUNT_UV((int)sv) :
				     INA219_SHUNT_UV((int)sv);
	uint16_t bv = ina2xx_read(idx, INA2XX_REG_BUS_VOLT);
	int bv_mv = IS_INA231(idx) ? INA231_BUS_MV((int)bv) :
				     INA219_BUS_MV((int)bv);
	uint16_t pow = ina2xx_read(idx, INA2XX_REG_POWER);
	int pow_mw = IS_INA231(idx) ? INA231_POW_MW((int)pow) :
				      INA219_POW_MW((int)pow);
	int16_t curr = ina2xx_read(idx, INA2XX_REG_CURRENT);
