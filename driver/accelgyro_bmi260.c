 ...
 */
static int enqueue_bmi_data(const struct motion_sensor_t *s, uint8_t *data)
{
	int ret;
...
/*
 * Load the BMI configuration data from a compressed file.
 *
 * Compression scheme:
...
int bmi_compressed_config_load(const struct motion_sensor_t *s)
{
	...
	for (i = 0; i < length; i++) {
		...
		if (key == COMPRESS_KEY) {
			...
			data8 = (uint8_t *)&bmi_compressed_config[i];

			while (repeat_count-- > 0) {
				ret = enqueue_bmi_data(s, data8);
				if (ret)
					return ret;
