 ...
 */
static int enable_reg_bits(const struct motion_sensor_t *s, int reg, uint8_t bits) {
	int ret, val;
 ...
 */
static int disable_reg_bits(const struct motion_sensor_t *s, int reg, uint8_t bits) {
	int ret, val;
				...
				int rnd)
{
	...
	if (rate == 0) {
		...
		if (IS_ENABLED(CONFIG_ACCEL_FIFO))
			enable_fifo(s, 0);
		ret = disable_reg_bits(s, BMI260_PWR_CTRL, BMI260_PWR_EN(s->type));
		ret = disable_reg_bits(s, BMI260_CONF_REG(s->type), BMI260_FILTER_PERF);
		msleep(3);
		data->odr = 0;
#ifdef CONFIG_MAG_BMI260_BMM150
		if (s->type == MOTIONSENSE_TYPE_MAG)
		...
		enable_reg_bits(s, BMI260_PWR_CTRL, BMI260_PWR_EN(s->type));
		enable_reg_bits(s, BMI260_CONF_REG(s->type), BMI260_FILTER_PERF);
