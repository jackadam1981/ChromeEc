			 ...
			 const uint8_t reg, int *data_ptr)
{
	...
	raw_write8(port, i2c_spi_addr_flags,
		   BMI260_MAG_I2C_READ_ADDR, reg);
	return raw_read8(port, i2c_spi_addr_flags,
			 BMI260_MAG_I2C_READ_DATA, data_ptr);
}
...
{
	raw_write8(port, i2c_spi_addr_flags,
		   BMI260_MAG_I2C_WRITE_DATA, data);
	return raw_write8(port, i2c_spi_addr_flags,
			  BMI260_MAG_I2C_WRITE_ADDR, reg);
}
#endif
...
static int enable_fifo(const struct motion_sensor_t *s, int enable)
{
	...
	if (enable) {
		...
		ret = raw_read8(s->port, s->i2c_spi_addr_flags,
				BMI260_FIFO_CONFIG_1, &val);
		val |= BMI260_FIFO_SENSOR_EN(s->type);
				...
				int rnd)
{
	...
	if (rate == 0) {
		...
		ret = raw_write8(s->port, s->i2c_spi_addr_flags,
				 BMI260_CMD_REG,
				 BMI260_CMD_MODE_SUSPEND(s->type));
		msleep(3);
		data->odr = 0;
#ifdef CONFIG_MAG_BMI260_BMM150
		if (s->type == MOTIONSENSE_TYPE_MAG)
...
static int perform_calib(const struct motion_sensor_t *s, int enable)
{
	...
	case MOTIONSENSE_TYPE_ACCEL:
		...
		if (s->rot_standard_ref == NULL ||
			...
			val = BMI260_FOC_ACC_PLUS_1G;
		else
			val = BMI260_FOC_ACC_MINUS_1G;
		val = (BMI260_FOC_ACC_0G << BMI260_FOC_ACC_X_OFFSET) |
			(BMI260_FOC_ACC_0G << BMI260_FOC_ACC_Y_OFFSET) |
			(val << BMI260_FOC_ACC_Z_OFFSET);
		en_flag = BMI260_OFFSET_ACC_EN;
		break;
	case MOTIONSENSE_TYPE_GYRO:
		val = BMI260_FOC_GYRO_EN;
		en_flag = BMI260_OFFSET_GYRO_EN;
		break;
	default:
	...
	ret = raw_write8(s->port, s->i2c_spi_addr_flags,
			 BMI260_FOC_CONF, val);
  /* TODO(henryhsu): check 260 how to start fast offset calibration */
/*  ret = raw_write8(s->port, s->i2c_spi_addr_flags,*/
/*       BMI260_CMD_REG, BMI260_CMD_START_FOC);*/
	...
	do {
		...
		if (ret != EC_SUCCESS)
			goto end_perform_calib;
	} while ((status & BMI260_FOC_RDY) == 0);

	/* Calibration is successful, and loaded, use the result */
	ret = raw_read8(s->port, s->i2c_spi_addr_flags,
			BMI260_OFFSET_EN_GYR98, &val);
	ret = raw_write8(s->port, s->i2c_spi_addr_flags,
			 BMI260_OFFSET_EN_GYR98, val | en_flag);
end_perform_calib:
		    ...
		    const struct ec_motion_sense_activity *param)
{
	...
	case MOTIONSENSE_ACTIVITY_SIG_MOTION: {
		int tmp;
		ret = raw_read8(s->port, s->addr, BMI260_INT_EN_0, &tmp);
		if (ret)
			return ret;
		if (enable) {
			...
			raw_write8(s->port, s->i2c_spi_addr_flags,
				BMI260_INT_MOTION_3,
				BMI260_MOTION_PROOF_TIME(
					CONFIG_GESTURE_SIGMO_PROOF_MS) <<
				BMI260_MOTION_PROOF_OFF |
				BMI260_MOTION_SKIP_TIME(
					CONFIG_GESTURE_SIGMO_SKIP_MS) <<
				BMI260_MOTION_SKIP_OFF |
				BMI260_MOTION_SIG_MOT_SEL);
			raw_write8(s->port, s->i2c_spi_addr_flags,
				BMI260_INT_MOTION_1,
				BMI260_MOTION_TH(s,
					CONFIG_GESTURE_SIGMO_THRES_MG));
			tmp |= BMI260_INT_ANYMO_X_EN |
				BMI260_INT_ANYMO_Y_EN |
				BMI260_INT_ANYMO_Z_EN;
		} else {
			tmp &= ~(BMI260_INT_ANYMO_X_EN |
				 BMI260_INT_ANYMO_Y_EN |
				 BMI260_INT_ANYMO_Z_EN);
...
/*       BMI260_CMD_REG, BMI260_CMD_INT_RESET);*/
...
#ifdef CONFIG_GESTURE_SENSOR_DOUBLE_TAP
  /* set 0x2F to page 1, 0x3e bit[0] to 1, bit[3:1] to 0 (high sensitive) */
  ...
  /* set 0x2F to page 2, 0x30 bit[0] to 1, bit[1] to 0, bit[3:2] to 0, other use default */
/*  if (s->location == MOTIONSENSE_LOC_LID) {*/
/*    ret = raw_write8(s->port, s->i2c_spi_addr_flags,*/
/*         BMI260_INT_ORIENT_0,*/
/*         BMI260_INT_ORIENT_0_INIT_VAL);*/
/*    ret = raw_write8(s->port, s->i2c_spi_addr_flags,*/
/*         BMI260_INT_ORIENT_1,*/
/*         BMI260_INT_ORIENT_1_INIT_VAL);*/
/*  }*/
#endif

#ifdef CONFIG_ACCELGYRO_BMI260_INT2_OUTPUT
	ret = raw_write8(s->port, s->i2c_spi_addr_flags,
			 BMI260_INT_LATCH, BMI260_LATCH_5MS);
#else
	/* Also, configure int2 as an external input. */
	ret = raw_write8(s->port, s->i2c_spi_addr_flags,
			 BMI260_INT_LATCH,
			 BMI260_INT2_INPUT_EN | BMI260_LATCH_5MS);
#endif

	/* configure int1 as an interrupt */
	ret = raw_write8(s->port, s->i2c_spi_addr_flags,
			 BMI260_INT_OUT_CTRL,
			 BMI260_INT_CTRL(1, OUTPUT_EN));

	/* Map activity interrupt to int 1 */
	tmp = 0;
#ifdef CONFIG_GESTURE_SIGMO
	tmp |= BMI260_INT_ANYMOTION;
#endif
#ifdef CONFIG_GESTURE_SENSOR_DOUBLE_TAP
	tmp |= BMI260_INT_D_TAP;
#endif
#ifdef CONFIG_BMI260_ORIENTATION_SENSOR
	/* enable orientation interrupt for lid sensor only */
	if (s->location == MOTIONSENSE_LOC_LID)
		tmp |= BMI260_INT_ORIENT;
#endif
	ret = raw_write8(s->port, s->i2c_spi_addr_flags,
			 BMI260_INT_MAP_REG(1), tmp);

	if (IS_ENABLED(CONFIG_ACCEL_FIFO)) {
		/* map fifo water mark to int 1 */
		ret = raw_write8(s->port, s->i2c_spi_addr_flags,
				 BMI260_INT_FIFO_MAP,
				 BMI260_INT_MAP(1, FWM) |
				 BMI260_INT_MAP(1, FFULL));
				...
				int interrupt)
{
	int shifted_masked_orientation =
		(interrupt >> 24) & BMI260_ORIENT_XY_MASK;
	if (BMI260_GET_DATA(s)->raw_orientation != shifted_masked_orientation) {
...
#ifdef CONFIG_GESTURE_SENSOR_DOUBLE_TAP
		if (interrupt & BMI260_D_TAP_INT)
			*event |= TASK_EVENT_MOTION_ACTIVITY_INTERRUPT(
					MOTIONSENSE_ACTIVITY_DOUBLE_TAP);
#endif
#ifdef CONFIG_GESTURE_SIGMO
		if (interrupt & BMI260_SIGMOT_INT)
			*event |= TASK_EVENT_MOTION_ACTIVITY_INTERRUPT(
...
/*         BMI260_CMD_EXT_MODE_ADDR, 0);*/
    ...
    /* TODO(henryhsu): 260 doesn't have the setting */
		raw_write8(s->port, s->i2c_spi_addr_flags,
			   BMI260_PMU_TRIGGER, 0);
	}
	...
	if (s->type == MOTIONSENSE_TYPE_MAG) {
		 ...
		 * the BMI260 magnetometer part (a pass through) in normal mode.
		 */
		raw_write8(s->port, s->i2c_spi_addr_flags,
			   BMI260_CMD_REG, BMI260_CMD_MODE_NORMAL(s->type));
		msleep(wakeup_time[s->type]);

		if ((data->flags & BMI260_FLAG_SEC_I2C_ENABLED) == 0) {
			 ...
			 * https://github.com/BoschSensortec/BMI260_driver
			 *
			 ...
			 */
			raw_read8(s->port, s->i2c_spi_addr_flags,
				  BMI260_CMD_EXT_MODE_ADDR, &ext_page_reg);
			raw_write8(s->port, s->i2c_spi_addr_flags,
				   BMI260_CMD_EXT_MODE_ADDR,
				   ext_page_reg | BMI260_CMD_TARGET_PAGE);
			raw_read8(s->port, s->i2c_spi_addr_flags,
				  BMI260_CMD_EXT_MODE_ADDR, &ext_page_reg);
			raw_write8(s->port, s->i2c_spi_addr_flags,
				   BMI260_CMD_EXT_MODE_ADDR,
				   ext_page_reg | BMI260_CMD_PAGING_EN);
			raw_read8(s->port, s->i2c_spi_addr_flags,
				  BMI260_COM_C_TRIM_ADDR, &pullup_reg);
			raw_write8(s->port, s->i2c_spi_addr_flags,
				   BMI260_COM_C_TRIM_ADDR,
				   pullup_reg | BMI260_COM_C_TRIM);
			raw_read8(s->port, s->i2c_spi_addr_flags,
				  BMI260_CMD_EXT_MODE_ADDR, &ext_page_reg);
			raw_write8(s->port, s->i2c_spi_addr_flags,
				   BMI260_CMD_EXT_MODE_ADDR,
			...
			ret = raw_write8(s->port, s->i2c_spi_addr_flags,
					 BMI260_IF_CONF,
					 BMI260_IF_MODE_AUTO_I2C <<
					     BMI260_IF_MODE_OFF);
			data->flags |= BMI260_FLAG_SEC_I2C_ENABLED;
		}


		bmi260_sec_access_ctrl(s->port, s->i2c_spi_addr_flags, 1);
		...
		raw_write8(s->port, s->i2c_spi_addr_flags,
			   BMI260_MAG_I2C_READ_ADDR, BMM150_BASE_DATA);
		/*
		 * Put back the secondary interface in normal mode.
		 * BMI260 will poll based on the configure ODR.
		 */
		bmi260_sec_access_ctrl(s->port, s->i2c_spi_addr_flags, 0);

		/*
		 * Clean interrupt event that may have occurred while the
		 * BMI260 was in management mode.
		 */
		task_set_event(TASK_ID_MOTIONSENSE,
				CONFIG_ACCELGYRO_BMI260_INT_EVENT, 0);
	}
