...
static int bmi_config_load(const struct motion_sensor_t *s)
{
	...
	if (IS_ENABLED(CONFIG_CHIP_INIT_ROM_REGION)) {
		bmi_config = (uint8_t *)init_rom_get_addr(
			(int)g_bmi260_config_tbin, g_bmi260_config_tbin_len);

		/*
		 * If the init_rom region is memory mapped, we need to
		 * lock it before reading the BMI260 config data. Otherwise,
		 * we will copy the data through a RAM buffer and we need to
		 * limit the I2C burst length to the size of the RAM buffer.
		 */
		if (bmi_config)
			init_rom_lock(true);
		else
			burst_write_len = BMI_RAM_BUFFER_SIZE;
	} else {
		bmi_config = g_bmi260_config_tbin;
	}

	/* We have to write the config even bytes of data every time */
	ASSERT((burst_write_len & 1) == 0);
	...
	for (i = 0; i < g_bmi260_config_tbin_len; i += burst_write_len) {
		...
		if (IS_ENABLED(CONFIG_CHIP_INIT_ROM_REGION) && !bmi_config) {
			/*
