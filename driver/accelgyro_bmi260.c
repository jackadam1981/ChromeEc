...
unsigned char g_bmi260_config_tbin[] = {
...
unsigned int g_bmi260_config_tbin_len = 8192;
...
/* In fast start mode the accel/gyro start-up time is 2ms*/
static int wakeup_time[] = {
	[MOTIONSENSE_TYPE_ACCEL] = 45,
 ...
 */
#if defined(CONFIG_GESTURE_SENSOR_DOUBLE_TAP) || \
	defined(CONFIG_BMI260_ORIENTATION_SENSOR)
static int raw_write16(const int port,
		      const uint16_t i2c_spi_addr_flags,
...
#endif /*CONFIG_GESTURE_SENSOR_DOUBLE_TAP || CONFIG_BMI260_ORIENTATION_SENSOR*/
 ...
 */
static int enable_reg8_bits(const struct motion_sensor_t *s,
			   int reg,
			   uint8_t bits)
{
	int ret, val;
 ...
 */
static int disable_reg8_bits(const struct motion_sensor_t *s,
			    int reg,
			    uint8_t bits)
{
	int ret, val;
...
static int enable_fifo(const struct motion_sensor_t *s, int enable)
{
	...
	if (enable) {
		...
		ret = enable_reg8_bits(s, BMI260_FIFO_CONFIG_1,
		...
		ret = disable_reg8_bits(s, BMI260_FIFO_CONFIG_1,
					BMI260_FIFO_SENSOR_EN(s->type));
		if (ret == EC_SUCCESS)
			data->flags &=
				...
				int rnd)
{
	...
	if (rate == 0) {
		...
		if (IS_ENABLED(CONFIG_ACCEL_FIFO))
			enable_fifo(s, 0);
		/* disable sensor */
		ret = disable_reg8_bits(s, BMI260_PWR_CTRL,
				BMI260_PWR_EN(s->type));
		if (s->type == MOTIONSENSE_TYPE_GYRO) {
			ret = disable_reg8_bits(s, BMI260_CONF_REG(s->type),
		...
		/* enable sensor */
		enable_reg8_bits(s, BMI260_PWR_CTRL,
				 BMI260_PWR_EN(s->type));
		if (s->type == MOTIONSENSE_TYPE_GYRO) {
			/* switch to performance mode */
			enable_reg8_bits(s, BMI260_CONF_REG(s->type),
					 BMI260_FILTER_PERF |
...
static int perform_calib(const struct motion_sensor_t *s, int enable)
{
	...
	case MOTIONSENSE_TYPE_ACCEL:
		...
		for (i = 0; i < n_sample; ++i) {
			intv3_t v;
			/* Wait data for 5 * 10 msec */
			ret = wait_and_read_data(s, v, 5, 10);
			if (ret) {
				CPRINTS("%s timeout", __func__);
				goto end_perform_calib;
			}
			/* Assume the device is laying flat for calibration */
			data_diff[X] += v[X];
			data_diff[Y] += v[Y];
			if (s->rot_standard_ref == NULL ||
				(*s->rot_standard_ref)[2][2] > INT_TO_FP(0))
				data_diff[Z] += v[Z] -
					BMI260_ACC_DATA_MINUS_1G(range);
			else
				data_diff[Z] += v[Z] -
					BMI260_ACC_DATA_PLUS_1G(range);
		}
		/* The accel data LSB: 2000 * 2^range / 32768 mg */
		for (i = X; i <= Z; ++i)
			offset[i] -= (data_diff[i] / n_sample) *
				     (2000 * (1 << range)) / 32768;
	...
	case MOTIONSENSE_TYPE_GYRO:
		...
		/* The gyro data LSB: 1000 / (16.384 * 2^range) mdps */
		for (i = X; i <= Z; ++i)
...
static int config_interrupt(const struct motion_sensor_t *s)
{
	...
	if (IS_ENABLED(CONFIG_ACCEL_FIFO)) {
		...
		ret = raw_write8(s->port, s->i2c_spi_addr_flags,
				 ...
				 BMI260_FIFO_TAG_INT1_EN_OFFSET) |
...
static void irq_set_orientation(struct motion_sensor_t *s)
{
	...
	if (BMI260_GET_DATA(s)->raw_orientation != new_orientation) {
		...
		BMI260_GET_DATA(s)->raw_orientation = new_orientation;
...
static int init_config(const struct motion_sensor_t *s)
{
	...
	raw_write8(s->port, s->i2c_spi_addr_flags, BMI260_PWR_CONF, 2);
	msleep(1);
	/* prepare for config load */
	raw_write8(s->port, s->i2c_spi_addr_flags, BMI260_INIT_CTRL, 0);
	...
	/* finish config load */
	raw_write8(s->port, s->i2c_spi_addr_flags, BMI260_INIT_CTRL, 1);
