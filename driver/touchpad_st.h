...
/*
 * A structure to maintain current system state.
 */
...
struct st_tp_system_info_t {
	...
	uint16_t chip0_ver;
	uint16_t chip0_id;  /* should be 0x3936 */
	uint16_t chip1_ver;
	uint16_t chip1_id;
	uint16_t fw_ver;
	uint16_t svn_rev;
...
#define ST_TP_SYSTEM_INFO_LEN	(sizeof(struct st_tp_system_info_t) + 16)
...
#define ST_TP_TOUCH_TYPE_PALM		0x4
			...
			unsigned x:12;
			uint8_t z;
			uint8_t minor:4;  // need to be concat with reserved_2
			uint8_t major:4;  // need to be concat with reserved
		} __packed finger;
