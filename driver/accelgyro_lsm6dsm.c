...
static int set_fifo_mode(const struct motion_sensor_t *s, enum fifo_mode fmode)
{
	...
	case BYPASS:
		...
		break;
	case CONTINUOUS:
		reg_value = LSM6DSM_FIFO_MODE_CONTINUOUS_VAL;
		enable_fifo = 1;
		break;
	default:
		return EC_ERROR_INVAL;
 ...
 * with acc/gyro
 */
static int set_fifo_params(const struct motion_sensor_t *s)
{
	int err;
			   ...
			   uint16_t flen)
{
	...
	while (fifo_offset < flen) {
		...
		for (i = FIFO_DEV_GYRO; i < FIFO_DEV_NUM; i++) {
			j = agm_maps[i];
			drvdata = (s + j)->drv_data;
			samples_in_pattern[i] = drvdata->samples_in_pattern;
			ratio[i] = drvdata->samples_in_pattern;
		}
		...
		do {
			...
			for (j = FIFO_DEV_GYRO; j < FIFO_DEV_NUM; j++) {
				...
				if (fifo_order(j, samples_in_pattern, ratio))
					continue;
 ...
 */
static int configure_fifo(const struct motion_sensor_t *s)
{
	int err, fifo_len;
	...
	if (fifo_len > 0) {
		err = set_fifo_mode(s, CONTINUOUS);
		if (err != EC_SUCCESS)
			return err;
	}
...
{
	int ret, tmp;
	const struct motion_sensor_t *ps = s - OFFSET_DEV_MAG;
...
#ifdef CONFIG_ACCEL_FIFO
		configure_fifo(s - OFFSET_DEV_MAG);
#endif /* CONFIG_ACCEL_FIFO */
