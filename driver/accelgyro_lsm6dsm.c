...
/*
 * Struct for Full Scale ranges
 * val is the range (in g or dps)
 * reg_val is value to write in Full Scale register
 */
...
/*
 * Table for Full Scale Ranges
 * It's not easy to find an interpolation formula
 * so we use a table for Acc and Gyro
 */
static const struct lsm6dsm_fs_reg a_ranges[] = {
	{2, LSM6DSM_ACCEL_FS_2G_VAL},
	{4, LSM6DSM_ACCEL_FS_4G_VAL},
	{8, LSM6DSM_ACCEL_FS_8G_VAL},
	{16, LSM6DSM_ACCEL_FS_16G_VAL}
};

const struct lsm6dsm_fs_reg g_ranges[] = {
	{245, LSM6DSM_GYRO_FS_245_VAL},
	{500, LSM6DSM_GYRO_FS_500_VAL},
	{1000, LSM6DSM_GYRO_FS_1000_VAL},
	{2000, LSM6DSM_GYRO_FS_2000_VAL}
};

/**
 * @return full scale table and size
 */
static inline const struct lsm6dsm_fs_reg *get_range_table(
		enum motionsensor_type type, int *psize)
{
	if (MOTIONSENSE_TYPE_ACCEL == type) {
		if (psize)
			*psize = ARRAY_SIZE(a_ranges);
		return a_ranges;
	} else {
		if (psize)
...
/**
 * @return full scale range data register
 */
static int get_reg_val(const int eng_val, const int round_up,
		       const struct lsm6dsm_fs_reg *pairs, const int size)
{
	int i;
...
static int get_engineering_val(const int reg_val,
			       const struct lsm6dsm_fs_reg *pairs,
			       const int size)
{
	int i;
...
static int set_range(const struct motion_sensor_t *s,
		     int range, int rnd)
{
	int err, range_tbl_size = 0;
	uint8_t ctrl_reg, reg_val;
	struct lsm6dsm_data *data = s->drv_data;
	const struct lsm6dsm_fs_reg *ranges;
	...
	if (err == EC_SUCCESS)
		data->base.range = get_engineering_val(reg_val, ranges, range_tbl_size);
...
static int read(const struct motion_sensor_t *s, vector_3_t v)
{
	...
	for (i = X; i <= Z; i++) {
		...
		if (s->type == MOTIONSENSE_TYPE_ACCEL)
			v[i] = v[i] * LSM6DSM_ACCEL_FS_GAIN(data->base.range);
		else
			v[i] = v[i] * LSM6DSM_GYRO_FS_GAIN(data->base.range);
	}
