...
int bmi160_decode_header(struct motion_sensor_t *accel,
...
static uint8_t bmi160_buffer[BMI_FIFO_BUFFER];
...
int bmi_load_fifo(struct motion_sensor_t *s, uint32_t last_ts)
{
	...
	bmi_read_n(s->port, s->addr, BMI160_FIFO_LENGTH_0,
	...
	bmi_read_n(s->port, s->addr, BMI160_FIFO_DATA, bmi160_buffer,
			length);
			...
			int16_t    *temp)
{
	...
	case MOTIONSENSE_TYPE_ACCEL:
		 ...
		 */
		for (i = X; i <= Z; i++) {
			bmi_read8(s->port, s->addr, BMI160_OFFSET_ACC70 + i,
				  &val);
			if (val > 0x7f)
				val = -256 + val;
			v[i] = round_divide(
	...
	case MOTIONSENSE_TYPE_GYRO:
		/* Read the MSB first */
		bmi_read8(s->port, s->addr, BMI160_OFFSET_EN_GYR98, &val98);
		/*
		 ...
		 */
		for (i = X; i <= Z; i++) {
			bmi_read8(s->port, s->addr, BMI160_OFFSET_GYR70 + i,
				  &val);
			val |= ((val98 >> (2 * i)) & 0x3) << 8;
			if (val > 0x1ff)
				val = -1024 + val;
