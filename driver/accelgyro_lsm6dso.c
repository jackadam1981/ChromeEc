...
static int config_interrupt(const struct motion_sensor_t *s)
{
	...
	ret = st_raw_read8(s->port, s->addr, LSM6DSO_INT1_CTRL, &int1_ctrl_val);
	if (ret != EC_SUCCESS)
		return ret;
	...
	ret = st_raw_write8(s->port, s->addr, LSM6DSO_INT1_CTRL, int1_ctrl_val);
...
static void push_fifo_data(struct motion_sensor_t *accel, uint8_t *fifo)
{
	 ...
	 *
	 * TODO: Add support to HW timestamp.
	 */
 ...
 */
int accelgyro_config_fifo(const struct motion_sensor_t *s)
{
	int err;
 ...
 * @rnd: Round up/down flag
 *
 * For mag in cascade with lsm6dso/l we use acc trigger and FIFO decimator
 */
...
static int init(const struct motion_sensor_t *s)
{
	 ...
	 * LSM6DSO/L supports both accel & gyro features
