			    ...
			    uint32_t saved_ts)
{
	...
	for (fifo_depth = fsts->len & LSM6DSO_FIFO_DIFF_MASK; fifo_depth > 0; fifo_depth--) {
		err = st_raw_read_n_noinc(main_s->port, main_s->i2c_spi_addr_flags,
...
static int irq_handler(struct motion_sensor_t *s, uint32_t *event)
{
	...
	if (IS_ENABLED(CONFIG_ACCEL_FIFO)) {
		while (true) {
			...
			if (ret != EC_SUCCESS) {
				CPRINTS("%s fail read_n %d", s->name, ret);
				return ret;
			}
