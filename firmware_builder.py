...
def build(opts):
    ...
    subprocess.run(cmd,
                   ...
                   check=True,
                   env=env)
    cmd = ['make', 'out=build/cr50_dbg', 'BOARD=cr50', 'CR50_DEV=1', '-j{}'.format(opts.cpus)]
    print(f'# Running {" ".join(cmd)}.')
    subprocess.run(cmd,
                   cwd=os.path.dirname(__file__),
                   check=True,
                   env=env)
    cmd = ['make', 'out=build/cr50_ct', 'BOARD=cr50', 'CRYPTO_TEST=1', '-j{}'.format(opts.cpus)]
    print(f'# Running {" ".join(cmd)}.')
    subprocess.run(cmd,
                   cwd=os.path.dirname(__file__),
                   check=True,
                   env=env)
    cmd = ['make', 'out=build/cr50_ct_rb', 'BOARD=cr50', 'CRYPTO_TEST=1', 'H1_RED_BOARD=1', '-j{}'.format(opts.cpus)]
    print(f'# Running {" ".join(cmd)}.')
    subprocess.run(cmd,
                   cwd=os.path.dirname(__file__),
                   check=True,
