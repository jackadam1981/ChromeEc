...
def bundle(opts):
    ...
    bundle_dir = os.path.join('/tmp', opts.target_dir) if opts.target_dir else DEFAULT_BUNDLE_DIRECTORY
    # TODO: create the target_dir if needed
    for target_build_dir in os.listdir(os.path.dirname(__file__), "build"):
       # TODO: include target in the name of the tarball or make subdirs
...
def parse_args(args):
    ...
    parser.add_argument(
        '--target_dir',
        required=False,
        help='Directory in which to bundle build artifacts (relative to /tmp).',
    )
