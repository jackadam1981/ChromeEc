...
def bundle(opts):
    ...
    with open(opts.metadata, "w", encoding="utf-8") as file:
        file.write(
            json_format.MessageToJson(firmware_pb2.FirmwareArtifactInfo())
        )
...
def test(opts):
    ...
    renode_install_dir = os.path.join(working_dir, "renode")
    ...
    os.environ["PATH"] += ":" + os.path.join(renode_install_dir, "bin")
