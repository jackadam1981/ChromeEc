...
def bundle(opts):
    ...
    for build_target in os.listdir(
        ...
        tarball_name = ''.join([build_target, '.firmware_from_source.tar.bz2'])
        subprocess.run([
            'tar', 'cvfj',
            os.path.join(bundle_dir, tarball_name), '--exclude=*.o.d',
            '--exclude=*.o', '.'
        ],
                       cwd=os.path.join(
                           os.path.dirname(__file__), 'build', build_target),
                       check=True)
