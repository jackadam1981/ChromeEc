...
/* clamp sensor raw i32 values to 0 ~ 65536 */
static inline uint16_t ec_motion_sensor_clamp_u16(const int32_t value)
{
	return (uint16_t)CLAMP(value, (int32_t)0, (int32_t)UINT16_MAX);
}
...
static inline int16_t ec_motion_sensor_clamp_value(enum motionsensor_type type,
						   const int32_t v)
{
	if (type == MOTIONSENSE_TYPE_LIGHT ||
	    type == MOTIONSENSE_TYPE_LIGHT_RGB)
		return (int16_t)ec_motion_sensor_clamp_u16(v);
	else
		return ec_motion_sensor_clamp_i16(v);
}
static inline void ec_motion_sensor_clamp_values(enum motionsensor_type type,
						 int16_t *arr, const int32_t *v)
{
	if (type == MOTIONSENSE_TYPE_LIGHT ||
	    type == MOTIONSENSE_TYPE_LIGHT_RGB)
		ec_motion_sensor_clamp_u16s((uint16_t *)arr, v);
	else
		ec_motion_sensor_clamp_i16s(arr, v);
}
