...
#define IOEX_IH_COUNT ioex_ih_count
...
struct ioexpander_drv {
	...
	/* Enable interrupt for the IOEX pin */
	int (*enable_interrupt)(int ioex, int port, int mask);
	/* Disable interrupt for the IOEX pin */
	int (*disable_interrupt)(int ioex, int port, int mask);
};
