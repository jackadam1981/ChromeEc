...
__packed struct usb_function_section {
	uint8_t bFirstIntefaceNumber;
	uint8_t reserved_1;
...
__packed struct usb_ms_ext_compat_id_desc {
	uint32_t dwLength;
	uint16_t bdcVersion;
	uint16_t wIndex;
	uint8_t bCount;
	uint8_t Reserved[7];
	/*
	 * The spec allows for mulitple function sections to be included, but
