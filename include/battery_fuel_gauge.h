...
/**
 * Return whether a board supports BCIC or not.
 *
 * If a board needs to support units without battery config in CBI and units
 * with battery config in CBI, it needs to implement this callback so that BCIC
 * can distinguish the two groups. This is needed because without this callback,
 * BCIC can't tell battery config is missing because it's an old unit or because
 * the default config is applicable.
 *
 *   bool board_batt_conf_supported(void)
 *   {
 *       if (BOARD_VERSION > 0)
 *           return true;
 *       else
 ...
 */
__override_proto bool board_batt_conf_supported(void);
