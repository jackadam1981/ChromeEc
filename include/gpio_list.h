 ...
 *    IOEX(name, EXPIN(ioex_port, port, offset), group, flags)
 ...
 *      - group: alternate group of the same name pin.
 *      - flags: the same as the flags of GPIO.
 *
 */
#define IOEX(name, expin, group, flags) {#name, IOEX_##expin, group, flags},
/*
 *  Define the IO expander IO which supports interrupt in gpio.inc by
 *  the format:
 *    IOEX_INT(name, EXPIN(ioex_port, port, offset), group, flags, handler)
 *      - name: the name of this IO pin
 *      - EXPIN(ioex, port, offset)
 *         - ioex: the IO expander port (defined in board.c) this IO
 *                 pin belongs to.
 ...
 */
#define IOEX_INT(name, expin, group, flags, handler) {#name, IOEX_##expin, group, flags},
...
#define IOEX_INT(name, expin, group, flags, handler) handler,
 ...
 * file.
 */
#define IOEX_INT(name, expin, group, flags, handler)		\
	BUILD_ASSERT(IOEX_##name##_GRP_##group - IOEX_SIGNAL_START	\
		     < ARRAY_SIZE(ioex_irq_handlers));
#include "gpio.wrap"

#define IOEX(name, expin, group, flags) expin
#define IOEX_INT(name, expin, group, flags, handler) expin
