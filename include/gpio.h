...
/**
 * Configure a GPIO as wake source on a given condition, and enable it.
 *
 * @param signal       GPIO to enable to wake Cr50 up
 * @param flags        Wake condition. Should be one among
 *                     GPIO_HIB_WAKE_{HIGH, LOW, RISING, FALLING}.
 */
void gpio_enable_wakepin(enum gpio_signal signal, uint32_t flags);

/**
 * Disable a GPIO to wake Cr50 up.
 *
 * @param signal       GPIO to disable
 */
void gpio_disable_wakepin(enum gpio_signal signal);
