...
/*
 * Number of times a port may try to clear its overcurrent event counter before
 * introducing a 2 second delay for clearing the event counter.  If the port
 * tries to clear its overcurrent event counter exceeding this, future clears
 * will not occur for at least 2 seconds.  This allows port to become latched
 * off.
 */
#define PPC_OC_CLR_CNT_THRESH 3
 ...
 * @return EC_SUCCESS on success, EC_ERROR_INVAL if non-existent port,
 *         EC_ERROR_BUSY if clearing the port's overcurrent event counter is
