...
struct ppc_drv {
	...
	/**
	 * Put the PPC into its lowest power state. In this state is should
	 * still fire interrupts if Vbus changes etc.
	 *
	 * @param port: The Type-C port number.
	 * @return EC_SUCCESS on success, error otherwise.
	 */
	int (*enter_low_power_mode)(int port);
#endif /* CONFIG_USBC_PPC_LOW_POWER */
};
...
/**
 * Put the PPC into its lowest power state. In this state is should still fire
 * interrupts if Vbus changes etc.
 *
 * @param port: The Type-C port number.
 * @return EC_SUCCESS on success, error otherwise.
 */
