...
struct accelgyro_method {
	 ...
	 * @return EC_SUCCESS if successful, non-zero if error.
	 */
	int (*read)(struct motion_sensor_t *s,
		    int *const x_acc,
		    int *const y_acc,
		    int *const z_acc);
	...
	int (*set_range)(struct motion_sensor_t *s,
			 const int range,
			 const int rnd);
	int (*get_range)(struct motion_sensor_t *s,
			 int * const range);
	 ...
	 * @return EC_SUCCESS if successful, non-zero if error.
	 */
	int (*set_resolution)(struct motion_sensor_t *s,
			      const int res,
			      const int rnd);
	int (*get_resolution)(struct motion_sensor_t *s,
			      int * const res);
	 ...
	 * @return EC_SUCCESS if successful, non-zero if error.
	 */
	int (*set_data_rate)(struct motion_sensor_t *s,
			    const int rate,
			    const int rnd);
	int (*get_data_rate)(struct motion_sensor_t *s,
			    int * const rate);
	 ...
	 */
	int (*set_interrupt)(struct motion_sensor_t *s,
