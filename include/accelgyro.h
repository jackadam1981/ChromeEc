...
/* This struct must be defined in board.c. */
/* SHAWNN - define it */
...
struct accelgyro_info {
	 ...
	 * @return EC_SUCCESS if successful, non-zero if error.
	 */
	int (*set_range)(const int range,
			 const int rnd);
	int (*get_range)(int * const range);
	 ...
	 * @return EC_SUCCESS if successful, non-zero if error.
	 */
	int (*set_resolution)(const int res,
			      const int rnd);
	int (*get_resolution)(int * const res);
	 ...
	 * @return EC_SUCCESS if successful, non-zero if error.
	 */
	int (*set_datarate)(const int rate,
			    const int rnd);
	int (*get_datarate)(int * const rate);
	 ...
	 */
	int (*set_interrupt)(unsigned int threshold);
#endif
};
