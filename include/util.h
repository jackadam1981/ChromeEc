...
#ifdef CONFIG_ASSEMBLY_MAC32
/*
 * Compute (a*b)+c[+d], where a, b, c[, d] are 32-bit integers, and the result
 * is 64-bit long.
 */
uint64_t mac32(uint32_t a, uint32_t b, uint32_t c);
uint64_t mac32_2(uint32_t a, uint32_t b, uint32_t c, uint32_t d);
#else
static inline uint64_t mac32(uint32_t a, uint32_t b, uint32_t c)
{
	uint64_t ret = a;
...
static inline uint64_t mac32_2(uint32_t a, uint32_t b, uint32_t c, uint32_t d)
{
	uint64_t ret = a;
