 ...
 */
uint16_t u2f_apdu_rcv(const uint8_t *in, uint16_t in_len, uint8_t *out);
...
/**
 * Generate ASN.1 DER encoded signature.
 *
 * @param p ptr to store the signature.
 * @return size of the generated ASN.1 content.
 */
size_t asn1_sigp(uint8_t* p, const p256_int* r, const p256_int* s);

/**
 * Get a board/chip specific 32-byte serial number.
 *
 * @param n pointer to store the serial.
 */
...
uint16_t u2f_custom_dispatch(uint8_t ins, struct apdu apdu,
			     uint8_t* obuf, uint16_t* obuf_len)
			     __attribute__((weak));
