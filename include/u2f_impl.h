...
#define U2F_V1_KH_SIZE              64
#define U2F_V2_KH_HEADER_SIZE       65
#define U2F_V2_KH_SIZE              113
...
BUILD_ASSERT(sizeof(struct u2f_key_handle) <= U2F_MAX_KH_SIZE);
BUILD_ASSERT(sizeof(struct u2f_key_handle) == U2F_V1_KH_SIZE);
...
BUILD_ASSERT(sizeof(struct u2f_versioned_key_handle) <= U2F_MAX_KH_SIZE);
BUILD_ASSERT(sizeof(struct u2f_versioned_key_handle) == U2F_V2_KH_SIZE);
 ...
 * on this device (signed with U2F state HMAC key). It doesn't verify
 * authentication time secret.
 *
 * @param state initialized u2f state
