...
/* Float to fixed-point, only for compile-time constants */
 ...
 */
static __inline fp_t fp_mul(fp_t a, fp_t b)
{
 ...
 * Division - return (a / b)
 */
static __inline fp_t fp_div(fp_t a, fp_t b)
{
 ...
 * Square (a * a)
 */
static __inline fp_t fp_sq(fp_t a)
{
	return (fp_t)(((int64_t)a * a) >> FP_BITS);
}

/**
 * Absolute value
 */
static __inline fp_t fp_abs(fp_t a)
{
	return (a >= 0 ? a : -a);
}
 ...
 * Integer vector.
 *
 * Some of the math code (such as vector_magnitude() assumes this contains
 * count values which are only signed 16-bit.  If this assumption becomes
 * false, some of the code in math_util.c will need to be promoted to 64-bit.
 */
