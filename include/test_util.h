 ...
 * @param perif_addr The address of the detached device
 * @return EC_SUCCESS if detached; EC_ERROR_OVERFLOW if too many devices are
 *         detached.
 */
int test_detach_i2c(const int port, const uint16_t perif_addr_flags);
 ...
 */
int test_attach_i2c(const int port, const uint16_t perif_addr_flags);
