...
struct ec_response_usb_pd_control_v2 {
	...
	/*
	 * Get early boot state of Type-C Sub Systems (TCSS)
	 ...
	 * (Power Management Controller). Kenrel reads this state at init and
	 * decides to set the next TCSS state of USB-C port.
	 */
	uint8_t tcss_state;
...
struct ec_params_usb_pd_set_tcss_state {
	 ...
	 * Note: These states are Intel Virtual MUX states (but not limited) and
	 * are used in the Coreboot and Kernel to set the Virtual MUX of PMC
	 * (Power Management Controller). Coreboot sets this state based on the
	 * early boot TCSS state of USB-C port.
	 */
	uint8_t tcss_state;
} __ec_align1;
