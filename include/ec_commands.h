 ...
 * the most cases. (Old->New means old kernel sends get_next_event command to
 * new FW.)
 *
 * 1. Extend existing field (e.g. key_matrix[16]->[18])
 * Old->New:
 *    Existing kernels send a smaller container (e.g.
 *    ec_response_get_next_data) which may or may not fit the last few bytes.
 *    The FW copies as many bytes as possible to the container. The kernel
 *    processes as many leading bytes as it can understand.
 * New->Old:
 *    New kernels send a bigger container. Existing FW copies as many bytes as
 *    it wants, leaving the last few bytes empty. The kernel knows it didn't
 *    receive full size data from the returned data length.
 */
