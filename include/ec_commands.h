...
/*
 * Slave address is 7-bit.  If this flag is present, the flags byte is followed
 * by a byte containing the slave address.  Otherwise, the last slave address
 * is used.  Note that either this or EC_I2C_FLAG_10BIT _must_ be present in
 * the flags for the first message (since otherwise the EC doesn't know what
 * address to read/write).
 */
#define EC_I2C_FLAG_7BIT	(1 << 0)
 ...
 */
#define EC_I2C_FLAG_10BIT	(1 << 1)

/*
 * Do not send start bit before this segment.  Note that regardless of this
 * flag, a start bit will be sent for the first message.
 */
#define EC_I2C_FLAG_NOSTART	(1 << 2)

/*
 * Send stop bit after this segment.  Note that regardless of this flag, a stop
 * bit will be sent after the last message.
 */
#define EC_I2C_FLAG_STOP	(1 << 3)

/* Read data; if not present, message is a write and data to write follows */
#define EC_I2C_FLAG_READ	(1 << 4)
...
#define EC_I2C_STATUS_READ	(1 << 5) /* Message was read; data follows */
