...
/* let FIFO task know that work is imminent to avoid excessive AP interrupts */
void keyboard_fifo_work_in_progress(void);

/* clear any saved keyboard state (empty FIFO, etc) */
void keyboard_clear_state(void);

/* push an entry onto the keyboard FIFO */
int keyboard_fifo_push(uint8_t *buffp);

/* pop an entry from the keyboard FIFO */
