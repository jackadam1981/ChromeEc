...
#define WNG_PACKED __attribute__((__packed__))

#define WNG_ALIGN_TO(x) __attribute__((aligned(16)))

#define WNG_PROTOCOL_VERSION 0

#define WNG_MAX_MESSAGE_SIZE 2048

/* The block size of encryption used for wrapped_leaf_data_t. */
#define WNG_WRAP_BLOCK_SIZE 16

typedef enum {
	WNG_ERR_VERSION_MISMATCH = 0x10000, /* EC_ERROR_INTERNAL_FIRST */
	WNG_ERR_TREE_INVALID,
...
typedef struct WNG_PACKED WNG_ALIGN_TO(WNG_WRAP_BLOCK_SIZE) {
	uint32_t version;
	/* Immutable state. */
...
typedef struct WNG_PACKED {
	 ...
	 */
	hash_t hmac;
	uint8_t iv[WNG_WRAP_BLOCK_SIZE];
...
typedef struct WNG_PACKED {
	...
	union {
		...
		uint8_t raw[WNG_MAX_MESSAGE_SIZE -
				sizeof(wng_request_header_t)];
...
#define WNG_MAX_PATH_SIZE (WNG_MAX_MESSAGE_SIZE - sizeof(wng_request_header_t) \
		- sizeof(union { \
				wng_request_insert_leaf_t insert_leaf; \
				wng_request_remove_leaf_t remove_leaf; \
				wng_request_try_auth_t try_auth; \
