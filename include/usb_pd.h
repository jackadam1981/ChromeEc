 ...
 * HW Device ID: unique descriptor for each ChromeOS device (1 byte)
 * SW Debug Version: Software version useful for debugging (1 byte)
 * IS RW: True if currently in RW, False otherwise (1 bit)
 */
#define VDO_INFO(id, ver, is_rw) ((id) << 24 | (ver) << 16 | (is_rw))
#define VDO_INFO_HW_DEV_ID(x)    ((x) >> 24)
#define VDO_INFO_SW_DBG_VER(x)   (((x) >> 16) & 0xff)
