...
/*
 * TBT3 cable Discover mode responses
...
struct tbt_mode_resp_cable {
	union {
		struct {
			...
			uint32_t tbt_cable_type : 1;
			uint32_t tbt_retimer : 1;
			uint32_t link_lsrx_comm : 1;
...
struct pd_cable {
	...
	struct tbt_mode_resp_device tbt_dev_resp;
	/* For storing Discover mode response from cable */
	struct tbt_mode_resp_cable tbt_cable_resp;
};
...
/**
 * Enables SOP prime communication if CONFIG_USB_PD_DECODE_SOP is enabled
 *
 * @param port     USB-C port number
 */
...
/**
 * Disables SOP prime communication if CONFIG_USB_PD_DECODE_SOP is enabled
 *
 * @param port     USB-C port number
 */
void disable_transmit_sop_prime(int port);

/** Returns the current status of compatible tbt flag
 *
 * @param port  USB-C port number
...
/** Clears compat thunderbolt mode and sets cable[port].is_identifed byte
 *
 * @param port  USB-C port number
 */
