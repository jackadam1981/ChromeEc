...
struct svdm_mode_data {
	uint16_t svid;
	void (*enter)(uint32_t mode_vdo);
	void (*exit)(void);
};
 ...
 *
 * @param cnt       number of svids.
 * @param svids     svid data.
 * @param mode_vdo  pointer to mode capabilities that have been chosen.
 * @param mode_idx  pointer to mode index that will be used.
 * @return mode data for entry / exit
 */
struct svdm_mode_data *pd_dfp_choose_mode(int cnt, struct svdm_svid_data
					  *svids, uint32_t **mode_vdo,
					  int *mode_idx);
/**
