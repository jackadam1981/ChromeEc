...
enum vendor_cmd_cc {
	 ...
	 *
	 * The message sent and recieved to this vendor command, with the exception
	 * of SET responses, uses the following form:
	 *
	 * ```c
	 * struct __attribute__((__packed__)) command_msg {
	 *	 uint8_t version;           // Current version of the API
	 *	 uint8_t command;           // Command to operate on; determines payload type
	 *	 struct command_data data;  // Type here depends on command
	 * };
	 * ```
	 */
