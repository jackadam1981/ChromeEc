...
/*
 * Access to the I2C/SPI Slave address information within the muxes
 */
#define MUX_SLAVE_ADDR(port) (usb_muxes[port].slave_addr)
...
static inline int mux_write(int port, int reg, int val)
{
	return usb_muxes[port].flags & USB_MUX_FLAG_NOT_TCPC
		? i2c_write8(MUX_SLAVE_ADDR(port), reg, val)
...
{
	return usb_muxes[port].flags & USB_MUX_FLAG_NOT_TCPC
		? i2c_read8(MUX_SLAVE_ADDR(port), reg, val)
...
{
	return usb_muxes[port].flags & USB_MUX_FLAG_NOT_TCPC
		? i2c_write16(MUX_SLAVE_ADDR(port), reg, val)
...
{
	return usb_muxes[port].flags & USB_MUX_FLAG_NOT_TCPC
		? i2c_read16(MUX_SLAVE_ADDR(port), reg, val)
