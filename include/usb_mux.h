...
void usb_mux_set(int port, mux_state_t mux_mode, enum usb_switch usb_config,
		 int polarity);
#else
static inline void usb_mux_set(int port, mux_state_t mux_mode, enum usb_switch usb_config,
          int polarity)
{
}
#endif
...
#else
static inline void usb_mux_hpd_update(int port, mux_state_t hpd_state)
{
}
#endif
