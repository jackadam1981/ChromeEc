 ...
 * character is acceptable. Zero-length KEYs are not allowed, and assigning a
 * NULL or zero-length VALUE to a KEY just deletes that KEY (if it existed).
 *
 ...
 *
 * CAUTION: The implementation does not provide any protection against
 * simultaneous access by multiple tasks. Callers should establish their own
 * locks or mutexes as needed. In general that would mean aquiring a lock
 * before calling getvar() or setvar(), and releasing it after calling
 * writevars().
 *
