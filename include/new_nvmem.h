 ...
 *
 * size: size of the payload, in fact could fit in 11 bits
 *
...
/*
 * Helper structure to keep track of accesses to the flash storage, used to
 * store a context of sequential accesses to the storage.
 *
 * Container fetches are tracked separately (using ch_ph and
 * container_data_offset fields), as the location of containers has special
 * significance: it is both part of the seed used when encrypting/decryping
 * container contents, and also is necessary to unwind reading of the
 * container header when the end of storage is reached and a header of all
 * 0xff is read.
 *
 * Keeping track of delimiters is also important when assessing flash contents
 * integrity. If during startup the last item in flash is not a delimiter,
 * this is an indication of a failed transaction, all data after the previous
 * delimiter needs to be discarded.
 *
 * ph: pointer to the flash page which the next read will read from
 *
 * ch_ph: pointer to the flash page where the most recent container header was
 *        read from.
 *
 * dh_ph: pointer to the flash page where the most recent delimiter was read
 *        from.
 *
 * data_offset: offset where the next read will read from.
 *
 * cont_data_offset: offset where the most recent container header was read
 *		    from.
 *
 * delim_data_offset: offset where the most recent delimiter was read from.
 *
