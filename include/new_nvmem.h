 ...
 * store a context of sequential accesses to the flash storage.
 *
 * The 'last container read' state has special significance: this is both part
 * of the seed for encrypting/decryping container contents, and can be used to
 * unwind reading of the container header when the end of storage is reached.
 *
 * list_index; index of the current page in the list of pages
 *
 * data_offset: offset where the next read will read from
 *
 * orig_data_offset; offset where the container header was read from last
 *
 * ph: pointer to the flash page which the next read will read from
 *
 * orig_ph: pointer to the flash page where the container header was read
 *          from last.
 */
struct page_tracker {
	uint8_t list_index;
...
extern struct page_tracker master_pt;
#endif
