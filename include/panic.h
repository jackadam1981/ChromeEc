...
#ifdef CONFIG_PANIC_DATA_ADDR
#define PANIC_DATA_PTR ((struct panic_data *) CONFIG_PANIC_DATA_ADDR)
#else
/*
 * By default, panic data goes at the end of RAM.  This is safe because we don't
 * context switch away from the panic handler before rebooting, and stacks and
 * data start at the beginning of RAM.
 */
