 ...
 * @param input_encryption_salt the salt for deriving secret, must be at least
 * FP_CONTEXT_SALT_BYTES in size.
 * @return EC_SUCCESS on success and error code otherwise.
 */
...
/**
 * Derive validation value (hmac_SHA256 of positive match secret).
 *
 * @param output buffer to store validation value, must be at lease
 * SHA256_DIGEST_SIZE in size.
 * @param positive_match_secret input positive match secret.
 */
