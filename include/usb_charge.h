...
extern struct bc12_config bc12_chips[];
...
{
	bc12_chips[port].drv->set_switches(port, setting);
}
...
static inline int usb_charger_ramp_allowed(int port, int supplier)
{
	if (port == CHARGE_PORT_NONE)
		return 0;
	return bc12_chips[port].drv->ramp_allowed(port, supplier);
}
...
static inline int usb_charger_ramp_max(int port, int supplier, int sup_curr)
{
	if (port == CHARGE_PORT_NONE)
		return 0;
	return bc12_chips[port].drv->ramp_max(port, supplier, sup_curr);
}
