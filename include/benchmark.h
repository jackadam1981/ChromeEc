 ...
 *
 * Benchmark utility functions. This is a C++ porting of lib_benchmark.h.
 */

#ifndef MCUTEST_LIB_BENCHMARK_PLUS_H_
#define MCUTEST_LIB_BENCHMARK_PLUS_H_
...
/* Benchmark main class responsible to running the experiments and
...
template <int MAX_NUM_RESULTS = 5> class Benchmark {
	 ...
	 * f() can be either a function pointer or a lambda function.
	 *
...
/* Table header separator characters */
#define SEP(n) n, "-------------------------------------"
	...
	void print_header(void)
	{
		...
		ccprintf("%.*s | %.*s | %.*s | %.*s | %.*s | %.*s\n", SEP(16),
			 SEP(15), SEP(13), SEP(13), SEP(13), SEP(13));
		cflush();
	}
...
#endif /* MCUTEST_LIB_BENCHMARK_PLUS_H_ */
