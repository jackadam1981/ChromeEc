 ...
 * reseting the buffer. This buffer is intended to be dumped all at once.
 *
...
typedef struct {
	...
	/* User specfied version. Update if higher level schema changes. */
...
typedef struct {
	...
	/* Word size in bytes */
	uint32_t word_size;
} preserved_ring_buf_preserved_t;
...
typedef struct {
	...
	volatile preserved_ring_buf_preserved_t *preserved;
...
#define DECLARE_PRESERVED_RING_BUF(word_type, name, _capacity, _version)          \
	...
	static volatile preserved_ring_buf_preserved_t                            \
		_preserved_ring_buf_##name##_preserved __uncached __aligned(4)    \
			__noinit_end_of_ram(                                      \
				_preserved_ring_buf_##name##_preserved);          \
	static const preserved_ring_buf_t _preserved_ring_buf_##name##_data = { \
			.buffer = { \
				.as_uint8 = _preserved_ring_buf_##name##_buffer, \
			}, \
			.preserved = &_preserved_ring_buf_##name##_preserved, \
			.constants = { \
				.capacity = _capacity, \
				.version = _version, \
