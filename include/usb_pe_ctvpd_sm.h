...
/* USB Policy Engine CTVPD module */
...
static unsigned int pe_request_exit(int port);
...
{
	pe[port].sm_flags |= PE_FLAGS_MSG_RECEIVED;
...
static unsigned int pe_request_run(int port)
{
	...
	if (pe[port].sm_flags & PE_FLAGS_MSG_RECEIVED) {
		pe[port].sm_flags &= ~PE_FLAGS_MSG_RECEIVED;
