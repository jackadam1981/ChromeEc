 ...
 *
 * Based on Revsion 3.0, Version 1.2 of
 * the USB Power Delivery Specification.
 */

#ifndef __CROS_EC_USB_PE_DRP_H
#define __CROS_EC_USB_PE_DRP_H
...
#define SET_FLAG(port, flag) pe[port].flags  |=  (flag)
#define CLR_FLAG(port, flag) pe[port].flags  &= ~(flag)
#define CHK_FLAG(port, flag) (pe[port].flags &   (flag))

#define SET_DPM_REQUEST(port, req) pe[port].dpm_request  |=  (req)
#define CLR_DPM_REQUEST(port, req) pe[port].dpm_request  &= ~(req)
#define CHK_DPM_REQUEST(port, req) (pe[port].dpm_request &   (req))
...
#define PE_FLAGS_POWER_ROLE_SWAP                BIT(10)
#define PE_FLAGS_SNK_WAIT_CAP_TIMEOUT           BIT(11)
#define PE_FLAGS_PS_TRANSITION_TIMEOUT          BIT(12)
#define PE_FLAGS_INTERRUPTIBLE_AMS              BIT(13)
#define PE_FLAGS_PS_RESET_COMPLETE              BIT(14)
#define PE_FLAGS_SEND_SVDM                      BIT(15)
#define PE_FLAGS_VCONN_SWAP_COMPLETE            BIT(16)
#define PE_FLAGS_DISCOVER_PORT_IDENTITY_DONE    BIT(17)
#define PE_FLAGS_DISCOVER_CABLE_IDENTITY_DONE   BIT(18)

#define N_CAPS_COUNT 25
#define N_DISCOVER_IDENTITY_COUNT 10
...
/* Source Port */
static unsigned int pe_src_startup(int port, enum signal sig);
static unsigned int pe_src_startup_entry(int port); 
static unsigned int pe_src_startup_run(int port); 

static unsigned int pe_src_discovery(int port, enum signal sig);
static unsigned int pe_src_discovery_entry(int port);
static unsigned int pe_src_discovery_run(int port);

static unsigned int pe_src_send_capabilities(int port, enum signal sig);
static unsigned int pe_src_send_capabilities_entry(int port);
static unsigned int pe_src_send_capabilities_run(int port);

static unsigned int pe_src_negotiate_capability(int port, enum signal sig);
static unsigned int pe_src_negotiate_capability_entry(int port);

static unsigned int pe_src_transition_supply(int port, enum signal sig);
static unsigned int pe_src_transition_supply_entry(int port);
static unsigned int pe_src_transition_supply_run(int port);

static unsigned int pe_src_ready(int port, enum signal sig);
static unsigned int pe_src_ready_entry(int port);
static unsigned int pe_src_ready_run(int port);
static unsigned int pe_src_ready_exit(int port);

static unsigned int pe_src_disabled(int port, enum signal sig);
static unsigned int pe_src_disabled_entry(int port);
static unsigned int pe_src_disabled_run(int port);

static unsigned int pe_src_capability_response(int port, enum signal sig);
static unsigned int pe_src_capability_response_entry(int port);
static unsigned int pe_src_capability_response_run(int port);

static unsigned int pe_src_hard_reset(int port, enum signal sig);
static unsigned int pe_src_hard_reset_entry(int port);
static unsigned int pe_src_hard_reset_run(int port);

static unsigned int pe_src_hard_reset_received(int port, enum signal sig);
static unsigned int pe_src_hard_reset_received_entry(int port);
static unsigned int pe_src_hard_reset_received_run(int port);

static unsigned int pe_src_transition_to_default(int port, enum signal sig);
static unsigned int pe_src_transition_to_default_entry(int port);
static unsigned int pe_src_transition_to_default_run(int port);

static unsigned int pe_src_vdm_identity(int port, enum signal sig);
static unsigned int pe_src_vdm_identity_entry(int port);
static unsigned int pe_src_vdm_identity_run(int port);

/* Sink Port */
static unsigned int pe_snk_startup(int port, enum signal sig);
static unsigned int pe_snk_startup_entry(int port);
static unsigned int pe_snk_startup_run(int port);

static unsigned int pe_snk_discovery(int port, enum signal sig);
static unsigned int pe_snk_discovery_entry(int port);
static unsigned int pe_snk_discovery_run(int port);

static unsigned int pe_snk_wait_for_capabilities(int port, enum signal sig);
static unsigned int pe_snk_wait_for_capabilities_entry(int port);
static unsigned int pe_snk_wait_for_capabilities_run(int port);

static unsigned int pe_snk_evaluate_capability(int port, enum signal sig);
static unsigned int pe_snk_evaluate_capability_entry(int port);

static unsigned int pe_snk_select_capability(int port, enum signal sig);
static unsigned int pe_snk_select_capability_entry(int port);
static unsigned int pe_snk_select_capability_run(int port);

static unsigned int pe_snk_transition_sink(int port, enum signal sig);
static unsigned int pe_snk_transition_sink_entry(int port);
static unsigned int pe_snk_transition_sink_run(int port);
static unsigned int pe_snk_transition_sink_exit(int port);

static unsigned int pe_snk_ready(int port, enum signal sig);
static unsigned int pe_snk_ready_entry(int port);
static unsigned int pe_snk_ready_run(int port);
static unsigned int pe_snk_ready_exit(int port);

static unsigned int pe_snk_hard_reset(int port, enum signal sig);
static unsigned int pe_snk_hard_reset_entry(int port);
static unsigned int pe_snk_hard_reset_run(int port);

static unsigned int pe_snk_transition_to_default(int port, enum signal sig);
static unsigned int pe_snk_transition_to_default_entry(int port);
static unsigned int pe_snk_transition_to_default_run(int port);

static unsigned int pe_give_sink_cap(int port, enum signal sig);
static unsigned int pe_give_sink_cap_entry(int port);
static unsigned int pe_give_sink_cap_run(int port);

static unsigned int pe_snk_get_source_cap(int port, enum signal sig);
static unsigned int pe_snk_get_source_cap_entry(int port);
static unsigned int pe_snk_get_source_cap_run(int port);

static unsigned int pe_dr_snk_give_source_cap(int port, enum signal sig);
static unsigned int pe_dr_snk_give_source_cap_entry(int port);
static unsigned int pe_dr_snk_give_source_cap_run(int port);


/* Soft Reset */
static unsigned int pe_send_soft_reset(int port, enum signal sig);
static unsigned int pe_send_soft_reset_entry(int port);
static unsigned int pe_send_soft_reset_run(int port);
static unsigned int pe_send_soft_reset_exit(int port);

static unsigned int pe_soft_reset(int port, enum signal sig);
static unsigned int pe_soft_reset_entry(int port);
static unsigned int pe_soft_reset_run(int port);

/* Not Supported Message */
static unsigned int pe_send_not_supported(int port, enum signal sig);
static unsigned int pe_send_not_supported_entry(int port);
static unsigned int pe_send_not_supported_run(int port);

/* PE_SRC_Ping Message */
static unsigned int pe_src_ping(int port, enum signal sig);
static unsigned int pe_src_ping_entry(int port);
static unsigned int pe_src_ping_run(int port);

/* PE_Give_Battery_Cap Message */
static unsigned int pe_give_battery_cap(int port, enum signal sig);
static unsigned int pe_give_battery_cap_entry(int port);
static unsigned int pe_give_battery_cap_run(int port);

/* PE_Give_Battery_Status Message */
static unsigned int pe_give_battery_status(int port, enum signal sig);
static unsigned int pe_give_battery_status_entry(int port);
static unsigned int pe_give_battery_status_run(int port);

static unsigned int pe_drs_evaluate_swap(int port, enum signal sig);
static unsigned int pe_drs_evaluate_swap_entry(int port);
static unsigned int pe_drs_evaluate_swap_run(int port);

static unsigned int pe_drs_change(int port, enum signal sig);
static unsigned int pe_drs_change_entry(int port);
static unsigned int pe_drs_change_run(int port);

static unsigned int pe_drs_send_swap(int port, enum signal sig);
static unsigned int pe_drs_send_swap_entry(int port);
static unsigned int pe_drs_send_swap_run(int port);

static unsigned int pe_prs_src_snk_evaluate_swap(int port, enum signal sig);
static unsigned int pe_prs_src_snk_evaluate_swap_entry(int port);
static unsigned int pe_prs_src_snk_evaluate_swap_run(int port);

static unsigned int pe_prs_src_snk_transition_to_off(int port, enum signal sig);
static unsigned int pe_prs_src_snk_transition_to_off_entry(int port);
static unsigned int pe_prs_src_snk_transition_to_off_run(int port);

static unsigned int pe_prs_src_snk_assert_rd(int port, enum signal sig);
static unsigned int pe_prs_src_snk_assert_rd_entry(int port);
static unsigned int pe_prs_src_snk_assert_rd_run(int port);

static unsigned int pe_prs_src_snk_wait_source_on(int port, enum signal sig);
static unsigned int pe_prs_src_snk_wait_source_on_entry(int port);
static unsigned int pe_prs_src_snk_wait_source_on_run(int port);
static unsigned int pe_prs_src_snk_wait_source_on_exit(int port);

static unsigned int pe_prs_src_snk_send_swap(int port, enum signal sig);
static unsigned int pe_prs_src_snk_send_swap_entry(int port);
static unsigned int pe_prs_src_snk_send_swap_run(int port);
static unsigned int pe_prs_src_snk_send_swap_exit(int port);

static unsigned int pe_prs_snk_src_evaluate_swap(int port, enum signal sig);
static unsigned int pe_prs_snk_src_evaluate_swap_entry(int port);
static unsigned int pe_prs_snk_src_evaluate_swap_run(int port);

static unsigned int pe_prs_snk_src_transition_to_off(int port, enum signal sig);
static unsigned int pe_prs_snk_src_transition_to_off_entry(int port);
static unsigned int pe_prs_snk_src_transition_to_off_run(int port);

static unsigned int pe_prs_snk_src_assert_rp(int port, enum signal sig);
static unsigned int pe_prs_snk_src_assert_rp_entry(int port);
static unsigned int pe_prs_snk_src_assert_rp_run(int port);

static unsigned int pe_prs_snk_src_source_on(int port, enum signal sig);
static unsigned int pe_prs_snk_src_source_on_entry(int port);
static unsigned int pe_prs_snk_src_source_on_run(int port);

static unsigned int pe_prs_snk_src_send_swap(int port, enum signal sig);
static unsigned int pe_prs_snk_src_send_swap_entry(int port);
static unsigned int pe_prs_snk_src_send_swap_run(int port);
static unsigned int pe_prs_snk_src_send_swap_exit(int port);

#ifdef CONFIG_USBC_VCONN
/* VCONN Swap */
static unsigned int pe_vcs_evaluate_swap(int port, enum signal sig);
static unsigned int pe_vcs_evaluate_swap_entry(int port);
static unsigned int pe_vcs_evaluate_swap_run(int port);

static unsigned int pe_vcs_send_swap(int port, enum signal sig);
static unsigned int pe_vcs_send_swap_entry(int port);
static unsigned int pe_vcs_send_swap_run(int port);

static unsigned int pe_vcs_wait_for_vconn_swap(int port, enum signal sig);
static unsigned int pe_vcs_wait_for_vconn_swap_entry(int port);
static unsigned int pe_vcs_wait_for_vconn_swap_run(int port);

static unsigned int pe_vcs_turn_on_vconn_swap(int port, enum signal sig);
static unsigned int pe_vcs_turn_on_vconn_swap_entry(int port);
static unsigned int pe_vcs_turn_on_vconn_swap_run(int port);

static unsigned int pe_vcs_turn_off_vconn_swap(int port, enum signal sig);
static unsigned int pe_vcs_turn_off_vconn_swap_entry(int port);
static unsigned int pe_vcs_turn_off_vconn_swap_run(int port);

static unsigned int pe_vcs_send_ps_rdy_swap(int port, enum signal sig);
static unsigned int pe_vcs_send_ps_rdy_swap_entry(int port);
static unsigned int pe_vcs_send_ps_rdy_swap_run(int port);
#endif

/* Initiator to Port VDM Discover Identity States */
static unsigned int pe_handle_svdm_request(int port, enum signal sig);
static unsigned int pe_handle_svdm_request_entry(int port);
static unsigned int pe_handle_svdm_request_run(int port);
static unsigned int pe_handle_svdm_request_exit(int port);

static unsigned int pe_handle_custom_vdm_request(int port, enum signal sig);
static unsigned int pe_handle_custom_vdm_request_entry(int port);
static unsigned int pe_handle_custom_vdm_request_run(int port);
static unsigned int pe_handle_custom_vdm_request_exit(int port);

static unsigned int pe_wait_for_error_recovery(int port, enum signal sig);
static unsigned int pe_wait_for_error_recovery_entry(int port);
static unsigned int pe_wait_for_error_recovery_run(int port);
static unsigned int pe_wait_for_error_recovery_exit(int port);

static unsigned int pe_bist(int port, enum signal sig);
static unsigned int pe_bist_entry(int port);
static unsigned int pe_bist_run(int port);
static unsigned int pe_bist_exit(int port);

static unsigned int do_nothing(int port);
static unsigned int get_super_state(int port);


static const state_sig pe_src_startup_sig[] = {
...
static const state_sig pe_src_discovery_sig[] = {
...
static const state_sig pe_src_send_capabilities_sig[] = {
...
static const state_sig pe_src_negotiate_capability_sig[] = {
...
static const state_sig pe_src_transition_supply_sig[] = {
...
static const state_sig pe_src_ready_sig[] = {
	pe_src_ready_entry,
	pe_src_ready_run,
	pe_src_ready_exit,
	get_super_state
...
static const state_sig pe_src_disabled_sig[] = {
...
static const state_sig pe_src_capability_response_sig[] = {
...
static const state_sig pe_src_hard_reset_sig[] = {
	pe_src_hard_reset_entry,
	pe_src_hard_reset_run,
	do_nothing,
	get_super_state
};

static const state_sig pe_src_hard_reset_received_sig[] = {
	pe_src_hard_reset_received_entry,
	pe_src_hard_reset_received_run,
	do_nothing,
	get_super_state
};

static const state_sig pe_src_transition_to_default_sig[] = {
...
static const state_sig pe_src_vdm_identity_sig[] = {
	pe_src_vdm_identity_entry,
	pe_src_vdm_identity_run,
	do_nothing,
	get_super_state
};

static const state_sig pe_snk_startup_sig[] = {
...
static const state_sig pe_snk_discovery_sig[] = {
...
static const state_sig pe_snk_wait_for_capabilities_sig[] = {
...
static const state_sig pe_snk_evaluate_capability_sig[] = {
...
static const state_sig pe_snk_select_capability_sig[] = {
...
static const state_sig pe_snk_transition_sink_sig[] = {
...
static const state_sig pe_snk_ready_sig[] = {
	pe_snk_ready_entry,
	pe_snk_ready_run,
	pe_snk_ready_exit,
	get_super_state
...
static const state_sig pe_snk_hard_reset_sig[] = {
...
static const state_sig pe_snk_transition_to_default_sig[] = {
...
static const state_sig pe_give_sink_cap_sig[] = {
	pe_give_sink_cap_entry,
	pe_give_sink_cap_run,
	do_nothing,
	get_super_state
};

static const state_sig pe_snk_get_source_cap_sig[] = {
...
static const state_sig pe_dr_snk_give_source_cap_sig[] = {
	pe_dr_snk_give_source_cap_entry,
	pe_dr_snk_give_source_cap_run,
	do_nothing,
	get_super_state
};

static const state_sig pe_send_soft_reset_sig[] = {
...
static const state_sig pe_soft_reset_sig[] = {
	pe_soft_reset_entry,
	pe_soft_reset_run,
	do_nothing,
	get_super_state
...
static const state_sig pe_send_not_supported_sig[] = {
...
static const state_sig pe_src_ping_sig[] = {
	pe_src_ping_entry,
	pe_src_ping_run,
	do_nothing,
	get_super_state
...
static const state_sig pe_give_battery_cap_sig[] = {
	pe_give_battery_cap_entry,
	pe_give_battery_cap_run,
	do_nothing,
	get_super_state
};

static const state_sig pe_give_battery_status_sig[] = {
	pe_give_battery_status_entry,
	pe_give_battery_status_run,
	do_nothing,
	get_super_state
};

static const state_sig pe_drs_evaluate_swap_sig[] = {
...
static const state_sig pe_drs_change_sig[] = {
	pe_drs_change_entry,
	pe_drs_change_run,
	do_nothing,
	get_super_state
...
static const state_sig pe_drs_send_swap_sig[] = {
...
static const state_sig pe_prs_src_snk_evaluate_swap_sig[] = {
	pe_prs_src_snk_evaluate_swap_entry,
	pe_prs_src_snk_evaluate_swap_run,
	do_nothing,
	get_super_state
};

static const state_sig pe_prs_src_snk_transition_to_off_sig[] = {
	pe_prs_src_snk_transition_to_off_entry,
	pe_prs_src_snk_transition_to_off_run,
	do_nothing,
	get_super_state
};

static const state_sig pe_prs_src_snk_assert_rd_sig[] = {
	pe_prs_src_snk_assert_rd_entry,
	pe_prs_src_snk_assert_rd_run,
	do_nothing,
	get_super_state
};

static const state_sig pe_prs_src_snk_wait_source_on_sig[] = {
	pe_prs_src_snk_wait_source_on_entry,
	pe_prs_src_snk_wait_source_on_run,
	pe_prs_src_snk_wait_source_on_exit,
	get_super_state
};

static const state_sig pe_prs_src_snk_send_swap_sig[] = {
	pe_prs_src_snk_send_swap_entry,
	pe_prs_src_snk_send_swap_run,
	pe_prs_src_snk_send_swap_exit,
	get_super_state
};


static const state_sig pe_prs_snk_src_evaluate_swap_sig[] = {
	pe_prs_snk_src_evaluate_swap_entry,
	pe_prs_snk_src_evaluate_swap_run,
	do_nothing,
	get_super_state
};

static const state_sig pe_prs_snk_src_transition_to_off_sig[] = {
	pe_prs_snk_src_transition_to_off_entry,
	pe_prs_snk_src_transition_to_off_run,
	do_nothing,
	get_super_state
};

static const state_sig pe_prs_snk_src_assert_rp_sig[] = {
	pe_prs_snk_src_assert_rp_entry,
	pe_prs_snk_src_assert_rp_run,
	do_nothing,
	get_super_state
};

static const state_sig pe_prs_snk_src_source_on_sig[] = {
	pe_prs_snk_src_source_on_entry,
	pe_prs_snk_src_source_on_run,
	do_nothing,
	get_super_state
};

static const state_sig pe_prs_snk_src_send_swap_sig[] = {
	pe_prs_snk_src_send_swap_entry,
	pe_prs_snk_src_send_swap_run, 
	pe_prs_snk_src_send_swap_exit,
	get_super_state
};

static const state_sig pe_bist_sig[] = {
	pe_bist_entry,
	pe_bist_run, 
	pe_bist_exit,
	get_super_state
...
static const state_sig pe_wait_for_error_recovery_sig[] = {
	pe_wait_for_error_recovery_entry,
	pe_wait_for_error_recovery_run, 
	pe_wait_for_error_recovery_exit,
	get_super_state
};

static const state_sig pe_handle_svdm_request_sig[] = {
	pe_handle_svdm_request_entry,
	pe_handle_svdm_request_run, 
	pe_handle_svdm_request_exit,
	get_super_state
};

static const state_sig pe_handle_custom_vdm_request_sig[] = {
	pe_handle_custom_vdm_request_entry,
	pe_handle_custom_vdm_request_run, 
	pe_handle_custom_vdm_request_exit,
	get_super_state
};
...
#ifdef CONFIG_USBC_VCONN
/* VCONN Swap */
static const state_sig pe_vcs_evaluate_swap_sig[] = {
	pe_vcs_evaluate_swap_entry,
	pe_vcs_evaluate_swap_run,
	do_nothing,
	get_super_state,
};

static const state_sig pe_vcs_send_swap_sig[] = {
	pe_vcs_send_swap_entry,
	pe_vcs_send_swap_run,
	do_nothing,
	get_super_state,
};

static const state_sig pe_vcs_wait_for_vconn_swap_sig[] = {
	pe_vcs_wait_for_vconn_swap_entry,
	pe_vcs_wait_for_vconn_swap_run,
	do_nothing,
	get_super_state,
};

static const state_sig pe_vcs_turn_on_vconn_swap_sig[] = {
	pe_vcs_turn_on_vconn_swap_entry,
	pe_vcs_turn_on_vconn_swap_run,
	do_nothing,
	get_super_state,
};

static const state_sig pe_vcs_turn_off_vconn_swap_sig[] = {
	pe_vcs_turn_off_vconn_swap_entry,
	pe_vcs_turn_off_vconn_swap_run,
	do_nothing,
	get_super_state,
};

static const state_sig pe_vcs_send_ps_rdy_swap_sig[] = {
	pe_vcs_send_ps_rdy_swap_entry,
	pe_vcs_send_ps_rdy_swap_run,
	do_nothing,
	get_super_state,
};
#endif
...
void pe_init(int port)
{
	...
	if (pe[port].power_role == PD_ROLE_SOURCE)
		init_state(port, PE_OBJ(port), pe_src_startup);
	else
		init_state(port, PE_OBJ(port), pe_snk_startup);
}
...
{
	return CHK_FLAG(port, PE_FLAGS_EXPLICIT_CONTRACT);
}

void pe_pass_up_message(int port)
{
	SET_FLAG(port, PE_FLAGS_MSG_RECEIVED);
	task_set_event(PD_PORT_TO_TASK_ID(port), PD_EVENT_SM, 0);
}

void pe_hard_reset_sent(int port)
{
	CLR_FLAG(port, PE_FLAGS_HARD_RESET_PENDING);
	task_set_event(PD_PORT_TO_TASK_ID(port), PD_EVENT_SM, 0);
}
...
void pe_got_hard_reset(int port)
{
	...
	pe[port].power_role = tc_get_power_role(port);
	if (pe[port].power_role == PD_ROLE_SOURCE)
		set_state(port, PE_OBJ(port), pe_src_hard_reset_received);
	else
		set_state(port, PE_OBJ(port), pe_snk_transition_to_default);
}

void pe_report_error(int port, enum pe_error e)
{
	/*
	 * Generate Hard Reset if Protocol Error occured
	 * while in PE_Send_Soft_Reset state.
	 */
	if (pe[port].obj.task_state == pe_send_soft_reset) {
		if (pe[port].power_role == PD_ROLE_SINK)
			set_state(port, PE_OBJ(port), pe_snk_hard_reset);
		else
			set_state(port, PE_OBJ(port), pe_src_hard_reset);
		return;
	}
	...
	if (pe[port].obj.task_state == pe_src_send_capabilities ||
		...
		SET_FLAG(port, PE_FLAGS_PROTOCOL_ERROR);
		return;
	}
	 ...
	 */
	if (!CHK_FLAG(port, PE_FLAGS_INTERRUPTIBLE_AMS) ||
					(e == ERR_TCH_XMIT)) {
		set_state(port, PE_OBJ(port), pe_send_soft_reset);
	}
	/*
	 ...
	 */
		...
		SET_FLAG(port, PE_FLAGS_PROTOCOL_ERROR);
		if (pe[port].power_role == PD_ROLE_SINK)
			set_state(port, PE_OBJ(port), pe_snk_ready);
		else
			set_state(port, PE_OBJ(port), pe_src_ready);
	}
}
...
void pe_got_soft_reset(int port)
{
	 ...
	 */
	set_state(port, PE_OBJ(port), pe_soft_reset);
}
...
void pe_dpm_request(int port, enum pe_dpm_request req)
{
	if (pe[port].state_id == PE_SRC_READY ||
		...
		SET_DPM_REQUEST(port, req);
}

void pe_vconn_swap_complete(int port)
{
	SET_FLAG(port, PE_FLAGS_VCONN_SWAP_COMPLETE);
	task_set_event(PD_PORT_TO_TASK_ID(port), PD_EVENT_SM, 0);
}

void pe_ps_reset_complete(int port)
{
	SET_FLAG(port, PE_FLAGS_PS_RESET_COMPLETE);
	task_set_event(PD_PORT_TO_TASK_ID(port), PD_EVENT_SM, 0);
}

void pe_message_sent(int port)
{
	SET_FLAG(port, PE_FLAGS_TX_COMPLETE);
	task_set_event(PD_PORT_TO_TASK_ID(port), PD_EVENT_SM, 0);
}
                        ...
                        int count)
{
	...
	SET_FLAG(port, PE_FLAGS_SEND_SVDM);
...
static void pe_prl_execute_hard_reset(int port)
{
	...
	SET_FLAG(port, PE_FLAGS_HARD_RESET_PENDING);
}
...
static unsigned int pe_src_startup_entry(int port)
{
	...
	/*
	 * Clear discover port discovery.
	 * Clear explicit contract.
	 */
	CLR_FLAG(port, PE_FLAGS_DISCOVER_PORT_IDENTITY_DONE |
	 ...
	 *
	 * NOTE: This delay also give time for a Cable and VCONN
	 *       Powered device to startup before sending a discover
	 *       identity message, so it is run every time
	 *       this state is entered.
	 */
	/*TODO(shurst): replace 50ms with macro */
	pe[port].swap_source_start_timer =
			get_time().val + PD_T_SWAP_SOURCE_START + (50*MSEC);
...
static unsigned int pe_src_startup_run(int port)
{
	/* Wait until protcol layer is done resetting */
	if (prl_get_local_state(port) != SM_RUN)
		return 0;

	if (get_time().val > pe[port].swap_source_start_timer)
		set_state(port, PE_OBJ(port), pe_src_vdm_identity);
...
static unsigned int pe_src_vdm_identity_run(int port)
{
	 ...
	 */
	if (!CHK_FLAG(port, PE_FLAGS_DISCOVER_CABLE_IDENTITY_DONE) &&
		...
		set_state(port, PE_OBJ(port), pe_handle_svdm_request);
		return 0;
	}

	set_state(port, PE_OBJ(port), pe_src_send_capabilities);
...
static unsigned int pe_src_discovery_run(int port)
{
	...
	if (pe[port].vpd_vdo >= 0 && VPD_VDO_CTS(pe[port].vpd_vdo)) {
		set_state(port, PE_OBJ(port), pe_src_disabled);
		return 0;
	}
	...
	if (get_time().val > pe[port].source_cap_timer) {
		if (pe[port].caps_counter <= N_CAPS_COUNT)
			set_state(port, PE_OBJ(port),
						pe_src_send_capabilities);
		else if (!CHK_FLAG(port, PE_FLAGS_PD_CONNECTION))
			set_state(port, PE_OBJ(port), pe_src_disabled);
		return 0;
	}
	 ...
	 */
	if (!CHK_FLAG(port,PE_FLAGS_PD_CONNECTION) &&
...
static unsigned int pe_src_send_capabilities_entry(int port)
{
	 ...
	 */
	CLR_FLAG(port, PE_FLAGS_INTERRUPTIBLE_AMS);
...
static unsigned int pe_src_send_capabilities_run(int port)
{
	 ...
	 */
	if (CHK_FLAG(port, PE_FLAGS_TX_COMPLETE) &&
				pe[port].sender_response_timer == 0) {
		CLR_FLAG(port, PE_FLAGS_TX_COMPLETE);
	 ...
	 */
	if (CHK_FLAG(port, PE_FLAGS_MSG_RECEIVED)) {
		CLR_FLAG(port, PE_FLAGS_MSG_RECEIVED);

		/* Looking for a data message */
		if (PD_HEADER_CNT(emsg[port].header) > 0) {
			...
			if (PD_HEADER_TYPE(emsg[port].header) ==
				...
				/* We are PD connected */
				SET_FLAG(port, PE_FLAGS_PD_CONNECTION);
	 ...
	 *       is not received.
	 */
	if (CHK_FLAG(port, PE_FLAGS_PROTOCOL_ERROR) &&
			!CHK_FLAG(port, PE_FLAGS_PD_CONNECTION)) {
		CLR_FLAG(port, PE_FLAGS_PROTOCOL_ERROR);
		set_state(port, PE_OBJ(port), pe_src_discovery);
		return 0;
	}
	...
	if (pe[port].no_response_timer > 0 &&
			...
			pe[port].hard_reset_counter > N_HARD_RESET_COUNT) {
		if (CHK_FLAG(port, PE_FLAGS_PD_CONNECTION))
			set_state(port, PE_OBJ(port), pe_wait_for_error_recovery);
		else
			set_state(port, PE_OBJ(port), pe_src_disabled);
		return 0;
	}
...
static unsigned int pe_src_negotiate_capability_entry(int port)
{
	...
	if (pd_check_requested_voltage(payload, port) != EC_SUCCESS) {
		set_state(port, PE_OBJ(port), pe_src_capability_response);
	} else {
		/* TODO(shurst): Implement GOTO_MIN */
		SET_FLAG(port, PE_FLAGS_ACCEPT);
		pe[port].requested_idx = RDO_POS(payload);
		set_state(port, PE_OBJ(port), pe_src_transition_supply);
	}
...
static unsigned int pe_src_transition_supply_entry(int port)
{
	...
	if (CHK_FLAG(port, PE_FLAGS_ACCEPT)) {
		CLR_FLAG(port, PE_FLAGS_ACCEPT);
...
static unsigned int pe_src_transition_supply_run(int port)
{
	...
	if (CHK_FLAG(port, PE_FLAGS_TX_COMPLETE)) {
		...
		if (CHK_FLAG(port, PE_FLAGS_PS_READY)) {
			CLR_FLAG(port, PE_FLAGS_PS_READY);
			/* NOTE: Second pass through this code block */
			/* Explicit Contract is now in place */
			SET_FLAG(port, PE_FLAGS_EXPLICIT_CONTRACT);
			set_state(port, PE_OBJ(port), pe_src_ready);
		} else {
			...
			SET_FLAG(port, PE_FLAGS_PS_READY);
		}
...
static unsigned int pe_src_ready_entry(int port)
{
	...
	/*
	 * If this is a DFP VCONN SRC which needs to establish communication
	 * with a Cable Plug, the DFP Shall:
	 *   1) Initialize and run the DiscoverIdentityTimer
	 */
	if (pe[port].data_role == PD_ROLE_DFP &&
			!CHK_FLAG(port, PE_FLAGS_MODAL_OPERATION) &&
		!CHK_FLAG(port, PE_FLAGS_DISCOVER_PORT_IDENTITY_DONE)) {
			pe[port].port_discover_identity_count = 0;
	...
	/*
	 * If the current Explicit Contract is for a PPS APDO, then the Policy
	 * Engine Shall do the following:
	 *   1) Initialize and run the SourcePPSCommTimer.
	 */
	
	/* TODO(shurst): Implement PPS */
...
static unsigned int pe_src_ready_run(int port)
{
	...
	if (pe[port].dpm_request) {
		if (CHK_DPM_REQUEST(port, DPM_REQUEST_DR_SWAP)) {
			CLR_DPM_REQUEST(port, DPM_REQUEST_DR_SWAP);
			if (CHK_FLAG(port, PE_FLAGS_MODAL_OPERATION))
				set_state(port, PE_OBJ(port),
							pe_src_hard_reset);
			else
				set_state(port, PE_OBJ(port),
		...
		else if (CHK_DPM_REQUEST(port, DPM_REQUEST_PR_SWAP)) {
			CLR_DPM_REQUEST(port, DPM_REQUEST_PR_SWAP);
			set_state(port, PE_OBJ(port),
						pe_prs_src_snk_send_swap);
		}
#ifdef CONFIG_USBC_VCONN
		else if (CHK_DPM_REQUEST(port, DPM_REQUEST_VCONN_SWAP)) {
			CLR_DPM_REQUEST(port, DPM_REQUEST_VCONN_SWAP);
			set_state(port, PE_OBJ(port), pe_vcs_send_swap);
		}
#endif
		else if (CHK_DPM_REQUEST(port, DPM_REQUEST_GOTO_MIN)) {
			CLR_DPM_REQUEST(port, DPM_REQUEST_GOTO_MIN);
			set_state(port, PE_OBJ(port),
						pe_src_transition_supply);
		}
		else if (CHK_DPM_REQUEST(port, DPM_REQUEST_SRC_CAP_CHANGE)) {
			CLR_DPM_REQUEST(port, DPM_REQUEST_SRC_CAP_CHANGE);
			set_state(port, PE_OBJ(port),
						pe_src_send_capabilities);
		}
#if 0
		else if (CHK_DPM_REQUEST(port, DPM_REQUEST_GET_SNK_CAPS)) {
			CLR_DPM_REQUEST(port, DPM_REQUEST_GET_SNK_CAPS);
			set_state(port, PE_OBJ(port), pe_src_get_sink_cap);
		}
#endif
		else if (CHK_DPM_REQUEST(port, DPM_REQUEST_SEND_PING)) {
			CLR_DPM_REQUEST(port, DPM_REQUEST_SEND_PING);
			set_state(port, PE_OBJ(port), pe_src_ping);
		}
		else if (CHK_DPM_REQUEST(port, DPM_REQUEST_SOURCE_CAP)) {
			CLR_DPM_REQUEST(port, DPM_REQUEST_SOURCE_CAP);
			set_state(port, PE_OBJ(port), pe_snk_get_source_cap);
		}
		else if (CHK_DPM_REQUEST(port, DPM_REQUEST_NEW_POWER_LEVEL)) {
			CLR_DPM_REQUEST(port, DPM_REQUEST_NEW_POWER_LEVEL);
			set_state(port, PE_OBJ(port),
						pe_snk_select_capability);
		}
		else if (CHK_DPM_REQUEST(port, DPM_REQUEST_DISCOVER_IDENTITY)) {
			CLR_DPM_REQUEST(port, DPM_REQUEST_DISCOVER_IDENTITY);
	...
	if (CHK_FLAG(port, PE_FLAGS_SEND_SVDM)) {
		CLR_FLAG(port, PE_FLAGS_SEND_SVDM);
		set_state(port, PE_OBJ(port), pe_handle_svdm_request);
		return 0;
	}
	...
	if (CHK_FLAG(port, PE_FLAGS_MSG_RECEIVED)) {
		...
		/* Data Message Requests */
			...
			case PD_DATA_VENDOR_DEF:
				...
				break;
			case PD_DATA_BIST:
				set_state(port, PE_OBJ(port), pe_bist);
				break;
			default:
				set_state(port, PE_OBJ(port),
					pe_send_not_supported);
...
static unsigned int pe_src_ready_exit(int port)
{
	CLR_FLAG(port, PE_FLAGS_TX_COMPLETE | PE_FLAGS_PROTOCOL_ERROR);
...
static unsigned int pe_src_capability_response_entry(int port)
{
	...
	/* TODO(shurst): Implement Wait Messaging */
	prl_send_ctrl_msg(port, TCPC_TX_SOP, PD_CTRL_REJECT);
...
static unsigned int pe_src_capability_response_run(int port)
{
	 ...
	 * Transition to the PE_SRC_Hard_Reset state when:
	 *  1) There is an Explicit Contract and
	 *  2) The Reject Message has been sent and the present Contract is Invalid
	 *
	...
	if (CHK_FLAG(port, PE_FLAGS_TX_COMPLETE)) {
		...
		if (CHK_FLAG(port, PE_FLAGS_EXPLICIT_CONTRACT))
			/*
			 ...
			 * NOTE: The src capabilities listed in
			 *       board/xxx/usb_pd_policy.c will not
			 *       change, so no since in resending them
...
static unsigned int pe_src_hard_reset_run(int port)
{
	...
	if (get_time().val > pe[port].ps_hard_reset_timer)
		set_state(port, PE_OBJ(port), pe_src_transition_to_default);
...
static unsigned int pe_src_transition_to_default_run(int port)
{
	 ...
	 */
	if (CHK_FLAG(port, PE_FLAGS_PS_RESET_COMPLETE)) {
		CLR_FLAG(port, PE_FLAGS_PS_RESET_COMPLETE);
		/* Inform the Protocol Layer that the Hard Reset is complete */
		prl_hard_reset_complete(port);
		set_state(port, PE_OBJ(port), pe_src_startup);
	}
...
ccprintf("SNK_START\n");
	 ...
	 * Clear explicit contract.
	 */
	CLR_FLAG(port, PE_FLAGS_EXPLICIT_CONTRACT |
...
static unsigned int pe_snk_startup_run(int port)
{
	 ...
	 */
	if (prl_get_local_state(port) == SM_RUN)
		set_state(port, PE_OBJ(port), pe_snk_discovery);
...
static unsigned int pe_snk_discovery_run(int port)
{
	...
	if (pd_is_vbus_present(port))
		set_state(port, PE_OBJ(port), pe_snk_wait_for_capabilities);
...
static unsigned int pe_snk_wait_for_capabilities_entry(int port)
{
ccprintf("PE_SNK_WAIT_FOR_CAPABILITIES\n");
	/* Set state id */
...
static unsigned int pe_snk_wait_for_capabilities_run(int port)
{
	...
	if (CHK_FLAG(port, PE_FLAGS_MSG_RECEIVED)) {
		...
		if ((ext == 0) && (cnt > 0) && (type == PD_DATA_SOURCE_CAP)) {
			set_state(port, PE_OBJ(port), pe_snk_evaluate_capability);
			return 0;
		}
	}
	...
	if (get_time().val > pe[port].timeout) {
		SET_FLAG(port, PE_FLAGS_SNK_WAIT_CAP_TIMEOUT);
		set_state(port, PE_OBJ(port), pe_snk_hard_reset);
	}
...
ccprintf("PE_SNK_EVALUATE_CAPABILITY\n");
	/* Set state id */
	...
	set_state(port, PE_OBJ(port), pe_snk_select_capability);
...
static unsigned int pe_snk_select_capability_entry(int port)
{
ccprintf("PE_SNK_SELECT_CAPABILITY\n");
	/* Set state id */
...
static unsigned int pe_snk_select_capability_run(int port)
{
	...
	if (pe[port].sender_response_timer == 0 &&
			(CHK_FLAG(port, PE_FLAGS_TX_COMPLETE) ||
			CHK_FLAG(port, PE_FLAGS_MSG_RECEIVED))) {
		CLR_FLAG(port, PE_FLAGS_TX_COMPLETE);
...
static unsigned int pe_snk_transition_sink_entry(int port)
{
ccprintf("PE_SNK_TRANSITION_SINK\n");
	/* Set state id */
...
static unsigned int pe_snk_transition_sink_run(int port)
{
	...
	if (get_time().val > pe[port].ps_transition_timer &&
			pe[port].hard_reset_counter <= N_HARD_RESET_COUNT) {
		SET_FLAG(port, PE_FLAGS_PS_TRANSITION_TIMEOUT);
...
static unsigned int pe_snk_transition_sink_exit(int port)
{
	...
	charge_manager_set_ceil(port, CEIL_REQUESTOR_PD, pe[port].curr_limit);
#endif
	return 0;
}
...
ccprintf("PE_SNK_READY\n");
	 ...
	 */
	if (CHK_FLAG(port, PE_FLAGS_WAIT)) {
		CLR_FLAG(port, PE_FLAGS_WAIT);
		pe[port].sink_request_timer = get_time().val + PD_T_SINK_REQUEST;
	}
	else {
	...
	/*
	 * On entry to the PE_SNK_Ready state if this is a DFP which needs to
	 * establish communication with a Cable Plug, then do the following:
	 *   1) Initialize and run the DiscoverIdentityTimer
	 */
//	if (pe[port].data_role == PD_ROLE_DFP &&
				if (!CHK_FLAG(port, PE_FLAGS_MODAL_OPERATION) &&
	...
	if (pe[port].dpm_request) {
		if (CHK_DPM_REQUEST(port, DPM_REQUEST_DR_SWAP)) {
			...
			if (CHK_FLAG(port, PE_FLAGS_MODAL_OPERATION))
				...
				set_state(port, PE_OBJ(port), pe_drs_send_swap);
		}
		else if (CHK_DPM_REQUEST(port, DPM_REQUEST_PR_SWAP)) {
			CLR_DPM_REQUEST(port, DPM_REQUEST_PR_SWAP);
			set_state(port, PE_OBJ(port), pe_prs_snk_src_send_swap);
		}
#ifdef CONFIG_USBC_VCONN
		else if (CHK_DPM_REQUEST(port, DPM_REQUEST_VCONN_SWAP)) {
			CLR_DPM_REQUEST(port, DPM_REQUEST_VCONN_SWAP);
...
static unsigned int pe_snk_hard_reset_entry(int port)
{
	 ...
	 */
	if (CHK_FLAG(port, PE_FLAGS_SNK_WAIT_CAP_TIMEOUT) &&
			pe[port].hard_reset_counter > N_HARD_RESET_COUNT) {
		set_state(port, PE_OBJ(port), pe_src_disabled);
	}

	CLR_FLAG(port, PE_FLAGS_SNK_WAIT_CAP_TIMEOUT);

	/* Rquest the generation of Hard Reset Signaling by the PHY Layer */
	 ...
	 * PSTransistionTimer timeout occured.
	 */
	if (CHK_FLAG(port, PE_FLAGS_PS_TRANSITION_TIMEOUT)) {
		SET_FLAG(port, PE_FLAGS_PS_TRANSITION_TIMEOUT);
...
static unsigned int pe_snk_hard_reset_run(int port)
{
	 ...
	 */
	if (CHK_FLAG(port, PE_FLAGS_HARD_RESET_PENDING))
		return 0;
...
static unsigned int pe_snk_transition_to_default_run(int port)
{
	if (CHK_FLAG(port, PE_FLAGS_PS_RESET_COMPLETE)) {
		CLR_FLAG(port, PE_FLAGS_PS_RESET_COMPLETE);
		set_state(port, PE_OBJ(port), pe_snk_startup);
	}
	return 0;
}
 ...
 */
static unsigned int pe_snk_give_sink_cap(int port, enum signal sig)
{
	int ret;
...
static unsigned int pe_snk_give_sink_cap_entry(int port)
{
...
static unsigned int pe_snk_give_sink_cap_run(int port)
{
	if (CHK_FLAG(port, PE_FLAGS_TX_COMPLETE)) {
		CLR_FLAG(port, PE_FLAGS_TX_COMPLETE);
...
static unsigned int pe_snk_give_sink_cap_exit(int port)
{
	return 0;
}
#endif
...
static unsigned int pe_dr_snk_give_source_cap(int port, enum signal sig)
{
	...
	ret = (*pe_dr_snk_give_source_cap_sig[sig])(port);
	return SUPER(ret, sig, 0);
...
static unsigned int pe_send_soft_reset_run(int port)
{
	 ...
	 */
	if (get_time().val > pe[port].sender_response_timer ||
			CHK_FLAG(port, PE_FLAGS_PROTOCOL_ERROR)) {
		CLR_FLAG(port, PE_FLAGS_PROTOCOL_ERROR);
...
static unsigned int pe_soft_reset_run(int port)
{
	...
	else if (CHK_FLAG(port, PE_FLAGS_PROTOCOL_ERROR)) {
		CLR_FLAG(port, PE_FLAGS_PROTOCOL_ERROR);
 ...
 * PE_Get_Battery_Cap
 */
static unsigned int pe_get_battery_cap(int port, enum signal sig)
{
	int ret;

	ret = (pe_get_battery_cap_sig[sig])(port);
	return SUPER(ret, sig, 0);
}

static unsigned int pe_get_battery_cap_entry(int port)
{
	prl_send_ctrl_msg(port, TCPC_TX_SOP, PD_EXT_GET_BATTERY_CAP);
	/* Initialize and run the SenderResponseTimer */
	pe[port].sender_response_timer = get_time().val + PD_T_SENDER_RESPONSE;
	return 0;
}

static unsigned int pe_get_battery_cap_run(int port)
{
	int type;
	int cnt;
	int ext;
	...
	/*
	 * Transition back to the PE_SNK_Ready or PE_SRC_Ready state when:
	 *   1) A Battery_Capabilities Message is received
	...
	if (CHK_FLAG(port, PE_FLAGS_MSG_RECEIVED)) {
		...
		cnt = PD_HEADER_CNT(emsg[port].header);
		ext = PD_HEADER_EXT(emsg[port].header);
		msg = (uint16_t 8)emsg[port].buf;

		if ((ext == 1) && (cnt == 3) && (type == PD_EXT_BATTERY_CAP)) {
			/*
			 * Inform the Device Policy Manager of the
			 * capabilities.
			 */

			/*
			 * msg[0] - Extended Header
			 *
			 * msg[1] - VID
			 *
			 * msg[2] - PID
			 *
			 * msg[3] - The Battery Design Capacity field shall
			 *          return the Battery’s design capacity in
			 *          tenths of Wh. If the Battery is Hot
			 *          Swappable and is not present, the Battery
			 *          Design Capacity field shall be set to 0.
			 *          If the Battery is unable to report its
			 *          Design Capacity, it shall return 0xFFFF
			 *
			 * msg[4] - The Battery Last Full Charge Capacity field
			 *          shall return the Battery’s last full charge
			 *          capacity in tenths of Wh. If the Battery is
			 *          Hot Swappable and is not present, the
			 *          Battery Last Full Charge Capacity field
			 *          shall be set to 0. If the Battery is unable
			 *          to report its Design Capacity, the Battery
			 *          Last Full Charge Capacity field shall be
			 *          set to 0xFFFF.
			 *
			 * msg[5] - Invalid battery reference.
			 */
...
static unsigned int pe_drs_evaluate_swap_run(int port)
{
	if (CHK_FLAG(port, PE_FLAGS_TX_COMPLETE)) {
		...
		if (CHK_FLAG(port, PE_FLAGS_ACCEPT)) {
			CLR_FLAG(port, PE_FLAGS_ACCEPT);
			set_state(port, PE_OBJ(port), pe_drs_change);
		}
		else {
			/*
...
static unsigned int pe_drs_send_swap_run(int port)
{
	 ...
	 */
		...
		if ((ext == 0) && (cnt == 0)) {
			...
			else if ((type == PD_CTRL_REJECT) || (type == PD_CTRL_WAIT)) {
				if (pe[port].power_role == PD_ROLE_SINK)
					set_state(port, PE_OBJ(port), pe_snk_ready);
				else
					set_state(port, PE_OBJ(port), pe_src_ready);
...
static unsigned int pe_prs_src_snk_evaluate_swap_run(int port)
{
	if (CHK_FLAG(port, PE_FLAGS_TX_COMPLETE)) {
		...
		if (CHK_FLAG(port, PE_FLAGS_ACCEPT)) {
			 ...
			 */
			set_state(port, PE_OBJ(port), pe_prs_src_snk_transition_to_off);
		}
		else {
...
static unsigned int pe_prs_src_snk_transition_to_off_entry(int port)
{
	...
	pe[port].ps_source_timer = get_time().val + PD_POWER_SUPPLY_TURN_OFF_DELAY;
#else
...
static unsigned int pe_prs_src_snk_transition_to_off_run(int port)
{
	...
	if (get_time().val > pe[port].ps_source_timer)
		set_state(port, PE_OBJ(port), pe_prs_src_snk_assert_rd);
#else
	if (get_typec_state_id(port) == ATTACHED_SNK && pe[port].ps_source_timer == 0) {
		/* Contract is invalid */
                CLR_FLAG(port, PE_FLAGS_EXPLICIT_CONTRACT);
		pe[port].ps_source_timer = get_time().val + PD_POWER_SUPPLY_TURN_OFF_DELAY;
	}
...
static unsigned int pe_prs_src_snk_assert_rd(int port, enum signal sig)
{
	...
	ret = (pe_prs_src_snk_assert_rd_sig[sig])(port);
	return SUPER(ret, sig, 0);
}
...
#if DEBUG_DP
	if (get_typec_state_id(port) == ATTACHED_SNK) {
		/* Contract is invalid */
		CLR_FLAG(port, PE_FLAGS_EXPLICIT_CONTRACT);
		set_state(port, PE_OBJ(port), pe_prs_src_snk_wait_source_on);
	}
#else
	set_state(port, PE_OBJ(port), pe_prs_src_snk_wait_source_on);
#endif
...
static unsigned int pe_prs_snk_src_evaluate_swap_run(int port)
{
	if (CHK_FLAG(port, PE_FLAGS_TX_COMPLETE)) {
		...
		if (CHK_FLAG(port, PE_FLAGS_ACCEPT)) {
			 ...
			 */
			set_state(port, PE_OBJ(port), pe_prs_snk_src_transition_to_off);
		}
		else {
...
static unsigned int pe_prs_snk_src_transition_to_off_run(int port)
{
	 ...
	 */
		...
		if ((ext == 0) && (cnt == 0)) {
			if (type == PD_CTRL_PS_RDY) {
				set_state(port, PE_OBJ(port), pe_prs_snk_src_assert_rp);
			}
		}
	}
...
static unsigned int pe_prs_snk_src_assert_rp_run(int port)
{
	...
	if (get_typec_state_id(port) == ATTACHED_SRC) {
		/* Contract is invalid now */
		CLR_FLAG(port, PE_FLAGS_EXPLICIT_CONTRACT);
		set_state(port, PE_OBJ(port), pe_prs_snk_src_source_on);
	}
	return 0;
}
...
static unsigned int pe_prs_snk_src_source_on_entry(int port)
{
	...
	pe[port].ps_source_timer = get_time().val + PD_POWER_SUPPLY_TURN_ON_DELAY;
...
static unsigned int pe_prs_snk_src_source_on_run(int port)
{
	if (pe[port].ps_source_timer > 0 && get_time().val < pe[port].ps_source_timer)
		return 0;
	...
	if (CHK_FLAG(port, PE_FLAGS_TX_COMPLETE)) {
		...
		SET_FLAG(port, PE_FLAGS_POWER_ROLE_SWAP);
		tc_pr_swap_complete(port);
		set_state(port, PE_OBJ(port), pe_src_startup);
	}
...
static unsigned int pe_bist_entry(int port)
{
	...
	ccprintf("\t%d: BIST mode %d\n", port, mode);

	if (mode == 5) {
		ccprintf("\t\t\t BIST CARRIER MODE\n");
		prl_send_ctrl_msg(port, TCPC_TX_BIST_MODE_2, 0);
		pe[port].bist_cont_mode_timer = get_time().val + (60 * MSEC);
	}
	else if (mode == 8) {
		ccprintf("\t\t\t BIST DATA\n");
...
static unsigned int pe_bist_run(int port)
{
	if (pe[port].bist_cont_mode_timer > 0 &&
		...
		if (pe[port].power_role == PD_ROLE_SOURCE)
			init_state(port, PE_OBJ(port), pe_src_transition_to_default);
		else
			init_state(port, PE_OBJ(port), pe_snk_transition_to_default);
	}
	else {
		/*
		 * We are in test mode and no further Messages except for
		 * GoodCRC Messages in response to received Messages will be sent.
		 */
		if (CHK_FLAG(port, PE_FLAGS_MSG_RECEIVED))
			CLR_FLAG(port, PE_FLAGS_MSG_RECEIVED);
	}
...
static unsigned int pe_give_sink_cap(int port, enum signal sig)
{
	...
	ret = (*pe_give_sink_cap_sig[sig])(port);
	return SUPER(ret, sig, 0);
}
...
static unsigned int pe_handle_custom_vdm_request_entry(int port)
{
	...
	SET_FLAG(port, PE_FLAGS_INTERRUPTIBLE_AMS);
...
static unsigned int pe_handle_custom_vdm_request_run(int port)
{
	...
	if (CHK_FLAG(port, PE_FLAGS_TX_COMPLETE)) {
		...
		/* Message sent. Transition back to PE_SRC_Ready or PE_SINK_Ready */
		if (pe[port].power_role == PD_ROLE_SOURCE)
			set_state(port, PE_OBJ(port), pe_src_ready);
		else
			set_state(port, PE_OBJ(port), pe_snk_ready);
...
static unsigned int pe_handle_svdm_request_entry(int port)
{
	...
	if (pe[port].vdm_cmd == DISCOVER_IDENTITY) {
		...
		*(uint32_t *)emsg[port].buf =
		...
		if (pe[port].is_cable) {
			/* Save power role */
			int saved_power_role = tc_get_power_role(port);
			...
			prl_send_data_msg(port, TCPC_TX_SOP_PRIME,
							PD_DATA_VENDOR_DEF);
			/* Restore power role */
			tc_set_power_role(port, saved_power_role);
		}
		else {
...
static unsigned int pe_handle_svdm_request_run(int port)
{
	...
	case VDM_WAIT_FOR_RESPONSE:
		if (get_time().val > pe[port].vdm_response_timer) {
			...
			/*
                         ...
                         */
			if (pe[port].is_cable)
				SET_FLAG(port, PE_FLAGS_DISCOVER_CABLE_IDENTITY_DONE);
			else
				SET_FLAG(port, PE_FLAGS_DISCOVER_PORT_IDENTITY_DONE);
	...
	if (cmd_type == CMDT_INIT) {
		...
		/*
		 * Set PE_FLAGS_DISCOVER_PORT_IDENTITY_DONE flag so returning state
		 * doesn't try to detect the port again.
		 */
		SET_FLAG(port, PE_FLAGS_DISCOVER_PORT_IDENTITY_DONE);
#ifdef CONFIG_USB_PD_ALT_MODE_DFP
...
static unsigned int pe_vcs_turn_on_vconn_swap_run(int port)
{
	...
	if (pe[port].timeout == 0 &&
			CHK_FLAG(port, PE_FLAGS_VCONN_SWAP_COMPLETE)) {
		CLR_FLAG(port, PE_FLAGS_VCONN_SWAP_COMPLETE);
		pe[port].timeout = get_time().val + PD_VCONN_SWAP_DELAY;
	}

	if (pe[port].timeout > 0 && get_time().val > pe[port].timeout)
		set_state(port, PE_OBJ(port), pe_vcs_send_ps_rdy_swap);
...
static void dfp_consume_modes(int port, int cnt, uint32_t *payload)
{
        ...
        if (pe[port].am.svids[idx].mode_cnt < 0) {
                ...
                memcpy(pe[port].am.svids[pe[port].am.svid_idx].mode_vdo, &payload[1],
...
uint32_t pd_dfp_enter_mode(int port, uint16_t svid, int opos)
{
	...
	SET_FLAG(port, PE_FLAGS_MODAL_OPERATION);
...
int pd_dfp_exit_mode(int port, uint16_t svid, int opos)
{
	...
	CLR_FLAG(port, PE_FLAGS_MODAL_OPERATION);
