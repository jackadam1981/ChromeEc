...
ccprintf("PE_SRC_STARTUP\n");
	 ...
	 */
	/*TODO(shurst): remove 20ms */
	pe[port].swap_source_start_timer =
			get_time().val + PD_T_SWAP_SOURCE_START + (20*MSEC);
...
static unsigned int pe_snk_ready_entry(int port)
{
	 ...
	 */
	if (pe[port].data_role == PD_ROLE_DFP) {
		if (!CHK_FLAG(port, PE_FLAGS_DISCOVER_PORT_IDENTITY_DONE)) {
			pe[port].port_discover_identity_count = 0;
			pe[port].discover_identity_timer =
					get_time().val + PD_T_DISCOVER_IDENTITY;
		}
...
static unsigned int pe_handle_svdm_request_entry(int port)
{
ccprintf("PE_HANDLE_SVDM_REQUEST\n");
	/* This is an Interruptible AMS */
	SET_FLAG(port, PE_FLAGS_INTERRUPTIBLE_AMS);
