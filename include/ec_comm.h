...
enum ec_comm_commands {
	EC_COMM_STATIC_INFO = 10,
	EC_COMM_DYNAMIC_INFO = 11,
...
struct ec_comm_dynamic_info {
	/*
	 * Charger current (mA): when base draws current from lid: positive
	 * number indicating power limit. In OTG mode, negative number indicates
	 * maximum current.
	 */
	int16_t charger_current;
	/*
	 * Charger OTG voltage (mV): 0 when base draws current from lid, OTG
	 * voltage otherwise.
	 */
	int16_t charger_otg_voltage;
