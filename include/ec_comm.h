...
enum ec_comm_commands {
	EC_COMM_STATIC_INFO = 0x10,
	EC_COMM_DYNAMIC_INFO = 0x11,
...
struct ec_comm_static_info {
	uint16_t usb_product_id; /* USB product ID for this base. */
	uint8_t supported_protocol_version; /* Should be 0 */
	uint8_t reserved;
...
BUILD_ASSERT((sizeof(struct ec_comm_static_info) % 4) == 0);

struct ec_comm_dynamic_info {
...
BUILD_ASSERT((sizeof(struct ec_comm_dynamic_info) % 4) == 0);
...
extern struct ec_comm_static_info base_static;
extern struct ec_comm_dynamic_info base_dynamic;

/* Packed TX/RX structures for the commands above, used by the lid. */
struct ec_comm_static_info_tx_rx {
	struct {
		uint32_t head;
		uint32_t crc32;
	} tx;
	struct {
		uint32_t head;
		struct ec_comm_static_info info;
		uint32_t crc32;
	} rx;
} __packed;

struct ec_comm_dynamic_info_tx_rx {
	struct {
		uint32_t head;
		uint32_t crc32;
	} tx;
	struct {
		uint32_t head;
		struct ec_comm_dynamic_info info;
		uint32_t crc32;
	} rx;
} __packed;
