...
static struct type_c {
	...
	/* state machine event */
	int evt;
	/* Time a ct port shall wait before it can determine it is attached */
...
static unsigned int tc_state_enable_pd(int port, enum signal sig);
static unsigned int tc_state_enable_pd_entry(int port);
static unsigned int tc_state_enable_pd_run(int port);
static unsigned int tc_state_enable_pd_exit(int port);
...
static const state_sig tc_state_enable_pd_sig[] = {
	tc_state_enable_pd_entry,
	tc_state_enable_pd_run,
	tc_state_enable_pd_exit,
	get_super_state
};
...
static unsigned int tc_state_disabled_entry(int port)
{
	SET_TEST(TC_TEST_OBJ(port), DISABLED);
...
static unsigned int tc_state_error_recovery_entry(int port)
{
	SET_TEST(TC_TEST_OBJ(port), ERROR_RECOVERY);
...
static unsigned int tc_state_unattached_snk_entry(int port)
{
	SET_TEST(TC_TEST_OBJ(port), UNATTACHED_SNK);
...
static unsigned int tc_state_unattached_snk_run(int port)
{
	...
	/* Check Charge-Through CC for connection */
	...
	if (host_new_cc_state != tc[port].host_cc_state) {
		tc[port].host_cc_state = host_new_cc_state;
		tc[port].host_cc_debounce = get_time().val + PD_T_DRP_SRC;
	}
...
static unsigned int tc_state_attach_wait_snk_entry(int port)
{
	SET_TEST(TC_TEST_OBJ(port), ATTACH_WAIT_SNK);
...
static unsigned int tc_state_attach_wait_snk_run(int port)
{
	...
	if (tc[port].host_cc_state != host_new_cc_state) {
		tc[port].host_cc_state = host_new_cc_state;
		tc[port].host_cc_debounce = get_time().val + PD_T_CC_DEBOUNCE;
		return 0;
	}
...
static unsigned int tc_state_attached_snk_entry(int port)
{
	SET_TEST(TC_TEST_OBJ(port), ATTACHED_SNK);
...
static unsigned int tc_state_unattached_src_entry(int port)
{
	SET_TEST(TC_TEST_OBJ(port), UNATTACHED_SRC);
...
static unsigned int tc_state_attach_wait_src_entry(int port)
{
	SET_TEST(TC_TEST_OBJ(port), ATTACH_WAIT_SRC);
...
static unsigned int tc_state_attach_wait_src_run(int port)
{
	...
	if (host_new_cc_state == PD_CC_NONE || !vpd_is_ct_vbus()) {
		set_state(port, TC_OBJ(port), tc_state_unattached_snk);
		return 0;
	}
...
static unsigned int tc_state_attached_src_entry(int port)
{
	SET_TEST(TC_TEST_OBJ(port), ATTACHED_SRC);
...
static unsigned int tc_state_try_snk_entry(int port)
{
	SET_TEST(TC_TEST_OBJ(port), TRY_SNK);
...
static unsigned int tc_state_try_wait_src_entry(int port)
{
	SET_TEST(TC_TEST_OBJ(port), TRY_WAIT_SRC);
...
static unsigned int tc_state_ct_try_snk_entry(int port)
{
	SET_TEST(TC_TEST_OBJ(port), CTTRY_SNK);
...
static unsigned int tc_state_ct_attach_wait_unsupported_entry(int port)
{
	SET_TEST(TC_TEST_OBJ(port), CTATTACH_WAIT_UNSUPPORTED);
...
static unsigned int tc_state_ct_attached_unsupported_entry(int port)
{
	SET_TEST(TC_TEST_OBJ(port), CTATTACHED_UNSUPPORTED);
...
static unsigned int tc_state_ct_unattached_unsupported_entry(int port)
{
	SET_TEST(TC_TEST_OBJ(port), CTUNATTACHED_UNSUPPORTED);
...
static unsigned int tc_state_ct_unattached_vpd_entry(int port)
{
	SET_TEST(TC_TEST_OBJ(port), CTUNATTACHED_VPD);
...
static unsigned int tc_state_ct_disabled_vpd_entry(int port)
{
	SET_TEST(TC_TEST_OBJ(port), CTDISABLED_VPD);
...
static unsigned int tc_state_ct_attached_vpd_entry(int port)
{
	...
	SET_TEST(TC_TEST_OBJ(port), CTATTACHED_VPD);
...
static unsigned int tc_state_ct_attach_wait_vpd_entry(int port)
{
	SET_TEST(TC_TEST_OBJ(port), CTATTACH_WAIT_VPD);
...
static unsigned int tc_state_host_rp3_ct_rd(int port, enum signal sig)
{
	...
	return SUPER(ret, sig, tc_state_enable_pd);
}
...
/*
 * Super State Enable_PD
 */
static unsigned int tc_state_enable_pd(int port, enum signal sig)
{
	int ret;

	ret = (*tc_state_enable_pd_sig[sig])(port);
	return SUPER(ret, sig, tc_state_vbus_cc_iso);
}
