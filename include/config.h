 ...
 * in ROM/flash. This is useful on on ECs that execute all code from RAM and
 * the RAM size is smaller than the flash size.
 *
 * Code can force objects into the .init_rom resident section using the
 * __init_rom macro. Objects should be copied into temporary RAM storage using
 * flash_read() at runtime.
 */
...
/*
 * When program memory is smaller than flash, the ROM_RESIDENT section
 * can be enabled using CONFIG_CHIP_INIT_ROM_REGION.
 */
