 ...
 * CONFIG_EMULATED_SYSRQ is enabled for the board, the EC will send a SysRq to
 * trigger a kernel panic in the AP. The kernel panic will output the stacks for
 * the active CPUs and then reboot the AP. After the EC sends the SysRq, it will
 * wait CONFIG_HARD_SLEEP_HANG_TIMEOUT for the AP to perform the reboot. If the
 * AP fails to reboot within CONFIG_HARD_SLEEP_HANG_TIMEOUT, the EC will
 * forcefully reset the AP to complete recovery.
 */
