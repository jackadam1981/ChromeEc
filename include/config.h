...
/*
 * Only check exclusiveness when not define CONFIG_MKBP_USE_CUSTOM.
 * Some boards may need all of these methods, e.g. nocturne.
 */
#if !defined(CONFIG_MKBP_USE_CUSTOM)
#if defined(CONFIG_MKBP_USE_GPIO) + defined(CONFIG_MKBP_USE_HOST_EVENT) > 1
#error Must select only one type of MKBP event delivery method.
#endif
#endif /* !CONFIG_MKBP_USE_CUSTOM */
