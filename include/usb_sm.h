 ...
 * parent- Optional parent usb_state that contains comment entry/run/exit
...
struct sm_ctx {
	...
	/* We use intptr_t type to accomidate host tests ptr size variance */
	intptr_t internal[2];
};
 ...
 * entering the taraget state. A common parent state will not exitted nor be
 * re-entered.
 *
 * @param port      USB-C port number
 * @param ctx       State machine context
 * @param new_state State to transition to (NULL is valid)
 */
