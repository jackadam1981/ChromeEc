...
#define __ARG_PLACEHOLDER_0 _, 0,
#define __ARG_PLACEHOLDER_1 _, 1,
...
#define __config_enabled(arg1_or_junk, cfg, value) ___config_enabled( \
	...
	({ \
		...
		extern int IS_ENABLED_BAD_ARGS(void) __attribute__(( \
			error(cfg " must be <blank>, 0, 1, or not defined.")));\
		if (!__undefined) \
			IS_ENABLED_BAD_ARGS(); \
		0; \
	}))
 ...
 *     #define CONFIG_BAR 0
 *
 * If the option is defined to any other value a compiler error will be thrown.
 *
