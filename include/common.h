...
/**
 * Makes a global variable static if it is enabled, extern otherwise
 * (with the intention to cause linker errors if the variable is used
 * outside of a config context, for example thru IS_ENABLED, that it
 * should be).
 *
 * This follows the same constraints as IS_ENABLED, the config option
 * should be defined to nothing or undefined.
 */
#define STATIC_REQUIRES(option)						\
	__cfg_select_build_assert(#option, option, static, extern)

/**
 * STATIC_UNLESS is just like STATIC_REQUIRES, but makes the variable
 * static only if the config option is *not* defined, extern if it is.
 *
 * This is to assert that a variable will go unused with a certain
 * config option.
 */
#define STATIC_UNLESS(option)						\
