...
#define LED_AUTO_CONTROL_ADAPTER (1 << 0)
#define LED_AUTO_CONTROL_BATTERY (1 << 1)
#define LED_AUTO_CONTROL_POWER   (1 << 2)
 ...
 *
 * @param led_id		ID of LED to query.
 * @param brightness_range	Modified via reference.
 *	Value per color channel: 0 unsupported, 1 on/off control,
 *	2 -> 255 max brightness under PWM control.
 *
 */
void led_brightness_range(enum ec_led_id, uint8_t *brightness_range);
