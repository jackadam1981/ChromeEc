...
/*
 * Preserved ram buffers exist in uninitialized ram. The buffers are semi
 * persistent across reboots and sysjumps. A regular buffer and ring buffer
 * version is provided. The word size can be any scalar type.
 *
 * Preserved ram buffers are designed to be especially fast for writes. This
 * makes the buffers well suited for tracing and logging use cases. Locking must
 * be provided at a higher level.
 *
 * A rolling checksum ensures the buffer is uncorrupted. Corruption may be
 * caused by power spikes, misalignment with RO, or conflicting writes.
 * Verification is slow, so it should only be performed before dumping the
 * buffer.
 *
 * For performance reasons, the ring buffer does not use a tail pointer and does
 * not support dequeuing. The buffer is intended to be dumped as an entire
 * chunk.
 */
...
#define DEFINE_PRESERVED_RAM_BUF_READ(word_type)                          \
	static inline word_type preserved_ram_buf_read_##word_type(       \
		const preserved_ram_buf_##word_type *buf, uint32_t index) \
	{                                                                 \
		return buf->buffer[index];                                \
	}

#define DEFINE_PRESERVED_RAM_BUF_WRITE(word_type)                         \
	static inline void preserved_ram_buf_write_##word_type(           \
		const preserved_ram_buf_##word_type *buf, uint32_t index, \
		word_type word)                                           \
	{                                                                 \
		if (index >= buf->constants.buffer_size)                  \
			return;                                           \
		buf->noinit->checksum += word - buf->buffer[index];       \
		buf->buffer[index] = word;                                \
	}

#define DEFINE_PRESERVED_RAM_BUF_CALC_CHECKSUM(word_type)                   \
	static inline uint32_t preserved_ram_buf_calc_checksum_##word_type( \
		const preserved_ram_buf_##word_type *buf)                   \
	{                                                                   \
		uint32_t sum = 0;                                           \
...
#define DEFINE_PRESERVED_RAM_BUF_VERIFY(word_type)                            \
	static inline bool preserved_ram_buf_verify_##word_type(              \
		preserved_ram_buf_##word_type *buf)                           \
	{                                                                     \
		return (buf->noinit->version == buf->constants.version &&     \
			buf->noinit->buffer_size ==                           \
				buf->constants.buffer_size &&                 \
			buf->noinit->word_size == buf->constants.word_size && \
			buf->noinit->checksum ==                              \
				preserved_ram_buf_calc_checksum_##word_type(  \
					buf));                                \
	}

#define DEFINE_PRESERVED_RAM_BUF_RESET(word_type)                              \
	static inline void preserved_ram_buf_reset_##word_type(                \
		preserved_ram_buf_##word_type *buf)                            \
	{                                                                      \
		...
		buf->noinit->word_size = buf->constants.word_size;             \
		memset((void *)buf->buffer, 0,                                 \
		       buf->constants.buffer_size * buf->constants.word_size); \
	}
...
#define DECLARE_PRESERVED_VAR(type, name)                 \
	static volatile type name __uncached __aligned(4) \
		__noinit_end_of_ram(name)
#define DECLARE_PRESERVED_ARRAY(type, name, size)               \
	static volatile type name[size] __uncached __aligned(4) \
		__noinit_end_of_ram(name)

#define DECLARE_PRESERVED_RAM_BUF(name, word_type, _buffer_size, _version)           \
	BUILD_ASSERT((_buffer_size & (_buffer_size - 1)) == 0);                      \
	DECLARE_PRESERVED_ARRAY(                                                     \
		word_type, __preserved_ram_buf_##name##_buffer, _buffer_size);       \
	DECLARE_PRESERVED_VAR(preserved_ram_buf_noinit_t,                            \
			      __preserved_ram_buf_##name##_noinit);                  \
	...
	static preserved_ram_buf_##word_type *name =                                 \
		&__preserved_ram_buf_##name##_data
...
#define DEFINE_PRESERVED_RAM_RING_BUF_READ(word_type)                    \
	static inline word_type preserved_ram_ring_buf_read_##word_type( \
		const preserved_ram_ring_buf_##word_type *buf,           \
		uint32_t offset)                                         \
	{                                                                \
		uint32_t tail = 0;                                       \
		if (buf->noinit->head > buf->constants.buffer_size)      \
			tail = buf->noinit->head;                        \
		return buf->buffer[(tail + offset) %                     \
				   buf->constants.buffer_size];          \
	}

#define DEFINE_PRESERVED_RAM_RING_BUF_WRITE(word_type)                         \
	static inline void preserved_ram_ring_buf_write_##word_type(           \
		const preserved_ram_ring_buf_##word_type *buf, word_type word) \
	{                                                                      \
		uint32_t write_index =                                         \
			buf->noinit->head++ % buf->constants.buffer_size;      \
...
#define DEFINE_PRESERVED_RAM_RING_BUF_LEN(word_type)                   \
	static inline uint32_t preserved_ram_ring_buf_len_##word_type( \
		const preserved_ram_ring_buf_##word_type *buf)         \
	{                                                              \
		if (buf->noinit->head < buf->constants.buffer_size)    \
...
#define DEFINE_PRESERVED_RAM_RING_BUF_CALC_CHECKSUM(word_type)             \
	static inline uint32_t                                             \
		preserved_ram_ring_buf_calc_checksum_##word_type(          \
			const preserved_ram_ring_buf_##word_type *buf)     \
	{                                                                  \
		uint32_t sum = 0;                                          \
		for (int i = 0;                                            \
		     i < preserved_ram_ring_buf_len_##word_type(buf); i++) \
			sum += buf->buffer[i];                             \
		return sum;                                                \
	}

#define DEFINE_PRESERVED_RAM_RING_BUF_VERIFY(word_type)                           \
	static inline bool preserved_ram_ring_buf_verify_##word_type(             \
		preserved_ram_ring_buf_##word_type *buf)                          \
	{                                                                         \
		return (buf->noinit->version == buf->constants.version &&         \
			buf->noinit->buffer_size ==                               \
				buf->constants.buffer_size &&                     \
...
#define DEFINE_PRESERVED_RAM_RING_BUF_RESET(word_type)                         \
	static inline void preserved_ram_ring_buf_reset_##word_type(           \
		preserved_ram_ring_buf_##word_type *buf)                       \
	{                                                                      \
		buf->noinit->checksum = 0;                                     \
		buf->noinit->head = 0;                                         \
...
#define DECLARE_PRESERVED_RAM_RING_BUF(name, word_type, _buffer_size,     \
				       _version)                          \
	BUILD_ASSERT((_buffer_size & (_buffer_size - 1)) == 0);           \
	DECLARE_PRESERVED_ARRAY(word_type,                                \
				__preserved_ram_ring_buf_##name##_buffer, \
				_buffer_size);                            \
	DECLARE_PRESERVED_VAR(preserved_ram_ring_buf_noinit_t,            \
			      __preserved_ram_ring_buf_##name##_noinit);  \
	static preserved_ram_ring_buf_##word_type \
		__preserved_ram_ring_buf_##name##_data = { \
	...
	static preserved_ram_ring_buf_##word_type *name =                 \
		&__preserved_ram_ring_buf_##name##_data
