...
enum pd_task_timer {
	...
	/*
	 * This timer is used when the task timer tick needs to be sped up.
	 * Usually it runs at 5ms but there are times that the state machine
	 * needs to be more responsive.  Creat an active/expired timer and
	 * leave it active/expired until the tick time should resume back to
	 * the normal rate.  No need to check it, just want to wake up faster
	 * than the normal tick.
	 */
	PE_TIMER_FAST_WAKEUP,
 ...
 *	2) Active
 *	3) Inactive
 * The third one is questionable right now. It looks like the code was created
 * with the intent of setting a magic number in the timer to perform some
 * special sequence. I am not sure if this needs to be there or if it can be
 * reworked out of the code. For now, I have created "inactive" to handle this
 * last situation.
 */

/*
 * pd_timer_active
 * Initiate an active timer
 *
 * @param port USB-C port number
 * @param timer Requested pd_task_timer
 * @param expires_us Expiration time relative to "now"
 */
void pd_timer_active(int port, enum pd_task_timer timer, uint32_t expires_us);

/*
 * pd_timer_inactive
 * Initiate an inactive timer
 *
 * @param port USB-C port number
 * @param timer Requested pd_task_timer
 */
void pd_timer_inactive(int port, enum pd_task_timer timer);
...
/*
 * pd_timer_is_active
 * Determine if a timer is currently active
 *
 * @param port USB-C port number
 * @param timer Requested pd_task_timer
 * @return True if the timer is active, otherwise false
 */
bool pd_timer_is_active(int port, enum pd_task_timer timer);

/*
 * pd_timer_is_inactive
 * Determine if a timer is currently inactive
 *
 * @param port USB-C port number
 * @param timer Requested pd_task_timer
 * @return True if the timer is inactive, otherwise false
 */
bool pd_timer_is_inactive(int port, enum pd_task_timer timer);
 ...
 * @param timer Requested pd_task_timer
 */
void pd_timer_expired_to_inactive(int port, enum pd_task_timer timer);
