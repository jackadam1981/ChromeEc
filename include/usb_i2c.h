 ...
 *   - Write at most 254 bytes and read no more than 126 (0x7E) bytes.
 *   +------+------+----+----+---------------+
 ...
 *         count exceeds 60 (size of a USB packet), following packets are
...
#define USB_I2C_CONFIG_BUFFER_SIZE					\
	CONFIG_USB_I2C_READ_BUFFER > CONFIG_USB_I2C_WRITE_BUFFER ?	\
		CONFIG_USB_I2C_READ_BUFFER : CONFIG_USB_I2C_WRITE_BUFFER

BUILD_ASSERT(POWER_OF_TWO(CONFIG_USB_I2C_READ_BUFFER));
BUILD_ASSERT(POWER_OF_TWO(CONFIG_USB_I2C_WRITE_BUFFER));
...
#define USB_I2C_CONFIG(NAME,						\
	...
	static struct queue const CONCAT2(NAME, _to_usb_) =		\
		QUEUE_DIRECT(CONFIG_USB_I2C_READ_BUFFER, uint8_t,	\
		null_producer, CONCAT2(NAME, _usb_).consumer);		\
	static struct queue const CONCAT3(usb_to_, NAME, _) =		\
		QUEUE_DIRECT(CONFIG_USB_I2C_WRITE_BUFFER, uint8_t,	\
