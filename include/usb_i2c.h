...
/*
 * The default write / read count are both 60. We use assertion to ensure
 * write count only goes larger so those variable defined with
 * CONFIG_USB_I2C_MAX_WRITE_COUNT can exchange data with those defined by
 * USB_MAX_PACKET_SIZE. In addition, since the queue size must be a power
 * of two, we use USB_I2C_BUFFER_SIZE as an alias to avoid repeatedly padding
 * a length of four.
 */
#define USB_I2C_MAX_READ_COUNT  60
#define USB_I2C_MAX_WRITE_COUNT CONFIG_USB_I2C_MAX_WRITE_COUNT
#define USB_I2C_MAX_QUEUE_SIZE (CONFIG_USB_I2C_MAX_WRITE_COUNT + 4)

BUILD_ASSERT(USB_MAX_PACKET_SIZE <= (1 + 1 + 1 + 1 + CONFIG_USB_I2C_MAX_WRITE_COUNT));
BUILD_ASSERT(USB_MAX_PACKET_SIZE == (2 + 1 + 1 + USB_I2C_MAX_READ_COUNT));
BUILD_ASSERT(POWER_OF_TWO(USB_I2C_MAX_QUEUE_SIZE));
...
#define USB_I2C_CONFIG(NAME,						\
		       ...
		       ENDPOINT)					\
	static uint16_t							\
		CONCAT2(NAME, _buffer_)[USB_I2C_MAX_QUEUE_SIZE/2]; \
	...
	static struct queue const CONCAT3(usb_to_, NAME, _) =		\
                QUEUE_DIRECT(USB_I2C_MAX_QUEUE_SIZE, uint8_t,        \
		CONCAT2(NAME, _usb_).producer, NAME.consumer);		\
	static void CONCAT2(NAME, _deferred_)(void)			\
	{ usb_i2c_deferred(&NAME); }

/*
		CONCAT2(NAME, _buffer_)[(USB_I2C_MAX_WRITE_COUNT+4)/2]; \
