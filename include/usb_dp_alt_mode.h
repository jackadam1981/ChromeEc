...
#ifdef USB_VID_ACER
/*
 * Initialize acer mode state for the specified port.
 *
 * @param port USB-C port number
 */
...
#ifdef USB_VID_ACER
/*
 * Checks whether the mode entry sequence for ACER alternate mode is done
 * for a port.
 *
 * @param port      USB-C port number
 * @return          True if entry sequence for ACER mode is completed
 *                  False otherwise
 */
...
#ifdef USB_VID_ACER
/*
 * Handles received ACER mode VDM ACKs.
 *
 * @param port      USB-C port number
 * @param type      Transmit type (SOP, SOP') for received ACK
 * @param vdo_count The number of VDOs in the ACK VDM
...
#ifdef USB_VID_ACER
/*
 * Construct the next ACER VDM that should be sent.
 *
 * @param[in] port	    USB-C port number
 * @param[in,out] vdo_count The number of VDOs in vdm; must be at least
 *			    VDO_MAX_SIZE.  On success, number of populated VDOs
