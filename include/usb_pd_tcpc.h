...
/* If we are a TCPC but not a TCPM, then we implement the device TCPCI */
#if defined(CONFIG_USB_PD_TCPC) && !defined(CONFIG_USB_PD_TCPM_STUB)
#define TCPCI_I2C_DEVICE
#endif

#ifdef TCPCI_I2C_DEVICE
...
/* Methods for TCPCI devices (e.g. zinger) to get/set their internal state */
