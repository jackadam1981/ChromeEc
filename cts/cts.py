...
class Cts:
    ...
    def get_macro_args(self, filepath, macro):
        ...
        with open(filepath, "r", encoding="utf-8") as fff:
            ...
            for lll in joined:
                if not lll.strip().startswith(macro):
                    continue
                ddd = {}
                lll = lll.strip()[len(macro) :]
                lll = lll.strip("()").split(",")
                ddd["name"] = lll[0].strip()
                ddd["th_rc"] = self.get_return_code_value(
                    lll[1].strip().strip('"')
                )
                ddd["th_string"] = lll[2].strip().strip('"')
                ddd["dut_rc"] = self.get_return_code_value(
                    lll[3].strip().strip('"')
                )
                ddd["dut_string"] = lll[4].strip().strip('"')
                tests.append(ddd)
        return tests
