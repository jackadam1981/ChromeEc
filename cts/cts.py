...
class Cts(object):
  ...
  def evaluate_run(self, dut_output, th_output):
    ...
    self.formated_results = head.format('test name', 'TH result', 'DUT result')
    for test_name in self.test_names:
      th_cn = self.get_return_code_name(th_results[test_name])
      dut_cn = self.get_return_code_name(dut_results[test_name])
      self.formated_results += fmt.format(test_name, th_cn, dut_cn)
  ...
  def run(self):
    ...
    with open(dest, 'w') as fl:
      fl.write(self.formated_results)
