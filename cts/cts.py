...
class Board(object):
  ...
  def __init__(self, board, hla_serial=''):
    """Initializes a board object with given attributes

    Args:
      board: String containing board board
...
class DeviceUnderTest(Board):
  ...
  def __init__(self, board, th, hla_ser=''):
...
class Cts(object):
  ...
  def set_dut_board(self, brd):
    ...
    """
    self.dut.setboard(brd)

  def set_module(self, mod):
    """Sets the module instance variable. Also sets test_names and
    debug_output, since they depend directly on which module we are using
    ...
    for output_str in [r1, r2]:
      ...
      for ln in [ln.strip() for ln in output_str.split('\n')]:
        ...
        try:
          ...
          continue # The second token was not an integer
        ...
        if t_name in self.test_results.keys():
          if self.test_results[t_name] != t_result:
            ...
            elif t_result != CTS_SUCCESS_CODE:
