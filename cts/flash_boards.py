...
def get_stlink_serial_numbers():
    ...
    grep_args = ['grep', 'iSerial']

    usb_process = sp.Popen(usb_args, stdout=sp.PIPE, shell=False)
    grep_process = sp.Popen(grep_args, stdin=usb_process.stdout, stdout=sp.PIPE, shell=False)
    usb_process.stdout.close()

    st_serials = grep_process.communicate()[0]
    lines = st_serials.split('\n')[:-1]
    serials = []
    for l in lines:
...
def identify_dut(th_hla_serial):
    ...
    if len(stlink_serials) == 1:
        return None
    elif len(stlink_serials) == 2: # If 2 st-link devices connected, find the one that doesn't have the TH serial number
...
def update_th_serial(dest_dir):
    ...
    if len(serial) != 1:
        ...
        ser = serial[0]
        f = open(dest_dir + '/th_hla_serial', mode='w')
        f.write(ser)
        f.close()
        return ser
...
def flash_boards(th_board, dut_board, th_hla_serial, dut_hla_serial=None):
    ...
    openocd_cmd(['init', 'reset init', 'resume', 'shutdown'], th_cfg)
    openocd_cmd(['init', 'reset init', 'resume', 'shutdown'], dut_cfg)

def main():
    path = os.path.abspath(__file__)
    ec_dir = os.path.dirname(path) + '/..'
    os.chdir(ec_dir)
    th_serial_dir = ec_dir + '/build/' + th_board
    dut_board = ''
    module = ''
    ...
    th_hla = open(th_serial_dir + '/th_hla_serial').read()
