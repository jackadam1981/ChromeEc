...
static void abort_transfer(int port)
{
	...
	ROTOR_MCU_I2C_INTR_MASK(port) = M_TX_ABRT;

	/* Issue the abort. */
	ROTOR_MCU_I2C_ENABLE(port) |= ABORT;
...
static int disable_i2c(int port)
{
	...
	if (!(ROTOR_MCU_I2C_ENABLE_STATUS(port) & IC_EN))
		return EC_SUCCESS;

	/* Try disabling the port. */
	ROTOR_MCU_I2C_ENABLE(port) &= ~ENABLE;

	/* Check to see that the hardware actually shuts down. */
	while (ROTOR_MCU_I2C_ENABLE_STATUS(port) & IC_EN) {
		usleep(10);
		timeout--;
...
static int wait_byte_done(int port, int timeout)
{
	...
	ROTOR_MCU_I2C_INTR_MASK(port) |= M_TX_EMPTY;
...
static int wait_byte_ready(int port, int timeout)
{
	...
	task_waiting[port] = task_get_current();
	ROTOR_MCU_I2C_INTR_MASK(port) |= M_RX_FULL;
		  ...
		  uint8_t *in, int in_size, int flags)
{
	...
	for (i = 0; i < out_size; i++) {
		...
		if ((i == (out_size-1)) && (flags & I2C_XFER_STOP))
			ROTOR_MCU_I2C_DATA_CMD(port) |= (STOP | out[i]);
		else
...
static void set_port_speed(int port, enum i2c_freq freq)
{
	...
	case I2C_FREQ_100KHZ: /* Standard Mode */
		ROTOR_MCU_I2C_CON(port) = SPEED_STD_MODE;
	...
	case I2C_FREQ_400KHZ: /* Fast Mode */
		ROTOR_MCU_I2C_CON(port) = SPEED_FAST_MODE;
	...
	case I2C_FREQ_1000KHZ: /* Fast Mode Plus */
		ROTOR_MCU_I2C_CON(port) = SPEED_HISPD_MODE;
...
static void i2c_init_port(const struct i2c_port_t *p)
{
	...
	if (!(ROTOR_MCU_I2C_REFCLKGEN(port) & CLKEN))
		ROTOR_MCU_I2C_REFCLKGEN(port) |= CLKEN;
	 ...
	 * addressing.
	 */
	ROTOR_MCU_I2C_CON(port) |= MASTER_MODE | IC_SLAVE_DISABLE |
		IC_RESTART_EN;
...
static void handle_interrupt(int port)
{
	 ...
	 */
	if ((ROTOR_MCU_I2C_INTR_MASK(port) & M_TX_EMPTY) &
	    (ROTOR_MCU_I2C_INTR_STAT(port) & M_TX_EMPTY))
		ROTOR_MCU_I2C_INTR_MASK(port) &= ~M_TX_EMPTY;

	/* Same for RX_FULL */
	if ((ROTOR_MCU_I2C_INTR_MASK(port) & M_RX_FULL) &
	    (ROTOR_MCU_I2C_INTR_STAT(port) & M_RX_FULL))
		ROTOR_MCU_I2C_INTR_MASK(port) &= ~M_RX_FULL;
