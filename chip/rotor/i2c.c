...
;
	/*
	 * TODO(aaboagye): Remove this hack. Trying to make the clock
	 * quicker.
	 */
	i2c_clk = 5000;
...
static void handle_interrupt(int port)
{
	 ...
	 */
	if ((ROTOR_MCU_I2C_INTR_MASK(port) & (1 << 4)) &
	    (ROTOR_MCU_I2C_INTR_STAT(port) & (1 << 4)))
		ROTOR_MCU_I2C_INTR_MASK(port) &= ~(1 << 4);

	/* Same for RX_FULL */
	if ((ROTOR_MCU_I2C_INTR_MASK(port) & (1 << 2)) &
	    (ROTOR_MCU_I2C_INTR_STAT(port) & (1 << 2)))
		ROTOR_MCU_I2C_INTR_MASK(port) &= ~(1 << 2);
