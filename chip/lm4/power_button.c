...
{
	uart_printf("[PB PCH pwrbtn=%s @ %d us]\n",
		    (high ? "HIGH" : "LOW"), get_time().le.lo);
...
static void lid_switch_changed(uint64_t tnow)
{
	...
	/* Pass signal on to PCH; this is how the PCH knows to suspend when
	 * the lid is closed. */
...
void power_button_task(void)
{
	...
	while (1) {
		 ...
		 * theoretically overflow if we wanted to sleep for more than
		 * 2^32 us, but our timeouts are small enough that can't
		 * happen.) */
		if (tsleep > t) {
