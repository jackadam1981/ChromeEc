...
{
	/*
	 * Transmit is in progress if the FIFO is not empty or the TX
	 * busy bit is set.
	 */
	return !(LM4_UART_FR(0) & 0x80) || (LM4_UART_FR(0) & 0x08);
}
...
void uart_console_disable_for_dsleep(void)
{
...
void uart_console_enable_from_dsleep(void)
{
	const struct gpio_info g = gpio_list[GPIO_UART0_RX];
