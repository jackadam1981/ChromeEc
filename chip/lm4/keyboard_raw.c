...
void keyboard_raw_init(void)
{
	...
	/*
	 * Set column outputs (kso pins) as open-drain;
	 * we either pull them low or let
	 * them float high.
	 */
...
test_mockable void keyboard_raw_drive_kso_pins(int kso)
{
	...
	if (kso == KEYBOARD_KSO_PINS_NONE)
		mask = 0x1fff;			/* Tri-state all outputs */
	else if (kso == KEYBOARD_KSO_PINS_ALL)
...
void keyboard_raw_enable_interrupt(int enable)
{
	if (enable) {
		 ...
		 * explicitly check the raw row (KSI pin) state
