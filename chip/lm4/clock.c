...
#define CPRINTF(format, args...) info_printf(CC_CLOCK, format, ## args)
...
void __idle(void)
{
	 ...
	 */
	CPRINTF("low power idle task started");
	...
	while (1) {
		...
		if (DEEP_SLEEP_ALLOWED && time_for_dsleep) {
			...
			if (margin_us < 0)
				CPRINTF("overslept by %dus", -margin_us);
