...
{
	if (enable) {
		/* SSI0 on PA2(CLK), 4(RX), 5(TX) alternate function 2 */
		    ...
		    uint8_t *rxdata, int rxlen)
{
	...
	while (rxcount < totallen) {
		/* Handle received bytes if any.  Note that we've already
		 * checked rxcount < totallen above, so we don't need to
		 * worry about overflowing the receive buffer. */
		if (LM4_SSI_SR(0) & LM4_SSI_SR_RNE) {
			if (rxcount < txlen) {
				/* Byte received while we were transmitting */
				dummy = LM4_SSI_DR(0);
			} else
				*(rxdata++) = LM4_SSI_DR(0);
			rxcount++;
		...
		/* Add byte to transmit queue if space */
...
static int spi_init(void)
{
	...
	/* Note that we haven't enabled the SPI port.  This keeps us from
	 * interfering with the main chipset when we're not explicitly using
	 * the SPI bus. */
