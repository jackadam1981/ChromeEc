...
struct i2c_port_data {
	int started;		/* Already sent start bit? */
...
static int wait_idle(int port)
{
	...
	/* Kick the port interrupt handler */
	task_trigger_irq(i2c_irqs[port]);

	/* Wait for I2C transfer complete or timeout */
	     ...
	     uint8_t *in, int in_size, int flags)
{
	...
	pd->started = (flags & I2C_XFER_START) ? 0 : 1;
	pd->out = out;
...
static void handle_interrupt(int port)
{
	...
	/* If done doring work, wake up the task waiting for the transfer */
	if (!i2c_do_work(port))
