...
static void clock_high_enable(int osc)
{
	...
	case 0:
		...
		CLK_CFG_UPDATE3 = F_ULPOSC_CK_UPDATE;
		clock_busy_udelay(50);
		break;
	case 1:
		...
		CLK_CFG_UPDATE3 = F_ULPOSC_CORE_CK_UPDATE;
		clock_busy_udelay(50);
		break;
	default:
		break;
...
static void clock_select_clock(enum scp_clock_source src, uint32_t clk_speed)
{
	...
	/*
	 * SCP_CLK_DIV_SEL needs to be set before SCP_CLK_SW_SEL to prevent
	 * over-clocking.
	 */
...
void clock_init(void)
{
	...
	clock_select_clock(SCP_CLK_26M, SCP_CLK_NO_OPTION);
...
int command_ulposc(int argc, char *argv[])
{
	...
	for (osc = 0; osc <= 1; ++osc)
		ccprintf("ULPOSC%u frequency: %u kHz\n", osc + 1,
			 clock_ulposc_measure_freq(osc) * 26 * 1000 / 1024);
