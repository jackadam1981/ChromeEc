...
#define SCP_CLK_NO_OPTION	BIT(0)
#define SCP_CLK_LOW_SPEED	BIT(1)
#define SCP_CLK_HIGH_SPEED	BIT(2)

enum scp_clock_source {
	SCP_CLK_SYSTEM = CLK_SW_SEL_SYSTEM,
...
static uint32_t clock_ulposc_measure_freq(uint32_t osc)
{
	...
	/*
	 * Frequency meter counts cycles in 1 / (26 * 512) second period.
	 *   freq_in_hz = freq_counter * 26 * 512
	 *
...
static int clock_ulposc_is_calibrated(struct opp_ulposc_cfg *opp)
{
	...
	CPRINTF("osc:%u, target=%uMHz, curr=%uMHz, cal:%u\n",
...
static void clock_select_clock(enum scp_clock_source src, uint32_t clk_speed)
{
	uint32_t i;

	if (src == SCP_CLK_ULPOSC2) {
		/* parking at scp system clk until ulposc clk is ready */
		SCP_CLK_SW_SEL = SCP_CLK_SYSTEM;

		if (clk_speed == SCP_CLK_HIGH_SPEED)
			i = ARRAY_SIZE(opp) -1;
		else
			i = 0;

		clock_ulposc_config_cali(&opp[i], opp[i].cali);
		SCP_CLK_DIV_SEL = opp[i].clk_div;
	} else {
				      ...
				      struct host_sleep_event_context *ctx)
{
	if (state == HOST_SLEEP_EVENT_S3_SUSPEND) {
		CPRINTS("AP suspend");
		clock_select_clock(SCP_CLK_ULPOSC2, SCP_CLK_LOW_SPEED);
	} else if (state == HOST_SLEEP_EVENT_S3_RESUME) {
		CPRINTS("AP resume");
		clock_select_clock(SCP_CLK_ULPOSC2, SCP_CLK_HIGH_SPEED);
	}
}
...
void clock_init(void)
{
	...
	clock_select_clock(SCP_CLK_SYSTEM, SCP_CLK_NO_OPTION);
