...
void insert_host_command(uint32_t command)
{
	...
	memset(&status_log[next_entry], 0, sizeof(struct espi_log));
	status_log[next_entry].host_command = command;
	next_entry++;
}
...
void insert_port80(int data)
{
	...
	memset(&status_log[next_entry], 0, sizeof(struct espi_log));
	status_log[next_entry].port80 = data;
	next_entry++;
}
...
void insert_acpi_cmd(uint8_t cmd)
{
	...
	memset(&status_log[next_entry], 0, sizeof(struct espi_log));
	status_log[next_entry].acpi_cmd = cmd;
	next_entry++;
}
...
void insert_acpi_data(uint8_t data)
{
	...
	memset(&status_log[next_entry], 0, sizeof(struct espi_log));
	status_log[next_entry].acpi_data = data;
	next_entry++;
}
...
static int command_espi(int argc, char **argv)
{
	...
	/* Get value of eSPI registers */
		...
		if (strcasecmp(argv[1], "cfg") == 0) {
			...
			ccprintf("Status log: ne %d, w %d\n", next_entry, wrapped);
			if (wrapped)
				i = next_entry % LOG_SIZE;
			else
				i = 0;
			do {
				if (!wrapped && next_entry == 0)
					break;

				ccprintf("[%3d] ", j);
				if (status_log[i].status)
					ccprintf("status 0x%08x\n", status_log[i].status);
