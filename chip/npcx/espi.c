...
/**
 * Set eSPI Virtual-Wire event to Host
 *
 * @param event vw signal needs to set
 * @param wire  wire of vw signal
 * @return      none
 */
void espi_vw_set_wire(enum espi_vw_signal signal, uint8_t wire)
{
...
/**
 * Get eSPI Virtual-Wire event from host
 *
 * @param event vw signal needs to get
 * @return      1: set by host, otherwise: no signal
 */
int espi_vw_get_wire(enum espi_vw_signal signal)
{
	uint8_t offset, sig_idx;
	...
	if (NPCX_VWEVMS(offset) & vw_events_list[sig_idx].evt_val)
		return 1;
	else
		return 0;
}
...
/**
 * Enable VW interrupt of power sequence event
 *
 * @param event vw signal needs to enalbe interrupt
 */
void espi_vw_enable_wire_int(enum espi_vw_signal signal)
{
...
/**
 * Disable VW interrupt of power sequence event
 *
 * @param event vw signal needs to disable interrupt
 */
void espi_vw_disable_wire_int(enum espi_vw_signal signal)
{
	if (signal == VW_SLP_S3_L)
...
void espi_vw_evt_sus_warn(void)
{
	...
	/* Send ACK to host if WARN#'s wire is set */
	if (espi_vw_get_wire(VW_SUS_WARN_L))
		espi_vw_set_wire(VW_SUS_ACK, 1);
}
...
DECLARE_IRQ(NPCX_IRQ_WKINTA_2, __espi_wk2a_interrupt, 1);
...
DECLARE_IRQ(NPCX_IRQ_WKINTB_2, __espi_wk2b_interrupt, 1);
...
DECLARE_IRQ(NPCX_IRQ_ESPI, espi_interrupt, 2);
...
static int command_espi(int argc, char **argv)
{
	...
	/* Enalbe/Disable the channels of eSPI */
	} else if (argc == 3) {
