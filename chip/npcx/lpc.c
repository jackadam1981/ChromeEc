...
/* Timeout to wait LRESET is deasserted */
#define LPC_LRST_TIMEOUT_US 800000
...
int lpc_get_lrst_asserted(void)
{
	/* Read PLTRST status */
...
void lpc_host_register_init(void){
	...
	deadline.val = 0;
	deadline = get_time();
	deadline.val += LPC_LRST_TIMEOUT_US;

	/* Make sure LRESET is de-asserted. Or any setting for LPC is useless */
	while (lpc_get_lrst_asserted())
		if (timestamp_expired(deadline, NULL)) {
			CPRINTS("LRESET is asserted. LPC settings are ignored!");
			return;
		}
