 ...
 * To prevent HW issue, we need to wait >= 1/8 baudrate before switching pad,
 * i.e. (1/8)/115200 = 1.1us (rounded to 2), see b/65526215.
 * udelay is not reliable for such short delays, so we use __udelay.
 */
void npcx_gpio2uart(int pad)
{
	if (NPCX_UART_MODULE2 ^ pad) {
		CLEAR_BIT(NPCX_DEVALT(0x0A), NPCX_DEVALTA_UART_SL1);
		__udelay(3); /* Sleep for 2 to 3 us. */
...
void uart_set_pad(enum uart_pad newpad)
{
	...
	/*
	 * Pad switching can corrupt the first byte from the HW block, let's
	 * wait for ~1 byte (9/115200 = 78 us + margin), then flush the FIFO.
	 * See b/65526215.
	 */
	udelay(100);
