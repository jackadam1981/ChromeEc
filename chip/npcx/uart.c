...
{
	CLEAR_BIT(NPCX_WKEDG(1, NPCX_UART_WK_PORT_N), NPCX_UART_WK_BIT);
 ...
 * on the global variable "pad".
 * Note that, when switching pad, we first switch on the new module, then switch
...
void npcx_gpio2uart(void)
{
	...
	SET_BIT(NPCX_WKEDG(1, NPCX_UART_WK_PORT_N), NPCX_UART_WK_BIT);
}
...
#ifdef CONFIG_UART_PAD_SWITCH
void uart_set_pad(enum uart_pad newpad, int panic)
{
	/*
	 * If we are switching pad to print a panic, just switch pad immediately
	 * and don't care if we output a bit of garbage.
	 */
	if (panic) {
		pad = newpad;
		...
		/* Flush ~2 bytes, to help the receiver resync. */
		udelay(200);
	 ...
	 * be currupted, let's wait for ~1 byte (9/115200 = 78 us + margin),
...
void uart_default_pad_rx_interrupt(enum gpio_signal signal)
{
	...
	uart_set_pad(UART_DEFAULT_PAD, 0);
	last_default_pad_rx_time = get_time();
}
			    ...
			    int timeout_us)
{
	...
	uart_set_pad(UART_ALTERNATE_PAD, 0);
...
#error "Unsupported apb2 clock for UART!"
	 ...
	 * modify the delay in npcx_gpio2uart.
	 */
	NPCX_UPSR = 0x38;
	NPCX_UBAUD = 0x01;
