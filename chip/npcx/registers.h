...
#define UPDATE_BIT(reg, bit, cond)		{if(cond) SET_BIT(reg, bit); else CLEAR_BIT(reg, bit);}
...
#define NPCX_SMB_BASE_ADDR(mdl)     	((mdl<2) ? (0x40009000 + ((mdl) * 0x2000L)) : \
...
/*******************************************************************************************/
/* MIWU registers */
#define NPCX_WKEDG_ADDR(port,n)        (NPCX_MIWU_BASE_ADDR(port) + 0x00 + ((n) * 2L) + ((n) < 5 ? 0 : 0x1E))
#define NPCX_WKAEDG_ADDR(port,n)       (NPCX_MIWU_BASE_ADDR(port) + 0x01 + ((n) * 2L) + ((n) < 5 ? 0 : 0x1E))
#define NPCX_WKPND_ADDR(port,n)        (NPCX_MIWU_BASE_ADDR(port) + 0x0A + ((n) * 4L) + ((n) < 5 ? 0 : 0x10))
#define NPCX_WKPCL_ADDR(port,n)        (NPCX_MIWU_BASE_ADDR(port) + 0x0C + ((n) * 4L) + ((n) < 5 ? 0 : 0x10))
#define NPCX_WKEN_ADDR(port,n)         (NPCX_MIWU_BASE_ADDR(port) + 0x1E + ((n) * 2L) + ((n) < 5 ? 0 : 0x12))
...
#define CGC_I2C_MASK 	((1<<NPCX_PWDWN_CTL3_SMB0_PD) | (1<<NPCX_PWDWN_CTL3_SMB1_PD) | (1<<NPCX_PWDWN_CTL3_SMB2_PD))
#define CGC_ADC_MASK 	(1<<NPCX_PWDWN_CTL4_ADC_PD)
#define CGC_TIMER_MASK 	((1<<NPCX_PWDWN_CTL4_ITIM1_PD)  | (1<<NPCX_PWDWN_CTL4_ITIM2_PD)   | (1<<NPCX_PWDWN_CTL4_ITIM3_PD))
#define CGC_LPC_MASK 	((1<<NPCX_PWDWN_CTL5_C2HACC_PD) | (1<<NPCX_PWDWN_CTL5_SHM_REG_PD) | (1<<NPCX_PWDWN_CTL5_SHM_PD) \
                         | (1<<NPCX_PWDWN_CTL5_DP80_PD) | (1<<NPCX_PWDWN_CTL5_MSWC_PD))
