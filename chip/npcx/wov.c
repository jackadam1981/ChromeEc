...
#define EC_ERROR_INVALID_CONFIG 0
...
/* mask defentions that clear reserved fields for WOV registers.*/
...
static const struct wov_pll_set_options_val pll_set_opt_16khz[] = {
	{1, 1125, 4, 3, 4},
	{1, 1000, 4, 4, 4},
	{1, 960,  4, 4, 4},
	{1, 750,  4, 4, 4},
...
static const struct wov_pll_set_options_val pll_set_opt_48khz[] = {
	{1, 500, 4, 4, 4},
	{1, 250, 6, 4, 2},
	{1, 320, 4, 4, 4},
	{1, 250, 4, 4, 4},
...
struct wov_cfifo_buff {
	uint32_t *buff; /* Pointer to a buffer. */
	int size;       /* Buffer size in words. */
};

struct wov_config wov_conf;

static struct wov_cfifo_buff cfifo_buff;
static wov_call_back_t callback_fun;

const uint32_t voice_buffer[16000] = {0};
...
void wov_cfifo_read_handler_l(uint32_t num_elements)
{
	...
	for (index = 0; index < num_elements; index++)
		cfifo_buff.buff[index] = NPCX_WOV_FIFO_OUT;

	cfifo_buff.buff = &cfifo_buff.buff[index];
	cfifo_buff.size -= num_elements;
}
 ...
 *          sample_rate  - Sample rate in KHz (16KHz or 48KHz)
 *          pll_div      - PLL dividers.
 *
 * @return  None
 */
 ...
 * @return  none.
 */
enum ec_error_list wov_set_i2s_config_l(void)
{
	struct wov_ppl_divider pll_div;
	enum ec_error_list ret_code;
	enum wov_i2s_chan_triger triger_0, triger_1;
	...
	if (ret_code == EC_SUCCESS) {
		...
		case WOV_DAI_FMT_I2S:
			triger_0 = WOV_I2S_SAMPLED_0_AFTER_1;
			triger_1 = WOV_I2S_SAMPLED_1_AFTER_0;
			start_delay_0 = 1;
			start_delay_1 = 1;
			break;

		case WOV_DAI_FMT_RIGHT_J:
			triger_0 = WOV_I2S_SAMPLED_1_AFTER_0;
			triger_1 = WOV_I2S_SAMPLED_0_AFTER_1;
...
void wov_interrupt_handler(void)
{
	...
	wov_status = GET_FIELD(NPCX_WOV_STATUS, NPCX_WOV_STATUS_BITS);
	...
	if (WOV_IS_CFIFO_OVERRUN(wov_status)) {
		 ...
		 */
		if (WOV_IS_CFIFO_INT_THRESHOLD(wov_status)) {
			if (cfifo_buff.buff != NULL) {
				/* Copy data from CFIFO to RAM. */
				wov_cfifo_read_handler_l(
					(WOV_GET_CORE_FIFO_THRESHOLD * 2));

				if (cfifo_buff.size <
				    (WOV_GET_CORE_FIFO_THRESHOLD * 2)) {
					cfifo_buff.buff = NULL;
					cfifo_buff.size = 0;
...
{
	SET_BIT(NPCX_FMUL2_FM2CTRL, NPCX_FMUL2_FM2CTRL_LOCK2);
}
...
static struct wov_fmul2_multiplier_setting_val freq_multiplier_set[] =
	/* FM2MH  FM2MH FM2N */
	{
		{0x0B, 0x72, 0x02}, /* 48 MHz */
};

/**
 * wov_fmul2_clk_freq_set
 *
 * @param  clk_freq - Clock frequency (24 or 48 MHz)
 *
 * @return  EC error
 */
enum ec_error_list wov_fmul2_clk_freq_set(enum fmul2_clk_freq clk_freq)
{
	uint32_t index;
	/* Set clock multipliers values. */
	NPCX_FMUL2_FM2ML = freq_multiplier_set[clk_freq].fm2ml;
	NPCX_FMUL2_FM2MH = freq_multiplier_set[clk_freq].fm2mh;
	SET_FIELD(NPCX_FMUL2_FM2N, NPCX_FMUL2_FM2N_FM2N,
			freq_multiplier_set[clk_freq].fm2n);

	/* Set clock frequency. */
	SET_BIT(NPCX_FMUL2_FM2CTRL, NPCX_FMUL2_FM2CTRL_LOAD2);

	/* Wait until clock is stable. */
	for (index = 0;
	     IS_BIT_SET(NPCX_FMUL2_FM2CTRL, NPCX_FMUL2_FM2CTRL_CLK2_CHNG) &&
	     index < WOV_FMUL2_MAX_RETRIES;
	     index++)
		;

	if (index < WOV_FMUL2_MAX_RETRIES)
		return EC_SUCCESS;
	else
...
{
	wov_fmul2_clk_conf_tuning();
}

/**
 * Configure FMUL2 clock.
 *
 * @param   enable      - enabled flag, 1 means enable
 *          clk_div     - FMUL2 clock division factor
 *          clk_freq    - Clock frequency
 * @return  None
 */
void wov_fmul2_clk_config(int enable,
			  enum fmul2_clk_divider clk_div,
			  enum fmul2_clk_freq clk_freq)
{
	/*
	 * Currently the only ful2 frequency is 48MHZ so
	 * there is no need to configure it.
	 */

	/* If FMUL2 enabled then configured its clock.*/
	if (enable)
		wov_fmul2_set_clk_divider(clk_div);

	wov_fmul2_enable(enable);
}

int wov_get_cfifo_threshold_l(void)
{
	int fifo_threshold;
 ...
 * @return  None
 */
void wov_config(void)
{
	/* TODO: */

	/* Select FMUL2 as clock origin. */
	wov_set_clk_selection(WOV_FMUL2_CLK_SRC);

	wov_fmul2_clk_config(1, FMUL2_CLK_DIVIDER_BY_4, FMUL2_48_MHZ);

	wov_fmul2_conf_tuning();
}
 ...
 *          enable    - enabled flag, 1 means enable
 *
 * @return  None.
 */
...
int wov_set_buffer(uint32_t *buff, int size_in_words)
{
	...
	cfifo_buff.buff = buff;
	cfifo_buff.size = size_in_words;
...
enum ec_error_list wov_i2s_channel_config(uint32_t channel_num,
					uint32_t bit_count,
					enum wov_i2s_chan_triger triger,
					int32_t start_delay)
{
	/* Check the parameters correctnes. */
	...
	if (triger == WOV_I2S_SAMPLED_1_AFTER_0)
...
int wov_set_sample_rate(uint32_t samples_per_second)
{
	...
	if ((samples_per_second !=  8000) && (samples_per_second != 12000) &&
	    (samples_per_second != 16000) && (samples_per_second != 24000) &&
	    (samples_per_second != 32000) && (samples_per_second != 48000))
		return EC_ERROR_INVAL;
...
void wov_set_gain(int left_chan_gain, int right_chan_gain)
{
	...
	ret_code = apm_adc_gain_config(APM_ADC_CHAN_GAINS_INDEPENDENT,
				left_chan_gain, right_chan_gain);

	if (ret_code == EC_SUCCESS) {
		wov_conf.left_chan_gain = left_chan_gain;
		wov_conf.rigth_chan_gain = right_chan_gain;
	}
}
 ...
 *        noise_gate_threshold  - Noise Gate system select. 1 means enable.
				...
				float max_applied_gain, float min_applied_gain)
{
	...
	for (ngth_code = 0; ngth_code <= 42; ngth_code += 6) {
		if ((-68 + ngth_code) == noise_gate_threshold)
			break;
	}
	if (ngth_code > 42)
		return EC_ERROR_INVAL;
	...
	for (attack_time_code = 0; attack_time_code <= 512;
			attack_time_code += 32) {
		if (attack_time_code == attack_time)
			break;
	}
	if (attack_time_code > 512)
		return EC_ERROR_INVAL;

	for (decay_time_code = 0; decay_time_code <= 512;
			decay_time_code += 32) {
		if (decay_time_code == decay_time)
			break;
	}
	if (decay_time_code > 512)
		return EC_ERROR_INVAL;
	...
	ret_code = apm_adc_auto_gain_config(
		(stereo == 0), (enum apm_adc_target_out_level)target_code,
		(noise_gate_threshold != 0),
		(enum apm_noise_gate_threshold)ngth_code,
		(enum apm_agc_adj_hold_time)hold_time,
		(enum apm_gain_ramp_time)attack_time_code,
		(enum apm_gain_ramp_time)decay_time_code,
		(enum apm_gain_values)max_applied_gain_code,
		(enum apm_gain_values)min_applied_gain_code);
...
{
	return apm_get_vad_sensitivity();
}
...
static void wov_system_init(void)
{
	...
	wov_set_sample_rate(16000);
	wov_set_sample_depth(16);
	wov_set_i2s_config(768000, WOV_DAI_FMT_I2S);
	wov_set_i2s_tdm_config(0, 0, 0x00000001);
	apm_set_vad_sensitivity(9);
	wov_mute(1);
...
static int command_wov(int argc, char **argv)
{
	...
	if (argc == 2) {
		...
		if (strcasecmp(argv[1], "cfgsfs") == 0) {
			...
			fs = atoi(argv[2]);
			if ((fs == 8000) || (fs == 12000) || (fs == 16000) ||
			    (fs == 24000) || (fs == 32000) || (fs == 48000)) {
				wov_set_sample_rate(fs);
				return EC_SUCCESS;
			}
		}
