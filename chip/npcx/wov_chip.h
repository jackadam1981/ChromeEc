 ...
 * @return  None
 */
void wov_fmul2_enable(int enable);
 ...
 * @return  None
 */
void wov_fmul2_clk_conf_tuning(void);
...
/**
 * Disable write to FMUL2 registers.
 *
 * @param   None
 * @return  None
 */
void wov_fmul2_disable_reg_write(void);

/**
 * Gets FMUL2 version.
 *
 * @param   None
 * @return  FMUL2 version
 */
uint8_t wov_fmul2_get_ver(void);
 ...
 *			right_chan_gain - Right channel gain
 * @return	None
 */
 ...
 *
 * @param stereo - Stereo enabled flag, true means enable.
 *	  target		- Target output level of the ADC.
 *	  noise_gate_threshold	- Noise Gate system select. true means enable.
 *	  nois_gate_thold	- Noise Gate Threshold Value.
 *	  hold_time		- Hold time before starting AGC adjustment to
 *				  the TARGET value.
 *	  attack_time		- Attack time - gain ramp down.
 *	  decay_time		- Decay time - gain ramp up.
 *	  max_applied_gain	- Maximum Gain Value to apply to the ADC path.
 *	  min_applied_gain	- Minimum Gain Value to apply to the ADC path.
 * @return	EC_ERROR_INVAL or EC_SUCCESS
 */
 ...
 *	  format    - one of the following: I2S mode, Right Justified mode,
 ...
 *	  ch1_delay - -1 to 496. Defines the delay from the SYNC till the
 *		      first bit (MSB) of channel 1 (right channel).
 *		      If channel 1 is not used set this field to -1.
 *
 *	  flags	    - WOV_TDM_ADJACENT_TO_CH0 = (1 << 0).  There is a
 ...
 * @return	None
 */
void wov_set_i2s_tdm_config(int ch0_delay, int ch1_delay, uint32_t flags);
 ...
 * @return	None
 */
extern void wov_config(void);

/**
 * Configure DMIC clock.
 *
 * @param   enable          - DMIC enabled , true means enable
 *          clk_div         - DMIC clock division factor (disable, divide by 2
 *			      divide by 4)
 * @return  None
 */
 ...
 *	  div_factor	- When ext_div_sel is WOV_PLL_EXT_DIV_BIN_CNT
 ...
 *	  out_div_2	- PLL output divider #2, valid values 1-7
 *	  feedback_div	- PLL feadback divider (Default is 375 decimal)
 *	  in_div	- PLL input divider
 * @return	EC_ERROR_INVAL or EC_SUCCESS
 */
 ...
 *	  enable	- enabled flag, true means enable
 *
 * @return	None.
 */
 ...
 *			  aligned.
 *	  size_in_words - Size must be a multiple of CONFIG_WOV_THRESHOLD_WORDS
 *			  (defaulte = 80 words)
 *
 * @return	None
 *
 ...
 *	  i2s_hiz	- Defines if the I2S data output is always floating.
 *	  clk_invert	- Defines the I2S bit clock edge sensitivity
 *	  out_pull_en	- Enable a pull-up or a pull-down resistor on
 *							I2S output
 *	  out_pull_mode - Select a pull-up or a pull-down resistor on
 *			  I2S output
 *	  in_pull_en	- Enable a pull-up or a pull-down resistor on
 *			  I2S input
 *	  in_pull_mode	- Select a pull-up or a pull-down resistor on
 *			  I2S intput
 *	  test_mode	- Selects I2S test mode
 *
 * @return	EC_ERROR_INVAL or EC_SUCCESS
 */
 ...
 *	   bit_count	- I2S channel bit count.
 *	   triger	- Define the I2S chanel triger 1->0 or 0->1
 *	   start_delay	- Defines the delay from the trigger defined for
 *			  the channel till the first bit (MSB) of the data.
 *
 * @return	 EC_ERROR_INVAL or EC_SUCCESS
 */
enum ec_error_list wov_i2s_channel_config(uint32_t channel_num,
			uint32_t bit_count, enum wov_i2s_chan_triger triger,
			int32_t start_delay);
