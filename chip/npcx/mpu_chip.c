...
static uint32_t align_to_bits(uint32_t addr, uint8_t addr_bits)
{
	if (addr_bits < 32)
...
static int mpu_chip_update_lock_region(uint8_t region, uint32_t addr,
				       uint8_t lock_bit)
{
	uint32_t offset;
	...
	if (region == REGION_DATA_RAM) {
		SET_FIELD(NPCX_RAM_FLK(offset / NPCX_RAM_BLOCK),
			  NPCX_RAM_LK_FIELD, lock_bit);
	} else if (region == REGION_STORAGE) {
		SET_FIELD(NPCX_RAM_WLK(offset / NPCX_RAM_BLOCK),
			  NPCX_RAM_LK_FIELD, lock_bit);
	}
...
int mpu_chip_config_lock_region(uint8_t region, uint32_t addr, uint32_t size)
{
	...
	/* Check address is alignment */
	if (addr & (NPCX_RAM_SECTOR - 1)) {
		return EC_ERROR_INVAL;
	}

	/* Check size is alignment */
	 ...
	 */
	natural_alignment += 3;
	subregion_base = align_to_bits(addr, natural_alignment);
