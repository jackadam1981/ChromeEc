...
void hw_clock_source_set_preload(uint32_t ts, uint8_t clear)
{
	...
	/* Wait for disable to take effect before updating count */
	while (IS_BIT_SET(NPCX_ITCTS(ITIM32), NPCX_ITCTS_ITEN))
		;
	...
	/* Wait for enable to take effect before updating count */
	while (!IS_BIT_SET(NPCX_ITCTS(ITIM32), NPCX_ITCTS_ITEN))
		;
}
