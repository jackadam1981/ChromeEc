...
SMB_ERROR i2c_master_transaction (int port)
{
	 ...
	 */
	else if(pStsObj->err_code == SMB_BUS_ERROR ||
...
inline void i2c_handle_sda_irq(int port)
{
	...
	if (pStsObj->oper_state == SMB_MASTER_START
			|| pStsObj->oper_state == SMB_REPEAT_START) {
	...
	/* 2 Handle master write operation  */
		...
		if (pStsObj->idxBuf == pStsObj->szTxBuf){
			...
			if(pStsObj->szRxBuf == 0){
				...
				/* Notify upper layer */
				pStsObj->oper_state
				= (pStsObj->flags & I2C_XFER_STOP)
				? SMB_IDLE :SMB_WAIT_REPEAT_START;
				task_set_event(pStsObj->task_waiting,
	...
	/* 3 Handle master read operation (read or after a write operation) */
		...
		if (pStsObj->idxBuf == pStsObj->szRxBuf) {
			...
			pStsObj->oper_state = (pStsObj->flags & I2C_XFER_STOP)
					? SMB_IDLE : SMB_WAIT_REPEAT_START;
			task_set_event(pStsObj->task_waiting,
					TASK_EVENT_I2C_IDLE, 0);
			CPUTS("-END");
...
void i2c_master_int_handler (int port)
{
	...
	if (IS_BIT_SET(NPCX_SMBST(port), NPCX_SMBST_BER)) {
		...
		SET_BIT(NPCX_SMBST(port), NPCX_SMBST_BER);
		/* Set error code */
		pStsObj->err_code = SMB_BUS_ERROR;
		/* Notify upper layer */
