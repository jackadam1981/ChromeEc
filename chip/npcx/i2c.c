...
struct i2c_status {
	...
	uint8_t               i2c_addr__7bf;  /* Target slave address */
...
{
	volatile struct i2c_status *p_status = i2c_stsobjs + controller;
	uint8_t ctrl_reg_val = I2C_ADDR__7b(p_status->i2c_addr__7bf) << 1;
	...
	if (p_status->oper_state == SMB_MASTER_START
		...
		if (p_status->sz_txbuf == 0) {/* Receive mode */
			...
			I2C_WRITE_BYTE(controller, (ctrl_reg_val | 0x1));
			CPRINTS("-ARR-0x%02x", addr);
		} else {/* Transmit mode */
			p_status->oper_state = SMB_WRITE_OPER;
			/* Write the address to the bus W bit*/
			I2C_WRITE_BYTE(controller, ctrl_reg_val);
		  ...
		  uint8_t *in, int in_size, int flags)
{
	...
	p_status->i2c_addr__7bf = i2c_addr__7bf;
