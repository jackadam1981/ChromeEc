...
#if defined(CONFIG_I2C_DATA_FIFO) && defined(CHIP_FAMILY_NPCX5)
#error "Do not enable CONFIG_I2C_DATA_FIFO if npcx ec is NPCX5 !"
#endif
...
#define I2C_SCL_STALL(ctrl)        (NPCX_SMBCTL3(ctrl) = \
			(NPCX_SMBCTL3(ctrl) & 0x7F) | 0x40)
/*
 ...
 */
#define I2C_SCL_FREE(ctrl)         (NPCX_SMBCTL3((ctrl)) |= 0xC0)
...
{
	/* Enable FIFO mode */
	if (IS_ENABLED(CONFIG_I2C_DATA_FIFO))
...
{
	NPCX_SMBCTL3(controller) = bank ?
			NPCX_SMBCTL3(controller) | BIT(NPCX_SMBCTL3_BNK_SEL) :
			NPCX_SMBCTL3(controller) & ~BIT(NPCX_SMBCTL3_BNK_SEL);
}
...
static void i2c_recovery(int controller, volatile struct i2c_status *p_status)
{
	...
	if (IS_ENABLED(CONFIG_I2C_DATA_FIFO)) {
		CLEAR_BIT(NPCX_SMBRXF_CTL(controller),
...
static void i2c_fifo_write_data(int controller)
{
	...
	len = len > fifo_avail ? fifo_avail : len;
	for (i = 0; i < len; i++) {
...
void i2c_handle_receive(int controller)
{
	uint8_t data;
	volatile struct i2c_status *p_status = i2c_stsobjs + controller;
...
void i2c_fifo_handle_receive(int controller)
{
