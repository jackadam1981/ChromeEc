...
/* I2C moudule automatically stall bus after sending peripheral address */
...
enum smb_error {
	...
	SMB_CONTROLLER_NO_ADDRESS_MATCH,/* No peripheral address match        */
					/*  (Controller Mode)                 */
...
struct i2c_status {
	...
	uint16_t              peripheral_addr_flags;/* Target address */
...
{
	volatile struct i2c_status *p_status = i2c_stsobjs + controller;
	uint8_t addr_8bit = I2C_STRIP_FLAGS(
		p_status->peripheral_addr_flags) << 1;
...
static void i2c_main_int_handler(int controller)
{
	volatile struct i2c_status *p_status = i2c_stsobjs + controller;
	...
	if (IS_BIT_SET(NPCX_SMBST(controller), NPCX_SMBST_NEGACK)) {
		...
		SET_BIT(NPCX_SMBST(controller), NPCX_SMBST_NEGACK);
		/* Set error code */
		p_status->err_code = SMB_CONTROLLER_NO_ADDRESS_MATCH;
		/* Notify upper layer */
		p_status->oper_state = SMB_IDLE;
		task_set_event(p_status->task_waiting, TASK_EVENT_I2C_IDLE);
		CPUTS("-NA");
...
{
	i2c_main_int_handler(controller);
}
		  ...
		  uint8_t *in, int in_size, int flags)
{
	...
	p_status->peripheral_addr_flags = peripheral_addr_flags;
