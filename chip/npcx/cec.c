...
static volatile uint32_t cec_enabled;
...
/*
 * Mutex for the circular buffer. Maybe this can be done lockless, but right
 * now it is used for allowing flushing the buffer when it becomes full
 */
static struct mutex circbuf_mutex;
...
{
	mutex_lock(&circbuf_mutex);
	cb->read_offset = 0;
	cb->write_offset = 0;
	mutex_unlock(&circbuf_mutex);
}
