 ...
 */
#define NOMINAL_BIT_TIME_TICKS APB1_TICKS(2400)
 /* Resend */
#define FREE_TIME_RS_TICKS (2 * (NOMINAL_BIT_TIME_TICKS))
/* New initiator */
#define FREE_TIME_NI_TICKS (4 * (NOMINAL_BIT_TIME_TICKS))
/* Present initiator */
#define FREE_TIME_PI_TICKS (6 * (NOMINAL_BIT_TIME_TICKS))
...
void enter_state(enum cec_state new_state)
{
	...
	case CEC_STATE_INITIATOR_ACK_VERIFY:
		 ...
		 */
		timeout = NOMINAL_BIT_TIME_TICKS - NOMINAL_SAMPLE_TIME_TICKS;
		break;
...
static void cec_event_cap(void)
{
	...
	case CEC_STATE_FOLLOWER_START_LOW:
		...
		break;
	case CEC_STATE_FOLLOWER_START_HIGH:
		if (VALID_HIGH(START_BIT, cec_rx.low_time_ticks, tmr_cap_get()))
	...
	case CEC_STATE_FOLLOWER_HEADER_INIT_HIGH:
		...
		if (VALID_DATA_HIGH(data, cec_rx.low_time_ticks, t)) {
