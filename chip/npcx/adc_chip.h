 ...
 * member variables.  For single threshold detection, only set thresh_assert to
 * the desired level.  The board must set thresh_deassert to -1 for single
 * threshold detection.  The lower_or_higher member controls the L_H bit.  If
 * set, a threshold event is asserted when the measured voltage is lower than or
 * equal to the threshold level. If not set, a threshold event is generated when
 * the measured voltage is higher than the threshold level. When the interrupt
 * fires, the callback, (adc_thresh_cb) if specified, will be executed.
 */
