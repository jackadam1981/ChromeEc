...
void cec_tmr_cap_start(int port, enum cec_cap_edge edge, int timeout)
{
	...
	if (edge == CEC_CAP_EDGE_NONE) {
		...
		UPDATE_BIT(NPCX_TMCTRL(mdl), capture_flags->edge,
...
void cec_disable_timer(int port)
{
	...
	/* If this was the only enabled timer, turn off interrupts */
	for (int port = 0; port < CEC_PORT_COUNT; port++) {
		if (npcx_cec_port[port].tcr)
			return;
	}
...
void cec_init_timer(int port)
{
	...
	if (port == 0) {
		...
		capture->edge = NPCX_TMCTRL_TAEDG;
		...
		capture->edge = NPCX_TMCTRL_TBEDG;
