...
void cec_tmr_cap_start(int port, enum cec_cap_edge edge, int timeout)
{
	...
	struct npcx_cec_timer *timer = cec_port->timer;
	const struct npcx_cec_timer_cfg *timer_cfg = cec_port->timer_cfg;
...
static void cec_isr(void)
{
	...
	for (int port = 0; port < CEC_PORT_COUNT; port++) {
		if (events &
		    BIT(npcx_cec_port[port].timer_cfg->capture.pending)) {
			/* Capture event */
			cec_event_cap(port);
		} else if (events &
			   BIT(npcx_cec_port[port]
				       .timer_cfg->underflow.pending)) {
			/*
			 * Capture timeout
...
void cec_enable_timer(int port)
{
	...
	SET_BIT(NPCX_TIEN(mdl), cec_port->timer_cfg->capture.interrupt);
...
void cec_disable_timer(int port)
{
	...
	CLEAR_BIT(NPCX_TIEN(mdl), cec_port->timer_cfg->capture.interrupt);
	...
	for (int p = 0; p < CEC_PORT_COUNT; p++) {
		if (IS_BIT_SET(NPCX_TIEN(mdl),
			       npcx_cec_port[p].timer_cfg->capture.interrupt))
			return;
	}
...
void cec_init_timer(int port)
{
	...
	cec_port->timer_cfg = &npcx_cec_timer_cfg[drv_config->timer];
	cec_port->timer = &npcx_cec_timer[drv_config->timer];
	...
	SET_BIT(NPCX_TMCTRL(mdl), cec_port->timer_cfg->enable);
}
