...
/*****************************************************************************/
/* Memory mapping */
#define CONFIG_BTRAM_SIZE  0x800 /* 2KB data ram used by booter. */
#define CONFIG_RAM_BASE    0x200C0000 /* memory address of data ram */
#define CONFIG_RAM_SIZE    (0x0008000 - CONFIG_BTRAM_SIZE) /* 30KB data ram */
...
#define CONFIG_NPCX_RAM_SIZE (CONFIG_BTRAM_SIZE + CONFIG_RAM_SIZE + \
				NPCX_PROGRAM_MEMORY_SIZE)
#if defined(CHIP_VARIANT_NPCX5M5G)
/* 128KB RAM in NPCX5M5G */
#if (CONFIG_NPCX_RAM_SIZE != 0x20000)
#error "Wrong memory mapping layout for NPCX5M5G"
#endif
#elif defined(CHIP_VARIANT_NPCX5M6G)
/* 256KB RAM in NPCX5M6G */
#if (CONFIG_NPCX_RAM_SIZE != 0x40000)
