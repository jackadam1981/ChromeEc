...
/* A struct isn't used since the function (not pointer) is at the addr*/
#define NCL_MISC_INIT_ADDR 0x000060D1UL
enum ncl_status (*ncl_misc_init)(void *f) = (void *)NCL_MISC_INIT_ADDR;

#define NCL_MISC ((const struct ncl_misc *)NCL_MISC_INIT_ADDR)
...
{
#ifdef CHIP_FAMILY_NPCX9
	enum ncl_status status = NCL_STATUS_FAIL;
	uint32_t context_size = 0;

	status = ncl_misc_init(0);
	if (status != NCL_STATUS_OK)
		ccprintf("ERROR! MISC init returned %x\n", status);
	...
	status = NCL_DRBG->init_context(ctx_p);
	if (status != NCL_STATUS_OK)
		ccprintf("ERROR! DRBG init_context returned %x\r", status);

	status = NCL_DRBG->init(ctx_p, 0);
	if (status != NCL_STATUS_OK)
		ccprintf("ERROR! DRBG init returned %x\r", status);

	status = NCL_DRBG->config(NULL, 100, false);
	if (status != NCL_STATUS_OK)
		ccprintf("ERROR! DRBG config returned %x\r", status);

	status = NCL_DRBG->instantiate(NULL, NCL_DRBG_SECURITY_STRENGTH_128b,
				       NULL, 0);
	if (status != NCL_STATUS_OK)
		ccprintf("ERROR! DRBG instantiate returned %x\r", status);
