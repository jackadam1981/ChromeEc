...
test_mockable void trng_init(void)
{
	...
	if (state_p->trng_init != NCL_STATUS_OK) {
		ccprintf("ERROR! DRBG init_context returned %x\r", state_p->trng_init);
		return;
	}
	...
	if (state_p->trng_init != NCL_STATUS_OK) {
		ccprintf("ERROR! DRBG config returned %x\r", state_p->trng_init);
		return;
	}

	state_p->trng_init = NCL_DRBG->instantiate(ctx_p,
							NCL_DRBG_SECURITY_STRENGTH_128b, NULL, 0);
	if (state_p->trng_init != NCL_STATUS_OK) {
		ccprintf("ERROR! DRBG instantiate returned %x\r", state_p->trng_init);
		return;
	}
...
test_mockable void trng_rand_bytes(void *buffer, size_t len)
{
	while (len) {
		...
		if (len < 4 || align) {
			cnt = MIN(sizeof(uint32_t) - align, len);
