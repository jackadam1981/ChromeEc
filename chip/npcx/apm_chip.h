...
/* APM interrupt mode. */
enum apm_interrupt_mode {
	APM_IRQ_ACT_HIGH = 0,
	APM_IRQ_ACT_LOW,
	APM_IRQ_ACT_HIGH_PLUSE_CLK_DUR,
	APM_IRQ_ACT_LOW_PLUSE_CLK_DUR
};
...
void apm_vad_config(int zero_cross_detec_enable, int vad_adc_wakeup_enable,
		...
		enum apm_vad_in_channel_src chan_src,
		uint8_t power_level_sens);
 ...
 * Enables/Disables the automatic gain.
 *
 * @param stereo_enable   - Stereo enabled flag, true means enable.
 *        agc_target      - Target output level of the ADC.
 *        nois_gate_en    - Noise Gate system select.
