...
const struct gpio_wui_item gpio_wui_table[] = {
	...
	{ {	{ NPCX_GPIO(B,3), NPCX_WUI(0,5,0) },
	    ...
	    { NPCX_GPIO_NONE, NPCX_WUI(0,5,6) },	/* Host Access Wake-Up	*/
...
int gpio_is_reboot_warm(void)
{
	...
	if (IS_BIT_SET(NPCX_RSTCTL,NPCX_RSTCTL_DBGRST_STS)  /* debugger warm reset */
	/* TODO: 5M5G has cleared WDRST_STS bit in booter */
#if !defined(CHIP_NPCX5M5G)
		|| (IS_BIT_SET(NPCX_T0CSR,NPCX_T0CSR_WDRST_STS)&&(IS_BIT_SET(NPCX_TWCFG,NPCX_TWCFG_WDRST_MODE))))      		/* watch-dog warm reset */
#else
		)
#endif
		return 1;
	...
	for (i = 0; i < ARRAY_SIZE(gpio_wui_table); i++) {
		...
		if(gpio_wui_table[i].irq == int_no){
			...
			if(wui_mask){
				...
				NPCX_WKPCL(map->wui_table , map->wui_group) = wui_mask;
