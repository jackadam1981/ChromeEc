...
/* Constants for i2c ports and MIWU WKINEN mapping */
struct i2c_wkinen_item {
	struct npcx_wui i2c_wkinen[2];
};

/* Constants for low-voltage WKINEN bypass mapping */
const struct i2c_wkinen_item i2c_wkinen_table[] = NPCX_I2C_WKINEN_TABLE;
#endif
...
void gpio_dis_low_voltage_pins_input(void)
{
	...
	for (i = 0; i < i2c_ports_used; i++) {
		const struct npcx_wui *wui =
			i2c_wkinen_table[i2c_ports[i].port].i2c_wkinen;

		/* Check SCL */
		g = gpio_list + i2c_ports[i].scl;
		if (g->flags & GPIO_SEL_1P8V)
			/* Disable input buffer for 1.8V i2c SCL */
			CLEAR_BIT(NPCX_WKINEN(wui->table, wui->group),
					wui->bit);

		/* Check SDA */
		wui++;
		g = gpio_list + i2c_ports[i].sda;
		if (g->flags & GPIO_SEL_1P8V)
			/* Disable input buffer for 1.8V i2c SDA */
			CLEAR_BIT(NPCX_WKINEN(wui->table, wui->group),
					wui->bit);
	}
}
...
void gpio_en_low_voltage_pins_input(void)
{
	...
	for (i = 0; i < i2c_ports_used; i++) {
		...
		g = gpio_list + i2c_ports[i].scl;
		if (g->flags & GPIO_SEL_1P8V)
			/* Enable input buffer for 1.8V i2c SCL */
