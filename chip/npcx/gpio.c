...
{
	uint8_t table = gpio_wui_table[signal].table;
	uint8_t group = gpio_wui_table[signal].group;
	uint8_t pmask = 1 << gpio_wui_table[signal].bit;
...
void gpio_set_flags_by_mask(uint32_t port, uint32_t mask, uint32_t flags)
{
	...
	if (flags & GPIO_INT_ANY) {
		...
		/* Set gpio's interrupt type if gpio_int is valid */
		if (gpio_int < ARRAY_SIZE(gpio_wui_table))
