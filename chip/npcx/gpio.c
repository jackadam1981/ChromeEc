...
{
	struct npcx_wui wui;
	uint8_t pmask, table, group;
	enum gpio_signal gpio_int = GPIO_COUNT;

	/*
	 * Only GPIOs which have interrupt handler can select interrupt type.
	 */
	...
	if (gpio_int >= GPIO_IH_COUNT)
		return;

	wui = gpio_wui_table[gpio_int];
	table = wui.table;
	group = wui.group;
	pmask = 1 << wui.bit;
 ...
 *
 * @param table	MIWU table for GPIO interrupt no.
 * @param group	MIWU table for GPIO interrupt no.
 */

static void gpio_interrupt(uint8_t table, uint8_t group)
{
	uint8_t wui_mask;
	int i;
...
#define GPIO_IRQ_FUNC(_irq_func, table, group)		\
void _irq_func(void)					\
{							\
	gpio_interrupt(table, group);			\
}
...
#endif
	{
		gpio_interrupt(MIWU_TABLE_0, MIWU_GROUP_5);
		gpio_interrupt(MIWU_TABLE_0, MIWU_GROUP_6);
		gpio_interrupt(MIWU_TABLE_0, MIWU_GROUP_7);
		gpio_interrupt(MIWU_TABLE_0, MIWU_GROUP_8);
	}
}
...
#endif
	{
		gpio_interrupt(MIWU_TABLE_0, MIWU_GROUP_1);
		gpio_interrupt(MIWU_TABLE_0, MIWU_GROUP_4);
	}
}

GPIO_IRQ_FUNC(__gpio_wk0b_interrupt, MIWU_TABLE_0, MIWU_GROUP_2);
GPIO_IRQ_FUNC(__gpio_wk0c_interrupt, MIWU_TABLE_0, MIWU_GROUP_3);
GPIO_IRQ_FUNC(__gpio_wk1a_interrupt, MIWU_TABLE_1, MIWU_GROUP_1);
GPIO_IRQ_FUNC(__gpio_wk1b_interrupt, MIWU_TABLE_1, MIWU_GROUP_2);
#ifndef HAS_TASK_KEYSCAN
/* Delcare GPIO irq functions for KSI pins if there's no keyboard scan task, */
GPIO_IRQ_FUNC(__gpio_wk1c_interrupt, MIWU_TABLE_1, MIWU_GROUP_3);
#endif
GPIO_IRQ_FUNC(__gpio_wk1d_interrupt, MIWU_TABLE_1, MIWU_GROUP_4);
GPIO_IRQ_FUNC(__gpio_wk1e_interrupt, MIWU_TABLE_1, MIWU_GROUP_5);
GPIO_IRQ_FUNC(__gpio_wk1f_interrupt, MIWU_TABLE_1, MIWU_GROUP_6);
GPIO_IRQ_FUNC(__gpio_wk1g_interrupt, MIWU_TABLE_1, MIWU_GROUP_7);
GPIO_IRQ_FUNC(__gpio_wk1h_interrupt, MIWU_TABLE_1, MIWU_GROUP_8);
