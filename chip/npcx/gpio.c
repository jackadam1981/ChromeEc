...
/* Investage Power consumpation by disable input buffer one by one */
static int command_invest_pwr(int argc, char **argv)
{
	uint8_t i;
	uint8_t offset;
	...
	if (argc == 2) {
		if (!strcasecmp(argv[1], "info")) {
			...
			ccprintf("Total Non-ISR GPIO declaration: %d\n",
						non_isr_gpio_num);
			ccprintf("Next GPIO Number to check by \"inv next\"\n");
			ccprintf("  offset: %d\n", offset);
			...
			while (1) {
				...
				if (gpio_is_i2c_pin(offset)) {
					...
					if ((flags & GPIO_INPUT) ||
						...
						ccprintf("Disable Input buffer!\n");
						gpio_enable_wake_up_input(offset, 0);
						idx++;
						break;
					}
...
DECLARE_CONSOLE_COMMAND(invest_pwr, command_invest_pwr, "", "");
