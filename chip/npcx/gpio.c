...
struct gpio_wui_table_item {
	const struct gpio_wui_item *items;
...
#define NPCX_WUI_TABLE_INT(irq)  gpio_wui_table_int##irq
#define NPCX_WUI_TABLE_ITEM(irq) { NPCX_WUI_TABLE_INT(irq), \
				   ARRAY_SIZE(NPCX_WUI_TABLE_INT(irq)), \
				   irq }
...
{
	int i, j, k;
	const struct gpio_wui_table_item *table = gpio_wui_tables;

	for (i = 0; i < ARRAY_SIZE(gpio_wui_tables); i++, table++) {
		for (j = 0; j < table->sz_wui_items; j++) {
			const struct gpio_wui_item *wui_item = &table->items[j];
...
static void gpio_init(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(gpio_wui_tables); i++, table++)
		if (table->irq < NPCX_IRQ_COUNT)
			task_enable_irq(table->irq);
...
static void gpio_interrupt(const struct gpio_wui_item *wui_items,
		int sz_wui_items)
{
	uint8_t i, pin, wui_mask;

	for (i = 0; i < sz_wui_items; i++, wui_items++) {
		const struct npcx_gpio *gpio = wui_items->gpio;
		uint8_t table = wui_items->table;
		uint8_t group = wui_items->group;
