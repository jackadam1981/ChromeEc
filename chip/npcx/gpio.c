...
static void gpio_interrupt_type_sel(enum gpio_signal signal, uint32_t flags)
{
	...
	/* skip if not an interrupt handler */
...
static void gpio_set_flags_with_int(uint32_t port, uint32_t mask,
				    uint32_t flags, enum gpio_signal gpio_int)
{
	/*
...
{
	enum gpio_signal gpio_int = GPIO_COUNT;

	/*
	 * Optimization: look-up for interrupt gpio number
	 * only if we have an interrupt flag.
	 * Note: this won't disable interrupts...
	 */
	if (flags & GPIO_INT_ANY) {
		const struct gpio_info *g = gpio_list;

		for (gpio_int = 0; gpio_int < GPIO_COUNT; gpio_int++, g++)
			if ((g->port == port) && (g->mask & mask))
				break;
	}

	gpio_set_flags_with_int(port, mask, flags, gpio_int);
}
...
int gpio_enable_interrupt(enum gpio_signal signal)
{
	...
	wui = gpio_wui[signal];
	/* Set MIWU enable bit */
	NPCX_WKEN(WUI_MIWU(wui), WUI_GROUP(wui)) |= 1 << WUI_BIT(wui);
...
int gpio_clear_pending_interrupt(enum gpio_signal signal)
{
	...
	wui = gpio_wui[signal];
	NPCX_WKPCL(WUI_MIWU(wui), WUI_GROUP(wui)) |= 1 << WUI_BIT(wui);
...
void gpio_pre_init(void)
{
	...
	for (i = 0; i < GPIO_COUNT; i++, g++) {
		...
		gpio_set_flags_with_int(g->port, g->mask, flags, i);
...
static void gpio_interrupt(uint8_t wui)
{
	int miwu = WUI_MIWU(wui);
	int group = WUI_GROUP(wui);
	uint8_t wui_mask;
	int i;

	/* Get pending mask */
	wui_mask = NPCX_WKPND(miwu, group) & NPCX_WKEN(miwu, group);

	/* Find GPIOs and execute interrupt service routine */
	for (i = 0; i < GPIO_IH_COUNT && wui_mask; i++) {
		uint8_t pin_mask = 1 << WUI_BIT(gpio_wui[i]);

		if (((gpio_wui[i] & 0xF0) == wui) && (wui_mask & pin_mask)) {
			/* Clear pending bit of GPIO */
			NPCX_WKPCL(miwu, group) = pin_mask;
			/* Execute GPIO's ISR */
...
#define GPIO_IRQ_FUNC(_irq_func, wui)		\
void _irq_func(void)				\
{						\
	gpio_interrupt(wui);			\
}
...
#endif
	{
		gpio_interrupt(WUI(0, MIWU_GROUP_5, 0));
		gpio_interrupt(WUI(0, MIWU_GROUP_6, 0));
		gpio_interrupt(WUI(0, MIWU_GROUP_7, 0));
		gpio_interrupt(WUI(0, MIWU_GROUP_8, 0));
	}
}
...
#endif
	{
		gpio_interrupt(WUI(0, MIWU_GROUP_1, 0));
		gpio_interrupt(WUI(0, MIWU_GROUP_4, 0));
	}
}

GPIO_IRQ_FUNC(__gpio_wk0b_interrupt, WUI(0, MIWU_GROUP_2, 0));
GPIO_IRQ_FUNC(__gpio_wk0c_interrupt, WUI(0, MIWU_GROUP_3, 0));
GPIO_IRQ_FUNC(__gpio_wk1a_interrupt, WUI(1, MIWU_GROUP_1, 0));
GPIO_IRQ_FUNC(__gpio_wk1b_interrupt, WUI(1, MIWU_GROUP_2, 0));
#ifndef HAS_TASK_KEYSCAN
/* Delcare GPIO irq functions for KSI pins if there's no keyboard scan task, */
GPIO_IRQ_FUNC(__gpio_wk1c_interrupt, WUI(1, MIWU_GROUP_3, 0));
#endif
GPIO_IRQ_FUNC(__gpio_wk1d_interrupt, WUI(1, MIWU_GROUP_4, 0));
GPIO_IRQ_FUNC(__gpio_wk1e_interrupt, WUI(1, MIWU_GROUP_5, 0));
GPIO_IRQ_FUNC(__gpio_wk1f_interrupt, WUI(1, MIWU_GROUP_6, 0));
GPIO_IRQ_FUNC(__gpio_wk1g_interrupt, WUI(1, MIWU_GROUP_7, 0));
GPIO_IRQ_FUNC(__gpio_wk1h_interrupt, WUI(1, MIWU_GROUP_8, 0));
