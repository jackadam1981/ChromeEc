...
{
	gpio_set_level(GPIO_KSO_00, (level >> 0) & 1);
	gpio_set_level(GPIO_KSO_01, (level >> 1) & 1);
	gpio_set_level(GPIO_KSO_02, (level >> 2) & 1);
	gpio_set_level(GPIO_KSO_03, (level >> 3) & 1);
	gpio_set_level(GPIO_KSO_04, (level >> 4) & 1);
	gpio_set_level(GPIO_KSO_05, (level >> 5) & 1);
	gpio_set_level(GPIO_KSO_06, (level >> 6) & 1);
	gpio_set_level(GPIO_KSO_07, (level >> 7) & 1);
	gpio_set_level(GPIO_KSO_08, (level >> 8) & 1);
	gpio_set_level(GPIO_KSO_09, (level >> 9) & 1);
	gpio_set_level(GPIO_KSO_10, (level >> 10) & 1);
	gpio_set_level(GPIO_KSO_11, (level >> 11) & 1);
	gpio_set_level(GPIO_KSO_12, (level >> 12) & 1);
	gpio_set_level(GPIO_KSO_13, (level >> 13) & 1);
	gpio_set_level(GPIO_KSO_14, (level >> 14) & 1);
}
...
void keyboard_raw_enable_interrupt(int enable)
{
	if (enable) {
		...
		gpio_enable_interrupt(GPIO_RFR_KEY);
	} else {
		...
		gpio_disable_interrupt(GPIO_RFR_KEY);
	}
}

void keyboard_raw_gpio_interrupt(enum gpio_signal signal)
{
	CPRINTS("*** Interrupted by key press, waking Keyscan task ***");
