...
{
	int mft_pwm_id = fans[ch].ch;
	int mdl = mft_channels[mft_pwm_id].module;

	/* Set the clock source type and start counting */
	SET_FIELD(NPCX_TCKC(mdl), NPCX_TCKC_C1CSEL_FIELD,
			mft_channels[mft_pwm_id].clk_src);
}
...
static int mft_final_measure(int ch)
{
	 ...
	 */
	tacho = mft_channels[mft_pwm_id].default_count - NPCX_TCRA(mdl);
...
static void fan_config(int ch, int enable_mft_read_rpm)
{
	int mft_pwm_id = fans[ch].ch;
	int mdl = mft_channels[mft_pwm_id].module;
	enum npcx_mft_clk_src clk_src = mft_channels[mft_pwm_id].clk_src;
	...
	if (enable_mft_read_rpm) {
		...
		NPCX_TCNT1(mdl) = mft_channels[mft_pwm_id].default_count;
		NPCX_TCRA(mdl)  = mft_channels[mft_pwm_id].default_count;
...
void fan_set_duty(int ch, int percent)
{
	...
	pwm_set_duty(mft_pwm_id, percent);
...
int fan_get_duty(int ch)
{
	...
	/* Return percent */
	return pwm_get_duty(mft_pwm_id);
}
/**
...
{
	return pwm_get_enabled(fans[ch].ch);
}
/**
 * Set fan enabled.
 *
