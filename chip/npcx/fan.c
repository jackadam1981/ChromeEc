...
#define TACH_TO_RPM(mft_channel, tach) \
	((mft_channels[mft_channel].freq)*30*RPM_EDGES*RPM_SCALE/ MAX((tach), 1))
...
static void mft_finalmeasure (int ch)
{
	...
	/* Start of the last tacho cycle is detected - calculated tacho cycle duration */
    if (NPCX_MFT_MODULE_PORT_TB == mft_channels[mft_ch].port)
		tacho_status.edge_interval = (uint32_t)(mft_channels[mft_ch].default_count
    ...
    if ((fan_get_enabled(ch)) && (fan_get_duty(ch))) {
        ...
        mft_stopmeasure(ch);
					...
					rpm_actual = (tacho_status.edge_interval>0) ?
						(TACH_TO_RPM(mft_ch, tacho_status.edge_interval)) : 0;
        /* Back to Idle mode*/
        tacho_status.cur_state = TACHO_IN_IDLE;
        		    CPRINTS("TACHO_WAIT_FOR_2_EDGE");
        CPRINTS("edge_interval=%x", tacho_status.edge_interval);
