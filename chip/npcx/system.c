...
void system_jump_to_booter(void)
{
	...
	if (IS_BIT_SET(NPCX_FWCTRL, NPCX_FWCTRL_RO_REGION)) {
		flash_offset = CONFIG_RO_MEM_OFF + CONFIG_RO_HEADER_SIZE;
		flash_used = CONFIG_RO_SIZE - CONFIG_RO_HEADER_SIZE;
	} else { /* RW region FW */
	...
	addr_entry = *(uintptr_t *)(base_addr + 4);
...
static void system_sysjump(void)
{
	...
	if (base_addr == CONFIG_FLASH_BASE + CONFIG_RW_MEM_OFF)
