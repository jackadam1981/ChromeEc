...
uintptr_t __lpram_lfw_start = CONFIG_LPRAM_BASE + 0x160;
...
void __keep __attribute__ ((section(".lowpower_ram2")))
__start_gdma(uint32_t exeAddr)
{
	...
	/* Jump to the exeAddr address if needed. */
	if (exeAddr != 0x0)
		...
		uint32_t size, uint32_t exeAddr)
{
	...
	/* Check alignment of the srcAddr & dstAddr. */
	if ((srcAddr & 0x03) != 0x0 || (dstAddr & 0x03) != 0x0)
		return;

	/* Check size validation */
	if ((size == 0) || (size & 0x03) != 0x0)
		return;

	/* Check alignment for burst mode */
	if ((size % chunkSize) != 0 && (srcAddr % chunkSize) != 0 &&
			(dstAddr % chunkSize) != 0)
		return;
