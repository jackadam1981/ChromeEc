...
void system_check_bbram_on_reset(void)
{
	if (IS_BIT_SET(NPCX_BKUP_STS, NPCX_BKUP_STS_IBBR)) {
		system_set_reset_flags(EC_RESET_FLAG_BROWNOUT);
		/*
...
static int bbram_data_write(enum bbram_data_index index, uint32_t value)
{
	...
	if (IS_BIT_SET(NPCX_BKUP_STS, NPCX_BKUP_STS_IBBR)) {
		...
		CPRINTS("Cleared IBBR");
		bbram_data_write(BBRM_DATA_INDEX_SAVED_RESET_FLAGS,
...
static void check_reset_cause(void)
{
	...
	system_set_reset_flags(system_get_reset_flags() | flags);
}
