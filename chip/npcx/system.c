...
void system_pre_init(void)
{
	...
	if (IS_ENABLED(CONFIG_HIBERNATE_PSL)) {
		 ...
		 * the booter.
		 */
		/* Enable VCC1_RST as the wake-up source from hibernation */
		SET_BIT(NPCX_GLUE_PSL_MCTL1, NPCX_GLUE_PSL_MCTL1_VCC1_RST_PSL);
		/* Disable VCC_RST Pull-Up */
		SET_BIT(NPCX_DEVALT(ALT_GROUP_G), NPCX_DEVALTG_VCC1_RST_PUD);
		/*
		 * Lock this bit itself and VCC1_RST_PSL in the PSL_MCTL1
		 * register to read-only.
		 */
