...
#if NPCX_FAMILY_VERSION <= NPCX_FAMILY_NPCX7
		/*
		 * Don't set PD of ITIM6 for NPCX9 and latter chips because
...
const char *system_get_chip_revision(void)
{
	...
	*p++ = '.';
	 ...
	 * For NPCX9 and latter chips, the revision number is 4 bytes.
	 */
	for (int s = sizeof(rev_num) - 1; s >= 0; s--) {
		uint8_t rev = rev_num >> (s * 8);

		*p++ = system_to_hex((rev & 0xF0) >> 4);
		*p++ = system_to_hex(rev & 0x0F);
	}
	*p++ = '\0';
