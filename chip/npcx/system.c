...
#endif
	default:
		*(p + 8) = system_to_hex((chip_gen & 0xF0) >> 4);
		*(p + 9) = system_to_hex(chip_gen & 0x0F);
		p = rev + 9;
		break;
	}

	*(p + 1) = '.';
	*(p + 2) = system_to_hex((rev_num & 0xF0) >> 4);
	*(p + 3) = system_to_hex(rev_num & 0x0F);
	*(p + 4) = '\0';
