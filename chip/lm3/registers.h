...
#define LM3_ADC_ADCTMLB        REG32(0x40038100)
...
static inline int lm3_adc_addr(int ss, int offset)
{
	return offset + LM3_ADC_SS0_BASE + LM3_ADC_SS_SEP * ss;
}
#define LM3ADCREG(ch, offset) REG32(lm3_adc_addr(ch, offset))

#define LM3_ADC_SSMUX(ss)      LM3ADCREG(ss, 0x000)
#define LM3_ADC_SSCTL(ss)      LM3ADCREG(ss, 0x004)
#define LM3_ADC_SSFIFO(ss)     LM3ADCREG(ss, 0x008)
#define LM3_ADC_SSFSTAT(ss)    LM3ADCREG(ss, 0x00c)
