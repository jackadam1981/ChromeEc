...
#define IRQ_UART_HOST CONCAT2(LM3_IRQ_UART, CONFIG_UART_HOST)
#endif
...
void uart_tx_start(void)
{
	 ...
	 */
	LM3_UART_IM(0) |= 0x20;
	task_trigger_irq(LM3_IRQ_UART0);
}
...
void uart_tx_flush(void)
{
	...
	while (!(LM3_UART_FR(0) & 0x80))
		;
}

int uart_tx_ready(void)
{
	return !(LM3_UART_FR(0) & 0x20);
}

int uart_tx_in_progress(void)
{
	/* Transmit is in progress if the TX busy bit is set. */
	return LM3_UART_FR(0) & 0x08;
}

int uart_rx_available(void)
{
	return !(LM3_UART_FR(0) & 0x10);
}
...
{
	return LM3_UART_DR(0);
}

static void uart_clear_rx_fifo(int channel)
{
	volatile int scratch __attribute__ ((unused));

	while (!(LM3_UART_FR(channel) & 0x10))
		scratch = LM3_UART_DR(channel);
}
...
DECLARE_IRQ(LM3_IRQ_UART0, uart_ec_interrupt, 1);
...
{
	/* Clear transmit and receive interrupt status */
	LM3_UART_ICR(CONFIG_UART_HOST) = 0x70;
	 ...
	 */
	if (!(LM3_UART_FR(CONFIG_UART_HOST) & 0x20) && lpc_comx_has_char()) {
		/* Copy the next byte then disable transmit interrupt */
		LM3_UART_DR(CONFIG_UART_HOST) = lpc_comx_get_char();
		LM3_UART_IM(CONFIG_UART_HOST) &= ~0x20;
	}
	 ...
	 */
	if (!(LM3_UART_FR(CONFIG_UART_HOST) & 0x10))
		lpc_comx_put_char(LM3_UART_DR(CONFIG_UART_HOST));
#endif
}
...
{
	/* Disable the port */
	LM3_UART_CTL(port) = 0x0300;
	/* Use the internal oscillator */
	LM3_UART_CC(port) = 0x1;
	/* Set the baud rate divisor */
	LM3_UART_IBRD(port) = (INTERNAL_CLOCK / 16) / CONFIG_UART_BAUD_RATE;
	LM3_UART_FBRD(port) =
	 ...
	 */
	LM3_UART_IM(port) = 0x50;
	/* Enable the port */
	LM3_UART_CTL(port) |= 0x0001;
}
...
void uart_init(void)
{
	 ...
	 */
	mask |= 1;
	clock_enable_peripheral(CGC_OFFSET_UART0, mask, CGC_MODE_ALL);

#ifdef CONFIG_UART_HOST
	mask |= (1 << CONFIG_UART_HOST);
#endif

	clock_enable_peripheral(CGC_OFFSET_UART0, mask,
...
void uart_enter_dsleep(void)
{
	...
	task_disable_irq(LM3_IRQ_UART0);
...
{
	if (LM3_UART_FR(CONFIG_UART_HOST) & 0x20) {
		/*
...
{
	LM3_UART_DR(CONFIG_UART_HOST) = c;
}
