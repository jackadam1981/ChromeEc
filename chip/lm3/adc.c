...
/* LM3-specific ADC module for Chrome EC */
...
static volatile task_id_t task_waiting_on_ss[LM3_ADC_SEQ_COUNT];
...
static int flush_and_read(enum lm4_adc_sequencer seq)
{
	...
	while (!(LM3_ADC_SSFSTAT(seq) & 0x100))
		scratch = LM3_ADC_SSFIFO(seq);
	...
	LM3_ADC_ADCISC |= 0x01 << seq;

	/* Enable interrupt */
	LM3_ADC_ADCIM |= 0x01 << seq;

	/* Initiate sample sequence */
	LM3_ADC_ADCPSSI |= 0x01 << seq;
...
static void adc_configure(const struct adc_t *adc)
{
	...
	LM3_ADC_ADCACTSS &= ~(0x01 << seq);

	/* Trigger sequencer by processor request */
	LM3_ADC_ADCEMUX = (LM3_ADC_ADCEMUX & ~(0xf << (seq * 4))) | 0x00;

	/* Sample internal temp sensor */
	if (adc->channel == LM3_AIN_NONE) {
		LM3_ADC_SSMUX(seq) = 0x00;
		LM3_ADC_SSEMUX(seq) = 0x00;
	} else {
		LM3_ADC_SSMUX(seq) = adc->channel & 0xf;
		LM3_ADC_SSEMUX(seq) = adc->channel >> 4;
	}
	LM3_ADC_SSCTL(seq) = adc->flag;
...
DECLARE_IRQ(LM3_IRQ_ADC0_SS0, ss0_interrupt, 2);
DECLARE_IRQ(LM3_IRQ_ADC0_SS1, ss1_interrupt, 2);
DECLARE_IRQ(LM3_IRQ_ADC0_SS2, ss2_interrupt, 2);
DECLARE_IRQ(LM3_IRQ_ADC0_SS3, ss3_interrupt, 2);
...
static void adc_init(void)
{
	...
	for (i = 0; i < LM3_ADC_SEQ_COUNT; i++)
		task_waiting_on_ss[i] = TASK_ID_INVALID;

	/* Enable IRQs */
	task_enable_irq(LM3_IRQ_ADC0_SS0);
	task_enable_irq(LM3_IRQ_ADC0_SS1);
	task_enable_irq(LM3_IRQ_ADC0_SS2);
	task_enable_irq(LM3_IRQ_ADC0_SS3);
