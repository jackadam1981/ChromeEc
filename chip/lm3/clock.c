...
{
	/* Switch to 12MHz internal oscillator and power down the PLL */
	LM3_SYSTEM_RCC = LM3_SYSTEM_RCC_SYSDIV(0) |
		LM3_SYSTEM_RCC_BYPASS |
		LM3_SYSTEM_RCC_PWRDN |
		LM3_SYSTEM_RCC_OSCSRC(1) |
		LM3_SYSTEM_RCC_MOSCDIS;
	 ...
	 */
	LM3_SYSTEM_RCC |= LM3_SYSTEM_RCC_ACG;
#endif
...
static void enable_pll(void)
{
	 ...
	 */
	LM3_SYSTEM_RCC = LM3_SYSTEM_RCC_XTAL(0xB) |
		LM3_SYSTEM_RCC_SYSDIV(2) |
		LM3_SYSTEM_RCC_USESYSDIV |
		LM3_SYSTEM_RCC_BYPASS |
		LM3_SYSTEM_RCC_OSCSRC(0);
	...
	while (!(LM3_SYSTEM_RIS & LM3_SYSTEM_RIS_PLLLRIS))
		;

	/* Remove bypass on PLL */
	LM3_SYSTEM_RCC &= ~LM3_SYSTEM_RCC_BYPASS;
	freq = PLL_CLOCK;
}
...
{
	uint32_t cgc[] = {
		LM3_SYSTEM_RCGC0_PWM, LM3_SYSTEM_RCGC0_ADC,
		LM3_SYSTEM_RCGC0_WDT, LM3_SYSTEM_RCGC1_COMP0,
		LM3_SYSTEM_RCGC1_TIMER2, LM3_SYSTEM_RCGC1_TIMER1,
		LM3_SYSTEM_RCGC1_TIMER0, LM3_SYSTEM_RCGC1_I2C0,
		LM3_SYSTEM_RCGC1_SSIO, LM3_SYSTEM_RCGC1_UART1,
		LM3_SYSTEM_RCGC1_UART0,	LM3_SYSTEM_RCGC2_GPIOA,
		LM3_SYSTEM_RCGC2_GPIOB,	LM3_SYSTEM_RCGC2_GPIOC,
		LM3_SYSTEM_RCGC2_GPIOD,	LM3_SYSTEM_RCGC2_GPIOE };

	if (mode & CGC_MODE_RUN) {
		switch (offset) {
		case CGC_OFFSET_PWM ... CGC_OFFSET_WDT:
			LM3_SYSTEM_RCGC0 |= mask ? cgc[offset] : 0;
			break;
		case CGC_OFFSET_COMP0 ... CGC_OFFSET_UART0:
			LM3_SYSTEM_RCGC1 |= mask ? cgc[offset] : 0;
			break;
		case CGC_OFFSET_GPIOA ... CGC_OFFSET_GPIOE:
			LM3_SYSTEM_RCGC2 |= mask ? cgc[offset] : 0;
			break;
		}
	}
	...
	if (mode & CGC_MODE_SLEEP) {
		switch (offset) {
		case CGC_OFFSET_PWM ... CGC_OFFSET_WDT:
			LM3_SYSTEM_SCGC0 |= mask ? cgc[offset] : 0;
			break;
		case CGC_OFFSET_COMP0 ... CGC_OFFSET_UART0:
			LM3_SYSTEM_SCGC1 |= mask ? cgc[offset] : 0;
			break;
		case CGC_OFFSET_GPIOA ... CGC_OFFSET_GPIOE:
			LM3_SYSTEM_SCGC2 |= mask ? cgc[offset] : 0;
			break;
		}
	}
	...
	if (mode & CGC_MODE_DSLEEP) {
		switch (offset) {
		case CGC_OFFSET_PWM ... CGC_OFFSET_WDT:
			LM3_SYSTEM_DCGC0 |= mask ? cgc[offset] : 0;
			break;
		case CGC_OFFSET_COMP0 ... CGC_OFFSET_UART0:
			LM3_SYSTEM_DCGC1 |= mask ? cgc[offset] : 0;
			break;
		case CGC_OFFSET_GPIOA ... CGC_OFFSET_GPIOE:
			LM3_SYSTEM_DCGC2 |= mask ? cgc[offset] : 0;
			break;
		}
	}
...
void clock_disable_peripheral(uint32_t offset, uint32_t mask, uint32_t mode)
{
	...
	if (mode & CGC_MODE_RUN) {
		switch (offset) {
		case CGC_OFFSET_PWM ... CGC_OFFSET_WDT:
			LM3_SYSTEM_RCGC0 &= mask ? ~cgc[offset] : -1;
			break;
		case CGC_OFFSET_COMP0 ... CGC_OFFSET_UART0:
			LM3_SYSTEM_RCGC1 &= mask ? ~cgc[offset] : -1;
			break;
		case CGC_OFFSET_GPIOA ... CGC_OFFSET_GPIOE:
			LM3_SYSTEM_RCGC2 &= mask ? ~cgc[offset] : -1;
			break;
		}
	}
	...
	if (mode & CGC_MODE_SLEEP) {
		switch (offset) {
		case CGC_OFFSET_PWM ... CGC_OFFSET_WDT:
			LM3_SYSTEM_SCGC0 &= mask ? ~cgc[offset] : -1;
			break;
		case CGC_OFFSET_COMP0 ... CGC_OFFSET_UART0:
			LM3_SYSTEM_SCGC1 &= mask ? ~cgc[offset] : -1;
			break;
		case CGC_OFFSET_GPIOA ... CGC_OFFSET_GPIOE:
			LM3_SYSTEM_SCGC2 &= mask ? ~cgc[offset] : -1;
			break;
		}
	}
	...
	if (mode & CGC_MODE_DSLEEP) {
		switch (offset) {
		case CGC_OFFSET_PWM ... CGC_OFFSET_WDT:
			LM3_SYSTEM_DCGC0 &= mask ? ~cgc[offset] : -1;
			break;
		case CGC_OFFSET_COMP0 ... CGC_OFFSET_UART0:
			LM3_SYSTEM_DCGC1 &= mask ? ~cgc[offset] : -1;
			break;
		case CGC_OFFSET_GPIOA ... CGC_OFFSET_GPIOE:
			LM3_SYSTEM_DCGC2 &= mask ? ~cgc[offset] : -1;
			break;
		}
	}
}
...
void __idle(void)
{
	...
	while (1) {
		...
		if (DEEP_SLEEP_ALLOWED && time_for_dsleep) {
			 ...
			 */
			LM3_SYSTEM_DSLPCLKCFG = use_low_speed_clock ?
					0x32 : 0x10;
#else
			/*
			 * Set the deep sleep clock register. Use either the
...
static int command_sleep(int argc, char **argv)
{
	...
	if (clock) {
		...
		if (uartfbrd) {
			...
			LM3_UART_CTL(0) = 0x0320;
			/* Set the baud rate divisor. */
			LM3_UART_IBRD(0) = uartibrd;
			LM3_UART_FBRD(0) = uartfbrd;
			/* Poke UARTLCRH to make the new divisor take effect. */
			LM3_UART_LCRH(0) = LM3_UART_LCRH(0);
			/* Enable the port. */
			LM3_UART_CTL(0) |= 0x0001;
		}
	...
	if (level & 1) {
		clock_disable_peripheral(CGC_OFFSET_WDT,     0xffffffff,
				CGC_MODE_ALL);
	...
	LM3_SYSTEM_SLPPWRCFG = (flash_pm << 4) | sram_pm;
	LM3_SYSTEM_DSLPPWRCFG = (flash_pm << 4) | sram_pm;
...
static int command_pll(int argc, char **argv)
{
	...
	if (argc > 1) {
		if (parse_bool(argv[1], &v)) {
			...
			LM3_SYSTEM_RCC = LM3_SYSTEM_RCC_SYSDIV(v - 1) |
				LM3_SYSTEM_RCC_BYPASS |
				LM3_SYSTEM_RCC_PWRDN |
				LM3_SYSTEM_RCC_OSCSRC(1) |
				LM3_SYSTEM_RCC_MOSCDIS;
...
static int command_clock_gating(int argc, char **argv)
{
	...
	ccprintf("PWM:      0x%08x, ",
				LM3_SYSTEM_RCGC0 & LM3_SYSTEM_RCGC0_PWM);
	ccprintf("0x%08x, ",	LM3_SYSTEM_SCGC0 & LM3_SYSTEM_RCGC0_PWM);
	ccprintf("0x%08x\n",	LM3_SYSTEM_DCGC0 & LM3_SYSTEM_RCGC0_PWM);

	ccprintf("ADC:      0x%08x, ",
				LM3_SYSTEM_RCGC0 & LM3_SYSTEM_RCGC0_ADC);
	ccprintf("0x%08x, ",	LM3_SYSTEM_SCGC0 & LM3_SYSTEM_RCGC0_ADC);
	ccprintf("0x%08x\n",	LM3_SYSTEM_DCGC0 & LM3_SYSTEM_RCGC0_ADC);

	ccprintf("WDT:      0x%08x, ",
				LM3_SYSTEM_RCGC0 & LM3_SYSTEM_RCGC0_WDT);
	ccprintf("0x%08x, ",	LM3_SYSTEM_SCGC0 & LM3_SYSTEM_RCGC0_WDT);
	ccprintf("0x%08x\n",	LM3_SYSTEM_DCGC0 & LM3_SYSTEM_RCGC0_WDT);

	ccprintf("COMP0:    0x%08x, ",
				LM3_SYSTEM_RCGC1 & LM3_SYSTEM_RCGC1_COMP0);
	ccprintf("0x%08x, ",	LM3_SYSTEM_SCGC1 & LM3_SYSTEM_RCGC1_COMP0);
	ccprintf("0x%08x\n",	LM3_SYSTEM_DCGC1 & LM3_SYSTEM_RCGC1_COMP0);

	ccprintf("TIMER0:   0x%08x, ",
				LM3_SYSTEM_RCGC1 & LM3_SYSTEM_RCGC1_TIMER0);
	ccprintf("0x%08x, ",	LM3_SYSTEM_SCGC1 & LM3_SYSTEM_RCGC1_TIMER0);
	ccprintf("0x%08x\n",	LM3_SYSTEM_DCGC1 & LM3_SYSTEM_RCGC1_TIMER0);

	ccprintf("TIMER1:   0x%08x, ",
				LM3_SYSTEM_RCGC1 & LM3_SYSTEM_RCGC1_TIMER1);
	ccprintf("0x%08x, ",	LM3_SYSTEM_SCGC1 & LM3_SYSTEM_RCGC1_TIMER1);
	ccprintf("0x%08x\n",	LM3_SYSTEM_DCGC1 & LM3_SYSTEM_RCGC1_TIMER1);

	ccprintf("TIMER2:   0x%08x, ",
				LM3_SYSTEM_RCGC1 & LM3_SYSTEM_RCGC1_TIMER2);
	ccprintf("0x%08x, ",	LM3_SYSTEM_SCGC1 & LM3_SYSTEM_RCGC1_TIMER2);
	ccprintf("0x%08x\n",	LM3_SYSTEM_DCGC1 & LM3_SYSTEM_RCGC1_TIMER2);

	ccprintf("I2C0:     0x%08x, ",
				LM3_SYSTEM_RCGC1 & LM3_SYSTEM_RCGC1_I2C0);
	ccprintf("0x%08x, ",	LM3_SYSTEM_SCGC1 & LM3_SYSTEM_RCGC1_I2C0);
	ccprintf("0x%08x\n",	LM3_SYSTEM_DCGC1 & LM3_SYSTEM_RCGC1_I2C0);

	ccprintf("SSIO:     0x%08x, ",
				LM3_SYSTEM_RCGC1 & LM3_SYSTEM_RCGC1_SSIO);
	ccprintf("0x%08x, ",	LM3_SYSTEM_SCGC1 & LM3_SYSTEM_RCGC1_SSIO);
	ccprintf("0x%08x\n",	LM3_SYSTEM_DCGC1 & LM3_SYSTEM_RCGC1_SSIO);

	ccprintf("UART0:    0x%08x, ",
				LM3_SYSTEM_RCGC1 & LM3_SYSTEM_RCGC1_UART0);
	ccprintf("0x%08x, ",	LM3_SYSTEM_SCGC1 & LM3_SYSTEM_RCGC1_UART0);
	ccprintf("0x%08x\n",	LM3_SYSTEM_DCGC1 & LM3_SYSTEM_RCGC1_UART0);

	ccprintf("UART1:    0x%08x, ",
				LM3_SYSTEM_RCGC1 & LM3_SYSTEM_RCGC1_UART1);
	ccprintf("0x%08x, ",	LM3_SYSTEM_SCGC1 & LM3_SYSTEM_RCGC1_UART1);
	ccprintf("0x%08x\n",	LM3_SYSTEM_DCGC1 & LM3_SYSTEM_RCGC1_UART1);

	ccprintf("GPIOA:    0x%08x, ",
				LM3_SYSTEM_RCGC2 & LM3_SYSTEM_RCGC2_GPIOA);
	ccprintf("0x%08x, ",	LM3_SYSTEM_SCGC2 & LM3_SYSTEM_RCGC2_GPIOA);
	ccprintf("0x%08x\n",	LM3_SYSTEM_DCGC2 & LM3_SYSTEM_RCGC2_GPIOA);

	ccprintf("GPIOB:    0x%08x, ",
				LM3_SYSTEM_RCGC2 & LM3_SYSTEM_RCGC2_GPIOB);
	ccprintf("0x%08x, ",	LM3_SYSTEM_SCGC2 & LM3_SYSTEM_RCGC2_GPIOB);
	ccprintf("0x%08x\n",	LM3_SYSTEM_DCGC2 & LM3_SYSTEM_RCGC2_GPIOB);

	ccprintf("GPIOC:    0x%08x, ",
				LM3_SYSTEM_RCGC2 & LM3_SYSTEM_RCGC2_GPIOC);
	ccprintf("0x%08x, ",	LM3_SYSTEM_SCGC2 & LM3_SYSTEM_RCGC2_GPIOC);
	ccprintf("0x%08x\n",	LM3_SYSTEM_DCGC2 & LM3_SYSTEM_RCGC2_GPIOC);

	ccprintf("GPIOD:    0x%08x, ",
				LM3_SYSTEM_RCGC2 & LM3_SYSTEM_RCGC2_GPIOD);
	ccprintf("0x%08x, ",	LM3_SYSTEM_SCGC2 & LM3_SYSTEM_RCGC2_GPIOD);
	ccprintf("0x%08x\n",	LM3_SYSTEM_DCGC2 & LM3_SYSTEM_RCGC2_GPIOD);

	ccprintf("GPIOE:    0x%08x, ",
				LM3_SYSTEM_RCGC2 & LM3_SYSTEM_RCGC2_GPIOE);
	ccprintf("0x%08x, ",	LM3_SYSTEM_SCGC2 & LM3_SYSTEM_RCGC2_GPIOE);
	ccprintf("0x%08x\n",	LM3_SYSTEM_DCGC2 & LM3_SYSTEM_RCGC2_GPIOE);
