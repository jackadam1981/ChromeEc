...
static int wait_tsk_id; /* To remember which task to wake up */

static unsigned char bus_freq[ISH_I2C_PORT_COUNT] = {
...
void i2c_init_transaction(struct i2c_context *ctx,
			  unsigned char slave_addr, unsigned char flags)
{
	...
	case I2C_SPEED_STD:
		i2c_mmio_write(base, IC_SS_SCL_HCNT,
			       NS_2_COUNTERS(board_config[ctx->bus].std_speed.
					     hcnt, clk_in[bus_freq[ctx->bus]]));
		i2c_mmio_write(base, IC_SS_SCL_LCNT,
			       NS_2_COUNTERS(board_config[ctx->bus].std_speed.
					     lcnt, clk_in[bus_freq[ctx->bus]]));
	...
	case I2C_SPEED_FAST:
		i2c_mmio_write(base, IC_FS_SCL_HCNT,
			       NS_2_COUNTERS(board_config[ctx->bus].fast_speed.
					     hcnt, clk_in[bus_freq[ctx->bus]]));
	...
	case I2C_SPEED_HIGH:
		i2c_mmio_write(base, IC_HS_SCL_HCNT,
			       NS_2_COUNTERS(board_config[ctx->bus].high_speed.
					     hcnt, clk_in[bus_freq[ctx->bus]]));
		  ...
		  uint8_t *in, int in_size, int flags)
{
	...
	total_len = ctx->current_req->extra_w_len + ctx->current_req->w_len
	    + ctx->current_req->r_len;

	i2c_init_transaction(ctx, ctx->current_req->slave_addr,
			     ctx->current_req->flags);
	/* write extra data */
	i2c_write_buffer(ctx->base, ctx->current_req->extra_w_len,
			 ctx->current_req->extra_w_data, &curr_index,
			 total_len);
	/* write W data */
	i2c_write_buffer(ctx->base, ctx->current_req->w_len,
			 ctx->current_req->w_data, &curr_index, total_len);

	if (ctx->current_req->r_len > 0) {
		/* write R commands */
		i2c_write_read_commands(ctx->base, ctx->current_req->r_len);

		/* set rx_theshold */
		i2c_mmio_write(ctx->base, IC_RX_TL,
			       ctx->current_req->r_len - 1);
	}

	/* enable interrupts */
	i2c_intr_switch(ctx->base, mask_arr[ctx->current_req->operation]);
	...
	if (((ctx->interrupts & M_TX_ABRT) == 0)
		...
		for (i = 0; i < ctx->current_req->r_len; i++) {
			ctx->current_req->r_data[i] =
			    i2c_read_byte(ctx->base, IC_DATA_CMD, 0);
		}
...
void i2c_initial_board_config(struct i2c_context *ctx)
{
	uint8_t bus = ctx->bus;
...
void i2c_interrupt_handler(void *param)
{
	...
	/* check interrupts */
	i2c_ctxs[bus_number].interrupts =
	    i2c_mmio_read(i2c_ctxs[bus_number].base, IC_INTR_STAT);
	i2c_ctxs[bus_number].reason =
	    (uint16_t) i2c_mmio_read(i2c_ctxs[bus_number].base,
				     IC_TX_ABRT_SOURCE);
...
void i2c_init_hardware(struct i2c_context *ctx)
{
	uint32_t *base = ctx->base;
