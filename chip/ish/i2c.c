		  ...
		  uint8_t *in, int in_size, int flags)
{
	...
	total_len = 1 + (is_write ? in_size : 1);
	...
	/* Write W data */
	i2c_write_buffer(ctx->base, (is_write ? 0 : out_size - 1),
			 (is_write ? NULL : out + 1),
	...
	i2c_intr_switch(ctx->base,
			is_write ? ENABLE_WRITE_INT : ENABLE_READ_INT);

	/* Wait for interrupt */
	ctx->wait_task_id = task_get_current();
	task_wait_event_mask(EVENT_FLAG_I2C_TIMEOUT, -1);

	if (((ctx->interrupts & M_TX_ABRT) == 0) && (is_write)) {
		/* read data */
		for (i = 0; i < in_size; i++)
