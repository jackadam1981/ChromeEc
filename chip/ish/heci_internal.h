...
struct heci_bus_dma_alloc_notify_req_t {
...
enum HECI_BUS_DMA_MSG_STATUS {
   HECI_DMA_ALLOC_STATUS_OK    = 0,
   HECI_DMA_ALLOC_STATUS_ERROR = 1,
} __packed;

union heci_bus_dma_alloc_response_t {
	struct {
...
struct heci_add_client_req_t {
	...
	uint8_t	client_addr;
	uint8_t	reserved[2];
	heci_cient_properties_t client_properties;
} __packed;
...
struct heci_ipc_bup_t {
	heci_hdr_t hdr;
	heci_bus_msg_t payload;
};

bool heci_send_to_connection(heci_conn_t *connection, const mrd_t *msg);
