 ...
 */
#define RECOVER_FROM_LOW_POWER_US 48  /* 48 usec */
/* delay due to wake timer,timer(2) */
#define WAKE_DELAY_DUE_TO_TIMER_US 24 /* 24 usec */
#define ENOUGH_TIME_FOR_LOW_POWER_US \
	(RECOVER_FROM_LOW_POWER_US + WAKE_DELAY_DUE_TO_TIMER_US)
	...
	while (1) {
		...
		if (DEEP_SLEEP_ALLOWED && max_sleep_time > 0
			...
			if ((sleep_mask & SLEEP_MASK_CONSOLE) &&
				...
				/* CPU & others in halt/low power */
				cpu_enter_idle();

				/*
				 * Now CPU is up by wake timer while other
				 * modules are not active.
				 * Disable interrupt immediately until
				 * 'pm_return_from_idle()" for full system
				 * recovery.
				 */
