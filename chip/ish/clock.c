...
void __idle(void)
{
	...
	while (1) {
		...
		if (DEEP_SLEEP_ALLOWED && max_sleep_time > 0
			...
			/*
			 * TODO: if only d0i1 uses 'hlt' as entry to low power,
			 * move this line to pm_d0i1.
			 */
			cpu_enter_idle(); /* CPU & others in halt/low power */


			/*
			 * Now CPU is up by wake timer while other modules
			 * are not active.
			 * Disable interrupt immediately until
			 * 'pm_return_from_idle()" for full system recovery.
			 */
