...
uint32_t idle_sleep_cnt;
uint32_t idle_deep_sleep_cnt;

uint64_t actual_sleep;
uint64_t total_deep_sleep;
 ...
 */
#define RECOVER_FROM_LOW_POWER (48 * MSEC / 1000)  /* 96 usec, value in counts */
/* delay due to wake timer,timer(2) */
#define WAKE_DELAY_DUE_TO_TIMER (24 * MSEC / 1000) /* 64 usec, value in counts */
...
#ifdef CONFIG_LOW_POWER_IDLE
extern void __hw_clock_wake_set(uint32_t deadline);
extern void __hw_clock_wake_clear(void);
...
void __idle(void)
{
	...
	while (1) {
		...
		early_wakeup -= 1000; //ENOUGH_TIME_FOR_LOW_POWER;

		max_sleep_time = early_wakeup - t0.le.lo;


		if ( DEEP_SLEEP_ALLOWED &&
                     /* enought_time_for_low_power */
                     max_sleep_time > 0 &&
                     allow_low_power != 0 )  {
