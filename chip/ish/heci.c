...
{
	heci_flow_ctrl_t flow_ctrl;

	if (HECI_MAX_NUM_OF_CONNECTIONS <= conn_id) {
		CPRINTS("Bad conn id %d, can't send FC", conn_id);
		return false;
	}

	if (!(HECI_CONN_STATE_OPEN & heci_dev.connections[conn_id].state)) {
		CPRINTS("Conn CLOSED %d", conn_id);
		return false;
	}
	...
	if(heci_dev.connections[conn_id].rx_buffer != NULL) {
		...
		buff->length = 0;
		buff->type = 0;
		buff->msg_lock = UNLOCKED;
		heci_dev.connections[conn_id].rx_buffer = NULL;
	}

	/* build flow control message */
	flow_ctrl.s.command = HECI_BUS_MSG_FLOW_CONTROL;
	flow_ctrl.s.host_addr = heci_dev.connections[conn_id].host_addr ;
	flow_ctrl.s.ish_addr = heci_dev.connections[conn_id].ish_addr;
	flow_ctrl.s.number_of_packets = 1;
	flow_ctrl.s.reserved = 0;

	/* send flow control messages to host */
	if (heci_send_single_fragment(
		HECI_DRIVER_ADDRESS, HECI_DRIVER_ADDRESS,
		true, (uint8_t *)flow_ctrl.dw, sizeof(flow_ctrl))) {
		heci_dev.stat->client[conn_id].send_flow_ctrl++;
		return true;
	}

	heci_dev.stat->client[conn_id].flow_ctrl_failed++;
	CPRINTS( "fail send FC to host\n");
	return false;
}

uint8_t heci_send_fragments(heci_conn_t *connection, const mrd_t *msg)
{
	uint8_t buffer[HECI_IPC_PACKET_SIZE];
	heci_hdr_t *header = (heci_hdr_t *) buffer;
	int written;
	unsigned fragment_size = 0;
	unsigned buffer_offset = 0;
	CPRINTS("heci send IPC fragments. total length = %d", msg->length);

	/* Initialize heci header */
	header->h = 0;
	header->hdr.host_addr = connection->host_addr;
	header->hdr.ish_addr = connection->ish_addr;
	...
	while(msg != NULL) {
		...
		while ((fragment_size < HECI_MAX_PAYLOAD_SIZE) &&
			...
			unsigned copy_size = MIN(msg->length - buffer_offset,
					HECI_MAX_PAYLOAD_SIZE - fragment_size);

			memcpy_s(&buffer[sizeof(*header) + fragment_size],
				copy_size, msg->buffer + buffer_offset,
				copy_size);
			buffer_offset += copy_size;
			fragment_size += copy_size;

			if (buffer_offset == msg->length) {
		...
		if (msg == NULL) {
			/* Last fragment. raise the last_frg flag */
			header->hdr.last_frg = 1;
		}

		header->hdr.length = fragment_size;
		fragment_size += sizeof(*header);
		heci_dev.stat->write++;

		written = ipc_write(heci_dev.ipc_fd, buffer, fragment_size);
		if (written !=  fragment_size) {
			CPRINTS("can't send msg to ipc len = %d err = %d",
	...
	uint32_t length)
{
	...
	if (heci_dev.ipc_fd == -1) {
		/* IPC isn't ready yet (expected at init) */
	        return false;
	}

	header->h =  0;
	header->hdr.host_addr = host_addr;
	header->hdr.ish_addr = ish_addr;
	header->hdr.reserved = 0;
	header->hdr.last_frg = (last_fragment ? 1 : 0);
	/* packet length in bytes w/o the header */
	header->hdr.length = length;

	if ((NULL == data)
		|| (length > (HECI_IPC_PACKET_SIZE - sizeof(heci_hdr_t)))) {
		heci_dev.stat->write_error++;
		CPRINTS("bad frag len %d\n", length);
		return false;
	}
	memcpy_s(&buffer[sizeof(*header)],
		sizeof(buffer) - sizeof(*header),data, length);
	length += sizeof(*header);


	heci_dev.stat->write++;
	written = ipc_write(heci_dev.ipc_fd, buffer, length);
...
void heci_connection_error(uint8_t host_addr, uint8_t ish_addr)
{
	heci_disconn_req_t disconnect_req;
	heci_conn_t *con;
	/* Close the connection and sending disconnect request to host */
	disconnect_req.s.command = HECI_BUS_MSG_CLIENT_DISCONNECT_REQ;
	disconnect_req.s.host_addr = host_addr;
	disconnect_req.s.ish_addr = ish_addr;
	disconnect_req.s.reserved = 0;
	con = heci_find_conn(ish_addr, host_addr,HECI_CONN_STATE_OPEN);
	if (NULL != con) {
		heci_connection_reset(con);
	}
	heci_send_single_fragment(HECI_DRIVER_ADDRESS,
		HECI_DRIVER_ADDRESS, true,
		(uint8_t *) &disconnect_req, sizeof(disconnect_req));
}
	...
	if (packet->payload.dma_req.s.length <= HECI_MAX_DMA_SIZE &&
		...
		if (NULL != connection->client)	{
			if (connection->client->properties.dma_header_length ==
				...
				connection->dma_ts.val = get_time().val; /*os_ticks();*/
...
void heci_process_driver_packet(heci_ipc_bup_t *packet, uint32_t length)
{
	...
	case HECI_BUS_MSG_ADD_CLIENT_RESP:
		...
		break;
#ifdef DMA_XFER_SUPPORTED
	/* host notification for DMA buffer allocation */
	case HECI_BUS_MSG_DMA_ALLOC_NOTIFY:
		heci_dma_alloc_notification_recieved(packet, length);
		break;

	/* DMA transfer to FW */
	case HECI_BUS_MSG_DMA_XFER_REQ:
		heci_dma_xfer_recieved(packet, length);
		break;

	/* Ack for DMA transfer from FW */
	case HECI_BUS_MSG_DMA_XFER_RESP:
...
/* Either ipc_packet or dma_xfer_packet should have message */
...
{
	heci_conn_t *connection;
	heci_rx_msg_t *buffer = NULL;
	uint8_t *data;
	uint8_t ish_addr = 0, host_addr = 0;
	uint8_t dma_copy_required = 0;

	heci_client_ctrl_t *client;

	if (ipc_packet != NULL)	{
		ish_addr = ipc_packet->hdr.hdr.ish_addr;
		host_addr = ipc_packet->hdr.hdr.host_addr;
	}
#ifdef DMA_XFER_SUPPORTED
	else if (dma_xfer_packet != NULL)
	{
		dma_copy_required = 1;
		ish_addr = dma_xfer_packet->s.ish_addr;
		host_addr = dma_xfer_packet->s.host_addr;	
	}
#else
	else {
		CPRINTS("client msg is NULL");
		return;
	}
#endif
	connection = heci_find_conn(ish_addr, host_addr,
		HECI_CONN_STATE_OPEN | HECI_CONN_STATE_PROCESSING_MSG);

	/* Discard the message if an appropriate connection is not found */
	if(connection == NULL) {
		heci_dev.stat->error++;
		CPRINTS("did not find conn %d %d len: %d\n",
			ish_addr, host_addr, length);
		return;
	}

	/* First fragment in one heci message */
	if (!(connection->state & HECI_CONN_STATE_PROCESSING_MSG)) {
		connection->rx_buffer =
			heci_get_buffer_from_pool(connection->client);
		if(connection->rx_buffer == NULL) {
			return;
		}
		connection->state |=  HECI_CONN_STATE_PROCESSING_MSG;
		connection->rx_buffer->length = 0;
	}

	buffer = connection->rx_buffer;
	/* bad packet */
	if (((!dma_copy_required) &&
		(ipc_packet->hdr.hdr.last_frg == 0 &&
		length != (HECI_IPC_PACKET_SIZE - 4))) ||
		((buffer->length + length) > connection->client->properties.rx_buffer_len) ||
		buffer->length + length > connection->client->properties.max_msg_size) {
		heci_dev.stat->error++;
		heci_connection_error (host_addr, ish_addr);
		connection->state &= ~HECI_CONN_STATE_PROCESSING_MSG;
		return;
	}

	data = &connection->client->buffer[buffer->length];

	/* Copy to client buffer */
	if (!dma_copy_required)	{
		memcpy_s(data,
			connection->client->properties.rx_buffer_len-buffer->length,
			ipc_packet->payload.data, length);
	}
#ifdef DMA_XFER_SUPPORTED
	else {
		int ret;
		uint32_t alligned_msg_length;

		alligned_msg_length = (length % DMA_ALIGN_SIZE) ?
			(((length % DMA_ALIGN_SIZE)+1) * DMA_ALIGN_SIZE) : length;

		heci_dev.stat->dma_xfer_read++;
		/* TODO: do we need to pin buffer */
		/* pin_buf((void*)data, alligned_msg_length); */
		ret = dma_copy_kern((uint64_t)(uint32_t)data,
			dma_xfer_packet->s.msg_addr_in_host,
			alligned_msg_length, HOST_DRAM_TO_SRAM);
		unpin_buf((void*)data, alligned_msg_length);

		if(ret != DMA_RC_OK) {
			heci_dev.stat->dma_xfer_read_error++;
			return;
		}
	}
#endif
	...
	/* send msg to client */
	if(dma_copy_required || ipc_packet->hdr.hdr.last_frg) {
		client = connection->client;
		buffer->type = HECI_REQUEST;
		buffer->connection_id = connection->connection_id;
	        heci_dev.stat->client[connection->connection_id].client_req++;
		/* TODO: check if this work fine heci_signal_to_client(
		client, client->properties.new_msg_event); */
		client->properties.heci_msg_cb();
		connection->state &= ~HECI_CONN_STATE_PROCESSING_MSG;
	}
}
...
void heci_rx_task(void)
{
	...
	/* TODO: ipc_open() */
	heci_init();
	ipc_handle = ipc_open(IPC_PEER_HOST_ID,
		IPC_PROTOCOL_HECI, task_get_current());
	if (ipc_handle != -1) {
		heci_dev.ipc_fd = ipc_handle;
	} /*else  TODO: sleep? */

	for (;;) {
		/* task will be blocked here, waiting for event */
		pkt_len = ipc_read(heci_dev.ipc_fd, &heci_msg_frag_from_ipc,
				sizeof(heci_msg_frag_from_ipc));
...
static uint8_t heci_client_find(const heci_client_t *client)
{
	...
	for (i = 0; i <  HECI_MAX_NUM_OF_CLIENTS; i++, client_ctrl++) {
		...
		if(memcmp(&client->protocol_id, &client_ctrl->properties.protocol_id,
...
static void heci_send_new_client_msg(const heci_client_ctrl_t* client)
{
	if (heci_dev.notify_new_clients) {
		heci_add_client_req_t add_client_req;
		memset(&add_client_req, 0, sizeof(heci_add_client_req_t));
		add_client_req.client_properties.protocol_id =
			client->properties.protocol_id;
		add_client_req.client_properties.dma_enabled =
			client->properties.dma_enabled;
		add_client_req.client_properties.dma_header_length =
			client->properties.dma_header_length;
		add_client_req.client_properties.fixed_address =
			client->client_addr;
		add_client_req.client_properties.max_message_length =
			client->properties.max_msg_size;
		add_client_req.client_properties.max_number_of_connections =
			client->properties.max_n_of_connections;
		add_client_req.client_properties.protocol_version =
			client->properties.protocol_ver;
		add_client_req.client_addr = client->client_addr;
		add_client_req.command = HECI_BUS_MSG_ADD_CLIENT_REQ;

		if (!heci_send_single_fragment(HECI_DRIVER_ADDRESS,
				HECI_DRIVER_ADDRESS, true,
				(uint8_t *)&add_client_req,
				sizeof(heci_add_client_req_t)))	{
			CPRINTS( "fail send new client notif to host\n");
		}
	}
}

static void heci_open (void)
{     
	heci_dev.registered_clients++;
	
	if (HECI_WAIT_FOR_CLIENTS_NUM > heci_dev.registered_clients)
	{
		/* Block connecting to host over IPC until
		   all clients(SMHI, HID etc) register */ 
		return;
	} else if (HECI_MAX_NUM_OF_CLIENTS < heci_dev.registered_clients) {
		CPRINTS("too many heci client registered.\n");
		return;
	}

#if 0 /* TODO: need to modify this */
   	while (heci_dev.client_ready == 0) {
		CPRINTS("wait for heci_rx_task start\n");
		/*task_sleep (1);*/
		usleep(1);/*TODO: is this ok*/
	}

	Give_EVENT(heci_dev.client_ready);
#endif
}

/* 
 * Assumption: HECI task priority > heci_client priority,
 * so heci_register will not be called before heci_task() finishes.
 */
HECI_REGISTER_STATUS heci_register (const heci_client_t  *client)
{
	heci_client_ctrl_t *client_ctrl = heci_dev.clients;
	HECI_REGISTER_STATUS status = HECI_REGISTER_OUT_OF_RESOURCES;

	int i;
	heci_conn_t *connection;

	if ((NULL == client) || (client->heci_msg_cb == NULL) ||
			(client->heci_disconn_cb == NULL) ||
			(client->rx_buffers_pool == NULL) ||
			(client->rx_buffers_pool->buffer == NULL)) {

		CPRINTS("bad params, can't register client\n");
		return HECI_REGISTER_INTERNAL_ERROR;
	}

	if (client->max_msg_size > HECI_MAX_MSG_SIZE) {
		CPRINTS("client msg size couldn't be larger than 4K\n");
		return HECI_REGISTER_INTERNAL_ERROR;
	}

	if ((uint32_t)client->rx_buffers_pool->buffer & ALIGN_MASK_64) {
		CPRINTS("client rx address must be alligend to 64\n");
		return HECI_REGISTER_INTERNAL_ERROR;
	}

	/* TODO: can we have timeout */
	mutex_lock(&heci_dev.lock);
	
	/* Check if can find the client on the list. */
	if (heci_client_find(client)) {
		/* duplicate registration. */
		CPRINTS("client already registered");
		status = HECI_REGISTER_ALREADY_REGISTERED;
	} else {
		/* Try to find a free conn */
		for (connection = heci_dev.connections, i = 0 ;
			i < HECI_MAX_NUM_OF_CONNECTIONS; connection++ , i++) {

			if (connection->state == HECI_CONN_STATE_UNUSED) {
				break;
			}
		}

		if (i >= HECI_MAX_NUM_OF_CONNECTIONS)
			goto end;
		
		for (i = 0; i <  HECI_MAX_NUM_OF_CLIENTS; i++, client_ctrl++) {
			/* If found then return success. */
			if (client_ctrl->client_addr == 0) {
				/* Copy all information to properties */
				client_ctrl->properties = *client;
				client_ctrl->buffer =
					client->rx_buffers_pool->buffer;
				memset(client_ctrl->buffer, 0,
					client->max_n_of_connections * client->rx_buffer_len);
				client_ctrl->client_addr = (uint16_t) (i + 1);
				client_ctrl->n_of_conns = 0;
				client_ctrl->active = false;
				heci_send_new_client_msg(client_ctrl);
				status = HECI_REGISTER_SUCCEED;
				heci_dev.stat->client_register++;
				heci_open();
				goto end;
			}
