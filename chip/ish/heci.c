...
{
	uint8_t cli_idx = fw_addr - HECI_DYN_CLIENT_ADDR_START;
			...
			struct hbm_client_connect_req *client_connect_req)
{
	...
	heci_send_heci_msg(&heci_msg);
	if (!client_connect_res->status) /* no error, send flow control */
...
{
	if (!heci_msg->hdr.host_addr) { /* Msg for HECI Bus or Fixed client */
		/*
