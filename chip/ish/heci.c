...
#define HECI_IS_VALID_HANDLE(handle) \
		HECI_IS_VALID_CLIENT_ADDR((uintptr_t)(handle))
...
int heci_set_client_data(const heci_handle_t handle, void *data)
{
	...
	cli_ctx = HECI_CLIENT_CONTEXT(TO_FW_ADDR(handle));
	cli_ctx->data = data;
		  ...
		  const size_t buf_size)
{
	...
	connect = HECI_CLIENT_CONNECT(TO_FW_ADDR(handle));
	mutex_lock(&connect->lock);

	if (!HECI_CLIENT_IS_CONNECTED(TO_FW_ADDR(handle))) {
	...
	msg.hdr.fw_addr = TO_FW_ADDR(handle);
		   ...
		   const struct heci_msg_list *msg_list)
{
	...
	int total_size = 0;
	int i, msg_consumed, buf_size, copy_size;
	struct heci_client_connect *connect;
	struct heci_msg msg;
	...
	while (1) {
		...
		/* fill ipc msg buffer */
		if (msg_item->size - msg_consumed >
		    HECI_IPC_PAYLOAD_SIZE - buf_size) {
			copy_size = HECI_IPC_PAYLOAD_SIZE - buf_size;
		} else {
			copy_size = msg_item->size - msg_consumed;
		}

		memcpy(msg.payload + buf_size, msg_item->buf + msg_consumed,
		       copy_size);

		msg_consumed += copy_size;
		buf_size += copy_size;
	}
...
void heci_rx_task(void)
{
	 ...
	 * the task will read msg with blocking read so no need to send event
	 */
