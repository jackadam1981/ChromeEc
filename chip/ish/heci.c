	...
	uint32_t length)
{
...
#endif
	connection = heci_find_conn (ish_addr,  host_addr, HECI_CONN_STATE_OPEN | HECI_CONN_STATE_PROCESSING_MSG);

	// Discard the message if an appropriate connection is not found
	if(connection == NULL)
	{
		heci_dev.stat->error++;
		HECI_LOG_ERR("did not find conn %d %d len: %d\n", (int)ish_addr,  (int)host_addr, (int)length);
		return;
	}


	if(!(connection->state & HECI_CONN_STATE_PROCESSING_MSG)) // if it's the first fragment
	{
		connection->rx_buffer = heci_get_buffer_from_pool(connection->client);
		if(connection->rx_buffer == NULL)
		{
			DBG_ASSERT(0);
			return;
	...
	// bad packet
	if( ((!dma_copy_required) && (ipc_packet->hdr.hdr.last_frg == 0 && length != (HECI_IPC_PACKET_SIZE - 4))) ||
	...
	{
		memcpy_s(data, connection->client->properties.rx_buffer_len-buffer->length, ipc_packet->payload.data, length);
	}
	else
	{
		uint32_t alligned_msg_length = (length % DMA_ALIGN_SIZE)? (((length % DMA_ALIGN_SIZE)+1)*DMA_ALIGN_SIZE) : length;
		heci_dev.stat->dma_xfer_read++;
		pin_buf((void*)data, alligned_msg_length);
		int ret = dma_copy_kern((uint64_t)(uint32_t)data, dma_xfer_packet->s.msg_addr_in_host,  alligned_msg_length , HOST_DRAM_TO_SRAM );
	...
	if(dma_copy_required || ipc_packet->hdr.hdr.last_frg)
	{
		...
		heci_signal_to_client (client, client->properties.new_msg_event);
		connection->state &= ~HECI_CONN_STATE_PROCESSING_MSG;
	}
#endif
