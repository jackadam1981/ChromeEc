...
#define TO_FW_ADDR(handle)		((uint8_t)(uint32_t)handle)
#define TO_HECI_HANDLE(fw_addr)		((heci_handle_t)(uint32_t)fw_addr)
...
int heci_set_client_data(heci_handle_t handle, void *data)
{
	struct heci_client_context *cli_ctx;

	if (!HECI_IS_VALID_CLIENT_ADDR(TO_FW_ADDR(handle)))
...
void *heci_get_client_data(heci_handle_t handle)
{
	struct heci_client_context *cli_ctx;
...
int heci_send_msg(heci_handle_t handle, uint8_t *buf, size_t buf_size)
{
...
int heci_send_msgs(heci_handle_t handle, struct heci_msg_list *msg_list)
{
