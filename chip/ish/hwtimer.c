...
{
	const register uint64_t a = HPET_MAIN_COUNTER_64;
	const register uint64_t b = HPET_MAIN_COUNTER_64;
	return (a >> 32 == b >> 32) ? a : HPET_MAIN_COUNTER_64;
}
#endif
...
{
#if defined(CHIP_FAMILY_ISH3)
	const register uint64_t tmp = read_main_timer();
