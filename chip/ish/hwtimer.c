...
/* main counter value */
/* return in usec unit */
uint64_t __hw_clock_main_count_get(void)
{
    uint32_t high_dword;
    uint32_t low_dword;
    ...
    do
    {
        high_dword = HPET_MAIN_COUNTER_HIGH;
        low_dword = HPET_MAIN_COUNTER;
    }
    while (high_dword != HPET_MAIN_COUNTER_HIGH);

    /* TODO:
     * Following should be fixed here.
     * if ISH3, it is (64bit counter value * 1MHz / 12MHz) usec (12MHz)
     * if ISH4, it is (64bit counter value * 1MHz / 32.768KHz) sec (32KHz)
     *
     * need to conert to 1 usec in user program.
     */
    return ((uint64_t)high_dword << 32) | low_dword;
}
#endif
...
{
	/* Do nothing, just generate interrupt to wake CPU up from HLT */
	/* Clear interrupt */
	HPET_INTR_CLEAR = (1 << 2);
