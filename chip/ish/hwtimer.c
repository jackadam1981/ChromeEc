...
#if defined(CHIP_FAMILY_ISH3)
	/*
	 * In ~6 mins (0xFFFFFFFF/0xC=357s), there is overflow,
	 * we don't worry about this because system is not 
	 * expected to idle this long. 
	 */
	HPET_TIMER_COMP(2) = deadline * CLOCK_FACTOR;
#else
...
static void __hw_clock_source_irq(int timer_id)
{
	 ...
	 * timer 2 is for idle wake up only, should not be processed
	 * by OS.
	 */
	if (timer_id < 2)
