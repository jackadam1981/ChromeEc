...
static int dma_init_called = 0; /* If ish_dma_init is called */
...
{
	uint32_t ctrl = REG32(OCP_AGENT_CONTROL);
	REG32(OCP_AGENT_CONTROL) = ctrl & OCP_RESPONSE_TO_DISABLE;
}

void dma_configure_psize(void)
{
	/* Give chan0 512 bytes for high performance, and chan1 128 bytes. */
	REG32(DMA_PSIZE_01) = DMA_PSIZE_UPDATE |
...
void ish_dma_init(void)
{
	...
	uma_msb = REG32(IPC_UMA_RANGE_LOWER_1);
		 ...
		 dma_mode_t mode)
{
	...
	REG32(DMA_CFG_REG) = DMA_EN_MASK; /* Enable DMA module */
	REG32(chan_reg + DMA_LLP) = 0;    /* Linked lists are not used */
	REG32(chan_reg + DMA_CTL) =
	...
	while (length) {
		...
		interrupt_disable();
		MISC_CHID_CFG_REG = chan; /* Set channel to configure */
		REG32(chan_reg + DMA_CTL + 0x4) =
			chunk; /* Set number of bytes to transfer */
		REG32(chan_reg + DMA_DAR) = dst; /* Destination address */
		REG32(chan_reg + DMA_SAR) = src; /* Source address */
		REG32(DMA_EN_REG) =
			DMA_CH_EN_BIT(chan) |
...
void ish_dma_disable(void)
{
	...
	for (channel = 0; channel <= DMA_MAX_CHANNEL; channel++) {
		...
		if (REG32(DMA_EN_REG) & DMA_CH_EN_BIT(channel)) {
			/* Write 0 to channel enable bit ... */
			REG32(DMA_EN_REG) = DMA_CH_EN_WE_BIT(channel);

			/* Wait till it shuts up. */
			counter = 0;
			while ((REG32(DMA_EN_REG) & DMA_CH_EN_BIT(channel)) &&
			       counter < (UINT32_MAX / 64))
				counter++;
		}
	}
	REG32(DMA_CLR_ERR_REG) = 0xFFFFFFFF;
	REG32(DMA_CLR_BLOCK_REG) = 0xFFFFFFFF;

	REG32(DMA_CFG_REG) = 0; /* Disable DMA module */
}

int ish_wait_for_dma_done(uint32_t ch)
{
	return dma_poll(DMA_EN_REG, 0, DMA_CH_EN_BIT(ch));
}
