...
def gen_manifest(ext_id, comp_app_name, code_offset, module_size):
    ...
    mmm = bytearray(MANIFEST_ENTRY_SIZE)

    # 4 bytes of ASCII encode ID (little endian)
    struct.pack_into("<4s", mmm, 0, ext_id)
    # 8 bytes of ASCII encode ID (little endian)
    struct.pack_into("<8s", mmm, 32, comp_app_name)
    # 4 bytes of code offset (little endian)
    struct.pack_into("<I", mmm, 96, code_offset)
    # 2 bytes of module in page size increments (little endian)
    struct.pack_into("<H", mmm, 100, module_size)
...
def main():
    ...
    with open(args.output, "wb") as fff:
        ...
        fff.write(gen_manifest(b"ISHE", b"", 0, 0))

        # Pad the remaining HEADER with 0s
        if args.aon is not None:
            fff.write(b"\x00" * (HEADER_SIZE - (MANIFEST_ENTRY_SIZE * 3)))
        else:
            fff.write(b"\x00" * (HEADER_SIZE - (MANIFEST_ENTRY_SIZE * 2)))

        # Append original kernel image
        with open(args.kernel, "rb") as in_file:
            fff.write(in_file.read())
        # Filling padings due to size round up as pages
        fff.write(b"\x00" * (kern_rdup_pg_size * PAGE_SIZE - args.kernel_size))
        ...
        if args.aon is not None:
            ...
            with open(args.aon, "rb") as in_file:
                fff.write(in_file.read())
            # Filling padings due to size round up as pages
            fff.write(b"\x00" * (aon_rdup_pg_size * PAGE_SIZE - args.aon_size))
