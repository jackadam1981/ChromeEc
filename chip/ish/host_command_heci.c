					...
					uint8_t *msg, const size_t msg_size)
{
	...
	if (msg_size < sizeof(struct cros_ec_ishtp_in_msg)) {
		CPRINTS("ERROR: Size of packet is: %d", msg_size);	
	}
	...
	/* Need to know size here; what is going on here? */
...
/*
 * IPC packet size max is actual 4K, but we don't need kernel buffers that big
 */
#define HECI_CROS_EC_MAX_PACKET_SIZE 512
/**
...
static int heci_get_protocol_info(struct host_cmd_handler_args *args)
{
	...
	r->max_request_packet_size = HECI_CROS_EC_MAX_PACKET_SIZE;
	/* The kernel does not use the response dout buffer, so don't allocation
	 * a big one */
