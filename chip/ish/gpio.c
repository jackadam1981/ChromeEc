...
{
	if (value)
		ISH_GPIO_GPSR |= gpio_list[signal].mask;
	else
		ISH_GPIO_GPCR |= gpio_list[signal].mask;
}
...
{
	/* Return if no pin is masked or masked pin is not supported */
	if ((gpio_list[signal].mask == 0) ||
		gpio_list[signal].mask >= (1 << ISH_TOTAL_GPIO_PINS))
		return EC_SUCCESS;

	ISH_GPIO_GIMR |= gpio_list[signal].mask;
...
int gpio_clear_pending_interrupt(enum gpio_signal signal)
{
	...
	ISH_GPIO_GISR |= gpio_list[signal].mask;
	return EC_SUCCESS;
}
