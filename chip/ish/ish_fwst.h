...
{
	return REG32(IPC_ISH_HOST_FWSTS);
}

static inline void ish_fwst_set_ilup(void)
{
	REG32(IPC_ISH_HOST_FWSTS) |= (1<<IPC_ISH_FWSTS_ILUP_SHIFT);
}

static inline void ish_fwst_clear_ilup(void)
{
	REG32(IPC_ISH_HOST_FWSTS) &= ~IPC_ISH_FWSTS_ILUP_MASK;
}

static inline int ish_fwst_is_ilup_set(void)
{
	return !!(REG32(IPC_ISH_HOST_FWSTS) &= IPC_ISH_FWSTS_ILUP_MASK);
}

static inline void ish_fwst_set_hup(void)
{
	REG32(IPC_ISH_HOST_FWSTS) |= (1<<IPC_ISH_FWSTS_HUP_SHIFT);
}

static inline void ish_fwst_clear_hup(void)
{
	REG32(IPC_ISH_HOST_FWSTS) &= ~IPC_ISH_FWSTS_HUP_MASK;
}

static inline int ish_fwst_is_hup_set(void)
{
	return !!(REG32(IPC_ISH_HOST_FWSTS) &= IPC_ISH_FWSTS_HUP_MASK);
}

static inline void ish_fwst_set_fail_reason(uint32_t val)
{
	uint32_t fwst = REG32(IPC_ISH_HOST_FWSTS);

	REG32(IPC_ISH_HOST_FWSTS) = (fwst & ~IPC_ISH_FWSTS_FAIL_REASON_MASK) |
				    (val << IPC_ISH_FWSTS_FAIL_REASON_SHIFT);
}

static inline uint32_t ish_fwst_get_fail_reason(void)
{
	return (REG32(IPC_ISH_HOST_FWSTS) & IPC_ISH_FWSTS_FAIL_REASON_MASK)
...
static inline void ish_fwst_set_reset_id(uint32_t val)
{
	...
	REG32(IPC_ISH_HOST_FWSTS) = (fwst & ~IPC_ISH_FWSTS_RESET_ID_MASK) |
				    (val << IPC_ISH_FWSTS_RESET_ID_SHIFT);
}

static inline uint32_t ish_fwst_get_reset_id(void)
{
	return (REG32(IPC_ISH_HOST_FWSTS) & IPC_ISH_FWSTS_RESET_ID_MASK)
...
static inline void ish_fwst_set_fw_state(uint32_t val)
{
	...
	REG32(IPC_ISH_HOST_FWSTS) = (fwst & ~IPC_ISH_FWSTS_FW_STATE_MASK) |
				    (val << IPC_ISH_FWSTS_FW_STATE_SHIFT);
}

static inline uint32_t ish_fwst_get_fw_state(void)
{
	return (REG32(IPC_ISH_HOST_FWSTS) & IPC_ISH_FWSTS_FW_STATE_MASK)
