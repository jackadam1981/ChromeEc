...
#define handle_to_dev_id(_handle)		(_handle)
#define dev_id_to_handle(_dev_id)		((int)(_dev_id))

static inline int device_index_to_handle(int device_index)
{
	return device_index + 1;
}

static inline int is_valid_handle(int handle)
{
	return handle > 0 && handle <= hid_subsys_ctx.num_of_hid_devices;
}

static inline
struct hid_subsys_hid_device *handle_to_hid_device(int handle)
{
	if (!is_valid_handle(handle))
		return NULL;

	return &hid_subsys_ctx.hid_devices[handle - 1];
}
				 ...
				 const size_t buf_size)
{
	...
	hid_device = handle_to_hid_device(hid_handle);
	if (!hid_device)
...
static int handle_hid_device_msg(struct hid_msg *hid_msg)
{
	...
	hid_handle = dev_id_to_handle(hid_msg->hdr.device_id);
	hid_dev = handle_to_hid_device(hid_handle);

	ASSERT(hid_dev != NULL); /* TODO: need to check interface with host */
	...
	case  HID_GET_HID_DESCRIPTOR:
		if (cbs->get_hid_descriptor)
			ret = cbs->get_hid_descriptor(hid_handle, payload,
						      buf_size);

		break;
	case HID_GET_REPORT_DESCRIPTOR:
		if (cbs->get_report_descriptor)
			ret = cbs->get_report_descriptor(hid_handle, payload,
							 buf_size);
	...
	case HID_GET_FEATURE_REPORT:
		if (cbs->get_feature_report)
			ret = cbs->get_feature_report(hid_handle, payload[0],
						      payload, buf_size);
	...
	case HID_SET_FEATURE_REPORT:
		if (cbs->set_feature_report) {
			ret = cbs->set_feature_report(hid_handle,
						      payload[0],
						      payload,
						      payload_size);
	...
	case HID_GET_INPUT_REPORT:
		if (cbs->get_input_report)
			ret = cbs->get_input_report(hid_handle, payload[0],
						    payload, buf_size);
...
static int hid_subsys_initialize(const heci_handle_t handle)
{
	hid_subsys_ctx.heci_handle = handle;

	return 0;
}

static int hid_subsys_resume(const heci_handle_t handle)
{
	int i, ret = 0;
...
static int hid_subsys_suspend(const heci_handle_t handle)
{
	int i, ret = 0;
