 ...
 * handle the orignal interrupt normally.
 *
...
static void pmu_wakeup_isr(void)
{
	 ...
	 */
	*(volatile uint32_t *)IOAPIC_EOI_REG = ISH_PMU_WAKEUP_VEC;
...
void ish_aon_main(void)
{
	...
	aon_idt[AON_IDT_ENTRY_VEC_LAST - AON_IDT_ENTRY_VEC_FIRST].dword_lo =
			GEN_IDT_DESC_LO(&reset_prep_isr, 0x4, IDT_DESC_FLAG);

	aon_idt[AON_IDT_ENTRY_VEC_LAST - AON_IDT_ENTRY_VEC_FIRST].dword_hi =
			GEN_IDT_DESC_HI(&reset_prep_isr, 0x4, IDT_DESC_FLAG);
#endif
