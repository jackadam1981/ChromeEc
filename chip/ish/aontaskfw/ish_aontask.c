...
/**
 * 8 byte reserved on stack, not a bug, just for GDB to show correct stack
...
static struct ldt_entry aon_ldt[2] = {
	 ...
	 */
	{
		.dword_hi = GEN_GDT_DESC_HI(0x0, 0xFFFFFFFF, 0x9B),
		.dword_lo = GEN_GDT_DESC_LO(0x0, 0xFFFFFFFF, 0x9B)
	},
	 ...
	 */
	{
		.dword_hi = GEN_GDT_DESC_HI(0x0, 0xFFFFFFFF, 0x93),
		.dword_lo = GEN_GDT_DESC_LO(0x0, 0xFFFFFFFF, 0x93)
	}
};

/* shared data structure between main FW and aon task */
struct ish_aon_share aon_share = {
	.aon_tss = (struct tss_entry *)((uint32_t)&aon_tss),
