...
#define HID_SUBSYS_ERR_INIT_FAIL			-5

struct hid_callbacks {
	/* function called during registration */
	int (*initialize)(const int hid_handle);

	/* return size of data copied to buf */
	int (*get_hid_descriptor)(const int hid_handle, uint8_t *buf,
				  const size_t buf_size);
	/* return size of data copied  to buf */
	int (*get_report_descriptor)(const int hid_handle, uint8_t *buf,
				     const size_t buf_size);
	/* return size of data copied to buf */
	int (*get_feature_report)(const int hid_handle, const uint8_t report_id,
				  uint8_t *buf, const size_t buf_size);
	/* return tranferred data size */
	int (*set_feature_report)(const int hid_handle, const uint8_t report_id,
				  const uint8_t *data, const size_t data_size);
	/* return size of data copied to buf */
	int (*get_input_report)(const int hid_handle, const uint8_t report_id,
				uint8_t *buf, const size_t buf_size);

	/* suspend/resume */
	int (*resume)(const int hid_handle);
	int (*suspend)(const int hid_handle);
};
 ...
 */
int hid_subsys_register_device(const struct hid_device *dev_info);
/* send HID input report */
int hid_subsys_send_input_report(const int hid_handle, uint8_t *buf,
				 const size_t buf_size);
/* store HID device specific data */
int hid_subsys_set_device_data(const int hid_handle, void *data);
/* retrieve HID device specific data */
void *hid_subsys_get_device_data(const int hid_handle);
