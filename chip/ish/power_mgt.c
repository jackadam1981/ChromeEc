...
void __idle(void)
{
	...
	/**
	 * ish_dma_init() enabled interrupt ('sti' called) in latest patch set,
	 * calling ish_dma_init() in ish_pm_init() before task_start() is not
	 * safe.
	 * TODO this DMA driver's bug, will request ish_dma_init() fix it
	 * */
	ish_dma_init();
