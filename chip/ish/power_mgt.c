...
{
    if (argc == 1) {
        ccprintf("Usage: enterpmstate d0/d0i0/d0i1/d0i2/d0i3/np. \n\
        Need to reboot to exit the state. np: no parking, \n\
        disable the parking, only work when in D0.\n");
        ...
        s_bD0i2=false;
        s_bD0i3=true;
    } else if (!strncmp(argv[1], "np", strlen(argv[1]))) {
        s_bpowermeas=false;
        s_bD0=false;
        s_bD0i0=false;
        s_bD0i1=false;
        ...
        ccprintf("Wrong command. Usage: enterpmstate d0/d0i0/d0i1/d0i2/d0i3/np. \n\
        Need to reboot to exit the state. np: no parking, \n\
        disable the parking, only work when in D0.\n");
    }
    return EC_SUCCESS;
...
DECLARE_CONSOLE_COMMAND(enterpmstate, command_enter_pmstate, "",
			"Usage: enterpmstate d0/d0i0/d0i1/d0i2/d0i3/np. \n\
        Need to reboot to exit the state. np: no parking, \n\
        disable the parking, only work when in D0.\n");
