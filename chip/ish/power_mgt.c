 ...
 */
static int print_pm_stats(void)
{
#if defined(CONFIG_ISH_PM_D0I2) || defined(CONFIG_ISH_PM_D0I3)
	struct ish_aon_share *aon_share = pm_ctx.aon_share;
#endif
...
static inline void set_dsleep(int enable)
{
	if (enable) {
...
DECLARE_CONSOLE_COMMAND(dsleep, command_dsleep,
			"[on | off | stats]",
			"Deep sleep settings:"
			"\nUse 'on' to enable deep sleep."
			"\nUse 'off' to disable deep sleep."
			"\nUse 'stats' or no parameter to print deep sleep"
			"status and statistic data.");
...
DECLARE_HOST_COMMAND(EC_CMD_DSLEEP,
		     host_cmd_dsleep,
