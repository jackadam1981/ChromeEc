...
#if defined(CONFIG_ISH_PM_D0I1) || defined(CONFIG_ISH_PM_AONTASK)
/**
 * ISH PMU not support both edge interrupt triggered gpio configuration,
 * otherwise ISH can't enter low poer mode, will exit immediately.
 * so, before enter low poer mode, we need scan all gpio pins which have been
 * configured as both edge triggered mode, and reset it to needed single edge
 * triggered mode, and restore them after exit low power mode
 */
static uint32_t covert_both_edge_gpio_to_single_edge(void)
{
	...
	for (i = 0; i < 32; i++) {
		if (ISH_GPIO_GIMR & BIT(i) &&
			...
			/* convert to needed single edge trigger mode */
			pin_level = !!(ISH_GPIO_GPLR & BIT(i));
