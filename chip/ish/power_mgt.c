 ...
 * @param desc_hi	high DWORD of the entry descriptor
 *
...
static uint32_t add_gdt_entry(uint32_t desc_lo, uint32_t desc_hi)
{
	...
	*limit += sizeof(struct gdt_entry);

	return *limit - sizeof(struct gdt_entry);
}
...
static void init_aon_task(void)
{
	...
	desc_lo = GEN_GDT_DESC_LO((uint32_t)&main_tss,
			GDT_DESC_TSS_LIMIT, GDT_DESC_TSS_FLAG);
	desc_hi = GEN_GDT_DESC_HI((uint32_t)&main_tss,
			GDT_DESC_TSS_LIMIT, GDT_DESC_TSS_FLAG);
	...
	desc_lo = GEN_GDT_DESC_LO((uint32_t)aon_tss,
			GDT_DESC_TSS_LIMIT, GDT_DESC_TSS_FLAG);
	desc_hi = GEN_GDT_DESC_HI((uint32_t)aon_tss,
			GDT_DESC_TSS_LIMIT, GDT_DESC_TSS_FLAG);
	...
	desc_lo = GEN_GDT_DESC_LO((uint32_t)aon_share->ldt_ptr,
				  aon_share->ldt_size, GDT_DESC_LDT_FLAG);
