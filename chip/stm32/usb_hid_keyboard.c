...
/*
 * Coallesce events happening within some interval. The value must be greater
 ...
 * is passed to AP, and that we do not coallesce press and release events (which
 * would result in lost keys).
 */
#define COALLESCE_INTERVAL (18 * MSEC)
...
static void keyboard_process_queue(void)
{
	...
	/*
	 * Pick key events from the queue, coallescing events older than events
	...
	while (queue_count(&key_queue) > 0) {
		...
		if ((now - ev.time) <= KEY_DISCARD_MAX_TIME &&
		    (ev.time - first_key_time) >= COALLESCE_INTERVAL)
			break;
