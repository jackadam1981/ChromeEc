...
/*
 * Allow to coallesce events happening within x ms, must be greater
 * than EP interval to ensure we cannot have a backlog of keys.
 * It must also be short enough to ensure that the intended order
 * of key presses is passed to AP, and that we do not coallesce
 * press and release events (which would result in lost keys).
 */
#define COALLESCE_INTERVAL (18*MSEC) /* ms */

/* Discard key events in the FIFO buffer that are older than x ms. */
#define KEY_DISCARD_MAX_TIME (1000*MSEC) /* ms */
...
static void keyboard_process_queue(void)
{
	...
	/* FIXME: Trim old events from queue here. */

	if (queue_is_full(&key_queue)) {
		/* FIXME: If the queue is full (but USB is still suspended, we
		 * need to process the events,
		 * update the matrix state, but not write the descriptor.
		 */
		CPRINTF("Queue if full (%d %d %d)\n", queue_count(&key_queue),
			usb_is_suspended(), hid_ep_data_ready);
	}
	...
	/*
	 * ev.time is garbage if queue is empty, but that's ok
	...
	while (queue_count(&key_queue) > 0) {
		...
		if ((ev.time - first_key_time) >= COALLESCE_INTERVAL)
			break;
		...
		if (ev.keycode == HID_KEYBOARD_NEW_KEY) {
			...
			/*
			 * FIXME: This can potentially add duplicated keys
			 * if release events are dropped.
			 */
