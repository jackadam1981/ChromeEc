...
/* Define summary flags for write-protection at boot */
#define RO_AT_BOOT (EC_FLASH_PROTECT_ALL_AT_BOOT | EC_FLASH_PROTECT_RO_AT_BOOT)
#ifdef CONFIG_ROLLBACK
#define RB_AT_BOOT (EC_FLASH_PROTECT_ALL_AT_BOOT | \
		    EC_FLASH_PROTECT_ROLLBACK_AT_BOOT)
#else /* !CONFIG_ROLLBACK */
#define RB_AT_BOOT EC_FLASH_PROTECT_ALL_AT_BOOT
#endif /* !CONFIG_ROLLBACK */
#ifdef CONFIG_FLASH_PROTECT_RW
#define RW_AT_BOOT (EC_FLASH_PROTECT_ALL_AT_BOOT | \
		    EC_FLASH_PROTECT_RW_AT_BOOT)
#else /* !CONFIG_FLASH_PROTECT_RW */
#define RW_AT_BOOT EC_FLASH_PROTECT_ALL_AT_BOOT
...
int flash_physical_protect_at_boot(uint32_t new_flags)
{
	 ...
	 * rollback and RW regions (and assume that by definition, we cannot
	 * have RB unprotected and RW protected).
	 */
	if (new_flags & RO_AT_BOOT)
		ro_range = FLASH_WRP_RANGE(WP_BANK_OFFSET,
					   WP_BANK_OFFSET + WP_BANK_COUNT);

	if (new_flags & (RB_AT_BOOT | RW_AT_BOOT)) {
#ifdef CONFIG_ROLLBACK
		int strt = ROLLBACK_BANK_OFFSET;
		int end = new_flags & RW_AT_BOOT ? 0xFF
				: ROLLBACK_BANK_OFFSET + ROLLBACK_BANK_COUNT;
#else
		int strt = WP_BANK_OFFSET + WP_BANK_COUNT;
		int end = 0xFF;
#endif
...
#endif /* CONFIG_FLASH_PROTECT_RW */
		if (end == 0xff && flags & EC_FLASH_PROTECT_RO_AT_BOOT)
