...
static int count_keys_pressed(const uint8_t *state)
{
	int i, n = 0;

	for (i = 0; i < KB_OUTPUTS; i++) {
		uint8_t u = state[i];
...
{
	select_column(COL_ASSERT_ALL);
	usleep(config.output_settle_us);
	/* FIXME(dhendrix): make this less board-specific */
...
{
	int keys_changed = 0, num_keys_pressed = 0;
	int wait_time = 0;
	...
	while (1) {
		...
		if (num_keys_pressed == 0) {
			...
			/*
			 * Check inputs once again to reduce possibility of
			 * missing a keypress. Key releases should not be
			 * a problem since num_keys_pressed will remain >0
			 * until the release is detected by the scan loop.
			 */
			if (!kb_input_asserted()) {
				task_wait_event(-1);
				wait_time = 0;	/* scan immediately */
			}
		...
		/*
		 * Scan matrix until keys are no longer pressed. If a change
		 * is detected, break so that the host can be notified. The
		 * loop will subsequently be re-entered until the number of
		 * keys pressed in the debounced state is zero.
		 */
		do {
			...
			num_keys_pressed = count_keys_pressed(debounced_state);
		...
		} while ((num_keys_pressed > 0) && (keys_changed == 0));
	}
}
