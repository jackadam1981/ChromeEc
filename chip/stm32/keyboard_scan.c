...
static uint8_t debounced_state[KB_INPUTS];   /* Debounced key matrix */
static uint8_t prev_state[KB_INPUTS];        /* Matrix from previous scan */
static uint8_t debouncing[KB_INPUTS];        /* Mask of keys being debounced */
...
static uint8_t scan_edge_index[KB_INPUTS][8];
 ...
 *
 * @param state		Destination for new state (must be KB_INPUTS long).
 *
