...
typedef struct
{
	 ...
	 */
	usb_stream_state volatile * state;
 ...
 */
extern in_stream_ops const  usb_stream_in_stream_ops;
extern out_stream_ops const usb_stream_out_stream_ops;
...
#define USB_STREAM_CONFIG(NAME,						\
	...
	static usb_uint NAME##_ep_rx_buffer[USB_MAX_PACKET_SIZE / 2] __usb_ram; \
	static usb_uint NAME##_ep_tx_buffer[USB_MAX_PACKET_SIZE / 2] __usb_ram; \
	static usb_stream_state NAME##_state;				\
	usb_stream_config const NAME =					\
	{								\
		.state  = &NAME##_state,				\
		.rx_ep  = RX_ENDPOINT,					\
		.tx_ep  = TX_ENDPOINT,					\
		.rx_ram = NAME##_ep_rx_buffer,				\
		.tx_ram = NAME##_ep_tx_buffer,				\
		.rx = {							\
	...
	static void NAME##_ep_tx      (void) { usb_stream_tx      (&NAME); } \
	static void NAME##_ep_tx_reset(void) { usb_stream_tx_reset(&NAME); } \
	static void NAME##_ep_rx      (void) { usb_stream_rx      (&NAME); } \
	static void NAME##_ep_rx_reset(void) { usb_stream_rx_reset(&NAME); } \
 ...
 */
void usb_stream_tx(usb_stream_config const * config);
void usb_stream_rx(usb_stream_config const * config);
void usb_stream_tx_reset(usb_stream_config const * config);
void usb_stream_rx_reset(usb_stream_config const * config);
