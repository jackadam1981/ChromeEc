...
#define USB_STREAM_CONFIG(NAME,						\
	...
	{								\
		usb_stream_tx_reset(&NAME);				\
	}								\
	static void CONCAT2(NAME, _ep_rx_reset)(void)			\
	{								\
		usb_stream_rx_reset(&NAME);				\
	}								\
...
void usb_stream_tx_reset(struct usb_stream_config const *config);
void usb_stream_rx_reset(struct usb_stream_config const *config);
