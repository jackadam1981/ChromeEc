...
#define STM32G4_PLL_IN_FREQ_HZ 4000000
...
static int stm32g4_config_pll(uint32_t hclk_hz, uint32_t pll_src,
			      uint32_t pll_clk_in_hz)
{
	 ...
	 *
	 * TODO -> for debug have MCO set to sysclk/4
	 */
	STM32_RCC_CFGR = 0x11000000 | STM32_RCC_CFGR_SW_PLL;
...
static void stm32g4_config_rtc_clock(void)
{
...
void clock_init(void)
{
	 ...
	 *   1. HSE -> external cyrstal osciallator circuit
	 ...
	 *   1. LSE -> external crystal osciallator (max = 1 MHz)
...
{
	/*
	* STM32G4 timer clocks (TCLK) are either at the same frequency as PCLK_N
	* when the APB prescaler is 1, and TLCK = 2 * PCLK if APBn_pre > 1. It's
	* expected that PCLK1 == PCLK2, so only have to check either of the apb
	* prescalar settings.
	*/
