...
__override void debugger_disable(void)
{
	if (IS_ENABLED(CHIP_FAMILY_STM32F4) ||
		...
		// panic_printf(
		// 	"PANIC: Called unimplemented security function %s.",
		// 	__func__);
		// while (1)
		// 	;
		// __builtin_unreachable();
		const int CALLED_SECURITY_FUNCTION_THAT_IS_NOT_IMPLEMENTED = 0;
		assert(CALLED_SECURITY_FUNCTION_THAT_IS_NOT_IMPLEMENTED);
		__builtin_unreachable();
	}
}
