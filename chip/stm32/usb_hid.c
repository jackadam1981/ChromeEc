...
static usb_uint hid_ep_buf[HID_REPORT_SIZE / 2] __usb_sram;

void set_keyboard_report(uint64_t rpt)
{
	memcpy_usbram((const uint8_t *)&rpt, hid_ep_buf, sizeof(rpt));
	/* enable TX */
...
static void hid_iface_request(uint16_t req)
{
	if ((req == (USB_DIR_IN | USB_RECIP_INTERFACE |
		    (USB_REQ_GET_DESCRIPTOR << 8))) &&
		    (sram_ep0_rx[1] == (USB_HID_DT_REPORT << 8))) {
		/* Setup : HID specific : Get Report descriptor */
		memcpy_usbram(report_desc, sram_ep0_tx,
				 sizeof(report_desc));
		btable_ep[0].tx_count = MIN(sram_ep0_rx[3],
				   sizeof(report_desc));
