...
void clock_enable_module(enum module_id module, int enable)
{
	...
	if ((!!new_mask) != (!!clock_mask)) {
		...
		else if (module == MODULE_SPI || module == MODULE_SPI_MASTER) {
...
#ifdef STM32_USE_PLL
#ifdef SECTION_IS_RW
	/* We do not need to re-configure system clock for RW again */
	uint32_t val;

	val = (STM32_RCC_CFGR & STM32_RCC_CFGR_SWS_MSK);
	current_osc = (val == STM32_RCC_CFGR_SWS_PLL) ? OSC_PLL : (
		(STM32_RCC_CFGR_SW & STM32_RCC_CFGR_SW_MSK) == STM32_RCC_CFGR_SW_MSI ? OSC_MSI : OSC_HSI);

	if (current_osc == OSC_PLL)
		freq = CPU_CLOCK;
	else
#endif
...
static uint32_t rtc_tr_to_sec(uint32_t rtc_tr)
{
	...
	sec += ((rtc_tr & RTC_TR_ST) >> 4) * RTC_TR_ST_POS +
	       (rtc_tr & RTC_TR_SU);
	return sec;
}
...
void low_power_init(void)
{
	...
	/* TODO: CHECK ME!! STOP2 MODE IS ENTERED.
	 * Is there any other peripherals configuration required?
	 */
	uint32_t val;

	val = STM32_PWR_CR1;
	val &= ~PWR_CR1_LPMS_MSK;
	val |= PWR_CR1_LPMS_STOP2;
	STM32_PWR_CR1 = val;
}
