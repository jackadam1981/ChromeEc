...
/* cmp, sec, tb, bp, start, len */
static struct protect_ranges w25q64[] = {
/*	{ 0, 'X', 'X', 0x0, 0x000000, 0 }, */
	{ 0, 0, 0, 0x1, 0x7e0000, 128 * 1024 },
	{ 0, 0, 0, 0x2, 0x7c0000, 256 * 1024 },
	{ 0, 0, 0, 0x3, 0x780000, 512 * 1024 },
	{ 0, 0, 0, 0x4, 0x700000, 1024 * 1024 },
	{ 0, 0, 0, 0x5, 0x600000, 2048 * 1024 },
	{ 0, 0, 0, 0x6, 0x400000, 4096 * 1024 },

	{ 0, 0, 1, 0x1, 0x000000, 128 * 1024 },
	{ 0, 0, 1, 0x2, 0x000000, 256 * 1024 },
	{ 0, 0, 1, 0x3, 0x000000, 512 * 1024 },
	{ 0, 0, 1, 0x4, 0x000000, 1024 * 1024 },
	{ 0, 0, 1, 0x5, 0x000000, 2048 * 1024 },
	{ 0, 0, 1, 0x6, 0x000000, 4096 * 1024 },
	{ 0, 'X', 'X', 0x7, 0x000000, 8192 * 1024 },

	{ 0, 1, 0, 0x1, 0x7ff000, 4 * 1024 },
	{ 0, 1, 0, 0x2, 0x7fe000, 8 * 1024 },
	{ 0, 1, 0, 0x3, 0x7fc000, 16 * 1024 },
	{ 0, 1, 0, 0x4, 0x7f8000, 32 * 1024 },
	{ 0, 1, 0, 0x5, 0x7f8000, 32 * 1024 },

	{ 0, 1, 1, 0x1, 0x000000, 4 * 1024 },
	{ 0, 1, 1, 0x2, 0x000000, 8 * 1024 },
	{ 0, 1, 1, 0x3, 0x000000, 16 * 1024 },
	{ 0, 1, 1, 0x4, 0x000000, 32 * 1024 },
	{ 0, 1, 1, 0x5, 0x000000, 32 * 1024 },

	{ 1, 'X', 'X', 0x0, 0x000000, 8192 * 1024 },
	{ 1, 0, 0, 0x1, 0x000000, 8064 * 1024 },
	{ 1, 0, 0, 0x2, 0x000000, 7936 * 1024 },
	{ 1, 0, 0, 0x3, 0x000000, 7680 * 1024 },
	{ 1, 0, 0, 0x4, 0x000000, 7168 * 1024 },
	{ 1, 0, 0, 0x5, 0x000000, 5120 * 1024 },
	{ 1, 0, 0, 0x6, 0x000000, 4096 * 1024 },

	{ 1, 0, 1, 0x1, 0x020000, 8064 * 1024 },
	{ 1, 0, 1, 0x2, 0x040000, 7936 * 1024 },
	{ 1, 0, 1, 0x3, 0x080000, 7680 * 1024 },
	{ 1, 0, 1, 0x4, 0x100000, 7168 * 1024 },
	{ 1, 0, 1, 0x5, 0x200000, 5120 * 1024 },
	{ 1, 0, 1, 0x6, 0x400000, 4096 * 1024 },
/*	{ 1, 'X', 'X', 0x7, 0x000000, 0 }, */

	{ 1, 1, 0, 0x1, 0x000000, 8188 * 1024 },
	{ 1, 1, 0, 0x2, 0x000000, 8184 * 1024 },
	{ 1, 1, 0, 0x3, 0x000000, 8176 * 1024 },
	{ 1, 1, 0, 0x4, 0x000000, 8160 * 1024 },
	{ 1, 1, 0, 0x5, 0x000000, 8160 * 1024 },

	{ 1, 1, 1, 0x1, 0x001000, 8188 * 1024 },
	{ 1, 1, 1, 0x2, 0x002000, 8184 * 1024 },
	{ 1, 1, 1, 0x3, 0x004000, 8176 * 1024 },
	{ 1, 1, 1, 0x4, 0x008000, 8176 * 1024 },
	{ 1, 1, 1, 0x5, 0x008000, 8160 * 1024 },
};
...
static int spi_flash_erase_32kb(int offset)
{
	...
	/* Chip has protection */
	if (spi_flash_check_protect(offset, 32 * 1024))
...
static int spi_flash_erase_4kb(int offset)
{
	...
	/* Chip has protection */
	if (spi_flash_check_protect(offset, 4 * 1024))
		return EC_ERROR_ACCESS_DENIED;
 ...
 * Will consider hardware WP to be no protection
 ...
 * @return EC_SUCCESS for no protection, or non-zero if error.
 */
int spi_flash_check_protect(int offset, int bytes)
{
	int i;
	char cmp;
	char sec;
	...
	bp = (sr1 & (SPI_FLASH_SR1_BP2 | SPI_FLASH_SR1_BP1
		 | SPI_FLASH_SR1_BP0)) >> 2;

	/* No protection set */
	if ((!cmp && !bp) || (cmp && bp == 0x7))
		return EC_SUCCESS;

	/* Find current protect range */
	for (i = 0; i < ARRAY_SIZE(w25q64); i++)
		if ((w25q64[i].cmp == cmp)
		&& (w25q64[i].sec == sec || w25q64[i].sec == 'X')
		&& (w25q64[i].tb == tb || w25q64[i].tb == 'X')
		&& (w25q64[i].bp == bp))
			if (MAX(w25q64[i].start, offset)
		      < MIN(w25q64[i].start + w25q64[i].len, offset + bytes))
				return EC_ERROR_ACCESS_DENIED;
 ...
 */
int spi_flash_set_protect(int offset, int bytes)
{
	int i;
	uint8_t sr1 = spi_flash_get_status1();
	uint8_t sr2 = spi_flash_get_status2();
	...
	/* Find desired protect range */
	for (i = 0; i < ARRAY_SIZE(w25q64); i++) {
		if (w25q64[i].start == offset && w25q64[i].len == bytes) {
			sr1 &= ~(SPI_FLASH_SR1_SEC | SPI_FLASH_SR1_TB
					| 0x7 << 2);
			sr1 |= (w25q64[i].sec ? SPI_FLASH_SR1_SEC : 0)
					| (w25q64[i].tb ? SPI_FLASH_SR1_TB : 0)
					| (w25q64[i].bp << 2);
			sr2 &= ~SPI_FLASH_SR2_CMP;
			sr2 |= (w25q64[i].cmp ? SPI_FLASH_SR2_CMP : 0);
			return spi_flash_set_status(sr1, sr2);
		}
	}
