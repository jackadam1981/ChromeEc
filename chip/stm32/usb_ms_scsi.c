...
#define SCSI_VPD_SUPPORTED_PAGES_SIZE				6
...
#define SCSI_VPD_SERIAL_NUMBER_SIZE				15
/* Vital product data (VPD) response for serial number page */
uint8_t SCSI_VPD_SERIAL_NUMBER[] = {
	0x00, /* Peripheral Qualifier | Peripheral Device Type (SBC-3) */
	SCSI_VPD_CODE_SERIAL_NUMBER, /* Page Code */
	(SCSI_VPD_SERIAL_NUMBER_SIZE - 3), /* Page Length */
	' ', /* Serial Number */
	' ',
	' ',
	' ',
...
const uint8_t SCSI_VPD_DEVICE_ID[] = {
	...
	'G', 'O', 'O', 'G', 'L', 'E', '\0', '\0', /* Vendor ID */
	'F', 'r', 'u', 'i', 't', 'P', 'i', 'e', '\0', /* Vendor Specific ID */
};
...
const uint8_t SCSI_CAPACITY_LIST[] = {
	...
	((CONFIG_SPI_FLASH_SIZE /
		SCSI_BLOCK_SIZE_BYTES) >> 24) & 0xff, /* Number of Blocks */
	((CONFIG_SPI_FLASH_SIZE /
		SCSI_BLOCK_SIZE_BYTES) >> 16) & 0xff, /* Number of Blocks */
	((CONFIG_SPI_FLASH_SIZE /
		SCSI_BLOCK_SIZE_BYTES) >> 8) & 0xff, /* Number of Blocks */
	(CONFIG_SPI_FLASH_SIZE /
		SCSI_BLOCK_SIZE_BYTES) & 0xff, /* Number of Blocks */
	0x02, /* Reserved | Descriptor Code (Formatted Media) */
	(SCSI_BLOCK_SIZE_BYTES >> 16) & 0xff, /* Block Length */
	(SCSI_BLOCK_SIZE_BYTES >> 8) & 0xff, /* Block Length */
	SCSI_BLOCK_SIZE_BYTES & 0xff, /* Block Length */
};
...
static void scsi_read_format_capacities(uint8_t *block, uint8_t in_len)
{
	if (state == USB_MS_SCSI_STATE_PARSE) {
		...
		memcpy_usbram(ms_ep_tx, SCSI_CAPACITY_LIST,
...
static void scsi_write10(uint8_t *block, uint8_t in_len)
{
	...
	if (state == USB_MS_SCSI_STATE_PARSE) {
		...
		write_len = MIN(bytes, btable_ep[USB_EP_MS_RX].rx_count & 0x3ff);
		...
		/* received too much data */
			...
			return scsi_sense_code(SCSI_SENSE_ILLEGAL_REQUEST,
				SCSI_SENSE_CODE_LBA_OUT_OF_RANGE);
#else
		rv = spi_flash_write(offset, write_len, (uint8_t *) ms_ep_rx);
		if (rv == EC_ERROR_INVAL)
			return scsi_sense_code(
				SCSI_SENSE_ILLEGAL_REQUEST,
				SCSI_SENSE_CODE_LBA_OUT_OF_RANGE);
...
void scsi_init(void)
{
	/* copy stm32 LOT_NUM and WAF_NUM for specific id */
	memcpy(SCSI_STANDARD_INQUIRY + 24, ((uint8_t *) STM32_UNIQUE_ID) + 4, 7 * sizeof(uint8_t));
	/* copy stm32 unique id as serial number */
	memcpy(SCSI_VPD_SERIAL_NUMBER + 3, ((uint8_t *) STM32_UNIQUE_ID), 12 * sizeof(uint8_t));
}
DECLARE_HOOK(HOOK_INIT, scsi_init, HOOK_PRIO_DEFAULT);
