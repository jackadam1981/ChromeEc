...
#define SCSI_SUPPORTED_VPD_PAGES_SIZE				3
/* Vital product data (VPD) response for supported VPD pages */
const uint8_t SCSI_SUPPORTED_VPD_PAGES[] = {
0x00, /* Peripheral Qualifier | Peripheral Device Type (SBC-3) */
0x00, /* Page Code */
(SCSI_SUPPORTED_VPD_PAGES_SIZE - 3), /* Page Length */
/* No VPD pages supported */
};
BUILD_ASSERT(sizeof(SCSI_SUPPORTED_VPD_PAGES) == SCSI_SUPPORTED_VPD_PAGES_SIZE);

#define SCSI_DEVICE_ID_SIZE					3
/* Vital product data (VPD) response for supported VPD pages */
const uint8_t SCSI_DEVICE_ID[] = {
0x00, /* Peripheral Qualifier | Peripheral Device Type (SBC-3) */
0x00, /* Page Code */
(SCSI_DEVICE_ID_SIZE - 3), /* Page Length */
/* No VPD pages supported */
};
BUILD_ASSERT(sizeof(SCSI_DEVICE_ID) == SCSI_DEVICE_ID_SIZE);
...
uint8_t temp_buf[USB_MS_PACKET_SIZE];
		...
		if (block[1] & 0x1) {
			...
			case SCSI_VPD_SUPPORTED_PAGES:
				...
				if ((block[3] << 8 | block[4]) <
				     sizeof(SCSI_SUPPORTED_VPD_PAGES))
...
static void scsi_write10(uint8_t *block, uint8_t in_len)
{
	...
	if (state == USB_MS_SCSI_STATE_PARSE) {
		...
		rv = spi_flash_write(offset, write_len, (uint8_t *) ep_buf_rx);
		if (rv == EC_ERROR_INVAL)
			return scsi_sense_code(SCSI_SENSE_ILLEGAL_REQUEST,
				SCSI_SENSE_CODE_LBA_OUT_OF_RANGE);
		else if (rv == EC_ERROR_ACCESS_DENIED)
