...
const uint8_t SCSI_VPD_DEVICE_ID[] = {
	...
	'G', 'O', 'O', 'G', 'L', 'E', '\0', '\0', /* Vendor ID */
	'F', 'r', 'u', 'i', 't', 'P', 'i', 'e', /* Vendor Specific ID */
...
void scsi_init(void)
{
	/* copy STM32 LOT_NUM (ascii) for vendor specific id */
	memcpy(SCSI_STANDARD_INQUIRY + 24,
		((uint8_t *) STM32_UNIQUE_ID) + 4 + 1, 7 * sizeof(uint8_t));

	/* copy STM32 LOT_NUM (ascii) for serial number */
	memcpy(SCSI_VPD_SERIAL_NUMBER + 4,
		((uint8_t *) STM32_UNIQUE_ID) + 4 + 1, 7 * sizeof(uint8_t));
	/* copy STM32 WAF_NUM for serial number */
	SCSI_VPD_SERIAL_NUMBER[4 + 7] = ((uint8_t *) STM32_UNIQUE_ID)[4];
	/* copy STM32 UID for serial number */
	memcpy(SCSI_VPD_SERIAL_NUMBER + 4 + 8,
		((uint8_t *) STM32_UNIQUE_ID), 4 * sizeof(uint8_t));
}
