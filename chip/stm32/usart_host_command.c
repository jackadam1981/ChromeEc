...
static struct usart_config const tl_usart = {
	...
	&usart_host_command_tx_interrupt,
	&((struct usart_state){}),
		...
		const void *src, size_t count)
{
	...
	((usart_in_usart_head + count) < USART_MAX_REQUEST_SIZE)) {
		...
		memcpy(usart_in_usart_bytes + usart_in_usart_head,
			src, count);
	}
		...
		void *dest)
{
	...
	&& (usart_out_datalen != 0)) {
		*((uint8_t *) dest) =
			usart_out_usart_bytes[usart_out_usart_head++];

		bytes_remaining = usart_out_datalen - usart_out_usart_head;
