 ...
 * at least 2x of that time, before declaring timeout and dropping the packet.
 *
 ...
 */
static uint8_t tl_in_usart_bytes[USART_MAX_REQUEST_SIZE] __aligned(4);
static uint8_t tl_out_usart_bytes[USART_MAX_RESPONSE_SIZE] __aligned(4);
 ...
 */
static struct usart_rx_dma const tl_usart1_rx_dma = {
        .usart_rx = {
                .producer_ops = {
                        .read = NULL,
                        },
 ...
 */
static void tl_usart_request_timeout(void) {
    if (current_state == TL_USART_RECEIVING) {
        /* If state is receiving then timeout was hit due to underrun */
        CPRINTS("TL USART ERROR: Request underrun detected.");
    } else if (current_state == TL_USART_RX_OVERRUN) {
        /* If state is rx_overrun then timeout was hit because
         * process request was cancelled and extra rx bytes were dropped*/
        CPRINTS("TL USART ERROR: Request overrun detected.");
    } else if (current_state == TL_USART_RX_BAD) {
        CPRINTS("TL USART ERROR: Bad packet header detected.");
    } else
    {
        CPRINTS("TL USART ERROR: Request timeout mishandled");
    }

    /* Move current state to rx bad */
...
/*
 * This function handles incoming bytes from DMA interrupt handler
 */
void tl_usart_rx_add_bytes(const void *src, size_t count) {
    ...
    if ((current_state == TL_USART_READY_TO_RX) ||
            ...
            ((tl_in_usart_head + count) < USART_MAX_REQUEST_SIZE)) {
        /* Copy all the bytes from DMA FIFO */
        memcpy(tl_in_usart_bytes + tl_in_usart_head, src, count);
    }

    tl_in_usart_head += count;
    ...
    (tl_in_usart_head >= sizeof(struct ec_host_request))) {
        ...
        if ((tl_in_datalen == 0) ||
                (tl_in_datalen > USART_MAX_REQUEST_SIZE)) {
            /* EC host request version not compatible or reserved byte is a
             * not zero */
            current_state = TL_USART_RX_BAD;

        } else if (tl_in_datalen == sizeof(struct ec_host_request)) {
    ...
    (tl_in_datalen > sizeof(struct ec_host_request))) ||
    (current_state == TL_USART_RX_OVERRUN)) {
        /* Cancel deferred call to process_request */
        hook_call_deferred(&tl_usart_process_request_data, -1);
