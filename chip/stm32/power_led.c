...
static uint32_t power_led_percent;
...
static void power_led_timer_init(void)
{
	...
	/*
	 * Timer configuration : Upcounter, counter enabled, update
	 * event on capture/compare
	 */
	...
	STM32_TIM_ARR(2) = 100;			/* auto-reload value (freq) */
	...
	/* generate an update event manually to force loading of
	   shadow registers */
	STM32_TIM_EGR(2) |= 1;
...
static int power_led_step(void)
{
	...
	if (0 == power_led_percent) {
		...
		/* decrease timeout as duty cycle percentage approaches
	...
	if (dir == UP)
		power_led_percent += LED_STEP_PERCENT;
	else
		power_led_percent -= LED_STEP_PERCENT;
