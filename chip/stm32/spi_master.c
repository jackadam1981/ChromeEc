 ...
 */
static int spi_master_initialize(const struct spi_port_t *spi_port)
{
	int i;

	stm32_spi_regs_t *spi = SPI_REGS[spi_port->port];
	...
	for (i = 0; i < spi_port->gpio_cs_len; i++) {
		/* Drive SS high */
		gpio_set_level(spi_port->gpio_cs[i], 1);
	}

	/* Set flag */
	spi_enabled[spi_port->port] = 1;
 ...
 * Shutdown SPI module
 */
static int spi_master_shutdown(const struct spi_port_t *spi_port)
{
	int rv = EC_SUCCESS;
	int port = spi_port->port;
...
{
	if (enable == spi_enabled[spi_port->port])
		return EC_SUCCESS;
	if (enable)
		return spi_master_initialize(spi_port);
	else
		return spi_master_shutdown(spi_port);
}
