...
static inline int spi_tx_done(stm32_spi_regs_t *spi)
{
	return !(spi->sr & (STM32_SPI_SR_FTLVL | STM32_SPI_SR_BSY));
}

static inline int spi_rx_done(stm32_spi_regs_t *spi)
{
	return !(spi->sr & (STM32_SPI_SR_FRLVL | STM32_SPI_SR_RXNE));
}

/* Read until RX FIFO is empty (i.e. RX done) */
static inline int spi_clear_rx_fifo(stm32_spi_regs_t *spi)
{
...
static inline int spi_clear_tx_fifo(stm32_spi_regs_t *spi)
{
	timestamp_t timeout;
...
static int spi_dma_wait(int port)
{
	...
	if (dma_is_enabled(&dma_tx_option[port])) {
		 ...
		 * Therefore, even though `dma_wait` pulls with interval 0.1ms,
	...
	if (dma_is_enabled(&dma_rx_option[port])) {
		/* Because `dma_wait` pulls with interval 0.1ms, we will read at
		 * least ~100 bytes.  If you don't want this overhead, you can
		 * use interrupt handler (`dma_enable_tc_interrupt_callback`)
		 * and disable SPI interface in callback function.
		 */
