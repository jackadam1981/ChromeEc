...
#define STM32_IRQ_ADC1_2          18 /* STM32F10x only */
#define STM32_IRQ_USB_HP          19
#define STM32_IRQ_USB_HP_CAN_TX   19 /* STM32F10x only */
...
#define STM32_BKP_DATA(n) (				\
	if (n < 11)					\
		REG16(STM32_BKP_BASE + 0x4 + 4 * (n));	\
	else						\
		REG16(STM32_BKP_BASE + 0x40 + 4 * (n)); \
)
#else
