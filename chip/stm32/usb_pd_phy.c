...
static struct dma_option dma_tx_option[PD_PORT_COUNT];
...
{
	dma_tx_option[port].flags |= STM32_DMA_CCR_CIRC;
}
...
void pd_start_tx(int port, int polarity, int bit_len)
{
	 ...
	 */
	pd_timers_tx[port]->cnt = TX_CLOCK_DIV - 1;

	/* update DMA configuration */
	dma_prepare_tx(&dma_tx_option[port], DIV_ROUND_UP(bit_len, 8),
...
void pd_rx_start(int port)
{
	...
	dma_start_rx(&dma_tim_option[port], PD_MAX_RAW_SIZE,
			pd_phy[port].raw_samples);
	/* enable TIM2 DMA requests */
	pd_timers_rx[port]->egr = 0x0001; /* reset counter / reload PSC */;
	pd_timers_rx[port]->sr = 0; /* clear overflows */
	pd_timers_rx[port]->cr1 |= 1;
}
...
int pd_rx_started(int port)
{
	...
	return pd_timers_rx[port]->cr1 & 1;
}
...
void pd_hw_init(int port)
{
	...
	/* configure TX DMA */
	dma_tx_option[port].channel = DMAC_SPI_TX(port);
	dma_tx_option[port].periph = (void *)&SPI_REGS(port)->dr;
	dma_tx_option[port].flags = STM32_DMA_CCR_MSIZE_8_BIT |
				    STM32_DMA_CCR_PSIZE_8_BIT;
	dma_prepare_tx(&dma_tx_option[port], PD_MAX_RAW_SIZE,
			pd_phy[port].raw_samples);
	...
	/* Timer configuration */
	pd_timers_tx[port]->cr1 = 0x0000;
	pd_timers_tx[port]->cr2 = 0x0000;
	pd_timers_tx[port]->dier = 0x0000;
	/* Auto-reload value : 600000 Khz overflow */
	pd_timers_tx[port]->arr = TX_CLOCK_DIV;
	/* 50% duty cycle on the output */
	pd_timers_tx[port]->ccr[1] = pd_timers_tx[port]->arr / 2;
	/* Timer CH1 output configuration */
	pd_timers_tx[port]->ccmr1 = (6 << 4) | (1 << 3);
	pd_timers_tx[port]->ccer = 1;
	pd_timers_tx[port]->bdtr = 0x8000;
	/* set prescaler to /1 */
	pd_timers_tx[port]->psc = 0;
	/* Reload the pre-scaler and reset the counter */
	pd_timers_tx[port]->egr = 0x0001;
#endif

	/* --- set counter for RX timing : 2.4Mhz rate, free-running --- */
	__hw_timer_enable_clock(TIM_CLOCK_PD_RX(port), 1);
	/* Timer configuration */
	pd_timers_rx[port]->cr1 = 0x0000;
	pd_timers_rx[port]->cr2 = 0x0000;
	pd_timers_rx[port]->dier = 0x0000;
	/* Auto-reload value : 16-bit free running counter */
	pd_timers_rx[port]->arr = 0xFFFF;

	/* Timeout for message receive : 2.7ms */
	pd_timers_rx[port]->ccr[2] = 2400000 * 27 / 10000;
	/* Timer ICx input configuration */
	if (TIM_CCR_IDX(port) == 1)
		pd_timers_rx[port]->ccmr1 |= TIM_CCR_CS << 0;
	else
		/*  Unsupported RX timer capture input */
		ASSERT(0);

	pd_timers_rx[port]->ccer = 0xB << ((TIM_CCR_IDX(port) - 1) * 4);
	/* configure DMA request on CCRx update */
	pd_timers_rx[port]->dier |= 1 << (8 + TIM_CCR_IDX(port)); /* CCxDE */;
	/* set prescaler to /26 (F=1.2Mhz, T=0.8us) */
	pd_timers_rx[port]->psc = (clock_get_freq() / 2400000) - 1;
	/* Reload the pre-scaler and reset the counter (clear CCRx) */
	pd_timers_rx[port]->egr = 0x0001 | (1 << TIM_CCR_IDX(port));
	/* clear update event from reloading */
	pd_timers_rx[port]->sr = 0;
...
{
	pd_timers_tx[port]->arr = clock_get_freq() / (2*freq);
}
