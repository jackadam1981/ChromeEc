...
static uint32_t raw_samples[PD_PORT_COUNT][DIV_ROUND_UP(PD_MAX_RAW_SIZE,
		sizeof(uint32_t))];
...
void pd_start_tx(void *ctxt, int port, int polarity, int bit_len)
{
	...
	dma_prepare_tx(&dma_tx_option[port], DIV_ROUND_UP(bit_len, 8), ctxt);
...
void pd_rx_start(int port)
{
	...
	dma_start_rx(&dma_tim_option[port], PD_MAX_RAW_SIZE, raw_samples[port]);
...
#error "CPU_CLOCK must be 38.4MHz to use SPI master for USB PD Tx"
	...
	else if (TIM_CCR_IDX(port) == 2)
		pd_timers_rx[port]->ccmr1 |= TIM_CCR_CS << 8;
	else
