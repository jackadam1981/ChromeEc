...
{
	int port = (int *)data - tx_dma_polarities;
	int polarity = *(int *)data;
	stm32_spi_regs_t *spi = SPI_REGS(port);
...
int pd_start_tx(int port, int polarity, int bit_len)
{
	...
	dma_enable_tc_interrupt(DMAC_SPI_TX(port), &tx_dma_done,
				&tx_dma_polarities[port]);
#endif
	dma_go(tx);
...
void pd_tx_done(int port, int polarity)
{
	...
	while (rv != TASK_EVENT_TIMER && rv != TASK_EVENT_DMA_TC)
