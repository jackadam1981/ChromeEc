...
static inline void _i2c_dma_wake_callback(void *cb_data, int port)
{
        ...
        if (id != TASK_ID_INVALID)
                task_set_event(id, I2C_COMPLETION(port), 0);
}

static void _i2c_dma_wake_callback_0(void *cb_data) {_i2c_dma_wake_callback(cb_data, 0);}
static void _i2c_dma_wake_callback_1(void *cb_data) {_i2c_dma_wake_callback(cb_data, 1);}
static void _i2c_dma_wake_callback_2(void *cb_data) {_i2c_dma_wake_callback(cb_data, 2);}
static void _i2c_dma_wake_callback_3(void *cb_data) {_i2c_dma_wake_callback(cb_data, 3);}
...
static int wait_sr1_poll(int port, int mask, int val, int poll)
{
	...
	while (get_time().val < timeout) {
		...
		if (sr1 & (STM32_I2C_SR1_ARLO | STM32_I2C_SR1_BERR |
			   STM32_I2C_SR1_AF)) {
			CPRINTS("I2C Err: p:%d m:%x sr1:0x%08x", port, mask, sr1);
...
/*
 * Returns i2c_port_t * associated with this port number.
 */
...
static int wait_fmpi2c_isr_poll(int port, int mask, int val, int poll)
{
	...
	while (get_time().val < timeout) {
		...
		if (isr & (FMPI2C_ISR_ARLO | FMPI2C_ISR_BERR |
			   FMPI2C_ISR_NACKF)) {
			CPRINTS("FMPI2C err p:%d, m:0x%08x, isr:0x%08x", port, mask, isr);
			return EC_ERROR_UNKNOWN;
		}
...
static int send_fmpi2c_start(int port, int slave_addr, int size, int is_read)
{
	...
	reg &= ~FMPI2C_CR2_SADD_MASK & ~FMPI2C_CR2_SIZE_MASK & ~FMPI2C_CR2_RELOAD &
...
static void i2c_set_freq_port(const struct i2c_port_t *p)
{
	...
	if (p->type == 0) {
		...
		if (p->kbps > 100) {
			...
			STM32_I2C_CCR(port) = STM32_I2C_CCR_FM | STM32_I2C_CCR_DUTY |
		...
		prescalar = (freq / (p->kbps * 1000 * (0x12 + 1 + 0xe + 1 + 1))) - 1;
		...
		CPRINTS("port %d target %d, pre %d, act %d, reg 0x%08x", port, p->kbps, prescalar, actual, reg);
...
static void i2c_init_port(const struct i2c_port_t *p)
{
	...
	if (p->type != 0) {
		...
		/* STM32_FMPI2C_CR1(port) = FMPI2C_CR1_RXDMAEN | FMPI2C_CR1_TXDMAEN; */
	} else {
		STM32_I2C_CR1(port) |= STM32_I2C_CR1_SWRST;
		STM32_I2C_CR1(port) &= ~STM32_I2C_CR1_SWRST;
		udelay(10);
...
int complete_fmpi2c_read(int port) {
...
int start_fmpi2c_read(int port, int slave_addr, int in_bytes, uint8_t *in) {
	const struct dma_option *dma = dma_rx_option + port;
	int rv;
#if 1
	...
	rv = send_fmpi2c_start(port, slave_addr, in_bytes, FMPI2C_READ);
	if (rv) {
		CPRINTS("start_fmpi2c_read: p:%d fail to START", port);
	}

	rv = wait_fmpi2c_isr(port, FMPI2C_ISR_RXNE);
	if (rv) {
		CPRINTS("start_fmpi2c_read: p:%d fail to find RXNE", port);
		/*complete_fmpi2c_read(port);*/
		return rv;
	}
	STM32_FMPI2C_CR1(port) |= FMPI2C_CR1_RXDMAEN;
	...
	rv = wait_fmpi2c_isr_poll(port, FMPI2C_ISR_BUSY, UNSET, 1);
	if (rv) {
	...
	CPRINTS("start_fmpi2c_read: p:%d find RXNE", port);
	rv = wait_fmpi2c_isr_poll(port, FMPI2C_ISR_RXNE, SET, 1);
	if (rv) {
		CPRINTS("start_fmpi2c_read: p:%d fail to find RXNE", port);
		complete_fmpi2c_read(port);
		return rv;
...
int chip_fmpi2c_xfer(int port, int slave_addr, const uint8_t *out, int out_bytes,
		  uint8_t *in, int in_bytes, int flags)
{
	...
	if (out_bytes || !in_bytes) {
		if (!started) {
			rv = send_fmpi2c_start(port, slave_addr, out_bytes, FMPI2C_WRITE);
			if (rv)
				goto xfer_exit;
		}
	...
	if (in_bytes == 2) {
		...
		rv_start = send_fmpi2c_start(port, slave_addr, in_bytes, FMPI2C_READ);
		if (rv_start)
			goto xfer_exit;
		...
		if (!rv_start) {
			rv = task_wait_event_mask(I2C_COMPLETION(port),
		...
		if (rv_start) {
			rv = rv_start;
                ...
                rv_start = start_i2c_read(port, slave_addr, in_bytes, in);
                if (!rv_start) {
                        rv = task_wait_event_mask(I2C_COMPLETION(port),
                                        DMA_TRANSFER_TIMEOUT_US);
                        if (rv & I2C_COMPLETION(port)) {
                                ...
                                rv = EC_ERROR_TIMEOUT;
                                CPRINTS("FMPI2C timeout: p:%d", port);
                        }
                }

                rv = complete_i2c_read(port);
		if (rv) {
			CPRINTS("chip_fmpi2c_xfer: complete_i2c_read fail rv:%x", rv);
		}
...
int complete_i2c_read(int port) {
        const struct i2c_port_t *p = find_port(port);
        const struct dma_option *dma = dma_rx_option + port;

        if (p->type != 0)
                return complete_fmpi2c_read(port);
...
/* Start an I2C read and wait for completion. 
 * This bust be called in sequence with:
 *   start_i2c_read(port)
 *   task_wait_event_mask(I2C_COMPLETION(port), timeout)
 *   complete_i2c_read(port)
 *   - Data is now valid.
 *
 * Args:
 * port: I2C port
 * slave_addr: 7 bit slave address (in bits 1-7 of byte)
 * in_bytes: bytes to transfer
 * in: pointer to DMA buffer. WIll be valid after completion.
 */
int start_i2c_read(int port, int slave_addr, int in_bytes, uint8_t *in) {
	const struct i2c_port_t *p = find_port(port);
	const struct dma_option *dma = dma_rx_option + port;
	int rv;

	if (p->type != 0)
		return start_fmpi2c_read(port, slave_addr, in_bytes, in);

	rv = wait_sr1_poll(port, STM32_I2C_SR2_BUSY, UNSET, 1);
	if (rv) {
		CPRINTS("i2c port %d busy", port);
		return EC_ERROR_BUSY;
	}
	...
	rv = send_start(port, slave_addr | 1);
	if (rv) {
		CPRINTS("start_i2c_read: send_start failed p:%d", port);
		complete_i2c_read(port);
	}
		  ...
		  uint8_t *in, int in_bytes, int flags)
{
	...
	if (in_bytes == 2) {
		...
		rv_start = send_start(port, slave_addr | 1);
