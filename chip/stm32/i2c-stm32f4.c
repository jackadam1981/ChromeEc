 ...
 */
static int send_start(const int port, const uint16_t addr_reg_val)
{
	int rv;
	...
	/* Write slave address */
	STM32_I2C_DR(port) = addr_reg_val;
 ...
 * @return Non-zero if error.
 */
static int send_fmpi2c_start(const int port, const uint16_t addr_reg_val,
			     int size, int is_read)
{
	...
	reg |= FMPI2C_CR2_START | FMPI2C_CR2_AUTOEND |
		addr_reg_val | FMPI2C_CR2_SIZE(size) |
 ...
 */
static int chip_fmpi2c_xfer(const int port, const uint16_t addr_reg_val,
			    ...
			    uint8_t *in, int in_bytes, int flags)
{
	...
	if (out_bytes || !in_bytes) {
		rv = send_fmpi2c_start(
			port, addr_reg_val, out_bytes, FMPI2C_WRITE);
		if (rv)
			goto xfer_exit;
	...
	if (in_bytes) {
		...
		rv_start = send_fmpi2c_start(
				port, addr_reg_val, in_bytes, FMPI2C_READ);
		if (rv_start)
			goto xfer_exit;
...
/* Perform an i2c transaction. */
int chip_i2c_xfer__7bf(const int port, const uint16_t i2c_addr__7bf,
		  const uint8_t *out, int out_bytes,
		  uint8_t *in, int in_bytes, int flags)
{
	int addr_reg_val = I2C_ADDR__7b(i2c_addr__7bf) << 1;
	int started = (flags & I2C_XFER_START) ? 0 : 1;
	int rv = EC_SUCCESS;
	int i;
	...
	if (p->port == STM32F4_FMPI2C_PORT) {
		return chip_fmpi2c_xfer(port, addr_reg_val,
					out, out_bytes,
					in, in_bytes, flags);
	}
	...
	/* No out bytes and no in bytes means just check for active */
	if (out_bytes || !in_bytes) {
		rv = send_start(port, addr_reg_val);
		if (rv)
			goto xfer_exit;
	...
	if (in_bytes) {
		...
		rv_start = send_start(port, addr_reg_val | 0x01);
	...
	if (i2c_sr2 & STM32_I2C_SR2_TRA) {
		if (i2c_sr1 & (STM32_I2C_SR1_BTF | STM32_I2C_SR1_TXE)) {
			if (tx_pending) {
				...
				STM32_I2C_CR2(port) &= ~STM32_I2C_CR2_ITBUFEN;
#ifdef CONFIG_BOARD_I2C_SLAVE_ADDR__7bf
				if (addr == CONFIG_BOARD_I2C_SLAVE_ADDR__7bf)
...
#ifdef CONFIG_BOARD_I2C_SLAVE_ADDR__7bf
		if (rx_pending && addr == CONFIG_BOARD_I2C_SLAVE_ADDR__7bf)
