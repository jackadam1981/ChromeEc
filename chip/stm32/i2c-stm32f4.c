...
int chip_i2c_xfer__7bf(const int port, const uint16_t i2c_addr__7bf,
		  ...
		  uint8_t *in, int in_bytes, int flags)
{
	int slave_addr__8b = I2C_ADDR__7b(i2c_addr__7bf) << 1;
	int started = (flags & I2C_XFER_START) ? 0 : 1;
	int rv = EC_SUCCESS;
	int i;
	...
	if (p->port == STM32F4_FMPI2C_PORT) {
		return chip_fmpi2c_xfer__8b(port, slave_addr__8b,
					out, out_bytes,
					in, in_bytes, flags);
	}
	...
	/* No out bytes and no in bytes means just check for active */
	if (out_bytes || !in_bytes) {
		rv = send_start__8b(port, slave_addr__8b);
		if (rv)
			goto xfer_exit;
	...
	if (in_bytes) {
		...
		rv_start = send_start__8b(port, slave_addr__8b | 0x01);
...
#ifdef CONFIG_BOARD_I2C_SLAVE_ADDR__7bf
		if (rx_pending &&
		    (addr__8b >> 1) == CONFIG_BOARD_I2C_SLAVE_ADDR__7bf)
...
static void i2c_init(void)
{
	...
	STM32_I2C_OAR1(I2C_PORT_EC) = STM32_I2C_OAR1_B14
		| (I2C_ADDR__7b(CONFIG_HOSTCMD_I2C_SLAVE_ADDR__7bf) << 1);
#ifdef CONFIG_BOARD_I2C_SLAVE_ADDR__7bf
	STM32_I2C_OAR2(I2C_PORT_EC) = STM32_I2C_OAR2_ENDUAL
		| (I2C_ADDR__7b(CONFIG_BOARD_I2C_SLAVE_ADDR__7bf) << 1);
#endif
