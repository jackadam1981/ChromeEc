...
static int wait_fmpi2c_isr(int port, int mask)
{
	...
	while (get_time().val < timeout) {
		...
		if (isr & (FMPI2C_ISR_ARLO | FMPI2C_ISR_BERR |
			   FMPI2C_ISR_NACKF)) {
			dump_i2c_reg(port, "wait_fmpi2c_isr failed");
			return EC_ERROR_UNKNOWN;
		}
	...
	CPRINTS("port %d, mask 0x%08x, isr 0x%08x", port, mask, STM32_FMPI2C_ISR(port));
...
static int send_fmpi2c_start(int port, int slave_addr, int size, int is_read)
{
	...
	/*CPRINTS("send_fmpi2c_start addr 0x%x, size %d, read %d, reg 0x%08x", slave_addr,size,is_read, reg);*/
