...
/*
 * Mask of the flags that cannot be ignored (if any of those are given, then
 * the standard spi_transaction() that does not take eeprom_flags cannot
 * possibly handle it.
 */ 
#define EEPROM_FLAGS_REQUIRES_SUPPORT \
	(EEPROM_FLAG_OPCODE_WIDTH_MSK \
	| EEPROM_FLAG_OPCODE_DTR \
	| EEPROM_FLAG_ADDR_WIDTH_MSK \
	| EEPROM_FLAG_ADDR_DTR \
	| EEPROM_FLAG_ALT_WIDTH_MSK \
	| EEPROM_FLAG_ALT_DTR \
	| EEPROM_FLAG_DUMMY_CYCLES_MSK \
	| EEPROM_FLAG_DATA_WIDTH_MSK \
...
#define EEPROM_FLAG_DOUBLE_BUFFER_POS 30U
#define EEPROM_FLAG_DOUBLE_BUFFER (0x1UL << EEPROM_FLAG_DOUBLE_BUFFER_POS)
