 ...
 *          bits 7:8  width (0: N=2, 1: N=4, 2: N=8)
 *             bit 9  double transfer rate
 *        bits 10:14  number of dummy cycles (0-31)
 *        bits 15:26  reserved, must be zero
 ...
 *                    length + count, (the last one only if bit 31 indicates a
 *                    write operation). Due to data alignment constraints, this
 *                    must be an even number of bytes unless this is the final
 *                    packet.
 *
 *
 * USB Error Codes:
...
#define USB_SPI_CONFIG(INTERFACE, ENDPOINT)                                 \
	static uint16_t usb_spi_buffer1_[(USB_SPI_BUFFER_SIZE + 1) / 2];    \
	static uint16_t usb_spi_buffer2_[(USB_SPI_BUFFER_SIZE + 1) / 2];    \
	static usb_uint                                                     \
	...
	struct usb_spi_state usb_spi_state = {                              \
		...
		.txn[0].spi_write_ctx.buffer = (uint8_t *)usb_spi_buffer1_, \
		.txn[0].spi_read_ctx.buffer = (uint8_t *)usb_spi_buffer1_,  \
		.txn[1].spi_write_ctx.buffer = (uint8_t *)usb_spi_buffer2_, \
		.txn[1].spi_read_ctx.buffer = (uint8_t *)usb_spi_buffer2_,  \
...
#define FLASH_FLAG_WIDTH_2WIRE (0x0U << FLASH_FLAG_WIDTH_POS)
#define FLASH_FLAG_WIDTH_4WIRE (0x1U << FLASH_FLAG_WIDTH_POS)
#define FLASH_FLAG_WIDTH_8WIRE (0x2U << FLASH_FLAG_WIDTH_POS)
 ...
 * opcode/address/data.
 */
#define FLASH_FLAGS_REQUIRING_SUPPORT (FLASH_FLAG_MODE_MSK | FLASH_FLAG_DTR)
