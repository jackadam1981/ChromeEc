...
#define USB_POWER_MAX_READ_COUNT 29
...
struct __attribute__ ((__packed__)) usb_power_report {
	...
	uint8_t size;
	uint32_t timestamp;
	uint16_t voltage[USB_POWER_MAX_READ_COUNT];
};
...
#define USB_POWER_CONFIG(NAME,						\
	...
	struct usb_power_config const NAME = {				\
		...
		.deferred_cap  = CONCAT2(NAME, _deferred_cap_),		\
