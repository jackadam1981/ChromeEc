...
void usb_spi_deferred(struct usb_spi_config const *config)
{
	...
	if (receive_packet->packet_size) {
		int write_count = receive_packet->packet_v1.command.write_count;
		int read_count = receive_packet->packet_v1.command.read_count;
		int status_code = USB_SPI_SUCCESS;

		receive_packet->header_size = offsetof(usb_spi_command_v1_t, data);
		transmit_packet->header_size = offsetof(usb_spi_response_v1_t, data);
		...
		if (!config->state->enabled) {
			...
			status_code = usb_spi_read_usb_packet(
					&config->state->spi_write_ctx, receive_packet);
		}
		...
		transmit_packet->packet_v1.response.status_code = status_code;
