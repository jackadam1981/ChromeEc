...
void usb_spi_deferred(struct usb_spi_config const *config)
{
	...
	if (config->state->other_txn_status == OTHER_TXN_IN_PROGRESS) {
		...
		if (custom_board_driver ? usb_spi_board_transaction_is_complete(current_device) : spi_transaction_is_complete(current_device)) {
			config->state
				->txn[1 - config->state->usb_txn_idx]
				.status_code =
				custom_board_driver ?
				usb_spi_board_transaction_flush(
					current_device) :
				spi_transaction_flush(
					current_device);
	...
	if (config->state->mode == USB_SPI_MODE_START_SPI) {
		...
		// TODO: Confusion of error types above (USB_SPI_SUCCESS/EC_SUCCESS)
