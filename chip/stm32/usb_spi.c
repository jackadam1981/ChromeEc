 ...
 */
static int usb_spi_read_usb_packet(usb_spi_receive_ctx_t* dst,
 ...
 */
static void usb_spi_fill_usb_packet(usb_spi_packet_ctx_t* dst,
		usb_spi_transmit_ctx_t* src)
{
	int transfer_size = src->transmit_size - src->transmit_index;
	int max_buffer_size = USB_MAX_PACKET_SIZE - dst->header_size;
		...
		int write_count, int read_count)
{
	...
	config->state->spi_write_ctx.receive_size = write_count;
	config->state->spi_write_ctx.receive_index = 0;
	config->state->spi_read_ctx.transmit_size = read_count;
	config->state->spi_read_ctx.transmit_index = 0;
}
...
static bool usb_spi_response_in_progress(struct usb_spi_config const *config) {
	if ((config->state->mode == USB_SPI_TASK_START_RESPONSE) ||
	    (config->state->mode == USB_SPI_TASK_CONTINUE_RESPONSE)) {
		return true;
	}
	return false;
}

/* Prep the state to construct a new response
 *
 * @param config        USB SPI config
 * @param
 */
		...
		uint16_t status_code)
{
	...
	config->state->spi_read_ctx.transmit_index = 0;
	config->state->mode = USB_SPI_TASK_START_RESPONSE;

	/* If an error occured, transmit an empty start packet. */
	if (status_code != USB_SPI_SUCCESS) {
		config->state->spi_read_ctx.transmit_size = 0;
	}
}
...
{
	/* Clear the state. */
	config->state->mode = USB_SPI_TASK_IDLE;

	/* Construct the response packet. */
	packet->packet_v2.packet_id = USB_SPI_PKT_ID_RSP_USB_SPI_CONFIG;
	packet->packet_v2.rsp_config.max_write_count = USB_SPI_MAX_WRITE_COUNT;
	packet->packet_v2.rsp_config.max_read_count = USB_SPI_MAX_READ_COUNT;
	packet->packet_size = sizeof(usb_spi_response_configuration_v2_t);
}
...
static void usb_spi_create_spi_transfer_response (
		...
		usb_spi_packet_ctx_t *transmit_packet)
{
	...
	if (config->state->spi_read_ctx.transmit_index == 0) {

		/* Transmit the first packet with the status code. */
		transmit_packet->header_size = offsetof(usb_spi_response_v2_t, data);
		transmit_packet->packet_v2.rsp_start.packet_id =
				USB_SPI_PKT_ID_RSP_TRANSFER_START;
		transmit_packet->packet_v2.rsp_start.status_code =
				config->state->status_code;

		usb_spi_fill_usb_packet(transmit_packet, &config->state->spi_read_ctx);
	} else if (config->state->spi_read_ctx.transmit_index <
	           config->state->spi_read_ctx.transmit_size) {

		/* Transmit the continue packets. */
		transmit_packet->header_size = offsetof(usb_spi_continue_v2_t, data);
		transmit_packet->packet_v2.rsp_continue.packet_id =
				USB_SPI_PKT_ID_RSP_TRANSFER_CONTINUE;
		transmit_packet->packet_v2.rsp_continue.data_index =
				config->state->spi_read_ctx.transmit_index;

		usb_spi_fill_usb_packet(transmit_packet, &config->state->spi_read_ctx);
	}
	if (config->state->spi_read_ctx.transmit_index <
		config->state->spi_read_ctx.transmit_size) {
		config->state->mode = USB_SPI_TASK_CONTINUE_RESPONSE;
	} else {
		config->state->mode = USB_SPI_TASK_IDLE;
		...
		usb_spi_packet_ctx_t* packet)
{
	...
	/* Check to see if there's valid data in the packet. */
	if (packet->packet_size == 0) {
		return;
	} else if (packet->packet_size < USB_SPI_MIN_PACKET_SIZE) {
		/* No valid packet exists smaller than the packet id. */
		setup_transfer_response(config, USB_SPI_RX_UNEXPECTED_PACKET);
		return;
	}

	switch (packet->packet_v2.packet_id) {
		case USB_SPI_PKT_ID_CMD_GET_USB_SPI_CONFIG:
		{
			/* The host requires the SPI configuration. */
			config->state->mode = USB_SPI_TASK_SEND_CONFIGURATION;
			break;
		}
		case USB_SPI_PKT_ID_CMD_RESTART_RESPONSE:
		{
			/* The host has requested the device restart the last transfer. */
			setup_transfer_response(config, config->state->status_code);
			break;
		}
		case USB_SPI_PKT_ID_CMD_START_TRANSFER:
		{
			/* The host started a new USB SPI transfer */
			int write_count = packet->packet_v2.cmd_start.write_count;
			int read_count = packet->packet_v2.cmd_start.read_count;
			...
			if (!config->state->enabled) {
				...
				setup_transfer_response(config, USB_SPI_WRITE_COUNT_INVALID);
			} else if (read_count > USB_SPI_MAX_READ_COUNT) {
				setup_transfer_response(config, USB_SPI_READ_COUNT_INVALID);
			} else {
				config->state->spi_write_ctx.receive_size = write_count;
				config->state->spi_read_ctx.transmit_size = read_count;

				packet->header_size =
						offsetof(usb_spi_command_v2_t, data);
			...
			if (config->state->spi_write_ctx.receive_index ==
					config->state->spi_write_ctx.receive_size) {
				config->state->mode = USB_SPI_TASK_START_SPI;
			}
		...
		case USB_SPI_PKT_ID_CMD_TRANSFER_CONTINUE:
		{
			 ...
			 */
			packet->header_size = offsetof(usb_spi_continue_v2_t, data);
			if (config->state->status_code == USB_SPI_SUCCESS) {
				config->state->status_code = usb_spi_read_usb_packet(
						&config->state->spi_write_ctx, packet);
			}
...
{
	int enabled;
	usb_spi_packet_ctx_t *receive_packet = &config->state->receive_packet;
	usb_spi_packet_ctx_t *transmit_packet = &config->state->transmit_packet;
	...
	if (config->state->mode == USB_SPI_TASK_SEND_CONFIGURATION) {
	...
	if (config->state->mode == USB_SPI_TASK_START_SPI) {
		uint16_t status_code;
...
{
	/*
	 * We need to set both the TX and RX endpoints to NAK to prevent transfers.
	 * The protocol requires responses to follow a command, but the USB host
	 * will request the next packet from the TX endpoint before the USB SPI
	 * has updated the memory in the buffer. By setting it to NAK in the
	 * ISR, it will not perform a transfer until the next packet is ready.
	 *
