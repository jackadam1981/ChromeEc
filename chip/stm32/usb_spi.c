...
{
	const struct spi_device_t *current_device = &spi_devices[config->state->current_spi_device_idx];
	...
	if (status_code == EC_SUCCESS && config->state->eeprom_flags & EEPROM_FLAG_WRITE_ENABLE) {
