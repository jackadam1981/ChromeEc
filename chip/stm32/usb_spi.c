...
{
	int rv = usb_spi_board_transaction_async(spi_device, eeprom_flags,
...
#endif
#ifdef CONFIG_USB_SPI_EEPROM_SUPPORT
	packet->rsp_config.feature_bitmap |= USB_SPI_FEATURE_EEPROM_SUPPORTED;
#endif
...
#ifdef CONFIG_USB_SPI_EEPROM_SUPPORT
static void setup_eeprom_transfer(struct usb_spi_config const *config,
				  struct usb_spi_packet_ctx *packet)
{
	...
	if ((packet->cmd_eeprom_start.flags & EEPROM_FLAG_READ_WRITE_MSK) ==
		...
		if (write_count > USB_SPI_MAX_WRITE_COUNT) {
			...
			usb_spi_setup_transfer(config,
					       opcode_count + addr_count +
						       alt_count + write_count,
					       0);
			packet->header_size =
		...
		if (read_count > USB_SPI_MAX_READ_COUNT) {
			...
			usb_spi_setup_transfer(
				config, opcode_count + addr_count + alt_count,
				read_count);
			packet->header_size =
				offsetof(struct usb_spi_eeprom_command, data);
			config->state->status_code = usb_spi_read_usb_packet(
				      ...
				      struct usb_spi_packet_ctx *packet)
{
	...
	case USB_SPI_PKT_ID_CMD_EEPROM_TRANSFER_START: {
		...
		if (!config->state->enabled) {
			...
			setup_eeprom_transfer(config, packet);
		}
 ...
 */
static uint16_t start_eeprom_transfer(const struct spi_device_t *current_device,
				      struct spi_transaction_state_t *txn)
{
	bool custom_board_driver = current_device->usb_flags &
				   USB_SPI_CUSTOM_SPI_DEVICE;
#ifdef CONFIG_USB_SPI_EEPROM_SUPPORT
	const uint32_t eeprom_flags = txn->eeprom_flags;
#else
	const uint32_t eeprom_flags = 0;
#endif
	...
	if (!custom_board_driver &&
	    (eeprom_flags & EEPROM_FLAGS_REQUIRING_SUPPORT)) {
		/*
 ...
 */
static uint16_t
finish_eeprom_transfer(const struct spi_device_t *current_device,
		       struct spi_transaction_state_t *txn)
{
	bool custom_board_driver = current_device->usb_flags &
				   USB_SPI_CUSTOM_SPI_DEVICE;
	...
	if (eeprom_flags & EEPROM_FLAG_POLL) {
		/*
		...
		while (status_code == EC_SUCCESS) {
			...
			if (custom_board_driver) {
				status_code = usb_spi_board_transaction(
					current_device, 0, &read_status, 1,
					&status_byte, 1);
			} else {
				status_code = spi_transaction(current_device,
							      &read_status, 1,
							      &status_byte, 1);
			}
