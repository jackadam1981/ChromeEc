...
static void
create_spi_device_select_response(struct usb_spi_config const *config,
				  struct usb_spi_packet_ctx *packet)
{
	/* Construct the response packet. */
				      ...
				      struct usb_spi_packet_ctx *packet)
{
	...
	case USB_SPI_PKT_ID_CMD_DEVICE_SELECT: {
		...
		if (flags & USB_SPI_CHIP_SELECT) {
			...
			gpio_set_level(
				spi_devices[current_spi_device_idx].gpio_cs, 0);
		} else {
			/* Set chip select high (adesserted). */
			gpio_set_level(
				spi_devices[current_spi_device_idx].gpio_cs, 1);
		}
...
void usb_spi_deferred(struct usb_spi_config const *config)
{
	...
	if (config->state->mode == USB_SPI_MODE_START_SPI) {
		...
		status_code = (spi_devices[current_spi_device_idx].port &
