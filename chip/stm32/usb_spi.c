				...
				struct usb_spi_packet_ctx_t *packet)
{
	...
	packet->packet_size =
		sizeof(struct usb_spi_response_configuration_v2_t);
}
				...
				struct usb_spi_packet_ctx_t *transmit_packet)
{
	...
	if (config->state->spi_read_ctx.transfer_index == 0) {
		...
		transmit_packet->header_size =
			offsetof(struct usb_spi_response_v2_t, data);
		...
		transmit_packet->header_size =
			offsetof(struct usb_spi_continue_v2_t, data);
				...
				struct usb_spi_packet_ctx_t *packet)
{
	...
	case USB_SPI_PKT_ID_CMD_START_TRANSFER:
	{
		...
		int write_count = packet->cmd_start.write_count;
		int read_count = packet->cmd_start.read_count;
		...
		if (!config->state->enabled) {
			...
			packet->header_size =
				offsetof(struct usb_spi_command_v2_t, data);
...
void usb_spi_deferred(struct usb_spi_config const *config)
{
	...
	if (config->state->mode == USB_SPI_MODE_START_SPI) {
		uint16_t status_code = spi_transaction(SPI_FLASH_DEVICE,
