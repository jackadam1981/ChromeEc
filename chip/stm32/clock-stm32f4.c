...
static int current_sysclk_freq = STM32F4_PLL_CLOCK;
static int current_io_freq = STM32F4_IO_CLOCK;
static int current_timer_freq = STM32F4_TIMER_CLOCK;

int clock_get_cpu_freq(void)
{
	return current_sysclk_freq;
}
...
static void clock_set_osc(enum clock_osc osc)
{
	...
	case OSC_HSI:
		...
		current_sysclk_freq = STM32F4_HSI_CLOCK;
	...
	case OSC_HSE:
		/* new clock settings: no dividers */
		current_sysclk_freq = CONFIG_STM32_CLOCK_HSE_HZ;
	...
	case OSC_PLL:
		/* new clock settings */
		current_sysclk_freq = STM32F4_PLL_CLOCK;
...
{
	/* we cannot go to low power mode as we will be running on the PLL */
	...
	wait_for_ready(&(STM32_RCC_CR), STM32_RCC_CR_HSEON,
		       STM32_RCC_CR_HSERDY);
#endif
...
void __idle(void)
{
	...
	while (1) {
		...
		if (DEEP_SLEEP_ALLOWED &&
			...
			/*uart_enable_wakeup(1); NOT SUPPORTED on STM32F4*/
