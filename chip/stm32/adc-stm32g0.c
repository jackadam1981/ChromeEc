...
static void adc_init(const struct adc_t *adc)
{
	...
	CPRINTF("ADC calibration...\n");
	STM32_ADC_CR = STM32_ADC_CR_ADVREGEN; /* Enable ADC voltage regulator */
	fisk = STM32_ADC_CR;
	CPRINTF("ADC cr=0x%08x\n", fisk);
	/* ADC calibration (done with ADEN = 0) */
	STM32_ADC_CR = STM32_ADC_CR_ADVREGEN | STM32_ADC_CR_ADCAL; /* set ADCAL = 1, ADC off */
	fisk = STM32_ADC_CR;
	CPRINTF("ADC cr=0x%08x\n", fisk);
...
int adc_read_channel(enum adc_channel ch)
{
	...
	CPRINTF("adc_read_channel(%d)\n", ch);
	mutex_lock(&adc_lock);

	CPRINTF("adc_init()\n");
	adc_init(adc);
	CPRINTF("adc_init() done\n");
	...
	CPRINTF("adc_configure()\n");
	adc_configure(adc->channel, adc->sample_rate);
	CPRINTF("adc_configure() done\n");
	...
	CPRINTF("waiting for end of conversion\n");
