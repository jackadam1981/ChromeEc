...
static int protect_ro_at_boot(int enable, int force)
{
	...
	if (pstate.flags != new_flags || force) {
		...
		flash_physical_set_protect_at_boot(RO_BANK_OFFSET,
						   RO_BANK_COUNT,
						   new_flags);
		flash_physical_set_protect_at_boot(PSTATE_BANK,
						   PSTATE_BANK_COUNT,
						   new_flags);
	}
...
static int get_protect_rw_at_boot(void)
{
	/*
	 * Use protect state of first block of RW as RW_PROTECT_AT_BOOT flag.
	 * This way we don't have to use extra storage space and we don't
	 * need to check if write protect register state matches this flag as
	 * we get INCONSISTENT flag when they mismatches.
	 */
	int block = RW_BANK_OFFSET;
	return flash_physical_get_protect_at_boot(block);
}

static int protect_rw_at_boot(int enable, int force)
{
	int old_flags = get_protect_rw_at_boot() ? 1 : 0;
	int new_flags = enable ? 1 : 0;

	/* Update state if necessary */
	if (old_flags != new_flags || force)
...
static int register_need_reset(void)
{
	...
	for (i = RO_BANK_OFFSET; i < RO_BANK_OFFSET + RO_BANK_COUNT; i++)
		if (flash_physical_get_protect_at_boot(i) != ro_at_boot)
			return 1;
	for (i = PSTATE_BANK; i < PSTATE_BANK + PSTATE_BANK_COUNT; i++)
		if (flash_physical_get_protect_at_boot(i) != ro_at_boot)
			return 1;
...
int flash_pre_init(void)
{
	...
	if (prot_flags & EC_FLASH_PROTECT_GPIO_ASSERTED) {
		...
		if ((prot_flags & EC_FLASH_PROTECT_RO_NOW) ||
			 ...
			 * protected. Drop it and reboot. Note that rw_at_boot
			 * flag is lost here.
			 */
