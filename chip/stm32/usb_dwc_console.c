...
static void rx_fifo_handler(void)
{
	...
	if (rx_left) {
		size_t added = QUEUE_ADD_UNITS(&rx_q, ep->out_databuffer + rx_handled,
					       rx_left);
		rx_handled += added;
		rx_left -= added;
	}
	...
	if (!rx_left) {
		...
		CPRINTF("DIDN'T PROCESS ALL left: %d", rx_left);
	}
#else
	int rx_in_fifo;
	size_t added;
...
static void tx_fifo_handler(void)
{
	...
	count = QUEUE_REMOVE_UNITS(&tx_q, ep->in_databuffer, USB_MAX_PACKET_SIZE);
	if (count)
		usb_enable_tx(count);
}
