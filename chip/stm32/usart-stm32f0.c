 ...
 */
static usart_config const * configs[STM32_USARTS_MAX];

static void usart_variant_enable(usart_config const * config)
{
	/*
...
static void usart_variant_disable(usart_config const * config)
{
...
static usart_hw_ops const usart_variant_hw_ops =
{
...
void usart3_4_interrupt(void)
{
	 ...
	 */
	if (configs[2]) usart_interrupt(configs[2]);
	if (configs[3]) usart_interrupt(configs[3]);
}
