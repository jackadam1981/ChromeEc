 ...
 *
 *     data:          write payload up to 62 bytes of data to write,
 ...
 *         0x0005: The I2C bridge is disabled.
...
enum usb_i2c_error {
	...
	USB_I2C_DISABLED            = 0x0005,
	USB_I2C_PORT_INVALID        = 0x0006,
	USB_I2C_UNKNOWN_ERROR       = 0x8000,
};

enum usb_i2c_request {
	USB_I2C_REQ_ENABLE  = 0x0000,
	USB_I2C_REQ_DISABLE = 0x0001,
};
...
struct usb_i2c_state {
	/*
	 * The i2c bridge must be enabled both locally and by the host to allow
	 * access to the i2c device.  The enabled_host flag is set and cleared
	 * by sending USB_I2C_REQ_ENABLE and USB_I2C_REQ_DISABLE to the device
	 ...
	 * different, then one of usb_i2c_board_enable or usb_i2c_board_disable
...
struct usb_i2c_config {
	/*
	 * In RAM state of the USB I2C bridge.
	 */
	struct usb_i2c_state *state;
...
#define USB_I2C_CONFIG(NAME,						\
	...
	struct usb_i2c_state CONCAT2(NAME, _state_) =			\
	{								\
		.enabled_host   = 0,					\
		.enabled_device = 0,					\
		.enabled        = 0,					\
	...
	static int CONCAT2(NAME, _interface_)(usb_uint *rx_buf,		\
					      usb_uint *tx_buf)		\
	{ return usb_i2c_interface(&NAME, rx_buf, tx_buf); }		\
...
/*
 * Set the enable state for the USB-I2C bridge.
 *
 * The bridge must be enabled from both the host and device side
 * before the I2C bus is usable.  This allows the bridge to be
 * available for host tools to use without forcing the device to
 * disconnect or disable whatever else might be using the I2C bus.
 */
 ...
 */
void usb_i2c_board_enable(struct usb_i2c_config const *config);
void usb_i2c_board_disable(struct usb_i2c_config const *config);
