...
static uint32_t write_buffer[ACTUAL_FLASH_WRITE_SIZE / sizeof(uint32_t)];
	...
	iram_flash_write(uint32_t *addr, uint32_t *data)
{
	...
	for (i = 0; i < ACTUAL_FLASH_WRITE_SIZE / sizeof(uint32_t); i++)
		*addr++ = *data++;
...
int flash_physical_write(int offset, int size, const char *data)
{
...
#ifdef CONFIG_64B_WORKAROUND
		if ((size < ACTUAL_FLASH_WRITE_SIZE) || (offset & 64)) {
			if ((size != 64) ||
	...
	for (address = (uint32_t *)(CONFIG_FLASH_BASE + offset) ;
	     size > 0; size -= ACTUAL_FLASH_WRITE_SIZE) {
		...
		address += ACTUAL_FLASH_WRITE_SIZE / sizeof(uint32_t);
		data32 += ACTUAL_FLASH_WRITE_SIZE / sizeof(uint32_t);
		if (STM32_FLASH_SR & 1) {
