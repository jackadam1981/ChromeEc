...
/* FIXME: This code copies a lot from common/flash.c:flash_get_protect,
 * is there a way to reuse that? */
uint32_t flash_physical_get_protect_flags(void)
{
	...
	if (protected_at_boot[0] && protected_at_boot[1])
		flags |= EC_FLASH_PROTECT_ALL_AT_BOOT;

	if (protected_at_boot[1])
		flags |= EC_FLASH_PROTECT_RO_AT_BOOT;

	if (protected_now[0] && protected_now[1])
