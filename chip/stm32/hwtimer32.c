...
void __hw_clock_vsync_source_irq(void)
{
	...
	/* Check if any counter value is captured in channel 1 */
	if (STM32_TIM_SR(TIM_VSYNC) & 0x0002) {
		time_stamp = STM32_TIM_CCR1(TIM_VSYNC);
		CPRINTS("Capture TimeStamp: %d", time_stamp);
	} else {
		CPRINTS("Doesn't capture TimeStamp");
	}
	/* Clear status */
...
void hwtimer_setup_vsync(void)
{
	...
	/*
	 * Timer configuration : Upcounter, counter disabled.
	 */
	STM32_TIM_CR1(TIM_VSYNC) = 0x0000;
	/* No special configuration */
	STM32_TIM_CR2(TIM_VSYNC) = 0x0000;
	STM32_TIM_SMCR(TIM_VSYNC) = 0x0000;

	/* Auto-reload value : free-running counter */
	STM32_TIM_ARR(TIM_VSYNC) = 9999;

	/* Update prescaler to increment every microsecond */
	STM32_TIM_PSC(TIM_VSYNC) = (clock_get_timer_freq() / MSEC) - 1;

	/* Set timer channel 1 as input */
	STM32_TIM_CCMR1(TIM_VSYNC) = 0x0001;

	/* Enable the TIM channel1 and keep the default configuration  */
	STM32_TIM_CCER(TIM_VSYNC) = 0x0001;

	/* Set up the CC1IE interrupt */
	STM32_TIM_DIER(TIM_VSYNC) = 0x0002;

	/* Start counting */
	STM32_TIM_CR1(TIM_VSYNC) = 0x0001;
