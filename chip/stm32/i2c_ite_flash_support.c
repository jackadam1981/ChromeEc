...
static int command_enable_ite_dfu(int argc, char **argv)
{
	...
	/*
	 * Enable output compare 1 (or it's N counter part). Note that if only
	 * OC1N is enabled, then it is not complimented. From datasheet:
	 * "When only OCxN is enabled (CCxE=0, CCxNE=1), it is not complemented"
	 */
	if (ite_dfu_config.use_compliment_timer_channel) {
