...
/* Local buffer for incomplete packets */
...
ccprintf("clock: %d\n", clk);
	...
	if (!tim->cr1 & (1 << 0)) {
		/* Set JTAG TCK to output mode /
		STM32_GPIO_MODER(GPIO_C) &= ~0x3000;
		STM32_GPIO_MODER(GPIO_C) |= 0x1000;

		/ Set high speed /
		STM32_GPIO_OSPEEDR(GPIO_C) |= 0x3000;
*/
...
void ftdi_interrupt(void)
{
	...
	if (bitbang_pos >= bitbang_len) {
		...
		tim->cr1 &= ~(1 << 0);
//		task_disable_irq(IRQ_TIM(TIM_JTAG_TCK));
	}

	gpio_set_level(GPIO_EC_JTAG_TCK, ~gpio_get_level(GPIO_EC_JTAG_TCK));

	switch (bitbang_op) {
		case USB_FTDI_CMD_CLOCK_WRITE_TMS_BIT_PCE:
		case USB_FTDI_CMD_CLOCK_WRITE_TMS_BIT_MCE:
			gpio_set_level(GPIO_EC_JTAG_TMS,
				bitbang_buf[bitbang_pos / 8] & (1 << (bitbang_pos % 8)));
		break;

		case USB_FTDI_CMD_CLOCK_RW_TMS_BIT_PCE_PCE:
		case USB_FTDI_CMD_CLOCK_RW_TMS_BIT_PCE_MCE:
		case USB_FTDI_CMD_CLOCK_RW_TMS_BIT_MCE_PCE:
		case USB_FTDI_CMD_CLOCK_RW_TMS_BIT_MCE_MCE:
			if (bitbang_pos % 2)
				gpio_set_level(GPIO_EC_JTAG_TMS,
					bitbang_buf[bitbang_pos / 8] & (1 << (bitbang_pos % 8)));
			else
				bitbang_buf[bitbang_pos / 8] |= (gpio_get_level(GPIO_EC_JTAG_TDI) << (bitbang_pos % 8));
		break;
		...
		case USB_FTDI_CMD_CLOCK_RW_BIT_LSB_PCE:
		case USB_FTDI_CMD_CLOCK_RW_BYTE_LSB_PCE:
	...
	task_clear_pending_irq(IRQ_TIM(TIM_JTAG_TCK));
}
			...
			switch (msg[pos]) {
				...
				case USB_FTDI_CMD_CLOCK_WRITE_TMS_BIT_MCE:
					if (len - pos > 2) {
						...
						ftdi_bitbang(msg[pos], &(msg[pos + 2]), msg[pos + 1]);
						ftdi_wait();
						pos += 3;
					} else {
						cmd_pos = len - pos;
				...
				case USB_FTDI_CMD_WRITE_HIGH_BYTE:
					if (len - pos > 2) {
						ftdi_set_byte((msg[pos] & 0x2) ? 1 : 0, msg[pos + 1], 8 * msg[pos + 2]);
						pos += 3;
					} else {
						cmd_pos = len - pos;
						cmd_len = 3;
