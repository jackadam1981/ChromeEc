...
#define USB_FTDI_CMD_CLOCK_WRITE_BIT_LSB_MCE	0x1b
...
static void ftdi_send_queue(uint8_t *src, int len)
{
	...
	ASSERT(send_len < USB_FTDI_BUFFER_SIZE);
}
...
static void ftdi_set_clock(uint8_t low, uint8_t high)
{
	...
	if (!tim->cr1 & (1 << 0)) {
		...
		/* Upcounting edge-aligned mode, interrupt from overflow */
		tim->cr1 = (1 << 2);
...
/*
 * Bitbangs an operation. Will block until bitbang completes.
...
/*
 * Handles timer interrupts and executes bitbang commands.
 */
void ftdi_interrupt(void)
{
	...
	STM32_GPIO_BSRR(GPIO_C) |= (bitbang_pos % 2) ?
		(1 << (6 + 16)) : (1 << 6);
...
{
	switch (cmd_buf[0]) {
	/* NOTE: FT2232 bug is not emulated
	 * http://developer.intra2net.com/mailarchive/html/libftdi/
	 * 2009/msg00292.html */
	case USB_FTDI_CMD_CLOCK_RW_TMS_BIT_PCE_MCE:
	case USB_FTDI_CMD_CLOCK_WRITE_BIT_LSB_MCE:
	case USB_FTDI_CMD_CLOCK_WRITE_TMS_BIT_MCE:
		gpio_set_level(GPIO_EC_JTAG_TDI, cmd_buf[2] & (1 << 7));

		ftdi_bitbang(cmd_buf + 2, cmd_buf[1] + 1);
		if (cmd_buf[0] == USB_FTDI_CMD_CLOCK_RW_TMS_BIT_PCE_MCE)
	...
	case USB_FTDI_CMD_CLOCK_RW_BYTE_LSB_MCE:
	case USB_FTDI_CMD_CLOCK_WRITE_BYTE_LSB_MCE:
		ftdi_bitbang(cmd_buf + 3, 8 *
			(1 + ((cmd_buf[2] << 8) | cmd_buf[1])));

		if (cmd_buf[0] == USB_FTDI_CMD_CLOCK_RW_BYTE_LSB_MCE) {
...
void ftdi_task(void)
{
	...
	uint8_t buffer[2] = {0};
	unsigned int pos, len, write_len;
	...
	while (1) {
		...
		if (cmd_pos < cmd_len) {
			...
			if (cmd_len == -1 &&
			   (cmd_buf[0] == USB_FTDI_CMD_CLOCK_RW_BYTE_LSB_MCE ||
			    cmd_buf[0] ==
			    USB_FTDI_CMD_CLOCK_WRITE_BYTE_LSB_MCE)) {
				cmd_len = 3 + 1 +
				((msg[2 - cmd_pos] << 8) | msg[1 - cmd_pos]);
			}

			write_len = MIN(cmd_len - cmd_pos, len);
			memcpy(cmd_buf + cmd_pos,
				ftdi_ep_rx, write_len * sizeof(*msg));
		...
		while (pos < len) {
			...
			case USB_FTDI_CMD_CLOCK_WRITE_TMS_BIT_MCE:
				cmd_len = 3;
				write_len = MIN(cmd_len, len - pos);
				memcpy(cmd_buf, msg + pos,
					write_len * sizeof(*msg));
