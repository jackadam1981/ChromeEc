...
static void uart_interrupt(void)
{
	...
	if (STM32_USART_SR(UARTN_BASE) & STM32_USART_SR_TC) {
		 ...
		 * using STM32_USART_ICR_TCCF field rather than
		 * STM32_USART_SR_TC. Note that both are bit 6 so the effect is
		 * the same but it is a bit misleading. The STM32H7
		 * documentation reads: TC bit is is cleared by software, by
		 * writing 1 to the TCCF in the USART_ICR register or by a write
		 * to the USART_TDR register.
		 */
