...
int chip_i2c_xfer(const int port, const uint16_t peripheral_addr_flags,
		  const uint8_t *out, int out_bytes,
		  uint8_t *in, int in_bytes, int flags)
{
	int addr_8bit = I2C_GET_ADDR(peripheral_addr_flags) << 1;
	int rv = EC_SUCCESS;
	int i;
	...
	if (port == CONFIG_I2C_SCL_GATE_PORT &&
	    peripheral_addr_flags == CONFIG_I2C_SCL_GATE_ADDR_FLAGS)
