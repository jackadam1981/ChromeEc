...
#if defined(CONFIG_HOSTCMD_I2C_SLAVE_ADDR__8b) && \
		  ...
		  uint8_t *in, int in_bytes, int flags)
{
	...
	if (port == CONFIG_I2C_SCL_GATE_PORT &&
	    slave_addr__8b == CONFIG_I2C_SCL_GATE_ADDR__8b)
	...
	if (in_bytes) {
		...
		STM32_I2C_CR2(port) = ((in_bytes & 0xFF) << 16)
			| STM32_I2C_CR2_RD_WRN | slave_addr__8b
...
#endif
	STM32_I2C_OAR1(I2C_PORT_EC) = 0x8000 | CONFIG_HOSTCMD_I2C_SLAVE_ADDR__8b;
#ifdef TCPCI_I2C_SLAVE
	/*
	 * Configure TCPC address with OA2[1] masked so that we respond
	 * to CONFIG_TCPC_I2C_BASE_ADDR and CONFIG_TCPC_I2C_BASE_ADDR + 2.
	 */
	STM32_I2C_OAR2(I2C_PORT_EC) = 0x8100 | CONFIG_TCPC_I2C_BASE_ADDR__8b;
#endif
