...
/* 4Mhz Prescaler frequency */
#define PRESC_FREQ 4000000

static void i2c_set_freq_port(const struct i2c_port_t *p)
{
	int port = p->port;
	int freq = clock_get_freq();
	int presc = (freq / PRESC_FREQ) - 1; /* 4Mhz Prescaler frequency */
	uint8_t scl = (PRESC_FREQ / (2 * MSEC * p->kbps)) - 1;
	uint8_t hold = 3 * scl;
	...
	/* Set clock frequency */
	STM32_I2C_TIMINGR(port) = ((presc & 0xF) << 28) | (hold << 16)
				| (hold << 20) |  (scl << 8) | scl;
	STM32_I2C_TIMINGR(port) = 0xB0420F13; /* valid 100kbps */
