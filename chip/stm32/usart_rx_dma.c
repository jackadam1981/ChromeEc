...
typedef size_t (*add_units_t)(struct usart_config const *config,
...
static size_t queue_add(struct usart_config const *config,
			const void *src, size_t count)
{
	return queue_add_units(config->producer.queue, src, count);
}
