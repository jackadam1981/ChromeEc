...
static void clock_flash_latency(enum freq axi_freq, enum voltage_scale vos)
{
	...
	} else if (axi_freq == FREQ_140MHZ && vos == VOLTAGE_SCALE0) {
		target_acr = STM32_FLASH_ACR_WRHIGHFREQ(1) |
			     STM32_FLASH_ACR_LATENCY(3);
 ...
 * @return The bus clock speed selected and confirgured
 */
static enum freq clock_peripheral_configure(enum freq sysclk) {
	...
	case FREQ_200MHZ:
		...
		return FREQ_200MHZ;
	case FREQ_280MHZ:
		/* Divide all downstream buses by 2 --> Bus matrix 140MHz */
...
#		ifdef  CHIP_VARIANT_STM32H7A3_B3
		 ...
		 *          = 64 / 4 * 25 / 2
		 *          = 280 Mhz
		 * System clock = 280 Mhz
		 *  HPRE = /2  => AHB/Timer clock = 140 Mhz
		 */
		target_sysclk_freq = FREQ_280MHZ;
		target_voltage_scale = VOLTAGE_SCALE0;
#		endif /* CHIP_VARIANT_STM32H7A3_B3 */
