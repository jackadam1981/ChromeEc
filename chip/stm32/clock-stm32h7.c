...
static void clock_set_osc(enum clock_osc osc)
{
	...
	case OSC_PLL:
		/* Configure PLL1 */
		/* use 64Mhz HSI as input, feed VCOH with 64 / DIVM = 64/4 = 16Mhz */
		STM32_RCC_PLLCKSELR = STM32_RCC_PLLCKSEL_PLLSRC_HSI |
				      STM32_RCC_PLLCKSEL_DIVM1(4);
		/* in integer mode, wide range VCO with 16Mhz input,
		 * divP enabled.
		 */
		STM32_RCC_PLLCFGR = (3 << 2) | (1 << 16);
		STM32_RCC_PLL1DIVR = STM32_RCC_PLLDIVR_DIVP(2) | STM32_RCC_PLLDIVR_DIVN(16);
		/* turn on PLL1 and wait that it's ready */
		clock_enable_osc(OSC_PLL);
		// freq = STM32_HSI_CLOCK / 4 * 16/*DIVN*/ / 2/*DIVP*/;
		/* but lie to the world, they are clocked on something else */
		freq = STM32_HSI_CLOCK;

		/* Adjust flash latency */
		val = STM32_FLASH_ACR_WRHIGHFREQ_185MHZ |
				(1 << STM32_FLASH_ACR_LATENCY_SHIFT);
		STM32_FLASH_ACR = val;
