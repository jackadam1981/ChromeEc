...
#define RW_MEM_START	(CONFIG_PROGRAM_MEMORY_BASE + CONFIG_RW_MEM_OFF)

/* Registers to validate the backup memory region. */
#define BACKUP_VALUE_MASK				(0x00FF)
#define BACKUP_VALID_MASK				(0xFF00)
#define BACKUP_VALID_CHECK				(0xAA00)

#define DFU_BOOTMANAGER_VALUE_CLEAR 	(0)
#define DFU_BOOTMANAGER_VALUE_DFU 		(UINT8_MAX)

#ifdef CONFIG_DFU_BOOTMANAGER_MAX_REBOOT
BUILD_ASSERT(CONFIG_DFU_BOOTMANAGER_MAX_REBOOT <= UINT8_MAX);
#endif /* CONFIG_DFU_BOOTMANAGER_MAX_REBOOT */
...
void dfu_bootmanager_backup_write(uint8_t value) {
	uint16_t data = BACKUP_VALID_CHECK | value;
...
int dfu_bootmanager_backup_read(uint8_t* value) {
	...
	*value = (uint8_t)(data & BACKUP_VALUE_MASK);
	if ((data & BACKUP_VALID_MASK) != BACKUP_VALID_CHECK) {
...
int dfu_bootmanager_enter_dfu() {
	dfu_bootmanager_backup_write(DFU_BOOTMANAGER_VALUE_DFU);
	system_reset(0);
	return 0;
}

void dfu_bootmanager_clear_reboot_counter() {
 ...
 *
 * The normal DFU programming sequence programming will still work, but
 * by splitting into the following sequence we can protect against additional
 * failures.
 *
...
static bool rw_is_empty(void) {
	const uint64_t *start = (const uint64_t *)(RW_MEM_START);
...
static bool backup_boot_checks(void) {
	...
	if (dfu_bootmanager_backup_read(&value)) {
		/* Value stored is not valid, set it valid value. */
		dfu_bootmanager_backup_write(DFU_BOOTMANAGER_VALUE_CLEAR);
		return false;
	}
...
{
	void (*address)(void) =
		(void (*)(void)) (*((uint32_t *) (RW_MEM_START + 4)));
    address();
}

static void jump_to_dfu(void) {
	void (*address)(void) =
		(void (*)(void)) (*((uint32_t *) (STM32_DFU_BASE + 4)));
