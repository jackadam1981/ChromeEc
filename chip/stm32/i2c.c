...
static int i2c_write_raw_slave(int port, void *buf, int len)
{
	...
	dma_enable_interrupts(DMAC_I2C_TX, DMA_TCIE);
	...
	dma_disable_interrupts(DMAC_I2C_TX, DMA_TCIE);
	STM32_I2C_CR2(port) &= ~(1 << 11);
	...
	int size)
{
	...
	if (size > 1) {
		...
		dma_enable_interrupts(DMAC_I2C_RX, DMA_TCIE);
		...
		dma_disable(DMAC_I2C_RX);
		dma_disable_interrupts(DMAC_I2C_RX, DMA_TCIE);
		STM32_I2C_CR2(port) &= ~CR2_DMAEN;
		disable_ack(port);
