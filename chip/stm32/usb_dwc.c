...
void no_print_later(const char *fmt, int a0, int a1, int a2, int a3, int a4)
{
	int next;
...
#if 0
/* Flush fifo or fifos */
static void flush_fifo(int num)
{
	/* 0x10 indiates all fifos. Otherwise it is the fifo to flush. */
	GR_USB_GRSTCTL = GRSTCTL_TXFNUM(num) | GRSTCTL_TXFFLSH;
...
int send_in_packet(uint32_t ep_num)
{
	...
	if (ep->in_packets == 0) {
		report_error(ep_num);
		print_later("send_in_packet(ep = %d): NO PACKET TO SEND", ep_num, 0, 0, 0, 0);
		return -1;
	}

	print_later("send_in_packet(ep %d, pkt %d, len %d, diepctl %x)",
		ep_num, ep->in_packets, ep->in_pending, GR_USB_DIEPCTL(0), 0);
...
static void expect_data_phase_in(enum table_case tc)
{
	...
	if (tc == TABLE_CASE_SETUP)
		GR_USB_DIEPCTL(0) |= DXEPCTL_USBACTEP | DXEPCTL_CNAK | DXEPCTL_EPENA;
	else
				      ...
				      struct usb_setup_packet *req)
{
	...
	case USB_REQ_GET_DESCRIPTOR: {
		...
		switch (type) {
		case USB_DT_DEVICE:
			print_later("REQ: dt_device", 0, 0, 0, 0, 0);
			data = &dev_desc;
			len = sizeof(dev_desc);
			break;
		case USB_DT_CONFIGURATION:
			print_later("REQ: dt_config", 0, 0, 0, 0, 0);
			data = __usb_desc;
			len = USB_DESC_SIZE;
...
#ifdef CONFIG_USB_BOS
		case USB_DT_BOS:
			print_later("REQ: dt_bos", 0, 0, 0, 0, 0);
			data = bos_ctx.descp;
			len = bos_ctx.size;
			break;
#endif
		case USB_DT_STRING:
			print_later("REQ: dt_str idx %d", idx, 0, 0, 0, 0);
		...
		case USB_DT_DEVICE_QUALIFIER:
			print_later("REQ: dt_qualifier", 0, 0, 0, 0, 0);
	...
	case USB_REQ_GET_STATUS: {
		...
		print_later("REQ: get_status", 0, 0, 0, 0, 0);
		data = &zero;
		len = sizeof(zero);
		break;
	}
	case USB_REQ_GET_CONFIGURATION:
		print_later("REQ: get_cfg", 0, 0, 0, 0, 0);
					   ...
					   struct usb_setup_packet *req)
{
	...
	case USB_REQ_SET_ADDRESS:
		...
		print_later("REQ: SETAD 0x%02x (%d)", set_addr, set_addr, 0, 0, 0);
		device_state = DS_ADDRESS;
		break;

	case USB_REQ_SET_CONFIGURATION:
		print_later("REQ: SETCFG 0x%x", req->wValue, 0, 0, 0, 0);
		switch (req->wValue) {
		case 0:
...
static void ep0_interrupt(uint32_t intr_on_out, uint32_t intr_on_in)
{
	...
	print_later("doepint%c 0x%08x diepint%c 0x%08x what %s",
		    intr_on_out ? '!' : '_', doepint,
		    intr_on_in ? '!' : '_', diepint,
		    (int)wat[what_am_i_doing]);
	if (intr_on_out)
		print_later("intr_out doepint 0x%08x, doepctl 0x%x diepctl 0x%x, wat: %s", 
			doepint, GR_USB_DOEPCTL(0), GR_USB_DIEPCTL(0), (int)wat[what_am_i_doing], 0);
	if (intr_on_in)
		print_later("intr_in diepint 0x%08x, doepctl 0x%x diepctl 0x%x, wat: %s", 
			diepint, GR_USB_DOEPCTL(0), GR_USB_DIEPCTL(0), (int)wat[what_am_i_doing], 0);
#if 0
	CPRINTS("doepint%c 0x%08x diepint%c 0x%08x what %d",
		    intr_on_out ? '!' : '_', doepint,
		    intr_on_in ? '!' : '_', diepint,
		    what_am_i_doing);
#endif		
	...
	print_later("cur_out_idx %d flags 0x%08x case=%c SR=%d",
		    ...
		    "0ABCDE67"[tc], !!sr, 0);
	CPRINTS("cur_out_idx %d flags 0x%08x case=%c SR=%d",
		    cur_out_idx, cur_out_desc->flags,
		    "0ABCDE67"[tc], !!sr);
#endif

	/*print_later("doing: %d, cmpl %d, setup %d", what_am_i_doing, !!complete, !!setup, 0, 0);*/
	...
	case WAITING_FOR_SETUP_PACKET:
		if (out_setup) {
			...
			print_later("What is going on!...",
				    0, 0, 0, 0, 0);
			CPRINTS("SETUP: What is going on!?");
#if 0
			stall_both_fifos();
#endif
		}
	...
	case DATA_STAGE_IN:
		if (intr_on_in && in_complete) {
			...
			if (ep->in_packets > 0) {
				/* Send another packet. */
				print_later("Send IN packet %d",
				    ep->in_packets, 0, 0, 0, 0);
				send_in_packet(0);
				expect_data_phase_in(tc);
			} else {
				print_later("IN is complete.",
				    0, 0, 0, 0, 0);
			}
		}
		...
		if (out_setup) {
			print_later("WTF is going on. IN has been detected...", 0, 0, 0, 0, 0);
	...
	case NO_DATA_STAGE:
		...
		if (out_setup) {
			print_later("WHAAAT IN has been detected...", 0, 0, 0, 0, 0);
#if 0
			/* Let the IN proceed */
			GR_USB_DIEPCTL(0) |= DXEPCTL_CNAK | DXEPCTL_EPENA;
...
#if 0
		if (tc == TABLE_CASE_COMPLETE || tc == TABLE_CASE_WTF) {
			if (setup) {
				/* We expected an IN, but got a Setup. */
				print_later("WTFFF Early Setup", 0, 0, 0, 0, 0);
				handle_setup(tc);
				break;
			}
		}
		...
		print_later("I AM ERROR Status phase complete. Maybe.",
			    0, 0, 0, 0, 0);
		expect_setup_packet();
		break;
	}
...
void usb_interrupt(void)
{
	...
	/*CPRINTS("usb_interrupt: GINTSTS 0x%08x", status);*/
	/*showbits(status);*/
#if 0
	...
	if (oepint || iepint) {
		...
		print_later("  oepint%c iepint%c daint 0x%08x",
			    oepint ? '!' : '_', iepint ? '!' : '_',
			    daint, 0, 0);
		/*CPRINTS("oepint%c iepint%c daint 0x%08x",oepint ? '!' : '_', iepint ? '!' : '_',daint);*/
#endif
...
DECLARE_IRQ(DWC_USB_IRQ, usb_interrupt, 1);
...
{
#ifdef CONFIG_DWC_ULPI
	GR_USB_GCCFG &= ~GCCFG_PWRDWN;
	GR_USB_GUSBCFG &= ~(GUSBCFG_TSDPS | GUSBCFG_ULPIFSLS | GUSBCFG_PHYSEL);
	GR_USB_GUSBCFG &= ~(GUSBCFG_ULPIEVBUSD | GUSBCFG_ULPIEVBUSI);
	/* No suspend */
	GR_USB_GUSBCFG |= GUSBCFG_ULPICSM | GUSBCFG_ULPIAR;
	
#ifdef CONFIG_DWC_EXTERNAL_VBUS
	GR_USB_GUSBCFG |= GUSBCFG_ULPIEVBUSD;
#endif
	usb_softreset();
#else
...
void usb_init(void)
{
	...
	GR_USB_PCGCCTL = 0;
#ifdef CONFIG_DWC_ULPI
	/*GR_USB_DCFG = (GR_USB_DCFG & ~GC_USB_DCFG_DEVSPD_MASK) | DCFG_DEVSPD_HSULPI;*/
	GR_USB_DCFG = (GR_USB_DCFG & ~GC_USB_DCFG_DEVSPD_MASK) | DCFG_DEVSPD_FSULPI;
#else
	...
	GR_USB_DTHRCTL |= DTHRCTL_RXTHREN | DTHRCTL_ISOTHREN | DTHRCTL_NONISOTHREN;
	i = GR_USB_DTHRCTL;
#endif
	GR_USB_GINTSTS = 0xFFFFFFFF;

	GR_USB_GAHBCFG |= GAHBCFG_GLB_INTR_EN | GAHBCFG_TXFELVL | GAHBCFG_PTXFELVL;
#ifndef CONFIG_DWC_DMA
	POOOOOO;
...
#endif
#ifdef CONFIG_DWC_VBUS_SENSE
	GR_USB_GINTMSK |= GINTMSK(SESSREQINT) | GINTMSK(OTGINT);
#endif
