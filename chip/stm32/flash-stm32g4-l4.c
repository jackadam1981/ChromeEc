...
static void optb_get_wrp(enum wrp_region region, struct wrp_info *wrp)
{
	...
	if (region == WRP_RO) {
		 ...
		 * then WRP2AR must also be checked  to determine the full
		 * range of flash page indices being protected.
		 */
		wrp->start = FLASH_WRP_START(STM32_FLASH_WRP1BR);
		wrp->end = FLASH_WRP_END(STM32_FLASH_WRP1BR);
		wrp->enable = wrp->end >= wrp->start;

		if (IS_ENABLED(STM32_FLASH_DBANK_MODE)) {
			int start = FLASH_WRP_START(STM32_FLASH_WRP2AR);
			int end = FLASH_WRP_END(STM32_FLASH_WRP2AR);

			/*
			 * If WRP2AR protection is enabled, then need to adjust
			 * either the start, end, or both indices.
			 */
			if (end >= start) {
				if (wrp->enable) {
					/* WRP1BR is active, adjust end */
					wrp->end += end;
				} else {
					/*
					 * WRP1BR is not active, so RW
					 * protection, if enabled, is fully
					 * controlled by WRP2AR.
					 */
					wrp->start = start;
					wrp->end = end;
					wrp->enable = 1;
	...
	if (region == WRP_RO) {
		...
		if (IS_ENABLED(STM32_FLASH_DBANK_MODE)) {
			if (wrp->enable) {
				...
				/*
				 * If the 1st RW flash page is in the 1st half
				 * of memory, then at least one block will be
				 * protected by WRP1BR. If the end flash page
				 * is in the 2nd half of memory, then cap the
				 * end for WRP1BR to its max value. Otherwise,
				 * can use end passed in directly.
				 */
				...
				/*
				 * If the last RW flash page is in the 2nd half
				 * of memory, then at least one block will be
				 * protected by WRP2AR. If the start flash page
				 * is in the 2nd half of memory, can use start
				 * directly. Otherwise, start needs to be set
				 * to 0 here.
				 */
				if (end > FLASH_WRP_MASK) {
					rw_end = end & FLASH_WRP_MASK;
					STM32_FLASH_WRP2AR =
						FLASH_WRP_RANGE(0, rw_end);
				}
			} else {
				/*
				 * RW write protect is being disabled. Set
				 * both WRP1BR and WRP2AR to default start >
				 * end not protected
				 * state.
				 */
				STM32_FLASH_WRP1BR =
					FLASH_WRP_RANGE(start, end);
				STM32_FLASH_WRP2AR =
					FLASH_WRP_RANGE(start, end);
			}
		} else {
			/* Single bank case, WRP1BR can cover the full memory */
