...
static void usart_rx_init(struct usart_config const *config)
{
	...
	STM32_USART_CR1(base) |= STM32_USART_CR1_RXNEIE | STM32_USART_CR1_RE |
				 STM32_USART_CR1_RTOIE;
	STM32_USART_RTOR(base) = 20;
	STM32_USART_CR2(base) |= STM32_USART_CR2_RTOEN;

	/*
	 * Invoking flush() once signals that the consumer is allowed to buffer
	 * characters indefinitely, and is only strictly required to empty the
	 * queue when flush() is invoked again in the future.
	 */
