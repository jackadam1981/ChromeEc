...
{
	size_t  i;
	uint16_t usb_ram_mask = CONFIG_USB_RAM_SIZE - 1;
	uint8_t count = btable_ep[config->endpoint].rx_count & usb_ram_mask;
				 ...
				 uint8_t count)
{
	...
	if (count > USB_MAX_PACKET_SIZE) {
		CPRINTS("ERROR: usb_i2c_write_packet count %d > 64",
			(int)count);
		return;
	}
	/*
...
{
	if (config->state->request_enabled ^ config->state->enabled)
		config->state->enabled = config->state->request_enabled;
	...
	if (!rx_valid(config)) {
		...
		int port            = (config->buffer[0] >> 0) & 0xff;
		...
		} else if (write_count > USB_I2C_MAX_WRITE_COUNT ||
			   write_count != (count - 4)) {
...
void usb_i2c_enable(struct usb_i2c_config const *config, int enabled)
{
	config->state->request_enabled = enabled;
