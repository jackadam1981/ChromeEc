...
void usb_i2c_deferred(struct usb_i2c_config const *config)
{
	...
	if (enabled ^ config->state->enabled) {
		if (enabled)
			usb_i2c_board_enable(config);
		else
			usb_i2c_board_disable(config);
...
int usb_i2c_interface(struct usb_i2c_config const *config,
		      usb_uint *rx_buf,
		      usb_uint *tx_buf)
{
	...
	case USB_I2C_REQ_ENABLE:
		config->state->enabled_host = 1;
		break;

	case USB_I2C_REQ_DISABLE:
