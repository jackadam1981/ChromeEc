...
enum {
	...
	SPI_MSG_HEADER_BYTE	= 0xec,

	SPI_MSG_HEADER_LEN	= 3,
 ...
 */
static uint8_t out_msg[EC_HOST_PARAM_SIZE_MIN + SPI_MSG_PROTO_LEN];
static uint8_t in_msg[EC_HOST_PARAM_SIZE_MIN + SPI_MSG_PROTO_LEN];
		  ...
		  enum ec_status status, char *msg_ptr, int msg_len)
{
	 ...
	 * Odd corner case:
	 *
	 * Put something in the holding register. This stops the dma
	 * from immediately being triggered, maybe a only a single
	 * clock cycle before the next data value is moved into the
	 * shift register. In this case we will increment the DMA
	 * pointer but the data will be lost because it is written too
	 * late.
	 */
	REG16(&spi->data) = 0xfe;
 ...
 */
static void setup_for_transaction(int port)
{
	int dmac;
...
static void spi_send_response(struct host_cmd_handler_args *args)
{
	...
	spi = (struct spi_ctlr *)stm32_spi_addr(STM32_SPI1_PORT, 0);
	...
	/* Transmit the reply */
	txdma = dma_get_channel(DMA_CHANNEL_FOR_SPI_TX(STM32_SPI1_PORT));
...
void spi_event(enum gpio_signal signal)
{
	...
	if (REG16(nss_reg) & nss_mask) {
		setup_for_transaction(port);
		return;
	}

	active = 1;
	rxdma = dma_get_channel(DMA_CHANNEL_FOR_SPI_RX(STM32_SPI1_PORT));
...
static int spi_init(void)
{
	...
	STM32_SPI_CR2(port) = CR2_RXDMAEN | CR2_TXDMAEN;
	setup_for_transaction(port);
