...
void spi_event(enum gpio_signal signal)
{
	...
	if (wait_for_bytes(rxdma, 3+(autorxoffset*rxoffset), nss_reg, nss_mask))
	...
	if(autorxoffset) {
		...
		while(in_msg[rxoffset]==0x00) {
			rxoffset+=1;
			if (wait_for_bytes(rxdma, 1, nss_reg, nss_mask))
				goto spi_event_error;
		}
	}
...
static int command_rxoffset(int argc, char **argv)
{
	...
	rxoffset = strtoi(argv[1], &e, 0);

	ccprintf("rxoffset is now=%d\n", rxoffset);
	autorxoffset=0;
	ccprintf("autorxoffset is now=%d\n", autorxoffset);
...
static int command_autorxoffset(int argc, char **argv)
{
	parse_bool(argv[1], &autorxoffset);

	ccprintf("autorxoffset is now=%d\n", autorxoffset);
	if(!autorxoffset) rxoffset=0;
	ccprintf("rxoffset is now=%d\n", rxoffset);

	return EC_SUCCESS;
}
DECLARE_CONSOLE_COMMAND(autorxoffset, command_autorxoffset,
			"",
			"autorxoffset",
			NULL);
