...
static char otp_serial[CONFIG_SERIALNO_LEN];
 ...
 * @param data          Destination buffer for data, 32 bits aligned.
 */
static int otp_read(uint8_t block, int size, char *data)
{
	int i;
	...
	for (i = 0; i < size; i++)
		word[i] = REG32(STM32_OTP_BLOCK_DATA(block, i));
	return EC_SUCCESS;
}
...
const char *otp_read_serial(void)
{
	...
	ret = otp_read(OTP_SERIAL_BLOCK, OTP_SERIAL_BLOCK_SIZE,
		       otp_serial);
	if (ret == EC_SUCCESS)
		return otp_serial;
	else
