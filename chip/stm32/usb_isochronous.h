 ...
 *		config, header, sizeof(header), offset, &buffer_id, 0);
 *	if (ret < 0) goto FAILED;
 *	offset += ret;
 *      // write body
 *	ret = usb_isochronous_write_buffer(
 *		config, body, sizeof(body), offset, &buffer_id, 1);
 *
 * On the first invocation, on success, `ret` will be number of bytes been
 * written, and `buffer_id` will be 0 or 1, depends on which buffer we are
 * writing.  And commit=0 means there are pending data, so buffer count won't be
 * set yet.
 *
 * On the second invocation, since buffer_id is not -1, we will abort if
 * hardware has switch to this buffer (it means we spent too much time on
