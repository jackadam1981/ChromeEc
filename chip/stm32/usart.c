			 ...
			 size_t count)
{
	...
	return fifo_read(&config->rx, buffer, count);
}
			  ...
			  size_t count)
{
	...
	size_t               wrote  = fifo_write(&config->tx, buffer, count);

	/*
	 * Trigger the USART interrupt.  This causes the USART interrupt
	 * handler to start fetching from the TX FIFO if it wasn't already,
...
static int usart_resume(out_stream const * stream)
{
	...
	/*
	 * Trigger the USART interrupt.  This causes the USART interrupt
	 * handler to start fetching from the TX FIFO.  If the FIFO is empty
...
static void usart_interrupt_tx(usart_config const * config)
{
	...
	if (!config->state->paused && fifo_read(&config->tx, &byte, 1))
	{
	...
	{
		/*
		 * The TX FIFO is empty, or we have been asked to pause.  In
		 ...
		 * the FIFO is empty, the transmit interrupt will once again be
...
static void usart_interrupt_rx(usart_config const * config)
{
	...
	fifo_write(&config->rx, &byte, 1);

	/*
	 * Wake up whoever is listening on the other end of the FIFO.  The
	 * fifo_write call above may have failed due to a full FIFO, but it
	 * doesn't really matter to the ready callback because there will be
	 * something in the FIFO to consume either way.
	 */
