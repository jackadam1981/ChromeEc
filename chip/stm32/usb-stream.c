...
{
	size_t c = queue_count(config->consumer.queue);
	if ((config->state->flags &
		 ...
		 * flushed the data in queue, and there is not enough to fill a
		 * USB packet, so wait for either getting more data or an
		 * explicit flush().
		 */
