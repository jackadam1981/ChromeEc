 ...
 * routines need to be employted.  Furthermore, reading from and writing to the
 ...
 * the queue.
 */
static size_t rx_read(usb_stream_config const * config)
{
	size_t count = btable_ep[config->rx_ep].rx_count & 0x3ff;
...
static size_t tx_write(usb_stream_config const * config)
{
	...
	btable_ep[config->tx_ep].tx_count = count;

	return count;
}

static size_t usb_read(in_stream const * stream,
		       uint8_t * buffer,
		       size_t count)
{
	usb_stream_config const * config =
		DOWNCAST(stream, usb_stream_config, in);
	...
	if (config->state->rx_waiting && rx_read(config))
	{
		...
		STM32_TOGGLE_EP(config->rx_ep, EP_RX_MASK, EP_RX_VALID, 0);
...
static int tx_valid(usb_stream_config const * config)
{
	return (STM32_USB_EP(config->tx_ep) & EP_TX_MASK) == EP_TX_VALID;
}

static size_t usb_write(out_stream const * stream,
			uint8_t const * buffer,
			size_t count)
{
	usb_stream_config const * config =
		DOWNCAST(stream, usb_stream_config, out);
	...
	if (!tx_valid(config) && tx_write(config))
		STM32_TOGGLE_EP(config->tx_ep, EP_TX_MASK, EP_TX_VALID, 0);

	return wrote;
}

static void usb_flush(out_stream const * stream)
{
	usb_stream_config const * config =
		DOWNCAST(stream, usb_stream_config, out);
...
in_stream_ops const usb_stream_in_stream_ops =
{
	.read = usb_read,
};

out_stream_ops const usb_stream_out_stream_ops =
{
	.write = usb_write,
	.flush = usb_flush,
};
...
void usb_stream_tx(usb_stream_config const * config)
{
	if (tx_write(config))
		...
		STM32_TOGGLE_EP(config->tx_ep, 0, 0, 0);
...
void usb_stream_rx(usb_stream_config const * config)
{
	...
	{
		/*
		 * There is not enough space in the RX queue to recieve this
		...
		STM32_TOGGLE_EP(config->rx_ep, 0, 0, 0);
	}
...
void usb_stream_tx_reset(usb_stream_config const * config)
{
	...
	btable_ep[ep].tx_addr  = usb_sram_addr(config->tx_ram);
	btable_ep[ep].tx_count = 0;
	btable_ep[ep].rx_count = 0;
...
void usb_stream_rx_reset(usb_stream_config const * config)
{
	...
	btable_ep[ep].rx_addr  = usb_sram_addr(config->rx_ram);
	btable_ep[ep].tx_count = 0;
	btable_ep[ep].rx_count = (0x8000 |
