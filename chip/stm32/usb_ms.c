...
const struct usb_endpoint_descriptor USB_EP_DESC(USB_IFACE_MS, USB_EP_MS_TX) = {
	...
	.bEndpointAddress = 0x80 | USB_EP_MS_TX,
...
usb_uint ep_buf_tx[USB_MS_PACKET_SIZE] __usb_ram;
usb_uint ep_buf_rx[USB_MS_PACKET_SIZE] __usb_ram;

/* Current state of USB state machine */
static int ms_dir;
static int ms_xfer_len;
static int ms_tag;
static int scsi_rv;
static int scsi_xfer_len;

static void ms_tx_reset(void)
{
	btable_ep[USB_EP_MS_TX].tx_addr = usb_sram_addr(ep_buf_tx);
...
{
	btable_ep[USB_EP_MS_RX].rx_addr = usb_sram_addr(ep_buf_rx);
...
{
	struct usb_ms_csw *resp = (struct usb_ms_csw *) ep_buf_tx;

	/* construct CSW response */
	resp->dCSWSignature = UBS_MS_CSW_SIGNATURE;
	resp->dCSWTag = ms_tag;
	resp->dCSWDataResidue = (ms_xfer_len > scsi_xfer_len) ?
				(ms_xfer_len - scsi_xfer_len) :
				(scsi_xfer_len - ms_xfer_len);
	if (scsi_rv != SCSI_SENSE_HARDWARE_ERROR)
		resp->bCSWStatus = (scsi_rv == SCSI_SENSE_NO_SENSE) ?
				USB_MS_CSW_CMD_PASSED :
				USB_MS_CSW_CMD_FAILED;
	else {
		ms_state = USB_MS_STATE_PHASE_ERROR;
		resp->bCSWStatus = USB_MS_CSW_CMD_PHASE_ERR;
	}
...
static void ms_iface_request(usb_uint *ep0_buf_rx, usb_uint *ep0_buf_tx)
{
	...
	if ((req[0] & (USB_DIR_OUT | USB_RECIP_INTERFACE | USB_TYPE_CLASS)) ==
		...
		case USB_MS_REQ_GET_MAX_LUN:
			if (req[1] == 0 && req[2] == USB_IFACE_MS &&
			    req[3] == 1) {
				ep0_buf_tx[0] = max_lun;
				btable_ep[0].tx_count = sizeof(max_lun);
...
void ms_task(void)
{
	struct usb_ms_cbw *req = (struct usb_ms_cbw *) ep_buf_rx;
	uint8_t evt;
	...
	while (1) {
		...
		case USB_MS_STATE_IDLE:
			...
			if (evt & USB_MS_EVENT_RX) {
				...
				ms_tag = req->dCBWTag;
				ms_xfer_len = req->dCBWDataTransferLength;
				ms_dir = req->bmCBWFlags;
				scsi_xfer_len = 0;

				/* parse message and get next state */
				scsi_rv = scsi_parse(req->CBWCB,
					req->bCBWCBLength);
