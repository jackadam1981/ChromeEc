...
static void set_em(struct usb_gpio_config const *config,
...
{
	static const uint32_t seq[] = {0x02, 0x10, 0x04, 0x08};
	static int now;
	int next;
	...
	next = (now + 1) % 4;
	set_em(config, seq[next], seq[now]);
	now = next;
}
...
void usb_gpio_rx(struct usb_gpio_config const *config)
{
	...
	/* To resume ticking, send a do-nothing command */
	overridden = set_mask + clear_mask;

	if ((btable_ep[config->endpoint].rx_count & 0x3ff) ==
	    USB_GPIO_RX_PACKET_SIZE)
		set_em(config, set_mask, clear_mask);
