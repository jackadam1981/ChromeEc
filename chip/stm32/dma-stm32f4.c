...
void dma_dump(enum dma_channel stream)
{
	...
	CPRINTF("stream %d, isr=%x, ifcr=%x\n",
		stream,
		(STM32_DMA_ISR(dma, stream) >> ((stream % 4) * 6)) & 0x1f,
		(STM32_DMA_IFCR(dma, stream) >> ((stream % 4) * 6)) & 0x1f);
}
...
{
	stm32_dma_regs_t *dma = dma_get_ctrl(stream);
	const uint32_t mask = STM32_DMA_ISR_TCIF(stream);
	timestamp_t deadline;

	deadline.val = get_time().val + DMA_TRANSFER_TIMEOUT_US;
	while ((STM32_DMA_ISR(dma, stream) & mask) != mask) {
...
void dma_clear_isr(enum dma_channel stream)
{
	...
	STM32_DMA_IFCR(dma, stream) |= STM32_DMA_ISR_ALL(stream);
}
