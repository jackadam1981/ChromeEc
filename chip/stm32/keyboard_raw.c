 ...
 * input and output entries in the board's gpio_list[].Each set of inputs
 * (ksi pins) or outputs(kso pins) must be listed in consecutive, increasing
 * order so that scan loops can iterate beginning at KB_IN00 or KB_OUT00 for
 * however many GPIOs are utilized (KEYBOARD_KSI_COUNT or KEYBOARD_KSO_COUNT).
 */
...
static void set_irq_mask(void)
{
	...
	for (i = GPIO_KB_IN00; i < GPIO_KB_IN00 + KEYBOARD_KSI_COUNT; i++)
