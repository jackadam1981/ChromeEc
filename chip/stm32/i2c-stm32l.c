...
static void i2c_init_port(const struct i2c_port_t *p, int skip_unwedge_check);
...
static int try_pull_up_scl(enum gpio_signal scl)
{
	...
	CPRINTF("[%T clock stretched for too long?!]\n");
	return -1;
}
 ...
 *
 * @param port		I2C port
 * @param check_wedged	check if bus is wedged before performing unwedge
 */
static void i2c_try_unwedge(int port, int check_wedged)
{
	...
	if (check_wedged && gpio_get_level(scl) && gpio_get_level(sda))
	...
	/*
	 * SCL is high. No matter whether SDA is 0 or 1, we generate 9 clocks
	 * with SDA released and then send a STOP. If a slave is in the middle
	 * of writing, one of the cycles should be a NACK.
 ...
 * @param check_wedged	check if bus is wedged before performing unwedge
 */
static void i2c_init_port(const struct i2c_port_t *p, int check_wedged)
{
	int port = p->port;

	/* Unwedge the bus if it seems wedged */
	i2c_try_unwedge(port, check_wedged);
...
static void i2c_init(void)
{
	...
	for (i = 0; i < I2C_PORTS_USED; i++, p++)
		i2c_init_port(p, 1/* check if wedged */);
}
