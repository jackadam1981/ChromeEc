...
static void i2c_init_port(const struct i2c_port_t *p)
{
	...
	STM32_RCC_CCIPR &= ~STM32_RCC_CCIPR_I2C2SEL_MASK;
	STM32_RCC_CCIPR |= STM32_RCC_CCIPR_I2C_SYSCLK << STM32_RCC_CCIPR_I2C2SEL_SHIFT;
...
extern uint8_t in_mailbox[];
extern uint8_t out_mailbox[];
extern uint8_t in_ready;

extern void get_response(void);

static int tx_pending;
static int tx_index, tx_end;

static void dut_process_packet(int port)
{
	uint8_t size;
...
static void i2c_event_handler(int port)
{
	...
	if (i2c_isr & STM32_I2C_ISR_RXNE)
		in_mailbox[buf_idx++] = STM32_I2C_RXDR(port);
	...
	if (i2c_isr & STM32_I2C_ISR_TXIS) {
		if (port == I2C_PORT_EC) { /* host is waiting for PD response */
			if (tx_pending) {
				if (tx_index < tx_end) {
					STM32_I2C_TXDR(port) =
						out_mailbox[tx_index++];
				} else {
					STM32_I2C_TXDR(port) = CTS_I2C_FAILURE;
					/*
				 ...
				 * be prepared by i2c receive task
				 */
				STM32_I2C_CR1(port) &= ~STM32_I2C_CR1_TXIE;

				dut_process_packet(port);
