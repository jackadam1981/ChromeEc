...
static int usb_power_write_line(struct usb_power_config const *config)
{
	...
	if (config->state->reports_head != config->state->reports_tail) {
		/* We'll copy the data into a buffer for now to send it. */
		memcpy((void*)address, r, bytes);

		state->reports_tail = (state->reports_tail + 1) %
			state->max_cached;

		usb_write_ep(config->endpoint, bytes, address);
		return bytes;
	}
				 ...
				 union usb_power_command_data *cmd, int count)
{
	...
	state->ina_count += 1;
	CPRINTS("[ADDINA] port:%d addr:0x%02x rs:%d count:%d",
	        ina->port, ina->addr, ina->rs, state->ina_count);
	return USB_POWER_SUCCESS;
}
...
#if 1
		{
		int busv_mv = (ina2xx_read(ina->port, ina->addr, INA231_REG_BUSV) * 125) / 100;
		CPRINTS("[CAP] %d (%d,0x%02x): busv:%dmv", i, ina->port, ina->addr, busv_mv);
		}
#endif
...
static int usb_power_get_samples(struct usb_power_config const *config) {
	uint64_t time = get_time().val - config->state->base_time;
	struct usb_power_state *state = config->state;
	...
	while (1) {
		...
		for (port = 0; port < I2C_PORT_COUNT; port++) {
			...
			if (!(channel_state[port] & CHANNEL_READY))
				continue;
			/* Store the result */
			r->power[ina_index[port]] = SWIZZLE(channel_data[port]);
			/*CPRINTS("p:%d a:%02x d:%04x read complete", ina->port, ina->addr, channel_data[port]);*/
			channel_data[port] = SWIZZLE(0xbad);
	...
	state->reports_head = (state->reports_head + 1) % USB_POWER_MAX_CACHED(state->ina_count);
...
void usb_power_deferred_cap(struct usb_power_config const *config)
{
	...
	/* Double check if we have stopped capturing. */
	if (config->state->state == USB_POWER_STATE_CAPTURING)
