...
uint16_t ina2xx_readagain__7bf(uint8_t port, uint16_t i2c_addr__7bf)
{
	int res;
	uint16_t val;

	res = i2c_xfer__7bf(port, i2c_addr__7bf,
		       NULL, 0, (uint8_t *)&val, sizeof(uint16_t));

	if (res) {
		CPRINTS("INA2XX I2C readagain failed p:%d a:%02x",
			(int)port, (int)I2C_ADDR__7b(i2c_addr__7bf));
		return 0x0bad;
	}
...
uint16_t ina2xx_read__7bf(uint8_t port, uint16_t i2c_addr__7bf,
		     uint8_t reg)
{
	int res;
	int val;

	res = i2c_read16__7bf(port, i2c_addr__7bf, reg, &val);
	if (res) {
		CPRINTS("INA2XX I2C read failed p:%d a:%02x, r:%02x",
			(int)port, (int)I2C_ADDR__7b(i2c_addr__7bf), (int)reg);
		return 0x0bad;
	}
	return (val >> 8) | ((val & 0xff) << 8);
}

int ina2xx_write__7bf(uint8_t port, uint16_t i2c_addr__7bf,
		 uint8_t reg, uint16_t val)
{
	int res;
	uint16_t be_val = (val >> 8) | ((val & 0xff) << 8);

	res = i2c_write16__7bf(port, i2c_addr__7bf, reg, be_val);
	if (res)
...
static int usb_power_init_inas(struct usb_power_config const *config)
{
	...
	for (i = 0; i < state->ina_count; i++) {
		...
		CPRINTS("[CAP] %d (%d,0x%02x): conf:%x, cal:%x",
			i, ina->port, I2C_ADDR__7b(ina->addr__7bf), conf, cal);
		}
#endif
		/*
		...
		CPRINTS("[CAP] %d (%d,0x%02x): conf:%x, act:%x",
			i, ina->port, I2C_ADDR__7b(ina->addr__7bf),
			value, actual);
		}
#endif
#ifdef USB_POWER_VERBOSE
