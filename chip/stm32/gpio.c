...
void gpio_pre_init(void)
{
	...
	const struct unconnected_pin_info *u = unconnected_pin_list;
	...
	/* Configure optional unconnected pins for low power optimization. */
	for (i = 0; i < unconnected_pin_count; i++, u++) {
		if (IS_ENABLED(CHIP_FAMILY_STM32F4)) {
			/*
			 * Configure unconnected pins as ANALOG INPUT to save power.
