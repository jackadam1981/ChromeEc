...
typedef size_t (*remove_unit_t)(struct usart_config const *config, void *dest);
...
static size_t queue_remove(struct usart_config const *config, void *dest)
{
	return queue_remove_unit(config->producer.queue, dest);
}
