...
static void ep0_rx(void)
{
	...
	if (req == (USB_DIR_IN | (USB_REQ_GET_DESCRIPTOR << 8))) {
		...
		case USB_DT_DEVICE: /* Setup : Get device descriptor */
			memcpy_usbram((void *)&dev_desc, ep0_buf_tx,
					 sizeof(dev_desc));
		...
		case USB_DT_CONFIGURATION: /* Setup : Get configuration desc */
			memcpy_usbram(__usb_desc, ep0_buf_tx,
					 USB_DESC_SIZE);
		...
		case USB_DT_STRING: /* Setup : Get string descriptor */
			...
			memcpy_usbram(str_desc, ep0_buf_tx, str_desc[0]);
		...
		case USB_DT_DEVICE_QUALIFIER: /* Get device qualifier desc */
			...
			STM32_TOGGLE_EP(0, EP_TX_RX_MASK, EP_RX_VALID | EP_TX_STALL,
				  0);
			break;
		...
		uint16_t zero = 0;
		/* Get status */
		memcpy_usbram((void *)&zero, ep0_buf_tx, 2);
