...
static uint32_t date_to_sec(struct calandar_date time)
{
	int i;
	uint32_t sec;
...
static struct calandar_date sec_to_date(uint32_t sec)
{
...
static uint32_t sec_to_rtc_dr(uint32_t sec)
{
	...
	rtc_dr = ((time.year / 10) << 20) | ((time.year % 10) << 16);
	rtc_dr |= ((time.month / 10) << 12) | ((time.month % 10) << 8);
	rtc_dr |= ((time.day / 10) << 4) | (time.day % 10);
