		   ...
		   struct rtc_time_reg *rtc)
{
	...
	/* Check for match on hours, minutes, and seconds */
	STM32_RTC_ALRMAR |= 0xc0000000;

	if (delay_us) {
		/* Set alarm time in subseconds */
		STM32_RTC_ALRMASSR = us_to_rtcss(alarm_us);
		/* Check for match on subseconds */
		STM32_RTC_ALRMASSR |= 0x0f000000;
	} else
		/*
		 * If the caller doesn't specify subsecond delay
		 * (e.g. host command), just align the alarm time to second.
		 */
