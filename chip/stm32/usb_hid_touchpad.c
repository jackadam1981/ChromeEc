 ...
 * FIXME(drinkcat): There are ways to reduce flash usage, as the Finger Usage is
 * repeated 5 times.
 * FIXME(drinkcat): Touchpad specific values should be probed from touchpad
 * itself.
 */
...
void set_touchpad_report(struct usb_hid_touchpad_report *report)
{
	...
	while ((STM32_USB_EP(USB_EP_HID_TOUCHPAD) & EP_TX_MASK)
		...
		if (!--timeout) {
			CPRINTF("TP: timeout waiting for EP transfer\n");
			return;
		}
	}
