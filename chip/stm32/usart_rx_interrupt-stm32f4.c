...
static void tl_usart_rx_interrupt_handler(struct usart_config const *config)
{
	intptr_t base   = config->hw->base;
	int32_t  status = STM32_USART_SR(base);
	...
	if (status & STM32_USART_SR_RXNE) {
		...
		tl_usart_rx_char(&byte);
	}
}

struct usart_rx const tl_usart_rx_interrupt = {
	.producer_ops = {
		/*
		 * Nothing to do here, we either had enough space in the queue
		 * when a character came in or we dropped it already.
	...
	.init      = usart_rx_init,
	.interrupt = tl_usart_rx_interrupt_handler,
	.info      = NULL,
};
