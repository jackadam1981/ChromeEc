...
#define BLOCKS_HWBANK_MASK (0xFFFFFFFF)
#endif
 ...
 * will have write protections enabled. The first BLOCKS_HWBANK_MASK bits
 * correspond to bank 1 and the next BLOCKS_HWBANK_MASK bits correspond
 * to bank 2.
 *
 * STM32H743 has blocks of size 128kb. Each bit maps to a single block.
 * STM32H7A3 has blocks of size 8kb. Each bit maps to four blocks.
 *  Bit 0 --> Sectors 0 to 3
 *  Bit 1 --> Sectors 4 to 7
 *  ...
...
int flash_physical_write(int offset, int size, const char *data)
{
	...
	for (; size > 0; size -= CONFIG_FLASH_WRITE_SIZE) {
		...
		/* write a CONFIG_FLASH_WRITE_SIZE flash word */
		if (unaligned) {
...
int flash_physical_get_protect(int block)
{
	...
	int index = (block % BLOCKS_PER_HWBANK) / CONFIG_FLASH_WP_BANKS;
