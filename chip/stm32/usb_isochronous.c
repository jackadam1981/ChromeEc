 ...
 * by `GET_TX_DTOG()`.
 */
static inline int get_tx_dtog(struct usb_isochronous_config const *config)
{
	if (STM32_USB_EP(config->endpoint) & EP_TX_DTOG)
		return 1;
	else
		return 0;
}

static inline usb_uint *get_app_addr(
...
static inline usb_uint get_app_count(
		struct usb_isochronous_config const *config,
		int dtog_value)
{
	if (dtog_value)
		return btable_ep[config->endpoint].tx_count;
	else
		return btable_ep[config->endpoint].rx_count;
}

static inline void set_app_count(struct usb_isochronous_config const *config,
				 int dtog_value,
				 usb_uint count)
{
	if (dtog_value)
