...
void board_i2c_post_init(int port)
	 ...
	 * the I2C port consumes power when it is stopped, but not enabled.
	 * They recommend disabling it when not in use.
	 */

static inline void i2c_teardown_port(unsigned int port)
{
	enum gpio_signal pin;

	/* Disable the port */
	NRF51_TWI_ENABLE(port) = NRF51_TWI_DISABLE_VAL;
	NRF51_TWI_PSELSCL(port) = 0xffffffff;
	NRF51_TWI_PSELSDA(port) = 0xffffffff;
	...
	if (get_scl_from_i2c_port(port, &pin) == EC_SUCCESS)
		gpio_set_flags(pin,
			GPIO_INPUT | GPIO_PULL_UP | GPIO_OPEN_DRAIN);
