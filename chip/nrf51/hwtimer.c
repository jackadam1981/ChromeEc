...
void __hw_clock_event_set(uint32_t deadline)
{
	...
	NRF51_TIMER_CC(0, CC_INTERRUPT) = deadline - shift;

	/* enable interrupt */
	NRF51_TIMER_INTENSET(0) = 1 << NRF51_TIMER_COMPARE_BIT(CC_INTERRUPT);
}
...
{
	/* disable interrupt */
	NRF51_TIMER_INTENCLR(0) = 1 << NRF51_TIMER_COMPARE_BIT(CC_INTERRUPT);
}

uint32_t __hw_clock_source_read(void)
{
	/* to capture the current value */
	NRF51_TIMER_CAPTURE(0, CC_CURRENT) = 1;
	return NRF51_TIMER_CC(0, CC_CURRENT) + shift;
}
...
void __hw_clock_source_set(uint32_t ts)
{
	...
	NRF51_TIMER_STOP(0) = 1;
	NRF51_TIMER_CLEAR(0) = 1;

	/* So that no interrupt until next __hw_clock_event_set() */
	NRF51_TIMER_CC(0, CC_INTERRUPT) = ts - 1;

	/* Update the overflow point */
	NRF51_TIMER_CC(0, CC_OVERFLOW) = 0 - shift;

	/* Start the timer again */
	NRF51_TIMER_START(0) = 1;
}
...
void timer_irq(void)
{
	...
	/* clear status */
	NRF51_TIMER_COMPARE(0, CC_INTERRUPT) = 0;

	if (NRF51_TIMER_COMPARE(0, CC_OVERFLOW)) {
		NRF51_TIMER_COMPARE(0, CC_OVERFLOW) = 0;
		overflow = 1;
	}
...
int __hw_clock_source_init(uint32_t start_t)
{
	...
	/* 32-bit timer mode */
	NRF51_TIMER_MODE(0) = NRF51_TIMER_MODE_TIMER;
	NRF51_TIMER_BITMODE(0) = NRF51_TIMER_BITMODE_32;
	 ...
	 */
	NRF51_TIMER_PRESCALER(0) = 4;  /* actual value is 2**4 = 16 */

	/* Not to trigger interrupt until __hw_clock_event_set() is called. */
	NRF51_TIMER_CC(0, CC_INTERRUPT) = 0xffffffff;

	/* Set to 0 so that the next overflow can trigger timer_irq(). */
	NRF51_TIMER_CC(0, CC_OVERFLOW) = 0;
	NRF51_TIMER_INTENSET(0) = 1 << NRF51_TIMER_COMPARE_BIT(CC_OVERFLOW);
	...
	/* Enable interrupt */
	task_enable_irq(NRF51_PERID_TIMER(0));

	/* Start the timer */
	NRF51_TIMER_START(0) = 1;

	return NRF51_PERID_TIMER(0);
}
