...
char message[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrs";
...
static int fill_payload(uint8_t *payload, uint64_t addr, int name_length)
{
	...
	curr = pack_adv(curr, name_length, GAP_COMPLETE_NAME, message);
...
{
	int i, size = 0, dacnf = NRF51_RADIO_DACNF;
...
int ble_radio_add_device_to_white_list(uint8_t *addr_ptr, uint8_t rand)
{
	...
	/* Find a free entry */
	for (i = 0; i < 8 && (dacnf & NRF51_RADIO_DACNF_ENA(i)); i++)
		;
...
int ble_radio_remove_device_from_white_list(uint8_t *addr_ptr, uint8_t rand)
{
	...
	if (i == 8) /* Not found is successfully removed */
		return EC_SUCCESS;
...
int ble_adv_packet(struct ble_pdu *adv_packet, int chan)
{
	...
	case BLE_ADV_HEADER_PDU_TYPE_SCAN_REQ:
		...
		if (memcmp(&rcv_packet.payload[6], &adv_packet->payload[0], 6))
			return rv;
	break;
	case BLE_ADV_HEADER_PDU_TYPE_CONNECT_REQ:
		...
		if (adv_packet->header.adv.type ==
		     BLE_ADV_HEADER_PDU_TYPE_ADV_DIRECT_IND &&
		    memcmp(&adv_packet->payload[6], &rcv_packet.payload[0], 6))
			return rv;
	break;
