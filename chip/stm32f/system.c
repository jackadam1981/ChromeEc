...
static void check_reset_cause(void)
{
	...
	uint32_t raw_cause = STM32F_RCC_CSR;
...
int system_pre_init(void)
{
	...
	while (!(STM32F_RCC_CSR & (1 << 1)))
		;
	/* re-configure RTC if needed */
	if ((STM32F_RCC_BDCR & 0x00008300) != 0x00008200) {
		/* the RTC settings are bad, we need to reset it */
		STM32F_RCC_CSR |= 0x00800000;
		/* Enable RTC and use LSI as clock source */
		STM32F_RCC_CSR = (STM32F_RCC_CSR & ~0x00000300) | 0x00000200;
	}
