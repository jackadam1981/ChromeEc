...
#define NUM_PORTS 2
#define I2C1      STM32F_I2C1_PORT
#define I2C2      STM32F_I2C2_PORT
...
static void i2c_event_handler(int port)
{
	...
	i2c_sr1[port] = STM32F_I2C_SR1(port);
	STM32F_I2C_SR1(port) = 0;
	...
	if (i2c_sr1[port] & (1 << 1)) {
		...
		STM32F_I2C_SR1(port);
		STM32F_I2C_CR1(port) = STM32F_I2C_CR1(port);
#ifdef CONFIG_DEBUG
		CPRINTF("%s: STOPF\n", __func__);
#endif
	} else {
...
DECLARE_IRQ(STM32F_IRQ_I2C2_EV, i2c2_event_interrupt, 3);
...
static void i2c_error_handler(int port)
{
	...
	CPRINTF("%s: I2C_SR2(%s): 0x%04x\n",
			__func__, port, STM32F_I2C_SR2(port));
#endif

	STM32F_I2C_SR1(port) &= ~0xdf00;
}
static void i2c2_error_interrupt(void) { i2c_error_handler(I2C2); }
DECLARE_IRQ(STM32F_IRQ_I2C2_ER, i2c2_error_interrupt, 2);
...
static int i2c_init2(void)
{
	...
	/* enable I2C2 clock */
	STM32F_RCC_APB1ENR |= 1 << 22;

	/* set clock configuration : standard mode (100kHz) */
	STM32F_I2C_CCR(I2C2) = I2C_CCR;

	/* set slave address */
	STM32F_I2C_OAR1(I2C2) = I2C_ADDRESS;

	/* configuration : I2C mode / Periphal enabled, ACK enabled */
	STM32F_I2C_CR1(I2C2) = (1 << 10) | (1 << 0);
	/* error and event interrupts enabled / input clock is 16Mhz */
	STM32F_I2C_CR2(I2C2) = (1 << 9) | (1 << 8) | 0x10;

	/* clear status */
	STM32F_I2C_SR1(I2C2) = 0;
	...
	task_enable_irq(STM32F_IRQ_I2C2_EV);
	task_enable_irq(STM32F_IRQ_I2C2_ER);
