...
void spi_work_task(void)
{
	...
	while (1) {
		...
		STM32F_SPI_CR2(port) &= ~CR2_TXDMAEN;
		STM32F_SPI_DR(port) = 0xff;
	}
}
 ...
 *
 * @param port		Port to send reply back on (STM32F_SPI0/1_PORT)
...
static void reply(int port, char *msg, int msg_len)
{
	...
	dma_start_tx(dmac, msg_len, (void *)&STM32F_SPI_DR(port), out_msg);
}
 ...
 * the incoming data.
 *
 * @param port	Port that the interrupt came in on (STM32F_SPI0/1_PORT)
 */
static void spi_interrupt(int port)
{
	...
	if (!(STM32F_SPI_SR(port) & SR_RXNE))
		return;

	/* Get the command byte */
	cmd = STM32F_SPI_DR(port);

	/* Read the rest of the message - for now we do nothing with it */
	dmac = DMA_CHANNEL_FOR_SPI_RX(port);
	dma_start_rx(dmac, sizeof(in_msg), (void *)&STM32F_SPI_DR(port),
		     in_msg);
...
static void spi1_interrupt(void) { spi_interrupt(STM32F_SPI1_PORT); };

DECLARE_IRQ(STM32F_IRQ_SPI1, spi1_interrupt, 2);
