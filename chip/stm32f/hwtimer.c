...
void __hw_clock_event_set(uint32_t deadline)
{
	...
	if ((deadline >> 16) == STM32F_TIM_CNT(2)) {
		/* we can set a match on the LSB only */
		STM32F_TIM_CCR1(3) = deadline & 0xffff;
		/* disable MSB match */
		STM32F_TIM_DIER(2) &= ~2;
		...
		STM32F_TIM_DIER(3) |= 2;
	} else if ((deadline >> 16) > STM32F_TIM_CNT(2)) {
		/* first set a match on the MSB */
		STM32F_TIM_CCR1(2) = deadline >> 16;
		/* disable LSB match */
		STM32F_TIM_DIER(3) &= ~2;
		/* Clear the match flags */
		STM32F_TIM_SR(2) = ~2;
...
{
	uint32_t stat_tim2 = STM32F_TIM_SR(2);
...
DECLARE_IRQ(STM32F_IRQ_TIM2, __hw_clock_source_irq, 1);
DECLARE_IRQ(STM32F_IRQ_TIM3, __hw_clock_source_irq, 1);
...
int __hw_clock_source_init(void)
{
	...
	STM32F_RCC_APB1ENR |= 0x3;

	/*
	 * Timer configuration : Upcounter, counter disabled, update event only
	 * on overflow.
	 */
	STM32F_TIM_CR1(2) = 0x0004;
	STM32F_TIM_CR1(3) = 0x0004;
	/* TIM3 (master mode) generates a periodic trigger signal on each UEV */
	STM32F_TIM_CR2(2) = 0x0000;
	STM32F_TIM_CR2(3) = 0x0020;
	/* TIM2 (slave mode) uses ITR2 as internal trigger */
	STM32F_TIM_SMCR(2) = 0x0027;
	STM32F_TIM_SMCR(3) = 0x0000;
	/* Auto-reload value : 16-bit free-running counters */
	STM32F_TIM_ARR(2) = 0xffff;
	STM32F_TIM_ARR(3) = 0xffff;
	/* Pre-scaler value :
	 * TIM3 is counting microseconds, TIM2 is counting every TIM3 overflow.
	 */
	STM32F_TIM_PSC(2) = 0;
	STM32F_TIM_PSC(3) = CLOCKSOURCE_DIVIDER - 1;

	/* Reload the pre-scaler */
	STM32F_TIM_EGR(2) = 0x0000;
	STM32F_TIM_EGR(3) = 0x0000;

	/* setup the overflow interrupt on TIM2 */
	STM32F_TIM_DIER(2) = 0x0001;
	STM32F_TIM_DIER(3) = 0x0000;
	...
	task_enable_irq(STM32F_IRQ_TIM2);
	task_enable_irq(STM32F_IRQ_TIM3);

	return STM32F_IRQ_TIM3;
}
