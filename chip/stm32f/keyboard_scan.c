...
static void select_column(int col)
{
	...
	for (i = 0; i < ARRAY_SIZE(ports); i++) {
		...
		if (bsrr)
			STM32F_GPIO_BSRR_OFF(ports[i]) = bsrr;
...
void wait_for_interrupt(void)
{
	...
	pr_before = STM32F_EXTI_PR;
	select_column(COL_ASSERT_ALL);
	pr_after = STM32F_EXTI_PR;
	STM32F_EXTI_PR |= ((pr_after & ~pr_before) & IRQ_MASK);

	STM32F_EXTI_IMR |= IRQ_MASK;	/* 1: unmask interrupt */
}


void enter_polling_mode(void)
{
	STM32F_EXTI_IMR &= ~IRQ_MASK;	/* 0: mask interrupts */
	select_column(COL_TRI_STATE_ALL);
}
