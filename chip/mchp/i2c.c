...
static void configure_controller(int controller, int port, int kbps)
{
	...
	cfg |= 0;
	MCHP_I2C_CONFIG(controller) = port & 0xf;
...
err_chip_i2c_xfer:
	...
	if (ret_done == STS_LRB) {
		...
		reset_controller(controller);
		/* return EC_ERROR_TIMEOUT; */
		skip = EC_ERROR_TIMEOUT;
		} else {
			/* return EC_ERROR_UNKNOWN; */
			skip = EC_ERROR_UNKNOWN;
		}
	}
 ...
 * MCHP does not implement pins for port 1.
 * MCHP can map any port to any of the 4 controllers.
