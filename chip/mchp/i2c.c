...
/*
 * Return I2C controller's bus clock in kbps units.
 * MCHP MEC17xx family uses 16MHz I2C BAUD clock.
 * kbps = 16e3 / ((MCHP_I2C_BUS_CLK(controller)[7:0]+1) +
 *          		MCHP_I2C_BUS_CLK(controller)[15:8]+1))
 */
static int i2c_bus_kbps(int controller)
{
	uint32_t bclk;

	bclk = MCHP_I2C_BUS_CLK(controller);
	/* computed divisor is never 0 */
	bclk = ((bclk & 0xFF) + 1) + (((bclk >> 8) & 0xFF) + 1);
	return (int)(16000ul / bclk);
}
...
static void i2c_init(void)
{
	...
	for (i = 0; i < i2c_ports_used; ++i) {
		...
		/* get controller's current speed */
		controller_kbps = i2c_bus_kbps(controller);
		if (i2c_ports[i].kbps < controller_kbps)
			controller_kbps = i2c_ports[i].kbps;
