...
def GetDriveStrengthParameter(args):
  assert args.drive_strength in DRIVE_STRENGTH_LIST, \
         "Unsupported drive strength %d mA" & args.drive_strength
  return DRIVE_STRENGTH_LIST.index(args.drive_strength)
...
def BuildHeader(args, payload_len, load_addr, rorofile):
  ...
  b |= (GetDriveStrengthParameter(args) << 2)
  header.append(b)
...
def parseargs():
  ...
  parser.add_argument("--load_addr", type=functools.partial(int, base=0),
                      ...
                      default=0x0E0000)
  parser.add_argument("--drive_strength", type=functools.partial(int, base=0),
                      help="Drive strength. 2, 4, 8, or 12 mA",
                      default=4)
