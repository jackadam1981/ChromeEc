...
void wait_for_idle(void)
{
	...
	while (((MCHP_PECI_STATUS2 & MCHP_PECI_STATUS2_IDLE) == 0) &&
	       wait_timeout--)
		udelay(1 * MSEC);
...
void write_command(uint8_t cmd_fifo[], uint8_t cmd_length)
{
	...
	while ((!(MCHP_PECI_STATUS2 & MCHP_PECI_STATUS2_WFF)) &&
	       (i < cmd_length))
	...
	while (((MCHP_PECI_STATUS1 & MCHP_PECI_STATUS1_EOF) == 0) &&
	       wait_timeout--)
		udelay(1 * MSEC);
...
void cleanup(uint8_t done, uint8_t zero_err)
{
	...
	if (errval) {
		...
		MCHP_PECI_ERROR = errval; /* RWC clear error by writing value
					     back */
		...
		if (done) {
			...
			reset_PECI(MCHP_PECI_CONTROL_RST |
				   MCHP_PECI_CONTROL_FRST);
		} else
			trace0(0, PECI, 0, "retry command");
	}
}
...
static uint8_t peci_trans(struct peci_params_t *peci)
{
	...
	while (!done) {
		...
		if (MCHP_PECI_ERROR & MCHP_PECI_ERROR_FERR) {
			...
			if (peci->read_length) {
				read_databytes(peci->data_fifo,
					       (peci->read_length + 1));

				if ((!error) && (peci->cmd_FCS != 0)) {
					for (i = 0; i < (peci->read_length + 1);
					     i++) {
		...
		if (error) {
			...
			if (done) {
				if (cc_error) {
					if (cc_error > 1)
						trace0(0, PECI, 0,
						       "Illegal Request (no retry)");
					else {
						if (peci->data_fifo[COMP_CODE] ==
						    CC_TIMED_OUT)
							trace0(0, PECI, 0,
							       "Timeout");
						else
...
int peci_transaction(struct peci_data *peci)
{
	...
	if (peci->cmd_code == PECI_CMD_WR_PKG_CFG) {
		aw_FCS_calc = calc_AWFCS(peci_params.cmd_fifo,
					 peci_params.cmd_length - 1);
