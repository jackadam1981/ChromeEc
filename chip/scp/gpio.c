...
void gpio_set_alternate_function(uint32_t port, uint32_t mask, int func)
{
	...
	/* Up tp 8 alt functions per port */
	if (func > 7)
		return;
...
{
	ASSERT(!((flags & GPIO_HIGH) && (flags & GPIO_LOW)));
	ASSERT(!((flags & GPIO_INT_F_HIGH) && (flags & GPIO_INT_F_LOW)));
	ASSERT(!((flags & GPIO_INT_F_RISING) && (flags & GPIO_INT_F_FALLING)));
