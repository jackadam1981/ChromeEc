...
void gpio_set_alternate_function(uint32_t port, uint32_t mask, int func)
{
	...
	while (mask) {
		 ...
		 * b[2:0] - mode
		 */
		AP_GPIO_MODE(mode_reg_index) = (0x8 | func) << (bit & 7);
	}
}
...
void gpio_set_flags_by_mask(uint32_t port, uint32_t mask, uint32_t flags)
{
	...
	if (flags & GPIO_OUTPUT) {
		/* Set level before change to output mode */
		if (flags & GPIO_HIGH)
	...
	/* Set sens register on edge trigger */
...
static enum gpio_signal eint_signal_map[MAX_NUM_EINT];
	...
	for (port = 0; port <= MAX_EINT_PORT; port++) {
		...
		while (pending) {
			...
			if (SCP_EINT_MASK_GET[port] & (1 << bit))
				continue;
			/* Call handler */
			signal = eint_signal_map[port * 32 + bit];
