...
static uint32_t timer_reload_event_timer(void)
{
	if (event_deadline) {
		timer_reload(TIMER_EVENT, event_deadline);
		event_deadline = 0;
		return SCP_TIMER_RESET_VAL(TIMER_EVENT);
	} else if (event_high) {
...
{
	uint64_t deadline_26m = deadline * 26;
	uint64_t now_26m = timer_read_raw_system();

	if (deadline_26m > now_26m) {
		deadline_26m -= now_26m;
		event_deadline = deadline_26m & 0xffffffff;
		event_high = deadline_26m >> 32;
	} else {
		event_deadline = 1;
		event_high = 0;
	}

	timer_reload_event_timer();
}
...
{
	return (uint32_t)(timer_read_raw_system() / 26);
}
