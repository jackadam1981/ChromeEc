...
static struct queue const event_vdec_queue = QUEUE_DIRECT(9,
	struct vdec_service, null_producer, event_vdec_consumer);
static struct consumer const event_vdec_consumer = {
...
void vdec_service_task(void *u)
{
	...
	/* register IPI handler for H264 decoder */
	ipi_register(IPI_VDEC_H264, vdec_h264_ipi_handler);
	ipi_wakeup_ap_register(IPI_VDEC_H264);
