...
static void scp_release_resource(void)
{
	...
	/* release DARM and BUS are in use to suspend */
	SCP_BUS_RESOURCE = 0;
	SCP_AP_RESOURCE = 0;
}
DECLARE_DEFERRED(scp_release_resource);
...
#define TASK(n, ...) TASK_ID_##n,
	...
	if (state == HOST_SLEEP_EVENT_S3_SUSPEND) {
		...
		/*
		 * Wait 10ms and release DRAM and BUS to ensure host command
		 * transmission is finished.
		 */
		hook_call_deferred(&scp_release_resource_data, 10 * MSEC);
	} else if (state == HOST_SLEEP_EVENT_S3_RESUME) {
		ccprints("AP resume");
		hook_call_deferred(&scp_release_resource_data, -1);
...
static void ipi_handler(void)
{
	 ...
	 * be printed at host command driver
	 */
