...
static void ipi_hostcmd_handler(int id, void *buf, unsigned int len)
{
	...
	if (in_msg[0] == EC_HOST_REQUEST_VERSION) {
		...
		/*
		 * Just assigned the share_buf to request, host_packet_receive
...
/*
 * Host command IPI must be registered after host_command_task up.
 * If registering hostcmd handler too early (ex, at HOOK_INIT), then the system
 * will crash when the ipi hostcmd handler wake disabled host_command_task up.
 * This is because all tasks are enabled after hook_task invoke HOOK_INIT hooks.
 */
