...
static void ipi_handler(void)
{
	...
	memcpy((void *)in_msg, (void *)scp_rcv_obj->share_buf,
	       scp_rcv_obj->len);

	ipi_desc[scp_rcv_obj->id].handler(
		scp_rcv_obj->id, in_msg, scp_rcv_obj->len);
}
...
{
	if (((uint8_t *)buf)[0] == EC_HOST_REQUEST_VERSION) {
		/* Protocol version 3 */
		struct ec_host_request *r = (struct ec_host_request *)buf;
		int pkt_size;
	...
	} else if (((uint8_t *)buf)[0] >= EC_CMD_VERSION0) {
