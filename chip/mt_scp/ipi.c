 ...
 * prioirty of IPC is proportional to its IPC index. IPC3 has the highest
 ...
 * should sync across AP and SCP.  shared_buf should designated which IPI ID it
 * talks to.
...
#define IPI_MAX_REQUEST_SIZE CONFIG_IPC_SHARED_OBJ_SHARED_BUF_SIZE
#define IPI_MAX_RESPONSE_SIZE CONFIG_IPC_SHARED_OBJ_SHARED_BUF_SIZE
...
int ipi_send(int32_t id, void *buf, uint32_t len, int wait)
{
	...
	if (len > sizeof(scp_send_obj->shared_buf))
	...
	memcpy(scp_send_obj->shared_buf, buf, len);
...
static void ipi_handler(void)
{
	...
	/*
	 * Pass the shared_buf to handler. Each handler should be in charge of
	 * the buffer copying/reading before returning from handler.
	 */
	ipi_handler_table[scp_recv_obj->id](
		scp_recv_obj->id, scp_recv_obj->shared_buf, scp_recv_obj->len);
}
...
static void ipi_hostcmd_handler(int32_t id, void *buf, uint32_t len)
{
	...
	/*
	 * Just assigned the shared_buf to request, host_packet_receive
	...
	ipi_packet.response = scp_send_obj->shared_buf;
...
DECLARE_IPI(IPI_HOST_COMMAND, ipi_hostcmd_handler)
