...
static struct hostcmd_data {
	uint8_t type;
	uint8_t response[IPI_MAX_RESPONSE_SIZE];
} hc_obj;
...
{
	const uint8_t type_hostevent = HOSTCMD_TYPE_HOSTEVENT;
	if (active)
		ipi_send(IPI_HOST_COMMAND, &type_hostevent, sizeof(uint8_t), 1);
}
#endif
...
static void ipi_send_response_packet(struct host_packet *pkt)
{
	...
	ret = ipi_send(IPI_HOST_COMMAND, &hc_obj,
		       pkt->response_size + 1 /* +1 for type */, 1);
	if (ret)
...
static void ipi_hostcmd_handler(int32_t id, void *buf, uint32_t len)
{
	...
	hc_obj.type = HOSTCMD_TYPE_HOSTCMD;
	ipi_packet.response = hc_obj.response;
