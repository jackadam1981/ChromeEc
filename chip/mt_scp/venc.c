...
static void venc_h264_service_init(void) {}
static venc_msg_handler venc_h264_msg_handler;
...
void venc_service_task(void *u)
{
	...
	/* register IPI handler for H264 encoder */
	ipi_register(IPI_VENC_H264, venc_h264_ipi_handler);
	ipi_wakeup_ap_register(IPI_VENC_H264);
	...
	while (1) {
		...
		if (!rsv_msg.handler[rsv_msg.type])
