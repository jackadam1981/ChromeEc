...
void clock_init(void)
{
	...
	/* Initialize 26MHz system clock counter by set its value to 1. */
	SCP_CLK_SYS_VAL = (SCP_CLK_SYS_VAL & ~CLK_SYS_VAL_MASK) | 1;
	/* Initialize high frequency ULPOSC counter and VREQ counter. */
	SCP_CLK_HIGH_VAL = (SCP_CLK_HIGH_VAL & ~CLK_HIGH_VAL_MASK) | 1;
	...
	SCP_CLK_SLEEP = 0;
	SCP_CLK_ON_CTRL = (SCP_CLK_ON_CTRL & ~HIGH_FINAL_VAL_MASK) | 0x300;
	SCP_FAST_WAKE_CNT_END = (SCP_FAST_WAKE_CNT_END &
				 ~FAST_WAKE_CNT_END_MASK) | 0x18;

	/* Set slow wake clock */
	SCP_WAKE_CKSW = (SCP_WAKE_CKSW & ~WAKE_CKSW_SEL_SLOW_MASK) | 0x10;

	/* Select CLK_HIGH as wakeup clock */
	SCP_CLK_SLOW_SEL = (SCP_CLK_SLOW_SEL & ~(CKSW_SEL_SLOW_MASK |
			    CKSW_SEL_SLOW_DIV_MASK)) | 3;
