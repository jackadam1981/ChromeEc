...
ipc_if_ctx_t ipc_ctx[IPC_PEERS_TOTAL]  = {
	[IPC_PEER_HOST] = {
		.in_msg_reg = IPC_PEER02ISH_MSG_REGS,
		.out_msg_reg = IPC_ISH2PEER0_MSG_REGS,
		.in_drbl_reg = IPC_PEER02ISH_DOORBELL,
		.out_drbl_reg = IPC_ISH2PEER0_DOORBELL,
		.clr_bit = IPC_INT_ISH2PEER0_CLR_BIT,
		.irq_in = ISH30_IPC_IRQ_PEER02ISH,
		.irq_clr = ISH30_IPC_IRQ_ISH2PEER0_CLR,
...
uint8_t new_pimr_bit_arrr[IPC_PEERS_TOTAL][3] = {
	{
		IPC_PIMR_PEER02ISH_OFFS,
		IPC_PIMR_PEER02ISH_OFFS,
		IPC_PIMR_ISH2PEER0_CLR_OFFS
	}
};
...
static void setup_ipc(void)
{
	...
	out_drbl = REG32(IPC_PEER02ISH_DOORBELL);
	if (!IPC_IS_BUSY(out_drbl)) {
		task_set_event(-1, EVENT_FLAG_BIT_WRITE_IPC, 0);
	}

	task_enable_irq(ISH30_IPC_IRQ_HOST2ISH);
	task_enable_irq(ISH30_IPC_IRQ_ISH2PEER0_CLR);
...
void ipc_interrupt_handler(void)
{
	...
	uint32_t drbl = REG32(IPC_ISH2PEER0_MSG_REGS);
	uint8_t proto, cmd;

	//CPRINTS("IPC ISR");
	if ((pisr & IPC_PISR_PEER02ISH_BIT)
	    && (pimr & IPC_PIMR_PEER02ISH_BIT)) {
	...
	if ((busy_clear & IPC_INT_ISH2PEER0_CLR_BIT)
	    && (pimr & IPC_PIMR_ISH2PEER0_CLR_MASK_BIT)) {
