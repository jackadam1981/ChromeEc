...
unsigned char bus_freq[ISH30_I2C_PORT_COUNT];
unsigned char init_freq_mode = I2C_FREQ_120;
i2c_context i2c_ctxs[ISH30_I2C_PORT_COUNT];
i2c_bus_info_t board_config[ISH30_I2C_PORT_COUNT];
int i2c_wait_for_activity_timeout_counter = 0;
		  ...
		  uint8_t *in, int in_size, int flags)
{
	...
	slave_address = (uint8_t)slave_addr >> 1;

	operation = (in_size > 0) ? I2C_READ : I2C_WRITE;
	if (operation == I2C_WRITE) {
		w_len = (uint8_t) out_size - 1;
		w_data = &tmp_buf[1];
		r_len = 0;
		r_data = NULL;
#ifdef ISH_DEBUG
		CPRINTF("I2C_WRITE len: %d [", w_len);
		for (i = 0; i < w_len; i++)
			CPRINTF("0x%0x ", w_data[i]);
		CPUTS("]\n");
#endif
	} else {
		r_len = (uint8_t) in_size;
		r_data = in;
		w_data = NULL;
		w_len = 0;
	}
	...
	/* extra data length is 1 */
	total_len = 1 + w_len + r_len;
	reg = &tmp_buf[0];

	i2c_init_transaction(ctx, base, slave_address, flags);

	/* write extra data */
	i2c_write_buffer(base, 1, reg, &curr_index, total_len);

	/* write W data */
	i2c_write_buffer(base, w_len, w_data, &curr_index, total_len);

	if (r_len > 0) {
		/* write R commands */
		i2c_write_read_commands(base, r_len);

		/* set rx_theshold */
		i2c_mmio_write(base, IC_RX_TL, r_len - 1);
	}
	/* enable interrupts */
	i2c_intr_switch(base, mask_arr[operation]);
	...
	if (operation == I2C_READ) {
		CPRINTF("I2C read len: %d [", r_len);
		for (i = 0; i < r_len; i++)
			CPRINTF("0x%0x ", r_data[i]);
		CPUTS("]\n");
	}
#endif
	return 0;
...
static void i2c_init(void)
{
	...
	i2c_ctxs[0].base = (uint32_t *) I2C0_BASE;
	i2c_ctxs[1].base = (uint32_t *) I2C1_BASE;
	i2c_ctxs[2].base = (uint32_t *) I2C2_BASE;
	...
	task_enable_irq(ISH30_I2C0_IRQ);
	task_enable_irq(ISH30_I2C1_IRQ);
	task_enable_irq(ISH30_I2C2_IRQ);
