...
typedef struct {
	...
	uint8_t num_wr; // keep track of number of bytes loaded in the fifo
		// during slave transmit
} i2c_req_state_t;
...
static int I2C_Api_Setspeed(mxc_i2c_regs_t *i2c, i2c_speed_t i2cspeed)
{
	...
	if (i2cspeed == I2C_HS_MODE) {
		...
		tPCLK = 1000000 / (PeripheralClock / 1000);

		targBusFreq = i2cspeed;
		if (targBusFreq < 1000) {
			return EC_ERROR_INVAL;
		}

		tSCLmin = 1000000 / (targBusFreq / 1000);
		cklMin = ((T_LOW_MIN + T_F_MAX_HS + (tPCLK - 1) - T_AF_MIN) /
			  tPCLK) -
			 1;
		ckhMin = ((T_HIGH_MIN + T_R_MAX_HS + (tPCLK - 1) - T_AF_MIN) /
			  tPCLK) -
			 1;
		ckh_cklMin = ((tSCLmin + (tPCLK - 1)) / tPCLK) - 2;

		ticks_lo = (cklMin > (ckh_cklMin - ckhMin)) ?
				   (cklMin) :
				   (ckh_cklMin - ckhMin);
		ticks_hi = ckhMin;
	   ...
	   high-speed transaction starts with. Switch setting to fast mode and
	...
	if (states[i2c_num].slave_state == I2C_SLAVE_ADDR_MATCH) {
		...
		if (i2c->ctrl & MXC_F_I2C_CTRL_READ) {
			...
			if (slave_tx_remain != 0) {
				...
				if (int0 & I2C_ERROR) {
					i2c->int_en0 = 0;
					// Calculate the number of bytes sent by
					// the slave
					req->tx_num =
						states[i2c_num].num_wr -
						((i2c->tx_ctrl1 &
...
#if 0
                // Set TXLAST bit after writing the last byte
                i2c->tx_ctrl1 |= MXC_F_I2C_TX_CTRL1_TX_LAST;
               I2C_Api_DrainTX(i2c);
                // Disable TXTH interrupt
                i2c->int_en0 &= ~(MXC_F_I2C_INT_EN0_TX_THRESH);
                // Store the current state of the slave
                states[i2c_num].slave_state = I2C_SLAVE_WRITE_COMPLETE;
#endif
#ifdef I2C_SEND_BYTE_EC
				for (i = 0; i < 1; i++) {
					if (!(i2c->status &
