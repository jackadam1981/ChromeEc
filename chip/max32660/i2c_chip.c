...
static int i2c_master_write(mxc_i2c_regs_t *i2c, uint8_t addr, int start, int stop,
			const uint8_t *data, int len, int restart);
static int i2c_master_read(mxc_i2c_regs_t *i2c, uint8_t addr, int start, int stop,
		  ...
		  int out_size, uint8_t *in, int in_size, int flags)
{
	...
	if (out_size) {
		status = i2c_master_write(i2c_bus_ports[port],
	...
	if (in_size) {
		status = i2c_master_read(i2c_bus_ports[port],
					    slave_addr_flags, xfer_start,
...
static void i2c_init(void)
{
	...
	for (i = 0; i < i2c_ports_used; i++) {
		...
		i2c_init_peripheral(i2c_bus_ports[port], i2c_ports[i].kbps * 1000);
		i2c_set_timeout(i, 0);
	}
...
/**
 * I2C_SlaveRead() - Handles async read request from the I2c master.
 ...
 * Return EC_SUCCESS if successful, otherwise returns a common error code.
*/
static int I2C_SlaveRead(mxc_i2c_regs_t *i2c, i2c_req_t *req, uint32_t int_flags) {
	int i2c_num;
...
/**
 * I2C_SlaveWrite() - Handles async write request from the I2c master.
 * @i2c: I2C Peripheral pointer for the 
 * @req: Pointer to the request info.
 * @int_flags: Current state of the interrupt flags for this request.
 * 
 * Return EC_SUCCESS if successful, otherwise returns a common error code.
 */
static int I2C_SlaveWrite(mxc_i2c_regs_t *i2c, i2c_req_t *req, uint32_t int_flags) {
	int i2c_num;
...
static void I2C_SlaveHandler(mxc_i2c_regs_t *i2c)
{
	...
	if (i2c->ctrl & MXC_F_I2C_CTRL_READ) {
		status = I2C_SlaveRead(i2c, req, int_flags);
		if (status != EC_SUCCESS) {
			return;
		}
	} else {
		status = I2C_SlaveWrite(i2c, req, int_flags);
		if (status != EC_SUCCESS) {
			return;
		}
	}
