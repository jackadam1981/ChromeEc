...
#undef CHROMIUM_EC_PLATFORM_EMULATE
...
/*
 *
 *  I2C Master Implentation
 *
 */
int chip_i2c_xfer(int port, const uint16_t slave_addr_flags, const uint8_t *out, int out_size, uint8_t *in, int in_size, int flags)
{
	int xfer_start;
	int xfer_stop;
	int status;
	...
	ret = EC_SUCCESS;
	if (out_size) {
		status = I2C_Api_MasterWrite(i2c_bus_ports[port], slave_addr_flags, xfer_start, xfer_stop, out, out_size, 0);
		if (status != out_size)
			ret = EC_ERROR_UNKNOWN;
	}
	if (in_size && (status == EC_SUCCESS)) {
		status = I2C_Api_MasterRead(i2c_bus_ports[port], slave_addr_flags, xfer_start, xfer_stop, in, in_size, 0);
		if (status != in_size)
			ret = EC_ERROR_UNKNOWN;
	}
	return ret;
...
static void i2c_init(void)
{
	...
	/* Initialize ports as master, set the speed */
	...
	init_i2cs(CONFIG_HOSTCMD_I2C_SLAVE_ADDR);
#endif
}
...
static int command_master_write_test0(int argc, char **argv)
{
	...
	out_size = 16;
	in_size = 0;
	chip_i2c_xfer(port, slave_addr, out, out_size, in, in_size, I2C_XFER_START | I2C_XFER_STOP);

	return EC_SUCCESS;
}
DECLARE_CONSOLE_COMMAND(i2ctest0, command_master_write_test0,
												"i2ctest0",
												"Custom test for I2C");
...
DECLARE_CONSOLE_COMMAND(i2ctest1, command_master_write_test1,
												"i2ctest1",
												"Custom test for I2C");
...
static void i2c_send_response_packet(struct host_packet *pkt)
{
	...
	*out++ = size;
	 ...
	 */
	I2C_Api_Handler(i2c_bus_ports[CONFIG_HOSTCMD_I2C_SLAVE_ADDR]);
}
...
void i2c_chip_callback(i2c_req_t *req, int error)
{
	...
	req->addr = CONFIG_HOSTCMD_I2C_SLAVE_ADDR;
...
int init_i2cs(int port)
{
	...
	i2c_bus_ports[port]->int_en0 = 0;
	i2c_bus_ports[port]->int_en1 = 0;
	i2c_bus_ports[port]->int_fl0 = i2c_bus_ports[port]->int_fl0;
	i2c_bus_ports[port]->int_fl1 = i2c_bus_ports[port]->int_fl1;
	// clear the RX and TX FIFO
	i2c_bus_ports[port]->rx_ctrl0 |= MXC_F_I2C_RX_CTRL0_RX_FLUSH;
	i2c_bus_ports[port]->tx_ctrl0 |= MXC_F_I2C_TX_CTRL0_TX_FLUSH;
	// clear the control register
	i2c_bus_ports[port]->ctrl = 0;

	if ((error = I2C_Api_Init(i2c_bus_ports[port], I2C_STD_MODE)) != EC_SUCCESS) {
		while (1)
			;
	}
	// Prepare SlaveAsync
	req_slave.addr = CONFIG_HOSTCMD_I2C_SLAVE_ADDR;
	...
	if ((error = I2C_Api_SlaveAsync(i2c_bus_ports[port], &req_slave)) != EC_SUCCESS) {
		while (1)
			;
	}
