 ...
 * than the I2C Peripheral is able to send.
 */
...
/**
 * typedef i2c_controller_state_t - Available transaction states for
 * I2C Controller.
 */
typedef enum {
...
/**
 * typedef i2c_peripheral_state_t - Available transaction states for I2C
 * Peripheral.
 */
typedef enum {
	I2C_PERIPHERAL_WRITE_COMPLETE = 0,
	I2C_PERIPHERAL_ADDR_MATCH_READ = 1,
	I2C_PERIPHERAL_ADDR_MATCH_WRITE = 2,
} i2c_peripheral_state_t;
 ...
 *        will not be shifted. Used for both controller and peripheral
 *        transactions.
 * @addr_match_flag: Indicates which peripheral address was matched.
 *                   0x1 indicates first peripheral address matched.
 *                   0x2 indicates second peripheral address matched.
 *                   0x4 indicates third peripheral address matched.
 *                   0x8 indicates fourth peripheral address matched.
 * @tx_data: Data for controller write/peripheral read.
 * @rx_data: Data for controller read/peripheral write.
 ...
 * @state: I2C peripheral state that indicates address match, read and
 *         write status.
...
struct i2c_req {
	...
	volatile i2c_peripheral_state_t state;
...
static int i2c_peripheral_async(mxc_i2c_regs_t *i2c, i2c_req_t *req);
static void i2c_peripheral_handler(mxc_i2c_regs_t *i2c);
...
#ifdef CONFIG_BOARD_I2C_ADDR_FLAGS
	/*
	 * Set the secondary I2C peripheral address for the board.
	 */
	/* Index the secondary peripheral address. */
	i2c_bus_ports[I2C_PORT_EC]->peripheral_addr =
		(i2c_bus_ports[I2C_PORT_EC]->peripheral_addr &
		 ~(MXC_F_I2C_PERIPHERAL_ADDR_PERIPHERAL_ADDR_IDX |
		   MXC_F_I2C_PERIPHERAL_ADDR_PERIPHERAL_ADDR_DIS)) |
		(1 << MXC_F_I2C_PERIPHERAL_ADDR_PERIPHERAL_ADDR_IDX_POS);
	/* Set the secondary peripheral address. */
	i2c_bus_ports[I2C_PORT_EC]->peripheral_addr =
		(1 << MXC_F_I2C_PERIPHERAL_ADDR_PERIPHERAL_ADDR_IDX_POS) |
...
static i2c_req_t req_peripheral;
...
static void i2c_send_response_packet(struct host_packet *pkt)
{
	...
	*out++ = size;
	...
	req_peripheral.tx_remain = size + 2;
	req_peripheral.response_pending = true;

	/* Call the handler to send the response packet. */
	i2c_peripheral_handler(i2c_bus_ports[I2C_PORT_EC]);
}
...
/**
 * i2c_peripheral_service() - Called by the I2C peripheral interrupt controller.
 * @req:   Request currently being processed.
 */
void i2c_peripheral_service(i2c_req_t *req)
{
	/* Check if there was a host command (I2C controller write). */
	if (req->state == I2C_PERIPHERAL_ADDR_MATCH_WRITE) {
		req->state = I2C_PERIPHERAL_WRITE_COMPLETE;
...
/**
 * I2C0_IRQHandler() - Async Handler for I2C Peripheral driver.
 */
static void I2C0_IRQHandler(void)
{
	i2c_peripheral_handler(i2c_bus_ports[0]);
}

/**
 * I2C1_IRQHandler() - Async Handler for I2C Peripheral driver.
 */
static void I2C1_IRQHandler(void)
{
	i2c_peripheral_handler(i2c_bus_ports[1]);
}
...
/**
 * i2c_peripheral_service_read() - Services the Controller I2C read from the
 * peripheral.
 * @i2c: I2C peripheral pointer.
 * @req: Pointer to the request info.
 */
static void i2c_peripheral_service_read(mxc_i2c_regs_t *i2c, i2c_req_t *req)
{
	/*
	 * Clear the RX Threshold interrupt if set. Make sure and preserve
	 * a possible done bit, address match, or multiple peripheral address
	 * flags.
	 */
	...
	if (req->tx_remain != 0) {
		/*
		 * There is no longer a response pending from the peripheral to
		 * the controller.
		 */
...
/**
 * i2c_peripheral_service_write() - Services the Controller I2C write to the
 * peripheral.
 * @i2c: I2C peripheral pointer.
 * @req: Pointer to the request info.
 */
static void i2c_peripheral_service_write(mxc_i2c_regs_t *i2c, i2c_req_t *req)
{
...
/**
 * i2c_peripheral_handler() - I2C interrupt handler.
...
static void i2c_peripheral_handler(mxc_i2c_regs_t *i2c)
{
	...
	if ((req->expecting_start) && (i2c->int_fl0 & MXC_F_I2C_INT_FL0_ADDR_MATCH)) {
		...
		*/
		...
		/* Check if Controller is writing to the peripheral. */
		if (!(i2c->ctrl & MXC_F_I2C_CTRL_READ)) {
			/* I2C Controller is writing to the peripheral. */
			...
			req->state = I2C_PERIPHERAL_ADDR_MATCH_WRITE;
			/*
			...
			/* The Controller is reading from the peripheral. */
			/*
			 * Start transmitting to the Controller from the start
			 * of buffer.
			 */
			req->tx_data = host_buffer;
			req->state = I2C_PERIPHERAL_ADDR_MATCH_READ;
			/* Set the threshold for TX, the threshold is a four bit field. */
			i2c->tx_ctrl0 = ((i2c->tx_ctrl0 & ~(MXC_F_I2C_TX_CTRL0_TX_THRESH)) |
				(2 << MXC_F_I2C_TX_CTRL0_TX_THRESH_POS));
#ifdef CONFIG_BOARD_I2C_ADDR_FLAGS
	...
	} else if (req->state == I2C_PERIPHERAL_ADDR_MATCH_READ) {
		/* Service a read request from the I2C Controller. */
		i2c_peripheral_service_read(i2c, req);
	} else if (req->state == I2C_PERIPHERAL_ADDR_MATCH_WRITE) {
		/* Service a write request from the I2C Controller. */
		i2c_peripheral_service_write(i2c, req);
	}

}

/**
 * init_i2cs() - Async Handler for I2C Peripheral driver.
...
void init_i2cs(int port)
{
	...
	/* Prepare for interrupt driven peripheral requests. */
	req_peripheral.addr = CONFIG_HOSTCMD_I2C_ADDR_FLAGS;
	req_peripheral.tx_data = host_buffer; /* Transmitted to host. */
	req_peripheral.tx_remain = -1;
	req_peripheral.rx_data = host_buffer; /* Received from host. */
	req_peripheral.restart = 0;
	req_peripheral.response_pending = false;
	states[port].req = &req_peripheral;
	error = i2c_peripheral_async(i2c_bus_ports[port], &req_peripheral);
	if (error != EC_SUCCESS) {
		while (1)
			;
	}
...
/**
 * i2c_peripheral_async() - Peripheral Read and Write Asynchronous.
...
static int i2c_peripheral_async(mxc_i2c_regs_t *i2c, i2c_req_t *req)
{
	...
	/* Set the Peripheral Address in the I2C peripheral register. */
	i2c->peripheral_addr = req->addr;
...
static void i2c_send_board_response(int len)
{
	...
	req_peripheral.tx_remain = len;
	/* Indicate that there is a response pending from the peripheral. */
	req_peripheral.response_pending = true;
}
				...
				int restart)
{
	...
	/**
	 * Load FIFO with peripheral address for WRITE and as much data as we
	 * can.
	 */
	while (i2c->status & MXC_F_I2C_STATUS_TX_FULL) {
	}
	...
	if (start) {
		/**
		 * The peripheral address is right-aligned, bits 6 to 0, shift
			       ...
			       int stop, uint8_t *data, int len, int restart)
{
	...
	/* Load FIFO with peripheral address. */
	if (start) {
		i2c->controller_ctrl |= MXC_F_I2C_CONTROLLER_CTRL_START;
		while (i2c->status & MXC_F_I2C_STATUS_TX_FULL) {
		}
