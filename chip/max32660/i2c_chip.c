...
struct i2c_req {
	...
	volatile int expecting_done;
	volatile int expecting_start;
};
...
static void i2c_slave_service_read(mxc_i2c_regs_t *i2c, i2c_req_t *req)
{
	...
	i2c->int_fl0 = i2c->int_fl0 & ~(MXC_F_I2C_INT_FL0_ADDR_MATCH | MXC_F_I2C_INT_FL0_MAMI_MASK | MXC_F_I2C_INT_FL0_DONE);
...
static void i2c_slave_handler(mxc_i2c_regs_t *i2c)
{
	...
	if (req->expecting_start) {
		...
		if (i2c->int_fl0 & MXC_F_I2C_INT_FL0_ADDR_MATCH) {
			req->expecting_done = 1;
			req->expecting_start = 0;
			/*
	...
	if (req->expecting_done) {
		if (i2c->int_fl0 & MXC_F_I2C_INT_FL0_DONE) {
			req->expecting_start = 1;
			req->expecting_done = 0;
			/* Clear all interrupts except a possible address match. */
			i2c->int_fl0 = i2c->int_fl0 & ~(MXC_F_I2C_INT_FL0_ADDR_MATCH | MXC_F_I2C_INT_FL0_MAMI_MASK);
			i2c->int_fl1 = i2c->int_fl1;
...
void init_i2cs(int port)
{
	...
	states[port].req->expecting_done = 0;
	states[port].req->expecting_start = 1;
