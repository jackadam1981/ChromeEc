...
#define CLOCK_SOURCE EXT_PSR_32P768K_HZ
...
uint32_t cec_us_to_ticks(uint32_t us)
{
	return us * CLOCK_FREQ_HZ / 1000000;
}

/* Timer clock ticks to us */
#ifdef CONFIG_CEC_DEBUG
uint32_t cec_ticks_to_us(uint32_t ticks)
{
	return ticks * 1000000 / CLOCK_FREQ_HZ;
}
#endif
...
void cec_tmr_cap_start(enum cec_cap_edge edge, int timeout)
{
	...
	if (timeout > 0) {
		...
		if (timer_count < 0) {
			timer_count = 0;
			cprints(CC_CEC, "CEC WARNING: timer_count < 0");
		}

		/* Start the timer and enable the timer interrupt */
		ext_timer_ms(CEC_EXT_TIMER, CLOCK_SOURCE, 1, 1, timer_count, 0,
			     1);
	} else {
...
{
	ext_timer_ms(CEC_EXT_TIMER, CLOCK_SOURCE, 0, 0, 0, 1, 0);
}
