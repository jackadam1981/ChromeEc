...
const struct i2c_perif_ctrl_t i2c_perif_ctrl[] = {
...
void i2c_peripheral_read_write_data(int port)
{
	...
	if (port < I2C_STANDARD_PORT_COUNT) {
		...
		if (perif_status & IT83XX_SMB_SPDS) {
		...
		if (perif_status & IT83XX_SMB_STS) {
			/* Reset read and write buffer index */
			buffer_index_reset();
		}

		/* Write clear the peripheral status */
		IT83XX_SMB_SLSTA = perif_status;
	}
	/* Enhanced I2C peripheral channel D, E, F DMA mode */
		...
		ch = i2c_perif_ctrl[port].offset / I2C_ENHANCED_CH_INTERVAL;

		idx = i2c_perif_ctrl[port].dma_index;

		/* Interrupt pending */
		if (IT83XX_I2C_STR(ch) & IT83XX_I2C_INTPEND) {

			perif_status = IT83XX_I2C_IRQ_ST(ch);

			/* Master to read data */
			if (perif_status & IT83XX_I2C_IDR_CLR) {
			/*
			...
			/* Master to write data */
			if (perif_status & IT83XX_I2C_IDW_CLR) {
				/* Master to write data finish flag */
				wr_done[idx] = 1;
			}
			/* Peripheral finish */
			if (perif_status & IT83XX_I2C_P_CLR) {
				if (wr_done[idx]) {
			/*
			...
			/* Write clear the peripheral status */
			IT83XX_I2C_IRQ_ST(ch) = perif_status;
		}
...
void i2c_perif_interrupt(int port)
{
	/* Peripheral to read and write fifo data */
	i2c_peripheral_read_write_data(port);

	/* Clear the interrupt status */
	task_clear_pending_irq(i2c_perif_ctrl[port].irq);
}

void i2c_peripheral_enable(int port, uint8_t perif_addr)
{

	clock_enable_peripheral(i2c_perif_ctrl[port].clock_gate, 0, 0);
	...
	/* Enhanced I2C peripheral channel D, E, F DMA mode */
		...
		IT83XX_I2C_IDR(ch) = perif_addr << 1;
...
static void i2c_peripheral_init(void)
{
	...
	for (i = 0; i < i2c_perifs_used; i++) {

		/* I2c peripheral port mapping. */
		p = i2c_perif_ports[i].port;

		/* To enable peripheral ch[x] */
		i2c_peripheral_enable(p, i2c_perif_ports[i].adr);

		/* Clear the interrupt status */
		task_clear_pending_irq(i2c_perif_ctrl[p].irq);

		/* enable i2c interrupt */
		task_enable_irq(i2c_perif_ctrl[p].irq);
	}
}
