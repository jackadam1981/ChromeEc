...
void __ram_code clock_pll_changed(void)
{
	 ...
	 * complete PLL update.
	 */
	IT83XX_ECPM_PLLFREQR = pll_freq;
	/* Pre-set FND (e-flash) clock frequency = PLL / 3 */
...
static void clock_set_pll(enum pll_freq_idx idx)
{
	...
	pll_freq     = clock_pll_ctrl[idx].pll_freq;

	if (((IT83XX_ECPM_PLLFREQR & 0xf) != pll_freq) ||
	...
	/* Get current PLL frequency setting */
	pll_idx = pll_to_idx[IT83XX_ECPM_PLLFREQR & 0xf];
	/* USB and UART */
	IT83XX_ECPM_SCDCR1 = (clock_pll_ctrl[pll_idx].div_usb << 4) |
				clock_pll_ctrl[pll_idx].div_uart;
	/* SSPI and SMB */
	IT83XX_ECPM_SCDCR2 = (clock_pll_ctrl[pll_idx].div_sspi << 4) |
				clock_pll_ctrl[pll_idx].div_smb;
	/* USBPD and PWM */
	IT83XX_ECPM_SCDCR4 = (clock_pll_ctrl[pll_idx].div_usbpd << 4) |
				clock_pll_ctrl[pll_idx].div_pwm;
}
