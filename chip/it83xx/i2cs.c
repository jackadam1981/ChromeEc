...
const struct i2c_slv_ctrl_t i2c_slv_ctrl[] = {
	[IT83XX_I2C_CH_A] = {IT83XX_IRQ_SMB_A, -1, CGC_OFFSET_SMBA, -1, -1},
#if 1
	[IT83XX_I2C_CH_D] = {IT83XX_IRQ_SMB_D, 3, CGC_OFFSET_SMBD, 0, 0},
	[IT83XX_I2C_CH_E] = {IT83XX_IRQ_SMB_E, 0, CGC_OFFSET_SMBE, 1, 1},
	[IT83XX_I2C_CH_F] = {IT83XX_IRQ_SMB_F, 1, CGC_OFFSET_SMBF, 2, 2},
#endif
};
...
void i2c_slave_enable(int port, uint8_t slv_addr1, uint8_t slv_addr2)
{
	...
	/* I2C slave channel D, E, F DMA mode */
		...
		memset(in_data[i2c_slv_ctrl[port].tx], 0, I2C_MAX_BUFFER_SIZE);
		memset(out_data[i2c_slv_ctrl[port].rx], 0, I2C_MAX_BUFFER_SIZE);

		/* DMA write target address register */
		IT83XX_I2C_RAMHA(sft_port) =
				((uint32_t)in_data[i2c_slv_ctrl[port].tx]>>8);
		IT83XX_I2C_RAMLA(sft_port) =
				(uint32_t)in_data[i2c_slv_ctrl[port].tx];

		/* DMA read target address register */
		IT83XX_I2C_RAMHA2(sft_port) =
				((uint32_t)out_data[i2c_slv_ctrl[port].rx]>>8);
		IT83XX_I2C_RAMLA2(sft_port) =
				(uint32_t)out_data[i2c_slv_ctrl[port].rx];
	}
}
