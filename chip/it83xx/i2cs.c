...
void i2c_slave_enable(int port, uint8_t slv_addr1, uint8_t slv_addr2)
{
	...
	/* I2C slave channel D, E, F DMA mode */
		...
		memset(in_data[i2c_slv_ctrl[port].dma_index], 0,
			I2C_MAX_BUFFER_SIZE);
		memset(out_data[i2c_slv_ctrl[port].dma_index], 0,
			I2C_MAX_BUFFER_SIZE);

		/* DMA write target address register */
		IT83XX_I2C_RAMHA(sft_port) =
			((uint32_t)in_data[i2c_slv_ctrl[port].dma_index] >> 8);
		IT83XX_I2C_RAMLA(sft_port) =
			(uint32_t)in_data[i2c_slv_ctrl[port].dma_index];

		/* DMA read target address register */
		IT83XX_I2C_RAMHA2(sft_port) =
			((uint32_t)out_data[i2c_slv_ctrl[port].dma_index] >> 8);
		IT83XX_I2C_RAMLA2(sft_port) =
			(uint32_t)out_data[i2c_slv_ctrl[port].dma_index];
	}
}
