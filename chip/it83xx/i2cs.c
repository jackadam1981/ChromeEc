...
static uint8_t eh_in_data[I2C_MAX_BUFFER_SIZE]
			__attribute__((section(".h2ram.pool.i2cslvtx")));
static uint8_t eh_out_data[I2C_MAX_BUFFER_SIZE]
...
void i2c_slave_enable(int port, uint8_t slv_addr1, uint8_t slv_addr2)
{
	...
	if (port < I2C_STANDARD_PORT_COUNT) {
		...
		memset(eh_in_data, 0, I2C_MAX_BUFFER_SIZE);
		memset(eh_out_data, 0, I2C_MAX_BUFFER_SIZE);

		IT83XX_I2C_RAMHA(sft_port) = ((uint32_t)eh_in_data>>8);
		IT83XX_I2C_RAMLA(sft_port) = (uint32_t)eh_in_data;
		IT83XX_I2C_RAMHA2(sft_port) = ((uint32_t)eh_out_data>>8);
		IT83XX_I2C_RAMLA2(sft_port) = (uint32_t)eh_out_data;
