		...
		if (IT83XX_I2C_STR(r_sh) & IT83XX_I2C_INTPEND) {
			...
			if (!(slv_status & IT83XX_I2C_P_CLR)) {
				...
				if (slv_status & IT83XX_I2C_IDR_CLR) {
					CPRINTS("TODO:Master to read data "
					"from out_data array = 0x%x"
					, out_data[idx]);
				}
				/* Master to write data */
				if (slv_status & IT83XX_I2C_IDW_CLR) {
					CPRINTS("TODO:To read master write "
					"data from in_data array = 0x%x"
					, in_data[idx]);
				}
			}
			/* Slave finish */
				...
				if (slv_status & IT83XX_I2C_SLVDATAFLG)
					CPRINTS("Slave received data finsh");
				else
					CPRINTS("Master to read data finsh");
		}
