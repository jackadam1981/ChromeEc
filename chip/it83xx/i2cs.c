...
const struct i2c_slv_ctrl_t i2c_slv_ctrl[] = {
	[IT83XX_I2C_CH_A] = {I2C_SLAVE_ADDRA, 0x00, IT83XX_IRQ_SMB_A},
#if 0
	[IT83XX_I2C_CH_D] = {I2C_SLAVE_ADDRD, 0x00, IT83XX_IRQ_SMB_D},
	[IT83XX_I2C_CH_E] = {I2C_SLAVE_ADDRE, 0x00, IT83XX_IRQ_SMB_E},
	[IT83XX_I2C_CH_F] = {I2C_SLAVE_ADDRF, 0x00, IT83XX_IRQ_SMB_F},
#endif
};
...
void i2c_slave_isr(int port)
{
	...
	if (port < I2C_STANDARD_PORT_COUNT) {
		...
		if (IT83XX_SMB_SLSTA(port) & IT83XX_SMB_SDS) {
			/* Master read data */
			if (IT83XX_SMB_SLSTA(port) & IT83XX_SMB_RCS) {
				...
				IT83XX_SMB_SLSTA(port) |= IT83XX_SMB_SDS;
		...
		else if (IT83XX_SMB_SLSTA(port) & IT83XX_SMB_SPDS) {
			/* Write clear the stop condition */
			IT83XX_SMB_SLSTA(port) |= IT83XX_SMB_SPDS;
		...
		else if (IT83XX_SMB_SLSTA(port) & IT83XX_SMB_STS) {
			/* Write clear the timeout status */
			IT83XX_SMB_SLSTA(port) |= IT83XX_SMB_STS;
...
void i2c_slave_enable(int port, uint8_t slv_addr1, uint8_t slv_addr2)
{
	if (port < I2C_STANDARD_PORT_COUNT) {
		 ...
		 */
		IT83XX_SMB_HOCTL(port) |= IT83XX_SMB_KILL;
...
static void i2c_slave_init(void)
{
	...
	for (i = 0; i < i2c_slvs_used; i++) {
		...
		if (i2c_slv_ports[i].name != NULL) {
			...
			i2c_slave_enable(i, i2c_slv_ctrl[i].slave_adr,
				i2c_slv_ctrl[i].slave_adr2);
		}
	}
}
