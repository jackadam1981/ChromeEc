...
void i2c_slave_read_write_data(int port)
{
	...
	/* I2C slave channel D, E, F DMA mode */
		...
		if (IT83XX_I2C_STR(r_sh) & IT83XX_I2C_INTPEND) {
			...
			/* TODO: Master to read data from "out_data" array */
				/* Store test data from 0x00 to 0xFF */
				for (i = 0; i < I2C_MAX_BUFFER_SIZE; i++)
					out_data[idx][i] = i;
			}
			/* Master to write data */
			if (slv_status & IT83XX_I2C_IDW_CLR) {
			/* TODO: Slave to read data from "in_data" array */
				wr_done = 1;
			}

			if (slv_status & IT83XX_I2C_SLVDATAFLG)
				CPRINTS("Slave received data flag");
...
void i2c_slave_enable(int port, uint8_t slv_addr1, uint8_t slv_addr2)
{
	...
	/* I2C slave channel D, E, F DMA mode */
		...
		/* Software reset */
		IT83XX_I2C_DHTR(r_sh) |= 0x80;
		IT83XX_I2C_DHTR(r_sh) &= 0x7F;
