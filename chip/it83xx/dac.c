...
static void dac_set_output_voltage(enum chip_dac_channel ch, int mv)
{
	IT83XX_DAC_DACDAT(ch) = mv * DAC_MAX_RAW_DATA / DAC_MAX_MVOLT;
}

/* Get DAC output voltage */
static int dac_get_output_voltage(enum chip_dac_channel ch)
{
	return IT83XX_DAC_DACDAT(ch) * DAC_MAX_MVOLT / DAC_MAX_RAW_DATA;
}
...
DECLARE_CONSOLE_COMMAND(dactest, command_dactest,
			"r/w ch2-ch5 [enable=1|disable=0] [0-3300mV]",
			"Read or write DAC");
