 ...
 *
 * Setting the timeout of HOOK_TICK_INTERVAL to avoid watchdog reset
 * while waiting rx fifo to be full. The timeout is HOOK_TICK_INTERVAL
 * because the watchdog time will be reset during this interval.
 */
#define SPI_WAIT_REQUEST_TIMEOUT_US HOOK_TICK_INTERVAL
...
static int wait_rx_fifo_full(void)
{
	timestamp_t deadline;

	deadline.val = get_time().val + SPI_WAIT_REQUEST_TIMEOUT_US;
	while (1) {
		/* Waiting for RX FIFO to be full */
		if (IT83XX_SPI_RXFSR & IT83XX_SPI_RXF1FS)
			return 0;
		...
		if (timestamp_expired(deadline, NULL)) {
			CPRINTS("Ths Rx transfer timeout");
			return -1;
		}
	}
}
