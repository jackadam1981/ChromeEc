...
const int ec_spi_state[] = {
	[EC_SPI_RX_READY] = SPI_STATE_RX_READY,
	[EC_SPI_RECEIVING] = SPI_STATE_RECEIVING,
	[EC_SPI_PROCESSING] = SPI_STATE_PROCESSING,
	[EC_SPI_RX_BAD_DATA] = SPI_STATE_RX_BAD,
	[EC_SPI_NOT_READY] = SPI_STATE_RX_BAD,
	[EC_SPI_OLD_READY] = SPI_STATE_READY_TO_RECV,
};

static int spi_slv_response_state(int state)
{
	IT83XX_SPI_SPISRDR = state;

	return ec_spi_state[state];
}
...
static void reset_fifo_for_transfer(void)
{
	...
	/* Ready to receive */
	spi_slv_state = spi_slv_response_state(EC_SPI_OLD_READY);
...
static void spi_bad_received_data(int count)
{
	...
	spi_slv_state = spi_slv_response_state(EC_SPI_RX_BAD_DATA);
...
static void spi_send_response_packet(struct host_packet *pkt)
{
	...
	spi_slv_state = SPI_STATE_SENDING;
}
...
static void spi_parse_header(void)
{
	...
	if (spi_slv_state != SPI_STATE_READY_TO_RECV) {
		...
		spi_slv_state = spi_slv_response_state(EC_SPI_NOT_READY);
		return;
	}

	/* EC has started receiving the request from the AP */
	spi_slv_state = spi_slv_response_state(EC_SPI_RECEIVING);
	...
	if (in_msg[0] == EC_HOST_REQUEST_VERSION) {
		...
		spi_slv_state = spi_slv_response_state(EC_SPI_PROCESSING);
...
static void spi_slave_enable(void)
{
	...
	spi_slv_state = spi_slv_response_state(EC_SPI_RX_READY);
}
...
{
	spi_slv_state = SPI_STATE_DISABLED;
