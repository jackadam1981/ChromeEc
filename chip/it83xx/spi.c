...
#define EC_ALIGN_DATA_TO_WORD 4
...
static void spi_init(void)
{
	...
	/*
	 * There setting of below only supports version 3 protocol of host
	 * command interface.
	 */
	/*
	 * Target count means the first 8 bytes of requested data by the host.
	 * There is 12 bytes input because the CPU accesses FIFO base on word.
	 * If host requested data length is a byte, we need to align the data
	 * length to 4 bytes.
	 */
	IT83XX_SPI_FTCB1R = ((sizeof(*r) + EC_ALIGN_DATA_TO_WORD) >> 8) & 0xff;
	IT83XX_SPI_FTCB0R = (sizeof(*r) + EC_ALIGN_DATA_TO_WORD) & 0xff;
	/*
	 * The register setting can capture the data length of host requested
	 * data at 7th and 8th byte.
	 */
	IT83XX_SPI_TCCB1 = ((uint8_t *)&(r->data_len)-(uint8_t *)r) >> 8;
	IT83XX_SPI_TCCB0 = (uint8_t *)&(r->data_len)-(uint8_t *)r;
#else
