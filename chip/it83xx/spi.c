...
#define SPI_MAX_RESPONSE_SIZE 0x80
...
static void spi_send_response_packet(struct host_packet *pkt)
{
	...
	for (i = 0; i < SPI_PAST_END_LENGTH; i++)
...
static void spi_host_request_data(uint16_t count)
{
	...
	for (i = 0; i < count; i+=4) {
		...
		in_msg[(w_index + i + 1)] = IT83XX_SPI_RXFRDRB1;
		in_msg[(w_index + i + 2)] = IT83XX_SPI_RXFRDRB2;
		in_msg[(w_index + i + 3)] = IT83XX_SPI_RXFRDRB3;
	}
...
static void spi_parse_header(void)
{
	...
	if (wait_rx_fifo_count(sizeof(*r)))
		return spi_bad_received_data(sizeof(*r));
	...
	if (in_msg[0] == EC_HOST_REQUEST_VERSION) {
		...
		if (wait_rx_fifo_count(pkt_size))
		...
		spi_packet.response_max = sizeof(out_msg)
			- sizeof(out_preamble) - SPI_PAST_END_LENGTH;
