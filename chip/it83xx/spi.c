...
{
	//CPRINTS("[SPI] fillout_status0= %x ", IT83XX_SPI_INTSTATUS);
	/* Select FIFO Mode */
	...
	/* Fill out the status in FIFO register */
	IT83XX_SPI_SLAVEDATA0 = byte<<24 | byte<<16 | byte<<8 | byte;

	IT83XX_SPI_SLAVECTRL1 |= IT83XX_SPI_FIFOPNTRST;
	IT83XX_SPI_SLAVECTRL1 = 0x00;

	//CPRINTS("[SPI] fillout_status1= %x ", IT83XX_SPI_INTSTATUS);
	/* Select DMA Mode */
	IT83XX_SPI_SLAVECTRL0 |= IT83XX_SPI_SLAVEMODE;
...
//TODO(b/):/* Wait until we have received a certain number of bytes */
#if 0
...
static void spi_parse_header(void)
{
	...
	//TODO(b/): /* Wait for the packet data */
...
{
	CPRINTS("[SPI] spi_interrupt0= %x ", IT83XX_SPI_INTSTATUS);

	/* Clear the interrupt status */
	task_clear_pending_irq(IT83XX_IRQ_SPI);

	/* Detect DMA transfer done interrupt */
	if (IT83XX_SPI_INTSTATUS & IT83XX_SPI_DMATRSFDONE) {
	/* Detect 4-byte register RX full interrupt */
	//if (IT83XX_SPI_INTSTATUS & IT83XX_SPI_4BYTERXFULL) {
		...
		//CPRINTS("[SPI] IT83XX_SPI_SLAVEDATA0=%x"
		//, IT83XX_SPI_SLAVEDATA0);
		CPRINTS("[SPI] spi_interrupt1= %x ", IT83XX_SPI_INTSTATUS);
...
static void spi_init(void)
{
	 ...
	 */
	IT83XX_SPI_INTENBSTATUS = 0x08;
	//IT83XX_SPI_INTENBSTATUS = 0x02;
	...
	task_enable_irq(IT83XX_IRQ_SPI);
