...
static int spi_set_state(int state)
{
	spi_slv_state = state;

	return spi_response_state[state];
}
...
static void reset_fifo_for_transfer(void)
{
	...
	/* Ready to receive */
	IT83XX_SPI_SPISRDR = spi_set_state(SPI_STATE_READY_TO_RECV);
...
static void spi_bad_received_data(int count)
{
	...
	IT83XX_SPI_SPISRDR = spi_set_state(SPI_STATE_RX_BAD);
...
static void spi_send_response_packet(struct host_packet *pkt)
{
	...
	IT83XX_SPI_SPISRDR = spi_set_state(SPI_STATE_SENDING);
}
...
static void spi_parse_header(void)
{
	...
	if (spi_slv_state != SPI_STATE_READY_TO_RECV) {
		...
		IT83XX_SPI_SPISRDR = spi_set_state(SPI_STATE_NOT_READY);
		return;
	}

	/* EC has started receiving the request from the AP */
	IT83XX_SPI_SPISRDR = spi_set_state(SPI_STATE_RECEIVING);
	...
	if (in_msg[0] == EC_HOST_REQUEST_VERSION) {
		...
		IT83XX_SPI_SPISRDR = spi_set_state(SPI_STATE_PROCESSING);
...
{
	IT83XX_SPI_SPISRDR = spi_set_state(SPI_STATE_DISABLED);
