...
#define PNPCFG(_s)	{&_s##_settings[0], ARRAY_SIZE(_s##_settings)}
...
static void pnpcfg_init(void)
{
	...
	for (table = 0; table < ARRAY_SIZE(pnpcfg_settings); table++) {
		settings = pnpcfg_settings[table].settings;
		for (i = 0; i < pnpcfg_settings[table].entries; i++) {
			if (ec2i_write(settings[i].index_port,
				       settings[i].data_port) ==
			    EC2I_WRITE_ERROR) {
				ccprints("Failed to apply setting for table %d entry %d",
					 table, i);
				break;
			}
		}
