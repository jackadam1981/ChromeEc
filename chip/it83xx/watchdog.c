...
void watchdog_warning_irq(void)
{
	...
	/*
	* On first invocation set in_watchdog_extension_period=1, reload the
	* watchdog and return to effectively extend the watchdog timeout.
	*/
	...
	if (!atomic_add(&in_watchdog_extension_period, 1)) {
		...
		panic_printf("Pre-WDT warning! Skip to extend watchdog by %d ms\n",
...
int watchdog_init(void)
{
	...
	/*
	 * Do not lock access to watchdog registers, because
	 * we might want to change them later in RW image.
	 * see b:416109671 for context. 
	 */
