...
const struct i2c_ctrl_t i2c_ctrl_regs[] = {
	...
	{IT83XX_IRQ_SMB_D, CGC_OFFSET_SMBD},
	{IT83XX_IRQ_SMB_E, CGC_OFFSET_SMBE},
	{IT83XX_IRQ_SMB_F, CGC_OFFSET_SMBF},
};
...
static int i2c_pio_trans_data(int p, int direct, uint8_t data, int index)
{
	struct i2c_port_data *pd = pdata + p;
	uint8_t	p_ch;
...
static void i2c_freq_changed(void)
{
	...
	for (i = 0; i < i2c_ports_used; i++) {
		...
		if (i2c_ports[i].port < STANDARD_I2C_PORT_NUM) {
