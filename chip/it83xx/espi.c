...
/* VW configuration at initialization */
...
/* VW configuration for host startup */
static const struct vw_channel_t vw_startup_setting[] = {
	/*
	 * index 40h
	 * bit 0 / 4:     VW_SCI_L
	 * bit 1 / 5:     VW_SMI_L
...
int espi_vw_enable_wire_int(enum espi_vw_signal signal)
{
	 ...
	 * sequence signal individually, but we don't have register to do that
	 * (The VW interrupt control bit is used for all VW index flag updated,
	 * and enabled after initialization).
...
static void vw_config_host_startup(void)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(vw_startup_setting); i++)
		IT83XX_ESPI_VWIDX(vw_startup_setting[i].index) =
			(vw_startup_setting[i].level_mask |
			vw_startup_setting[i].valid_mask);
}
