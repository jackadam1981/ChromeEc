...
void espi_oob_send(uint8_t *oob_data, int oob_len, int oob_tag)
{
	int i;
	uint8_t reg;

	/* Set upstream cycle type */
	IT83XX_ESPI_ESUCTRL1 = 0x7;

	/* Set upstream Tag & length[11:8] */
	IT83XX_ESPI_ESUCTRL2 = (oob_tag & 0xF) | ((oob_len >> 8) & 0x0F);
	/* length[7:0] */
	IT83XX_ESPI_ESUCTRL3 = oob_len & 0xFF;

	/* Set upstream data */
	for (i = 0; i < oob_len; i++)
		IT83XX_ESPI_QUEUE_PUT_OOB(i, 1) = oob_data[i];
