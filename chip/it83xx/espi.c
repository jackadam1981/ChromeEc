...
#define CHIP_ESPI_INTERRUPT_NUM    8
...
/*
 * The ISR of espi VW interrupt in array need to be matched bit order in
...
BUILD_ASSERT(ARRAY_SIZE(espi_isr) == CHIP_ESPI_INTERRUPT_NUM);
...
void espi_interrupt(void)
{
	...
	for (i = 0; i < CHIP_ESPI_INTERRUPT_NUM; i++) {
		if (espi_event & (1 << i))
			espi_isr[i]();
	}
