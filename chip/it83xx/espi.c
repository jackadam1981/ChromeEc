...
int espi_oob_receive(uint8_t *oob_data)
{
	...
	/* Read PUT_OOB length */
	oob_len = IT83XX_ESPI_ESOCTRL4 & 0x7F;
	if (oob_len <= ESPI_OOB_MAX_LENGTH) {
		/* Read PUT_OOB data */
		for (i = 0; i < oob_len; i++)
			oob_data[i] = IT83XX_ESPI_QUEUE(PUT_OOB_OFFSET, i);
	} else {
...
enum ec_error_list espi_oob_send(uint8_t *oob_data)
{
	...
	char espi_cycle_type_table[ESPI_OOB_CYCLE_TYPE + 1] = {0};

	/* Construct cycle type table */
	espi_cycle_type_table[ESPI_FLASH_READ_CYCLE_TYPE] = 0x8;
	espi_cycle_type_table[ESPI_FLASH_WRITE_CYCLE_TYPE] = 0x9;
	espi_cycle_type_table[ESPI_FLASH_ERASE_CYCLE_TYPE] = 0xA;
	espi_cycle_type_table[ESPI_OOB_CYCLE_TYPE] = 0x7;
	...
	if (!espi_cycle_type_table[oob_data[0]])
		return EC_ERROR_INVAL;
	/* Set upstream cycle type */
	IT83XX_ESPI_ESUCTRL1 = espi_cycle_type_table[oob_data[0]];
	/* Set upstream Tag */
