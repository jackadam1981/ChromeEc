...
struct gpio_addr_t {
	uint32_t gpdr_addr;
	uint32_t gpotr_addr;
	uint32_t gpcrx0_addr;
};

/* GPIO group index convert to GPIO data/output type/ctrl group address */
static const struct gpio_addr_t gpio_group_idx_to_addr[GPIO_PORT_COUNT] = {
...
#define IT83XX_GPIO_DATA(port)                 \
	REG8(gpio_group_idx_to_addr[port].gpdr_addr)
#define IT83XX_GPIO_GPOT(port)                 \
        REG8(gpio_group_idx_to_addr[port].gpotr_addr)
#define IT83XX_GPIO_CTRL(port, pin_offset)     \
	REG8(gpio_group_idx_to_addr[port].gpcrx0_addr + pin_offset)
