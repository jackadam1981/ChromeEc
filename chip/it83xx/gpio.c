...
void gpio_set_flags_by_mask(uint32_t port, uint32_t mask, uint32_t flags)
{
	...
	if ((flags & GPIO_INT_BOTH) == GPIO_INT_BOTH)
		...
		*(wubemr(gpio_irqs[irq].wuc_group)) |= gpio_irqs[irq].wuc_mask;
		...
		/* Apply the setting of WUEMR register. */
		*(wubemr(gpio_irqs[irq].wuc_group)) &= ~gpio_irqs[irq].wuc_mask;
	...
	/*
	 * Set falling edge or both edges (apply to pins in WUC groups
	 * 7, 10, and 12 on IT8320 BX version and before) interrupt.
	 */
	if (flags & GPIO_INT_F_FALLING)
		*(wuemr(gpio_irqs[irq].wuc_group)) |= gpio_irqs[irq].wuc_mask;

	/*
	 * Always write 1 to clear the WUC status register after modifying edge
	 * mode selection register (WUBEMR and WUEMR).
	 */
