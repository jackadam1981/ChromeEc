...
void gpio_pre_init(void)
{
	...
	if (!IS_ENABLED(CONFIG_USB_PD_TCPM_ITE83XX) &&
		...
		for (i = 0; i < CONFIG_PD_PHY_PORT_COUNT; i++)
			it83xx_disable_cc_module(i);
	/*
	 * 2) If we use ITE TCPC by order of port index: TCPC physical support
	 * port counts > board active ITE pd port counts, then disable ITE TCPC
	 * not active ports.
	 */
	} else if (CONFIG_PD_PHY_PORT_COUNT > CONFIG_USB_PD_PORT_MAX_COUNT) {
		for (i = CONFIG_USB_PD_PORT_MAX_COUNT;
		     i < CONFIG_PD_PHY_PORT_COUNT; i++)
			it83xx_disable_cc_module(i);
	}
