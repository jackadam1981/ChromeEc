 ...
 * index should be a GPIO port (e.g. GPIO_A).
 */
...
static volatile uint8_t * wuesr(uint8_t grp)
{
	 ...
	 */
	if (grp <= 4)
		return (volatile uint8_t *)(IT83XX_WUC_WUESR1 + grp-1);
	else
		return (volatile uint8_t *)(IT83XX_WUC_WUESR6 + 4*(grp-6));
}
...
static volatile uint8_t * wuemr(uint8_t grp)
{
	 ...
	 */
	if (grp <= 4)
		return (volatile uint8_t *)(IT83XX_WUC_WUEMR1 + grp - 1);
	else
		return (volatile uint8_t *)(IT83XX_WUC_WUEMR6 + 4*(grp-6));
}
 ...
 */
GPIO_IRQ_FUNC(__gpio_irq_wko_107);

#undef GPIO_IRQ_FUNC

DECLARE_IRQ(IT83XX_IRQ_WKO107, __gpio_irq_wko_107, 1);
