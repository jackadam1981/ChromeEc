...
void gpio_set_flags_by_mask(uint32_t port, uint32_t mask, uint32_t flags)
{
	 ...
	 */
	if ((flags & GPIO_INT_F_FALLING) || (flags & GPIO_INT_F_BOTH)) {
		irq = gpio_to_irq(port, mask);
...
/**
 * Handlers for each GPIO port.  These clear the interrupt status register,
 * clears the WUC sense register, and then calls the master handler above.
 */
#define GPIO_IRQ_FUNC(irqfunc) 						\
	static void irqfunc(void)					\
	{								\
		int irq = IT83XX_INTC_AIVCT - 16;			\
		gpio_interrupt(gpio_irqs[irq].gpio_port,		\
				gpio_irqs[irq].gpio_mask);		\
		*(wuesr(gpio_irqs[irq].wuc_group)) |= gpio_irqs[irq].wuc_mask; \
		task_clear_pending_irq(irq); 				\
	}

/*
 * To add a new GPIO interrupt, add one GPIO_IRQ_FUNC to declare the IRQ
 * handler, and then add a DECLARE_IRQ calling the IRQ handler and identifying
 * the desired WKO interrupt signal. To find the correct WKO signal look in
 * gpio_irqs[]. Avoiding listing every WKO because that would add a lot of
 * functions to the list of IRQs and would make processing IRQs slower.
 */
GPIO_IRQ_FUNC(__gpio_irq_wko_70);
GPIO_IRQ_FUNC(__gpio_irq_wko_71);
GPIO_IRQ_FUNC(__gpio_irq_wko_72);
GPIO_IRQ_FUNC(__gpio_irq_wko_73);

#undef GPIO_IRQ_FUNC

DECLARE_IRQ(IT83XX_IRQ_WKO70, __gpio_irq_wko_70, 1);
DECLARE_IRQ(IT83XX_IRQ_WKO71, __gpio_irq_wko_71, 1);
DECLARE_IRQ(IT83XX_IRQ_WKO72, __gpio_irq_wko_72, 1);
DECLARE_IRQ(IT83XX_IRQ_WKO73, __gpio_irq_wko_73, 1);
