...
#if (CONFIG_FLASH_SIZE == 512) && defined(CHIP_CORE_NDS32)
...
static enum flash_wp_status flash_check_wp(void)
{
	...
	all_bank_count = CONFIG_FLASH_SIZE * 1024 / CONFIG_FLASH_BANK_SIZE;
...
int flash_physical_protect_now(int all)
{
	if (all) {
		...
		flash_protect_banks(0,
			CONFIG_FLASH_SIZE * 1024 / CONFIG_FLASH_BANK_SIZE,
			FLASH_WP_EC);
		all_protected = 1;
	} else {
