...
enum flash_status_mask {
	FLASH_SR_NO_BUSY = 0,
	/* Internal write operation is in progressn */
	FLASH_SR_BUSY = 0x01,
...
static enum flash_wp_status flash_check_wp(void)
{
	...
	if (bank == RO_BANK_COUNT)
		wp_status = FLASH_WP_STATUS_PROTECT_RO;
	else if (bank == (RO_BANK_COUNT + PSTATE_BANK_COUNT))
		wp_status = FLASH_WP_STATUS_PROTECT_RO;
...
int flash_physical_protect_now(int all)
{
	if (all) {
		...
		flash_protect_banks(RO_BANK_OFFSET,
			RO_BANK_COUNT, FLASH_WP_EC);
#ifdef PSTATE_BANK
