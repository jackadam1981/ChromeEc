...
struct i2c_slv_ctrl_t {
	int irq;              /* slave irq */
	/*
	 * To calculate address of channel D, E, F register respectively.
	 * address:   D: 0x00F03680, E: 0x00F03500, F: 0x00F03580
	 * reg_shift: D: 3           E: 0           F: 1
	 * address = BASEADDRESS(0x00F03500) + (reg_shift << 7)
	 * Channel A and D, E, F are located in different base address.
	 * reg_shift = -1 means unused.
	 */
	int reg_shift;
...
const struct i2c_slv_ctrl_t i2c_slv_ctrl[] = {
	[IT83XX_I2C_CH_A] = {.irq = IT83XX_IRQ_SMB_A, .reg_shift = -1,
		.clock_gate = CGC_OFFSET_SMBA, .dma_index = -1},
	[IT83XX_I2C_CH_D] = {.irq = IT83XX_IRQ_SMB_D, .reg_shift = 3,
		.clock_gate = CGC_OFFSET_SMBD, .dma_index = 0},
	[IT83XX_I2C_CH_E] = {.irq = IT83XX_IRQ_SMB_E, .reg_shift = 0,
		.clock_gate = CGC_OFFSET_SMBE, .dma_index = 1},
	[IT83XX_I2C_CH_F] = {.irq = IT83XX_IRQ_SMB_F, .reg_shift = 1,
...
void i2c_slave_read_write_data(int port)
{
	...
	/* I2C slave channel D, E, F DMA mode */
		...
		if (IT83XX_I2C_STR(r_sh) & IT83XX_I2C_INTPEND) {
			...
			slv_status = IT83XX_I2C_IRQ_ST(r_sh);

			/* Master to read data */
			if (slv_status & IT83XX_I2C_IDR_CLR) {
			/* TODO: Return buffer data by "out_data" array. */
			...
			if (slv_status & IT83XX_I2C_P_CLR) {
				if (wr_done[idx]) {
			/* TODO: Handle master write data by "in_data" array. */
					CPRINTS("WData: %.*h\n",
			...
			/* Write clear the slave status */
			IT83XX_I2C_IRQ_ST(r_sh) = slv_status;
		}
...
void i2c_slave_enable(int port, uint8_t slv_addr1, uint8_t slv_addr2)
{
	...
	/* I2C slave channel D, E, F DMA mode */
		...
		/* Software reset */
		IT83XX_I2C_DHTR(r_sh) |= (1 << 7);
		IT83XX_I2C_DHTR(r_sh) &= ~(1 << 7);

		/* This field defines the SMCLK3/4/5 clock/data low timeout. */
		IT83XX_I2C_TOR(r_sh) = I2C_CLK_LOW_TIMEOUT;
		...
		IT83XX_I2C_RAMHA(r_sh) = ((uint32_t)in_data[idx] >> 8) & 0x0F;
		IT83XX_I2C_RAMLA(r_sh) = (uint32_t)in_data[idx] & 0xFF;

		/* DMA read target address register */
		IT83XX_I2C_RAMHA2(r_sh) = ((uint32_t)out_data[idx] >> 8) & 0x0F;
