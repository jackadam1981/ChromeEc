	...
	if (IS_ENABLED(IT83XX_INTC_PLUG_IN_OUT_SUPPORT)) {
		if (USBPD_IS_PLUG_IN_OUT_DETECT(port)) {
			if (USBPD_IS_PLUG_IN(port)) {
				 ...
				 * plug out is detected by polling Vbus, later
				 * HOOK_USB_PD_DISCONNECT will enable plug in
				 * detection. Disable also can avoid any cc volt
				 * changes (ex. pd power negotiation) firing
				 * plug in interrupt.
				 ...
				 * plug out and HOOK_USB_PD_DISCONNECT, so we
				 * disable detect plug in/out interrupt here,
				 * and later HOOK_USB_PD_DISCONNECT will enable
				 * it.
				 */
