	...
	if (USBPD_IS_TX_DONE(port)) {
		...
		if (USBPD_IS_TX_ERR(port)) {
			if (USBPD_IS_TX_DISCARD(port))
				task_set_event(PD_PORT_TO_TASK_ID(port),
					TASK_EVENT_PHY_TX_MSG_DISCARD, 0);
			else if (USBPD_IS_TX_NO_GOODCRC(port))
				task_set_event(PD_PORT_TO_TASK_ID(port),
					TASK_EVENT_PHY_TX_DONE_NO_GDCRC, 0);
			else if (USBPD_IS_TX_NOT_EN(port))
				task_set_event(PD_PORT_TO_TASK_ID(port),
					TASK_EVENT_PHY_TX_NOT_EN, 0);
		}
#endif
