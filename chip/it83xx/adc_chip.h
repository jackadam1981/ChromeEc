...
struct vcmp_t {
	const char *name;
	int threshold;
	/* Select greater/less equal threshold, and edge/level trigger */
	char flag;
	/*
 ...
 */
extern struct vcmp_t vcmp_list[];
extern const struct vcmp_ctrl_t vcmp_ctrl_regs[];
void set_voltage_comparator_condition(int index, enum chip_vcmp vcmpx);
#endif

/*
 * Optional board-level callback functions called after voltage comparator
 * interrupts. Board set voltage comparator condituins, ex. voltage comparator
 * threshold, trigger level and so on.
 */
void board_vcmp_ctrl(int index, enum chip_vcmp vcmpx) __attribute__((weak));
