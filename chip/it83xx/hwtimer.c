...
{
	/* Match time is the next event time rounded down to tick interval. */
	uint32_t match_time = next_event_time & ~TICK_INTERVAL_MASK;
	 ...
	 * that we have to check if the match time is equal to the time now or
	 * the time on the last tick because since the last tick a new event
	 * could have been added with a value in between the last time_us and
	 * the new time_us.
	 */
	if (time_us == match_time || (time_us-TICK_INTERVAL) == match_time)
		process_timers(0);
	else if (time_us == 0)
		process_timers(1);
}
