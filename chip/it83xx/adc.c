...
void set_voltage_comparator_condition(int index, int ch)
{
	...
	*vcmp_ctrl_regs[ch].vcmp_datm = (uint8_t)(temp >> 8);
	...
	if (vcmp_channels[index].comparator)
	...
	temp = ((*vcmp_ctrl_regs[ch].vcmp_datm << 8) | *vcmp_ctrl_regs[ch].vcmp_datl);
	ccprints("threshold = 0x%x ", temp);
	ccprints("set condition, index = %d, ch = %d, vcmp_reg = 0x%x", index, ch, *vcmp_ctrl_regs[ch].vcmp_ctrl);
}
...
void volt_comp_interrupt(void)
{
	...
	//task_disable_irq(IT83XX_IRQ_V_COMP);
	...
	/* Get asserted voltage comparator channel */
	...
	*vcmp_ctrl_regs[ch].vcmp_ctrl &= ~ADC_VCMP_CMPEN;
	...
	if (ch <= 2) {
		...
		//need clear GPIO status ? GPIO status wired
		IT83XX_ADC_VCMPSTS = BIT(ch + 4);
	} else {
		IT83XX_ADC_VCMPSTS2 = BIT(ch - 3);
		//need clear GPIO status ? GPIO status wired
		IT83XX_ADC_VCMPSTS2 = BIT(ch + 1);
	}
	...
	if (ch == CHIP_VCMP_CH0) {
		if (*vcmp_ctrl_regs[ch].vcmp_ctrl & ADC_VCMP_GREATER_THRESHOLD) {
			...
			vcmp_channels[index].comparator = LESS_EQUAL_THRESHOLD;
			ccprints("INT detect High, switch detect Low");
		} else {
			vcmp_channels[index].threshold = 2800;
			vcmp_channels[index].comparator = GREATER_THRESHOLD;
...
static void voltage_comparator_init(void)
{
	...
	ccprints("voltage comparator init");
	...
	for (index = 0; index < VCMP_CH_COUNT; index++) {
		...
		*vcmp_ctrl_regs[ch].vcmp_ctrl |= (vcmp_channels[index].adcx_channel & 0x07);
		if (vcmp_channels[index].adcx_channel & BIT(3))
		...
		/* Check if need comparator output to specific GPIO or not */
		if (vcmp_channels[index].output_gpio) {
			/* Enable comparator output to specific GPIO */
			IT83XX_GPIO_GRC15 |= BIT(ch);
			/* Select specific GPIO alternate mode */
			*vcmp_ctrl_regs[ch].vcmp_output_pin_ctrl = 0x00;
			/* Select specific GPIO active mode: low or high level */
			if (vcmp_channels[index].gpio_active)
				*vcmp_ctrl_regs[ch].vcmp_ctrl |=
					ADC_VCMP_GPIO_ACTIVE_LOW;
			else
				*vcmp_ctrl_regs[ch].vcmp_ctrl &=
					 ~ADC_VCMP_GPIO_ACTIVE_LOW;
		}
		...
		if (ch <= 2) {
			IT83XX_ADC_VCMPSTS = BIT(ch);
			/* Clear GPIO status ? waiting ADC not trigger, seems not */
			IT83XX_ADC_VCMPSTS = BIT(ch + 4);
		} else {
			IT83XX_ADC_VCMPSTS2 = BIT(ch - 3);
			/* Clear GPIO status ? waiting ADC not trigger, seems not */
