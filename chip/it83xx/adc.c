...
void set_voltage_comparator_condition(int index, int ch)
{
	...
	/* Threshold volt = 3000mv * CMP0THRDAT[9:0] / 1024 */
	...
	*vcmp_ctrl_regs[ch].vcmp_datm = (uint8_t)(temp >> 8);
	...
	/*temp = (vcmp_channels[index].comparator << 5) |
		(vcmp_channels[index].trigger_level << 4) |
		(vcmp_channels[index].adcx_channel)
	*vcmp_ctrl_regs[ch].vcmp_ctrl = temp;*/
...
void volt_comp_interrupt(void)
{
	...
	ch = 0xFF;
	/* Only handle one vcmp channel at a time */
