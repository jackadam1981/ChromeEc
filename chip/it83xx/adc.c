...
void clear_vcmp_status(int index)
{
	if (index <= CHIP_VCMP2)
...
void vcmp_enable(int index, int enable)
{
	if (enable) {
		...
		*vcmp_ctrl_regs[index].vcmp_ctrl |= ADC_VCMPX_CMPINTEN;
		/* Start voltage comparator */
		*vcmp_ctrl_regs[index].vcmp_ctrl |= ADC_VCMPX_CMPEN;
	} else {
	...
	for (index = 0; index < BOARD_VCMP_COUNT; index++) {
		...
		*vcmp_ctrl_regs[index].vcmp_ctrl |=
		...
		if (vcmp_list[index].adc_ch & VCMPX_ADC_CH_MASK_H)
			*vcmp_ctrl_regs[index].vcmp_adc_chm |=
				ADC_VCMPX_VCMPCSELM;

		/* Set voltage comparator conditions */
		set_voltage_comparator_condition(index);
