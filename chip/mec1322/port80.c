...
static void restart_timer(void)
{
	...
	/* Set the count to 1000us. (1ms). */
	MEC1322_TMR16_CNT(1) = 1000;
...
static void port_80_interrupt_init(void)
{
	...
	/* The timers are driven by a 48MHz oscillator. */
	/* Prescale down to 1MHz. 48MHz/48 -> 1MHz*/
	val = MEC1322_TMR16_CTL(1);
	val = (val & 0xFFFF) | (47 << 16);
	/* Do not automatically restart the count. */
	val &= ~(1 << 3);
	...
	val = MEC1322_TMR16_IEN(1);
	val |= 1;
	MEC1322_TMR16_IEN(1) = val;

	/* Timer16_1 is on GIRQ23, bit 1. */
	val = MEC1322_INT_SOURCE(23);
	val |= (1 << 1);
	MEC1322_INT_SOURCE(23) = val;
	val = MEC1322_INT_ENABLE(23);
	val |= (1 << 1);
	MEC1322_INT_ENABLE(23) = val;

	/* Enable IRQ vector 23. */
	val = MEC1322_INT_BLK_EN;
	val |= (1 << 23);
	MEC1322_INT_BLK_EN = val;
