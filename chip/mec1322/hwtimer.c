...
{
	int overflow = MEC1322_TMR32_STS(timer_id) & 1;
	
	/* Clear interrupt */
	MEC1322_TMR32_STS(timer_id) |= 1;

	/* If IRQ is from timer 0, 32-bit timer overflowed */
	process_timers(timer_id == 0 && overflow);
}
...
static void configure_timer(int timer_id)
{
	...
	MEC1322_TMR32_PRE(timer_id) = 1000000; /*0xffffffff;*/
}
...
int __hw_clock_source_init(uint32_t start_t)
{
	...
	MEC1322_TMR32_CNT(0) = 1000000 - start_t;
