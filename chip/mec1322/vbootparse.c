...
uint32_t vboot_hash_get_image_size(enum system_image_copy_t copy)
{
	...
	if ((image_size <= 0) | (image_addr == 0xFFFFFFFFF))
		return 0;

	ccprintf("image addr %x\n!!!", image_addr);
	...
	while (image_size > 0) {
		...
		watchdog_reload();
		data_size = MIN(CHUNK_SIZE, image_size);

		spi_flash_read(databuf,
				(image_addr + image_size - data_size),
				data_size);
