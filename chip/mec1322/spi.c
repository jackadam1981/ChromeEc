...
static const struct dma_option spi_rx_option = {
	MEC1322_DMAC_SPI0_RX, (void *)&MEC1322_SPI_RD(0),
		    ...
		    uint8_t *rxdata, int rxlen)
{
	...
	if (ret != EC_SUCCESS)
		goto spi_transaction_err;

	for (i = 0; i < rxlen; ++i) {
		MEC1322_SPI_TD(CONFIG_SPI_PORT) = dummy;
		ret = wait_byte();
		if (ret != EC_SUCCESS)
			goto spi_transaction_err;
		rxdata[i] = MEC1322_SPI_RD(CONFIG_SPI_PORT);
	}
...
{
	int ret = dma_wait(MEC1322_DMAC_SPI0_RX);
