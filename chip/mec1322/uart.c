...
int uart_tx_ready(void)
{
	 ...
	 */
	return tx_fifo_space > 0 || MEC1322_UART_LSR & (1 << 5);
}
...
void uart_write_char(char c)
{
	...
	/*
	 * We are about to write a character, so decrease the number of empty
	 * FIFO space by one. If tx_fifo_space is already 0, we've already
	 * checked that FIFO is empty above. In this case, reset tx_fifo_space
	 * to 15.
	 */
	tx_fifo_space = (tx_fifo_space - 1 + 16) % 16;
	MEC1322_UART_TB = c;
}
