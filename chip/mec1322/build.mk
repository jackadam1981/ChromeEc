...
mec1322-lfw-flat = $(out)/$(BLD)/$(mec1322-lfw)-lfw.flat

# build these specifically for lfw with -lfw suffix
objs_lfw = $(patsubst %, $(out)/$(BLD)/%-lfw.o, \
...
objs_lfw += $(out)/$(BLD)/common/version.o

dirs-y+=chip/$(CHIP)/lfw

# objs with -lfw suffix are to include lfw's gpio
$(out)/$(BLD)/%-lfw.o: private CC+=-Iboard/$(BOARD)/lfw
$(out)/$(BLD)/%-lfw.o: %.c
	$(call quiet,c_to_o,CC     )

# let lfw's elf link only with selected objects
$(out)/$(BLD)/%-lfw.elf: private objs = $(objs_lfw)
$(out)/$(BLD)/%-lfw.elf: override shlib :=
$(out)/$(BLD)/%-lfw.elf: %.ld $(objs_lfw)
	$(call quiet,elf,LD     )
