...
#define HEAVY_SLEEP_RECOVER_TIME_USEC 1000
#define SET_HTIMER_MATCH_DELAY_USEC 0
...
static void system_set_htimer_alarm(uint32_t seconds, uint32_t microseconds)
{
	if (seconds || microseconds) {
		...
		if (seconds > 2) {
			...
			MEC1322_HTIMER_CONTROL = 1; /* 0.125 sec */
			MEC1322_HTIMER_PRELOAD =
			(seconds * 8 + microseconds / 125000);
		} else {
			MEC1322_HTIMER_CONTROL = 0; /* 30.5 usec */
			MEC1322_HTIMER_PRELOAD =
...
static timestamp_t system_get_htimer(void)
{
	...
	if (MEC1322_HTIMER_CONTROL == 1)
		time.le.lo = (uint32_t)(count * 125000);  /* 0.125 sec */
	else
		time.le.lo = (uint32_t)(count * 71 / 2);   /* 30.5 uSec */
	time.le.hi = 0;
...
#ifdef MY_DEBUG
static timestamp_t static_t0;
static timestamp_t static_t1;
static timestamp_t static_tc;
static uint32_t static_ht_t1;
static uint32_t static_next_delay;
static uint64_t static_idle_dsleep_time_us;
#endif
...
void __idle(void)
{
	...
	while (1) {
		...
		if (DEEP_SLEEP_ALLOWED && time_for_dsleep) {
			 ...
			 * mode timers stops.  And timer should be corrected
			 * after wakeup.
...
#ifdef MY_DEBUG
	ccprintf("static_next_delay (uS):		     %u,0x%08x\n\n",
					static_next_delay, static_next_delay);
	ccprintf("static_ht_t1 (uS):                   %u , 0x%08x\n\n",
						static_ht_t1, static_ht_t1);
	ccprintf("Initial time t0:                     %.6ld\n", static_t0.val);
	ccprintf("Corrected time t1:                   %.6ld\n", static_t1.val);
	ccprintf("after correction tc:                 %.6ld\n", static_tc.val);
#endif
	return EC_SUCCESS;
}
