 ...
 *
 */
static void lpc_enable_wake_mask_for_lid_open(void)
{
 ...
 *
 */
static void lpc_disable_wake_mask_for_lid_open(void)
{
	if (chipset_in_state(CHIPSET_STATE_STANDBY | CHIPSET_STATE_ON)) {
		lpc_set_host_event_mask(LPC_HOST_EVENT_WAKE, 0);
...
DECLARE_HOOK(HOOK_CHIPSET_SUSPEND, lpc_enable_wake_mask_for_lid_open,
	     HOOK_PRIO_DEFAULT);

DECLARE_HOOK(HOOK_CHIPSET_RESUME, lpc_disable_wake_mask_for_lid_open,
	     HOOK_PRIO_DEFAULT);
#endif
