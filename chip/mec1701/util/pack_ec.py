...
# Python ecdsa module https://pypi.python.org/pypi/ecdsa
# Includes an implementation of RFC6979
from ecdsa import SigningKey, VerifyingKey, NIST256p

# pycrypto wrapper around libopenssl is in chroot
from Crypto.Cipher import AES

# Chromiumos chroot does have a version of Python cryptography
# How buggy is it? Does it implement RFC6979?
...
#
def SignByteArray(data, signing_key):
  if signing_key != None:
    print("Sign with ECDSA")
    sigb = bytearray(signing_key.sign_deterministic(data))
  else:
...
def encrypt(aes256_key, aes_iv, data):
  #print("encrypt:")
  #printByteArrayAsHex(aes256_key, "aes256_key")
  #printByteArrayAsHex(aes_iv, "aes_iv")
  key = bytes(aes256_key)
  iv = bytes(aes_iv)
  cipher = AES.new(key, AES.MODE_CBC, iv)
  data_bytes = bytes(data)
  edata = cipher.encrypt(data_bytes)
  return edata
...
# TODO - If encryption enabled then encrypt RO data length = image_size - 32
...
#
def PacklfwRoImage(rorw_file, loader_file, image_size, aes256_key, aes_iv):
  ...
  if aes256_key:
    print("Encrypting LWF + EC_RO")
    ro = encrypt(aes256_key, aes_iv, ro)
...
# ( AES-256 key of length 32 bytes,
#   AES IV of length 16 bytes,
#   ECDH shared secret of length 64 bytes
# )
#
def GetECDHInfo(args):
  ecdh_key1 = SigningKey.from_pem(open(args.ecdh_packer_key).read())
  ecdh_key2_pub = VerifyingKey.from_pem(open(args.ecdh_mec1701_pub_key).read())

  # 1. Compute K = ECScalarPointMultiply(key1.private, key2.public)
  k = ecdh_key1.privkey.secret_multiplier * ecdh_key2_pub.pubkey.point
  x = k.x()
  l1 = []
  while x:
    l1.append(int2byte(x & 0xff))
    x >>= 8

  for b in range(32 - len(l1)):
      l1.insert(0, 0)

  # debug
  print("GenECDHInfo: Length of Kx =", len(l1))

  # 2. AES-256 Key = SHA256(Kx || 0x00 || 0x00 || 0x00 || 0x01)
  l1.append(int2byte(0))
  l1.append(int2byte(0))
  l1.append(int2byte(0))
  l1.append(int2byte(1))
  xb = bytearray(l1)
  printByteArrayAsHex(xb, "Kx || 0x00000001")
  aes256_key = HashByteArray(xb)
  printByteArrayAsHex(aes256_key, "aes256_key")
  l1[35] = int2byte(2)
  xb = bytearray(l1)
  printByteArrayAsHex(xb, "Kx || 0x00000002")
  # 3. AES IV = first 16 bytes of SHA256(Kx || 0x00 || 0x00 || 0x00 || 0x02)
  aes_iv = HashByteArray(xb)[0:16]
  printByteArrayAsHex(aes_iv, "aes_iv")
  return (aes256_key, aes_iv, ecdh_key1.get_verifying_key().to_string())
...
def parseargs():
  ...
  parser.add_argument("--sign", action="store_true",
                      help="Sign Header and Payload using ECDSA",
                      default=False)
  parser.add_argument("--encrypt", action="store_true",
                      help="Encrypt payloads using AES-256",
                      default=False)
  parser.add_argument("--signing_key",
                      help="Key PEM file for ECDSA signing of header and payload",
                      default=rpath + os.sep + "signing_key.pem")
  parser.add_argument("--ecdh_packer_key",
                      help="This Packer's ECDH EC key pair PEM file",
                      default=rpath + os.sep + "ecdh_packer_key.pem")
  parser.add_argument("--ecdh_mec1701_pub_key",
                      help="MEC1701 EC public key PEM file for ECDH",
                      default=rpath + os.sep + "ecdh_mec1701_key_pub.pem")
...
def print_args(args):
  ...
  print(".sign    = ", args.sign)
  print(".encrypt = ", args.encrypt)
  print(".signing_key = ", args.signing_key)
  print(".ecdh_packer_key = ", args.ecdh_packer_key)
  print(".ecdh_mec1701_pub_key = ", args.ecdh_mec1701_pub_key)
...
# pack_ec.py -o build/glados_mec1701/ec.bin.tmp
#   -i build/glados_mec1701/ec.bin.tmp1
...
#NOTE: chip/mec1701/config_flash_layout.h
#  _rw_off = offset from start of EC Code space where RO or RW is loaded
#For MEC17xx it is immediately after LFW region (Code SRAM start + 4KB)
#
#_rw_size_str:=$(shell echo "CONFIG_RW_SIZE" | $(CPP) $(CPPFLAGS) -P \
#		-Ichip/$(CHIP) -I$(BDIR) -imacros include/config.h)
#_rw_size:=$(shell echo "$$(($(_rw_size_str)))")
#NOTE: chip/mec1701/config_flash_layout.h
#  _rw_size = 188KB = (256KB total SRAM - 32KB data SRAM - 4KB LFW)
#
#_program_memory_base_str:=$(shell echo "CONFIG_PROGRAM_MEMORY_BASE" | \
#		$(CPP) $(CPPFLAGS) -P \
#		-Ichip/$(CHIP) -I$(BDIR) -imacros include/config.h)
#_program_memory_base=$(shell echo "$$(($(_program_memory_base_str)))")
#NOTE: chip/mec1701/config_chip.h CONFIG_PROGRAM_MEMORY_BASE = 0xE0000
#
# chip/mec1701/build.mk
#CHIP_SPI_SIZE_KB?=512
#cmd_obj_to_bin = $(OBJCOPY) --gap-fill=0xff -O binary $< $@.tmp1 ; \
#		cp $@.tmp1 $@.ec_tmp1 ; \
#		 ${SCRIPTDIR}/pack_ec.py -o $@.tmp -i $@.tmp1 \
#		--loader_file $(mec1701-lfw-flat) \
...
def main():
  ...
  sk = None
  if args.sign:
      print("ECDSA signing enabled. Read signing key from file ", args.signing_key)
      sk = SigningKey.from_pem(open(args.signing_key).read())

  aes256_key = None
  aes_iv = None
  ecdh_ss = None
  if args.encrypt:
    ecdh_info = GetECDHInfo(args)
    aes256_key = ecdh_info[0]
    aes_iv = ecdh_info[1]
    ecdh_ss = ecdh_info[2]
  ...
  payload_signature = SignByteArray(payload, sk)
  ...
  header_signature = SignByteArray(header, sk)
  # debug
  ...
  print("args.input = ", args.input) # TODO file is ec.bin.tmp1 of length 16MB due to CONFIG_FLASH_SIZE change
  print("args.image_size = ", hex(args.image_size))
  ...
  # Now extend size out to 192KB, shouldn't it be 192 - 4(LFW) = 188 same as
  # CONFIG_RO_SIZE
  ...
  if args.encrypt:
    print("Encrypting EC_RW")
    payload_rw = encrypt(aes256_key, aes_iv, payload_rw)
  ...
  payload_rw_sig = SignByteArray(payload_rw, sk)
  ...
  header_rw_sig = SignByteArray(header_rw, sk)
  ...
  if args.encrypt:
    spi_list.append((offset, ecdh_ss, "ECDH Shared Secret"))
    offset += 64
