...
def main():
  ...
  # debug, compute SHA256 of EC_RO except for last 32 bytes.
  # skip over 4KB LFW
  # Store SHA256 digest in last 32 bytes of payload
  ro_sha256 = HashByteArray(payload[LFW_SIZE:(payload_len - 32)])
  printByteArrayAsHex(ro_sha256, "SHA256(EC_RO)")
  i = 0
  for b in ro_sha256:
    payload[payload_len - len(ro_sha256) + i] = b
    i += 1
  ...
  # debug, compute SHA256 of EC_RW except for last 32 bytes
  # Store SHA256 digest in last 32 bytes of EC_RW
  rw_sha256 = HashByteArray(payload_rw[:(payload_rw_len - 32)])
  printByteArrayAsHex(rw_sha256, "SHA256(EC_RW)")
  i = 0
  for b in rw_sha256:
    payload_rw[payload_rw_len - len(rw_sha256) + i] = b
    i += 1
