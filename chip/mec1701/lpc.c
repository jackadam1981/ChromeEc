...
static void update_host_event_status(void)
{
	...
	TRACE0(74, LPC, 0, "LPC update_host_event_status");
...
static void lpc_send_response_packet(struct host_packet *pkt)
{
	...
	TRACE1(75, LPC, 0, "LPC Set EC2OS(1,0)=0x%02x", pkt->driver_result);

	p8 = (uint8_t *)pkt->response;
	if (p8 != NULL) {
		for (i = 0; i < pkt->response_size; i++)
			TRACE2(76, LPC, 0, "response[%d] = 0x%02x", i, *p8++);
	}
...
static void lpc_sysjump(void)
{
	...
	TRACE0(77, LPC, 0, "LPC HOOK_SYSJUMP");
...
static void lpc_resume(void)
{
	...
	TRACE0(78, HOOK, 0, "HOOK_CHIPSET_RESUME - lpc_resume");
...
static void lpc_init(void)
{
	...
	TRACE0(79, HOOK, 0, "HOOK_INIT - lpc_init");
...
void emi0_interrupt(void)
{
	...
	TRACE1(80, LPC, 0, "EMI0 H2E = 0x%02x", h2e);
	port_80_write(h2e);
}
...
void acpi_0_interrupt(void)
{
	...
	if (acpi_ap_to_ec(is_cmd, value, &result)) {
		...
		TRACE1(84, LPC, 0, "ACPI EC0 ISR: result=0x%02x", result);
...
void acpi_1_interrupt(void)
{
	...
	TRACE1(87, LPC, 0, "ACPI EC1 ISR: sts=0x%02x", st);
	...
	if (host_cmd_args.command == EC_COMMAND_PROTOCOL_3) {
		...
		TRACE0(90, LPC, 0, "ACPI EC1 ISR: Invalid protocol");
		/* Old style command unsupported */
		host_cmd_args.result = EC_RES_INVALID_COMMAND;

		/* Hand off to host command handler
		 * TODO MCHP - BUG!!! Observed usage fault when this
		 * routine is called.
	...
	TRACE0(91, LPC, 0, "ACPI EC1 ISR: Exit");
}
...
{
	TRACE2(92, KBD, 0, "chr=0x%02x send_irq=%d", chr, send_irq);
...
static int lpc_get_protocol_info(struct host_cmd_handler_args *args)
{
	...
	TRACE0(93, LPC, 0, "Handler EC_CMD_GET_PROTOCOL_INFO");
