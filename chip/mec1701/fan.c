...
void fan_set_enabled(int ch, int enabled)
{
	if (in_rpm_mode) {
		if (enabled)
			...
			MEC17XX_FAN_TARGET(0) = FAN_OFF_TACH;
	} else {
		if (enabled)
			fan_set_duty(ch, duty_setting);
		else
			MEC17XX_FAN_SETTING(0) = 0;
	}
	clear_status();
}
...
{
	if (in_rpm_mode)
		return (MEC17XX_FAN_TARGET(0) & 0xff00) != 0xff00;
	else
		return !!MEC17XX_FAN_SETTING(0);
}
...
void fan_set_duty(int ch, int percent)
{
	...
	MEC17XX_FAN_SETTING(0) = percent * 255 / 100;
	clear_status();
}
...
{
	return !!(MEC17XX_FAN_CFG1(0) & (1 << 7));
}

void fan_set_rpm_mode(int ch, int rpm_mode)
{
	if (rpm_mode)
		MEC17XX_FAN_CFG1(0) |= 1 << 7;
	else
		MEC17XX_FAN_CFG1(0) &= ~(1 << 7);
	clear_status();
}
...
{
	if ((MEC17XX_FAN_READING(0) >> 8) == 0xff)
		return 0;
	else
		return TACH_TO_RPM(MEC17XX_FAN_READING(0) >> 3);
}
...
{
	rpm_setting = rpm;
	MEC17XX_FAN_TARGET(0) = RPM_TO_TACH(rpm) << 3;
	clear_status();
}

enum fan_status fan_get_status(int ch)
{
	uint8_t sts = MEC17XX_FAN_STATUS(0);
...
void fan_channel_setup(int ch, unsigned int flags)
{
	...
	MEC17XX_PCR_SLP_DIS_DEV(MEC17XX_PCR_RPMPWM0);
	...
	if (flags & FAN_USE_RPM_MODE)
		MEC17XX_FAN_CFG1(0) = 0xab;
	else
		MEC17XX_FAN_CFG1(0) = 0x2b;
	MEC17XX_FAN_CFG2(0) = 0x1a;
	clear_status();
}
