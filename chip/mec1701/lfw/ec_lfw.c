...
#ifdef CONFIG_CHIP_LFW_USE_ROM_SPI
static int spi_flash_readloc(uint32_t *buf, uint32_t offset,
		uint32_t bytes, uint32_t spi_cmd)
{
	uint64_t m, m2;
	uint32_t u, u2, u3;
...
/*
 * For testing LFW SPI read data integrity and/or SPI image generation.
 * Assumes image length is a multiple of 64 bytes.
 * EC_RO/RW SHA256 is located in last 32 bytes of RO/RW binaries.
 */
static int check_spi_image(uint32_t img_type,
			uint32_t *digest, uint32_t *block2)
{
	const uint32_t *img;
	const uint8_t *rem;
	uint32_t i, data_len;
	...
	switch (img_type) {
	case SYSTEM_IMAGE_RO: /* SHA256(EC_RO) */
		img = (const uint32_t *)(CONFIG_RO_MEM_OFF +
					 CONFIG_PROGRAM_MEMORY_BASE);
		data_len = CONFIG_RO_SIZE;
		break;
	case SYSTEM_IMAGE_RW: /* SHA256(EC_RW) */
		img = (const uint32_t *) (CONFIG_RW_MEM_OFF +
				    CONFIG_PROGRAM_MEMORY_BASE);
	...
	data_len -= 32; /* don't include expected digest in hash */

	rom_sha_init(MEC17XX_ROM_SHA_MODE_256, digest);
	trace12(0, LFW, 0, "SHA256 Update at 0x%08X of length 0x%08X",
		(uint32_t)img, data_len);
	rom_sha_update((const uint32_t *)img, (data_len >> 6), 0x05);
	rc = hash_done(TMOUT_SHA256_HW_UPDATE);
	if (rc != EC_SUCCESS) {
		trace0(0, LFW, 0, "SHA256 finalize timeout");
		return EC_ERROR_TIMEOUT;
	}

	rem = (const uint8_t *)img;
	/* point to any remaining bytes */
	rem += (data_len & ~(0x3F));

	rom_sha_final(block2, data_len, rem, 0x05);
	...
	i = 0;
	/* point to SHA256 digest embedded in image */
	img += (data_len >> 2);
	trace11(0, LFW, 0, "SHA256 embedd in image at 0x%08X", (uint32_t)img);
	while (i < 32/4) {
		if (digest[i] != img[i]) {
			trace0(0, LFW, 0, "spi load SHA256 mismatch");
			break;
		}
		i++;
	}
	if (i == 8) {
		trace0(0, LFW, 0, "spi_load_image SHA256 OK");
		return EC_SUCCESS;
	}
...
#ifdef LFW_DEBUG_USE_CHIP_SHA256
		rc = check_spi_image(SYSTEM_IMAGE_RW,
				sha256_digest, sha256_block2);
		if (rc != EC_SUCCESS) {
			uart_puts("lfw-RW load image check failed!\n");
			while (1);
		}
#endif
		break;
...
#ifdef LFW_DEBUG_USE_CHIP_SHA256
		rc = check_spi_image(SYSTEM_IMAGE_RO,
				sha256_digest, sha256_block2);
		if (rc != EC_SUCCESS) {
			uart_puts("lfw-RO load image check failed!\n");
			while (1);
		}
#endif
		break;
