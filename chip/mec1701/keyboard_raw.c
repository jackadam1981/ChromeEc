...
void keyboard_raw_init(void)
{
	...
	MEC17XX_INT_ENABLE(MEC17XX_KS_GIRQ) = MEC17XX_KS_GIRQ_BIT;
	/*
	 * TODO not need for direct mode interrupts only aggregated.
	 * MEC17XX_INT_BLK_EN |= 1 << MEC17XX_KS_GIRQ;
	*/
	MEC17XX_KS_KSI_INT_EN = 0xff;
}

void keyboard_raw_task_start(void)
{
	task_enable_irq(MEC17XX_IRQ_KSC_INT);
}

test_mockable void keyboard_raw_drive_column(int out)
{
	if (out == KEYBOARD_COLUMN_ALL) {
		MEC17XX_KS_KSO_SEL = 1 << 5; /* KSEN=0, KSALL=1 */
#ifdef CONFIG_KEYBOARD_COL2_INVERTED
		gpio_set_level(GPIO_KBD_KSO2, 1);
#endif
	} else if (out == KEYBOARD_COLUMN_NONE) {
		MEC17XX_KS_KSO_SEL = 1 << 6; /* KSEN=1 */
#ifdef CONFIG_KEYBOARD_COL2_INVERTED
		...
		if (out == 2) {
			...
			MEC17XX_KS_KSO_SEL = out + CONFIG_KEYBOARD_KSO_BASE;
			gpio_set_level(GPIO_KBD_KSO2, 0);
		}
#else
		MEC17XX_KS_KSO_SEL = out + CONFIG_KEYBOARD_KSO_BASE;
...
test_mockable int keyboard_raw_read_rows(void)
{
	...
	b1 = MEC17XX_KS_KSI_INPUT;
	b2 = (b1 & 0xff) ^ 0xff;
	TRACE2(70, MEC, 0, "raw_read_rows: KSI_IN=0x%02x Invert=0x%02x",
	       b1, b2);

	/* Invert it so 0=not pressed, 1=pressed */
	/* return (MEC17XX_KS_KSI_INPUT & 0xff) ^ 0xff; */
	return b2;
}
...
{
	if (enable) {
		MEC17XX_INT_SOURCE(MEC17XX_KS_GIRQ) = MEC17XX_KS_GIRQ_BIT;
		task_clear_pending_irq(MEC17XX_IRQ_KSC_INT);
		task_enable_irq(MEC17XX_IRQ_KSC_INT);
	} else {
		task_disable_irq(MEC17XX_IRQ_KSC_INT);
	}
}
...
void keyboard_raw_interrupt(void)
{
	TRACE3(71, MEC, 0,
	       ...
	       MEC17XX_KS_KSI_INPUT,
	       MEC17XX_KS_KSI_STATUS,
	       MEC17XX_KS_KSI_INT_EN);

	/* Clear interrupt status bits */
	MEC17XX_KS_KSI_STATUS = 0xff;
...
DECLARE_IRQ(MEC17XX_IRQ_KSC_INT, keyboard_raw_interrupt, 1);
...
int keyboard_factory_test_scan(void)
{
	...
	for (i = 0; i < keyboard_factory_scan_pins_used; i++) {
		...
		for (j = 0; j < i; j++) {
			 ...
			 */
			val = MEC17XX_GPIO_CTL(keyboard_factory_scan_pins[j][0],
