 ...
 * GIRQ=21, aggregator bit = 1
 * TODO - If NVIC direct connect interrupts are used for all peripherals 
...
static void system_set_htimer_alarm(uint32_t seconds, uint32_t microseconds)
{
	if (seconds || microseconds) {
		...
		if (seconds > 2) {
			...
			MEC17XX_HTIMER_CONTROL(0) = 1; /* 0.125(=1/8) per clock */
...
static void prepare_for_deep_sleep(void)
{
	...
	/* 
	* TODO - is the best way to shut down blocks?
	* SLEEP_ALL would do this?
	* MEC17xx does not have an internal ADC Vref 
	...
	*/
	...
	/* TODO - should we use SLEEP_ALL ? */
	MEC17XX_PCR_SYS_SLP_CTL = 0x1;  /* heavysleep */
...
static void resume_from_deep_sleep(void)
{
	...
	MEC17XX_PCR_SLP_EN0 &= MEC17XX_PCR_SLP_EN0_WAKE;  
	MEC17XX_PCR_SLP_EN1 &= MEC17XX_PCR_SLP_EN1_WAKE;
	MEC17XX_PCR_SLP_EN2 &= MEC17XX_PCR_SLP_EN2_WAKE;
	MEC17XX_PCR_SLP_EN3 &= MEC17XX_PCR_SLP_EN3_WAKE;
	MEC17XX_PCR_SLP_EN4 &= MEC17XX_PCR_SLP_EN4_WAKE;
...
void __idle(void)
{
	...
	TRACE0(0, IDLE, 0, "MEC1701 low power idle task started");
		...
		if (DEEP_SLEEP_ALLOWED && time_for_dsleep) {
			...
			if ((sleep_mask & SLEEP_MASK_CONSOLE) &&
				...
				if (LOW_SPEED_DEEP_SLEEP_ALLOWED) {
					CPRINTS("MEC1701 Disable console in deepsleep");
					TRACE0(0, IDLE, 0, "MEC1701 disable console in deepsleep");
				}
			}
