...
void clock_init(void)
{
	...
	while (!(MEC17XX_PCR_CHIP_OSC_ID & 0x100))
		;
#endif
	TRACE0(22, MEC, 0, "PLL OSC is Locked");
#ifndef LFW
	dummy = shared_mem_size();
	TRACE11(23, MEC, 0, "Shared Memory size = 0x%08x", (uint32_t)dummy);
#endif
}
...
#ifdef CONFIG_LOW_POWER_IDLE
/*
 * TODO MCHP re-implement per MEC1701 deep sleep requirements
 */
/**
...
/**
 * Additional settings before we enter deep sleep
 */
static void mec1701_low_power(void)
{
	/* int i;*/

	/* Do we need this here or has to be taken care at task level */
	/* Disable SMB controllers */
	/* for (i=0; i < 4; i++)
	 *	MEC17XX_I2C_CONFIG(i) &= ~(1 << 10);
	 */

#if 0
	/* Enabled by BootROM */
	MEC17XX_TFDP_CTRL &= ~1;
	MEC17XX_QMSPI0_MODE &= ~1;
#endif

	/* Disable ESPI block */
	MEC17XX_ESPI_ACTIVATE &= ~1;
	/* Disable Aleternate NVIC */
	MEC17XX_EC_INT_CNTRL &= ~1;
...
static void prepare_for_deep_sleep(void)
{
	...
	TRACE0(24, MEC, 0, "Prepare for Deep Sleep");
...
void __idle(void)
{
	...
	while (1) {
		...
		if (DEEP_SLEEP_ALLOWED && time_for_dsleep) {
			TRACE0(25, MEC, 0, "Enough time for Deep Sleep");
			/*
