...
static uint32_t pcr_slp_en[MEC17XX_PCR_SLP_RST_REG_MAX];
static uint32_t pcr_clk_req[MEC17XX_PCR_SLP_RST_REG_MAX];
static uint32_t ecia_result[MEC17XX_INT_GIRQ_NUM];
#endif
...
{
	MEC17XX_PCR_SLP_DIS_DEV(MEC17XX_PCR_HTMR0);
	MEC17XX_INT_ENABLE(MEC17XX_HTIMER_GIRQ) =
			MEC17XX_HTIMER_GIRQ_BIT(0);
...
static void system_reset_htimer_alarm(void)
{
	...
	MEC17XX_INT_SOURCE(MEC17XX_HTIMER_GIRQ) =
			MEC17XX_HTIMER_GIRQ_BIT(0);
}
...
static void print_pcr_regs(void)
{
	...
	for (i = 0; i < 5; i++) {
		trace12(0, MEC, 0, "REG  SLP_EN[%d] = 0x%08X",
			i, MEC17XX_PCR_SLP_EN(i));
		trace12(0, MEC, 0, "REG CLK_REQ[%d] = 0x%08X",
			i, MEC17XX_PCR_CLK_REQ(i));
	}
}
...
static void print_ecia_regs(void)
{
	...
	for (i = MEC17XX_INT_GIRQ_FIRST;
			i <= MEC17XX_INT_GIRQ_LAST; i++)
		trace12(0, MEC, 0, "GIRQ[%d].Result = 0x%08X",
			i, MEC17XX_INT_RESULT(i));
}
...
static void save_regs(void)
{
	...
	for (i = 0; i < MEC17XX_PCR_SLP_RST_REG_MAX; i++) {
		pcr_slp_en[i] = MEC17XX_PCR_SLP_EN(i);
		pcr_clk_req[i] = MEC17XX_PCR_CLK_REQ(i);
	}

	for (i = 0; i < MEC17XX_INT_GIRQ_NUM; i++)
		ecia_result[i] =
			MEC17XX_INT_RESULT(MEC17XX_INT_GIRQ_FIRST + i);
}
...
static void print_saved_regs(void)
{
	...
	for (i = 0; i < MEC17XX_INT_GIRQ_NUM; i++)
		trace12(0, BRD, 0, "GIRQ[%d].Result = 0x%08X",
			(i+MEC17XX_INT_GIRQ_FIRST), ecia_result[i]);
}
#endif /* #ifdef CONFIG_MEC17XX_DEEP_SLP_DEBUG */
...
static void prepare_for_deep_sleep(void)
{
	...
	MEC17XX_INT_DISABLE(MEC17XX_TMR32_GIRQ) =
			MEC17XX_TMR32_GIRQ_BIT(0) +
			MEC17XX_TMR32_GIRQ_BIT(1);
	MEC17XX_INT_SOURCE(MEC17XX_TMR32_GIRQ) =
			MEC17XX_TMR32_GIRQ_BIT(0) +
			MEC17XX_TMR32_GIRQ_BIT(1);
	MEC17XX_INT_DISABLE(MEC17XX_TMR16_GIRQ) =
			MEC17XX_TMR16_GIRQ_BIT(0);
	MEC17XX_INT_SOURCE(MEC17XX_TMR16_GIRQ) =
			MEC17XX_TMR16_GIRQ_BIT(0);
	...
	MEC17XX_INT_SOURCE(22) = MEC17XX_INT22_WAKE_ONLY_ESPI;
	MEC17XX_INT_ENABLE(22) = MEC17XX_INT22_WAKE_ONLY_ESPI;
	#else
	MEC17XX_ESPI_ACTIVATE &= ~1;
	#endif
#else
	#ifdef CONFIG_POWER_S0IX
	MEC17XX_INT_SOURCE(22) = MEC17XX_INT22_WAKE_ONLY_LPC;
	MEC17XX_INT_ENABLE(22) = MEC17XX_INT22_WAKE_ONLY_LPC;
	#else
	MEC17XX_LPC_ACT |= 1;
	#endif
#endif
...
static void resume_from_deep_sleep(void)
{
...
#ifdef CONFIG_ESPI
	#ifdef CONFIG_POWER_S0IX
	MEC17XX_INT_DISABLE(22) = MEC17XX_INT22_WAKE_ONLY_ESPI;
	MEC17XX_INT_SOURCE(22) = MEC17XX_INT22_WAKE_ONLY_ESPI;
	#else
	MEC17XX_ESPI_ACTIVATE |= 1;
	#endif
#else
	#ifdef CONFIG_POWER_S0IX
	MEC17XX_INT_DISABLE(22) = MEC17XX_INT22_WAKE_ONLY_LPC;
	MEC17XX_INT_SOURCE(22) = MEC17XX_INT22_WAKE_ONLY_LPC;
	#else
	MEC17XX_LPC_ACT |= 1;
	#endif
	...
	MEC17XX_TMR32_CTL(1) |= 1;
	MEC17XX_TMR16_CTL(0) |= 1;
	MEC17XX_INT_ENABLE(MEC17XX_TMR32_GIRQ) =
			MEC17XX_TMR32_GIRQ_BIT(0) +
			MEC17XX_TMR32_GIRQ_BIT(1);
	MEC17XX_INT_ENABLE(MEC17XX_TMR16_GIRQ) =
			MEC17XX_TMR16_GIRQ_BIT(0);
...
void __idle(void)
{
	...
	while (1) {
		...
		/* __hw_clock_event_get() is next
		 * programmed timer event */
		...
		if (DEEP_SLEEP_ALLOWED && time_for_dsleep) {
			...
			/*
			 * Since MEC17XX's heavy sleep mode requires all
			...
			if (uart_ready_for_deepsleep) {
				...
				MEC17XX_PCR_SYS_SLP_CTL |=
						MEC17XX_PCR_SYS_SLP_HEAVY;
				MEC17XX_PCR_SYS_SLP_CTL |=
						MEC17XX_PCR_SYS_SLP_ALL;
