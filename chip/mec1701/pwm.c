...
void pwm_keep_awake(void)
{
	if (pwm_keep_awake_mask) {
		/* b[4,20:27] */
		MEC17XX_PCR_SLP_EN1 &= ~(pwm_keep_awake_mask &
					 (MEC17XX_PCR_SLP_EN1_PWM_ALL));
		/* b[31] */
		MEC17XX_PCR_SLP_EN3 &= ~(pwm_keep_awake_mask &
					 (MEC17XX_PCR_SLP_EN3_PWM_ALL));
		/* b[1:0] */
		MEC17XX_PCR_SLP_EN4 &= ~(pwm_keep_awake_mask &
					 (MEC17XX_PCR_SLP_EN4_PWM_ALL));
	} else {
...
static const uint16_t pwm_pcr[MEC17XX_PWM_ID_MAX] = {
	MEC17XX_PCR_PWM0,
	MEC17XX_PCR_PWM1,
	MEC17XX_PCR_PWM2,
	MEC17XX_PCR_PWM3,
...
{
	if ((pwm_id < 0) || (pwm_id > MEC17XX_PWM_ID_MAX))
		return;

	if (sleep_en)
		MEC17XX_PCR_SLP_EN_DEV(pwm_pcr[pwm_id]);
	else
		MEC17XX_PCR_SLP_DIS_DEV(pwm_pcr[pwm_id]);
}
