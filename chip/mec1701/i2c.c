 ...
 * MEC17XX_I2C0_n defines
 */
static const char __attribute__((unused))
*i2c_port_names[MEC17XX_I2C_PORT_COUNT] = {
	"0_0",
	"N/A", /* MEC17xx I2C Port 1 pins not present */
	"0_2",
	"1_3",
	"2_4",
	"3_5",
...
static const uint16_t i2c_controller_pcr[MEC17XX_I2C_CTRL_MAX] = {
	MEC17XX_PCR_I2C0,
	MEC17XX_PCR_I2C1,
	MEC17XX_PCR_I2C2,
	MEC17XX_PCR_I2C3
...
{
	if ((controller < 0) || (controller > MEC17XX_I2C_CTRL_MAX))
		return;

	if (sleep_en)
		MEC17XX_PCR_SLP_EN_DEV(i2c_controller_pcr[controller]);
	else
		MEC17XX_PCR_SLP_DIS_DEV(i2c_controller_pcr[controller]);
}
...
{
	MEC17XX_INT_ENABLE(MEC17XX_I2C_GIRQ) =
			MEC17XX_I2C_GIRQ_BIT(controller);
	task_enable_irq(MEC17XX_IRQ_I2C_0 + controller);
}

static void disable_controller_irq(int controller)
{
	MEC17XX_INT_DISABLE(MEC17XX_I2C_GIRQ) =
			MEC17XX_I2C_GIRQ_BIT(controller);
...
static void configure_controller(int controller, int port, int kbps)
{
	...
	cfg = MEC17XX_I2C_CONFIG(controller) & ~0xf;
	cfg |= 0;
	MEC17XX_I2C_CONFIG(controller) = port & 0xf;
 ...
 * MEC17xx does not implement pins for port 1.
 * MEC17xx can map any port to any of the 4 controllers.
