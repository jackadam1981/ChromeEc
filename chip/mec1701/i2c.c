...
static const char *i2c_port_names[MEC17XX_I2C_PORT_COUNT] = {
	"0_0",
	"0_2",
	"1_3",
	"2_4",
...
static void configure_controller_speed(int controller, int kbps)
{
	...
	if (kbps > 400) {
		...
		MEC17XX_I2C_DATA_TIM(controller) = 0x06060601;
		MEC17XX_I2C_DATA_TIM_2(controller) = 0x06;
	} else if (kbps > 100) {
		/* Fast mode */
		/* By spec, clk low period is 1.3us min */
		t_low = MAX((int)(I2C_CLOCK * 1.3 / 1000000), period / 2 - 1);
		t_high = period - t_low - 2;
		MEC17XX_I2C_DATA_TIM(controller) = 0x040a0a01;
		MEC17XX_I2C_DATA_TIM_2(controller) = 0x0a;
	} else {
		/* Standard mode */
		t_low = t_high = period / 2 - 1;
		MEC17XX_I2C_DATA_TIM(controller) = 0x0c4d5006;
		MEC17XX_I2C_DATA_TIM_2(controller) = 0x4d;
	}

	/* Clock periods is one greater than the contents of these fields */
	MEC17XX_I2C_BUS_CLK(controller) = ((t_high & 0xff) << 8) |
					  (t_low & 0xff);
}

static void configure_controller(int controller, int kbps)
{
	MEC17XX_I2C_CTRL(controller) = CTRL_PIN;
	MEC17XX_I2C_OWN_ADDR(controller) = 0x0;
	configure_controller_speed(controller, kbps);
	MEC17XX_I2C_CTRL(controller) = CTRL_PIN | CTRL_ESO |
				       CTRL_ACK | CTRL_ENI;
	MEC17XX_I2C_CONFIG(controller) |= 1 << 10; /* ENAB */

	/* Enable interrupt */
	MEC17XX_I2C_CONFIG(controller) |= 1 << 29; /* ENIDI */
	MEC17XX_INT_ENABLE(MEC17XX_I2C_GIRQ) = MEC17XX_I2C_GIRQ_BIT(controller);
	/* Not necessary when using direct mode interrupts
	MEC17XX_INT_BLK_EN |= 1 << MEC17XX_I2C_GIRQ;
	*/
}
...
static void reset_controller(int controller)
{
	...
	MEC17XX_I2C_CONFIG(controller) |= 1 << 9;
	udelay(100);
	MEC17XX_I2C_CONFIG(controller) &= ~(1 << 9);
...
static int wait_for_interrupt(int controller, int timeout)
{
	...
	task_enable_irq(MEC17XX_IRQ_I2C_0 + controller);

	/* Wait until I2C interrupt or timeout. */
	event = task_wait_event_mask(TASK_EVENT_I2C_IDLE, timeout);

	task_disable_irq(MEC17XX_IRQ_I2C_0 + controller);
...
{
	uint8_t sts = MEC17XX_I2C_STATUS(controller);
	...
	while (!(sts & STS_NBB)) {
		...
		sts = MEC17XX_I2C_STATUS(controller);
	}
...
/* TODO - what are the values of port, zero based or any value?
...
static void select_port(int port)
{
	...
	uint32_t cfg = MEC17XX_I2C_CONFIG(controller) & ~0xf;
	MEC17XX_I2C_CONFIG(controller) = cfg | port_sel;
...
static inline int get_line_level(int controller)
{
	...
	*/
	ctrl = MEC17XX_I2C_BB_CTRL(controller);
	MEC17XX_I2C_BB_CTRL(controller) |= 1;
	ret = (MEC17XX_I2C_BB_CTRL(controller) >> 5) & 0x3;
	MEC17XX_I2C_BB_CTRL(controller) = ctrl;
	return ret;
}
		  ...
		  uint8_t *in, int in_size, int flags)
{
	...
	reg = MEC17XX_I2C_STATUS(controller);
	if (send_start &&
	    cdata[controller].transaction_state == I2C_TRANSACTION_STOPPED &&
		...
		TRACE4(0, I2C, 0, "MEC1701 I2C port %d bad status 0x%02x, SCL=%d, SDA=%d",port,reg,get_line_level(controller) & I2C_LINE_SCL_HIGH,get_line_level(controller) & I2C_LINE_SDA_HIGH);
	...
	if (out_size) {
		if (send_start) {
			MEC17XX_I2C_DATA(controller) = (uint8_t)slave_addr;
		...
		for (i = 0; i < out_size; ++i) {
			...
			MEC17XX_I2C_DATA(controller) = out[i];
		}
		ret_done = wait_byte_done(controller);
		if (ret_done)
			goto err_chip_i2c_xfer;
	...
	if (in_size) {
		...
		if (out_size || send_start) {
			...
			if (cdata[controller].transaction_state ==
			    I2C_TRANSACTION_OPEN)
				MEC17XX_I2C_CTRL(controller) = CTRL_ESO |
							       CTRL_STA |
							       CTRL_ACK |
							       CTRL_ENI;
		...
		for (i = 0; i < bytes_to_read; ++i) {
			...
			if (ret_done)
				goto err_chip_i2c_xfer;
			push_in_buf(&in, MEC17XX_I2C_DATA(controller), skip);
			skip = 0;
		}
		ret_done = wait_byte_done(controller);
		if (ret_done)
		...
		if (send_stop) {
			 ...
			 */
			MEC17XX_I2C_CTRL(controller) = CTRL_ESO | CTRL_ENI;
			push_in_buf(&in, MEC17XX_I2C_DATA(controller), skip);
			ret_done = wait_byte_done(controller);
			if (ret_done)
				goto err_chip_i2c_xfer;
			 ...
			 * issue the stop.
			 */
			push_in_buf(&in, MEC17XX_I2C_DATA(controller),
				    in_size == 1);
		}
	}

	/* Check for error conditions */
	if (MEC17XX_I2C_STATUS(controller) & (STS_LAB | STS_BER))
		return EC_ERROR_UNKNOWN;
...
/*
 * MEC17xx port numbers may skip integer values.
 * MEC17XX_I2C0_0 and MEC17XX_I2C0_1 use Controller 0.
 * Ports 3,4,5 use Controllers 1,2,3 respectively
 */
int i2c_port_to_controller(int port)
{
	if ((1ul << (uint32_t)port) & MEC17XX_I2C_PORT_MASK) {
		if (port < MEC17XX_I2C1_3) {
			return 0;
		}
		return (port - 2);
	}
...
static void handle_interrupt(int controller)
{
	...
	MEC17XX_I2C_COMPLETE(controller) &= (COMP_RW_BITS_MASK | COMP_IDLE);

	MEC17XX_INT_SOURCE(MEC17XX_I2C_GIRQ) = MEC17XX_I2C_GIRQ_BIT(controller);
...
DECLARE_IRQ(MEC17XX_IRQ_I2C_0, i2c0_interrupt, 2);
DECLARE_IRQ(MEC17XX_IRQ_I2C_1, i2c1_interrupt, 2);
DECLARE_IRQ(MEC17XX_IRQ_I2C_2, i2c2_interrupt, 2);
DECLARE_IRQ(MEC17XX_IRQ_I2C_3, i2c3_interrupt, 2);
