...
static int gpspi_wait_byte(const int ctrl)
{
	...
	while ((MEC17XX_SPI_SR(ctrl) & 0x3) != 0x3) {
...
static void gpspi_rx_fifo_clean(const int ctrl)
{
	...
	if ((MEC17XX_SPI_SR(ctrl) & 0x4) == 0x4) {
		dummy += MEC17XX_SPI_RD(ctrl);
	}
	if ((MEC17XX_SPI_SR(ctrl) & 0x2) == 0x2) {
		dummy += MEC17XX_SPI_RD(ctrl);
	}
}
/*
				...
				uint8_t *rxdata, int rxlen)
{
	...
	trace1(0, GPSPI, 0, "gpspi async: spi_device->port = 0x%02x",
			hw_port);
	trace14(0, GPSPI, 0,
		"gpspi async: td=0x%08X tlen=0x%08X rd=0x%08X rlen=0x%08X",
		(uint32_t)txdata, txlen, (uint32_t)rxdata, rxlen);

	ctrl = gpspi_port_to_ctrl_id(hw_port);

	/* Disable auto read */
	MEC17XX_SPI_CR(ctrl) &= ~(1 << 5);
...
#endif
		trace2(0, GPSPI, 0,
			"gpspi async: after tx rc = %d txdata = 0x%02x",
			ret, txdata[0]);
	}
	...
	if (ret == EC_SUCCESS)
		if ((rxlen != 0) && (rxdata != NULL)) {
			...
			if (opdma != NULL) {
				...
				MEC17XX_SPI_TD(ctrl) = 0;
				ret = EC_SUCCESS;
			}
		}

	trace1(0, GPSPI, 0, "gpspi async: exit ret = 0x%02x", ret);
...
int gpspi_transaction_flush(const struct spi_device_t *spi_device)
{
	...
	trace1(0, GPSPI, 0, "gpspi_trans_flush: port = %d",
			spi_device->port);
	...
	ret = dma_wait(chan);
	trace1(0, GPSPI, 0, "gpspi_trans_flush: dma_wait ret = %d", ret);
	...
	while ((MEC17XX_SPI_SR(ctrl) & 0x01) != 0x1) {
		if (timestamp_expired(deadline, NULL)) {
	...
	dma_disable(chan);
	dma_clear_isr(chan);
	if (MEC17XX_SPI_SR(ctrl) & 0x2)
		hw_port = MEC17XX_SPI_RD(ctrl);

	gpio_set_level(spi_device->gpio_cs, 1);

	hw_port = MEC17XX_SPI_SR(ctrl); /* debug */
	trace2(0, GPSPI, 0,
			"gpspi flush: exit ret = %d GPSPI.Status = 0x%02X",
			ret, hw_port);
...
int gpspi_enable(int hw_port, int enable)
{
	...
	trace2(0, GPSPI, 0, "gpspi_enable: HW port = %d enable = 0x%02x",
			hw_port, enable);
	...
	if (enable) {
		...
		if (ctrl)
			MEC17XX_PCR_SLP_DIS_DEV(MEC17XX_PCR_GPSPI1);
		else
			MEC17XX_PCR_SLP_DIS_DEV(MEC17XX_PCR_GPSPI0);

		/* Set enable bit in SPI_AR */
		MEC17XX_SPI_AR(ctrl) |= 0x1;

		/* Set SPDIN to 0 -> Full duplex */
		MEC17XX_SPI_CR(ctrl) &= ~(0x3 << 2);

		/* Set CLKPOL, TCLKPH, RCLKPH to 0 */
		MEC17XX_SPI_CC(ctrl) &= ~0x7;

		/* Set LSBF to 0 -> MSB first */
		MEC17XX_SPI_CR(ctrl) &= ~0x1;
	} else {
		/* soft reset */
		MEC17XX_SPI_CR(ctrl) |= (1u << 4);

		/* Clear enable bit in SPI_AR */
		MEC17XX_SPI_AR(ctrl) &= ~0x1;

		if (ctrl)
			MEC17XX_PCR_SLP_EN_DEV(MEC17XX_PCR_GPSPI1);
		else
			MEC17XX_PCR_SLP_EN_DEV(MEC17XX_PCR_GPSPI0);
	}
