...
void gpio_set_flags_by_mask(uint32_t port, uint32_t mask, uint32_t flags)
{
	...
	while (mask) {
		...
		if (flags & GPIO_POWER_DOWN) {
			val = (MEC17XX_GPIO_CTRL_PWR_OFF +
					MEC17XX_GPIO_INTDET_DISABLED);
			MEC17XX_GPIO_CTL(port, i) = val;
			continue;
		}
#endif
		val &= ~(MEC17XX_GPIO_CTRL_PWR_MASK);
		val |= MEC17XX_GPIO_CTRL_PWR_VTR;
		...
		if (flags & GPIO_OPEN_DRAIN)
			val |= (MEC17XX_GPIO_OPEN_DRAIN);
		else
			val &= ~(MEC17XX_GPIO_OPEN_DRAIN);

		if (flags & GPIO_OUTPUT) {
			val |= (MEC17XX_GPIO_OUTPUT);
			val &= ~(MEC17XX_GPIO_OUTSEL_PAR);
		} else {
			val &= ~(MEC17XX_GPIO_OUTPUT);
			val |= (MEC17XX_GPIO_OUTSEL_PAR);
		}
		...
		val &= ~(MEC17XX_GPIO_CTRL_PUD_MASK);
		if (flags & GPIO_PULL_UP)
			val |= MEC17XX_GPIO_CTRL_PUD_PU;
		else if (flags & GPIO_PULL_DOWN)
			val |= MEC17XX_GPIO_CTRL_PUD_PD;
		else
			val |= MEC17XX_GPIO_CTRL_PUD_NONE;

		/* Set up interrupt */
		val &= ~(MEC17XX_GPIO_INTDET_MASK);
		switch (flags & GPIO_INT_ANY) {
		case GPIO_INT_F_RISING:
			val |= MEC17XX_GPIO_INTDET_EDGE_RIS;
			break;
		case GPIO_INT_F_FALLING:
			val |= MEC17XX_GPIO_INTDET_EDGE_FALL;
			break;
		case GPIO_INT_BOTH: /* both edges */
			val |= MEC17XX_GPIO_INTDET_EDGE_BOTH;
			break;
		case GPIO_INT_F_LOW:
			val |= MEC17XX_GPIO_INTDET_LVL_LO;
			break;
		case GPIO_INT_F_HIGH:
			val |= MEC17XX_GPIO_INTDET_LVL_HI;
			break;
		default:
			val |= MEC17XX_GPIO_INTDET_DISABLED;
			break;
		}

		/* Set up level */
		if (flags & GPIO_HIGH)
			val |= (MEC17XX_GPIO_CTRL_OUT_LVL);
...
void gpio_power_off_by_mask(uint32_t port, uint32_t mask)
{
	...
	while (mask) {
		...
		MEC17XX_GPIO_CTL(port, i) = (MEC17XX_GPIO_CTRL_PWR_OFF +
					MEC17XX_GPIO_INTDET_DISABLED);
	}
}
