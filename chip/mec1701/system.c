...
void system_pre_init(void)
{
	 ...
	 */
	if (MEC17XX_VBAT_RAM(MEC17XX_IMAGETYPE_IDX) == SYSTEM_IMAGE_UNKNOWN) {
#ifdef SECTION_IS_RW
	...
	*/
	 ...
	 * wake.
	 */
	MEC17XX_PCR_SLP_EN0 = (MEC17XX_PCR_SLP_EN0_EFUSE);
	MEC17XX_PCR_SLP_EN1 = 0;
	MEC17XX_PCR_SLP_EN2 = 0;
	MEC17XX_PCR_SLP_EN3 = 0;
	MEC17XX_PCR_SLP_EN4 = (MEC17XX_PCR_SLP_EN4_FJCL +
			       MEC17XX_PCR_SLP_EN4_PSPI);
	 ...
	 * We clear this bit for eSPI connected EC.
	...
	TRACE1(102, MEC, 0, "System pre-init. Image type = 0x%02x", imgtype);

	/* Debug: dump some signals */
	imgtype = gpio_get_level(GPIO_PCH_RSMRST_L);
	TRACE1(103, MEC, 0, "PCH_RSMRST_L = %d", imgtype);

	imgtype = gpio_get_level(GPIO_RSMRST_L_PGOOD);
	TRACE1(104, MEC, 0, "RSMRST_L_PGOOD = %d", imgtype);
	imgtype = gpio_get_level(GPIO_POWER_BUTTON_L);
	TRACE1(105, MEC, 0, "POWER_BUTTON_L = %d", imgtype);
	imgtype = gpio_get_level(GPIO_PMIC_DPWROK);
	TRACE1(106, MEC, 0, "PMIC_DPWROK = %d", imgtype);
	imgtype = gpio_get_level(GPIO_ALL_SYS_PWRGD);
	TRACE1(107, MEC, 0, "ALL_SYS_PWRGD = %d", imgtype);
	imgtype = gpio_get_level(GPIO_AC_PRESENT);
	TRACE1(108, MEC, 0, "AC_PRESENT = %d", imgtype);
	imgtype = gpio_get_level(GPIO_PCH_SLP_SUS_L);
	TRACE1(109, MEC, 0, "PCH_SLP_SUS_L = %d", imgtype);
	imgtype = gpio_get_level(GPIO_PMIC_INT_L);
	TRACE1(110, MEC, 0, "PCH_PMIC_INT_L = %d", imgtype);
	imgtype = gpio_get_level(GPIO_PD_MCU_INT);
	TRACE1(111, MEC, 0, "PCH_PD_MCU_INT = %d", imgtype);
}
...
void _system_reset(int flags, int wake_from_hibernate)
{
	 ...
	 * WARNING: WDT requires 32KHz clock source
