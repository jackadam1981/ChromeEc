...
void system_pre_init(void)
{
	...
	*/
	...
	/*
	 * !!! New. Clear all PCR SLP_EN bits except for blocks
	 * EC does not use.
	 * MEC17xx SLP_ALL feature will save these registers, set
	 * all sleep enables and then restore original values after
	 * wake.
	 * TODO - How do we know at chipset level how many PWM or TACH
	 * or other peripherals will be used?
	 * board_config_pre_init() could set bits based on its knowledge
	 * then this routine could OR in bits is knows about.
	 * This will only work if chip code only sets sleep enables
	 * for blocks that will never be used. There's still a problem.
	 * Chip code should be first and disable most peripherals and 
	 * board code enables what it needs.
	 */
#ifdef CONFIG_CHIPSET_DEBUG
	MEC17XX_PCR_SLP_EN0 = 0x07;
#else
	MEC17XX_PCR_SLP_EN0 = 0x06;
#endif

#ifdef CONFIG_MEC1701_TFDP
	MEC17XX_PCR_SLP_EN1 = 0x80000002;
#else
	MEC17XX_PCR_SLP_EN1 = 0x80000082;
#endif

#ifdef CONFIG_ESPI
	MEC17XX_PCR_SLP_EN2 = 0x68005;
#else
	MEC17XX_PCR_SLP_EN2 = 0x68004;
#endif

#ifdef CONFIG_MEC1701_GP_SPI
	MEC17XX_PCR_SLP_EN3 = 0x7E7F10E7;
#else
	MEC17XX_PCR_SLP_EN3 = 0x7E7F12E7;
#endif

	MEC17XX_PCR_SLP_EN4 = 0xFFFFFEFF;
	 ...
	 * platform reset.
	 * POR default is 1
	 *  1 = LRESET# pin generates MEC17xx internal platform reset.
	 *  0 = eSPI_PLTRST# signal generates internal platform reset.
	 * Clear this bit for eSPI connected EC.
	 * NOTE: MEC17xx documentation uses a different naming convention
	 * for nRESET_OUT and nRESET_IN.
	...
	imgtype = gpio_get_level(GPIO_PD_MCU_INT);
	trace1(0, MEC, 0, "PCH_PD_MCU_INT = %d", imgtype);
}
...
int system_get_bbram(enum system_bbram_idx idx, uint8_t *value)
{
	...
	*value = MEC17XX_VBAT_RAM(hibdata);
	return EC_SUCCESS;
}
...
int system_set_bbram(enum system_bbram_idx idx, uint8_t value)
{
	...
	MEC17XX_VBAT_RAM(hibdata) = value;
	return EC_SUCCESS;
}
...
void system_hibernate(uint32_t seconds, uint32_t microseconds)
{
	...
	if (hibernate_wake_pins_used > 0) {
		...
		task_enable_irq(MEC17XX_IRQ_GIRQ12);
	}
