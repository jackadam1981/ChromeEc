 ...
 */
#ifdef CONFIG_MEC1701_ESPI_RESET_DEASSERT_INIT
 ...
 */
#ifdef CONFIG_MEC1701_ESPI_VW_SAVE_ON_SLEEP
...
int espi_vw_set_wire(enum espi_vw_signal signal, uint8_t level)
{
	...
	trace2(0, ESPI, 0, "VW SetWire[%d] = %d", signal, level);
#endif
...
int espi_vw_pulse_wire(enum espi_vw_signal signal)
{
	...
	signal -= VW_SIGNAL_BASE;
	trace2(0, ESPI, 0, "VW PulseWire[%d] = %d", signal, (~level & 0x01));
#endif
...
int espi_vw_get_wire(enum espi_vw_signal signal)
{
	...
	if (tidx >= 0 && (0 == (vw_info_tbl[tidx].flags & (1u << 0)))) {
		...
		signal -= VW_SIGNAL_BASE;
		trace2(0, ESPI, 0, "VW GetWire[%d] = %d", signal, vw);
#endif
	}
...
int espi_vw_enable_wire_int(enum espi_vw_signal signal)
{
	...
	signal -= VW_SIGNAL_BASE;
	trace1(0, ESPI, 0, "VW IntrEn for VW[%d]", signal);
#endif
...
int espi_vw_disable_wire_int(enum espi_vw_signal signal)
{
	...
	signal -= VW_SIGNAL_BASE;
	trace1(0, ESPI, 0, "VW IntrDis for VW[%d]", signal);
#endif
...
#ifdef CONFIG_ESPI
/*
 * Hook called in Power State machine when it detects exit from G3 and
 * enters G3S5 state.
 * Power state machine will check if SLP_Sx signals have de-asserted.
 * If they have not de-asserted Power state machine gives up and returns
 * to G3 state. We use this hook to hold off reading of SLP_Sx signals
 * until Host chipset has enabled Virtual Wire channel so that reads
 * of virtual wires return real values.
 * Honestly I don't like to spin forever but what can we do if the Host
 * chipset does not bring up eSPI? We could implement a timeout but Host
 * chipset maximum appears to change with each succeeding generation.
 */
static void espi_wait_ready(void)
{
	while (!espi_chan_ready())
		;
	trace0(0, ESPI, 0, "HOOK_CHIPSET_PRE_INIT: eSPI VW & FC are ready!");
}
