...
static void espi_vw_save(void)
{
	...
	r = MEC17XX_VBAT_RAM(MEC17XX_VBAT_VWIRE_BACKUP);
	r = (r & 0xFFFFFF00) | vb;
	MEC17XX_VBAT_RAM(MEC17XX_VBAT_VWIRE_BACKUP) = r;
}

/*
 * Update MEC1701H VBAT powered VWire backup values restored on
 * MEC17xx chip reset. MEC17xx Boot-ROM loads these values into
...
static void espi_vw_restore(void)
{
...
#else	
	vb = MEC17XX_VBAT_RAM(MEC17XX_VBAT_VWIRE_BACKUP) & 0xff;
#endif
	r = 0;
	for (i = 0; i < 4; i++) {
		if (vb & (1u << i))
	...
	r = MEC17XX_VBAT_RAM(MEC17XX_VBAT_VWIRE_BACKUP);
	MEC17XX_VBAT_RAM(MEC17XX_VBAT_VWIRE_BACKUP) = r & 0xFFFFFF00;
...
static void espi_host_init(void)
{
	...
	task_enable_irq(MEC17XX_IRQ_8042EM_OBE);
		...
		uint8_t level)
{
	...
	MEC17XX_ESPI_VW_S2M_SRC(ridx, src_num) = level & 0x01;

	for (i = 0; i < ESPI_S2M_VW_PULSE_LOOP_CNT; i++) {
		if ((MEC17XX_ESPI_VW_S2M_CHANGE(ridx) &
		     (1u << src_num)) == 0)
			return EC_SUCCESS;
 ...
 *	activate ESPI_RESET#.
 *
 * ESPI_RESET# assertion will disable all MEC17xx eSPI channel ready
...
void espi_init(void)
{
	...
	MEC17XX_PCR_SLP_DIS_DEV(MEC17XX_PCR_ESPI);
