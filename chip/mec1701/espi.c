...
static const struct vw_info_t vw_info_tbl[] = {
	...
	{VW_SLP_S3_L, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, "VW_SLP_S3_L"},
	...
	{VW_SUS_STAT_L,  0x03, 0x00, 0x10, 0x01, 0x00, 0x00, "VW_SUS_STAT_L"},
	{VW_PLTRST_L, 0x03, 0x00, 0x10, 0x01, 0x01, 0x00, "VW_PLTRST_L"},
	{VW_OOB_RST_WARN,
	 0x03, 0x00, 0x10, 0x01, 0x02, 0x00, "VW_OOB_RST_WARN"},
	/* SMVW00 Host Index 04h (Out) */
	{VW_OOB_RST_ACK,
	 0x04, 0x00, 0x01, 0x00, 0x00, 0x00, "VW_OOB_RST_ACK"},
	...
	{VW_ERROR_FATAL,
	 0x05, 0x00, 0x01, 0x01, 0x01, 0x00, "VW_ERROR_FATAL"},
	{VW_ERROR_NON_FATAL,
	 0x05, 0x00, 0x01, 0x01, 0x02, 0x00, "VW_ERROR_NON_FATAL"},
	{VW_SLAVE_BTLD_STATUS_DONE,
	 0x05, 0x00, 0x01, 0x01, 0x30, 0x00, "VW_SLAVE_BTLD_STATUS_DONE"},
	/* SMVW02 Host index 06h (Out) */
	{VW_SCI_L, 0x06, 0x01, 0x01, 0x02, 0x00, 0x00, "VW_SCI_L"},
	{VW_SMI_L, 0x06, 0x01, 0x01, 0x02, 0x01, 0x00, "VW_SMI_L"},
	{VW_RCIN_L, 0x06, 0x01, 0x01, 0x02, 0x02, 0x00, "VW_RCIN_L"},
	{VW_HOST_RST_ACK,
	 0x06, 0x00, 0x01, 0x02, 0x03, 0x00, "VW_HOST_RST_ACK"},
	/* MSVW02 Host index 07h (In) */
	{VW_HOST_RST_WARN,
	 0x07, 0x00, 0x10, 0x02, 0x00, 0x00, "VW_HOST_RST_WARN"},
	/* SMVW03 Host Index 40h (Out) */
	{VW_SUS_ACK, 0x40, 0x00, 0x01, 0x03, 0x00, 0x00, "VW_SUS_ACK"},
	/* MSVW03 Host Index 41h (In) */
	{VW_SUS_WARN_L, 0x41, 0x00, 0x10, 0x03, 0x00, 0x00, "VW_SUS_WARN_L"},
	{VW_SUS_PWRDN_ACK_L,
	 0x41, 0x00, 0x10, 0x03, 0x01, 0x00, "VW_SUS_PWRDN_ACK_L"},
	{VW_SLP_A_L, 0x41, 0x00, 0x10, 0x03, 0x03, 0x00, "VW_SLP_A_L"},
	/* MSVW04 Host index 42h (In) */
	{VW_SLP_LAN, 0x42, 0x00, 0x10, 0x04, 0x00, 0x00, "VW_SLP_LAN"},
...
static void espi_vw_save(void)
{
	...
	vb = 0;
	r = MEC17XX_ESPI_VW_M2S_SRC_ALL(MSVW_H42);
	for (i = 0; i < 4; i++) {
		if (r & (1ul << (i << 3)))
			vb |= (1u << i);
	}
	...
	vb <<= 4;
	r = MEC17XX_ESPI_VW_M2S_SRC_ALL(MSVW_H02);
	for (i = 0; i < 4; i++) {
		if (r & (1ul << (i << 3)))
			vb |= (1u << i);
	}
...
static void espi_vw_restore(void)
{
	...
	MEC17XX_ESPI_VW_M2S_SRC_ALL(MSVW_H02) = r;
	CPRINTS("eSPI restore MSVW00(Index 02h) = 0x%08x", r);
	TRACE11(26, ESPI, 0, "eSPI restore MSVW00(Index 02h) = 0x%08x", r);
	...
	MEC17XX_ESPI_VW_M2S_SRC_ALL(MSVW_H42) = r;
	CPRINTS("eSPI restore MSVW00(Index 42h) = 0x%08x", r);
	TRACE11(27, ESPI, 0, "eSPI restore MSVW04(Index 42h) = 0x%08x", r);
...
static uint8_t __attribute__((unused)) espi_msvw_srcs_get(uint8_t msvw_id)
{
	...
	msvw = 0;
	if (msvw_id < MSVW_MAX) {
		uint32_t r = MEC17XX_ESPI_VW_M2S_SRC_ALL(msvw_id);
						       ...
						       uint8_t src_bitmap)
{
	if (msvw_id < MSVW_MAX) {
		...
		r |= (src_bitmap & 0x02) << 7;
		r |= (src_bitmap & 0x01);
		MEC17XX_ESPI_VW_M2S_SRC_ALL(msvw_id) = r;
	}
}
...
static uint8_t __attribute__((unused)) espi_smvw_srcs_get(uint8_t smvw_id)
{
	...
	smvw = 0;
	if (smvw_id < SMVW_MAX) {
		uint32_t r = MEC17XX_ESPI_VW_S2M_SRC_ALL(smvw_id);
						       ...
						       uint8_t src_bitmap)
{
	if (smvw_id < SMVW_MAX) {
		...
		r |= (src_bitmap & 0x02) << 7;
		r |= (src_bitmap & 0x01);
		MEC17XX_ESPI_VW_S2M_SRC_ALL(smvw_id) = r;
	}
}
...
static void espi_vw_pre_init(void)
{
	...
	TRACE0(28, ESPI, 0, "eSPI VW Pre-Init");

	espi_vw_restore();

	/* disable all */
	for (i = 0; i < MSVW_MAX; i++)
		MEC17XX_ESPI_VW_M2S_IRQSEL_ALL(i) = 0x0f0f0f0ful;

	/* clear spurious status */
	MEC17XX_INT_SOURCE(24) = 0xfffffffful;
	MEC17XX_INT_SOURCE(25) = 0xfffffffful;

	MEC17XX_ESPI_VW_M2S_IRQSEL_ALL(MSVW_H02) = 0x040f0f0ful;
	MEC17XX_ESPI_VW_M2S_IRQSEL_ALL(MSVW_H03) = 0x040f0f0ful;
	MEC17XX_ESPI_VW_M2S_IRQSEL_ALL(MSVW_H07) = 0x0404040ful;
	MEC17XX_ESPI_VW_M2S_IRQSEL_ALL(MSVW_H41) = 0x0f040f0ful;
	MEC17XX_ESPI_VW_M2S_IRQSEL_ALL(MSVW_H42) = 0x04040f0ful;
	MEC17XX_ESPI_VW_M2S_IRQSEL_ALL(MSVW_H47) = 0x0404040ful;
	...
	TRACE0(29, ESPI, 0, "eSPI VW Pre-Init Done");
}
...
static void espi_send_boot_load_done(void)
{
	...
	MEC17XX_ESPI_VW_S2M_SRC3(SMVW_H05) = 1;
	/* Next set SLAVE_BOOT_LOAD_DONE = 1 */
	MEC17XX_ESPI_VW_S2M_SRC0(SMVW_H05) = 1;

	CPRINTS("eSPI Send SLAVE_BOOT_LOAD_STATUS/DONE = 1");
	TRACE0(30, ESPI, 0, "VW SLAVE_BOOT_LOAD_STATUS/DONE = 1");
}
...
{
	MEC17XX_ESPI_VW_M2S_SRC0(MSVW_H07) = 0;
	MEC17XX_ESPI_VW_M2S_SRC0(MSVW_H47) = 0;
	espi_smvw_srcs_set(SMVW_H06, 0x07);
}
#endif

/*
 * Clear SUS_STAT# (M2S 03h:0), PLTRST# (M2S 03h:1),
 *   OOB_RST_WARN (M2S 03h:2) = 0
 * Clear SUS_WARN# (M2S 41h:0), SUS_PWRDN_ACK (M2S 41h:1),
 *   SLP_A# ((M2S 41h:3) = 0
...
{
	MEC17XX_ESPI_VW_M2S_SRC_ALL(MSVW_H03) &= 0xFF000000ul;
	MEC17XX_ESPI_VW_M2S_SRC_ALL(MSVW_H41) &= 0x00FF0000ul;

	MEC17XX_ESPI_VW_S2M_SRC_ALL(SMVW_H04) &= 0x0000FF00ul;
	MEC17XX_ESPI_VW_S2M_SRC_ALL(SMVW_H05) = 0;
	MEC17XX_ESPI_VW_S2M_SRC0(SMVW_H40) = 0;
...
static void espi_host_init(void)
{
	...
	TRACE0(31, ESPI, 0, "eSPI Host Init");
	...
	/*
	 * Access data RAM
	 * MEC17xx EMI Base address register = physical address in
	 * SRAM of buffer.
	...
	*/
	...
	TRACE0(32, ESPI, 0, "eSPI Host Init Done");
}
...
void espi_vw_power_signal_interrupt(enum espi_vw_signal signal)
{
	...
	TRACE1(33, ESPI, 0, "eSPI pwr intr VW %d", (signal - VW_SIGNAL_BASE));
...
int espi_vw_set_wire(enum espi_vw_signal signal, uint8_t level)
{
	...
	if (signal == VW_SLAVE_BTLD_STATUS_DONE) {
		...
		MEC17XX_ESPI_VW_S2M_SRC3(ridx) = level;
		/* SLAVE_BOOT_LOAD_DONE after status */
		MEC17XX_ESPI_VW_S2M_SRC0(ridx) = level;
	} else {
		MEC17XX_ESPI_VW_S2M_SRC(ridx, src_num) = level;
	}

#if DEBUG_ESPI
	CPRINTS("eSPI VW Set Wire %s = %d", vw_info_tbl[tidx].name_str, level);
	signal -= VW_SIGNAL_BASE;
	TRACE2(34, ESPI, 0, "VW SetWire[%d] = %d", signal, level);
#endif
 ...
 * TODO - Should a timeout mechanism be added because we are
 ...
 * Intel PCH eSPI maximum payload length of 64 bytes + packet overhead.
 * Be careful if calling this routine from an ISR.
 */
int espi_vw_pulse_wire(enum espi_vw_signal signal)
{
	...
	level = MEC17XX_ESPI_VW_S2M_SRC(ridx, src_num) & 0x01;

#if DEBUG_ESPI
	CPRINTS("eSPI VW Pulse Wire %s", vw_info_tbl[tidx].name_str);
	signal -= VW_SIGNAL_BASE;
	TRACE2(35, ESPI, 0, "VW PulseWire[%d] = %d", signal, (~level & 0x01));
#endif

	MEC17XX_ESPI_VW_S2M_SRC(ridx, src_num) = ~level;
	/* wait for Master to read the VWire index */
	while (MEC17XX_ESPI_VW_S2M_CHANGE(ridx) & (1u << src_num))
		;
...
int espi_vw_get_wire(enum espi_vw_signal signal)
{
	...
	if (tidx >= 0 && (0 == (vw_info_tbl[tidx].flags & (1u << 0)))) {
		ridx = vw_info_tbl[tidx].reg_idx;
		src_num = vw_info_tbl[tidx].src_num;
		vw = MEC17XX_ESPI_VW_M2S_SRC(ridx, src_num) & 0x01;
#if DEBUG_ESPI
		CPRINTS("VW GetWire %s = %d", vw_info_tbl[tidx].name_str, vw);
		signal -= VW_SIGNAL_BASE;
		TRACE2(36, ESPI, 0, "VW GetWire[%d] = %d", signal, vw);
#endif
	}
...
int espi_vw_enable_wire_int(enum espi_vw_signal signal)
{
...
#if DEBUG_ESPI
	CPRINTS("VW IntrEn for VW[%s]", vw_info_tbl[tidx].name_str);
	signal -= VW_SIGNAL_BASE;
	TRACE1(37, ESPI, 0, "VW IntrEn for VW[%d]", signal);
#endif
	 ...
	 */
	MEC17XX_ESPI_VW_M2S_IRQSEL(ridx, src_num) =
			MEC17XX_ESPI_MSVW_IRQSEL_BOTH_EDGES;
...
int espi_vw_disable_wire_int(enum espi_vw_signal signal)
{
...
#if DEBUG_ESPI
	CPRINTS("VW IntrDis for VW[%s]", vw_info_tbl[tidx].name_str);
	signal -= VW_SIGNAL_BASE;
	TRACE1(38, ESPI, 0, "VW IntrDis for VW[%d]", signal);
#endif
	 ...
	 */
	MEC17XX_ESPI_VW_M2S_IRQSEL(ridx, src_num) =
			MEC17XX_ESPI_MSVW_IRQSEL_DISABLED;
...
void espi_vw_evt_slp_s3_n(uint32_t wire_state, uint32_t bpos)
{
	...
	TRACE1(39, ESPI, 0, "VW_SLP_S3_L change to %d", wire_state);
...
void espi_vw_evt_slp_s4_n(uint32_t wire_state, uint32_t bpos)
{
	...
	TRACE1(40, ESPI, 0, "VW_SLP_S4_L change to %d", wire_state);
...
void espi_vw_evt_slp_s5_n(uint32_t wire_state, uint32_t bpos)
{
	...
	TRACE1(41, ESPI, 0, "VW_SLP_S5_L change to %d", wire_state);
...
void espi_vw_evt_sus_stat_n(uint32_t wire_state, uint32_t bpos)
{
	...
	TRACE1(42, ESPI, 0, "VW_SUS_STAT change to %d", wire_state);
...
void espi_vw_evt_pltrst_n(uint32_t wire_state, uint32_t bpos)
{
	...
	TRACE1(43, ESPI, 0, "VW_PLTRST# change to %d", wire_state);
...
void espi_vw_evt_oob_rst_warn(uint32_t wire_state, uint32_t bpos)
{
	...
	TRACE1(44, ESPI, 0, "VW_OOB_RST_WARN change to %d", wire_state);

	/*
	 * TODO if OOB_RST_WARN == 1 then
	 * flush OOB and idle OOB channel upstream requests
	 */
	espi_oob_flush();
...
void espi_vw_evt_sus_warn_n(uint32_t wire_state, uint32_t bpos)
{
	...
	TRACE1(45, ESPI, 0, "VW_SUS_WARN# change to %d", wire_state);
...
{
	TRACE1(46, ESPI, 0, "VW_SUS_PWRDN_ACK change to %d", wire_state);
...
void espi_vw_evt_slp_a_n(uint32_t wire_state, uint32_t bpos)
{
	...
	TRACE1(47, ESPI, 0, "VW_SLP_A# change to %d", wire_state);
...
void espi_vw_evt_host_rst_warn(uint32_t wire_state, uint32_t bpos)
{
	...
	TRACE1(48, ESPI, 0, "VW_HOST_RST_WARN change to %d", wire_state);

	/*
	 * TODO If HOST_RST_WARN = 1 then we must
	 * flush and idle Peripheral Channel
	 */
	espi_pc_flush();
...
void espi_vw_evt_slp_lan_n(uint32_t wire_state, uint32_t bpos)
{
	...
	TRACE1(49, ESPI, 0, "VW_SLP_LAN# change to %d", wire_state);
}
...
void espi_vw_evt_slp_wlan_n(uint32_t wire_state, uint32_t bpos)
{
	...
	TRACE1(50, ESPI, 0, "VW_SLP_WLAN# change to %d", wire_state);
}

void espi_vw_evt_host_c10(uint32_t wire_state, uint32_t bpos)
{
	CPRINTS("VW HOST_C10: %d", wire_state);
	TRACE1(51, ESPI, 0, "VW_HOST_C10 change to %d", wire_state);
}
...
void espi_mswv1_interrupt(void)
{
	...
	while (bpos != 32) {
		d = *(uint8_t *)(MEC17XX_ESPI_MSVW_BASE + 8 +
...
void espi_msvw2_interrupt(void)
{
	...
	while (bpos != 32) {
		d = *(uint8_t *)(MEC17XX_ESPI_MSVW_BASE + (12 * 7) + 8 +
				(12 * (bpos >> 2)) + (bpos & 0x03)) & 0x01;
...
void espi_reset_isr(void)
{
	...
	if (erst & (1ul << 1)) { /* rising edge - reset de-asserted */
		...
		TRACE0(52, ESPI, 0, "eSPI Reset de-assert");
		...
		TRACE0(53, ESPI, 0, "eSPI Reset assert");
	}
}
...
void espi_vw_en_isr(void)
{
	...
	TRACE0(54, ESPI, 0, "VW Enable. Set VW Ready");
...
void espi_oob_rx_isr(void)
{
	...
	TRACE11(58, ESPI, 0, "eSPI OOB_RX Status = 0x%08x", sts);
}
...
void espi_fc_isr(void)
{
	...
	if (sts & (1ul << 1)) {
		...
		if (sts & (1ul << 0)) { /* enable? */
			...
			TRACE0(59, ESPI, 0, "eSPI FC Enable");
			if (0x03 == (espi_channels_ready & 0x03))
				espi_send_boot_load_done();
		} else { /* no, disabled by Master */
			espi_channels_ready &= ~(1ul << 1);
			CPRINTS("eSPI FC ISR: Disable");
			TRACE0(60, ESPI, 0, "eSPI FC Disable");
		}
	} else {
		/* TODO handle FC command status: done and/or errors */
		CPRINTS("eSPI FC status = 0x%x", sts);
		TRACE11(61, ESPI, 0, "eSPI FC Status = 0x%08x", sts);
	}
}
...
void espi_pc_isr(void)
{
	...
	if (sts & (1ul << 25)) {
		if (sts & (1ul << 24)) {
			...
			TRACE0(62, ESPI, 0, "eSPI PC Enable");
		} else {
			espi_channels_ready &= ~(1ul << 3);
			CPRINTS("eSPI PC Channel Disable");
			TRACE0(63, ESPI, 0, "eSPI PC Disable");
		}

	} else {
		/* TODO */
		CPRINTS("eSPI PC status = 0x%x", sts);
		TRACE11(64, ESPI, 0, "eSPI PC Status = 0x%08x", sts);
	}
}
...
static void espi_reset_ictrl(int enable, int clr_status)
{
	if (enable) {
		if (clr_status) {
			MEC17XX_ESPI_IO_RESET_STATUS =
					MEC17XX_ESPI_RST_CHG_STS;
			MEC17XX_INT_SOURCE(MEC17XX_ESPI_GIRQ) =
					MEC17XX_ESPI_RESET_GIRQ_BIT;
		}
...
/*
 * Are eSPI Virtual Wire and flash channels ready?
 * 0 = Not ready
 * Non-zero ready
 * Return non-zero only if both VW and FC are ready
 */
static int espi_chan_ready(void)
{
	int ready;

	interrupt_disable();
#ifdef EVB_NO_ESPI_TEST_MODE
	espi_channels_ready = 0x0f;
#endif
...
#ifdef CONFIG_ESPI
static void espi_wait_ready(void)
{
	while (!espi_chan_ready())
		;
	TRACE0(65, ESPI, 0, "HOOK_CHIPSET_PRE_INIT: eSPI VW & FC are ready!");
}
DECLARE_HOOK(HOOK_CHIPSET_PRE_INIT, espi_wait_ready, HOOK_PRIO_INIT_LPC);
#endif
...
void espi_init(void)
{
	...
	MEC17XX_ESPI_IO_CAP0 = CONFIG_ESPI_EC_CHAN_BITMAP;
#endif

#ifdef CONFIG_ESPI_EC_MAX_FREQ
	MEC17XX_ESPI_IO_CAP1 &= ~(MEC17XX_ESPI_CAP1_MAX_FREQ_MASK);
#if CONFIG_ESPI_EC_MAX_FREQ == 25
	MEC17XX_ESPI_IO_CAP1 |= MEC17XX_ESPI_CAP1_MAX_FREQ_25M;
#elif CONFIG_ESPI_EC_MAX_FREQ == 33
	MEC17XX_ESPI_IO_CAP1 |= MEC17XX_ESPI_CAP1_MAX_FREQ_33M;
#elif CONFIG_ESPI_EC_MAX_FREQ == 50
	MEC17XX_ESPI_IO_CAP1 |= MEC17XX_ESPI_CAP1_MAX_FREQ_50M;
#elif CONFIG_ESPI_EC_MAX_FREQ == 66
	MEC17XX_ESPI_IO_CAP1 |= MEC17XX_ESPI_CAP1_MAX_FREQ_66M;
#else
	MEC17XX_ESPI_IO_CAP1 |= MEC17XX_ESPI_CAP1_MAX_FREQ_20M;
#endif
#endif

#ifdef CONFIG_ESPI_EC_MODE
	MEC17XX_ESPI_IO_CAP1 &= ~(MEC17XX_ESPI_CAP1_IO_MASK);
	MEC17XX_ESPI_IO_CAP1 |= ((CONFIG_ESPI_EC_MODE)
		<< MEC17XX_ESPI_CAP1_IO_BITPOS);
#endif
	...
	MEC17XX_PCR_PWR_RST_CTL &=
		~(1ul << MEC17XX_PCR_PWR_HOST_RST_SEL_BITPOS);
