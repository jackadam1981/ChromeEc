 ...
 * @note MEC17xx DMA does not require address aliasing. Because count
		...
		const void *memory, uint32_t dma_xfr_units)
{
	...
	if (option != NULL) {
		nflags = option->flags & ~(MEC17XX_DMA_XFER_SIZE_MASK);
		nflags |= MEC17XX_DMA_XFER_SIZE(dma_xfr_units & 0x07);
		/*
		 * Cast away const for memory pointer; this is ok because
		 * we know we're preparing the channel for transmit.
		 */
		...
		unsigned count, void *memory)
{
	...
	if (option != NULL) {
		...
		if (ch < MEC17XX_DMAC_COUNT) {
			...
			MEC17XX_DMA_CH_CTRL(ch) = 0;
			MEC17XX_DMA_CH_MEM_START(ch) = (uint32_t)memory;
			MEC17XX_DMA_CH_MEM_END(ch) = (uint32_t)memory +
					count;

			MEC17XX_DMA_CH_DEV_ADDR(ch) =
					(uint32_t)option->periph;

			ctrl = option->flags &
					~(MEC17XX_DMA_XFER_SIZE_MASK);
			ctrl |= MEC17XX_DMA_INC_MEM;
			ctrl |= MEC17XX_DMA_XFER_SIZE(dma_xfr_ulen);
			ctrl |= MEC17XX_DMA_DEV(option->channel);
			MEC17XX_DMA_CH_CTRL(ch) = ctrl;
			MEC17XX_DMA_CH_ACT(ch) = MEC17XX_DMA_ACT_EN;
		}
...
void dma_init(void)
{
	MEC17XX_PCR_SLP_DIS_DEV(MEC17XX_PCR_DMA);
	MEC17XX_DMA_MAIN_CTRL = MEC17XX_DMA_MAIN_CTRL_SRST;
