				...
				unsigned int block_nb)
{
	...
	TRACE11(95, SHA, 0, "SHA256_Transform: pblock  = 0x%08x",
		(uint32_t)pblocks);
	TRACE11(96, SHA, 0, "SHA256_Transform: nblocks = 0x%08x", block_nb);

	rom_hash_iclr();
	rc = rom_sha_raw_update_wrap(pblocks, ctx->h, block_nb);
	if (rc == EC_SUCCESS) {
		TRACE0(97, SHA, 0, "Hash Wait");
		rc = hash_wait(block_nb);
		TRACE0(98, SHA, 0, "Hash Wait Done");
	}
			...
			uint32_t len)
{
	...
	TRACE12(99, SHA, 0, "chip_sha256_update: data=0x%08x  len=0x%08x",
		(uint32_t)data, len);
	TRACE12(100, SHA, 0,
