...
#endif
	/* TxE not set if NACK is received */
//	wait_tx();
...
static void i2c_handle_interrupt(int port)
{
	...
	if (stat1 & (1 << 1)) {
		...
		/* Received stop bit; release SCL and SDA after current byte
		 * transfer and wait for hardware to clear the bit. Writing
		 * to CR1 also clears the STOPF bit in SR1. */
//		STM32L_I2C_CR1(2) |= 1 << 9;	
		/* cleared by HW when a stop condition is detected */
//		while (STM32L_I2C_CR1(2) & (1 << 9))
//			;
		STM32L_I2C_SR1(2);
