...
static uint32_t i2c_sr1[NUM_PORTS];
...
static void i2c_event_handler(int port)
{
	...
	if (i2c_sr1[port] & ((1 << 6) | (1 << 7))) {
		if (port == I2C2)
			task_send_msg(TASK_ID_I2C2_WORK, TASK_ID_I2C2_WORK, 0);
	}
}
...
static void i2c_error_handler(int port)
{
	...
	uart_printf("%s: I2C_SR1(%s): 0x%08x\n", __func__, port, i2c_sr1[port]);
	uart_printf("%s: I2C_SR2(%s): 0x%08x\n",
...
static int i2c_init2(void)
{
	...
	uart_printf("%s: initializing i2c port 2... ", __func__);
