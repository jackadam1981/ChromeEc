...
#error "Need to enable clock for GPIO modules for this board"
	...
	for (i = 0; i < ARRAY_SIZE(ports); i++) {
		...
		STM32L_GPIO_MODER_OFF(ports[i]) |= mode;

		STM32L_GPIO_PUPDR_OFF(ports[i]) &= ~mask32;
		STM32L_GPIO_PUPDR_OFF(ports[i]) |= pupd;

		STM32L_GPIO_OTYPER_OFF(ports[i]) &= ~mask16;
		STM32L_GPIO_OTYPER_OFF(ports[i]) |= otype;
	}
...
#if defined(BOARD_daisy) || defined(BOARD_discovery)
	/* PD2 */
	exticr = STM32L_SYSCFG_EXTICR(0);
	exticr &= ~0x0f00;
	exticr |= 0x0300;
	STM32L_SYSCFG_EXTICR(0) = exticr;

	/* PC8:11 */
	STM32L_SYSCFG_EXTICR(2) = 0x2222;

	/* PC12, PC14:15 */
	exticr = STM32L_SYSCFG_EXTICR(3);
	exticr &= ~0xff0f;
	exticr |= 0x2202;
	STM32L_SYSCFG_EXTICR(3) = exticr;
