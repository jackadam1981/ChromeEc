...
STATIC int _check_flash_is_awake(void) {
  ...
  write_reg(FLASH0_BASE_ADDR + FLASH_FSH_TRANS_OFFSET, 0xFFFFFFFF);
  retval = read_reg(FLASH0_BASE_ADDR + FLASH_FSH_TRANS_OFFSET);
  write_reg(FLASH0_BASE_ADDR + FLASH_FSH_TRANS_OFFSET, 0x0);

  if (retval == 0) {
    DEBUG_PRINTF("ERROR:FLASH Controller seems unresponsive. Did you make sure to run 'reseth'?\n");
...
STATIC int _flash_error(void) {
  int retval = read_reg(FLASH0_BASE_ADDR + FLASH_FSH_ERROR_OFFSET);
  if (retval) {
    DEBUG_PRINTF("Register FLASH_FSH_ERROR is not zero (found %x). Will read again to verify FSH_ERROR was cleared and then continue...\n", retval);
    if (read_reg(FLASH0_BASE_ADDR + FLASH_FSH_ERROR_OFFSET)) {
      DEBUG_PRINTF("ERROR: Read to FLASH_FSH_ERROR did not clear it\n");
    }
  }
  return retval;
}
...
STATIC int _flash_cmd(uint32_t fidx, uint32_t cmd) {
  ...
  // activate controller
  write_reg(FLASH0_BASE_ADDR + FLASH_FSH_PE_EN_OFFSET, FSH_OP_ENABLE);
  write_reg(FLASH0_BASE_ADDR + FLASH_FSH_PE_CONTROL0_OFFSET + (4*fidx), cmd);

  // wait on FSH_PE_EN (means the operation started)
  cnt = 500;  // TODO(mschilder): pick sane value.
  retval = !0;
  while (retval && cnt) {
    retval = read_reg(FLASH0_BASE_ADDR + FLASH_FSH_PE_EN_OFFSET);
    DEBUG_PRINTF("FLASH_FSH_PE_EN%d is %x\n", fidx, retval);
    cnt--;
  }
  if (retval) {
    DEBUG_PRINTF("ERROR: FLASH_FSH_PE_EN never went to 0, is 0x%x after timeout\n", retval);
    return E_FL_TIMEOUT;
  }

  // wait 100us before checking FSH_PE_CONTROL (means the operation ended)
  cnt = 1000000;
  retval = !0;
  while (retval && cnt) {  // TODO(mschilder): pick sane timeout.
    retval = read_reg(FLASH0_BASE_ADDR + FLASH_FSH_PE_CONTROL0_OFFSET + (4*fidx));
    DEBUG_PRINTF("FLASH_FSH_PE_CONTROL%d is %x\n", fidx, retval);
    cnt--;
  }

  if (retval) {
    DEBUG_PRINTF("ERROR: FLASH_FSH_PE_CONTROL%d is 0x%x after timeout\n", fidx, retval);
    write_reg(FLASH0_BASE_ADDR + FLASH_FSH_PE_CONTROL0_OFFSET + (4*fidx), 0);
    return E_FL_TIMEOUT;
  }
...
int flash_info_read(uint32_t offset, uint32_t* dst) {
  ...
  write_reg(FLASH0_BASE_ADDR + FLASH_FSH_TRANS_OFFSET,
            ((offset << FLASH_FSH_TRANS_OFFSET_LSB) & FLASH_FSH_TRANS_OFFSET_MASK) |
            ((1      << FLASH_FSH_TRANS_MAINB_LSB)  & FLASH_FSH_TRANS_MAINB_MASK ) |
            ((1      << FLASH_FSH_TRANS_SIZE_LSB)   & FLASH_FSH_TRANS_SIZE_MASK  ));

  retval = _flash_cmd(1, FSH_OP_READ);
  if (retval) return retval;

  if (_flash_error()) return E_FL_ERROR;

  if (!retval) {
    *dst = read_reg(FLASH0_BASE_ADDR + FLASH_FSH_DOUT_VAL1_OFFSET);
  }
