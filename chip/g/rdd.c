...
{
	if (enable) {
		/* Clear pending interrupts then enable */
		GWRITE_FIELD(RDD, INT_STATE, INTR_DEBUG_STATE_DETECTED, 1);
		GWRITE_FIELD(RDD, INT_ENABLE, INTR_DEBUG_STATE_DETECTED, 1);
	} else {
		/* Disable then clear any pending interrupts */
		GWRITE_FIELD(RDD, INT_ENABLE, INTR_DEBUG_STATE_DETECTED, 0);
		GWRITE_FIELD(RDD, INT_STATE, INTR_DEBUG_STATE_DETECTED, 1);
	}
}
...
{
	/*
	 * Delay sleep long enough that HOOK_SECOND will be called to re-enable
	 * our interrupt.
	 */
	delay_sleep_by(1 * SECOND);

	/* Trigger the deferred handler */
	hook_call_deferred(&rdd_connect_data, 0);

	/*
	 * Disable our interrupt; this also clears the pending interrupt.  This
	 * prevents interrupt storms due to CC activity; the interrupt can
	 * trigger at most once a second.
	 */
	enable_interrupt(0);
}
...
static void rdd_detect(void)
{
	 ...
	 * signal to make sure it's consistent.
	 */
