...
static uint16_t valid_map = 1 << 15;

static int debug_cable_is_detected(void)
{
	return (GREAD_FIELD(RDD, INPUT_PIN_VALUES, CC1) == CC1_VALID &&
		GREAD_FIELD(RDD, INPUT_PIN_VALUES, CC2) == CC2_VALID);
}
...
void rdd_interrupt(void)
{
	if (debug_cable_is_detected()) {
		...
		GWRITE(RDD, PROG_DEBUG_STATE_MAP, ~valid_map);
