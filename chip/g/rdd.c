...
uint8_t get_rdd_metrics(void)
{
	...
	if (rdd_is_detected())
		status |= (1 << METRICSV_RDD_IS_DETECTED_SHIFT);
	if (force_detected)
		status |= (1 << METRICSV_RDD_KEEPALIVE_EN_SHIFT);
	if (ccd_get_flag(CCD_FLAG_RDDKEEPALIVE_AT_BOOT))
		status |= (1 << METRICSV_RDD_KEEPALIVE_EN_ATBOOT_SHIFT);
	if (gpio_get_level(GPIO_CCD_MODE_L))
		status |= (1 << METRICSV_CCD_MODE_L_SHIFT);
	return status;
}
