...
static void con_ep_rx(void)
{
	int i;
	for (i = 0;
	     i < USB_MAX_PACKET_SIZE - (ep_out_desc.flags & DOEPDMA_RXBYTES_MASK);
	     i++) {
...
static void ep_reset(void)
{
	...
	GR_USB_DOEPCTL(USB_EP_CONSOLE) = DXEPCTL_MPS64 | DXEPCTL_USBACTEP |
					 DXEPCTL_EPTYPE_BULK |
					 DXEPCTL_CNAK | DXEPCTL_EPENA;
	GR_USB_DIEPCTL(USB_EP_CONSOLE) = DXEPCTL_MPS64 | DXEPCTL_USBACTEP |
					 DXEPCTL_EPTYPE_BULK;
