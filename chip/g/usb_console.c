...
struct usb_stream_config const cr50_usb;

USB_STREAM_CONFIG(cr50_usb,
		  USB_IFACE_CONSOLE,
...
static inline void handle_output(void)
{
	...
	hook_call_deferred(cr50_usb.deferred_tx, 0);
}

static inline int _tx_fifo_is_ready(void)
{
	return tx_fifo_is_ready(&cr50_usb);
}
