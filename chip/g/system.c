...
void system_print_extended_version_info(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(rw_images); i++) {
		...
		ccprintf("BID %c:   %08x:%08x:%08x %s\n", 'A' + i,
			 ss->bid.board_id_type ^ SIGNED_HEADER_PADDING,
			 ss->bid.board_id_type_mask ^ SIGNED_HEADER_PADDING,
			 ss->bid.board_id_flags ^ SIGNED_HEADER_PADDING,
			 check_board_id_vs_header(&bid, &ss->bid) ?
			 " No" : "Yes");
	}
}
