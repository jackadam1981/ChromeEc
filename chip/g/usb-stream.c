...
static void usb_enable_tx(struct usb_stream_config const *config,
			  int const len[MAX_IN_DESC])
{
	const uint32_t last_flags = DIEPDMA_BS_HOST_RDY | DIEPDMA_IOC |
				    DIEPDMA_LAST;
	int idx = 0;
	...
	config->in_desc[idx].flags = DIEPDMA_TXBYTES(len[idx]) | last_flags;
...
{
	GR_USB_DOEPINT(config->endpoint) = DOEPINT_MASK;
...
static inline int tx_fifo_is_ready(struct usb_stream_config const *config)
{
	...
	status = config->in_desc[0].flags & DIEPDMA_BS_MASK;
	if (status != DIEPDMA_BS_DMA_DONE && status != DIEPDMA_BS_HOST_BSY)
		return 0;

	status = config->in_desc[1].flags & DIEPDMA_BS_MASK;
	if (status != DIEPDMA_BS_DMA_DONE && status != DIEPDMA_BS_HOST_BSY)
		return 0;
...
void usb_stream_tx(struct usb_stream_config const *config)
{
	...
	GR_USB_DIEPINT(config->endpoint) = DIEPINT_MASK;
