...
void tx_stream_handler(struct usb_stream_config const *config)
{
	...
	/*
	 * Check tx ep is writable. If not, then just return because it is being
	 * modified in some other instance.
	 */
	if (!*config->tx_ep_writable)
		return;
	/* Mark TX EP not-writable to prevent from being overwritten. */
	*config->tx_ep_writable = 0;
