...
/**
 * @brief Internal helper to convert value to trim code
 * @note This is currently a port of xo.py
 */
...
/**
 * @brief Run the RC calibration counters
 * @details This must be used otherwise the counters may get stuck
 */
...
/**
 * @brief Calibrate RC trim
 * @note This is currently a port of xo.py
 */
uint32_t pmu_calibrate_rctrim(void)
{
	uint32_t size, iter;
	uint32_t mid;
	uint32_t diff;
	...
	*/
	...
	/* pmu_clock_switch_rctrim(); */

	return _pmu_value_to_trim(mid);
}

/**
 * @brief Switch system clock to RC no trim
 * @note This is currently a port of xo.py
 */
uint32_t pmu_clock_switch_rcnotrim(void)
{
	uint32_t osc_sel;
...
/**
 * @brief Switch system clock to RC trim
 * @note This is currently a port of xo.py
 */
uint32_t pmu_clock_switch_rctrim(uint32_t skip_calibration)
{
	uint32_t trimmed;
	uint32_t trim_code;
	uint32_t osc_sel;
	...
	if (GREAD_FIELD(XO, OSC_RC_STATUS, EN)) {
		...
		if (skip_calibration) {
			...
			trim_code = pmu_calibrate_rctrim();
			trimmed = 1;
		}
	}
...
/**
 * @brief Switch system clock to XO
 * @note This is currently a port of xo.py
...
/**
 * @brief Enter sleep mode and handle exiting from sleep mode
 ...
 * @details This function does not return. The powerdown exit event will
 ...
 * @details This function should be called after a powerdown exit event. It
 * handles turning the power domains back on. Clocks will be left in RC no trim.
 */
