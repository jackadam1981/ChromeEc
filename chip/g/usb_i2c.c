...
void usb_i2c_deferred(struct usb_i2c_config const *config)
{
	...
	if (!rx_valid(config)) {
		...
		uint8_t slave_addr  = (config->buffer[1] >> 7) & 0xfe;
		...
		if (write_count > USB_I2C_MAX_WRITE_COUNT ||
			...
			CPRINTS("i2c port: %d addr: %d write: %d read: %d",
				port,
				slave_addr,
				write_count,
				read_count);
			config->buffer[0] = usb_i2c_map_error(EC_ERROR_UNKNOWN);
			/*
			 * TODO: do i2c_xfer with request info
			 *
