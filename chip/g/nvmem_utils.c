...
static uint8_t cache_ram_index;
...
static void verify_log_to_phy_map(void)
{
	int block;
...
static int nvmem_verify_block_sha(struct nvmem_phy_block *p_block)
{
	...
	sha_comp = nvmem_compute_sha((uint8_t *)p_block,
				     (NVMEM_PHY_BLOCK_SIZE - sizeof(sha_comp)));
	/* Check if computed value matches stored value. */
	return sha_comp == p_block->tag.sha ? EC_SUCCESS : EC_ERROR_CRC;
}
...
static int nvmem_setup(void)
{
	...
	for (block = 0; block < NVMEM_LOG_BLOCKS; block++) {
		...
		p_tag->reserved = 0;
		p_tag->sha = nvmem_compute_sha(cache_ram,
