...
BUILD_ASSERT(ARRAY_SIZE(INTR_STATUS_ADDR) * 32 >= ARRAY_SIZE(alerts));
...
static int command_alerts(int argc, char **argv)
{
	...
	for (i = 0; i < ARRAY_SIZE(alerts); i++) {
		...
		int status = !!(*INTR_STATUS_ADDR[reg] & (1 << offset));
...
#define GLOBALSEC_ALERT_COUNTER(name) \
	...
	void handler_##name(void) { \
		int sts_reg = ALERT_NUM_##name / 32; \
		int sts_offset = ALERT_NUM_##name % 32; \
		alerts[ALERT_NUM_##name].counter++; \
		*INTR_STATUS_ADDR[sts_reg] = 1 << sts_offset; \
	}
