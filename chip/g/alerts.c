...
static void command_alerts_list(void)
{
	...
	ccprintf("Globalsec alerts status\nColumns:\n"
		" * name\n"
		" * fuse state: '?' - fuse is not defined, '#' fuse or "
		"header.applysec_ is disabled, '+'' fuse is enabled\n"
		" * intr state\n"
		" * alert counter\n");
...
static int command_alerts(int argc, char **argv)
{
	...
	if (argc == 3) {
		if (!strcasecmp(argv[1], "fire")) {
			...
			if (*e || alert < 0 || alert > 4) {
				ccprintf("interrupt number must be in range [0..3]\n");
