...
#ifdef CONFIG_ALERTS_CONSOLE_COMMAND
...
#endif /* CONFIG_ALERTS_CONSOLE_COMMAND */
...
static int command_alerts(int argc, char **argv)
{
	...
	for (i = 0; i < ALERTS_NUM; i++) {
		const char *name = alerts_descs[i].name;
		char fuse_status;

		int status = alert_intr_status(i);
		int8_t fuse = alerts_descs[i].fuse;
...
#define GLOBALSEC_ALERT_COUNTER(name) \
	...
	{ \
		alert_counters[ALERT_NUM_##name]++; \
