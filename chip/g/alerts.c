...
struct alert_desc {
	const char *name;
	const int8_t fuse; // BROM_FWBIT_APPLYSEC_* fuse that gates the alert
	uint16_t counter;
};
...
BUILD_ASSERT(ARRAY_SIZE(alerts) == ALERTS_NUM);
...
static int command_alerts(int argc, char **argv)
{
	...
	fuse_enabled =  GR_FUSE(FW_DEFINED_BROM_APPLYSEC) & hdr->applysec_;
	...
	for (i = 0; i < ALERTS_NUM; i++) {
		...
		int8_t fuse = alerts[i].fuse;

		if (fuse == BROM_FWBIT_APPLYSEC_UNKNOWN) {
			fuse_status = '?';
		} else if (fuse_enabled & (1 << fuse)) {
			fuse_status = '+';
		} else {
			fuse_status = '#';
		}
...
static int command_alert_fire(int argc, char **argv)
{
	GWRITE(GLOBALSEC, ALERT_FW_TRIGGER, 0xa9); // firing FW0 irq
	GWRITE(GLOBALSEC, ALERT_FW_TRIGGER, 0xaa); // back to normal
	//task_trigger_irq(GC_IRQNUM_GLOBALSEC_GLOBALSEC_FW0_ALERT_INT);
	return EC_SUCCESS;
}

DECLARE_CONSOLE_COMMAND(alert_fire, command_alert_fire,
	NULL,
	"Print alerts status");
...
{
	int i;
	struct alerts_data *resp = buf;

	if (input_size < sizeof(struct alerts_data)) {
		return VENDOR_RC_RESPONSE_TOO_BIG;
	}

	resp->version_id = ALERTS_HAVEN;
	resp->alerts_num = ARRAY_SIZE(alerts);
	interrupt_disable();
	for (i = 0; i < ALERTS_NUM; i++) {
		resp->counters[i] = alerts[i].counter;
		alerts[i].counter = 0;
	}
	interrupt_enable();

	*response_size = sizeof(struct alerts_data);
