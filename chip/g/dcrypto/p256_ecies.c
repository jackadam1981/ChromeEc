	...
	const uint8_t *info, size_t info_len)
{
	...
	in_len -= 1 + P256_NBYTES + P256_NBYTES + auth_data_len +
		SHA256_DIGEST_BYTES;
	...
	hmac_key = &key[AES_KEY_BYTES];
	dcrypto_HMAC_SHA256_init(&ctx, hmac_key, HMAC_KEY_BYTES);
	dcrypto_HMAC_update(&ctx, inp, auth_data_len + in_len);
	/* TODO(ngm): replace with constant time verify. */
	if (memcmp(inp + auth_data_len + in_len, dcrypto_HMAC_final(&ctx),
