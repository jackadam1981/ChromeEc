			   ...
			   const p256_int *in_y, p256_int *x, p256_int *y)
{
	...
	dcrypto_p256_sub_d(k, 1, &tmp_k);
	dcrypto_p256_rnd(&rnd);
	dmem_permuted_blinded_copy(DMEM_OFFSET(k), &tmp_k, &rnd);
	CP8W(rnd, &rnd);
	CP8W(x, in_x);
	CP8W(y, in_y);
...
int dcrypto_p256_sub_d(const p256_int *a, const p256_digit d, p256_int *b)
{
	int i;
	p256_sddigit borrow = 0;
	p256_int tmp = P256_ZERO;

	tmp.a[0] = d;

	for (i = 0; i < P256_NDIGITS; ++i) {
		borrow += (p256_sddigit)P256_DIGIT(a, i) - tmp.a[i];
		if (b != NULL)
