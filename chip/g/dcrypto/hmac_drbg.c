 ...
 * p1          |  p1_len  | nonce for INIT & SEED, input for GENERATE
 * p2_len      |    2     | size of third input in bytes for DRBG_INIT
 * p2          |  p2_len  | perso for INIT & SEED, input for GENERATE
 *
 ...
 * DRBG_GENERATE returns p1_len of generated data
 */
				    ...
				    size_t input_size, size_t *response_size)
{
	...
	size_t out_len = *response_size;
	enum drbg_command drbg_op = *cmd++;

	*response_size = 0;
	/* there is always first parameter, even if empty */
	...
	case DRBG_GENERATE: {
		if (p1_len > sizeof(output) || out_len < p1_len)
			return VENDOR_RC_BOGUS_ARGS;
