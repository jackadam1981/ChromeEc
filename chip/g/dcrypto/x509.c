...
/* Object with a short-length encoding (i.e. len <= 127). */
#define IS_SHORT_ASN1(x) (((x) & 128) == 0)
			...
			size_t *out_len, size_t *remaining)
{
	...
	short_length = in[1];
	if (IS_SHORT_ASN1(short_length)) {
		if (2 + short_length > available)
			return 0;
		if (out)
			*out = &in[2];
		if (out_len)
			*out_len = short_length;
		consumed = 2 + short_length;
	} else {
		int i;
		size_t obj_len_bytes = in[1] & ~128;

		if (obj_len_bytes > MAX_ASN1_OBJ_LEN_BYTES ||
			2 + obj_len_bytes > available)
			return 0;
		...
		if (2 + obj_len_bytes + obj_len > available)
			return 0;
		if (out)
			*out = &in[2 + obj_len_bytes];
		if (out_len)
			*out_len = obj_len;
		consumed = 2 + obj_len_bytes + obj_len;
	}
...
size_t asn1_parse_tbs(const uint8_t **p, size_t *available, size_t *tbs_len)
{
	size_t consumed;
...
size_t asn1_parse_signature_algorithm(const uint8_t **p, size_t *available)
{
...
size_t asn1_parse_signature_value(const uint8_t **p, size_t *available,
