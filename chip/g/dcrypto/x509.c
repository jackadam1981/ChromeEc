...
static void u2f_name(struct asn1 *ctx, uint8_t tag_encoding, int unique)
{
	if (unique) {
		/* TODO: add build version info? */
		asn1_string(ctx, tag_encoding, STRINGIFY(BOARD));
	} else {
		asn1_string(ctx, tag_encoding, "U2F");
	}
}
	...
	SEQ_START(ctx, V_SEQ, SEQ_MEDIUM) {
		...
		SEQ_START(ctx, V_SEQ, SEQ_SMALL) {
			SEQ_START(ctx, V_SET, SEQ_SMALL) {
				SEQ_START(ctx, V_SEQ, SEQ_SMALL) {
					asn1_object(&ctx, OID(orgName));
					u2f_name(&ctx, V_ASN1_UTF8, !!serial);
				}
				SEQ_END(ctx);
			}
			SEQ_END(ctx);
		...
		SEQ_START(ctx, V_SEQ, SEQ_SMALL) {
			SEQ_START(ctx, V_SET, SEQ_SMALL) {
				SEQ_START(ctx, V_SEQ, SEQ_SMALL) {
					...
					u2f_name(&ctx, V_ASN1_ASCII, !!serial);
				}
				SEQ_END(ctx);
			}
			SEQ_END(ctx);
