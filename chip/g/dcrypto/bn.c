...
size_t bn_len(const struct LITE_BIGNUM* a) {
	size_t n = a->dmax - 1;
	while (BN_DIGIT(a, n) == 0 && n) --n;
	return n + 1;
}
...
{
	const uint32_t div = BN_DIGIT(v, 0);
	uint32_t rem = 0;
	int i;
 ...
 */
int DCRYPTO_bn_div_ex(struct LITE_BIGNUM* q,
		struct LITE_BIGNUM* r,
		const struct LITE_BIGNUM* u, int m,
		const struct LITE_BIGNUM* v, int n)
{
	...
	int64_t t, k;
	int s, i, j;
	uint32_t vn[RSA_MAX_WORDS];
	uint32_t un[RSA_MAX_WORDS + 1];
	...
	if (n == 1) {
		return bn_div_word_ex(q, r, u, m, v, n);
	}

	/* Compute shift factor to make qtop have high bit set. */
	s = 0;
	while ((qtop & 0x80000000) == 0) {
		s = s + 1;
		qtop = qtop << 1;
	}
	...
	if (s != 0) {
		for (i = n - 1; i > 0; i--) {
			vn[i] = (BN_DIGIT(v, i) << s) | (BN_DIGIT(v, i - 1) >> (32 - s));
		}
		vn[0] = BN_DIGIT(v, 0) << s;

		un[m] = BN_DIGIT(u, m - 1) >> (32 - s);
		for (i = m - 1; i > 0; i--) {
			un[i] = (BN_DIGIT(u, i) << s) | (BN_DIGIT(u, i - 1) >> (32 - s));
		}
		un[0] = BN_DIGIT(u, 0) << s;
	} else {
		for (i = 0; i < n; ++i) vn[i] = BN_DIGIT(v, i);
		for (i = 0; i < m; ++i) un[i] = BN_DIGIT(u, i);
		un[m] = 0;
	}
	...
	for (j = m - n; j >= 0; j--) {
		...
		if (un[j + n] == vn[n - 1]) {
			...
			/* Top three of un divided by top two of vn. */
			uint64_t rhat = ((uint64_t)un[j + n] << 32) + un[j + n - 1];
			qd = rhat / vn[n - 1];
			rhat = rhat - (uint64_t)qd * vn[n - 1];
			while ((rhat >> 32) == 0 &&
				(uint64_t)qd * vn[n - 2] > (rhat << 32) + un[j + n - 2]) {
				qd = qd - 1;
				rhat = rhat + vn[n - 1];
			}
		}
	...
	if (r != NULL) {
		...
		if (s != 0) {
			for (i = 0; i < n - 1; i++) {
				BN_DIGIT(r, i) = (un[i] >> s) | (un[i + 1] << (32 - s));
			}
...
static void bn_set_bn(struct LITE_BIGNUM *d, const struct LITE_BIGNUM *src, size_t n)
{
	size_t i = 0;
	for (; i < n && i < d->dmax; ++i) BN_DIGIT(d, i) = BN_DIGIT(src, i);
	for (; i < d->dmax; ++i) BN_DIGIT(d, i) = 0;
}
...
{
	int src_len = bn_len(src);
	int div_len = bn_len(divisor);
	int i, result;

	if (src_len < div_len) return 0;

	result = DCRYPTO_bn_div_ex(quotient, remainder,
			src, src_len,
			divisor, div_len);
		...
		const struct LITE_BIGNUM *mod)
{
	...
	uint32_t nT_buf[RSA_MAX_WORDS + 1];
	uint32_t T_buf[RSA_MAX_WORDS + 1];
	...
	/* Trim nR */
	while (nr_len && BN_DIGIT(&nR, nr_len - 1) == 0) --nr_len;

	while (nr_len) {
		size_t q_len = r_len - nr_len + 1;

		/* (r, nr) = (nr, r % nr), q = r / nr */
		if (!DCRYPTO_bn_div_ex(&Q, pR, pR, r_len, pnR, nr_len)) {
			return 0;
		}
		...
		/* trim nR and Q */
		while (nr_len && BN_DIGIT(pnR, nr_len - 1) == 0) --nr_len;
		while (q_len && BN_DIGIT(&Q, q_len - 1) == 0) --q_len;
		...
		if (q_len == 1 && BN_DIGIT(&Q, 0) <= 2) {
			...
			/* Smallest operand first. */
			if (nt_neg) {
				/* Negative numbers use all digits thus pnT is large. */
				bn_mul_ex(&tmp, &Q, q_len, pnT);
			} else {
				int nt_len = bn_len(pnT);
				if ( q_len < nt_len) {
					bn_mul_ex(&tmp, &Q, q_len, pnT);
				} else {
