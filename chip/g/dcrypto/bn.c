...
void bn_init(struct BIGNUM *b, uint8_t *buf, size_t len)
{
...
static void bn_mont_mul_add(struct BIGNUM *c, const uint32_t a, const struct BIGNUM *b,
			const uint32_t nprime, const struct BIGNUM *N)
{
	uint32_t A, B, d0;
	int i;
		...
		const struct BIGNUM *exp, const struct BIGNUM *N)
{
	...
	for (i = exp->dmax * BN_BITS2 - 1; i >= 0; i--) {
		...
		/* Poke the watchdog.
		 * TODO(ngm): may be unecessary with
