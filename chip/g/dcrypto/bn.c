		...
		const struct BIGNUM *MOD)
{
	...
	/* Add a word to avoid dealing with carry */
	uint32_t A_buf[RSA_MAX_WORDS + 1];
	/* Add a word to avoid dealing with carry */
	uint32_t B_buf[(RSA_MAX_WORDS / 2) + 1];
	/* Add a word to avoid dealing with carry */
	uint32_t C_buf[RSA_MAX_WORDS + 1];
	/* Add a word to avoid dealing with carry */
	uint32_t D_buf[(RSA_MAX_WORDS / 2) + 1];
	...
	for (i = 0;; i++) {
		...
		if (bn_is_even(&U)) {
			...
			if (bn_gte(&U, &V)) {
				assert(!bn_sub(&U, &V));
				assert(!bn_signed_sub(&A, &a_neg, &C, c_neg));
				assert(!bn_signed_sub(&B, &b_neg, &D, d_neg));
				if (bn_is_zero(&U))
					break;  /* done. */
			} else {
				assert(!bn_sub(&V, &U));
				assert(!bn_signed_sub(&C, &c_neg, &A, a_neg));
				assert(!bn_signed_sub(&D, &d_neg, &B, b_neg));
			}
		}
		if ((i + 1) % 1000 == 0)
