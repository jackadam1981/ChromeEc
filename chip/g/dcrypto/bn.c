 ...
 * any measure. */
static void bn_modexp_internal(struct LITE_BIGNUM *output, const struct LITE_BIGNUM *input,
		...
		uint32_t pubexp)
{
	...
	ccprintf("bn_modexp w/ %d bit modulus!\n", bn_bits(N));
		...
		uint32_t pubexp)
{
	...
	if ((bn_bits(N) & 255) == 0) {
		...
		dcrypto_modexp(output, input, exp, N, pubexp);
		return;
	} else 
#endif
	bn_modexp_internal(output, input, exp, N, pubexp);
}
