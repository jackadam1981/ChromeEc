...
/* r[] >>= 1. */
static void bn_rshift(struct BIGNUM *r, int highbit)
{
	int i;
	...
	BN_DIGIT(r, i) = (BN_DIGIT(r, i) >> 1) | (highbit << (BN_BITS2 - 1));
}
...
/* As per HAC Table 4.4, 3 rounds suffices for 2^80 security
 * @1024 bits; do a few more for good measure. */
#define ROUNDS 7
...
int DCRYPTO_bn_generate_prime(struct BIGNUM *p)
{
	int i;
	int j;
	/* Using a window size of 2048-bits results in a failure rate
	 * of ~0.5% @ 1024-bit candidates.  The failure rate rises to ~6%
	 * if the window size is halved. */
	...
	/* composites now populated, apply Miller-Rabin to candidates. */
	j = 0;
	for (i = 0; i < bn_bits(&composites); i++) {
		...
		/* Recover increment from the composites window. */
		diff_buf = (i << 1) - j;
		j = (i << 1);
