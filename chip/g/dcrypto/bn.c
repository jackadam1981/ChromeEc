 ...
 * PRIME1.
 *
 * Most byte entries in the array contain two sequential differencials between
 * two adjacent prime numbers, each differencial halved (as the difference is
 * always even) and packed into 4 bits.
 *
 * If a halved differencial value exceeds 0xf (and as such does not fit into 4
 * bits), the value is prepended by a byte of zero and then placed in the
 * array as is, without halving.
 *
 * If out of two consequitive differencials only the second one exceeds 0xf,
 * the first one still is put into the array in its own byte prepended by a
 * zero.
 *
 * This encoding allowed to reduce array size from 4096 to 2178 bytes. Using
 * 4096 of the first primes results in a 5-10% improvement in running time
 * over using the first 2048.
 */
...
int DCRYPTO_bn_generate_prime(struct LITE_BIGNUM *p)
{
	...
	for (i = 0; i < ARRAY_SIZE(PRIME_DELTAS); i++) {
		uint16_t rem;
		uint8_t two_deltas[2];
		uint8_t delta = PRIME_DELTAS[i];
		int k;
		int m;

		if (delta) {
			two_deltas[0] = (delta >> 4) << 1;
			two_deltas[1] = (delta & 0xf) << 1;
			m = 2;
		} else {
			i += 1;
			two_deltas[0] = PRIME_DELTAS[i + 1];
			m = 1;
		}
		for (k = 0; k < m; k++) {
			prime += two_deltas[k];
