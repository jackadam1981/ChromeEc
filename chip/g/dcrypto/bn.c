...
/* r[] >>= 1. */
static void bn_rshift(struct BIGNUM *a, int highbit)
{
	int i;

	for (i = 0; i < a->dmax - 1; ++i) {
		uint32_t accu = (BN_DIGIT(a, i) >> 1);

		accu |= (BN_DIGIT(a, i + 1) << (BN_BITS2 - 1));
		BN_DIGIT(a, i) = accu;
	}
	BN_DIGIT(a, i) = (BN_DIGIT(a, i) >> 1) |
		(highbit << (BN_BITS2 - 1));
}
...
void bn_mul(struct BIGNUM *c, const struct BIGNUM *a, const struct BIGNUM *b)
{
	...
	memset(c->d, 0, c->dmax * sizeof(uint32_t));
	for (i = 0; i < a->dmax; i++) {
...
/* d = (e ^ -1) mod MOD  */
void bn_modinv_vartime(struct BIGNUM *r, uint32_t e, const struct BIGNUM *MOD)
{
