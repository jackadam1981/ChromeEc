...
static void init_det_k(const uint32_t *x, const uint32_t *m, uint32_t *k,
		       uint32_t *v)
{
	HASH_CTX ctx;
	uint32_t h1[SHA256_DIGEST_WORDS];
	/* h1 = H(m) */
	DCRYPTO_SHA256_init(&ctx, 0);
	HASH_update(&ctx, m, sizeof(*m));
		...
		p256_int *r, p256_int *s)
{
	...
	return dcrypto_p256_ecdsa_internal(key, message, r, s, stir_det_k);
}
