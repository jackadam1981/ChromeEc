...
static int montmul(struct DMEM_ctx *ctx,
		uint32_t pA, uint32_t pB, uint32_t pOut) {
...
#define MONTOUT(ctx, a, b) \
	montout(ctx, DMEM_INDEX(ctx, a), DMEM_INDEX(ctx, b))

static int montout(struct DMEM_ctx *ctx,
		uint32_t pA, uint32_t pOut) {
...
#define MODEXP(ctx, in, exp, out) \
	modexp(ctx, CF_modexp_adr, DMEM_INDEX(ctx, RR), \
		DMEM_INDEX(ctx, in), DMEM_INDEX(ctx, exp), DMEM_INDEX(ctx, out))

#define MODEXP_BLINDED(ctx, in, exp, out) \
	modexp(ctx, CF_modexp_blinded_adr, DMEM_INDEX(ctx, RR), \
		DMEM_INDEX(ctx, in), DMEM_INDEX(ctx, exp), DMEM_INDEX(ctx, out))

static int modexp(struct DMEM_ctx *ctx, uint32_t adr, uint32_t rr,
		uint32_t pIn, uint32_t pExp, uint32_t pOut) {
	/* in = in * RR */
	ctx->in_ptrs.pA = pIn;
	ctx->in_ptrs.pB = rr;
	ctx->in_ptrs.pC = pIn;
