...
#define START_PROFILE(x) x -= cyclecounter()
#define END_PROFILE(x) x += cyclecounter()
...
static void run_sha512_cmd(void)
{
	...
	uint32_t sw_time = 0, hw_time = 0;
	int i;

	t_transform = 0;
	t_dcrypto = 0;

	START_PROFILE(sw_time);
	SHA512_init(&sw);
