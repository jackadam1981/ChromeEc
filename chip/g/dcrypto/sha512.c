...
#define CF_expandw_adr 0
    ...
    0x0c000000, /* ret */
/* } */
/* @0x52: function Sha512_a[125] { */
#define CF_Sha512_a_adr 82
    ...
    0x0c000000, /* ret */
/* } */
/* @0xcf: function Sha512_b[125] { */
#define CF_Sha512_b_adr 207
    0x68588d40, /* rshi r22, r13, r4 >> 64 */
    ...
    0x0c000000, /* ret */
/* } */
/* @0x14c: function compress[62] { */
#define CF_compress_adr 332
    0xfc000000, /* nop */
    ...
    0x8c440800, /* ldc *1, *0++ */
    0x8c081900, /* ld *2, *1++ */
    0x08000052, /* call &Sha512_a */
    0x8c440800, /* ldc *1, *0++ */
    0x8c081900, /* ld *2, *1++ */
    0x080000cf, /* call &Sha512_b */
    0xfc000000, /* nop */
    /*		   ) */
...
static void DCRYPTO_SHA512_Transform(LITE_SHA512_CTX *ctx)
{
	int result = 0;
	...
	/* Pass in buf[] */
	copy_words(ctx->buf, p512->buf, ARRAY_SIZE(p512->buf));
...
static const uint8_t *DCRYPTO_SHA512_final(LITE_SHA512_CTX *ctx)
{
	...
	DCRYPTO_SHA512_update(ctx, (uint8_t *) "\x80", 1);
	while ((ctx->count & 127) != 112) {
		DCRYPTO_SHA512_update(ctx, (uint8_t *) "\0", 1);
	}
	for (i = 0; i < 8; ++i) {
		DCRYPTO_SHA512_update(ctx, (uint8_t *) "\0", 1);
	}
	for (i = 0; i < 8; ++i) {
		...
		cnt <<= 8;
		DCRYPTO_SHA512_update(ctx, &tmp, 1);
	}
...
static void run_sha512_cmd(void)
{
	...
	const uint8_t *sw_digest;
	const uint8_t *hw_digest;
	uint32_t sw_time, hw_time;
	int len = strlen(msg);
	int i;

	sw_time = -cyclecounter();
	SHA512_init(&sw);
	for (i = 0; i < N; ++i) {
		HASH_update(&sw, msg, len);
	}
	sw_digest = HASH_final(&sw);
	sw_time += cyclecounter();

	hw_time = -cyclecounter();
	DCRYPTO_SHA512_init(&hw);
	for (i = 0; i < N; ++i) {
		HASH_update(&hw, msg, len);
	}
	hw_digest = HASH_final(&hw);
	hw_time += cyclecounter();

	ccprintf("sw(%u): ", sw_time);
	for (i = 0; i < 64; ++i)
		ccprintf("%02x", sw_digest[i]);
	ccprintf("\n");

	ccprintf("hw(%u): ", hw_time);
	for (i = 0; i < 64; ++i)
...
static int cmd_sha512(int argc, char *argv[])
{
...
DECLARE_SAFE_CONSOLE_COMMAND(sha512, cmd_sha512, NULL, NULL);
#endif
