...
void ladder_out(uint32_t output[8])
{
 ...
 */
static const uint32_t KL_SEED_ATTEST[8] = { 0x40640139, 0xcbfacf4a, 0xc2c2c27b, 0x9f2d9cba, 0x8e3d41c3, 0x43bfe954, 0x81cd534f, 0x23804b05 };
static const uint32_t KL_SEED_WRAP[8] =   { 0x4161c150, 0xb43c0c3c, 0xb1c62871, 0xa2abfc84, 0x666d2091, 0x47c8f902, 0xdc5b993e, 0xe89daab8 };
static const uint32_t KL_SEED_ORIGIN[8] = { 0x06a7f502, 0x213c40c4, 0x5f3d4f19, 0x52ca943b, 0x234e2fae, 0xddb6dc13, 0xaa9556c0, 0xb2d538f1 };
int kl_derive(const uint32_t salt[8],
	...
	if (error)
		ccprintf("%s:1 error %x/%x\n", __func__, error, GREG32(KEYMGR, HKEY_ERR_FLAGS));
	if (!error)
		ladder_out(output);
...
int kl_derive_wrap(const uint32_t input[8], uint32_t output[8]) {
	return kl_derive(KL_SEED_WRAP, input, output);
}
