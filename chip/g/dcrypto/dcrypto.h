 ...
 *
 * The internal hash state of the hardware implementation is
 * non-exportable.  Therefore, if callers require the ability to
 * export SHA state, then the sw_required argument should be set to
 * true.  If sw_required is set to false, then a hardware context is
 * initialized based on availability, with a fallback to software.
 */
