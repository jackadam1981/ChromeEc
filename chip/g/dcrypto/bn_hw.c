...
/*
 * The block below contains instructions for Montgomery exponentiation
 * on the crypto accelerator.  Multiple of 256 bit modulii are supported.
 *
 * The instructions are produced by an assembler (which is not part of
 * this repository).
 *
 * The block contains several functions, whose offsets into the instruction
 * sequence are declared by CF_ #defines.   The individual functions may be
 * invoked via dcrypto_call(CF_foo).
 */
