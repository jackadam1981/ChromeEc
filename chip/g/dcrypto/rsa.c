			...
			enum hashing_mode hashing, const char *label)
{
	...
	bad = DCRYPTO_memcmp(phash, HASH_final(&ctx), hash_size);
	bad |= padded[0];
				...
				enum hashing_mode hashing)
{
	...
	if (padded[i++] != 0)
		return 0;
	if (DCRYPTO_memcmp(&padded[i], der, der_size) != 0)
		return 0;
	i += der_size;
	return DCRYPTO_memcmp(msg, &padded[i], hash_size) == 0;
}
			...
			enum hashing_mode hashing)
{
	...
	bad |= DCRYPTO_memcmp(padded + db_len, HASH_final(&ctx), hash_size);
	return !bad;
}
