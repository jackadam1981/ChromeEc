...
void sps_print_sts(void)
{
	CPRINTS("sps tx:%d", sps_sts_tx_count);
	CPRINTS("sps rx:%d", sps_sts_rx_count);
}
...
#endif
	for (i = 0; i < (SPS_FIFO_CMD_SIZE>>2); i++) {
		d32 = (0xFACE0000 | (i<<8));
		put_bytes_to_out_blob((uint8_t *)&d32, sizeof(d32));
	}
}
...
#else
	put_bytes_to_blob((uint8_t *)&d32, sizeof(d32));
#endif
	return;
}
...
static void sps_tx_interrupt(int port)
{
	...
	while (num > 0) {
#ifdef MYFIFO128
		get_bytes_from_blob((uint8_t *)&d32, sizeof(d32));
		fifo128_deque32(&g_sps_rx_fifo, &d32);
#else
		get_bytes_from_blob((uint8_t *)&d32, sizeof(d32));
#endif
		c = sps_push32(port, d32);
		sps_sts_tx_count += c;
		num -= c;
	}
	GWRITE_FIELD(SPS, ICTRL, TXFIFO_LVL, 0);
...
{
#if 0
	hook_call_deferred(_sps0_rx_interrupt, 0);
#endif
}
void blob_is_ready_to_emit_bytes(void)
{
#if 0
	hook_call_deferred(_sps0_tx_interrupt, 0);
#endif
}
