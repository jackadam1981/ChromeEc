...
static void sps_rx_interrupt(uint32_t port, int cs_deasserted)
{
	...
	if (cs_deasserted) {
		if (seen_data) {
			...
			/*
			 * If ap_start_ack_completion() returns true, it means
			 * this platform supports INT_AP extension, and it
			 * started or scheduled a INT_AP assertion. In that
			 * case, let it return. Otherwise, it should generate an
			 * INT_AP_L assertion in line.
			 */
