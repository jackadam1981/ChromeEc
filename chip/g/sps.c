...
/* Hardware pointers use one extra bit */
 ...
 *
 * If RX fifo is wrapping around, the callback will be called twice whith to
 * flat pointers.
 *
 * If the CS has been deasseted, after all remaining RX FIFO data has been
 * passed to the callbak the callback is called one last time with zero data
 ...
 */
  ...
  * three satates: 0 - not started, 1 -receiving frame, 2 - frame finished.
  */
static int rx_state;
				 ...
				 size_t data_size, int cs_status)
{
	...
	if (rx_state == 0) {
		...
		rx_state = 1; /* Enter 'receiving frame' state. */
		frame_base = (frame_base + 1) % 3;
		frame_index = frame_base;

		/*
		 * If we won't be able to receve this much enter 'frame
		 * finished' state.
		 */
