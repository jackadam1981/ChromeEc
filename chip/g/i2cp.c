 ...
 *
 * The interface is has two register files, 64 bytes each, one for storing
 * data received from the controller, and one for storing data to be transmitted
 * to the controller. Both files are accessed only as 4 byte quantities, so the
 ...
 * register showing where the interface accessed the file last, comparing it
 ...
 * with it showing where was the latest BIT the interface transmitted.
 *
 * The interface can generate interrupts on three different conditions:
...
/* Number of times SDA must be low between i2c writes before the i2cp interface
 * is restarted.
 *
...
/*
 * Restart the i2cp interface if the interface gets stuck transmitting a 0 on
 * SDA.
 *
 * This can happen anytime the i2cp interface has control of SDA and the
 * i2cc happens to fail and stops clocking.
 *
 * For example when the i2cp interface is:
 * 1) Transmitting an ACK for the periph address byte.
 ...
 *
 * We don't have to deal with the scenario where the interface gets stuck
 * transmitting a 1 on SDA since the i2cc can recover the bus by issuing a
 * normal transaction. The i2cc will at minimum clock 9 times on any
 * transaction. This is enough for the periph to complete its current operation
 * and NACK.
 */
static void poll_read_state(void)
{
	if (!ap_is_on() || gpio_get_level(GPIO_I2CP_SDA)) {
		 ...
		 * occurring. We will assume the interface is stuck.
