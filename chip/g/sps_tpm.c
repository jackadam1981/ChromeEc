 ...
 *
 * The master sends 4 bytes:       [R/W+0x40+size-1] [Addr] [Addr] [Addr]
 ...
 *   MOSI: 43 aa bb cc 11 11 11 11 22 33 44
 ...
 *   MOSI: c3 aa bb cc xx xx xx xx xx xx xx xx
...
static enum sps_state {
	...
	/* Finished the write transaction, waiting for transmit to finish. */
...
static void tmp_rx_handler(uint8_t *data, size_t data_size, int cs_enabled)
{
	...
	if (!cs_enabled &&
		...
		/*
		 * Chip select deasserted, we're not in the receive state,
		 * nothing left to do.
		 */
	...
	switch (sps_tpm_state) {
	case SPS_TPM_STATE_RECEIVING_HEADER:
	case SPS_TPM_STATE_RECEIVING_WRITE_DATA:
		/* still gathering bytes */
		break;

	case SPS_TPM_STATE_DISABLED:
		/*
		 * The master started a transaction but we weren't ready for
		 * it. Just ignore everything until the master gives up.
		 */
		CPRINTS("TPM SPI not ready (in state %d)", sps_tpm_state);
...
static void sps_tmp_enable(void)
{
	rxbuf_count = 0;
	rxbuf_needed = 4;
...
DECLARE_HOOK(HOOK_INIT, sps_tmp_enable, HOOK_PRIO_DEFAULT);

static void sps_tmp_disable(void)
{
	sps_tpm_state = SPS_TPM_STATE_DISABLED;
