...
/* Update the RX descriptor stuff after each SETUP/OUT packet is received so
 * we can prepare to receive another without losing track of this one. */
...
int load_in_fifo(const void *source, uint32_t len)
{
	...
	for (d = l = 0; len >= USB_MAX_PACKET_SIZE; d++) {
		...
		l = d;
		print_later("  desc[%d] at 0x%08x, len 0x%x", d,
			    (uint32_t)ep0_in_desc[d].addr,
			    USB_MAX_PACKET_SIZE, 0, 0);
	}
	...
	if (len || short_packet) {
		...
		l = d;
		print_later("  desc[%d] at 0x%08x, len 0x%x", d,
			    (uint32_t)ep0_in_desc[d].addr, len, 0, 0);
	}
	/* Mark the last descriptor as last. */
	ep0_in_desc[l].flags |= (DIEPDMA_LAST | DIEPDMA_IOC);
	cur_in_desc = ep0_in_desc; // + l ; HEY?

	print_later("  desc[%d] is last, cur_in_desc is 0x%08x", l,
		    (uint32_t)cur_in_desc, 0, 0, 0);
...
static void expect_setup_packet(void)
{
	...
	/* Let it run */
	// HEY: Do we need DXEPCTL_CNAK here too?
...
/* Return nonzero to stall the Data and Status Phase. */
					   ...
					   struct usb_setup_packet *req)
{
	...
	case USB_REQ_SET_CONFIGURATION:
		/* TODO: Sanity-check this? We've only got one. */
...
static void handle_setup(enum table_case tc)
{
	...
	/* We say "no" to intentionally unhandled requests by stalling the
	 * Data and/or Status stage. */
	if (bytes < 0) {

		/* Stall any OUT request */
// HEY: just stall everything		if (data_phase_out)
			stall_out_fifo();
...
static void ep0_interrupt(uint32_t intr_on_out, uint32_t intr_on_in)
{
	 ...
	 * if they really apply. I think they don't if we didn't actually get
	 * an interrupt. */
	...
	print_later("next_out_idx %d flags 0x%08x",
		    next_out_idx, next_out_desc->flags, 0, 0, 0);
				expect_setup_packet();
			}
		}
		break;
	...
	case DATA_STAGE_IN:
		if (intr_on_in && (diepint & DIEPINT_XFERCOMPL)) {
			print_later("IN is complete? Maybe? How do we know?",
				    0, 0, 0, 0, 0);;
			/* IF the IN transfer is done, stall any new INs, but
			 * keep waiting for the status phase to complete. */
//HEY: do nothing. 			stall_in_fifo();
			/* The documentation is stupid. Do I stop here? */
		}
		/* I don't think we can look at any of the rest of the choices
		 * If we didn't actually get an OUT interrupt. */
		if (!intr_on_out) {
			/* Do I need this too? */
			GR_USB_DOEPCTL(0) = DXEPCTL_EPENA;
			break;
	...
	case NO_DATA_STAGE:
		...
		/* SIE has seen the IN. Let it in. */
		if (intr_on_out && (tc == TABLE_CASE_B)) {
			print_later("IN has been detected...", 0, 0, 0, 0, 0);;
			GR_USB_DIEPCTL(0) = DXEPCTL_CNAK | DXEPCTL_EPENA;
			break;
		}

		/* Yep, that was the IN. All done. */
		if (intr_on_in && (diepint & DIEPINT_XFERCOMPL)) {
		...
		/* Got some sort of OUT instead. */
		if (intr_on_out && (tc == TABLE_CASE_A || tc == TABLE_CASE_C)) {
			if (sr) {
		...
		/* Expected an IN, but got something else. Now what? */
		report_error();
		expect_setup_packet();
		break;
	}
...
void usb_interrupt(void)
{
	...
	/* Endpoint interrupts */
	if (oepint || iepint) {
		/* Note: It appears that the DAINT bits only trustworthy for
