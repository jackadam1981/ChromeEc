...
// TODO(crosbug.com/p/33919): should this be commented out?
...
void usb_init(void)
{
	...
	/*
	 * WORKAROUND: FIFO size is limited to 256 entries.
	 * Use 128 entries for EP_INFO and 128 entries for RX/TX FIFOs.
	 */
	GR_USB_GDFIFOCFG = (0x80 << 16) | 0x100;
	 ...
	 * WORKAROUND: only 128 usable entries
	 * RX FIFO needs more than 64 entries (for reserved space)
	 * set RX FIFO to 80 entries
	 * set TX0-TX2 FIFO to 64 bytes = 16 (x 32-bit) entries
	 * let TX4-TX15 uninitialized for now (WORKAROUND).
	 */
	GR_USB_GRXFSIZ = 80;
