...
static void ep0_interrupt(uint32_t intr_on_out, uint32_t intr_on_in)
{
	...
	case DATA_STAGE_IN:
		if (intr_on_in && (diepint & DIEPINT_XFERCOMPL)) {
			...
			GR_USB_DIEPCTL(0) = DXEPCTL_EPENA;
			/* The documentation says to stall any further INs, but
			 * that's stupid because it'll destroy the packet we
			 * just tranferred to SPRAM, so don't. Also don't stop
			 * here, but keep looking at stuff. */
		}
...
static void ep0_stupid(void)
{
	print_later("ep0_stupid()", 0, 0, 0, 0, 0);
}

/* We don't really need this, since this endpoint is special. */
USB_DECLARE_EP(USB_EP_CONTROL, ep0_stupid, ep0_stupid, ep0_reset);
