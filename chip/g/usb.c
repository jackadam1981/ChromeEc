...
#if 1
/* do nothing */
#define print_later(...) udelay(10)		// HEY: This is stupid!
#else
/*
...
/* TEMPORARY: debugging interrupt stuff */
...
#define report_error(dummy)				\
	do {						\
		print_later("ERROR at usb.c line %d",	\
			    __LINE__, 0, 0, 0, 0);	\
	} while (0)
...
/* Update the RX descriptor stuff after each SETUP/OUT packet is received */
...
/* Load the EP0 IN FIFO buffer with some data (zero-length works too). */
int load_in_fifo(const void *source, uint32_t len)
{
	uint8_t *buffer = ep0_in_buf;
	int short_packet = !len;
	int d, l;

	print_later("load_in_fifo(0x%08x, 0x%x)",
		    (uint32_t)source, len, 0, 0, 0);
	...
	cur_in_desc = ep0_in_desc + l;

	return 0;
}

static void stall_out_fifo(void)
{
	print_later("stall_out_fifo()", 0, 0, 0, 0, 0);
	/*
	 * We should be able to do this with the normal Setup descriptor
	 * because the hardware will clear the stall when it gets a Setup
	 * token.
	 */
...
static void stall_in_fifo(void)
{
	print_later("stall_in_fifo()", 0, 0, 0, 0, 0);
	flush_in_fifo();
...
static void expect_setup_packet(void)
{
	...
	/* We don't care about IN packets right now, only OUT. Right? */
...
static void expect_status_phase_in(void)
{
	print_later("expect_status_phase_in()", 0, 0, 0, 0, 0);

	/* The next IN can just be a zero-length packet. */
	(void)load_in_fifo(0, 0);

	/* We don't need to be interrupted when it completes, but
	 * the next thing after that is another Setup packet. */
	expect_setup_packet();
...
static void expect_status_phase_out(void)
{
	print_later("expect_status_phase_out()", 0, 0, 0, 0, 0);

	/* We expect a zero-length OUT, but it could be a SETUP too. We'll
	 * have to deal with either one when it arrives. */
	next_out_desc->flags =
		DOEPDMA_RXBYTES(USB_MAX_PACKET_SIZE)
		| DOEPDMA_IOC | DOEPDMA_LAST;
...
static void expect_data_phase_in(void)
{
	print_later("expect_data_phase_in()", 0, 0, 0, 0, 0);

	/* We expect to send back some data */
	what_am_i_doing = DATA_STAGE_IN;

	/* We apparently have to do this every time we transmit anything */
	flush_in_fifo();

	/* When the IN is done, expect an OUT for the status phase */
	expect_status_phase_out();
...
static void expect_data_phase_out(void)
{
	print_later("expect_data_phase_out()", 0, 0, 0, 0, 0);

	/* TODO: We don't support this. We should. */
	report_error();
	stall_out_fifo();
	stall_in_fifo();

	/* Whether we stall or not, Setup packets are always accepted */
	expect_setup_packet();
}

static int handle_setup_with_in_stage(struct usb_setup_packet *req)
{
	const void *data = 0;
	uint32_t len = 0;
	int ugly_hack = 0;
	...
	print_later("handle_setup_with_in_stage()", 0, 0, 0, 0, 0);
...
static int handle_setup_with_out_stage(struct usb_setup_packet *req)
{
	print_later("handle_setup_with_out_stage()", 0, 0, 0, 0, 0);

	/* TODO: We don't support any of these. We should. */
	return -1;
}

static int handle_setup_with_no_data_stage(struct usb_setup_packet *req)
{
	uint8_t set_addr;

	print_later("handle_setup_with_no_data_stage()", 0, 0, 0, 0, 0);
...
static void handle_setup(void)
{
	...
	if (0 == (req->bmRequestType & (USB_TYPE_MASK | USB_RECIP_MASK))) {
		...
		if (data_phase_in)
			stall = handle_setup_with_in_stage(req);
		else if (data_phase_out)
			stall = handle_setup_with_out_stage(req);
		else
			stall = handle_setup_with_no_data_stage(req);
	} else if (USB_RECIP_INTERFACE ==
		   (req->bmRequestType & USB_RECIP_MASK)) {
		/* Interface-specific requests */
		uint8_t iface = req->wIndex & 0xff;
		print_later("iface %d (vs %d)",
			    iface, USB_IFACE_COUNT, 0, 0, 0);
		if (iface < USB_IFACE_COUNT) {
			stall = usb_iface_request[iface](req);
			print_later("  iface returned %d", stall, 0, 0, 0, 0);
		}
	} else {
	...
	print_later("data_phase_in %d data_phase_out %d stall %d",
		    data_phase_in, data_phase_out, stall, 0, 0);

	/* We say "no" for intentionally unhandled requests by stalling the
	 * Data or Status stage. */
	if (stall) {

		/* Stall any OUT request */
		if (data_phase_out)
			stall_out_fifo();

		/* Always stall IN, to handle the no-data-stage case too */
		stall_in_fifo();

		/* Wait for the next Setup packet */
		expect_setup_packet();

	} else {
		if (data_phase_in)
			expect_data_phase_in();
		else if (data_phase_out)
			expect_data_phase_out();
		else
			expect_status_phase_in();
	}
}

/* This handles both IN and OUT interrupts for EP0 */
static void ep0_interrupt(void)
{
	uint32_t doepint, diepint;
	enum table_case tc;
	int sr;

	/* Determine the interrupt cause and clear the bits quickly */
	doepint = diepint = 0;
	if (GR_USB_DAINTMSK & DAINT_OUTEP(0)) {
		doepint = GR_USB_DOEPINT(0);
		GR_USB_DOEPINT(0) = doepint;
	}
	if (GR_USB_DAINTMSK & DAINT_INEP(0)) {
		diepint = GR_USB_DIEPINT(0);
		GR_USB_DIEPINT(0) = diepint;
	}

	print_later("doepint0 0x%08x diepint0 0x%08x what %d",
		    doepint, diepint, what_am_i_doing, 0, 0);
	...
	// HEY: Can this logic be simplified? Should it be?
	switch (what_am_i_doing) {
	case WAITING_FOR_SETUP_PACKET:
		if ((tc == TABLE_CASE_A || tc == TABLE_CASE_C) && sr)
			handle_setup();
		break;

	case DATA_STAGE_IN:
		if (diepint & DIEPINT_XFERCOMPL) {
			print_later("IN is complete, still need OUT for Status",
				    0, 0, 0, 0, 0);;
			/* The IN transfer is done. Stall any new INs, but
			 * keep waiting for the status phase to complete. */
			stall_in_fifo();
			/* The documentation is stupid. Don't stop here. */
		}
		if (tc == TABLE_CASE_B) {
		...
		/* Not only got a Setup, but we've seen the data packet too */
		if (tc == TABLE_CASE_C) {
			print_later("Early Setup, and data packet detected...",
				    0, 0, 0, 0, 0);
			handle_setup();
			break;
		}
...
{
	print_later("ep0_reset()", 0, 0, 0, 0, 0);
...
/* Set up the handlers for this endpoint */
USB_DECLARE_EP(0, ep0_interrupt, ep0_interrupt, ep0_reset);
...
void usb_interrupt(void)
{
	...
	if (status & (GINTSTS(OEPINT) | GINTSTS(IEPINT))) {
		...
		int ep;
		print_later("  DAINT 0x%08x", daint, 0, 0, 0, 0);

		/* EP0 has a combined IN/OUT handler. Only call it once. */
		if (daint & (DAINT_INEP(0) | DAINT_OUTEP(0)))
			ep0_interrupt();

		/* Invoke the registered IN and OUT functions for the others.
		 * Each handler must clear their DIEPINTn/DOEPINTn bits. */
