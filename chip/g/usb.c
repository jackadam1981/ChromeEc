...
static void ep0_rx(void)
{
	...
	//CPRINTF("rxEPINT DO %08x DI %08x flags rx %08x tx %08x\n", epint, GR_USB_DIEPINT(0),ep0_out_desc.flags, ep0_in_desc.flags); /* DEBUG ONLY */
...
static void ep0_tx(void)
{
	...
	//CPRINTF("txEPINT DI %08x DO %08x flags rx %08x tx %08x\n", epint, GR_USB_DOEPINT(0),ep0_out_desc.flags, ep0_in_desc.flags); /* DEBUG ONLY */
...
static void ep0_reset(void)
{
	...
	ep0_in_desc.flags = DIEPDMA_TXBYTES(0) | DIEPDMA_LAST | DIEPDMA_BS_HOST_RDY | DIEPDMA_IOC;
...
void usb_init(void)
{
	...
	GR_USB_GUSBCFG = GUSBCFG_PHYSEL_FS | GUSBCFG_FSINTF_6PIN | GUSBCFG_TOUTCAL(7) | (9 << 10);
	usb_softreset();
	...
	GR_USB_GRXFSIZ = 16 * 4; /* USB2.0 FS: 64-byte packets max */
	GR_USB_GNPTXFSIZ = (16 * 4) | (16 << 16);
	for (i = 1; i < 16; i++)
		GR_USB_DIEPTXF(i) = (16*4 + i*16) | (16 << 16);
	/* Flush all FIFOs */
