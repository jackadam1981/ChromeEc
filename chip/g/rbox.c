...
/*
 * Check RDD connection, and setup or delay EC_RST_L release.
 */
static void rbox_check_rdd(void)
{
#ifdef CR50_DEV
	...
	if (rbox_powerbtn_is_pressed()) {
		if (GREAD(RDD, CUR_WAIT_TIME_COUNTER)) {
			...
			hook_call_deferred(&rbox_check_rdd_data,
					RDD_WAIT_TIME);
			return;
		}
...
static void rbox_release_ec_reset(void)
{
	...
	/*
	 * After a POR, if it finds Power button held, then it delays releasing
	 * EC_RST_L until it is able to recognize RDD connection.
	 */
