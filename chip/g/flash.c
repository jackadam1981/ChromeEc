...
#ifdef CR50_SQA
	/*
	 * SQA images erase INFO1 RW mask, but do do not allow erasing board
	 * ID.
	 *
	 * If compiled with CR50_SQA=1, board ID flags will be erased, if
	 * compiled with CR50_SQA=2 or greater, board ID flags can be set to
	 * an arbitrary value passed in in the command line, but guaranteeing
	 * not to lock up the currently running image.
	 */
	{
		uint32_t flags = 0;
#if CR50_SQA > 1
