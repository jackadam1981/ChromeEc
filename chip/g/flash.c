 ...
 */
static void log_do_flash_op_errors(enum flash_event_type event)
{
#ifdef CONFIG_FLASH_LOG
	uint32_t flash_error_count = GREG32(PMU, PWRDN_SCRATCH24) + 1;

	/* Don't log every error, just 1st, 65th, etc*/
	if ((flash_error_count & 63) == 1)
		flash_log_add_event(event, sizeof(flash_error_count),
				    &flash_error_count);
	/* Count errors in scratch regs to survive sleep. */
	GREG32(PMU, PWRDN_SCRATCH24) = flash_error_count;
#else
	(void)event; /* silence compiler warning */
#endif
}
		       ...
		       int words, const uint8_t *data)
{
	...
	if (rv != EC_SUCCESS)
		log_do_flash_op_errors(FE_LOG_FLASH_ERROR_WRITE);
...
int flash_physical_info_read_word(int byte_offset, uint32_t *dst)
{
	...
	mutex_unlock(&flash_mtx);
	if (ret != EC_SUCCESS)
		log_do_flash_op_errors(FE_LOG_FLASH_ERROR_READ);
...
int flash_physical_erase(int byte_offset, int num_bytes)
{
	...
	while (num_bytes) {
		...
		if (ret) {
			log_do_flash_op_errors(FE_LOG_FLASH_ERROR_ERASE);
