...
int tx_stream_handler(struct usb_stream_config const *config)
{
	...
	count = ( {      size_t result; if (config->tx_size == 1) result = queue_remove_unit(config->consumer.queue, config->tx_ram); else result = queue_remove_units(config->consumer.queue, config->tx_ram, config->tx_size); result;});
	if (count)
