...
static uint8_t g_sps_data[SPS_FIFO_CMD_SIZE];
static uint8_t g_tx_data[SPS_FIFO_CMD_SIZE];
static uint8_t g_rx_data[SPS_FIFO_CMD_SIZE];
...
static int sps_push(uint32_t inst, const uint8_t data)
{
	...
	offset = GREG32_I(SPS, inst, TXFIFO_WPTR) & 0x2FF;
	sps_base[offset] = data;
	GREG32_I(SPS, inst, TXFIFO_WPTR) = (offset+1) & 0x2FF;
	return 1;
}
...
static int sps_pop(uint32_t inst, uint8_t *data)
{
	...
	*data = sps_top(inst);
	...
	offset  = GREG32_I(SPS, inst, RXFIFO_RPTR) & 0x2FF;
	GREG32_I(SPS, inst, RXFIFO_RPTR) = (offset + 1) & 0x2FF;
	return 1;
}
...
static void sps_init(void)
{
	...
	CPRINTS("sps: g_sps_data:%p", g_sps_data);
	CPRINTS("sps: g_tx_data:%p", g_tx_data);
	CPRINTS("sps: g_rx_data:%p", g_rx_data);
	for (i = 0; i < SPS_FIFO_CMD_SIZE; i++)
		g_sps_data[i] = g_tx_data[i] = i;
	g_sps_data[0] = g_tx_data[0] = SPS_FIFO_CMD_SIZE - 1;
}
...
static void sps_invoke_handler(uint8_t *data)
{
	return;
}
...
{
	sps_read(port, g_sps_data, SPS_FIFO_CMD_SIZE);
	sps_invoke_handler(g_sps_data);
	sps_sts_rx_count++;
}

static void sps_tx_interrupt(int port)
{
	sps_write(port, g_sps_data, SPS_FIFO_CMD_SIZE);
	sps_sts_tx_count++;
}
...
static int spi_sps_loopback_test(int val, int num)
{
	...
	memset(g_rx_data, 0x00, SPS_FIFO_CMD_SIZE);
	memset(g_tx_data, val, SPS_FIFO_CMD_SIZE);
	for (i = 0; i < SPS_FIFO_CMD_SIZE; i++)
		g_tx_data[i] = (val == 0xad) ? i : val;
	g_tx_data[0] = SPS_FIFO_CMD_SIZE - 1;

	CPRINTS("SPS FIFO BASE: %08X, %08X num:%d (%p %p)",
	...
	for (i = 0; i < num; i++) {
		...
		rc = spi_transaction(g_tx_data, tx_len, g_rx_data, rx_len);
		if (rc) {
		...
		rc = memcmp(g_rx_data, g_tx_data, SPS_FIFO_CMD_SIZE);
		if (rc) {
			CPRINTS("memcmp rc:%d", rc);
			for (i = 0; i < SPS_FIFO_CMD_SIZE; i++)
				CPRINTS("%02X %02X",
...
DECLARE_CONSOLE_COMMAND(spitest, command_spi,
			"[num] [seed]",
