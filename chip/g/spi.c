...
/*
 * SPI Max Packet Size: 128-Bytes
 */
#define SPI_MAX_PACKET_SIZE  128
...
static uint32_t g_tx_data[SPI_MAX_PACKET_SIZE>>2];
static uint32_t g_rx_data[SPI_MAX_PACKET_SIZE>>2];
#endif
		...
		const uint32_t *data, uint32_t len)
{
	...
	if (len > SPI_MAX_PACKET_SIZE)
		return 0;

	if (queue_space(&g_spi_tx_submission_queue) == 0)
		return 0;

	QUEUE_ADD_UNITS(&g_spi_tx_submission_queue, &req, sizeof(req));

	spi_length_set(port, len);

	dest = GREG32_ADDR_I(SPI, port, TX_DATA);
	spi_memcpy4((void *)dest, data, len);
...
static int spi_loopback_test(int val, int num, uint8_t pkt_size)
{
	int rc = 0, i, c;
	uint8_t tx_len = pkt_size;
	uint8_t rx_len = pkt_size;
	...
	for (i = 0; i < num; i++) {
		...
		tx_len = (pkt_size + 4*i) % (SPI_MAX_PACKET_SIZE+1);
		rx_len = (pkt_size + 4*i) % (SPI_MAX_PACKET_SIZE+1);

		CPRINTS("Test:%d %d", i, tx_len);
		loopback_pkt_seq++;
		for (c = 0; c < SPI_MAX_PACKET_SIZE/4; c++) {
			g_tx_data[c] = (val == SPI_TEST_DATA_ADDRESS_MODE) ?
					(val + c) : val;
			g_tx_data[c] <<= 16;
			g_tx_data[c] |= loopback_pkt_seq;
			g_rx_data[c] = 0;
		}
...
{
	char *e;
	uint32_t seed = SPI_TEST_DATA_ADDRESS_MODE, num = 32;
	uint8_t pkt_size = 4;
	...
	spi_loopback_test(seed, num, pkt_size);
