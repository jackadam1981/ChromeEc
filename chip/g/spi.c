...
static uint32_t spi_tx_count;
static uint32_t sps_tx_count;
static uint32_t sps_rx_count;
static uint8_t g_sps_data;
...
static void sps_init(void)
{
	...
	CPRINTS("sps: g_sps_data:%02X  addr:%p\n", g_sps_data, &g_sps_data);
...
static int spi_sps_loopback_test(int val, int num)
{
	...
	memset(tx_data, val, SELF_TEST_LEN);
	...
	for (i = 0; i < num; i++) {
		rc = spi_transaction(tx_data, tx_len, rx_data, 0);
		if (rc) {
			CPRINTS("spi tx error; rc:%d\n", rc);
			cflush();
			return rc;
		}
		...
		rc = spi_transaction(tx_data, 0, rx_data, rx_len);
		if (rc) {
			CPRINTS("spi tx error; rc:%d\n", rc);
			cflush();
			return rc;
		...
		rc = memcmp(rx_data, tx_data, SELF_TEST_LEN);
		if (rc) {
			CPRINTS("memcmp rc:%d\n", rc);
			return rc;
		}
