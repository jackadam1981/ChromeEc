...
{
	g_sps_data = ~d;
	CPRINTS("sps handler:%02X -> %02X", d, g_sps_data);
}
...
#define SELF_TEST_LEN 1
	...
	CPRINTS("SPS FIFO BASE: %08X, %08X\n",
		SPS_TX_FIFO_BASE_ADDR, SPS_RX_FIFO_BASE_ADDR);

	usleep(10);
	rc = spi_transaction(tx_data, tx_len, rx_data, 0);
	CPRINTS("SPI TX: %02X rc:%d cnt:%d", tx_data[0], rc, spi_tx_count);
	...
	CPRINTS("SPI RX: %02X %d", rx_data[0], rc);
	return memcmp(rx_data, tx_data, SELF_TEST_LEN);
}
