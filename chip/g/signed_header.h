 ...
 * contains number 1 for regular images OR the 'micro' version field, which
 * allows extra granularity when one needs to build an image slightly
 * different from a previously released one.
 *
 * Valid values of the 'micro' version field are in 2..255 range so that it
 * fits into a byte.
 */
