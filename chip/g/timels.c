...
/* Used to determine if there is a timer overshoot */
#define TIMER_MAX_THRESHOLD_US (TIMER_MAX / TICKS_PER_USEC - TIMER_MIN_US)
...
static inline uint32_t usec_to_ticks(uint64_t next_evt_us)
{
...
void timels_set_alarm(uint64_t next_evt_us)
{
...
{
	uint32_t next_evt_us = __hw_clock_event_get() - get_time().le.lo;
	 ...
	 * sleep and resume.
	 * If the next_event is farther away then the max threshold then it is
	 * likely that the system time was read right after the next event was
	 * scheduled to occur.
	 */
	if ((next_evt_us < TIMER_MIN_US) ||
	    (next_evt_us > TIMER_MAX_THRESHOLD_US)) {
