...
{
	GREG32(TIMELS, TIMER0_LOAD) = MIN(usec_to_ticks(next_evt_us),
					  TIMER_MAX);
}

int timels_setup_sleep(int is_deep_sleep)
{
	uint64_t next_evt_us = __hw_clock_event_get() - get_time().le.lo;
	 ...
	 * sleep and resume
	 */
	if ((__hw_clock_event_get() < get_time().le.lo) ||
	    (next_evt_us < TIMER_MIN_US)) {
