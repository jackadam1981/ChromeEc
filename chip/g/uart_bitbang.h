...
struct uart_bitbang_properties {
	...
	int baud_rate;
	uint8_t uart;
	/* 7:5 head, 4:2 tail 1:0 parity. */
	uint8_t head_tail_parity;
};

#define HEAD_MASK 0xE0
#define TAIL_MASK 0x1C
#define PARITY_MASK 0x03
#define HEAD_OFFSET 5
#define TAIL_OFFSET 2

#define EXTRACT_HEAD(htp) ((htp & HEAD_MASK) >> HEAD_OFFSET)
#define EXTRACT_TAIL(htp) ((htp & TAIL_MASK) >> TAIL_OFFSET)
#define EXTRACT_PARITY(htp) (htp & PARITY_MASK)

/* In order to bitbang a UART, a board must define a bitbang_config. */
extern struct uart_bitbang_properties bitbang_config[];
extern uint8_t bitbang_uart_count;
