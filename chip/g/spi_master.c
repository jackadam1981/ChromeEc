...
int spi_enable(int port, int enable)
{
	...
	if (enable) {
		...
		/* Set the clock divider to div, where freq / (div + 1). */
		GWRITE_FIELD_I(SPI, port, CTRL, IDIV, div);
...
static void spi_init(void)
{
	...
	/* Ensure the SPI ports are disabled to prevent us from interfering
	 * with the main chipset when we're not explicitly using the SPI bus. */
	for (i = 0; i < SPI_NUM_PORTS; i++)
		spi_enable(i, 0);
}
