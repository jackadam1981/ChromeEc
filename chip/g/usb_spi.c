...
static enum raiden_state state_;
...
static const struct {
	...
	uint16_t max_read;
	uint16_t features;
} config_rsp = { USB_SPI_PKT_ID_RSP_USB_SPI_CONFIG, MAX_USB_SPI_WRITE,
		 MAX_USB_SPI_READ, 1 };

/*
 * This is mostly a debug aid function which allows to determine the exact
 * error by examining flashrom error messages. The offset value is added to
 * the base 'unknown error' value.
 *
 * The return value is the number of bytes to send to the controller.
 */
static uint8_t report_error(void *packet, int offset)
{
	...
	rsp->value = USB_SPI_UNKNOWN_ERROR + offset;
				     ...
				     size_t count)
{
	...
	static uint16_t total_write_count;
	static uint16_t wrote_so_far;

	if (count < 2)
		return report_error(config->buffer, 0);
	...
	case RAIDEN_IDLE:
		...
		if ((packet_id != USB_SPI_PKT_ID_CMD_TRANSFER_START) ||
			...
			return report_error(config->buffer, 1);

		/* This is request to start a new transaction. */
		total_write_count = packet->s.write_count;
		total_read_count = packet->s.read_count;

		/* Some basic validity checks. */
		if ((total_read_count > config_rsp.max_read) ||
		    (total_write_count > config_rsp.max_write))
			return report_error(config->buffer, 2);
		...
		if ((total_write_count <=
		    ...
		    (total_write_count !=
		     (count - sizeof(struct raiden_cmd_start_hdr))))
			return report_error(config->buffer, 3);
		 ...
		 */
		if (total_read_count &&
		    (total_write_count > START_PAYLOAD_SIZE))
			return report_error(config->buffer, 4);

		read_so_far = 0;
		wrote_so_far = 0;

		if (total_write_count > START_PAYLOAD_SIZE)
			this_write_count = START_PAYLOAD_SIZE;
		else
			this_write_count = total_write_count;
		...
		while (this_write_count || (read_so_far != total_read_count)) {
			...
			wrote_so_far += this_write_count;
			read_so_far += this_read_count;

			if (total_write_count > wrote_so_far) {
				state_ = RAIDEN_WRITING;
				return 0;
			}
		...
		if (total_write_count &&
		    (total_write_count == this_write_count))
			return sizeof(*rsp);
		return 0;
	...
	case RAIDEN_WRITING:
		if ((packet_id != USB_SPI_PKT_ID_CMD_TRANSFER_CONTINUE) ||
			...
			return report_error(config->buffer, 5);

		if (packet->c.offset != wrote_so_far)
			return report_error(config->buffer, 6);

		this_write_count = count -
			sizeof(struct raiden_cmd_continue_hdr);
		 ...
		 * deasserted.
		 */
		last_sub_transaction = (this_write_count + wrote_so_far) ==
				       total_write_count;
		...
		res = usb_spi_map_error(spi_sub_transaction(
			...
			this_write_count,
			config->buffer, 0, last_sub_transaction));
		if (res) {
			rsp->value = res;
			rsp->packet_id = USB_SPI_PKT_ID_RSP_TRANSFER_START;
			state_ = RAIDEN_IDLE;
