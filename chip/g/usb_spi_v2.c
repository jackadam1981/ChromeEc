		     ...
		     size_t size)
{
	...
	do {
		...
		usb_spi_write_packet(config, sizeof(*rsp) + this_count);
...
/*
 * Most SPI transaction request received from the host, used when it is
				     ...
				     size_t count)
{
	...
	case RAIDEN_IDLE: {
		...
		/* Preserve the request in case host asks to restar. */
		if (packet_id == USB_SPI_PKT_ID_CMD_TRANSFER_START) {
			if (count <=
			    (sizeof(last_cmd) - sizeof(last_cmd.count))) {
		...
		/*
		 * Maximum SPI transaction size (read + write phases)
		 * supported by the H1 SPI driver in
		 * chip/g/spi_controller.c.
		 */
#define SPI_MAX_SIZE 128
