				     ...
				     size_t count)
{
	...
	case RAIDEN_IDLE: {
		...
		/*
		 * This is a transfer start request, preserve it to use in
		 * case host asks to restart.
		 */
		if (count <= (sizeof(last_cmd) - sizeof(last_cmd.count))) {
			...
			/*
			 * The request is too long, this means this is not a
			 * read request, so no need to preserve it.
			 */
			last_cmd.count = 0;
		}
		...
		if (state->total_write_count > START_PAYLOAD_SIZE) {
			/*
			 * Transactions with write count exceeding one USB
			 * packet AND requiring a read are not supported.
			 */
			if (total_read_count)
				return report_error(config->buffer,
		...
		/* At this point we are on one of the following states:
		 *
		 * read counter is zero, write counter fits into one USB packet
		 * read counter is zero, write counter does not fit into one
		 *     USB packet
		 * read counter is non-zero, write counter fits into one USB
		 *     packet.
		 *
		 * First things first - send to the chip the write component.
		 */
		if (this_write_count) {
			/* Need to send something to the SPI chip. */
			if ((this_write_count == state->total_write_count) &&
			    !total_read_count)
			...
			res = usb_spi_map_error(spi_sub_transaction(
				...
				this_write_count, NULL,
				0, last_sub_transaction));
			if (res) {
				rsp->value = res;
				rsp->packet_id =
					USB_SPI_PKT_ID_RSP_TRANSFER_START;
			...
			if (this_write_count < state->total_write_count) {
				/*
				 * This is a multipacket write only request,
				 * move to RAIDEN_WRITING state and wait for
				 * the follow up USB packets.
				 */
			...
			/*
			 * If it was a write only transaction fitting in one
			 * USB packet.
			 */
		...
		/*
		 * We are here because there is something to read from the
		 * chip. Allocate buffer to store the entire requested read
		 * size. This might be not necessary if the read size is less
		 * than the USB PDU response payload, but let's keep code
		 * simpler.
		 */
		...
		/*
		 * If there was a write portion, the read limit for the first
		 * SPI block needs to be reduced.
		 */
		read_limit = SPI_BUF_SIZE - this_write_count;

		/* Now complete the read part of the transaction. */
		while (read_so_far != total_read_count) {
			...
			res = usb_spi_map_error(spi_sub_transaction(
				SPI_FLASH_DEVICE,
				NULL, 0,
				read_buf + read_so_far, this_read_count,
			...
			if (last_sub_transaction) {
				ship_pdu(config, read_buf, total_read_count);
				shared_mem_release(read_buf);
				break;
			}

			/*
			 * Full buffer is the read limit after the first
			 * block.
			 */
			read_limit = SPI_BUF_SIZE;
		}
		return 0;
	}
