 ...
 * than 1 ms.
 */
#define READ_STATUS_CHECK_INTERVAL (150 * MSEC)
 ...
 * 3) START <i2c_addr|R> <0000_0000> STOP (i2cs_sda_low_count++)
 * 4) START <i2c_addr|W> <XX>* STOP ( i2cs_sda_low_count++)
...
static void poll_read_state(void)
{
	...
	if (gpio_get_level(GPIO_I2CS_SDA))
		/*
		 * The bus is either idle, the master has stopped clocking while
		 * SDA is high, or we have polled in the middle of a transaction
