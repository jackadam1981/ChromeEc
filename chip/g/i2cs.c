...
/* Poll SDA line every 3 seconds when trying to detect the "hosed" condition. */
#define READ_STATUS_CHECK_INTERVAL (3 * SECOND)

/*
 * Check for receive problems, if found - reinitialze the i2c slave
 * interface.
 */
static void poll_read_state(void)
{
	static uint16_t last_i2cs_read_irq_count;
	static uint8_t hosed_state_count;

	if ((last_i2cs_read_irq_count == i2cs_read_irq_count) &&
	    !gpio_get_level(GPIO_I2CS_SDA)) {
		/*
		 * SDA line is low and number of RX interrupts has not changed
		 * since last poll.  Is this the first or second poll interval
		 * that we are in this state?
		 */
		if (hosed_state_count++) {
			/*
			 * This is the second poll interval, we must be hosed.
			 * Reinitialize the i2c interface (it will also
			 * restart this polling function).
			 */
...
int i2cs_register_write_complete_handler(wr_complete_handler_f wc_handler)
{
	...
	/*
	 * Start a self perpetuating polling funciton to check for 'hosed'
