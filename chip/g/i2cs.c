...
static void poll_read_state(void)
{
	...
	if (!gpio_get_level(GPIO_I2CS_SDA)) {
		if (last_i2cs_read_irq_count == i2cs_read_irq_count) {
			/*
			 * SDA line is low and number of RX interrupts has not
			 * changed since last poll when it was low, it must be
			 * hosed. Reinitialize the i2c interface (which will
			 * also restart this polling function).
			 */
