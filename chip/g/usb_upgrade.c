...
static void upgrade_out_handler(struct consumer const *consumer, size_t count)
{
	...
	if (rx_state_ == rx_idle) {
		/*
		 * This must be an update initiating PDU.
		 *
		 * The size of the response returned in the same buffer will
		 * exceed the pdu header size; Let's make sure there is enough
		 * room for response in the buffer.
		 */
		union {
			struct update_pdu_header updu;
			struct {
		...
		if (!valid_transfer_start(consumer, count, &u.updu)) {
			/*
			 * Someting is wrong, this is not a valid update start
			 * PDU. Let'w indicate this by returning a single byte
			 * error code.
			 */
		...
		fw_upgrade_command_handler(&u.updu.cmd, count -
