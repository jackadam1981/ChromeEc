/* Copyright 2016 The Chromium OS Authors. All rights reserved.
 * Use of this source code is governed by a BSD-style license that can be
 * found in the LICENSE file.
 */

#include "blob.h"
#include "consumer.h"
#include "forward_uart.h"
#include "producer.h"
#include "queue.h"
#include "queue_policies.h"
#include "registers.h"
#include "task.h"
#include "uartn.h"
#include "usb-stream.h"

#define USE_UART_INTERRUPTS (!(defined(CONFIG_CUSTOMIZED_RO) && \
defined(SECTION_IS_RO)))
#define UART_BUF_SIZE 64

struct uart_config const ap_uart;
struct usb_stream_config const ap_usb;

static struct queue const ap_uart_to_usb = QUEUE_DIRECT(UART_BUF_SIZE, uint8_t,
							ap_uart.producer,
							ap_usb.consumer);
static struct queue const ap_usb_to_uart = QUEUE_DIRECT(UART_BUF_SIZE, uint8_t,
							ap_usb.producer,
							ap_uart.consumer);
struct usart_config const ap_uart = USART_CONFIG(UART_AP,
						 ap_uart_to_usb,
						 ap_usb_to_uart)

USB_STREAM_CONFIG(ap_usb,
		  USB_IFACE_AP,
		  USB_STR_AP_NAME,
		  USB_EP_AP,
		  USB_MAX_PACKET_SIZE,
		  USB_MAX_PACKET_SIZE,
		  ap_usb_to_uart,
		  ap_uart_to_usb)

struct uart_config const ec_uart;
struct usb_stream_config const ec_usb;

static struct queue const ec_uart_to_usb = QUEUE_DIRECT(64, uint8_t,
							ec_uart.producer,
							ec_usb.consumer);
static struct queue const ec_usb_to_uart = QUEUE_DIRECT(64, uint8_t,
							ec_usb.producer,
							ec_uart.consumer);
struct usart_config const ec_uart = USART_CONFIG(UART_EC,
						 ec_uart_to_usb,
						 ec_usb_to_uart)

USB_STREAM_CONFIG(ec_usb,
		  USB_IFACE_EC,
		  USB_STR_EC_NAME,
		  USB_EP_EC,
		  USB_MAX_PACKET_SIZE,
		  USB_MAX_PACKET_SIZE,
		  ec_usb_to_uart,
		  ec_uart_to_usb)

void get_data_from_usb(struct uart_config const *config)
{
	struct queue const *uart_out = config->consumer.queue;
	int c;

	/* Copy output from buffer until TX fifo full or output buffer empty */
	while (queue_count(uart_out) && QUEUE_REMOVE_UNITS(uart_out, &c, 1))
		uartn_write_char(config->uart, c);

	/* If output buffer is empty, disable transmit interrupt */
	if (!queue_count(uart_out))
		uartn_tx_stop(config->uart);
}

void send_data_to_usb(struct uart_config const *config)
{
	struct queue const *uart_in = config->producer.queue;

	/* Copy input from buffer until RX fifo empty or the queue is full */
	while (uartn_rx_available(config->uart) && queue_space(uart_in)) {
		int c = uartn_read_char(config->uart);

		QUEUE_ADD_UNITS(uart_in, &c, 1);
	}
}

static void uart_read(struct producer const *producer, size_t count)
{
}

static void uart_written(struct consumer const *consumer, size_t count)
{
	struct uart_config const *config =
		DOWNCAST(consumer, struct uart_config, consumer);

	if (uartn_tx_ready(config->uart), queue_count(consumer->queue))
		uartn_tx_start(config->uart);
}

static void uart_flush(struct consumer const *consumer)
{
	struct uart_config const *config =
		DOWNCAST(consumer, struct uart_config, consumer);

	uartn_tx_flush(config->uart);
}

struct producer_ops const uart_producer_ops = {
	.read = uart_read,
};

struct consumer_ops const uart_consumer_ops = {
	.written = uart_written,
	.flush   = uart_flush,
};

#if USE_UART_INTERRUPTS
/**
*  * Interrupt handlers for UART1
*   */
void ap_usart_tx_interrupt(void)
{
	/* Clear transmit interrupt status */
	GR_UART_ISTATECLR(UART_AP) = GC_UART_ISTATECLR_TX_MASK;
	/* Fill output FIFO */
	get_data_from_usb(ap_uart);
}
DECLARE_IRQ(GC_IRQNUM_UART1_TXINT, ap_usart_tx_interrupt, 1);

void ap_usart_rx_interrupt(void)
{
	/* Clear receive interrupt status */
	GR_UART_ISTATECLR(UART_AP) = GC_UART_ISTATECLR_RX_MASK;
	/* Read input FIFO until empty */
	send_data_to_usb(ap_uart);
}
DECLARE_IRQ(GC_IRQNUM_UART1_RXINT, ap_usart_rx_interrupt, 1);

/**
*  * Interrupt handlers for UART2
*   */
void ec_usart_tx_interrupt(void)
{
	/* Clear transmit interrupt status */
	GR_UART_ISTATECLR(UART_EC) = GC_UART_ISTATECLR_TX_MASK;

	/* Fill output FIFO */
	get_data_from_usb(ec_uart);
}
DECLARE_IRQ(GC_IRQNUM_UART2_TXINT, ec_usart_tx_interrupt, 1);

void ec_usart_rx_interrupt(void)
{
	/* Clear receive interrupt status */
	GR_UART_ISTATECLR(UART_EC) = GC_UART_ISTATECLR_RX_MASK;

	/* Read input FIFO until empty */
	send_data_to_usb(ec_uart);
}
DECLARE_IRQ(GC_IRQNUM_UART2_RXINT, ec_usart_rx_interrupt, 1);
#endif
