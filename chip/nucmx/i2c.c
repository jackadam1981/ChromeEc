...
typedef enum
{
	...
	SMB_BUS_ERRROR,              	  /* Encounter bus error   				   			*/
...
/* Internal SMBus Interface driver states values, which reflect events which occured on the bus */
typedef enum
{
    SMB_IDLE,						/* default driver state							*/
    SMB_MASTER_START,
    SMB_WRITE_OPER,
    SMB_READ_OPER,
    SMB_REPEAT_START,
...
struct i2c_status {
	...
	const uint8_t*  	txBuf; 			/* Entry pointer of transmit buffer */
	uint8_t*  			rxBuf; 			/* Entry pointer of receive buffer  */
	uint16_t  			szTxBuf;        /* Size of Tx buffer in bytes    	*/
	uint16_t  			szRxBuf;      	/* Size of rx buffer in bytes    	*/
	uint16_t 			idxBuf;			/* Current index of Tx/Rx buffer 	*/
	uint8_t     		slave_addr;		/* target slave address 		 	*/
	SMB_OPER_STATE_T 	oper_state;		/* smbus operation state 			*/
	SMB_ERROR			err_code;		/* Error code, if any 				*/
	int task_waiting;					/* Task waiting on port, or TASK_ID_INVALID if none. */
};
...
SMB_ERROR i2c_master_transaction (int port)
{
	/* Set i2c mode to object */
	int events = 0;
	volatile struct i2c_status *pStsObj = i2c_stsobjs + port;

	if(pStsObj->oper_state == SMB_IDLE)
		pStsObj->oper_state = SMB_MASTER_START;
	else if(pStsObj->oper_state == SMB_WAIT_REPEAT_START){
		pStsObj->oper_state = SMB_REPEAT_START;
		ccputs("R");
	}
	...
	if ((events & TASK_EVENT_I2C_IDLE) == 0){
		/* Recovery I2C port */
		i2c_recovery(port);
		pStsObj->err_code = SMB_TIMEOUT_ERROR;
	}

	return pStsObj->err_code;
}
...
void i2c_master_int_handler (int port)
{
	...
	if (IS_BIT_SET(NUCMX_SMBST(port), NUCMX_SMBST_BER))
	{
		...
		task_set_event(pStsObj->task_waiting, TASK_EVENT_I2C_IDLE, 0);
		ccputs("-BER");
	}
	...
	if (IS_BIT_SET(NUCMX_SMBST(port), NUCMX_SMBST_NEGACK))
	{
		...
		/* Set error code */
		pStsObj->err_code = SMB_MASTER_NO_ADDRESS_MATCH;
		/* Notify upper layer */
		pStsObj->oper_state = SMB_IDLE;
		task_set_event(pStsObj->task_waiting, TASK_EVENT_I2C_IDLE, 0);
		ccputs("-NA");
	...
	if (IS_BIT_SET(NUCMX_SMBST(port), NUCMX_SMBST_SDAST))
	{
		...
		{
			uint8_t addr = pStsObj->slave_addr;
			/* Prepare address byte */
			if (pStsObj->szTxBuf == 0) /* Receive mode */
			{
				pStsObj->oper_state = SMB_READ_OPER;

				/* Receiving one byte only - set nack just before writing address byte */
				if (pStsObj->szRxBuf == 1){
					I2C_NACK(port);
				}
			...
			{
				pStsObj->oper_state = SMB_WRITE_OPER;
		...
		/* 3.2 Handle master write operation  */
		else if (pStsObj->oper_state == SMB_WRITE_OPER)
		{
			/* all bytes have been written, in a pure write operation */
			if (pStsObj->idxBuf == pStsObj->szTxBuf){
				/*  no more message */
				if(pStsObj->szRxBuf == 0){
					/* need to STOP or not */
					if(pStsObj->flags & I2C_XFER_STOP){
					...
					/* Set error code */
					pStsObj->err_code = SMB_OK;
					/* Notify upper layer */
					pStsObj->oper_state = (pStsObj->flags & I2C_XFER_STOP) ? SMB_IDLE :SMB_WAIT_REPEAT_START;
					task_set_event(pStsObj->task_waiting, TASK_EVENT_I2C_IDLE, 0);
					ccputs("-END");
				}
				/* need to restart & send slave address immediately */
				else {
					uint8_t addr_byte = pStsObj->slave_addr;
					/* Prepare address byte & start to receive bytes */
					pStsObj->oper_state = SMB_READ_OPER;
			...
			{
				I2C_WRITE_BYTE(port, pStsObj->txBuf[pStsObj->idxBuf++]);
				ccprintf("-W(%02x)",pStsObj->txBuf[pStsObj->idxBuf-1]);
			}
		}
		/* 3.3 Handle master read operation (pure read or following a write operation) */
		else if (pStsObj->oper_state == SMB_READ_OPER)
		{
			uint8_t data;
			/* last byte is about to be read - end of transaction */
			if (pStsObj->idxBuf == (pStsObj->szRxBuf - 1))
			{
				/* need to STOP or not */
				if(pStsObj->flags & I2C_XFER_STOP){
			...
			else if (pStsObj->idxBuf == (pStsObj->szRxBuf - 2))
			{
			...
			ccprintf("-R(%02x)",data);
			/* Read to buffer */
			pStsObj->rxBuf[pStsObj->idxBuf++] = data;

			/* last byte is read - end of transaction */
			if (pStsObj->idxBuf == pStsObj->szRxBuf)
			{
				/* Set error code */
				pStsObj->err_code = SMB_OK;
	     ...
	     uint8_t *in, int in_size, int flags)
{
	...
	pStsObj->flags      = flags;
	pStsObj->txBuf 		= out;
	pStsObj->szTxBuf 	= out_size;
	pStsObj->rxBuf 		= in;
	pStsObj->szRxBuf 	= in_size;
#if I2C_7BITS_ADDR
	/* Set slave address from 7-bits to 8-bits */
	pStsObj->slave_addr = (slave_addr<<1);
#else
	/* Set slave address (8-bits) */
	pStsObj->slave_addr = slave_addr;
#endif
	/* Reset index & error */
	pStsObj->idxBuf 	= 0;
	pStsObj->err_code 	= SMB_OK;
	...
	if ((flags & I2C_XFER_START) &&
			(i2c_bus_busy(port) || (i2c_get_line_levels(port) != I2C_LINE_IDLE))) {
	...
	pStsObj->task_waiting = task_get_current();
	...
	return (pStsObj->err_code == SMB_OK) ? EC_SUCCESS : EC_ERROR_UNKNOWN;
}
