...
static int PECI_Pending_TaskID;
...
/*                                                                                                         */
/* Parameters:                                                                                             */
/*                  wr_length - How many byte of *wr_data went to be send                                  */
/*                  rd_length - How many byte went to received (not include FCS)                           */
/*                  cmd_code  - Command code.                                                              */
/*                  *wr_data  - Buffer pointer of write data.			                                   */
/*                                                                                                         */
/* Returns:         if return  TASK_EVENT_PECI_DONE that mean slave had a response		                   */
/* Side effects:                                                                                           */
/* Description:                                                                                            */
...
int peci_get_cpu_temp(void)
{
	...
	if((events&TASK_EVENT_PECI_DONE)==TASK_EVENT_PECI_DONE){
		...
		if(events){
			//ccprintf("PECI Transaction ERROR:%02x\r\n",events);
			;
		}else{
			uint16_t *ptr;
