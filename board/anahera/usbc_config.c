...
static int usba0_retimer_init(const struct usb_mux *me)
{
	int rv;
	int tries = 2;
	...
	if (rv) {
		CPRINTS("A0: PS8811 retimer not detected!");
		return rv;
	}
...
static int usba1_retimer_init(const struct usb_mux *me)
{
	int rv;
	int tries = 2;
...
{
	usba0_retimer_init(&usba_ps8811[USBA_PORT_A0]);
	usba1_retimer_init(&usba_ps8811[USBA_PORT_A1]);
}
