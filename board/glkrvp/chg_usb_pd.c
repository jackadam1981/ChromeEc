...
#define GLKRVP_SNK_ENABLE	1
#define GLKRVP_SNK_DISABLE	0
#define GLKRVP_VBUS_ENABLE	1
#define GLKRVP_VBUS_DISABLE	0
...
static int board_charger_port_is_sourcing_vbus(int port)
{
	...
	return !!(reg & PTN5110_EXT_GPIO_EN_SRC) == GLKRVP_VBUS_ENABLE ? 1 : 0;
}
...
{
	ptn5110_ext_gpio_enable(port, enable ? GLKRVP_SNK_ENABLE :
			GLKRVP_SNK_DISABLE, PTN5110_EXT_GPIO_EN_SNK1);
}

void board_vbus_enable(int port, int enable)
{
	ptn5110_ext_gpio_enable(port, enable ? GLKRVP_VBUS_ENABLE :
			GLKRVP_VBUS_DISABLE, PTN5110_EXT_GPIO_EN_SRC);
}
...
void board_tcpci_tcpm_post_init(int port)
{
	int reg;
	...
	reg |= PTN5110_EXT_GPIO_FRS_EN | PTN5110_EXT_GPIO_EN_SRC |
		PTN5110_EXT_GPIO_EN_SNK1 | PTN5110_EXT_GPIO_IILIM_5V_VBUS;
