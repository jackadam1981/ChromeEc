...
*/
static void set_keyboard_flag(int status, int flag)
{
	if (status) {
...
{
	set_keyboard_flag(lid_is_open(), GPIO_ODR_HIGH);
}
...
*/
static void kbd_lid_change(void)
{
	set_keyboard_flag(!lid_is_open(), GPIO_ODR_HIGH | GPIO_PULL_UP);
}
DECLARE_HOOK(HOOK_LID_CHANGE, kbd_lid_change, HOOK_PRIO_LAST);
