...
int board_set_debug(int enable)
{
	...
	if (enable) {
		...
		/* Put power on USB_C_5V_EN pin to allow switching debug mux*/
		gpio_set_level(GPIO_USB_C_5V_EN, 1);

		/* Enable manual switching and switch debug mux */
		rv = tsu6721_mux(TSU6721_MUX_DEBUG);
		if (rv)
			return rv;
		...
		/* Set all four pins to input mode */
		STM32_GPIO_MODER(GPIO_B) &= ~0xff000000;

		/* Set all four pins to low speed */
		STM32_GPIO_OSPEEDR(GPIO_B) &= ~0xff000000;

		/* Enable automatic switching and turn off debug mux */
