...
/* NX20P348X Software workaround switch vchg*/
#define NX20P348X_SWITCH_CONTROL_HIDDEN_REG 0x80
/* NX20P348X Software workaround control hidden register enable */
...
int board_set_ppc_vchg(int port, bool enable)
{
	...
	if (enable) {
		...
		/*
		 * NX20P348X Software workaround solution.
		...
		case OTG_MODE:
			...
			if (rv) {
				CPRINTSUSB(
					"set NX20P348X control hidden reg 0xEA fail");
				return rv;
			}
			rv = write_reg(USBC_PORT_C1,
				       NX20P348X_SWITCH_CONTROL_HIDDEN_REG,
			...
			if (rv) {
				CPRINTSUSB(
					"set NX20P348X control hidden reg 0xAF fail");
				return rv;
			}
			rv = write_reg(USBC_PORT_C1,
			...
			if (rv) {
				CPRINTSUSB(
					"set NX20P348X VCHG hidden reg 0x98 fail");
				return rv;
			}
