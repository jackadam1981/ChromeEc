...
static int32_t send_cmd(uint16_t cmd, uint8_t *tx, uint32_t tx_lng,
			uint8_t *rx, uint32_t rx_lng);
			...
			uint32_t rx_lng)
{
	...
	ret = i2c_xfer_unlocked(I2C_PORT_TP, HIDDESC_ADR_SLAVE, data, lng,
				rx, rx_lng, I2C_XFER_SINGLE);
...
int32_t i2c_hid_get_hid_desc(void)
{
	...
	i2c_lock(I2C_PORT_TP, 1);
	ret = i2c_xfer_unlocked(I2C_PORT_TP, HIDDESC_ADR_SLAVE, data, 2,
				...
				, uint32_t max_lng)
{
	...
	i2c_lock(I2C_PORT_TP, 1);
	ret = i2c_xfer_unlocked(I2C_PORT_TP, HIDDESC_ADR_SLAVE, tmp, lng,
...
int32_t i2c_hid_set_mouse(void)
{
	...
	for (i = 0; i < DATA_INPUTMOD_LNG; i++)
		data[3 + i] = (uint8_t)((DATA_INPUTMOD_MOUSE >> (i * 8))
					& 0xFF);
	lng += DATA_INPUTMOD_LNG;
	data[0] = (uint8_t)(lng & 0x00FF);
	data[1] = (uint8_t)((lng >> 8) & 0x00FF);
