...
uint64_t stress_switch_count = 0;
...
void dump_tp_int(void)
{
	...
	CPRINTF("TP int:%u ack:%u duration:%u us, time: %11.6ld s %016lx, "
		"HPET %016lx, stress %ld\n",
		cnt_int_now - cnt_int_last,
		cnt_ack_now - cnt_ack_last,
		(uint32_t)(ts.val - ts_last.val),
		ts.val, ts.val, HPET_MAIN_COUNTER_64,
		stress_switch_count);
...
void stress_task_1(void)
{
	while (1) {
		task_wait_event(-1);
		msleep(1);
		stress_switch_count++;
		task_wake(TASK_ID_STRESS2);
	}
}

void stress_task_2(void)
{
	task_wake(TASK_ID_STRESS1);
