...
/* Specail battery parameter */
#define GREEN_BOOK_SUPPORT	(1 << 2)

static const struct battery_info info_old = {
...
static const struct battery_info info_new = {
	/* New battery, use BOARD_ID to seperate it. */
	.voltage_max    = 12600,	/* mV */
	.voltage_normal = 11100,
	.voltage_min    = 9000,
...
{
	if (0x02 == system_get_board_version())
		return &info_new;
	else
		return &info_old;
}
...
static void wakeup(void)
{
	 ...
	 */
	/* Only new battery needs call wakeup_deferred  */
	if (0x02 == system_get_board_version())
