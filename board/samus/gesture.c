...
/* Output datarate for tap sensor */
 ...
 *
 * Define an outer and inner window. The inner window essentiall specifies how
...
static int gesture_tap_for_battery(void)
{
	...
	/* Flag for whether Z variation has dropped below req'd threshold */
	static int z_dropped;
	static int z_drop_thresh, z_rise_thresh;

	int history_idx_inner;
	int ret = 0;
	...
	case 1:
		...
		if (state_cnt >= INNER_WINDOW) {
			...
			if (tap_debug)
				CPRINTS("Max dZ %d, at %d cnts "
					"Z-drop %d, Z-rise %d",
					delta_z_inner_max, cnts_since_max,
					z_drop_thresh, z_rise_thresh);
		}
		break;
	case 2:
		/* Interstice period */
		/* Check for z motion to go back down first */
		if (!z_dropped && delta_z_inner < z_drop_thresh) {
			z_dropped = 1;
			if (tap_debug)
				CPRINTS("Z dropped low %d at %d cnts",
					delta_z_inner, state_cnt);
		}

		/* Then, check for z motion to go back up */
		if (z_dropped && delta_z_inner > z_rise_thresh) {
			if (state_cnt < MIN_INTERSTICE) {
				if (tap_debug)
					CPRINTS("Tap2 too early %d cnts",
						state_cnt);
				state = 0;
			} else {
				if (tap_debug)
					CPRINTS("Z rise at %d cnts",
						delta_z_inner, state_cnt);
	...
	case 4:
		...
		if (delta_z_inner_max2 > 2 * delta_z_outer &&
			...
			ret = 1;
			if (tap_debug)
				CPRINTS("Tap Z_in:Z_out %d, Z_out:XY_out %d",
					delta_z_inner_max2 / delta_z_outer,
					delta_z_outer / delta_xy_outer);
		} else {
			if (tap_debug)
				CPRINTS("Too much after motion"
					"Z_in:Z_out %d, Z_out:XY_out %d",
					delta_z_inner_max2 / delta_z_outer,
					delta_z_outer / delta_xy_outer);
		}
