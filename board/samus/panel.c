...
static void lp8555_enable_pwm_mode(void)
{
	...
	/*
	 * NOTE: we expect that this might fail i2c transactions in certain
	 * cases because the lp8555 is not powered. Power to the chip is
	 * a logical AND of an enable signal from the PCH and from the EC
	 * (GPIO_ENABLE_BACKLIGHT). So, if one of them isn't on, i2c will
	 * fail. That is ok, because this is called by both the EC and the
	 * PCH immediately after each asserts their enable.
	 */
