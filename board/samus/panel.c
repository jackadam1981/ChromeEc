...
#define I2C_ADDR_BACKLIGHT__7b (0x2C | I2C_FLAG_BIG_ENDIAN)
...
static int lp8555_read_with_retry(int reg, int *data)
{
	...
	for (i = 0; i < I2C_RETRIES; i++) {
		rv = i2c_read8__7b(I2C_PORT_BACKLIGHT, I2C_ADDR_BACKLIGHT__7b,
			       reg, data);
		if (rv == EC_SUCCESS)
			return EC_SUCCESS;
...
static int lp8555_write_with_retry(int reg, int data)
{
	...
	for (i = 0; i < I2C_RETRIES; i++) {
		rv = i2c_write8__7b(I2C_PORT_BACKLIGHT, I2C_ADDR_BACKLIGHT__7b,
			       reg, data);
		if (rv == EC_SUCCESS)
			return EC_SUCCESS;
		usleep(I2C_RETRY_DELAY);
