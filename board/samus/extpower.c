...
static void extpower_board_hacks(int extpower, int extpower_prev)
{
	...
	hook_call_deferred(&__deferred_allow_max_request, -1);
	...
	if (extpower && !extpower_prev) {
		/* AC connected */
		charger_disable(0);
		hook_call_deferred(&__deferred_allow_max_request, 500*MSEC);
		...
		hook_call_deferred(&__deferred_allow_min_charging, 100*MSEC);
