...
int sb_temp(int idx, int *temp_ptr);
int ds1624_get_val(int idx, int *temp_ptr);
...
#else
const struct i2c_port_t i2c_ports[]  = {
	{"sensors",  MEC17XX_I2C_PORT4, 100,  GPIO_SMB04_SCL, GPIO_SMB04_SDA},
	{"batt",     MEC17XX_I2C_PORT5, 100,  GPIO_SMB05_SCL, GPIO_SMB05_SDA},
};
const unsigned int i2c_ports_used = ARRAY_SIZE(i2c_ports);
#endif
...
const uint16_t i2c_port_to_ctrl[I2C_PORT_COUNT] = {
	(MEC17XX_I2C_CTRL0 << 8) + MEC17XX_I2C_PORT4,
	(MEC17XX_I2C_CTRL1 << 8) + MEC17XX_I2C_PORT5
};
...
/*
 * If I2C controllers are used as masters only
 * we must disable each one to insure they no
 * longer require clocks. If controllers used
 * as I2C slaves then only disable if external
 * master will not wake the EC from deep sleep.
 * Power down GPIO pads that are not marked as deep sleep.
 */
void board_prepare_for_deep_sleep(void)
{
	...
	for (i = 0; i < I2C_PORT_COUNT; i++) {
		ctrl = i2c_port_to_ctrl[i] >> 8;
		MEC17XX_I2C_CONFIG(ctrl) &= ~(1 << 10);
	}
...
/*
 * Power on GPIO pads that are not marked as deep sleep.
 * If I2C controllers are used as masters only
 * we must re-enable each one on exit from deep sleep.
 */
