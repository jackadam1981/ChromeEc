...
static uint8_t fast_scan_buf[COL_COUNT * 2];
static uint8_t scan_needed[COL_COUNT * 2];
...
int fast_scan(uint8_t *data)
{
	...
	for (col = 0; col < COL_SPAN; ++col)
		v += fast_scan_buf[col];
	for (col = 0; col < COL_COUNT * 2; ++col) {
		if (col + COL_SPAN < COL_COUNT * 2)
			v += fast_scan_buf[col + COL_SPAN];
		if (col - COL_SPAN - 1 >= 0)
			v -= fast_scan_buf[col - COL_SPAN - 1];
		data[col] = !!v;
	}
