...
#define CC0_DAC_ADDR 0x48
#define CC1_DAC_ADDR 0x49
...
void enable_dac(enum dac_t dac, uint8_t en)
{
	if (dac == CC0_DAC) {
		if (en) {
			...
			en_vout_buf_cc1(1);
			/* Power ON DAC */
			i2c_write8(1, CC0_DAC_ADDR, REG_CONFIG, 1);
			dac_enabled |= CC0_DAC;
		} else {
			en_vout_buf_cc1(0);
			/* Power OFF DAC */
			i2c_write8(1, CC0_DAC_ADDR, REG_CONFIG, 0);
			dac_enabled &= ~CC0_DAC;
		}
	} else {
		if (en) {
...
int write_dac(enum dac_t dac, uint16_t value)
{
	...
	if (dac == CC0_DAC) {
		if (!(dac_enabled & CC0_DAC)) {
			ccprintf("CC0_DAC is disabled\n");
			return EC_ERROR_ACCESS_DENIED;
		}
		i2c_write16(1, CC0_DAC_ADDR, REG_DAC, tmp);
	} else {
