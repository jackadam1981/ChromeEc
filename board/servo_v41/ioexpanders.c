...
void init_ioexpanders(void)
{
	...
	i2c_write8(1, TCA0_ADDR, TCA0_OUT_PORT_A, 0x0);
	 ...
	 */
	i2c_write8(1, TCA0_ADDR, TCA0_DIR_PORT_A, 0x0);
	i2c_write8(1, TCA0_ADDR, TCA0_DIR_PORT_B, 0xb8);

	/* Init TCA6424A */

	/* Write all GPIO to output 0 */
	i2c_write8(1, TCA1_ADDR, TCA1_OUT_PORT_A, 0x0);
	i2c_write8(1, TCA1_ADDR, TCA1_OUT_PORT_B, 0x0);
	i2c_write8(1, TCA1_ADDR, TCA1_OUT_PORT_C, 0x0);
	...
	i2c_write8(1, TCA1_ADDR, TCA1_DIR_PORT_C, 0x8c);
}
...
inline void usb3_a0_power_en(int en)
{
...
inline void uservo_fastboot_mux_sel(int en)
{
	write_ioexpander(TCA0_ADDR, TCA0_OUT_PORT_B, 0, en);
}

inline void usb3_a1_power_en(int en)
{
...
inline int board_id_det(void)
{
	...
	id = read_ioexpander_bit(TCA0_ADDR, TCA0_OUT_PORT_B, 5) << 2;
	id |= read_ioexpander_bit(TCA0_ADDR, TCA0_OUT_PORT_B, 4) << 1;
	id |= read_ioexpander_bit(TCA0_ADDR, TCA0_OUT_PORT_B, 3);
...
{
	return read_ioexpander_bit(TCA0_ADDR, TCA0_OUT_PORT_B, 7);
}
...
inline int read_faults(void)
{
	...
	fault = read_ioexpander_bit(TCA1_ADDR, TCA1_OUT_PORT_B, 7) << 7;
	fault |= read_ioexpander_bit(TCA1_ADDR, TCA1_OUT_PORT_B, 6) << 6;
	fault |= read_ioexpander_bit(TCA1_ADDR, TCA1_OUT_PORT_B, 5) << 5;
	fault |= read_ioexpander_bit(TCA1_ADDR, TCA1_OUT_PORT_B, 4) << 4;
	fault |= read_ioexpander_bit(TCA1_ADDR, TCA1_OUT_PORT_B, 3) << 3;
	fault |= read_ioexpander_bit(TCA1_ADDR, TCA1_OUT_PORT_B, 2) << 2;
	fault |= read_ioexpander_bit(TCA1_ADDR, TCA1_OUT_PORT_B, 1) << 1;
	fault |= read_ioexpander_bit(TCA1_ADDR, TCA1_OUT_PORT_B, 0) << 0;
