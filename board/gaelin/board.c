...
#define NAVKEY_SCREEN_LOCK_MASK BIT(4)
...
void osd_int_deferred(void)
{
	static uint8_t old_navkey_state;
	uint8_t navkey_state_change;
	int navkey;

	if (gpio_get_level(GPIO_OSD_INT) == 0) {
		navkey = 0;
		i2c_write8(I2C_PORT_SCALER, I2C_ADDR_SCALER_FLAGS, 0xFF, 0x03);
		i2c_read8(I2C_PORT_SCALER, I2C_ADDR_SCALER_FLAGS, 0x81,
			  &navkey);
		if (navkey == 1) {
		...
		// } else if (navkey == 16) {
		// 	/* screen-lock is pressed */
		// 	new_navkey_state = NAVKEY_SCREEN_LOCK_MASK;
		} else {
			new_navkey_state = 0;
		}
	} else {
	...
	if (new_navkey_state != old_navkey_state) {
		...
		// if (navkey_state_change & NAVKEY_SCREEN_LOCK_MASK)
		// 	button_interrupt(GPIO_SCREEN_LOCK_L);

		old_navkey_state = new_navkey_state;
	}
}
DECLARE_DEFERRED(osd_int_deferred);

void osd_int_interrupt(enum gpio_signal signal)
{
	/* Reset lid debounce time */
	hook_call_deferred(&osd_int_deferred_data, 20 * MSEC);
}
...
int pseudo_to_physical_value(enum gpio_signal gpio)
{
	...
	else if (gpio == GPIO_SCREEN_LOCK_L)
		return !!(new_navkey_state & NAVKEY_SCREEN_LOCK_MASK);

	CPRINTS("Not a volume/brightness up or down key");
	return 0;
}

static void disp_mode_deferred(void);
DECLARE_DEFERRED(disp_mode_deferred);

static void disp_mode_deferred(void)
{
	static bool lock_screen_flag = false;
	static uint8_t old_state;

	if (new_navkey_state != NAVKEY_SCREEN_LOCK_MASK) {
		new_navkey_state = NAVKEY_SCREEN_LOCK_MASK;
	...
	if (new_navkey_state != old_state) {
		button_interrupt(GPIO_SCREEN_LOCK_L);
		old_state = new_navkey_state;

		if (lock_screen_flag == true)
			hook_call_deferred(&disp_mode_deferred_data, 120 * MSEC);
	}
}
void disp_mode_interrupt(enum gpio_signal signal)
{
	hook_call_deferred(&disp_mode_deferred_data, 20 * MSEC);
