...
void scaler_send_scan_code(uint8_t action_keys) //for Navi key  //raymondchung: ???done
{
	const struct ec_response_keybd_config *scaler_keybd = board_vivaldi_keybd_config();
	uint8_t scaler_row = 0, scaler_col = 0;

	for(int i = 0; i < scaler_keybd->num_top_row_keys; i++)
		if(action_keys == scaler_keybd->action_keys[i])
		{
			scaler_row = vivaldi_keys[i].row;
			scaler_col = vivaldi_keys[i].col;
		}

	keyboard_state_changed(scaler_row, scaler_col, 1);
	keyboard_state_changed(scaler_row, scaler_col, 0);
}
...
void scaler_test(void) //for scaler test //raymondchung: ???
{
	...
	scaler_send_scan_code(scaler_action_keys);
#endif
