...
{
	enum mt6360_ldo_id ldo_id = index;
	/* TODO(pihsun): Fix this */
	int voltage = selector == 0 ? 1800000 : 3300000;

	return mt6360_ldo_set_voltage(ldo_id, voltage, voltage);
}
...
int board_regulator_get_voltage(uint32_t index, uint32_t *selector)
{
	...
	rv = mt6360_ldo_get_voltage(ldo_id, &voltage);
	if (rv)
		return rv;

	/* TODO(pihsun): Fix this */
	*selector = ldo_id == MT6360_LDO5 ? 0 : (voltage == 1800000 ? 0 : 1);
