...
{
	/*
	 * GPIO_EN_SLP_Z not implemented in rev0,
...
{
	enum mt6360_regulator_id rgl_id = index;

	return mt6360_regulator_get_info(rgl_id, name, num_voltages,
					 voltages_mv);
}
...
int board_regulator_enable(uint32_t index, uint8_t enable)
{
	...
	return mt6360_regulator_enable(rgl_id, enable);
}
...
int board_regulator_is_enabled(uint32_t index, uint8_t *enabled)
{
	...
	return mt6360_regulator_is_enabled(rgl_id, enabled);
}
				...
				uint32_t max_mv)
{
	...
	return mt6360_regulator_set_voltage(rgl_id, min_mv, max_mv);
}
...
int board_regulator_get_voltage(uint32_t index, uint32_t *voltage_mv)
{
	...
	return mt6360_regulator_get_voltage(rgl_id, voltage_mv);
}
