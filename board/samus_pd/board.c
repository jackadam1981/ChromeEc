...
static int ec_status_host_cmd(struct host_cmd_handler_args *args)
{
	...
	if (p->charge_state != charge_state &&
	    p->charge_state != PD_CHARGE_NO_CHANGE) {
		switch (p->charge_state) {
		case PD_CHARGE_NONE:
			/*
			 * No current allowed in, set new power request so that
			 * PD negotiates down to vSafe5V.
			 */
		...
		case PD_CHARGE_MAX:
			/*
			 * Allow negotiation above vSafe5V. Should only ever
			 * get this command when 5V charging is allowed.
			 */
