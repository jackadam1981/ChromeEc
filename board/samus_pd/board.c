...
static int set_usb_debug(int enable)
{
	...
	if (!gpio_get_level(GPIO_SPI_FLASH_WP_L))
		return EC_ERROR_ACCESS_DENIED;

	if (enable) {
		/* Switch PI3USB9281 chip 0 to manual mode */
...
/*
 * Used to enable SPI flash access on SPI2.
 * Note: This will disable USB PD on C0. On pre-EVT boards, it will also
 * interfere with C1 PD operation due to hardware bugs.
 */
static int set_spi_debug(int enable)
{
	...
	if (enable) {
		...
		/* Set NSS to general purpose output mode. */
		gpio_set_flags(GPIO_SPI_FLASH_CS_L,
			GPIO_OUT_HIGH | GPIO_PULL_UP);
		...
		/* Set PD_SPI_CS to input */
		gpio_set_flags(GPIO_SPI_FLASH_CS_L, GPIO_INPUT);

		/* Set SCK, MOSI, and MISO to input mode */
 ...
 * access on SPI2.
 *
 * Note: this will disconnect USB lines from AP. On pre-EVT boards, both
 * features will not operate correctly and interfere with C1 PD operation
