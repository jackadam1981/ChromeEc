...
static void board_init(void)
{
	...
	charge.current = charge.voltage = 0;
	for (i = 0; i < PD_PORT_COUNT; ++i) {
		/* PD task initially assumes 0 charge limit to start */
		charge_manager_update(CHARGE_SUPPLIER_PD, i, &charge);
		/* TODO(shawnn): implement type-C charging */
		charge_manager_update(CHARGE_SUPPLIER_TYPEC, i, &charge);
	}
...
void board_set_active_charge_port(int charge_port)
{
	...
	CPRINTS("Set active charge port %d\n", charge_port);
}
