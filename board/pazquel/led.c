...
extern int cprintf_ex(enum console_channel channel, const char *format, ...);
...
static void board_led_set_battery(void)
{
	...
	case PWR_STATE_DISCHARGE:
		...
		/*  in S3 S0 and  battery level <20*/
		/* Red 1 sec, off 1 sec*/
			if (charge_get_display_charge() < 200) {
	...
	case PWR_STATE_ERROR:
		/* Battery error: Blue 1 sec, off 1 sec */
              period = (1 + 1) * LED_ONE_SEC;
              battery_ticks = battery_ticks % period;
              if (battery_ticks < 1 * LED_ONE_SEC)
                color = LED_BLUE;
