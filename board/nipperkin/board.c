...
void board_set_current_limit(void)
{
	...
	/*
	 * When there is no battery or battery is malfunctioning, override
	 * charger current limit to prevent brownout during boot.
	 */
	if (battery_is_present() == BP_NO ||
	    battery_state_of_charge_abs(&state_of_charge) != EC_SUCCESS) {
