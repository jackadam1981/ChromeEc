...
static void board_init(void)
{
	...
	/*
	 * Set Charger ISL9241 Control2 bit<10:9> = 00,
	 * PROCHOT# Debounce = 7us
	 */
	if (i2c_read16(I2C_PORT_CHARGER, ISL9241_ADDR_FLAGS,
		...
		if (i2c_write16(I2C_PORT_CHARGER, ISL9241_ADDR_FLAGS,
			...
			ccprintf("Failed to set charger setting");
	}
...
void board_set_charger_limit(void)
{
	...
	if (battery_is_present() == BP_NO) {
		...
		isl9241_set_input_current_limit(CHARGER_SOLO, 0x1770);
		
	} else {
		/* Battery present and < 4%, or battery error */
		if (((battery_state_of_charge_abs(&soc) == EC_SUCCESS && soc < 4)
			...
			isl9241_set_ac_prochot(CHARGER_SOLO, ac_limit_backup);
				...
				/* Battery present >= 4%, AC >= 45W and <= 65W */
				if (((charge_manager_get_power_limit_uw() / 1000 >= 45000)
				&& (charge_manager_get_power_limit_uw() / 1000 <= 65000))
				&& (ac_limit_backup != 3328)) {
					ccprintf("limit_3\n");
					ac_limit_backup = 3328;
					isl9241_set_ac_prochot(CHARGER_SOLO, ac_limit_backup);
				/* Battery present >= 4%, AC = 90W or 100W */
				} else if (((charge_manager_get_power_limit_uw() / 1000 == 90000)
				|| (charge_manager_get_power_limit_uw() / 1000 == 100000))
...
DECLARE_HOOK(HOOK_BATTERY_SOC_CHANGE, board_set_charger_limit, HOOK_PRIO_DEFAULT);
