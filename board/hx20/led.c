...
struct pwm_led_color_map breath_led_color_map[EC_LED_COLOR_COUNT] = {
	/* White, Green, Red */
	[EC_LED_COLOR_WHITE] = { FP_LED_HIGH, 0, 0 },
};
...
static void led_set_battery(void)
{
	...
	/* Blink both mainboard LEDS as a warning if the chasssis is open and
	 * power is on */
	if (!gpio_get_level(GPIO_CHASSIS_OPEN)) {
		set_pwm_led_color(PWM_LED0, (battery_ticks & 0x2) ?
						    EC_LED_COLOR_RED :
							  -1);
		set_pwm_led_color(PWM_LED1, (battery_ticks & 0x2) ?
						    EC_LED_COLOR_RED :
							  -1);
		return;
	}
...
static void led_set_power(void)
{
	...
	if (!lid_is_open()) {
		...
		// enable_pwr_breath(PWM_LED2, EC_LED_COLOR_WHITE,
		// breath_led_length, 0);
		return;
	}
	...
	if (chipset_in_state(CHIPSET_STATE_ON) | power_button_enable) {
		if (charge_prevent_power_on(0))
			set_pwr_led_color(PWM_LED2,
					  (power_tick % LED_TICKS_PER_CYCLE <
					   LED_ON_TICKS) ?
						  EC_LED_COLOR_RED :
							-1);
		else
