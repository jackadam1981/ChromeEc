...
static struct pd_chip_config_t pd_chip_config[] = {
	[PD_CHIP_0] = {
		...
		.addr_flags = CYP5525_I2C_CHIP0,
		.state = CYP5525_STATE_POWER_ON,
		.gpio = GPIO_EC_PD_INTA_L,
	},
	[PD_CHIP_1] = {
		.i2c_port = I2C_PORT_PD_MCU,
		.addr_flags = CYP5525_I2C_CHIP1,
		.state = CYP5525_STATE_POWER_ON,
...
enum pd_port { PD_PORT_0, PD_PORT_1, PD_PORT_2, PD_PORT_3, PD_PORT_COUNT };
...
int pd_extpower_is_present(void)
{
	...
	usb_c_extpower_present |=
		gpio_get_level(GPIO_TYPEC0_VBUS_ON_EC) ? BIT(0) : 0;
	usb_c_extpower_present |=
		gpio_get_level(GPIO_TYPEC1_VBUS_ON_EC) ? BIT(1) : 0;
	usb_c_extpower_present |=
		gpio_get_level(GPIO_TYPEC2_VBUS_ON_EC) ? BIT(2) : 0;
	usb_c_extpower_present |=
		gpio_get_level(GPIO_TYPEC3_VBUS_ON_EC) ? BIT(3) : 0;
...
void pd_extpower_is_present_interrupt(enum gpio_signal signal)
{
	...
	hook_call_deferred(&pd_extpower_deferred_data, 1 * MSEC);
}
...
static inline int reg16_to_le(int reg)
{
...
int cypd_write_reg_block(int controller, int reg, void *data, int len)
{
	...
	rv = i2c_write_offset16_block(i2c_port, addr_flags, reg16_to_le(reg),
				      data, len);
	if (rv != EC_SUCCESS)
		CPRINTS("%s failed: ctrl=0x%x, reg=0x%02x", __func__,
			controller, reg);
	return rv;
}
...
int cypd_write_reg16(int controller, int reg, int data)
{
	...
	rv = i2c_write_offset16(i2c_port, addr_flags, reg16_to_le(reg), data,
				2);
	if (rv != EC_SUCCESS)
		CPRINTS("%s failed: ctrl=0x%x, reg=0x%02x", __func__,
			controller, reg);
...
int cypd_read_reg_block(int controller, int reg, void *data, int len)
{
	...
	rv = i2c_read_offset16_block(i2c_port, addr_flags, reg16_to_le(reg),
				     data, len);
	if (rv != EC_SUCCESS)
		CPRINTS("%s failed: ctrl=0x%x, reg=0x%02x", __func__,
			controller, reg);
...
int cypd_read_reg16(int controller, int reg, int *data)
{
	...
	rv = i2c_read_offset16(i2c_port, addr_flags, reg16_to_le(reg), data, 2);
	if (rv != EC_SUCCESS)
		CPRINTS("%s failed: ctrl=0x%x, reg=0x%02x", __func__,
			controller, reg);
	return rv;
...
int cypd_read_reg8(int controller, int reg, int *data)
{
	...
	rv = i2c_read_offset16(i2c_port, addr_flags, reg16_to_le(reg), data, 1);
	if (rv != EC_SUCCESS)
		CPRINTS("%s failed: ctrl=0x%x, reg=0x%02x", __func__,
			controller, reg);
	return rv;
...
int cypd_get_int(int controller, int *intreg)
{
	...
	if (rv != EC_SUCCESS)
		CPRINTS("%s failed: ctrl=0x%x, rv=0x%02x", __func__, controller,
			rv);
	return rv;
}
int cypd_clear_int(int controller, int mask)
{
	...
	if (rv != EC_SUCCESS)
		CPRINTS("%s failed: ctrl=0x%x, mask=0x%02x", __func__,
			controller, mask);
	return rv;
}
...
{
	/*
	 * Device Reset: This command is used to request the CCG device to
	 * perform a soft reset and start at the boot-loader stage again Note:
	 * need barrel AC or battery
	 */
	return cypd_write_reg16(controller, CYP5525_RESET_REG,
				CYP5225_RESET_CMD);
}
...
int cypd_set_power_state(int power_state)
{
	...
	for (i = 0; i < PD_CHIP_COUNT; i++) {
		rv = cypd_write_reg8_wait_ack(i, CYP5525_SYS_PWR_STATE,
					      power_state);
		if (rv != EC_SUCCESS)
			break;
	}
...
int cypd_update_power_status(void)
{
	...
	for (i = 0; i < PD_CHIP_COUNT; i++) {
		rv = cypd_write_reg8_wait_ack(i, CYP5525_POWER_STAT,
					      power_stat);
		if (rv != EC_SUCCESS)
			break;
	}
...
void enable_compliance_mode(int controller)
{
	...
	rv = cypd_write_reg16(controller, CYP5525_ICL_BB_RETIMER_CMD_REG,
			      debug_ctl);
	if (rv != EC_SUCCESS)
...
void cypd_reset_source_ports(void)
{
	...
	for (i = 0; i < PD_PORT_COUNT; i++) {
		...
		if (pd_port_states[i].power_role == PD_ROLE_SOURCE) {
			...
			cypd_write_reg8(controller,
					CYP5525_PD_CONTROL_REG(port),
					CYPD_PD_CMD_HARD_RESET);
		}
	}
}
...
void cypd_set_retimer_power(enum power_state power)
{
	if (power != saved_bb_power_state) {
		...
		cypd_enque_evt(CYPD_EVT_RETIMER_PWR);
	}
}

void cypd_set_power_active(enum power_state power)
{
	cypd_enque_evt(CYPD_EVT_S_CHANGE);
}
...
{
	/* 1. CCG notifies EC with "RESET Complete event after Reset/Power
	 * up/JUMP_TO_BOOT
	 * 2. EC Reads DEVICE_MODE register does not in Boot Mode
	 * 3. CCG will enters 100ms timeout window and waits for "EC Init
	 * Complete" command
...
#define CYPD_SETUP_CMDS_LEN 5
	struct {
		...
		{ CYP5525_EVENT_MASK_REG(0), 0x7ffff, 4,
		  CYP5525_PORT0_INTR }, /* Set the port 0 event mask */
		{ CYP5525_EVENT_MASK_REG(1), 0x7ffff, 4,
		  CYP5525_PORT1_INTR }, /* Set the port 1 event mask */
		{ CYP5525_VDM_EC_CONTROL_REG(0), CYP5525_EXTEND_MSG_CTRL_EN, 1,
		  CYP5525_PORT0_INTR }, /* Set the port 0 event mask */
		{ CYP5525_VDM_EC_CONTROL_REG(1), CYP5525_EXTEND_MSG_CTRL_EN, 1,
		  CYP5525_PORT1_INTR }, /* Set the port 1 event mask */
		{ CYP5525_PD_CONTROL_REG(0), CYPD_PD_CMD_EC_INIT_COMPLETE,
		  CYP5525_PORT0_INTR }, /* EC INIT Complete */
	};
	...
	if (gpio_get_level(pd_chip_config[controller].gpio) == 0) {
		...
		cypd_clear_int(controller,
			       CYP5525_DEV_INTR + CYP5525_PORT0_INTR +
				       CYP5525_PORT1_INTR + CYP5525_UCSI_INTR);
	}
	for (i = 0; i < CYPD_SETUP_CMDS_LEN; i++) {
		rv = cypd_write_reg_block(controller, cypd_setup_cmds[i].reg,
					  (void *)&cypd_setup_cmds[i].value,
					  cypd_setup_cmds[i].length);
		if (rv != EC_SUCCESS) {
			CPRINTS("%s command: 0x%04x failed", __func__,
				cypd_setup_cmds[i].reg);
			return EC_ERROR_INVAL;
		}
		/* wait for interrupt ack to be asserted */
...
void cypd_set_source_pdo_mask(int enabled_mask)
{
	...
	for (i = 0; i < PD_PORT_COUNT; i++) {
		port = i % 2;
		controller = i >> 1;
		cypd_write_reg8(controller, CYP5525_SELECT_SOURCE_PDO_REG(port),
				enabled_mask);
	}
}
 ...
 */
void cypd_set_source_pdo(int controller, int port, uint32_t *pdos, int num_pdos,
 ...
 */
void cypd_send_msg(int controller, int port, uint32_t pd_header,
		   uint16_t ext_hdr, bool pd30, bool response_timer, void *data,
		   uint32_t data_size)
{
	uint16_t header[2] = { 0 };
	...
	/**
	 * The DM_CONTROL register should then be written to in the following
	 *format: Byte 0
	 *	- BIT 1 - 0 : Packet type should be set to SOP(0), SOP'(1), or
	 *SOP''(2).
	 ...
	 * Byte 1 : The data length specified here will be the actual length of
	 *data written into the write data memory, inclusive of the 4 byte
	 *header
	 *
	...
	if (!response_timer)
		dm_control_data |=
			CYP5525_DM_CTRL_SENDER_RESPONSE_TIMER_DISABLE;
	dm_control_data += ((data_size + 4) << 8);

	cypd_write_reg16(controller, CYP5525_DM_CONTROL_REG(port),
			 dm_control_data);
}
...
int cypd_handle_extend_msg(int controller, int port, int len,
			   enum tcpci_msg_type sop_type)
{
	/**
	...
	rv = cypd_read_reg_block(controller,
				 CYP5525_READ_DATA_MEMORY_REG(port, 0),
				 (void *)&(rx_emsg[port_idx].len), len);
	/*avoid a memcopy so direct copy into the buffer and then swap header
	 * and len look at the memory layout for the rx_emsg structure to see
	 * why we do this */
	...
	case PD_EXT_GET_BATTERY_STATUS:
		rv = cypd_response_get_battery_status(controller, port,
						      pd_header, sop_type);
		break;
	default:
...
void cypd_update_port_state(int controller, int port)
{
	...
	rv = cypd_read_reg_block(controller, CYP5525_PD_STATUS_REG(port),
				 pd_status_reg, 4);
	if (rv != EC_SUCCESS)
		CPRINTS("CYP5525_PD_STATUS_REG failed");
	pd_port_states[port_idx].pd_state =
		pd_status_reg[1] & BIT(2) ? 1 : 0; /*do we have a valid PD
						      contract*/
	...
	pd_port_states[port_idx].vconn = pd_status_reg[1] & BIT(5) ?
						 PD_ROLE_VCONN_SRC :
						       PD_ROLE_VCONN_OFF;

	rv = cypd_read_reg8(controller, CYP5525_TYPE_C_STATUS_REG(port),
			    &typec_status_reg);
	if (rv != EC_SUCCESS)
		CPRINTS("CYP5525_TYPE_C_STATUS_REG failed");

	pd_port_states[port_idx].cc = typec_status_reg & BIT(1) ? POLARITY_CC2 :
									POLARITY_CC1;
	...
	rv = cypd_read_reg_block(controller, CYP5525_CURRENT_PDO_REG(port),
				 pdo_reg, 4);
	pd_current = (pdo_reg[0] + ((pdo_reg[1] & 0x3) << 8)) * 10;
	pd_voltage =
		(((pdo_reg[1] & 0xFC) >> 2) + ((pdo_reg[2] & 0xF) << 6)) * 50;

	cypd_read_reg_block(controller, CYP5525_CURRENT_RDO_REG(port), rdo_reg,
			    4);
	/*rdo_current = ((rdo_reg[0] + (rdo_reg[1]<<8)) & 0x3FF)*10,*/
	rdo_max_current =
		(((rdo_reg[1] >> 2) + (rdo_reg[2] << 6)) & 0x3FF) * 10;
	...
	if (pd_port_states[port_idx].c_state == CYPD_STATUS_SOURCE) {
		typec_set_input_current_limit(port_idx, type_c_current,
					      TYPE_C_VOLTAGE);
	...
	if (pd_port_states[port_idx].pd_state) {
		if (pd_port_states[port_idx].power_role == PD_ROLE_SINK) {
			pd_set_input_current_limit(port_idx, pd_current,
						   pd_voltage);
			charge_manager_set_ceil(port_idx, CEIL_REQUESTOR_PD,
						pd_current);
...
void cypd_print_version(int controller, const char *vtype, uint8_t *data)
{
	CPRINTS("Controller %d  %s version B:%d.%d.%d.%d", /* AP:%d.%d.%d.%c%c
							    */
		controller, vtype, (data[3] >> 4) & 0xF, (data[3]) & 0xF,
		data[2], data[0] + (data[1] << 8)
		/* (data[7]>>4) & 0xF, (data[7]) & 0xF, data[6], data[5],
		   data[4] */
	);
}
void cyp5525_get_version(int controller)
{
	...
	rv = i2c_read_offset16_block(i2c_port, addr_flags,
				     reg16_to_le(CYP5525_READ_ALL_VERSION_REG),
				     data, 24);
	if (rv != EC_SUCCESS)
...
void cyp5525_port_int(int controller, int port)
{
	...
	int port_idx = (controller << 1) + port;
	enum tcpci_msg_type sop_type;
	rv = i2c_read_offset16_block(
		i2c_port, addr_flags,
		reg16_to_le(CYP5525_PORT_PD_RESPONSE_REG(port)), data2, 4);
	if (rv != EC_SUCCESS)
	...
	case CYPD_RESPONSE_PD_CONTRACT_NEGOTIATION_COMPLETE:
		CPRINTS("CYPD_RESPONSE_PD_CONTRACT_NEGOTIATION_COMPLETE %d",
			port_idx);
	...
	case CYPD_RESPONSE_EXT_SOP2_RX:
		...
		cypd_handle_extend_msg(controller, port, response_len,
				       sop_type);
	...
	default:
		if (response_len && verbose_msg_logging) {
			...
			i2c_read_offset16_block(
				i2c_port, addr_flags,
				CYP5525_READ_DATA_MEMORY_REG(port, 0), data2,
				MIN(response_len, 32));
...
static bool bb_retimer_init_pending = true;

static void bb_retimer_chipset_shutdown(void)
{
	if (!bb_retimer_init_pending) {
		cypd_set_retimer_power(POWER_G3);
		gpio_set_level(GPIO_PCH_PWR_EN, 0);
	}
}
DECLARE_HOOK(HOOK_CHIPSET_SHUTDOWN, bb_retimer_chipset_shutdown,
	     HOOK_PRIO_DEFAULT);

void pd_bb_powerdown_deferred(void)
{
	bb_retimer_init_pending = false;
}
...
int cyp5525_device_int(int controller)
{
	...
	if (cypd_read_reg16(controller, CYP5525_RESPONSE_REG, &data) ==
	    EC_SUCCESS) {
...
void cypd_handle_state(int controller)
{
	...
	case CYP5525_STATE_POWER_ON:
		...
		if (cypd_read_reg8(controller, CYP5525_DEVICE_MODE, &data) ==
		    EC_SUCCESS) {
			if ((data & 0x03) == 0x00) {
				/*pd_chip_config[controller].state =
				 * CYP5525_STATE_BOOTLOADER;*/
				CPRINTS("CYPD %d is in bootloader 0x%04x",
					controller, data);
				// delay = 25 * MSEC;
				if (cypd_read_reg16(controller,
						    CYP5525_BOOT_MODE_REASON,
						    &data) == EC_SUCCESS) {
					CPRINTS("CYPD bootloader reason 0x%02x",
						data);
				}
	...
	case CYP5525_STATE_APP_SETUP:
		...
		cypd_write_reg8_wait_ack(controller,
					 CYP5225_USER_MAINBOARD_VERSION,
					 board_get_version());

		bb_retimer_init_pending = true;
		cypd_update_power();
		...
		hook_call_deferred(&pd_bb_powerdown_deferred_data,
				   BB_PWR_DOWN_TIMEOUT);
		...
		cypd_write_reg16_wait_ack(controller, CYP5225_USER_BB_POWER_EVT,
					  RT_EVT_VSYS_ADDED);
		...
		break;
	default:
		CPRINTS("PD handle_state but in 0x%02x state!",
			pd_chip_config[controller].state);
		break;
	}
}
...
{
	task_set_event(TASK_ID_CYPD, evt);
}

void pd0_chip_interrupt_deferred(void)
{
	task_set_event(TASK_ID_CYPD, CYPD_EVT_INT_CTRL_0);
}
...
{
	task_set_event(TASK_ID_CYPD, CYPD_EVT_INT_CTRL_1);
}
...
void soc_plt_reset_interrupt(enum gpio_signal signal)
{
	...
	/*hook_call_deferred(&soc_plt_reset_interrupt_deferred_data, 25 *
	 * MSEC);*/
}
...
void cypd_interrupt_handler_task(void *p)
{
	...
	cypd_enque_evt(CYPD_EVT_STATE_CTRL_0 | CYPD_EVT_STATE_CTRL_1);

	for (i = 0; i < PD_CHIP_COUNT; i++) {
		gpio_enable_interrupt(pd_chip_config[i].gpio);
		cypd_enque_evt(CYPD_EVT_STATE_CTRL_0 << i);
	}
	while (1) {
		...
		if (evt & CYPD_EVT_AC_PRESENT) {
			CPRINTS("GPIO_AC_PRESENT_PD_L changed: value: 0x%02x",
				gpio_get_level(GPIO_AC_PRESENT_PD_L));
		}
...
{
	/**
	 * Specification 5.3.3 Port Configuration Change descripes the steps to
	 *change port configuration.
	 *
	 * Step1: Disabled the port using PDPORT_ENABLE register
	 * Step2: Writed the data memory register in the follow format
	 *			Byte 0 : desired port role (0: Sink, 1: Source,
	 *2: Dual Role) Byte 1 : Default port role in case of Dual Role (0:
	 *Sink, 1: Source) Byte 2 : DRP toggle enable (in case of Dual Role
	 *port) Byte 3 : Try.SRC enable (in case of Daul Role port) Step3: Using
	 *the "Change PD Port Parameters" command in PD_CONTROL register Step4:
	 *Enabled the port using PDPORT_ENABLE register
	 */
	...
	rv = cypd_write_reg8(controller, CYP5525_PDPORT_ENABLE_REG,
			     (0x03 & ~BIT(cyp_port)));
	if (rv != EC_SUCCESS)
		return rv;

	/**
	 * Stopping an active PD port can take a long time (~1 second) in case
	 * VBUS is being provided and needs to be discharged.
	 */
	cyp5225_wait_for_ack(controller, 1 * SECOND);

	rv = cypd_write_reg_block(
		controller, CYP5525_WRITE_DATA_MEMORY_REG(port, 0), data, 4);
	if (rv != EC_SUCCESS)
		return rv;
...
int board_set_active_charge_port(int charge_port)
{
	...
	for (i = 0; i < PD_CHIP_COUNT; i++)
		cypd_write_reg8(i, CYP5225_USER_DISABLE_LOCKOUT,
				disable_lockout);

	cypd_enque_evt(CYPD_EVT_UPDATE_PWRSTAT);
			    ...
			    int len)
{
	...
	if (verbose_msg_logging) {
		CPRINTS("PD Controller %d Port %d  Code 0x%02x %s %s Len: 0x%02x",
			controller, port, id, code,
			id & 0x80 ? "Response" : "Event", len);
	}
}
...
static int cmd_cypd_get_status(int argc, char **argv)
{
	...
	static const char *const port_status[] = { "Nothing",	  "Sink",
						   "Source",	  "Debug",
						   "Audio",	  "Powered Acc",
						   "Unsupported", "Invalid" };
	static const char *const current_level[] = { "DefaultA", "1.5A", "3A",
						     "InvA" };
	static const char *const state[] = { "ERR", "POWER_ON", "APP_SETUP",
					     "READY", "BOOTLOADER" };
	CPRINTS("AC_PRESENT_PD value: %d",
		gpio_get_level(GPIO_AC_PRESENT_PD_L));
	for (i = 0; i < PD_CHIP_COUNT; i++) {
		CPRINTS("PD%d INT value: %d", i,
			gpio_get_level(pd_chip_config[i].gpio));
	}
	...
	if (argc == 2) {
		...
		if (i < PD_CHIP_COUNT) {
			...
			CPRINTS("CYPD_DEVICE_MODE: 0x%02x %s", data,
				mode[data & 0x03]);
			cypd_read_reg_block(i, CYP5525_HPI_VERSION, data16, 4);
			CPRINTS("HPI_VERSION: 0x%02x%02x%02x%02x", data16[3],
				data16[2], data16[1], data16[0]);
			cypd_read_reg8(i, CYP5525_INTR_REG, &data);
			CPRINTS("CYPD_INTR_REG: 0x%02x %s %s %s %s", data,
			...
			cypd_read_reg16(i, CYP5525_PORT_PD_RESPONSE_REG(0),
					&data);
			CPRINTS("CYPD_PORT0_PD_RESPONSE_REG: 0x%02x", data);
			cypd_read_reg16(i, CYP5525_PORT_PD_RESPONSE_REG(1),
					&data);
			...
			for (p = 0; p < 2; p++) {
				...
				cypd_read_reg_block(i,
						    CYP5525_CURRENT_RDO_REG(p),
						    data16, 4);
				CPRINTS("             RDO : Current:%dmA MaxCurrent%dmA 0x%08x",
					((data16[0] + (data16[1] << 8)) &
					 0x3FF) *
						10,
					(((data16[1] >> 2) + (data16[2] << 6)) &
					 0x3FF) *
						10,
					*(uint32_t *)data16);

				cypd_read_reg_block(i,
						    CYP5525_CURRENT_PDO_REG(p),
						    data16, 4);
				CPRINTS("             PDO : MaxCurrent:%dmA Voltage%dmA 0x%08x",
					...
					(((data16[1] >> 2) + (data16[2] << 6)) &
					 0x3FF) *
						50,
					*(uint32_t *)data16);
				cypd_read_reg8(i, CYP5525_TYPE_C_VOLTAGE_REG(p),
					       &data);
				CPRINTS("  TYPE_C_VOLTAGE : %dmV", data * 100);
				cypd_read_reg16(i,
						CYP5525_PORT_INTR_STATUS_REG(p),
						&data);
				CPRINTS(" INTR_STATUS_REG0: 0x%02x", data);
				cypd_read_reg16(
					i, CYP5525_PORT_INTR_STATUS_REG(p) + 2,
					&data);
			...
			cypd_read_reg_block(i, CYP5525_MESSAGE_IN_REG, data16,
					    16);
			cypd_print_buff(" Msg  In:", data16, 16);
			cypd_read_reg_block(i, CYP5525_MESSAGE_OUT_REG, data16,
					    16);
...
static int cmd_cypd_control(int argc, char **argv)
{
	...
	if (argc >= 3) {
		...
		if (!strncmp(argv[1], "en", 2) || !strncmp(argv[1], "dis", 3)) {
			...
			/*
			 * see if we can talk to the PD chip yet - issue a reset
			 * command Note that we cannot issue a full reset
			 * command if the PD controller has a device attached -
			 * as it will return with an invalid command due to
			 * needing to disable all ports first.
			 */
			...
			r = strtoull(argv[3], &e, 0);
			regval = strtoull(argv[4], &e, 0);
...
DECLARE_CONSOLE_COMMAND(cypdbb, cmd_cypd_bb, "controller 0x0000 0xdata ",
...
static int cmd_cypd_msg(int argc, char **argv)
{
	...
	if (argc >= 3) {
		...
		/*note when in chunked mode the first chunk does not set
		 * request_chunk=1 see example 6.2.1.2.5.2 in usb pd rev 3.0
		 */
		data[1] |= chunked ? BIT(15) : 0x00;
		/*ext msg data*/
		data[2] = 0; /*internal battery 0*/
		data_len = 5;
		cypd_write_reg_block(ctrl,
				     CYP5525_WRITE_DATA_MEMORY_REG(port, 0),
				     (void *)data, 5);
		/*the request has 1 byte which should be set to 0 for battery
		 * idx 0*/
		cmd = CYP5525_DM_CTRL_SOP +
		      CYP5525_DM_CTRL_EXTENDED_DATA_REQUEST + (data_len << 8);
