...
{
	/*
	 * power limit is related to AC state, battery percentage, and power
	 * budget
	 */
	...
	if (!extpower_is_present() || (active_power < 55)) {
		...
		pl2_watt = 64;
		pl4_watt = 121;
		/* psys watt = adp watt * 0.95 + battery watt(55 W) * 0.7 - pps
		 * power budget */
...
DECLARE_HOOK(HOOK_BATTERY_SOC_CHANGE, update_soc_power_limit_hook,
	     HOOK_PRIO_DEFAULT);
...
DECLARE_CONSOLE_COMMAND(cpupower, cmd_cpupower, "cpupower pl1 pl2 pl4 psys ",
