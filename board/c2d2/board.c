...
/*
 * Tracks if bus pins are locked by some function like UART holding, I2C,
 * or SPI.
 *
...
const char *to_string(const enum bus_lock val)
{
...
/**
 * Checks the specified pins if they are unlocked are already the specified
 * bus val.
 *
 * @param val value of bus that is acceptable besides being unlocked
 * @param pins the pointer to the pins being checked
 *
 * @return true if the pins are indeed unlocked or the specified value
 */
bool ensure_unlocked_or(const enum bus_lock val, const enum bus_lock *pins)
{
	if (*pins == BUS_UNLOCKED || *pins == val)
		return true;

	ccprintf("Cannot update to %s! Pin busy with %s.\n", to_string(val),
		 to_string(*pins));
	return false;
}
...
static int command_hold_usart_low(int argc, char **argv)
{
	...
	if (argc == 3) {
		...
		if ((*bus == BUS_UART_HELD) == hold_low) {
			/* Do nothing since there is no change */
		} else if (hold_low) {
			/* Ensure no other use of these pins */
			if (!ensure_unlocked_or(BUS_UART_HELD, bus))
...
static int command_enable_spi(int argc, char **argv)
{
	...
	if (argc == 2) {
		...
		if (current_spi_vref_state == spi_vref) {
			...
			if (!ensure_unlocked_or(BUS_SPI, &h1_pins) ||
			    !ensure_unlocked_or(BUS_SPI, &ec_pins))
				goto busy_error_unlock;
...
static int command_enable_i2c(int argc, char **argv)
{
	...
	if (argc == 3) {
		...
		if ((*bus == BUS_I2C) == !!speed) {
			/* No change, do nothing */
		} else if (speed) {
			/* Ensure no one is using these pins */
			if (!ensure_unlocked_or(BUS_I2C, bus))
				goto busy_error_unlock;
