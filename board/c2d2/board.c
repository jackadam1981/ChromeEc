...
static void update_vrefs_and_shifters(void)
{
	 ...
	 * to read in the background every second until we get out of that state
	 */
	if (h1_vref != OFF && ec_vref == OFF) {
		CPRINTS("Looks like DUT power button is held. Will try again.");
		h1_vref = ec_vref = OFF;
		hook_call_deferred(&update_vrefs_and_shifters_data, SECOND);
	} else {
		/* Cancel any future deferred call */
		hook_call_deferred(&update_vrefs_and_shifters_data, -1);
	}
