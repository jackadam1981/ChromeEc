...
enum battery_type {
	...
	SANYO,
	UNKNOWN,
	INIT,	/* only use this as default static value */
};
...
static struct battery_device *battery_inserted = &support_batteries[INIT];
...
const struct battery_info *battery_get_info(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(support_batteries); ++i) {
		if ((strcasecmp(support_batteries[i].manuf, manuf) == 0) &&
			...
			battery_inserted = &support_batteries[i];

			return battery_inserted->battery_info;
		}
	}
...
err_unknown:
	battery_inserted = &support_batteries[UNKNOWN];
	return battery_inserted->battery_info;
}
...
enum battery_present battery_is_present(void)
{
	...
	if (batt_pres == BP_YES &&
		can_battery_provide_power(battery_inserted->type) == UNABLE)
			batt_pres = BP_NO;
