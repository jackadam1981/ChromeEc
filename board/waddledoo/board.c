...
{
	int hdmi_hpd_level = gpio_get_level(GPIO_EC_I2C_SUB_C1_SDA_HDMI_HPD);

	CPRINTS("HDMI HPD: %d", hdmi_hpd_level);
	gpio_set_level(GPIO_EC_AP_USB_C1_HDMI_HPD, hdmi_hpd_level);
}
...
void board_init(void)
{
	...
	if (get_cbi_fw_config_db() == DB_1A_HDMI) {
		...
		gpio_config_pin(MODULE_I2C, GPIO_EC_I2C_SUB_C1_SDA_HDMI_HPD,
									    0);
		...
		gpio_enable_interrupt(GPIO_EC_I2C_SUB_C1_SDA_HDMI_HPD);
	} else {
		/* Set SDA as an input */
		gpio_set_flags(GPIO_EC_I2C_SUB_C1_SDA_HDMI_HPD, GPIO_INPUT);
...
DECLARE_HOOK(HOOK_CHIPSET_RESUME, hdmi_enable, HOOK_PRIO_DEFAULT);

static void hdmi_disable(void)
{
	gpio_set_level(GPIO_EC_I2C_SUB_C1_SCL_HDMI_EN_ODL, 1);
}
DECLARE_HOOK(HOOK_CHIPSET_SUSPEND, hdmi_disable, HOOK_PRIO_DEFAULT);
