...
void board_set_power_supply_ready(int ready)
{
	...
	if (ready) {
		hook_call_deferred(&power_on_ap_data, 0);
		gpio_set_level(GPIO_USB_CC_POLARITY, !pd_get_polarity(0));
	} else {
		hook_call_deferred(&power_off_ap_data, 0);
	}
}
...
void usb_spi_board_disable(struct usb_spi_config const *config)
{
	...
	power_off_ap();
	hook_call_deferred(&power_on_ap_data, 500*MSEC);
}
...
static int gale_command_ssmux(int argc, char **argv)
{
	...
	CPRINTF("%8s - %s\n", argv[0],
		gpio_get_level(GPIO_USB_SS_MUX_EN_L) ? "disable"
						     : "enable");
	return EC_SUCCESS;
}
