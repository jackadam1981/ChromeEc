...
static void set_pmic_v5a_ds3_ctrl(int forced_pwm, const char *caller)
{
	/* Set V5ADS3CNT
	 ...
	 */
	int value = forced_pwm ? 0x13 : 0x12, ret;

	ret = i2c_write8(I2C_PORT_PMIC, I2C_ADDR_BD99992, 0x31, value);
	ccprints("%s call set_pmic_v5a_ds3_ctrl(%d): write 0x%02x %s",
		caller,
		forced_pwm,
		value,
		ret == EC_SUCCESS ? "SUCCESS" : "FAILED");
}
...
static void board_pmic_init(void)
{
	...
	if (system_jumped_to_this_image()) {
		set_pmic_v5a_ds3_ctrl(1, __func__);
		return;
	} else {
		set_pmic_v5a_ds3_ctrl(0, __func__);
	}
