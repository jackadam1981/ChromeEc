 ...
 *
 * Power and battery LED control for cave.
 */
...
static int cave_led_set_color_battery(enum led_color color)
{
	return bat_led_set_color(color);
}

static int cave_led_set_color(enum ec_led_id led_id, enum led_color color)
{
	int rv;
	...
	case EC_LED_ID_BATTERY_LED:
		rv = cave_led_set_color_battery(color);
		break;
	default:
		return EC_ERROR_UNKNOWN;
	}
...
int led_set_brightness(enum ec_led_id led_id, const uint8_t *brightness)
{
	if (brightness[EC_LED_COLOR_RED] != 0 &&
		...
		cave_led_set_color(led_id, LED_AMBER);
	else if (brightness[EC_LED_COLOR_RED] != 0)
		cave_led_set_color(led_id, LED_RED);
	else if (brightness[EC_LED_COLOR_GREEN] != 0)
		cave_led_set_color(led_id, LED_GREEN);
	else
		cave_led_set_color(led_id, LED_OFF);

	return EC_SUCCESS;
}

static void cave_led_set_battery(void)
{
	...
	case PWR_STATE_DISCHARGE:
		...
		/* Less than 10%, blink one second every four seconds */
			...
			cave_led_set_color_battery(LED_OFF);
		break;
	...
	case PWR_STATE_CHARGE_NEAR_FULL:
		cave_led_set_color_battery(LED_GREEN);
		break;
