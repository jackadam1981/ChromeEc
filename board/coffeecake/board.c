...
void board_set_usb_output_voltage(int mv)
{
	...
	STM32_DAC_DHR12RD = dac_mv * 4096 / 3300;
}
...
static void board_post_init(void)
{
	...
	if (!gpio_get_level(GPIO_AC_PRESENT_L)) {
		/*
		 * AC powered
		 *  - sy21612 source mode: VBAT -> VBUS
		 *  - DRP SRC
		 */
		...
		/*
		 * DUT powered
		 *  - sy21612 sink mode: VBUS -> VBAT(9V)
		 *  - DRP SRC
		 */
...
const struct adc_t adc_channels[] = {
	...
	[ADC_DAC_REF]   = {"DAC_REF", 3300, 4096, 0, STM32_AIN(4)},
};
