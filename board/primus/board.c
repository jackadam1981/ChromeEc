...
static void assert_prochot(void)
{
	...
	IDPM = ((PD_MAX_POWER_MW * 95 / 100) / 20) /1000; //mA, we need to use W, not mW
	CPRINTS("IDPM=%dmA", IDPM);

	V_acpacn = IDPM * CONFIG_CHARGER_SENSE_RESISTOR; //mV, 10m ohm
	...
	total_W = W_adpt + battery_design_wattage;
	CPRINTS("total_W=%d", total_W);
	CPRINTS("PD_MAX_POWER_MW + battery_max_continue_discharge=%d", PD_MAX_POWER_MW + battery_max_continue_discharge);
	if(total_W > (PD_MAX_POWER_MW + battery_max_continue_discharge)) {
		CPRINTS("assert prochot");
		gpio_set_level(GPIO_EC_PROCHOT_ODL, 1);
	}
	else if(total_W < (PD_MAX_POWER_MW + (battery_max_continue_discharge*90/100))) {
		CPRINTS("de-assert prochot");
		gpio_set_level(GPIO_EC_PROCHOT_ODL, 0);
	}
}
DECLARE_HOOK(HOOK_CHIPSET_STARTUP, assert_prochot, HOOK_PRIO_DEFAULT);
