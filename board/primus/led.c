...
#include "common.h"
#include "console.h"
>>>>>>> aa20e41d2b (primus: add LED behavior)
...
#define LED_EVENT_S0	TASK_EVENT_CUSTOM_BIT(1)
...
{
	task_set_event(TASK_ID_LOGOLED, LED_EVENT_S0);
}
...
void logoled_task(void *u)
{
	...
	while (1) {
		...
		if (evt & LED_EVENT_SUSPEND) {
			...
			suspend_led_update_deferred();
		} else if (evt & LED_EVENT_S0) {
			hook_call_deferred(&suspend_led_update_deferred_data,
				-1);
		}
	}
}
