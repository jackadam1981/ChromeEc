...
static void oak_led_set_battery(int board_version)
{
	...
	default:
		...
		if (chipset_in_state(CHIPSET_STATE_ANY_OFF))
			...
			int cycle_time = 3;
			/* Oak rev5 with GlaDOS ID has a extremelty power
			 * comsuming LED. Incrase LED blink cycle time in half
			 * to reduce S3 power comsuption. */
			if (board_version == OAK_REV5) {
				cycle_time = 10;
			}
			bat_led_set(BAT_LED_GREEN,
				    (battery_second % cycle_time != 0) ? 0 : 1);
    }
