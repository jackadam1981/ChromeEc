 ...
 */
void board_typec_dp_on(int port, int *dp_flags)
{
	int other_port = port ? 0 : 1;
	mutex_lock(&dp_hw_lock);

	if (dp_hw_port != other_port) {
...
void board_typec_dp_off(int port, int *dp_flags)
{
	...
	if (dp_flags[other_port] & DP_FLAGS_DP_ON)
		board_typec_dp_on(other_port, dp_flags);
}

/**
 * Set DP hotplug detect level.
 */
void board_typec_dp_set(int port, int *dp_flags, int level)
{
	mutex_lock(&dp_hw_lock);
