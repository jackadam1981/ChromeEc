...
DECLARE_CONSOLE_COMMAND(amonbmon, console_command_amon_bmon,
			"amonbmon [a|b]",
			"Get charger AMON and/or BMON voltage and current",
			NULL);
#endif /* !CONFIG_BOARD_OAK_REV_1 */

#if (defined(CONFIG_BOARD_OAK_REV_1) || defined(CONFIG_BOARD_OAK_REV_2))
/* Using this hook if system doesn't have enough external interrupt line. */
