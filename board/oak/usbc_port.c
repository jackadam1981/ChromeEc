...
static struct ec_response_pd_port pd_ports[PD_PORT_COUNT];
...
/* TEST ONLY: Switch port C1 to DP path */
int dp_test(int port)
{
	uint8_t mode = PI3USB30532_MODE_DP;
	ccprintf("Switch Port%d to DP lane (polarity: %d)\n",
		port, pd_ports[port].polarity);
	if (pd_ports[port].polarity)
		mode |= PI3USB30532_BIT_SWAP;
	return pi3usb30532_set_switch(port, mode);
}
...
void board_sync_usbc_port()
{
	...
	for (port = 0; port < PD_PORT_COUNT; port++) {
		...
		if (pd_ports[port].data_role != port_status.data_role ||
			pd_ports[port].power_role != port_status.power_role)
		{
			pd_ports[port].data_role = port_status.data_role;
			pd_ports[port].power_role = port_status.power_role;

			/* 5V controlled by (DFP && !PWR_SINK) */
			if (port_status.data_role == PD_ROLE_DFP &&
				port_status.power_role != PD_ROLE_SINK) {
				CPRINTS("Enable C%d 5V\n", port);
				if (port == 0)
					gpio_set_level(GPIO_USB_C0_5V_OUT, 1);
				else
					gpio_set_level(GPIO_USB_C1_5V_OUT, 1);
			} else {
				CPRINTS("DISABLE C%d 5V\n", port);
				if (port == 0)
					gpio_set_level(GPIO_USB_C0_5V_OUT, 0);
				else
					gpio_set_level(GPIO_USB_C1_5V_OUT, 0);
			}
		}
