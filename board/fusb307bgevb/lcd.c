...
static struct lcd_state_info state{
	...
	.displayfuction = LCD_4BITMODE | LCD_1LINE | LCD_5x8DOTS;
};

static void lcd_init(uint8_t cols, uint8_t rows, uint8_t dotsize) {
	state.cols = cols;
	state.rows = rows;
	if (rows > 1) {
...
static void lcd_setCursor(uint8_t col, uint8_t row){
        ...
        if ( row > (state_rows-1) ) {
                row = state_rows-1;    // we count rows starting w/0
        }
        command(LCD_SETDDRAMADDR | (col + row_offsets[row]));
}

// Turn the display on/off (quickly)
static void lcd_noDisplay() {
...
static void lcd_noBacklight(void) {
...
void expanderWrite(uint8_t _data){                                        
	i2c_write8(I2C_PORT_TCPT, LCD_SLAVE_ADDR, 0x00, _data | state.backlightval);
}
