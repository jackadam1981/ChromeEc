...
static const struct fan_rpm fan_rpm_ddr = {
	.rpm_min = 4000,
	.rpm_start = 4000,
	.rpm_max = 4200,
};
...
static const struct fan_rpm fan_rpm_ambient = {
	.rpm_min = 4000,
	.rpm_start = 4000,
	.rpm_max = 4200,
};
...
static void fan_set_percent(int fan, int pct, int soc_temp, int fan_triggered)
{
	...
	else if (fan_triggered == 0) {
		if (soc_temp > SENSOR_SOC_FAN_MID)
			fans[fan].rpm = &fan_rpm_cpu_1;
		else
			fans[fan].rpm = &fan_rpm_cpu_0;
	} else if (fan_triggered == 1)
		fans[fan].rpm = &fan_rpm_ddr;
	else if (fan_triggered == 3)
		fans[fan].rpm = &fan_rpm_ambient;
...
void board_override_fan_control(int fan, int *tmp)
{
	 ...
	 * When other sensors charger is triggered, the fan is control
	 * by other sensors.
	 *
	...
	ddr_temp_c = tmp[TEMP_SENSOR_2_DDR];
	charger_temp_c = tmp[TEMP_SENSOR_3_CHARGER];
	ambient_temp_c = tmp[TEMP_SENSOR_4_AMBIENT];
	...
	if (soc_temp_c > SENSOR_SOC_FAN_MID) {
		thermal_params[0].temp_fan_off = sensor_soc_fan_mid;
		thermal_params[0].temp_fan_max = sensor_soc_fan_max;
	} else {
		thermal_params[0].temp_fan_off = sensor_soc_fan_off;
		thermal_params[0].temp_fan_max = sensor_soc_fan_mid;
	}

	sensor_soc = thermal_fan_percent(thermal_params[0].temp_fan_off,
					 thermal_params[0].temp_fan_max,
					 C_TO_K(soc_temp_c));
	sensor_ddr = thermal_fan_percent(thermal_params[1].temp_fan_off,
