...
void fan_set_percent(int fan, int pct, int fan_rpm)
{
	...
	ccprints("🈲🈲 min_rpm %d",min_rpm);
	ccprints("🈲🈲 pct %d",pct);
	new_rpm = fan_percent_to_rpm(fan, pct);
	actual_rpm = fan_get_rpm_actual(FAN_CH(fan));
	ccprints("🈲🈲 new_rpm %d",new_rpm);
	ccprints("🈲🈲 actual_rpm %d",actual_rpm);
	if (new_rpm &&
	    actual_rpm < min_rpm &&
...
void board_override_fan_control(int fan, int *tmp)
{
	...
	sensor_0 = thermal_fan_percent(thermal_params[0].temp_fan_off -1 ,
				thermal_params[0].temp_fan_max,
				C_TO_K(tmp[0]));
	sensor_1 = thermal_fan_percent(thermal_params[1].temp_fan_off -1 ,
				thermal_params[1].temp_fan_max,
				C_TO_K(tmp[1]));
	sensor_2 = thermal_fan_percent(thermal_params[2].temp_fan_off -1 ,
	...
	ccprints("🈲🈲🈲🈲🈲🈲 sensor_0 %d",sensor_0);
	ccprints("🈲🈲🈲🈲🈲🈲 sensor_1 %d",sensor_1);
	ccprints("🈲🈲🈲🈲🈲🈲 sensor_2 %d",sensor_2);
	ccprints("🈲🈲🈲🈲🈲🈲 f %d",f);
