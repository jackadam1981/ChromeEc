...
int board_set_active_charge_port(int port)
{
	...
	if (port == CHARGE_PORT_NONE)
		return EC_SUCCESS;
	/*
	 * Allow only the barreljack charger.
	 */
	 ...
	 * This can happen if one of the type-C ports is
	 * sourcing VBUS and the barreljack is not connected.
	 */
	if (gpio_get_level(GPIO_BJ_ADP_PRESENT_L)) {
		CPRINTS("Barreljack not connected");
		return EC_ERROR_INVAL;
	}

	CPRINTS("Enabling barreljack charger");
