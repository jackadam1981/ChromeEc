...
/* convert ATIME register to integration time, in FP */
static fp_t tcs3400_get_integration_time(int atime)
{
	return fp_mul(FLOAT_TO_FP(TCS_MAX_INTEGRATION_TIME),
			INT_TO_FP(TCS_ATIME_GRANULARITY - atime));
}
				     ...
				     int32_t *crgb_data, int32_t *xyz_data)
{
	int n, cur_gain, dgf;
	fp_t n_interval;
	fp_inter_t integration_time_ms;
	...
	integration_time_ms = tcs3400_get_integration_time(sat_p->atime);

	/* n_interval = (G+B)/C, to use different coefficient*/
	n_interval = fp_div(INT_TO_FP(crgb_data[2]+crgb_data[3])
	...
	case 1:
		...
		(crgb_data[0]*(fp_inter_t)FLOAT_TO_FP(3.8) +
		 ...
		 FP_TO_INT(integration_time_ms*cur_gain));
	break;
	case 2:
		xyz_data[1] = FP_TO_INT(dgf *
	...
	/* reverse lux data affect by normalize_channel_data() */
