...
#define FACTORY_CLEAR BIT(2)

static int als_enable = 1;
...
static int als_eeprom_write(uint8_t offset, uint8_t *data)
{
	int rv;
...
static void check_als_status(void)
{
	...
	/* Check the first byte not "CBI", otherwise we need disable
	 * als function and wait factory clear eeprom data.
	 */
