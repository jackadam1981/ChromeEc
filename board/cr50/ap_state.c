 ...
 */
static void ap_detect_with_tpm_rst(void)
{
	/*
	 * If cr50 is using the ap_uart state to signal the ap state, the
	 * ap_uart state machine will handle everything and pass the on/off
	 * state to ap state after debouncing. Nothing needs to be done.
	 */
	...
	if (gpio_get_level(GPIO_TPM_RST_L)) {
		 ...
		 *
		 * This can happen if the hook task calls
		 * ap_detect_with_tpm_rst() before deferred_tpm_rst_isr(). In
		 * this case, the deferred handler is already pending so calling
		 * the ISR has no effect.
		 *
...
DECLARE_HOOK(HOOK_SECOND, ap_detect_with_tpm_rst, HOOK_PRIO_DEFAULT);
