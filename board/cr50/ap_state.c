...
{
	/*
	 * TPM_RST_L must be used to detect the AP state. If it isn't refuse to
	 * turn 'off' the AP because that's dangerous when we cannot detect the
	 * state of the device. This will lead to increased cr50 power
	 * consumption, but wont prevent the device from booting.
	 */
	if (!board_detect_ap_with_tpm_rst()) {
		CPRINTS("Cannot turn off AP state with unsupported UART "
			"detection");
		/* Turn the AP on again. That's safest */
		set_ap_on();
		return;
	}
	CPRINTS("AP off");
