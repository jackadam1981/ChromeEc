 ...
 *
 * @param asserted: false power-up tests should run on wake from sleep,
 * otherwise can be skipped.
 */
static void fips_set_powerup_test_done(bool asserted)
{
	board_set_fips_policy_test(asserted);
...
void fips_set_status(enum fips_status status)
{
	...
	if (status & FIPS_ERROR_MASK) {
		status = _fips_status & ~FIPS_POWER_UP_TEST_DONE;
		/* TBD where */
		fips_set_powerup_test_done(false);
	}
}
...
static size_t read_tpm_nvmem_size(uint16_t obj_index)
{
#if 0
...
static uint64_t fips_power_up_tests(void)
{
	...
	if (EC_SUCCESS ==
		...
		/* if no errors, don't run tests on wake from sleep */
		if (!(_fips_status & FIPS_ERROR_MASK))
