...
static bool fips_u2f_compliant(void)
{
	...
	if (!val_len && !read_tpm_nvmem_size(TPM_HIDDEN_U2F_KEK) &&
		...
		/* but apparantally, board FIPS mode wasn't set yet */
 ...
 */
static void *kat_buf;  /* pointer to scratch buffer for known-answer tests */
...
static bool fips_rsa2048_verify_kat(void)
{
	...
	DCRYPTO_SHA256_hash(msg, sizeof(msg), b->digest);
	passed = DCRYPTO_rsa_verify(&rsa, b->digest, sizeof(b->digest), sig,
				    sizeof(sig), PADDING_MODE_PKCS1,
				    HASH_SHA256);
	if (!passed)
		return false;
	DCRYPTO_SHA256_hash(bad_msg, sizeof(bad_msg), b->digest);

	/* now signature should fail */
	return !DCRYPTO_rsa_verify(&rsa, b->digest, sizeof(b->digest), sig,
				   sizeof(sig), PADDING_MODE_PKCS1,
				   HASH_SHA256);
}
#endif
...
static void fips_print_mode(void)
{
	if (_fips_status == FIPS_UNINITIALIZED)
		ccprints("FIPS mode not initialized");
	if (_fips_status & FIPS_ERROR_MASK)
		ccprints("FIPS error code 0x%08x\n", _fips_status);

	ccprints("Running in FIPS 140-2 %s mode",
...
static void fips_print_test_time(uint64_t time)
{
	if (time == FIPS_POWERON_TEST_ERROR)
		ccprints("FIPS test failed to run");
	else if (time != -1ULL)
		ccprints("FIPS power-up tests completed in %llu", time);
}
