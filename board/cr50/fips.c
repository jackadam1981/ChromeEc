...
void fips_set_status(enum fips_status status)
{
	 ...
	 */
	if ((_fips_status | status) & FIPS_MODE_REQUESTED) {
...
void _fips_throw_err(enum fips_status err, const char *func, int line)
{
	...
	if (_fips_status & FIPS_ERROR_MASK) {
		flash_log_add_event(FE_LOG_FIPS_FAILURE, 0, NULL);
...
static void fips_sha256_kat(void *buf)
{
	struct sha256_kat *b = buf;
	...
	if (memcmp(HASH_final(&b->ctx), ans, SHA256_DIGEST_SIZE))
...
/* Test values from OpenSSL */
static void fips_hmac_sha256_kat(void *buf)
{
	struct hmac_kat *b = buf;
	...
	if (memcmp(DCRYPTO_HMAC_final(&b->ctx), ans, SHA256_DIGEST_SIZE))
...
static void fips_hmac_drbg_instantiate_kat(struct hmac_drbg_kat *b)
{
	...
	if (memcmp(b->ctx.v, V0, sizeof(V0)) ||
	    memcmp(b->ctx.k, K0, sizeof(K0)))
		fips_throw_err(FIPS_FATAL_HMAC_DRBG);
}

static void fips_hmac_drbg_reseed_kat(struct hmac_drbg_kat *b)
{
	/* Expected internal drbg state */
	...
	if (memcmp(b->ctx.v, V1, sizeof(V1)) ||
	    memcmp(b->ctx.k, K1, sizeof(K1)))
		fips_throw_err(FIPS_FATAL_HMAC_DRBG);
}

static void fips_hmac_drbg_generate_kat(struct hmac_drbg_kat *b)
{
	/* Expected internal drbg state */
	 ...
	 */
	hmac_drbg_generate(&b->ctx, b->buf, sizeof(b->buf), NULL, 0);
	if (memcmp(b->buf, KA, sizeof(KA)))
		fips_throw_err(FIPS_FATAL_HMAC_DRBG);
}

static void fips_hmac_drbg_kat(void *buf)
{
	struct hmac_drbg_kat *b = buf;

	fips_hmac_drbg_instantiate_kat(b);
	fips_hmac_drbg_reseed_kat(b);
	fips_hmac_drbg_generate_kat(b);
}
...
static void fips_ecdsa_verify_kat(void *buf)
{
	struct ecdsa_kat *b = buf;
...
#ifdef CR50_FIPS_RSA
static void fips_rsa2048_verify_kat(void *buf)
{
	struct rsa2048_kat *b = buf;
	...
	passed = DCRYPTO_rsa_verify(&rsa, digest, sizeof(b->digest), sig,
				    sizeof(sig), PADDING_MODE_PKCS1,
				    HASH_SHA256);
	if (passed) {
		fips_throw_err(FIPS_FATAL_RSA2048);
...
static void fips_aes256_kat(void *buf)
{
	struct aes256_kat *b = buf;
	...
	if (memcmp(b->enc, ans_aes128, AES_BLOCK_LEN)) {
		fips_throw_err(FIPS_FATAL_AES256);
		return;
	}
	...
	if (memcmp(kat_aes128_msg, b->dec, AES_BLOCK_LEN)) {
		fips_throw_err(FIPS_FATAL_AES256);
		return;
	}
}
 ...
 * fatal error.
 */
static int fips_power_up_tests(void)
{
	char *kat_buf;
	uint64_t starttime;
	int shmem;
	...
	if (EC_SUCCESS ==
	    shared_mem_acquire(sizeof(union kat_tests), &kat_buf)) {
		fips_sha256_kat(kat_buf);
		fips_hmac_sha256_kat(kat_buf);
		fips_ecdsa_verify_kat(kat_buf);
		fips_aes256_kat(kat_buf);
		fips_hmac_drbg_kat(kat_buf);
		...
		shared_mem_release(kat_buf);
	}
	fips_trng_startup();
	starttime = get_time().val - starttime;
	/**
	 * once FIPS power-up tests completed we can enable console output
	 * and allow AP to boot
	 */
	deassert_sys_rst();
	console_enable_output();
	_fips_status |= FIPS_POWER_UP_TEST_DONE;
	ccprints(
		"FIPS power-up tests completed in %llu, shmem before %d, shmem after %d",
		starttime, shmem, shared_mem_size());
	if (_fips_status & FIPS_ERROR_MASK)
