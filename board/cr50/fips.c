...
#define U2F_DATA_FLASH_OFFSET                              \
	(CONFIG_FLASH_LOG_BASE - CONFIG_FLASH_ERASE_SIZE - \
...
static bool is_flash_address(const void *ptr)
{
	...
	else if (my_addr >= CONFIG_RW_B_MEM_OFF &&
...
static bool erase_u2f(void)
{
	...
	flash_open_ro_window(U2F_DATA_FLASH_OFFSET, CONFIG_FLASH_ERASE_SIZE);
	rv = flash_physical_erase(U2F_DATA_FLASH_OFFSET,
...
/**
 * Zeroize all non-empty partitions with U2F keys. Eventhough there should be
 ...
 * @returns true, if successully loaded U2F keys into provided state parameter
 */
...
int u2f_origin_key(const uint8_t *seed, p256_int *d)
{
	...
	if (!app_hw_device_id(U2F_ORIGIN, tmp, tmp))
...
{
	LITE_HMAC_CTX ctx;
	struct fips_u2f_state *state = u2f_fips_get_state();
	 ...
	 */
	DCRYPTO_HMAC_SHA256_init(&ctx, state->hmac_key, SHA256_DIGEST_SIZE);
	...
	uint8_t version, struct u2f_versioned_key_handle *key_handle)
{
	...
	/**
	 * Construct public key handle as:
	 * origin_seed || HMAC(salt_kek, origin || user ||
	 *                     origin_seed || version)
	 * Later we can verify that given key handle was indeed created using
	 * this device's local salt_kek.
	 */
			    ...
			    p256_int *d, p256_int *pk_x, p256_int *pk_y)
{
	 ...
	 */
	hmac_drbg_init(&drbg, state->entropy, state->drbg_entropy_size,
...
int u2f_gen_kek(const uint8_t *origin, uint8_t *kek, size_t key_len)
{
	...
	if (!app_hw_device_id(U2F_WRAP, state->hmac_key, buf))
		return EC_ERROR_UNKNOWN;
...
int g2f_individual_keypair(p256_int *d, p256_int *pk_x, p256_int *pk_y)
{
	...
	if (fips_u2f_compliant()) {
		...
		/* Compute hw-bound additional data */
		if (!app_hw_device_id(U2F_ATTEST, state->hmac_key,
				      (uint32_t *)buf))
			return EC_ERROR_UNKNOWN;
		/**
	...
	if (!app_hw_device_id(U2F_ATTEST, state->g2f_entropy, (uint32_t *)buf))
		return EC_ERROR_UNKNOWN;
...
{
	return (u2f_fips_zeroize()) ? EC_SUCCESS : EC_ERROR_UNKNOWN;
}
