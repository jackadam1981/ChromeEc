				  ...
				  uint8_t *kh_auth_mac)
{
	 ...
	 */
	HMAC_SHA256_hw_init(&ctx, state->salt_kek, SHA256_DIGEST_SIZE);
	HMAC_SHA256_update(&ctx, kh->vkh.authorization_salt,
			   U2F_AUTHORIZATION_SALT_SIZE);
	HMAC_SHA256_update(&ctx, (uint8_t *)&kh->vkh.header,
			     ...
			     const uint8_t *origin)
{
	...
	result = DCRYPTO_equals(&recreated_kh_hmac, kh_hmac,
				sizeof(recreated_kh_hmac));
	always_memset(recreated_kh_hmac, 0, sizeof(recreated_kh_hmac));
	return result == 1;
}

bool u2f_authorize_keyhandle_with_secret(const struct u2f_state *state,
					 const union u2f_key_handle_variant *kh,
					 uint8_t kh_version,
					 const uint8_t *user,
					 const uint8_t *origin,
...
{
	/* Re-created key handle. */
	uint8_t recreated_kh_auth_hmac[SHA256_DIGEST_SIZE];
	int result;
	...
	if (kh_version == 0) {
		/**
		 * In case of function misuse for old key handles, fallback to
		 * regular authorization.
		 */
		return u2f_authorize_keyhandle(state, kh, kh_version, user,
					       origin);
	}

	u2f_authorization_mac(state, kh, authTimeSecretHash,
			      recreated_kh_auth_hmac);

	result = DCRYPTO_equals(&recreated_kh_auth_hmac,
				kh->vkh.authorization_hmac,
				sizeof(recreated_kh_auth_hmac));

	always_memset(recreated_kh_auth_hmac, 0,
		      sizeof(recreated_kh_auth_hmac));
	return result == 1;
}
...
static int cmd_u2f_test(int argc, char **argv)
{
	...
	ccprintf("u2f_authorize_keyhandle_with_secret - %s\n",
		 expect_bool(u2f_authorize_keyhandle_with_secret(
				     &state, &kh, 1, user, origin, authTime),
			     true));
