...
int u2f_origin_user_keyhandle(const uint8_t *origin, const uint8_t *user,
			      const uint8_t *origin_seed, int versioned,
			      uint8_t *key_handle)
{
	LITE_HMAC_CTX ctx;
	...
	if (!versioned)
		hmac_offset = P256_NBYTES;
	else
		hmac_offset = P256_NBYTES + U2F_KH_VERSION_SIZE;

	memcpy(key_handle, origin_seed, hmac_offset);
	...
	HASH_update(&ctx.hash, key_handle, hmac_offset);

	memcpy(key_handle + hmac_offset, DCRYPTO_HMAC_final(&ctx),
	       SHA256_DIGEST_SIZE);

	return EC_SUCCESS;
}

int u2f_origin_user_keypair(const uint8_t *key_handle, int versioned,
			    p256_int *d, p256_int *pk_x, p256_int *pk_y)
{
	...
	if (versioned) {
		hmac_drbg_generate(&drbg, key_seed, sizeof(key_seed),
				   key_handle,
				   P256_NBYTES * 2 + U2F_KH_VERSION_SIZE);
	} else {
		hmac_drbg_generate(&drbg, key_seed, sizeof(key_seed),
