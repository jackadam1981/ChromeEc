...
enum ec_error_list u2f_generate_state_keys(struct u2f_state *state)
{
	...
	result = u2f_generate_hmac_key(state);
...
static int cmd_u2f_test(int argc, char **argv)
{
	...
	ccprintf("u2f_generate_state_keys - %s\n",
		 expect_bool(u2f_generate_state_keys(&state), EC_SUCCESS));
