...
/* TODO(ngm): stub implementation that allows tests to pass. */
CRYPT_RESULT _cpri__GenerateKeyRSA(TPM2B *n, TPM2B *p, uint16_t key_bits,
				uint32_t e, TPM_ALG_ID hash_alg, TPM2B *seed,
				const char *label, TPM2B *extra,
				uint32_t *counter)
{
	/* Tests only generate 2048-bit keys. */
	if (key_bits != 2048 || e != 65537)
		return CRYPT_FAIL;

	n->size = RSA_2048_N.b.size;
	memcpy(n->buffer, RSA_2048_N.b.buffer, n->size);
	p->size = RSA_2048_P.b.size;
	memcpy(p->buffer, RSA_2048_P.b.buffer, p->size);
	return CRYPT_SUCCESS;
}
				...
				size_t *response_size_out)
{
	...
	case TEST_RSA_KEYGEN:
		if (_cpri__GenerateKeyRSA(&n.b, &p.b, key_len,
						65537, TPM_ALG_ERROR, NULL,
						NULL, NULL, NULL)
			!= CRYPT_SUCCESS ||
			n.b.size != RSA_2048_N.b.size ||
			memcmp(n.b.buffer, RSA_2048_N.b.buffer,
				n.b.size) != 0 ||
			p.b.size != RSA_2048_P.b.size ||
			memcmp(p.b.buffer, RSA_2048_P.b.buffer,
				p.b.size) != 0) {
			*response_size = 0;
		} else {
			*out = 1;
