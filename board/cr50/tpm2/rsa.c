			...
			TPM2B *N_buf, TPM2B *p_buf, TPM2B *q_buf)
{
	...
	/* Both primes must be provided as bn_div() is unimplemented. */
	if (!p_buf || !q_buf)
		return CRYPT_PARAMETER;

	if (N_buf->size < p_buf->size + q_buf->size)
		return CRYPT_PARAMETER;  /* Insufficient output buffer space. */
	if (d_buf->size < N_buf->size)
		return CRYPT_PARAMETER;  /* Insufficient output buffer space. */
	...
	if (DCRYPTO_rsa_key_compute(&N, &d, &p, &q, e))
		return CRYPT_SUCCESS;
	else
		return CRYPT_FAIL;
}
