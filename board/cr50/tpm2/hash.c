...
/* Various errors reported by the test. */
enum {
	/* Attempt to start a session on an active handle. */
	DUPLICATED_HANDLE = 1,
	TOO_MANY_HANDLES = 2,  /* No room to allocate a new context. */
	UNKNOWN_CONTEXT = 3    /* Continuation/finish on unknown context. */
};
			     ...
			     void *response_body, size_t *response_size)
{
	...
	if (!context) {
		*((uint8_t *)response_body) = UNKNOWN_CONTEXT;
		*response_size = 1;
		return;
	}
