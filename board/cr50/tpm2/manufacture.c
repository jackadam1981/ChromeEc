...
#define INFO1_MANUFACTURE_SIZE             512
#define INFO1_SENTINEL_SIZE                4
#define INFO1_EPS_SIZE                     PRIMARY_SEED_SIZE

/* TODO(ngm): figure out write offset. */
#define INFO1_MANUFACTURE_STATE_OFFSET (FLASH_INFO_SIZE - \
						INFO1_MANUFACTURE_SIZE)
#define INFO1_SENTINEL_OFFSET INFO1_MANUFACTURE_STATE_OFFSET
#define INFO1_EPS_OFFSET (INFO1_SENTINEL_OFFSET + INFO1_SENTINEL_SIZE)
		...
		const struct cros_perso_certificate_response_v0 *cert)
{
	...
	if (component_type == CROS_PERSO_COMPONENT_TYPE_RSA_CERT)
		in.nvIndex = ek_nv_index_0;
	else   /* P256 certificate. */
		in.nvIndex = ek_nv_index_1;
...
static int decrypt_and_copy_eps(void)
{
	...
	for (i = 0; i < INFO1_EPS_SIZE / sizeof(uint32_t); i++) {
		...
		if (flash_info_read(INFO1_EPS_OFFSET + i, &word))
...
static void manufacture_complete(void)
{
	...
	for (i = 0; i < INFO1_MANUFACTURE_SIZE; i += sizeof(erase))
		flash_info_physical_write(
			INFO1_MANUFACTURE_STATE_OFFSET + i, sizeof(erase),
			(unsigned char *) &erase);

	/* TODO(ngm): lock HIK export. */
	/* TODO(ngm): lock writing to INFO1? */
}
...
int tpm_manufactured(void)
{
	...
	flash_info_read(INFO1_SENTINEL_OFFSET / sizeof(uint32_t),
			&sentinel);
	return sentinel == INFO1_SENTINEL_MANUFACTURE_DONE;
}

static void ack_command_handler(void *request,
				size_t command_size,
...
static int rsa_cert_done = 0;
static int p256_cert_done = 0;

static void perso_command_handler(void *request,
				  size_t command_size,
				  size_t *response_size)
{
	const struct cros_perso_response_v0  *perso_response = request;
	struct cros_perso_ok_response_v0 *ok_response =
		(struct cros_perso_ok_response_v0 *) perso_response;
	...
	*response_size = 0;
	...
	if (!validate_cert(&perso_response->cert_info, &perso_response->cert))
		return;  /* Invalid cert. */
	...
	/* TODO: generate RSA and ECC keys, and verify matching cert. */
	/* Mark as manufactured. */
	manufacture_complete();

	*response_size = sizeof(*ok_response);
	ok_response->ok = RESPONSE_OK;
}

DECLARE_EXTENSION_COMMAND(EXTENSION_ACK, ack_command_handler);
DECLARE_EXTENSION_COMMAND(EXTENSION_PERSO, perso_command_handler);
