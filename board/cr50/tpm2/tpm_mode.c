...
#define PWRDN_SCRATCH_TPM_REG		GREG32(PMU, PWRDN_SCRATCH20)
#define PWRDN_SCRATCH_TPM_MODE_MASK	0x03
...
{
	tpm_stop();
	PWRDN_SCRATCH_TPM_REG =
		(PWRDN_SCRATCH_TPM_REG & ~PWRDN_SCRATCH_TPM_MODE_MASK)
		| (TPM_MODE_DISABLED & PWRDN_SCRATCH_TPM_MODE_MASK);
	nvmem_clear_cache();
...
{
	return (PWRDN_SCRATCH_TPM_REG & PWRDN_SCRATCH_TPM_MODE_MASK) ==
		TPM_MODE_DISABLED;
}
...
void print_tpm_mode(void)
{
	ccprintf("TPM MODE:    %s (%d)\n",
		...
		s_tpm_mode);
	ccprintf("             and used to be %s\n",
		text_abilities[!tpm_was_disabled_before()]);
