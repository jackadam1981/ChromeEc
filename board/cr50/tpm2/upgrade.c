 ...
 * misformed, a protocol error is returned.
 *
 * If there was no errors, the number of restored headers is returned to the
 * host in a single byte. A nonzero return value indicates that the reset is
 * going to happen after the timeout, unless the timeout value is zero.
 */
