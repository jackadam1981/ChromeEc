...
BUILD_ASSERT(RO_CERTS_START_ADDR == 0x43800);
...
static uint32_t hw_key_ladder_step(uint32_t cert)
{
	...
	/* Here and below, do full register writes to clear set bits if any. */
...
int tpm_endorse(void)
{
	 ...
	 */
	const uint8_t *ro_certs_start_addr =
		(const uint8_t *) RO_CERTS_START_ADDR;
	const uint32_t ro_certs_start_word =
		*((const uint32_t *) RO_CERTS_START_ADDR);
	...
	if (ro_certs_start_word == 0xFFFFFFFF)
		return 0;

	/* Unpack rsa cert struct. */
	rsa_cert = (const struct ro_cert *) ro_certs_start_addr;
	...
	ecc_cert = (const struct ro_cert *) (
		ro_certs_start_addr + sizeof(struct ro_cert) +
