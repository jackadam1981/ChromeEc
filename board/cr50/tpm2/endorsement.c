...
enum manufacturing_status tpm_endorse(void)
{
	 ...
	 * Do it early, so that EPS will be available for TPM2 even if no certs
	 * are present (dev mode with no fixed cert used).
	 */
	if (!store_eps(eps)) {
	...
	/* Certs not yet installed (first boot) or destroyed by spiflash. */
	if (*c == 0xFFFFFFFF)
		return mnf_no_certs;
