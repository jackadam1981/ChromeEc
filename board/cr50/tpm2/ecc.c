		    ...
		    uint8_t *out, size_t out_len)
{
	...
	*len = 0;
	...
	if (*cmd_size < sizeof(l))
		return 0;
	*cmd_size -= sizeof(l);
	l = ((uint16_t)((*ptr)[0] << 8)) | (*ptr)[1];
	*ptr += 2;
	if (*cmd_size < l || l > out_len) {
		*cmd_size = 0;
		ccprintf(
			"%s error: length = %d, data available = %d, out_len = %d\n",
			__func__, l, *cmd_size, out_len);
		return 0;
	}
	*len = l;
	memcpy(out, *ptr, l);
			...
			size_t *response_size_out)
{
	...
	*response_size = 0;
	...
	if (op == TEST_SIGN || op == TEST_SIGN_ANY) {
		if (!parse_2b(&cmd, &cmd_size, &in_len, in, sizeof(in)))
			return;
