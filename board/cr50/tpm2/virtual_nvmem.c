...
// Used to check if indices are virtual.
#define VIRTUAL_NV_INDEX_MASK (~(VIRTUAL_NV_INDEX_MAX - VIRTUAL_NV_INDEX_START))
...
BUILD_ASSERT(VIRTUAL_NV_INDEX_END <= VIRTUAL_NV_INDEX_MAX);
...
{
	NV_INDEX *index = (NV_INDEX *) dest;
	const NV_INDEX nv_index_template = NV_INDEX_TEMPLATE;

	memcpy(index, &nv_index_template, sizeof(NV_INDEX));
...
{
	return (handle & VIRTUAL_NV_INDEX_MASK) == VIRTUAL_NV_INDEX_START;
}
				...
				void *data)
{
	...
	// necessary.
	if (size > 0) {
		nvIndexConfig.get_data_fn((BYTE *)data + copied,
...
static void GetBoardId(BYTE *to, size_t offset, size_t size)
{
	...
	memcpy(to + offset, &board_id_tmp, size);
}
