 ...
 */
static void wp_ccd_config_changed(void)
{
	static int called_already;

	/*
	 * We only need to set the WP state after the CCD config is first
	 * loaded at boot, to see if we're overriding it.  Other CCD changes
	 * after boot don't affect the WP state.
	 */
	if (called_already)
		return;
	called_already = 1;
	...
	if ((system_get_reset_flags() & RESET_FLAG_HIBERNATE) &&
		...
		/* Reset to WP based on battery-present */
		force_write_protect(0, -1);
	}
}
DECLARE_HOOK(HOOK_CCD_CHANGE, wp_ccd_config_changed, HOOK_PRIO_DEFAULT);
