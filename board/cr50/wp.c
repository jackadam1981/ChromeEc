...
static int command_bpforce(int argc, char **argv)
{
	if (argc > 1) {
		...
		if (argc > 2 && !strcasecmp(argv[2], "atboot")) {
			...
			ccd_set_flag(CCD_FLAG_OVERRIDE_BATT_STATE_DISCONNECT,
				bp_disconnect);
		}

		/* Update the WP state based on battery presence the changes */
	...
	if (ccd_get_flag(CCD_FLAG_OVERRIDE_BATT_AT_BOOT))
		ccprintf("override to %sconnect\n",
			 ccd_get_flag(CCD_FLAG_OVERRIDE_BATT_STATE_DISCONNECT)
			 ? "dis" : "");
	else
