...
int board_wipe_tpm(int reset_required)
{
	...
	/*
	 * Disable all TPM communications. Asserting EC_RST_L may not hold
	 * the AP in reset. Use tpm_stop to guarantee the AP isn't using the
	 * TPM while it's being wiped.
	 */
	tpm_stop();
	 ...
	 * from the AP until next reboot. Asserting EC_RST_L above may not
	 * guarantee the AP is being held in reset. Disable TPM communications
	 * until board_reboot_ec, so we guarantee the system has reset after
	 * wiping the TPM. If we aren't resetting the system now, TPM will stay
	 * disabled until the user resets the system.
	 */
	tpm_stop();
