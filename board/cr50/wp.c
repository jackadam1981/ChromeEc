...
{
	uint8_t nv_console_lock_state, key, key_len, val;
	const struct tuple *t;
	int rv;

	CPRINTS("%s(%d)", __func__, lock_state);
	...
	key_len = 1;
	t = getvar((const uint8_t *)&key, key_len);
	if (t == NULL) {
...
static void unlock_the_console(void)
{
	...
	task_set_event(TASK_ID_TPM, TASK_EVENT_CUSTOM(1 << 1), 0);
...
static void init_console_lock_and_wp(void)
{
	...
	if (t == NULL) {
		...
		set_console_lock_state((int)lock_state);
	}
