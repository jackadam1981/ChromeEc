...
void fail_fips(void)
{
	GREG32(PMU, PWRDN_SCRATCH22) = 0xBEBEBEBE;
}

#define FWMP_SPACE_SIZE 40
void check_fwmp(void)
{
	char fwmp[FWMP_SPACE_SIZE];
	enum tpm_read_rv rv;
	int ccd_unlock_allowed;
	int policy_update_allowed;

	rv = read_tpm_nvmem(FWMP_NV_INDEX, FWMP_SPACE_SIZE, fwmp);
	ccd_unlock_allowed = board_fwmp_allows_unlock();
	policy_update_allowed = board_fwmp_allows_boot_policy_update();

	ccprintf("FWMP read result: %d\n"
			"  ccd unlock allowed: %d\n"
			"  policy update allowed: %d\n",
			rv, ccd_unlock_allowed, policy_update_allowed);
}
