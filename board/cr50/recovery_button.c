...
/*
 * The recovery button on some systmes is wired to KEY0 in rbox.  For testing,
 * we need to be able override the value.  We'll have a vendor command such that
 * the AP can query the state of the recovery button.  However, the reported
 * state can only be overridden with a console command given sufficient
 * privileges.
 */
