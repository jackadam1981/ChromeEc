...
static void set_ap_uart_off(void)
{
	...
	/*
	 * If Cr50 isn't using TPM_RST_L it relies on the AP uart to detect the
	 * ap state. Call set_ap_off update the AP state.
	 */
...
static void set_ap_uart_on_deferred(void)
{
	...
	/*
	 * If Cr50 isn't using TPM_RST_L it relies on the AP uart to detect the
	 * ap state. Call set_ap_on update the AP state.
	 */
	if (!board_detect_ap_with_tpm_rst())
		set_ap_on();
