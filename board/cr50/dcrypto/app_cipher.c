 ...
 * get maximum throughput try to load it at least at 50%
 */
size_t __optimize("O")
	dcrypto_aes_process(uint32_t **out, const uint32_t **in, size_t len)
{
	...
	if (len >= 16) {
		...
		while (reg_aes->rfifo_empty)
			;
		outw[0] = reg_aes->rfifo_data;
		outw[1] = reg_aes->rfifo_data;
		outw[2] = reg_aes->rfifo_data;
		outw[3] = reg_aes->rfifo_data;
		outw += 4;
	}
	*in = inw;
	*out = outw;
