...
static void
dcrypto_sha_wait(uint32_t *digest, size_t digest_words)
{
	size_t i;
	...
	GREG32(KEYMGR, SHA_TRIG) = GC_KEYMGR_SHA_TRIG_TRIG_RESET_MASK;
}
...
static void dcrypto_load_hmac_key(uint32_t *hmac_key)
{
	...
	GREG32(KEYMGR, SHA_USE_HIDDEN_KEY) = 0;
	GREG32(KEYMGR, SHA_USE_CERT) = 0;

	/* Only SHA2-256 is supported for HMAC. */
	for (i = 0; i < SHA256_DIGEST_WORDS; ++i)
		*(GREG32_ADDR(KEYMGR, SHA_KEY_W0) + i) = hmac_key[i];
}
 ...
 */
static void dcrypto_sha_start(enum sha_cfg mode, enum keymgr_stall_cfg stall)
{
	/* Clear interrupt status. */
	GREG32(KEYMGR, SHA_ITOP) = 0;
	/* Turn off random nops (which are enabled by default) to reprogram. */
	GREG32(KEYMGR, SHA_RAND_STALL_CTL) = 0;

	/* Configure and enable random nop percentage. */
	GREG32(KEYMGR, SHA_RAND_STALL_CTL) = stall;

	GREG32(KEYMGR, SHA_CFG_EN) = mode;
...
static const union sha_digests *dcrypto_hmac_final(union hmac_ctx *const ctx)
{
	...
	for (i = 0; i < SHA256_DIGEST_WORDS; ++i)
		*(GREG32_ADDR(KEYMGR, SHA_KEY_W0) + i) = 0;
	dcrypto_release_sha_hw();
...
static void dcrypto_sha1_init(union hash_ctx *ctx)
{
	...
	ctx->f = &hw_sha1_vtab;
	GREG32(KEYMGR, SHA_TRIG) = GC_KEYMGR_SHA_TRIG_TRIG_RESET_MASK;
	dcrypto_sha_start(MODE_SHA1_LIVESTREAM, KEYMGR_STALL_6);
}
...
static void dcrypto_sha256_init(union hash_ctx *ctx)
{
	...
	GREG32(KEYMGR, SHA_TRIG) = GC_KEYMGR_SHA_TRIG_TRIG_RESET_MASK;
	dcrypto_sha_start(MODE_SHA256_LIVESTREAM, KEYMGR_STALL_6);
}
				...
				uint32_t *digest, size_t digest_words)
{
	...
	if (n != 0) { /* One-shot only works for non-empty data. */
		...
		dcrypto_sha_start(mode, KEYMGR_STALL_6);

		dcrypto_sha_fifo_load(data, n);
	} else /* Due to bug in SHA engine, force livestream for this case. */
		dcrypto_sha_start(mode | GC_KEYMGR_SHA_CFG_EN_LIVESTREAM_MASK,
				  KEYMGR_STALL_6);
			 ...
			 size_t len)
{
	...
	dcrypto_sha_start(MODE_HMAC_SHA256_LIVESTREAM, KEYMGR_STALL_25);
...
#if defined(SHA1_TEST) || defined(SHA256_TEST) || defined(HMAC_SHA256_TEST)
...
DECLARE_SAFE_CONSOLE_COMMAND(hmac, cmd_hmac, NULL, NULL);
#endif
