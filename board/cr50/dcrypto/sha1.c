...
static const uint8_t *SHA1_final_as_hash(union hash_ctx *const ctx)
...
void SHA1_sw_init(struct sha1_ctx *const ctx)
{
	...
	static const uint32_t sha1_init[5] = { 0x67452301, 0xEFCDAB89,
			    ...
			    uint8_t digest[SHA1_DIGEST_SIZE])
{
	...
	memcpy(digest, SHA1_sw_final(&ctx), SHA1_DIGEST_SIZE);
	return digest;
}
