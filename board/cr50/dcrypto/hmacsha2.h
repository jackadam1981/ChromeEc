 ...
 */
union hash_ctx; /* forward declaration of generic hash type */
union hmac_ctx; /* forward declaration of generic HMAC type */
...
struct hash_vtable {
	...
	const uint8_t *(*const final)(union hash_ctx *const);

	/* HW HMAC support may require special ending. */
	const uint8_t *(*const hmac_final)(union hmac_ctx *const);
...
struct sha256_ctx {
	...
	uint32_t state[8]; /* up to SHA2-256 */
	union {
...
struct sha512_ctx {
	const struct hash_vtable *f; /* metadata & vtable. */
	size_t count; /* number of bytes processed. */
	uint64_t state[8]; /* up to SHA2-512. */
	union {
 ...
 * Will crash if previously not condifured!
 */
 ...
 */
#ifndef CONFIG_DCRYPTO_MOCK
static inline const uint8_t *HASH_final(union hash_ctx *const ctx)
{
	return ctx->f->final(ctx);
}
#else
const uint8_t *HASH_final(union hash_ctx *const ctx);
#endif

static inline const uint8_t *SHA1_final(struct sha1_ctx *const ctx)
{
	return ctx->f->final((union hash_ctx *)ctx);
}

static inline const uint8_t *SHA256_final(struct sha256_ctx *const ctx)
{
	return ctx->f->final((union hash_ctx *)ctx);
}
...
const uint8_t *SHA1_sw_final(struct sha1_ctx *const ctx);
const uint8_t *SHA1_sw_hash(const void *data, size_t len,
			    uint8_t digest[SHA1_DIGEST_SIZE]);
void SHA256_sw_init(struct sha256_ctx *const ctx);
void SHA256_sw_update(struct sha256_ctx *const ctx, const void *data,
		      size_t len);
const uint8_t *SHA256_sw_final(struct sha256_ctx *const ctx);
const uint8_t *SHA256_sw_hash(const void *data, size_t len,
			      uint8_t digest[SHA256_DIGEST_SIZE]);
void SHA224_sw_init(struct sha224_ctx *const ctx);
void SHA224_sw_update(struct sha224_ctx *const ctx, const void *data,
		      size_t len);
const uint8_t *SHA224_sw_final(struct sha224_ctx *const ctx);
const uint8_t *SHA224_sw_hash(const void *data, size_t len,
...
void SHA384_sw_update(struct sha384_ctx *const ctx, const void *data,
		      size_t len);
const uint8_t *SHA384_sw_final(struct sha384_ctx *const ctx);
const uint8_t *SHA384_sw_hash(const void *data, size_t len,
			      uint8_t digest[SHA384_DIGEST_SIZE]);
void SHA512_sw_init(struct sha512_ctx *const ctx);
void SHA512_sw_update(struct sha512_ctx *const ctx, const void *data,
		      size_t len);
const uint8_t *SHA512_sw_final(struct sha512_ctx *ctx);
const uint8_t *SHA512_sw_hash(const void *data, size_t len,
...
const uint8_t *HMAC_sw_final(union hmac_ctx *const ctx);
...
static inline const uint8_t *HMAC_final(union hmac_ctx *const ctx)
{
	return ctx->f->hmac_final(ctx);
}

/**
 * HMAC SHA1 initalization.
 */
...
static inline const uint8_t *HMAC_SHA1_final(struct hmac_sha1_ctx *const ctx)
{
	return ctx->hash.f->hmac_final((union hmac_ctx *)ctx);
}

/**
 * HMAC SHA2-224 initalization.
 */
...
static inline const uint8_t *HMAC_SHA256_final(struct hmac_sha256_ctx *ctx)
{
	return ctx->hash.f->hmac_final((union hmac_ctx *)ctx);
}
...
static inline const uint8_t *HMAC_SHA384_final(struct hmac_sha384_ctx *ctx)
{
	return ctx->hash.f->hmac_final((union hmac_ctx *)ctx);
}
/**
...
static inline const uint8_t *HMAC_SHA512_final(struct hmac_sha512_ctx *ctx)
{
	return ctx->hash.f->hmac_final((union hmac_ctx *)ctx);
}
#endif
