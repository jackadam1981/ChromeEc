...
static const uint8_t *SHA512_final_as_hash(union hash_ctx *const ctx)
...
void SHA512_sw_init(struct sha512_ctx *const ctx)
{
	...
	memcpy(ctx->state, sha512_init, sizeof(sha512_init));
	ctx->f = &sha512_vtable;
	ctx->count = 0;
}

/* SHA2-384 and SHA2-512 use same internal structure */
...
const uint8_t *SHA512_sw_final(struct sha512_ctx *const ctx)
{
			      ...
			      uint8_t digest[SHA512_DIGEST_SIZE])
{
	...
	memcpy(digest, SHA512_sw_final(&ctx), SHA512_DIGEST_SIZE);
	return digest;
}
...
void SHA384_sw_init(struct sha384_ctx *ctx)
{
	...
	memcpy(ctx->state, sha384_init, sizeof(sha384_init));
	ctx->count = 0;
	ctx->f = &sha384_vtable;
}
			      ...
			      uint8_t digest[SHA384_DIGEST_SIZE])
{
	...
	memcpy(digest, SHA384_sw_final(&ctx), SHA384_DIGEST_SIZE);
	return digest;
}

void DCRYPTO_SHA512_init(struct sha512_ctx *ctx)
{
	SHA512_sw_init(ctx);
}
...
const uint8_t *DCRYPTO_SHA512_hash(const void *data, uint32_t n,
				   uint8_t *digest)
{
	return SHA512_sw_hash(data, n, digest);
}

void DCRYPTO_SHA384_init(struct sha384_ctx *ctx)
{
	SHA384_sw_init(ctx);
}
...
const uint8_t *DCRYPTO_SHA384_hash(const void *data, uint32_t n,
				   uint8_t *digest)
{
	return SHA384_sw_hash(data, n, digest);
}
