...
#define ENTROPY_SIZE_WORDS ((ENTROPY_SIZE_BITS + 31) / 32)
...
static struct {
	...
	uint8_t apt_initialized; /* flag APT window is filled with data */
	uint8_t oldest; /* position in APT window */
	uint8_t drbg_initialized;
} rand_state;

/**
 * NIST SP 800-90B 4.4.1
 * The repitition count test detects abnormal runs of 0s or 1s.
 ...
 */
bool repetition_count_test(uint32_t rnd)
{
	...
	rand_state.last_clz = clz + ((!!rnd - 1) & rand_state.last_clz);
	rand_state.last_clo = clo + ((!!~rnd - 1) & rand_state.last_clo);
...
bool adaptive_proportion_test(uint32_t rnd)
{
	...
	if (++rand_state.oldest >= APT_WINDOW_SIZE_NWORDS) {
		rand_state.apt_initialized = 1; /* saw full window */
		rand_state.oldest = 0;
	}
...
static struct rand_result fips_trng32(int power_up)
{
	...
	if (!fips_crypto_allowed() ||
	    (board_fips_enforced() && !power_up && !fips_powerup_passed()))
...
bool fips_trng_startup(int stage)
{
	...
	for (uint32_t i = 0; i < (TRNG_INIT_WORDS) / 2; i++) {
		...
		entropy_fifo[i & (ENTROPY_SIZE_WORDS - 1)] = r.random_value;
	}
	return true;
}
