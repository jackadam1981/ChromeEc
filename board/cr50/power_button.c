...
static bool ap_ro_validation_failed_;

DECLARE_DEFERRED(keep_ec_in_reset);

static void keep_ec_in_reset(void)
{
	if (!ap_ro_validation_failed_)
		return;
...
static int val_state_cmd(int argc, char **argv)
{
#ifdef CR50_DEV
	...
	if (argc == max_args) {
		...
		if (ap_ro_validation_failed_) {
			ap_ro_validation_failed_ = false;
			deassert_ec_rst();
		}
	}
#endif
	ccprintf("%sAP RO validation failure detected\n",
		 ap_ro_validation_failed_ ? "" : "NO ");

	return EC_SUCCESS;
}
DECLARE_SAFE_CONSOLE_COMMAND(val_state, val_state_cmd,
#ifdef CR50_DEV
			     "[clear]",
