...
void power_button_release_interrupt_enable(int enable)
{
...
static void power_button_release_handler(void)
{
	...
	deassert_ec_rst();
	/* deassert_ec_rst() calls power_button_release_interrupt_disable(),
	 * which is intended. Note that this is for one-time use through this
	 * power on.
	 */
}
