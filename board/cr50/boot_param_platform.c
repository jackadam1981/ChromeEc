...
const char *g_owner_data_var_name = "BPO";
...
static inline void invalidate_ap_boot_config(void)
{
	...
	if (setvar(g_auth_seed_var_name, strlen(g_auth_seed_var_name),
		   NULL, 0) != 0) {
...
static inline bool get_hidden_owner_data(uint8_t owner_data[DIGEST_BYTES])
{
	...
	var = getvar(g_owner_data_var_name, strlen(g_owner_data_var_name));
	if (var) {
	...
	if (setvar(g_owner_data_var_name, strlen(g_owner_data_var_name),
