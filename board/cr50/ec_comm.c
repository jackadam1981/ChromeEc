 ...
 */
static void transfer_response_to_ec_(const enum cr50_comm_err response_code)
{
...
{
	enum cr50_comm_err response = CR50_COMM_SUCCESS;
	...
	case PHASE_RECEIVING_HEADER:
		...
		if (ec_comm_ctx.ph.struct_version != CR50_COMM_PACKET_VERSION) {
			response = CR50_COMM_ERR_STRUCT_VERSION;
			break;
		}
