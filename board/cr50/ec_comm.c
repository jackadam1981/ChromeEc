...
static uint16_t decode_packet_(const struct cr50_comm_packet *ph, int bytes)
{
	...
	CPRINTS("header: %ph", HEX_BUF((uint8_t *)ec_comm_ctx.packet,
		sizeof(struct cr50_comm_packet)));
	CPRINTS("body  : %ph", HEX_BUF((uint8_t *)ec_comm_ctx.packet +
		sizeof(struct cr50_comm_packet), CR50_COMM_MAX_DATA_SIZE));
#endif
