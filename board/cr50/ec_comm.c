...
int ec_comm_process_packet(uint8_t ch)
{
	...
	case PHASE_RECEIVING_HEADER:
		...
		if (ec_comm_ctx.ph.magic != CR50_PACKET_MAGIC) {
			transfer_response_to_ec_(CR50_COMM_ERR_MAGIC);
			break;
		}
		...
		if (ec_comm_ctx.ph.struct_version != CR50_COMM_PACKET_VERSION) {
			transfer_response_to_ec_(CR50_COMM_ERR_STRUCT_VERSION);
			break;
		}

		if (ec_comm_ctx.ph.size == 0) {
			/* Data size zero, then process the packet now. */
			transfer_response_to_ec_(decode_packet_(
			...
			transfer_response_to_ec_(CR50_COMM_ERR_SIZE);
		} else {
