...
#ifdef CR50_RELAXED
	CPRINTS("decoding a packet: %ph", HEX_BUF((uint8_t *)ec_comm_ctx.packet,
...
{
	struct cr50_comm_packet resp_packet;
	uint8_t *ptr_packet = (uint8_t *)&resp_packet;
	int uart = ec_comm_ctx.uart;
	int resp_size = offsetof(struct cr50_comm_packet, data);
	const int offset_cmd = offsetof(struct cr50_comm_packet, cmd);

	/* Let's response to EC */
	CPRINTS("EC-CR50 packet response: 0x%02x", response);

	resp_packet.magic = CR50_COMM_MAGIC_WORD;
	resp_packet.cmd = response;
	resp_packet.size = 0;
	resp_packet.crc = crc8((const uint8_t *)&resp_packet.cmd,
				resp_size - offset_cmd);

	/* Send the response to EC. */
	while (resp_size--) {
		uartn_write_char(uart, *ptr_packet);
		ptr_packet++;
	}
...
void ec_comm_packet_mode_dis(enum gpio_signal signal)
{
	 ...
	 * Let's transfer any blocked console input data if
	 * CCD_ENABLE_UART_EC_TX is set.
	 */
