...
static uint8_t decode_packet_(struct ec_packet_context_ *ctx)
{
	...
	if (!check_command_validity_(ctx->ph.cmd,
	...
	case CR50_COMM_CMD_SET_BOOT_MODE:
		...
		CPRINTS("EC boot_mode: 0x%02x -> 0x%02x",
			ec_efs_ctx.scratch.b.boot_mode, boot_mode);
		ec_efs_ctx.scratch.b.boot_mode = boot_mode;
		break;
	...
	case CR50_COMM_CMD_VERIFY_HASH:
		...
		ec_efs_ctx.scratch.b.boot_mode = EC_EFS_BOOT_MODE_NORMAL;
		break;
...
static void transfer_response_to_ec_(uint8_t response)
{
	...
	/*
	 * Connect DIOB3 to UART2_TX so that Cr50 can send the response
	 * to EC. Must disable DIOB3 interrupt.
	 */
	gpio_disable_interrupt(GPIO_EC_PACKET_MODE_EN);
	gpio_disable_interrupt(GPIO_EC_PACKET_MODE_DIS);
	GWRITE(PINMUX, DIOB3_SEL, GC_PINMUX_UART2_TX_SEL);
	...
	/*
	 * Disconnect DIOB3 from UART2_TX, and re-enable its interrupt.
	 */
...
{
	return ec_comm_is_enabled() && (ec_packet_ctx.uart == UART_EC);
}
...
void ec_comm_configure_wakepin(void)
{
	...
	GWRITE_FIELD(PINMUX, EXITEN0,   DIOB3, 0); /* disable powerdown exit */
	GWRITE_FIELD(PINMUX, EXITEDGE0, DIOB3, 1); /* edge triggered */
	GWRITE_FIELD(PINMUX, EXITINV0,  DIOB3, 0); /* wake on rising */
	GWRITE_FIELD(PINMUX, EXITEN0,   DIOB3, 1); /* enable powerdown exit */
...
void ec_comm_init(void)
{
	...
	CPRINTS("EC boot_mode: -> 0x%02x", ec_efs_ctx.scratch.b.boot_mode);
