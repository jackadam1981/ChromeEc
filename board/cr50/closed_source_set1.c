 ...
 */
#define FACTORY_MODE		GPIO_I2C_SCL_INA
#define CHROME_SEL		GPIO_EN_PP3300_INA_L
#define EXIT_FACTORY_MODE	GPIO_I2C_SDA_INA
...
void closed_source_set1_configure_gpios(void)
{
	...
	GWRITE_FIELD(PINMUX, DIOB0_CTL, DS, 3);
	GWRITE_FIELD(PINMUX, DIOB7_CTL, DS, 3);

	/* Enable pull-up on the B1 pad (I2C_SDA_SCL/EXIT_FACTORY_MODE) */
	 ...
	 */
	gpio_set_flags(EXIT_FACTORY_MODE, GPIO_INPUT | GPIO_INT_F_FALLING);
...
static void closed_source_set1_update_ec_trust_level(enum ec_trust_level tl)
{
	...
	case EC_TL_FACTORY_MODE:
		 ...
		 * simultaneously.
		 */
		gpio_set_flags(CHROME_SEL, GPIO_OUT_LOW);
		gpio_set_flags(FACTORY_MODE, GPIO_OUT_HIGH);
		break;
	...
	case EC_TL_DIAGNOSTIC_MODE:
		...
		gpio_set_flags(CHROME_SEL, GPIO_OUT_LOW);
		gpio_set_flags(FACTORY_MODE, GPIO_OUT_LOW);
		break;
	...
	case EC_TL_COREBOOT:
		 ...
		 */
		gpio_set_flags(FACTORY_MODE, GPIO_OUT_LOW);
		gpio_set_flags(CHROME_SEL, GPIO_OUT_HIGH);
		break;
	default:
...
static int ec_trust_level(int argc, char **argv)
{
	...
	ccprintf("FACTORY_MODE      = %d\n",
		gpio_get_level(FACTORY_MODE));
	ccprintf("CHROME_SEL        = %d\n",
		gpio_get_level(CHROME_SEL));
	ccprintf("EXIT_FACTORY_MODE = %d\n",
		gpio_get_level(EXIT_FACTORY_MODE));

	return 0;
}
DECLARE_CONSOLE_COMMAND(ectrust, ec_trust_level,
	"[0|1|2]",
