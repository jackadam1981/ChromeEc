...
/*
 * Registered function that should be called when INT_AP_L is enabled or
 * disabled respectively.
 */
static void (*interface_func_enable_)(void);
static void (*interface_func_disable_)(void);
...
void int_ap_register(void (*func_enable)(void), void (*func_disable)(void))
{
	interface_func_enable_ = func_enable;
	interface_func_disable_ = func_disable;
}

void int_ap_extension_enable(void)
{
	/*
	 * TODO(b148691139/): Just return if TPM_BOARD_CFG does not have
	 * INT_AP_L pulse extension enabled.
	 *
	...
	/* Initialize the status on a delayed assertion request. */
	assertion_delayed_ = false;
	disable_timer_();
...
void int_ap_extension_disable(void)
{
	...
	if (interface_func_disable_)
		interface_func_disable_();
