...
const struct strap_desc strap_regs[NUM_STRAP_PINS] = {
	...
	{
		GPIO_STRAP_A0,
		GOFFSET(PINMUX, DIOA1_SEL),
		GC_PINMUX_DIOA1_SEL,
	},
	/* Strap A1 -> DIOA9 */
	{
		GPIO_STRAP_A1,
		GOFFSET(PINMUX, DIOA9_SEL),
		GC_PINMUX_DIOA9_SEL,
	},
	/* Strap B0 -> DIOA6 */
	{
		GPIO_STRAP_B0,
		GOFFSET(PINMUX, DIOA6_SEL),
		GC_PINMUX_DIOA6_SEL,
	},
	/* Strap B1 -> DIOA12 */
	{
		GPIO_STRAP_B1,
		GOFFSET(PINMUX, DIOA12_SEL),
...
{
	/*
	 * This function duplicates some of the functionality in chip/g/gpio.c
	 * in order to configure a given strap pin to be either a low gpio
	 * output, a gpio input with or without an internal pull resistor, or
	 * disconnect the gpio signal from the pin pad.
	 *
	 * The desired gpio functionality is contained in the input parameter
	 * flags, while the strap parameter is an index into the array
	 * strap_regs.
	 */
...
static void init_board_properties(void)
{
	...
	if (!properties || (system_get_reset_flags() & RESET_FLAG_HARD)) {
		...
		if (get_strap_config(&config) != EC_SUCCESS) {
			...
			properties = BOARD_PORPERTIES_DEFAULT;
			goto save_properties;
		}
