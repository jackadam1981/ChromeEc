...
{
	/*
	 * SYS_RST_L is pseudo open drain. We only drive it low when asserting
	 * SYS_RST_L. It is an input when it's deasserted. Check if the output
	 * is enabled to tell if cr50 is asserting SYS_RST_L.
	 */
	ASSERT(GREAD(PINMUX, DIOM0_SEL) == GC_PINMUX_GPIO0_GPIO4_SEL);
	return GR_GPIO_SETDOUTEN(0) & (1 << 4);
}
