...
int get_strap_config(uint16_t *config)
{
	 ...
	 *      and s0 = DI0A6. Else check for high on DIOA12|DIOA6. If true,
	 *      then s1 = DIOA9, s0 = DIOA1.
	 *   3. For s1 and s0 do the following:
	 *      a. b1 = gpio_get_level(pin), where pin = s0 or s1
	...
	/* Check for A or B pair being used. */
	for (s0 = a0; s0 < NUM_STRAP_PINS; s0++)
	...
	*config = 0;
	...
	/* Find s1|s0 state based on pull up/dn and strength. */
	for (i = 0; i < 2; i++) {
