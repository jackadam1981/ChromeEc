 ...
 *
 * @return EC_SUCCESS if the device moved from UNKNOWN->OFF, or non-zero error
 * code (without changing state) for any other requested state transition.
 */
...
static void servo_deferred(void)
{
	 ...
	 * again.
	 *
	 * I don't see how this can ever return EC_SUCCESS.
	 *
	 * If servo is UNKNOWN, we'll return above.
	 *
	 * If servo is ON, device_powered_off() will return error from its
	 * first check.
	 *
	 * If servo is OFF, device_powered_off() will return error from its
	 * second check.
	 */
...
void board_update_device_state(enum device_type device)
{
	...
	if (device == DEVICE_CCD_MODE) {
		/*
		 * CCD mode pin does not have an interrupt handler, so
		 * this is the only way the battery present device state
		 * changes.
		 *
		 * This could potentially be moved to a HOOK_SECOND handler
