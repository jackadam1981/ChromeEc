...
static void process_brdprop_flog(void)
{
	...
	if (!bp.events ||
	    (board_uses_closed_loop_reset() &&
	     bp.events == (1 << BRDPROP_AMBIGUOUS)))
		return;

	bp.reset_flags = system_get_reset_flags();
	flash_log_add_event(FE_LOG_BRDPROP, sizeof(bp), &bp);
}
