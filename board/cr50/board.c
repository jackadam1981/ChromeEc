...
/*
 * The recovery button is wired to KEY0 in rbox.  For testing, we need to be
 * able override the value.  We'll have a vendor command such that the AP can
 * query the state of the recovery button.  However, the reported state can only
 * be overridden with a console command given sufficient privileges.
 */
...
static int command_recbtnforce(int argc, char **argv)
{
	...
	if (argc == 2) {
		...
		if (!ccd_is_cap_enabled(CCD_CAP_CR50_RESTRICTED_CONSOLE))
			return EC_ERROR_ACCESS_DENIED;

		if (!strncasecmp(argv[1], "enable", 6))
			rec_btn_force_pressed = 1;
		else if (!strncasecmp(argv[1], "disable", 7))
			rec_btn_force_pressed = 0;
		else
