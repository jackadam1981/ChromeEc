...
void pmu_wakeup_interrupt(void)
{
	...
	if (wakeup_src & GC_PMU_EXITPD_SRC_PIN_PD_EXIT_MASK) {
		...
		if (!gpio_get_level(GPIO_SYS_RST_L_IN) && wake_on_low)
...
static void configure_board_specific_gpios(void)
{
	...
	if (board_use_plt_rst()) {
		/* Use DIOM3 as the system reset signal */
		/* Signal using GPIO1 pin 0 for DIOM3 */
		...
		/* Use DIOM0 as the system reset signal */
		/* Signal using GPIO1 pin 0 for DIOM0 */
