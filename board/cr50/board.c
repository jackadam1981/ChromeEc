...
static int get_strap_config(uint8_t *config)
{
	...
	*config = 0;
	...
	/*
	 * The unused strap signals are used for the TPM. If the AP has added
	 * pullups to the signals, it could interfere with the strap readings.
	 * If pullups are found on both the SPI and I2C straps, use the board
	 * properties to determine SPI vs I2C. We can use this to mask unused
	 * config pins the AP is interfering with.
	 */
	if (use_i2c && use_spi) {
		...
		/* Make sure only one interface is selected */
		if ((i2c_prop && spi_prop) || (!spi_prop && i2c_prop))
			return EC_ERROR_INVAL;
		CPRINTS("Ambiguous strap config. Using last setting.");
		use_spi = spi_prop;
	}
