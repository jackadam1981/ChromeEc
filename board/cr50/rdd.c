...
static int command_bitbang(int argc, char **argv)
{
	...
	if (argc > 1) {
		...
		if (argc == 4) {
			...
			/*
			 * Temporarily disable the UART while we check if servo
			 * is attached.  We can't bitbang the UART if servo is
			 * attached.  We'll also need to keep the UART disabled
			 * when bitbanging.
			 */
			uart_was_enabled = uartn_enabled(uart);
			uartn_tx_disconnect(uart);
			if (device_get_state(DEVICE_SERVO) == DEVICE_STATE_ON) {
				ccprintf("Err: servo connected.\n");
				if (uart_was_enabled)
			...
			rv = uart_bitbang_enable(uart, baud_rate, parity);
			if (rv != EC_SUCCESS) {
				if (uart_was_enabled)
					uartn_tx_connect(uart);
				return rv;
	...
	ccprintf("  %1d  ", bitbang_config.uart);
