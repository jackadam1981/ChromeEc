...
struct uart_config {
	...
	enum gpio_signal servo_signal;
};
static struct uart_config uarts[] = {
	[UART_AP] = {"AP", DEVICE_AP, GPIO_SERVO_UART1_ON, GPIO_AP_ON,
		GPIO_SERVO_UART1_ON},
	[UART_EC] = {"EC", DEVICE_EC, GPIO_SERVO_UART2_ON, GPIO_EC_ON,
		GPIO_SERVO_UART2_ON},
};
...
void uartn_tx_connect(int uart)
{
	...
	if (device_get_state(uarts[uart].device) == DEVICE_STATE_ON) {
		gpio_disable_interrupt(uarts[uart].servo_signal);
