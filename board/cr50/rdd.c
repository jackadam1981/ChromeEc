...
void uartn_tx_connect(int uart)
{
	...
	if (uart == UART_AP) {
		...
		if (!ap_is_on()) {
			CPRINTS("AP is off; can't enable UART TX");
			return;
		}
		...
		if (!ec_is_on()) {
			CPRINTS("EC is off; can't enable UART TX");
			return;
		}
...
enum rdd_flag {
	...
	// TODO: this is wrong; can keep RX enabled
...
static void rdd_change_hook(void)
{
	...
	// TODO: If servo is debouncing at init, don't let EC turn on TX
	// yet
