...
static uint32_t rdd_get_flags(void)
{
	...
	if (ccd_usb_spi.state->enabled_device)
		flags_now |= RDD_FLAG_SPI;

	return flags_now;
}

/**
 * Print the RDD flags to the specified output channel
 *
 * @param channel	Console channel
 * @param flags		Flags to print
 */
static void rdd_print_flags(enum console_channel channel, uint32_t flags)
{
	if (flags & RDD_FLAG_UART_AP)
		cprintf(channel, " APRX");
	if (flags & RDD_FLAG_UART_AP_TX)
		cprintf(channel, " APTX");
	if (flags & RDD_FLAG_UART_EC)
		cprintf(channel, " ECRX");
	if (flags & RDD_FLAG_UART_EC_TX)
		cprintf(channel, " ECTX");
	if (flags & RDD_FLAG_UART_EC_BITBANG)
		cprintf(channel, " ECBB");
	if (flags & RDD_FLAG_I2C)
		cprintf(channel, " I2C");
	if (flags & RDD_FLAG_SPI)
		cprintf(channel, " SPI");
}
...
static void rdd_change_hook(void)
{
	...
	if (ccd_ext_is_enabled())
		flags_want |= (RDD_FLAG_UART_AP_TX | RDD_FLAG_UART_EC_TX |
			       RDD_FLAG_I2C | RDD_FLAG_SPI);
	...
	CPRINTF("[%T CCD");
	rdd_print_flags(CC_USB, flags_want);
	CPRINTF("]\n");
...
static void rdd_check_pin(void)
{
	...
	if (enable) {
		 ...
		 */
		// TODO: check what usb_release() does.  Anything different
		// from the power-on state?
		// Does this have something to do with detecting PHY?
		// But it only checks on the first connect after boot...
...
static int command_ccd(int argc, char **argv)
{
	...
	ccprintf("Flags:  ");
	rdd_print_flags(CC_COMMAND, rdd_get_flags());
	ccprintf("\n");
