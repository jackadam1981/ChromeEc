...
{
	/* Enable servo detect interrupt to reduce latency in detecting it */
	// TODO: should be like AP, starting debouncing immediately
	gpio_enable_interrupt(GPIO_DETECT_SERVO);
...
static void servo_reconnect(void)
{
	...
	// Should connect all the time, like EC/AP state machines?
	if (state == DEVICE_STATE_DEBOUNCING)
		state = DEVICE_STATE_CONNECTED;
}
