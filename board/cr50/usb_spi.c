 ...
 */
static int set_spi_bus_user(enum spi_bus_user user, int want_bus)
{
	int rv = EC_SUCCESS;
 ...
 */
static enum spi_bus_user get_spi_bus_user(void)
{
	return spi_bus_user;
}
...
static int spi_hash_device = USB_SPI_DISABLE;

/* Do we need to use NPCX7 gang programming mode? */
static int use_npcx_gang_mode;

/*
 * Device and gang mode selected by last spihdev command (for end of physical
 * presence).
 */
static int new_device;
static int new_gang_mode;
...
static void spi_hash_stop_ec_device(void)
{
	...
	if (use_npcx_gang_mode) {
		/* Reset the EC to take it out of gang mode */
		assert_ec_rst();
		usleep(200);
	...
	/*
	 * Release EC from reset (either from above, or because it was held
	 * in reset during SPI access).
	 */
	deassert_ec_rst();
}
...
static void spi_hash_pp_done(void)
{
	...
	if (new_device == USB_SPI_AP) {
		...
		/* Force the EC into reset */
		assert_ec_rst();

		enable_ec_spi();

		/*
		 * If EC is headed into gang programmer mode,
		 * need to release EC from reset after
		 * acquiring the bus.
		 */
		if (new_gang_mode) {
			// TODO: We need to pull EC UART RX
			// low during this, don't we?
			usleep(200);
			deassert_ec_rst();
...
static int command_spi_hash_set_device(int argc, char **argv)
{
	...
	if (new_device != spi_hash_device) {
		...
		if (new_device != USB_SPI_DISABLE &&
		    !(ccd_is_cap_enabled(CCD_CAP_HASH_FLASH)))
			return EC_ERROR_ACCESS_DENIED;
