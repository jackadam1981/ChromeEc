					...
					uint32_t size)
{
	...
	if (spi_read_chunk(dest, offset, size) != EC_SUCCESS) {
		CPRINTS("SPI read error at 0x%x\n", offset);
					  ...
					  uint32_t size)
{
	...
	CPRINTS("SPI hash 0x%x 0x%x", offset, size);
...
static int hash_command_wrapper(int argc, char *argv[])
{
	...
	/* Acquire shared memory; need to give it back past here */
	rv = shared_mem_acquire(command_size, (char **)&tpm_header);
	if (rv != EC_SUCCESS)
		return rv;
	...
	/* Release shared memory before exiting */
	shared_mem_release(tpm_header);
	return rv;
}
