...
int usb_spi_board_enable(int host)
{
	...
	if (host & USB_SPI_CUSTOM_RESET) {
		if (!is_sys_rst_asserted() && !is_ec_rst_asserted()) {
			CPRINTS("Assert sysrst or ecrst to do custom flash");
			return EC_ERROR_ACCESS_DENIED;
		}
		      ...
		      struct usb_setup_packet *req)
{
	...
	case USB_SPI_REQ_ENABLE_AP_CUSTOM:
		config->state->enabled_host = USB_SPI_AP | USB_SPI_CUSTOM_RESET;
		break;
