...
void board_power_led_config(enum powerled_config config)
{
	...
	case POWERLED_CONFIG_MANUAL_OFF:
		/*
		 * two ways to do this: configure GPIO as an open-drain output
		 * and drive high, or make it an input and enable pull-down
		 * resistor
		 */
#if 0
	...
	ccprintf("%s: config : %d, orig: 0x%08x, val: 0x%08x\n",
		 __func__, config, orig, val);
}
