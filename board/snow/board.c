...
void board_power_led_config(enum powerled_config config)
{
	...
	case POWERLED_CONFIG_MANUAL_OFF:
		/*
		 * Re-configure GPIO as a floating input and enable pull-down.
		 * Alternatively we could configure it as an open-drain
		 * output and set it to high impedence, but reconfiguring as
		 * an input had better results in testing.
		 */
		val |= 0x00008000;	/* input with pull up/down */
		STM32_GPIO_CRL_OFF(GPIO_B) = val;
		STM32_GPIO_BSRR_OFF(GPIO_B) |= (1 << 3) << 16;
		break;
