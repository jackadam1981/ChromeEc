...
void configure_board(void)
{
	...
	/* Enable SPI */
	STM32F_RCC_APB2ENR |= (1<<12);

	/* SPI1 on pins PA4-7 (alt. function push-pull, 10MHz) */
	val = STM32F_GPIO_CRL_OFF(GPIO_A) & ~0xffff0000;
	val |= 0x99990000;
	STM32F_GPIO_CRL_OFF(GPIO_A) = val;

	/*
	 * I2C SCL/SDA on PB10-11, bi-directional, no pull-up/down, initialized
	 * as hi-Z until alt. function is set
	 */
	val = STM32F_GPIO_CRH_OFF(GPIO_B) & ~0x0000ff00;
	val |= 0x0000dd00;
	STM32F_GPIO_CRH_OFF(GPIO_B) = val;
	STM32F_GPIO_BSRR_OFF(GPIO_B) |= (1<<11) | (1<<10);

	/* Select Alternate function for USART1 on pins PA9/PA10 */
	val = STM32F_GPIO_CRH_OFF(GPIO_A) & ~0x00000ff0;
	val |= 0x00000990;
	STM32F_GPIO_CRH_OFF(GPIO_A) = val;

	/* EC_INT is output, open-drain */
	val = STM32F_GPIO_CRH_OFF(GPIO_B) & ~0xf0;
	val |= 0x50;
	STM32F_GPIO_CRH_OFF(GPIO_B) = val;
