...
static const struct fan_step fan_table_DTA_clamshell[] = {
	{
		/* level 0 */
		.on = {-1, -1, -1, -1},
		.off = {-1, -1, -1, -1},
...
static const struct fan_step fan_table_DTA_tablet[] = {
	{
		/* level 0 */
		.on = {-1, -1, -1, -1},
		.off = {-1, -1, -1, -1},
...
static const struct fan_step fan_table_DTA_stand[] = {
	{
		/* level 0 */
		.on = {-1, -1, -1, -1},
		.off = {-1, -1, -1, -1},
...
static const struct fan_step fan_table_SOL_clamshell[] = {
	{
		/* level 0 */
		.on = {-1, -1, -1, -1},
		.off = {-1, -1, -1, -1},
...
static const struct fan_step fan_table_SOL_tablet[] = {
	{
		/* level 0 */
		.on = {-1, -1, -1, -1},
		.off = {-1, -1, -1, -1},
...
static const struct fan_step fan_table_SOL_stand[] = {
	{
		/* level 0 */
		.on = {-1, -1, -1, -1},
		.off = {-1, -1, -1, -1},
...
#define NUM_FAN_LEVELS ARRAY_SIZE(fan_table_DTA_clamshell)

#define lid_angle_tablet	340

BUILD_ASSERT(ARRAY_SIZE(fan_table_DTA_clamshell) ==
	ARRAY_SIZE(fan_table_DTA_tablet));
...
int fan_table_to_rpm(int fan, int *temp)
{
	...
	/*
	 * Select different fan curve table by mode and fan id
	 */
	if (tablet_get_mode()) {
		if (gpio_get_level(GPIO_FAN_ID))
			fan_step_table = fan_table_DTA_stand;
		else
			fan_step_table = fan_table_SOL_stand;

		if (lid_angle >= lid_angle_tablet) {
			if (gpio_get_level(GPIO_FAN_ID))
				fan_step_table = fan_table_DTA_tablet;
			else
				fan_step_table = fan_table_SOL_tablet;
		}
	} else {
		if (gpio_get_level(GPIO_FAN_ID))
			fan_step_table = fan_table_DTA_clamshell;
		else
			fan_step_table = fan_table_SOL_clamshell;
	}
