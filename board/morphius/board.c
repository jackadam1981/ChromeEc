...
static uint32_t get_board_ver(void)
{
	...
	if (board_ver == 0xFFFFFFFF) {
		 ...
		 *
		 * USB-C1 OPT1 DB uses DP2_HPD. (default)
		 * USB-C1 OPT3 DB
...
static void board_remap_gpio(void)
{
	...
	if (board_ver >= 3) {
		 ...
		 */
		g = ioex_list + IOEX_HDMI_POWER_EN_DB - IOEX_SIGNAL_START;
		g->flags = GPIO_OUT_LOW;

		g = ioex_list + IOEX_USB_C1_PPC_ILIM_3A_EN - IOEX_SIGNAL_START;
		g->flags = GPIO_OUT_LOW;

		rv = ioex_init(USBC_PORT_C1);
		if (rv)
