...
int led_set_brightness(enum ec_led_id led_id, const uint8_t *brightness)
{
	if (led_id == EC_LED_ID_BATTERY_LED) {
		...
		if (brightness[EC_LED_COLOR_WHITE] != 0)
			pwm_set_duty(PWM_CH_POWER_LED, color_brightness[LED_WHITE]);
		else
			pwm_set_duty(PWM_CH_POWER_LED, color_brightness[LED_OFF]);
	}
...
static void suspend_led_update_deferred(void)
{
	...
	if (ticks <= TICKS_STEP2_DIMMER) {
		pwm_set_duty(PWM_CH_POWER_LED, ticks*5);
	} else if (ticks <= TICKS_STEP3_OFF) {
		pwm_set_duty(PWM_CH_POWER_LED, (TICKS_STEP3_OFF - ticks)*5);
	} else {
