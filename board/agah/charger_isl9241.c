...
static int board_get_bj_type(void)
{
	return bj_type;
}
...
{
	int adp_typ_mv, temp_i;

	adp_typ_mv = adc_read_channel(ADC_ADP_TYP);

	for (temp_i = ADAPTER_45W; temp_i < ADAPTER_UNKNOWN; temp_i++) {
		if (adp_typ_mv >= bj_adp_type[temp_i].low_limit &&
		    adp_typ_mv <= bj_adp_type[temp_i].high_limit) {
			CPRINTS("Barrel Jack adapter is %s (%dmv)",
				adapter_name[temp_i], adp_typ_mv);
			break;
		}
	}

	if (temp_i == ADAPTER_UNKNOWN ||
	    adp_typ_mv > bj_adp_type[ADAPTER_150W].high_limit)
		CPRINTS("Barrel Jack adapter is unknown");

	charge_manager_update_charge(
		CHARGE_SUPPLIER_DEDICATED, DEDICATED_CHARGE_PORT,
		temp_i != ADAPTER_UNKNOWN ? &bj_power[temp_i] : NULL);
...
{
	return board_get_bj_type() != ADAPTER_UNKNOWN ?
