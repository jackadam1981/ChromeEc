...
/*
 *
 * The diagram below shows the states from the connected chargers' perspective.
 *
 *  +---------+    +USB     +---------+
 *  | BATTERY |------------>| BATTERY |
 *  |         |<------------|    +USB |
 ...
 * the system power state, transition/throttling may or may not occur. But the
 * base flow of charger switch is the following:
 *
 ...
 * 3. board_set_active_charge_port is called. It triggers hard & soft throttling
 *    for AP & GPU if needed.
 * 4. Disable active port.
 * 5. Enables new port.
 * 6. HOOK_POWER_SUPPLY_CHANGE is called. We disables hard throttling. It may
 *    keep soft throttling enabled.
 */
