...
enum bj_type_t {
	...
	BJ_TYPE_150W,
	BJ_TYPE_UNKNOWN,
	BJ_TYPE_COUNT = BJ_TYPE_UNKNOWN,
};

static const char *const adapter_name[] = {
	[BJ_TYPE_45W] = "45W",	       [BJ_TYPE_65W] = "65W",
	[BJ_TYPE_90W] = "90W",	       [BJ_TYPE_120W] = "120W",
	[BJ_TYPE_135W] = "135W",       [BJ_TYPE_150W] = "150W",
	[BJ_TYPE_UNKNOWN] = "unknown",
};

struct adp_type_voltage {
	/* adp_typ up and low limit in mV */
	int low_limit;
	int high_limit;
};

static const struct adp_type_voltage bj_adp_type[] = {
	/* 45W */
	[BJ_TYPE_45W] = { .low_limit = 799, .high_limit = 884 },
	/* 65W */
	[BJ_TYPE_65W] = { .low_limit = 1009, .high_limit = 1115 },
	/* 90W */
	[BJ_TYPE_90W] = { .low_limit = 1284, .high_limit = 1419 },
	/* 120W */
	[BJ_TYPE_120W] = { .low_limit = 1558, .high_limit = 1721 },
	/* 135W */
	[BJ_TYPE_135W] = { .low_limit = 1739, .high_limit = 1865 },
	/* 150W */
	[BJ_TYPE_150W] = { .low_limit = 1881, .high_limit = 2017 },
};
...
static enum bj_type_t bj_type;

static enum bj_type_t detect_bj(void)
{
	int mv;
	enum bj_type_t type;
	...
	if (mv < 0)
		return BJ_TYPE_UNKNOWN;

	for (type = BJ_TYPE_45W; type < BJ_TYPE_COUNT; type++) {
		if (mv >= bj_adp_type[type].low_limit &&
		    mv <= bj_adp_type[type].high_limit) {
			CPRINTS("Barrel Jack adapter is %s (%dmv)",
				adapter_name[type], mv);
			break;
		}
	}

	if (type == BJ_TYPE_COUNT)
		CPRINTS("Barrel Jack adapter is unknown (%dmv)", mv);
...
static void bj_connect(void)
{
	...
	CPRINTS("BJ %s %sconnected", adapter_name[bj_type],
		bj_connected ? "" : "dis");

	charge_manager_update_charge(CHARGE_SUPPLIER_DEDICATED,
				     DEDICATED_CHARGE_PORT,
				     connected && bj_type != BJ_TYPE_COUNT ?
					     &bj_power[bj_type] :
					     NULL);
}
...
DECLARE_HOOK(HOOK_INIT, bj_state_init, HOOK_PRIO_INIT_ADC + 1);
