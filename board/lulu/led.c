...
#define LED_ERROR_FREQ		2	/* LED on 250ms every 500ms */
#define LED_FORCE_IDLE_FREQ	4	/* LED on 250ms every 1000ms */
...
int led_set_brightness(enum ec_led_id led_id, const uint8_t *brightness)
{
	if (brightness[EC_LED_COLOR_WHITE] != 0)
		lulu_led_set_color(led_id, LED_WHITE);
...
static void lulu_led_set_battery(void)
{
	...
	case PWR_STATE_CHARGE:
		lulu_led_set_color_battery(LED_WHITE);
		break;
