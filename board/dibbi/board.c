 ...
 * input current (read from a shunt), averaged over 5 readings. The power budget
 * limit is from the charge manager.
 *
 ...
 * at least 2W. We use 4W to ensure we throttle Type-C before we start throtting
 * SoC power.
 *
...
static void power_monitor(void)
{
	...
	if (chipset_in_state(CHIPSET_STATE_ANY_OFF | CHIPSET_STATE_SUSPEND)) {
		...
		/* Get current charger limit. If not configured yet, skip. */
