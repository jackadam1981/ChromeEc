...
static int set_period_bytes(uint32_t period_bytes)
{
	if (!period_bytes)
		return EC_ERROR_INVAL;

	/* TODO: check the upper boundary if any */

	ctx.period_bytes = period_bytes;
	return EC_RES_SUCCESS;
}

static int get_period_bytes(uint32_t *period_bytes)
{
	*period_bytes = ctx.period_bytes;
	return EC_RES_SUCCESS;
}
...
{
	*capabilities = 0
#ifdef CONFIG_EC_CODEC_WOV_CAP_SHM
		| EC_CODEC_WOV_CAP_SHM
#endif
#ifdef CONFIG_EC_CODEC_WOV_CAP_SHM_PASSIVE
		| EC_CODEC_WOV_CAP_SHM_PASSIVE
#endif
		;
...
static int enable(void)
{
	...
	/* check prerequisite configurations */
	if (!ctx.period_bytes)
...
static int read(uint8_t *buf, uint32_t len, uint32_t *n)
{
	...
	*n = (*n / ctx.period_bytes) * ctx.period_bytes;
...
static int ec_codec_wov_set_period_bytes(struct host_cmd_handler_args *args)
{
	struct ec_param_ec_codec_wov *param =
		(struct ec_param_ec_codec_wov *)args->params;

	args->response_size = 0;
	return set_period_bytes(param->period_bytes);
}

static int ec_codec_wov_get_period_bytes(struct host_cmd_handler_args *args)
{
	struct ec_response_ec_codec_wov_get_period_bytes *r = args->response;

	args->response_size = sizeof(*r);
	return get_period_bytes(&r->period_bytes);
}
...
int handle_set_period_bytes(int argc, char **argv)
{
	...
	ret = set_period_bytes(strtoul(argv[2], NULL, 0));
	if (!ret)
		ccprintf("Success!\n");
leave:
	return ret;
...
int handle_get_period_bytes(int argc, char **argv)
{
	int ret = EC_ERROR_INVAL;
	uint32_t period_bytes;
	...
	ret = get_period_bytes(&period_bytes);
	if (ret)
		goto leave;
...
int handle_get_capabilities(int argc, char **argv)
{
	...
	if (cap & EC_CODEC_WOV_CAP_SHM)
		ccprintf("  EC_CODEC_CAP_SHM\n");
	if (cap & EC_CODEC_WOV_CAP_SHM_PASSIVE)
		ccprintf("  EC_CODEC_CAP_SHM_PASSIVE\n");
leave:
	return ret;
}
...
static int wov_console_command(int argc, char **argv)
{
	struct {
		...
		{"set_period_bytes",	handle_set_period_bytes},
		{"get_period_bytes",	handle_get_period_bytes},
