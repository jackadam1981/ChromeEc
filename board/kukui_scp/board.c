...
static __attribute__((always_inline)) inline uint64_t perf_test(void)
{
	volatile int x = 0;
	int i, it;
	uint64_t total;
...
__SECTION(dram) static int command_icachetest(int argc, char **argv)
{
	...
	ccprintf("run from DRAM (cached %08x): ", test_cached);
	(*test_cached)();
	ccprintf("run from DRAM (direct %08x): ", test_direct);
	(*test_direct)();
