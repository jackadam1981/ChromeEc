...
static void config_port_discrete_tcpc(int port)
{
	...
	/* Ping TUSB422 and RT1715 addresses */
	if (!i2c_xfer_unlocked(i2c_port, TUSB422_I2C_ADDR_FLAGS, NULL, 0, &tmp,
			       1, I2C_XFER_SINGLE)) {
		CPRINTS("C%d: TUSB422 detected", port);
	} else if (!i2c_xfer_unlocked(i2c_port, RT1715_I2C_ADDR_FLAGS, NULL, 0,
				      &tmp, 1, I2C_XFER_SINGLE)) {
