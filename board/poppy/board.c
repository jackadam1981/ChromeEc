...
static void get_dynamic_info(void)
{
	struct {
		uint32_t head[3];
	...
	if (write_command(EC_COMM_DYNAMIC_INFO, (void*)&data,
...
static void get_static_info(void)
{
	struct {
		uint32_t head[3];
	...
	if (write_command(EC_COMM_STATIC_INFO, (void*)&data,
				sizeof(data), 10000) != EC_SUCCESS) {
		CPRINTF("%s: error\n", __func__);
		return;
	}
