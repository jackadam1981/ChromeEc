...
#define IN_PGOOD_S3          (IN_PGOOD_PP1350 | IN_PGOOD_PP5000)
/* Rails required for S0 */
#define IN_PGOOD_S0          (IN_PGOOD_PP5000 | IN_PGOOD_ALL_NONCORE)

/* All PM_SLP signals from PCH deasserted */
#define IN_ALL_PM_SLP_DEASSERTED (IN_PCH_SLP_S3n_DEASSERTED |		\
				  IN_PCH_SLP_S5n_DEASSERTED)
/* All inputs in the right state for S0 */
#define IN_ALL_S0 (IN_PGOOD_PP5000 | IN_PGOOD_ALL_NONCORE |		\
...
static void update_in_signals(void)
{
	...
	if (gpio_get_level(GPIO_VCORE_PGOOD))
		inew |= IN_PGOOD_VCORE;
	if (gpio_get_level(GPIO_CPU_PGOOD))
		inew |= IN_PGOOD_CPU;
	...
	if (gpio_get_level(GPIO_PCH_SUSWARN_L))
...
void chipset_force_shutdown(void)
{
	...
	/* TODO(adurbin): need to do anything about RSMRST_L */
...
void chipset_task(void)
{
	...
	while (1) {
		...
		case X86_S3S0:
			...
			/* Wait 5 ms before asserting SYS_PWROK. */
			msleep(5);

			/* Set PCH_PWROK which is also tied to APWROK. */
			gpio_set_level(GPIO_PCH_PWROK, 1);
