...
static void set_board_id_3_gpios(void)
{
	if (get_board_id() < 4) {
		power_signal_list[X86_ALL_SYS_PGOOD].gpio =
			GPIO_ID_3_SEQ_EC_ALL_SYS_PG;
	}
}
DECLARE_HOOK(HOOK_INIT, set_board_id_3_gpios, HOOK_PRIO_FIRST);

__override int intel_x86_get_pg_ec_all_sys_pwrgd(void)
{
	if (get_board_id() < 4)
		return gpio_get_level(GPIO_ID_3_SEQ_EC_ALL_SYS_PG);
