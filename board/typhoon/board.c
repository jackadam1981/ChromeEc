...
const void *const usb_strings[] = {
	...
	[USB_STR_PRODUCT] = USB_STRING_DESC("Typhoon"),
	[USB_STR_SERIALNO] = USB_STRING_DESC("1234-a"),
	[USB_STR_VERSION] = USB_STRING_DESC(CROS_EC_VERSION32),
	[USB_STR_I2C_NAME] = USB_STRING_DESC("I2C"),
	[USB_STR_CONSOLE_NAME] = USB_STRING_DESC("Typhoon EC Shell"),
...
{
	/* Enable peripheral clock to the DAC */
...
static int command_dactest(int argc, char **argv)
{
	...
	ccprintf("Usage: dactest <dac value>\n");

	return EC_SUCCESS;
}
DECLARE_SAFE_CONSOLE_COMMAND(dactest, command_dactest, "[name]",
			     "Test DAC");

static int command_enable_quad(int argc, char **argv)
{
	int i;

	/* Add check for power good and capability match before starting!!!! */
	if (argc == 2) {
		i = atoi(argv[1]);
		gpio_set_level(GPIO_QUAD_EN, i);

		return EC_SUCCESS;
	}

	ccprintf("Usage: enable_quad <0 or 1>\n");

	return EC_SUCCESS;
}
DECLARE_SAFE_CONSOLE_COMMAND(enable_quad, command_enable_quad, "[name]",
			     "Enable quad output");
