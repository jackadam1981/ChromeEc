...
const struct pwm_t pwm_channels[] = {
	[PWM_CH_FULL_LED] = { 0, PWM_CONFIG_DSLEEP_CLK, 100 },
	[PWM_CH_BKLIGHT] =  { 2, 0, 10000 },
	[PWM_CH_CHG_LED] =  { 3, PWM_CONFIG_DSLEEP_CLK, 100 },
	[PWM_CH_PWR_LED] =  { 4, PWM_CONFIG_DSLEEP_CLK, 100 },
};
...
struct {
	...
	int target_voltage; /* mV */
} const kevin_board_versions[] = {
	{ BOARD_VERSION_PROTO1, 74 },   /* 2.2 ohm */
	{ BOARD_VERSION_PROTO2, 211 },  /* 6.8 ohm */
};
...
int board_get_version(void)
{
	...
	for (i = 0; i < BOARD_VERSION_COUNT; ++i) {
		if (mv > kevin_board_versions[i].target_voltage * 9 / 10 &&
		    mv < kevin_board_versions[i].target_voltage * 11 / 10) {
			version = i;
			break;
		}
	}
