...
void board_overcurrent_event(int port, int is_overcurrented)
{
	...
	int i2c_chan = port < TYPE_C_PORT_2 ?
			I2C_CHAN_TYPEC_1 : I2C_CHAN_TYPEC_3;
	int i2c_port = i2c_ports[i2c_chan].port;

	if (is_overcurrented) {
		pca9675_update_pins(i2c_port, I2C_ADDR_PCA9675_TCPC_AIC_IOEX,
				TCPC_AIC_IOE_OC, 0);
	} else {
		pca9675_update_pins(i2c_port, I2C_ADDR_PCA9675_TCPC_AIC_IOEX,
				0, TCPC_AIC_IOE_OC);
...
{
	/* Based on different I2C channel of each type C port, distinguishes
	 * the control pins of  each retimer.
	 */
	int i2c_port = i2c_ports[me->usb_port + I2C_CHAN_TYPEC_0].port;
...
static void board_connect_c0_sbu_deferred(void)
{
	...
	if (ccd_intr_level) {
		...
		pca9675_update_pins(i2c_ports[I2C_CHAN_TYPEC_0].port,
			I2C_ADDR_PCA9675_TCPC_AIC_IOEX,
			0,
...
static void tcpc_aic_init(void)
{
	...
	for (i = 0; i < CONFIG_USB_PD_PORT_MAX_COUNT; i++)
		pca9675_init(i2c_ports[I2C_CHAN_TYPEC_0 + i].port,
			I2C_ADDR_PCA9675_TCPC_AIC_IOEX);

	/* Default set the SBU lines to AUX mode on both the TCPC-AIC */
	board_connect_c0_sbu_deferred();

	/* TODO(b/169814014):call pca9675 functions using ioexpander functionality
	 *
	 */
	pca9675_update_pins(i2c_ports[I2C_CHAN_TYPEC_2].port,
		I2C_ADDR_PCA9675_TCPC_AIC_IOEX, 0,
