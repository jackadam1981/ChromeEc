...
static void board_en_all_sys_pwrgd_intr(void)
{
	gpio_enable_interrupt(GPIO_ALL_SYS_PWRGD);
}
DECLARE_HOOK(HOOK_INIT, board_en_all_sys_pwrgd_intr, HOOK_PRIO_INIT_CHIPSET);
...
/* MCHP mec1521_evb connected to Intel SKL RVP3 with Kabylake
...
static void board_pmic_init(void)
{
	...
	/* Send start command */
	i2c_write8(I2C_PORT_THERMAL, DS1624_I2C_ADDR_FLAGS,
		   DS1624_CMD_START, 1);
...
{
	CPRINTS("MEC1521 HOOK_INIT - board");
...
{
	CPRINTS("MEC1521 HOOK_AC_CHANGE");
	gpio_set_level(GPIO_PCH_ACOK, extpower_is_present());
}
...
int board_set_active_charge_port(int charge_port)
{
	...
	if (is_real_port && source) {
		CPRINTS("MEC1521 Skip enable p%d", charge_port);
		return EC_ERROR_INVAL;
	}

	CPRINTS("MEC1521 New chg p%d", charge_port);
...
{
	CPRINTS("MEC1521 HOOK_CHIPSET_STARTUP");
...
{
	CPRINTS("MEC1521 HOOK_CHIPSET_SHUTDOWN");
...
{
	CPRINTS("MEC1521_EVG HOOK_CHIPSET_RESUME");
...
{
	CPRINTS("MEC1521 HOOK_CHIPSET_SUSPEND");
...
static void board_handle_reboot(void)
{
	...
	int flags;
#endif
	CPRINTS("MEC HOOK_INIT - board level reboot");
...
static void sb_update(void)
{
	...
	if (rv != EC_SUCCESS) {
		CPRINTS("sb_read temperature rv=%d temp=%d K",
			rv, smart_batt_temp);
	}
}
...
static void ds1624_update(void)
{
	...
	i2c_read16(I2C_PORT_THERMAL, DS1624_I2C_ADDR_FLAGS,
...
#endif /* #ifdef HAS_TASK_MOTIONSENSE */

#ifdef MEC1521_EVB_TACH_TEST /* PWM/TACH test */
void tach0_isr(void)
{
