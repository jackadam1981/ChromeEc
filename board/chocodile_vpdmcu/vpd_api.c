...
void vpd_host_set_pull(int pull, int rp_value)
{
	...
	case TYPEC_CC_RP:
		...
		case TYPEC_RP_USB:
			...
			break;
		/*
		 * TODO(shurst): Remove "case TYPEC_RP_1A5" on next revision of
		 * chocodile board
		 */
		case TYPEC_RP_1A5:
...
{
	/* If we're not configuring for CMP, disable COMP1 if it was enabled */
	if (cfg != PIN_CMP && cmp1_enabled)
		STM32_COMP_CSR &= ~STM32_COMP_CMP1EN;
	...
	if (cfg == PIN_GPO) {
		...
		STM32_GPIO_MODER(GPIO_A) = (STM32_GPIO_MODER(GPIO_A)
				...
				|  (1 << (2*1)); /* Set as GPO */
	} else if (cfg == PIN_ADC || cfg == PIN_CMP) {
		/* Enable Analog mode */
		STM32_GPIO_MODER(GPIO_A) = (STM32_GPIO_MODER(GPIO_A)
			|  (3 << (2*1))); /* PA1 in ANALOG mode */

		/* Re-enable COMP1 if it was disabled in this function */
		if ((cfg == PIN_CMP) && cmp1_enabled)
			STM32_COMP_CSR |= STM32_COMP_CMP1EN;
	}
}
...
inline int vpd_is_host_vbus(void)
{
	return (vpd_read_host_vbus() >= PD_SNK_VA);
}

inline int vpd_is_ct_vbus(void)
{
	return (vpd_read_ct_vbus() >= PD_SNK_VA);
}

inline int vpd_is_vconn(void)
{
...
/* TODO(shurst): Remove on next revision of chocodile board */
