...
void vpd_ct_set_pull(int pull, int rp_value)
{
	...
	case TYPEC_CC_RD:
		vpd_config_cc1_rpusb_odh(PIN_ADC, 0);
		vpd_config_cc2_rpusb_odh(PIN_ADC, 0);
		/* FIXME: Change to PIN_ADC in next board rev. */
...
void vpd_ct_get_cc(int *cc1, int *cc2)
{
	...
	case TYPEC_CC_RP:
		...
		case TYPEC_RP_USB:
			cc1_v = adc_read_channel(ADC_CC1_RP1A5_ODH);
			cc2_v = adc_read_channel(ADC_CC2_RP1A5_ODH);
			break;
		case TYPEC_RP_1A5:
...
void vpd_host_get_cc(int *cc)
{
	...
	*cc = vpd_cc_voltage_to_status(
	...
	vpd_config_cc_vpdmcu(PIN_CMP, 0);
}

void vpd_rx_enable(int en)
{
	if (en) {
		/* Disconnect charge-through CC1 and CC2 lines from host */
		gpio_set_level(GPIO_CC1_SEL, 0);
		gpio_set_level(GPIO_CC2_SEL, 0);

		/* Set high to connect MCU CC to host CC line */
		gpio_set_level(GPIO_VPDMCU_CC_EN, 1);
	}
...
void vpd_config_cc_vpdmcu(enum vpd_pin cfg, int en)
{
	if (cfg == PIN_GPO) {
		...
		STM32_GPIO_MODER(GPIO_A) = (STM32_GPIO_MODER(GPIO_A)
				...
				|  (1 << (2*1)); /* Set as GPO */
		gpio_set_level(GPIO_CC_VPDMCU, en ? 1:0);
	}
...
void vpd_config_cc1_rp1a5_odh(enum vpd_pin cfg, int en)
{
	if (cfg == PIN_GPO) {
		...
		STM32_GPIO_MODER(GPIO_A) = (STM32_GPIO_MODER(GPIO_A)
				...
				|  (1 << (2*4)); /* Set as GPO */
		/* Default high. Enable cc1 Rp1A5 pullup */
		gpio_set_level(GPIO_CC1_RP1A5_ODH, en ? 1:0);
	}
...
void vpd_config_cc2_rp1a5_odh(enum vpd_pin cfg, int en)
{
	if (cfg == PIN_GPO) {
		...
		STM32_GPIO_MODER(GPIO_A) = (STM32_GPIO_MODER(GPIO_A)
				...
				|  (1 << (2*5)); /* Set as GPO */
		gpio_set_level(GPIO_CC2_RP1A5_ODH, en ? 1:0);
	}
...
{
	return adc_read_channel(ADC_HOST_VBUS_VSENSE);
}

int vpd_read_ct_vbus(void)
{
	return adc_read_channel(ADC_CHARGE_VBUS_VSENSE);
}

int vpd_read_vconn(void)
{
	return adc_read_channel(ADC_VCONN_VSENSE);
}
...
int vpd_read_cc1_ct(void)
{
	return adc_read_channel(ADC_CC1_RP1A5_ODH);
}

int vpd_read_cc2_ct(void)
{
	return adc_read_channel(ADC_CC2_RP1A5_ODH);
}

void vpd_red_led(int on)
{
	gpio_set_level(GPIO_DEBUG_LED_R_L, (on) ? 1 : 0);
}

void vpd_green_led(int on)
{
	gpio_set_level(GPIO_DEBUG_LED_G_L, (on) ? 1 : 0);
}
...
void vpd_present_billboard(int en)
{
	gpio_set_level(GPIO_PRESENT_BILLBOARD, (en) ? 0 : 1);
}
...
void vpd_cc1_sel(int en)
{
	gpio_set_level(GPIO_CC1_SEL, (en) ? 1 : 0);
}

void vpd_cc2_sel(int en)
{
	gpio_set_level(GPIO_CC2_SEL, (en) ? 1 : 0);
}
...
{
	if (val == GPO_HZ) {
		gpio_set_flags(GPIO_CC_RP1A5_ODH, GPIO_INPUT);
	} else {
		gpio_set_flags(GPIO_CC_RP1A5_ODH, GPIO_OUTPUT);
		gpio_set_level(GPIO_CC_RP1A5_ODH, (val == GPO_HIGH) ? 1:0);
	}
}
...
{
	if (val == GPO_HZ) {
		gpio_set_flags(GPIO_CC1_CC2_RD_L, GPIO_INPUT);
	} else {
		gpio_set_flags(GPIO_CC1_CC2_RD_L, GPIO_OUTPUT);
		gpio_set_level(GPIO_CC1_CC2_RD_L, (val == GPO_HIGH) ? 1:0);
	}
}

void vpd_vconn_pwr_sel_odl(enum vpd_pwr en)
{
	gpio_set_level(GPIO_VCONN_PWR_SEL_ODL, en);
}
