...
static void board_motion_sense_resume(void)
{
	if (board_is_convertible()) {
		...
		ret = i2c_xfer(motion_sensors[LID_ACCEL].port,
			motion_sensors[LID_ACCEL].addr,
			&who_am_i_ret, 1, &who_am_i, 1);
		ccprints("ret=%d, who_am_i_ret=%x", ret, who_am_i_ret);

		if (ret) {
			uint8_t tmp;

			if (!i2c_xfer(motion_sensor_bma253.port,
					motion_sensor_bma253.addr, NULL, 0, &tmp, 1)) {
				motion_sensors[LID_ACCEL] = motion_sensor_bma253;
				ccprints("LID accel: BMA253");
			} else {
				ccprints("LID accel: ERROR");
			}
		} else {
