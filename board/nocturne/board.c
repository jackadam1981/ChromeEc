...
static inline int mp2949a_imvp8_set_page(int page)
{
	int rv;
...
static inline int mp2949a_imvp8_reg_write(int page, int reg, int data)
{
	...
	rv = mp2949a_imvp8_set_page(page);
	if (rv)
		return rv;
...
static inline int mp2949a_imvp8_no_data_reg_write(int page, int reg)
{
	int rv;
	...
	i2c_lock(I2C_PORT_PMIC, 1);
	rv = i2c_xfer(I2C_PORT_PMIC, I2C_ADDR_MP2949A, (const uint8_t *) &reg,
			1, 0, 0, I2C_XFER_SINGLE);
	i2c_lock(I2C_PORT_PMIC, 0);
...
static int board_mp2949a_imvp8_init(int argc, char **argv)
{
	...
	if (mp2949a_imvp8_no_data_reg_write(0x3, MP2949A_REG_STORE_USER_ALL))
		return -1;
