...
static int should_discharge(void)
{
	int t_dram;

	/* We can only discharge on AC, if AC is plugged in. */
...
{
	static uint8_t discharging_on_ac;
	int enable_discharge;

	enable_discharge = should_discharge();
	if (enable_discharge != discharging_on_ac) {
		if (enable_discharge) {
