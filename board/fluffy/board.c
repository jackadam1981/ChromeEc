...
static void set_led(enum led_ch led, int enable)
{
	...
	switch (led) {
	case LED_5V:
		write_ioexpander(0, 0, !!enable);
		break;

	case LED_9V:
		write_ioexpander(0, 2, !!enable);
		break;

	case LED_12V:
		write_ioexpander(0, 4, !!enable);
		break;

	case LED_15V:
		write_ioexpander(1, 0, !!enable);
		break;

	case LED_20V:
		write_ioexpander(1, 2, !!enable);
		break;
...
void show_output_voltage_on_leds(void)
{
	...
	if (vbus_mv != ADC_READ_ERROR) {
		...
		act = (vbus_mv * 7700) / 1000;
		if (vbus_mv != prev_vbus_mv) {
