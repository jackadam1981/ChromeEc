...
{
	unsigned int bj;
	unsigned int bj_other;

	bj = fw_config.bj_power;
	bj_other = fw_config.bj_power_other;

	/* Out of range value defaults to 0 */
	if (bj >= ARRAY_SIZE(bj_power) &&
	    bj_other >= ARRAY_SIZE(bj_power_other)) {
		bj = 0;
		bj_other = 0;
		CPRINTS("fw_config: bj_power out of range");
	}
	...
	if (bj_other == 0) {
		...
		*voltage = bj_power_other[bj_other].voltage;
		*current = bj_power_other[bj_other].current;
	}
}
