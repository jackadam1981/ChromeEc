...
static void fsm_send(int pol, uint32_t w)
{
	...
	send_message(pol, header, cnt, inj_cmds + idx);
}

static void fsm_wave(int pol, uint32_t w)
{
	...
	/* Transmit the packet */
	pd_start_tx(0, pol, off);
	pd_tx_done(0, pol);
}

static void fsm_wait(int pol, uint32_t w)
{
	uint32_t timeout_ms = INJ_ARG0(w);
	uint32_t min_edges = INJ_ARG12(w);

	wait_packet(pol,  min_edges, timeout_ms * 1000);
}
...
static int fsm_run(int index)
{
	while (index < INJ_CMD_COUNT) {
		...
		case INJ_CMD_SEND1:
		case INJ_CMD_SEND2:
			fsm_send(cmd - INJ_CMD_SEND1, w);
			break;
		case INJ_CMD_WAVE1:
		case INJ_CMD_WAVE2:
			fsm_wave(cmd - INJ_CMD_WAVE1, w);
			break;
		case INJ_CMD_HRST1:
		case INJ_CMD_HRST2:
			send_hrst(cmd - INJ_CMD_HRST1);
			break;
		case INJ_CMD_WAIT1:
		case INJ_CMD_WAIT2:
			fsm_wait(cmd - INJ_CMD_WAIT1, w);
			break;
		case INJ_CMD_GET:
			fsm_get(w);
			break;
...
static int cmd_bufwr(int argc, char **argv)
{
	...
	for (i = idx; i < idx + cnt; i++)
		if (hex8tou32(argv[i+1], inj_cmds + i))
			return EC_ERROR_INVAL;
...
{
	/* PING */
	send_message(0, 0x0045, 0, NULL);
	wait_packet(0, 230, 5000);
	/* PING */
	send_message(0, 0x0045, 0, NULL);
	wait_packet(0, 0, 5000);

	/* GET_SOURCE_CAP */
	send_message(0, 0x0047, 0, NULL);
	wait_packet(0, 230, 5000);
	wait_packet(0, 280, 50000);
	/* GoodCRC to SRC_CAP */
	send_message(0, 0x0041, 0, NULL);
...
static int command_tw(int argc, char **argv)
{
	...
	else if (!strncasecmp(argv[1], "test", 3))
		return cmd_test();
