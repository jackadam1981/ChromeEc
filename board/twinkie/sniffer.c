...
static usb_uint ep_buf[2][USB_MAX_PACKET_SIZE / 2] __usb_ram;
...
static void ep_tx(void)
{
	...
	//task_set_event(TASK_ID_SNIFFER, TASK_EVENT_CUSTOM(0xbad), 0);
	return;
}
...
static int tx_idx;
void tmp_putb(uint8_t val)
{
	if (!(tx_idx & 1))
		ep_buf[0][tx_idx/2] = val;
	else
		ep_buf[0][tx_idx/2] |= val << 8;
	tx_idx++;
	if (tx_idx > 63)
		tx_idx = 0;
...
#if 0
	/* TODO TODO TODO*/
	btable_ep[USB_EP_SNIFFER].tx_count = tx_idx;
	/* enable TX */
