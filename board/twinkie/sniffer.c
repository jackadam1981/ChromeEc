...
static uint32_t vbus_vol_queue_count = 0;
static int vbus_vol_head = 0;
static int vbus_vol_tail = -1;

static struct vbus_curr_info vbus_curr_array[VBUS_ARRAY_SIZE];
static uint32_t vbus_curr_queue_count = 0;
static int vbus_curr_head = 0;
static int vbus_curr_tail = -1;
...
{
	/* read may be interrupted, use average of start and end as the tstamp */
	/* Unit: ms */
	uint16_t tstamp_bf;
	uint16_t tstamp_af;
	/* Unit: mV */
	uint16_t vol;
	if (vbus_vol_queue_count < VBUS_ARRAY_SIZE) {
		tstamp_bf = __hw_clock_source_read();
		vol = ina2xx_read(0, INA2XX_REG_BUS_VOLT)*125 / 100;
		tstamp_af = __hw_clock_source_read();

		vbus_vol_tail = (vbus_vol_tail + 1) % VBUS_ARRAY_SIZE;
		if (tstamp_bf > tstamp_af)
			vbus_vol_array[vbus_vol_tail].tstamp = ((tstamp_bf + tstamp_af + 65535)>>1) & 0xFFFF;
		else
			vbus_vol_array[vbus_vol_tail].tstamp = (tstamp_bf + tstamp_af)/2;
		vbus_vol_array[vbus_vol_tail].vol = vol;
		atomic_add(&vbus_vol_queue_count, 1);
	}
...
static void vbus_curr_read_deferred(void)
{
	...
	/* Unit: mV */
	uint16_t curr;
	if (vbus_curr_queue_count < VBUS_ARRAY_SIZE) {
		tstamp_bf = __hw_clock_source_read();
		curr = ina2xx_read(0, INA2XX_REG_CURRENT);
		tstamp_af = __hw_clock_source_read();

		vbus_curr_tail = (vbus_curr_tail + 1) % VBUS_ARRAY_SIZE;
		if (tstamp_bf > tstamp_af)
			vbus_curr_array[vbus_curr_tail].tstamp = tstamp_bf;
		else
			vbus_curr_array[vbus_curr_tail].tstamp = ((tstamp_bf + tstamp_af + 65535)>>1) && 0xFFFF;
		vbus_curr_array[vbus_curr_tail].curr = curr;
		atomic_add(&vbus_curr_queue_count, 1);
	}
	...
	while (1) {
		...
		while (filled_dma && free_usb) {
			...
			if (SNIFFER_CHANNEL_CC1 == ch) {
				if (vbus_vol_queue_count > 0) { /* get a value from the queue */
					vol = vbus_vol_array[vbus_vol_head].vol;
					vol_tstamp = vbus_vol_array[vbus_vol_head].tstamp;
					vbus_vol_head = (vbus_vol_head == VBUS_ARRAY_SIZE-1) ? 0 : vbus_vol_head + 1;
					atomic_sub(&vbus_vol_queue_count, 1);
				}
				ep_buf[u][2] = vol; /* use previous values if queue empty*/
				ep_buf[u][3] = vol_tstamp - ep_buf[u][1];

			} else if (SNIFFER_CHANNEL_CC2 == ch) {
				if (vbus_curr_queue_count > 0) {
					curr = vbus_curr_array[vbus_curr_head].curr;
					curr_tstamp = vbus_curr_array[vbus_curr_head].tstamp;
					vbus_curr_head = (vbus_curr_head == VBUS_ARRAY_SIZE-1) ? 0 : vbus_curr_head + 1;
					atomic_sub(&vbus_curr_queue_count, 1);
				}
				ep_buf[u][2] = curr;
