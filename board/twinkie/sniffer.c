...
/* Is the endpoint initialized for transfer ? */
static int is_ep_ready;
...
{
	uint16_t ep = STM32_USB_EP(USB_EP_SNIFFER);
	/* clear IT */
	STM32_USB_EP(USB_EP_SNIFFER) = (ep & EP_MASK);
	/* TODO wake up the processing */
	//task_set_event(TASK_ID_SNIFFER, TASK_EVENT_CUSTOM(1), 0);
	return;
}
...
static const struct dma_option dma_tim_option = {
	...
	STM32_DMA_CCR_MSIZE_8_BIT | STM32_DMA_CCR_PSIZE_8_BIT | STM32_DMA_CCR_CIRC,
};
...
void sniffer_task(void)
{
	while (1) {
		...
		int off = evt & STM32_DMA_ISR_TCIF(DMAC_TIM_RX)
			  ? RX_COUNT/2 : 0;
		/* data from "off" to "off + RX_COUNT/2 - 1" */
		memcpy_usbram(ep_buf[0], samples+off, EP_BUF_SIZE/*RX_COUNT/2*/);
#if 0  /* TODO TODO TODO*/
	btable_ep[USB_EP_SNIFFER].tx_count = EP_BUF_SIZE;
	/* enable TX */
	STM32_TOGGLE_EP(USB_EP_SNIFFER, EP_TX_MASK, EP_TX_VALID, 0);
#endif /* TODO TODO TODO*/
	}
}
