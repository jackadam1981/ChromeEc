 ...
 *
 * SPI1 and SPI2 use SYSCLK (110 MHz) as base frequency.
 ...
 * The last one is SYSCLK.
 */
static const uint32_t base_frequencies[13] = {
	100000,
	200000,
	400000,
	800000,
...
static int command_spi_set_speed(int argc, const char **argv)
{
	...
	if (spi_devices[index].usb_flags & USB_SPI_CUSTOM_SPI_DEVICE) {
		...
		find_best_divisor(
			desired_speed, base_frequencies, NUM_MSI_FREQUENCIES + 1,
			&best_divisor, &best_base_frequency_index);
		...
		if (best_base_frequency_index < NUM_MSI_FREQUENCIES) {
			...
			STM32_RCC_CR = (STM32_RCC_CR & ~STM32_RCC_CR_MSIRANGE_MSK)
				| (best_base_frequency_index << STM32_RCC_CR_MSIRANGE_POS);
