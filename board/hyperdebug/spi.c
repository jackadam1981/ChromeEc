				    ...
				    uint8_t *rxdata, int rxlen)
{
	...
	if (opcode_len == 0) {
		...
		if (eeprom_flags & EEPROM_FLAG_OPCODE_DTR)
	...
	if (addr_len == 0) {
		...
		if (eeprom_flags & EEPROM_FLAG_ADDR_DTR)
	...
	if (alt_len == 0) {
		...
		if (eeprom_flags & EEPROM_FLAG_ALT_DTR)
	...
	if (data_len == 0) {
		...
		if (eeprom_flags & EEPROM_FLAG_DATA_DTR)
			control_value |= STM32_OCTOSPI_CCR_DDTR;
	}

	ccprintf(
		"EEPROM flags: 0x%08x opcode:%x:%d, addr:%x:%d, alt:%x:%d, dummy:%d, data:%d\n",
		eeprom_flags, opcode, opcode_len, address, addr_len, alternate,
