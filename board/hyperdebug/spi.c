				    ...
				    int txlen, uint8_t *rxdata, int rxlen)
{
	...
	uint8_t mode = flash_flags & FLASH_FLAG_MODE_MSK;
	uint8_t width = (flash_flags & FLASH_FLAG_WIDTH_MSK) >>
	...
	if (opcode_len == 0) {
		...
		control_value |= (opcode_len - 1) << STM32_OCTOSPI_CCR_ISIZE_POS;
		if (mode < FLASH_FLAG_MODE_NNN) {
			// Opcode phase is single-lane
			control_value |=
				1 << STM32_OCTOSPI_CCR_IMODE_POS;
		} else {
			// Opcode phase is multi-lane
			control_value |=
				(width + 2) << STM32_OCTOSPI_CCR_IMODE_POS;
	...
	if (addr_len == 0) {
		...
		if (mode < FLASH_FLAG_MODE_1NN) {
			...
			control_value |= (width + 2) << STM32_OCTOSPI_CCR_ADMODE_POS;
			if (flash_flags & FLASH_FLAG_DTR)
...
*/
	...
	if (data_len == 0) {
		...
		if (mode < FLASH_FLAG_MODE_11N) {
			...
			control_value |= (width + 2) << STM32_OCTOSPI_CCR_DMODE_POS;
			if (flash_flags & FLASH_FLAG_DTR)
