 ...
 * QSPI uses either SYSCLK (110 MHz) or MSI (4 MHz) as base frequency.
 *
...
uint32_t octospi_clock(void)
{
	...
	case STM32_RCC_CCIPR2_OSPISEL_MSI:
		switch (STM32_RCC_CR & STM32_RCC_CR_MSIRANGE_MSK) {
		case STM32_RCC_CR_MSIRANGE_0:
			return 100000;
		case STM32_RCC_CR_MSIRANGE_1:
			return 200000;
		case STM32_RCC_CR_MSIRANGE_2:
			return 400000;
		case STM32_RCC_CR_MSIRANGE_3:
			return 800000;
		case STM32_RCC_CR_MSIRANGE_4:
			return 1000000;
		case STM32_RCC_CR_MSIRANGE_5:
			return 2000000;
		case STM32_RCC_CR_MSIRANGE_6:
			return 4000000;
		case STM32_RCC_CR_MSIRANGE_7:
			return 8000000;
		case STM32_RCC_CR_MSIRANGE_8:
			return 16000000;
		case STM32_RCC_CR_MSIRANGE_9:
			return 24000000;
		case STM32_RCC_CR_MSIRANGE_10:
			return 32000000;
		case STM32_RCC_CR_MSIRANGE_11:
			return 48000000;
		default:
			return 0;
		}
...
static int command_spi_set_speed(int argc, const char **argv)
{
	...
	if (spi_devices[index].usb_flags & USB_SPI_CUSTOM_SPI_DEVICE) {
		 ...
		 * (110 MHz) or MSI (4 MHz), attempt calculation with both, and
		 * see which one gets closest to the requested frequency,
		 * without exceeding.
		 */
		...
		uint32_t msi_divisor = STM32_MSI_CLOCK / (desired_speed + 1);
		uint32_t msi_based_freq = STM32_MSI_CLOCK / (msi_divisor + 1);

		int divisor;
		if (sysclk_divisor >= 256 ||
		    msi_based_freq > sysclk_based_freq) {
			/*
