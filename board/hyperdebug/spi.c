 ...
 * every spi device is declared as requiring board specific driver.
 */
 ...
 * followed by a number of data bytes.  Respects TPM protocol code polling
 * when data phase can proceed, as well as optionally Google proprietary
 * "ready signal".
 */
 ...
 * every spi device is declared as requiring board specific driver.
 *
 * Handle certain non-standard bits in `flash_flags` by invoking logic tailored
 * to TPM SPI devices.
 */
