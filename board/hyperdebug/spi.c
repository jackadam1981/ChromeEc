 ...
 * a "ready pulse" on a particular pin, before reporting success.  (TODO: Or
 * between write and read phase, in case there is a read phase.)
 *
...
/*
 * This flag indicates that the SPI request is for a TPM, that is, as the fourth
 * and final command byte is written to the SPI bus, the controller must pay
 * attention to the output fromthe SPI device.  If the byte has the value 0x01,
 * it means that the transaction can immediately proceed (read or write), of the
 * value is 0x00, then the controller must continue polling, until a value 0x01
				   ...
				   int txlen, uint8_t *rxdata, int rxlen)
{
	...
	if (flash_flags & FLASH_FLAG_TPM_WAIT_FOR_READY) {
		if (gsc_ready_pin == GPIO_COUNT) {
			/*
			 * Waiting for ready pulse was requested, but ready pin
			 * not declared.
			 */
			return USB_SPI_UNSUPPORTED_FLASH_MODE;
		}
	...
	/* TPM protocol, poll after transmitting 4-byte command/address. */
	if (txlen < 4)
		return USB_SPI_UNSUPPORTED_FLASH_MODE;
			      ...
			      int txlen, uint8_t *rxdata, int rxlen)
{
	if (flash_flags & FLASH_FLAG_TPM) {
		/* Taylored logic for TPM transactions. */
