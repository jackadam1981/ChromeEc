...
static const struct dma_option dma_optospi_option = {
	STM32_DMAC_CH13, (void *)&STM32_OCTOSPI_DR,
				    ...
				    uint8_t *rxdata, int rxlen)
{
	...
	uint8_t alt_len = (flash_flags & FLASH_FLAG_ALT_LEN_MSK) >>
			  FLASH_FLAG_ALT_LEN_POS;
	uint8_t dummy_cycles = (flash_flags & FLASH_FLAG_DUMMY_CYCLES_MSK) >>
	...
	if (!flash_flags) {
		/*
		 * This is a request for doing plain write-then-read of single
		 * lane (COPI/CIPO) SPI date.
		 */
	...
	if (opcode_len == 0) {
		...
		uint32_t mode = (flash_flags & FLASH_FLAG_OPCODE_WIDTH_MSK) >>
				FLASH_FLAG_OPCODE_WIDTH_POS;
		control_value |= (mode + 1) << STM32_OCTOSPI_CCR_IMODE_POS |
				 (opcode_len - 1)
					 << STM32_OCTOSPI_CCR_ISIZE_POS;
		if (flash_flags & FLASH_FLAG_OPCODE_DTR)
	...
	if (addr_len == 0) {
		...
		uint32_t mode = (flash_flags & FLASH_FLAG_ADDR_WIDTH_MSK) >>
				FLASH_FLAG_ADDR_WIDTH_POS;
		control_value |= (mode + 1) << STM32_OCTOSPI_CCR_ADMODE_POS |
				 (addr_len - 1) << STM32_OCTOSPI_CCR_ADSIZE_POS;
		if (flash_flags & FLASH_FLAG_ADDR_DTR)
	...
	if (alt_len == 0) {
		...
		uint32_t mode = (flash_flags & FLASH_FLAG_ALT_WIDTH_MSK) >>
				FLASH_FLAG_ALT_WIDTH_POS;
		control_value |= (mode + 1) << STM32_OCTOSPI_CCR_ABMODE_POS |
				 (alt_len - 1) << STM32_OCTOSPI_CCR_ABSIZE_POS;
		if (flash_flags & FLASH_FLAG_ALT_DTR)
	...
	if (data_len == 0) {
		...
		uint32_t mode = (flash_flags & FLASH_FLAG_DATA_WIDTH_MSK) >>
				FLASH_FLAG_DATA_WIDTH_POS;
		control_value |= (mode + 1) << STM32_OCTOSPI_CCR_DMODE_POS;
		if (flash_flags & FLASH_FLAG_DATA_DTR)
			control_value |= STM32_OCTOSPI_CCR_DDTR;
	}

	/*ccprintf(
		"FLASH flags: 0x%08x opcode:%x:%d, addr:%x:%d, alt:%x:%d, dummy:%d, data:%d\n",
		flash_flags, opcode, opcode_len, address, addr_len, alternate,
		alt_len, dummy_cycles, data_len);*/
	...
	if ((flash_flags & FLASH_FLAG_READ_WRITE_MSK) ==
		...
		if (txlen > 0) {
			...
			dma_go(txdma);
			gpio_set_level(GPIO_CN10_34, 0);
		} else {
			gpio_set_level(GPIO_CN10_34, 0);
			gpio_set_level(GPIO_CN10_33, 1);
		}
	} else {
		if (rxlen > 0) {
...
{
	if (dma_is_enabled(dma_get_channel(STM32_DMAC_CH13))
	    && !dma_is_complete(STM32_DMAC_CH13))
		return 0;
	return 1;
}
