...
static uint32_t jtag_clock_hz = DEFAULT_JTAG_CLOCK_HZ;
static uint16_t jtag_half_period_count = CPU_CLOCK / DEFAULT_JTAG_CLOCK_HZ / 2;
...
static void dap_swj_clock(void)
{
	...
	const uint32_t OVERHEAD_CLOCK_CYCLES = 50;

	if (queue_count(&cmsis_dap_rx_queue) < 5)
		return;
	queue_remove_units(&cmsis_dap_rx_queue, rx_buffer, 7);

	memcpy(&new_clock_hz, rx_buffer + 1, sizeof(jtag_clock_hz));
	...
	if (!new_clock_hz) {
		 ...
		 *
		 * Empirically, it has been stablished that about CPU 50 clock
		 * cycles are used by execution of GPIO manipulations involved
		 * in clock toggling and data shifting, so we subtract that from
		 * the number of cycles that will be "burned" while generating
		 * the waveform.
		 */
		if (new_half_period_count <= OVERHEAD_CLOCK_CYCLES) {
			/* At the limit of capabilities, no delay at all. */
