...
/*
 * If this is any value other than TASK_ID_INVALID, it means that the given task
 * (typically HOOKS or CONSOLE) has requested that the CMSIS_DAP task abort any
 * partially received request or partially sent response, and return to its main
 * loop ASAP.
 */
...
static K_MUTEX_DEFINE(reinit_mutex);

static void cmsis_dap_reinit(void)
{
	mutex_lock(&reinit_mutex);
