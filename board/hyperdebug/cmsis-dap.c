...
static void dap_swj_pins(void)
{
	...
	if ((rx_buffer[2] & PIN_SwClk_Tck) && shield_reset_pin != GPIO_COUNT)
		gpio_set_level(jtag_pins[JTAG_TCLK], !!(rx_buffer[1] & PIN_SwClk_Tck));
	if ((rx_buffer[2] & PIN_SwDio_Tms) && shield_reset_pin != GPIO_COUNT)
		gpio_set_level(jtag_pins[JTAG_TMS], !!(rx_buffer[1] & PIN_SwDio_Tms));
	...
	if ((rx_buffer[2] & PIN_Trst) && shield_reset_pin != GPIO_COUNT)
		gpio_set_level(jtag_pins[JTAG_TRSTn], !!(rx_buffer[1] & PIN_Trst));
...
static void dap_jtag_sequence(void)
{
	...
	const uint32_t jtag_clk_mask_clear = gpio_list[jtag_pins[JTAG_TCLK]].mask << 16;
	...
	const uint32_t jtag_tms_mask_clear = gpio_list[jtag_pins[JTAG_TMS]].mask << 16;
	...
	const uint32_t jtag_tdi_mask_clear = gpio_list[jtag_pins[JTAG_TDI]].mask << 16;
...
static struct queue const cmsis_dap_rx_queue =
	QUEUE_DIRECT(sizeof(rx_buffer), uint8_t, cmsis_dap_usb.producer,
		     cmsis_dap_consumer);
