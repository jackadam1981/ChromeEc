...
static void i2c_interrupt(int index)
{
	...
	if ((isr & STM32_I2C_ISR_ADDR) && (STM32_I2C_CR1(index) & STM32_I2C_CR1_ADDRIE)) {
	...
	if (isr & STM32_I2C_ISR_TIMEOUT) {
		 ...
		 */
		uint32_t prev_cr1 = STM32_I2C_CR1(index);
		...
		STM32_I2C_CR1(index) = prev_cr1;
		STM32_I2C_ICR(index) = STM32_I2C_ICR_TIMEOUTCF;
	}
