 ...
 *
 * Actual I2C data is tunelled through CMSIS-DAP using one of two "vendor
 * extension" commands.
 *
 ...
 * flags : Bitfield:
 *         01: Timeout, HyperDebug did not have response for I2C host
...
void dap_goog_i2c_device(size_t peek_c)
{
	...
	case 0: {
		...
		while (remaining > 0 && !state->blocked_read_addr &&
	...
	case 1:
		...
		if (state->blocked_read_addr) {
			/* I2C bus is currently awaiting ACK on our address,
			 * READ transfer. */
...
/*
>>>>>>> 4e13b90ced (board/hyperdebug: I2C device support)
...
static int command_i2c_set_mode(int argc, const char **argv)
{
	...
	if (!strcasecmp(mode, "host")) {
		STM32_I2C_CR1(index) &=
			~(STM32_I2C_CR1_TXIE | STM32_I2C_CR1_RXIE |
			  STM32_I2C_CR1_ADDRIE | STM32_I2C_CR1_NACKIE |
			  STM32_I2C_CR1_STOPIE | STM32_I2C_CR1_ERRIE);
