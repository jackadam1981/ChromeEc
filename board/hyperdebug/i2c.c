...
struct i2c_device_status_t {
	 ...
	 * 2: No data prepared, read in progress (currently blocked indefinitely)
	 */
	uint8_t read_status;
...
void dap_goog_i2c_device(size_t peek_c)
{
	...
	case 0: {
		...
		while (remaining > 0 && !state->blocked_read_addr && state->head == state->tail) {
			task_wait_event(remaining);
			remaining = deadline.val - get_time().val;
		}

		/* TODO, consider blocking interrupts while inspecting, to
		 * ensure that read_status and the transcript willbe
		 * consistent */
		struct i2c_device_status_t *status = (struct i2c_device_status_t *)(rx_buffer + 1);
		...
		if (state->tail <= head) {
			...
			queue_add_units(&cmsis_dap_tx_queue, rx_buffer, 1 + sizeof(*status));
			queue_add_units(&cmsis_dap_tx_queue, state->tail, status->transcript_size);
		} else {
			/* Data wraps around */
			status->transcript_size = head - state->tail + sizeof(state->data_buffer);
			queue_add_units(&cmsis_dap_tx_queue, rx_buffer, 1 + sizeof(*status));
			queue_add_units(&cmsis_dap_tx_queue, state->tail,
					state->data_buffer + sizeof(state->data_buffer) - state->tail);
	...
	case 1:
		...
		//ccprintf("Preparing %d bytes\n", len);
		/* TODO Check that len does not exceed size of prepared_data_data */
		queue_remove_units(&cmsis_dap_rx_queue, rx_buffer, 5);
		queue_remove_units(&cmsis_dap_rx_queue, state->prepared_read_data, len);
		queue_add_unit(&cmsis_dap_tx_queue, rx_buffer);
		state->prepared_read_len = len;
		if (state->blocked_read_addr) {
			/* I2C bus is currently awaiting ACK on our address, READ transfer. */
			 ...
			 * stretching.
			 */
			STM32_I2C_ICR(index) = BIT(3);
			/* Re-enable interrupt on START condition, for next transfer. */
			STM32_I2C_CR1(index) |= BIT(3);
		}
		break;
	}
	cflush();
...
static void i2c_interrupt(int index)
{
	...
	//gpio_set_level(GPIO_CN9_25, false);
	//ccprintf("i2c_interrupt[%d]() ISR=0x%08x\n", index, isr);
	struct i2c_state_t *state = &i2c_port_state[index];
		
	if ((isr & BIT(3)) && (STM32_I2C_CR1(index) & BIT(3))) {
		//gpio_set_level(GPIO_CN10_6, false);
		//ccprintf("address match %s\n", isr & BIT(16) ? "READ" : "WRITE");
		...
		if (state->cur_transfer) {
			...
			if (!(state->cur_transfer->addr & 0x01)) {
				...
				state->byte_head += -state->cur_transfer->num_bytes & 0x03;
				if (state->byte_head == state->data_buffer + sizeof(state->data_buffer))
		...
		if (isr & BIT(16)) {
			...
			/* Write transfer */
			STM32_I2C_ICR(index) = BIT(3) | BIT(0);
			/* Discard any prepared data, may be stale. */
			state->prepared_read_len = 0;
		}
		//gpio_set_level(GPIO_CN10_6, true);
		//gpio_set_level(GPIO_CN9_25, true);
	}
	if (isr & BIT(2)) {
	...
	if (isr & BIT(12)) {
		...
		STM32_I2C_CR1(index) = BIT(0) | BIT(1) | BIT(2) | BIT(3) | BIT(4) | BIT(5) | BIT(7);
		STM32_I2C_ICR(index) = BIT(12);
		//gpio_set_level(GPIO_CN10_6, true);
		//gpio_set_level(GPIO_CN9_25, true);
	}
	if (isr & BIT(1)) {
		//gpio_set_level(GPIO_CN10_6, false);
		if (state->cur_transfer->num_bytes >= state->prepared_read_len) {
			STM32_I2C_TXDR(index) = 0xFF;
			//ccprintf("Sent additional byte\n");
		} else {
			uint8_t data_byte = state->prepared_read_data[state->cur_transfer->num_bytes];
			STM32_I2C_TXDR(index) = data_byte;
			//ccprintf("Sent a byte %02x\n", data_byte);
		}
		state->cur_transfer->num_bytes++;
		//gpio_set_level(GPIO_CN10_6, true);
		//gpio_set_level(GPIO_CN9_25, true);
	...
	if (isr & BIT(4)) {
		//ccprintf("NAK\n");
		state->cur_transfer->num_bytes--;
		STM32_I2C_ICR(index) = BIT(4);
		//gpio_set_level(GPIO_CN9_25, true);
	}
	if (isr & BIT(5)) {
		//ccprintf("STOP\n");
		STM32_I2C_ICR(index) = BIT(5);
		STM32_I2C_ISR(index) = BIT(0);
		if (!(state->cur_transfer->addr & 0x01)) {
			/* Write transaction, pad data to align by 4 bytes. */
			state->byte_head += -state->cur_transfer->num_bytes & 0x03;
			if (state->byte_head == state->data_buffer + sizeof(state->data_buffer))
...
static void i2c_interrupt_1(void)
{
	i2c_interrupt(0);
}

static void i2c_interrupt_2(void)
{
	i2c_interrupt(1);
}

static void i2c_interrupt_3(void)
{
	i2c_interrupt(2);
}

static void i2c_interrupt_4(void)
{
	i2c_interrupt(3);
}

DECLARE_IRQ(STM32_IRQ_I2C1_EV, i2c_interrupt_1, 1);
DECLARE_IRQ(STM32_IRQ_I2C2_EV, i2c_interrupt_2, 1);
DECLARE_IRQ(STM32_IRQ_I2C3_EV, i2c_interrupt_3, 1);
DECLARE_IRQ(STM32_IRQ_I2C4_EV, i2c_interrupt_4, 1);
DECLARE_IRQ(STM32_IRQ_I2C1_ER, i2c_interrupt_1, 1);
DECLARE_IRQ(STM32_IRQ_I2C2_ER, i2c_interrupt_2, 1);
DECLARE_IRQ(STM32_IRQ_I2C3_ER, i2c_interrupt_3, 1);
DECLARE_IRQ(STM32_IRQ_I2C4_ER, i2c_interrupt_4, 1);
...
static void print_i2c_info(int index)
{
	...
	ccprintf("  %d %s %d bps %s\n", index, i2c_ports[index].name,
		 i2c_port_state[index].bits_per_second,
		 is_device ? "d" : "h");
...
static int command_i2c_set_mode(int argc, const char **argv)
{
	...
	if (!strcasecmp(mode, "host")) {
		...
		STM32_I2C_CR1(index) |= BIT(1) | BIT(2) | BIT(3) | BIT(4) | BIT(5) | BIT(7);
...
static void board_i2c_init(void)
{
	...
	for (unsigned int i = 0; i < i2c_ports_used; i++) {
		...
		i2c_port_state[i].tail = i2c_port_state[i].head = i2c_port_state[i].data_buffer;
	}
}
