...
/******************************************************************************
 * Forward USART9 as a simple USB serial interface.
 */

#ifdef CONFIG_STREAM_USART9
...
const struct spi_device_t spi_devices[] = {
	{ /*CONFIG_SPI_FLASH_PORT*/ 1, 7, GPIO_SPI2_CS},
};
...
void hyper_task(void *u)
{
	while (1) {
		//usleep(1);
		//STM32_GPIO_BSRR(STM32_GPIOE_BASE) |= 0x08000000;
		task_wait_event(-1);  /* Wait for more input */
		//STM32_GPIO_BSRR(STM32_GPIOE_BASE) |= 0x00000800;
		ccprintf("h");
	}
}

static void tim3_interrupt(void)
{
	uint32_t stat = STM32_TIM_SR(3);
	STM32_GPIO_BSRR(STM32_GPIOE_BASE) |= 0x00000400;

	if (stat & 1) { /* Counter overflow */
		/*uint16_t val = STM32_TIM_ARR(3);
		if (val == 0x0200) {
			STM32_TIM_ARR(3) = 0x1FFF;
		} else {
			STM32_TIM_ARR(3) = val - 1;
			}*/
		overflow_count++;
		//ccprintf("o");
		task_wake(TASK_ID_HYPER);
	}

	STM32_TIM_SR(3) = ~stat & 3; /* clear interrupt flags */
	task_clear_pending_irq(STM32_IRQ_TIM3);
	STM32_GPIO_BSRR(STM32_GPIOE_BASE) |= 0x04000000;
}
DECLARE_IRQ(STM32_IRQ_TIM3, tim3_interrupt, 1);
#endif
static void timers_init(void)
{
	...
	STM32_TIM_ARR(3) = 0x1FFF;
	STM32_TIM_PSC(3) = CPU_CLOCK / 1000000 - 1;
	STM32_TIM_EGR(3) = 0x0001; /* Reload the pre-scaler */
	STM32_TIM_CR1(3) = 1;
	STM32_TIM_CNT(3) = 0x00000000;
	STM32_TIM_SR(3) = 0; /* Clear pending interrupts */
	STM32_TIM_DIER(3) = 1; /* Overflow interrupt */
...
static int command_gpio_cfg(int argc, char **argv)
{
	int gpio;
	int flags = 0;
	int af = -1;
...
DECLARE_CONSOLE_COMMAND_FLAGS(gpiocfg, command_gpio_cfg,
