...
static void octospi_write(const uint8_t *txdata, int txlen)
{
...
static void octospi_read(const uint8_t *txdata, int txlen,
...
/*
 * Board-specific SPI driver.  In the case of HyperDebug, the only spi device
 * declared as requiring board specific driver is OCTOSPI.
 */
			      ...
			      int rxlen)
{
	 ...
	 * type of operation can be preceeded by up to 12 bytes of
	...
	if (rxlen == SPI_READBACK_ALL) {
		 ...
		 * OctoSPI.
		 */
		octospi_read(txdata, txlen, rxdata, rxlen);
	} else {
		/*
		 * Sending many bytes, followed by reading.  This is implemented
		 * as two separate OctoSPI transactions.
		 */
		octospi_write(txdata, txlen);
		octospi_read(NULL, 0, rxdata, rxlen);
	}
