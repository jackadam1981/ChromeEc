...
/* Additional board-specific information for each SPI device. */
const char *spi_device_names[ARRAY_SIZE(spi_devices)] = {
	"SPI2", "QSPI"
};
...
/*
 * Timeout for complete SPI transaction.  Users can potentially set the clock
 ...
 */
static int octospi_wait_for(uint32_t flags, uint32_t start)
{
	while ((STM32_OCTOSPI_SR & flags) != flags) {
		uint32_t elapsed = __hw_clock_source_read() - start;
		if (elapsed >= OCTOSPI_TRANSACTION_TIMEOUT_US)
...
static int octospi_indirect_write(const uint8_t *txdata, int txlen)
{
	...
	for (int i = 0; i < txlen; i += 4) {
		...
		if ((rv = octospi_wait_for(STM32_OCTOSPI_SR_FTF, start)))
			return rv;
		STM32_OCTOSPI_DR = value;
	}
	/* Wait for transaction completion flag. */
	return octospi_wait_for(STM32_OCTOSPI_SR_TCF, start);
}
 ...
 */
static int octospi_indirect_read(const uint8_t *txdata, int txlen, uint8_t *rxdata,
				 int rxlen)
{
	uint32_t start = __hw_clock_source_read();
	...
	if (txlen == 0) {
		...
		for (int i = 0; i < 4; i++) {
			address <<= 8;
			address |= txdata[txlen - 8 + i];
		}
		for (int i = 0; i < 4; i++) {
			alternate <<= 8;
			alternate |= txdata[txlen - 4 + i];
		}
			      ...
			      int rxlen)
{
	...
	if (rxlen == SPI_READBACK_ALL) {
		cprints(CC_SPI, "Full duplex not supported by OctoSPI hardware");
		...
		/*
		 * Sending of up to 8 bytes, followed by reading a possibly
...
static int find_spi_by_name(const char *name)
{
	...
	for (i = 0; i < spi_devices_used; i++) {
		if (!strcasecmp(name, spi_device_names[i]))
			return i;
	}
...
DECLARE_CONSOLE_COMMAND_FLAGS(spisetspeed, command_spi_set_speed, "name bits_per_second",
			      "Get SPI configuration", CMD_FLAG_RESTRICTED);
