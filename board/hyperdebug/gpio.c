...
const struct dac_t dac_channels[GPIO_COUNT] = {
	[GPIO_CN7_9] = { STM32_DAC_CR_EN1, &STM3_DAC_DHR12R1 },
...
struct monitoring_slot_t {
	...
	int gpio_signal;
	/* Most recently recorded level of the signal. (0: low, gpio_pin_mask: high) */
	volatile uint32_t head_level;
	/* Level as of the current oldest end (tail) of the recording. (0: low, 1: high) */
	uint8_t tail_level;
...
void gpio_edge(enum gpio_signal signal)
{
	...
	uint32_t current_level = STM32_GPIO_IDR(slot->gpio_base) & slot->gpio_pin_mask;
	struct cyclic_buffer_header_t *buffer_header = slot->buffer;
