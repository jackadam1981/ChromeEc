...
/* Size of buffer used for bitbanging. */
...
static void board_gpio_init(void)
{
	...
	/*
	 * Prepare TIMER5 for use in GPIO bit-banging.
	 */
 	STM32_RCC_APB1ENR1 |= STM32_RCC_APB1ENR1_TIM5EN;
...
{
	//STM32_GPIO_BSRR(STM32_GPIOA_BASE) = BIT(9); /* PA9 high */
	uint32_t triggered = STM32_TIM_SR(BITBANG_TIMER);
	if (!(triggered & 1)) {
		ccprintf("IRQ %08x\n", triggered);
		//STM32_GPIO_BSRR(STM32_GPIOA_BASE) = BIT(9 + 16); /* PA9 low */
		return;
	}
	...
	if (data_byte & 0x80) {
		...
		while (data_byte & 0x80) {
			...
			if (bitbang_ptr == bitbang_data + sizeof(bitbang_data))
				bitbang_ptr = bitbang_data;
			bitbang_countdown += ((data_byte & 0x7F) << delay_scale);
			delay_scale += 7;
		}
		bitbang_countdown--; /* One cycle of delay already spent processing */
		//STM32_GPIO_BSRR(STM32_GPIOA_BASE) = BIT(9 + 16); /* PA9 low */
		return;
	}
	...
	for (uint8_t i = 0; i < num_bitbang_pins; i++) {
		input_data |= !!(STM32_GPIO_IDR(bitbang_pin_bases[i]) & bitbang_pin_masks[i]) << i;
	}
	/*
	...
	for (uint8_t i = 0; i < num_bitbang_pins; i++) {
		if (data_byte & (1 << i)) {
			STM32_GPIO_BSRR(bitbang_pin_bases[i]) = bitbang_pin_masks[i];
		} else {
			STM32_GPIO_BSRR(bitbang_pin_bases[i]) = bitbang_pin_masks[i] << 16;
		}
	}
	//STM32_GPIO_BSRR(STM32_GPIOA_BASE) = BIT(9 + 16); /* PA9 low */
}
...
void dap_goog_gpio_bitbang(size_t peek_c, bool streaming)
{
	...
	if (bitbang_tail + data_len < bitbang_data + sizeof(bitbang_data)) {
		queue_blocking_remove(&cmsis_dap_rx_queue, bitbang_tail, data_len);
		bitbang_tail += data_len;
	} else {
		uint16_t remaning_space = bitbang_data + sizeof(bitbang_data) - bitbang_tail;
		queue_blocking_remove(&cmsis_dap_rx_queue, bitbang_tail, remaning_space);
		queue_blocking_remove(&cmsis_dap_rx_queue, bitbang_data, data_len - remaning_space);
	...
	if (!(timer_cr1 & 1) && bitbang_tail != bitbang_ptr) {
		/*
		 * First few interrupts have higher latency, in order to avoid
		 * jitter in the bit-banged waveform, set up such that the first
		 * three timer interrupts do nothing.
		 *
		 * TODO: Only at high clock rates.
		 */
		bitbang_countdown = 3;
	...
	if (!streaming) {
		while (STM32_TIM_CR1(BITBANG_TIMER) & 1)
			;
	} else {
		uint16_t used_bytes = bitbang_dist(bitbang_head, bitbang_tail);
		while (bitbang_dist(bitbang_head, bitbang_ptr) < (used_bytes + 1) / 2)
			;
	}
	...
	*(uint16_t*)(tx_buffer + 2) = free_bytes;
	...
	if (bitbang_head <= ptr) {
		*(uint16_t*)(tx_buffer + 4) = ptr - bitbang_head;
		...
		queue_blocking_add(&cmsis_dap_tx_queue, bitbang_head, ptr - bitbang_head);
	} else {
		*(uint16_t*)(tx_buffer + 4) = ptr + sizeof(bitbang_data) - bitbang_head;
		queue_add_units(&cmsis_dap_tx_queue, tx_buffer, 6);
		queue_blocking_add(&cmsis_dap_tx_queue, bitbang_head, bitbang_data + sizeof(bitbang_data) - bitbang_head);
		queue_blocking_add(&cmsis_dap_tx_queue, bitbang_data, ptr - bitbang_data);
	}
	bitbang_head = ptr;
}
