 ...
 * values of the involved pins as seen just before the waveform data was
...
/*
 * Pointer indicating how far the interrupt handler can process, usually will be
 * identical to bitbang_tail, but may lag by a few bytes, in cases when a
 * multi-byte delay encoding has been only partially received from the USB host.
 */
...
static uint8_t validate_received_waveform(uint16_t data_len, bool streaming)
{
	...
	while (idx != tail_goal) {
		if (*bitbang_data_ptr(idx) & BITBANG_DELAY_BIT) {
			uint8_t delay_scale = 0;
			while (idx != tail_goal && *bitbang_data_ptr(idx) & BITBANG_DELAY_BIT) {
				if ((*bitbang_data_ptr(idx) & BITBANG_DATA_MASK)
				    >> (32 - delay_scale)) {
					/* Delay does not fit in 32-bit unsigned integer. */
					ccprintf("reject A %d %d %d %d\n", bitbang_head, bitbang_irq, bitbang_irq_tail, bitbang_tail);
...
void dap_goog_gpio_bitbang(size_t peek_c, bool streaming)
{
	...
	ccprintf("req(%d)  %d %d %d %d\n", data_len, bitbang_head, bitbang_irq, bitbang_irq_tail, bitbang_tail);
	...
	if (status != 0) {
		...
		uint16_t free_bytes = bitbang_irq + BITBANG_BUFFER_SIZE - bitbang_tail;
	...
	ccprintf("recv     %d %d %d %d\n", bitbang_head, bitbang_irq, bitbang_irq_tail, bitbang_tail);

	uint32_t timer_cr1 = STM32_TIM_CR1(BITBANG_TIMER);
	if (!(timer_cr1 & STM32_TIM_CR1_CEN) && bitbang_irq_tail != bitbang_irq) {
		/*
		...
		if (divisor > counts_in_1ms) {
			 ...
			 * seconds.
			 */
			STM32_TIM32_CNT(BITBANG_TIMER) = divisor - counts_in_1ms;
			bitbang_countdown = 0;
		} else {
			/*
	...
	tx_buffer[1] = bitbang_head != bitbang_tail ? STATUS_BITBANG_ONGOING : STATUS_BITBANG_IDLE;
