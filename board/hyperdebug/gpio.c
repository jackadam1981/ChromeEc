...
void user_button_edge(enum gpio_signal signal)
{
	...
	if (pressed) {
		/* Blue user button pressed, assert the user-specified reset
		 * signal. */
		gpio_set_level(shield_reset_gpio, false);
	} else {
		/* Blue user button released, deassert the user-specified reset
		 * signal. */
...
static int command_reinit(int argc, const char **argv)
{
	 ...
	 * pin is used for RESET on both Dauntless shield and OpenTitan
	 * shield.
	 */
