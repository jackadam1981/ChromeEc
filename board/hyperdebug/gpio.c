 ...
 */
static int shield_reset_gpio = GPIO_CN10_29;
...
void user_button_edge(enum gpio_signal signal)
{
	...
	if (pressed) {
		/* Blue user button pressed, assert the user-specified reset signal. */
		gpio_set_level(shield_reset_gpio, false);
	} else {
		/* Blue user button released, deassert the user-specified reset signal. */
		gpio_set_level(shield_reset_gpio, true);
	}
}
...
static void stop_all_gpio_monitoring(void)
{
	...
	/* Ensure handling of the blue user button of Nucleo-L552ZE-Q is enabled. */
	gpio_clear_pending_interrupt(GPIO_NUCLEO_USER_BTN);
	gpio_enable_interrupt(GPIO_NUCLEO_USER_BTN);
}
...
/*
 * Choose the pin that should be pulled low when the blue user button is pressed.
 */
...
static int command_reinit(int argc, const char **argv)
{
	...
	/* Default behavior of blue user button. */
