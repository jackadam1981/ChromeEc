 ...
 */
bstatic volatile uint32_t bitbang_irq_tail = 0;
...
static int command_gpio_pwm(int argc, const char **argv)
{
	...
	if (strcasecmp(argv[3], "off") == 0) {
		...
		/* Clear output ensable bit for this channel. */
	...
	// TODO: scale divisor to fit in 16-bit field, using prescaler if
	// necessary, (sharing code with bitbanging above).  Verify that if
	// timer in use already, it uses same frequency, parse duty cycle, and
	// enable timer and channel.
	...
	if (prescaler_shift > 0) {
		// Divide by power of two, rounding to nearest.
		high_count = (high_count >> prescaler_shift) +
			     ((high_count >> (prescaler_shift - 1)) & 1);
	}
	...
	tim->ccr[pwm_pins[gpio].channel] = high_count;
