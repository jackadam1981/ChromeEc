 ...
 *
 * So conceptually the buffer entries are pairs of (u64, u8).  These entries
 * are encoded as bytes in the following way: First the timestamp u64 is
 * shifted left by signal_bits, and the value of the u8 is put into the lower
 * bits freed up this way.  Now we have a single u64, which often will be a
 * small value (or at least, when the edges happen rapidly, and the need to
