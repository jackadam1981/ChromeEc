...
static const uint8_t *traverse_buffer(struct cyclic_buffer_header_t *buf,
				      int gpio_signals_by_no[], timestamp_t now,
				      size_t limit, bool print);
...
static int command_gpio_monitoring_read(int argc, const char **argv)
{
	 ...
	 */
	buf->tail =
		traverse_buffer(buf, gpio_signals_by_no, get_time(), 32, true);
	return EC_SUCCESS;
}

/*
 * This routine iterates through buffered entries starting from buf->tail,
 * stopping when reaching an entry beyond the `now` timestamp, or when having
 * processed a certain number of entries given by `limit`, or when there are not
 * more entries in the cyclic buffer, whichever comes first.  The return value
 * indicate a new value which the caller must put into buf->tail.  As soon as
 * the caller does this, the traversed range is free to be overwritten by the
 * interrupt handler.
 */
static const uint8_t *traverse_buffer(struct cyclic_buffer_header_t *buf,
				      int gpio_signals_by_no[], timestamp_t now,
				      size_t limit, bool print)
{
	...
	while (tail != head && limit-- > 0) {
		...
		if (print) {
			/* To conserve bandwidth, timestamps are relative to
			 * `now`. */
			ccprintf("  %d %lld %s\n", signal_no,
				 tail_time.val - now.val,
				 slot->tail_level ? "F" : "R");
