 ...
 */
static int shield_reset_gpio = GPIO_COUNT; /* "no pin" value */
...
void user_button_edge(enum gpio_signal signal)
{
	...
	if (shield_reset_gpio < GPIO_COUNT)
		gpio_set_level(shield_reset_gpio, !pressed); /* Active low */
}
...
static int command_gpio_set_reset(int argc, const char **argv)
{
	...
	if (!strcasecmp(argv[2], "none")) {
		shield_reset_gpio = GPIO_COUNT; /* "no pin" value */
		return EC_SUCCESS;
	}		
