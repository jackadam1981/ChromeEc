...
/*
 * Size of buffer used for bitbanging waveform. (Actual useable part of buffer
...
static inline uint16_t bitbang_dist(uint8_t *volatile from, uint8_t *volatile to)
{
	if (to >= from)
		return to - from;
	else
...
/* Compute distance between two pointers in cyclic buffer, in bytes. */
static inline uint8_t *volatile bitbang_inc_ptr(uint8_t *volatile ptr)
{
	ptr++;
	if (ptr == bitbang_data + sizeof(bitbang_data))
		return bitbang_data;
	return ptr;
}
...
void IRQ_HANDLER(IRQ_TIM(BITBANG_TIMER))(void)
{
	...
	if (data_byte & BITBANG_DELAY_BIT) {
		...
		bitbang_countdown = 0;
		do {
			bitbang_ptr = bitbang_inc_ptr(bitbang_ptr);
			bitbang_countdown +=
 ...
 * data between bitbang_head and bitbang_ptr.
 */
void dap_goog_gpio_bitbang(size_t peek_c, bool streaming)
{
	...
	do {
		if (!streaming) {
			...
			uint16_t used_bytes =
				bitbang_dist(bitbang_head, bitbang_tail);
