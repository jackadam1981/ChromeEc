 ...
 * bytes with value 0x80, surrounded by bytes with the high bit of zero, is used
 ...
 * the given `pattern` for all bits which are set in the given `mask`.
 *
 * +----------------+---------------+-----------------+---------------+
...
void IRQ_HANDLER(IRQ_TIM(BITBANG_TIMER))(void)
{
	...
	if (bitbang_mask) {
		 ...
		 * generating the remaining part of the bitbang waveform.
		 */
