...
void sys_tick_handler(void)
{
	if (gpio_get_level(gsc_ready_pin)) {
		...
		if (gsc_ready_count == GSC_WAITING_FOR_HIGH_LEVEL)
			gsc_ready_count = GSC_WAITING_FOR_FALLING_EDGE;
	} else {
		if (gsc_ready_count == GSC_WAITING_FOR_FALLING_EDGE) {
			/* Low level after above, we detected falling edge. */
			gsc_ready_count = GSC_DETECTED_FALLING_EDGE;
...
{
	gsc_ready_pin = pin;
	gsc_ready_count = GSC_WAITING_FOR_HIGH_LEVEL;

	/* SysTick interrupt every 5 us, assuming 110MHz core clock. */
	CPU_NVIC_ST_RVR = 5 * 110 - 1;
...
{
	while (gsc_ready_count != GSC_DETECTED_FALLING_EDGE) {
		timestamp_t now = get_time();
