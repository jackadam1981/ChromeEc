...
static void queue_blocking_add(struct queue const *q, const void *src, size_t count)
{
	while (true) {
...
void dap_goog_gpio_monitoring(size_t peek_c)
{
	...
	case GPIO_REQ_MONITORING_READ: {
		...
		for (int i = 0; i < gpio_num; i++) {
			...
			queue_remove_units(&cmsis_dap_rx_queue, rx_buffer,
					   str_len);
			rx_buffer[str_len] = '\0';
		 ...
		 * in tx_buffer, such that our header struct will be 8-byte
		 * aligned.
		 */
		tx_buffer[7] = tx_buffer[0];
		...
		if (header->status != 0) {
			...
			queue_add_units(&cmsis_dap_tx_queue, tx_buffer + 7,
					1 + sizeof(*header));
			return;
		}
		...
		while (tail != head) {
			...
			if (tail_time.val + diff.val > now.val) {
				/*
				 * Do not consume this or subsequent records,
				 * which apparently happened after our "now"
				 * timestamp from earlier in the execution of
				 * this method.
				 */
				break;
			}
