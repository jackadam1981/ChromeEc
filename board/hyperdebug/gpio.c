...
/*
 * The arm architechture recognizes the "thumb" 16-bit instruction set by
 * setting the least significant bit of the instruction pointer.  The code still
 * is stored in 16-bit instructions at even addresses, but all function pointers
...
{
	size_t instruction_offset = instr->location - THUMB_CODE_TO_DATA_PTR(&edge_int);
...
static void board_gpio_init(void)
{
	...
	for (int i = 0; i < 16; i++) {
		 ...
		 */
		sram_vectors[16 + STM32_IRQ_EXTI0 + i] = DATA_TO_THUMB_CODE_PTR(&monitoring_slots[i].code);
	}
}
