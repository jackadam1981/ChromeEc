...
/* Function to retreive state of servo charger port */
...
/* Cache previous power in milliWats in order to detect changes */
static int old_pwr_mw;

int is_bc12_enabled(void)
{
...
void get_input_power(void)
{
	...
	if (gl3590_ufp_pwr(0, &host_hub_pwr)) {
		ccprintf("Cannot get host connection power data, assuming "
			 "5V/500mA\n");
	...
	if (get_alternate_port_pwr(&srv_chg_pwr))
		available_pwr = pwr_con_to_milliwats(&host_hub_pwr) >
				pwr_con_to_milliwats(&bc12_pwr) ?
					&host_hub_pwr : &bc12_pwr;
	else
		available_pwr = &srv_chg_pwr;

	if (pwr_con_to_milliwats(available_pwr) != old_pwr_mw) {
		ccprintf("Servo now powered %dV/%dmA", available_pwr->volts,
			 available_pwr->milli_amps);
		sr_chg_power_limit(pwr_con_to_milliwats(available_pwr) *
				   PWR_ALERT_TH / 100);
		old_pwr_mw = pwr_con_to_milliwats(available_pwr);
	}
}
