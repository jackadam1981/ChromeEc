...
static int is_pd_allowed(void)
{
	...
	CPRINTS("PD allowed [%d] Power [%d] DRP [%d] CC_CONFIG [%x]", allow_pd, current_power, current_drp, cc_config);
	return allow_pd;
}
...
static void tusb1064_tcpm_hook_disconnect(void)
{
	...
	/* Handle various mux disconnect cases */
	// TODO: Put this in a mux driver (for UFP)!
	CPRINTS("RESET data role is [%d] [st%d] [%s]", current_data, pd_get_task_state(port), pd_get_task_state_name(port));
	...
	pd_execute_data_swap(port, current_data);
	// TODO: This should be PD_ROLE_DISCONNECTED
	...
	*/
	...
	if (is_charge_through_allowed()){
		...
		pd_comm_enable(DUT,0);
		CPRINTS("RESET PD allowed [%d] Power [%d] DRP [%d] DRP-Pref [%d] CC_CONFIG [%x]", allow_pd, current_power, current_drp, preferred_drp, cc_config);
				     ...
				     enum pd_data_role data_role)
{
	...
	/*
	*  ServoV4p1 REV1 (*default init)
	*
	*                         HOST HUB
	*  DUT HUB                   |
	*    ^                       v
	*    |          1 [USERVO_FASTBOOT_MUX_SEL] 0*
	*    |          |         (EN_L=Gnd*)       |
	*    |          |                           |
	*    |          | ----------------|         |
	*    |                            v         v
	*    1* [FASTBOOT_DUTHUB_MUX_SEL] 0       uServo
	*            (EN_L=1*)
	*/  
	...
	case PD_ROLE_DFP:
		...
		CPRINTS("Execute_DRS: DFP: Killing USB3");
	...
	case PD_ROLE_UFP:
		...
		uservo_to_host();
		/* Enable USB3 lines */
		CPRINTS("Execute_DRS: UFP: Enabling USB3");
	...
	case PD_ROLE_DISCONNECTED:
		...
		usb_mux_set(port, USB_PD_MUX_NONE, USB_SWITCH_DISCONNECT,
		   pd_get_polarity(port));
		CPRINTS("C%d: EXEC_DRS disconnect data:%d",port, data_role);
		break;
	default:
		/* Panic */
...
static int svdm_response_identity(int port, uint32_t *payload)
{
	...
	// "Modal supported" should be a fixed item.
...
static int svdm_enter_mode(int port, uint32_t *payload)
{
	...
	/* CCD uses the SBU lines; don't enable DP when dts-mode enabled */
	if (!(cc_config & CC_DISABLE_DTS)) {
		CPRINTS("WARNING: Tried to EnterMode DP with [CCD AUX/SBU]");
		return 0; /* NAK */
	}
	else if (!(alt_dp_config & ALT_DP_ENABLE)) {
		CPRINTS("WARNING: Tried to EnterMode DP with [usbc dp disable]");
		return 0; /* NAK */
	}
...
static int command_cc(int argc, char **argv)
{
	...
	if (!strcasecmp(argv[1], "help")){
		CPRINTS("Usage: cc [off|on|src|snk|pdsnk|pdsrc|srcdts|"
			"snkdts|pdsrcdts|pdsnkdts|drp|drpdts|emca|nonemca]"
			" [cc1|cc2]");
	}
