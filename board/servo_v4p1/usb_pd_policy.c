...
static int is_typec_dp_muxed(void)
{
	...
	if (value < 0 || value & REG_GENERAL_CTLSEL_4_DP_LANES)
		return 0;
...
static void set_typec_mux(int pin_cfg)
{
	...
	value &= ~(REG_GENERAL_CTLSEL_4_DP_LANES | REG_GENERAL_CTLSEL_USB3_1);
	switch (pin_cfg) {
	case 0:
		CPRINTS("PinCfg:off");
		break;
	case MODE_DP_PIN_C:
		value |= REG_GENERAL_CTLSEL_4_DP_LANES;
		CPRINTS("PinCfg:C");
		break;
	case MODE_DP_PIN_D:
		value |= REG_GENERAL_CTLSEL_4_DP_LANES |
					REG_GENERAL_CTLSEL_USB3_1;
		CPRINTS("PinCfg:D");
		break;
	default:
