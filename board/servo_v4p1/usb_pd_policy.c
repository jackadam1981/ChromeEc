...
/* Just want the .h for the PS8742 definitions, not the large object file. */
...
{
	if (charge_port == DUT || charge_port == ALT)
		return -1;
...
static int board_set_rp(int rp)
{
	if (cc_config & CC_DISABLE_DTS) {
		...
		(cc_config & CC_EMCA_SERVO) ?
			DUT_INACTIVE_CC_PD(RA) : DUT_INACTIVE_CC_OPEN(RA);
...
static int svdm_response_identity(int port, uint32_t *payload)
{
	 ...
	 */
	int dp_supported = gpio_get_level(GPIO_STM_DP_HPD);
...
static int dp_status(int port, uint32_t *payload)
{
	...
	int hpd = gpio_get_level(GPIO_STM_DP_HPD);
