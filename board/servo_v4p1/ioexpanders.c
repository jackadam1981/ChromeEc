...
static void ioexpanders_irq(void)
{
	...
	if (!(fault & USERVO_FAULT_L)) {
		...
		cprintf(CC_SYSTEM,
			"FAULT: Microservo USB A port load switch\n");
	}

	if (!(fault & USB3_A0_FAULT_L)) {
		ec_usb3_a0_pwr_en(0);
		cprintf(CC_SYSTEM,
			"FAULT: USB3 A0 port load switch\n");
	}

	if (!(fault & USB3_A1_FAULT_L)) {
		ec_usb3_a1_pwr_en(0);
		cprintf(CC_SYSTEM,
			"FAULT: USB3 A1 port load switch\n");
	}

	if (!(fault & USB_DUTCHG_FLT_ODL)) {
		cprintf(CC_SYSTEM,
			"FAULT: Overcurrent on Charger or DUT CC/SBU lines\n");
	}

	if (!(fault & PP3300_DP_FAULT_L)) {
		cprintf(CC_SYSTEM,
			"FAULT: Overcurrent on DisplayPort\n");
	}

	if (!(fault & DAC_BUF1_LATCH_FAULT_L)) {
		cprintf(CC_SYSTEM,
			"FAULT: CC1 drive circuitry has exceeded thermal "
	...
	if (!(fault & DAC_BUF2_LATCH_FAULT_L)) {
		cprintf(CC_SYSTEM,
			"FAULT: CC2 drive circuitry has exceeded thermal "
	...
	if (!(irqs & SYS_PWR_IRQ_ODL)) {
		cprintf(CC_SYSTEM,
			"System full power threshold exceeded\n");
	}
}
