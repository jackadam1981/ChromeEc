...
#define TC_T_CC_DEBOUNCE     25   /* (100*MSEC) between 100ms and 200ms */
...
static void start_timer(enum timer_t timer)
{
	switch (timer) {
	case TC_CC_DEBOUNCE:
		tc.cc_debounce_timer = TIMER_ENABLE | TC_T_CC_DEBOUNCE;
		break;
	case TC_PD_DEBOUNCE:
		tc.pd_debounce_timer = TIMER_ENABLE | TC_T_PD_DEBOUNCE;
		break;
	}
}
...
{
	if (tc.cc_debounce_timer > TIMER_ENABLE)
		tc.cc_debounce_timer--;

	if (tc.pd_debounce_timer > TIMER_ENABLE)
...
static bool is_expired_timer(enum timer_t timer)
{
	switch (timer) {
	case TC_CC_DEBOUNCE:
		return (tc.cc_debounce_timer == TIMER_ENABLE);
	case TC_PD_DEBOUNCE:
		return (tc.pd_debounce_timer == TIMER_ENABLE);
	}
