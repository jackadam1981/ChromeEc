...
servo_write_poweron_conf(const struct servo_poweron_conf *servo_poweron_conf)
{
	...
	if (!board_read_poweron_conf(buf, CONFIG_POWERON_CONF_LEN))
		return EC_SUCCESS;
	else
...
servo_read_poweron_conf(struct servo_poweron_conf *servo_poweron_conf)
{
	...
	rv = board_read_poweron_conf(buf, CONFIG_POWERON_CONF_LEN);
	if (rv) {
		/*
	...
	/* Parse values from buffer to struct, its specific for servo_v4p1. */
	servo_poweron_conf->top_usb = buf[0];
	servo_poweron_conf->bottom_usb = buf[1];
	servo_poweron_conf->uservo_usb = buf[2];
	servo_poweron_conf->cc_config = buf[3];

	return EC_SUCCESS;
}

/*
 * poweron_conf top_usb [mux_host|mux_dut] [mux_en|mux_dis] [pwr_en|pwr_dis]
