...
/*
 * Present 1.5A power profile on CC termination resistors, when servo is SRC.
 * This can happen only if enough power available DUT_POWER.
 * The CC resistors configuration matches the one used in Suzyq.
 */
#define CC_SUZYQ_ALIKE BIT(7)
#define CC_FASTBOOT_DFP BIT(8) /* Allow mux uServo->Fastboot on DFP */
#define CC_SRC_WITHOUT_PD BIT(9) /* Force disable PD comm for source role */
#define CC_SRC_1A5 BIT(10) /* Use 1A5 Rp when PD comm is disabled */
#define CC_SRC_3A0 BIT(11) /* Use 3A0 Rp when PD comm is disabled */
...
static inline void pd_set_host_mode(int port, int enable)
{
	...
	if (enable) {
		 ...
		 */
		if (chg_port_pd_supports_1a5() && (cc_config & CC_SUZYQ_ALIKE))
			rp_val = TYPEC_RP_1A5;
