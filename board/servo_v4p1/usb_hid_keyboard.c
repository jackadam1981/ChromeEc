...
static int command_hid_press(int argc, char *argv[])
{
	uint8_t keycode;
	...
	keycode = strtoi(argv[1], NULL, 16);
	queue_keycode_event(keycode, 1);
	return EC_SUCCESS;
}
DECLARE_CONSOLE_COMMAND(hid_press, command_hid_press, "none",
			"press a key on the servo's STM32 hid keyboard");

static int command_hid_release(int argc, char *argv[])
{
	uint8_t keycode;
	...
	keycode = strtoi(argv[1], NULL, 16);
	queue_keycode_event(keycode, 0);
	return EC_SUCCESS;
}
DECLARE_CONSOLE_COMMAND(hid_release, command_hid_release, "none",
			"release a key on the servo's STM32 hid keyboard");
