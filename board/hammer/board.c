...
static void write_reply(uint16_t cmd, int seq, uint32_t *data, int len)
{
...
void ec_comm_task(void *u)
{
	...
	while (1) {
		...
		if (header->cmd == EC_CMD_BATTERY_GET_STATIC) {
			write_reply(EC_CMD_BATTERY_GET_STATIC, header->seq,
				(void *)&base_battery_static,
				sizeof(base_battery_static));
		} else if (header->cmd == EC_CMD_BATTERY_GET_DYNAMIC) {
			write_reply(EC_CMD_BATTERY_GET_DYNAMIC, header->seq,
				(void *)&base_battery_dynamic,
				sizeof(base_battery_dynamic));
		} else if (header->cmd == EC_CMD_CHARGER_CONTROL) {
			...
			struct ec_params_charger_control *params =
				...
				charge_set_input_current_limit(params->max_current, 1 /*FIXME*/);
			} else {
				/* FIXME: reset to minimum */
				charge_set_input_current_limit(128, 1 /*FIXME*/);
				/* Do OTG */
				charger_set_otg_current_voltage(
					-params->max_current, params->otg_voltage);
				charger_enable_otg_power(1);
			}

                        response.res = EC_RES_SUCCESS;
                        write_reply(EC_CMD_CHARGER_CONTROL, header->seq,
