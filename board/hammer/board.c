 ...
 * disconneceted to the nocturne.  Therefore, let's perodically burn a little
 * more power such that nocturne can differentiate between whiskers actually
 * being connected and not.  Nocturne normally polls for a detach at 250ms
 * intervals.  Once it reads a sample that is less than the disconnection
 * threshold, it starts to poll faster (every 4ms) for a total of 20ms while
 * taking an average of the samples.  We just need to ensure one of those
 * samples is taken while this function is running.
 */
