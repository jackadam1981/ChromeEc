...
static int touchpad_usb_set_interface(usb_uint alternate_setting,
				      usb_uint interface)
{
	int old_value = enabled;
	if (alternate_setting > 1) /* we only have two settings */
		return -1;
	enabled = alternate_setting;
	CPRINTF("%s: enabled = %d -> %d\n", __func__, old_value, enabled);
	return 0;
}
