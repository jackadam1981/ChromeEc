...
static void board_init(void)
{
	...
	gpio_enable_interrupt(GPIO_VBUS_SENSE);
}
...
const struct adc_t adc_channels[] = {
	...
	[ADC_VBUS] = {"VBUS",  33000, 4096, 0, STM32_AIN(0)},
	/* USB PD CC lines sensing. Converted to mV (3300mV/4096). */
	[ADC_CC1_PD] = {"CC1_PD", 3300, 4096, 0, STM32_AIN(1)},
	[ADC_CC2_PD] = {"CC1_PD", 3300, 4096, 0, STM32_AIN(3)},
	/* Charger current sensing. Converted to mA. */
	[ADC_IADP] = {"IADP",  8250, 4096, 0, STM32_AIN(8)},
	[ADC_IBAT] = {"IBAT", 41250, 4096, 0, STM32_AIN(13)},
};
...
void board_set_usb_mux(int port, enum typec_mux mux)
{
	...
	gpio_set_level(GPIO_USBC_SS1_USB_MODE_L, 0);
	gpio_set_level(GPIO_USBC_SS2_USB_MODE_L, 0);
	switch (mux) {
	case TYPEC_MUX_NONE:
...
static int command_typec(int argc, char **argv)
{
	...
	if (argc < 2) {
		/* dump current state */
		ccprintf("Port CC1 %d mV  CC2 %d mV\n",
			pd_adc_read(0),	pd_adc_read(1));
		ccprintf("DP %d Polarity %d\n",
			!gpio_get_level(GPIO_USBC_DP_MODE_L),
			!!gpio_get_level(GPIO_USBC_DP_POLARITY)	+ 1);
		ccprintf("Superspeed %s\n",
			gpio_get_level(GPIO_USBC_SS_EN_L) ? "None" :
			(!gpio_get_level(GPIO_USBC_DP_MODE_L) ? "DP" :
			(!gpio_get_level(GPIO_USBC_SS1_USB_MODE_L) ?
					"USB1" : "USB2")));
		return EC_SUCCESS;
	}
