...
struct led_info {
	...
	const uint8_t *charge_lvl_table;
 ...
 */
static const uint8_t charging_lvl_default_table[] = { 100, 100, 100 };
/* COLOR_1 = Amber, COLOR_2 = Blue */
static const struct led_descriptor led_default_state_table[][LED_NUM_PHASES] = {
	{ {LED_COLOR_1, -1}, {LED_OFF, -1} },
	{ {LED_COLOR_2, -1}, {LED_COLOR_1, -1} },
	{ {LED_COLOR_2, -1}, {LED_OFF, -1} },
	{ {LED_COLOR_2, -1}, {LED_OFF, -1} },
	{ {LED_COLOR_2, -1}, {LED_OFF, -1} },
	{ {LED_OFF, -1}, {LED_OFF, -1} },
...
static enum led_states led_get_state(void)
{
	...
	case PWR_STATE_CHARGE:
		...
		for (i = 0; i < ARRAY_SIZE(charging_lvl_default_table); i++) {
			if (charge_lvl <= led.charge_lvl_table[i])
				break;
		}
		new_state = i;
		break;
...
static void led_update_battery(void)
{
	 ...
	 */
	phase = 0;
	if ((led.state_table[led.state][LED_PHASE_0].time !=
	     LED_TIME_INDEFINITE) &&
...
static void led_init(void)
{
	...
	if ((sku >= 70 && sku <= 79) || (sku >= 124 && sku <= 125) ||
	    (sku >= 144 && sku <= 145)) {
		led.charge_lvl_table = charging_lvl_robo_table;
		led.state_table = led_robo_state_table;
		led.update_power = NULL;
	} else {
		led.charge_lvl_table = charging_lvl_default_table;
