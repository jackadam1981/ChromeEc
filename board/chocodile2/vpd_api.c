...
#include "console.h"
#define CPRINTF(format, args...) cprintf(10, format, ## args)
...
{
	*cc1 = pd_read_cc_status(0, 0);
	*cc2 = pd_read_cc_status(0, 1);
}

void vpd_host_set_pull(int pull, int rp_value)
{
	CPRINTF("vpd_host_set_pull(%d, %d)\n", pull, rp_value);
	...
	case TYPEC_CC_RP:
		...
		*/
		CPRINTF("vpd_host_set_pull(%d, %d) unimplemented\n", pull, rp_value);
		break;
	case TYPEC_CC_RD:
		/*
		vpd_cc_rpusb_odh(GPO_HZ);
		...
		*/
		...
		break;
	case TYPEC_CC_RA_RD:
		gpio_set_level(GPIO_VCONN_RA_EN_ODH, 1);
		break;
	case TYPEC_CC_OPEN:
		/*
		vpd_cc_rpusb_odh(GPO_HZ);
...
void vpd_host_get_cc(int *cc)
{
	...
	*cc = pd_read_cc_status(1, 0);
	if (last_cc != *cc) {
		last_cc = *cc;
		CPRINTF("vpd_host_get_cc() = %d\n", last_cc);
	}
}

void vpd_rx_enable(int en)
{
	tcpm_set_rx_enable(1, en);
}
#if 0
/*
...
{
	gpio_set_level(GPIO_DEBUG_LED_R_ODL, (on) ? 0 : 1);
}

void vpd_green_led(int on)
{
	gpio_set_level(GPIO_DEBUG_LED_G_ODL, (on) ? 0 : 1);
}
...
{
	CPRINTF("vpd_present_billboard(%d)\n", bb);
	/*
	switch (bb) {
	case BB_NONE:
...
void vpd_ct_cc_sel(enum vpd_cc sel)
{
	switch (sel) {
	case CT_OPEN:
		gpio_set_level(GPIO_CC_SEL, 0);
		gpio_set_level(GPIO_CC_THRU_EN, 0);
		break;
	case CT_CC1:
		gpio_set_level(GPIO_CC_THRU_EN, 1);
		gpio_set_level(GPIO_CC_SEL, 0);
		break;
	case CT_CC2:
		gpio_set_level(GPIO_CC_THRU_EN, 1);
		gpio_set_level(GPIO_CC_SEL, 1);
		break;
	}
}
#if 0
