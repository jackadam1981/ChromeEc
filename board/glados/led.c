...
static int bat_led_set_color(enum led_color color)
{
	switch (color) {
	case LED_OFF:
		gpio_set_level(GPIO_CHARGE_LED_1, BAT_LED_OFF);
		gpio_set_level(GPIO_CHARGE_LED_2, BAT_LED_OFF);
		break;
	case LED_RED:
		gpio_set_level(GPIO_CHARGE_LED_1, BAT_LED_ON);
		gpio_set_level(GPIO_CHARGE_LED_2, BAT_LED_OFF);
		break;
	case LED_AMBER:
		gpio_set_level(GPIO_CHARGE_LED_1, BAT_LED_ON);
		gpio_set_level(GPIO_CHARGE_LED_2, BAT_LED_ON);
		break;
	case LED_GREEN:
		gpio_set_level(GPIO_CHARGE_LED_1, BAT_LED_OFF);
		gpio_set_level(GPIO_CHARGE_LED_2, BAT_LED_ON);
...
int led_set_brightness(enum ec_led_id led_id, const uint8_t *brightness)
{
	...
	case EC_LED_ID_BATTERY_LED:
		gpio_set_level(GPIO_CHARGE_LED_1,
				(brightness[EC_LED_COLOR_RED] != 0) ?
					BAT_LED_ON : BAT_LED_OFF);
		gpio_set_level(GPIO_CHARGE_LED_2,
