...
/* When blinking is enabled, led will blinking according to led_blinking_ary,
...
#define LED_BLINKING_ARY_MAX	21
const int led_alert_ary[] = {1, 1, 0, 1, 1, 0, 1, 0, 1, 0, -1};
const int led_alert_count = ARRAY_SIZE(led_alert_ary);
static int led_blinking_ary[LED_BLINKING_ARY_MAX];
...
static void led_blinking(void)
{
	...
	if (led_auto_control_is_enabled(EC_LED_ID_POWER_LED)) {
		switch (led_blinking_ary[led_blinking_index]) {
		case 0:
		...
		if (led_blinking_ary[led_blinking_index] == -1)
			led_blinking_index = 0;
	}
...
void led_alert(int enable)
{
	if (enable) {
		...
		hook_call_deferred(&led_tick_data, -1);
		memcpy(led_blinking_ary, led_alert_ary, led_alert_count);
		led_blinking_index = 0;
		led_blinking();
	} else {
...
{
	config_tick(500 * MSEC, 100, LED_WHITE);
	led_tick();
}
