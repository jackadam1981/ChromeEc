...
void led_alert(int enable)
{
	if (enable) {
		...
		hook_call_deferred(&led_tick_data, -1);
		hook_call_deferred(&led_blinking_data, -1);
		memcpy(led_blinking_array, led_alert_array,
			sizeof(int) * led_alert_count);
		led_blinking_index = 0;
		led_blinking();
	} else {
...
__override void show_critical_error(void)
{
	hook_call_deferred(&led_tick_data, -1);
	hook_call_deferred(&led_blinking_data, -1);
	memcpy(led_blinking_array, led_critical_array,
		sizeof(int) * led_critical_count);
	led_blinking_index = 0;
	led_blinking();
}
...
DECLARE_CONSOLE_COMMAND(led, command_led,
			"[debug|red|white|amber|off|alert|crit|sus]",
			"Turn on/off LED.");
