...
static int led_step;
static void led_2_long_2_short(void);
DECLARE_DEFERRED(led_2_long_2_short);
static void led_2_long_2_short(void)
{
	uint32_t elapsed;
	uint32_t next = 0;
	uint32_t start = get_time().le.lo;
	uint32_t interval_local = 0;
	...
	if (led_auto_control_is_enabled(EC_LED_ID_POWER_LED)) {
		...
		case 2:
			led_set_color(EC_LED_ID_POWER_LED, LED_WHITE, 100);
			interval_local = 1 * SECOND;
			break;
		case 4:
		case 6:
			led_set_color(EC_LED_ID_POWER_LED, LED_WHITE, 100);
			interval_local = 500 * MSEC;
			break;
		case 1:
		case 3:
		case 5:
		case 7:
			led_set_color(EC_LED_ID_POWER_LED, LED_WHITE, 0);
			interval_local = 500 * MSEC;
			break;
		default:
			led_step = 0;
	...
	elapsed = get_time().le.lo - start;
	next = interval_local > elapsed ? interval_local - elapsed : 0;
	hook_call_deferred(&led_2_long_2_short_data, next);
}
