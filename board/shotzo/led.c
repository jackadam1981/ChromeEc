 ...
 * 1 means led on, 0 means led off, -1 means tail and go to the head.
...
static void led_blinking(void)
{
	...
	uint32_t next = 0;
	uint32_t start = get_time().le.lo;
	int signal = *(led_blinking_array + led_blinking_index);

	if (signal == -1) {
		signal = *led_blinking_array;
...
static int led_alert_array[] = { 1, 0, -1 };
...
static int led_critical_array[] = { 1, 1, 0, 1, 1, 0, 1, 0, 1, 0, -1 };
