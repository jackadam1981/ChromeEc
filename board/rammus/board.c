...
static enum lid_accel_type lid_accel_config;
...
static void board_detect_motionsense(void)
{
	...
	rv = i2c_read8(I2C_PORT_ACCEL, KX022_ADDR1_FLAGS, KX022_WHOAMI, &val);

	if (rv == EC_SUCCESS && val == KX022_WHO_AM_I_VAL) {
		motion_sensors[LID_ACCEL] = lid_accel_kx022;
		lid_accel_config = LID_ACCEL_KX022;
		ccprints("Lid Accel: KX022");
	} else {
		lid_accel_config = LID_ACCEL_BMA253;
