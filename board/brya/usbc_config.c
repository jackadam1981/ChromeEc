...
struct tcpc_config_t crt_tcpc_config = {
...
const struct ppc_config_t crt_ppc_chips = {
	...
	*/
	...
	if (me->usb_port == USBC_PORT_C0) {
		...
		if (get_board_id() == 1)
			...
			ec_cfg_usb_db_type() == DB_USB4_CRT) {
	...
	if (enable) {
		...
		if (me->usb_port == USBC_PORT_C1) {
			if (ec_cfg_usb_db_type() == DB_USB4_CRT) {
...
{
	tcpc_config[USBC_PORT_C1] = crt_tcpc_config;
	ppc_chips[USBC_PORT_C1] = crt_ppc_chips;
	usb_muxes[USBC_PORT_C1] = crt_usbc1_conn_side_bb_retimer1_usb_mux;
}
