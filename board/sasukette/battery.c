 ...
 *	1. bat_temp < 5 &&		|	batt_temp >= 10,
 *	bat_cell_voltage < 4.15		|	cv = (cell CV-50mv)*series
 *					|	   = 8700mv
 *	cv = 4150mv*series = 8300mv	|	cc = FCC*C_rate*0.4 = 1464ma
 *	cc = FCC*C_rate*0.4		|
 *					|
 *	2. bat_temp < 15 &&		|	batt_temp >= 20,
 *	bat_cell_voltage < 4.15		|	cv = (cell CV-50mv)*series
 *					|	   = 8700mv
 *	cv = 4150mv*series = 8300mv	|	cc = FCC*C_rate*0.9 = 3294ma
 *	cc = FCC*C_rate*0.4= 1464ma	|
 *					|
 *	3. bat_temp >= 45 &&		|	batt_temp < 43,
 	bat_cell_voltage < 4.15		|	cv = (cell CV-50mv)*series
 *	cv = 4150mv*series = 8300mv	|	   = 8700mv
 *	cc = FCC*C_rate*0.45= 1647ma	|	cc = FCC*C_rate*0.9 = 3294ma  
 *					|	
 *					|
 *	4. bat_temp >= 50		|	batt_temp < 45,
