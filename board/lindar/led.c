...
enum ec_lightbar_colors {
	BAR_OFF             = 0x00,
	BAR_COLOR_ORG_20_PERCENT     = 0x01,
	BAR_COLOR_ORG_40_PERCENT     = 0x02,
	BAR_COLOR_ORG_60_PERCENT     = 0x03,
	BAR_COLOR_ORG_80_PERCENT     = 0x04,
	BAR_COLOR_ORG_FULL           = 0x05,
	BAR_COLOR_GRN_FULL           = 0x06,
...
#define I_ON				0x02
#define SEL_OFF				0x00
#define SEL_1ST_LED			0x08
#define SEL_2ND_LED			0x80
#define SEL_BOTH_LED		(SEL_1ST_LED | SEL_2ND_LED)
#define SKU_ID_NONE			0xFF
#define LB_SUPPORTED_SKUID_LOWER 458700
#define LB_SUPPORTED_SKUID_UPPER 458800

static int lightbar_is_supported(void)
{
	uint32_t skuid;
	static int result = SKU_ID_NONE;

	if (result != SKU_ID_NONE)
		return result;

	if (cbi_get_sku_id(&skuid)) {
		CPRINTS("Cannot get SKU ID for lightbar supported check");
		result = 0;
	} else {
		if (skuid >= LB_SUPPORTED_SKUID_LOWER &&
			...
			CPRINTS("This sku %d unsupport lightbar", skuid);
			result = 0;
		}
	}
	return result;
 ...
 */
static int lightbar_is_enabled(void)
{
...
static void lightbar_init(void)
{
	...
	controller_write(KTD20XX_CTRL_CFG, DISABLE_LIGHTBAR);
	controller_write(KTD20XX_IRED_SET0, I_OFF);
	controller_write(KTD20XX_IGRN_SET0, I_OFF);
	controller_write(KTD20XX_IBLU_SET0, I_OFF);
	controller_write(KTD20XX_IRED_SET1, I_OFF);
	controller_write(KTD20XX_IGRN_SET1, I_OFF);
	controller_write(KTD20XX_IBLU_SET1, I_OFF);
...
{
	switch (color) {
	case BAR_COLOR_ORG_20_PERCENT:
		i2c_lock(I2C_PORT_LIGHTBAR, 1);
		controller_write(KTD20XX_IRED_SET0, I_ON);
		controller_write(KTD20XX_IGRN_SET0, I_OFF);
		controller_write(KTD20XX_IBLU_SET0, I_OFF);
		controller_write(KTD20XX_IRED_SET1, I_OFF);
		controller_write(KTD20XX_IGRN_SET1, I_OFF);
		...
		controller_write(KTD20XX_ISEL_A12, SEL_BOTH_LED);
		controller_write(KTD20XX_ISEL_A34, SEL_OFF);
		controller_write(KTD20XX_ISEL_B12, SEL_OFF);
		controller_write(KTD20XX_ISEL_B34, SEL_OFF);
		controller_write(KTD20XX_ISEL_C12, SEL_OFF);
		controller_write(KTD20XX_CTRL_CFG, ENABLE_LIGHTBAR);
		i2c_lock(I2C_PORT_LIGHTBAR, 0);
		break;
	case BAR_COLOR_ORG_40_PERCENT:
		i2c_lock(I2C_PORT_LIGHTBAR, 1);
		controller_write(KTD20XX_IRED_SET0, I_ON);
		controller_write(KTD20XX_IGRN_SET0, I_OFF);
		controller_write(KTD20XX_IBLU_SET0, I_OFF);
		...
		controller_write(KTD20XX_ISEL_A12, SEL_BOTH_LED);
		controller_write(KTD20XX_ISEL_A34, SEL_BOTH_LED);
		controller_write(KTD20XX_ISEL_B12, SEL_OFF);
		controller_write(KTD20XX_ISEL_B34, SEL_OFF);
		controller_write(KTD20XX_ISEL_C12, SEL_OFF);
		controller_write(KTD20XX_CTRL_CFG, ENABLE_LIGHTBAR);
	...
	case BAR_COLOR_ORG_60_PERCENT:
		...
		controller_write(KTD20XX_ISEL_A12, SEL_BOTH_LED);
		controller_write(KTD20XX_ISEL_A34, SEL_BOTH_LED);
		controller_write(KTD20XX_ISEL_B12, SEL_BOTH_LED);
		controller_write(KTD20XX_ISEL_B34, SEL_OFF);
		controller_write(KTD20XX_ISEL_C12, SEL_OFF);
		controller_write(KTD20XX_CTRL_CFG, ENABLE_LIGHTBAR);
		i2c_lock(I2C_PORT_LIGHTBAR, 0);
		break;
	case BAR_COLOR_ORG_80_PERCENT:
		i2c_lock(I2C_PORT_LIGHTBAR, 1);
		controller_write(KTD20XX_IRED_SET0, I_ON);
		controller_write(KTD20XX_IGRN_SET0, I_OFF);
		controller_write(KTD20XX_IBLU_SET0, I_OFF);
		...
		controller_write(KTD20XX_ISEL_A34, SEL_BOTH_LED);
		controller_write(KTD20XX_ISEL_B12, SEL_BOTH_LED);
		controller_write(KTD20XX_ISEL_B34, SEL_BOTH_LED);
		controller_write(KTD20XX_ISEL_C12, SEL_OFF);
		controller_write(KTD20XX_CTRL_CFG, ENABLE_LIGHTBAR);
		i2c_lock(I2C_PORT_LIGHTBAR, 0);
		break;
	case BAR_COLOR_ORG_FULL:
		...
		controller_write(KTD20XX_ISEL_B12, SEL_BOTH_LED);
		controller_write(KTD20XX_ISEL_B34, SEL_BOTH_LED);
		controller_write(KTD20XX_ISEL_C12, SEL_BOTH_LED);
		controller_write(KTD20XX_CTRL_CFG, ENABLE_LIGHTBAR);
		i2c_lock(I2C_PORT_LIGHTBAR, 0);
		break;
	case BAR_COLOR_GRN_FULL:
		i2c_lock(I2C_PORT_LIGHTBAR, 1);
		controller_write(KTD20XX_IRED_SET0, I_OFF);
		controller_write(KTD20XX_IGRN_SET0, I_ON);
		controller_write(KTD20XX_IBLU_SET0, I_OFF);
		controller_write(KTD20XX_IRED_SET1, I_OFF);
		controller_write(KTD20XX_IGRN_SET1, I_OFF);
		controller_write(KTD20XX_IBLU_SET1, I_OFF);
...
static enum lightbar_states lightbar_get_state(void)
{
	...
	/* Due to system will be waken up when plug in AC in S0ix, it
	 * may be late to update chipset state and cause lightbar flash
	 * a while that system is already back to S0. We add to check
	 * SLP_S3_L and SLP_S0_L and avoid lightbar blinking.
	 */
	if (gpio_get_level(GPIO_SLP_S3_L) && gpio_get_level(GPIO_PCH_SLP_S0_L))
		return LB_STATE_OFF;
