...
#define LIGHTBAR_10_LED_TYPE SSFC_LIGHTBAR_TYPE1
#define LIGHTBAR_12_LED_TYPE SSFC_LIGHTBAR_TYPE2
 ...
 * Lightbar V9 SPEC define lightbar resuming behavior.
 */
...
static void lightbar_set_color(enum ec_lightbar_colors color)
{
	...
	if (get_cbi_ssfc_lightbar() == LIGHTBAR_12_LED_TYPE)
...
static void lightbar_update(void)
{
	...
	if (lightbar_is_auto_control() == true)
...
static int command_lightbar(int argc, char **argv)
{
	...
	if (argc == 1) {
		ccprintf("lightbar is %ssupported and %sabled\n",
				lightbar_is_supported()?"":"un-",
				lightbar_is_enabled()?"en":"dis");
		ccprintf("auto_control: %sabled\n",
	...
	if (!lightbar_is_supported() || !lightbar_is_enabled()) {
		uint32_t cbi_bid, cbi_skuid;
		int cbi_ssfc_lightbar;
	...
	if (!strcasecmp(argv[1], "demo")) {
		...
		if (lightbar_is_auto_control()) {
			ccprintf("Please turn off auto control before demo.\n");
...
static enum ec_status lpc_cmd_lightbar(struct host_cmd_handler_args *args)
{
	...
	case LIGHTBAR_CMD_SEQ:
		...
		if (lightbar_is_auto_control()) {
			CPRINTS("Please turn off auto control before demo.");
