 ...
 *
 * Power and battery LED control for Yuna.
 */
...
static int yuna_led_set_gpio(enum led_color color,
...
static int yuna_led_set_color_battery(enum led_color color)
{
	return yuna_led_set_gpio(color, GPIO_BAT_LED0_L, GPIO_BAT_LED1_L);
}

static int yuna_led_set_color_power(enum led_color color)
{
	return yuna_led_set_gpio(color, GPIO_PWR_LED0_L, GPIO_PWR_LED1_L);
}

static int yuna_led_set_color(enum ec_led_id led_id, enum led_color color)
{
	int rv;
	...
	case EC_LED_ID_BATTERY_LED:
		rv = yuna_led_set_color_battery(color);
		break;
	case EC_LED_ID_POWER_LED:
		rv = yuna_led_set_color_power(color);
		break;
	default:
		return EC_ERROR_UNKNOWN;
	}
...
int led_set_brightness(enum ec_led_id led_id, const uint8_t *brightness)
{
	if (brightness[EC_LED_COLOR_BLUE] != 0 &&
		...
		yuna_led_set_color(led_id, LED_PINK);
	else if (brightness[EC_LED_COLOR_BLUE] != 0)
		yuna_led_set_color(led_id, LED_BLUE);
	else if (brightness[EC_LED_COLOR_YELLOW] != 0)
		yuna_led_set_color(led_id, LED_AMBER);
	else
		yuna_led_set_color(led_id, LED_OFF);
...
static void yuna_led_set_power(void)
{
	...
	if (chipset_in_state(CHIPSET_STATE_ANY_OFF))
		yuna_led_set_color_power(LED_OFF);
	else if (chipset_in_state(CHIPSET_STATE_ON))
		yuna_led_set_color_power(LED_BLUE);
}

static void yuna_led_set_battery(void)
{
	...
	case PWR_STATE_DISCHARGE:
		...
		/* Less than 10%, blink one second every four seconds */
			...
			yuna_led_set_color_battery(LED_OFF);
		break;
