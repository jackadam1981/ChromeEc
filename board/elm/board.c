...
void usb_evt(enum gpio_signal signal)
{
	 ...
	 */
	CPRINTS("%s bc %d cable %d pwr %d reset %d", __func__,
		gpio_get_level(GPIO_BC12_WAKE_L),
		gpio_get_level(GPIO_USB_C0_CABLE_DET_L),
		gpio_get_level(GPIO_USB_C0_PWR_EN_L),
		gpio_get_level(GPIO_USB_C0_RST));
...
/*
 * TODO: 7688 POWER_STATUS[6] is not reliable for tcpci_tcpm_init()
 * due to it is default 0 in HW. It needs another 60ms
 * after tcpc_read16() returns EC_SUCCESS to wait FW ready then
 * we can do TCPC initialization.
 * To find a better way to do this
 */
static int anx7688_ready = 0;
void deferred_set_anx7688_ready(void);
DECLARE_DEFERRED(deferred_set_anx7688_ready);
...
void deferred_reset_pd_mcu(void)
{
	...
	case 2:
		...
		/* TODO: 7688 needs another 60 ms to be ready */
		hook_call_deferred(&deferred_set_anx7688_ready_data, 60*MSEC);
		break;
	}
}
