 ...
 *  - overall ssytem power as measured on the main power supply rail.
...
static void power_monitor(void)
{
	 ...
	 *	* If not in use, prefer to limit type C [1].
	 *	* If in use:
	...
	if (headroom_5v < 0) {
		/*
		 * Check whether type C is not throttled, and is not in use.
		 */
		if (!(new_state & THROT_TYPE_C) && !usbc_overcurrent) {
			/*
			 * [1] Type C not in use, throttle it.
			 */
			headroom_5v += PWR_C_HIGH - PWR_C_LOW;
			new_state |= THROT_TYPE_C;
		}
