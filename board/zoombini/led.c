...
static enum pwm_channel led_pwm_ch_map[] = {
#ifdef BOARD_MEOWTH
	[EC_LED_ID_POWER_LED] = PWM_CH_DB0_LED_GREEN,
	[EC_LED_ID_BATTERY_LED] = PWM_CH_DB0_LED_RED,
#else /* !defined(BOARD_MEOWTH) */
	[EC_LED_ID_POWER_LED] = PWM_CH_LED_GREEN,
	[EC_LED_ID_BATTERY_LED] = PWM_CH_LED_RED,
#endif /* defined(BOARD_MEOWTH) */
};

static void set_led_state(enum ec_led_id id, int on)
{
	int val;
#ifdef BOARD_MEOWTH
	val = on ? 98 : 100;
...
static void pulse_power_led(void)
{
	...
	if (tick_count == 0)
		set_led_state(EC_LED_ID_POWER_LED, 1);
	else
		set_led_state(EC_LED_ID_POWER_LED, 0);
...
static void battery_led_update(void)
{
	...
	case PWR_STATE_IDLE:
		set_led_state(EC_LED_ID_BATTERY_LED, 0);
		break;
	...
	case PWR_STATE_CHARGE:
		set_led_state(EC_LED_ID_BATTERY_LED, 1);
		break;
