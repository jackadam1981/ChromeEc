...
void board_overcurrent_event(int port)
{
	...
	if (!gpio_get_level(GPIO_USB_C0_PD_INT_L)) {
		ccprints("C0 INT still asserted, re-rollin'\n");
		usb_c0_interrupt(GPIO_USB_C0_PD_INT_L);
	}

	if (!gpio_get_level(GPIO_USB_C1_PD_INT_L)) {
		ccprints("C1 INT still asserted, re-rollin'\n");
		usb_c1_interrupt(GPIO_USB_C1_PD_INT_L);
	}
#endif /* defined(BOARD_MEOWTH) */
}
