...
int board_set_active_charge_port(int port)
{
	...
	/*
	 * Turn on the FET such that power actually flows and turn off the
	 * non-charge ports' PP FETs.
	 */
	for (i = 0; i < sn5s330_cnt; i++) {
		if (sn5s330_pp_fet_enable(sn5s330_tbl[0].i2c_port,
					  sn5s330_tbl[0].i2c_addr,
					  2,
					  port == i))
			CPRINTF("Setting p%d PP FET failed.", port);
	}
