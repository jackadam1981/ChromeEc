...
void it83xx_tcpc_update_hpd_status(int port, int hpd_lvl, int hpd_irq)
{
	...
	if (port) {
		gpio_set_level(GPIO_USB_C1_HPD_1P8_ODL, hpd_lvl);
		if (hpd_irq) {
			gpio_set_level(GPIO_USB_C1_HPD_1P8_ODL, 1);
			msleep(1);
			gpio_set_level(GPIO_USB_C1_HPD_1P8_ODL, hpd_lvl);
		}
	} else {
		gpio_set_level(GPIO_USB_C0_HPD_1P8_ODL, hpd_lvl);
		if (hpd_irq) {
			gpio_set_level(GPIO_USB_C0_HPD_1P8_ODL, 1);
			msleep(1);
			gpio_set_level(GPIO_USB_C0_HPD_1P8_ODL, hpd_lvl);
		}
	}
