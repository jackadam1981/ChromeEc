...
#else
	pd_port_status[0].wakeup_port = 0;
	pd_send_ec_int();
#endif
}
...
#else
	/* pd_port_status[1].wakeup_port = 0; */
	pd_send_ec_int();
#endif
}
...
void board_config_pre_init(void)
{
	 ...
	 *  Chan 3 : SPI1_TX   (C1 TX) --> (C0 TX)
	 *  Chan 4 : USART1_TX
	 *  Chan 5 : USART1_RX
	 *  Chan 6 : TIM3_CH1  (C1 RX)
	 *  Chan 7 : SPI2_TX   (C0 TX) --> (C1 TX)
	 */
...
{
	/* OAK_PD: TODO: check is it should move to EC side */
#if 0
...
static int ec_get_pd_port_host_cmd(struct host_cmd_handler_args *args)
{
	...
	if (args->version == 1) {
		CPRINTS("ec_get_pd_port_host_cmd()\n");
		pd_port_status[p->port].active_charge_port =
			(uint8_t) pd_status.active_charge_port;
