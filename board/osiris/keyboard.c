...
static void keyboard_init(void)
{
	...
	CPRINTS("FW_CONFIG: run register_scancode_set2");
...
#ifdef TEST_SCANCODE
static int command_manual_scancode_copy(int argc, char **argv)
{
	cprintf(CC_KEYSCAN, "%s start.\n", __func__);

	register_scancode_set2((uint16_t *) &scancode_set2_rgb,
					sizeof(scancode_set2_rgb));

	cprintf(CC_KEYSCAN, "%s done.\n", __func__);

	return EC_SUCCESS;
}
DECLARE_CONSOLE_COMMAND(sc, command_manual_scancode_copy,
				"None", "replace scancode_set2");
#endif
