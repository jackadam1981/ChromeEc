...
const struct fan_step fan_table_tablet[] = {
	...
	{.on1 =   0, .off1 = 99, .on2 =  43, .off2 = 42, .on3 =  42, .off3 =  37, .rpm0 =    0, .rpm1 =    0},
	{.on1 =   0, .off1 = 99, .on2 =  44, .off2 = 43, .on3 =  44, .off3 =  39, .rpm0 =    0, .rpm1 =    0},
...
int fan_table_to_rpm(int fan, int *temps)
{
	...
	static int prev_temp1, prev_temp3, prev_temp4;
	static int new_rpm;
	int i;
	...
	if (tablet_get_mode()) {
		if (temps[TEMP_SENSOR_1] < prev_temp1 ||
		    temps[TEMP_SENSOR_3] < prev_temp3 ||
			...
			for (i = current_level; i >= 0; i--) {
				if ((temps[TEMP_SENSOR_1] < fan_table_tablet[i].off1 &&
				     ...
				     temps[TEMP_SENSOR_3] < fan_table_tablet[i].off3)
					current_level = i - 1;
				else
					break;
			}
			...
			for (i = current_level; i < NUM_FAN_LEVELS; i++) {
				if ((temps[TEMP_SENSOR_1] > fan_table_tablet[i].on1 &&
				     ...
				     temps[TEMP_SENSOR_3] > fan_table_tablet[i].on3)
					current_level = i + 1;
				else
					break;
			}
		...
		if (temps[TEMP_SENSOR_1] < prev_temp1 ||
			...
			for (i = current_level; i >= 0; i--) {
				if ((temps[TEMP_SENSOR_1] < fan_table_clamshell[i].off1 &&
				     ...
				     temps[TEMP_SENSOR_3] < fan_table_clamshell[i].off3)
					current_level = i - 1;
				else
					break;
			}
			...
			for (i = current_level; i < NUM_FAN_LEVELS; i++) {
				if ((temps[TEMP_SENSOR_1] > fan_table_clamshell[i].on1 &&
				     ...
				     temps[TEMP_SENSOR_3] > fan_table_clamshell[i].on3)
					current_level = i + 1;
				else
					break;
			}
	...
	prev_temp3 = temps[TEMP_SENSOR_3];
	prev_temp4 = temps[TEMP_SENSOR_4];

	switch (fan) {
	case FAN_CH_0:
		if (tablet_get_mode())
			new_rpm = fan_table_tablet[current_level].rpm0;
		else
			new_rpm = fan_table_clamshell[current_level].rpm0;
		break;
	case FAN_CH_1:
		if (tablet_get_mode())
			new_rpm = fan_table_tablet[current_level].rpm1;
		else
			new_rpm = fan_table_clamshell[current_level].rpm1;
		break;
	default:
		break;
	}
