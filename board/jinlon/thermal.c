...
const struct fan_step fan_table[] = {
	{.on1 =   0, .off1 = 99, .on2 =  45, .off2 =  0, .rpm0 =    0, .rpm1 =    0},
	{.on1 =   0, .off1 = 99, .on2 =  46, .off2 = 45, .rpm0 = 4200, .rpm1 = 4700},
	{.on1 =   0, .off1 = 99, .on2 =  47, .off2 = 46, .rpm0 = 4400, .rpm1 = 4900},
	{.on1 =   0, .off1 = 99, .on2 =  48, .off2 = 47, .rpm0 = 4600, .rpm1 = 5100},
	{.on1 =  80, .off1 = 74, .on2 =  49, .off2 = 48, .rpm0 = 4800, .rpm1 = 5300},
	{.on1 =  85, .off1 = 79, .on2 =  50, .off2 = 49, .rpm0 = 5200, .rpm1 = 5700},
	{.on1 =  90, .off1 = 84, .on2 =  51, .off2 = 50, .rpm0 = 5600, .rpm1 = 6100},
	{.on1 = 127, .off1 = 89, .on2 = 127, .off2 = 51, .rpm0 = 6000, .rpm1 = 6500},
};
...
int fan_table_to_rpm(int fan, int *temps)
{
	...
	if (temps[TEMP_SENSOR_1] < prev_temp1 ||
		 temps[TEMP_SENSOR_4] < prev_temp2) {
		for (i = current_level; i >= 0; i--) {
			if (temps[TEMP_SENSOR_1] <= fan_table[i].off1 &&
					temps[TEMP_SENSOR_4] <= fan_table[i].off2)
				current_level = i - 1;
			else
				break;
		}
		...
		for (i = current_level; i < NUM_FAN_LEVELS; i++) {
			if (temps[TEMP_SENSOR_1] >= fan_table[i].on1 &&
					temps[TEMP_SENSOR_4] >= fan_table[i].on2)
				current_level = i + 1;
			else
				break;
		}
	...
	prev_temp2 = temps[TEMP_SENSOR_4];
