...
#ifdef CONFIG_BOARD_SYS_PRE_INIT
/*
 * Called from system_pre_init
 * Board specific configuration at system init time.
 */
void board_sys_pre_init(void)
{
#ifdef CONFIG_MCHP_TFDP
	/* Debug: dump some signals */
	trace1(0, MEC, 0, "PCH_RSMRST_L = %d",
		gpio_get_level(GPIO_PCH_RSMRST_L));
	trace1(0, MEC, 0, "RSMRST_L_PGOOD = %d",
		gpio_get_level(GPIO_RSMRST_L_PGOOD));
	trace1(0, MEC, 0, "POWER_BUTTON_L = %d",
		gpio_get_level(GPIO_POWER_BUTTON_L));
	trace1(0, MEC, 0, "SUSPWRNACK = %d",
		gpio_get_level(GPIO_SUSPWRNACK));
	trace1(0, MEC, 0, "PCH_SLP_S4_L = %d",
		gpio_get_level(GPIO_PCH_SLP_S4_L));
	trace1(0, MEC, 0, "PCH_SLP_S3_L = %d",
		gpio_get_level(GPIO_PCH_SLP_S3_L));
#ifdef CONFIG_POWER_S0IX
	trace1(0, MEC, 0, "PCH_SLP_S0_L = %d",
		gpio_get_level(GPIO_PCH_SLP_S0_L));
#endif
	trace1(0, MEC, 0, "ALL_SYS_PGOOD = %d",
		gpio_get_level(GPIO_ALL_SYS_PGOOD));
	trace1(0, MEC, 0, "AC_PRESENT = %d",
...
{
	trace0(0, ISR, 0, "tcpc_alert_event");
...
static void anx74xx_cable_det_handler(void)
{
	...
	trace2(0, BRD, 0, "anx74xx cbldet handler: det=%d reset_n=%d",
		cable_det, reset_n);
	/*
...
{
	trace0(0, ISR, 0, "USB_C0_CABLE_DET GPIO_0246 Edge");
	/* debounce for 2 msec */
...
{
	trace0(0, ISR, 0, "Tablet Mode Interrupt");
 ...
 * Map ports to controller.
 * Ports may map to the same controller.
 * TEST: Is PD commication failures due to port
 * switching? Put PD controllers on different I2C controllers.
 * Share Gyro and Sensors on same controller.
 */
const uint16_t i2c_port_to_ctrl[I2C_PORT_COUNT] = {
	(MCHP_I2C_CTRL0 << 8) + MCHP_I2C_PORT0,
	(MCHP_I2C_CTRL1 << 8) + MCHP_I2C_PORT2,
	(MCHP_I2C_CTRL2 << 8) + MCHP_I2C_PORT3,
	(MCHP_I2C_CTRL3 << 8) + I2C_PORT_GYRO,
	(MCHP_I2C_CTRL3 << 8) + MCHP_I2C_PORT7,
};
#endif
...
#ifdef CONFIG_BOARD_MCHP_I2C2_SLAVE_ADDRS
	(MCHP_I2C_CTRL2 + (CONFIG_BOARD_MCHP_I2C2_SLAVE_ADDRS << 16)),
#else
	(MCHP_I2C_CTRL2 + (CONFIG_MCHP_I2C2_SLAVE_ADDRS << 16)),
#endif
#ifdef CONFIG_BOARD_MCHP_I2C3_SLAVE_ADDRS
	(MCHP_I2C_CTRL3 + (CONFIG_BOARD_MCHP_I2C3_SLAVE_ADDRS << 16)),
#else
	(MCHP_I2C_CTRL3 + (CONFIG_MCHP_I2C3_SLAVE_ADDRS << 16)),
#endif
...
uint16_t tcpc_get_alert_status(void)
{
	...
	trace1(0, BRD, 0, "board tcpc_get_alert_status: 0x%04x",
		status);
	return status;
}
...
static int ps8751_tune_mux(const struct usb_mux *mux)
{
	...
	trace0(0, BRD, 0, "ps8751_tune_mux");
	...
	if (rv)
		trace1(0, BRD, 0, "i2c fail rv=%d", rv);

	/* TCPCI spec. delay msleep(6); */

	trace0(0, BRD, 0, "ps8751_tune_mux: done after wake delay");
 ...
 * port, device slave address, and a fuction pointer into the driver code.
 */
...
{
	trace2(0, BRD, 0, "board_set_tcpc_power_mode: port=%d mode=%d",
		port, mode);
	...
	case ANX74XX_NORMAL_MODE:
		trace0(0, BRD, 0,
			"Enable ANX74XX Power and release Reset");
	...
	case ANX74XX_STANDBY_MODE:
		trace0(0, BRD, 0,
			"Assert ANX74XX Reset and disable Power");
	...
	trace0(0, BRD, 0, "board_set_tcpc_power_mode: done");
}
...
{
	trace0(0, BRD, 0, "board_reset_pd_mcu - entry");
	...
	trace0(0, BRD, 0, "board_reset_pd_mcu - exit");
}
...
void board_print_tcpc_fw_version(int port)
{
	...
	if (port)
		rv = ps8xxx_tcpc_get_fw_version(port, &version);
	else
...
void board_tcpc_init(void)
{
	...
	trace0(0, BRD, 0, "board_tcpc_init");

	/* Only reset TCPC if not sysjump */
	if (!system_jumped_to_this_image()) {
		trace0(0, BRD, 0, "board_tcpc_init: Not sysjump");
		board_reset_pd_mcu();
	}
	 ...
	 */
	trace0(0, BRD, 0, "board_tcpc_init: PS8751 A2 work-around");
	trace3(0, BRD, 0, " i2c_read8 port=%d, addr=0x%0x, reg=0x%0x",
		MCHP_I2C_PORT2, 0x10, 0xA0);
	reg = 0;
	/* TODO MCHP:
	 ...
	 */
	rv = i2c_read8(MCHP_I2C_PORT2, 0x10, 0xA0, &reg);
	if (rv)
		trace1(0, BRD, 0,
			"board_tcpc_init: i2c_read8 error=%d", rv);

	trace0(0, BRD, 0, "Enable TCPC0 interrupt");
	/* Enable TCPC0 interrupt */
	gpio_enable_interrupt(GPIO_USB_C0_PD_INT_ODL);

	trace0(0, BRD, 0, "Enable TCPC1 interrupt");
	/* Enable TCPC1 interrupt */
	gpio_enable_interrupt(GPIO_USB_C1_PD_INT_ODL);

#ifdef CONFIG_USB_PD_TCPC_LOW_POWER
	trace0(0, BRD, 0, "Enable ANX3429 cable detect interrupt");
	...
	for (port = 0; port < CONFIG_USB_PD_PORT_COUNT; port++) {
		...
		trace1(0, BRD, 0,
			"Call hpd_update for USB MUX at port %d", port);
		mux->hpd_update(port, 0, 0);
	}

	trace0(0, BRD, 0, "board_tcpc_init - done");
}
...
void chipset_pre_init_callback(void)
{
	...
	trace0(0, PWR, 0, "chipset_pre_init: enter...");
	/*
	...
	trace0(0, PWR, 0,
		"chipset_pre_init: exit, enable pmic");
}
...
{
	trace0(0, BRD, 0, "board_init");
	CPUTS("board_init: enable charger pin interrupt");
	/* Ensure tablet mode is initialized according to the hardware state
	 * so that the cached state reflects reality.
	 */
	board_set_tablet_mode();

	trace0(0, BRD, 0, "board_init: Enable Table Mode interrupt");
	gpio_enable_interrupt(GPIO_TABLET_MODE_L);

	trace0(0, BRD, 0, "board_init: Enable Charger interrupt");
	/* Enable charger interrupts */
	gpio_enable_interrupt(GPIO_CHARGER_INT_L);

	trace0(0, BRD, 0, "board_init: Enable Gyro interrupt");
	/* Enable Gyro interrupts */
	gpio_enable_interrupt(GPIO_BASE_SIXAXIS_INT_L);
	trace0(0, BRD, 0, "board_init - exit");
}
...
int board_set_active_charge_port(int charge_port)
{
	...
	case CHARGE_PORT_NONE:
		//CPUTS("board_set_active_charge_port: CHARGE_PORT_NONE\n");
		trace0(0, BRD, 0,
			"brd set_active_charge_port: CHARGE_PORT_NONE");
	...
	trace1(0, BRD, 0, "brd set_active_charge_port=%d", charge_port);
			    ...
			    int max_ma, int charge_mv)
{
	...
	bd_sts = bd9995x_bc12_enable_charging(port, bc12_enable);
	trace3(0, BRD, 0,
		"brd_set_charge_limit: port=%d, en=%d returned %d",
		port, bc12_enable, bd_sts);
	if (bd_sts) {
		trace1(0, BRD, 0, "brd_set_charge_limit:fail=%d",
			bd_sts);
		return;
	}
	...
	trace1(0, BRD, 0, "board_set_charge_limit: %d mA", charge_ma);
}

/* MCHP from firmware-reef-9042.B
 * Do we need this?
 */
#if 1
/**
...
int board_is_vbus_too_low(int port, enum chg_ramp_vbus_state ramp_state)
{
	...
	rv = charger_get_vbus_voltage(port) < BD9995X_BC12_MIN_VOLTAGE;
	trace1(0, BRD, 0, "board_is_vbus_too_low: %d", rv);
	return rv;
}
...
{
	trace0(0, BRD, 0,
		"brd_chipset_startup: Enable USB-A and trackpad");
	...
	trace1(0, BRD, 0,
		"brd_chipset_startup: GPIO_USB1_ENABLE = %d",
		gpio_get_level(GPIO_USB1_ENABLE));
...
static void board_chipset_shutdown(void)
{
	...
	trace1(0, BRD, 0,
		"brd_chipset_shutdown: GPIO_USB1_ENABLE = %d",
		gpio_get_level(GPIO_USB1_ENABLE));
...
{
	trace0(0, PWR, 0,
		"chipset_do_shutdown: shut down pmic, pp33, pp50");
	/* Disable PMIC */
	...
	while (gpio_get_level(GPIO_PP3300_PG))
		;
	trace0(0, PWR, 0, "chipset_do_shutdown: GPIO_PP3300_PG = 0");

	/*Disable 5V rail */
	gpio_set_level(GPIO_EN_PP5000, 0);
	while (gpio_get_level(GPIO_PP5000_PG))
		;
	trace0(0, PWR, 0, "chipset_do_shutdown: GPIO_PP5000_PG = 0");
}
...
{
	trace0(0, BRD, 0, "board_hibernate");
	/*
	...
	msleep(100);
	trace0(0, BRD, 0, "board_hibernate: Enable Vbus & Vcc");

	/* Enable both the VBUS & VCC ports before entering PG3 */
	bd9995x_select_input_port(BD9995X_CHARGE_PORT_BOTH, 1);

	trace0(0, BRD, 0, "Disable BGATE_OFF on BD9995x");
...
int board_get_version(void)
{
	...
	trace2(0, BRD, 0,
		"board_get_version: adc_read_channel %d returned %d",
		ADC_BOARD_ID, mv);
	...
	trace1(0, BRD, 0, "board_get_version: version=%d", version);
