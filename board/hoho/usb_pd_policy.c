...
static int svdm_enter_mode(int port, uint32_t *payload)
{
	...
	if ((PD_VDO_VID(payload[0]) == USB_SID_DISPLAYPORT) &&
		...
		rv = 1;
		pd_log_event(PD_EVENT_ACC_DP_MODE, 0, 1, NULL);
	} else if ((PD_VDO_VID(payload[0]) == USB_VID_GOOGLE) &&
		   (PD_VDO_OPOS(payload[0]) == OPOS_GFU)) {
		alt_mode[PD_AMODE_GOOGLE] = OPOS_GFU;
		rv = 1;
		pd_log_event(PD_EVENT_ACC_GFU_MODE, 0, 1, NULL);
	}
...
static int svdm_exit_mode(int port, uint32_t *payload)
{
	if (PD_VDO_VID(payload[0]) == USB_SID_DISPLAYPORT) {
		...
		pd_log_event(PD_EVENT_ACC_DP_MODE, 0, 0, NULL);
	} else if (PD_VDO_VID(payload[0]) == USB_VID_GOOGLE) {
		alt_mode[PD_AMODE_GOOGLE] = 0;
		pd_log_event(PD_EVENT_ACC_GFU_MODE, 0, 0, NULL);
	} else {
