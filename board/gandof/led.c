 ...
 *
 * Power and battery LED control for Gandof.
 */
...
static int gandof_led_set_gpio_bat(enum led_color color,
...
static int gandof_led_set_gpio_pwr(enum led_color color,
			      enum gpio_signal gpio_led_blue_l,
			      enum gpio_signal gpio_led_amber_l)
{
	switch (color) {
...
{
	return gandof_led_set_gpio_bat(color, GPIO_BAT_LED0_L, GPIO_BAT_LED1_L);
}

static int gandof_led_set_color_power(enum led_color color)
{
	return gandof_led_set_gpio_pwr(color, GPIO_PWR_LED0_L, GPIO_PWR_LED1_L);
}
...
static int gandof_led_set_color(enum ec_led_id led_id, enum led_color color)
{
	...
	case EC_LED_ID_BATTERY_LED:
		rv = gandof_led_set_color_battery(color);
		break;
	case EC_LED_ID_POWER_LED:
		rv = gandof_led_set_color_power(color);
		break;
	default:
		return EC_ERROR_UNKNOWN;
	}
...
static void gandof_led_set_battery(void)
{
	...
	case PWR_STATE_DISCHARGE:
		...
		/* Less than 10%, blink one second every four seconds */
			...
			gandof_led_set_color_battery(LED_OFF);
		break;
...
static void led_tick(void)
{
	if (led_auto_control_is_enabled(EC_LED_ID_POWER_LED))
		gandof_led_set_power();

	if (led_auto_control_is_enabled(EC_LED_ID_BATTERY_LED))
		gandof_led_set_battery();
}
