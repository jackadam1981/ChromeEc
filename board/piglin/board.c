...
#include "driver/accelgyro_lsm6dstq.h"
...
const mat33_fp_t base_standard_ref_lsm6dstq = {
	{ FLOAT_TO_FP(1), 0, 0},
	{ 0, FLOAT_TO_FP(1), 0},
	{ 0, 0, FLOAT_TO_FP(-1)}
};

static const mat33_fp_t lid_standard_ref_lsm6dstq = {
	{ 0, FLOAT_TO_FP(1), 0},
	{ FLOAT_TO_FP(1), 0, 0},
	{ 0, 0, FLOAT_TO_FP(1)}
};
...
struct motion_sensor_t motion_sensors[] = {
	[LID_ACCEL] = {
		...
		.port = I2C_PORT_SENSOR,
		.i2c_spi_addr_flags = LSM6DSO_ADDR1_FLAGS,
		/* identity matrix */
		.rot_standard_ref = &lid_standard_ref_lsm6dstq,
		.default_range = 4, /* g */
	...
	[BASE_ACCEL] = {
		...
		.rot_standard_ref = &base_standard_ref_lsm6dstq,
		.default_range = 4,  /* g */
		.min_frequency = LSM6DSO_ODR_MIN_VAL,
		.max_frequency = LSM6DSO_ODR_MAX_VAL,
		.config = {
...
static void board_detect_motionsensor(void)
{
	...
	ret = i2c_read8(I2C_PORT_SENSOR, LSM6DSO_ADDR0_FLAGS,
		BMA2x2_CHIP_ID_ADDR, &val);

	CPRINTS("Lid Accel: %s", ret ? "Not Found" : "LSM6DSTQ");

	/* Check base accelgyro chip */
	ret = i2c_read8(I2C_PORT_SENSOR, LSM6DSO_ADDR1_FLAGS,
			BMA2x2_CHIP_ID_ADDR, &val);

	CPRINTS("Base Accelgyro: %s", (ret) ? "Not Found" : "LSM6DSTQ");
}
...
__override int board_get_version(void)
{
	...
	if (board_id == -1) {
		...
		bits[0] = gpio_get_ternary(GPIO_BRD_ID0);
		bits[1] = gpio_get_ternary(GPIO_BRD_ID1);
		bits[2] = gpio_get_ternary(GPIO_BRD_ID2);
		board_id = binary_first_base3_from_bits(bits, ARRAY_SIZE(bits));
	}
