...
All the following instructions have been verified in a ChromiumOS
...
The [schematic for the design](glados_pd.pdf) shows three main areas.
...
and the data transmitted on `USB_Cx_CCy_TX_DATA` and the two resistor
...
provide the required Vgs=-0.7 (Vgsth max should be 1V) to turn on the
FET and connect the Rd pulldown. When there is power and the
microcontroller is running it will drive `EN_PP3300_USB_PD` high and
disable the FET.
...
runs the [`glados_pd`](../glados_pd) code.
