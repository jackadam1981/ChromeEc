...
static void board_setup_panel(void)
{
	...
	if (board_version >= 3) { /* P0d onward */
		switch(lcm_id) {
			case PANEL_BOE_HIMAX8279D8P_ID:
				channel = 0xfa;
				dim = 0xc8;
				break;
			case PANEL_BOE_HIMAX8279D10P_ID:
				channel = 0xfe;
				dim = 0xc4;
				break;
			default:
...
static void board_get_lcm_id(void)
{
	int mv;
	int i;
	...
	for (i = 0; i < ARRAY_SIZE(lcm_ids); i++) {
		if (ABS(mv - lcm_ids[i].expect_mv) < MARGIN_MV) {
			lcm_id = lcm_ids[i].id;
			break;
		}
	}

	CPRINTS("LCM ID: %d", lcm_id);
}
DECLARE_HOOK(HOOK_INIT, board_get_lcm_id, HOOK_PRIO_FIRST);
