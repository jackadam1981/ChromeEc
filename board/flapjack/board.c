...
static void board_get_panel_id(void)
{
	int mv;
	int i;
...
DECLARE_HOOK(HOOK_INIT, board_get_panel_id, HOOK_PRIO_FIRST);
...
static void cbi_init(void)
{
	...
	if (cbi_get_sku_id(&val) == EC_SUCCESS)
		sku = val;
	if (board_version >= FLAPJACK_BOARD_P0D) /* P0d onward */
		sku |= ((pid & 0xf) << 16); /* Use bit[19-16] for lcm */
