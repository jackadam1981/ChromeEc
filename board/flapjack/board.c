...
struct {
	...
	{ BATTERY_ATL_8,       900 },  /* 100K  */
	{ BATTERY_SUNWODA_8,  1484 },  /* 470K  */
	{ BATTERY_ATL_10,      576 },  /* 47K   */
	{ BATTERY_SUNWODA_10, 1200 },  /* 200K  */
};
...
{
	static int batt_type = BATTERY_UNKNOWN;
	int mv;
	int i;

	if (batt_type != BATTERY_UNKNOWN)
		return batt_type;

	mv = board_read_adc(ADC_BATT_ID);

	/* CPRINTS("*** BATT_ID mv: %d", mv); */
	...
	for (i = 0; i < ARRAY_SIZE(flapjack_batteries); ++i) {
		if ((mv <= flapjack_batteries[i].expect_mv + THRESHOLD_MV) &&
			...
			batt_type = flapjack_batteries[i].battery_type;
			break;
		}
	}

	/* CPRINTS("*** BATT_ID: %d", batt_type); */
	return batt_type;
}
