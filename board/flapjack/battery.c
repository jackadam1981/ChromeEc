...
#define BATTERY_UNKNOWN_CHARGE_MIN_TEMP 0
#define BATTERY_UNKNOWN_CHARGE_MAX_TEMP 60

/* TODO: need to fill in battery details for ATL_8, SUNWODA_8, ATL_10,
 * and SUNWODA_10 */

static const struct battery_info info[BATT_COUNT] = {
	[BATT_ID_UNKNOWN] = {
		.voltage_max		= 4400,
		.voltage_normal		= 3860,
		.voltage_min		= 3000,
...
static const struct max17055_batt_profile batt_profile[BATT_COUNT] = {
	[BATT_ID_UNKNOWN] = {
		.is_ez_config		= 1,
...
static const struct max17055_alert_profile alert_profile[BATT_COUNT] = {
	[BATT_ID_UNKNOWN] = {
		.v_alert_mxmn = VALRT_DISABLE,
		.t_alert_mxmn = MAX17055_TALRTTH_REG(
			BATTERY_UNKNOWN_CHARGE_MAX_TEMP,
			BATTERY_UNKNOWN_CHARGE_MIN_TEMP),
...
{
	return &info[board_get_battery_id()];
}

const struct max17055_batt_profile *max17055_get_batt_profile(void)
{
	return &batt_profile[board_get_battery_id()];
}
...
int charger_profile_override(struct charge_state_data *curr)
{
	...
	} temp_zones[BATT_COUNT][TEMP_ZONE_COUNT] = {
		[BATT_ID_UNKNOWN] = {
			/* TEMP_ZONE_0 */
			{BATTERY_UNKNOWN_CHARGE_MIN_TEMP * 10, 150, 1772, 4376},
			/* TEMP_ZONE_1 */
			{150, 450, 4020, 4376},
			/* TEMP_ZONE_2 */
			{450, BATTERY_UNKNOWN_CHARGE_MAX_TEMP * 10, 3350, 4300},
		},
	};
	BUILD_ASSERT(ARRAY_SIZE(temp_zones[0]) == TEMP_ZONE_COUNT);
	BUILD_ASSERT(ARRAY_SIZE(temp_zones) == BATT_COUNT);

	if ((curr->batt.flags & BATT_FLAG_BAD_TEMPERATURE) ||
	    (bat_temp_c < temp_zones[board_get_battery_id()][0].temp_min) ||
	    (bat_temp_c >= temp_zones[board_get_battery_id()][TEMP_ZONE_COUNT - 1].temp_max))
		temp_zone = TEMP_OUT_OF_RANGE;
	else {
		for (temp_zone = 0; temp_zone < TEMP_ZONE_COUNT; temp_zone++) {
			if (bat_temp_c <
				temp_zones[board_get_battery_id()][temp_zone].temp_max)
				break;
		}
	}
	...
	case TEMP_ZONE_2:
		curr->requested_current =
			temp_zones[board_get_battery_id()][temp_zone].desired_current;
		curr->requested_voltage =
			temp_zones[board_get_battery_id()][temp_zone].desired_voltage;
		break;
