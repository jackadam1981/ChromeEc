...
int charger_profile_override(struct charge_state_data *curr)
{
	...
	if ((curr->batt.flags & BATT_FLAG_BAD_TEMPERATURE) ||
		...
		cur_temp_zone = TEMP_OUT_OF_RANGE;
	else {
		for (cur_temp_zone = TEMP_ZONE_0;
		     cur_temp_zone < TEMP_ZONE_COUNT;
		     cur_temp_zone++) {
			if (bat_temp_c <
				temp_zones[batt_type][cur_temp_zone].temp_max)
				break;
		}
	}

	if (cur_temp_zone == TEMP_OUT_OF_RANGE) {
		curr->requested_current = curr->requested_voltage = 0;
		curr->batt.flags &= ~BATT_FLAG_WANT_CHARGE;
		curr->state = ST_IDLE;
	} else {
		curr->requested_current =
			temp_zones[batt_type][cur_temp_zone].desired_current;
		curr->requested_voltage =
			temp_zones[batt_type][cur_temp_zone].desired_voltage;
	}
