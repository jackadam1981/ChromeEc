...
static const struct max17055_alert_profile alert_profile[BATTERY_COUNT] = {
	[BATTERY_ATL_8] = {
		...
		.t_alert_mxmn = MAX17055_TALRTTH_REG(60, 0),
...
{
#if 1
	return BATTERY_ATL_8;
#else
	static int b_id = BATTERY_UNKNOWN;
	if (b_id != BATTERY_UNKNOWN)
		return b_id;
	b_id = board_get_battery_type();
	return b_id;
#endif 
}
...
int charger_profile_override(struct charge_state_data *curr)
{
	...
	if ((curr->batt.flags & BATT_FLAG_BAD_TEMPERATURE) ||
	    (bat_temp_c < temp_zones[get_battery_id()][0].temp_min) ||
	    (bat_temp_c >= temp_zones[get_battery_id()][TEMP_ZONE_COUNT - 1].temp_max))
		temp_zone = TEMP_OUT_OF_RANGE;
	else {
		for (temp_zone = 0; temp_zone < TEMP_ZONE_COUNT; temp_zone++) {
			if (bat_temp_c <
				temp_zones[get_battery_id()][temp_zone].temp_max)
				break;
		}
	}
	...
	case TEMP_ZONE_2:
		curr->requested_current =
			temp_zones[get_battery_id()][temp_zone].desired_current;
		curr->requested_voltage =
			temp_zones[get_battery_id()][temp_zone].desired_voltage;
		break;
