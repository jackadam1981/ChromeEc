...
const struct usb_mux_chain usb_muxes[] = {
	[USBC_PORT_C0] = {
		.mux = &(const struct usb_mux) {
			.usb_port = USBC_PORT_C0,
			/*
			 * USBA card connect to chromebook the
			 * USB_3_CONNECTION bit would be enable.  It
			 * will increase BBR power consumption, so clear
			 * USB3_Connection bit in S0ix and enable when
			 * return S0.
			 */
