...
{
	enum gpio_signal tcpc_rst_L;
	enum gpio_signal tcpc_rst_R;

	tcpc_rst_L = GPIO_USB_C0_C1_TCPC_RST_ODL;
	tcpc_rst_R = GPIO_USB_C2_C3_TCPC_RST_ODL;
	/*
	 * TODO(b/179648104): figure out correct timing
	 */
	gpio_set_level(tcpc_rst_L, 0);
	gpio_set_level(tcpc_rst_R, 0);
	/*
	 * delay for power-on to reset-off and min. assertion time
	 */
	msleep(20);
	gpio_set_level(tcpc_rst_L, 1);
	gpio_set_level(tcpc_rst_R, 1);
...
uint16_t tcpc_get_alert_status(void)
{
	...
	if ((ec_cfg_usb_db_type() != DB_USB_ABSENT) &&
	    gpio_get_level(GPIO_USB_C2_C3_TCPC_INT_ODL) == 0)
...
void ppc_interrupt(enum gpio_signal signal)
{
	...
	case GPIO_USB_C3_PPC_INT_ODL:
		nx20p348x_interrupt(USBC_PORT_C3);
		break;
	default:
		break;
	}
