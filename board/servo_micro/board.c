...
enum digital_line_level {
	LINE_LEVEL_LOW = 0,
	LINE_LEVEL_HIGH = 1,
};

/* The static constants below are values that one might want to try changing if
...
static const int START_DELAY_MS = 5;
static const int SPECIAL_WAVEFORM_MS = 50;
static const int PLL_STABLE_MS = 10;

/* Digital line levels to hold before (PRE_) or after (POST_) sending the
 * special waveforms.
 */
static const enum digital_line_level SMCLK_PRE_LEVEL = LINE_LEVEL_LOW;
static const enum digital_line_level SMDAT_PRE_LEVEL = LINE_LEVEL_LOW;
static const enum digital_line_level SMCLK_POST_LEVEL = LINE_LEVEL_LOW;
static const enum digital_line_level SMDAT_POST_LEVEL = LINE_LEVEL_LOW;
