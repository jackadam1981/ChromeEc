...
static void sub_hdmi_hpd_interrupt(enum gpio_signal s)
{
	...
	CPRINTS("HDMI plug-%s", !hdmi_hpd_odl ? "in" : "out");
}
...
void board_init(void)
{
	...
	if (base_gyro_config == SSFC_SENSOR_LSM6DSM) {
		...
		ccprints("SSFC: BASE GYRO is LSM6DSM");
	} else
		ccprints("SSFC: BASE GYRO is BMI160");

	if (get_cbi_ssfc_lid_sensor() == SSFC_SENSOR_KX022) {
		motion_sensors[LID_ACCEL] = kx022_lid_accel;
		ccprints("SSFC: LID ACCEL is KX022");
	} else
		ccprints("SSFC: LID ACCEL is BMA253");
