...
static void board_led_mode_switch(void)
{
	...
	if (chipset_in_state(CHIPSET_STATE_ANY_SUSPEND) &&
		...
		task_wake(TASK_ID_LED_BREATHONG);
	} else {
...
static void led_breathong(void)
{
	static int count = 100;
	static int state_up = 1;

	//CPRINTS("%s run", __func__);
	...
	//CPRINTS("pwm_set_duty: %d", count);
	pwm_set_duty(PWM_CH_LED_AMBER, count);
}

void led_breathong_task(void *u)
{
	CPRINTS("%s init", __func__);
	while (1) {
