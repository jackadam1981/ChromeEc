...
static int uart_detect = UART_AUTO;
static void set_uart_gpios(int state)
{
	...
	gpio_set_flags(GPIO_USART1_FLIP_TX, uart_flip);
	gpio_set_flags(GPIO_USART1_FLIP_RX, uart_flip);

	STM32_USART_CR1(STM32_USART1_BASE) &= ~(STM32_USART_CR1_UE);
	if (dir)
		STM32_USART_CR2(STM32_USART1_BASE) &= ~(1 << 15);
	else
		STM32_USART_CR2(STM32_USART1_BASE) |= (1 << 15);
	STM32_USART_CR1(STM32_USART1_BASE) |= STM32_USART_CR1_UE;
...
static int detect_uart_enabled(void)
{
	int sbu1 = adc_read_channel(ADC_SBU1);
	int sbu2 = adc_read_channel(ADC_SBU2);
	int enabled = 0;

	if ((sbu1 > 1600) && (sbu1 < 1900) && (sbu2 > 1600) && (sbu2 < 1900))
		enabled = 1;
...
void set_uart_state(int state)
{
	if (state == UART_AUTO) {
		...
		uart_detect = UART_AUTO;
		state = detect_uart_orientation();
	} else {
		uart_detect = UART_OFF;
	}
 ...
 * in the new state. TODO: Is theis reliable and fast enough?
 */
void uart_sbu_tick(void)
{
	static int debounce;  /* = 0 */

	if (uart_detect != UART_AUTO)
		return;
	...
	if (uart_state == UART_OFF) {
		...
		if (state != UART_OFF) {
			...
			if (debounce > 4) {
				debounce = 0;
				ccprintf("UART autoenable\n");
				set_uart_state(state);
			}
		}
	} else {
		int enabled = detect_uart_enabled();

		if (!enabled) {
			debounce++;
			if (debounce > 4) {
				debounce = 0;
				ccprintf("UART autodisable\n");
