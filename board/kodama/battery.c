	...
	if (!(curr->batt.flags & BATT_FLAG_BAD_VOLTAGE)) {
		if (curr->batt.voltage < batt_info->voltage_min) {
			...
			if (!normal_charge_lock) {
				...
				/*
				 * b/148045048: When the battery is
				 * activated in shutdown mode, the
				 * adapter cannot boot DUT automatically.
				 * It's a workaround to delay 4.5 second
				 * to enable charger EOC and TE function.
				 */
				hook_call_deferred(
