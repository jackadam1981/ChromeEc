...
int charger_profile_override(struct charge_state_data *curr)
{
	...
	if (!curr->batt.is_present &&
		 ...
		 * shutdown.Keep max charging voltage as 4.4V when remove
		 * battery in S0 to not let the system to trigger under
		 * voltage (Vsys < 2.5V).
		 */
		CPRINTS("found battery in disconnect state");
		curr->requested_voltage = batt_info->voltage_maxD;
