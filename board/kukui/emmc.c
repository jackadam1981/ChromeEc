...
static uint32_t in_msg[SPI_RX_BUF_SIZE_32];
...
static void bootblock_stop(void)
{
	...
	/* Then flush SPI FIFO, and make sure DAT line stays idle (1) */
...
static enum emmc_cmd emmc_parse_command(int index)
{
	...
	mask1 = (shift0 > 0) ? ((int32_t)0x80000000 >> (shift0-1)) : 0x00000000;
...
void emmc_task(void *u)
{
	...
	while (1) {
		...
		/* We missed the command? That should not happen. */
		if (i == dma_pos) {
			CPRINTF("!");
			continue;
		}
		...
		if (!tx) {
			 ...
			 * then Boot commands. But all we really care about is
			 * the Boot command: start the transfer.
			 */
			if (cmd == EMMC_BOOT) {
				tx = 1;
				bootblock_transfer();
			 ...
			 * incorrect number of lanes is used), but also react
			 * to Pre-Idle.
			 */
