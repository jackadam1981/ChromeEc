...
struct {
	...
	[DEVICE_TYPE_KEYBOARD] = {270, 330},   /* 10K, 1K ohm */
};
BUILD_ASSERT(ARRAY_SIZE(pogo_detect_table) == DEVICE_TYPE_COUNT);

static uint64_t base_detect_debounce_time;

enum kukui_pogo_device_type get_device_type(int mv)
{
	int i;
...
static void base_detect_deferred(void)
{
	...
	if (mv == ADC_READ_ERROR || device_type == DEVICE_TYPE_UNKNOWN) {
		hook_call_deferred(&base_detect_deferred_data,
				BASE_DETECT_RETRY_US);
		return;
	}

	if (device_type == DEVICE_TYPE_DOCK) {
		enable_power_supply(0);
		enable_charge(1);
	}

	if (device_type == DEVICE_TYPE_DETACHED) {
		enable_power_supply(0);
		enable_charge(0);
	}
