...
static void  in_ready( in_stream const * stream) { task_wake(TASK_ID_ECHO); }
static void out_ready(out_stream const * stream) { task_wake(TASK_ID_ECHO); }
...
{
	size_t wrote;
} stream_console_state;

typedef struct
{
	stream_console_state * state;

	in_stream  const * in;
	out_stream const * out;
} stream_console_config;

#define STREAM_CONSOLE_CONFIG(NAME, IN, OUT)		\
	static stream_console_state NAME##_state;	\
	stream_console_config const NAME =		\
	{						\
		.state = &NAME##_state,			\
		.in    = IN,				\
		.out   = OUT,				\
...
static stream_console_config const * const consoles[] =
{
...
static size_t echo(stream_console_config const * const consoles[],
