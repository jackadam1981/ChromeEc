...
static void (*kblight_set)(int level);
 ...
 * PWM routines
 */
static void kblight_pwm_set(int level)
{
	pwm_set_duty(PWM_CH_KBLIGHT, level);
}
 ...
 * I2C routines
 */
static void kblight_i2c_set(int level)
{
	lm3509_set_brightness(level);
}

static int kblight_i2c_get(void)
{
	int level;
	if (lm3509_get_brightness(&level))
		level = 0;
	return level;
}
