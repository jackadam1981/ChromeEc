...
static uint8_t battery_error_grace_period;
static void battery_disconnect_timer(void)
{
	battery_error_grace_period = 2;
}
...
static int battery_check_disconnect_ti_bq40z50(void)
{
	...
	if ((data[3] & (BATTERY_DISCHARGING_DISABLED |
		 ...
		 */
		if (battery_error_grace_period == 2)
			return BATTERY_DISCONNECTED;
		if (battery_error_grace_period == 0)
			hook_call_deferred(&battery_disconnect_timer_data,
					   5 * SECOND);
		battery_error_grace_period = 1;
		return BATTERY_DISCONNECT_ERROR;
	}
