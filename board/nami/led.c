 ...
 *
 * Since we care about transitional power states (e.g. s0->s3) not stable power
 * state (e.g. s3), the current power state is saved in power_state. Charging
 * states are queried each time there is a state transition, thus, not stored.
 *
...
/* Charging states from LED's perspective */
...
/* Power states from LED's perspective */
...
static int set_color_power(enum led_color color, int duty)
{
	if (color == LED_OFF)
		duty = 0;
	gpio_set_level(GPIO_LED1, duty ? 0 : 1 /* Reversed logic */);
	return EC_SUCCESS;
}
