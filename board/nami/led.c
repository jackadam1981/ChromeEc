...
DECLARE_DEFERRED(led_status_update);

/* Deferred 500 ms to wait for battery/system status stable,
 * then to update LEAD status.
 */
static void led_change_deferred(void)
{
	hook_call_deferred(&led_status_update_data, 500 * MSEC);
}
DECLARE_HOOK(HOOK_AC_CHANGE, led_change_deferred, HOOK_PRIO_DEFAULT);
DECLARE_HOOK(HOOK_BATTERY_SOC_CHANGE, led_change_deferred, HOOK_PRIO_DEFAULT);
DECLARE_HOOK(HOOK_CHIPSET_SUSPEND, led_change_deferred, HOOK_PRIO_DEFAULT);
DECLARE_HOOK(HOOK_CHIPSET_SHUTDOWN, led_change_deferred, HOOK_PRIO_DEFAULT);
DECLARE_HOOK(HOOK_CHIPSET_RESUME, led_change_deferred, HOOK_PRIO_DEFAULT);
