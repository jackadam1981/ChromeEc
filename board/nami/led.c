...
#define SET_TICK(interval, color) \
	set_tick((interval), 100 / (LED_PULSE_US / (interval)), (color))

static void set_tick(uint32_t interval, int duty_inc, enum led_color color)
{
...
static void board_led_set_battery(void)
{
	...
	case PWR_STATE_ERROR:
		if (!breath_enabled) {
			SET_TICK(LED_PULSE_TICK_US, LED_AMBER);
			led_tick();
			breath_enabled = 1;
		}
		break;
