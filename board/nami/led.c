...
static void led_error_tick(void);
DECLARE_DEFERRED(led_error_tick);
static void led_error_tick(void)
{
	if (led_auto_control_is_enabled(EC_LED_ID_LEFT_LED))
		led_battery_error();
	hook_call_deferred(&led_error_tick_data, led_interval);
}
...
static void led_update(enum led_states desired_state)
{
	...
	case STATE_CHARGING:
		...
		hook_call_deferred(&led_error_tick_data, -1);
...
DECLARE_HOOK(HOOK_AC_CHANGE, led_ac_change, HOOK_PRIO_DEFAULT);
