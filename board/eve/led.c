...
static void led_change_color(void)
{
	...
	while (led[LED_LEFT].state != LED_STATE_DONE ||
		...
		for (j = 0; j < LED_BOTH; j++) {
			int change = 0;
			uint8_t lvl = led[j].trans[led[j].step];
			uint8_t *rgb_c = led[j].rgb_current;
			const uint8_t *rgb_t = led[j].rgb_target;

			if (led[j].state == LED_STATE_INTENSITY_DOWN) {
				/*
				 * Colors are going from higher to lower level.
				 * If the current level of R, G, or B is higher
				 * than both the next step in the transition
				 * table and and the target level, then move to
				 * the larger of the two. The MAX is used to
				 * make sure that it doens't drop below
				 * the target level.
				 */
				for (i = 0; i < PWM_CHAN_PER_LED; i++) {
					if ((rgb_c[i] > rgb_t[i]) &&
				...
				/*
				 * If nothing changed this iteration, or if
				 * lowest table entry has been used, then the
				 * change is complete.
				 */
				if (!change || --led[j].step < 0)
					led[j].state = LED_STATE_DONE;

			} else if (led[j].state == LED_STATE_INTENSITY_UP) {
				/*
				 * Colors are going from lower to higher level.
				 * If the current level of R, G, B is lower than
				 * both the target level and the transition
				 * table entry for a given color, then move up
				 * to the MIN of next transition step and target
				 * level.
				 */
				for (i = 0; i < PWM_CHAN_PER_LED; i++) {
					if ((rgb_c[i] < rgb_t[i]) &&
				...
				/*
				 * If nothing changed this iteration, or if
				 * highest table entry has been used, then the
				 * change is complete.
				 */
				if (!change || ++led[j].step >=
				    ARRAY_SIZE(trans_steps))
					led[j].state = LED_STATE_DONE;
				}
...
static void led_manage_patterns(enum led_pattern *pattern_desired, int tap)
{
	...
	for (i = 0; i < LED_BOTH; i++) {
		...
		if (led[i].color != color) {
			CPRINTS("color chg[%d]: old = %d new = %d",
				i, led[i].color, color);
...
static void eve_led_select_pattern(enum led_pattern *pattern_desired, int tap)
{
	 ...
	 * active charging port, then charge_side = -1. This is value is used to
	...
	if (side == LED_BOTH) {
		 ...
		 * occurred.
		 */
		if (tap == 0 && new_pattern <= WHITE_RED)
			new_pattern = OFF;
		/*
		...
		/* Check for double tap for side not associcated with charger */
		new_pattern = led_get_double_tap_pattern(percent_chg);
...
static void led_init(void)
{
	 ...
	 * colors don't neet to be intialized as they are always computed when a
...
void led_task(void)
{
	...
	while (1) {
		...
		if (led_auto_control_is_enabled(EC_LED_ID_LEFT_LED) &&
		    ...
		    led_debug != 1) {
			eve_led_select_pattern(pattern_desired, tap);
