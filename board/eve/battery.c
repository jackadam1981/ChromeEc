...
static int board_battery_sb_write(int access, int cmd)
{
	int rv;
				      ...
				      uint8_t *buf, int len)
{
	...
	reg = access;
	rv = i2c_xfer(I2C_PORT_BATTERY, BATTERY_ADDR, &reg, 1, &block_len, 1,
		      I2C_XFER_START);
	if (rv) {
		i2c_lock(I2C_PORT_BATTERY, 0);
		return rv;
	...
	/* Compare block length to desried read length */
	if (len && (block_len > len))
		block_len = len;

	rv = i2c_xfer(I2C_PORT_BATTERY, BATTERY_ADDR, 0, 0, buf, block_len,
		      I2C_XFER_STOP);
	i2c_lock(I2C_PORT_BATTERY, 0);
...
int board_battery_write_flash(int addr, uint32_t data, int len)
{
	int rv;
...
static int board_battery_disable_cto(uint32_t value)
{
	...
	if (board_battery_read_flash(0x482c, 1, &protect_c) == EC_SUCCESS)
