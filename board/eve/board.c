...
static void board_pre_init(void)
{
	...
	if (system_get_board_version() == BOARD_VERSION_P0) {
		...
		for (rail = 0; rail <= 2; rail++) {
			/* Select rail */
			i2c_write8(I2C_PORT_MP2949, I2C_ADDR_MP2949,
				   0x00, rail);
			/* Set slew rate to 40mV/uS */
			i2c_write16(I2C_PORT_MP2949, I2C_ADDR_MP2949,
				    0xFA, 0x0AC5);
		}

		/* Reset to rail 0 */
		i2c_write8(I2C_PORT_MP2949, I2C_ADDR_MP2949, 0x00, 0x00);

		CPRINTS("P0 board - IMVP8 workaround applied");
	}
}
