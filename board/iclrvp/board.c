...
int board_get_version(void)
{
	 ...
	 * Port0: FAB ID([1:0] + 1)
	 * return format: FAB_ID[9:8], BOM ID[7:5], BOARD ID[4:0]
	 */
	return (data_port1 & 0xff) | (((data_port0 & 0x03) + 1) << 8);
}
...
#define IOEXPANDER_A IT83XX_I2C_CH_E
#define IOEXPANDER_A_ADDRESS 0x40

#define EC_DS4		IT8300_GPX_6 /* GPB6 */
#define EC_SPI_OE_N	IT8300_GPX_3 /* GPB3 */

static void board_init(void)
{
	int data;

	if (it8300_write(IOEXPANDER_A, IOEXPANDER_A_ADDRESS,
			IT8300_GPCR_B6, IT8300_GPCR_GP0_MODE))
		return;
	if (it8300_write(IOEXPANDER_A, IOEXPANDER_A_ADDRESS,
			IT8300_GPCR_B3, IT8300_GPCR_GP0_MODE))
		return;
	if (it8300_read(IOEXPANDER_A, IOEXPANDER_A_ADDRESS,
			IT8300_PDGR_B, &data))
		return;
	if (it8300_write(IOEXPANDER_A, IOEXPANDER_A_ADDRESS,
			IT8300_PDGR_B, data | EC_SPI_OE_N | EC_DS4))
		return;
}
