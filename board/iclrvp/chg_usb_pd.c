...
{
	/* TODO: Check polarity */
	gpio_set_level(port ? GPIO_USB_C0_SNK_EN_L : GPIO_USB_C1_SNK_EN_L, 1);
}

void board_vbus_enable(int port, int enable)
{
	/* TODO: Check polarity */
	gpio_set_level(port ? GPIO_USB_C0_SRC_EN_L : GPIO_USB_C1_SRC_EN_L, 1);
}
...
int pd_snk_is_vbus_provided(int port)
{
	...
	/* TODO: Check polarity */
	return gpio_get_level(port ?
		GPIO_USB_C0_VBUS_INT_ODL : GPIO_USB_C1_VBUS_INT_ODL);
}
...
void board_tcpc_init(void)
{
	...
	gpio_enable_interrupt(GPIO_USB_C0_VBUS_INT_ODL);
	gpio_enable_interrupt(GPIO_USB_C1_VBUS_INT_ODL);
}
...
uint16_t tcpc_get_alert_status(void)
{
	...
	if (!gpio_get_level(GPIO_USB_C0_VBUS_INT_ODL))
		status |= PD_STATUS_TCPC_ALERT_0;

	if (!gpio_get_level(GPIO_USB_C1_VBUS_INT_ODL))
