...
static void led_set_battery(void)
{
	...
	case PWR_STATE_DISCHARGE:
		/*
		 * When battery is nearly full, we stop charging to protect
		 * battery even with a charger is plugged.
		 */
		if (chipset_in_state(CHIPSET_STATE_ON)) {
			led_set_color_battery(LED_BLUE);
		} else if (extpower_is_present()) {
			/* Charger says discharging but we know battery is
			 * working and external charger is plugged. That's
			 * enough to believe it'll start charging soon or
			 * later. */
			led_set_color_battery(LED_AMBER);
