...
{
	int rv;
	enum battery_type board_battery =
			board_battery_type == BATTERY_TYPE_COUNT ?
				DEFAULT_BATTERY_TYPE : board_battery_type;

	/* Ship mode command must be sent twice to take effect */
	rv = sb_write(info[board_battery].ship_mode_reg,
			info[board_battery].ship_mode_data);
	if (rv != EC_SUCCESS)
		return rv;

	return sb_write(info[board_battery].ship_mode_reg,
			info[board_battery].ship_mode_data);
}
	...
	if (batt_pres == BP_YES && batt_pres_prev != batt_pres &&
		...
		if (board_get_battery_type() == BATTERY_TYPE_COUNT) {
			if (bd9995x_get_battery_voltage() >=
			    info[DEFAULT_BATTERY_TYPE].batt_info.voltage_min)
				batt_pres = BP_NO;
