...
int charger_profile_override(struct charge_state_data *curr)
{
	...
	if (!(curr->batt.flags & BATT_FLAG_BAD_TEMPERATURE)) {
		...
		else if (temp_c > -1 && temp_c < 150)
			temp_range = TEMP_RANGE_2;
		else if (temp_c > 149 && temp_c < 200)
			temp_range = TEMP_RANGE_3;
		else if (temp_c > 199 && temp_c < 450)
	 ...
	 *
	 * TODO: Info not available in the RD_Doc:EE_PS_T2_Rev01
	 * When battery is < 0C:
	 * CC at 472mA @ 8.7V
	 * CV at 8.7V
	 *
