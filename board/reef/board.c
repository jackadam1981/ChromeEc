...
const struct tcpc_config_t tcpc_config[CONFIG_USB_PD_PORT_COUNT] = {
	...
	{NPCX_I2C_PORT0_0, 0x7c, &anx74xx_tcpm_drv},
	{NPCX_I2C_PORT0_1, 0x96, &tcpci_tcpm_drv},
};
...
void board_reset_pd_mcu(void)
{
	 ...
	 * does not indicate such a long delay is necessary. Leave it in out
	 * of paranoia.
	 */
	msleep(50);
}
...
int board_get_charger_temp(int idx, int *temp_ptr)
{
	...
	/* FIXME(dhendrix): convert raw voltage value to Kelvin */
	*temp_ptr = 0;
	return 0;
}
...
static void board_init(void)
{
	gpio_set_level(GPIO_EN_PP5000, 1);
	/*
	 * FIXME(dhendrix): power_wait_signals() would be better but can't be
	 * used here due to test build omitting it. Instead, double the PG low
	 * to high delay from power supply datasheet.
	 */
	udelay(6);
	/* FIXME(dhendrix): For debugging only */
	cprintf(CC_HOOK, "PP5000_PG: %d", gpio_get_level(GPIO_PP5000_PG));
...
static void board_chipset_shutdown(void)
{
	...
	udelay(10);	/* double EN low to power good delay for converter */
}
