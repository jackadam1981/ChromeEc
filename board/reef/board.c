...
const struct i2c_port_t i2c_ports[]  = {
	 ...
	 * tcpc0 (anx3429): 400KHz or 1000KHz (must enable HS mode if desired)
	 * tcpc1 (ps8751A): Up to 1000KHz (How to enable HS mode?
	 * gyro: Up to 1000KHz
	 * sensors:
	 * batt: Max is 100KHz
	 */
	{"tcpc0",     NPCX_I2C_PORT0_0, 400, GPIO_EC_I2C_USB_C0_PD_SCL, GPIO_EC_I2C_USB_C0_PD_SDA},
	{"tcpc1",     NPCX_I2C_PORT0_1, 400, GPIO_EC_I2C_USB_C1_PD_SCL, GPIO_EC_I2C_USB_C1_PD_SDA},
...
{
	/*
	 * FIXME(dhendrix): The CHIPSET_PRE_INIT hook (and thus board_pre_init()
	 * get called from within APL power state code. After PP5000 comes up we
	 * should see a change on RSMRSTB on the PMIC, thus causing an interrupt
	 * on PMIC_EC_RSMRST_ODL. So to avoid being interrupted while going thru
	 * the power states macgine, let's enable PP5000 in here instead.
	 */
	gpio_set_level(GPIO_EN_PP5000, 1);
	usleep(100);
	cprintf(CC_HOOK, "%s: PP5000_PG: %d", __func__, gpio_get_level(GPIO_PP5000_PG));
...
void board_hibernate(void)
{
	...
	/* FIXME(dhendrix): What to do here? EC is always on. */
