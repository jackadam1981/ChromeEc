...
{
	/*
	 * FIXME(dhendrix): The CHIPSET_PRE_INIT hook is called from within the
	 * APL power state code. After PP5000 comes up we should see a change on
	 * RSMRSTB on the PMIC, thus causing an interrupt on PMIC_EC_RSMRST_ODL
	 * So to avoid being interrupted while going thru the power state
	 * machine, let's enable PP5000 in here instead.
	 */
	gpio_set_level(GPIO_EN_PP5000, 1);
	usleep(100);
	cprintf(CC_HOOK, "%s: PP5000_PG: %d", __func__, gpio_get_level(GPIO_PP5000_PG));
