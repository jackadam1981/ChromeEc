...
DECLARE_DEFERRED(enable_sensor_i2c_bus);
...
static void enable_sensor_i2c_bus(void)
{
	...
	gpio_set_alternate_function(8, 0x80, 1);
	/* GPIO90 for EC_I2C_GYRO_SCL */
	gpio_set_alternate_function(9, 0x01, 1);
	/* GPIO92-91 for EC_I2C_SENSOR_SDA/SCL */
	gpio_set_alternate_function(9, 0x06, 1);
}
static void board_chipset_resume(void)
{
	hook_call_deferred(&enable_sensor_i2c_bus_data, 0);
}
...
static void board_chipset_startup(void)
{
	 ...
	 * other hooks and tasks of resources that are time-sensitive in
	 * the S3 -> S0 stage. To avoid timeouts, allow the hash to finish
...
{
	/* _S rails go low in S3 */
	/* GPIO87 for EC_I2C_GYRO_SDA */
	gpio_set_flags_by_mask(8, 0x80, GPIO_INPUT | GPIO_SEL_1P8V);
	/* GPIO90 for EC_I2C_GYRO_SCL */
	gpio_set_flags_by_mask(9, 0x01, GPIO_INPUT | GPIO_SEL_1P8V);
...
static void board_chipset_shutdown(void)
{
	...
	/* _U rails go low in S5 */
	/* GPIO92-91 for EC_I2C_SENSOR_SDA/SCL */
	gpio_set_flags_by_mask(9, 0x06, GPIO_INPUT | GPIO_SEL_1P8V);
}
