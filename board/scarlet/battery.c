...
/* Chagnge Charge Voltage by cyclye zones table*/
void charge_cyclevoltage(struct charge_state_data *curr)
{
	int bat_cycle;
...
static int charge_lowvoltage(void)
{
	if (acin_start_time.val &&
	   (get_time().val - acin_start_time.val > AC_IN_TIMEOUT_US))
		return 1;
	return 0;
}
