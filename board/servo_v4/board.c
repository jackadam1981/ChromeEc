...
/* Init the RO only features */
static void jump_to_rw_deferred(void)
{
	system_run_image_copy(SYSTEM_IMAGE_RW);
}
DECLARE_DEFERRED(jump_to_rw_deferred);

static void ro_board_init(void)
{
	/* We will automatically bump to RW if we have just powered on.
	 * After a 10 second timeout, if USB console or update has not
	 *  connected, we'll sysjump to the more full featured RW.
	 */
	if (!system_jumped_to_this_image())
		hook_call_deferred(&jump_to_rw_deferred_data, SECOND*10);
}

/* Dummmy pd_task since RO doesn't support it. */
void pd_task(void)
{
	while (1)
		task_wait_event(0);
