...
int pd_snk_is_vbus_provided(int port)
{
	...
	do {
		mv = 0;
		res = i2c_read16(1, 0x80, 0x2, &mv);
		mv = ((mv >> 8) & 0xff) | ((mv << 8) & 0xff00);
		mv = (40958 * mv) / 0x7fff;
		tries ++;
	} while ((res != EC_SUCCESS) && (tries < 10));

	//CPRINTF("pd_snk_is_vbus_provided: mv = %d, r = %d\n", mv, res);
	if (mv > 4500)
		return 1;
	/* Cant check this for CHG!! Use INA for DUT? */
	return 0;
#endif
}
...
{
	/* TODO: Allow power swap on DUT, not on CHG (force sink)*/
	/*
...
{
	/* TODO(nsanders): I have no idea what this is. */
	return 0;
}
