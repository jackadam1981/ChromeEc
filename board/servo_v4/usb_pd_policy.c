...
{
	int rp;
	static uint32_t pdo_count_prev, pdo_prev[2];
	 ...
	 */
	if (pdo_count_prev != chg_pdo_cnt ||
	    pdo_prev[0] != pd_src_chg_pdo[0] ||
	    pdo_prev[1] != pd_src_chg_pdo[1])
		/*
...
{
	int v5_ma;
	uint32_t pdo_index, max_ma, max_mv;
	...
	if (!charge_port_is_active()) {
		...
		/*
		 * Check if a CHG partner > 5V PDO is available and
		 * advertise it to the DUT.
		 */
		if (active_charge_supplier == CHARGE_SUPPLIER_PD) {
			max_mv = board_get_version() >= BOARD_VERSION_BLACK ?
				 PD_MAX_VOLTAGE_MV : MAX_MV_RED_BLUE;
			pdo_index = pd_find_pdo_index(CHG, max_mv);
			if (pdo_index > 0) {
				pd_extract_pdo_power(CHG, pdo_index,
						     &max_ma, &max_mv);
				pd_src_chg_pdo[1] =
...
void pd_transition_voltage(int idx)
{
	...
	if (charge_port_is_active() && vbus[CHG].mv != mv) {
		...
		pd_set_external_voltage_limit(
			CHG, board_get_version() >= BOARD_VERSION_BLACK ?
			PD_MAX_VOLTAGE_MV : MAX_MV_RED_BLUE);
