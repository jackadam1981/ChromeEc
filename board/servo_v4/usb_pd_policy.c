 ...
 *
 * TODO(b/137219603): Make this pin assignment configurable by a console command
 * that some tests can check different dongle behaviors.
 */
...
static int is_typec_dp_muxed(void)
{
	...
	i2c_read8(I2C_PORT_MASTER, PS8742_I2C_ADDR0, PS8742_REG_MODE, &value);
	return value & PS8742_MODE_DP ? 1 : 0;
}
...
static void set_typec_mux(int pin_cfg)
{
	...
	case MODE_DP_PIN_C:
		 ...
		 * to 2-lane, i.e. PigCfg:D to workaround.
		 */
		value = PS8742_MODE_DP | PS8742_MODE_USB;
		CPRINTS("PinCfg:C\n");
		break;
	case MODE_DP_PIN_D:
		value = PS8742_MODE_DP | PS8742_MODE_USB;
		...
		break;
	case MODE_DP_PIN_E:
		value = PS8742_MODE_DP | PS8742_MODE_PIN_E_OR_C;
		CPRINTS("PinCfg:E\n");
		break;
	default:
		CPRINTS("PinCfg not supported: %d\n", pin_cfg);
		return;
	}
	if (value && cc_config & CC_POLARITY)
		value |= 0x10;
	i2c_write8(I2C_PORT_MASTER, PS8742_I2C_ADDR0, PS8742_REG_MODE, value);
}
