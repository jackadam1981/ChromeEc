 ...
 * capablities with supporting all of them.
 */
...
static int is_typec_dp_muxed(void)
{
	...
	i2c_read8(I2C_PORT_MASTER, PS874X_I2C_ADDR0_FLAG, PS874X_REG_MODE,
		  &value);
	return value & PS874X_MODE_DP_ENABLED ? 1 : 0;
}
...
static void set_typec_mux(int pin_cfg)
{
	...
	case MODE_DP_PIN_D:
		...
		break;
	case MODE_DP_PIN_E:
		value = PS874X_MODE_DP_ENABLED | PS874X_MODE_CE_DP_ENABLED;
		CPRINTS("PinCfg:E");
		break;
	default:
	...
	if (value && cc_config & CC_POLARITY)
		value |= PS874X_MODE_POLARITY_INVERTED;
	i2c_write8(I2C_PORT_MASTER, PS874X_I2C_ADDR0_FLAG, PS874X_REG_MODE,
		   value);
}

static int get_hpd_level(void)
{
	if (alt_dp_config & ALT_DP_MOD_HPD)
		return !!(alt_dp_config & ALT_DP_HPD_LVL);
	else
...
static int cmd_dp_action(int argc, char *argv[])
{
	...
	if (!strcasecmp(argv[1], "enable")) {
		...
		CPRINTS("MF pref: %d", !!(alt_dp_config & ALT_DP_MF_PREF));
		...
		CPRINTS("HPD source: %s",
			(alt_dp_config & ALT_DP_MOD_HPD) ? "modified"
							 : "external");
