...
static void board_manage_port(int port, int enable)
{
	int rp;
	 ...
	 */
	if (port == CHG) {
		/* CHG port has connected or disconnected, so force a DUT port
		 *  disconnect so that the DUT port can be reconnected with the
		 *  correct VBUS supply. If the DUT port is not supplying VBUS,
		 *  then there is nothing to be done.
		 */
		if (vbus_en[DUT])
			tcpm_select_rp_value(DUT, TYPEC_RP_RESERVED);
		return;
	}
