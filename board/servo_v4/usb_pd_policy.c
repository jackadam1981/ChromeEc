...
static int cmd_usbc_action(int argc, char *argv[])
{
	...
	/* TODO(b:140256624): drop *v command if we migrate to limit_v cmd. */
	...
	} else if (!strcasecmp(argv[1], "limit_v")) {
		int sink_v;
...
DECLARE_CONSOLE_COMMAND(usbc_action, cmd_usbc_action,
			"5v|12v|20v|dev|pol0|pol1|drp|limit_v x(x=voltage)|dev",
