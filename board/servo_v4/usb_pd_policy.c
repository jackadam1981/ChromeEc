...
static void dut_allow_charge(void)
{
	...
	if (charge_port_is_active() && allow_src_mode &&
	    pd_get_dual_role_port(DUT) != PD_DRP_FORCE_SOURCE) {
		CPRINTS("Enable DUT charge through");
		pd_set_dual_role_port(DUT, PD_DRP_FORCE_SOURCE);
		/* pd_config_init(port, enable_src=1). */
...
static void board_manage_dut_port(void)
{
	...
	current_role = pd_get_dual_role_port(DUT);
	if (current_role != allowed_role) {
		...
		if (allowed_role == PD_DRP_FORCE_SINK) {
			...
			/* Mark as SNK only. */
			pd_set_dual_role_port(DUT, PD_DRP_FORCE_SINK);

			/* Reset PD. pd_config_init(port, enable_src=0). */
			pd_config_init(DUT, 0);
		} else {
			/* Allow chargethrough after PD negotiate. */
...
static int command_cc(int argc, char **argv)
{
	...
	if (!strcasecmp(argv[1], "snk")) {
		disable_dts_new = 1;
		allow_src_new = 0;
	} else
	if (!strcasecmp(argv[1], "srcdts")) {
		disable_dts_new = 0;
		allow_src_new = 1;
	} else
	if (!strcasecmp(argv[1], "snkdts")) {
		disable_dts_new = 0;
		allow_src_new = 0;
	} else {
