 ...
 * multiple entries so that can offer greater than 5V charging. The 1st entry
 * will be fixed 5V, but its current value may change based on the CHG port
 * vbus info. Subsequent entries are used for when offering vbus greater
 * than 5V.
 */
static const uint16_t pd_src_voltages_mv[] = { 5000, 9000, 12000, 20000 };
...
static void update_ports(void)
{
	...
	if (!charge_port_is_active()) {
		...
		/* Advertise the 'best' PDOs at various discreet voltages */
