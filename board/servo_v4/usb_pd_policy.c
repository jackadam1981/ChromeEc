...
static void board_manage_chg_port(void)
{
	...
	vbus_volt_mv[CHG] = vbus_mv_pend;
	vbus_curr_ma[CHG] = vbus_ma_pend;
	/* Set VBUS status for CHG port */
	vbus_en[CHG] = vbus_mv_pend >= 5000 ? 1 : 0;

	ccprintf("defer: current: %d mV, new: %d mV\n",
		 vbus_volt_mv[CHG], vbus_mv_pend);
	/*
	...
	if (!vbus_en[CHG]) {
		...
		pd_src_chg_pdo[chg_pdo_idx ^ 1] = PDO_FIXED_VOLT(vbus_mv_pend) |
				PDO_FIXED_CURR(vbus_ma_pend) | PDO_FIXED_FLAGS;
...
static void board_notify_chg_port(int port, int max_ma, int vbus_mv)
{
...
{
	board_notify_chg_port(port, max_ma, supply_voltage);
}
