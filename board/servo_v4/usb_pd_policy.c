...
static void board_manage_chg_port(void)
{
	...
	if (!vbus[CHG].mv) {
		...
		int vsafe5_ma = vbus[CHG].mv > 5000 ? 500 : vbus[CHG].ma;
...
static void board_update_vbus(int port, int max_ma, int vbus_mv)
{
	if (port == DUT) {
		vbus[DUT].mv = vbus_mv;
		vbus[DUT].ma = max_ma;
...
{
	board_update_vbus(port, max_ma, supply_voltage);
}
