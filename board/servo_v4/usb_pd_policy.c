...
static int is_dp_enabled(void)
{
	int tmp;

	i2c_read8(1, 0x20, 0x0, &tmp);
	return tmp & 0x40 ? 1 : 0;
}

static void enable_dp(void)
{
	/*
	 * Write USB3 Mode to PS8742 USB/DP Mux.
	 * 0x0:disable, 0x20:USB3, 0x40:DP
	 */
	i2c_write8(1, 0x20, 0x0, 0x40);
}

static void disable_dp(void)
{
...
static int cmd_usbc_action(int argc, char *argv[])
{
	...
	if (!strcasecmp(argv[1], "5v")) {
		...
		do_cc(cc_config | CC_POLARITY);
	} else if (!strcasecmp(argv[1], "hpd0")) {
		fake_hpd = 0;
		ccprintf("hpd0\n");
		pd_send_hpd(1, hpd_low);
	} else if (!strcasecmp(argv[1], "hpd1")) {
		fake_hpd = 1;
		ccprintf("hpd1\n");
		pd_send_hpd(1, hpd_high);
	} else if (!strcasecmp(argv[1], "drp")) {
