...
static inline int pd_set_rp_mode(int port, int cc_pull, int rp_value)
{
	...
	if (cc_pull == TYPEC_CC_RP) {
		...
		switch (rp_value) {
		case TYPEC_RP_USB:
			gpio_set_flags(GPIO_USB_DUT_CC1_RP3A0, GPIO_OUTPUT);
			gpio_set_flags(GPIO_USB_DUT_CC2_RP1A5, GPIO_OUTPUT);
			gpio_set_level(GPIO_USB_DUT_CC1_RP3A0, 1);
...
static inline void pd_set_host_mode(int port, int enable)
{
	...
	if (enable) {
		 ...
		 * VBUS is enabled.
		 */
		pd_set_rp_mode(port, TYPEC_CC_RP, TYPEC_RP_USB);

		gpio_set_flags(GPIO_USB_DUT_CC1_TX_DATA, GPIO_INPUT);
		gpio_set_flags(GPIO_USB_DUT_CC2_TX_DATA, GPIO_INPUT);
	} else {
		/* Select Rd, the Rp value is a don't care */
		pd_set_rp_mode(port, TYPEC_CC_RD, TYPEC_RP_RESERVED);
	}
}
