...
{
	cprintf(CC_USBPD, "pd_tx_enable port=%d polarity=%d\n", port,polarity);
	if (port == 0) {
		/* put SPI function on TX pin */
		if (polarity) {
			const struct gpio_info *g = gpio_list + GPIO_USB_CHG_CC2_TX_DATA;
			gpio_set_alternate_function(g->port, g->mask, 0);

			/* set the low level reference */
			gpio_set_flags(GPIO_USB_CHG_CC2_PD, GPIO_OUT_LOW);
		} else {
			const struct gpio_info *g = gpio_list + GPIO_USB_CHG_CC1_TX_DATA;
			gpio_set_alternate_function(g->port, g->mask, 0);
		...
		if (polarity) {
			const struct gpio_info *g = gpio_list + GPIO_USB_DUT_CC2_TX_DATA;
			...
			const struct gpio_info *g = gpio_list + GPIO_USB_DUT_CC1_TX_DATA;
			gpio_set_alternate_function(g->port, g->mask, 0);
...
{
	cprintf(CC_USBPD, "pd_tx_disable port=%d polarity=%d\n", port,polarity);
	if (port == 0) {
		if (polarity) {
...
static inline void pd_select_polarity(int port, int polarity)
{
	...
	cprintf(CC_USBPD, "pd_select_polarity port=%d polarity=%d\n", port, polarity);
...
{
	cprintf(CC_USBPD, "pd_set_host_mode port=%d enable=%d\n", port, enable);
	if (port == 0) {
