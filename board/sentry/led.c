...
static int set_battery_color(enum led_color color)
{
	switch (color) {
	case LED_OFF:
		gpio_set_level(GPIO_CHARGE_LED1, BAT_LED_OFF);
		gpio_set_level(GPIO_CHARGE_LED2, BAT_LED_OFF);
		break;
	case LED_GREEN:
		gpio_set_level(GPIO_CHARGE_LED1, BAT_LED_ON);
		gpio_set_level(GPIO_CHARGE_LED2, BAT_LED_OFF);
		break;
	case LED_AMBER:
		gpio_set_level(GPIO_CHARGE_LED1, BAT_LED_OFF);
		gpio_set_level(GPIO_CHARGE_LED2, BAT_LED_ON);
		break;
	default:
...
static void sentry_led_set_power(void)
{
...
DECLARE_HOOK(HOOK_INIT, led_init, HOOK_PRIO_DEFAULT+1);
