...
struct motion_sensor_t motion_sensors[] = {
	{.name = "Base Accel",
	 ...
	 .min_frequency = MOTIONSENSE_CHIP_KXCJ9_ACCEL_MIN_FREQ,
	 .max_frequency = MOTIONSENSE_CHIP_KXCJ9_ACCEL_MAX_FREQ,
	...
	{.name = "Lid Gyro",
	 ...
	 .min_frequency = MOTIONSENSE_CHIP_L3GD20H_GYRO_MIN_FREQ,
	 .max_frequency = MOTIONSENSE_CHIP_L3GD20H_GYRO_MAX_FREQ,
	 .fifo_max_event_count =
...
*/
static void set_kso_pins(int status, uint32_t flag)
{
	if (status) {
 ...
 * scanning is enabled.
*/
static void disable_kso_pu(void)
{
	set_kso_pins(lid_is_open(), GPIO_ODR_HIGH);
}
DECLARE_HOOK(HOOK_LID_CHANGE, disable_kso_pu, HOOK_PRIO_FIRST);

/*
 * Using hook prio last to ensure this gets called after keyboard
 * scanning is disabled.
*/
static void enable_kso_pu(void)
{
	set_kso_pins(!lid_is_open(), GPIO_ODR_HIGH | GPIO_PULL_UP);
}
DECLARE_HOOK(HOOK_LID_CHANGE, enable_kso_pu, HOOK_PRIO_LAST);
...
#define FAN_STEPS 5
static const int fan_table[FAN_STEPS] = {
	0, 3300, 3500, 3900, 4500,
};
...
int fan_percent_to_rpm(int fan, int pct)
{
	...
	if (pct < 13)
		new_rpm_target = fan_table[0];
	else if (pct < 19)
		new_rpm_target = (current_rpm_target > fan_table[0]) ?
				fan_table[1] : fan_table[0];
	else if (pct < 21)
		new_rpm_target = fan_table[1];
	else if (pct < 25)
		new_rpm_target = (current_rpm_target > fan_table[1]) ?
				fan_table[2] : fan_table[1];
	else if (pct < 28)
		new_rpm_target = (current_rpm_target > fan_table[1]) ?
				fan_table[3] : fan_table[1];
	else if (pct < 31)
		new_rpm_target = (current_rpm_target > fan_table[2]) ?
				fan_table[3] : fan_table[2];
	else if (pct < 44)
		new_rpm_target = fan_table[3];
	else if (pct < 50)
		new_rpm_target = (current_rpm_target > fan_table[3]) ?
				fan_table[4] : fan_table[3];
	else
		new_rpm_target = fan_table[4];
