...
int fan_table_to_rpm(int fan, int *temp)
{
	...
	CPRINTS("temp: %d, prev_tmp: %d", temp[TEMP_SENSOR_1_CHARGER],
		prev_tmp[TEMP_SENSOR_1_CHARGER]);
	if (temp[TEMP_SENSOR_1_CHARGER] < prev_tmp[TEMP_SENSOR_1_CHARGER]) {
		for (i = current_level; i > 0; i--) {
			if (temp[TEMP_SENSOR_1_CHARGER] <
				fan_step_table[i].off[TEMP_SENSOR_1_CHARGER])
				current_level = i - 1;
			else
				break;
		}
	} else if (temp[TEMP_SENSOR_1_CHARGER] >
		prev_tmp[TEMP_SENSOR_1_CHARGER]) {
		for (i = current_level; i < NUM_FAN_LEVELS; i++) {
			if (temp[TEMP_SENSOR_1_CHARGER] >
				fan_step_table[i].on[TEMP_SENSOR_1_CHARGER])
				current_level = i + 1;
			else
				break;
		}
