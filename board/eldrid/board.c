...
//static void host_command_get_chg_info(void)
{
	...
	//CPRINTS("[SC] batt_new.state_of_charge=%d", batt_new.state_of_charge);
	...
	isl9241_read(0, ISL9241_REG_CHG_CURRENT_LIMIT, &current);
	r1->ChargingCurrent = current;

	isl9241_read(0, ISL9241_REG_MAX_SYSTEM_VOLTAGE, &voltage);
	...
	//sb_read_string(SB_DEVICE_NAME, dest, size);
	//r1->device_name = dest;
...
//DECLARE_HOOK(HOOK_SECOND, host_command_get_chg_info, HOOK_PRIO_DEFAULT);
