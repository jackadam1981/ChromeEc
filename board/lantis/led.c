...
static void led_set_battery(void)
{
	...
	if (get_cbi_fw_config_tablet_mode() == TABLET_MODE_ABSENT) {
		if (chipset_in_state(CHIPSET_STATE_ANY_SUSPEND) &&
		    led_pwr_get_state() != LED_PWR_STATE_CHARGE) {
			power_ticks++;
	...
	case LED_PWR_STATE_CHARGE:
		set_active_port_color(LED_AMBER);
		break;
	case LED_PWR_STATE_DISCHARGE_FULL:
		if (extpower_is_present()) {
			set_active_port_color(LED_WHITE);
			break;
		}
		__fallthrough;
	case LED_PWR_STATE_DISCHARGE:
		/*
	...
	case LED_PWR_STATE_ERROR:
		...
		break;
	case LED_PWR_STATE_CHARGE_NEAR_FULL:
		set_active_port_color(LED_WHITE);
		break;
	case LED_PWR_STATE_IDLE: /* External power connected in IDLE */
		set_active_port_color(LED_WHITE);
		break;
	case LED_PWR_STATE_FORCED_IDLE:
