...
/* Time in tenths of a millisecond to timer clock ticks */
...
#if CEC_CIRCBUF_SIZE - 1 < MAX_CEC_MSG_LEN
...
#define START_BIT_MIN_HIGH	APB1_TICKS(600)
#define START_BIT_MAX_HIGH	APB1_TICKS(1000)
...
#define DATA_ZERO_MIN_HIGH	APB1_TICKS(750)
#define DATA_ZERO_MAX_HIGH	APB1_TICKS(1050)
...
#define DATA_ONE_MIN_HIGH	APB1_TICKS(1650)
#define DATA_ONE_MAX_HIGH	APB1_TICKS(1950)

#define DATA_BIT_MAX_LOW (MAX(DATA_ZERO_MAX_LOW, DATA_ONE_MAX_LOW))
#define DATA_BIT_MAX_HIGH (MAX(DATA_ZERO_MAX_HIGH, DATA_ONE_MAX_HIGH))
...
#define VALID_HIGH(bit, t) VALID_TIME(HIGH, bit, t)
...
static void tmr2_start(int tmo)
{
	int mdl = NPCX_MFT_MODULE_1;
...
static bool msgt_get_bit(const struct cec_msg_transfer *msgt)
{
	return msgt->buf[msgt->byte] & (1 << (7 - msgt->bit));
}

static void msgt_set_bit(struct cec_msg_transfer *msgt, bool val)
{
	uint8_t bit_flag;

	bit_flag = 1 << (7 - msgt->bit);
...
static bool msgt_is_eom(const struct cec_msg_transfer *msgt, int len)
{
	if (msgt->bit)
		return false;
...
{
	int i;
	uint32_t msg_offset, next_offset, read_offset;

	/* TODO(sadolfson): Use a mutex, we run from task context! */
	read_offset = cb->read_offset;
	msg_offset = cb->write_offset;
	for (i = 0 ; i < msg_len + 1; i++) {
		next_offset = (msg_offset + 1) % CEC_CIRCBUF_SIZE;
		if (next_offset == read_offset) {
			CPRINTF("Push to full buffer\n");
			return -1;
		}

		/*
		 * First byte is length, set that zero until we
		 * know the message fit
		 */
		cb->buf[msg_offset] = (i == 0) ? 0 : msg[i-1];
		msg_offset = next_offset;
	}

	/* Commit the push */
	cb->buf[cb->write_offset] = msg_len;
	cb->write_offset = msg_offset;
...
void enter_state(enum cec_state new_state)
{
	...
	case CEC_STATE_INITIATOR_EOM_LOW:
		gpio = 0;
		tmo = DATA_LOW(msgt_is_eom(&cec_tx.msgt, cec_tx.len));
		break;
	case CEC_STATE_INITIATOR_EOM_HIGH:
		gpio = 1;
		tmo = DATA_HIGH(msgt_is_eom(&cec_tx.msgt, cec_tx.len));
		break;
	...
	case CEC_STATE_FOLLOWER_ACK_LOW:
		...
		if (addr == cec_addr) {
			...
			/* Don't ack broadcast, but continue reading */
			/* TODO(sadolfsson): Check for broadcast nacks */
			tmo = DATA_ZERO_LOW;
		} else {
			/* Destination is another CEC device, stop parsing */
...
static void cec_ev_cap(void)
{
	...
	case CEC_STATE_FOLLOWER_START_LOW:
		...
		break;
	case CEC_STATE_FOLLOWER_START_HIGH:
		if (VALID_HIGH(START_BIT, tmr_cap_get()))
	...
	case CEC_STATE_FOLLOWER_HEADER_INIT_HIGH:
		...
		if ((data && VALID_HIGH(DATA_ONE, t)) ||
		    (!data && VALID_HIGH(DATA_ZERO, t))) {
...
static int hc_cec_write(struct host_cmd_handler_args *args)
{
	...
	if (cec_send(params->msg, params->msg_len) != 0)
		return EC_RES_BUSY;
...
static int hc_cec_read(struct host_cmd_handler_args *args)
{
	...
	if (cec_recv(response->msg, &response->msg_len) != 0)
...
static int hc_cec_addr(struct host_cmd_handler_args *args)
{
	const struct ec_params_cec_addr *params = args->params;

	if (params->logical_addr >= CEC_BROADCAST_ADDR)
		return EC_RES_INVALID_PARAM;

	cec_addr = params->logical_addr;
	CPRINTF("CEC address set to: %u\n", cec_addr);

	return EC_RES_SUCCESS;
}
DECLARE_HOST_COMMAND(EC_CMD_CEC_SET_LOGICAL_ADDR, hc_cec_addr, EC_VER_MASK(0));
...
static void cec_init(void)
{
	...
	/* Capture falling edge of first start bit to get things going */
