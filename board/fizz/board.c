...
#define BOARD_INFO_VERSION_MAJOR 0
static const uint8_t board_info_magic[] = { 0x42, 0x44, 0x49 };
...
static int init_board_info(void)
{
	...
	/* Read header */
	offset = 0;
	if (i2c_xfer(NPCX_I2C_PORT0_1, I2C_ADDR_EEPROM,
		     &offset, 1, buf, sizeof(bi.head), I2C_XFER_SINGLE)) {
		CPRINTS("Failed to read BI header");
		return EC_ERROR_INVAL;
	}
	memcpy(&bi.head, buf, sizeof(bi.head));

	/* Check magic */
	if (memcmp(bi.head.magic, board_info_magic, sizeof(bi.head.magic))) {
		CPRINTS("Invalid BI magic");
		return EC_ERROR_INVAL;
	}

	/* check version */
	if (bi.head.major > BOARD_INFO_VERSION_MAJOR) {
		CPRINTS("BI major version too high");
		return EC_ERROR_INVAL;
	}
	...
	if (i2c_xfer(NPCX_I2C_PORT0_1, I2C_ADDR_EEPROM, &offset, 1,
		     ...
		     I2C_XFER_SINGLE)) {
		CPRINTS("Failed to read BI body");
		return EC_ERROR_INVAL;
	}

	/* Check CRC */
	if (crc8(buf + 4, bi.head.total_size - 4) != bi.head.crc) {
		CPRINTS("BI CRC mismatch");
		return EC_ERROR_INVAL;
	}
...
/*
 * It can be named as system_get_sku_id() to plum it with the existing host
 * command.
 */
