...
const struct temp_sensor_t temp_sensors[] = {
	{"TMP432_Internal", TEMP_SENSOR_TYPE_CPU,
			tmp432_get_val, TMP432_IDX_LOCAL, 4},
};
...
static int board_get_oem_id(void)
{
...
{
	uint32_t version;
	uint32_t oem_id = board_get_oem_id();
...
BUILD_ASSERT(ARRAY_SIZE(fan_tables) == OEM_COUNT);
...
{
	return get_custom_rpm(fan, pct, board_get_oem_id());
}
