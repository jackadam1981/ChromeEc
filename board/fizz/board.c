			    ...
			    int max_ma, int charge_mv)
{
	 ...
	 *
	 * The system power consumption is capped by PR259, which is staffed
	...
	if (supplier == CHARGE_SUPPLIER_DEDICATED) {
		gpio_set_level(GPIO_TYPE_C_87W, 0);
		gpio_set_level(GPIO_TYPE_C_65W, 0);
		gpio_set_level(GPIO_TYPE_C_60W, 0);
	} else if (charge_ma >= 4300) { /* Apple 87W charger offers 4.3A @20V */
		gpio_set_level(GPIO_TYPE_C_87W, 1);
		gpio_set_level(GPIO_TYPE_C_65W, 0);
		gpio_set_level(GPIO_TYPE_C_60W, 0);
	} else if (charge_ma >= 3250) {
		gpio_set_level(GPIO_TYPE_C_87W, 0);
		gpio_set_level(GPIO_TYPE_C_65W, 1);
		gpio_set_level(GPIO_TYPE_C_60W, 0);
	} else if (charge_ma >= 3000) {
		gpio_set_level(GPIO_TYPE_C_87W, 0);
		gpio_set_level(GPIO_TYPE_C_65W, 0);
		gpio_set_level(GPIO_TYPE_C_60W, 1);
	} else {
		/* TODO(http://crosbug.com/p/65013352) */
		CPRINTS("Current %dmA not supported", charge_ma);
	}
