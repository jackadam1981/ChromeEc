...
int board_set_active_charge_port(int charge_port)
{
	...
	const int is_real_port = (charge_port >= 0 &&
			charge_port < CHARGE_PORT_COUNT);
	const int active_port = charge_manager_get_active_charge_port();
	int barreljack_enable;

	/* No switching charge port after boot */
	if (is_real_port && active_port != CHARGE_PORT_NONE &&
			charge_port != active_port) {
		CPRINTF("Switching charge port not allowed: p%d", charge_port);
		return EC_ERROR_INVAL;
	}

	/* Don't charge from a source port. This guarantees type-c output
	 * (GPIO_USB_C0_5V_EN) is disabled. */
	if (charge_manager_is_valid_pd_port(charge_port) &&
			board_vbus_source_enabled(charge_port))
		return EC_ERROR_INVAL;
	...
	/* Enable/Disable type-c input */
	gpio_set_level(GPIO_USB_C0_CHARGE_L, barreljack_enable);
	/* Enable/Disable barrel jack input */
	gpio_set_level(GPIO_AC_JACK_CHARGE_L, !barreljack_enable);
