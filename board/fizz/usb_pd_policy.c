...
/* Indicates vbus is currently providing current to an external device. */
static uint8_t vbus_source_en[CHARGE_PORT_COUNT];

int board_vbus_source_enabled(int port)
{
	return vbus_source_en[port];
}

static void board_vbus_source_enable(int enable)
{
	/* Port = 0 is assumed because the baord has only one type-c port.
	 * Barrel jack port stays disabled. */
	gpio_set_level(GPIO_USB_C0_5V_EN, enable);
	vbus_source_en[CHARGE_PORT_TYPEC0] = enable;
}
				...
				uint32_t supply_voltage)
{
	...
	CPRINTS("%s: p%d %dma %dmv", __func__, port, max_ma, supply_voltage);
