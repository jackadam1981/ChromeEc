...
static void configure_tick(uint32_t interval, enum led_color color)
{
	led_pulse.interval = interval;
	led_pulse.duty_inc = 100 / (LED_PULSE_US / interval);
...
static void led_suspend(void)
{
	...
	configure_tick(LED_PULSE_TICK_US, LED_AMBER);
	led_tick();
}
...
void led_alert(void)
{
	/* This can race with led_tick but it's ok. We'll resume tick anyway */
	hook_call_deferred(&led_tick_data, -1);
	configure_tick(LED_PULSE_US, LED_RED);
	led_tick();
}
