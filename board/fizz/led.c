...
static int led_set_color_power(enum led_color color)
{
	switch (color) {
	case LED_OFF:
		gpio_set_level(GPIO_PWR_GRN_LED, 0);
		gpio_set_level(GPIO_PWR_RED_LED, 0);
		break;
	case LED_GREEN:
		gpio_set_level(GPIO_PWR_GRN_LED, 1);
		gpio_set_level(GPIO_PWR_RED_LED, 0);
		break;
	case LED_RED:
		gpio_set_level(GPIO_PWR_GRN_LED, 0);
		gpio_set_level(GPIO_PWR_RED_LED, 1);
		break;
	case LED_AMBER:
		gpio_set_level(GPIO_PWR_GRN_LED, 1);
		gpio_set_level(GPIO_PWR_RED_LED, 1);
