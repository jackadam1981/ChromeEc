...
/* led task gradually increases brightness every <task_frequency_us> to go from
 * 0% to 100% in <LED_PULSE_US>. Then it decreases brightness likewise in
 * <LED_PULSE_US>. So, one cycle is <LED_PULSE_US> x 2. */
#define LED_PULSE_US	(2 * SECOND)
static uint32_t task_frequency_us;
static int duty_inc;

static void set_led_task_frequency(uint32_t usec)
{
...
static void led_set_power(void)
{
	...
	if (chipset_in_state(CHIPSET_STATE_ON))
		set_color(EC_LED_ID_POWER_LED, LED_GREEN, duty);
...
static void led_init(void)
{
	...
	set_led_task_frequency(80 * MSEC);
...
static int command_led(int argc, char **argv)
{
	...
	if (!strcasecmp(argv[1], "debug")) {
		...
		set_led_task_frequency(msec * MSEC);
	}
	return EC_SUCCESS;
}
