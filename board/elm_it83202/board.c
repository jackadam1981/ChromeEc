...
#else
	[ADC_PSYS]      = {"PSYS",      34492, 1024, 0, CHIP_ADC_CH0},
	[ADC_AMON_BMON] = {"AMON_BMON", 16667, 1024, 0, CHIP_ADC_CH1},
	[ADC_VBUS]      = {"VBUS",      3000, 1024, 0, CHIP_ADC_CH2},
#endif
};
...
const enum gpio_signal hibernate_wake_pins[] = {
	GPIO_POWER_BUTTON_L, GPIO_LID_OPEN, GPIO_AC_PRESENT
};
...
const struct spi_device_t spi_devices[] = {
	{ CONFIG_SPI_ACCEL_PORT,    0, -1 }, /* mainboard */
	{ CONFIG_SPI_ACCEL_PORT + 1, 0, -1 } /* daughterboard */
};
...
static void tmp432_set_power_deferred(void)
{
	...
	if (!extpower_is_present() && !chipset_in_state(CHIPSET_STATE_ON)) {
		if (tmp432_set_power(TMP432_POWER_OFF) != EC_SUCCESS)
			CPRINTS("ERROR: Can't shutdown TMP432.");
		return;
	}

	/*  else, turn it on. */
	if (tmp432_set_power(TMP432_POWER_ON) != EC_SUCCESS)
...
static void board_chipset_resume(void)
{
	...
	hook_call_deferred(&tmp432_set_power_deferred_data, 0);
#endif
	/* TODO: workaround of KSO2 */
	IT83XX_KBS_KSOLGCTRL = 4;
	IT83XX_KBS_KSOLGOEN = 4;
}
...
{
	/* TODO: workaround of KSO2 */
	IT83XX_KBS_KSOLGCTRL = 0;
	IT83XX_KBS_KSOLGOEN = 0;
#ifdef CONFIG_TEMP_SENSOR_TMP432
	hook_call_deferred(&tmp432_set_power_deferred_data, 0);
#endif
}
...
#if 1
static enum ec_status keyboard_get_scan(struct host_cmd_handler_args *args)
{
#if 0
	kb_fifo_remove(args->response);
#endif
	memset(args->response, 0, 13);
...
#endif
	args->response_size = 13; /* KEYBOARD_COLS; */
