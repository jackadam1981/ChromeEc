...
/*
 * cmp_strs(char *str_1, char *str_2): Compare 2 strings.
 *
 * The numbers of chars to compare are determined by the size of str_1.
 *
 * @return	0:	equal
 *		!0:	not equal
 */

static int cmp_strs(char *str_1, char *str_2)
{
	return memcmp(str_1, str_2, sizeof(str_1));
}
...
const struct battery_info *battery_get_info(void)
{
	...
	char device[9];
	int design_mv;
	struct battery_info *info_src = NULL;
	...
	for (i = 0; i < ARRAY_SIZE(support_batteries); ++i) {
		if ((cmp_strs(support_batteries[i].manuf, manuf) == 0) &&
		    (cmp_strs(support_batteries[i].device, device) == 0) &&
			...
			info_src = support_batteries[i].battery_info;
