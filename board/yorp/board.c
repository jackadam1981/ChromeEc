...
/* Sets the gpio flags only if this is the first time since power-on-reset */
static void set_gpio_flags_if_por(enum gpio_signal signal, int flags)
{
...
static void update_drivers_from_board_id(void)
{
	...
	if (board_id == 0) {
		...
		set_gpio_flags_if_por(GPIO_WFCAM_VSYNC, GPIO_OUT_LOW);
		set_gpio_flags_if_por(GPIO_C1_EN_SNK_V0, GPIO_OUT_LOW);
		...
		set_gpio_flags_if_por(GPIO_SYS_RESET_L_V0, GPIO_ODR_HIGH);
		gpio_sys_reset_l_runtime = GPIO_SYS_RESET_L_V0;     /* GPIO02 */

		set_gpio_flags_if_por(GPIO_ENTERING_RW_V0, GPIO_OUT_LOW);
		gpio_entering_rw_runtime = GPIO_ENTERING_RW_V0;     /* GPIO80 */

		set_gpio_flags_if_por(GPIO_USB_A1_CHARGE_EN_L, GPIO_OUT_LOW);
