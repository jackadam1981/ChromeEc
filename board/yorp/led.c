...
static const struct led_descriptor led_state_table[][LED_NUM_PHASES] = {
	/* STATE_CHARGING */
	{ {LED_COLOR_ORANGE, LED_INDEFINITE}, {LED_OFF, LED_INDEFINITE} },
	/* STATE_CHARGING_FULLY_CHARGED */
	{ {LED_COLOR_BLUE, LED_INDEFINITE}, {LED_OFF, LED_INDEFINITE} },
	/* STATE_DISCHARGE_S0 */
	{ {LED_COLOR_BLUE, LED_INDEFINITE}, {LED_OFF, LED_INDEFINITE} },
	/* STATE_DISCHARGE_S3 */
	{ {LED_COLOR_ORANGE, 4 * LED_ONE_SEC},
					{LED_OFF, 1 * LED_ONE_SEC} },
	/* STATE_DISCHARGE_S5 */
	{ {LED_OFF, LED_INDEFINITE}, {LED_OFF, LED_INDEFINITE} },
	/* STATE_BATTERY_ERROR */
	{ {LED_COLOR_BLUE, 2 * LED_ONE_SEC},
				{LED_COLOR_ORANGE, 2 * LED_ONE_SEC} },
};
...
int led_set_brightness(enum ec_led_id led_id, const uint8_t *brightness)
{
	if (brightness[EC_LED_COLOR_BLUE] != 0)
		...
		led_set_color_battery(LED_COLOR_ORANGE);
	else
...
static void led_update_battery(void)
{
	...
	if (desired_state < LED_NUM_STATES && desired_state != led_state) {
		/* State is changing */
		led_state = desired_state;
		/* Reset ticks counter when state changes */
		ticks = 0;
	} else if (desired_state == LED_NUM_STATES) /* No change needed */
		return;
	 ...
	 */
	phase = 0;
	if ((led_state_table[led_state][LED_PHASE_0].time != LED_INDEFINITE) &&
	    (led_state_table[led_state][LED_PHASE_1].time != LED_INDEFINITE)) {
		int period;
		...
		if (period)
			phase = ticks % period <
				led_state_table[led_state][LED_PHASE_0].time ?
				0 : 1;
	}
