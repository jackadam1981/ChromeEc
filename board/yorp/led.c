...
{
	static uint8_t ticks, period = UINT8_MAX;
	...
	/*
	 * Determine which phase of the state table to use. Assume it's
	 * phase 0. If the time values for first phase of the current state is
	...
	if (led_state_table[led_state][LED_PHASE_0].time != LED_INDEFINITE) {
		phase = ticks < led_state_table[led_state][LED_PHASE_0].time ?
				0 : 1;
