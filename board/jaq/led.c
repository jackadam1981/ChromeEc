...
static void jaq_led_set_battery_poweron(void)
{
...
static void jaq_led_set_battery_poweroff(void)
{
	switch (charge_get_state()) {
	case PWR_STATE_CHARGE:
		bat_led_set_color(LED_AMBER);
...
static void jaq_led_set_battery_suspend(void)
{
	switch (charge_get_state()) {
	case PWR_STATE_CHARGE:
		bat_led_set_color((battery_second & 0x1)
...
static void jaq_led_set_battery(void)
{
	battery_second++;

	if (chipset_in_state(CHIPSET_STATE_ON))
		jaq_led_set_battery_poweron();
	else if (chipset_in_state(CHIPSET_STATE_ANY_OFF))
		jaq_led_set_battery_poweroff();
	else if (chipset_in_state(CHIPSET_STATE_SUSPEND))
		jaq_led_set_battery_suspend();
}
