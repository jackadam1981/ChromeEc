...
#define CHARGER_TIMEOUT_HOURS_SETZER 10

#include "gpio_list.h"

static int prev_stats;
static uint64_t charge_timeout;
...
void check_charger_timeout_second(void){
	if (charge_get_state() != prev_stats) {
		if (charge_get_state() == PWR_STATE_CHARGE)
			charge_timeout = get_time().val +
				CHARGER_TIMEOUT_HOURS_SETZER * HOUR;
		prev_stats = charge_get_state();
	}
	if (charge_get_state() == PWR_STATE_CHARGE &&
		get_time().val > charge_timeout) {
		CPRINTS("Charge timed out after %d hours",
			CHARGER_TIMEOUT_HOURS_SETZER);
