...
static void lte_reset_startup(void)
{
	gpio_set_level(GPIO_EC_LTE_RST_L, 1);
}
DECLARE_DEFERRED(lte_reset_startup);

static void lte_function_startup(void)
{
	gpio_set_level(GPIO_LTE_FCPO, 1);
	hook_call_deferred(&lte_reset_startup_data, 20 * MSEC);
}
DECLARE_DEFERRED(lte_function_startup);

static void lte_power_startup(void)
{
	gpio_set_level(GPIO_LTE_EN, 1);
	gpio_set_level(GPIO_LTE_W_DISABLE_L, 1);
}
DECLARE_DEFERRED(lte_power_startup);

static void lte_power_shutdown(void)
{
	gpio_set_level(GPIO_LTE_EN, 0);
	gpio_set_level(GPIO_LTE_W_DISABLE_L, 0);
}
DECLARE_DEFERRED(lte_power_shutdown);

static void lte_function_shutdown(void)
{
	gpio_set_level(GPIO_LTE_FCPO, 0);
	hook_call_deferred(&lte_power_shutdown_data, 20 * MSEC);
}
DECLARE_DEFERRED(lte_function_shutdown);

static void lte_reset_shutdown(void)
{
	gpio_set_level(GPIO_EC_LTE_RST_L, 0);
}
DECLARE_DEFERRED(lte_reset_shutdown);

static void  wwan_lte_startup(void)
{
	/* Turn on WWAN LTE function as we go into S0 from S5. */
	hook_call_deferred(&lte_reset_shutdown_data, -1);
	hook_call_deferred(&lte_function_shutdown_data, -1);
	hook_call_deferred(&lte_power_shutdown_data, -1);
	lte_power_startup();
	hook_call_deferred(&lte_function_startup_data, 10 * MSEC);
}
DECLARE_HOOK(HOOK_CHIPSET_RESUME, wwan_lte_startup, HOOK_PRIO_DEFAULT);

static void wwan_lte_shutdown(void)
{
	/* Turn off WWAN LTE function as we go back to S5. */
	hook_call_deferred(&lte_power_startup_data, -1);
	hook_call_deferred(&lte_function_startup_data, -1);
	hook_call_deferred(&lte_reset_startup_data, -1);
	lte_reset_shutdown();
	hook_call_deferred(&lte_function_shutdown_data, 10 * MSEC);
}
DECLARE_HOOK(HOOK_CHIPSET_SUSPEND, wwan_lte_shutdown, HOOK_PRIO_DEFAULT);
