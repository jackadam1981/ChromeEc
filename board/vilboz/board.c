...
static void lte_reset_enable(void)
{
	gpio_set_level(GPIO_EC_LTE_RST_L, 1);
}
DECLARE_DEFERRED(lte_reset_enable);

static void lte_function_enable(void)
{
	gpio_set_level(GPIO_LTE_FCPO, 1);
	hook_call_deferred(&lte_reset_enable_data, 20 * MSEC);
}
DECLARE_DEFERRED(lte_function_enable);

static void lte_power_enable(void)
{
	gpio_set_level(GPIO_LTE_EN, 1);
	gpio_set_level(GPIO_LTE_W_DISABLE_L, 1);
}
DECLARE_DEFERRED(lte_power_enable);

static void lte_power_disable(void)
{
	gpio_set_level(GPIO_LTE_EN, 0);
	gpio_set_level(GPIO_LTE_W_DISABLE_L, 0);
}
DECLARE_DEFERRED(lte_power_disable);

static void lte_function_disable(void)
{
	gpio_set_level(GPIO_LTE_FCPO, 0);
	hook_call_deferred(&lte_power_disable_data, 20 * MSEC);
}
DECLARE_DEFERRED(lte_function_disable);

static void lte_reset_disable(void)
{
	gpio_set_level(GPIO_EC_LTE_RST_L, 0);
}
DECLARE_DEFERRED(lte_reset_disable);

static void  wwan_lte_startup(void)
{
	/* Turn on WWAN LTE function as we go into S0 from S5. */
	hook_call_deferred(&lte_reset_disable_data, -1);
	hook_call_deferred(&lte_function_disable_data, -1);
	hook_call_deferred(&lte_power_disable_data, -1);
	lte_power_enable();
	hook_call_deferred(&lte_function_enable_data, 10 * MSEC);
}
...
static void wwan_lte_shutdown(void)
{
	...
	hook_call_deferred(&lte_power_enable_data, -1);
	hook_call_deferred(&lte_function_enable_data, -1);
	hook_call_deferred(&lte_reset_enable_data, -1);
	lte_reset_disable();
	hook_call_deferred(&lte_function_disable_data, 10 * MSEC);
}
