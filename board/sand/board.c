...
/* Sand board-specific configuration */
...
void board_tcpc_init(void)
{
	...
	/*
	 * TODO: Remove when Sand is updated with PS8751 A3.
	 *
...
struct {
	enum sand_board_version version;
	int thresh_mv;
} const sand_board_versions[] = {
...
BUILD_ASSERT(ARRAY_SIZE(sand_board_versions) == BOARD_VERSION_COUNT);
...
int board_get_version(void)
{
	...
	for (i = 0; i < BOARD_VERSION_COUNT; i++) {
		if (mv < sand_board_versions[i].thresh_mv) {
			version = sand_board_versions[i].version;
			break;
		}
	}
