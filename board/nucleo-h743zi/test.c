...
static int command_write_rstdata(int argc, char **argv)
{
	...
	for (i = 0; i < ARRAY_SIZE(regions); i++) {
		ccprintf("# Region: %s\n", regions[i].name);
		pattern_write(regions[i].values, regions[i].count, pattern_index);
		ccprintf("\n");
		cflush();
	}
...
static int command_read_rstdata(int argc, char **argv)
{
	...
	for (i = 0; i < ARRAY_SIZE(regions); i++) {
		ccprintf("# Region: %s\n", regions[i].name);
		pattern_read(regions[i].values, regions[i].count, pattern_index);
		ccprintf("\n");
		cflush();
	}
...
static int command_read_ber_rstdata(int argc, char **argv)
{
	...
	for (i = 0; i < ARRAY_SIZE(regions); i++) {
		ccprintf("# Region: %s\n", regions[i].name);
		ber[i] = pattern_read_ber(regions[i].values, regions[i].count, pattern_index);
		ccprintf("\n");
		cflush();
	}
...
static int command_zero_rstdata(int argc, char **argv)
{
	...
	for (i = 0; i < ARRAY_SIZE(regions); i++) {
		ccprintf("# Region: %s\n", regions[i].name);
		pattern_write(regions[i].values, regions[i].count, pattern_zero);
		ccprintf("\n");
		cflush();
	}
...
static int command_zero_ber_rstdata(int argc, char **argv)
{
	...
	for (i = 0; i < ARRAY_SIZE(regions); i++) {
		ccprintf("# Region: %s\n", regions[i].name);
		ber[i] = pattern_read_ber(regions[i].values, regions[i].count, pattern_zero);
		ccprintf("\n");
		cflush();
	}
...
static int command_ones_rstdata(int argc, char **argv)
{
	...
	for (i = 0; i < ARRAY_SIZE(regions); i++) {
		ccprintf("# Region: %s\n", regions[i].name);
		pattern_write(regions[i].values, regions[i].count, pattern_ones);
		ccprintf("\n");
		cflush();
	}
...
static int command_ones_ber_rstdata(int argc, char **argv)
{
	...
	for (i = 0; i < ARRAY_SIZE(regions); i++) {
		ccprintf("# Region: %s\n", regions[i].name);
		ber[i] = pattern_read_ber(regions[i].values, regions[i].count, pattern_ones);
		ccprintf("\n");
		cflush();
	}
