...
static void board_init(void)
{
	...
	if (system_get_board_version() >= 2)
		gpio_enable_interrupt(GPIO_PEN_DETECT);
...
{
	int pen_charge_enable = !gpio_get_level(GPIO_PEN_DETECT) &&
			!chipset_in_state(CHIPSET_STATE_ANY_OFF);

	if (pen_charge_enable)
		gpio_set_level(GPIO_PEN_PWR_EN, 1);
	else
		gpio_set_level(GPIO_PEN_PWR_EN, 0);
