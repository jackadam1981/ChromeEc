...
struct temp_chg_step {
	int on;		/* temp thershold ('C) to higher level */
	int off;	/* temp thershold ('C) to lower level*/
	int current;	/* charging limitation (mA) */
};

static const struct temp_chg_step temp_chg_table[] = {
	{.on =  50, .off =  0, .current = 3000},	/* Lv0: normal charge */
	{.on =  53, .off =  48, .current = 1500},
	{.on =  56, .off =  51, .current = 1000},
	{.on =  100, .off =  54, .current = 800},
};
...
int charger_profile_override(struct charge_state_data *curr)
{
	...
	if (chipset_in_state(CHIPSET_STATE_ON)) {
		...
		if (charger_temp_c <= temp_chg_table[current_level].off)
			current_level--;
		else if (charger_temp_c >= temp_chg_table[current_level].on)
			current_level++;
