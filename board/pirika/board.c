...
void board_init(void)
{
	...
	if (get_cbi_ssfc_charger_type() == SSFC_CHARGER_SM5803) {
		...
		SM5803_ADDR_CHARGER_FLAGS;
		chg_chips[CHARGER_PRIMARY].drv = &sm5803_drv;
		chg_chips[CHARGER_SECONDARY].i2c_addr_flags =
		SM5803_ADDR_CHARGER_FLAGS;
		chg_chips[CHARGER_SECONDARY].drv = &sm5803_drv;

		tcpc_config[0].bus_type = EC_BUS_TYPE_I2C;
		tcpc_config[0].drv = &it83xx_tcpm_drv;
		tcpc_config[1].i2c_info.addr_flags =
		AN7447_TCPC0_I2C_ADDR_FLAGS;
		tcpc_config[1].drv = &anx7447_tcpm_drv;

		usb_muxes[1].i2c_addr_flags = AN7447_TCPC0_I2C_ADDR_FLAGS;
		usb_muxes[1].driver = &anx7447_usb_mux_driver;
	...
	if (get_cbi_ssfc_charger_type() == SSFC_CHARGER_SM5803) {
		/*
		 * Charger on the MB will be outputting PROCHOT_ODL
		 * and OD CHG_DET.
		 */
...
uint16_t tcpc_get_alert_status(void)
{
	...
	if (get_cbi_ssfc_charger_type() == SSFC_CHARGER_RAA489000_DEFAULT) {
		if (!gpio_get_level(GPIO_USB_C0_INT_ODL)) {
...
int board_set_active_charge_port(int port)
{
	...
	if (get_cbi_ssfc_charger_type() == SSFC_CHARGER_SM5803) {
		 ...
		 * Otherwise, we can overcurrent the adapter we're
		 * switching to. (crbug.com/926056)
		 */
