...
int fan_table_to_rpm(int fan, int *temp)
{
	 ...
	 */
	CPRINTS("temp: %d, prev_temp: %d", temp[TEMP_SENSOR_1_DDR_SOC],
		prev_temp[TEMP_SENSOR_1_DDR_SOC]);
	if (temp[TEMP_SENSOR_1_DDR_SOC] < prev_temp[TEMP_SENSOR_1_DDR_SOC]) {
		for (i = current_level; i > 0; i--) {
			if (temp[TEMP_SENSOR_1_DDR_SOC] <
				fan_table[i].off[TEMP_SENSOR_1_DDR_SOC])
				current_level = i - 1;
			else
				break;
		}
	} else if (temp[TEMP_SENSOR_1_DDR_SOC] >
		prev_temp[TEMP_SENSOR_1_DDR_SOC]) {
		for (i = current_level; i < num_fan_levels; i++) {
			if (temp[TEMP_SENSOR_1_DDR_SOC] >
				fan_table[i].on[TEMP_SENSOR_1_DDR_SOC])
				current_level = i + 1;
			else
				break;
		}
