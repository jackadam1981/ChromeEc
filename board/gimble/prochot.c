...
static int set_register_charge_option(void)
{
	...
	if (i2c_read16(I2C_PORT_CHARGER, BQ25710_SMBUS_ADDR1_FLAGS,
		...
		/* if AC only, disable IDPM, because it will cause charger keep asserting PROCHOT */
...
static void assert_prochot(void)
{
	...
	if (!extpower_is_present()) {
		if (!battery_hw_present()) {
			...
			batt_params.battery_continuous_discharge_mw =
				ABS(batt_params.battery_continuous_discharge_mw);
			if ((batt_params.battery_continuous_discharge_mw / 1000) > 
				...
				/ 1000) < BATT_MAX_CONTINUE_DISCHARGE_WATT*85/100)
				gpio_set_level(GPIO_EC_PROCHOT_ODL, 1);
		}
		return;
	}
	...
	if (adapter_wattage >= ADT_RATING_W) {
		...
		if (!battery_hw_present() || batt_params.state_of_charge <= 10) {
			...
			/* AC + battery */
			if (total_W > (ADT_RATING_W +BATT_MAX_CONTINUE_DISCHARGE_WATT))
				gpio_set_level(GPIO_EC_PROCHOT_ODL, 0);
			else if (total_W < (ADT_RATING_W +BATT_MAX_CONTINUE_DISCHARGE_WATT) * 95/100)
				gpio_set_level(GPIO_EC_PROCHOT_ODL, 1);
		}
	} else {
		/* if adapter < 60W */
