...
static int command_mp2964_dump(int argc, char **argv)
{
	...
	reg_val = rail_b_rev4;
	//ccprints("[SC] ARRAY_SIZE(rail_b_rev4)=%d", ARRAY_SIZE(rail_b_rev4));
	/* Dump TCPC registers. */
	for (i = 0; i < ARRAY_SIZE(rail_b_rev4); ++i, ++reg_val) {
		mp2964_read16(reg_val->reg, &outval);
		msleep(10);
		ccprints("0x%x = 0x%x", reg_val->reg, outval);
	}

	return EC_SUCCESS;
}
DECLARE_CONSOLE_COMMAND(mp2964_dump, command_mp2964_dump, "",
			"dump the mp2964 regs");
...
static int mp2964_patch(int argc, char **argv)
{
	...
	if (!strcasecmp(argv[1], "3")) {
		status = mp2964_tune(rail_a_rev3, ARRAY_SIZE(rail_a_rev3),
			     rail_b_rev3, ARRAY_SIZE(rail_b_rev3));
	} else if (!strcasecmp(argv[1], "4")) {
		mp2964_tune(rail_a_rev3, ARRAY_SIZE(rail_a_rev4),
			     rail_b_rev3, ARRAY_SIZE(rail_b_rev4));
	} else {
		ccprintf("ERROR: param1 has to be one of 3|4\n");
