...
void board_init(void)
{
	...
	for (port = 0; port < board_get_usb_pd_port_count(); port++) {
		/* set ac prochot */
		isl923x_set_ac_prochot(port, 1696);
		CPRINTF("C%d set ac prochot success", port);
...
#define SUPPORT_ADAPTER_POWER 35000
...
void board_input_current_setting(void)
{
	...
	if (charge_port < 0) {
		...
		CPRINTF("C%d--input_current=%d mA", charge_port, input_current);
