...
/* 8-bit address */
#define SN75DP130_I2C_ADDR__7b 0x2e
/*
...
{
	return i2c_write8__7b(I2C_PORT_MASTER, SN75DP130_I2C_ADDR__7b, index, value);
}
...
static int sn75dp130_reset(void)
{
	...
	rv = pca9534_config_pin__7b(I2C_PORT_MASTER, 0x20, REDRIVER_RST_PIN,
				PCA9534_OUTPUT);
	/* Assert (its active high) */
	rv |= pca9534_set_level__7b(I2C_PORT_MASTER, 0x20, REDRIVER_RST_PIN, 1);
	/* datasheet recommends > 100usec */
	usleep(200);

	/* De-assert */
	rv |= pca9534_set_level__7b(I2C_PORT_MASTER, 0x20, REDRIVER_RST_PIN, 0);
...
int board_in_hub_mode(void)
{
	...
	ret = pca9534_config_pin__7b(I2C_PORT_MASTER, 0x20, 6, PCA9534_INPUT);
	if (ret)
		return -1;
	ret = pca9534_get_level__7b(I2C_PORT_MASTER, 0x20, 6, &level);
	if (ret)
		return -1;
	return level;
}
...
static int board_usb_hub_reset(void)
{
	...
	ret = pca9534_config_pin__7b(I2C_PORT_MASTER, 0x20, 7, PCA9534_OUTPUT);
	if (ret)
		return ret;
	ret = pca9534_set_level__7b(I2C_PORT_MASTER, 0x20, 7, 0);
	if (ret)
		return ret;
	usleep(100 * MSEC);
	return pca9534_set_level__7b(I2C_PORT_MASTER, 0x20, 7, 1);
}
