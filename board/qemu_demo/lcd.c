...
void lcd_prints(char const *str, uint8_t line)
{
	int j;
	int k;
	int len;
...
void lcd_clear(uint8_t line)
{
	...
	i2c_write8(0, 0x54, 0x80, 0xab);

}

static void lcd_print(int words, char **str, uint8_t line)
{
	int j;
	int k;
	char *w;
	...
	for (j = 0; j < words; j++) {
		...
		lcd_prints(w, line);
		for (k = 0; k < CHAR_LEN; k++)
			i2c_write8(0, 0x3d, 0x40, codes[0][k]);
	}
	i2c_write8(0, 0x3d, 0x80, 0xe3);
}
...
static int command_lcd_print(int argc, char **argv)
{
	...
	lcd_print(argc - 2, &argv[2], line);

	return EC_SUCCESS;
}
DECLARE_CONSOLE_COMMAND(lcdprint, command_lcd_print, "", "Print to LCD");
