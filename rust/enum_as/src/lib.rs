...
///
/// ```
/// # #[macro_use] extern crate enum_as;
...
/// Generates an impl to_string for enum which retunrs &str.
...
pub fn enum_as(repr: TokenStream, input: TokenStream) -> TokenStream {
    ...
    for tt in enum_stream {
        ...
        state = match (tt, state) {
            ...
            (TokenTree::Literal(l), Val) => {
                let val = parse_to_u64(&l.to_string());
                let expected = end - 1;
