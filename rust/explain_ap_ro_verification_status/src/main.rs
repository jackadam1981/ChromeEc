...
fn explain_code(code: u32) {
    ...
    /// Bit that indicates that the AllowUnverifiedRo CCD capability was enabled.
    const ALLOW_UNVERIFIED_RO_ENABLED: u32 = 1 << 30;

    /// Bit that indicates that the AllowUnverifiedRo CCD capability was never.
    ...
    if code & ALLOW_UNVERIFIED_RO_ENABLED != 0 {
        println!("The system had the `AllowUnverifiedRo` capability enabled")
    }
    if code & ALLOW_UNVERIFIED_RO_NEVER != 0 {
        println!("The system had the `AllowUnverifiedRo` capability set to never")
    }
    let code = err.code();
