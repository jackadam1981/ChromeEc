...
static uint8_t ff_algo_buf[FF_ALGO_SIZE] __attribute__((aligned(4))); // Algo
								      // buffer
static uint8_t g_feature[FT_TPL_SUBTPL_SIZE]; // feature data for one image
static uint8_t
	g_finger_template_data[FT_TPL_HEAD_SIZE +
			       FT_TPL_SUBTPL_SIZE * SINGLE_FINGER_ENROLL_NUM]
	__attribute__((aligned(4))); // template data for one
				     // finger
			      ...
			      const uint8_t *const image, int *completion)
{
	...
	ret = focal_algo_get_feature((uint8_t *)image, g_feature,
				     &feature_size);
	if (ret == 0) {
		ret = focal_algo_enroll_by_feature(g_feature, enroll_index,
						   g_finger_template_data);
		if (ret == 0) {
			LOG_DBG("enroll success: %d", enroll_index);
			data->remain -= 1;
			*completion =
				(data->max_enroll_samples - data->remain) *
				100 / data->max_enroll_samples;
			memcpy(g_finger_template_data + data->tpl_head_size +
				       data->tpl_subtemplate_size *
					       enroll_index,
				...
				void *templ)
{
	...
	if (templ)
		memcpy(templ, g_finger_template_data,
		       sizeof(g_finger_template_data));
	...
	if (ret == 0) {
		for (int i = 0; i < templ_count; i++) {
			ret = focal_algo_verify_by_feature(
				g_feature, templ + i * FINGER_TEMPLATE_SIZE,
				&update_flag);
