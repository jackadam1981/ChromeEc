...
LOG_MODULE_REGISTER(egis_ec6601_alg, LOG_LEVEL_INF);
...
const egis_bio_info_t egis_bio_info = {
	.algorithm = &egis_bep_algorithm_pfe_6601g175_as,
...
static int egis_bep_algorithm_init(const struct fingerprint_algorithm *const alg)
{
	int rc;

	if (!IS_ENABLED(CONFIG_HAVE_EGIS_EC6601_PRIVATE_ALGORITHM)) {
...
static int egis_bep_algorithm_exit(const struct fingerprint_algorithm *const alg)
{
	int rc;
