...
#if defined(CONFIG_HAVE_ELAN80SG_PRIVATE_ALGORITHM)
#include <zephyr/drivers/fingerprint/fingerprint_elan80sg_private.h>
#elif defined(CONFIG_HAVE_ELANI80SA_PRIVATE_ALGORITHM)
#include <zephyr/drivers/fingerprint/fingerprint_elani80sa_private.h>
#else
#error "No supported ELAN fingerprint algorithm configured"
#endif
...
elan_elan80series_enroll_start(const struct fingerprint_algorithm *const alg)
{
	if (!IS_ENABLED(CONFIG_HAVE_ELAN80SG_PRIVATE_ALGORITHM) &&
	    !IS_ENABLED(CONFIG_HAVE_ELANI80SA_PRIVATE_ALGORITHM)) {
		return -ENOTSUP;
	}
