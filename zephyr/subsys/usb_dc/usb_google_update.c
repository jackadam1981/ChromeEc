...
static K_KERNEL_STACK_DEFINE(rx_thread_stack, CONFIG_GOOGLE_UPDATE_RX_STACK_SIZE);
static struct k_thread rx_thread_data;
static K_KERNEL_STACK_DEFINE(tx_thread_stack, CONFIG_GOOGLE_UPDATE_TX_STACK_SIZE);
...
static void google_update_read(uint8_t ep, int size, void *priv)
{
	...
	usb_transfer(ep_cfg[OUT_EP_IDX].ep_addr, data,
		     USB_MAX_FS_BULK_MPS, USB_TRANS_READ, google_update_read, NULL);
}
...
static void google_update_tx_thread(void *p1, void *p2, void *p3)
{
	...
	while (true) {
		...
		usb_transfer_sync(ep_cfg[IN_EP_IDX].ep_addr,
...
SYS_INIT(usb_google_update_init, APPLICATION, CONFIG_KERNEL_INIT_PRIORITY_DEVICE);
