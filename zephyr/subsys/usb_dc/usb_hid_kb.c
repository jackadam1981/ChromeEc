...
#define FEATURE_REPORT_SIZE sizeof(uint32_t) * CONFIG_USB_HID_KB_NUM_TOP_ROW_KEYS + 1
...
static void hid_keyboard_feature_init(void)
{
	...
	for (int i = 0; i < CONFIG_USB_HID_KB_NUM_TOP_ROW_KEYS; i++) {
		...
		if (IN_RANGE(key, 0, ARRAY_SIZE(action_key) - 1))
			memcpy(feature_report + i * sizeof(uint32_t) + 1, &action_key[key].usage, sizeof(uint32_t));
	}
}
...
static void hid_kb_proc_queue(void)
{
	...
	if (boot_proto_is_set()) {
		kb_data.byte_0.modifiers = kb_data.byte_1.modifiers;
		kb_data.byte_1.reserved = 0x0;
		size = HID_KEYBOARD_BOOT_SIZE;
	} else {
		kb_data.byte_0.report_id = REPORT_ID_1;
