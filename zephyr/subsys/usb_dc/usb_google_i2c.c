...
LOG_MODULE_REGISTER(usb_google_i2c, LOG_LEVEL_DBG);

#define AUTO_EP_IN 0x80
#define AUTO_EP_OUT 0x00

NET_BUF_POOL_FIXED_DEFINE(i2c_rx_pool, 2, 128, USB_MAX_FS_BULK_MPS, NULL);
NET_BUF_POOL_FIXED_DEFINE(i2c_tx_pool, 2, 128, USB_MAX_FS_BULK_MPS, NULL);
...
static void google_i2c_read(uint8_t ep, int size, void *priv)
{
	...
	usb_transfer(ep_cfg[OUT_EP_IDX].ep_addr, data, USB_MAX_FS_BULK_MPS,
		     USB_TRANS_READ, google_i2c_read, NULL);
}
				    ...
				    const uint8_t *param)
{
	...
	case USB_DC_CONFIGURED:
		...
		google_i2c_read(ep_cfg[OUT_EP_IDX].ep_addr, 0, NULL);
		break;
	default:
		break;
	}
...
// void i2c_stream_written(struct consumer const *consumer, size_t count)
// {
// 	static uint8_t data[USB_MAX_FS_BULK_MPS];
...
static void google_i2c_tx_thread(void *p1, void *p2, void *p3)
{
	...
	while (true) {
		...
		LOG_HEXDUMP_DBG(buf->data, buf->len, "Google I2C Tx:");
...
static void google_i2c_rx_thread(void *p1, void *p2, void *p3)
{
	...
	while (true) {
		...
		LOG_HEXDUMP_DBG(buf->data, buf->len, "Google I2C Rx:");
		net_buf_unref(buf);
	}
}
...
static int usb_google_i2c_init(void)
{
	...
	k_thread_name_set(&rx_thread_data, "google_i2c_rx");
	...
	k_thread_name_set(&tx_thread_data, "google_i2c_tx");

	return 0;
}

SYS_INIT(usb_google_i2c_init, APPLICATION,
