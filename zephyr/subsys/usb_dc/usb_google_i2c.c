...
NET_BUF_POOL_FIXED_DEFINE(i2c_rx_pool, 2, USB_MAX_FS_BULK_MPS, 0, NULL);
NET_BUF_POOL_FIXED_DEFINE(i2c_tx_pool, 2, USB_MAX_FS_BULK_MPS, 0, NULL);
...
static void google_i2c_rx_thread(void *p1, void *p2, void *p3)
{
	...
	while (true) {
		struct net_buf *buf;
		const struct queue *usb_to_i2c = usb_i2c.producer.queue;
