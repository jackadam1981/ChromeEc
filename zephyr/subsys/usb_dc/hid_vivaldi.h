...
#define KEYBOARD_TOP_ROW_FEATURE_DESC                                          \
	0x06, 0xd1, 0xff, /* Usage Page (Google) */                            \
		...
		0x29, CONFIG_PLATFORM_EC_KEYBOARD_NUM_TOP_ROW_KEYS, /* Usage   \
								       Maximum \
								     */        \
		0x95, CONFIG_PLATFORM_EC_KEYBOARD_NUM_TOP_ROW_KEYS, /* Report  \
								       Count   \
								     */        \
...
__override_proto int32_t get_feature_report(uint8_t report_id, uint8_t *data);
__override_proto uint32_t maybe_convert_function_key(int keycode);
