...
static void send_report(struct k_work *work)
{
	...
	if (!atomic_test_and_set_bit(hid_ep_in_busy, HID_EP_BUSY_FLAG)) {
		ret = hid_int_ep_write(hid_dev, (uint8_t *)&report_1,
...
static int usb_hid_test_init(void)
{
	hid_dev = DEVICE_DT_GET(DT_NODELABEL(hid_test_dev));
...
SYS_INIT(usb_hid_test_init, APPLICATION, CONFIG_KERNEL_INIT_PRIORITY_DEVICE);
