...
__overridable uint8_t *get_touchpad_report_descriptor(size_t *size)
{
	return NULL;
}

static size_t load_report_descriptor(void)
{
	uint8_t report_desc_header[6] = {
		HID_USAGE_PAGE(0x0D),
		HID_USAGE(0x05),
		HID_COLLECTION(HID_COLLECTION_APPLICATION)};
	uint8_t caps_and_cert_report_desc[] = {
		...
		HID_FEATURE(0x02)};
	size_t tp_report_desc_size;
	size_t report_desc_size;
	uint8_t *tp_report_desc =
		get_touchpad_report_descriptor(&tp_report_desc_size);

	if (!tp_report_desc) {
		LOG_ERR("failed to load touchpad report descriptor\n");
		return 0;
	}

	report_desc_size = ARRAY_SIZE(report_desc_header) +
			   tp_report_desc_size +
			   ARRAY_SIZE(caps_and_cert_report_desc) + 1;
	report_desc = (uint8_t *)malloc(report_desc_size);
	if (!report_desc) {
	...
	memcpy(report_desc + ARRAY_SIZE(report_desc_header), tp_report_desc,
	       tp_report_desc_size);
	memcpy(report_desc + ARRAY_SIZE(report_desc_header) +
		       tp_report_desc_size,
	       caps_and_cert_report_desc,
	       ARRAY_SIZE(caps_and_cert_report_desc));

	report_desc[report_desc_size - 1] = HID_END_COLLECTION;
...
static int usb_hid_tp_init(void)
{
	...
	size_t report_desc_size = load_report_descriptor();
	if (!report_desc_size) {
		goto out;
	}
	...
	usb_hid_register_device(hid_dev, report_desc, report_desc_size, &ops);
