...
__subsystem struct pd_altmode_driver_api {
	int (*altmode_read)(const struct device *dev,
			    union data_status_reg *data);
	int (*altmode_write)(const struct device *dev,
	...
	void (*altmode_set_result_cb)(const struct device *dev,
				      pd_altmode_callback cb);
};
