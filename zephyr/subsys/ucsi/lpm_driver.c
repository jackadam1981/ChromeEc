...
LOG_MODULE_REGISTER(lpm, LOG_LEVEL_INF);
...
static int ucsi_init_ppm(struct ucsi_pd_device *device)
{
	struct lpm_device *dev = (struct lpm_device *)device;

	return dev->ppm->init_and_wait(dev->ppm->dev, ARRAY_SIZE(dev->pdc));
}

static struct ucsi_ppm_driver *ucsi_get_ppm(struct ucsi_pd_device *device)
{
	struct lpm_device *dev = (struct lpm_device *)device;
...
static int ucsi_execute_cmd(struct ucsi_pd_device *device,
			    struct ucsi_control *control, uint8_t *lpm_data_out)
{
	struct lpm_device *lpm = (struct lpm_device *)device;
	struct ppm_common_device *dev =
		(struct ppm_common_device *)lpm->ppm->dev;
	...
	return pdc_execute_command_sync(lpm->pdc[conn - 1], ucsi_command,
...
static int lpm_init(const struct device *device)
{
	...
	for (int i = 0; i < ARRAY_SIZE(dev->pdc); i++) {
		rts54_set_handler_cb_ex(dev->pdc[i], ppm_cci_cb, NULL);
	}

	return 0;
}
DEVICE_DT_INST_DEFINE(0, &lpm_init, NULL, &lpm_dev, NULL, POST_KERNEL,
		      CONFIG_PDC_POWER_MGMT_INIT_PRIORITY, &lpm_drv);
