...
#define DT_DRV_COMPAT ucsi_ppm_driver
#define UCSI_7BIT_PORTMASK(p) ((p) & 0x7F)
#define DT_PPM_DRV DT_INST(0, ucsi_ppm_driver)
...
static struct ucsi_ppm_driver *ucsi_get_ppm(const struct device *device)
{
	struct ppm_data *dat = (struct ppm_data *)device->data;
...
static int ucsi_execute_cmd(const struct device *device,
