				     ...
				     uint8_t *lpm_data_out)
{
	...
	events = k_event_wait(&ppm_event, PPM_EVENT_ALL, true, K_MSEC(timeout));
...
static int ppm_init(const struct device *device)
{
	...
	for (int i = 0; i < cfg->active_port_count; i++) {
		int rv = pdc_add_ci_callback(cfg->lpm[i], &data->cb);
		if (rv)
			LOG_ERR("C%d: Failed to add CI callback (%d)", rv, i);
	}
