...
static void ppm_common_task(void *context)
{
	...
	if (!dev) {
		LOG_ERR("Cannot start PPM task without valid device pointer: %p",
			dev);
		return;
	}
...
K_THREAD_STACK_DEFINE(ppm_stack, STACK_SIZE);

static void ppm_common_thread_init(struct ppm_common_device *dev)
{
	dev->ppm_task_id = k_thread_create(&dev->ppm_task_data, ppm_stack,
					   STACK_SIZE, (void *)ppm_common_task,
					   (void *)dev, 0, 0,
					   CONFIG_PDC_POWER_MGMT_THREAD_PRIORTY,
					   0, K_NO_WAIT);
}
...
static void ppm_common_cleanup(struct ucsi_ppm_driver *driver)
{
	if (driver->dev) {
		...
		if (k_thread_join(&dev->ppm_task_data, K_FOREVER) != 0) {
			LOG_ERR("Failed to wait for ppm task to complete.");
		}
