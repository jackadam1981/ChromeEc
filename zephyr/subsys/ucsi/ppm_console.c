...
static int cmd_get_alt_modes(const struct shell *sh, int argc, char **argv)
{
	...
	if (!dev) {
		shell_error(sh, "PPM device not ready");
		return 1;
	}
	...
	struct ucsi_control_t get_am_cmd = {
                ...
                .command_specific = {
                        recipient & 0x07,
                        (port + 1) & 0x7F, /* Convert to 1-indexed port number */
	...
	for (int i = 0; i < ARRAY_SIZE(ucsi_response); i++) {
		...
		if (rv < 0) {
			...
			return 1;
                ...
                /* Debug */
                shell_info(sh, "Command returned %d (i=%d)", rv, i);
	}
	...
	for (int i = 0; i < ARRAY_SIZE(ucsi_response); i++) {
		shell_info(sh, "%03d    | SVID=0x%04x MID=0x%08x", 2 * i,
			   ucsi_response[2 * i].altmode_fields[0].svid,
			   ucsi_response[2 * i].altmode_fields[0].mid);
		shell_info(sh, "%03d    | SVID=0x%04x MID=0x%08x", 2 * i + 1,
			   ucsi_response[2 * i + 1].altmode_fields[1].svid,
			   ucsi_response[2 * i + 1].altmode_fields[1].mid);
	}
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(
		...
		"Usage: pdc status <port> [conn|sop|sopprime|sopprimeprime]",
