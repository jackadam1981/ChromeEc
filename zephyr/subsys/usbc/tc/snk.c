...
static void tc_detached(struct usb_tc_port_info *tc_port)
{
	if (USB_TC_INCLUDES_DBGACC_OPTION &&
	    tc_port->supports_debug_accessory)
		tcpm_debug_accessory(tc_port, 0);
}
...
void tc_set_data_role(struct usb_tc_port_info *tc_port,
		      enum pd_data_role role)
{
	tc_port->data_role = role;

	/* Notify TCPC of role update */
	tcpm_set_msg_header(tc_port->port, PD_ROLE_SINK, tc_port->data_role);
}
...
static void tc_snk_unattached_snk_entry(const int port)
{
	...
	tc_port->data_role = PD_ROLE_DISCONNECTED;
...
static void tc_snk_cc_rd_entry(const int port)
{
	...
	tcpm_set_msg_header(port, PD_ROLE_SINK, tc_port->data_role);
}
