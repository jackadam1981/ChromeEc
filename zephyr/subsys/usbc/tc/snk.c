 ...
 */
static struct usbc_tc_port_t tc[CONFIG_USB_PD_PORT_MAX_COUNT];
...
static void set_state_tc(const struct usbc_tc_port_t *tc_port,
		const enum usb_tc_state new_state)
{
	assert(tc_port->tid == k_current_get());

	smf_set_state(SMF_CTX(tc_port), &tc_states[new_state]);
}

/* Get the current TypeC state. */
static enum usb_tc_state get_state_tc(const struct usbc_tc_port_t *tc_port)
{
 ...
 * Public Functions
 */
const char *tc_get_current_state(struct usbc_tc_port_t *tc_port)
{
	if (CONFIG_USB_TC_SM_LOG_LEVEL != LOG_LEVEL_NONE)
...
uint32_t tc_get_flags(struct usbc_tc_port_t *tc_port)
{
	return tc_port->flags;
}

bool tc_is_attached_snk(struct usbc_tc_port_t *tc_port)
{
...
void pd_set_suspend(struct usbc_tc_port_t *tc_port, int suspend)
{
...
void pd_set_error_recovery(struct usbc_tc_port_t *tc_port)
{
	TC_SET_FLAG(tc_port, TC_FLAGS_REQUEST_ERROR_RECOVERY);
}

bool pd_is_port_enabled(struct usbc_tc_port_t *tc_port)
{
	/*
...
int tc_update_cc(struct usbc_tc_port_t *tc_port)
{
	int rv;
	enum tc_cc_pull pull = tc_port->select_cc_pull;
	enum tc_rp_value rp = TC_RP_USB;

	rv = tcpc_set_cc(tc_port->tcpc_dev, pull, rp);
	if (rv)
		LOG_ERR("C%d: TCPC set CC failed (%d)", tc_port->port, rv);
	return rv;
}

void tc_start_error_recovery(struct usbc_tc_port_t *tc_port)
{
	assert(tc_port->tid == k_current_get());
...
static void restart_tc_sm(struct usbc_tc_port_t *tc_port,
...
void tc_state_init(struct usbc_tc_port_t *tc_port)
{
	enum usb_tc_state first_state;
	struct tc_data_t *data = (struct tc_data_t *)tc_port->data;
	 ...
	 */
	first_state = (data->sm_initial_state == NULL)
				? TC_ERROR_RECOVERY
				: data->sm_initial_state(tc_port);
...
uint8_t tc_get_polarity(struct usbc_tc_port_t *tc_port)
{
...
static enum usb_tc_state get_last_state_tc(
				const struct usbc_tc_port_t *tc_port)
{
	return tc_port->ctx.previous - &tc_states[0];
}

static void print_current_state(const struct usbc_tc_port_t *tc_port)
{
	if (CONFIG_USB_TC_SM_LOG_LEVEL != LOG_LEVEL_NONE) {
		if (tc_debug_level > 1) {
			LOG_INF("C%d: %s\n", tc_port->port,
...
static void sink_stop_drawing_current(struct usbc_tc_port_t *tc_port)
{
	if (IS_ENABLED(CONFIG_CHARGE_MANAGER)) {
...
static void sink_power_sub_states(struct usbc_tc_port_t *tc_port)
{
	int rv;
	enum tc_cc_voltage_status cc1, cc2, cc;
	enum tc_cc_voltage_status new_cc_voltage;

	rv = tcpc_get_cc(tc_port->tcpc_dev, &cc1, &cc2);
	if (rv) {
...
static void tc_detach(struct usbc_tc_port_t *tc_port)
{
	int rv;

	if (USB_TC_INCLUDES_DBGACC_OPTION &&
	    tc_port->supports_debug_accessory) {
		rv = tcpc_set_debug_accessory(tc_port->tcpc_dev, false);
		if (rv) {
 ...
 */
static void tc_select_pull(struct usbc_tc_port_t *tc_port,
...
{
	struct usbc_tc_port_t *tc_port = tc_port_obj;
...
static void tc_snk_unattached_snk_entry(void *tc_port_obj)
{
	...
	if (USB_TC_INCLUDES_DBGACC_OPTION &&
	    tc_port->supports_debug_accessory) {
		rv = tcpc_set_debug_detach(tc_port->tcpc_dev);
		if (rv) {
	...
	rv = tcpc_set_roles(tc_port->tcpc_dev,
			    TC_ROLE_SINK,
...
static void tc_snk_unattached_snk_run(void *tc_port_obj)
{
	 ...
	 * manager is now seeded if it was not.
	 */
	if (IS_ENABLED(CONFIG_CHARGE_MANAGER))
...
static void tc_snk_attached_debug_accessory_snk_shared_entry(void *tc_port_obj)
{
	 ...
	 */
	rv = tcpc_enable_auto_discharge_disconnect(tc_port->tcpc_dev, true);
	if (rv) {
		LOG_ERR("C%d: TCPC "
...
static void tc_snk_debug_accessory_snk_entry(void *tc_port_obj)
{
	...
	rv = tcpc_set_debug_accessory(tc_port->tcpc_dev, true);
	if (rv) {
		LOG_ERR("C%d: TCPC set DebugAcc failed (%d)",
			tc_port->port, rv);
		return;
...
static void tc_snk_attached_debug_accessory_snk_shared_exit(void *tc_port_obj)
{
	 ...
	 */
	if (!TC_CHK_FLAG(tc_port, TC_FLAGS_REQUEST_SUSPEND)) {
		rv = tcpc_enable_auto_discharge_disconnect(tc_port->tcpc_dev,
							   false);
		if (rv) {
			LOG_ERR("C%d: TCPC "
				"enable AutoDischargeDisconnect failed (%d)",
