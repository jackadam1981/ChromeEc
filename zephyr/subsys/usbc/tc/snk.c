 ...
 *
 * if CONFIG_USBC_PD_DEBUG_LEVEL is defined then set that as the debug_level
 * and make it a constant that can not be changed.
 *
 * if CONFIG_USBC_PD_DEBUG_LEVEL is NOT defined then make debug_level
 * changeable with an initial value of DEBUG_LEVEL_1
 */
...
void tc_set_debug_level(const enum debug_level debug_level)
{
...
/* Return the TypeC state for the given context state */
static const enum usbc_tc_state context_state(
	const struct usbc_state *const ctx_state)
{
	/* Default to returning TC_STATE_COUNT if no state has been set */
	if (ctx_state == NULL)
		return TC_STATE_COUNT;
	else
		return ctx_state - &tc_states[0];
}

/* Get the current TypeC state. */
static const enum usbc_tc_state get_state_tc(
	const struct usbc_port_data *const port_data)
{
	return context_state(port_data->tc.ctx.current);
}

/* Get the previous TypeC state. */
static const enum usbc_tc_state get_last_state_tc(
	const struct usbc_port_data *const port_data)
{
	return context_state(port_data->tc.ctx.previous);
}


/**
 * Console log print for state machine tracking
 */
static void log_current_state(
...
/**
 * TypeC CC/Rp management
 */
static void tc_select_pull(
	struct usbc_port_data *const port_data,
	const enum tcpc_cc_pull pull)
{
	port_data->tc.select_cc_pull = pull;
}

/**
 * TypeC StateMachine Restart
 */
static void tc_sm_restart(
	struct usbc_port_data *const port_data,
	const enum usbc_tc_state start_state)
{
	int rv;
 ...
 * Public Functions
 */
const char *tc_get_current_state_name(
	const struct usbc_port_data *const port_data)
{
	if (CONFIG_USBC_TC_SM_LOG_LEVEL != LOG_LEVEL_NONE)
...
const uint32_t tc_get_flags(
	const struct usbc_port_data *const port_data)
{
	return port_data->tc.flags;
}

const bool tc_is_attached_snk(
	const struct usbc_port_data *const port_data)
{
	return IS_ATTACHED_SNK(port_data);
}

const bool tc_is_port_enabled(
	const struct usbc_port_data *const port_data)
{
	return !TC_CHK_FLAG(port_data, TC_FLAGS_SUSPENDED);
}

const int tc_update_cc(
	const struct usbc_port_data *const port_data)
{
	int rv;
	const enum tc_cc_pull pull = port_data->tc.select_cc_pull;
	const enum tc_rp_value rp = TC_RP_USB;
...
void tc_set_error_recovery(
	struct usbc_port_data *const port_data)
{
	TC_SET_FLAG(port_data, TC_FLAGS_REQUEST_ERROR_RECOVERY);
}

void tc_start_error_recovery(
	struct usbc_port_data *const port_data)
{
	...
	struct usbc_port_data *const port_data)
{
	 ...
	 */
	tc_sm_restart(port_data, first_state);
}
 ...
 */
static void tc_snk_disabled_entry(void *const port_obj)
{
	struct usbc_port_data *const port_data = port_obj;

	log_current_state(port_data);
...
static void tc_snk_disabled_run(void *const port_obj)
{
	struct usbc_port_data *const port_data = port_obj;
...
static void tc_snk_disabled_exit(void *const port_obj)
{
	int rv;
	struct usbc_port_data *const port_data = port_obj;
 ...
 */
static void tc_snk_error_recovery_entry(void *const port_obj)
{
	struct usbc_port_data *const port_data = port_obj;
...
static void tc_snk_error_recovery_run(void *const port_obj)
{
	struct usbc_port_data *const port_data = port_obj;
	...
	tc_sm_restart(port_data, TC_UNATTACHED_SNK);
}

static void tc_snk_error_recovery_exit(void *const port_obj)
{
	struct usbc_port_data *const port_data = port_obj;
 ...
 * Unattached.SNK
 */
static void tc_snk_unattached_snk_entry(void *const port_obj)
{
	int rv;
	struct usbc_port_data *const port_data = port_obj;
...
static void tc_snk_unattached_snk_run(void *const port_obj)
{
	int rv;
 ...
 * AttachWait.SNK
 */
static void tc_snk_attach_wait_snk_entry(void *const port_obj)
{
	struct usbc_port_data *const port_data = port_obj;
...
static void tc_snk_attach_wait_snk_run(void *const port_obj)
{
	int rv;
...
static void tc_snk_attach_wait_snk_exit(void *const port_obj)
{
	struct usbc_port_data *const port_data = port_obj;
 ...
 */
static void tc_snk_attached_debug_accessory_snk_shared_entry(
	void *const port_obj)
{
	int rv;
	enum tc_cc_voltage_state cc1, cc2;
...
static void tc_snk_attached_snk_entry(void *const port_obj)
{
	struct usbc_port_data *const port_data = port_obj;
...
#if (USBC_TC_INCLUDES_DBGACC_OPTION)
static void tc_snk_debug_accessory_snk_entry(void *const port_obj)
{
	int rv;
	struct usbc_port_data *const port_data = port_obj;
...
static void tc_snk_attached_debug_accessory_snk_shared_run(
	void *const port_obj)
{
	struct usbc_port_data *const port_data = port_obj;
...
static void tc_snk_attached_snk_run(void *const port_obj)
{
	tc_snk_attached_debug_accessory_snk_shared_run(port_obj);
}
#if (USBC_TC_INCLUDES_DBGACC_OPTION)
static void tc_snk_debug_accessory_snk_run(void *const port_obj)
{
	tc_snk_attached_debug_accessory_snk_shared_run(port_obj);
}
#endif /* USBC_TC_INCLUDES_DBGACC_OPTION */
...
static void tc_snk_attached_debug_accessory_snk_shared_exit(
	void *const port_obj)
{
	int rv;
	struct usbc_port_data *const port_data = port_obj;
...
static void tc_snk_attached_snk_exit(void *const port_obj)
{
	tc_snk_attached_debug_accessory_snk_shared_exit(port_obj);
}
#if (USBC_TC_INCLUDES_DBGACC_OPTION)
static void tc_snk_debug_accessory_snk_exit(void *const port_obj)
{
	int rv;
	struct usbc_port_data *const port_data = port_obj;
 ...
 * Super State CC_OPEN
 */
static void tc_snk_cc_open_entry(void *const port_obj)
{
	int rv;
	struct usbc_port_data *const port_data = port_obj;
