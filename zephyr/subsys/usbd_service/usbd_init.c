...
/* Set the configuration description string to empty string ("") because
 * the application Hammerd parses this string as the firmware version.
 * For the Zephyr codebase, the firmware version should be retrieved from
 * an extra command (UPDATE_EXTRA_CMD_GET_VERSION_STRING) rather than the
 * configuration description string.
 */
USBD_DESC_CONFIG_DEFINE(fs_cfg_desc, "");
