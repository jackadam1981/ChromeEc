...
LOG_MODULE_REGISTER(usbd_google_i2c, LOG_LEVEL_DBG);

static K_KERNEL_STACK_DEFINE(rx_thread_stack,
			     CONFIG_GOOGLE_I2C_RX_STACK_SIZE);
static struct k_thread rx_thread_data;
static K_KERNEL_STACK_DEFINE(tx_thread_stack,
			     CONFIG_GOOGLE_I2C_TX_STACK_SIZE);
...
// TODO: Think
NET_BUF_POOL_FIXED_DEFINE(gi2c_pool, 10, GOOGLE_EP_FS_MPS, sizeof(struct udc_buf_info), NULL);
...
static uint8_t google_get_in_ep(struct usbd_class_data *const c_data)
{
...
static uint8_t google_get_out_ep(struct usbd_class_data *const c_data)
{
	struct google_data *data = usbd_class_get_private(c_data);
	struct google_desc *desc = data->desc;
...
static int gi2c_out_start(struct usbd_class_data *const c_data)
{
	...
	if (!atomic_test_bit(&data->state, GI2C_DEV_CLASS_ENABLED)) {
		return -EPERM;
	}

	if (atomic_test_and_set_bit(&data->state, GI2C_DEV_CLASS_OUT_BUSY)) {
		return -EBUSY;
	}
...
static void gi2c_rx_thread(void *arg1, void *arg2, void *arg3)
{
	...
	while (true) {
		...
		atomic_clear_bit(&data->state, GI2C_DEV_CLASS_OUT_BUSY);
				...
				struct net_buf *const buf, const int err)
{
	...
	if (err) {
		LOG_ERR("request ep 0x%02x, len %u failed, err %d", bi->ep, buf->len, err);
...
static void usbd_gi2c_enable(struct usbd_class_data *const c_data)
{
	...
	atomic_set_bit(&data->state, GI2C_DEV_CLASS_ENABLED);
...
static void usbd_gi2c_disable(struct usbd_class_data *const c_data)
{
	...
	atomic_clear_bit(&data->state, GI2C_DEV_CLASS_ENABLED);
...
#define DEFINE_GI2C_CLASS_DATA(n)                             \
	static struct google_data gi2c_data_##n = {   \
		.sync_sem = Z_SEM_INITIALIZER(                   \
			gi2c_data_##n.sync_sem, 0, 1), \
		.desc = &gi2c_desc_##n,                       \
		.fs_desc = gi2c_fs_desc_##n,                  \
	};                                                       \
                                                                 \
	USBD_DEFINE_CLASS(gi2c_##n, &gi2c_api,      \
			  &gi2c_data_##n, NULL);
#else
...
#define DEFINE_GI2C_CLASS_DATA(n)                             \
	static struct google_data gi2c_data_##n = {   \
		...
		.fs_desc = gi2c_fs_desc_##n,                  \
                                                                 ...
                                                                 \
	USBD_DEFINE_CLASS(vendor_gi2c_##n, &gi2c_api,      \
			  &gi2c_data_##n, NULL);
#endif
...
static int gi2c_preinit(void)
{
	k_thread_create(&rx_thread_data, rx_thread_stack,
			...
			gi2c_rx_thread, (void *)&vendor_gi2c_0, NULL, NULL,
	...
	k_thread_create(&tx_thread_data, tx_thread_stack,
			...
			gi2c_tx_thread, (void *)&vendor_gi2c_0, NULL,
			NULL,
