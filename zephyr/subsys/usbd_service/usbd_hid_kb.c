...
static void kb_iface_ready(const struct device *dev, const bool ready)
{
	...
	if (ready) {
		atomic_set_bit(&keyboard.state, HID_IFACE_READY);
	} else {
		atomic_clear_bit(&keyboard.state, HID_IFACE_READY);
	}
}
...
#ifdef CONFIG_USBD_HID_VIVALDI
		if (get_vivaldi_feature_report(len, buf) != 0) {
			return len;
		}
#endif
			 ...
			 const uint8_t *const buf)
{
	...
	LOG_HEXDUMP_ERR(buf, len, "report:");
	return 0;
}
...
__overridable void keyboard_state_changed(int row, int col, int is_pressed)
{
	...
	if (!keyboard.dev ||
	    !atomic_test_bit(&keyboard.state, HID_IFACE_READY)) {
		return;
	}
...
static void hid_kb_proc_queue(void)
{
	...
	if (!atomic_test_bit(&keyboard.state, HID_IFACE_READY)) {
...
static void hid_kb_msg_cb(enum usbd_msg_type type)
{
	switch (type) {
	case USBD_MSG_RESET:
...
static int usb_hid_kb_init(void)
{
	...
	ret = usb_msg_callback_register(hid_kb_msg_cb);
	if (ret) {
		LOG_ERR("failed to register keyboard message callback");
		goto error;
	}

	keyboard.report_queue = QUEUE_NULL(32, struct usb_hid_keyboard_report);
