...
LOG_MODULE_DECLARE(usbd_kb_hid, LOG_LEVEL_INF);
...
#if defined(CONFIG_USBD_HID_KEYBOARD_ASSISTANT_KEY) || \
	defined(CONFIG_USBD_HID_KEYBOARD_TABLET_MODE_SWITCH)
...
#ifdef CONFIG_USBD_HID_KEYBOARD_ASSISTANT_KEY
...
#endif /* !CONFIG_USBD_HID_KEYBOARD_ASSISTANT_KEY */

#ifdef CONFIG_USBD_HID_KEYBOARD_TABLET_MODE_SWITCH
...
#define KEYBOARD_TABLET_MODE_SWITCH_DESC \
	HID_REPORT_COUNT(1), HID_REPORT_SIZE(1), HID_INPUT(0x01)
#endif /* CONFIG_USBD_HID_KEYBOARD_TABLET_MODE_SWITCH */
...
static int usb_hid_kb_init(void)
{
	...
	LOG_INF("HID Device: dev %p", hid_dev);
