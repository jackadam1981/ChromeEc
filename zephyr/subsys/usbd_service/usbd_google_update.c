...
// TODO: Think
NET_BUF_POOL_FIXED_DEFINE(gupdate_pool, 10, GOOGLE_EP_FS_MPS, sizeof(struct udc_buf_info), NULL);
...
static int gupdate_out_start(struct usbd_class_data *const c_data)
{
	...
	if (atomic_test_and_set_bit(&data->state, GUPDATE_DEV_CLASS_OUT_BUSY)) {
		return -EBUSY;
	}
...
static void gupdate_rx_thread(void *arg1, void *arg2, void *arg3)
{
	...
	while (true) {
		...
		atomic_clear_bit(&data->state, GUPDATE_DEV_CLASS_OUT_BUSY);
...
static void gupdate_tx_thread(void *arg1, void *arg2, void *arg3)
{
	...
	while (true) {
		...
		ret = usbd_ep_enqueue(c_data, buf);
		if (ret) {
			LOG_ERR("failed to send Gupdate data, ret %d", ret);
			net_buf_unref(buf);
			continue;
		}
...
static int gupdate_out_cb(struct usbd_class_data *const c_data,
				   ...
				   const enum usbd_speed speed)
{
	...
	LOG_ERR("Gupdate only supports full-speed");
...
static int gupdate_preinit(void)
{
	k_thread_create(&rx_thread_data, rx_thread_stack,
			...
			gupdate_rx_thread, (void *)&vendor_gupdate_0, NULL, NULL,
