...
// TODO: Think
NET_BUF_POOL_FIXED_DEFINE(gupdate_pool, 10, 128, GOOGLE_UPDATE_EP_FS_MPS, NULL);
...
static inline uint8_t gupdate_get_in_ep(struct usbd_class_data *const c_data)
{
	...
	struct gupdate_desc *desc = data->desc;
...
#define DEFINE_GUPDATE_CLASS_DATA(n)                                        \
	static struct gupdate_data gupdate_data_##n = {                     \
		...
		.fs_desc = gupdate_fs_desc_##n,                             \
                                                                            ...
                                                                            \
	USBD_DEFINE_CLASS(gupdate_##n, &gupdate_api, &gupdate_data_##n, NULL);
...
void updater_stream_written(struct consumer const *consumer, size_t count)
{
	...
	while (!queue_is_empty(consumer->queue)) {
		...
		if (count > GOOGLE_UPDATE_EP_FS_MPS) {
		...
		buf = gupdate_buf_alloc(gupdate_get_in_ep(&gupdate_0));
		if (!buf) {
			LOG_ERR("Failed to allocate buffer");
			return;
		}
...
static int gupdate_preinit(void)
{
	...
	k_thread_create(&tx_thread_data, tx_thread_stack,
			...
			gupdate_tx_thread, (void *)&gupdate_0, NULL, NULL,
