...
static void tp_iface_ready(const struct device *dev, const bool ready)
{
	...
	if (ready) {
		atomic_set_bit(&touchpad.state, HID_IFACE_READY);
	} else {
		atomic_clear_bit(&touchpad.state, HID_IFACE_READY);
	}
}
...
__overridable void set_touchpad_report(struct usb_hid_touchpad_report *report)
{
	...
	if (!touchpad.dev ||
	    !atomic_test_bit(&touchpad.state, HID_IFACE_READY)) {
		return;
	}

	if (touchpad.suspended) {
		int ret;

		ret = request_usb_wake();
		if (ret || touchpad.suspended) {
			LOG_ERR("failed to wakeup host");
			return;
		}
	}
...
static void hid_tp_proc_queue(void)
{
	...
	if (!atomic_test_bit(&touchpad.state, HID_IFACE_READY) ||
...
static void hid_tp_msg_cb(enum usbd_msg_type type)
{
	switch (type) {
	case USBD_MSG_RESET:
	...
	case USBD_MSG_SUSPEND:
		touchpad.suspended = true;
		break;
	case USBD_MSG_RESUME:
		touchpad.suspended = false;
		break;
	default:
		break;
	}
...
static int usb_hid_tp_init(void)
{
	...
	ret = usb_msg_callback_register(hid_tp_msg_cb);
	if (ret) {
		LOG_ERR("failed to register keyboard message callback");
		goto error;
	}

	touchpad.report_queue = QUEUE_NULL(8, struct usb_hid_touchpad_report);
