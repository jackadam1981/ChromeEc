...
static const char *const pdc_state_names[] = {
	...
	[PDC_SNK_TYPEC_ONLY] = "TypeCAttached.SNK",
	[PDC_SRC_TYPEC_ONLY] = "TypeCAttached.SRC",
};
...
/**
 * @brief Exit unattached state
 */
static void pdc_unattached_exit(void *obj)
{
	struct pdc_port_t *port = (struct pdc_port_t *)obj;

	port->return_state = get_pdc_state(port);
}
 ...
 * @brief Run source attached state
 */
static void pdc_src_attached_exit(void *obj)
{
	struct pdc_port_t *port = (struct pdc_port_t *)obj;
...
static int start_pdc_cmd(struct pdc_port_t *port, enum pdc_cmd_t cmd)
{
	int ret;

	switch (cmd) {
	case CMD_PDC_RESET:
		ret = pdc_reset(port->pdc);
		break;
	case CMD_PDC_GET_INFO:
		ret = pdc_get_info(port->pdc, &port->info);
		break;
	case CMD_PDC_SET_POWER_LEVEL:
		ret = pdc_set_power_level(port->pdc, port->una_policy.tcc);
		break;
	case CMD_PDC_SET_CCOM:
		ret = pdc_set_ccom(port->pdc, port->una_policy.cc_mode,
				   port->una_policy.drp_mode);
		break;
	case CMD_PDC_GET_PDOS:
		ret = pdc_get_pdos(port->pdc, SOURCE_PDO, PDO_OFFSET_0, PDO_NUM,
				   true, &port->snk_policy.pdos[0]);
		break;
	case CMD_PDC_GET_RDO:
		ret = pdc_get_rdo(port->pdc, &port->snk_policy.rdo);
		break;
	case CMD_PDC_SET_RDO:
		ret = pdc_set_rdo(port->pdc, port->snk_policy.rdo_to_send);
		break;
	case CMD_PDC_GET_VBUS_VOLTAGE:
		ret = pdc_get_vbus_voltage(port->pdc, &port->vbus);
		break;
	case CMD_PDC_SET_SINK_PATH:
		ret = pdc_set_sink_path(port->pdc, port->sink_path_en);
		break;
	case CMD_PDC_READ_POWER_LEVEL:
		ret = pdc_read_power_level(port->pdc);
		break;
	case CMD_PDC_GET_CONNECTOR_CAPABILITY:
		ret = pdc_get_connector_capability(port->pdc, &port->ccaps);
		break;
	case CMD_PDC_SET_UOR:
		ret = pdc_set_uor(port->pdc, port->uor);
		break;
	case CMD_PDC_SET_PDR:
		ret = pdc_set_pdr(port->pdc, port->pdr);
		break;
	case CMD_PDC_GET_CONNECTOR_STATUS:
		ret = pdc_get_connector_status(port->pdc,
	...
	if (ret) {
		LOG_DBG("Unable to send command: %s", pdc_cmd_names[cmd]);
	}
...
static void pdc_send_cmd_start_run(void *obj)
{
	...
	rv = start_pdc_cmd(port, port->cmd->cmd);
	...
	if (rv) {
		...
		if (port->send_cmd.retry_counter > RETRY_MAX) {
			...
			set_pdc_state(port, port->return_state);
		}
		return;
	}
...
static void pdc_send_cmd_start_exit(void *obj)
{
}
...
static void pdc_src_typec_only_exit(void *obj)
{
}
...
static const struct smf_state pdc_states[] = {
	...
	[PDC_SRC_ATTACHED] = SMF_CREATE_STATE(pdc_src_attached_entry,
					      pdc_src_attached_run,
					      pdc_src_attached_exit, NULL),
	[PDC_SEND_CMD_START] = SMF_CREATE_STATE(pdc_send_cmd_start_entry,
						pdc_send_cmd_start_run,
						pdc_send_cmd_start_exit, NULL),
	[PDC_SEND_CMD_WAIT] = SMF_CREATE_STATE(pdc_send_cmd_wait_entry,
					       pdc_send_cmd_wait_run,
					       pdc_send_cmd_wait_exit, NULL),
	[PDC_SNK_TYPEC_ONLY] = SMF_CREATE_STATE(pdc_snk_typec_only_entry, pdc_snk_typec_only_run, pdc_snk_typec_only_exit, 0),
	[PDC_SRC_TYPEC_ONLY] = SMF_CREATE_STATE(pdc_src_typec_only_entry, pdc_src_typec_only_run, pdc_src_typec_only_exit, 0),
};
