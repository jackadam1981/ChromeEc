...
static void pdc_src_attached_run(void *obj)
{
	...
	case SRC_ATTACHED_GET_PDOS:
		...
		of
		/* PDOs 2 Number of PDOs to return starting from the PDO Offset.
		/* The number of PDOs to return is the value in this field
		plus 1. **/
		if (port->get_pdo.pdo_nums > 4) {
			port->snk_attached_local_state = SRC_ATTACHED_GET_PDOS;
		} else {
...
static int send_pdc_cmd(struct pdc_port_t *port)
{
	...
	case CMD_PDC_GET_PDOS:
		rv = pdc_get_pdos(
			...
			port->get_pdo.pdo_nums > 4 ? 4 : port->get_pdo.pdo_nums,
			port->get_pdo.pdo_source,
			get_pdc_pdos_ptr(port, &port->get_pdo)->pdos +
				port->get_pdo.pdo_offset);
		if (port->get_pdo.pdo_nums > 4) {
			port->get_pdo.pdo_nums -= 4;
