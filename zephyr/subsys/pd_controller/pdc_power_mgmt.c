...
/**
 * @brief Short button press limit in us.
 */
#define PD_POWER_BUTTON_SHORT_PRESS_US 4000000

/**
 * @brief Long button press limit in us.
 */
#define PD_POWER_BUTTON_LONG_PRESS_US 8000000
...
static void handle_alert(struct pdc_port_t *port, uint32_t ado)
{
	...
	if (event_type == ADO_POWER_BUTTON_PRESS) {
		port->pd_button_pressed = get_time();
	} else if (event_type == ADO_POWER_BUTTON_RELEASE) {
		if (port->pd_button_pressed.val == 0) {
			/* If a release alert is received while the stored power
			 * button state is "released", there was likely a quick
			 * button press.
			 */
			pdc_power_mgmt_simulate_power_button_press(500);
		} else {
			/* Ignore presses over the long button press threshold.
			 * Short presses will always simulate a short PD button
			 * press. Long presses while the device is on will a
			 * shutdown the device.
			 */
			uint32_t press_duration =
				time_since32(port->pd_button_pressed);
			if (press_duration <= PD_POWER_BUTTON_LONG_PRESS_US) {
				if (press_duration >
					    PD_POWER_BUTTON_SHORT_PRESS_US &&
				    (chipset_in_state(
					...
					pdc_power_mgmt_simulate_power_button_press(
						500);
				}
			}
		}
		port->pd_button_pressed.val = UINT64_C(0);
	}
}
