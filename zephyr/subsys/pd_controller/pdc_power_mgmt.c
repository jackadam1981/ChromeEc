...
static void pdc_send_cmd_start_run(void *obj)
{
	...
	if (rv) {
		LOG_DBG("Unable to send command: %s", pdc_cmd_names[port->cmd->cmd]);
	}
...
static const struct smf_state pdc_states[] = {
	...
	[PDC_SRC_SNK_TYPEC_ONLY] = SMF_CREATE_STATE(pdc_src_snk_typec_only_entry, pdc_src_snk_typec_only_run, NULL, NULL),
};
