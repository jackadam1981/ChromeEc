...
static void pdc_update_battery_status(pdc_port_t *port);
static void pdc_update_battery_capability(pdc_port_t *port);
...
static void pdc_update_battery_status(struct pdc_port_t *port)
{
	...
	if (pdc_power_mgmt_is_sink_connected(port_number)) {
		atomic_set_bit(port->snk_policy.flags, SNK_POLICY_UPDATE_BATTERY_STATUS);
	} else if (pdc_power_mgmt_is_source_connected(port_number)) {
		atomic_set_bit(port->src_policy.flags, SRC_POLICY_UPDATE_BATTERY_STATUS);
	}
}
...
static void pdc_update_battery_capability(struct pdc_port_t *port)
{
	...
	port->bcap = bcdb;
	if (pdc_power_mgmt_is_sink_connected(port_number)) {
		atomic_set_bit(port->snk_policy.flags, SNK_POLICY_UPDATE_BATTERY_CAPABILITY);
	} else if (pdc_power_mgmt_is_source_connected(port_number)) {
		atomic_set_bit(port->src_policy.flags, SRC_POLICY_UPDATE_BATTERY_CAPABILITY);
	}
}
