...
static struct pdc_pdos_t *get_pdc_pdos_ptr(struct pdc_port_t *port, struct get_pdo_t *pdo_req)
{
	struct pdc_pdos_t *pdc_pdos;

	if (pdo_req->pdo_source == LPM_PDO && pdo_req->pdo_type == SINK_PDO) {
		pdc_pdos = &port->snk_policy.snk;
	} else if (pdo_req->pdo_source == LPM_PDO && pdo_req->pdo_type == SOURCE_PDO) {
		pdc_pdos = &port->src_policy.src;
	} else if (pdo_req->pdo_source == PARTNER_PDO && pdo_req->pdo_type == SINK_PDO) {
	...
	case SNK_ATTACHED_EVALUATE_PDOS:
		for (int i = 0; i < PDO_NUM; i++) {
			LOG_INF("PDO%d: %08x, %d %d", i,
				...
				PDO_FIXED_GET_VOLT(port->snk_policy.src.pdos[i]),
				PDO_FIXED_GET_CURR(port->snk_policy.src.pdos[i]));
		}
		...
		port->snk_policy.pdo =
			port->snk_policy.src.pdos[RDO_POS(port->snk_policy.rdo) - 1];
...
static int send_pdc_cmd(struct pdc_port_t *port)
{
	...
	case CMD_PDC_GET_PDOS:
		rv = pdc_get_pdos(port->pdc, port->get_pdo.pdo_type, PDO_OFFSET_0,
				  PDO_NUM, port->get_pdo.pdo_source,
	...
	case CMD_PDC_SET_PDOS:
		rv = pdc_set_pdos(port->pdc, port->set_pdo.type, port->set_pdo.pdos,
			port->set_pdo.count);
		break;
	default:
...
static void pdc_send_cmd_wait_exit(void *obj)
{
	...
	case CMD_PDC_GET_PDOS:
		...
		for (int i = 0; i < PDO_NUM; i++) {
			pdo = pdc_pdos->pdos[i];
			if (pdo && !(pdo & PDO_TYPE_AUGMENTED)) {
				LOG_INF("[PDO %d]: %08x: mv = %d, ma = %d",
					count, pdo, PDO_FIXED_GET_VOLT(pdo),
					PDO_FIXED_GET_CURR(pdo));
				count++;
				pdc_pdos->pdo_count++;
			}
		}
		pdc_pdos->pdo_count = count;
		break;
	default:
		break;
	}
...
{
	LOG_INF("get_snk_cap_cnt[%d]: count = %d, src_connected = %d",
		port,
		pdc_data[port]->port.src_policy.snk.pdo_count,
