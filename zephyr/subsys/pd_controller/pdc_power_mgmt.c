...
static const uint32_t pdc_src_pdo_nom[] = {
...
static void pdc_src_attached_run(void *obj)
{
	...
	case SRC_ATTACHED_GET_PDOS:
		...
		/* Evaluate SNK CAP after its been retrieved from the PDC */
...
enum usb_typec_current_t pdc_power_mgmt_get_default_current_limit_rp(int port)
{
 ...
 */
int pdc_power_mgmt_set_current_limit(int port_num, enum usb_typec_current_t tcc)
{
	struct pdc_port_t *pdc;
	...
	if (pdc->attached_state == SRC_ATTACHED_STATE) {
		...
		pdc->set_pdo.count = 1;
		pdc->set_pdo.type = SOURCE_PDO;
		pdc->set_pdo.pdos[0] = TC_CURRENT_3_0A ? pdc_src_pdo_max[0] :
							 pdc_src_pdo_nom[0];
