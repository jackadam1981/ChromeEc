...
static bool pdc_is_one_or_fewer_snk_path_en(void)
{
...
pdc_snk_attached_evaluate_pdos(struct pdc_port_t *port)
{
	...
	if ((pdc_is_one_or_fewer_snk_path_en() &&
	     (selected_port == config->connector_num ||
	      selected_port == CHARGE_PORT_NONE)) ||
	    port->sink_path_en == false) {
		/* Set RDO when snk_path at most is enabled on one port,
		 * and we're either the selected port or no port is selected. */
	...
	if (port->sink_path_en && selected_port >= 0 &&
	    selected_port != config->connector_num) {
		/* Disable sink path when we're not the selected port */
		port->sink_path_en = false;
		queue_internal_cmd(port, CMD_PDC_SET_SINK_PATH);
		return SNK_ATTACHED_EVALUATE_PDOS;
	}

	/* More than one sink path is enabled, notify other ports
	 * to evaluate their PDOs and if they should turn off their
	 * sink path */
	uint32_t snk_path_en_mask = pdc_get_snk_path_en_mask();
	...
	for (int i = 0; i < CONFIG_USB_PD_PORT_MAX_COUNT; i++) {
		if (selected_port != CHARGE_PORT_NONE && selected_port != i &&
...
static void pdc_snk_attached_run(void *obj)
{
	...
	case SNK_ATTACHED_GET_PDOS:
		...
		queue_internal_cmd(port, CMD_PDC_GET_PDOS);
		return;
	case SNK_ATTACHED_GET_RDO_INIT:
		port->snk_attached_local_state = SNK_ATTACHED_SYNC_CHARGE_MGR;
		queue_internal_cmd(port, CMD_PDC_GET_RDO);
		return;
	case SNK_ATTACHED_SYNC_CHARGE_MGR:
		/* Update Charge Manager with PDO/RDO the PDC booted up with */
		port->snk_attached_local_state = SNK_ATTACHED_EVALUATE_PDOS;
		pdc_snk_seed_charge_manager(
			port, port->snk_policy.src
				      .pdos[RDO_POS(port->snk_policy.rdo) - 1]);
		return;
	...
	case SNK_ATTACHED_START_CHARGING:
		pdc_snk_seed_charge_manager(port, port->snk_policy.pdo);
...
uint32_t pdc_power_mgmt_get_requested_voltage(int port)
{
	...
	pd_extract_pdo_power(pdc_data[port]->port.snk_policy.pdo, &max_ma,
			     &max_mv, &tmp);
