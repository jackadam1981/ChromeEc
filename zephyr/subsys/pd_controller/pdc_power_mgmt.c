...
static void pdc_snk_attached_run(void *obj)
{
	...
	case SNK_ATTACHED_SYNC_CHARGE_MGR:
		/* Update Charge Manager with PDO/RDO the PDC has negotiated.
		 * Proceed to RUN state as charge manager module will evaluate
		 * for better PDOs and notify PDC Power MGMT to update as
		 * needed. */
		port->snk_attached_local_state = SNK_ATTACHED_RUN;
		pdc_snk_seed_charge_manager(
			port,
			port->snk_policy.src
				.pdos[RDO_POS(port->connector_status.rdo)]);
		return;
...
{
	uint32_t max_ma, max_mv, tmp, pdo;
	...
	pdo = pdc_power_mgmt_get_selected_pdo(port);
