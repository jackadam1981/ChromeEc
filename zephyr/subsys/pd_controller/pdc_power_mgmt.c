...
enum attached_flag_t {
	...
	/** SNK_ATTACHED_FLAG */
	SNK_ATTACHED_FLAG,
	/** SRC_ATTACHED_TYPEC_ONLY_FLAG */
	SRC_ATTACHED_TYPEC_ONLY_FLAG,
	/** SNK_ATTACHED_TYPEC_ONLY_FLAG */
	SNK_ATTACHED_TYPEC_ONLY_FLAG,
};
...
struct pdc_port_t {
	...
	enum attached_flag_t attached_flag;
...
static void handle_connector_status(struct pdc_port_t *port)
{
	if (!port->connector_status.connect_status) {
		...
		case USB_TC_CURRENT_1_5A:
			port->typec_charge = 1500;
			break;
		case USB_TC_CURRENT_3A:
			port->typec_charge = 3000;
			break;
		case USB_TC_CURRENT_5A:
			port->typec_charge = 5000;
			break;
		}
...
static void pdc_unattached_entry(void *obj)
{
	...
	port->attached_flag = UNATTACHED_FLAG;
	port->send_cmd.intern.pending = false;
...
static void pdc_src_attached_run(void *obj)
{
	...
	case SRC_ATTACHED_RUN:
		port->attached_flag = SRC_ATTACHED_FLAG;
...
static void pdc_snk_attached_run(void *obj)
{
	...
	case SNK_ATTACHED_RUN:
		port->attached_flag = SNK_ATTACHED_FLAG;
...
static void pdc_src_typec_only_run(void *obj)
{
	...
	port->attached_flag = SRC_ATTACHED_TYPEC_ONLY_FLAG;
...
static void pdc_snk_typec_only_entry(void *obj)
{
	...
	typec_set_input_current_limit(config->connector_num, port->typec_charge,
				      5000);
...
static void pdc_snk_typec_only_run(void *obj)
{
	...
	port->attached_flag = SNK_ATTACHED_TYPEC_ONLY_FLAG;
...
static bool pdc_power_mgmt_is_sink_connected(int port)
{
	...
	return pdc_data[port]->port.attached_flag == SNK_ATTACHED_FLAG;
}
...
static bool pdc_power_mgmt_is_source_connected(int port)
{
	...
	return pdc_data[port]->port.attached_flag == SRC_ATTACHED_FLAG;
}

static bool pdc_power_mgmt_is_source_typec_connected(int port)
{
	if (!is_pdc_port_valid(port)) {
		return false;
	}

	return pdc_data[port]->port.attached_flag == SRC_ATTACHED_TYPEC_ONLY_FLAG;
}

static bool pdc_power_mgmt_is_sink_typec_connected(int port)
{
	if (!is_pdc_port_valid(port)) {
		return false;
	}

	return pdc_data[port]->port.attached_flag == SNK_ATTACHED_TYPEC_ONLY_FLAG;
}
...
bool pdc_power_mgmt_is_connected(int port)
{
	...
	return pdc_data[port]->port.attached_flag != UNATTACHED_FLAG;
}
...
bool pdc_power_mgmt_pd_capable(int port)
{
	...
	return (pdc_data[port]->port.attached_flag == SNK_ATTACHED_FLAG) || (pdc_data[port]->port.attached_flag == SRC_ATTACHED_FLAG);
}
...
{
	if (pdc_power_mgmt_is_source_typec_connected(port)) {
		return pdc_state_names[PDC_SRC_TYPEC_ONLY];
	} else if (pdc_power_mgmt_is_sink_typec_connected(port)) {
		return pdc_state_names[PDC_SNK_TYPEC_ONLY];
