...
{
	const enum snk_attached_local_state_t same_state =
		port->snk_attached_local_state;
	enum snk_attached_local_state_t next_state;
	const struct pdc_config_t *const config = port->dev->config;
	...
	pdc_snk_seed_charge_manager(port, selected_pdo);
	selected_port = charge_manager_get_selected_charge_port();
	...
	if (pdc_snk_policy_is_pdo_same(port, selected_port, selected_pdo,
		...
		next_state = SNK_ATTACHED_START_CHARGING;
	} else if ((pdc_is_one_or_less_snk_path_en() &&
		...
		for (int i = 0; i < CONFIG_USB_PD_PORT_MAX_COUNT; i++) {
			if (selected_port != CHARGE_PORT_NONE &&
			    selected_port != i &&
