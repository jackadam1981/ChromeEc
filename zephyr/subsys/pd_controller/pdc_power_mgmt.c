...
static bool handle_connector_status(struct pdc_port_t *port)
{
	...
	if (!status->connect_status) {
		...
		case PD_OPERATION:
			...
			/* If there are no status change bits set and the
			 * partner is using an alternate mode, this is possibly
			 * a DP status update (Attention).
			 */
			if (!conn_status_change_bits.raw_value &&
			    (status->conn_partner_flags &
