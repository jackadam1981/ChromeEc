...
uint8_t pdc_power_mgmt_get_task_state(int port)
{
	...
	if (port < 0 || port >= CONFIG_USB_PD_PORT_MAX_COUNT) {
		/* Return INVALID for ports outside of the bounds. Note: ports
		 * that area not INVALID can still be temporarily or permanently
		 * inoperable, as reflected by other states. */
		return PDC_INVALID;
	}
