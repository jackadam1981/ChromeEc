...
static void pdc_snk_attached_run(void *obj)
{
	...
	case SNK_ATTACHED_EVALUATE_PDOS:
		...
		for (int i = 0; i < PDO_NUM; i++) {
			...
			tmp_volt_mv = PDO_FIXED_GET_VOLT(port->snk_policy.pdos[i]);
			tmp_curr_ma = PDO_FIXED_GET_CURR(port->snk_policy.pdos[i]);
			tmp_pwr_mw = (tmp_volt_mv * tmp_curr_ma) / 1000;

			LOG_INF("PDO%d: %08x, %d %d %d", i,
				port->snk_policy.pdos[i], tmp_volt_mv, tmp_curr_ma,
				tmp_pwr_mw);
