	...
	if (port->cmd->cmd == CMD_PDC_RESET) {
		...
		if (port->cmd->cmd == CMD_PDC_GET_CONNECTOR_STATUS) {
			...
			if (port->cmd->cmd == CMD_PDC_GET_PD_VDO_DP_STATUS) {
				const struct pdc_config_t *config =
					port->dev->config;
				if (pdc_power_mgmt_check_hpd_wake(
					    config->connector_num)) {
					LOG_DBG("C%d: HPD Wake",
						config->connector_num);
				}
			}
...
test_mockable bool pdc_power_mgmt_check_hpd_wake(int port)
{
	/* Check for an HPD wake. The conditions are...
