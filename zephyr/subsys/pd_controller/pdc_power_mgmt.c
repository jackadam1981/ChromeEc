...
static void pdc_update_battery_status(void)
{
	...
	if (battery_is_present()) {
		...
		if (battery_status(&c) != 0) {
			bsdo.battery_state = BSDO_BATTERY_STATE_IDLE; /* assume
									 idle */
		} else {
	...
	for (int i = 0; i < pdc_power_mgmt_get_usb_pd_port_count(); i++) {
				pdc_data[i]->port.bstat = bsdo;
				if (pdc_power_mgmt_is_sink_connected(i))
					atomic_set_bit(pdc_data[i]->port.snk_policy.flags,
											SNK_POLICY_UPDATE_BATTERY_STATUS);
				else if (pdc_power_mgmt_is_source_connected(i))
					atomic_set_bit(pdc_data[i]->port.src_policy.flags,
											SRC_POLICY_UPDATE_BATTERY_STATUS);
    }
}
