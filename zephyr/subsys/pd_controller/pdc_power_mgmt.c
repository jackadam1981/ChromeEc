...
uint8_t Select_9V_0A = 0;
uint8_t Select_5V_0A = 0;
			      ...
			      struct pdc_snk_attached_policy_t *snk_policy)
{
	...
	if ((snk_policy->pdo & PDO_TYPE_MASK) == PDO_TYPE_BATTERY) {
		...
		if (Select_9V_0A) {
			/* Select 9V/3A SRC_CAP and operate current to 0 */
			snk_policy->pdo_index = 2;
			max_ma = 3000;
			snk_policy->rdo_to_send =
				RDO_FIXED(snk_policy->pdo_index, 0/*500*/, max_ma, flags);
			Select_9V_0A = 0;
			Select_5V_0A = 1;
		} else if (Select_5V_0A) {
			/* Select 5V/3A SRC_CAP and operate current to 0 */
