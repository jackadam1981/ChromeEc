...
#define PDC_POWER_STATE_DEBOUNCE (2 * SECOND)
 ...
 *
 * The hooks and this function will not be triggered at all during a late
 * sysjump, leaving the PDC and subsystem in an inconsistent state. This
 * function should be called to corect that.
 *
 * Note: this should run once, and not per-port.
 */
static void pdc_apply_power_state_policy(void)
{
...
DECLARE_DEFERRED(pdc_apply_power_state_policy);
...
static void pdc_init_run(void *obj)
{
	...
	if (pdc_is_init_done(port->pdc)) {
		...
		if (system_jumped_late() && pdc_all_ports_ready()) {
			...
			hook_call_deferred(&pdc_apply_power_state_policy_data,
					   0);
		}
