...
uint16_t pd_get_identity_vid(int port)
{
	...
	if (!pdc_data[port]->port.vdo[IDENTITY_VID_VDO_IDX]) {
		pdc_run_get_discovery(port);
	}

	if (pdc_data[port]->port.vdo[IDENTITY_VID_VDO_IDX]) {
		vid = PD_IDH_VID(pdc_data[port]->port.vdo[IDENTITY_VID_VDO_IDX]);
	}
...
uint16_t pd_get_identity_pid(int port)
{
	...
	if (pdc_data[port]->port.vdo[IDENTITY_PID_VDO_IDX]) {
		pid = PD_PRODUCT_PID(pdc_data[port]->port.vdo[IDENTITY_PID_VDO_IDX]);
	}
...
uint8_t pd_get_product_type(int port)
{
	...
	if (pdc_data[port]->port.vdo[IDENTITY_PTYPE_VDO_IDX]) {
		ptype = PD_IDH_PTYPE(pdc_data[port]->port.vdo[IDENTITY_PTYPE_VDO_IDX]);
	}
