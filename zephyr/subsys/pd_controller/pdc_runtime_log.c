...
static void set_pdc_log(uint32_t level, bool silent)
{
	...
	for (size_t i = 0; i < sizeof(log_modules) / sizeof(log_modules[0]);
	     ++i) {
		const char *log_module = log_modules[i];
		int source_id = log_source_id_get(log_module);
		if (source_id < 0) {
			if (!silent)
		...
		if (!silent)
			ccprintf(
				"pdc_log module:%s try to set log level to %" PRIu32
				"\n",
				log_module, level);
...
static int command_pdc_log(int argc, const char **argv)
{
	...
	set_pdc_log(level, false);
	return EC_SUCCESS;
}
DECLARE_CONSOLE_COMMAND(pdc_log, command_pdc_log, "<NONE|ERR|WRN|INF|DBG>",
			"Control pdc log at runtime.");

static void pdc_runtime_init(void)
{
	LOG_INF("init: disable pdc log\n");
	set_pdc_log(LOG_LEVEL_NONE, true);
}
DECLARE_HOOK(HOOK_INIT, pdc_runtime_init, HOOK_PRIO_DEFAULT);

static void pdc_runtime_startup(void)
{
	LOG_INF("startup: enable pdc log\n");
	set_pdc_log(LOG_LEVEL_INF, true);
}
DECLARE_HOOK(HOOK_CHIPSET_STARTUP, pdc_runtime_startup, HOOK_PRIO_DEFAULT);
