...
static enum ec_status hc_remote_pd_chip_info(struct host_cmd_handler_args *args)
{
	...
	resp.fw_version_string[0] = PDC_FWVER_GET_MAJOR(pdc_info.fw_version);
	resp.fw_version_string[1] = PDC_FWVER_GET_MINOR(pdc_info.fw_version);
	resp.fw_version_string[2] = PDC_FWVER_GET_PATCH(pdc_info.fw_version);
...
/**
 * @brief Stub for sending PD host events to the AP (not currently implemented)
 */
