...
static void pdc_dpm_balance_source_ports(struct k_work *work)
{
	...
	while (new_ports) {
		...
		if (count_port_bits(max_current_claimed) <
			...
			rp = pdc_power_mgmt_get_default_current_limit_rp(
				rem_non_pd);
...
void pdc_dpm_evaluate_request_rdo(int port, uint32_t rdo)
{
	...
	op_ma = (rdo >> 10) & 0x3FF;
	if (atomic_test_bit(&sink_max_pdo_requested, port) && (op_ma <= 150)) {
		/*
...
void pdc_dpm_remove_sink(int port)
{
	...
	rp = pdc_power_mgmt_get_default_current_limit_rp(port);
