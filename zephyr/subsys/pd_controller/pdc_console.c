...
static int cmd_pdc_srccaps(const struct shell *sh, size_t argc, char **argv)
{
	...
	for (uint8_t i = 0; i < src_caps_count; i++) {
		uint32_t s = src_caps[i];
		uint32_t max_ma = 0, max_mv = 0, min_mv = 0;
		const char *type_str;

		pd_extract_pdo_power(s, &max_ma, &max_mv, &min_mv);

		switch (s & PDO_TYPE_MASK) {
		case PDO_TYPE_FIXED:
			type_str = "FIX";
			/* Fixed PDOs have flags and a single voltage */
			shell_fprintf(
				sh, SHELL_INFO,
				"Src %02u: %08x %s %13umV, %5umA [%s %s %s %s %s]\n",
				i, s, type_str, max_mv, max_ma,
				s & PDO_FIXED_DUAL_ROLE ? "DRP" : "   ",
				s & PDO_FIXED_UNCONSTRAINED ? "UP" : "  ",
				s & PDO_FIXED_COMM_CAP ? "USB" : "   ",
				s & PDO_FIXED_DATA_SWAP ? "DRD" : "   ",
				s & PDO_FIXED_FRS_CURR_MASK ? "FRS" : "   ");
			continue;
		...
		shell_fprintf(sh, SHELL_INFO,
			      ...
			      (((s & PDO_TYPE_MASK) == PDO_TYPE_BATTERY) ?
				       'W' :
				       'A'));
	}
