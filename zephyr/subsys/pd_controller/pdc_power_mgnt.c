...
/**
 * @brief maximum numer of counts to wait the subsystem to respond to an API
 * call
 */
#define BLOCK_COUNTER_MAX 10

/**
 * @breif PDC driver commands
 */
enum pdc_cmd_t {
	/** CMD_PDC_RESET */
	CMD_PDC_RESET,
 ...
 * @brief PDC thread
 */
static ALWAYS_INLINE void pdc_thread(void *pdc_dev)
{
...
#define PDC_SUBSYS_INIT(inst)                                                  \
	K_THREAD_STACK_DEFINE(my_stack_area_##inst, CONFIG_PDC_STACK_SIZE);    \
                                                                               \
	static void run_pdc_##inst(void *pdc_dev, void *unused1,               \
				   void *unused2)                              \
	{                                                                      \
		while (1) {                                                    \
			pdc_thread(pdc_dev);                                   \
	...
	{                                                                      \
		...
		data->thread = k_thread_create(                                \
			...
			run_pdc_##inst, (void *)dev, 0, 0,                     \
			CONFIG_PDC_THREAD_PRIORITY, K_ESSENTIAL, K_NO_WAIT);   \
		k_thread_name_set(data->thread, "PDC");                        \
	}                                                                      \
                                                                               \
	static struct pdc_data_t data_##inst = {                               \
		/* Initial policy read from device tree */                     \
	...
	static struct pdc_config_t config_##inst = {                           \
		.connector_num = 0, /* TODO: Read from DT */                   \
...
/**
 * @breif Entering connector change event state
 */
...
static void pdc_cmd_send_run(void *o)
{
	...
	case CMD_SEND_START:
		...
		case CMD_PDC_GET_PDOS:
			rv = pdc_get_pdos(port->pdc, SOURCE_PDO, PDO_OFFSET_0,
					  7, true,
					  &port->snk_policy.pdos[0].raw_value);
			break;
	...
	case CMD_SEND_WAIT:
		...
		/* Test if an error occured trying to send the command */
		else if (atomic_test_and_clear_bit(&port->cci_flags,
						   CCI_ERROR)) {
			/* Increment retry counter */
			port->retry_counter++;
...
static void pdc_cmd_send_exit(void *o)
{
	...
	/* Clear PDC command */
	port->pdc_cmd = 0;
	/* Cleare PDC command flags */
	port->pdc_cmd_flags = ATOMIC_INIT(0);
}
	...
	case SNK_ATTACHED_START_CHARGING:
		for (int i = 0; i < 7; i++) {
			printk("PDO%d: %08x, %d %d\n", i,
			       port->snk_policy.pdos[i].raw_value,
			       port->snk_policy.pdos[i].voltage * 50,
			       port->snk_policy.pdos[i].max_current * 10);
		}

		printk("RDO: %d\n", RDO_POS(port->snk_policy.rdo));
		port->snk_policy.pdo.raw_value =
			port->snk_policy.pdos[RDO_POS(port->snk_policy.rdo) - 1]
				.raw_value;

		/* Extract Current, Voltage, and calculate Power */
		max_ma = port->snk_policy.pdo.max_current * 10;
		max_mv = port->snk_policy.pdo.voltage * 50;
		...
		if ((port->snk_policy.pdo.type == FIXED &&
		     (!port->snk_policy.pdo.drp ||
		      port->snk_policy.pdo.unconstrained_pwr)) ||
...
static void pdc_snk_attached_exit(void *o)
{
	...
	if (port->next_state != PDC_CMD_SEND) {
		...
		/* Invalidate PDO */
		port->snk_policy.pdo.raw_value = 0;
	}
}
...
/**
 * PDC Power Managment Public API
 */
...
bool pm_get_partner_dual_role_power(int port)
{
	 ...
	 * connector capabilites are read.
	 */
