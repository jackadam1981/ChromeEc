...
static int cmd_effect_set1(const struct shell *sh, size_t argc, char **argv)
{
	if (argc != 5) {
		shell_error(sh, "Usage: set1 <index> <r> <g> <b> (values 0-255)");
		return -EINVAL;
	}
	...
	long index = strtol(argv[1], NULL, 0);

	if (index < 0 || index >= STRIP_NUM_PIXELS) {
		shell_error(sh, "Invalid index: %ld. Must be 0-%d.",
			    index, STRIP_NUM_PIXELS - 1);
		return -EINVAL;
	}

	struct led_rgb color = {
		.r = (uint8_t)strtol(argv[2], NULL, 0),
		.g = (uint8_t)strtol(argv[3], NULL, 0),
		.b = (uint8_t)strtol(argv[4], NULL, 0),
	};

	shell_print(sh, "Setting pixel %ld to R:%d G:%d B:%d",
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_effect,
	...
	SHELL_CMD_ARG(set, NULL, "Set a solid color: set <r> <g> <b>",
		      cmd_effect_set, 4, 3),
	SHELL_CMD_ARG(set1, NULL, "Set a single pixel: set1 <index> <r> <g> <b>",
		      cmd_effect_set1, 5, 4),
