 ...
 *        One node contains at least one output signal and mote than one intput
 *        signal.
 */
					     ...
					     gpio_port_pins_t pins)
{
	...
	LOG_DBG("INT: Set Signal %s -> %d", out_signal->desc.name, out_value);
	for (int i = 0; i < node->inputs_count; i++) {
		...
		LOG_DBG("INT: Delay Signal %s", in_signal->desc.name);
