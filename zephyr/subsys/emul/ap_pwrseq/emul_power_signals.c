 ...
 *        One node contains at least one output signal and one or more intput
 *        signals.
 */
struct power_signal_emul_node {
	...
	int outputs_count;
	struct power_signal_emul_output outputs[];
};
...
#define EMUL_POWER_SIGNAL_NODES_DEF(inst)                                      \
	...
	static struct power_signal_emul_node DT_CAT(inst, _node) = {           \
		...
		.outputs = EMUL_POWER_SIGNAL_OUT_ARRAY_DEF(inst),              \
	};
			    ...
			    int value)
{
	...
	if (desc->source == PWR_SIG_EMUL_SRC_EXT) {
		...
		zassert_ok(ret, "Setting %s Signal value!!", desc->name);
	}
 ...
 *
 * This will enable correspondinf initiator power signal interruption and
 * its handlers power signals work structures.
 *
 * @param node Pointer to node containing power signals.
 */
static void power_signal_init_node(struct power_signal_emul_node *node)
{
	...
	const struct power_signal_emul_test_platform *test_platform)
{
	...
	for (int i = 0; i < cur_test_platform->nodes_count; i++) {
		power_signal_init_node(cur_test_platform->nodes[i]);
	}
