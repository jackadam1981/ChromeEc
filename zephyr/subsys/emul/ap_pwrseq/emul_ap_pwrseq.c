...
static void ap_pwrseq_emul_entry_callback(const struct device *dev,
...
static void ap_pwrseq_emul_exit_callback(const struct device *dev,
					 enum ap_pwrseq_state new,
					 enum ap_pwrseq_state curr)
...
void ap_pwrseq_post_event(const struct device *dev, enum ap_pwrseq_event event)
{
	...
	while (true) {
		...
		if (cur_state != new_state) {
			ap_pwrseq_emul_entry_callback(dev, new_state,
						      cur_state);
			ap_pwrseq_emul_exit_callback(dev, new_state, cur_state);
		} else {
			return;
		}
	}
