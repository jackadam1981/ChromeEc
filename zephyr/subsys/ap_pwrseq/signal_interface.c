...
#define SELECT_SIGNAL(id, compat, src, tag)		\
...
DT_FOREACH_CHILD_VARGS(DT_COMPAT_GET_ANY_STATUS_OKAY(intel_ap_pwrseq),
	SELECT_SIGNAL,
	intel_ap_pwrseq_input_gpio,
	PWR_SIG_SRC_GPIO,
	TAG_GPIO_PWR_SIGNAL)
DT_FOREACH_CHILD_VARGS(DT_COMPAT_GET_ANY_STATUS_OKAY(intel_ap_pwrseq),
	SELECT_SIGNAL,
	intel_ap_pwrseq_output_gpio,
	PWR_SIG_SRC_GPIO,
	TAG_GPIO_PWR_SIGNAL)
DT_FOREACH_CHILD_VARGS(DT_COMPAT_GET_ANY_STATUS_OKAY(intel_ap_pwrseq),
	SELECT_SIGNAL,
...
int power_signal_enable_interrupt(enum power_signal signal)
{
	...
	case PWR_SIG_SRC_GPIO:
		return power_signal_gpio_enable_int(cp->src_id);
	}
}
...
int power_signal_disable_interrupt(enum power_signal signal)
{
	...
	case PWR_SIG_SRC_GPIO:
		return power_signal_gpio_disable_int(cp->src_id);
	}
}
