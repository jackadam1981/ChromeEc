...
#define AP_PWRSEQ_WAIT_EVENT_MASK GENMASK(AP_PWRSEQ_EVENT_COUNT, 0)
...
static uint32_t ap_pwrseq_wait_event(const struct device *dev)
{
	...
	events = k_event_wait(&data->evt, AP_PWRSEQ_WAIT_EVENT_MASK, false,
			      Z_FOREVER);
	...
	while (true) {
		...
		while (!data->state_locked) {
			...
			if (cur_state == new_state) {
				/* Reset events that have been processed */
				events = 0;
				break;
			}
...
void ap_pwrseq_state_unlock(const struct device *dev)
{
	...
	if (data->state_locked) {
		data->state_locked = false;
		/*
		 * This will wake up driver thread in case there are events
		 * ready to be processed by state machine.
		 */
		k_event_post(&data->evt, AP_PWRSEQ_EVENT_STATE_UNLOCK);
	}
