...
ap_pwrseq_sm_init_state __weak init_state_func = NULL;
...
int ap_pwrseq_sm_init(void *const data, k_tid_t tid)
{
	...
	if (init_state_func) {
		state = init_state_func(data);
	}
	if (state >= AP_POWER_STATE_COUNT) {
		return -EINVAL;
	}

	k_mutex_init(&sm_data->mux);
...
{
	struct ap_pwrseq_sm_data *sm_data = data;
	enum ap_pwrseq_state ret = AP_POWER_STATE_UNDEF;

	k_mutex_lock(&sm_data->mux, K_FOREVER);

	if (sm_data->smf.current != NULL) {
		ret = ((struct ap_pwrseq_smf *)sm_data->smf.current)->state;
	}
