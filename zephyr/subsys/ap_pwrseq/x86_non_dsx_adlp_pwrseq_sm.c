 ...
 * This will be overriden if the custom signal handler is needed
 */
...
int generate_pch_pwrok_handler()
{
	int pch_pok;
...
*/
	...
	/* TODO: Check board specific reason. Turn off RMSRST_L  to meet tPCH12 */
...
/*	board_after_rsmrst(0);			*/
	...
	while (intel_x86_get_pg_ec_dsw_pwrok() &&
	       gpio_get_lvl(GPIO_NET_NAME(VR_PG_EC_RSMRST_ODL)) && (timeout_ms > 0)) {
		k_msleep(1);
		timeout_ms--;
	};
...
{
	shell_fprintf(shell, SHELL_INFO, "Hello this message is for test\n");
