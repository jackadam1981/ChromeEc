...
#define INIT_ESPI_SIGNAL(id)						\
	COND_CODE_1(DT_NODE_HAS_COMPAT(id, intel_ap_pwrseq_vw),		\
	({								\
...
struct vw_config {
	...
	uint8_t invert;		/* Invert the signal value */
};
...
DT_FOREACH_CHILD(DT_COMPAT_GET_ANY_STATUS_OKAY(intel_ap_pwrseq),
	INIT_ESPI_SIGNAL)
/*
 * If there are no VW signals, the compiler does not like an empty array.
 */
{
}
};
...
{
	int value = vw_get_level(vw_config[vw].espi_signal);
