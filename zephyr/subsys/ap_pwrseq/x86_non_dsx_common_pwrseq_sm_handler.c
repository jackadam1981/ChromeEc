...
void s5_inactive_timer_fn(struct k_timer *timer)
{
	ap_pwrseq_post_event(ap_pwrseq_get_instance(), AP_PWRSEQ_EVENT_POWER_TIMEOUT);
}
...
{
	ap_pwrseq_sm_set_state((struct ap_pwrseq_sm_data*) ap_pwrseq_get_instance(), AP_POWER_STATE_G3);
}
...
static void x86_non_dsx_g3_run(void *arg)
{
	...
	/* TODO-Start: This solution needs some further investigation */
