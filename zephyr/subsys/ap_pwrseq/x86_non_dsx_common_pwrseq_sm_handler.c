...
enum ap_pwrseq_state pwr_sm_get_state(void)
{
	...
	ap_pwrseq_get_current_state(ap_pwrseq_get_instance(), &state);
	return state;
}
...
static void x86_non_dsx_g3_entry(void *data)
{
	ap_power_ev_send_callbacks(AP_POWER_SHUTDOWN_COMPLETE);
	if (atomic_test_bit(flags, START_FROM_G3)) {
		ap_pwrseq_post_event(data,
				     AP_PWRSEQ_EVENT_POWER_STARTUP);
	}
}

static void x86_non_dsx_g3_run(void *data)
{
	/*
	...
	/* Ready to do state transition, check if signal is OK  */
...
static void x86_non_dsx_s5_run(void *data)
{
	if (IS_EVENT_SET(data, AP_PWRSEQ_EVENT_POWER_SHUTDOWN)) {
...
static void x86_non_dsx_s4_run(void *data)
{
	if (IS_EVENT_SET(data, AP_PWRSEQ_EVENT_POWER_SHUTDOWN)) {
		ap_pwrseq_sm_set_state(data, AP_POWER_STATE_G3);
		return;
...
static void x86_non_dsx_s3_run(void *data)
{
	if (IS_EVENT_SET(data, AP_PWRSEQ_EVENT_POWER_SHUTDOWN)) {
		ap_pwrseq_sm_set_state(data, AP_POWER_STATE_G3);
		return;
...
static void x86_non_dsx_s0_run(void *data)
{
	if (IS_EVENT_SET(data, AP_PWRSEQ_EVENT_POWER_SHUTDOWN)) {
		ap_pwrseq_sm_set_state(data, AP_POWER_STATE_G3);
		return;
	...
	if (signals_valid_and_on(IN_PCH_SLP_S3)) {
		...
		ap_pwrseq_sm_set_state(data, AP_POWER_STATE_S0IX);
	} else if (ap_power_sleep_get_notify() ==
...
static void x86_non_dsx_s0ix_entry(void *data)
{
	/*
...
static void x86_non_dsx_s0ix_run(void *data)
{
...
static void x86_non_dsx_s0ix_exit(void *data)
{
	/*
