			...
			uint32_t pin)
{
	...
	power_signal = cb - &intr_callbacks[0];
	if (power_signal >= POWER_SIGNAL_GPIO_COUNT)
		return;
...
static void pwrseq_gpio_init(void)
{
	...
	for (i = 0; i < POWER_SIGNAL_COUNT; i++) {
		const struct power_signal_gpio_config *int_config = NULL;
