...
int power_signal_disable_interrupt(enum power_signal signal)
{
	return gpio_pin_interrupt_configure_dt(
			&power_signal_list[signal].gpio_config.spec,
			GPIO_INT_DISABLE);
}
...
int power_signal_enable_interrupt(enum power_signal signal)
{
	return gpio_pin_interrupt_configure_dt(
			&power_signal_list[signal].gpio_config.spec,
			power_signal_list[signal].gpio_config.intr_flags);
}
...
__attribute__((weak)) int power_signal_is_asserted(enum power_signal signal)
{
	...
	if (power_signal_list[signal].source == SOURCE_GPIO)
		...
		return vw_get_level(power_signal_list[signal].vw_signal) ==
			!!(flags & POWER_SIGNAL_ACTIVE_STATE);
...
void power_signal_interrupt(const struct device *gpiodev,
			...
			uint32_t pin)
{
	struct power_signal_info *info;

	info = CONTAINER_OF(cb, struct power_signal_info, gpio_config.intr_cb);
	if (info->power_sig >= POWER_SIGNAL_COUNT)
		return;
...
static void pwrseq_gpio_init(void)
{
	...
	for (i = 0; i < POWER_SIGNAL_COUNT; i++) {
		struct power_signal_gpio_config *int_config = NULL;

		if (power_signal_list[i].source != SOURCE_GPIO)
			continue;

		int_config = &power_signal_list[i].gpio_config;
