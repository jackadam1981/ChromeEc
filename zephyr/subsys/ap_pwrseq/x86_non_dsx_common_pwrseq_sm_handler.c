...
__attribute__((weak)) int power_signal_is_asserted_override(
	enum power_signal signal)
{
	return 0;
}
...
int power_signal_is_asserted(enum power_signal signal)
{
	...
	flags = power_signal_list[signal].flags;
	...
	if (power_signal_list[signal].source == SOURCE_GPIO) {
		...
		/*
		 * If signal is not generated by GPIO or VW, check it in
		 * override function
		 */
		return power_signal_is_asserted_override(signal);
	}
}
...
{
	int i;
	struct power_signal_gpio_config *intr_config = NULL;

	for (i = 0; i < POWER_SIGNAL_COUNT; i++) {
		if (power_signal_list[i].source == SOURCE_GPIO) {
			intr_config = &power_signal_list[i].gpio_config;
			if (gpiodev == intr_config->spec.port &&
				pin == BIT(intr_config->spec.pin)) {
