...
static int check_power_rails_enabled()
{
	int out = 1;
               ...
               uint32_t pin)
{
	...
	if (!intr_config) {
		...
		LOG_ERR("intr %d: gpio %s(%d)", i,
			intr_config->config->net_name, gpio_get_lvl(intr_config->config->net_name)
			);
		}
	}
...
void pwr_sm_set_state(enum power_states_ndsx new_state)
{
	...
	LOG_DBG("Power state: %s --> %s\n",pwrsm_dbg[power_state], pwrsm_dbg[new_state]);
...
__attribute__((weak)) void rsmrst_pass_thru_handler()
{
  ...
  */
	...
	case SYS_POWER_STATE_G3S5:
		...
		if(check_pch_out_of_suspend()) {
			...
			/* Start timer to monitor S5 inactive timeout */
	...
	case SYS_POWER_STATE_S3:
		...
		} else if (power_signal_get_level(IN_PCH_SLP_S3_DEASSERTED) == 1) {
			/* Power up to next state */
			return SYS_POWER_STATE_S3S0;
		} else if (power_signal_get_level(IN_PCH_SLP_S4_DEASSERTED) == 0) {
	...
	case SYS_POWER_STATE_S3S0:
		...
		if (power_wait_signals(X86_ALL_SYS_PGOOD))
			break;
	...
	case SYS_POWER_STATE_S0:
		...
		} else if (power_signal_get_level(IN_PCH_SLP_S3_DEASSERTED) == 0) {
			/* Power down to next state */
			return SYS_POWER_STATE_S0S3;
		}
		/* TODO: S0ix */
...
static inline void create_pwrseq_thread()
{
	/* TODO: Configure parameters for delay to the thread */
	pwrseq_thread_id = k_thread_create(&pwrseq_thread_data, pwrseq_thread_stack,
