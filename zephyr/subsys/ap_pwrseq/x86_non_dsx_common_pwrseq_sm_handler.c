...
__attribute__((weak)) int power_signal_gpio_is_asserted(const struct power_signal_info *s)
{
...
static int common_pwr_sm_run(int state)
{
	...
	case SYS_POWER_STATE_S5:
		...
		if (check_power_rails_enabled() && check_rsmrst_ok()) {
			...
			if (!power_signal_is_asserted(X86_SLP_S5)) {
				k_timer_stop(&s5_inactive_timer);
	...
	case SYS_POWER_STATE_S4:
		...
		else if (!power_signal_is_asserted(X86_SLP_S4))
	...
	case SYS_POWER_STATE_S3:
		...
		if (!power_has_signals(IN_PGOOD_ALL_CORE)) {
			...
			chipset_force_shutdown(CHIPSET_SHUTDOWN_POWERFAIL);
			return SYS_POWER_STATE_G3;
		} else if (!power_signal_is_asserted(X86_SLP_S3))
