...
static int x86_non_dsx_chipset_register_events(void)
{
	...
	ap_pwrseq_register_state_entry_callback(dev, &ap_pwrseq_entry_cb);

	ap_pwrseq_exit_cb.cb = x86_non_dsx_chipset_state_exit_cb;
	ap_pwrseq_exit_cb.states_bit_mask =
		(BIT(AP_POWER_STATE_G3) | BIT(AP_POWER_STATE_S5) |
		 BIT(AP_POWER_STATE_S4) | BIT(AP_POWER_STATE_S3) |
		 BIT(AP_POWER_STATE_S0)
#if CONFIG_AP_PWRSEQ_S0IX
		 | BIT(AP_POWER_STATE_S0IX)
#endif
	...
	ap_pwrseq_register_state_exit_callback(dev, &ap_pwrseq_exit_cb);

	ap_power_ev_send_callbacks(AP_POWER_INITIALIZED);
	return 0;
}

enum ap_pwrseq_state ap_pwrseq_board_sm_init(void *data)
{
	power_signal_mask_t sig;

	power_signal_init();

	x86_non_dsx_chipset_register_events();
...
AP_POWER_INIT_STATE_FUNC(ap_pwrseq_board_sm_init);
