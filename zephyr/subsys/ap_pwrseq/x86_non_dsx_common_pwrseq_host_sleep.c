...
#ifdef CONFIG_SLPS0_DEBUG
#include "util.h"
static void slp_s0_debug_alarm(struct k_work *work)
{
	/* Wake up host by lid open */
	host_set_single_event(EC_HOST_EVENT_LID_OPEN);
}
static K_WORK_DELAYABLE_DEFINE(slp_s0_debug_alarm_data, slp_s0_debug_alarm);

static enum ec_status host_command_slp_s0_debug_alarm(struct host_cmd_handler_args *args)
{
	const struct ec_params_rtc *p = args->params;
	struct k_work_sync work_sync;

	if (p->time < 1)
		k_work_cancel_delayable_sync(&slp_s0_debug_alarm_data, &work_sync);
	else
		k_work_schedule(&slp_s0_debug_alarm_data, K_SECONDS(p->time));

	return EC_RES_SUCCESS;
}
DECLARE_HOST_COMMAND(EC_CMD_RTC_SET_ALARM, host_command_slp_s0_debug_alarm,
		     EC_VER_MASK(0));

/**
 * Test the RTC alarm by setting an interrupt on RTC match.
 */
static int console_command_slp_s0_debug_alarm(int argc, const char **argv)
{
	int s = 1;
	char *e;
	struct k_work_sync work_sync;
...
DECLARE_CONSOLE_COMMAND(slp_s0_debug_alarm, console_command_slp_s0_debug_alarm, "[seconds]",
			"EC alarm");
#endif
...
#ifdef CONFIG_SLPS0_DEBUG
	ccprintf("Detected sleep hang cancle the slp_s0_debug_alarm\n");
	k_work_cancel_delayable(&slp_s0_debug_alarm_data);
#endif /* CONFIG_SLPS0_DEBUG */
