...
static struct gpio_callback int_cb[ARRAY_SIZE(int_config)];

int power_signal_gpio_enable_int(uint8_t index)
{
	gpio_flags_t flags;

	if (index >= ARRAY_SIZE(int_config)) {
		return -EINVAL;
	}
	flags = int_config[index].flags;
...
int power_signal_gpio_disable_int(uint8_t index)
{
	gpio_flags_t flags;
...
int power_signal_gpio_get(uint8_t index)
{
	return gpio_pin_get_dt(&spec[index]);
}

int power_signal_gpio_set(uint8_t index, int value)
{
