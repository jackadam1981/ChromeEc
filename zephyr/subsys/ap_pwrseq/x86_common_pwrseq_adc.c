...
#define X86_COMMON_ADC_INSTS_WITH_COMMA(node_id)                              \
	[DT_STRING_UPPER_TOKEN(node_id, pwrseq_signal_enum)] =                \
	&x86_adc_##node_id,
...
{
	if (signal >= POWER_SIGNAL_ADC_COUNT) {
		return 0;
	}
	return !!adc_inst[signal]->value;
}
