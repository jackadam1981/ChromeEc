...
#define NPCX_MTC_ALARM_MASK (BIT(25) - 1) /* Valid field of alarm in WTC */
...
/* Counter inline local functions */
static inline uint32_t counter_npcx_get_val(const struct device *dev)
{
...
static inline void counter_npcx_set_val(const struct device *dev, uint32_t val)
{
	struct mtc_reg *const inst = HAL_INSTANCE(dev);
...
static inline uint32_t counter_npcx_get_alarm_val(const struct device *dev)
{
	struct mtc_reg *const inst = HAL_INSTANCE(dev);
...
static inline void counter_npcx_set_alarm_val(const struct device *dev,
								uint32_t val)
{
	struct mtc_reg *const inst = HAL_INSTANCE(dev);
...
static inline void counter_npcx_reset_alarm(const struct device *dev)
{
	struct mtc_reg *const inst = HAL_INSTANCE(dev);
...
static int cros_rtc_npcx_get_alarm(const struct device *dev,
...
static int cros_rtc_npcx_set_alarm(const struct device *dev,
				uint32_t seconds, uint32_t microseconds)
{
...
static int cros_rtc_npcx_init(const struct device *dev)
{
	 ...
	 * edge on its signal.
	 */
	npcx_miwu_interrupt_configure(&config->mtc_alarm,
...
DEVICE_DT_INST_DEFINE(0, cros_rtc_npcx_init, device_pm_control_nop,
			&cros_rtc_npcx_data_0, &cros_rtc_npcx_cfg_0,
			POST_KERNEL,
