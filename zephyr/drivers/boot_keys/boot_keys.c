...
#define BOOT_KEY_INIT(prop)                                 \
	{                                                   \
		.row = (DT_INST_PROP(0, prop) >> 8) & 0xff, \
		.col = DT_INST_PROP(0, prop) & 0xff,        \
	}
...
{
#ifdef CONFIG_BOOT_KEYS_COL2_WORKAROUND
	if (row == 2) {
		...
		LOG_DBG("boot_keys: ignoring row=%d col=%d\n", row, col);
		return true;
	}
#endif
...
static void process_key(uint8_t row, uint8_t col, bool pressed)
{
	...
	LOG_DBG("boot_keys: boot_keys_value=%x counter=%d (set row=%d col=%d)",
...
static void power_button_change(void)
{
	...
	LOG_DBG("boot_keys: boot_keys_value=%x counter=%d (power)",
...
static void boot_keys_timeout_handler(struct k_work *work)
{
	...
	LOG_INF("boot_keys: boot_keys_value=%08x", boot_keys_value);
