...
static uint32_t cros_flash_andes_xip_get_protect_flags(const struct device *dev)
{
	 ...
	 * write protecion is enabled. QSPI mode is required to speed up the XIP
	 * mode. That means the WP pin is not used for protection.
	 */
	...
	if (prot_start <= CONFIG_WP_STORAGE_OFF) {
		/* This logic assumes a certain flash layout: RO -> ROLLBACKS ->
		 * RW. */
						...
						uint32_t new_flags)
{
	 ...
	 * is within pretection ranges of Rollbacks */
	...
	/* Just exit if the protection rage matches new values. */
	if ((curr_prot_start == 0) && (curr_prot_end == new_prot_end)) {
		goto unlock_irq;
	}

	if (status_locked) {
		/* Allow modifing status registers in case deasserted WP, but
	...
	/* Alway restore status of the status register lock in case .*/
	if (status_locked) {
		int ret2;
		ret2 = flash_andes_xip_lock_status(dev, true);
		if (ret2) {
			LOG_ERR("Failed to restore flash status lock");
...
static int check_modify_status(const struct device *dev)
{
	int ret = 0;
...
BUILD_ASSERT(CONFIG_FLASH_ANDES_QSPI_INIT_PRIORITY <
	     CONFIG_CROS_FLASH_ANDES_XIP_INIT_PRIORITY);
