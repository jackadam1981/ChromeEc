...
static int cros_flash_andes_xip_init(const struct device *dev)
{
	...
	if (!system_is_in_rw()) {
		 ...
		 * erase/write cycles which is e.g. 100k for Puya P25Q16, so it
		 * shouldn't be an issue. There is a possibility of volatile
		 * status register write, but it is not supported by all flash
		 * NOR chips. Additionally, the cros_flash driver wouldn't know
		 * when the volatile status register write can be used or not.
		 */
		if (flags & EC_FLASH_PROTECT_ALL_AT_BOOT) {
			ret = cros_flash_andes_xip_protect_at_boot(
				dev, EC_FLASH_PROTECT_RO_AT_BOOT);
