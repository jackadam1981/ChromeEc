				      ...
				      uint32_t new_flags)
{
	struct cros_flash_data *const data = DRV_DATA(dev);
	int err, first_err = 0;
	uint32_t range_sectors, enable_mask = 0, disable_mask = 0;
	...
	if (write_protect_is_asserted()) {
		if (new_flags & EC_FLASH_PROTECT_ALL_AT_BOOT) {
			first_err = cros_flash_sector_mask_from_offset(
				flash_controller, 0, FLASH_SIZE, &enable_mask);
		} else {
...
static bool wp_all_disabled(const struct device *dev)
{
	...
	if (flash_get_wp(flash_controller, &protected_mask)) {
		return false;
	}
...
static int cros_flash_disable_wp_all(const struct device *dev)
{
	...
	if (err == 0) {
		err = flash_change_wp(flash_controller, disable_mask, 0);
	}
...
static int cros_flash_get_protect(const struct device *dev, int bank)
{
	...
	return (protected_mask & (1UL << bank)) ? 1 : 0;
}
...
static int disable_option_bytes(const struct device *dev)
{
	...
	ret = flash_disable_option_bytes(flash_controller);
	if (ret)
		return ret;
...
static int disable_control_register(const struct device *dev)
{
	...
	ret = flash_disable_control_register(flash_controller);
	if (ret)
		return ret;
...
static int cros_flash_protect_now(const struct device *dev, int all)
{
	...
	first_err = disable_option_bytes(dev);

	if (all) {
		err = disable_control_register(dev);
		if (err && first_err == EC_SUCCESS)
			first_err = err;
	}
...
static void cros_flash_restore_state(const struct device *dev,
				     uint32_t prot_flags)
{
	struct cros_flash_data *const data = DRV_DATA(dev);
	...
	if (decode_wp_from_sysjump(&data->protection, prot_flags, prev, size,
				   version) == EC_SUCCESS)
		return;
#else
	if (version == CROS_FLASH_PROT_VERSION &&
	    size == sizeof(struct cros_flash_protection)) {
		const struct cros_flash_protection *prev_prot =
			(const struct cros_flash_protection *)prev;
...
static int cros_flash_driver_init(const struct device *dev)
{
	...
	data->protection.access_disabled = false;
	data->protection.option_disabled = false;
	 ...
	 */
	if (reset_flags & EC_RESET_FLAG_SYSJUMP) {
		cros_flash_restore_state(dev, prot_flags);
		return EC_SUCCESS;
	}
