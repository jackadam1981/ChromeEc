...
void sync_wp_assert_status(void)
{
	...
	gpio_pin_set_dt(&wp_config->wp_gpios[0],
			gpio_pin_get_dt(&wp_config->ex_wp_gpios[0]));
}

void enable_mchp_wp(void)
{
	gpio_pin_set_dt(&wp_config->wp_gpios[0], 0);
}

int get_mchp_wp_gpio(void)
{
	return gpio_pin_get_dt(&wp_config->wp_gpios[0]);
}
...
static const struct mchp_wp_config mchp_wp_cfg = {
	.wp_gpios =
		(const struct gpio_dt_spec[]){ FOREACH_MCHP_GPIO(0, wp_gpios) },
	.ex_wp_gpios = (const struct gpio_dt_spec[]){ FOREACH_MCHP_GPIO(
		0, wp_ex_gpios) },
