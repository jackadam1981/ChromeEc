 ...
 *
 * This restoration is crucial when transitioning from a legacy Cros-EC RO
 ...
 * SR2, and all_protected and the new Zephyr-EC RW image must restore these to
 * understand the true protection status. Zephyr itself doesn't use UMA_LOCK,
 * but needs to honor the state set by the legacy CROS-EC RO.
 *
 * @retval true The system was reset via a sysjump and the flash WP state has
 * been restored.
 * @retval false The system performed a standard boot (no sysjump
 * detected).Nothing should be done.
 */
bool flash_physical_restore_state(void)
{
	...
	if (reset_flags & EC_RESET_FLAG_SYSJUMP) {
		...
		if (prev && version == FLASH_HOOK_VERSION &&
		    size == sizeof(*prev)) {
			/*
			 * This restoration is crucial when transitioning from a
			 * legacy Cros-EC RO image. Such images used UMA_LOCK,
			 * preventing direct reads of the status registers after
			 * protection was enabled. The RO image would have saved
			 * SR1, SR2, and all_protected and the new Zephyr-EC RW
			 * image must restore these to understand the true
			 * protection status. Zephyr itself doesn't use
			 * UMA_LOCK, but needs to honor the state set by the
			 * legacy CROS-EC RO.
			 */
