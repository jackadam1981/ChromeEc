...
static int cros_flash_get_status_reg(const struct device *dev,
...
static int cros_flash_set_status_reg(const struct device *dev, char *data)
{
	int ret = 0;
	struct fiu_reg *const inst = HAL_INSTANCE(dev);
...
static int cros_flash_write_protection_set(const struct device *dev,
						bool enable)
{
	int ret = 0;
...
static int cros_flash_write_protection_is_set(const struct device *dev)
{
	return npcx_pinctrl_flash_write_protect_is_set();
}

static int cros_flash_uma_lock(const struct device *dev, bool enable)
{
	struct fiu_reg *const inst = HAL_INSTANCE(dev);
...
static int flash_get_status1(const struct device *dev)
{
	...
	cros_flash_get_status_reg(dev, CMD_READ_STATUS_REG, &reg);
...
static int flash_get_status2(const struct device *dev)
{
	...
	cros_flash_get_status_reg(dev, CMD_READ_STATUS_REG2, &reg);
...
{
	return cros_flash_set_status_reg(dev, data);
}

static int is_int_flash_protected(const struct device *dev)
{
	return cros_flash_write_protection_is_set(dev);
}
...
static void flash_protect_int_flash(const struct device *dev, int enable)
{
	 ...
	 */
	if (enable)
		cros_flash_write_protection_set(dev, enable);
}
...
static void flash_uma_lock(const struct device *dev, int enable)
{
	...
	cros_flash_uma_lock(dev, enable);
