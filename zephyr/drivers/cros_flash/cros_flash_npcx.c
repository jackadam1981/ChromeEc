...
static int cros_flash_npcx_set_write_enable(const struct device *dev)
{
	...
	ret = spi_transceive(data->spi_ctrl_dev, &spi_cfg, &tx_set, 0);
	if (ret != 0)
		return ret;
...
static int cros_flash_npcx_set_status_reg(const struct device *dev, char *data)
{
	...
	ret = spi_transceive(dev_data->spi_ctrl_dev, &spi_cfg, &tx_set, 0);
	if (ret != 0)
		return ret;
...
#error "flash_npcx_init must be initialized after spi_not_init"
#endif
