...
#define DRV_CONFIG(dev) ((const struct cros_flash_em32f967_config *)(dev)->config)
...
{
	LOG_INF("cros_flash_em32f967_init.");

	return 0;
}

static int cros_flash_em32f967_write(const struct device *dev, int offset, int size,
				 const char *src_data)
{
	const struct cros_flash_em32f967_config *cfg = DRV_CONFIG(dev);
	int ret = 0;

	LOG_INF("cros_flash_em32f967_write.");

	/* Lock physical flash operations */
	crec_flash_lock_mapped_storage(1);

	LOG_INF("cros_flash_em32f967_write 0x%x 0x%x.", offset, size);
...
static int cros_flash_em32f967_erase(const struct device *dev, int offset, int size)
{
	const struct cros_flash_em32f967_config *cfg = DRV_CONFIG(dev);
	int ret = 0;

	LOG_INF("cros_flash_em32f967_erase.");
	...
	for (; size > 0; size -= CONFIG_FLASH_ERASE_SIZE) {
		LOG_INF("cros_flash_em32f967_erase 0x%x 0x%x.", offset,
...
{
	// LOG_INF("cros_flash_em32f967_get_protect.");
	return 0;
}

static uint32_t cros_flash_em32f967_get_protect_flags(const struct device *dev)
{
	// LOG_INF("cros_flash_em32f967_get_protect_flags.");
	return 0;
}
...
{
	LOG_INF("cros_flash_em32f967_protect_now.");
	return 0;
}

static int cros_flash_em32f967_get_jedec_id(const struct device *dev,
					uint8_t *manufacturer, uint16_t *device)
{
	LOG_INF("cros_flash_em32f967_get_jedec_id.");
	return 0;
}

static int cros_flash_em32f967_get_status(const struct device *dev, uint8_t *sr1,
				      uint8_t *sr2)
{
	LOG_INF("cros_flash_em32f967_get_status.");
	return 0;
}
...
static DEVICE_API(cros_flash, cros_flash_em32f967_driver_api) = {
	...
	.physical_get_jedec_id = cros_flash_em32f967_get_jedec_id,
	.physical_get_status = cros_flash_em32f967_get_status,
};
