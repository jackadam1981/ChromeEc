...
int flash_disable_option_bytes(const struct device *dev)
{
	return flash_ex_op(dev, FLASH_STM32_EX_OP_BLOCK_OPTION_REG,
			   (uintptr_t)NULL, NULL);
}

int flash_disable_control_register(const struct device *dev)
{
			   ...
			   size_t size, int version)
{
	...
	if (size != sizeof(*wp) || version != CROS_FLASH_STM32_PROT_VERSION)
		return -EINVAL;
