...
/* AT_BOOT flags for the next boot */
...
static void lock_rw_rb(void)
{
#ifdef CONFIG_FLASH_PROTECT_RW
	bool need_reset = false;
	uint8_t bbram_flags = 0;
	bool lock_rw, lock_rb;

	/* Failed to read bbram, assume all region should be locked */
	if (read_bbram_flags(&bbram_flags)) {
		LOG_ERR("read_bbram_flags failed");
		bbram_flags = 0;
	}

	lock_rw = !(bbram_flags & IT8XXX2_UNLOCK_RW_AT_BOOT);
	lock_rb = !(bbram_flags & IT8XXX2_UNLOCK_ROLLBACK_AT_BOOT);
	...
	/* TODO: check if RW/RB locked state is correct */
	if (need_reset) {
