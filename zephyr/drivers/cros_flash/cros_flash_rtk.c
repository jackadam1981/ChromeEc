...
#define BBRAM_WP_FLAG_UNVALID 0xFF
					  ...
					  uint32_t new_flags)
{
	...
	LOG_ERR("set AT_BOOT = %x", lock_flags);
				     ...
				     uint8_t *sr2)
{
	...
	if (read_bbram_flags(&lock_flags)) {
		...
		lock_flags = BBRAM_WP_FLAG_UNVALID;
	}

	LOG_ERR("got AT_BOOT = %x, WP = %d", lock_flags,
		is_int_flash_protected(dev));
...
static int cros_flash_rtk_init(const struct device *dev)
{
	...
	LOG_ERR("got AT_BOOT = %x", lock_flags);
	...
	/* handle the *_AT_BOOT */
	if (lock_flags != BBRAM_WP_FLAG_UNVALID) {
