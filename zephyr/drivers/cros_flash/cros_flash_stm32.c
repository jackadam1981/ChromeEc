...
#include <zephyr/drivers/flash/flash_stm32_ex_op.h>
	...
	if (new_flags & EC_FLASH_PROTECT_ALL_AT_BOOT) {
		...
		if (err == 0) {
			if (new_flags & EC_FLASH_PROTECT_RW_AT_BOOT)
				;
			wp_request.enable_mask |= range_sectors;
			else wp_request.disable_mask |= range_sectors;
		} else if (first_err == 0) {
			first_err = err;
		}
#endif
...
static int cros_flash_stm32_get_protect(const struct device *dev, int bank)
{
	...
	if (flash_ex_op(flash_controller, FLASH_STM32_EX_OP_SECTOR_WP, NULL,
			(uintptr_t *)&wp_status)) {
		return 0;
	}
...
#if defined(CONFIG_CROS_FLASH_STM32_READOUT_PROTECTION)
	if (flash_ex_op(flash_controller, FLASH_STM32_EX_OP_RDP, NULL,
			(uintptr_t *)&rdp_status))
...
static int registers_need_reset(const struct device *dev)
{
	...
	err = flash_ex_op(flash_controller, FLASH_STM32_EX_OP_SECTOR_WP, NULL,
			  (uintptr_t *)&wp_status);
	if (err) {
