...
static int cros_flash_control_write_protect(const struct device *dev,
	off_t offset, size_t size, bool enable)
{
	int err;

	err = flash_set_write_protect(dev, offset, size, enable);
	if (err) {
		LOG_ERR("Can't set flash WP for offset 0x%x size 0x%x to %d, "
			"error: %d", enable, 0, FLASH_SIZE, err);
	}
					      ...
					      uint32_t new_flags)
{
	...
	if (new_flags & EC_FLASH_PROTECT_ALL_AT_BOOT) {
		err = cros_flash_control_write_protect(flash_controller, 0, FLASH_SIZE,
						       true);
		if (err != 0 && first_err == 0)
			first_err = err;
	} else {
		err = cros_flash_control_write_protect(flash_controller,
	...
	if (new_flags & EC_FLASH_PROTECT_RO_AT_BOOT) {
		err = flash_set_readout_protection(flash_controller, 0, FLASH_SIZE, true,
						   false);
		if (err != 0) {
...
static int cros_flash_stm32_get_protect(const struct device *dev, int bank)
{
	...
	if (flash_get_page_info_by_idx(flash_controller, bank, &info)) {
		return 0;
	}

	if (flash_get_write_protect(flash_controller, info.start_offset, info.size,
	    &enabled)) {
	    	return 0;
	}
...
static uint32_t cros_flash_stm32_get_protect_flags(const struct device *dev)
{
	struct cros_flash_stm32_data *const data = DRV_DATA(dev);
	uint32_t flags = 0;
	bool rdp_enabled, rdp_permanent;
...
#if defined(CONFIG_CROS_FLASH_STM32_READOUT_PROTECTION)
	if (flash_get_readout_protection(flash_controller, 0, FLASH_SIZE, &rdp_enabled, &rdp_permanent))
...
static int cros_flash_stm32_protect_now(const struct device *dev, int all)
{
	...
	if (all) {
		//disable_flash_control_register(dev);
		data->all_protected = true;

		return EC_SUCCESS;
	}

	//disable_flash_option_bytes(dev);
...
{
	bool ro_at_boot = (crec_flash_get_protect() &
	    EC_FLASH_PROTECT_RO_AT_BOOT) ? true : false;
	bool enabled;
	int err;

	err = flash_get_write_protect(flash_controller, CONFIG_WP_STORAGE_OFF,
	    CONFIG_WP_STORAGE_SIZE, &enabled);
	if (err) {
		LOG_ERR("Can't get flash WP status for offset 0x%x size 0x%x, "
			"error: %d", 0, FLASH_SIZE, err);
		return err;
	}
...
static int cros_flash_stm32_init(const struct device *dev)
{
	...
	if (prot_flags & EC_FLASH_PROTECT_GPIO_ASSERTED) {
		...
		if ((prot_flags & EC_FLASH_PROTECT_RO_AT_BOOT) &&
			 ...
			 * it takes effect.
			 */
			cros_flash_stm32_protect_at_boot(dev,
		...
		if (prot_flags & EC_FLASH_PROTECT_RO_NOW) {
			 ...
			 */
			if (cros_flash_control_write_protect(flash_controller,
			    0, FLASH_SIZE, false) == 0)
				need_reset = 1;
		}
	}
...
#if CONFIG_CROS_FLASH_STM32_INIT_PRIORITY <= \
