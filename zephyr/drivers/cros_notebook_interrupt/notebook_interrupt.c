...
LOG_MODULE_REGISTER(notebook_interrupt, CONFIG_LOG_DEFAULT_LEVEL);

#define DT_DRV_COMPAT cros_notebook_interrupt
...
static int pin_state_at_interrupt;
...
static void interrupt_handler(struct k_work *work)
{
	...
	tablet_set_mode(pin_state_at_interrupt, TABLET_TRIGGER_LID);
			       ...
			       struct gpio_callback *cb, uint32_t pins)
{
	...
	pin_state_at_interrupt = gpio_pin_get_dt(&interrupt_spec) == 0 ? 0 : 1;
	k_work_submit(&interrupt_work);
}

static int init_notebook_mode_interrupt(void)
{
	int ret;

	if (!gpio_is_ready_dt(&interrupt_spec)) {
		LOG_ERR("gpio port %s not read", interrupt_spec.port->name);
		return -EINVAL;
	}
