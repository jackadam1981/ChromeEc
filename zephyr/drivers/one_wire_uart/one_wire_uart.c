...
void one_wire_uart_send(const struct device *dev, uint8_t cmd,
...
/* process rx queue in non-irq context */
static void process_packet(void)
{
	struct RoachMessage msg;
	static int last_msg_id = -1;
	const struct device *device = DEVICE_DT_GET(DT_DRV_INST(0));
	struct one_wire_uart_data *data = device->data;

	while (k_msgq_get(&rx_queue, &msg, K_NO_WAIT) == 0) {
		if (last_msg_id != msg.header.msg_id && data->msg_received_cb) {
			data->msg_received_cb(msg.header.cmd, msg.payload, msg.header.payload_len);
		}
