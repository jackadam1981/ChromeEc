...
static void recv_cb(uint8_t cmd, const uint8_t *payload, int length)
{
	...
	if (cmd == ROACH_CMD_TP_PASSTHRU) {
		static uint8_t writebuf[CONFIG_I2C_CHIP_MAX_TRANSFER_SIZE + 2] = {};
		static int writebuf_count = 0;

		bool is_last_chunk;
		uint8_t readbuf[CONFIG_I2C_CHIP_MAX_TRANSFER_SIZE] = {};
		int readcount;
		int rv;
		...
		readcount = MIN(readcount, CONFIG_I2C_CHIP_MAX_TRANSFER_SIZE);

		rv = i2c_xfer(CONFIG_TOUCHPAD_I2C_PORT, CONFIG_TOUCHPAD_I2C_ADDR_FLAGS,
			writebuf + 2, writebuf_count - 2, readbuf, readcount);
		if (!rv) {
			one_wire_uart_send(one_wire_uart, ROACH_CMD_TP_PASSTHRU, readbuf, readcount);
		}
		writebuf_count = 0;
	}
}
