 ...
 */
static void get_led_brightness_max(int handler, uint8_t *br)
{
	switch (handlers[handler].h_type) {
	default:
		__ASSERT(false, "Unknown handler");
...
#if DT_HAS_COMPAT_STATUS_OKAY(COMPAT_GPIO)
	case LED_HANDLER_GPIO:
		gpio_get_led_brightness_max(handlers[handler].h_index, br);
		break;
#endif
#if DT_HAS_COMPAT_STATUS_OKAY(COMPAT_PWM)
	case LED_HANDLER_PWM:
		pwm_get_led_brightness_max(handlers[handler].h_index, br);
		break;
#endif
	}
}
...
#if DT_HAS_COMPAT_STATUS_OKAY(COMPAT_GPIO)
	DT_FOREACH_STATUS_OKAY(COMPAT_GPIO, GEN_LABEL_ENUM)
#endif
#if DT_HAS_COMPAT_STATUS_OKAY(COMPAT_PWM)
...
void led_get_brightness_range(enum ec_led_id led_id, uint8_t *brightness_range)
{
	...
	if (hand >= 0) {
		get_led_brightness_max(hand, brightness_range);
	}
}
