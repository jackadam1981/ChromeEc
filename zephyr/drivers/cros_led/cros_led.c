...
static void cros_led_ec_set_color_battery(const struct device *dev, enum led_color color)
{
    ...
    case COACHZ:
#if DT_NODE_EXISTS(DT_PATH(named_gpios, ec_chg_led_y_c0))
        gpio_set_level(GPIO_EC_CHG_LED_Y_C0,
		    (color == LED_AMBER) ? BAT_LED_ON : BAT_LED_OFF);
#endif

#if DT_NODE_EXISTS(DT_PATH(named_gpios, ec_chg_led_w_c0))
    ...
    case PAZQUEL:
#if DT_NODE_EXISTS(DT_PATH(named_gpios, ec_chg_led_y_c1))
        gpio_set_level(GPIO_EC_CHG_LED_Y_C1,
            (color == LED_RED) ? BAT_LED_ON : BAT_LED_OFF);
#endif

#if DT_NODE_EXISTS(DT_PATH(named_gpios, ec_chg_led_w_c1))
...
#if DT_NODE_EXISTS(DT_PATH(named_gpios, ec_chg_led_b_c1))
    ...
    case MRBLAND:
    case WORMDINGLER:
#if DT_NODE_EXISTS(DT_PATH(named_gpios, ec_chg_led_r_c0))
#if DT_NODE_EXISTS(DT_PATH(named_gpios, ec_chg_led_g_c0))
...
static int led_init(const struct device *dev)
{
    ...
    /* Board Name */
    data->board_name = DT_ENUM_TOKEN(DT_NODELABEL(current_board), current_board_name);

    /* Charge State */
    data->battery_led.charging_led = DT_ENUM_TOKEN(DT_NODELABEL(charge), led_color);

    /* Discharge state */
#if DT_NODE_HAS_PROP(DT_NODELABEL(power_state_discharge_s3), period) 
    data->battery_led.discharge_s0_led = DT_ENUM_TOKEN(DT_NODELABEL(discharge_s0), led_color);
    data->battery_led.discharge_s3_led_1 = DT_ENUM_TOKEN(DT_NODELABEL(discharge_s3), led_color_1);
    data->battery_led.discharge_s3_led_2 = DT_ENUM_TOKEN(DT_NODELABEL(discharge_s3), led_color_2);
    data->battery_led.discharge_s3_led_period = DT_ENUM_TOKEN(DT_NODELABEL(discharge_s3), period);
    data->battery_led.discharge_s5_led = DT_ENUM_TOKEN(DT_NODELABEL(discharge_s5), led_color);
#endif

    /* Error state */
    data->battery_led.error_led_1 = DT_ENUM_TOKEN(DT_NODELABEL(error), led_color_1);
    data->battery_led.error_led_2 = DT_ENUM_TOKEN(DT_NODELABEL(error), led_color_2);
    data->battery_led.error_led_period = DT_PROP(DT_NODELABEL(error),period);

    /* Near full state */
    data->battery_led.near_full_led = DT_ENUM_TOKEN(DT_NODELABEL(near_full), led_color);

    /* Idle state */
    data->battery_led.idle_led_1 = DT_ENUM_TOKEN(DT_NODELABEL(idle), led_color_1);
    data->battery_led.idle_led_2 = DT_ENUM_TOKEN(DT_NODELABEL(idle), led_color_2);
    data->battery_led.idle_led_3 = DT_ENUM_TOKEN(DT_NODELABEL(idle), led_color_3);
    data->battery_led.idle_led_period = DT_PROP(DT_NODELABEL(idle), period);


#if DT_NODE_EXISTS(DT_NODELABEL(led_id_power))
    data->battery_led.s0_led = DT_ENUM_TOKEN(DT_NODELABEL(power_state_s0), led_color);
    data->battery_led.s3_led_1 = DT_ENUM_TOKEN(DT_NODELABEL(power_state_s3), led_color_1);
    data->battery_led.s3_led_2 = DT_ENUM_TOKEN(DT_NODELABEL(power_state_s3), led_color_2);
    data->battery_led.s3_period = DT_PROP(DT_NODELABEL(power_state_s3), period);
    data->battery_led.s5_led = DT_ENUM_TOKEN(DT_NODELABEL(power_state_s5), led_color);
#endif

    data->led_control = DT_ENUM_TOKEN(DT_NODELABEL(led_control), led_color);
