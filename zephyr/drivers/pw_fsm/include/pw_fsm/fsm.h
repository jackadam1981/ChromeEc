...
constexpr void visitTransitionNodes(
  ...
  for (const auto& transition : transitions) {
    ...
    bool is_from_any =
        ...
        std::get<StateMatcher>(transition.from()) == StateMatcher::ANY;
...
class Transition {
 public:
   ...
   * may be of type StateMatcher::ANY which symbolizes that the transition can
...
class FsmConfig : public FsmConfigStorage<State, kNumTransitions> {
 public:
  ...
  constexpr FsmConfig(
      State starting_state,
      const std::array<Transition<State>, kNumTransitions>& transitions)
      : FsmConfigStorage<State, kNumTransitions>(starting_state, transitions) {}
...
/**
 * Helper function used to build an FsmConfig entry.
 *
 * Example:
 * @code
 * enum TestState { STATE1, STATE2 };
 * constexpr auto kTestFsmConfig = pw::fsm::MakeFsmConfig<TestState>(
 *   TestState::STATE1,
 *   std::to_array({
 *     pw::fsm::Transition<TestState>(STATE1, STATE2),
 *     pw::fsm::Transition<TestState>(STATE2, STATE1),
 *   })
 * );
 * @endcode
 *
 * @param starting_state The starting state of the state machine
 * @param transition An array of transitions allowed in the state machine.
 */
template <typename State, size_t kNumTransitions>
constexpr FsmConfig<State, kNumTransitions> MakeFsmConfig(
    State starting_state,
    std::array<Transition<State>, kNumTransitions> transitions) {
  return FsmConfig<State, kNumTransitions>(starting_state, transitions);
}
...
class StateMachine {
 ...
 public:
  ...
  class Transaction {
   public:
    /** Delete the copy constructor, only 1 copy allowed. */
     ...
     *
     * If the transaction has already been commited, this call will have no
     * effect.
     *
   ...
   * simply call Transaction::SetStatus() with a non-ok status before commiting
