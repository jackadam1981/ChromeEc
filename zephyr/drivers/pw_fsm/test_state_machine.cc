...
constexpr auto kLedStateConfig =
    MakeFsmConfig(LedState::OFF,
                  std::to_array({
                      Transition<LedState>(LedState::OFF, LedState::ON),
                      Transition<LedState>(LedState::ON, LedState::OFF),
                  }));
constexpr auto kLedOnStateSubConfig =
    MakeFsmConfig(LedColor::GREEN,
                  std::to_array({
                      Transition<LedColor>(LedColor::GREEN, LedColor::BLUE),
                      Transition<LedColor>(LedColor::BLUE, LedColor::GREEN),
                  }));
...
class LedFsm : public EventStateMachine<LedState, Event> {
 ...
 protected:
  LedState OnEvent(const LedState& state, const Event& event) override {
    switch (state) {
      case LedState::OFF:
        return HandleOnEvent<LedState::OFF>(event);
      case LedState::ON:
        return HandleOnEvent<LedState::ON>(event);
    }
  }

 private:
  template <LedState state>
  LedState HandleOnEvent(const Event& event);

  template <>
  LedState HandleOnEvent<LedState::OFF>(const Event& event) {
    switch (event) {
  ...
  template <>
  LedState HandleOnEvent<LedState::ON>(const Event& event) {
    switch (event) {
      case Event::TIMER_EXPIRE_RESET:
        return LedState::OFF;
      case Event::BUTTON_PRESS:
...
TEST(StateMachine, TransitionConstruction) {
  ...
  ASSERT_EQ(VariantType(StateMatcher::ANY),
...
TEST(StateMachine, StaticAssertConfig) {
  ...
  constexpr auto kInvalidLedStateConfig =
      MakeFsmConfig(LedState::OFF,
                    std::to_array({
                        Transition<LedState>(LedState::ON, LedState::OFF),
                    }));
