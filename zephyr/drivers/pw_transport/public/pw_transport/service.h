...
class Transport {
 ...
 public:
  ...
  pw::Status SetStatusBit(uint8_t bit_num, bool on) {
    if (auto status = DoSetStatusBit(bit_num, on); !status.ok()) {
      printk("Failed to set status bit %u to %d\n", bit_num, on);
      return status;
    }
    printk("Status.flags = [");
    for (size_t i = 0; i < sizeof(pw_transport_Status::flags); ++i) {
      if (i > 0) {
        printk(", ");
      }
      printk("0x%02x", status_.flags[i]);
    }
    printk("]\n");

    if (state_machine_.current_state() == impl::ServiceState::IDLE) {
      printk("Transitioning to HAS_STATUS\n");
      auto transaction =
 ...
 private:
  ...
  pw::Status DoSetStatusBit(uint8_t bit_num, bool on) {
    ...
    if (on) {
      status_.flags[byte_index] |= bit_mask;
    } else {
      status_.flags[byte_index] &= ~bit_mask;
    }
