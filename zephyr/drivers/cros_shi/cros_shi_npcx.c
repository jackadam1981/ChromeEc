...
#include <arch/arm/aarch32/cortex_m/cmsis.h>.h>
#include <assert.h>
...
#define NPCX_SHI_BASE_ADDR      DT_INST_REG_ADDR(0)
/* Start address of SHI output buffer */
#define SHI_OBUF_OFFSET         (offsetof(struct shi_reg, OBUF))
#define SHI_OBUF_START_ADDR     \
		(volatile uint8_t *)(NPCX_SHI_BASE_ADDR + SHI_OBUF_OFFSET)
...
#define SHI_IBUF_OFFSET         (offsetof(struct shi_reg, IBUF))
#define SHI_IBUF_START_ADDR     \
		(volatile uint8_t *)(NPCX_SHI_BASE_ADDR + SHI_IBUF_OFFSET)
/* Current address of SHI input buffer */
#define SHI_IBUF_CUR_ADDR(dev)  \
		(SHI_IBUF_START_ADDR + shi_read_buf_pointer())
/*
...
volatile enum cros_shi_npcx_state state;
...
/* Forward declaration */
static void cros_shi_npcx_reset_prepare(void);
...
static int shi_read_inbuf_wait(const struct device *dev, uint16_t szbytes);
...
static void shi_send_response_packet(struct host_packet *pkt)
{
	...
	if (state == SHI_STATE_PROCESSING) {
		 ...
		 */
		cros_shi_npcx_reset_prepare();
		DEBUG_CPRINTF("END\n");
	} else
		DEBUG_CPRINTS("Unexpected state %d in response handler",
			 state);
...
void shi_handle_host_package(const struct device *dev)
{
	...
	if (!shi_read_inbuf_wait(dev, remain_bytes))
...
static void shi_parse_header(const struct device *dev)
{
	...
	if (in_msg[0] == EC_HOST_REQUEST_VERSION) {
		...
		if (!shi_read_inbuf_wait(dev, sizeof(*r) - 3))
			return shi_bad_received_data();
...
static int shi_read_inbuf_wait(const struct device *dev, uint16_t szbytes)
{
	...
	for (i = 0; i < szbytes; i++, shi_params.sz_received++) {
		 ...
		 */
		while (shi_params.rx_buf == SHI_IBUF_CUR_ADDR(dev))
 ...
 */
static enum cros_shi_npcx_state last_error_state = SHI_STATE_UNDETERMINED;

static void log_unexpected_state(char *isr_name)
{
	if (state != last_error_state)
		LOG_DBG("Unexpected state %d in %s ISR", state, isr_name);
	last_error_state = state;
}

static void shi_handle_cs_assert(const struct device *dev)
{
	...
	if (state != SHI_STATE_READY_TO_RECV) {
		...
		LOG_ERR("Unexpected state %d in CS ISR", state);
		return;
	}
...
static void cros_shi_npcx_isr(const struct device *dev)
{
	...
	if (IS_BIT_SET(stat_reg, NPCX_EVSTAT_IBHF)) {
		if (state == SHI_STATE_RECEIVING) {
			...
			shi_read_half_inbuf();
			return shi_handle_host_package(dev);
...
static void cros_shi_npcx_reset_prepare(void)
{
	...
	last_error_state = SHI_STATE_UNDETERMINED;
...
{
	const struct cros_shi_npcx_config *const config = DRV_CONFIG(dev);
	struct cros_shi_npcx_data *const data = DRV_DATA(dev);
...
DEVICE_DT_INST_DEFINE(0, shi_npcx_init, device_pm_control_nop,
