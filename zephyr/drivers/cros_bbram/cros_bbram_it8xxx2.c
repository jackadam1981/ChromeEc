...
#define CROS_BBRAM_INIT(inst)                                                \
	...
	static const struct cros_bbram_it8xxx2_config                        \
				cros_bbram_cfg_##inst = {                    \
		.base_addr = DT_INST_REG_ADDR_BY_NAME(inst, memory),         \
		.size = DT_INST_REG_SIZE_BY_NAME(inst, memory),              \
	};                                                                   \
	DEVICE_DEFINE(cros_bbram_it8xxx2_##inst, DT_INST_LABEL(inst),        \
		      bbram_it8xxx2_init, NULL, &cros_bbram_data_##inst,     \
