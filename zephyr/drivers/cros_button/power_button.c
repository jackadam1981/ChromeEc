...
const struct device *power_button_dev =
	DEVICE_DT_GET(DT_NODELABEL(power_button));
...
static int power_button_signal_asserted(const struct device *dev)
{
	...
	button_get_config(dev, &cfg);

	const int is_active_high =
		((cfg.spec.dt_flags & GPIO_ACTIVE_HIGH) ? 1 : 0);

	return button_is_pressed_raw(dev) == is_active_high;
}
...
int power_button_wait_for_release(int timeout_us)
{
	...
	while (!power_button_is_stable || power_button_is_pressed()) {
		 ...
		 */
		button_get_debounce_us(power_button_dev, &debounce_us);
		task_wait_event(MIN(debounce_us, deadline.val - now.val));
	}
...
static int power_button_init(const struct device *dev)
{
	...
	button_enable_interrupt(dev, power_button_interrupt);

	return 0;
}

static const struct cros_button_api power_button_api = {
...
#define POWER_BUTTON_INIT(i)                                                 \
	static const struct cros_button_config power_button_cfg_##i =        \
		BUTTON_CFG_DEF(i);                                           \
	static struct cros_button_data power_button_data_##i;                \
