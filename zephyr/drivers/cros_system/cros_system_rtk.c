...
static int cros_system_rtk_init(const struct device *dev)
{
	...
	bbram_read(bbram_dev, BBRAM_REGION_OFFSET(key),
	...
	if (key_val != 0xA5) {
		bbram_write(bbram_dev, BBRAM_REGION_OFFSET(saved_reset_flags),
		    BBRAM_REGION_SIZE(saved_reset_flags), (uint8_t *)&value);
		...
		/* If key remained and not reset from wdt, setup EC_RESET_FLAG_RESET_PIN flag for hard reset */
		if (flag == 0) {
