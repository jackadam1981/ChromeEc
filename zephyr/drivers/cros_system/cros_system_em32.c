...
/*  
 * TODO(b/8708638): The em32_hwinfo structure is supported by 
 * elan hwinfo_em32 driver, which will be upsteamed later. 
 */
struct em32_hwinfo {
	uint32_t chip_id;
	uint32_t device_id;
...
static int system_em32_get_chip_id(void)
{
...
static int system_em32_get_chip_version(void)
{
	struct em32_hwinfo dev_hwinfo = { 0 };
	ssize_t ret = 0;
...
static const char *cros_system_em32_get_chip_name(const struct device *dev)
{
	...
	uint32_t chip_id = 0;
        ...
        // Get Chip ID
	ret = system_em32_get_chip_id();
	if(ret < 0)
	{
		LOG_ERR("system_em32_get_chip_id fail, err=%d.", ret);
		return "";
	}
	chip_id = (uint32_t) ret;
...
static const char *cros_system_em32_get_chip_revision(const struct device *dev)
{
	...
	uint8_t chip_version = 0;
        ...
        // Get Chip Version
	ret = system_em32_get_chip_version();
	if(ret < 0)
	{
		LOG_ERR("system_em32_get_chip_id fail, err=%d.", ret);
		return "";
	}
        chip_version = (uint8_t) ret;
...
#endif //CONFIG_CROS_SYSTEM_EM32_INIT_PRIORITY
