...
#if DT_NODE_HAS_STATUS(PSL_NODE, okay)
/**
 * Check if a PSL input node triggered the wake.
 *
 * @param name Node full name.
 * @param psl_cts PSL_CTS register value.
 * @return True if the PSL input corresponding to the name is the wake source.
 *
 */
static bool cros_system_npcx_is_psl_node_triggered(const char *name,
						   uint8_t psl_cts)
{
	/* PSL input nodes are named psl_in1, psl_in2, etc. */
	const char *p;

	p = strstr(name, "psl-in");

	if (!p)
		LOG_ERR("String error");
...
#define CHECK_PSL_WAKE(node_id, prop, idx)                                    \
	if (cros_system_npcx_is_psl_node_triggered(                           \
