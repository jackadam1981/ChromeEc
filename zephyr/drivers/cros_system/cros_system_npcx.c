...
static const char *system_npcx_hwinfo_vendor(struct mswc_reg *const inst_mswc)
{
...
static const char *system_npcx_hwinfo_name(struct mswc_reg *const inst_mswc)
{
...
static const char *system_npcx_hwinfo_revision(struct mswc_reg *const inst_mswc)
{
...
const char *cros_system_npcx_chip_info(const struct device *dev,
				       enum system_chip_info type)
{
	struct mswc_reg *const inst_mswc = HAL_MSWC_INST(dev);

	if (type == CHIP_VENDOR) {
		return system_npcx_hwinfo_vendor(inst_mswc);
	} else if (type == CHIP_NAME) {
		return system_npcx_hwinfo_name(inst_mswc);
	} else if (type == CHIP_REVISION) {
		return system_npcx_hwinfo_revision(inst_mswc);
	}
...
static const struct cros_system_driver_api cros_system_driver_npcx_api = {
	...
	.chip_info = cros_system_npcx_chip_info,
};
