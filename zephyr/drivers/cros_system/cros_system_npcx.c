...
#if defined(EC_CROS_HIBERNATE_GPIOS)
	/* GPIOs used for wake-up from hibernating */
	const enum gpio_signal hibernate_wake_pins[] = {
		EC_CROS_HIBERNATE_GPIOS,
	};

	/* Reconfigure wake-up GPIOs */
	for (int i = 0; i < ARRAY_SIZE(hibernate_wake_pins); i++) {
					 ...
					 uint32_t pd_ram_mask)
{
	...
	if (IS_BIT_SET(inst_mtc->WTC, NPCX_WTC_PTO)) {
		...
		*saved_reset_flags++ |= EC_RESET_FLAG_RTC_ALARM & 0xFF;
		*saved_reset_flags++ |= (EC_RESET_FLAG_RTC_ALARM >> 8) & 0xFF;
		*saved_reset_flags++ |= (EC_RESET_FLAG_RTC_ALARM >> 16) & 0xFF;
		*saved_reset_flags |= (EC_RESET_FLAG_RTC_ALARM >> 24) & 0xFF;
	} else {
		/* Otherwise, we treat it as GPIOs wake-up */
		*saved_reset_flags++ |= EC_RESET_FLAG_WAKE_PIN & 0xFF;
		*saved_reset_flags++ |= (EC_RESET_FLAG_WAKE_PIN >> 8) & 0xFF;
		*saved_reset_flags++ |= (EC_RESET_FLAG_WAKE_PIN >> 16) & 0xFF;
		*saved_reset_flags |= (EC_RESET_FLAG_WAKE_PIN >> 24) & 0xFF;
	}
