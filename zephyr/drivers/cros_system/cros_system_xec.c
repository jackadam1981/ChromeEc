...
const struct app_vci_pin app_vci_table[] = { DT_FOREACH_STATUS_OKAY(cros_ec_hibernate_vci_pin,
							      HIB_VCI_ENTRY) };
#endif
...
static int cros_system_xec_init(const struct device *dev)
{
	...
	if (IS_ENABLED(CONFIG_PLATFORM_EC_HIBERNATE_VCI)) {
		...
		for (size_t id = 0; id < ARRAY_SIZE(app_vci_table); id++) {
			...
			uint8_t vci_node_en =
				MCHP_VCI_INFO_NODE_EN(pvci->vci_info);
			uint8_t vci_clear =
				!MCHP_VCI_INFO_GET_PRESERVE(pvci->vci_info);
