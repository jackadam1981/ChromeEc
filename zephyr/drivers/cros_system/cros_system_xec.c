...
#define MCHP_VCI_INFO_POLARITY_LOW   0
#define MCHP_VCI_INFO_POLARITY_HIGH  1
#define MCHP_VCI_INFO_POLARITY_BOTH  2
...
static void cros_system_xec_vci_init(void)
{
	...
	for (size_t id = 0; id < ARRAY_SIZE(app_vci_table); id++) {
		...
		switch (vci_polarity) {
		case MCHP_VCI_INFO_POLARITY_LOW:
			vci->POLARITY &= ~BIT(id);
			break;
		case MCHP_VCI_INFO_POLARITY_HIGH:
			vci->POLARITY |= BIT(id);
			break;
		case MCHP_VCI_INFO_POLARITY_BOTH:
			if (vci->CONFIG & BIT(id)) {
				vci->POLARITY &= ~BIT(id);
			} else {
				vci->POLARITY |= BIT(id);
			}
...
static void cros_system_xec_configure_vci_in(void)
{
	...
	for (size_t id = 0; id < ARRAY_SIZE(app_vci_table); id++) {
		...
		if (vci_polarity == MCHP_VCI_INFO_POLARITY_BOTH) {
			if (vci->CONFIG & BIT(id)) {
				vci->POLARITY &= ~BIT(id);
			} else {
				vci->POLARITY |= BIT(id);
