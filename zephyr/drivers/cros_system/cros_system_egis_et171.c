...
/* TODO: adjust implemenation based on Egis chip support. Maybe move to hwinfo driver in Zephyr. */

/* Registers offsets */
#define AOSMU_VERSION 0x0 /* Chip version */
#define AOSMU_SECURE_CON 0xc /* Secure key handling */
#define AOSMU_DUMMY 0x1c /* Temporary name? */

#define AOSMU_VERSION_PROJECT 0xfff0 /* Project code */
#define AOSMU_VERSION_PROJECT_C 0 /* Project C */
...
#define AOSMU_SECURE_CON_MCU_RESET BIT(3) /* issue reset to MCU core */

#define AOSMU_DUMMY_RST_EVENT_WDT BIT(12) /* WDT event */
#define AOSMU_DUMMY_RST_EVENT_POR BIT(13) /* POR event from analog */
#define AOSMU_DUMMY_RST_EVENT_RSTN BIT(14) /* RSTN pin event */
...
static int cros_system_et171_soc_reset(const struct device *dev)
{
	...
	if (chip_reset_flags & EC_RESET_FLAG_HARD) {
		syscon_write_reg(syscon_dev, AOSMU_SECURE_CON, AOSMU_SECURE_CON_SYSTEM_RESET);
	} else {
		syscon_write_reg(syscon_dev, AOSMU_SECURE_CON, AOSMU_SECURE_CON_MCU_RESET);
	}
...
static int cros_system_et171_init(const struct device *dev)
{
	...
	syscon_read_reg(syscon_dev, AOSMU_VERSION, &reg);

	/* Reset cause is known only for C project version. */
	if ((reg & AOSMU_VERSION_PROJECT) == AOSMU_VERSION_PROJECT_C) {
		syscon_read_reg(syscon_dev, AOSMU_DUMMY, &reg);
		if (reg & AOSMU_DUMMY_RST_EVENT_WDT) {
			data->reset = WATCHDOG_RST;
		} else if (reg & AOSMU_DUMMY_RST_EVENT_POR) {
			data->reset = POWERUP;
		} else if (reg & AOSMU_DUMMY_RST_EVENT_RSTN) {
...
DEVICE_DEFINE(cros_system_et171_0, "CROS_SYSTEM", cros_system_et171_init, NULL,
	      ...
	      CONFIG_CROS_SYSTEM_EGIS_ET171_INIT_PRIORITY,
	      &cros_system_driver_et171_api);

#if CONFIG_CROS_SYSTEM_EGIS_ET171_INIT_PRIORITY >= \
