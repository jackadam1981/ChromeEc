...
static int cros_system_it8xxx2_get_reset_cause(const struct device *dev)
{
	...
	/*
	 * We can't differentiate between power-on and reset pin because the
	 * LRSIWR bit in SPCTRL4 is set on both ~WRST assertion and power-on,
	 * and LRS is either 0 or 1 in both cases. Conservatively treat both
	 * as power-on.
	 */
	return POWERUP;
}
