...
#ifdef CONFIG_PLATFORM_EC_HIBERNATE_ELPM
	if (seconds || microseconds) {
		/* TODO: the ELPM time-based wake-up is currently unsupported; ITE
		 * will add this feature in future update.
		 */
		LOG_ERR("unsupported elpm time-based wake-up, hibernating until wake pin asserted");
	}
