...
#define XLPINS_POL_GET_BY_IDX(idx, id) \
	DT_ENUM_IDX_BY_IDX(id, xlpins_polarity, idx)
#define XLPINS_POL_ELEMENTS_DEFINE(node_id)              \
	{ LISTIFY(DT_PROP_LEN(node_id, xlpins_polarity), \
		  XLPINS_POL_GET_BY_IDX, (, ), node_id) }

static const uint8_t xlpin_polarity_ctrl[] =
	XLPINS_POL_ELEMENTS_DEFINE(ELPM_NODE);
					    ...
					    uint32_t microseconds)
{
	...
	/* TODO: need to set xlpin de-bounce? */

	/* set xlpin[i] polarity */
	for (uint8_t i = 0; i < 7; i++) {
		if (xlpin_polarity_ctrl[i] != ELPM_POL_DEFAULT) {
			xlpins_enable |= BIT(i);
		}
		if (xlpin_polarity_ctrl[i] == ELPM_POL_HIGH) {
			/* set polarity control of xlpin[i] as active high */
	...
	/* enable xlpout control by xlpin */
