...
static struct rt3645_info update_entries;
...
static int rt3645_apply_update_data(const struct device *dev)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(raw_data); i++) {
		...
		if (prev_page != update_entries.page) {
			...
			/* Upon failure of set page, return */
			if (rv)
				return rv;
		...
		rv = rt3645_write_reg(dev, update_entries.reg,
					update_entries.val);
		/* Upon failure of write register, return */
		if (rv)
			return rv;

		LOG_INF("page:0x%x Reg:0x%x Data:0x%x ", update_entries.page,
							update_entries.reg,
							update_entries.val);
	}
