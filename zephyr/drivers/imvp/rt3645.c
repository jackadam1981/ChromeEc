...
uint8_t config_seq[]= {0x24, 0x54, 0x02};

/* Get raw data from devicetree */
static const uint32_t raw_data[] = DT_PROP(DT_DRV_INST(0), update_data);
static struct rt3645_info update_entries[ARRAY_SIZE(raw_data)];
static const uint8_t crc = DT_PROP(DT_DRV_INST(0), update_crc);
...
#if defined(CONFIG_AP_PWRSEQ_DRIVER) && defined(CONFIG_IMVP_FACTORY_UPDATE)
...
static int rt3645_apply_update_data(const struct device *dev)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(raw_data); i++) {
		...
		/* Unpack each entry */
		update_entries[i].page = (raw_data[i] >> 16) & 0xFF;
		update_entries[i].reg  = (raw_data[i] >> 8) & 0xFF;
		update_entries[i].val  = raw_data[i] & 0xFF;

		if (prev_page != update_entries[i].page) {

			/* Any new page write will need a delay of 1msec */
			k_msleep(1);

			/* Set the correct page */
			rv = rt3645_set_page(dev, update_entries[i].page);
			prev_page = update_entries[i].page;
		}

		/* Write the register value */
		rv = rt3645_write_reg(dev, update_entries[i].reg,
					update_entries[i].val);

		LOG_INF("page:0x%x Reg:0x%x Data:0x%x ", update_entries[i].page,
							update_entries[i].reg,
							update_entries[i].val);
	}
...
static int rt3645_crc_check(const struct device *dev)
{
	...
	if (crc_val == crc) {
		LOG_INF("Good CRC value - 0x%0x", crc_val);
		return EC_SUCCESS;
	}
...
static int rt3645_nvm_program_status(const struct device *dev, int stat_bit)
{
	...
	while (retry < retry_cnt) {
		...
		if (stat_bit == NVM_STAT_BITS) {
			if (status_reg == 0xE0) {
				LOG_INF("Match to NVM_STAT");
				return EC_SUCCESS;
			}
		} else {
			if ((status_reg >> stat_bit) & 0x1 ) {
				LOG_INF("NVM_Reload/Programming Done");
				return EC_SUCCESS;
			}
		}
		retry++;
...
static int rt3645_update(const struct device *dev)
{
	...
	/* Print Config Values */
	for(int i=0;i<len;i++)
	LOG_INF("Config_sequence - 0x%x",config_seq[i]);
	...
	LOG_INF("Product id Read - %d ",reg_val);
	...
	if (rt3645_crc_check(dev) == EC_SUCCESS)
		LOG_INF(" CRC Match. Imvp already updated");
	else {
