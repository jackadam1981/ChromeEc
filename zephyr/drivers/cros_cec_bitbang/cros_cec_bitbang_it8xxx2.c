...
BUILD_ASSERT(DT_NUM_INST_STATUS_OKAY(DT_DRV_COMPAT) == 1,
	     "Only one instance of cros-ec,cec-bitbang should be defined.");
...
static void cros_cec_bitbang_it8xxx2_tmr_cap_start(const struct device *dev,
						   int port,
...
static void cros_cec_bitbang_it8xxx2_tmr_cap_stop(const struct device *dev,
						  int port)
{
	ARG_UNUSED(dev);
	const struct bitbang_cec_config *drv_config =
...
static void cros_cec_bitbang_it8xxx2_debounce_enable(const struct device *dev,
						     int port)
{
	ARG_UNUSED(dev);
	const struct bitbang_cec_config *drv_config =
...
static void cros_cec_bitbang_it8xxx2_debounce_disable(const struct device *dev,
						      int port)
{
	ARG_UNUSED(dev);
	const struct bitbang_cec_config *drv_config =
...
static void cros_cec_bitbang_it8xxx2_trigger_send(const struct device *dev,
						  int port)
{
	ARG_UNUSED(dev);
...
static void cros_cec_bitbang_it8xxx2_enable_timer(const struct device *dev,
						  int port)
{
	ARG_UNUSED(dev);
	const struct bitbang_cec_config *drv_config =
...
static void cros_cec_bitbang_it8xxx2_disable_timer(const struct device *dev,
						   int port)
{
	ARG_UNUSED(dev);
...
static void cros_cec_bitbang_it8xxx2_init_timer(const struct device *dev,
						int port)
{
	ARG_UNUSED(dev);
...
DEVICE_DT_INST_DEFINE(0, cec_bitbang_ite_init, NULL, NULL, NULL, POST_KERNEL,
