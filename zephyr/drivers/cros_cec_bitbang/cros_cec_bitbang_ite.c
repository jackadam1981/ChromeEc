 ...
 */
void cec_ext_timer_alarm_handler(const struct device *counter_dev,
						  ...
						  int timeout)
{
	...
	if (timeout > 0) {
		 ...
		 * timer_count, 0, 1);
		 */
		alarm_cfg.callback = cec_ext_timer_alarm_handler;
		alarm_cfg.ticks = timer_count;
		alarm_cfg.flags = 0;
		alarm_cfg.user_data = NULL;
		counter_set_channel_alarm(counter_ite_dev, 0, &alarm_cfg);
	} else {
		/*
		 * In chip/it83xx/cec_bitbang.c
		 * ext_timer_stop(drv_config->timer, 1);
		 */
		counter_cancel_channel_alarm(counter_ite_dev, 0);
	}
					       ...
					       int port)
{
	...
	alarm_cfg.callback = cec_ext_timer_alarm_handler;
	alarm_cfg.ticks = counter_us_to_ticks(counter_ite_dev, 0);
	alarm_cfg.flags = 0;
	alarm_cfg.user_data = NULL;
	counter_set_channel_alarm(counter_ite_dev, 0, &alarm_cfg);
	return 0;
