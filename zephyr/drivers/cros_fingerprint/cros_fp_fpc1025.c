...
static int fpc1025_init(const struct device *dev)
{
	...
	data->errors = FP_ERROR_DEAD_PIXELS_UNKNOWN;
	...
	if (rc) {
		...
		data->errors |= FP_ERROR_INIT_FAIL;
		return rc;
	}
			       ...
			       size_t size)
{
	...
	data->errors |= FP_ERROR_DEAD_PIXELS(sensor_info.num_defective_pixels);
...
#define FPC1025_DEFINE(inst)                                                 \
	...
	static const struct fpc1025_cfg fpc1025_cfg##inst = {                \
		...
		.info = {                                                    \
			...
			.pixel_format =                                      \
			FP_SENSOR_V4L2_PIXEL_FORMAT(DT_DRV_INST(inst)),      \
			.width = FP_SENSOR_RES_X(DT_DRV_INST(inst)),         \
			.height = FP_SENSOR_RES_Y(DT_DRV_INST(inst)),        \
			.bpp = FP_SENSOR_RES_BPP(DT_DRV_INST(inst)),         \
		},                                                           \
	};                                                                   \
	BUILD_ASSERT(CONFIG_FP_SENSOR_IMAGE_SIZE >=                          \
			     FP_SENSOR_REAL_IMAGE_SIZE(DT_DRV_INST(inst)),   \
