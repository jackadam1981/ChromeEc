...
BUILD_ASSERT(DT_INST_PROP(0, id) >= 0);
BUILD_ASSERT(DT_INST_PROP(0, id) <= 3);

static const struct device* uart = DEVICE_DT_GET(DT_INST_PARENT(0));
...
const static int SELF_ID = DT_INST_PROP(0, id);
	...
	while (true) {
		...
		if (tail >= msg_size) {
			if (checksum(&recv_buf.msg) == 0) {
				if (recv_buf.msg.header.sender != SELF_ID) {
