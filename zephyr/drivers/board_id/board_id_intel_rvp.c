...
__override int board_get_version(void)
{
	...
	board_id |= gpio_pin_get_dt(&rvp_config->board_gpios_config[1]) << 1;
	board_id |= gpio_pin_get_dt(&rvp_config->board_gpios_config[2]) << 2;
	board_id |= gpio_pin_get_dt(&rvp_config->board_gpios_config[3]) << 3;
	board_id |= gpio_pin_get_dt(&rvp_config->board_gpios_config[4]) << 4;
	board_id |= gpio_pin_get_dt(&rvp_config->board_gpios_config[5]) << 5;

	rvp_board_id = board_id;

	LOG_INF("BOARD_ID:0x%x", board_id);

	if (rvp_config->bom_gpios_config) {
		/*
		 * BOM ID [2]   : IOEX[0]
	...
	if (rvp_config->fab_gpios_config) {
		/*
				     ...
				     const enum ap_pwrseq_state exit)
{
	...
	if (exit == AP_POWER_STATE_G3) {
		...
		for (int i = 0; i < BOM_GPIOS_COUNT; i++) {
			...
			if (!device_is_ready(gpio_port)) {
				LOG_INF("Initializing bom_gpios controller port");
		...
		for (int i = 0; i < FAB_GPIOS_COUNT; i++) {
			...
			if (!device_is_ready(gpio_port)) {
				LOG_INF("Initializing fab_gpios controller port");
				device_init(gpio_port);
			}
		}
		for (int i = 0; i < BOARD_GPIOS_COUNT; i++) {
			...
			if (!device_is_ready(gpio_port)) {
				LOG_INF("Initializing board_gpios controller port");
				device_init(gpio_port);
			}
		}
	}
