...
int get_rvp_id_config(enum rvp_id_type id_type)
{
	...
	if (id_type == BOARD_ID) {
		...
		for (int i = 0; i < BOARD_GPIOS_COUNT; ++i) {
			int pin = gpio_pin_get_dt(&rvp_config->board_gpios_config[i]);
			if (pin < 0)
				return -1;
			board_id |= pin << i;
		}
	...
	if (id_type == BOM_ID && rvp_config->bom_gpios_config) {
		...
		for (int i = 0; i < BOM_GPIOS_COUNT; ++i) {
			int pin = gpio_pin_get_dt(&rvp_config->bom_gpios_config[i]);
			if (pin < 0)
				return -1;
			bom_id |= pin << i;
		}
	...
	if (id_type == FAB_ID && rvp_config->fab_gpios_config) {
		...
		for (int i = 0; i < FAB_GPIOS_COUNT; ++i) {
			int pin = gpio_pin_get_dt(&rvp_config->fab_gpios_config[i]);
			if (pin < 0)
				return -1;
			fab_id |= pin << i;
		}
