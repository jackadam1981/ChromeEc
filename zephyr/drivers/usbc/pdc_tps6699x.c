 ...
 */
#define PDC_INIT_ERROR_RECOVERY_DELAY_MS (50)
...
static void tps_check_and_notify_irq(int skip_port);
...
static void set_all_ports_to_init(void)
{
...
static int handle_irqs(struct pdc_data_t *data)
{
	...
	if (!interrupt_pending) {
		/* If the pending interrupt wasn't for this port, notify the
		 * port that is currently pending.
		 */
		tps_check_and_notify_irq(cfg->connector_number);
		return 0;
	}
	 ...
	 */
	tps_check_and_notify_irq(/*skip_port=*/-1);
...
static void tps_check_and_notify_irq(int skip_port)
{
	for (int port = 0; port < NUM_PDC_TPS6699X_PORTS; port++) {
		...
		/* Skip given port. */
		if (skip_port == cfg->connector_number) {
			continue;
		}

		/* Check if there is currently an interrupt pending. */
