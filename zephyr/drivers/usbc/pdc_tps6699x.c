...
static void st_init_run(void *o)
{
	...
	union reg_global_system_configuration gsc;
	int ret;

	ret = tps_rd_version(&cfg->i2c, &version);
	if (ret) {
		set_state(data, ST_ERROR_RECOVERY);
		return;
	}

	LOG_INF("PDC VERSION: %08x\n", version.version);

	ret = tps_rw_global_system_configuration(&cfg->i2c, &gsc, I2C_MSG_READ);
	if (ret) {
		set_state(data, ST_ERROR_RECOVERY);
		return;
	}

	for (int i=2; i < sizeof(union reg_global_system_configuration); i++) {
		LOG_INF("Byte%d: %02x", i-2, gsc.raw_value[i]);
	}
	LOG_INF("\n");
#endif
