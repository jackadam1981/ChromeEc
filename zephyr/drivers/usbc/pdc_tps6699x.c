...
static void cmd_set_rdo(struct pdc_data_t *data)
{
	...
	uint32_t pdo = data->cached_pdos[RDO_POS(data->rdo)];

	if (!data->cached_pdos[0]) {
		/* TODO: retrieve PDOs from port partner
		   if no prior GET_PDOS is called */
		LOG_ERR("Cached PDOs are not initialized.");
		goto error_recovery;
	}
	...
	if ((pdo & PDO_TYPE_MASK) == PDO_TYPE_BATTERY) {
		...
		max_v = min_v = (pdo >> 10) & 0x3FF;
		max_a = pdo & 0x3FF;
		/* (max_v * 50) * (max_a * 10) / 250 */
		min_power = max_v * max_a * 2;
	}

	an_snk.auto_compute_sink_min_power = 0;
	an_snk.auto_compute_sink_min_voltage = 0;
	an_snk.auto_compute_sink_max_voltage = 0;
	an_snk.auto_neg_max_current = max_a;
	an_snk.auto_neg_sink_min_required_power = min_power;
	an_snk.auto_neg_max_voltage = max_v;
	an_snk.auto_neg_min_voltage = min_v;
