...
static void st_irq_run(void *o)
{
	...
	if (interrupt_pending) {
		if (pdc_interrupt.patch_loaded) {
			 ...
			 * ports to avoid it is cleared before handling irq on
			 * other ports. */
			for (int port = 0; port < board_get_usb_pd_port_count();
			     port++) {
...
static void st_init_run(void *o)
{
	...
	/* We won't see patch_loaded is asserted while handing the IRQ
	   later on if boot from dead battery as it is cleared here. */
