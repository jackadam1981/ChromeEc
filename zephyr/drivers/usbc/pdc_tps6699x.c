...
static void st_task_wait_run(void *o)
{
	...
	case CMD_GET_PDOS: {
		...
		if (!data->cci_event.error) {
			/* TODO (Command doesn't sent PDO0): Create 5V PDO */
			uint32_t pdo0 = 0x0a01912c;

			/* Offset 1 is the number of PDOS times 4 */
			len = cmd_data.data[1];
			/* PDO Data starts at offset 2 */
			offset = 2;
	...
	if (data->user_buf && len && data->cmd != CMD_GET_PDOS) {
		if (data->cci_event.error) {
			memset(data->user_buf, 0, len);
		} else {
