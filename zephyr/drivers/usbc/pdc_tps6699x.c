...
struct sync_task_data_t {
	/* Task name: 4-byte, non-null-terminated ASCII string, represented as
	 * little-endian uint32_t. COMMAND_TASK_NO_COMMAND is a placeholder when
	 * no task is active.
	 */
...
static void task_sync(struct pdc_data_t *data);
...
static void task_sync(struct pdc_data_t *data)
{
	...
	if (data->sync_task.task == COMMAND_TASK_NO_COMMAND) {
		set_state(data, ST_IDLE);
		return;
	}
	...
	rv = write_task_cmd(cfg, data->sync_task.task, cmd_data);

	if (data->sync_task.no_validate_and_wait) {
		/* Mark task completed. */
		data->sync_task.task = COMMAND_TASK_NO_COMMAND;
			    ...
			    void *user_buf)
{
	...
	if (get_state(data) != ST_IDLE && cmd != CMD_SYNC) {
		return -EBUSY;
	}
...
static int do_reset_pdc(const struct device *dev)
{
	...
	rv = run_task_sync(dev, COMMAND_TASK_GAID, &cmd_data, NULL, true);
		      ...
		      int metadata_offset, int data_block_offset)
{
	...
	if (ret < 0 || ret != tfud->data_block_size) {
		LOG_ERR("Streaming data block failed. Expected to write %d but "
			"result was %d",
...
int tps6699x_do_firmware_update(const struct device *dev)
{
	...
	ret = run_task_sync(dev, COMMAND_TASK_TFUS, /*cmd_data=*/NULL,
	...
	if (ret < 0 || ret != HEADER_BLOCK_LENGTH) {
		LOG_ERR("Streaming header failed. Expected to write %d but "
			"result was %d",
	...
	for (int block = 0; block < tfui->num_blocks; ++block) {
		...
		if (ret >= 0) {
			LOG_INF("TFUq says current block was written=%d, status "
				"= 0x%02x",
	...
	if (!dry_run) {
		...
		LOG_INF("TFUq bytes [Success: 0x%02x, State: 0x%02x, Complete: "
			"0x%02x]",
		...
		ret = run_task_sync(dev, COMMAND_TASK_TFUE, NULL, rbuf,
				    /*no_wait=*/false);
		if (ret < 0 || rbuf[0] != 0) {
			LOG_ERR("Cleaning up resulted in ret=%d and result "
				"byte=0x%02x",
				ret, rbuf[0]);
		}
