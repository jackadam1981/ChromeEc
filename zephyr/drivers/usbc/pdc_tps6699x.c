...
static void st_irq_run(void *o)
{
	...
	data->cci_event.connector_change = pdc_interrupt.plug_insert_or_removal;
	/* Set CCI EVENT for not supported */
	data->cci_event.not_supported = pdc_interrupt.not_supported_received;
	/* Set CCI EVENT for vendor defined indicator (informs subsystem that an
	 * interrupt occurred */
...
static int tps_set_notification_enable(const struct device *dev,
...
static const struct pdc_driver_api_t pdc_driver_api = {
	...
	.set_notification_enable = tps_set_notification_enable
};
...
static int pdc_init(const struct device *dev)
{
	...
	LOG_INF("TI TPS6699X PDC DRIVER");
