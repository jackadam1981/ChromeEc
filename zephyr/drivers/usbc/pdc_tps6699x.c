...
#define tps_post_event(data, event) pdc_thread_post_msg(data->dev, event)

#else
static int tps_post_event(struct pdc_data_t *data, uint32_t event)
{
...
static void set_state(struct pdc_data_t *data, const enum state_t next_state)
{
	...
	case ST_IRQ:
	case ST_SUSPENDED:
		tps_post_event(data, PDC_INTERNAL_EVENT);
		break;
	default:
		break;
	}
					  ...
					  struct pdc_callback *callback)
{
	...
	if (k_mutex_lock(&data->mtx, K_MSEC(100)) == 0) {
		...
		k_mutex_unlock(&data->mtx);
		tps_post_event(data, PDC_CMD_EVENT);
	} else {
		return -EBUSY;
	}
			 ...
			 uint16_t vendor_defined)
{
	...
	tps_post_event(data, PDC_CMD_COMPLETE_EVENT);
...
static int tps_set_comms_state(const struct device *dev, bool comms_active)
{
	...
	if (comms_active) {
		 ...
		 */
		enable_comms();
		tps_post_event(data, PDC_IRQ_EVENT);
		 ...
		 */
		tps_post_event(data, PDC_CMD_SUSPEND_REQUEST_EVENT);
...
{
	LOG_INF("pdc_interrupt_callback dev=%s pins=0x%X", dev->name, pins);
	...
	for (int i = 0; i < NUM_PDC_TPS6699X_PORTS; i++) {
		tps_post_event(pdc_data[i], PDC_IRQ_EVENT);
	}
}
