...
static void st_irq_run(void *o)
{
	...
	LOG_INF("RD IRQP %d: %02x", cfg->connector_number, REG_INTERRUPT_EVENT_FOR_I2C1);
	for (i = 2; i < sizeof(union reg_interrupt); i++) {
		LOG_INF("Byte%d: %02x", i-2, pdc_interrupt.raw_value[i]);
	}
	LOG_INF("\n");
#endif
...
#if 0
		LOG_INF("WR IRQP %d: %02x", cfg->connector_number, REG_INTERRUPT_CLEAR_FOR_I2C1);
		for (i = 2; i < sizeof(union reg_interrupt); i++) {
			LOG_INF("Byte%d: %02x", i-2, pdc_interrupt.raw_value[i]);
		}
		LOG_INF("\n");
