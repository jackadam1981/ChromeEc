...
typedef struct mz_stream_s {
	...
	mz_alloc_func zalloc; /* optional heap allocation function (defaults to
				 malloc) */
...
typedef enum {
	...
	MZ_ZIP_FLAG_VALIDATE_LOCATE_FILE_FLAG =
		0x1000, /* if enabled,
			   ...
			   dir (intended for
			   testing) */
	MZ_ZIP_FLAG_VALIDATE_HEADERS_ONLY = 0x2000, /* validate the local
						       headers, but don't
						       decompress the entire
						       file and check the crc32
						     */
	MZ_ZIP_FLAG_WRITE_ZIP64 = 0x4000, /* always use the zip64 file format,
					     instead of the original zip file
					     format with automatic switch to
...
MINIZ_EXPORT mz_bool mz_zip_writer_add_from_zip_reader(
	mz_zip_archive *pZip, mz_zip_archive *pSource_zip,
