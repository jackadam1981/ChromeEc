			...
			uint8_t *buf, uint8_t len, int flag)
{
	...
	if (flag == I2C_MSG_READ) {
		...
		/* Skip reg value in buf[0] by starting ate buf[1] and decrement
		 * length by 1 */
		msg[1].buf = &buf[1];
		msg[1].len = len - 1;
...
int tps_rd_vendor_id(const struct i2c_dt_spec *i2c, union reg_vendor_id *buf)
{
	...
	return tps_xfer_reg(i2c, REG_VENDOR_ID, buf->raw_value,
			    sizeof(buf->raw_value), I2C_MSG_READ);
}
...
int tps_rd_uid(const struct i2c_dt_spec *i2c, union reg_uid *buf)
{
	...
	return tps_xfer_reg(i2c, REG_UID, buf->raw_value,
			    sizeof(buf->raw_value), I2C_MSG_READ);
}
...
int tps_rd_interrupt_event(const struct i2c_dt_spec *i2c, uint8_t port,
			   union reg_interrupt *buf)
{
	enum tps6699x_reg reg = (port == 0) ? REG_INTERRUPT_EVENT_FOR_I2C1 :
	...
	return tps_xfer_reg(i2c, reg, buf->raw_value, sizeof(buf->raw_value),
			    I2C_MSG_READ);
}

int tps_rw_interrupt_mask(const struct i2c_dt_spec *i2c, uint8_t port,
			  union reg_interrupt *buf, int flag)
{
	enum tps6699x_reg reg = (port == 0) ? REG_INTERRUPT_MASK_FOR_I2C1 :
...
int tps_rw_interrupt_clear(const struct i2c_dt_spec *i2c, uint8_t port,
			   union reg_interrupt *buf, int flag)
{
	enum tps6699x_reg reg = (port == 0) ? REG_INTERRUPT_CLEAR_FOR_I2C1 :
	...
	union reg_received_source_capabilities *buf)
{
	...
	return tps_xfer_reg(i2c, REG_RECEIVED_SOURCE_CAPABILITIES,
			    buf->raw_value, sizeof(buf->raw_value),
			    I2C_MSG_READ);
}
	...
	union reg_global_system_configuration *buf, int flag)
{
	...
	return tps_xfer_reg(i2c, REG_GLOBAL_SYSTEM_CONFIGURATION,
			    buf->raw_value, sizeof(buf->raw_value), flag);
}
