...
{
	enum tps6699x_reg reg = (port == 0) ? REG_INTERRUPT_EVENT_FOR_I2C2 :
...
{
	enum tps6699x_reg reg = (port == 0) ? REG_INTERRUPT_MASK_FOR_I2C2 :
...
int tps_rw_interrupt_clear(const struct i2c_dt_spec *i2c, uint8_t port,
			   union reg_interrupt *buf, int flag)
{
	enum tps6699x_reg reg = (port == 0) ? REG_INTERRUPT_CLEAR_FOR_I2C2 :
