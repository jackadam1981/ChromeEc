...
struct pdc_data_t {
	...
	/** CCI Event */
	union cci_event_t cci_event; //most same as ping_status_t
	/** CC Event callback */
	struct pdc_callback *cc_cb; //pdc_cc_handler_cb()
	...
	sys_slist_t ci_cb_list; //list: pdc_ci_handler_cb()
...
static const char *const cmd_names[] = {
	...
	[CMD_CLR_ALERT_STATUS] = "CMD_CLR_ALERT_STATUS",
};
...
static int it52xx_get_error_status(const struct device *dev,
				  union error_status_t *es);
static int it52xx_clr_alert_status(const struct device *dev)
...
static enum smf_state_result st_init_run(void *o)
{
	...
	case INIT_PDC_ENABLE: /* need? wakeup pdc? */
		...
		return SMF_EVENT_HANDLED;
	case INIT_PDC_GET_IC_STATUS: /* UCSI get pdc chip ID reg (it5271 not support, hardcode) */
		...
		return SMF_EVENT_HANDLED;
	case INIT_PDC_SET_NOTIFICATION_ENABLE: /* can notify first? -> ppm reset? */
		...
		return SMF_EVENT_HANDLED;
	case INIT_PDC_RESET:
		rv = it52xx_ppm_reset(data->dev); /* place ucsi data */
		if (rv) {
			LOG_ERR("C:%d, Internal(INIT_PDC_RESET)", cnum);
			set_state(data, ST_DISABLE);
			return SMF_EVENT_HANDLED;
		}
		init_write_cmd_and_change_state(data, INIT_PDC_COMPLETE); /* trigger state machine to write by i2c */
		return SMF_EVENT_HANDLED;
...
static void handle_irqs(struct pdc_data_t *data)
{
	...
	for (int i = 0; i < pdc_power_mgmt_get_usb_pd_port_count(); i++) {
		 ...
		 */
		//each port has each thread & different GPIO_INT pin
		//we can just read reg 0xBD (instead of ARA)
		...
		//each port has each i2c address
		//we can just set pdc_int_data (instead of for loop)
		for (int j = 0; j < pdc_power_mgmt_get_usb_pd_port_count();
			...
			if ((ara >> 1) == cfg->i2c.addr) {
				...
				/* Clear the pending interrupt events */
				//clear it52xx status in here, it won't clear CCI event
				rv = it52xx_clr_alert_status(data->dev);
				rv = it52xx_i2c_write(data->dev); //seems can't call it52xx_i2c_write() directly
				if (rv) {
					LOG_ERR("Clear interrupt events failed");
					return rv;
				}
...
static enum smf_state_result st_write_run(void *o)
{
	...
	data->next_ping_status = sys_timepoint_calc(K_MSEC(T_PING_STATUS)); //TODO: change to wait INT in [ST_PING_STATUS], then read INT -> get CCI
...
static enum smf_state_result st_ping_status_run(void *o)
{
	...
	case CMD_DONE:
		...
		if (data->cmd == CMD_PPM_RESET) {
			...
			LOG_DBG("C%d: ping_status (CCI): { 0x%x, 0x%x, 0x%x, 0x%x, 0x%x }",
				cfg->connector_number,
				data->ping_status.raw_value[0],
				data->ping_status.raw_value[1],
				data->ping_status.raw_value[2],
				data->ping_status.raw_value[3],
				data->ping_status.raw_value[4]);
			/*
	...
	default:
		...
		LOG_ERR("C%d: unknown ping_status (CCI): { 0x%x, 0x%x, 0x%x, 0x%x, 0x%x }",
				cfg->connector_number,
				data->ping_status.raw_value[0],
				data->ping_status.raw_value[1],
				data->ping_status.raw_value[2],
...
static DEVICE_API(pdc, pdc_driver_api) = {
	...
	.set_handler_cb = it52xx_set_handler_cb, /* mgmt: pdc_cc_handler_cb() */
	...
	.manage_callback = it52xx_manage_callback, /* mgmt: pdc_ci_handler_cb() */
