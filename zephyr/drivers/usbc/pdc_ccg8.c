...
static int get_port(char *arg, int *port)
{
	...
	*port = strtol(arg, &e, 0);
	if (*e || (*port >= CONFIG_USB_PD_PORT_MAX_COUNT) || (*port < 0))
		return -EINVAL;
...
static int cmd_read_register(const struct shell *sh, size_t argc, char **argv)
{
	...
	/* Read for PD registers */
	rv = pd_read_ccg(pd_pow_config_array[port], reg, len, read_buf);
	if (rv) {
		shell_error(sh, "Read Failed, rv = %d", rv);
		return rv;
	}
	for (int i = 0; i < len; i++)
		shell_info(sh, "[%d] = %x", i, read_buf[i]);
...
static int cmd_write_register(const struct shell *sh, size_t argc, char **argv)
{
	...
	/* Length of data in terms of bytes to be written to PD */
	len = argc - 3 < PD_MAX_WRITE_SIZE ? argc - 3 :
						      PD_MAX_WRITE_SIZE;
