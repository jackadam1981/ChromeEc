		...
		struct cmd_args *args)
{
	...
	if (!strcmp(argv[0], "read")) {
		...
		rv = get_int_val(argv[3], &args->len, sizeof(args->len));
		if (rv) {
		...
		for (int i = 0; i < args->len; i++) {
			rv = get_int_val(argv[3 + i], &args->data[i], sizeof(args->data[i]));
			if (rv) {
...
static int cmd_write_register(const struct shell *sh, size_t argc, char **argv)
{
	...
	/* Process command arguments. Process length before other arguments */
	write_args.len = argc - 3 < PD_MAX_WRITE_SIZE ? argc - 3 :
						      PD_MAX_WRITE_SIZE;
