...
static int get_port(char *arg, int *port)
{
	char *e;
	...
	*port = strtol(arg, &e, 0);
	if (e != (arg + strlen(arg)))
		return -EINVAL;
	if (*port >= ARRAY_SIZE(pd_pow_config_array) || port < 0)
		return -EINVAL;
...
static int cmd_read_register(const struct shell *sh, size_t argc, char **argv)
{
	...
	int port;
	uint8_t red_buf[4] = { 0 };
	int rv = get_port(argv[1], &port);

	if (rv) {
		shell_print(sh, "Invalid port");
		return rv;
	}
	rv = get_int_val(argv[2], &reg, sizeof(reg));
	if (rv) {
		shell_print(sh, "Invalid register");
		return rv;
	}
	rv = get_int_val(argv[3], &len, sizeof(len));
	if (rv) {
		shell_print(sh, "Invalid length");
		return rv;
	}
	rv = pd_read_ccg(pd_pow_config_array[port], reg, len, red_buf);
	if (rv) {
		shell_print(sh, "Read Failed, rv = %d", rv);
		return rv;
	}
	for (int i = 0; i < len; i++)
		shell_print(sh, "[%d] = %x\n", i, red_buf[i]);
...
static int cmd_write_register(const struct shell *sh, size_t argc, char **argv)
{
	...
	for (int i = 0; i < len; i++) {
		rv = get_int_val(argv[3 + i], &buf[i], sizeof(buf[i]));
		if (rv) {
			shell_print(sh, "Invalid data");
			return rv;
		}
	}
	rv = pd_write_ccg(pd_pow_config_array[port], reg, len, buf);
	if (rv) {
		shell_print(sh, "Write failed, rv = %d", rv);
		return rv;
	}
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(write, NULL,
		      "write to ccg PD\n"
		      "usage: write <port> <reg> <data 0> <data1>...<data3>",
