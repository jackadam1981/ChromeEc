...
#define PD_CHIP_FIND(usbc_id, pd_id) \
	CHECK_COMPAT(DT_DRV_COMPAT, usbc_id, pd_id, DEVICE_DT_GET)
...
static int ccg_enable(const struct device *dev)
{
	return 0;
}
...
static int ccg_set_notification_enable(const struct device *dev,
...
static int ccg_is_flash_code(const struct device *dev, uint8_t *is_flash_code)
{
	return 0;
}

static int ccg_get_current_flash_bank(const struct device *dev, uint8_t *bank)
{
	return 0;
}

static int ccg_get_fw_version(const struct device *dev, uint32_t *fw_version)
{
	return 0;
}

static int ccg_get_vid_pid(const struct device *dev, uint32_t *vidpid)
{
	return 0;
}

static int ccg_get_pd_version(const struct device *dev, uint32_t *pd_version)
{
	return 0;
}
...
static const struct pdc_driver_api_t pdc_driver_api = {
	...
	.set_notification_enable = ccg_set_notification_enable,
	...
	.is_flash_code = ccg_is_flash_code,
	.get_current_flash_bank = ccg_get_current_flash_bank,
	.get_fw_version = ccg_get_fw_version,
	.get_vid_pid = ccg_get_vid_pid,
	.get_pd_version = ccg_get_pd_version,
