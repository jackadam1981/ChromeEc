...
static int cmd_read_register(const struct shell *sh, size_t argc, char **argv)
{
	...
	rv = pd_read_ccg(pd_pow_config_array[0], reg, len, red_buf);
	if (rv) {
...
static int cmd_write_register(const struct shell *sh, size_t argc, char **argv)
{
	...
	for (int i = 0; i < len; i++) {
		rv = get_int_val(argv[2 + i], &buf[i], sizeof(buf[i]));
		if (rv) {
			shell_print(sh, "Invalid data");
			return rv;
		}
	}
	rv = pd_write_ccg(pd_pow_config_array[0], reg, len, buf);
	if (rv) {
...
SHELL_STATIC_SUBCMD_SET_CREATE(ccg_sub_cmds,
