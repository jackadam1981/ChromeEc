...
/*
 * ITE it5272 Power Delivery Controller Driver
 */
...
LOG_MODULE_REGISTER(pdc_it5272, CONFIG_USBC_LOG_LEVEL);
...
#define DT_DRV_COMPAT ite_it5272_pdc
 ...
 */
struct pdc_config_t { //TI and RTK are the same
	/** I2C config */
 ...
 */
struct pdc_data_t { //what should we need? like TI or RTK?
...
static void it5272_thread(void *dev, void *unused1, void *unused2)
{
	struct pdc_data_t *data = ((const struct device *)dev)->data;
	while (1) {
		//wait timeout/event: TI does in idle state, RTK does in thread, what should we do?
...
#define PDC_DEFINE(inst)                                                       \
	K_THREAD_STACK_DEFINE(thread_stack_area_##inst,                        \
			      CONFIG_USBC_PDC_IT5272_STACK_SIZE);              \
                                                                               \
	...
	{                                                                      \
		...
		data->thread = k_thread_create(                                \
			...
			it5272_thread, (void *)dev, 0, 0,                      \
			CONFIG_USBC_PDC_IT5272_THREAD_PRIORITY, K_ESSENTIAL,   \
			K_NO_WAIT);                                            \
		k_thread_name_set(data->thread, "it5272" STRINGIFY(inst));     \
	}                                                                      \
                                                                               \
	...
	static const struct pdc_config_t pdc_config##inst = {                  \
		...
		.bits.error = 1,                                               \
		.bits.sink_path_status_change = 1, /* TI only? */              \
