...
enum init_state_t {
	...
	INIT_PDC_NTF_SET_ACK_CC_CI,
	/** Get the PDC IC Status by VDM */
	INIT_PDC_VDM_GET_IC_STATUS,
	/** Set the PDC ACK of VDM */
	INIT_PDC_VDM_SET_ACK_CC_CI,
...
//static int it52xx_pdc_enable(const struct device *dev);
...
static void perform_pdc_init(struct pdc_data_t *data)
{
	...
	data->init_local_state = INIT_PDC_RESET /* INIT_PDC_ENABLE */;
...
static enum smf_state_result st_init_run(void *o)
{
	...
	case INIT_PDC_ENABLE:
#if 0 /* Not UCSI cmd, it5271 not support */ //TODO: [ST_SUSPENDED] call to here(reset driver + PDC), so may need, do like read/clear status
		rv = it52xx_pdc_enable(data->dev);
		if (rv) {
			LOG_ERR("ST%d: Internal(INIT_PDC_ENABLE)", cnum);
		...
		return SMF_EVENT_HANDLED;
#endif
	case INIT_PDC_RESET: /* flow ppm reset -> set ntf (spec): it5271 ignore cmd that prior ppm_reset */
	...
	case INIT_PDC_SET_NOTIFICATION_ENABLE:
		...
		init_write_cmd_and_change_state(data, INIT_PDC_NTF_SET_ACK_CC_CI);
		return SMF_EVENT_HANDLED;
	case INIT_PDC_NTF_SET_ACK_CC_CI: /* it5271 is waiting ACK and ignore other [CONTROL] cmd */
		ci.connect_change = 0;
		rv = it52xx_ack_cc_ci(data->dev, ci, cc, 0);
		if (rv) {
			LOG_ERR("ST%d: Internal(INIT_PDC_NTF_SET_ACK_CC_CI)", cnum);
			set_state(data, ST_DISABLE);
			return SMF_EVENT_HANDLED;
		}
		init_write_cmd_and_change_state(data, INIT_PDC_VDM_GET_IC_STATUS);
		return SMF_EVENT_HANDLED;
	case INIT_PDC_VDM_GET_IC_STATUS: /* 1. Not UCSI cmd, so can't run sm: [ST_WRITE] -> [PING_STATUS] -> [ST_READ] */
		...
		if (rv) {
			LOG_ERR("ST%d: Internal(INIT_PDC_VDM_GET_IC_STATUS)", cnum);
			set_state(data, ST_DISABLE);
			return SMF_EVENT_HANDLED;
		}
		init_write_cmd_and_change_state(data, INIT_PDC_VDM_SET_ACK_CC_CI);
		return SMF_EVENT_HANDLED;
	case INIT_PDC_VDM_SET_ACK_CC_CI: /* it5271 is waiting ACK and ignore other [CONTROL] cmd  */
		rv = it52xx_ack_cc_ci(data->dev, ci, cc, 0);
		if (rv) {
			LOG_ERR("ST%d: Internal(INIT_PDC_VDM_SET_ACK_CC_CI)", cnum);
			set_state(data, ST_DISABLE);
			return SMF_EVENT_HANDLED;
		}
	...
	case INIT_ERROR:
		...
		init_write_cmd_and_change_state(data, INIT_PDC_RESET /* INIT_PDC_ENABLE */);
		return SMF_EVENT_HANDLED;
			...
			if (data->init_local_current_state == INIT_ERROR) {
				...
				if (data->init_retry_counter <=
					...
					data->init_local_state =
						INIT_PDC_RESET /* INIT_PDC_ENABLE */;
				} else {
					set_state(data, ST_DISABLE);
				}
				return SMF_EVENT_HANDLED;
...
static void handle_irqs(struct pdc_data_t *data)
{
	...
	for (int i = 0; i < pdc_power_mgmt_get_usb_pd_port_count(); i++) {
		...
		if (pdc_int_data->rd_buf[0] & IT52XX_INT_UCSI_STATUS) {
			if (get_state(pdc_int_data) == ST_TASK_WAIT) {
				...
				rv = it52xx_clr_alert_status(pdc_int_data->dev);
				if (rv) {
	...
	for (int i = 0; i < pdc_power_mgmt_get_usb_pd_port_count(); i++) {
		...
		for (int j = 0; j < pdc_power_mgmt_get_usb_pd_port_count();
			...
			if ((ara >> 1) == cfg->i2c.addr) {
				LOG_INF("ST%d: ITE IRQ", cfg->connector_number);

				/* Found pending interrupt, handle it */
				/* Get interrupt status */
				rv = it52xx_get_alert_status(pdc_int_data->dev);
				if (rv) {
					LOG_ERR("ST%d: Get interrupt status failed", cfg->connector_number);
					/* TODO: Refer to st_ping_status_entry & run() but not do retry, not verified */
					/* Clear the CCI Event */
					pdc_int_data->cci_event.raw_value = 0;
					/*
...
static enum smf_state_result st_ping_status_run(void *o)
{
	...
	case CMD_DONE:
		...
		if (data->cmd == CMD_PPM_RESET) {
			...
			LOG_INF("ST%d: ping_status (CCI) { 0x%x, 0x%x, 0x%x, 0x%x, 0x%x }",
				cfg->connector_number,
		...
		break;
	case CMD_ERROR:
		LOG_ERR("ST%d: Ping Status Error", cfg->connector_number);
		/*
		 * The command was not successfully completed,
		 * so set cci.error to 1b.
		 */
	...
	default:
		...
		LOG_ERR("ST%d: unknown ping_status (CCI) { 0x%x, 0x%x, 0x%x, 0x%x, 0x%x }",
				cfg->connector_number,
				data->ping_status.raw_value[0],
				data->ping_status.raw_value[1],
				data->ping_status.raw_value[2],
...
static int it52xx_clr_alert_status(const struct device *dev)
{
	...
	//data->cmd = CMD_CLR_ALERT_STATUS; shouldn't overwrite (keep last cmd)
...
#if 0 /* Not UCSI cmd, it5271 not support */
static int it52xx_pdc_enable(const struct device *dev)
{
	struct pdc_data_t *data = dev->data;
			  ...
			  enum pdo_source_t source, uint32_t *pdos)
{
	...
	get_pdo->connector_number = cfg->connector_number + 1; //set 1, it5271 support only one port
...
static DEVICE_API(pdc, pdc_driver_api) = {
	...
	.reset = it52xx_pdc_reset, //not ucsi cmd, reset lpm, it5271 not support
	...
	.get_info = it52xx_get_info, //not ucsi cmd, add support by [CONTROL] VDM
...
static int pdc_init(const struct device *dev)
{
	...
	/* Set initial state */
	data->init_local_state = INIT_PDC_RESET; /* TODO: original is INIT_PDC_ENABLE */
