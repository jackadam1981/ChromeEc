...
static enum smf_state_result st_init_run(void *o)
{
	...
	case INIT_ERROR:
		...
		//it52xx_get_error_status(data->dev, &data->es); //not support,already report in [CCI]
...
{
	const struct pdc_config_t *cfg = data->dev->config;
	struct pdc_data_t *const pdc_int_data = pdc_data[cfg->connector_number]; //TODO: delete, and change below pdc_int_data to data
	int rv;

	LOG_INF("ST%d: IRQ", cfg->connector_number);

	/* Found pending interrupt and handle it */
	rv = it52xx_get_alert_status(data->dev);
	if (rv) {
	...
	if (pdc_int_data->rd_buf[0] & IT52XX_INT_UCSI_STATUS) {
		if (get_state(data) == ST_TASK_WAIT) {
			LOG_INF("CC");
...
static int it52xx_clr_alert_status(const struct device *dev)
{
	...
	//pdc_int_data->cmd = CMD_CLR_ALERT_STATUS; shouldn't overwrite (keep last cmd)
				      ...
				      union connector_status_t *cs)
{
	...
	uint8_t payload[] = {
		...
		0x00, /* Data Length --> set to 0x00 */
		(cfg->connector_number + 1), /* Connector number --> start from 1 */
		0x00,
		0x00,
		0x00,
		0x00,
			 ...
			 uint32_t *pdo, int count)
{
	...
	pdo_info = (cfg->connector_number + 1) | (type << 10) | ((count & 0xF) << 11) |
		   (END_OF_MESSAGE << 22);
...
static DEVICE_API(pdc, pdc_driver_api) = {
	...
	.get_error_status = it52xx_get_error_status, /* not support */
