...
static void call_cci_event_cb(struct pdc_data_t *data)
{
	...
	/* Report CCI flags to PPM, this value may not be the same as [CCI] of LPM replied. ex. ACK_CC_CI */
	LOG_INF("ST%d: CCI=0x%x", cfg->connector_number, cci.raw_value);
...
static bool max_i2c_retry_reached(struct pdc_data_t *data, int type)
{
	...
	if (data->i2c_transaction_retry_counter > N_I2C_TRANSACTION_COUNT) {
		...
		LOG_ERR("ST%d: %s i2c error", cfg->connector_number,
...
static int get_ping_status(const struct device *dev)
{
	...
	msg[1].buf = data->ping_status.raw_value; /* original 1byte (read rtk reg), we need 5byte space */
...
static void init_display_error_status(struct pdc_data_t *data)
{
	...
	if (data->es.unrecognized_command) {
		LOG_ERR("ST%d: Unrecognized Command", cnum);
	}

	if (data->es.non_existent_connector_number) {
		LOG_ERR("ST%d: Invalid Connector Number", cnum);
	}

	if (data->es.invalid_command_specific_param) {
		LOG_ERR("ST%d: Invalid Param", cnum);
	}

	if (data->es.incompatible_connector_partner) {
		LOG_ERR("ST%d: Invalid Connector Partner", cnum);
	}

	if (data->es.cc_communication_error) {
		LOG_ERR("ST%d: CC Comm Error", cnum);
	}

	if (data->es.cmd_unsuccessful_dead_batt) {
		LOG_ERR("ST%d: Dead Batt Error", cnum);
	}

	if (data->es.contract_negotiation_failed) {
		LOG_ERR("ST%d: Contract Negotiation Failed", cnum);
	}
}
...
static enum smf_state_result st_init_run(void *o)
{
	...
	case INIT_PDC_SET_PPM_RESET: /* flow ppm reset -> set ntf (spec): it5271 ignore cmd that prior ppm_reset */
       ...
       */
		...
		if (rv) {
			LOG_ERR("ST%d: Internal(INIT_PDC_RESET)", cnum);
		...
		if (rv) {
			LOG_ERR("ST%d: Internal(INIT_PDC_SET_PPM_RESET)", cnum);
			set_state(data, ST_DISABLE);
			return SMF_EVENT_HANDLED;
		}
	...
	case INIT_PDC_SET_NOTIFICATION_ENABLE:
		...
		if (rv) {
			LOG_ERR("ST%d: Internal(INIT_PDC_SET_NOTIFICATION_ENABLE)",
				cnum);
			set_state(data, ST_DISABLE);
			return SMF_EVENT_HANDLED;
		}
	...
	case INIT_PDC_SET_ACK_CC_CI_TO_NTF: /* it5271 is waiting ACK and ignore other [CONTROL] cmd */
		...
		if (rv) {
			LOG_ERR("ST%d: Internal(INIT_PDC_SET_ACK_CC_CI_TO_NTF)", cnum);
			set_state(data, ST_DISABLE);
			return SMF_EVENT_HANDLED;
		}
		init_write_cmd_and_change_state(data, INIT_PDC_GET_VDM_IC_STATUS);
		return SMF_EVENT_HANDLED;
	case INIT_PDC_GET_VDM_IC_STATUS: /* 1. Not UCSI cmd, so can't run sm: [ST_WRITE] -> [PING_STATUS] -> [ST_READ] */
		...
		if (rv) {
			LOG_ERR("ST%d: Internal(INIT_PDC_GET_VDM_IC_STATUS)", cnum);
			set_state(data, ST_DISABLE);
			return SMF_EVENT_HANDLED;
		}
	...
	case INIT_PDC_SET_ACK_CC_CI_TO_VDM: /* it5271 is waiting ACK and ignore other [CONTROL] cmd  */
		rv = it52xx_ack_cc_ci(data->dev, ci, cc, 0);
		if (rv) {
			LOG_ERR("ST%d: Internal(INIT_PDC_SET_ACK_CC_CI_TO_VDM)", cnum);
			set_state(data, ST_DISABLE);
			return SMF_EVENT_HANDLED;
		}
	...
	case INIT_PDC_CMD_WAIT:
		...
		if (data->cci_event.error) {
			...
			if (data->error_status.pdc_internal_error) {
				LOG_INF("ST%d: PDC not responding",
					cfg->connector_number);
			...
			if (data->init_local_current_state == INIT_ERROR) {
				LOG_INF("ST%d: PDC error status read fail ",
					cfg->connector_number);
				set_state(data, ST_ERROR_RECOVERY);
				return SMF_EVENT_HANDLED;
			}
			...
			//LOG_INF("ST1: INIT_PDC_CMD_WAIT set st");
...
static void handle_irqs(struct pdc_data_t *data)
{
	...
	for (int i = 0; i < pdc_power_mgmt_get_usb_pd_port_count(); i++) {
		...
		if (rv) {
			LOG_ERR("ST%d: ITE IRQ Get interrupt status failed", cfg->connector_number);
			/* TODO: Refer to st_ping_status_entry & run() but not do retry, not verified */
			/* Clear the CCI Event */
		...
		if (interrupt_status & IT52XX_INT_UCSI_STATUS) {
			if (get_state(pdc_int_data) == ST_TASK_WAIT) {
				LOG_INF("ST%d: ITE IRQ-CC", cfg->connector_number);
				...
				if (rv) {
					LOG_ERR("ST%d: Clear interrupt status failed", cfg->connector_number);
				...
				set_state(pdc_int_data, ST_PING_STATUS);
				/* done with this port */
				break;
			} else {
				LOG_INF("ST%d: ITE IRQ-CI", cfg->connector_number);
			...
			LOG_INF("ST%d: ITE IRQ-unknown event", cfg->connector_number);
...
static enum smf_state_result st_write_run(void *o)
{
	...
	/* TODO: need T_PING_STATUS delay to avoid starvation? */
...
static void st_ping_status_entry(void *o)
{
	...
	for (int i = 0; i < PDC_MAX_CCI_LENGTH; i++) {
		data->ping_status.raw_value[i] = 0;
	}
	/* Clear the CCI Event */
	data->cci_event.raw_value = 0;
}
...
static enum smf_state_result st_ping_status_run(void *o)
{
	...
	//case CMD_DEFERRED:
		...
		if (data->ping_retry_counter > N_RETRY_COUNT) {
			...
			LOG_ERR("ST%d: Failed to read Ping Status",
				cfg->connector_number);
			/*
			 * The command was not successfully completed,
			 * so set cci.error to 1b.
	...
	case CMD_DONE:
		...
		if (data->cmd == CMD_PPM_RESET) {
			...
			/* Notify system of status change */
			call_cci_event_cb(data);
			LOG_INF("ST%d: ITE PDC reset complete",
				cfg->connector_number);
			/* All done, return to Init or Idle state */
			TRANSITION_TO_INIT_OR_IDLE_STATE(data);
		} else {
			LOG_INF("ST%d: [CCI] = { 0x%x, 0x%x, 0x%x, 0x%x, 0x%x }",
				cfg->connector_number,
		...
		break;
	case CMD_ERROR:
		LOG_ERR("ST%d: [CCI] Error indicator", cfg->connector_number);
		/*
		 * The command was not successfully completed,
		 * so set cci.error to 1b.
		 */
	...
	default:
		...
		LOG_ERR("ST%d: unknown [CCI] = { 0x%x, 0x%x, 0x%x, 0x%x, 0x%x }",
				cfg->connector_number,
				data->ping_status.raw_value[0],
				data->ping_status.raw_value[1],
				data->ping_status.raw_value[2],
	...
	case CMD_GET_SBU_MUX_MODE: {
		...
		default:
			*mode_out = PDC_SBU_MUX_MODE_INVALID;
			LOG_ERR("ST%d: Unknown raw SBU mux value: 0x%02x",
...
#if 0 //not ucsi cmd, need it5271 support? Notify the PDC of the current AP power state and then?
...
#if 0 //not ucsi cmd, need it5271 support? set it5271 DRP/ trysrc DRP (default)/ trysnk DRP
...
#if 0 //not ucsi cmd, need it5271 support?
...
static int it52xx_get_vdo(const struct device *dev, union get_vdo_t vdo_req,
			 uint8_t *vdo_req_list /* = mgmt vdo_type -> VDO_PD_DP_CFG... */, uint32_t *vdo)
{
	struct pdc_data_t *data = dev->data;
	uint16_t vdo_type = 0;
...
#if 0 //GET ALTERNATE MODE cmd of UCSI
	uint8_t byte5, byte6, byte7, byte8 = 0;

	/* bit[16:23]: recipient 3'b, reserved 5'b */
	byte5 = vdo_req.vdo_origin;
	/* bit[24:31]: connector num 7'b, reserved 1'b */
	byte6 = (IT5271_CONNECTOR_NUMBER & 0x7f);
	/* bit[32:39]: alternate mode offset 8'b */
	byte7 = 0;
	/* bit[40:47]: number of alternate mode 2'b, reserved 6'b */
	byte8 = vdo_req.num_vdos;
...
static int it52xx_set_bbr_cts(const struct device *dev, bool enable)
{
	...
	LOG_INF("ST%d: SET_BBR_CTS = %d", cfg->connector_number, enable);
...
static DEVICE_API(pdc, pdc_driver_api) = {
	...
	.reset = it52xx_pdc_reset, //not ucsi cmd, reset lpm
	.connector_reset = it52xx_connector_reset, //[CONTROL] connector reset: Hard/Data Reset
	.get_capability = it52xx_get_capability,
	.get_connector_capability = it52xx_get_connector_capability,
	.set_ccom = it52xx_set_ccom,
	//.set_drp_mode = it52xx_set_drp_mode, //not ucsi cmd, need it5271 support? it5271 default is trySRC DRP by build
	.get_drp_mode = it52xx_get_drp_mode, //not ucsi cmd, need it5271 support?
	...
	.get_rdo = it52xx_get_rdo, //not ucsi cmd & not optional, support by [CONTROL] VDM
	.set_rdo = it52xx_set_rdo, //not ucsi cmd & not optional, support by [CONTROL] VDM
	.get_error_status = it52xx_get_error_status,
	.get_vbus_voltage = it52xx_get_vbus_voltage,
	.get_current_pdo = it52xx_get_current_pdo, //not ucsi cmd, need it5271 support?
	.set_handler_cb = it52xx_set_handler_cb, /* call mgmt: pdc_cc_handler_cb() */
	.read_power_level = it52xx_read_power_level,
	.get_info = it52xx_get_info, //not ucsi cmd, support by [CONTROL] VDM
	.get_hw_config = it52xx_get_hw_config, //not ucsi cmd
	.set_power_level = it52xx_set_power_level,
	.reconnect = it52xx_reconnect, //not ucsi cmd, need it5271 support?
	.update_retimer = it52xx_set_retimer_update_mode,
	.get_cable_property = it52xx_get_cable_property,
	.get_vdo = it52xx_get_vdo, //not UCSI cmd, support by [CONTROL] VDM
	.get_identity_discovery = it52xx_get_identity_discovery, //call to rts54_get_rtk_status()
	.set_comms_state = it52xx_set_comms_state, //communicate control
	.is_vconn_sourcing = it52xx_is_vconn_sourcing, //call to rts54_get_rtk_status()
	.set_pdos = it52xx_set_pdo,
	.get_pch_data_status = it52xx_get_pch_data_status, //not ucsi cmd , it5271 not support
	.execute_ucsi_cmd = it52xx_execute_ucsi_cmd,
	.manage_callback = it52xx_manage_callback, /* call mgmt: pdc_ci_handler_cb() */
	.ack_cc_ci = it52xx_ack_cc_ci,
	.get_lpm_ppm_info = it52xx_get_lpm_ppm_info,
	//.set_frs = it52xx_set_frs, //not ucsi cmd, need it5271 support?
	...
	//.set_ap_power_state = it52xx_set_ap_power_state, //not ucsi cmd, need it5271 support? what purpose & need support?
	//.set_battery_capability = rts54_set_battery_capability, //not ucsi cmd, need it5271 support?
	//.set_battery_status = rts54_set_battery_status, //not ucsi cmd, need it5271 support?
};
...
static int pdc_init(const struct device *dev)
{
	...
	LOG_INF("ST%d: ITE it52xx PDC DRIVER", cfg->connector_number);
...
static void it52xx_thread(void *dev, void *unused1, void *unused2)
{
	...
	while (1) {
		...
		if (((get_state(data) == ST_IDLE) || (get_state(data) == ST_TASK_WAIT)) &&
			...
			irq_pending_for_idle = false;
			if (check_comms_suspended()) {
				LOG_INF("ST%d: Ignoring interrupt",
					cfg->connector_number);
				continue;
			}
			handle_irqs(data);
