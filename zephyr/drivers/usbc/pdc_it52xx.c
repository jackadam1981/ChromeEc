...
static const struct smbus_cmd_t GET_IC_STATUS = { 0xB6 };
...
static enum smf_state_result st_init_run(void *o)
{
	...
	case INIT_PDC_ENABLE:
		...
		return SMF_EVENT_HANDLED;
#endif
	case INIT_PDC_GET_IC_STATUS: /* TODO: */
				     /* 1. Not UCSI cmd (it5271 not support, hardcode), so can't like rtk fit to sm: [ST_WRITE] -> [PING_STATUS] -> [ST_READ] */
				     /* 2. it5271 add support: [CONTROL] VDM to get pdc_info_t data (LPM_PPM info not enough data), before/after set PPM_RESET? */
#if 0
		rv = it52xx_get_info(data->dev, &data->info, true);
		if (rv) {
			LOG_ERR("ST%d: Internal(INIT_PDC_GET_IC_STATUS)", cnum);
			set_state(data, ST_DISABLE);
			return SMF_EVENT_HANDLED;
		}
	...
	case INIT_PDC_SET_NOTIFICATION_ENABLE:
		...
		init_write_cmd_and_change_state(data, INIT_PDC_SET_ACK_CC_CI);
		return SMF_EVENT_HANDLED;
	case INIT_PDC_SET_ACK_CC_CI: /* need ACK, it5271 is waiting and ignore other [CONTROL] cmd */
		union conn_status_change_bits_t ci;
		bool cc = 1;
		ci.connect_change = 0;
		rv = it52xx_ack_cc_ci(data->dev, ci, cc, 0);
		if (rv) {
			LOG_ERR("ST%d: Internal(INIT_PDC_SET_ACK_CC_CI)", cnum);
			set_state(data, ST_DISABLE);
			return SMF_EVENT_HANDLED;
		}
	...
	case INIT_PDC_CMD_WAIT:
		LOG_ERR("C1: INIT_PDC_CMD_WAIT, RC %d", data->cci_event.reset_completed);
		...
		if (data->cci_event.error) {
			...
			LOG_INF("C1: INIT_PDC_CMD_WAIT set st");
	...
	for (int i = 0; i < pdc_power_mgmt_get_usb_pd_port_count(); i++) {
		...
		for (int j = 0; j < pdc_power_mgmt_get_usb_pd_port_count();
			...
			if ((ara >> 1) == cfg->i2c.addr) {
				LOG_INF("ST%d: IRQ", cfg->connector_number);
...
static enum smf_state_result st_read_run(void *o)
{
	...
	case CMD_GET_IC_STATUS: {
		...
		if (data->init_local_state != INIT_PDC_COMPLETE) {
			LOG_INF("ST%d: ITE: FW Version: %u.%u.%u (%s)",
				...
				info->project_name);
			LOG_INF("ST%d: ITE: PD Version: %u, Rev %u",
	...
	case CMD_GET_ERROR_STATUS: {
		...
		/* ITE GET_ERROR_STATUS bits are equal to UCSI GET_ERROR_STATUS bits */
		es->raw_value = ((data->rd_buf[2] << 8) | data->rd_buf[1]);
		//TODO: we reply 16bytes, check 14bytes vendor defined error with RD
#if 0
...
static int it52xx_clr_alert_status(const struct device *dev)
{
	...
	//TODO: directly set payload to msg.buf (not through data->wr_buf_len) => can't, payload is local var
			  ...
			  bool live)
{
	...
	/* TODO & HACK: fillup info by [CONTROL] VDM (it5271 need to support)*/
	data->info.fw_version = 0x0012;
	data->info.vid = 0x048D;
	data->info.pid = 0x0000;
	return 0;
