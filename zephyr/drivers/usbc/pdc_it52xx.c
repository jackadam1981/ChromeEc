...
/**
 * @brief Time before sending a get [CCI] message
 */
...
struct pdc_data_t {
	...
	/** Get [CCI] message retry counter */
	uint8_t get_cci_retry_counter;
...
static enum smf_state_result st_write_run(void *o)
{
	...
	/* TODO: need T_GET_CCI_MSG delay to avoid starvation? */
...
static void st_get_cci_entry(void *o)
{
	...
	/* Clear I2c Transaction Retry Counter */
	data->i2c_transaction_retry_counter = 0;
	/* Clear Get [CCI] Message Rety Counter */
	data->get_cci_retry_counter = 0;
	/* Clear [CCI] Message */
...
static enum smf_state_result st_get_cci_run(void *o)
{
	...
	//case CMD_DEFERRED:
		 ...
		 * Stay here and resend get [CCI] message.
		 */
		data->get_cci_retry_counter++;
		if (data->get_cci_retry_counter > N_RETRY_COUNT) {
			/* MAX get [CCI] message retries exceeded */
			LOG_ERR("ITE%d: Failed to get [CCI] message",
				cfg->connector_number);
			/*
			 * The command was not successfully completed,
			 * so set cci_event.error to 1b.
			 ...
			 * for the first time.
			 */
			if (data->get_cci_retry_counter == 1) {
...
static DEVICE_API(pdc, pdc_driver_api) = {
	...
	//.execute_ucsi_cmd = it52xx_execute_ucsi_cmd, //optional api
