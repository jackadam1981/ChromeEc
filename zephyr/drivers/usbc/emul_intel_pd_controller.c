...
#include <zephyr/drivers/usbc/emul_intel_pd_controller.h>
...
static int intel_pd_controller_emul_transfer(const struct emul *emul,
				             ...
				             int addr)
{
	const struct intel_pd_controller_emul_config *cfg = emul->config;
	...
	if (num_msgs == 1) {
		...
		if (data->control.i2c_int_ack) {
			gpio_emul_input_set(int_gpio->port, int_gpio->pin, 1);
		}
	} else if (num_msgs == 2) {
		if (!(((msgs[0].flags & I2C_MSG_RW_MASK) == I2C_MSG_WRITE) &&
		    (msgs[0].len == 1) &&
					...
					const union data_status_reg status)
{
	...
	if (!cfg->shared_irq) {
		gpio_emul_input_set(int_gpio->port, int_gpio->pin, 0);
	}
	return 0;
}
