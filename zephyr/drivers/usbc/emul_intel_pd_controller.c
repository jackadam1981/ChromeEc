...
static int intel_pd_controller_emul_connect_data(const struct emul *emul)
{
	struct intel_pd_controller_emul_data *data = emul->data;
...
static int intel_pd_controller_emul_set_connect_orient(const struct emul *emul,
						       const uint8_t orient)
{
	struct intel_pd_controller_emul_data *data = emul->data;

	data->status.conn_ori = !!orient;
...
static int intel_pd_controller_emul_connect_usb2(const struct emul *emul)
{
	struct intel_pd_controller_emul_data *data = emul->data;
...
static int intel_pd_controller_emul_connect_usb3_2(const struct emul *emul)
{
	struct intel_pd_controller_emul_data *data = emul->data;
...
static int intel_pd_controller_emul_set_usb3_2_speed(const struct emul *emul,
						     const uint8_t speed)
{
	struct intel_pd_controller_emul_data *data = emul->data;

	data->status.usb3_2_speed = !!speed;
...
static int intel_pd_controller_emul_connect_dp(const struct emul *emul)
{
	struct intel_pd_controller_emul_data *data = emul->data;
...
static int intel_pd_controller_emul_reset(const struct emul *emul)
{
	struct intel_pd_controller_emul_data *data = emul->data;
...
static int intel_pd_controller_emul_set_dp_irq(const struct emul *emul)
{
	struct intel_pd_controller_emul_data *data = emul->data;
...
static int intel_pd_controller_emul_set_hpd_lvl(const struct emul *emul)
{
	struct intel_pd_controller_emul_data *data = emul->data;
...
static int intel_pd_controller_emul_set_reg_value(const struct emul *emul, uint16_t reg,
			     const uint8_t *value, const uint8_t len)
{
	struct intel_pd_controller_emul_data *data = emul->data;
	uint8_t cpy_len;

	if (reg != INTEL_ALTMODE_REG_DATA_STATUS) {
		return -ENXIO;
	}

	cpy_len = len <= INTEL_ALTMODE_DATA_STATUS_REG_LEN ? len :
		INTEL_ALTMODE_DATA_STATUS_REG_LEN;

	memcpy(&data->status, value, cpy_len);
...
struct emul_intel_pd_controller_backend_api
	intel_pd_controller_emul_backend_api = {
		...
		.set_reg_value = intel_pd_controller_emul_set_reg_value,
		.reset = intel_pd_controller_emul_reset,
	};
