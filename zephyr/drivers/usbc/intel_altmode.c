...
static int rt_smbus_read_reg(rt_smbus_cmd cmd, uint8_t *data, uint8_t len)
{
	...
	rv = i2c_xfer_unlocked(I2C_PORT_TYPEC_AIC_1, 0x69, NULL, 0, data, len,
				I2C_XFER_STOP);
	i2c_lock(I2C_PORT_TYPEC_AIC_1, 0);
	if (rv) {
		return rv;
...
static int rt_smbus_get_resp(uint8_t *resp, uint8_t len)
{
	int rv;
	uint8_t cmd = 0x80;
...
static void init_drp(void)
{
	...
	uint8_t cmd2[] = RT_SMBUS_CMD_SET_NOTIF_ENABLE;
	uint8_t resp_len;
	int rv;
