	...
	case CMD_DONE:
		...
		if (data->cci_cb) {
			if (data->cmd == CMD_PPM_RESET) {
				 ...
				 * data is available.
				 */
				data->cci_event.data_len = data->ping_status.data_len;
				/*
...
static void st_read_run(void *o)
{
	...
	case CMD_GET_CONNECTOR_STATUS:
		 ...
		 */
		cs->conn_status_change.raw_value = data->rd_buf[1] << 16 |
						   data->rd_buf[2];
...
static int rts54_get_capability(const struct device *dev, struct device_capability_t *caps)
{
	struct pdc_data_t *data = dev->data;
...
static int rts54_get_error_status(const struct device *dev, struct error_status_t *es)
{
	struct pdc_data_t *data = dev->data;
...
static int rts54_is_pd_ready(const struct device *dev, uint8_t *result)
{
	if (result == NULL) {
		return -EINVAL;
	}

	return rts54_get_ic_status(dev, 8, 1, CMD_IS_PD_READY, result);
}

static int rts54_is_typec_connected(const struct device *dev, uint8_t *result)
{
	if (result == NULL) {
		return -EINVAL;
	}

	return rts54_get_ic_status(dev, 8, 1, CMD_IS_TYPEC_CONNECTED, result);
}
...
static int rts54_get_vid_pid(const struct device *dev, uint32_t *vid_pid)
{
	...
	return rts54_get_ic_status(dev, 9, 4, CMD_GET_VIDPID,(uint8_t *)vid_pid);
}
...
static int rts54_get_vbus_current(const struct device *dev, uint16_t *current)
{
	...
	return rts54_get_rtk_status(dev, 14, 2, CMD_GET_VBUS_CURRENT, (uint8_t *)current);
}
...
static int rts54_get_alternate_mode(const struct device *dev, enum sop_t sop, uint8_t alt_mode_offset, uint8_t num_alt_modes, struct alt_mode_t *alt_modes)
{
	struct pdc_data_t *data = dev->data;
...
static const struct pdc_driver_api_t pdc_driver_api = {
	...
	.is_pd_ready = rts54_is_pd_ready,
	.is_typec_connected = rts54_is_typec_connected,
...
static void create_thread(const struct device *dev)
{
	...
	data->thread = k_thread_create(
		...
		K_THREAD_STACK_SIZEOF(thread_stack_area), run_driver, (void *)dev,
