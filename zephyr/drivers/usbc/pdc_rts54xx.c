...
struct pdc_data_t {
	...
	/** True if PDC is executing from flash, else False if executing from ROM */
...
/**
 * @breif List of human readable state names for console debugging
 */
...
static int rts54_get_ic_status(const struct device *dev, uint8_t offset, uint8_t len, enum cmd_t cmd);
...
static void st_read_run(void *o)
{
	...
	case CMD_GET_VBUS_VOLTAGE:
		*(uint16_t *)data->user_buf = (data->rd_buf[1] << 8 | data->rd_buf[0]) * 50;
		break;
	...
	case CMD_GET_ERROR_STATUS:
		...
		/* Realtek Command unsuccessful Dead Battery condition, Byte 1, Bit 5 */
	...
	case IRQ_READ_ARA:
		...
		if (rv == 0) {
			...
			if (data->irq_counter == 4) {
				/*
				 * Can't read from the ARA, so inform the subsystem of the problem
				 * by setting the cci_event.vendor_defined_indicator and cci_event.error.
				 */
				/* Clear the CCI Event */
				data->cci_event.raw_value = 0;
				/* Set the port the CCI Event occurred on */
...
static int rts54_set_handler_cb(const struct device *dev, pdc_cci_handler_cb_t cci_cb, void *cb_data)
{
	struct pdc_data_t *data = dev->data;
...
static int rts54_set_power_level(const struct device *dev, enum usb_typec_current_t tcc)
{
	struct pdc_data_t *data = dev->data;
	uint8_t byte = 0;
