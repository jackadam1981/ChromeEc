...
static int rts54_i2c_write(const struct device *dev)
{
	...
	LOG_HEXDUMP_DBG(data->wr_buf, data->wr_buf_len, "Write PDC command:");
...
static void st_read_run(void *o)
{
	...
	case CMD_GET_SBU_MUX_MODE: {
		/* This is parsing an extended GET_IC_STATUS response. Ensure
		 * that the PDC firmware supports the extra mux state byte
		 * before parsing the response. */

		if (len < RTS54XX_GET_IC_STATUS_SBU_MUX_MODE_OFFSET) {
			/* Report an error with this command */
			data->cci_event.error = 1;
			data->cci_event.command_completed = 1;
			data->error_status.invalid_command_specific_param = 1;
		...
		uint8_t raw_mode =
			data->rd_buf[RTS54XX_GET_IC_STATUS_SBU_MUX_MODE_OFFSET];
				  ...
				  enum pdc_sbu_mux_mode *mode)
{
	...
	/* SBU mux mode is encoded in an extended GET_IC_STATUS response */
	uint8_t payload[] = {
		GET_IC_STATUS.cmd, GET_IC_STATUS.len, 0, 0x00, 39,
	};
				  ...
				  enum pdc_sbu_mux_mode mode)
{
	...
	/* TODO: send correctly formatted command and set buffer size */
	switch (mode) {
