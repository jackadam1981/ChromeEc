...
/**
 * @brief Each port has its own ST_IRQ state. When an interrupt is pending, one
 * of the ST_IRQ states will execute and handle all pending interrupts for all
 * ports. If another ST_IRQ state is executed, it will return because there will
 * be no pending interrupts.
 */
		...
		if (rv == 0) {
			...
			for (int j = 0; j < CONFIG_USB_PD_PORT_MAX_COUNT; j++) {
				...
				if ((ara >> 1) == cfg->i2c.addr) {
					 ...
					 * on */
					dat->cci_event.connector_change =
						cfg->connector_number;
					/* Set the interrupt event */
					dat->cci_event.vendor_defined_indicator =
						1;
					/* Notify system of status change */
