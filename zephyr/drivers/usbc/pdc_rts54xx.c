...
static struct {
	...
	gpio_pin_t pin;
} irq_list[DT_NUM_INST_STATUS_OKAY(DT_DRV_COMPAT)];
...
static void handle_irqs(struct pdc_data_t *data)
{
	...
	int rv;
	struct i2c_dt_spec i2c =
		((const struct pdc_config_t *)data->dev->config)->i2c;
...
static int pdc_init(const struct device *dev)
{
	...
	for (int i = 0; i < ARRAY_SIZE(irq_list); i++) {
		if (irq_list[i].port == cfg->irq_gpios.port &&
		    irq_list[i].pin == cfg->irq_gpios.pin) {
			irq_init_done = true;
			break;
		}

		if (irq_list[i].port == NULL) {
			irq_list[i].port = cfg->irq_gpios.port;
			irq_list[i].pin = cfg->irq_gpios.pin;
			break;
		}
	}
