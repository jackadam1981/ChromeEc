...
struct pdc_data_t {
	...
	uint8_t rd_buf_len;
	/** Internal buffer used during initialization */
	uint8_t internal_buf[PDC_MAX_DATA_LENGTH];
...
static void print_current_state(struct pdc_data_t *data)
{
	...
	if (st == ST_WRITE) {
		LOG_INF("ST(%d): %s %s", cfg->connector_number, state_names[st], cmd_names[data->cmd]);
	} else {
		LOG_INF("ST(%d): %s", cfg->connector_number, state_names[get_state(data)]);
	}
}
...
static void call_cci_event_cb(struct pdc_data_t *data)
{
	...
	if (data->cci_cb) {
		LOG_INF("C%d: cci_event_cb event=0x%x", cfg->connector_number, data->cci_event.raw_value);
...
static void st_init_entry(void *o)
{
	...
	/* Use internal buffer during initializtion */
	data->user_buf = data->internal_buf;
...
static void st_ping_status_run(void *o)
{
	...
	case CMD_DEFERRED:
		...
		if (data->ping_retry_counter > N_RETRY_COUNT) {
			...
			if (data->cci_event.busy == 0) {
				/* Only notify subsystem of busy event onece */
	...
	case CMD_DONE:
		if (data->cmd == CMD_PPM_RESET) {
			...
			/* Notify system of status change */
			call_cci_event_cb(data);
			LOG_DBG("C%d: Realtek PDC reset complete", cfg->connector_number);
...
static void st_read_run(void *o)
{
	...
	case CMD_GET_IC_STATUS: {
		...
		if (data->init_local_state != INIT_PDC_COMPLETE) {
			LOG_INF("C%d: Realtek: FW Version: %04x", cfg->connector_number, info->fw_version);
			LOG_INF("C%d: Realtek: PD Version: %04x, Rev %04x",
				cfg->connector_number, info->pd_version, info->pd_revision);
		}
		break;
	}
		...
		if (rv == 0) {
			...
			for (int j = 0; j < CONFIG_USB_PD_PORT_MAX_COUNT; j++) {
				...
				if ((ara >> 1) == cfg->i2c.addr) {
					...
					/* Clear the CCI Event */
					dat->cci_event.raw_value = 0;
					/* Set the port the CCI Event occurred on */
					dat->cci_event.connector_change = cfg->connector_number;
					/* Set the interrupt event */
					dat->cci_event.vendor_defined_indicator = 1;
					/* Notify system of status change */
					call_cci_event_cb(dat);
					/* done with this port */
					break;
				}
			}
