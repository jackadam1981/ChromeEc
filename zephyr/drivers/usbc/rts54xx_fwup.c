...
INCBIN(rts54xx_fw_00, STRINGIFY(RTS54XX_FW_ROOT) "/rts54xx_00");
INCBIN(rts54xx_fw_01, STRINGIFY(RTS54XX_FW_ROOT) "/rts54xx_01");
#else
INCBIN(rts54xx_fw_00, STRINGIFY(RTS54XX_FW_ROOT) "/zero.bin");
INCBIN(rts54xx_fw_01, STRINGIFY(RTS54XX_FW_ROOT) "/zero.bin");
#endif
...
static int decompress_fw(int index)
{
	int size = 0;
	if (index == 0)
		size = lzav_decompress(g_rts54xx_fw_00_data, fw_bin,
				       g_rts54xx_fw_00_size, 64 * 1024);
	else
		size = lzav_decompress(g_rts54xx_fw_01_data, fw_bin,
				       g_rts54xx_fw_01_size, 64 * 1024);
	return size;
}
...
static int rts545x_flash_write(/*const uint8_t *image, size_t image_size*/)
{
	...
	for (int i = 0; i < 2; i++) {
		...
		LOG_INF("Decompressed FW idx %d, size: %d", i,
			decompress_fw(i));
		while (offset < image_size * (i + 1)) {
			...
			size = MIN(FW_CHUNK_SIZE,
				   MIN(seg_boundary, image_size * (i + 1)) -
					   offset);
			...
			memcpy(&flash_write[DATA_OFF],
			       &fw_bin[offset - i * image_size], size);
