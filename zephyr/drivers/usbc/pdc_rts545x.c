...
static int rts545x_flash_write(const struct shell *sh, size_t argc, char **argv)
{
	...
	while (offset < grts545x_fw_size) {
		...
		flash_write[BYTE_COUNT_OFF] = size + 3;
...
static int rts545x_flash_erase(const struct shell *sh, size_t argc,
			       char **argv)
{
	char *s_dev_name = argv[ARGV_DEV];
	const struct device *dev;
	uint8_t flash_erase[] = { 0x03, 0xDA, 0x0B, 0x00 };
	uint8_t ping_status;
	int ret;
