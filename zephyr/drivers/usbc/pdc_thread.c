...
int pdc_thread_wait_for_idle(uint32_t timeout_ms)
{
	const uint32_t sleep_interval_ms = 50;
	uint32_t cnt = timeout_ms / sleep_interval_ms;
	uint32_t remain_ms = timeout_ms % sleep_interval_ms;

#ifdef CONFIG_ZTEST
	LOG_INF("Wait for %d ms for idle", timeout_ms);
#endif

	while (!pdc_thread_is_idle() && cnt--) {
		k_msleep((cnt > 0 ? sleep_interval_ms : remain_ms));
	}
	return pdc_thread_is_idle() ? 0 : -ETIMEDOUT;
}
