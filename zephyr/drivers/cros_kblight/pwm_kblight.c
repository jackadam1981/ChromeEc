...
BUILD_ASSERT(DT_NUM_INST_STATUS_OKAY(DT_DRV_COMPAT) == 1,
	     "Only one instance of cros-ec,kblight-pwm should be defined.");

#define KBLIGHT_PWM_DEV DEVICE_DT_GET(DT_INST_PWMS_CTLR(0))
...
static void kblight_pwm_set_duty(int percent)
{
	...
	LOG_DBG("LED PWM %s set percent (%d), pulse %d", KBLIGHT_PWM_DEV->name,
		percent, pulse_us);

	rv = pwm_pin_set_usec(KBLIGHT_PWM_DEV, KBLIGHT_PWM_CHANNEL,
			      KBLIGHT_PWM_PERIOD_US, pulse_us,
			      KBLIGHT_PWM_FLAGS);
	if (rv) {
		LOG_ERR("pwm_pin_set_usec() failed %s (%d)",
			KBLIGHT_PWM_DEV->name, rv);
	}
}
