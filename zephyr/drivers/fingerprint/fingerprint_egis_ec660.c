...
#define DT_DRV_COMPAT egis_ec660
...
/* The 16-bit hardware ID is 0x02d1 */
...
{
	struct ec660_data *data = dev->data;
...
static inline int ec660_enable_irq(const struct device *dev)
{
	const struct ec660_cfg *cfg = dev->config;
	int rc;
...
static inline int ec660_disable_irq(const struct device *dev)
{
	const struct ec660_cfg *cfg = dev->config;
	int rc;
...
static int ec660_set_mode(const struct device *dev,
			    enum fingerprint_sensor_mode mode)
{
	...
	case FINGERPRINT_SENSOR_MODE_DETECT:
		if (IS_ENABLED(CONFIG_HAVE_EGIS_EC660_PRIVATE_DRIVER)) {
			fp_sensor_configure_detect();
			rc = ec660_enable_irq(dev);
		} else {
			rc = -ENOTSUP;
		}
		break;
	...
	case FINGERPRINT_SENSOR_MODE_LOW_POWER:
		rc = ec660_disable_irq(dev);
		rc_cmd = fp_sensor_deepsleep();
...
static int ec660_init(const struct device *dev)
{
	struct ec660_data *data = dev->data;
	uint16_t id = 0;
	int rc;
	...
	if (id != FP_SENSOR_HWID_EGIS) {
		LOG_ERR("EGIS unknown silicon 0x%04x", id);
		return -EINVAL;
	}

	LOG_INF("EC660 id 0x%04x\n", id);
	...
	ec660_set_mode(dev, FINGERPRINT_SENSOR_MODE_LOW_POWER);

	return 0;
}

static int ec660_deinit(const struct device *dev)
{
	int rc;
...
static int ec660_config(const struct device *dev, fingerprint_callback_t cb)
{
	struct ec660_data *data = dev->data;
...
static int ec660_maintenance(const struct device *dev, uint8_t *buf,
			       size_t size)
{
	struct ec660_data *data = dev->data;
...
static int ec660_acquire_image(const struct device *dev,
...
static int ec660_finger_status(const struct device *dev)
{
	int rc;
...
//static DEVICE_API(fingerprint, cros_fp_ec660_driver_api) = {
static const struct fingerprint_driver_api cros_fp_ec660_driver_api = {
	.init = ec660_init,
	.deinit = ec660_deinit,
	.config = ec660_config,
	.get_info = ec660_get_info,
	.maintenance = ec660_maintenance,
	.set_mode = ec660_set_mode,
	.acquire_image = ec660_acquire_image,
	.finger_status = ec660_finger_status,
};

static void ec660_irq(const struct device *dev, struct gpio_callback *cb,
			uint32_t pins)
{
	struct ec660_data *data =
		CONTAINER_OF(cb, struct ec660_data, irq_cb);

	ec660_disable_irq(data->dev);
...
static int ec660_init_driver(const struct device *dev)
{
	const struct ec660_cfg *cfg = dev->config;
	struct ec660_data *data = dev->data;
	int ret;
	...
	data->dev = dev;
	gpio_init_callback(&data->irq_cb, ec660_irq, BIT(cfg->interrupt.pin));
	gpio_add_callback_dt(&cfg->interrupt, &data->irq_cb);

	return 0;
}

#define EC660_SENSOR_INFO(inst)                                           \
	{                                                                  \
...
#define EC660_IMAGE_PARAM_INITIALIZER(idx, inst)                              \
	{                                                                      \
		.frame_size =                                                  \
...
#define EC660_BUILD_ASSERT_IMAGE_SIZE(idx, inst)                              \
	BUILD_ASSERT(                                                          \
		CONFIG_FINGERPRINT_SENSOR_IMAGE_SIZE >=                        \
			FINGERPRINT_SENSOR_FRAME_SIZE(idx, DT_DRV_INST(inst)), \
		"FP image buffer size smaller than raw image size at index " #idx);

#define EC660_ASSERT_FRAME_SIZE_CONSISTENT(idx, inst)                         \
	BUILD_ASSERT((FINGERPRINT_SENSOR_FRAME_SIZE(idx, DT_DRV_INST(inst)) == \
		      FINGERPRINT_SENSOR_FRAME_SIZE(0, DT_DRV_INST(inst))),    \
		     "EC660: frame_size of config " #idx                      \
		     " does not match config 0");

#define EGIS_EC660_DEFINE(inst)                                                    \
	static struct ec660_data ec660_data_##inst;                               \
	static const struct ec660_cfg ec660_cfg_##inst = {                        \
		...
		.sensor_info = EC660_SENSOR_INFO(inst),                            \
		.sensor_image_configs = { LISTIFY(                                  \
			FINGERPRINT_SENSOR_NUM_CONFIGS(DT_DRV_INST(inst)),          \
			EC660_IMAGE_PARAM_INITIALIZER, (, ), inst) },              \
	};                                                                          \
	LISTIFY(FINGERPRINT_SENSOR_NUM_CONFIGS(DT_DRV_INST(inst)),                  \
		EC660_BUILD_ASSERT_IMAGE_SIZE, (;), inst)                          \
	LISTIFY(FINGERPRINT_SENSOR_NUM_CONFIGS(DT_DRV_INST(inst)),                  \
		EC660_ASSERT_FRAME_SIZE_CONSISTENT, (;), inst)                     \
	BUILD_ASSERT(                                                               \
		FINGERPRINT_SENSOR_NUM_CONFIGS(DT_DRV_INST(inst)) <=                \
			NUM_IMAGE_CAPTURE_TYPES,                                    \
		"EC660: Number of image configs exceeds NUM_IMAGE_CAPTURE_TYPES"); \
	DEVICE_DT_INST_DEFINE(inst, ec660_init_driver, NULL,                       \
			      &ec660_data_##inst, &ec660_cfg_##inst,              \
			      POST_KERNEL,                                          \
			      CONFIG_FINGERPRINT_SENSOR_INIT_PRIORITY,              \
			      &cros_fp_ec660_driver_api)

DT_INST_FOREACH_STATUS_OKAY(EGIS_EC660_DEFINE);
