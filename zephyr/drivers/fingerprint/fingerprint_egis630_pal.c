				      ...
				      uint8_t *read, uint32_t read_len)
{
	...
	const struct spi_buf tx_buf[1] = { { .buf = write, .len = write_len } };
	const struct spi_buf rx_buf[1] = { { .buf = read, .len = read_len } };
...
{
	return (k_ticks_to_us_near64(k_uptime_ticks())) / USEC_PER_MSEC;
}
