...
typedef struct
{
    ...
    uint32_t rows;
    uint32_t cols;
    uint32_t algo_size_limit;                   // max algo ram size (bytes) (minimum. 140 * 1024)
    ...
    uint8_t  finger_template_read_type; // 0 = without segmented execution , 1 = with segmented excution
