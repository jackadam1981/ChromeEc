...
/* it's a workaround for leave_cs_asserted is true, it should keep cs pin high by
 * not calling fp_sensor_unlock but it not works for now, so we combine two times
 * call this function into one time to call spi_transceive_dt, we know when its
 * true for leave_cs_asserted, the write size is 1, so write_pending occupies a
 * maximum of 1 bytes, we set 8 just for temporary test */
					...
					bool leave_cs_asserted)
{
	...
	if (leave_cs_asserted) {
		...
		if (write_pending_size) {
			memcpy(data + write_size, write_pending,
			       write_pending_size);
		...
		if (read_pending_size) {
			...
			if (data_tmp == NULL) {
				LOG_ERR("fail to malloc %d bytes",
					rx_buf[1].len);
		...
		if (read_pending_size) {
			memcpy(data + write_size, data_tmp + read_pending_size,
			       read_size);
			egis_free(data_tmp);
			data_tmp = NULL;
			read_pending_size = 0;
