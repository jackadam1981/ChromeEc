...
void egis_fp_reset_sensor(const struct egis630_cfg *cfg)
{
	...
	k_usleep(FP_SENSOR_RESET_DURATION_MS * USEC_PER_MSEC);
...
static void convert_egis_sensor_init_error_code(egis_api_return_t code,
...
static int egis630_init(const struct device *dev)
{
	...
	convert_egis_sensor_init_error_code(ret, data);
