...
#define FPC1025_IMAGE_PARAM_INITIALIZER(idx, inst)                        \
	{                                                                 \
		.frame_size =                                             \
			SENSOR_CONFIG_FRAME_SIZE(DT_DRV_INST(inst), idx), \
		.pixel_format = FINGERPRINT_SENSOR_V4L2_PIXEL_FORMAT(     \
			DT_DRV_INST(inst), idx),                          \
		.width = SENSOR_CONFIG_WIDTH(DT_DRV_INST(inst), idx),     \
		.height = SENSOR_CONFIG_HEIGHT(DT_DRV_INST(inst), idx),   \
		.bpp = SENSOR_CONFIG_BPP(DT_DRV_INST(inst), idx),         \
		.fp_capture_type = 0, .reserved = 0,                      \
>>>>>>> 950f57f589 (zephyr: Introduce fpinfo version 2)
	}
