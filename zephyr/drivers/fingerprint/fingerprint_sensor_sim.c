...
static int
fp_simulator_get_info(const struct device *dev,
				      ...
				      uint8_t *image_buf, size_t image_buf_size)
{
	...
	struct fp_simulator_data *data = dev->data;
	size_t size =
		MIN(config->sensor_image_configs[0].frame_size, image_buf_size);
...
#define FP_SIMULATOR_IMAGE_PARAM_INITIALIZER(idx, inst)                        \
	{                                                                      \
		.frame_size =                                                  \
			FINGERPRINT_SENSOR_FRAME_SIZE(DT_DRV_INST(inst), idx), \
		.pixel_format = FINGERPRINT_SENSOR_V4L2_PIXEL_FORMAT(          \
			DT_DRV_INST(inst), idx),                               \
		.width = FINGERPRINT_SENSOR_RES_X(DT_DRV_INST(inst), idx),     \
		.height = FINGERPRINT_SENSOR_RES_Y(DT_DRV_INST(inst), idx),    \
		.bpp = FINGERPRINT_SENSOR_RES_BPP(DT_DRV_INST(inst), idx),     \
				   ...
				   size_t image_size)
{
	const struct fp_simulator_cfg *config = dev->config;
	size_t size =
		MIN(config->sensor_image_configs[0].frame_size, image_size);
