...
__weak int fp_sensor_get_hwid(uint16_t *id)
{
	uint8_t cmd[3];
	int rc;

	cmd[0] = EGIS_CMD_HW_ID;

	rc = egis_sensor_spi_write_read(cmd, 1, 2, false);
	if (rc != 0) {
		return rc;
	}

	/* HWID is in big endian, so convert it CPU endianness. */
	*id = sys_be16_to_cpu(*(uint16_t *)&cmd[1]);

	return 0;
}

__weak int fp_sensor_deepsleep(void)
{
	uint8_t cmd[1] = { EGIS_CMD_DEEPSLEEP };
	return egis_sensor_spi_write_read(cmd, 1, 0, false);
}
