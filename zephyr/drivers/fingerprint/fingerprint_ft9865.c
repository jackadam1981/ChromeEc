...
extern uint8_t ff_algo_buf[32 *  1024]; //share memory with alg to save sram
			    ...
			    enum fingerprint_sensor_mode mode)
{
	...
	switch (mode)
	{
		case FINGERPRINT_SENSOR_MODE_IDLE:
			ret = ft_sensor_set_mode(0);
			ft9865_disable_irq(dev);
			break;

		case FINGERPRINT_SENSOR_MODE_LOW_POWER:
			ret = ft_sensor_set_mode(1); 
			ft9865_disable_irq(dev);
			break;

		case FINGERPRINT_SENSOR_MODE_DETECT:
			ret = ft_sensor_set_mode(2);
...
static int ft9865_init(const struct device *dev)
{
	...
	sensor_param.raw_buf = ff_algo_buf + sizeof(ff_algo_buf) - 10 * 1024; //ff_raw_buf;
				 ...
				 uint8_t *image_buf, size_t image_buf_size)
{
	...
	switch (capture_type)
	{
		...
		case FINGERPRINT_CAPTURE_TYPE_SIMPLE_IMAGE:
			ret = ft_sensor_capture_process(image_buf);
			break;
