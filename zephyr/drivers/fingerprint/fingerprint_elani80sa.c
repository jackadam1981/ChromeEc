...
#include "fingerprint_elani80sa.h"
#include "fingerprint_elani80sa_pal.h"
#include "fingerprint_elani80sa_private.h"
...
static int elani80sa_get_hwid(const struct device *dev, uint16_t *id)
{
	int rc;
...
static int elani80sa_check_hwid(const struct device *dev)
{
	struct elani80sa_data *data = dev->data;
	uint16_t id = 0;
	int status;

	status = elani80sa_get_hwid(dev, &id);
	if (status < 0) {
...
static inline int elani80sa_enable_irq(const struct device *dev)
{
	const struct elani80sa_cfg *cfg = dev->config;
	int rc;
...
static inline int elani80sa_disable_irq(const struct device *dev)
{
	const struct elani80sa_cfg *cfg = dev->config;
	int rc;
...
static int elani80sa_init(const struct device *dev)
{
	struct elani80sa_data *data = dev->data;
	int rc;
	...
	if (IS_ENABLED(CONFIG_HAVE_ELANI80SA_PRIVATE_DRIVER)) {
		elan_execute_reset();
...
static int elani80sa_deinit(const struct device *dev)
{
	if (!IS_ENABLED(CONFIG_HAVE_ELANI80SA_PRIVATE_DRIVER)) {
		return 0;
	}
...
static int elani80sa_get_info(const struct device *dev,
...
static int elani80sa_config(const struct device *dev, fingerprint_callback_t cb)
{
	struct elani80sa_data *data = dev->data;
...
static int elani80sa_maintenance(const struct device *dev, uint8_t *buf,
				 size_t size)
{
...
static int elani80sa_set_mode(const struct device *dev,
...
static int elani80sa_acquire_image(const struct device *dev,
...
static int elani80sa_finger_status(const struct device *dev)
{
...
static DEVICE_API(fingerprint, cros_fp_elani80sa_driver_api) = {
	.init = elani80sa_init,
	.deinit = elani80sa_deinit,
	.config = elani80sa_config,
	.get_info = elani80sa_get_info,
	.maintenance = elani80sa_maintenance,
	.set_mode = elani80sa_set_mode,
	.acquire_image = elani80sa_acquire_image,
	.finger_status = elani80sa_finger_status,
};

static void elani80sa_irq(const struct device *dev, struct gpio_callback *cb,
			  uint32_t pins)
{
	struct elani80sa_data *data =
		CONTAINER_OF(cb, struct elani80sa_data, irq_cb);

	elani80sa_disable_irq(data->dev);
...
static int elani80sa_init_driver(const struct device *dev)
{
	const struct elani80sa_cfg *cfg = dev->config;
	struct elani80sa_data *data = dev->data;
	int ret;
	...
	data->dev = dev;
	gpio_init_callback(&data->irq_cb, elani80sa_irq,
...
#define ELANI80SA_SENSOR_INFO(inst)                                    \
	{                                                              \
...
#define ELANI80SA_DEFINE(inst)                                                 \
	static struct elani80sa_data elani80sa_data_##inst;                    \
	static const struct elani80sa_cfg elani80sa_cfg_##inst = {             \
		...
		.info = ELANI80SA_SENSOR_INFO(inst),                           \
	};                                                                     \
	BUILD_ASSERT(                                                          \
	...
	DEVICE_DT_INST_DEFINE(inst, elani80sa_init_driver, NULL,               \
			      &elani80sa_data_##inst, &elani80sa_cfg_##inst,   \
			      POST_KERNEL,                                     \
			      CONFIG_FINGERPRINT_SENSOR_INIT_PRIORITY,         \
			      &cros_fp_elani80sa_driver_api)

DT_INST_FOREACH_STATUS_OKAY(ELANI80SA_DEFINE);
