...
#define EC6601_IMAGE_PARAM_INITIALIZER(idx, inst)                              \
	{                                                                      \
		.frame_size =                                                  \
...
#define EGIS_EC6601_DEFINE(inst)                                                    \
	...
	static const struct ec6601_cfg ec6601_cfg_##inst = {                        \
		...
		.sensor_info = EC6601_SENSOR_INFO(inst),                            \
		.sensor_image_configs = { LISTIFY(                                  \
			FINGERPRINT_SENSOR_NUM_CONFIGS(DT_DRV_INST(inst)),          \
			EC6601_IMAGE_PARAM_INITIALIZER, (, ), inst) },              \
	};                                                                          \
	LISTIFY(FINGERPRINT_SENSOR_NUM_CONFIGS(DT_DRV_INST(inst)),                  \
		EC6601_BUILD_ASSERT_IMAGE_SIZE, (;), inst)                          \
	LISTIFY(FINGERPRINT_SENSOR_NUM_CONFIGS(DT_DRV_INST(inst)),                  \
		EC6601_ASSERT_FRAME_SIZE_CONSISTENT, (;), inst)                     \
	BUILD_ASSERT(                                                               \
		FINGERPRINT_SENSOR_NUM_CONFIGS(DT_DRV_INST(inst)) <=                \
			NUM_IMAGE_CAPTURE_TYPES,                                    \
		"EC6601: Number of image configs exceeds NUM_IMAGE_CAPTURE_TYPES"); \
