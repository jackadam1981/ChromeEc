...
int __unused elan_write_cmd(uint8_t fp_cmd)
{
	...
	const struct spi_buf write_buf[1] = { { .buf = tx_buf, .len = 2 } };
...
int __unused elan_set_hv_chip(bool state)
{
	...
	if (state) {
		...
		const struct spi_buf read_buf[] = {
			{ .buf = NULL, .len = 2 }, { .buf = rx_buf, .len = 2 }
		};
		const struct spi_buf_set tx = {
			.buffers = write_buf, .count = ARRAY_SIZE(write_buf)
		};
		const struct spi_buf_set rx = { .buffers = read_buf,
						.count = ARRAY_SIZE(read_buf) };
		...
		const struct spi_buf_set tx_2 = {
			.buffers = write_buf_2, .count = ARRAY_SIZE(write_buf_2)
		};
		const struct spi_buf_set rx_2 = {
			.buffers = read_buf_2, .count = ARRAY_SIZE(read_buf_2)
		};
