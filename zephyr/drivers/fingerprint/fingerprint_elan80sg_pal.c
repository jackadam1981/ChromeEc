...
int __unused elan_set_hv_chip(bool state)
{
	...
	if (state) {
		...
		const uint8_t charge_pump[] = { HV_CONTROL,
						(uint8_t)CHARGE_PUMP_HVIC };

		elan_write_register(charge_pump[0], charge_pump[1]);

		const uint8_t disable_hv[] = { HV_ENABLE, VOLTAGE_HVIC };

		elan_write_register(disable_hv[0], disable_hv[1]);
