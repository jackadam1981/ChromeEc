...
LOG_MODULE_REGISTER(elani80sa_pal, LOG_LEVEL_INF);
...
K_HEAP_DEFINE(fp_driver_heap, CONFIG_FINGERPRINT_SENSOR_ELANI80SA_HEAP_SIZE);
					 ...
					 size_t trx_len)
{
	...
	const struct elani80sa_cfg *cfg = fp_sensor_dev->config;
	...
	for (i = 0; i < ELAN_DMA_LOOP; i++) {
		...
		do {
			...
			if (cnt_timer > POLLING_SCAN_TIMER) {
				ret = ELAN_ERROR_SCAN;
				LOG_ERR("%s regdata = 0x%x, fail ret = %d",
