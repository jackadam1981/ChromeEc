...
#include <drivers/kscan_matrix.h>
...
static int cros_kb_enable_interrupt(const struct device *dev,
					     int enable)
{
	return kscan_matrix_resume_detection(kscan_dev, enable);
}
...
static int cros_kb_read_row(const struct device *dev)
{
	...
	kscan_matrix_read_row(kscan_dev, &val);
...
static int cros_kb_drive_column(const struct device *dev, int col)
{
	...
	if (col == KEYBOARD_COLUMN_NONE) {
		...
		ret = kscan_matrix_drive_column(kscan_dev,
						KEYBOARD_COLUMN_DRIVE_NONE);
	}
	...
	/* Drive one line to low for determining which key's state changed. */
		...
		ret = kscan_matrix_drive_column(kscan_dev, col_out);
	}
...
{
	kscan_matrix_configure(kscan_dev, cros_kb_ksi_isr_cb);
