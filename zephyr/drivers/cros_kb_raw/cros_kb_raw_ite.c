					    ...
					    int enable)
{
	...
	if (enable) {
		...
		ite_intc_isr_clear(IT8XXX2_IRQ_WKINTC);
		irq_enable(IT8XXX2_IRQ_WKINTC);
	} else {
		irq_disable(IT8XXX2_IRQ_WKINTC);
	}
...
static int cros_kb_raw_ite_drive_column(const struct device *dev, int col)
{
	 ...
	 * we are using).
	 */
	IT8XXX2_KBS_KSOH1 = (IT8XXX2_KBS_KSOH1 & ~KSOH_PIN_MASK) |
...
static int cros_kb_raw_ite_init(const struct device *dev)
{
	 ...
	 */
	IT8XXX2_KBS_KSOCTRL = (IT8XXX2_KBS_KSOPU | IT8XXX2_KBS_KSOOD);
	/* bit2, 1 enables the internal pull-up of the KSI[7:0] pins. */
	IT8XXX2_KBS_KSICTRL = IT8XXX2_KBS_KSIPU;
#ifdef CONFIG_PLATFORM_EC_KEYBOARD_COL2_INVERTED
	/* KSO[2] output high, others output low. */
	IT8XXX2_KBS_KSOL = BIT(2);
	/* Enable KSO2's push-pull */
	IT8XXX2_KBS_KSOLGCTRL |= IT8XXX2_KBS_KSO2GCTRL;
	IT8XXX2_KBS_KSOLGOEN |= IT8XXX2_KBS_KSOLGOEN;
#else
	/* KSO[7:0] pins output low. */
	IT8XXX2_KBS_KSOL = 0x00;
#endif
	/* critical section with interrupts off */
	key = irq_lock();
	/*
	 ...
	 */
	IT8XXX2_KBS_KSOH1 &= ~KSOH_PIN_MASK;
	/* restore interrupts */
	irq_unlock(key);
	...
	IRQ_CONNECT(IT8XXX2_IRQ_WKINTC, 0, cros_kb_raw_ite_ksi_isr, NULL, 0);
