...
struct pinctrl_kscan_it8xxx2_config {
	/*
	 * KSI[7:0]/KSO[15:8]/KSO[7:0] port gpio control register (bit mapping
	 * to pin)
	 */
	...
	/*
	 * KSO push-pull/open-drain bit of KSO[15:0] control register (one bit
	 * to all pins)
	 */
	int pushpull_od_mask;
	/*
	 * KSI/KSO pullup bit of KSI[7:0]/KSO[15:0] control register (one bit to
	 * all pins)
	 */
	int pullup_mask;
};
...
{
	const struct pinctrl_kscan_it8xxx2_config *const config =
		pins->pinctrls->config;
				 ...
				 uint8_t pin_cnt)
{
	...
	for (uint8_t i = 0U; i < pin_cnt; i++) {
		...
		/*
		 * Set a pin of KSI[7:0]/KSO[15:0] to pullup,
...
#define PINCTRL_KSCAN_ITE_INIT(inst)                                           \
	static const struct pinctrl_kscan_it8xxx2_config                       \
		pinctrl_kscan_it8xxx2_cfg_##inst = {                           \
			.reg_ksi_kso_gctrl =                                   \
				(uint8_t *)DT_INST_REG_ADDR_BY_IDX(inst, 0),   \
			.reg_ksi_kso_ctrl =                                    \
				(uint8_t *)DT_INST_REG_ADDR_BY_IDX(inst, 1),   \
			.pushpull_od_mask =                                    \
				(uint8_t)DT_INST_PROP(inst, pushpull_od_mask), \
			.pullup_mask =                                         \
				(uint8_t)DT_INST_PROP(inst, pullup_mask),      \
		};                                                             \
									       \
	DEVICE_DT_INST_DEFINE(inst, &pinctrl_kscan_it8xxx2_init, NULL, NULL,   \
			      &pinctrl_kscan_it8xxx2_cfg_##inst, PRE_KERNEL_1, \
