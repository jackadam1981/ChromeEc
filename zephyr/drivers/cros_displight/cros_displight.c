...
static struct pwm_dt_spec displight_pwm = PWM_DT_SPEC_INST_GET(0);
...
static void displight_set_duty(int percent)
{
	...
	rv = pwm_set_dt(&displight_pwm, displight_pwm.period, pulse_ns);
	if (rv) {
		LOG_ERR("pwm_set_dt failed %s (%d)", pwm_dev->name, rv);
	}
}
