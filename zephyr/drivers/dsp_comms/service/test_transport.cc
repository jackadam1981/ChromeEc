...
class TestTransport
    : public cros::dsp::service::Transport<
          TestStatus, &TestStatus::flags, &TestStatus::response_length,
          kTestResponseReadyBit, kTestResponseBufferSize, sizeof(TestStatus)> {
public:
  TestTransport()
      : cros::dsp::service::Transport<
            TestStatus, &TestStatus::flags, &TestStatus::response_length,
            kTestResponseReadyBit, kTestResponseBufferSize, sizeof(TestStatus)>(
            ...
            [this](bool has_data) { has_data_ = has_data; }) {}

  TestTransport(pw::Function<pw::StatusWithSize(const TestStatus *data,
                                                pw::ByteSpan out)> &&fn)
      : cros::dsp::service::Transport<
            TestStatus, &TestStatus::flags, &TestStatus::response_length,
            kTestResponseReadyBit, kTestResponseBufferSize, sizeof(TestStatus)>(
            std::move(fn), [this](bool has_data) { has_data_ = has_data; }) {}
...
TEST(TransportLayer, CantGetNextResponseWhenEmpty) {
  TestTransport t;
  EXPECT_EQ(t.GetNextResponse().status(), pw::Status::NotFound());
  EXPECT_FALSE(t.HasPendingStatus());
}

TEST(TransportLayer, SetStatusFlags) {
  TestTransport t;
  ASSERT_TRUE(t.SetStatusFlag(0).ok());
  ASSERT_TRUE(t.SetStatusFlag(9).ok());
  ASSERT_EQ(t.SetStatusFlag(16), pw::Status::OutOfRange());
  EXPECT_TRUE(t.HasPendingStatus());

  auto status_result = t.GetNextResponse();
...
TEST(TransportLayer, StageResponse) {
  ...
  auto response_result = t.GetNextResponse();
  ...
  ASSERT_FALSE(t.GetNextResponse().ok());
}
...
TEST(TransportLayer, SetFlagsBeforeResponse) {
  ...
  ASSERT_TRUE(t.SetStatusFlag(second_flag).ok());
...
TEST(TransportLayer, SetFlagsAfterReadingStatus) {
  ...
  // response followed by a new state
  ASSERT_TRUE(t.SetStatusFlag(0).ok());
  EXPECT_TRUE(t.HasPendingStatus());
  ...
  status_result = t.GetNextResponse();
  ASSERT_TRUE(status_result.ok());
  ASSERT_EQ(status_result->size(), sizeof(TestStatus));
...
TEST(TransportLayer, FailToSerializeStatus) {
  ...
  ASSERT_TRUE(t.SetStatusFlag(0).ok());
  EXPECT_TRUE(t.HasPendingStatus());
  ASSERT_EQ(t.GetNextResponse().status(), pw::Status::Unknown());
  EXPECT_TRUE(t.HasPendingStatus());
}
