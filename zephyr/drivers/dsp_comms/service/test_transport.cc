...
using TestTransportBase = cros::dsp::service::Transport<TestStatus,
...
class TestTransport : public TestTransportBase {
 public:
  TestTransport()
      : TestTransport([](const auto& status, pw::ByteSpan out) -> pw::StatusWithSize {
  ...
  explicit TestTransport(
      pw::Function<pw::StatusWithSize(const TestStatus& data, pw::ByteSpan out)>&& fn)
...
TEST(TransportLayer, SetStatusBits) {
  ...
  const auto* status = reinterpret_cast<const TestStatus*>(status_result->data());
...
TEST(TransportLayer, StageResponse) {
  ...
  ASSERT_TRUE(t.StageResponse<int>(expected_response,
                                   [](const int& data, pw::ByteSpan out) -> pw::StatusWithSize {
                                     if (out.size() < sizeof(int)) {
                                       return pw::StatusWithSize(pw::Status::ResourceExhausted(),
                                                                 0);
                                     }
...
TEST(TransportLayer, SetFlagsBeforeResponse) {
  ...
  const uint8_t expected_flags[] = {
      ...
      (((1 << kTestResponseReadyBit) >> 8) & 0xff) | (((1 << second_flag) >> 8) & 0xff),
  };
...
TEST(TransportLayer, StageResultTwice) {
  ...
  auto serialize_fn = [](const int& data, pw::ByteSpan out) -> pw::StatusWithSize {
    if (out.size() < sizeof(int)) {
  ...
  EXPECT_TRUE(t.HasPendingStatus());
  ASSERT_EQ(t.StageResponse<int>(expected_response, serialize_fn), pw::Status::ResourceExhausted());
  EXPECT_TRUE(t.HasPendingStatus());
}
...
TEST(TransportLayer, FailToSerializeResult) {
  ...
  const int expected_response = 54321;
  ASSERT_EQ(t.StageResponse<int>(expected_response, serialize_fn), pw::Status::Unknown());
  EXPECT_FALSE(t.HasPendingStatus());
}
