...
static void dsp_service_do_init(struct k_work* work_item) {
  dsp_service_data* data =
      cros::dsp::util::ContainerOf(work_item, &dsp_service_data::do_init_work);
  const auto* config = data->dev_config;

  LOG_INF("Setting up target %s::0x%02x",
          config->bus->name,
          data->target_config.address);

  if (int rc = i2c_target_register(config->bus, &data->target_config);
      rc != 0) {
  ...
  if (int rc = gpio_pin_configure_dt(&config->interrupt, GPIO_OUTPUT);
      rc != 0) {
  ...
  if (int rc = gpio_pin_set_dt(&data->dev_config->interrupt, CROS_DSP_GPIO_OFF);
      rc != 0) {
  ...
  data->transport_.SetNotifyClientCallback([data](bool has_data) {
    ...
    gpio_pin_set_dt(&data->dev_config->interrupt,
...
static void dsp_service_handle_get_cbi_flags_request(struct k_work* work_item) {
  ...
  dsp_service_data* data = cros::dsp::util::ContainerOf(
      work_item, &dsp_service_data::get_cbi_flags_work);
...
bool dsp_service_attempt_to_decode(dsp_service_data* data) {
  ...
  return pb_decode(&istream,
                   cros_dsp_comms_EcService_fields,
  ...
  switch (data->pending_service_request.which_request) {
    case cros_dsp_comms_EcService_notify_notebook_mode_change_tag:
      ...
      if (IS_ENABLED(CONFIG_PLATFORM_EC_DSP_SERVICE_REMOTE_LID_ANGLE)) {
        switch (data->pending_service_request.request
...
static int dsp_service_init(const struct device* dev) {
  const auto* config =
      reinterpret_cast<const struct dsp_service_config*>(dev->config);
  auto* data = reinterpret_cast<dsp_service_data*>(dev->data);
  int rc = 0;

  LOG_ERR("Initializing DSP (%p) service with callbacks @%p",
          data,
