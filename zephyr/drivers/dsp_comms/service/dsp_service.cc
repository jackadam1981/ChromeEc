...
static void dsp_service_startup(struct ap_power_ev_callback* cb,
  ...
  LOG_WRN(
      "\n\n*******************************\n"
      "Calling Init on DSP comms service");
...
pw::Status cros::dsp::service::Driver::Init() {
  int rc = 0;
  if (fw_config_values_.is_enabled) {
    // Check if the FW config is set to the disable value
    uint32_t ish_enabled;

    rc |= cros_cbi_get_fw_config(fw_config_values_.field_id, &ish_enabled);
    PW_CHECK_INT_EQ(rc, 0);

    if (ish_enabled == fw_config_values_.value_id) {
      // Match, disable the service
      LOG_INF("\n\nDisabling DSP comms service");
      return pw::OkStatus();
    }
  }
  LOG_WRN("\n\nInitializing DSP comms service");
