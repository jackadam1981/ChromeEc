...
static void dsp_service_suspend_hook(void)
{
    ...
    LOG_INF("ITE Debug - dsp: suspend work");

    rc = i2c_target_unregister(bus, &cros::dsp::service::driver.target_cfg_);
    if (rc) {
      LOG_INF("Failed to unregister i2c target device");
    }
}
DECLARE_HOOK(HOOK_CHIPSET_SUSPEND_COMPLETE, dsp_service_suspend_hook, HOOK_PRIO_DEFAULT);
...
static void dsp_service_resume_hook(void)
{
    ...
    LOG_INF("ITE Debug - dsp: resume work");

    rc = i2c_target_register(bus, &cros::dsp::service::driver.target_cfg_);
    if (rc) {
      LOG_INF("Failed to register i2c target device");
    }
}
