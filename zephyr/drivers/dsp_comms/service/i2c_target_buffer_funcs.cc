...
void dsp_service_buf_write_received(struct i2c_target_config* cfg, uint8_t* ptr, uint32_t len) {
  ...
  if (len > cros_dsp_comms_EcService_size) {
    LOG_ERR("Overflow, trying to write (%uB), capacity is %uB", len, cros_dsp_comms_EcService_size);
...
int dsp_service_buf_read_requested(struct i2c_target_config* cfg, uint8_t** ptr, uint32_t* len) {
  dsp_service_data* data = cros::dsp::util::ContainerOf(cfg, &dsp_service_data::target_config);
