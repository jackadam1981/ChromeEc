...
extern "C" int dsp_service_write_requested(struct i2c_target_config* cfg) {
...
extern "C" int dsp_service_write_received(struct i2c_target_config* cfg,
                                          uint8_t in) {
  LOG_DBG("Taking semaphore...");
  if (k_sem_take(&cros::dsp::service::driver.data_processing_semaphore_,
                 K_NO_WAIT) != 0) {
...
extern "C" int dsp_service_read_requested(struct i2c_target_config* cfg,
                                          uint8_t* out) {
  if (k_sem_take(&cros::dsp::service::driver.data_processing_semaphore_,
                 K_NO_WAIT) != 0) {
...
extern "C" int dsp_service_read_processed(struct i2c_target_config* cfg,
                                          uint8_t* out) {
  if (k_sem_take(&cros::dsp::service::driver.data_processing_semaphore_,
                 K_NO_WAIT) != 0) {
    LOG_ERR("Can't start a read");
...
extern "C" int dsp_service_stop(struct i2c_target_config* cfg) {
  LOG_DBG("Done");
  return 0;
}
