...
#include "cros/dsp/internal/service_transport_state_machine.hh"
...
TEST(TransportStateMachine, CheckTransitionsFromIdle) {
  ...
  {
    TransportStateMachine state_machine;
    EXPECT_OK(state_machine.SetNewState(TransportStateMachine::HAS_STATUS_AND_RESPONSE));
    EXPECT_EQ(state_machine.current_state(), TransportStateMachine::HAS_STATUS_AND_RESPONSE);
  }
  {
    TransportStateMachine state_machine;
...
TEST(TransportStateMachine, CheckTransitionFromHasStatusAndResponse) {
  {
    TransportStateMachine state_machine;
    ASSERT_OK(state_machine.SetNewState(TransportStateMachine::HAS_STATUS_AND_RESPONSE));
    EXPECT_EQ(pw::Status::InvalidArgument(),
  ...
  }
  {
    ...
    EXPECT_EQ(state_machine.current_state(), TransportStateMachine::HAS_RESPONSE);
  }
}
...
TEST(TransportStateMachine, CheckTransitionFromHasResponse) {
  ...
  }
  {
    ...
    ASSERT_OK(state_machine.SetNewState(TransportStateMachine::HAS_RESPONSE));
    EXPECT_EQ(pw::Status::InvalidArgument(),
              state_machine.SetNewState(TransportStateMachine::HAS_STATUS_AND_RESPONSE));
    EXPECT_EQ(state_machine.current_state(), TransportStateMachine::HAS_RESPONSE);
  }
  {
    TransportStateMachine state_machine;
...
TEST(TransportStateMachine, CheckStateAnyOf) {
  ...
  EXPECT_TRUE(
      state_machine.StateAnyOf({TransportStateMachine::HAS_RESPONSE, TransportStateMachine::IDLE}));
  EXPECT_FALSE(state_machine.StateAnyOf(
      {TransportStateMachine::HAS_RESPONSE, TransportStateMachine::HAS_STATUS}));
}
