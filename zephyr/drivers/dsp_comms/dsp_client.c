...
#define GET_STATUS_RESPONSE_FLAG_SIZE \
  ARRAY_SIZE(((pw_transport_Status*)0)->flags)
...
int dsp_client_get_cbi_flags(const struct device* dev,
  ...
  if (!is_status_bit_set(cros_dsp_comms_StatusFlag_STATUS_FLAG_RESPONSE_READY,
                         &status)) {
    LOG_ERR("Something went wrong, bit %d not set",
