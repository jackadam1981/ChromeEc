...
int cbi_remote_get_board_info(enum cbi_data_tag tag, uint8_t* buffer, uint8_t* buffer_size) {
  ...
  switch (response.which_flags) {
    ...
    case cros_dsp_comms_GetCbiFlagsResponse_flags_string_tag:
      ...
      memcpy(buffer, &response.flags.flags_string, strlen(response.flags.flags_string));
...
#define GET_STATUS_RESPONSE_FLAG_SIZE ARRAY_SIZE(((cros_dsp_comms_GetStatusResponse*)0)->flags)

static inline bool is_status_bit_set(uint8_t bit, const cros_dsp_comms_GetStatusResponse* status) {
...
static int dsp_client_enable_interrupt(struct dsp_client_data* data, bool enable) {
  ...
  if (!enable) {
    ...
    return gpio_pin_interrupt_configure_dt(&config->interrupt, GPIO_INT_DISABLE);
  }

  // Try level active if we haven't tried before or if previous attempt was a
  // level active
  if (data->interrupt_config == 0 || data->interrupt_config == GPIO_INT_LEVEL_ACTIVE) {
    LOG_INF("Enabling level interrupts!");
    rc = gpio_pin_interrupt_configure_dt(&config->interrupt, GPIO_INT_LEVEL_ACTIVE);
...
int dsp_client_get_cbi_flags(const struct device* dev,
  ...
  pb_ostream_t stream = pb_ostream_from_buffer(data->request_buffer, cros_dsp_comms_EcService_size);
  bool encode_status = pb_encode(&stream, cros_dsp_comms_EcService_fields, &service);
  ...
  uint32_t events = k_event_wait(
      &data->response_ready_event, 1, true, K_MSEC(CONFIG_PLATFORM_EC_DSP_CLIENT_TIMEOUT_MS));
  ...
  pb_istream_t istream = pb_istream_from_buffer(status_buffer, ARRAY_SIZE(status_buffer));
  bool decode_status =
      pb_decode_delimited(&istream, cros_dsp_comms_GetStatusResponse_fields, &status);

  __ASSERT_NO_MSG(decode_status);
  __ASSERT_NO_MSG(status.response_length <= RESPONSE_BUFFER_SIZE);

  if (!is_status_bit_set(cros_dsp_comms_GetStatusResponse_Flag_RESPONSE_READY, &status)) {
    LOG_ERR("Something went wrong, flags=[0x%02x, 0x%02x]", status.flags[0], status.flags[1]);
    k_mutex_unlock(&data->mutex);
    return -EINTR;
  }
  ...
  istream = pb_istream_from_buffer(data->response_buffer, status.response_length);
  decode_status = pb_decode(&istream, cros_dsp_comms_GetCbiFlagsResponse_fields, mem);
...
static void dsp_client_gpio_callback(const struct device* port,
                                     ...
                                     uint32_t pin) {
  struct dsp_client_data* data = CONTAINER_OF(cb, struct dsp_client_data, gpio_cb);
...
static int dsp_client_gpio_init(const struct device* dev) {
  ...
  gpio_init_callback(&data->gpio_cb, dsp_client_gpio_callback, BIT(config->interrupt.pin));
