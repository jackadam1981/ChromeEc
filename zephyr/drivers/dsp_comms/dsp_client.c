...
static int command_cbi_get_fw_config(const struct shell *shell, size_t argc,
                                     char **argv) {
  uint32_t buffer;
  uint8_t buffer_size = 4;
  int rc = cbi_remote_get_board_info(CBI_TAG_FW_CONFIG, (uint8_t *)&buffer,
                                     &buffer_size);

  shell_fprintf(shell, SHELL_NORMAL, "rc=%d, result=0x%08x\n", rc, buffer);
  return 0;
}

SHELL_CMD_REGISTER(cbi_get_fw_config, NULL, "Get the FW_CONFIG CBI",
                   command_cbi_get_fw_config);
...
int cbi_remote_get_board_info(enum cbi_data_tag tag, uint8_t *buffer,
  ...
  case cros_dsp_comms_GetCbiFlagsResponse_flags_string_tag:
    ...
    memcpy(buffer, &response.flags.flags_string,
...
static inline bool
is_status_bit_set(uint8_t bit, const cros_dsp_comms_GetStatusResponse *status) {
...
int dsp_client_get_cbi_flags(const struct device *dev,
  ...
  LOG_DBG("Waiting...");
  uint32_t events =
      k_event_wait(&data->response_ready_event, 1, true,
  ...
  if (!is_status_bit_set(cros_dsp_comms_GetStatusResponse_Flag_RESPONSE_READY,
                         &status)) {
    LOG_ERR("Something went wrong, flags=[0x%02x, 0x%02x]", status.flags[0],
            status.flags[1]);
    k_mutex_unlock(&data->mutex);
    return -EINTR;
  }
...
static int dsp_client_gpio_init(const struct device *dev) {
  ...
  gpio_init_callback(&data->gpio_cb, dsp_client_gpio_callback,
...
#define DSP_CLIENT_DEFINE(inst)                                                \
  ...
  DEVICE_DT_INST_DEFINE(inst, dsp_client_init, NULL, &dsp_client_data_##inst,  \
                        &dsp_client_config_##inst, POST_KERNEL,                \
                        CONFIG_PLATFORM_EC_DSP_INIT_PRIORITY, NULL);
