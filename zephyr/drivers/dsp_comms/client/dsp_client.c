...
static int dsp_client_gpio_init(const struct device* dev) {
  ...
  __ASSERT_NO_MSG(gpio_pin_configure_dt(&config->interrupt, GPIO_INPUT) == 0);

  gpio_init_callback(
      &data->gpio_cb, dsp_client_gpio_callback, BIT(config->interrupt.pin));
  __ASSERT_NO_MSG(gpio_add_callback(config->interrupt.port, &data->gpio_cb) ==
                  0);

  __ASSERT_NO_MSG(dsp_client_enable_interrupt(data, true) == 0);
