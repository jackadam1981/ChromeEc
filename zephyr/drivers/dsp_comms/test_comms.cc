...
constexpr const struct device *kService =
    DEVICE_DT_GET_OR_NULL(DT_ALIAS(test_dsp_service));
...
protected:
  ...
  void SetUp() override {
    ...
    ASSERT_EQ(
        0, cbi_set_board_info(CBI_TAG_SSFC,
    ...
    gpio_callbacks_.handler = [](const struct device *port,
      ...
      printf("Got service callback:\n");
      while (pins != 0) {
        int pin = __builtin_ctz(pins);
        int value = gpio_emul_output_get(port, pin);
        printf("    pin=%d, value=%d. Forwarding value...\n", pin, value);
        SUSPEND();
        gpio_emul_input_set(kClientInterruptSpec.port, kClientInterruptSpec.pin,
                            value);
    ...
    printf("kServiceInterruptSpec(port=%p, pin=%d, flags=0x%04x, "
           "current_value=%d)\n",
           (void *)kServiceInterruptSpec.port, kServiceInterruptSpec.pin,
           kServiceInterruptSpec.dt_flags, current_service_pin_value);
    printf(
        "kClientInterruptSpec(port=%p, pin=%d, flags=0x%04x, "
        "current_value=%d)\n",
        (void *)kClientInterruptSpec.port, kClientInterruptSpec.pin,
        kClientInterruptSpec.dt_flags,
        gpio_pin_get_raw(kClientInterruptSpec.port, kClientInterruptSpec.pin));
    ASSERT_EQ(0,
  ...
  void TearDown() override {
    ASSERT_EQ(
        0, gpio_remove_callback_dt(&kServiceInterruptSpec, &gpio_callbacks_));
  }
...
TEST_F(DspComms, ReadCbiVersion) {
  ...
  ASSERT_EQ(0, cbi_remote_get_board_info(CBI_TAG_BOARD_VERSION,
...
TEST_F(DspComms, ReadCbiOemId) {
  ...
  ASSERT_EQ(0, cbi_remote_get_board_info(
...
TEST_F(DspComms, ReadCbiSkuId) {
  ...
  ASSERT_EQ(0, cbi_remote_get_board_info(
                   CBI_TAG_SKU_ID, reinterpret_cast<uint8_t *>(&out),
                   &size));
...
TEST_F(DspComms, ReadCbiModelId) {
  ...
  ASSERT_EQ(0, cbi_remote_get_board_info(
                   CBI_TAG_MODEL_ID, reinterpret_cast<uint8_t *>(&out),
                   &size));
...
TEST_F(DspComms, ReadCbiFwConfig) {
  ...
  ASSERT_EQ(0, cbi_remote_get_board_info(CBI_TAG_FW_CONFIG,
                                         reinterpret_cast<uint8_t *>(&out),
                                         &size));
...
TEST_F(DspComms, ReadCbiPcbSupplier) {
  ...
  ASSERT_EQ(0, cbi_remote_get_board_info(CBI_TAG_PCB_SUPPLIER,
                                         reinterpret_cast<uint8_t *>(&out),
                                         &size));
...
TEST_F(DspComms, ReadCbiReworkId) {
  ...
  ASSERT_EQ(0, cbi_remote_get_board_info(CBI_TAG_REWORK_ID,
                                         reinterpret_cast<uint8_t *>(&out),
                                         &size));
...
TEST_F(DspComms, ReadCbiFactoryCalibrationData) {
  ...
  ASSERT_EQ(0, cbi_remote_get_board_info(CBI_TAG_FACTORY_CALIBRATION_DATA,
                                         reinterpret_cast<uint8_t *>(&out),
                                         &size));
...
TEST_F(DspComms, ReadCbiDramPartNum) {
  ...
  ASSERT_EQ(0,
            cbi_remote_get_board_info(CBI_TAG_DRAM_PART_NUM,
                                      reinterpret_cast<uint8_t *>(out),
                                      &size));
...
TEST_F(DspComms, ReadCbiOemName) {
  ...
  ASSERT_EQ(0, cbi_remote_get_board_info(
                   CBI_TAG_OEM_NAME, reinterpret_cast<uint8_t *>(out),
                   &size));
