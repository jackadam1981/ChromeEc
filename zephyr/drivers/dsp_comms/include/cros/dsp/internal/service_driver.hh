...
ClassType* ContainerOf(void* ptr, ValueType ClassType::*member) {
  return reinterpret_cast<ClassType*>(reinterpret_cast<char*>(ptr) - OffsetOf(member));
}

template <typename ValueType, typename ClassType>
const ClassType* ContainerOf(const void* ptr, ValueType ClassType::*member) {
  return reinterpret_cast<const ClassType*>(reinterpret_cast<const char*>(ptr) - OffsetOf(member));
}
...
class dsp_service_data {
 public:
  dsp_service_data(struct i2c_target_config target_cfg, const struct dsp_service_config* cfg)
