...
using CrosTransportParent = Transport<cros_dsp_comms_GetStatusResponse,
...
class CrosTransport : public CrosTransportParent {
 public:
  CrosTransport()
      : CrosTransportParent([](const auto& status, pw::ByteSpan out) -> pw::StatusWithSize {
          printf("Serializing status flags=[0x%02x, 0x%02x], size=%u\n",
                 status.flags[0],
                 status.flags[1],
                 status.response_length);
          pb_ostream_t response_ostream =
              pb_ostream_from_buffer(reinterpret_cast<pb_type_t*>(out.data()), out.size());
          bool response_encode_success = pb_encode_delimited(
              &response_ostream, cros_dsp_comms_GetStatusResponse_fields, &status);
  ...
  pw::Status StageResponse(const cros_dsp_comms_GetCbiFlagsResponse& response) {
    return this->template StageResponse<cros_dsp_comms_GetCbiFlagsResponse>(
        response, [](const auto& response, pw::ByteSpan out) -> pw::StatusWithSize {
          pb_ostream_t response_ostream =
              pb_ostream_from_buffer(reinterpret_cast<pb_type_t*>(out.data()), out.size());
          bool response_encode_success =
              pb_encode(&response_ostream, cros_dsp_comms_GetCbiFlagsResponse_fields, &response);
          if (!response_encode_success) {
            return pw::StatusWithSize(pw::Status::ResourceExhausted(), 0);
          }
 ...
 protected:
  ...
  pw::Status UpdateResponseSize(cros_dsp_comms_GetStatusResponse& status, size_t new_size) {
