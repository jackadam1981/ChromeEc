...
template <typename StatusType, uint8_t kStatusFlagHasResponse,
          size_t kResponseBufferSize, size_t kStatusBufferSize,
          typename StatusFieldType, StatusFieldType StatusType::*flags_field,
...
public:
  ...
  constexpr Transport(SerializeFn<StatusType> &&serialize_status_fn)
      : status_(), status_flags_(),
        serialize_status_fn_(std::move(serialize_status_fn)),
        notify_client_fn_(nullptr) {
    status_flags_ = pw::ByteSpan(
        reinterpret_cast<std::byte *>(&(status_.*flags_field)),
...
public:
  ...
  pw::Status StageResponse(const T &response, SerializeFn<T> serialize) {
    ...
    if (!StateAnyOf({TransportStateMachine::State::IDLE,
                     TransportStateMachine::State::HAS_STATUS})) {
...
private:
  ...
  pw::Status DoSetStatusBit(uint8_t bit_num) {
    ...
    reinterpret_cast<uint8_t *>(status_flags_.data())[bit_num / 8] |=
        (1 << (bit_num % 8));
    return pw::OkStatus();
  }
