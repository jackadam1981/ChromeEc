...
#define DT_DRV_COMPAT ite_it8xxx2_cros_spi_slv
...
LOG_MODULE_REGISTER(cros_spi_slv, LOG_LEVEL_ERR);
...
static void spi_send_response_packet(struct host_packet *pkt)
{
	...
	if (spi_slv_state != SPI_STATE_PROCESSING) {
		LOG_ERR("The request data is not processing.");
		return;
	}
...
static void spi_slv_int_handler(const void *arg)
{
//printk("[spislv]spi_slv_int_handler\n");
	/*
...
static int cros_spi_ite_init(const struct device *dev)
{
	/* Set FIFO data target count */
	struct ec_host_request cmd_head;

	printk("[spislv]cros_spi_ite_init\n");
	ite_write(0xf016a0,1,0x00);
	ite_write(0xf016a1,1,0x00);
	ite_write(0xf016a4,1,0x00);
	ite_write(0xf016a5,1,0x00);
	...
	//irq_connect_dynamic(DT_INST_IRQN(0), 0, &spi_slv_int_handler, NULL, 0);
	IRQ_CONNECT(DT_INST_IRQN(0), 0, spi_slv_int_handler, 0, 0);
	irq_enable(DT_INST_IRQN(0));

	return 0;
}
SYS_INIT(cros_spi_ite_init, PRE_KERNEL_1, 61);
