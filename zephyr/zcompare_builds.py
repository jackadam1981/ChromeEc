...
def main():
    ...
    projects_built = []
    if args.project[0] == "-a":
        for project in zmake.project.find_projects(ec_zephyr_dir).values():
    ...
    for project in projects_built:
        output_bin = pathlib.Path(project) / "output" / "ec.bin"
        bin1 = build_dir1 / "modules" / "ec" / "build" / "zephyr" / output_bin
        bin2 = build_dir2 / "modules" / "ec" / "build" / "zephyr" / output_bin
        bin1_path = pathlib.Path(bin1)
        bin2_path = pathlib.Path(bin2)
        if not os.path.isfile(bin1_path) or not os.path.isfile(bin2_path):
            logging.error("Output binary %s not found", output_bin)
    ...
    if failed_projects or missing_projects:
        ...
        logging.error(
            "The following projects failed to build: %s", missing_projects
        )
        sys.exit(1)
    else:
