...
static int it52xx_i2c_write_vendor_cmd(/*const struct device *dev,*/
				       const struct i2c_dt_spec *i2c_dt)
{
	struct pdc_data_t *data = &data_0 /*dev->data*/;
	/* const struct pdc_config_t *cfg = dev->config;*/
	...
	if (data->cmd == CMD_GET_IC_STATUS) {
		read_len = IT52XX_GET_IC_STATUS_LENGTH;
	}

	msg[0].buf = data->wr_buf;
	msg[0].len = data->wr_buf_len;
	msg[0].flags = I2C_MSG_WRITE;
...
int main(void)
{
	...
	/* CMD_GET_IC_STATUS */ //not UCSI cmd, so can't like rtk fit to sm: [ST_WRITE] -> [PING_STATUS] -> [ST_READ]
	//experiment: get fw version by 0xB6
	it52xx_get_info(/*data->dev,*/ &data->info/*, true*/);
	it52xx_i2c_write_vendor_cmd(/*dev,*/ &i2c_dt);
	data->info.fw_version = ((data->rd_buf[1] << 8) | data->rd_buf[0]);
	LOG_ERR("fw_version 0x%x\n", data->info.fw_version);
	...
	/* CMD_ACK_CC_CI, have [MSGIN] why need? */
	cc = 1;
	ci.connect_change = 0;
	it52xx_ack_cc_ci(/*data->dev,*/ci, cc/*, vendor_defined*/);
#if 1
