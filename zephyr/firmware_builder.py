...
def build(opts):
    ...
    for target in targets:
        ...
        # TODO(kmshelton): Use pathlib.Path manipulations instead of
        # os.path.join (here and elsewhere).
        rv = subprocess.run([
                             'zmake', '-D', 'configure', '-b', '-B',
                             os.path.join(BUILD_DIR, target), target
                            ],
...
def bundle_firmware(opts):
    ...
    for project in zmake.project.find_projects(zephyr_dir):
        # Remove /mnt/host/source/src/platform/ec/zephyr in order to find the
        # project dir relative to the build directory.
        relative_project_dir = '/'.join(project.project_dir.parts[8:])
        print(relative_project_dir)
        artifacts_dir = os.path.join(BUILD_DIR,
                                     relative_project_dir,
                                     'output')
        if not os.path.isdir(artifacts_dir):
            continue
        tarball_name = ''.join([project.config.board, '.firmware.tbz2'])
