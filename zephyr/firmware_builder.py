...
def bundle_firmware(opts):
    ...
    for project in zmake.project.find_projects(zephyr_dir):
        artifacts_dir = os.path.join(BUILD_DIR, 'projects',
        ...
        find_cmd = ['find', '.', '-name', 'zephyr*', '-wholename', '*output*']
        tar_cmd = ['tar', 'cvfj', tarball_path, '--files-from', '-']
        find = subprocess.Popen(find_cmd,
                                cwd=artifacts_dir,
