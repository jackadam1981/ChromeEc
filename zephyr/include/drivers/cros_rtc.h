...
typedef int (*cros_rtc_api_get_value)(const struct device *dev, uint32_t *value);
 ...
 *
 * @param dev Pointer to the device structure for the RTC driver instance.
 * @param value Nnumber of desired real-time clock value.
 *
 * @return 0 If successful.
 * @retval -ENOTSUP Not supported api function.
 */
 ...
 * @retval -ENOTSUP Not supported api function.
 */
__syscall int cros_rtc_get_alarm(const struct device *dev,
				uint32_t *seconds, uint32_t *microseconds);
 ...
 * @retval -ENOTSUP Not supported api function.
 */
__syscall int cros_rtc_set_alarm(const struct device *dev,
				uint32_t seconds, uint32_t microseconds);
