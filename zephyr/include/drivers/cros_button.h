...
#define BUTTON_CFG_DEF(i)                                                \
	{                                                                \
		...
		.spec = CROS_EC_GPIO_DT_SPEC_GET(DT_DRV_INST(i), gpios), \
		.debounce_us = DT_INST_PROP(i, debounce_us),             \
	}

__subsystem struct cros_button_api {
	int (*get_config)(const struct device *, struct cros_button_config *);
	int (*get_debounce_us)(const struct device *, int *);
	int (*enable_interrupt)(const struct device *, gpio_callback_handler_t);
	int (*disable_interrupt)(const struct device *);
	int (*is_pressed)(const struct device *);
	int (*is_pressed_raw)(const struct device *);
};
