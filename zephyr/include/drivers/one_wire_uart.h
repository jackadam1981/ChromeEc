...
void one_wire_uart_send(const struct device *device, enum RoachCommand cmd, const uint8_t* payload, int size);

void one_wire_uart_enable(const struct device *dev);

typedef void (*one_wire_uart_msg_received_cb_t)(const struct RoachMessage *msg);
