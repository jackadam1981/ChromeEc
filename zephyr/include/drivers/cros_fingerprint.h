 ...
 */
#define FP_SENSOR_RES_X(node_id) DT_PROP(node_id, width)

/**
 * @brief Get fingerprint sensor height.
 */
#define FP_SENSOR_RES_Y(node_id) DT_PROP(node_id, height)

/**
 * @brief Get fingerprint sensor width.
 */
#define FP_SENSOR_RES_BPP(node_id) DT_PROP(node_id, bits_per_pixel)

/**
 * @brief Get fingerprint sensor width.
 */
#define FP_SENSOR_V4L2_PIXEL_FORMAT(node_id) DT_PROP(node_id, v4l2_pixel_format)

/**
 * @brief Get size of raw fingerprint image (in bytes)
 */
#define FP_SENSOR_REAL_IMAGE_SIZE(node_id)                      \
	((FP_SENSOR_RES_X(node_id) * FP_SENSOR_RES_Y(node_id) * \
	  FP_SENSOR_RES_BPP(node_id)) /                         \
	 8)
