 ...
 * @retval -EBUSY if not ready to execute the command
 */
static inline int pdc_get_alternate_mode(const struct device *dev,
			enum sop_t sop, uint8_t alt_mode_offset,
			uint8_t num_alt_modes, struct alt_mode_t *alt_modes)
{
	const struct pdc_driver_api_t *api =
			(const struct pdc_driver_api_t *)dev->api;

	return api->get_alternate_mode(dev, sop, alt_mode_offset,
			num_alt_modes, alt_modes);
}
 ...
 */
static inline int pdc_getvbus_voltage(const struct device *dev,
					uint16_t *voltage)
{
	const struct pdc_driver_api_t *api =
			(const struct pdc_driver_api_t *)dev->api;
 ...
 * @retval negative value on error
 */
static inline int pdc_get_vbus_current(const struct device *dev,
				uint16_t *current)
{
	const struct pdc_driver_api_t *api =
			(const struct pdc_driver_api_t *)dev->api;
 ...
 * @retval -EBUSY if not ready to execute the command
 */
static inline int pdc_get_pdo(const struct device *dev,
			...
			uint8_t num_pdos, bool port_partner_pdo, uint32_t *pdos)
{
	...
	return api->get_pdo(dev, pdo_type, pdo_offset, num_pdos,
