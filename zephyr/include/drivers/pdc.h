...
typedef int (*pdc_get_drp_mode_t)(const struct device *dev, enum drp_mode_t *dm);
...
static inline int pdc_get_drp_mode(const struct device *dev, enum drp_mode_t *dm)
{
	const struct pdc_driver_api_t *api =
		(const struct pdc_driver_api_t *)dev->api;
