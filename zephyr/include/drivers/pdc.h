...
typedef int (*pdc_is_vconn_sourcing_t)(const struct device *dev,
				       bool *vconn_sourcing);
typedef int (*pdc_execute_command_t)(const struct device *dev,
...
__subsystem struct pdc_driver_api_t {
	...
	pdc_execute_command_t execute_command;
};
/**
 * @endcond
 */
 ...
 *         to finish.
 */
static inline int pdc_execute_command(const struct device *dev,
				      uint8_t ucsi_command, uint8_t data_size,
				      uint8_t *command_specific,
				      uint8_t *lpm_data_out)
{
	...
	if (api->execute_command == NULL) {
		return -ENOSYS;
	}

	return api->execute_command(dev, ucsi_command, data_size,
