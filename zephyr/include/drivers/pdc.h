...
typedef void (*pdc_cci_handler_cb_t)(union cci_event_t cci_event, void *cb_data);
...
typedef int (*pdc_set_power_level_t)(const struct device *dev, enum usb_typec_current_t tcc);
typedef int (*pdc_reconnect_t) (const struct device *dev);
typedef int (*pdc_get_current_flash_bank_t)(const struct device *dev, uint8_t *bank);
/**
 ...
 * @retval -ENOSYS if not implemented
 */
static inline int pdc_set_power_level(const struct device *dev, enum usb_typec_current_t tcc)
{
	const struct pdc_driver_api_t *api =
		(const struct pdc_driver_api_t *)dev->api;
 ...
 * @retval -ENOSYS if not implemented
 */
static inline int pdc_get_current_flash_bank(const struct device *dev, uint8_t *bank)
{
	const struct pdc_driver_api_t *api =
			(const struct pdc_driver_api_t *)dev->api;
