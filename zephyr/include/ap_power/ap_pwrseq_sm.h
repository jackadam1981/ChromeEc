 ...
 * for all states and any substate declared in devicetree, it also defines
 * functions to initialize state machine, execute curren state `run` action,
 * perform state transition and query state machine basic information.
 *
 ...
 * context. During initialization, driver will query implementation to provide
 * initial state to start state machine execution.
 *
 ...
 * returning value to different anything than zero.
 *
...
typedef enum ap_pwrseq_state (*ap_pwrseq_sm_init_state)(void *const data);

#define AP_POWER_INIT_STATE_FUNC(func) ap_pwrseq_sm_init_state      \
						init_state_func = func
