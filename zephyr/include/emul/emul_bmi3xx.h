...
struct bmi3xx_emul_type_data {
	 ...
	 */
	int (*handle_write)(uint16_t *regs, const struct emul *emul, int reg,
			    int byte, uint16_t val);
	/**
	 ...
	 * @return other on error
	 */
	int (*handle_read)(uint16_t *regs, const struct emul *emul, int reg,
			   int byte, char *buf);
	/**
	 ...
	 * @return 0 on success
	 */
	int (*finish_read)(uint16_t *regs, const struct emul *emul, int reg,
			   int bytes);
	/**
 ...
 * @param emul Pointer to BMI emulator
 */
void bmi3xx_emul_reset(const struct emul *emul);
/**
 * @}
 */
