...
#ifndef __EMUL_BMI3XX_H
#define __EMUL_BMI3XX_H
 ...
 * @defgroup bmi3xx_emul BMI emulator
 * @{
 *
 ...
 * - call @ref bmi3xx_emul_set_reg and @ref bmi3xx_emul_get_reg to set and get
 * value of BMI registers
 * - call @ref bmi3xx_emul_set_off and @ref bmi3xx_emul_get_off to set and get
 *   internal offset value
 * - call @ref bmi3xx_emul_set_value and @ref bmi3xx_emul_get_value to set and
 * get accelerometer or gyroscope value
 * - call bmi3xx_emul_set_err_* to change emulator behaviour on inadvisable
 * driver behaviour
 * - call @ref bmi3xx_emul_simulate_cmd_exec_time to enable or disable
 * simulation of command execution time
 * - call @ref bmi3xx_emul_append_frame to add frame to FIFO
 * - call @reg bmi3xx_emul_set_skipped_frames to generate skip frame on next
 * access to FIFO
...
/**
 * Axis argument used in @ref bmi3xx_emul_set_value @ref bmi3xx_emul_get_value
 * @ref bmi3xx_emul_set_off and @ref bmi3xx_emul_get_off
 */
enum bmi3xx_emul_axis {
	BMI3XX_EMUL_ACC_X,
	BMI3XX_EMUL_ACC_Y,
	BMI3XX_EMUL_ACC_Z,
	BMI3XX_EMUL_GYR_X,
...
/** BMI emulator models */
#define BMI3XX_EMUL_3XX 1

/** Last register supported by emulator */
#define BMI3XX_EMUL_MAX_REG 0x80
/** Maximum number of registers that can be backed in NVM */
#define BMI3XX_EMUL_MAX_NVM_REGS 10

/**
 * Acceleration 1g in internal emulator units. It is helpful for using
 * functions @ref bmi3xx_emul_set_value @ref bmi3xx_emul_get_value
 * @ref bmi3xx_emul_set_off and @ref bmi3xx_emul_get_off
 */
#define bmi3xx_EMUL_1G BIT(14)
/**
 * Gyroscope 125°/s in internal emulator units. It is helpful for using
 * functions @ref bmi3xx_emul_set_value @ref bmi3xx_emul_get_value
 * @ref bmi3xx_emul_set_off and @ref bmi3xx_emul_get_off
 */
#define BMI3XX_EMUL_125_DEG_S BIT(15)

/** Type of frames that can be added to the emulator frames list */
#define BMI3XX_EMUL_FRAME_ACC BIT(0)
#define BMI3XX_EMUL_FRAME_GYR BIT(1)
#define BMI3XX_EMUL_FRAME_TEMP BIT(2)
#define BMI3XX_EMUL_FRAME_TIME BIT(3)
 ...
 */
#define BMI3XX_EMUL_ACCESS_E 1

/** Structure used to describe single FIFO frame */
struct bmi3xx_emul_frame {
	/** Type of frame */
	uint8_t type;
	...
	struct bmi3xx_emul_frame *next;
};

/** Structure describing specific BMI model */
struct bmi3xx_emul_type_data {
	 ...
	 */
	int (*handle_write)(const struct emul *emul, int reg, int byte,
			    uint16_t val);
	/**
	 ...
	 * @return other on error
	 */
	int (*handle_read)(const struct emul *emul, int reg, int byte,
			   char *buf);

	/**
	 * @brief Model specific start read function. It should modify state
	 ...
	 * @return 0 on success
	 */
	int (*start_read)(const struct emul *emul, int reg);

	/**
	 * @brief Model specific start write function. It should modify state
	 * of emulator if required.
	 *
	 * @param regs Pointer to array of emulator's registers
	 * @param emul Pointer to BMI emulator
	 ...
	 * @return 0 on success
	 */
	int (*start_write)(const struct emul *emul, int reg);
	 ...
	 * @return 0 on success
	 */
	int (*finish_read)(const struct emul *emul, int reg, int bytes);
	 ...
	 * @return 0 on success
	 */
	int (*finish_write)(const struct emul *emul, int reg, int bytes);
	 ...
	 * @param emul Pointer to BMI emulator
	 */
	void (*reset)(const struct emul *emul);

	/** Array of reserved bits mask for each register */
	const uint16_t *rsvd_mask;
...
/**
 * @brief Get BMI324 model specific structure.
 *
 * @return Pointer to BMI160 specific structure
 */
const struct bmi3xx_emul_type_data *get_bmi324_emul_type_data(void);
 ...
 */
int16_t bmi3xx_emul_get_off(const struct emul *emul,
			    enum bmi3xx_emul_axis axis);
 ...
 *         gyroscope is 0.0037°/s.
 */
int32_t bmi3xx_emul_get_value(const struct emul *emul,
			      enum bmi3xx_emul_axis axis);
 ...
 *            gyroscope is 0.0037°/s.
 */
void bmi3xx_emul_set_value(const struct emul *emul, enum bmi3xx_emul_axis axis,
			   int32_t val);
 ...
 */
void bmi3xx_emul_set_err_on_ro_write(const struct emul *emul, bool set);
 ...
 * @param set Check for this error
 */
void bmi3xx_emul_set_err_on_rsvd_write(const struct emul *emul, bool set);
 ...
 * @param set Check for this error
 */
void bmi3xx_emul_set_err_on_wo_read(const struct emul *emul, bool set);
 ...
 */
void bmi3xx_emul_simulate_cmd_exec_time(const struct emul *emul, bool set);
 ...
 */
void bmi3xx_emul_set_skipped_frames(const struct emul *emul, uint16_t skip);
 ...
 *              it out through I2C)
 */
void bmi3xx_emul_append_frame(const struct emul *emul,
