 ...
 * Emulator allows to set callbacks on change of CC status or transmiting
 ...
 */
typedef void (*tcpc_emul_alert_state_func)(struct i2c_emul *emul, int alert,
					   void *data);
...
struct tcpc_emul_dev_ops {
	 ...
	 */
	enum tcpc_emul_ops_resp (*read_byte)(struct i2c_emul *emul,
					     struct tcpc_emul_dev_ops *ops,
	 ...
	 * @return TCPC_EMUL_ERROR to immedietly return error
	 */
	enum tcpc_emul_ops_resp (*write_byte)(struct i2c_emul *emul,
					      struct tcpc_emul_dev_ops *ops,
					      int reg, uint8_t val, int bytes);
	 ...
	 * @return TCPC_EMUL_ERROR to immedietly return error
	 */
	enum tcpc_emul_ops_resp (*handle_write)(struct i2c_emul *emul,
						struct tcpc_emul_dev_ops *ops,
						int reg, int msg_len);
	 ...
	 * @param ops Pointer to device operations structure
	 */
	void (*reset)(struct i2c_emul *emul, struct tcpc_emul_dev_ops *ops);
};

/** TCPC partner operations. Not all of them needs to be implemented. */
struct tcpc_emul_partner_ops {
	/**
	 * @brief Function called when TCPM wants to trnasmit message to partner
	 *        connected to TCPC
	 *
	 * @param emul Pointer to TCPC emulator
	 ...
	 */
	void (*transmit)(struct i2c_emul *emul,
			 struct tcpc_emul_partner_ops *ops,
	 ...
	 * @param ops Pointer to partner operations structure
	 */
	void (*control_change)(struct i2c_emul *emul,
			       struct tcpc_emul_partner_ops *ops);
};

/**
 * @brief Get pointer to TCPC emulator using device tree order number.
 *
 * @param ord Device tree order number obtained from DT_DEP_ORD macro
 *
 * @return Pointer to TCPC emulator
 */
...
/**
 * @brief Set value of given two bytes register of TCPC
 *
 * @param emul Pointer to TCPC emulator
 * @param reg Register address which value will be changed
 * @param val New value of the register
...
/**
 * @brief Get value of given two bytes register of TCPC
 *
 * @param emul Pointer to TCPC emulator
 * @param reg Register address
 *
