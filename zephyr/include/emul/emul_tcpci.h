 ...
 *
 * @param emul Pointer to TCPCI emulator
 * @param src If true, connected device power role is source. Otherwise its role
 *            is sink.
 * @param cc1 If true, connected device is on CC1 line. Otherwise it is on CC2
 *            line.
 * @param cc_v Voltage detected on connected CC line
 * @param active_cable If true, active cable is connected (RA voltage on
 *                     remaining CC line). Otherwise remaining CC line is open.
 *
 * @return 0 on success
 * @return negative on error
 */
int tcpci_emul_connect_partner(const struct emul *emul, bool src, bool cc1,
			       enum tcpc_cc_voltage_status cc_v,
			       bool active_cable);
