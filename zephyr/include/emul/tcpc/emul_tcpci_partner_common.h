...
struct tcpci_partner_data {
	...
	/**
	 * True if accpet and reject messages shouldn't trigger soft reset
	 * in common message handler
	 */
	bool wait_for_response;
	/**
	 * If emulator triggers soft reset, it waits for accept. If accept
	 * doesn't arive, hard reset is triggerd.
	 */
	bool in_soft_reset;
};
 ...
 *        repeated messages. It handles vendor defined messages by skiping them.
 *        Accept and reject messages are handled when soft reset is send.
 *        Accpet/reject messages are skipped when wait_for_response flag is set.
