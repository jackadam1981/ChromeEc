...
class CompareBuilds:
    ...
    def _compare_configs(self, project, build_mode):
        ...
        try:
            ...
            config_data1 = config_data1.replace(self.checkouts[0].full_ref, "")
            config_data2 = config_data2.replace(self.checkouts[1].full_ref, "")
    ...
    def _compare_devicetrees(self, project, build_mode):
        build_path = (
            pathlib.Path("ec")
            / "build"
            / "zephyr"
        ...
        dt_output1 = build_dir1 / "zephyr.dts"
        dt_output2 = build_dir2 / "zephyr.dts"

        try:
            dt_data1 = ""
            dt_data2 = ""
            with open(dt_output1) as dt1, open(dt_output2) as dt2:
                dt_data1 = dt1.read()
                dt_data2 = dt2.read()
            return dt_data1 == dt_data2
        except FileNotFoundError as err:
            logging.error(
                "Zephyr build-%s zephyr.dts file not found for project %s: %s",
                build_mode,
                project.config.project_name,
                err,
            )
    ...
    def check_devicetrees(self, projects):
        ...
        for project in projects:
            ...
            if not (
                self._compare_devicetrees(project, "ro")
                and self._compare_devicetrees(project, "rw")
            ):
                failed_projects.append(project.config.project_name)
