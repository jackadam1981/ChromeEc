...
def get_cros_fwid(project_name):
    version_script_path = (
        "/mnt/host/source/src/third_party/"
        "chromiumos-overlay/chromeos/config/chromeos_version.sh"
    )

    result = subprocess.run(
        ["sh", version_script_path],
        stdout=subprocess.PIPE,
    ...
    official = os.environ.get("CHROMEOS_OFFICIAL", 0)
    return format_cros_fwid(result.stdout, project_name, official)


def format_cros_fwid(chromeos_version, project_name, official):
    """Formats a cros_fwid string

    Args:
        project_name: The name of the project
        chromeos_version: The output from the chromeos_version.sh script
        official: If this is an official build

    Returns:
        The cros_fwid string
    """
    version_vars = {}
    version_lines = chromeos_version.splitlines()
    for line in version_lines:
        split_line = line.strip().split("=")

        # Only look at lines of the from name=value
        if len(split_line) != 2:
            continue

        name = split_line[0].strip()
        value = split_line[1].strip()
        version_vars[name] = value

    chromeos_patch = version_vars["CHROMEOS_PATCH"]
    chromeos_build = version_vars["CHROMEOS_BUILD"]
    chromeos_branch = version_vars["CHROMEOS_BRANCH"]
    ...
    if not official:
        ...
        chromeos_patch = datetime.date.today().strftime("%y_%m_%d")

    return f"{project_name}_{chromeos_build}.{chromeos_branch}.{chromeos_patch}"[:31]


def write_version_header(version_str, cros_fwid, output_path, static=False):
    ...
    Args:
        ...
        cros_fwid: The cros_fwid string or None
    ...
    cros_fwid_def = "CROS_FWID32"
    if cros_fwid is None:
        add_def_unquoted(cros_fwid_def, "CROS_FWID_MISSING_STR")
    else:
        add_def(cros_fwid_def, cros_fwid)
