...
def sign_fw(
    ...
    _run_futility(["dump_fmap", "-x", bin_file, f"EC_RW:{ec_rw}",
                   f"SIG_RW:{sig_file}"],
                  work_dir, jobclient, logger)
    data_size = os.path.getsize(ec_rw) - os.path.getsize(sig_file)

    _run_futility(["create", key_path, work_dir / "key"],
                  work_dir, jobclient, logger)

    _run_futility(["sign", "--type", "rwsig", "--data_size", str(data_size),
                   "--prikey", pri_key, ec_rw, sig_file],
                  work_dir, jobclient, logger)

    _run_futility(["load_fmap", "-o", signed_bin, bin_file,
                   f"KEY_RO:{pub_key}", f"SIG_RW:{sig_file}"],
                  work_dir, jobclient, logger)

    return signed_bin

def _run_futility(args: List[Union[str, Path]],
                  work_dir: Path,
                  jobclient: zmake.jobserver.JobClient,
