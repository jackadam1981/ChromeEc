...
class ElfPacker(BasePacker):
    ...
    def pack_firmware(
        self, work_dir, jobclient, dir_map, ec_dir, version_string=""
    ):
        del version_string
...
class BinmanPacker(BasePacker):
    ...
    def pack_firmware(
        ...
        if util.read_kconfig_autoconf_value(
            ro_kconfig_dir, "CONFIG_PLATFORM_EC_RWSIG"
        ):
            raw_key_path = util.read_kconfig_autoconf_value(
                dir_map["rw"] / "zephyr" / "include" / "generated",
                "CONFIG_PLATFORM_EC_RWSIG_KEY_PATH",
            )
            key_path = ast.literal_eval(raw_key_path)
