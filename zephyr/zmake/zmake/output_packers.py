...
class RawBinPacker(BasePacker):
    ...
    def pack_firmware(self, work_dir, jobclient, singleimage,
                      version_string=""):
        yield self._check_packed_file_size(
            singleimage / 'zephyr' / 'zephyr.bin',
            {'singleimage': singleimage}), 'zephyr.bin'

    def _get_max_image_bytes(self, singleimage):
        size = util.read_kconfig_autoconf_value(
            singleimage / 'zephyr' / 'include' / 'generated',
            'CONFIG_FLASH_SIZE')
...
class NpcxPacker(BasePacker):
    ...
    def _get_max_image_bytes(self, ro, rw):
        ...
        rw_size = util.read_kconfig_autoconf_value(
            ro / 'zephyr' / 'include' / 'generated',
            'CONFIG_FLASH_SIZE')
        return max(int(ro_size), int(rw_size)) * 1024
