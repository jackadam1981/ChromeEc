    ...
    def configure(  # pylint: disable=too-many-arguments,too-many-locals
        ...
        try:
            ...
            if len(test_projects) > 1 and coverage and test_after_configure:
                ...
                if result:
                    failed_projects.append(str(build_dir / "all_tests.info"))
                    return result
            ...
            if len(non_test_projects) > 1 and coverage and build_after_configure:
                ...
                if result:
                    failed_projects.append(str(build_dir / "all_builds.info"))
                    return result
            return 0
        finally:
            if failed_projects:
                logging.error("Failed projects: %s", failed_projects)
