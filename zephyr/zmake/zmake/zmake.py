...
def ninja_log_lvl_override(line, default_log_lvl):
    """Update the lov level for ninja builds if we hit an error.

    Ninja builds print everything to standard out, but really we want to start
    logging things to CRITICAL

    :param line:
    :param default_log_lvl:
    :return:
    """
    ...
    def build(self, build_dir, output_files_out=None):
        ...
        for build_name, build_config in project.iter_builds():
            ...
            zmake.multiproc.log_output(
                ...
                log_lvl_override_func=ninja_log_lvl_override)
            zmake.multiproc.log_output(self.logger, logging.ERROR, proc.stderr)
            procs.append(proc)
