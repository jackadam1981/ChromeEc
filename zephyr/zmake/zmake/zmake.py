    ...
    def _configure(
        ...
        try:
            with self.jobserver.get_job():
                ...
                if "pigweed" in self.module_paths:
                    default_cmake_defs["PW_ROOT"] = str(
                        self.module_paths["pigweed"]
                    )
                if "nanopb" in self.module_paths:
                    default_cmake_defs["NANOPB_DIR"] = str(
                        self.module_paths["nanopb"]
                    )
                if ec_version_flags:
                    default_cmake_defs[
