...
def test_find_projects(tmp_path):
    ...
    assert projects["four"].get_full_name() == "baseboard.four"

    assert projects["five"].config.project_dir == cf2_dir
    assert projects["five"].config.zephyr_board == "foo"
    assert projects["five"].get_full_name() == "root.myboard.five"
...
another = some_variant.variant(
    ...
    assert projects["some"].get_full_name() == "some"
    assert projects["some-variant"].get_full_name() == "some.some-variant"
    assert projects["another"].get_full_name() == "some.some-variant.another"
