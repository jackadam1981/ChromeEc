...
def test_merge(coins, combined):
    ...
    # Split the originial config into two
...
def parse_cmake_args(argv):
    ...
    # Build the definiton dictionary
    cmake_defs = {}
...
def test_popen_cmake_kconfig_but_no_file(conf, project_dir, build_dir):
    # Running popen_cmake with Kconfig definitions to write out, but
    # no path to do so, should raise an error.
    job_client = FakeJobClient()
