...
def test_version_string_static(tmp_path):
    ...
    assert (
        ...
        == "prj-123.456.789-STATIC-c0d52f1"
    )
...
def test_version_string_default_static(fake_date, tmp_path):
    ...
    assert (
        ...
        == "prj-0.0.0-STATIC-c0d52f1"
    )


def test_version_string_official(tmp_path):
    """Test official build version string generation"""
    project, ver, git_path = _setup_example_repos(tmp_path)
    assert (
        version.get_version_string(
            project.config.project_name,
...
def test_version_string_official_default(tmp_path):
    """Test official build without version string provided."""
    project, _, git_path = _setup_example_repos(tmp_path)
    with pytest.raises(ValueError) as excinfo:
        version.get_version_string(
            project.config.project_name, git_path=git_path, official_build=True
        )
    assert "No version information provided" in str(excinfo.value)
...
HEADER_VERSION_STR_STATIC = "prj-123.456.789-STATIC-c0d52f1"
EXPECTED_HEADER_STATIC = (
    "/* This file is automatically generated by zmake_tests */\n"
    '#define VERSION "prj-123.456.789-STATIC-c0d52f1"\n'
    '#define CROS_EC_VERSION32 "prj-123.456.789-STATIC-c0d52f1"\n'
