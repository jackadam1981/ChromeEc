...
def test_sign_firmware():
    ...
    with tempfile.TemporaryDirectory() as temp_dir_name:
        ...
        subprocess.run(["binman", "build", "--dt", dts_path, "--outdir", temp_dir], check=True)
        key_path = temp_dir / "key.pem"
        subprocess.run(["openssl", "genrsa", "-3", "-out", key_path, "3072"],
                       check=True)

        # Sign the firmware.
        signed_fw = rwsig.sign_fw(temp_dir / "ec.bin", key_path, temp_dir, jobclient, logger)
