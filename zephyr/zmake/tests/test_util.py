...
def test_resolve_build_dir_with_build_dir(platform_ec_subdir, project_subdir, build_subdir):
    with tempfile.TemporaryDirectory() as temp_dir_name:
        ec_dir = pathlib.Path(temp_dir_name) / platform_ec_subdir
        build_dir = util.resolve_build_dir(
            ec_dir=ec_dir,
            project_dir=ec_dir / project_subdir,
            build_dir=ec_dir / build_subdir)

        assert build_dir == ec_dir / build_subdir
...
def test_resolve_build_dir_from_project(platform_ec_subdir, project_subdir):
    with tempfile.TemporaryDirectory() as temp_dir_name:
        ...
        build_dir = util.resolve_build_dir(
            ...
            build_dir=None)
        assert build_dir == ec_dir / 'build' / project_subdir
