...
ZTEST(vivaldi_kbd, test_get_vivaldi_keybd_config)
{
	...
	zassert_equal(ret, EC_RES_SUCCESS);
	zassert_equal(resp.num_top_row_keys, 11);
	zassert_equal(resp.action_keys[0], TK_PLAY_PAUSE);
	zassert_equal(resp.action_keys[1], TK_NEXT_TRACK);
	zassert_equal(resp.action_keys[2], TK_PREV_TRACK);
	zassert_equal(resp.action_keys[3], TK_ABSENT);
	zassert_equal(resp.action_keys[4], TK_ABSENT);
	zassert_equal(resp.action_keys[5], TK_ABSENT);
	zassert_equal(resp.action_keys[6], TK_ABSENT);
	zassert_equal(resp.action_keys[7], TK_ABSENT);
	zassert_equal(resp.action_keys[8], TK_KBD_BKLIGHT_TOGGLE);
	zassert_equal(resp.action_keys[9], TK_MICMUTE);
	zassert_equal(resp.action_keys[10], TK_MENU);
	zassert_equal(resp.capabilities, KEYBD_CAP_SCRNLOCK_KEY |
						 KEYBD_CAP_NUMERIC_KEYPAD |
						 KEYBD_CAP_SCRNLOCK_KEY);
#elif CONFIG_VIVALDI_KBD_TEST_IDX_VALUE == -1
	zassert_equal(ret, EC_RES_ERROR);
#else
...
#elif CONFIG_VIVALDI_KBD_TEST_IDX_VALUE == 1
	/* 11 keys on the second config */
	zassert_equal(keyscan_config.actual_key_mask[0], 0);
	zassert_equal(keyscan_config.actual_key_mask[1], BIT(0));
	zassert_equal(keyscan_config.actual_key_mask[2],
		      BIT(0) | BIT(2) | BIT(3));
	zassert_equal(keyscan_config.actual_key_mask[3], 0);
	zassert_equal(keyscan_config.actual_key_mask[4], BIT(0));
	zassert_equal(keyscan_config.actual_key_mask[5], 0);
	zassert_equal(keyscan_config.actual_key_mask[6], 0);
	zassert_equal(keyscan_config.actual_key_mask[7], 0);
	zassert_equal(keyscan_config.actual_key_mask[8], 0);
	zassert_equal(keyscan_config.actual_key_mask[9], BIT(1));
	zassert_equal(keyscan_config.actual_key_mask[10], 0);
	zassert_equal(keyscan_config.actual_key_mask[11], 0);
#elif CONFIG_VIVALDI_KBD_TEST_IDX_VALUE == -1
	zassert_equal(keyscan_config.actual_key_mask[0], 0);
	zassert_equal(keyscan_config.actual_key_mask[1], 0);
	zassert_equal(keyscan_config.actual_key_mask[2], 0);
	zassert_equal(keyscan_config.actual_key_mask[3], 0);
	zassert_equal(keyscan_config.actual_key_mask[4], 0);
	zassert_equal(keyscan_config.actual_key_mask[5], 0);
	zassert_equal(keyscan_config.actual_key_mask[6], 0);
	zassert_equal(keyscan_config.actual_key_mask[7], 0);
	zassert_equal(keyscan_config.actual_key_mask[8], 0);
	zassert_equal(keyscan_config.actual_key_mask[9], 0);
	zassert_equal(keyscan_config.actual_key_mask[10], 0);
	zassert_equal(keyscan_config.actual_key_mask[11], 0);
#else
#error "invalid config"
...
#elif CONFIG_VIVALDI_KBD_TEST_IDX_VALUE == 1
	zassert_equal(set2_test.codes[0][2], SCANCODE_PLAY_PAUSE);
	zassert_equal(set2_test.codes[3][2], SCANCODE_NEXT_TRACK);
	zassert_equal(set2_test.codes[2][2], SCANCODE_PREV_TRACK);
	zassert_equal(set2_test.codes[1][2], 0);
	zassert_equal(set2_test.codes[3][4], 0);
	zassert_equal(set2_test.codes[2][4], 0);
	zassert_equal(set2_test.codes[1][4], 0);
	zassert_equal(set2_test.codes[2][9], 0);
	zassert_equal(set2_test.codes[1][9], SCANCODE_KBD_BKLIGHT_TOGGLE);
	zassert_equal(set2_test.codes[0][4], SCANCODE_MICMUTE);
	zassert_equal(set2_test.codes[0][1], SCANCODE_MENU);
	zassert_equal(set2_test.call_count, 6);
#else
	zassert_equal(set2_test.call_count, 0);
#endif
}
