...
FAKE_VALUE_FUNC(int, board_is_sourcing_vbus, int);
...
static const struct gpio_dt_spec usb_c0_c1_rst = {
	.port = DEVICE_DT_GET(DT_GPIO_CTLR(DT_NODELABEL(usb_c0_c1_tcpc_rst_odl), gpios)),
	.pin = DT_GPIO_PIN(DT_NODELABEL(usb_c0_c1_tcpc_rst_odl), gpios),
	.dt_flags = 0xFF & DT_GPIO_FLAGS(DT_NODELABEL(usb_c0_c1_tcpc_rst_odl), gpios)
};
...
{
	gpio_pin_configure(usb_c0_c1_rst.port, usb_c0_c1_rst.pin, (GPIO_OUTPUT | GPIO_ACTIVE_HIGH));
        /* Execute the function to test */
        board_reset_pd_mcu();

	int gpio_val = gpio_emul_output_get(usb_c0_c1_rst.port, usb_c0_c1_rst.pin);
	zassert_equal(gpio_val, 1, "gpio usb_c0_c1_tcpc_rst_odl not set");
        zassert_equal(2, nct38xx_reset_notify_fake.call_count, "nct38xx_reset_notify call count mismatch");

#if defined(HAS_TASK_PD_C2)
        zassert_equal(1, ccgxxf_reset_fake.call_count, "ccgxxf_reset call count mismatch");
        zassert_equal(1, ioex_init_fake.call_count, "ioex_init call count mismatch");
#endif
}
