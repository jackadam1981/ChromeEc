...
static void configure_board_id_gpios_input(void) {
	for (int i = 0; i < NUM_BOM_GPIOS; i++)
		gpio_pin_configure(bom_id_config[i].port, bom_id_config[i].pin, (GPIO_INPUT | GPIO_ACTIVE_HIGH));

	for (int i = 0; i < NUM_FAB_GPIOS; i++)
		gpio_pin_configure(fab_id_config[i].port, fab_id_config[i].pin, (GPIO_INPUT | GPIO_ACTIVE_HIGH));

	for (int i = 0; i < NUM_BOARD_GPIOS; i++)
		gpio_pin_configure(board_id_config[i].port, board_id_config[i].pin, (GPIO_INPUT | GPIO_ACTIVE_HIGH));
}
...
ZTEST(board_version_tests, test_board_get_version)
{
    ...
    /* Verification: Correct version is computed and returned. here 1100110101b */
    int expected_id = 1051 /* Compute the expected ID based on the fake inputs */;
    zassert_equal(expected_id, version, "Expected version didn't match actual version");
}
