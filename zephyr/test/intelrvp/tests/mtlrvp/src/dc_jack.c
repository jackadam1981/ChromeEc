...
static void set_dc_jack_gpio()
{
        ...
        gpio_emul_input_set(dc_jack_gpio_device.port, 
                                    dc_jack_gpio_device.pin, 1);
}

ZTEST_USER(test_dc_jack, test_board_is_dc_jack_present)
{
        set_dc_jack_gpio();
        zassert_equal(1, board_is_dc_jack_present());
}

static void mock_charge_manager_update_charge(int port, int en, const struct charge_port_info *info)
{
        port_info.current = info->current;
        port_info.voltage = info->voltage;
}

ZTEST_USER(test_dc_jack, test_charger_jack)
{
        charge_manager_update_charge_fake.custom_fake = mock_charge_manager_update_charge;

	/* Running the hook will check if dc jack present */
        hook_notify(HOOK_INIT);
        k_sleep(K_MSEC(500));

	/* Since dc jack gpio is set in test_board_is_dc_jack_present port current will be non zero */
        zassert_equal(0, !port_info.current, "port current:%d", port_info.current);
}
