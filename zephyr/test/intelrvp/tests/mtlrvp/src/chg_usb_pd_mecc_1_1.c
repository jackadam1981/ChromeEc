...
ZTEST_USER(mtlrvp_chg_usb_pd_mecc, test_tcpc_get_alert_status_bus_type_embedded)
{
        ...
        zassert_equal(expt_ret, ret, "Value mismatch expt_ret:%u while ret:%u", expt_ret, ret);
}
...
ZTEST_USER(mtlrvp_chg_usb_pd_mecc, test_tcpc_get_alert_status_gpio_all_notset)
{
        ...
        for (int i = 0; i < CONFIG_USB_PD_PORT_MAX_COUNT; i++) {
                tcpc_config[i].bus_type = !EC_BUS_TYPE_EMBEDDED;
                gpio_pin_configure(tcpc_aic_gpios_device[i].port, tcpc_aic_gpios_device[i].pin, (GPIO_INPUT | GPIO_ACTIVE_LOW));
                gpio_emul_input_set(tcpc_aic_gpios_device[i].port, tcpc_aic_gpios_device[i].pin, 0);
                expt_ret |= PD_STATUS_TCPC_ALERT_0 << i;
        }
        uint16_t ret = tcpc_get_alert_status();

        zassert_equal(expt_ret, ret, "Value mismatch expt_val:%u while ret:%u", expt_ret, ret);
}
...
ZTEST_USER(mtlrvp_chg_usb_pd_mecc, test_tcpc_get_alert_status_gpio_all_set)
{
        ...
        for (int i = 0; i < CONFIG_USB_PD_PORT_MAX_COUNT; i++) {
                tcpc_config[i].bus_type = !EC_BUS_TYPE_EMBEDDED;
                gpio_pin_configure(tcpc_aic_gpios_device[i].port, tcpc_aic_gpios_device[i].pin, (GPIO_INPUT | GPIO_ACTIVE_LOW));
                gpio_emul_input_set(tcpc_aic_gpios_device[i].port, tcpc_aic_gpios_device[i].pin, 1);
        }
...
ZTEST_USER(mtlrvp_chg_usb_pd_mecc, test_board_charging_enable_ppc_enable_0)
{
        ...
        zassert_equal(1, ppc_vbus_sink_enable_fake.call_count, "ppc_vbus_sink_enable mismatch");
}
