 ...
 *
 * Called by external IRQ configuration so that an interrupt can be handled.
 * This may be called in a separate context (i.e. thread) from the main driver
 * so it must protect any shared data access.
 *
