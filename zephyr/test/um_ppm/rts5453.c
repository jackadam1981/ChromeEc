...
int rts5453_write_to_flash(struct rts5453_device* dev, int flash_bank,
  ...
  return rts5453_smbus_command(dev, RTS_DEFAULT_PORT, flash_cmd, cmd, size, NULL,
                               0);
}
...
static int rts5453_ppm_reset(struct rts5453_device* dev, uint8_t port) {
  ...
  return rts5453_smbus_command(dev, port, SC_UCSI_COMMANDS, cmd, ARRAY_SIZE(cmd),
                               unused_out, SMBUS_MAX_BLOCK_SIZE);
}
...
static int rts5453_ucsi_init_ppm(struct ucsi_pd_device* device) {
  ...
  int bytes_read;
  uint8_t num_ports = 0;
  uint8_t max_num_ports = dev->driver_config->max_num_ports; //1; //dev->driver_config->max_num_ports;
