...
int rts5453_do_firmware_update(struct ucsi_pd_driver* pd, const char* filepath,
  ...
  if (rts5453_vendor_cmd_enable_smbus_flash_access(dev, RTS_DEFAULT_PORT) == -1) {
    ELOG("Failed to enable flash access");
    goto cleanup;
  }

#ifdef DO_FLASH_PROTECT
  if (rts5453_set_flash_protection(dev, RTS5453_FLASH_PROTECT_DISABLE) == -1) {
  ...
  while ((bytes_read = read(fd, fbuf, FW_BLOCK_CHUNK_SIZE)) > 0) {
    if (rts5453_write_to_flash(dev, flash_bank, fbuf, bytes_read, offset) ==
        -1) {
...
#ifdef DO_FLASH_PROTECT
  if (rts5453_set_flash_protection(dev, RTS5453_FLASH_PROTECT_ENABLE) == -1) {
...
int rts5453_get_info(struct ucsi_pd_driver* pd) {
  ...
  printf("Fw version: %d.%d.%d\n", status.major_version, status.minor_version,
         status.patch_version);
  printf("VID:PID: %02x%02x:%02x%02x\n", status.vid_pid[1], status.vid_pid[0],
         status.vid_pid[3], status.vid_pid[2]);
