...
struct um_ppm_cdev {
  ...
  struct task_handle* interrupt_task;
};
...
void um_ppm_cdev_cleanup(struct um_ppm_cdev* cdev) {
  if (cdev) {
    ...
    platform_task_complete(cdev->interrupt_task);
...
static void um_ppm_block_for_interrupt(void* context) {
  struct um_ppm_cdev* cdev = (struct um_ppm_cdev*)context;
  struct smbus_driver* smbus = cdev->smbus;
  struct ucsi_pd_driver* pd= cdev->pd;

  DLOG("Start blocking for smbus interrupt");
  while (smbus->block_for_interrupt(smbus->dev) != -1) {
      pd->handle_interrupt(pd->dev);
  }

  ELOG("Exiting smbus interrupt block. This is fatal.");
  abort();
}
...
void um_ppm_cdev_mainloop(struct um_ppm_cdev* cdev) {
  ...
  // Start the interrupt task or bail out.
  cdev->interrupt_task = platform_task_init(um_ppm_block_for_interrupt, cdev);
  if (!cdev->interrupt_task) {
    return;
  }
