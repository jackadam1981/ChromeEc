...
/*
 * When building the module under test its board_power_signal_* functions are
 * renamed to real_board_power_signal_* so we can monitor accesses through these
 * fakes while still calling the versions from the module under test as needed.
 */
...
ZTEST(nissa_board_power, test_assert_pch_pwrok)
{
	...
	zassert_equal(board_power_signal_set_fake.arg0_val, PWR_PCH_PWROK);
	zassert_equal(board_power_signal_set_fake.arg1_val, 1);
}
...
ZTEST(nissa_board_power, test_force_shutdown)
{
	...
	s0_stable = true;
	board_power_signal_get_fake.return_val_seq = signal_get_results;
	board_power_signal_get_fake.return_val_seq_len =
	...
	zassert_equal(board_power_signal_set_fake.call_count, 4);
	zassert_mem_equal(
		board_power_signal_set_fake.arg0_history,
		((enum power_signal[]){ PWR_EC_SOC_DSW_PWROK, PWR_EC_PCH_RSMRST,
					PWR_EN_PP3300_A, PWR_EN_PP5000_A }),
		4);
	zassert_mem_equal(board_power_signal_set_fake.arg1_history,
			  ((int[]){ 0, 0, 0, 0 }), 4,
	 ...
	 */
	zassert_equal(board_power_signal_get_fake.call_count,
		      ARRAY_SIZE(signal_get_results),
		      "recorded %d calls but expected %d",
		      board_power_signal_get_fake.call_count,
		      ARRAY_SIZE(signal_get_results));
	zassert_mem_equal(board_power_signal_get_fake.arg0_history,
