...
ZTEST_SUITE(craask_form_factor, NULL, form_factor_setup, NULL, NULL, NULL);

ZTEST(craask_form_factor, test_03_clamshell_gmr_tablet_switch_disabled)
{
	const struct device *tablet_mode_gpio = DEVICE_DT_GET(
		DT_GPIO_CTLR(DT_NODELABEL(gpio_tablet_mode_l), gpios));
	const gpio_port_pins_t tablet_mode_pin =
...
ZTEST(craask_form_factor, test_04_clamshell_base_imu_irq_disabled)
{
...
ZTEST(craask_form_factor, test_01_convertible_gmr_tablet_switch_enabled)
{
	const struct device *tablet_mode_gpio = DEVICE_DT_GET(
		DT_GPIO_CTLR(DT_NODELABEL(gpio_tablet_mode_l), gpios));
	const gpio_port_pins_t tablet_mode_pin =
...
ZTEST(craask_form_factor, test_02_convertible_base_imu_irq_enabled)
{
	const struct device *base_imu_gpio = DEVICE_DT_GET(
		DT_GPIO_CTLR(DT_NODELABEL(gpio_imu_int_l), gpios));
	const gpio_port_pins_t base_imu_pin =
