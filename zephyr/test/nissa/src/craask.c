...
ZTEST(craask, test_base_orientation)
{
	...
	const void *const base_normal_rotation =
		&SENSOR_ROT_STD_REF_NAME(DT_NODELABEL(base_rot_ref));
	const void *const base_inverted_rotation =
		&SENSOR_ROT_STD_REF_NAME(DT_NODELABEL(base_rot_ver1));

	motion_sensors[BASE_SENSOR].rot_standard_ref = base_normal_rotation;
...
static bool cbi_ssfc_check_match_base_sensor(enum cbi_ssfc_value_id value_id)
{
	if (base_sensor_id == value_id)
		return true;
	else
		return false;
}
...
ZTEST(craask, test_alt_sensor_base_lsm6dso)
{
	...
	cros_cbi_ssfc_check_match_fake.custom_fake =
		cbi_ssfc_check_match_base_sensor;

	base_sensor_id = CBI_SSFC_VALUE_ID(DT_NODELABEL(base_sensor_0));
	alt_sensor_init();
...
ZTEST(craask, test_alt_sensor_base_bmi323)
{
	...
	base_sensor_id = CBI_SSFC_VALUE_ID(DT_NODELABEL(base_sensor_1));
	alt_sensor_init();
...
ZTEST(craask, test_alt_sensor_lid_lis2dw12)
{
	...
	base_sensor_id = CBI_SSFC_VALUE_ID(DT_NODELABEL(lid_sensor_0));
	alt_sensor_init();
...
ZTEST(craask, test_alt_sensor_lid_bma422)
{
	...
	base_sensor_id = CBI_SSFC_VALUE_ID(DT_NODELABEL(lid_sensor_1));
	alt_sensor_init();
