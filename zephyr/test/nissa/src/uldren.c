...
ZTEST(uldren, test_sub_board_power_handler)
{
	const struct gpio_dt_spec *s5_rail =
		GPIO_DT_FROM_ALIAS(gpio_en_sub_s5_rails);

	ap_power_ev_send_callbacks(AP_POWER_PRE_INIT);
	zassert_equal(gpio_emul_output_get(s5_rail->port, s5_rail->pin), 1);

	ap_power_ev_send_callbacks(AP_POWER_HARD_OFF);
	zassert_equal(gpio_emul_output_get(s5_rail->port, s5_rail->pin), 0);
}