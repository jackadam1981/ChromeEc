...
{
	struct fan_config fan_config_new[FAN_CH_COUNT];

	cros_cbi_get_fw_config_fake.custom_fake = get_fan_config_present;
	cbi_get_board_version_fake.custom_fake = cbi_get_board_version_1;
	fan_init();

	fan_config_new[0].tach = DEVICE_DT_GET(DT_NODELABEL(tach1));
	zassert_equal_ptr(fan_config[0].tach, fan_config_new[0].tach,
	...
	fan_config_new[0].tach = DEVICE_DT_GET(DT_NODELABEL(tach0));
	zassert_equal_ptr(fan_config[0].tach, fan_config_new[0].tach,
			  "fan_config should not change");
}
