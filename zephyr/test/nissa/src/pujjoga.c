...
ZTEST(pujjoga, test_pd_check_vbus_level)
{
	...
	/* TCPC success path */
	tcpci_get_vbus_voltage_fake.return_val = EC_SUCCESS;

	/* Case: VBUS_PRESENT → voltage above 4750 mV */
	tcpci_get_vbus_voltage_fake.custom_fake = PD_V_SAFE5V_MIN + 1;
	zassert_true(pd_check_vbus_level(port, VBUS_PRESENT), NULL);

	/* Case: VBUS_SAFE0V → voltage below or equal to 800 mV */
	tcpci_get_vbus_voltage_fake.custom_fake = PD_V_SAFE0V_MAX - 1;
	 ...
	 */
	tcpci_get_vbus_voltage_fake.custom_fake =
		PD_V_SINK_DISCONNECT_MAX - 101;
