...
FAKE_VOID_FUNC(toggle_ps8xxx_rst_gpio, int);
...
static void test_before(void *fixture)
{
	...
	RESET_FAKE(toggle_ps8xxx_rst_gpio);
}
...
ZTEST(nokris, test_reset_pd_mcu)
{
	...
	zassert_equal(toggle_ps8xxx_rst_gpio_fake.call_count, 0);

	nissa_get_sb_type_fake.return_val = NISSA_SB_C_A;
	board_reset_pd_mcu();
	zassert_equal(nct38xx_reset_notify_fake.call_count, 2);
	zassert_equal(nct38xx_reset_notify_fake.arg0_val, 0);
	zassert_equal(toggle_ps8xxx_rst_gpio_fake.call_count, 1);
}
