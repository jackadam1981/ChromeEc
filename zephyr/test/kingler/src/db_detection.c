...
#define LOG_LEVEL 0
LOG_MODULE_REGISTER(db_detection);
...
struct db_detection_fixture {
	int place_holder;
};

static void *db_detection_setup(void)
{
	static struct db_detection_fixture f;

	return &f;
}

static void db_detection_reset_rule_before(const struct ztest_unit_test *test,
					   void *data)
{
	ARG_UNUSED(test);
	ARG_UNUSED(data);
...
ZTEST_RULE(db_detection_reset_rule, db_detection_reset_rule_before, NULL);
ZTEST_SUITE(db_detection, NULL, db_detection_setup, NULL, NULL, NULL);

ZTEST_F(db_detection, test_detect_db)
{
	struct {
