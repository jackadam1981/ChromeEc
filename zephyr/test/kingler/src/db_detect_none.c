...
ZTEST(db_detection, test_db_detect_none)
{
	...
	zassert_ok(gpio_emul_flags_get(int_x_ec_gpio1, int_x_ec_pin1, flags),
		   NULL);
	zassert_equal(*flags, (GPIO_INPUT | GPIO_PULL_UP), "flags=%d", *flags);
	zassert_ok(gpio_emul_flags_get(int_x_ec_gpio2, int_x_ec_pin2, flags),
		   NULL);
	zassert_equal(*flags, (GPIO_INPUT | GPIO_PULL_UP), "flags=%d", *flags);
	zassert_ok(gpio_emul_flags_get(int_x_ec_gpio3, int_x_ec_pin3, flags),
		   NULL);
	zassert_equal(*flags, (GPIO_INPUT | GPIO_PULL_UP), "flags=%d", *flags);
	free(flags);
	...
	interrupt_count = 0;
	zassert_ok(gpio_emul_input_set(int_x_ec_gpio2, int_x_ec_pin2, 0), NULL);
	k_sleep(K_MSEC(100));
	zassert_ok(gpio_emul_input_set(int_x_ec_gpio2, int_x_ec_pin2, 1), NULL);
	k_sleep(K_MSEC(100));
