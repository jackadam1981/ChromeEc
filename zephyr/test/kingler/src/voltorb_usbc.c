...
enum chipset_state_mask set_mask;
...
ZTEST(current_limit, test_check_src_port_1)
{
	...
	set_mask = CHIPSET_STATE_SUSPEND;
	hook_notify(HOOK_CHIPSET_SUSPEND);
	k_sleep(K_SECONDS(3));
	zassert_equal(1, check_src_port_fake.call_count,"call_count is = %d",
		check_src_port_fake.call_count);
	zassert_equal(1, dpm_get_source_pdo(fake_pdo, fake_port),"call_count is = %d",
		check_src_port_fake.call_count);
}
...
ZTEST(current_limit, test_check_src_port_4)
{
	...
	charge_get_percent_fake.return_val = 40;
	tc_is_attached_src_fake.custom_fake = tc_is_attached_src_mock;
	set_mask = CHIPSET_STATE_SOFT_OFF;
	hook_notify(HOOK_CHIPSET_SUSPEND);
...
{
	tc_is_attached_src_fake.custom_fake = tc_is_attached_src_mock;
	set_mask = CHIPSET_STATE_ON;
	hook_notify(HOOK_CHIPSET_RESUME);
	k_sleep(K_SECONDS(3));
	zassert_equal(1, resume_src_port_fake.call_count, "call_count is = %d",
		resume_src_port_fake.call_count);
}
