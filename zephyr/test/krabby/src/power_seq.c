...
static void power_chipset_init_subtest(enum power_state signal_state,
				       ...
				       int line)
{
	extern bool is_exiting_off;
	extern bool is_shutdown;
	extern bool is_resetting;

	/* Reset global variables to their initial value.
	 * Need this workaround because we don't expect power_chipset_init() to
	 * be called multiple times in normal flow.
	 */
	is_exiting_off = true;
	is_shutdown = false;
	is_resetting = false;
	...
	enum power_state init_state = power_chipset_init();

	if (expected_state == init_state) {
		zassert_false(is_exiting_off,
			      "test_power_chipset_init line %d failed", line);
		zassert_false(is_shutdown,
			      "test_power_chipset_init line %d failed", line);
	} else if (expected_state == POWER_S0 && init_state == POWER_G3) {
		zassert_true(is_exiting_off,
			     "test_power_chipset_init line %d failed", line);
		zassert_false(is_shutdown,
			      "test_power_chipset_init line %d failed", line);
	} else if (expected_state == POWER_G3 && init_state == POWER_S0) {
		zassert_false(is_exiting_off,
			      "test_power_chipset_init line %d failed", line);
		zassert_true(is_shutdown,
			     "test_power_chipset_init line %d failed", line);
	} else {
		zassert_unreachable();
	}
