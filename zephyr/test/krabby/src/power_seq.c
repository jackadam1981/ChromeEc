...
static void pp4200_handler(const struct device *port, struct gpio_callback *cb, gpio_port_pins_t pins)
{
	const struct gpio_dt_spec *en_pp4200_s5 = GPIO_DT_FROM_NODELABEL(en_pp4200_s5);
	const struct gpio_dt_spec *pg_pp4200_s5_od = GPIO_DT_FROM_NODELABEL(pg_pp4200_s5_od);
...
{
	zassert_ok(gpio_add_callback_dt(GPIO_DT_FROM_NODELABEL(en_pp4200_s5), &pp4200_callback));
...
{
	zassert_ok(gpio_remove_callback_dt(GPIO_DT_FROM_NODELABEL(en_pp4200_s5), &pp4200_callback));
}
#else
static void *power_seq_setup(void)
{
...
ZTEST_SUITE(power_seq, krabby_predicate_post_main, power_seq_setup, power_seq_before, NULL,
	    power_seq_teardown);
