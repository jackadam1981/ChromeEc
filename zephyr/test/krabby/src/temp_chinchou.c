					...
					uint16_t current,enum power_state power)
{
	...
	ignore_first_minute();
	wait_heat_stable(&curr);
	zassert_equal(curr.requested_current, current,"curr.requested_current:%d,current:%d",curr.requested_current,current);
}
...
ZTEST(temp_current, test_current_limit_in_each_zone)
{
	...
	for (int i = 0; i < ARRAY_SIZE(testdata); i++) {
		test_table(testdata[i].batt, testdata[i].chgv1, testdata[i].chgv2, 
				   testdata[i].current, testdata[i].power);
	}
}
