...
{
	const size_t params_buffer_size = 16;
	uint8_t params_buffer[params_buffer_size];

	struct ec_params_fp_template *params =
		(struct ec_params_fp_template *)params_buffer;
	BUILD_ASSERT(params_buffer_size > sizeof(*params));

	const size_t data_size = params_buffer_size - sizeof(*params);
	...
	while (offset < FP_ALGORITHM_ENCRYPTED_TEMPLATE_SIZE) {
		...
		zassert_ok(
			ec_cmd_fp_template(NULL, params, params_buffer_size));
	}
...
ZTEST_USER(fpsensor_template, test_fp_template_load_template_invalid_tag)
{
	...
	while (offset < FP_ALGORITHM_ENCRYPTED_TEMPLATE_SIZE) {
		...
		if (offset != FP_ALGORITHM_ENCRYPTED_TEMPLATE_SIZE) {
			...
			zassert_equal(EC_RES_UNAVAILABLE,
				      ec_cmd_fp_template(NULL, params,
							 params_buffer_size));
		}
	}
