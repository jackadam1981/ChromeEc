	   ...
	   test_fp_frame_v1_encryption_status_ready_set_and_cleared)
{
	...
	/* Verify FP_TEMPLATE_ENCRYPTION_IN_PROGRESS IS set. */
	zassert_ok(ec_cmd_fp_encryption_status(NULL, &status_response));
	zassert_true(status_response.status &
	...
	/* Verify FP_TEMPLATE_ENCRYPTION_IN_PROGRESS IS NOT set. */
	zassert_false(status_response.status &
		      FP_TEMPLATE_ENCRYPTION_IN_PROGRESS);
