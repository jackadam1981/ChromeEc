...
{
	return std::make_unique<FpFrameSizeCache>(sizeof(image_buffer));
}
...
static void fpsensor_frame_size_setup(void *fixture)
{
	...
	zassert_not_null(cache_fixture.get(),
			 "FpFrameSizeCache failed unexpectedly in setup.");
}
...
ZTEST(fpsensor_frame_size, test_cache_initialization_on_size_constraint_failure)
{
	...
	std::unique_ptr<FpFrameSizeCache> failing_cache =
		std::make_unique<FpFrameSizeCache>(max_frame_size_bytes);

	zassert_false(
		failing_cache->is_initialized(),
		"Cache must be uninitialized when max_frame_size_bytes is exceeded");
