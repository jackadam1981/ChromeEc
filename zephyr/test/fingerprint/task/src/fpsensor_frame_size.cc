...
static const struct fingerprint_image_frame_params image_frame_params_array[] =
...
ZTEST(fpsensor_frame_size, test_cache_initialization_and_lookup) {
  ...
  zassert_not_null(cache.get(),
                   "FpFrameSizeCache::Create() failed (returned nullptr).");

  for (const auto& image_frame_params : image_frame_params_array) {
    ...
    zassert_equal(expected_size, actual_size,
                  "Mismatch for type %d. Expected: %u, Actual: %u", (int)type,
