...
static int get_sdcp_claim_custom_ret;

int get_sdcp_claim(ec_response_fp_sdcp_claim *res)
{
	std::ranges::copy(pk_f, std::begin(res->pk_f));
	return get_sdcp_claim_custom_ret;
}

static std::array<uint8_t, 32> get_sdcp_sk_f_custom_sk_f;
static int get_sdcp_sk_f_custom_ret;

int get_sdcp_sk_f(uint8_t *buf, size_t buf_size)
{
	zassert_equal(buf_size, 32);
	zassert_not_equal(buf, nullptr);
	std::ranges::copy(get_sdcp_sk_f_custom_sk_f, buf);
	return get_sdcp_sk_f_custom_ret;
}
...
{
	char console_input[] = "fpsdcp";
	get_sdcp_claim_custom_ret = EC_RES_ERROR;
	int rv = shell_execute_cmd(get_ec_shell(), console_input);
...
{
	int ret;
	ec_params_fp_sdcp_establish params;
	std::ranges::copy(pk_g, std::begin(params.pk_g));

	ret = ec_cmd_fp_sdcp_establish(nullptr, &params);
	zassert_equal(EC_RES_SUCCESS, ret);
...
ZTEST(hc_fp_sdcp, test_fp_sdcp_establish_get_sdcp_sk_f_not_ok)
{
	int ret;
	ec_params_fp_sdcp_establish params;
	std::ranges::copy(pk_g, std::begin(params.pk_g));

	get_sdcp_sk_f_custom_ret = EC_ERROR_UNKNOWN;
...
ZTEST(hc_fp_sdcp, test_fp_sdcp_establish_sk_f_not_ok)
{
	...
	std::ranges::fill(get_sdcp_sk_f_custom_sk_f, 0);
...
static void reset(void *data)
{
	...
	get_sdcp_claim_custom_ret = 0;
	get_sdcp_sk_f_custom_ret = 0;
	std::ranges::copy(sk_f, std::begin(get_sdcp_sk_f_custom_sk_f));
}
