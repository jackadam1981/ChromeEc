...
static std::array<uint8_t, 32> fp_sdcp_sk_f_custom_sk_f;
...
int fp_sdcp_sk_f(uint8_t *buf, size_t buf_size)
{
	...
	std::ranges::copy(fp_sdcp_sk_f_custom_sk_f, buf);
...
ZTEST(hc_fp_sdcp, test_fp_sdcp_establish_sk_f_not_ok)
{
	...
	std::ranges::fill(fp_sdcp_sk_f_custom_sk_f, 0);
...
static void reset(void *data)
{
	...
	std::ranges::copy(sk_f, std::begin(fp_sdcp_sk_f_custom_sk_f));
}
