...
static int get_ascp_claim_custom_ret;

int get_ascp_claim(ec_response_fp_ascp_claim *res)
{
	std::ranges::copy(pk_f, std::begin(res->pk_f));
	return get_ascp_claim_custom_ret;
}

static std::array<uint8_t, 32> get_ascp_sk_f_custom_sk_f;
static int get_ascp_sk_f_custom_ret;

uint8_t *get_ascp_sk_f()
{
	if (get_ascp_sk_f_custom_ret != 0)
		return nullptr;
	return get_ascp_sk_f_custom_sk_f.data();
}

const STRUCT_SECTION_ITERABLE(ascp_api, ascp_api_test) = {
	.get_claim = get_ascp_claim,
	.get_sk_f = get_ascp_sk_f,
};

ZTEST(hc_fp_ascp, test_fp_ascp_ok)
{
	int ret;
...
ZTEST(hc_fp_ascp, test_fp_ascp_custom_not_ok)
{
	int ret;
	ec_response_fp_ascp_claim res;

	get_ascp_claim_custom_ret = -1;
	ret = ec_cmd_fp_ascp_claim(nullptr, &res);
	zassert_true(std::ranges::equal(res.pk_f, pk_f));
	zassert_equal(EC_RES_ERROR, ret);
}

ZTEST(hc_fp_ascp, test_fp_ascp_args_not_ok)
{
	int ret;
	int too_small_res;
...
ZTEST(hc_fp_ascp, test_shell_command_sucess)
{
...
ZTEST(hc_fp_ascp, test_shell_command_failure)
{
	char console_input[] = "fpascp";
	get_ascp_claim_custom_ret = EC_RES_ERROR;
	int rv = shell_execute_cmd(get_ec_shell(), console_input);
...
ZTEST(hc_fp_ascp, test_fp_ascp_establish_get_ascp_sk_f_not_ok)
{
	int ret;
	ec_params_fp_ascp_establish params;
	std::ranges::copy(pk_g, std::begin(params.pk_g));

	get_ascp_sk_f_custom_ret = -1;
...
ZTEST(hc_fp_ascp, test_fp_ascp_establish_sk_f_not_ok)
{
	...
	std::ranges::fill(get_ascp_sk_f_custom_sk_f, 0);
...
static void reset(void *data)
{
	...
	get_ascp_claim_custom_ret = 0;
	get_ascp_sk_f_custom_ret = 0;
	std::ranges::copy(sk_f, std::begin(get_ascp_sk_f_custom_sk_f));
}
