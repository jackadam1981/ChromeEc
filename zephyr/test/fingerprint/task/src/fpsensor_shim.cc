...
ZTEST_USER(fpsensor_shim, test_shim_get_info_success)
{
	...
	for (int i = 0; i < NUM_IMAGE_CAPTURE_TYPES; ++i) {
		zassert_equal(
			test_info_buffer->image_frame_params[i].frame_size,
			frame_size[i]);
		zassert_equal(
			test_info_buffer->image_frame_params[i].pixel_format,
			pixel_format[i]);
		zassert_equal(test_info_buffer->image_frame_params[i].width,
			      width[i]);
		zassert_equal(test_info_buffer->image_frame_params[i].height,
			      height[i]);
		zassert_equal(test_info_buffer->image_frame_params[i].bpp,
			      bpp[i]);
		zassert_equal(
			test_info_buffer->image_frame_params[i].fp_capture_type,
			capture_types[i]);
	}
}
...
ZTEST_SUITE(fpsensor_shim, NULL, NULL, fpsensor_before, fpsensor_after, NULL);
