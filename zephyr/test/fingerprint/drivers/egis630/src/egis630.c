...
{
	size_t size = 64;
	void *ptr = sys_alloc(1, size);
	zassert_not_null(ptr, "sys_alloc should return a valid pointer");
	sys_free(ptr);
}
...
ZTEST_F(egis630, test_output_log_success)
{
	char expected_printf_buffer[14] = "<func:10> info";

	egis630_output_log(LOG_INFO, "tag", "file", "func", 10, "info %f");

	zassert_mem_equal(printf_buffer, expected_printf_buffer,
			  sizeof(expected_printf_buffer),
			  "LOG message not match expected message");
}

ZTEST_F(egis630, test_output_log_format_NULL)
{
	char expected_printf_buffer[14] = "";

	egis630_output_log(LOG_INFO, "tag", "file", "func", 10, NULL);
