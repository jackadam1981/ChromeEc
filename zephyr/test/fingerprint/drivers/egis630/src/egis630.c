...
typedef enum {
	...
	kInitHwidNotFound = 2,
	kInitIrqFailure = 3,
	kInitUnknownFailure = 4,
} InitStatus;

static bool egis_get_hwid_succeed = true;
static InitStatus init_status = kInitSuccess;

static void egis630_before(void *data)
{
	memset(printf_buffer, 0, sizeof(printf_buffer));
	g_log_level = LOG_INFO;
	egis_get_hwid_succeed = true;
	init_status = kInitSuccess;
...
{
	const struct emul *target = EMUL_DT_GET(DT_NODELABEL(egis630));

	switch (init_status) {
	case kInitSuccess:
		egis630_emul_gpio_flip_irq(target);
		return EGIS_API_OK;
	case kInitSpiFailure:
		egis630_emul_gpio_flip_irq(target);
		return EGIS_API_ERROR_IO_SPI;
	case kInitHwidNotFound:
		egis630_emul_gpio_flip_irq(target);
		return EGIS_API_ERROR_DEVICE_NOT_FOUND;
	case kInitIrqFailure:
		return EGIS_API_OK;
	case kInitUnknownFailure:
		egis630_emul_gpio_flip_irq(target);
		return !EGIS_API_OK;
	default:
		egis630_emul_gpio_flip_irq(target);
...
ZTEST_F(egis630, test_init_spi_failure)
{
	init_status = kInitSpiFailure;
...
ZTEST_F(egis630, test_init_bad_hwid)
{
	init_status = kInitHwidNotFound;
	struct fingerprint_info info;
...
ZTEST_F(egis630, test_init_irq_failure)
{
	init_status = kInitIrqFailure;
	struct fingerprint_info info;
...
ZTEST_F(egis630, test_init_unknown_failure)
{
	init_status = kInitUnknownFailure;
	struct fingerprint_info info;
...
ZTEST_F(egis630, test_get_info)
{
	...
	zassert_equal(info.model_id, 0x276);
...
ZTEST_F(egis630, test_periphery_spi_write_read)
{
	uint8_t tx_buf[] = { 0xFD, 0x13, 0x54 };
...
ZTEST_F(egis630, test_egis630_rbs_check_if_null_error)
{
	int error_code = 123;
	char expected_printf_buffer[sizeof(printf_buffer)] =
		"Error~! <z_impl_egis630_rbs_check_if_null:53> "
...
ZTEST_F(egis630, test_CPRINT_success)
{
	CPRINTF("This is the %s output number %d\n", "last", 5);
	CPRINTS("This is the %s output number %d", "last", 5);
	zassert_ok(0);
}
...
ZTEST_F(egis630, test_egislog_e)
{
	...
	char expected_printf_buffer[sizeof(printf_buffer)];
	memset(expected_printf_buffer, 0, sizeof(printf_buffer));
	snprintf(expected_printf_buffer, sizeof(printf_buffer),
		 "Error~! <egis630_test_egislog_e:%d> The output is 6 and 2.3",
		 __LINE__ - 5);
	zassert_mem_equal(printf_buffer, expected_printf_buffer,
			  sizeof(printf_buffer),
			  "LOG message not match expected message");
