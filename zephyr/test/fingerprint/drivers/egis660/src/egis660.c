...
ZTEST_F(egis660, test_init_success)
{
	...
	zassert_equal(egis660_get_low_power_mode(fixture->target), 0);
}

// ZTEST_F(egis660, test_init_failure_bad_hwid)
//{
//	egis660_set_hwid(fixture->target, 0x0);
...
ZTEST_F(egis660, test_get_info)
{
	 ...
	 */
	zassert_equal(sensor_info.model_id, 0x2d1);
...
ZTEST_F(egis660, test_enter_low_power_mode)
{
...
ZTEST_F(egis660, test_pal_check_irq)
{
	...
	zassert_equal(egis660_pal_spi_check_irq(), true);
	gpio_emul_input_set(irq_pin.port, irq_pin.pin, 0);
	zassert_equal(egis660_pal_spi_check_irq(), false);
}
...
ZTEST_F(egis660, test_pal_read_irq)
{
	...
	gpio_emul_input_set(irq_pin.port, irq_pin.pin, 1);
	zassert_equal(egis660_pal_spi_read_irq(), true);
	gpio_emul_input_set(irq_pin.port, irq_pin.pin, 0);
	zassert_equal(egis660_pal_spi_read_irq(), false);
}
...
ZTEST_F(egis660, test_pal_oops_on_memory_alloc_fail)
{
	expected_reason = K_ERR_KERNEL_OOPS;
	egis660_pal_malloc(CONFIG_FINGERPRINT_SENSOR_EGIS660_HEAP_SIZE);
