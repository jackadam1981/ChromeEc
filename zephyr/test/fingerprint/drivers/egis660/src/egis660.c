...
ZTEST_F(egis660, test_init_success)
{
	...
	zassert_equal(egis660_get_low_power_mode(fixture->target), 1);
}
...
ZTEST_F(egis660, test_pal_timebase_delay_ms)
{
	...
	/* Wait 100ms, it's not accuate */
	egis660_pal_timebase_delay_ms(100);

	t2 = egis660_pal_timebase_get_tick();

	/* Check that at least 100ms passed but not more than 110ms */
	zassert_true((t2 - t1) >= 100 && (t2 - t1) <= 110);
}
