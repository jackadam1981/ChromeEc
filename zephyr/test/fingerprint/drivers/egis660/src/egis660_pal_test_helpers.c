...
{
	return egis_sensor_spi_write_read(data, write_size, read_size, leave_cs_asserted);
}

bool z_impl_egis660_pal_spi_check_irq(void)
{
	return egis_sensor_spi_check_irq();
}

bool z_impl_egis660_pal_spi_read_irq(void)
{
	return egis_sensor_spi_read_irq();
}

void z_impl_egis660_pal_spi_reset(bool state)
{
	return egis_sensor_spi_reset(state);
}
