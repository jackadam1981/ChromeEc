...
ZTEST_F(fpc1025, test_get_info)
{
	...
	for (int i = 0; i < NUM_IMAGE_CAPTURE_TYPES; ++i) {
		zassert_equal(image_frame_params_array[i].frame_size,
			      expected_image_frame_params_array[i].frame_size);
		zassert_equal(
			image_frame_params_array[i].pixel_format,
			expected_image_frame_params_array[i].pixel_format);
		zassert_equal(image_frame_params_array[i].width,
			      expected_image_frame_params_array[i].width);
		zassert_equal(image_frame_params_array[i].height,
			      expected_image_frame_params_array[i].height);
		zassert_equal(image_frame_params_array[i].bpp,
			      expected_image_frame_params_array[i].bpp);
		zassert_equal(
			image_frame_params_array[i].fp_capture_type,
			expected_image_frame_params_array[i].fp_capture_type);
	}
}
