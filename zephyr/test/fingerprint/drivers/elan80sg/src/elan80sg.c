...
ZTEST_F(elan80sg, test_init_success)
{
	zassert_ok(fingerprint_init(fixture->dev));
}

ZTEST_F(elan80sg, test_init_failure_bad_hwid)
{
	elan80sg_set_hwid(fixture->target, 0x0, 0x0);
	zassert_equal(fingerprint_init(fixture->dev), -ENOTSUP);
}

ZTEST_F(elan80sg, test_init_failure_spi)
{
	struct fingerprint_info info;

	elan80sg_stop_spi(fixture->target);
	zassert_equal(fingerprint_init(fixture->dev), -ENOTSUP);
	zassert_not_ok(fingerprint_get_info(fixture->dev, &info));
	elan80sg_start_spi(fixture->target);
...
ZTEST_F(elan80sg, test_get_info)
{
	struct fingerprint_info info;
	...
	zassert_equal(info.vendor_id, FOURCC('E', 'L', 'A', 'N'));
	zassert_equal(info.product_id, 0x0903);
	zassert_equal(info.model_id, 0x4f4f);
	zassert_equal(info.version, 0x100B);
	...
	zassert_equal(info.pixel_format, FINGERPRINT_SENSOR_V4L2_PIXEL_FORMAT(
						 DT_NODELABEL(elan80sg)));
	zassert_equal(info.width,
		      FINGERPRINT_SENSOR_RES_X(DT_NODELABEL(elan80sg)));
	zassert_equal(info.height,
		      FINGERPRINT_SENSOR_RES_Y(DT_NODELABEL(elan80sg)));
	zassert_equal(info.bpp,
		      FINGERPRINT_SENSOR_RES_BPP(DT_NODELABEL(elan80sg)));
...
ZTEST_F(elan80sg, test_interrupt)
{
	const struct gpio_dt_spec spec =
		GPIO_DT_SPEC_GET(DT_NODELABEL(elan80sg), irq_gpios);
...
ZTEST_F(elan80sg, test_acquire_image_wrong_capture_type)
{
	uint8_t buffer[CONFIG_FINGERPRINT_SENSOR_IMAGE_SIZE] = { 0 };
	enum fingerprint_capture_type capture_type =
		FINGERPRINT_CAPTURE_TYPE_MAX;
...
ZTEST_F(elan80sg, test_elan_read_register)
{
	...
	zassert_ok(elan80sg_pal_read_register(hwid_lo_cmd, &regdata));
	zassert_equal(expected_hwid_lo_regdata, regdata);

	zassert_ok(elan80sg_pal_read_register(hwid_hi_cmd, &regdata));
	zassert_equal(expected_hwid_hi_regdata, regdata);
}

ZTEST_F(elan80sg, test_elan_read_register_spi_stoppped)
{
	uint8_t hwid_lo_cmd = 0x04;
	uint8_t regdata = 0;

	elan80sg_stop_spi(fixture->target);
	zassert_equal(elan80sg_pal_read_register(hwid_lo_cmd, &regdata), -EIO);
}

ZTEST_F(elan80sg, test_elan_read_cmd)
{
	...
	zassert_ok(elan80sg_pal_read_cmd(cmd_hwid_lo_cmd, &regdata));
	zassert_equal(expected_hwid_lo_regdata, regdata);

	zassert_ok(elan80sg_pal_read_cmd(cmd_hwid_hi_cmd, &regdata));
	zassert_equal(expected_hwid_hi_regdata, regdata);
}

ZTEST_F(elan80sg, test_elan_read_cmd_spi_stoppped)
{
	uint8_t cmd_hwid_lo_cmd = 0x44;
	uint8_t regdata = 0;

	elan80sg_stop_spi(fixture->target);
	zassert_equal(elan80sg_pal_read_cmd(cmd_hwid_lo_cmd, &regdata), -EIO);
}
...
ZTEST_F(elan80sg, test_elan_usleep)
{
	...
	t1 = k_uptime_get();
	zassert_ok(elan80sg_pal_usleep(usecs));
	t2 = k_uptime_get();

	zassert_within((t2 - t1) * USEC_PER_MSEC, usecs, tick_usecs);
}

ZTEST_F(elan80sg, test_pal_get_tick)
{
	zassert_equal(elan80sg_pal_get_tick(), k_uptime_get_32());
}

ZTEST_F(elan80sg, test_elan_malloc)
{
	size_t size = sizeof(int);
	void *void_ptr = elan80sg_pal_malloc(size);
...
ZTEST_F(elan80sg, test_pal_oops_on_memory_alloc_fail)
{
	expected_reason = K_ERR_KERNEL_OOPS;
	elan80sg_pal_malloc(CONFIG_FINGERPRINT_SENSOR_ELAN80SG_HEAP_SIZE);

	ztest_test_fail();
}

ZTEST_F(elan80sg, test_pal_reset_pin)
{
	const struct gpio_dt_spec reset_pin =
		GPIO_DT_SPEC_GET(DT_NODELABEL(elan80sg), reset_gpios);

	/* Reset is active when GPIO output is low. */
	elan80sg_pal_sensor_set_rst(true);
	zassert_equal(gpio_emul_output_get(reset_pin.port, reset_pin.pin), 0);

	/* Reset is inactive when GPIO output is high. */
	elan80sg_pal_sensor_set_rst(false);
