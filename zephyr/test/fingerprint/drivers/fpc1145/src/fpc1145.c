...
ZTEST_F(fpc1145, test_get_info)
{
	...
	for (int i = 0; i < NUM_IMAGE_CAPTURE_TYPES; ++i) {
		zassert_equal(image_frame_params_array[i].frame_size, 10752);
		zassert_equal(image_frame_params_array[i].pixel_format,
			      V4L2_PIX_FMT_GREY);
		zassert_equal(image_frame_params_array[i].width, 56);
		zassert_equal(image_frame_params_array[i].height, 192);
