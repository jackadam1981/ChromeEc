...
ZTEST_F(fpc1145, test_pal_memory_alloc)
{
	void *p = NULL;
	int32_t ret;

	ret = fpc1145_pal_malloc(
		&p, CONFIG_FINGERPRINT_SENSOR_FPC1145_HEAP_SIZE / 2);
	zassert_equal(ret, 0);
	zassert_not_null(p);
...
ZTEST_F(fpc1145, test_pal_oops_on_memory_alloc_fail)
{
	void *p = NULL;

	expected_reason = K_ERR_KERNEL_OOPS;
	fpc1145_pal_malloc(&p, CONFIG_FINGERPRINT_SENSOR_FPC1145_HEAP_SIZE);
