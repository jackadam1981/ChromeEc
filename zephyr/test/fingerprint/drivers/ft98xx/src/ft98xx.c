...
ZTEST_F(ft98xx, test_enter_idle)
{
...
ZTEST_F(ft98xx, test_maintenance_not_supported)
{
...
ZTEST_F(ft98xx, test_maintenance_image_small_buffer_size)
{
	uint8_t buffer[FP_SENSOR_IMAGE_SIZE - 1];
...
ZTEST_F(ft98xx, test_periphery_spi_write_read_success)
{
	...
	uint8_t rx_buf[3] = { 0 };
	uint8_t expeceted_rx_buf[3] = { 0x1, 0x49, 0x98 };
