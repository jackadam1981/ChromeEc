...
/* Second for platform/ec task API (in micro seconds). */
...
static void test_task_get_current1(void)
{
	zassert_equal(task_get_current(), TASK_ID_TASK_1, "ID matches");
}

static void test_task_get_current2(void)
{
...
{
	run_test(&test_task_get_current1, &test_task_get_current2);
}
...
static void empty_set_mask1(void)
{
	 ...
	 * condition  and excersise fallback code in task_wait_event
	 */
...
static void empty_set_mask2(void)
{
	...
	zassert_within(end_ms - start_ms, 2000, 100, "Timeout for 1 second");
}
