...
struct usbc_svdm_dfp_only_fixture {
	...
	struct tcpci_snk_emul_data snk_emul_data;
};
 ...
 * if any extra (or too few) PD messages are sent.
 */
...
get_identity_response(struct usbc_svdm_dfp_only_fixture *fixture)
{
	...
	zassert_true(sys_slist_is_empty(&fixture->partner.msg_log),
		     "logged more than two PD messages");
...
/**
 * Verify the values of the common VDOs which don't vary with PD version:
 *  * VDM Header
 *  * Cert Stat VDO
 *  * Product VDO
 */
static void verify_common_fields(const struct identity_response response)
{
...
ZTEST_F(usbc_svdm_dfp_only, verify_identity)
{
...
ZTEST_F(usbc_svdm_dfp_only, verify_pd20_nak)
{
...
static void *usbc_svdm_dfp_only_setup()
{
	...
	fixture.partner.extensions = tcpci_snk_emul_init(
		&fixture.snk_emul_data, &fixture.partner, NULL);
	fixture.snk_emul_data.pdo[0] =
		PDO_FIXED(5000, 3000, PDO_FIXED_COMM_CAP);
