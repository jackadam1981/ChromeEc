...
/*
 * TODO(b/245338219): What is the expected behavior of
 * mkbp_fifo_get_next_event() after a mkbp_fifo_depth_update()?
 */
ZTEST_EXPECT_FAIL(mkbp_fifo, test_fifo_depth_update_after_add);
ZTEST_F(mkbp_fifo, test_fifo_depth_update_after_add)
{
	uint8_t out[KEY_MATRIX_EVENT_DATA_SIZE + 1];
	uint8_t new_depth = 0;
