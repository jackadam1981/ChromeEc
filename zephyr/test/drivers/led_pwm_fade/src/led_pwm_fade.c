...
ZTEST(led_pwm_fade, test_led_fade)
{
	...
	/*
	 * LED starts off at a minimum of 0.0001% brightness when on
	 * exponential scale. So it reaches 10% brightness after 5s
	 * and reaches 100% brightness after 6s.
	 */
	...
	zassert_equal(pwm_mock_get_duty(pwm_white_left, 0), 0, NULL);
	zassert_equal(pwm_mock_get_duty(pwm_amber_right, 0), 0, NULL);
	zassert_true(pwm_mock_get_duty(pwm_white_right, 0) < 3, NULL);
}
