...
ZTEST_F(usbc_usb4_mode, verify_usb4_entry_fail)
{
	...
	/* TODO(b/260095516): Test EC-driven mode entry (requires a separate
	 * config).
	 */
	...
	/* Verify we refrained from sending USB_Enter. */
...
ZTEST_F(usbc_usb4_mode, verify_usb4_passive_entry_exit)
{
	...
	/* TODO(b/260095516): Test EC-driven mode entry (requires a separate
	 * config). In addition, could validate the contents of the USB_Enter
	 * message here, if desired using message log. PD logs are enabled
	 * enabled here for this test, but messages from the logs are not being
	 * explicitly checked.
	 */
	tcpci_partner_common_enable_pd_logging(&fixture->partner, true);
