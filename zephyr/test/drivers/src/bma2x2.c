...
/**
 * Custom emulator function which returns error when data.reg is accessed or
 * always when data.reg equals -1.
 */
...
static void test_bma_set_offset(void)
{
	...
	/* Remove custom emulator write function */
...
static void test_bma_set_range(void)
{
	...
	/* Setup emulator fail read function */
	func_data.reg = BMA2x2_RANGE_SELECT_ADDR;
	bma_emul_set_read_func(emul, emul_read_func_fail, &func_data);
...
static void test_bma_init(void)
{
	...
	/* Setup emulator fail read function */
	func_data.reg = BMA2x2_CHIP_ID_ADDR;
	bma_emul_set_read_func(emul, emul_read_func_fail, &func_data);
...
static void test_bma_rate(void)
{
	...
	/* Setup emulator fail read function */
	func_data.reg = BMA2x2_BW_SELECT_ADDR;
	bma_emul_set_read_func(emul, emul_read_func_fail, &func_data);
...
static void test_bma_read(void)
{
	...
	/* Test fail on each axis */
	func_data.reg = BMA2x2_X_AXIS_LSB_ADDR;
	zassert_equal(-EIO, ms.drv->read(&ms, ret_acc_v), NULL);
	func_data.reg = BMA2x2_X_AXIS_MSB_ADDR;
	zassert_equal(-EIO, ms.drv->read(&ms, ret_acc_v), NULL);
	func_data.reg = BMA2x2_Y_AXIS_LSB_ADDR;
	zassert_equal(-EIO, ms.drv->read(&ms, ret_acc_v), NULL);
	func_data.reg = BMA2x2_Y_AXIS_MSB_ADDR;
	zassert_equal(-EIO, ms.drv->read(&ms, ret_acc_v), NULL);
	func_data.reg = BMA2x2_Z_AXIS_LSB_ADDR;
	zassert_equal(-EIO, ms.drv->read(&ms, ret_acc_v), NULL);
	func_data.reg = BMA2x2_Z_AXIS_MSB_ADDR;
	zassert_equal(-EIO, ms.drv->read(&ms, ret_acc_v), NULL);
