...
ZTEST_USER(espi_peripheral, test_host_read_and_write_kb_char)
{
	...
	espi_emul_io_read(ESPI_DEVICE, 1, EMUL_ESPI_KBC8042_PORT_IN_DATA, &reg);
...
ZTEST_USER(espi_peripheral, test_lpc_input_pending)
{
	...
	espi_emul_io_write(ESPI_DEVICE, 1,
		EMUL_ESPI_KBC8042_PORT_OUT_DATA, reg);
...
ZTEST_USER(espi_peripheral, test_lpc_kb_put_char)
{
	...
	espi_emul_io_read(ESPI_DEVICE, 1,
		EMUL_ESPI_KBC8042_PORT_IN_DATA, &reg_recv);
...
ZTEST_SUITE(espi_peripheral, drivers_predicate_post_main,
	NULL, NULL, NULL, NULL);
