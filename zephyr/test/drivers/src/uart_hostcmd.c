 ...
 */
static void setup_snapshots_and_messages(void)
{
	char response[1024];
...
static void test_uart_hc_read_next(int ver)
{
	...
	zassert_equal(EC_RES_SUCCESS, host_command_process(&args), NULL);
	zassert_equal('\0', response[args.response_size],
	 ...
	 * NULL char.
	 */
	msg2_start = response + args.response_size - 1 - MSG_LEN(msg2);
...
static void test_uart_hc_read_recent_v1(void)
{
	...
	/* Only message after second snapshot should be read */
	zassert_equal(EC_RES_SUCCESS, host_command_process(&args), NULL);
	zassert_equal('\0', response[args.response_size],
		      "Last byte of response is not '\\0' (got 0x%x)",
		      response[args.response_size]);
	/* Account additional NULL char at the end */
	zassert_equal(MSG_LEN(msg2) + 1, args.response_size,
		      "expected message length %d, got %d",
		      MSG_LEN(msg2) + 1, args.response_size);
...
void test_suite_uart_hostcmd(void)
{
	ztest_test_suite(uart_hostcmd,
			 ztest_unit_test_setup_teardown(
				test_uart_hc_read_next_v0,
				setup_snapshots_and_messages, unit_test_noop),
			 ztest_unit_test_setup_teardown(
				test_uart_hc_read_next_v1,
				setup_snapshots_and_messages, unit_test_noop),
			 ztest_unit_test_setup_teardown(
				test_uart_hc_read_recent_v1,
				setup_snapshots_and_messages, unit_test_noop));
	ztest_run_test_suite(uart_hostcmd);
}
