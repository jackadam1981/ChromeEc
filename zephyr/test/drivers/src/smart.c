...
static void test_battery_fake_charge(void)
{
	...
	/* Fail on command with too big charge level */
	zassert_equal(EC_ERROR_PARAM1,
		      shell_execute_cmd(shell_backend_uart_get_ptr(),
					"battfake 123"), NULL);

	/* Fail on command with too small charge level */
	zassert_equal(EC_ERROR_PARAM1,
		      shell_execute_cmd(shell_backend_uart_get_ptr(),
					"battfake -23"), NULL);
	...
	/* Test if fake charge level is applied */
	flags = BATT_FLAG_WANT_CHARGE | BATT_FLAG_RESPONSIVE;
	battery_get_params(&batt);
	zassert_equal(flags, batt.flags, "0x%x != 0x%x", flags, batt.flags);
	zassert_equal(fake_charge, batt.state_of_charge, "0x%x != 0x%x",
		      fake_charge, batt.state_of_charge);
	remaining_cap = bat->full_cap * fake_charge / 100;
	zassert_equal(remaining_cap, batt.remaining_capacity, "0x%x != 0x%x",
	...
	/* Test if fake charge level is not applied */
	flags = BATT_FLAG_WANT_CHARGE | BATT_FLAG_RESPONSIVE;
	battery_get_params(&batt);
	zassert_equal(flags, batt.flags, "0x%x != 0x%x", flags, batt.flags);
	charge = 100 * bat->cap / bat->full_cap;
	zassert_equal(charge, batt.state_of_charge, "0x%x != 0x%x",
		      charge, batt.state_of_charge);
	zassert_equal(bat->cap, batt.remaining_capacity, "0x%x != 0x%x",
...
static void test_battery_fake_temperature(void)
{
	...
	/* Fail on command with too high temperature */
	zassert_equal(EC_ERROR_PARAM1,
		      shell_execute_cmd(shell_backend_uart_get_ptr(),
					"batttempfake 5001"), NULL);

	/* Fail on command with too low temperature */
	zassert_equal(EC_ERROR_PARAM1,
		      shell_execute_cmd(shell_backend_uart_get_ptr(),
	...
	/* Test if fake temperature is applied */
	flags = BATT_FLAG_WANT_CHARGE | BATT_FLAG_RESPONSIVE;
	battery_get_params(&batt);
	zassert_equal(flags, batt.flags, "0x%x != 0x%x", flags, batt.flags);
	zassert_equal(fake_temp, batt.temperature, "0x%x != 0x%x",
	...
	battery_get_params(&batt);
	zassert_equal(flags, batt.flags, "0x%x != 0x%x", flags, batt.flags);
	zassert_equal(bat->temp, batt.temperature, "0x%x != 0x%x",
