...
static void test_tcpci_alert(void)
{
	...
	tcpci_tcpc_alert(USBC_PORT_C1);
	check_tcpci_reg(emul, 0x0, TCPC_REG_ALERT);
	check_tcpci_reg(emul, 0x0, TCPC_REG_FAULT_STATUS);
...
static void test_tcpci_drp_toggle(void)
{
	...
	exp_tcpc_ctrl = 0x9;
	tcpci_emul_set_reg(emul, TCPC_REG_TCPC_CTRL, exp_tcpc_ctrl);
	exp_tcpc_ctrl |= 0x40;
	zassert_equal(EC_SUCCESS, tcpci_tcpc_drp_toggle(USBC_PORT_C1), NULL);
	check_tcpci_reg(emul, exp_tcpc_ctrl, TCPC_REG_TCPC_CTRL);
	check_tcpci_reg(emul, exp_role_ctrl, TCPC_REG_ROLE_CTRL);
	check_tcpci_reg(emul, TCPC_REG_COMMAND_LOOK4CONNECTION,
			TCPC_REG_COMMAND);
...
static void test_tcpci_low_power_mode(void)
{
	...
	zassert_equal(EC_SUCCESS, tcpci_enter_low_power_mode(USBC_PORT_C1),
		      NULL);
	check_tcpci_reg(emul, TCPC_REG_COMMAND_I2CIDLE, TCPC_REG_COMMAND);
}
...
static void test_tcpci_set_bist_mode(void)
{
	...
	exp_mask = 0x7fff;
	tcpci_emul_set_reg(emul, TCPC_REG_ALERT_MASK, exp_mask);
...
static void test_tcpci_debug_accessory(void)
{
	...
	exp_val = 0x42;
	tcpci_emul_set_reg(emul, TCPC_REG_CONFIG_STD_OUTPUT, exp_val);

	/* Test discharge enable (disable bit 6) */
	exp_val &= ~0x40;
	tcpci_tcpc_debug_accessory(USBC_PORT_C1, 1);
	check_tcpci_reg(emul, exp_val, TCPC_REG_CONFIG_STD_OUTPUT);
...
static void test_tcpci_mux_init(void)
{
	...
	i2c_common_emul_set_read_fail_reg(i2c_emul,
					  I2C_COMMON_EMUL_FAIL_ALL_REG);
	set_usb_mux_tcpc();
	zassert_equal(EC_SUCCESS, tcpci_tcpm_mux_init(&usbc1_tcpci_usb_mux),
		      NULL);

	/* Test fail on power status read */
	set_usb_mux_not_tcpc();
	i2c_common_emul_set_read_fail_reg(i2c_emul, TCPC_REG_POWER_STATUS);
	zassert_equal(EC_ERROR_INVAL, tcpci_tcpm_mux_init(&usbc1_tcpci_usb_mux),
		      NULL);
	i2c_common_emul_set_read_fail_reg(i2c_emul,
					  I2C_COMMON_EMUL_NO_FAIL_REG);
	...
	zassert_equal(EC_ERROR_TIMEOUT,
		      tcpci_tcpm_mux_init(&usbc1_tcpci_usb_mux), NULL);
...
static void test_tcpci_mux_enter_low_power(void)
{
	...
	set_usb_mux_tcpc();
	zassert_equal(EC_SUCCESS,
		      tcpci_tcpm_mux_enter_low_power(&usbc1_tcpci_usb_mux),
		      NULL);
...
static void test_tcpci_mux_set_get(void)
{
	...
	zassert_equal(EC_ERROR_INVAL,
		      tcpci_tcpm_mux_set(&usbc1_tcpci_usb_mux, mux_state, &ack),
		      NULL);
	zassert_equal(EC_ERROR_INVAL,
		      tcpci_tcpm_mux_get(&usbc1_tcpci_usb_mux, &mux_state_get),
		      NULL);
	i2c_common_emul_set_read_fail_reg(i2c_emul,
					  I2C_COMMON_EMUL_NO_FAIL_REG);
	...
	tcpci_emul_set_reg(emul, TCPC_REG_CONFIG_STD_OUTPUT, 0xff);
...
void test_suite_tcpci(void)
{
	...
	ztest_test_suite(tcpci,
			 ...
			 ztest_unit_test_setup_teardown(test_tcpci_mux_set_get,
				set_usb_mux_tcpc, set_usb_mux_not_tcpc),
