...
static void integration_usb_attach_src_then_snk_before(void *state)
{
	...
	__ASSERT_NO_MSG(tcpc_config[0].drv->init(0) == EC_SUCCESS);
	/*
	 * Arbitrary FW ver. The emulator should really be setting this
	 * during its init.
	 */
	tcpci_emul_set_reg(tcpci_emul_snk, PS8XXX_REG_FW_REV, 0x31);
	__ASSERT_NO_MSG(tcpc_config[1].drv->init(1) == EC_SUCCESS);
	tcpci_emul_set_rev(tcpci_emul_src, TCPCI_EMUL_REV1_0_VER1_0);
	pd_set_suspend(0, 0);
	pd_set_suspend(1, 0);
	/* Reset to disconnected state. */
	__ASSERT_NO_MSG(tcpci_emul_disconnect_partner(tcpci_emul_src) ==
			EC_SUCCESS);
	__ASSERT_NO_MSG(tcpci_emul_disconnect_partner(tcpci_emul_snk) ==
			EC_SUCCESS);
	...
	__ASSERT_NO_MSG(tcpci_src_emul_connect_to_tcpci(
				&my_charger.data, &my_charger.common_data,
				&my_charger.ops, tcpci_emul_src) == EC_SUCCESS);
	...
	__ASSERT_NO_MSG(tcpci_snk_emul_connect_to_tcpci(
				&my_sink.data, &my_sink.common_data,
				&my_sink.ops, tcpci_emul_snk) == EC_SUCCESS);
...
static void integration_usb_attach_src_then_snk_after(void *state)
{
	...
	__ASSERT_NO_MSG(tcpci_emul_disconnect_partner(tcpci_generic_emul) ==
			EC_SUCCESS);
	__ASSERT_NO_MSG(tcpci_emul_disconnect_partner(tcpci_ps8xxx_emul) ==
			EC_SUCCESS);
