...
static void *integration_usb_src_snk__tcpci_rev1(void)
{
	...
	verify_src_port_typec_status)
{
	...
	/* The sink emulator is being attached to a source port (our policy
	 * engine) so it does not send any sink caps, so source port received
	no
	 * sink caps.
	 */
	zassert_equal(response.source_cap_count, 0,
...
ZTEST_SUITE(integration_usb_attach_src_then_snk__tcpci_rev1,
	    drivers_predicate_post_main, integration_usb_src_snk__tcpci_rev1,
