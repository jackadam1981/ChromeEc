...
struct usb_attach_5v_3a_pd_charger_fixture {
	struct tcpci_src_emul charger_5v;
	const struct emul *tcpci_emul;
	const struct emul *charger_emul;
};

static inline void
connect_charger_to_port(struct usb_attach_5v_3a_pd_charger_fixture *fixture)
{
	set_ac_enabled(true);
	zassume_ok(tcpci_src_emul_connect_to_tcpci(
			   &fixture->charger_5v.data,
			   &fixture->charger_5v.common_data,
			   &fixture->charger_5v.ops, fixture->tcpci_emul),
		   NULL);

	isl923x_emul_set_adc_vbus(
		fixture->charger_emul,
		PDO_FIXED_GET_VOLT(fixture->charger_5v.data.pdo[1]));
...
static inline void disconnect_charger_from_port(
	struct usb_attach_5v_3a_pd_charger_fixture *fixture)
{
...
static void *usb_attach_5v_3a_pd_charger_setup(void)
{
	static struct usb_attach_5v_3a_pd_charger_fixture test_fixture;
	...
	tcpci_src_emul_init(&test_fixture.charger_5v);
	test_fixture.charger_5v.data.pdo[1] =
		PDO_FIXED(5000, 3000, PDO_FIXED_UNCONSTRAINED);

	return &test_fixture;
}

static void usb_attach_5v_3a_pd_charger_before(void *data)
{
	connect_charger_to_port(
		(struct usb_attach_5v_3a_pd_charger_fixture *)data);
}

static void usb_attach_5v_3a_pd_charger_after(void *data)
{
	disconnect_charger_from_port(
		(struct usb_attach_5v_3a_pd_charger_fixture *)data);
}

ZTEST_SUITE(usb_attach_5v_3a_pd_charger, drivers_predicate_post_main,
	    usb_attach_5v_3a_pd_charger_setup,
	    usb_attach_5v_3a_pd_charger_before,
	    usb_attach_5v_3a_pd_charger_after, NULL);

ZTEST(usb_attach_5v_3a_pd_charger, test_battery_is_charging)
{
...
ZTEST(usb_attach_5v_3a_pd_charger, test_charge_state)
{
...
ZTEST(usb_attach_5v_3a_pd_charger, test_typec_status)
{
...
ZTEST(usb_attach_5v_3a_pd_charger, test_power_info)
{
...
ZTEST_F(usb_attach_5v_3a_pd_charger, test_disconnect_battery_not_charging)
{
	struct i2c_emul *i2c_emul = sbat_emul_get_ptr(BATTERY_ORD);
	uint16_t battery_status;
...
ZTEST_F(usb_attach_5v_3a_pd_charger, test_disconnect_charge_state)
{
...
ZTEST_F(usb_attach_5v_3a_pd_charger, test_disconnect_typec_status)
{
...
ZTEST_F(usb_attach_5v_3a_pd_charger, test_disconnect_power_info)
{
