...
ZTEST_F(usb_attach_5v_3a_pd_source, verify_alert_on_power_state_change)
{
	...
	k_sleep(K_SECONDS(2));
	zassume_true(fixture->src_ext.alert_received, NULL);
	zassume_true(fixture->src_ext.status_received, NULL);
	tcpci_src_emul_clear_alert_received(&fixture->src_ext);
	tcpci_src_emul_clear_status_received(&fixture->src_ext);
	zassume_false(fixture->src_ext.alert_received, NULL);
	zassume_false(fixture->src_ext.status_received, NULL);
...
ZTEST_F(usb_attach_5v_3a_pd_source, verify_inaction_on_invalid_pd_button_press)
{
	uint32_t ado;
...
ZTEST_F(usb_attach_5v_3a_pd_source, verify_startup_on_pd_button_press)
{
	uint32_t ado;
