...
ZTEST_F(usb_attach_5v_3a_pd_source_rev3, test_batt_cap)
{
	...
	tcpci_partner_common_send_get_battery_capabilities(&this->source_5v_3a,
							   battery_index);

	/* Allow some time for TCPC to process and respond */
	k_sleep(K_SECONDS(1));

	zassert_true(this->source_5v_3a.battery_capabilities
			     .have_response[battery_index],
		     "No battery capabilities response stored.");

	/* The response */
	struct pd_bcdb *bcdb =
		&this->source_5v_3a.battery_capabilities.bcdb[battery_index];
