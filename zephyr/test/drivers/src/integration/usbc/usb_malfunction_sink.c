...
connect_sink_to_port(struct usb_malfunction_sink_fixture *fixture)
{
	...
	tcpci_tcpc_alert(0);
	// TODO: Check, here we wait for DRPToggle and Look4Connection command.
	k_sleep(K_SECONDS(1));
...
static void *usb_malfunction_sink_setup(void)
{
	...
	// TODO: Check this
	tcpci_emul_set_rev(test_fixture.tcpci_emul, TCPCI_EMUL_REV1_0_VER1_0);
...
static void usb_malfunction_sink_after(void *data)
{
	...
	while (!k_fifo_is_empty(&fixture->sink.data.action_list)) {
		k_fifo_get(&fixture->sink.data.action_list, K_FOREVER);
	}
...
ZTEST_F(usb_malfunction_sink, test_fail_source_cap_and_pd_disable)
{
	...
	k_fifo_put(&this->sink.data.action_list, &this->actions[0]);
...
ZTEST_F(usb_malfunction_sink, test_ignore_source_cap)
{
	...
	tcpci_partner_common_clear_logged_msgs(&this->sink.common_data);
}
...
ZTEST_F(usb_malfunction_sink, test_ignore_source_cap_and_pd_disable)
{
	...
	k_fifo_put(&this->sink.data.action_list, &this->actions[1]);
