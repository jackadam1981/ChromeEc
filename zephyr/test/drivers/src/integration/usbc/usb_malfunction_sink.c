...
static void *usb_malfunction_sink_setup(void)
{
	...
	test_fixture.faulty_snk_ext.ext.ops = &tcpci_faulty_snk_emul_ops;
	test_fixture.faulty_snk_ext.ext.next = &test_fixture.snk_ext.ext;
	test_fixture.snk_ext.ext.ops = &tcpci_snk_emul_ops;
	test_fixture.snk_ext.ext.next = NULL;
	test_fixture.sink.extensions = &test_fixture.faulty_snk_ext.ext;
...
static void usb_malfunction_sink_before(void *data)
{
	...
	tcpci_partner_init(&test_fixture->sink);
...
static void usb_malfunction_sink_after(void *data)
{
	...
	while (!k_fifo_is_empty(&fixture->faulty_snk_ext.action_list)) {
		k_fifo_get(&fixture->faulty_snk_ext.action_list, K_FOREVER);
	}
...
ZTEST_F(usb_malfunction_sink, test_fail_source_cap_and_pd_disable)
{
	...
	k_fifo_put(&this->faulty_snk_ext.action_list, &this->actions[0]);
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&this->sink.msg_log, msg, node) {
		if (expect_hard_reset) {
			if (msg->sop != TCPCI_MSG_TX_HARD_RESET) {
				tcpci_partner_common_print_logged_msgs(
					&this->sink);
				zassert_unreachable(
			...
			msg_cnt++;
			expect_hard_reset = false;
			continue;
		}

		header = sys_get_le16(msg->buf);
		if (msg->sop != TCPCI_MSG_SOP || PD_HEADER_CNT(header) == 0 ||
		    PD_HEADER_TYPE(header) != PD_DATA_SOURCE_CAP) {
			tcpci_partner_common_print_logged_msgs(&this->sink);
			zassert_unreachable(
				"Expected message %d to be SourceCapabilities",
				msg_cnt);
		}

		msg_cnt++;
		expect_hard_reset = true;
	}

	tcpci_partner_common_clear_logged_msgs(&this->sink);
}
...
ZTEST_F(usb_malfunction_sink, test_ignore_source_cap_and_pd_disable)
{
	...
	k_fifo_put(&this->faulty_snk_ext.action_list, &this->actions[1]);
