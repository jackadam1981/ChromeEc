...
/* Zephyr virtual wires that have counterparts in the EC */
enum espi_vwire_signal VW_ZEPHYR_SIGNALS[] = {
...
/* EC virtual wires that have counterparts in the Zephyr */
enum espi_vw_signal VW_EC_SIGNALS[] = {
	VW_SLP_S3_L,
	VW_SLP_S4_L,
	VW_SLP_S5_L,
	VW_SUS_STAT_L,
...
static int vwire_received_cnt;
static struct espi_event vwire_received_evt;

static int sci_received_cnt;
static int sci_received_vals[8] = {0};
static int smi_received_cnt;
...
ZTEST_USER(espi_vwire, test_convert_all_valid_signals_to_zephyr_vwires)
{
	...
	/* Pre-set vwires (zephyr API) */
	for (a = 0; a < ARRAY_SIZE(VW_ZEPHYR_SIGNALS); a++) {
		res = espi_emul_host_set_vwire(
				ESPI_DEVICE, VW_ZEPHYR_SIGNALS[a], 1);
	}

	for (a = 0; a < ARRAY_SIZE(VW_EC_SIGNALS); a++) {
		res = espi_vw_set_wire(VW_EC_SIGNALS[a], 1);

		/* If it can't be set, it is probably input vwire */
		if (res) {
			res = (espi_vw_get_wire(VW_EC_SIGNALS[a]) != 1);
		}

		zassert_equal(res, 0, "should succeed");
	}
}
...
ZTEST_USER(espi_vwire, test_convert_invalid_vwire_signal)
{
	...
	res = espi_vw_set_wire(0xBAADF00D, 1);
	zassert_not_equal(res, 0, "should fail");
}
...
ZTEST_USER(espi_vwire, test_convert_all_valid_zephyr_vwires_to_signals)
{
	...
	/* TODO: at the moment this test just increases the branches coverage.
	 * There is no method to check if conversion is valid, so we
	 * are just checking if all switch-cases are covered in the report.
	 */
...
ZTEST_USER(espi_vwire, set_vwire)
{
	...
	espi_init_callback(&espi_cb_obj, espi_vwire_cb,
		ESPI_BUS_EVENT_VWIRE_RECEIVED);
	espi_emul_host_manage_callback(ESPI_DEVICE, &espi_cb_obj, 1);

	espi_vw_set_wire(VW_WAKE_L, 0);
	zassert_equal(vwire_received_cnt, 1, "count should be 1");
	zassert_equal(vwire_received_evt.evt_details, ESPI_VWIRE_SIGNAL_WAKE,
		"invalid signal");
	...
	zassert_equal(vwire_received_cnt, 3,
		"Invalid number of vwire received");

	espi_emul_host_manage_callback(ESPI_DEVICE, &espi_cb_obj, 0);
}
...
ZTEST_USER(espi_vwire, get_vwire)
{
	...
	espi_emul_host_set_vwire(ESPI_DEVICE, ESPI_VWIRE_SIGNAL_SLP_S5, 1);
	value = espi_vw_get_wire(VW_SLP_S5_L);
	zassert_equal(value, 1, "vwire value should be correct");

	espi_emul_host_set_vwire(ESPI_DEVICE, ESPI_VWIRE_SIGNAL_SLP_S5, 0);
	value = espi_vw_get_wire(VW_SLP_S5_L);
...
ZTEST_USER(espi_vwire, get_vwire_inv_signal)
{
	...
	value = espi_vw_get_wire(VW_SIGNAL_END);
...
/* TODO: Uncomment when this test can be run on x86 board */
/*ZTEST_USER(espi_vwire, get_vwire_interrupt)
{
	espi_vw_enable_wire_int(VW_PLTRST_L);

	espi_emul_host_set_vwire(ESPI_DEVICE, ESPI_VWIRE_SIGNAL_PLTRST, 0);
	int power_signals = power_get_signals();

	// Signals should change
	espi_emul_host_set_vwire(ESPI_DEVICE, ESPI_VWIRE_SIGNAL_PLTRST, 1);
	zassert_not_equal(power_get_signals(), power_signals,
		"Signals should change");
	espi_emul_host_set_vwire(ESPI_DEVICE, ESPI_VWIRE_SIGNAL_PLTRST, 0);
	zassert_equal(power_get_signals(), power_signals,
		"Signals should change");
...
{
	struct espi_evt_data_acpi *acpi;
	uint32_t value;

	acpi = (struct espi_evt_data_acpi *)&value;
...
ZTEST_USER(espi_vwire, acpi_need_sci_smi)
{
	...
	espi_init_callback(&espi_cb_obj, espi_smi_sci_cb,
		ESPI_BUS_EVENT_VWIRE_RECEIVED);
	espi_emul_host_manage_callback(ESPI_DEVICE, &espi_cb_obj, 1);
	...
	zassert_equal(sci_received_cnt, 0, "SCI shouldn't be triggered");
	zassert_equal(smi_received_cnt, 0, "SMI shouldn't be triggered");
	...
	zassert_equal(sci_received_cnt, 0, "SCI shouldn't be triggered");
	zassert_equal(smi_received_cnt, 3, "SMI should be triggered");
	...
	zassert_equal(sci_received_cnt, 3, "SCI should be triggered");
	zassert_equal(smi_received_cnt, 0, "SMI shouldn't be triggered");
