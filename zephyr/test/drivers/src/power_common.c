...
static void test_power_board_system_is_idle(void)
{
	...
	/* Setup remaining capacity to trigger stay-up */
	bat->cap = response.dzone.stayup - 5;
	zassert_equal(CRITICAL_SHUTDOWN_HIBERNATE,
		      board_system_is_idle(last_shutdown_time, &target, now),
		      NULL);

	/* Setup remaining capacity to be safe and not shutdown */
	bat->cap = response.dzone.stayup + 5;
	zassert_equal(CRITICAL_SHUTDOWN_IGNORE,
		      board_system_is_idle(last_shutdown_time, &target, now),
		      NULL);
 ...
 * battery is set in stay-up zone (which trigger hibernation), power state is
...
static void setup_hibernation_delay(void)
{
	...
	/* Setup smart discharge zone and set capacity to stay-up zone */
	args.params = &params;
	args.params_size = sizeof(params);
	params.drate.hibern = 100; /* uA */
	params.drate.cutoff = 10; /* uA */
	...
	zassert_equal(EC_RES_SUCCESS, host_command_process(&args), NULL);
	/*
	 * Make sure that battery is in stay-up zone in good condition to
	...
	/* Disable AC */
	zassert_ok(gpio_emul_input_set(acok_dev, GPIO_ACOK_OD_PORT, 0), NULL);
