			...
			const struct tcpci_emul_partner_ops *ops)
{
	...
	switch (my_state->state) {
		case 0:
			my_state->state = 1;
			/* Set both CC lines to Ra to indicate connect. */
			zassert_ok(tcpci_emul_get_reg(emul, TCPC_REG_CC_STATUS, &val), NULL);
			zassert_ok(tcpci_emul_set_reg(emul, TCPC_REG_CC_STATUS,
				TCPC_REG_CC_STATUS_SET(TCPC_REG_CC_STATUS_TERM(val), TYPEC_CC_VOLT_RA,
				TYPEC_CC_VOLT_RA)), NULL);
		break;
	default:
...
static void test_attach_compliant_charger(void)
{
	...
	tcpci_emul_connect_partner(tcpci_emul, (struct tcpci_emul_partner_ops *) &compliant_charger);
