...
static void test_deattach_on_msg_recv(void)
{
	const struct emul *tcpci_emul =
		emul_get_binding(DT_LABEL(TCPCI_EMUL_LABEL));
	struct tcpci_snk_emul_data snk1;
	struct tcpci_snk_emul_data snk2;
	...
	/* Attach snk1 */
	zassert_ok(tcpci_snk_emul_connect_to_tcpci(&snk1, tcpci_emul),
		   NULL);
	...
	/* Attach snk2 */
	zassert_ok(tcpci_snk_emul_connect_to_tcpci(&snk2, tcpci_emul),
		   NULL);
