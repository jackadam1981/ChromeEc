...
static void integration_usb_before(void *state)
{
	...
	/* Fails USB Mux tests */
	/* Arbitrary FW ver. The emulator should really be setting this during
	 * its init.
	 */
...
ZTEST(integration_usb, test_attach_src_then_snk)
{
	...
	/*
	 * TODO(b/209907615): Refactor to direct assert calls
	 */
	check_usb_pd_power_info(0, USB_PD_PORT_POWER_SINK, USB_CHG_TYPE_PD,
				5000, 3000);
	 ...
	 */
	check_usb_pd_power_info(0, USB_PD_PORT_POWER_SOURCE, USB_CHG_TYPE_PD,
				5000, 3000);
#endif /* !test_integration_src_snk */
}
