...
static void siracha_setup(void * data)
{
	const struct emul *ps8xxx_emul = emul_get_binding(PS8XXX_EMUL_LABEL);
	const struct emul *tcpci_emul_ps8xxx = ps8xxx_emul_get_tcpci(ps8xxx_emul);
	/* struct i2c_emul *p1_i2c_emul = */
	/* 	ps8xxx_emul_get_i2c_emul(ps8xxx_emul, PS8XXX_EMUL_PORT_1); */
	/* struct i2c_emul *tcpci_i2c_emul = tcpci_emul_get_i2c_emul(tcpci_emul_ps8xxx); */

	/* Set arbitrary FW reg value != 0 for this test */
	tcpci_emul_set_reg(tcpci_emul_ps8xxx, PS8XXX_REG_FW_REV, 0x31);
	/* /\* Set correct power status for this test *\/ */
	/* tcpci_emul_set_reg(tcpci_emul_ps8xxx, TCPC_REG_POWER_STATUS, 0x0); */
	...
	int ec = ps8xxx_tcpm_drv.init(USBC_PORT_C1);
	printf("starting here ec = 0x%x\n", ec);
	zassert_equal(EC_SUCCESS, ec, NULL);
}
...
/* 	 *\/ */
/* 	zassert_equal(PE_SRC_READY, get_state_pe(USBC_PORT_C1), NULL); */
/* } */
...
ZTEST(integration_usb, test_attach_drp)
{
	 ...
	 * TODO: Change it to examining EC_CMD_TYPEC_STATUS
	 */
	enum usb_pe_state state = get_state_pe(USBC_PORT_C1);
	printf("%x", state);
	zassert_equal(PE_SNK_READY, state, NULL);
}

/* ZTEST(integration_usb_ps8xxx, myinittest) */
/* { */
/* 	const struct emul *ps8xxx_emul = emul_get_binding(PS8XXX_EMUL_LABEL); */
/* 	const struct emul *tcpci_emul_ps8xxx = ps8xxx_emul_get_tcpci(ps8xxx_emul); */
/* 	struct i2c_emul *p1_i2c_emul = */
...
/* ZTEST_SUITE(integration_usb_ps8xxx, drivers_predicate_post_main, NULL, */
