...
{
	/* Test invalid signal */
	zassert_equal(-1, gpio_disable_interrupt(GPIO_COUNT), NULL);
	zassert_equal(-1, gpio_enable_interrupt(GPIO_COUNT), NULL);

	/* Test valid signal */
	zassert_ok(gpio_disable_interrupt(GPIO_TEST), NULL);
	zassert_ok(gpio_enable_interrupt(GPIO_TEST), NULL);
}
