...
ZTEST_USER_F(console_cmd_charge_state, test_idle_on_from_normal)
{
	...
	zassume_equal(CHARGE_CONTROL_NORMAL, get_chg_ctrl_mode(), NULL);

	/* Move to idle */
	zassert_ok(shell_execute_cmd(get_ec_shell(), "chgstate idle on"), NULL);
	zassert_equal(CHARGE_CONTROL_IDLE, get_chg_ctrl_mode(), NULL);
}
...
ZTEST_USER_F(console_cmd_charge_state, test_normal_from_idle)
{
	...
	zassume_equal(CHARGE_CONTROL_IDLE, get_chg_ctrl_mode(), NULL);

	/* Move back to normal */
	zassert_ok(shell_execute_cmd(get_ec_shell(), "chgstate idle off"),
		   NULL);
	zassert_equal(CHARGE_CONTROL_NORMAL, get_chg_ctrl_mode(), NULL);
}
