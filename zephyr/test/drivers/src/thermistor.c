...
#define NAMED_TEMP_SENSORS_SIZE                       \
	DT_FOREACH_CHILD(DT_PATH(named_temp_sensors), \
			 _NAMED_TEMP_SENSOR_COUNTER)  \
	0

#define _ENABLED_TEMP_SENSOR_COUNTER(x) 1 +
#define TEMP_SENSORS_ENABLED_SIZE \
	DT_FOREACH_STATUS_OKAY(cros_temp_sensor, _ENABLED_TEMP_SENSOR_COUNTER) 0
...
#define MOCK_VALID_ADC_VALUE(node_id)                                        \
	zassert_ok(                                                          \
		adc_emul_const_value_set(                                    \
			adc_dev, DT_PROP(DT_PHANDLE(node_id, adc), channel), \
			A_VALID_VOLTAGE),                                    \
		"adc_emul_const_value_set() failed on %s", DT_LABEL(node_id));

	DT_FOREACH_CHILD(DT_PATH(named_temp_sensors), MOCK_VALID_ADC_VALUE)
	...
	for (sensor_idx = 0;
	     sensor_idx < NAMED_TEMP_SENSORS_SIZE;
	     sensor_idx++) {

		const struct temp_sensor_t *sensor = &temp_sensors[sensor_idx];

		zassert_equal(
			EC_ERROR_NOT_POWERED, sensor->read(sensor, &temp),
			"%s failed",
			sensor->name);
	}
...
#define MOCK_READ_ERROR(node_id)                                             \
	zassert_ok(                                                          \
		adc_emul_value_func_set(                                     \
			adc_dev, DT_PROP(DT_PHANDLE(node_id, adc), channel), \
			adc_error_func, NULL),                               \
		"adc_emul_value_func_set() failed on %s", DT_LABEL(node_id));

	DT_FOREACH_CHILD(DT_PATH(named_temp_sensors), MOCK_READ_ERROR)
