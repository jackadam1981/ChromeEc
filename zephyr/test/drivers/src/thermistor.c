...
#define ASSERT_READ_ERROR(node_id)				\
	...
	for (sensor_idx = 0; sensor_idx < NAMED_TEMP_SENSORS_SIZE; sensor_idx++) {
		const struct temp_sensor_t *sensor = & temp_sensors[sensor_idx];
		printf("sensor read value is %d", sensor->read(sensor, &temp));
		printf("ec success read value is %d", EC_SUCCESS);
		printf("ec error value is %d", EC_ERROR_UNKNOWN);
		printf("return value is %d", temp);
