...
static void set_ac(bool connected)
{
	...
	__ASSERT(extpower_is_present() != connected,
		 "AC change was requested, but it's already in that state");

	__ASSERT(!gpio_emul_input_set(acok_dev, GPIO_ACOK_OD_PORT, connected),
		 "Failed to set AC GPIO");
...
static void set_lid(bool open, bool inhibit_boot)
{
	...
	__ASSERT(!gpio_emul_input_set(lid_switch_dev, GPIO_LID_OPEN_EC_PORT,
				      open),
...
static void reset_state(void *unused)
{
	...
	__ASSERT(!shell_execute_cmd(get_ec_shell(), "chargesplash reset"),
