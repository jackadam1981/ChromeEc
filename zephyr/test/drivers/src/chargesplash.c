		     ...
		     struct ec_response_chargesplash *response)
{
	...
	LOG_INF("response: requested=0x%X, tries=0x%X, max_tries=0x%X",
		response->requested, response->tries, response->max_tries);
...
static void wait_for_chipset_startup(void)
{
	...
	/* Give the EC a bit to "calm down" after reaching S0 (b/230362548) */
	msleep(2000);
}
...
ZTEST_USER(chargesplash, test_manual_lockout_via_console)
{
	zassert_ok(shell_execute_cmd(get_ec_shell(), "chargesplash lockout"),
		   NULL);
	zassert_ok(shell_execute_cmd(get_ec_shell(), "chargesplash boot"),
		   NULL);
	zassert_false(is_chargesplash_requested(),
...
/* Cancel a request via the shell */
ZTEST_USER(chargesplash, test_cancel_via_console)
{
	zassert_ok(shell_execute_cmd(get_ec_shell(), "chargesplash boot"),
		   NULL);
	zassert_true(is_chargesplash_requested(), NULL);

	zassert_ok(shell_execute_cmd(get_ec_shell(), "chargesplash cancel"),
		   NULL);
	zassert_false(is_chargesplash_requested(),
		      "after a cancel, chargesplash should not be requested");
	wait_for_chipset_startup();
}

/* Manually request via host command */
ZTEST_USER(chargesplash, test_cancel_via_hostcmd)
{
	struct ec_response_chargesplash response;
...
ZTEST_USER(chargesplash, test_display_loop)
{
	...
	zassert_ok(chargesplash_hostcmd(EC_CHARGESPLASH_INIT_DISPLAY,
					&response),
		   NULL);
