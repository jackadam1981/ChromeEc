...
static void test_isl923x_is_acok(void)
{
	...
	/* Part 3: successful path */
	raa489000_emul_set_acok_pin(isl923x_emul, 1);

	rv = raa489000_is_acok(CHARGER_NUM, &acok);
	zassert_equal(EC_SUCCESS, rv, "Expected %d but got %d", EC_SUCCESS, rv);
...
static void test_isl923x_hibernate__happy_path(void)
{
	...
	uint16_t expected =
		isl923x_emul_peek_reg(i2c_emul, ISL923X_REG_CONTROL0);

	expected &= ~(RAA489000_C0_EN_CHG_PUMPS_TO_100PCT |
		      RAA489000_C0_BGATE_FORCE_ON);

	MOCK_ASSERT_I2C_READ(hibernate_mock_read_fn, 0, ISL923X_REG_CONTROL0);
	MOCK_ASSERT_I2C_READ(hibernate_mock_read_fn, 1, ISL923X_REG_CONTROL0);
	...
	expected = isl923x_emul_peek_reg(i2c_emul, ISL923X_REG_CONTROL1);
	expected &= ~(RAA489000_C1_ENABLE_SUPP_SUPPORT_MODE |
		      ISL923X_C1_ENABLE_PSYS);
	expected |= RAA489000_C1_BGATE_FORCE_OFF | ISL923X_C1_DISABLE_MON;

	MOCK_ASSERT_I2C_READ(hibernate_mock_read_fn, 2, ISL923X_REG_CONTROL1);
	MOCK_ASSERT_I2C_READ(hibernate_mock_read_fn, 3, ISL923X_REG_CONTROL1);
	...
	expected = isl923x_emul_peek_reg(i2c_emul, ISL9238_REG_CONTROL4);
	expected |= RAA489000_C4_DISABLE_GP_CMP;

	MOCK_ASSERT_I2C_READ(hibernate_mock_read_fn, 6, ISL9238_REG_CONTROL4);
	MOCK_ASSERT_I2C_READ(hibernate_mock_read_fn, 7, ISL9238_REG_CONTROL4);
	MOCK_ASSERT_I2C_WRITE(hibernate_mock_write_fn, 6, ISL9238_REG_CONTROL4,
			      expected & 0xff);
	MOCK_ASSERT_I2C_WRITE(hibernate_mock_write_fn, 7, ISL9238_REG_CONTROL4,
			      expected >> 8);
}
