...
ZTEST(isl923x, test_isl923x_set_current)
{
	...
	i2c_common_emul_set_read_fail_reg(isl923x_emul,
					  ISL923X_REG_CHG_CURRENT);
...
ZTEST(isl923x, test_isl923x_set_input_current_limit)
{
	...
	i2c_common_emul_set_write_fail_reg(isl923x_emul,
...
ZTEST(isl923x, test_device_id)
{
	...
	/* Test read error */
	i2c_common_emul_set_read_fail_reg(isl923x_emul, ISL923X_REG_DEVICE_ID);
...
ZTEST(isl923x, test_options)
{
	...
	i2c_common_emul_set_read_fail_reg(isl923x_emul, ISL923X_REG_CONTROL0);
	zassert_equal(EC_ERROR_INVAL,
		      isl923x_drv.get_option(CHARGER_NUM, &option), NULL);

	/* Test failed control 1 read */
	i2c_common_emul_set_read_fail_reg(isl923x_emul, ISL923X_REG_CONTROL1);
	zassert_equal(EC_ERROR_INVAL,
		      isl923x_drv.get_option(CHARGER_NUM, &option), NULL);
	...
	i2c_common_emul_set_write_fail_reg(isl923x_emul, ISL923X_REG_CONTROL0);
	zassert_equal(EC_ERROR_INVAL,
		      isl923x_drv.set_option(CHARGER_NUM, option), NULL);

	/* Test failed control 1 write */
	i2c_common_emul_set_write_fail_reg(isl923x_emul, ISL923X_REG_CONTROL1);
	zassert_equal(EC_ERROR_INVAL,
		      isl923x_drv.set_option(CHARGER_NUM, option), NULL);
...
ZTEST(isl923x, test_comparator_inversion)
{
	...
	i2c_common_emul_set_read_fail_reg(isl923x_emul, ISL923X_REG_CONTROL2);
	i2c_common_emul_set_write_func(isl923x_emul, mock_write_fn_always_fail,
				       NULL);
	zassert_equal(EC_ERROR_INVAL,
	...
	i2c_common_emul_set_read_fail_reg(isl923x_emul,
					  I2C_COMMON_EMUL_NO_FAIL_REG);
	i2c_common_emul_set_write_func(isl923x_emul, NULL, NULL);
	...
	i2c_common_emul_set_write_fail_reg(isl923x_emul, ISL923X_REG_CONTROL2);
	zassert_equal(EC_ERROR_INVAL,
...
ZTEST(isl923x, test_get_vbus_voltage)
{
	...
	i2c_common_emul_set_read_fail_reg(isl923x_emul, RAA489000_REG_ADC_VBUS);
	zassert_equal(EC_ERROR_INVAL,
...
ZTEST(isl923x, test_init)
{
	...
	isl923x_emul_reset_registers(isl923x_emul);
	i2c_common_emul_set_read_fail_reg(isl923x_emul, ISL9238_REG_CONTROL3);
	isl923x_drv.init(CHARGER_NUM);
	i2c_common_emul_set_read_fail_reg(isl923x_emul,
					  I2C_COMMON_EMUL_NO_FAIL_REG);
	zassert_ok(isl923x_drv.get_input_current_limit(CHARGER_NUM,
	...
	isl923x_emul_reset_registers(isl923x_emul);
	i2c_common_emul_set_write_fail_reg(isl923x_emul, ISL9238_REG_CONTROL3);
	isl923x_drv.init(CHARGER_NUM);
	i2c_common_emul_set_write_fail_reg(isl923x_emul,
					   I2C_COMMON_EMUL_NO_FAIL_REG);
	zassert_ok(isl923x_drv.get_input_current_limit(CHARGER_NUM,
...
ZTEST(isl923x, test_isl923x_is_acok)
{
	...
	i2c_common_emul_set_read_fail_reg(isl923x_emul, ISL9238_REG_INFO2);
...
static void isl923x_hibernate_before(void *state)
{
	...
	i2c_common_emul_set_read_func(isl923x_emul, hibernate_mock_read_fn,
				      NULL);
	i2c_common_emul_set_write_func(isl923x_emul, hibernate_mock_write_fn,
				       NULL);
...
static void isl923x_hibernate_after(void *state)
{
	...
	i2c_common_emul_set_read_func(isl923x_emul, NULL, NULL);
	i2c_common_emul_set_write_func(isl923x_emul, NULL, NULL);
...
ZTEST(isl923x_hibernate, test_isl923x_hibernate__fail_at_ISL9238_REG_CONTROL4)
{
	...
	i2c_common_emul_set_read_fail_reg(isl923x_emul, ISL9238_REG_CONTROL4);
...
ZTEST(isl923x_hibernate, test_isl9238c_hibernate)
{
	...
	for (int i = 0; i < ARRAY_SIZE(registers); i++) {
		i2c_common_emul_set_read_fail_reg(isl923x_emul, registers[i]);
