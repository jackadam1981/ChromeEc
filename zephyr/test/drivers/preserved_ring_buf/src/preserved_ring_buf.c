			    ...
			    const int word_size)
{
	...
	zassert_equal(buf->preserved->word_size, word_size);
	zassert_equal(buf->constants.capacity, capacity);
	zassert_equal(buf->preserved->capacity, capacity);
	...
	for (int i = 0; i < capacity / 2; i++) {
		...
		// printk("%d = %d = %d, mask = %x\n", i, i & word_mask,
		// preserved_ring_buf_read(buf, i), WORD_MASK(word_size));
...
ZTEST_USER(preserved_ring_buf, test_corrupt)
{
	...
	preserved_ring_buf_reset(buf);
	zassert_true(preserved_ring_buf_verify(buf));
	buf->preserved->checksum = -1;
	zassert_false(preserved_ring_buf_verify(buf));

	preserved_ring_buf_reset(buf);
	zassert_true(preserved_ring_buf_verify(buf));
	buf->preserved->capacity -= 1;
	zassert_false(preserved_ring_buf_verify(buf));

	preserved_ring_buf_reset(buf);
	zassert_true(preserved_ring_buf_verify(buf));
	buf->preserved->version -= 1;
	zassert_false(preserved_ring_buf_verify(buf));

	preserved_ring_buf_reset(buf);
	zassert_true(preserved_ring_buf_verify(buf));
	buf->preserved->word_size -= 1;
	zassert_false(preserved_ring_buf_verify(buf));
}
...
ZTEST_USER(preserved_ring_buf, test_reset)
{
	...
	DECLARE_PRESERVED_RING_BUF(uint16_t, buf, capacity, 1);
	preserved_ring_buf_reset(buf);
...
ZTEST_USER(preserved_ring_buf, test_head_wrap)
{
	...
	buf->preserved->head = UINT32_MAX - capacity + 1;
...
ZTEST_USER(preserved_ring_buf, test_capacity)
{
	...
	DECLARE_PRESERVED_RING_BUF(uint16_t, buf, capacity, 1);
	preserved_ring_buf_reset(buf);
	zassert_equal(preserved_ring_buf_capacity(buf), capacity);
}
