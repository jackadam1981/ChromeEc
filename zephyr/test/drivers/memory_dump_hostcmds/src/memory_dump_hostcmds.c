...
enum ec_status send_host_command(int command, int version, const void *params,
				 int params_size, void *resp, int resp_max,
				 int *resp_size)
{
	int rv;
...
enum ec_status fetch_memory_dump(struct mem_dump *dump)
{
	...
	zassert_ok(send_host_command(EC_CMD_GET_MEMORY_DUMP_METADATA, 0, NULL,
				     0, &metadata_response,
				     sizeof(protocol_info_response), NULL));
	...
	for (int seg = 0; seg < metadata_response.memory_dump_entry_count;
		...
		while (offset < entry_info_response.size) {
			...
			zassert_ok(send_host_command(
				EC_CMD_READ_MEMORY_DUMP, 0, &read_mem_params,
				sizeof(read_mem_params), read_mem_response,
