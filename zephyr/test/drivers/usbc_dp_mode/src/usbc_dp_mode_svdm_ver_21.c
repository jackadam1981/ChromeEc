...
ZTEST_F(usbc_dp_mode_svdm_ver_21, test_discovery_send_config)
{
	...
	vdm_payload[0] = VDO(USB_SID_DISPLAYPORT, 1,
			     CMD_ATTENTION | VDO_OPOS(1)); /* OPos 1 for DP Status */
	vdm_payload[1] = VDO_DP_STATUS(1, /* hpd_state: HPD High */
				       1, /* irq_hpd: IRQ asserted */
				       0, /* mf_pref: Multi-function preferred: No */
				       0, /* ufp_d_connected: No DFP_D or UFP_D connected */
				       0, /* ufp_c_connected: No DFP_C or UFP_C connected */
				       1, /* dp_gen_cap: DP General Capabilities (e.g., Gen1) */
	...
	status = host_cmd_typec_status(TEST_PORT);
	zassert_equal((status.mux_state & USB_MUX_CHECK_MASK), mux_state,
	...
	zassert_equal(tcpci_partner_send_data_msg(&fixture->partner,
						  PD_DATA_VENDOR_DEF,
						  vdm_payload, 2, 50),
		      NULL);
	k_sleep(K_MSEC(200));
