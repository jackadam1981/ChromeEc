...
ZTEST_F(usbc_dp_mode_svdm_ver_21, test_discovery_send_config)
{
	 ...
	 * DP <4>    : Multi-function preference : 0 == no pref, 1 == MF
	 * preferred. <3>    : enabled : is DPout on/off. <2>    : power low : 0
	 * == normal or LPM disabled, 1 == DP disabled for LPM <1:0>  : connect
	 * status : 00b ==  no (DFP|UFP)_D is connected or disabled. 01b ==
	 * DFP_D connected, 10b == UFP_D connected, 11b == both.
	 */
	...
	assert_mux_state_is(TEST_PORT, , "Failed to see DP set");

	/* IRQ: high, power low: DP disabled for LPM */
	...
	/* Verify mux state changed to IRQ_HPD high */
	assert_mux_state_is(TEST_PORT,
			    USB_MUX_USB_DP_ENABLED | USB_PD_MUX_HPD_IRQ,
			    "Failed to see HPD set");

	/* IRQ: high, HPD: high, UFP_D: connected*/
	send_dp_attention_vdm(&fixture->partner,
			      VDO_DP_STATUS(1, 1, 0, 0, 0, 1, 0, 2));

	assert_mux_state_is(TEST_PORT,
			    USB_MUX_USB_DP_ENABLED | USB_MUX_HPD_MASK,
