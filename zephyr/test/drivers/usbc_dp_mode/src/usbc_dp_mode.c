...
static struct tcpci_cable_data non_emark = {
...
/* Passive cable with USB2 support only when used set Ptype*/
...
/* Active cable base config when used add VDO 1, VDO 2 and svdm type */
...
/* no modal cable don't care about SOP' responses */
static struct tcpci_cable_data no_modal_cable = {
	.identity_vdm[VDO_INDEX_HDR] =
		VDO(USB_SID_PD, /* structured VDM */ true,
		    VDO_CMDT(CMDT_RSP_ACK) | CMD_DISCOVER_IDENT) |
		VDO_SVDM_VERS_MAJOR(SVDM_VER_2_1) | VDM_VERS_MINOR,
...
static void setup_active_tbt_base_cable(struct tcpci_partner_data *partner)
{
	...
	active_cable.identity_vdm[VDO_INDEX_PTYPE_CABLE1] =
		optical_redriver.raw_value;
	active_cable.identity_vdm[VDO_INDEX_PTYPE_CABLE2] =
		optical_redriver_vdo2.raw_value;

	active_cable.modes_vdm[VDO_INDEX_HDR] =
		VDO(USB_VID_INTEL, /* structured VDM */ true,
		    VDO_CMDT(CMDT_RSP_ACK) | CMD_DISCOVER_MODES) |
		VDO_SVDM_VERS_MAJOR(SVDM_VER_2_1) | VDM_VERS_MINOR;

	active_cable.modes_vdm[VDO_INDEX_HDR + 1] = cable_resp.raw_value;
	active_cable.modes_vdos = VDO_INDEX_HDR + 2;

	/* Set svdm to USB_VID_INTEL */
	active_cable.svids_vdm[VDO_INDEX_HDR + 1] = VDO_SVID(USB_VID_INTEL, 0),
...
static void setup_active_tbt_no_modal_cable(struct tcpci_partner_data *partner)
{
	...
	no_modal_cable.identity_vdm[VDO_INDEX_PTYPE_CABLE1] =
		optical_redriver.raw_value;
	no_modal_cable.identity_vdm[VDO_INDEX_PTYPE_CABLE2] =
		optical_redriver_vdo2.raw_value;

	add_dp_21_discovery(partner);
	partner->cable = &no_modal_cable;
	add_displayport_mode_responses(partner);
}
...
static void setup_active_dp_base_cable(struct tcpci_partner_data *partner)
{
	...
	active_cable.svids_vdm[VDO_INDEX_HDR + 1] =
		VDO_SVID(USB_SID_DISPLAYPORT, 0),
...
static void setup_non_emark(struct tcpci_partner_data *partner)
{
	/* Set up the partner as DP-capable with a passive cable */
	add_dp_21_discovery(partner);
	partner->cable = &non_emark;
	add_displayport_mode_responses(partner);
}
...
ZTEST_F(usbc_dp_mode, test_dp21_no_sop_prime)
{
	union tbt_mode_resp_cable cable_resp;
	struct ec_response_typec_status status;
...
ZTEST_F(usbc_dp_mode, test_dp21_entry_no_modal)
{
	union tbt_mode_resp_cable cable_resp;
	struct ec_response_typec_status status;
...
ZTEST_F(usbc_dp_mode, test_dp21_non_emark)
{
	...
	setup_non_emark(&fixture->partner);
	connect_sink_to_port(&fixture->partner, fixture->tcpci_emul,
			     fixture->charger_emul);
