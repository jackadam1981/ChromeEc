...
ZTEST_F(usbc_alt_mode, verify_displayport_mode_entry)
{
	...
	k_sleep(K_SECONDS(1));
	/* Verify the board's HPD notificaiton triggered */
	zassert_equal((usb_mux_get(TEST_PORT) & USB_PD_MUX_HPD_LVL),
		      USB_PD_MUX_HPD_LVL, "Failed to set HPD level in mux");
	zassert_equal((usb_mux_get(TEST_PORT) & USB_PD_MUX_HPD_IRQ),
