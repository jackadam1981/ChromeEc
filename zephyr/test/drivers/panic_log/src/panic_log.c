...
/* TODO: printk is only hooked in log mode minimal */
...
/* Verify panic log is discared if panic log is invalid */
ZTEST_USER(panic_log, test_init_old_panic_invalid)
{
	...
	/* Panic log should be reset and unfrozen during init, so string will be
	 * missing */
	zassert_false(panic_log_frozen);
	freeze_panic_log();
	zassert_false(panic_log_contains(test_str));
...
ZTEST_USER(panic_log, test_info_host_cmd)
{
...
ZTEST_USER(panic_log, test_info_host_cmd_reset)
{
	const char *test_str = __func__;
	const struct ec_params_panic_log_info params = {
		.freeze = 0,
...
ZTEST_USER(panic_log, test_info_host_cmd_freeze)
{
	const char *test_str = __func__;
...
ZTEST_USER(panic_log, test_info_host_cmd_unfreeze)
{
	const char *test_str = __func__;
	struct ec_params_panic_log_info params = {
		.freeze = 0,
...
ZTEST_USER(panic_log, test_info_host_cmd_invalid)
{
	struct ec_params_panic_log_info params = {
		.freeze = 1,
		.unfreeze = 1,
