...
ZTEST_USER(host_cmd_motion_sense, test_odr_set_suspend)
{
	...
	zassert_ok(motion_sensors[9].drv->set_data_rate(&motion_sensors[9], 0,
							false),
		   NULL);
	zassert_ok(host_cmd_motion_sense_odr(/*sensor_num=*/9,
					     /*odr=*/10000,
					     /*round_up=*/true, &response),
		   NULL);
	...
	zassert_equal(10000 | ROUND_UP_FLAG,
		      motion_sensors[9].config[SENSOR_CONFIG_AP].odr,
	...
	k_sleep(K_SECONDS(2));
	zassert_equal(0,
		      motion_sensors[9].drv->get_data_rate(&motion_sensors[9]),
		      "%s expected %d, but got %d", 0, motion_sensors[9].name,
		      motion_sensors[9].drv->get_data_rate(&motion_sensors[9]));
	k_sleep(K_SECONDS(2));
