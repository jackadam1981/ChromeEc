					 ...
					 const char *expected)
{
	...
	zassert_equal('[', output_ptr[0],
		      "Missing expected openinging bracket is output: %s",
		      output);
	output_ptr += 1;
	output_ptr = strstr(output_ptr, HOST_MSG_PREFIX);
	zassert_not_null(output_ptr,
			 "Missing expected host message prefix in output: %s",
			 output);
	output_ptr += strlen(HOST_MSG_PREFIX);
	output_ptr = strstr(output_ptr, expected);
	zassert_not_null(output_ptr, "Missing expected message in output: %s",
			 output);
	output_ptr += strlen(expected);
	zassert_equal(']', output_ptr[0],
		      "Missing expected closing bracket in output: %s", output);
}
...
ZTEST_USER(host_cmd_console_print, test_long_message)
{
	...
	args = BUILD_HOST_COMMAND_PARAMS(EC_CMD_CONSOLE_PRINT, 0, msg);
