...
ZTEST_USER(host_cmd_usb_pd_control, test_good_index_no_partner)
{
	...
	zassert_equal(response.enabled, 0, "Failed to nothing enabled: 0x%02x",
		      response.enabled);
...
static void host_cmd_usb_pd_control_begin(void *data)
{
	ARG_UNUSED(data);
...
ZTEST_SUITE(host_cmd_usb_pd_control, drivers_predicate_post_main,
	    host_cmd_usb_pd_control_setup, host_cmd_usb_pd_control_begin, NULL,
	    NULL);
