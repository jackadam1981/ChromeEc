...
ZTEST(tcpc_shim, test_tcpc_alts_exist)
{
	...
	zassert_not_null(&TCPC_ALT_FROM_NODELABEL(tcpc_rt171x_alt));
...
ZTEST(tcpc_shim, test_tcpc_alt_enable)
{
	...
	TCPC_ENABLE_ALTERNATE_BY_NODELABEL(1, tcpc_rt171x_alt);
	...
	zassert_mem_equal(&tcpc_config[1],
			  &TCPC_ALT_FROM_NODELABEL(tcpc_rt171x_alt),
			  sizeof(struct tcpc_config_t));

	TCPC_ENABLE_ALTERNATE_BY_NODELABEL(0, tcpc_raa489000_alt);
	zassert_mem_equal(&tcpc_config[0],
			  &TCPC_ALT_FROM_NODELABEL(tcpc_raa489000_alt),
			  sizeof(struct tcpc_config_t));
}
