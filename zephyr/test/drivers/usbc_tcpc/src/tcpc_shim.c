...
{
	zassert_not_null(&TCPC_ALT_FROM_NODELABEL(tcpc_rt171x_0_alt));
	zassert_not_null(&TCPC_ALT_FROM_NODELABEL(tcpc_rt171x_1_alt));
}

ZTEST(tcpc_shim, test_tcpc_alt_enable)
{
	/* Enable an alternate TCPC on each USB-C port */
	TCPC_ENABLE_ALTERNATE_BY_NODELABEL(0, tcpc_rt171x_0_alt);
	TCPC_ENABLE_ALTERNATE_BY_NODELABEL(1, tcpc_rt171x_1_alt);

	zassert_mem_equal(&tcpc_config[0],
			  &TCPC_ALT_FROM_NODELABEL(tcpc_rt171x_0_alt),
			  sizeof(struct tcpc_config_t));
	zassert_mem_equal(&tcpc_config[1],
			  &TCPC_ALT_FROM_NODELABEL(tcpc_rt171x_1_alt),
			  sizeof(struct tcpc_config_t));

	TCPC_ENABLE_ALTERNATE_BY_NODELABEL(0, tcpc_rt171x_1_alt);
	zassert_mem_equal(&tcpc_config[0],
			  &TCPC_ALT_FROM_NODELABEL(tcpc_rt171x_1_alt),
			  sizeof(struct tcpc_config_t));
}
