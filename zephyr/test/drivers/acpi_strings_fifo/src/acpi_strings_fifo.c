...
static void setup_battery_static()
{
...
ZTEST_USER(acpi_battery, test_read_manufacturer)
{
	 ...
	 */
	for (int i = 0; i < sizeof(battery_static[0].manufacturer_ext); i++) {
		zassert_not_equal(battery_static[0].manufacturer_ext[i], 0,
				  "found nonzero byte in buffer at index %d",
				  i);
	}
	assert_reads_string(EC_ACPI_MEM_STRINGS_FIFO_ID_BATTERY_MANUFACTURER,
			    "Honest Eve's Very Safe Batteries");
}
