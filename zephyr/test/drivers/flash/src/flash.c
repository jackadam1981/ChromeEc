...
ZTEST_USER(flash, test_console_cmd_flash_write__bad_args)
{
	/* No args*/
	zassert_equal(EC_ERROR_PARAM_COUNT,
		      shell_execute_cmd(get_ec_shell(), "flashwrite"));

	/* Check for 1 of 2 required args */
	zassert_equal(EC_ERROR_PARAM_COUNT,
		      shell_execute_cmd(get_ec_shell(), "flashwrite 0x1000"));

	/* Check for alpha arg instead of number*/
	zassert_equal(EC_ERROR_PARAM1,
		      shell_execute_cmd(get_ec_shell(), "flashwrite xyz 100"));
	zassert_equal(EC_ERROR_PARAM2,
		      shell_execute_cmd(get_ec_shell(), "flashwrite 100 xyz"));
}

ZTEST_USER(flash, test_console_cmd_flash_write__too_big)
{
	zassert_ok(!shell_execute_cmd(
		get_ec_shell(), "flashwrite 0x10000 " STRINGIFY(INT_MAX)));
}
...
ZTEST_USER(flash, test_console_cmd_flash_write__happy)
{
	 ...
	 * pattern 03 02 01 00.
	 */
	zassert_ok(shell_execute_cmd(get_ec_shell(), "flashwrite 0x10000 4"));

	uint32_t output;

	/* Check for pattern */
	zassert_ok(read_flash_helper32(0x10000, &output));
	zassert_equal(output, 0x03020100, "Got %08x", output);
