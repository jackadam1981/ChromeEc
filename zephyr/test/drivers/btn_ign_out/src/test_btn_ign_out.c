...
ZTEST_SUITE(test_btn_ign_out, NULL, NULL, test_btn_ign_init, NULL, NULL);

ZTEST(test_btn_ign_out, test_activate)
{
	btn_ign_activate();
...
ZTEST(test_btn_ign_out, test_deactivate)
{
	btn_ign_activate();
	k_sleep(K_MSEC(100));
	btn_ign_deactivate();
...
ZTEST(test_btn_ign_out, test_activate_deactivate_consecutive_calls)
{
	btn_ign_activate();
	zassert_equal(btn_ign_get(), 1,
...
ZTEST(test_btn_ign_out, test_deactivate_multiple_calls)
{
	btn_ign_activate();
