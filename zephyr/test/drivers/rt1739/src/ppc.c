...
static struct set_reg_entry_t *get_next_reg_set_entry(struct _snode **iter_node,
						      uint8_t target_reg)
{
...
{
	struct set_reg_entry_t *entry = NULL;
...
ZTEST(rt1739_ppc, test_init_with_dead_battery)
{
	...
	struct set_reg_entry_t *set_sys_ctrl = NULL;
	struct set_reg_entry_t *set_vbus_switch_ctrl = NULL;
...
ZTEST(rt1739_ppc, test_es2_specific_init)
{
	uint8_t val;
	struct _snode *iter_node = NULL;
	struct set_reg_entry_t *hidden_mode_F1 = NULL;
	struct set_reg_entry_t *vbus_switch_ctrl = NULL;
