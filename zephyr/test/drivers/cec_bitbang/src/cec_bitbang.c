...
/*
 * 1 timer tick is 100 us, and the shortest delays we use are ~50 us. So all
 * times are multiplied by 10. E.g. when the driver requests a 600 us delay, we
 * actually sleep for 6000 us. And when reporting the elapsed time to the
 * driver, we dived by 10.
 */
...
void cec_tmr_cap_start(int port, enum cec_cap_edge edge, int timeout)
{
	...
	if (timeout > 0) {
		start_time = get_time();
		k_timer_start(&timer, K_USEC(timeout * TIME_SCALE), K_NO_WAIT);
	}
	...
	if (mock_ack && (state == CEC_STATE_INITIATOR_ACK_LOW ||
		...
		k_timer_start(&ack_timer,
			      K_USEC(CEC_DATA_ZERO_LOW_US * TIME_SCALE),
			      K_NO_WAIT);
	}
}
...
{
	return (get_time().val - start_time.val) / TIME_SCALE;
}
			      ...
			      gpio_port_pins_t pins)
{
	...
	if (gpio_index > 0)
		gpio_recordings[gpio_index - 1].duration_us =
			(get_time().val - previous_time.val) / TIME_SCALE;
			 ...
			 bool should_ack)
{
	...
	for (int bit = 7; bit >= 0; bit--) {
		if (msg[byte] & BIT(bit)) {
			/* 1 bit */
			edge_received(CEC_CAP_EDGE_FALLING);
			sleep_us(CEC_DATA_ONE_LOW_US);
			edge_received(CEC_CAP_EDGE_RISING);
			sleep_us(CEC_DATA_ONE_HIGH_US);
		} else {
			/* 0 bit */
			edge_received(CEC_CAP_EDGE_FALLING);
			sleep_us(CEC_DATA_ZERO_LOW_US);
			edge_received(CEC_CAP_EDGE_RISING);
			sleep_us(CEC_DATA_ZERO_HIGH_US);
		}
	}
	 ...
	 */
	sleep_us(CEC_NOMINAL_SAMPLE_TIME_US);
	zassert_equal(gpio_emul_output_get(CEC_OUT_PORT, CEC_OUT_PIN),
		      should_ack ? 0 : 1);
	sleep_us(CEC_NOMINAL_BIT_PERIOD_US - CEC_NOMINAL_SAMPLE_TIME_US);
}
...
ZTEST_USER(cec_bitbang, test_send_retransmit_success)
{
	...
	sleep_us(CEC_FREE_TIME_NI_US - CEC_NOMINAL_BIT_PERIOD_US +
