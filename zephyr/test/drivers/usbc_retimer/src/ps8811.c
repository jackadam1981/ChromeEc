...
ZTEST(ps8811, test_page1_registers_rw)
{
	...
	rv = ps8811_write1(PS8811_REG1_USB_BDE_CONFIG, 0xff);
	zexpect_ok(rv);
	...
	zexpect_ok(rv);
	zexpect_equal(val, a_swing_expected);
	...
	zexpect_ok(rv);
	zexpect_equal(val, adjust_b_50ohm_expected);
	...
	zexpect_ok(rv);
	zexpect_equal(val, b_swing_expected);

	/* This register has its highest reserved bit set by default. */
	const uint8_t b_de_ps_lsb_expected =
		(uint8_t)(~PS8811_REG1_USB_CHAN_B_DE_PS_LSB_RESERVED_MASK) | 0x80;
	...
	zexpect_ok(rv);
	zexpect_equal(val, b_de_ps_lsb_expected);
	...
	zexpect_ok(rv);
	zexpect_equal(val, b_de_ps_msb_expected);
