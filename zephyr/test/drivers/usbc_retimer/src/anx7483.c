...
static int anx7483_set_reg(int reg, uint8_t val)
{
...
{
	return i2c_read8(mux.i2c_port, mux.i2c_addr_flags, reg, data);
}

static int anx7483_i2c_write(int reg, int data)
{
	return i2c_write8(mux.i2c_port, mux.i2c_addr_flags, reg, data);
}
...
ZTEST(anx7483, test_registers_rw)
{
	int rv;
	uint8_t expected;
	int val;
...
ZTEST(anx7483, test_mux_state_get)
{
	...
	rv = anx7483_set_reg(ANX7483_ANALOG_STATUS_CTRL_REG, val);
	zexpect_ok(rv);
...
ZTEST(anx7483, test_reserved)
{
	int rv;

	rv = anx7483_set_reg(ANX7483_LFPS_TIMER_REG,
			     ANX7483_LFPS_TIMER_REG_RESERVED_MASK);
	zexpect_not_equal(rv, 0);

	rv = anx7483_set_reg(ANX7483_ANALOG_STATUS_CTRL_REG,
			     ANX7483_ANALOG_STATUS_CTRL_REG_RESERVED_MASK);
	zexpect_not_equal(rv, 0);

	rv = anx7483_set_reg(ANX7483_ENABLE_EQ_FLAT_SWING_REG,
			     ANX7483_ENABLE_EQ_FLAT_SWING_REG_RESERVED_MASK);
	zexpect_not_equal(rv, 0);

	rv = anx7483_set_reg(ANX7483_AUX_SNOOPING_CTRL_REG,
			     ANX7483_AUX_SNOOPING_CTRL_REG_RESERVED_MASK);
	zexpect_not_equal(rv, 0);

	/* CFG0 */
	rv = anx7483_set_reg(ANX7483_UTX1_PORT_CFG0_REG,
			     ANX7483_UTX1_PORT_CFG0_REG_RESERVED_MASK);
	zexpect_not_equal(rv, 0);

	rv = anx7483_set_reg(ANX7483_UTX2_PORT_CFG0_REG,
			     ANX7483_UTX2_PORT_CFG0_REG_RESERVED_MASK);
	zexpect_not_equal(rv, 0);

	rv = anx7483_set_reg(ANX7483_URX1_PORT_CFG0_REG,
			     ANX7483_URX1_PORT_CFG0_REG_RESERVED_MASK);
	zexpect_not_equal(rv, 0);

	rv = anx7483_set_reg(ANX7483_URX2_PORT_CFG0_REG,
			     ANX7483_URX2_PORT_CFG0_REG_RESERVED_MASK);
	zexpect_not_equal(rv, 0);

	rv = anx7483_set_reg(ANX7483_DRX1_PORT_CFG0_REG,
			     ANX7483_DRX1_PORT_CFG0_REG_RESERVED_MASK);
	zexpect_not_equal(rv, 0);

	rv = anx7483_set_reg(ANX7483_DRX2_PORT_CFG0_REG,
			     ANX7483_DRX2_PORT_CFG0_REG_RESERVED_MASK);
	zexpect_not_equal(rv, 0);

	/* CFG2 */
	rv = anx7483_set_reg(ANX7483_UTX1_PORT_CFG2_REG,
			     ANX7483_UTX1_PORT_CFG2_REG_RESERVED_MASK);
	zexpect_not_equal(rv, 0);

	rv = anx7483_set_reg(ANX7483_UTX2_PORT_CFG2_REG,
			     ANX7483_UTX2_PORT_CFG2_REG_RESERVED_MASK);
	zexpect_not_equal(rv, 0);

	rv = anx7483_set_reg(ANX7483_URX1_PORT_CFG2_REG,
			     ANX7483_URX1_PORT_CFG2_REG_RESERVED_MASK);
	zexpect_not_equal(rv, 0);

	rv = anx7483_set_reg(ANX7483_URX2_PORT_CFG2_REG,
			     ANX7483_URX2_PORT_CFG2_REG_RESERVED_MASK);
	zexpect_not_equal(rv, 0);

	rv = anx7483_set_reg(ANX7483_DRX1_PORT_CFG2_REG,
			     ANX7483_DRX1_PORT_CFG2_REG_RESERVED_MASK);
	zexpect_not_equal(rv, 0);

	rv = anx7483_set_reg(ANX7483_DRX2_PORT_CFG2_REG,
			     ANX7483_DRX2_PORT_CFG2_REG_RESERVED_MASK);
	zexpect_not_equal(rv, 0);

	/* CFG4 */
	rv = anx7483_set_reg(ANX7483_UTX1_PORT_CFG4_REG,
			     ANX7483_UTX1_PORT_CFG4_REG_RESERVED_MASK);
	zexpect_not_equal(rv, 0);

	rv = anx7483_set_reg(ANX7483_UTX2_PORT_CFG4_REG,
			     ANX7483_UTX2_PORT_CFG4_REG_RESERVED_MASK);
	zexpect_not_equal(rv, 0);

	rv = anx7483_set_reg(ANX7483_URX1_PORT_CFG4_REG,
			     ANX7483_URX1_PORT_CFG4_REG_RESERVED_MASK);
	zexpect_not_equal(rv, 0);

	rv = anx7483_set_reg(ANX7483_URX2_PORT_CFG4_REG,
			     ANX7483_URX2_PORT_CFG4_REG_RESERVED_MASK);
	zexpect_not_equal(rv, 0);

	rv = anx7483_set_reg(ANX7483_DRX1_PORT_CFG4_REG,
			     ANX7483_DRX1_PORT_CFG4_REG_RESERVED_MASK);
	zexpect_not_equal(rv, 0);

	rv = anx7483_set_reg(ANX7483_DRX2_PORT_CFG4_REG,
			     ANX7483_DRX2_PORT_CFG4_REG_RESERVED_MASK);
	zexpect_not_equal(rv, 0);

}

/* Corresponds to anx7483_usb_enabled. */
ZTEST(anx7483, test_tuning_usb)
{
	...
	rv = anx7483_get_reg(ANX7483_DTX1_PORT_CFG3_REG, &val);
	zexpect_ok(rv);
	zexpect_equal(val, ANX7483_CFG3_90Ohm_IN);

	rv = anx7483_get_reg(ANX7483_DTX2_PORT_CFG3_REG, &val);
	zexpect_ok(rv);
	zexpect_equal(val, ANX7483_CFG3_90Ohm_IN);
	...
	rv = anx7483_get_reg(ANX7483_DTX1_PORT_CFG4_REG, &val);
	zexpect_ok(rv);
	zexpect_equal(val, ANX7483_CFG4_TERM_DISABLE);

	rv = anx7483_get_reg(ANX7483_DTX2_PORT_CFG4_REG, &val);
	zexpect_ok(rv);
	zexpect_equal(val, ANX7483_CFG4_TERM_DISABLE);
}

/* Corresponds to anx7483_dp_enabled. */
...
/* Corresponds to anx7483_dock_flip. */
ZTEST(anx7483, test_tuning_dock_flipped)
{
	int rv;
	uint8_t val;
