...
{
	typedef const uint8_t br_array_type[EC_LED_COLOR_COUNT];

	RESET_FAKE(led_set_color);

	/* Zero array, call led_set_color(LED_OFF, LEFT_LED) */
	led_set_brightness(EC_LED_ID_LEFT_LED, (br_array_type){});
	/* Unsupported color, call led_set_color(LED_OFF, LEFT_LED) */
	led_set_brightness(EC_LED_ID_LEFT_LED,
			   (br_array_type){ [EC_LED_COLOR_AMBER] = 1 });
	/* Supported, call led_set_color(AMBER, RIGHT_LED) */
	led_set_brightness(EC_LED_ID_RIGHT_LED,
			   (br_array_type){ [EC_LED_COLOR_AMBER] = 1 });
