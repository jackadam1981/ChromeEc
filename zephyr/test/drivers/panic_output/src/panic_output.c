...
static void crash_thread_worker(void *a, void *b, void *c)
{
	...
	*return_val = command_crash(argc, argv);
}
...
ZTEST_SUITE(panic_output, drivers_predicate_post_main, setup, reset, reset,
	    NULL);
