...
ZTEST(rt9490_chg, aicr)
{
	...
	/* values outside (150mA, 5000mA) are illegal */
	zassert_not_equal(rt9490_drv.set_input_current_limit(chgnum, 90), 0,
			  NULL);
	zassert_not_equal(rt9490_drv.set_input_current_limit(chgnum, 3400), 0,
			  NULL);
}
...
ZTEST(rt9490_chg, misc_info)
{
	 ...
	 */
	zassert_ok(i2c_write8(chg_chips[chgnum].i2c_port,
			      chg_chips[chgnum].i2c_addr_flags,
			      RT9490_REG_CHG_STATUS4, RT9490_JEITA_HOT_MASK));
	zassert_ok(rt9490_drv.get_status(chgnum, &status), NULL);
	zassert_equal(status, CHARGER_RES_HOT | CHARGER_RES_OR, NULL);

	zassert_ok(i2c_write8(chg_chips[chgnum].i2c_port,
			      chg_chips[chgnum].i2c_addr_flags,
			      RT9490_REG_CHG_STATUS4, RT9490_JEITA_WARM_MASK));
	zassert_ok(rt9490_drv.get_status(chgnum, &status), NULL);
	zassert_equal(status, CHARGER_RES_HOT, NULL);

	zassert_ok(i2c_write8(chg_chips[chgnum].i2c_port,
			      chg_chips[chgnum].i2c_addr_flags,
			      RT9490_REG_CHG_STATUS4, RT9490_JEITA_COOL_MASK));
	zassert_ok(rt9490_drv.get_status(chgnum, &status), NULL);
	zassert_equal(status, CHARGER_RES_COLD, NULL);

	zassert_ok(i2c_write8(chg_chips[chgnum].i2c_port,
			      chg_chips[chgnum].i2c_addr_flags,
			      RT9490_REG_CHG_STATUS4, RT9490_JEITA_COLD_MASK));
	zassert_ok(rt9490_drv.get_status(chgnum, &status), NULL);
