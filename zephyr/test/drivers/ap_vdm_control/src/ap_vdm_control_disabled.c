...
ZTEST(ap_vdm_control_disabled, test_ap_vdm_stub_returns)
{
	uint8_t temp1, temp2;
	uint32_t data[2];

	zassert_equal(ap_vdm_attention_pop(TEST_PORT, data, &temp1), 0);
	zassert_equal(ap_vdm_copy_reply(TEST_PORT, &temp1, &temp2, data),
		      EC_RES_INVALID_COMMAND);
}
