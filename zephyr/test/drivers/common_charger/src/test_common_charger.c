...
ZTEST(common_charger, test_charger_set_vsys_compensation__bad_arg)
{
	...
	zassert_equal(charger_set_vsys_compensation(
			      board_get_charger_chip_count(), &unused, 0, 0),
		      EC_ERROR_INVAL);
}
...
ZTEST(common_charger, test_charger_get_battery_cells__bad_arg)
{
	...
	zassert_equal(charger_get_battery_cells(board_get_charger_chip_count(),
						&unused),
		      EC_ERROR_INVAL);
}
