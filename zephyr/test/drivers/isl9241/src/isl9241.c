...
#if 0
/* need precise tuning of setup and checks */
ZTEST_F(isl9241_driver, test_bypass_sequence_no_charging)
{
	int temp;

	/* Battery isn't charging */
	zassert_ok(charger_set_current(CHARGER_SOLO, 0));

	/* Start us with 20V AC we can use in bypass mode, battery not charging
	 */
	isl9241_emul_set_vbus(fixture->isl9241_emul, 20000);
	zassert_ok(charger_enable_bypass_mode(CHARGER_SOLO, true));

	/* Verify we turned on bypass mode */
	zassert_equal((isl9241_emul_peek(fixture->isl9241_emul,
					 ISL9241_REG_CONTROL0) &
		       ISL9241_CONTROL0_EN_BYPASS_GATE),
		      ISL9241_CONTROL0_EN_BYPASS_GATE);
	zassert_ok(charger_get_vsys_voltage(CHARGER_SOLO, &temp));
	zassert_equal(temp, 0);

	/* AC still present, but no longer in bypass mode */
	zassert_ok(charger_enable_bypass_mode(CHARGER_SOLO, false));
	zassert_equal((isl9241_emul_peek(fixture->isl9241_emul,
					 ISL9241_REG_CONTROL0) &
		       ISL9241_CONTROL0_EN_BYPASS_GATE),
		      0);
	zassert_ok(charger_get_vsys_voltage(CHARGER_SOLO, &temp));
	zassert_not_equal(temp, 0);
}

ZTEST_F(isl9241_driver, test_bypass_sequence_charging)
{
	int temp;

	/* Battery is "already charging" */
	zassert_ok(charger_set_current(CHARGER_SOLO, 3000));

	/* Start us with 20V AC we can use in bypass mode */
	isl9241_emul_set_vbus(fixture->isl9241_emul, 20000);
	zassert_ok(charger_enable_bypass_mode(CHARGER_SOLO, true));
