...
ZTEST_USER(usbc_intel_altmode, test_read_error_console_cmd)
{
	const char *buffer;
	size_t buffer_size;
	...
	zassert_equal(-EINVAL,
		      shell_execute_cmd(get_ec_shell(), "altmode read 5"));
...
ZTEST_USER(usbc_intel_altmode, test_write_error_console_cmd)
{
	const char *buffer;
	size_t buffer_size;
	const char *err_msg = "Invalid port";
	...
	zassert_equal(
		-EINVAL,
		shell_execute_cmd(get_ec_shell(),
				  "altmode write 5 0x0 0x1 0x2 0x3 0x4 0x5"));
