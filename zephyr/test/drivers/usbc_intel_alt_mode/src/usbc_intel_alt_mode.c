...
void usbc_intel_altmode_before(void *fixture)
{
	...
	for (int i = 0; i < USBC_PORT_COUNT; i++) {
		emul_intel_pd_controller_reset(emul_pd_ctrlr[i]);
	}
...
ZTEST_USER(usbc_intel_altmode, test_read_console_cmd)
{
	...
	emul_intel_pd_controller_set_reg_value(
		emul_pd_ctrlr[USBC_PORT0], INTEL_ALTMODE_REG_DATA_STATUS,
		status.raw_value, INTEL_ALTMODE_DATA_STATUS_REG_LEN);
