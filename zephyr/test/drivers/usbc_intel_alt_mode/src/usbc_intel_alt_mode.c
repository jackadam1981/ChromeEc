...
void usbc_intel_altmode_before(void *fixture)
{
	struct emul_intel_pd_controller_backend_api *backend_api =
		(struct emul_intel_pd_controller_backend_api *)
			emul_pd_ctrlr[USBC_PORT0]
				->backend_api;

	/*Reset resgister value */
	for (int i = 0; i < USBC_PORT_COUNT; i++) {
		backend_api->reset(emul_pd_ctrlr[i]);
	}
...
ZTEST_USER(usbc_intel_altmode, test_conn)
{
	...
	for (int i = 0; i < USBC_PORT_COUNT; i++) {
		backend_api->connect_data(emul_pd_ctrlr[i]);
	}
	USBC_INTEL_ALTMODE_WAIT();
	for (int i = 0; i < USBC_PORT_COUNT; i++) {
...
ZTEST_USER(usbc_intel_altmode, test_dp)
{
	...
	zassert_false(CHECK_MUX_FLAGS(USBC_PORT1, USB_PD_MUX_USB_ENABLED));
	backend_api->connect_data(emul_pd_ctrlr[USBC_PORT0]);
	backend_api->connect_dp(emul_pd_ctrlr[USBC_PORT0]);
	USBC_INTEL_ALTMODE_WAIT();
	...
	backend_api->connect_data(emul_pd_ctrlr[USBC_PORT1]);
	backend_api->connect_dp(emul_pd_ctrlr[USBC_PORT1]);
	backend_api->set_dp_irq(emul_pd_ctrlr[USBC_PORT1]);
	USBC_INTEL_ALTMODE_WAIT();
	zassert_true(CHECK_MUX_FLAGS(USBC_PORT0, USB_PD_MUX_DP_ENABLED));
...
ZTEST_USER(usbc_intel_altmode, test_mux_usb2)
{
	...
	for (int i = 0; i < USBC_PORT_COUNT; i++) {
		backend_api->connect_data(emul_pd_ctrlr[i]);
		backend_api->connect_usb2(emul_pd_ctrlr[i]);
	}
	USBC_INTEL_ALTMODE_WAIT();
...
ZTEST_USER(usbc_intel_altmode, test_mux_usb3_2)
{
	...
	for (int i = 0; i < USBC_PORT_COUNT; i++) {
		backend_api->connect_data(emul_pd_ctrlr[i]);
		backend_api->connect_usb3_2(emul_pd_ctrlr[i]);
	}
	USBC_INTEL_ALTMODE_WAIT();
...
ZTEST_USER(usbc_intel_altmode, test_mux_hpd_lvl)
{
	...
	for (int i = 0; i < USBC_PORT_COUNT; i++) {
		backend_api->connect_data(emul_pd_ctrlr[i]);
		backend_api->set_hpd_lvl(emul_pd_ctrlr[i]);
	}
	USBC_INTEL_ALTMODE_WAIT();
...
ZTEST_USER(usbc_intel_altmode, test_read_console_cmd)
{
	...
	backend_api->set_reg_value(emul_pd_ctrlr[USBC_PORT0],
				   INTEL_ALTMODE_REG_DATA_STATUS,
				   status.raw_value,
