...
ZTEST(test_btn_ign_in, test_btn_ign_not_set)
{
	deassert_btn_ign();
	...
	k_sleep(K_SECONDS(4));
	zassert_equal(1, power_button_is_pressed(),
		      "Power button should not be ignored after 6s");
}

ZTEST(test_btn_ign_in, test_btn_ign_set)
{
	assert_btn_ign();
	power_button_simulate_press(10000);
	...
	k_sleep(K_SECONDS(1));
	zassert_equal(0, power_button_is_pressed(),
		      "Power button should be ignored after 1s");
	k_sleep(K_SECONDS(4));
	zassert_equal(1, power_button_is_pressed(),
		      "Power button should not be ignored after 6s");
}

ZTEST_SUITE(btn_ign, drivers_predicate_pre_main, NULL, NULL, NULL, NULL);
