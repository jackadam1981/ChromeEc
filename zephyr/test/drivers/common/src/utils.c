...
host_cmd_get_next_event_v2(struct ec_response_get_next_event_v1 *response)
{
	...
	LOG_INF("lschyi: args.response_size = %d", args.response_size);
	return host_command_process(&args);
}
...
{
	LOG_INF("lschyi: port equal? %d, port = %d",
		(EC_MKBP_EVENT_CEC_GET_PORT(event->data.cec_events) == port),
		EC_MKBP_EVENT_CEC_GET_PORT(event->data.cec_events));
	LOG_INF("lschyi: event equal? %d, events = %d",
