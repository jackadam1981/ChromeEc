...
{
	static const size_t copy_buffer_size = 128;
	...
	if (expected_output) {
		...
		if (!strstr(buffer, expected_output)) {
			copy_size = MIN(buffer_size, copy_buffer_size);
			memcpy(copy_buffer, buffer, copy_size);
		}
		zassert_true(strstr(buffer, expected_output),
			     "Invalid console output %.*s", copy_size,
			     copy_buffer);
	}
}
		      ...
		      const int line, const char *format, ...)
{
	...
	va_end(args);
	/* zassert_equal overwrites the backend buffer so we copy it first. */
	...
	zassert_equal(
		...
		file, line, cmd, copy_size, copy_buffer, format);
}
