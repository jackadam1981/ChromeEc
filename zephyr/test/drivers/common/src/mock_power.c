...
#include "test/drivers/mock_power.h"
				   ...
				   void *data)
{
	...
	power_handle_state_fake.custom_fake = mock_power_handle_state;
	chipset_force_shutdown_fake.custom_fake = mock_chipset_force_shutdown;
	chipset_power_on_fake.custom_fake = mock_chipset_power_on;
	command_power_fake.custom_fake = mock_command_power;
}
...
void handle_power_request(enum power_request_t req)
{
	if (current_power_request == POWER_REQ_NONE) {
		...
		CPRINTS("MOCK: Handling %s, pend %s request",
...
void mock_chipset_force_shutdown(enum chipset_shutdown_reason reason)
{
	CPRINTS("MOCK %s(%d)", __func__, reason);
	handle_power_request(POWER_REQ_OFF);
	task_wake(TASK_ID_CHIPSET);
}

void mock_chipset_power_on(void)
{
...
int mock_command_power(int argc, char **argv)
{
	int v, req;
	...
	ccprintf("MOCK: Requesting power %s\n", power_req_name[req]);
	task_wake(TASK_ID_CHIPSET);
...
static void mock_lid_event(void)
{
	...
	ccprintf("MOCK: lid opened %s\n", power_req_name[POWER_REQ_ON]);
	handle_power_request(POWER_REQ_ON);
	task_wake(TASK_ID_CHIPSET);
}
DECLARE_HOOK(HOOK_LID_CHANGE, mock_lid_event, HOOK_PRIO_DEFAULT);

enum power_state mock_power_handle_state(enum power_state state)
{
	...
	printf("MOCK: power request=%u, state=%u -> new_state=%u\n", request,
	       state, new_state);
