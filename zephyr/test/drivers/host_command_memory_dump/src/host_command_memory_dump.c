...
void register_known_threads_memory_dump(void);
...
static void find_thread_by_name_cb(k_tid_t thread, const char *user_data)
{
...
ZTEST_USER(memory_dump, dump_thread_stack)
{
	...
	register_known_threads_memory_dump();
...
ZTEST_USER(memory_dump, verify_excluded_threads_not_dumped)
{
	...
	keyproto_thread = find_thread_by_name("KEYPROTO");
	...
	/*
	 * Verify KEYSCAN and KEYPROTO thread stack is NOT in dump.
