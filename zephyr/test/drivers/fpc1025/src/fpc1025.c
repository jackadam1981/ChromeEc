...
ZTEST_F(fpc1025, test_get_info)
{
	...
	zassert_equal(info.pixel_format, FINGERPRINT_SENSOR_V4L2_PIXEL_FORMAT(DT_NODELABEL(fpc1025)));
	zassert_equal(info.width, FINGERPRINT_SENSOR_RES_X(DT_NODELABEL(fpc1025)));
	zassert_equal(info.height, FINGERPRINT_SENSOR_RES_Y(DT_NODELABEL(fpc1025)));
	zassert_equal(info.bpp, FINGERPRINT_SENSOR_RES_BPP(DT_NODELABEL(fpc1025)));
	zassert_equal(info.errors, FINGERPRINT_ERROR_DEAD_PIXELS_UNKNOWN);
}

ZTEST_F(fpc1025, test_enter_low_power_mode)
{
	zassert_ok(fingerprint_set_mode(fixture->dev, FINGERPRINT_SENSOR_MODE_LOW_POWER));
	zassert_equal(fpc1025_get_low_power_mode(fixture->target), 1);
}

ZTEST_F(fpc1025, test_enter_idle)
{
	zassert_ok(fingerprint_set_mode(fixture->dev, FINGERPRINT_SENSOR_MODE_IDLE));
}
...
{
        const struct gpio_dt_spec spec = GPIO_DT_SPEC_GET(DT_NODELABEL(fpc1025), irq_gpios);
	...
	zassert_ok(gpio_pin_interrupt_configure_dt(&spec, GPIO_INT_EDGE_TO_ACTIVE));
...
ZTEST_F(fpc1025, test_maintenance_not_supported)
{
	...
	zassert_equal(fingerprint_maintenance(fixture->dev, buffer, sizeof(buffer)), -ENOTSUP);
}
...
ZTEST_F(fpc1025, test_acquire_image_not_supported)
{
	...
	zassert_equal(fingerprint_acquire_image(fixture->dev, 0, buffer, sizeof(buffer)), -ENOTSUP);
}

ZTEST_F(fpc1025, test_sensor_mode_detect_not_supported)
{
	zassert_equal(fingerprint_set_mode(fixture->dev, FINGERPRINT_SENSOR_MODE_DETECT), -ENOTSUP);
}
