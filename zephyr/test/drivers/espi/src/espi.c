...
ZTEST_USER(espi, espi_host_cmd)
{
	...
	request->data_len = 4;
	*data_ptr = 0x10203040;
	request->checksum = calculate_hostcmd_checksum(
		(char *)request, sizeof(*request) + sizeof(*data_ptr));
	...
	zassert_equal(hostcmd_resp_recv, 1, "result callback should be called");
