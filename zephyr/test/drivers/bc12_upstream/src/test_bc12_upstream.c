...
/*
 * This test suite verifies integration of upstream BC1.2 drivers with the
 * EC application. This test suite is driver agnostic, and should not
 * perform any driver specific checks.
 */
...
ZTEST_F(bc12_upstream, test_bc12_client_mode_sdp)
{
	...
	k_sleep(K_MSEC(CHARGE_DETECT_DELAY * 2));
...
ZTEST_F(bc12_upstream, test_bc12_client_mode_cdp)
{
	struct ec_response_usb_pd_power_info response;
...
ZTEST_F(bc12_upstream, test_bc12_client_mode_dcp)
{
	struct ec_response_usb_pd_power_info response;
...
{
	static struct bc12_upstream_fixture fixture = {
...
static void bc12_before(void *f)
{
	struct bc12_upstream_fixture *fixture = f;
...
ZTEST_SUITE(bc12_upstream, drivers_predicate_post_main, bc12_setup, bc12_before,
	    bc12_after, NULL);
