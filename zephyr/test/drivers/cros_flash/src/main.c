...
FAKE_VALUE_FUNC(int, flash_disable_option_bytes, const struct device *);
FAKE_VALUE_FUNC(int, flash_disable_control_register, const struct device *);

static uint32_t protected_mask;
static bool option_bytes_disabled;
static bool control_register_disabled;
static bool rdp_enabled;

static int flash_disable_option_bytes_custom_fake(const struct device *dev)
{
	option_bytes_disabled = true;

	return 0;
}

static int flash_disable_control_register_custom_fake(const struct device *dev)
{
	control_register_disabled = true;
...
static int flash_change_rdp_custom_fake(const struct device *dev, bool enable,
					bool permanent)
{
	/* We don't support disabling RDP or enabling RDP permanently. */
	if ((!enable && rdp_enabled) || permanent)
		ztest_test_fail();
...
ZTEST_USER(cros_flash, test_init_no_hwwp_no_protection)
{
	...
	zassert_equal(flash_disable_control_register_fake.call_count, 0);
	zassert_equal(flash_disable_option_bytes_fake.call_count, 0);
...
ZTEST_USER(cros_flash, test_init_hwwp_rdp_and_wp_enabled_disables_option)
{
	 ...
	 */
	zassert_equal(flash_disable_option_bytes_fake.call_count, 1);
}
...
ZTEST_USER(cros_flash, test_protect_now_all)
{
	...
	/* Check that only option bytes are disabled. */
	zassert_equal(flash_disable_option_bytes_fake.call_count, 1);
	zassert_equal(flash_disable_control_register_fake.call_count, 1);
}

ZTEST_USER(cros_flash, test_protect_at_boot_option_disabled)
{
	cros_flash_init(cros_flash_dev);
...
void cros_flash_before(void *fixture)
{
	...
	option_bytes_disabled = false;
	control_register_disabled = false;
	RESET_FAKE(flash_disable_option_bytes);
	RESET_FAKE(flash_disable_control_register);
	...
	flash_disable_option_bytes_fake.custom_fake =
		flash_disable_option_bytes_custom_fake;
	flash_disable_control_register_fake.custom_fake =
		flash_disable_control_register_custom_fake;
}
