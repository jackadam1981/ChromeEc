...
ZTEST(anx7452, test_anx7452_init)
{
	...
	zassert_equal(anx7452_emul_get_reg(EMUL, ANX7452_TOP_STATUS_REG), 0, NULL);
	zassert_equal(
		EC_SUCCESS,
	...
	zassert_equal(1, gpio_emul_output_get(gpio_dev, GPIO_USB_C1_USB_EN_PORT),
		      NULL);

	/* Setup emulator fail on write */
	i2c_common_emul_set_write_fail_reg(COMMON_DATA, ANX7452_TOP_STATUS_REG);
	...
	i2c_common_emul_set_read_fail_reg(COMMON_DATA, ANX7452_TOP_STATUS_REG);
	/* With reg read fail, init should fail and pins should be unset */
	zassert_equal(
		EC_ERROR_TIMEOUT,
		anx7452_usb_retimer_driver.init(usb_muxes[USBC_PORT_C1].mux),
