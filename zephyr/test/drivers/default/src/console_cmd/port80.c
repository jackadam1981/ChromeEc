 ...
 * @param shell_buffer Set to the shell output buffer so caller can perform
 * additional checks.
 * @return 0 on success
 */
int check_port80_output(int head, int tail, const char **shell_buffer)
{
	const char *buffer;
	size_t buffer_size;
	char port_80_str[3];
	...
	if (strstr(buffer, port80_header) == NULL) {
	...
	for (int i = head; i < tail; i++) {
		...
		if (strstr(buffer, port_80_str) == NULL) {
