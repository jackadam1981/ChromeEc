...
ZTEST_F(battery, test_is_charge_fet_disabled)
{
	 ...
	 * original code had 5 more 0x00 pad bytes, but fuel gauge assumes 16b
	 */
	static uint8_t values[] = { 0x20, 0x54, 0XFF };

	static struct battery2_read_data data = {
		.count = ARRAY_SIZE(values) - 1, /* drop PEC */
		.values = values,
	};

	/* TODO: This code should actually live in an API to tell the
	 * emulator what an expected response is, rather than in the test
	 */
	if (IS_ENABLED(CONFIG_SMBUS_PEC)) {
		uint8_t pec;

		uint8_t addr_8bit = 0xb << 1; /* make room for the R/W bit */
...
ZTEST_F(battery, test_get_disconnect_state)
{
	...
	struct battery2_read_data data = {
		.count = 2, /* default value for non PEC */
		.values = values,
	};
	...
	if (IS_ENABLED(CONFIG_SMBUS_PEC)) {
		...
		uint8_t addr_8bit = 0xb << 1; /* make room for the R/W bit */
		/* reg is 0x0 for Manufacturer access */
		uint8_t out[3] = { addr_8bit, 0x0, addr_8bit | 1 };
