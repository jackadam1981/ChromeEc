...
ZTEST_F(battery, test_is_charge_fet_disabled)
{
	...
	/* Check that battery 2 returns 0 (cfet_mask is 0) */
	battery_fuel_gauge_type_override = 2;
	zassert_equal(0, battery_is_charge_fet_disabled());

	/* Setup an error condition for battery 1 to fail writing to i2c */
	battery_fuel_gauge_type_override = 1;
	fixture->battery_i2c_common->finish_write = NULL;
	battery2_write_func_fake.return_val = -1;
	i2c_common_emul_set_write_func(fixture->battery_i2c_common,
	...
	/* Succeed writing to i2c and the function should now return true */
	battery2_write_func_fake.return_val = 0;
...
static int battery2_read_fet(const struct emul *target, int reg, uint8_t *val,
		int bytes, void *data)
{
	static uint8_t values[] = { 0x00, 0x20 };

	if (bytes < ARRAY_SIZE(values)) {
		*val = values[bytes];
	}
...
{
	/* Check disconnect error for invalid battery type */
	battery_fuel_gauge_type_override = BATTERY_TYPE_COUNT;
	zassert_equal(BATTERY_DISCONNECT_ERROR, battery_get_disconnect_state());

	/* Use battery 0 and configure i2c to fail on read */
	battery_fuel_gauge_type_override = 0;
	...
	battery2_read_func_fake.return_val = 0;
	battery2_read_func_fake.custom_fake = battery2_read_fet;
