...
ZTEST(power_common_bring_up, test_siglog_output)
{
	...
	buffer = shell_backend_dummy_get_output(get_ec_shell(), &buffer_size);
	zassert_equal(NULL, strstr(buffer, "2 signal changes:\n"));
	zassert_equal(NULL, strstr(buffer, "+0.000000  mb_power_good => 1\n"));
	zassert_equal(NULL, strstr(buffer, "mb_power_good => 0\n"));
	shell_backend_dummy_clear_output(get_ec_shell());
	...
	zassert_equal(NULL, strstr(buffer, "24 signal changes:\n"));
	zassert_equal(NULL, strstr(buffer, "+0.000000  mb_power_good => 1\n"));
