...
static int kbc8042_ibf_cnt;
static uint8_t kbc8042_data_recv;
static uint8_t kbc8042_data_recv_is_cmd;

static void espi_kbc_ibf_cb(const struct device *dev, struct espi_callback *cb,
...
ZTEST_USER(espi_peripheral, test_host_read_and_write_kb_char)
{
	...
	espi_emul_host_io_read(ESPI_DEVICE, 1, ESPI_EMUL_KBC8042_PORT_IN_DATA,
			       &reg);
...
ZTEST_USER(espi_peripheral, test_lpc_input_pending)
{
	...
	/* Prepare callback */
	espi_init_callback(&espi_cb_obj, espi_kbc_ibf_cb,
			   ESPI_BUS_PERIPHERAL_NOTIFICATION);
	espi_add_callback(ESPI_DEVICE, &espi_cb_obj);

	espi_write_lpc_request(ESPI_DEVICE, E8042_PAUSE_IRQ, NULL);

	zassert_equal(lpc_keyboard_input_pending(), 0,
		      "should be no input pending");
	espi_emul_host_io_write(ESPI_DEVICE, 1, ESPI_EMUL_KBC8042_PORT_OUT_DATA,
				reg);
	zassert_equal(lpc_keyboard_input_pending(), 1,
		      "should be input pending");
...
{
	uint32_t data = 0;
	struct espi_evt_data_kbc *kbc;

	kbc = (struct espi_evt_data_kbc *)&data;
...
ZTEST_SUITE(espi_peripheral, drivers_predicate_post_main, NULL, NULL, NULL,
	    NULL);
