...
static __sighandler_t prev_handler;

/*
 * The test below fails by trying to dereference a NULL pointer.
 * In that case simply mark the test as failed instead of crashing ourselves.
 */
...
ZTEST_USER(motion_sense, test_fifo_data_validation)
{
	...
	prev_handler = signal(SIGSEGV, handle_signal);

	fifo_stage_unit(&data, NULL, 0);
	motion_sense_fifo_commit_data();
	/* Fail condition is checked in the signal handler. */
}
