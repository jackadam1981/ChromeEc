...
ZTEST(usb_attach_5v_100ma_pd_sink, test_sink_caps_pdos)
{
	...
	test_fixture->snk_ext.pdo[0] = TEST_SINK_CAP_5V_3000MA;

	/* Send Request with Cap Mismatch */
	tcpci_snk_emul_send_request_msg(test_fixture->snk_ext, test_fixture->sink_5v_100ma, 3000, true);

	k_sleep(K_SECONDS(1));

	/* Checking the received sink caps with the new ones we set */
    zassert_within(info.meas.voltage_now, TEST_SRC_PORT_VBUS_MV, 500,
