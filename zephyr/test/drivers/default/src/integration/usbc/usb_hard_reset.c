...
/**
 * @brief TestPurpose: Verify that if VBUS goes to 0V after hard reset
 *        and doesn't return, that we go to disconnected state.
 */
ZTEST_F(usb_hard_reset_source, test_no_vbus_after_hard_reset)
{
	/* Partner sends the Hard Reset. */
	tcpci_partner_common_send_hard_reset(&fixture->partner_emul);
	/* Partner waits ~30 ms - 650 ms */
	k_sleep(K_MSEC(30));
	...
	k_sleep(K_SECONDS(2));
	zassert_equal(get_state_tc(TEST_USB_PORT), /* TC_UNATTACHED_SNK */ 1);
}
