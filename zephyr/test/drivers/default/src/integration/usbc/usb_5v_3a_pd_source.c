...
ZTEST_F(usb_attach_5v_3a_pd_source, verify_dps_battery_absent)
{
	...
	k_sleep(K_MSEC(5000));
	zassert_true(dps_get_flag() & DPS_FLAG_NO_BATTERY, "DPS_FLAG_NO_BATTERY is set");
	control_battery_present(true);
	zassert_true(battery_is_present(), "dps battery is not present");
}

ZTEST_F(usb_attach_5v_3a_pd_source, verify_dps_enable)
{
	dps_enable(false);
	...
	/* wait dps_config.t_check*/
	k_sleep(K_MSEC(5000));
	zassert_true(dps_get_flag() & DPS_FLAG_DISABLED, "DPS_FLAG_DISABLED is set");
	dps_enable(true);
