...
static void tcpm_before(void *state)
{
	...
	fixture->saved_driver_ptr = tcpm_test_config->drv;
}
...
static void tcpm_after(void *state)
{
	...
	tcpm_test_config->drv = fixture->saved_driver_ptr;

	RESET_FAKE(set_vconn);
}


ZTEST_SUITE(tcpm, drivers_predicate_pre_main, NULL, NULL, tcpm_before,
	tcpm_after);
