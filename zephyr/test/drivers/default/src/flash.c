...
#define WP_L_GPIO_PIN DT_GPIO_PIN(WP_L_GPIO_PATH, gpios)
...
static int gpio_wp_l_set(int value)
{
	...
	return gpio_emul_input_set(wp_l_gpio_dev, WP_L_GPIO_PIN, value);
}
...
static void flash_after(void *data)
{
	...
	zassert_ok(gpio_wp_l_set(0), NULL);

	/* Reset the protection flags */
	cros_flash_emul_protect_reset();
}

ZTEST_SUITE(flash, drivers_predicate_post_main, NULL, NULL, flash_after,
	    NULL);
