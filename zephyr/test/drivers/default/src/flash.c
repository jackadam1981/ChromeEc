...
ZTEST_USER(flash, test_console_cmd_flashwp__noall)
{
	uint32_t current;

	gpio_wp_l_set(1);

	zassert_ok(shell_execute_cmd(get_ec_shell(), "flashwp noall"), NULL);

	current = crec_flash_get_protect();
	zassert_equal(0, current, "current = %08x", current);
}
 ...
 * @param make_write If true, write an arbitray byte after erase so the region
