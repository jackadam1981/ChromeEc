...
ZTEST(system_save_flags, test_system_encode_save_flags)
{
	...
	zassert_true(saved_flags & EC_RESET_FLAG_AP_OFF, NULL);
	zassert_true(saved_flags & EC_RESET_FLAG_STAY_IN_RO, NULL);
	zassert_true(saved_flags & EC_RESET_FLAG_AP_WATCHDOG, NULL);

	/* Verify mutually exlusivity of reset reason flags */
	...
	zassert_true(saved_flags & EC_RESET_FLAG_HARD, NULL);
	zassert_false(saved_flags & EC_RESET_FLAG_HIBERNATE, NULL);
	zassert_false(saved_flags & EC_RESET_FLAG_SOFT, NULL);
	...
	zassert_false(saved_flags & EC_RESET_FLAG_HARD, NULL);
	zassert_true(saved_flags & EC_RESET_FLAG_HIBERNATE, NULL);
	zassert_false(saved_flags & EC_RESET_FLAG_SOFT, NULL);

	/* Verify reset soft is always saved */
	flags_to_save = 0;
	...
	zassert_false(saved_flags & EC_RESET_FLAG_HARD, NULL);
	zassert_false(saved_flags & EC_RESET_FLAG_HIBERNATE, NULL);
	zassert_true(saved_flags & EC_RESET_FLAG_SOFT, NULL);
