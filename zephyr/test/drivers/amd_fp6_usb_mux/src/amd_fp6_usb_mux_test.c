...
ZTEST_SUITE(amd_fp6_usb_mux, drivers_predicate_post_main, NULL,
...
ZTEST(amd_fp6_usb_mux, test_mux_not_ready)
{
	/* Set the crossbar to not ready yet */
	amd_fp6_emul_set_xbar(amd_fp6_emul, false);
	...
	amd_fp6_emul_set_xbar(amd_fp6_emul, true);
...
ZTEST(amd_fp6_usb_mux, test_chipset_reset)
{
	...
	amd_fp6_emul_reset_regs(amd_fp6_emul);
	zassert_equal(usb_mux_get(TEST_PORT), USB_PD_MUX_NONE);
...
ZTEST(amd_fp6_usb_mux, test_long_command)
{
	/* Allow the mux to take a while, like on real systems */
	amd_fp6_emul_set_delay(amd_fp6_emul, 100);
