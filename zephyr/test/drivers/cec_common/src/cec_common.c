...
static int mock_get_enable_custom_fake(int port, uint8_t *enable)
{
...
static int mock_get_logical_addr_custom_fake(int port, uint8_t *logical_addr)
{
...
{
	memcpy(received_message_custom_fake, msg, len);
	received_message_len_custom_fake = len;
}
static int mock_get_received_message_custom_fake(int port, uint8_t **msg,
						 uint8_t *len)
{
...
static int host_cmd_cec_set(int port, enum cec_command cmd, uint8_t val)
{
	struct ec_params_cec_set params = {
		.cmd_port = EC_CEC_CMD_PORT_PACK(cmd, port),
		.val = val,
	};
...
ZTEST_USER_F(cec_common, test_hc_cec_get_enable_0)
{
	...
	mock_get_enable_fake.custom_fake = mock_get_enable_custom_fake;
...
ZTEST_USER_F(cec_common, test_hc_cec_get_logical_addr)
{
	...
	mock_get_logical_addr_fake.custom_fake =
		mock_get_logical_addr_custom_fake;
...
ZTEST_USER_F(cec_common, test_mkbp_event_multiple_events)
{
	...
	zassert_ok(get_next_cec_mkbp_event(&event));
	zassert_true(cec_event_matches(&event, TEST_PORT,
				       EC_MKBP_CEC_SEND_OK |
...
ZTEST_USER_F(cec_common, test_receive_message)
{
	...
	mock_get_received_message_fake.custom_fake =
		mock_get_received_message_custom_fake;
