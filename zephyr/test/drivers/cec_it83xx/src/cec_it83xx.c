...
FAKE_VOID_FUNC(it8xxx2_cec_irq_connect);
...
static void cec_it83xx_after(void *fixture)
{
	...
	RESET_FAKE(it8xxx2_cec_irq_connect);
...
ZTEST_USER(cec_it83xx, test_setup_for_it8xxx2)
{
	...
	zassert_equal(it8xxx2_cec_irq_connect_fake.call_count, 1);
	zassert_equal(sleep_mask, SLEEP_MASK_FORCE_NO_DSLEEP);
	drv->get_enable(TEST_PORT, &enable);
	zassert_equal(enable, 1);
	...
	zassert_equal(it8xxx2_cec_clock_enable_peripheral_fake.arg0_val, 1);
	zassert_equal(it8xxx2_cec_irq_connect_fake.call_count, 2);
	zassert_equal(sleep_mask, SLEEP_MASK_FORCE_NO_DSLEEP);
	drv->get_enable(TEST_PORT, &enable);
	zassert_equal(enable, 1);
	drv->set_enable(TEST_PORT, 1);
