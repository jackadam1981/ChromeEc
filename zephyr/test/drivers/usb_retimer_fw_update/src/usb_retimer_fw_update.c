...
ZTEST(usb_retimer_fw_update, verify_port_query)
{
...
ZTEST(usb_retimer_fw_update, verify_port_suspend)
{
	...
	zassert_true(acpi_read(EC_ACPI_MEM_USB_RETIMER_FW_UPDATE) == 0,
		     "Failed to see successful suspend");
}

ZTEST(usb_retimer_fw_update, verify_port_resume)
{
	...
	zassume_true(acpi_read(EC_ACPI_MEM_USB_RETIMER_FW_UPDATE) == 0,
		     "Failed to see successful suspend");
	...
	zassert_true(acpi_read(EC_ACPI_MEM_USB_RETIMER_FW_UPDATE) == 1,
		     "Failed to see successful resume");
}

ZTEST(usb_retimer_fw_update, verify_mux_get)
{
	...
	zassert_true(acpi_read(EC_ACPI_MEM_USB_RETIMER_FW_UPDATE) ==
			     typec_status.mux_state,
		     "Failed to match mux state");
}

/* Commands which first require supsend to be run */
ZTEST(usb_retimer_fw_update, verify_mux_usb_set)
{
	/* Write our command to suspend the port first */
	acpi_write(EC_ACPI_MEM_USB_RETIMER_FW_UPDATE,
		   USB_RETIMER_FW_UPDATE_SUSPEND_PD
...
ZTEST(usb_retimer_fw_update, verify_mux_safe_set)
{
	/* Write our command to suspend the port first */
	acpi_write(EC_ACPI_MEM_USB_RETIMER_FW_UPDATE,
		   USB_RETIMER_FW_UPDATE_SUSPEND_PD
...
ZTEST(usb_retimer_fw_update, verify_mux_tbt_set)
{
	/* Write our command to suspend the port first */
	acpi_write(EC_ACPI_MEM_USB_RETIMER_FW_UPDATE,
		   USB_RETIMER_FW_UPDATE_SUSPEND_PD
...
ZTEST(usb_retimer_fw_update, verify_update_disconnect)
{
	 ...
	 * could be brittle.
	 */
	zassume_false(pd_is_port_enabled(TEST_PORT), "Port not suspended");
