...
ZTEST_USER_F(src_policy, test_src_policy_one_3a)
{
	...
	zassert_true(TEST_WAIT_FOR(pdc_power_mgmt_is_connected(TEST_USBC_PORT0),
				   PDC_TEST_TIMEOUT));

	/* pdc_power_mgmt_is_connected() waits until the SRC_ATTACHED sub-state
	 * is SRC_ATTACHED_RUN, but we also need to wait for the source policies
	 * to run.
	 */
	k_msleep(PDC_POLICY_DELAY_MS);
	...
	zassert_true(TEST_WAIT_FOR(pdc_power_mgmt_is_connected(TEST_USBC_PORT1),
				   PDC_TEST_TIMEOUT));

	/* pdc_power_mgmt_is_connected() waits until the SRC_ATTACHED sub-state
	 * is SRC_ATTACHED_RUN, but we also need to wait for the soure policies
	 * to run.
	 */
	k_msleep(PDC_POLICY_DELAY_MS);
...
ZTEST_USER_F(src_policy, test_src_policy_disconnect_3a)
{
	...
	zassert_true(
		TEST_WAIT_FOR(!pdc_power_mgmt_is_connected(TEST_USBC_PORT0),
			      PDC_TEST_TIMEOUT));
	k_msleep(PDC_POLICY_DELAY_MS);
