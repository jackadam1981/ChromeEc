...
/* Verify FRS is intially enabled by the PDC power manager when 3A is
 * available
 */
ZTEST_USER_F(src_policy, test_src_policy_early_frs_enable)
{
	...
	while (!!emul_pdc_get_frs(fixture->emul_pdc[TEST_USBC_PORT0],
				  &frs_enabled)) {
		k_msleep(10);
	}
...
/* Verify FRS is intially disable by the PDC power manager when 3A is not
 * available.
 */
ZTEST_USER_F(src_policy, test_src_policy_early_frs_disable)
{
	...
	/* Wait for FRS to be set by the PDC power manager */
	while (!!emul_pdc_get_frs(fixture->emul_pdc[TEST_USBC_PORT1],
				  &frs_enabled)) {
		k_msleep(10);
	}
