...
FAKE_VALUE_FUNC(int, mock_pdc_set_sink_path, const struct device *, bool);
...
static uint8_t sink_path_en_mask = BIT_MASK(CONFIG_USB_PD_PORT_MAX_COUNT);
...
static void dead_battery_policy_before(void *f)
{
	...
	RESET_FAKE(mock_pdc_set_sink_path);

	chipset_in_state_fake.custom_fake = custom_fake_chipset_in_state;
	mock_pdc_set_sink_path_fake.custom_fake = custom_fake_pdc_set_sink_path;
