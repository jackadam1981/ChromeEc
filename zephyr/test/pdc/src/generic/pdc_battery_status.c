...
ZTEST_USER(pdc_battery_status, test_battery_status_update)
{
    ...
    /* Assert functions are called only once*/
    zassert_equal(battery_remaining_capacity_fake.call_count, 1);
    zassert_equal(battery_design_voltage_fake.call_count, 1);
    zassert_equal(battery_status_fake.call_count, 1);
}
...
ZTEST_USER(pdc_battery_status, test_battery_capability_update)
{
    ...
    /* Assert functions are called only once*/
    zassert_equal(battery_design_voltage_fake.call_count, 1);
    zassert_equal(battery_design_capacity_fake.call_count, 1);
    zassert_equal(battery_full_charge_capacity_fake.call_count, 1);
}