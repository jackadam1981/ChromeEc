...
{
	int rv, expect = (USB_PD_MUX_USB_ENABLED | USB_PD_MUX_DP_ENABLED |
			  USB_PD_MUX_HPD_LVL);
	...
	param.port = 0;
	rv = ec_cmd_usb_pd_mux_info(NULL, &param, &resp);

	zassert_equal(EC_RES_SUCCESS, rv, "ret: Got %d, expected %d", rv,
		      EC_RES_SUCCESS);
