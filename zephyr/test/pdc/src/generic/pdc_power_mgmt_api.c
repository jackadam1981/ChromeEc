...
ZTEST_USER(pdc_power_mgmt_api, test_connectionless_cmds)
{
	...
	zassert_false(pdc_power_mgmt_reset(TEST_PORT));

	emul_pdc_disconnect(emul);
	zassert_false(TEST_WAIT_FOR(pd_capable(TEST_PORT), PDC_TEST_TIMEOUT));

	LOG_INF("Sending GET INFO");
	zassert_false(pdc_power_mgmt_get_info(TEST_PORT, &pdc_info, true));

	LOG_INF("Sending GET PCH DATA_STATUS");
	zassert_false(pdc_power_mgmt_get_pch_data_status(TEST_PORT,
							 status.raw_value));

	LOG_INF("Sending GET LPM PPM INFO");
	zassert_false(
		pdc_power_mgmt_get_lpm_ppm_info(TEST_PORT, &lpm_ppm_info));
