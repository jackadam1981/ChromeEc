...
#define ASSUME_CONNECTED(port, timeout_ms) \
	ASSUME_FOR_TRUE(pdc_power_mgmt_is_connected(port), timeout_ms)

#define ASSUME_DISCONNECTED(port, timeout_ms) \
	ASSUME_FOR_FALSE(pdc_power_mgmt_is_connected(port), timeout_ms)
...
ZTEST_USER(pdc_power_mgmt_api, test_pd_get_data_role)
{
	...
	emul_pdc_configure_src(emul, &connector_status);
	emul_pdc_connect_partner(emul, &connector_status);
	ASSERT_FOR_TRUE(PD_ROLE_UFP ==
				pdc_power_mgmt_pd_get_data_role(TEST_PORT),
			1000);

	connector_status.conn_partner_type = UFP_ATTACHED;
	emul_pdc_configure_src(emul, &connector_status);
	emul_pdc_connect_partner(emul, &connector_status);
	ASSERT_FOR_TRUE(PD_ROLE_DFP ==
				pdc_power_mgmt_pd_get_data_role(TEST_PORT),
			1000);
}
...
ZTEST_USER(pdc_power_mgmt_api, test_pd_get_power_role)
{
	...
	emul_pdc_configure_src(emul, &connector_status);
	emul_pdc_connect_partner(emul, &connector_status);
	ASSERT_FOR_TRUE(PD_ROLE_SOURCE ==
				pdc_power_mgmt_get_power_role(TEST_PORT),
			1000);

	emul_pdc_configure_snk(emul, &connector_status);
	emul_pdc_connect_partner(emul, &connector_status);
	ASSERT_FOR_TRUE(
		PD_ROLE_SINK == pdc_power_mgmt_get_power_role(TEST_PORT), 1000);
}
...
ZTEST_USER(pdc_power_mgmt_api, test_pd_get_task_cc_state)
{
	...
	for (i = 0; i < ARRAY_SIZE(test); i++) {
		...
		emul_pdc_configure_src(emul, &connector_status);
		emul_pdc_connect_partner(emul, &connector_status);
		ASSERT_FOR_TRUE(test[i].out == pdc_power_mgmt_get_task_cc_state(
						       TEST_PORT),
				1000);
	}
}
...
ZTEST_USER(pdc_power_mgmt_api, test_pd_capable)
{
	...
	emul_pdc_disconnect(emul);
	ASSERT_FOR_FALSE(pdc_power_mgmt_pd_capable(TEST_PORT), 1000);
	...
	emul_pdc_connect_partner(emul, &connector_status);
	ASSERT_FOR_TRUE(pdc_power_mgmt_pd_capable(TEST_PORT), 2000);
}
...
ZTEST_USER(pdc_power_mgmt_api, test_get_partner_usb_comm_capable)
{
	...
	for (i = 0; i < ARRAY_SIZE(test); i++) {
		...
		emul_pdc_disconnect(emul);
		ASSUME_DISCONNECTED(TEST_PORT, 1000);
	}
}
...
ZTEST_USER(pdc_power_mgmt_api, test_get_info)
{
	...
	emul_pdc_configure_src(emul, &connector_status);
	emul_pdc_connect_partner(emul, &connector_status);
	ASSUME_CONNECTED(TEST_PORT, 1000);
...
ZTEST_USER(pdc_power_mgmt_api, test_request_power_swap)
{
	...
	for (i = 0; i < ARRAY_SIZE(test); i++) {
		...
		emul_pdc_connect_partner(emul, &connector_status);
		ASSUME_CONNECTED(TEST_PORT, 2000);
...
ZTEST_USER(pdc_power_mgmt_api, test_get_partner_unconstr_power)
{
	...
	ASSERT_FOR_FALSE(pdc_power_mgmt_get_partner_unconstr_power(TEST_PORT),
			 1000);
...
ZTEST_USER(pdc_power_mgmt_api, test_set_dual_role)
{
	...
	for (i = 0; i < ARRAY_SIZE(test); i++) {
		...
		emul_pdc_disconnect(emul);
		ASSUME_DISCONNECTED(TEST_PORT, 2000);
	}
}
