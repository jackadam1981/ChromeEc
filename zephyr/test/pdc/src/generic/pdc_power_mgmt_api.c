...
ZTEST_USER(pdc_power_mgmt_api, test_pd_power_button)
{
	...
	/* Check no simulated press on release ADO following a press */
	zassert_equal(
		1, pdc_power_mgmt_simulate_power_button_press_fake.call_count,
		"Unexpected PD power button press.");
	...
	/* Check for simulated press on release ADO without preceeding press ADO
	 */
	zassert_equal(
