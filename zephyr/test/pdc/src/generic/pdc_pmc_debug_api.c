...
ZTEST_USER(pdc_pmc_debug_api, test_data_connection)
{
	...
	zassert_true(status.usb2 ==
		     (connector_status.conn_partner_flags & 0x1));
	zassert_true(status.usb3_2 ==
		     (connector_status.conn_partner_flags & 0x1));
	zassert_true(status.dp ==
		     (connector_status.conn_partner_flags & 0x2 >> 1));
	zassert_true(status.usb4 ==
		     ((connector_status.conn_partner_flags & 0x4 >> 2) ||
		      (connector_status.conn_partner_flags & 0x8 >> 3)));
