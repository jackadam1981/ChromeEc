...
ZTEST_USER(pdc_trace_msg, test_fifo_ops)
{
	...
	zassert_ok(shell_execute_cmd(get_ec_shell(),
				     "pdctrace " STRINGIFY(TEST_PORT)),
		   "could not run pdctrace " STRINGIFY(TEST_PORT));
...
ZTEST_USER(pdc_trace_msg, test_console_cmd_syntax)
{
	...
	zassert_not_ok(
		shell_execute_cmd(get_ec_shell(), "pdctrace 0z"),
		"pdctrace should have rejected malformed port number \"0z\"");

	zassert_not_ok(
		shell_execute_cmd(get_ec_shell(), "pdctrace -1"),
		"pdctrace should have rejected invalid port number \"-1\"");

	snprintf(cmd_buf, sizeof(cmd_buf), "pdctrace %d",
		 EC_PDC_TRACE_MSG_PORT_NONE);
	zassert_not_ok(
		shell_execute_cmd(get_ec_shell(), cmd_buf),
		"pdctrace should have rejected reserved port number \"%d\"",
		EC_PDC_TRACE_MSG_PORT_NONE);
	...
	zassert_ok(shell_execute_cmd(get_ec_shell(), "pdctrace on"), NULL);
	...
	zassert_ok(shell_execute_cmd(get_ec_shell(), "pdctrace"),
		   "could not run pdctrace");

	zassert_ok(shell_execute_cmd(get_ec_shell(), "pdctrace off"), NULL);
}
