...
ZTEST(ucsi_ppm, test_unexpected_command)
{
	struct ucsi_control_t ctrl = {
		.command = UCSI_GET_CONNECTOR_STATUS,
		.data_length = 0,
		.command_specific = { 1, },
	};
	int rv;
...
ZTEST(ucsi_ppm, test_set_notification_enable)
{
	...
	rv = write_command(&enable_all_notifications);
	zassert_equal(rv, 0);
	zassert_true(wait_for_cmd_to_process());

	LOG_INF("Acking SET_NOTIFICATION_ENABLE");
	rv = write_ack_command(false, true);
	zassert_equal(rv, 0);
	zassert_true(wait_for_cmd_to_process());
}
