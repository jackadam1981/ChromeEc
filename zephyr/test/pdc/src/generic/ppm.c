...
ZTEST_USER_F(ppm_test, test_CCACK_support_simultaneous_ack_CC_and_CI)
{
	...
	zassert_equal(get_ppm_data(fixture)
		      ...
		      0);
	zassert_equal(get_ppm_data(fixture)->last_connector_changed, 0);
}
 ...
 */
ZTEST_F(ppm_test, test_lpm_error_accepts_new_command)
{
	initialize_fake_to_idle_notify(fixture);
	int notified_count = fixture->notified_count;
 ...
 */
ZTEST_F(ppm_test, test_ppm_reset_works_in_all_states)
{
	/* Test at IDLE_NOTIFY. */
	initialize_fake_to_idle_notify(fixture);
