...
ZTEST_USER(pdc_api, test_execute_ucsi_cmd_get_connector_status)
{
	...
	/* Clear the cache and expect the emulator's status. */
	pdc_rts54xx_clear_get_connector_status_cache();
	...
	/*
	 * Expect the command to return the previous status (from the cache)
	 * and to ignore the emulator's status.
	 */
	in.connect_status = 0;
	zassert_ok(emul_pdc_set_connector_status(emul, &in));
	zassert_ok(pdc_execute_ucsi_cmd(dev, UCSI_CMD_GET_CONNECTOR_STATUS, 0,
