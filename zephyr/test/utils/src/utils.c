...
/* task_wait_event works only with the shimed task list, which is hardcoded.
 * The wait_event like task synchronisation functions are covered by Zephyr
 * tests. Add for compability.
 */
...
ZTEST(utils, test_shared_mem)
{
	...
	for (i = 0; i < 256; ++i) {
		...
		if ((i & 0xf) == 0) {
			ccprintf("Yieling after %d iterations\n", i + 1);
...
ZTEST(utils, test_bytes_are_trivial)
{
	...
	zassert_true(!bytes_are_trivial(nontrivial1, sizeof(nontrivial1)));
	zassert_true(!bytes_are_trivial(nontrivial2, sizeof(nontrivial2)));
	zassert_true(!bytes_are_trivial(nontrivial3, sizeof(nontrivial3)));
	zassert_true(!bytes_are_trivial(nontrivial4, sizeof(nontrivial4)));
}
