...
FAKE_VALUE_FUNC(enum ec_error_list, charger_get_minimum_charging_mv, int,
		uint32_t *);

#define DEFAULT_VOLTAGE 15000
static uint32_t test_voltage = DEFAULT_VOLTAGE;

enum ec_error_list fake_get_minimum_charging_mv(int charger, uint32_t *voltage)
{
...
{
	RESET_FAKE(charger_get_minimum_charging_mv);
...
ZTEST(charge_state, test_sufficient_charger)
{
	...
	charger_get_minimum_charging_mv_fake.custom_fake =
		fake_get_minimum_charging_mv;
