...
ZTEST(uart_printf, test_uart_put_success)
{
	...
	zassert_ok(uart_put(test_string, ARRAY_SIZE(test_string)));
	zassert_equal(ARRAY_SIZE(test_string),
		      uart_tx_char_raw_fake.call_count);

	/* Copy the history so it's easier to assert */
	for (int i : std::views::iota(0, (int)ARRAY_SIZE(test_string))) {
		output_string += uart_tx_char_raw_fake.arg1_history[i];
	}

	/* Verify that the string was passed to uart_tx_char_raw() */
	zassert_mem_equal(test_string, output_string.c_str(),
			  ARRAY_SIZE(test_string));
}
...
ZTEST(uart_printf, test_uart_put_fail_tx)
{
	...
	zassert_equal(EC_ERROR_OVERFLOW, uart_put(test_string, 1));
...
ZTEST(uart_printf, test_uart_put_raw_fail_tx)
{
	...
	/* Try printing the newline */
	zassert_equal(EC_ERROR_OVERFLOW, uart_put_raw(test_string, 1));
	zassert_equal(1, uart_tx_char_raw_fake.call_count);
