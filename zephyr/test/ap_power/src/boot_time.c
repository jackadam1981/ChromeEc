...
ZTEST(boot_time_no_tasks, test_boot_time_set)
{
	...
	on_new_state(AP_POWER_STATE_G3);

	zassert_ok(get_boot_time(AP_POWER_STATE_S5, &time_S5));
	zassert_ok(get_boot_time(AP_POWER_STATE_S0, &time_S0));
	...
	on_new_state(AP_POWER_STATE_S5);
	...
	on_new_state(AP_POWER_STATE_S0);
...
ZTEST_USER(boot_time, test_boot_time_console_cmd)
{
	...
	[00:00:24.764,900] <inf> ap_pwrseq: first S5: 1000ms
	*/
	SCAN_CONSOLE_CMD("boottime S5", EC_SUCCESS, 1,
			 "%*[^f]f%*[^i]irst S5: %lldms", &time_S5);
	SCAN_CONSOLE_CMD("boottime S0", EC_SUCCESS, 1,
			 "%*[^f]f%*[^i]irst S0: %lldms", &time_S0);
	zassert_not_equal(time_S5, -1);
	zassert_not_equal(time_S0, -1);
	zassert_true(time_S5 <= time_S0);
}

ZTEST_SUITE(boot_time_no_tasks, ap_power_predicate_pre_main, NULL,
	    time_mock_after, NULL, NULL);
ZTEST_SUITE(boot_time, ap_power_predicate_post_main, NULL, NULL, NULL, NULL);
