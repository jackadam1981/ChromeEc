/* Copyright 2023 The ChromiumOS Authors
 * Use of this source code is governed by a BSD-style license that can be
 * found in the LICENSE file.
 */
#include <gpio.h>
#include <usbc_ppc.h>

#include <zephyr/drivers/gpio/gpio_emul.h>
#include <zephyr/fff.h>
#include <zephyr/ztest.h>

DECLARE_FAKE_VOID_FUNC(aoz1380_interrupt, int);
DEFINE_FAKE_VOID_FUNC(aoz1380_interrupt, int);
DECLARE_FAKE_VOID_FUNC(nx20p348x_interrupt, int);
DEFINE_FAKE_VOID_FUNC(nx20p348x_interrupt, int);

static int aoz1380_interrupt_arg;
static int nx20p348x_interrupt_arg;

static void aoz1380_interrupt_mock(int port)
{
	aoz1380_interrupt_arg = port;
}

static void nx20p348x_interrupt_mock(int port)
{
	nx20p348x_interrupt_arg = port;
}

static void ppc_config_before(void *fixture)
{
	ARG_UNUSED(fixture);
	RESET_FAKE(aoz1380_interrupt);
	RESET_FAKE(nx20p348x_interrupt);

	aoz1380_interrupt_fake.custom_fake = aoz1380_interrupt_mock;
	nx20p348x_interrupt_fake.custom_fake = nx20p348x_interrupt_mock;
}

void ppc_interrupt(enum gpio_signal signal);
int board_aoz1380_set_vbus_source_current_limit(int port,
						enum tcpc_rp_value rp);

ZTEST_SUITE(ppc_config, NULL, NULL, ppc_config_before, NULL, NULL);

ZTEST(ppc_config, board_aoz1380_set_vbus_source_current_limit)
{
	const struct gpio_dt_spec *gpio =
		GPIO_DT_FROM_NODELABEL(ioex_usb_c0_ilim_3a_en);

	/*
	 * board_aoz1380_set_vbus_source_current_limit should set
	 * ioex_usb_c0_ilim_3a_en to 1 for 3A, 0 otherwise.
	 */
	zassert_equal(
		board_aoz1380_set_vbus_source_current_limit(0, TYPEC_RP_3A0),
		EC_SUCCESS
	);
	zassert_equal(gpio_emul_output_get(gpio->port, gpio->pin), 1);

	zassert_equal(
		board_aoz1380_set_vbus_source_current_limit(0, TYPEC_RP_1A5),
		EC_SUCCESS
	);
	zassert_equal(gpio_emul_output_get(gpio->port, gpio->pin), 0);

	zassert_equal(
		board_aoz1380_set_vbus_source_current_limit(1, TYPEC_RP_3A0),
		EC_SUCCESS
	);
	zassert_equal(gpio_emul_output_get(gpio->port, gpio->pin), 1);

	zassert_equal(
		board_aoz1380_set_vbus_source_current_limit(1, TYPEC_RP_1A5),
		EC_SUCCESS
	);
	zassert_equal(gpio_emul_output_get(gpio->port, gpio->pin), 0);
}

ZTEST(ppc_config, ppc_interrupt)
{
	ppc_interrupt(GPIO_USB_C0_PPC_INT_ODL);
	zassert_equal(aoz1380_interrupt_arg, 0);
	zassert_equal(aoz1380_interrupt_fake.call_count, 1);

	ppc_interrupt(GPIO_USB_C1_PPC_INT_ODL);
	zassert_equal(nx20p348x_interrupt_arg, 1);
	zassert_equal(nx20p348x_interrupt_fake.call_count, 1);
}
