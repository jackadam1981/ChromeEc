...
static void *tablet_setup(void)
{
	...
	zassert_ok(cbi_set_fw_config(FW_FF_CONVERTIBLE << 0), NULL);
	/* Run init hooks to initialize cbi. */
	hook_notify(HOOK_INIT);

	/* Check if CBI write worked. */
	zassert_ok(cros_cbi_get_fw_config(FW_FORM_FACTOR, &val), NULL);
	zassert_equal(FW_FF_CONVERTIBLE, val, "val=%d", val);

	return NULL;
}

ZTEST_SUITE(skyrim_convertible, NULL, tablet_setup, NULL, NULL, NULL);

ZTEST(skyrim_convertible, test_gmr_tablet_switch_enabled)
{
	const struct device *tablet_mode_gpio =
		DEVICE_DT_GET(DT_GPIO_CTLR(DT_NODELABEL(tablet_mode_l), gpios));
	const gpio_port_pins_t tablet_mode_pin =
...
ZTEST(skyrim_convertible, test_base_imu_irq_enabled)
{
