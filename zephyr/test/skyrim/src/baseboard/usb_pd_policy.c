...
/* Structs and functions used to validate function call arguments. */
struct port_enable_args {
	int port;
	int enable;
};

static struct port_enable_args ppc_vbus_source_enable_args;
static struct port_enable_args pd_set_vbus_discharge_args;
static struct port_enable_args ppc_vbus_sink_enable_args;
static int tcpm_get_src_ctrl_arg;

static void pd_set_vbus_discharge_mock(int port, int enable)
{
	pd_set_vbus_discharge_args.port = port;
	pd_set_vbus_discharge_args.enable = enable;
}

static bool tcpm_get_src_ctrl_mock_false(int port)
{
	tcpm_get_src_ctrl_arg = port;
	return false;
}

static bool tcpm_get_src_ctrl_mock_true(int port)
{
	tcpm_get_src_ctrl_arg = port;
	return true;
}

static int ppc_vbus_source_enable_mock_success(int port, int enable)
{
	ppc_vbus_source_enable_args.port = port;
	ppc_vbus_source_enable_args.enable = enable;
	return EC_SUCCESS;
}

static int ppc_vbus_sink_enable_mock_success(int port, int enable)
{
	ppc_vbus_sink_enable_args.port = port;
	ppc_vbus_sink_enable_args.enable = enable;
	return EC_SUCCESS;
}

static int ppc_vbus_source_enable_mock_failure(int port, int enable)
{
	ppc_vbus_source_enable_args.port = port;
	ppc_vbus_source_enable_args.enable = enable;
...
static int ppc_vbus_sink_enable_mock_failure(int port, int enable)
{
	ppc_vbus_sink_enable_args.port = port;
	ppc_vbus_sink_enable_args.enable = enable;
	return EC_ERROR_INVAL;
...
{
	ppc_vbus_source_enable_fake.custom_fake =
		ppc_vbus_source_enable_mock_success;
	ppc_vbus_sink_enable_fake.custom_fake =
		ppc_vbus_sink_enable_mock_success;

	pd_power_supply_reset(0);

	zassert_equal(ppc_vbus_source_enable_fake.call_count, 1);
	zassert_equal(ppc_vbus_source_enable_args.port, 0);
	zassert_equal(ppc_vbus_source_enable_args.enable, 0);

	if (IS_ENABLED(CONFIG_USB_PD_DISCHARGE)) {
		zassert_equal(pd_set_vbus_discharge_fake.call_count, 1);
		zassert_equal(pd_set_vbus_discharge_args.port, 0);
		zassert_equal(pd_set_vbus_discharge_args.enable, 1);
	}
...
ZTEST(usb_pd_policy, pd_power_supply_reset_c1_success)
{
	...
	zassert_equal(ppc_vbus_source_enable_fake.call_count, 1);
	zassert_equal(ppc_vbus_source_enable_args.port, 1);
	zassert_equal(ppc_vbus_source_enable_args.enable, 0);

	if (IS_ENABLED(CONFIG_USB_PD_DISCHARGE)) {
		zassert_equal(pd_set_vbus_discharge_fake.call_count, 1);
		zassert_equal(pd_set_vbus_discharge_args.port, 1);
		zassert_equal(pd_set_vbus_discharge_args.enable, 1);
	}
...
ZTEST(usb_pd_policy, pd_set_power_supply_ready_c0_success)
{
	...
	zassert_equal(ppc_vbus_sink_enable_args.port, 0);
	zassert_equal(ppc_vbus_sink_enable_args.enable, 0);

	if (IS_ENABLED(CONFIG_USB_PD_DISCHARGE)) {
		zassert_equal(pd_set_vbus_discharge_fake.call_count, 1);
		zassert_equal(pd_set_vbus_discharge_args.port, 0);
		zassert_equal(pd_set_vbus_discharge_args.enable, 0);
	}

	zassert_equal(ppc_vbus_source_enable_fake.call_count, 1);
	zassert_equal(ppc_vbus_source_enable_args.port, 0);
	zassert_equal(ppc_vbus_source_enable_args.enable, 1);
...
ZTEST(usb_pd_policy, pd_set_power_supply_ready_c1_success)
{
	...
	zassert_equal(ppc_vbus_sink_enable_fake.call_count, 1);
	zassert_equal(ppc_vbus_sink_enable_args.port, 1);
	zassert_equal(ppc_vbus_sink_enable_args.enable, 0);
...
ZTEST(usb_pd_policy, pd_set_power_supply_ready_c0_failure)
{
	...
	ppc_vbus_sink_enable_fake.custom_fake =
		ppc_vbus_sink_enable_mock_failure;
	...
	ppc_vbus_sink_enable_fake.custom_fake =
		ppc_vbus_sink_enable_mock_success;
	ppc_vbus_source_enable_fake.custom_fake =
		ppc_vbus_source_enable_mock_failure;
	zassert_not_equal(pd_set_power_supply_ready(0), EC_SUCCESS);
	zassert_equal(ppc_vbus_source_enable_fake.call_count, 1);
	zassert_equal(ppc_vbus_source_enable_args.port, 0);
	zassert_equal(ppc_vbus_source_enable_args.enable, 1);
...
{
	tcpm_get_src_ctrl_fake.custom_fake = tcpm_get_src_ctrl_mock_true;
	zassert_true(board_is_sourcing_vbus(0));
	zassert_equal(tcpm_get_src_ctrl_fake.call_count, 1);
	zassert_equal(tcpm_get_src_ctrl_arg, 0);
}

ZTEST(usb_pd_policy, board_is_sourcing_vbus_c0_false)
{
	tcpm_get_src_ctrl_fake.custom_fake = tcpm_get_src_ctrl_mock_false;
...
ZTEST(usb_pd_policy, board_is_sourcing_vbus_c1_true)
{
	...
	zassert_equal(tcpm_get_src_ctrl_fake.call_count, 1);
	zassert_equal(tcpm_get_src_ctrl_arg, 1);
}
...
static void usb_pd_policy_before(void *fixture)
{
	...
	pd_set_vbus_discharge_fake.custom_fake = pd_set_vbus_discharge_mock;
}
