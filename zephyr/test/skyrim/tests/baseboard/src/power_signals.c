...
FAKE_VOID_FUNC(test_interrupt_handler, enum gpio_signal);
...
static int test_interrupt(const struct gpio_dt_spec *dt)
{
	int rv;
	int old_count = test_interrupt_handler_fake.call_count;
	...
	return test_interrupt_handler_fake.call_count <= old_count;
}
