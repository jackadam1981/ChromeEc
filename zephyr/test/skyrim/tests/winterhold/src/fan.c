...
static void fan_tick(void)
{
	...
	/* Clamp rpm_diff. This essentially emulates fan intertia. */
	if (rpm_diff > 0)
...
static void fan_test_init(void *fixture)
{
	ARG_UNUSED(fixture);
	 ...
	 * The problem is that we don't wan't to pull the entire fan framweork
	 * for this test. Instaed initialize it here.
	 */
...
/* Check whether we can ramp up into 5600 in 5s */
ZTEST(fan, fan_max_rpm)
{
	enum fan_status status;
	int deviation;

	fan_data[0].rpm_target = 5600;
	...
	deviation = rpm.rpm_deviation * 5600 / 100;

	zassert_within(fan_data[0].rpm_actual, 5600, deviation);
}
...
ZTEST(fan, fan_frustrated_max)
{
	...
	/* 600 ticks should be more than enough. */
	for (i = 0; i < 600; i++) {
		status = board_override_fan_control_duty(0);
...
ZTEST_SUITE(fan, NULL, NULL, fan_test_init, NULL, NULL);
