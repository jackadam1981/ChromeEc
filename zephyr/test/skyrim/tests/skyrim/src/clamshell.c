...
static void *clamshell_setup(void)
{
	...
	/* Set CBI form factor to CLAMSHELL. */
	zassert_ok(cbi_set_fw_config(FW_FF_CLAMSHELL << 0), NULL);
	/* Run init hooks to initialize cbi. */
	hook_notify(HOOK_INIT);

	/* Check if CBI write worked. */
	zassert_ok(cros_cbi_get_fw_config(FW_FORM_FACTOR, &val), NULL);
	zassert_equal(FW_FF_CLAMSHELL, val, "val=%d", val);

	return NULL;
}

ZTEST_SUITE(skyrim_clamshell, NULL, clamshell_setup, NULL, NULL, NULL);

ZTEST(skyrim_clamshell, test_gmr_tablet_switch_disabled)
{
	const struct device *tablet_mode_gpio =
		DEVICE_DT_GET(DT_GPIO_CTLR(DT_NODELABEL(tablet_mode_l), gpios));
	const gpio_port_pins_t tablet_mode_pin =
...
ZTEST(skyrim_clamshell, test_base_imu_irq_disabled)
{
