...
ZTEST(rollback_entropy, test_add_entropy)
{
	...
	/* As opposite to CrosEC, both rollback regions are always inited in
	 * Zephyr, not only the first one. rollback_initial_data structure
	 * defines the initial values. It is inited as a part of the firmware
	 * image, see binman node for more details.
	 */
	for (int i = 0; i <= 1; i++) {
