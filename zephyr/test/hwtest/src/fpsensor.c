...
static void vailidate_fp_buffer_offset(void)
{
	zassert_equal(validate_fp_buffer_offset(1, 0, 1), EC_SUCCESS);
	zassert_equal(validate_fp_buffer_offset(1, 1, 1), EC_ERROR_INVAL);
	zassert_equal(validate_fp_buffer_offset(1, UINT32_MAX, 1), EC_ERROR_OVERFLOW);
}

ZTEST_SUITE(fpsensor_spi, NULL, fpsensor_setup_spi, NULL, NULL, NULL);

/* SPI is a default HC backend. Issue a EC_CMD_GET_PROTOCOL_INFO command via
 * dummy backend to get protocol_info for SPI.
 */
ZTEST(fpsensor_spi, test_fpsensor_spi)
{
	const struct ec_response_get_protocol_info *expected = &expected_info[FP_TRANSPORT_TYPE_SPI];
	...
	zassert_equal(info->max_request_packet_size, expected->max_request_packet_size);
	zassert_equal(info->max_response_packet_size, expected->max_response_packet_size);
	zassert_equal(info->protocol_versions, expected->protocol_versions);
}

ZTEST_SUITE(fpsensor_uart, NULL, NULL, NULL, NULL, NULL);

/* UART uses the same EC_CMD_GET_PROTOCOL_INFO HC handler as SPI. If the
 * output from the command if correct for SPI, it should be correct for UART as well.
 * It would be hard to replace already initialized HC backed (SPI).
 */
ZTEST(fpsensor_uart, test_fpsensor_uart)
{
	const struct ec_response_get_protocol_info *expected = &expected_info[FP_TRANSPORT_TYPE_UART];
	...
	backend_uart = ec_host_cmd_backend_get_uart(DEVICE_DT_GET(DT_CHOSEN(zephyr_host_cmd_uart_backend)));
	rx_ctx.buf = &rx_buf;
