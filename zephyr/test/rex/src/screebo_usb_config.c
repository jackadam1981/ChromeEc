...
struct tcpc_config_t tcpc_config[2];
...
{
	/* Fake gpio for pd mcu reset test */
	const struct gpio_dt_spec *fake_gpio =
		GPIO_DT_FROM_NODELABEL(ioex_usb_c0_rt_rst_ls_l);
	tcpc_config[1].rst_gpio = *fake_gpio;
