...
ZTEST_USER(i2c_policy, test_we_got_called)
{
	zassert_false(false);
}
...
ZTEST_USER(i2c_policy, test_allow_hb)
{
	...
	zassert_equal(board_allow_i2c_passthru(&cmd_desc_hb0), true);
	zassert_equal(board_allow_i2c_passthru(&cmd_desc_hb1), false);

	cros_cbi_get_fw_config_fake.custom_fake =
		mock_cros_cbi_get_fw_config_hb;

	zassert_equal(board_allow_i2c_passthru(&cmd_desc_hb1), true);
}

ZTEST_USER(i2c_policy, test_allow_usb3_only)
{
	/* Some implausible I2C target */
	const struct i2c_cmd_desc_t cmd_desc_99 = {
		.port = 99,
