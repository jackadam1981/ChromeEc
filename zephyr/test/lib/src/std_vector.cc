...
ZTEST(std_vector, test_stack_init_elements)
{
	...
	zassert_equal(static_cast<int32_t>(vec.size()), 5, "%d");
	zassert_equal(vec[0], 10, "%d");
	zassert_equal(vec[1], 11, "%d");
	zassert_equal(vec[2], 12, "%d");
	zassert_equal(vec[3], 13, "%d");
	zassert_equal(vec[4], 14, "%d");
}
...
ZTEST(std_vector, test_static_init_elements)
{
	...
	zassert_equal(static_cast<int32_t>(vec.size()), 5, "%d");
	zassert_equal(vec[0], 20, "%d");
	zassert_equal(vec[1], 21, "%d");
	zassert_equal(vec[2], 22, "%d");
	zassert_equal(vec[3], 23, "%d");
	zassert_equal(vec[4], 24, "%d");
}

std::vector<int32_t> global_vec{ 30, 31, 32, 33, 34 };
ZTEST(std_vector, test_global_init_elements)
{
	zassert_equal(static_cast<int32_t>(global_vec.size()), 5, "%d");
	zassert_equal(global_vec[0], 30, "%d");
	zassert_equal(global_vec[1], 31, "%d");
	zassert_equal(global_vec[2], 32, "%d");
	zassert_equal(global_vec[3], 33, "%d");
	zassert_equal(global_vec[4], 34, "%d");
}
...
ZTEST(std_vector, test_push_back_elements)
{
	...
	zassert_equal(static_cast<int32_t>(vec.size()), 4, "%d");
	zassert_equal(vec[0], 0, "%d");
	zassert_equal(vec[1], 1, "%d");
	zassert_equal(vec[2], 2, "%d");
	zassert_equal(vec[3], 3, "%d");
}
...
ZTEST(std_vector, test_fill_one_vector)
{
	...
	zassert_equal(static_cast<int>(vec.size()), num_elements, "%d");
	for (int i = 0; i < num_elements; ++i) {
		zassert_true(vec[i] == i);
