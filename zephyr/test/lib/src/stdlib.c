...
ZTEST(stdlib, test_memmove)
{
	...
	if (!IS_ENABLED(CONFIG_ARCH_POSIX)) {
		/* Do not compare time for aligned and unligned access, because
		 * it is required that the aligned access has to be faster.
		 */
		ccprintf("Unaligend memmove: %" PRId64 " us\n",
			 t1.val - t0.val);
		ccprintf("Aligend memmove  : %" PRId64 " us\n",
			 t3.val - t2.val);
	}
...
ZTEST(stdlib, test_memmove_overlap)
{
	...
	memmove(buf + 101, buf, len); /* unaligned */
	zassert_mem_equal(buf + 101, cmp_buf, len);
	...
	for (i = len; i < buf_size; ++i)
		buf[i] = 0;
	memmove(buf + 100, buf, len); /* aligned */
	zassert_mem_equal(buf + 100, cmp_buf, len);
...
ZTEST(stdlib, test_memcpy)
{
	...
	if (!IS_ENABLED(CONFIG_ARCH_POSIX)) {
		 ...
		 * it is required that the aligned access has to be faster.
		 */
		ccprintf("Unaligend memcpy: %" PRId64 " us\n", t1.val - t0.val);
		ccprintf("Aligend memcpy  : %" PRId64 " us\n", t3.val - t2.val);
	}
