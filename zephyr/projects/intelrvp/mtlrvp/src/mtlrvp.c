...
#include "driver/tcpm/nct38xx.ha"
...
void board_reset_pd_mcu(void)
{
	...
	nct38xx_tcpm_drv.release(USBC_PORT_C0);
	nct38xx_tcpm_drv.release(USBC_PORT_C1);
	nct38xx_tcpm_drv.init(USBC_PORT_C0);
	nct38xx_tcpm_drv.init(USBC_PORT_C1);
...
#if defined(HAS_TASK_PD_C2)
	[USBC_PORT_C2] = {
		.retimer_rst_gpio = GPIO_SIGNAL(DT_NODELABEL(usb_c2_hb_retimer_rst)),
		.usb_ls_en_gpio = GPIO_SIGNAL(DT_NODELABEL(usb_c2_hb_retimer_ls_en)),
	},
#endif
#if defined(HAS_TASK_PD_C3)
	[USBC_PORT_C3] = {
		.retimer_rst_gpio = GPIO_SIGNAL(DT_NODELABEL(usb_c3_hb_retimer_rst)),
		.usb_ls_en_gpio = GPIO_SIGNAL(DT_NODELABEL(usb_c3_hb_retimer_ls_en)),
	},
#endif
};
