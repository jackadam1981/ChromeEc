# Bare-bones Example Zephyr EC Project
...
whole project directory with a `BUILD.py` and all, and this bare-bones
example is likely not of use to you.  Check out the [project config
...
``` shellsession
(chroot) $ zmake build barebone-posix
```

To build the NPCX9 example, run:

``` shellsession
(chroot) $ zmake build barebone-npcx9
```

For the IT8XXX2 example, run:

``` shellsession
(chroot) $ zmake build barebone-it8xxx2
```
