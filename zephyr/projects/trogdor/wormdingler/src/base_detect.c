...
static void base_detect_change(enum base_status status)
{
	...
	if (current_base_status == status)
		return;
	gpio_pin_set_dt(GPIO_DT_FROM_NODELABEL(gpio_en_base), connected);
...
static void base_detect_deferred(void)
{
	...
	adc_channels[ADC_BASE_DET].input_ch = 5;
	v = adc_read_channel(ADC_BASE_DET);
	if (v == ADC_READ_ERROR)
		return;
	adc_channels[ADC_BASE_DET].input_ch = 6;
...
{
	return gpio_pin_get_dt(GPIO_DT_FROM_NODELABEL(gpio_base_det_l)) == 0;
}
...
static void base_enable(void)
{
	...
	gpio_enable_dt_interrupt(
			GPIO_INT_FROM_NODELABEL(int_base_det));
}
