...
void board_chipset_pre_init(void)
{
	...
	gpio_pin_configure_dt(GPIO_DT_FROM_NODELABEL(gpio_en_pp3300_a), 1);

	/* Turn on the 5V rail. */
#ifdef CONFIG_POWER_PP5000_CONTROL
	power_5v_enable(task_get_current(), 1);
#else /* !defined(CONFIG_POWER_PP5000_CONTROL) */
	gpio_pin_configure_dt(GPIO_DT_FROM_NODELABEL(gpio_en_pp5000_a), 1);
...
#else /* !defined(CONFIG_POWER_PP5000_CONTROL) */
	gpio_pin_configure_dt(GPIO_DT_FROM_NODELABEL(gpio_en_pp5000_a), 0);
#endif /* defined(CONFIG_POWER_PP5000_CONTROL) */

	/* Turn off the 3.3V and 5V rails. */
	gpio_pin_configure_dt(GPIO_DT_FROM_NODELABEL(gpio_en_pp3300_a), 0);
}
