 ...
 __pd_ram_code int hpd_pulse(int argc, char **argv)
{
	...
	int_num_temp = int_num - int_num_temp;
	//irq_unlock(key);

	ccprintf("Time elaspe: %d us, INT[%d] = {%d, %d, %d...}\n", dticks_temp, int_num_temp, int_list[0], int_list[1], int_list[2]);
	//ccprintf("Time elaspe: %d us\n", (dticks - dticks_temp)*30); //57n0, 660~690us
