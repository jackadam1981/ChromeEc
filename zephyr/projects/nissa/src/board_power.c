...
LOG_MODULE_REGISTER(common_pwrseq, LOG_LEVEL_DBG);
...
int generate_pch_pwrok_handler(int delay)
{
	...
	if (power_signal_get(PWR_PCH_PWROK) == 0) {
		k_msleep(delay);
		LOG_INF("Turning on PWR_PCH_PWROK");
...
DECLARE_HOOK(HOOK_INIT, pwrseq_init, HOOK_PRIO_DEFAULT);
#endif
