...
/*
 * Temporary for prototype form factor support.
 */
...
static int form_factor_init(const struct device *unused)
{
	 ...
	 * rotation matrix.
	 */
	ret = cros_cbi_get_fw_config(FW_SCREEN_INVERSION, &val);
	if (ret != 0) {
		LOG_WRN("Error retrieving CBI FW_CONFIG field %d",
			FW_SCREEN_INVERSION);
		return 0;
	}
	if (val == FW_SCREEN_INVERTED) {
		LOG_INF("Switching to inverted screen");
		motion_sensors[LID_SENSOR].rot_standard_ref = &ALT_MAT;
	}
	return 0;
}

SYS_INIT(form_factor_init, APPLICATION, HOOK_PRIO_DEFAULT);
