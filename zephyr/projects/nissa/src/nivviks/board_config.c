...
static void nivviks_subboard_init(void)
{
	...
	if (sb != NISSA_SB_C_A && sb != NISSA_SB_HDMI_A) {
		...
		gpio_pin_configure_dt(DT_GPIO_SUB_USB_A1_ILIMIT_SDP,
				      GPIO_DISCONNECTED);
		gpio_pin_configure_dt(DT_GPIO_EN_SUB_USB_A1_VBUS,
				      GPIO_DISCONNECTED);
	}
	if (sb == NISSA_SB_C_A || sb == NISSA_SB_C_LTE) {
		/* Enable type-C port 1 */
		gpio_reset(GPIO_SIGNAL(DT_NODELABEL(gpio_usb_c1_int_odl)));
	} else {
	...
	if (sb == NISSA_SB_HDMI_A) {
		...
		/* Enable HDMI GPIOs */
		gpio_reset(GPIO_SIGNAL(DT_NODELABEL(gpio_en_sub_rails_odl)));
		gpio_reset(GPIO_SIGNAL(DT_NODELABEL(gpio_hdmi_en_sub_odl)));
		/* Configure the interrupt separately */
		gpio_reset(GPIO_SIGNAL(DT_NODELABEL(gpio_hpd_sub_odl)));
	}
}
