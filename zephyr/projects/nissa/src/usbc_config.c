...
uint16_t tcpc_get_alert_status(void)
{
	...
	/* TODO(b:211693800) ignore C1 interrupts if port is not present. */
 ...
 */
#define USBC_INT_POLL_FUNC(port) poll_c ## port ## _int
#define USBC_INT_POLL_DATA(port) USBC_INT_POLL_FUNC(port) ## _data
#define USBC_INT_POLL(port)						\
	static void USBC_INT_POLL_FUNC(port) (void);			\
	DECLARE_DEFERRED(USBC_INT_POLL_FUNC(port));			\
	static void USBC_INT_POLL_FUNC(port) (void)			\
	{								\
		if (!gpio_get_level(GPIO_USB_C ## port ## _INT_ODL)) {	\
...
/* TODO interrupts must be enabled on init */
void usb_c0_interrupt(enum gpio_signal gpio)
{
	 ...
	 */
	hook_call_deferred(&USBC_INT_POLL_FUNC_DATA(0), -1);
	/* Trigger polling of TCPC and BC1.2 in respective tasks */
	usbc_interrupt_trigger(0);
	/* Check for lost interrupts in a bit */
	hook_call_deferred(&USBC_INT_POLL_FUNC_DATA(0), USBC_INT_POLL_DELAY_US);
}

void usb_c1_interrupt(enum gpio_signal gpio)
{
	hook_call_deferred(&USBC_INT_POLL_FUNC_DATA(1), -1);
	usbc_interrupt_trigger(1);
	hook_call_deferred(&USBC_INT_POLL_FUNC_DATA(1), USBC_INT_POLL_DELAY_US);
}
