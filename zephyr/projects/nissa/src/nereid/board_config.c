			       ...
			       struct ap_power_ev_data data)
{
	...
	const struct gpio_dt_spec *ddc_select = GPIO_DT_FROM_NODELABEL(gpio_hdmi_sel);
...
static void nereid_subboard_init(void)
{
	...
	if (sb == NISSA_SB_HDMI_A) {
		...
		/* Configure HPD input GPIO */
		gpio_pin_configure_dt(hpd_gpio, 0);
		/* Register interrupt handler for HPD changes */
		gpio_init_callback(&hdmi_hpd_cb, hdmi_hpd_interrupt, BIT(hpd_gpio->pin));
