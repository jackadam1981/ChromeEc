 ...
 *
 * The first deals with the LED handlers that drive * the LED hardware
 * via PWM or GPIOs, and defined via the cros_ec_multi_pwm_leds and
 * cros_ec_gpio_leds compats.
 * A handler is the driver for the one or more LEDs that are managed
 * as a unit. Each handler has a type, such as a PWM handler or
 * a GPIO handler.  The handlers are all identified using a single enum
 * or index.  This index is used to lookup a table that maps the index
 * to a particular handler type (PWM or GPIO), and a type specific
 * index that is used to lookup the instance of that type.
 *
 * For instance, if configuration exists for a PWM handler and
 * 2 GPIO handlers, the handler index will be (0-2), and this index
 * will map to separate indices for each type (GPIO index 0-1, and
 * PWM index 0).
 *
 * Typically on a chromebook there will not be a mixture of PWM and GPIO
 * handlers, but the cost to support both is low.
 *
 * TODO: Complete GPIO implementation.
 *
 * The second is the policy portion, configured using the cros_ec_led_policy
 * compat, that maps the inputs to the LED behaviour and actions.
 * These inputs comprise of the AP state, the charger state, and the
 * battery state.
 *
 * The intent is that the first part defines the behaviour and actions of
 * the LEDs (such as colour, brightness, flashing etc.), and the second
...
static void set_pwm_led_values(const struct led_multi_pwm *pwm,
			       const uint8_t *colors)
{
#define	PWM_PERIOD	(10*1000)	/* 10 ms period */
	...
	for (int i = 0; i < pwm->pwm_count; i++, pl++, colors++) {
		 ...
		 */
		uint32_t pulse = *colors * (PWM_PERIOD / 100);

		pwm_pin_set_usec(pl->pwm, pl->chan, PWM_PERIOD,
				 pulse, pl->flags);
	}
}
 ...
 */
static void pwm_set_led_action(enum led_pwm_hand h, int action)
{
 ...
 */
static void pwm_led_shutdown(enum led_pwm_hand h)
{
	struct led_multi_pwm_state *state = &pwm_state[h];
