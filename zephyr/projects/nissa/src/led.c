...
{								\
	...
	.alt_colors = DT_PROP(id, alt_colors),			\
},
...
static void pwm_led_timer(struct k_timer *timer)
{
	...
	if (state->current_state) {
		...
		k_timer_start(&state->timer, K_MSEC(ap->blink[1]),
			      K_FOREVER);
		set_pwm_led_values(pwm, ap->alt_colors);
	}
	printk("timer: state %d, ticks %lld\n", state->current_state,
			k_timer_remaining_ticks(&state->timer));
}
