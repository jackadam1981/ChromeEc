...
__override void led_set_color_battery(enum ec_led_colors color)
{
	...
	led1_duty = led2_duty = led3_duty = led4_duty = BAT_LED_ON_LVL;

	ccprints("will tsai battery color = %d", color);
	...
	if (led_auto_control_is_enabled(EC_LED_ID_BATTERY_LED)) {
		...
		case LED_SIDESEL_MB_PORT:
			...
			case EC_LED_COLOR_AMBER:
				led1_duty = BAT_LED_OFF_LVL;
				break;
			case EC_LED_COLOR_WHITE:
				led2_duty = BAT_LED_OFF_LVL;
				break;
		...
		case LED_SIDESEL_DB_PORT:
			switch (color) {
			case EC_LED_COLOR_AMBER:
				led3_duty = BAT_LED_OFF_LVL;
				break;
			case EC_LED_COLOR_WHITE:
				led4_duty = BAT_LED_OFF_LVL;
				break;
			default: /* LED_OFF and other unsupported colors */
				break;
			}
	...
	gpio_pin_set_dt(
			GPIO_DT_FROM_NODELABEL(gpio_pwm_led_1_odl),
			led1_duty);
	gpio_pin_set_dt(
			GPIO_DT_FROM_NODELABEL(gpio_pwm_led_2_odl),
			led2_duty);
	gpio_pin_set_dt(
			GPIO_DT_FROM_NODELABEL(gpio_led_3_l),
			led3_duty);
	gpio_pin_set_dt(
			GPIO_DT_FROM_NODELABEL(gpio_led_4_l),
			led4_duty);
}

__override void led_set_color_power(enum ec_led_colors color)
{
	ccprints("will tsai power color = %d", color);
	if (color == EC_LED_COLOR_WHITE)
		gpio_pin_set_dt(
			GPIO_DT_FROM_NODELABEL(gpio_power_led_gate),
			PWR_LED_ON_LVL);
	else
