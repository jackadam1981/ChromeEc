...
void board_keyboard_drive_col(int col)
{
	...
	if (col == KEYBOARD_COLUMN_NONE)
		gpio_pin_set_dt(GPIO_DT_FROM_NODELABEL(gpio_ec_kso_02_inv), 0);

	/* Set KBSOUT to zero to detect key-press */
	else if (col == KEYBOARD_COLUMN_ALL)
		gpio_pin_set_dt(GPIO_DT_FROM_NODELABEL(gpio_ec_kso_02_inv), 1);

	/* Drive one line for detection */
	else {
		if (col == 2)
			gpio_pin_set_dt(
				GPIO_DT_FROM_NODELABEL(gpio_ec_kso_02_inv), 1);
		else
			gpio_pin_set_dt(
				GPIO_DT_FROM_NODELABEL(gpio_ec_kso_02_inv), 0);
	}
}
