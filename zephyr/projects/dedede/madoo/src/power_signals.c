...
#define pp3300_on_dev DEVICE_DT_GET(DT_NODELABEL(cmp_pp3300_high))
#define pp3300_off_dev DEVICE_DT_GET(DT_NODELABEL(cmp_pp3300_low))
...
static void pp3300_a_pgood_cb(const struct device *dev, const struct sensor_trigger *trigger)
{
	...
	if (dev == pp3300_on_dev) {
		atomic_or(&pp3300_a_pgood, 1);

		/* Disable this interrupt while it's asserted. */
		val.val1 = false;
		sensor_attr_set(pp3300_on_dev, SENSOR_CHAN_VOLTAGE,
			SENSOR_ATTR_ALERT, &val);

		/* Enable the voltage low interrupt. */
		val.val1 = true;
		sensor_attr_set(pp3300_off_dev, SENSOR_CHAN_VOLTAGE,
			SENSOR_ATTR_ALERT, &val);
	}
	else if (dev == pp3300_off_dev) {
...
static int baseboard_init(const struct device *device)
{
	...
	sensor_trigger_set(pp3300_on_dev, &trig, pp3300_a_pgood_cb);
	val.val1 = true;
	sensor_attr_set(pp3300_on_dev, SENSOR_CHAN_VOLTAGE,
		SENSOR_ATTR_ALERT, &val);
	sensor_trigger_set(pp3300_off_dev, &trig, pp3300_a_pgood_cb);
