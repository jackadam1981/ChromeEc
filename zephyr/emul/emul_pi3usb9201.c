...
static int pi3usb9201_emul_handle_write(struct pi3usb9201_emul_data *data,
					int reg, uint8_t val)
{
	if (reg < 0 || reg >= EMUL_REG_COUNT)
		return -EIO;
...
static int pi3usb9201_emul_handle_read(struct pi3usb9201_emul_data *data,
					int reg, uint8_t *val)
{
	if (reg < 0 || reg >= EMUL_REG_COUNT)
		return -EIO;
				    ...
				    int num_msgs, int addr)
{
	...
	if (num_msgs == 1) {
		...
		return pi3usb9201_emul_handle_write(data, msgs[0].buf[0],
						    msgs[0].buf[1]);
	} else if (num_msgs == 2) {
		if (!(((msgs[0].flags & I2C_MSG_RW_MASK) == I2C_MSG_WRITE)
		      && (msgs[0].len == 1)
		...
		return pi3usb9201_emul_handle_read(data, msgs[0].buf[0],
						   &(msgs[1].buf[0]));
	} else {
