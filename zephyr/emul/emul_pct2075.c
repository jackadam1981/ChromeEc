...
#define DT_DRV_COMPAT zephyr_pct2075_emul
...
static const uint16_t default_values[PCT2075_REG_NUMBER] = {
	[PCT2075_REG_TEMP] = 0x00,
	[PCT2075_REG_CONF] = 0x00,
	[PCT2075_REG_THYST] = 0x4b00,
	[PCT2075_REG_TOS] = 0x5000,
...
int pct2075_emul_write_byte(const struct emul *target, int reg, uint8_t val,
			    int bytes)
{
	...
	if (bytes == 0) {
		...
		data->regs[reg] = (data->regs[reg] & 0xff00) | val;
	} else {
		/* Support up to 2 bytes write */
		return -1;
	}
...
#define INIT_PCT2075_EMUL(n)                                           \
	...
	static struct pct2075_data pct2075_data_##n = {              \
		.common = { \
			...
			.write_byte = pct2075_emul_write_byte,		\
		},							\
	}; \
