...
#define DT_DRV_COMPAT zephyr_amd_fp6_usb_mux_emul
...
struct amd_fp6_data {
	...
	int64_t finish_delay;
	int64_t set_time;
	uint8_t last_mux_set;
	uint8_t reg[AMD_FP6_MAX_REG];
};
...
void amd_fp6_emul_set_delay(const struct emul *emul, int delay_ms)
{
	...
	data->finish_delay = K_MSEC(delay_ms);
}
...
void amd_fp6_emul_set_xbar(const struct emul *emul, bool ready)
{
	...
	data->reg[AMD_FP6_STATUS] = ready ? (AMD_FP6_MUX_PD_STATUS_READY
					     << AMD_FP6_MUX_PD_STATUS_OFFSET) :
					    0;
}
			     ...
			     int bytes, void *unused_data)
{
	...
	if (pos == AMD_FP6_PORT0 && data->finish_delay > 0) {
		if ((regs[pos] >> AMD_FP6_MUX_PORT_STATUS_OFFSET) ==
		    ...
		    k_uptime_delta(&data->set_time) >= finish_delay) {
			regs[pos] = amd_fp6_emul_mux_complete(last_mux_set);
		}
	}
...
ZTEST_RULE(emul_amd_fp6_usb_mux_reset, NULL, amd_fp6_emul_reset_regs);
