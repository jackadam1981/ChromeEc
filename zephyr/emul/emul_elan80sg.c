...
#define DT_DRV_COMPAT egis_elan80sg
...
enum elan80sg_cmd {
	ELAN80SG_CMD_INT_CLR = 0x1C,
	ELAN80SG_CMD_HW_ID = 0xFD,
};
			    ...
			    const struct spi_buf_set *rx_bufs)
{
	...
	case ELAN80SG_CMD_HW_ID:
		...
		elan80sg_write_response(rx_bufs, resp);
		break;

	case ELAN80sg_CMD_INT_CLR:
...
static void elan80sg_emul_gpio_irq(const struct device *dev,
				   struct gpio_callback *cb, uint32_t pins)
{
	struct elan80sg_emul_data *data =
		CONTAINER_OF(cb, struct elan80sg_emul_data, irq_cb);
	const struct elan80sg_emul_cfg *cfg = data->target->cfg;
			      ...
			      const struct device *parent)
{
	...
	gpio_init_callback(&data->irq_cb, elan80sg_emul_gpio_irq,
