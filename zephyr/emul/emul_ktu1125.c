 ...
 */
static void ktu1125_emul_reset(const struct emul *emul)
{
	struct ktu1125_data *data = emul->data;
...
static int ktu1125_emul_read(const struct emul *emul, int reg, uint16_t *val,
			     int bytes, void *unused_data)
{
	...
	if (!IN_RANGE(pos, 0, KTU1125_REG_MAX)) {
		return -1;
	}
	*val = regs[pos];
