...
static const uint16_t bmi3xx_emul_3xx_rsvd_mask[] = {
 ...
 */
static int bmi3xx_emul_start_cmd(uint16_t *regs, const struct emul *emul,
				 int cmd)
{
	int time;
 ...
 * @param emul Pointer to BMI emulator
 */
static void bmi3xx_emul_end_cmd(uint16_t *regs, const struct emul *emul)
{
	bool tag_time;
	int cmd;
	...
	case BMI3_CMD_SOFT_RESET:
		...
		break;
	default:
		/* TODO(yllin): support more commands */
		break;
	}
}
...
static int bmi3xx_emul_access_reg(const struct emul *emul, int reg, int byte,
				  bool read)
{
	printf("\033[33m%s emul 0x%x 0x%x 0x%d\033[m\n", __func__, (int)emul,
	       reg, byte);
 ...
 * @return -EIO on error
 */
static int bmi3xx_emul_handle_write(uint16_t *regs, const struct emul *emul,
				    int reg, int byte, uint16_t val)
{
	uint16_t mask;
	bool tag_time;
	...
	printf("\033[33m%s emul 0x%x 0x%x 0x%d\033[m\n", __func__, (int)emul,
	       reg, byte);
	reg = bmi3xx_emul_access_reg(emul, reg, byte, false /* = read */);
	...
	if (regs[BMI3_REG_CMD] != 0 && bmi3xx_emul_is_cmd_end(emul)) {
	...
	switch (reg) {
	case BMI3_REG_CMD:
		if (regs[BMI3_REG_CMD] != 0) {
			LOG_ERR("Issued command before previous end");
			return -EIO;
		}

		return bmi3xx_emul_start_cmd(regs, emul, val);
 ...
 */
static int bmi3xx_emul_handle_read(uint16_t *regs, const struct emul *emul,
				   int reg, int byte, char *buf)
{
	...
	reg = bmi3xx_emul_access_reg(emul, reg, byte, true /* = read */);
...
struct bmi3xx_emul_type_data bmi3xx_emul = {
	...
	.rsvd_mask = bmi3xx_emul_3xx_rsvd_mask,
