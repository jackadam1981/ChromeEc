...
#define MAX_KB_ROW 30
#define MAX_KB_COL 30

struct kb_raw_emul_data {
	int active_column;
	int matrix[MAX_KB_COL + 1];
};
...
{
	struct kb_raw_emul_data *data = device->data;

	memset(data, 0, sizeof(struct kb_raw_emul_data));
	return 0;
}
			    ...
			    int pressed)
{
	...
	if (col > MAX_KB_COL || row > MAX_KB_ROW)
		return -EINVAL;
	if (pressed)
