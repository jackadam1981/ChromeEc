...
/**
 * @brief Clear all interupt registers
 *
 * @param emul Pointer to BMA255 emulator
 */
...
static int bma_emul_handle_read(struct i2c_emul *emul, int reg, char *buf)
{
	...
	case BMA2x2_X_AXIS_LSB_ADDR:
		...
		ret = bma_emul_get_acc_val(emul, reg, &data->lsb_x_read, false,
					   data->acc_x + data->off_x);
		break;
	case BMA2x2_X_AXIS_MSB_ADDR:
		ret = bma_emul_get_acc_val(emul, reg - 1, &data->lsb_x_read, true,
					   data->acc_x + data->off_x);
		if (ret) {
	...
	case BMA2x2_Y_AXIS_LSB_ADDR:
		/* Shouldn't fail for LSB */
		ret = bma_emul_get_acc_val(emul, reg, &data->lsb_y_read, false,
					   data->acc_y + data->off_y);
		break;
	case BMA2x2_Y_AXIS_MSB_ADDR:
		ret = bma_emul_get_acc_val(emul, reg - 1, &data->lsb_y_read, true,
					   data->acc_y + data->off_y);
		if (ret) {
	...
	case BMA2x2_Z_AXIS_LSB_ADDR:
		/* Shouldn't fail for LSB */
		ret = bma_emul_get_acc_val(emul, reg, &data->lsb_z_read, false,
					   data->acc_z + data->off_z);
		break;
	case BMA2x2_Z_AXIS_MSB_ADDR:
		ret = bma_emul_get_acc_val(emul, reg - 1, &data->lsb_z_read, true,
					   data->acc_z + data->off_z);
		if (ret) {
