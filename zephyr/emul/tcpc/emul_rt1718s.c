...
static int copy_reg_bytes(uint8_t *dst, uint8_t src_reg[], int reg, int bytes)
{
	if ((reg + bytes) > RT1718S_EMUL_REG_COUNT_PER_PAGE) {
		return -EIO;
	}
	for (int i = reg; i < reg + bytes; i++) {
		*dst = src_reg[i];
		dst++;
	}
	return EC_SUCCESS;
				  ...
				  uint8_t *val, int bytes)
{
	...
	if (current_page == 2) {
		...
		return copy_reg_bytes(val, rt1718s_data->reg_page2,
				      rt1718s_data->current_page2_register,
				      bytes);
	} else if (is_valid_rt1718s_page1_register(reg)) {
		return copy_reg_bytes(val, rt1718s_data->reg_page1, reg, bytes);
	} else {
