...
/**
 * @brief Start source capabality timer. Capability message will be send after
 *        @p time.
 *
...
/**
 * @brief Start source capabality timer. Capability message will be send after
 *        TCPCI_SOURCE_CAPABILITY_TIMEOUT miliseconds.
 *
 * @param data Pointer to USB-C source device emulator data
 */
...
/**
 * @brief Stop source capabality timer. Capability message will not be repeated.
 *
 * @param data Pointer to USB-C source device emulator data
 */
				       ...
				       uint64_t delay, bool start_timer)
{
	...
	ret = tcpci_partner_send_data_msg(common_data,
					  PD_DATA_SOURCE_CAP,
					  data->pdo, pdos, delay);
	if (!start_timer) {
		return ret;
	...
	const struct tcpci_emul_msg *msg)
{
	...
	if (PD_HEADER_CNT(header)) {
		...
		case PD_CTRL_GET_SOURCE_CAP:
			tcpci_src_emul_send_capability_msg(data, common_data, 0,
							   false /* = timer */);
			return TCPCI_PARTNER_COMMON_MSG_HANDLED;
...
static void tcpci_src_emul_source_capability_timeout(struct k_work *work)
{
	...
	if (k_mutex_lock(&data->common_data->transmit_mutex, K_NO_WAIT) != 0) {
		/*
	...
	if (k_work_busy_get(work) & K_WORK_CANCELING) {
		k_mutex_unlock(&data->common_data->transmit_mutex);
		return;
	}

	tcpci_src_emul_send_capability_msg(data, data->common_data, 0, true);
...
void tcpci_src_emul_hard_reset(void *data)
{
	...
	tcpci_src_emul_send_capability_msg(src_emul_data,
					   src_emul_data->common_data, 0, true);
}
