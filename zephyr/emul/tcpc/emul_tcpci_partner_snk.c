 ...
 *         current for sink. Ammount of missing current is returned value in
 *         10mA units.
 */
			...
			const struct tcpci_emul_msg *msg, int pdo_num)
{
	...
	/* Skip 2 bytes of header. Each PDO is 4 bytes. */
	addr = 2 + pdo_num * 4;
					  ...
					  int src_pdo_num)
{
	...
	case PDO_TYPE_FIXED:
		...
		/* Force mismatch flag if higher capability bit is set */
		if (snk_pdo & BIT(28)) {
			flags = RDO_CAP_MISMATCH;
		}
