 ...
 * @param common_data Pointer to USB-C device emulator common data
 *
 * @return 1 on success after attempt to connect to TCPCI emulator
 * @return 0 when no attempt to connect to TCPCI emulator
	...
	struct tcpci_partner_data *common_data)
{
	...
	ret = tcpci_partner_make_connection(common_data, TYPEC_CC_VOLT_RD,
 ...
 * @param common_data Pointer to USB-C device emulator common data
 */
static void tcpci_snk_emul_init_data(struct tcpci_partner_extension *ext,
				     struct tcpci_partner_data *common_data)
{
	struct tcpci_snk_emul_data *data =
		CONTAINER_OF(ext, struct tcpci_snk_emul_data, ext);
