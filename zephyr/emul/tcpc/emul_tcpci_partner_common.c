...
void tcpci_set_header(struct tcpci_partner_data *data,
				     ...
				     const struct tcpci_emul_msg *tx_msg)
{
	...
	} else if (PD_HEADER_GET_SOP(header) == TCPCI_MSG_SOP_PRIME) {
		if (PD_HEADER_ID(header) == data->sop_prime_recv_msg_id &&
		    msg_type != PD_CTRL_SOFT_RESET) {
			/* Repeated message mark as handled */
			return TCPCI_PARTNER_COMMON_MSG_HANDLED;
		}
		data->sop_prime_recv_msg_id = PD_HEADER_ID(header);
	}
