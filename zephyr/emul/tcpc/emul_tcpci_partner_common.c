	...
	case CMD_DISCOVER_IDENT:
		if (data->identity_vdos > 0) {
			tcpci_partner_send_data_msg(data, PD_DATA_VENDOR_DEF,
					data->identity_vdm, data->identity_vdos,
					0);
		}
		return TCPCI_PARTNER_COMMON_MSG_HANDLED;
...
void tcpci_partner_set_discovery_info(struct tcpci_partner_data *data,
		...
		int svids_vdos, uint32_t *svids_vdm, int modes_vdos,
		uint32_t *modes_vdm)
{
	...
	memcpy(data->svids_vdm, svids_vdm,
			svids_vdos * sizeof(*svids_vdm));
	data->modes_vdos = modes_vdos;
	memcpy(data->modes_vdm, modes_vdm,
			modes_vdos * sizeof(*modes_vdm));
}
