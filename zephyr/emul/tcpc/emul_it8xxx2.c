...
struct it8xxx2_emul_cfg {
	/** Common I2C configuration used by "hidden" ports */
	const struct i2c_common_emul_cfg spi_cfg;
};

/**
 * @brief Function called for each byte of read message from it8xxx2 emulator
 *
 * @param emul Pointer to I2C it8xxx2 emulator
				  ...
				  uint8_t *val, int bytes)
{
	...
	struct it8xxx2_emul_data *it8xxx2_data = tcpc_data->chip_data;

	if (reg >= TCPCI_VENDOR_REGS_BASE) {
		*val = it8xxx2_data->tcpci_extra_regs[reg + bytes];
		return 0;
	}
...
/**
 * @brief Function called for each byte of write message to it8xxx2 emulator
 *
 * @param emul Pointer to I2C it8xxx2 emulator
				   ...
				   uint8_t val, int bytes)
{
	...
	if (reg >= TCPCI_VENDOR_REGS_BASE) {
		...
		it8xxx2_data->tcpci_extra_regs[reg + bytes - 1] = val;
		return 0;
	}
...
/**
 * @brief Function called on the end of write message to it8xxx2 emulator
 *
 * @param emul Pointer to I2C it8xxx2 emulator
 ...
 *        register from it8xxx2 emulator.
 *
 * @param emul Pointer to I2C it8xxx2 emulator
 * @param reg First byte of last write message
				     ...
				     int addr)
{
	...
	struct it8xxx2_emul_data *it8xxx2_data = tcpc_data->chip_data;
	const struct it8xxx2_emul_cfg *it8xxx2_cfg = target->cfg;
	...
	if (addr == tcpc_data->i2c_cfg.addr) {
		return i2c_common_emul_transfer_workhorse(target,
							  ...
							  &tcpc_data->i2c_cfg,
							  msgs, num_msgs, addr);
	} else if (addr == it8xxx2_cfg->spi_cfg.addr) {
		const struct i2c_common_emul_cfg *common_cfg =
			&it8xxx2_cfg->spi_cfg;
		common_data = &it8xxx2_data->spi_data;
 ...
 *
 * @param emul Pointer to it8xxx2 emulator
 */
void it8xxx2_emul_reset(const struct emul *emul)
{
	struct tcpc_emul_data *tcpc_data = emul->data;
	struct it8xxx2_emul_data *data = tcpc_data->chip_data;
...
int it8xxx2_emul_peek_spi_reg(const struct emul *emul, int reg)
{
	struct tcpc_emul_data *tcpc_data = emul->data;
	struct it8xxx2_emul_data *data = tcpc_data->chip_data;
...
void it8xxx2_emul_set_spi_reg(const struct emul *emul, int reg, int val)
{
	struct tcpc_emul_data *tcpc_data = emul->data;
	struct it8xxx2_emul_data *data = tcpc_data->chip_data;
...
int it8xxx2_emul_peek_tcpci_extra_reg(const struct emul *emul, int reg)
{
	struct tcpc_emul_data *tcpc_data = emul->data;
	struct it8xxx2_emul_data *data = tcpc_data->chip_data;
...
void it8xxx2_emul_set_tcpci_extra_reg(const struct emul *emul, int reg, int val)
{
	struct tcpc_emul_data *tcpc_data = emul->data;
	struct it8xxx2_emul_data *data = tcpc_data->chip_data;
...
/**
 * @brief Set up a new it8xxx2 emulator
 *
 * This should be called for each it8xxx2 device that needs to be
 * emulated.
 *
			     ...
			     const struct device *parent)
{
	...
	struct it8xxx2_emul_data *data = tcpc_data->chip_data;
	struct tcpci_ctx *tcpci_ctx = tcpc_data->tcpci_ctx;
	const struct it8xxx2_emul_cfg *cfg = emul->cfg;
	...
	data->spi_data.emul.api = &i2c_it8xxx2_emul_api;
...
static int it8xxx2_emul_spi_write_byte(const struct emul *emul, int reg,
				       uint8_t val, int bytes)
{
	struct tcpc_emul_data *tcpc_data;
	struct it8xxx2_emul_data *data;
	struct i2c_emul *i2c_emul = emul->bus.i2c;

	LOG_DBG("IT8XXX2 0x%x: read reg 0x%x", i2c_emul->addr, reg);
...
static int it8xxx2_emul_spi_read_byte(const struct emul *emul, int reg,
				      uint8_t *val, int bytes)
{
	struct tcpc_emul_data *tcpc_data;
	struct it8xxx2_emul_data *data;
	...
	LOG_DBG("IT8XXX2 0x%x: write reg 0x%x", i2c_emul->addr, reg);
...
it8xxx2_emul_get_i2c_common_data(const struct emul *emul)
{
	struct tcpc_emul_data *tcpc_data = emul->data;
	struct it8xxx2_emul_data *data = tcpc_data->chip_data;
...
#define IT8XXX2_EMUL(n)                                                    \
	static struct it8xxx2_emul_data it8xxx2_emul_data_##n = {       \
		.spi_data = {                                           \
			.write_byte = it8xxx2_emul_spi_write_byte,      \
			.read_byte = it8xxx2_emul_spi_read_byte,        \
		},                                                      \
	};  \
	static const struct ait8xxx2_emul_cfg it8xxx2_emul_cfg_##n = {   \
		.spi_cfg = {                                            \
			.dev_label = DT_NODE_FULL_NAME(DT_DRV_INST(n)), \
			.data = &it8xxx2_emul_data_##n.spi_data,        \
			.addr = DT_INST_PROP(n, spi_addr),              \
		},                                                      \
	}; \
	TCPCI_EMUL_DEFINE(n, it8xxx2_emul_init, &it8xxx2_emul_cfg_##n,     \
			  &it8xxx2_emul_data_##n, &i2c_it8xxx2_emul_api, NULL)

DT_INST_FOREACH_STATUS_OKAY(IT8XXX2_EMUL)

#ifdef CONFIG_ZTEST
#define IT8XXX2_EMUL_RESET_RULE_BEFORE(n) \
	it8xxx2_emul_reset(EMUL_DT_GET(DT_DRV_INST(n)))
static void it8xxx2_emul_reset_rule_before(const struct ztest_unit_test *test,
					   void *data)
{
	ARG_UNUSED(test);
	ARG_UNUSED(data);
	DT_INST_FOREACH_STATUS_OKAY(IT8XXX2_EMUL_RESET_RULE_BEFORE);
}
ZTEST_RULE(IT8XXX2_emul_reset, it8xxx2_emul_reset_rule_before, NULL);
#endif /* CONFIG_ZTEST */
