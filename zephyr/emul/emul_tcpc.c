...
/** Check description in emul_tcpc.h */
int tcpc_emul_get_reg(const struct emul *emul, int reg, uint16_t *val)
{
	struct tcpc_emul_data *data = emul->data;
	int byte;

	if (reg < 0 || reg > TCPC_EMUL_REG_COUNT || val == NULL) {
		return -EINVAL;
	}
 ...
 */
static bool tcpc_emul_check_int(const struct emul *emul)
{
	struct tcpc_emul_data *data = emul->data;
	uint16_t alert_mask;
	uint16_t alert;

	tcpc_emul_get_reg(emul, TCPC_REG_ALERT, &alert);
	tcpc_emul_get_reg(emul, TCPC_REG_ALERT_MASK, &alert_mask);
			 ...
			 bool alert)
{
	...
	if (data->rx_msg == NULL) {
		tcpc_emul_get_reg(emul, TCPC_REG_DEV_CAP_2, &dev_cap_2);
...
/** Check description in emul_tcpc.h */
void tcpc_emul_set_rev(const struct emul *emul, enum tcpc_emul_rev rev)
{
	switch (rev) {
	case TCPC_EMUL_REV1_0_VER1_0:
		tcpc_emul_set_reg(emul, TCPC_REG_PD_INT_REV,
				  (TCPC_REG_PD_INT_REV_REV_1_0 << 8) |
				   TCPC_REG_PD_INT_REV_VER_1_0);
		return;
	case TCPC_EMUL_REV2_0_VER1_1:
		tcpc_emul_set_reg(emul, TCPC_REG_PD_INT_REV,
...
static void tcpc_emul_reset_role_ctrl(const struct emul *emul)
{
	...
	tcpc_emul_get_reg(emul, TCPC_REG_DEV_CAP_1, &dev_cap_1);
...
static int tcpc_emul_handle_command(const struct emul *emul)
{
	 ...
	 * command sent
	 */
	tcpc_emul_set_reg(emul, TCPC_REG_COMMAND, data->write_data & 0xff);
	return 0;
}
				  ...
				  int msg_len)
{
	...
	case TCPC_REG_ALERT_EXT:
		...
		tcpc_emul_get_reg(emul, reg, &alert_val);
