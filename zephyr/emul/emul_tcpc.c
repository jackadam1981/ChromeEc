			 ...
			 bool alert)
{
	...
	if (alert) {
		...
		data->reg[TCPC_REG_ALERT + 1] &= TCPC_REG_ALERT_RX_STATUS;
...
static void tcpc_emul_reset(struct i2c_emul *emul)
{
	...
	// Default depends on device capabilities -- aditional function
	...
	// Look for defualts/inteligent set
	// or generate on read
	data->reg[TCPC_REG_CC_STATUS]				= 0x00;
	data->reg[TCPC_REG_POWER_STATUS]			= 0x00;
...
//	data->partner->transmit(data->tx_msg,
...
static void tcpc_emul_set_i2c_interface_err(struct i2c_emul *emul)
{
	...
	data->reg[TCPC_REG_ALERT] &= TCPC_REG_ALERT_FAULT >> 8;
}
...
static int tcpc_emul_handle_write(struct i2c_emul *emul, int reg, int msg_len)
{
	...
	/* 16 bits values */
	case TCPC_REG_ALERT:
		/* Overflow is cleared by Recive SOP message status */
	...
	if (data->error_on_sequential_write && msg_len - 1 != bytes_to_set) {
		tcpc_emul_set_i2c_interface_err(emul);
		return -EIO;
	}
...
static struct i2c_emul_api tcpc_emul_api = {
