			   ...
			   int sensortime_reg)
{
	...
	bmi_emul_set_data_reg(emul, data->gyr_x, &(data->reg[gyr_reg]),
			      gyr_shift);
	bmi_emul_set_data_reg(emul, data->gyr_y, &(data->reg[gyr_reg + 2]),
			      gyr_shift);
	bmi_emul_set_data_reg(emul, data->gyr_z, &(data->reg[gyr_reg + 4]),
			      gyr_shift);

	bmi_emul_set_data_reg(emul, data->acc_x, &(data->reg[acc_reg]),
			      acc_shift);
	bmi_emul_set_data_reg(emul, data->acc_y, &(data->reg[acc_reg + 2]),
			      acc_shift);
	bmi_emul_set_data_reg(emul, data->acc_z, &(data->reg[acc_reg + 4]),
			      acc_shift);
