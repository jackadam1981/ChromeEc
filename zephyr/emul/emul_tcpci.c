...
int tcpci_emul_set_reg(const struct emul *emul, int reg, uint16_t val)
{
	...
	for (byte = tcpci_emul_reg_bytes(reg); byte > 0; byte--) {
		if (reg == TCPC_REG_ALERT_MASK) {
			LOG_WRN("tcpci_emul_set_reg %s TCPC_REG_ALERT_MASK: %x", emul->dev_label, val & 0xff);
		}
		if (reg == TCPC_REG_ALERT_MASK + 1) {
			LOG_WRN("tcpci_emul_set_reg %s TCPC_REG_ALERT_MASK+1: %x", emul->dev_label, val & 0xff);
		}
...
static int tcpci_emul_alert_changed(const struct emul *emul)
{
	...
	if (data->alert_gpio_port != NULL) {
		...
		LOG_WRN("Setting GPIO to %d", alert_is_active ? 0 : 1);
		rc = gpio_emul_input_set(data->alert_gpio_port, data->alert_gpio_pin, alert_is_active ? 0 : 1);
		if (rc != 0) {
			return rc;
		}
	}
	...
	if (data->alert_callback == NULL) {
		LOG_ERR("No alert callback");
		return 0;
	}

	LOG_WRN("Sending alert");
...
int tcpci_emul_disconnect_partner(const struct emul *emul)
{
	...
	rc = tcpci_emul_alert_changed(emul);
	if (rc != 0) { return rc; }
	/* TODO: Wait until DisableSourceVbus (TCPC_REG_COMMAND_SRC_CTRL_LOW?), and then set VBUS present = 0 and vSafe0V = 1 after appropriate delays. */
	return 0;
}
