			   ...
			   const struct spi_buf_set *rx_bufs)
{
	...
	if (gpio_pin_get_dt(&cfg->reset_pin) == 0) {
		return -EAGAIN;
	}
			     ...
			     const struct device *parent)
{
	...
	gpio_init_callback(&data->irq_cb, egis630_emul_gpio_irq,
			   BIT(cfg->interrupt_pin.pin));
	gpio_add_callback_dt(&cfg->interrupt_pin, &data->irq_cb);
	gpio_pin_interrupt_configure_dt(&cfg->interrupt_pin,
