...
static int tcs_emul_handle_write(struct i2c_emul *emul, int reg, uint8_t val)
{
	...
	if (reg >= TCS_EMUL_FIRST_REG && reg <= TCS_EMUL_LAST_REG) {
		...
		reg -= TCS_EMUL_FIRST_REG;
		if (data->error_on_rsvd_write && tcs_emul_rsvd_mask[reg] & val) {
	...
	case TCS_I2C_IR:
		...
		break;
	case TCS_I2C_IFORCE:
		/* Interupt generate is not supported */
		break;
 ...
 *
 * @param emul Pointer to TCS3400 emulator
 * @param lsb_reg LSB register address (TCS_I2C_CDATAL, TCS_I2C_RDATAL,
 *                TCS_I2C_GDATAL, TCS_I2C_BDATA_L)
 ...
 */
static int tcs_emul_get_reg_val(struct i2c_emul *emul, int lsb_reg,
				bool *lsb_read, bool lsb, unsigned int val)
{
	...
	if (lsb) {
		*lsb_read = 1;
		...
		/*
		 * Iferror on first accessing MSB is set and LSB wasn't accessed
		 * before, then return error.
		 */
	 ...
	 * registers value
	 */
	val = val * cycles * gain / TCS_EMUL_MAX_CYCLES / TCS_EMUL_MAX_GAIN;

	if (val > UINT16_MAX) {
		val = UINT16_MAX;
	}

	data->reg[lsb_reg] = val & 0xff;
	data->reg[msb_reg] = (val >> 8) & 0xff;
...
static int tcs_emul_handle_read(struct i2c_emul *emul, int reg, char *buf)
{
	...
	case TCS_I2C_CDATAL:
		...
		c_ir = data->ir_select ? data->ir : data->c;
		ret = tcs_emul_get_reg_val(emul, reg, &data->lsb_c_ir_read,
					   true, c_ir);
		break;
	case TCS_I2C_CDATAH:
		c_ir = data->ir_select ? data->ir : data->c;
		ret = tcs_emul_get_reg_val(emul, reg - 1, &data->lsb_c_ir_read,
					   false, c_ir);
		if (ret) {
			LOG_ERR("MSB C readed before LSB C");
			return -EIO;
		}
		break;
	case TCS_I2C_RDATAL:
		...
		break;
	case TCS_I2C_RDATAH:
		ret = tcs_emul_get_reg_val(emul, reg - 1, &data->lsb_r_read,
					   false, data->r);
		if (ret) {
			LOG_ERR("MSB R readed before LSB R");
			return -EIO;
		}
		break;
	case TCS_I2C_GDATAL:
		...
		break;
	case TCS_I2C_GDATAH:
		ret = tcs_emul_get_reg_val(emul, reg - 1, &data->lsb_g_read,
					   false, data->g);
		if (ret) {
			LOG_ERR("MSB G readed before LSB G");
			return -EIO;
		}
		break;
	case TCS_I2C_BDATAL:
		...
		break;
	case TCS_I2C_BDATAH:
		ret = tcs_emul_get_reg_val(emul, reg - 1, &data->lsb_b_read,
					   false, data->b);
		if (ret) {
			LOG_ERR("MSB B readed before LSB B");
			return -EIO;
		}
		break;
	case TCS_I2C_IR:
