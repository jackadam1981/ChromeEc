...
static int emul_lsm6dsm_get_fifo_count(const struct emul *emul)
{
	...
	return (FIELD_GET(0x7, data->reg[0x3b]) << 8) | data->reg[0x3a];
}
...
static int emul_lsm6dsm_get_fifo_wm(const struct emul *emul)
{
	...
	return (FIELD_GET(0x7, data->reg[0x07]) << 8) | data->reg[0x06];
}
...
static void emul_lsm6dsm_set_fifo_count(const struct emul *emul, int count)
{
	...
	if (count == 0) {
		...
		data->reg[0x3b] |= FIELD_GET(0x700, count);
		data->reg[0x3c] = FIELD_GET(0xff, count);
	}
	LOG_DBG("FIFO_STATUS: 0x%02x %02x", data->reg[0x3b], data->reg[0x3c]);
}
