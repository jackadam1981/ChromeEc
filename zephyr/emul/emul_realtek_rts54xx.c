...
static int set_battery_capability(struct rts5453p_emul_pdc_data *data,
				  const union rts54_request *req)
{
	LOG_INF("SET_BATTERY_CAPABILITY 0x%X", );
...
static int emul_realtek_rts54xx_get_battery_capability(
...
static int emul_realtek_rts54xx_get_battery_status(
	const struct emul *target, union battery_status_t *bstat)
{
	struct rts5453p_emul_pdc_data *data =
		rts5453p_emul_get_pdc_data(target);
