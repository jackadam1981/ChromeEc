...
typedef int (*handler)(struct rts5453p_emul_pdc_data *data,
		       const struct rts54_command *req);

static int unsupported(struct rts5453p_emul_pdc_data *data,
		       const struct rts54_command *req)
{
	LOG_ERR("cmd=0x%X, subcmd=0x%X is not supported", req->command_code,
		req->data[1]);
			 ...
			 const struct rts54_command *req)
{
	...
	data->ic_status.byte_count = sizeof(struct rts54_ic_status) - 1;
	data->ic_status.is_flash_code = 1;
	data->ic_status.fw_main_version = 0x2;
	data->ic_status.fw_sub_version[0] = 0x3;
	data->ic_status.fw_sub_version[1] = 0x4;
	data->ic_status.pd_revision[0] = 0x5;
	data->ic_status.pd_revision[1] = 0x6;
	data->ic_status.pd_version[0] = 0x7;
	data->ic_status.pd_version[1] = 0x8;

	set_ping_status(data, CMD_COMPLETE, sizeof(struct rts54_ic_status));

	memcpy(data->response, &data->ic_status,
	       sizeof(struct rts54_ic_status));
...
int process_request(struct rts5453p_emul_pdc_data *data,
		    const struct rts54_command *req, uint8_t code,
	...
	for (i = 0; i < num_cmds; i++) {
		if (cmds[i].code == code) {
			if (cmds[i].type == HANDLER) {
				...
				return process_request(data, req, req->data[1],
						       cmds->sub_cmd,
						       cmds->num_cmds);
			}
		}
...
static int rts5453p_emul_start_write(const struct emul *emul, int reg)
{
	...
	memset(&data->request, 0, sizeof(struct rts54_command));

	data->request.command_code = reg;
				    ...
				    uint8_t val, int bytes)
{
	...
	data->request.data[data->request.byte_count] = val;
	data->request.byte_count++;
				      ...
				      int bytes)
{
	...
	return process_request(data, &data->request, data->request.command_code,
