 ...
 *               accelerometer value
 */
static int bmi160_emul_range_to_shift(uint8_t range)
{
	switch (range & 0xf) {
	case BMI160_GSEL_2G:
		return 0;
 ...
 *
 * @return 0 on success
 * @return BMI_EMUL_ACCESS_E on writing read only register
 * @return -EIO on error
 */
 ...
 *
 * @return 0 on success
 * @return BMI_EMUL_ACCESS_E on reading write only register
				   ...
				   int *reg, int byte, char *buf)
{
	...
	gyr_shift = 4 - (regs[BMI160_GYR_RANGE] & 0x7);
	acc_shift = bmi160_emul_range_to_shift(regs[BMI160_ACC_RANGE]);
