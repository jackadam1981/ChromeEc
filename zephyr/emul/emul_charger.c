...
static void charger_emul_set_header(struct charger_emul_data *data,
				    ...
				    int type, int cnt)
{
	uint16_t header = PD_HEADER(type, PD_ROLE_SOURCE, PD_ROLE_UFP,
				    data->msg_id & 0x7, cnt, PD_REV20,
				    0 /* ext */);
	data->msg_id++;
				 ...
				 struct charger_emul_msg *msg, uint64_t delay)
{
	...
	now = k_uptime_get();
	msg->time = now + delay;
	if (k_fifo_is_empty(&data->to_send)) {
				     ...
				     int retry)
{
	...
	/* Acknowledge that message was send successfully */
...
/**
 * @brief Function called when TCPM consumes message. Free message that is not
 *        longer needed.
 *
 * @param emul Pointer to TCPCI emulator
 * @param ops Pointer to partner operations structure
				  ...
				  const struct emul *tcpci_emul)
{
	...
	ec = tcpci_emul_connect_partner(tcpci_emul, true /* as source */,
					true /* on CC1 */, TYPEC_CC_VOLT_RP_3_0,
					false /* no active cable */);
	if (ec) {
		return ec;
	}
