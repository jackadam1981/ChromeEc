...
static struct sbat_emul_data *SBAT_EMUL_DATA_SINGLETON = NULL;

/** Check description in emul_smart_battery.h */
struct sbat_emul_bat_data *sbat_emul_get_bat_data(const struct emul *emul)
{
/*
 * TODO(b/240710893): Fix type likely type coercion error.
 */
#ifdef CONFIG_B240710893
...
#else
	ARG_UNUSED(emul);
	return &SBAT_EMUL_DATA_SINGLETON->bat;
#endif
}
...
static int sbat_emul_init(const struct emul *emul, const struct device *parent)
{
	...
	__ASSERT_NO_MSG(SBAT_EMUL_DATA_SINGLETON == NULL);
	SBAT_EMUL_DATA_SINGLETON = data;
