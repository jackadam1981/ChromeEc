...
def log_state(charge_state, charge_port, chipset_state, batt_min, batt_range):
    ...
    if batt_range == 101:
        ...
        logging.error(
            "No LED policy at battery level %i%% for %s%s%s",
            batt_min,
            (charge_state + ", ") if charge_state else "",
            f"port {charge_port}, " if charge_port is not None else "",
            (chipset_state + ", ") if chipset_state else "",
        ...
        logging.error(
            "No LED policy for battery range %i%% to %i%% for %s%s%s",
            batt_min,
