...
def insert_motionsense_component(node, i2c_portmap, manifest):
    ...
    component_loc = node.props["location"].val
    if component_loc == "MOTIONSENSE_LOC_BASE":
        component_type = "base_sensor"
    elif component_loc == "MOTIONSENSE_LOC_LID":
        component_type = "lid_sensor"
    elif component_loc == "MOTIONSENSE_LOC_CAMERA":
    ...
    device_name = compatible_name_parser(
        "accel", node.props["compatible"].val[0]
    )
