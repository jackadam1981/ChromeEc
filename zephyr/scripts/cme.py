...
def compatible_name_parser(ctype_list, name):
    """Parses the compatible into actual component name.

    Args:
        ctype_list: a list of types of components that matches to a single AVL
...
def insert_motionsense_component(node, i2c_portmap, manifest):
    ...
    if i2c_addr in SENSOR_I2C_ADDRESSES:
        i2c_addr_reg = SENSOR_I2C_ADDRESSES[i2c_addr]
    else:
        logging.error(
    ...
    for part_number in ALS_MOTIONSENSORS:
        if part_number in compatible_name:
            component_type = "als"

    manifest.insert_component(
        component_type,
        compatible_name_parser(["clear", "accel", "gyro"], compatible_name),
