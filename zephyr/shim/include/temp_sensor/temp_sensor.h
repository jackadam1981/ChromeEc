...
#define ANY_INST_HAS_POWER_GOOD_PIN (FOREACH_TEMP_SENSOR(HAS_POWER_GOOD_PIN) 0)
...
#define NAMED_TEMP_SENSOR(node_id) DT_STRING_UPPER_TOKEN(node_id, enum_name)
#define NAMED_TEMP_SENSOR_WITH_COMMA(node_id) \
	NAMED_TEMP_SENSOR(node_id) = TEMP_SENSOR_ID_BY_NAMED(node_id),

enum named_temp_sensor {
#if DT_NODE_EXISTS(DT_PATH(named_temp_sensors))
	DT_FOREACH_CHILD(DT_PATH(named_temp_sensors),
			 NAMED_TEMP_SENSOR_WITH_COMMA)
#endif
		NAMED_TEMP_SENSOR_COUNT
};
