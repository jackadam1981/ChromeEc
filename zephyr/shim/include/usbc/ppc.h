...
#define ALT_PPC_CHIP_CHK(usbc_id, usb_port_num) \
	COND_CODE_1 \
		(DT_REG_HAS_IDX(usbc_id, usb_port_num), \
		(COND_CODE_1(DT_NODE_HAS_PROP(usbc_id, ppc_alt), \
			( || 1), \
			( || 0))), \
		( || 0))

#define ALT_PPC_CHIP_CPY(usbc_id, usb_port_num) \
	COND_CODE_1 \
		(DT_REG_HAS_IDX(usbc_id, usb_port_num), \
		(COND_CODE_1 \
			...
			(COND_CODE_1(DT_NODE_HAS_PROP(usbc_id, ppc_alt), \
				(memcpy \
					(&ppc_chips[USBC_PORT_NEW(usbc_id)], \
...
#define PPC_ENABLE_ALTERNATE(usb_port_num) \
	do { \
		BUILD_ASSERT( \
			(0 DT_FOREACH_STATUS_OKAY_VARGS \
				(named_usbc_port, \
				ALT_PPC_CHIP_CHK, \
				usb_port_num \
				)), \
		...
		memcpy \
			(&ppc_chips[usb_port_num], \
			&ppc_chips_alt[usb_port_num], \
			sizeof(struct ppc_config_t) \
			); \
	} while (0)
