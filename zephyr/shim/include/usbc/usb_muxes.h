 ...
 *           driver config as arguments.
 */
#define USB_MUX_FOREACH_MUX_DT_VA(op) \
 ...
 */
#define USB_MUX_LIST LIST_DROP_EMPTY(USB_MUX_FOREACH_MUX_DT_VA(USB_MUX_TO_LIST))
 ...
 *        like USB_MUX_FOREACH_MUX_DT_VA(), except
...
#define USB_MUX_PORT_IF_SAME_NODES(chain_id, unused2, idx, mux_id) \
	DT_SAME_NODE(mux_id, USB_MUX_GET_CHAIN_N(idx, chain_id)) ? \
		USBC_PORT(chain_id):
 ...
 *
 * This expands to:
 *     (DT_DEP_ORD(mux_id) == DT_DEP_ORD(USB_MUX_GET_CHAIN_N(0, chain1_id))) ?
 *         USBC_PORT(chain1_id) :
 *     (DT_DEP_ORD(mux_id) == DT_DEP_ORD(USB_MUX_GET_CHAIN_N(1, chain1_id))) ?
 *         USBC_PORT(chain1_id) :
 *         ...
 *     (DT_DEP_ORD(mux_id) == DT_DEP_ORD(USB_MUX_GET_CHAIN_N(n, chain1_id))) ?
 *         USBC_PORT(chain1_id) :
 *     (DT_DEP_ORD(mux_id) == DT_DEP_ORD(USB_MUX_GET_CHAIN_N(0, chain2_id))) ?
 *         USBC_PORT(chain2_id) :
 *         ...
 *     (DT_DEP_ORD(mux_id) == DT_DEP_ORD(USB_MUX_GET_CHAIN_N(k, chainm_id))) ?
