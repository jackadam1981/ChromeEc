...
/**
 * @brief Get USB-C port number for given @p chain_id using port of the first
 *        USB mux in the chain.
 *
 * @param chain_id Chain DTS node ID
 */
#define USB_MUX_CHAIN_PORT(chain_id) USBC_PORT(USB_MUX_GET_CHAIN_N(0, chain_id))
 ...
 */
#define USB_MUX_GET_CHAIN_N(idx, chain_id) \
...
#define USB_MUX_POINTER(chain_id, idx) \
	&USB_MUX_STRUCT_NAME(USB_MUX_GET_CHAIN_N(idx, chain_id))
...
#define USB_MUX_BB_RETIMER_CONTROL_ARRAY(mux_id) \
	[USBC_PORT(mux_id)] = BB_RETIMER_CONTROLS_CONFIG(mux_id),
...
/**
 * @brief Call @p op opertaion for each node that is compatible with @p driver
 *
 * @param driver USB mux driver description in format (compatible, config)
...
#define USB_MUX_CHAIN_STRUCT_DECLARE_EXTERN_OP(chain_id, idx) \
	extern USB_MUX_CHAIN_STRUCT_DECLARE(USB_MUX_CHAIN_PORT(chain_id), idx);
...
#define USB_MUX_CHAIN_STRUCT_DECLARE_OP(chain_id, idx) \
	USB_MUX_CHAIN_STRUCT_DECLARE(USB_MUX_CHAIN_PORT(chain_id), idx);
...
/**
 * @brief Call @p op if @p idx is not 0 (is not first mux in chain)
 *
 * @param chain_id Chain DTS node ID
 ...
 */
#define USB_MUX_SKIP_FIRST(chain_id, unused2, idx, op) \
	COND_CODE_1(UTIL_BOOL(idx), (op(chain_id, idx)), ())

/**
 * @brief Call @p op for each mux in @p chain_id chain except the first mux
 *
 * @param chain_id Chain DTS node ID
 * @param op Operation to perform on USB muxes
 */
#define USB_MUX_FOREACH_NO_FIRST_MUX(chain_id, op) \
	DT_FOREACH_PROP_ELEM_VARGS(chain_id, usb_muxes, USB_MUX_SKIP_FIRST, op)
 ...
 * @param chain_id Chain DTS node ID
 */
#define USB_MUX_DEFINE_FIRST_MUX(chain_id) \
	[USB_MUX_CHAIN_PORT(chain_id)] = USB_MUX_CHAIN_STRUCT_INIT(chain_id, 0),
...
#define USB_MUX_SET_ALTERNATIVE(chain_id, idx)                         \
	USB_MUX_CHAIN_STRUCT_NAME(idx, USB_MUX_CHAIN_PORT(chain_id)) = \
...
#define USB_MUX_ENABLE_ALTERNATIVE_NODE(chain_id)                     \
	do {                                                          \
		usb_muxes[USB_MUX_CHAIN_PORT(chain_id)] =             \
			USB_MUX_CHAIN_STRUCT_SET(chain_id, 0);        \
		USB_MUX_FOREACH_NO_FIRST_MUX(chain_id,                \
					     USB_MUX_SET_ALTERNATIVE) \
	} while (0)
