 ...
 * inp = gpio_pin_get_dt(GPIO_DT_LABEL(gpio_ec_wp_l)); // Zephyr API
 * enum gpio_signal sig = GPIO_WP_L;
 * inp = gpio_pin_get_dt(gpio_get_dt_spec(sig)); // Zephyr API
 *
 * DT_GPIO_LID_OPEN and GPIO_DT_LABEL(gpio_ec_wp_l) will resolve at build time,
 * whereas gpio_get_dt_spec() will resolve at run-time.
 */
