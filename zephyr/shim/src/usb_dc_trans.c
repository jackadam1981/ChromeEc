...
#include <zephyr/usb/usb_device.h>
#include "../../../../third_party/zephyr/main/subsys/usb/device/usb_descriptor.h"
#endif
LOG_MODULE_REGISTER(usb_dc_trans, LOG_LEVEL_ERR);

static K_KERNEL_STACK_DEFINE(usb_dc_thread_stack, CONFIG_USB_DC_STACK_SIZE);
static struct k_thread usb_dc_thread_data;
static struct k_sem usb_dc_sem;
...
void usb_keyboard_state_changed(int row, int col, int is_pressed)
{
	...
	if (is_pressed) {
	/* Add the key to the HID report if it is not already present */
...
#endif /* #ifdef CONFIG_USB_DC_TRANS */
...
static void usb_dc_loop_thread(void *p1, void *p2, void *p3)
{

}

static inline void create_usb_dc_thread(void)
{
	k_thread_create(&usb_dc_thread_data, usb_dc_thread_stack,
			K_KERNEL_STACK_SIZEOF(usb_dc_thread_stack),
			(k_thread_entry_t)usb_dc_loop_thread, NULL, NULL, NULL,
			CONFIG_USB_DC_THREAD_PRIORITY, 0,
			IS_ENABLED(CONFIG_USB_DC_TRANS_AUTOSTART) ? K_NO_WAIT :
								 K_FOREVER);

	k_thread_name_set(&usb_dc_thread_data, "usb_dc_task");
}

void usb_dc_task_start(void)
{
	if (!IS_ENABLED(CONFIG_USB_DC_TRANS_AUTOSTART)) {
		k_thread_start(&usb_dc_thread_data);
	}
}
...
static int usb_dc_init(void)
{
	...
	k_sem_init(&usb_dc_sem, 0, 1);
	/* Create usb dc translation handler core function thread */
	create_usb_dc_thread();
