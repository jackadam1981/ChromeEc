...
void pwm_set_raw_duty(enum pwm_channel ch, uint16_t duty)
{
	...
	pwm->pulse_us = DIV_ROUND_NEAREST(pwm->period_us * duty,
		EC_PWM_MAX_DUTY);

	LOG_DBG("PWM %s set raw duty (0x%04x), pulse %d",
		pwm->name, duty, pwm->pulse_us);
...
void pwm_set_duty(enum pwm_channel ch, int percent)
{
	...
	LOG_DBG("PWM %s set percent (%d), pulse %d",
		pwm->name, percent, pwm->pulse_us);
