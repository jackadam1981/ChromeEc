...
#define CROS_EC_STACK_SIZE 1024
...
#define HECI_CROS_EC_REQEST_MAX \
...
static heci_rx_msg_t cros_ec_rx_msg;
static uint8_t cros_ec_rx_buffer[CROS_EC_ISHTP_RX_BUF_SIZE];

static K_THREAD_STACK_DEFINE(cros_ec_stack, CROS_EC_STACK_SIZE);
static struct k_thread cros_ec_thread;

static K_SEM_DEFINE(cros_ec_event_sem, 0, 1);
static uint32_t cros_ec_event;
static uint32_t cros_ec_conn_id;
...
int heci_send_mkbp_event(uint32_t *timestamp)
{
	...
	ret = heci_send(cros_ec_conn_id, &m) ? EC_SUCCESS : EC_ERROR_UNKNOWN;
	*timestamp = k_cycle_get_32() * USEC_PER_SEC /
...
static void heci_send_hostcmd_response(struct host_packet *pkt)
{
	...
	if (!heci_send(cros_ec_conn_id, &m)) {
		LOG_ERR("HC response failed");
	}

	heci_send_flow_control(cros_ec_conn_id);
}

static void cros_ec_process_msg(uint8_t *msg, const size_t msg_size)
{
	...
	heci_packet.request_max = HECI_CROS_EC_REQEST_MAX;
...
static enum ec_status heci_get_protocol_info(struct host_cmd_handler_args *args)
{
	...
	r->max_request_packet_size = HECI_CROS_EC_REQEST_MAX;
...
static void cros_ec_event_callback(uint32_t event, void *arg)
{
	ARG_UNUSED(arg);
	cros_ec_event = event;
	k_sem_give(&cros_ec_event_sem);
}

static void cros_ec_task(void *p1, void *p2, void *p3)
{
	ARG_UNUSED(p1);
	ARG_UNUSED(p2);
	ARG_UNUSED(p3);
	...
	while (true) {
		k_sem_take(&cros_ec_event_sem, K_FOREVER);

		LOG_DBG("cros ec new heci event %u", cros_ec_event);

		switch (cros_ec_event) {
		case HECI_EVENT_NEW_MSG:
			...
			if (cros_ec_rx_msg.type == HECI_CONNECT) {
				cros_ec_conn_id = cros_ec_rx_msg.connection_id;
				heci_send_flow_control(cros_ec_conn_id);
				LOG_DBG("cros ec new conn: %u",
					cros_ec_conn_id);
			} else if (cros_ec_rx_msg.type == HECI_REQUEST) {
				cros_ec_process_msg(cros_ec_rx_msg.buffer,
		...
		case HECI_EVENT_DISCONN:
			LOG_DBG("cros ec disconnect request conn %d",
				cros_ec_conn_id);
			heci_complete_disconnect(cros_ec_conn_id);
			break;

		default:
			LOG_ERR("cros ec wrong heci event %u", cros_ec_event);
			break;
		}
	}
}

static int cros_ec_client_init(const struct device *arg)
{
	ARG_UNUSED(arg);

	int ret;
	heci_client_t cros_ec_client = {
		...
		.event_cb = cros_ec_event_callback
	};

	cros_ec_client.rx_msg = &cros_ec_rx_msg;
	cros_ec_client.rx_msg->buffer = cros_ec_rx_buffer;

	ret = heci_register(&cros_ec_client);
	if (ret) {
		LOG_ERR("failed to register cros ec client %d", ret);
		return ret;
	}

	k_thread_create(&cros_ec_thread, cros_ec_stack,
			K_THREAD_STACK_SIZEOF(cros_ec_stack), cros_ec_task,
			NULL, NULL, NULL, K_PRIO_PREEMPT(1), 0, K_NO_WAIT);
	k_thread_name_set(&cros_ec_thread, "cros_ec_client");

	return 0;
}

SYS_INIT(cros_ec_client_init, APPLICATION, 99);
