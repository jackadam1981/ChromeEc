...
BUILD_ASSERT(CONFIG_NUM_PREEMPT_PRIORITIES >= TASK_ID_COUNT,
	     "Must increase number of available preempty priorities");

/* Ensure all of the manually defined HAS_TASK_ defines are present */
#define CROS_EC_TASK(name, ...) BUILD_ASSERT(HAS_TASK_##name, "Must define HAS_TASK_*");
CROS_EC_TASK_LIST
#undef CROS_EC_TASK

/* Declare all task stacks here */
#define CROS_EC_TASK(name, e, p, size) K_THREAD_STACK_DEFINE(name##_STACK, size);
CROS_EC_TASK_LIST
#undef CROS_EC_TASK
...
uint32_t task_wait_event(int timeout_us)
{
	...
	/* What for signal then clear it before reading events */
	...
	if (events == 0) {
		...
		if (ticks_left > 0) {
			return task_wait_event(k_ticks_to_us_near64(ticks_left));
		}
...
void task_entry(void* task_contex, void* unused1, void* unused2)
{
	ARG_UNUSED(unused1);
	ARG_UNUSED(unused2);
