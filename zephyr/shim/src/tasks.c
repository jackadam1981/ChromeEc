...
static void print_threads(const struct k_thread *thread, void *user_data)
{
...
task_id_t thread_id_to_task_id(k_tid_t thread_id)
{
	...
	LOG_ERR("$$$ Current TID = %p (%s), state = %x", k_current_get(),
		SAFE_THREAD_NAME(k_current_get()), thread_id->base.thread_state);
	...
	LOG_ERR("$$$ Dump CrOS task array (len %d)", ARRAY_SIZE(task_to_k_tid));
