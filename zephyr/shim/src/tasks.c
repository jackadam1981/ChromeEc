...
test_mockable_static inline k_tid_t get_idle_thread(void)
{
...
test_mockable_static inline k_tid_t get_sysworkq_thread(void)
{
...
test_mockable_static inline k_tid_t get_main_thread(void)
{
...
task_id_t thread_id_to_task_id(k_tid_t thread_id)
{
	...
	__ASSERT(false, "Unable to map thread %s to task", thread_id->name);
