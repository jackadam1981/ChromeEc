...
/* Ensure that the idle task is at a lower priority than lowest priority task.
 **/
BUILD_ASSERT(EC_TASK_PRIORITY(0) < K_IDLE_PRIO,
...
int init_signals(const struct device *unused)
{
	...
	for (size_t i = 0; i < EXTRA_TASK_COUNT; ++i) {
		struct task_ctx_base_data *const data = &extra_tasks_data[i];
