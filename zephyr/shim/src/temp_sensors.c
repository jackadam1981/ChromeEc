...
#define POWER_DOMAIN(sensor_id)                                           \
	COND_CODE_1(DT_NODE_HAS_PROP(sensor_id, power_domain),            \
		    (DEVICE_DT_GET(DT_PHANDLE(sensor_id, power_domain))), \
		    (NULL))
...
void temp_sensors_poll(void)
{
	for (int i = 0; i < TEMP_SENSOR_COUNT; i++) {
		const struct temp_sensor_t *sensor = temp_sensors + i;
		if (sensor->zephyr_info->poll) {
			if (sensor->zephyr_info->domain) {
				enum pm_device_state state =
					PM_DEVICE_STATE_OFF;
				pm_device_state_get(sensor->zephyr_info->domain,
						    &state);
				if (state != PM_DEVICE_STATE_ACTIVE) {
					return;
				}
			...
			if (sensor->zephyr_info->power_good_dev) {
				if (!gpio_pin_get(
					    sensor->zephyr_info->power_good_dev,
					    sensor->zephyr_info->power_good_pin))
					return;
			}
#endif
			sensor->zephyr_info->poll();
		}
	}
}
DECLARE_HOOK(HOOK_SECOND, temp_sensors_poll, HOOK_PRIO_TEMP_SENSOR);
