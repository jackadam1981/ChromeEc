...
#define GET_THERMISTOR_DATUM(node_sample_id)                     \
	[DT_PROP(node_sample_id, sample_number)] =               \
...
#define DEFINE_THERMISTOR_DATA(node_id)                         \
	...
	DT_CAT(node_id, _thermistor_data)[] = {			\
...
DT_FOREACH_STATUS_OKAY(cros_thermistor, DEFINE_THERMISTOR_DATA)

const struct temp_sensor_t temp_sensors[] = {
	DT_FOREACH_STATUS_OKAY(cros_temp_sensor, TEMP_THERMISTOR)
};
