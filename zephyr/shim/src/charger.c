...
#define CHG_CHIP_ENTRY(usbc_id, chg_id, chip_fn) \
	[USBC_PORT_NEW(usbc_id)] = chip_fn(chg_id)

#define CHECK_COMPAT(compat, usbc_id, chg_id, config)   \
	COND_CODE_1(DT_NODE_HAS_COMPAT(chg_id, compat), \
		    (CHG_CHIP_ENTRY(usbc_id, chg_id, config)), ())
