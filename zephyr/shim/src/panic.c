...
/* Arch-specific include for arch_is_in_nested_exception */
...
#else
/* Forward declare unused function for other architectures */
bool arch_is_in_nested_exception(const struct arch_esf *esf);
#endif
 ...
 */
static inline bool is_in_nested_exception(const struct arch_esf *esf)
{
	if (IS_ENABLED(CONFIG_ARCH_HAS_NESTED_EXCEPTION_DETECTION))
		return (esf != NULL) && arch_is_in_nested_exception(esf);
	return false;
}
