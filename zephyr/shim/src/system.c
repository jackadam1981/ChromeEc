...
static int system_preinitialize(const struct device *unused)
{
	...
	if (!sys_dev) {
		/*
		 * TODO(b/183022804): This should not happen in normal operation.
		 * Check whether the error check can be change to build-time
		 * error, or at least a fatal run-time error.
		 */
