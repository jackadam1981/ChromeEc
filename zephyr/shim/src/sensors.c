...
#define SENSOR_BASIC_INFO(id)						\
	.name = DT_LABEL(id),						\
	.active_mask = DT_PROP(id, active_mask),			\
	.location = DT_ENUM_IDX(id, location),				\
...
#define SENSOR_I2C_SPI_ADDR_FLAGS(id)					\
	IF_ENABLED(DT_NODE_HAS_PROP(id, i2c_spi_addr_flags),		\
		(.i2c_spi_addr_flags = DT_PROP(id, i2c_spi_addr_flags),))
...
#define MAT_ITEM(i, id)	FLOAT_TO_FP(DT_PROP_BY_IDX(id, mat33, i))
