 ...
 * interrupt.
 */
void gpio_enable_dt_interrupt(struct gpio_int_config *conf)
{
	gpio_flags_t flags;
	/*
 ...
 */
void gpio_disable_dt_interrupt(struct gpio_int_config *conf)
{
	gpio_pin_interrupt_configure(conf->port, conf->pin, GPIO_INT_DISABLE);
}
