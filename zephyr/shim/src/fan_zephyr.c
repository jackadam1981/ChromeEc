...
int dptf_get_fan_duty_target()
{
	...
	ret = fan_get_mode(fans[0].dev, &mode);
	if (ret || mode != FAN_MODE_PWM)
		return -1;

	ret = fan_get_speed(fans[0].dev, FAN_SPEED_TYPE_PWM, &duty);
	if (ret != 0)
		return ret;
...
void dptf_set_fan_duty_target(int duty)
{
	...
	for (i = 0; i < fan_get_count(); i++) {
		ret = fan_set_mode(fans[i].dev, FAN_MODE_PWM);
		if (ret != 0)
			return;

		(void)fan_set_target(fans[i].dev, duty);
	}
}
