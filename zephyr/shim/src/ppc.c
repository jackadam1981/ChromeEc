...
#define PPC_CHIP_ENTRY(usbc_id, ppc_id, chip_fn) \
	[USBC_PORT_NEW(usbc_id)] = chip_fn(ppc_id)

#define CHECK_COMPAT(compat, usbc_id, ppc_id, config)   \
	COND_CODE_1(DT_NODE_HAS_COMPAT(ppc_id, compat), \
		    (PPC_CHIP_ENTRY(usbc_id, ppc_id, config)), ())
