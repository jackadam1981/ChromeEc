...
#define HOOK_LIST_EXTERNS(type)                              \
	extern const struct zephyr_shim_hook_info            \
		zephyr_shim_hook_list_##type##_list_start[]; \
	extern const struct zephyr_shim_hook_info            \
		zephyr_shim_hook_list_##type##_list_end[];
FOR_EACH(HOOK_LIST_EXTERNS, (), HOOK_TYPES_LIST)

#define HOOK_LIST_ENTRY(type)                                       \
	[type] = {                                                  \
		.start = zephyr_shim_hook_list_##type##_list_start, \
		.end = zephyr_shim_hook_list_##type##_list_end,     \
	}
...
void hook_notify(enum hook_type type)
{
	...
	while (1) {
		...
		for (const struct zephyr_shim_hook_info *p = start; p != end;
		     p++) {
			__ASSERT(p->priority >= HOOK_PRIO_FIRST,
				 "Hook priority %d (%p) is below minimum",
				 p->priority, p->handler);
