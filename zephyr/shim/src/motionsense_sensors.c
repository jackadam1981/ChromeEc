...
/*
 * The indentation below is a result of clang-formatter not being able to parse
 * nested macros properly. For some reason it thinks that initialization of
 * subsequent fields needs to be nested. Can't really do anything about it.
 */
#define DECLARE_BODYDETECT_CONFIG(id)                                          \
	const struct body_detect_params SENSOR_BODYDETECT_NAME(id) = {         \
		IF_ENABLED(DT_NODE_HAS_PROP(id, var_noise_factor),             \
			   (.var_noise_factor =                                \
				    DT_PROP(id, var_noise_factor), ))          \
			IF_ENABLED(DT_NODE_HAS_PROP(id, var_threshold),        \
				   (.var_threshold =                           \
					    DT_PROP(id, var_threshold), ))     \
				IF_ENABLED(DT_NODE_HAS_PROP(id,                \
							    confidence_delta), \
					   (.confidence_delta = DT_PROP(       \
						    id, confidence_delta), ))  \
	};
...
#define SENSOR_BODYDETECT(id)                               \
	IF_ENABLED(DT_NODE_HAS_PROP(id, bodydetect_config), \
		   (.bd_params = &SENSOR_BODYDETECT_NAME(   \
