...
/* Construct an entry iff the is_alternate property is 'false'. */
#define MK_SENSOR_ENTRY(inst, s_compat, s_chip, s_type, s_drv, s_min_freq, \
			s_max_freq)                                        \
	COND_CODE_0(DT_PROP(DT_INST(inst, s_compat), is_alternate),        \
		    (DO_MK_SENSOR_ENTRY(DT_INST(inst, s_compat), s_chip,   \
					s_type, s_drv, s_min_freq,         \
					s_max_freq)),                      \
		    ())

/* Construct an entry iff the is_alternate property is 'true'. */
#define MK_SENSOR_ALT_ENTRY(inst, s_compat, s_chip, s_type, s_drv, s_min_freq, \
			    s_max_freq)                                        \
	COND_CODE_1(DT_PROP(DT_INST(inst, s_compat), is_alternate),            \
		    (DO_MK_SENSOR_ENTRY(DT_INST(inst, s_compat), s_chip,       \
					s_type, s_drv, s_min_freq,             \
					s_max_freq)),                          \
		    ())
