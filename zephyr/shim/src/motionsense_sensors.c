...
#define RUNTIME_PROBE_REQUIRED_CHECK(id) \
	COND_CODE_1(DT_PROP(id, runtime_probe),				\
		(|| 1),							\
		())

#define RUNTIME_PROBE_REQUIRED						\
	COND_CODE_1(DT_NODE_EXISTS(SENSOR_ALT_NODE),			\
	((0 DT_FOREACH_CHILD(SENSOR_ALT_NODE, RUNTIME_PROBE_REQUIRED_CHECK))), \
	(0))
