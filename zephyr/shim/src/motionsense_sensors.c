...
#define ENABLE_ALT_CASE_GENERATE(alt_id)                                      \
	case SENSOR_ID(alt_id):                                               \
		ENABLE_ALT_MOTION_SENSOR(alt_id,                              \
					  DT_PHANDLE(alt_id, alternate_for)); \
		break;

#define ENABLE_ALT_CASE_CHECK(alt_id)                        \
	COND_CODE_1(DT_NODE_HAS_PROP(alt_id, alternate_for), \
		    (ENABLE_ALT_CASE_GENERATE(alt_id)), ())

int motion_sense_enable_alt(enum sensor_alt_id alt_idx)
{
	switch (alt_idx) {
		DT_FOREACH_CHILD(SENSOR_ALT_NODE, ENABLE_ALT_CASE_CHECK)
	default:
