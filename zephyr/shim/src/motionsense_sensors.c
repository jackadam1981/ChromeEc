...
#define DO_MK_SENSOR_ENTRY(id, s_i2c_spi_addr, s_chip, s_type, s_drv,	\
		s_min_freq, s_max_freq)					\
	[SENSOR_ID(id)] = {						\
		SENSOR_BASIC_INFO(id)					\
		.i2c_spi_addr_flags = s_i2c_spi_addr,			\
		.chip = s_chip,						\
		.type = s_type,						\
		.drv = &s_drv,						\
...
#define MK_SENSOR_ENTRY(inst, s_compat, s_chip, s_type, s_drv,		\
		s_min_freq, s_max_freq)					\
	DO_MK_SENSOR_ENTRY(DT_PARENT(DT_INST(inst, s_compat)),		\
		SENSOR_I2C_SPI_ADDR_FLAGS(DT_INST(inst, s_compat)),	\
