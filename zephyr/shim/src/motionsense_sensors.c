...
#if DT_NODE_HAS_PROP(SENSOE_SENSOR_INFO_NODE, als_sensors)
...
const struct motion_sensor_t *motion_als_sensors[] = {
	UTIL_LISTIFY(DT_PROP_LEN(SENSOE_SENSOR_INFO_NODE, als_sensors),
		     ALS_SENSOR_ENTRY_WITH_COMMA, SENSOE_SENSOR_INFO_NODE)
};
BUILD_ASSERT(ARRAY_SIZE(motion_als_sensors) == ALS_COUNT);
#endif
#if DT_NODE_HAS_PROP(SENSOE_SENSOR_INFO_NODE, sensor_irqs)
...
{
	UTIL_LISTIFY(DT_PROP_LEN(SENSOE_SENSOR_INFO_NODE, sensor_irqs),
		     SENSOR_GPIO_ENABLE_INTERRUPT, SENSOE_SENSOR_INFO_NODE)
}
...
/*
 * TODO(b/173507858) : move to board specific file.
 */
