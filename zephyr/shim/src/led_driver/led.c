...
static void led_execute_patterns(uint32_t increment)
{
	...
	for (int i = 0; i < ARRAY_SIZE(policy_groups); i++) {
		...
		if (group_status.is_animating || group_status.needs_apply) {
	...
	if (continue_animating) {
		elapsed = k_uptime_delta(&start_time);
		delay = MAX(0, (int64_t)LED_ANIMATION_TICK_MS - elapsed);
		k_work_schedule(&led_worker_data, K_MSEC(delay));
	} else {
...
static void led_animation_worker(struct k_work *work)
{
	led_execute_patterns(LED_ANIMATION_TICK_MS);
}
