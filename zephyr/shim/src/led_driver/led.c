...
static void set_color(int node_idx)
{
	...
	for (int i = 0; i < node_array[node_idx].num_patterns; i++) {
		...
		patterns[i].pattern_color[0].led_color_node->api.led_set_color_with_pattern(&patterns[i]);
...
#define INVOKE_APPLY_COLOR_API(id)  PINS_NODE(id).api.led_asynchronous_apply_color(has_transitions);
