			...
			if (next_color[i].pulse_ns > prev_color[i].pulse_ns) {
				...
				cur_color[i].pulse_ns =
					...
					<< (MSB(scale) * pattern->ticks /
					    duration);
				/*
				 * boost the strength by 1 tick so it actually
				 * reaches the maximum value.
				 */
				cur_color[i].pulse_ns <<= 1;
				if (cur_color[i].pulse_ns >
				    next_color[i].pulse_ns)
					cur_color[i].pulse_ns =
						next_color[i].pulse_ns;
...
#define PIN_APPLY_PULSE_HELPER(pd) pwm_set_pulse_dt(&pd.pwm_spec, pd.pulse_ns);
#define PIN_APPLY_PULSE(node_id, prop, idx) \
	PIN_APPLY_PULSE_HELPER(DATA_NODE(DT_PHANDLE_BY_IDX(node_id, prop, idx)))
