...
#if !DT_NODE_EXISTS(DT_ALIAS(cbi_wp)) && \
	!defined(CONFIG_PLATFORM_EC_BYPASS_CBI_EEPROM_WP_CHECK)
#error CBI EEPROM requires GPIO WP to be pointed with cbi_wp alias.
#endif
...
static int eeprom_is_write_protected(void)
{
	...
	return gpio_pin_get_dt(GPIO_DT_NAME(GPIO_SIGNAL(DT_ALIAS(cbi_wp))));
}
