...
#define I2C_PORT_INIT(id)                                                    \
	COND_CODE_1(DT_NODE_HAS_PROP(id, kbps),                              \
		    (                                                        \
			    {                                                \
				    .name = DT_LABEL(id),                    \
				    .port = I2C_PORT(id),                    \
				    .kbps = DT_PROP(id, kbps),               \
				    .scl = DT_ENUM_UPPER_TOKEN(              \
					    DT_PHANDLE(id, scl), enum_name), \
				    .sda = DT_ENUM_UPPER_TOKEN(              \
					    DT_PHANDLE(id, sda), enum_name), \
			    }, ),                                            \
		    ())
/*
