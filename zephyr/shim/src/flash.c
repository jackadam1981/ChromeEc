...
extern const char __ram_code_start;
#define RAM_CODE_START ((uint32_t)&__ram_code_start)
...
void __ram_code dma_reset_immu(void)
{
	/* Immu tag sram reset */
...
void __ram_code dma_flash_follow_mode(void)
{
	 ...
	 */
	flash_regs->SMFI_ECINDAR3 = (EC_INDIRECT_READ_INTERNAL_FLASH | 0x0F);

	/* Set FSCE# as high level by writing 0 to address xfff_fe00h */
	flash_regs->SMFI_ECINDAR2 = 0xFF;
	flash_regs->SMFI_ECINDAR1 = 0xFE;
	flash_regs->SMFI_ECINDAR0 = 0x00;

	/* EC-indirect memory data register */
	flash_regs->SMFI_ECINDDR = 0x00;
}

void __ram_code dma_flash_follow_mode_exit(void)
{
	struct flash_it8xxx2_regs *const flash_regs = FLASH_IT8XXX2_REG_BASE;
...
void __ram_code dma_flash_fsce_high(void)
{
	struct flash_it8xxx2_regs *const flash_regs = FLASH_IT8XXX2_REG_BASE;
...
void __ram_code dma_flash_write_dat(uint8_t wdata)
{
	struct flash_it8xxx2_regs *const flash_regs = FLASH_IT8XXX2_REG_BASE;
...
void __ram_code dma_flash_transaction(int wlen, uint8_t *wbuf, int rlen,
...
void __ram_code dma_flash_cmd_read_status(enum flash_status_mask mask,
...
void __ram_code dma_flash_cmd_write_enable(void)
{
...
void __ram_code dma_flash_cmd_write_disable(void)
{
...
int __ram_code dma_flash_verify(int addr, int size, const char *data)
{
	int i;
...
void __ram_code dma_flash_cmd_write(int addr, int wlen, uint8_t *wbuf)
{
	int i;
...
void __ram_code dma_flash_write(int addr, int wlen,
						const char *wbuf)
{
...
void __ram_code dma_flash_cmd_erase(int addr, int cmd)
{
...
void __ram_code dma_flash_erase(int addr, int cmd)
{
	dma_flash_cmd_write_enable();
...
int flash_physical_read(int offset, int size, char *data)
{
	...
	for (i = 0; i < size; i++) {
		flash_regs->SMFI_ECINDAR3 = EC_INDIRECT_READ_INTERNAL_FLASH;
		flash_regs->SMFI_ECINDAR2 = (offset >> 16) & 0xFF;
		flash_regs->SMFI_ECINDAR1 = (offset >> 8) & 0xFF;
		flash_regs->SMFI_ECINDAR0 = (offset & 0xFF);

		/*
		 * Read/Write to this register will access one byte on the
		 * flash with the 32-bit flash address defined in ECINDAR3-0
		 */
		data[i] = flash_regs->SMFI_ECINDDR;
...
static void flash_code_static_dma(void)
{
	...
	flash_regs->SMFI_SCAR0H = BIT(3);

	memcpy((void *)CHIP_RAMCODE_BASE, (const void *)RAM_CODE_START,
	 ...
	 */
	flash_regs->SMFI_SCAR0L = RAM_CODE_START & 0xFF;
	flash_regs->SMFI_SCAR0M = (RAM_CODE_START >> 8) & 0xFF;
	flash_regs->SMFI_SCAR0H = (RAM_CODE_START >> 16) & 0x7;

	if (RAM_CODE_START & BIT(19))
		flash_regs->SMFI_SCAR0H |= BIT(7);
	else
		flash_regs->SMFI_SCAR0H &= ~BIT(7);
...
static int flash_dev_init(const struct device *unused)
{
	...
	cros_flash_dev = DEVICE_DT_GET(DT_NODELABEL(fiu0));
