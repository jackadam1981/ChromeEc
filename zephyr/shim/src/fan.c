...
#define FAN_CONFIGS(node_id)                                                   \
	const struct fan_conf node_id_conf = {                                 \
		...
		.pgood_gpio = COND_CODE_1(                                     \
			...
			(GPIO_SIGNAL(DT_PHANDLE(node_id, pgood_gpio))), (-1)), \
...
static int fan_rpm(int ch)
{
	...
	sensor_sample_fetch_chan(fan_control[ch].tacho, SENSOR_CHAN_RPM);
	sensor_channel_get(fan_control[ch].tacho, SENSOR_CHAN_RPM, &val);
	return (int)val.val1;
}
...
{
	return fan_status[ch].fan_mode == FAN_RPM ? 1 : 0;
}

void fan_set_rpm_mode(int ch, int rpm_mode)
{
	if (rpm_mode && (fan_status[ch].flags & FAN_USE_RPM_MODE))
		fan_status[ch].fan_mode = FAN_RPM;
	else
		fan_status[ch].fan_mode = FAN_DUTY;
}
