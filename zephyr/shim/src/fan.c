...
static void fan_pwm_update(int ch)
{
	...
	const struct device *dev = cfg->pwm.dev;
	uint32_t pulse_ns;
	int ret;

	if (!device_is_ready(dev)) {
		LOG_ERR("PWM device %s not ready", dev->name);
		return;
	}
	...
	LOG_DBG("FAN PWM %s set percent (%d), pulse %d", dev->name,
		data->pwm_percent, pulse_ns);

	ret = pwm_set_dt(&(cfg->pwm), cfg->pwm.period, pulse_ns);
	if (ret) {
		LOG_ERR("pwm_set() failed %s (%d)", dev->name, ret);
	}
}
