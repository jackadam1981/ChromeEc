...
int tcpc_enable_interrupt(void)
{
	...
	for (int i = 0; i < ARRAY_SIZE(tcpc_config); i++) {
		 ...
		 * pin exists in the devicetree.
		 */
		if (!int_gpio_cb[i].handler && tcpc_config[i].gpio_port) {
			/*
			 * Initialise and add the callback.
			 */
			gpio_init_callback(&int_gpio_cb[i],
					   tcpc_int_gpio_callback,
					   BIT(tcpc_config[i].interrupt_pin));
			gpio_add_callback(tcpc_config[i].gpio_port,
					  &int_gpio_cb[i]);
		}
		...
		ret = gpio_pin_interrupt_configure(tcpc_config[i].gpio_port,
						   tcpc_config[i].interrupt_pin,
						   flags);
		if (ret != 0)
			return ret;
	}
