...
#define TCPC_CHIP_ENTRY(usbc_id, tcpc_id, chip_fn) \
	[USBC_PORT_NEW(usbc_id)] = chip_fn(tcpc_id)

#define CHECK_COMPAT(compat, usbc_id, tcpc_id, config)   \
	COND_CODE_1(DT_NODE_HAS_COMPAT(tcpc_id, compat), \
		    (TCPC_CHIP_ENTRY(usbc_id, tcpc_id, config)), ())
