...
const struct gpio_dt_spec *int_cfg = GPIO_DT_FROM_NODELABEL(gpio_usb_c0_pd_int_odl);
...
void tcpc_enable_interrupt(void)
{
	...
	for (int i = 0; i < ARRAY_SIZE(tcpc_config); i++) {
		...
		gpio_pin_interrupt_configure(tcpc_config[i].int_cfg.port,
					     tcpc_config[i].int_cfg.pin, flags);
	}
}
