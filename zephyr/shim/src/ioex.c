...
#define IOEX_GPIO_CONFIG(id)                                                \
	{                                                                   \
		...
		.ioex = COND_CODE_1( DT_NODE_HAS_COMPAT(CHIP_FROM_GPIO(id), \
				cros_ioex_chip),                            \
...
static const struct ioex_gpio_config ioex_configs[] = {
#if DT_NODE_EXISTS(DT_PATH(named_ioexes))
	DT_FOREACH_CHILD(DT_PATH(named_ioexes), IOEX_GPIO_CONFIG)
#endif
};

static gpio_flags_t ioexes_flags[] = {
#if DT_NODE_EXISTS(DT_PATH(named_ioexes))
	DT_FOREACH_CHILD(DT_PATH(named_ioexes), IOEX_INIT_FLAGS)
#endif
};

#define CTLR_FROM_IOEX(id) DT_STRING_TOKEN(DT_GPIO_CTLR(id, gpios), port_named)
#define NAME_OF_I2C(id) NAMED_I2C(CTLR_FROM_IOEX(id))

#define IOEX_I2C_PORT(id) \
	COND_CODE_1(DT_NODE_HAS_PROP(DT_GPIO_CTLR(id, gpios), port_named), \
	(NAME_OF_I2C(id),), \
	(-1,))
	...
	enum ioex_signal signal)
{
	...
	LOG_ERR("No interrupt defined for GPIO %s",
		ioex_configs[signal - IOEX_SIGNAL_START].name);
...
int ioex_enable_interrupt(enum ioex_signal signal)
{
	...
	res = gpio_pin_interrupt_configure(ioex_configs[offset].dev,
					   ioex_configs[offset].pin,
...
{
	if (signal >= IOEX_SIGNAL_START && signal < IOEX_SIGNAL_END) {
		*flags = convert_from_zephyr_flags(
			ioexes_flags[signal - IOEX_SIGNAL_START]);
		return EC_SUCCESS;
	}
...
{
	const struct ioex_gpio_config *g = ioex_get_signal_info(signal);
	gpio_port_value_t portval;
	int res;

	if (g == NULL)
		return EC_ERROR_INVAL;

	res = gpio_port_get_raw(g->dev, &portval);
	if (res)
		return EC_ERROR_UNKNOWN;
...
static int ioex_init_default(const struct device *unused)
{
	...
	for (i = 0; i < ARRAY_SIZE(ioex_int_configs); i++) {
		...
		gpio_init_callback(&ioex_int_configs[i].callback,
				   ioex_isr,
				   (1 << ioex_configs[offset].pin));
		gpio_add_callback(ioex_configs[offset].dev,
				  &ioex_int_configs[i].callback);

		ioex_enable_interrupt(ioex_int_configs[i].signal);
	}
