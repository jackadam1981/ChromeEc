...
static bool is_cbi_empty(uint8_t *data, int len)
{
	for (int i = 0; i < len; i++) {
		if (data[i] != 0xFF) {
			return false;
...
static bool is_valid_cbi(const uint8_t *cbi)
{
	...
	/* Copy the data */
	memcpy(head->data, cbi + sizeof(*head),
...
void cros_cbi_transfer_eeprom_to_flash(void)
{
	...
	ry = flash_cbi_config.drv->load(0, cbi, CBI_IMAGE_SIZE);
	if (rv) {
		LOG_ERR("Failed to read CBI from Flash");
		return;
	}
	if (is_cbi_empty(cbi, CBI_IMAGE_SIZE) || (!is_valid_cbi(cbi))) {
		...
		if (is_valid_cbi(cbi)) {
			...
			if (rv) {
				LOG_ERR("Failed to write CBI in Flash");
				return;
			}
		} else {
			LOG_ERR("Bad EEPROM data, transfer to flash failed");
			return;
		}
	}
}
