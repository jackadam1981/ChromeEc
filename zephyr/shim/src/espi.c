...
#define CASE_AB(A, B) \
	case A:       \
		return B;
#define CASE_BA(A, B) CASE_AB(B, A)
...
{
	switch (signal) {
		VW_SIGNAL_TRANSLATION_LIST(CASE_AB);
	default:
...
{
	switch (vwire) {
		VW_SIGNAL_TRANSLATION_LIST(CASE_BA);
	default:
...
/*
 * For each of the virtual wires, this determines if interrupts on the
 * wire should be routed to the power sequencing interrupt handler.
 * espi_vw_enable_wire_int and espi_vw_disable_wire_int are used to
 * toggle these values on and off.
 */
static bool signal_interrupt_enabled[VW_SIGNAL_COUNT];
			       ...
			       struct espi_event event)
{
	...
	if (IS_ENABLED(CONFIG_PLATFORM_EC_POWERSEQ) &&
	    signal_interrupt_enabled[ec_signal - VW_SIGNAL_START]) {
...
static int init_espi(const struct device *unused)
{
	ARG_UNUSED(unused);
...
SYS_INIT(init_espi, APPLICATION, 10);
...
{
	signal_interrupt_enabled[signal - VW_SIGNAL_START] = true;
	return 0;
}

int espi_vw_disable_wire_int(enum espi_vw_signal signal)
{
	signal_interrupt_enabled[signal - VW_SIGNAL_START] = false;
	return 0;
}
