...
static uint64_t watchdog_reload_period_max_ts_ms;
static uint32_t watchdog_reload_period_max_ms;
static uint32_t watchdog_reload_last_ts_ms;
static uint32_t watchdog_reload_count;
static uint64_t watchdog_stats_reset_ts_ms;
...
void watchdog_reload(void)
{
	...
	if (IS_ENABLED(CONFIG_PLATFORM_EC_HOSTCMD_WATCHDOG_INFO) ||
		...
		K_SPINLOCK(&watchdog_stats_lock)
		{
			...
			uint32_t elapsed = now - watchdog_reload_last_ts_ms;
			watchdog_reload_count++;
			if (elapsed > watchdog_reload_period_max_ms) {
				watchdog_reload_period_max_ms = elapsed;
				watchdog_reload_period_max_ts_ms = now;
			}
			watchdog_reload_last_ts_ms = now;
		}
	}
...
__maybe_unused static void reset_watchdog_stats(void)
{
	K_SPINLOCK(&watchdog_stats_lock)
	{
		watchdog_stats_reset_ts_ms = k_uptime_get();
...
static enum ec_status hostcmd_watchdog_info(struct host_cmd_handler_args *args)
{
	...
	K_SPINLOCK(&watchdog_stats_lock)
	{
		...
		r->watchdog_stats_elapsed_ms = now - watchdog_stats_reset_ts_ms;
	}
	if (p->reset_stats) {
...
static void print_watchdog_info(void)
{
	...
	printk("Reload Period Nominal: %u ms\n",
	       HOOK_TICK_INTERVAL / USEC_PER_MSEC);
	K_SPINLOCK(&watchdog_stats_lock)
	{
		uint64_t now = k_uptime_get();
		uint64_t elapsed = now - watchdog_stats_reset_ts_ms;
		printk("Stats Elapsed Time: %llu.%03llu s\n",
		       elapsed / MSEC_PER_SEC, elapsed % MSEC_PER_SEC);
		printk("Reload Count: %u\n", watchdog_reload_count);
		printk("Reload Period Max: %u ms @ %llu.%03llu s\n",
		       watchdog_reload_period_max_ms,
		       watchdog_reload_period_max_ts_ms / MSEC_PER_SEC,
		       watchdog_reload_period_max_ts_ms % MSEC_PER_SEC);
		printk("Reload Period Avg: %llu ms\n",
		       watchdog_reload_count > 0 ?
			       elapsed / watchdog_reload_count :
			       0);
	}
}
