...
struct ioex_drv_data {
	...
	int cached_values;
	int pin_trig_edge_rising;
	int pin_trig_edge_falling;
	int pin_trig_level_zero;
	int pin_trig_level_one;
};

static int shim_ioex_pin_configure(const struct device *dev,
				   gpio_pin_t pin,
				   gpio_flags_t flags)
{
	...
	res = cfg->drv->set_flags_by_mask(drv_data->ioex,
			drv_data->port, (1 << pin),
					     ...
					     enum gpio_int_trig trig)
{
	...
	res = cfg->drv->get_flags_by_mask(drv_data->ioex,
					  drv_data->port,
					  (1 << pin),
					  &flags);
	if (res)
	...
	res = cfg->drv->enable_interrupt(drv_data->ioex,
			drv_data->port,
			(1 << pin),
...
static void shim_ioex_isr(const struct device *dev,
			  ...
			  gpio_port_pins_t pins)
{
	struct ioex_drv_data *drv_data = CONTAINER_OF(callback,
						      struct ioex_drv_data,
						      int_gpio_callback);
...
{
	struct ioex_drv_data *drv_data = CONTAINER_OF(worker,
						      struct ioex_drv_data,
						      worker);
	...
	/* Level 1 */
	interrupted_pins_level |= (current_values &
					drv_data->pin_trig_level_one);
	/* Level 0 */
	interrupted_pins_level |= ((~current_values) &
					drv_data->pin_trig_level_zero);
...
static int shim_ioex_init(const struct device *dev)
{
	...
	if (drv_data->int_gpio_dev) {
		...
		gpio_init_callback(&drv_data->int_gpio_callback,
				   shim_ioex_isr,
...
{ \
	.i2c_host_port = NAMED_I2C(DT_STRING_TOKEN(id, port_named)), \
...
{ \
	...
	COND_CODE_1(DT_NODE_HAS_PROP(DT_PARENT(DT_DRV_INST(idx)), int_gpios), \
		( \
		.int_gpio_dev = DEVICE_DT_GET( \
			DT_PHANDLE(DT_PARENT(DT_DRV_INST(idx)), \
				int_gpios)), \
		.int_gpio_pin = DT_GPIO_PIN( \
			DT_PARENT(DT_DRV_INST(idx)), \
			int_gpios), \
...
struct ioexpander_config_t ioex_config[] = {
	DT_FOREACH_STATUS_OKAY(DT_DRV_COMPAT_CHIP, IOEX_INIT_CONFIG_ELEM)
};

#define GPIO_PORT_INIT(idx) \
static struct ioex_drv_data ioex_##idx##_data = IOEX_INIT_DATA(idx); \
DEVICE_DT_INST_DEFINE(idx, shim_ioex_init, NULL, \
		      &ioex_##idx##_data, \
		      &ioex_config[IOEXPANDER_ID( \
				DT_PARENT(DT_DRV_INST(idx)))], \
