...
#define WDT_CTRL ((volatile uint32_t *)0x4000C000ul)
#define WDT_INTEN ((volatile uint32_t *)0x4000C010ul)
...
{
    uint32_t i;
	void (*__start_lfw_in_sram)(uint32_t, uint32_t, uint32_t, uint32_t) =
		(void (*)(uint32_t, uint32_t, uint32_t, uint32_t))(__lfw_sram_start | 0x01);

    __ASSERT_NO_MSG((size % 4) == 0 && (srcAddr % 4) == 0 && (dstAddr % 4) == 0);
