...
int spic_flash_upload(void)
{
	...
	*flag_upload = 0;
	...
	slowtmr_dealyus(1000);
	while ((UART->LSR & UART_LSR_THRE_Msk) == 0) {
		if (SLWTMR0->INTSTS_b.STS) {
			UART->THR = 0x10;
			break;
		}
	}
	if (SLWTMR0->INTSTS_b.STS) {
		...
		*flag_upload = 0x10;
		...
		if ((GPIO->GCR_b[113].MFCTRL == 0x00) ||
		    (GPIO->GCR_b[114].MFCTRL == 0x00)) {
			GPIO->GCR_b[113].MFCTRL = 0x01;
			GPIO->GCR_b[114].MFCTRL = 0x01;
		}
		UART->THR = 0x11;
		return -1;
	}
