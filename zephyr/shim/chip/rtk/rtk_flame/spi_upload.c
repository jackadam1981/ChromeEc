...
#define GPIO_UART_FUNCTION_CHECK                   \
	((GPIO->GCR[113] & GPIO_GCR_MFCTRL_Msk) || \
	 (GPIO->GCR[114] & GPIO_GCR_MFCTRL_Msk))
/**********************
 *   GLOBAL FUNCTIONS
 **********************/
	...
	if (uut_tag == RTS_MONITOR_UUT_TAG) {
		...
		*flag_upload = 0x08;
		if (GPIO_UART_FUNCTION_CHECK) {
			GPIO->GCR[113] &= ~GPIO_GCR_MFCTRL_Msk;
			GPIO->GCR[114] &= ~GPIO_GCR_MFCTRL_Msk;
			GPIO->GCR[113] |= 0x01 << GPIO_GCR_MFCTRL_Pos;
			GPIO->GCR[114] |= 0x01 << GPIO_GCR_MFCTRL_Pos;
		}
		return -1;
	}
...
static void uart_init_pll_115200(void)
{
	...
	SYSTEM->UARTCLK &= ~SYSTEM_UARTCLK_PWR_Msk;
