...
BUILD_ASSERT(DT_HAS_CHOSEN(cros_ec_cec_bitbang_tmr),
	     "a cros-ec,cec-bitbang-tmr device must be chosen");
#define cec_bitbang_tmr_dev DEVICE_DT_GET(DT_CHOSEN(cros_ec_cec_bitbang_tmr))
				    ...
				    int timeout)
{
	...
	if (timeout > 0) {
		...
		int timer_count = counter_us_to_ticks(cec_bitbang_tmr_dev,
						      (timeout - delay));
		...
		top_cfg.flags = 0;
		counter_set_top_value(cec_bitbang_tmr_dev, &top_cfg);
	} else {
		/*
		 * In chip/it83xx/cec_bitbang.c
		 * ext_timer_stop(drv_config->timer, 1);
		 */
		counter_stop(cec_bitbang_tmr_dev);
	}
}
...
{
	return counter_us_to_ticks(
		cec_bitbang_tmr_dev,
		(interrupt_time.val - prev_interrupt_time.val));
}
