...
1. Add the current date to the new `npcx_monitor.bin` filename. For example
  `npcx_monitor.bin-2025.02.07.bin`.
1. Upload the renamed monitor file to Google Cloud Storage (GCS).
    * Upload it to https://pantheon.corp.google.com/storage/browser/chromeos-localmirror/distfiles/cros_ec/npcx_monitor
    * Note that the name of the file must be unique.
1. Edit the NPCX monitor binary installed by the [servo dockerfile]
    * Find the instruction that fetches teh NPCX monitor from GSC
...
The `zephyr-npcx-monitor` ebuild is a dependency of the `ec-devutils` ebuild.
...
This the workflow to program a new firmware image over the UART interface.
...
write - typically `ec.bin`
1. De-assert the flash program pin.
1. Reset the EC
1. Your new image should boot and run

The workflow to perform flash updates over the UART interface is managed
by the `flash_ec` script.
