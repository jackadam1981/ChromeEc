...
static void check_reset_cause(void)
{
	...
	case VCC1_RST_PIN:
		if (IS_ENABLED(CONFIG_BOARD_RESET_AFTER_POWER_ON)) {
			/* The first reset pin reset treat as a power-up. */
			if ((system_flags & EC_RESET_FLAG_HAD_RESET_PIN) == 0) {
				/*
				...
				chip_flags |= EC_RESET_FLAG_HAD_RESET_PIN;
			} else {
				/*
	...
	if (IS_ENABLED(CONFIG_BOARD_RESET_AFTER_POWER_ON)) {
		if (system_flags & EC_RESET_FLAG_HAD_RESET_PIN) {
			chip_flags |= EC_RESET_FLAG_HAD_RESET_PIN;
			system_flags &= ~EC_RESET_FLAG_HAD_RESET_PIN;
		}
	}
...
void system_reset(int flags)
{
	...
	if (IS_ENABLED(CONFIG_BOARD_RESET_AFTER_POWER_ON)) {
		save_flags |= chip_read_reset_flags() &
			      EC_RESET_FLAG_HAD_RESET_PIN;
	}
