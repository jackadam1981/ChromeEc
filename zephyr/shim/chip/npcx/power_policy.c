...
static int command_idle_stats(int argc, char **argv)
{
	...
	printk("Num idle calls that sleep:           %d\n", npcx_idle_count(false));
	printk("Num idle calls that deep-sleep:      %d\n", npcx_idle_count(true));
	printk("Time spent in deep-sleep:            %llu\n",
		npcx_clock_get_sleep_ticks());
	printk("Total time on:                       %llu\n", ts.val);
	return EC_SUCCESS;
}
