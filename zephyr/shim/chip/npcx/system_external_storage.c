...
void system_jump_to_booter(void)
{
	...
	CPRINTS("magic_numer=0x%x", magic_number);
	magic_number = 0;
...
//	);
//#else
	CPRINTS("download_from_flash(flash_offset=0x%x, ram_addr=0x%x, size=0x%x, jump_to=0x%x)",
		flash_offset, CONFIG_PROGRAM_MEMORY_BASE, flash_used, addr_entry);
	download_from_flash(
		flash_offset,      /* The offset of the data in spi flash */
		CONFIG_PROGRAM_MEMORY_BASE, /* RAM Addr of downloaded data */
		flash_used,        /* Number of bytes to download      */
