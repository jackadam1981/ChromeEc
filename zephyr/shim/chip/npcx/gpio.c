...
int gpio_config_unused_pins(void)
{
	for (size_t i = 0; i < ARRAY_SIZE(unused_pin_configs); ++i) {
		...
		/*
		 * The floating IOs cause the leakage current. Set unused pins
		 * as input with internal PU to prevent extra power consumption.
		 */
		rv = gpio_pin_configure(dev, unused_pin_configs[i].pin,
