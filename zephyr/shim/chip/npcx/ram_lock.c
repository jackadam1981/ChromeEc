...
#define NPCX_RAM_LK_CTL 0x02F
#define NPCX_RAM_LK_FETCH_BF 1
#define NPCX_RAM_WRITE_LOCK(n) (0x050 + (n))
#define NPCX_RAM_FETCH_LOCK(n) (0x070 + (n))
...
static int set_lock_bit(uint32_t offset, uint8_t lock_bit)
{
	...
	ret = syscon_write_reg(mdc_dev, offset, lock_bit);
...
{
	uint32_t data_cached = 0x0;
	int ret;

	ret = syscon_read_reg(mdc_dev, NPCX_RAM_LK_CTL, &data_cached);

	if (ret != 0) {
		return ret;
	}

	if ((data_cached & BIT(NPCX_RAM_LK_FETCH_BF)) == 0) {
		data_cached |= BIT(NPCX_RAM_LK_FETCH_BF);
		ret = syscon_write_reg(mdc_dev, NPCX_RAM_LK_CTL, data_cached);
...
int ram_lock_config_lock_region(uint8_t region, uint32_t addr, uint32_t size)
{
	...
	uint8_t natural_alignment = LOG2(NPCX_RAM_BLOCK);
	...
	subregion_size = NPCX_RAM_SECTOR;
