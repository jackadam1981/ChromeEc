...
enum ec_error_list otp_key_read(uint8_t *key_buffer)
{
    ...
    const size_t otp_key_index = 2;
    uintptr_t otp_address = 0x64;

	HAL_OTP_Read32(otp_address + otp_key_index * OTP_KEY_SIZE_BYTES / sizeof(uint32_t), OTP_KEY_SIZE_BYTES / sizeof(uint32_t), (uint32_t *)key_buffer);
