...
#define FAILURE_RESP_STATUS_MASK 0x80
...
/* (256 * 1024 = 256KB = 262144B) */
#define MAX_CHUNK_SIZE 262144U
#define MAX_CHUNK_BUFF_OFFSET 0x3FFFF
#define BOARD_INIT_ERR 1
...
#define HDR_PKT_SIZE 240
#define PGM_PKT_PAYLOAD_SIZE 128
#define PKT_BUF_MAX_SIZE 250
			      ...
			      uint8_t *tx_buff)
{
	...
	tx_buff[RESP_CMD_POS] = command | FAILURE_RESP_STATUS_MASK;

	switch (type) {
	case HEADER_PACKET_CRC_FAILURE:
	case PGM_PACKET_CRC_FAILURE: {
		tx_buff[RESP_FAILURE_STATUS_POS] = CRC_MISMATCH;
		break;
	}
	case HEADER_PACKET_ILLEGAL_LENGTH:
	case PGM_PACKET_ILLEGAL_LENGTH: {
		tx_buff[RESP_FAILURE_STATUS_POS] =
			PACKET_ILLEGAL_PAYLOAD_LEN_POS;
		break;
	}
	case HEADER_PACKET_ILLEGAL_OFFSET:
	case PGM_PACKET_ILLEGAL_OFFSET: {
		tx_buff[RESP_FAILURE_STATUS_POS] =
			PACKET_ILLEGAL_HEADER_OFFSET_POS;
		break;
	}
	case SPI_OPERATION_FAILURE: {
		tx_buff[RESP_FAILURE_STATUS_POS] = SPI_OPERATION_FAILURE_POS;
		break;
	}
	case PGM_FLASH_DATA_LEN_INCORRECT: {
		tx_buff[RESP_FAILURE_STATUS_POS] =
			PGM_FLASH_DATA_LEN_INCORRECT_POS;
		break;
	}
	case HEADER_PACKET_INVALID: {
		tx_buff[RESP_FAILURE_STATUS_POS] = HEADER_PACKET_INVALID_POS;
		break;
	}
	default:
		break;
...
uint32_t receive_packet(uint8_t command, uint8_t *data_buffer, uint8_t *pkt_len)
{
	uint32_t ret;
	uint8_t len;

	data_buffer[PKT_CMD_IDX] = command;
...
uint8_t verify_pkt_integrity(uint8_t *data_buffer, uint8_t pkt_len)
{
	uint8_t crc32_status = CRC_MATCH;
	...
	for (i = 0; i < CRC32_SIZE; i++) {
		host_crc32 += ((uint32_t)data_buffer[len + i] << (i << 3));
	}

	if (command_crc32 != host_crc32) {
		crc32_status = CRC_MISMATCH;
	}

	return crc32_status;
}

uint8_t extract_program_data(uint8_t *data_buffer, uint32_t offset,
			     uint8_t *len, uint8_t *pkt_buffer)
{
	uint8_t i;
	...
	if (pkt_buffer[PKT_PAYLOAD_LEN_IDX] != PGM_PKT_PAYLOAD_SIZE) {
		send_response(PGM_PACKET_ILLEGAL_LENGTH, CMD_PRG_FILE);
		return PGM_PACKET_ILLEGAL_LENGTH;
	}

	for (i = 0; i < PKT_OFFSET_LEN; i++) {
		ptr_off_set +=
			((uint32_t)pkt_buffer[PKT_OFFSET_IDX + i] << (i << 3));
	}

	if (ptr_off_set != offset) {
		send_response(PGM_PACKET_ILLEGAL_OFFSET, CMD_PRG_FILE);
...
uint32_t verify_program_data(uint8_t *pgm_buff, uint32_t flash_addr,
			     uint32_t progrm_length)
{
	...
	for (sector_address = flash_addr; sector_address < progrm_length;) {
		spi_err_flag = spi_splash_check_sector_content_same(
			sector_address, input_data_offset, &status, pgm_buff);
...
uint32_t program_data(uint32_t flash_addr, uint8_t *pgm_buffer,
		      uint32_t progrm_length)
{
	uint32_t ret;
	uint32_t sector_address;
	...
	for (sector_address = flash_addr; sector_address < progrm_length;) {
		ret = spi_splash_check_sector_content_same(
			sector_address, input_data_offset, &status, pgm_buffer);
		if (ret != NO_ERROR)
			return ret;
		...
		if (status) {
			...
			ret = spi_flash_program_sector(
				sector_address, input_data_offset, pgm_buffer);
			if (ret != NO_ERROR)
				return ret;
		}
...
uint8_t extract_header_info(uint32_t *read_hdr_ptr,
...
void process_rxd_data(uint8_t rx_data)
{
	...
	static uint8_t data_buffer[256 * 1024]
		__attribute__((section(".buffer_256K")));
	static struct header_conf hdr_info;
	static uint32_t total_offset;
	uint32_t progrm_length;
	uint32_t status;
	enum failure_resp_type failure_type;
	...
	case STATE_WAIT_FOR_HEADER_CMD: {
		...
		status = receive_packet(rx_data, pkt_buff, &len);
		if (status != NO_ERROR) {
			state = STATE_WAIT_FOR_ACK1;
			break;
		}
		...
		if (status == CRC_MISMATCH) {
			...
			send_response(HEADER_PACKET_CRC_FAILURE, rx_data);
			break;
		}
		...
		if (status == HEADER_PACKET_INVALID) {
			state = STATE_WAIT_FOR_ACK1;
			send_response(HEADER_PACKET_INVALID, rx_data);
			break;
		}
	...
	case STATE_WAIT_FOR_PROGRAM_CMD: {
		...
		if (!(total_offset & MAX_CHUNK_BUFF_OFFSET)) {
			...
			if (status != NO_ERROR) {
				state = STATE_WAIT_FOR_ACK1;
				failure_type = SPI_OPERATION_FAILURE;
		...
		send_response(failure_type, rx_data);
		break;
	}
	default: {
		/* Nothing to do */
