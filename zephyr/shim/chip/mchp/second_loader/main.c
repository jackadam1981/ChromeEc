...
uint32_t receive_packet(uint8_t *data_buffer, uint8_t *pkt_len)
{
	uint32_t ret;
	uint8_t len;
			     ...
			     uint8_t *len, uint8_t *pkt_buffer)
{
	...
	if (pkt_buffer[PKT_PAYLOAD_LEN_IDX] != PGM_PKT_PAYLOAD_SIZE) {
		send_response(PACKET_PAYLOAD_ILLEGAL_LEN, CMD_PRG_FILE);
		return PACKET_PAYLOAD_ILLEGAL_LEN;
	}
...
void process_rxd_data(uint8_t rx_data)
{
	...
	case STATE_WAIT_FOR_HEADER_CMD:
		...
		if (pkt_buff[PKT_PAYLOAD_LEN_IDX] != HDR_PKT_PAYLOAD_SIZE) {
			send_response(PACKET_PAYLOAD_ILLEGAL_LEN, CMD_HDR_FILE);
			break;
		}
