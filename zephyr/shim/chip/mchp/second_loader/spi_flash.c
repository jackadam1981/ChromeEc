...
#define SPI_WAIT_BUSY_TIMEOUT_ERR 10
#define MBX_DMA_WRITE_ERR 11
#define MBX_DMA_READ_ERR 12
#define POLL_STATUS_TO_ERR 13
...
static uint32_t qmspi_poll_for_status(uint32_t status_val)
{
...
static uint32_t qmspi_reset(void)
{
...
static uint32_t qmspi_read_status(void)
{
...
static uint32_t qmspi_wait_for_not_busy(uint32_t extended_timeout)
{
...
static uint32_t qmspi_dma_write(uint32_t flash_addr, uint8_t *data_src,
				uint32_t length)
{
	/*
				  ...
				  uint8_t *input_data_ptr)
{
	...
	for (page_address = 0; page_address < SECTOR_SIZE;) {
		...
		if (ret != NO_ERROR) {
			spi_err_flag = MBX_DMA_WRITE_ERR;
			return spi_err_flag;
		}
...
static uint32_t qmspi_dma_read(uint32_t addr, uint32_t length,
			       uint8_t *read_buff)
{
					      ...
					      uint8_t *input_data_ptr)
{
	...
	static uint8_t read_data_ptr[4 * 1024]
		__attribute__((section(".buffer_4K")));
	uint32_t ret;
	uint32_t i;
	uint32_t spi_err_flag = NO_ERROR;
	...
	if (ret != NO_ERROR) {
		spi_err_flag = MBX_DMA_READ_ERR;
		return spi_err_flag;
	}
	dwn_loaded_data_ptr = (uint8_t *)(input_data_ptr + ip_offset);
