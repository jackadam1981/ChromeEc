...
static uint32_t qmspi_dma_write(uint32_t addr, uint8_t *data_src,
				uint32_t length);
...
static uint32_t qmspi_poll_for_status(uint32_t stat);
					      ...
					      uint8_t *status)
{
	...
	input_data_ptr = (uint8_t *)&DATA_IO_BUFFER;
	read_data_ptr = (uint8_t *)&READ_SECTOR_BUFFER;
	...
	*status = 0;
	...
	for (i = 0; i < SECTOR_SIZE; i++) {
		 ...
		 */
		if (read_data_ptr[i] != DwnloadedDataPtr[i]) {
			*status = 1;
			break;
		}
	}
