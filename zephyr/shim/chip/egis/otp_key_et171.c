...
/*
 * The base Word offset for the EGIS ET171 EGIS_KEY area in the OTP memory map.
 * This entire area is locked.
 */
#define EGIS_ET171_OTP_EGIS_KEY_BASE_WORD_OFFSET 0x64

/* Index of the secret key (EGIS_KEY3) within the EGIS_KEY OTP region */
#define EGIS_ET171_OTP_EGIS_KEY_INDEX 2

/*
 * The size of the EGIS ET171 OTP key sub-unit, expressed in words (0x8 words).
 * For a 4-byte word, this corresponds to 32 bytes (8 words * 4 bytes/word).
 */
#define EGIS_ET171_OTP_KEY_WORD_SIZE 0x8
BUILD_ASSERT(OTP_KEY_SIZE_BYTES / sizeof(uint32_t) ==
	     EGIS_ET171_OTP_KEY_WORD_SIZE);
BUILD_ASSERT(OTP_KEY_SIZE_BYTES % sizeof(uint32_t) == 0);

static const uint32_t otp_key_word_len = EGIS_ET171_OTP_KEY_WORD_SIZE;

/* Calculated starting Word offset of EGIS_KEY3 in the OTP memory. */
static const uint32_t otp_key_word_addr =
	EGIS_ET171_OTP_EGIS_KEY_BASE_WORD_OFFSET +
	EGIS_ET171_OTP_EGIS_KEY_INDEX * EGIS_ET171_OTP_KEY_WORD_SIZE;
...
enum ec_error_list otp_key_read(uint8_t *key_buffer)
{
	...
	if (HAL_OTP_Read32(otp_key_word_addr, otp_key_word_len,
...
{
	__aligned(4) uint8_t otp_key_buffer[OTP_KEY_SIZE_BYTES] = { 0 };
