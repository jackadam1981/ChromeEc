 ...
 */
static const uint32_t egis_et171_otp_egis_key_base_word_offset = 0x64;

/* Index of the secret key (EGIS_KEY3) within the EGIS_KEY OTP region */
static const uint32_t egis_et171_otp_egis_key_index = 2;
 ...
 * bytes/word).
 */
static const uint32_t egis_et171_otp_key_word_size = 0x8;

/*
 * Note: The BUILD_ASSERT for word size was removed because
 * egis_et171_otp_key_word_size is now a variable, not a constant expression.
 */
...
enum ec_error_list otp_key_read(uint8_t *key_buffer)
{
	...
	if (HAL_OTP_Read32(otp_key_word_addr, egis_et171_otp_key_word_size,
