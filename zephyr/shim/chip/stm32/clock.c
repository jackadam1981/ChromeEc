...
#define PLL_FREQ	(((PLLSRC_FREQ)*(STM32_PLL_N_MULTIPLIER))/((STM32_PLL_M_DIVISOR)*(STM32_PLL_P_DIVISOR)))
...
test_mockable void clock_enable_module(enum module_id module, int enable)
{
	if (module == MODULE_FAST_CPU) {
		if (enable) {
			...
			/* Set HPRE to 2 not to extend AHB max freq. Also update the flash
			 * latency acording to the new AHB freq.
			 */
			LL_RCC_SetAHBPrescaler(RCC_AHB_DIV(PLL_RCC_AHB_DIV));
			// TODO assert na max ahb?

			// TODO limit APB freq not to extend 120MHz max and restore from DTS?
			//&rcc {
			...
			if (IS_ENABLED(CONFIG_TIMER_READS_ITS_FREQUENCY_AT_RUNTIME)) {
			...
			/* Set HPRE to the original value and update flash latency. */
