...
static void ps8811_db_addr_init(void)
{
	switch (usb_db_type) {
	case FW_USB_DB_USB4_ANX7452_V2:
		usba_ps8811[USBA_PORT_A1].i2c_addr_flags =
			PS8811_I2C_ADDR_FLAGS1;
		break;
	default:
		break;
	}
}
DECLARE_HOOK(HOOK_INIT, ps8811_db_addr_init, HOOK_PRIO_DEFAULT);
...
static int usba_retimer_init(int port)
{
	...
	if (rv) {
		CPRINTSUSB("A1: PS8811 retimer response fail! : %d", rv);
	}
...
static void board_resume_change(struct ap_power_ev_callback *cb,
