...
/*
 * Semaphore to control the animation loop. The shell command 'on' will give
 * the semaphore, and the thread will take it to run one full animation cycle.
 */
K_SEM_DEFINE(animation_sem, 1, 1);
...
void led_strip_thread(void *p1, void *p2, void *p3)
{
	...
	while (1) {
		// Block here until the shell command 'on' gives the semaphore.
		k_sem_take(&animation_sem, K_FOREVER);

		LOG_INF("Starting animation cycle...");
		...
		while (1) {
			...
			// If the semaphore was taken by 'off' or 'set', break the loop.
			if (k_sem_take(&animation_sem, K_NO_WAIT) != 0) {
				// Semaphore was not available, continue animation
				k_sem_give(&animation_sem);
			} else {
				// Semaphore was taken, so stop animating
				LOG_INF("Animation stopped by command.");
				break;
			}
		}
	}
...
K_THREAD_DEFINE(
    ...
    7,                                      // 5. Thread priority (0=highest, 15=lowest)
...
{
	// If the semaphore count is 0, it means the thread is sleeping.
	if (k_sem_count_get(&animation_sem) == 0) {
		k_sem_give(&animation_sem);
...
{
	// Take the semaphore. This will cause the animation loop to break.
	if (k_sem_take(&animation_sem, K_NO_WAIT) == 0) {
		shell_print(sh, "LED animation stopping...");
	} else {
		// If we can't take it, it means it's already 0 (thread is stopped).
...
static int cmd_effect_set(const struct shell *sh, size_t argc, char **argv)
{
	...
	set_solid_color(&color, K_NO_WAIT);
