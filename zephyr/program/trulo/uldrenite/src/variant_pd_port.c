...
/* Uldrenite sub-board hardware configuration */
...
__override uint8_t board_get_usb_pd_port_count(void)
{
	...
	ccprints("cached_usb_pd_port_count = %d ", cached_usb_pd_port_count);
	return cached_usb_pd_port_count;
}
...
enum uldrenite_typec_type uldrenite_get_typec_type(void)
{
	...
	switch (val) {
	default:
		ccprints("No typec port defined, default two type-c ports");
		break;
	case TWO_PORTS:
		uldrenite_cached_typec_type = ULDRENITE_TWO_C;
		ccprints("USB two type-C ");
		break;

	case ONE_PORT:
		uldrenite_cached_typec_type = ULDRENITE_ONE_C;
		ccprints("USB type C only");
		break;
	}
	return uldrenite_cached_typec_type;
}
