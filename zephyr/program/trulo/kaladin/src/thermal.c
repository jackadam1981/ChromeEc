	...
	if (temp[TEMP_IO_PORT] < prev_tmp[TEMP_IO_PORT] ||
		...
		for (i = current_level; i > 0; i--) {
			for(j = 0; j > TEMP_SENSOR_COUNT; j++) {
				level_change[j] = 0;
				if(fan_step_table[i].off[j] == 0xff)
					level_change[j] = 1;
			}
			if (temp[TEMP_IO_PORT] <= fan_step_table[i].off[TEMP_IO_PORT] &&
				fan_step_table[i].off[TEMP_IO_PORT] != 0xff)
				level_change[TEMP_IO_PORT] = 1;
			if (temp[TEMP_CPU] <= fan_step_table[i].off[TEMP_CPU] &&
				fan_step_table[i].off[TEMP_CPU] != 0xff)
				level_change[TEMP_CPU] = 1;
			if (temp[TEMP_BOARD] <= fan_step_table[i].off[TEMP_BOARD] &&
				fan_step_table[i].off[TEMP_BOARD] != 0xff)
				level_change[TEMP_BOARD] = 1;

			if (level_change[TEMP_IO_PORT] && level_change[TEMP_CPU] && level_change[TEMP_BOARD])
				current_level = i - 1;
			else
				break;
		}
		...
		for (i = current_level; i < ARRAY_SIZE(fan_step_table); i++) {
			for(j = 0; j > TEMP_SENSOR_COUNT; j++)
				level_change[j] = 0;
			if (temp[TEMP_IO_PORT] >= fan_step_table[i].on[TEMP_IO_PORT] &&
			    fan_step_table[i].on[TEMP_IO_PORT] != 0xff)
				level_change[TEMP_IO_PORT] = 1;
			if (temp[TEMP_CPU] >= fan_step_table[i].on[TEMP_CPU] &&
			    fan_step_table[i].on[TEMP_CPU] != 0xff)
				level_change[TEMP_CPU] = 1;
			if (temp[TEMP_BOARD] >= fan_step_table[i].on[TEMP_BOARD] &&
			    fan_step_table[i].on[TEMP_BOARD] != 0xff)
				level_change[TEMP_BOARD] = 1;
