...
static int try_vendor_command(const uint8_t *buffer, size_t count)
{
	...
	if (update_pdu_valid(&cmd_buffer->cmd,
		...
		one_wire_uart_send(one_wire_uart, ROACH_CMD_UPDATER_COMMAND, &response, response_size);
	}
...
void mock_updater(const uint8_t *payload, int len)
{
	...
	if (state == RX_IDLE) {
		...
		one_wire_uart_send(one_wire_uart, ROACH_CMD_UPDATER_COMMAND, (uint8_t*)&u.startup_resp, resp_size);
		return;
		...
		if (len == 4) {
			...
			if (command == UPDATE_DONE) {
				...
				one_wire_uart_send(one_wire_uart, ROACH_CMD_UPDATER_COMMAND, &resp_value, 1);
				state = RX_IDLE;
				return;
			}
		}
		...
		if (block_size <= sizeof(struct update_command) ||
			...
			one_wire_uart_send(one_wire_uart, ROACH_CMD_UPDATER_COMMAND, &err, 1);
			return;
		}
		...
		*/
		block_buffer[0] = 0;
		one_wire_uart_send(one_wire_uart, ROACH_CMD_UPDATER_COMMAND, &block_buffer[0], 1);
		state = RX_OUTSIDE_BLOCK;
	}
}

static void recv_cb(uint8_t cmd, uint8_t *payload, int length)
{
	/* TODO: handle ROACH_CMD_SUSPEND/RESUME after touchpad driver ready */

	if (cmd == ROACH_CMD_UPDATER_COMMAND) {
		mock_updater(payload, length);
	}
}
