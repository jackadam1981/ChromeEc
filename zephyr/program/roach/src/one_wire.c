...
static int try_vendor_command(const uint8_t *buffer, size_t count)
{
	...
	if (update_pdu_valid(&cmd_buffer->cmd,
		...
		board_uart_tx(ROACH_CMD_UPDATER_COMMAND, &response, response_size);
	}
...
void mock_updater(const uint8_t *payload, int len)
{
	...
	if (state == RX_IDLE) {
		...
		board_uart_tx(ROACH_CMD_UPDATER_COMMAND, (uint8_t*)&u.startup_resp, resp_size);
		return;
		...
		if (len == 4) {
			...
			if (command == UPDATE_DONE) {
				...
				board_uart_tx(ROACH_CMD_UPDATER_COMMAND, &resp_value, 1);
				state = RX_IDLE;
				return;
			}
		}
		...
		if (block_size <= sizeof(struct update_command) ||
			...
			board_uart_tx(ROACH_CMD_UPDATER_COMMAND, &err, 1);
			return;
		}
		...
		*/
		block_buffer[0] = 0;
		board_uart_tx(ROACH_CMD_UPDATER_COMMAND, &block_buffer[0], 1);
...
#ifndef CONFIG_PLATFORM_EC_USB_HID_TOUCHPAD
