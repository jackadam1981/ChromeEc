...
struct chg_curr_step {
	int on_temp; /* on temperature (Degree C) */
	int off_temp; /* off temperature (Degree C) */
	int curr; /* charge current (mA) */
};

static const struct chg_curr_step chg_curr_table[] = {
	{ .on_temp = 0, .off_temp = 0, .curr = __INT32_MAX__ }, /* normal */
	{ .on_temp = 56, .off_temp = 50, .curr = 2000 },
};
...
__override int board_charger_profile_override(struct charge_state_data *curr)
{
	...
	if (current_temp < previous_temp) {
		for (i = current_level; i >= 0; i--) {
			if (current_temp <= chg_curr_table[i].off_temp)
				current_level = i - 1;
			else
				break;
		}
		...
		for (i = current_level + 1; i < NUM_CHG_CURRENT_LEVELS; i++) {
			if (current_temp >= chg_curr_table[i].on_temp)
				current_level = i;
			else
				break;
		}
	...
	LOG_WRN("level: %d, batt_current: %d, limit_current: %d", current_level,
		curr->requested_current, chg_curr_table[current_level].curr);
#endif
