...
/*
 * Due to the delay of the 5V rail enabling until 5V@3A is ready
 * ,the ppc_init may run when the PPC is not powered on. So here
 * rerunning the ppc_init function after the 5V rail enables to
 * prevent Type-C port no function.
 */
...
static void board_chipset_pre_init(void)
{
	...
	if (!pp5000_inited) {
		...
		/* rerun the ppc_init after turn on 5v */
