...
// static int time[ROW_NUM][COL_NUM];
...
static void average_tempature(void)
{
	...
	// }
	CPRINTS("---cpu : %d,charge : %d----", cpu_temp_c, charger_temp_c);

	if (cpu_temp_c >= 55) {
		if (charger_temp_c >= 75 && temp_zone != TEMP_ZONE_1) {
			// clear_remaining_array(time, ROW_NUM, 0, 1);
			increment_level++;
			decline_level = 0;
		}
		else if (charger_temp_c < 70 && temp_zone != TEMP_ZONE_0) {
	...
	// 	}
	// }
	CPRINTS("---increment : %d,decline : %d----", increment_level, decline_level);
	if (increment_level == 3) {
		temp_zone = 1;
		increment_level = 0;
	}
	else if (decline_level == 1) {
		temp_zone = 0;
		decline_level = 0;
	}
