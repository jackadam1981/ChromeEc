...
static void sim_det_control(bool enabled) {
	write_gpio(GPIO_LABEL_SIM_DET_CNTRL, 0, enabled);
}

static void update_mux(void)
{
	struct mux_state mux = state.mux_states[0].mux;
	sim_det_control(false);
