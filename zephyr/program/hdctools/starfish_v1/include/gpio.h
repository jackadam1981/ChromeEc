...
/*
 * Defines the comma ',' character in the preprocessor and is needed for
 * other macro expansions. The presubmit check does not like this or any
 * other variation found which actually compiles correctly.
 */
#define COMMA (, )

/*
 * Internal definitions used to help create GPIO_CTX structs
 * by navigating the device tree
 */
...
/*
 * Helper macro that allows us to create a array of structs for each GPIO
 * within a node label. GPIOs are tagged with one of the enum GPIO_LABEL
 * values and an index which allow us to identify groupings.
 */
