...
int cbi_board_override(enum cbi_data_tag tag,
...
void check_fan_status(void)
{
	...
	LOG_INF("Fan rpm : %d", rpm);

	dptf_set_fan_duty_target(100);
	duty = fan_get_duty(0);
	LOG_INF("Fan duty : %d", duty);;
	rpm = fan_get_rpm_actual(0);
	if (rpm != 0) {
		override_flag = true;
	} else {
...
void set_fan_status(void)
{
	...
	dptf_set_fan_duty_target(100);
	rpm = fan_get_rpm_actual(0);
	LOG_INF("Fan1 rpm : %d", rpm);
	hook_call_deferred(&check_fan_status_data, 1000 * MSEC);
}
...
test_export_static void fan_init(void)
{
	...
	if (val != FW_FAN_PRESENT) {
		...
		gpio_pin_configure_dt(GPIO_DT_FROM_NODELABEL(gpio_fan_enable),
				      GPIO_OUTPUT);
		dptf_set_fan_duty_target(100);
		hook_call_deferred(&set_fan_status_data, 2000 * MSEC);
	} else {
		/* Configure the fan enable GPIO */
		gpio_pin_configure_dt(GPIO_DT_FROM_NODELABEL(gpio_fan_enable),
				      GPIO_OUTPUT);
