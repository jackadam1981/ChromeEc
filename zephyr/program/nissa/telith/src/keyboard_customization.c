...
/* telith keyboard without numpad
 * G (Win key) : 30 (row:3, col: 0)
 * Fn          : 127(row:0, col:16)
 */
static uint16_t telith_scancode_set2[KEYBOARD_COLS_MAX][KEYBOARD_ROWS] = {
	...
	/*  7 */ { 0x0000, 0x0000, 0x005d, 0x0000, 0x0000, 0x0012, 0x0000, 0x0059 },
...
/* teltic keyboard with numpad
 * G (Win key) : 30 (row:3, col: 0)
 * Fn          : 127(row:4, col:10)
 */
static uint16_t teltic_scancode_set2[KEYBOARD_COLS_MAX][KEYBOARD_ROWS] = {
	/* KSO      KSI0    KSI1    KSI2    KSI3    KSI4    KSI5    KSI6    KSI7
	 */
	/*  0 */ { 0x0000, 0x0000, 0x0000, 0xe01f, 0x0000, 0x0000, 0x0000,
		   0x0000 },
...
static uint16_t (*scancode_set2)[KEYBOARD_ROWS] = telith_scancode_set2;
...
test_export_static void keyboard_matrix_init(void)
{
	...
	case FW_KB_NUMERIC_PAD_PRESENT:
		scancode_set2 = teltic_scancode_set2;
		LOG_INF("teltic keyboard matrix");
		break;
	case FW_KB_NUMERIC_PAD_ABSENT:
		scancode_set2 = telith_scancode_set2;
		LOG_INF("telith keyboard matrix");
		break;
	default:
