...
int fan_table_to_rpm(int fan, int *temp)
{
	...
	if (temp[TEMP_MEMORY] < prev_tmp[TEMP_MEMORY] ||
		...
		for (i = current_level; i > 0; i--) {
			if (temp[TEMP_MEMORY] < fan_step_table[i].off[TEMP_MEMORY] &&
			    temp[TEMP_AMBIENT] < fan_step_table[i].off[TEMP_AMBIENT] &&
			    temp[TEMP_CHARGER] <
		...
		for (i = current_level; i < ARRAY_SIZE(fan_step_table); i++) {
			if (temp[TEMP_MEMORY] > fan_step_table[i].on[TEMP_MEMORY] ||
			    (temp[TEMP_AMBIENT] > fan_step_table[i].on[TEMP_AMBIENT] &&
			     temp[TEMP_CHARGER] >
