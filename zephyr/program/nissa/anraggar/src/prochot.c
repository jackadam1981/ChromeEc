...
#define PROC_HOLD_CURR (-4700) /* mA */
#define PROC_RLS_CURR (-4000) /* mA */
#define BATTCURR_CNT 4 /* Read the battery discharge current counter */
#define BATT_LVL 51 /* Battery level percent(%) */
...
static void update_prochot_deferred(void)
{
	...
	/* Check battery level every 60 seconds when battery level is enough */
	if (charge_get_percent() > BATT_LVL) {
		// CPRINTS("over 50%%, tracking");
		hook_call_deferred(&update_prochot_deferred_data, 60 * SECOND);
		return;
	}
	...
	if (hold_cnt == BATTCURR_CNT) {
		if (proc_status == true) {
			hook_call_deferred(&update_prochot_deferred_data,
					   500 * MSEC);
			return;
		}
		...
		/* If the battery is switched to charging state,
		 * the prochot should be released.
		 */
		...
		CPRINTS("Not discharge, release prochot.");
		gpio_pin_set_dt(GPIO_DT_FROM_NODELABEL(gpio_ec_prochot_odl), 1);
		proc_status = false;
		charge_status = true;
	}
...
static void check_batt_current(void)
{
	...
	/* Deferred 5s to avoid state conflict */
