...
void board_to_desired_mode(enum tablet_mode_override mode)
{
	int tablet_mode = tablet_get_mode();
	bool mode_need_change = false;

	if (mode == TABLET_MODE_FORCE_TABLET && !tablet_mode) {
		mode_need_change = true;
		tablet_mode_set_override(TABLET_MODE_FORCE_TABLET);
	}
	if (mode == TABLET_MODE_FORCE_CLAMSHELL && tablet_mode) {
		mode_need_change = true;
		tablet_mode_set_override(TABLET_MODE_FORCE_CLAMSHELL);
	}
	if (mode_need_change) {
		/* Add delay when the mode changed to prevent the OS
		 * lost button event or scancode.
		 */
		k_sleep(K_MSEC(100));
	}
}
	...
	for (i = 0; i < BOARD_BUTTON_COUNT; i++) {
		...
		if (state[i].debounce_time <= time_now) {
			...
			if (state[i].debounced_pressed != new_pressed) {
				...
				if (i == BUTTON_CAMERA) {
					/*
					 * BUTTON_CAMERA is for capture image
					 * when camera app is open in tablet
					 * mode.
					 */
					if (new_pressed) {
						board_to_desired_mode(
...
static int console_command_button(int argc, const char **argv)
{
	...
	for (argv_idx = 1; argv_idx < argc; argv_idx++) {
		if (!strcasecmp(argv[argv_idx], "camera"))
			button = button_present(KEYBOARD_BUTTON_CAMERA);
...
DECLARE_CONSOLE_COMMAND(board_button, console_command_button,
			"camera|back|overview|launcher msec",
			"Simulate button press");

const struct board_button_config board_buttons[BOARD_BUTTON_COUNT] = {
	[BUTTON_CAMERA] = {
		.name = "Camera",
		.type = KEYBOARD_BUTTON_CAMERA,
		.gpio = GPIO_SIGNAL(DT_NODELABEL(gpio_camera_btn_odl)),
