...
static bool thermal_solution = FW_THERMAL_6W;
...
static const struct fan_step fan_step_table_15w[] = { DT_FOREACH_CHILD(
	DT_INST(1, cros_ec_fan_steps), FAN_TABLE_ENTRY) };

static const struct fan_step *fan_step_table;

#define NUM_FAN_LEVELS ARRAY_SIZE(fan_step_table_6w)

BUILD_ASSERT(ARRAY_SIZE(fan_step_table_6w) == ARRAY_SIZE(fan_step_table_15w));
...
int fan_table_to_rpm(int fan, int *temp)
{
	...
	if (thermal_solution == FW_THERMAL_6W)
