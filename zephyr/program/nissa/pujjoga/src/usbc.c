...
__override bool pd_check_vbus_level(int port, enum vbus_level level)
{
	 ...
	 * VBUS Detection.
	 */
	tcpc_rv = tcpci_get_vbus_voltage(port, &tcpc_vbus_voltage);

	/* If TCPC is not powered, check immediately for PPC presence verfiying
	 * here might save some I2C reads */
	if (tcpc_rv == EC_ERROR_NOT_POWERED)
		ppc_vbus_present = ppc_is_vbus_present(port);
	...
	} else if (tcpc_rv == EC_ERROR_NOT_POWERED) {
		/* Fallback: PPC digital presence */
		switch (level) {
		case VBUS_PRESENT:
			return ppc_vbus_present;
		case VBUS_SAFE0V:
		case VBUS_REMOVED:
			return !ppc_vbus_present;
		}
