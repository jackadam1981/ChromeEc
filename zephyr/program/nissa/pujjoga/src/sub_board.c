...
test_export_static enum pujjoga_sub_board_type cached_sub_board =
	PUJJOGA_SB_UNKNOWN;
/*
...
enum pujjoga_sub_board_type pujjoga_get_sb_type(void)
{
	 ...
	 * Return cached value.
	 */
	if (cached_sub_board != PUJJOGA_SB_UNKNOWN)
		return cached_sub_board;

	cached_sub_board = PUJJOGA_SB_NONE; /* Defaults to none */
	...
	default:
		...
		break;
	case FW_SUB_BOARD_1:
		cached_sub_board = PUJJOGA_SB_HDMI_A;
...
static void pujjoga_subboard_config(void)
{
	...
	BUILD_ASSERT(USB_PORT_ENABLE_COUNT == 1,
		     "Nissa assumes no more than 1 USB-A ports");
	/*
