...
/* Pujjoniru PPC/BC12 (RT1739) configuration */
...
{
	gpio_enable_dt_interrupt(GPIO_INT_FROM_NODELABEL(int_gpio_usb_c0_ppc_int_odl));
	gpio_enable_dt_interrupt(GPIO_INT_FROM_NODELABEL(int_gpio_usb_c1_ppc_int_odl));
}
DECLARE_HOOK(HOOK_INIT, board_usbc_init, HOOK_PRIO_POST_DEFAULT);

void c_ppc_interrupt(enum gpio_signal signal)
{
