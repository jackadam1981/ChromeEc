...
{
	/*
	 * ectool set brightness to 0xff (the maximum value) if we
	 * don't specify the brightness, set it to 100 (maximum value
	 * of PWM LED duty).
	 */
	if (duty == 0xff)
		duty = 100;
	...
	case LED_OFF:
		...
		break;
	case LED_WHITE:
		board_led_pwm_set_duty(&board_led_power, duty);
		break;
	default:
		break;
	}
