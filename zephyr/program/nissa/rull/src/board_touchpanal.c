...
static void set_tp_vcc_pin(void)
{
	gpio_pin_set_dt(GPIO_DT_FROM_NODELABEL(gpio_tp_vcc_od), value_en);
}
DECLARE_DEFERRED(set_tp_vcc_pin);
				    ...
				    struct ap_power_ev_data data)
{
	...
	// gpio_pin_set_dt(GPIO_DT_FROM_NODELABEL(gpio_ec_edp_bl_en_od), value);
}
...
static int install_backlight_handler(void)
{
	 ...
	 *Only when the board ID is greater than 0, the old sequence needs to be
	 *enabled.
	 */
