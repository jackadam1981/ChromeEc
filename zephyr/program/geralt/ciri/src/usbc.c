...
static volatile uint32_t prev_bitmap;
...
void manage_source_port(void)
{
	...
	for (p = 0; p < board_get_usb_pd_port_count(); p++) {
		if (tc_is_attached_src(p))
			atomic_or((atomic_t *)&source_port_bitmap, 1 << p);
		else
			atomic_clear_bits((atomic_t *)&source_port_bitmap,
					  1 << p);
	}
	...
	if (prev_bitmap == source_port_bitmap)
		return;
	prev_bitmap = source_port_bitmap;
