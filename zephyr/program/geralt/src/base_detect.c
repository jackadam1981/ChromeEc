...
static void base_update(bool attached)
{
	...
	if (!attached && (current_base_status == BASE_CONNECTED)) {
		...
		CPRINTS("ap off, base connected, lid enable#####");
	} else {
		enable_lid_detect(attached);
		if (attached)
			CPRINTS("base connected, lid enable#####");
		else
			CPRINTS("base disconnected, lid disenable#####");
	}

	if (chipset_in_state(CHIPSET_STATE_ANY_OFF)) {
		gpio_pin_set_dt(GPIO_DT_FROM_NODELABEL(en_ppvar_base_x), false);
...
#ifdef CONFIG_GERALT_LID_DETECTION_SELECTED
		   ||
		   (base_need_power && !chipset_in_state(CHIPSET_STATE_ANY_OFF))
#endif
	) {
		if (!debouncing) {
			debouncing = true;
