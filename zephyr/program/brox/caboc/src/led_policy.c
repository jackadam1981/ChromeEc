...
static struct pwm_led_tick_config {
	...
	uint64_t on_step_time;
	uint64_t off_step_time;
	uint64_t full_on_period;
	uint64_t full_off_period;
	int duty_step_inc;
	int duty_step_dec;
...
static void pwm_led_config_tick(struct pwm_led_tick_config *led_tick_config,
				enum led_color color, uint64_t on_step_time,
				uint64_t off_step_time, uint64_t full_on_period,
				uint64_t full_off_period, int duty_step_inc,
				int duty_step_dec)
{
...
static uint64_t pwm_led_tick_control(enum ec_led_id led_id)
{
	...
	if (led_tick_config->duty_step_is_inc) {
		led_tick_config->duty_val += led_tick_config->duty_step_inc;
		if (next_delay == 0)
			next_delay = led_tick_config->on_step_time;
	} else {
		led_tick_config->duty_val -= led_tick_config->duty_step_dec;
		if (next_delay == 0)
			next_delay = led_tick_config->off_step_time;
	}
...
static void batt_led_tick_control(struct k_work *work);
static K_WORK_DELAYABLE_DEFINE(batt_led_tick_control_data,
			       batt_led_tick_control);
...
static void batt_led_tick_control(struct k_work *work)
{
	...
	delay = pwm_led_tick_control(LED_BATTERY);
	elapsed = k_uptime_delta(&start);
	delay = (delay > elapsed) ? (delay - elapsed) : 0;
	k_work_schedule(&batt_led_tick_control_data, K_MSEC(delay));
}

#define BATT_LOW_LED_FULL_ON_PERIOD_MS 1000
#define BATT_LOW_LED_FULL_OFF_PERIOD_MS 1000
#define BATT_ERR_LED_FULL_ON_PERIOD_MS 500
#define BATT_ERR_LED_FULL_OFF_PERIOD_MS 500
#define BATT_FORCE_IDLE_LED_FULL_ON_PERIOD_MS 1000
#define BATT_FORCE_IDLE_LED_FULL_OFF_PERIOD_MS 1000

static void battery_led_mode_set(enum batt_led_mode new_mode)
{
	if (new_mode != BATT_MODE_NO_CHANGE) {
		batt_current_mode = new_mode;
		k_work_cancel_delayable(&batt_led_tick_control_data);
	}
	...
	if (led_auto_control_is_enabled(LED_BATTERY)) {
		...
		case BATT_MODE_DISCHG_LOW:
			...
			PWM_LED_CONFIG_TICK(&batt_led_tick_config, LED_AMBER, 0,
					    0, BATT_LOW_LED_FULL_ON_PERIOD_MS,
					    BATT_LOW_LED_FULL_OFF_PERIOD_MS, 0,
					    0);
			k_work_schedule(&batt_led_tick_control_data, K_NO_WAIT);
			break;
		case BATT_MODE_ERROR:
			/* Blinking amber 0.5s on, 0.5s off */
			PWM_LED_CONFIG_TICK(&batt_led_tick_config, LED_AMBER, 0,
...
static enum power_led_mode pwr_current_mode;

static void pwr_led_tick_control(struct k_work *work);
static K_WORK_DELAYABLE_DEFINE(pwr_led_tick_control_data, pwr_led_tick_control);
...
static void pwr_led_tick_control(struct k_work *work)
{
	...
	delay = pwm_led_tick_control(LED_POWER);
	elapsed = k_uptime_delta(&start);
	delay = (delay > elapsed) ? (delay - elapsed) : 0;
	k_work_schedule(&pwr_led_tick_control_data, K_MSEC(delay));
}
 ...
 */
#define PWR_LED_CPU_DELAY_MS 2000

/* 30ms time on/off step for 1.5s pulse on/off smoothly */
#define PWR_SUSPEND_LED_ON_STEP_TIME_MS 30
#define PWR_SUSPEND_LED_OFF_STEP_TIME_MS 30
#define PWR_SUSPEND_LED_FULL_OFF_PERIOD_MS 1500
#define PWR_SUSPEND_LED_PULSE_ON_PERIOD_MS 1500
#define PWR_SUSPEND_LED_PULSE_OFF_PERIOD_MS 1500

static void power_led_mode_set(enum power_led_mode new_mode)
{
	if (new_mode != PWR_MODE_NO_CHANGE) {
		pwr_current_mode = new_mode;
		k_work_cancel_delayable(&pwr_led_tick_control_data);
	}
	...
	if (led_auto_control_is_enabled(LED_POWER)) {
		...
		case PWR_MODE_SUSPEND:
			/*
			 * Breathing white with 1.5s pulse on/off, and 1.5s off
			 */
			PWM_LED_CONFIG_TICK(
				&pwr_led_tick_config, LED_WHITE,
				PWR_SUSPEND_LED_ON_STEP_TIME_MS,
				PWR_SUSPEND_LED_OFF_STEP_TIME_MS, 0,
				PWR_SUSPEND_LED_FULL_OFF_PERIOD_MS,
				PWR_SUSPEND_LED_PULSE_ON_PERIOD_MS,
				PWR_SUSPEND_LED_PULSE_OFF_PERIOD_MS);
			k_work_schedule(&pwr_led_tick_control_data, K_NO_WAIT);
			break;
		case PWR_MODE_OFF:
			led_set_color_duty(LED_POWER, LED_OFF, 0);
			break;
 ...
 */
K_TIMER_DEFINE(pwr_led_control_timer, NULL, NULL);
...
{
	k_timer_stop(&pwr_led_control_timer);
	k_timer_init(&pwr_led_control_timer, pwr_led_shutdown, NULL);
	k_timer_start(&pwr_led_control_timer, K_MSEC(PWR_LED_CPU_DELAY_MS),
		      K_FOREVER);
}
...
{
	k_timer_stop(&pwr_led_control_timer);
	k_timer_init(&pwr_led_control_timer, pwr_led_suspend, NULL);
	k_timer_start(&pwr_led_control_timer, K_MSEC(PWR_LED_CPU_DELAY_MS),
		      K_FOREVER);
}
