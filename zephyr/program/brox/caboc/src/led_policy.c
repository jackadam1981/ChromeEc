...
#define BATT_LOW_LED_PULSE_TICK_MS (60 * USEC_PER_MSEC)
#define BATT_CRI_LED_PULSE_TICK_MS (25 * USEC_PER_MSEC)
...
static void led_set_battery(void)
{
	...
	case LED_PWRS_DISCHARGE:
		if (charge_get_percent() <= BATTERY_LEVEL_CRITICAL &&
		    !batt_cri_triggered) {
			/* Flashing amber LED quickly if battery is at critical
			 * level */
			batt_cri_triggered = 1;
			batt_low_triggered = 0;
			BATT_PWM_LED_CONFIG_TICK(BATT_CRI_LED_PULSE_TICK_MS,
						 BATT_CRI_LED_PULSE_MS,
						 LED_AMBER);
			hook_call_deferred(&batt_pwm_led_tick_data, 0);
			   ...
			   !batt_low_triggered) {
			/*
			 * Flashing amber LED slowly if battery is lower than 8%
			 * while not at critical level
			 */
			batt_low_triggered = 1;
			batt_cri_triggered = 0;
			BATT_PWM_LED_CONFIG_TICK(BATT_LOW_LED_PULSE_TICK_MS,
						 BATT_LOW_LED_PULSE_MS,
						 LED_AMBER);
			hook_call_deferred(&batt_pwm_led_tick_data, 0);
		} else {
			batt_cri_triggered = 0;
