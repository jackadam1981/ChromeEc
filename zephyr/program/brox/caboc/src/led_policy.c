...
static const struct board_led_pwm_dt_channel bat_led_white =
	BOARD_LED_PWM_DT_CHANNEL_INITIALIZER(DT_NODELABEL(pwm_battery_led_white));
static const struct board_led_pwm_dt_channel bat_led_amber =
	BOARD_LED_PWM_DT_CHANNEL_INITIALIZER(DT_NODELABEL(pwm_battery_led_amber));
...
int led_set_brightness(enum ec_led_id led_id, const uint8_t *brightness)
{
	...
	case EC_LED_ID_BATTERY_LED:
		if (brightness[EC_LED_COLOR_WHITE] != 0)
			led_set_color_battery(LED_WHITE, brightness[EC_LED_COLOR_WHITE]);
		else if (brightness[EC_LED_COLOR_AMBER] != 0)
			led_set_color_battery(LED_AMBER, brightness[EC_LED_COLOR_AMBER]);
		else
			led_set_color_battery(LED_OFF, 0);
		break;
	case EC_LED_ID_POWER_LED:
		if (brightness[EC_LED_COLOR_WHITE] != 0)
			led_set_color_power(LED_WHITE, brightness[EC_LED_COLOR_WHITE]);
		else
...
static void led_set_battery(void)
{
	...
	case LED_PWRS_DISCHARGE:
		/* Blinking amber LED slowly if battery is lower than 10% */
		if (charge_get_percent() < BATT_LOW_BCT)
			led_set_color_battery((battery_ticks % LED_TICKS_PER_CYCLE <
						LED_ON_TICKS) ?
							LED_AMBER :
							LED_OFF, 100);
		else
	...
	case LED_PWRS_ERROR:
		...
		led_set_color_battery((battery_ticks & 0x1) ?
						LED_AMBER :
						LED_OFF, 100);
		break;
