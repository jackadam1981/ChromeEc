...
void base_batt_soc_setting(void)
{
	...
	if( (curr_batt_lvl < 20) && !extpower_is_present()) {
        gpio_pin_set_dt(GPIO_DT_FROM_NODELABEL(gpio_en_ppvar_base_x), false);
    }
    else if(attached == true) {
        gpio_pin_set_dt(GPIO_DT_FROM_NODELABEL(gpio_en_ppvar_base_x), true);
    }
}
