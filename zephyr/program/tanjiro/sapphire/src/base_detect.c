...
#define ATTACH_MAX_THRESHOLD_MV 2000
#define ATTACH_MIN_THRESHOLD_MV 500
...
static void base_batt_soc_setting(void)
{
	...
	int current_gpio_level =
		gpio_pin_get_dt(GPIO_DT_FROM_NODELABEL(gpio_kb_charge_blocked));

	if (!attached || debouncing) {
		if (!current_gpio_level)
			gpio_pin_set_dt(
				GPIO_DT_FROM_NODELABEL(gpio_en_ppvar_base_x),
				false);
		return;
	}
	...
	if (current_gpio_level != target_level) {
		gpio_pin_set_dt(GPIO_DT_FROM_NODELABEL(gpio_kb_charge_blocked),
				target_level);
	}
}
