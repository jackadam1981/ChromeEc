...
static int slp_event_init(void)
{
	 ...
	 * The timing of getting the initial state is critical for enabling the SHI host command
	 * driver. Using k_work_submit(&slp_event_work) defers SHI enable to the lowest-priority
	 * system workqueue thread, which is too late and may cause three consecutive retry errors
	 * for the host command 0x011d. See b:427468960#110 for details.
	 */
