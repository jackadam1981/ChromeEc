...
{
	/* PLL is enabled in DTS to configure it by the STM clock driver. ... speed up CPU, switch sysclk*/
	LL_RCC_PLL1_Disable();
	...
	/* Console. */
	// TODO add second clock to uart dts? STM32_UART_DOMAIN_CLOCK_SUPPORT. To get a proper source clock
	// Current version will work only for APB prescalers = 1 and SYSCLK = HSI
//	LL_RCC_SetUSARTClockSource(LL_RCC_USART16_CLKSOURCE_HSI);
	...
	// Change to #define USART16_SEL(val)	STM32_CLOCK(val, 7, 3, D2CCIP2R_REG)
