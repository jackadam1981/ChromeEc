...
static void ts_wot_lid_change(void)
{
	bool lid_open = lid_is_open();

	LOG_INF("WOT: lid change, open=%d", lid_open);

	if (!lid_open) {
		/* Lid Closed -> Enter WOT sleep mode */
...
DECLARE_HOOK(HOOK_LID_CHANGE, ts_wot_lid_change, HOOK_PRIO_DEFAULT);
