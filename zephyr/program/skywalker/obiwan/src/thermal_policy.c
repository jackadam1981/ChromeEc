...
static uint8 pre_battery_cells = BATT_3_CELL;
static uint8 battery_cells = BATT_3_CELL;
static BOOL check_charger_state = false;
static uint8 retry_charger_cyc = 0;
...
void battery_policy(void)
{
	...
	if (strcasecmp(bs->model_ext, "l25d2pk6") ||
		...
		if (pre_battery_cells != BATT_3_CELL) {
			pre_battery_cells = BATT_3_CELL check_charger_state =
				true;
			retry_charger_cyc = 0;
		}
	} else {
		if (pre_battery_cells != BATT_2_CELL) {
			pre_battery_cells = BATT_2_CELL check_charger_state =
				true;
			retry_charger_cyc = 0;
		}
	}
