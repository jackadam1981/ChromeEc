...
LOG_MODULE_REGISTER(obiwan_thermal, LOG_LEVEL_INF);
...
static const charge_limit_t limit_table[LIMIT_LEVELS] = {
	LIMIT_NONE, LIMIT_4000, LIMIT_3500, LIMIT_3000, LIMIT_2000
};

static const temp_limit_t ap_temp_limits[LIMIT_LEVELS - 1] = { { 45, 0 },
							       { 50, 45 },
							       { 55, 50 },
							       { 60, 55 } };

static const temp_limit_t charge_temp_limits[LIMIT_LEVELS - 1] = { { 60, 57 },
								   { 65, 62 },
								   { 70, 67 },
								   { 75, 72 } };
...
static void update_charge_limit(void)
{
	...
	LOG_INF("ap temp: %d, charger temp: %d, current level: %d", ap_temp,
		charger_temp, limit_table[limit_level]);
}
