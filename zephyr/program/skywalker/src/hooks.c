...
/* USB-A */
static void skywalker_usba_set_mode(enum usb_charge_mode mode)
{
...
{
	skywalker_usba_set_mode(USB_CHARGE_MODE_ENABLED);
}
...
void xhci_interrupt(enum gpio_signal signal)
{
	...
	if (xhci_stat)
		...
		skywalker_usba_set_mode(USB_CHARGE_MODE_DISABLED);
#endif
