...
void skywalker_dp_attention(int port, uint32_t vdo_dp_status)
{
...
void skywalker_set_unattached(int port)
{
	svdm_set_hpd_gpio(port, 0);
}

static void skywalker_conn_status(int port, const union connector_status_t* conn_status) {
	if (port != 0) {
		return;
	}
	...
	gpio_pin_set_dt(GPIO_DT_FROM_NODELABEL(gpio_en_usb_c0_dev_mode), is_attached && is_ufp);
}

static void skywalker_pdc_cb_init(void)
{
...
DECLARE_HOOK(HOOK_INIT, skywalker_pdc_cb_init, HOOK_PRIO_LAST);
