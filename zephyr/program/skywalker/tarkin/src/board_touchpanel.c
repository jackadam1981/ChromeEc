...
static void board_tp_resume(void)
{
	...
	hook_call_deferred(&set_tp_rst_pin_data, 8 * USEC_PER_MSEC);
	hook_call_deferred(&set_tp_en_pin_data, 55 * USEC_PER_MSEC);
...
static void board_tp_shutdown(void)
{
	...
	hook_call_deferred(&set_tp_rst_pin_data, 3 * USEC_PER_MSEC);
	hook_call_deferred(&set_tp_vtsp_en_pin_data, 3 * USEC_PER_MSEC);
}
DECLARE_HOOK(HOOK_CHIPSET_SUSPEND, board_tp_shutdown, HOOK_PRIO_DEFAULT);
