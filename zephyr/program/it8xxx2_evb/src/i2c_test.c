...
static int test_i2c_host(void)
{
	...
	if (i2c_write(i2c_dev, w_datas1, sizeof(w_datas1), 0x21)) {
		printf("Fail to write\n");
		return -1;
	}
	...
	int ret2 = i2c_write(i2c_dev, w_datas1, sizeof(w_datas1), 0x21);
	if (ret2) {
		printf("Fail to write = ret=%d\n",ret2);
		return -1;
	}

	ret2 = i2c_read(i2c_dev, r_datas, sizeof(r_datas), 0x21);
	if (ret2) {
	...
	if (i2c_burst_read(i2c_dev, 0x21, 0xaa, r_datas1, sizeof(r_datas1))) {
		printf("Fail to read\n");
		return -1;
	}
	//printf("axis raw data: %x %x %x %x %x %x\n",
