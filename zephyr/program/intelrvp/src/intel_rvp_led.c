...
static void update_charger_led(enum pwm_led_id id)
{
	 ...
	 */
	if (chg_st == LED_PWR_STATE_CHARGE ||
	    chg_st == LED_PWR_STATE_CHARGE_NEAR_FULL) {
		/* Charging: LED ON */
		rvp_led[id].led_is_pulsing = false;
		set_pwm_led_color(id, EC_LED_COLOR_GREEN);
	} else if (chg_st == LED_PWR_STATE_DISCHARGE ||
		   chg_st == LED_PWR_STATE_DISCHARGE_FULL) {
	...
	} else if (chg_st == LED_PWR_STATE_ERROR) {
		/* Charging error:
