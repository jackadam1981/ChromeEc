...
static uint16_t scancode_set2[KEYBOARD_COLS_MAX][KEYBOARD_ROWS] = {
	...
	{ 0x0037, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000 }, //16
	{ 0x0000, 0x0000, 0x006a, 0x0000, 0x0000, 0x0000, 0x005d, 0x0061 }, //17
};
...
uint16_t get_scancode_set2(uint8_t row, uint8_t col)
{
	...
	if (col < KEYBOARD_COLS_MAX && row < KEYBOARD_ROWS)
		ret = scancode_set2[col][row];
	ccprintf("****get R%d:C%d=0x%x\n", row, col, ret);
	return ret;
}
