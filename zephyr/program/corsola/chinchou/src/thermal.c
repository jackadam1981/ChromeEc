...
#define CHARGING_CURRENT_MA_SAFE 3000
#define COL_NUM 5
#define ROW_NUM 2
...
static void initializearray(int arr[][COL_NUM], int row, int exceptrow,
			    int exceptcol)
{
	int i, j;
...
static void average_tempature(void)
{
	...
	if (thermals[4]) {
		if (chipset_state != POWER_S0) {
			if (charger_temp_ave >= 60 &&
			    temp_zone != TEMP_OUT_OF_RANGE)
				initializearray(time, ROW_NUM, 0, 4);
			if (charger_temp_ave < 55 && temp_zone != TEMP_ZONE_0)
	...
	CPRINTS("---temp_zone : %d, ave_bef : %d, charger_temp_ave : %d---",
		temp_zone, charger_temp_ave_bef, charger_temp_ave);
}
