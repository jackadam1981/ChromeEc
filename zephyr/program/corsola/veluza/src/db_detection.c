...
test_export_static void board_x_ec_gpio2_init(void)
{
	...
	/*
	 * If a HDMI DB is attached, C0 port tasks will be exiting in that
	 ...
	 * count by 1 for usb_mux to access the C0 virtual mux for notifying
	 * mainlink direction.
...
{
	if (veluza_get_db_type() == VELUZA_DB_HDMI && port == USBC_PORT_C0) {
