...
#define BATT_LOW_LED_PULSE_US (875 * MSEC)
#define BATT_CRI_LED_PULSE_US (375 * MSEC)
#define BATT_LED_ON_TIME_US (125 * MSEC)
#define BATT_LED_PULSE_TICK_US (25 * MSEC)

#define BATT_LOW_LED_CONFIG_TICK(interval, color)                        \
	batt_led_config_tick((interval),                                 \
			     100 / (BATT_LOW_LED_PULSE_US / (interval)), \
			     (color), (BATT_LED_ON_TIME_US))

#define BATT_CRI_LED_CONFIG_TICK(interval, color)                        \
	batt_led_config_tick((interval),                                 \
			     100 / (BATT_CRI_LED_PULSE_US / (interval)), \
			     (color), (BATT_LED_ON_TIME_US))
	...
	case LED_PWRS_DISCHARGE:
		if (led_auto_control_is_enabled(EC_LED_ID_BATTERY_LED)) {
			if (charge_get_percent() <= BATTERY_LEVEL_CRITICAL &&
				...
				BATT_CRI_LED_CONFIG_TICK(BATT_LED_PULSE_TICK_US,
							 LED_AMBER);
				hook_call_deferred(
					&battery_set_pwm_led_tick_data, 0);
			} else if (charge_get_percent() <= 99 &&
				   charge_get_percent() >
					   BATTERY_LEVEL_CRITICAL &&
				   !battery_low_triggeied) {
				battery_critical_triggeied = 0;
				battery_low_triggeied = 1;
				BATT_LOW_LED_CONFIG_TICK(BATT_LED_PULSE_TICK_US,
							 LED_AMBER);
				hook_call_deferred(
					&battery_set_pwm_led_tick_data, 0);
