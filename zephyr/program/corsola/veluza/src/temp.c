...
static void current_update(void)
{
	...
	if (temp < current_table[current_level].temperature) {
		/* Decrease level */
		uptime = 0;
		if (dntime < KEEP_TIME) {
			dntime++;
			...
			dntime = 0;
			current_level--;
			ccprints("level = %d. avg_temp=%d", current_level,
				 temp);
		}
	} else if ((current_level + 1) < CURRENT_LEVELS) {
		if (temp >= current_table[current_level + 1].temperature) {
			/* Increase level */
			dntime = 0;
			if (uptime < KEEP_TIME) {
				uptime++;
