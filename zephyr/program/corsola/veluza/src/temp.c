...
static void average_tempature(void)
{
	...
	for (enum temperature_sensor_type type = 0; type < NUM_THERMISTOR_TYPES;
		...
		case I2C_CHARGER:
			...
			break;
		case ADC_5V:
			/* TODO: need to modfiy lable name temp_adc5v */
			temp_sensor_read(
				TEMP_SENSOR_ID(DT_NODELABEL(temp_charger)), &t);
			break;
		case ADC_AMB:
			/* TODO: need to modfiy lable name temp_adcambient */
			temp_sensor_read(
				TEMP_SENSOR_ID(DT_NODELABEL(temp_charger)), &t);
			break;
		default:
...
#define AMBIENT_TEMP_THRESHOLD 40
...
static void safety_protection(void)
{
	...
	if (!chipset_in_state(CHIPSET_STATE_ON)
		...
		if (delay_time) {
			delay_time--;
		} else if (avg_temp[ADC_5V] <
			   safety_table[safety_level].release_temp) {
			/* Increase level */
			uptime = 0;
			if (dntime < KEEP_TIME) {
				dntime++;
