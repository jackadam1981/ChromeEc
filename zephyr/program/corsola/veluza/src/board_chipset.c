...
#define FORM_FACTOR_UNKNOWN 0xffffffff
...
static uint32_t form_factor = FORM_FACTOR_UNKNOWN;

static void ap_bl_en_init(void)
{
	/*
	 * Check the board version and form factor to control panel sequence.
	 */
	...
	if (form_factor == FORM_FACTOR_UNKNOWN || IS_ENABLED(CONFIG_TEST)) {
		if (cros_cbi_get_fw_config(FW_FORM_FACTOR, &form_factor) !=
		    EC_SUCCESS) {
			LOG_ERR("Failed to get form form_factor.");
			form_factor = 0;
		}
	}

	if (board_version > 1 && form_factor == FW_FORM_FACTOR_CLAMSHELL)
...
static int install_backlight_handler(void)
{
	...
	/*
	 * Add a callback for suspend/resume to control the panel
	 * backlight and touch enable gpio pin.
	 */
