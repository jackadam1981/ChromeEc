...
static volatile int pen_chg_time;
static volatile int pen_stp_time;
static volatile int pen_err_time;

#define CHG_TIME 43200 /* 12 hours */
#define STP_TIME 1 /* 1 second */
#define ERR_TIME 600 /* 10 minutes */

static void pen_charge(void)
{
	if (pen_err_time > 0) {
...
void pen_fault_interrupt(enum gpio_signal signal)
{
	if (signal == GPIO_SIGNAL(DT_NODELABEL(pen_fault_od))) {
		/* Set error timer and disable pen charge */
