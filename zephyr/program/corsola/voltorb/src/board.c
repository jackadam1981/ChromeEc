...
{
	int status;
	uint64_t wait_timeout = get_time().val + BATTERY_STABLE_TIMEOUT;

	while (get_time().val < wait_timeout) {
		/* Starting pinging battery */
		battery_status(&status);

		if ((status & STATUS_TERMINATE_DISCHARGE_ALARM) &&
			(status & STATUS_TERMINATE_CHARGE_ALARM)) {
			msleep(25); /* clock stretching could hold 25ms */
			continue;
		}

		CPRINTS("battery stable status %x", status);
		return EC_SUCCESS;
	}

	CPRINTS("battery wait stable timeout %x", status);
