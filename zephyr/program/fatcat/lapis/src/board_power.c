...
void board_ap_power_force_shutdown(void)
{
	...
	gpio_pin_set_dt(GPIO_DT_FROM_NODELABEL(gpio_ec_vsus_on_ec), 0);
...
static int board_ap_power_action_g3_run(void *data)
{
	...
	if (ap_pwrseq_sm_is_event_set(data, AP_PWRSEQ_EVENT_POWER_STARTUP)) {
		...
		gpio_pin_set_dt(GPIO_DT_FROM_NODELABEL(gpio_ec_vsus_on_ec), 1);
		printk("--gpio_ec_vsus_on_ec 1\n");
...
static int board_ap_power_action_s3_run(void *data)
{
	...
	if (power_signal_get(PWR_RSMRST_PWRGD) == 0 ||
		...
		// gpio_pin_set_dt(GPIO_DT_FROM_NODELABEL(gpio_ec_cpu_vr_en), 0);
		gpio_pin_set_dt(GPIO_DT_FROM_NODELABEL(gpio_ec_susb_ec), 0);
		gpio_pin_set_dt(GPIO_DT_FROM_NODELABEL(gpio_ec_vr_on), 0);
		gpio_pin_set_dt(GPIO_DT_FROM_NODELABEL(gpio_ec_ssd_pln), 0);
		// gpio_pin_set_dt(GPIO_DT_FROM_NODELABEL(gpio_ec_slp_sx_n), 0);
		return 0;
	}
	...
	if (!power_wait_signals_on_timeout(
		...
		// k_msleep(2);
		gpio_pin_set_dt(GPIO_DT_FROM_NODELABEL(gpio_ec_susb_ec), 1);
		gpio_pin_set_dt(GPIO_DT_FROM_NODELABEL(gpio_ec_vr_on), 1);
		gpio_pin_set_dt(GPIO_DT_FROM_NODELABEL(gpio_ec_ssd_pln), 1);
