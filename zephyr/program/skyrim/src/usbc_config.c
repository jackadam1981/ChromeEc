...
static void reset_nct38xx_port(int port)
{
	...
	if (power_get_state() == POWER_S0) {
		 ...
		 * powered in S0.
		 */
		rv = usb_charge_set_mode(USBC_PORT_C0, USB_CHARGE_MODE_ENABLED,
					 USB_ALLOW_SUSPEND_CHARGE);
		if (rv)
			CPRINTSUSB("S0 TCPC enable failure on port 0(%d)", rv);

		rv = usb_charge_set_mode(USBC_PORT_C1, USB_CHARGE_MODE_ENABLED,
					 USB_ALLOW_SUSPEND_CHARGE);
		if (rv)
			CPRINTSUSB("S0 TCPC enable failure on port 1(%d)", rv);
	}
}
