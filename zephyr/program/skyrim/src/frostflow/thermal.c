...
int fan_table_to_rpm(int fan, int *temp)
{
	...
	/*
	 * The Frostflow fan table only needs to check the amb-pct2075 sensor
	 * temps. So fixed the index at 0.
	 */
	int amb_fantable_index = 0;
	...
	if (temp[TEMP_AMB] < prev_tmp[TEMP_AMB]) {
		for (i = current_level; i > 0; i--) {
			if (temp[TEMP_AMB] <
			    fan_step_table[i].off[amb_fantable_index]) {
				current_level = i - 1;
				break;
			}
		}
		...
		for (i = current_level; i < ARRAY_SIZE(fan_step_table); i++) {
			if (temp[TEMP_AMB] >
			    fan_step_table[i].on[amb_fantable_index]) {
				current_level = i + 1;
				break;
			}
		}
