...
#define TEMP_CPU TEMP_SENSOR_ID(DT_NODELABEL(temp_soc))
...
int fan_table_to_rpm(int fan, int *temp)
{
	...
	if (temp[TEMP_CPU] < prev_tmp[TEMP_CPU] ||
	    temp[TEMP_AMB] < prev_tmp[TEMP_AMB]) {
		for (i = current_level; i > 0; i--) {
			if (temp[TEMP_CPU] < fan_step_table[i].off[TEMP_CPU] &&
			    temp[TEMP_AMB] < fan_step_table[i].off[TEMP_AMB]) {
				current_level = i - 1;
				break;
			}
		}
		...
		for (i = current_level; i < ARRAY_SIZE(fan_step_table); i++) {
			if (temp[TEMP_CPU] > fan_step_table[i].on[TEMP_CPU] ||
			    temp[TEMP_AMB] > fan_step_table[i].on[TEMP_AMB]) {
				current_level = i + 1;
				break;
			}
		}
