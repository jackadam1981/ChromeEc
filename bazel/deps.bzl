...
def _ec_deps_impl(module_ctx):
    ...
    deps_file = "chromiumos/src/platform/ec/util/coreboot_sdk_portage_deps.py"
    script_output = module_ctx.execute([deps_file], working_directory = "../../../../../../")

    deps = {}

    # Remove the curly braces and split by commas
    dict_string = script_output.stdout.strip("{}")
    if dict_string:
        ...
        for item in items:
            ...
            version, hash = value.split("', '")
            deps[key] = (version, hash)

    # Use the data from portage_deps.bzl
    for arch, (version, hash) in deps.items():
        _coreboot_sdk_subtool(arch, "%s/%s" % (version, hash), "")
    print(deps)
