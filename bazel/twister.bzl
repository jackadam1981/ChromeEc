...
# bazel_skylib's shell.quote only does a single quote
def _double_quote(s):
    return "\"" + s.replace("'", "'\\''") + "\""
...
def _impl(ctx):
    ...
    twister_test_script = _gen_bash_snippet(["cp", "-L", "-r", build_dir.short_path, test_dir_path], test_env)
    twister_test_script += _gen_bash_snippet([ctx.executable._twister_bin.short_path, "--test-only"] + ctx.attr.args, {})
    ...
    # Add build only argument to twister because we want to only build first.
    args += ["-b"]
