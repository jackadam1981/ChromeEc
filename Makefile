...
rw-objs_from_dir=$(sort $(foreach obj, $($(2)-y), \
	        $(out)/RW/$(1)/$(firstword $($(2)-mock-$(PROJECT)-$(obj)) $(obj))))
...
all-rw-y=$(call rw-objs_from_dir,core/$(CORE),core)
all-rw-y+=$(call rw-objs_from_dir,chip/$(CHIP),chip)
all-rw-y+=$(call rw-objs_from_dir,board/$(BOARD),board)
all-rw-y+=$(call rw-objs_from_dir,private,private)
all-rw-y+=$(call rw-objs_from_dir,common,common)
all-rw-y+=$(call rw-objs_from_dir,driver,driver)
all-rw-y+=$(call rw-objs_from_dir,power,power)
all-rw-y+=$(call rw-objs_from_dir,test,$(PROJECT))
...
SHOBJLIBFILES = $(def_libsharedobjs_deps) -Wl,-T,common/ec.$(SHOBJLIB).ld
