...
fp-private:=../ec-private
ifneq ($(wildcard $(fp-private)/build.mk),)
HAVE_PRIVATE:=y
...
-include $(fp-private)/task_list_flags.mk
...
cmd_get_configs = $(CPP) $(foreach BLD,$(1),$(CPPFLAGS)) -P -dM \
	...
	$(if $(HAVE_PRIVATE), -I$(fp-private)) include/config.h | \
...
ifneq ($(CONFIG_DFU_BOOTMANAGER_MAIN),ro)
	_irq_list:=$(call shell_echo,$(CPP) $(CPPFLAGS) -P -Ichip/$(CHIP) \
		-I$(BASEDIR) -I$(BDIR) $(if $(HAVE_PRIVATE),-I$(fp-private)) \
...
cmd_config_eval = echo "$(1)" | $(CPP) $(CPPFLAGS) -P \
	-Ichip/$(CHIP) -I$(BASEDIR) -I$(BDIR) \
	$(if $(HAVE_PRIVATE), -I$(fp-private)) -imacros include/config.h -
...
includes-y+=$(call objs_from_dir_p,$(fp-private),private-incs,y)
...
dirs-y+=$(call objs_from_dir_p,$(fp-private),private-dirs,y)
...
all-obj-$(1)+=$(call objs_from_dir_p,$(fp-private),private,$(1))
ifneq ($(PDIR),)
