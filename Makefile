...
# Directory where the board is configured
BDIR:=$(patsubst %/,%,$(dir $(wildcard board/$(BOARD) private*/board/$(BOARD))))
...
ifeq "$(TEST_BUILD)" "y"
	...
	_tsk_lst:=$(shell echo "CONFIG_TASK_LIST" | $(CPP) -P \
		    -I$(BDIR)/$(BOARD) -D"TASK_NOTEST(n, r, d, s)=n" \
		    -D"TASK_ALWAYS(n, r, d, s)=n" -imacros $(_tsk_lst_file))
endif
_tsk_cfg:=$(foreach t,$(_tsk_lst) ,HAS_TASK_$(t))
CPPFLAGS+=$(foreach t,$(_tsk_cfg),-D$(t))
_flag_cfg:=$(shell $(CPP) $(CPPFLAGS) -P -dM -Ichip/$(CHIP) -I$(BDIR)/$(BOARD) \
...
ifneq "$(CONFIG_COMMON_RUNTIME)" "y"
	_irq_list:=$(shell $(CPP) $(CPPFLAGS) -P -Ichip/$(CHIP) -I$(BDIR)/$(BOARD) \
		-D"ENABLE_IRQ(x)=EN_IRQ x" -imacros chip/$(CHIP)/registers.h \
		$(BDIR)/$(BOARD)/ec.irqlist | grep "EN_IRQ .*" | cut -c8-)
...
_rw_off_str:=$(shell echo "CONFIG_RW_MEM_OFF" | $(CPP) $(CPPFLAGS) -P \
		-Ichip/$(CHIP) -I$(BDIR)/$(BOARD) -imacros include/config.h)
...
all-ro-y+=$(call ro-objs_from_dir,$(BDIR)/$(BOARD),board)
...
dirs=core/$(CORE) chip/$(CHIP) $(BDIR)/$(BOARD) common power test
